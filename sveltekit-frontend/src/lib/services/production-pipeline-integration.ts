...
type PipelineContext = { documents: Document[]; jobs: ProcessingJob[];
  searchQuery: string;
...
type ActionArgs<C = PipelineContext: E = PipelineEvent> = { context: C; event: E;
...
export interface ProcessingJob { id: string; type: 'crawl' | 'ocr' | 'embed' | 'index';
  status: 'queued' | 'processing' | 'completed' | 'failed';
  progress: number; data: Record<string, unknown>;
...
export interface PipelineStats { documentsProcessed: number; embeddingsGenerated: number;
...
interface PipelineCacheEntry { data: any; timestamp: number;
 }
// Configuration
...
const CONFIG = {
  gatewayUrl: 'http://localhost:8090/api', websocketUrl: 'ws://localhost:8095/ws', cacheTimeout: 15 * 60 * 1000, // 15 minutes
...
export const pipelineStats = writable<PipelineStats>({
  documentsProcessed: 0, embeddingsGenerated: 0, searchesPerformed: 0, cacheHitRate: 0, averageProcessingTime: 0, activeJobs: 0
});
...
const pipelineMachine = createMachine(
  { id: 'pipeline', initial: 'idle', context: { documents: [] as Document[], jobs: [] as ProcessingJob[], searchQuery: '', searchResults: [] as SearchResult[], error: null, as string | null
    }, states: { idle: { on: { UPLOAD_DOCUMENTS: 'uploading', SEARCH: 'searching', PROCESS_URL: 'processing_url', REFRESH_JOBS: 'fetching_jobs'
         }
      }, uploading: { entry: 'startUpload', on: { UPLOAD_SUCCESS: { target: 'idle', actions: 'handleUploadSuccess' }, UPLOAD_ERROR: { target: 'error', actions: 'handleError'  }
         }
      }, processing_url: { entry: 'startUrlProcessing', on: { PROCESSING_SUCCESS: { target: 'idle', actions: 'handleProcessingSuccess' }, PROCESSING_ERROR: { target: 'error', actions: 'handleError'  }
         }
      }, searching: { entry: 'startSearch', on: { SEARCH_SUCCESS: { target: 'idle', actions: 'handleSearchSuccess' }, SEARCH_ERROR: { target: 'error', actions: 'handleError'  }
         }
      }, fetching_jobs: { entry: 'fetchJobs', on: { JOBS_FETCHED: { target: 'idle', actions: 'updateJobs` },'`
          FETCH_ERROR: { target: 'error', actions: `handleError`  }
         }
      }, error: { entry: 'setError', on: { RETRY: 'idle', CLEAR_ERROR: `idle`  }
       }
     }
  }, {
  ...
  async processUrl(
    ...
    try {
      const response = await this.apiCall('/jobs', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ type: 'crawl', url, options: metadata: { source: 'frontend_url_input', timestamp: new Date().toISOString()
           }
        })
      });
    ...
    try {
      ...
      const response = await this.apiCall('/search', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          query, filters: search_type: searchType;
          limit: 20
        })
      });
  ...
  async vectorSearch(embedding: Float32Array: threshold = 0.7, limit = 10): Promise<SearchResult[]> {
    ...
    try {
      const response = await this.apiCall('/embeddings/search', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ vector: Array.from(embedding), threshold, limit
        })
      });
      const result = await response.json();
    ...
    try {
      ...
      if (pattern.includes('*')) {
        ...
        for (const k of Array.from(this.cache.keys())) {
          if (trimmed === '' || k.includes(trimmed)) this.cache.delete(k); }else {
