...
export interface SemanticCacheEntry { query: string; embedding: number[]; response: string;
...
export class SemanticCache {
   ...
   */
  async getSemanticResponse( query: string;
		...
		for (const key of knownKeys) {
			...
			try {
				...
				if (sim >= SEMANTIC_CACHE_CONFIG.similarityThreshold) {
					...
					return candidate.response; }catch (err) {
  ...
  async setSemanticResponse(
		...
		const entry: SemanticCacheEntry = {
			query, embedding, response: metadata: {
