<script lang="ts"> import { Dialog } from 'bits-ui'; import  Button  from "$lib/components/ui/button/Button.svelte"; import X from 'lucide-svelte'; import { goto } from '$app/navigation'; import { superForm } from 'sveltekit-superforms'; import { zod } from 'sveltekit-superforms/adapters'; import { loginSchema } from '$lib/schemas/auth'; import { toastStore } from '$lib/stores/toast'; interface Props { onlogin?: () => void; open?: boolean; }
  let { onlogin, open = $bindable() }: Props = $props(); const { form, errors, enhance, submitting, message } = superForm( { email: '', password: '', rememberMe: false }, {
    ...
    > <div class="flex items-center justify-between"> <Dialog.Title class="text-xl font-bold">Sign In</Dialog.Title> <Dialog.Close, asChild> <button class="p-1 hover:bg-slate-100"> <X class="w-5" /> </button> </Dialog.Close> </div> {#if $message} <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded text-red-700"> {$message} {/if} <form class="space-y-4" method="POST" action="/api/auth/login" use:enhance> <div> <label for="email" class="block text-sm font-medium text-slate-700">Email</label> <input id="email"
            type="email"
            name="email"
          ...
          /> {#if $errors.email} <p class="text-red-600 text-xs">{$errors.email}</p> {/if} </div> <div> <label for="password" class="block text-sm font-medium text-slate-700">Password</label> <input id="password"
            type="password"
            name="password"
            ...
            placeholder="••••••••"
          /> {#if $errors.password} <p class="text-red-600 text-xs">{$errors.password}</p> {/if} </div> <div class="flex items-center"> <label class="flex items-center"> <input type="checkbox"
              name="rememberMe"
          ...
          > {#if $submitting} Signing in... {:else} Sign In {/if} </Button> </div> </form> <div class="mt-4 text-center text-sm"> Don't have an account?{' '} <button type="button"'
