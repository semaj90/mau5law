...
<script lang="ts">
	...
	// Use a flexible type for items to avoid many repetitive unknown casts
	...
	let galleryStats = $derived(() => {
		return {
			...
			evidence: mediaItems.filter((it: GalleryItem) => it.category === 'evidence').length,
			images: mediaItems.filter((it: GalleryItem) => it.category === 'images').length,
			documents: mediaItems.filter((it: GalleryItem) => it.category === 'documents').length,
			aiGenerated: mediaItems.filter((it: GalleryItem) => it?.metadata?.aiGenerated).length,
		};
	});
	...
	let processedItems = $derived(() => {
		...
		if (selectedCategory !== 'all') {
			if (selectedCategory === 'ai-generated') {
				items = items.filter((it: GalleryItem) => it?.metadata?.aiGenerated);
			} else {
		...
		if (selectedCaseId !== 'all') {
			items = items.filter((it: GalleryItem) => it.caseId === selectedCaseId);
		}
		// Search filter
		if (searchQuery.trim()) {
			const query = searchQuery.toLowerCase();
			items = items.filter((it: GalleryItem) => {
				const title = (it.title || '').toString().toLowerCase();
				const description = (it.description || '').toString().toLowerCase();
				const caseTitle = (it.caseTitle || '').toString().toLowerCase();
	...
	// Initialize on mount (use window only in browser)
	onMount(() => {
		loadGalleryData();
		loadCases();
	...
	async function loadCases() {
		try {
			...
			if (response.ok) {
				const data = await response.json();
				availableCases = Array.isArray(data?.cases) ? data.cases : [];
			}
		} catch (err) {
	...
	function getItemPreview(item: GalleryItem): string {
		const it = item as GalleryItem;
		return it.fileUrl || it.imageUrl || it.thumbnailUrl || '/api/placeholder-image';
	}
	...
	function downloadItem(item: GalleryItem) {
		const it = item as GalleryItem;
		const url = it.fileUrl || it.imageUrl;
		if (!url) return;
		const a = document.createElement('a');
		a.href = url;
		a.download = it.title || `item-${it.id}`;
	...
	async function deleteItem(item: GalleryItem) {
		...
		try {
			const response = await fetch(`/api/gallery/${it.id}`, { method: 'DELETE' });
			if (response.ok) {
				mediaItems = mediaItems.filter((existing: GalleryItem) => existing.id !== it.id);
				selectedItem = null;
			} else {
	...
	function shareItem(item: GalleryItem) {
		const it = item as GalleryItem;
		const shareData = {
			title: it.title || 'Gallery Item',
			text: it.description || '',
	...
	// Helper type-check predicates (avoid 'as' casts in template)
	function isImageItem(it: GalleryItem): boolean {
		return it?.type === 'image' || it?.category === 'images';
	}
	function isVideoItem(it: GalleryItem): boolean {
		return it?.type === 'video';
	}
	function isAudioItem(it: GalleryItem): boolean {
	...
	<div class="gallery-controls nes-container is-rounded">
		<div class="controls-grid">
			...
			<div class="control-group">
				<label class="nes-text" for="search">Search:</label>
				<input id="search"
					type="text"
					class="nes-input"
						...
						<div class="item-preview" role="button" tabindex="0"
							...
							<div class="item-overlay">
								...
								<div class="overlay-actions">
									<button class="nes-btn is-small" on:click|stopPropagation={() => downloadItem(item)}>
										⬇️
									</button>
									<button class="nes-btn is-small" on:click|stopPropagation={() => shareItem(item)}>
										📤
									</button>
								</div>
							</div>
