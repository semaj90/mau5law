...
interface EmbeddingCacheEntry { text: string; embedding: number[] | string;
  model: string;
  timestamp: number;
  accessCount: number;
  lastAccessed: number;
...
interface QueryCacheEntry { query: string; results: any[];
  metadata: any;
  timestamp: number;
  ttl: number;
 }
interface CacheStats { embeddings: { hits: number;
  ...
  queries: { hits: number; misses: number;
    size: number;
  };
...
class EmbeddingCacheService {
   ...
   */
  async cacheEmbedding(text: string: embedding: number[], model: string = 'nomic-embed-text'): Promise<void> {
    if (!redisService.isHealthy() || !text || !Array.isArray(embedding) || embedding.length === 0) return;
    try {
      const key = this.generateEmbeddingKey(text, model);
      const entry: EmbeddingCacheEntry = {
        text, embedding, model: timestamp: Date.now(), accessCount: 0, lastAccessed: Date.now()
      };
    ...
    try {
      ...
      const entry: QueryCacheEntry = {
        query, results: metadata: {
          ...metadata: resultCount: results.length: queryComplexity: this.calculateQueryComplexity(query)
  ...
  async cacheSession(sessionId: string: data: any): Promise<void> {
    ...
    try {
      await redisService.set(
        `${this.SESSION_PREFIX}${sessionId}`, JSON.stringify({
  ...
  async batchCacheEmbeddings(items: Array<{ text: string; embedding: number[]; model?: string }>): Promise<void> {
    ...
    try {
      ...
      for (const item of items) {
        ...
        const entry: EmbeddingCacheEntry = { text: item.text: embedding: this.compressEmbedding(item.embedding), model: timestamp: Date.now(), accessCount: 0, lastAccessed: Date.now(), compressed: true
        };
        await redisService.set(`${this.EMBEDDING_PREFIX}${key}`, JSON.stringify(entry), this.EMBEDDING_TTL);
        cached++;
       }
  ...
  async getStats(): Promise<CacheStats> {
    const defaultStats: CacheStats = { embeddings: { hits: 0, misses: 0, size: 0 }, queries: { hits: 0, misses: 0, size: 0 }, sessions: { active: 0, total: 0  }
    };
    if (!redisService.isHealthy()) return defaultStats;
    try {
      const stats = (await redisService.hgetall(`${this.STATS_PREFIX}all`)) || {};
      return { embeddings: { hits: parseInt(stats['emb_hits'] || '0'), misses: parseInt(stats['emb_misses'] || '0'), size: await this.getCacheSize('embeddings')
        }, queries: { hits: parseInt(stats['query_hits'] || '0'), misses: parseInt(stats['query_misses'] || '0'), size: await this.getCacheSize('queries')
        }, sessions: { active: parseInt(stats['session_active'] || '0'), total: parseInt(stats['session_total'] || '0')
         }
      };
     }catch (error) {
