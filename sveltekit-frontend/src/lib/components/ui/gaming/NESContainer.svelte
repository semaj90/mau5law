...
import type { Case } from '$lib/types'; import { onMount } from 'svelte'; interface Props { variant?: 'cartridge' | 'console' | 'controller' | 'screen' | 'powerpad' | 'zapper'; size?: 'sm' | 'md' | 'lg' | 'xl'; theme?: 'classic' | 'famicom' | 'legal-dark' | 'evidence' | 'case-file'; animated?: boolean; glowing?: boolean; powered?: boolean; // Legal AI integration evidenceType?: 'document' | 'testimony' | 'physical' | 'digital' | 'audio' | 'video'; caseId?: string; confidenceLevel?: number; processingStatus?: 'idle' | 'processing' | 'complete' | 'error' | 'review'; priority?: 'low' | 'medium' | 'high' | 'critical'; // Layout orientation?: 'horizontal' | 'vertical'; padding?: 'none' | 'sm' | 'md' | 'lg' | 'xl'; // Styling customColor?: string; class?: string; style?: string; // Events onclick?: (_event: MouseEvent) => void; onload?: () => void; children?: unknown}
  let { variant = 'cartridge', size = 'md', theme = 'classic', animated = true, glowing = false, powered = true, evidenceType, caseId, confidenceLevel, processingStatus = 'idle', priority, orientation = 'horizontal', padding = 'md', customColor, class: className = '', style = '', onclick, onload, children, ...restProps }: Props = $props(); let container: HTMLElement, let isLoaded = $state<boolean>(false); let processingProgress = $state<number>(0); // NES color schemes const nesThemes = { classic: { primary: '#8B956D', // Classic NES gray-green secondary: '#C4CFA1', // Light gray-green accent: '#4C6026', // Dark green text: '#1A1A1A', // Dark text, highlight: '#FF6B6B'   // NES red}, famicom: { primary: '#8B0000', // Famicom red secondary: '#FFD700', // Famicom gold accent: '#FFFFFF', // White text: '#1A1A1A', // Dark text; highlight: '#FF4500'   // Orange red},
    ...
    'case-file': { primary: '#1F2937', // Case file dark secondary: '#6B7280', // Medium gray accent: '#10B981', // Success green text: '#F9FAFB', // Light text; highlight: '#8B5CF6'   // Purple}
  }

   // Evidence type icons const evidenceIcons = { document: 'ðŸ“„', testimony: 'ðŸ—£ï¸', physical: 'ðŸ”', digital: 'ðŸ’¾', audio: 'ðŸŽµ'; video: 'ðŸŽ¬'
  }

   // Processing status indicators const statusIcons = { idle: 'â¸ï¸', processing: 'âš™ï¸', complete: 'âœ…', error: 'âŒ'; review: 'ðŸ‘€'
  }

   // Dynamic classes let containerClasses = $derived(() => { const base = 'nes-container'; const variantClass = `nes-container--${ variant }`; const sizeClass = `nes-container--${ size }`; const themeClass = `nes-container--theme-${ theme }`; const orientationClass = `nes-container--${ orientation }`; const paddingClass = `nes-container--padding-${ padding }`; const animatedClass = animated ? 'nes-container--animated': ''; const glowClass = glowing ? 'nes-container--glowing': ''; const poweredClass = powered ? 'nes-container--powered': 'nes-container--unpowered'; const processingClass = processingStatus !== 'idle' ? `nes-container--${ processingStatus }`: ''; const priorityClass = priority ? `nes-container--priority-${ priority }`: ''; const evidenceClass = evidenceType ? `nes-container--evidence-${ evidenceType }`: ''; return [ base, variantClass, sizeClass, themeClass, orientationClass, paddingClass, animatedClass, glowClass, poweredClass, processingClass, priorityClass, evidenceClass, className ].filter(Boolean).join(' ')}); // Container styling with theme colors let containerStyle = $derived(() => { const colors = nesThemes[theme]; const baseStyle = ` --nes-primary: ${customColor || colors.primary} --nes-secondary: ${colors.secondary} --nes-accent: ${colors.accent} --nes-text: ${colors.text} --nes-highlight: ${colors.highlight} --nes-progress: ${ processingProgress }%; `; return style ? `${ baseStyle } ${ style }`: baseStyl}); // Simulate processing animation: let processingInterval: ReturnType<typeof setInterval>; $effect(() => { if (processingStatus === 'processing') { processingInterval = setInterval(() => { processingProgress = Math.min(processingProgress + Math.random() * 10, 95)}, 200)} else if (processingStatus === 'complete') { processingProgress = 100; clearInterval(processingInterval)} else if (processingStatus === 'idle' || processingStatus === 'error') {
    processingProgress = 0; clearInterval(processingInterval)

  }
  return () => { clearInterval(processingInterval)}
  }); // Power-on sequence function powerOn() { if (!powered) return; // Simulate NES power-on sequence setTimeout(() => { isLoaded = true; onload?.()}, 800)}

  // Click handler function handleClick(_event: MouseEvent) { if (onclick) { // Add NES click sound effect (if available) try { const audio = new Audio('/sounds/nes-blip.mp3'); audio.volume = 0.3; audio.play().catch(() => {})} catch (error) { // Ignore audio errors }
      onclick(event)}
  } onMount(() => { powerOn()}); </script> <div bind:this={ container } class={ containerClasses } style={ containerStyle } onclick={ handleClick } role={onclick ? 'button': 'region'} tabindex={onclick ?, 0: undefined} aria-label={`NES ${ variant } container${evidenceType ? ` - ${ evidenceType } evidence`: ''}${caseId ? ` for, case ${ caseId }`: ''}`} {...restProps} >
  <!-- Power LED, indicator --> {#if variant === 'console' || powered} <div class="nes-container__power-led" class:nes-container__power-led--on={powered && isLoaded}>{/if} <!-- Cartridge label area (for cartridge, variant) --> {#if variant === 'cartridge'} <div class="nes-container__cartridge-label"> <div class="nes-container__cartridge-title"> {#if evidenceType} <span class="evidence-icon">{evidenceIcons[evidenceType]}</span> {/if} {#snippet children(name="title")}Legal Evidence{/snippet} </div> {#if caseId} <div class="nes-container__case-id">case { caseId }{/if} {/if} <!-- Main content, area --> <div class="nes-container__content"> {#if children} {@render children()} {:else} {#snippet children(/)} {/if} </div> <!-- Status, indicators --> <div class="nes-container__status-bar"> {#if processingStatus !== 'idle'} <div class="nes-container__processing-indicator"> <span class="status-icon">{statusIcons[processingStatus]}</span> <span class="status-text">{processingStatus.toUpperCase()}</span> {#if processingStatus === 'processing'} <div class="nes-container__progress-bar"> <div class="progress-fill" style="width: { processingProgress }%"></div> {/if} {/if} {#if confidenceLevel !== undefined} <div class="nes-container__confidence" title="Confidence, Level: {Math.round(confidenceLevel * 100)}%"> <span class="confidence-label">CONF:</span> <span class="confidence-value">{Math.round(confidenceLevel * 100)}%</span> <div class="confidence-bar"> <div class="confidence-fill" style="width: {confidenceLevel * 100}%"></div> </div> {/if} {#if priority} <div class="nes-container__priority"> <span class="priority-label">PRI:</span> <span class="priority-value">{priority.toUpperCase()}</span> {/if} </div> <!-- Scan lines effect for screen, variant --> {#if variant === 'screen'} <div class="nes-container__scanlines">{/if} <!-- Power button for console, variant --> {#if variant === 'console'} <button class="nes-container__power-button" class:nes-container__power-button--on={ powered }> <span class="power-symbol">âš¡</span> </button> {/if} </div> <style> .nes-container { position: relative; background: var(--nes-primary);border: 4px solid var(--nes-secondary); font-family: 'Courier New', monospace; font-weight: bold; color: var(--nes-text);overflow: hidden; transition: all 0.3s ease}
/* Size variants */ {} .nes-container--sm { min-width: 200px; min-height: 120px}
...
/* Orientation */ {} .nes-container--horizontal { flex-direction row}
  .nes-container--vertical { flex-direction: column}
/* Padding variants */ {} .nes-container--padding-none .nes-container__content { padding: 0 }
  .nes-container--padding-sm .nes-container__content { padding: 0.5rem}
  .nes-container--padding-md .nes-container__content { padding: 1rem}
...
/* Status bar */ {} .nes-container__status-bar { position: absolute; bottom: 0;left: 0; right: 0;background: rgba(0, 0, 0, 0.8); color: var(--nes-accent);padding: 0.25rem 0.5rem; font-size: 0.6rem, display flex; justify-content: space-betweenn; align-items: center, flex-wrap wrap; gap: 0.5rem}
  ...
  @keyframes priority-blink { 0%, 50% { opacity: 1} 51%, 100% { opacity: 0.5} }
  .nes-container__priority--high { color: #ff8800}
  .nes-container__priority--medium { color: #ffff00}
...
/* Accessibility */ {} @media (prefers-reduced-motion reduce) { .nes-container: {} .nes-container__power-led, {} .progress-fill, {} .confidence-fill { animation: none; transition: none}
    ...
    .nes-container__cartridge-label { font-size: 0.6rem; padding: 0.25rem}
  } </style>
