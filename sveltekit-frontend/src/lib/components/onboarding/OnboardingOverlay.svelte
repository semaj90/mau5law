...
interface Props { open?: boolean; currentStep?: number; steps?: OnboardingStep[]; autoProgress?: boolean; progressDelay?: number; showMinimap?: boolean; allowSkip?: boolean; onclose?: (event?: unknown) => void; oncomplete?: (event?: unknown) => void}
 ...
 <div class="w-4">
  {#if autoProgress} <Button class="bits-btn w-4"
              variant="ghost"
              size="sm"
 ...
 <!-- Navigation --> <div class="w-4"> <div class="w-4">
  {#if allowSkip} <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={() => skipOnboarding()}> <SkipForward class="w-4" /> Skip Tour </Button> {/if}
  </div>
 <div class="w-4">
 ...
 <div class="w-4"> <Button.Root class="bits-btn"
            variant="ghost"
            size="sm"
            onclick={() => previousStep()} disabled={currentStep === 0} >
            <ArrowLeft class="w-4" /> Back </Button>
 <Button.Root class="bits-btn" onclick={() => nextStep()} size="sm">
