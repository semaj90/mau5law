...
export type DataType = "case" | "evidence" | "document";

/** Shared embedding vector type (GPU / Ollama compatible) */
export type EmbeddingVector = number[];

/** Common metadata type for all RAG objects */
...
/**
 * LegalDocument - used by GPU bridge and other processors.
 * Note: documentType set intentionally to a broader union used across the codebase.
 */
export type LegalDocumentType = 'contract' | 'evidence' | 'brief' | 'citation' | 'precedent' | DataType;
 ...
 * Keeps minimal fields required by the runtime logic.
 */
export interface CUDAMemoryRegion {
  ...
  createdAt?: number;
  // raw data held (optional) — can be ArrayBuffer / Uint8Array / Float32Array
 ...
 * Real runtime will use GPUTexture / GPUBuffer — these stubs allow tests & SSR.
 */
...
export interface TextureCacheEntry {
  textureId: string;
  texture?: GPUResourceStub;
  gpuBuffer?: GPUResourceStub;
  width?: number;
  height?: number;
  createdAt?: number;
}
