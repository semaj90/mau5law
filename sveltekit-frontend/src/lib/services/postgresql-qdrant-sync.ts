...
type DocumentEmbeddingItem = typeof documentEmbeddings.$inferSelect;
...
export class PostgreSQLQdrantSyncService {
  ...
  private async clearQdrantCollection(): Promise<void> {
    try {
      // Recreate collection (deletes if exists, then creates). This is the fastest way to clear.
      const collectionsResponse = await this.qdrant.getCollections(); // Changed from collections.get()
      const exists = collectionsResponse.collections.some((c: CollectionInfo) => c.name === this.config.collectionName);
      if (exists) {
   ...
   */
  private async getEvidenceWithEmbedding(evidenceId: string): Promise<EvidenceItem | undefined> {
    // Changed return type
   ...
   */
  private async getDocumentWithEmbedding(documentId: string): Promise<DocumentData | undefined> {
    // Changed return type
    const [result] = await db
      .select({
        embedding: documentEmbeddings,
   ...
   */
  private async syncSingleEvidence(evidenceItem: EvidenceItem): Promise<boolean> {
    // Changed type
    try {
  ...
  async searchForWASMInference({
    ...
    try {
      ...
      const cacheKey = this.generateCacheKey({ embedding: queryEmbedding, limit, filters }); // Changed
      ...
      const results: WASMVectorSearchResult[] = searchResponse // Added type
        .filter((hit: ScoredPoint) => hit.score >= scoreThreshold) // Typed hit as ScoredPoint
        .slice(0, limit)
        .map((hit: ScoredPoint) => ({
