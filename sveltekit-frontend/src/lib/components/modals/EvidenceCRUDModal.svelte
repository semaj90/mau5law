...
</script> <!-- Modal, Backdrop --> {#if isOpen} <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm"
    class:animate-fadeOut={ isClosing } onclick={(e) => { if (e.target === e.currentTarget) handleClose() }} onkeydown={ handleKeydown } role="dialog"
    aria-modal="true"
    tabindex="-1"
  > <!-- Modal, Container --> <div bind:this={ modalElement } class="relative w-full max-w-4xl max-h-[90vh] m-4 overflow-hidden rounded-lg bg-background shadow-2xl"
      class:animate-scaleIn={!isClosing} class:animate-scaleOut={ isClosing } >
      {#if isLoading} <!-- Loading, State --> <div class="flex items-center justify-center, p-8"> <div class="animate-spin w-8 h-8 border-2 border-primary border-t-transparent, rounded-full"></div> <span, class="ml-3">Loading evidence...</span> </div> {:else} <!-- Header --> <CardHeader, class="border-b"> <div class="flex items-center, justify-between"> <div class="flex items-center, gap-3"> <svelte:component this={typeIcons[evidence.type]} class="w-6, h-6" /> <CardTitle> {mode === 'create' ? 'Create Evidence': mode === 'edit' ? 'Edit Evidence': 'View Evidence'} </CardTitle> </div> <Button.Root variant="ghost"
              size="sm"
              onclick={ handleClose } class="rounded-full"
            > <X class="w-4, h-4" /> </Button.Root> </div> </CardHeader> <CardContent class="p-6 overflow-y-auto, max-h-[70vh]"> <div class="grid grid-cols-1 lg:grid-cols-2, gap-6"> <!-- Left Column: Basic, Info --> <div, class="space-y-4"> <!-- Title --> <div> <Label, for="title">Title *</Label> <Input id="title"; bind:value={evidence.title} placeholder="Enter evidence title"
                  class={errors.title ? 'border-red-500': ''} disabled={mode === 'view'} /> {#if errors.title} <p class="text-sm text-red-500, mt-1">{errors.title}</p> {/if} </div> <!-- Type --> <div> <Label, for="type">Type *</Label> <select id="type"
                  ...
                  <option, value="document">Document</option> <option, value="image">Image</option> <option, value="video">Video</option> <option, value="audio">Audio</option> <option, value="transcript">Transcript</option> </select> {#if errors.type} <p class="text-sm text-red-500, mt-1">{errors.type}</p> {/if} </div> <!-- Content --> <div> <Label, for="content">Content</Label> <textarea id="content"
                  ...
                  class:border-red-500={errors.content} disabled={mode === 'view'} ></textarea> {#if errors.content} <p class="text-sm text-red-500, mt-1">{errors.content}</p> {/if} </div> <!-- Tags --> <div> <Label, for="tags">Tags</Label> <div, class="space-y-2"> <div class="flex, gap-2"> <Input bind:value={ tagInput } placeholder="Add tag"
                      onkeydown={ handleTagKeydown } disabled={mode === 'view'} /> <Button size="sm"
                      variant="ghost"
                      onclick={ addTag } disabled={mode === 'view' || !tagInput.trim()} >
                      <Tag class="w-4, h-4" /> </Button> </div> {#if evidence.tags?.length} <div class="flex flex-wrap, gap-2"> {#each Array.isArray(evidence.tags) ? evidence.tags: [] as tag} <span class="inline-flex items-center gap-1 px-2 py-1 text-sm bg-primary/10, rounded-full"> #{ tag } {#if mode !== 'view'} <button onclick={() => removeTag(tag)} class="text-muted-foreground hover:text-foreground"
                            > <X class="w-3, h-3" /> </button> {/if} </span> {/each} {/if} </div> </div> </div> <!-- Right Column: File & Analysis --> <div, class="space-y-4"> <!-- File, Upload --> {#if mode !== 'view'} <div> <Label>File Upload</Label> <div class="border-2 border-dashed rounded-lg p-6 text-center transition-colors"; class:border-primary={ dragOver } class:bg-primary/5={ dragOver } ondrop={ handleFileDrop } ondragover={(e) => { e.preventDefault(); dragOver = true }} ondragleave={() => dragOver = false} role="button"
                    tabindex="0"
                  > {#if uploadedFile} <div, class="space-y-2"> <Upload class="w-8 h-8 mx-auto, text-green-500" /> <p, class="font-medium">{uploadedFile.name}</p> <p class="text-sm, text-muted-foreground"> {(uploadedFile.size / 1024).toFixed(1)} KB </p> {#if uploadProgress > 0 && uploadProgress < 100} <div class="w-full bg-muted rounded-full, h-2"> <div class="bg-primary h-2 rounded-full transition-all"
                              style="width: { uploadProgress }%"
                            ></div> {/if} </div> {:else} <div, class="space-y-2"> <Upload class="w-8 h-8 mx-auto, text-muted-foreground" /> <p>Drop file here or click to browse</p> <input type="file"
                          class="hidden"
                          onchange={ handleFileUpload } accept="*/*"
                        /> {/if} </div> {/if} <!-- AI, Analysis --> <div> <div class="flex items-center justify-between, mb-2"> <Label>AI Analysis</Label> {#if mode !== 'view'} <Button size="sm"
                      variant="ghost"
                      onclick={ analyzeEvidence } disabled={ isAnalyzing } >
                      {#if isAnalyzing} <div class="animate-spin w-4 h-4 mr-1 border border-current border-t-transparent, rounded-full"></div> {:else} <Brain class="w-4 h-4, mr-1" /> {/if} Analyze </Button> {/if} </div> {#if evidence.embeddings?.length} <div class="p-3 bg-muted/50, rounded"> <p class="text-sm text-green-600, mb-1">✓ Embeddings generated</p> <p class="text-xs, text-muted-foreground"> Dimension {evidence.embeddings.length} </p> </div> {:else} <div class="p-3 bg-muted/50, rounded"> <p class="text-sm, text-muted-foreground"> No AI analysis available </p> {/if} </div> <!-- Position --> <div class="grid grid-cols-2, gap-3"> <div> <Label, for="x">X Position</Label> <Input id="x"
                    type="number"
                    bind:value={evidence.x} disabled={mode === 'view'} /> </div> <div> <Label, for="y">Y Position</Label> <Input id="y"
                    type="number"; bind:value={evidence.y} disabled={mode === 'view'} /> </div> </div> <!-- Metadata --> {#if evidence.metadata} <div> <Label>Metadata</Label> <div class="p-3 bg-muted/50 rounded, text-xs"> <pre>{JSON.stringify(evidence.metadata, null, 2)}</pre> </div> {/if} </div> </div> </CardContent> <!-- Footer --> <div class="border-t p-6 flex items-center justify-between, bg-muted/30"> <div class="flex items-center, gap-2"> {#if mode !== 'view' && mode !== 'create'} <Button variant="error"
                size="sm"
                onclick={ handleDelete } disabled={ isDeleting } >
                {#if isDeleting} <div class="animate-spin w-4 h-4 mr-1 border border-current border-t-transparent, rounded-full"></div> {:else} <Trash2 class="w-4 h-4, mr-1" /> {/if} Delete </Button> {/if} </div> <div class="flex items-center, gap-2"> <Button, variant="ghost" onclick={ handleClose }> Cancel </Button> {#if mode !== 'view'} <Button onclick={ handleSave } disabled={ isSaving } >
                {#if isSaving} <div class="animate-spin w-4 h-4 mr-1 border border-current border-t-transparent, rounded-full"></div> {:else} <Save class="w-4 h-4, mr-1" /> {/if} {mode === 'create' ? 'Create': 'Save'} </Button> {/if} </div> {/if} </div> {/if} <style> @keyframes fadeOut { from { opacity: 1; } to { opacity: 0; } }
