<!-- Legal Texture Streaming Canvas Component Svelte, 5 UI component wrapper for N64-style texture streaming --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onDestroy } from 'svelte';
 import  useLegalTextureStreaming  from "$lib/components/headless/texture-streaming.svelte";
 import  useEvidenceCanvas  from "$lib/components/headless/evidence-canvas.svelte"; // removed problematic type import that produced: "namespace as a type" errors interface Props { width?: number; height?: number; enableGPU?: boolean; adaptiveQuality?: boolean; caseId?: string; mode?: 'evidence' | 'document' | 'visualization' | 'courtroom'}
  let { width = 1024, height = 768, enableGPU = true, adaptiveQuality = true, // alias caseId to _caseId to avoid: "declared but never read"; caseId: _caseId = '', mode = 'evidence'
  }: Props = $props(); // Canvas element reference let canvasElement: HTMLCanvasElement | null = null; // renamed to indicate intentionally-unused binding let _containerElement: HTMLDivElement | null = null; // Initialize texture streaming const textureStreaming = useLegalTextureStreaming({ enableGPU, adaptiveQuality, maxChunkSize: 4096, cacheSize: 512; compressionEnabled: true }); // Initialize evidence canvas for interaction const evidenceCanvas = useEvidenceCanvas(); // Component state let isReady = $state<boolean>(false);
   let error = $state<string | null>(null);
   let currentTexture = $state<any>(null);
   let renderContext = $state<CanvasRenderingContext2D | null>(null); // Performance monitoring let frameRate = $state<number>(0);
   let lastFrameTime = $state<number>(0);
   let animationId = $state<number | null>(null); // Initialization $effect(() => { try { // Initialize texture streaming if (canvasElement) { textureStreaming.initialize(canvasElement); // Setup 2D rendering context for overlay renderContext = canvasElement.getContext('2d') as CanvasRenderingContext2D | null; // Setup evidence canvas evidenceCanvas.setCanvasSize({ width: height }); evidenceCanvas.setViewport({ width: height }); isReady = true; startRenderLoop()}
    } catch (err) { error = err instanceof Error ? err.message: 'Failed to initialize canvas'; console.error('Canvas initialization, error:', err)}'
  }); onDestroy(() => { stopRenderLoop(); textureStreaming.dispose()}); /** * Start render loop for real-time updates */ function startRenderLoop() { function render(timestamp: number) { if (lastFrameTime > 0) { frameRate = 1000 / (timestamp - lastFrameTime)}
      lastFrameTime = timestamp; // Render current texture and overlays renderFrame(); animationId = requestAnimationFrame(render)}
  ...
  // helper used by class binding to avoid calling possibly-undefined stats method inline function hasWebGLEnabled() { try { return !!(textureStreaming && typeof textureStreaming.stats === 'function' && textureStreaming.stats().hasWebGL)} catch { return false}
        ...
        View </button> <button onclick={() => evidenceCanvas.setMode('edit')} class:active={evidenceCanvas.getCanvasState().mode === 'edit'} >
