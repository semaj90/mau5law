...
export const evidenceRelationships = pgTable('evidence_relationships', {
  id: uuid('id').primaryKey().defaultRandom(),
  fromEvidenceId: uuid('from_evidence_id').notNull().references(() => evidence.id, { onDelete: 'cascade' }),
  toEvidenceId: uuid('to_evidence_id').notNull().references(() => evidence.id, { onDelete: 'cascade' }),
  label: text('label'),
