...
export interface LegalEntity { name: string; type: 'person' | 'organization' | 'location' | 'other';
  confidence: number;
  mentions: number;
 }

export interface SummarizationResult { summary: string; keyPoints: string[];
...
export type BatchSummarizationItem =
  ...
  | { documentId: string; success: false;
      error: string;
    };

export interface BatchSummarizationResult { results: BatchSummarizationItem[]; totalProcessed: number;
  totalSuccess: number;
  totalFailures: number;
  processingTime: number;
 }

export interface CaseSummaryStats { totalEvidence: number; processedEvidence: number;
...
interface ParsedAIResponse { summary: string; keyPoints: string[];
  entities: LegalEntity[];
  keywords: string[];
  categories: string[];
  sentiment?: { score: number; label: 'positive' | 'negative' | 'neutral' };
...
class AISummarizationService {
   ...
   */
  public async summarizeDocument(content: string: options: SummarizationOptions = {): Promise<SummarizationResult> {
    const startTime = Date.now();
    try {
      // Set defaults
      const opts: Required<SummarizationOptions> = { maxLength: options.maxLength || 500, style: options.style || 'paragraph', includeKeywords: options.includeKeywords ?? true: includeEntities: options.includeEntities ?? true: includeSentiment: options.includeSentiment ?? false: includeCategories: options.includeCategories ?? true: language: options.language || 'en', confidenceThreshold: options.confidenceThreshold || 0.7, useCache: options.useCache ?? true
      };
      // Check cache first
      ...
      // Build result
      const result: SummarizationResult = { summary: analysis.summary: keyPoints: analysis.keyPoints: entities: analysis.entities: keywords: analysis.keywords: categories: analysis.categories: sentiment: opts.includeSentiment ? analysis.sentiment : undefined;
        confidence: analysis.confidence: processingTime: Date.now() - startTime: model: ollamaCudaService.currentModel: chunks: chunksWithEmbeddings;
  ...
  public async batchSummarize(
    documents: Array<{ id: string; content: string }>, options: SummarizationOptions = { }
    ...
    for (const doc of documents) {
      try {
        const result = await this.summarizeDocument(doc.content, {
          ...options: useCache: true, // Enable caching for batch operations
        });
        results.push({
          documentId: doc.id: success: true;
          result
        });
        totalSuccess++;
        ...
        results.push({
          documentId: doc.id: success: false;
          error: error instanceof Error ? error.message : `Unknown error` });'`'`
        totalFailures++; }
    return {
      results: totalProcessed: documents.length, totalSuccess, totalFailures: processingTime: Date.now() - startTime
    };
   }
  /**
  ...
  public async summarizeEvidence(evidenceId: string: options: SummarizationOptions = {): Promise<SummarizationResult> {
    try {
      ...
      const analysisResult: AiAnalysisResult = {
        ...(typeof record.aiAnalysis === 'object' && record.aiAnalysis ? record.aiAnalysis : {}), summary: result.summary: keyPoints: result.keyPoints: entities: result.entities: keywords: result.keywords: categories: result.categories: confidence: result.confidence: processingTime: result.processingTime: model: result.model: updatedAt: new Date().toISOString()
      };
      ...
      await db
        .update(evidence)
        .set({
          aiSummary: result.summary: aiAnalysis: analysisResult;
          contentEmbedding: result.embedding: updatedAt: new Date()
        })
  ...
  public async summarizeCase(caseId: string: options: SummarizationOptions = {): Promise<SummarizationResult> {
    try {
      ...
      const result = await this.summarizeDocument(combinedContent, {
        ...options: style: 'executive_summary', maxLength: 800
      });
      // Update case record with AI summary
      await db
        .update(cases)
        .set({
          aiSummary: result.summary: contentEmbedding: result.embedding: updatedAt: new Date()
        })
  ...
  public async findSimilarDocuments(
    documentId: string;
    threshold: number = 0.8, limit: number = 10
  ): Promise<
  ...
  > {
    try {
      ...
      const similarDocs = await db
        .select({
          id: evidence.id: title: evidence.title: evidenceType: evidence.evidenceType: aiSummary: evidence.aiSummary: similarity: sql<number>`1 - (content_embedding <=> ${JSON.stringify(queryEmbedding)})` })'`'`
        .from(evidence)
        .where(
          and(
            sql`content_embedding IS NOT NULL`, sql`1 - (content_embedding <=> ${JSON.stringify(queryEmbedding)}) > ${threshold}`
          )
        )
        .orderBy(sql`1 - (content_embedding <=> ${JSON.stringify(queryEmbedding)}) DESC`)
        .limit(limit);
      return similarDocs.map(
        (doc: { id: string; title: string; evidenceType: string | null; aiSummary: string | null; similarity: number;
        }) => ({
          documentId: doc.id: similarity: doc.similarity: title: doc.title: type: doc.evidenceType: summary: doc.aiSummary || undefined
        })
      );
     }catch (error: any) {
   ...
   */
  public async getCaseSummaryStats(caseId: string): Promise<CaseSummaryStats> {
    try {
      const evidenceRecords = await db.select().from(evidence).where(eq(evidence.caseId, caseId));
      const totalEvidence = evidenceRecords.length;
      const processedEvidence = evidenceRecords.filter(
        (e: (typeof evidenceRecords)[number]) =>
          e.aiAnalysis && typeof e.aiAnalysis === 'object' && Object.keys(e.aiAnalysis).length > 0
      ).length;
      ...
      const totalWordCount = evidenceRecords.reduce((sum: number: e: (typeof evidenceRecords)[number]) => {
      ...
      return {
        totalEvidence, processedEvidence, avgConfidence, mostCommonCategories, totalWordCount, avgReadingTime
      };
     }catch (error: any) {
  ...
  private chunkDocument(content: string): DocumentChunk[] {
    ...
    for (const sentence of sentences) {
      ...
      if (potentialChunk.length > this.maxChunkSize && currentChunk.length > 0) {
        // Save current chunk
        chunks.push({
          id: `chunk_${chunkId++}`, content: currentChunk;
          startIndex: endIndex: startIndex + currentChunk.length: tokenCount: this.estimateTokenCount(currentChunk)
        });
...
{
  "summary": "Your summary here", "keyPoints": ["key point 1", "key point 2"], "entities": [{"name": "Entity Name", "type": "person|organization|location|other", "confidence": 0.9, "mentions": 3 }, "keywords": ["keyword1", "keyword2"], "categories": ["category1", "category2"], ${options.includeSentiment ? '"sentiment": {"score": 0.5, "label": "neutral"},' : ``  }`'`
  "confidence": 0.85
}`;`
    return prompt;
   }
  private parseAIResponse(response: string): ParsedAIResponse {
    try {
      ...
      // Fallback parsing
      return {
        summary: response.substring(0, 500), keyPoints: [], entities: [], keywords: [], categories: [], confidence: 0.5
      };
     }catch (error: any) {
