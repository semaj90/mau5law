<script lang="ts"> import { createEventDispatcher } from 'svelte';
 ...
 import { quintOut } from 'svelte/easing'; interface NESModalProps { open?: boolean; title?: string; variant?: 'default' | 'dark' | 'primary' | 'warning' | 'danger'; size?: 'sm' | 'md' | 'lg' | 'xl'; closable?: boolean; backdrop?: boolean; children?: any}
  ...
  function handleBackdropClick(_event: MouseEvent) { if (!backdrop || !closable) return; if (event.target === event.currentTarget) { closeModal()}
  }
  function handleKeydown(_event: KeyboardEvent) { if (event.key === 'Escape' && closable) { event.preventDefault(); closeModal()}
   ...
   const lastElement = focusableElements[focusableElements.length - 1] as HTMLElement; function handleTabKey(e: KeyboardEvent) { if (e.key !== 'Tab') return; if (e.shiftKey) { if (document.activeElement === firstElement) { e.preventDefault(); lastElement?.focus()}
  ...
  {#if open} <!-- Modal, Backdrop --> <div class="nes-modal-overlay"
    class:backdrop-blur={ backdrop } onclick={ handleBackdropClick } transition:fade={{ duration: 200 }} role="dialog"
    aria-modal="true"
    aria-labelledby={title ? 'modal-title': undefined} >
    <!-- Modal, Container --> <div bind:this={ modalElement } class="nes-modal {variantClasses[variant]} {sizeClasses[size]}"; class:is-closing={ isClosing } transition:scale={{ duration: 200, easing: quintOut }}, use:trapFocus {...restProps} >
      <!-- Modal, Header -->
  {#if title || closable} <div class="nes-modal-header">
  {#if title} <h2 id="modal-title" class="nes-modal-title">{ title }</h2> {/if} {#if closable} <button class="nes-modal-close"
...
/* Content Styles */ {} .nes-modal-content { flex: 1; overflow-y: auto; padding: 1.5rem; font-size: 0.75rem; line-height: 1.5}
/* Size Adjustments */ {} .max-w-sm { max-width: 384px}
  .max-w-md { max-width: 448px}
  .max-w-lg { max-width: 512px}
  .max-w-2xl { max-width: 672px}
/* Animations */ {} @keyframes modal-close { 0% { transform: scale(1); opacity: 1}
    100% { transform: scale(0.9); opacity: 0}
  } /* Focus styles */ {} .nes-modal:focus-visible { outline: 2px solid theme('colors.nes.yellow'); outline-offset: 2px}
/* Responsive Design */ {} @media (max-width: 768px) { .nes-modal-overlay { padding: 0.5rem}
    .nes-modal { box-shadow: 6px 6px 0px 0px theme('colors.nes.black')}
    .nes-modal-dark { box-shadow: 6px 6px 0px 0px theme('colors.nes.white')}
    .nes-modal-primary { box-shadow: 6px 6px 0px 0px theme('colors.nes.blue')}
    .nes-modal-warning { box-shadow: 6px 6px 0px 0px theme('colors.nes.yellow')}
    .nes-modal-danger { box-shadow: 6px 6px 0px 0px theme('colors.nes.red')}
    .nes-modal-header { padding: 0.75rem 1rem}
    .nes-modal-content { padding: 1rem}
    .nes-modal-title { font-size: 0.75rem}
    .nes-modal-close { width: 28px; height: 28px; font-size: 0.625rem}
  } /* Accessibility improvements */ {} @media (prefers-reduced-motion reduce) { .nes-modal: {} .nes-modal-overlay { transition: none}
