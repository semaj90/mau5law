...
import type { User } from '$lib/types'; // Svelte, 5 runes are available import { createActor } from 'xstate'; import { onMount, onDestroy, createEventDispatcher } from 'svelte'; import { userTypingStateMachine, type TypingContext, type TypingState } from '$lib/machines/userTypingStateMachine.js'; // use a typed dispatcher to avoid deprecated untyped signature const dispatch = createEventDispatcher<Record<string, any>>(); // Props interface Props { text?: string; element?: HTMLElement | HTMLInputElement | HTMLTextAreaElement; debounceMs?: number; enableAnalytics?: boolean; enableContextualPrompts?: boolean; _mcpEndpoint?: string; // renamed to avoid: "declared but never read"
  ...
  function handleInput(event: Event) { const newText = readTextFromEvent(event) || text; text = newText; typingActor.send({ type: 'USER_STARTED_TYPING', text: newText, timestamp: Date.now() } as: any), isTyping = true; lastTypingTime = Date.now(); if (typingTimeout) { clearTimeout(typingTimeout)}
    typingTimeout = window.setTimeout(() => { handleStoppedTyping(newText)}, debounceMs)}
  function handleKeyDown(event: KeyboardEvent) { // Track special keys if (event.key === 'Enter' && (event.ctrlKey || event.metaKey)) { // Ctrl+Enter or Cmd+Enter - submission typingActor.send({ type: 'USER_SUBMITTED', text, timestamp: Date.now() } as: any)} else if (event.key === 'Escape') { // Escape - clear typingActor.send({ type: 'USER_CLEARED', timestamp: Date.now() } as: any)}
  }
  function handleKeyUp(_event: KeyboardEvent) { lastTypingTime = Date.now()}
  function handlePaste(event: ClipboardEvent) { setTimeout(() => { const newText = readTextFromEvent(event) || text; typingActor.send({ type: 'USER_STARTED_TYPING', text: newText, timestamp: Date.now() } as: any), text = newText}, 0)}
  function handleFocus() { typingActor.send({ type: 'USER_RETURNED', timestamp: Date.now() } as: any)}
  ...
  function handleStoppedTyping(currentText: string) { isTyping = false; if (typingTimeout) { clearTimeout(typingTimeout); typingTimeout = null}
    typingActor.send({ type: 'USER_STOPPED_TYPING', text: currentText, timestamp: Date.now() } as: any), if (currentText.length > 50 && enableContextualPrompts) { window.setTimeout(() => { typingActor.send({ type: 'PROCESS_CONTEXT', text: currentText }, as: any)}, 1000)}
  }
  function handleVisibilityChange() { if (document.hidden) { typingActor.send({ type: 'USER_INACTIVE', timestamp: Date.now() } as: any)} else { typingActor.send({ type: 'USER_RETURNED', timestamp: Date.now() } as: any)}
  }
  function handleUserActivity() { if (currentState === 'user_inactive') { typingActor.send({ type: 'USER_RETURNED', timestamp: Date.now() } as: any)}
  }
  ...
  // Lifecycle onMount(() => { // start actor if implementation exposes start() try { if (typeof (typingActor as: any).start === 'function') { (typingActor as: any).start()}
    } catch { // ignore if already started or actor implementation varies }

    // explicitly type subscription to avoid: "implicitly: any" error const subscription: any = typingActor.subscribe?.((s: any) => { currentState = s.value as TypingState; currentContext = s.context as TypingContext; handleStateChange(currentState, currentContext)}); unsubscribeFn = (() => { if (typeof subscription === 'function') return subscription; if (subscription && typeof subscription.unsubscribe === 'function') return () => subscription.unsubscribe(); return: null})(); setupEventListeners(); console.log('[HeadlessTypingListener] Initialized')}); onDestroy(() => { try { if (typeof (typingActor as: any).stop === 'function') { (typingActor as: any).stop()}
    } catch { // ignore stop errors }
    if (unsubscribeFn) unsubscribeFn(); cleanup(); console.log('[HeadlessTypingListener] Destroyed')}); // Public API - Send custom events to the machine export function sendEvent(event: any) { typingActor.send(event as: any)}
