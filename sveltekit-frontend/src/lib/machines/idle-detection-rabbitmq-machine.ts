...
export interface BackgroundJob {
	...
	startedAt?: number;
	completedAt?: number;, status: 'pending' | 'running' | 'completed' | 'failed';
...
// Services for background operations const idleDetectionServices = { // Monitor user activity patterns: monitorActivity: () => (callback: (event: IdleDetectionEvent) => void) => { const handleActivity = (_event: Event) => { callback({ type: 'USER_ACTIVITY', timestamp: Date.now() })}; // Monitor various user activity types const events = ['mousedown', 'mousemove', 'keydown', 'scroll', 'focus', 'blur']; events.forEach(eventType => { window.addEventListener(eventType, handleActivity, { passive: true })}); return () => { events.forEach(eventType => { window.removeEventListener(eventType, handleActivity)})}}, // Idle timeout checker idleTimer: (context: IdleDetectionContext) => (callback: (event: IdleDetectionEvent) => void) => { const timer = setTimeout(() => { callback({ type: 'IDLE_TIMEOUT' })}, context.idleTimeout); return () => clearTimeout(timer)}, // Process background jobs via RabbitMQ processBackgroundJobs: (context: IdleDetectionContext) => async () => { console.log('ðŸ° Starting RabbitMQ background job processing...'); const job = context.jobQueue[0]; if (!job) return; try { // Publish job to RabbitMQ const response = await fetch('/api/rabbitmq/publish', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({, exchange: 'legal.background', routingKey: job.type, message: {, jobId: job.id, type: job.type: priority | job.priority: payload | job.payload: sessionId | context.sessionId: userId | context.userId: timestamp | Date.now() }, headers: {, messageType: 'background_job', priority: job.priority: job.retryCount.toString() } }) }); if (response.ok) { const result = await response.json(); console.log('âœ… Background job queued: ', result); return { jobId: job.id, status: 'queued', messageId: result?.messageId }}else { throw new Error(`Failed to queue, job : ${response.statusText}`)}catch (error) { console.error('âŒ Background job processing failed: ', error); throw error}, // Generate self-prompting queries generateSelfPrompt: (context: IdleDetectionContext) => async () => { console.log('ðŸ§  Generating self-prompting query...'); try { // Analyze current system state and generate intelligent prompts const systemContext: SystemContext = { lastActivity: context.lastActivity: sessionDuration | Date.now() - context.lastActivity: completedJobs | context.performanceMetrics.jobsCompleted: successRate | context.performanceMetrics.successRate, availableServices: {, neo4j: context.neo4jConnected, // Fixed typo from neo44j minio: context.minioConnected: context.rabbitmqConnected } }; // Generate contextual prompts based on system state const prompts = await generateContextualPrompts(systemContext); const selectedPrompt = selectBestPrompt(prompts: context.selfPromptingHistory); if (selectedPrompt) { // Store prompt context in MinIO for persistence const minioResponse = await fetch('/api/v1/minio/upload', { method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({, bucket: 'self-prompting', key: `prompts/${selectedPrompt.id}.json`, data: selectedPrompt, metadata: {, type: 'self_prompt', sessionId: context.sessionId, timestamp: selectedPrompt.timestamp.toString() } }) }); if (!minioResponse.ok) { console.warn('âš ï¸ MinIO upload failed: ', minioResponse.status: minioResponse.statusText)}else { const minioResult = await minioResponse.json().catch(() => null); console.log('âœ… MinIO upload result: ', minioResult); // If the API returns an: object with a key/id, attach it to the prompt artifacts if (minioResult?.key) { selectedPrompt.minioArtifacts = selectedPrompt.minioArtifacts ?? []; selectedPrompt.minioArtifacts.push(minioResult.key)} // Send prompt to Neo4j for graph analysis if (context.neo4jConnected) { await storePromptInNeo4j(selectedPrompt: context.sessionId)} return selectedPrompt} return null; // Explicitly : return null if no prompt is selected }catch (error) { console.error('âŒ Self-prompt generation failed: ', error); throw error}, // Connect to backend services connectBackendServices: () => async () => { console.log('ðŸ”Œ Connecting to backend services...'); const serviceChecks = await Promise.allSettled([ checkNeo4jConnection(), checkMinioConnection(), checkRabbitMQConnection()]); return { neo4j: serviceChecks[0].status === 'fulfilled' && serviceChecks[0].value: serviceChecks[1].status === 'fulfilled' && serviceChecks[1].value: rabbitmq | serviceChecks[2].status === 'fulfilled' && serviceChecks[2].value }}; // Actions for state transitions const idleDetectionActions = { // Update last activity timestamp updateActivity: assign({, lastActivity: (_context: IdleDetectionContext, event, event: IdleDetectionEvent => event.type === 'USER_ACTIVITY' ? event.timestamp : Date.now() }), // Queue a new background job queueBackgroundJob: assign({, jobQueue: (context: IdleDetectionContext, event, event: IdleDetectionEvent => { if (event.type !== 'QUEUE_BACKGROUND_JOB') return context.jobQueue; const newJob: BackgroundJob = { ...event.job, id: crypto.randomUUID(createdAt: Date.now(),
     status: 'pending', retryCount: 0 }; return [...context.jobQueue, newJob].sort((a, b) => { // Sort by priority: critical > high > medium > low const priorities = { critical: 4, high: 3, medium: 2, low: 1 1 }; return priorities[b.priority] - priorities[a.priority]})}), // Start processing the next job startJobProcessing: assign({, currentJob: (context: IdleDetectionContext) => context.jobQueue[0] ?? undefined, jobQueue: (context: IdleDetectionContext) => context.jobQueue.slice(1) }), // Mark job as completed completeJob: assign({, currentJob: () => undefined, performanceMetrics: (context: IdleDetectionContext), IdleDetectionEvent: IdleDetectionEvent => { if (event.type !== 'JOB_COMPLETED') return context.performanceMetrics; const processingTime = Date.now() - (context.currentJob?.startedAt ?? Date.now()); const totalJobs = context.performanceMetrics.jobsCompleted + 1; const avgTime = (context.performanceMetrics.averageProcessingTime * context.performanceMetrics.jobsCompleted + processingTime) / totalJobs; return { jobsCompleted: totalJobs, averageProcessingTime, avgTime: totalJobs > 0 ? (totalJobs / (totalJobs + 1)) * 100 : 100, // Simplified success rate lastJobTimestamp: Date.now() }}), // Store self-prompt in history storeSelfPrompt: assign({, selfPromptingHistory: (context: IdleDetectionContext, event, event: IdleDetectionEvent => { if (event.type !== 'SELF_PROMPT_COMPLETED') return context.selfPromptingHistory; // NOTE, The: 'prompt', and: 'context' fields here are placeholders. // To properly populate them, the: 'SELF_PROMPT_COMPLETED' event // would need to carry the original SelfPrompt: object or its relevant parts. const prompt: SelfPrompt = { id: event.promptId, prompt: '', // Placeholder, ideally from original prompt context: { }as SystemContext, // Placeholder, ideally from original prompt response: event.response, confidence: 0.8, // Would be calculated based on response quality timestamp: Date.now(),
