	...
	try {
        ...
        if (existsSync(migrationPath)) {
             try {
			    ...
			    console.warn('[Legal DB] Error executing migration:', migrationError);
			    // Continue with Drizzle schema creation as fallback if needed but usually better to fail logic wise
		    }
        } else {
             console.warn('[Legal DB] Migration file not found:', migrationPath);
        }
...
export async function checkLegalSearchHealth() {
	try {
		...
		const health = result[0] as unknown as Record<string, boolean>;
