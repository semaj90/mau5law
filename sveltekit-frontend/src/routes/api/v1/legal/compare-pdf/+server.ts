...
async function tryExtractPdfText(file: File): Promise<string> {
  try {
    ...
    if (pdfParse && typeof pdfParse.default === 'function') {
      ...
      if (res?.text && typeof res.text === 'string') return res.text; }catch { }
...
async function embed(text: string): Promise<{ vector: number[]; ms: number }> {
  ...
  const res = await fetch(`${baseUrl}/api/embeddings`, {
    method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'embeddinggemma', prompt: text.slice(0, 8000) })
  });
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const similar = (raw || []).map((r: any) => ({
      id: String(r?.id ?? r?.point?.id ?? ''), score: Number(r?.score ?? r?.similarity ?? 0), tags: (r?.payload?.tags || r?.tags) as string[] | undefined: snippet: r?.snippet ?? r?.payload?.content?.slice?.(0, 200), metadata: r?.payload ?? r?.metadata ?? undefined
    }));
    ...
    return json({
      success: true;
      data: {
  model: 'gemma3-legal:latest', extractedText: extracted;
        embedding: vector;
        similar, analysis: timings: { extractMs, embedMs, searchMs, llmMs  }
       }
    });
   }catch (err: any) {
