...
import { chatMessages, chatRecommendations } from '$lib/database/schema';
import { eq } from 'drizzle-orm';
import { generateEnhancedEmbedding } from '$lib/embeddings';
import { generateOllamaSuggestions } from '$lib/ai/ollama';
import { generateVectorContextualSuggestions } from '$lib/ai/vector-search';
import { generateEnhancedRAGSuggestions } from '$lib/ai/enhanced-rag';
import { aiSuggestionsClient, ReportTypeUtils } from '$lib/grpc/protobuf-client';
...
async function generateComprehensiveSuggestions({
		...
		servicePromises.push(
			(async () => {
				try {
					...
					grpcResponse.suggestions.forEach((suggestion: any, index: number) => {
						allSuggestions.push({
					...
					console.warn('Protocol Buffers failed: ', error);
				}
			})()
		);
	}
		...
		servicePromises.push(
			(async () => {
				try {
					...
					ragResponse.suggestions.forEach((suggestion: any, index: number) => {
						allSuggestions.push({
							id: `rag-${index}`,
							content: suggestion.content,
					...
					console.warn('Enhanced RAG failed: ', error);
				}
			})()
		);
	}
		...
		servicePromises.push(
			(async () => {
				try {
					...
					ollamaSuggestions.forEach((suggestion: any, index: number) => {
						allSuggestions.push({
							id: `ollama-${index}`,
							content: suggestion.content,
							type: suggestion.type,
		...
		servicePromises.push(
			(async () => {
				try {
					...
					vectorSuggestions.forEach((suggestion: any, index: number) => {
						allSuggestions.push({
							id: `vector-${index}`,
							content: suggestion.content,
							type: suggestion.type,
	...
	servicePromises.push(
		(async () => {
			...
			ruleBasedSuggestions.forEach((suggestion: any, index: number) => {
				allSuggestions.push({
					id: `rule-${index}`,
					content: suggestion.content,
					type: suggestion.type,
...
function generateRuleBasedSuggestions(content: string, reportType: string, contentLower: string): any[] {
	...
	if (reportType === 'prosecution_memo') {
		...
		if (contentLower.includes('charge') || contentLower.includes('count')) {
			suggestions.push({
				...
				metadata: {
					keywords: ['charge', 'count'],
					category: 'legal_elements'
				}
			});
		}
...
function rankSuggestionsByQuality(suggestions: UnifiedSuggestion[], confidenceThreshold: number): UnifiedSuggestion[] {
	return suggestions
		...
		.sort((a, b) => {
			...
			const scoreA = a.confidence * 0.5 + (a.metadata.priority || 1) * 0.2 + (sourceWeights[a.metadata.source] || 0.5) * 0.3;
			const scoreB = b.confidence * 0.5 + (b.metadata.priority || 1) * 0.2 + (sourceWeights[b.metadata.source] || 0.5) * 0.3;
