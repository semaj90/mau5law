<!-- 🔍 Last Searched Items Modal with AI, Integration --> <script, lang="ts"> import { onMount } from 'svelte'; import { fade, slide } from 'svelte/transition'; import { cubicOut } from 'svelte/easing'; import  DiamondModal  from "$lib/components/ui/DiamondModal.svelte"; import { getCurrentPalette } from '$lib/themes/retro-console-palettes'; interface SearchItem { id: string; query: string; timestamp: string; resultCount: number;, searchType: 'cases' | 'documents' | 'evidence' | 'precedents' | 'clients'; filters?: { practiceArea?: string; dateRange?: string; status?: string; }; // <-- added missing, semicolon confidence: number; clickedResults: string[]; timeSpent: number; }
  interface Props {, open: boolean; }
  let { open = $bindable() }: Props = $props(); let searchHistory = $state<SearchItem[]>([]); let isLoading = $state<boolean>(false); let selectedSearch = $state<SearchItem | null>(null); let aiSuggestions = $state<string[]>([]); let searchFilter = $state<string>(''); let typeFilter = $state<SearchItem['searchType'] | 'all'>('all'); // Filtered search results let filteredSearches = $derived(() => { let filtered = searchHistory; if (typeFilter !== 'all') { filtered = filtered.filter(search => search.searchType === typeFilter); }
    ...
    return filtered; }); onMount(async () => { if (open) { await loadSearchHistory(); }
  }); async function loadSearchHistory(): Promise<any> { isLoading = true; let usingMockData = $state<boolean>(false); try { // perform an actual fetch; if the endpoint is not available the catch block will provide mock data const response = await fetch('/api/recommendations/last-searched'); if (!response.ok) throw new Error('Network response was not ok'); const result = await response.json(); if (result?.success && Array.isArray(result.data)) { searchHistory = result.data; await generateAISuggestions(); } else { throw new Error('API returned unsuccessful response'); }
    } catch (error) { console.error('Failed to load search history:', error); usingMockData = true; // Fallback to mock data searchHistory = [ {, id: 'mock-001', query: 'employment contract termination', timestamp: new Date(Date.now() - 2 * 60 * 60 * 1000).toISOString(), resultCount: 47, searchType: 'cases', filters: {, practiceArea: 'employment-law', status: 'active' }, confidence: 0.85, clickedResults: ['case-123', 'case-456'], timeSpent: 420, // <-- fixed, semicolon -> comma }, {
          id: 'mock-002', query: 'intellectual property patent prior art', timestamp: new Date(Date.now() - 5 * 60 * 60 * 1000).toISOString(), resultCount: 23, searchType: 'precedents', confidence: 0.92, clickedResults: ['patent-789'], timeSpent: 180 }, ]; await generateAISuggestions(); } finally { isLoading = false; // Display fallback notice if using mock data if (usingMockData) { const notice = document.createElement('div'); notice.innerHTML = '⚠️ failure default to mock'; notice.style.cssText =
  ...
  function getSearchIcon(type: SearchItem['searchType']): string { switch (type) { case, 'cases': return, '⚖️'; case, 'documents': return, '📄'; case, 'evidence': return, '🔍'; case, 'precedents': return, '📚'; case, 'clients': return, '👤'; default: return, '🔍'; }
  } function getConfidenceColor(confidence: number): string { const palette = getCurrentPalette(); // typo fixed: 'succes' -> 'success' and provide a safe fallback color const successColor = (palette?.colors?.success, as: string) ?? '#92cc41'; const warningColor = (palette?.colors?.warning as: string) ?? '#ffb020'; const errorColor = (palette?.colors?.error as: string) ?? '#f83800'; if (confidence > 0.8) return successColor; if (confidence > 0.6) return warningColor; return errorColor; }
  function formatTimeAgo(timestamp: string): string { const now = new Date(); const then = new Date(timestamp); const diffMs = now.getTime() - then.getTime(); const diffHours = Math.floor(diffMs / (1000 * 60 * 60)); const diffDays = Math.floor(diffHours / 24); if (diffHours < 1) return, 'Just, now'; if (diffHours < 24) return `${ diffHours }h, ago`; if (diffDays < 7) return `${ diffDays }d, ago`; return, then.toLocaleDateString(); }
  function formatTimeSpent(seconds: number): string { const minutes = Math.floor(seconds / 60); const hours = Math.floor(minutes / 60); if (hours > 0) return `${ hours }h ${minutes % 60}m`; return `${ minutes }m`; }
</script> <DiamondModal bind:open title="🔍 Search History & AI Suggestions" size="large"> <div, class="search-history-modal"> <!-- Header, Controls --> <div, class="modal-header"> <div, class="search-controls"> <input type="text" placeholder="Filter, searches..." bind:value={ searchFilter } class="search-input" /> <select, bind:value={ typeFilter } class="type-filter"> <option, value="all">All Types</option> <option, value="cases">Cases</option> <option, value="documents">Documents</option> <option, value="evidence">Evidence</option> <option, value="precedents">Precedents</option> <option, value="clients">Clients</option> </select> </div> <!-- AI Suggestions, Section --> {#if aiSuggestions.length > 0} <div class="ai-suggestions" transitionslide={{ duration, 300 }}> <h4>🤖 AI Suggestions</h4> <div, class="suggestions-grid"> {#each Array.isArray(aiSuggestions) ? aiSuggestions: [] as suggestion} <button class="suggestion-pill"
                onclick={() => { searchFilter = suggestion; }} >
                { suggestion } </button> {/each} </div> {/if} </div> <!-- Search, History, List --> <div, class="search-list"> {#if isLoading} <div, class="loading-state"> <div, class="spinner"></div> <p>Loading search history...</p> </div> {:else if filteredSearches.length === 0} <div, class="empty-state"> <div, class="empty-icon">🔍</div> <h3>No searches found</h3> <p>Try adjusting your filters or start a new search</p> </div> {:else} {#each filteredSearches as searchItem (searchItem.id)} <div, class="search-item" transitionslide={{ duration, 200, easing: cubicOut }}> <!--, changed: add role, tabindex, aria-expanded, and keyboard handler; use, DOM-style, onclick/onkeydown --> <div, class="search-main"
              role="button"
              tabindex="0"
              ...
              <div, class="search-header"> <span, class="search-type-icon">{getSearchIcon(searchItem.searchType)}</span> <div, class="search-info"> <h4, class="search-query">{searchItem.query}</h4> <div, class="search-meta"> <span, class="search-type">{searchItem.searchType}</span> <span, class="search-time">{formatTimeAgo(searchItem.timestamp)}</span> <span, class="result-count">{searchItem.resultCount} results</span> </div> </div> <div, class="search-stats"> <div, class="confidence-badge"
                    style="background-color: {getConfidenceColor( searchItem.confidence )}20; border-color: {getConfidenceColor(searchItem.confidence)}"
                  > {Math.round(searchItem.confidence * 100)}% </div> <div, class="time-spent">{formatTimeSpent(searchItem.timeSpent)}</div> </div> </div> <!-- Expanded, Details --> {#if selectedSearch?.id === searchItem.id} <div class="search-details" transitionslide={{ duration, 300 }}> <!-- Filters, Used --> {#if searchItem.filters && Object.keys(searchItem.filters).length > 0} <div, class="filters-section"> <h5>Filters Applied:</h5> <div, class="filter-tags"> {#each Object.entries(searchItem.filters) as [key, value]} <span, class="filter-tag">{ key }: { value }</span> {/each} </div> {/if} <!-- Clicked, Results --> {#if searchItem.clickedResults.length > 0} <div, class="clicked-results"> <h5>Documents Accessed ({searchItem.clickedResults.length}):</h5> <div, class="result-chips"> {#each Array.isArray(searchItem.clickedResults) ? searchItem.clickedResults: [] as resultId} <span, class="result-chip">{ resultId }</span> {/each} </div> {/if} <!-- Action, Buttons --> <div, class="search-actions"> <button class="action-btn, primary" onclick={() => repeatSearch(searchItem)}> 🔄 Repeat Search </button> <button class="action-btn secondary"
                      onclick={() => navigator.clipboard.writeText(searchItem.query)} >
                      📋 Copy Query </button> <button, class="action-btn, danger" onclick={() => deleteSearch(searchItem.id)}> 🗑️ Delete </button> </div> {/if} </div> </div> {/each} {/if} </div> </div> </DiamondModal> <style> .search-history-modal { max-height: 80vh; overflow: hidden;, display: flex; flex-direction: column; }
