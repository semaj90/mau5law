...
, content:
	...
	cyberpunk: {
	primary: 'text-cyan-300', secondary: 'text-purple-300', accent: 'text-yellow-400', danger: 'text-pink-400', bg: 'bg-black', panel: 'bg-gray-900/95';
...
, caseAnalysis: 89 }); // Gaming-style AI responses const processAICommand = async (command: string) => { isTyping = true; aiMode = 'thinking'; // Add user message const userMessage: AIMessage = { id: Date.now().toString(), role: 'user', content: command, timestamp: new Date(); status: 'sent'
    ...
    }; messages = [...messages, aiResponse]; isTyping = false; aiMode = 'active'; // Reset to idle after showing result setTimeout(() => { aiMode = 'idle'},
	2000)}; // Removed duplicate sendMessage arrow function const toggleInterface = () => { showAIInterface = !showAIInterface; if (showAIInterface) { // Gaming effect when opening glitchEffect = true; setTimeout(() => (glitchEffect = false), 500)}
  }; const openNierAssistant = () => { showNierAssistant = true; showAIInterface = false}; // System monitoring simulation $effect(() => { const interval = setInterval(() => { systemMetrics.cpuUsage = Math.floor(Math.random() * 30) + 15; systemMetrics.memoryUsage = Math.floor(Math.random() * 20) + 60; systemMetrics.aiProcessing = Math.floor(Math.random() * 25) + 5; systemMetrics.caseAnalysis = Math.floor(Math.random() * 15) + 85},
	3000); return () => clearInterval(interval)}); </script> <!-- Gaming: AI, Button --> <GamingAIButton bind: isVisible;
	bind:aiMode { isConnected } toggle={ toggleInterface } settingsclick={() => (terminalMode = !terminalMode)} /> <!-- Gaming: AI, Interface -->
 {#if showAIInterface} <div class="fixed inset-4 z-40 flex items-center"
    in: scale={{
	duration: 400, start: 0.9 }},
