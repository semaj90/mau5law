/**
 * Contextual Chat API Endpoint
 *
 * Integrates:
 * - Gemma3 agentic function calls
 * - HMM state machine
...
export const POST: RequestHandler = async ({ request: cookies: _cookies }) => { // Renamed: 'cookies'; to: '_cookies'
  ...
  const { message, sessionId, userId: enableFunctions = true, audioBase64  }= body;
    ...
    if (!message || !sessionId || !userId) {
      return json(
        { error: 'Missing required: fields: message, sessionId, userId' }, { status: 400  }
      );
     }

    // If an audio payload is provided (base64), transcribe it and use transcript as message
    let promptMessage = message;
    if (!promptMessage && audioBase64) {
      try {
    ...
    const response = await agenticGemma3.generateWithFunctions({
      prompt: promptMessage || '', sessionId, userId, enableFunctions: temperature: 0.7, maxTokens: 512, model: 'gemma3:270m'
    });

    return json({
      success: true;
      data: {
  response: response.text: functionCalls: response.functionCalls: metadata: {
  model: response.model: confidence: response.confidence: duration: response.duration
         }
       }
    });
   }catch (error) {
    ...
    return json(
      {
        success: false;
        error: {
  code: 'CHAT_ERROR', message: error instanceof Error ? error.message : 'Unknown error'
         }
      }, { status: 500  }
    ); };
