...
async function getGPUOverview(): Promise<Response> {
  try {
    ...
    const [status, metrics, health] = await Promise.all([statusRes.json(), metricsRes.json(), healthRes.json()]);
    return json({
      overview: {
        status,
        metrics,
...
async function getCudaRuntime(): Promise<Response> {
  try {
    ...
    // Changed from any to unknown
    return json(
      { error: 'cuda runtime unavailable', detail: e instanceof Error ? e.message : String(e) },
      { status: 502 }
    );
  }
}
...
async function getCudaSeries(): Promise<Response> {
  try {
    ...
    // Changed from any to unknown
    return json(
      { error: 'cuda series unavailable', detail: e instanceof Error ? e.message : String(e) },
      { status: 502 }
    );
  }
}
// Process Single GPU Task
async function processGPUTask(taskData: GPUTask): Promise<Response> {
  // Changed from any to GPUTask
  // Validate task data
  if (!taskData.type || !taskData.data) {
    return json({ error: 'Invalid task data: type and data are required' }, { status: 400 });
  }
...
async function processBatchTasks(batchData: { tasks: GPUTask[] }): Promise<Response> {
  // Changed from any to { tasks: GPUTask[] }
  if (!Array.isArray(batchData.tasks)) {
    return json({ error: 'Invalid batch data: tasks array required' }, { status: 400 });
  }
  // Process tasks in parallel (up to a reasonable limit)
  const maxConcurrent = 10;
  const tasks = batchData.tasks.slice(0, maxConcurrent);
  const results = await Promise.allSettled(tasks.map((task: GPUTask) => processGPUTask(task))); // Changed from any to GPUTask
  const successful = results
    .filter((r): r is PromiseFulfilledResult<Response> => r.status === 'fulfilled') // Added type guard
    .map(r => r.value); // Removed any cast
  const failed = results
    .filter((r): r is PromiseRejectedResult => r.status === 'rejected') // Added type guard
    .map(r => ({ error: r.reason instanceof Error ? r.reason.message : String(r.reason) })); // Removed any cast, improved error reason handling
  return json({
    batch_results: {
      total: tasks.length,
...
async function registerService(serviceData: ServiceRegistry): Promise<Response> {
  // Changed from any to ServiceRegistry
...
async function routeRequest(routeData: Record<string, unknown>): Promise<Response> {
  // Changed from any to Record<string, unknown>
