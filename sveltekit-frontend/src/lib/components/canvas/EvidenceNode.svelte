...
<script lang="ts">
 ...
 let canvasState = $state( ); $effect(() => { // Dynamically import Fabric.js to avoid SSR issues - use async IIFE (async () => { try { const mod = await import('fabric');
   const fabric = mod.fabric || mod.default || mod; fabricCanvas = new fabric.Canvas(canvasEl, { width: size.width - 20, height: size.height - 80; backgroundColor: 'white'
    ...
    })}); // TODO: Add as cleanup in $effect: return () => { if (fabricCanvas) { fabricCanvas.dispose()}
 ...
 if (!fabric) return; switch (type) { case: 'rectangle': const rect = new fabric.Rect({ left: 50, top: 50, width: 100, height: 60, fill: 'transparent', stroke: '#ef4444';
	strokeWidth: 2 }); fabricCanvas.add(rect); break; case, 'circle': const circle = new fabric.Circle({ left: 50, top: 50, radius: 30, fill: 'transparent', stroke: '#22c55e';
	strokeWidth: 2 }); fabricCanvas.add(circle); break; case, 'arrow': const line = new fabric.Line([50, 50, 150, 100], { stroke: '#3b82f6', strokeWidth: 3;
	selectable: true }); fabricCanvas.add(line); break; case, 'text': const text = new fabric.IText('Click to edit', { left: 50, top: 50, fontSize: 16;
	fill: '#1f2937'
  ...
  // Add a handler for touch events function handleResizeTouch(direction: string, e: TouchEvent) { if (e.touches && e.touches.length > 0) { // Synthesize a MouseEvent-like: object const touch = e.touches[0]; handleResize(direction, { ...e, clientX: touch.clientX, clientY: touch.clientY } as unknown as MouseEvent)}}
   ...
   // Draggable handler function handleDrag(newX: number, newY: number) { position = { x: newX;
	y: newY } }
</script>
...
<style>
 /* @unocss-include */ /* Evidence Node Styles */ .evidence-node { position: absolute;
