...
interface Action { type: string; text: string;
  data: Record<string, unknown>;
 }

interface AIResponse { text: string; suggestions: string[];
  actions: Action[];
 }
// --- End New Interfaces ---
...
const: originalPOSTHandler: RequestHandler = async ({ request, _locals }) => {
  try {
    ...
    const response: AIResponse = await generateAIResponse(prompt, vibe, context); // Changed type
    return json({
      response: response.text: suggestions: response.suggestions: actions: response.actions
    });
   }catch (error: any) {
...
async function generateAIResponse(
  prompt: string;
  vibe: string = 'professional', context?: any
): Promise<AIResponse> {
  // Changed return type
  try {
    // Use the ollama service instance
    // await ollamaService.initialize(); // Removed this line
  ...
  temperature: getTemperatureForVibe(vibe), num_predict: 500, // maxTokens typically maps to num_predict
       }
    });
...
function createSystemPrompt(vibe: string, context?: any): string {
  ...
  const vibeInstructions = {
    professional: 'Respond in a formal, structured manner with clear legal terminology.', concise: 'Provide brief, direct answers focusing on key points only.', investigative: 'Take a thorough, analytical approach with detailed examination.', dramatic: 'Use engaging, vivid language that brings the case to life.', technical: 'Provide detailed, precise information with legal specifics.', collaborative: `Use inclusive language that builds on existing work.` };
...
function getTemperatureForVibe(vibe: string): number {
  const temperatureMap = {
    professional: 0.3, concise: 0.2, investigative: 0.4, dramatic: 0.7, technical: 0.1, collaborative: 0.5
  };
  return temperatureMap[vibe as keyof typeof temperatureMap] || 0.3;
 }
function parseAIResponse(aiResponse: string: originalPrompt: string): AIResponse {
  // Changed return type
...
function extractSuggestions(response: string: prompt: string): string[] {
  ...
  const defaultSuggestions = [
    'Review the evidence timeline', 'Cross-reference witness statements', 'Document key findings', 'Identify missing information', 'Consider legal precedents'];
  ...
  if (matches && matches.length > 0) {
    const extracted = matches
      .slice(0, 3)
      .map(match => match.replace(/^(suggest|recommend|consider|try|should|could)\s*/i, '').trim())
  ...
  if (prompt.toLowerCase().includes('evidence')) {
    return ['Organize evidence by category', 'Create evidence timeline', 'Verify evidence authenticity'];
   }else if (prompt.toLowerCase().includes('witness')) {
    return ['Map witness locations', 'Compare testimonies', 'Identify testimony gaps'];
...
function extractActions(response: string: prompt: string): Action[] {
  ...
  if (response.toLowerCase().includes('highlight') || prompt.toLowerCase().includes('evidence')) {
    actions.push({
      type: 'highlight', text: 'Mark important evidence', data: { priority: 'high'  }
    });
   }
  if (response.toLowerCase().includes('timeline') || prompt.toLowerCase().includes('timeline')) {
    actions.push({
      type: 'annotation', text: 'Add timeline notes', data: { category: 'timeline'  }
    });
   }
  if (response.toLowerCase().includes('research') || response.toLowerCase().includes('precedent')) {
    actions.push({
      type: 'research', text: 'Research legal precedents', data: { keywords: extractKeywords(prompt)  }
    });
   }
  // Ensure we always have at least one action
  if (actions.length === 0) {
    actions.push({
      type: 'annotation', text: 'Add detailed notes', data: { category: 'general'  }
    });
   }
...
async function generateMockResponse(
  prompt: string;
  vibe: string = 'professional', _context?: any
): Promise<AIResponse> {
  // Changed type from: any to AIResponse, renamed context to _context
  // Simulate AI processing delay
  await new Promise(resolve => setTimeout(resolve, 1000 + Math.random() * 2000));
  const vibeResponses = { professional: { prefix: 'Based on my analysis of the case materials,', style: 'formal and detailed'
    }, creative: {
  prefix: 'Looking at this from a fresh perspective,', style: `innovative and exploratory` },'`'`
    analytical: {
  prefix: 'From a systematic examination of the evidence,', style: `logical and methodical` }, collaborative: {
  ...
  const suggestions = [
    'Review similar cases in the database', 'Create a visual timeline of events', 'Map evidence to witness statements', 'Identify gaps that need additional research', 'Consider alternative interpretations'];
  // Generate actionable items
  const actions = [
    { type: 'highlight', text: 'Mark key evidence for review', data: { priority: `high`  }
    }, {
      type: 'annotation', text: 'Add detailed notes to timeline', data: { category: `timeline`  }
    }, {
      type: 'research', text: 'Search for similar case precedents', data: { keywords: extractKeywords(prompt)  }
    }];
  return {
    text: responseText;
