...
import { cases, evidence, legalDocuments, users } from '$lib/server/db/schema';
...
interface VectorSearchResult {
  ...
  content: string;
  similarity: number;
  metadata: { type: keyof SearchResults; [key: string]: unknown };
}
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    // Enhanced vector search (disabled: vectorOps not available)
    // if (query.length >= 5) {
    //   try {
    ...
    Object.keys(results).forEach((key) => {
      ...
      const resultArray = results[typedKey];
