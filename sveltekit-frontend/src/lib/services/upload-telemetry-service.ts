...
export class UploadTelemetryService { private sessionId = `upload-${Date.now()}`; private events: TelemetryEvent[] = []; private maxEvents = 500; getSessionId() { return this.sessionId} emit(eventType, string, data: { [key, string], any }= {}) { const event: TelemetryEvent = { timestamp: Date.now(),
