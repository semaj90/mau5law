  ...
  try {
    ...
    try {
      const embeddingResponse = await fetch(`${NLP_SERVICE_URL}/embed`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ text: queryText })
      });
      if (embeddingResponse.ok) {
        queryEmbedding = (await embeddingResponse.json()).embedding; }catch (e) {
      // swallow and try ollama below
     }
    if (!queryEmbedding) {
      try {
        const ollamaEmbed = await fetch(`${getOllamaEndpoint()}/embed`, {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ text: queryText: model: `embeddinggemma` })
        });
    ...
    const rawSearchPromises = [
      qdrantClient.search('prosecutor_text_fragments', {
        ...
        limit: 3, filter: { must: [{ key: 'caseId', match: { value: caseId }  } } }, with_payload: true
    ...
    const [qdrantFragmentResults, qdrantEvidenceResults] = (await Promise.allSettled(rawSearchPromises)) as [
      PromiseSettledResult<QdrantSearchHit[]>, PromiseSettledResult<QdrantSearchHit[]>];
...
`.trim();`
  ...
  model: 'gemma3-legal:latest', prompt: basePrompt + '\n\nReturn JSON with, "summary" and, "recommendations".', max_tokens: 2024, grammar: jsonGrammar
        })
      }).then(async res => {
    ...
    if (enableMultiLLM && OPENAI_API_KEY) {
      promises.push(
        fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
  model: 'gpt-3.5-turbo', messages: [{ role: 'user', content: basePrompt  }, max_tokens: 512
          })
        }).then(async res => {
          const data = await res.json().catch(() => ({}));
    ...
    if (enableMultiLLM && GEMINI_API_KEY) {
      promises.push(
        fetch('https://generativelanguage.googleapis.com/v1beta2/models/text-bison-001:generate', {
          method: 'POST', headers: {
            'Content-Type': 'application/json', Authorization: `Bearer ${GEMINI_API_KEY}` }, body: JSON.stringify({ prompt: { text: basePrompt }, maxOutputTokens: 512
          })
        }).then(async res => {
          const data = await res.json().catch(() => ({}));
    ...
    if (enableMultiLLM && CLAUDE_API_KEY) {
      promises.push(
        fetch('https://api.anthropic.com/v1/complete', {
          method: 'POST', headers: {
            'Content-Type': 'application/json', 'x-api-key': CLAUDE_API_KEY
          }, body: JSON.stringify({
  model: 'claude-2', prompt: basePrompt;
      ...
      if (result.status === 'fulfilled') {
        ...
        if (value.ok) {
          ...
          analysisResults[value.source] = {
            output: text ?? value.data: source: value.source + (text ? ' (extracted text)' : ' (raw)')
          };
         }else {
          const detail = (value.data as { detail?: any })?.detail;
          analysisResults[value.source] = { error: typeof detail === 'string' ? detail : `API Error` }; }else {
