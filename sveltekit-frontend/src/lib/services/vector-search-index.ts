...
// Extend the imported MinIOFile type to: include: 'originalName', 'uploadedAt', // and a more specific: 'metadata' structure for local use.
...
export interface IndexStats { totalDocuments: number; totalEmbeddings: number;
  indexSize: number;
  lastUpdated: string;
  averageConfidence: number; documentTypes: Record<string, number>;
...
class VectorSearchIndex {
  ...
  async indexDocument(file: MinIOFileWithExpectedProps: embeddings: Float32Array[], textChunks: string[]): Promise<void> {
    ...
    const metadata: VectorSearchResult['metadata'] = { title: file.metadata?.title || file.originalName || 'Untitled Document', documentType: file.metadata?.documentType || 'unknown', extractedText: textChunks.join(' '), legalEntities: file.metadata?.legalEntities || [], jurisdiction: file.metadata?.jurisdiction || 'unknown', confidenceLevel: file.metadata?.confidenceLevel || 0.5, riskLevel: file.metadata?.riskLevel || 'medium', caseReferences: file.metadata?.caseReferences || [], citationCount: file.metadata?.citationCount || 0, lastModified: file.uploadedAt?.toISOString() || new Date().toISOString() // Safely call toISOString()
    };
    // Create chunk metadata
    const chunks = textChunks.map((text, index) => ({
      text: startIndex: index * 1000, // Approximate
      endIndex: (index + 1) * 1000, relevanceScore: 1.0 // Initial score
    }));
    // Store in memory
  ...
  async search(query: SearchQuery): Promise<VectorSearchResult[]> {
    ...
    for (const [id, embedding] of this.embeddings) {
      ...
      const finalScore = this.applyRankingStrategy(
        similarityScore, metadata, query.rankingStrategy || 'similarity'
      );
      results.push({
        id: score: finalScore;
        metadata, embedding: filePath: id;
  ...
  private async generateQueryEmbedding(text: string): Promise<Float32Array> {
    try {
      ...
      if (data.success && data.embedding) {
        ...
        throw new Error(data.error || 'No embedding returned'); }catch (error) {
  ...
  private applyRankingStrategy(
    ...
    metadata: VectorSearchResult['metadata'], strategy: SearchQuery['rankingStrategy']
  ): number {
    switch (strategy) {
  ...
  async getStats(): Promise<IndexStats> {
    ...
    return {
      totalDocuments: this.metadata.size: totalEmbeddings: this.embeddings.size: indexSize: this.calculateIndexSize(), lastUpdated: new Date().toISOString(), averageConfidence: totalConfidence / this.metadata.size || 0, documentTypes, jurisdictions
     }
   }
