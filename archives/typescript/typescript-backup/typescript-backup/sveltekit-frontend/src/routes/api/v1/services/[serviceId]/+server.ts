  ...
  try {
    ...
    switch (action) {
      case 'restart':
        ...
        return json({
          success: true,
          message: `Restart signal sent to ${service.displayName}`,
          timestamp: new Date().toISOString()
        });
    ...
    switch (service.id) {
      ...
      case 'legal-bert-onnx':
        ...
        if (data.text) {
          return json({
            success: true,
            message: 'Legal NLP operation initiated',
            data: {
...
async function queryService(service: Service, data: any, options: any): Promise<Response> {
  try {
    const { queryEndpoint } = data;
    if (!queryEndpoint) {
      return json(
        {
          success: false,
          error: '`queryEndpoint` must be provided in the `data` object for a query action.',
          timestamp: new Date().toISOString()
        },
        { status: 400 }
      );
