...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const result = await ollamaService.generate(prompt, {
      model: model || 'gemma:legal', options: { temperature: num_predict: maxTokens  }
    });
    ...
    return new Response(
      JSON.stringify({
  text:
          (result as { response?: any; text?: any }).response || (result as { response?: any; text?: any }).text || '', model: result?.model || 'gemma:legal', qualityScore: 0.82, citations: [], issues: entities: [], domain: domain || 'general', documentType: documentType || 'generic', latencyMs
