...
<RotateCcw size={ 16 } /> </Button> </div> </h3> </div>
 ...
 <Progress value={$progress} class="h-2" />
  {#if stateValue === "uploading"} <p class="text-sm nes-text">Uploading file...</p> {:else if stateValue === "processing"} <p class="text-sm nes-text">Processing with AI...</p> {/if}
  </div> {/if}
  </div>
