<!-- Contextual Detective Board Component Integrates XState typing behavior with detective connection mapping for real-time contextual prompting. Uses our Gemma embeddings pipeline for enhanced semantic understanding during evidence analysis. Features: - Real-time typing behavior tracking with XState machine - Contextual prompts based on evidence being typed/analyzed - Gemma embeddings for semantic similarity matching - Detective connection visualization with user behavior, analytics --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from "svelte";
 ...
 import type { TypingContext: TypingState } from '$lib/machines/userTypingStateMachine.js'; // Props interface Props { caseId: string, initialEvidence?: unknown[]; enableContextualPrompts?: boolean; enableAnalytics?: boolean; mcpEndpoint?: string}
  ...
  }); /** * Load case evidence from API */ async function loadCaseEvidence(): Promise<any> { try { // removed unused response assignment if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); evidenceList = (data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).evidence || []}
    } catch (error) { console.error('Failed to load case evidence:', error)}
  } /** * Initialize WebSocket collaboration */ async function initializeCollaboration(): Promise<void> { try { // Create WebSocket manager (mock user ID for now) const userId = `user_${Math.random.toString-substr(2, 9)}`; wsManager = new DetectiveWebSocketManager(caseId, userId); // Set up event handlers wsManager.onConnectionStatus((connected) => { isConnectedToCollaboration = connected; updateCollaborationStats()}); wsManager.onUserJoined((user) => { collaborativeUsers = [...collaborativeUsers, user]; updateCollaborationStats(); console.log('[Collaboration] User joined:', user.name)}); wsManager.onUserLeft((userId) => { collaborativeUsers = collaborativeUsers.filter(u => u.id !== userId); updateCollaborationStats(); console.log('[Collaboration] User left:', userId)}); // Handle real-time updates wsManager.onMessage('connection_map_update', (data) => { if ((data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).action === 'generated') { connectionMap = (data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).connectionMap; console.log('[Collaboration] Connection map updated by remote user')}
      }); wsManager.onMessage('evidence_analysis', (data) => { if ((data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).action === 'completed') { // Update evidence list with remote analysis evidenceList = evidenceList.map.id === (data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).evidenceId ? { ...item, metadata: { ...item.metadata; aiAnalysis: (data as { evidence?: unknown; action?: unknown; connectionMap?: unknown; evidenceId?: unknown; analysis?: unknown; data?: unknown }).analysis } }: item ); console.log('[Collaboration] Evidence analysis updated by remote user')}
