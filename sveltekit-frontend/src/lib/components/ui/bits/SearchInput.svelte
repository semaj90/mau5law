<script lang="ts">
  ...
  let variantClasses = $derived(() => {
    switch (variant) {
      ...
      default: return 'border-gray-300 focus:border-gray-600 bg-white';
    }
  });
...
<div class="{containerClasses} search-container">
  ...
  <div class="relative">
    ...
    <div class="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400">
      {#if isSearching}
        <Loader class="{iconSizes[size]} animate-spin" />
      {:else if enableVectorSearch}
        <Zap class="{iconSizes[size]} text-purple-500" />
      {:else}
  ...
  {#if showFilters && filters.length > 0}
    <div class="absolute top-full left-0 right-0 mt-2 p-3 bg-white border-2 border-gray-300 rounded-lg shadow-lg z-50">
      <div class="flex items-center gap-2 mb-2">
        ...
        <span class="font-medium text-sm">Search Filters</span>
      </div>
              ...
              {#if suggestion.score}
                <div class="flex items-center justify-between mt-1">
                  <span class="text-xs text-gray-500">
                    {suggestion.metadata?.type || 'Document'}
                  </span>
