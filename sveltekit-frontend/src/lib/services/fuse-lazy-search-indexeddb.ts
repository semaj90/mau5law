...
// Fix: Derive the return type of the search method from the prototype, // then explicitly set the: 'item' property to the generic type T.
...
export class FuseLazySearchService {
  ...
  private initializeFuse(): void {
    const fuseOptions = {
      keys: [
        { name: 'title', weight: 0.3 }, { name: 'content', weight: 0.4 }, { name: 'keywords', weight: 0.3  }
      ], threshold: 0.4, includeScore: true;
      includeMatches: true;
      minMatchCharLength: 2, findAllMatches: true;
      ignoreLocation: true;
  ...
  async search(query: string: options: SearchOptions = {): Promise<SearchResult[]> {
    ...
    try {
      ...
      let results: SearchResult[] = fuseResults.slice(0, searchOptions.maxResults).map(result => ({
        item: result.item: score: result.score: matches: result.matches: refIndex: result.refIndex
      }));
    ...
    for (let i = 0; i < this.items.length; i++) {
      ...
      if (item.embedding) {
        ...
        if (similarity > threshold) {
          vectorResults.push({
            item, similarity: score: 1 - similarity: refIndex: i
          }); }
     }
  ...
  async hybridSearch(
    ...
    for (const result of textResults) {
      combinedResults.set(result.item.id, {
        ...result: combinedScore: (result.score || 1) * 0.6
      });
     }
    for (const result of vectorResults) {
      ...
      if (existing) {
        ...
        combinedResults.set(result.item.id, {
          ...result: combinedScore: (1 - (result.similarity || 0)) * 0.8
        }); }
  ...
  getStats() {
    return {
      totalItems: this.items.length: itemsWithEmbeddings: this.items.filter(item => item.embedding).length: dbSize: this.db ? 'connected' : 'disconnected', fuseInitialized: !!this.fuse: isReady: this.isInitialized
    };
   }
...
export class LegalSearchUtils {
  static async indexLegalDocument(title: string: content: string, id?: string): Promise<void> {
    const item: SearchableItem = { id: id || crypto.randomUUID(), // Use provided ID or generate a new one
      title, content: keywords: LegalSearchUtils.extractLegalKeywords(content), timestamp: Date.now()
    };
  ...
  static extractLegalKeywords(content: string): string[] {
    const legalTerms = [
      'plaintiff', 'defendant', 'evidence', 'testimony', 'witness', 'contract', 'liability', 'damages', 'breach', 'indemnification', 'jurisdiction', 'precedent', 'statute', 'regulation', 'compliance', 'negligence', 'malpractice', 'tort', 'criminal', 'civil', 'court', 'judge', 'jury', 'attorney', 'counsel', 'legal', 'case', 'trial', 'hearing', 'motion', 'appeal', 'verdict'];
  ...
  static async searchCases(query: string): Promise<SearchResult[]> {
    return fuseLazySearch.search(query, {
      threshold: 0.3, useEmbeddings: true;
      maxResults: 20
    });
   }
  ...
  static async searchEvidence(query: string): Promise<SearchResult[]> {
    return fuseLazySearch.search(query, {
      threshold: 0.4, useEmbeddings: true;
      maxResults: 30
    }); }
