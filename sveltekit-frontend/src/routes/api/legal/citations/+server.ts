...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const citationType = url.searchParams.get('citationType');
    ...
    let query = db
      ...
      .leftJoin(cases, eq(citations.caseId, cases.id))
      .orderBy(desc(citations.relevanceScore), desc(citations.createdAt))
      .limit(limit);

    // Apply filters
    const conditions = [];
    if (caseId) conditions.push(eq(citations.caseId, caseId));
    if (documentId) conditions.push(eq(citations.documentId, documentId));
