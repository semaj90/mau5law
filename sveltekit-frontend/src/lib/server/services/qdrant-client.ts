...
async function ensureFetch(): Promise<FetchFn> {
  ...
  try {
    ...
    const mod: unknown = nf;
    return ((mod as any)?.default || mod) as FetchFn;
  } catch {
...
async function httpRequest(path: string, method = 'GET', body?: unknown): Promise<any> {
...
async function httpCreatePayloadIndex(
  collectionName: string,
  body: PayloadIndexBody
): Promise<any> {
  const base = getQdrantUrl().replace(/\/$/, '');
  const tries = [
...
async function httpDelete(collectionName: string, ids: (string | number)[]): Promise<any> {
  return await httpRequest(
  ...
  async createCollection(collectionName: string, body: CreateCollectionBody): Promise<any> {
    ...
    if (sdk) {
      try {
        ...
        if (typeof sdk.createCollection === 'function') {
          return await sdk.createCollection(collectionName, body);
        }
      } catch (e) {
  ...
  async createPayloadIndex(collectionName: string, body: PayloadIndexBody): Promise<any> {
    ...
    if (sdk) {
      try {
        ...
        if (sdk.collections?.createFieldIndex) {
          return await sdk.collections.createFieldIndex(collectionName, body);
        }
        if (typeof sdk.createFieldIndex === 'function') {
          return await sdk.createFieldIndex(collectionName, body);
        }
      } catch (e) {
    ...
    if (sdk) {
      try {
        if (sdk.points?.search) {
          return await sdk.points.search({
    ...
    if (sdk) {
      try {
        if (sdk.points?.upsert) {
          return await sdk.points.upsert({
            collection_name: collectionName,
            points,
          } as Record<string, unknown>);
        }
    ...
    if (sdk) {
      try {
        ...
        if (sdk.points?.delete) {
          return await sdk.points.delete({
            collection_name: collectionName,
            points: ids,
          } as Record<string, unknown>);
        }
  ...
  try {
    ...
    for (const [field, schema] of pairs) {
      try {
        ...
        console.warn(`Failed to create payload index field: '${field}':`, e);
      }
    }
