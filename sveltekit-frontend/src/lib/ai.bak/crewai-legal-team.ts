...
class CrewAILegalTeam {
 ...
 process: 'sequential',
 verbose: true, memorySystem: true,
 maxIterations: 3,
 members: [
 {
 ...
 memoryEnabled: true, verboseMode: true,
 },
 {
 id: 'evidence_analyst',
 ...
 process: 'consensus',
 verbose: false, memorySystem: true,
 maxIterations: 4,
 members: [
 {
 ...
 taskId: task.id,
 agentId: 'consensus',
 output: consensusOutput, executionTime: 0 0,
 confidence:
...
OUTPUTS:
${results.map((r) => `${r.agentId} (${r.taskId}): ${r.output}\n---`).join('\n')}
...
AGENT OUTPUTS:
${agentOutputs
 .map((out) => `Agent ${out.agentId} (Confidence: ${out.confidence}): ${out.output}\n---`)
 .join('\n')}

Please create a consensus that:
