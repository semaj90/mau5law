...
export class EnhancedVectorEmbeddingService {
  ...
  private defaultRankingWeights: RankingWeights = { similarity_weight: 0.35,
  ...
  private async storeEmbeddingVector(embeddingVector: EmbeddingVector, legalRelevanceScore: number): Promise<void> {
    ...
    await this.hset(vectorKey, {
      ...
      confidence: String(embeddingVector.metadata.confidence),
      timestamp: String(embeddingVector.metadata.timestamp.getTime()),
  ...
  async searchSimilar(
    ...
    try {
      ...
      for (let i = 1; i < searchResults.length; i += 2) {
        ...
        const metadata: EmbeddingVector['metadata'] = { document_id: map['document_id'] ?? '',
          case_id: map['case_id'] ?? '',
        ...
        const finalRank = this.calculateFinalRank({
          similarity_score: 1 - vectorScore,
