...
export const poiService = {
     ...
     * Update a POI
     */
    async updatePOI(poiId: string, data: Partial<POICreateRequest>): Promise<PersonOfInterest> {
        const response = await fetch(`${API_BASE}/${poiId}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(data),
     ...
     */
    async addAssociate(poiId: string, data: Partial<KnownAssociate>): Promise<KnownAssociate> {
     ...
     */
    async removeAssociate(poiId: string, associateId: string): Promise<void> {
