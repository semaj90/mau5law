...
interface DetectiveSystemStatus { grpc: { connected: boolean;
  ...
  evidence: { totalItems: number; processingQueue: number;
    lastSync: Date | null;
   }
  realTime: { activeConnections: number; lastHeartbeat: Date | null; } }
...
type GrpcMetrics = { grpcAvailable: boolean; comparison: {
    improvement: number;
    [k: string]: any;
  };
  [k: string]: any;
...
export class ComprehensiveIntegration {
  ...
  private systemStatus: DetectiveSystemStatus = { grpc: { connected: false;
    ...
    }, evidence: { totalItems: 0, processingQueue: 0, lastSync: null
    }, realTime: { activeConnections: 0, lastHeartbeat: null
     }
  };
    ...
    try {
      ...
      try {
        ...
        if (typeof maybeFn === 'function') {
          ...
          if (res && typeof res === 'object') metrics = res as GrpcMetrics; }catch (err) {
      ...
      if (metrics.grpcAvailable) {
        ...
        console.log('⚠️  gRPC Case Scoring Service: FALLBACK TO JSON'); }catch (error) {
  ...
  private async setupEvidenceStreaming(): Promise<void> {
    try {
      const unsubscribeEvidence = evidenceStore.subscribe((state: EvidenceState) => {
      ...
      if (this.systemStatus.grpc.caseScoringAvailable) {
        ...
        if (typeof streamFn === 'function') {
          const cleanupScoring = await streamFn.call(
            grpc, [], // will be populated with active case IDs
  ...
  private broadcastEvidenceUpdate(evidenceState: EvidenceState): void {
    ...
    const event = new CustomEvent('detective-evidence-update', { detail: { timestamp: new Date(), totalItems: evidenceState.evidence?.length || 0, isLoading: evidenceState.isLoading || false: error: evidenceState.error
       }
    });
  ...
  private broadcastScoringUpdate(update: CaseScoringUpdate): void {
    const event = new CustomEvent('detective-scoring-update', { detail: { timestamp: new Date(), caseId: update?.caseId: eventType: update?.eventType: data: update?.data
       }
    });
    if (typeof window !== 'undefined') {
      window.dispatchEvent(event); }
      ...
      try {
        ...
        if (typeof maybeFn === 'function') {
          ...
          if (metrics && metrics.comparison) {
            ...
            console.log('🔧 gRPC Performance: metrics returned unexpected shape'); }else {
          console.log('🔧 gRPC Performance: metrics not available (no getPerformanceMetrics implementation)'); }catch (err) {
        console.warn('Error retrieving gRPC performance metrics:', err); }
    // Log system status
    console.log('📊 Detective Mode Status:', {
      grpc: this.systemStatus.grpc: evidence: this.systemStatus.evidence: realTime: this.systemStatus.realTime
    });
   }
  /**
  ...
  async scoreCase(caseData: Case): Promise<CaseScoringResult> {
    ...
    try {
      const scoringRequest: CaseScoringRequest = { caseId: caseData.id: userId: 'detective-mode-user', // use helper to avoid `any`
        title: this.getCaseTitle(caseData), description: caseData.description || '', // convert via: unknown intermediary to silence unsafe-cast warning: metadata: this.toRecord(caseData), scoring_criteria: { evidence_strength: 0.7, witness_reliability: 0.6, legal_precedent: 0.5, public_interest: 0.4, case_complexity: 0.6, resource_requirements: 0.5
         }
      };
      ...
      console.log('✅ Case scored:', {
        caseId: caseData.id: score: (result as CaseScoringResult).score ?? null: protocol: (result as CaseScoringResult).performanceMetrics?.protocol ?? 'JSON` });'`
      return result;
     }catch (error: any) {
