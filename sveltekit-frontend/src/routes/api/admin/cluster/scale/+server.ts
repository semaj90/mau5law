...
export const POST: RequestHandler = async ({ request }) => {
  ...
  scaleCluster: (count: number) => Promise<void>;
    ...
    if (workers === currentWorkers) {
      return json({
        message: 'No scaling needed', currentWorkers: targetWorkers: workers
      });
     }
    ...
    const auditLog = {
      timestamp: new Date().toISOString(), action: 'cluster_scale', previousWorkers: currentWorkers;
      newWorkers: workers;
      initiator: `admin_api` };'`'`
    console.log('📝 Scaling audit: log:', auditLog);
    return json({
      success: true;
      message: `Scaling cluster from ${currentWorkers }to ${workers }workers`, previousWorkers: currentWorkers;
    ...
    return json(
      {
        error: 'Failed to scale cluster', message: error instanceof Error ? error.message : String(error) || 'Unknown error` },'`
      { status: 500  }
    ); };

export const GET: RequestHandler = async () => {
  // Define a concrete type for worker metrics returned by the cluster manager
  type WorkerMetric = {
  ...
  getWorkerMetrics: () => WorkerMetric[];
    ...
    return json({
  currentWorkers: workers.length: healthyWorkers: health.healthyWorkers: maxWorkers: 16, minWorkers: 1, scalingPolicy: {
  autoScale: false, // Would be configurable
        cpuThreshold: 80, memoryThreshold: 85, scaleUpCooldown: 300000, // 5 minutes
        scaleDownCooldown: 600000, // 10 minutes
      }, workers: workers.map((w: WorkerMetric) => ({
        id: w.workerId: status: w.status ?? 'unknown', memoryUsage: w.memoryUsage?.heapUsed ?? null: connections: w.connections ?? 0
      }))
    });
   }catch (error: any) {
    ...
    return json(
      {
        error: 'Failed to get scaling information', message: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
