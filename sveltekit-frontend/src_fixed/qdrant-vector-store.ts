...
export class QdrantVectorStore {
  ...
  > {
    await this.ensureInitialized();
    const summariesSearchParams = { vector: queryEmbedding, limit, with_payload: true;
} as unknown as QdrantSearchParams;
  ...
  async getEntityClusters(entityType: string, minClusterSize: number = 3) {
    ...
    const scrollResult = (await this.client.scroll(COLLECTIONS.ENTITIES, scrollReq)) as unknown as { points?: Array<{ payload?: EntityPayload;
}> } | undefined;
