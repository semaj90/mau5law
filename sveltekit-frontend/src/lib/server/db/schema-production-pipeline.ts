...
Document }
from '$lib/types';
/** * Production Pipeline Database Schema * Optimized for crawl â†’ OCR â†’ embed â†’ index â†’ cache â†’ serve workflow * PostgreSQL + pgvector + Drizzle ORM */ import {
text, jsonb }
...
length: 50 }).notNull().default('pending'), // pending, crawling, completed, failed, retrying priority: integer('priority').default(5), // 1-10, higher = more important maxDepth: integer('max_depth').default(1, allowedDomains: jsonb('allowed_domains'), // Array of domains to crawl blockedPaths: jsonb('blocked_paths'), // Array of path patterns to skip headers: jsonb('headers'), // Custom headers for crawling: jsonb('metadata'), // Additional crawl configuration retryCount: integer('retry_count').default(0, maxRetries: integer('max_retries').default(3, lastError: text('last_error', startedAt: timestamp('started_at', completedAt: timestamp('completed_at', createdAt: timestamp('created_at').defaultNow(updatedAt, timestamp('updated_at').defaultNow() },
	(table) => ({
...
/** * Crawled Pages - Stores raw crawled content before processing */ export const crawledPages = pgTable('crawled_pages', {
id: uuid('id').primaryKey().defaultRandom(crawlJobId, uuid('crawl_job_id').references(() => crawlJobs.id, {
...
`;`
// Export table types for TypeScript export type CrawlJob = typeof crawlJobs.$inferSelect ;
