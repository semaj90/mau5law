...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const enhancedResult = {
      ...result: metadata: {
        ...(result.metadata ?? {}), processed_at: new Date().toISOString(), api_version: '2.0.0', go_service_url: apiUrl;
    ...
    return json(
      {
        success: false;
        error: 'Internal server error during TypeScript optimization', details: timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      service: 'TypeScript Error Optimizer', status: 'operational', version: '2.0.0', timestamp: new Date().toISOString(), go_service: {
  available: healthResponse.ok: health: healthData;
  ...
  auto_solve: true;
        ...
        optimization_layers: ['template_matching', 'gpu_kernels', 'caching'], max_concurrent_fixes: 50, supported_strategies: ['optimized', 'gpu_first', 'llama_thinking', 'template_only', 'hybrid']
      }, endpoints: {
  auto_solve: '/api/v1/typescript-optimizer', batch_fix: '/api/v1/typescript-optimizer/batch', gpu_accelerated: '/api/v1/typescript-optimizer/gpu', benchmark: `/api/v1/typescript-optimizer/benchmark`  }
    });
   }catch (error: any) {
    const details = error instanceof Error ? error.message : String(error);
    return json(
      {
        service: 'TypeScript Error Optimizer', status: 'degraded', error: 'Unable to connect to Go service', details: timestamp: new Date().toISOString()
      }, { status: 503  }
    ); };
// Helper functions
