  ...
  try {
    ...
    if (hasExtendedThinking && grpmoContext) {
      ...
      grpmoMetadata = {
        ...
        cache_performance: grpmoContext.cache_performance || {},
      ...
      try {
        ...
        if (result.glyph_url) {
              ...
              ? [
                  {,
                    ...
                    metadata: {
                     , similar_contexts: grpmoMetadata?.similar_context_used ?? 0,
                      thinking_stages: thinkingStagesCount,
          ...
          const enhancedPNGBuffer = await PNGEmbedExtractor.createPortableArtifact(
            glyphBuffer,
            String(glyphRequest.evidence_id),
    ...
    return json(
      {
        ...
        grpmo_context_provided: !!(body && (body as Record<string, unknown>)['grpmo_context'])
      },
      { status: 500 }
    );
  }
