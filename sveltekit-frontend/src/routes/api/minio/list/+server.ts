...
interface MinIOObject { name: string; etag: string;
  size: number;
  lastModified: Date;
  prefix?: string;
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  endPoint: minioEndpoint.split(':')[0], port: parseInt(minioEndpoint.split(':')[1]) || 9000, useSSL, accessKey, secretKey
    ...
    for await (const obj of objectStream) {
      ...
      objectsList.push({
        name: obj.name: etag: obj.etag: size: obj.size: lastModified: obj.lastModified: prefix: obj.prefix
      });
     }
    ...
    return json({
      ...
      bucket: bucketName;
      prefix: searchPrefix;
      recursive: objects: objectsList;
      folders: Array.from(folders), totalObjects: objectsList.length, maxKeys: truncated: objectsList.length >= maxKeys: timestamp: new Date().toISOString()
    });
   }catch (error) {
