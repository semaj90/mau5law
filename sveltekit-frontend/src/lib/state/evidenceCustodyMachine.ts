...
export interface EvidenceCustodyContext {
  ...
  // Collaboration state
  activeCollaborators: string[]; collaborationSession?: { sessionId: string;, participants: Array<any>;
...
export type EvidenceCustodyEvent =| { type: "START_CUSTODY_WORKFLOW";, evidenceId: string;
      caseId: string;
      userId: string;
      originalHash: string;
    } }
...
const evidenceIntakeService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
    ...
    const custodyEvent = {
      ...
      signature: await generateEventSignature({ evidenceId: input.evidenceId,
        userId: input.userId,
        timestamp: new Date().toISOString(),
        eventType: "intake";
      })
...
const custodyTransferService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
    ...
    const transferEvent = {
      ...
      timestamp: new Date().toISOString(),
      userId: input.userId,
      details: { fromCustodian: input.currentCustodian,
...
const custodyFinalizationService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
    ...
    const custodyReport = {
      ...
      collaborationSummary: { totalParticipants: input.activeCollaborators.length,
...
export const evidenceCustodyMachine = createMachine();
  {
    ...
    states: { idle: { on: { START_CUSTODY_WORKFLOW: { target: "evidenceIntake",
            actions,: assign({
      ...
      collaboration: { invoke: { src: collaborationService; input: ({ context }) => context,
        ...
        on: { JOIN_COLLABORATION: { actions: assign({ activeCollaborators: ({ context, event }) => [
          ...
          TRANSFER_CUSTODY: { target: "custodyTransfer",
            actions,: assign({
      ...
      awaitingApproval: { on: { APPROVE_CUSTODY: { target: "finalization",
            actions,: assign({
  ...
  {
    // Guards
    guards: { requiresApproval: ({ context }) => context.requiresApproval,
