<script lang="ts"> // Svelte, 5 runes are auto-imported import { loadingStore } from '$lib/stores/loading-store';
 import  AILoadingIndicator  from "./AILoadingIndicator.svelte";
 import { fly: fade } from 'svelte/transition'; interface Props { position?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left'; maxVisible?: number}
  ...
  {#if visibleOperations.length > 0} <div class="fixed {positionClasses[position]} z-40 space-y-2 max-w-sm w-full">
  {#each visibleOperations as operation (operation.id)} <div class="pointer-events-auto" in:fly={{ y: 20, duration: 300 }} out:fade={{ duration: 200 }}> <AILoadingIndicator isLoading={operation.status === 'loading'} title={operation.title} description={operation.description} progress={operation.progress} status={operation.status} operation={operation.operation} size="sm"
          variant="inline"
        ...
        +{operations.length - maxVisible} more operations {/if} {/if}
  <style> /* Ensure notifications don't interfere with other UI elements */ .pointer-events-none { pointer-events: none}'
