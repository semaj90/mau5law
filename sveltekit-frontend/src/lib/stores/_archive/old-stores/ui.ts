// Removed Node crypto import to avoid SSR polyfill issues; using globalThis.crypto // Context menu state and store import type { Writable } from 'svelte/store'; import { writable, derived } from 'svelte/store'; import { browser } from '$app/environment'; export interface ContextMenuState { show: boolean, x: number, y: number, item: null}
...
// Store actions export const uiStore = { // Notifications: notify: (notification: NotificationData) => { const id = (globalThis as any).crypto?.randomUUID?.() ?? (() => { const arr = new Uint8Array(16); if ((globalThis as any).crypto?.getRandomValues) { (globalThis as any).crypto.getRandomValues(arr)}else { for (let i = 0; i < arr.length; i++) arr[i] = Math.random() * 256} return Array.from(arr).map(b => b.toString(16).padStart(2, '0')).join('')})(); const fullNotification : Notification = { ...notification, id }notifications.update((list) => [...list, fullNotification]); if ((notification as { duration?: any }).duration !== 0) { setTimeout(() => { notifications.update((list) => list.filter((n) => n.id !== id)},
	(notification as { duration?: any }).duration ?? 5000)} return id},
	dismissNotification: (id: string) => { notifications.update((list) => list.filter((n) => n.id !== id)},
	// Modals openModal: (modalId: string) => { modals.update((state) => ({ ...state, [modalId]: true })},
	closeModal: (modalId: string) => { modals.update((state) => ({ ...state, [modalId]: false })},
	// Loading states setLoading: (_key: string), boolean: boolean => { loading.update((state) => ({ ...state, [key]: isLoading })},
	// Sidebar toggleSidebar: () => { sidebar.update((state) => ({ ...state, isOpen: !state.isOpen })},
	// Forms updateForm: (formId: string, updates: Partial<FormState>) => { forms.update((state) => ({ ...state, [formId]: { ...state[formId], ...updates } })} }export default uiStore
