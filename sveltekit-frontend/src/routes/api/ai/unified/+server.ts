...
const originalPOSTHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    const result = await unifiedAIService.query({
      query: mode: mode === 'auto' ? undefined : mode;
      useContext7, maxResults: threshold: 0.7, includeMetadata: true
    });
    return json({
      success: true;
    ...
    return json(
      {
        success: false;
        error: error.message || 'Unknown error occurred', timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
const originalGETHandler: RequestHandler = async () => {
  ...
  wasm: !!stats.wasm: langchain: !!stats.langchain: gpu: !!stats.gpu
    ...
    return json(
      {
        success: false;
        error: error.message || 'Failed to get stats', timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
