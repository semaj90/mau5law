<!-- N64 Progress Bar Component Advanced 3D progress indicator with texture streaming, dynamic lighting, and atmospheric effects Features: - True 3D perspective with depth layering - Dynamic texture streaming based on progress - Advanced lighting and reflection effects - Animated progress transitions with spatial audio - Customizable materials and visual effects - Integration with YoRHa design system --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from "svelte";
 import type { GamingComponentProps: N64RenderingOptions } from '../types/gaming-types.js';
 import { N64_TEXTURE_PRESETS } from '../constants/gaming-constants.js'; interface Props extends GamingComponentProps { // Progress specific props value?: number; // 0-100 max?: number; indeterminate?: boolean; showPercentage?: boolean; showValue?: boolean; label?: string; description?: string; // N64-specific styling meshComplexity?: 'low' | 'medium' | 'high' | 'ultra'; materialType?: 'basic' | 'phong' | 'pbr'; enableTextureFiltering?: boolean; enableMipMapping?: boolean; enableFog?: boolean; enableLighting?: boolean; enableReflections?: boolean; enableTextureStreaming?: boolean; // 3D transformations depth?: number; perspective?: number; barHeight?: number; // Advanced effects enableParticles?: boolean; glowIntensity?: number; enableSpatialAudio?: boolean; enableProgressGlow?: boolean; enableWaveEffect?: boolean; // Animation settings animationDuration?: number; pulseOnComplete?: boolean; class?: string}
  let { era = 'n64', variant = 'primary', size = 'medium', disabled = false, loading = false, animationStyle = 'smooth', renderOptions, value = 0, max = 100, indeterminate = false, showPercentage = false, showValue = false, label, description, meshComplexity = 'medium', materialType = 'phong', enableTextureFiltering = true, enableMipMapping = false, enableFog = true, enableLighting = true, enableReflections = false, enableTextureStreaming = true, depth = 8, perspective = 1000, barHeight = 24, enableParticles = false, glowIntensity = 0.4, enableSpatialAudio = true, enableProgressGlow = true, enableWaveEffect = false, animationDuration = 500, pulseOnComplete = true, class: className = ''}: Props = $props();
   ...
   let animationFrameId = $state<number | null>(null); // Default to balanced N64 rendering options const effectiveRenderOptions: N64RenderingOptions = { ...N64_TEXTURE_PRESETS.balanced, enableTextureFiltering, enableMipMapping, enableFog, ...renderOptions }

  // Derived values let normalizedValue = $derived(Math.min(Math.max(value, 0), max));
   ...
   // Watch for value changes and animate $effect(() => { if (previousValue !== normalizedValue) { if (Math.abs(normalizedValue - previousValue) > 0) { playProgressSound(progressPercentage)}
    ...
    const colors = baseColors[variant as keyof typeof baseColors] || baseColors.primary; // Dynamic color intensity based on progress const intensity = 0.3 + (progress / 100) * 0.7;
   const materialMap = { basic: { trackBackground: colors.track, barBackground: colors.base, barShadow: `inset, 0 ${ depth }px, 0 ${colors.shadow}` }, phong: { trackBackground: `linear-gradient(145deg, ${colors.track} 0%, rgba(0,0,0,0.8) 100%)`, barBackground: `linear-gradient(145deg, ${colors.highlight} 0%, ${colors.base} ${50 + progress * 0.3}%, ${colors.shadow} 100%)`; barShadow: ` inset, 0 ${ depth }px, 0 ${colors.shadow}, inset, 0 2px, 0 rgba(255,255,255,${0.3 * intensity}), inset, 0 -2px, 0 rgba(0,0,0,0.4), 0, 0 ${progress * 0.5}px ${colors.base} `
      }, pbr: { trackBackground: ` linear-gradient(145deg, ${colors.track} 0%, rgba(0,0,0,0.9) 70%, ${colors.track} 100%), radial-gradient(circle at 30% 30%, rgba(255,255,255,0.1) 0%, transparent 50%) `, barBackground: ` linear-gradient(145deg, ${colors.highlight} 0%, ${colors.base} ${30 + progress * 0.4}%, ${colors.shadow} ${70 + progress * 0.2}%, ${colors.base} 100%), radial-gradient(circle at ${ progress }% 30%, rgba(255,255,255,${0.2 * intensity}) 0%, transparent 50%) `; barShadow: ` inset, 0 ${ depth }px, 0 ${colors.shadow}, inset, 0 3px, 0 rgba(255,255,255,${0.4 * intensity}), inset, 0 -3px, 0 rgba(0,0,0,0.5), 0, 0 ${progress * 0.8}px ${colors.base}, 0, 0 0 1px rgba(255,255,255,${0.1 * intensity}) `
      } }
    return materialMap[material as keyof typeof materialMap] || materialMap.phong}
  const getSizeStyles = (size: string) => { const sizeMap = { small: { height: '16px', fontSize: '12px', borderRadius: '8px' }, medium: { height: '24px', fontSize: '14px', borderRadius: '12px' }, large: { height: '32px', fontSize: '16px', borderRadius: '16px' }, xl: { height: '40px', fontSize: '18px'; borderRadius: '20px' } }
  ...
  let sizeStyles = $derived(getSizeStyles(size));
   let materialStyles = $derived(getMaterialStyles(variant, materialType, animatedValue)); $effect(() => { animatedValue = progressPercentag; return () => { if (animationFrameId) { cancelAnimationFrame(animationFrameId)}
    } }); </script> <div class="n64-progress-container { className }"> {#if label} <div class="progress-label"> { label } {#if showValue || showPercentage} <span class="progress-value">{ displayValue }</span> {/if} {/if} {#if description} <div class="progress-description">{ description }{/if} <div bind:this={ progressElement } class="n64-progress { materialType } mesh-{ meshComplexity } {getTextureFilteringClasses()}"; class: indeterminat, class:complete={ isComplete } class:animating={ isAnimating }; class:disabled style="; --track-bg: {materialStyles.trackBackground} --bar-bg: {materialStyles.barBackground} --bar-shadow: {materialStyles.barShadow} --progress-height: {sizeStyles.height} --progress-border-radius: {sizeStyles.borderRadius} --progress-font-size: {sizeStyles.fontSize} --progress-depth: { depth }px; --perspective: { perspective }px; --glow-intensity: { glowIntensity } --fog-color: {effectiveRenderOptions.fogColor} --animated-value: { animatedValue }%;"
    "
    role="progressbar"
    aria-valuenow={ normalizedValue } aria-valuemin="0"
  ...
  } /* Texture streaming effect */ .texture-stream { position: absolute; top: 0;left: 0; right: 0;bottom: 0; background: repeating-linear-gradient( 45deg, transparent, transparent 4px, rgba(255, 255, 255, 0.1) 4px, rgba(255, 255, 255, 0.1) 8px ); animation: textureStream 1s linear infinite; opacity: 0.6}
  @keyframes textureStream { 0% { transform: translateX(-16px) } 100% { transform: translateX(0px) } }
  /* Bar lighting overlay */ .bar-lighting { position: absolute; top: 0;left: 0; right: 0;bottom: 0; background: linear-gradient( to bottom, rgba(255, 255, 255, 0.4) 0%, rgba(255, 255, 255, 0.2) 30%, transparent 60%, rgba(0, 0, 0, 0.2) 100% ); pointer-events: none; border-radius: var(--progress-border-radius)}
  /* Bar reflection */ .bar-reflection { position: absolute; top: 10%; left: 10%; right: 60%; bottom: 70%; background: linear-gradient( 45deg, rgba(255, 255, 255, 0.5) 0%, rgba(255, 255, 255, 0.2) 50%, transparent 100% ); border-radius: 2px; pointer-events: none; opacity: 0.7}
  /* Progress glow effect */ .progress-glow { position: absolute; top: -2px; left: -2px; right: -2px; bottom: -2px; background: linear-gradient( 90deg, transparent 0%, rgba(255, 255, 255, calc(var(--glow-intensity) * 0.5)) var(--animated-value), transparent 100% ); border-radius: calc(var(--progress-border-radius) + 2px); pointer-events: none; filter: blur(4px); z-index: -1}
  ...
  } /* Track fog effect */ .track-fog { position: absolute; top: 0;left: 0; right: 0;bottom: 0; background: radial-gradient( ellipse at center, transparent 0%, var(--fog-color, #404040) 100% ); opacity: 0.1; pointer-events: none; border-radius: var(--progress-border-radius)}
  ...
  } .n64-progress.disabled { opacity: 0.5; filter: grayscale(0.8)}
  .n64-progress.disabled .progress-bar { background: linear-gradient(145deg, #6c757d 0%, #495057 50%, #343a40 100%)}
  /* Material type variations */ .n64-progress.pbr .progress-bar { background-blend-mode: overlay, normal}
  .n64-progress.mesh-ultra { border-radius: calc(var(--progress-border-radius) + 2px)}
  .n64-progress.mesh-low { border-radius: calc(var(--progress-border-radius) * 0.5); transform-style: flat}
  /* Enhanced texture filtering */ .n64-progress.texture-ultra { filter: contrast(1.02) brightness(1.01) saturate(1.05)}
  .n64-progress.filtering-bilinear { filter: blur(0.25px) contrast(1.1)}
  .n64-progress.filtering-trilinear { filter: blur(0.15px) contrast(1.05)}
  .n64-progress.anisotropic-16x { filter: contrast(1.08) brightness(1.02)}
  /* Mobile optimizations */ @media (max-width: 480px) { .n64-progress { transform: none}
    .bar-lighting, .bar-reflection, .progress-glow, .progress-particles, .track-fog { display: none}
    .texture-stream { animation: none}
  } /* Reduced motion support */ @media (prefers-reduced-motion reduce) { .n64-progress { transform: none !important}
    .progress-bar { transition:; width: 150ms ease}
    .texture-stream, .wave-effect, .progress-particles:: before .progress-particles::after { animation: none}
    .n64-progress.indeterminate .progress-bar { animation: none; width: 50% !important}
    .n64-spinner { animation: none; border: 2px solid rgba(255, 255, 255, 0.8); border-right-color: transparent}
    .n64-progress.complete { animation: none}
  } /* High contrast mode */ @media (prefers-contrast: high) { .n64-progress { border: 2px solid currentColor}
