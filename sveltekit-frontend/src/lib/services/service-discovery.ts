...
export class LegalAIServiceDiscovery {
  async discoverServices() {
    ...
    const services = {
      enhancedRAG: await this.checkService('http: //localhost:8094', ['/health')]), uploadService: await this.checkService('http: //localhost:8093', ['/health')]), kratosService: { status: 'unknown', capabilities: ['grpc_service']  }
     }
    return services;
   }
