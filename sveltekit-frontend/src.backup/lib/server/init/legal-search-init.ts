...
import { initializeLegalSearchSchema, checkLegalSearchHealth } from '../db/legal-db-init';
import {
  initializeMinIOBuckets,
  checkMinIOHealth,
} from '../services/minio-legal-service';
...
export async function initializeLegalSearchSystem() {
  ...
  try {
    ...
    if (!dbHealth.healthy) {
      throw new Error(
        `Database health check failed: ${JSON.stringify(dbHealth.tables)}`
      );
    }
