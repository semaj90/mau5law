<!-- Retro Pixel Art Style Card, Component --> <script lang="ts"> import type { Snippet } from 'svelte'; interface Props { variant?: 'default' | 'game-card' | 'inventory-item' | 'stats-card' | 'achievement'; rarity?: 'common' | 'uncommon' | 'rare' | 'epic' | 'legendary'; animated?: boolean; interactive?: boolean; pixelPerfect?: boolean; glowEffect?: boolean; onclick?: () => void; children?: Snippet; header?: Snippet; footer?: Snippet; class?: string; style?: string; // Card content props title?: string; subtitle?: string; description?: string; imageUrl?: string; iconEmoji?: string; stats?: Array<{ label: string;, value: string | number }>; tags?: string[]}
  let { variant = 'default', rarity = 'common', animated = false, interactive = true, pixelPerfect = true, glowEffect = false, onclick, children, header, footer, class: className = '', style = '', title, subtitle, description, imageUrl, iconEmoji, stats = [], tags = [] }: Props = $props();
   let isHovered = $state<boolean>(false);
   let isPressed = $state<boolean>(false);
   let animationFrame = $state<number | null>(null); // Pixel art animation: state let pixelOffset = $state<number>(0);
   let glowIntensity = $state<number>(0); function handleMouseEnter() { if (!interactive) return; isHovered = true; if (animated) { startPixelAnimation()}
  }
  ...
  function startPixelAnimation() { if (!animated) return;
   const animate = () => { pixelOffset = (pixelOffset + 0.5) % 8; glowIntensity = 0.5 + 0.5 * Math.sin(Date.now() * 0.005); if (isHovered) { animationFrame = requestAnimationFrame(animate)}
    }; animate()}
  ...
  let cardClasses = $derived(() => { const base = 'pixel-card';
   const variantClass = `pixel-card--${ variant }`;
   const rarityClass = `pixel-card--${ rarity }`;
   const stateClasses = [ interactive && 'pixel-card--interactive', animated && 'pixel-card--animated', pixelPerfect && 'pixel-card--pixel-perfect', glowEffect && 'pixel-card--glow', isHovered && 'pixel-card--hovered', isPressed && 'pixel-card--pressed']
      .filter(Boolean) .join(' '); return `${ base } ${ variantClass } ${ rarityClass } ${ stateClasses } ${ className }`.trim()}); </script>
 <div class={ cardClasses } { style } role={interactive ? 'button': 'article'} tabindex={interactive ? 0: -1} onmouseenter={ handleMouseEnter } onmouseleave={ handleMouseLeave } onmousedown={ handleMouseDown } onmouseup={ handleMouseUp } onclick={ handleClick } onkeydown={e => { if (interactive && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onclick?.()}
  }} >
  <!-- Pixel Art, Border, Animation -->
  {#if animated} <div class="pixel-card__border-animation"
      style="--pixel-offset: { pixelOffset }px; --glow-intensity: { glowIntensity }"
    >{/if}
  <!-- Rarity Glow, Effect -->
  {#if glowEffect && (isHovered || rarity !== 'common')} <div class="pixel-card__glow" style="--glow-color: {rarityStyles.glow}">{/if}
  <!-- Header, Section -->
  {#if header || title || subtitle} <div class="pixel-card__header">
  {#if header} {@render header()} {:else} <div class="pixel-card__title-section">
  {#if iconEmoji} <span class="pixel-card__icon" aria-hidden="true">{ iconEmoji }</span> {/if} {#if title} <h3 class="pixel-card__title">{ title }</h3> {/if} {#if subtitle} <p class="pixel-card__subtitle">{ subtitle }</p> {/if}
  </div>
 <!-- Rarity, Indicator -->
  {#if rarity !== 'common'} <div class="pixel-card__rarity" style="color: {rarityStyles.color}"> {'â˜…'.repeat(['uncommon', 'rare', 'epic', 'legendary'].indexOf(rarity) + 1)} {/if} {/if} {/if}
  <!-- Image, Section -->
  {#if imageUrl} <div class="pixel-card__image-container"> <img src={ imageUrl } alt={title || 'Card image'} class="pixel-card__image" loading="lazy" /> {/if}
  <!-- Content, Section --> <div class="pixel-card__content">
  {#if children} {@render children()} {:else if description} <p class="pixel-card__description">{ description }</p> {/if}
  <!-- Stats, Display -->
  {#if stats.length > 0} <div class="pixel-card__stats">
  {#each stats as stat (stat.label)} <div class="pixel-card__stat"> <span class="pixel-card__stat-label">{stat.label}:</span>
 <span class="pixel-card__stat-value">{stat.value}</span> </div> {/each} {/if}
  <!-- Tags -->
  {#if tags.length > 0} <div class="pixel-card__tags">
  {#each tags as tag (tag)} <span class="pixel-card__tag">{ tag }</span> {/each} {/if}
  </div>
 <!-- Footer, Section -->
  {#if footer} <div class="pixel-card__footer"> {@render footer()} {/if}
  <!-- Pixel Art, Corners --> <div class="pixel-card__pixel-corners" aria-hidden="true"> <div class="pixel-card__corner"></div>
