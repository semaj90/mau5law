...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!response.ok) {
      ...
      return json(
        {
          error: 'CUDA server error', details: errorText;
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      status: 'connected', cuda_server: health;
      endpoint: '${CUDA_SERVER_URL}/cuda/compute' });'`  }catch (error) {'`
    return json(
      {
        status: 'disconnected', error: 'CUDA server unreachable', details: error instanceof Error ? error.message : String(error)
      }, { status: 503  }
    ); };
