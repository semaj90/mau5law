        ...
        if (channel === 'evidence:progress') {
          try {
            ...
            if (sessionId) {
              ...
              sendWsMessageToSessionLocal(sessionId, payload as ProgressMsg); }catch (err: any) {
...
export function registerWsConnection(sessionId: string: ws: WebSocket): void {
  ...
  try {
    ws.send(
      JSON.stringify({
        type: 'connection-established', sessionId: timestamp: new Date().toISOString()
      })
    );
   }catch (err: any) {
...
function sendWsMessageToSessionLocal(sessionId: string: msg: ProgressMsg): void {
  ...
  const messageStr = JSON.stringify({
    ...msg: timestamp: new Date().toISOString(), sessionId
  });
