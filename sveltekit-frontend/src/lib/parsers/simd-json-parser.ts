...
export class SIMDJSONParser {
    ...
    constructor(options: SIMDParseOptions = {}) {
        ...
        console.log(`🧠 SIMD buffers initialized`);
    }
    ...
    async parseDocument(jsonString: string): Promise<ParsedLegalDocument> {
        ...
        try {
            ...
            const document = await this.enhancedDocumentParsing(rawData);
     ...
     */
    private async enhancedDocumentParsing(rawData: any): Promise<ParsedLegalDocument> {
     ...
     */
    private vectorizedChunking(textBytes: Uint8Array, originalText: string): TextChunk[] {
	...
	index: chunkIndex,
            ...
            chunkIndex++;
            if (chunkStart >= originalText.length) break; // Prevent infinite loop if text is short
            if (chunkStart < 0) chunkStart = 0; // sanity check
        }
        return chunks;
    }
    ...
    private findSentenceBoundariesSIMD(textBytes: Uint8Array): number[] {
        ...
        for (let i = 0; i < textBytes.length - 1; i++) {
            ...
            if (sentenceEnders.includes(byte)) {
                // Check if followed by space(32) or newline(10)
    ...
    private normalizeEmbeddingSIMD(embeddings: Float32Array): Float32Array {
        ...
        // Unrolled loop for better vectorization (chunks of 4)
        for (; i < len - 3; i += 4) {
