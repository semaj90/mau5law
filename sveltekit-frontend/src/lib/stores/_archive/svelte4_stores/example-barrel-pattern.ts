/** * Example Barrel Stores Pattern - Svelte, 5 with TypeScript * * This file demonstrates the modern barrel stores pattern using Svelte, 5 runes. * Benefits: * - Type-safe centralized state management * - Auto-completion in IDEs * - Easy to import import { counterStore, todoStore } from '$lib/stores' * - Performance optimized with fine-grained reactivity */ // ================================================== // Example 1: Simple Counter Store // ================================================== export const counterStore = (() => { let count = $state<number>(0); let doubled = $derived(count * 2); return { get count() { return count},
	...
	// Actions addTodo: (text: string) => { todos.push({ id: crypto.randomUUID(text: completed, false: new Date() })},
