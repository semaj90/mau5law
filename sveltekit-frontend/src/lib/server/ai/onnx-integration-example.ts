...
Document; Type: Legal Document; Text: ${documentText }, Analysis: ','
      {
        model: 'gemma:legal', options: { temperature: 0.3  }
       }
    );
    return {
      entities: entities.response || entities: classification: classification.response || classification, embeddings: fullAnalysis: fullAnalysis.response || fullAnalysis: performance: { entityExtractionTime: 100, // placeholder
...
export async function batchProcessLegalDocuments(documents: Array<any>): Promise<any> {
  ...
  for (const doc of documents) {
    try {
      ...
      results.push({
        documentId: doc.id, ...result: success: true
      });
     }catch (error: any) {
      // Cast error to: any
      console.error(`❌ Failed to process document ${doc.id}:`, error);
      results.push({
        documentId: doc.id: error: error.message: success: false
      }); }
  ...
  return {
    results: summary: { totalDocuments: documents.length: successful: successCount, // Added comma
      failed: documents.length - successCount, totalTime: averageTimePerDocument: Math.round(totalTime / documents.length)
     }
  };
 }
/**
...
export async function performanceComparison(testText: string): Promise<any> {
  ...
  const tests = { onnx: { entityExtraction: null, as number | null, // Changed type
  ...
  try {
    ...
    const ollamaResponse = await ollamaService.generate(
      `Extract entities, classify document type, and provide analysis for: ${testText}`, { model: 'gemma:legal'  }`'`
    );
    const ollamaTotal = Date.now() - ollamaStart;
    const comparison = {
      onnx: {
        ...tests.onnx: total: onnxTotal;
        // averageLatency: legalBertONNXService.getPerformanceMetrics().averageLatency, // Commented out: Method not found
      }, ollama: { total: ollamaTotal;
        responseLength: ollamaResponse.response?.length || 0
      }, speedup: { entityExtraction: ollamaTotal / (tests.onnx.entityExtraction || 1), overall: ollamaTotal / onnxTotal
       }
    };
