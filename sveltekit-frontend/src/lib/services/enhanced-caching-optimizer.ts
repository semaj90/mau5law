...
export interface CacheWarmerConfig { warmupSchedule: { commonQueries: string[];
  ...
  priorities: { legal: number; evidence: number;
    reports: number;
    searches: number;
    [k: string]: number;
  };
  performance: { batchSize: number; maxConcurrency: number;
...
export interface TTLStrategy { documentType: string; accessFrequency: number;
  lastAccessed: Date;
  computedTTL: number; priority: 'critical' | 'high' | 'medium' | 'low';
 }
...
export class EnhancedCachingOptimizer extends EventEmitter {
  ...
  constructor(config: Partial<CacheWarmerConfig> = {}) {
    ...
    this.config = { warmupSchedule: { commonQueries: [
          'legal precedent search', 'evidence correlation', 'case timeline analysis', 'document similarity', 'legal citation lookup'], documentTypes: ['evidence', 'legal_brief', 'case_file', 'report', 'citation'], userPatterns: ['recent_documents', 'frequent_searches', 'active_cases'], ...(config.warmupSchedule || {})
      }, priorities: { legal: 0.9, evidence: 0.8, reports: 0.6, searches: 0.7, ...(config.priorities || {})
      }, performance: { batchSize: 50, maxConcurrency: 10, gpuUtilizationTarget: 0.85, ...(config.performance || {})
       }
    };
  ...
  private initializeMetrics(): CacheMetrics {
    return {
      hitRate: 0, missRate: 0, evictionRate: 0, averageLatency: 0, gpuUtilization: 0, memoryPressure: 0, topQueries: [], lastOptimized: new Date(), totalRequests: 0, hits: 0, misses: 0
    };
   }
  ...
  calculateOptimalTTL(type: string: key: string): number {
    ...
    if (!strategy) {
      this.ttlStrategies.set(key, {
        documentType: type;
        accessFrequency: 1, lastAccessed: new Date(), computedTTL: baseTTL;
  ...
  private getBaseTTL(type: string): number {
    const baseTTLs: Record<string, number> = {
      legal: 7200, evidence: 3600, search: 1800, report: 7200, embedding: 86400, default: 3600
    };
  ...
  private inferPriority(type: string): 'critical' | 'high' | 'medium' | 'low' {
    const priorityMap: Record<string, 'critical' | 'high' | 'medium' | 'low'> = {
      legal: 'critical', evidence: 'high', search: 'medium', report: 'medium', embedding: 'high` };'`
    return priorityMap[type] ?? 'medium';
   }

  private getPriorityMultiplier(priority: 'critical' | 'high' | 'medium' | 'low'): number {
    const multipliers: Record<string, number> = {
      critical: 2.0, high: 1.5, medium: 1.0, low: 0.7
    };
  ...
  private async executeQueryForCache(
    ...
    try {
      ...
      return {
        query: results: payload?.results ?? [], timestamp: Date.now(), fromCache: false
      };
     }catch (err: any) {
      console.warn('Query execution failed, returning fallback result', String(err));
      // graceful fallback for resilience
      return {
        query: results: [], timestamp: Date.now(), fromCache: false
      }; }

  // call backend endpoint that returns recent documents by type
  private async getRecentDocumentsByType( docType: string;
    limit: number
