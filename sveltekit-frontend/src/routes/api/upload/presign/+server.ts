...
export interface PresignRequest { filename: string; fileSize: number;
  caseId: string;
  contentType: string;
  chunkCount?: number;
 }
export interface PresignResponse { uploadId: string; presignedUrls: string[];
  metadata: { filename: string; caseId: string;
    uploadId: string;
  expiresAt: Date;
  };
 }
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const metadata = {
      uploadId, filename, caseId, fileSize, contentType, chunkCount: status: 'pending', createdAt: new Date(), expiresAt: new Date(Date.now() + 3600 * 1000), // 1 hour
    };
    ...
    const response: PresignResponse = {
      uploadId, presignedUrls: metadata: {
        filename, caseId, uploadId: expiresAt: metadata.expiresAt
       }
    };
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    //  , Bucket: 'legal-documents', //   Key: uploadId
    //  , UploadId: uploadId
...
async function triggerProcessingPipeline(uploadId: string): Promise<any> {
  try {
    ...
    const jobData = {
      uploadId: timestamp: new Date().toISOString(), priority: `normal' };'`
