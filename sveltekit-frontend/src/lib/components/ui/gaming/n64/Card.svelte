<!-- N64 Card Component Advanced 3D card container with depth layering, texture filtering, and atmospheric effects Features: - True 3D perspective transformations with depth layering - Advanced texture filtering and anti-aliasing - Fog effects and atmospheric depth - Interactive hover and focus states with spatial audio - Integration with YoRHa design system - Multiple material types (basic, phong, PBR) --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from "svelte";
 ...
 import { N64_TEXTURE_PRESETS } from '../constants/gaming-constants.js'; interface Props extends GamingComponentProps { // Card specific props padding?: 'none' | 'small' | 'medium' | 'large' | 'xl'; elevation?: number; // 3D depth in pixels clickable?: boolean; hoverable?: boolean; // N64-specific styling meshComplexity?: 'low' | 'medium' | 'high' | 'ultra'; materialType?: 'basic' | 'phong' | 'pbr'; enableTextureFiltering?: boolean; enableMipMapping?: boolean; enableFog?: boolean; enableLighting?: boolean; enableReflections?: boolean; enableAtmosphere?: boolean; // 3D transformations rotationX?: number; rotationY?: number; rotationZ?: number; perspective?: number; layerDepth?: number; // Advanced effects enableParticles?: boolean; glowIntensity?: number; enableSpatialAudio?: boolean; enableDepthShadows?: boolean; // Content slots header?: unknown; footer?: unknown; children?: unknown; class?: string}
  ...
  // Create spatial audio feedback const playCardSound = async (frequency: number; duration: number = 0.2) => { if (!enableSpatialAudio) return; try { if (!audioContext) { audioContext = new (window.AudioContext || (window as: unknown).webkitAudioContext()}
   ...
   const reverbNode = audioContext.createConvolver(); // Configure 3D spatial audio pannerNode.panningModel = 'HRTF'; pannerNode.positionX.setValueAtTime(mouseX / window.innerWidth - 0.5, audioContext.currentTime); pannerNode.positionY.setValueAtTime(mouseY / window.innerHeight - 0.5, audioContext.currentTime); pannerNode.positionZ.setValueAtTime(-elevation / 100, audioContext.currentTime); // Create reverb for atmospheric depth const impulseLength = audioContext.sampleRate * 0.3;
   const impulse = audioContext.createBuffer(2, impulseLength, audioContext.sampleRate);
   const impulseL = impulse.getChannelData(0);
   const impulseR = impulse.getChannelData(1); for (let i = 0; i < impulseLength; i++) { const decay = Math.pow(1 - i / impulseLength, 2); impulseL[i] = (Math.random() * 2 - 1) * decay * 0.2; impulseR[i] = (Math.random() * 2 - 1) * decay * 0.2}
      reverbNode.buffer = impul; // Connect audio chain oscillator.connect(pannerNode); pannerNode.connect(reverbNode); reverbNode.connect(gainNode); gainNode.connect(audioContext.destination); // Rich harmonic content for depth oscillator.type = 'sawtooth'; oscillator.frequency.setValueAtTime(frequency, audioContext.currentTime); oscillator.frequency.exponentialRampToValueAtTime(frequency * 0.8, audioContext.currentTime + duration); gainNode.gain.setValueAtTime(0.15, audioContext.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + duration); oscillator.start(); oscillator.stop(audioContext.currentTime + duration)} catch (error) { console.warn('Could not play card sound:', error)}
  } const handleMouseMove = (_event: MouseEvent) => { if (!cardElement || disabled) return;
   const rect = cardElement.getBoundingClientRect(); mouseX = event.clientX; mouseY = event.clientY; // Calculate relative position for 3D tilt const relativeX = ((event.clientX - rect.left) / rect.width) * 2 - 1;
   const relativeY = ((event.clientY - rect.top) / rect.height) * 2 - 1}
  const handleClick = async () => { if (!clickable || disabled || loading) return; isPressed = true; await playCardSound(440, 0.3); setTimeout(() => { isPressed = false}, 200); onClick?.(); // ondispatch removed}
  const handleHover = () => { if (disabled || !hoverable) return; isHovered = true; playCardSound(550, 0.15); onHover?.(); // ondispatch removed}
  ...
  // Dynamic 3D transformations based on interaction state let dynamicRotationX = $derived(rotationX + (isHovered ? mouseY * 0.05: 0) + (isPressed ? 2: 0));
 ...
 <div bind:this={ cardElement } role={clickable ? 'button': 'region'} tabindex={clickable ?, 0: -1} onclick={ handleClick } onmouseenter={ handleHover } onmouseleave={ handleUnhover } onmousemove={ handleMouseMove } onfocus={ handleFocus } onblur={ handleBlur } onkeydown={ handleKeyDown } class="n64-nier-bits-card { className } { materialType } mesh-{ meshComplexity } {getTextureFilteringClasses()}"
  class: clickable class:hoverable, class: disabled, class:loading, class:hovered={ isHovered } class:focused={ isFocused }; class:pressed={ isPressed } style="; --material-bg: {materialStyles.background} --material-border: {materialStyles.borderColor} --material-shadow: {materialStyles.boxShadow} --card-padding: { paddingStyle } --transform-3d: { transform3D } --fog-color: {effectiveRenderOptions.fogColor} --glow-intensity: { glowIntensity } --card-elevation { dynamicElevation }px; --layer-depth: { layerDepth }px;"
  "
  aria-disabled={ disabled } aria-busy={ loading } >
  {#if header} <div class="nier-bits-yorha-panel-header"> {@render header()} {/if}
...
/* Enhanced texture filtering */ .n64-card.texture-ultra { -webkit-font-smoothing: antialiased; -moz-osx-font-smoothing: grayscale; filter: contrast(1.02) brightness(1.01) saturate(1.05)}
  .n64-card.filtering-bilinear { filter: blur(0.25px) contrast(1.1)}
  .n64-card.filtering-trilinear { filter: blur(0.15px) contrast(1.05)}
  .n64-card.anisotropic-4x { filter: blur(0.1px) contrast(1.03)}
  .n64-card.anisotropic-8x { filter: blur(0.05px) contrast(1.05)}
  .n64-card.anisotropic-16x { filter: contrast(1.08) brightness(1.02)}
