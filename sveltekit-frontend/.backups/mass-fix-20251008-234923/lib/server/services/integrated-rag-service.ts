...
export async function initializeIntegratedRAG() {
  ...
  if (!minioClient) {
    ...
    } catch { console.warn('⚠️ MinIO unavailable'); }
  }
  ...
  if (!qdrantClient) {
    ...
    } catch { console.warn('⚠️ Qdrant unavailable'); }
  }
}
...
export async function processDocument(file: File, content: string) {
  ...
  if (minioClient) {
    ...
    } catch (e) { console.warn('⚠️ MinIO upload failed'); }
  }
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    } catch (e) { console.error(`❌ Chunk ${i} insert failed`); }
  }
  ...
  if (qdrantClient) {
    ...
    } catch { console.warn('⚠️ Qdrant storage failed'); }
  }

  lokiCollection.insert({ id: documentId, title: filename, content, chunks: chunks.length, timestamp: Date.now() });
  rebuildFuseIndex();

  return { documentId, filename, chunks: chunks.length, minioUrl, qdrantStored: !!qdrantClient, cudaUsed: cudaAvailable };
}
...
export async function searchSimilarDocuments(query: string, limit: number = 5): Promise<SearchResult[]> {
  ...
  if (qdrantClient) {
    try {
      const qdrantResults = await qdrantClient.search('legal-documents', { vector: queryEmbedding, limit, with_payload: true });
      ...
      return results;
    } catch { console.warn('⚠️ Qdrant search failed'); }
  }
  ...
  } catch (e) { console.error('❌ pgvector search failed'); }
...
export async function getSystemHealth() {
  await initializeIntegratedRAG();
  return { database: !!queryClient, redis: !!redisClient, minio: !!minioClient, qdrant: !!qdrantClient, cuda: cudaAvailable, loki: lokiCollection.count(), fuse: !!fuseInstance };
}
