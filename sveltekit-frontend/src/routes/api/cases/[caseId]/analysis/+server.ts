...
export const POST: RequestHandler = async ({ params, locals, request }) => {
  ...
  try {
    ...
    const currentCaseResults = await db
      .select()
      .from(cases)
      .where(eq(cases.id, caseId)) // Fixed: Added closing parenthesis for eq and where
      .limit(1);
    ...
    const recentActivities = await db
      .select()
      .from(caseActivities)
      .where(eq(caseActivities.caseId, caseId)) // Fixed: Added closing parenthesis for eq and where
  ...
  prompt: prompt, // Fixed: Added comma; max_tokens: 512
      ...
      if (analysisResponse.ok) {
        ...
        return json({
          success: true;
          analysis: analysisData.response: source: 'Local LLM', context: ragContext
        });
       }else {
        throw new Error('Analysis service unavailable'); }catch (vectorError) {
      ...
      return json({
        ...
        source: 'Fallback Analysis', context: basicContext
      }); }catch (error: any) {
