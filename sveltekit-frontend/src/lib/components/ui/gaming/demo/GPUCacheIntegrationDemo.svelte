...
<script lang="ts">
  ...
  let N64TextureFilteringCache = $state<any>(null);
  interface Props {
  ...
  $effect(() => {
    ...
    cacheActor = createActor(graphCacheMachine) as any;
    cacheActor.subscribe((state: any) => {
      machineState = state?.value ?? machineState;
  ...
  onMount(async () => {
    try {
      const mod1: any = await import('../8bit/NES8BitButton.svelte');
      NES8BitButton = mod1?.default ?? mod1?.NES8BitButton ?? mod1;
    } catch (e) {
      // ignore load errors — component will simply not render
      NES8BitButton = null;
    }
    try {
      const mod2: any = await import('../16bit/SNES16BitButton.svelte');
      SNES16BitButton = mod2?.default ?? mod2?.SNES16BitButton ?? mod2;
    } catch {
      SNES16BitButton = null;
    }
    try {
      const mod3: any = await import('../n64/N643DButton.svelte');
      N643DButton = mod3?.default ?? mod3?.N643DButton ?? mod3;
    } catch {
      N643DButton = null;
    }
    try {
      const mod4: any = await import('../n64/N64TextureFilteringCache.svelte');
      N64TextureFilteringCache = mod4?.default ?? mod4?.N64TextureFilteringCache ?? mod4;
    } catch {
