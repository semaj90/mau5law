...
export class DocumentProcessingAPI {
  ...
  constructor(baseUrl: string = import.meta.env.VITE_API_BASE_URL || 'http://localhost:3000', token?: string) {
   ...
   */
  async retryPage(documentId: string, pageNumber: number): Promise<ProcessingResponse> {
    return this.sendCommand({
      action: 'retry',
      documentId,
      pageNumber,
  ...
  > {
    try {
      const response = await fetch(`${this.baseUrl}/api/document-processing/history/${documentId}`, {
        headers: {
          ...(this.token && { Authorization: `Bearer ${this.token}` }),
        },
      });
   ...
   */
  async uploadDocument(file: File, metadata?: Record<string, string>): Promise<{
    documentId: string;
    fileName: string;
    fileSize: number;
    uploadedAt: string;
  ...
  async getResults(documentId: string): Promise<{
    ...
    try {
      const response = await fetch(`${this.baseUrl}/api/document-processing/results/${documentId}`, {
        headers: {
          ...(this.token && { Authorization: `Bearer ${this.token}` }),
        },
      });
