        ...
        NEW_CONVERSATION: {
          actions: assign({
            currentConversation: ({ event }) => {
              const title = event.type === "NEW_CONVERSATION" ? event.title || "New Conversation" : "New Conversation";
              return {
        ...
        DELETE_CONVERSATION: {
          actions: assign({
            ...
            currentConversation: ({ context, event }) => {
              ...
              return context.currentConversation?.id === event.conversationId ? null : context.currentConversation;
            }
          })
        },
        UPDATE_SETTINGS: {
    ...
    sendingMessage: {
        ...
        currentConversation: ({ context, event }) => {
          ...
          if (!context.currentConversation) {
            const conversation: Conversation = {
              id: crypto.randomUUID(),
              title: event.message.slice(0, 50) + (event.message.length > 50 ? "..." : ""),
              createdAt: new Date(),
              messages: []
            };
            return conversation;
      ...
      invoke: {
        ...
        onError: {
          target: "error",
          actions: assign({
            error: ({ event }) => new Error(event.error?.message || "Unknown error")
          })
        }
      },
      on: {
    ...
    streaming: {
      ...
      on: {
        ...
        STREAM_ERROR: {
          target: "error",
          actions: assign({
            error: ({ event }) => event.type === "STREAM_ERROR" ? event.error : null,
            stream: () => null
          })
        }
      }
