 ...
 */
import { text, jsonb, pgTable, timestamp, uuid, integer, boolean, decimal, varchar, index } from 'drizzle-orm/pg-core';
...
export const shaderCacheEntries = pgTable("shader_cache_entries", {
    ...
    embedding: text("embedding"), // pgvector embedding stored as text since helper doesn't support generic vector type easily here
    ...
    metadata: jsonb("metadata").default("{}"), // Audit fields
    ...
    // embeddingIdx: index("shader_embedding_idx").using("ivfflat", table.embedding).with({ lists: 100 }) // Commented out to avoid build errors if vector extension missing type support
}));
