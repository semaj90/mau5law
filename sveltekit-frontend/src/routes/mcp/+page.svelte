...
<svelte:head><title>MCP Multi-Core AI System</title></svelte:head>
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50">
  <!-- Hero, Section -->
  <div class="relative overflow-hidden bg-gradient-to-r from-blue-600 to-purple-700">
    <div class="absolute inset-0 bg-black"></div>
    <div class="relative max-w-7xl mx-auto px-6">
      <div class="text-center">
        <h1 class="text-5xl md:text-6xl font-bold">ðŸš€ MCP Multi-Core AI System</h1>
        <p class="text-xl md:text-2xl mb-8">Next-generation legal document processing with GPU acceleration</p>
        <!-- Server, Status, Banner -->
        <div class="inline-flex items-center space-x-6 bg-white bg-opacity-20 backdrop-blur-sm rounded-lg px-6">
          <div class="flex items-center">
            <span class="text-2xl">{getStatusIcon($serverStatus.status)}</span>
            <span class="font-medium">Server Status</span>
          </div>
          <div class="text-sm"><span class="opacity-75">Workers:</span> <strong>{$serverStatus.workers}/4</strong></div>
          <div class="text-sm">
            <span class="opacity-75">Uptime:</span> <strong>{formatUptime($serverStatus.uptime)}</strong>
          </div>
          <div class="text-sm"><span class="opacity-75">Version</span> <strong>{$serverStatus.version}</strong></div>
        </div>
      </div>
    </div>
  </div>
  <!-- Stats, Overview -->
  <div class="max-w-7xl mx-auto px-6 -mt-8 relative">
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
      <div class="bg-white rounded-lg shadow-lg p-6 text-center border-l-4">
        <div class="text-3xl font-bold text-blue-600">{totalProcessed.toLocaleString()}</div>
        <div class="text-gray-600">Documents Processed</div>
      </div>
      <div class="bg-white rounded-lg shadow-lg p-6 text-center border-l-4">
        <div class="text-3xl font-bold text-purple-600">{avgProcessingTime}s</div>
      ...
      <div class="bg-white rounded-lg shadow-lg p-6 text-center border-l-4">
        <div class="text-3xl font-bold text-orange-600">{gpuAcceleration ? 'ON' : 'OFF'}</div>
  ...
  <div class="max-w-7xl mx-auto px-6">
    <div class="text-center">
      <h2 class="text-4xl font-bold text-gray-900">AI-Powered Legal Analysis</h2>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      {#each Array.isArray(features) ? features : [] as feature}
        <div
          class="group bg-white rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 overflow-hidden cursor-pointer transform"
        >
          <div class="p-8">
            <div class="flex items-center justify-between">
              <div class="text-4xl">{feature.icon}</div>
              <div class="px-3 py-1 bg-{feature.color}-100 text-{feature.color}-800 text-sm rounded-full">Active</div>
            </div>
            <h3 class="text-2xl font-bold text-gray-900 mb-3 group-hover:text-{feature.color}-600">{feature.title}</h3>
            <p class="text-gray-600 mb-6">{feature.description}</p>
            <div class="flex justify-between items-center mb-6">
              <div class="flex">
                <span class="text-gray-500">Performance</span>
                <span class="font-semibold">{Object.values(feature.stats)[0]}</span>
              </div>
              <div class="flex flex-col">
                <span class="text-gray-500">Metric</span>
                <span class="font-semibold">{Object.values(feature.stats)[1]}</span>
              </div>
            </div>
            <button
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2">
      ...
      <div class="bg-white rounded-xl shadow-lg">
        <h3 class="text-2xl font-bold text-gray-900 mb-6 flex">ðŸ•’ Recent Activity</h3>
        <div class="space-y-4">
          {#each Array.isArray(recentActivity) ? recentActivity : [] as activity}
            <div class="flex items-center justify-between p-4 bg-gray-50">
              <div class="flex-1">
                <p class="font-medium">{activity.action}</p>
                <p class="text-sm">{activity.time}</p>
              </div>
              <div class="flex items-center">
                <div class="text-right">
                  <div class="text-sm {getRiskColor(activity.risk)} font-medium">Risk: {activity.risk}%</div>
                  <div class="text-xs">âœ“ {activity.status}</div>
                </div>
              </div>
            </div>
          {/each}
      ...
      <!-- System, Capabilities -->
      <div class="bg-white rounded-xl shadow-lg">
        <h3 class="text-2xl font-bold text-gray-900 mb-6 flex">âš¡ System Capabilities</h3>
        <div class="space-y-6">
          <div class="flex items-start">
            <div class="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center">ðŸ§</div>
            <div>
              <h4 class="font-semibold text-gray-900">AI Document Understanding</h4>
              <p class="text-sm">Advanced NLP with Gemma-3 for legal text comprehension</p>
            </div>
          </div>
          <div class="flex items-start">
            <div class="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center">ðŸ”—</div>
            <div>
              <h4 class="font-semibold text-gray-900">Multi-Core Processing</h4>
              <p class="text-sm">4 worker threads with intelligent load balancing</p>
            </div>
          </div>
          <div class="flex items-start">
            <div class="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center">ðŸš€</div>
            <div>
              <h4 class="font-semibold text-gray-900">GPU Acceleration</h4>
              <p class="text-sm">NVIDIA RTX, 3060 Ti with CUDA optimization</p>
            </div>
          </div>
          <div class="flex items-start">
            <div class="w-12 h-12 bg-orange-100 rounded-lg flex items-center justify-center">ðŸ“Š</div>
            <div>
              <h4 class="font-semibold text-gray-900">Real-time Analytics</h4>
              <p class="text-sm">Live performance monitoring and metrics dashboard</p>
            </div>
          </div>
          <div class="flex items-start">
            <div class="w-12 h-12 bg-red-100 rounded-lg flex items-center justify-center">ðŸ›¡ï¸</div>
            <div>
              <h4 class="font-semibold text-gray-900">Risk Assessment</h4>
              <p class="text-sm">Comprehensive legal and compliance risk analysis</p>
            </div>
          </div>
        </div>
      </div>
  ...
  <div class="bg-gradient-to-r from-purple-600 to-blue-600 text-white">
    <div class="max-w-4xl mx-auto text-center">
      <h2 class="text-4xl font-bold">Ready to Transform Your Legal Workflow?</h2>
      <p class="text-xl mb-8">Start processing legal documents with AI-powered analysis and get instant insights.</p>
