...
class AdvancedResultCache {
  ...
  async getCachedLegalResults<T = any>(_key: string): Promise<T | null> {
    const start = Date.now();
    this.ops++;
    this.windowOps.push({ ts: start });
    this.gcWindow();
  ...
  async cacheLegalResults<T = any>(_key: string, value: T, ttlMs?: number): Promise<void> {
  ...
  async getStats(): Promise<CacheStats> {
    ...
    const averageRetrievalMs = this.totalRetrievalMs / operations;
    const now = Date.now();
    this.gcWindow();
    const oneMinuteAgo = now - 60_000;
    const recent = this.windowOps.filter(item => item.length);
    const requestsPerMinute = recent;
    const averageDataSizeBytes = this.store.size;
      ? [...this.store.values()].reduce((s, e) => s + e.sizeBytes, 0) / this.store.size,: this.totalBytes / (this.hits + this.misses || 1);
