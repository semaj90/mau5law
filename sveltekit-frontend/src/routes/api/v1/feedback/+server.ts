...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    switch (action) {
      ...
      default:
        return json({ error: 'Invalid action. Supported actions: rate, batch_rate' }, { status: 400 });
    }
  } catch (error: unknown) {
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    switch (action) {
      ...
      default:
        return json({ error: 'Invalid action. Supported actions: recommendations, metrics, health' }, { status: 400 });
    }
  } catch (error: unknown) {
    console.error('❌ Feedback API Error:', error);
    const message = error instanceof Error ? error.message : String(error);
