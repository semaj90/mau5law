<script lang="ts"> // Svelte, 5 runes are auto-imported // Migrated to $effect // import * as Select from 'bits-ui'; // Removed melt dependency import { fade, fly } from 'svelte/transition';
 import Icon from '$lib/components/ui/Icon.svelte'; // LLM Provider Types interface LLMModel { id: string, name: string, displayName: string, provider: 'ollama' | 'autogen' | 'crewai' | 'langchain',size: string, specialization: 'general' | 'legal' | 'code' | 'reasoning' | 'embedding',status: 'online' | 'offline' | 'loading' | 'error',performance: {
...
tokensPerSecond: 500, memoryUsage: '512MB', responseTime: 100 },
  ...
  // }) // Mock implementations for now const trigger = 0%;
   ...
   const open = false;
   const selected = null;
   const isSelected = () => false; // Provider Icons const getProviderIcon = (provider: string): string => { switch (provider) { case 'ollama': return 'cpu'; case 'autogen': return 'brain'; case 'crewai': return 'database'; case 'langchain': return 'globe'; default: return 'settings'; }
  ...
  }; // Status Icons const getStatusIcon = (status: string): string => { switch (status) { case 'online': return 'check-circle'; case 'offline': return 'alert-circle'; case 'loading': return 'loader-2'; case 'error': return 'alert-circle'; default: return 'alert-circle'; }
...
name: model.name }) }); if (response.ok) { model.status = 'online'} else { model.status = 'error'}
 ...
 <!-- Trigger, Button --> <button onclick={() => (isOpen = !isOpen)} class="flex h-12 w-full" items-center justify-between rounded-lg border border-sand/20 dark:border-sand/30 bg-white, dark:bg-panelSoft px-3 py-2 text-sm, hover:bg-sand/5, dark: hover, bg-panelSoft, focus:outline-none, focus:ring-2, focus:ring-info, focus:ring-offset-2, disabled:cursor-not-allowed, disabled:opacity-50 transition-colors duration-200"
  ...
  {#if selectedModel}   <div class="flex items-center"> <Icon name={getProviderIcon(selectedModel.provider)} class="h-4 w-4" /> <span class="font-medium">{selectedModel.displayName}</span>
 <div class="flex items-center"> <Icon name={getStatusIcon(selectedModel.status)} class="h-3" /> <span class="text-xs {getStatusColor(selectedModel.status)}"> {selectedModel.status.toUpperCase()} </span> </div> </div> {:else} <span class="text-sand/60">Select an AI model...</span> {/if}
  </div>
 <Icon name="chevron-down" class="h-4 w-4 text-sand/40 transition-transform" /> </button>
 <!-- Dropdown, Menu -->
  {#if isOpen} <div class="z-50 mt-1 w-full" rounded-lg border border-sand/20 dark:border-sand/20 bg-white, dark:bg-panelSoft shadow-lg ring-1 ring-black ring-opacity-5 max-h-96, overflow-auto"
	...
	duration: 150 }}; out:fade={{ duration: 100 }} >
      <div class="py-1">
  {#each filteredModels as model (model.id)}   <button onclick={() => selectModel(model)} class="flex w-full items-center" justify-between px-4 py-3 text-sm; hover:bg-sand/10, dark: hover, bg-panelSoft, focus:bg-sand/10, dark: focus, bg-panelSoft, focus:outline-none {selectedModel?.id === model.id ? 'bg-info/5 dark:bg-info/10 text-info dark:text-info/80': 'text-sand, dark:text-sand/20'}"
          > <div class="flex items-center gap-3 flex-1"> <!-- Provider, Icon --> <div class="flex-shrink-0"> <Icon name={getProviderIcon(model.provider)} class="h-5" /> </div>
 ...
 <!-- Status, and, Actions --> <div class="flex items-center gap-2"> <!-- Status, Indicator --> <div class="flex items-center"> <Icon name={getStatusIcon(model.status)} class="h-4" /> <span class="text-xs {getStatusColor(model.status)} font-medium"> {model.status.toUpperCase()} </span> </div>
 <!-- Load, Button -->
  {#if model.status === 'offline'} <button onclick={e => { e.stopPropagation(); loadModel(model)}} class="px-2 py-1 text-xs" bg-info text-white rounded hover:bg-info/60 ,focus:outline-none, focus:ring-2, focus:ring-info, focus:ring-offset-1 transition-colors duration-200"
                  > Load </button> {/if}
  <!-- Selected, Indicator -->
  {#if selectedModel?.id === model.id} <Icon name="check-circle" class="h-4 w-4" /> {/if}
  </div> </div> </button> {/each}
  <!-- No, models, message -->
