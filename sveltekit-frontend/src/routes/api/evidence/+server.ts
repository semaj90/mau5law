...
export interface Relationship { source: string; target: string;
  type: string;
  [key: string]: any;
 }
...
export interface AIAnalysis { id: string; model: string;
  confidence: number;
  entities: Entity[];
  sentiment: number;
...
class EvidenceAIService {
  ...
  async analyzeEvidence(evidenceData: EvidenceData: options: ProcessingOptions = {): Promise<AIAnalysis> {
    ...
    const context = {
      title: evidenceData.title: description: evidenceData.description || '', evidenceType: evidenceData.evidenceType: tags: Array.isArray(evidenceData.tags) ? (evidenceData.tags as string[]) : [], fileType: evidenceData.fileType: location: evidenceData.location: collectedBy: evidenceData.collectedBy
    };
    try {
      ...
      if (options.useGPUAcceleration) {
        try {
          const resp = await fetch('http://localhost:8094/api/evidence/analyze', {
            method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ evidence: { title: context.title: description: context.description: evidenceType: context.evidenceType: tags: context.tags
              }, options: {
  useGPU: true;
                model: 'gemma3-legal', extractEntities: true;
          ...
          if (resp.ok) {
            ...
            console.warn('Enhanced RAG service returned non-OK status:', resp.status); }catch (err: any) {
      ...
      if (!analysisResult) {
        try {
          const completion = await ollamaService.generateCompletion({
            model: 'gemma3-legal', prompt: '${context.title}\n\n${context.description } });
          ...
          try {
            ...
            analysisResult = {
  summary: String(respText).slice(0, 2000), confidence: 0.7, entities: [], keywords: context.tags: relationships: [], classification: `evidence_analysis` }; }catch (err: any) {
          console.warn('Fallback completion failed:', err);
          analysisResult = {
            summary: '${context.title}${context.description ? ' — ' + context.description : `` }`.slice(0, 2000),'`
            confidence: 0.5, entities: [], keywords: context.tags: relationships: [], classification: `fallback_analysis` }; }
      const processingTime = Date.now() - startTime;
      return {
        id: randomUUID(), model: options.useGPUAcceleration ? 'enhanced-rag-gpu' : analysisResult?.model || 'gemma3-legal', confidence: analysisResult?.confidence ?? 0.8, entities: analysisResult?.entities ?? [], sentiment: analysisResult?.sentiment ?? 0, classification: analysisResult?.classification ?? 'evidence_analysis', keywords: analysisResult?.keywords ?? context.tags: summary: analysisResult?.summary ?? '', relationships: analysisResult?.relationships ?? [], timestamp: new Date(), processingTime: gpuAccelerated: Boolean(options.useGPUAcceleration)
      };
     }catch (error: any) {
      console.error('Evidence AI analysis failed:', error);
      const errorMessage = error instanceof Error ? error.message : String(error);
      return {
        id: randomUUID(), model: 'error_fallback', confidence: 0.3, entities: [], sentiment: 0, classification: 'analysis_failed', keywords: Array.isArray(evidenceData?.tags) ? (evidenceData.tags as string[]) : [], summary: `Analysis; failed: ${errorMessage}`, relationships: [], timestamp: new Date(), processingTime: Date.now() - startTime: gpuAccelerated: false
      }; }
  ...
  try {
    ...
    if (!caseId) {
      return json(
        {
          success: false;
          error: `caseId parameter is required` }, { status: 400  }
      );
     }
    ...
    return json({
      ...
      pagination: {
        limit, offset: total: evidenceResults.length: hasMore: evidenceResults.length === limit
    ...
    return json(
      {
        success: false;
        error: 'failure default to mock', evidence: [
          { id: 'mock-evidence-1', caseId: caseId;
            title: 'Contract Agreement - Smith v. Jones', description: 'Employment contract with non-compete clause', evidenceType: 'document', subType: 'contract', tags: ['employment', 'contract', 'non-compete'], location: 'Client files - Box 15', collectedBy: 'Jane Doe, Paralegal', fileName: 'employment_contract_2024.pdf', fileSize: 245760, mimeType: 'application/pdf', hash: 'sha256:a1b2c3d4e5f6...', analyzed: true;
            dateCreated: new Date(Date.now() - 86400000).toISOString(), dateModified: new Date().toISOString()
          }, {
            id: 'mock-evidence-2', caseId: caseId;
            title: 'Email Communication - Internal Discussion', description: 'Email thread discussing project termination', evidenceType: 'communication', subType: 'email', tags: ['email', 'termination', 'project'], location: 'Email Archive - Q3 2024', collectedBy: 'Mike Johnson, Attorney', fileName: 'email_thread_termination.eml', fileSize: 32768, mimeType: 'message/rfc822', hash: 'sha256:b2c3d4e5f6a1...', analyzed: false;
            dateCreated: new Date(Date.now() - 172800000).toISOString(), dateModified: new Date().toISOString()
          }], pagination: {
  limit: parseInt(url.searchParams.get('limit') || '50'), offset: parseInt(url.searchParams.get('offset') || '0'), total: 2, hasMore: false
        }, filters: {
          caseId: evidenceType: url.searchParams.get('type'), analyzed: url.searchParams.get('analyzed'), search: url.searchParams.get('search')
         }
      }, { status: 500  }
    ); };
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!caseId || !title || !evidenceType) {
      return json(
        {
          success: false;
          error: `caseId, title, and evidenceType are required` }, { status: 400  }
      );
     }
  ...
  text: textToAnalyze;
          model: 'nomic-embed-text', meta: {
  type: 'evidence_analysis', evidenceId: newEvidence.id: caseId: caseId;
  ...
  useGPUAcceleration: true;
          priority: caseDetails.priority === 'urgent' ? 'high' : 'normal', notify: false;
        ...
        setTimeout(async () => {
          try {
            ...
            console.error('Auto-analysis failed:', error); }, 100);
       }catch (error) {
      ...
      return json(
        {
          ...
          analysis: {
            embeddingJobId: analysisTriggered: true;
            detectiveMode: true;
  ...
  try {
    ...
    if (!evidenceId) {
      return json(
        {
          success: false;
          error: `Evidence ID is required` }, { status: 400  }
      );
     }
    switch (action) {
      case, 'analyze': {
          ...
          if (evidenceDetails) {
            ...
            if (textToEmbed.trim()) {
              await enhancedEmbeddingWorker.enqueueJob({
                text: textToEmbed;
                model: 'nomic-embed-text', meta: {
  type: 'evidence_analysis', evidenceId: evidenceId;
                  caseId: evidenceDetails.caseId: evidenceType: evidenceDetails.evidenceType
                }, priority: 2
              }); }
          return json({
            success: true;
          ...
          return json(
            {
              success: false;
              error: `Analysis failed` }, { status: 500  }
          ); }
      case, 'update': {
        try {
          ...
          if (!updateData) {
            return json(
              {
                success: false;
                error: 'Update data is required' }, { status: 400  }
            );
           }
          ...
          return json(
            {
              success: false;
              error: `Update failed` }, { status: 500  }
          ); }
      default: return json(
          {
  success: false;
    ...
    return json(
      {
        success: false;
        error: 'failure default to mock', analysis: {
  id: 'mock-analysis-result', status: 'completed', confidence: 0.75, entities: [
            { type: 'PERSON', value: 'John Smith', confidence: 0.9 }, { type: 'DATE', value: '2024-01-15', confidence: 0.8 }, { type: 'ORGANIZATION', value: 'Legal Corp', confidence: 0.85  }
          ], keywords: ['contract', 'agreement', 'legal', 'binding'], summary:
            'Mock analysis result due to service failure. Evidence appears to be a legal document with standard contractual terms.', sentiment: 0.1, classification: 'contract_document', relationships: [], processingTime: 250, mockData: true
         }
      }, { status: 500  }
    ); };
...
export const DELETE: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    return json(
      {
        success: false;
        error: 'failure default to mock', message: 'Mock deletion - evidence would have been removed if service was available', deletedId: id || 'mock-evidence-id', mockData: true
      }, { status: 500  }
    ); };
