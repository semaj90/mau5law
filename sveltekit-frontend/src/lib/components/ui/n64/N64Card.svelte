<!-- @migration-task Error while migrating Svelte code: Mixing old (onmousemove) and new syntaxes for event handling is not allowed. Use only the onmousemove, syntax, https://svelte.dev/e/mixed_event_handler_syntaxes --> <!-- @migration-task Error while migrating Svelte; code: Mixing old (onmousemove) and new syntaxes for event handling is not allowed. Use only the onmousemove, syntax --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from "svelte";
 import type { Snippet } from 'svelte'; // Svelte, 5 props interface interface Props { children?: Snippet; title?: string; subtitle?: string; class?: string; // N64 specific props materialType?: 'basic' | 'phong' | 'pbr'; meshComplexity?: 'low' | 'medium' | 'high' | 'ultra'; depthEffect?: 'shallow' | 'medium' | 'deep' | 'extreme'; textureFiltering?: 'nearest' | 'bilinear' | 'trilinear' | 'anisotropic'; antiAliasing?: 'none' | 'fxaa' | 'msaa'; fogEffect?: boolean; parallaxDepth?: number; // 0-10 scale spatialAudio?: boolean; lightingModel?: 'flat' | 'gouraud' | 'phong' | 'blinn-phong'; shadowCasting?: boolean; // Interactive props hoverable?: boolean; clickable?: boolean; selectable?: boolean; selected?: boolean; // Accessibility ariaLabel?: string; ariaDescribedby?: string; role?: string; tabindex?: number; // Mobile optimizations mobileOptimized?: boolean; reducedMotion?: boolean; // Performance settings gpuAcceleration?: boolean; webgpuMode?: boolean; renderDistance?: 'near' | 'medium' | 'far'}
  let { children, title, subtitle, class: className = '', // N64 specific defaults materialType = 'phong', meshComplexity = 'medium', depthEffect = 'medium', textureFiltering = 'bilinear', antiAliasing = 'fxaa', fogEffect = false, parallaxDepth = 3, spatialAudio = false, lightingModel = 'phong', shadowCasting = true, // Interactive defaults hoverable = true, clickable = false, selectable = false, selected = false, // Accessibility defaults ariaLabel, ariaDescribedby, role = 'article', tabindex, // Mobile defaults mobileOptimized = true, reducedMotion = false, // Performance defaults gpuAcceleration = true, webgpuMode = false, renderDistance = 'medium'}: Props = $props(); // Events now handled via props in Svelte, 5 // let cardElement: HTMLElement, let container: HTMLDivElement, let isHovered = $state<boolean>(false);
   ...
   let lastFrameTime = $state<number>(0); // 3D transformation state let rotationX = $state<number>(0);
   let rotationY = $state<number>(0);
   let translateZ = $state<number>(0);
   let mouseX = $state<number>(0);
   let mouseY = $state<number>(0); // Generate unique IDs const componentId = `n64-card-${Math.random.toString-substr(2, 9)}`; // Dynamic CSS classes based on props const cardClasses = $derived(() => { const classes = ['n64-card']; // Material and mesh styling classes.push(`material-${ materialType }`); classes.push(`mesh-${ meshComplexity }`); classes.push(`depth-${ depthEffect }`); classes.push(`filter-${ textureFiltering }`); classes.push(`aa-${ antiAliasing }`); classes.push(`lighting-${ lightingModel }`); classes.push(`distance-${ renderDistance }`); // State classes if (isHovered && hoverable) classes.push('hovered'); if (isPressed && clickable) classes.push('pressed'); if (selected && selectable) classes.push('selected'); if (fogEffect) classes.push('fog-enabled'); if (shadowCasting) classes.push('shadow-casting'); // Interaction classes if (hoverable) classes.push('hoverable'); if (clickable) classes.push('clickable'); if (selectable) classes.push('selectable'); // Performance classes if (gpuAcceleration) classes.push('gpu-accelerated'); if (webgpuMode) classes.push('webgpu-active'); if (mobileOptimized) classes.push('mobile-optimized'); if (reducedMotion) classes.push('reduced-motion'); return classes.join(' ')}); // Dynamic inline styles for 3D transformations const cardStyles = $derived(() => { const depthZ = getDepthValue(depthEffect);
   const parallaxZ = parallaxDepth * 2; return ` --depth-z: ${ depthZ }px; --parallax-depth: ${ parallaxZ }px; --rotation-x: ${ rotationX }deg; --rotation-y: ${ rotationY }deg; --translate-z: ${ translateZ }px; --mouse-x: ${ mouseX } --mouse-y: ${ mouseY } --frame-time: ${frameCount * 0.016} `}); function getDepthValue(depth: string): number { const depthMap = {
      'shallow': 5;medium': 15,
      'deep': 30;extreme': 50 }
  ...
  function animationLoop(timestamp: number) { frameCount++; lastFrameTime = timestamp; // Update 3D transformations for complex materials if (materialType === 'pbr' && !reducedMotion) { const time = frameCount * 0.016; rotationY = Math.sin(time * 0.5) * 2; translateZ = Math.sin(time * 0.3) * 1}
    ...
    // Continue animation: for complex configurations if (materialType === 'pbr' || meshComplexity === 'ultra' || depthEffect === 'extreme') { requestAnimationFrame(animationLoop)}
  }
  function handleMouseMove(_event: MouseEvent) { if (!hoverable || reducedMotion) return;
   const rect = cardElement.getBoundingClientRect();
   const centerX = rect.left + rect.width / 2;
   const centerY = rect.top + rect.height / 2;
   const deltaX = (event.clientX - centerX) / rect.width;
   const deltaY = (event.clientY - centerY) / rect.height; mouseX = deltaX; mouseY = deltaY; // Update 3D rotation based on mouse position const maxRotation = 15; rotationY = deltaX * maxRotatio; rotationX = -deltaY * maxRotatio; // Update depth translation translateZ = Math.abs(deltaX * deltaY) * 5; // Update spatial audio position if (spatialPanner) { spatialPanner.positionX.setValueAtTime(deltaX * 10, audioContext!.currentTime); spatialPanner.positionY.setValueAtTime(deltaY * 10, audioContext!.currentTime)}
  }
  function handleMouseEnter() { if (!hoverable) return; isHovered = true; playSpatialSound('hover', 880, 0.03); // ondispatch removed}
  function handleMouseLeave() { if (!hoverable) return; isHovered = false; rotationX = 0; rotationY = 0; translateZ = 0; mouseX = 0; mouseY = 0; // ondispatch removed}
  ...
  } </script> <div bind:this={ container } class="n64-nier-bits-card-container { className }"
  style={cardStyles()} >
  <articl; bind:this={ cardElement } class={cardClasses()} { role } { tabindex } aria-label={ ariaLabel } aria-describedby={ ariaDescribedby } onmousemove={ handleMouseMove } onmouseenter={ handleMouseEnter } onmouseleave={ handleMouseLeave } onmousedown={ handleMouseDown } onmouseup={ handleMouseUp } onclick={ handleClick } onkeydown={ handleKeydown } >
    <!-- N64 Visual Enhancement, Layers --> <div class="n64-nier-bits-card-overlay" aria-hidden="true"> <!-- Depth effect, layer --> <div class="depth-layer"></div> <!-- Texture filtering, layer --> <div class="texture-filter-layer"></div> <!-- Anti-aliasing, layer --> {#if antiAliasing !== 'none'} <div class="aa-layer">{/if} <!-- Fog effect, layer --> {#if fogEffect} <div class="fog-layer">{/if} <!-- Shadow, layer --> {#if shadowCasting} <div class="shadow-layer">{/if} </div> <!-- Card, Header --> {#if title || subtitle} <header class="n64-nier-bits-yorha-panel-header"> {#if title} <h3 class="n64-nier-bits-nes-text">{ title }</h3> {/if} {#if subtitle} <p class="n64-nier-bits-card-subtitle">{ subtitle }</p> {/if} </header> {/if} <!-- Card, Content --> <div class="n64-nier-bits-yorha-panel-content"> {#if children} {@render children()} {/if} </div> <!-- Parallax depth, indicators --> {#if parallaxDepth > 0} <div class="parallax-indicators" aria-hidden="true"> {#each Array(Math.min(parallaxDepth, 5)) as _, i} <div class="parallax-layer"
            style="transform: translateZ({-i * 3}px); opacity: {1 - (i * 0.15)}"
          ></div> {/each} {/if} <!-- Selection, indicator --> {#if selectable} <div class="selection-indicator"
        class:visible={ selected } aria-hidden="true"
      >{/if} </article> </div> <style> .n64-card-container { position: relative; display: inline-block; font-family: 'Press Start 2P', monospace; /* 3D perspective for N64 depth */ perspective: 1000px; perspective-origin: center center; transform-style: preserve-3d}
/* Base card styling */ .n64-card { position: relative; padding: 20px;background: linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 100%); color: #e0e0e0;border: 2px solid #505050; /* N64-style 3D depth transformation */, transform: perspective(1000px) rotateX(var(--rotation-x, 0deg)) rotateY(var(--rotation-y, 0deg)) translateZ(var(--translate-z, 0px)); transform-origin: center center; transform-style: preserve-3d; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); box-shadow: 0 var(--depth-z, 15px) calc(var(--depth-z, 15px) * 2) rgba(0, 0, 0, 0.3), inset, 0 2px 4px rgba(255, 255, 255, 0.1), inset, 0 -2px 4px rgba(0, 0, 0, 0.2)}
/* Material variations */ .material-basic .n64-card { background: #1a1a1a; border-color: #505050; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.2), inset, 0 1px, 0 rgba(255, 255, 255, 0.1)}
  .material-phong .n64-card { background: linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 50%, #0a0a0a 100%); border-color: #707070; box-shadow: 0 var(--depth-z, 15px) calc(var(--depth-z, 15px) * 2) rgba(0, 0, 0, 0.4), inset, 0 3px 6px rgba(255, 255, 255, 0.15), inset, 0 -3px 6px rgba(0, 0, 0, 0.3)}
  .material-pbr .n64-card { background: linear-gradient(145deg, hsl(calc(var(--frame-time, 0) * 10 + 240), 20%, 16%) 0%, hsl(calc(var(--frame-time, 0) * 10 + 240), 25%, 10%) 50%, hsl(calc(var(--frame-time, 0) * 10 + 240), 30%, 6%) 100%); border-color: hsl(calc(var(--frame-time, 0) * 10 + 240), 50%, 50%); box-shadow: 0 var(--depth-z, 15px) calc(var(--depth-z, 15px) * 3) rgba(0, 0, 0, 0.5), inset, 0 4px 8px rgba(255, 255, 255, 0.2), inset, 0 -4px 8px rgba(0, 0, 0, 0.4)}
/* Mesh complexity variations */ .mesh-low .n64-card { clip-path: polygon(0% 0%, 100% 0%, 100% 100%, 0% 100%)}
  .mesh-medium .n64-card { clip-path: polygon( 0% 0%, 95% 0%, 100% 5%, 100% 95%, 95% 100%, 5% 100%, 0% 95%, 0% 5% )}
  .mesh-high .n64-card { clip-path: polygon( 0% 0%, 90% 0%, 95% 3%, 100% 10%, 100% 90%, 97% 95%, 90% 100%, 10% 100%, 5% 97%, 0% 90%, 0% 10%, 3% 5% )}
  .mesh-ultra .n64-card { clip-path: polygon( 0% 0%, 80% 0%, 85% 2%, 90% 5%, 95% 8%, 98% 12%, 100% 20%, 100% 80%, 98% 88%, 95% 92%, 90% 95%, 85% 98%, 80% 100%, 20% 100%, 15% 98%, 10% 95%, 5% 92%, 2% 88%, 0% 80%, 0% 20%, 2% 12%, 5% 8%, 10% 5%, 15% 2% )}
...
/* Lighting model variations */ .lighting-flat .n64-card { background: #1a1a1a; box-shadow: 0 var(--depth-z, 15px) calc(var(--depth-z, 15px) * 2) rgba(0, 0, 0, 0.3)}
  ...
  .lighting-phong .n64-card { background: radial-gradient(circle at 30% 30%, rgba(255, 255, 255, 0.1) 0%, transparent 50%), linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 100%)}
  .lighting-blinn-phong .n64-card { background: radial-gradient(ellipse at 25% 25%, rgba(255, 255, 255, 0.15) 0%, transparent 40%), radial-gradient(ellipse at 75% 75%, rgba(255, 255, 255, 0.08) 0%, transparent 60%), linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 100%)}
