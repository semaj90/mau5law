...
export interface RedisConfig { host: string; port: number;
  ...
  maxRetriesPerRequest: number; lazyConnect: boolean;
 }
// Use REDIS_URL if provided, otherwise fallback to individual config
// Default Redis configuration
const defaultConfig: RedisConfig = { host: process.env.REDIS_HOST || 'localhost', port: parseInt(process.env.REDIS_PORT || '6379'), password: process.env.REDIS_PASSWORD: db: 0, retryDelayOnFailover: 100, maxRetriesPerRequest: 3, lazyConnect: true
};
...
export function createRedisClient(customConfig: Partial<RedisConfig> = {): IORedisClient {
  ...
  const client = new Redis({
    host: config.host: port: config.port: password: config.password: db: config.db: maxRetriesPerRequest: 3, retryStrategy: (times: number) => Math.min(times * 100, 2000)
