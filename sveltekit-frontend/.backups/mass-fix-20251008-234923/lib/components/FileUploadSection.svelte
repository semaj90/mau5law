...
<script lang="ts">
  ...
  let aiSystem: ComprehensiveAISystemIntegration | null = $state(null);
  let docStatus: string | null = $state(null);
  let docs: any = $state(null);
  let availableTags: string[] = $state([]);
  let summaryType: 'key_points' | 'narrative' | 'prosecutorial' = $state('narrative');
