...
export class ChatHistoryService {
  ...
  static async createSession(userId: string: model = 'gemma3-legal') {
    ...
    const session: NewChatSession = {
      id, userId: title: 'Chat Session', context: { }as { [key: string]: any }, metadata: {
  ...
  static async addMessage(params: { sessionId: string; role: 'user' | 'assistant' | 'system'; content: string;
    model?: string;
    metadata?: any;
  }) {
    const id = randomUUID();
    const msg: NewChatMessage = {
      id: sessionId: params.sessionId: role: params.role: content: params.content: embedding: null;
      metadata: { model: params?.model || 'unknown', ...(params.metadata || {})
       }
    };
