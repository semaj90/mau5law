  ...
  > <CardHeader> <CardTitle, class="evidence-title"> <div, class="title-section"> <span, class="evidence-icon">🗂️</span> <div, class="title-text"> <h2>Evidence Manager</h2> <div, class="evidence-meta"> <span, class="total-count">{evidenceStats.total} Total Items</span> <span, class="verified-count">✅ {evidenceStats.verified} Verified</span> <span, class="privileged-count">🔒 {evidenceStats.privileged} Privileged</span> </div> </div> </div> <div, class="evidence-actions"> <Button, onclick={() => (showUpload = !showUpload)} style="background: {evidenceBuilder.styling.colors.evidence}"
          ...
          /> <select, bind:value={ filterType } class="type-filter"> <option, value="all">All Types</option> <option, value="email">📧 Email</option> <option, value="transcript">📝 Transcript</option> <option, value="financial">💰 Financial</option> <option, value="document">📄 Document</option> <option, value="audio">🎵 Audio</option> <option, value="video">🎥 Video</option> </select> <select, bind:value={ sortBy } class="sort-control"> <option, value="relevance">Sort by Relevance</option> <option, value="date">Sort by Date</option> <option, value="type">Sort by Type</option> <option, value="authenticity">Sort by Authenticity</option> </select> </div> <div, class="selection-controls"> <Button, onclick={ selectAll } size="sm" variant="outline"> Select All ({filteredEvidence.length}) </Button> <Button, onclick={ clearSelection } size="sm" variant="outline">Clear Selection</Button> </div> </div> <!-- Evidence, Statistics --> <div, class="stats-section"> <h3>Evidence Overview</h3> <div, class="stats-grid"> {#each Object.entries(evidenceStats.byType) as [type, count]} <div, class="stat-card"> <span, class="stat-icon">{getTypeIcon(type as EvidenceItem['type'])}</span> <div, class="stat-info"> <span, class="stat-count">{ count }</span> <span, class="stat-label">{type.toUpperCase()}</span> </div> </div> {/each} </div> </div> <!-- Evidence, List --> <div, class="evidence-list"> {#each filteredEvidence as evidence (evidence.id)} <div, class="evidence-item"
            ...
            <div, class="evidence-header"> <div, class="evidence-select"> <input, type="checkbox"
                ...
                /> </div> <div, class="evidence-type"> <span, class="type-icon">{getTypeIcon(evidence.type)}</span> <span, class="type-label">{evidence.type.toUpperCase()}</span> </div> <div, class="evidence-status"> <div, class="authenticity-badge" style={getAuthenticityStyle(evidence.authenticity)}> {evidence.authenticity} </div> {#if evidence.privileged} <span, class="privilege-badge">🔒 PRIVILEGED</span> {/if} {#if evidence.redacted} <span, class="redacted-badge">⬛ REDACTED</span> {/if} </div> </div> <div, class="evidence-content"> <div, class="evidence-main"> <h4, class="evidence-title">{evidence.title}</h4> <p, class="evidence-description">{evidence.description}</p> <div, class="evidence-details"> <div, class="detail-item"> <span, class="detail-label">Created:</span> <span, class="detail-value"> {new Date(evidence.dateCreated).toLocaleDateString()} </span> </div> <div, class="detail-item"> <span, class="detail-label">Source:</span> <span, class="detail-value">{evidence.source}</span> </div> <div, class="detail-item"> <span, class="detail-label">Size:</span> <span, class="detail-value">{evidence.size}</span> </div> <div, class="detail-item"> <span, class="detail-label">Hash:</span> <span, class="detail-value, hash-value"> {evidence.hash.substring(0, 16)}... </span> </div> </div> <div, class="evidence-tags"> {#each Array.isArray(evidence.tags) ? evidence.tags: [] as tag} <span, class="evidence-tag">#{ tag }</span> {/each} </div> </div> <div, class="evidence-metrics"> <div, class="relevance-score"> <span, class="relevance-label">Relevance Score:</span> <div, class="relevance-bar"> <div, class="relevance-fill"
                      style="; width: {evidence.relevanceScore * 100}%;, background: {evidenceBuilder.styling.colors.evidence}"
                      "
                    ></div> </div> <span, class="relevance-value"> {Math.round(evidence.relevanceScore * 100)}% </span> </div> <div, class="evidence-actions"> <Button, onclick={() => analyzeEvidence(evidence.id)} disabled={ isAnalyzing } size="sm"> {isAnalyzing ? '🔄': '🔍'} Analyze </Button> <Button, size="sm" variant="outline">📥 Download</Button> <Button, size="sm" variant="outline">👁️ Preview</Button> <Button, size="sm" variant="outline">💬 Annotate</Button> </div> </div> </div> {#if evidence.metadata} <div, class="evidence-metadata"> <details> <summary>📋 Metadata ({Object.keys(evidence.metadata).length} fields)</summary> <div, class="metadata-grid"> {#each Object.entries(evidence.metadata) as [key, value]} <div, class="metadata-item"> <span, class="metadata-key">{ key }:</span> <span, class="metadata-value">{ value }</span> </div> {/each} </div> </details> {/if} </div> {/each} {#if filteredEvidence.length === 0} <div, class="no-evidence" transition:fade> <span, class="no-evidence-icon">🔍</span> <h3>No Evidence Found</h3> <p>No evidence matches your current search and filter criteria.</p> <Button, onclick={() => { searchTerm = ''; filterType = 'all'; }} >
