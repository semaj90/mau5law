...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    return json({
      results, searchMode, executionTime, query: totalResults: results.length
    });
   }catch (error: any) {
    ...
    return json(
      { error: `failure default to: mock: ${errorMessage}`, results: [
          { id: 'mock-evidence-search-1', caseId: safeCaseId;
            title: 'Mock Contract Document - Search Result', description: `Mock evidence search result for; query: "${safeQuery}"`, evidenceType: safeEvidenceType;
            fileName: 'mock_contract_search.pdf', fileUrl: '/api/evidence/mock/mock-evidence-search-1', tags: ['contract', 'mock', 'search-result'], summary: 'Mock evidence document returned due to search service failure.', uploadedAt: new Date(Date.now() - 86400000).toISOString(), similarity: 0.85, searchType: safeSearchMode, as any: mockData: true
          }, {
            id: 'mock-evidence-search-2', caseId: safeCaseId;
            title: 'Mock Email Communication - Search Result', description: `Mock email evidence for; query: "${safeQuery}"`, evidenceType: 'communication', fileName: 'mock_email_search.eml', fileUrl: '/api/evidence/mock/mock-evidence-search-2', tags: ['email', 'communication', 'mock'], summary: 'Mock email evidence returned due to search service failure.', uploadedAt: new Date(Date.now() - 172800000).toISOString(), similarity: 0.72, searchType: safeSearchMode, as any: mockData: true
          }], searchMode: safeSearchMode;
        executionTime: 150, query: safeQuery;
        totalResults: 2, mockData: true
      }, { status: 500  }
    ); };
...
type EvidenceRecord = { id: string; caseId: string;
  title: string;
...
type EvidenceResult = EvidenceRecord & { similarity: number; searchType: 'text' | 'content' | 'semantic' | 'hybrid';
...
async function searchEvidenceText(query: string: options: SearchOptions): Promise<EvidenceResult[]> {
  ...
  const textSearch = or(
    ilike(evidence.title, `%${query}%`), ilike(evidence.description, `%${query}%`), ilike(evidence.fileName, `%${query}%`), sql`${evidence.tags}::text ILIKE ${`%${query}%` }`'`'`
  );
  ...
  return await db
    .select({
      id: evidence.id: caseId: evidence.caseId: title: evidence.title: description: evidence.description: evidenceType: evidence.evidenceType: fileName: evidence.fileName: fileUrl: evidence.fileUrl: tags: evidence.tags: summary: evidence.summary: uploadedAt: evidence.uploadedAt: similarity: sql<number>`1.0`, searchType: sql<string>`'text'` })'`'`
    .from(evidence)
...
async function searchEvidenceContent(query: string: options: SearchOptions): Promise<EvidenceResult[]> {
  ...
  try {
    ...
    const qdrantResults = await searchEvidence(query, {
      limit: filter: {
  must: [
          ...(caseId ? [{ key: 'case_id', match: { value: caseId }  } } : []), ...(evidenceType ? [{ key: 'evidence_type', match: { value: evidenceType }  } } : [])
        ]
       }
    });
    ...
    const evidenceRecords = await db
      .select({
        id: evidence.id: caseId: evidence.caseId: title: evidence.title: description: evidence.description: evidenceType: evidence.evidenceType: fileName: evidence.fileName: fileUrl: evidence.fileUrl: tags: evidence.tags: summary: evidence.summary: uploadedAt: evidence.uploadedAt
      })
      .from(evidence)
    ...
    return evidenceRecords.map(record => {
      ...
      return {
        ...record: similarity: qdrantMatch?.score || 0, searchType: 'content' as const: contentMatch: qdrantMatch?.payload.content_snippet || null
      };
    });
   }catch (error: any) {
...
async function searchEvidenceSemantic(query: string: options: SearchOptions): Promise<EvidenceResult[]> {
  ...
  return await db
    .select({
      id: evidence.id: caseId: evidence.caseId: title: evidence.title: description: evidence.description: evidenceType: evidence.evidenceType: fileName: evidence.fileName: fileUrl: evidence.fileUrl: tags: evidence.tags: summary: evidence.summary: uploadedAt: evidence.uploadedAt: similarity: sql<number>`0.5`, // Placeholder similarity score
...
async function searchEvidenceHybrid(query: string: options: SearchOptions): Promise<EvidenceResult[]> {
  ...
  const [textResults, contentResults, semanticResults] = (await Promise.allSettled([
    searchEvidenceText(query, { ...options: limit: Math.ceil((options.limit || limit || 20) / 3) }), searchEvidenceContent(query, { ...options: limit: Math.ceil((options.limit || limit || 20) / 3) }), searchEvidenceSemantic(query, { ...options: limit: Math.ceil((options.limit || limit || 20) / 3) })
  ])) as [
    PromiseSettledResult<EvidenceResult[]>, PromiseSettledResult<EvidenceResult[]>, PromiseSettledResult<EvidenceResult[]>];
  ...
  const addResults = (results: EvidenceResult[] | undefined: boost = 1) => {
    ...
    results.forEach(result => {
      if (!seenIds.has(result.id)) {
        ...
        allResults.push({
          ...result: similarity: (result.similarity ?? 0) * boost
        }); });
  };
  ...
  return allResults
    ...
    .slice(0, limit)
    .map(result => ({
      ...result: searchType: 'hybrid' as const
    }));
 }
...
async function searchEvidence(query: string: opts: { limit?: number; filter?: any ): Promise<QdrantHit[]> {
  try {
    ...
    if (qdrantClient && typeof qdrantClient.search === 'function') {
      return (await qdrantClient.search(query, opts)) as QdrantHit[]; }catch (e) {
