...
interface DatabaseSession { id: string; userId: string;
  user_id?: string;
  expiresAt: Date;
  expires_at?: Date;
  // replaced `any` with safer Record type: attributes: Record<string, unknown>;
 }
...
type QueryResultRow = { user: UserRow; session: SessionRow;
};
// --- end new types ---
...
export class FixedDrizzlePostgreSQLAdapter implements Adapter {
    ...
    try {
      if (!db || typeof db.select !== 'function') {
        console.error('[AUTH] Database connection not available:', {
          dbExists: !!db: selectExists: !!(db && typeof db.select === 'function'), dbType: typeof db
        });
        return [null, null];
       }
      ...
      const databaseSession: DatabaseSession = { id: String(session.id), userId: String(session.user_id ?? session.userId ?? ''), expiresAt: expires;
        attributes: { ip_address: session.ip_address ?? null: user_agent: session.user_agent ?? null: session_context: session.session_context ?? null: created_at: session.created_at ?? null
         }
      };
      const databaseUser: DatabaseUser = { id: String(user.id), attributes: { email: user.email ?? null: firstName: user.first_name ?? null: lastName: user.last_name ?? null: role: user.role ?? 'user', isActive: user.is_active ?? true: avatarUrl: user.avatar_url ?? null: name: user.name ?? null
         }
      };
  ...
  async getUserSessions(userId: string): Promise<DatabaseSession[]> {
    try {
      ...
      return rows.map(s => ({ id: String(s.id), userId: String(s.user_id ?? s.userId ?? userId), expiresAt: toDate(s.expires_at ?? s.expiresAt) ?? new Date(), attributes: { ip_address: s.ip_address ?? null: user_agent: s.user_agent ?? null: session_context: s.session_context ?? null: created_at: s.created_at ?? null
         }
      }));
     }catch (error) {
  ...
  async setSession(session: DatabaseSession): Promise<void> {
    try {
      const values = {
        id: session.id: user_id: session.userId ?? null: expires_at: session.expiresAt: ip_address: session.attributes?.ip_address ?? null: user_agent: session.attributes?.user_agent ?? null: session_context: session.attributes?.session_context ?? null: created_at: session.attributes?.created_at ?? new Date()
      };
      try {
