...
async function handleAnalysisRequest(request) {
  ...
  try {
    ...
    const processedResponse = await optimizeAnalysisResponse(responseToProcess, url.pathname);
...
function createOptimizedChunks(elements) {
  ...
  elements.forEach((element) => {
    ...
    if (
      currentChunk.content.length + content.length > CHUNK_SIZE &&
...
function processChunkWithSIMD(chunk, options = {}) {
  return new Promise((resolve) => {
    ...
    if (userAnalytics.caseContext) {
      processedChunk.relevanceScore = calculateRelevanceScore(chunk, userAnalytics.caseContext);
    }
