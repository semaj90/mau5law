...
export const shaderCacheEntries = pgTable('shader_cache_entries', {
  ...
  compilationSuccess: boolean('compilation_success').default(false), // Semantic embeddings for similarity search
  ...
  legalContext: jsonb('legal_context').$type<Record<string, unknown> | null>(), // Performance metrics
  performanceMetrics: jsonb('performance_metrics').$type<Record<string, unknown> | null>(), // Reinforcement learning data
  reinforcementData: jsonb('reinforcement_data').$type<Record<string, unknown> | null>(), // Version and lifecycle
  version: integer('version').default(1), deprecated: boolean('deprecated').default(false), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(), updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow(), lastAccessedAt: timestamp('last_accessed_at', { withTimezone: true }), // MinIO integration for large assets
...
export const shaderUserPatterns = pgTable('shader_user_patterns', {
  id: serial('id').primaryKey(), // User and session context
  shaderId: uuid('shader_id')
    .references(() => shaderCacheEntries.id)
    .onDelete('CASCADE'), userId: text('user_id').notNull(), sessionId: text('session_id'), accessTimestamp: timestamp('access_timestamp', { withTimezone: true }).defaultNow(), timeOfDay: integer('time_of_day'), // hour of day (0-23)
  workflowStep: text('workflow_step'), loadLatencyMs: integer('load_latency_ms'), cacheHit: boolean('cache_hit'), preloadSuccessful: boolean('preload_successful'), userSatisfaction: real('user_satisfaction'), // -1 to, 1
  // Contextual metadata
  documentContext: jsonb('document_context').$type<Record<string, unknown> | null>(), // Reinforcement learning features
  ...
  reinforcement_data: jsonb('reinforcement_data').$type<Record<string, unknown> | null>(), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow()
});
export const shaderPreloadRules = pgTable('shader_preload_rules', {
  ...
  ruleKey: text('rule_key').notNull().unique(), ruleName: text('rule_name').notNull(), ruleType: text('rule_type').notNull(), // 'sequential', 'conditional', 'temporal', 'similarity'
  // Condition matching
  triggerConditions: jsonb('trigger_conditions').$type<Record<string, unknown> | null>(), // Preload specifications
  preloadTargets: jsonb('preload_targets').$type<Record<string, unknown> | null>(), accuracy: real('accuracy').notNull(), // Historical accuracy
  // Performance metrics
  triggerCount: integer('trigger_count').default(0), successCount: integer('success_count').default(0), preloadSavingsMs: integer('preload_savings_ms').default(0), // Lifecycle;
  active: boolean('active').default(true), learningRate: real('learning_rate').default(0.01), lastTriggered: timestamp('last_triggered', { withTimezone: true }), lastUpdated: timestamp('last_updated', { withTimezone: true }).defaultNow(), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow()
});
export const shaderDependencies = pgTable('shader_dependencies', {
  id: serial('id').primaryKey(), // Dependency relationship
  parentShaderId: uuid('parent_shader_id')
    .references(() => shaderCacheEntries.id)
    .onDelete('CASCADE'), childShaderId: uuid('child_shader_id')
    .references(() => shaderCacheEntries.id)
    .onDelete('CASCADE'), // Dependency metadata
  ...
  // Performance impact
  parallelLoadSafe: boolean('parallel_load_safe').default(true), loadLatencyImpactMs: integer('load_latency_impact_ms'), // Usage statistics
...
export const shaderCompilationQueue = pgTable('shader_compilation_queue', {
  ...
  queueKey: text('queue_key').notNull().unique(), priority: text('priority').notNull(), // 'immediate', 'high', 'normal', 'low', 'preload';
  status: text('status').notNull().$type<ShaderCompilationStatus>(), // restrict to valid status values
  // Shader information
  shaderKey: text('shader_key').notNull(), sourceCode: text('source_code').notNull(), shaderType: text('shader_type').notNull(), targetGPU: text('target_gpu'), // GPU-specific compilation
  // Processing context
  userId: text('user_id'), sessionId: text('session_id'), workflowContext: jsonb('workflow_context').$type<Record<string, unknown> | null>(), // Queue timing
  queuedAt: timestamp('queued_at', { withTimezone: true }).defaultNow(), startedAt: timestamp('started_at', { withTimezone: true }), completedAt: timestamp('completed_at', { withTimezone: true }), // Processing results
  compilationResult: jsonb('compilation_result').$type<Record<string, unknown> | null>(), // Retry logic
  retryCount: integer('retry_count').default(0), maxRetries: integer('max_retries').default(3), nextRetryAt: timestamp('next_retry_at', { withTimezone: true }), updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow()
});
/**
...
export const shaderRecommendationsView = pgTable('shader_recommendations_view', {
  id: serial('id').primaryKey(), userId: text('user_id').notNull(), shaderKey: text('shader_key').notNull(), recommendationType: text('recommendation_type').notNull(), // 'similar', 'next', 'popular', 'optimal'
  confidence: real('confidence').notNull(), reasoning: text('reasoning'), // Recommendation context
  ...
  // Metadata
  computedAt: timestamp('computed_at', { withTimezone: true }).defaultNow(), validUntil: timestamp('valid_until', { withTimezone: true }), // Performance tracking
