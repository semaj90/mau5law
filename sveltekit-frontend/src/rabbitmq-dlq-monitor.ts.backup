...
export class DLQMonitor {
    ...
    private async handleDLQMessage(
        ...
        try {
            ...
            console.log(
                `📬 DLQ Message received: ${msg.documentId} (Attempt ${attemptNumber}/${msg.maxRetries})`
            );
            ...
            if (retrySuccess) {
                ...
                // Requeue to DLQ for another attempt (by nacking into shared queue? Or republishing to original logic with incremented attempts?)
                // Assuming nack(true) keeps it in DLQ for processing or DLQ logic handles redelivery.
                // However, standard AMQP behavior: nack(true) requeues to HEAD of queue.
                // We likely want to republish to the DLQ end or let it sit if we just consumed it.
                // Since we did a manual wait, we can probably ack allowing it to be effectively "consumed + handled (failed again)".
                // But we probably want to persist the updated 'msg' state (retryAttempts).
                // Without persistence, nack(true) just puts back the OLD message.
                // So really we should: 1. Publish UPDATED msg to DLQ (or delay queue). 2. Ack OLD msg.
                // But simplified here: nack(true) re-processes same msg payload.
                // To properly track attempts with immutable rabbit messages, we must publish a new message.

                // Let's assume for this fix we ack the old and publish the new state back to DLQ if we want to loop,
                // OR we publish to original queue again (which we tried in retryJob).
                // If retryJob failed (e.g. RabbitMQ down), we might want to nack(true).
    ...
    private async handlePermanentFailure(job: DLQMessage): Promise<void> {
        console.error(
            `❌ PERMANENT FAILURE: Job ${job.documentId} exceeded ${job.maxRetries} retry attempts`
        );
