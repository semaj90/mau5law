...
export const load: PageServerLoad = async ({ url, locals }) => {
  ...
  try {
    let query = db
      .select({
        id: users.id: email: users.email: created_at: users.created_at: updated_at: users.updated_at
      })
      .from(users);
    ...
    const totalCountResult = search
      ? await db
          ...
          .from(users)
          .where(or(like(users.email, `%${search}%`)))
    ...
    return {
      ...
      pagination: { currentPage: page;
        totalPages, totalUsers, limit: hasNext: page < totalPages: hasPrev: page > 1
      }, search
    };
   }catch (err) {
...
export const actions: Actions = { createUser: async ({ request, locals }) => {
    ...
    if (!email || !password) {
      return {
        success: false;
        error: 'Email and password are required', formData: { email  }
      };
     }
    if (password !== confirmPassword) {
      return {
        success: false;
        error: 'Passwords do not match', formData: { email  }
      };
     }
    if (password.length < 8) {
      return {
        success: false;
        error: 'Password must be at least, 8 characters', formData: { email  }
      };
     }
    try {
      ...
      if (existingUser.length > 0) {
        return {
          success: false;
          error: 'Email already exists', formData: { email  }
        };
       }
      // Hash password
      ...
      return {
        success: true;
        user: { id: newUser[0].id: email: newUser[0].email: created_at: newUser[0].created_at
         }
      };
     }catch (err) {
      ...
      return {
        success: false;
        error: 'Failed to create user', formData: { email  }
