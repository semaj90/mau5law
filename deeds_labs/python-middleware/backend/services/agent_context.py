#!/usr/bin/env python3
"""
Agentic Context Anchor (ACA) - Prevents context loss during long agent sessions.

Layers:
1. Plan record (high-level goal, tasks, spec refs)
2. Latent markers (small text IDs pointing to Redis summaries)
3. Re-summary / retokenization when near token limit

Key Redis shapes:
  agent:plan:{session_id}
  agent:timeline:{session_id}
  agent:summary:{session_id}:{version}
  agent:spec_summary:{session_id}:{version}
"""

from __future__ import annotations
from typing import Dict, Any, List, Optional, Tuple
from datetime import datetime, timezone
import subprocess
import textwrap
import logging

logger = logging.getLogger(__name__)


class AgentContextAnchor:
    """Manages plan, summaries, and context overflow for agentic sessions."""

    def __init__(self, redis_cache, granite_client):
        """
        Args:
            redis_cache: RedisCache instance
            granite_client: GraniteClient instance for summarization
        """
        self.redis = redis_cache
        self.granite = granite_client

    def _now_iso(self) -> str:
        """ISO timestamp."""
        return datetime.now(timezone.utc).isoformat()

    # ============ Plan Management ============

    def get_plan(self, session_id: str) -> Dict[str, Any]:
        """Fetch the session plan from Redis."""
        plan = self.redis.get_json(f"agent:plan:{session_id}")
        return plan or {}

    def set_plan(
        self,
        session_id: str,
        goal: str,
        spec_files: List[str],
        current_step: str = "",
        ttl: int = 7 * 24 * 3600,
    ) -> None:
        """Create or update the session plan."""
        plan = {
            "session_id": session_id,
            "goal": goal,
            "current_step": current_step,
            "spec_files": spec_files,
            "summary_version": 0,
            "spec_summary_version": 0,
            "created_at": self._now_iso(),
            "updated_at": self._now_iso(),
        }
        self.redis.set_json(f"agent:plan:{session_id}", plan, ttl=ttl)
        logger.info(f"Plan set for {session_id}: {goal}")

    def update_plan_step(self, session_id: str, current_step: str) -> None:
        """Update the current step in the plan."""
        plan = self.get_plan(session_id)
        if plan:
            plan["current_step"] = current_step
            plan["updated_at"] = self._now_iso()
            self.redis.set_json(f"agent:plan:{session_id}", plan)

    # ============ Timeline Management ============

    def append_timeline(
        self, session_id: str, kind: str, payload: Dict[str, Any], description: str = ""
    ) -> None:
        """Append an event to the session timeline."""
        key = f"agent:timeline:{session_id}"
        event = {
            "ts": self._now_iso(),
            "kind": kind,
            "payload": payload,
            "description": description,
        }
        self.redis.lpush_json(key, event)
        # Keep last 500 events
        self.redis.ltrim(key, 0, 499)

    def get_timeline_snippet(self, session_id: str, limit: int = 50) -> str:
        """Get a text snippet of recent timeline events."""
        key = f"agent:timeline:{session_id}"
        events = self.redis.lrange_json(key, 0, limit - 1) or []

        lines = []
        for event in reversed(events):  # oldest first
            ts = event.get("ts", "?")
            kind = event.get("kind", "?")
            desc = event.get("description", "")
            payload_str = str(event.get("payload", {}))[:100]
            line = f"[{ts}] {kind}: {desc or payload_str}"
            lines.append(line)

        return "\n".join(lines)

    # ============ Spec Extraction (ripgrep + awk) ============

    def _ripgrep_spec(self, paths: List[str], keywords: List[str]) -> str:
        """
        Use ripgrep to extract relevant spec chunks.
        Falls back gracefully if ripgrep not available.
        """
        if not paths or not keywords:
            return ""

        # Build pattern: keyword1|keyword2|...
        pattern = "|".join(keywords)

        try:
            cmd = ["rg", pattern, "--max-count=5"] + paths
            output = subprocess.check_output(
                cmd, stderr=subprocess.DEVNULL, text=True, timeout=5
            )
            return output
        except (FileNotFoundError, subprocess.TimeoutExpired, subprocess.CalledProcessError):
            logger.warning(f"ripgrep failed for keywords {keywords}; returning empty")
            return ""

    # ============ Summary Management ============

    def ensure_summaries(
        self, session_id: str, goal: str, force: bool = False
    ) -> Dict[str, Any]:
        """
        Ensure we have:
        - Session summary of timeline
        - Spec summary via ripgrep

        Returns:
            Dict with summary_version, spec_summary_version, summary_text, spec_text, latent_marker
        """
        plan = self.get_plan(session_id)
        spec_files: List[str] = plan.get("spec_files", [])
        summary_version = plan.get("summary_version", 0)
        spec_summary_version = plan.get("spec_summary_version", 0)

        # 1) Timeline summary
        summary_key = f"agent:summary:{session_id}:{summary_version}"
        summary_text = self.redis.get(summary_key) if not force else None

        if not summary_text:
            timeline_text = self.get_timeline_snippet(session_id, limit=80)
            prompt = textwrap.dedent(
                f"""\
                You are summarizing the agent's recent work on a legal task.
                Goal: {goal}

                Timeline events:
                {timeline_text}

                Write a concise summary (<= 400 tokens) focused on:
                - What has been done
                - What is pending
                - Any open questions
                """
            )
            try:
                summary_text = self.granite.generate(prompt, max_tokens=400)
            except Exception as e:
                logger.error(f"Failed to generate summary: {e}")
                summary_text = f"[Summary generation failed: {e}]"

            summary_version += 1
            summary_key = f"agent:summary:{session_id}:{summary_version}"
            self.redis.set(summary_key, summary_text, ttl=7 * 24 * 3600)

        # 2) Spec summary
        spec_key = f"agent:spec_summary:{session_id}:{spec_summary_version}"
        spec_text = self.redis.get(spec_key) if not force else None

        if not spec_text:
            # Extract keywords from goal
            keywords = [goal[:64], "alignment", "search", "KAG", "RAG", "requirements"]
            raw_spec = self._ripgrep_spec(spec_files, keywords)

            if raw_spec:
                prompt = textwrap.dedent(
                    f"""\
                    You are summarizing project specs for a legal AI system.
                    Goal: {goal}

                    Raw spec excerpts (from ripgrep):
                    {raw_spec[:2000]}

                    Summarize into <= 300 tokens focusing on:
                    - Constraints
                    - Invariants (don't break)
                    - Required outputs
                    """
                )
                try:
                    spec_text = self.granite.generate(prompt, max_tokens=300)
                except Exception as e:
                    logger.error(f"Failed to generate spec summary: {e}")
                    spec_text = f"[Spec summary generation failed: {e}]"
            else:
                spec_text = "[No spec files found or ripgrep unavailable]"

            spec_summary_version += 1
            spec_key = f"agent:spec_summary:{session_id}:{spec_summary_version}"
            self.redis.set(spec_key, spec_text, ttl=7 * 24 * 3600)

        # Update plan with latest versions
        plan["summary_version"] = summary_version
        plan["spec_summary_version"] = spec_summary_version
        plan["updated_at"] = self._now_iso()
        self.redis.set_json(f"agent:plan:{session_id}", plan)

        # Build latent marker
        latent_marker = f"[[ACA:{session_id}:s{summary_version}:p{spec_summary_version}]]"

        return {
            "summary_version": summary_version,
            "spec_summary_version": spec_summary_version,
            "summary_text": summary_text,
            "spec_text": spec_text,
            "latent_marker": latent_marker,
        }

    # ============ LLM Prompt Building ============

    def build_llm_prompt(
        self, session_id: str, goal: str, user_message: str
    ) -> Tuple[str, str]:
        """
        Build system + user prompts with ACA context.

        Returns:
            (system_prompt, user_prompt)
        """
        ctx = self.ensure_summaries(session_id, goal)
        marker = ctx["latent_marker"]
        summary_text = ctx["summary_text"]
        spec_text = ctx["spec_text"]

        system = textwrap.dedent(
            f"""\
            You are YoRHa Legal Agent working on session {session_id}.

            Agentic Context Anchor: {marker}

            High-level plan summary:
            {summary_text}

            Relevant spec summary:
            {spec_text}

            If you need more context, refer to the ACA marker and ask the tool RECOVER_CONTEXT("{marker}") instead of hallucinating.
            """
        ).strip()

        timeline_excerpt = self.get_timeline_snippet(session_id, limit=10)
        user = textwrap.dedent(
            f"""\
            User message: {user_message}

            Recent events:
            {timeline_excerpt}
            """
        ).strip()

        return system, user

    # ============ Token Budget Management ============

    def estimate_tokens(self, text: str) -> int:
        """
        Rough token estimate (1 token ≈ 4 chars for English).
        Can be refined with actual tokenizer.
        """
        return len(text) // 4

    def maybe_compact_context(
        self, session_id: str, goal: str, token_limit: int = 8192, safety_margin: float = 0.7
    ) -> bool:
        """
        Check if context is getting too large; if so, compact it.

        Args:
            session_id: Session ID
            goal: Session goal
            token_limit: Model context limit (e.g., 8192)
            safety_margin: Compact if usage > safety_margin * token_limit

        Returns:
            True if compaction was triggered, False otherwise
        """
        # Estimate current context size
        timeline_text = self.get_timeline_snippet(session_id, limit=100)
        est_tokens = self.estimate_tokens(timeline_text)

        threshold = token_limit * safety_margin

        if est_tokens > threshold:
            logger.info(
                f"Context approaching limit ({est_tokens} > {threshold}); compacting..."
            )
            # Force re-summarization
            self.ensure_summaries(session_id, goal, force=True)

            # Truncate old timeline events (keep last 100)
            key = f"agent:timeline:{session_id}"
            self.redis.ltrim(key, 0, 99)

            return True

        return False

    # ============ Context Recovery ============

    def recover_context(self, marker: str) -> Dict[str, Any]:
        """
        Decode a latent marker and recover the associated context.

        Marker format: [[ACA:session_id:s{summary_version}:p{spec_summary_version}]]

        Returns:
            Dict with summary_text, spec_text, plan
        """
        # Parse marker
        try:
            parts = marker.strip("[]").split(":")
            if len(parts) < 4 or parts[0] != "ACA":
                return {"error": "Invalid marker format"}

            session_id = parts[1]
            summary_version = int(parts[2][1:])  # s{N} -> N
            spec_summary_version = int(parts[3][1:])  # p{N} -> N
        except (ValueError, IndexError) as e:
            logger.error(f"Failed to parse marker {marker}: {e}")
            return {"error": f"Marker parse error: {e}"}

        # Fetch summaries
        summary_key = f"agent:summary:{session_id}:{summary_version}"
        spec_key = f"agent:spec_summary:{session_id}:{spec_summary_version}"

        summary_text = self.redis.get(summary_key) or "[Summary not found]"
        spec_text = self.redis.get(spec_key) or "[Spec summary not found]"
        plan = self.get_plan(session_id)

        return {
            "session_id": session_id,
            "summary_version": summary_version,
            "spec_summary_version": spec_summary_version,
            "summary_text": summary_text,
            "spec_text": spec_text,
            "plan": plan,
        }
