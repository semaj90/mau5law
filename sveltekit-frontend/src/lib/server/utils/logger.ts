...
class Logger {
    ...
    private async processLogQueue(): Promise<void> {
        ...
        try {
            ...
            const retriableLogs = logsToProcess.filter(
                (log) => (log.metadata?.retryCount ?? 0) < this.config.maxRetries
            );
	...
	${log.performance ? JSON.stringify(log.performance) : null}
                    ...
                    console.warn('Failed to store log:', dbError);
                }
            }
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            throw new Error(`Database failed: ${message}`);
        }
    }

    private async writeToFile(logs: LogEntry[]): Promise<void> {
        // File logging implementation would go here
        // Skip file logging in the browser environment
    ...
    async initializeLogTable(): Promise<void> {
        try {
            ...
            await db.execute(
                sql`CREATE INDEX IF NOT EXISTS idx_system_logs_component ON system_logs (component)`
            );
            await db.execute(
                sql`CREATE INDEX IF NOT EXISTS idx_system_logs_conversation ON system_logs (conversation_id)`
            );

            console.log('System logs table initialized successfully');
        } catch (error) {
            console.error('Failed to initialize system table:', error);
        }
    }
}
