...
interface SIMDLangExtractRequest {
  text?: string; // Made optional to allow for batch/benchmark requests without a single text field
  model?: string;
  tags?: string[];
  ...
  // New fields for batch/benchmark operations
  benchmark?: boolean;
  iterations?: number;
...
interface SIMDLangExtractResponse {
  ...
  simd_results: {
    // use the typed CompressedTile instead of Array<any>
...
type CompressedTile = {
  id?: string;
  // compressed_data is produced by Array.from(Uint8Array) in response construction
...
export const POST: RequestHandler = async ({ request, fetch }) => {
  try {
    ...
    const response: SIMDLangExtractResponse = {
      ...
      type,
      // Enhanced SIMD fields
      simd_results: { compressed_tiles: simdResult.compressedTiles.map(tile => ({, id: tile.id,
...
type SimdProcessResult = {
  originalText?: string;
  // typed compressed tiles so call-sites can safely read expected properties
  ...
  // allow either ArrayBuffer or Uint8Array for binary buffers returned by different runtimes
  gpuBufferData?: Uint8Array | number[] | ArrayBuffer;
  vertexBufferCache?: Uint8Array | ArrayBuffer;
  uiComponents?: {
    instantRender?: boolean;
    // accept ArrayBuffer or Uint8Array to match engine implementations
    componentData?: Uint8Array | ArrayBuffer;
...
async function handleBenchmarkTesting(
  ...
  for (const target of compressionTargets) {
    ...
    const avgGpuUtilization =
      compressionResults.reduce((sum, r) => sum + r.processingStats.gpuUtilization, 0) / compressionResults.length;
