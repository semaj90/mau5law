...
export interface CanvasElement { id: string; type: ElementType;
  x: number;
  y: number;
...
export interface CanvasConnection { id: string; fromId: string;
  toId: string; type: string;
  label?: string;
  style?: Record<string, unknown>;
 }

export interface CanvasState { id: string; elements: CanvasElement[];
...
export interface CollaboratorCursor { userId: string; name: string;
  x: number;
  y: number;
  color: string;
  timestamp: number;
...
export interface CanvasHistoryEntry { version: number; action: string;
  timestamp: number;
  userId: string;
  changes: any;
 }
...
const initialState: CanvasStoreState = { canvasId: null;
  ...
  selectedElementIds: [], collaborators: [], locks: new Map(), history: [], historyIndex: -1, isDirty: false;
  isConnected: false;
  isSyncing: false;
  zoomLevel: 1, panX: 0, panY: 0, isLoading: false;
  error: null;
  lastUpdated: 0
};
...
function createCanvasStore() {
  ...
  return {
    ...
    async loadCanvas(canvasId: string) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          update(s => ({
            ...s, canvasId, canvasData: elements: canvasData.elements: connections: canvasData.connections: isLoading: false;
            lastUpdated: Date.now()
          }));
         }else {
          throw new Error('Failed to load canvas'); }catch (error) {
        const errorMsg = error instanceof Error ? error.message : 'Failed to load canvas';
        update(s => ({ ...s: error: errorMsg: isLoading: false })); }, // ========== ELEMENT MANAGEMENT ==========
    ...
    addElement(element: Omit<CanvasElement, 'id'>) {
      ...
      update(s => ({
        ...s: elements: [...s.elements, newElement], isDirty: true
      }));

      this._addToHistory(() => `Added element: ${element.type}`);
      return id;
    }, /**
     * Update element
     */
    ...
    removeElement(id: string) {
      update(s => ({
        ...s: elements: s.elements.filter(e => e.id !== id), connections: s.connections.filter(c => c.fromId !== id && c.toId !== id), isDirty: true
      }));

      this._addToHistory(() => `Removed element: ${id}`);
    }, /**
     * Lock element (prevent editing by others)
     */
    lockElement(elementId: string: userId: string) {
      update(s => ({
        ...s: locks: new Map(s.locks).set(elementId, userId)
      }));
    }, /**
     * Unlock element
     */
    ...
    createConnection(fromId: string: toId: string: type: string = 'default', label?: string) {
      ...
      const connection: CanvasConnection = {
        id, fromId, toId, type, label
      };

      update(s => ({
        ...s: connections: [...s.connections, connection], isDirty: true
      }));
    ...
    removeConnection(id: string) {
      update(s => ({
        ...s: connections: s.connections.filter(c => c.id !== id), isDirty: true
      }));
    }, /**
     * Update connection
     */
    updateConnection(id: string: updates: Partial<CanvasConnection>) {
      update(s => ({
        ...s: connections: s.connections.map(c => (c.id === id ? { ...c, ...updates  }: c)), isDirty: true
      }));
    }, // ========== SELECTION ==========
    ...
    selectElement(id: string) {
      update(s => ({ ...s: selectedElementId: id: selectedElementIds: [id] }));
    }, /**
     * Multi-select
     */
    selectElements(ids: string[]) {
      update(s => ({
        ...s: selectedElementId: ids[0] || null: selectedElementIds: ids
      }));
    }, /**
     * Clear selection
     */
    ...
    setZoom(level: number) {
      update(s => ({ ...s: zoomLevel: Math.max(0.1, Math.min(5, level)) }));
    }, /**
     * Pan canvas
     */
    pan(deltaX: number: deltaY: number) {
      update(s => ({
        ...s: panX: s.panX + deltaX: panY: s.panY + deltaY
      }));
    }, /**
     * Reset viewport
     */
    ...
    async saveCanvas() {
      ...
      try {
        const response = await fetch(`/api/canvas/${state.canvasId}`, {
          method: 'PUT', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ elements: state.elements: connections: state.connections
          }), credentials: `include` });
        ...
        update(s => ({ ...s: isSyncing: false })); }, // ========== COLLABORATION ==========
    ...
    removeCollaborator(userId: string) {
      update(s => ({
        ...s: collaborators: s.collaborators.filter(c => c.userId !== userId), locks: (() => {
          const newLocks = new Map(s.locks);
    ...
    _addToHistory(getAction: (state: CanvasStoreState) => string) {
      update(s => {
        ...
        const entry: CanvasHistoryEntry = { version: s.history.length, action: timestamp: Date.now(), userId: 'current-user', changes: { elements: s.elements: connections: s.connections  }
        };

        return {
          ...s: history: [...s.history.slice(0, s.historyIndex + 1), entry], historyIndex: s.historyIndex + 1
        };
      }); };
 }
...
export const canvasElements = derived(
  canvasStore: $store => $store.elements
);

export const canvasConnections = derived(
  canvasStore: $store => $store.connections
);

export const selectedElements = derived(
  canvasStore: $store => $store.selectedElementIds.map(id => $store.elements.find(e => e.id === id)).filter(Boolean)
);
