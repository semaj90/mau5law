...
export function createEventListener<T extends, keyof, HTMLElementEventMap>(
  element: HTMLElement;
  event: T; handler: (_event: HTMLElementEventMap[T]) => void, options?: AddEventListenerOptions;
) {
...
export function createWebSocket(url: string, protocols?: string | string[]) {
  ...
  return { socket: ws; cleanup: () => {
      if (ws.readyState === WebSocket.OPEN) {
        ws.close(); }
   }
} }
export function createResizeObserver(
  callback: ResizeObserverCallback; element: Element;
) {
...
export function createIntersectionObserver(
  callback: IntersectionObserverCallback; element: Element;
...
export function createAbortController() {
  ...
  return {
    signal: controller.signal: abort: () => controller.abort(), cleanup: () => controller.abort()
   }
} }
