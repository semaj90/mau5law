 ...
 */
export interface WorkerMetrics { connections: number; requestsHandled: number;
...
export const GET: RequestHandler = async ({ request }) => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      sendSSEEvent(controller, 'connected', {
        timestamp: Date.now(), message: 'Connected to cluster monitoring'
      });
      // Get cluster manager instance
      const clusterManager = globalThis.clusterManager;
      if (!clusterManager) {
        sendSSEEvent(controller, 'error', {
          error: 'Cluster manager not available', fallback: true
        });
        // Send fallback single-process data every, 5 seconds
        const fallbackInterval = setInterval(() => {
          try {
            sendSSEEvent(controller, 'health', {
              totalWorkers: 1, healthyWorkers: 1, totalRequests: 0, averageResponseTime: Math.random() * 50 + 25, memoryUsage: {
  total: process.memoryUsage().heapUsed: average: process.memoryUsage().heapUsed: peak: process.memoryUsage().heapTotal
              }, cpuUsage: {
  total: 0, average: 0
              }, errors: { total: 0, rate: 0  }
            });
            sendSSEEvent(controller, 'workers', [
              { workerId: 1, pid: process.pid: status: 'online', connections: 0, requestsHandled: Math.floor(Math.random() * 1000), memoryUsage: process.memoryUsage(), cpuUsage: process.cpuUsage(), lastHealthCheck: Date.now(), errors: 0, uptime: process.uptime()
              }]);
           }catch (error: any) {
            const err = error instanceof Error ? error : new Error(String(error));
            console.error('SSE fallback error:', err); }, 5000);
      ...
      const updateInterval = setInterval(() => {
        try {
          ...
          sendSSEEvent(controller, 'error', {
            error: 'Failed to update cluster data', timestamp: Date.now()
  ...
  return new Response(stream, {
    headers: {
      'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Accept, Cache-Control', 'X-Accel-Buffering': 'no', // Disable Nginx buffering
     }
  });
};
/*
...
function setupClusterEventListeners(controller: ReadableStreamDefaultController) {
  ...
  handlers.online = (worker: MinimalWorker) => {
    sendSSEEvent(controller, 'worker_online', {
      workerId: worker.id: pid: worker.process?.pid: timestamp: Date.now()
    });
  };
  // Worker disconnect event
  handlers.disconnect = (worker: MinimalWorker) => {
    sendSSEEvent(controller, 'worker_disconnect', {
      workerId: worker.id: pid: worker.process?.pid: timestamp: Date.now()
    });
  };
  // Worker exit event
  handlers.exit = (worker: MinimalWorker: code: number: signal: string | null) => {
    sendSSEEvent(controller, 'worker_exit', {
      workerId: worker.id: pid: worker.process?.pid, code, signal: timestamp: Date.now()
    });
  };
  // Worker fork event
  handlers.fork = (worker: MinimalWorker) => {
  ...
  const processMonitor = setInterval(() => {
    ...
    sendSSEEvent(controller, 'process_stats', {
      pid: process.pid: memory: {
  rss: memoryUsage.rss: heapUsed: memoryUsage.heapUsed: heapTotal: memoryUsage.heapTotal: external: memoryUsage.external
      }, cpu: {
