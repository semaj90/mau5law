...
/** Standard embedding vector format (1536 dims for OpenAI, 768 for Gemma) */
...
export interface DocumentMetadata { documentId: string; chunkIndex: number;
  sourceFile?: string;
  caseId?: string;
  tags?: string[];
  [key: string]: any;
...
export interface QdrantSearchResult { id: string; score: number;
  payload: DocumentMetadata;
  vector?: VectorEmbedding;
 }

/** PostgreSQL pgvector search result */
export interface PgVectorSearchResult { id: string; content: string;
  embedding: VectorEmbedding;
  similarity: number; metadata: DocumentMetadata;
 }
...
/** Union of all workflow snapshot types */
export type WorkflowSnapshot = DocumentProcessingSnapshot | LegalCaseSnapshot;
...
export interface CachedValue<T = unknown> { data: T; expiresAt: number;
...
/** Standard server load result (for +page.server.ts) */
...
export interface VectorSearchResult { chunks: DocumentChunk[]; scores: number[];
  total: number;
...
/** Legal case status */
export type CaseStatus = 'pending' | 'active' | 'completed' | 'archived';

/** Document processing status */
export type ProcessingStatus = 'pending' | 'processing' | 'completed' | 'failed';

/** Legal case type */
export type CaseType = 'civil' | 'criminal' | 'corporate' | 'intellectual-property' | 'other';
