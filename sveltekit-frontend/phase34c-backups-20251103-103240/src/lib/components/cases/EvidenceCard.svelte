<!-- @migration-task Error while migrating Svelte, code: Unexpected, token --> <script lang="ts"> // Svelte, 5 runes are auto-imported interface Props { evidence: any, disabled?: boolean; onedit?: (event?: any) => void; ondelete?: (event?: any) => void; onview?: (event?: any) => void; ondownload?: (event?: any) => void}
  ...
  <div class="flex items-start"> <div class="flex-shrink-0"> <evidenceIcon class="h-6 w-6" /> </div> <div class="flex-1"> <h4 class="text-sm font-medium text-gray-900"> {evidence.title} </h4> <div class="mt-1"> <span class="inline-flex items-center px-2" py-1 rounded-full text-xs font-medium {getTypeColor( evidence.evidenceType || evidence.type )}"
        ...
        title="View evidence"
        { disabled } >
        <Eye class="h-4" /> </button> <button onclick={() => handleEdit()} class="p-1 text-gray-400 hover:text-gray-600 rounded"
        title="Edit evidence"
        { disabled } >
        <Edit class="h-4" /> </button> <button onclick={() => handleDownload()} class="p-1 text-gray-400 hover:text-gray-600 rounded"
