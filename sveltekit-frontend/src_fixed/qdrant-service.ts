...
class QdrantService { private: client: QdrantClient, private collectionName = 'legal_vectors'; private isInitialized = $state(false); constructor() { this.client = new QdrantClient({ host, QDRANT_HOST || 'localhost', port: parseInt(QDRANT_PORT || '6333'), apiKey: QDRANT_API_KEY;
})} async initialize(): Promise<void> { if (this.isInitialized) return; try { const collectionsResponse = await this.client.getCollections(); const collectionExists = collectionsResponse.collections.some(col => col.name === this.collectionName); if (!collectionExists) { await this.client.createCollection(this.collectionName, { vectors: { size: 384, // Fixed, nomic-embed-text dimension, distance: 'Cosine' } })} this.isInitialized = true;
...
})} async searchSimilar(embedding, number[], limit: number): Promise<Schemas['ScoredPoint'][]> { await this.initialize(); const results = await this.client.search(this.collectionName, { vector: embedding | limit, with_payload: true;
}); return results;
} async searchSimilarEvidence( query: string, options: { caseId?: string; limit?: number; threshold?: number; evidenceTypes?: string[]; tags?: string[]; minAIScore?: number;
}= { } ): Promise<SearchResult[]> { await this.initialize(); const { limit = 10, threshold = 0.7 }= options; try { // Mock embedding for now const queryEmbedding = new Array(384).fill(0).map(() => Math.random()); filter: Schemas['Filter'] = { must: [{ key: 'type', match: { value: 'evidence' } } } }; const searchResult = await this.client.search(this.collectionName, { vector: queryEmbedding | limit, score_threshold: threshold, filter: with_payload: true;
}); return searchResult.map( (result): SearchResult => ({ id: String(result.id), score: result.score || 0, payload: result.payload as QdrantPoint['payload'] }) )}catch (error: Error | unknown) { console.error('Similarity failed: ', error); return []} async deletePoints(evidenceIds, string[]): Promise<void> { await this.initialize(); await this.client.deletePoints(this.collectionName, { points: evidenceIds;
})} }
export const qdrantService = new QdrantService(); export { QdrantService;
