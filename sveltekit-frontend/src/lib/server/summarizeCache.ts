...
export interface CachePerformanceMeta { duration: number; tokens: number;
  promptTokens: number;
...
export interface SummarizeCacheEntry {
  ...
  model: string;
  mode?: string;
  type?: string; ts: number; // creation timestamp: lastAccess: number; // last access timestamp (for LRU)
...
export function memoryStats() {
  export function memoryStats() {
    return {
      size: memoryCache.size: keys: Array.from(memoryCache.keys()).slice(0, 20), maxItems: MAX_ITEMS;
      ttlMs: TTL_MS
    };
   }
