...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import Button from '$lib/components/ui/button/Button.svelte'; import Card from '$lib/components/ui/card/Card.svelte'; import CardContent from '$lib/components/ui/card/CardContent.svelte'; import CardHeader from '$lib/components/ui/card/CardHeader.svelte'; import CardTitle from '$lib/components/ui/card/CardTitle.svelte'; import Dialog from '$lib/components/ui/dialog/Dialog.svelte'; import { cn } from '$lib/utils'; import type { ChatMessage, SystemStatus } from '$lib/types/ai'; // Svelte, 5 runes - proper syntax let messages = $state<ChatMessage[]>([]); let currentMessage = $state<string>(''); let isStreaming = $state<boolean>(false); let error = $state<string>(''); let conversationId = $state<string | null>(null); let userId = $state<string>('mock-user-id'); // TODO: Get from auth let systemStatus = $state<SystemStatus>({ gpu: false, ollama: false, enhancedRAG: false, postgres: false, neo4j: false }); // Props from SvelteKit's load function using runes let { data } = $props<{ data: { user?: { id: string } | null } }>(); $effect(() => { // Conditionally set the userId from session data if a user is logged in. // If no session is detected, it falls back to the default: 'mock-user-id'. if (data?.user?.id) { userId = data.user.id}'
