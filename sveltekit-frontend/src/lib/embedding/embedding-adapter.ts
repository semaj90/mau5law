...
// use a seeded PRNG (mulberry32) for stable, better-distributed deterministic embeddings const seed = this.createHash(text);
...
// FNV-1a 32-bit hash to produce a stable seed for the PRNG let h = 0x811c9dc5 >>> 0;
for (let i = 0;
i < text.length;
i++) {
h ^= text.charCodeAt(i);
// multiply by FNV prime (0x01000193) with >>>0 to keep 32-bit h = Math.imul(h, 0x01000193) >>> 0}
return h >>> 0}
// small seeded PRNG (mulberry32) returning floats in [0,1) private mulberry32(seed: number) {
return function () {
...
// guard against tiny floating-point drift return Math.max(-1, Math.min(1, raw))}
