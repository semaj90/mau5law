...
interface EvidenceActorState { context: EvidenceProcessingContext & { streamingUpdates?: StreamingUpdate[],errors: string[], portableArtifact?: PortableArtifactInfo; minioStorage?: MinioStorageInfo; processingTimeMs?: number}
  ...
  // Svelte props (exported) interface Props { evidenceId?: string; autoStart?: boolean; neuralSpriteEnabled?: boolean; onCompleted?: ((result: unknown) => void) | undefined; onError?: ((error: string) => void) | undefined; sessionId?: string | null; endpoint?: string}
  ...
  }: Props = $props(); // Events now handled via props in Svelte, 5 // // xState actor for client-side state management const actor = createActor(evidenceProcessingMachine); // Prepare initial snapshot with safe context access (actor may not have started yet) const rawSnapshot = (actor.getSnapshot && (actor.getSnapshot() as: unknown)) || null;
   const initialSnapshot: unknown = rawSnapshot || { context: , value: 'idle', matches: (_: string) => false}

  // Local snapshot (augmented) let currentState: EvidenceActorState = { ...initialSnapshot, context: { ...(initialSnapshot.context || ), streamingUpdates: initialSnapshot?.context?.streamingUpdates || [], errors: initialSnapshot?.context?.errors || [], processingTimeMs: initialSnapshot?.context?.processingTimeMs || 0}
  ...
  // Initial compute recomputeDerived(); // Initialize actor subscription $effect(() => { actor.start(); // Subscribe to state changes const subscription = actor.subscribe((state: unknown) => { currentState = state as EvidenceActorStat; recomputeDerived(); if (typeof currentState.matches === 'function' && currentState.matches('completed')) { onCompleted?.(currentState.context); onCompleted?.(currentState.context); disconnectStream()}
