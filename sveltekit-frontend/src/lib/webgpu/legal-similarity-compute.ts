...
export class LegalSimilarityWebGPU {
      ...
      code: `
        ...
        fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
          ...
          for (var i: u32 = 0u; i < vector_chunks; i = i + 1u) {
            ...
            let w0 = legal_domain_weights[query_offset + base_idx];
            let w1 = legal_domain_weights[query_offset + base_idx + 1u];
            let w2 = legal_domain_weights[query_offset + base_idx + 2u];
            let w3 = legal_domain_weights[query_offset + base_idx + 3u];
            dot_product = dot_product + (q0 * d0 + q1 * d1 + q2 * d2 + q3 * d3);
          ...
          for (var i: u32 = vector_chunks * 4u; i < uniforms.vector_dimension; i = i + 1u) {
            ...
            let w_val = legal_domain_weights[query_offset + i];
          ...
          let confidenceVal = similarity_strength * magnitude_balance * uniforms.confidence_boost;
          ...
          if (final_score >= uniforms.similarity_threshold) {
            ...
            results[idx].query_index = query_idx;
            results[idx].document_index = doc_idx;
            results[idx].similarity = final_score;
  ...
  async computeLegalSimilarity(
    ...
    try {
      ...
      const strideBytes = 6 * 4; // 6 fields * 4 bytes
      for (let i = 0; i < totalResults; i++) {
        const offset = i * strideBytes;
        const qIdx = dataView.getUint32(offset, true);
        const dIdx = dataView.getUint32(offset + 4, true);
        const similarity = dataView.getFloat32(offset + 8, true);
        const confidence = dataView.getFloat32(offset + 12, true);
        const legalScore = dataView.getFloat32(offset + 16, true);
        const riskAssessment = dataView.getFloat32(offset + 20, true);
        // Skip invalid
  ...
  private async applyTopKOptimization(totalResults: number, k: number): Promise<void> {
    ...
    for (let i = 0; i < k; i++) {
      const offset = i * 6;
      // Use bit pattern for invalid u32 indexes via DataView when writing, but for simplicity write floats:
      invalidResults[offset] = Number(4294967295); // placeholder; will be interpreted as float, but shader checks uses 0xFFFFFFFFu
      invalidResults[offset + 1] = Number(4294967295);
      invalidResults[offset + 2] = -1.0;
    ...
    commandEncoder.copyBufferToBuffer(topKResultsBuffer, 0, this.resultsBuffer, 0, topKResultsSize);
    this.device.queue.submit([commandEncoder.finish()]);
