...
const originalPOSTHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    const reportContent = generateReportContent(template, caseId, reportId);
    return json({
      success: true,
  ...
  let content = `
    <div style="text-align: center; margin-bottom: 40px;">
      ...
      <p style="color: #6b7280; font-size: 16px; margin: 0;">
        Generated on ${formattedDate} | ID: ${caseId || 'N/A'} | Report ID: ${reportId || 'N/A'}
      </p>
    </div>
  `;
  ...
  content += `
    <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 16px; margin-top: 40px;">
      <p style="margin: 0; font-size: 14px; color: #6b7280; font-style: italic;">
        <strong>AI Disclaimer:</strong> This report was generated using AI assistance. Please review all content for accuracy and completeness. Legal analysis should be verified by qualified legal professionals before use in official proceedings.
      </p>
    </div>
  `;
  return content;
...
function generateSectionContent(section: string, reportType: string): string {
  const sampleContent: { [key: string]: string } = {
    'Executive Summary': `
      <p>This ${reportType.toLowerCase()} provides a comprehensive analysis of the case materials and evidence. Based on the available information, this report identifies key findings and recommendations for further action.</p>
    ...
    `,
    'Key Facts': `
      <p>The following facts have been established through investigation and analysis:</p>
      <ol>
    ...
    `,
    'Evidence Summary': `
      <p>Evidence collected includes:</p>
  ...
  return sampleContent[section] || `
    <p>This section will contain detailed information about ${section.toLowerCase()}. Please review and customize this content based on the specific case requirements.</p>
