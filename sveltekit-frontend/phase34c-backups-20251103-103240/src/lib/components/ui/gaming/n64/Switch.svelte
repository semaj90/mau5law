  ...
  async function playSwitchSound(isOn: boolean): Promise<any> {
    ...
    try {
      ...
      if (isOn) {
        ...
        oscillator2.frequency.exponentialRampToValueAtTime(880, ctx.currentTime + 0.08)} else {
        oscillator1.type = 'square';
      ...
      oscillator2.stop(ctx.currentTime + 0.12)} catch (error) {
...
<style>
  ...
  .n64-switch {
    ...
    height: var(--switch-height); cursor: pointer
    /* 3D transformations */
  ...
  .switch-track {
   ...
   , background: var(--track-bg); border-radius: calc(var(--switch-height) / 2);
    overflow: hidden
    /* 3D track styling */
    box-shadow:
  ...
  .switch-knob {
    ...
    height: var(--knob-size); background: var(--knob-bg),
    border-radius: 50%;
    /* 3D knob styling */
