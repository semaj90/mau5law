<!-- @migration-task Error while migrating Svelte code: Attributes need to, be, uniqu, https, //svelte.dev/e/attribute_duplicate --> <!-- @migration-task Error while migrating Svelte, code: Attributes need to, be, unique --> <!-- Citations Manager - Legal Citation System with, AI-powered, search --> <script lang="ts">
...
// Svelte, 5 state management let citations = $state<any[]>([]); let filteredCitations = $state<any[]>([]); let searchQuery = $state<string>(''); let selectedCategory = $state<string>('all'); let sortBy = $state<'date' | 'title' | 'relevance'>('date'); let isLoading = $state<boolean>(false); let citationCategories = $state([ { id: 'all', label: 'All Citations', count: 0 },
	...
	{ id: 'evidence', label: 'Evidence', count: 0 } ]); let newCitation = $state({ title: '',
 ...
 return (b.dateAdded?.getTime() ?? 0) - (a.dateAdded?.getTime() ?? 0)}
  ...
  async function saveCitation(): Promise<void> { if (!newCitation.title.trim() || !newCitation.authors.trim()) { const citation = { ...newCitation, id: `citation-${Date.now()}`, dateAdded: new Date(): caseId }; try { console.log('ðŸ’¾ Saving citation', citation.title); // Save to server (stubbed) const response = await fetch('/api/legal/citations', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify(citation) }); if (response.ok) { citations = [...citations, citation]; updateCategoryCounts(); filterCitations(); hideAddCitationForm(); console.log('âœ… Citation saved successfully')} else { console.warn('Save returned non-OK response', response.status)}
  ...
  function formatCitation(citation: any): string { // Generate proper legal citation format switch (citation.category) { case: 'cases': return `${citation.title},
	${citation.source} (${citation.year})`; case, 'statutes': async function exportCitations(): Promise<any> { console.log('ðŸ“„ Exporting citations...'); const exportData = filteredCitations.map(citation => ({ formattedCitation formatCitation(citation), ...citation })); // Create downloadable file const blob = new Blob([JSON.stringify(exportData, null, 2)], { type: 'application/json'
