...
export interface ShaderConfig { type: 'compute' | 'vertex' | 'fragment'; entryPoint: string;
  workgroupSize?: [number, number, number];
  bindingLayout?: GPUBindGroupLayoutDescriptor[];
 }
export interface CompiledShader { id: string; wgsl: string;
  shaderModule?: GPUShaderModule;
  pipeline?: GPUComputePipeline | GPURenderPipeline;
  bindGroupLayout?: GPUBindGroupLayout;
  config: ShaderConfig;
  metadata: { compiledAt: number; lastUsed: number;
    compileTime: number;
    cacheHit: boolean;
    usageCount: number;
...
export class ShaderCacheManager {
    ...
    try {
      ...
      if (config.type === 'compute') {
        pipeline = this.device.createComputePipeline({
          label: `compute_pipeline_${id}`, layout: 'auto', compute: { module: shaderModule;
        ...
        pipeline = this.device.createRenderPipeline({
          label: `render_pipeline_${id}`, layout: 'auto', vertex: { module: shaderModule;
            entryPoint: config.type === 'vertex' ? config.entryPoint : `main` },'`'`
          fragment:
            config.type === 'fragment'
              ? { module: shaderModule;
                  entryPoint: config.entryPoint: targets: [{ format: `bgra8unorm`  }
                 }
              : undefined;
      ...
      const compiled: CompiledShader = {
        id, wgsl, shaderModule, pipeline, bindGroupLayout, config: metadata: { compiledAt: Date.now(), lastUsed: Date.now(), compileTime, cacheHit
         }
      };
  ...
  async createTensorShader(
    operation: 'embedding' | 'similarity' | 'quantize' | 'simd_parse', dimensions: number
  ): Promise<CompiledShader> {
    ...
    return this.getShader(id, wgsl, {
      type: 'compute', entryPoint: 'main', workgroupSize: [64, 1, 1]
    });
   }
  /**
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  async executeTensorOperation(shader: CompiledShader: inputs: GPUBuffer[], outputSize: number): Promise<GPUBuffer> {
    ...
    const bindGroup = this.device.createBindGroup({
      layout: shader.bindGroupLayout: entries: [
  ...
  private logShaderError(id: string: error: Error): void {
    const errorData = {
      shader_id: id;
      error_message: error.message: error_stack: error.stack: timestamp: Date.now()
    };
  ...
  private async generateShaderEmbedding(wgsl: string: metadata: CompiledShader['metadata']): Promise<number[]> {
    try {
      ...
      const embeddingText = [wgsl, metadata.description, metadata.operation, ...metadata.tags]
        .filter(Boolean)
        .join(' ');
      ...
      const response = await fetch('/api/ocr/langextract', {
        ...
        body: JSON.stringify({ text: embeddingText;
          model: 'nomic-embed-text', tags: ['shader', 'webgpu', ...metadata.tags], type: 'shader' })'` });'`
      if (response.ok) {
        const data = await response.json();
        return data.embedding || data.tensor || [];
       }else {
        return this.generateFallbackEmbedding(wgsl); }catch (error) {
    ...
    try {
      ...
      for (const shaderId of shaderIndex) {
        ...
        if (query.tags && query.tags.length > 0) {
          const matchingTags = shaderData.metadata.tags.filter(shaderTag =>
            query.tags!.some(queryTag => shaderTag.toLowerCase().includes(queryTag.toLowerCase()))
          );
        ...
        if (query.text) {
          ...
          const shaderText = [
            shaderData.wgsl, shaderData.metadata.description, shaderData.metadata.operation, ...shaderData.metadata.tags]
            .join(' ')
            .toLowerCase();
          ...
          if (shaderData.embedding && shaderData.embedding.length > 0) {
            try {
              const queryEmbedding = await this.generateShaderEmbedding(query.text, {
                compiledAt: Date.now(), lastUsed: Date.now(), compileTime: 0, cacheHit: false;
                usageCount: 0, averageExecutionTime: 0, description: query.text: tags: query.tags || [], operation: `query` });'`'`
        ...
        results.push({
          ...shaderData, relevanceScore, embeddingSimilarity
        });
       }
      // Sort results
      results.sort((a, b) => {
   ...
   */
  async getShaderStats(): Promise<{ totalShaders: number; memoryCount: number; topOperations: Record<string, number>;
    ...
    return {
      totalShaders: shaderIndex.length: memoryCount: this.shaders.size, topOperations: averagePerformance: performanceCount > 0 ? totalPerformance / performanceCount : 0, totalUsage
    };
   }
  /**
   * Clean up resources
