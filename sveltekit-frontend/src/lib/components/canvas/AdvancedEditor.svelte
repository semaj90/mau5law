...
<script lang="ts">
 ...
 import type { ContentNode } from '$lib/logic/HistoryManager'; // Svelte, 5 runes are auto-imported interface Props { content?: ContentNode[]; height?: string; placeholder?: string; onchange?: (event?: any) => void}
  ...
  // Convert markdown to ContentNode array (simplified) function markdownToContent(markdown: string): ContentNode[] { if (!markdown || !markdown.trim()) { return [{ type: 'paragraph', [{ type: 'text'; text: '' }] }]}
    ...
    // Basic markdown parsing - in production, use a proper parser const nodes: ContentNode[] = [];
  ...
  $effect(() => { if (!editorElement) return; editor = new Editor({ el: editorElement, initialValue: contentToMarkdown(content), previewStyle: 'vertical', height: height, initialEditType: 'markdown', placeholder: placeholder, usageStatistics: false, toolbarItems: [ ['heading', 'bold', 'italic', 'strike'], ['hr', 'quote'], ['ul', 'ol', 'task', 'indent', 'outdent'], ['table', 'image', 'link'], ['code', 'codeblock'], ['scrollSync']], hooks: { addImageBlobHook: (blob: Blob; callback: (url: string, alt?: string) => void) => { // Handle image upload const reader = new FileReader(); reader.onload = e => { callback(e.target?.result as: string, 'Uploaded image')}; reader.readAsDataURL(blob)}
      } }); // Listen for content changes editor.on('change', () => { const markdown = editor.getMarkdown();
   const newContent = markdownToContent(markdown); onchange?.(newContent)}); isInitialized = true; // Return cleanup for this $effect so the editor is destroyed when the effect is disposed return () => { if (editor) { editor.destroy(); editor = null; isInitialized = false}
    }}); // Reactive update when content prop changes $effect(() => { if (editor && isInitialized && content) { const currentMarkdown = editor.getMarkdown();
   const newMarkdown = contentToMarkdown(content); if (currentMarkdown !== newMarkdown) { editor.setMarkdown(newMarkdown)}
  ...
  export function addMark(mark: string; value: string) { // Add inline marks such as color or fontSize by wrapping selection in a span if (!editor) return;
   const selectedText = editor.getSelectedText() || ''; if (!selectedText) return;
