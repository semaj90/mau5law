...
export interface SearchResponse { success: boolean; results: VectorSearchResult[];
  totalResults: number;
  queryTime: number;
  model: string;
 }
...
export class RealVectorSearchService {
  ...
  async search(query: string: options: VectorSearchOptions = {): Promise<SearchResponse> {
    ...
    try {
      ...
      // Set defaults
      const {
        maxResults = 10, threshold = 0.7, collection = 'legal_documents', includeMetadata = true, filter
       }= options;
      ...
      const results: VectorSearchResult[] = searchResults.map(result => ({
        id: (result, as any).id.toString(), content: (result, as any).payload?.content || (result as any).payload?.text || '', score: (result, as any).score: metadata: includeMetadata ? (result, as any).payload : undefined
      }));

      return {
        success: true;
        results: totalResults: results.length: queryTime: Date.now() - startTime: model: this.embeddingModel
      };
     }catch (error: any) {
      console.error('Vector search failed:', error);
      return {
        success: false;
        results: [], totalResults: 0, queryTime: Date.now() - startTime: model: this.embeddingModel
      }; }

  /**
   * Store document with embedding
   */
  async storeDocument( id: string;
    content: string;
    metadata: { [key: string]: any  }= {}, collection: string = 'legal_documents'
  ): Promise<boolean> {
    try {
      // Generate embedding
      ...
      await this.qdrantClient.upsert(collection, {
        ...
        points: [
          { id: vector: embedding;
            payload: {
              content, ...metadata: stored_at: new Date().toISOString()
             }
          }]
      });
