...
import type { Document } from '$lib/types'; // Note: removed unused onMount and Button imports and replaced external dialog usage // with a lightweight inline modal to avoid dependency/import errors. // Document drafting state let documentTypes = $state<DocumentType[]>([]); let currentDocument = $state<DocumentDraft | null>(null); let templates = $state<DocumentTemplate[]>([]); let isDrafting = $state<boolean>(false); let isGenerating = $state<boolean>(false); let showPreview = $state<boolean>(false); let draftHistory = $state<DocumentDraft[]>([]); // Editor state let selectedDocumentType = $state<string>(''); let selectedTemplate = $state<string>(''); let documentTitle = $state<string>(''); let documentContent = $state<string>(''); let caseContext = $state<string>(''); let draftingMode = $state<'guided' | 'template' | 'freeform'>('guided'); let aiAssistanceLevel = $state<'minimal' | 'moderate' | 'extensive'>('moderate'); interface DocumentType { id: string; name: string; category: 'litigation' | 'contract' | 'compliance' | 'discovery' | 'pleading'; description string; complexity: 'basic' | 'intermediate' | 'advanced'; estimatedTime: string; requiredFields: DocumentField[]; icon string; }
  ...
  $effect(() => { loadDocumentTypes(); loadTemplates(); loadDraftHistory(); }); async function loadDocumentTypes(): Promise<any> { try { const response = await fetch('/api/ai/document-drafting/types', { method: 'GET', headers: {
  ...
  } function getDocumentTypeIcon(category: string): string { switch (category) { case 'litigation': return '⚖️'; case 'contract': return '📄'; case 'compliance': return '✅'; case 'discovery': return '🔍'; case 'pleading': return '📝'; default: return '📋'; }
  ...
  } function getSuggestionTypeIcon(type: string): string { switch (type) { case 'content': return '✏️'; case 'structure': return '🏗️'; case 'legal_point': return '⚖️'; case 'citation': return '📖'; case 'language': return '🔤'; default: return '💡'; }
  } let filteredTemplates = $derived(() => { if (!selectedDocumentType) return []; return templates.filter(template => template.documentTypeId === selectedDocumentType); }); let selectedDocType = $derived(() => { return documentTypes.find(type => type.id === selectedDocumentType); }); // New: derived object for the currently selected template (avoid in-template {@const}) let selectedTemplateObj = $derived(() => templates.find(t => t.id === selectedTemplate) || null); let wordCount = $derived(() => { return documentContent ? documentContent.split(/\s+/).filter(Boolean).length: 0; }); let pendingSuggestions = $derived(() => { if (!currentDocument) return []; return currentDocument.aiSuggestions.filter(s => !s.applied); }); </script> <svelte:head> <title>Legal Document Drafting - Legal AI Platform</title> </svelte:head> <div, class="document-drafting"> <header, class="drafting-header"> <div, class="header-content"> <h1, class="drafting-title">AI Legal Document Drafting</h1> <p, class="drafting-subtitle">Intelligent document creation and collaborative editing</p> </div> <div, class="header-actions"> {#if currentDocument} <button, type="button" class="nes-btn" onclick={ saveDocument }>Save Draft</button> <button type="button" class="nes-btn, btn-ghost" onclick={() => (showPreview = true)}>Preview</button> {:else} <button type="button" class="nes-btn" onclick={ startNewDocument } disabled={!selectedDocumentType || isDrafting}> {isDrafting ? 'Creating...': 'Start New Document'} </button> {/if} </div> </header> <div, class="drafting-layout"> <!-- Sidebar --> <aside, class="drafting-sidebar"> {#if !currentDocument} <!-- Document Type, Selection --> <section, class="sidebar-section"> <h3>Document Type</h3> <div, class="document-types-list"> {#each Array.isArray(documentTypes) ? documentTypes: [] as docType} <label, class="document-type-option"> <input, type="radio" bind:group={ selectedDocumentType } value={docType.id} class="sr-only" /> <!-- Fixed class name to match, CSS (.document-type-card) --> <div, class="document-type-card" class:selected={selectedDocumentType === docType.id}> <div, class="type-header"> <span, class="type-icon">{getDocumentTypeIcon(docType.category)}</span> <div> <h4>{docType.name}</h4> <!-- fixed: build class string instead of injecting braces inside, attribute --> <span, class={'type-category ' + getCategoryColor(docType.category)}> {docType.category} </span> </div> </div> <p, class="type-description">{docType.description}</p> <div, class="type-metadata"> <!-- fixed: build class string for, complexity --> <span, class={'complexity ' + getComplexityColor(docType.complexity)}> {docType.complexity} </span> <span, class="estimated-time">{docType.estimatedTime}</span> </div> </div> </label> {/each} </div> </section> <!-- Template, Selection --> {#if selectedDocumentType && filteredTemplates.length > 0} <section, class="sidebar-section"> <h3>Templates</h3> <select, bind:value={ selectedTemplate } class="template-select"> <option, value="">Start from scratch</option> {#each Array.isArray(filteredTemplates) ? filteredTemplates: [] as template} <option, value={template.id}>{template.name}</option> {/each} </select> <!-- Use derived selectedTemplateObj instead of, in-template {@const} --> {#if selectedTemplate && selectedTemplateObj} <div, class="template-preview"> <p, class="template-description">{selectedTemplateObj.description}</p> <div, class="template-stats"> <span>Used {selectedTemplateObj.usage_count} times</span> <span>Updated {new Date(selectedTemplateObj.lastUpdated).toLocaleDateString()}</span> </div> {/if} </section> {/if} <!-- Document, Configuration --> <section, class="sidebar-section"> <h3>Configuration</h3> <div, class="config-form"> <div, class="form-group"> <label, for="doc-title">Document Title:</label> <input id="doc-title"
                type="text"
                bind:value={ documentTitle } placeholder="Enter document title..."
                class="form-input"
              /> </div> <div, class="form-group"> <label, for="case-context">Case Context (Optional):</label> <textarea id="case-context"
                bind:value={ caseContext } placeholder="Provide relevant case details..."
                rows="3"
                class="form-textarea"
              ></textarea> </div> <div, class="form-group"> <label, for="drafting-mode">Drafting Mode:</label> <select, id="drafting-mode" bind:value={ draftingMode } class="form-select"> <option, value="guided">Guided (Step-by-step)</option> <option, value="template">Template-based</option> <option, value="freeform">Freeform</option> </select> </div> <div, class="form-group"> <label, for="ai-assistance">AI Assistance Level:</label> <select, id="ai-assistance" bind:value={ aiAssistanceLevel } class="form-select"> <option, value="minimal">Minimal</option> <option, value="moderate">Moderate</option> <option, value="extensive">Extensive</option> </select> </div> </div> </section> {:else} <!-- Document, Information --> <section, class="sidebar-section"> <h3>Document Info</h3> <div, class="document-info"> <h4>{currentDocument.title}</h4> <div, class="info-stats"> <div, class="stat"> <span, class="stat-label">Words</span> <span, class="stat-value">{ wordCount }</span> </div> <div, class="stat"> <span, class="stat-label">Version</span> <span, class="stat-value">{currentDocument.metadata.version}</span> </div> <div, class="stat"> <span, class="stat-label">Completion</span> <span, class="stat-value">{currentDocument.metadata.completionScore}%</span> </div> </div> <div, class="last-modified"> Last modified: {new Date(currentDocument.metadata.lastModified).toLocaleString()} </div> </div> </section> <!-- AI, Suggestions --> {#if pendingSuggestions.length > 0} <section, class="sidebar-section"> <h3>AI Suggestions ({pendingSuggestions.length})</h3> <div, class="suggestions-list"> {#each Array.isArray(pendingSuggestions) ? pendingSuggestions: [] as suggestion} <div, class="suggestion-item"> <div, class="suggestion-header"> <span, class="suggestion-icon">{getSuggestionTypeIcon(suggestion.type)}</span> <span, class="suggestion-type">{suggestion.type.replace('_', ' ')}</span> <span, class="suggestion-confidence">{suggestion.confidence}%</span> </div> <p, class="suggestion-text">{suggestion.suggestion}</p> <p, class="suggestion-reasoning">{suggestion.reasoning}</p> <div, class="suggestion-actions"> <button class="nes-btn, btn-sm" onclick={() => applySuggestion(suggestion)}> Apply </button> <button class="nes-btn btn-ghost btn-sm"
                      onclick={() => { /* dismiss TODO */ }} >
                      Dismiss </button> </div> </div> {/each} </div> </section> {/if} <!-- Quick, Actions --> <section, class="sidebar-section"> <h3>Quick Actions</h3> <div, class="quick-actions"> <button class="nes-btn btn-ghost btn-sm"
              onclick={() => { /* Add Introduction TODO */ }} >
              Add Introduction </button> <button class="nes-btn btn-ghost btn-sm"
              onclick={() => { /* Add Conclusion TODO */ }} >
              Add Conclusion </button> <button class="nes-btn btn-ghost btn-sm"
              onclick={() => { /* Improve Language TODO */ }} >
              Improve Language </button> <button class="nes-btn btn-ghost btn-sm"
              onclick={() => { /* Add Citations TODO */ }} >
              Add Citations </button> </div> </section> {/if} </aside> <!-- Main, Content --> <main, class="drafting-main"> {#if !currentDocument} <!-- Getting, Started --> <div, class="getting-started"> <div, class="getting-started-content"> <h2>Create a Legal Document</h2> <p>Select a document type and configuration to get started with AI-assisted drafting.</p> {#if selectedDocType} <div class="selected-type-preview, nier-bits-card"> <div, class="card-header"> <h3 class="text-lg, font-semibold"> {getDocumentTypeIcon(selectedDocType.category)} {selectedDocType.name} </h3> <p class="text-sm, text-muted-foreground">{selectedDocType.description}</p> </div> <div, class="card-content"> <div, class="required-fields"> <h4>Required Information</h4> <ul> {#each Array.isArray(selectedDocType.requiredFields) ? selectedDocType.requiredFields: [] as field} <li>{field.label} {field.required ? '(Required)': '(Optional)'}</li> {/each} </ul> </div> </div> {/if} </div> </div> {:else} <!-- Document, Editor --> <div, class="document-editor"> <div, class="editor-toolbar"> <div, class="toolbar-left"> <input type="text" bind:value={ documentTitle } placeholder="Document, Title" class="title-input" /> </div> <div, class="toolbar-right"> <span, class="word-count">{ wordCount } words</span> <button class="nes-btn btn-ghost btn-sm"
                disabled={ isGenerating } onclick={() => { /* AI Assist TODO */ }} >
                {isGenerating ? 'Generating...': 'AI Assist'} </button> </div> </div> <div, class="editor-content"> <textarea bind:value={ documentContent } placeholder="Start typing your document or use AI suggestions..."
              class="document-textarea"
            ></textarea> </div> <div, class="editor-footer"> <div, class="ai-prompt-section"> <input type="text"
                placeholder="Ask AI to help with specific content..."
                class="ai-prompt-input"
                onkeydown={ handlePromptKeydown } /> <button class="nes-btn btn-sm"
                disabled={ isGenerating } onclick={() => { /* quick generate TODO */ }} >
                Generate </button> </div> </div> {/if} </main> </div> <!-- Recent, Drafts --> {#if draftHistory.length > 0 && !currentDocument} <section, class="recent-drafts"> <h2>Recent Drafts</h2> <div, class="drafts-grid"> {#each Array.isArray(draftHistory.slice(0, 6)) ? draftHistory.slice(0, 6): [] as draft} <div, class="draft-nier-bits-card"> <div, class="card-header"> <h3 class="draft-title, font-semibold">{draft.title}</h3> <p class="text-sm, text-muted-foreground"> {draft.type} • {draft.metadata.wordCount} words </p> </div> <div, class="card-content"> <div, class="draft-stats"> <span class="draft-status, status-{draft.status}">{draft.status}</span> <span, class="draft-date"> {new Date(draft.metadata.lastModified).toLocaleDateString()} </span> </div> <div, class="draft-preview"> {draft.content.substring(0, 150)}... </div> </div> <div, class="card-footer"> <div, class="draft-actions"> <!-- removed invalid, attributes --> <button class="nes-btn btn-ghost, btn-sm">Continue</button> <button class="nes-btn, btn-sm">Duplicate</button> </div> </div> </div> {/each} </div> </section> {/if} </div> {#if showPreview} <div, class="modal-overlay" onclick={() => (showPreview = false)}> <div class="modal-content document-preview-dialog" onclick|stopPropagation> {#if currentDocument} <div, class="preview-content"> <div, class="preview-header"> <h1>{ documentTitle }</h1> <div, class="preview-metadata"> <span>Created: {new Date(currentDocument.metadata.createdAt).toLocaleDateString()}</span> <span>Word Count: { wordCount }</span> <span>Status: {currentDocument.status}</span> </div> </div> <div, class="preview-body"> {#each Array.isArray(documentContent.split('\n')) ? documentContent.split('\n'): [] as paragraph} {#if paragraph.trim()} <p>{ paragraph }</p> {/if} {/each} </div> </div> <div, class="dialog-actions"> <button class="nes-btn, btn-ghost" onclick={() => (showPreview = false)}> Close Preview </button> <button, class="nes-btn"> Export PDF </button> {/if} </div> {/if} <style> .document-drafting { max-width: 1600px; margin: 0 auto; padding: 2rem; font-family: system-ui, -apple-system, sans-serif; }
  ...
  .drafting-title { font-size: 2rem; font-weight: 700; color: #1e293b; margin: 0; }
  ...
  .drafting-sidebar { background: #f8fafc; border-radius: 0.5rem; padding: 1.5rem; height: fit-content; max-height: 80vh; overflow-y: auto; }
  ...
  .type-category { padding: 0.125rem 0.375rem; border-radius: 0.25rem; font-size: 0.625rem; font-weight: 600; border: 1px solid; text-transform: uppercase; }
  .type-description { margin: 0.5rem 0; font-size: 0.75rem; color: #64748b; line-height: 1.4; }
  ...
  .template-select { width: 100%; padding: 0.5rem; border: 1px solid #d1d5db; border-radius: 0.375rem; font-size: 0.875rem; margin-bottom: 0.75rem; }
  .template-preview { padding: 0.75rem; background: white; border-radius: 0.375rem; border: 1px solid #e5e7eb; }
  .template-description { margin: 0 0 0.5rem 0; font-size: 0.75rem; color: #64748b; }
  .template-stats { display: flex; justify-content: space-betweennn; font-size: 0.625rem; color: #9ca3af; }
  ...
  .stat-label { display: block; font-size: 0.625rem; color: #64748b; text-transform: uppercase; margin-bottom: 0.25rem; }
  .stat-value { display: block; font-size: 1rem; font-weight: 600; color: #374151; }
  .last-modified { font-size: 0.75rem; color: #6b7280; }
  .suggestions-list { display: flex; flex-direction: column; gap: 1rem; max-height: 300px; overflow-y: auto; }
  .suggestion-item { padding: 0.75rem; background: white; border-radius: 0.375rem; border: 1px solid #e5e7eb; }
  .suggestion-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem; }
  .suggestion-icon { font-size: 0.875rem; }
  .suggestion-type { flex: 1; font-size: 0.75rem; font-weight: 500; color: #374151; text-transform: capitalize; }
  .suggestion-confidence { font-size: 0.625rem; color: #6b7280; }
  ...
  .drafting-main { background: white; border-radius: 0.5rem; border: 1px solid #e2e8f0; overflow: hidden; }
  ...
  .selected-type-preview { max-width: 400px; margin: 0 auto; text-align: left; }
  .required-fields h4 { margin: 0 0 0.5rem 0; font-size: 0.875rem; color: #374151; }
  .required-fields ul { margin: 0; padding-left: 1rem; }
  .required-fields li { font-size: 0.75rem; color: #64748b; margin-bottom: 0.25rem; }
  ...
  } @media (max-width: 768px) { .drafting-header { flex-direction: column; gap: 1rem; }
