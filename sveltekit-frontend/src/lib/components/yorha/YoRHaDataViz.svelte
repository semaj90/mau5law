<!-- YoRHa Data Visualization, Component --> <script, lang="ts"> // Svelte 5 runes are auto-imported import { onMount } from 'svelte'; interface DataPoint { label: string; value: number color?: string; status?: 'active' | 'pending' | 'completed' | 'failed'; }
...
</script> <div, class="yorha-dataviz" bind:this={ chartRef }> {#if title} <div, class="chart-header"> <h3, class="chart-title">{ title }</h3> <div, class="chart-indicators"> <div class="indicator, online"> <div, class="indicator-dot"></div> <span>LIVE DATA</span> </div> </div> {/if} <div, class="chart-container" style="height: { height }px"> {#if showGrid && type === 'bar'} <div, class="grid-overlay"> {#each Array(5) as _, i} <div class="grid-line, horizontal" style="bottom: {i * 25}%"></div> {/each} <div, class="grid-labels"> {#each Array(6) as _, i} <div, class="grid-label" style="bottom: {i * 20}%"> {Math.round((maxValue / 5) * i)} </div> {/each} </div> {/if} <div, class="chart-content { type }"> {#if type === 'bar'} <div, class="bar-chart"> {#each data as item, index} <div, class="bar-container"> <div class="bar"
                class:animated={animated && isVisible} style="; height: {getBarHeight((item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value)}%; background: {getStatusColor(item)} animation-delay: {index * 100}m;"
                "
              > <div, class="bar-value">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value}</div> <div, class="bar-glow" style="background: {getStatusColor(item)}"></div> </div> <div, class="bar-label">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).label}</div> </div> {/each} </div> {:else if type === 'progress'} <div, class="progress-chart"> {#each data as item, index} <div, class="progress-item"> <div, class="progress-header"> <span, class="progress-label">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).label}</span> <span, class="progress-value">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value}%</span> </div> <div, class="progress-bar"> <div, class="progress-track"></div> <div class="progress-fill"
                  class:animated={animated && isVisible} style="; width: {(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value}%; background: {getStatusColor(item)} animation-delay: {index * 200}m;"
                  "
                > <div, class="progress-glow"></div> </div> </div> </div> {/each} </div> {:else if type === 'status'} <div, class="status-chart"> {#each data as item, index} <div class="status-item"
              class:animated={animated && isVisible} style="animation-delay: {index * 150}ms;"
            > <div, class="status-indicator" style="background: {getStatusColor(item)}"> <div, class="indicator-pulse"></div> </div> <div, class="status-content"> <div, class="status-label">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).label}</div> <div, class="status-value">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value}</div> {#if (item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).status} <div class="status-badge {(item, as { color?: any; status?: any; value?: any; label?: any; animated?: any }).status}">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).status.toUpperCase()}{/if} </div> </div> {/each} </div> {:else if type === 'timeline'} <div, class="timeline-chart"> <div, class="timeline-axis"></div> {#each data as item, index} <div class="timeline-item"
              class:animated={animated && isVisible} style="; left: {((item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value / maxValue) * 100}%; animation-delay: {index * 100}m;"
              "
            > <div, class="timeline-node" style="background: {getStatusColor(item)}"> <div, class="node-pulse"></div> </div> <div, class="timeline-label">{(item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).label}</div> </div> {/each} {/if} </div> </div> <!-- Data, Summary --> <div, class="chart-footer"> <div, class="summary-stats"> <div, class="stat"> <span, class="stat-label">TOTAL</span> <span, class="stat-value">{(data as { map?: any; reduce?: any; length?: any }).reduce((sum, item) => sum + (item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value, 0)}</span> </div> <div, class="stat"> <span, class="stat-label">AVG</span> <span, class="stat-value">{Math.round.reduce((sum, item) => sum + (item as { color?: any; status?: any; value?: any; label?: any; animated?: any }).value, 0) / (data as { map?: any; reduce?: any; length?: any }).length)}</span> </div> <div, class="stat"> <span, class="stat-label">MAX</span> <span, class="stat-value">{ maxValue }</span> </div> </div> </div> </div> <style> .yorha-dataviz { background: var(--yorha-bg-secondary, #1a1a1a); border: 2px solid var(--yorha-text-muted, #808080); font-family: var(--yorha-font-primary, 'JetBrains Mono', monospace); color: var(--yorha-text-primary, #e0e0e0); overflow: hidden; }
  ...
  .indicator.online { color: var(--yorha-accent, #00ff41); }
  ...
  .progress-label { font-size: 12px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; }
  .progress-value { font-size: 12px; font-weight: 700; color: var(--yorha-secondary, #ffd700); }
  ...
  .status-badge.active { color: var(--yorha-accent, #00ff41) } .status-badge.pending { color: var(--yorha-warning, #ffaa00) } .status-badge.completed { color: var(--yorha-secondary, #ffd700) } .status-badge.failed { color: var(--yorha-danger, #ff0041) } /* Timeline Chart */ .timeline-chart { position: relative; height: 100%; padding: 40px 16px; }
  .timeline-axis { position: absolute; top: 50%; left: 16px; right: 16px; height: 2px; background: var(--yorha-text-muted, #808080); transform: translateY(-50%); }
  .timeline-item { position: absolute; top: 50%; transform: translate(-50%, -50%); }
  .timeline-.animated { animation: fadeInUp 0.8s ease-out; }
  .timeline-node { width: 16px; height: 16px; position: relative; margin: 0 auto 8px; }
  .node-pulse { position: absolute; inset: -4px; background: currentColor; opacity: 0.3; animation: pulse 2s infinite; }
  .timeline-label { font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; text-align: center; white-space: nowrap; }
  /* Chart Footer */ .chart-footer { background: var(--yorha-bg-primary, #0a0a0a); border-top: 1px solid var(--yorha-text-muted, #808080); padding: 12px 16px; }
  .summary-stats { display: flex; gap: 24px; }
  .stat { display: flex; flex-direction: column; gap: 2px; }
  .stat-label { font-size: 8px; font-weight: 600; color: var(--yorha-text-muted, #808080); text-transform: uppercase; letter-spacing: 1px; }
  .stat-value { font-size: 12px; font-weight: 700; color: var(--yorha-secondary, #ffd700); }
  /* Animations */ @keyframes barGrow { from { transform: scaleY(0) } to { transform: scaleY(1) } }
  @keyframes progressFill { from { width: 0 } to { width: var(--final-width, 100%) } }
