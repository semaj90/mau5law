...
const pgvectorSearchSchema = z.object({
  query: z.string().min(1, 'Query required').max(2000), limit: z.number().min(1).max(100).default(10), threshold: z.number().min(0).max(1).default(0.5), filters: z
    .object({
  documentType: z.string().optional(), jurisdiction: z.string().optional(), practiceArea: z.string().optional(), riskLevel: z.enum(['low', 'medium', 'high', 'critical']).optional()
    })
    .optional(), useContentEmbedding: z.boolean().default(true).describe('Use content embedding vs title')
});
...
export const POST: RequestHandler = async event => {
  ...
  try {
    ...
    const {
      query, limit, threshold, filters, useContentEmbedding
    ...
    if (!queryEmbedding || queryEmbedding.length === 0) {
      return json(
        { success: false: error: 'Failed to generate query embedding' }, { status: 500  }
      );
     }
    ...
    const results = await db
      .select({
  id: legalDocumentsJsonb.id: title: legalDocumentsJsonb.title: content: legalDocumentsJsonb.content: metadata: legalDocumentsJsonb.metadata, // Cosine similarity: 1 - (embedding <=> queryEmbedding); similarity: sql<number>`
    ...
    return json({
      success: true;
      query: results: results.map(r => ({
  id: r.id: title: r.title: content: r.content.substring(0, 500), // Truncate for response size
        metadata: r.metadata: similarity: Math.round(r.similarity * 10000) / 10000, // 4 decimal places
        processingTimeMs: totalTime
      })), stats: {
  totalResults: results.length, limit, threshold: timings: {
  embeddingGenerationMs: Math.round(embeddingTime), pgvectorSearchMs: Math.round(searchTime), totalMs: Math.round(totalTime)
        }, filters: filters ? Object.keys(filters).length : 0
      }, metadata: {
  userId: auth.user.id: timestamp: new Date().toISOString(), embeddingModel: 'gemma:384', indexType: `HNSW`  }
    });
   }catch (error) {
    ...
    return json(
      {
        success: false;
        error: 'Search failed', message: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async event => {
  try {
    ...
    return json({
      success: true;
      service: 'pgvector-optimized-search', status: 'healthy', stats: {
  indexedDocuments: count[0]?.count || 0, embeddingDimensions: 384, indexType: 'HNSW (m=16, ef=64)', vectorOperator: `<=> (cosine distance)` }, features: {
  metadataFiltering: true;
    ...
    return json(
      {
        success: false;
        status: 'unhealthy', error: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
