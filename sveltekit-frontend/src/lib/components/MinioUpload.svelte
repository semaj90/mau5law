<script lang="ts"> import { writable } from 'svelte/store'; let file: File | null = null; const message = writable<string>(''); const uploading = writable(false); function handleFileChange(e: Event) { const input = e.currentTarget as HTMLInputElement | null; file = input?.files?.[0] ?? null; if (file) message.set(''); }
