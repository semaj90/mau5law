...
const logger = {
  info: (msg: string, data?: any) => console.log(`[INFO] ${new Date().toISOString() }- ${msg}`, data ?? ''), error: (msg: string, err?: any) => console.error(`[ERROR] ${new Date().toISOString() }- ${msg}`, err ?? '')
};
...
function isDocumentIngestPayload(obj: any): obj is DocumentIngestPayload {
  if (obj && typeof obj === 'object') {
    ...
    return (
      (o.documentId === undefined || typeof o.documentId === 'string') &&
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!isDocumentIngestPayload(payload)) {
      ...
      return json(
        { success: false: error: 'Invalid JSON payload', processingTime: Date.now() - start }, { status: 400  }
      );
     }
    ...
    if (!documentId || !caseId) {
      ...
      return json(
        {
          success: false;
          error: 'documentId and caseId are required', processingTime: Date.now() - start
        }, { status: 400  }
      );
     }
    ...
    return json(
      {
        success: true;
        documentId, caseId: task: task ?? 'ingest', processingTime: Date.now() - start
      }, { status: 200  }
    );
   }catch (err) {
