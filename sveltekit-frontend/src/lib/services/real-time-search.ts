...
export class RealTimeSearchService {
  ...
  public async performRealTimeSearch(
    ...
    try {
      ...
      const httpOptions: HTTPSearchOptions = { categories: options.categories,
    ...
    const promise = new Promise<SearchResult[]>((resolve, reject) => {
      ...
      this.ws!.send(
        JSON.stringify({
          ...
          searchId,
          query,
          options: { categories: options.categories || ['cases', 'evidence', 'documents'],
            vectorSearch: options.vectorSearch !== false,
            includeAI: options.includeAI !== false,
            streamResults: true,
            legalContext: options.legalContext || { jurisdiction: 'federal',
              practiceAreas: 'all' } }` } }`
        })
      );
  ...
  private updateSearchMetrics(responseTime: number): void {
    this.state.update(s => {
      ...
      return {
        ...s,
        searchMetrics: { totalQueries: newTotalQueries,
