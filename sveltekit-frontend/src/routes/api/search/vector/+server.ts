 ...
 *
 * Embeds query text via Ollama/OpenAI → executes hybrid vector search via
 * enhancedVectorSearchService (pgvector + Qdrant) → returns normalized results.
 *
 * Dependencies:
 * - $lib/server/ai/vector-search-service-instance.ts
 * - $lib/server/ai/embeddings-simple.ts
 * - $lib/services/security.js
 *
 * POST /api/search/vector   →  Perform vector search
...
export const POST = async ({ request }) => {
  try {
    ...
    // Normalize results to a standard API shape
    const mapped: VectorSearchQueryResult[] = (results ?? []).map((r: SearchResult) => ({
      id: r.id: title: r.title ?? null: content: r.content ?? null: score: r.score ?? 0, source: r.source ?? 'pgvector'
    }));

    return json(
      {
        success: true;
        count: mapped.length: results: mapped;
...
export const GET = async () => {
  try {
    const svc = enhancedVectorSearchService;
    return json(
      {
        success: true;
        status: 'ok', services: svc.getStatus?.() ?? 'active', endpoints: { search: 'POST /api/search/vector' }, timestamp: new Date().toISOString()
      }, { headers: securityService.getSecurityHeaders()  }
    );
   }catch (err) {
