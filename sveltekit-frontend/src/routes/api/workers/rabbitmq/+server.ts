...
import { rabbitmqServiceWorker, startRabbitMQWorker, stopRabbitMQWorker } from '$lib/workers/rabbitmq-service-worker.js'
	...
	try {
		...
		switch (action) {
			case 'start': {
				...
				const enableN64Logging =
					typeof rawConfig['enableN64Style'] === 'boolean'
						? rawConfig['enableN64Style']
						: false
			...
			case 'publish': {
				...
				const message =
					typeof rawMessage === 'object' && rawMessage !== null
						? (rawMessage as MessageWithOptionalId)
						: undefined
						...
						try {
							...
							const msgPayload =
								typeof m.message === 'object' && m.message !== null
									? (m.message as MessageWithOptionalId)
									: undefined
							...
							const failedMsgId =
								typeof failed.message?.id === 'string' ? failed.message!.id : null
