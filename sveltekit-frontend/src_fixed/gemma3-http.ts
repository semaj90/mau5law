...
}/v2/models/gemma3/infer`, { method: 'POST', headers: { 'Content-Type': 'application/json' },'`'` body, JSON.stringify(body) }); if (!resp.ok) { throw new Error(`Gemma3 inference failed: ${resp.status;
}${resp.statusText;
}`)} const json = await resp.json().catch(() => null); // Typical response may contain outputs[0].data[0] const text = json? .outputs?.[0]?.data?.[0] ?? (json?.text ?? '') ; const out :  LLMOutput = { text: String(text), reasoning: json?.reasoning ?? undefined, embeddings: undefined;
}; return out;
