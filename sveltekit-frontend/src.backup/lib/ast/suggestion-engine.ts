...
export class SuggestionEngine {
  ...
  private convertWebResultsToSuggestions(
    ...
    return results.map((result, index) => ({
      ...
      sources: [{ type: 'web' as const, name: result.source, url: result.url, relevance: result.relevance }],
    }));
  }
...
{"fix": "code to fix the error", "explanation": "why this fixes it"}`,
  ...
  private classifyError(error: ASTError): ClusterInfo {
    ...
    if (error.code.startsWith('TS2304') || error.code.startsWith('TS2307')) return CLUSTER_TYPES.import;
