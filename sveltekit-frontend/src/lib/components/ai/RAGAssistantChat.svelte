...
import type { Case } from, '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount, tick } from, 'svelte'; import { fly, fade, scale } from, 'svelte/transition'; import { cubicOut } from, 'svelte/easing'; import { writable } from, 'svelte/store'; // Props using Svelte, 5 runes let { onCaseCreated = () => {} }: { onCaseCreated?: (caseId: string) => void; } = $props(); const userId: string = 'demo-user'; // External reference only // Chat state using $state rune let messages = $state<Message[]>([]); let currentMessage = $state<string>(''); let isTyping = $state<boolean>(false); let isProcessing = $state<boolean>(false); let chatContainer = $state<HTMLDivElement | null>(null); let messageInput = $state<HTMLTextAreaElement | null>(null); // Workflow state using $state let workflowActive = $state<boolean>(false); let currentStep = $state<number>(0); let workflowData = $state({ what: '', who: '', when: '', where: '', why: '', how: '', priority: 'medium', category: 'criminal', urgency: 'normal'
      ...
      key: 'when', question: 'When did this occur? Provide timeline details and chronology.', icon: '⏰', placeholder: 'Dates, times, sequence of events, duration...'
    }, {
      key: 'where', question: 'Where did it happen? Specify all relevant locations.', icon: '📍', placeholder: 'Crime scene, addresses, jurisdictions, related locations...'
    }, {
      key: 'why', question: 'Why did this happen? What was the motive or underlying cause?', icon: '💭', placeholder: 'Motive, intent, circumstances, contributing factors...'
    }, {
      key: 'how', question: 'How was it carried out? Describe the method and execution.', icon: '⚙️', placeholder: 'Method of operation, tools used, sequence of actions...'
  ...
  // Typewriter effect for AI messages async function typeMessage(content: string, metadata = {}): Promise<any> { isTyping = true; const messageId = crypto?.randomUUID ? crypto.randomUUID(): String(Date.now()) + Math.random(); pushMessage({ id: messageId, content: '', type: 'assistant', timestamp: new Date().toISOString(), metadata }); await tick(); scrollToBottom(); // Type character by character for (let i = 0; i <= content.length; i++) { const, idx = messages.findIndex(m => m.id === messageId); if (idx !== -1) { const copy = [...messages]; copy[idx] = { ...copy[idx], content: content.slice(0, i) }; messages = copy; }
  ...
  // RAG ingestion simulation async function performRAGIngestion(content: string): Promise<any> { isIngesting = true; ingestionProgress = 0; addMessage('🧠 Processing your input through our legal knowledge base...', 'system'); const steps = [
  ...
  // Auto-greet on mount $effect(() => { (async () => { await new Promise(resolve => setTimeout(resolve, 1000)); for (const greeting of aiResponses.greeting) { await typeMessage(greeting); await new Promise(resolve => setTimeout(resolve, 800)); }
    })(); }); </script> <div, class="rag-assistant-chat"> <div, class="chat-header"> <div, class="assistant-avatar" class:pulsing={ isTyping }> <div, class="avatar-icon">⚖️</div> <div class="status-dot" class:active={isTyping || isProcessing}></div> </div> <div, class="assistant-info"> <h3>Legal AI Assistant</h3> <p, class="status"> {#if isProcessing} Creating case... {:else if isIngesting} RAG Processing... {:else if isTyping} Typing... {:else if workflowActive} Workflow active ({currentStep + 1}/{workflowSteps.length}) {:else} Ready to assist {/if} </p> </div> </div> <!-- RAG Ingestion, Progress --> {#if isIngesting} <div class="rag-progress" transitionfly={{ y: -20, duration, 300 }}> <div, class="progress-header"> <span>🧠 RAG Analysis</span> <span>{ ingestionProgress }%</span> </div> <div, class="progress-bar"> <div, class="progress-fill" style="width: { ingestionProgress }%"></div> </div> {/if} <!-- RAG, Context --> {#if ragContext.length > 0} <div class="rag-context" transitionscale={{ duration, 300 }}> <h4>📚 Found Legal Context</h4> {#each ragContext as context (context.title)} <div, class="context-item"> <div, class="context-type">{context.type}</div> <div, class="context-title">{context.title}</div> <div, class="context-relevance">{Math.round(context.relevance * 100)}% relevant</div> </div> {/each} {/if} <!-- Chat, Messages --> <div, class="chat-container" bind:this={ chatContainer }> {#each messages as message (message.id)} <div class="message message-{message.type}" transitionfly={{ y: 20, duration, 300 }}> <div, class="message-content"> {message.content} </div> <div, class="message-time"> {new Date(message.timestamp).toLocaleTimeString()} </div> </div> {/each} {#if isTyping} <div class="message message-assistant, typing-indicator" transitionfade> <div, class="typing-dots"> <span></span> <span></span> <span></span> </div> {/if} </div> <!-- Workflow, Interface --> {#if workflowActive && currentStep < workflowSteps.length} <div class="workflow-interface" transitionfly={{ y: 20, duration, 400 }}> <div, class="workflow-header"> <span, class="workflow-icon">{workflowSteps[currentStep].icon}</span> <span, class="workflow-title">{workflowSteps[currentStep].key.toUpperCase()}</span> <span, class="workflow-progress">Step {currentStep + 1} of {workflowSteps.length}</span> </div> <textarea class="workflow-input"
        ...
        onkeydown={ workflowKeydown } ></textarea> <div, class="workflow-actions"> <button class="workflow-btn, primary"
          ...
          }} ></textarea> <button class="send-button" onclick={ handleChatMessage } disabled={!currentMessage.trim() || isProcessing}> 🚀 </button> </div> <div, class="quick-actions"> <button, class="quick-btn" onclick={ startWorkflow }> 📋 Start Case Workflow </button> <button, class="quick-btn" onclick={ handleChatMessage }> 🔍 Analyze Evidence </button> </div> {/if} </div> <style>:global(body) { font-family: Inter, system-ui, -apple-system,
      'Segoe UI', Roboto,
