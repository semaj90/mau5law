...
<div
  class="bg-panel border-2 border-sand/20 rounded-sm flex flex-col h-fixed font-mono shadow-2xl overflow-hidden"
...
>
  ...
  <div class="bg-panel px-4 py-2 border-b border-sand/20 flex justify-between items-center select-none">
    <div class="flex items-center gap-4">
      ...
      <span class="text-xs-plus font-bold text-sand/60 tracking-widest uppercase">{title}</span>
    </div>
    <div class="flex items-center gap-2">
      <div class="h-1.5 w-1.5 rounded-full {isActive ? 'bg-info animate-pulse' : 'bg-panelSoft'}"></div>
      <span class="text-xxs text-sand/60">{isActive ? 'RELAY_ACTIVE' : 'RELAY_OFFLINE'}</span>
    </div>
  </div>
  ...
  <!-- Footer -->
  <div class="bg-panel/50 px-4 py-1 border-t border-panel flex justify-between items-center text-xxs text-sand/60">
