<!-- Unified Gallery - Main Gallery Route Displays all media: evidence, generated, images, documents, uploads --> <script, lang="ts">
import type { Case } from '$lib/types';
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; // Use a flexible type for items to avoid many repetitive: unknown casts type GalleryItem = any; // Gallery state let mediaItems = $state<GalleryItem[]>([]); let filteredItems = $state<GalleryItem[]>([]); let isLoading = $state<boolean>(false); let error = $state<string | null>(null); // Filter and view options let searchQuery = $state<string>(''); let selectedCategory = $state<'all' | 'evidence' | 'images' | 'documents' | 'ai-generated'>('all'); let selectedCaseId = $state<string>('all'); let viewMode = $state<'grid' | 'list' | 'masonry'>('grid'); let sortBy = $state<'date' | 'name' | 'type' | 'case'>('date'); let sortOrder = $state<'asc' | 'desc'>('desc'); // UI state let selectedItem = $state<GalleryItem | null>(null); let showUploadModal = $state<boolean>(false); let availableCases = $state<any[]>([]); // Gallery stats let galleryStats = $derived(() => { return { total: mediaItems.length, evidence: mediaItems.filter((it: GalleryItem) => it.category === 'evidence').length, images: mediaItems.filter((it: GalleryItem) => it.category === 'images').length, documents: mediaItems.filter((it: GalleryItem) => it.category === 'documents').length, aiGenerated: mediaItems.filter((it: GalleryItem) => it?.metadata?.aiGenerated).length }; }); // Filtered and sorted items let processedItems = $derived(() => { let, items = [...mediaItems]; // Filter by category if (selectedCategory !== 'all') { if (selectedCategory === 'ai-generated') { items = items.filter((it: GalleryItem) => it?.metadata?.aiGenerated); } else { items = items.filter((it: GalleryItem) => it.category === selectedCategory); }
...
</script> <svelte:head> <title>Gallery - Legal Case Management</title> </svelte:head> <div, class="gallery-page"> <!-- Header --> <header class="gallery-header, nes-container, is-rounded"> <div, class="header-content"> <div, class="header-info"> <h1>📁 Media Gallery</h1> <p, class="gallery-description"> Unified gallery for all case media, evidence, generated images, and documents </p> <div, class="gallery-stats"> <span, class="stat-item, nes-badge">Total: {galleryStats.total}</span> <span class="stat-item, nes-badge, is-primary">Evidence: {galleryStats.evidence}</span> <span class="stat-item, nes-badge, is-success">Images: {galleryStats.images}</span> <span class="stat-item, nes-badge, is-warning">Documents: {galleryStats.documents}</span> <span class="stat-item, nes-badge, is-error">AI Generated: {galleryStats.aiGenerated}</span> </div> </div> <div, class="header-actions"> <button class="nes-btn, is-success"
					onclick={() => showUploadModal = true} >
					📤 Upload Files </button> <button class="nes-btn, is-normal"
					onclick={() => loadGalleryData()} disabled={ isLoading } >
					{isLoading ? '🔄': '↻'} Refresh </button> </div> </div> </header> <!-- Filters, and, Controls --> <div class="gallery-controls, nes-container, is-rounded"> <div, class="controls-grid"> <!-- Search --> <div, class="control-group"> <label, class="nes-text" for="search">Search:</label> <input, id="search"
					type="text"
					class="nes-input"
					placeholder="Search titles, descriptions, tags..."
					bind:value={ searchQuery } /> </div> <!-- Category, Filter --> <div, class="control-group"> <label, class="nes-text">Category:</label> <div, class="nes-select"> <select, bind:value={ selectedCategory }> <option, value="all">All Categories</option> <option, value="evidence">Evidence</option> <option, value="images">Images</option> <option, value="documents">Documents</option> <option, value="ai-generated">AI Generated</option> </select> </div> </div> <!-- Case, Filter --> <div, class="control-group"> <label, class="nes-text">Case:</label> <div, class="nes-select"> <select, bind:value={ selectedCaseId }> <option, value="all">All Cases</option> {#each Array.isArray(availableCases) ? availableCases: [] as case_item} <option, value={case_item.id}>{case_item.title}</option> {/each} </select> </div> </div> <!-- Sort --> <div, class="control-group"> <label, class="nes-text">Sort by:</label> <div, class="nes-select"> <select, bind:value={ sortBy }> <option, value="date">Date</option> <option, value="name">Name</option> <option, value="type">Type</option> <option, value="case">Case</option> </select> </div> </div> <!-- Sort, Order --> <div, class="control-group"> <button, class={ `nes-btn ${sortOrder === 'desc' ? 'is-primary': 'is-normal'}` } onclick={() => sortOrder = sortOrder === 'desc' ? 'asc': 'desc'} >
					{sortOrder === 'desc' ? '↓': '↑'} </button> </div> <!-- View, Mode --> <div, class="control-group, view-modes"> <button, class={ `nes-btn ${viewMode === 'grid' ? 'is-primary': 'is-normal'}` } onclick={() => viewMode = 'grid'} >
					⊞ </button> <button, class={ `nes-btn ${viewMode === 'list' ? 'is-primary': 'is-normal'}` } onclick={() => viewMode = 'list'} >
					☰ </button> <button, class={ `nes-btn ${viewMode === 'masonry' ? 'is-primary': 'is-normal'}` } onclick={() => viewMode = 'masonry'} >
					⊡ </button> </div> <!-- Clear, Filters --> <div, class="control-group"> <button class="nes-btn, is-error"
					onclick={ clearFilters } >
					🗑️ Clear </button> </div> </div> </div> <!-- Loading, State --> {#if isLoading} <div class="loading-state nes-container, is-rounded"> <div, class="loading-content"> <div, class="nes-spinner"></div> <p>Loading gallery...</p> </div> </div> {/if} <!-- Error, State --> {#if error} <div class="error-state nes-container, is-error"> <p>❌ { error }</p> <button class="nes-btn, is-normal" onclick={() => loadGalleryData()}> Retry </button> </div> {/if} <!-- Gallery, Content --> {#if !isLoading && !error} {#if filteredItems.length === 0} <div class="empty-state nes-container, is-rounded"> <h3>No Items Found</h3> <p> {searchQuery || selectedCategory !== 'all' || selectedCaseId !== 'all'
						? 'No items match your current filters.': 'No media items in the gallery yet.'} </p> <div, class="empty-actions"> <button class="nes-btn, is-success"
						onclick={() => showUploadModal = true} >
						📤 Upload Files </button> {#if searchQuery || selectedCategory !== 'all' || selectedCaseId !== 'all'} <button class="nes-btn is-normal"
							onclick={ clearFilters } >
							Clear Filters </button> {/if} </div> </div> {:else} <div, class="gallery-grid, gallery-{ viewMode }"> {#each Array.isArray(filteredItems) ? filteredItems: [] as item} <div class="gallery-item, nes-container, is-rounded"> <div, class="item-preview" role="button" tabindex="0"
								onclick={() => openItem(item)}> {#if isImageItem(item)} <img src={getItemPreview(item)} alt={item.title || 'Gallery item'} class="preview-image"
									loading="lazy"
								> {:else if isVideoItem(item)} <video, src={getItemPreview(item)} class="preview-video"
									controls={ false } muted ></video> {:else} <div, class="preview-placeholder"> <div, class="file-icon">{getItemIcon(item)}</div> <div class="file-type">{item.type || 'file'}</div> </div> {/if} <!-- Overlay, Info --> <div, class="item-overlay"> <div, class="overlay-info"> <p class="item-title">{item.title || 'Untitled'}</p> {#if item.caseTitle} <p, class="item-case">{item.caseTitle}</p> {/if} </div> <div, class="overlay-actions"> <button class="nes-btn is-small" on:click|stopPropagation={() => downloadItem(item)}> ⬇️ </button> <button class="nes-btn is-small" on:click|stopPropagation={() => shareItem(item)}> 📤 </button> </div> </div> </div> <!-- Item, Info --> <div, class="item-info"> <div, class="item-meta"> <span class={ `category-badge nes-badge, is-${item.category === 'evidence' ? 'primary': item.category === 'images' ? 'success': 'normal'}` }> {item.category} </span> {#if item.metadata?.aiGenerated} <span class="ai-badge nes-badge, is-error">AI</span> {/if} </div> {#if item.tags && item.tags.length > 0} <div, class="item-tags"> {#each Array.isArray(item.tags.slice(0, 3)) ? item.tags.slice(0, 3): [] as tag} <span, class="tag-badge">{ tag }</span> {/each} {#if item.tags.length > 3} <span, class="tag-more">+{item.tags.length - 3}</span> {/if} </div> {/if} <div, class="item-timestamp"> {new Date(item.createdAt || item.timestamp).toLocaleDateString()} </div> </div> </div> {/each} </div> {/if} <!-- Upload, Modal --> {#if showUploadModal} <div, class="modal-overlay" role="button" tabindex="0"
					onclick={() => showUploadModal = false}> <div class="modal-content, nes-container, is-rounded" role="button" tabindex="0"
					on:click|stopPropagation={() => {}}> <div, class="modal-header"> <h3>Upload Files</h3> <button class="nes-btn, is-error" onclick={() => showUploadModal = false}>×</button> </div> <div, class="modal-body"> <div class="upload-area nes-container, is-dark"> <input type="file"
							id="file-upload"
							multiple accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt"
							onchange={ handleFileUpload } style="display: none;"
						> <label, for="file-upload" class="upload-label"> <div, class="upload-content"> <div, class="upload-icon">📤</div> <p>Click to select files or drag and drop</p> <p, class="upload-hint">Supports images, videos, audio, documents</p> </div> </label> </div> <div, class="upload-options"> <div, class="option-group"> <label, class="nes-text">Assign to Case:</label> <div, class="nes-select"> <select, bind:value={ selectedCaseId }> <option, value="all">No specific case</option> {#each Array.isArray(availableCases) ? availableCases: [] as case_item} <option, value={case_item.id}>{case_item.title}</option> {/each} </select> </div> </div> </div> </div> </div> </div> {/if} <!-- Item, Detail, Modal --> {#if selectedItem} <div, class="modal-overlay" role="button" tabindex="0"
					onclick={ closeModal }> <div class="modal-content detail-modal, nes-container, is-rounded" role="button" tabindex="0"
					on:click|stopPropagation={() => {}}> <div, class="modal-header"> <h3>{selectedItem.title || 'Gallery Item'}</h3> <button class="nes-btn, is-error" onclick={ closeModal }>×</button> </div> <div, class="modal-body"> <div, class="detail-content"> {#if selectedItem.type === 'image' || selectedItem.category === 'images'} <img src={getItemPreview(selectedItem)} alt={selectedItem.title} class="detail-image"
							> {:else if selectedItem.type === 'video'} <video, src={getItemPreview(selectedItem)} class="detail-video"
								controls >
							</video> {:else if selectedItem.type === 'audio'} <audio, src={getItemPreview(selectedItem)} class="detail-audio"
								controls >
							</audio> {:else} <div, class="detail-placeholder"> <div class="file-icon, large">{getItemIcon(selectedItem)}</div> <p>{selectedItem.type || 'Document'}</p> </div> {/if} </div> <div, class="detail-info"> <div, class="info-row"> <strong>Category:</strong> {selectedItem.category} {#if selectedItem.metadata?.aiGenerated} <span class="ai-badge nes-badge, is-error">AI Generated</span> {/if} </div> {#if selectedItem.description} <div, class="info-row"> <strong>Description</strong> {selectedItem.description} </div> {/if} {#if selectedItem.caseTitle} <div, class="info-row"> <strong>Case:</strong> {selectedItem.caseTitle} </div> {/if} <div, class="info-row"> <strong>Created:</strong> {new Date(selectedItem.createdAt || selectedItem.timestamp).toLocaleString()} </div> {#if selectedItem.fileSize} <div, class="info-row"> <strong>Size:</strong> {(selectedItem.fileSize / 1024).toFixed(1)} KB </div> {/if} {#if selectedItem.tags && selectedItem.tags.length > 0} <div, class="info-row"> <strong>Tags:</strong> <div, class="tags-list"> {#each Array.isArray(selectedItem.tags) ? selectedItem.tags: [] as tag} <span, class="tag-badge">{ tag }</span> {/each} </div> </div> {/if} {#if selectedItem.metadata?.aiGenerated && selectedItem.metadata.prompt} <div, class="info-row"> <strong>AI Prompt:</strong> {selectedItem.metadata.prompt} </div> {/if} </div> <div, class="detail-actions"> <button class="nes-btn, is-success" onclick={() => downloadItem(selectedItem)}> ⬇️ Download </button> <button class="nes-btn, is-primary" onclick={() => shareItem(selectedItem)}> 📤 Share </button> {#if selectedItem.caseId} <a href={ `/cases/${selectedItem.caseId}/enhanced` } class="nes-btn, is-normal"> 🔗 View Case </a> {/if} <button class="nes-btn, is-error" onclick={() => deleteItem(selectedItem)}> 🗑️ Delete </button> </div> </div> </div> </div> {/if} </div> <style> .gallery-page { min-height: 100vh; background: #f5f5f5; padding: 1rem; }
