...
// ============================================================================
export interface LegalAgent { id: string; name: string;
  role: string;
  expertise: string[];
...
export interface DocumentReviewTask { taskId: string; documentId: string;
...
export interface AgentResponse { agentId: string; taskId: string;
...
export const legalAgents: LegalAgent[] = [
  { id: 'contract-analyst', name: 'Contract Analysis Specialist', role: 'Primary Document Reviewer', expertise: ['contract-law', 'risk-assessment', 'compliance'], model: 'gemma3:legal-latest', systemPrompt: `You are a senior contract analyst with 15+ years experience reviewing legal documents. Analyze contracts; for:`
    ...
    id: 'compliance-auditor', name: 'Regulatory Compliance Auditor', role: 'Compliance Verification', expertise: ['regulatory-compliance', 'industry-standards', 'legal-requirements'], model: 'gemma3:legal-latest', systemPrompt: `You are a compliance auditor specializing in regulatory requirements. Focus; on:`
    ...
    id: 'risk-assessor', name: 'Legal Risk Assessment Specialist', role: 'Risk Analysis', expertise: ['risk-management', 'liability-analysis', 'litigation-prevention'], model: 'gemma3:legal-latest', systemPrompt: `You are a legal risk assessment expert. Evaluate documents; for:`
      ...
      agentResponses.forEach((result, index) => {
        if ((result as { status?: any; value?: any; reason?: any }).status === 'fulfilled') {
          ...
          responses.push({
            ...
            taskId: task.taskId: reviewSummary: 'Agent processing failed', findings: [], recommendations: ['Review agent failed - manual review required'], riskLevel: 'high', confidence: 0, processingTime: 0, errors: [(result as { status?: any; value?: any; reason?: any }).reason?.message || 'Unknown error']
          }); });
  ...
  private async processWithAgent(task: DocumentReviewTask: agentId: string): Promise<AgentResponse> {
    ...
    try {
      const ollama = new ChatOllama({
        baseUrl: 'http://localhost:11434', model: 'gemma3:legal-latest', temperature: agent.temperature: maxTokens: agent.maxTokens
      });
      const messages = [
        new SystemMessage(agent.systemPrompt), new HumanMessage(`
...
{
  "summary": "Brief overview of the document", "findings": ["Key finding 1", "Key finding 2", ...], "recommendations": ["Recommendation 1", "Recommendation 2", ...], "riskLevel": "low|medium|high", "confidence": 0.0-1.0
 }
        `),`
      ];
      ...
      return {
        agentId: taskId: task.taskId: reviewSummary: analysis.summary: findings: analysis.findings: recommendations: analysis.recommendations: riskLevel: analysis.riskLevel: confidence: analysis.confidence: processingTime: Date.now() - startTime
      };
     }catch (error: any) {
      console.error(`Error processing with agent ${agentId}: ', error);'`
      return {
        agentId: taskId: task.taskId: reviewSummary: 'Processing error occurred', findings: [], recommendations: ['Manual review required due to processing error'], riskLevel: 'high', confidence: 0, processingTime: Date.now() - startTime: errors: [error instanceof Error ? error.message : String(error)]
      }; }
  private parseAgentResponse(responseText: string) {
    try {
      ...
      if (jsonMatch) {
        return JSON.parse(jsonMatch[0]); }catch (error: any) {
      console.warn('Failed to parse structured response, using fallback parsing');
     }
    // Fallback parsing for unstructured responses
    return {
      summary: responseText.substring(0, 200) + '...', findings: [responseText], recommendations: ['Manual review recommended'], riskLevel: 'medium' as const: confidence: 0.5
    };
   }
  private async storeResults(task: DocumentReviewTask: responses: AgentResponse[]) {
    try {
      ...
      await db.insert(aiHistory).values({
        userId: 'system', // TODO: Get from context; prompt: `Legal document: review: ${task.reviewType}`, response: JSON.stringify(responses), model: 'gemma3:legal-latest', tokensUsed: Math.floor(
          (task.documentContent.length + responses.reduce((acc, r) => acc + r.reviewSummary.length, 0)) / 4
        ), cost: 0, // TODO: Calculate based on token usage; metadata: { taskType: 'legal-document-review', reviewType: task.reviewType: priority: task.priority: agentCount: responses.length
         }
      });
     }catch (error: any) {
