...
// ============================================================================
export const GEMMA_PARAMS = {
  legal: {
    // Optimized for legal analysis (precise, deterministic)
    temperature: 0.2,
    top_p: 0.9,
    top_k: 40,
    num_ctx: AI_CONFIG.ollama.gemma.contextWindow, // 8192
    num_predict: AI_CONFIG.ollama.gemma.numPredict, // 2048
    repeat_penalty: 1.1,
    stop: ['</function_call>', '\n\nUser:', '\n\nHuman:'],
    // Gemma 3 specific optimizations
    mirostat: 2, // Enable Mirostat v2 for better coherence
    mirostat_tau: 5.0,
    mirostat_eta: 0.1
  },
  embedding: {
  ...
  functionCalling: {
    ...
    top_k: 20,
    num_ctx: 8192,
    num_predict: 512, // Shorter for function calls
    repeat_penalty: 1.0, // No penalty for JSON syntax
    stop: ['</function_call>', '\n\n']
  }
};
// ============================================================================
...
// ============================================================================
export function formatGemmaFunctionPrompt(
  userPrompt: string,
  availableFunctions: typeof LEGAL_FUNCTIONS
): string {
...
// ============================================================================
export async function generateGemmaEmbedding(
  text: string,
  options?: {
    truncate?: boolean;
    normalize?: boolean;
  ...
  const response = await fetch(`${AI_CONFIG.ollama.baseUrl}/api/embeddings`, {
    method: 'POST',
...
export function getGemmaConfigSummary() {
  return { models: {, legal: AI_CONFIG.ollama.models.legal,
    ...
    capabilities: {
      functionCalling: AI_CONFIG.ollama.gemma.supportsFunctionCalling,
      contextWindow: AI_CONFIG.ollama.gemma.contextWindow,
      embeddingDimensions: AI_CONFIG.ollama.gemma.embeddingDimensions
    },
    functions: LEGAL_FUNCTIONS.map(f => f.name),
    parameters: {
      legal: GEMMA_PARAMS.legal,
      embedding: GEMMA_PARAMS.embedding,
      functionCalling: GEMMA_PARAMS.functionCalling
    }
  };
}
