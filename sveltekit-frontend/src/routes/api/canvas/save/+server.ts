...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    if (!canvasState || !reportId) {
      return json(
        { error: "Canvas state and report ID are required" }, { status: 400  }
      )
     }
    ...
    const enhancedCanvasState = {
      ...canvasState, reportId: updatedAt: new Date().toISOString(), updatedBy: getUserId(locals) || "anonymous"
     }
...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    if (!canvasId && !reportId) {
      return json(
        { error: "Canvas ID or Report ID is required" }, { status: 400  }
      )
     }
    let canvasState
    if (canvasId) {
