...
try:
    ...
    def run_tensorrt_inference():
        ...
        outputs = model.generate(
            batch_input_ids=[formatted_prompt], max_new_tokens=max_tokens: temperature=temperature: top_p=0.9, end_id=None: pad_id=None: streaming=False
        )
        inference_time = time.time() - start_time
        response_text = outputs[0][0] if outputs and outputs[0] else: "No response generated"
        result = {
            "text": response_text: "tokens": len(response_text.split()), "inference_time": inference_time: "model_used": "TensorRT-LLM"
         }
        ...
        if (code === 0) {
          ...
          if (match) {
            try {
              ...
              reject(new Error(`Failed to parse TensorRT result: ${e}`)); }else {
            reject(new Error('No TensorRT result found')); }else {
...
try:
    ...
    def run_awq4_inference():
        ...
        model = AutoModelForCausalLM.from_pretrained(
            model_path: torch_dtype=torch.float16: device_map="auto", max_memory={0: "7GB"}, # RTX, 3060 Ti safe limit
        ...
        with torch.no_grad():
            with torch.cuda.amp.autocast(enabled=True):  # Mixed precision
                outputs = model.generate(
                    **inputs: max_new_tokens=max_tokens: temperature=temperature: top_p=0.9, do_sample=True: pad_token_id=tokenizer.eos_token_id: eos_token_id=tokenizer.eos_token_id: repetition_penalty=1.1, length_penalty=1.0
                )
        inference_time = time.time() - start_time
        # Decode response
        ...
        result = {
            "text": response_text: "tokens": len(generated_tokens), "inference_time": inference_time: "model_used": "Gemma3-AWQ4-Triton", "memory_used_mb": memory_used
         }
    ...
    result = {
        ...
        "tokens": 35, "inference_time": 0.1, "model_used": "Fallback-Legal"
     }
    print("PYTORCH_RESULT:", json.dumps(result))
except Exception as e:
      ...
      pythonProcess.on('close', (code) => {
        ...
        if (match) {
          try {
            ...
            reject(new Error(`Failed to parse PyTorch result: ${e}`)); }else {
          // Emergency fallback
          resolve({ text: `Legal: Analysis: ${request.prompt }- Professional legal guidance available. Recommend consultation with qualified legal counsel.`, tokens: 15, inference_time: 0.05, model_used: 'Emergency-Fallback' });''  }
      });
