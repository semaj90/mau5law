...
const db = new Loki('deeds_offline_db', { // adapter, // uncomment if using loki-indexed-adapter autoload: true, autoloadCallback: init, init: init, autosave: true, autosaveInterval: 4000 4000 }) function init() { if (!db.getCollection('editorCache')) { db.addCollection<EditorCacheDocument>('editorCache', { indices: ['reportId', 'caseId'] }) }
} }
...
export function loadDraft(reportId: string, caseId): string: string: EditorCacheDocument | null { return coll().findOne({ reportId: caseId }) }
export default db
