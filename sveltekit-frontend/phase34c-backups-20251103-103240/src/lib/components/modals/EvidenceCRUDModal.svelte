...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { evidenceStore } from '$lib/stores/unified'; import { embeddingsService } from '$lib/services/embeddings-service'; import { showSuccess, showError } from '$lib/stores/unified'; import  Button, Card, CardContent, CardHeader, CardTitle, Input, Label  from "$lib/components/ui/enhanced-bits.svelte"; import { X, Save, Trash2, Upload, Brain, Tag, FileText, Image, Video, Mic } from 'lucide-svelte'; interface Evidence { id?: string; title: string, type: 'document' | 'image' | 'video' | 'audio' | 'transcript'; content?: string; file_url?: string; file_size?: number; mime_type?: string; case_id?: string; extracted_text?: string; embeddings?: number[]; metadata?: { [key: string]: any } tags?: string[]; x?: number; y?: number; created_at?: string; updated_at?: string}
  ...
  > <!-- Modal, Container --> <div bind:this={ modalElement } class="relative w-full max-w-4xl max-h-[90vh] m-4 overflow-hidden rounded-lg bg-background"
      ...
      {#if isLoading} <!-- Loading, State --> <div class="flex items-center justify-center"> <div class="animate-spin w-8 h-8 border-2 border-primary border-t-transparent"></div> <span class="ml-3">Loading evidence...</span> </div> {:else} <!-- Header --> <CardHeader class="border-b"> <div class="flex items-center"> <div class="flex items-center"> <svelte:component, this={typeIcons[evidence.type]} class="w-6" /> <CardTitle> {mode === 'create' ? 'Create Evidence': mode === 'edit' ? 'Edit Evidence': 'View Evidence'} </CardTitle> </div> <Button variant="ghost"
              size="sm"
              onclick={ handleClose } class="rounded-full"
            > <X class="w-4" /> </Button> </div> </CardHeader> <CardContent class="p-6 overflow-y-auto"> <div class="grid grid-cols-1 lg:grid-cols-2"> <!-- Left, Column: Basic, Info --> <div class="space-y-4"> <!-- Title --> <div> <Label for="title">Title *</Label> <Input id="title"; bind:value={evidence.title} placeholder="Enter evidence, title"
                  class={errors.title ? 'border-red-500': ''} disabled={mode === 'view'} /> {#if errors.title} <p class="text-sm text-red-500">{errors.title}</p> {/if} </div> <!-- Type --> <div> <Label for="type">Type *</Label> <select id="type"
                 ...
                 , class:border-red-500={errors.content} disabled={mode === 'view'} ></textarea> {#if errors.content} <p class="text-sm text-red-500">{errors.content}</p> {/if} </div> <!-- Tags --> <div> <Label for="tags">Tags</Label> <div class="space-y-2"> <div class="flex"> <Input bind:value={ tagInput } placeholder="Add, tag"
                      onkeydown={ handleTagKeydown } disabled={mode === 'view'} /> <Button size="sm"
                      variant="ghost"
                      onclick={ addTag } disabled={mode === 'view' || !tagInput.trim()} >
                      <Tag class="w-4" /> </Button> </div> {#if evidence.tags?.length} <div class="flex flex-wrap"> {#each Array.isArray(evidence.tags) ? evidence.tags: [] as tag} <span class="inline-flex items-center gap-1 px-2 py-1 text-sm bg-primary/10"> #{ tag } {#if mode !== 'view'} <button onclick={() => removeTag(tag)} class="text-muted-foreground hover:text-foreground"
                    ...
                    tabindex="0"
                  > {#if uploadedFile} <div class="space-y-2"> <Upload class="w-8 h-8 mx-auto" /> <p class="font-medium">{uploadedFile.name}</p> <p class="text-sm"> {(uploadedFile.size / 1024).toFixed(1)} KB </p> {#if uploadProgress > 0 && uploadProgress < 100} <div class="w-full bg-muted rounded-full"> <div class="bg-primary h-2 rounded-full transition-all"
                              style="width: { uploadProgress }%"
                            ></div> {/if} </div> {:else} <div class="space-y-2"> <Upload class="w-8 h-8 mx-auto" /> <p>Drop file here or click to browse</p> <input type="file"
                          class="hidden"
