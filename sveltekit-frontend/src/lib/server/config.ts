...
export const MCP_CONFIG = { context7: { enabled: true,
	...
	multicore: { enabled: true,
		...
		workers: parseInt(process.env.MCP_WORKERS || '4'),
...
export const AI_CONFIG = {
	// Primary: Ollama with Gemma models; ollama: { baseUrl: process.env.OLLAMA_URL || process.env.OLLAMA_BASE_URL || 'http://localhost:11434',
		models: {
	...
	tensorrt: { enabled: process.env.TENSORRT_ENABLED === 'true',
		...
		modelName: 'gemma_legal_tensorrt',
		modelVersion: '1',
		timeout: 30000,
		batchSize: parseInt(process.env.TENSORRT_BATCH_SIZE || '8'),
		maxTokens: 2048,
...
export const VECTOR_SEARCH_CONFIG = {
	...
	qdrant: { enabled: process.env.QDRANT_ENABLED === 'true',
		...
		vectorConfig: { distance: 'Cosine' as const,
			onDisk: false // Keep vectors in memory for speed
		},
	...
	hybrid: {
		...
		minSimilarity: 0.5,
		// Fall back to pgvector-only if Qdrant is unavailable
...
// ============================================================================
export const DATABASE_CONFIG = { url: process.env.DATABASE_URL || 'postgresql://legal_admin:123456@localhost:5432/legal_ai_db',
	postgres: { host: process.env.DATABASE_HOST || 'localhost',
		...
		idleTimeout: parseInt(process.env.DATABASE_IDLE_TIMEOUT || '20'),
...
export const RAG_CONFIG = {
	// Document chunking
	chunking: { chunkSize: 1500,
		chunkOverlap: 300,
		strategy: 'legal-aware' as const // 'legal-aware' | 'semantic' | 'fixed'
	},

	// Search and retrieval
	search: { maxSources: 10,
...
export function getConfigSummary() {
	return { aiProviders: { ollama: AI_CONFIG.ollama.baseUrl,
		...
		vectorSearch: { pgvector: VECTOR_SEARCH_CONFIG.pgvector.enabled,
			qdrant: VECTOR_SEARCH_CONFIG.qdrant.enabled,
			hybrid: VECTOR_SEARCH_CONFIG.hybrid
		},
		gemmaModels: { legal: AI_CONFIG.ollama.models.legal,
			embedding: AI_CONFIG.ollama.models.embedding,
			dimensions: AI_CONFIG.ollama.gemma.embeddingDimensions
		},
		healthMonitoring: { enabled: HEALTH_CONFIG.circuitBreaker.failureThreshold > 0,
