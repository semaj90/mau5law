...
export class AccessibilityValidator {
        ...
        // Legal AI Specific Rules
        {
            ...
            validate: (element) => {
                ...
                const hasLegalClasses = classList.some(
                    cls => cls.startsWith('legal-') || cls.includes('legal-ai') || cls.includes('bg-legal-') || cls.includes('text-legal-')
                );
    ...
    validateElement(element: Element): AccessibilityReport[] {
        return this.rules.map(rule => ({
            ...
            message: rule.validate(element) ? `✅ ${rule.name} : Passed` : `❌ ${rule.name}: ${rule.description}${rule.fix ? ` - ${rule.fix}` : ''}`
        }));
    }
