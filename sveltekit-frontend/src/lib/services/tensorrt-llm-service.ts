...
class TensorRTLLMService {
  ...
  private async generateTensorRT(request: InferenceRequest): Promise<InferenceResponse> {
    const payload = {
      ...
      gpu_config: { batch_size: Math.min(1, RTX_3060_CONFIG.max_batch_size),
  ...
  private async generateOllama(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    const payload = {
      ...
      options: { num_predict: request.max_tokens || 512,
        temperature: request.temperature || 0.1,
  ...
  async warmupModels(): Promise<void> {
    ...
    for (const model of models) {
      try {
        const warmupRequest: InferenceRequest = { prompt: 'This is a warmup request.',
          max_tokens: 10,
          temperature: 0.1,
          model: model.name
        };
