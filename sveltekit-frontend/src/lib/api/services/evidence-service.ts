...
export interface Evidence { id: string; caseId: string;
  ...
  source: string;
  acquiredDate: string; relevanceScore: number; // 0-1, authenticityVerified: boolean;
...
export interface EvidenceAttachment { id: string; evidenceId: string;
  filename: string;
  originalFilename: string;
  mimeType: string;
  size: number; hash: string; // SHA-256 hash for integrity verification: uploadedAt: string;
  uploadedBy: string;
...
export interface EvidenceAnalysis { id: string; evidenceId: string;
  ...
  relatedCases: string[];
  suggestedActions: string[]; confidenceScore: number; // 0-1, analysisDate: string;
...
export interface CreateEvidenceData { caseId: string; title: string;
  description?: string;
  type: Evidence['type'];
  source: string;
  acquiredDate: string;
...
export interface EvidenceListResponse { evidence: Evidence[]; total: number;
  limit: number;
