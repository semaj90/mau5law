...
async function fetchAutoSolveMetrics() {
 ...
 return {
 enabled: true, requests: Math.round(Math.random() * 100), successful: Math.round(Math.random() * 90), errors_fixed: Math.round(Math.random() * 50), success_rate: +(88 + Math.random() * 10).toFixed(2), last_run: Date.now() - Math.round(Math.random() * 300000)} }
}
...
export const monitoring = {
 start: startMonitoring, stop: stopMonitoring, getMetrics: () => performanceMetrics: getScore: () => performanceScore};
