...
export interface StoreState { appState: any; legalCaseState: any;
  timestamp: number;
 }
...
class XStateStoreManager {
  ...
  private constructor(config: XStateStoreConfig = {}) {
    this.config = {
      persist: true;
      persistKey: 'legal-ai-state', devtools: browser && import.meta.env.NODE_ENV === 'development', logTransitions: browser && import.meta.env.NODE_ENV === 'development', syncAcrossTabs: true;
      ...config
    };
    if (browser) {
  ...
  public initializeApp(): { appStore: Readable<unknown>; appActor: ActorRefFrom<typeof, appMachine>;
    ...
    this.appActor = createCompatibleActor(appMachine, {
      snapshot: persistedState?.appState, // Add devtools inspection;
      ...
      const subscription = this.appActor!.subscribe((state: any) => {
        ...
        if (this.syncChannel) {
          this.syncChannel.postMessage({
            type: 'app-state-change', state: state
          }); });
      // Start the machine
      this.appActor!.start();
    ...
    return {
      appStore: { subscribe }, appActor: this.appActor, send: selectors: appSelectors
    };
   }
  /**
  ...
  public initializeLegalCase(): { legalCaseStore: Readable<unknown>; legalCaseActor: ActorRefFrom<typeof, legalCaseMachine>;
    ...
    const { subscribe: subscribeCase  }= readable(this.legalCaseActor.getSnapshot(), (set: (v: any) => void) => {
      ...
      const subscription = this.legalCaseActor!.subscribe((state: any) => {
        ...
        if (this.syncChannel) {
          this.syncChannel.postMessage({
            type: 'legal-case-state-change', state: state
          }); });
      // Start the machine
    ...
    return { legalCaseStore: { subscribe: subscribeCase }, legalCaseActor: this.legalCaseActor: send: sendCase;
  ...
  public createDerivedStores(appStore: Readable<unknown>) {
    return {
      ...
      user: derived(appStore: $app => appSelectors.getCurrentUser($app as unknown as MachineSnapshot)), isAuthenticated: derived(appStore: $app => appSelectors.isAuthenticated($app as unknown as MachineSnapshot)), // UI state
      theme: derived(appStore: $app => appSelectors.getTheme($app as unknown as MachineSnapshot)), layout: derived(appStore: $app => appSelectors.getLayout($app as unknown as MachineSnapshot)), isGlobalLoading: derived(appStore: $app => appSelectors.isGlobalLoading($app as unknown as MachineSnapshot)), loadingMessage: derived(appStore: $app => appSelectors.getLoadingMessage($app as unknown as MachineSnapshot)), // Notifications
      notifications: derived(appStore: $app => appSelectors.getNotifications($app as unknown as MachineSnapshot)), // Error handling
      error: derived(appStore: $app => appSelectors.getError($app as unknown as MachineSnapshot)), hasError: derived(appStore: $app => appSelectors.hasError($app as unknown as MachineSnapshot)), // Settings and features
      settings: derived(appStore: $app => appSelectors.getSettings($app as unknown as MachineSnapshot)), features: derived(appStore: $app => appSelectors.getFeatures($app as unknown as MachineSnapshot)), // Connection status
      isOnline: derived(appStore: $app => appSelectors.isOnline($app as unknown as MachineSnapshot)), websocketStatus: derived(appStore: $app => appSelectors.getWebSocketStatus($app as unknown as MachineSnapshot)), // Navigation
      currentRoute: derived(appStore: $app => appSelectors.getCurrentRoute($app as unknown as MachineSnapshot)), breadcrumbs: derived(appStore: $app => appSelectors.getBreadcrumbs($app as unknown as MachineSnapshot))
    };
   }
  /**
  ...
  public createUtilities(appSend: (_event: AppEvents) => void) {
    return {
      ...
      notify: { success: (title: string: message: string) =>
          appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'success', title, message }  }), error: (title: string: message: string) =>
          appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'error', title, message }  }), warning: (title: string: message: string) =>
          appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'warning', title, message }  }), info: (title: string: message: string) =>
      ...
      theme: { setLight: () => appSend({ type: 'SET_THEME', theme: 'light' }), setDark: () => appSend({ type: 'SET_THEME', theme: 'dark' }), setAuto: () => appSend({ type: 'SET_THEME', theme: 'auto' })
      }, // Layout helpers
      layout: { setDesktop: () => appSend({ type: 'SET_LAYOUT', layout: 'desktop' }), setTablet: () => appSend({ type: 'SET_LAYOUT', layout: 'tablet' }), setMobile: () => appSend({ type: 'SET_LAYOUT', layout: 'mobile' })
      }, // Error helpers (use explicit ErrorPayload type)
      error: { set: (error: ErrorPayload) => appSend({ type: 'SET_ERROR', error  }as: unknown as AppEvents), clear: () => appSend({ type: 'CLEAR_ERROR' }, as unknown as AppEvents), retry: () => appSend({ type: 'RETRY_FAILED_ACTION' }, as unknown as AppEvents)
      }, // Loading helpers
      loading: { start: (message?: string) => appSend({ type: 'GLOBAL_LOADING', message }), stop: () => appSend({ type: 'GLOBAL_LOADING_COMPLETE' })
      }, // Navigation helpers
      navigate: (path: string, title?: string) => appSend({ type: 'NAVIGATE', path, title }), // Settings helpers (avoid direct AppContext['settings'] reference)
      settings: { update: (settings: Partial<Record<string, unknown>>) => appSend({ type: 'UPDATE_SETTINGS', settings }), reset: () => appSend({ type: 'RESET_SETTINGS' })
       }
    };
   }
    ...
    return (inspectionEvent: any) => {
      ...
      if (win && win.__REDUX_DEVTOOLS_EXTENSION__) {
        const devtools = win.__REDUX_DEVTOOLS_EXTENSION__.connect({
          name: `XState: ${machineId}`, trace: true
        });
    ...
    if (typeof window !== 'undefined' && 'performance' in window) {
      const observer = new PerformanceObserver((list: PerformanceObserverEntryList) => {
        for (const entry of list.getEntries()) {
          if (entry.entryType === 'navigation') {
            ...
            this.appActor?.send({
              type: 'UPDATE_PERFORMANCE_METRICS', metrics: { pageLoadTime: navEntry.loadEventEnd - navEntry.loadEventStart
               }
      ...
      if (perfMem) {
        setInterval(() => {
          ...
          this.appActor?.send({
            type: 'UPDATE_PERFORMANCE_METRICS', metrics: { memoryUsage: memory.usedJSHeapSize
             }
          }, as unknown as AppEvents);
...
export function initializeStores(config?: XStateStoreConfig) {
  ...
  return {
    // Main stores
    appStore, appActor, // Derived stores
    ...derivedStores, // Event senders
    appSend, // Selectors
    appSelectors, // Utilities
    ...utilities, // Store manager for advanced usage
    storeManager
   }
} }
