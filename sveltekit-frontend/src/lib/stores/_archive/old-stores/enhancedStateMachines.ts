...
export interface ProcessingResult { id: string;, evidenceId: string;
...
export interface VectorMatch { id: string;, evidenceId: string;
  similarity: number;
  content: string;
...
export interface GraphConnection { to: string;, type: string;
  strength: number;
  bidirectional: boolean;
  metadata?: { [key: string]: any };
} }
export interface StreamingUpdate { id: string;, type: 'evidence_added' | 'analysis_complete' | 'relationship_found';
  data: any;
  timestamp: Date;
  source: string;
} }
...
// ======================================================================
export const evidenceProcessingMachine = setup({ types: { context: {} }as EnhancedAIContext,
      ...
      try {
        ...
        const [embeddingResponse, taggingResponse, analysisResponse] = await Promise.allSettled([
          ...
          fetch('/api/ai/embedding', {
            ...
            body: JSON.stringify({ content: input.evidence.description || input.evidence.title,
    ...
    searchSimilarEvidence: fromPromise(async ({ input }: { input: { embeddings: number[]; limit?: number } }}) => {
      try {
        const response = await fetch('/api/vector/search', {
          method: 'POST',
          headers: { 'Content-Type': `application/json` },
          body: JSON.stringify({ vector: input.embeddings,
  ...
  guards: { hasQueuedEvidence: ({ context }) => context.evidenceQueue.length > 0,
  ...
  states: { idle: { on: { ADD_EVIDENCE: { target: 'queueing',
    ...
    processing: { initial: 'aiProcessing',
      states: { aiProcessing: { invoke: { src: 'processEvidenceAI',
        ...
        vectorSearch: { invoke: { src: 'searchSimilarEvidence',
            input: ({ context }) => {
            ...
            onDone: { target: 'relationshipDiscovery',
              actions: assign({ vectorMatches: ({ event }) =>
            ...
            onError: { target: 'relationshipDiscovery',
              actions: assign({ errors: ({ context, event }) => [
                  ...context.errors,
                  {
                    id: crypto.randomUUID(),
        ...
        relationshipDiscovery: { invoke: { src: 'discoverRelationships',
            input: ({ context }) => ({
              evidenceId: context.evidenceQueue[0].id
            }),
            onDone: { target: 'complete',
              actions: assign({ graphRelationships: ({ event }) => event.output.nodes || [],
        ...
        complete: { always: [
            { target: '#evidenceProcessing.queueing',
              actions: assign({ evidenceQueue: ({ context }) => context.evidenceQueue.slice(1),
                retryAttempts: 0
              })
            },
          ]
        },
        error: { on: { RETRY_FAILED: { target: 'aiProcessing',
              guard: 'canRetry',
              actions: assign({ retryAttempts: ({ context }) => context.retryAttempts + 1
              })
            },
            PROCESS_NEXT: { target: 'complete',
              actions: assign({ evidenceQueue: ({ context }) => context.evidenceQueue.slice(1),
                retryAttempts: 0
              })
            } }
    ...
    monitoring: { invoke: { src: 'systemHealthCheck',
        onDone: { target: 'idle',
          actions: assign({ systemHealth: ({ event }) => event.output.health as: 'healthy' | 'degraded' | 'critical' })'` },'`
        onError: { target: 'idle',
          actions: assign({ systemHealth: 'critical',
    ...
    syncing: { invoke: { src: 'syncCache',
        onDone: { target: 'idle',
          actions: assign({ lastSync: new Date(),
  ...
  on: { CLEAR_ERRORS: { actions: assign({ errors: ({ context }) => context.errors.map(error => ({ ...error, resolved: true }))
    ...
    STREAM_RESULTS: { actions: assign({ liveUpdates: ({ context, event }) => [...context.liveUpdates, ...event.updates],
