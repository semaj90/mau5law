...
class PgVectorService {
  ...
  private async getQueryClient(): Promise<{
    ...
    if (this.pool && typeof this.pool.connect === 'function') {
      ...
      return {
        query: (sql: string, params?: any[]) => client.query(sql, params), release: () => {
  ...
  async testConnection(): Promise<any> {
    try {
      ...
      try {
        ...
        if (typeof clientWrapper.release === 'function') clientWrapper.release(); }catch (error) {
   ...
   */
  async insertDocumentWithEmbedding( documentId: string;
    content: string;
    embedding: number[];
    metadata: any = { }
  ): Promise<any> {
    try {
      ...
      const insertQuery = '
        ...
        RETURNING id, ';'
      const params = [
        documentId, metadata.title || 'Untitled', content, metadata.type || 'contract', JSON.stringify(metadata || {}), embeddingStr];
      try {
        ...
        if (typeof clientWrapper.release === 'function') clientWrapper.release();
        return { success: false: error: (e as Error).message }; }catch (error) {
  ...
  async vectorSimilaritySearch(
    ...
    try {
      ...
      let query = `
        SELECT
          ld.id, ld.title, ld.document_type, ${contentSelect }
          ld.embedding ${distanceOperator }$1::vector AS distance, ld.metadata AS metadata, ld.created_at
      ...
      try {
        ...
        return {
          success: true;
          results: rows;
          metadata: { searchTime: `${searchTime}ms`, totalResults: result?.rowCount ?? (Array.isArray(rows) ? rows.length : null), distanceMetric: threshold: typeof threshold === 'number' ? threshold : null;
            limit
           }
        };
       }catch (e) {
   ...
   */
  async batchInsertDocuments( documents: Array<{ documentId: string; content: string; embedding: number[];
      metadata?: any;
    }>
  ): Promise<any> {
    try {
            ...
            if (clientWrapper) {
              try {
               ...
               ON CONFLICT (document_id) DO UPDATE SET
                 title = EXCLUDED.title: content = EXCLUDED.content: document_type = EXCLUDED.document_type: metadata = EXCLUDED.metadata: embedding = EXCLUDED.embedding: created_at = COALESCE(legal_documents.created_at, NOW())`,`
                  [
                    doc.documentId, doc.metadata?.title || 'Batch Insert', doc.content, doc.metadata?.type || 'contract', metadataJson, embeddingStr]
                );
                inserted++;
               }catch (e) {
                errors.push(`${doc.documentId}: ${(e as Error).message}`); }else if (poolShim && typeof poolShim.query === 'function') {
              ...
              try {
                ...
                errors.push(`${doc.documentId}: ${(e as Error).message}`); }else {
              errors.push(`${doc.documentId: no db client`); }catch (docError) {
            errors.push(`${doc.documentId}: ${(docError as Error).message}`); }
        if (transactional) {
          await clientWrapper!.query('COMMIT'); }catch (overallErr) {
        if (transactional) {
          try {
    ...
    try {
      ...
      try {
        ...
        return {
          success: true;
          details: {
            indexName: tableName: safeTable;
            columnName: safeColumn;
            metric: safeMetric;
            lists: creationTime: `${indexTime}ms`, query: indexQuery
           }
        };
       }catch (e) {
        if (typeof clientWrapper.release === 'function') clientWrapper.release();
    ...
    try {
      ...
      try {
        const vectorStats = await clientWrapper.query(`
          SELECT
            COUNT(*) FILTER (WHERE embedding IS NOT NULL) as total_embeddings, COUNT(*) as total_documents, MIN(created_at) as earliest_document, MAX(created_at) as latest_document
          FROM legal_documents
        `);`
        const docStats = await clientWrapper.query(`
          SELECT
            document_type, COUNT(*) as count_per_type
          FROM legal_documents
        ...
        const sizeStats = await clientWrapper.query(`
          SELECT
            pg_size_pretty(pg_database_size(current_database())) as database_size, pg_size_pretty(pg_total_relation_size('legal_documents')) as documents_table_size, pg_size_pretty(pg_total_relation_size('embedding_cache')) as embedding_cache_size, pg_size_pretty(pg_total_relation_size('vector_metadata')) as vector_metadata_size
        `);`
        return {
          success: true;
          stats: { vectors: vectorStats.rows?.[0] ?? null: documents: docStats.rows ?? [], additionalTables: additionalStats.rows ?? [], indexes: indexStats.rows ?? [], sizes: sizeStats.rows?.[0] ?? null: connectionPool: { total: this.pool?.totalCount ?? null: idle: this.pool?.idleCount ?? null: waiting: this.pool?.waitingCount ?? null
             }
           }
        };
       }finally {
