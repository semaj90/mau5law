...
function getTTL(type: CacheType): number {
  switch (type) {
    ...
    case 'chat': return CACHE_TTL_CHAT;
    case 'health': return CACHE_TTL_HEALTH;
    case 'tags': return CACHE_TTL_TAGS;
...
export async function semanticCacheSearch(embedding: number[], threshold = 0.95): Promise<SemanticCacheEntry | null> {
  try {
    if (!(await isRedisAvailable())) {
      return null;
    }
    ...
    for (const key of keys.slice(0, 10)) { // Check last 10
      const v = await r.get(key);
      if (v) {
