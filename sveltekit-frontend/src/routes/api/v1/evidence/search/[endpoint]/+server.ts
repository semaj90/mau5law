...
const SimilarSearchSchema = z.object({
  query: z.string().min(1), evidenceId: cuidSchema.optional(), limit: z.number().min(1).max(20).default(5), threshold: z.number().min(0).max(1).default(0.7)
});
const SuggestionSchema = z.object({
  query: z.string().min(1), context: z.string().optional(), type: z.enum(['search', 'legal', 'case', 'precedent']).default('legal'), limit: z.number().min(1).max(10).default(5)
});
// Types
interface SearchSuggestion { text: string; type: 'case' | 'law' | 'evidence' | 'precedent';
  confidence: number;
  source: string;
  reasoning?: string;
 }
interface SimilarEvidence { id: string; filename: string;
  similarity: number;
  summary: string;
  relevantLaws: string[];
  type: string;
  ...
  if (endpoint === 'similar') {
    try {
      ...
      const mockSimilarEvidence: SimilarEvidence[] = [
        {
  id: 'evidence-001', filename: 'financial_records_2023.pdf', // initial similarity kept for fallback, but we'll compute real similarity from embeddings below'
          similarity: 0.87, summary: 'Financial records showing suspicious transactions', relevantLaws: ['Money Laundering Prevention Act', '18 USC 1956'], type: 'document', embedding: [0.12, 0.34, -0.08, 0.45]
        }, {
          id: 'evidence-002', filename: 'witness_statement_john_doe.txt', similarity: 0.73, summary: 'Witness testimony corroborating financial irregularities', relevantLaws: ['Federal Rules of Evidence 801'], type: 'document', embedding: [0.05, 0.22, -0.01, 0.31]
        }, {
          id: 'evidence-003', filename: 'bank_correspondence.pdf', similarity: 0.68, summary: 'Bank emails discussing account activity', relevantLaws: ['Bank Secrecy Act', '31 USC 5311'], type: 'document', embedding: [0.02, 0.18, -0.03, 0.25]
        }]
        .map(item => {
      ...
      return json({
        success: true;
        data: {
          query: results: mockSimilarEvidence;
          total: mockSimilarEvidence.length, threshold: embedding: queryEmbedding, // Return for client-side caching
      ...
      if (error instanceof z.ZodError) {
        return json(
          {
            message: 'Invalid search request', details: error.errors
          }, { status: 400  }
        );
       }
      return json(
        {
          message: 'Similar evidence search failed', details: getErrorMessage(error)
        }, { status: 500  }
      ); }
  /*
...
[
  { "text": "suggested search term or phrase", "type": "case|law|evidence|precedent", "confidence": 0.85, "source": "why this suggestion is relevant", "reasoning": "brief explanation"
   }
]
Focus on legal terminology, case citations, statutory references, and evidence categories that would be most useful for legal research.`;`
      ...
      try {
        ...
        if (jsonMatch && jsonMatch[0]) {
          ...
          throw new Error('No JSON found'); }catch (parseError) {
        // Fallback suggestions if AI response parsing fails
        suggestions = [
          { text: query + ' legal precedent', type: 'precedent', confidence: 0.6, source: 'Automated suggestion', reasoning: 'Adding legal precedent context'
          }, {
            text: query + ' evidence analysis', type: 'evidence', confidence: 0.6, source: 'Automated suggestion', reasoning: `Evidence-focused search` },'`'`
          {
            text: query + ' case law', type: 'case', confidence: 0.6, source: 'Automated suggestion', reasoning: `Case law research`  }
        ];
       }
      return json({
  success: true;
        data: {
          query: suggestions: suggestions.slice(0, limit), type: generatedAt: new Date().toISOString(), model: LEGAL_MODEL
         }
      });
     }catch (error: any) {
      ...
      return json(
        {
          message: 'Suggestion generation failed', details: getErrorMessage(error)
        }, { status: 500  }
      ); }
