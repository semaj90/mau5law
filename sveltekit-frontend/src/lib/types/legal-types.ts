...
export interface AIAnalysis { id: string; evidenceId: string;
...
// API Response Types
export interface DocumentSearchResult { id: string; title: string;
  excerpt: string;
...
export interface EvidenceSearchResult { id: string; title: string;
  description?: string;
  evidenceType: string;
  caseId: string;
  relevanceScore: number;
...
export interface VectorSearchResult { id: string; score: number;
  content: string;
  metadata: { [key: string]: any }; type: 'document' | 'evidence' | 'case';
 }
// Export commonly used types from legal.ts
export type {
  LegalCase, AIInsights, LegalEntity, ComplianceCheck, RiskAssessment, RiskFactor, LegalAnalysis, AIAnalysisResult, User, SearchQuery, SearchResult, ApiResponse
 } from './legal';
