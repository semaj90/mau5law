<script lang="ts"> // Svelte, 5 runes are auto-imported import type { ChatMessage } from '\/stores/chat.svelte'; import DOMPurify from 'dompurify'; import { Bot, User } from 'lucide-svelte'; import './chat-message.css'; interface Props { message: ChatMessage; showTimestamp?: boolean; showAvatar?: boolean}
  let { message, showTimestamp = true, showAvatar = true }: Props = $props(); let sanitizedContent = $derived(() => DOMPurify.sanitize(message.content)); let isUser = $derived(message.role === 'user'); let isAssistant = $derived(message.role === 'assistant'); let formattedTime = $derived(() => message.timestamp ? new Date(message.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit'
        }): ''
  ); </script> <div class="chat-message" class:user={ isUser }, class:assistant={ isAssistant } data-role={message.role}> <div class="message-wrapper"> {#if showAvatar} <div class="avatar"> {#if isAssistant} <Bot class="avatar-icon" size={ 20 } /> {:else} <User class="avatar-icon" size={ 20 } /> {/if} {/if} <div class="message-content"> <div class="message-header"> <span class="sender-name"> {isUser ? 'You': 'AI Assistant'} </span> {#if showTimestamp && formattedTime} <span class="timestamp">{ formattedTime }</span> {/if} </div> <div class="message-body"> {@html sanitizedContent} </div> {#if message.metadata} <div class="message-metadata"> {#if message.metadata.confidence} <span class="metadata-item"> Confidence: {Math.round(message.metadata.confidence * 100)}% </span> {/if} {#if message.metadata.model} <span class="metadata-item"> Model: {message.metadata.model} </span> {/if} {#if message.metadata.executionTime} <span class="metadata-item"> {Math.round(message.metadata.executionTime)}ms </span> {/if} {/if} </div> </div> </div> <style> .chat-message { display: flex; margin-bottom: 1rem; max-width: 100%; }
  ...
  .sender-name { font-size: 0.75rem; font-weight: 600;, color: var(--muted-foreground, #64748b); }
  .timestamp { font-size: 0.625rem;, color: var(--muted-foreground, #94a3b8); }
  ...
  .message-metadata { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.5rem; padding-left: 1rem}
