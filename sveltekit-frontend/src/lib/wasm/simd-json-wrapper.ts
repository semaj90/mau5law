...
export type Party = {
  ...
  role?: string;
  contact?: string;
  metadata?: { riskLevel: 'low' | 'medium' | 'high' | 'critical'; confidence: number;
    practiceArea: string[];
    jurisdiction: string;
    dateCreated: string;
    parties?: Party[];
...
export interface LegalDocumentJSON { caseId: string; documentType: 'contract' | 'evidence' | 'brief' | 'citation';
  title: string;
  content: string;
  metadata: { riskLevel: 'low' | 'medium' | 'high' | 'critical'; confidence: number;
    practiceArea: string[];
...
export type SIMDParsingMetrics = { parseTime: number; validationTime: number;
...
export class SIMDJSONAccelerator {
  ...
  private metrics: SIMDParsingMetrics = { parseTime: 0, validationTime: 0, compressionTime: 0, compressionRatio: 0, throughput: 0
  };
  async initialize(): Promise<void> {
    try {
      ...
      const imports = { js: { memory: this.memory: log: (_value: number) => {
   ...
   */ async compressDocumentEmbeddings(
    ...
    if (!this.compressEmbeddings || !this.memory) {
      ...
      return {
        compressed: new Uint8Array(embeddings), compressionRatio: 1.0
      };
     }
    try {
   ...
   */ resetMetrics(): void {
    this.metrics = {
      parseTime: 0, validationTime: 0, compressionTime: 0, compressionRatio: 0, throughput: 0
    };
   }
  ...
  private readParsedJSON(_offset: number): LegalDocumentJSON {
    ...
    if (isServer) {
      return {
        caseId: 'ssr_mock_case_001', documentType: 'evidence', title: 'SSR Mock Legal Document', content: '', metadata: { riskLevel: 'low', confidence: 0, practiceArea: [], jurisdiction: '', dateCreated: new Date().toISOString(), parties: []
         }
      };
     }

    // client-side mock
    return { caseId: 'client_mock_case_001', documentType: 'contract', title: 'Client Mock Legal Document', content: 'This is a client-side fallback used when the WASM parser is unavailable.', metadata: { riskLevel: 'medium', confidence: 0.5, practiceArea: ['general'], jurisdiction: 'unknown', dateCreated: new Date().toISOString(), parties: []
       }
    };
   }
  private readMetadataFromMemory(_offset: number): LegalDocumentJSON['metadata'] {
    ...
    return {
      riskLevel: 'medium', confidence: 0.85, practiceArea: ['corporate'], jurisdiction: 'federal', dateCreated: '2024-01-01', parties: []
    };
   }
  ...
  private extractMetadataJS(jsonString: string): LegalDocumentJSON['metadata'] {
    try {
      ...
      return {
        riskLevel: 'low', confidence: 0, practiceArea: [], jurisdiction: '', dateCreated: '', parties: []
      }; }
  /**
   * Dispose resources
   */ dispose(): void {
