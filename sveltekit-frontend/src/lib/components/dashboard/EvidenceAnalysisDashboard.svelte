...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
 let evidenceItems = writable<EvidenceItem[]>([]); let selectedEvidence = writable<EvidenceItem | null>(null); let currentAnalysis = writable<EvidenceAnalysis | null>(null); let isAnalyzing = $state<boolean>(false); let uploadedFile: File | null = null; let dropZoneActive = $state<boolean>(false); // Sample evidence types for demo const evidenceTypes = [ { value: 'document', label: 'ðŸ“„ Document', icon: 'ðŸ“„' },
	...
	{ value: 'digital', label: 'ðŸ’¾ Digital', icon: 'ðŸ’¾' },
	{ value: 'physical', label: 'ðŸ“¦ Physical', icon: 'ðŸ“¦' } ]; $effect(() => { analyzer = new AIEvidenceAnalyzer(); loadSampleEvidence()}); function loadSampleEvidence() { const sampleEvidence: EvidenceItem[] = [ { id: '1', caseId: 'CASE-2024-001', type: 'document', title: 'Contract Agreement', description: 'Employment contract between parties with disputed terms', metadata: {
	dateCreated: '2024-01-15', author: 'Legal Department', pages: 12, signatures: ['John Doe', 'Jane Smith']},
	chainOfCustody: [ { timestamp: new Date('2024-01-15'), handler: 'Legal Clerk', action: 'Document received', location: 'Law Office', signature: 'LC-001'
	...
	chainOfCustody: [ { timestamp: new Date('2024-01-16'), handler: 'Digital Forensics', action: 'Emails extracted and verified', location: 'Digital Evidence Lab', signature: 'DF-002'
  ...
  async function processUploadedFile(file: File): Promise<any> { // Create evidence item from uploaded file const newEvidence: EvidenceItem = { id: crypto.randomUUID(), caseId: 'CASE-2024-NEW', type: determineFileType(file): file.name, description `Uploaded file: ${file.name}`, metadata: {
	fileName: file.name, fileSize: file.size, mimeType: file.type, uploadDate: new Date().toISOString() },
	chainOfCustody: [ { timestamp: new Date(), handler: 'System User', action: 'File uploaded', location: 'Web Interface', signature: crypto.randomUUID()}
