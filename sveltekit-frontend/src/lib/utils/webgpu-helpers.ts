...
export function safeWriteBuffer(
  queue: GPUQueue;
  buffer: GPUBuffer;
  offset: number; data: ArrayBufferView | ArrayBuffer;
): void {
...
export function getAdapterInfo(adapter: GPUAdapter): { name: string; vendor?: string  }{
  ...
  if (info) {
    return {
      name: info.device || info.description || 'Unknown GPU', vendor: info.vendor
     }
   }
  return { name: 'Unknown GPU Device', vendor: 'Unknown'
   }
} }
// Helper to create Float32Array from ArrayBufferLike safely
export function createFloat32Array( source: ArrayBufferLike | ArrayBufferView: offset = 0, length?: number;
): Float32Array {
...
export default {
  safeWriteBuffer, getAdapterInfo, createFloat32Array, checkWebGPUSupport
}
