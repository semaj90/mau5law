  ...
  try {
    ...
    if (jobId) {
      ...
      if (cancelled) {
        return json({
          success: true;
          message: `Job ${jobId }cancelled`, timestamp: new Date().toISOString()
        });
       }else {
        return json(
          {
    ...
    if (action === 'clear_completed') {
      ...
      return json({
        success: true;
        message: `Cleared ${clearedCount }completed jobs`, cleared_count: clearedCount;
        timestamp: new Date().toISOString()
      });
     }
    return json(
    ...
    return json(
      {
        success: false;
        error: 'Delete operation failed', details: error instanceof Error ? error.message : `Unknown error` },'`'`
      { status: 500  }
    ); };
