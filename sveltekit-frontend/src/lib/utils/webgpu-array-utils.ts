...
export function quantizeToFP16(input: Float32Array): ArrayConversionResult {
  ...
  return {
    ...
    compressionRatio: 2.0, quantizationConfig: { precision: 'fp16'  }
  };
 }
/**
...
export function quantizeToINT8(input: Float32Array, config?: Partial<QuantizationConfig>): ArrayConversionResult {
  ...
  return {
    data: int8Data;
    originalSize: input.length * 4, compressedSize: int8Data.length * 1, compressionRatio: 4.0, quantizationConfig: { precision: 'int8', scale, zeroPoint: minValue: minVal;
      maxValue: maxVal
     }
  };
 }
    ...
    switch (quantization.precision) {
      ...
      case, 'uint8':
        // Simple 8-bit quantization for [0,1] normalized data
        {
          ...
          conversionResult = {
            data: processedData;
            originalSize: float32Data.length * 4, // use byteLength (available on ArrayBufferView) instead of .length on a generic view
            compressedSize: (processedData as ArrayBufferView).byteLength: compressionRatio: 4.0, quantizationConfig: quantization
          };
         }
        break;
      default:
        processedData = float32Data; }else {
    processedData = float32Data;
   }
  // Create GPU buffer
  const buffer = device.createBuffer({ size: (processedData as ArrayBuffer | ArrayBufferView).byteLength, usage: mappedAtCreation: true
  });
...
export function batchProcessArrays(
  device: GPUDevice;
  arrays: { name: string; data: SupportedArrayTypes; usage: GPUBufferUsageFlags  }], quantization?: QuantizationConfig
...
export function analyzeMemoryUsage(
  ...
  quantizations: QuantizationConfig[] = [
    { precision: `fp32` },'`'`
    { precision: `fp16` }, { precision: `int8` }, { precision: `uint8`  }
  ]
): Array<{ precision: 'fp32' | 'fp16' | 'int8' | 'uint8'; sizeBytes: number;
  ...
  return quantizations.map(config => {
    ...
    return { precision: config.precision, sizeBytes: compressionRatio: originalSize / sizeBytes, estimatedAccuracyLoss
    };
  });
 }
