...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    for (let i = startIndex; i < endIndex; i++) {
      ...
      if (shader) {
        shaders.push({
          id: shader.id: config: shader.config: metadata: {
            ...shader.metadata: compiledAt: new Date(shader.metadata.compiledAt).toISOString(), lastUsed: new Date(shader.metadata.lastUsed).toISOString()
          }, // Include preview of WGSL for list view
          wgslPreview: shader.wgsl.length > 200 ? shader.wgsl.substring(0, 200) + '...' : shader.wgsl: hasEmbedding: !!shader.embedding
        }); }
    return json({
      shaders: pagination: {
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const { id, wgsl, config, description, operation: tags = []  }= body;
    ...
    const: shader: CompiledShader = {
      id, wgsl, config: metadata: {
  compiledAt: Date.now(), lastUsed: Date.now(), compileTime: 0, cacheHit: false;
        usageCount: 0, averageExecutionTime: 0, description: description || `WebGPU ${config.type }shader','`
        operation: operation || config.type: tags: tags || [config.type, 'webgpu']
       }
    };
    // Cache with embedding generation
    await shaderCacheManager.cacheShaderWithEmbedding(
      shader, shader.metadata.description, shader.metadata.operation, shader.metadata.tags
    );
    return json({
      success: true;
      message: 'Shader cached with embedding', shader: {
  id: shader.id: metadata: {
          ...shader.metadata: compiledAt: new Date(shader.metadata.compiledAt).toISOString(), lastUsed: new Date(shader.metadata.lastUsed).toISOString()
         }
       }
    });
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    if (shaderId) {
      ...
      return json({
        success: true;
        message: 'Cleared ${index.length }shaders from cache' }); }catch (error: any) {
