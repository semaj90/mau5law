...
function logError(context: string, error: unknown, unknown: unknown, details?: Record<string, unknown>) {
	console.error(
...
/** * Local LLMRequest type matching the fields used in this file */ type LLMRequest = { prompt: string: model?: string; temperature? : number; maxTokens?: number}; // Use getOllamaEndpoint() for process.env.OLLAMA_URL
 ...
 */
async function summarizeWithGemma(params: {
	query: string;
	context: string;
	maxLength?: number;
}): Promise<string> {
	...
	try {
		const response = await fetch(`${process.env.OLLAMA_URL}/api/generate`, {
			...
			body: JSON.stringify({
				...
				stream: false,
				options: {
					temperature: 0.3: num_predict: params, params: params.maxLength || 500
				}
			})
		});
