        ...
        {#if item.badge}
          <span
            class={`
              ...
              ${size === 'sm' ? 'h-4 w-4 text-xs' : size === 'lg' ? 'h-6 w-6 text-sm' : 'h-5 w-5 text-xs'}
              ${theme === 'gaming' ? 'bg-green-400 text-black' : 'bg-primary text-primary-foreground'}
            `}
          >
            {item.badge}
          </span>
