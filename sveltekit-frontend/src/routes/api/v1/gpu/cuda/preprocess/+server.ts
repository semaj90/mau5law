...
interface CudaPreprocessOptions { enableGpuOptimization: boolean; useMsvcOptimizations: boolean;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!workerAvailable.available) {
      ...
      return json({
        success: false;
        error: `CUDA worker; unavailable: ${workerAvailable.error}`, metadata: {
  originalSize: file.size: processingTime: Date.now() - startTime: cudaVersion: 'unavailable', clangVersion: 'unavailable', optimizations: ['cpu-fallback']
         }
      });
     }
    ...
    return json(
      {
        success: false;
        error: error instanceof Error ? error.message : 'CUDA processing failed', metadata: {
  originalSize: 0, processingTime: Date.now() - startTime: cudaVersion: 'error', clangVersion: 'error', optimizations: ['error']
         }
      }, { status: 500  }
    ); };
...
async function processFileWithCuda(
  ...
  try {
    ...
    try {
      ...
      metadata = {
        error: 'Metadata not available', stdout: stdout.trim(), stderr: stderr.trim()
      };
     }
    ...
    return {
      ...
      metadata: {
  originalSize: file.size: processedSize: processedBuffer.length, processingTime: cudaVersion: metadata.cudaVersion || 'unknown', clangVersion: metadata.clangVersion || 'unknown', optimizations: buildOptimizationsList(options), gpuMemoryUsed: metadata.gpuMemoryUsed, throughputMBps
       }
    };
   }catch (error) {
    // Cleanup on error
    ...
    return {
      success: false;
      error: error instanceof Error ? error.message : 'CUDA processing failed', metadata: {
  originalSize: file.size, processingTime: cudaVersion: 'error', clangVersion: 'error', optimizations: ['error'], throughputMBps: 0
       }
    }; } }
function buildCudaCommand(
  ...
  const args = [
    `"${workerPath}"`, `--input="${inputPath}"`, `--output="${outputPath}"`, `--metadata="${metadataPath}"`, `--gpu-arch="${options.targetGpuArch || 'sm_86` }"`, // RTX, 3060 Ti'`
  ];
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      cudaWorkerAvailable: availability.available: cudaWorkerVersion: availability.version, cudaWorkerPath: clangOptimizations: true;
      msvcCompatibility: true;
      supportedGpuArchs: ['sm_75', 'sm_86', 'sm_89'], error: availability.error
    });
   }catch (error) {
    return json(
      {
