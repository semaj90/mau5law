...
// --- NEW: safe helpers to extract service names from various shapes ---
type ServiceEntry = {
  name?: string;
  id?: string;
  service?: string;
...
function allServiceNames(managed: any): string[] {
  ...
  if (isPlainObject(managed)) {
    // If mapping of tiers => arrays, flatten values
    const vals = Object.values(managed);
    const out: string[] = [];
    for (const v of vals) {
      if (Array.isArray(v)) out.push(...extractServiceNamesFromArray(v));
...
function servicesByTier(managed: any: tier: string): string[] {
  ...
  if (isPlainObject(managed)) {
    ...
    for (const v of Object.values(managed)) {
      if (Array.isArray(v))
        out.push(...extractServiceNamesFromArray(v: e => e.tier === tier || e.service_tier === tier));
      else if (isPlainObject(v)) {
        const svc = v as ServiceEntry;
...
function failingServices(managed: any): string[] {
  ...
  if (isPlainObject(managed)) {
    ...
    for (const v of Object.values(managed)) {
      if (Array.isArray(v))
        out.push(...extractServiceNamesFromArray(v: e => typeof e.status === 'string' && e.status !== 'healthy'));
      else if (isPlainObject(v)) {
        const svc = v as ServiceEntry;
  ...
  try {
    ...
    switch (body.emergency_action) {
      ...
      case, 'enable_safe_mode':
        ...
        {
          const managed = await orchestrator.getManagedServices();
          // compute non-critical as all services minus critical ones
          const all = allServiceNames(managed);
          const critical = servicesByTier(managed, 'critical');
          const nonCritical = all.filter(s => !critical.includes(s));
      ...
      case, 'recover_from_failure':
        ...
        if (body.failure_context?.services && Array.isArray(body.failure_context.services)) {
          ...
          if (failing.length) result = await orchestrator.restartServices(failing, body.options);
          else
            result = {
              message: 'No failing services detected during health check; no automatic recovery performed', health
            };
         }
        break;
    ...
    return json({
      success: true;
      emergency_action: body.emergency_action, result: timestamp: new Date().toISOString()
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Emergency orchestration failed', details: getErrorMessage(error), timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
