...
export interface StreamedVertexPayload { id: string; tensor: Float32Array; metadata: Record<string, unknown>;
 }
export class WebGPUVertexStreamer {
  ...
  async streamTensor(id: string: tensor: Float32Array): Promise<void> {
    ...
    const buffer = ctx.device.createBuffer({
      size: tensor.byteLength: usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST: mappedAtCreation: true
    });
    ...
    try {
      if (mapped instanceof ArrayBuffer) {
        ...
        const target = new Float32Array(view.buffer, view.byteOffset, view.byteLength / Float32Array.BYTES_PER_ELEMENT);
        target.set(tensor); }catch {
