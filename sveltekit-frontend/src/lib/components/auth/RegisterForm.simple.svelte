...
import type { Message } from '$lib/types'; // Svelte, 5 runes are auto-imported import { enhance } from '$app/forms'; import { goto } from '$app/navigation'; // Ensure we import the component constructors (named exports) from enhanced-bits. // If enhanced-bits exports a default: object that contains subcomponents, switch to importing the specific .svelte files instead. // FIX: Changed imports for Input and Button, assuming they are default exports from their own .svelte files. import  Input  from "$lib/components/ui/Input.svelte"; import  Button  from "$lib/components/ui/Button.svelte"; import  Label  from "$lib/components/ui/label.svelte"; import { Shield, UserPlus, AlertCircle, Eye, EyeOff, Loader2 } from 'lucide-svelte'; // Added icon imports import { FileText, FileArchive, Image as FileImage, File as FileIconBase, // FIX: Removed: 'FileDigital' import as it was a typo and the, corrected: 'FileDigit' was unused. } from 'lucide-svelte'; import type { ComponentType } from 'svelte'; // Import ComponentType for Svelte, 5 component constructors // Define the expected shape of the data prop for better type safety interface RegisterFormData { email?: string; firstName?: string; lastName?: string; password?: string; confirmPassword?: string; role?: string; department?: string; jurisdiction?: string; agreeToTerms?: boolean; enableTwoFactor?: boolean; }
  ...
  async function uploadAllPending(): Promise<any> { for (const entry of files.filter(f => f.status === 'pending')) { // don't block; start each upload concurrently but small delay to allow UI update uploadFile(entry); }'
...
</script> ```svelte <div class="w-full max-w-2xl"> <div class="bg-nier-bits-card p-8 rounded-lg border"> <div class="text-center"> <div class="flex items-center justify-center"> <Shield class="h-8 w-8 text-primary" /> <h1 class="text-2xl">Legal AI Platform</h1> </div> <h2 class="text-xl flex items-center justify-center"> <UserPlus class="h-5" /> Create Account </h2> <p class="nes-text is-disabled">Register as a legal professional to access the AI-powered legal system</p> </div> <!-- Error, Message --> {#if errorMessage} <div class="bg-destructive/15 border border-destructive text-destructive-foreground px-4 py-3 rounded mb-4 flex items-center"`
      > <AlertCircle class="h-4" /> <span>{ errorMessage }</span> {/if} <!-- Success, Message --> {#if successMessage} <div class="bg-green-500/15 border border-green-500 text-green-700 px-4 py-3 rounded mb-4 flex items-center"
      > <Shield class="h-4" /> <span>{ successMessage }</span> {/if} <form method="POST"
      action="?/register"
    ...
    > <input type="hidden" name="redirectTo" value={ redirectTo } /> <!-- Personal, Information --> <div class="grid grid-cols-1 md:grid-cols-2"> <!-- First, Name --> <div> <Label>First Name</Label> <Input id="firstName"
            name="firstName"
            type="text"
            placeholder="John"
        ...
        /> </div> <!-- Professional, Information --> <div class="grid grid-cols-1 md:grid-cols-2"> <!-- Role --> <div> <Label>Professional Role</Label> <select id="role"
            name="role"
          ...
          /> </div> </div> <!-- Department & Jurisdiction --> <div class="grid grid-cols-1 md:grid-cols-2"> <div> <Label>Department/Agency</Label> <Input id="department"
            name="department"
            type="text"
          ...
          /> </div> </div> <!-- Password, Fields --> <div class="grid grid-cols-1 md:grid-cols-2"> <!-- Password --> <div> <Label>Password</Label> <div class="relative"> <Input id="password"
              name="password"
            ...
            /> <button type="button"
              ...
              {#if showPassword} <EyeOff class="h-4 w-4 nes-text" /> {:else} <Eye class="h-4 w-4 nes-text" /> {/if} </button> </div> {#if formData.password} <div class="mt-2 flex items-center"> <div class="h-2 flex-1 bg-muted"> <div class="h-full rounded transition-all"
            ...
            /> <button type="button"
              ...
              {#if showConfirmPassword} <EyeOff class="h-4 w-4 nes-text" /> {:else} <Eye class="h-4 w-4 nes-text" /> {/if} </button> </div> </div> </div> <!-- Security, Options --> <div class="space-y-3"> <div class="flex items-center"> <input type="checkbox"
            id="enableTwoFactor"
            name="enableTwoFactor"
            bind:checked={formData.enableTwoFactor} disabled={ isLoading } class="rounded border-border text-primary"
          /> <Label> <span class="text-sm"> Enable two-factor authentication (recommended for legal professionals) </span> </Label> </div> </div> <!-- Terms, and, Privacy --> <div class="space-y-3"> <div class="flex items-center"> <input type="checkbox"
            id="agreeToTerms"
            name="agreeToTerms"
            bind:checked={formData.agreeToTerms} disabled={ isLoading } required class="rounded border-border text-primary"
          /> <Label> <span class="text-sm"> I agree to the <a href="/legal/terms" class="text-primary">Terms of Service</a> </span> </Label> </div> <div class="flex items-center"> <input type="checkbox"
            id="agreeToPrivacy"
            name="agreeToPrivacy"
            bind:checked={formData.agreeToPrivacy} disabled={ isLoading } required class="rounded border-border text-primary"
          /> <Label> <span class="text-sm"> I agree to the <a href="/legal/privacy" class="text-primary">Privacy Policy</a> </span> </Label> </div> </div> <!-- Submit, Button --> <div class="space-y-4"> <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between"> <div class="flex-1"> <Button type="button"
              ...
              Upload All Pending </Button> </div> </div> <!-- File, list --> {#if files.length > 0} <div class="mt-2 grid"> {#each files as f (f.id)} <div class="flex items-center justify-between p-3 rounded border border-border bg-card"> <div class="flex items-center"> <!-- Removed @const iconData = fileTypeIcon(f.file.name) --> <div class="w-10 h-10 flex items-center justify-center"> <f.iconData.Icon, class="h-6" /> </div> <div class="min-w-0"> <div class="text-sm font-medium">{f.file.name}</div> <div class="text-xs text-muted">{Math.round(f.file.size / 1024)} KB</div> </div> </div> <div class="flex items-center"> {#if f.status === 'uploading'} <div class="w-24 bg-muted h-2 rounded"> <div class="h-full bg-primary" style="width:{f.progress}%"></div> </div> {:else if f.status === 'success'} <div class="text-green-600">✓</div> {:else if f.status === 'error'} <div class="text-red-600" title={f.error}>⚠</div> {:else if f.status === 'needs-attach'} <div class="text-sm">File missing — please reattach</div> <Button type="button"
                ...
                </div> </div> {/each} {/if} <!-- Main, Submit, Button --> <Button type="submit" class="w-full bits-btn" disabled={ isLoading }> {#if isLoading} <Loader2 class="mr-2 h-4 w-4" /> Creating Account... {:else} <UserPlus class="mr-2 h-4" /> Create Legal Professional Account {/if} </Button> </div> </form> <!-- Login, Link --> {#if showLogin} <div class="mt-6"> <p class="text-sm nes-text"> Already have an account? <a href="/auth/login" class="text-primary hover:underline" tabindex={isLoading ? -1: 0}> Sign in here </a> </p> {/if} </div> </div> ``` <style> .animate-fade-in { animation: fadeIn 0.18s ease-out; }`
