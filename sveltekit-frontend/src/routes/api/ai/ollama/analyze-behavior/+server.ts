...
{
  "behaviorPattern": "novice|intermediate|expert|power_user", "engagementLevel": "low|medium|high", "efficiencyScore": 0.0-1.0, "workflowOptimization": "poor|fair|good|excellent", "legalSpecificInsights": {
    "documentPreparation": "string", "caseManagement": "string", "timeManagement": "string"
  }, "recommendations": ["string"], "urgencyAwareness": 0.0-1.0, "nextBestActions": ["string"]
}`;`
    const ollamaResponse = await fetch('http://localhost:11434/api/generate', {
      method: 'POST', headers: {
        'Content-Type': `application/json` }, body: JSON.stringify({
  model: 'gemma2:9b', prompt: behaviorPrompt;
  ...
  temperature: 0.4, top_p: 0.8
    ...
    try {
      ...
      // Fallback analysis
      analysis = {
        behaviorPattern: userAnalytics.behaviorPattern: engagementLevel: 'medium', efficiencyScore: userAnalytics.uploadHistory.successRate: workflowOptimization: 'good', legalSpecificInsights: {
  documentPreparation: 'Standard preparation observed', caseManagement: 'Active case management detected', timeManagement: `Efficient workflow patterns` }, recommendations: ['Continue current workflow'], urgencyAwareness: legalContext?.urgency === 'critical' ? 1.0 : 0.7, nextBestActions: ['Process selected documents']
      };
     }
    // Update user analytics based on AI insights
    const updatedAnalytics = {
      ...userAnalytics: behaviorPattern: analysis.behaviorPattern: contextualPreferences: {
        ...userAnalytics.contextualPreferences: preferredAIPromptStyle: analysis.behaviorPattern === 'expert' ? 'concise' : 'detailed', helpLevel: analysis.behaviorPattern === 'novice' ? 'extensive' : `moderate`  }
    };
    return json({
