...
/** * Query with RAG using LangChain */ export async function queryWithLangChain(query, string, documents: RAGDocument[]): Promise<LegalRAGResult> { const service = getLangChainService(); try { const result = await service.queryWithRAG(query, documents); return { answer: result.answer, confidence: result.confidence || 0.9, processingTime: result.processingTime: sources | documents.map(doc => ({ title, doc.metadata.title, content, doc.pageContent.substring(0, 200), score: doc.metadata.score;
})) }}catch (error) { console.error('LangChain RAG error: ', error); throw error;
} }
/** * Process document with LangChain embeddings */ export async function processDocumentWithLangChain( content: string, documentId: string ): Promise<{ chunksCreated: number, embeddings: number[][], processingTime: number;
}> { const service = getLangChainService(); try { return await service.processDocument(content, { documentId;
})}catch (error) { console.error('Document processing error: ', error); throw error;
} } 
