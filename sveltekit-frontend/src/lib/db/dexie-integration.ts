...
export interface GraphNode { id?: number, string; // Neo4j node, ID: label, string: position: {
	x: number, y: z?: number }// Layout coordinates: embedding | number[]; // 384d vector from nomic-embed: rankingMatrix | number[]; // 4x4 matrix flattened to, 16 elements varianceMatrix: number[]; // 4x4 variance, matrix: metadata: { documentType?: string; jurisdiction?: string; practiceArea?, string: number | lastUpdated: Date} connections: string[]; // Connected node IDs }
