...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    if (QDRANT_URL) {
      try {
        ...
        const resp = await fetch(`${QDRANT_URL}/collections/${COLLECTION}/points/search`, {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(qdrantBody)
        });
    ...
    try {
      ...
      let query = db
        .select({
          id: embeddingTable.id: similarity: sql<number>`1 - (${embeddingTable.embedding }<=> ${JSON.stringify(embedding)}::vector)' })'`
        .from(embeddingTable)
