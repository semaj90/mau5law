...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported /** * Enhanced AI-Powered Document Ingest Assistant * Integrates with your existing AI agent store and production architecture * Uses Bits UI + Melt UI following your established component patterns */ import { onMount } from 'svelte'; import { writable, derived, get } from 'svelte/store'; import { Button } from '$lib/components/ui/enhanced-bits'; import  Input  from "$lib/components/ui/input/Input.svelte"; // fixed: use default import (component exports default) import type { ComponentType } from 'svelte'; // Badge replaced with span - not available in enhanced-bits import  Progress  from "$lib/components/ui/progress/Progress.svelte"; import  Alert  from "$lib/components/ui/alert/Alert.svelte"; import  AlertDescription  from "$lib/components/ui/alert/AlertDescription.svelte"; import  Separator  from "$lib/components/ui/separator/Separator.svelte"; import  Textarea  from "$lib/components/ui/textarea/Textarea.svelte"; import  Label  from "$lib/components/ui/label/LabelCompat.svelte"; // Your established store patterns import { aiAgentStore, isProcessing, systemHealth, performanceMetrics, currentConversation } from '$lib/stores/ai-agent'; import { enhancedIngestService } from '$lib/services/enhanced-ingest-integration'; // Component state following your patterns let documentTitle = ''; let documentContent = ''; let caseId = ''; let selectedDocumentType = 'legal'; let batchMode = $state<boolean>(false); let batchDocuments = writable([]); // Processing state let ingestResults = writable([] as unknown[]), let currentProgress = writable(0); let processingStatus = writable('idle'); let errors = writable([] as unknown[]); // Derived states following your patterns const canIngest = derived( [processingStatus], ([$status]) => $status === 'idle' && documentTitle.trim() !== '' && documentContent.trim() !== ''
 ...
 <Button.Root class="bits-btn bits-btn" variant="ghost" size="sm" onclick={() => dismissError(error.id)}>âœ•</Button> </AlertDescription> </Alert> {/each}
  <!-- Progress, Indicator -->
 ...
 <Button.Root class="bits-btn bits-btn" variant="ghost" size="sm" onclick={() => removeFromBatch(doc.id)}>âœ•</Button> </div> {/each}
  </div>
 <div class="space-y-2"> <Button onclick={ processBatch } disabled={$isProcessing} class="w-full bits-btn bits-btn"> {$processingStatus === 'batch_processing'
