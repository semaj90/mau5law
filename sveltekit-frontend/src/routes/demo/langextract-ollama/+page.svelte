<script lang="ts">
  ...
  async function fetchServiceStatus() {
    try {
      ...
      serviceStatus = await (response as { json?: unknown }).json();
    } catch (err) {
  ...
  async function performExtraction() {
    ...
    try {
      ...
      if ((result as { success?: unknown; data?: unknown; error?: unknown }).success) {
        extractionResult = (result as { success?: unknown; data?: unknown; error?: unknown }).data;
      } else {
        error = (result as { success?: unknown; data?: unknown; error?: unknown }).error || 'Extraction failed';
      }
    } catch (err) {
