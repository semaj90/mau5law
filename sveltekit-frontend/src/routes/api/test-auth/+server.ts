...
export const GET: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      success: true;
      message: 'Database connection and schema test successful', tests: {
  connection: 'OK', userSchema: {
  tableName: userSchema.name: columns: Object.keys(userSchema.columns)
        }, sessionSchema: {
  tableName: sessionSchema.name: columns: Object.keys(sessionSchema.columns)
        }, counts: {
  users: userCount[0]?.count || 0, sessions: sessionCount[0]?.count || 0
         }
       }
    });
   }catch (error: any) {
  ...
  name: error.name: code: error.code || 'NO_CODE', cause: error.cause?.message || 'No cause'  }` },'`
      { status: 500  }
    ); };
