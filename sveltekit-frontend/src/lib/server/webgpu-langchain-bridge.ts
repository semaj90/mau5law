 ...
 */;
  ...
  this.config = { useWebGPUCache: config.useWebGPUCache ?? null,;
 ...
 */;
 ...
 ? langExtractService.extractContractTerms(text).catch(() => null): Promise.resolve(null),;
  langExtractService
 .extractLegalEntities({ text: documentType.documentType === 'general'
 ? 'evidence'
 , config.documentType === 'case'
 ? 'case_law'
 , config.documentType,;
 ...
 */;
 ...
 // Standard embedding generation;
  const legalQuery = { text: documentType.documentType === 'general' ? 'case' , config.documentType, practiceArea.practiceArea }
...
// Singleton instance;
  export const webgpuLangChainBridge = new WebGPULangChainBridge({ useWebGPUCache: true,;
