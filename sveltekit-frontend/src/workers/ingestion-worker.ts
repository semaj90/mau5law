 ...
 * (Corrected worker/manager implementation: syntax fixes, event handling, typings)
 */
import type { SOMConfig } from '$lib/ai/som-neural-network';
...
const isWorkerEnv =
  ...
  typeof (globalSelf as MaybeWorker).postMessage === 'function' &&
  // ensure this is not a window/tab environment
  typeof window === 'undefined';

// Local, minimal EmbeddingOptions shape to avoid importing a namespace-as-type
  ...
  class DocumentIngestionWorker {
    ...
    private async processIngestion(task: IngestionTask): Promise<IngestionResult> {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        // Pass a typed UploadOptions object (no "as any")
    ...
    private async simulateMinIOUpload(
      ...
      // Delegate to shared minioService (keeps the worker lean and testable)
      return minioService.upload(file, originalName, options);
    }
    private async generateEmbeddingsForFiles(uploadResults: UploadResult[], options: EmbeddingOptions) {
      return embeddingService.generateForFiles(uploadResults, options);
    }
    ...
    private async applyRTXCompression(uploadResults: UploadResult[]) {
      ...
      for (const result of uploadResults) {
        if (result && result.success) {
          ...
          compressedSize += Math.floor(size / 50) || 0;
        }
      }
      // Simulate RTX processing time
      await new Promise(resolve => setTimeout(resolve, 1500));
      const safeCompressed = compressedSize === 0 ? 1 : compressedSize;
      return {
        originalSize,
        compressedSize,
    ...
    private async processEmbeddingGeneration(data: { texts: string[]; options?: EmbeddingOptions } | unknown) {
      ...
      // derive embedding dimension from options (fallback to 384)
      const dimension =
    ...
    private async processSOMClustering(data: { embeddings?: number[][]; config?: SOMConfig } | unknown) {
      const payload = (data as { embeddings?: number[][]; config?: SOMConfig }) || {};
      const embeddings = Array.isArray(payload.embeddings) ? payload.embeddings : [];
      const config = payload.config;
      const som = new SOMNeuralNetwork(config);
      return som.train(embeddings, config);
    }
    private async processRTXCompression(
      data: { documents?: Array<{ size?: number }>; compressionRatio?: number } | unknown
    ) {
      const payload = (data as { documents?: Array<{ size?: number }>; compressionRatio?: number }) || {};
      const documents = Array.isArray(payload.documents) ? payload.documents : [];
      const compressionRatio = payload.compressionRatio;
      let totalOriginal = 0;
      let totalCompressed = 0;
      for (const doc of documents) {
        const size = doc.size || 1000;
        totalOriginal += size;
        totalCompressed += Math.floor(size / (compressionRatio || 50));
      }
      // Simulate RTX processing
      await new Promise(resolve => setTimeout(resolve, 2000));
      const safeCompressed = totalCompressed === 0 ? 1 : totalCompressed;
      return {
    ...
    private getMemoryUsage() {
      // Use a typed intersection for performance and guard memory existence
      const perf =
        typeof performance !== 'undefined' ? (performance as Performance & { memory?: PerformanceMemory }) : undefined;
      if (perf?.memory) {
    ...
    private postResponse(response: WorkerResponse): void {
      ...
      try {
        ...
        try {
          ...
          console && console.warn && console.warn('Worker postMessage failed:', msg);
        } catch {
...
export class IngestionWorkerManager {
  ...
  // remove the invalid in-class type declaration (was: "private type PendingTask = { ... }")
  // use the top-level PendingTask type for the Map below
  ...
  private async executeTask(
    ...
    return new Promise((resolve, reject) => {
      // Wrap reject so it matches (error: Error) => void
      const rejectWrapper = (err: Error) => reject(err);
      const taskEntry: PendingTask = { resolve, reject: rejectWrapper };
      if (onProgress) taskEntry.onProgress = onProgress;
...
// --- Production-ready minimal MinIO service (simulates upload; replace with real MinIO client in prod) ---
class MinioService {
  async upload(file: File | Buffer, originalName: string, options?: UploadOptions): Promise<UploadResult> {
    // Minimal deterministic simulation of an upload; in production replace with SDK calls (minio or aws s3)
    await new Promise(resolve => setTimeout(resolve, Math.random() * 600 + 400));
    const fileSize = file instanceof File ? file.size : (file as Buffer).length || 0;
    const fileId = `${Date.now()}-${Math.random().toString(36).substring(2)}`;
    const bucket = options?.bucket ?? 'legal-documents';
    ...
    if (options?.caseId !== undefined) metadata.caseId = options.caseId;
...
// --- Embedding service (replace with real embedding provider integration in prod) ---
class EmbeddingService {
  async generate(texts: string[], options: EmbeddingOptions = {}) {
    const dimension =
      typeof options.dimension === 'number' && Number.isFinite(options.dimension) && options.dimension > 0
        ? Math.max(1, Math.floor(options.dimension))
        : 384;
    const embeddings = [];
    for (const text of texts) {
      // lightweight deterministic pseudo-embedding for testing
      await new Promise(resolve => setTimeout(resolve, 20));
      const embedding = new Array(dimension).fill(0).map((_, i) => Math.sin((text.length + i) % (dimension || 1)));
      embeddings.push({
        text,
        embedding,
        metadata: {
  ...
  async generateForFiles(uploadResults: UploadResult[], options: EmbeddingOptions = {}) {
    const start = performance.now();
    let documentEmbeddings = 0;
    let chunkEmbeddings = 0;
    const chunkSize =
      typeof options.chunkSize === 'number' && Number.isFinite(options.chunkSize) && options.chunkSize > 0
        ? options.chunkSize
        : 600;
    const overlap =
      typeof options.overlap === 'number' && Number.isFinite(options.overlap) && options.overlap >= 0
        ? options.overlap
        : 60;
...
// --- Simple SOM Neural Network wrapper used by worker and by other code paths ---
export class SOMNeuralNetwork {
  // make the stored config explicitly possibly undefined so assignments from optional params succeed
  private config: SOMConfig | undefined;
  constructor(config?: SOMConfig) {
    this.config = config;
  }

  async train(embeddings: number[][], config?: SOMConfig) {
    // Use provided config parameter if given, otherwise fall back to the instance config.
    // This ensures the constructor-stored config is actually used (avoids "value is never read").
    type SOMConfigWithEpochs = SOMConfig & { epochs?: number };
    const cfg = (config ?? this.config) as SOMConfigWithEpochs | undefined;

    const epochs =
      typeof cfg?.epochs === 'number' && Number.isFinite(cfg.epochs) && cfg.epochs > 0
        ? Math.floor(cfg.epochs)
        : Math.floor(Math.random() * 50) + 25;

    // Simulate deterministic training time proportional to inputs
    await new Promise(resolve => setTimeout(resolve, Math.min(5000, embeddings.length * 10 + epochs * 10)));
    const clusters = Math.max(2, Math.min(20, Math.floor(embeddings.length / 3) || Math.floor(Math.random() * 10) + 2));
    return {
      clusters,
  ...
  // Convenience cluster method (returns mocked cluster assignment)
  cluster(embeddings: number[][]) {
    const k = Math.max(2, Math.min(20, Math.floor(embeddings.length / 3) || 2));
    return new Array(embeddings.length).fill(0).map((_, i) => i % k);
  }
}

// --- small helpers exported for reuse by embeddingService above ---
