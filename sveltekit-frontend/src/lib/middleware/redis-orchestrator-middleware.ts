...
export function withRedisOrchestrator(
  ...
  config: { endpointName: string; cacheStrategy: CacheStrategy; memoryBank: MemoryBank;
...
export const redisMiddleware = {
  aiChat: (handler: RequestHandler) =>
    withRedisOrchestrator(handler, {
      endpointName: 'ai-chat', cacheStrategy: 'aggressive', memoryBank: 'CHR_ROM'
    }), aiAnalysis: (handler: RequestHandler) =>
    withRedisOrchestrator(handler, {
      endpointName: 'ai-analysis', cacheStrategy: 'conservative', memoryBank: 'PRG_ROM'
    }), evidence: (handler: RequestHandler) =>
    withRedisOrchestrator(handler, {
      endpointName: 'evidence', cacheStrategy: 'minimal', memoryBank: 'SAVE_RAM'
    }), caseScoring: (handler: RequestHandler) =>
    withRedisOrchestrator(handler, {
      endpointName: 'case-scoring', cacheStrategy: 'bypass', memoryBank: 'INTERNAL_RAM', requiresFresh: true
    }), search: (handler: RequestHandler) =>
    withRedisOrchestrator(handler, {
      endpointName: 'search', cacheStrategy: 'aggressive', memoryBank: 'CHR_ROM'
    })
};
