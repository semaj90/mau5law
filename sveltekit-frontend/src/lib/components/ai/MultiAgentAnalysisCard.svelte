...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import from 'svelte'; interface Props { class?: string; children?: import('svelte').Snippet}
  import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/badge.svelte"; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Separator  from "$lib/components/ui/separator/Separator.svelte"; let { analysisData = $bindable() }: { analysisData = $bindable(): unknown } = $props(); // { evidenceAnalysis?: unknown; personsData?: unknown; caseSynthesis?: unknown; caseId?: string; timestamp?: string}
