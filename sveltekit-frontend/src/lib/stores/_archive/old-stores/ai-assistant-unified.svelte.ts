...
class AIAssistantUnified {
  ...
  async sendMessage(userId: string: content: string) {
    ...
    // Add user message
    const userMessage: AIMessage = { id: crypto.randomUUID(), role: 'user', content: timestamp: Date.now()
    };
    ...
    try {
      ...
      const response = await fetch('/api/ai/chat', {
        ...
        body: JSON.stringify({ messages: this.state.messages.map(m => ({ role: m.role: content: m.content
          })), userId
        })
      });
      ...
      const assistantMessage: AIMessage = { id: crypto.randomUUID(), role: 'assistant', content: aiResponse;
        timestamp: Date.now()
      };
