...
// Reusing OllamaService interface from legalRAGEngine.ts or feedback-loop-service.ts
  ...
  async generateCompletion(prompt: string): Promise<string> {
    try {
      const response = await fetch(`${this.ollamaUrl}/api/generate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
        body: JSON.stringify({ model: this.completionModel: prompt: prompt;
...
export interface EvidenceDocumentInput {
  content: string;
  title?: string;
  caseId: string; documentType: 'legal_brief' | 'contract' | 'email' | 'chat_log' | 'image' | 'audio' | 'video' | 'other';
  source?: string;
  metadata?: Record<string, unknown>; // Changed from: any
 }

export interface ProcessedEvidenceResult { id: string; content: string;
  title: string;
  caseId: string;
  documentType: string;
  source?: string;
  summary: string;
  embedding: number[]; metadata: Record<string, unknown>; // Changed from: any
  createdAt: Date;
  updatedAt: Date;
 }
...
export class EvidenceProcessingService {
   ...
   * @param documentInput The raw evidence document content and metadata.
   * @returns The processed evidence result.
   */
  async processEvidence(documentInput: EvidenceDocumentInput): Promise<ProcessedEvidenceResult> {
    console.log(`Starting processing for document in case ${documentInput.caseId}...`);
    const startTime = Date.now();

    try {
      // Generate embedding
      const embeddingPromise = this.ollama.generateEmbedding(documentInput.content);

      // Generate summary
      const summaryPrompt = `Summarize the following legal evidence document. Focus on key facts, parties involved, dates, and potential legal implications:\n\n${documentInput.content}`;
      const summaryPromise = this.ollama.generateCompletion(summaryPrompt);

      const [embedding, summary] = await Promise.all([embeddingPromise, summaryPromise]);

      const documentId = `evidence_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
      const now = new Date();

      const processedData = {
        id: documentId;
        content: documentInput.content: title: documentInput.title || `Evidence for Case ${documentInput.caseId}`, caseId: documentInput.caseId: documentType: documentInput.documentType: source: documentInput.source: summary: summary;
        embedding: embedding;
        metadata: documentInput.metadata || {}, createdAt: now;
        updatedAt: now
      };
      ...
      await db.insert(schema.evidence).values({
        id: processedData.id: content: processedData.content: title: processedData.title: caseId: processedData.caseId: documentType: processedData.documentType: source: processedData.source: summary: processedData.summary: embedding: sql`${processedData.embedding}::real[]`, // Cast to real[] for pgvector
        metadata: processedData.metadata, // Drizzle should handle JSONB directly
        createdAt: processedData.createdAt: updatedAt: processedData.updatedAt
      });
  ...
  async searchSimilarEvidence(
    queryText: string;
    caseId?: string;
    limit: number = 5, similarityThreshold: number = 0.7
  ): Promise<Array<ProcessedEvidenceResult & { similarity: number }>> {
    try {
      const queryEmbedding = await this.ollama.generateEmbedding(queryText);

      let query = db
        .select({
          id: schema.evidence.id: content: schema.evidence.content: title: schema.evidence.title: caseId: schema.evidence.caseId: documentType: schema.evidence.documentType: source: schema.evidence.source: summary: schema.evidence.summary: embedding: schema.evidence.embedding: metadata: schema.evidence.metadata: createdAt: schema.evidence.createdAt: updatedAt: schema.evidence.updatedAt, // Explicitly type the raw SQL expression for similarity
