...
describe('Authentication Machine - Phase 5-7 Performance Testing', () => {
  ...
  describe('XState v5 Machine Logic (Unit Tests)', () => {
    it('should transition to authenticated state on successful login', async () => {
      const endMeasure = perf.start('xstate-v5-login-success');

      // Create XState v5 actor
      ...
      expect(finalSnapshot.context.user).toBeDefined();
      expect(finalSnapshot.context.authToken).toBe('mock-auth-token');
      ...
      expect(duration).toBeLessThan(200); // Performance baseline for gRPC comparison

      authActor.stop();
    });

    it('should measure performance for gRPC optimization baseline', async () => {
      const authActor = createActor(authMachine);
      authActor.start();

      // Performance tracking for Phase 5-7 gRPC migration
      ...
      expect(averageTime).toBeLessThan(300); // Current HTTP baseline
  ...
  describe('Token Management for gRPC Authentication', () => {
    it('should handle token refresh for long-running gRPC connections', async () => {
      const authActor = createActor(authMachine);
      authActor.start();
      ...
      const finalSnapshot = authActor.getSnapshot();
      expect(finalSnapshot.value).toBe('authenticated');
      expect(finalSnapshot.context.authToken).toBe('new-token-456');
      expect(mockServices.refreshAuthToken).toHaveBeenCalledTimes(1);

      authActor.stop();
    });

    it('should logout on failed token refresh', async () => {
      mockServices.refreshAuthToken.mockRejectedValueOnce(
      ...
      expect(finalSnapshot.context.user).toBeUndefined();
      expect(finalSnapshot.context.authToken).toBeUndefined();

      authActor.stop();
    });
  });

  describe('Integration with Service Coordination (Phase 5-7)', () => {
    it('should coordinate with other machines through events', async () => {
      const authActor = createActor(authMachine);

      // Mock event handlers for cross-machine communication
      const sessionMachineHandler = vi.fn();
...
describe('Phase 5-7 Performance Benchmarks', () => {
  it('should establish HTTP baseline for gRPC comparison', () => {
    ...
    if (stats) {
      ...
      console.log(`\n🎯 Phase 5-7 gRPC Targets:`);
      console.log(`   Target Average: ${(stats.average * 0.4).toFixed(2)}ms (60% improvement)`);
  ...
  describe('Login Flow', () => {
    it('should transition to authenticated state on successful login', async () => {
      const endMeasure = perf.start('login-success');

      // Create machine instance for step-by-step testing
      ...
      // Assert final state
      expect(finalState.value).toBe('authenticated');
      expect(finalState.context.user).toBeDefined();
      expect(finalState.context.user.id).toBe('test-123');
      expect(finalState.context.authToken).toBe('mock-auth-token');
      expect(finalState.context.error).toBeUndefined();
      ...
      expect(mockServices.validateCredentials).toHaveBeenCalledTimes(1);

      const duration = endMeasure();
      expect(duration).toBeLessThan(200); // Performance expectation
    ...
    it('should transition to error state on authentication failure', async () => {
      ...
      expect(finalState.context.error).toBe('Invalid credentials');
      expect(finalState.context.user).toBeUndefined();
      expect(finalState.context.retryCount).toBe(1);

      service.stop();
    });

    it('should prevent login after 3 failed attempts', async () => {
      mockServices.validateCredentials.mockRejectedValue(
        new Error('Invalid credentials')
      );
      ...
      const stateAfter3Attempts = service.getSnapshot();
      expect(stateAfter3Attempts.context.retryCount).toBe(3);
      ...
      const finalState = service.getSnapshot();
      expect(finalState.value).toBe('error'); // Should remain in error state
      expect(mockServices.validateCredentials).toHaveBeenCalledTimes(3); // No additional call
  ...
  describe('Token Refresh Flow', () => {
    it('should automatically refresh expired tokens', async () => {
      ...
      expect(service.getSnapshot().value).toBe('authenticated');

      // Simulate token expiration
      service.send({ type: 'TOKEN_EXPIRED' });
      await new Promise(resolve => setTimeout(resolve, 100));

      const finalState = service.getSnapshot();

      expect(finalState.value).toBe('authenticated');
      expect(finalState.context.authToken).toBe('new-token-456');
      expect(mockServices.refreshAuthToken).toHaveBeenCalledTimes(1);
    ...
    it('should logout user if token refresh fails', async () => {
      ...
      // Simulate token expiration with failed refresh
      service.send({ type: 'TOKEN_EXPIRED' });
      await new Promise(resolve => setTimeout(resolve, 100));
      ...
      expect(finalState.context.user).toBeUndefined();
      expect(finalState.context.authToken).toBeUndefined();
      expect(finalState.context.error).toBe('Refresh token expired');
  ...
  describe('Performance Requirements', () => {
    it('should complete login within performance thresholds', async () => {
      const measurements: number[] = [];

      // Run multiple login cycles to get average performance
      for (let i = 0; i < 10; i++) {
        const endMeasure = perf.start(`login-cycle-${i}`);
      ...
      expect(averageTime).toBeLessThan(150); // Average under 150ms
      expect(maxTime).toBeLessThan(250); // Max under 250ms
    });
  });

  describe('Integration with XState Service Coordination', () => {
    it('should coordinate with other machines through events', async () => {
      const service = interpret(testMachine);
      service.start();

      // Mock event handler for cross-machine communication
      const sessionMachineHandler = vi.fn();
      service.onTransition((state) => {
        if (state.value === 'authenticated' && state.context.user) {
          // Simulate sending event to session machine
          sessionMachineHandler('USER_AUTHENTICATED', {
...
describe('Authentication Machine Performance Report', () => {
  it('should report performance statistics', () => {
    const stats = perf.getStats('login-success');
    if (stats) {
