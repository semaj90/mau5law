<script lang="ts">
  ...
  interface Props {
    width?: number;
    height?: number;
    data?: CanvasDataPoint[];
    theme?: 'yorha' | 'nes' | 'legal';
    interactive?: boolean;
    showGrid?: boolean;
    onNodeClick?: (node: CanvasDataPoint) => void;
    onNodeHover?: (node: CanvasDataPoint | null) => void;
    children?: Snippet;
  }
  let {
    width = 800,
  ...
  function drawConnections() {
    if (!ctx) return;
    ctx.strokeStyle = currentTheme.connectio;
    ctx.lineWidth = 2;
    ctx.globalAlpha = 0.3;
    // Draw connections between related nodes
    data.forEach(node => {
      if (node.metadata.connections) {
        node.metadata.connections.forEach((connectionId: string) => {
          const targetNode = data.find(n => n.id === connectionId);
          if (targetNode) {
            ctx.beginPath();
  ...
  function drawNodes() {
    ...
    data.forEach(node => {
      ...
      const baseSize = 8;
      const size = baseSize + (isHovered ? 4 : 0) + (isSelected ? 6 : 0);
      // Color based on type and risk level
      const typeColor = currentTheme[node.type] || currentTheme.accent;
      const riskColor = currentTheme[`risk${node.riskLevel.charAt(0).toUpperCase() + node.riskLevel.slice(1)}`];
  ...
  function drawHoverEffects() {
    if (!ctx || !hoveredNode) return;
    // Draw tooltip background
    const tooltipText = `${hoveredNode.label} (${hoveredNode.type})`;
    ...
    ctx.fillText(`Risk: ${hoveredNode.riskLevel}`, tooltipX + 10, tooltipY - 8);
  }
  function drawStats() {
    if (!ctx) return;
    const stats = {
...
;
    ...
    ctx.textAlign = 'left';
    let y = 25;
    ctx.fillText(`Nodes: ${stats.nodes}`, 20, y);
    y += 12;
    ctx.fillText(`Evidence: ${stats.evidence}`, 20, y);
    y += 12;
    ctx.fillText(`Cases: ${stats.cases}`, 20, y);
    y += 12;
    ctx.fillText(`Documents: ${stats.documents}`, 20, y);
    y += 12;
    ctx.fillText(`High Risk: ${stats.highRisk}`, 20, y);
    y += 12;
    ctx.fillText(`WebGPU: ${isWebGPUSupported ? 'Yes' : 'No'}`, 20, y);
  }
  function getNodeAtPosition(x: number, y: number): CanvasDataPoint | null {
  ...
  $effect(() => {
    if (data.length === 0) {
      // Generate sample legal data points
      const sampleData: CanvasDataPoint[] = [
        {
          id: 'case-001',
          x: 100,
          y: 100,
...
<div class="canvas-container nes-container with-title">
  ...
  {#if selectedNode}
    <div class="node-details nes-container">
