...
export const LOCAL_LLM_PATHS = {
  // Ollama installation
  ollama: { executable: join(projectRoot, 'Ollama', 'ollama.exe'),
  ...
  llamaCpp: { basePath: join(projectRoot, 'llama.cpp'),
    executable: join(projectRoot, 'llama.cpp', 'llama-server.exe'),
    baseUrl: 'http://localhost:8080'
  },
  // Local Gemma3 Q4_K_M model
  gemmaModel: { path: join(projectRoot, 'gemma3Q4_K_M', 'mohf16-Q4_K_M.gguf'),
...
export function checkLocalInstallations() {
  const status = { ollama: { available: existsSync(LOCAL_LLM_PATHS.ollama.executable),
      path: LOCAL_LLM_PATHS.ollama.executable
    },
    llamaCpp: { available: existsSync(LOCAL_LLM_PATHS.llamaCpp.basePath),
      path: LOCAL_LLM_PATHS.llamaCpp.basePath
    },
    gemmaModel: { available: existsSync(LOCAL_LLM_PATHS.gemmaModel.path),
