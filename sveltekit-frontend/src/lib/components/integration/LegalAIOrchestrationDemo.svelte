...
<script lang="ts">
	...
	import { currentWorkflowStore, healthStore, isSystemHealthy, workflowOrchestrator, workflowStore } from '$lib/services/end-to-end-api-integration';
...
<div class="max-w-6xl mx-auto p-6">
 ...
 {:else}
 Execute {selectedWorkflow.split('-').map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')} Workflow
 {/if}
 </Button>
 </div>
 </CardContent>
 ...
 <div class="text-lg">{activeWorkflowStatus.progress}%</div>
 </div>
 </div>
 <div class="mt-4">
