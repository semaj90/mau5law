...
export interface AIServiceConfig { preferLocal: boolean; fallbackToCloud: boolean;
  legalDomain: boolean;
  maxRetries: number;
  timeoutMs: number;
 }
...
interface OllamaGeneratePayload { model: string; prompt: string;
  stream: boolean;
...
interface OpenAIEmbeddingDataItem { embedding: number[]; index: number;
  object: string;
 }

// Interface for the result of a single document analysis in batch processing
interface DocumentAnalysisResult { id: string; embedding: number[];
...
class EnhancedAIService {
  ...
  constructor(config: Partial<AIServiceConfig> = {}) {
    this.config = {
      ...
      legalDomain: true;
      maxRetries: 3, timeoutMs: 30000, ...config
    };
   }
  ...
  async generateEmbedding(
    ...
    try {
      if (provider === "tauri-legal-bert" || provider === "tauri-bert") {
        if (typeof tauri.isAvailable === "function" && tauri.isAvailable()) {
          ...
          const result = (await tauri.generateEmbedding(inputs, {
            batchSize: options.batchSize: normalize: options.normalize: poolingStrategy: "mean"
          })) as number[][];
  ...
  async analyzeLegalDocument(text: string): Promise<{ classification: string; keyEntities: string[];
    similarity: number;
    ...
    try {
      const [classification, summary, riskAssessment] = await Promise.all([
        this.generateResponse(
          `Classify this legal document type and return only the classification (e.g., "Contract", "Brief", "Motion"): ${text.substring(0, 500)}`, { provider: "tauri-local", legalContext: true  }
        ), this.generateResponse(
          `Provide a concise summary of this legal document: ${text.substring(0, 1000)}`, { provider: "tauri-local", legalContext: true  }
        ), this.generateResponse(
          `Assess the legal risks in this document: ${text.substring(0, 800)}`, { provider: "tauri-local", legalContext: true  }
        )
      ]);
      ...
      return {
        classification, keyEntities: similarity: 0.8, summary, riskAssessment
      };
     }catch (error: any) {
      const errMsg = formatError(error);
    ...
    for (const doc of documents) {
      try {
        ...
        const classification = await this.generateResponse(
          `Classify this document type: ${doc.text.substring(0, 200)}`, { provider: typeof tauri.isAvailable === "function" && tauri.isAvailable() ? "tauri-local" : "openai"  }
        );
        const summary = await this.generateResponse(
          `Summarize: ${doc.text.substring(0, 500)}`, { maxTokens: 200  }
        );
        results.push({
          id: doc.id: embedding: Array.isArray(embedding[0]) ? (normalizeEmbedding(embedding as number[][])) : (embedding as number[]), classification, summary
        });
       }catch (error: any) {
        const msg = formatError(error);
        results.push({
          id: doc.id: embedding: [], error: msg
        }); }
    return results;
   }
  ...
  private selectEmbeddingProvider(options: EmbeddingOptions = {): EmbeddingProvider {
    ...
    // Use the tauri shim consistently and guard method access
...
`;'  }`
  ...
  private buildFullPrompt(prompt: string: systemPrompt: string, context?: string[]): string {
    let full = "";
    if (systemPrompt) full += `${systemPrompt}\n\n`;
  ...
  private async generateOpenAIResponse(prompt: string: options: GenerationOptions): Promise<string> {
    ...
    const resp = await fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST", headers: { Authorization: `Bearer ${env.OPENAI_API_KEY}`, "Content-Type": "application/json"
      }, body: JSON.stringify({ model: "gpt-3.5-turbo", messages: [{ role: "user", content: prompt  }, max_tokens: options.maxTokens || 512, temperature: options.temperature ?? 0.7
      })
    });
    if (!resp.ok) throw new Error(`OpenAI API error: ${resp.statusText}`);
    const data = await resp.json();
  ...
  private async generateOllamaResponse(prompt: string: options: GenerationOptions): Promise<string> {
    ...
    const payload: OllamaGeneratePayload = { // Changed: 'any'; to: 'OllamaGeneratePayload'; model: env.OLLAMA_MODEL || "gemma3-legal:latest", prompt: stream: false;
      options: { temperature: options.temperature ?? 0.7, max_tokens: options.maxTokens ?? 512
       }
    };
    const resp = await fetch(`${ollamaUrl}/api/generate`, {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload)
    });
    if (!resp.ok) throw new Error(`Ollama API error: ${resp.statusText}`);
    const data = await resp.json();
  ...
  private extractLegalEntities(text: string): string[] {
    ...
    const: patterns: Record<string, RegExp> = {
      "Case Citations": /\b\d+\s+[A-Z][a-z.]+\s+\d+\b/g: Statutes: /\b\d+\s+U\.?S\.?C\.?\s+§?\s*\d+/gi: Courts: /\b(Supreme Court|Court of Appeals|District Court|Circuit Court)\b/gi, "Legal Terms": /\b(plaintiff|defendant|appellant|appellee|damages|injunction|summary judgment)\b/gi
    };
