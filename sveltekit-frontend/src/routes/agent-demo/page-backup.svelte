...
<div class="agent-demo-container p-6 max-w-7xl mx-auto">
	<div class="header mb-8">
		...
		<p class="text-gray-600">
			RAG-powered autonomous coding with Gemma3 + TensorRT-LLM + pgvector
		</p>
	</div>
			...
			<CardContent>
				<div class="agent-status mb-4">
					<div class="flex justify-between items-center mb-2">
						...
						<span class="px-2 py-1 rounded text-xs {$agentStatus === 'working' ? 'text-blue-600 bg-blue-50' : 'text-green-600 bg-green-50'}">
							{$agentStatus}
						</span>
					</div>
				...
				<div class="log-container max-h-64 overflow-y-auto">
					{#each $logs as log (log.id)}
						<div class="log-entry flex items-start gap-2 py-1 text-sm">
							...
							<span class="flex-1 {log.type === 'error' ? 'text-red-600' : log.type === 'success' ? 'text-green-600' : log.type === 'warning' ? 'text-yellow-600' : 'text-gray-700'}">
								{log.message}
							</span>
						</div>
					{/each}
