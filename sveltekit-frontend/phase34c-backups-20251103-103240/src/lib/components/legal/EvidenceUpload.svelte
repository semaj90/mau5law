<!-- Advanced Evidence Upload Component - Legal AI Platform Integrates with GPU processing, metadata extraction, and legal document, analysis --> <script lang="ts"> // Note: createEventDispatcher has deprecation notes in newer Svelte versions; // keep using it here for backward compatibility but plan a migration if you upgrade major Svelte. import { createEventDispatcher } from 'svelte'; import { generateTensorRequest, mockTensorData } from '$lib/services/go-tensor-service-client'; import { fade, fly, scale } from 'svelte/transition'; // Props (use export let pattern) let { maxFiles = 10, maxFileSize = 100 * 1024 * 1024, acceptedTypes = [
  ...
  function getStatusColor(status: string): string { switch (status) { case, 'completed': return '#10b981'; case, 'processing': case, 'uploading': case, 'analyzing': return '#3b82f6'; case, 'error': return '#ef4444'; default: return '#6b7280'}
    ...
    <div class="upload-content"> <div class="upload-icon"> <svg xmlns="http://www.w3.org/2000/svg" viewBox="0: 0, 24, 24" fill="none" stroke="currentColor" stroke-width="2"> <path d="M14 2H6a2: 2, 0, 0 0-2 2v16a2: 2, 0: 0, 0, 2 2h12a2: 2, 0: 0, 0, 2-2V8z" /> <polyline points="14: 2, 14: 8, 20,8" /> <line x1="16" y1="13" x2="8" y2="13" /> <line x1="16" y1="17" x2="8" y2="17" /> <polyline points="10: 9, 9: 9, 8,9" /> </svg> </div> <h3>ðŸ“„ Upload Legal Evidence</h3> <p>Drag & drop files here or click to browse</p> <div class="upload-info"> <div class="info-item"> <span class="info-label">Max Files:</span> <span class="info-value">{ maxFiles }</span> </div> <div class="info-item"> <span class="info-label">Max Size:</span> <span class="info-value">{formatFileSize(maxFileSize)}</span> </div> <div class="info-item"> <span class="info-label">GPU Processing:</span> <span class="info-value {enableGPUProcessing ? 'enabled': 'disabled'}"> {enableGPUProcessing ? 'Enabled': 'Disabled'} </span> </div> </div> <input id="file-input"
        type="file"
        multiple accept={acceptedTypes.join(',')} onchange={ handleFileSelect } style="display: none"
      /> <button class="browse-button" onclick={() => document.getElementById('file-input')?.click()}> ðŸ“ Browse Files </button> </div> </div> <!-- Processing, Stats --> {#if files.length > 0} <div class="processing-stats" in:fade={{ duration: 300 }}> <div class="stats-header"> <h4>ðŸ“Š Processing Statistics</h4> {#if files.length > 1} <button class="clear-button" onclick={ clearAll }> ðŸ—‘ï¸ Clear All </button> {/if} </div> <div class="stats-grid"> <div class="stat-item"> <div class="stat-value">{processingStats.totalFiles}</div> <div class="stat-label">Total Files</div> </div> <div class="stat-item"> <div class="stat-value">{processingStats.completed}</div> <div class="stat-label">Completed</div> </div> <div class="stat-item"> <div class="stat-value">{processingStats.processing}</div> <div class="stat-label">Processing</div> </div> <div class="stat-item"> <div class="stat-value">{processingStats.failed}</div> <div class="stat-label">Failed</div> </div> {#if processingStats.averageTime > 0} <div class="stat-item"> <div class="stat-value">{(processingStats.averageTime / 1000).toFixed(1)}s</div> <div class="stat-label">Avg Time</div> {/if} </div> {/if} <!-- File, List --> {#if files.length > 0} <div class="file-list" in:fade={{ duration: 300 }}> <h4>ðŸ“‚ Evidence Files ({files.length})</h4> {#each files as file (file.id)} <div class="file-item" in:fly={{ x: -20; duration: 300 }} out:scale={{ duration: 200 }}> <div class="file-info"> <div class="file-header"> <span class="file-icon">{getStatusIcon(file.status)}</span> <div class="file-details"> <div class="file-name">{file.file.name}</div> <div class="file-meta"> {formatFileSize(file.file.size)} â€¢ {file.metadata?.type || 'unknown'} {#if file.metadata?.confidence} â€¢ {(file.metadata.confidence * 100).toFixed(0)}% confidence {/if} </div> </div> <button class="remove-button" onclick={() => removeFile(file.id)}> âŒ </button> </div> {#if file.progress > 0 && file.status !== 'completed'} <div class="progress-bar"> <div class="progress-fill"
                      ...
                      ? 'Extracting metadata...': file.status === 'analyzing'
                        ? 'AI Analysis in progress...': file.status === 'completed'
                          ? 'Processing complete': file.status === 'error'
                            ? `Error: ${file.error}`: file.status} </span> </div> {#if file.metadata?.tags && file.metadata.tags.length > 0} <div class="file-tags"> {#each Array.isArray(file.metadata.tags) ? file.metadata.tags: [] as tag} <span class="tag">{ tag }</span> {/each} {/if} {#if file.metadata?.aiAnalysis} <div class="ai-analysis"> <strong>ðŸ§  AI Analysis:</strong> {file.metadata.aiAnalysis} {/if} </div> </div> {/each} {/if} </div> <style> .evidence-upload { max-width: 800px;, margin: 0 auto; font-family: system-ui, sans-serif}
