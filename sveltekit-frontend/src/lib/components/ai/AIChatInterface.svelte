...
<script lang="ts">
	...
	// Props (exported)
	let { visible = false, minimized = false, draggable = true, width = 400, height = 600, apiEndpoint = 'http://localhost:11434/api/generate', fallbackEndpoint = 'http://localhost:8000/v1/chat/completions', modelName = 'gemma3-legal:latest', title = 'YoRHa Legal AI', subtitle = 'Powered by Gemma3', onclose = undefined, onminimize = undefined, onmaximize = undefined, onmessage = undefined, onsettingschange = undefined } = $props<{
		visible?: boolean;
		minimized?: boolean;
		draggable?: boolean;
		width?: number;
	...
	// Debounced helpers
	const debouncedAutoResize = _debounce(autoResize, 300);
	// Initialize welcome message & initial position
	onMount(() => {
		addMessage('system', `Hello! I'm your YoRHa Legal AI Assistant powered by ${modelName}. How can I assist you today?`);
		position = {
	...
	async function sendMessage() {
		...
		isTyping = true;
		const typingMsg = addMessage('assistant', '', { streaming: true });
		try {
			let response = await callGemma3API(userText);
			if (!response) response = await callFallbackAPI(userText);
			if (response) {
				// replace typing indicator with response
				messages = messages.filter((m) => m.id !== typingMsg.id);
	...
	async function callGemma3API(message: string): Promise<string | null> {
		...
		try {
			...
			if (!resp.ok) throw new Error(`Primary API HTTP ${resp.status}`);
	...
	// Fallback API call (generic chat completions)
	async function callFallbackAPI(message: string): Promise<string | null> {
		try {
			...
			const resp = await fetch(fallbackEndpoint, {
				...
				// Use AbortSignal.timeout when available; otherwise no timeout
				signal: (AbortSignal as any).timeout ? (AbortSignal as any).timeout(60000) : undefined,
			});
			if (!resp.ok) throw new Error(`Fallback API HTTP ${resp.status}`);
			const data = await resp.json().catch(() => ({}));
			return (data.response || data.choices?.[0]?.message?.content || data.choices?.[0]?.text) ?? null;
		} catch (err) {
...
{#if visible}
  ...
  <nav class="flex items-center space-x-1" role="toolbar" aria-label="Chat window controls">
				<button type="button" class="w-8 h-8 flex items-center justify-center border border-yorha-border text-yorha-text-secondary hover:border-yorha-primary hover:text-yorha-primary focus:border-yorha-primary focus:outline-none focus:ring-2 focus:ring-yorha-primary/50 transition-colors" onclick={toggleSettings} aria-label={settingsOpen ? 'Close settings' : 'Open settings'} aria-expanded={settingsOpen}>
				...
				<button type="button" class="w-8 h-8 flex items-center justify-center border border-yorha-border text-yorha-text-secondary hover:border-yorha-primary hover:text-yorha-primary focus:border-yorha-primary focus:outline-none focus:ring-2 focus:ring-yorha-primary/50 transition-colors" onclick={minimizeWindow} aria-label={minimized ? 'Restore window' : 'Minimize window'}>
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
						{#if minimized}
				...
				<button type="button" class="w-8 h-8 flex items-center justify-center border border-yorha-border text-yorha-text-secondary hover:border-red-500 hover:text-red-500 focus:border-red-500 focus:outline-none focus:ring-2 focus:ring-red-500/50 transition-colors" onclick={closeWindow} aria-label="Close chat window">
					<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
		...
		{#if !minimized}
			{#if settingsOpen}
				<div class="border-b border-yorha-border bg-yorha-bg-primary p-4">
					<div class="space-y-3">
						<div>
							<label class="block text-xs text-yorha-text-secondary mb-1" for="model">Model</label>
							<select id="model" bind:value={settings.model} class="w-full bg-yorha-bg-tertiary border border-yorha-border text-yorha-text-primary text-xs p-2 focus:border-yorha-primary">
						...
						<div class="flex space-x-2">
							<button type="button" onclick={updateSettings} class="flex-1 bg-yorha-primary text-yorha-bg-primary text-xs p-2 hover:bg-yorha-secondary transition-colors">Apply</button>
							<button type="button" onclick={clearChat} class="flex-1 bg-yorha-error text-white text-xs p-2 hover:bg-red-600 transition-colors">Clear</button>
						</div>
					</div>
				{/if}
			<main bind:this={messagesContainer} class="flex-1 overflow-y-auto p-4 bg-yorha-bg-primary space-y-4" role="log" aria-live="polite" aria-label="Chat conversation">
				{#each messages as message (message.id)}
					<article class="flex" class:justify-end={message.role === 'user'} in:fly={{ y: 20, duration: 300 }}>
						<div class="max-w-[85%] border p-3 relative shadow-sm" class:bg-yorha-bg-tertiary={message.role === 'user'}>
							{#if message.role === 'assistant'}
								<div class="absolute left-0 top-0 bottom-0 w-1 bg-yorha-accent">{/if}
							...
							{#if message.error}
								<div class="mt-2 text-xs text-red-400" role="alert">
									Failed to get response. <button type="button" onclick={sendMessage} class="underline">Retry</button>
								{/if}
							<time class="mt-2 text-xs text-yorha-text-muted" datetime={message.timestamp.toISOString()}>{formatTime(message.timestamp)}</time>
						</div>
					</article>
				{/each}
				{#if isTyping}
					<div class="flex justify-start" in:fade role="status" aria-live="polite">
						<div class="bg-yorha-bg-secondary border p-3 relative shadow-sm">
			...
			<footer class="border-t border-yorha-border bg-yorha-bg-secondary p-4">
				<form class="flex space-x-3" on:submit|preventDefault={sendMessage} role="search" aria-label="Send message to AI">
					<textarea
						...
						required
					></textarea>
					<button type="submit" disabled={!inputValue.trim() || isTyping} class="w-10 h-10 bg-yorha-primary text-yorha-bg-primary flex items-center justify-center" aria-label={isTyping ? 'AI is responding' : 'Send message'}>
						<svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 19l9 2-9-18-9 18 9-2zm0 0v-8" /></svg>
					</button>
				</form>
