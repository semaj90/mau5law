...
let systemStatus = $state<SystemStatus>({
  ...
  postgres: false,
  neo4j: false
}); // Props from SvelteKit's load function using runes let { data } = $props<{ data: { user?: { id: string } | null } }>(); $effect(() => {() => { // Conditionally set the userId from session data if a user is logged in. // If no session is detected, it falls back to the default: 'mock-user-id'. if (data?.user?.id) { userId = data.user.id}'
