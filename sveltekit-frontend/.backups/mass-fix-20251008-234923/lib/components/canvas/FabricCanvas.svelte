<script lang="ts">
  ...
  // Props interface (fixed commas / formatting)
  interface Props {
    width?: number;
    height?: number;
    caseId?: string;
  ...
  // Fabric.js canvas instance & state (fixed generic spacing)
  ...
  // Helper: dynamic import of Fabric.js with common export shapes
  async function getFabric(): Promise<any> {
    const mod: any = await import('fabric');
    return mod.fabric ?? mod.default ?? mod;
  }

  // Initialize Fabric once via effect (guarded for SSR)
  $effect(() => {
    (async () => {
      try {
  ...
  function drawGrid(fabric: any) {
    ...
    for (let i = 0; i < Math.ceil(width / gridSize); i++) {
      const x = i * gridSize;
      lines.push(new fabric.Line([x, 0, x, height], { stroke: '#edf2f7', selectable: false, evented: false }));
    }
    for (let j = 0; j < Math.ceil(height / gridSize); j++) {
      const y = j * gridSize;
      lines.push(new fabric.Line([0, y, width, y], { stroke: '#edf2f7', selectable: false, evented: false }));
    }
  ...
  // Add evidence (image or document) to canvas
    ...
    try {
      ...
      if (evidence.type === 'image' && evidence.url) {
        ...
        if (!fabric || !fabric.Rect || !fabric.Text || !fabric.Group) {
          console.warn('Required Fabric constructors missing; skipping document:', evidence.id);
          return;
        }
  ...
  // Single file input handler (consolidated)
  ...
  // Upload evidence (MinIO first, fallback to demo)
  async function uploadEvidence(file: File) {
    ...
    try {
      ...
      const evidence: EvidenceItem = {
        id: uploadResult.id || uploadResult.fileId || fileId,
        type: file.type.startsWith('image/') ? 'image' : 'document',
        title: file.name,
        url: uploadResult.url,
        urlExpiry: uploadResult.bucket ? Date.now() + (23 * 60 * 60 * 1000) : undefined,
        x: Math.random() * Math.max(width - 200, 100) + 50,
        y: Math.random() * Math.max(height - 200, 100) + 50,
        metadata: {
          fileSize: file.size,
          fileType: file.type,
