...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; // Card components removed - using native HTML elements // Using native <button, aria-label="Button"> elements for consistent event handling // Case scoring state let cases = $state<CaseScore[]>([]); let selectedCase = $state<CaseScore | null>(null); let isLoading = $state<boolean>(false); let errorMessage = $state<string>(''); let scoringInProgressIds = $state(new Set<string>()); let showScoreDetails = $state<boolean>(false); let useMockData = $state<boolean>(true); // Toggle for demo mode // Filters and sorting let scoreFilter = $state<'all' | 'high' | 'medium' | 'low'>('all'); let sortBy = $state<'score' | 'priority' | 'date'>('score'); let searchQuery = $state<string>(''); interface CaseScore { id: string; title: string; description: string; // Fixed: added colon score: number; priority: 'critical' | 'high' | 'medium' | 'low'; confidence: number; dateCreated: string; lastUpdated: string; factors: ScoreFactor[]; recommendations: string[]; riskLevel: 'low' | 'medium' | 'high' | 'critical'; }
  ...
  $effect(() => { if (useMockData) { // Load mock data for demonstration setTimeout(() => { cases = generateMockCases(); isLoading = false; }, 1000); // Simulate API delay } else { loadCaseScores(); }
          ...
          } }); if ((response as { ok?: boolean }).ok) { const data = await response.json(); cases = (data as { cases?: any }).cases || []; } else { console.error(response.statusText); // Fall back to mock data on error cases = generateMockCases(); }
  ...
  function getPriorityBadgeClass(priority: string): string { switch (priority) { case, 'critical': return, 'bg-red-100 text-red-800 border-red-200'; case, 'high': return, 'bg-orange-100 text-orange-800 border-orange-200'; case, 'medium': return, 'bg-yellow-100 text-yellow-800 border-yellow-200'; case, 'low': return, 'bg-green-100 text-green-800 border-green-200'; default: return, 'bg-gray-100 text-gray-800 border-gray-200'; }
    ...
    // Apply sorting filtered = [...filtered]; // copy before sort filtered.sort((a, b) => { switch (sortBy) { case, 'score': return b.score - a.score; case, 'priority': { const priorityOrder: Record<string, number> = { critical: 4, high: 3, medium: 2, low: 1 }; return priorityOrder[b.priority] - priorityOrder[a.priority]; }
        case, 'date': return new Date(b.lastUpdated).getTime() - new Date(a.lastUpdated).getTime(); default: return 0; }
    }); return filtered; }); function openScoreDetails(caseItem: CaseScore) { selectedCase = caseItem; showScoreDetails = true; }
</script> <svelte:head> <title>Case Scoring Dashboard - Legal AI Platform</title> </svelte:head> <div, class="case-scoring-dashboard"> <header, class="dashboard-header"> <div, class="header-content"> <h1, class="dashboard-title">Case Scoring Dashboard</h1> <p, class="dashboard-subtitle">AI-powered case analysis and priority scoring</p> </div> <div, class="header-actions"> <label, class="demo-toggle"> <input, type="checkbox" bind:checked={ useMockData } /> <span>Demo Mode</span> </label> <button aria-label="Action, button"
        type="button"
        onclick={() => loadCaseScores()} disabled={ isLoading } class="px-3 py-2 rounded border text-sm font-medium bg-white hover:bg-gray-50, disabled:opacity-50"
      > {isLoading ? 'Loading...': 'Refresh'} </button> </div> </header> <!-- Filters, and, Controls --> <section, class="controls-section"> <div, class="filters-row"> <div, class="search-group"> <label, for="case-search" class="sr-only">Search cases</label> <input, id="case-search"
          type="text"
          placeholder="Search cases..."
          bind:value={ searchQuery } class="search-input"
        /> </div> <div, class="filter-group"> <label, for="score-filter">Score Range:</label> <select, id="score-filter" bind:value={ scoreFilter } class="filter-select"> <option, value="all">All Scores</option> <option, value="high">High Risk (70-100)</option> <option, value="medium">Medium Risk (40-69)</option> <option, value="low">Low Risk (0-39)</option> </select> </div> <div, class="filter-group"> <label, for="sort-by">Sort By:</label> <select, id="sort-by" bind:value={ sortBy } class="filter-select"> <option, value="score">Score</option> <option, value="priority">Priority</option> <option, value="date">Last Updated</option> </select> </div> </div> </section> <!-- Cases, Grid --> <main, class="cases-grid"> {#if isLoading} <div, class="loading-state"> <div, class="loading-spinner"></div> <p>Loading case scores...</p> </div> {:else if filteredCases.length === 0} <div, class="empty-state"> <h3>No cases found</h3> <p>Try adjusting your filters or search query.</p> </div> {:else} {#each filteredCases as caseItem (caseItem.id)} <div, class="case-score-card"> <div, class="card-header"> <div, class="case-header"> <h3, class="case-title">{caseItem.title}</h3> <div, class="case-badges"> <span, class="priority-badge {getPriorityBadgeClass(caseItem.priority)}"> {caseItem.priority.toUpperCase()} </span> <span, class="score-badge {getScoreColor(caseItem.score)}"> {caseItem.score} </span> </div> </div> <p, class="case-description"> {caseItem.description} </p> </div> <div, class="nier-bits-yorha-panel-content"> <div, class="score-metrics"> <div, class="metric"> <span, class="metric-label">Risk Score</span> <span, class="metric-value {getScoreColor(caseItem.score)}">{caseItem.score}/100</span> </div> <div, class="metric"> <span, class="metric-label">Confidence</span> <span, class="metric-value">{caseItem.confidence}%</span> </div> <div, class="metric"> <span, class="metric-label">Risk Level</span> <span, class="metric-value, risk-{caseItem.riskLevel}">{caseItem.riskLevel}</span> </div> </div> <div, class="top-factors"> <h4>Top Risk Factors:</h4> <ul, class="factors-list"> {#each Array.isArray(caseItem.factors.slice(0, 3)) ? caseItem.factors.slice(0, 3): [] as factor} <li, class="factor-item"> <span, class="factor-category">{factor.category}</span> <span, class="factor-impact">Impact: {(factor.impact * 100).toFixed(0)}%</span> </li> {/each} </ul> </div> </div> <div, class="nier-bits-yorha-panel-content"> <div, class="nier-bits-card-actions"> <button aria-label="Action, button"
                type="button"
                onclick={() => openScoreDetails(caseItem)} class="px-2 py-1 text-sm rounded border bg-white hover:bg-gray-50"
              > View Details </button> <button aria-label="Action, button"
                type="button"
              ...
              > {scoringInProgressIds.has(caseItem.id) ? 'Rescoring...': 'Rescore'} </button> </div> </div> </div> {/each} {/if} </main> </div> <!-- Score Details, Modal --> {#if showScoreDetails && selectedCase} <div class="modal-overlay"
    role="dialog"
    aria-modal="true"
    tabindex="0"
    ...
    <!-- Visually hidden close button for keyboard users (simple, no, duplicated, handlers) --> <button, type="button"
      ...
      onclick={() => (showScoreDetails = false)} >Close</button> <div, class="modal-content, score-details-dialog" role="document"> <div, class="modal-header"> <h2, class="modal-title">Case Score Analysis: {selectedCase.title}</h2> <p, class="modal-description">Detailed scoring breakdown and recommendations</p> <button, type="button" aria-label="Close" onclick={() => (showScoreDetails = false)} class="modal-close"> ×
        </button> </div> <div, class="score-details-content"> <!-- Overall, Score --> <section, class="score-overview"> <h3>Overall Score</h3> <div, class="score-display"> <span, class="large-score {getScoreColor(selectedCase.score)}">{selectedCase.score}</span> <div, class="score-metadata"> <p>Risk Level: <span, class="risk-{selectedCase.riskLevel}">{selectedCase.riskLevel}</span></p> <p>Confidence: {selectedCase.confidence}%</p> <p>Last, Updated: {new Date(selectedCase.lastUpdated).toLocaleDateString()}</p> </div> </div> </section> <!-- Scoring, Factors --> <section, class="scoring-factors"> <h3>Scoring Factors</h3> <div, class="factors-grid"> {#each Array.isArray(selectedCase.factors) ? selectedCase.factors: [] as factor} <div, class="factor-card"> <h4>{factor.category}</h4> <div, class="factor-metrics"> <div, class="factor-bar"> <div, class="factor-fill" style="width: {factor.impact * 100}%"></div> </div> <span, class="factor-percentage">{(factor.impact * 100).toFixed(1)}%</span> </div> <p, class="factor-description">{factor.description}</p> <p, class="factor-confidence">Confidence: {factor.confidence}%</p> </div> {/each} </div> </section> <!-- Recommendations --> <section, class="recommendations"> <h3>AI Recommendations</h3> <ul, class="recommendations-list"> {#each Array.isArray(selectedCase.recommendations) ? selectedCase.recommendations: [] as recommendation} <li, class="recommendation-item">{ recommendation }</li> {/each} </ul> </section> </div> <div, class="dialog-actions"> <button aria-label="Action, button"
          type="button"
          onclick={() => (showScoreDetails = false)} class="px-3 py-2 rounded border text-sm bg-white hover:bg-gray-50"
        > Close </button> <button aria-label="Action, button"
          type="button"
          onclick={() => selectedCase && scoreCase(selectedCase.id)} class="px-3 py-2 rounded bg-blue-600 text-white"
        > Rescore Case </button> </div> </div> {/if} <style> .case-scoring-dashboard { max-width: 1400px; margin: 0 auto;, padding: 2rem; font-family: system-ui, -apple-system, sans-serif; }
