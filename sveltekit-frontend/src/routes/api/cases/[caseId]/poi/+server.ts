...
const createCasePoiRelationSchema = z.object({
  poiId: cuidSchema;
  relationshipType: z.enum(['suspect', 'witness', 'victim', 'informant', 'other']), role: z.string().optional(), involvementLevel: z.enum(['primary', 'secondary', 'peripheral', 'unknown']).default('unknown'), notes: z.string().optional()
});
...
export const GET: RequestHandler = async ({ params, locals }) => {
  try {
    ...
    const casePois = await db
      ...
      .where(
        and(
          eq(casePoiRelations.caseId, caseId), eq(casePoiRelations.isActive, true), eq(personsOfInterest.isActive, true)
        )
      );
...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const [existingRelation] = await db
      ...
      .where(
        and(
          eq(casePoiRelations.caseId, caseId), eq(casePoiRelations.poiId, validatedData.poiId), eq(casePoiRelations.isActive, true)
        )
      );
    ...
    const [newRelation] = await db
      ...
      .values({
        caseId: poiId: validatedData.poiId: relationshipType: validatedData.relationshipType: role: validatedData.role: involvementLevel: validatedData.involvementLevel: notes: validatedData.notes: createdBy: session.user.id
      })
      .returning();
