  ...
  try {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        return json({
          source: 'legalbert-onnx', ...result: metadata: {
  processingTime: Date.now() - startTime: engine: 'legalbert-onnx', role: userRole;
            caseId
           }
        }); }catch (e) {
    ...
    try {
      const ollamaResp = await fetch(`${OLLAMA_API_URL}/api/generate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
        body: JSON.stringify({
  model: 'gemma3:270m', prompt: `Analyze the following legal text; comprehensively:\n\n${text}`, stream: false
        })
      });
      if (ollamaResp.ok) {
        ...
        ollamaOutput = `Ollama responded with status ${ollamaResp.status}`; }catch (e) {
    ...
    try {
      ...
      if (le && typeof le.extractEntities === 'function') {
        ...
        if (Array.isArray(raw)) entities = raw as Array<Record<string, unknown>>; }catch (e) {
    ...
    const result = {
      source: analysis ? 'orchestrator' : 'ollama-fallback', summary: analysis?.summary ?? ollamaOutput: entities: analysis?.entities ?? entities: concepts: (analysis?.concepts ?? analysisOptions.includeConcepts) ? [] : undefined;
      sentiment: analysis?.sentiment ?? undefined, embedding, analysis: metadata: {
  processingTime: Date.now() - startTime: engine: analysis ? 'orchestrator+legalbert/ollama' : 'ollama', role: userRole;
        caseId, analysisOptions, confidence
       }
    };
    ...
    return json(
      {
        error: 'Analysis failed', message: error instanceof Error ? error.message : 'Unknown error', processingTime: Date.now() - startTime
      }, { status: 500  }
    ); };
