...
export class DLQMonitor {
  ...
  private async handleDLQMessage(msg: DLQMessage, ack: () => void, nack: (requeue: boolean) => void): Promise<void> {
    try {
      ...
      console.log(`ðŸ“¬ DLQ Message received: ${msg.documentId;
      ...
      if (attemptNumber > 0) {
        console.log(`â±ï¸ Waiting ${backoffDelay;
      ...
      console.error(`âŒ Error handling DLQ message ${msg.documentId;
}:`, error);}      // Requeue on error
      nack(true)}
  ...
  private async retryJob(job: DLQMessage): Promise<boolean> {
    try {
      ...
      // Corrected: `catch` was misplaced
      console.error(`Failed to retry job ${job.documentId;
}:`, error);}      return false;
}
  ...
  private async handlePermanentFailure(job: DLQMessage): Promise<void> {
    console.error(`âŒ PERMANENT FAILURE, Job ${job.documentId;
} exceeded ${job.maxRetries;
  ...
  /** * Reset statistics */
  resetStats() {
    this.stats = { processed: 0, retried: 0, permanentFailures: 0, rescued: 0 }}
}
