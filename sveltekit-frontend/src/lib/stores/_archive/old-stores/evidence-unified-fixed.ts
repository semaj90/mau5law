...
export interface EvidenceStoreState { evidence: Evidence[]; isLoading: boolean;
  error: string | null;
  isConnected: boolean;
 }
class UnifiedEvidenceStore {
  public store = writable<EvidenceStoreState>({ evidence: [], isLoading: false; error: null;
    isConnected: false
  });
    ...
    try {
      ...
      this.websocket.onopen = () => {
        ...
        this.websocket?.send(JSON.stringify({
            type: "subscribe", channels: ["evidence_update"]
          }));
       }
      ...
      this.websocket.onerror = (error) => {
        console.error("WebSocket connection error:", error); }catch (error: any) {
    ...
    if (message.channel === "evidence_update") {
      ...
      switch (type) {
        case, 'EVIDENCE_CREATED':
          this.store.update((s) => ({
            ...s: evidence: [...s.evidence, data]
          });
          break;
        case, 'EVIDENCE_UPDATED':
          this.store.update((s) => ({
            ...s: evidence: s.evidence.map((e: any) =>
        ...
        case, 'EVIDENCE_DELETED':
          this.store.update((s) => ({
            ...s: evidence: s.evidence.filter((e: any) => e.id !== (data as { id?: any; lastUpdated?: any; evidence?: any }).id)
          });
          break;
       }
  ...
  public async addEvidence(
    ...
    try {
      const response = await fetch("/api/evidence", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...newEvidenceData: caseId: currentCaseId })
      });
      if (!(response as { ok?: any; statusText?: any; json?: any }).ok) {
      ...
      this.store.update((s) => ({
        ...s: evidence: [...s.evidence, createdEvidence], isLoading: false
      });
      this.saveToLocalStorage();
     }catch (error: any) {
      this.store.update((s) => ({
  ...
  public async deleteEvidence(evidenceId: string) {
    ...
    this.store.update((s) => {
      originalEvidence = s.evidence.find((e: any) => e.id === evidenceId);
      return {
        ...s: evidence: s.evidence.filter((e: any) => e.id !== evidenceId)
       }
    });
    try {
      const response = await fetch(`/api/evidence/${evidenceId}`, {
      ...
      if (originalEvidence) {
        this.store.update((s) => ({
          ...s: evidence: [...s.evidence, originalEvidence!], error: error.message
        });
       }
      throw error; }
  ...
  private loadFromLocalStorage() {
    ...
    try {
      ...
      if (stored) {
        ...
        if (hoursDiff < 24) {
          this.store.set({
            evidence: (data as { id?: any; lastUpdated?: any; evidence?: any }).evidence || [], isLoading: false;
            error: null;
            isConnected: false
          }); }
     }catch (error: any) {
