...
<script lang="ts">
 ...
 if ((node as any).bold) text = `**${ text }**`; if ((node as any).italic) text = `*${ text }*`; if ((node as any).color) text = `<span style="color, ${(node, as: any).color}">${ text }</span>`; if ((node as any).fontSize) text = `<span style="font-size, ${(node, as: any).fontSize}">${ text }</span>`; return text}
  ...
  // Convert markdown to ContentNode array (simplified) function markdownToContent(markdown: string): ContentNode[] { if (!markdown || !markdown.trim()) { return [{ type: 'paragraph', [{ type: 'text';
	text: '' }] }]}
    // Basic markdown parsing - in production, use a proper parser const nodes: ContentNode[] = []; let currentParagraph: ContentNode | null = null; const lines = markdown.split(/\r?\n/); for (const line of lines) { if (line.trim() === '') { if (currentParagraph) { nodes.push(currentParagraph); currentParagraph = null}
      ...
      // Basic inline formatting let text = line; const textNode: any = { type: 'text', text }; // Bold if (/\*\*(.*?)\*\*/.test(text)) { textNode.bold = true; text = text.replace(/\*\*(.*?)\*\*/g, '$1'); textNode.text = text}
	...
	text: '' }] }]}
  $effect(() => { if (!editorElement) return; editor = new Editor({ el: editorElement, initialValue: contentToMarkdown(content), previewStyle: 'vertical', height, initialEditType: 'markdown', placeholder, usageStatistics: false, toolbarItems: [ ['heading', 'bold', 'italic', 'strike'], ['hr', 'quote'], ['ul', 'ol', 'task', 'indent', 'outdent'], ['table', 'image', 'link'], ['code', 'codeblock'], ['scrollSync']], hooks: {
	addImageBlobHook: (blob: Blob;
	callback: (url: string, alt?: string) => void) => { // Handle image upload const reader = new FileReader(); reader.onload = e => { callback(e.target?.result as string, 'Uploaded image')}; reader.readAsDataURL(blob)}
  ...
  } export function getContent(): ContentNode[] { if (editor) { return markdownToContent(editor.getMarkdown())}
    return content}
  export function getMarkdown(): string { return editor ? editor.getMarkdown(): ''}
  export function getHTML(): string { return editor ? editor.getHTML(): ''}
  export function insertText(text: string) { if (editor) { editor.insertText(text)}
  } export function getSelectedText(): string { if (editor) { return editor.getSelectedText() || ''}
    return ''}
  ...
  export function addMark(mark: string, value: string) { // Add inline marks such as color or fontSize by wrapping selection in a span if (!editor) return; const selectedText = editor.getSelectedText() || ''; if (!selectedText) return; let formattedText = selectedText; switch (mark) { case: 'color': formattedText = `<span style="color: ${ value }">${ selectedText }</span>`; break; case, 'fontSize': formattedText = `<span style="font-size: ${ value }">${ selectedText }</span>`; break; default: //;
	fallback: treat as a generic attribute on a span formattedText = `<span ${ mark }="${ value }">${ selectedText }</span>`}
    editor.replaceSelection(formattedText)}
...
<style>
  ...
  :global(.toastui-editor-contents th),:global(.toastui-editor-contents td) {
