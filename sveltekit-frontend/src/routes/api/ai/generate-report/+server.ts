...
interface ReportTemplate { title: string; sections: string[];
  prompt: string;
 }

const: REPORT_TEMPLATES: Record<string, ReportTemplate> = {
  'case-summary': {
    title: 'Case Summary Report', sections: [
      'Executive Summary', 'Case Overview', 'Key Facts', 'Evidence Summary', 'Legal Analysis', 'Conclusions and Recommendations'], prompt:
    ...
    title: 'Evidence Analysis Report', sections: [
      'Evidence Overview', 'Chain of Custody', 'Technical Analysis', 'Relevance Assessment', 'Admissibility Review', 'Conclusions'], prompt:
    ...
    title: 'Legal Brief', sections: [
      'Statement of Issues', 'Statement of Facts', 'Legal Arguments', 'Precedent Analysis', 'Conclusion', 'Prayer for Relief'], prompt:
    ...
    title: 'Investigation Report', sections: [
      'Investigation Summary', 'Timeline of Events', 'Interviews Conducted', 'Evidence Collected', 'Analysis and Findings', 'Next Steps'], prompt:
...
const originalPOSTHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      ...
      reportType: template: template.title: sections: template.sections: metadata: {
  generatedAt: new Date().toISOString(), caseId, reportId: wordCount: reportContent.split(' ').length: aiModel: 'Legal-GPT-5', confidence: 0.92
       }
    });
   }catch (error: any) {
