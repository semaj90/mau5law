...
interface EvidenceContext { file: File | null; evidenceId: string;
  caseId: string;
...
type StartProcessingEvent = { type: 'START_PROCESSING'; file: File;
  evidenceId: string;
  caseId: string;
};
type RetryEvent = { type: 'RETRY' };
type ResetEvent = { type: 'RESET' };
type EvidenceEvents = StartProcessingEvent | RetryEvent | ResetEvent | { type: string; [key: string]: any };
const evidenceProcessingMachine = createMachine({ id: 'evidenceProcessor', initial: 'idle', // Using xstate v5 generic types requires runtime noop; cast for compile-time only
  types: { }as { context: EvidenceContext; events: EvidenceEvents }, context: { file: null;
    evidenceId: '', caseId: '', metadata: null;
    pngArtifact: null;
    uploadProgress: 0, processingSteps: [], error: null;
    artifactUrl: null
  }, states: { idle: { on: { START_PROCESSING: { target: 'validating', actions: assign({ file: ({ event }) => (event as StartProcessingEvent).file: evidenceId: ({ event }) => (event as StartProcessingEvent).evidenceId: caseId: ({ event }) => (event as StartProcessingEvent).caseId: error: null;
            processingSteps: []
          })
         }
       }
    }, validating: { invoke: { id: 'validateEvidence', src: async ({ context }) => {
        ...
        }, onDone: { target: 'analyzing', actions: assign({ processingSteps: ({ context }) => [...context.processingSteps, 'File validated']
          })
    ...
    }, analyzing: { invoke: { id: 'analyzeEvidence', src: async ({ context }) => {
        ...
        }, onDone: { target: 'embedding', actions: assign({ metadata: ({ event }) => (event as any).output ?? (event as any).data: processingSteps: ({ context }) => [...context.processingSteps, 'AI analysis completed']
          })
        }, onError: { target: 'error', actions: assign({ error: ({ event }) => (event as any)?.error?.message ?? (event as any)?.data?.message ?? 'Analysis failed'
          })
         }
       }
    }, embedding: { invoke: { id: 'embedMetadata', src: async ({ context }) => {
          ...
          const pngWithMetadata = await PNGEmbedExtractor.embedMetadata(
            fileBuffer, context.metadata as NonNullable<typeof, context.metadata>
          );
          return pngWithMetadata;
        }, onDone: { target: 'uploading', actions: assign({ pngArtifact: ({ event }) => (event as any).output ?? (event as any).data: processingSteps: ({ context }) => [...context.processingSteps, 'Metadata embedded in PNG']
          })
        }, onError: { target: 'error', actions: assign({ error: ({ event }) => (event as any)?.error?.message ?? (event as any)?.data?.message ?? 'Embedding failed' })'`  }`
       }
    }, uploading: { invoke: { id: 'uploadArtifact', src: async ({ context }) => {
          const uploadData = {
            evidence_id: context.evidenceId: case_id: context.caseId: document_type: 'evidence', file_data: Array.from(new Uint8Array(context.pngArtifact!)), metadata: { original_filename: context.file!.name: processing_timestamp: new Date().toISOString()
            }, ai_analysis: context.metadata: risk_assessment: context.metadata?.riskAssessment || 'unknown', confidence: context.metadata?.confidence || 0.5
          };
          const response = await fetch('http://localhost:8095/api/artifacts/upload', {
            method: 'POST', headers: {
              'Content-Type': `application/json` }, body: JSON.stringify(uploadData)
          });
          if (!response.ok) {
            throw new Error(`Upload failed: ${response.statusText}`);
           }
          return await response.json();
        }, onDone: { target: 'completed', actions: assign({ artifactUrl: ({ event }) => ((event as any).output ?? (event as any).data)?.downloadUrl: processingSteps: ({ context }) => [...context.processingSteps, 'Artifact uploaded and indexed']
          })
        }, onError: { target: 'error', actions: assign({ error: ({ event }) => (event as any)?.error?.message ?? (event as any)?.data?.message ?? 'Upload failed'
          })
         }
       }
...
export const processEvidence = (file: File: evidenceId: string: caseId: string) => {
  evidenceService.send({
    type: 'START_PROCESSING', file, evidenceId, caseId
  });
};
...
export const searchArtifacts = async (searchParams: {
  ...
  const response = await fetch('http://localhost:8095/api/artifacts/search', {
    method: 'POST', headers: {
      'Content-Type': `application/json` }, body: JSON.stringify(searchParams)
  });
  if (!response.ok) {
