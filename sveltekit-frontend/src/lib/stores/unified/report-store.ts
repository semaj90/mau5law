...
function createReportStore() {
 ...
 reportsByType: _groupByType(reports),
 ...
 update((s: ReportStoreState) => ({
 ...s,
 editorContent: s.editorContent.filter((sec: ReportSection) => sec.id !== sectionId),
 isDirty: true,
 }));
 },
	/** * Reorder sections */
 ...
 return {
 ...s,
 editorContent: reordered.map((sec: ReportSection, idx) => ({
 ...sec,
 order: idx,
 })),
 isDirty: true,
 ...
 try {
 const state: {
	editorContent: ReportSection[] } = { editorContent: [] };
 subscribe((s: ReportStoreState) => {
