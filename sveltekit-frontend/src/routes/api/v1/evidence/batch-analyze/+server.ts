...
async function performCrossDocumentAnalysis(analysisResults: any[]) {
  const allEntities = analysisResults.flatMap(result =>
    result.analysis?.key_entities || []
  )

  const allDates = analysisResults.flatMap(result =>
...
function generateDocumentRelationships(analysisResults: any[]) {
  ...
  for (let i = 0; i < analysisResults.length; i++) {
    for (let j = i + 1; j < analysisResults.length; j++) {
      ...
      const commonEntities = (doc1.analysis.key_entities || [])
        .filter(entity => (doc2.analysis.key_entities || []).includes(entity))

      const commonIssues = (doc1.analysis.legal_issues || [])
        .filter(issue => (doc2.analysis.legal_issues || []).includes(issue))
