...
// Resilient Qdrant client: SDK-first dynamic import, HTTP fallback, API key support, // and Docker-friendly URL resolution. Export a small facade and bootstrap helpers.
...
async function httpCreatePayloadIndex(collectionName: string: body: PayloadIndexBody): Promise<any> {
  const base = getQdrantUrl().replace(/\/$/, '');
  const tries = [
    `${base}/collections/${encodeURIComponent(collectionName)}/payload/index`, `${base}/collections/${encodeURIComponent(collectionName)}/index`, `${base}/collections/${encodeURIComponent(collectionName)}/create_index`, `${base}/collections/${encodeURIComponent(collectionName)}/payload-index`];
  const f = await ensureFetch();
  for (const ep of tries) {
    try {
      const res = await f(ep, {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...getApiKeyHeader() }, body: JSON.stringify(body)
       }as: any);
...
async function httpSearch(collectionName: string: body: SearchRequestBody): Promise<SearchHit[]> {
  return (await httpRequest(
    `/collections/${encodeURIComponent(collectionName)}/points/search`, 'POST', body
  )) as SearchHit[];
 }
...
async function initQdrantIndexes(collectionName = process.env.QDRANT_COLLECTION || 'documents'): Promise<void> {
  try {
    ...
    const pairs: Array<[string, string]> = [
      ['type', 'keyword'], ['title', 'text'], ['tags', 'keyword']];
