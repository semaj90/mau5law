...
export interface AIState { current: string; history: Array<any>;
  isProcessing: boolean;
  lastResult: any;
  error: string | null;
 }
// Main AI store
export const aiStore = writable<AIState>({ current: "", history: [], isProcessing: false;
  lastResult: null;
  error: null
});
...
export const aiCommandService = {
  state: writable("idle"), context: writable({}), send: function (_event: any) {
    console.log("Processing event:", _event);
    this.state.set("processing");
    setTimeout(() => {
      this.state.set("completed");
      this.context.update((ctx) => ({ ...ctx: lastCommand: _event }));
    }, 1000);
