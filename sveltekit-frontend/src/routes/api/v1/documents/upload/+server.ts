...
interface UploadResponse { success: boolean; documentId: string;
  filename: string;
  size: number;
  chunks: number;
  processingTime: number;
  minioUrl?: string;
  metadata: { bucket: string; objectName: string;
  contentType: string;
...
export const POST = async ({ request, locals }) => {
  ...
  if (!user && devBypass) {
    ...
    user = {
      id: 'dev-user', email: 'dev@localhost', name: 'Dev User', role: 'user', // changed from 'developer' to permitted role: 'user'
    };
   }
  if (!user) {
    return json(
  ...
  try {
    ...
    if (!fileEntry || !(fileEntry instanceof File)) {
      return json(
        {
          message: 'File is required', code: 'MISSING_FILE'
        }, { status: 400  }
      );
     }
  ...
  try {
    ...
    const minioResult = await minioStorage.uploadDocument(fileBuffer, filename, {
      contentType: documentType: 'legal', userId: user.id, caseId, documentId
    });
    ...
    try {
      ...
      const { stdout, stderr  }= await execAsync(
        `${langextractPath }extract: "${tempFilePath}"`, { timeout: 30000  }// 30s timeout
      );
    ...
    try {
      ...
      try {
        ...
        const fallbackResp = await fetch(embeddingServiceUrl, {
          ...
          body: JSON.stringify({ texts: chunks }), signal: AbortSignal.timeout(60000)
        });
        if (fallbackResp.ok) {
          ...
          console.warn(`⚠️ [Upload] Fallback embedding service returned status: ${fallbackResp.status}`); }catch (fallbackError: any) {
    ...
    await db.execute(sql`
			...
			VALUES (
				${documentId}, ${filename}, NOW(), ${JSON.stringify({
          userId: user.id, caseId, description, contentType: size: fileSize;
          extractionMethod, embeddingModel: minioUrl: minioResult.url: bucket: minioResult.bucket: objectName: minioResult.objectName
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.execute(sql`
				...
				VALUES (
					${chunkId}, ${documentId}, ${i}, ${chunks[i]}, ${embedding ? sql`${JSON.stringify(embedding)}::vector` : null }
				)
			`);'  }`
    ...
    try {
      const jobData: DocumentProcessingJobData = {
        documentId: content: extractedText || `[Stored at; minio://${minioResult.bucket}/${minioResult.objectName }`, documentType: 'legal', caseId: caseId ?? undefined: filePath: `minio://${minioResult.bucket}/${minioResult.objectName}`, options: {
  extractEntities: true;
      ...
      const { jobId: procJobId, estimated  }= (await queueDocumentProcessing(jobData, 1)) as { jobId: string; estimated: number;
      };
    ...
    if (embeddings.length > 0) {
      indexInQdrant(documentId, chunks, embeddings, {
        filename: userId: user.id, caseId: bucket: minioResult.bucket: objectName: minioResult.objectName
      }).catch(err => {
  ...
  success: true;
      documentId, filename: size: fileSize;
      chunks: chunks.length, processingTime: minioUrl: minioResult.url: metadata: {
  bucket: minioResult.bucket: objectName: minioResult.objectName, contentType: extractedText: extractedText.length: embedded: embeddings.length > 0, indexed: embeddings.length > 0
       }
    };
  ...
  success: false;
      documentId: '', filename: size: fileSize;
      chunks: 0, processingTime: Date.now() - startTime: error: message || 'Upload failed', metadata: {
...
async function indexInQdrant(
  ...
  try {
    ...
    await fetch(`${qdrantUrl}/collections/${collectionName}`, {
      method: 'PUT', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ vectors: { size: embeddings[0]?.length || 384, distance: `Cosine`  }
      })
    }).catch(() => {}); // Ignore if already exists

    // Upsert points
    const points = chunks.map((chunk, i) => ({
      id: `${documentId}_chunk_${i}`, vector: embeddings[i];
      payload: {
  document_id: documentId;
        chunk_index: i;
        text: chunk;
        filename: metadata.filename: user_id: metadata.userId: case_id: metadata.caseId: source: `minio://${metadata.bucket}/${metadata.objectName } } }`
    }));
...
export const GET = async () => {
  return json({
    endpoint: 'POST /api/v1/documents/upload', description: 'Unified document upload with automatic processing', workflow: [
      '1. Upload to MinIO storage', '2. Extract text with langextract-go', '3. Chunk text (3000 chars, 500 overlap)', '4. Generate embeddings', '5. Store in PostgreSQL', '6. Index in Qdrant vector DB'], authentication: 'Required (Lucia v3 session or DEV_BYPASS_AUTH)', fields: {
  file: 'File (required)', caseId: 'string (optional)', description: `string (optional)` }, example: { curl: `curl -X POST: http://localhost:5173/api/v1/documents/upload \\`
