...
export class FlashAttentionGPUErrorProcessor {
    ...
    private createErrorBatch(batchId: string, errors: TypeScriptError[]): GPUErrorBatch {
        const priority = errors.length > 1000 ? 'critical' : errors.length > 500 ? 'high' : errors.length > 100 ? 'medium' : 'low';
        return {
            id: batchId,
            errors,
            priority,
    ...
    private async generateErrorFix(error: TypeScriptError, _batchId: string): Promise<ErrorFix | null> {
        ...
        const modelText: string = (resultBody && (resultBody.response || resultBody.output || resultBody.text)) ?? JSON.stringify(resultBody);
...
Provide ONLY the corrected code snippet that fixes this specific error. Do not include explanations or markdown formatting.`;
    ...
    private calculateConfidence(category: string, responseLength: number): number {
        const baseConfidence = { svelte5: 0.9, import: 0.8, type: 0.7, syntax: 0.9, binding: 0.8, unknown: 0.5 };
    ...
    private parseTypeScriptOutput(output: string): TypeScriptError[] {
        ...
        return errorLines.map((line, index) => {
            // Match like: path/to/file.ts(12,34): TS2322:...
