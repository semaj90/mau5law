...
export class WebGPUSIMDAccelerator {
   ...
   */
  public async acceleratedParse(jsonString: string, mode: ParseMode = ParseMode.ULTRA_PERFORMANCE): Promise<AccelerationResult> {
   ...
   */
  public async acceleratedBatchParse(jsonStrings: string[], mode: ParseMode = ParseMode.ULTRA_PERFORMANCE): Promise<AccelerationResult[]> {
    const startTime = performance.now();
    ...
    const batchPromises = [
      batches.webgpu.length > 0 ? this.webgpuBatchProcess(batches.webgpu, mode) : Promise.resolve([] as AccelerationResult[]),
      batches.simd.length > 0 ? this.simdBatchProcess(batches.simd, mode) : Promise.resolve([] as AccelerationResult[]),
      batches.standard.length > 0 ? this.standardBatchProcess(batches.standard, mode) : Promise.resolve([] as AccelerationResult[])
    ];
