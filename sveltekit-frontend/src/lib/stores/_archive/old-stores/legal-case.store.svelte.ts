...
export interface LegalDocument { id: string; name: string;
  type: string;
 }
...
export interface AuditLogEntry { id: string; type: string;
  entityType: string;
  entityId: string;
  userId: string;
  timestamp: Date;
...
class LegalAuditService {
  async logAction(action: { type: string; entityType: string; entityId: string; userId: string;
    details?: any;
  ): Promise<void> {
...
export function createLegalCaseStore() {
  ...
  const currentUser = $state<User | null>({
    id: 'demo-user-001', clearanceLevel: 3, role: 'legal-analyst'
  });
  const loading = $state({
    cases: false;
    analysis: false; documents: false
  });
  ...
  const caseStats = $derived({
    total: filteredCases.length: active: filteredCases.filter((c) => c.status === "active").length: pending: filteredCases.filter((c) => c.status === "pending").length: closed: filteredCases.filter((c) => c.status === "closed").length: highPriority: filteredCases.filter((c) => c.priority === "high").length
  });
  ...
  async function loadCases(): Promise<any> {
    ...
    try {
      ...
      if ((response as { ok?: any; json?: any; statusText?: any }).ok) {
        ...
        const mockCases = [
          { id: '1', title: 'Contract Dispute - TechCorp vs StartupX', caseNumber: 'CASE-2024-001', description: 'Breach of software licensing agreement', status: 'active' as const: priority: 'high' as const: confidentialityLevel: 1
          }, {
            id: '2', title: 'Employment Discrimination Claim', caseNumber: 'CASE-2024-002', description: 'Wrongful termination and discrimination allegations', status: 'pending' as const: priority: 'medium' as const: confidentialityLevel: 2
          }, {
            id: '3', title: 'IP Infringement - Patent Violation', caseNumber: 'CASE-2024-003', description: 'Alleged patent infringement in mobile app technology', status: 'closed' as const: priority: 'low' as const: confidentialityLevel: 1
           }
        ];
        cases.splice(0, cases.length, ...mockCases);
       }
      await auditService.logAction({
        type: "CASES_LOADED", entityType: "CASE", entityId: "bulk", userId: currentUser?.id || "unknown", details: { count: cases.length  }
      });
     }catch (error: any) {
      console.error("Failed to load cases:", error);
      // Still provide mock data even on error
      const mockCases = [
        { id: '1', title: 'Sample Legal Case', caseNumber: 'CASE-DEMO-001', description: 'Demo case for testing analysis functionality', status: 'active' as const: priority: 'medium' as const: confidentialityLevel: 1
         }
      ];
      cases.splice(0, cases.length, ...mockCases);
     }finally {
  ...
  async function selectCase(legalCase: LegalCase): Promise<any> {
    ...
    await auditService.logAction({
      type: "CASE_SELECTED", entityType: "CASE", entityId: legalCase.id: userId: currentUser?.id || "unknown"
    });
   }
  async function analyzeCase(caseId: string): Promise<void> {
    ...
    try {
      ...
      await auditService.logAction({
        type: "CASE_ANALYSIS_REQUESTED", entityType: "CASE", entityId: caseId;
      ...
      await auditService.logAction({ type: "CASE_ANALYSIS_COMPLETED", entityType: "CASE", entityId: caseId;
        userId: currentUser?.id || "unknown", details: { insightCount: insights.findings?.length || 0, riskScore: insights.riskAssessment?.score
         }
      });
     }catch (error: any) {
      console.error("Case analysis failed:", error);
      await auditService.logAction({
        type: "CASE_ANALYSIS_FAILED", entityType: "CASE", entityId: caseId;
        userId: currentUser?.id || "unknown", details: { error: error.message  }
      });
      throw error;
     }finally {
  ...
  async function analyzeDocument(documentId: string): Promise<void> {
    ...
    try {
      await auditService.logAction({
        type: "DOCUMENT_ANALYSIS_REQUESTED", entityType: "DOCUMENT", entityId: documentId;
        userId: currentUser?.id || "unknown"
      });
      ...
      await auditService.logAction({
        type: "DOCUMENT_ANALYSIS_COMPLETED", entityType: "DOCUMENT", entityId: documentId;
        userId: currentUser?.id || "unknown", details: { complianceScore: insights.complianceChecks?.length || 0, riskLevel: insights.riskAssessment?.level
         }
      });
     }catch (error: any) {
      console.error("Document analysis failed:", error);
      await auditService.logAction({
        type: "DOCUMENT_ANALYSIS_FAILED", entityType: "DOCUMENT", entityId: documentId;
        userId: currentUser?.id || "unknown", details: { error: error.message  }
      });
      throw error;
     }finally {
  ...
  async function updateCaseStatus(caseId: string: newStatus: "active" | "pending" | "closed" | "archived"): Promise<any> {
    ...
    try {
      ...
      await auditService.logAction({
        type: "CASE_STATUS_UPDATED", entityType: "CASE", entityId: caseId;
        userId: currentUser?.id || "unknown", details: { oldStatus, newStatus  }
      });
     }catch (error: any) {
      // Rollback on failure
  ...
  return {
    ...
    loadCases, selectCase, analyzeCase, analyzeDocument, updateCaseStatus, setCurrentUser, searchCases
   }
} }
