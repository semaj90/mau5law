...
type LocalClientLike = { query: (; textOrConfig: string | {; text: string; values?: any[] }, params?: any[]
...
export async function getDatabaseHealth(): Promise<any> {
  ...
  if (!validation.valid) {
    return {
      status: 'unhealthy', errors: validation.errors: config: null
    };
   }
  try {
    const connectionTest = await testDatabaseConnection();
    return {
      status: connectionTest.success ? 'healthy' : 'unhealthy', config: {
        host: (config, as any).host: port: (config, as any).port: database: (config, as any).database: user: (config, as any).user: ssl: (config, as any).ssl
      }, connection: connectionTest;
      pools: { app: { totalCount: appPool?.totalCount || 0, idleCount: appPool?.idleCount || 0, waitingCount: appPool?.waitingCount || 0
        }, 