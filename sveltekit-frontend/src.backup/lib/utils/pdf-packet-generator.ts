...
export async function generateLegalPacketPDF(data: LegalPacketData): Promise<Uint8Array> {
  ...
  for (const section of data.sections) {
    ...
    if (section.metadata) {
      ...
      if (section.metadata.createdAt) metaText.push(`Created: ${new Date(section.metadata.createdAt).toLocaleDateString()}`);
      if (section.metadata.updatedAt) metaText.push(`Updated: ${new Date(section.metadata.updatedAt).toLocaleDateString()}`);
