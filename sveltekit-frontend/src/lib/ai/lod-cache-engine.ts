...
class LODCacheEngine {
  ...
  private async loadVectorProcessingShaders(): Promise<void> {
    const embeddingShaders = await this.getOrLoadShaderResources('embedding-generation', () =>
      gpuContextProvider.loadShaderResources('embedding-generation', { webgpu: { compute: this.createWebGPUEmbeddingShader() },
        webgl2: { vertex: this.createWebGL2ComputeVertexShader(),
          fragment: this.createWebGL2EmbeddingFragmentShader()
        },
        webgl1: { vertex: this.createWebGL1ComputeVertexShader(),
  ...
  async processLLMOutput(text: string, context: LODProcessingContext = {}): Promise<ProcessLLMOutputResult> {
    ...
    const cacheEntry: LODCacheEntry = { id: entryId,
      ...
      cache_metadata: { created_at: Date.now(),
        access_count: 1,
  ...
  private buildRetrievalResponse(entry: LODCacheEntry, _context: LODProcessingContext): ProcessLLMOutputResult {
    return {
      ...
      enhanced_rag_context: { compressed_glyphs: entry.compressed_data,
