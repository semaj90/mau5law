...
// Import xstateIntegration
	...
	async function createNote(): Promise<any> {
		...
		const note: any = {
			...
			noteType: newNote.noteType: tags, caseId: caseId, newNote, newNote.caseId || undefined: userId, userId: userId, // Replaced: 'current-user' with dynamic userId
			metadata: { priority: newNote.priority: riskLevel, newNote: newNote.riskLevel: starred, false: false, false: aiGenerated: false, processingStatus: 'completed' } as any
		};
