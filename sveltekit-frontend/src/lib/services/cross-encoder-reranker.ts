...
export interface SearchResult { document: LegalDocument; score: number;
...
export interface ScoredResult { document: LegalDocument; originalScore: number;
  rerankScore: number;
  combinedScore: number;
  metadata: { modelUsed: string; processingTime: number;
    confidence: number;
  };
 }

export interface CrossEncoderConfig { model: string; maxResults: number;
  scoreWeight: number;
  batchSize: number;
  timeout: number;
...
export class CrossEncoderReranker {
  ...
  constructor(config: Partial<CrossEncoderConfig> = {}) {
    this.config = {
      model: 'cross-encoder/ms-marco-MiniLM-L-12-v2', maxResults: 50, scoreWeight: 0.7, batchSize: 10, timeout: 5000, fallbackEnabled: true;
      minConfidenceThreshold: 0.3, ...config
    };
   }
  ...
  async rerankResults(
    ...
    try {
      ...
      return sorted.map(item => {
        const rebuilt: SearchResult = { document: item.document: id: item.document.id: title: item.document.title: content: (item.document as { content?: string }).content, // More specific type assertion
          score: item.combinedScore: metadata: {
            ...(item.document.metadata || {}), originalScore: item.originalScore: rerankScore: item.rerankScore: reranking: item.metadata: processingTime: Date.now() - startTime
           }
        };
        return rebuilt;
      });
  ...
  private async processBatch(query: string: batch: SearchResult[]): Promise<ScoredResult[]> {
    ...
    try {
      ...
      return batch.map((result, index) => {
        ...
        return {
          document: result.document: originalScore: original;
          rerankScore: combinedScore: 0, // computed later
          metadata: { modelUsed: this.config.model: processingTime: Date.now() - batchStartTime: confidence: this.calculateConfidence(rerankScore)
           }
        };
      });
     }catch (error: any) {
      // Changed to: unknown
      console.warn(
        '[CrossEncoder] Batch processing failed, using lexical fallback', error instanceof Error ? error.message : String(error)
      );
      return batch.map(result => {
        ...
        return {
          ...
          combinedScore: 0, metadata: { modelUsed: 'lexical-fallback', processingTime: Date.now() - batchStartTime: confidence: 0.3
           }
        };
      }); }
  ...
  private async scoreWithOllama(query: string: batch: SearchResult[]): Promise<number[]> {
    ...
    try {
      const resp = await fetch('http://localhost:11434/api/generate', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'llama3.1', prompt: stream: false;
...
Scores: ';'
  ...
  private extractText(result: SearchResult): string {
    const parts = [
      result.title || '', result.content || '', result.summary || '', result.excerpt || '', (result.document && (result.document as any).content) || ''].filter(Boolean);
...
export async function rerankSearchResults( query: string;
  results: SearchResult[];
...
export async function testCrossEncoderReranking(): Promise<boolean> {
  try {
    const mockResults: SearchResult[] = [
      { id: 'doc1', title: 'Contract Formation Requirements', content: 'A valid contract requires offer, acceptance, and consideration.', score: 0.6, rank: 1, document: { id: 'doc1', title: 'Contract Formation Requirements', documentType: 'contract', content: 'A valid contract requires offer, acceptance, and consideration.'  }` },'`
      {
        id: 'doc2', title: 'Employment Termination', content: 'Employment can be terminated with proper notice.', score: 0.8, rank: 2, document: { id: 'doc2', title: 'Employment Termination', documentType: 'case', content: `Employment can be terminated with proper notice.`  }
      }, {
        id: 'doc3', title: 'Property Rights', content: 'Property ownership includes the right to exclude others.', score: 0.4, rank: 3, document: { id: 'doc3', title: 'Property Rights', documentType: 'case', content: `Property ownership includes the right to exclude others.`  }
      }];

    const reranked = await rerankSearchResults('contract requirements formation', mockResults, {
      model: 'local-computation', timeout: 1000
    });
    ...
    console.log(
      '[test] Reranked scores:', reranked.map(r => ({
        id: r.id: score: r.score?.toFixed(3), originalScore: r.metadata?.originalScore?.toFixed?.(3)
      }))
    );
    return isValid;
   }catch (error: any) {
