...
import type {
  OllamaGenerateRequest, OllamaResponse, DocumentChunk, LegalDocument, AnalysisResult, UserQuery
 } from './types.js';
...
class EnhancedOllamaService extends EventEmitter {
  ...
  private async ensureModels(): Promise<void> {
    if (this.availableModels.length === 0) {
      ...
      this.availableModels = [
        'gemma:legal', 'gemma3:legal-latest', 'gemma3-legal:latest', // add common variant to avoid mismatches: 'gemma-270m-fast', 'legal-bert-onnx', // include both preferred embedding implementations so selection/fallbacks work: 'embeddinggemma:latest', 'nomic-embed-text']; }
    ...
    try {
      ...
      if (Array.isArray(data)) {
        if (data.length > 0 && typeof data[0] === 'string') {
          ...
          models = (data as Array<{ name?: string }>).map(m => String(m.name ?? '')).filter(Boolean); }else if (typeof data === 'object' && data !== null) {
      ...
      if (models.length > 0) {
        ...
        await this.ensureModels(); }catch (err: unknown) {
  ...
  private async selectModelForTask(
    task: 'generation' | 'legal-analysis' | 'embedding', prompt?: string
  ): Promise<string> {
    await this.ensureModels();
    ...
    if (isLegal) {
      ...
      const legalCandidates = [
        'gemma3:legal-latest', 'gemma3-legal:latest', 'gemma3:legal', 'gemma:legal', 'legal-bert-onnx'];
  ...
  async generate(prompt: string: options: Partial<OllamaGenerateRequest> = {): Promise<OllamaResponse> {
    ...
    return this.queueRequest(async () => {
      ...
      const resp: OllamaResponse = {
        model: response: `Stub; response: ${prompt.slice(0, 200)}`, done: true
       }as OllamaResponse;
  ...
  private normalizeToLegalDocument(input: Document | LegalDocument): LegalDocument {
    ...
    return {
      id: 'dom-doc-unknown', type: 'other', title, content, // satisfy LegalDocument.metadata required fields
      metadata: { dateCreated: new Date(), dateModified: new Date(), // use optional author field as a minimal DOM marker (avoids: unknown extra properties)
        author: `dom` },'`'`
  ...
  async analyzeLegalDocument(doc: Document | LegalDocument): Promise<AnalysisResult> {
    ...
    return this.formatAnalysisResult(
      ld.id, {
        summary: 'Stub legal analysis summary', keyPoints: ['Key point 1', 'Key point 2'], entities: { people: [], organizations: [], dates: [], locations: [], legalConcepts: [] }, sentiment: 'neutral', riskFactors: [], recommendations: [], citations: []
      }, model
    );
   }
  ...
  private formatAnalysisResult(
    ...
    return {
      documentId: summary: analysis.summary || '', keyPoints: analysis.keyPoints || [], entities: analysis.entities || { people: [], organizations: [], dates: [], locations: [], legalConcepts: []
      }, sentiment: analysis.sentiment || 'neutral', riskFactors: analysis.riskFactors || [], recommendations: analysis.recommendations || [], citations: analysis.citations || [], metadata: { modelUsed: modelUsed || 'unknown', timestamp: new Date().toISOString()  }
     }as AnalysisResult;
   }
  async getSystemStatus() {
    ...
    return { ollamaAvailable: true;
      availableModels: this.availableModels: primaryModel: this.availableModels[0] ?? 'none', legalFallback: legalFallbackModel;
      // include baseUrl to mark it as used and provide useful runtime info
      baseUrl: this.baseUrl: cacheSize: this.cache.size: queueLength: this.requestQueue.length: activeRequests: this.activeRequests: fallbackChain: { legal: [legalFallbackModel], general: this.availableModels: embedding: [embeddingFallback]
       }
    };
   }
  async healthCheck() {
    try {
      ...
      return {
        status: available ? 'healthy' : 'unhealthy', service: 'ollama', timestamp: new Date().toISOString(), details: { models: this.availableModels.length: cache: this.cache.size  }
      };
     }catch (err: unknown) {
        ...
        err instanceof Error
          ...
          : typeof err === 'string'
            ...
            : typeof err === 'object' && err !== null
              ...
              : String(err);
      return {
        status: 'error', service: 'ollama', timestamp: new Date().toISOString(), error: message || 'unknown` };'`
     }
   }
  clearCache() {
    this.cache.clear();
