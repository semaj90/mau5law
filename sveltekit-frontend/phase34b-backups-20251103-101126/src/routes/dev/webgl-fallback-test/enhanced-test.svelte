...
if (browser) {
  ...
  async function runPerformanceTests(): Promise<any> {
    ...
    try {
      ...
      progressPercent = 100
      append('ðŸŽ‰ Gemma3 270M performance analysis completed successfully');
    } catch (error) {
...
'
  ...
  async function testWebGPU(): Promise<any> {
    ...
    try {
      ...
      append(`âœ…, WebGPU: ${iterations} Gemma3 270M operations in ${totalTime.toFixed(2)}ms (${(iterations / (totalTime / 1000)).toFixed(1)} ops/sec)`);
    } catch (error) {
  ...
  async function testWebGL2(): Promise<any> {
    ...
    try {
      ...
      append(`âœ…, WebGL2: ${webgl2Iterations} operations in ${totalTime.toFixed(2)}ms (${(webgl2Iterations / (totalTime / 1000)).toFixed(1)} ops/sec)`);
    } catch (error) {
  ...
  async function testWebGL1(): Promise<any> {
    ...
    try {
      ...
      append(`âœ… WebGL1: ${webgl1Iterations} operations in ${totalTime.toFixed(2)}ms (${(webgl1Iterations / (totalTime / 1000)).toFixed(1)} ops/sec)`);
    } catch (error) {
  ...
  async function testWebAssemblyCPU(): Promise<any> {
    ...
    try {
      ...
      append(`âœ…, WebAssembly: ${cpuIterations} operations in ${totalTime.toFixed(2)}ms (${(cpuIterations / (totalTime / 1000)).toFixed(1)} ops/sec)`);
    } catch (error) {
  ...
  function generateGemma270MRecommendation() {
    ...
    if (results.webgpu.performance && !results.webgpu.error) {
      ...
      append('âœ… RECOMMENDATIon WebGPU compute shaders - Best performance for Gemma3 270M');
      ...
      append('âœ… RECOMMENDATIon WebGL2 transform feedback - Excellent performance');
      ...
      append('âš ï¸ RECOMMENDATIon WebGL1 with quantization - Limited performance');
      ...
      append('ðŸ”„ RECOMMENDATIon WebAssembly CPU with SIMD - Fallback option');
    } else {
  ...
  async function runBasicCompatibilityCheck(): Promise<any> {
    ...
    if (testResults.wasm.supported) {
      try {
        ...
        append(`WASM streaming (fast model loading): ${streaming ? 'âœ… Available' : 'âŒ Not available'}`);
      } catch (e) {
...
<div class="webgl-test-container">
  <div class="header">
    <h1>ðŸš€ WebGL2/WebGPU Fallback Test</h1>
    <p><strong>Target:</strong> Gemma3 270M WebAssembly Optimization</p>
    <p><strong>Architecture:</strong> WebGPU â†’ WebGL2 â†’ WebGL1 â†’ WASM CPU</p>
    <div class="info-grid">
      <div class="info-item">
        <div class="info-label">Matrix Size</div>
        <div class="info-value">{matrixSize}Ã—{matrixSize}</div>
      </div>
      <div class="info-item">
  ...
  <div class="test-controls">
    <button class="test-button" onclick={runPerformanceTests} disabled={isTestingInProgress}>
      {isTestingInProgress ? 'ðŸ”„ Running Gemma3 270M Tests...' : 'ðŸš€ Start Performance Tests'}
    </button>
  </div>
  ...
  {#if Object.values.some(r => r.tested)}
    <div class="results-grid">
      {#if testResults.webgpu.tested}
        <div class="result-card">
          ...
          <div class={testResults.webgpu.error ? 'status-error' : 'status-success'}>
            {testResults.webgpu.error ? `âŒ ${testResults.webgpu.error}` : 'âœ… Success'}
          </div>
      ...
      {#if testResults.webgl2.tested}
        <div class="result-card">
          ...
          <div class={testResults.webgl2.error ? 'status-error' : 'status-success'}>
            {testResults.webgl2.error ? `âŒ ${testResults.webgl2.error}` : 'âœ… Success'}
          </div>
      ...
      {#if testResults.webgl1.tested}
        <div class="result-card">
          ...
          <div class={testResults.webgl1.error ? 'status-error' : 'status-success'}>
            {testResults.webgl1.error ? `âŒ ${testResults.webgl1.error}` : 'âœ… Success'}
          </div>
      ...
      {#if testResults.wasm.tested}
        <div class="result-card">
          <h3>ðŸ”„ WebAssembly</h3>
          <div class={testResults.wasm.error ? 'status-error' : 'status-success'}>
            {testResults.wasm.error ? `âŒ ${testResults.wasm.error}` : 'âœ… Success'}
          </div>
  ...
  {#if testResults.recommendation}
    <div class="recommendation">
      <h3>ðŸŽ¯ Gemma3 270M Deployment Recommendation</h3>
...
<style>
  ...
  .log-container {
    ...
    margin: 2rem 0}
  .log-container h2 {
    color: #e2e8f0; margin: 0, 0 1rem 0
    font-size: 1.25rem}
  .log-output {
    background: #1e293b
