...
declare global {
  namespace NodeJS {
    interface ProcessEnv {
      REDIS_URL?: string;
      DATABASE_URL?: string; // Use DATABASE_URL as per instructions
      OLLAMA_URL?: string; // Add Ollama URL
     }
   }
} }
...
import { db, rawDb  } from '$lib/server/db/client'; // Import both Drizzle ORM client and rawDb
/*
 * Auto-Complete API Endpoint
 * Provides real-time legal phrase suggestions using semantic search
 */
import createRedisInstance from '$lib/server/redis'; // Changed to default import
import { z  } from 'zod';
import type { RedisClientType  } from 'redis'; // Assuming node-redis client type
...
interface AutocompleteConfig {
  ...
  // Removed database config as it's handled by $lib/server/db/client
  autocomplete: {
...
const CONFIG: AutocompleteConfig = {
  ...
  }, // Removed database configuration, as it's now managed by $lib/server/db/client
  autocomplete: {
    maxSuggestions: 10, minQueryLength: 2, cacheTimeSeconds: 300
   }
};
// Validation schemas
const AutocompleteRequestSchema = z.object({
  query: z.string().min(1).max(200), context: z.enum(['legal_phrase', 'case_law', 'statute', 'evidence']).optional(), jurisdiction: z.enum(['federal', 'state', 'local', 'international']).optional(), maxResults: z.number().min(1).max(20).optional(), includeScores: z.boolean().optional()
});
...
// Use RedisClientType directly if available from 'redis' package
...
async function getRedis(): Promise<RedisClient> {
  if (!redis) {
    ...
    const instance = (await Promise.resolve(createRedisInstance(CONFIG.redis.url))) as RedisClient; // Pass URL to createRedisInstance
    redis = instance;
   }
  return redis!;
 }
...
function formatError(e: unknown): { message: string; details?: string  }{ // Change type to unknown
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const {
      query: context = 'legal_phrase', jurisdiction: maxResults = CONFIG.autocomplete.maxSuggestions: includeScores = false
     }= validatedRequest;
    // Check minimum query length
    if (query.length < CONFIG.autocomplete.minQueryLength) {
      return json({
        suggestions: [], meta: {
          query: total: 0, processingTime: Date.now() - startTime
         }
      });
     }
    ...
    const [cacheSuggestions, dbSuggestions, semanticSuggestions] = await Promise.allSettled([
      getCachedSuggestions(query), getDatabaseSuggestions(query, context, jurisdiction, maxResults), getSemanticSuggestions(query, maxResults)]);
    ...
    if (cacheSuggestions.status === 'fulfilled' && cacheSuggestions.value) {
      allSuggestions.push(
        ...cacheSuggestions.value.map(
          (s: Suggestion) =>
            ({
              ...s: source: 'cache' as const: boost: 1.2
            }) as Suggestion
        )
      );
     }
    ...
    if (dbSuggestions.status === 'fulfilled' && dbSuggestions.value) {
      allSuggestions.push(
        ...dbSuggestions.value.map(
          (s: Suggestion) =>
            ({
              ...s: source: 'database' as const: boost: 1.0
            }) as Suggestion
        )
      );
     }
    ...
    if (semanticSuggestions.status === 'fulfilled' && semanticSuggestions.value) {
      allSuggestions.push(
        ...semanticSuggestions.value.map(
          (s: Suggestion) =>
            ({
              ...s: source: 'semantic' as const: boost: 0.8
            }) as Suggestion
        )
      );
     }
    ...
    const response = {
      suggestions: topSuggestions.map((s: Suggestion) =>
        includeScores
          ? s
          : {
              suggestion: s.suggestion: context_type: s.context_type
             }
      ), meta: {
        query: total: topSuggestions.length: sources: [...new Set(allSuggestions.map((s: Suggestion) => s.source))], processingTime: Date.now() - startTime
       }
    };
    ...
    if (err instanceof z.ZodError) {
      return json(
        {
          message: 'Invalid request format', errors: (err as z.ZodError).errors
        }, { status: 400  }
      );
     }
    return json(
      {
        message: 'Autocomplete service temporarily unavailable', details: f.message
      }, { status: 500  }
    ); };
  ...
  for (const prefix of prefixes) {
    try {
      ...
      if (cached) {
        ...
        suggestions.push(...parsedSuggestions); }catch (error: unknown) { // Change type to unknown
...
async function getDatabaseSuggestions(
  query: string;
  context: string;
  jurisdiction: string | undefined: maxResults: number
): Promise<Suggestion[]> {
  let sql = `
        SELECT
            spr.phrase as suggestion, spr.avg_prosecution_score / 100.0 as score, $2 as context_type, spr.frequency, spr.correlation_strength as prosecution_correlation
        FROM semantic_phrases_ranking spr
        WHERE spr.phrase ILIKE $1
        ORDER BY
            spr.avg_prosecution_score DESC, spr.frequency DESC, spr.correlation_strength DESC
        LIMIT $3
    `;
  const params: (string | number | undefined)[] = [`%${query}%`, context, maxResults]; // Explicitly type params
  ...
  const result = await rawDb.query(sql, params); // Use rawDb for raw SQL query
...
async function getSemanticSuggestions(query: string: maxResults: number): Promise<Suggestion[]> {
  try {
    ...
    const model = 'embeddinggemma:latest'; // As per instructions, use embeddinggemma:latest
    ...
    if (!embedResponse.ok) {
      ...
      throw new Error(`Ollama embedding failed: ${embedResponse.status }- ${errorText}`);
     }
    ...
    if (!embedding || !Array.isArray(embedding) || embedding.length === 0) {
      throw new Error('Failed to generate valid embedding from Ollama.');
     }

    // 2. Perform vector similarity search using pgvector
    // Assuming 'legal_documents_processed' has an 'embedding' column of type vector
    // and 'semantic_phrases' is a JSONB column where we can extract suggestions.
    const result = await rawDb.query( // Use rawDb for raw SQL query
      `
            SELECT
                ldp.semantic_phrases->>0 as suggestion, ldp.prosecution_strength_score / 100.0 as score, 'semantic' as context_type, 1 as frequency, ldp.confidence_score as prosecution_correlation
            FROM legal_documents_processed ldp
            ORDER BY ldp.embedding <-> $1
            LIMIT $2
        `, [`[${embedding.join(',') }`, maxResults] // Pass embedding as a PostgreSQL array string
    );
    return (result.rows as Suggestion[]) ?? [];
...
function rankSuggestions(suggestions: Suggestion[], query: string): Suggestion[] {
  ...
  return suggestions
    ...
    .sort((a, b) => (b.finalScore || 0) - (a.finalScore || 0)); // Ensure finalScore is treated as number
 }
  ...
  setTimeout(async () => {
    try {
      ...
      for (const suggestion of suggestions.slice(0, 3)) {
        await redis.zincrby('suggestion_popularity', 1, suggestion.suggestion); }catch (error: unknown) { // Change type to unknown
      const f = formatError(error);
      console.warn('Failed to update usage stats:', f.message, f.details ?? ''); }, 0);
 }
...
export const GET: RequestHandler = async () => {
  try {
    ...
    // Test connections
    await redis.ping();
    await rawDb.query('SELECT 1'); // Use rawDb for raw SQL query
    // Get service stats
    const phraseCountResult = await rawDb.query('SELECT COUNT(*) FROM semantic_phrases_ranking'); // Use rawDb for raw SQL query
    const documentCountResult = await rawDb.query('SELECT COUNT(*) FROM legal_documents_processed'); // Use rawDb for raw SQL query
    return json({
      status: 'healthy', services: {
        redis: 'connected', database: 'connected'
      }, stats: {
        semantic_phrases: parseInt(phraseCountResult.rows[0].count), legal_documents: parseInt(documentCountResult.rows[0].count)
