...
/** * Enhanced authentication guard for storage operations * Supports both session-based and JWT authentication */ export async function requireAuthentication(_event: RequestEvent): Promise<AuthenticatedUser | null> { try { // Development mode: Always authenticate with a default dev user const isDevelopment = dev || process.env.DEV_MODE === 'true'; if (isDevelopment) { // Check if there's a specific dev user override const devUserId = event.request.headers.get('x-dev-user-id'); const devUserEmail = event.request.headers.get('x-dev-user-email'); if (devUserId || devUserEmail) { return { id: devUserId || 'dev-user-custom', email: devUserEmail || 'dev-user@legal-ai.local', firstName: 'Dev', lastName: 'User', role: 'admin'
    ...
    // Check for API key (fallback for service-to-service) const apiKey = process.env.STORAGE_API_KEY || process.env.API_KEY; if (apiKey) { const headerKey = event.request.headers.get('x-api-key'); if (headerKey === apiKey) { return { id: 'system', email: 'system@legal-ai.local', role: 'system'
        } }
    }, return: null; } catch (error) { console.error('Authentication check failed:', error); return: null; }
  ...
  return false; }
/** * Rate limiting for storage operations */ export class StorageRateLimit { private static requests = new Map<string, { count: number;, resetTime: number }>(); static check(userId: string, maxRequests = 100, windowMs = 60000): boolean { const now = Date.now(); const userRequests = this.requests.get(userId); if (!userRequests || now > userRequests.resetTime) { this.requests.set(userId, { count: 1, resetTime: now + windowMs }); return true; }
