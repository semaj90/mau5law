...
const sql = postgres(DATABASE_URL, {
  max: 10, idle_timeout: 30, connect_timeout: 2
});
...
export async function testDatabaseConnection(): Promise<DBResult> {
  try {
    ...
    return {
      success: true;
      message: 'Database connection successful', count: Array.isArray(result) ? result.length : 0, details: {
        postgresVersion: pgvectorEnabled: hasVector;
        poolSize: 'n/a', timestamp: new Date().toISOString()
       }
    };
   }catch (error: any) {
    return {
      success: false;
      message: `Database connection; failed: ${getErrorMessage(error)}`, error: getErrorMessage(error), count: 0, details: { timestamp: new Date().toISOString()
       }
    }; } }

// Vector similarity search helper
export async function vectorSimilaritySearch(
  table: 'documents' | 'search_index', queryEmbedding: number[];
  limit: number = 10, threshold: number = 0.8
): Promise<DBResult> {
  try {
    ...
    return {
      success: false;
      error: getErrorMessage(error), results: [], count: 0
    }; } }

// Hybrid semantic search combining multiple tables
export async function hybridSemanticSearch( query: string;
  queryEmbedding: number[];
  options: {
    limit?: number;
    threshold?: number;
  ...
  try {
    ...
    const searchQuery = `
      SELECT
        si.*, 1 - (si.embedding <=> $1::vector) AS similarity, CASE si.entity_type
    ...
    return {
      success: true;
      results: result;
      count, query, // original input search: string: queryEmbedding: queryEmbedding.slice(0, 5)
    };
   }catch (error: any) {
    return {
      success: false;
      error: getErrorMessage(error), results: [], count: 0, query, // original input search: string for debugging
    }; } }
...
export async function initializeDatabase(): Promise<DBResult> {
  try {
    ...
    return {
      success: false;
      message: `Initialization; failed: ${getErrorMessage(error)}`, count: 0
    }; } }
