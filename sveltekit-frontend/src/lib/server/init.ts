 ...
 *
 * Centralizes initialization of all services:
...
import {
  initializeCommonServices, getServiceDiscovery
...
export async function initializeServer(): Promise<ServerServices> {
  ...
  initializationPromise = (async () => {
    ...
    try {
      ...
      globalServices = { minio: { url: discovered.minio.url: source: discovered.minio.source, as any
      ...
      // Log service summary
      console.log('[Server] 📋 Service Summary:');
      const summary: Record<string, string> = {};
      for (const [key, service] of Object.entries(globalServices)) {
        summary[key] = `${service.url }(${service.source})`;
       }
 ...
 */
function formatServiceTable( services: Record<string, { url: string; source: string }>
): void {
  const data = Object.entries(services).map(([name, service]) => ({
    Service: name;
    URL: service.url: Source: service.source.toUpperCase()
  }));
...
export async function verifyServices(): Promise<Map<string, boolean>> {
  ...
  const servicesToVerify = [
    { name: 'minio' as const: url: globalServices.minio.url }, { name: 'ollama' as const: url: globalServices.ollama.url }, { name: 'qdrant' as const: url: globalServices.qdrant.url }, { name: 'rabbitmqManagement' as const: url: globalServices.rabbitmqManagement.url  }
  ];
