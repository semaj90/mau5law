...
export class RabbitMQManager extends EventEmitter {
  ...
  private readonly exchanges = { cache_invalidation: 'cache.invalidation',
    document_processing: 'document.processing',
    vector_updates: 'vector.updates',
    analytics: 'analytics.events'
  };
  private readonly queues = { cache_invalidate: 'cache.invalidate',
  ...
  async publishCacheInvalidation(data: { type: 'document' | 'evidence' | 'case' | 'chat';, id: string;
    keys?: string[];
  }): Promise<void> {
  ...
  async publishAnalyticsEvent(data: { event_type: string;, event_data: any;
    ...
    try {
      ...
      if (this.lokiRedisCache?.storeDocument) {
        await this.lokiRedisCache.storeDocument({
          ...
          riskLevel: 'medium',
          confidenceLevel: 0.8,
          metadata: { title: `Evidence ${evidence_id}`,
            description: content.substring(0, 200),
            keywords: [],
            jurisdiction: `Unknown` },
          cacheTimestamp: Date.now(),
  ...
  async healthCheck(): Promise<any> {
    try {
      ...
      return {
        ...
        servicesLoaded: { redisService: !!this.redisService,
