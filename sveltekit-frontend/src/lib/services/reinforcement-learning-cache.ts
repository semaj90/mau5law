/**
 * Simple Reinforcement Learning Cache
 * - In-memory Map with lightweight LRU+frequency eviction
 * - Exposes initialize, set, get, cleanup
 * - Exports both named and default instances to match dynamic import expectations
 *
 * Note: keep implementation small and dependency-free so it can be imported dynamically
 * by the multi-dimensional-image-cache module without creating circular imports.
 */
type RLValue = unknown;
...
class ReinforcementLearningCache {
	...
	private evictOne(): void {
		...
		for (const [k, v] of this.store.entries()) {
			// Score: lower is worse. Combine recency and frequency.
			// recentness weight -> small penalty for old entries, freq reduces score.
			const age = now - v.lastAccess;
			const ageScore = age / 1000; // seconds
			const freqScore = 1 / Math.max(1, v.freq);
