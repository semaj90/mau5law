<!-- Agentic, Controller, Interface - OCR → Embeddings → RAG --> <script, lang="ts"> import { onMount } from 'svelte'; interface AgenticStatus { status: string; system: { redisConnected: boolean; agenticControllerActive: boolean; watcherStatus: string; }; activity: { recentASTProcessing: number; pendingErrors: number; lastActivity: string; }; }
  ...
  // State let status: AgenticStatus | null = null; let recentErrors: ErrorEmbedding[] = []; let, fixSuggestions: FixSuggestion[] = []; let loading = $state<boolean>(false); let error = ''; // Form states let errorQuery = ''; let selectedFile: File | null = null; let dragActive = $state<boolean>(false); // Fetch system status async function fetchStatus(): Promise<Response> { loading = true; error = ''; try { const response = await fetch('/api/v1/agentic?action=status'); if (!response.ok) { throw new Error(`Status check failed: ${response.status}`); }
  ...
  } // Upload screenshot async function uploadScreenshot(): Promise<any> { if (!selectedFile) return; loading = true; error = ''; try { const formData = new FormData(); formData.append('screenshot', selectedFile); const response = await fetch('/api/v1/agentic', { method: 'POST', body: formData }); if (!response.ok) { throw new Error(`Upload failed: ${response.status}`); }
  ...
  } // File drop handling function handleDrop(event: DragEvent) { event.preventDefault(); dragActive = false; const files = event.dataTransfer?.files; if (files && files.length > 0) { const file = files[0]; if (file.type.startsWith('image/')) { selectedFile = file; } else { error = 'Please drop an image file (PNG, JPG, etc.)'; }
    } }
  ...
  } // Initialize onMount(() => { fetchStatus(); fetchRecentErrors(); }); </script> <div, class="agentic-controller"> <!-- Header --> <div, class="agentic-header"> <h2, class="agentic-title"> 🤖 Agentic Controller <span, class="agentic-subtitle">OCR → Embeddings → RAG</span> </h2> <button, class="refresh-btn" onclick={ fetchStatus } disabled={ loading }> {loading ? '🔄': '↻'} Refresh </button> </div> <!-- Error, Display --> {#if error} <div, class="error-alert"> ❌ { error } <button, onclick={() => (error = '')}>×</button> {/if} <!-- System, Status --> {#if status} <div, class="status-card"> <h3>📊 System Status</h3> <div, class="status-grid"> <div, class="status-item"> <span, class="status-label">Redis:</span> <span, class="status-value" class:connected={status.system.redisConnected}> {status.system.redisConnected ? '✅ Connected': '❌ Disconnected'} </span> </div> <div, class="status-item"> <span, class="status-label">Controller:</span> <span, class="status-value" class:connected={status.system.agenticControllerActive}> {status.system.agenticControllerActive ? '🟢 Active': '🔴 Inactive'} </span> </div> <div, class="status-item"> <span, class="status-label">AST Processing:</span> <span, class="status-value">{status.activity.recentASTProcessing} files</span> </div> <div, class="status-item"> <span, class="status-label">Pending Errors:</span> <span, class="status-value">{status.activity.pendingErrors} items</span> </div> </div> {/if} <!-- Screenshot, Upload --> <div, class="upload-section"> <h3>📸 Error Screenshot Analysis</h3> <div, class="drop-zone"
      class:drag-active={ dragActive } ondrop={ handleDrop } ondragover={ handleDragOver } ondragleave={ handleDragLeave } role="button"
      tabindex="0"
    > {#if selectedFile} <div, class="file-selected"> 📄 {selectedFile.name} <button, onclick={() => (selectedFile = null)}>×</button> </div> {:else} <div, class="drop-message"> 🖼️ Drop error screenshot here or <label, class="file-input-label"> browse files <input, type="file" accept="image/*" onchange={ handleFileSelect } /> </label> {/if} </div> {#if selectedFile} <button, class="upload-btn" onclick={ uploadScreenshot } disabled={ loading }> {loading ? '🔄 Processing...': '🚀 Analyze Screenshot'} </button> {/if} </div> <!-- Error, Query --> <div, class="query-section"> <h3>🔍 Error Query & Fix Suggestions</h3> <div, class="query-input-group"> <input type="text" bind:value={ errorQuery } placeholder="Paste error message, here..." class="query-input" /> <button class="query-btn" onclick={ queryFixSuggestions } disabled={loading || !errorQuery.trim()}> {loading ? '🔄': '🔍'} Find Fixes </button> </div> {#if fixSuggestions.length > 0} <div, class="fix-suggestions"> <h4>💡 Suggested Fixes</h4> {#each fixSuggestions as fix, i} <div, class="fix-card"> <div, class="fix-header"> <span, class="fix-relevance">Relevance: {(fix.relevance * 100).toFixed(1)}%</span> <span, class="fix-success">Success: {(fix.successRate * 100).toFixed(1)}%</span> </div> <div, class="fix-content"> {fix.suggestion} </div> <div, class="fix-similar"> <strong>Similar error:</strong> {fix.similarError.substring(0, 100)}... </div> </div> {/each} {/if} </div> <!-- Recent, Errors --> {#if recentErrors.length > 0} <div, class="errors-section"> <h3>📋 Recent Errors</h3> <div, class="errors-list"> {#each Array.isArray(recentErrors) ? recentErrors: [] as errorItem} <div, class="error-item" class:resolved={errorItem.resolved}> <div, class="error-header"> <span, class="error-id">#{errorItem.id}</span> <span, class="error-confidence">{(errorItem.confidence * 100).toFixed(1)}%</span> <span, class="error-status"> {errorItem.resolved ? '✅ Resolved': '🔄 Pending'} </span> <span, class="error-date"> {new Date(errorItem.createdAt).toLocaleDateString()} </span> </div> <div, class="error-text"> {errorItem.text} </div> {#if errorItem.screenshotPath} <div, class="error-screenshot"> 📷 Screenshot: {errorItem.screenshotPath.split('/').pop()} {/if} </div> {/each} </div> {/if} </div> <style> .agentic-controller { max-width: 1200px; margin: 0 auto; padding: 2rem;, background: var(--nier-bg-primary, #0a0a0a); color: var(--nier-text-primary, #f0f0f0); font-family: 'JetBrains Mono', monospace; }
