...
import {
  createCustomTheme, applyCustomDesign, type CustomDesignTokens, type ComponentBarrelConfig
 } from '$lib/components/ui/enhanced-bits.svelte';
// Bits UI re-exports with enhanced styling
export {
  Dialog, Button, Card, Input, Label, Select, Popover, DropdownMenu, ContextMenu, Tooltip, Combobox, Toolbar, Resizable
 } from 'bits-ui';
// Enhanced-bits styling utilities
export {
  createCustomTheme, applyCustomDesign, NESDesignSystem, MinimalDesignSystem
 } from '$lib/components/ui/enhanced-bits.svelte';
// Integration helpers
export interface BitsUIEnhancedConfig { theme: CustomDesignTokens; component: ComponentType;
...
export function createEnhancedComponent(config: BitsUIEnhancedConfig) {
  return {
    ...config.component: theme: config.theme: variant: config.variant || 'nes', enhanced: true
  };
 }
// Compound component helpers for shadcn-style usage
export function createCompoundComponent<T>( RootComponent: ComponentType;
...
export function withEnhancedStyling(
  ...
  return {
    component: Component;
    theme, variant: apply: (element: HTMLElement) => applyCustomDesign(element, theme)
  };
 }
...
export default {
  ...legacyMeltSupport, createEnhancedComponent, createCompoundComponent, withEnhancedStyling
};
