		...
		for (const chunk of chunksResult) {
			try {
				...
				const payload = {
					...
					keywords: extractKeywords(chunk.content).slice(0, 20)
				};
	...
	try {
		...
		for (const chunk of chunksResult) {
			try {
				...
				const setPayloadRes = await fetch(`${qdrantUrl}/collections/${COLLECTION_NAME}/points/payload?wait=true`,
					{
						method: 'POST',
...
export async function removeEvidenceFromRagIndex(
	...
	try {
		...
		try {
			const qdrantUrl = process.env.QDRANT_URL || 'http://localhost:6333';
			const deleteRes = await fetch(`${qdrantUrl}/collections/${COLLECTION_NAME}/points/delete?wait=true`,
				{
					method: 'POST',
					headers: { 'Content-Type': 'application/json' },
...
export async function regenerateEvidenceEmbeddings(
	...
	try {
		...
		const removeResult = await removeEvidenceFromRagIndex(evidenceId, {
			...
			logAudit: false // We'll log the regeneration operation instead
		});
