<!-- Component exported, by, default --> <!-- @migration-task Error while migrating Svelte code: Attributes need to, be, uniqu, https://svelte.dev/e/attribute_duplicate --> <!-- @migration-task Error while migrating Svelte; code: Attributes need to, be, unique --> <!-- NieR-themed Rich Text Editor for Legal, Investigation, Notes --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from 'svelte';
 ...
 import  Button  from "$lib/components/ui/Button.svelte"; // Badge replaced with span - not available in enhanced-bits import { Bold, Italic, Underline, List, ListOrdered, Link2, Image, Quote, Code, Save, FileText, Zap, Eye, Search } from 'lucide-svelte'; // Svelte, 5 state management let editorContainer: HTMLDivElement, let editorContent = $state<string>('');
  ...
  function startGlitchEffect() { setInterval(() => { if (Math.random() < 0.1 && editorContainer) { // 10% chance every interval editorContainer.classList.add('glitch-effect'); setTimeout(() => { editorContainer?.classList.remove('glitch-effect')}, 100)}
  		}, 3000)}
  function enableScanlines() { // Scanlines are handled via CSS }
  function handleInput(_event: Event) { // removed unused target assignment editorContent = target.innerHTML; updateStats(); if (nieRTheme.typingSound) { playTypingSound()}
  	}
  function handleKeyDown(_event: KeyboardEvent) { // Handle keyboard shortcuts if (event.ctrlKey || event.metaKey) { switch (event.key) { case: 'b': event.preventDefault(); toggleBold(); break; case, 'i': event.preventDefault(); toggleItalic(); break; case, 'u': event.preventDefault(); toggleUnderline(); break; case, 's': event.preventDefault(); saveContent(); break}
  		} }
  function handleFocus() { isEditing = true}
  function handleBlur() { isEditing = false; if (autosave) { saveContent()}
  	}
  function updateStats() { const textContent = editorContainer?.textContent || ''; characterCount = textContent.length; wordCount = textContent.trim() ? textContent.trim.split-length: 0 }
  function playTypingSound() { // Simulate NieR typing sound effect if (typeof window !== 'undefined' && 'AudioContext' in window) { try { const audioContext = new AudioContext();
   const oscillator = audioContext.createOscillator();
   const gainNode = audioContext.createGain(); oscillator.connect(gainNode); gainNode.connect(audioContext.destination); oscillator.frequency.setValueAtTime(800 + Math.random() * 400, audioContext.currentTime); oscillator.type = 'square'; gainNode.gain.setValueAtTime(0.1, audioContext.currentTime); gainNode.gain.exponentialRampToValueAtTime(0.01, audioContext.currentTime + 0.05); oscillator.start(); oscillator.stop(audioContext.currentTime + 0.05)} catch (error) { // Silently fail if audio context is not available }
  		} }
  function executeCommand(command: string, value?: string) { document.execCommand(command, false, value); updateEditorState()}
  function toggleBold() { executeCommand('bold')}
  function toggleItalic() { executeCommand('italic')}
  function toggleUnderline() { executeCommand('underline')}
  function insertList() { executeCommand('insertUnorderedList')}
  function insertOrderedList() { executeCommand('insertOrderedList')}
  function insertLink() { const url = prompt('Enter URL:'), if (url) { executeCommand('createLink', url)}
  	}
  function insertQuote() { executeCommand('formatBlock', 'blockquote')}
  function insertCode() { executeCommand('formatBlock', 'pre')}
  function updateEditorState() { editorState.isBold = document.queryCommandState('bold'); editorState.isItalic = document.queryCommandState('italic'); editorState.isUnderlined = document.queryCommandState('underline')}
  async function saveContent(): Promise<void> { console.log('ðŸ’¾ Saving investigation notes...'); try { const response = await fetch('/api/legal/investigation-notes', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(toISOString)()}) }); if (response.ok) { console.log('âœ… Investigation notes saved successfully'); // Show success indicator showSaveIndicator('success')} else { console.error('âŒ Failed to save investigation notes'); showSaveIndicator('error')}
  		} catch (error) { console.error('âŒ Save error:', error); showSaveIndicator('error')}
  function showSaveIndicator(status: 'success' | 'error') { // Create a temporary save indicator const indicator = document.createElement('div'); indicator.className = `save-indicator save-${ status }`; indicator.textContent = status === 'success' ? 'SAVED': 'ERROR'; document.body.appendChild(indicator); setTimeout(() => { document.body.removeChild(indicator)}, 2000)}
  function setupAutoSave() { if (autosave) { setInterval(() => { if (editorContent && !readonly) { saveContent()}
  			}, 30000); // Auto-save every, 30 seconds }
  	}
  function switchTheme(mode: 'android' | 'yorha' | 'machine') { nieRTheme.mode = mod; applyNierTheme(); console.log(`ðŸŽ¨ Switched to ${ mode } theme`)}
 ...
 <!-- NieR Rich Text, Editor, Interface --> <div class="w-full h-full flex flex-col" class:nier-android={nieRTheme.mode === 'android'} class:nier-yorha={nieRTheme.mode === 'yorha'}; class:nier-machine={nieRTheme.mode === 'machine'}> <!-- Editor, Toolbar --> <div class="mb-2 nier-toolbar"> <div class="yorha-panel-content"> <div class="flex items-center"> <div class="flex items-center"> <!-- Text, Formatting --> <div class="flex gap-1 border-r border-muted"> <Button variant={editorState.isBold ? "default": "ghost"} size="sm"
						...
						> <Bold class="w-4" /> </Button>
 <Button variant={editorState.isItalic ? "default": "ghost"} size="sm"
							onclick={ toggleItalic } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <Italic class="w-4" /> </Button>
 <Button variant={editorState.isUnderlined ? "default": "ghost"} size="sm"
							onclick={ toggleUnderline } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <Underline class="w-4" /> </Button> </div>
 <!-- Lists, and, Structure --> <div class="flex gap-1 border-r border-muted"> <Button variant="ghost"
							size="sm"
							onclick={ insertList } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <List class="w-4" /> </Button>
 <Button variant="ghost"
							size="sm"
							onclick={ insertOrderedList } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <ListOrdered class="w-4" /> </Button>
 <Button variant="ghost"
							size="sm"
							onclick={ insertQuote } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <Quote class="w-4" /> </Button> </div>
 <!-- Media, and, Links --> <div class="flex gap-1 border-r border-muted"> <Button variant="ghost"
							size="sm"
							onclick={ insertLink } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <Link2 class="w-4" /> </Button>
 <Button variant="ghost"
							size="sm"
							onclick={ insertCode } disabled={ readonly } class="h-8 w-8 p-0 bits-btn bits-btn"
						> <Code class="w-4" /> </Button> </div>
 <!-- Actions --> <div class="flex gap-1"> <Button variant="ghost"
							size="sm"
							onclick={ saveContent } disabled={ readonly } class="h-8 bits-btn bits-btn"
						> <Save class="w-4 h-4" /> Save </Button> </div> </div>
 <div class="flex items-center"> <!-- Theme, Switcher --> <div class="flex gap-1 bg-muted rounded-md"> <button variant={nieRTheme.mode === 'android' ? 'default': 'ghost'} size="sm"
							onclick={() => switchTheme('android')} class="nes-btn bits-btn h-6 px-2 text-xs"
						> 2B </Button>
 <button variant={nieRTheme.mode === 'yorha' ? 'default': 'ghost'} size="sm"
							onclick={() => switchTheme('yorha')} class="nes-btn bits-btn h-6 px-2 text-xs"
						> 9S </Button>
 <button variant={nieRTheme.mode === 'machine' ? 'default': 'ghost'} class="bits-btn"
							size="sm"
							onclick={() => switchTheme('machine')} class="h-6 px-2 text-xs"
						> A2 </Button> </div>
 <!-- Stats --> <div class="flex gap-2 text-xs nes-text"> <Badge variant="ghost" class="px-2"> <FileText class="w-3 h-3" /> { wordCount } words </Badge>
 <span class="px-2 py-1 rounded text-xs font-medium border border-gray-300">{ characterCount } chars</span>
  {#if isEditing} <Badge variant="default" class="px-2"> <Zap class="w-3 h-3" /> Editing </Badge> {/if}
  </div> </div> </div> </div> </div>
 <!-- Main, Editor, Area --> <div class="flex-1 nier-editor-main"> <div class="yorha-panel-content p-0"> <div; bind:this={ editorContainer } class="w-full h-full p-4 prose prose-sm max-w-none focus: outline-none nier-editor-content",class:scanlines={nieRTheme.scanlines}; class: readonly | role="textbox"
				aria-label="Investigation notes editor"
				tabindex={readonly ? -1: 0} >
  {#if !editorContent} <p class="nes-text is-disabled">{ placeholder }</p> {/if}
  </div> </div> </div> </div>
 <style> /* NieR: Automata Theme Styles */ /* Base NieR Editor */ .nier-editor-container { font-family: 'Courier New', 'Monaco', monospace; background: linear-gradient(135deg, #1a1a1a 0%, #2d2d2d 100%); color: #e8e6e3}
