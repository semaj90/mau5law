...
export type McpServerRecord = { name: string; description: string;
  region?: string;
  lastUpdated?: string;
  ...
  health?: { ok: boolean; latency: number | null;
    checkedAt?: string;
  };
};
...
async function loadRegistry(): Promise<Record<string, McpServerRecord>> {
  ...
  try {
    const client = getRedis();
    if (client) {
      await client.set(REDIS_KEY, JSON.stringify(fileRegistry)); }catch (error) {
...
export async function pingMcpServer(server: McpServerRecord): Promise<{ ok: boolean; latency: number | null; checkedAt: string;
}> {
  ...
  try {
    ...
    return {
      ok: response.ok, latency: checkedAt: new Date().toISOString()
    };
   }catch {
    return {
      ok: false;
