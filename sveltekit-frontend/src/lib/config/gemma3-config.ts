...
export interface Gemma3ModelConfig { modelId: string; name: string;
  description: string;
  modelPath: string;
...
export const GEMMA3_MODELS: Gemma3ModelConfig[] = [
  { modelId: "gemma3-legal-q4_k_m", name: "Gemma3 Legal (Q4_K_M)", description: "Local Gemma3 8B model optimized for legal analysis", modelPath: join(projectRoot, "gemma3Q4_K_M", "mo16.gguf"), contextLength: 8192, maxTokens: 4096, quantization: "q4_k_m", memoryRequirement: 6, inferenceSpeed: "medium", quality: "high", useCase: [
      "complex_analysis", "legal_research", "case_review", "contract_analysis", "document_summarization", "quick_queries"
], promptTemplate: `<bos><start_of_turn>user`
{system_prompt }
{user_input}<end_of_turn>
<start_of_turn>model
`,`
    ollamaModel: "gemma3-legal:latest", fallbackModel: "gemma3-legal:latest", format: "gguf", size: "8B"
 }
];
...
export const GEMMA3_INFERENCE_SETTINGS = { quick_response: { temperature: 0.3, topP: 0.8, topK: 20, repeatPenalty: 1.05, maxTokens: 512
}, balanced: { temperature: 0.7, topP: 0.9, topK: 40, repeatPenalty: 1.1, maxTokens: 1024
}, creative: { temperature: 0.9, topP: 0.95, topK: 60, repeatPenalty: 1.15, maxTokens: 2048
}, precise: { temperature: 0.1, topP: 0.7, topK: 10, repeatPenalty: 1.05, maxTokens: 512
 }
} }
export function selectOptimalGemmaModel( contextLength: number;
  availableMemory: number): Gemma3ModelConfig | null {
...
export function formatGemmaPrompt( template: string;
  systemPrompt: string;
...
export function getSystemPromptForContext(
  queryType: string;
  hasLegalContext: boolean): string {
  if (hasLegalContext) {
    switch (queryType) {
      case, "case_analysis":
...
export function getInferenceSettings( priorityLevel: "quick_response" | "balanced" | "creative" | "precise" = "balanced") {
