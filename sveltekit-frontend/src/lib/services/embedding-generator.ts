...
export interface EmbeddingCacheStats { totalRequests: number; cacheHits: number;
  cacheMisses: number;
  hitRate: number;
  avgGenerationTime: number; modelUsage: Record<string, number>;
...
export class GemmaEmbeddingService {
  private stats: EmbeddingCacheStats = { totalRequests: 0, cacheHits: 0, cacheMisses: 0, hitRate: 0, avgGenerationTime: 0, modelUsage: {}, batchesProcessed: 0, lastCleanup: Date.now()
  };
  ...
  constructor() {
    ...
    try {
      componentTextureRegistry.register('GemmaEmbeddingService', {
        componentName: 'GemmaEmbeddingService', textureSlots: ['embedding_patterns'], memoryBank: 'CHR_ROM', sharingPolicy: 'shared', updateFrequency: 'periodic', priority: 180, estimatedUsage: 512 * 1024
      });
     }catch {
      ...
      for (let i = 0; i < uncachedChunks.length; i += BATCH_SIZE) {
        ...
        const batchPromises = slice.map(async ({ index, text }) => {
          const emb = await this.generateWithGemma(
            text, options?.model ?? PRIMARY_MODEL, options?.priority ?? 'medium'
          );
  ...
  private async generateWithGemma( textChunk: string;
    preferredModel: string;
    priority: string = 'medium'
  ): Promise<number[]> {
    const modelsToTry = [
      preferredModel, ...(preferredModel !== PRIMARY_MODEL ? [PRIMARY_MODEL] : []), ...(preferredModel !== FALLBACK_MODEL ? [FALLBACK_MODEL] : []), SECONDARY_FALLBACK];

    for (const model of modelsToTry) {
      try {
        const response = await callOllamaApi({
          model: prompt: textChunk;
          options: { temperature: 0, top_p: 1, num_ctx: priority === 'critical' ? 8192 : priority === 'high' ? 4096 : 2048
           }
        });
  ...
  private async generateChrRomEmbeddingPattern(
    ...
    try {
      ...
      await chrRomCacheReader.cachePattern(
        `embedding_pattern:${cacheKey}`, 'embedding_visualization', visualizationSVG, {
          ttl: EMBEDDING_CACHE_TTL;
          metadata: {
            embedding, dimensionality, magnitude, avgValue: textPreview: textChunk.substring(0, 100)
           }
         }
      );
     }catch (err) {
    ...
    try {
      ...
      if (typeof (typedRedis, as any).keys === 'function') {
        ...
        for await (const k of (typedRedis as any).scanIterator({ MATCH: 'legal:embedding:*', COUNT: 100 })) {
          keys.push(k as string); }else if (typeof (typedRedis as any).scan === 'function') {
