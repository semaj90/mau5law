...
class VectorPipelineService {
	...
	async processDocuments(request: VectorPipelineRequest): Promise<ProcessingResult> {
		...
		try {
			...
			const objectsToProcess = request.batch_objects || 
				(request.object_key ? [request.object_key] : await this.listBucketObjects(request.bucket_name));
	...
	private async processDocument(
		...
		const embeddings = await this.generateEmbeddings(chunks, request.embed_model || 'BAAI/bge-small-en-v1.5');
	...
	async searchSimilar(query: string, options: {
		...
		const queryEmbedding = await this.generateEmbeddings([query], options.model || 'BAAI/bge-small-en-v1.5');
		...
		try {
			const results = await db.execute(query_sql, params);
