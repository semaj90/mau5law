...
import {
  ...
  const initialData = (data as ProfilePageData) ?? {};
  ...
  const apiOrigin = $derived(() => {
    const pick = (...keys: string[]): string | undefined => {
      for (const key of keys) {
        const dynamicValue = (publicEnv as Record<string, string | undefined>)[key];
        if (dynamicValue && dynamicValue.trim().length) {
          return dynamicValue;
        }
        const viteValue = (import.meta as any)?.env?.[key];
        if (typeof viteValue === 'string' && viteValue.trim().length) {
          return viteValue;
        }
      }
      return undefined;
    ...
    const candidate = pick('PUBLIC_API_BASE_URL', 'PUBLIC_API_PREFIX', 'PUBLIC_BACKEND_URL', 'VITE_API_ORIGIN');
    if (!candidate) return '';
    return candidate.endsWith('/') ? candidate.slice(0, -1) : candidate;
  });

  const dockerDiscoveryFlag = $derived(
    () => (publicEnv as Record<string, string | undefined>)['DEV_DOCKER_DISCOVERY'] ?? 'false'
  );
