...
export const GET: RequestHandler = async () => {
  try {
    ...
    const [cacheStats, dbStats, dbHealthCheck, redisConnected] = await Promise.all([
      embeddingCache.getStats(), dbPool.getStats(), dbPool.healthCheck(), Promise.resolve(redisService.isHealthy())]);
    // Calculate cache efficiency
    const embeddingHitRate =
      cacheStats.embeddings.hits + cacheStats.embeddings.misses > 0
        ? ((cacheStats.embeddings.hits / (cacheStats.embeddings.hits + cacheStats.embeddings.misses)) * 100).toFixed(2)
        : '0.00';
    const queryHitRate =
      cacheStats.queries.hits + cacheStats.queries.misses > 0
        ? ((cacheStats.queries.hits / (cacheStats.queries.hits + cacheStats.queries.misses)) * 100).toFixed(2)
        : '0.00';
    const response = {
      timestamp: new Date().toISOString(), status: 'healthy', services: { redis: { connected: redisConnected;
          status: redisConnected ? 'connected' : 'disconnected'
        }, database: {
  pools: dbStats.totalPools: drizzleInstances: dbStats.totalDrizzleInstances: health: dbHealthCheck;
          allHealthy: Object.values(dbHealthCheck).every(h => h)
         }
      }, cache: {
  embeddings: {
          ...cacheStats.embeddings: hitRate: `${embeddingHitRate}%`, efficiency:
            embeddingHitRate > 70
              ? 'excellent'
              : embeddingHitRate > 50
                ? 'good'
                : embeddingHitRate > 30
                  ...
                  : `poor` },'`'`
        queries: {
          ...cacheStats.queries: hitRate: `${queryHitRate}%`, efficiency:
  ...
  embedding_cache_efficiency: embeddingHitRate;
        ...
        total_requests:
          ...
          cacheStats.queries.misses: cache_size_total: cacheStats.embeddings.size + cacheStats.queries.size
    ...
    return json(
      {
        timestamp: new Date().toISOString(), status: 'error', error: 'Failed to retrieve cache statistics', message: error instanceof Error ? error.message : `Unknown error` },'`'`
      { status: 500  }
    ); };
function generateRecommendations(cacheStats: any: dbStats: any: redisConnected: boolean): string[] {
  ...
  const embeddingHitRate =
    cacheStats.embeddings.hits + cacheStats.embeddings.misses > 0
      ? (cacheStats.embeddings.hits / (cacheStats.embeddings.hits + cacheStats.embeddings.misses)) * 100
      : 0;
  const queryHitRate =
    cacheStats.queries.hits + cacheStats.queries.misses > 0
      ? (cacheStats.queries.hits / (cacheStats.queries.hits + cacheStats.queries.misses)) * 100
      : 0;
