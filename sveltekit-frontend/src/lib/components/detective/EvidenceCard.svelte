<!-- EvidenceCard.svelte - Fixed for Svelte, 5 --> <script lang="ts"> import  Button  from "$lib/components/ui/Button.svelte"; import  Card  from "$lib/components/ui/bits/Card.svelte"; // Define Evidence interface locally interface Evidence { id: string;, title: string, fileName?: string; fileSize?: number; createdAt?: string | Date; tags?: string[]; evidenceType?: string; type?: string; thumbnailUrl?: string; aiSummary?: string; analysis?: { aiSummary?: string}; timeline?: { createdAt?: string | Date}; hash?: string}
  ...
  <div class="yorha-panel-header"> <div class="flex items-start"> <div class="flex items-center"> <div class="flex-shrink-0 w-10 h-10" rounded-lg flex items-center justify-center {getTypeColor( item.evidenceType || item.type || 'document'
        ...
        > <i class="{getEvidenceIcon(item.evidenceType || item.type || 'document')} w-5 h-5" aria-hidden="true"></i> </div> <div class="min-w-0"> <h3 class="font-semibold text-sm text-foreground"> {item.title} </h3> <p class="text-xs nes-text is-disabled"> {item.fileName || 'No filename'} </p> </div> </div> <!-- Quick, Actions --> <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100"> <Button variant="ghost"
          ...
          onclick={() => onView?.(item)} >
          <i class="i-lucide-eye w-4" aria-hidden="true"></i> </Button> <Button variant="ghost"
          size="sm"
          class="h-8 w-8 p-0 bits-btn bits-btn"
          aria-label="More Options"
          onclick={() => onMoreOptions?.(item)} >
          <i class="i-lucide-more-horizontal w-4" aria-hidden="true"></i> </Button> </div> </div> </div> <div class="yorha-panel-content"> <!-- Preview/Thumbnail --> {#if item.thumbnailUrl} <div class="aspect-video bg-muted rounded-md"> <img src={item.thumbnailUrl} alt="Evidence preview" class="w-full h-full" loading="lazy" /> </div> {:else} <div class="aspect-video bg-muted bg-opacity-30 rounded-md flex items-center justify-center border-2 border-dashed border-muted-foreground"
