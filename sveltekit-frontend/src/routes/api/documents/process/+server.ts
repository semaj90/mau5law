...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // --- New: normalize/validate strict union types expected by DocumentProcessingConfig ---
    ...
    const normalizeOutputFormat = (raw: string | null | undefined): OutputFormat => {
      const s = String(raw ?? '')
        .trim()
        .toLowerCase();
    ...
    type ModelName =
      ...
      | 'legal-bert'
      | 'auto';
    const normalizeModel = (raw: string | null | undefined): ModelName => {
      const s = String(raw ?? '').trim();
      if (
        s === 'gemma3-legal:latest' ||
        s === 'embeddinggemma:latest' ||
        s === 'nomic-embed-text:latest' ||
        s === 'legal-bert' ||
        s === 'auto'
      ) {
    ...
    // Use the ProcessingResult exported by the unified-document-processor package
    let results: ProcessingResult;

    // Helper to safely compute total processing time for either a single result or an array
