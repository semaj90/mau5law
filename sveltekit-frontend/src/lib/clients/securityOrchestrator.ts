...
export async function validateSecurity(
    ...
    if (!res.ok) {
        ...
        const msg = detail && typeof detail === 'object' && 'error' in (detail as Record<string, unknown>)
    ...
    const validationId = typeof apiResponse['validationId'] === 'string'
    ...
    const verification = apiResponse['professionalVerification'] && typeof apiResponse['professionalVerification'] === 'object'
    ...
    const status = riskLevel === 'critical' ? 'deny' : riskLevel === 'high' ? 'review' : 'allow';

    const durationMs = typeof apiResponse['processingTime'] === 'number'
        ? (apiResponse['processingTime'] as number)
        : 0;

    const modelVersion = typeof apiResponse['modelVersion'] === 'string'
        ? (apiResponse['modelVersion'] as string)
        : 'unknown';

    const timestamp = typeof apiResponse['timestamp'] === 'string'
