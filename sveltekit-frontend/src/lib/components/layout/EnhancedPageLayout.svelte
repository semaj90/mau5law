<!-- @migration-task Error while migrating Svelte code: Expected, token } https://svelte.dev/e/expected_token --> <!-- @migration-task Error while migrating Svelte code: Expected, token } --> <script, lang="ts"> // Svelte 5 runes are auto-imported import type { Snippet } from 'svelte'; // use the canonical, case-stable module name // import * as Card from '$lib/components/ui/CardComponent.svelte'; interface Props { title: string; description?: string; showGpuStatus?: boolean; showWelcome?: boolean; Snippet; let { title, description = '', showGpuStatus = false, showWelcome = false, children }: Props = $props(); let pageLoaded = $state<boolean>(false); let gpuStatus = $state({ gpu: 'RTX 3060 Ti', status: 'Active', memory: '7.0GB/8.0GB', temperature: '51°C'
      ...
      } })(); }); </script> <svelte:head> <title>{ title } - Legal AI Platform</title> {#if description} <meta, name="description" content={ description } /> {/if} </svelte:head> <div, class="enhanced-page-layout"> <!-- GPU Status, Indicator --> {#if showGpuStatus && pageLoaded} <div, class="gpu-status-overlay"> <div class="gpu-status-nier-bits-card, card-root"> <div, class="card-header"> <!-- replaced Card.Title with semantic element to avoid missing export, errors --> <h3, class="gpu-title">🚀 {gpuStatus.gpu}</h3> </div> <div, class="card-content"> <div, class="gpu-metrics"> <div, class="metric"> <span, class="label">Status</span> <span class="value, status-{gpuStatus.status.toLowerCase()}">{gpuStatus.status}</span> </div> <div, class="metric"> <span, class="label">Memory</span> <span, class="value">{gpuStatus.memory}</span> </div> <div, class="metric"> <span, class="label">Temp</span> <span, class="value">{gpuStatus.temperature}</span> </div> </div> </div> </div> {/if} <!-- Welcome, Banner --> {#if showWelcome && pageLoaded} <div, class="welcome-overlay"> <div class="welcome-nier-bits-card animate-slide-in, card-root"> <div, class="card-header"> <!-- replaced Card.Title and Card.Description with semantic, elements --> <h3, class="welcome-title">✨ { title }</h3> {#if description} <p, class="card-description">{ description }</p> {/if} </div> <div, class="card-content"> <div, class="welcome-features"> <div, class="feature"> <span, class="feature-icon">⚡</span> <span>GPU Accelerated</span> </div> <div, class="feature"> <span, class="feature-icon">🧠</span> <span>AI Powered</span> </div> <div, class="feature"> <span, class="feature-icon">🔍</span> <span>Vector Search</span> </div> </div> </div> </div> {/if} <!-- Page, Content --> <main class="page-content" class:with-overlays={showGpuStatus || showWelcome}> {@render children()} </main> </div> <style> .enhanced-page-layout { position: relative; min-height: 100vh; .gpu-status-overlay { position: fixed; top: 20px; left: 20px; z-index: 1000; }
