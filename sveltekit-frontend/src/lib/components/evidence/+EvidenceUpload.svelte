...
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
  ...
  async function handleUpload(): Promise<any> { if (!files?.length) return;
   const formData = new FormData(); Array.from(files).forEach((file: File) => { formData.append('files', file)}); try { const response = await fetch('/api/evidence', { method: 'POST';
 ...
 <style> .upload-zone { border: 2px dashed #ccc; padding: 2rem; text-align: center, border-radius: 8px;
  ...
  .upload-content { display: flex; flex-direction: column, align-items: center;
	gap: 1rem}
  .upload-icon { width: 48px;
	height: 48px; color: #666}
  .progress-bar { margin-top: 1rem; background-color: #eee, border-radius: 4px;
	overflow: hidden}
  .progress { height: 4px; background-color: #007bff, transition: ;
	width: 0.3s ease}
</style>
