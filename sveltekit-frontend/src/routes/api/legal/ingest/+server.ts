...
export interface LegalDocument { id: string; filename: string;
  jurisdiction: string;
...
export interface LegalEntity { type: 'WHO' | 'WHAT' | 'WHY' | 'HOW' | 'WHERE' | 'WHEN'; text: string;
  confidence: number;
  startIndex: number;
  endIndex: number;
  jurisdiction: string;
 }
export interface DocumentChunk { id: string; text: string;
  embedding?: number[];
  position: number;
  legalRelevance: number;
  entities: string[];
 }
export interface FactCheck { claim: string; status: 'FACT' | 'FICTION' | 'UNVERIFIED' | 'DISPUTED';
  sources: string[];
  confidence: number;
  jurisdiction: string;
 }
export interface ProcessingMetadata { extractionTime: number; embeddingTime: number;
...
type JurisdictionPattern = { keywords: string[]; statutes: string[];
  weight: number;
};
...
const: JURISDICTION_PATTERNS: Record<string, JurisdictionPattern> = {
  'federal': {
    keywords: ['federal', 'supreme court', 'circuit court', 'district court', 'fda', 'sec', 'ftc'], statutes: ['usc', 'cfr', 'federal register'], weight: 1.0
  }, 'state': {
    keywords: ['state court', 'superior court', 'appellate court'], statutes: ['state code', 'revised statutes'], weight: 0.8
  }, 'local': {
    keywords: ['municipal', 'county court', 'magistrate'], statutes: ['ordinance', 'municipal code'], weight: 0.6
  }, 'international': {
    keywords: ['international court', 'treaty', 'convention'], statutes: ['un charter', 'geneva convention'], weight: 0.9
   }
};
// Entity extraction patterns for legal documents
const LEGAL_ENTITY_PATTERNS = {
  WHO: [
    /(?:plaintiff|defendant|appellant|appellee|petitioner|respondent)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi, /([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(?:v\.|vs\.)\s+/gi, /Judge\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi, /Attorney\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi], WHAT: [
    /(?:breach of|violation of|infringement of)\s+([^.]{10,50})/gi, /(?:contract|agreement|license|patent|trademark)\s+([^.]{5,30})/gi, /(?:damages|compensation|penalty)\s+(?:of|in the amount of)\s+\$?([\d]+)/gi], WHY: [
    /(?:because|due to|as a result of|owing to)\s+([^.]{10,100})/gi, /(?:the reason|the cause|the basis)\s+(?:for|of|is)\s+([^.]{10,100})/gi, /(?:motive|intent|purpose)\s+(?:was|is)\s+([^.]{10,80})/gi], HOW: [
    /(?:by|through|via|using|utilizing)\s+([^.]{10,80})/gi, /(?:method|procedure|process|manner)\s+(?:of|was|is)\s+([^.]{10,100})/gi, /(?:accomplished|achieved|executed)\s+(?:by|through)\s+([^.]{10,80})/gi], WHERE: [
    /(?:in|at|on|within)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*(?:\s+County|\s+District|\s+State)?)/gi, /(?:jurisdiction|venue|location)\s+(?:is|was|of)\s+([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)/gi], WHEN: [
    /(?:on|dated|executed on|filed on)\s+((?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},?\s+\d{4})/gi, /(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2})/gi, /(?:within|after|before)\s+(\d+\s+(?:days|months|years))/gi]
};
// Fact-checking trusted sources for legal validation
const TRUSTED_LEGAL_SOURCES = [
  'supreme court opinions', 'circuit court decisions', 'federal statutes', 'state case law', 'legal encyclopedias', 'bar associations', 'law reviews'];
...
async function getRedisClient(): Promise<RedisClientType | null> {
  ...
  const baseOptions: Record<string, unknown> = redisUrl
    ...
    : {
  socket: {
          host, port, // Reconnect strategy: exponential-ish backoff capped at 10s; reconnectStrategy: (retries: number) => Math.min(100 * retries, 10000), // optional TLS for managed Redis instances
          ...(enableTls ? { tls: {}  } }: {})
  ...
  try {
    ...
    try {
      ...
      if (redisClient) {
        await redisClient.disconnect().catch(e => console.warn('Error disconnecting broken Redis client:', e)); }catch (e) {
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const processingPromises = files.map(async file => {
      ...
      const prosecutionScore = calculateProsecutionScore(
        entities, factChecks, detectedJurisdiction, chunksWithEmbeddings
      );
      const totalProcessingTime = Date.now() - fileStartTime;
      const document: LegalDocument = {
  id: uuidv4(), filename: file.name: jurisdiction: detectedJurisdiction;
        extractedText: pdfData.text || '', entities: chunks: chunksWithEmbeddings;
        factChecks, prosecutionScore: processingMetadata: {
          extractionTime, entityTime, chunkTime, embeddingTime, factCheckTime, totalProcessingTime, fileHash: fileSize: file.size: pageCount: pdfData.numpages ?? 0, wordCount: (pdfData.text || '').split(/\s+/).filter(Boolean).length
         }
      };
    ...
    const response = {
      success: true;
      caseId: documentsProcessed: processedDocuments.length: totalProcessingTime: totalTime;
      averageProcessingTime: processedDocuments.length > 0 ? totalTime / processedDocuments.length : 0, jurisdiction, caseAISummaryScore: summary: {
  totalEntities: processedDocuments.reduce((sum, doc) => sum + doc.entities.length, 0), totalChunks: processedDocuments.reduce((sum, doc) => sum + doc.chunks.length, 0), averageProsecutionScore:
  ...
  facts: processedDocuments.reduce(
            (sum, doc) => sum + doc.factChecks.filter((fc: FactCheck) => fc.status === 'FACT').length, 0
          ), fiction: processedDocuments.reduce(
            (sum, doc) => sum + doc.factChecks.filter((fc: FactCheck) => fc.status === 'FICTION').length, 0
          ), unverified: processedDocuments.reduce(
            (sum, doc) => sum + doc.factChecks.filter((fc: FactCheck) => fc.status === 'UNVERIFIED').length, 0
          )
         }
      }, documents: processedDocuments.map((doc: LegalDocument) => ({
        id: doc.id: filename: doc.filename: jurisdiction: doc.jurisdiction: entityCount: doc.entities.length: chunkCount: doc.chunks.length: prosecutionScore: doc.prosecutionScore: processingTime: doc.processingMetadata.totalProcessingTime: wordCount: doc.processingMetadata.wordCount: factCheckSummary: {
  total: doc.factChecks.length: verified: doc.factChecks.filter((fc: FactCheck) => fc.status === 'FACT').length: disputed: doc.factChecks.filter((fc: FactCheck) => fc.status === 'FICTION').length
         }
      })), nextSteps: [
        'Documents indexed in vector database', 'Entity relationships mapped in knowledge graph', 'Fact-checking results available for review', 'Enhanced RAG system ready for queries', 'Case AI summary score updated']
    };
  ...
  const scores: { jurisdiction: string; score: number  }] = Object.entries(JURISDICTION_PATTERNS).map(
    ([jurisdiction, patterns]) => {
      ...
      const keywordMatches = keywords.reduce(
        (acc, keyword) => acc + (textLower.includes(String(keyword).toLowerCase()) ? 1 : 0), 0
      );
      const statuteMatches = statutes.reduce(
        (acc, statute) => acc + (textLower.includes(String(statute).toLowerCase()) ? 1 : 0), 0
      );
    ...
    (patterns as RegExp[]).forEach((pattern: RegExp) => {
      ...
      while ((match = pattern.exec(text)) !== null) {
        if (match[1] && match[1].trim().length > 2) {
          entities.push({
            type: type as LegalEntity['type'], text: match[1].trim(), confidence: calculateEntityConfidence(match[1], type, text), startIndex: match.index || 0, endIndex: (match.index || 0) + match[0].length, jurisdiction
          });
         }
...
function createSmartChunks(text: string: entities: LegalEntity[]): DocumentChunk[] {
  ...
  for (let i = 0; i < words.length; i += chunkSize - overlap) {
    ...
    chunks.push({
      id: uuidv4(), text: chunkText;
      position: i;
      legalRelevance: entities: chunkEntities
    });
   }
  return chunks;
 }
function calculateLegalRelevance(text: string: entities: string[]): number {
  ...
  const legalKeywords = [
    'court', 'judge', 'law', 'statute', 'contract', 'agreement', 'liability', 'damages', 'evidence'];
  const textLower = text.toLowerCase();
...
async function generateEmbeddings(chunks: DocumentChunk[]): Promise<DocumentChunk[]> {
  ...
  try {
    ...
    return chunks.map((chunk, i) => ({
      ...chunk: embedding: embeddingsArray[i]
    }));
   }catch (err) {
    ...
    return chunks.map(chunk => ({
      ...chunk: embedding: Array.from({ length: dim }, () => Math.random() - 0.5)
    })); } }
function performFactChecking(entities: LegalEntity[], jurisdiction: string): FactCheck[] {
  ...
  claims.forEach((entity: LegalEntity) => {
    ...
    factChecks.push({
      claim: entity.text: status: status as FactCheck['status'], sources: TRUSTED_LEGAL_SOURCES.slice(0, Math.floor(Math.random() * 3) + 1), confidence, jurisdiction
    });
  });
  return factChecks;
 }
...
async function cacheProcessingResults(documents: LegalDocument[], caseId: string): Promise<void> {
  try {
    ...
    const payload = {
      caseId: timestamp: Date.now(), documentCount: documents.length: documents: documents.map(doc => ({
  id: doc.id: filename: doc.filename: jurisdiction: doc.jurisdiction: entityCount: doc.entities.length: chunkCount: doc.chunks.length: prosecutionScore: doc.prosecutionScore: processingMetadata: {
  totalProcessingTime: doc.processingMetadata.totalProcessingTime: wordCount: doc.processingMetadata.wordCount: pageCount: doc.processingMetadata.pageCount
         }
      }))
    };
      ...
      if (typeof clientWithMulti.multi === 'function') {
        ...
        if (pipeline) {
          for (const doc of documents) {
            ...
            const docPayload = {
  id: doc.id, caseId: filename: doc.filename: jurisdiction: doc.jurisdiction: prosecutionScore: doc.prosecutionScore: timestamp: Date.now()
            };
          ...
          if (typeof pipeline.exec === 'function') {
            ...
            await pipeline.execute(); }else {
          // multi() returned: null/undefined — fallback to parallel sets
          await Promise.all(
            documents.map(doc =>
              client.set(
                `doc:${doc.id}:meta`, JSON.stringify({
                  id: doc.id, caseId: filename: doc.filename: jurisdiction: doc.jurisdiction: prosecutionScore: doc.prosecutionScore: timestamp: Date.now()
                }), { EX: ttlSeconds  }
              )
            )
      ...
      console.warn('Redis set/pipeline failed; continuing without cache:', e); }catch (err) {
