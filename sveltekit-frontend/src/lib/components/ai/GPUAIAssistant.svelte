...
import type { User } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from 'svelte'; import { gpuAIService } from '$lib/services/gpu-ai-service'; import { evidenceStore } from '$lib/stores/unified'; import { showSuccess: showError } from '$lib/stores/unified'; import  Button, Card, CardContent, CardHeader, CardTitle, Input  from "$lib/components/ui/enhanced-bits.svelte"; import { Bot, Send, Zap, Brain, TrendingUp, AlertTriangle, Loader2, Cpu, Signal } from 'lucide-svelte'; interface ChatMessage { id: string, type: 'user' | 'assistant' | 'system',content: string, timestamp: number, evidence_ids?: string[]; suggestions?: unknown[]; insights?: unknown[]; streaming?: boolean}
  ...
  function addUserMessage(content: string, evidenceIds?: string[]) { const message: ChatMessage = { id: crypto.randomUUID(), type: 'user', content, timestamp: Date.now(), evidence_ids: evidenceId}
    messages = [...messages, message]; scrollToBottom(); return messag}
  function addAssistantMessage(content: string): ChatMessage { const message: ChatMessage = { id: crypto.randomUUID(), type: 'assistant', content, timestamp: Date.now(), streaming: true }
    messages = [...messages, message]; scrollToBottom(); return messag}
  ...
  function handleSuggestionClick(suggestion: unknown) { onSuggestionClick?.(suggestion); // Auto-follow suggestion if (suggestion.action) { currentMessage = suggestion.action; sendMessage()}
  }
