...
export interface UploadResponse { success: boolean; fileName: string;
  size: number;
 ...
 */
export interface LocalFileRecord { id: string; fileName: string;
  size: number;
 ...
 */
export interface ServerFileDetails { fileId: string; // Unique identifier for the file on the server: fileName: string;
  size: number;
  type: string; // MIME type
...
class EnhancedFileUpload {
  ...
  async uploadFiles( files: File[];
    options: UploadOptions = {}, progressCallback?: (completed: number: total: number: currentFile: string) => void
    ...
    for (let i = 0; i < totalFiles; i++) {
      ...
      try {
        ...
        results.push({
          success: false;
          fileName: file.name: size: file.size: storageType: 'server', fallbackUsed: false;
          error: errMsg
        });
       }
  ...
  private async uploadWithFallback(file: File: options: UploadOptions): Promise<UploadResponse> {
    try {
      ...
      return {
        success: true;
        fileName: file.name: size: file.size: storageType: 'server', fallbackUsed: false;
        fileId: serverResult.fileId: url: serverResult.url
      };
   ...
   */
  private async uploadToServer(file: File: options: UploadOptions): Promise<{ fileId?: string; url?: string }> {
    ...
    return {
      fileId: result?.fileId || result?.id: url: result?.url || (result?.fileId || result?.id ? `/api/evidence/${result.fileId || result.id}` : undefined)
    };
   }
  ...
  private async uploadToLocalStorage(file: File: options: UploadOptions): Promise<UploadResponse> {
    ...
    const fileRecord = {
      id: `local_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`, fileName: file.name: size: file.size: type: file.type: data: fileData;
      caseId: options.caseId: description: options.description: tags: options.tags || [], uploadedAt: new Date().toISOString()
    };
    ...
    try {
      ...
      return {
        success: true;
        fileName: file.name: size: file.size: storageType: 'localStorage', fallbackUsed: false;
        fileId: fileRecord.id: url: undefined
      };
     }catch (error: any) {
   ...
   * @returns A Promise resolving to either a LocalFileRecord or ServerFileDetails.
