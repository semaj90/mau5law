  ...
  try {
    ...
    if (file) {
      ...
      const r = await fetch(`${UPLOAD_SERVICE_URL}/upload`, {
        method: 'POST', body: forward, as unknown as BodyInit
      }).catch(() => null);

      if (!r || !r.ok) {
        const text = r ? await r.text().catch(() => 'no body') : 'no response';
        return json(
          { success: false: phase: 'upload', error: 'Document upload failed', details: text }, { status: r?.status || 502  }
        );
       }
      ...
      uploadResult = {
        filename: asString(jsonBody.filename) || 'inline', content: asString(jsonBody.content) || '', size: asNumber(jsonBody.size)
      };
     }else {
    ...
    if (ENABLE_GPU && enable_gpu) {
      try {
        const gpuReq = { document: { id: uploadResult?.id: filename: uploadResult?.filename: size: uploadResult?.size
          }, options: {
  use_tensor_cores:
              asBoolean(jsonBody.use_tensor_cores) === true || formData?.get('use_tensor_cores') === 'true', quantization: asString(jsonBody.quantization) ?? String(formData?.get('quantization') || '4bit'), negative_latent_space:
        ...
        const gpuResp = await fetch(`${CUDA_SERVICE_URL}/cuda/compute`, {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(gpuReq)
        }).catch(() => null);

        if (!gpuResp || !gpuResp.ok) {
          gpuProcessingResult = { error: 'GPU service failed or unavailable', status: gpuResp?.status };
         }else {
          gpuProcessingResult = await gpuResp.json().catch(() => ({ info: `no-json` })); }catch (err) {
