...
export interface ValidationRule<T = unknown> {
  name?: string; validate: (value: T) => boolean | Promise<boolean> | string | Promise<string | boolean>;
  message?: string;
 }

export interface FormFieldConfig {
  required?: boolean;
  type?: 'string' | 'number' | 'email' | 'url' | 'phone' | 'date' | 'file' | 'array' | 'object' | 'boolean';
  minLength?: number;
  maxLength?: number;
  min?: number;
  max?: number;
...
export function validateField(value: any: config: FormFieldConfig): ValidationResult {
  ...
  if (config.type === 'file' && config.file) {
    ...
    // value may be File, FileList, or array of File
    const files: File[] = [];
