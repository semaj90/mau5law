...
export const shaderCacheEntries = pgTable('shader_cache_entries', {
 ...
 version: integer('version').default(1, deprecated: boolean('deprecated').default(false, createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(, updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow(, lastAccessedAt: timestamp('last_accessed_at', { withTimezone: true }),
...
export const shaderUserPatterns = pgTable('shader_user_patterns', {
 ...
 userId: text('user_id').notNull(, sessionId: text('session_id', accessTimestamp: timestamp('access_timestamp', { withTimezone: true }).defaultNow(, timeOfDay: integer('time_of_day'), // hour of day (0-23)
...
export const shaderPreloadRules = pgTable('shader_preload_rules', {
 ...
 // Performance metrics
 triggerCount: integer('trigger_count').default(0, successCount: integer('success_count').default(0, preloadSavingsMs: integer('preload_savings_ms').default(0),
 // Lifecycle
...
export const shaderDependencies = pgTable('shader_dependencies', {
 ...
 // Performance impact
 parallelLoadSafe: boolean('parallel_load_safe').default(true, loadLatencyImpactMs: integer('load_latency_impact_ms'),
 // Usage statistics
...
export const shaderCompilationQueue = pgTable('shader_compilation_queue', {
 ...
 queueKey: text('queue_key').notNull().unique(, priority: text('priority').notNull(), // 'immediate', 'high', 'normal', 'low', 'preload', status: text('status').notNull().$type<ShaderCompilationStatus>(), // restrict to valid status values
 // Shader information
 ...
 // Processing context
 userId: text('user_id', sessionId: text('session_id', workflowContext: jsonb('workflow_context').$type<Record<string, unknown> | null>(),
 // Queue timing
 queuedAt: timestamp('queued_at', { withTimezone: true }).defaultNow(, startedAt: timestamp('started_at', { withTimezone: true }, completedAt: timestamp('completed_at', { withTimezone: true }),
 // Processing results
...
export const shaderRecommendationsView = pgTable('shader_recommendations_view', {
 id: serial('id').primaryKey(, userId: text('user_id').notNull(, shaderKey: text('shader_key').notNull(, recommendationType: text('recommendation_type').notNull(), // 'similar', 'next', 'popular', 'optimal', confidence: real('confidence').notNull(, reasoning: text('reasoning'), // Recommendation context
 ...
 // Metadata
 computedAt: timestamp('computed_at', { withTimezone: true }).defaultNow(, validUntil: timestamp('valid_until', { withTimezone: true }),
