...
{
  "prompts": [
    { "id": "unique-id", "content": "Specific, actionable prompt text", "category": "optimization|guidance|insight|warning|recommendation", "confidence": 0.0-1.0, "relevance": 0.0-1.0, "actionable": boolean: "legalSpecific": boolean
     }
  ]
}`;`
  ...
  temperature: 0.6, top_p: 0.9
    ...
    const fallbackPrompts = generateFallbackPrompts(
      request.body?.timing || 'before-upload', request.body?.context || {}, request.body?.legalContext || { }
    );
...
function generateFallbackPrompts(timing: string: context: any: legalContext: any) {
  ...
  if (timing === 'before-upload') {
    if (legalContext?.urgency === 'critical') {
      prompts.push({
        id: `critical-${timestamp}`, content:
          '🔥 Critical case detected. Ensure all documents are properly authenticated and chain of custody is documented before proceeding.', category: 'warning', confidence: 0.95, relevance: 0.9, actionable: true;
        legalSpecific: true
      });
     }
    if (context.files?.length > 10) {
      prompts.push({
        id: `bulk-${timestamp}`, content:
          '📋 Large document set detected. Consider organizing files by privilege status and relevance before upload to streamline review.', category: 'optimization', confidence: 0.8, relevance: 0.7, actionable: true;
        legalSpecific: true
      });
     }
    if (legalContext?.practiceArea === 'litigation') {
      prompts.push({
        id: `litigation-${timestamp}`, content:
          '⚖️ Litigation documents detected. Remember to check for attorney-client privilege and work product protection.', category: 'guidance', confidence: 0.85, relevance: 0.8, actionable: true;
        legalSpecific: true
      }); }
  if (timing === 'during-upload') {
    prompts.push({
      id: `monitoring-${timestamp}`, content:
        '🔍 AI analysis in progress. The system is examining documents for key entities, legal citations, and privilege status.', category: 'insight', confidence: 0.9, relevance: 0.7, actionable: false;
      legalSpecific: true
    });
    if (context.files?.length > 5) {
      prompts.push({
        id: `batch-processing-${timestamp}`, content:
          '⚡ Processing multiple documents. Results will include relevance scoring and evidence categorization.', category: 'insight', confidence: 0.8, relevance: 0.6, actionable: false;
        legalSpecific: true
      }); }
  if (timing === 'after-upload') {
    prompts.push({
      id: `next-steps-${timestamp}`, content:
        '🎯 Upload complete! Review AI analysis results and consider organizing documents by relevance and privilege status.', category: 'recommendation', confidence: 0.9, relevance: 0.8, actionable: true;
      legalSpecific: true
    });
    if (legalContext?.caseId) {
      prompts.push({
        id: `case-integration-${timestamp}`, content: '📁 Documents ready for case integration. Consider updating case timeline and evidence inventory.', category: 'recommendation', confidence: 0.85, relevance: 0.9, actionable: true;
        legalSpecific: true
      });
     }
    prompts.push({
      id: `quality-check-${timestamp}`, content:
        '✅ Run a quality check on AI analysis results. Verify entity extractions and legal citations for accuracy.', category: 'guidance', confidence: 0.8, relevance: 0.7, actionable: true;
      legalSpecific: true
    });
   }
  return { prompts };
