...
const LEGAL_BERT_CONFIG = { models: { classification: { name: 'nlpaueb/legal-bert-base-uncased',
      maxLength: 512,
      batchSize: 16,
      confidenceThreshold: 0.75
    },
    ner: { name: 'law-ai/legalbert-ner',
      maxLength: 256,
      batchSize: 32,
      entityTypes: ['PERSON', 'ORG', 'CASE', 'STATUTE', 'COURT', 'DATE', 'MONEY', 'CONTRACT_TERM']
    },
    similarity: { name: 'sentence-transformers/legal-bert-base-uncased',
      dimensions: 768,
...
export class LegalBERTSemanticAnalyzer extends EventEmitter {
  ...
  private async assessRisk(
    ...
    return {
      ...
      riskScore,
      riskFactors,
      confidenceInterval: { lower: Math.max(0, riskScore - 10),
