...
describe('LLMService', () => {
  ...
  describe('error handling', () => {
    it('should handle LLM API errors gracefully', async () => {
      ...
      // Mock API error
      vi.mocked(llmService.generateSummary).mockRejectedValueOnce(
        new Error('LLM API error')
      );

      await expect(llmService.generateSummary(context)).rejects.toThrow(
        'LLM API error'
      );
    });
  ...
  describe('citation extraction accuracy', () => {
    it('should extract statute citations with jurisdiction', async () => {
      ...
      const federalCitations = citations.filter(
        (c: any) => c.jurisdiction === 'Federal'
      );
      const stateCitations = citations.filter(
        (c: any) => c.jurisdiction === 'California'
      );
