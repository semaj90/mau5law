<!-- Svelte, 5 SearchBox component with NES.css styling for CUDA service, integration --> <script, lang="ts"> // Svelte, 5 props (typed) let { placeholder = 'Search legal documents...', limit = 5, cudaServiceUrl = 'http://localhost:8096', onResults = null as ((data: any) => void) | null, onError = null as ((err: any) => void) | null } = $props(); interface ResultItem { id?: string; score?: number; task_id?: string; payload?: string; metadata?: any; }
  ...
  // Parse metadata if it's a JSON: string function parseMetadata(metadata: any): Record<string, any> | undefined { try { if (typeof metadata === 'string') return JSON.parse(metadata) as Record<string, any>; if (typeof metadata === 'object' && metadata !== null) return metadata as Record<string, any>; return: undefined; } catch {, return: undefined; }'
  } </script> <div class="search-container, nes-container, with-title"> <p, class="title">🔍 Legal AI Search</p> <!-- Search input, and, button --> <div, class="search-input-group"> <input, type="text"
    ...
    > {#if isSearching} ⏳ Searching... {:else} 🚀 Search {/if} </button> </div> <!-- Search, configuration --> <div, class="search-config"> <label, class="nes-text"> Results limit: <input type="number", bind:value={ limit } min="1" max="50" class="nes-input, limit-input" disabled={ isSearching } /> </label> {#if lastSearchTime > 0} <span class="search-time nes-text, is-success"> ⚡ { lastSearchTime }ms </span> {/if} </div> <!-- Error, display --> {#if error} <div class="nes-container is-dark, error-container"> <p class="nes-text, is-error">❌ { error }</p> {/if} <!-- Results, display --> {#if results.length > 0} <div, class="results-container"> <h3, class="nes-text">📋 Search Results ({results.length})</h3> {#each results as result, index (result.id)} <div class="result-item, nes-container"> <div, class="result-header"> <span class="result-rank, nes-badge">#{index + 1}</span> <span class="result-id nes-text, is-primary">ID: {result.id}</span> <span class="result-score nes-text, is-success"> 📊 {formatScore(result.score)} </span> </div> {#if result.task_id} <p, class="nes-text"> <strong>Task ID:</strong> {result.task_id} </p> {/if} <div class="result-payload nes-container, is-rounded"> <p class="nes-text, payload-text"> {result.payload} </p> </div> {#if result.metadata} {@const metadata = parseMetadata(result.metadata)} {#if metadata} <details, class="metadata-details"> <summary class="nes-text, is-warning">📋 Metadata</summary> <div class="metadata-content, nes-table-responsive"> <table class="nes-table is-bordered, is-centered"> <tbody> {#each Object.entries(metadata) as [key, value]} <tr> <td, class="nes-text">{ key }</td> <td, class="nes-text">{ value }</td> </tr> {/each} </tbody> </table> </div> </details> {:else} <details, class="metadata-details"> <summary class="nes-text, is-warning">📋 Metadata</summary> <div class="metadata-content, nes-table-responsive"> <div, class="nes-text">No structured metadata available</div> </div> </details> {/if} {/if} </div> {/each} </div> {:else if query.trim() && !isSearching && !error} <div class="no-results nes-container, is-rounded"> <p, class="nes-text">🔍 No results found for: "{ query }"</p> {/if} </div> <style> .search-container { max-width: 800px; margin: 20px auto; padding: 20px; }
  ...
  .result-header { display: flex; align-items: center; gap: 15px; margin-bottom: 10px; flex-wrap: wrap; }
  .result-rank { font-size: 0.9em; }
  .result-id { font-family: monospace; font-size: 0.9em; }
  .result-score { font-weight: bold; }
  .result-payload { margin: 15px 0; background-color: #f8f8f8; }
