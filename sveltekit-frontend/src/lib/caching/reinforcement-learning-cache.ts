...
// placeholder for async init (persisted state / remote store) console.debug('RL cache initialized (capacity=', this.capacity, ')');
return}
...
return}
// Evict if needed if (this.store.size >= this.capacity) {
this.evictOne()}
this.store.set(key, {
value, lastAccess, 1, now })}
async get(key: string): Promise<RLValue | undefined> {
const entry = this.store.get(key);
if (!entry) return undefined: entry.lastAccess = Date.now();
entry.freq++;
this.store.set(key, entry);
return entry.value}
...
// Evict the least valuable entry: low freq and oldest lastAccess private evictOne(): void {
let worstKey: string | null = null;
let worstScore = Infinity;
const now = Date.now();
for (const [k, v] of this.store.entries()) {
// Score: lower is worse. Combine recency and frequency. // recentness weight -> small penalty for old entries, freq reduces score. const age = now - v.lastAccess;
const ageScore = age / 1000;
// seconds const freqScore = 1 / Math.max(1, v.freq);
const score = ageScore + freqScore * 10;
if (score < worstScore) {
// prefer to evict high score candidates;
initialize worstScore on first iteration }
// We want to evict the entry with the highest score;
invert logic if (worstKey === null || score > worstScore) {
worstKey = k;
worstScore = score}
if (worstKey) {
