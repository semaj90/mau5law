...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    const { prompt, options: requestOptions = {}, mode = 'comprehensive' } = body;
    ...
    const response = formatResponse(result, requestOptions.outputFormat || 'structured');
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (mode === 'examples') {
    return json({ examples: getCopilotUsageExamples() });
  }
  return json({
...
function formatResponse(result: SelfPromptResult, outputFormat: string) {
  switch (outputFormat) {
    ...
    case 'markdown':
      return { analysis: formatAsMarkdown(result), data: result, format: 'markdown' };
    case 'structured':
    default:
      return {
...
function validateRequestBody(body: any): { isValid: boolean; errors: string[] } {
  ...
  if (body.mode && !['quick', 'comprehensive', 'autonomous'].includes(body.mode)) {
    errors.push('mode must be one of "quick", "comprehensive", or "autonomous"');
  }
