 ...
 */
export function emitUnifiedDiff(
	filePath: string,
	before: string,
	after: string
): string {
	...
	while (i < beforeLines.length || j < afterLines.length) {
		...
		while (hunkEndBefore < beforeLines.length) {
			if (hunkEndAfter < afterLines.length && beforeLines[hunkEndBefore] === afterLines[hunkEndAfter]) {
				break;
			}
		...
		while (hunkEndAfter < afterLines.length) {
			if (hunkEndBefore < beforeLines.length && beforeLines[hunkEndBefore] === afterLines[hunkEndAfter]) {
				break;
			}
