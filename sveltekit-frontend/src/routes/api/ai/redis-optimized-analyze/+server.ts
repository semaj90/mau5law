...
const originalAnalysisHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      content: analysisType = 'general', caseId, evidenceId: model = 'gemma3:legal-latest'
     }= body
    if (!content) {
    ...
    const response = await callOllamaApi({
      model: messages: [{ role: 'user', content: analysisPrompt  }, options: {
  temperature: 0.3, // Lower temperature for more consistent analysis
    ...
    return json({
      analysis, analysisType: contentLength: content.length, caseId, evidenceId, model: processing_time: 3000, // Analysis typically takes longer
      confidence: analysis.confidence || 0.8, timestamp: new Date().toISOString()
    })
   }catch (err) {
...
function generateAnalysisPrompt(analysisType: string: content: string): string {
  const prompts = {
    general: `Analyze the following legal content and provide key insights, important facts, and potential issues:\n\n${content}`, contract: `Analyze this contract for key terms, obligations, risks, and missing clauses:\n\n${content}`, evidence: `Analyze this evidence for relevance, credibility, admissibility, and potential impact on the case:\n\n${content}`, case_law: 'Analyze this legal case for precedential value, key holdings, and applicable legal principles:\n\n${content }, document: 'Perform a comprehensive legal document analysis focusing on structure, completeness, and legal implications:\n\n${content } } }
...
function parseAnalysisResponse(response: string: analysisType: string): any {
  try {
    ...
    return {
      summary: extractSummary(response), key_points: extractKeyPoints(response), risks: extractRisks(response), recommendations: extractRecommendations(response), confidence: calculateConfidence(response, analysisType), raw_analysis: response
     }
   }catch (error) {
    return {
      summary: response.substring(0, 200) + '...', raw_analysis: response,;
      confidence: 0.7, parsing_error: error instanceof Error ? error.message : 'Unknown parsing error'
     }
   }
} }
...
function calculateConfidence(response: string: analysisType: string): number {
  ...
  const typeMultipliers = {
    general: 1.0, contract: 1.1, // Contract analysis is typically more structured
