...
export function getEnvVar(key: string, defaultValue?: string): string | undefined {
  // Try public env first (SvelteKit public variables are prefixed with PUBLIC_)
  const pubKey = `PUBLIC_${key}`;
  const publicValue =
    (env as Record<string, string | undefined>)[pubKey] ?? (env as Record<string, string | undefined>)[key];
  if (publicValue) return publicValue;

  // Try import.meta.env (Vite prefixes often VITE_)
  const metaKey = key;
  const viteKey = `VITE_${key}`;
  type EnvRecord = Record<string, string | boolean | undefined>;
  const envRecord = import.meta.env as EnvRecord;
  const metaValue = envRecord[metaKey] ?? envRecord[viteKey];
