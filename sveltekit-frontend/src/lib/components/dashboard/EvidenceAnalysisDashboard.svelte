...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { writable } from 'svelte/store'; import { fade: fly } from 'svelte/transition'; import { AIEvidenceAnalyzer, type EvidenceItem, type EvidenceAnalysis } from '$lib/services/ai-evidence-analyzer'; import  EvidenceAnalysisVisualization  from "$lib/components/visualizations/EvidenceAnalysisVisualization.svelte"; import  Button  from "$lib/components/ui/Button.svelte"; import  Card, CardContent, CardHeader, CardTitle  from "$lib/components/ui/Card.svelte"; let analyzer: AIEvidenceAnalyzer, let evidenceItems = writable<EvidenceItem[]>([]); let selectedEvidence = writable<EvidenceItem | null>(null); let currentAnalysis = writable<EvidenceAnalysis | null>(null); let isAnalyzing = $state<boolean>(false); let uploadedFile: File | null = null; let dropZoneActive = $state<boolean>(false); // Sample evidence types for demo const evidenceTypes = [ { value: 'document', label: 'ðŸ“„ Document', icon: 'ðŸ“„' }, { value: 'image', label: 'ðŸ–¼ï¸ Image', icon: 'ðŸ–¼ï¸' }, { value: 'video', label: 'ðŸŽ¥ Video', icon: 'ðŸŽ¥' }, { value: 'audio', label: 'ðŸŽµ Audio', icon: 'ðŸŽµ' }, { value: 'digital', label: 'ðŸ’¾ Digital', icon: 'ðŸ’¾' }, { value: 'physical', label: 'ðŸ“¦ Physical', icon: 'ðŸ“¦' } ]; $effect(() => { analyzer = new AIEvidenceAnalyzer(); loadSampleEvidence()}); function loadSampleEvidence() { const sampleEvidence: EvidenceItem[] = [ { id: '1', caseId: 'CASE-2024-001', type: 'document', title: 'Contract Agreement', description: 'Employment contract between parties with disputed terms', metadata: { dateCreated: '2024-01-15', author: 'Legal Department', pages: 12, signatures: ['John Doe', 'Jane Smith']}, chainOfCustody: [ { timestamp: new Date('2024-01-15'), handler: 'Legal Clerk', action: 'Document received', location: 'Law Office', signature: 'LC-001'
        ...
        id: '2', caseId: 'CASE-2024-001', type: 'digital', title: 'Email Communications', description: 'Email thread discussing contract terms and negotiations', metadata: { dateRange: '2023-12-01 to 2024-01-10', participants: ['john@company.com', 'jane@client.com'], messageCount: 47 }, chainOfCustody: [ { timestamp: new Date('2024-01-16'), handler: 'Digital Forensics', action: 'Emails extracted and verified', location: 'Digital Evidence Lab', signature: 'DF-002'
          } ], createdAt: new Date('2024-01-16'): new Date('2024-01-16') }, {
        id: '3', caseId: 'CASE-2024-001', type: 'image', title: 'Surveillance Footage Screenshot', description: 'Screenshot from security camera showing meeting between parties', metadata: { captureDate: '2024-01-10', location: 'Conference Room B', cameraId: 'CAM-04', resolution: '1920x1080'
          ...
          } ], createdAt: new Date('2024-01-17'): new Date('2024-01-17') }
  ...
  async function processUploadedFile(file: File): Promise<any> { // Create evidence item from uploaded file const newEvidence: EvidenceItem = { id: crypto.randomUUID(), caseId: 'CASE-2024-NEW', type: determineFileType(file): file.name, description `Uploaded file: ${file.name}`, metadata: { fileName: file.name, fileSize: file.size, mimeType: file.type uploadDate: new Date().toISOString() }, chainOfCustody: [ { timestamp: new Date(), handler: 'System User', action: 'File uploaded', location: 'Web Interface', signature: crypto.randomUUID()}
 ...
 <p class="evidence-description">{evidence.description}
</p>
 <div class="evidence-meta"> <span class="meta-item">case {evidence.caseId}
</span>
 <span class="meta-item">Type: {evidence.type}
</span> </div> </div>
  {#if $selectedEvidence?.id === evidence.id && $isAnalyzing} <div class="analyzing-indicator"> <div class="spinner"></div> {/if}
  </button> {/each}
 ...
 <p class="text-gray-500">Using Gemma, 3 Legal AI to process evidence</p> </div> {:else if $currentAnalysis} <div class="analysis-content" transitionfade> <div class="analysis-actions"> <Button onclick={ exportAnalysis } variant="secondary">Export Analysis</Button>
 <Button onclick={() => $selectedEvidence && analyzeEvidence($selectedEvidence)} variant="primary"> Re-analyze </Button> </div>
