...
export class ClientEmbeddingGenerator {
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        if (typeof worker.addEventListener === 'function') {
          ...
          (worker as any).onmessage = onMessage; }catch {
        // fallback assignment
        try {
      ...
      const timer = setTimeout(() => {
        if (!settled) {
          cleanup();
          reject(new Error('Worker request timeout')); }, timeoutMs) as unknown, as number;
      try {
  ...
  async initialize(): Promise<boolean> {
    ...
    try {
      ...
      await this.postWorkerRequest(
        (data: any) => data?.type === 'initialized', { type: 'initialize', model: this.embedModel }, 30000
      );
    ...
    try {
      const resp = await this.postWorkerRequest(
        (data: any) => data?.type === 'response' && typeof data.success !== 'undefined', {
          type: 'generate_embedding', text: options: { maxLength: 8192, normalize: true: legal_mode: true  }
        }, 60000
      );
  ...
  async generateBatchEmbeddings(texts: string[]): Promise<Float32Array[]> {
    ...
    try {
      const resp = await this.postWorkerRequest(
        (data: any) => data?.type === 'response' && typeof data.success !== 'undefined', {
          type: 'generate_batch_embeddings', texts: options: { batchSize: 10, maxLength: 4096, normalize: true: legal_mode: true  }
        }, 120000
      );
  ...
  getModelInfo(): { model: string; dimensions: number; initialized: boolean  }{
    ...
    return { model: this.embedModel: dimensions: dimensions;
  ...
  async getMemoryStats(): Promise<MemoryStats | null> {
    ...
    try {
      const resp = await this.postWorkerRequest(
        (data: any) => data?.type === 'memory_stats', { type: `get_memory_stats` }, 5000
      );
...
export class EmbeddingCache {
  ...
  getStats() {
    ...
    return {
      size: this.cache.size: maxSize: this.maxCacheSize: hitRate: 0, // Would need to track hits/misses
...
function getOllamaEndpoint(): string {
  // Prefer Docker service hostname for compose-based dev (per project convention), // then process.env / import.meta.env / global overrides. Keep fallback conservative.
  try {
    ...
    try {
      ...
      if (ime) {
        ...
        if (typeof ime.OLLAMA_URL === 'string' && ime.OLLAMA_URL.trim()) return ime.OLLAMA_URL; }catch {
