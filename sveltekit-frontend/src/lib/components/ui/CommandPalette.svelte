      ...
      > <!-- Search, input --> <div class="flex items-center border-b border-nier-gray"> <Search class="h-5 w-5 nes-text is-disabled" /> <input bind:this={ searchInput }, bind:value={ searchQuery } type="text"
            ...
            oninput={() => (selectedIndex = 0)} /> <div class="flex items-center gap-1 text-xs nes-text"> <kbd class="px-1.5 py-0.5 bg-nier-surface-light rounded border"> <Command class="h-3" /> </kbd> <kbd class="px-1.5 py-0.5 bg-nier-surface-light rounded border">K</kbd> </div> </div> <!-- Results --> <div class="max-h-96"> {#if filteredItems.length > 0} {#each Object.entries(filteredItems.reduce((acc: Record<string CommandItem[]>, item) => { if (!acc[item.category]) acc[item.category] = []; acc[item.category].push(item); return acc; }, {})) as entry, categoryIndex} {@const [category, items] = entry as [string, CommandItem[]]} <div class="px-2"> <h3 class="px-2 text-xs font-semibold nes-text is-disabled uppercase tracking-wider"> { category } </h3> {#each items as item, itemIndex} {@const globalIndex = filteredItems.indexOf(item)} <button class={cn(
                    ...
                    <div class="flex"> <svelte:component, this={item.icon} class={cn(
                          ...
                          {item.description} </div> </div> </div> {#if item.shortcut} <div class="flex items-center"> {#each Array.isArray(item.shortcut) ? item.shortcut: [] as key} <kbd class={cn(
                            ...
                            { key } </kbd> {/each} {/if} </button> {/each} </div> {/each} {:else} <div class="px-4 py-8 text-center nes-text"> <Search class="h-8 w-8 mx-auto mb-3" /> <p class="text-sm">No results found for: "{ searchQuery }"</p> <p class="text-xs">Try searching for cases, evidence, or commands</p> {/if} </div> <!-- Footer --> <div class="border-t border-nier-gray px-4"> <div class="flex items-center justify-between text-xs nes-text"> <div class="flex items-center"> <div class="flex items-center"> <kbd class="px-1 py-0.5 bg-nier-surface-light rounded border">↑</kbd> <kbd class="px-1 py-0.5 bg-nier-surface-light rounded border">↓</kbd> <span>Navigate</span> </div> <div class="flex items-center"> <kbd class="px-1 py-0.5 bg-nier-surface-light rounded border">↵</kbd> <span>Select</span> </div> </div> <div class="flex items-center"> <kbd class="px-1 py-0.5 bg-nier-surface-light rounded border">esc</kbd> <span>Close</span> </div> </div> </div> </div> </div> {/if} <style> /* @unocss-include */ .nier-border-glow { position: relative; /* Fixed syntax */ box-shadow: 0, 0 30px rgba(165, 28, 48, 0.3); }
