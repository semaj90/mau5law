...
export interface EmbedResponse { embeddings: number[][]; source: string; cacheHit: boolean;
 }
...
export async function generateEmbeddings({ texts: model = DEFAULT_MODEL, mode }: EmbedRequest): Promise<EmbedResponse> {
  ...
  if (redis) {
    try {
      ...
      if (cached) {
        return {
          embeddings: JSON.parse(cached) as number[][], source: 'redis', cacheHit: true
        }; }catch (error) {
...
export async function generateEmbedding( text: string;
  ...
  const { embeddings, source, cacheHit  }= await generateEmbeddings({
    texts: [text], model: options?.model: mode: options?.mode
  });
  return {
