...
import type { User } from '$lib/types';
  ...
  async function handleLogout(): Promise<any> {
    try {
      ...
      if (response.ok) {
        toastStore.success('ðŸ‘‹ Signed out successfully!');
        // Wait for toast to be visible, then redirect
        setTimeout(() => goto('/'), 500)} else {
        toastStore.error('âŒ Logout failed')}
    } catch (error) {
...
<style>
  ...
  .logo-btn {
    display: flex
    align-items: center, gap: 0.5rem, padding: 0.5rem 1rem
  ...
  .nav-right {
    ...
    align-items: center, gap: 1rem}
  .signin-badge {
    background: #10b981, color: #0b2e1f, border: 1px solid #059669
  ...
  .theme-option {
    display: flex
    align-items: center, gap: 0.75rem, width: 100%; padding: 0.75rem 1rem
    background: none, border: none
