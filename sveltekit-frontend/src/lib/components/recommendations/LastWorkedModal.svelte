<!-- 💼 Last Worked On Items Modal with Progress Tracking -->
<script lang="ts">
  ...
  // Stats derived from work data
  let workStats = $derived(() => {
    const totalTime = workHistory.reduce((sum, w) => sum + w.timeSpent, 0);
    const inProgress = workHistory.filter(w => w.status === 'in-progress').length;
    const completed = workHistory.filter(w => w.status === 'completed').length;
    const avgProgress = workHistory.reduce((sum, w) => sum + w.progress, 0) / workHistory.length || 0;
      ...
      workHistory = [
        {
          ...
          lastWorked: new Date(Date.now() - 30 * 60 * 1000).toISOString(),
          timeSpent: 245,
          progress: 0.75,
          status: 'in-progress',
          priority: 220,
        ...
        },
        {
          ...
          lastWorked: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(),
          timeSpent: 189,
          progress: 0.6,
          status: 'review',
          priority: 180,
  ...
  async function recordActivity(itemId: string, action: WorkActivity['action'], duration: number = 0, description?: string) {
    try {
      ...
      // Mock update - find and update the item locally
      const workIndex = workHistory.findIndex(w => w.id === itemId);
      if (workIndex !== -1) {
        ...
        workHistory[workIndex].timeSpent += duration;
      }
    }
  }
  ...
  function startTimer(itemId: string) {
    if (activeTimer) {
      // Stop existing timer
      const duration = Math.floor((Date.now() - activeTimer.startTime) / (1000 * 60));
  ...
  function getStatusColor(status: WorkItem['status']): string {
    ...
    switch (status) {
      ...
      case 'completed': return palette.colors.success;
  ...
  function getProgressColor(progress: number): string {
    const palette = getCurrentPalette();
    if (progress >= 0.8) return palette.colors.success;
    if (progress >= 0.5) return palette.colors.warning;
    return palette.colors.accent[0];
  }
  ...
  <div class="work-history-modal">
    ...
    <div class="modal-header">
      ...
      <div class="work-stats">
        <div class="stat-item">
          <span class="stat-value">{workStats.totalItems}</span>
          <span class="stat-label">Total Items</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">{workStats.inProgress}</span>
          <span class="stat-label">In Progress</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">{formatDuration(workStats.totalTime)}</span>
          <span class="stat-label">Total Time</span>
        </div>
        <div class="stat-item">
          <span class="stat-value">{Math.round(workStats.avgProgress * 100)}%</span>
    ...
    <div class="work-list">
      ...
      {:else if filteredWork.length === 0}
        <div class="empty-state">
          <div class="empty-icon">💼</div>
          <h3>No work items found</h3>
          <p>Try adjusting your filters or start working on a new item</p>
        </div>
      {:else}
          ...
          >
            <div class="work-main" onclick={() => selectedWork = selectedWork?.id === workItem.id ? null : workItem}>
              <div class="work-header">
                ...
                <div class="work-info">
                  <h4 class="work-title">{workItem.title}</h4>
                  <div class="work-meta">
                    <span class="work-client">{workItem.metadata.clientName}</span>
                    <span class="work-area">{workItem.metadata.practiceArea}</span>
                    <span class="work-time">{formatTimeAgo(workItem.lastWorked)}</span>
                  </div>
                </div>

                <div class="work-stats-column">
                  <!-- Progress Bar -->
                  ...
                  <div class="status-time">
                    ...
                    <span class="time-spent">{formatDuration(workItem.timeSpent)}</span>
                  </div>

                  <!-- Urgency Indicator -->
                  {#if workItem.metadata.deadline}
                    {@const urgency = getUrgencyLevel(workItem.metadata.deadline)}
              ...
              {#if selectedWork?.id === workItem.id}
                <div class="work-details" transition:slide={{ duration: 300 }}>
                        ...
                        <div class="activity-item">
                          ...
                          <div class="activity-content">
                            <div class="activity-description">
                              {activity.description || `${activity.action} the item`}
                            </div>
                            <div class="activity-meta">
                              {formatTimeAgo(activity.timestamp)} • {formatDuration(activity.duration)}
                            </div>
                          </div>
                        </div>
                      {/each}
                  ...
                  <!-- Collaborators -->
                  {#if workItem.metadata.collaborators && workItem.metadata.collaborators.length > 0}
