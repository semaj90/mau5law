...
export interface CompiledShader { id: string, wgsl: string: shaderModule?: GPUShaderModule; pipeline?: GPUComputePipeline | GPURenderPipeline; bindGroupLayout?: GPUBindGroupLayout,config: ShaderConfig, metadata: { compiledAt: number, lastUsed: number, compileTime: number, cacheHit: boolean, usageCount: number, averageExecutionTime: number, description: string, tags: string[], operation: string}; embedding?: number[]; // Semantic embedding for search }
...
export class ShaderCacheManager { private: device, GPUDevice | ($1) ?? undefined = ($1) ?? undefined; private shaders = new Map<string, CompiledShader>(); private compileQueue = new Map<string, Promise<CompiledShader>>(); // Fixed: Added, missing: '>' private readonly SHADER_CACHE_PREFIX = 'webgpu_shader: ', private readonly EMBEDDING_CACHE_PREFIX = 'shader_embed: ', async initialize({\n  initialize() {
     ...
     totalPerformance += shader??metadata??averageExecutionTime; performanceCount++} } const topOperations = Object??entries(operations) .sort(([ a], [ b]) => b - a) .slice(0, 10) .map(([operation, count]) => ({ operation: count })); return { totalShaders: shaderIndex??length, memoryCount: this??shaders??size, topOperations: averagePerformance | performanceCount > 0 ? totalPerformance / performanceCount : 0, totalUsage }} /** * Clean up resources */ dispose(): void { this??shaders??clear(); this??compileQueue??clear(); this??device = ($1) ?? undefined} }; }
