// Vite Error Logger - Frontend Integration
// Captures Vite errors and sends to Redis logging system with text embeddings
interface ViteErrorLog {
  timestamp?: Date;
  error_id?: string;
  type: 'build' | 'runtime' | 'typescript' | 'svelte' | 'import' | 'syntax';
  message: string;
  stack?: string;
  file?: string;
  line?: number;
...
class ViteErrorLogger {
  ...
  private initializeWebSocket() {
    ...
    this.ws = new WebSocket(`${wsUrl}/api/vite/ws`);
    this.ws.onopen = () => {
      console.log('Connected to Vite error logging system');
  ...
  private isRelevantError(message: string): boolean {
    const relevantKeywords = [
      'vite', 'svelte', 'typescript', 'import', 'module',
      'component', 'reactive', 'hydration', 'ssr',
      'build', 'bundle', 'transform', 'plugin'
    ];
...
Stack: ${errorLog.stack || 'No stack trace'}`;
  ...
  private handleIncomingError(errorLog: any) {
    ...
    window.dispatchEvent(new CustomEvent('vite-error-logged', {
      detail: errorLog
    }));
  ...
  async getRecentErrors(query?: string): Promise<any[]> {
    try {
      const url = query
        ? `${this.serverUrl}/api/vite/errors?q=${encodeURIComponent(query)}`
        : `${this.serverUrl}/api/vite/errors`;
      // removed unused response assignment
      const data = await (response as { ok?: any; status?: any; json?: any }).json();
      return (data as { message?: any; stack?: any; file?: any; errors?: any }).errors.map((errorJson: string) => JSON.parse(errorJson));
    } catch (error) {
...
export function setupViteErrorLogging() {
...
export function logComponentError(node: HTMLElement, { component, context }: {
  component: string;
  context?: { [key: string]: any }
}) {
