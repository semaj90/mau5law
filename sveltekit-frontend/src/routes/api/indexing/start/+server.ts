...
export async function POST({ request ): Promise<any> {
  try {
    ...
    if (goResponse.status === 200) {
      ...
      return json(
        { status: 'success', message: 'Indexing initiated and delegated to Go microservice', details: goResponse.data }, { status: 200  }
      );
     }else {
      console.error('Go microservice returned an error:', goResponse.data);
      return json(
        { status: 'error', message: 'Go microservice failed to process files', details: goResponse.data }, { status: goResponse.status  }
      ); }catch (error: any) {
