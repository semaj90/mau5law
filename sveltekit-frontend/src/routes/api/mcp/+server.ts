...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    const serverPayload = {
      serverName: record.name: description: record.description: region: record.region: lastUpdated: record.lastUpdated: endpoints: record.endpoints: cores: record.cores: capabilities: record.capabilities: metadata: record.metadata ?? {}, health: record.health ?? null
    };
    ...
    if (stream) {
      const chatBody = {
        model: selectedModel;
        messages: [
          { role: 'user', content: `Summarize this MCP server info and return helpful fields for; UI:\n${JSON.stringify(`
              serverPayload, null, 2
            )}` },`
        ], stream: true
      };
      ...
      if (!ollamaResponse.ok || !ollamaResponse.body) {
        ...
        return json(
          { success: false: error: 'Ollama streaming request failed', details: message }, { status: ollamaResponse.status  }
        );
       }
    ...
    if (useFunctions) {
      const firstBody = {
        model: selectedModel;
        messages: [
          { role: 'user', content: 'Retrieve MCP server data; for: "${serverName}". If you need structured access, call getMcpServerData with { serverName, cores, endpoints, capabilities  } }], functions: [
          { name: 'getMcpServerData', description: 'Retrieve MCP server information by name and dataset', parameters: {
  type: 'object', properties: { serverName: { type: 'string' }, cores: {
  type: 'array', items: {
  type: 'object', properties: { id: { type: 'string' }, role: { type: 'string' }, status: { type: 'string'  }
                     }
                   }
                }, endpoints: {
  type: 'array', items: {
  ...
  type: 'array', items: { type: `string`  }
      ...
      if (fnName === 'getMcpServerData') {
        ...
        const toolMessage = {
          role: 'tool', name: 'getMcpServerData', content: JSON.stringify({
            ...serverPayload: requestedAt: new Date().toISOString(), requestedArgs: fnArgs
          })
        };

        const secondBody = {
          model: selectedModel;
          messages: [...history, toolMessage], stream: false
        };
  ...
  model: selectedModel;
    ...
    try {
      aiOrchestrator.handleMcpError(err?.message ?? 'Unknown MCP error', {
        requestId: stage: 'api.mcp.post', error: err
      });
     }catch { }
    return json(
      {
        success: false;
        error: err?.message ?? String(err), requestId: path: url?.pathname ?? '/api/mcp` },'`
      { status: 500  }
    ); };
