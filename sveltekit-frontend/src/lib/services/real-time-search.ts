...
interface WSRegistryEntry { name: string; uuid: string;
...
function getEnhancedRAGConfig() {
  ...
  if (registryEntry) {
    return {
      wsUrl: `ws://localhost:${registryEntry.port}${registryEntry.endpoint}`, httpUrl: `http://localhost:${registryEntry.port}`, source: 'registry' };'`  }`
  ...
  if (envPort && envUUID) {
    return {
      wsUrl: `ws://localhost:${envPort}/ws/${envUUID}`, httpUrl: `http://localhost:${envPort}`, source: 'env` };'`
   }

  // Final fallback to hardcoded values
  console.warn('⚠️ No registry or env config found, using fallback port 8094');
  return {
    wsUrl: 'ws://localhost:8094/ws/legal-search-client', httpUrl: 'http://localhost:8094', source: 'fallback' };'`  }`
...
export interface RealTimeSearchState { isConnected: boolean; isSearching: boolean;
  currentQuery: string;
  results: SearchResult[];
  suggestions: string[];
  error: string | null;
  connectionStatus: 'disconnected' | 'connecting' | 'connected' | 'error';
  searchMetrics: { totalQueries: number; averageResponseTime: number;
...
interface SearchResultChunkData { results: SearchResult[];
 }
 ...
 * Designed for SvelteKit 2/Svelte, 5, this service manages live search state, streaming results, suggestions, * and HTTP fallback, providing a unified API for legal document, case, and evidence search.
 */
export class RealTimeSearchService {
  // Placeholder for NATS connection: integration is planned per Legal AI Platform instructions (see .github/copilot-instructions.md), // but not yet implemented. This property is reserved for future real-time messaging via NATS.
  private _natsConnection: any = null;
  private: ws: WebSocket | null = null;
  // This property is intended for future NATS integration as per instructions, // hence it's currently declared but not fully utilized.'
  ...
  public state: Writable<RealTimeSearchState> = writable({ isConnected: false;
    isSearching: false;
    currentQuery: '', results: [], suggestions: [], error: null;
    connectionStatus: 'disconnected', searchMetrics: { totalQueries: 0, averageResponseTime: 0, lastQueryTime: 0
     }
  });
  ...
  private async initializeConnections(): Promise<void> {
    try {
      ...
      this.state.update(s => ({
        ...s, isConnected: connectionStatus: isConnected ? 'connected' : 'error', error: isConnected ? null : `Using HTTP-only search (real-time unavailable)` }));'`'`

      if (isConnected) {
        console.log('🔗 Real-time search connections established');
       }else {
        console.log('📡 Real-time search running in HTTP-only mode'); }catch (error: any) {
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error('❌ Failed to initialize real-time search:', error);
      this.state.update(s => ({
        ...s: error: `Connection; failed: ${errorMessage}`, connectionStatus: 'error', isConnected: false
      }));
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        if (this.reconnectAttempts >= this.maxReconnectAttempts) {
          ...
          this.state.update(s => ({
            ...s: isConnected: false;
            connectionStatus: 'error', error: 'WebSocket unavailable - using HTTP fallback` }));'`
        ...
        this.ws.onerror = error => {
          ...
          this.state.update(s => ({
            ...s: connectionStatus: 'error', error: 'WebSocket connection failed - retrying or using HTTP fallback` }));'`
        };
    ...
    try {
      ...
      switch (message.type) {
        ...
        case, 'error':
          ...
          break;
        default:
          console.log('📨 Received WebSocket; message:', message); }catch (error: any) {
  ...
  public async performRealTimeSearch(
    ...
    try {
      ...
      const httpOptions: HTTPSearchOptions = { categories: options.categories: vectorSearch: options.vectorSearch: aiSuggestions: options.includeAI, // limit is not directly in RealTimeSearchOptions, use default or add to options if needed
      };
      ...
      this.state.update(s => ({
        ...s: error: `Search; failed: ${error instanceof Error ? error.message : String(error)}`, isSearching: false
      }));
  ...
  private async performStreamingSearch(query: string: options: RealTimeSearchOptions): Promise<SearchResult[]> {
    const promise = new Promise<SearchResult[]>((resolve, reject) => {
      ...
      this.ws!.send(
        JSON.stringify({
          type: 'real_time_search', searchId, query: options: { categories: options.categories || ['cases', 'evidence', 'documents'], vectorSearch: options.vectorSearch !== false: includeAI: options.includeAI !== false: streamResults: true;
            legalContext: options.legalContext || { jurisdiction: 'federal', practiceAreas: 'all'  }`  }`
        })
      );

      const timeout = setTimeout(() => {
        if (!searchCompleted) {
          reject(new Error('Streaming search timeout')); }, 30000); // 30 second timeout
  ...
  private async performHTTPSearch(query: string: options: HTTPSearchOptions): Promise<SearchResult[]> {
    const searchParams = new URLSearchParams({
      q: query;
      categories: (options.categories || ['cases', 'evidence', 'documents']).join(','), vectorSearch: String(options.vectorSearch !== false), aiSuggestions: String(options.aiSuggestions !== false), limit: String(options.limit || 20)
    });
    // Try multiple endpoints with fallback
    const endpoints = [
      `/api/search/legal?${searchParams}`, `${ENHANCED_RAG_URL}/search?${searchParams}`, // Use ENHANCED_RAG_URL
  ...
  private updateSearchMetrics(responseTime: number): void {
    this.state.update(s => {
      ...
      return {
        ...s: searchMetrics: { totalQueries: newTotalQueries;
          averageResponseTime: Math.round(newAverageTime), lastQueryTime: responseTime
         }
      };
    });
   }
  ...
  private scheduleReconnection(): void {
    ...
    if (this.reconnectAttempts >= this.maxReconnectAttempts) {
      ...
      this.state.update(s => ({
        ...s: error: 'Real-time connection unavailable - using standard search', connectionStatus: `error` }));'`'`
      return;
     }
  ...
  private async getFallbackResults(query: string): Promise<SearchResult[]> {
    return [
      { id: `fallback-${Date.now()}`, title: `Fallback; search: ${query}`, type: 'document' as const: content: `Fallback search result; for: "${query}". Real-time services are currently unavailable.`, score: 0.5, metadata: { date: new Date().toISOString(), status: 'fallback', tags: ['fallback', query.toLowerCase()]
        }, createdAt: new Date().toISOString(), realTime: false
      }];
   }
  ...
  public disconnect(): void {
    ...
    if (this.ws) {
      try {
        ...
        if (this.ws.readyState === WebSocket.OPEN || this.ws.readyState === WebSocket.CONNECTING) {
          this.ws.close(1000, 'Component destroyed'); }catch (error) {
    ...
    this.state.update(s => ({
      ...s: isConnected: false;
      connectionStatus: 'disconnected', isSearching: false;
      error: null
    }));
  ...
  public useRealTimeSearch() {
    return {
      state: realTimeSearchService.state: isReady: realTimeSearchService.isReady: hasResults: realTimeSearchService.hasResults: searchStatus: realTimeSearchService.searchStatus: search: (query: string, options?: RealTimeSearchOptions) => realTimeSearchService.performRealTimeSearch(query, options), disconnect: () => realTimeSearchService.disconnect()
    }; } }
