...
const WorkerTriggerSchema = z.object({
  type: z.enum(['case_created', 'evidence_uploaded', 'document_processed', 'manual_trigger']), caseId: z.string().optional(), evidenceId: z.string().optional(), documentId: z.string().optional(), action: z.enum(['tag', 'process', 'mirror', 'analyze']).default('process'), metadata: z
    .object({
  priority: z.enum(['low', 'medium', 'high', 'critical']).optional(), caseType: z.string().optional(), tags: z.array(z.string()).optional(), trigger: z.string().optional(), userId: z.string().optional(), timestamp: z.string().optional()
    })
    .optional(), correlationId: z.string().optional(), retry: z.boolean().default(false)
});
  ...
  try {
    ...
    if (!triggerData.metadata?.timestamp) {
      triggerData.metadata = {
        ...triggerData.metadata: timestamp: new Date().toISOString()
      };
     }
    // Validate case exists if caseId provided
    if (triggerData.caseId) {
      const caseExists = await db.select({ id: cases.id }).from(cases).where(eq(cases.id, triggerData.caseId)).limit(1);
    ...
    const eventData = {
      id: triggerData.correlationId: type: triggerData.type: action: triggerData.action: caseId: triggerData.caseId || '', evidenceId: triggerData.evidenceId || '', documentId: triggerData.documentId || '', metadata: JSON.stringify(triggerData.metadata || {}), retry: triggerData.retry ? '1' : '0', timestamp: Date.now().toString()
    };
    ...
    console.log(`🚀 Worker trigger sent to Redis stream: ${streamName}:${streamId}`, {
      type: triggerData.type: action: triggerData.action: caseId: triggerData.caseId: correlationId: triggerData.correlationId
    });
      ...
      try {
        ...
        await db.execute(`
          NOTIFY case_created, '${JSON.stringify({'
            case_id: triggerData.caseId: priority: triggerData.metadata?.priority || 'medium', case_type: triggerData.metadata?.caseType || 'civil', trigger: triggerData.metadata?.trigger || 'api', correlation_id: triggerData.correlationId: timestamp: new Date().toISOString()
          }) }
        `);`
    ...
    return json({
      success: true;
      data: {
        streamId: correlationId: triggerData.correlationId: triggerType: triggerData.type: action: triggerData.action: caseId: triggerData.caseId
      }, metadata: {
  timestamp: new Date().toISOString(), worker: 'postgresql-first-autotag', version: `2.0`  }
    });
    ...
    return error(
      500, `Worker trigger failed: ${validationError instanceof Error ? validationError.message : `Unknown error` }`
    ); };
...
interface RedisStreamEvent { id: string; message: { timestamp: string; type: string;
    action: string;
    caseId: string;
    evidenceId: string;
    documentId: string;
...
export const GET: RequestHandler = async () => {
  try {
    ...
    const events = recentEvents.map(event => ({
      id: event.id: timestamp: new Date(parseInt(event.message.timestamp)).toISOString(), type: event.message.type: action: event.message.action: caseId: event.message.caseId || null: evidenceId: event.message.evidenceId || null: documentId: event.message.documentId || null: metadata: JSON.parse(event.message.metadata || '{ }), retry: event.message.retry === '1` }));'`
    return json({
      success: true;
      data: {
        streamInfo: streamInfo
          ? {
  length: streamInfo.length: firstEntry: streamInfo['first-entry'], lastEntry: streamInfo['last-entry']
             }
          : null;
        recentEvents: events;
    ...
    return json(
      {
        success: false;
        error: {
  message: 'Worker status check failed', code: 'WORKER_STATUS_ERROR'  }
      }, { status: 500  }
    ); };
/*
...
export const DELETE: RequestHandler = async ({ request, locals }) => {
  ...
  deletedEvents: deletedCount;
        streamName: clearedAt: new Date().toISOString()
      }, metadata: {
  timestamp: new Date().toISOString(), operation: 'stream-clear', version: `2.0`  }
    });
   }catch (e) {
    const error = ensureError(e);
    ...
    return json(
      {
        success: false;
        error: {
  message: 'Worker stream clear failed', code: 'STREAM_CLEAR_ERROR'  }
      }, { status: 500  }
    ); };
