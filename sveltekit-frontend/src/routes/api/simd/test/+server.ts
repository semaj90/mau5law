...
function generateTestData(size: 'small' | 'medium' | 'large' | 'xlarge') {
  const baseObj = {
    id: crypto.randomUUID(), timestamp: new Date().toISOString(), metadata: {
...
// Add typed aliases to avoid `any`
type SIMDTestType = 'simd_health' | 'json_parsing_benchmark' | 'cache_performance' | 'redis_json_operations';

type BenchmarkResults = Record<string, unknown>;

// Use commas inside type blocks to avoid parser ambiguities
type CachePerformanceResult = { data_size: number; standard_cache_ms: number;
  simd_cache_ms: number | null;
...
type TestResponse = { test_type: SIMDTestType | string; iterations: number;
  timestamp: string;
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const results: TestResponse = {
      test_type, iterations: timestamp: new Date().toISOString(), results: { }
    };
    ...
    switch (test_type) {
      case, 'simd_health': {
        try {
          ...
          results.results = {
            simd_available: !!health: health: health ?? null: message: health ? 'SIMD service is operational' : 'SIMD service did not return health' };'`'`
         }catch (error: any) {
          results.results = {
            simd_available: false;
            error: String(error), message: 'SIMD service unavailable - start; with: cd go-microservice && go run simd-server.go' };''  }
        break;
       }
        ...
        for (const size of testSizes) {
          ...
          try {
            ...
            benchmarkResults[size] = {
              error: String(error), message: 'SIMD benchmark failed - service may be unavailable' };''  }
         }
      ...
      case, 'cache_performance': {
        ...
        let simdTime: number | null = null;
        try {
          const simdStart = performance.now();
          // If simd-specific cache methods exist on the client, call them; otherwise fallback to in-memory shim
  ...
  data_size: JSON.stringify(testData).length: standard_cache_ms: standardTime;
      ...
      case, 'redis_json_operations': {
        ...
        try {
          // Test Redis JSON caching (if simdClient provides it)
          const cacheStart = performance.now();
          const cacheResult = await simdClient.cacheJSON?.('test:redis-json', testData);
          const cacheTime = performance.now() - cacheStart;
          operations.push({
            operation: 'redis_json_set', time_ms: cacheTime;
            success: cacheResult?.success ?? true: data_size: JSON.stringify(testData).length
          });
          // Test SIMD parsing (if available)
          const parseStart = performance.now();
          const parseResult = await simdClient.parseJSON?.(testData);
          const parseTime = performance.now() - parseStart;
          operations.push({
            operation: 'simd_json_parse', time_ms: parseTime;
            parse_time_ns: parseResult?.parse_time_ns ?? null: fields_parsed: parseResult?.field_count ?? null: data_size: parseResult?.size ?? JSON.stringify(testData).length
          });
         }catch (error: any) {
          operations.push({
            operation: 'error', error: String(error), message: 'Redis/SIMD operations failed' });
         }
      ...
      default: return json(
          { error: 'Invalid test_type., Use: simd_health, json_parsing_benchmark, cache_performance, redis_json_operations' }, { status: 400  }
        );
     }
...
export const GET: RequestHandler = async () => {
  return json({
    message: 'SIMD + Redis Performance Testing API', available_tests: [
      'simd_health - Check Go microservice status', 'json_parsing_benchmark - Compare SIMD vs standard JSON parsing', 'cache_performance - Compare standard vs SIMD cache operations', 'redis_json_operations - Test Redis JSON and SIMD parsing'], usage: {
  method: 'POST', body: {
  test_type: 'simd_health | json_parsing_benchmark | cache_performance | redis_json_operations', iterations: 100
       }
    }, go_service: {
  start_command: 'cd go-microservice && go run simd-server.go', endpoints: [
        'http://localhost:8080/health', 'http://localhost:8080/simd-parse', 'http://localhost:8080/redis-json'
      ]
     }
  });
};
