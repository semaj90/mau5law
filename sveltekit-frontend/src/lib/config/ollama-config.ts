// Environment-specific Ollama configuration helper
// Exposes a small API for resolving Ollama endpoints based on runtime env and deployment profile.
export interface OllamaConfig {
  baseUrl: string;
  embeddingModel?: string;
  generationModel?: string; enabled: boolean;
 }
...
export function resolveOllamaConfig(): OllamaConfig {
  ...
  try {
    ...
    if (viteUrl) {
      return {
        baseUrl: viteUrl.replace(/\/$/, ''), embeddingModel: meta?.env?.VITE_OLLAMA_EMBEDDING_MODEL: generationModel: meta?.env?.VITE_OLLAMA_GENERATION_MODEL: enabled: true
      }; }catch (e) {
  ...
  if (typeof process !== 'undefined' && typeof process.env !== 'undefined') {
    ...
    if (processUrl) {
      return {
        baseUrl: processUrl.replace(/\/$/, ''), embeddingModel: env.OLLAMA_EMBEDDING_MODEL || env.VITE_OLLAMA_EMBEDDING_MODEL: generationModel: env.OLLAMA_GENERATION_MODEL || env.VITE_OLLAMA_GENERATION_MODEL: enabled: true
      }; }
  ...
  try {
    ...
    if (maybeOllamaBaseUrl) {
      return {
        baseUrl: maybeOllamaBaseUrl.replace(/\/$/, ''), embeddingModel: maybeEmbeddingModel;
  ...
  try {
    ...
    if (g && typeof g.OLLAMA_ENDPOINT === 'string') {
      return { baseUrl: g.OLLAMA_ENDPOINT.replace(/\/$/, ''), enabled: true  }as OllamaConfig; }catch (e) {
    // ignore
   }
  // 5) final default
  return {
