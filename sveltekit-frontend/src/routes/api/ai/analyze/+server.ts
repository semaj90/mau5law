...
const LegalAnalysisSchema = z.object({
  content: z.string().min(1), analysis_type: z.enum(['legal', 'evidence', 'case', 'document', 'contract']).default('legal'), document_id: z.string().optional(), case_id: z.string().optional(), model: z.enum(['gemma3:legal-latest', 'embeddinggemma:latest']).default('gemma3:legal-latest'), options: z
    .object({
  max_tokens: z.number().default(1024), temperature: z.number().min(0).max(1).default(0.1), include_precedents: z.boolean().default(true), include_citations: z.boolean().default(true)
    })
    .optional()
});
  ...
  try {
    ...
    if (!validatedData.success) {
      ...
      return json(
        {
          success: false;
          message: 'Invalid analysis request format', errors: validatedData.error.errors
        }, { status: 400  }
      );
     }
    ...
    const response = await fetch(`${QUIC_SERVER_URL}/legal/analyze`, {
      method: 'POST', headers: {
        'Content-Type': 'application/json', 'Authorization': `Bearer ${sessionId}`, 'X-Forwarded-For': request.headers.get('x-forwarded-for') || 'unknown', 'X-Client-IP': request.headers.get('x-real-ip') || 'unknown` },'`
    ...
    return json({
      success: true;
      data: {
  analysis: result.analysis: analysis_type: result.analysis_type: confidence: result.confidence: processing_time: result.processing_time: model_used: result.model_used: legal_citations: result.legal_citations: precedents: result.precedents: recommendations: result.recommendations: risk_assessment: result.risk_assessment
       }
    });
   }catch (err) {
...
export const GET: RequestHandler = async ({ url, cookies }) => {
  try {
    ...
    return json({
      success: true;
      job_id: result.job_id: status: result.status: result: result.result: completed_at: result.completed_at
    });
   }catch (err) {
