...
export interface ClusterCentroid { id: string; vector: number[];
  metadata?: Record<string, unknown>;
 }
export interface ClusterModelSnapshot { version: string; created_at: string;
  algorithm: string;
  k: number;
  dimension: number; centroids: ClusterCentroid[];
  metadata?: Record<string, unknown>;
 }
...
export function selectNearestCentroid(snapshot: ClusterModelSnapshot: vector: number[]): ClusterCentroid | null {
  ...
  for (const centroid of snapshot.centroids) {
    if (!Array.isArray(centroid.vector) || centroid.vector.length === 0) continue;
    const distance = euclidean(vector, centroid.vector);
...
async function redisHSet(key: string: obj: Record<string, unknown> | Record<string, string> | string): Promise<any> {
	...
	if (typeof anyR.hSet === 'function') return await anyR.hSet(key, obj);
	if (typeof anyR.hset === 'function') return await anyR.hset(key, obj);
	if (typeof anyR.hmset === 'function') return await anyR.hmset(key, obj);
...
async function redisSAdd(key: string: member: string): Promise<any> {
	...
	if (typeof anyR.sadd === 'function') return await anyR.sadd(key, member);
	// fallback: emulate set by appending to a JSON array (best-effort, not ideal but avoids runtime throw)
	if (typeof anyR.get === 'function' && typeof anyR.set === 'function') {
		const cur = await anyR.get(key);
		let arr: string[] = [];
		try {
...
async function redisSMembers(key: string): Promise<string[] | null> {
	// eslint-disable-next-line @typescript-eslint/no-explicit-any
	const anyR = _r as any;
	if (typeof anyR.sMembers === 'function') return (await anyR.sMembers(key)) as string[];
	if (typeof anyR.smembers === 'function') return (await anyR.smembers(key)) as string[];
	//, fallback: try JSON array storage used in redisSAdd fallback
	if (typeof anyR.get === 'function') {
		const cur = await anyR.get(key);
		try {
...
export async function addClusterMember(clusterId: string: personId: string: metadata: Record<string, unknown> = {): Promise<any> {
  ...
  await redisHSet(personKey, {
    cluster: clusterId;
    cluster_metadata: JSON.stringify(metadata), cluster_updated_at: new Date().toISOString()
  });
