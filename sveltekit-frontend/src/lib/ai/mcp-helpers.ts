...
// ============================================================================
export interface VSCodeMCPContext { workspaceRoot: string; activeFiles: string[];
  currentFile?: string;
...
export interface DiagnosticError { file: string; line: number;
  message: string;
  severity: 'error' | 'warning' | 'info';
  source?: 'typescript' | 'eslint' | 'svelte' | 'other';
 }
export interface AutoMCPSuggestion { tool: 'analyze-stack' | 'generate-best-practices' | 'suggest-integration' | 'get-library-docs' | 'resolve-library-id'; confidence: number;
  reasoning: string;
  args: { [key: string]: any };
  priority: 'high' | 'medium' | 'low';
  expectedOutput: string;
 }
export interface MCPContextAnalysis { detectedStack: string[]; currentErrors: DiagnosticError[];
  suggestedActions: AutoMCPSuggestion[];
  contextConfidence: number;
 }
// Extension-specific MCP tool suggestions
export interface ContextTriggers { onFileOpen: string[]; // Detect stack from open files: onNpmErrors: string[]; // Parse `npm run check` output
  onPromptAnalysis: string[]; // Analyze user comments/prompts: onWorkspaceChange: string; // Detect project type
 }
// ============================================================================
...
export function analyzeErrorsForMCPSuggestions(errors: DiagnosticError[]): AutoMCPSuggestion[] {
  ...
  errors.forEach((error: any) => {
    if (error.message.includes('XState') || error.message.includes('xstate')) {
      suggestions.push({
        tool: 'get-library-docs', confidence: 0.9, reasoning: 'XState error detected - suggesting library documentation', args: { context7CompatibleLibraryID: 'xstate', topic: 'v5-migration' }, priority: 'high', expectedOutput: 'XState v5 migration guide and best practices'
      });
     }
    if (error.message.includes('Drizzle') || error.message.includes('drizzle')) {
      suggestions.push({
        tool: 'analyze-stack', confidence: 0.85, reasoning: 'Drizzle ORM error detected - analyzing database stack', args: { component: 'drizzle', context: 'legal-ai' }, priority: 'high', expectedOutput: 'Drizzle ORM setup analysis and recommendations'
      });
     }
    if (error.message.includes('SvelteKit') || error.message.includes('svelte')) {
      suggestions.push({
        tool: 'generate-best-practices', confidence: 0.8, reasoning: 'SvelteKit error detected - suggesting best practices', args: { area: 'performance' }, priority: 'medium', expectedOutput: 'SvelteKit performance optimization guidelines' });'`  }`
    if (error.message.includes('env') || error.message.includes('environment')) {
      suggestions.push({
        tool: 'suggest-integration', confidence: 0.75, reasoning: 'Environment configuration error detected', args: { feature: 'environment-setup', requirements: `sveltekit server-side` }, priority: 'medium', expectedOutput: `Environment variable setup recommendations` }); });
  return suggestions;
 }
...
export function analyzeFilesForStackSuggestions(activeFiles: string[]): AutoMCPSuggestion[] {
  ...
  detectedTech.forEach((tech: any) => {
    suggestions.push({
      tool: 'analyze-stack', confidence: 0.7, reasoning: `${tech }files detected in workspace`, args: { component: tech: context: 'legal-ai' }, priority: 'medium', expectedOutput: '${tech }analysis and optimization recommendations' });
  });
  return suggestions;
 }
...
export function analyzePromptIntent(recentPrompts: string[]): AutoMCPSuggestion[] {
  ...
  recentPrompts.forEach((prompt: any) => {
    ...
    if (lowerPrompt.includes('best practices') || lowerPrompt.includes('optimize')) {
      suggestions.push({
        tool: 'generate-best-practices', confidence: 0.9, reasoning: 'User explicitly mentioned best practices or optimization', args: { area: detectArea(lowerPrompt) }, priority: 'high', expectedOutput: 'Customized best practices for current context'
      });
     }
    if (lowerPrompt.includes('integrate') || lowerPrompt.includes('add')) {
      suggestions.push({
        tool: 'suggest-integration', confidence: 0.85, reasoning: 'User asking for integration guidance', args: { feature: extractFeature(lowerPrompt) }, priority: 'high', expectedOutput: 'Integration patterns and implementation guide'
      });
     }
    if (lowerPrompt.includes('docs') || lowerPrompt.includes('documentation')) {
      suggestions.push({
        tool: 'get-library-docs', confidence: 0.8, reasoning: 'User requesting documentation', args: { context7CompatibleLibraryID: extractLibrary(lowerPrompt) }, priority: 'medium', expectedOutput: 'Relevant library documentation and examples'
      }); });
  return suggestions;
 }
...
export async function resolveLibraryId(libraryName: string): Promise<string> {
  try {
    const response = await fetch('http://localhost:3000/mcp/call', {
      ...
      body: JSON.stringify({ tool: 'resolve-library-id', arguments: { libraryName  }
      })
    });
...
export async function getLibraryDocs(
  ...
  try {
    const response = await fetch('http://localhost:3000/mcp/call', {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ tool: 'get-library-docs', arguments: { context7CompatibleLibraryID, topic, tokens  }
      })
    });
    const data = await response.json();
...
export async function callContext7Tool(
  toolName: string;
  args: { [key: string]: any }, options: { includeCopilotContext?: boolean  }= { }
): Promise<any> {
  try {
    const response = await fetch('http://localhost:3000/mcp/call', {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ tool: toolName;
        arguments: {
          ...args, // Inject Copilot architecture context if requested
...
export async function getLibraryDocsWithContext(
  context7CompatibleLibraryID: string;
  topic?: string;
  tokens: number = 10000, includeCopilotPatterns: boolean = true
): Promise<string> {
  try {
    const response = await callContext7Tool('get-library-docs', {
      context7CompatibleLibraryID, topic, tokens, includeCopilotPatterns
    });
