...
export class Gemma3LegalService {
  ...
  async generateLegalResponse(
   , prompt: string,
    ...
    const payload = { model: this.model,
      prompt: enhancedPrompt,
      stream: options.stream || false,
      options: { temperature: options.temperature || 0.3, // Lower for legal accuracy
...
IMPORTANT: Your responses should be informative but always include appropriate disclaimers that this is not legal advice and users should consult with qualified attorneys for specific legal matters.`;`
    const contextPrompts = { contract: `Focus on contract law, terms analysis, clause interpretation, and agreement structures. Consider enforceability, terms clarity, and standard practices.`,
