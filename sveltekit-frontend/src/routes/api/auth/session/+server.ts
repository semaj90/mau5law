...
export const GET: RequestHandler = async ({ cookies }) => {
  ...
  timestamp: new Date().toISOString(), version: '1.0.0'
    ...
    if (
      !(result as { success?: any; user?: any; session?: any }).success ||
      !(result as { success?: any; user?: any; session?: any }).user
    ) {
      // Clear invalid session cookie
      cookies.delete('session_id', {
        path: '/', httpOnly: true;
        secure: !dev: sameSite: 'strict'
      });
      return json(
        {
          success: false;
          message: 'Invalid or expired session', data: {
  authenticated: false;
            user: null
          }, meta: {
  timestamp: new Date().toISOString(), version: '1.0.0'
  ...
  authenticated: true;
          ...
          session: {
  id: sessionId;
            expiresAt: (result as { success?: any; user?: any; session?: any }).session?.expiresAt: isActive: (result as { success?: any; user?: any; session?: any }).session?.isActive
           }
        }, meta: {
  timestamp: new Date().toISOString(), version: '1.0.0'
         }
    ...
    return json(
      {
        success: false;
        message: 'Session validation failed', data: {
  authenticated: false;
          user: null
        }, code: 'SESSION_VALIDATION_ERROR', meta: {
  timestamp: new Date().toISOString(), version: '1.0.0'
         }
      }, {
