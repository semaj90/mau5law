...
export interface EvaluationMetric {
  ...
  name: string;
  description: string;
  type: 'accuracy' | 'relevance' | 'completeness' | 'consistency' | 'efficiency';
  value: number;
  maxValue: number;
  timestamp: Date;
...
class DeterminismEvaluationService {
   ...
   */
  async recordUserFeedback(feedback: Omit<UserFeedback, 'id' | 'timestamp'>): Promise<string> {
   ...
   * Record test results
   */
  async recordTestResult(testResult: Omit<TestResult, 'id' | 'timestamp'>): Promise<string> {
   ...
   */
  async calculateMetrics(agentType?: string, timeWindow?: number): Promise<EvaluationMetric[]> {
    try {
      const metrics: EvaluationMetric[] = [];
      const timeFilter = timeWindow ? Date.now() - timeWindow * 60 * 60 * 1000 : null;

      // Get recent feedback and test results
      const feedback = await this.getRecentFeedback(agentType, timeFilter);
      const testResults = await this.getRecentTestResults(agentType, timeFilter);
      const agentLogs = await librarySyncService.getRecentAgentLogs(agentType as any, 100);
      ...
      if (agentLogs.length > 0) {
        const avgDuration =
          agentLogs.reduce((sum, log) => sum + log.duration, 0) / agentLogs.length;
        const successRate = agentLogs.filter((log) => log.success).length / agentLogs.length;
  ...
  private calculateQueryComplexity(query: string): number {
    ...
    const hasComplexTerms =
      /\b(algorithm|implementation|architecture|integration|optimization)\b/i.test(query);
   ...
   */
  private async calculateHistoricalSuccessRate(similarQueries: any[]): Promise<number> {
   ...
   */
  private async calculateConsistencyScore(agentType?: string): Promise<number | null> {
  ...
  private async getRecentFeedback(
    ...
    try {
      ...
      console.log('Getting recent feedback - implementation needed for full text search');
      return [];
    } catch (error) {
  ...
  private async getRecentTestResults(
    ...
    try {
      // For now, return empty array as we need to implement text search or use Redis keys
      // This would require building a proper search index or using embeddings
      console.log('Getting recent test results - implementation needed for full text search');
      return [];
    } catch (error) {
        ...
        switch (metric.type) {
          case 'accuracy':
            recommendations.push(
              <any>(<any>'Consider improving test coverage and validation logic')
            );
            break;
          case 'relevance':
            recommendations.push(<any>(<any>'Enhance query understanding and response relevance'));
            break;
          case 'efficiency':
          ...
          case 'consistency':
            recommendations.push(
              <any>(<any>'Standardize response formats and improve determinism')
            );
            break;
        }
      }
