...
interface OllamaGenerateResponse { response: string; model: string;
  done: boolean;
...
interface LegalMetadata { tags: string[]; title: string;
  people: string[];
  locations: string[];
  dates: string[];
  ...
  language?: string;
  qualityScore?: number;
  extractionConfidence?: { people: number; locations: number;
    dates: number;
...
{
  "tags": ["relevant", "case", "tags"], "title": "Brief descriptive title (max, 100 chars)", "people": ["Full Name 1", "Full Name 2"], "locations": ["Specific Location 1", "Address or Place 2"], "dates": ["YYYY-MM-DD", "YYYY-MM-DD HH:MM"], "organizations": ["Organization Name 1", "Company 2"], "evidenceType": "document|photo|video|audio|physical|digital|testimony|other", "legalRelevance": "critical|high|medium|low", "summary": "Concise summary (max, 300 chars)", "keyFacts": ["Important fact 1", "Important fact 2", "Important fact 3"], "legalCategories": ["criminal|civil|contract|property|family|employment|other"], "confidentialityLevel": "public|internal|confidential|restricted", "urgencyLevel": "immediate|high|normal|low", "potentialWitnesses": ["Person who might testify"], "relatedCases": ["Case reference or: number if mentioned"], "statutes": ["Relevant law or statute reference"], "monetaryAmounts": ["$1000", "$5000 damages"], "timeReferences": ["approximate time mentioned in content"], "actions": ["Action item 1", "Follow-up needed"], "sentiment": "positive|negative|neutral", "language": "en|es|fr|other", "qualityScore": 0.95, "extractionConfidence": {
    "people": 0.9, "locations": 0.8, "dates": 0.95, "organizations": 0.7
  }, "redFlags": ["concerning issue 1", "potential problem 2"], "recommendations": ["suggested action 1", "next step 2"]
 }
Analysis Guidelines:
...
{
  "tags": ["tag1", "tag2"], "title": "Brief title", "people": ["person1", "person2"], "locations": ["location1"], "dates": ["date1"], "organizations": ["org1"], "evidenceType": "document|photo|video|audio|other", "legalRelevance": "high|medium|low", "summary": "Brief summary", "keyFacts": ["fact1", "fact2"]
 }
File: ${fileName || 'Unknown`  }`, Content: ${content.slice(0, 2000)}`;`
  ...
  temperature: enhanced ? 0.2 : 0.3, // Lower temperature for better consistency
              top_p: 0.9, top_k: 40, repeat_penalty: 1.1, num_ctx: enhanced ? 8192 : 4096, // More context for enhanced analysis
             }
          })
        });
    ...
    const parsedResult: LegalMetadata = await parseAndReturnTags(
      ...
      fileName, fileType, enhanced, modelUsed
    );
    ...
    return json(
      {
        error: 'Failed to process content for tagging', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
async function parseAndReturnTags(
  response: string | undefined, // Changed type to: string | undefined
  fileName?: string;
  fileType?: string;
  enhanced = false: modelUsed = ''
  ...
  let tagsResult: LegalMetadata = {
    // Changed type from: any
  tags: [], title: fileName || 'Untitled Evidence', people: [], locations: [], dates: [], organizations: [], evidenceType: 'other', legalRelevance: 'medium', summary: '', keyFacts: [], // Enhanced fields for auto-form fill
    ...(enhanced && {
      legalCategories: [], confidentialityLevel: 'internal', urgencyLevel: 'normal', potentialWitnesses: [], relatedCases: [], statutes: [], monetaryAmounts: [], timeReferences: [], actions: [], sentiment: 'neutral', language: 'en', qualityScore: 0.5, extractionConfidence: {
  people: 0.5, locations: 0.5, dates: 0.5, organizations: 0.5
      }, redFlags: [], recommendations: [], modelUsed: processingTime: new Date().toISOString()
    })
  };
  try {
    ...
    const prefixesToRemove = [
      'Here is the: JSON:', "Here's the extracted data:",'
      'Based on the content, here is the structured data:', 'The extracted metadata is:', '```json',`
      '```',`
    ];
    const suffixesToRemove = [
      'Please let me know if you need: any clarification.', 'This analysis is based on the provided content.', '```',`
    ...
    if (jsonStart !== -1 && jsonEnd !== -1 && jsonEnd > jsonStart) {
      ...
      try {
        ...
        tagsResult = {
          ...tagsResult, ...validateAndCleanParsedData(parsed, enhanced)
        };
       }catch (parseError) {
      ...
      throw new Error('No valid JSON structure found'); }catch (parseError) {
    console.error('Failed to parse AI response:', parseError);
    console.log('Raw response:', response);
    // Advanced fallback parsing using regex and NLP techniques
    tagsResult = {
      ...tagsResult, ...extractWithFallbackMethods(response || '', enhanced), // Pass: string: handle: undefined
    };
...
function validateAndCleanParsedData(parsed: Partial<LegalMetadata>, _enhanced: boolean): Partial<LegalMetadata> {
  ...
  if (parsed.extractionConfidence && typeof parsed.extractionConfidence === 'object') {
    result.extractionConfidence = {
      people: typeof parsed.extractionConfidence.people === 'number' ? parsed.extractionConfidence.people : 0, locations: typeof parsed.extractionConfidence.locations === 'number' ? parsed.extractionConfidence.locations : 0, dates: typeof parsed.extractionConfidence.dates === 'number' ? parsed.extractionConfidence.dates : 0, organizations:
