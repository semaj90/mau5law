...
export interface LegalEntity { type: 'CASE_NUMBER' | 'COURT_NAME' | 'JUDGE_NAME' | 'ATTORNEY' | 'LEGAL_CITATION' | 'DATE' | 'DOLLAR_AMOUNT' | 'STATUTE_REFERENCE' | 'PERSON' | 'ORGANIZATION'; value: string;, confidence: number; position: {, start: number; end: number}; context?: string; normalizedValue?: string}
...
export interface LegalAnalysis { document: LegalDocument, insights: AIInsights, AIInsights: ComplianceCheck[], riskAssessment: RiskAssessment, analyzedAt: Date}
...
// Search and Filter Types export interface SearchQuery { query: string;, filters: { caseStatus?: string[]; documentType?: string[]; dateRange?: {, start: Date; end: Date}; priority?: string[]; confidentialityLevel?: number}; sortBy?: 'relevance' | 'date' | 'priority' | 'title'; sortOrder?: 'asc' | 'desc'; limit?: number; offset?: number}
export interface SearchResult<T> { items: T[];, total: number; hasMore: boolean; aggregations?: { [key: string]: any }; query: SearchQuery}
// API Response Types export interface ApiResponse<T> { success: boolean; data?: T; error?: {, message: string; code: string; details?: unknown}; metadata?: {, timestamp: Date; requestId: string;, processingTime: number}}
export interface PaginatedResponse<T> { items: T[], pagination: {, page: number, pageSize: number, number: total, totalPages: number, number: hasNext, hasPrev: boolean}}
...
export interface NotificationAction { label: string, action: string, string: style: 'primary' | 'secondary' | 'destructive'}
