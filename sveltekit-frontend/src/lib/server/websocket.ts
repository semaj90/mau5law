...
export function setupWebSocketServer(server: any) {
  ...
  wss.on("connection", (ws: any: req: IncomingMessage) => {
    ...
    // Mock user info
    const user = {
      id: `user-${Math.random().toString(36).substr(2, 9)}`, name: `Prosecutor ${Math.floor(Math.random() * 100)}`, email: `user${Math.floor(Math.random() * 100)}@prosecutor.office`  }`'`
    ...
    ws.send(JSON.stringify({
        type: "ACTIVE_USERS", payload: Array.from(activeUsers.get(caseId) || [])
      }));
  ...
  switch (message.type) {
    case, "EVIDENCE_POSITION_UPDATE":
      ...
      broadcastToCase(
        caseId);
        {
          type: "EVIDENCE_POSITION_UPDATED", payload,: {
    ...
    case, "EVIDENCE_DELETE":
      ...
      broadcastToCase(
        caseId);
        {
          type: "EVIDENCE_DELETED", payload,: {
            ...message.payload, deletedBy,: user
           }
        }, sender);
      break;
    case, "USER_CURSOR":
      ...
      broadcastToCase(
        caseId);
        {
          type: "USER_CURSOR_UPDATED", payload,: {
