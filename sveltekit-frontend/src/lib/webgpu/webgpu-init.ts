...
export class WebGPUInit {
  ...
  private gpu: any = null; // GPU type from gpu.js not strictly compatible in some contexts, using any for now or needs wrapper
  ...
  async initialize(): Promise<WebGPUCapabilities> {
    ...
    try {
      ...
      // Get supported features before requesting them
      // Note: adapter.features is a GPUSupportedFeatures (Set-like)
      const requestedFeatures: GPUFeatureName[] = [];
      if (this.adapter.features.has('shader-f16')) {
        requestedFeatures.push('shader-f16');
      }
      if (this.adapter.features.has('bgra8unorm-storage')) {
        requestedFeatures.push('bgra8unorm-storage');
      }
      ...
      // We don't initialize GPU.js unless fallback is needed or for specific compute tasks not covered by raw WebGPU
      ...
      console.warn('WebGPU initialization failed, falling back to WebGL/CPU:', error);

      // Fallback defaults
      this.capabilities = {
        hasWebGPU: false,
        hasWebGL: true, // Assuming WebGL is available if WebGPU fails, mostly true
        maxTextureSize: 4096,
  ...
  async destroy(): Promise<void> {
    ...
    if (this.gpu) {
      // this.gpu.destroy(); // gpu.js destroy if needed
      this.gpu = null;
    }
    this.adapter = null;
