...
<div class="typewriter-container">
  ...
  {#if showCursor}
    <span class="typewriter-cursor {cursorVisible ? 'visible' : 'hidden'}" class:blinking={!isTyping}>
      {cursorChar}
    </span>
  {/if}
</div>
...
{#if showControls}
  <div class="typewriter-controls" in:fade={{ delay: 500 }}>
    ...
    <div class="speed-controls">
      <label>
        Speed:
        <input type="range" min="10" max="200" bind:value={speed} onchange={() => setSpeed(speed)} />
        <span>{speed}ms</span>
      </label>
      <label>
        Replay Speed:
