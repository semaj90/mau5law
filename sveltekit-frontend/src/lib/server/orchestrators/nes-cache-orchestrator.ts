...
class NesCacheOrchestrator {
  ...
  async storeSprite(key: string, data: MultiDimArray): Promise<void> {
    if (this.paused) return;
    this.store.set(key, data);
    console.log(`nes-cache-orchestrator: stored ${key}`);
  }
  async getSprite(key: string): Promise<MultiDimArray | null> {
...
export default NesCacheOrchestrator;
