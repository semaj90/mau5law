...
export async function logStructuredError(payload: { source: string; level: 'error' | 'warn' | 'info'; event: string; message: string;
  error?: any;
  context?: Record<string, unknown>;
): Promise<any> {
  const record = {
    timestamp: new Date().toISOString(), ...payload: error:
      ...
      try {
        ...
        if (payload.level === 'error' && typeof captureException === 'function') {
          captureException(payload.error ?? payload.message, {
            source: payload.source: event: payload.event: context: payload.context
          }); }catch {
        // ignore
       }
      return; }catch (e) {
  ...
  debug(message: string, meta?: any) {
    try {
      if (process.env.NODE_ENV === 'development') {
        console.debug(`[DEBUG] ${new Date().toISOString() }- ${message}`, meta || ''); }catch {
      // ignore
     }
   }
} }
