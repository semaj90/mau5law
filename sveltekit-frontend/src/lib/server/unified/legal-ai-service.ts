...
type FullDocument = { metadata: Record<string, unknown>;
  fileUrl: string;
  textContent: string;
...
export class UnifiedLegalAIService {
  ...
  async uploadDocument(upload: DocumentUpload): Promise<UploadResult> {
    ...
    try {
      ...
      // guard MinIO shape
      const minioObjectName = isMinioUploadResult(minioResult) ? minioResult.objectName : `${documentId}`;
      const minioSize = isMinioUploadResult(minioResult) ? minioResult.size : 0;
      const minioUrl = isMinioUploadResult(minioResult) ? minioResult.url : '';

      // Step 3: Generate embeddings and store in Qdrant (no unused var)
  ...
  async getDocument(id: string): Promise<FullDocument> {
    ...
    try {
      ...
      const similarDocs = (await this.qdrantSearch('cases', textForSimilarity, { limit: 5 })) as QdrantHit[];
