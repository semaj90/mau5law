...
async function loadEditor(): Promise<any> {
  try {
    ...
    EditorComponent = (mod as { default?: any }).default ?? (mod as { ReportEditor?: any }).ReportEditor;
  } catch (e) {
...
$effect(() => {
  reportId = (currentReport as unknown)?.id ?? 'temp-report-id';
});
</script>
...
<div class="container">
  ...
  {#if error}
    <div class="space-y-4">
      ❌ {error}
      <!-- changed: use onclick instead of deprecated on:click --> <button onclick={() => (error = '')}>×</button>
    </div>
  {/if}
  <!-- Loading State -->
  {#if isLoading}
  ...
  {:else}
    ...
    <aside class="space-y-4">
      ...
      <div class="space-y-4">
        <h3>⚡ Quick Actions</h3>
        <div class="space-y-4">
          <button>🖨️ Export PDF</button> <button>💾 Save Template</button> <button>🔄 Sync Offline</button>
        </div>
      </div>
    </aside>
  {/if}
