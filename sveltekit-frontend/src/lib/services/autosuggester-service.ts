...
export class AutosuggesterService {
  ...
  private determineScope(filePath: string, position: number): 'global' | 'class' | 'function' | 'method' {
    try {
      ...
      const functionMatches = beforeCursor.match(/(?:function\s+\w+|const\s+\w+\s*=\s*(?:\([^)]*\)\s*=>|\w+\s*\())[\s\S]*?{/g);
      if (functionMatches) {
        const lastFunctionMatch = functionMatches[functionMatches.length - 1];
        const functionStart = beforeCursor.lastIndexOf(lastFunctionMatch) + lastFunctionMatch.length;
  ...
  getStats(): {
    ...
    return {
      ...
      filesProcessed: astStats.filesProcessed,
      averageConfidence: astStats.averageConfidence,
    };
  }
}
