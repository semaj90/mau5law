...
function vectorToPgVectorString(vec: number[]): string {
    // a: string, like: "[0.1,0.2,0.3]" which matches the previous code expectation
    return `[${vec.join(',')}]`;
}
...
function parseVectorString(vecValue: any): number[] {
    ...
    if (typeof vecValue === 'string') {
        ...
        // formats: "[1,2,3]" or: "1,2,3" or JSON array
        try {
    ...
    // fallback attempt: number conversion
...
interface PipelineStats {
    ...
    pipeline_config: { fastembed_url: string; cuda_enabled: boolean; };
}
...
class VectorPipelineService {
    ...
    async processDocuments(request: VectorPipelineRequest): Promise<ProcessingResult> {
        ...
        try {
            ...
            const objectsToProcess = request.batch_objects || (request.object_key ? [request.object_key] : await this.listBucketObjects(request.bucket_name));
            if (objectsToProcess.length === 0) {
                throw new Error('No objects found to process');
            }
            console.log(`Processing ${objectsToProcess.length} objects in bucket: ${request.bucket_name}`);
            ...
            return {
                ...
                results: errors.length > 0 ? undefined : results,
        ...
        try {
            ...
            try {
                if (isEmbeddingService(embeddingService)) {
                    const maybeResult = await embeddingService.embed({ texts, model, normalize: true, device: this.cudaEnabled ? 'cuda' : 'cpu' });
                    // embeddingService return: number[][] or { embeddings: number[][] }
                    if (Array.isArray(maybeResult) && maybeResult.length > 0 && Array.isArray(maybeResult[0])) {
                        return maybeResult as number[][];
                    }
                    if (
                        maybeResult && typeof maybeResult === 'object' && 'embeddings' in (maybeResult as object) &&
                        Array.isArray((maybeResult as { embeddings?: unknown }).embeddings)
                    ) {
                        return (maybeResult as { embeddings: number[][] }).embeddings;
                    }
                    // shape: fall through to HTTP fallback
                    console.warn('embeddingService returned unexpected shape, falling back to FastEmbed HTTP endpoint');
                }
            } catch (svcErr: any) {
                console.warn('embeddingService failed, falling back to FastEmbed endpoint: ', svcErr);
            }
            ...
            const response = await fetch(`${this.fastEmbedUrl}/embed`, {
                ...
                body: JSON.stringify({ texts, model, normalize: true, device: this.cudaEnabled ? 'cuda' : 'cpu' })
            });

            if (!response.ok) {
                throw new Error(`FastEmbed error: ${response.status} ${response.statusText}`);
            }
            const result = await response.json();
            // shapes: either: number[][] or { embeddings: number[][] }
        ...
        try {
            ...
            for (const result of results) {
                ...
                await dbClient.execute(
                    `
                    INSERT INTO document_embeddings (document_id, chunk_id, content, embedding, metadata, processed_at, model_used)
    ...
    private async checkExistingEmbeddings(documentId: string): Promise<EmbeddingResult[]> {
        try {
            ...
            return rows.map(row => {
                ...
                const modelUsed = typeof row['model_used'] === 'string' ? (row['model_used'] as string) : String(row['model_used'] ?? 'unknown');
    ...
    private async listBucketObjects(bucketName: string): Promise<string[]> {
        try {
            ...
            throw new Error(`Failed to list objects: ${err}`);
        }
    }
    ...
    async searchSimilar(
        query: string,
        options: { model?: string; limit?: number; threshold?: number; filters?: Record<string, unknown> } = {}
        ...
        const query_sql = `
            SELECT document_id, chunk_id, content, metadata, model_used, processed_at,
        ...
        try {
            ...
            return results.map(row => {
                ...
                // normalize to string | null
                const processedAt = row['processed_at'] == null ? null : typeof row['processed_at'] === 'string' ? (row['processed_at'] as string) : String(row['processed_at']);
    ...
    async getStats(): Promise<PipelineStats> {
        try {
            ...
            throw new Error(`Failed to retrieve stats: ${err}`);
        }
    }
}

// Add a typed embedding service interface and a runtime type-guard
type EmbedParams = { texts: string[]; model: string; normalize?: boolean; device?: string; };
...
export const GET: RequestHandler = async ({ url }) => {
    try {
        ...
        switch (action) {
            case 'search': {
                if (!query) {
                    throw error(400, 'Query parameter q is required for search');
                }
                ...
                return json({ query, results: searchResults, count: searchResults.length, options: { limit, threshold, model, filters } });
            }
            case 'stats': {
            ...
            default:
                throw error(400, 'Invalid action. Use ?action=search&q=query or ?action=stats');
        }
    } catch (err: any) {
        console.error('Vector pipeline error:  ', err);
...
// Add getOllamaEndpoint function
function getOllamaEndpoint(): string {
  return process.env.OLLAMA_URL || 'http://localhost:11434';
}

// Define embeddingService locally to integrate with Ollama
export const embeddingService = {
  async embed({ texts, model, normalize, device }: EmbedParams): Promise<number[][]> {
    ...
    if (!result || !Array.isArray(result.data)) {
        throw new Error('Ollama embedding service returned unexpected payload shape');
    }
    return result.data.map((r: any) => r.embedding);
  },
};
