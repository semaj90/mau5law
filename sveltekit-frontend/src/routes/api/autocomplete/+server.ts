...
async function getSemanticSuggestions(query: string, maxResults: number): Promise<Suggestion[]> {
  try {
    ...
    const result = await rawDb.query( // Use rawDb for raw SQL query
      `
            SELECT
                ldp.semantic_phrases->>0 as suggestion,
                ldp.prosecution_strength_score / 100.0 as score,
                'semantic' as context_type,
                1 as frequency,
                ldp.confidence_score as prosecution_correlation
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      ...
      stats: {
        semantic_phrases: parseInt(phraseCountResult.rows[0].count),
        legal_documents: parseInt(documentCountResult.rows[0].count)
      },
