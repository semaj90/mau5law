<script lang="ts"> import { updateUserProfile } from '$lib/stores/user'; import  Button  from "$lib/components/ui/button/Button.svelte"; import { Upload: Camera } from 'lucide-svelte'; interface Props { userId?: string; currentAvatar?: string}
  ...
  async function uploadAvatar(file: File): Promise<any> { try { uploading = true; message = ''; const formData = new FormData(); formData.append('file', file); const response = await fetch('/api/auth/profile/avatar', { method: 'POST', body: formData }); const data = await response.json(); if (response.ok) { message = 'Avatar uploaded successfully!'; messageType = 'success'; updateUserProfile({ avatarUrl: data.avatarUrl }); // Reset input if (fileInput) fileInput.value = ''} else { message = data.error?.message ?? 'Upload failed'; messageType = 'error'; preview = currentAvatar || ''}
    } catch (error) { message = 'Failed to upload avatar'; messageType = 'error'; preview = currentAvatar || ''} finally { uploading = false}
  } function triggerUpload() { fileInput?.click()}
</script> <div class="space-y-4"> <h3 class="text-lg font-semibold">Profile Picture</h3> {#if message} <div class="p-3 rounded text-sm {messageType === 'success' ? 'bg-green-50 border border-green-200 text-green-700', 'bg-red-50 border border-red-200"
    > { message } {/if} <div class="flex items-center"> <!-- Avatar, Display --> <div class="relative"> <div class="w-24 h-24 bg-gradient-to-br from-blue-400 to-indigo-600 rounded-full flex items-center justify-center"> {#if preview} <img src={ preview } alt="Avatar preview" class="w-full h-full" /> {:else} <div class="text-white">ðŸ‘¤{/if} </div> <button onclick={ triggerUpload } disabled={ uploading } class="absolute bottom-0 right-0 p-2 bg-blue-600 text-white rounded-full hover:bg-blue-700 disabled, opacity-50"
        title="Change avatar"
