...
<div class="container mx-auto">
  ...
  {#each results ?? [] as item (item.timestamp ?? item.prompt)}
      <li class="container mx-auto">
        <div class="container mx-auto">{String((item as { prompt?: unknown }).prompt ?? '')}</div>

        <div class="container mx-auto">{String((item as { response?: unknown }).response ?? '')}</div>

        <div class="container mx-auto">{String((item as { timestamp?: unknown }).timestamp ?? '')}</div>
      </li>
    {/each}
  </ul>
</div>
