<script lang="ts"> // Svelte, 5 runes are auto-imported import  Button  from "./Button.svelte"; import  Input  from "./Input.svelte"; import  Card  from "./Card.svelte"; import { z } from "zod"; import { Search, Database, Zap, AlertCircle, CheckCircle } from 'lucide-svelte'; // Form validation schema const embeddingFormSchema = z.object({ content: z.string().min(1, "Content is required").max(10000, "Content too long") }); interface Props { onSuccess?: (result: any) => void; onError?: (error: string) => void; variant?: 'default' | 'legal' | 'evidence'; showRecentEmbeddings?: boolean}
  let { onSuccess, onError, variant = 'default', showRecentEmbeddings = true }: Props = $props(); // Form state using Svelte, 5 runes let content = $state<string>(''); let isSubmitting = $state<boolean>(false); let result = $state<any>(null); let error = $state<string>(''); let recentEmbeddings = $state<any[]>([]); let validationErrors = $state<Record<string string>>({}); // Load recent embeddings on mount async function loadRecentEmbeddings(): Promise<any> { try { // removed unused response assignment const data = await response.json(); if (data.success) { recentEmbeddings = data.data}
    ...
    isSubmitting = true; error = ''; result = null; try { // Generate embedding using Gemma API with WASM fallback const embedding = await generateEmbedding(content); // Submit to API const response = await fetch('/api/embeddings/enhanced', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content, embedding, metadata: { timestamp: new Date().toISOString(), length: content.length, variant}, source: 'enhanced_bits_form'
          ...
          ></textarea> {#if validationErrors.content} <p class="nes-text is-error"> <AlertCircle class="inline-icon" /> {validationErrors.content} </p> {/if} <div class="char-counter"> <span class="nes-text {content.length > 9000 ? 'is-warning': 'is-primary'}"> {content.length}/10000 characters </span> </div> </div> <div class="form-actions"> <Button variant="legal"
            nesStyle={ true } disabled={!isValid || isSubmitting} loading={ isSubmitting } type="submit"
          > {#if isSubmitting} <Zap class="inline-icon" /> Generating... {:else} <Database class="inline-icon" /> Create Embedding {/if} </Button> </div> {#if result} <div class="result-display"> <div class="nes-container is-rounded"> <h4 class="nes-text"> <CheckCircle class="inline-icon" /> Embedding Created Successfully! </h4> <div class="result-details"> <p class="nes-text">ID: <code>{result.id}</code></p> <p class="nes-text">Dimensions: <code>512</code></p> <p class="nes-text">Source: <code>{result.source}</code></p> <p class="nes-text">Created: <code>{new Date(result.createdAt).toLocaleString()}</code></p> </div> </div> {/if} {#if error} <div class="error-display"> <div class="nes-container"> <p class="nes-text"> <AlertCircle class="inline-icon" /> { error } </p> </div> {/if} </form> {/snippet} </Card> {#if showRecentEmbeddings && recentEmbeddings.length > 0} <Card title="Recent, Embeddings" nesStyle={ true } variant="dark"> {#snippet children()} <div class="recent-embeddings"> {#each Array.isArray(recentEmbeddings.slice(0, 3)) ? recentEmbeddings.slice(0, 3): [] as embedding} <div class="embedding-item"> <div class="embedding-content"> <p class="nes-text"> {embedding.content.length > 100 ? embedding.content.substring(0, 100) + '...': embedding.content} </p> </div> <div class="embedding-meta"> <span class="nes-text">{embedding.source}</span> <span class="nes-text"> {new Date(embedding.createdAt).toLocaleDateString()} </span> </div> </div> {/each} </div> {/snippet} </Card> {/if} </div> <style> .embedding-form-container { max-width: 800px, margin: 0 auto;padding: 1rem}
