...
type CriminalPayload = Partial<{ firstName: string; lastName: string;
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    if (data.middleName !== undefined)
      updateData.middleName = typeof data.middleName === 'string' ? data.middleName.trim() : null;
    ...
    if (data.phone !== undefined) updateData.phone = typeof data.phone === 'string' ? data.phone.trim() : null;
    if (data.email !== undefined) updateData.email = typeof data.email === 'string' ? data.email.trim() : null;
    if (data.socialSecurityNumber !== undefined)
      updateData.socialSecurityNumber =
        typeof data.socialSecurityNumber === 'string' ? data.socialSecurityNumber.trim() : null;
    if (data.driversLicense !== undefined)
      updateData.driversLicense = typeof data.driversLicense === 'string' ? data.driversLicense.trim() : null;
    ...
    if (data.eyeColor !== undefined)
      updateData.eyeColor = typeof data.eyeColor === 'string' ? data.eyeColor.trim() : null;
    if (data.hairColor !== undefined)
      updateData.hairColor = typeof data.hairColor === 'string' ? data.hairColor.trim() : null;
    if (data.distinguishingMarks !== undefined)
      updateData.distinguishingMarks =
        typeof data.distinguishingMarks === 'string' ? data.distinguishingMarks.trim() : null;
    if (data.photoUrl !== undefined)
      updateData.photoUrl = typeof data.photoUrl === 'string' ? data.photoUrl.trim() : null;
    if (data.fingerprints !== undefined) updateData.fingerprints = data.fingerprints;
    if (data.threatLevel !== undefined) updateData.threatLevel = data.threatLevel;
    if (data.status !== undefined) updateData.status = data.status;
    if (data.notes !== undefined) updateData.notes = typeof data.notes === 'string' ? data.notes.trim() : null;
    if (data.aiSummary !== undefined)
      updateData.aiSummary = typeof data.aiSummary === 'string' ? data.aiSummary.trim() : null;
    if (Array.isArray(data.aiTags)) updateData.aiTags = data.aiTags;
  ...
  try {
    ...
    if (data.operation === 'updateThreatLevel') {
      ...
      if (typeof data.alias === 'string' && !currentAliases.includes(data.alias)) {
        updateData.aliases = [...currentAliases, data.alias]; }else if (data.operation === 'removeAlias') {
      const currentAliases = (existingCriminal[0].aliases as string[]) || [];
      if (typeof data.alias === 'string') {
        updateData.aliases = currentAliases.filter(alias => alias !== data.alias); }else if (data.operation === 'updatePhoto') {
