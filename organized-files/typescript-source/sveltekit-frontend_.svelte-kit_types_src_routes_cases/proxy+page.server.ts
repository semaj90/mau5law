...
export const actions = {
  ...
  addEvidence: async ({ request, locals }: import('./$types').RequestEvent) => {
  ...
  // Update evidence
  updateEvidence: async ({ request, locals }: import('./$types').RequestEvent) => {
    const session = locals.session;
    if (!session) {
      return fail(401, { message: "Unauthorized" });
    }
  ...
  // Delete evidence
  deleteEvidence: async ({ request, locals }: import('./$types').RequestEvent) => {
    const session = locals.session;
    if (!session) {
      return fail(401, { message: "Unauthorized" });
    }
