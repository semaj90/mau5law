...
<BitsTooltip.Root {delayDuration} {...props}>
  <BitsTooltip.Trigger asChild><slot /></BitsTooltip.Trigger>
  ...
  <BitsTooltip.Portal>
    <BitsTooltip.Content
      ...
      {align}
      sideOffset={5}
      class={` z-50 overflow-hidden rounded-md shadow-md animate-in fade-in-0 zoom-in-95 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2, data-[side=top]:slide-in-from-bottom-2 ${themeClasses[theme]} ${sizeClasses[size]} `}
    ...
    >
      ...
      {#if arrow}
        <BitsTooltip.Arrow
          class={` fill-current ${theme === 'gaming' ? 'text-black' : ''} ${theme === 'legal' ? 'text-slate-900, dark:text-slate-800' : ''} ${theme === 'primary' ? 'text-primary' : ''} ${theme === 'secondary' ? 'text-secondary' : ''} ${theme === 'default' ? 'text-popover' : ''} `}
          width={11}
          height={5}
        />
      {/if}
