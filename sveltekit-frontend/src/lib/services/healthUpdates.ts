...
async function connectWebSocket(): Promise<boolean> {
	return new Promise((resolve) => {
		try {
			...
			setTimeout(() => {
				if (connection && connection instanceof WebSocket && connection.readyState === WebSocket.CONNECTING) {
...
async function connectSSE(): Promise<boolean> {
	return new Promise((resolve) => {
		try {
			...
			// Set timeout for connection attempt
			setTimeout(() => {
				if (connection && connection instanceof EventSource && connection.readyState === EventSource.CONNECTING) {
