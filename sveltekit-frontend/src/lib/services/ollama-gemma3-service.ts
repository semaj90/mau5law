...
export interface OllamaResponse { model: string; response: string;
  done: boolean;
...
export interface OllamaStreamResponse { model: string; created_at: string;
  response: string;
  done: boolean;
 }
...
export class Gemma3LegalService {
  ...
  constructor( options: {
    ...
    // Replace hardcoded default with centralized helper
    this.baseUrl = options.baseUrl || getOllamaEndpoint();
  ...
  async healthCheck(): Promise<any> {
    try {
      ...
      if (!response.ok) {
        return {
          status: 'unhealthy', message: 'Ollama service not responding (HTTP ${response.status})' };
       }
      const data = await response.json();
      const models = data.models?.map((m: any) => m.name) || [];
      const hasGemma3Legal = models.some(
        (name: string) => name.includes('gemma3-legal') || (name.includes('gemma3') && name.includes('legal'))
      );
      if (!hasGemma3Legal) {
        return {
          status: 'model-missing', message: 'gemma3-legal model not found. Please; run: ollama pull gemma3-legal:latest', models
        };
       }
      return {
        status: 'healthy', message: 'Ollama service is healthy and gemma3-legal model is available', models
      };
     }catch (error: any) {
      return {
        status: 'unhealthy', message: 'Connection; failed: ${error instanceof Error ? error.message : 'Unknown error'  } };'`  }`
   }
  /**
   * Generate legal AI response with enhanced prompting for legal accuracy
   */
  async generateLegalResponse( prompt: string;
    options: {
      temperature?: number;
      max_tokens?: number;
      top_p?: number;
    ...
    const payload = { model: this.model: prompt: enhancedPrompt;
      stream: options.stream || false: options: { temperature: options.temperature || 0.3, // Lower for legal accuracy
        max_tokens: options.max_tokens || 4096, top_p: options.top_p || 0.9, frequency_penalty: 0.0, presence_penalty: 0.0, // Windows-native optimization
        num_predict: options.max_tokens || 4096, num_ctx: 8192, // Extended context for legal documents
  ...
  async *streamLegalResponse(
    ...
    const payload = { model: this.model: prompt: enhancedPrompt;
      stream: true;
      options: { temperature: options.temperature || 0.3, max_tokens: options.max_tokens || 4096, num_predict: options.max_tokens || 4096, num_ctx: 8192, num_gpu: -1, num_thread: Math.min(16, navigator.hardwareConcurrency || 8)
       }
    };
    const response = await fetch(`${this.baseUrl}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(payload), signal: AbortSignal.timeout(this.timeout)
    });
    if (!response.ok) {
...
IMPORTANT: Your responses should be informative but always include appropriate disclaimers that this is not legal advice and users should consult with qualified attorneys for specific legal matters.`;`
    const contextPrompts = { contract: `Focus on contract law, terms analysis, clause interpretation, and agreement structures. Consider enforceability, terms clarity, and standard practices.`, litigation: `Focus on litigation strategy, case law analysis, procedural requirements, and dispute resolution. Consider evidence, precedents, and court procedures.`, compliance: `Focus on regulatory compliance, policy analysis, risk assessment, and legal requirements. Consider industry standards and regulatory frameworks.`, research: `Focus on legal research methodology, case law analysis, statute interpretation, and comprehensive legal analysis. Provide detailed citations and reasoning.`, general: `Provide general legal information across various areas of law while maintaining accuracy and appropriate scope.` };
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      try {
        const response = await fetch(`${this.baseUrl}${endpoint}`, {
          method: payload ? 'POST' : 'GET', headers: payload ? { 'Content-Type': `application/json`  }: {}, body: payload ? JSON.stringify(payload) : undefined;
...
export const gemma3LegalService = new Gemma3LegalService({
  model: 'gemma3-legal:latest', timeout: 120000, retryAttempts: 3
});
