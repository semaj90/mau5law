...
export interface ConnectedClient {
  ...
  lastSeen: Date;
  metadata?: { userAgent?: string; userId?: string; sessionId?: string };
}
class EnhancedWebSocketManager {
  ...
  broadcastCacheUpdate(cacheStats: any) {
    this.broadcast({ type: 'cache_update', data: cacheStats, timestamp: new Date() }, (client) =>
      client.subscriptions.has('cache_stats')
    );
  }
  private startHealthCheck() {
    this.healthCheckInterval = setInterval(() => {
      ...
      this.broadcast({
        type: 'system_health',
        data: { ping: 'health_check', serverTime: now, connectedClients: this.clients.size },
        timestamp: now,
      });
    }, 60000); // Every minute
  }
