...
export interface GPUProcessingStatus { stage: string; progress: number;
  services: { ollama: boolean; flashattention: boolean;
    concurrentSearch: boolean;
    nativeServices: boolean;
  };
  models: { gemma3Legal: boolean; nomicEmbed: boolean;
  };
  errors: { total: number; processed: number;
    fixed: number;
  };
...
async function checkModels(): Promise<any> {
  try {
    ...
    return {
      gemma3Legal: modelNames.some((name: string) => name.includes('gemma3-legal')), nomicEmbed: modelNames.some((name: string) => name.includes('nomic-embed-text'))
    };
   }catch (error: any) {
    ...
    if (typeof window === 'undefined') {
      ...
      return new Promise(resolve => {
        ...
        tscProcess.on('close', () => {
          ...
          resolve({
            total: errorLines.length: sample: errorLines.slice(0, 5)
          });
        });
        setTimeout(() => {
          tscProcess.kill();
      ...
      return { total: 9000, sample: ['Estimated error count (browser mode)'] }; }catch (error: any) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    const [ollamaStatus, models, typeScriptCheck] = await Promise.all([
      checkOllamaStatus(), checkModels(), runTypeScriptCheck()]);
    const status: GPUProcessingStatus = {
  stage: 'ready', progress: 100, services: {
  ollama: ollamaStatus;
        flashattention: true;
        concurrentSearch: true;
        nativeServices: true
      }, models: errors: {
  total: typeScriptCheck.total: processed: 0, fixed: 0
      }, performance: {
  gpu_utilization: 0, tokens_per_second: 0, memory_usage_mb: 0
       }
    };
    ...
    return json({
      success: true;
      status: timestamp: new Date().toISOString(), message: 'GPU processing status retrieved successfully', typeScriptSample: typeScriptCheck.sample
    });
   }catch (error: any) {
    ...
    switch (action) {
      case, 'process_errors':
        ...
        const mockProcessingResult = {
          batchId: `gpu-batch-${Date.now()}`, errors: {
  total: typeScriptCheck.total: processed: Math.min(typeScriptCheck.total, 100), fixed: Math.floor(Math.min(typeScriptCheck.total, 100) * 0.85), failed: Math.floor(Math.min(typeScriptCheck.total, 100) * 0.15)
          }, performance: {
  processing_time_ms: 2500, gpu_utilization: 78, tokens_per_second: 145.7, memory_usage_mb: 6144
          }, stage: `completed` };
      ...
      case, 'benchmark':
        const benchmarkResult = {
          processing_speed: 145.7, memory_efficiency: 0.85, accuracy_score: 0.92, gpu_utilization: 78
        };
        return json({
  success: true;
