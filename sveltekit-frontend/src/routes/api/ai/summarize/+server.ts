...
const, originalGETHandler: RequestHandler = async () => {
  try {
    ...
    return json({
      ...
      models: models.models?.map(m => m.name ?? 'unknown') || [],
    ...
    if (cacheKey && options.cache) {
      ...
      if (cached.entry) {
        ...
        return json({
          ...
          model: cachedEntry.model || 'unknown',
          type,
          mode,
          structured: cachedEntry.structured || null,
          cached: true,
          cacheKey,
          cacheSource: cached.source,
          originalLength: text.length,
          summaryLength: cachedEntry.summary.length,
