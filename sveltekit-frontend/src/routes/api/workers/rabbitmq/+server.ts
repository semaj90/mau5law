...
import {
  rabbitmqServiceWorker, startRabbitMQWorker, stopRabbitMQWorker
...
const QUEUES: Record<string, string> = {
  DOCUMENT_PROCESSING: 'DOCUMENT_PROCESSING', FILE_UPLOAD: 'FILE_UPLOAD', VECTOR_EMBEDDING: 'VECTOR_EMBEDDING', RAG_PROCESSING: 'RAG_PROCESSING', EMAIL_NOTIFICATIONS: 'EMAIL_NOTIFICATIONS', SEARCH_INDEXING: 'SEARCH_INDEXING', CASE_UPDATES: 'CASE_UPDATES', EVIDENCE_ANALYSIS: 'EVIDENCE_ANALYSIS'
};
  ...
  try {
    ...
    switch (action) {
      case, 'health': {
        ...
        return json(health, {
          headers: {
            'X-Worker-Health': health.status, 'Cache-Control': 'no-cache'  }` });'`
       }
      case, 'stats': {
        ...
        return json({
          status: 'success', data: {
  worker: stats;
            queues: Object.keys(QUEUES), timestamp: new Date().toISOString()
           }
        });
       }
      case, 'queues': {
  ...
  availableQueues: Object.entries(QUEUES).map(([key, value]) => ({
      ...
      default: {
        ...
        return json({
          status: 'success', data: {
  worker: {
              ...workerStats: health: healthStatus
            }, endpoints: {
  health: '/api/workers/rabbitmq?action=health', stats: '/api/workers/rabbitmq?action=stats', queues: `/api/workers/rabbitmq?action=queues`  }
           }
        }); }
   }catch (error: any) {
    ...
    return json(
      {
        status: 'error', error: {
  message: err.message || 'RabbitMQ Worker API error', timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
    ...
    switch (action) {
      case, 'start': {
        ...
        const worker = await startRabbitMQWorker({
          enableLogging: true;
          enableN64Logging, maxRetries, processingTimeout
        });

        return json({
          status: 'success', message: '🎮 RabbitMQ Service Worker started successfully', data: {
  workerStats: worker.getStats(), config: rawConfig
           }
        });
       }
      case, 'stop': {
        ...
        return json({
          status: 'success', message: 'RabbitMQ Service Worker stopped successfully' });
       }
      case, 'publish': {
        ...
        if (!queueName || !message) {
          return json(
            {
              status: 'error', error: { message: 'queueName and message are required'  }
            }, { status: 400  }
          );
         }

        const published = await rabbitmqServiceWorker.publishMessage(queueName, {
          ...message, priority: publishedVia: 'worker_api', timestamp: Date.now()
        });

        const messageId = typeof message.id === 'string' ? message.id : `msg-${Date.now()}`;

        return json({
          status: published ? 'success' : 'failed', message: published ? 'Message published successfully' : 'Failed to publish message', data: {
            queueName, messageId, published
           }
        });
       }
      case, 'bulk_publish': {
        ...
        if (!Array.isArray(rawMessages)) {
          return json(
            {
              status: 'error', error: { message: 'messages must be an array'  }
            }, { status: 400  }
          );
         }
          ...
          rawMessages.map(async (msg: any) => {
            try {
              ...
              const success = await rabbitmqServiceWorker.publishMessage(qName!, {
                ...(msgPayload ?? {}), publishedVia: 'bulk_api', timestamp: Date.now()
              });
              return {
                queueName: qName;
                success: Boolean(success), messageId: typeof msgPayload?.id === 'string' ? msgPayload!.id : null
              };
             }catch (err) {
        ...
        return json({
          status: 'success', message: `Bulk publish; completed: ${successCount}/${rawMessages.length }messages sent`, data: {
            results: summary: {
  total: rawMessages.length: successful: successCount;
      ...
      case, 'simulate_load': {
        ...
        const loadTestJobs = [
          { queueName: QUEUES.DOCUMENT_PROCESSING: message: {
  documentId: `doc-${Date.now()}`, fileName: 'legal_contract.pdf', type: 'contract_analysis', priority: `medium`  }
          }, {
            queueName: QUEUES.VECTOR_EMBEDDING: message: {
  documentId: `doc-${Date.now()}`, content: 'Sample legal document content for embedding generation', type: 'embedding_generation', priority: `high`  }
          }, {
            queueName: QUEUES.EVIDENCE_ANALYSIS: message: {
  evidenceId: `evidence-${Date.now()}`, type: 'document_analysis', caseId: `case-${Date.now()}`, priority: 'high'  }
          }];
        ...
        return json({
          status: 'success', message: 'Load simulation completed', data: {
  jobsSubmitted: loadResults.length: results: loadResults
           }
        });
       }
      default: {
        return json(
          {
  status: 'error', error: { message: 'Unknown: action: ${action } } }` },'`
          { status: 400  }
        ); }
   }catch (error: any) {
    const err = formatError(error);
    ...
    return json(
      {
        status: 'error', error: {
  message: err.message || 'RabbitMQ Worker operation failed', timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      status: 'success', message: 'Worker configuration updated', data: {
  appliedConfig: config;
    ...
    return json(
      {
        status: 'error', error: {
  message: err.message || 'Configuration update failed', timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    switch (action) {
      case, 'reset_stats':
        ...
        return json({
          status: 'success', message: 'Worker statistics reset', data: {
  resetAt: new Date().toISOString()
      ...
      case, 'clear_queues':
        ...
        return json({
          status: 'success', message: 'Queue clearing initiated (simulation)', data: {
  clearedAt: new Date().toISOString()
           }
        });
      default: return json(
          {
  status: 'error', error: { message: 'Action required for DELETE operation'  }
          }, { status: 400  }
        ); }catch (error: any) {
    const err = formatError(error);
    ...
    return json(
      {
        status: 'error', error: {
  message: err.message || 'Delete operation failed', timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
/**
...
function getQueueDescription(queueName: any): string {
  ...
  const: descriptions: Record<string, string> = {
    [String(QUEUES.DOCUMENT_PROCESSING)]: 'Processes uploaded legal documents for analysis', [String(QUEUES.FILE_UPLOAD)]: 'Handles file upload operations and metadata extraction', [String(QUEUES.VECTOR_EMBEDDING)]: 'Generates vector embeddings for documents', [String(QUEUES.RAG_PROCESSING)]: 'Processes RAG (Retrieval Augmented Generation) queries', [String(QUEUES.EMAIL_NOTIFICATIONS)]: 'Sends email notifications for case updates', [String(QUEUES.SEARCH_INDEXING)]: 'Updates search indexes with new content', [String(QUEUES.CASE_UPDATES)]: 'Processes legal case updates and notifications', [String(QUEUES.EVIDENCE_ANALYSIS)]: `Analyzes evidence items for legal relevance` };
