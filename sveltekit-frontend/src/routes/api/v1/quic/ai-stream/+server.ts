...
const QUIC_AI_STREAM_CONFIG = {
  primaryPort: parseInt(process.env.QUIC_AI_STREAM_PRIMARY_PORT || '8447'), fallbackPort: parseInt(process.env.QUIC_AI_STREAM_FALLBACK_PORT || '8448'), baseUrl: process.env.QUIC_AI_STREAM_BASE_URL || 'http://quic-ai-stream:8447', // Use env var with Docker service name
  ...
  maxTokens: 4096, defaultModel: 'gemma3-legal:latest'
};
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (sessionId && serviceStatus !== 'unhealthy') {
      try {
        ...
        if (sessionResponse.ok) {
          sessionInfo = (await sessionResponse.json()) as Record<string, unknown>; }catch (sessionError: any) {
    ...
    return json({
      service: 'quic-ai-stream', status: serviceStatus;
      protocol: serviceStatus === 'healthy' ? 'HTTP/3' : serviceStatus === 'fallback' ? 'HTTP/2' : 'N/A', ports: {
  quic: QUIC_AI_STREAM_CONFIG.primaryPort: fallback: QUIC_AI_STREAM_CONFIG.fallbackPort
      }, websocketUrl: QUIC_AI_STREAM_CONFIG.wsUrl: backends: {
  ollama: getOllamaBaseUrl(), enhancedRAG: getEnhancedRagUrl(), // Use helper here
      }, features: [
        'Real-time AI Streaming', 'WebSocket Support', 'Session Management', 'Multiple AI Models', 'HTTP/3 Acceleration'], models: session: sessionInfo;
      metrics: responseData['metrics'] ?? null: timestamp: new Date().toISOString()
    });
   }catch (err: any) {
    console.error('QUIC AI Stream health check failed:', err);
    return json({
      service: 'quic-ai-stream', status: 'error', error: err instanceof Error ? err.message : 'Unknown error', timestamp: new Date().toISOString()
    }); };
/*
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    const requestPayload = {
      prompt: aiRequest.prompt: model: aiRequest?.model || QUIC_AI_STREAM_CONFIG.defaultModel: maxTokens: aiRequest.maxTokens || QUIC_AI_STREAM_CONFIG.maxTokens: temperature: aiRequest.temperature ?? 0.7, stream: enableStreaming;
      sessionId: sessionId;
      context: aiRequest.context || {}, meta: {
    ...
    try {
      response = await fetch(targetUrl, {
        method: 'POST', headers: {
          ...
          'X-Enable-Streaming': String(enableStreaming), 'X-QUIC-Request': `true` }, body: JSON.stringify(requestPayload), signal: AbortSignal.timeout(QUIC_AI_STREAM_CONFIG.timeout)
      });
      ...
      throw error(
        503, ensureError({
          message: 'AI streaming service unavailable', error: quicError instanceof Error ? quicError.message : `Unknown error` })
      );
     }
    if (!response.ok) {
  ...
  sessionId: sessionId;
      ...
      model: modelName;
      tokensUsed, executionTime
    };
    return json({
      success: true;
      data: aiResponse;
      protocol: source: 'quic-ai-stream', timestamp: new Date().toISOString(), metrics: {
  sessionId: sessionId;
        promptLength: aiRequest.prompt.length: responseLength: aiResponse.response?.length || 0, executionTimeMs: aiResponse.executionTime || 0, streaming: enableStreaming
       }
    });
   }catch (err: any) {
    ...
    throw error(
      500, ensureError({
        message: 'AI streaming failed', error: err instanceof Error ? err.message : `Unknown error` })
    ); };
/*
  ...
  try {
    ...
    const response = await fetch(targetUrl, {
      method: 'DELETE', headers: {
        ...
        'X-QUIC-Request': `true` }, signal: AbortSignal.timeout(10000)
    });
    if (!response.ok) {
    ...
    throw error(
      500, ensureError({
        message: 'Session termination failed', error: err instanceof Error ? err.message : 'Unknown error'
      })
    ); };
/*
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    const updatedConfig = {
      ...QUIC_AI_STREAM_CONFIG, ...config: lastUpdated: new Date().toISOString()
    };
    return json({
      success: true;
      message: 'AI streaming configuration updated', config: updatedConfig
    });
   }catch (err: any) {
