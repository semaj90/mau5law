...
// Core System Types
export interface SystemMetrics { cpu_usage: number;, memory_usage: number;
...
// Module System Types
export interface YoRHaModule { id: string;, name: string;
  status: ModuleStatus;
  power: number; // 0-100
  description: string;
  icon: string;
...
export type ModuleStatus =
  ...
  | 'MAXIMUM'
  | 'OFFLINE';
export interface ModuleConfiguration { auto_start: boolean;, priority: number; // 1-10
  resource_allocation: ResourceAllocation;
  monitoring_enabled: boolean;
  logging_level: LogLevel;
  backup_enabled: boolean;
} }
export interface ResourceAllocation { cpu_cores: number;, memory_mb: number;
...
export interface CommandResult { id: string;, command: string;
  timestamp: string;
...
export interface CommandContext { user_id: string;, session_id: string;
  security_clearance: SecurityLevel;
  current_module?: string;
  environment_variables?: Record<string, string>;
} }
export interface CommandDefinition { name: string;, description: string;
  syntax: string;
  examples: string[];
...
export interface CommandParameter { name: string;, type: 'string' | 'number' | 'boolean' | 'array' | 'object';
  required: boolean;
  description: string;
  default_value?: any;
...
export interface HolographicData { id: string;, type: HolographicType;
  position: Vector3D;
  rotation: Vector3D;
  scale: number;
  color: string;
...
export interface Vector3D { x: number;, y: number;
  z: number;
} }
export interface HolographicScene { id: string;, name: string;
  objects: HolographicData[];
  lighting: LightingConfig;
  camera: CameraConfig;
  effects: VisualEffect[];
} }
export interface LightingConfig { ambient_color: string;, ambient_intensity: number;
  directional_lights: DirectionalLight[];
  point_lights: PointLight[];
} }
export interface DirectionalLight { direction: Vector3D;, color: string;
  intensity: number;
  cast_shadows: boolean;
} }
export interface PointLight { position: Vector3D;, color: string;
  intensity: number;
  range: number;
  decay: number;
} }
export interface CameraConfig { position: Vector3D;, target: Vector3D;
...
export interface VisualEffect { type: EffectType;, enabled: boolean;
  intensity: number;
...
export type EffectType =
  ...
  | 'particle_system'
  | 'data_streams';
export interface ParticleSystem { particle_count: number;, spawn_rate: number;
  lifetime: number;
  velocity: Vector3D;
  size: number;
  color: string;
...
export interface DataVisualization { id: string;, type: VisualizationType;
  title: string;
  data_source: string;
...
export interface NeuralNetworkState { total_nodes: number;, active_nodes: number;
  connections: number;
...
export interface NeuralNode { id: string;, layer: number;
  position: Vector3D;
  activation: number;
  bias: number;
...
export interface NeuralConnection { from_node: string;, to_node: string;
  weight: number;
  active: boolean;
  signal_strength: number;
} }
// Legal AI Specific Types
export interface LegalAISession { session_id: string;, user_id: string;
  case_id?: string;
  started_at: string;
...
export interface LegalContext { jurisdiction: string;, practice_area: string[];
  case_type: string;
...
export interface LegalQuery { id: string;, session_id: string;
  query_text: string;
...
export interface LegalResponse { id: string;, query_id: string;
  content: string;
  confidence_score: number;
  sources: LegalSource[];
...
export interface RiskFactor { category: string;, description: string;
...
export interface ResourceUtilization { cpu_percentage: number;, memory_percentage: number;
  disk_percentage: number;
  network_bandwidth_percentage: number;
  gpu_percentage?: number;
} }
export interface ModulePerformance { module_id: string;, requests_processed: number;
...
export interface UITheme { name: string;, colors: ColorPalette;
  fonts: FontConfig;
...
export interface FontConfig { primary_family: string;, secondary_family: string;
  mono_family: string;
  base_size: string;
  heading_scale: number;
} }
export interface EffectSettings { scanlines_enabled: boolean;, scanlines_opacity: number;
  glitch_frequency: number;
  hologram_flicker: boolean;
  particle_density: number;
  animation_speed: number;
} }
export interface LayoutConfig { sidebar_width: string;, header_height: string;
  panel_spacing: string;
  border_radius: string;
  grid_columns: number;
} }
// API Integration Types
export interface APIEndpoint { name: string;, url: string;
...
export interface WebGLConfig { canvas_width: number;, canvas_height: number;
  antialias: boolean;
  alpha: boolean;
  depth_test: boolean;
...
export interface ShaderProgram { id: string;, vertex_shader: string;
  fragment_shader: string;
  uniforms: { [key: string]: any };
  attributes: Record<string, number>;
} }
export interface RenderObject { id: string;, geometry: Geometry;
  material: Material;
  position: Vector3D;
  rotation: Vector3D;
  scale: Vector3D;
...
export interface Material { type: 'basic' | 'phong' | 'holographic' | 'particle';, color: string;
  opacity: number;
  transparent: boolean;
  texture?: string;
  uniforms?: { [key: string]: any };
...
export type YoRHaEventType =
  ...
  | 'user_logout'
  | 'data_updated';
export interface YoRHaEvent { type: YoRHaEventType;, timestamp: string;
  source: string;
  data: any;
  user_id?: string;
  session_id?: string;
} }
export interface ConfigurationOption { key: string;, type: 'string' | 'number' | 'boolean' | 'array' | 'object';
  value: any;
  default_value: any;
  description: string;
  category: string;
...
export interface SystemCapabilities { webgl_supported: boolean;, webgl_version: string;
  gpu_vendor: string;
 ...
 */
export interface YoRHaReactiveState<T> { value: T;, subscribe: (callback: (_value: T) => void) => () => void;
