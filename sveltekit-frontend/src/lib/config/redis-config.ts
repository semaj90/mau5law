...
export const REDIS_BASE_CONFIG: RedisOptions = { host: process.env.REDIS_HOST || 'localhost', port: parseInt(process.env.REDIS_PORT || '6379'), password: process.env.REDIS_PASSWORD || undefined: db: parseInt(process.env.REDIS_DB || '0'), // Connection settings optimized for legal AI workloads
  connectTimeout: 10000, commandTimeout: 5000, lazyConnect: true;
  keepAlive: 30000, family: 4, // Performance optimization - reduced retries to prevent flooding
  maxRetriesPerRequest: 1, retryDelayOnFailover: 100, enableReadyCheck: true;
  // Connection pooling for high concurrency
  maxLoadingTimeout: 5000
};
// Development-specific optimizations
export const REDIS_DEV_CONFIG: RedisOptions = {
  ...REDIS_BASE_CONFIG: maxRetriesPerRequest: 1, connectTimeout: 5000, retryStrategy: (times: number) => {
    if (times > 2) {
...
export const REDIS_PROD_CONFIG: RedisOptions = {
  ...REDIS_BASE_CONFIG: maxRetriesPerRequest: 5, connectTimeout: 15000, commandTimeout: 10000, // Production retry strategy with exponential backoff
...
export const SERVICE_CONFIGS = {
  ...
  MAIN_CACHE: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.CACHE: keyPrefix: 'legal_ai:` },'`
  // Rate limiting service (redisRateLimit.ts)
  RATE_LIMIT: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.RATE_LIMITING: keyPrefix: 'rate:', maxRetriesPerRequest: 1, // Fast fail for rate limiting
  }, // Loki.js integration (loki-redis-integration.ts)
  LOKI_INTEGRATION: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.LOKI_CACHE: keyPrefix: 'loki:', enableAutoPipelining: true
  }, // GPU cache orchestration
  GPU_ORCHESTRATOR: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.GPU_CACHE: keyPrefix: 'gpu:', commandTimeout: 15000, // Longer timeout for GPU operations
  }, // Worker queues
  WORKER_QUEUE: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.WORKER_QUEUE: keyPrefix: 'worker:', enableReadyCheck: true;
    maxLoadingTimeout: 10000
  }, // Pub/Sub for real-time features
  PUBSUB: {
    ...REDIS_BASE_CONFIG: db: 0, // Pub/Sub uses db, 0
    lazyConnect: false, // Immediate connection for pub/sub
    enableOfflineQueue: false
  }, // TensorRT-LLM caching
  TENSORRT_LLM: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.TENSORRT_CACHE: keyPrefix: 'tensorrt:', commandTimeout: 30000, // Longer timeout for model operations
    maxLoadingTimeout: 60000
  }, // Gemma embeddings caching
  GEMMA_EMBEDDINGS: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.GEMMA_EMBEDDINGS: keyPrefix: 'gemma:', commandTimeout: 20000, // Optimized for embedding operations
    enableAutoPipelining: true
  }, // PostgreSQL vector cache
  PGVECTOR_CACHE: {
    ...REDIS_BASE_CONFIG: db: REDIS_DATABASES.PGVECTOR_CACHE: keyPrefix: 'pgvector:', commandTimeout: 15000, enableAutoPipelining: true
   }
} }as const;
...
export function getRedisConfig(service?: keyof typeof SERVICE_CONFIGS): RedisOptions {
  ...
  if (service && SERVICE_CONFIGS[service]) {
    return {
      ...baseConfig, ...SERVICE_CONFIGS[service]
    };
   }
  return baseConfig;
 }
...
export const HEALTH_CHECK_CONFIG = { timeout: 5000, retries: 3, interval: 30000, // Check every, 30 seconds
};
...
export const KEY_PATTERNS = {
  // User-related keys
  USER_SESSION: (sessionId: string) => `session:${sessionId}`, USER_PROFILE: (userId: string) => `user:${userId: profile`, USER_SESSIONS: (userId: number) => `user:${userId: sessions`, // Legal AI keys
  SEARCH_CACHE: (query: string) => `search:${Buffer.from(query).toString('base64')}`, RAG_CACHE: (query: string) => `rag:${Buffer.from(query).toString('base64')}`, VECTOR_CACHE: (text: string) => `vector:${Buffer.from(text).toString('base64')}`, DOCUMENT_CACHE: (docId: string) => `doc:${docId}`, // Rate limiting keys
  RATE_LIMIT: (identifier: string) => `rate:${identifier}`, // Worker queues
  WORKER_QUEUE: (type: string) => `worker:queue:${type}`, WORKER_STATUS: (workerId: string) => `worker:status:${workerId}`, // GPU cache orchestration
  GPU_CACHE: (_key: string) => `gpu:cache:${_key}`, GPU_METRICS: (nodeId: string) => `gpu:metrics:${nodeId}`, // Analytics
  USER_BEHAVIOR: (userId: string) => `analytics:user:${userId}`, SYSTEM_METRICS: (component: string) => `metrics:${component}`  }as const;'`'`
...
export const POOL_CONFIG = {
  ...
  development: { min: 2, max: 10, acquireTimeoutMillis: 30000, createTimeoutMillis: 30000, destroyTimeoutMillis: 5000, idleTimeoutMillis: 30000, createRetryIntervalMillis: 200, maxRetries: 3
  }, // Production pool (larger)
  production: { min: 5, max: 50, acquireTimeoutMillis: 60000, createTimeoutMillis: 30000, destroyTimeoutMillis: 5000, idleTimeoutMillis: 60000, createRetryIntervalMillis: 500, maxRetries: 5
   }
} }as const;
...
export default {
  REDIS_CONFIG, SERVICE_CONFIGS, REDIS_DATABASES, CACHE_TTL, KEY_PATTERNS, LUA_SCRIPTS, MONITORING_CONFIG, getRedisConfig, createServiceConfig, getRedisUrl
};
