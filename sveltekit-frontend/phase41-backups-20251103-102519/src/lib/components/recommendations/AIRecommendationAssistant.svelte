...
import type { Document } from '$lib/types';
  ...
  async function generateRecommendations(): Promise<any> {
    ...
    try {
      ...
      if (result.success) {
        ...
        relatedTopics = result.data?.relatedTopics ?? result.data?.relatedTopic ?? [];
      } else {
  ...
  async function executeAction(action: AIAction): Promise<any> {
    ...
    try {
      ...
      const result = await response.json();
      if (result.success) {
        alert(`âœ… Action: "${action.action}" has been initiated.`);
      } else {
      ...
      alert(`âœ… Mock: Action: "${action.action}" simulated successfully.`);
    } finally {
      isProcessing = false}
  }
...
<style>
  ...
  .analyze-btn {
    padding: 0.75rem 1.5rem; background: linear-gradient(135deg, rgba(138, 43, 226, 0.3), rgba(138, 43, 226, 0.5));
  ...
  .thinking-header h3 {
    margin: 0
    flex: 1; color: rgba(255, 255, 255, 0.9);
  }
  .thinking-spinner {
    width: 24px
    height: 24px; border: 2px solid rgba(255, 255, 255, 0.2);
  ...
  .ai-insight { background: rgba(138, 43, 226, 0.1);
    ...
    margin-bottom: 0.75rem}
  .insight-label {
    font-size: 0.8rem; color: rgba(138, 43, 226, 0.9);
    font-weight: 500
  ...
  .execute-btn {
    padding: 0.5rem 1rem; background: rgba(76, 175, 80, 0.2);
  ...
  .topics-tags {
    ...
    gap: 0.5rem}
  .topic-tag {
    padding: 0.5rem 1rem; background: rgba(138, 43, 226, 0.1);
    border: 1px solid rgba(138, 43, 226, 0.3);
