...
import type { Case } from '$lib/types'; import { afterUpdate, onMount, tick } from "svelte"; import { elasticOut, quintOut } from "svelte/easing"; import { writable, type Writable } from "svelte/store"; import { fade, fly, scale } from "svelte/transition"; // Props (use standard export let for Svelte compatibility) const { conversationId } = $props<{ conversationId: string }>() const { userId } = $props<{ userId: string }>() const { caseId } = $props<{ caseId: string | null }>() const { open } = $props<{ open: boolean }>() const { title } = $props<{ title: string }>() const { onsuggestionsreceived } = $props<{ onsuggestionsreceived: ((suggestions: string[]) }>() const { onactionsreceived } = $props<{ onactionsreceived: ((actions: any[]) }>() const { onclose } = $props<{ onclose: (() }>() const { onaction } = $props<{ onaction: ((action: any) }>() // Message type interface Message { id: string; role: "user" | "assistant"; content: string; timestamp: string | Date; isTyping?: boolean; isError?: boolean; suggestions?: string[]; actions?: { text: string; [k: string]: any }[]; contextUsed?: any; }
  ...
  // Local state let currentMessage: string = ""; let isGenerating: boolean = false; let selectedMode: string = "professional"; let showModeSelector: boolean = false; let componentError: Error | null = null; let messagesContainer: HTMLElement | null = null; let messageInput: HTMLTextAreaElement | null = null; const messages: Writable<Message[]> = writable<Message[]>([]); const aiModes = [ { id: "professional", label: "Professional", icon: "⚖️", description: "Formal legal analysis" }, { id: "investigative", label: "Investigative", icon: "🧠", description: "Deep case analysis" }, { id: "evidence", label: "Evidence Focus", icon: "📄", description: "Evidence-centered responses" }, { id: "strategic", label: "Strategic", icon: "⚡", description: "Case strategy planning" }, ]; const quickActions = [ { text: "Analyze evidence timeline", icon: "📄" }, { text: "Review witness statements", icon: "👥" }, { text: "Check legal precedents", icon: "⚖️" }, { text: "Suggest next steps", icon: "⚡" }, ]; // Effects $: if (open) { focusInput(); loadConversationHistory(); }
  ...
  function handleActionClick(action: any) { if (onaction) onaction(action); }
    ...
    <div class="chat-container" role="document"> <div class="chat-header"> <div class="header-content"> <div class="title-section"> <div class="ai-indicator"> <!-- Sparkles -> emoji --> <span aria-hidden="true" style="font-size:18px">✨</span> </div> <h2 id="chat-title">{ title }</h2> </div> <div class="mode-section"> <button class="mode-button"
          ...
          > {#if isGenerating} <div class="spinner"></div> {:else} <!-- Send -> arrow emoji --> <span aria-hidden="true">➤</span> {/if} </button> </div> </div> </div> {/if} <style> .error-boundary { background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 1.5rem; margin: 1rem; color: #dc2626; }
