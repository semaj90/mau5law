 ...
 */
import type { WebASMInferenceMetrics, VectorSearchMetrics } from '$lib/stores/gpu-summary-store.svelte.ts';
...
export interface WASMExports {
  ...
  inference?: (...args: any[]) => any;
  forward?: (...args: any[]) => any;
  predict?: (...args: any[]) => any;

  // Allow for other arbitrary exports
  [key: string]: any;
}
...
export class WebASMInferenceService {
  ...
  private initializePerformanceMonitoring(): void {
    if (typeof PerformanceObserver !== 'undefined') {
      ...
      try {
        this.performanceMonitor.observe({ entryTypes: ['measure'] as any });
      } catch {
  ...
  private initializeSIMDSupport(): boolean {
    try {
      ...
      const simdSupported = typeof WebAssembly === 'object' && ('SIMD' in (globalThis as any) || false);
    ...
    try {
      ...
      const imports: any = {
        env: {
          ...
          trace: (value: any) => console.log(`WASM trace: ${value}`)
        },
      ...
      const instance = (instantiated as any).instance as WebAssembly.Instance;
      ...
      throw new Error(`WebASM model loading failed: ${error?.message ?? String(error)}`);
    }
  }
  ...
  private async executeInference(request: InferenceRequest): Promise<InferenceResult> {
    ...
    try {
      ...
      const inferenceFunc = model.exports[model.config.expectedExportFunction || 'inference'] || model.exports.forward || model.exports.predict;
      if (typeof inferenceFunc !== 'function') {
      ...
      const inferencePromise = (async () => {
        try {
          // call with (inputPtr, inputSize, outputPtr, outputSize, batchSize) if supported
      ...
      try {
        performance.measure(`webasm-inference-${modelName}`, `webasm-inference-${modelName}-start`, `webasm-inference-${modelName}-end`);
      } catch {
      ...
      // Integrate with global metrics store
      try {
        gpuSummaryStore.updateWebASMInference(metrics);
      } catch {
      ...
      throw new Error(`Inference failed for model '${modelName}': ${error?.message ?? String(error)}`);
    }
  }
  ...
  private deallocateWasmMemory(model: WebASMModel, ptr: number): void {
    ...
    if (typeof free === 'function') {
      try { free(ptr); } catch { /* ignore free failures */ }
    }
  }
  ...
  destroy(): void {
    ...
    if (this.performanceMonitor) {
      try { this.performanceMonitor.disconnect(); } catch {}
...
export class VectorSearchInferenceEngine {
  ...
  async performSimilaritySearch(
    ...
    const startTime = performance.now();
    const similarities = await Promise.all(candidateEmbeddings.map(async (candidate, index) => {
    ...
    try { gpuSummaryStore.updateVectorSearch(metrics); } catch {}
    return results;
  }
  ...
  private tokenizeText(text: string): Float32Array {
    ...
    const tokenIds = tokens.map(t => this.getTokenId(t));
  ...
  clearCache(): void { this.embeddingCache.clear(); }
