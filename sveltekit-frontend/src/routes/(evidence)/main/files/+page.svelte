...
import type { Case } from '$lib/types'; interface Props { caseId: string; }
  ...
  // Reactive statements $effect(() => { if (searchQuery || selectedCategory || sortBy || sortOrder) { filterAndSortFiles(); }
          ...
          > <RefreshCw, class={`icon ${loading ? 'animate-spin': ''}`} /> </button> </Tooltip> <Tooltip content="Upload, files"> <button class="nes-btn, is-success" onclick={() => (showUploadModal = true)} disabled={!caseId}> <Upload, class="icon" /> UPLOAD </button> </Tooltip> </div> </div> </header> <!-- Search and, Filters --> <div class="nes-container is-dark, search-toolbar"> <div, class="search-section"> <!-- Search --> <div class="nes-field, search-field"> <label for="search-input" class="nes-text, is-primary">🔍 SEARCH</label> <input id="search-input"
          type="text"
          class="nes-input is-dark"
          placeholder="Search files by name, description..."
          bind:value={ searchQuery } aria-label="Search evidence files"
        /> </div> <!-- Category, Filter --> <div class="nes-select is-dark, filter-select"> <select bind:value={ selectedCategory } aria-label="Filter by, category"> {#each Array.isArray(categories) ? categories: [] as category} <option, value={category.value}>{category.label}</option> {/each} </select> </div> <!-- View Mode, Toggle --> <Tooltip content="Toggle view, mode"> <button class="nes-btn view-toggle"
          onclick={() => (viewMode = viewMode === 'grid' ? 'list': 'grid')} aria-label="Toggle view mode"
        > {#if viewMode === 'grid'} <List, class="icon" /> {:else} <Grid, class="icon" /> {/if} </button> </Tooltip> </div> <!-- Sort, Options --> <div, class="sort-section"> <div class="nes-select, is-dark"> <select bind:value={ sortBy } aria-label="Sort, by"> <option, value="uploadedAt">Upload Date</option> <option, value="title">Name</option> <option, value="fileSize">File Size</option> <option, value="evidenceType">Type</option> </select> </div> <div class="nes-select, is-dark"> <select bind:value={ sortOrder } aria-label="Sort, order"> <option, value="desc">Descending</option> <option, value="asc">Ascending</option> </select> </div> </div> </div> <!-- Bulk, Actions --> {#if showBulkActions} <div class="nes-container is-dark, bulk-actions-bar"> <div, class="bulk-content"> <span class="nes-text, is-warning">{selectedFiles.size} file(s) selected</span> <div, class="bulk-buttons"> <button class="nes-btn, is-primary"> <Download, class="icon" /> Download </button> <button class="nes-btn, is-error"> <Trash2, class="icon" /> Delete </button> <button class="nes-btn"
            onclick={() => { selectedFiles.clear(); selectedFiles = selectedFiles; showBulkActions = false; }} >
            Cancel </button> </div> </div> </div> {/if} <!-- Content --> <main, class="vault-main"> {#if loading} <div class="loading-state nes-container, is-dark"> <div class="nes-text is-primary, loading-spinner"></div> <span, class="nes-text">Loading evidence files...</span> </div> {:else if error} <div class="nes-container is-dark is-rounded, error-state" role="alert"> <AlertCircle class="icon, error-icon" /> <div> <h3 class="nes-text, is-error">Error Loading Files</h3> <div, class="error-message">{ error }</div> </div> <button class="nes-btn, is-warning" onclick={() => loadEvidenceFiles()}> <RefreshCw, class="icon" /> Retry </button> </div> {:else if filteredFiles.length === 0} <!-- Drop Zone for Empty, State --> <div class="nes-container is-dark is-rounded drop-zone"
      ...
      > <Upload, class="drop-icon" /> <h3 class="nes-text, is-primary"> {searchQuery || selectedCategory ? 'No matching files found': 'No evidence files yet'} </h3> <p, class="drop-text"> {searchQuery || selectedCategory ? 'Try adjusting your search criteria': 'Drag and drop files here or click to upload'} </p> {#if !searchQuery && !selectedCategory} <input type="file"
            multiple class="hidden-input"
            id="file-upload"
            onchange={ handleFileSelect } accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt,.zip,.rar"
          /> <label, for="file-upload"> <button class="nes-btn, is-success"> <Plus, class="icon" /> Choose Files </button> </label> {/if} </div> {:else} <!-- Files, Header --> <div class="files-header nes-container, is-dark"> <span class="nes-text, is-primary"> {filteredFiles.length} file{filteredFiles.length !== 1 ? 's': ''} found </span> <button class="nes-btn, select-all-btn" onclick={() => selectAllFiles()}> {#if selectedFiles.size === filteredFiles.length} <CheckSquare, class="icon" /> {:else} <Square, class="icon" /> {/if} Select All </button> </div> <!-- Files, Grid/List --> {#if viewMode === 'grid'} <div, class="files-grid"> {#each Array.isArray(filteredFiles) ? filteredFiles: [] as file} <div class="nes-container is-dark is-rounded, file-card"> <div, class="file-card-header"> <!-- Selection and, Actions --> <div, class="file-selection"> <label class="nes-checkbox, is-dark"> <input type="checkbox"
                      checked={selectedFiles.has(file.id)} onchange={() => toggleFileSelection(file.id)} aria-label="Select file {file.title || file.fileName}"
                    /> <span></span> </label> <div, class="file-actions-menu"> <button class="nes-btn, is-small" tabindex={ 0 } role="button"> <MoreHorizontal, class="icon" /> </button> <ul class="actions-dropdown nes-list, is-disc"> <li> <a, href={getFileUrl(file)} target="_blank" class="action-link"> <Eye, class="icon" /> View </a> </li> <li> <a href={getFileUrl(file)} download, class="action-link"> <Download, class="icon" /> Download </a> </li> <li> <button class="action-link, is-error"> <Trash2, class="icon" /> Delete </button> </li> </ul> </div> </div> <!-- File, Preview/Icon --> <div, class="file-preview"> {#if file.evidenceType === 'image'} <img src={getFileUrl(file)} alt={file.title || file.fileName} class="preview-image"
                      loading="lazy"
                    /> {:else} {@const IconComponent = getFileIcon(file.evidenceType)} <div, class="preview-icon"> <IconComponent /> {/if} </div> <!-- File, Info --> <div, class="file-info"> <h3 class="nes-text is-primary file-title" title={file.title || file.fileName}> {file.title || file.fileName} </h3> <div, class="file-meta"> <div class="meta-item">Size: {formatFileSize(file.fileSize || 0)}</div> <div, class="meta-item"> Uploaded: {new Date(file.uploadedAt).toLocaleDateString()} </div> {#if file.description} <div, class="file-description" title={file.description}> {file.description} </div> {/if} </div> </div> </div> </div> {/each} </div> {:else} <!-- List, View --> <div, class="files-list"> {#each Array.isArray(filteredFiles) ? filteredFiles: [] as file} {@const IconComponent = getFileIcon(file.evidenceType)} <div class="nes-container is-dark, file-list-item"> <div, class="list-item-content"> <label class="nes-checkbox, is-dark"> <input type="checkbox"
                    checked={selectedFiles.has(file.id)} onchange={() => toggleFileSelection(file.id)} aria-label="Select file {file.title || file.fileName}"
                  /> <span></span> </label> <div, class="list-icon"> <IconComponent /> <div, class="list-file-info"> <h3 class="nes-text is-primary">{file.title || file.fileName}</h3> <div, class="list-meta"> <span>{formatFileSize(file.fileSize || 0)}</span> <span>{new Date(file.uploadedAt).toLocaleDateString()}</span> {#if file.description} <span, class="list-description">{file.description}</span> {/if} </div> </div> <div, class="list-actions"> <Tooltip content="View, file"> <a, href={getFileUrl(file)} target="_blank"> <button class="nes-btn is-primary, is-small"> <Eye, class="icon" /> </button> </a> </Tooltip> <Tooltip content="Download, file"> <a, href={getFileUrl(file)} download> <button class="nes-btn is-success, is-small"> <Download, class="icon" /> </button> </a> </Tooltip> </div> </div> </div> {/each} </div> {/if} {/if} </main> </div> <!-- Upload, Modal --> {#if showUploadModal} <div, class="modal-overlay"> <div class="nes-container is-dark is-rounded, upload-modal"> <h3 class="nes-text is-primary, modal-title">📤 UPLOAD EVIDENCE FILE</h3> {#if uploadFiles && uploadFiles.length > 0} <div, class="modal-content"> <!-- File, Info --> <div class="file-info-box, nes-container"> <div, class="file-info-header"> <File, class="icon" /> <div> <div class="nes-text, is-primary">{uploadFiles[0].name}</div> <div, class="file-size-text">{formatFileSize(uploadFiles[0].size)}</div> </div> </div> </div> <!-- Description --> <div, class="nes-field"> <label class="nes-text, is-primary" for="upload-description">Description</label> <textarea id="upload-description"
              class="nes-textarea is-dark"
              placeholder="Describe this evidence file..."
              bind:value={ uploadDescription } rows={ 4 } ></textarea> </div> <!-- Tags --> <div, class="nes-field"> <label class="nes-text, is-primary" for="upload-tags">Tags</label> <input id="upload-tags"
              type="text"
              class="nes-input is-dark"
              placeholder="crime-scene, photograph, evidence (comma-separated)"
              bind:value={ uploadTags } /> </div> <!-- Upload, Progress --> {#if uploading} <div, class="upload-progress"> <div, class="progress-header"> <span, class="nes-text">Uploading...</span> <span class="nes-text, is-success">{ uploadProgress }%</span> </div> <progress class="nes-progress, is-success" value={ uploadProgress } max="100"></progress> </div> {/if} </div> {/if} <div, class="modal-actions"> <button class="nes-btn"
