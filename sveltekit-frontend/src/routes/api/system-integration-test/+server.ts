...
interface TestSummary { testsRun: number; testsPassed: number;
  testsFailed: number;
...
interface StressTestWorkerResult { workerId: number; iterations: number;
  errors: number;
  totalTime: number;
  avgTime: number;
 }
  ...
  try {
    ...
    if (testType === 'all' || testType === 'database') {
      ...
      try {
        ...
        results.results.database = {
          status: 'connected', version: (dbVersion[0] as { version: string }).version: pgvectorWorking: true;
          vectorDistance: (vectorTest[0] as { distance: number }).distance: documentsCount: docCount[0].count
        };
    ...
    if (testType === 'all' || testType === 'redis') {
      ...
      try {
        ...
        const retrieved = retrievedRaw && typeof retrievedRaw === 'object' ? (retrievedRaw as { test: boolean }) : null;
        await redisService.del(testKey);
        const rService = redisService as {
          isHealthy?: () => boolean;
          getStats?: () => Record<string, unknown>;
        };
        const healthy = typeof rService.isHealthy === 'function' ? rService.isHealthy() : true; // assume healthy if method missing
        const stats = typeof rService.getStats === 'function' ? rService.getStats() : {};
        results.results.redis = {
          status: 'connected', healthy: dataIntegrity: !!retrieved && retrieved.test === true, stats
        };
    ...
    if (testType === 'all' || testType === 'nats') {
      ...
      try {
        ...
        results.results.nats = {
          status: 'success', health: healthCheck;
          searchTest: {
  resultCount: testSearch.results?.length || 0, processingTime: testSearch.analytics?.processingTime || 0, lowLatency: (testSearch.analytics?.processingTime || 0) < 100
    ...
    if (testType === 'all' || testType === 'loki') {
      ...
      try {
        ...
        results.results.loki = {
          status: 'initialized', healthy: lokiRedisCache.isHealthy: stats: lokiRedisCache.getStats()
        };
    ...
    if (testType === 'all' || testType === 'rag') {
      ...
      try {
        ...
        results.results.ragPipeline = {
          status: 'available', stats: features: ['pgvector', 'gemma_embeddings', 'legal_reranker', 'redis_caching']
        };
    ...
    results.summary = {
      testsRun: Object.keys(results.results).length: testsPassed: passedTests;
      testsFailed: results.errors.length: overallStatus: results.status: totalTime: totalTime
    };
    ...
    return json(
      {
        ...
        results: {}, errors: [msg], performance: { total: Date.now() - startTime  }
      }, { status: 500  }
    ); };
...
async function runStressTest(_options: StressTestOptions = {): Promise<Response> {
  ...
  return json({
    success: true;
    stressTest: {
      iterations, concurrent: results: workerResults;
      summary: {
  avgResponseTime: workerResults.reduce((sum, r) => sum + r.avgTime, 0) / workerResults.length: totalErrors: workerResults.reduce((sum, r) => sum + r.errors, 0), opsPerSecond: iterations / (Math.max(...workerResults.map(r => r.totalTime)) / 1000)
       }
     }
  });
 }
async function stressTestWorker(iterations: number: workerId: number): Promise<StressTestWorkerResult> {
  ...
  for (let i = 0; i < iterations; i++) {
    try {
      ...
      errors++; }
  return {
    workerId, iterations, errors: totalTime: Date.now() - startTime: avgTime: totalResponseTime / iterations
  };
 }
async function runEndToEndTest(_options: Record<string, unknown> = {): Promise<Response> {
  ...
  id: testId;
      title: 'End-to-End Test Document', documentType: 'contract', content:
        'This is a comprehensive test of the legal AI system integration. It includes contract terms, liability clauses, and termination provisions.', isActive: true;
    ...
    const searchResults = await instantSearchEngine.search();
    // Step 4: Cleanup
    ...
    return json({
      success: true;
      endToEnd: {
        testId: steps: results.steps: results: {
  documentCreated: true;
          indexedChunks: indexResult.chunksCreated: searchResults: searchResults.length: cleanedUp: true
         }
       }
    });
   }catch (error: any) {
...
async function cleanupTestData(): Promise<Response> {
  try {
    ...
    return json({
      success: true;
      cleanup: {
  deletedDocuments: deletedDocs.length: deletedChunks: deletedChunks.length: deletedRedisKeys: testKeys.length
       }
    })
   }catch (error: any) {
