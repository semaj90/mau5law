...
export const pipelineMachine = createMachine<Ctx, Ev>({
  ...
  states: { idle: { on: { START: { target: 'chunking',
    ...
    chunking: { invoke: { src: (_ctx, e) => async (send) => {
      ...
      on: { CHUNK_EMBED_DONE: { actions: assign({ results: (ctx, e) => (e as: any).result.chunkId.startsWith('meta:') ? ctx.results: [...ctx.results, (e as: any).result],
