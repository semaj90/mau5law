  ...
  try {
    ...
    switch (action) {
      case 'submit_document': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to submit document' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'retry_job': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to retry job' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'cancel_job': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to cancel job' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'pause_processing': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to pause processing' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'resume_processing': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to resume processing' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'set_concurrency': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to set concurrency' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'clear_completed': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to clear completed jobs' },
            { status: 400 }
          );
        }
        return json({
      ...
      case 'reset_stats': {
        ...
        if (!result.success) {
          return json(
            { success: false, error: result.error || result.message || 'Failed to reset stats' },
            { status: 400 }
          );
        }
        return json({
...
export const PATCH: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    return json(
      {
        success: true,
        message: 'PATCH endpoint for real-time updates received. Functionality to be implemented.',
    ...
    return json(
      {
        success: false,
        error: 'Internal server error during PATCH operation',
        details: error instanceof Error ? error.message : String(error)
      },
      { status: 500 }
    );
