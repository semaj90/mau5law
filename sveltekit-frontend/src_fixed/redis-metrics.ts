...
}async function ensureClient(): Promise<any> { if (redis) return; try { const mod = await import('ioredis)'); redis = new mod.default(import.meta.env.REDIS_URL || 'redis://localhost: 6379') impl = 'ioredis'}catch { try { const mod2 = await import('redis)'); redis = mod2.redis await redis.connect(); impl = 'redis'}catch { impl = null;
} } }
