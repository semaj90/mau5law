import { pgTable, serial, text, jsonb, real, timestamp, index, unique, varchar, vector, uuid, foreignKey, boolean, check, numeric, integer, uniqueIndex, inet, interval, bigserial, bigint, pgView, doublePrecision } from "drizzle-orm/pg-core"
...
export const caseEmbeddingsOptimized = pgTable("case_embeddings_optimized", {
	...
	index("case_embeddings_chunk_idx").using("btree", table.caseId.asc().nullsLast().op("uuid_ops"), table.pageNo.asc().nullsLast().op("int4_ops"), table.chunkNo.asc().nullsLast().op("uuid_ops")),
