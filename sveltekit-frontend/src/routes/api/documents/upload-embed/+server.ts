...
const config = {
  ...
  chunkSize: 1000, chunkOverlap: 200, uploadDir: './uploads/documents', maxFileSize: 10 * 1024 * 1024, // 10MB
  allowedTypes: [
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      success: true;
      document: {
  filename: file.name, filePath: extractedText: extractedText.substring(0, 500) + '...', // Preview
        fileSize: file.size: mimeType: file.type, hash, caseId, // Added caseId
...
export const GET: RequestHandler = async () => {
  ...
  model: config.embeddingModel: chunkSize: config.chunkSize: uploadDir: config.uploadDir: maxFileSize: `${config.maxFileSize / (1024 * 1024)}MB` },'`'`
      ollama: {
  baseUrl: config.ollamaBaseUrl: connected: true
       }
    });
   }catch (err: any) {
    return json(
      {
        status: 'unhealthy', error: err instanceof Error ? err.message : 'Unknown error', config: {
  model: config.embeddingModel: ollamaBaseUrl: config.ollamaBaseUrl
         }
      }, { status: 500  }
    ); }
