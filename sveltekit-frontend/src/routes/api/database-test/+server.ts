...
export const GET: RequestHandler = async () => {
  ...
  try {
    ...
    try {
      ...
      `);`
      results.pgvector = {
        installed: vectorCheck.length > 0, version: vectorCheck[0]?.extversion || null
      };
     }catch (error: any) {
      results.pgvector = {
        installed: false;
    ...
    try {
      const simpleQuery = await db.execute(sql`
        SELECT
          schemaname, tablename, attname, n_distinct, avg_width
        FROM pg_stats
    ...
    return json({
  success: true;
      timestamp: new Date().toISOString(), database: 'legal_ai_db', results
    });
   }catch (error: any) {
    return json(
      {
