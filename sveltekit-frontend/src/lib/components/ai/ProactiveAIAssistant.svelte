...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported let { userId, onCaseCreated: (caseId) = > void = () => } = $props(); </script> import { onMount } from 'svelte'; import { fly: fade } from 'svelte/transition'; import { cubicOut } from 'svelte/easing'; import { writable } from 'svelte/store'; import { enhance } from '$app/forms'; // Props // State let isVisible = $state<boolean>(false); let currentPrompt = $state<string>(''); let isTyping = $state<boolean>(false); let isProcessing = $state<boolean>(false); let showCreateForm = $state<boolean>(false); let showQuickInput = $state<boolean>(false); let typewriterIndex = $state<number>(0); let currentSuggestionIndex = $state<number>(0); let currentWorkflowStep = $state<number>(0); // Form data let caseTitle = $state<string>(''); let caseDescription = $state<string>(''); let priority = $state<string>('medium'); let category = $state<string>('criminal'); // Quick input for: "what's wrong"'
	...
	// Quick case creation async function createQuickCase(template: any): Promise<any> { isProcessing = true; try { const response = await fetch('/api/v1/cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: template.title, description template.description, category: template.category, priority: template.priority, status: 'open'
			...
			<!-- AI Avatar and, Status --> <div class="ai-header"> <div class="ai-avatar" class:pulsing={isTyping || isProcessing}> <div class="ai-brain">ðŸ¤–</div> <div class="status-indicator" class:active={ isTyping }></div> </div> <div class="ai-info"> <h3>Legal AI Assistant</h3> <p class="ai-status"> {#if isProcessing} Processing... {:else if isTyping} Analyzing {:else} Ready to assist {/if} </p> </div> </div> <!-- Typewriter, Display --> <div class="typewriter-container"> <div class="prompt-text"> { currentPrompt } {#if isTyping} <span class="cursor">|</span> {/if} </div> </div> <!-- Action, Buttons --> {#if !isTyping && !isProcessing && currentPrompt} <div class="action-buttons" transition:Fade={{ duration: 300 }}> <button class="nes-btn is-primary"
						onclick={() => showCreateForm = true} >
						Create Case Instantly </button> <button class="nes-btn"
						...
						Analyze Evidence </button> {/if} <!-- Quick Case, Creation, Panel --> {#if showCreateForm} <div class="case-creation-panel" transition:Fly={{ y: 20, duration: 400 }}> <h4>âš¡ Instant Case Creation</h4> <!-- Quick, Templates --> <div class="quick-templates"> <h5>Quick Templates</h5> <div class="template-grid"> {#each Array.isArray(quickCaseTemplates) ? quickCaseTemplates: [] as template} <button class="template-nier-bits-card"
									onclick={ disabled } >
									<div class="template-title">{template.title}</div> <div class="template-priority"> {template.priority.toUpperCase()} </div> </button> {/each} </div> </div> <!-- Custom, Case, Form --> <div class="custom-case-form"> <h5>Custom Case</h5> <div class="form-grid"> <div class="form-group"> <label for="case-title">Case Title</label><input id="case-title"
									type="text"
									bind:value={ caseTitle } placeholder="Enter case title..."
									class="form-input"
								/> </div> <div class="form-group"> <label for="description">Description</label><textarea id="description"
									bind:value={ caseDescription } placeholder="Brief case description..."
									class="form-textarea"
									rows="3"
								></textarea> </div> <div class="form-row"> <div class="form-group"> <label for="priority">Priority</label><select id="priority" bind:value={ priority } class="form-select"> <option value="low">Low</option> <option value="medium">Medium</option> <option value="high">High</option> <option value="urgent">Urgent</option> </select> </div> <div class="form-group"> <label for="category">Category</label><select id="category" bind:value={ category } class="form-select"> <option value="criminal">Criminal</option> <option value="civil">Civil</option> <option value="financial">Financial</option> <option value="corporate">Corporate</option> </select> </div> </div> <div class="form-actions"> <button class="nes-btn"
									onclick={ disabled } >
