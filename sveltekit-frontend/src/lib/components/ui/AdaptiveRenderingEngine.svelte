<script lang="ts">
  ...
  function analyzePerformance(): {
    ...
    const shouldDowngrade =
      (avgFps < 50 || systemMetrics.memoryUsage > 85 || systemMetrics.cacheHitRate < 60) &&
      currentQuality.tier !== '8-BIT_NES';
    const confidence = Math.min(
      1,
      Math.max(0, (avgFps / 60 + (100 - systemMetrics.memoryUsage) / 100 + systemMetrics.cacheHitRate / 100) / 3)
    );
  ...
  async function streamTexture(assetKey: string): Promise<string> {
    ...
    const chunkSize = currentQuality.tier === '64-BIT_N64' ? 1024 : currentQuality.tier === '16-BIT_SNES' ? 512 : 256;
