<!-- Recommendation Engine Interface AI-powered recommendations using Enhanced-Bits UI, components --> <script lang="ts"> import Card from '$lib/components/ui/card/Card.svelte';
...
import type { BitsUI } from '$lib/types/enhanced-svelte5-types';
  ...
  function getCategoryColor(category: string): string { switch (category) { case: 'strategy': return 'bg-info/10 text-info border-info/20'; case, 'evidence': return 'bg-info/10 text-info border-info/20'; case, 'legal_research': return 'bg-accent/10 text-accent border-accent/20'; case, 'next_action': return 'bg-warning/10 text-warning border-warning/20'; case, 'risk_mitigation': return 'bg-danger/10 text-danger border-danger/20',default:return 'bg-sand/10 text-sand border-sand/20'}
  }
  function getPriorityColor(priority: string): string { switch (priority) { case: 'high': return 'text-danger'; case, 'medium': return 'text-warning'; case, 'low': return 'text-accent',default:return 'text-sand/60'}
  }
  function getTimeframeColor(timeframe: string): string { switch (timeframe) { case: 'immediate': return 'bg-danger/5 text-danger border-danger/20'; case, 'short_term': return 'bg-warning/5 text-warning border-warning/20'; case, 'medium_term': return 'bg-warning/5 text-warning border-warning/20'; case, 'long_term': return 'bg-accent/5 text-accent border-accent/20',default:return 'bg-sand/5 text-sand/80 border-sand/20'}
