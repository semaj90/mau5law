...
import { startupFlagService } from '$lib/services/startup-flag-service';
...
function calculateOverallHealth(summary: any): string {
  ...
  const criticalFailed = Object.values(summary.services)
    .filter((s: any) => !s.isOptional && s.status === 'failed').length;
