 ...
 * Integrates with Go service recovery and session-specific draining
 */
}
...
export interface BatchedMetrics {
  ...
  minFps: number;
  maxFps: number;
  effectsSummary: Record<string, number>;
  totalSamples: number;
}
class GPUMetricsBatcher {
  ...
  private generateSessionId(): string {
    return `gpu_session_${Date.now()}_${Math.random().toString(36).substring(2)}`;
  }
  /**
  ...
  start(): void {
    ...
    this.frameCount = 0;
    this.metrics = [];
    console.log('🚀 GPU Metrics Batcher started, session:', this.sessionId);
    ...
    // Sample GPU state immediately
    this.sampleGPUMetrics();
  }
  /**
  ...
  stop(): void {
    ...
    // Final flush
    this.flushMetrics();
    console.log('🛑 GPU Metrics Batcher stopped');
  }
  /**
  ...
  private startFPSMonitoring(): void {
    const measureFPS = (timestamp: number) => {
      ...
      this.frameCount++;
      // Sample additional GPU metrics every 30 frames (roughly every 500ms at 60fps)
  ...
  /**
   * Sample GPU and browser metrics
   */
  private sampleGPUMetrics(): void {
    const metric: GPUMetric = {
      timestamp: Date.now(),
      effectsActive: this.detectActiveEffects(),
      batchProcessing: this.detectBatchProcessing(),
      tensorCoreActive: this.detectTensorCoreActivity(),
      cacheHitRate: this.detectCacheHitRate()
    }
    // Add WebGL/WebGPU specific metrics
    if (typeof navigator !== 'undefined' && 'gpu' in navigator) {
      metric.renderingMode = 'webgpu';
    } else {
      const canvas = document.createElement('canvas');
      const gl = canvas.getContext('webgl2') || canvas.getContext('webgl');
      metric.renderingMode = gl ? 'webgl' : 'software';
    }
    // Add memory usage if available
    if ('memory' in performance) {
      const memInfo = (performance as any).memory;
      metric.memoryUsage = Math.round(memInfo.usedJSHeapSize / 1024 / 1024); // MB
    }
    this.addMetric(metric);
  }
  /**
   * Detect active visual effects from DOM
   */
  private detectActiveEffects(): string[] {
    ...
    const effectSelectors = [
      ...
      '.tensor-processing',
      '.ai-visualization',
      // PS1 retro effects (your new implementation)
      '.ps1-surface',
      '.ps1-dither-pattern',
      '.ps1-texture-warp',
      '.ps1-vertex-jitter',
    ...
    const inlineEffects = this.detectInlineEffects();
    effects.push(...inlineEffects);
    return effects;
  }
  /**
   * Detect effects applied via inline styles or data attributes
   */
  private detectInlineEffects(): string[] {
    const effects: string[] = [];
    // Check for data-depth attributes (parallax)
    const parallaxElements = document.querySelectorAll('[data-depth]');
    if (parallaxElements.length > 0) {
      effects.push('data-depth-parallax');
    }
    // Check for transform3d styles (retro transforms)
    const transformElements = document.querySelectorAll('[style*="transform3d"], [style*="perspective"]');
    if (transformElements.length > 0) {
      effects.push('inline-transform3d');
    }
    // Check for WebGPU canvas elements
    const webgpuCanvas = document.querySelectorAll('canvas[data-webgpu="true"]');
    if (webgpuCanvas.length > 0) {
      effects.push('webgpu-canvas-active');
    }
    // Check for active retro transform instances
    if (typeof window !== 'undefined' && (window as any).retroTransformInstances) {
      const instances = (window as any).retroTransformInstances;
      if (instances && instances.length > 0) {
        effects.push('retro-transform-instances');
      }
    }
    return effects;
  }
  /**
   * Detect if batch processing is active
   */
  private detectBatchProcessing(): boolean {
    // Check for indicators of batch processing
    return document.querySelector('.batch-processing, .qlora-training, .gpu-batch-active') !== null;
  }
  /**
   * Detect Tensor Core activity
   */
  private detectTensorCoreActivity(): boolean {
    // Check for Tensor Core indicators
    return document.querySelector('.tensor-core-active, .rtx-processing, .gpu-ml-active') !== null;
  }
  /**
   * Detect cache hit rate (simulated for now)
   */
  private detectCacheHitRate(): number {
    // In a real implementation, this would query the GPU cache
    // For now, simulate based on performance indicators
    const hasCache = document.querySelector('.gpu-cache-active') !== null;
    return hasCache ? 0.85 + Math.random() * 0.1 : 0.5 + Math.random() * 0.3;
  }
  /**
  ...
  private detectRenderingMode(): 'webgl' | 'webgpu' | 'software' {
    ...
    const canvas = document.createElement('canvas');
    const gl = canvas.getContext('webgl2') || canvas.getContext('webgl');
    return gl ? 'webgl' : 'software';
  }
  /**
   * Add metric to batch
   */
  private addMetric(metric: GPUMetric): void {
  ...
  /**
   * Setup visibility change handling for smart flushing
   */
  private setupVisibilityHandling(): void {
    if (typeof document === 'undefined') return;
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Page is being hidden, flush metrics after a delay
        setTimeout(() => {
          if (document.hidden) {
            this.flushMetrics();
          }
        }, this.visibilityFlushTimeout);
      } else {
        // Page is visible again, ensure metrics are running
        if (!this.isActive) {
          this.start();
        }
      }
    ...
    // Also handle beforeunload for final flush
    window.addEventListener('beforeunload', () => {
      this.flushMetrics();
    });
  }
  /**
   * Check if server is healthy before sending metrics
   */
  private async checkServerHealth(): Promise<boolean> {
    const now = Date.now();
    // Implement exponential backoff for health checks after failures
    const backoffInterval = 30000 * this.backoffMultiplier;
    if (now - this.lastHealthCheck < backoffInterval && !this.serverHealthy && this.consecutiveFailures > 0) {
  ...
  private async flushMetrics(): Promise<void> {
    ...
    if (!isHealthy) {
      // Graceful degradation - store metrics locally when server is down
      this.handleOfflineMetrics();
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (this.consecutiveFailures > 0 || Date.now() - this.lastHealthCheck > 60000) {
          console.log('✅ GPU metrics batch sent successfully');
        }
        this.metrics = []; // Clear metrics after successful send
        this.consecutiveFailures = 0;
        this.backoffMultiplier = 1;
        this.serverHealthy = true;
        // Reset flush interval to normal on recovery
      ...
      if (this.consecutiveFailures < 3) {
        if (error.name === 'AbortError') {
          ...
          console.warn('🌐 Network error sending GPU metrics - server may be down');
        } else {
          console.error('❌ GPU metrics batch send error:', error.message || error);
        }
      }
      this.handleServerError();
    }
  ...
  private handleOfflineMetrics(): void {
    ...
    if (this.metrics.length > maxOfflineMetrics) {
      ...
      this.metrics = [...older, ...recent];
      console.log('📊 Pruned offline metrics cache to', this.metrics.length, 'samples');
    }
  ...
  private adjustFlushInterval(): void {
    ...
    const adjustedInterval = Math.min(baseInterval * Math.pow(2, Math.min(this.consecutiveFailures, 4)), 120000); // Max 2 minutes
    this.flushTimer = setInterval(() => {
      this.flushMetrics();
    }, adjustedInterval);
  ...
  private loadMetricsFromLocalStorage(): void {
    try {
      ...
      console.warn('Could not load offline metrics:', error);
    }
  }
  /**
...
// Global instance
export const gpuMetricsBatcher = new GPUMetricsBatcher();
/**
 * Initialize GPU metrics batcher (call from layout)
 */
export function initGpuMetricsBatcher(): void {
  if (typeof window === 'undefined') return;
  console.log('🎯 Initializing GPU Metrics Batcher...');
  gpuMetricsBatcher.start();
  // Check for Go service recovery
  checkGoServiceRecovery();
}
/**
 * Check if Go service has recovered and drain if needed
 */
async function checkGoServiceRecovery(): Promise<void> {
  try {
    ...
    if (healthResponse.ok) {
      ...
      console.warn('⚠️ Go service health check failed, metrics will queue');
    }
  } catch (error) {
    console.warn('⚠️ Go service not available, metrics will queue locally:', error);
  }
}
/**
 * Cleanup GPU metrics batcher
 */
export function cleanupGpuMetricsBatcher(): void {
  gpuMetricsBatcher.stop();
}