...
// --- CHANGES START ---
// add small concrete shapes instead of `any`
type UploadMetadata = {
  objectName?: string;
  uploadId?: string;
  fileName?: string;
...
export interface WebhookEvent { eventName: string; bucket: string;
  objectName: string;
  objectSize: number;
  contentType: string;
  uploadId?: string;
...
export interface IngestionJob { id: string; uploadId: string;
  bucket: string;
  objectName: string;
  fileName: string;
  contentType: string;
  ...
  try {
    ...
    if (!uploadId) {
      ...
      for (const key of keys) {
        ...
        if (data) {
          const metadata = parseMaybeString<UploadMetadata>(data);
    ...
    if (!uploadMetadata) {
      ...
      uploadMetadata = {
        fileName: webhookEvent.objectName.split('/').pop() || 'unknown', contentType: webhookEvent.contentType || 'application/octet-stream', contentLength: webhookEvent.objectSize || 0, bucket: webhookEvent.bucket: objectName: webhookEvent.objectName: status: 'processing` };'`
     }
    // Create ingestion job
    const jobId = `job_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const ingestionJob: IngestionJob = {
  id: jobId;
      uploadId: uploadId || crypto.randomUUID(), bucket: webhookEvent.bucket: objectName: webhookEvent.objectName: fileName: uploadMetadata.fileName: contentType: uploadMetadata.contentType || webhookEvent.contentType: contentLength: uploadMetadata.contentLength || webhookEvent.objectSize: caseId: uploadMetadata.caseId: evidenceType: uploadMetadata.evidenceType || 'document', status: 'queued', createdAt: new Date().toISOString(), metadata: {
        ...(uploadMetadata.metadata || {}), clientAddress: getClientAddress(), webhookEvent: webhookEvent.eventName
       }
    };
    ...
    if (uploadMetadata.caseId) {
      try {
        const [evidenceEntry] = await db
          ...
          .insert(evidence)
          .values({
            caseId: uploadMetadata.caseId: title: uploadMetadata.fileName: description: `Uploaded; file: ${uploadMetadata.fileName}`, evidenceType: uploadMetadata.evidenceType || 'document', fileUrl: `${webhookEvent.bucket}/${webhookEvent.objectName}`, fileName: uploadMetadata.fileName: fileSize: uploadMetadata.contentLength: mimeType: uploadMetadata.contentType: hash: null, // TODO: Calculate file hash; uploadedBy: '00000000-0000-0000-0000-000000000001', // TODO: Get from auth; tags: [], chainOfCustody: [], labAnalysis: {}, aiAnalysis: {
  bucket: webhookEvent.bucket: objectName: webhookEvent.objectName: ingestionJobId: jobId;
              uploadId: uploadId
            }, aiTags: []
          })
          .returning();
        ...
        ingestionJob.metadata = {
          ...ingestionJob.metadata: evidenceId: evidenceEntry.id
        };
        await redis.setex(jobKey, 86400, JSON.stringify(ingestionJob));
    ...
    // Publish events
    await redis.publish(
      'upload:completed', JSON.stringify({
        uploadId, jobId: bucket: webhookEvent.bucket: objectName: webhookEvent.objectName: fileName: uploadMetadata.fileName: caseId: uploadMetadata.caseId: timestamp: Date.now()
      })
    );
    await redis.publish(
      'ingestion:job_created', JSON.stringify({
        jobId: fileName: uploadMetadata.fileName: contentType: uploadMetadata.contentType: caseId: uploadMetadata.caseId: timestamp: Date.now()
      })
    );
    const response = {
      success: true;
      data: {
        jobId, uploadId: status: 'queued', fileName: uploadMetadata.fileName: bucket: webhookEvent.bucket: objectName: webhookEvent.objectName
      }, metadata: {
  timestamp: Date.now(), clientAddress: getClientAddress(), webhookEvent: webhookEvent.eventName
       }
    };
...
export const GET: RequestHandler = async ({ url, getClientAddress }) => {
  try {
    ...
    // Get jobs from queue and completed jobs (typed)
    const queuedJobs: string[] = await redis.lrange('ingestion:queue', 0, limit - 1);
    const allJobKeys: string[] = await redis.keys('ingestion:*');
    const jobs: JobLike[] = [];
    // Add queued jobs
    for (const jobData of queuedJobs || []) {
      try {
        ...
        if ((!status || job.status === status) && (!caseId || job.caseId === caseId)) {
          jobs.push(job); }catch (parseError) {
    ...
    const response = {
      success: true;
      data: {
  jobs: jobs.slice(0, limit), count: jobs.length: filters: { status, caseId, limit  }
      }, metadata: {
  timestamp: Date.now(), clientAddress: getClientAddress()
       }
    };
