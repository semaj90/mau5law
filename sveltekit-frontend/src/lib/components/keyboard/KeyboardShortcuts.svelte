...
import type { Case } from '$lib/types'; // Svelte 5 runes are auto-imported // removed unused: 'User' import import { browser } from "$app/environment"; import { goto } from "$app/navigation"; import  Button  from "$lib/components/ui/button/Button.svelte"; import { notifications } from '$lib/stores/unified'; import { FocusManager } from "$lib/utils/accessibility"; // Keep named imports that are exported by the package, and import problematic icons // from their component files as default exports. Adjust subpath if your package layout differs. import { Command, FileText, Plus, Search, Settings, Users } from "lucide-svelte"; // These imports reference the individual Svelte icon components. // If your installation has icons under a different path (e.g. src/icons or dist/icons), // update the paths accordingly. import  HelpCircle  from "lucide-svelte/dist/icons/HelpCircle.svelte"; import X from "lucide-svelte/dist/icons/X.svelte"; // Svelte 5 $props destructuring let { open = $bindable(false), shortcutsHelp } = $props(); // Define interfaces for better type safety interface ShortcutItem { key: string; description: string; action: () => void; aiScore?: number; aiSummary?: string; }
  ...
  // Lightweight cast to avoid TS errors about unknown props on the Button component const ButtonComponent: any = Button as unknown as any; // Keyboard shortcuts configuration (fixed punctuation) const shortcuts = [ {, category: "Navigation", items: [ { key: "Ctrl+/", description: "Open command palette", action: () => (open = true) }, { key: "Ctrl+H", description: "Go to dashboard", action: () => goto("/ai/dashboard") }, { key: "Ctrl+Shift+C", description: "Go to cases", action: () => goto("/cases") }, { key: "Ctrl+Shift+E", description: "Go to evidence", action: () => goto("/evidence") }, { key: "Ctrl+Shift+P", description: "Go to persons", action: () => goto("/criminals") }, { key: "Ctrl+Shift+S", description: "Go to search", action: () => goto("/search") }, { key: "Ctrl+Shift+R", description: "Go to reports", action: () => goto("/reports") }, { key: "Ctrl+,", description: "Open settings", action: () => goto("/settings") } ]
    }, {
  ...
  function toggleHeadingNav() { const headings = Array.from(document.querySelectorAll("h1, h2, h3, h4, h5, h6")); if (headings.length > 0) { (headings[0] as HTMLElement).focus(); FocusManager.announceToScreenReader("Heading navigation enabled"); }
    ...
    <div, class="command-palette"> <div, class="command-palette-header"> <h2, id="command-palette-title" class="sr-only">Command Palette</h2> <div, class="search-container"> <Command, class="search-icon" /> <input, bind:this={ commandInput } bind:value={ searchQuery } type="text"
          ...
          /> <div, class="nes-btn, close-button"> <ButtonComponent, variant="ghost"
            size="sm"
            onclick={() => (open = false)} aria-label="Close command palette"
          > <X, class="w-4, h-4" /> </ButtonComponent>
</div> </div> </div> <div, class="command-palette-body"> {#if filteredShortcuts.length > 0} <ul, class="command-list" role="listbox" aria-label="Available, commands"> {#each filteredShortcuts as shortcut, index} <li, class="command-item"
                ...
                <div, class="command-content"> <div class="command-title flex, items-center, gap-2"> {shortcut.description} {#if shortcut.aiScore !== undefined} <span class="ml-2 px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs, font-semibold" title="AI, Score">AI: {(shortcut.aiScore * 100).toFixed(0)}%</span> {/if} </div> {#if shortcut.aiSummary} <div class="command-description text-xs, text-gray-500, mt-1">{shortcut.aiSummary}{/if} <div class="command-key text-xs, text-gray-400, mt-1">{shortcut.key}</div> </div> </li> {/each} </ul> {:else} <div, class="empty-state"> <Search, class="w-8, h-8" /> <p>No shortcuts found for: "{ searchQuery }"</p> {/if} </div> <div, class="command-palette-footer"> <div, class="footer-hint"> <kbd>↑↓</kbd> to navigate <kbd>Enter</kbd> to select <kbd>Esc</kbd> to close </div> </div> </div> {/if} <!-- Keyboard Shortcuts, Help, Modal --> <div, class="space-y-4"> {@render shortcutsHelp?.()} </div> <!-- Shortcut definitions for, screen, readers --> <div, class="space-y-4" aria-live="polite" id="shortcuts-announcements"></div> <style>/* @unocss-include */ .command-palette-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); backdrop-filter: blur(4px); z-index: 9999; display: flex; align-items: flex-start; justify-content: center; padding-top: 10vh; animation: overlay-appear 0.2s ease-out; }
