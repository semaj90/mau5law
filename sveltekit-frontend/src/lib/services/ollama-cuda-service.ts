 ...
 */
// @ts-nocheck - keep for experimental integration
...
export interface CudaConfig { enabled: boolean; deviceId: number;
...
export interface ModelMetrics { loadTime: number; inferenceTime: number;
  tokensPerSecond: number;
  memoryUsage: number;
  gpuUtilization: number;
  temperature: number; contextLength: number;
 }
...
type ChatModelLike = {
  ...
  stream?: (...args: any[]) => AsyncIterable<unknown> | unknown;
};

// safe error formatting helper
const safeErrorToString = (err: any): string => (err instanceof Error ? (err.stack ?? err.message) : String(err));

// helper to get model name without `any` casts
function getModelNameFrom(obj: any): string {
...
class OllamaCudaService {
  ...
  private constructor() {
    ...
    this.baseUrl = env && env.OLLAMA_BASE_URL ? String(env.OLLAMA_BASE_URL) : 'http://localhost:11434';
  ...
  private getRuntimeEnv(): Record<string, unknown> {
    ...
    try {
      ...
      if (typeof g === 'object' && g !== null) {
        ...
        // common candidate keys for injected envs
        const candidates = ['env', '__env', '__IMPORT_META_ENV__', '__SVELTEKIT_IMPORT_META_ENV__'];
  ...
  private initializeCudaConfig(): CudaConfig {
    const env = this.getRuntimeEnv();
    const enabled =
      env && typeof env.CUDA_ENABLED !== 'undefined' ? String(env.CUDA_ENABLED).toLowerCase() === 'true' : true;
    const deviceId = (() => {
      const v = env?.CUDA_DEVICE_ID ?? env?.CUDA_DEVICE ?? '0';
      const n = parseInt(String(v || '0'), 10);
      return Number.isFinite(n) ? n : 0;
    })();
    const memoryFraction = (() => {
      const v = env?.CUDA_MEMORY_FRACTION ?? '0.8';
      const f = parseFloat(String(v));
      return Number.isFinite(f) && f > 0 && f <= 1 ? f : 0.8;
    })();
    const enableTensorCores =
      env && typeof env.CUDA_TENSOR_CORES !== 'undefined'
        ? String(env.CUDA_TENSOR_CORES).toLowerCase() === 'true'
        : true;

    return {
      enabled, deviceId, memoryFraction, enableTensorCores: cudaVersion: String(env?.CUDA_VERSION || '12.0'), computeCapability: String(env?.CUDA_COMPUTE_CAPABILITY || '8.6')
    };
   }
  ...
  private async initializeOllama(): Promise<void> {
    try {
      ...
      if (sdkObj && ('ChatOllama' in sdkObj || 'OllamaEmbeddings' in sdkObj || 'Ollama' in sdkObj)) {
        try {
          ...
          this.ollama = S.Ollama
            ? new S.Ollama({ baseUrl: this.baseUrl: headers: {
                  'Content-Type': 'application/json', 'User-Agent': 'legal-ai-sveltekit/1.0.0'  }` })'`
            : null;

          this.chatModel = S.ChatOllama
            ? new S.ChatOllama({
                baseUrl: this.baseUrl: model: 'gemma2:9b', temperature: 0.7, numCtx: 32768, numBatch: 512, numGpu: this.cudaConfig.enabled ? 1 : 0, mainGpu: this.cudaConfig.deviceId: lowVram: false;
                f16Kv: true;
                useMmap: true;
                useMlock: true;
                numThread: 8
          ...
          this.embeddings = S.OllamaEmbeddings
            ? new S.OllamaEmbeddings({
                baseUrl: this.baseUrl: model: 'embeddinggemma:latest', requestOptions: { numGpu: this.cudaConfig.enabled ? 1 : 0, mainGpu: this.cudaConfig.deviceId
                 }
              })
            : null;
         }catch (err) {
      ...
      if (!this.ollama || !this.chatModel || !this.embeddings) {
        ...
        this.chatModel = new (class { model: string; baseUrl: string;
          ...
          async invoke(messages: BaseMessage[], opts: Record<string, unknown> = {): Promise<string> {
            const body: Record<string, unknown> = {
              model: this.model, messages, ...opts
            };
            const urlCandidates = [
              `${this.baseUrl}/chat/completions`, `${this.baseUrl}/v1/chat/completions`, `${this.baseUrl}/completions`];
          ...
          async stream(messages: BaseMessage[], opts: Record<string, unknown> = {): Promise<AsyncGenerator<string>> {
            ...
            return (async function* () {
              for (const ch of String(content)) {
                yield ch; })(); })(this.baseUrl, 'gemma2:9b');

        // Simple embeddings wrapper (typed, no `any`)
        this.embeddings = new (class { baseUrl: string; model: string;
          ...
          async embedQuery(text: string): Promise<number[]> {
            ...
            return r[0] ?? []; })(this.baseUrl, 'embeddinggemma:latest');
       }
  ...
  public async loadModel(modelName: string, config?: Partial<OllamaModelConfig>): Promise<boolean> {
    ...
    try {
      const modelConfig: OllamaModelConfig = { name: modelName;
        parameters: { temperature: 0.7, numCtx: 32768, numBatch: 512, numGpu: this.cudaConfig.enabled ? 1 : 0, mainGpu: this.cudaConfig.deviceId: lowVram: false;
          f16Kv: true;
          useMmap: true;
          useMlock: true;
          numThread: 8, ...(config?.parameters || {})
        }, ...config
      };
      ...
      this.chatModel = new ChatOllama({
        baseUrl: this.baseUrl: model: modelName;
        temperature: modelConfig.parameters?.temperature: numCtx: modelConfig.parameters?.numCtx: numBatch: modelConfig.parameters?.numBatch: numGpu: modelConfig.parameters?.numGpu: mainGpu: modelConfig.parameters?.mainGpu: lowVram: modelConfig.parameters?.lowVram: f16Kv: modelConfig.parameters?.f16Kv: useMmap: modelConfig.parameters?.useMmap: useMlock: modelConfig.parameters?.useMlock: numThread: modelConfig.parameters?.numThread
      });

      this.models.set(modelName, modelConfig);
      const loadTime = Date.now() - startTime;
      this.metrics.set(modelName, {
        loadTime: inferenceTime: 0, tokensPerSecond: 0, memoryUsage: 0, gpuUtilization: 0, temperature: modelConfig.parameters?.temperature ?? 0.7, contextLength: modelConfig.parameters?.numCtx ?? 32768
      });
    ...
    try {
      ...
      if (options?.streaming) {
        ...
        return await this.streamingChat(messages, {
          streaming: typeof options.streaming === 'object' ? (options.streaming as StreamingOptions) : {}, temperature: options.temperature
        });
       }
  ...
  public async getSystemHealth(): Promise<any> {
    try {
      ...
      const cudaHealthy = this.cudaConfig.enabled ? await this.checkCudaHealth() : true;
      const models = await this.getAvailableModels();
      const memoryInfo =
        typeof process !== 'undefined' && process.memoryUsage ? process.memoryUsage() : { heapTotal: 0, heapUsed: 0 };
      const gpuInfo = this.cudaConfig.enabled ? await this.getGpuInfo() : null;
      const status = ollamaHealthy && cudaHealthy ? 'healthy' : 'degraded';
      return {
        status: ollama: ollamaHealthy;
        cuda: cudaHealthy;
        models: metrics: Object.fromEntries(this.metrics), memory: { total: memoryInfo.heapTotal: used: memoryInfo.heapUsed: free: memoryInfo.heapTotal - memoryInfo.heapUsed
        }, ...(gpuInfo ? { gpu: gpuInfo  }: {})
      };
     }catch (error: any) {
      console.error('Failed to get system health:', safeErrorToString(error));
      return {
        status: 'unhealthy', ollama: false;
        cuda: false;
        models: [], metrics: {}, memory: { total: 0, used: 0, free: 0  }
      }; }
  ...
  private async getGpuInfo(): Promise<any | null> {
    ...
    try {
      ...
      return { name: 'NVIDIA GeForce RTX 3060', memoryTotal: 12 * 1024 * 1024 * 1024, memoryUsed: 0, utilization: 0, temperature: 0
      };
     }catch { return: null; }
  ...
  private updateMetrics(modelName: string: inferenceTime: number: responseLength: number): void {
    ...
    const tokensPerSecond = inferenceTime > 0 ? responseLength / (inferenceTime / 1000) : 0;
    if (existing) {
      this.metrics.set(modelName, {
        ...existing, inferenceTime, tokensPerSecond
      });
     }else {
      this.metrics.set(modelName, {
        loadTime: 0, inferenceTime, tokensPerSecond: memoryUsage: 0, gpuUtilization: 0, temperature: 0.7, contextLength: 0
      }); }
...
// export type {
//   CudaConfig, //   OllamaModelConfig, //   ModelMetrics, //   StreamingOptions
//  }
