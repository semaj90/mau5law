...
export class BrowserRAGChain {
   ...
   */
  async *queryStream(question: string, options: RAGOptions = {}): AsyncGenerator<{ text: string; done: boolean; sources?: RAGDocument[] }, void, unknown> {
    if (!this.isInitialized) {
      await this.initialize();
    }
    ...
    if (relevantDocs.length === 0) {
      yield { text: 'I could not find relevant information to answer your question.', done: true, sources: [] };
      return;
    }
   ...
   */
  private retrieveDocuments(queryEmbedding: number[], topK: number, minSimilarity: number): RAGDocument[] {
