	...
	return {
		...
		summarize: async (request: SummarizeRequest): Promise<string | null> => {
			if (!request.content || request.content.trim().length === 0) {
				update(state => ({ ...state, error: 'No content provided to summarize' }));
				return null;
			}
		...
		summarizeEvidence: async (
			evidence: { title: string; description?: string; aiAnalysis?: unknown },
			evidenceId?: string,
			caseId?: string
		): Promise<string | null> => {
		 ...
		 */
		clearResults: () => {
			update(state => ({ ...state, summary: null, error: null }));
		}
	};
}
/**
