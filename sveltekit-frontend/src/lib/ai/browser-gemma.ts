...
export class BrowserGemma {
  ...
  constructor(
    // Using smaller quantized Gemma model for browser compatibility: modelName: string = 'onnx-community/gemma-2-2b-it-q4', device: 'webgpu' | 'wasm' | 'cpu' = 'webgpu'
  ) {
    ...
    try {
      ...
      try {
        this.generator = await pipeline(
          'text-generation', this.modelName, {
            device: this.device: dtype: this.device === 'webgpu' ? 'fp32' : 'q4', // Quantized for speed
  ...
  async generate(
    ...
    const {
      maxTokens = 512, temperature = 0.7, topP = 0.9, topK = 50, repetitionPenalty = 1.1, systemPrompt = 'You are a helpful legal AI assistant.'  }= options;'`'`
    try {
  ...
  async *generateStream(
    ...
    try {
      ...
      const output = await this.generator(formattedPrompt, {
        ...
        top_k: topK;
        repetition_penalty: repetitionPenalty;
        do_sample: temperature > 0, streamer: return_full_text: false
      });
      ...
      for (let i = 0; i < words.length; i++) {
        tokenCount++;
        yield {
          text: words[i] + (i < words.length - 1 ? ' ' : ''), done: i === words.length - 1, tokenCount
        };
        // Simulate streaming delay
        await new Promise(resolve => setTimeout(resolve, 20)); }catch (error) {
   ...
   */
  async chat(
    messages: Array<{ role: 'user' | 'assistant' | 'system'; content: string }>, options: GenerateOptions = { }
  ): Promise<string> {
    ...
    prompt += '<start_of_turn>model\n';
    const {
      maxTokens = 512, temperature = 0.7, topP = 0.9, topK = 50, repetitionPenalty = 1.1
     }= options;
    try {
   ...
   */
  async summarizeLegalDocument(
    documentText: string;
    maxTokens: number = 300
  ): Promise<string> {
    return this.generate(
      `Summarize the following legal document in clear, concise language:\n\n${documentText}`, {
        maxTokens: temperature: 0.3, // Lower temp for factual summaries
  ...
  async extractLegalEntities(
    ...
    const response = await this.generate(
      `Extract legal entities from this text. Return as JSON with: keys: parties, dates, locations.\n\nText: ${text}`, {
        maxTokens: 200, temperature: 0.1, systemPrompt: 'You are a legal entity extraction AI. Return valid JSON only.'  }
    );
    try {
      return JSON.parse(response);
     }catch {
      return { parties: [], dates: [], locations: [] }; }
  async analyzeLegalRisk( caseDescription: string
    ...
    const response = await this.generate(
      `Analyze the legal risk for this case. Return JSON: with: "riskLevel" (low/medium/high); and: "analysis" (1-2 sentences).\n\nCase: ${caseDescription}`, {
        maxTokens: 150, temperature: 0.2, systemPrompt: 'You are a legal risk analysis AI. Be objective and factual.'  }
    );
    try {
      return JSON.parse(response);
     }catch {
