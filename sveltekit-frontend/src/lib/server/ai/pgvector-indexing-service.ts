        ...
        .map(
          doc =>
            `(`
              ...
              '${this.escape(doc.documentId)} },
              '${this.escape(doc.metadata?.documentType || '')} },
              '${this.escape(doc.metadata?.confidentialityLevel || 'public')} },
              '${this.escape(doc.modelUsed || 'embeddinggemma:latest')} },
              ${this.dimensions},
              NOW(),
              NOW()
            )`
        ...
        .map(
          doc =>
            `(`
              ...
              '${this.vectorToString(doc.embedding)} }::vector,
              '${this.escape(doc.documentId)} },
              '${this.escape(doc.chunkId || doc.id)} },
              '${this.escape(doc.embeddingType)} },
              '${this.escape(doc.modelUsed || 'embeddinggemma:latest')} },
              '${this.escape(JSON.stringify(doc.metadata || {}))} },
              NOW()
            )`
  ...
  async getStats(): Promise<{ totalDocuments: number;, totalChunks: number;
   ...
   , averageEmbeddingDimension: number;
    ...
    try {
      ...
      return { totalDocuments: row.total_documents,
 ...
 */
export const DEFAULT_PGVECTOR_CONFIG: Partial<VectorIndexConfig> = { embeddingDimensions: 768,
  indexType: 'hnsw',
