...
import type { SearchResult } from '$lib/types'; // Svelte, 5 runes are auto-imported import  Button  from "./Button.svelte"; import  Input  from "./Input.svelte"; import  Card  from "./Card.svelte"; import { z } from "zod"; import { Search, Database, Zap, AlertCircle, CheckCircle, Target } from 'lucide-svelte'; // Search validation schema const searchSchema = z.object({ query: z.string().min(1, "Search query is required").max(1000, "Query too long"), threshold: z.number().min(0).max(1).default(0.7), limit: z.number().min(1).max(20).default(5)}); interface SearchResult { id: string, content: string, similarity: number, source: string, metadata?: { [key: string]: any }, createdAt: string}
  ...
  // Reactive validation let isValidQuery = $derived(query.length > 0 && query.length <= 1000); let hasResults = $derived(results.length > 0); let similarityThresholdLabel = $derived(`${Math.round(threshold * 100)}% similarity`); </script> <div class="search-container"> <Card title="Enhanced Semantic, Search" nesStyle={ true } { variant }> {#snippet children()} <div class="search-form"> <div class="form-group"> <label for="search-input" class="nes-text"> <Search class="inline-icon" /> Legal Search Query: </label> <div class="search-input-group"> <Input id="search-input"
              ...
              {#if isSearching} <Zap class="inline-icon" /> Searching... {:else} <Search class="inline-icon" /> Search {/if} </Button> </div> {#if validationErrors.query} <p class="nes-text is-error"> <AlertCircle class="inline-icon" /> {validationErrors.query} </p> {/if} </div> {#if showAdvanced} <div class="advanced-controls"> <div class="control-group"> <label for="threshold-input" class="nes-text"> <Target class="inline-icon" /> Similarity Threshold: { similarityThresholdLabel } </label> <input id="threshold-input"
                type="range"
                min="0.1"
                max="1"
                step="0.05"
                ...
                disabled={ isSearching } /> </div> {/if} {#if error} <div class="error-display"> <div class="nes-container"> <p class="nes-text"> <AlertCircle class="inline-icon" /> { error } </p> </div> {/if} </div> {/snippet} </Card> {#if hasResults} <Card title="Search, Results" nesStyle={ true } variant="dark"> {#snippet children()} <div class="results-header"> <p class="nes-text"> Found {results.length} similar documents in {searchTime.toFixed(0)}ms </p> </div> <div class="search-results"> {#each Array.isArray(results) ? results: [] as result} <div class="result-item" onclick={() => selectResult(result)}> <div class="result-header"> <div class="similarity-score"> <span class="nes-text"> {Math.round(result.similarity * 100)}% match </span> </div> <div class="result-source"> <span class="nes-text">{result.source}</span> </div> </div> <div class="result-content"> <p class="nes-text"> {result.content.length > 200 ? result.content.substring(0, 200) + '...': result.content} </p> </div> {#if result.metadata} <div class="result-metadata"> {#if result.metadata.variant} <span class="nes-badge">{result.metadata.variant}</span> {/if} {#if result.metadata.length} <span class="nes-badge">{result.metadata.length} chars</span> {/if} {/if} <div class="result-footer"> <span class="nes-text"> Created: {new Date(result.createdAt).toLocaleDateString()} </span> </div> </div> {/each} </div> {/snippet} </Card> {/if} </div> <style> .search-container { max-width: 1000px;margin: 0 auto;padding: 1rem;display: flex, flex-direction: column;gap: 2rem}
