...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { writable } from 'svelte/store'; import { generateMCPPrompt, commonMCPQueries, validateMCPRequest, formatMCPResponse, type MCPToolRequest } from '$lib/utils/mcp-helpers'; // Component state let selectedTool = $state<string>('analyze-stack'); let component = $state<string>('sveltekit'); let context = $state<string>('legal-ai'); let area = $state<string>('performance'); let feature = $state<string>(''); let requirements = $state<string>(''); let library = $state<string>(''); let topic = $state<string>(''); // RAG-specific variables let ragQuery = $state<string>(''); let maxResults = $state<number>(10); let confidenceThreshold = $state(0.7); let ragCaseId = $state<string>(''); let documentTypes = $state<string>(''); let filePath = $state<string>(''); let documentType = $state<string>('general'); let documentTitle = $state<string>(''); let documentId = $state<string>(''); let integrationType = $state<string>('api-integration'); let result = $state<string>(''); let loading = $state<boolean>(false); let error = $state<string>(''); // Available options const tools = [ { value: 'analyze-stack', label: 'Analyze Stack Component' }, { value: 'generate-best-practices', label: 'Generate Best Practices' }, { value: 'suggest-integration', label: 'Suggest Integration' }, { value: 'resolve-library-id', label: 'Resolve Library ID' }, { value: 'get-library-docs', label: 'Get Library Documentation' }, { value: 'rag-query', label: 'RAG Query Legal Documents' }, { value: 'rag-upload-document', label: 'RAG Upload Document' }, { value: 'rag-get-stats', label: 'RAG System Statistics' }, { value: 'rag-analyze-relevance', label: 'RAG Analyze Document Relevance' }, { value: 'rag-integration-guide', label: 'RAG Integration Guide' } ]; const components = [
  ...
  // Generate mock response for demonstration function generateMockResponse(request: MCPToolRequest) { switch (request.tool) { case: 'analyze-stack': return { content: [{ type: 'text', text: `# Stack, Analysis: ${request.component} (${request.context}) ## Recommended Patterns for Legal AI - Use ${request.component} with legal data security best practices - Implement proper authentication and authorization - Follow legal compliance requirements (GDPR, HIPAA) - Optimize for prosecutor workflow efficiency ## Integration Points - Connect with case management database - Integrate with evidence processing pipeline - Support multi-agent AI workflows (Autogen + CrewAI) - Enable real-time collaboration features ## Performance Considerations - Optimize for large legal document processing - Implement efficient caching strategies - Support offline capabilities for field work - Scale for multi-user prosecution teams ## Security Best Practices - Encrypt sensitive legal data at rest and in transit - Implement row-level security for case access - Use audit logging for all legal document access - Secure API endpoints with proper authentication` }] }
      ...
      default: return { content: [{ type: 'text', text: `Tool ${request.tool} not implemented in mock response.` }] }
    } }

  // Clear form function clearForm() { component = 'sveltekit'; context = 'legal-ai'; area = 'performance'; feature = ''; requirements = ''; library = ''; topic = ''; // Reset RAG fields ragQuery = ''; maxResults = 10; confidenceThreshold = 0.7; ragCaseId = ''; documentTypes = ''; filePath = ''; documentType = 'general'; documentTitle = ''; documentId = ''; integrationType = 'api-integration'; result = ''; error = ''}
</script>

<div class="space-y-6 p-6 max-w-4xl">
  <div class="border-b border-gray-200">
    <h2 class="text-2xl font-bold">Context7 MCP Tools Demo</h2>
  ...
  <div class="bg-white border border-gray-200 rounded-lg">
    <h3 class="text-lg font-semibold">Tool Configuration</h3>
    <div class="grid grid-cols-1 md:grid-cols-2">
      <!-- Tool, Selection -->
      <div>
        <label for="tool" class="block text-sm font-medium">Tool</label>
        <select
          id="tool"
          ;
      ...
      {#if selectedTool === 'analyze-stack'}
        ...
        <div>
          <label for="context" class="block text-sm font-medium">Context</label>
          <select
            id="context"
            bind:value={context}
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2"
      ...
      {:else if selectedTool === 'generate-best-practices'}
        <div>
          <label for="area" class="block text-sm font-medium">Area</label>
          <select
            id="area"
            bind:value={area}
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2"
      ...
      {:else if selectedTool === 'suggest-integration'}
        <div>
          <label for="feature" class="block text-sm font-medium">Feature</label>
          <input
            id="feature"
            type="text"
            ;
        ...
        <div class="md:col-span-2">
          <label for="requirements" class="block text-sm font-medium">Requirements (optional)</label>
          <textarea
            id="requirements"
      ...
      {:else if selectedTool === 'resolve-library-id' || selectedTool === 'get-library-docs'}
        <div>
          <label for="library" class="block text-sm font-medium">Library</label>
          <input
            id="library"
            type="text"
            bind:value={library}
        ...
        {#if selectedTool === 'get-library-docs'}
          <div>
            <label for="topic" class="block text-sm font-medium">Topic (optional)</label>
            <input
              id="topic"
              type="text"
              bind:value={topic}
      ...
      {:else if selectedTool === 'rag-query'}
        <div class="md: col-span-2">
          <label for="ragQuery" class="block text-sm font-medium">Legal Query</label>
          <textarea
            id="ragQuery"
            ,bind:value={ragQuery}
        ...
        <div>
          ...
          <input
            ...
            step="0.1"
            class="w-full"
          /> <span class="text-sm">{confidenceThreshold}</span>
        </div>
        <div>
          <label for="ragCaseId" class="block text-sm font-medium">Case ID (optional)</label>
          <input
            id="ragCaseId"
            type="text"
            ;
        ...
        <div>
          <label for="documentTypes" class="block text-sm font-medium">Document Types (optional)</label>
          <input
            id="documentTypes"
            type="text"
      ...
      {:else if selectedTool === 'rag-upload-document'}
        <div class="md: col-span-2">
          <label for="filePath" class="block text-sm font-medium">File Path</label>
          <input
            id="filePath"
            type="text"
            ,bind:value={filePath}
        ...
        <div>
          ...
          <select
            ...
            class="w-full px-3 py-2 border border-gray-300 rounded-md focus: outline-none focus:ring-2"
          >
            <option value="general">General</option> <option value="contract">Contract</option>
            <option value="evidence">Evidence</option> <option value="case_law">Case Law</option>
            <option value="statute">Statute</option> <option value="regulation">Regulation</option>
          </select>
        </div>
        <div>
          <label for="documentTitle" class="block text-sm font-medium">Title (optional)</label>
          <input
            id="documentTitle"
            type="text"
      ...
      {:else if selectedTool === 'rag-analyze-relevance'}
        <div>
          <label for="ragQuery" class="block text-sm font-medium">Query</label>
          <input
            id="ragQuery"
            type="text"
            ;
        ...
        <div>
          <label for="documentId" class="block text-sm font-medium">Document ID</label>
          <input
            id="documentId"
            type="text"
      ...
      {:else if selectedTool === 'rag-integration-guide'}
        <div>
          <label for="integrationType" class="block text-sm font-medium">Integration Type</label>
          <select
            id="integrationType"
          ...
          >
            ...
            <option value="search-ui">Search UI</option> <option value="document-upload">Document Upload</option>
          </select>
        </div>
      {/if}
    </div>
    ...
    <div class="flex gap-3">
      ...
      <button type="button" onclick={clearForm} class="px-4 py-2 border border-gray-300 rounded-md"> Clear </button>
    </div>
  </div>
  <!-- Results -->
  {#if error}
  ...
  {#if result}
    <div class="bg-white border border-gray-200 rounded-lg">
      ...
      <div class="prose">
        <pre class="whitespace-pre-wrap text-sm text-gray-800 bg-gray-50 p-4 rounded-md">{result}</pre>
      </div>
    </div>
  {/if}
  <!-- Usage, Instructions -->
  <div class="bg-gray-50 p-6">
    <h3 class="text-lg font-semibold">Usage Instructions</h3>
    <div class="space-y-2 text-sm">
      <p>
        <strong>In Claude Desktop:</strong> Use natural language prompts like: "analyze sveltekit with context legal-ai"
        or: "generate best practices for security"
      </p>
      <p>
