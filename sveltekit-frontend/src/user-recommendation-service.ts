...
export class UserRecommendationService {
    ...
    async storeAiChatInteraction(params: StoreAiChatParams): Promise<string> {
        try {
            const [insertedQuery] = await db.insert(userAiQueries)
                .values({
            ...
            if (params.sessionId) {
                ...
                const messageCount = results[0]?.messageCount ?? 0;
    ...
    async createRagSession(params: CreateRagSessionParams): Promise<string> {
        try {
            const [insertedSession] = await db.insert(ragSessions)
                .values({
                    userId: params.userId,
        ...
        try {
            const whereCondition = timeRange
                ? and(
                    ...
                    sql`"created_at" >= ${timeRange.from.toISOString()}`,
                    sql`"created_at" <= ${timeRange.to.toISOString()}`
                )
                : eq(userAiQueries.userId, userId);
            ...
            const success = Number(stats?.successfulQueries) || 0;
            const avgTime = Number(stats?.avgProcessingTime) || 0;
            ...
            return {
                ...
                topTopics,
                userSatisfaction: this.calculateSatisfactionScore(successRate, avgTime),
                improvementSuggestions: this.generateImprovementSuggestions(
                    { avgProcessingTime: avgTime, successfulQueries: success, totalQueries: total },
                    topTopics
                )
            };
        } catch (error) {
    ...
    private async getUserSessionStats(userId: string) {
        ...
        const activeHours = this.extractActiveHours(sessions.map(s => ({ startedAt: s.startedAt ?? null })));
        ...
        return {
            ...
            avgSessionLength: sessionLengths.length > 0
                ...
                : 0,
            avgQueriesPerSession: sessions.length > 0
    ...
    private async generateCaseRecommendations(
    ...
    private async extractTopTopics(
        ...
        return Array.from(topicCounts.entries())
            .sort(([a], [b]) => b - a) // Should be sort by count: sort(([, a], [, b]) => b - a). Original had bug?
    ...
    private assessQueryComplexity(queries: string[]): 'simple' | 'moderate' | 'complex' {
        if (queries.length === 0) return 'simple';
    ...
    private extractTopicsFromQueries(queries: string[]): string[] {
        ...
        const legalTerms = [
            'contract', 'liability', 'negligence', 'damages', 'evidence',
            'precedent', 'statute', 'regulation', 'compliance', 'litigation',
            'settlement', 'tort', 'property', 'intellectual', 'criminal',
            'civil', 'constitutional', 'employment'
        ];
    ...
    private generateImprovementSuggestions(
        ...
        const success = stats.successfulQueries ?? 0;
        ...
        if (topTopics.length > 0) {
            suggestions.push(
                `You seem interested in ${topTopics[0].topic} - explore related legal precedents`
            );
        }
