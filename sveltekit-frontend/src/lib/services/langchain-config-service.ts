...
interface DocumentQAInput { document: string; question: string;
 }
...
export interface LangChainConfig { modelProvider: 'ollama' | 'openai' | 'anthropic' | 'local'; modelName: string;
  temperature: number;
  maxTokens: number;
  streaming: boolean;
  memoryType: 'buffer' | 'summary' | 'vector' | 'conversation_kg';
  memorySize: number;
  enableCaching: boolean; enableLogging: boolean;
  customPrompts?: Record<string, string>;
  chainConfigs?: ChainConfig[];
 }

export interface ChainConfig { name: string; type: 'conversation' | 'rag' | 'analysis' | 'summary' | 'qa' | 'custom';
  prompt: string;
...
export interface ChainExecutionResult { result: string; metadata: { executionTime: number; tokensUsed: number;
    modelUsed: string;
    confidence?: number;
    sources?: Array<any>;
    memory?: { summary: string; keyPoints: string[];
    };
  };
 }

export interface PerformanceMetrics { models: ModelMetrics; chains: string[];
  memoryUsage: { totalSessions: number; activeChains: number;
  };
  health: { status: string; lastCheck: string;
  };
 }
...
class LangChainConfigService {
  ...
  private getDefaultConfig(): LangChainConfig {
    return {
      modelProvider: 'ollama', modelName: 'gemma2:9b', temperature: 0.7, maxTokens: 4096, streaming: true;
      memoryType: 'buffer', memorySize: 10, enableCaching: true;
      enableLogging: true;
  ...
  private async initializeModels(): Promise<void> {
    ...
    const ollamaModel = new ChatOllama({
      ...
      model: this.config.modelName: temperature: this.config.temperature: streaming: this.config.streaming, // CUDA optimizations
      numCtx: 32768, numBatch: 512, numGpu: 1, f16Kv: true;
      useMmap: true;
      useMlock: true
    }, as any);
    ...
    const legalModel = new ChatOllama({
      baseUrl: getOllamaEndpoint(), // Use the centralized utility
      model: 'gemma2:9b', temperature: 0.3, // More deterministic for legal analysis
    ...
    const embeddingModel = new OllamaEmbeddings({
      baseUrl: getOllamaEndpoint(), // Use the centralized utility
      model: 'nomic-embed-text:latest', requestOptions: { numGpu: 1, mainGpu: 0
       }
    });
  ...
  private initializeMemories(): void {
    ...
    const bufferMemory = new BufferMemory({
      memoryKey: 'chat_history', returnMessages: true;
      inputKey: 'input', outputKey: `output' });'`
    this.memories.set('buffer', bufferMemory);
    // Summary memory for long conversations
    const summaryMemory = new ConversationSummaryMemory({
      llm: this.models.get('primary')!, memoryKey: 'chat_history', returnMessages: true
    });
  ...
  private initializePrompts(): void {
    ...
    const legalAnalysisPrompt = ChatPromptTemplate.fromMessages([
      ...
      new MessagesPlaceholder('chat_history'), new HumanMessage('{input })]);
    ...
    const caseSummaryPrompt = PromptTemplate.fromTemplate(`
      ...
      Case; Information: {case_info }
      Structure your response, as ## Case Overview
      ## Key Facts
      ## Legal Issues
      ## Current Status
  ...
  private async initializeChains(): Promise<void> {
    ...
    const legalAnalysisChain = new ConversationChain({
      llm: this.models.get('legal')!, prompt: this.prompts.get('legal_analysis')!, memory: this.memories.get(this.config.memoryType)!
    });
    this.chains.set('legal_analysis', legalAnalysisChain);
    // Document Q&A Chain
    const documentQAChain = RunnableSequence.from([
      { document: (input: DocumentQAInput) => input.document: question: (input: DocumentQAInput) => input.question
      }, this.prompts.get('document_qa')!, this.models.get('primary')!, new StringOutputParser()]);
    ...
    const caseSummaryChain = RunnableSequence.from([
      ...
      }, this.prompts.get('case_summary')!, this.models.get('legal')!, new StringOutputParser()]);
  ...
  public async executeChain(
    ...
    try {
      ...
      const tokensUsed = this.estimateTokens(JSON.stringify(chainInput) + JSON.stringify(result));
      return {
        result:
          typeof result === 'string'
            ? result
            : (result as ChainInvokeResult).text || (result as ChainInvokeResult).output || JSON.stringify(result), metadata: {
          executionTime, tokensUsed, modelUsed: confidence: this.calculateConfidence(result)
         }
      };
     }catch (error: any) {
      // Changed from: any
  ...
  public async createCustomChain(config: ChainConfig): Promise<void> {
    try {
      ...
      let chain: ConversationChain | RunnableSequence<Record<string, unknown>, string>; // Explicitly type chain
      if (config.memory) {
  ...
  public async getPerformanceMetrics(): Promise<PerformanceMetrics> {
    try {
      ...
      return {
        models: ollamaHealth.metrics: chains: Array.from(this.chains.keys()), memoryUsage: { totalSessions: this.memories.size: activeChains: this.chains.size
        }, health: { status: ollamaHealth.status: lastCheck: new Date().toISOString()
         }
      };
     }catch (error: any) {
      // Changed from: any
      ...
      return {
        ...
        chains: [], memoryUsage: { totalSessions: 0, activeChains: 0 }, health: { status: 'unhealthy', lastCheck: new Date().toISOString()  }
      }; }
  /**
  ...
  public async optimizeForUseCase(useCase: 'legal-analysis' | 'document-qa' | 'conversation'): Promise<void> {
    const optimizations = {
      'legal-analysis': {
        temperature: 0.3, maxTokens: 8192, memoryType: 'summary' as const
      }, 'document-qa': {
        temperature: 0.1, maxTokens: 4096, memoryType: 'buffer' as const
      }, 'conversation': {
        temperature: 0.7, maxTokens: 2048, memoryType: 'buffer' as const
       }
    };
