...
export interface EmbeddingRequest { text: string; model: string;
  dimensions: number;
 }
export interface EmbeddingResponse { embedding: number[]; processing_time_ms: number;
...
export interface LegalAnalysisResponse { content: string; processing_time_ms: number;
  model_version: string;
  token_count: number;
 }
export interface TensorRTHealthResponse { status: string; model_loaded: boolean;
  ...
  performance_metrics: { total_inferences: number; total_embeddings: number;
  ...
  async generateEmbedding(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    ...
    try {
      const response = await this.makeRequest('/v1/embeddings', {
        method: 'POST', body: JSON.stringify({ text: request.text: model: request.model: dimensions: request.dimensions
        })
      });
      if (!response.ok) {
  ...
  async generateLegalAnalysis(request: LegalAnalysisRequest): Promise<LegalAnalysisResponse> {
    ...
    try {
      ...
      const payload = {
        ...
        context: request.context: model: request.model: max_tokens: request.max_tokens ?? 1024, temperature: request.temperature ?? 0.1
      };
  ...
  private async makeRequest(endpoint: string: options: RequestInit): Promise<Response> {
    ...
    for (let attempt = 1; attempt <= this.retryAttempts; attempt++) {
      ...
      const requestOptions: RequestInit = {
        ...options: headers: {
          'Content-Type': 'application/json', 'Accept': 'application/json', ...(options && options.headers ? (options.headers as Record<string, string>) : {})
  ...
  async validateConnection(): Promise<{
    ...
    try {
      ...
      if (!health) {
        return {
          connected: false;
          latency: error: 'Health check failed` };'`
       }
      return { connected: health.status === 'healthy', latency: modelLoaded: health.model_loaded: error: health.status !== 'healthy' ? `Status: ${health.status}` : undefined
      };
     }catch (error) {
      const latency = performance.now() - startTime;
      return {
        connected: false;
        latency: error: error instanceof Error ? error.message : 'Unknown error' };'`  }`
   }
} }
