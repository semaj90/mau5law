...
export class AdvancedBinaryEncodingService {
  ...
  createMiddleware(workflowContext?: LegalWorkflowContext) {
    return async (event: RequestEvent, resolve: (evt: RequestEvent) => Promise<Response>) => {
      ...
      if (acceptHeader.includes('application/cbor')) preferredFormat = 'cbor';
      else if (acceptHeader.includes('application/msgpack')) preferredFormat = 'msgpack';
      ...
      if (event.request.body && event.request.method !== 'GET') {
        ...
        if (contentType.includes('application/cbor')) format = 'cbor';
        else if (contentType.includes('application/msgpack')) format = 'msgpack';
      ...
      // Handle response encoding if client prefers non-JSON
      ...
      if (respContentType.includes('application/json') && preferredFormat !== 'json') {
        ...
        headers.set('x-encoding-format', format);
        headers.set('x-compression-ratio', metrics.compressionRatio.toFixed(2));
        headers.set('x-encode-time', `${metrics.encodeTime.toFixed(2)}ms`);
