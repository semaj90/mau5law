	...
	try {
		...
		for (const fnName of candidates) {
			...
			if (typeof fn === 'function') {
				// cast to EmbeddingFn and handle sync/async results safely
				const fnTyped = fn as EmbeddingFn;
...
/**
 * embedText
 * - tries Redis cache via getCachedEmbedding(text, model)
 * - falls back to callEmbeddingService
 * - caches result via cacheEmbedding(keyOrText, model, embedding)
 */
 ...
 */
export async function getVectorById(id: string): Promise<QdrantPoint | null> {
  try {
...
/**
 * Embeds text and upserts into Qdrant + Redis cache
