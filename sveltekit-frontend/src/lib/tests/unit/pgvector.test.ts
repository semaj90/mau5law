...
describe('pgvector adapter', () => {
  ...
  it('upsertToPGVector calls PG with expected SQL and parameters', async () => {
    // Dynamically import module AFTER mocking pg so Pool is our fake
    ...
    expect(callArgs[0].toString()).toContain('INSERT INTO embeddings');
    expect(callArgs[1][0]).toBe('doc1');
  });

  it('searchPGVector calls PG and returns shaped SearchResult items', async () => {
    const pgvector = await import('$lib/server/vector/pgvector');
    ...
    const results = await pgvector.searchPGVector([0.1, 0.2], 5);
    expect(mockQuery).toHaveBeenCalled();
