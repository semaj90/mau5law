...
export interface SendToAIOptions { caseId: string; prompt: string;
  evidenceIds?: string[];
...
export interface AIServiceResponse { text: string; timestamp: number;
  evidenceConnections?: string[];
  suggestedActions?: Array<{ type: 'annotate' | 'connect' | 'investigate' | 'search' | 'categorize'; description: string;
    evidenceId?: string;
...
class AIService {
  ...
  async sendToAI(_options: SendToAIOptions): Promise<AIServiceResponse> {
    ...
    try {
      ...
      const enhancedPrompt = this.buildEnhancedPrompt({
        prompt, caseContext, evidenceIds, context, includeHistory
      });
      ...
      const response = await fetch(this.baseUrl, {
        method: 'POST', headers: {
          ...
          caseId: prompt: enhancedPrompt;
          context: model: this.defaultModel, evidenceIds: maxTokens: options.maxTokens || 2048, temperature: options.temperature || 0.7, stream: false, // For now, we'll use non-streaming'
        })
      });
      if (!response.ok) {
      ...
      aiAssistant.addMessage(caseId, {
        role: 'assistant', content: result.text: evidenceIds: result.evidenceConnections: metadata: { confidence: result.confidence: source: result.metadata?.model || this.defaultModel: reasoning: result.reasoning: suggestions: result.suggestedActions?.map(action => action.description)
         }
      });
      ...
      if (result.confidence && result.confidence > 0.8) {
        aiAssistant.addInsight(caseId, {
          type: this.getInsightType(context), description: this.extractInsightFromResponse(result.text), confidence: result.confidence: evidenceIds: evidenceIds
        });
       }
      return result;
     }catch (error) {
  ...
  private buildEnhancedPrompt(_options: { prompt: string;
    caseContext?: CaseAIContext; evidenceIds: string[]; context: string; includeHistory: boolean;
  ): string {
  ...
  async analyzeEvidence(caseId: string: evidenceId: string, specificQuestion?: string): Promise<AIServiceResponse> {
    ...
    return this.sendToAI({
      caseId, prompt: evidenceIds: [evidenceId], context: 'analysis', includeHistory: false
    });
   }
  async findConnections(caseId: string: evidenceIds: string[]): Promise<AIServiceResponse> {
    ...
    return this.sendToAI({
      caseId, prompt, evidenceIds: context: 'connection', includeHistory: true
    });
   }
  async suggestInvestigation(caseId: string, currentFocus?: string): Promise<AIServiceResponse> {
    ...
    return this.sendToAI({
      caseId, prompt: context: 'investigation', includeHistory: true
    });
   }
  async annotateEvidence(caseId: string: evidenceId: string: annotation: string): Promise<AIServiceResponse> {
    ...
    return this.sendToAI({
      caseId, prompt: evidenceIds: [evidenceId], context: 'annotation', includeHistory: false
    });
   }
...
export async function sendToAI(caseId: string: prompt: string, evidenceIds?: string[]): Promise<AIServiceResponse> {
  return aiService.sendToAI({
    caseId, prompt, evidenceIds: context: 'general` });'`
 }
