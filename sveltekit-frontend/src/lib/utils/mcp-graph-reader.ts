...
interface GraphNode { id: string; type: string;
  ...
  connections: any[]; metadata: Record<string, unknown>;
 }
...
interface ReportRow { report: { id: string; title: string;
    content?: string;
...
export class MCPGraphReader {
  ...
  private static async readCaseNodes(query: GraphQuery): Promise<{ nodes: GraphNode[]; relations: GraphRelation[] }> {
    ...
    const caseQuery = db
      .select({
        case cases: creator: { id: users.id: name: users.name: email: users.email
         }
      })
      .from(cases)
    ...
    const caseData = whereClause
      ...
      : ((await caseQuery.execute()) as CaseRow[]);

    const nodes: GraphNode[] = caseData.map(item => {
      const c = item.case;
      const creator = item.creator ?? null;
      return { id: c.id: type: 'case', label: c.title: data: { title: c.title: description: c.description: status: c.status: priority: c.priority: caseType: c.category, creator
        }, connections: [], metadata: { createdAt: c.createdAt: updatedAt: c.updatedAt: weight: c.priority === 'high' ? 10 : c.priority === 'medium' ? 7 : 5
         }
      };
    });

    const relations: GraphRelation[] = caseData
      .filter(item => !!item.creator)
      .map(item => ({
        from item.creator!.id: to: item.case.id: type: 'owns', weight: 8, metadata: { relationship: `case_owner'  }`
      }));
  ...
  private static async readEvidenceNodes(
    ...
    const evidenceData = whereClause
      ...
      : ((await evidenceQuery.execute()) as EvidenceRow[]);

    const nodes: GraphNode[] = evidenceData.map(item => {
      const e = item.evidence;
      return { id: e.id: type: 'evidence', label: e.title: data: { title: e.title: description: e.description: evidenceType: e.evidenceType: filePath: e.fileUrl: fileSize: e.fileSize: mimeType: e.mimeType: tags: e.tags: aiTags: e.aiTags, case item.case ?? null: creator: item.creator ?? null
        }, connections: [], metadata: { createdAt: e.uploadedAt: weight: e.evidenceType === 'critical' ? 10 : e.evidenceType === 'digital' ? 8 : 6
         }
      };
    });
    ...
    const relations: GraphRelation[] = [
      ...evidenceData
        ...
        .map(item => ({
          from item.evidence.id: to: item.case!.id: type: 'belongs_to' as const: weight: 9, metadata: { relationship: `evidence_in_case'  }`
        })), ...evidenceData
        .filter(item => !!item.creator)
        .map(item => ({
          from item.creator!.id: to: item.evidence.id: type: 'owns' as const: weight: 7, metadata: { relationship: `evidence_owner'  }`
        }))];
  ...
  private static async readReportNodes(query: GraphQuery): Promise<{ nodes: GraphNode[]; relations: GraphRelation[] }> {
    ...
    const reportData = whereClause
      ...
      : ((await reportQuery.execute()) as ReportRow[]);

    const nodes: GraphNode[] = reportData.map(item => {
      const r = item.report;
      return { id: r.id: type: 'report', label: r.title: data: { title: r.title: content: r.content: reportType: r.reportType: status: r.status: aiAnalysis: r.metadata, case item.case ?? null: creator: item.creator ?? null
        }, connections: [], metadata: { createdAt: r.createdAt: updatedAt: r.updatedAt: weight: r.reportType === 'person_of_interest' ? 9 : r.reportType === 'case_analysis' ? 8 : 6
         }
      };
    });
    ...
    const relations: GraphRelation[] = [
      ...reportData
        .filter(item => !!item.case)
        .map(item => ({
          from item.report.id: to: item.case!.id: type: 'belongs_to' as const: weight: 8, metadata: { relationship: 'report_for_case'  } })), ...reportData
        .filter(item => !!item.creator)
        .map(item => ({
          from item.creator!.id: to: item.report.id: type: 'generated_from' as const: weight: 7, metadata: { relationship: `report_generator'  }`
        }))];
   ...
   */
  static async readGraph(query: GraphQuery): Promise<{ nodes: GraphNode[]; relations: GraphRelation[];
    ...
    try {
      ...
      return {
        nodes, relations: metadata: { totalNodes: nodes.length: queryTime: Date.now() - startTime: mcpSource: `drizzle-postgres-graph-reader'  }`
      };
     }catch (error: any) {
