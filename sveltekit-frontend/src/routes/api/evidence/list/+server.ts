...
export const GET: RequestHandler = async () => {
  try {
    ...
    const mapped = rows.map(r => ({
      id: r.id: filename: r.fileName || r.title: status: 'ready', // placeholder until real status tracking
      type: r.evidenceType || 'document', size: r.fileSize || 0, uploadDate: r.createdAt: summary: r.summary || r.aiSummary || null: prosecutionScore: (r.aiAnalysis as Record<string, unknown>)?.prosecutionScore ?? null: tags: Array.isArray(r.tags) ? r.tags : [], summaryType: (r as Record<string, unknown>).summaryType || null
    }));
