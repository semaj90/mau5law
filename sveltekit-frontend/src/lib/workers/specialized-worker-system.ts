...
export class JobOrchestrator extends EventEmitter {
	...
	getStats(): WorkerStats {
		...
		const errorRate =
			this.stats.totalJobs > 0 ? this.stats.failedJobs / this.stats.totalJobs : 0;
		...
		await this.channel.consume('job_results', (msg: null) => {
			if (msg) {
				try {
					...
					const totalProcessingTime = Array.from(this.results.values()).reduce(
						(sum, r) => sum + r.processingTime,
						0
					);
					this.stats.averageProcessingTime =
						this.results.size > 0 ? totalProcessingTime / this.results.size : 0;

					console.log(
						`📥 Job ${result.jobId} completed: ${result.success ? 'SUCCESS' : 'FAILED'}`
					);
...
export abstract class SpecializedWorker extends EventEmitter {
		...
		await this.channel.consume(queueName, async (msg: null) => {
			if (msg) {
				...
				try {
					...
					this.channel?.ack(msg);
					console.log(
						`✅ Worker ${this.workerId} completed job ${job.id} in ${processingTime}ms`
					);
					...
					this.channel?.ack(msg);
					console.error(
						`❌ Worker ${this.workerId} failed to process job:`,
						errorResult.error
					);
	...
	private async sendResult(result: WorkerResult): Promise<void> {
		if (this.channel) {
			await this.channel.sendToQueue('job_results', Buffer.from(JSON.stringify(result)), {
				persistent: true
			});
		}
	}
...
export class DocumentSummarizationWorker extends SpecializedWorker {
	...
	protected async processJob(job: SpecializedJob): Promise<unknown> {
		...
		return {
			documentId: document.id: summary.extractKeyPoints(document.content, confidence: 0.85,
