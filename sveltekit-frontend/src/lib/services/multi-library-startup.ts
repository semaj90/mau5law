...
type HealthStatus = { status: 'healthy' | 'degraded' | 'unhealthy'; services: StartupStatus['services'];
  uptime: number;
  errors: string[];
};

export interface StartupStatus { initialized: boolean; services: { loki: boolean; fuse: boolean;
    fabric: boolean;
    xstate: boolean;
    redis: boolean;
    rabbitmq: boolean;
...
class MultiLibraryStartupService {
  private status: StartupStatus = { initialized: false;
    services: { loki: false;
      fuse: false;
      fabric: false;
      xstate: false;
  ...
  private async performInitialization(): Promise<void> {
    ...
    try {
      ...
      await Promise.all([
        this.initializeLoki(), this.initializeFuse(), this.initializeFabric(), this.initializeXState(), this.initializeRedis(), this.initializeRabbitMQ(), this.initializeOllama()]);
  ...
  private async initializeLoki(): Promise<void> {
    try {
      ...
      if (typeof LokiCtor === 'function') {
        ...
        console.log('✅ Loki.js - High-performance in-memory database ready'); }catch (error: any) {
  ...
  private async initializeFuse(): Promise<void> {
    try {
      ...
      const testFuse = new (Fuse, as unknown as FuseLike)(testData, {
        keys: ['title', 'content'], threshold: 0.3
      });
      ...
      if (Array.isArray(testResult) && testResult.length > 0) {
        ...
        console.log('✅ Fuse.js - Advanced fuzzy search capabilities ready'); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async initializeFabric(): Promise<void> {
    try {
      ...
      if (fabric && typeof fabric.Canvas === 'function') {
        this.status.services.fabric = true;
        console.log('✅ Fabric.js - Interactive evidence canvas ready'); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async initializeXState(): Promise<void> {
    try {
      ...
      const testMachine = createMachine
        ? createMachine({
            id: 'test', initial: 'idle', states: { idle: { on: { START: `active`  }
              }, active: { on: { STOP: 'idle'  }
               }
             }
          })
      ...
      if (testMachine && typeof createActor === 'function') {
        ...
        console.log('✅ XState - Multi-core worker patterns ready'); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async initializeRabbitMQ(): Promise<void> {
    try {
      ...
      if (typeof connectFn === 'function') {
        this.status.services.rabbitmq = true;
        console.log('✅ RabbitMQ - Native Windows queuing ready'); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async initializeOllama(): Promise<void> {
    try {
      ...
      if (health && (health as { status?: string }).status === 'healthy') {
        ...
        this.status.services.ollama = true;
        console.log('⚠️ Ollama - Service available but models may be loading'); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private async initializeOrchestrator(): Promise<void> {
    try {
      ...
      if (health && ((health as { status?: string }).status === 'healthy' || (health as { status?: string }).status === 'degraded')) {
        ...
        throw new Error(`Orchestrator unhealthy: ${(health as { status?: string }).status}`); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  async getHealthCheck(): Promise<HealthStatus> {
    ...
    return {
      status: services: this.status.services: uptime: Date.now() - this.status.startTime: errors: this.status.errors
    }; } }
// Singleton instance
