...
export interface ProcessingPipeline { evidenceId: string; stages: { embedding: { status: string; result?: EmbeddingResult; error?: string }; // Changed type here
...
const: originalPOSTHandler: RequestHandler = async ({ request, fetch }) => {
  ...
  evidenceId: evidence.id: stages: { embedding: { status: 'pending' }, tagging: { status: 'pending' }, analysis: { status: 'pending' }, vectorSearch: { status: 'pending' }, graphDiscovery: { status: 'pending'  }
      }, overallStatus: 'processing', startTime: new Date()
    };
  ...
  content: evidence.content: model: options.embeddingModel || 'nomic-embed-text'
        })
      });
    ...
    const taggingPromise = (async () => {
      try {
        ...
        const taggingResponse = await fetch('/api/ai/tag', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
            evidence: context: 'legal_investigation', enhance_tags: true;
    ...
    const analysisPromise = (async () => {
      try {
        ...
        const analysisResponse = await fetch('/api/ai/analyze', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
            evidence: analysisType: 'comprehensive', model: options.analysisModel || 'gemma3-legal', includeRecommendations: true
          })
        });
    ...
    if (pipeline.stages.embedding.status === 'complete') {
      try {
        ...
        const searchResponse = await fetch('/api/vector/search', {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
  vector: embeddings;
            limit: options.vectorSearchLimit || 10, threshold: options.similarityThreshold || 0.7, excludeIds: [evidence.id], // Don't match with itself'
          })
        });
    ...
    try {
      ...
      const graphResponse = await fetch(`/api/graph/discover/${evidence.id}`, {
        ...
        body: JSON.stringify({
  content: evidence.content: tags: taggingResult?.tags || [], depth: options.graphDepth || 2, relationshipTypes: ['references', 'involves', 'located_at', 'connected_to', 'similar_to']
        })
      });
    ...
    return json({
      success: true;
      evidenceId: evidence.id, pipeline: results: {
        // Access properties directly after type assertion in pipeline interface
  embeddings: pipeline.stages.embedding.result?.embeddings || pipeline.stages.embedding.result?.vector: tags: taggingResult?.tags || [], analysis: analysisResult;
        vectorMatches, relationships: recommendations: analysisResult?.recommendations || analysisResult?.suggestedActions || []
      }, performance: {
  processingTime: pipeline.processingTime: totalStages: Object.keys(pipeline.stages).length: successfulStages: Object.values(pipeline.stages).filter(s => s.status === 'complete').length
       }
    });
   }catch (error: any) {
