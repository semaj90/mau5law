...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
  <div class="max-w-7xl mx-auto">
    ...
    <div class="text-center">
      <h1 class="text-4xl font-bold text-gray-900">âš–ï¸ Legal AI Suite</h1>
      <p class="text-lg">GPU-Accelerated Legal Document Analysis with Enhanced RAG</p>
    </div>
    <!-- System Status, Cards -->
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text is-primary text-sm">GPU Acceleration</h3></div>
        <div class="yorha-panel-content">
          <div class="flex items-center">
            {#if systemMetrics.gpuAcceleration}
              <CheckCircle class="h-4 w-4" /> <span class="text-sm">Enabled</span>
            {:else}
              <AlertTriangle class="h-4 w-4" /> <span class="text-sm">CPU Mode</span>
            {/if}
          </div>
        </div>
      </div>
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text is-primary text-sm">Ollama Status</h3></div>
        <div class="yorha-panel-content">
          <div class="flex items-center">
            {#if systemMetrics.ollamaStatus === 'healthy'}
              <CheckCircle class="h-4 w-4" /> <span class="text-sm">Healthy</span>
            {:else}
              <AlertCircle class="h-4 w-4" /> <span class="text-sm">Offline</span>
            {/if}
          </div>
        </div>
      </div>
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text is-primary text-sm">Processing Speed</h3></div>
        <div class="yorha-panel-content">
          <div class="text-sm">
            {#if isProcessing}
      ...
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text is-primary text-sm">Case AI Score</h3></div>
        <div class="yorha-panel-content">
          <div class="text-sm">{systemMetrics.caseAIScore}/100</div>
          <Progress value={systemMetrics.caseAIScore} class="h-2" />
        </div>
      </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-2">
      ...
      <div class="nes-container">
        <div class="yorha-panel-header">
          <h3 class="nes-text is-primary flex items-center">
            <UploadCloud class="h-5" /> <span>Document Processing</span>
          </h3>
        </div>
        <div class="yorha-panel-content">
          <div>
          ...
          <div class="flex">
            <button
              ...
              disabled={!canProcess}
              class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700"
            >
              {#if isProcessing}
                <div class="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                <span>Processing...</span>
              {:else}
                <UploadCloud class="h-4 w-4" /> <span>Process Documents</span>
              {/if}
            </button>
          </div>
          <!-- Processing, Summary -->
          {#if processingSummary}
            <div class="border-t pt-4">
              <h4 class="font-semibold">Processing Summary</h4>
              <div class="grid grid-cols-2 gap-4">
                <div>
                  <span class="text-gray-600">Total Entities:</span>
                  <span class="font-medium">{processingSummary.totalEntities}</span>
                </div>
                <div>
                  <span class="text-gray-600">Total Chunks:</span>
                  <span class="font-medium">{processingSummary.totalChunks}</span>
                </div>
                <div>
      ...
      <div class="nes-container">
        <div class="yorha-panel-header">
          <h3 class="nes-text is-primary flex items-center"><Search class="h-5" /> <span>Enhanced RAG Query</span></h3>
        </div>
        <div class="yorha-panel-content">
          <div>
            <label for="rag-query" class="block text-sm font-medium text-gray-700"> Legal Query </label>
            <input
              id="rag-query"
              bind:value={ragQuery}
            ...
            <div class="flex space-x-2">
              <button
                ...
                disabled={!canQuery}
                class="flex-1 inline-flex items-center justify-center px-4 py-2 bg-green-600 text-white rounded-md hover:bg-green-700"
              >
                <Brain class="h-4 w-4" /> Query Enhanced RAG
              </button>
            </div>
          </div>
          <!-- RAG, Results -->
          {#if ragResults.length > 0}
            <div class="border-t">
              <h4 class="font-semibold text-gray-800">Query Results</h4>
              <div class="space-y-3 max-h-64">
                {#each Array.isArray(ragResults) ? ragResults : [] as result}
                  <div class="p-3 bg-gray-50">
                    <div class="flex justify-between items-start">
                      <span class="text-sm font-medium"> {result.sourceDocument ?? 'Unknown Source'} </span>
                      <div class="flex">
                    ...
                    <p class="text-sm text-gray-700">{(result?.content ?? '').substring(0, 200)}...</p>
      ...
      <div class="nes-container">
        <div class="yorha-panel-header">
          <h3 class="nes-text is-primary flex items-center">
            <span>ðŸ”¥</span> <span>GPU-Accelerated Legal Search</span>
            {#if systemMetrics.gpuAcceleration}
              <span class="px-2 py-1 rounded text-xs font-medium bg-blue-500">GPU Active</span>
            {:else}
              <span class="px-2 py-1 rounded text-xs font-medium border border-gray-300">CPU Mode</span>
            {/if}
          </h3>
        </div>
        <div class="yorha-panel-content"><GPUAcceleratedLegalSearch /></div>
      </div>
    </div>
    <!-- Processed Documents, Display -->
    {#if processedDocuments.length > 0}
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text">Processed Documents</h3></div>
        <div class="yorha-panel-content">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
            {#each Array.isArray(processedDocuments) ? processedDocuments : [] as doc}
              <div class="p-4 border border-gray-200">
                <h4 class="font-semibold text-gray-800">{doc.filename ?? 'Untitled Document'}</h4>
                <div class="space-y-1">
                  <div class="flex">
                  ...
                  <div class="flex">
                    <span class="text-gray-600">Entities:</span> <span class="font-medium">{doc.entityCount ?? 0}</span>
                  </div>
                  <div class="flex">
                    <span class="text-gray-600">Chunks:</span> <span class="font-medium">{doc.chunkCount ?? 0}</span>
                  </div>
                  <div class="flex">
                  ...
                  {#if doc.factCheckSummary}
                    <div class="mt-2 pt-2 border-t">
                      <div class="text-xs">
                        Facts: <span class="text-green-600">{doc.factCheckSummary.verified ?? 0}</span> | Disputed:
    ...
    <div class="nes-container">
      <div class="yorha-panel-header flex flex-row items-center">
        <h3 class="nes-text is-primary flex items-center">
          <AlertCircle class="h-5" /> <span>Real-time System Logs</span>
        </h3>
        <button
          type="button"
          onclick={clearLogs}
      ...
      <div class="yorha-panel-content">
        <div class="bg-gray-900 text-green-400 p-4 rounded-md font-mono text-sm max-h-64">
    ...
    {#if processedDocuments.length > 0 || ragResults.length > 0}
      <div class="nes-container">
        <div class="yorha-panel-header"><h3 class="nes-text">System Performance Statistics</h3></div>
        <div class="yorha-panel-content">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <div class="text-2xl font-bold">{processedDocuments.length}</div>
              <div class="text-sm">Documents Processed</div>
            </div>
            <div>
              <div class="text-2xl font-bold">{totalEntities}</div>
              <div class="text-sm">Entities Extracted</div>
            </div>
            <div>
              <div class="text-2xl font-bold">{ragResults.length}</div>
              <div class="text-sm">RAG Results</div>
            </div>
            <div>
              <div class="text-2xl font-bold">{(averageProsecutionScore * 100).toFixed(0)}%</div>
              <div class="text-sm">Avg Prosecution Score</div>
            </div>
          </div>
        </div>
      </div>
