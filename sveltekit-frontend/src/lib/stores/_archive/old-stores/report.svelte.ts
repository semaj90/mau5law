...
// Evidence types
export interface Evidence { id: string; type: 'document' | 'image' | 'video' | 'audio' | 'link'; title: string;
  description?: string;
  url?: string;
  file?: File;
...
// Report structure
export interface Report { id: string; title: string; content: string; // TinyMCE HTML content: attachedEvidence: Evidence[];
  ...
  settings: { autoSave: boolean; theme: 'light' | 'dark';
    layout: 'single' | 'dual' | 'masonry';
  };
 }
// Default report
const defaultReport: Report = { id: crypto.randomUUID(), title: 'Untitled Report', content: '<p>Begin writing your report...</p>', attachedEvidence: [], metadata: { createdAt: new Date(), updatedAt: new Date(), version: 1, status: 'draft', tags: []
  }, settings: { autoSave: true;
    theme: 'light', layout: 'single'
   }
};
// Main report store
...
export const editorState = writable({
  ...
  lastSaved: new Date(), wordCount: 0, selectedText: '', cursorPosition: 0
});
// UI state for report editing
export const reportUI = writable({
  sidebarOpen: true;
  sidebarWidth: 300, evidencePanelOpen: true;
  evidencePanelHeight: 400, toolbarCollapsed: false;
  fullscreen: false
});
// Derived stores
...
// Small helper to call embedding API (replace with productionServiceClient as needed)
...
export const reportActions = {
  updateTitle: (title: string) => {
    report.update(r => ({
      ...r, title: metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
  }, updateContent: (content: string) => {
    report.update(r => ({
      ...r, content: metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    ...
    const newEvidence: Evidence = { id: crypto.randomUUID(), type: evidence.type ?? 'document', title: evidence.title ?? 'Untitled', description: evidence.description: url: evidence.url: file: evidence.file: metadata: (evidence.metadata ?? {}) as Record<string, unknown>, tags: evidence.tags ?? [], // preserve externalId/summary if passed
      externalId: evidence.externalId: summary: evidence.summary, // embeddings start empty until generated
      embedding: evidence.embedding ?? [], embeddingVersion: evidence.embeddingVersion: createdAt: evidence.createdAt ?? new Date(), updatedAt: evidence.updatedAt ?? new Date()
    };
    report.update(r => ({
      ...r: attachedEvidence: [...r.attachedEvidence, newEvidence], metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
  }, removeEvidence: (evidenceId: string) => {
    report.update(r => ({
      ...r: attachedEvidence: r.attachedEvidence.filter(e => e.id !== evidenceId), metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
  }, reorderEvidence: (newOrder: Evidence[]) => {
    report.update(r => ({
      ...r: attachedEvidence: newOrder;
      metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
  }, updateSettings: (settingsPatch: Partial<Report['settings']>) => {
    report.update(r => ({
      ...r: settings: { ...r.settings, ...settingsPatch }, metadata: { ...r.metadata: updatedAt: new Date()  }
    }));
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
  }, save: async () => {
  ...
  generateEmbeddingForEvidence: async (evidenceId: string, sourceText?: string): Promise<number[]> => {
    const embedding = await fetchEmbedding(sourceText ?? '');
    if (!embedding || embedding.length === 0) return [];
    report.update(r => {
      const updated = r.attachedEvidence.map(e => {
        if (e.id !== evidenceId) return e;
        return {
          ...e, embedding: embeddingVersion: 'local-emb-v1', // adjust to actual model/version identifier
          updatedAt: new Date()
        };
      });
    ...
    editorState.update(s => ({ ...s: hasUnsavedChanges: true }));
    return embedding;
  }, generateSummaryForEvidence: async (evidenceId: string): Promise<string> => {
    let source = '';
    let currentTitle = '';
    // pick text to summarize from the report store
    const unsub = report.subscribe(r => {
      const e = r.attachedEvidence.find(x => x.id === evidenceId);
      if (e) {
...
export const setupAutoSave = (intervalMs: number = 30000) => {
  ...
  const unsubscribe = report.subscribe($report => {
    const shouldAuto = !!$report.settings?.autoSave;
    if (shouldAuto && autoSaveInterval == null) {
