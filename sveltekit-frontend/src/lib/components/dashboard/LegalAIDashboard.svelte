...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import { apiClient } from '$lib/services/enhanced-api-client'; import { toast } from 'svelte-sonner'; // State using Svelte, 5 runes let cases = $state<any[]>([]); let evidence = $state<any[]>([]); let reports = $state<any[]>([]); let personsOfInterest = $state<any[]>([]); let loading = $state<boolean>(true); let error = $state<string | null>(null); let selectedTab = $state<string>('overview'); let refreshInterval = $state<ReturnType<typeof setInterval> | null>(null); // Statistics let stats = $state({ totalCases: 0, totalEvidence: 0, totalReports: 0, totalPersons: 0, activeCases: 0, pendingAnalysis: 0, recentActivity: 0 }); // Real-time processing status let systemHealth = $state({ api: 'unknown', database: 'unknown', aiServices: 'unknown', jobQueue: 'unknown'
  }); // Tabs configuration const tabs = [ { id: 'overview', label: 'Overview', icon: 'ðŸ“Š' }, { id: 'cases', label: 'Cases', icon: 'âš–ï¸' }, { id: 'evidence', label: 'Evidence', icon: 'ðŸ”' }, { id: 'reports', label: 'Reports', icon: 'ðŸ“„' }, { id: 'persons', label: 'Persons', icon: 'ðŸ‘¤' }, { id: 'processing', label: 'Processing', icon: 'âš™ï¸' }]; // Load dashboard data const loadDashboardData = async () => { try { loading = true; error = null; // Load all data in parallel const [casesResponse, evidenceResponse, reportsResponse, personsResponse] = await Promise.all([ (apiClient as: unknown).getCases?.({ limit: 10, sortBy: 'updated_at', sortOrder: 'desc' }) || { data: [], total: 0 }, (apiClient as: unknown).getEvidence?.({ limit: 10, sortBy: 'updated_at', sortOrder: 'desc' }) || { data: [], total: 0 }, (apiClient as: unknown).getReports?.({ limit: 10, sortBy: 'updated_at', sortOrder: 'desc' }) || { data: [], total: 0 }, (apiClient as: unknown).getPersonsOfInterest?.({ limit: 10, sortBy: 'updated_at', sortOrder: 'desc' }) || { data: [], total: 0 }]); // Update state with proper fallbacks cases = Array.isArray(casesResponse.data) ? casesResponse.data: [], evidence = Array.isArray(evidenceResponse.data) ? evidenceResponse.data: [], reports = Array.isArray(reportsResponse.data) ? reportsResponse.data: [], personsOfInterest = Array.isArray(personsResponse.data) ? personsResponse.data: []; // Calculate statistics stats = { totalCases: casesResponse.total || 0, totalEvidence: evidenceResponse.total || 0, totalReports: reportsResponse.total || 0, totalPersons: personsResponse.total || 0, activeCases: cases.length, pendingAnalysis: evidence.filter((item: unknown) => !item?.aiSummary).length, recentActivity: cases.filter(c => { if (!c?.updatedAt) return false; const dayAgo = new Date(Date.now() - 24 * 60 * 60 * 1000); return new Date(c.updatedAt) > dayAgo}).length }; // Load system health await loadSystemHealth()} catch (err: unknown) { console.error('Dashboard load error:', err); error = err instanceof Error ? err.message: 'Failed to load dashboard data'; toast.error('Failed to load dashboard', { description error })} finally { loading = false}
  }; // Load system health status const loadSystemHealth = async () => { try { const healthResponse = (await (apiClient as: unknown).getHealthStatus?.()) || {}; systemHealth = { api: healthResponse?.status === 'healthy' ? 'healthy': healthResponse?.status === 'error' ? 'error': 'warning', database: healthResponse.services?.database || 'unknown', aiServices: healthResponse.services?.aiServices || 'unknown', jobQueue: healthResponse.services?.jobQueue || 'unknown'
      }} catch (err: unknown) { console.error('Health check failed:', err); systemHealth = { api: 'error', database: 'unknown', aiServices: 'unknown', jobQueue: 'unknown'
      }}
  }; const createQuickCase = async () => { try { const caseData = { title: `New Case - ${new Date().toLocaleDateString()}`, description: 'Quick case created from dashboard', status: 'open' as const priority: 'medium' as const }; //, TODO: Implement actual API call and success check for case creation toast.success('Case created successfully!'); await loadDashboardData(); // Refresh data } catch (err: unknown) { console.error('Quick case creation error:', err); toast.error('Failed to create case')}
  ...
  }; // Lifecycle $effect(() => { loadDashboardData(); // Set up auto-refresh every, 30 seconds refreshInterval = setInterval(loadDashboardData, 30000); // Cleanup function return () => { if (refreshInterval) { clearInterval(refreshInterval as: unknown), refreshInterval = null}
    }}); </script>
 ...
 <div class="mt-2 text-sm"> <ul class="list-disc pl-5"> <li>Evidence analysis jobs are running automatically when evidence is uploaded</li>
