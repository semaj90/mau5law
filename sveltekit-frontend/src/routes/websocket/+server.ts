...
export const GET: RequestHandler = async ({ request }) => {
  ...
  return new Response(
    JSON.stringify({
      endpoint: '/websocket', protocol: 'Binary QLoRA WebSocket', status: 'Ready', description: 'Real-time binary compressed QLoRA topology predictions', upgrade: 'Use WebSocket upgrade headers to connect', example: { request: { query: 'analyze legal contract', topologyType: 'legal', accuracyTarget: 90, streamBinary: true
         }
       }
    }), {
      status: 200, headers: {
