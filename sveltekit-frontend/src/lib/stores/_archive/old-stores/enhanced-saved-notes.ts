...
export const noteStats = derived(legalNotes, ($legalNotes): NoteStats => {
  const stats: NoteStats = { total: $legalNotes.length,
    byType: {},
    byRiskLevel: {},
    aiGenerated: 0,
    ocrExtracted: 0,
...
class EnhancedNotesManager {
      ...
      if (note.noteType === 'legal_analysis' || note.content.length > 100) {
        ...
        if (analysisResult.success) {
          ...
          if (analysis.citations) {
            note.metadata.legalCitations = analysis.citations.map((citation: any) => ({ type: citation.type,
  ...
  private async storeInNeo4j(note: LegalNote): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        note.metadata.graphRelationships = result.relationships;
      } }
    } }catch (error) {
  ...
  private async indexInRAG(note: LegalNote): Promise<void> {
    try {
      const response = await fetch('/api/rag/index/notes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ documentId: note.id,
          content: note.content,
      ...
      if (response.ok) {
        ...
        note.metadata.ragChunks = result.chunks;
      } }
    } }catch (error) {
  ...
  private async getExistingNote(noteId: string): Promise<LegalNote | null> {
    ...
    try {
      ...
      if (stored) {
        return await this.simdCache.parse(stored);
      } }
    } }catch (error) {
