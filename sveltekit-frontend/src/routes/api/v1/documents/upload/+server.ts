...
import type { writeFile, unlink, mkdir  } from 'fs/promises';
...
import type { queueDocumentProcessing  } from '$lib/services/queue-service';
import type { generateEmbeddings, // Corrected: Use generateEmbeddings
...
// Define a local interface for the mock user to satisfy type checking
interface DevUser {
  id: string;
  email: string;
  name: string;
...
export const POST = async ({ request, locals }) => {
  ...
  let description: string | undefined; // Corrected declaration
  ...
  try {
    ...
    console.log(
      `✅ [Upload] MinIO upload complete: ${minioResult.bucket}/${minioResult.objectName}`
    );
    ...
    try {
      ...
      try {
        ...
        if (fallbackResp.ok) {
          ...
          console.warn(
            `⚠️ [Upload] Fallback embedding service returned status: ${fallbackResp.status}`
          );
        }
    ...
    try {
      ...
      const { jobId: procJobId, estimated } = (await queueDocumentProcessing(jobData)) as {
        // Corrected: Removed second argument
        jobId: string;
        estimated: number;
      };
    ...
    const response: UploadResponse = {
      ...
      processingTime,
      minioUrl: minioResult.url, // Corrected
      metadata: {
        bucket: minioResult.bucket,
        objectName: minioResult.objectName,
        contentType,
        extractedText: extractedText.length, // Corrected
  ...
  try {
    ...
    const points = chunks.map((chunk, i) => ({
      ...
      payload: {
        ...
        chunk_index: i,
        text: chunk,
        filename: metadata.filename, // Corrected
        user_id: metadata.userId, // Corrected
        case_id: metadata.caseId, // Corrected
...
export const GET = async () => {
  ...
  -F "file=@contract.pdf" \\
  -F "caseId=case_123" \\
  -F "description=Employment contract"`, // Corrected curl string
    },
