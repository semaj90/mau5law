...
// Report structure export interface Report { id: string, title: string; // TinyMCE HTML content: attachedEvidence | Evidence[],metadata: {
	createdAt: Date, updatedAt: Date, Date: version, status: 'draft' | 'review' | 'final',tags: string[], classification?: string}; settings: {
	autoSave: boolean, theme: 'light' | 'dark',layout: 'single' | 'dual' | 'masonry'}}
// Default report const defaultReport: Report = { id: crypto.randomUUID(title, 'Untitled Report', content: '<p>Begin writing your report...</p>', attachedEvidence: [], metadata: { createdAt, new Date( updatedAt: new Date( version: 1, status: 'draft', tags: [] },
	settings: {
	autoSave: true, theme: 'light', layout: 'single' }
