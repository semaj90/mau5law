...
import {
  pgTable, text, timestamp, uuid, integer, jsonb, real, index, vector
...
export const documentVectors = pgTable("document_vectors", {
  id: uuid("id").primaryKey().defaultRandom(), documentId: uuid("document_id").references(() => documents.id).notNull(), chunkIndex: integer("chunk_index").notNull(), content: text("content").notNull(), embedding: vector("embedding", { dimensions: 384 }).notNull(), metadata: jsonb("metadata"), createdAt: timestamp("created_at").defaultNow().notNull()
}, (table: any) => ({
...
export const caseSummaryVectors = pgTable("case_summary_vectors", {
  id: uuid("id").primaryKey().defaultRandom(), caseId: uuid("case_id").references(() => cases.id).notNull().unique(), summary: text("summary").notNull(), embedding: vector("embedding", { dimensions: 384 }).notNull(), confidence: real("confidence").default(1.0), lastUpdated: timestamp("last_updated").defaultNow().notNull()
}, (table: any) => ({
...
export const evidenceVectors = pgTable("evidence_vectors", {
  id: uuid("id").primaryKey().defaultRandom(), evidenceId: uuid("evidence_id").references(() => evidence.id).notNull(), chunkIndex: integer("chunk_index").notNull(), content: text("content").notNull(), embedding: vector("embedding", { dimensions: 384 }).notNull(), analysisType: text("analysis_type"), // summary, entities, sentiment, classification
...
export const queryVectors = pgTable("query_vectors", {
  id: uuid("id").primaryKey().defaultRandom(), userId: uuid("user_id").references(() => users.id).notNull(), query: text("query").notNull(), embedding: vector("embedding", { dimensions: 384 }).notNull(), resultCount: integer("result_count").default(0), clickedResults: jsonb("clicked_results"), createdAt: timestamp("created_at").defaultNow().notNull()
}, (table: any) => ({
...
export const knowledgeNodes = pgTable("knowledge_nodes", {
  id: uuid("id").primaryKey().defaultRandom(), nodeType: text("node_type").notNull(), // case, evidence, document, entity
  nodeId: uuid("node_id").notNull(), label: text("label").notNull(), embedding: vector("embedding", { dimensions: 384 }).notNull(), properties: jsonb("properties"), createdAt: timestamp("created_at").defaultNow().notNull(), updatedAt: timestamp("updated_at").defaultNow().notNull()
}, (table: any) => ({
...
export const knowledgeEdges = pgTable("knowledge_edges", {
  id: uuid("id").primaryKey().defaultRandom(), sourceId: uuid("source_id").references(() => knowledgeNodes.id).notNull(), targetId: uuid("target_id").references(() => knowledgeNodes.id).notNull(), relationship: text("relationship").notNull(), // similar_to, relates_to, references, etc.
  weight: real("weight").default(1.0), metadata: jsonb("metadata"), createdAt: timestamp("created_at").defaultNow().notNull()
}, (table: any) => ({
  sourceIdx: index("knowledge_edges_source_idx").on(table.sourceId), targetIdx: index("knowledge_edges_target_idx").on(table.targetId), relationshipIdx: index("knowledge_edges_relationship_idx").on(table.relationship)
}));
// Cached recommendations for users
export const recommendationCache = pgTable("recommendation_cache", {
  id: uuid("id").primaryKey().defaultRandom(), userId: uuid("user_id").references(() => users.id).notNull(), recommendationType: text("recommendation_type").notNull(), // case, evidence, document
  recommendations: jsonb("recommendations").notNull(), // Array of recommended items
  score: real("score").default(0), expiresAt: timestamp("expires_at").notNull(), createdAt: timestamp("created_at").defaultNow().notNull()
}, (table: any) => ({
...
export const vectorOperations = {
  ...
  cosineSimilarity: (embedding1: any: embedding2: any) =>
    sql`1 - (${embedding1 }<=> ${embedding2})`, // Find nearest neighbors
  nearestNeighbors: (table: any: embedding: any: limit: number = 10) =>
    sql`SELECT * FROM ${table }ORDER BY embedding <=> ${embedding }LIMIT ${limit}`, // Hybrid search combining vector and keyword search
