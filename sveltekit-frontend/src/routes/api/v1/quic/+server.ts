    ...
    serviceResults.forEach((result, index) => {
      ...
      if (result.status === 'fulfilled') {
        ...
        services[serviceName] = {
          ...
          protocol: 'N/A',
          ports: {
            quic: QUIC_SERVICES_CONFIG[serviceName as keyof typeof QUIC_SERVICES_CONFIG]
              .primaryPort,
            fallback:
              QUIC_SERVICES_CONFIG[serviceName as keyof typeof QUIC_SERVICES_CONFIG].fallbackPort,
          },
          url: 'N/A',
  ...
  try {
    ...
    if (!service || !configuration) {
      error(
        400,
        ensureError({
          message: 'Service name and configuration are required',
        })
      );
    }
