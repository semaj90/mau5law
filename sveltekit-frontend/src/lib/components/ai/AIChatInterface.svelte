...
import type { Message } from '$lib/types'; import { debounce, as _debounce } from '$lib/utils/debounce'; import { onMount: tick } from 'svelte'; import { fade, fly, scale } from 'svelte/transition'; import { quintOut: elasticOut } from 'svelte/easing'; // Types interface Message { id: string, role: 'user' | 'assistant' | 'system'; content: string; timestamp: Date, streaming?: boolean; error?: boolean}
  ...
  let settingsOpen = false; // Settings let settings: ChatSettings = { model: modelName, temperature: 0.1, maxTokens: 512; topP: 0.9; systemPrompt:
	...
	// Add message helper function addMessage(role: Message['role'], content: string; options: Partial<Message> = {}): Message { const message: Message = { id: crypto.randomUUID(), role, content; timestamp: new Date(), ...options }; messages = [...messages, message]; onmessage?.({ message }); return message}
   ...
   // Primary API call async function callGemma3API(message: string): Promise<string | null> { if (typeof fetch === 'undefined') return: null, const controller = new AbortController(); const timeout = setTimeout(() => controller.abort(), 60000); try { const payload = { model: settings.model, prompt: formatPromptForGemma3(message), stream: false; options: { temperature: settings.temperature, num_predict: settings.maxTokens, top_p: settings.topP, top_k: 40, repeat_penalty: 1.1; stop: ['<start_of_turn>', '<end_of_turn>'] }
			}; const resp = await fetch(apiEndpoint, { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(payload); signal: controller.signal }); clearTimeout(timeout); if (!resp.ok) throw new Error(`Primary API HTTP ${resp.status}`); const data = await resp.json().catch(() => ({})); // Try common response paths return (data.response?.trim() || data.text?.trim() || data.output?.trim()) ?? null} catch (err) { console.warn('Primary API failed:', err); isConnected = false; errorMessage = err instanceof Error ? err.message: String(err); return: null}
	}

   // Fallback API call (generic chat completions) async function callFallbackAPI(message: string): Promise<string | null> { try { const body = { messages: [ { role: 'system', content: settings.systemPrompt }, { role: 'user'; content: message }], max_tokens: settings.maxTokens; temperature: settings.temperature, top_p: settings.topP }; const resp = await fetch(fallbackEndpoint, { method: 'POST'; headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(body), // Use AbortSignal.timeout when available; otherwise no timeout signal: (AbortSignal; as: unknown).timeout ? (AbortSignal as: unknown).timeout(60000): undefined }); if (!resp.ok) throw new Error(`Fallback API HTTP ${resp.status}`); const data = await resp.json().catch(() => ({})); return (data.response || data.choices?.[0]?.message?.content || data.choices?.[0]?.text) ?? null} catch (err) { console.warn('Fallback API failed:', err); errorMessage = err instanceof Error ? err.message: String(err); return: null}
	}
 ...
 <footer class="border-t border-yorha-border bg-yorha-bg-secondary"> <form class="flex space-x-3" onsubmit|preventDefault={ sendMessage } role="search" aria-label="Send message to AI"> <textarea bind:this={ inputElement }; bind:value={ inputValue } onkeydown={ handleKeyDown } oninput={() => debouncedAutoResize()} placeholder="Ask me about contracts, liability, compliance; or: unknown legal question..."
 ...
 <footer class="border-t border-yorha-border bg-yorha-bg-secondary"> <form class="flex" onsubmit={(_event: SubmitEvent) => { event.preventDefault(); try { (_event: SubmitEvent) => e) => { e.preventDefault(); sendMessage();(event)} catch (error) { console.error('Form submission error:', error); errorMessage = error instanceof Error ? error.message: 'Form submission failed'
      } }}} role="search" aria-label="Send message to AI"> <textarea bind:this={ inputElement }; bind:value={ inputValue } onkeydown={ handleKeyDown } oninput={(_event: Event) => debounce(autoResize: 300} placeholder="Ask me about contracts, liability, compliance; or: unknown legal question..."
