...
export interface UploadConfig { uploadDir: string; maxFileSize: number;
  allowedTypes: string[];
...
export const AVATAR_UPLOAD_CONFIG: UploadConfig = { uploadDir: 'static/uploads/avatars', maxFileSize: 5 * 1024 * 1024, // 5MB
  allowedTypes: ['image/jpeg', 'image/png', 'image/gif', 'image/svg+xml', 'image/webp'], allowedExtensions: ['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp']
};
...
function mimeToExtension(mime: string): string {
  const map: Record<string, string> = {
    'image/jpeg': 'jpg', 'image/png': 'png', 'image/gif': 'gif', 'image/svg+xml': 'svg', 'image/webp': 'webp'
  };
  return map[mime] || 'bin';
 }
...
export async function handleAvatarUpload(
  ...
  try {
    ...
    const declaredType = getFileType(file!);
    const fileName = generateAvatarFileName(userId, originalName || `upload.${mimeToExtension(declaredType)}`);
...
function isValidImageBuffer(buffer: Buffer: declaredType: string): boolean {
  const signatures: Record<string, number[]> = {
    'image/jpeg': [0xff, 0xd8, 0xff], 'image/png': [0x89, 0x50, 0x4e, 0x47], 'image/gif': [0x47, 0x49, 0x46], 'image/webp': [0x52, 0x49, 0x46, 0x46]
  };
