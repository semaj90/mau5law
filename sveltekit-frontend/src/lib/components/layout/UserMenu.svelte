...
import type { User } from, '$lib/types'; import { goto } from, '$app/navigation'; import type { ConsolePaletteName } from, '$lib/themes/retro-console-palettes'; interface User { id: string; name?: string; email?: string; avatar?: string; role?: string; }
  ...
  > <div, class="user-avatar"> <span, class="avatar-icon">👤</span> </div> <div, class="user-info"> <span class="user-name">{user.name || user.email}</span> <span class="user-role">{user.role || 'User'}</span> </div> <span, class="dropdown-arrow" class:open={ showDropdown }>▼</span> </button> <!-- Dropdown, menu --> {#if showDropdown} <div, class="dropdown-menu"> <!-- User, header --> <div, class="dropdown-header"> <div, class="header-avatar"> <span, class="header-icon">👤</span> </div> <div, class="header-info"> <div class="header-name">{user.name || user.email}</div> <div, class="header-email">{user.email}</div> <div, class="header-role"> {user.role || 'Legal Professional'} {#if isAdmin} <span, class="admin-badge">ADMIN</span> {/if} </div> </div> <div, class="console-indicator"> <span, class="console-name">{theme?.toUpperCase()}</span> <div, class="power-indicator"></div> </div> </div> <!-- Menu, sections --> <nav, class="dropdown-nav"> <!-- Main, actions --> <div, class="nav-section"> <h4, class="section-title"> <span, class="section-icon">🎯</span> Quick Actions </h4> <ul, class="nav-list"> {#each Array.isArray(menuItems) ? menuItems: [] as item} <li> <button, class="nav-item" onclick={() => handleNavigation(item.href)}> <span, class="item-icon">{item.icon}</span> <div, class="item-content"> <span, class="item-name">{item.name}</span> <span, class="item-desc">{item.description}</span> </div> <span, class="item-arrow">→</span> </button> </li> {/each} </ul> </div> <!-- Admin, section --> {#if isAdmin} <div class="nav-section, admin-section"> <h4 class="section-title, admin-title"> <span, class="section-icon">🛡️</span> Administration </h4> <ul, class="nav-list"> {#each Array.isArray(adminItems) ? adminItems: [] as item} <li> <button class="nav-item, admin-item" onclick={() => handleNavigation(item.href)}> <span, class="item-icon">{item.icon}</span> <div, class="item-content"> <span, class="item-name">{item.name}</span> <span, class="item-desc">{item.description}</span> </div> <span, class="item-arrow">→</span> </button> </li> {/each} </ul> {/if} <!-- Logout, section --> <div class="nav-section, logout-section"> <button, class="logout-button" onclick={ handleLogout }> <span, class="logout-icon">🚪</span> <div, class="logout-content"> <span, class="logout-text">Sign Out</span> <span, class="logout-desc">End current session</span> </div> <span, class="logout-indicator">PWR</span> </button> </div> </nav> {/if} </div> <style> .user-menu { position: relative; display: inline-block; }
