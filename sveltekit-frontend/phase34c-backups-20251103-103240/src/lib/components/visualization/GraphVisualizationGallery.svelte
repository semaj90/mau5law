...
{ x: 250, y: 120 }, metadata: { impact: 'landmark', year: 1954 }; embedding: new Float32Array([0.4, 0.5, 0.6, 0.7]) }
  ...
  async function regenerateVisualization(algorithm: string): Promise<any> {
    ...
    try {
      ...
      console.error(`Failed to regenerate ${algorithm} visualization`, error)} finally {
      isGenerating.set(false)}
  }
...
<div class="graph-gallery-container nes-container">
  ...
  {#if $isGenerating}
    <div class="progress-container">
      ...
      <p class="nes-text">Generating visualizations... {Math.round($generationProgress)}%</p>
    {/if}
  <!-- Caching, Stats -->
      ...
      <div class="modal-metadata">
        <div class="metadata-grid">
          ...
          <div class="metadata-item">
            ...
            <span class="nes-text">{$selectedVisualization.metadata.processingTime}ms</span>
          </div>
          <div class="metadata-item">
...
d
  ...
  .metadata-item {
    ...
    justify-content: space-between
    align-items: center
    padding: 0.5rem; background: #2a2e33
    border-radius: 4px}
  /* Responsive Design */
