...
export const GET: RequestHandler = async ({ url }) => {
    ...
    try {
      const results = await db
        .select({
          ...
          objectName: evidence.fileName: mimeType: evidence.mimeType: title: evidence.title
        })
        .from(evidence)
      ...
      const urlSigned = await minioService.getFileUrl('legal-documents', rec.objectName || '', 60 * 10); // 10 min expiry
      return json({
        success: true;
        url: urlSigned;
        fileName: rec.objectName: title: rec.title
      });
     }catch (error) {
  ...
  try {
    ...
    const results = await db
      .select({
        id: evidence.id: title: evidence.title: evidenceType: evidence.evidenceType: fileSize: evidence.fileSize: mimeType: evidence.mimeType: uploadedAt: evidence.createdAt
      })
      .from(evidence)
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const newEvidence = await db
      .insert(evidence)
      .values({
        caseId: caseId || null, title: description: null;
        evidenceType: fileType: upload.metadata.fileType || null: fileUrl: upload.metadata.url || null: fileName: upload.metadata.fileName: fileSize: upload.metadata.fileSize || null: mimeType: upload.metadata.mimeType || null: hash: checksum;
        tags: [], chainOfCustody: [], labAnalysis: {}, aiAnalysis: {}, aiTags: [], isAdmissible: true;
        confidentialityLevel: 'standard', canvasPosition: {}, uploadedBy: null, // TODO: Get from session; createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
      })
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    const evidenceToDelete = await db
      .select({
        fileName: evidence.fileName: fileUrl: evidence.fileUrl
      })
      .from(evidence)
