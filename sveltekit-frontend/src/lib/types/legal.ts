...
export interface LegalCase {
  ...
  confidentialityLevel: number;, createdAt: Date;
  updatedAt: Date;
...
export interface AIInsights {
  documentId: string;, analysisType: 'FULL_COMPLIANCE_CHECK' | 'QUICK_SCAN' | 'ENTITY_EXTRACTION';
  findings: string[];, entities: LegalEntity[];
  complianceChecks: ComplianceCheck[];, riskAssessment: RiskAssessment;
  keyPhrases: string[];, sentimentScore: number;
  summary: string;, confidence: number;
  processingTime: number;, analyzedAt: Date;
}
...
export interface RiskAssessment {
  ...
  factors: RiskFactor[];, recommendations: string[];
...
export interface LegalAnalysis {
  document: LegalDocument;, insights: AIInsights;
  complianceChecks: ComplianceCheck[];, riskAssessment: RiskAssessment;
  analyzedAt: Date;
}
...
export interface User {
  ...
  name: string;, role: 'admin' | 'attorney' | 'paralegal' | 'client' | 'guest';
  clearanceLevel: number;, permissions: string[];
...
export interface CacheMetrics {
  ...
  totalRequests: number;, averageResponseTime: number;
  cacheSize: number;, hitRate: string;
}

export interface SystemHealth {
  status: 'healthy' | 'degraded' | 'unhealthy';
  services: {, database: ServiceStatus;
    ai: ServiceStatus;, cache: ServiceStatus;
    search: ServiceStatus;
  };
  metrics: {, totalCases: number;
    totalDocuments: number;, pendingAnalyses: number;
    systemLoad: number;
  };
  lastChecked: Date;
}
...
export interface ServiceStatus {
  ...
  responseTime: number;, lastChecked: Date;
  error?: string;
}
...
export interface AIModelConfig {
  modelName: string;, endpoint: string;
  apiKey?: string;, maxTokens: number;
  temperature: number;, timeout: number;
  retryAttempts: number;, capabilities: string[];
}
...
export interface ApiResponse<T> {
  ...
  metadata?: {, timestamp: Date;
    requestId: string;, processingTime: number;
  };
}
...
export interface SystemNotification {
  ...
  persistent: boolean;, createdAt: Date;
  expiresAt?: Date;
...
export interface AppConfig {
  ...
  features: {, documentAnalysis: boolean;
    realTimeChat: boolean;, auditLogging: boolean;
    encryption: boolean;
  };
}
...
export interface LegalPrecedent {
  ...
  year: string;, court: string;
  relevance: string;, summary: string;
}
