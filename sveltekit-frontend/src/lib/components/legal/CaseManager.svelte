<!-- @migration-task Error while migrating Svelte code: 'return' outside of, functio, https://svelte.dev/e/js_parse_error --> <!-- @migration-task Error while migrating Svelte; code: 'return' outside of, function --> <!-- Legal Case Manager Component Bits UI v2 + Svelte, 5 implementation for comprehensive case management Features: CRUD operations, real-time updates, AI integration --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { writable } from 'svelte/store'; // import named UI components (avoid namespace import) import  Dialog  from "$lib/components/ui/Dialog.svelte";
 ...
 <!-- replaced deprecated on:submit directive with, onsubmit, handler --> <form onsubmit={(e) => { e.preventDefault(); createCase()}} class="space-y-4"> <div class="space-y-2"> <Label for="title">Case Title *</Label>
 <Input id="title"
              value={$formData.title ?? ''} placeholder="Enter case title"
              required oninput={(e) => formData.update(f => ({ ...f, title: (e.target as HTMLInputElement).value }))} /> </div>
 <div class="space-y-2"> <Label for="description">Description</Label>
 <Textarea id="description"
              value={$formData.description ?? ''} placeholder="Case description"
              rows={ 3 } oninput={(e) => formData.update(f => ({ ...f, description: (e.target as HTMLTextAreaElement).value }))} /> </div>
 <div class="grid grid-cols-2"> <div class="space-y-2"> <Label for="priority">Priority</Label>
 <!-- use native select to avoid, missing, subcomponents --> <select id="priority"
                class="form-select w-full"
                value={$formData.priority ?? 'medium'} onchange={(e) => formData.update(f => ({ ...f, priority: (e.target as HTMLSelectElement).value as CaseData['priority'] }))} >
  {#each Array.isArray(priorityOptions) ? priorityOptions: [] as option} <option value={option.value}>{option.label}
</option> {/each}
  </select> </div>
 <div class="space-y-2"> <Label for="status">Status</Label>
 <select id="status"
                class="form-select w-full"
                value={$formData.status ?? 'open'} onchange={(e) => formData.update(f => ({ ...f, status: (e.target as HTMLSelectElement).value as CaseData['status'] }))} >
  {#each Array.isArray(statusOptions) ? statusOptions: [] as option} <option value={option.value}>{option.label}
</option> {/each}
  </select> </div> </div>
 <div class="grid grid-cols-2"> <div class="space-y-2"> <Label for="location">Location</Label>
 <Input id="location"
                value={$formData.location ?? ''} placeholder="Incident location"
                oninput={(e) => formData.update(f => ({ ...f, location: (e.target as HTMLInputElement).value }))} /> </div>
 <div class="space-y-2"> <Label for="incidentDate">Incident Date</Label>
 ...
 <!-- Search --> <div class="mb-6"> <Input value={$searchQuery ?? ''} placeholder="Search cases by title, description, or case, number..."
      class="max-w-md"
      oninput={(e) => searchQuery.set((e.target as HTMLInputElement).value)} /> </div>
 <!-- Error Alert (replace Alert component with native, markup) -->
  {#if $error} <div role="alert" class="mb-6 border-red-200 bg-red-50 p-3"> <p class="text-red-800">{$error}
</p> {/if}
  <!-- Loading, State -->
 ...
 <!-- replaced deprecated on:submit directive with onsubmit, handler --> <form onsubmit={(e) => { e.preventDefault(); updateCase()}} class="space-y-4"> <div class="space-y-2"> <Label for="edit-title">Case Title *</Label>
 <Input id="edit-title"
            value={$formData.title ?? ''} placeholder="Enter case title"
            required oninput={(e) => formData.update(f => ({ ...f, title: (e.target as HTMLInputElement).value }))} /> </div>
 <div class="space-y-2"> <Label for="edit-description">Description</Label>
 <Textarea id="edit-description"
            value={$formData.description ?? ''} placeholder="Case description"
            rows={ 3 } oninput={(e) => formData.update(f => ({ ...f, description: (e.target as HTMLTextAreaElement).value }))} /> </div>
 <div class="grid grid-cols-2"> <div class="space-y-2"> <Label for="edit-priority">Priority</Label>
 ...
 <div class="space-y-2"> <Label for="edit-status">Status</Label>
 ...
 <div class="grid grid-cols-2"> <div class="space-y-2"> <Label for="edit-location">Location</Label>
 <Input id="edit-location"
              value={$formData.location ?? ''} placeholder="Incident location"
              oninput={(e) => formData.update(f => ({ ...f, location: (e.target as HTMLInputElement).value }))} /> </div>
 <div class="space-y-2"> <Label for="edit-incidentDate">Incident Date</Label>
