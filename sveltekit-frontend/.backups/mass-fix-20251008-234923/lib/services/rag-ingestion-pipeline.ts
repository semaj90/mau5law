...
export const ragIngestionMachine = setup({
  ...
  actors: {
    ...
    performOCR: fromPromise(async ({ input }: { input: { document: RAGDocument; file: File | Buffer; options?: any } }) => {
      const startTime = Date.now();

      // Check if OCR is needed
      if (input.document.file_type === 'txt') {
        // No OCR needed for text files
        const text = input.file instanceof File
          ? await input.file.text()
          : input.file.toString('utf-8');
    ...
    chunkDocument: fromPromise(async ({ input }: { input: { document: RAGDocument; content: string; options?: any } }) => {
    ...
    generateEmbeddings: fromPromise(async ({ input }: { input: { chunks: RAGChunk[]; document: RAGDocument; options?: any } }) => {
...
export class RAGIngestionService {
  ...
  async batchIngest(inputs: IngestionInput[]): Promise<{
    ...
    const results = await Promise.all(
      inputs.map(input => this.ingestDocument(input))
    );
