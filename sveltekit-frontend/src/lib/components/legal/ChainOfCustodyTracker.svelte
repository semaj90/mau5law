...
<script lang="ts">
  ...
  const categoryConfig = {
    physical: { icon: '🛡️', color: 'text-blue-400', bg: 'bg-blue-500/10' },
    digital: { icon: '💾', color: 'text-green-400', bg: 'bg-green-500/10' },
    document: { icon: '📄', color: 'text-yellow-400', bg: 'bg-yellow-500/10' },
    biological: { icon: '🧪', color: 'text-red-400', bg: 'bg-red-500/10' },
    chemical: { icon: '⚗️', color: 'text-purple-400', bg: 'bg-purple-500/10' },
    other: { icon: '📦', color: 'text-gray-400', bg: 'bg-gray-500/10' }
  };
  ...
  >
    <div class="flex items-start justify-between mb-3">
      <div class="flex items-center gap-3">
        <!-- derive config locally to avoid {@const} placement issues -->
        {@html ''}
        <div class={cn('p-2 rounded-md', categoryConfig[evidence?.category ?? 'other'].bg)}>
          <span class="w-5 h-5 block" aria-hidden="true">
            {categoryConfig[evidence?.category ?? 'other'].icon}
          </span>
        </div>
        <div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm font-mono">
      ...
      <div>
        ...
        <span class={cn('inline-block px-2 py-0.5 rounded text-xs', conditionConfig[evidence?.condition ?? 'good'].class)}>
          {conditionConfig[evidence?.condition ?? 'good'].label}
        </span>
      </div>
      <div>
  ...
  <div class="bg-yorha-bg-secondary border border-yorha-border rounded-lg p-4">
    ...
    {#if (evidence?.chainOfCustody?.length ?? 0) === 0}
      <div class="text-center py-8 text-yorha-text-secondary font-mono">
        <span class="w-12 h-12 mx-auto mb-4 opacity-50 block">🛡️</span>
    ...
    {:else}
      ...
      <div class="relative">
        ...
        <div class="space-y-4">
          {#each (showFullHistory ? sortedTransfers : sortedTransfers.slice(0, 3)) as transfer (transfer.id)}
            <button
              type="button"
              aria-label={`View details for transfer at ${formatDateTime(transfer.timestamp)}`}
              class={cn(
                    ...
                    <div>
                      ...
                      <span
                        class={cn(
                          ...
                          conditionConfig[transfer.condition ?? 'good'].class
                        )}
                      >
