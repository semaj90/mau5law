...
import type { Document } from '$lib/types'; // WebGPU-accelerated canvas for high-performance legal data visualization import { onMount } from 'svelte'; import type { Snippet } from 'svelte'; interface WebGPUCanvasProps { width?: number; height?: number; enableWebGPU?: boolean; fallbackTo2D?: boolean; onWebGPUStatus?: (supported: boolean, device?: GPUDevice) => void; children?: Snippet; }
  ...
  // Set canvas size $effect(() => { if (canvas) { canvas.width = width; canvas.height = height; canvas.style.width = `${ width }px`; canvas.style.height = `${ height }px`; }
      ...
      2D Fallback </button> <button class="nes-btn" onclick={() => location.reload()}> Reset Canvas </button> </div> {#if children} <div class="additional-content"> {@render children()} {/if} </div> <style> .webgpu-canvas-container { margin: 1rem; padding: 1rem;, background: var(--yorha-bg-secondary); border: 2px solid var(--yorha-text-muted); }
