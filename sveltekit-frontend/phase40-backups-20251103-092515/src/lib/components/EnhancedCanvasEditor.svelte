...
<script lang="ts">
    ...
    lokiDb = new Loki(`canvas_${caseId}.db`, {
      ...
      autoloadCallback: () => {
        canvasCollection =
          ...
          lokiDb?.addCollection('canvas_data', { indices: ['id', 'caseId', 'timestamp', 'type'] });
      },
      autosave: true,
  ...
  function createThumbnail(evidence: any): any | null {
    ...
    if (fileType.startsWith('image/')) {
      ...
      return new (fabric as any).Text('PDF', {
        fontSize: 12,
        fontWeight: 'bold',
        top: 100,
        left: 140,
      ...
      return new (fabric as any).Text('VIDEO', {
        fontSize: 10,
        fontWeight: 'bold',
        top: 100,
        left: 140,
  ...
  function createTimelineVisualization(): any {
    ...
    for (let i = 0; i <= 4; i++) {
      ...
      const date = new (fabric as any).Text(`Event ${i + 1}`, {
        left: i * 100 - 25,
        top: 15,
        fontSize: 10,
        fill: '#374151',
  ...
  function createPersonVisualization(): any {
    ...
    const name = new (fabric as any).Text('Person Name', {
      fontSize: 12,
      fontWeight: 'bold',
      top: 40,
      left: -30,
    ...
    const role = new (fabric as any).Text('Role/Title', {
      fontSize: 10,
      top: 55,
      left: -25,
      fill: '#6b7280',
