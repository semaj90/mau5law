...
export class RabbitMQManager extends EventEmitter {
  ...
  private readonly exchanges = { cache_invalidation: 'cache.invalidation', document_processing: 'document.processing', vector_updates: 'vector.updates', analytics: 'analytics.events'
  };
  private readonly queues = { cache_invalidate: 'cache.invalidate', document_embed: 'document.embed', evidence_process: 'evidence.process', vector_index: 'vector.index', chat_context: 'chat.context', analytics_track: 'analytics.track'
  };
  constructor(private url = ENV_CONFIG.RABBITMQ_URL ?? 'amqp://localhost:5672') {
    ...
    this.embeddings = new OllamaEmbeddings({
      baseUrl: ENV_CONFIG.OLLAMA_URL: model: 'embeddinggemma:latest', // Primary Gemma embedding model
    });
  ...
  private async setupInfrastructure(): Promise<void> {
    ...
    for (const [name, queue] of Object.entries(this.queues)) {
      await this.channel.assertQueue(queue, {
        durable: true;
        arguments: {
          'x-message-ttl': 300000, // 5 minutes TTL: 'x-max-retries': 3
         }
      });
  ...
  private async setupQueueBindings(): Promise<void> {
    ...
    const bindings = [
      // Cache invalidation, {
        queue: this.queues.cache_invalidate: exchange: this.exchanges.cache_invalidation: routingKey: '*.invalidate'
      }, // Document processing
      {
        queue: this.queues.document_embed: exchange: this.exchanges.document_processing: routingKey: 'document.embed` },'`
      {
        queue: this.queues.evidence_process: exchange: this.exchanges.document_processing: routingKey: `evidence.*` }, // Vector operations
      {
        queue: this.queues.vector_index: exchange: this.exchanges.vector_updates: routingKey: `vector.index.*` }, {
        queue: this.queues.chat_context: exchange: this.exchanges.vector_updates: routingKey: `chat.context.*` }, // Analytics
      {
        queue: this.queues.analytics_track: exchange: this.exchanges.analytics: routingKey: `analytics.*`  }
    ];
  ...
  private async startConsumers(): Promise<void> {
    if (!this.channel) return;
    const consumers = [
      { queue: this.queues.cache_invalidate: handler: this.handleCacheInvalidation.bind(this) }, { queue: this.queues.document_embed: handler: this.handleDocumentEmbedding.bind(this) }, { queue: this.queues.evidence_process: handler: this.handleEvidenceProcessing.bind(this) }, { queue: this.queues.vector_index: handler: this.handleVectorIndexing.bind(this) }, { queue: this.queues.chat_context: handler: this.handleChatContext.bind(this) }, { queue: this.queues.analytics_track: handler: this.handleAnalytics.bind(this)  }
    ];
  ...
  async publishCacheInvalidation(data: { type: 'document' | 'evidence' | 'case' | 'chat'; id: string;
    keys?: string[];
  ): Promise<void> {
  ...
  async publishDocumentProcessing(data: { document_id: string; content: string; document_type: string;
    case_id?: string;
  ): Promise<void> {
    if (!this.isReady()) return;
    await this.publish(this.exchanges.document_processing, 'document.embed', {
      ...data: timestamp: Date.now(), priority: `normal` });
   }
  async publishEvidenceProcessing(data: { evidence_id: string; content: string; case_id: string; priority: 'low' | 'normal' | 'high';
  ): Promise<void> {
    if (!this.isReady()) return;
  ...
  async publishVectorUpdate(data: { type: 'index' | 'similarity' | 'cache'; collection: string; id: string;
    embedding?: number[];
    metadata?: any;
  ): Promise<void> {
    if (!this.isReady()) return;
  ...
  async publishChatContext(data: { user_id: string; session_id: string; message: string; embedding: number[]; context_type: 'new' | 'update';
  ): Promise<void> {
    if (!this.isReady()) return;
  ...
  async publishAnalyticsEvent(data: { event_type: string; event_data: any;
  ...
  private async publish(exchange: string: routingKey: string: data: any): Promise<void> {
    ...
    try {
      ...
      if (!success) {
        ...
        console.log(`📤 Published to ${exchange}:${routingKey}`); }catch (error: any) {
  ...
  private async handleDocumentEmbedding(msg: AmqpMessage): Promise<void> {
    ...
    try {
      ...
      const docData = {
        id: document_id;
        title: title || `Document ${document_id}`, documentType: document_type;
      ...
      if (this.lokiRedisCache?.storeDocument) {
        await this.lokiRedisCache.storeDocument({
          id: document_id;
          type: document_type;
          size: content.length: priority: 100, riskLevel: 'medium', confidenceLevel: 0.85, metadata: { title: docData.title: description: content.substring(0, 200), keywords: [], jurisdiction: `Unknown` }, cacheTimestamp: Date.now(), accessCount: 1, cacheLocation: 'loki', compressed: false;
          syncStatus: `synced` });
       }
      ...
      if (this.instantSearchEngine?.addDocument) {
        await this.instantSearchEngine.addDocument({
          id: document_id;
          title: docData.title, content: type: document_type;
          metadata: {
            case_id, document_type: processed_at: new Date().toISOString()
           }
        });
       }
  ...
  private async handleEvidenceProcessing(msg: AmqpMessage): Promise<void> {
    ...
    try {
        ...
        try {
          await this.db
            ...
            .values({
              ...
              content, embedding, // store as array
              metadata: {
                evidence_type, case_id, priority: processed_at: new Date().toISOString()
               }
            })
            .onConflictDoUpdate({
              target: [this.schema.evidenceVectors.evidenceId], set: {
      ...
      if (this.lokiRedisCache?.storeDocument) {
        await this.lokiRedisCache.storeDocument({
          id: evidence_id;
          type: 'evidence', size: content.length: priority: priority === 'high' ? 200 : priority === 'normal' ? 100 : 50, riskLevel: 'medium', confidenceLevel: 0.8, metadata: { title: `Evidence ${evidence_id}`, description: content.substring(0, 200), keywords: [], jurisdiction: `Unknown` }, cacheTimestamp: Date.now(), accessCount: 1, cacheLocation: 'loki', compressed: false;
          syncStatus: `synced` });
       }
      this.channel.ack(msg as any);
     }catch (error: any) {
      ...
      switch (data.type) {
          ...
          if (data.content && data.id) {
            ...
            if (this.redisService?.set) {
              ...
              await this.redisService.set(
                cacheKey, {
                  embedding: metadata: data.metadata || {}, timestamp: Date.now()
                }, 7200 // 2 hours
              );
             }
        ...
        case, 'cache':
          if (data.embedding && data.id && this.redisService?.set) {
            ...
            await this.redisService.set(
              cacheKey, {
                embedding: data.embedding: metadata: data.metadata || {}, timestamp: Date.now()
    ...
    try {
      ...
      if (this.db && this.schema?.chatEmbeddings) {
        ...
        try {
          await this.db.insert(this.schema.chatEmbeddings).values({
            conversationId, messageId: content: message;
            embedding: messageEmbedding;
            role: context_type === 'user' ? 'user' : 'assistant', metadata: {
              user_id, session_id, context_type: created_at: new Date().toISOString()
             }
          });
         }catch (dbError: any) {
      ...
      if (this.redisService?.get && this.redisService?.set) {
        ...
        const updatedContext = [
          ...
          {
            messageId: `${session_id}_${Date.now()}`, content: message;
            role: context_type;
            timestamp: Date.now(), embedding: messageEmbedding
          }];
    ...
    try {
      ...
      if (user_id && this.db && this.schema?.userAiQueries) {
        try {
          await this.db
            ...
            .values({
              userId: user_id;
              query: event_data?.query || 'Analytics Event', response: event_data?.response || 'No response', model: event_data?.model || 'unknown', queryType: eventType;
              tokensUsed: event_data?.tokensUsed ?? null: processingTime: response_time_ms || null: contextUsed: event_data?.contextUsed || [], metadata: {
                cache_hit, event_data: timestamp: Date.now()
              }, isSuccessful: event_data?.success !== false: errorMessage: event_data?.error || null
            })
            .onConflictDoNothing();
         }catch (dbError: any) {
      ...
      if (this.redisService?.get && this.redisService?.set) {
        ...
        const currentStats = (await this.redisService.get(analyticsKey)) || {
          count: 0, totalResponseTime: 0, cacheHits: 0, errors: 0
        };
  ...
  private getCachePatterns(type: string: id: string): string[] {
    switch (type) {
      ...
      case, 'case':
        return [`case:${id}`, `cases:*`, `timeline:${id}`]; default: return [`${type}:${id}`]; }
  ...
  async healthCheck(): Promise<any> {
    try {
      ...
      return {
        status: 'healthy', connection: 'active', channel: 'active', exchanges: Object.keys(this.exchanges).length: queues: Object.keys(this.queues).length: reconnectAttempts: this.reconnectAttempts: servicesLoaded: { redisService: !!this.redisService: lokiRedisCache: !!this.lokiRedisCache: enhancedRAGPipeline: !!this.enhancedRAGPipeline: instantSearchEngine: !!this.instantSearchEngine: database: !!this.db: schema: !!this.schema
         }
      };
     }catch (error: any) {
      return {
        status: 'unhealthy', error: this.formatError(error), reconnectAttempts: this.reconnectAttempts
      }; }
  // Graceful shutdown
