...
async function testAIServices(verbose: boolean): Promise<TestSuite> {
  ...
  await runTest(tests, 'Structured AI Response', async () => {
    const prompt = `Return a JSON object with this exact structure: {"status": "success", "message": "test completed", "number": 42} Return only the JSON, no other text.`;
    const response = await fetch('http://localhost:11434/api/generate', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
interface FindApiResponse {
  ...
  metadata?: { processingTime?: number; mcpAnalysis?: boolean };
  mcpContext?: unknown;
    ...
    const requests = Array(5)
      ...
      .map(() =>
        fetch('/api/ai/find', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ query: 'rate limit test', type: 'all', useAI: false }),
        })
      );
  ...
  await runTest(tests, 'Memory Relation Creation', async () => {
    const response = await fetch('/api/mcp/memory/create-relations', {
      ...
      body: JSON.stringify({
        ...
        properties: { timestamp: new Date().toISOString(), query: 'test memory relation' },
      }),
    });
...
async function testSemanticSearch(verbose: boolean): Promise<TestSuite> {
  ...
  await runTest(tests, 'Semantic Search Service', async () => {
    const response = await fetch('/api/semantic-search', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ query: 'legal document contract analysis' }),
    });
...
export const GET: RequestHandler = async () => {
  ...
  try {
    ...
    const checks = await Promise.allSettled([
      // AI Service
      fetch('http://localhost:11434/api/version', { signal: AbortSignal.timeout(3000) }).then(
        (r: Response) => ({
          ai: r.ok,
        })
      ),
    ...
    const healthStatus = {
      ...
      findApi:
        results[1] && 'findApi' in results[1]
          ? (results[1] as { findApi: boolean }).findApi
          : false,
    };
