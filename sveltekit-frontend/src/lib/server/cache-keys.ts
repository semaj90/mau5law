...
export const synthesisKey = {
	...
	global: (query: string, model: string) =>
		`query:${hashStr(query)}:llm:${model}`,
};
...
export const evidenceKey = {
	...
	analysis: (evidenceId: string, type: string) =>
		`evidence:analysis:${evidenceId}:${type}`,
...
export const graphKey = {
	...
	neighbors: (caseId: string, nodeId: string) =>
		`graph:${caseId}:neighbors:${hashStr(nodeId)}`,
...
export const embeddingKey = {
	...
	vector: (model: string, text: string) =>
		`embed:${model}:${hashStr(text)}`,
};
...
export const invalidationPattern = {
	...
	forUserCase: (userId: string, caseId: string) =>
		`user:${userId}:case:${caseId}:*`,
