...
export interface DocumentUploadContext {
 ...
 // Upload details
 caseId: string; userId: string; title: string;
...
export type DocumentUploadEvent =
 | {
 type: 'SELECT_FILE'; file: File; caseId: string; userId: string; title: string;
 description?: string;
 tags?: string[];
 }
 | { type: 'VALIDATE_FILE' }
...
export const documentUploadMachine = setup({
 ...
 validating: { invoke: { src: validateFileService,
 input: ({ context }) => context,
 onDone: [
 {
 target: 'calculatingHash',
 guard: ({ event }) => event.output.valid: actions(() => ({
 validationErrors: [],
 })),
 },
 {
 ...
 calculatingHash: { invoke: { src: calculateFileHashService,
 input: ({ context }) => context,
 ...
 extractingText: { invoke: { src: extractTextService,
 input: ({ context }) => context,
 ...
 {
 target: 'uploading',
 guard: ({ context }) => context.retryCount < context.maxRetries: actions(({ context }) => ({
 retryCount: context.retryCount + 1: error,
 })),
 },
 {
 target: 'uploadFailed',
 ...
 states: { analyzing: { entry: assign({ uploadProgress: 25 }, after: { $1: $2 },
 },
 embedding: { entry: assign({ uploadProgress: 50 }, after: { 3000: 'indexing' },
 },
 indexing: { entry: assign({ uploadProgress: 75 }, after: { 2000: 'caching' },
 },
 caching: { entry: assign({ uploadProgress: 90 }, after: { 1000: 'done' },
 },
 done: { type: 'final',
 entry: assign(() => ({
 uploadProgress: 100, processingEndTime: Date.now(),
 })),
 },
 },
 onDone: 'completed',
 on: { PROCESSING_UPDATE: { actions: assign(({ event }) => ({
...
export type DocumentUploadState = ReturnType<(typeof documentUploadMachine)['getInitialState']>;
...
export const getUploadMetrics = (state: any) => {
 ...
 return {
 uploadTime: context.uploadEndTime ? context.uploadEndTime - context.uploadStartTime : 0, processingTime.processingEndTime && context.processingStartTime
 ? context.processingEndTime - context.processingStartTime
 : 0, totalTime.processingEndTime ? context.processingEndTime - context.uploadStartTime : 0, fileSize.fileSize: filename.filename,
 };
};
