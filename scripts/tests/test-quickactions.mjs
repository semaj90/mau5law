...
const SCREENSHOT_PATH = join(SCRIPT_DIR, 'command-center-step4.png');
...
try {
  ...
  const quickActionsExists = await page.evaluate(() => {
    ...
    return {
      ...
      labels: Array.from(buttons).map((b) => b.querySelector('.action-label')?.textContent?.trim()),
    };
  });

  if (quickActionsExists.error) throw new Error(quickActionsExists.error);
