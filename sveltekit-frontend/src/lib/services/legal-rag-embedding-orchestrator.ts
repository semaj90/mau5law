...
export interface EmbeddingsBundle { case_context: number[]; legal_domain: number[];
  jurisdictional: number[];
  temporal: number[];
  models_used: string[];
 }

export interface VectorSearchResultItem { id: string; content: string;
...
export interface LegalCase { id: string; title: string;
  ...
  evidence_ids: string[];
  document_ids: string[];
  metadata: { complexity_score: number; precedent_value: number;
...
export interface LegalLaw { id: string; title: string;
  jurisdiction: string;
  ...
  metadata: { complexity_level: 'basic' | 'intermediate' | 'advanced' | 'expert'; applicability_score: number;
    controversy_level: number;
    enforcement_frequency: number;
  };
 }

export interface RAGContext { case_id: string; query_type: 'legal_research' | 'precedent_search' | 'statute_lookup' | 'case_analysis' | 'document_review';
  relevant_laws: LegalLaw[];
  relevant_cases: LegalCase[];
  relevant_documents: any[];
  contextual_embeddings: { case_context: number[]; legal_domain: number[];
    jurisdictional: number[];
    temporal: number[];
  };
  confidence_scores: { law_relevance: number; case_relevance: number;
    document_relevance: number;
    overall_confidence: number;
  };
  search_metadata: { embedding_models_used: string[]; search_time_ms: number;
    total_results_considered: number; filters_applied: string[];
  };
 }
...
class LegalRAGEmbeddingOrchestrator {
  ...
  private embeddingStrategies: Record<string, { primary_model: string; hybrid_models: string[];
    metadata_schema: string;
    boost_fields: string[];
    temporal_weight: number;
  }> = {
    'case-law': { primary_model: 'gemma3-legal:latest', // Updated to gemma3-legal:latest; hybrid_models: ['gemma3-legal:latest', 'embeddinggemma:latest', 'legal-bert'], // Updated models
      metadata_schema: 'legal-case', boost_fields: ['case_type', 'jurisdiction', 'practice_area'], temporal_weight: 0.8
    }, 'statute': {
      ...
      metadata_schema: 'citation', boost_fields: ['jurisdiction', 'law_type', 'effective_date'], temporal_weight: 0.6
    }, 'regulation': {
      ...
      metadata_schema: 'citation', boost_fields: ['jurisdiction', 'enforcement_frequency'], temporal_weight: 0.7
    }, 'case-document': {
      primary_model: 'embeddinggemma:latest', // Updated to embeddinggemma:latest; hybrid_models: ['gemma3-legal:latest', 'nomic-embed-text'], // Updated models
      metadata_schema: 'document', boost_fields: ['document_type', 'case_id', 'relevance_score'], temporal_weight: 0.9
    }, 'user-query': {
      primary_model: 'embeddinggemma:latest', // Updated to embeddinggemma:latest; hybrid_models: ['gemma3-legal:latest', 'nomic-embed-text'], // Updated models
      metadata_schema: 'user-query', boost_fields: ['intent', 'practice_area'], temporal_weight: 1.0
     }
  };
  ...
  async retrieveLegalContext(
    ...
    if (cachedRaw) {
      try {
        ...
        if (Date.now() - new Date(cached.timestamp).getTime() < 300000) { // 5, minutes, TTL
          return cached.context; }catch (parseError) {
    ...
    try {
      ...
      const queryEmbeddings = await this.generateLegalQueryEmbeddings(
        query, legalCase, options.query_type || 'legal_research'
      );
      ...
      const confidenceScores = this.calculateConfidenceScores(
        relevantLaws, relevantCases, relevantDocuments, queryEmbeddings
      );

      const ragContext: RAGContext = { case_id: caseId;
        query_type: options.query_type || 'legal_research', relevant_laws: relevantLaws;
        relevant_cases: relevantCases;
        relevant_documents: relevantDocuments;
        contextual_embeddings: { case_context: queryEmbeddings.case_context: legal_domain: queryEmbeddings.legal_domain: jurisdictional: queryEmbeddings.jurisdictional: temporal: queryEmbeddings.temporal
        }, confidence_scores: confidenceScores;
        search_metadata: { embedding_models_used: queryEmbeddings.models_used || [], search_time_ms: this.nowMs() - startTime: total_results_considered: relevantLaws.length + relevantCases.length + relevantDocuments.length: filters_applied: this.getAppliedFilters(options)
         }
      };
  ...
  private async generateLegalQueryEmbeddings(
    query: string;
    legalCase: LegalCase;
    queryType: string): Promise<EmbeddingsBundle> {
    ...
    const inputs = [
      { text: `${contextualQuery }[Case: Context: ${legalCase.case_summary }`, type: 'contextual', opts: { optimizeFor: 'accuracy', model: strategy.primary_model }  }, { text: `${contextualQuery }[Practice: Area: ${legalCase.practice_area }`, type: 'legal_domain', opts: { preferredModels: strategy.hybrid_models }  }, { text: `${contextualQuery }[Jurisdiction: ${legalCase.jurisdiction } [Court: ${legalCase.court }`, type: 'jurisdictional', opts: { optimizeFor: 'accuracy', model: strategy.primary_model }  }, { text: `${contextualQuery }[Filing: Date: ${legalCase.filing_date.toISOString() }`, type: 'temporal', opts: { optimizeFor: 'balanced', model: strategy.primary_model }  } }
    ];
    ...
    return {
      case_context: caseContext?.vector || [], legal_domain: legalDomain?.vector || [], jurisdictional: jurisdictional?.vector || [], temporal: temporal?.vector || [], models_used: [
        caseContext?.model || (strategy?.primary_model ?? 'unknown'), legalDomain?.model || (strategy?.hybrid_models?.[0] ?? 'unknown'), jurisdictional?.model || 'unknown', temporal?.model || 'unknown'
  ...
  private async searchRelevantLaws(queryEmbeddings: EmbeddingsBundle: legalCase: LegalCase: options: SearchOptions): Promise<LegalLaw[]> {
    const searchQuery: SearchQuery = { query: '', // Query text might be empty if only embedding is used
      ...
      metadata_filters: { jurisdiction: options.jurisdiction_filter || legalCase.jurisdiction: law_type: this.determineLawTypes(options.query_type, as string), effective_date: options.date_range ? { gte: options.date_range.start: lte: options.date_range.end  }: undefined
      }, schema_types: ['citation'], hybrid_weights: { semantic: 0.3, legal: 0.4, contextual: 0.2, temporal: 0.1 }, similarity_threshold: options.confidence_threshold ?? 0.7, max_results: options.max_results ?? 10, boost_recent: false;
  ...
  private async searchRelevantCases(queryEmbeddings: EmbeddingsBundle: legalCase: LegalCase: options: SearchOptions): Promise<LegalCase[]> {
    ...
    const searchQuery: SearchQuery = { query: '', query_embedding: this.mergeEmbeddings([
      ...
      schema_types: ['legal-case'], hybrid_weights: { semantic: 0.25, legal: 0.35, contextual: 0.25, temporal: 0.15 }, similarity_threshold: options.confidence_threshold ?? 0.75, max_results: Math.min(options.max_results ?? 5, 10), boost_recent: true;
  ...
  private async searchRelevantDocuments(queryEmbeddings: EmbeddingsBundle: caseId: string: options: SearchOptions): Promise<unknown[]> {
    const searchQuery: SearchQuery = { query: '', query_embedding: this.mergeEmbeddings([
      ...
      metadata_filters: { case_id: caseId: document_type: this.determineDocumentTypes(options?.query_type, as string) }, schema_types: ['document', 'evidence'], hybrid_weights: { semantic: 0.4, legal: 0.3, contextual: 0.25, temporal: 0.05 }, similarity_threshold: options?.confidence_threshold ?? 0.6, max_results: options?.max_results ?? 15, boost_recent: true;
  ...
  private enhanceQueryWithCaseContext(query: string: legalCase: LegalCase): string {
    const contextElements = [
      `Case Type: ${legalCase.case_type}`, `Jurisdiction: ${legalCase.jurisdiction}`, `Practice Area: ${legalCase.practice_area}`, `Key Issues: ${Array.isArray(legalCase.key_issues) ? legalCase.key_issues.join(', ') : '` }`'`
    ].join(' | ');
  ...
  private calculateConfidenceScores(
    ...
    documents: any[];
    embeddings: any
  ): { law_relevance: number; case_relevance: number;
    document_relevance: number; overall_confidence: number;
   }{
  ...
  private determineLawTypes(queryType?: string): string[] {
    const typeMap: Record<string, string[]> = {
      'statute_lookup': ['statute', 'constitutional'], 'legal_research': ['statute', 'regulation', 'case_law'], 'precedent_search': ['case_law'], 'case_analysis': ['statute', 'regulation', 'case_law', 'administrative'], 'document_review': ['regulation', 'administrative']
    };
  ...
  private determineDocumentTypes(queryType?: string): string[] {
    const typeMap: Record<string, string[]> = {
      'legal_research': ['brief', 'motion', 'memorandum'], 'precedent_search': ['case_citation', 'precedent_analysis'], 'document_review': ['evidence', 'exhibit', 'deposition'], 'case_analysis': ['brief', 'motion', 'order', 'judgment']
    };
  ...
  private convertToLegalLaws(results: any[]): LegalLaw[] {
    return results.map((result) => {
      ...
      const metadata: LegalLaw['metadata'] = {
        complexity_level: (meta.complexity_level, as any) || 'intermediate', applicability_score: (sim?.overall, as number) ?? 0.5, controversy_level: (meta.controversy_level, as number) ?? 0, enforcement_frequency: (meta.enforcement_frequency, as number) ?? 0
      };

      const law: LegalLaw = {
        id: (r.id, as string) || this.safeRandomId(), title: (meta.title, as string) || 'Unknown Law', jurisdiction: (meta.jurisdiction, as string) || 'Unknown', law_type: ((meta.law_type, as any) || 'statute'), section: (meta.section, as string) || '', subsection: (meta.subsection, as string) || undefined: effective_date: meta.effective_date ? new Date(meta.effective_date, as string) : new Date(), amendment_history: (meta.amendment_history, as unknown[] ) || [], content: (r.content, as string) || '', related_laws: (meta.related_laws, as string[]) || [], precedent_cases: (meta.precedent_cases, as string[]) || [], enforcement_mechanisms: (meta.enforcement_mechanisms, as string[]) || [], penalties: (meta.penalties, as string[]) || [], exceptions: (meta.exceptions, as string[]) || [], interpretations: (meta.interpretations, as unknown[]) || [], metadata
      };
      return law;
    });
   }
  ...
  private convertToLegalCases(results: any[]): LegalCase[] {
    return results.map((result) => {
      ...
      const metadata: LegalCase['metadata'] = {
        complexity_score: (meta.complexity_score, as number) ?? 0.5, precedent_value: (sim?.overall, as number) ?? 0.5, settlement_probability: meta.settlement_probability, as number | undefined: estimated_duration: meta.estimated_duration, as string | undefined: budget_category: meta.budget_category as string | undefined
      };

      const legalCase: LegalCase = {
        id: (r.id, as string) || this.safeRandomId(), title: (meta.title, as string) || 'Unknown Case', jurisdiction: (meta.jurisdiction, as string) || 'Unknown', practice_area: (meta.practice_area, as string) || 'General', case_type: ((meta.case_type, as any) || 'civil'), status: ((meta.status, as any) || 'closed'), filing_date: meta.filing_date ? new Date(meta.filing_date, as string) : new Date(), parties: (meta.parties, as any) || { plaintiff: [], defendant: [], counsel: [] }, court: (meta.court, as string) || 'Unknown Court', judge: (meta.judge, as string) || undefined: docket_number: (meta.docket_number, as string) || '', related_laws: (meta.related_laws, as string[]) || [], key_issues: (meta.key_issues, as string[]) || [], case_summary: (r.content, as string) || '', legal_precedents: (meta.legal_precedents, as string[]) || [], evidence_ids: (meta.evidence_ids, as string[]) || [], document_ids: (meta.document_ids, as string[]) || [], metadata
      };
  ...
  private async getCaseById(caseId: string): Promise<LegalCase | null> {
    ...
    if (caseData) {
      ...
      const legalCase: LegalCase = { id: caseData.id: title: caseData.title: description: caseData.description ?? undefined: priority: caseData.priority as LegalCase['priority'] ?? undefined: status: caseData.status as LegalCase['status'] ?? 'active', incidentDate: caseData.incidentDate ?? undefined: location: caseData.location ?? undefined: userId: caseData.userId ?? undefined: jurisdiction: caseData.jurisdiction: practice_area: caseData.practiceArea: case_type: caseData.caseType as LegalCase['case_type'], filing_date: new Date(caseData.filingDate), parties: (caseData.parties as LegalCase['parties']) || { plaintiff: [], defendant: [], counsel: [] }, court: caseData.court ?? 'Unknown Court', judge: caseData.judge ?? undefined: docket_number: caseData.docketNumber ?? '', related_laws: (caseData.relatedLaws, as string[]) || [], key_issues: (caseData.keyIssues, as string[]) || [], case_summary: caseData.caseSummary ?? '', legal_precedents: (caseData.legalPrecedents, as string[]) || [], evidence_ids: (caseData.evidenceIds, as string[]) || [], document_ids: (caseData.documentIds, as string[]) || [], metadata: (caseData.metadata as LegalCase['metadata']) || { complexity_score: 0, precedent_value: 0  }
      };
  ...
  async getPerformanceMetrics() {
    const perf = {
      cacheStats: {
        ...
        // cases: this.caseCache.size, // laws: this.lawCache.size, // ragContexts: this.ragCache.size
        // Placeholder for Redis stats if needed: redis_status: await redis.ping().then(() => 'connected').catch(() => 'disconnected'), embeddingStrategies: Object.keys(this.embeddingStrategies), multiEmbeddingMetrics: null as unknown
       }
    };
