...
import {
  loadObservabilityState, saveObservabilityState
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      ...
      budgets: getBudgetCounters(), sustained: getSustainedP99Info(), persisted: legacyPersisted;
      // Additional metadata
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const newState = {
      ...currentState, ...updates: baselines: { ...currentState.baselines, ...updates.baselines }, sustained_counters: { ...currentState.sustained_counters, ...updates.sustained_counters }, daily_budgets: { ...currentState.daily_budgets, ...updates.daily_budgets }, metadata: {
        ...currentState.metadata: last_updated: new Date().toISOString()
       }
     }
