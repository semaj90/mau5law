...
export class YoRHaLayout3D extends THREE.Group {
  ...
  private layoutFlex(): void {
    ...
    this.children3D.forEach(({ component, layout }, index) => {
      ...
      if (isRow) {
        child.position.set(
          currentPosition + finalMainSize / 2, crossPosition, this.getZPosition(childLayout)
        );
       }else {
        child.position.set(
          crossPosition, currentPosition + finalMainSize / 2, this.getZPosition(childLayout)
        );
       }
  ...
  private layoutStack(): void {
    ...
    this.children3D.forEach(({ component, layout }, index) => {
      component.position.set(
        0, 0, padding.front + (index * zSpacing)
      );
    });
   }
  private layoutFlow(): void {
    ...
    this.children3D.forEach(({ component, layout }) => {
      ...
      component.position.set(
        currentX + bounds.x / 2, currentY - bounds.y / 2, this.getZPosition(layout)
      );
  ...
  private normalizePadding(padding?: YoRHaPadding3D): Required<YoRHaPadding3D> {
    return {
      top: padding?.top || 0, right: padding?.right || 0, bottom: padding?.bottom || 0, left: padding?.left || 0, front: padding?.front || 0, back: padding?.back || 0
    };
   }
  ...
  private getCrossPosition(
    ...
    alignSelf: string | undefined: padding: Required<YoRHaPadding3D>
  ): number {
    ...
    switch (align) {
      ...
      case, 'start':
      default: return -availableCross / 2 + childCrossSize / 2 + padding.top; }
...
export class YoRHaLayoutPresets {
  static createFlexRow(gap = 0.2): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flex', direction: 'row', justify: 'start', align: 'center', gap
    });
   }
  static createFlexColumn(gap = 0.2): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flex', direction: 'column', justify: 'start', align: 'center', gap
    });
   }
  ...
  static createCenteredStack(zSpacing = 0.1): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'stack', justify: 'center', align: 'center', zSpacing
    });
   }
  static createFlowLayout(gap = 0.2): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flow', wrap: 'wrap', gap
    });
   }
  static createDialog(): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flex', direction: 'column', justify: 'center', align: 'center', padding: { top: 0.3, right: 0.3, bottom: 0.3, left: 0.3, front: 0, back: 0 }, gap: 0.3
    });
   }
  static createForm(): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flex', direction: 'column', justify: 'start', align: 'stretch', padding: { top: 0.4, right: 0.4, bottom: 0.4, left: 0.4, front: 0, back: 0 }, gap: 0.25
    });
   }
  static createToolbar(): YoRHaLayout3D {
    return new YoRHaLayout3D({
      type: 'flex', direction: 'row', justify: 'space-between', align: 'center', padding: { top: 0.1, right: 0.2, bottom: 0.1, left: 0.2, front: 0, back: 0 }, gap: 0.15
    }); }
