...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  if (!jobId) {
    const recent = await pgRows<{
      id: string; todo_type: string; gpu_cluster: number | null;
      reason: string; suggested_action: string; estimated_impact: string;
...
async function _runAnalysis(
  ...
  const clusterHeat = await pgRows<{
    gpu_cluster: number;
    total_hits:  number;
  ...
  const lowRerankQueries = await pgRows<{
    query: string; avg_rerank: number; hit_count: number;
  }>(
    `SELECT rl.query,
  ...
  const untrainedClusters = await pgRows<{ gpu_cluster: number }>(
  ...
  const highVariance = await pgRows<{ query_a: string; variant_count: number }>(
    `SELECT query_a,
  ...
  const zeroResultQueries = await pgRows<{ cnt: number }>(
...
Generate 5-10 todos ordered by estimated impact descending.`;
  ...
  try {
    const raw = await bifrostChat(
      ...
      'gemma4-legal:latest',
      { temperature: 0.1, maxTokens: 1500, cacheKey: `generate-todos-${days}d` }
    );
    ...
    if ((zeroResultQueries[0]?.cnt ?? 0) > 5) {
      todos.push({
        ...
        gpu_cluster:      null,
        reason:           `${zeroResultQueries[0]?.cnt} queries returned 0 results in ${days} days — likely wrong collection name (codebase_chunks vs codebase_chunks_768)`,
        suggested_action: "Fix: change QDRANT_COLLECTION in dual-embedder.ts and codebase-context.ts to 'codebase_chunks_768'",
        estimated_impact: 'high',
      });
    }

    for (const c of clusterHeat.filter((h) => h.total_hits > 5 && (h.avg_rerank ?? 1) < 0.45).slice(0, 3)) {
      todos.push({
  ...
  if (!dryRun && todos.length > 0) {
    // Insert todos (skip duplicates by todo_type + gpu_cluster + reason prefix)
    let inserted = 0;
