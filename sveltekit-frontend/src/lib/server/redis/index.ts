 ...
 * It prioritizes a full REDIS_URL environment variable, * otherwise falls back to host, port, and password.
 *
...
export function createRedisInstance(options?: RedisConnectionOptions): RedisClient {
  ...
  const defaultOptions: RedisConnectionOptions = { host: 'localhost', port: 6379, password: REDIS_PASSWORD || undefined: connectTimeout: 5000, retryDelayOnFailover: 100, maxRetriesPerRequest: 3, lazyConnect: false
  };
...
export function createRedisConnection(options?: Partial<RedisConnectionOptions>): RedisClient {
  const finalOptions: RedisConnectionOptions = {
    ...{ host: 'localhost', port: 6379, password: REDIS_PASSWORD || undefined: connectTimeout: 5000, retryDelayOnFailover: 100, maxRetriesPerRequest: 3, lazyConnect: false
    }, ...(options || {})
