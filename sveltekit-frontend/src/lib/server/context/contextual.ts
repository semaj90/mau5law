...
export class ContextualService {
  ...
  setContext(context: Partial<ContextualState>): void {
    ...
    if (this.isSignificantContext(newContext)) {
      this.memory.longTerm.set(`${newContext.contextType}-${newContext.caseId || 'general'}`, newContext);
    }
   ...
   */
  getPredictions(type?: string, minConfidence: number = 0.5): ContextualPrediction[] {
 ...
 */
export function setContextualService(service: ContextualService = ContextualService.getInstance()): void {
 ...
 */
export function createPredictionsStore(type?: string, minConfidence: number = 0.5) {
  const service = getContextualService();
