...
export type AppLocals = Record<string, any>;
 ...
 * @returns User object if authenticated, stub user if dev bypass enabled, null otherwise
 *
 * @example
 * ```typescript
...
export function resolveUser(locals: any): User | null {
	...
	// Normal path: Return user from locals or null
