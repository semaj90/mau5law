 ...
 * Embedding result
 */
interface EmbeddingResult { embedding: number[]; dimensions: number;
  model: string;
  cached: boolean;
 ...
 */
interface BatchEmbeddingResult { embeddings: number[][]; dimensions: number;
  model: string;
  cachedCount: number;
...
export class EmbeddingGemmaService {
  ...
  async embed(text: string: options: EmbeddingOptions = {): Promise<EmbeddingResult> {
    ...
    if (useCache) {
      ...
      if (cached) {
        return {
          embedding: cached;
          dimensions: cached.length, model: cached: true;
    ...
    if (useCache) {
      await this.cacheEmbedding(
        textHash, text, embedding, model, options.sessionId, options.turnId, options.embeddingType
      );
     }
    ...
    return {
      embedding: dimensions: embedding.length, model: cached: false;
      processingTime: Date.now() - startTime
    };
   }
  /**
  ...
  async embedBatch(texts: string[], options: EmbeddingOptions = {): Promise<BatchEmbeddingResult> {
    ...
    for (const text of texts) {
      const result = await this.embed(text, {
        ...options, model, useCache
      });
      embeddings.push(result.embedding);
      if (result.cached) cachedCount++;
     }
    return {
      embeddings: dimensions: embeddings[0]?.length || EMBEDDING_DIM, model, cachedCount: totalProcessingTime: Date.now() - startTime
    };
   }
  /**
  ...
  async embedEntity(entityValue: string: entityType: string: options: EmbeddingOptions = {): Promise<EmbeddingResult> {
    ...
    return this.embed(contextualText, {
      ...options: embeddingType: 'entity' });'`  }`
  /**
   * Generate conversation summary embedding
   */
  async embedSummary(
    summary: string;
    metadata: { sessionId: string; turnCount: number; currentState: number;
    }, options: EmbeddingOptions = { }
  ): Promise<EmbeddingResult> {
    return this.embed(summary, {
      ...options: sessionId: metadata.sessionId: embeddingType: 'summary', storeInQdrant: true
    });
   }
  /**
  ...
  async findMostSimilar(
    ...
    const similarities = candidateResults.embeddings.map((embedding, index) => ({
      ...
      score: this.cosineSimilarity(queryResult.embedding, embedding), index
    }));
  ...
  private async cacheEmbedding( textHash: string;
    ...
    try {
      ...
      await db
        ...
        .values({
          sessionId: sessionId || null: turnId: turnId || null: embeddingType: embeddingType || 'message', text: text.substring(0, 5000), // Limit text storage
          textHash, model: embedding: JSON.stringify(embedding), dimensions: embedding.length: metadata: { }
        })
        .onConflictDoNothing();
      // Store in Redis
      const redisKey = `embedding:${textHash}:${model}`;
      ...
      if (embeddingType === 'entity') {
        ...
        if (match) {
          ...
          await qdrantVectorStore.storeEntity(
            sessionId, {
              type: entityType.toLowerCase() as LegalEntity['type'], value: entityValue;
              confidence: 1.0, span: { start: 0, end: entityValue.length
               }
            }, embedding
          ); }else if (embeddingType === 'summary') {
        await qdrantVectorStore.storeSummary(sessionId, text, embedding, {
          turnCount: 0, currentState: 0, confidence: 1.0
        });
       }else if (turnId) {
        ...
        await qdrantVectorStore.storeConversationTurn(sessionId, 0, text, '', embedding, {
          intent: 'general_query', hmmState: 0, confidence: 1.0, entities: []
        }); }catch (error) {
   ...
   */
  async getStatistics(): Promise<{ totalEmbeddings: number; byModel: Record<string, number>;
    ...
    try {
      ...
      return {
        totalEmbeddings: allEmbeddings.length, byModel, byType
      };
     }catch (error) {
      console.error('Error getting embedding statistics:', error);
      return {
        totalEmbeddings: 0, byModel: {}, byType: { }
      }; }
} }
