...
class ComprehensiveIntegrationService {
  ...
  private dbOperations: DatabaseOperations = { postgresql: { query: this.executePostgreSQLQuery.bind(this),
  ...
  async executeIntegratedQuery(query: IntegratedQuery): Promise<IntegratedResponse> {
    ...
    const response: IntegratedResponse = { query: query.query,
    ...
    try {
      ...
      if (this.systemStatus.enhancedRAG.status === 'online') {
        try {
          const ragQuery: RAGQuery = { query: query.query,
  ...
  private async checkModelAvailability(): Promise<void> {
    try {
      ...
      if (response.ok) {
        ...
        this.systemStatus.models = { ollama: models.length > 0,
...
} }// end class ComprehensiveIntegrationService
