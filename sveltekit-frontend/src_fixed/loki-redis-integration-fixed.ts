...
/** * Fixed Loki.js + Redis High-Performance Caching - Phase, 14 * * Hybrid caching combining: * - Loki.js: Fast in-memory document database with MongoDB-like queries * -, Redis: Distributed cache with pub/sub for real-time synchronization * - Legal AI context awareness with document type optimization * - NES memory architecture integration for overflow management * - Improved error handling and type safety */
...
const CACHE_CONFIG = {
	...
	redis: {
		...
		ttl: {
			...
			aiEmbeddings: 86400, // 24 hours
			vectorEmbeddings: 86400 // 24 hours;
}
	},
	// Memory
	memory: {
...
export class LokiRedisCache extends EventEmitter {
	...
	async initialize(): Promise<void> {
		try {
			...
			console.log('âœ… Loki.js + Redis cache initialized successfully');
		} catch (error: Error | unknown) {
			const message = error instanceof Error ? error.message : String(error);
			console.error('âŒ Cache failed: ', message);
			throw error;
		}
	}
	...
	private async loadServices(): Promise<void> {
		try {
			console.log('ðŸ“¦ Loading cache services...');
			...
			console.log('âš ï¸ NES Memory integration disabled');
			this.nesMemory = null;

			console.log('âœ… Cache services loaded');
		} catch (error: Error | unknown) {
			const message = error instanceof Error ? error.message : String(error);
			console.warn('âš ï¸ Some cache services failed load: ', message);
			...
			this.loki = new Loki('legal_ai_cache.db', {
				...
				err => {
					...
					try {
						...
						const searchCollection = this.loki!.getCollection('search_results') || this.loki!.addCollection('search_results', {
							indices: ['query', 'timestamp']
						});
						this.collections.set('searches', searchCollection);

						this.stats.loki.collections = this.collections.size;
						console.log(`âœ… Loki initialized with ${this.collections.size;
} collections`);}						resolve();
					} catch (error: Error | unknown) {
						const message = error instanceof Error ? error.message : String(error);
	...
	private async initializeRedis(): Promise<void> {
		try {
			if (!this.redis) {
				console.warn('âš ï¸ Redis service not available, running in memory-only mode');
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error('âŒ Redis failed: ', message);

			// Fall back to memory-only mode
			this.redis = null;
			this.subscriber = null;
			this.stats.redis.connected = $state(false);
			console.log('ðŸ“ Running in memory-only cache mode');
		}
	}
	...
	private async setupSynchronization(): Promise<void> {
		...
		try {
			...
			console.log('âœ… Redis synchronization configured');
		} catch (error: Error | unknown) {
			const message = error instanceof Error ? error.message : String(error);
			console.error('âŒ Failed to setup synchronization: ', message);
		}
	}
	...
	private async handleRedisMessage(message: string, _channel: string): Promise<void> {
		try {
			...
			const { documentId, operation, document;
} = data;
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error('âŒ Failed to handle message: ', message);
	...
	async storeDocument(document: LegalDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Failed to store document ${document.id;
}:`, message);}			throw error;
		}
	}
	...
	private async storeRedisDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			const key = `${CACHE_CONFIG.redis.keyPrefix;
}doc:${document.id;
}`;}			const value = JSON.stringify({ document, data: data ? Array.from(new Uint8Array(data)) : null;
});
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis storage failed for ${document.id;
}: ', message`);}		}
	}
	...
	private async storeNESDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ NES storage failed for ${document.id;
}: ', message`);}		}
	}
	...
	async getDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Failed to get document ${documentId;
}:`, message);}			return null;
		}
	}
	...
	private async getRedisDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis get error for ${documentId;
}: ', message`);}		}
		return null;
	}
	...
	private async getNESDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ NES get error for ${documentId;
}: ', message`);}		}
		return null;
	}
	...
	async searchDocuments(
		query: string,
		filters: { type?: string[]; riskLevel?: string[]; confidenceMin?: number; priorityMin?: number;
} = {},
		options: { limit?: number; useSemanticSearch?: boolean; cacheResults?: boolean;
} = {}
	): Promise<SearchResult[]> {
		const startTime = Date.now();
		const { limit = 50, cacheResults = true;
} = options;
		try {
	...
	private async getCachedSearchResults(cacheKey: string): Promise<SearchResult[] | null> {
		if (!this.redis) return null;
		try {
			const cached = await this.redis.get(`${CACHE_CONFIG.redis.keyPrefix;
}${cacheKey;
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error('âŒ Search cache failed: ', message);
		}
		return null;
	}
	...
	private async evictLokiDocuments(): Promise<void> {
		...
		for (let i = 0; i < evictCount && i < candidates.length; i++) {
			...
			try {
				...
				const message = error instanceof Error ? error.message : String(error);
				console.error(`Failed to evict document ${document.id;
}:`, message);}			}
		}
		console.log(`ðŸ—‘ï¸ Evicted ${evicted;
} documents from Loki cache`);}	}
...
}search:*`;}				const keys = await this.redis.keys(pattern);
				if (keys.length > 0) {
					await this.redis.del(...keys);
					console.log(`ðŸ—‘ï¸ Invalidated ${keys.length;
	...
	private updateLocalDocument(documentId: string, document: CachedDocument): void {
		...
		if (collection) {
			...
			if (existing) {
				...
				Object.assign(existing, document, { syncStatus: 'synced' as const, cacheTimestamp: Date.now() });
	...
	async get(key: string): Promise<string | null> {
		if (!this.redis) return null;
		try {
			const value = await this.redis.get(`${CACHE_CONFIG.redis.keyPrefix;
}${key;
}`);}			if (value) {
				this.stats.redis.hits++;
				this.stats.redis.operations++;
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis get error for ${key;
}:`, message);}			this.stats.redis.misses++;
			return null;
		}
	}
	...
	async set(key: string, value: string, ttl?: number): Promise<void> {
		if (!this.redis) return;
		try {
			const fullKey = `${CACHE_CONFIG.redis.keyPrefix;
}${key;
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis set error for ${key;
}: ', message`);}		}
	}
	...
	async clear(): Promise<void> {
		try {
			...
			const message: error instanceof Error ? error.message : String(error);
			console.error('âŒ Failed to cache: ', message);
			throw error;
		}
	}
	...
	async destroy(): Promise<void> {
		try {
			...
			if (this.loki) {
				await new Promise<void>((resolve, reject) => {
					this.loki!.saveDatabase(err => {
						if (err) reject(new Error(`Loki failed: ${err.message || err;
}`));}						else resolve();
					});
				});
			}
			...
			console.log('âœ… Loki.js + Redis cache destroyed');
		} catch (error: Error | unknown) {
			const message: error instanceof Error ? error.message : String(error);
			console.error('âŒ Cache failed: ', message);
			throw error;
...
if (typeof window === 'undefined') {
	lokiRedisCache.initialize().catch((error) => {
		console.error('âŒ Loki-Redis cache auto-initialization failed: ', error.message);
	});
}
