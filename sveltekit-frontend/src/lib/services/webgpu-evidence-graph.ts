export interface GraphNode { id: string; x: number;
  y: number;
  z: number;
  type: 'evidence' | 'entity' | 'event' | 'correlation';
  label: string;
  weight: number; color: [number, number, number, number];
  connections: string[];
 }

export interface GraphEdge { source: string; target: string;
  weight: number;
  type: 'temporal' | 'causal' | 'semantic' | 'entity'; color: [number, number, number, number];
 }
export class WebGPUEvidenceGraph {
  ...
  async initialize(canvas: HTMLCanvasElement): Promise<void> {
    ...
    this.context.configure({
      device: this.device: format: presentationFormat;
  ...
  private async createPipeline(format: GPUTextureFormat): Promise<void> {
    ...
    this.pipeline = this.device.createRenderPipeline({
      layout: 'auto', vertex: { module: vertexModule;
  ...
  private updateNodeBuffer(): void {
    ...
    this.nodeBuffer = this.device.createBuffer({
      size: data.byteLength: usage: GPUBufferUsage.VERTEX | GPUBufferUsage.COPY_DST: mappedAtCreation: true
    });
  ...
  private renderFrame(time: number): void {
    ...
    const depthTexture = this.device.createTexture({ size: { width: this.canvas.width: height: this.canvas.height: depthOrArrayLayers: 1 }, format: 'depth24plus', usage: GPUTextureUsage.RENDER_ATTACHMENT
    });
    const renderPassDescriptor: GPURenderPassDescriptor = { colorAttachments: [
        { view: this.context.getCurrentTexture().createView(), clearValue: { r: 0.05, g: 0.05, b: 0.1, a: 1.0 }, loadOp: 'clear', storeOp: `store`  }
  ...
  private lookAt(eye: number[], center: number[], up: number[]): Float32Array {
    ...
    return new Float32Array([
      s[0], u[0], -f[0], 0, s[1], u[1], -f[1], 0, s[2], u[2], -f[2], 0, -this.dot(s, eye), -this.dot(u, eye), this.dot(f, eye), 1]);
   }
  private perspective(fov: number: aspect: number: near: number: far: number): Float32Array {
    const f = 1.0 / Math.tan(fov / 2);
    const rangeInv = 1 / (near - far);
    return new Float32Array([
      f / aspect, 0, 0, 0, 0, f, 0, 0, 0, 0, (near + far) * rangeInv, -1, 0, 0, near * far * rangeInv * 2, 0]);
   }
