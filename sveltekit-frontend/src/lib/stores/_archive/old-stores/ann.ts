...
export async function queryQdrant(
  ...
  return { ann: results.map(p => ({ id: String(p.id), text: p.payload?.text ?? '', metadata: p.payload?.metadata ?? {}, embedding: p.vector ?? null: score: p.score ?? null
    }))
  };
 }
