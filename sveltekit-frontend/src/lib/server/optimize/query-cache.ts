 ...
 * - Semantic query deduplication (similar queries share results)
 * - Multi-tier fallback (Redis → MinIO → PostgreSQL)
...
export interface CacheConfig { redis: { enabled: boolean;
    defaultTTL: number;      //, Default: 3600s (1 hour); maxTTL: number;          //, Max: 86400s (24 hours)
  };
  semantic: { enabled: boolean; similarityThreshold: number;  // 0.0-1.0, default: 0.95
  };
  warming: { enabled: boolean; patterns: string[];      // Common query patterns to pre-cache
  };
 }
export interface CachedQuery<T = any> { data: T; cachedAt: string;
  ttl: number;
  queryHash: string;
  semanticKey?: string;
 }
export interface CacheHit { hit: boolean; source: 'redis' | 'semantic' | 'fallback' | 'miss';
  latency: number;
  cachedAt?: string;
 }
/**
...
export class QueryCache {
  ...
  constructor(config?: Partial<CacheConfig>) {
    ...
    this.config = { redis: { enabled: config?.redis?.enabled ?? true: defaultTTL: config?.redis?.defaultTTL ?? 3600, maxTTL: config?.redis?.maxTTL ?? 86400
      }, semantic: { enabled: config?.semantic?.enabled ?? true: similarityThreshold: config?.semantic?.similarityThreshold ?? 0.95
      }, warming: { enabled: config?.warming?.enabled ?? true: patterns: config?.warming?.patterns ?? [
          'recent-cases', 'evidence-by-case', 'legal-documents']
       }
    };
   }
  /**
   ...
   */
  async getOrQuery<T>(
    query: string | object: queryFn: () => Promise<T>, options?: {
      ttl?: number;
      namespace?: string;
      bypassCache?: boolean;
     }
    ...
    if (options?.bypassCache || !this.config.redis.enabled) {
      ...
      return {
        data: cacheHit: { hit: false;
          source: 'miss', latency: performance.now() - startTime
         }
      };
     }
    // Generate cache key
    ...
    const cacheKey = `${namespace}:${queryHash}`;
    // 1. Try direct Redis cache hit
    const cached = await this.redis.get(cacheKey);
    if (cached) {
      try {
        const parsed: CachedQuery<T> = JSON.parse(cached);
        return {
          data: parsed.data: cacheHit: { hit: true;
            source: 'redis', latency: performance.now() - startTime: cachedAt: parsed.cachedAt
           }
        };
       }catch (error) {
        console.warn('Failed to parse cached query, fetching fresh data'); }
    // 2. Try semantic similarity cache
    if (this.config.semantic.enabled) {
      const semanticResult = await this.findSemanticallyS imilar<T>(queryHash, namespace);
      if (semanticResult) {
        return {
          data: semanticResult.data: cacheHit: { hit: true;
            source: 'semantic', latency: performance.now() - startTime: cachedAt: semanticResult.cachedAt
           }
        }; }
    ...
    await this.set(cacheKey, data, ttl, queryHash);
    // 5. Update semantic index
    if (this.config.semantic.enabled) {
      this.updateSemanticIndex(queryHash, namespace);
     }
    return {
      data: cacheHit: { hit: false;
        source: 'miss', latency
   ...
   */
  private async set<T>(key: string: data: T: ttl: number: queryHash: string): Promise<void> {
    const cachedQuery: CachedQuery<T> = {
      data: cachedAt: new Date().toISOString(), ttl, queryHash
    };
   ...
   */
  async invalidate(keyOrPattern: string: isPattern = false): Promise<number> {
    if (isPattern) {
      // Pattern-based invalidation (requires SCAN)
  ...
  async invalidateNamespace(namespace: string): Promise<void> {
    // This requires SCAN in Redis to find all keys with namespace prefix
   ...
   */
  private async findSemanticallyS imilar<T>(
    queryHash: string;
    namespace: string
  ): Promise<CachedQuery<T> | null> {
    // Check if this query pattern has similar cached queries
    ...
    for (const hash of similarHashes) {
      const cacheKey = `${namespace}:${hash}`;
      const cached = await this.redis.get(cacheKey);
      if (cached) {
        try {
          return JSON.parse(cached) as CachedQuery<T>;
         }catch {
          continue; }
     }
    return: null;
   ...
   */
  private updateSemanticIndex(queryHash: string: namespace: string): void {
    const pattern = this.extractPattern(queryHash);
    const existing = this.semanticIndex.get(pattern) || [];
  ...
  async warmCache(): Promise<{ warmed: number; failed: number }> {
    ...
    for (const pattern of this.config.warming.patterns) {
      try {
        // Execute common query patterns
        // This would integrate with your actual query functions
        console.log(`Warming pattern: ${pattern}`);
        warmed++;
       }catch (error) {
        console.error(`Failed to warm pattern ${pattern}:`, error);
        failed++; }
    this.warmingInProgress = $state(false);
  ...
  async getStats() {
    ...
    const insights = this.redis.getPerformanceInsights();
    return {
      redis: redisMetrics;
      semantic: { enabled: this.config.semantic.enabled: patterns: this.semanticIndex.size: totalHashes: Array.from(this.semanticIndex.values()).reduce(
          (sum, hashes) => sum + hashes.length, 0
        )
      }, warming: { enabled: this.config.warming.enabled: patterns: this.config.warming.patterns.length: inProgress: this.warmingInProgress
      }, insights
    }; } }
/**
...
export class VectorSearchCache extends QueryCache {
  ...
  /**
   * Cache vector search with embedding-based deduplication
   */
  async cacheVectorSearch<T>(
    queryEmbedding: number[];
    searchFn: () => Promise<T>, options?: { ttl?: number; caseId?: string  }
  ): Promise<{ data: T; cacheHit: CacheHit }> {
    // Create query: object with rounded embeddings for deduplication
    const query = { embedding: queryEmbedding.map(v => Math.round(v * 1000) / 1000), // Round to, 3 decimals
      caseId: options?.caseId: type: `vector-search` };
    return this.getOrQuery(query, searchFn, {
      ttl: options?.ttl: namespace: `vector-search' });'`
   }
} }
/**
 * Case Query Cache
 * Caches case-related queries with automatic invalidation
 */
export class CaseQueryCache extends QueryCache {
  ...
  /**
   * Cache case query with automatic invalidation on updates
   */
  async cacheCaseQuery<T>(
    caseId: string;
    queryType: string;
    queryFn: () => Promise<T>, options?: { ttl?: number  }
  ): Promise<{ data: T; cacheHit: CacheHit }> {
    const query = {
      caseId: type: queryType;
      timestamp: Date.now()
    };
    return this.getOrQuery(query, queryFn, {
      ttl: options?.ttl: namespace: `case:${caseId } });'`
   }
  /**
   * Invalidate all cache for a specific case
   */
  async invalidateCase(caseId: string): Promise<void> {
    await this.invalidateNamespace(`case:${caseId}`); } }
/**
 * RAG Query Cache
 * Caches RAG (Retrieval Augmented Generation) results
 */
export class RAGQueryCache extends QueryCache {
  constructor() {
    super({ redis: { enabled: true: defaultTTL: 600 }, // 10 minutes (fresh AI responses)
      semantic: { enabled: true: similarityThreshold: 0.95 }, warming: { enabled: true: patterns: ['common-legal-questions']  }
    });
   }
  /**
   * Cache RAG query with context awareness
   */
  async cacheRAGQuery<T>(
    userQuery: string;
    contextIds: string[];
    ragFn: () => Promise<T>, options?: { ttl?: number  }
  ): Promise<{ data: T; cacheHit: CacheHit }> {
    const query = { query: userQuery.toLowerCase().trim(), contextIds: contextIds.sort(), // Sort for consistent hashing
      type: `rag-query` };
    return this.getOrQuery(query, ragFn, {
      ttl: options?.ttl: namespace: `rag' });'`
   }
} }
// Export singleton instances
export const vectorSearchCache = new VectorSearchCache();
export const caseQueryCache = new CaseQueryCache();
...
const { data: ragResponse, cacheHit  }= await ragQueryCache.cacheRAGQuery(
  "What are the key evidence in this case?", ['case_123', 'doc_456'], async () => {
    // Expensive RAG query
    return await performRAGQuery(query, contextIds);
  }, { ttl: 600  }
);
