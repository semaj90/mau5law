...
const titles = [
	...
	{ code: 'frcp', name: 'federal-rules-civil-procedure', canonical: 'federal-rules-civil-procedure' },
	{ code: 'frre', name: 'federal-rules-evidence', canonical: 'federal-rules-evidence' },
	{ code: 'frcr', name: 'federal-rules-criminal-procedure', canonical: 'federal-rules-criminal-procedure' }
];

export function findStateBySlug(slug: string) {
	slug = slug.toLowerCase().trim();
	return states.find(
		(s) => s.abbr === slug || s.slug === slug || s.canonical === slug
	);
}
...
export function findTitleBySlug(slug: string) {
	slug = slug.toLowerCase().trim();
	return titles.find(
		(t) => t.code === slug || t.name === slug || t.canonical === slug
	);
}
