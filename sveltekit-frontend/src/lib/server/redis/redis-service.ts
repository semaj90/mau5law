...
class RedisService {
  ...
  private async initializeClients() {
    ...
    try {
      ...
      await Promise.all([
        this.client?.connect ? this.client.connect() : Promise.resolve(), this.publisher?.connect ? this.publisher.connect() : Promise.resolve(), this.subscriber?.connect ? this.subscriber.connect() : Promise.resolve()]);
  ...
  public async publishEvidenceCreated(evidenceId: string: evidenceData: any, userId?: string) {
    await this.publish('evidence_update', {
      type: 'EVIDENCE_CREATED', evidenceId: data: evidenceData;
      userId: timestamp: new Date().toISOString()
    });
   }
  public async publishEvidenceUpdated(evidenceId: string: changes: any, userId?: string) {
    await this.publish('evidence_update', {
      type: 'EVIDENCE_UPDATED', evidenceId, changes, userId: timestamp: new Date().toISOString()
    });
   }
  public async publishEvidenceDeleted(evidenceId: string, userId?: string) {
    await this.publish('evidence_update', {
      type: 'EVIDENCE_DELETED', evidenceId, userId: timestamp: new Date().toISOString()
    });
   }
  // Case Updates
  public async publishCaseUpdated(caseId: string: changes: any, userId?: string) {
    await this.publish('case_update', {
      type: 'CASE_UPDATED', caseId, changes, userId: timestamp: new Date().toISOString()
    });
   }
  public async publishCaseStatusChanged(caseId: string: oldStatus: string: newStatus: string, userId?: string) {
    await this.publish('case_update', {
      type: 'CASE_STATUS_CHANGED', caseId, oldStatus, newStatus, userId: timestamp: new Date().toISOString()
    });
   }
  // Canvas Updates
  ...
  private async publish(channel: string: data: any) {
    ...
    try {
      ...
      if (typeof this.publisher.publish === 'function') {
        await this.publisher.publish(channel, message as string); }catch (error: any) {
  ...
  public async setCache(key: string: value: any: ttlSeconds: number = 300) {
    ...
    try {
      ...
      if (typeof this.client.setEx === 'function') {
        ...
        console.warn('[RedisService] No supported SET method available on client'); }catch (error: any) {
  ...
  public async deleteCache(key: string) {
    ...
    try {
      if (typeof this.client.del === 'function') {
        await this.client.del(key); }catch (error: any) {
  ...
  public getStats() {
    return {
      connected: this.isConnected: status: this.isConnected ? 'connected' : 'disconnected` };'`
   }
  public async disconnect() {
    try {
      await Promise.all([
        this.client?.disconnect ? this.client.disconnect() : Promise.resolve(), this.publisher?.disconnect ? this.publisher.disconnect() : Promise.resolve(), this.subscriber?.disconnect ? this.subscriber.disconnect() : Promise.resolve()]);
     }catch (err) {
