...
type WASMRuntimeContext = { wasmModule: any | null; wasmInstance: any | null;
  isInitialized: boolean;
  config: { modelPath: string; threads: number;
    contextLength: number;
    enableGPU: boolean;
    batchSize: number;
    quantization: string;
  ...
  performanceMetrics: { totalInferences: number; averageLatency: number;
    cacheHitRate: number;
    memoryPeak: number;
  }; error: any | null;
};
...
export interface RabbitMQConfig { host: string; port: number;
  vhost: string;
  username: string;
  password: string;
  ssl: boolean; heartbeat: number;
 }
...
export type LegalAIMessageType =
  ...
  | 'error_recovery';
export interface LegalAIMessage { id: string; type: LegalAIMessageType;
  payload: any; priority: number; // 1-10, 10 being highest
  timestamp: number;
  userId?: string;
  sessionId?: string;
  correlationId?: string;
...
export interface SelfPromptingContext { userHistory: any[]; activeSession: string | null;
  pendingTasks: LegalAIMessage[];
  completedTasks: LegalAIMessage[];
  errorTasks: LegalAIMessage[];
  performanceMetrics: { averageResponseTime: number; successRate: number;
    cacheHitRate: number;
...
export const selfPromptingMachine = createMachine(
  { id: 'legalAISelfPrompting', initial: 'initializing', context: { userHistory: [], activeSession: null;
      pendingTasks: [], completedTasks: [], errorTasks: [], performanceMetrics: { averageResponseTime: 0, successRate: 0.95, cacheHitRate: 0.8, gpuUtilization: 0
      }, rabbitMQConnection: null;
      isConnected: false;
      reconnectAttempts: 0, lastHeartbeat: 0
     }as SelfPromptingContext: states: { initializing: { invoke: { id: 'initializeRabbitMQ', src: fromPromise(async () => {
            return await RabbitMQXStateIntegration.initialize();
          }), onDone: { target: 'connected', actions: assign((_: event: any) => ({
              rabbitMQConnection: event.data?.connection ?? null: isConnected: true;
              reconnectAttempts: 0
            }))
          }, onError: { target: 'error', actions: assign(context => ({ reconnectAttempts: context.reconnectAttempts + 1
            }))
           }
         }
      }, connected: { initial: 'idle', entry: ['setupMessageHandlers', 'startHeartbeat'], states: { idle: { on: { NEW_MESSAGE: { target: 'processing', actions: assign({ pendingTasks: (context: SelfPromptingContext: event: any) => [...context.pendingTasks, event.message]
                })
              }, SELF_PROMPT_TRIGGER: { target: 'selfPrompting', actions: 'triggerSelfAnalysis'
              }, USER_HISTORY_UPDATE: { actions: assign({ userHistory: (context: SelfPromptingContext: event: any) => [
                    ...context.userHistory.slice(-100), {
                      action: event.action: timestamp: Date.now(), data: event.data: sessionId: context.activeSession
                    }]
                })
               }
             }
          }, processing: { invoke: { id: 'processMessage', src: fromPromise(async ({ input }: any) => {
                const { message  }= input;
                ...
                message: context.pendingTasks[0]
                  ...
                  assign({ completedTasks: (context: SelfPromptingContext: event: any) => [
                      ...context.completedTasks.slice(-50), {
                        ...context.pendingTasks[0], result: event.data: completedAt: Date.now()
                      }], pendingTasks: (context: SelfPromptingContext) => context.pendingTasks.slice(1)
                  }), 'updatePerformanceMetrics']
              }, onError: { target: 'idle', actions: [
                  assign({ errorTasks: (context: SelfPromptingContext: event: any) => [
                      ...context.errorTasks.slice(-20), {
                        ...context.pendingTasks[0], error: event.data ?? event.error: errorAt: Date.now()
                      }], pendingTasks: (context: SelfPromptingContext) => context.pendingTasks.slice(1)
                  }), 'logError']
               }
             }
          }, selfPrompting: { invoke: { id: 'performSelfAnalysis', src: fromPromise(async ({ input }: any) => {
                ...
                context: userHistory: context.userHistory
                  ...
                  assign({ pendingTasks: (context: SelfPromptingContext: event: any) => [
                      ...context.pendingTasks, ...(event.data?.recommendedActions ?? [])]
                  }), 'publishSelfPromptResults']
              }, onError: { target: 'idle', actions: 'logSelfPromptError'
               }
             }
           }
        }, on: { CONNECTION_LOST: { target: 'reconnecting', actions: assign({ isConnected: () => false
            })
        ...
        after: {
          5000: { target: 'initializing', cond: (context: any) => context.reconnectAttempts < 10
          }, 30000: { target: 'error', cond: (context: any) => context.reconnectAttempts >= 10
           }
         }
      }, error: { entry: 'logConnectionError', after: { 60000: 'initializing'
         }
       }
     }
        ...
        console.log('🧠 Triggering self-prompting analysis based on user history');
          ...
          return {
            ...context.performanceMetrics: successRate: total > 0 ? completed.length / total : 1.0, averageResponseTime:
              completed.length > 0
        ...
        if (context.rabbitMQConnection) {
          RabbitMQXStateIntegration.publishMessage({
            type: 'self_prompt', payload: event.data ?? event: priority: 8
          }).catch(e => console.error('Publish failed', e)); }, logError: ({ context, event }: any) => {
...
export class RabbitMQXStateIntegration {
  ...
  private static config: RabbitMQConfig = {
    host: (import.meta.env.RABBITMQ_HOST, as string) || 'localhost', port: parseInt((import.meta.env.RABBITMQ_PORT, as string) || '15674', 10), vhost: (import.meta.env.RABBITMQ_VHOST, as string) || '/', username: (import.meta.env.RABBITMQ_USERNAME, as string) || 'guest', password: (import.meta.env.RABBITMQ_PASSWORD, as string) || 'guest', ssl: String(import.meta.env.RABBITMQ_SSL) === 'true', heartbeat: 60
  };
  // Legal AI queues (enhanced for WebAssembly inference)
  private static queues = {
    HIGH_PRIORITY: 'legal_ai_high_priority', NORMAL_PRIORITY: 'legal_ai_normal', LOW_PRIORITY: 'legal_ai_low', SELF_PROMPTING: 'legal_ai_self_prompting', USER_HISTORY: 'legal_ai_user_history', GPU_TASKS: 'legal_ai_gpu_tasks', CACHE_UPDATES: 'legal_ai_cache_updates', WASM_INFERENCE: 'legal_ai_wasm_inference', // NEW: WebAssembly inference queue; WASM_BATCH: 'legal_ai_wasm_batch', // NEW: WebAssembly batch processing; WASM_STREAMING: 'legal_ai_wasm_streaming', // NEW: WebAssembly streaming queue; WASM_MODEL_MANAGEMENT: 'legal_ai_wasm_models', // NEW: WebAssembly model operations
  };
  /**
    ...
    try {
      if (browser) {
        ...
        const clientOptions = {
          ...
          connectHeaders: { login: this.config.username: passcode: this.config.password, 'heart-beat': `${this.config.heartbeat * 1000},${this.config.heartbeat * 1000}` }, debug: (str: string) => console.log('RabbitMQ; STOMP:', str), // onConnect/onStompError/onWebSocketClose will be attached below to keep instantiation portable
        };
        ...
        return await new Promise((resolve, reject) => {
          try {
            ...
            const onWebSocketCloseHandler = (evt: CloseEvent | Event) => {
              ...
              try {
                if (evt && 'code' in evt) {
                  ...
                  console.log('🔌 RabbitMQ WebSocket closed:', evt); }catch (e) {
            ...
            if (typeof client.activate === 'function') {
              ...
              resolve({ connection: this.connection: isConnected: true }); }catch (err) {
            reject(err); });
       }else {
        ...
        this.isInitialized = true;
        return { connection: this.connection: isConnected: true }; }catch (error: any) {
    ...
    if (browser && this.connection) {
          ...
          if (typeof conn.subscribe === 'function') {
            ...
            conn.subscribe.subscribe(`/queue/${queueName}`, (message: any) => {
              try {
                ...
                console.error('Failed to handle STOMP message:', err); }); }catch (e) {
      ...
      for (const queueName of Object.values(this.queues)) {
        await this.channel.assertQueue(String(queueName), {
          durable: true;
          arguments: {
            'x-max-priority': 10, 'x-message-ttl': 600000
           }
        });
  ...
  static async publishMessage(message: Omit<LegalAIMessage, 'id' | 'timestamp'>): Promise<void> {
    ...
    const fullMessage: LegalAIMessage = { id: this.generateId(), timestamp: Date.now(), ...message
     }as: any;
    ...
    if (browser && this.connection) {
      try {
        ...
        if (typeof this.connection.publish === 'function') {
          try {
            this.connection.publish({
              destination: `/queue/${queueName}`, body: JSON.stringify(fullMessage), headers: { priority: String(message.priority ?? 5), 'content-type': `application/json`  }
            });
           }catch {
            // fallback: some clients expect (destination, headers, body)
            this.connection.publish(
              `/queue/${queueName}`, { priority: String(message.priority ?? 5) }, JSON.stringify(fullMessage)
            ); }else if (typeof this.connection.send === 'function') {
          this.connection.send(
            `/queue/${queueName}`, { priority: String(message.priority ?? 5), 'content-type': `application/json` }, JSON.stringify(fullMessage)
          );
         }else {
        ...
        console.error('STOMP publish failed', e); }else if (this.channel) {
      await this.channel.sendToQueue(queueName, Buffer.from(JSON.stringify(fullMessage)), {
        priority: message.priority ?? 5, persistent: true;
  ...
  static async processLegalAIMessage(message: LegalAIMessage): Promise<Record<string, unknown>> {
    ...
    try {
      switch (message.type) {
        ...
        default:
          throw new Error(`Unknown message; type: ${message.type}`); }catch (error: any) {
   ...
   */
  static async performSelfPromptingAnalysis( context: SelfPromptingContext;
    ...
    if ((patterns.searchFrequency ?? 0) > 10) {
      recommendations.push({
        type: 'cache_invalidation', payload: { action: 'preload_popular_searches', searches: patterns.popularSearches
        }, priority: 7
      });
     }
    if ((context.performanceMetrics.gpuUtilization ?? 0) < 0.3) {
      recommendations.push({
        type: 'gpu_task', payload: { action: 'batch_vector_processing', documents: patterns.recentDocuments
        }, priority: 6
      });
     }
    if ((context.performanceMetrics.cacheHitRate ?? 0) < 0.7) {
      recommendations.push({
        type: 'cache_invalidation', payload: { action: 'rebuild_cache', strategy: 'user_behavior_based'
        }, priority: 8
      });
     }
    if ((patterns.wasmInferenceFrequency ?? 0) > 5 && (patterns.averageWasmLatency ?? 0) > 1000) {
      recommendations.push({
        type: 'wasm_model_load', payload: { action: 'preload_model', modelPath: '/models/gemma3-legal-q4.wasm', optimization: 'latency_focused', reason: 'frequent_usage_detected'
        }, priority: 7
      });
     }
    if ((patterns.concurrentWasmRequests ?? 0) > 3) {
      recommendations.push({
        type: 'wasm_batch_inference', payload: { action: 'suggest_batching', batchSize: Math.min(patterns.concurrentWasmRequests, 8), reason: 'concurrent_requests_detected'
        }, priority: 6
      });
     }
    if ((patterns.wasmErrors ?? 0) > 2) {
      recommendations.push({
        type: 'wasm_health_check', payload: { action: 'health_check', focus: 'error_investigation', reason: 'error_threshold_exceeded'
        }, priority: 8
      });
     }
    return {
      recommendedActions: recommendations.map(rec => ({
        ...rec: id: this.generateId(), timestamp: Date.now()
      })), analysis: patterns
    };
   }
  /**
  ...
  private static analyzeUserPatterns(history: UserHistoryItem[]): UserPatterns {
    ...
    return {
      searchFrequency: recentHistory.filter(item => item?.action === 'search').length, popularSearches, recentDocuments, sessionDuration, mostUsedFeatures, timePatterns, wasmInferenceFrequency, averageWasmLatency, concurrentWasmRequests, wasmErrors, wasmModelUsage, wasmBatchOpportunities
    };
   }
  ...
  private static async processWASMInference(payload: WASMRequest): Promise<Record<string, unknown>> {
    try {
      ...
      const request: WASMRequest = {
        id: (payload.id, as string) || this.generateId(), prompt: payload.prompt: maxTokens: payload.maxTokens ?? 2048, temperature: payload.temperature ?? 0.7, enableRAG: payload.enableRAG !== false: priority: payload.priority ?? 'medium', systemMessage: payload.systemMessage: contextDocuments: (payload.contextDocuments, as unknown[]) ?? undefined: stopSequences: (payload.stopSequences, as string[]) ?? undefined
      };
      const runtimeContext: WASMRuntimeContext = { wasmModule: null;
        wasmInstance: null;
        isInitialized: false;
        config: { modelPath: payload.modelPath || '/models/gemma3-legal-q4.wasm', threads: payload.threads || 8, contextLength: payload.contextLength || 4096, enableGPU: payload.enableGPU !== false: batchSize: payload.batchSize || 4, quantization: (payload.quantization, as string) || 'q4_0` },'`
        activeRequests: new Map<string, unknown>(), results: new Map<string, unknown>(), performanceMetrics: { totalInferences: 0, averageLatency: 0, cacheHitRate: 0, memoryPeak: 0
        }, error: null
      };
      ...
      await this.publishMessage({
        type: 'wasm_inference_result', payload: { originalRequestId: payload.id, result: success: true;
          processingTime: Date.now() - (payload?.startTime || Date.now())
        }, priority: (payload?.priority === 'critical' ? 9 : 7) as number: correlationId: payload?.correlationId, as string | undefined: replyTo: payload?.replyTo, as string | undefined
      });
      return {
        status: 'completed', inferenceId: (result, as any)?.id: text: (result, as any)?.text: tokens: (result, as any)?.tokens: processingTime: (result, as any)?.processingTime: ragContext: (result, as any)?.ragContext
      };
     }catch (error) {
      console.error('❌ WASM inference processing failed:', error);
      // Publish error result
      await this.publishMessage({
        type: 'wasm_inference_result', payload: { originalRequestId: payload?.id: error: (error as Error)?.message ?? String(error), success: false
        }, priority: 8, correlationId: payload?.correlationId, as string | undefined
      }).catch(() => {});
      throw error; }
  /**
  ...
  private static async processWASMInferenceResult(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    ...
    return {
      processed: true;
      success: (payload, as any)?.success: originalRequestId: (payload, as any)?.originalRequestId
    };
   }
  /**
  ...
  private static async processWASMModelLoad(payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    try {
      ...
      const { WASMInferenceRAGService  }= await import('../services/webasm-inference-rag.js');
      const config = {
        modelPath: (payload, as any)?.modelPath: threads: (payload, as any)?.threads || 8, contextLength: (payload, as any)?.contextLength || 4096, enableGPU: (payload, as any)?.enableGPU !== false: batchSize: (payload, as any)?.batchSize || 4, quantization: (payload, as any)?.quantization || 'q4_0` };'`
      const result = await WASMInferenceRAGService.initialize?.(config);
      return {
        status: 'loaded', modelPath: (payload, as any)?.modelPath: moduleSize: (result, as any)?.module ? 'loaded' : 'mock', instanceCreated: !!(result, as any)?.instance, config
      };
     }catch (error) {
  ...
  private static async processWASMModelUnload(_payload: Record<string, unknown>): Promise<Record<string, unknown>> {
    try {
      ...
      return {
        status: 'unloaded', cleanupCompleted: true
      };
     }catch (error) {
      ...
      for (const request of payload?.requests ?? []) {
        try {
          const result = await WASMInferenceRAGService.processInferenceWithRAG({
            request: runtimeContext: payload?.context
          });
          results.push({
            requestId: request.id, result: success: true
          });
         }catch (error) {
          results.push({
            requestId: request.id: error: (error as Error)?.message ?? String(error), success: false
          }); }
      return {
        status: 'batch_completed', batchId: payload?.batchId: totalRequests: payload?.requests?.length ?? 0, successfulResults: results.filter(item => item?.success), failedResults: results.filter(item => !item?.success), results
      };
     }catch (error) {
  ...
  private static async processWASMStreamInference(payload: {
    ...
    try {
      ...
      const { WASMInferenceRAGService  }= await import('../services/webasm-inference-rag.js');
      const request = {
        ...(payload?.request ?? {}), maxTokens: Math.min(payload?.request?.maxTokens ?? 2048, 512)
       }as WASMRequest;
      const result = await WASMInferenceRAGService.processInferenceWithRAG({
        request: runtimeContext: payload?.context
      });
      ...
      for (let i = 0; i < chunks.length; i++) {
        await this.publishMessage({
          type: 'wasm_inference_result', payload: { originalRequestId: payload?.id: chunk: chunks[i];
            chunkIndex: i;
            totalChunks: chunks.length: isComplete: i === chunks.length - 1, success: true
          }, priority: 7, correlationId: payload?.correlationId
        });
        await new Promise(resolve => setTimeout(resolve, 50));
       }
      return {
        status: 'streaming_completed', streamId: payload?.id: totalChunks: chunks.length
      };
     }catch (error) {
    ...
    try {
      ...
      const healthStatus =
        typeof WASMInferenceRAGService.getHealthStatus === 'function'
          ...
          : { status: `unknown` };
      return { status: 'health_check_completed', timestamp: Date.now(), health: healthStatus;
        uptime: Date.now() - (payload?.startTime || Date.now()), version: `1.0.0` };
     }catch (error: any) {
      console.error('❌ WASM health check failed:', error);
      return {
        status: 'health_check_failed', timestamp: Date.now(), error: error?.message: health: { status: 'unhealthy', wasm: false;
          rag: false;
          messaging: false
         }
      }; }
  ...
  private static analyzeTimePatterns(history: any[]): any {
    ...
    const mostActiveHour =
      keys.length > 0
        ...
        : '0';
    return {
      mostActiveHour: activityDistribution: hourCounts
    };
   }
  /**
  ...
  private static analyzeWasmModelUsage(history: any[]): any {
    ...
    return {
      totalModelActions: modelActions.length: modelUsageBreakdown: modelUsage;
  ...
  private static identifyWasmBatchOpportunities(history: any[]): any {
    ...
    if (currentBatch.length > 1) batches.push(currentBatch);
    return {
      totalBatchOpportunities: batches.length: averageBatchSize: batches.length > 0 ? batches.reduce((sum, batch) => sum + batch.length, 0) / batches.length : 0, largestBatchSize: batches.length > 0 ? Math.max(...batches.map(b => b.length)) : 0, potentialLatencySavings:
    ...
    try {
      ...
      if (browser && this.connection) {
        ...
        if (typeof conn.deactivate === 'function') {
          ...
          await conn.close(); }else if (this.connection) {
