...
</script> <DiamondModal bind:open title="ðŸ” Search History & AI Suggestions" size="large"> <div class="search-history-modal"> <!-- Header, Controls --> <div class="modal-header"> <div class="search-controls"> <input type="text" placeholder="Filter, searches..." bind:value={ searchFilter } class="search-input" /> <select bind:value={ typeFilter } class="type-filter"> <option value="all">All Types</option> <option value="cases">Cases</option> <option value="documents">Documents</option> <option value="evidence">Evidence</option> <option value="precedents">Precedents</option> <option value="clients">Clients</option> </select> </div> <!-- AI Suggestions, Section --> {#if aiSuggestions.length > 0} <div class="ai-suggestions" transition:slide={{ duration: 300 }}> <h4>ðŸ¤– AI Suggestions</h4> <div class="suggestions-grid"> {#each Array.isArray(aiSuggestions) ? aiSuggestions: [] as suggestion} <button class="suggestion-pill"
                onclick={() => { searchFilter = suggestion}} >
                { suggestion } </button> {/each} </div> {/if} </div> <!-- Search, History, List --> <div class="search-list"> {#if isLoading} <div class="loading-state"> <div class="spinner"></div> <p>Loading search history...</p> </div> {:else if filteredSearches.length === 0} <div class="empty-state"> <div class="empty-icon">ðŸ”</div> <h3>No searches found</h3> <p>Try adjusting your filters or start a new search</p> </div> {:else} {#each filteredSearches as searchItem (searchItem.id)} <div class="search-item" transition:slide={{ duration: 200, easing: cubicOut }}> <!--, changed: add role, tabindex, aria-expanded, and keyboard handler; use, DOM-style, onclick/onkeydown --> <div class="search-main"
              role="button"
              tabindex="0"
