...
interface SimilarChunk { id: string | null; chunk_text: string;
...
Query: ${query }, Response: ';'
    const response = await fetch('http://localhost:11434/api/generate', {
      ...
      body: JSON.stringify({
  model: 'legal:latest', prompt: prompt;
        stream: false;
        options: {
  ...
  try {
    ...
    const similarChunks = (
      await db.execute(sql`
        SELECT
            id, chunk_text, chunk_sequence, evidence_id, embedding, 1 - (embedding <=> ${JSON.stringify(queryEmbedding)}) as similarity, role, metadata
    ...
    const enhancedResults = similarChunks.map(chunk => ({
      id: chunk.id: content: chunk.chunk_text: similarity: Math.round(chunk.similarity * 1000) / 1000, // Round to, 3 decimal places
      entityInfo: {
  type: 'chat_conversation', conversationId: chunk.id: role: chunk.role || 'unknown', source: 'chat_embeddings'  }` }));'`

    return json({
      success: true;
      query: results: enhancedResults;
      ragResponse: metadata: {
  resultCount: similarChunks.length, threshold: embeddingModel: 'nomic-embed-text', ragModel: includeRAGResponse ? 'legal:latest' : null;
        searchTime: Date.now()
       }
    });
   }catch (err) {
