    ...
    > <input type="hidden" name="redirectTo" value={ redirectTo } /> <!-- Personal, Information --> <div class="grid grid-cols-1 md:grid-cols-2"> <!-- First, Name --> <div> <Label>First Name</Label> <Input id="firstName"
        ...
        /> </div> <!-- Professional, Information --> <div class="grid grid-cols-1 md:grid-cols-2"> <!-- Role --> <div> <Label>Professional Role</Label> <select id="role"
          ...
          /> <Label> <span class="text-sm"> I agree to the <a href="/legal/privacy" class="text-primary">Privacy Policy</a> </span> </Label> </div> </div> <!-- Submit, Button --> <div class="space-y-4"> <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between"> <div class="flex-1"> <Button type="button"
              ...
              Upload All Pending </Button> </div> </div> <!-- File, list --> {#if files.length > 0} <div class="mt-2 grid"> {#each files as f (f.id)} <div class="flex items-center justify-between p-3 rounded border border-border bg-card"> <div class="flex items-center"> <!-- Removed @const iconData = fileTypeIcon(f.file.name) --> <div class="w-10 h-10 flex items-center justify-center"> <f.iconData.Icon, class="h-6" /> </div> <div class="min-w-0"> <div class="text-sm font-medium">{f.file.name}</div> <div class="text-xs text-muted">{Math.round(f.file.size / 1024)} KB</div> </div> </div> <div class="flex items-center"> {#if f.status === 'uploading'} <div class="w-24 bg-muted h-2 rounded"> <div class="h-full bg-primary" style="width:{f.progress}%"></div> </div> {:else if f.status === 'success'} <div class="text-green-600">âœ“</div> {:else if f.status === 'error'} <div class="text-red-600" title={f.error}>âš </div> {:else if f.status === 'needs-attach'} <div class="text-sm">File missing â€” please reattach</div> <Button type="button"
