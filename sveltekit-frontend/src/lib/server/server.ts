...
async function generateEmbedding(text: string): Promise<number[]> {
  console.log(`[Embedding] Generating embedding for text: "${text.substring(0, Math.min(text.length, 50))}..."`);
  // Simulate an embedding generation (e.g., 768-dimension embedding)
  return Array.from({ length: 768 }, () => Math.random());
}

// Initialize RedisService once
...
export async function getCachedEmbedding(text: string): Promise<number[]> {
  const cacheKey = `legal:embedding:${Buffer.from(text).toString('base64url')}`;
  const cached = await redisService.get<CachedEmbedding>(cacheKey);

  if (cached?.embedding) {
    console.log(`[Redis] Cache HIT for embedding: ${cacheKey}`);
    return cached.embedding;
  }

  console.log(`[Redis] Cache MISS for embedding: ${cacheKey}`);
  const embedding = await generateEmbedding(text);

  await redisService.cacheEmbedding(cacheKey, embedding, { cached_at: new Date().toISOString() });
  return embedding;
}
 ...
 */
export async function getCachedSearchResults(query: string, computeFn: () => Promise<unknown[]>): Promise<unknown[]> {
  const queryHash = Buffer.from(query).toString('base64url');
  const cached = await redisService.get<CachedSearch>(`legal:search:${queryHash}`);
  ...
  await redisService.cacheSearchResults(query, results, 1800); // 30min TTL
  console.log(`[Redis] Cache MISS for query: ${query}`);
  return results;
}
