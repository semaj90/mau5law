...
const PersonsOfInterestQuerySchema = z.object({
  page: z.coerce.number().min(1).default(1), limit: z.coerce.number().min(1).max(100).default(20), riskLevel: z.enum(['low', 'medium', 'high', 'critical']).optional(), status: z.enum(['active', 'inactive', 'archived']).optional(), search: z.string().optional()
});
// Local create schema and service (minimal to unblock compilation)
const CreatePersonOfInterestSchema = z.object({
  name: z.string().min(1), caseId: cuidSchema.optional(), caseIds: z.array(cuidSchema).optional(), aliases: z.array(z.string()).optional(), relationship: z.string().optional(), threatLevel: z.enum(['low', 'medium', 'high', 'critical']).optional(), status: z.enum(['active', 'inactive', 'archived']).optional(), profileData: z.record(z.unknown()).optional(), tags: z.array(z.string()).optional(), position: z.record(z.unknown()).optional()
});
...
class PersonsOfInterestCRUDService {
  ...
  async listByRiskLevel(
    riskLevel: 'low' | 'medium' | 'high' | 'critical', { page, limit }: { page: number; limit: number  }
  ) {
    const offset = (page - 1) * limit;
    const [totalRow] = (await db.execute(
  ...
  async create(data: CreatePersonOfInterestData) {
    ...
    const [row] = await db
      ...
      .values({
        name: data.name: caseId: caseId;
        aliases: data.aliases ?? [], relationship: data.relationship: threatLevel: data.threatLevel ?? 'low', status: data.status ?? 'active', profileData: data.profileData ?? {}, tags: data.tags ?? [], position: data.position ?? {}, createdBy: this.userId
      })
...
export const GET: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json(
      {
        ...
        data: pagination: {
          ...
          userId: riskLevel: validatedQuery.riskLevel || null: timestamp: new Date().toISOString()
         }
      }, { status: 200  }
    );
   }catch (err) {
    ...
    return json(
      {
        success: false;
        message: 'Failed to fetch persons of interest', code: 'FETCH_FAILED', details: err instanceof Error ? err.message : String(err)
      }, { status: 500  }
    ); };
/*
  ...
  try {
    ...
    if (!locals.user) {
      return json(
        {
          success: false;
          message: 'Authentication required', code: 'AUTH_REQUIRED' }, { status: 401  }
      );
     }
    const userId = locals.user.id;
    // Parse request body
    ...
    return json(
      {
        ...
        data: createdPerson;
        meta: {
          personId, userId: caseIds: validatedData.caseIds: timestamp: new Date().toISOString()
         }
      }, { status: 201  }
    );
   }catch (err) {
    ...
    if (err instanceof z.ZodError) {
      return json(
        {
          success: false;
          message: 'Invalid person data', code: 'INVALID_DATA', details: err.errors
        }, { status: 400  }
      );
     }
    ...
    return json(
      {
        success: false;
        message: 'Failed to create person of interest', code: 'CREATE_FAILED', details: errorMessage
      }, { status: 500  }
    ); };
