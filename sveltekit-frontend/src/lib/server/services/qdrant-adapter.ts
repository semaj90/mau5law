...
export async function upsertVector(
    ...
    const client: any = QdrantVectorService;
...
export async function searchVector(
    ...
    if (typeof client.search === 'function') {
        return await client.search({ collection_name: collection, vector, limit });
    }
    if (typeof client.searchPoints === 'function') {
        return await client.searchPoints({ collectionName: collection, vector, limit });
    }
    if (typeof client.request === 'function') {
        return await client.request('POST', `/collections/${collection}/points/search`, { vector, limit });
    }
