...
`;
  ...
  private initializeCollections() {
    ...
    this.cacheCollection = this.lokiDB.getCollection('cache') || this.lokiDB.addCollection('cache', { indices: ['key', 'timestamp'], ttl: 300000 });
  }
  ...
  private async computeErrorEmbeddingsGPU(errors: NPMError[]): Promise<Float32Array[]> {
    ...
    for (const error of errors) {
      ...
      const configBuffer = this.device.createBuffer({ size: 8, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
  ...
  private async refineRankingWithWebGPU(todos: IntelligentTodo[]): Promise<IntelligentTodo[]> {
    ...
    const paramsBuffer = this.device.createBuffer({ size: 12, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
