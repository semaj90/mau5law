            ...
            if (first?.actor && second?.actor && first.actor === second.actor) {
                ...
                if (travelTime > 0 && first?.time && second?.time) {
                    if (Math.abs(first.time - second.time) < travelTime) {
                        // Using 'impossible-presence' as fallback since 'travel-time-violation' isn't in the strict type definition yet
                        contradictions.push({
                            type: 'impossible-presence',
                            first,
                            second,
                            details: {
	travelTimeRequiredMs: travelTime, actualTimeDiffMs: Math.abs(first.time - second.time) },
	});
                    }
                }
            }
            ...
            // Duration contradiction check (overlapping events for same actor)
            if (first?.time && first?.endTime && second?.time && first.actor === second.actor) {
                 if (second.time < first.endTime && second.time >= first.time) {
                     contradictions.push({
