...
class PerformanceMonitor {
  ...
  private async collectMetrics(): Promise<PerformanceSnapshot> {
    ...
    const ai: AIMetrics = { documentsProcessed: 0, averageProcessingTime: 0, totalProcessingTime: 0,
      ...
      modelUsage: {},
      processingTimeHistory: toSeries((s) => s.ai.averageProcessingTime),
      confidenceHistory: toSeries((s) => s.ai.averageConfidence),
      throughputHistory: toSeries((s) => s.ai.documentsProcessed)
    } }
    const queues: QueueMetrics = { queues: {}, totalJobs: 0, totalCompleted: 0, totalFailed: 0, overallThroughput: 0, healthScore: 100,
    ...
    const cache: CacheMetrics = { hitRate: 0, missRate: 1, evictionRate: 0, averageAccessTime: 0,
    ...
    const system: SystemMetrics = { healthScore: 100, uptime: Date.now()-this.startTime,
