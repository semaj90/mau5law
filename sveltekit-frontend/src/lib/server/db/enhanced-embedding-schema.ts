...
import { pgTable, uuid, varchar, text, timestamp, integer, real, boolean, jsonb, bigint, index } from 'drizzle-orm/pg-core';
...
export const vectorOperations = {
  ...
  fromVector: (vec: string): number[] => vec.replace(/[\[\]]/g, '').split(',').map(n => parseFloat(n.trim())),
