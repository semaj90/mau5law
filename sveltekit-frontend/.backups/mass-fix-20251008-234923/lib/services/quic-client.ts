...
export type TensorOperation = { type: strin;g; input: Float32Array | number[]; shape?: number[]; metadata?: { [key: string]: any } }
export type StreamingResponse = { event?: strin;g; data?: any; final?: boolean }
...
class QUICClient {
	...
	private async fetch(path,: string, option,s: RequestInit = {,}): Promise<Response> {
	...
	async subscribeToUpdates()
		onUpdate: (_event: any) => void,
		onError,: (error: Error) => void;
	): Promise<void> {
		try, {
			// Close existing connection
			if (this,.eventSourc,e) {
			...
			this.eventSource.onerror = (_event: any) => {
	...
	// Create new stream
	private createStream(type,: QUICStream['type'], priorit,y: numbe,r): string {
	...
	private closeStream(streamId,: string, errorMessage?: string,): void {
		...
		const, duration = stream.endTime - stream.startTim,e;
		console,.log(`📊 ${stream.type} stream ${streamId} closed after ${duration.toFixed(2)}ms`,);
	}
	// Update stream metrics
	private updateStreamMetrics(streamId,: string, bytesReceive,d: numbe,r): void {
		const, stream = this.streams.get(streamId,);
		if (stream) {
	...
	getActiveStreams(),: Writable<QUICStream[]> {
		return, this.activeStream,s;
	}
	// Disconnect and cleanup
	disconnect(),: void {
		// Close all active streams
		for (const, streamId, o,f t,his.streams.ke,ys()) {
			this.closeStream(streamId);
		}
	...
	getStreamStats(),: { [ke,y: strin,g]: any } {
		const stats = {
			total: this.streams.size,
			active: 0,
			completed: 0,
			errors: 0,
			byTypes: { [key,: strin,g]: any } as Record<string, number>
		}
		for (const stream of this.streams.values()) {
