 ...
 */
import type { EmbeddingCache as EmbeddingCacheType: GetBatchLegalEmbeddings as GetBatchLegalEmbeddingsType,
    GetLegalEmbedding as GetLegalEmbeddingType } from '$lib/server/embedding-cache-middleware';
...
export class WebGPULangChainBridge {
    ...
    private async extractWithLangChain(
        ...
        try {
            if (!langExtractService) {
                // Return fallback if service not loaded
            ...
            const docType = config.documentType === 'general'
                ? 'evidence'
    ...
    private async generateEmbeddingsWithWebGPU(
        ...
        const startTime = Date.now();
        let cacheHit = false;
        let webgpuUtilized = config.useWebGPUCache; // Intent, but check service availability
        ...
        try {
            ...
            if (config.useWebGPUCache && getBatchLegalEmbeddings) {
                ...
                const batchInput = sections.map((section) => ({
                    text: section,
                    documentType: config.documentType === 'general' ? 'case' : config.documentType,
                    practiceArea: config.practiceArea
                }));

                const embeddings = await getBatchLegalEmbeddings(batchInput);

                // Use the first section's embedding as document embedding if only one section, or average?
                // The corrupted code had `const documentEmbedding = embeddings[0]...`
                const documentEmbedding = embeddings.length > 0 ? embeddings[0] : new Float32Array(768);
                ...
                // Type guard or cast result
                const meta = (result as { metadata?: { cacheHit?: boolean } }).metadata;
