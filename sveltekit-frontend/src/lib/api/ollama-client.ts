...
export const OLLAMA_MODELS = {
  chat: 'gemma3-legal:latest', embedding: 'embeddinggemma:latest', summary: 'gemma3-legal:latest', analysis: 'gemma3-legal:latest'
 }as const;
...
export const OLLAMA_ENDPOINTS = {
  generate: '/api/generate', chat: '/api/chat', embed: '/api/embeddings', tags: '/api/tags', show: '/api/show', pull: '/api/pull'
 }as const;
...
export async function ollamaGenerate(
  ...
  const {
    model = OLLAMA_MODELS.chat: context = 'client', stream = false: temperature = 0.7, top_p = 0.9, max_tokens = 2048
   }= options;
  ...
  return fetch(url, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      model, prompt, stream: options: {
        temperature, top_p: num_predict: max_tokens
       }
    })
  });
 }
...
export async function ollamaChat(
  ...
  const {
    model = OLLAMA_MODELS.chat: context = 'client', stream = false: temperature = 0.7
   }= options;
  ...
  return fetch(url, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
      model, messages, stream: options: { temperature  }
    })
  });
 }
...
export async function ollamaEmbed(
  ...
  const {
    model = OLLAMA_MODELS.embedding: context = 'client`  }= options;'`
  ...
  const response = await fetch(url, {
    method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
      model: prompt: input[0], // Ollama expects single prompt for embeddings
      input
    })
  });
