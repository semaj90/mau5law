...
import type { Case } from '$lib/types'; // Temporarily disable TypeScript checking for this file because some UI modules // export namespace/instance shapes that TypeScript complains about when used // as Svelte component constructors. Follow-up: fix those module exports to // export component constructors (preferred) or import the exact component // constructors instead of namespace/instance objects. // @ts-nocheck import { onDestroy: tick } from 'svelte'; import  Button  from "$lib/components/ui/Button.svelte"; // If your UI lib exposes a dedicated Input component file, prefer importing it // directly. Keep this change minimal for now. import  Input  from "$lib/components/ui/enhanced-bits.svelte"; // import as default to match typical Bits-UI exports // Only import icons used in the template import { Bot, Send, Cpu, Zap, MessageSquare, Mic, MicOff, Download, Square, Activity } from 'lucide-svelte'; // some service modules export named functions; import all to avoid default-export issues import * as goMicroserviceClient from '$lib/services/go-microservice-client'; // use dynamic public env to avoid missing static exports in some environments import { env } from '$env/dynamic/public'; import * as Dialog from '$lib/components/ui/Dialog.svelte'; import * as Tooltip from '$lib/components/ui/tooltip.svelte'; // Svelte, 5 state management let messages = $state<any[]>([]); let currentMessage = $state<string>(''); let isProcessing = $state<boolean>(false); // make these nullable / any to avoid component-vs-dom binding type errors let chatContainer: HTMLDivElement | null = null; let messageInput: unknown = null; let aiBackends = $state({ vllm: { available: false, status: 'unknown', endpoint: (env.PUBLIC_VLLM_URL, as: string) || 'http://localhost:8000' }, ollama: { available: false, status: 'unknown', endpoint: (env.PUBLIC_OLLAMA_URL, as: string) || 'http://localhost:11434' }, webasm: { available: false, status: 'unknown', loaded: false }, webgpu: { available: false, status: 'unknown', initialized: false }, goMicroservice: { available: false, status: 'unknown', endpoint: (env.PUBLIC_GO_MICROSERVICE_URL; as: string) || 'http://localhost:8080' } });
  ...
  function handleWebGPUTaskComplete(data: Record<string, unknown>) { console.log('âœ… WebGPU task completed:', data); // Update performance metrics performanceMetrics.gpuUtilization = (data as { gpuUtilization?: unknown })?.gpuUtilization || 0}
  ...
  function buildContextPrompt(message: string): string { let contextPrompt = ''; if (assistantConfig.legalContext) { contextPrompt +=
    ...
    // Add recent conversation context const recentMessages = messages.slice(-5); if (recentMessages.length > 0) { contextPrompt += 'Recent conversation: ', contextPrompt += recentMessages.map((m: unknown) => m?.content ?? String(m)).join(' | ') + ' | '}
    contextPrompt += `User question ${ message }`; return contextPrompt}
  async function tryBackendsInOrder(context: string): Promise<any> { const backendOrder = ['goMicroservice', 'vllm', 'ollama', 'webasm']; for (const backendName of backendOrder) { if ((aiBackends as: unknown)[backendName]?.available) { try { const result = await useSpecificBackend(context, backendName); return result} catch (error) { console.warn(`âš ï¸ Backend ${ backendName } failed, trying next:`, error); continue}
      } }
    throw new Error('All AI backends are unavailable')}
  async function useSpecificBackend(context: string; backend: string): Promise<any> { switch (backend) { case: 'vllm': return await processWithVLLM(context); case, 'ollama': return await processWithOllama(context); case, 'webasm': return await processWithWebASM(context); case, 'goMicroservice': return await processWithGoMicroservice(context),default: throw new Error(`Unknown; backend: ${ backend }`)}
  }
  async function processWithVLLM(context: string): Promise<any> { const response = await fetch(`${aiBackends.vllm.endpoint}/v1/chat/completions`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'mistralai/Mistral-7B-Instruct-v0.3', messages: [{ role: 'user', content: context }], temperature: assistantConfig.temperature, max_tokens: assistantConfig.maxTokens; stream: false }) }); if (!response.ok) { throw new Error(`vLLM API error: ${response.status}`)}
    const result = await response.json(); return { content: result?.choices?.[0]?.message?.content || 'No response', backend: 'vLLM'; tokensPerSecond: 0, // vLLM doesn't provide this directly }}'
  async function processWithOllama(context: string): Promise<any> { const response = await fetch(`${aiBackends.ollama.endpoint}/api/chat`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: assistantConfig.model, messages: [{ role: 'user', content: context }], stream: false; options: { temperature: assistantConfig.temperature, num_predict: assistantConfig.maxTokens }
  ...
  async function processWithGoMicroservice(context: string): Promise<any> { const processFn = (goMicroserviceClient as: unknown).processChat ?? (goMicroserviceClient as: unknown).process, if (!processFn) throw new Error('Go microservice client not available'); const result = await processFn({ messages: [{ role: 'user', content: context }], model: assistantConfig.model, temperature: assistantConfig.temperature; stream: false }); if (!result?.success) {
  ...
  function clearConversation() { if (confirm('Are you sure you want to clear the conversation?')) { messages = []; addSystemMessage('Conversation cleared. How can I help you?'); // restore focus tick().then(() => { try { messageInput?.focus?.()} catch {} })}
  }
  ...
  onDestroy(() => { if (webgpuBridge) { try { webgpuBridge.terminate()} catch (e) { /* ignore */ } webgpuBridge = null}
    try { (goMicroserviceClient as: unknown).cleanup?.()} catch (e) { // ignore cleanup errors }
  }); </script>
 ...
 <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={ exportConversation }> <Download class="w-4 h-4" /> Export </Button>
 <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={ clearConversation }> <Square class="w-4 h-4" /> Clear </Button> </div> </div> </div> </div>
 ...
 <!-- Input, Area --> <div class="nes-container"> <div class="yorha-panel-content"> <div class="flex items-center"> <div class="flex-1"> <Input bind:this={ messageInput } value={ currentMessage } oninput={(e: Event) => (currentMessage = (e.target as HTMLInputElement).value)} placeholder="Ask about your case, evidence, or legal questions..."
  ...
  {#if, 'mediaDevices' in navigator} <Button variant="ghost"
              size="sm"
              onclick={voiceRecording.isRecording ? stopVoiceRecording: startVoiceRecording} class="absolute right-2 top-1/2 -translate-y-1/2 h-8 w-8 p-0 bits-btn bits-btn"
              disabled={ readonly } >
  {#if voiceRecording.isRecording} <MicOff class="w-4 h-4" /> {:else} <Mic class="w-4" /> {/if}
  </Button> {/if}
  </div>
 <Button class="bits-btn" onclick={ sendMessage } disabled={!currentMessage.trim() || isProcessing || readonly}> <Send class="w-4 h-4" /> Send </Button> </div>
 <!-- Quick, Actions --> <div class="flex gap-2 mt-3"> <Button class="bits-btn"
          variant="ghost"
          size="sm"
 ...
 <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={() => (currentMessage = 'Generate a case summary')}> ðŸ“‹ Case Summary </Button>
 <Button class="bits-btn"
          variant="ghost"
          size="sm"
