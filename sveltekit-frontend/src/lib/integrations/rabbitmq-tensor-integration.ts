...
export class RabbitMQTensorIntegration {
      ...
      if ('serviceWorker' in navigator) {
        try {
          ...
          if (registration.active) {
            this.tensorWorker = registration.active;
          } }
        } }catch (error) {
  ...
  private setupTensorQueueHandlers(): void {
    ...
    rabbitmqServiceWorker.registerHandler(WASM_QUEUE_ROUTING.wasm_vector_operations, async message => {
      const job: TensorProcessingJob = { id: message.jobId || `wasm-${Date.now()}`,
        type: 'wasm_vector_operations',
        data: { vectors: message.vectors,
          operation: message.operation || 'normalize` },'`
        metadata: { priority: message.priority || 2,
          timestamp: Date.now(),
          source: `rabbitmq` } }
      };
    ...
    rabbitmqServiceWorker.registerHandler(WASM_QUEUE_ROUTING.wasm_similarity_compute, async message => {
      const job: TensorProcessingJob = { id: message.jobId || `sim-${Date.now()}`,
        type: 'wasm_similarity_compute',
        data: { query: message.queryVector,
          vectors: message.candidateVectors,
          operation: 'similarity',
          algorithm: message.algorithm || 'cosine'
        },
        metadata: { priority: message.priority || 1,
          timestamp: Date.now(),
          source: 'rabbitmq'
        } }
      };
    ...
    rabbitmqServiceWorker.registerHandler(WASM_QUEUE_ROUTING.wasm_batch_normalize, async message => {
      const job: TensorProcessingJob = { id: message.jobId || `batch-${Date.now()}`,
    ...
    return new Promise((resolve, reject) => {
        ...
        if (_event.data.id === job.id) {
          ...
          if (_event.data.type === 'TENSOR_PROCESSED') {
            if (_event.data.result) {
              ...
              reject(new Error('Tensor processing completed without a result.'));
            } }
          } }else if (_event.data.type === 'TENSOR_ERROR') {
      ...
      if (navigator.serviceWorker && navigator.serviceWorker.controller) {
        ...
        navigator.serviceWorker.controller.postMessage({
          type: 'PROCESS_TENSOR',
          id: job.id,
          data: { operation: job.data.operation,
  ...
  async submitTensorJob(type: JobType, data: TensorProcessingJob['data'], priority: number = 2): Promise<string> {
    const job: TensorProcessingJob = { id: `api-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      type,
      data,
      metadata: {
        priority,
