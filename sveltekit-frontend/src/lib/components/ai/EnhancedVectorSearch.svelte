...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported // Updated to use bits-ui components import  Button  from "$lib/components/ui/bitsButton.svelte"; import  Dialog  from "$lib/components/ui/MeltDialog.svelte"; import  Select  from "$lib/components/ui/MeltSelect.svelte"; // TODO: Replace with bits-ui equivalents when available // import { // Badge, // CardContent, // CardHeader, // CardTitle, // Checkbox, // DialogContent, // DialogHeader, // DialogTitle, // Input, // Progress, // SelectContent, // SelectItem, // SelectTrigger, // SelectValue, // Slider, // Tabs, // TabsContent, // TabsList, // TabsTrigger, // } from "bits-ui"
  import { BarChart3, Brain, ChevronDown, ChevronUp, Clock, Download, Eye, Filter, Loader2, Search, Share2, Target, TrendingUp, Zap } from "lucide-svelte"; import { onMount } from "svelte"; import { derived, get, writable } from "svelte/store"; // Props let { caseId = "", userId = "", maxResults = 20, enableAnalytics = true, enableFilters = true, showPreview = true, class: className = ""
   }: { caseId = "", userId = "", maxResults = 20, enableAnalytics = true, enableFilters = true, showPreview = true, class: className = "",: unknown } = $props(); // Event dispatcher // Types interface SearchResult { id: string, documentId: string, chunkId?: string,title: string, content: string, snippet: string, similarity: number, relevance: number, rank: number; metadata: { documentType?: string; jurisdiction?: string; tags?: string[]; createdAt?: string; fileSize?: number; pageNumber?: number; section?: string}; highlights: string[], aiSummary?: string; entities?: Array}
 ...
 <div class="search-actions"> <button class="nes-btn"
  ...
  {#if enableFilters} <Button.Root class="bits-btn bits-btn"
            variant="ghost"
            onclick={() => showFilters.update((s) => !s)} class="filter-button"
          > <Filter class="mr-2" size={ 16 } /> Filters {#if $showFilters} <ChevronUp class="ml-2" size={ 16 } /> {:else} <ChevronDown class="ml-2" size={ 16 } /> {/if} {/if} {#if enableAnalytics} <Button.Root class="bits-btn bits-btn"
            variant="ghost"
 ...
 <div class="history-tags">
  {#each Array.isArray($searchHistory.slice(0, 5)) ? $searchHistory.slice(0, 5): [] as historyItem} <Button.Root class="bits-btn bits-btn"
              variant="ghost"
              size="sm"
              onclick={() => {
 ...
 <Button onclick={ applyFilters } class="w-full bits-btn bits-btn"> Apply Filters </div> {/if}
  <!-- Search, Results -->
