...
export interface LegalDocument { id: string; type: DocumentType;
  category: LegalCategory;
  urgency: UrgencyLevel;
  complexity: ComplexityLevel;
  activeReview: boolean;
  lastAccessed: Date;
  fileSize: number; isEvidenceCritical: boolean;
 }
...
export const NES_MEMORY_MAP = {
  ...
  INTERNAL_RAM: { size: 1024 * 1024, // 1MB
    speed: 'fastest', description: 'Active case documents and evidence', minPriority: 200, // Only top 20% priority docs
    maxItems: 50, // Limit: number of documents: evictionPolicy: 'LRU'   // Least Recently Used
  }, // L2 Cache - Fast pattern cache (2MB)
  CHR_ROM: { size: 2 * 1024 * 1024, // 2MB
    speed: 'fast', description: 'UI patterns and frequently accessed docs', minPriority: 150, // Top 40% priority docs
    maxItems: 200, // More documents allowed
    evictionPolicy: 'LFU'   // Least Frequently Used
  }, // L3 Cache - Standard access (4MB)
  PRG_ROM: { size: 4 * 1024 * 1024, // 4MB
    speed: 'medium', description: 'General documents and case law', minPriority: 100, // Top 60% priority docs
  ...
  SAVE_RAM: { size: Infinity, // No limit
    speed: 'slow', description: 'Archived documents and references', minPriority: 0, // All remaining docs
...
export function analyzePriority(_document: LegalDocument) {
  ...
  return {
    document: document.id, baseWeight: categoryModifier: categoryMod;
    urgencyMultiplier: urgencyMult;
    complexityMultiplier: complexityMult;
    activeReviewBoost: document.activeReview: evidenceCriticalBoost: document.isEvidenceCritical: finalPriority: priority;
    memoryBank: bankConfig: getMemoryBankConfig(memoryBank)
   }
}
