/**
 * 🎮 REDIS-OPTIMIZED ENDPOINT - Laws & Statutes
 *
 * Endpoint: legal/laws
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (type === 'precedents') {
      ...
      const conditions = [];
      if (jurisdiction) conditions.push(eq(legalPrecedents.jurisdiction, jurisdiction));
      if (search) conditions.push(like(legalPrecedents.summary, `%${search}%`));
      ...
      let query = db
        ...
        .orderBy(desc(statutes.code))
        .limit(limit);

      const conditions = [eq(statutes.isActive, true)];
      if (jurisdiction) conditions.push(eq(statutes.jurisdiction, jurisdiction));
      if (category) conditions.push(eq(statutes.category, category));
      if (search) conditions.push(like(statutes.title, `%${search}%`));
