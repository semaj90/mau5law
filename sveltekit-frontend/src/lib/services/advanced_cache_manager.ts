...
class AdvancedCacheManager {
  ...
  async lazyLoad<T>(
    ...
    const { priority = 'medium', prefetch = false, legal_priority = false, ...cacheOptions } = options;
  ...
  async prefetchByPattern(
    patterns: string[],
    options?: { legal_only?: boolean }
  ): Promise<void> {
  ...
  async exportLegalData(options?: {
    ...
    for (const [key, item] of this.cache.entries()) {
      ...
      if (document_types.length > 0 && !document_types.includes(item.document_type || 'general')) continue;
      if (date_range && (item.timestamp < date_range.start || item.timestamp > date_range.end)) continue;
