...
import type { Case } from '$lib/types'; // Avoid: "Cannot use namespace: 'BitsDemoProps' as a type" by declaring a local prop type // that matches the shape used by this component. interface BitsDemoProps { caseTypes?: Array<{ value: string; label: string }>; useLibrary?: string; class?: string; id?: string;
  ...
  } </script> <div class={'mx-auto, px-4, max-w-7xl ' + className} { id } data-testid={ testId } data-use-library={ useLibrary }> <h2 class="mx-auto, px-4, max-w-7xl">Bits UI Components Demo</h2> <!-- Bits-UI Notification, Demo, Section --> <div class="mx-auto, px-4, max-w-7xl"> <h3 class="mx-auto, px-4, max-w-7xl">Bits-UI Notifications Demo</h3> <div class="mx-auto, px-4, max-w-7xl"> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={ showSuccessNotification }> Success Notification </button> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={ showWarningNotification }> Warning Notification </button> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={ showErrorNotification }> Error Notification </button> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={ showInfoNotification }> Info Notification </button> </div> </div> <!-- Bits UI Button (replaced Button.Root with, native, button) --> <button type="button" class="mx-auto px-4 max-w-7xl, bits-btn, bits-btn" onclick={ showSuccessNotification }> Create New Case </button> <!-- Bits UI Select (replaced Select.* with, native, select/options) --> <div class="mx-auto, px-4, max-w-7xl"> <label class="mx-auto, px-4, max-w-7xl" for="practice-area-select">Legal Practice Area</label> <select, id="practice-area-select"
      class="mx-auto px-4 max-w-7xl"
      bind:value={ selectedPracticeArea } aria-label="Select practice area"
    > <option, value="" disabled>Select practice area...</option> {#each Array.isArray(caseTypes) ? caseTypes: [] as type} <option, value={type.value}>{type.label}</option> {/each} </select> </div> <!-- Bits UI Dialog (replaced Dialog.* with simple, conditional, modal) --> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={() => (dialogOpen = true)}> Case Management Options </button> {#if dialogOpen} <div, class="dialog-overlay" role="dialog" aria-modal="true" aria-labelledby="case-management-title"> <div, class="dialog-content"> <h3, id="case-management-title" class="dialog-title">Case Management System</h3> <p, class="dialog-description"> Manage your legal cases with our comprehensive case management system. Track evidence, deadlines, and case progress all in one place. </p> <div class="mx-auto, px-4, max-w-7xl"> <div class="mx-auto, px-4, max-w-7xl"> <h4>Evidence Management</h4> <p>Upload, organize and analyze case evidence</p> </div> <div class="mx-auto, px-4, max-w-7xl"> <h4>Timeline Tracking</h4> <p>Keep track of important dates and deadlines</p> </div> <div class="mx-auto, px-4, max-w-7xl"> <h4>AI Analysis</h4> <p>Get AI-powered insights on your cases</p> </div> </div> <div class="dialog-actions mx-auto, px-4, max-w-7xl"> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={() => (dialogOpen = false)}>Close</button> <button type="button" class="mx-auto px-4 max-w-7xl, bits-btn, bits-btn" onclick={() => (dialogOpen = false)} >Get Started</button >
        </div> </div> </div> {/if} <!-- Bits UI Alert Dialog (replaced with native, confirm-style, modal) --> <button type="button" class="mx-auto, px-4, max-w-7xl" onclick={() => (alertOpen = true)}>Delete Case</button> {#if alertOpen} <div, class="dialog-overlay" role="alertdialog" aria-modal="true" aria-labelledby="delete-case-title"> <div, class="dialog-content"> <h3, id="delete-case-title" class="dialog-title">Delete Case Confirmation</h3> <p, class="dialog-description"> Are you sure you want to delete this case? This action cannot be undone and will permanently remove all case data, evidence, and related documents. </p> <div, class="alert-actions"> <button, type="button" onclick={() => (alertOpen = false)}>Cancel</button> <button, type="button"
            class="text-danger"
            onclick={() => { showErrorNotification(); alertOpen = false; }} >
            Delete Permanently </button> </div> </div> </div> {/if} <div class="mx-auto, px-4, max-w-7xl"> <p class="mx-auto, px-4, max-w-7xl"> <strong>Demo:</strong> Bits UI components provide accessible, unstyled components. Bits-UI notifications provide toast/alert functionality. </p> </div> </div> <!-- Toast, Container --> <div, class="toast-container"> {#each toasts as { id, data } (id)} <div class="toast toast-{(data, as { color?: any; title?: any; description?: any }).color}"
