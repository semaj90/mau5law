...
operations: testData })
      ...
      if (result.success) {
        addTestResult(
          'success',
          `Performance test completed: ${result.summary?.successful ?? 0}/${testSize} operations in ${duration}ms`
        );
      } else {
          ...
          <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
            ...
            <div class="space-y-1">
              <div class="flex justify-between">
                <span>Requests:</span>
                <span class="font-mono">{cacheStats.service?.requests ?? 0}</span>
              </div>
              <div class="flex justify-between">
                <span>Hits:</span> <span class="font-mono">{cacheStats.service?.hits ?? 0}</span>
              </div>
              <div class="flex justify-between">
                <span>Hit Rate:</span>
                <span class="font-mono">{formatPercentage(cacheStats.service?.hitRate ?? 0)}</span>
              </div>
            </div>
          </div>
            ...
            {#each cacheStats.layers.layers as layer}
              <div class="bg-gray-800 border border-gray-700 rounded-lg p-6">
                ...
                <div class="space-y-1">
                  <div class="flex justify-between">
                    <span>Items:</span>
                    <span class="font-mono">{layer.itemCount?.toLocaleString() ?? 0}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Hit Rate:</span>
                    <span class="font-mono">{formatPercentage(layer.hitRate ?? 0)}</span>
                  </div>
                  <div class="flex justify-between">
                    <span>Memory:</span>
