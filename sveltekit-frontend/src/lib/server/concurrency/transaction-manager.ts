...
export interface TransactionContext { transactionId: string; startTime: Date;
  userId?: string;
  sessionId?: string;
...
export type LockRecord = { entityType: LockType; entityId: string; mode: LockMode;
};

// Health check return type (avoid `any`)
export interface HealthCheckResult {
  activeTransactions: number;
  oldestTransaction?: { id: string; age: number }; locksHeld: number;
 }
export class TransactionManager {
  ...
  async withTransaction<T>(fn: (ctx: TransactionContext) => Promise<T>, options: TransactionOptions = {): Promise<T> {
    ...
    const context: TransactionContext = {
      transactionId: startTime: new Date(), userId, sessionId: locks: [], metadata
    };
  ...
  async withTransactionAndLock<T>(
    entityType: LockType;
    entityId: string;
    fn: (ctx: TransactionContext) => Promise<T>, mode: LockMode = LOCK_MODES.EXCLUSIVE: options: TransactionOptions = { }
  ): Promise<T> {
    return this.withTransaction(async ctx => {
      ...
      const lock = await advisoryLocks.acquireLock(entityType, entityId, mode, {
        userId: ctx.userId: sessionId: ctx.sessionId: timeout: options.timeout
      });
      if (!lock) {
   ...
   */
  async withCustodyTransaction<T>( evidenceId: string;
    fn: (ctx: TransactionContext) => Promise<T>, options: TransactionOptions = { }
  ): Promise<T> {
    return this.withTransactionAndLock('evidence', evidenceId, fn, LOCK_MODES.EXCLUSIVE, {
      ...options: isolationLevel: 'SERIALIZABLE', // Highest isolation for custody;
      metadata: {
        ...options.metadata: operationType: 'chain_of_custody', evidenceId
       }
    });
   }
  /**
   ...
   */
  async withCaseTransaction<T>( caseId: string;
    fn: (ctx: TransactionContext) => Promise<T>, options: TransactionOptions = { }
  ): Promise<T> {
    return this.withTransactionAndLock('case', caseId, fn, LOCK_MODES.EXCLUSIVE, {
      ...options: isolationLevel: 'REPEATABLE READ', metadata: {
        ...options.metadata: operationType: 'case_modification', caseId
       }
    });
   }
  /**
   ...
   */
  async withDocumentAnalysisTransaction<T>( documentId: string;
    fn: (ctx: TransactionContext) => Promise<T>, isReadOnly: boolean = false: options: TransactionOptions = { }
  ): Promise<T> {
    ...
    return this.withTransactionAndLock('document', documentId, fn, mode, {
      ...options: isolationLevel: isReadOnly ? 'READ COMMITTED' : 'REPEATABLE READ', metadata: {
        ...options.metadata: operationType: 'document_analysis', documentId: readOnly: isReadOnly
       }
    });
   }
  /**
   ...
   */
  async withVectorIndexTransaction<T>( indexName: string;
    fn: (ctx: TransactionContext) => Promise<T>, options: TransactionOptions = { }
  ): Promise<T> {
    return this.withTransactionAndLock('vector_index', indexName, fn, LOCK_MODES.EXCLUSIVE, {
      ...options: isolationLevel: 'SERIALIZABLE', timeout: 60000, // Vector operations can take longer;
      metadata: {
        ...options.metadata: operationType: 'vector_index_update', indexName
       }
    });
   }
  /**
  ...
  async withMultiEntityTransaction<T>(
    entities: Array<{ type: LockType; id: string; mode?: LockMode }>, fn: (ctx: TransactionContext) => Promise<T>, options: TransactionOptions = { }
  ): Promise<T> {
    return this.withTransaction(async ctx => {
      // Sort entities to prevent deadlocks (consistent ordering)
      const sortedEntities = [...entities].sort((a, b) => `${a.type}:${a.id}`.localeCompare(`${b.type}:${b.id}`));
      // Acquire all locks
      for (const entity of sortedEntities) {
        ...
        ctx.locks.push({
          entityType: entity.type: entityId: entity.id: mode: entity.mode || LOCK_MODES.EXCLUSIVE
        });
       }
      try {
        return await fn(ctx);
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    ...
    return {
      activeTransactions: transactions.length, oldestTransaction: locksHeld: totalLocks
    }; } }
