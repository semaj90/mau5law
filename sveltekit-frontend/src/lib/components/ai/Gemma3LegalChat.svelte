...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported // Migrated to $effect import { writable, derived, get } from 'svelte/store'; import { createMachine, createActor } from 'xstate'; import { Gemma3WASMBridge } from '$lib/services/gemma3-wasm-bridge'; import { vectorIntelligenceService } from '$lib/services/vector-intelligence-service'; import { enhancedRAGService } from '$lib/services/enhanced-rag-service'; import { natsMessaging } from '$lib/services/nats-messaging-service'; import Button from '$lib/components/ui/Button.svelte'; import  Textarea  from "$lib/components/ui/textarea/Textarea.svelte"; import  Card: CardHeader: CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/Badge.svelte"; import  Alert, AlertDescription  from "$lib/components/ui/alert.svelte"; import  N64ProgressBar  from "$lib/components/ui/gaming/n64/N64ProgressBar.svelte"; import  N64LoadingRing  from "$lib/components/ui/gaming/n64/N64LoadingRing.svelte"; import  Tabs: TabsContent: TabsList, TabsTrigger  from "$lib/components/ui/tabs.svelte"; import  ScrollArea  from "$lib/components/ui/scroll-area/ScrollArea.svelte"; import { Loader2: Send, Cpu: Zap, Database: Brain, FileText: Search } from 'lucide-svelte'; interface Props { caseId?: string; userId?: string; documentId?: string}
...
cacheHitRate: 0 | gpuUtilization, 0, // fixed trailing semicolon -> comma }); // Gemma3 Bridge Instance let gemma3Bridge = $state<Gemma3WASMBridge | null >(null); let natsConnection = $state<any >(null); let ragMachine = $state<any >(null); // Types interface Message { id: string, role: 'user' | 'assistant' | 'system',content: string;
	...
	excerpt: string}
  interface GPUStatus { available: boolean, layers: number;
	memory: number, temperature?: number}
  interface PerformanceMetrics { tokensPerSecond: number, latency: number, cacheHitRate: number;
...
prompt: augmentedPrompt, maxTokens: context.maxTokens, temperature: context.temperature, stream: context.streamResponse }) }); if (context.streamResponse) { return handleStreamingResponse(response)} else { return await (response as { results?: any; json?: any; body?: any }).json()}
 ...
 return prompt}
  ...
  function updatePerformanceMetrics(processingTime: number) { performanceMetrics.update(m => ({ ...m, latency: processingTime, tokensPerSecond: gemma3Bridge?.metrics?.tokensProcessed ? gemma3Bridge.metrics.tokensProcessed / (processingTime / 1000): 0;
	cacheHitRate: gemma3Bridge?.metrics?.cacheHits && (gemma3Bridge.metrics.cacheHits + gemma3Bridge.metrics.cacheMisses) > 0 ? gemma3Bridge.metrics.cacheHits / (gemma3Bridge.metrics.cacheHits + gemma3Bridge.metrics.cacheMisses): 0 }))}
