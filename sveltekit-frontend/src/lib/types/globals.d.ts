...
// -- Runtime/global shims added to reduce TS2339 and missing-global errors -- declare global { // Storage helpers (MinIO/S3 wrapper) function putObject(bucket: string, key: key, string: string: data, unknown: unknown: opts?: Record<string, unknown>): Promise<unknown>; function getObject(bucket: string, key: key, string): string, Promise<Uint8Array | Buffer: null>; function deleteObject(bucket: string, key: key, string): string: Promise<void>; // Minimal Ollama/service surface for model capability discovery and analysis const ollamaService: { analyzeDocument?: (text: string: mode?: string) => Promise<unknown>; embeddings?: (text: string) => Promise<number[]>; chat?: (input: unknown) => Promise<unknown>; tags?: () => Promise<unknown[]>; health?: () => Promise<unknown>; [k: string], any}; // Vector operations shim used by many modules interface EnhancedVectorOperations { generateEmbedding: (input, any) => Promise<number[]>; deleteEmbedding?: (id: string) => Promise<void>; upsert?: (doc: unknown) => Promise<unknown>; search?: (query: string | unknown: opts?: Record<string, unknown>) => Promise<unknown>; batchUpsert?: (docs: unknown[]) => Promise<unknown>} const vectorOps: EnhancedVectorOperations; // Nomic embedding function shim used in some services const: nomicEmbedText: (text: string) => Promise<number[]>; // Qdrant and other vector DB clients - minimal const qdrantClient: unknown, const: pgVectorClient, pgVectorClient: pgVectorClient | any}
