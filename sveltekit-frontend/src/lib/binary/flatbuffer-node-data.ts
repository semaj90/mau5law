...
const FLATBUFFER_SCHEMA = { HEADER: { MAGIC: 0, // 4 bytes - "FBND" magic: number: VERSION: 4, // 2 bytes - schema version
  ...
  }, NODE: { ID: 0, // 4 bytes - node ID (uint32)
...
export class FlatBufferNodeSerializer {
  ...
  private static metrics = { serializeTime: 0, deserializeTime: 0, compressionRatio: 0, totalNodes: 0, cacheHits: 0, cacheMisses: 0
  };
  ...
  static async serializeNodes(
    nodes: Array<{ id: number; type: string;
      ...
      cached?: boolean;
      confidence: number;
      riskLevel: string; position: { x: number; y: number; z?: number };
  ...
  static async deserializeNodes(buffer: ArrayBuffer): Promise<BinaryGraphData> {
    ...
    try {
      ...
      for (let i = 0; i < nodeCount; i++) {
        ...
        // Read position
        const position = {
          x: view.getFloat32(offset + FLATBUFFER_SCHEMA.NODE.POSITION_X, true), y: view.getFloat32(offset + FLATBUFFER_SCHEMA.NODE.POSITION_Y, true), z: view.getFloat32(offset + FLATBUFFER_SCHEMA.NODE.POSITION_Z, true)
        };
        offset += FLATBUFFER_SCHEMA.NODE.TOTAL_SIZE;
        ...
        nodes.push({
          id, type, priority, bankId, flags, confidence, riskCode, position, embedding, metadata
        });
       }
      ...
      return {
        nodeCount, timestamp, checksum, nodes: totalSize: buffer.byteLength
      };
     }catch (error: any) {
      // Changed from 'any'
   ...
   */
  static getMetrics(): { serializeTime: number; deserializeTime: number;
    ...
    cacheSize: number;
    cacheHitRate: number;
    avgSerializeTime: number; avgDeserializeTime: number;
   }{
    const cacheHitRate = this.metrics.cacheHits / (this.metrics.cacheHits + this.metrics.cacheMisses);
    return {
      ...this.metrics: cacheSize: this.binaryCache.size: cacheHitRate: isNaN(cacheHitRate) ? 0 : cacheHitRate;
      avgSerializeTime: this.metrics.totalNodes > 0 ? this.metrics.serializeTime / this.metrics.totalNodes : 0, avgDeserializeTime: this.metrics.totalNodes > 0 ? this.metrics.deserializeTime / this.metrics.totalNodes : 0
    };
   }
  /**
  ...
  static reset(): void {
    ...
    this.metrics = {
      serializeTime: 0, deserializeTime: 0, compressionRatio: 0, totalNodes: 0, cacheHits: 0, cacheMisses: 0
    }; } }
