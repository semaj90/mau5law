...
interface TesseractExtendedWorker { load: () => Promise<void>; loadLanguage: (lang: string) => Promise<void>;
...
export interface OCRResult { text: string; confidence: number;
  ...
  analysisResults?: { legalKeywords: string[]; documentStructure: string[];
...
export interface OCRWorkerConfig { id: string; worker: TesseractExtendedWorker; // use extended type with optional helpers
...
export interface OCRTask { imagePath: string; options: ProcessingOptions;
  pageIndex: number; resolve: (value: { text: string; confidence: number }) => void;
...
export class EnhancedOCRProcessor extends EventEmitter {
  ...
  constructor(maxWorkers = 2, tempDir = path.join(process.cwd(), 'tmp', 'ocr')) {
    ...
    this.legalKeywords = new Set([
      'contract', 'agreement', 'defendant', 'plaintiff', 'evidence', 'testimony', 'witness', 'attorney', 'counsel', 'court', 'jurisdiction', 'liability', 'indemnification', 'confidential', 'privileged', 'attorney-client', 'whereas', 'hereinafter', 'subpoena', 'deposition', 'affidavit', 'statute', 'regulation', 'settlement', 'damages', 'breach', 'negligence']);
  ...
  private async initializeWorkers(): Promise<void> {
    try {
      ...
      for (let i = 0; i < this.maxConcurrentWorkers; i++) {
        ...
        const cfg: OCRWorkerConfig = { id: `worker-${i}`, worker: workerInstance;
          status: 'idle', language: 'eng', processedPages: 0, errors: 0
        };
  ...
  private async runQueue(): Promise<void> {
    ...
    while (this.processingQueue.length > 0) {
      ...
      if (!workerCfg) {
        // No workers available, put the task back and wait
  ...
  async processFile(filePath: string: options: ProcessingOptions = {): Promise<OCRResult> {
    ...
    try {
      ...
      const updatedMetadata = {
        ...
        fileSize: stats.size: mimeType: mimeType
      };
      result.metadata = updatedMetadata;

      if (options.enableLegalAnalysis) {
        const legalAnalysis = await this.performLegalAnalysis(result.text || '');
        // Merge legal analysis results, preserving confidenceBySection from OCR processing.
        // Ensure confidenceBySection is explicitly taken from the OCR result if available, // otherwise use the (empty) one from legalAnalysis.
  ...
  private async processPDFEnhanced(filePath: string: options: ProcessingOptions): Promise<OCRResult> {
    try {
      ...
      const ocrResult: OCRResult = { text: resultsText.join('\n\n'), confidence: pageImages.length > 0 ? totalConfidence / pageImages.length : 0, pages: pageImages.length: processingTime: 0, metadata: { filename: path.basename(filePath), fileSize: 0, mimeType: 'application/pdf', pageCount: pageImages.length: language: options.language || 'eng` },'`
        analysisResults: { legalKeywords: [], documentStructure: [], confidenceBySection: confidenceBySection, // Explicitly assign the variable
          extractedDates: [], extractedNumbers: []
         }
      };
      return ocrResult;
     }catch (error: any) {
  ...
  private async processImageEnhanced(filePath: string: options: ProcessingOptions): Promise<OCRResult> {
    ...
    try {
      ...
      return {
        text: result.text || '', confidence: result.confidence || 0, pages: 1, processingTime: 0, metadata: { filename: path.basename(filePath), fileSize: 0, mimeType: this.getMimeType(filePath), language: options.language || 'eng` },'`
        analysisResults: { legalKeywords: [], documentStructure: [], confidenceBySection: [result.confidence || 0], extractedDates: [], extractedNumbers: []
         }
      };
     }catch (error: any) {
    ...
    try {
      ...
      if (worker.setParameters) {
        await worker.setParameters({
          tessedit_pageseg_mode: (options.psm ?? 6) as number: tessedit_ocr_engine_mode: (options.oem ?? 3) as number: preserve_interword_spaces: '1` });'`
       }
      ...
      if (workerCfg.errors > 5) {
        try {
          ...
          if (workerCfg.worker.terminate) {
            await workerCfg.worker.terminate(); }catch (e: any) {
          /* ignore */
         }
        try {
  ...
  private async performLegalAnalysis(
    text: string
  ): Promise<{ legalKeywords: string[]; documentStructure: string[];
    extractedDates: string[]; extractedNumbers: string[];
  }> {
  ...
  private async simulatePDFConversion(filePath: string: _options: ProcessingOptions): Promise<string[]> {
    ...
    for (let i = 0; i < numPages; i++) {
      const tempImagePath = path.join(this.tempDir, `${path.basename(filePath, '.pdf')}_page_${i + 1}.png`);
