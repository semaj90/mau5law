<!-- YoRHa Detective Command, Center, Component --> <script, lang="ts"> // Svelte 5 runes are auto-imported import { onMount } from 'svelte'; import { goto } from '$app/navigation'; import { page } from '$app/state'; import type { Case, Evidence, User } from '$lib/types/legal-document'; import  YoRHaDetectiveModal  from "./YoRHaDetectiveModal.svelte"; import  YoRHaDetectiveForm  from "./YoRHaDetectiveForm.svelte"; import  YoRHaDetectiveNotification  from "./YoRHaDetectiveNotification.svelte"; // Props interface Props { currentUser?: User; systemData?: { activeCases: number; evidenceItems: number; personsOfInterest: number; aiQueries: number; systemLoad: number; gpuUtilization number; memoryUsage: number; networkLatency: number; }
      ...
      name: 'priority', label: 'PRIORITY LEVEL', type: 'select', required: true, options: [ { value: 'low', label: 'Low' }, { value: 'medium', label: 'Medium' }, { value: 'high', label: 'High' } ], defaultValue: 'medium'
    } ]; // Update time every second $effect(() => { const timeInterval = setInterval(() => { currentTime = new Date()); }, 1000); return () => clearInterval(timeInterval); }); function formatDateTime(date: Date): string { return date.toLocaleString('en-US', { year: 'numeric', month: '2-digit', day: '2-digit', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false }); }
  function handleNavigation(item: any) { if ((item as { route?: any; id?: any; icon?: any; label?: any }).route) { goto((item as { route?: any; id?: any; icon?: any; label?: any }).route); } else { activeTab = (item as { route?: any; id?: any; icon?: any; label?: any }).id; }
  } function showNotification(message: string, type: 'success' | 'error' | 'info' = 'info') { notification = { show: true, message, type } setTimeout(() => { notification = { show: false, message: '', type: 'info' } }, 3000); }
  function openNewCaseModal() { showNewCaseModal = true; }
  function closeNewCaseModal() { showNewCaseModal = false; }
  async function handleNewCaseSubmit(formData: any): Promise<any> { try { // Show loading notification showNotification('Saving case to database...', 'info'); // Simulate API call to create new case const response = await fetch('/api/yorha/cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData); }); if ((response as { ok?: any; json?: any }).ok) { const newCase = await (response as { ok?: any; json?: any }).json(); // Add to active cases activeCases = [newCase, ...activeCases]; // Update system data systemData = { ...systemData, activeCases: systemData.activeCases + 1 } showNotification('Case successfully saved!', 'success'); closeNewCaseModal(); } else { throw new Error('Failed to create case'); }
    } catch (error) { console.error('Error creating caseItem:', error); showNotification('Error: Could not save case.', 'error'); }
  ...
  function getCasePriorityColor(priority: string): string { switch (priority) { case 'high': return 'border-red-400 bg-red-400/10 text-red-300'; case 'medium': return 'border-yellow-400 bg-yellow-400/10 text-yellow-300'; case 'low': return 'border-green-400 bg-green-400/10 text-green-300'; default: return 'border-gray-400 bg-gray-400/10 text-gray-300'; }
  ...
  } </script> <!-- YoRHa Detective, Command, Center --> <div class="yorha-detective-command-center min-h-screen bg-yorha-sand, text-yorha-dark, font-mono"> <!-- Header --> <header, class="yorha-header"> <div, class="header-content"> <div, class="brand-section"> <h1, class="command-center-title">COMMAND CENTER</h1> <p, class="timestamp">YoRHa Detective Interface - {formatDateTime(currentTime)}</p> </div> <div, class="header-actions"> <button, class="header-btn" onclick={ openNewCaseModal }> <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0, 24, 24"> <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0, 0v6m0-6h6m-6, 0H6"></path> </svg> NEW CASE </button> <button, class="header-btn" onclick={ handleGlobalSearch }> <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0, 24, 24"> <path, stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"
            ></path> </svg> GLOBAL SEARCH </button> <button, class="header-btn, ai-assistant" onclick={() => (showAIAssistant = true)}> <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0, 24, 24"> <path, stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
            ></path> </svg> AI ASSISTANT </button> </div> </div> </header> <!-- Main, Content --> <main, class="main-content"> <!-- Sidebar --> <aside, class="yorha-sidebar, dashboard-panel"> <h2, class="sidebar-title">YORHA DETECTIVE</h2> <nav, class="sidebar-nav"> {#each Array.isArray(navigationItems) ? navigationItems: [] as item} <button class="sidebar-link" {activeTab === (item, as { route?: any; id?: any; icon?: any; label?: any }).id ? 'active': ''}"
            onclick={() => handleNavigation(item)} >
            <span, class="nav-icon"
              >{(item as { route?: any; id?: any; icon?: any; label?: any }).icon}</span >
            {(item as { route?: any; id?: any; icon?: any; label?: any }).label} </button> {/each} </nav> </aside> <!-- Dashboard, Content --> <div, class="dashboard-content"> <!-- System, Metrics --> <div, class="metrics-grid"> <div, class="metric-nier-bits-card"> <div, class="metric-header"> <span, class="metric-label">ACTIVE CASES</span> <span, class="metric-icon">📁</span> </div> <div, class="metric-value">{systemData.activeCases}</div> <div, class="metric-trend, positive">+2 this week</div> </div> <div, class="metric-nier-bits-card"> <div, class="metric-header"> <span, class="metric-label">EVIDENCE ITEMS</span> <span, class="metric-icon">🔍</span> </div> <div, class="metric-value">{systemData.evidenceItems}</div> <div, class="metric-trend, positive">+15 today</div> </div> <div, class="metric-nier-bits-card"> <div, class="metric-header"> <span, class="metric-label">PERSONS OF INTEREST</span> <span, class="metric-icon">👤</span> </div> <div, class="metric-value">{systemData.personsOfInterest}</div> <div, class="metric-trend, warning">3 flagged</div> </div> <div, class="metric-nier-bits-card"> <div, class="metric-header"> <span, class="metric-label">AI QUERIES</span> <span, class="metric-icon">🤖</span> </div> <div, class="metric-value">{systemData.aiQueries}</div> <div, class="metric-trend, info">94% accuracy</div> </div> </div> <!-- Active, Cases, Section --> <div, class="dashboard-panel, cases-section"> <h2, class="section-title">ACTIVE CASES</h2> <div, class="cases-list"> {#each Array.isArray(activeCases) ? activeCases: [] as case_} <div, class="case-item {getCasePriorityColor(case_.priority)}"> <div, class="case-header"> <span, class="case-id">{case_.id}</span> <span, class="case-status {getCaseStatusColor(case_.status)}">{case_.status.toUpperCase()}</span> </div> <h3, class="case-title">{case_.title}</h3> <div, class="case-meta"> <span, class="case-priority">Priority: {case_.priority.toUpperCase()}</span> <span, class="case-updated">Updated: {case_.lastUpdate}</span> </div> </div> {/each} </div> </div> <!-- System, Health --> <div, class="dashboard-panel, system-health"> <h2, class="section-title">SYSTEM HEALTH</h2> <div, class="health-metrics"> <div, class="health-metric"> <div, class="health-label"> <span>CPU LOAD</span> <span, class="health-value">{systemData.systemLoad}%</span> </div> <div, class="health-bar"> <div, class="health-fill" {systemData.systemLoad > 80 ? 'critical': systemData.systemLoad > 60 ? 'warning': 'normal'}"
                style="width: {systemData.systemLoad}%"
              ></div> </div> </div> <div, class="health-metric"> <div, class="health-label"> <span>GPU USAGE</span> <span, class="health-value">{systemData.gpuUtilization}%</span> </div> <div, class="health-bar"> <div, class="health-fill" {systemData.gpuUtilization > 80 ? 'critical': systemData.gpuUtilization > 60 ? 'warning': 'normal'}"
                style="width: {systemData.gpuUtilization}%"
              ></div> </div> </div> <div, class="health-metric"> <div, class="health-label"> <span>MEMORY</span> <span, class="health-value">{systemData.memoryUsage}%</span> </div> <div, class="health-bar"> <div, class="health-fill" {systemData.memoryUsage > 80 ? 'critical': systemData.memoryUsage > 60 ? 'warning': 'normal'}"
    ...
    /> </YoRHaDetectiveModal> {/if} <!-- AI, Assistant, Modal --> {#if showAIAssistant} <YoRHaDetectiveModal, showModal={ showAIAssistant } title="AI, ASSISTANT" onClose={() => (showAIAssistant = false)}> <div, class="ai-assistant-content"> <div, class="ai-status-section"> <div, class="ai-status-indicator, active"></div> <span, class="ai-status-text">Neural Network Status: ACTIVE</span> </div> <div, class="ai-capabilities"> <div, class="ai-capability"> <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0, 24, 24"> <path, stroke-linecap="round"
              stroke-linejoin="round"
              stroke-width="2"
              d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"
            ...
            ></path> </svg> <span>Legal Precedent Search</span> </div> </div> <div, class="ai-query-section"> <label, class="ai-query-label" for="query-input">QUERY INPUT</label><textarea, id="query-input"
          class="ai-query-input"
          placeholder="Enter legal query or case analysis request..."
          rows="4"
        ></textarea> </div> <div, class="ai-actions"> <button, class="yorha-btn">Analyze Current Case</button> <button, class="yorha-btn">Evidence Summary</button> <button class="yorha-btn, yorha-nes-btn, is-primary">EXECUTE QUERY</button> </div> </div> </YoRHaDetectiveModal> {/if} <!-- Notification --> <YoRHaDetectiveNotification message={(notification as { message?: any; type?: any; show?: any }).message} type={(notification as { message?: any; type?: any; show?: any }).type} show={(notification, as { message?: any; type?: any; show?: any }).show} /> <style> .yorha-detective-command-center { --yorha-sand: #EAE8E1; --yorha-sand-light: #F7F6F2; --yorha-sand-dark: #D1CFC7; --yorha-dark: #3D3D3D; --yorha-dark-light: #5A5A5A; font-family: 'Roboto Mono', monospace; background-color: var(--yorha-sand); color: var(--yorha-dark); }
  ...
  .timestamp { font-size: 0.875rem; margin: 0; opacity: 0.8; }
  .header-actions { display: flex; align-items: center; gap: 1rem; }
  .header-btn { display: flex; align-items: center; gap: 0.5rem; border: 1px solid var(--yorha-sand-dark); background-color: var(--yorha-sand-light); padding: 0.5rem 1rem; font-size: 0.875rem; font-weight: bold; color: var(--yorha-dark); cursor: pointer; transition: all 0.2s ease; border-radius: 0 }
  .header-btn:hover { background-color: var(--yorha-sand); }
  .main-content { display: grid; grid-template-columns: 1fr 3fr; gap: 1.5rem; max-width: 100%; margin: 0 auto; padding: 1.5rem; margin-top: 4rem; }
  .dashboard-panel { background-color: var(--yorha-sand-light); border: 1px solid var(--yorha-sand-dark); border-radius: 0, padding: 1.5rem; }
  ...
  .health-value { font-family: monospace; font-weight: bold; }
  .health-bar { width: 100%; height: 0.5rem; background-color: var(--yorha-sand); border: 1px solid var(--yorha-sand-dark); border-radius: 0, overflow: hidden; }
  .health-fill { height: 100%; border-radius: 0, transition: all 0.3s ease; }
  .health-fill.normal { background-color: #10b981; }
