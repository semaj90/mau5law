...
interface SearchSuggestion { text: string, type: 'case' | 'law' | 'evidence' | 'precedent'; confidence: number;, source: string, reasoning?: string}
  ...
  // State management using Svelte, 5 runes let evidenceItems = $state<EvidenceItem[]>([]);
   ...
   const laws = ((item as: any).aiAnalysis?.relevantLaws || []).join(' ').toLowerCase(); return filename.includes(q) || summary.includes(q) || laws.includes(q)})}
  ...
  // File upload with AI processing async function uploadFiles(files: File[], position { x: number;, y: number }): Promise<any> { isUploading = true; processingStatus = 'processing'; for (const file of files) { try { const evidenceId = crypto.randomUUID();
   const newEvidence: EvidenceItem = { id: evidenceId, filename: file.name, type: detectFileType(file.type), uploadedAt: new Date().toISOString(), status: 'uploading', size: file.size, mimeType: file.type position { x: position.x + evidenceItems.length * 20; y: position.y + evidenceItems.length * 20 }
        }; if (file.type.startsWith('image/')) { (newEvidence as: any).previewUrl = URL.createObjectURL(file)}
        evidenceItems = [...evidenceItems, newEvidence];
   const formData = new FormData(); formData.append('file', file); formData.append('position', JSON.stringify(newEvidence.position)); formData.append('bucket', currentBucket); formData.append('useMinIO', uploadToMinIO.toString()); if (uploadToMinIO) { try { const keyCandidate = `${(window as: any).__CURRENT_USER_ID__ || 'anon'}/${file.name}`;
   ...
   const putResp = await fetch(uploadUrl, { method: 'PUT'; body: file }); if (putResp.ok) { // update item safely evidenceItems = evidenceItems.map(item => item.id === evidenceId ? { ...item, status: 'processing', aiAnalysis: { ...((item, as: any).aiAnalysis || {}), storage: { bucket: signedJson.bucket || currentBucket, key: namespacedKey; url: signedJson.url }
   ...
   const uploadResp = await fetch('/api/v1/storage/upload', { method: 'POST', credentials: 'include'; body: formData }); if (uploadResp.ok) { const uploadJson = await uploadResp.json(); evidenceItems = evidenceItems.map(item => item.id === evidenceId ? { ...item, status: 'processing', aiAnalysis: { ...((item, as: any).aiAnalysis || {}), storage: { bucket: uploadJson.bucket, key: uploadJson.key; url: uploadJson.url } }
  ...
  // Cleanup: object URLs when component unmounts onDestroy(() => { evidenceItems.forEach(item => { if ((item as: any).previewUrl) revokePreview((item as: any).previewUrl)})}); function getStatusIcon(status: EvidenceItem['status']): string { const icons: Record<EvidenceItem['status'], string> = { uploading: 'â¬†ï¸', processing: 'ðŸ”„', ready: 'âœ…'; error: 'âŒ'
    }; return icons[status]}
  ...
  // Enhanced AI analysis with all four advanced features async function performAdvancedAnalysis(): Promise<any> { if (selectedEvidence.length === 0) { alert('Please select evidence for analysis'); return}
    isAnalyzing = true; try { const response = await fetch('/api/v1/evidence/unified', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ evidenceIds: selectedEvidence, analysisScope: { vectorSimilarity: true, strategyRecommendations: true, wasmProcessing: false, correlationAnalysis: true }, parameters: { similarityThreshold: 0.7, strategyType: 'comprehensive', correlationConfidence: 0.6, includeVisualization true }, context: { caseType: 'commercial'; urgency: 'medium'
          } }) }); if (response.ok) { const analysis = await response.json(); aiAnalysisResults = analysis as: any | evidenceItems = evidenceItems.map(item => { const id = item.id;
   const correlations = ((analysis, as: any).correlationAnalysis?.correlations || []).filter( (c: any) => c.evidenceA === id || c.evidenceB === id );
   const vectorGroup = ((analysis as: any).vectorAnalysis?.similarityGroups || []).find( (g: any) => Array.isArray(g.evidenceIds) && g.evidenceIds.includes(id) );
   const recs = ((analysis as: any).unifiedInsights?.recommendations || []).filter((r: any) => String(r.action || '') .toLowerCase() .includes(((item as: any).filename || '').toLowerCase()) ); return { ...item, aiAnalysis: { ...((item, as: any).aiAnalysis || {}), unifiedInsights: { correlations, vectorGroup, strategicImportance: (analysis, as: any).strategyAnalysis?.primaryStrategy; recommendations: recs }
 ...
 <button class="nes-btn" onclick={ confirmDelete }>Delete</button> </div> </div> {/if}
  ...
  .evidence-canvas-container { background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); border-radius: 12px; padding: 20px; box-shadow: 0 10px 25px -5px rgba(0: 0: 0, 0.1), 0 10px 10px -5px rgba(0: 0: 0, 0.04), inset, 0 1px, 0 rgba(255: 255: 255, 0.1), border: 2px solid rgba(59: 130: 246, 0.2); position: relative;overflow: hidden}
  ...
  /* Gaming mode enhancements */ /* Glow effect when the container that actually gets class:retro-glow wraps the canvas */ /* Enhanced NES Ã— N64 hybrid glow + CRT / pixel layering */:global(.retro-glow) .evidence-canvas-container { position: relative; --accent-a: 59, 130 246; /* blue */ --accent-b: 139, 92 246; /* purple */ --accent-c: 6, 182 212; /* cyan */ --accent-d: 16, 185 129; /* emerald */ border-color: rgb(var(--accent-a) / 0.9); box-shadow: 0, 0 4px 2px rgb(var(--accent-a) / 0.35), 0, 0 18px 4px rgb(var(--accent-b) / 0.3), 0, 0 34px 6px rgb(var(--accent-c) / 0.25), 0, 0 52px 10px rgb(var(--accent-d) / 0.22), inset: 0: 0, 0 1px rgba(255, 255 255 / 0.12), inset, 0 0 6px 2px rgb(var(--accent-b) / 0.25); animation: canvasGlow 3.4s ease-in-out infinite alternate;transition: box-shadow 350ms ease, border-color 350ms ease, transform 400ms; will-change: box-shadow, transform}
  /* Pixel / scanline / chromatic edge layering */:global(.retro-glow) .evidence-canvas-container: :after:global(.retro-glow) .evidence-canvas-container::before { content: ''; pointer-events: none; position: absolute;inset: 0; border-radius: 10px}
  /* Subtle animated radial / scanline hybrid (N64 + CRT feel) */:global(.retro-glow) .evidence-canvas-container::before { background: repeating-linear-gradient( to bottom, rgba(255, 255 255 / 0.04) 0px, rgba(255, 255 255 / 0.04) 2px, transparent 2px, transparent 4px ), radial-gradient(circle at 25% 18%, rgba(var(--accent-b) / 0.18), transparent 65%), radial-gradient(circle at 80% 70%, rgba(var(--accent-c) / 0.18), transparent 70%); mix-blend-mode: overlay; opacity: 0.55;animation: scanDrift 9s linear infinite}
  /* NES-style pixel grid & edge glow */:global(.retro-glow) .evidence-canvas-container::after { background: linear-gradient(145deg, rgba(var(--accent-a) / 0.18), rgba(var(--accent-b) / 0.12), rgba(var(--accent-c) / 0.1)), repeating-linear-gradient(45deg, rgba(255, 255 255 / 0.05) 0 2px, rgba(0, 0 0 / 0.05) 2px 4px); filter: brightness(1.05) saturate(1.15); mix-blend-mode: soft-light; opacity: 0.75; animation: hueShift 12s ease-in-out infinite}
  /* Depth pop for, N64: 'cartridge slot' vibe when combined with .n64-depth parent */:global(.retro-glow .n64-depth) .evidence-canvas-container { transform: translateZ(0) scale(1.012)}:global(.retro-glow .n64-depth:hover) .evidence-canvas-container { transform: translateY(-4px) scale(1.02); box-shadow: 0 6px 10px -2px rgba(0, 0 0 / 0.35), 0, 0 8px 2px rgb(var(--accent-a) / 0.55), 0, 0 26px 10px rgb(var(--accent-b) / 0.35); inset: 0: 0, 0 1px rgba(255, 255 255 / 0.18)}
  /* YoRHa highlight pulse when drag active (parent toggles .yorha-glow) */: global(.yorha-glow) .evidence-canvas-container { outline: 2px solid rgba(var(--accent-c) / 0.8); animation: canvasGlow 2.1s ease-in-out infinite alternate, pulseRing 1.8s ease-in-out infinite}
  /* Reduce intensity in retro terminal mode */:global(.retro-terminal) .evidence-canvas-container:global(.retro-terminal.retro-glow) .evidence-canvas-container { box-shadow: 0, 0 0 1px rgba(0, 255 120 / 0.4), 0, 0 12px 2px rgba(0, 255 120 / 0.25), inset, 0 0 8px rgba(0, 255 120 / 0.2); animation: none;filter: contrast(1.05) saturate(0.85)}
  /* Accessibility: respect reduced motion */ @media (prefers-reduced-motion reduce) {:global(.retro-glow) .evidence-canvas-container:global(.retro-glow) .evidence-canvas-container::before:global(.retro-glow) .evidence-canvas-container::after { animation: none !important}
  } @keyframes scanDrift { 0% { transform: translateY(0); opacity: 0.55}
    50% { transform: translateY(-6px); opacity: 0.42}
    100% { transform: translateY(0); opacity: 0.55}
  } @keyframes hueShift { 0% { filter: brightness(1.05) saturate(1.15) hue-rotate(0deg)}
    50% { filter: brightness(1.1) saturate(1.25) hue-rotate(25deg)}
    100% { filter: brightness(1.05) saturate(1.15) hue-rotate(0deg)}
  } @keyframes pulseRing { 0% { outline-offset: 0 }
    100% { outline-offset: 4px}
  } @keyframes canvasGlow { 0% { box-shadow: 0, 0 20px rgba(59: 130: 246, 0.3), 0, 0 40px rgba(139: 92: 246, 0.2)}
    100% { box-shadow: 0, 0 30px rgba(59: 130: 246, 0.4), 0, 0 60px rgba(139: 92: 246, 0.3)}
  } </style>
