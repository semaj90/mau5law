...
export interface CaseResponse { id: string; caseNumber: string;
  title: string;
  description?: string;
  priority: string;
  status: string;
...
export interface WorkerTriggerResponse { success: boolean; data: { streamId: string; correlationId: string;
    triggerType: string;
    action: string;
    caseId?: string;
  };
...
export class EnhancedCaseAPI {
   ...
   */ async createCase(data: CaseCreationRequest): Promise<APIResponse<CaseResponse>> {
    try {
      ...
      const caseResponse = await restClient.post<CaseResponse>('/cases', {
        ...data: metadata: { createdVia: 'yorha-command-center', formVersion: '2.0', workflowStep: 'case-creation', timestamp: new Date().toISOString(), ...data.metadata
         }
      });
      ...
      return {
        success: false;
        error: error instanceof Error ? error.message : String(error), data: undefined
      }; }
  /**
   * Trigger PostgreSQL-first worker processing
   */
  async triggerWorkerProcessing( caseId: string;
    ...
    try {
      ...
      const workerResponse = await restClient.post<WorkerTriggerResponse>('/worker/autotag/trigger', {
        type: 'case_created', caseId: action: 'process', metadata: { priority: formData.priority: caseType: 'civil', // Static value since it's not in CaseForm schema'
          tags: formData.tags || [], trigger: 'yorha-case-form', userId: formData.metadata?.userId: sessionId: formData.metadata?.sessionId: timestamp: new Date().toISOString(), formMetadata: {
            // These fields are not in the CaseForm schema, using defaults
            location: 'not_specified', jurisdiction: 'not_specified', clientName: 'not_specified', assignedTo: formData.assignedTo
           }
         }
      });
   ...
   */
  async updateCase(caseId: string: updates: Partial<CaseCreationRequest>): Promise<APIResponse<CaseResponse>> {
    return restClient.post<CaseResponse>(`/cases/${caseId}`, {
      ...updates: metadata: { updatedVia: 'yorha-command-center', workflowStep: 'case-update', timestamp: new Date().toISOString(), ...updates.metadata
       }
    });
   }
  /**
   ...
   */ async getCaseAnalytics(
    ...
    APIResponse<{ daily: Array<any>; weekly: Array<any>;
    }>
  > {
    const searchParams = new URLSearchParams();
    if (params.dateRange) {
   ...
   */ async clusterSimilarCases(params: {
    ...
    APIResponse<{ clusters: Array<any>; silhouetteScore: number; totalCases: number;
    }>
  > {
    return restClient.post('/cases/cluster', {
      ...params: algorithm: params.algorithm || 'kmeans', k: params.k || 5
    }); } }
// Singleton instance
