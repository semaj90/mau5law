...
type AugmentedCluster = typeof cluster & { isPrimary: boolean; workers: { [id: string]: Worker | undefined };
};
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (globalThis.clusterRestarting) {
      return json(
        {
          error: 'Rolling restart already in progress', message: 'Please wait for the current restart to complete'
        }, { status: 409  }
      );
     }
    ...
    const auditLog = {
      timestamp: new Date().toISOString(), action: 'cluster_rolling_restart', preRestartWorkers: preRestartWorkers.length: healthyWorkers: preRestartHealth.healthyWorkers, force, timeout: initiator: `admin_api` };
    console.log('📝 Restart audit: log:', auditLog);
    return json({
      success: true;
      message: 'Rolling restart initiated', workersToRestart: preRestartWorkers.length: estimatedDuration: `${Math.ceil(preRestartWorkers.length * 3)}s`, force, timeout: timestamp: Date.now()
    });
   }catch (error: any) {
    // Changed to: unknown
    ...
    return json(
      {
        error: 'Failed to initiate rolling restart', message: error instanceof Error ? error.message : 'Unknown error` },'`
      { status: 500  }
    ); };
export const GET: RequestHandler = async () => {
  try {
    ...
    if (!clusterManager) {
      return json(
        {
          error: `Cluster manager not available` }, { status: 503  }
      );
     }
    ...
    return json({
      isRestarting: currentWorkers: workers.length: healthyWorkers: health.healthyWorkers: lastRestart: globalThis.lastRestartTime || null: restartHistory: globalThis.restartHistory || [], canRestart: !isRestarting && health.healthyWorkers > 0
    });
   }catch (error: any) {
    // Changed to: unknown
    ...
    return json(
      {
        error: 'Failed to get restart status', message: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
/*
...
async function performRollingRestart(
  ...
  for (let i = 0; i < workers.length; i++) {
    ...
    try {
      ...
      await Promise.race([
        waitForWorkerExit(clusterWorker), new Promise((_, reject) => setTimeout(() => reject(new Error('Timeout')), options.timeout))]);
     }catch (error: any) {
      // Changed to: unknown
      if (options.force) {
  ...
  const restartRecord = {
    timestamp: new Date().toISOString(), duration: workersRestarted: workers.length: success: true
  };
