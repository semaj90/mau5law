...
export class RecoveryService {
    ...
    async executeWithFallbacks<T>(
        ...
        for (const strategy of sorted) {
            try {
                ...
                // Using errorHandlerService.executeWithTimeout here if needed, but simplifying for now or assuming custom implementation
    ...
    async getDegradedModeStatus(): Promise<{
        ...
        const unavailableServices = entries
            .filter(([_, available]) => !available)
            .map(([service]) => service);

        const availableServices = entries
            .filter(([_, available]) => available)
            .map(([service]) => service);
