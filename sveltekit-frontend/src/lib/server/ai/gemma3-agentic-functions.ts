...
function logError(context: string: error: unknown, details?: Record<string, unknown>) {
  console.error(
    ...
    error instanceof Error ? error.message : String(error), details ? JSON.stringify(details) : ''
  );
...
export type ExtendedCaseScoringRequest = BaseCaseScoringRequest & {
  /**
   * Additional properties relevant to scoring, such as * -; jurisdiction: string
...
interface CaseScoringServiceResult { score: number; explanation: string;
...
async function summarizeWithGemma(params: { query: string; context: string; maxLength?: number ): Promise<string> {
  ...
  try {
    const response = await fetch(`${OLLAMA_URL}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: SUMMARIZATION_MODEL;
        prompt: `Context:\n${params.context}\n\nQuestion: ${params.query}\n\nProvide a concise legal: summary:`, stream: false;
...
export async function runLegalCaseScoringAgent(request: ExtendedCaseScoringRequest): Promise<AgenticScoringResult> {
  ...
  const contextualSummary = await summarizeWithGemma({
    query: `Summarize why this case received a score of ${result.score}`, context: result.explanation
  });
 ...
 */
export interface VectorSearchResultItem { id: string; content: string; score: number; // e.g., similarity score or relevance score
 ...
 */
export interface DocumentRecord { id: string; content: string;
  score?: number;
 ...
 */
interface ParsedFunctionCall { name: string; parameters: Record<string, unknown>;
...
export const agenticFunctions = {
   ...
   */
  get_contextual_state: { description: 'Retrieve current conversation context including history, entities, and HMM state', parameters: { type: 'object', properties: { sessionId: { type: 'string', description: 'Current session ID'
   ...
   */
  predict_next_steps: { description: 'Get AI-predicted next actions based on conversation patterns and HMM state', parameters: { type: 'object', properties: { sessionId: { type: 'string', description: 'Current session ID'
        }, userId: { type: 'string', description: 'Current user ID'
         }
      }, required: ['sessionId', 'userId']
   ...
   */
  extract_legal_entities: { description: 'Extract legal entities (parties, dates, case numbers, statutes) from text', parameters: { type: 'object', properties: { text: { type: 'string', description: 'Text to extract entities from'
         }
      }, required: ['text']
   ...
   */
  get_conversation_summary: { description: 'Get summary of recent conversation turns for context', parameters: { type: 'object', properties: { sessionId: { type: 'string', description: 'Current session ID'
        }, userId: { type: 'string', description: 'Current user ID'
        }, maxTurns: { type: 'number', description: 'Maximum: number of turns to include (default: 5)'
         }
      }, required: ['sessionId', 'userId']
   ...
   */
  get_session_stats: { description: 'Get analytics and statistics for current conversation session', parameters: { type: 'object', properties: { sessionId: { type: 'string', description: 'Current session ID'
        }, userId: { type: 'string', description: 'Current user ID'  }` },'`
      required: ['sessionId', 'userId']
   ...
   */
  voice_to_text: { description: 'Transcribe audio to text using Whisper.cpp or WebGPU fallback', parameters: { type: 'object', properties: { audioPath: { type: 'string', description: `Path to audio file` }, sessionId: { type: 'string', description: `Session ID for caching`  }
      }, required: ['audioPath']
      ...
      try {
        const response = await fetch(`${WHISPER_URL}/transcribe`, {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ audioPath: params.audioPath })
        });
   ...
   */
  retrieve_relevant_docs: { description: 'Retrieve relevant documents using Qdrant + pgvector hybrid search', parameters: { type: 'object', properties: { query: { type: 'string', description: 'Search query'
        }, topK: { type: 'number', description: 'Number of documents to retrieve (default: 8)'
        }, sessionId: { type: 'string', description: 'Session ID for context filtering'
        }, // Add a filter for document type if needed
        documentType: { type: 'string', description: 'Optional filter for document type (e.g., "case", "statute")'
         }
      }, required: ['query']
      ...
      try {
        ...
        const embeddingResult = await embeddingGemma.embed(params.query, {
          embeddingType: 'message', useCache: true
        });
        ...
        const results: VectorSearchResultItem[] = await VectorSearchService.hybridSearch(
          embeddingResult.embedding, params.query, // Pass the original query for keyword search if supported
          topK, filters
        );
        ...
        return results.map(r => ({
          ...
          score: r.score: meta: r.meta || {}, // Assuming meta is returned or can be empty
   ...
   */
  rerank_documents: { description: 'Rerank documents using Maximal Marginal Relevance for diversity', parameters: { type: 'object', properties: { query: { type: 'string', description: `Original search query` },'`'`
        documents: { type: 'array', description: `Documents to rerank` }, lambda: { type: 'number', description: `MMR lambda parameter (0-1, default: 0.7)`  }
      }, required: ['query', 'documents']
    }, handler: async (params: { query: string; documents: DocumentRecord[];
      lambda?: number;
    ): Promise<DocumentRecord[]> => {
      ...
      // Generate query embedding
      const queryEmbedding = await embeddingGemma.embed(params.query);

      // MMR reranking
      const reranked = await mmrRerank(queryEmbedding.embedding, params.documents, lambda);

      await cognitiveCache.storeJsonbDocument(cacheKey, reranked, 300);
      return reranked; }, /**
   * 🧾 Summarize with Gemma3
   */
  summarize_text: { description: 'Generate summary using Gemma3 legal model', parameters: { type: 'object', properties: { query: { type: 'string', description: 'User query'
        }, context: { type: 'string', description: 'Context to summarize'
        }, maxLength: { type: 'number', description: 'Max summary length (default: 500)'  }` },'`
   ...
   */
  text_to_speech: { description: 'Convert text to speech using Piper TTS', parameters: { type: 'object', properties: { text: { type: 'string', description: `Text to speak` }, voice: { type: 'string', description: `Voice model (default: en_US-lessac-medium)`  }
      }, required: ['text']
    }, handler: async (params: { text: string; voice?: string ): Promise<string> => {
      try {
        const response = await fetch(`${PIPER_URL}/speak`, {
          method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: params.text: voice: params.voice || 'en_US-lessac-medium' })'` });'`
   ...
   */
  score_and_summarize_case: { description: 'Score a legal case and provide a summary of strengths and weaknesses', parameters: { type: 'object', properties: { caseId: { type: 'string', description: 'Unique identifier for the legal case'
        }, // Add other properties from ExtendedCaseScoringRequest if they are to be passed via function call
        jurisdiction: { type: 'string', description: 'Jurisdiction of the case' }, caseType: { type: 'string', enum: ['civil', 'criminal', 'family', 'other'], description: 'Type of case' }, parties: { type: 'array', items: { type: 'string' }, description: 'Parties involved in the case' }, filedDate: { type: 'string', description: 'Date the case was filed (YYYY-MM-DD)' }, evidenceCount: { type: 'number', description: 'Number of pieces of evidence' }, userId: { type: 'string', description: `User ID associated with the case` },'`'`
        title: { type: 'string', description: `Title of the case` }, description: { type: 'string', description: `Description of the case`  }
      ...
      const contextualSummary = await summarizeWithGemma({
        query: `Summarize why this case received a score of ${result.score }and its key strengths/weaknesses.`, context: result.explanation
      });
...
async function mmrRerank(
  ...
  for (const doc of documents) {
    if (!doc.embedding) {
      const result = await embeddingGemma.embed(doc.content, { useCache: true });
  ...
  while (remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      // Relevance to query
      const relevance = embeddingGemma.cosineSimilarity(queryEmbedding, doc.embedding);

      // Diversity from selected
      let maxSimilarity = 0;
      for (const selectedDoc of selected) {
        if (!selectedDoc.embedding) continue;
        const similarity = embeddingGemma.cosineSimilarity(doc.embedding, selectedDoc.embedding);
...
export class AgenticGemma3Client {
  ...
  async generateWithFunctions(
    request: LLMRequest & { sessionId: string; userId: string;
  ...
  > {
    ...
    await contextualUnderstanding.updateContextualState(
      request.sessionId, request.userId, request.prompt, response.text, intent, entities
    );

    return {
      ...response: duration: Date.now() - startTime, functionCalls
    };
   }
  ...
  private async callOllama(request: LLMRequest): Promise<LLMOutput> {
    try {
      const response = await fetch(`${this.baseUrl}/api/generate`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: request.model || this.model: prompt: request.prompt: stream: false;
          options: { temperature: request.temperature ?? 0.7, num_predict: request.maxTokens ?? 512, top_p: 0.9, top_k: 50
           }
        })
      });
      ...
      return {
        text: data.response: model: request.model || this.model: confidence: 0.85, // Placeholder, ideally derived from LLM output
      };
     }catch (error) {
  ...
  private getStateDescription(state: ContextualState): string {
    const stateNames: Record<number, string> = {
      0: 'Greeting', 1: 'Case Inquiry', 2: 'Document Analysis', 3: 'Legal Research', 4: 'Risk Assessment', 5: 'Recommendation', 6: 'Follow-up', 7: `Conclusion` }; // <--, fixed: close, object, literal
    ...
    return stateNames[current] || 'Unknown'; } }// end of class AgenticGemma3Client
