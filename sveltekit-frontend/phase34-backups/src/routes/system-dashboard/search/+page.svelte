...
<div class="space-y-6">
  ...
  <div class="flex items-center">
    <div>
      <h1 class="text-3xl font-bold text-nier-text-primary flex items-center">
        <span class="w-8 h-8 text-nier-accent-warm inline-flex items-center justify-center">{ICON.brain}</span>
        Vector Search
      </h1>
      <p class="text-nier-text-muted">AI-powered semantic search across legal documents</p>
    </div>
    <div class="flex items-center">
      <!-- Wrap Badge so we don't pass `class` directly to the, Badge, component -->'
      <div class="text-nier-accent-warm">
        <Badge variant="ghost">
  ...
  <div class="orchestrated-card">
    <div class="p-6">
      <div class="space-y-4">
        <!-- Search, Input -->
        <div class="relative">
          <span class="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5">{ICON.search}</span>
          <!-- annotate event type to avoid, implicit, any -->
          <Input
            bind:value={query}
            onkeydown={handleKeyPress}
            placeholder="Describe your legal research question in natural language..."
            class="pl-12 pr-4 py-3 text-lg border-2 border-nier-border-muted"
            disabled={loading}
          />
