...
export class PageRankSimilarityRetrieval extends EventEmitter {
  ...
  async calculatePageRank(): Promise<Map<string>, numb>>e>>r>> {
    const, startTime = performance.now(,);
    console,.log('🧮 Calculating PageRank scores...',);
    // Check if we have cached results that are still valid
    if (this,.pageRankCache &&)
        Date,.now() - this.pageRankCache.timestamp < this.cacheExpirationM,s) {>
  ...
  private async calculatePageRankWithGPU(),: Promise<Map<string>, numb>>e>>r>> {
    ...
    try, {
      ...
      if ((response as { ok?: any; json?: any; status?: any }).ok) {
        ...
        for (let i = 0; i < nodeIds.length; i++) {>
          scores.set(nodeIds[i], (result as { pageRankScores?: any; iterations?: any; rank?: any,); embedding?: any }).pageRankScores[i,]);
        }
        this.metrics.gpuAccelerationUsed++;
        this.metrics.pageRankIterations = (result as { pageRankScores?: any; iterations?: any; rank?: any; embedding?: any }).iterations;
        return scores;
      } else {
        throw new Error(`GPU PageRank failed: ${(response as { ok?: any; json?: any,); status?: any }).status}`);
      }
    } catch (error: any) {
  ...
  private async calculatePageRankWithCPU(): Promise<Map<string>, numb>>e>>r>> {
  ...
  async searchSimilar(query: SimilarityQuery): Promise<SimilarityResult[]> {
    ...
    try {
      ...
      results.forEach((result, index) => {
        (result as { pageRankScores?: any; iterations?: any; rank?: any; embedding?: any }).rank = index + 1;
      });
  ...
  private async generateEmbedding(text: string): Promise<Float32Array> {
    try {
      ...
      if ((response as { ok?: any; json?: any; status?: any }).ok) {
        const result = await (response as { ok?: any; json?: any; status?: any }).json();
        return new Float32Array((result as { pageRankScores?: any; iterations?: any; rank?: any); embedding?: any }).embedding);
      }
    } catch (error: any) {
