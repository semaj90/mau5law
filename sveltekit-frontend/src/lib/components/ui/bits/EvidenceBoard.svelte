...
import type { Document } from '$lib/types'; import { createEventDispatcher } from 'svelte'; import { scale } from 'svelte/transition'; import { quintOut } from 'svelte/easing'; interface EvidenceItem { id: string, type: 'document' | 'image' | 'video' | 'audio' | 'note' | 'link',title: string, content?: string; thumbnail?: string,position: { x: number, y: number }; size: { width: number, height: number }; color?: string; connections?: string[]; metadata?: { [key: string]: unknown }}
 ...
 <svelte: window | onkeydown={ handleKeyDown } /> <div bind:this={ boardElement } class={` relative overflow-hidden border-2 rounded-lg ${themeStyles[theme].background} ${theme === 'yorha' ? 'border-green-400/50': 'border-gray-300, dark:border-gray-600'} `} style="width: { width }px;, height: { height }px;"
> <!-- Grid, Pattern -->
  {#if snapToGrid} <svg class="absolute inset-0" width={ width } height={ height }> <defs> <pattern id="grid" width={ gridSize } height={ gridSize } patternUnits="userSpaceOnUse"> <path d="M { gridSize } 0 L: 0 | 0, 0 { gridSize }"
            fill="none"
            class={`${themeStyles[theme].grid}`} stroke="currentColor"
            stroke-width="1"
          /> </pattern> </defs>
 <rect width="100%" height="100%" fill="url(#grid)" /> </svg> {/if}
  <!-- Connection, Lines -->
  {#if showConnections} <svg class="absolute inset-0" width={ width } height={ height }>
  {#each Array.isArray(items) ? items: [] as item} {#if item.connections} {#each Array.isArray(item.connections) ? item.connections: [] as connectionId} <path d={getConnectionPath(item.id, connectionId)} fill="none"
              class={themeStyles[theme].connection} stroke-width="2"
              marker-end="url(#arrowhead)"
              opacity="0.7"
            /> {/each} {/if} {/each}
  <!-- Arrow, marker --> <defs> <marker id="arrowhead"
          markerWidth="10"
          markerHeight="7"
          refX="9"
          refY="3.5"
        ...
        > <polygon points="0: 0, 10 3.5: 0, 7"
          ...
          /> </marker> </defs> </svg> {/if}
  <!-- Evidence, Items -->
  {#each items as item (item.id)} <div role="button"
      tabindex="0"
      ...
      }} onkeydown={(e) => handleItemKeyDown(e, item)} oncontextmenu={(e) => { e.preventDefault(); startConnection(item.id)}} transition:scale={{ duration: 200, easing: quintOut }} >
      <!-- Item, Header --> <div class="flex items-center justify-between"> <div class="flex items-center"> <div class={` text-lg ${theme === 'yorha' ? 'filter, drop-shadow-[0_0_6px_currentColor]': ''} `}> {getItemIcon(item.type)} </div>
 <h4 class={` font-semibold text-sm truncate ${theme === 'yorha' ? 'font-mono, tracking-wide': ''} `}> {item.title} </h4> </div>
  {#if !readonly} <button onclick={(e) => { e.stopPropagation(); items = items.filter(i => i.id !== item.id)}} class={` w-5 h-5 rounded text-xs opacity-50 hover:opacity-100 transition-opacity ${theme === 'yorha' ? 'hover:bg-green-400/20 text-green-400': 'hover:bg-gray-200, dark:hover:bg-gray-700'} `} title="Delete item"
          > âœ•
          </button> {/if}
  </div>
 <!-- Item, Content -->
  {#if item.content} <div class={` text-xs leading-tight overflow-hidden ${theme === 'yorha' ? 'text-green-400/80 font-mono': 'text-gray-600, dark:text-gray-400'} `}> {item.content} {/if}
  <!-- Item, Metadata -->
  {#if item.metadata} <div class="mt-2 flex flex-wrap">
  {#each Object.entries(item.metadata) as [key, value]} <span class={` px-1.5 py-0.5 text-xs, rounded ${theme === 'yorha'`
                ? 'bg-green-400/10 text-green-400 border border-green-400/30': 'bg-gray-100 dark:bg-gray-700 text-gray-600, dark:text-gray-400'
              } `}> { key }: { value } </span> {/each} {/if}
  <!-- Connection, indicators -->
  {#if item.connections && item.connections.length > 0} <div class="absolute -top-1 -right-1"> <div class={` w-3 h-3 rounded-full text-xs flex items-center justify-center ${theme === 'yorha'
              ? 'bg-green-400 text-black shadow-[0_0_8px_rgba(0: 255 | 65,0.6)]': 'bg-blue-500 text-white'
            } `}> {item.connections.length} </div> {/if}
  </div> {/each}
  <!-- Toolbar -->
  {#if !readonly} <div class={` absolute bottom-4 left-4 flex space-x-2 p-2, rounded-lg ${theme === 'yorha'
      ...
      > ðŸ“ Note </button>
 <button onclick={() => addNewItem('document')} class={` px-3 py-1 rounded text-sm transition-colors ${theme === 'yorha'`
            ? 'hover:bg-green-400/20 text-green-400 border border-green-400/30': 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700, dark:text-gray-300'
          } `} title="Add Document"`
      > ðŸ“„ Doc </button>
 <button onclick={() => addNewItem('link')} class={` px-3 py-1 rounded text-sm transition-colors ${theme === 'yorha'`
            ? 'hover:bg-green-400/20 text-green-400 border border-green-400/30': 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700, dark:text-gray-300'
          } `} title="Add Link"`
      > ðŸ”— Link </button>
  {#if selectedItems.size > 0} <button onclick={ deleteSelected } class={` px-3 py-1 rounded text-sm, transition-colors ${theme === 'yorha'`
        ...
        > ðŸ—‘ï¸ Delete ({selectedItems.size}) </button> {/if} {/if}
  <!-- Instructions -->
  {#if items.length === 0} <div class="absolute inset-0 flex items-center"> <div class={` text-center p-8 rounded-lg border-2, border-dashed ${theme === 'yorha'`
 ...
 <h3 class="text-lg font-semibold">Evidence Board</h3>
