...
export class TensorRtEmbeddings extends Embeddings {
  ...
  private async infer(text: string): Promise<number[]> {
    ...
    const payload = {
      inputs: [
        { name: 'TEXT_INPUT', shape: [1], datatype: 'BYTES', parameters: { binary_data: false }, data: [Buffer.from(text, 'utf-8').toString('base64')]
        }], outputs: [
        { name: 'VECTOR', parameters: { binary_data: false  }
        }]
    };
