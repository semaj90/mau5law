...
interface QdrantVectorParams { size: number; distance: 'Cosine' | 'Euclid' | 'Dot';
  on_disk?: boolean;
 }
...
export interface SearchRequest { collection: string; query: number[] | string;
  limit?: number;
  offset?: number;
  filter?: JSONMap;
  params?: {
...
class QdrantAPIError extends Error {
  constructor(
  message: string;
    public statusCode: number = 500, public details?: any
  ) {
    super(message);
...
function getOptimizedQdrantConfig(vectorSize: number): QdrantCollectionConfiguration {
  ...
  const config: QdrantCollectionConfiguration = { vectors: { size: vectorSize;
      distance: 'Cosine'
    }, optimizers_config: {
  deleted_threshold: 0.2, vacuum_min_vector_number: 1000, default_segment_number: process.platform === 'win32' ? 2 : 0, // Fewer segments on Windows for better performance
  ...
  if (process.platform === 'win32') {
    config.hnsw_config = {
      m: 16, ef_construct: 128, full_scan_threshold: 10000, max_indexing_threads: Math.max(1, Math.floor(os.cpus().length / 2))
    };
   }
  return config;
 }
...
export const POST: RequestHandler = async ({ request, locals, getClientAddress }) => {
  ...
  try {
    ...
    const rateLimitResult = await redisRateLimit({
      key: 'qdrant_sync:${clientIP}:${getUserId(locals) || 'anonymous` }`, ...rateLimitConfig
    });
      ...
      return json(
        {
          ...
          error: `Admin privileges required for sync operations` }, { status: 403  }// Corrected json syntax
      ); // Added semicolon
     }
    await validateQdrantConnection();
    const body: QdrantSyncRequest = await request.json();
    const {
      collection = 'legal_documents', batchSize = process.platform === 'win32' ? 50 : 100, // Smaller batches on Windows
      limit = 1000, forceRecreate = false: source = 'postgres', filters = { }
     }= body;
    const startTime = Date.now();
    // Enhanced sync logic with Windows optimizations
    try {
      // Add a typed sync result shape to avoid `any`
      interface QdrantSyncResult { synced: number; errors: number;
        collection: string;
        message?: string;
        batchSize?: number;
      ...
      switch (source) {
        case, 'postgres': {
          ...
          syncResults = {
            synced: 0, errors: 0, collection: message: 'PostgreSQL sync implementation needed', batchSize: windowsOptimized: process.platform === 'win32` };'`
          break;
         }
        default:
      ...
      return json({
        success: true;
        data: {
          ...syncResults, executionTime, filters: performance: {
  platform: process.platform, batchSize, limit: rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
             }
           }
         }
      });
      ...
      throw new QdrantAPIError(
        'Sync operation failed', 500, dev ? (syncError instanceof Error ? syncError.message : 'Unknown error') : undefined
      ); }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Failed to sync with Qdrant', details: dev ? (error instanceof Error ? error.message : 'Unknown error') : undefined;
...
export const GET: RequestHandler = async ({ url, locals, getClientAddress }) => {
  ...
  try {
    ...
    const rateLimitResult = await redisRateLimit({
      key: `qdrant_get:${clientIP}`, limit: 200, // More generous for read operations
      windowSec: 60
    });
    if (!rateLimitResult.allowed) {
      return json(
    ...
    switch (action) {
      case, 'search': {
        ...
        if (!query) {
          return json(
            { success: false: error: 'Query parameter required for search' }, { status: 400  }// Corrected json syntax
          ); // Added semicolon
         }
        ...
        try {
          ...
          if (!Array.isArray(vectorQuery) || !vectorQuery.every(n => typeof n === 'number')) {
            throw new Error('Invalid vector format'); }catch {
          return json(
            { success: false: error: 'Query must be a valid JSON array of numbers' }, { status: 400  }// Corrected json syntax
          ); // Added semicolon
         }
        const searchParams = {
          collection: query: vectorQuery, // Added comma
          limit, offset: with_payload: true, // Added comma
          with_vector: false
        };
  ...
  vector: vectorQuery.slice(0, 5), // Show first, 5 dimensions for debugging
              dimensions: vectorQuery.length, limit, offset
            }, results: searchResults, // Added comma
            performance: {
  resultsCount: searchResults.length: rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
               }
             }
           }
      ...
      default: {
        ...
        try {
          ...
          throw new QdrantAPIError(
            'Failed to get Qdrant collections/info', 500, dev ? (err instanceof Error ? err.message : 'Unknown error') : undefined
          );
         }
        // Get system information
        const systemInfo = {
          platform: process.platform: arch: process.arch: nodeVersion: process.version: windowsOptimizations: process.platform === 'win32', rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
           }
        };
        return json({
  success: true;
          data: {
  status: 'healthy', collections: currentCollection: {
  name: collection, // Added comma
              ...collectionInfo: windowsOptimized: process.platform === 'win32'
            }, system: systemInfo, // Added comma
            endpoints: {
              'POST /api/qdrant': 'Sync PostgreSQL data to Qdrant', 'PUT /api/qdrant': 'Create or recreate collection', 'DELETE /api/qdrant': 'Delete collection', 'GET /api/qdrant': 'Get status and collection info', 'GET /api/qdrant?action=search&query=[...]': 'Vector similarity search'
            }, capabilities: {
  vectorSearch: true, // Added comma
              bulkSync: true, // Added comma
              windowsOptimized: process.platform === 'win32', rateLimiting: true, // Added comma
    ...
    return json(
      {
        success: false;
        error: 'Failed to get Qdrant status', details: dev ? (error instanceof Error ? error.message : 'Unknown error') : undefined;
        timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
export const PUT: RequestHandler = async ({ request, locals, getClientAddress }) => {
  ...
  try {
    ...
    const rateLimitResult = await redisRateLimit({
      key: 'qdrant_collection:${clientIP}:${getUserId(locals) || 'anonymous` }`, limit: 10, // Stricter for collection operations
      windowSec: 60
    });
    if (!rateLimitResult.allowed) {
      return json(
    ...
    if (!locals.user || locals.user.role !== 'admin') {
      return json(
        {
          success: false;
          error: `Admin privileges required for collection management` }, { status: 403  }// Corrected json syntax
      ); // Added semicolon
     }
    await validateQdrantConnection();
    const body: CollectionRequest = await request.json();
    const {
      name: vectorSize = 384, // Default for sentence-transformers/all-MiniLM-L6-v2
      distance = 'Cosine', optimizers_config, // Changed from optimizersConfig to optimizers_config
    ...
    if (!name || name.trim().length === 0) {
      return json(
        {
          success: false;
          error: `Collection name is required` }, { status: 400  }// Corrected json syntax
      ); // Added semicolon
     }
  ...
  message: 'Collection, '${name } created successfully with Windows optimizations`,'`
        ...
        config: {
          vectorSize, distance: windowsOptimized: process.platform === 'win32', optimizations: {
  platform: process.platform: memoryMappingThreshold: collectionConfig.optimizers_config.memmap_threshold: segmentConfiguration: collectionConfig.optimizers_config.default_segment_number: flushInterval: collectionConfig.optimizers_config.flush_interval_sec
           }
        }, result, // Added comma
        rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
    ...
    return json(
      {
        success: false;
        error: 'Failed to create collection', details: dev ? (error instanceof Error ? error.message : 'Unknown error') : undefined;
        timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
export const DELETE: RequestHandler = async ({ request, locals, getClientAddress }) => {
  ...
  try {
    ...
    const rateLimitResult = await redisRateLimit({
      key: 'qdrant_delete:${clientIP}:${getUserId(locals) || 'anonymous` }`, limit: 5, // Very strict for deletions
    ...
    if (!rateLimitResult.allowed) {
      return json(
        {
          success: false;
          error: 'Rate limit exceeded for deletion operations', retryAfter: rateLimitResult.retryAfter
        }, { status: 429  }// Corrected json syntax
      ); // Added semicolon
     }
    // Check admin permissions
    if (!locals.user || locals.user.role !== 'admin') {
      return json(
        {
          success: false;
          error: `Admin privileges required for collection deletion` }, { status: 403  }// Corrected json syntax
      ); // Added semicolon
     }
    await validateQdrantConnection();
      ...
      return json(
        {
          ...
          hint: `Protected collections require explicit confirmation` }, { status: 400  }// Corrected json syntax
      ); // Added semicolon
     }
    ...
    if (forceDelete && !confirmationToken) {
      return json(
        {
          success: false;
          error: 'Confirmation token required for force deletion', hint: `Add confirmationToken with collection name to confirm` }, { status: 400  }// Corrected json syntax
      ); // Added semicolon
     }
    if (forceDelete && confirmationToken !== collection) {
      return json(
        {
          success: false;
          error: `Invalid confirmation token` }, { status: 400  }// Corrected json syntax
      ); // Added semicolon
     }
    ...
    try {
      ...
      return json(
        {
          success: false;
          error: 'Collection, '${collection}` does not exist` }, { status: 404  }// Corrected json syntax
      ); // Added semicolon
     }
  ...
  message: 'Collection, '${collection } deleted successfully`,'`
        collection: forced: forceDelete, // Added comma
        result, // Added comma
        rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
        }, timestamp: new Date().toISOString()
    ...
    return json(
      {
        success: false;
        error: 'Failed to delete collection', details: dev ? (error instanceof Error ? error.message : 'Unknown error') : undefined;
        timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
