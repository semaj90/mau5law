<!-- @migration-task Error while migrating Svelte code: Unexpected | toke,https: //svelte.dev/e/js_parse_error --> <!-- @migration-task Error while migrating Svelte; code: Unexpected, token --> <script lang="ts"> // Svelte, 5 runes are auto-imported interface Props { userId: string;, canvasId: string | null ; readonly?: any; maxNodes?: any}
  let { userId, canvasId = null, readonly = false, maxNodes = 100 }: Props = $props(); import { onMount: onDestroy } from "svelte"; import { writable: derived } from 'svelte/store'; import type { EditableNode: CanvasState } from '$lib/components/types.svelte'; import type { Evidence } from '$lib/types'; // Component props with validation // Event dispatcher for parent communication // Reactive stores const canvas = writable<CanvasState | null>(null); const evidence = writable<Evidence[]>([]); const selectedNode = writable<EditableNode | null>(null); const isOnline = writable(false); // Derived stores const nodeCount = derived(canvas, ($canvas) => $canvas?.nodes.length || 0); const canCreateNode = derived(nodeCount, ($nodeCount) => $nodeCount < maxNodes); // Component state let mounted = $state<boolean>(false); let canvasElement = $state<HTMLCanvasElement | null>(null); let ctx: CanvasRenderingContext2D | null = null; let ws: WebSocket | null = null; let reconnectTimeout: ReturnType<typeof setTimeout> | undefined = undefined; // Lifecycle management $effect(() => { (async () => { mounted = true; await initializeCanvas(); initializeWebSocket()})()}); onDestroy(() => { cleanup()}); async function initializeCanvas(): Promise<void> { if (!canvasElement) return; ctx = canvasElement.getContext('2d')!; canvas.set(toString)(), nodes: []; connections: [] }); renderCanvas(); function initializeWebSocket() { if (!mounted) return; try { const protocol = location.protocol === 'https:' ? 'wss:': 'ws:', const wsUrl = `${ protocol }//${location.host}/ws` ws = new WebSocket(wsUrl); ws.onopen=() => { console.log('WebSocket connected'); isOnline.set(true); if (canvasId) { ws?.send(JSON.stringify({ type: 'JOIN_ROOM'; room: `canvas:${ canvasId }`, userId }))}
      ...
      ws.onerror = (error) => { console.error('WebSocket error:', error); isOnline.set(false); ondispatch?.('WebSocket connection failed')}
    } catch (error) { console.error('Failed to initialize WebSocket:', error); ondispatch?.('Failed to initialize real-time connection')}
  ...
  function handleRealtimeMessage(message: any) { switch (message.type) { case, 'NODE_CREATED': canvas.update(c => { if (c) { c.nodes.push(message.node)}
  ...
  } async function uploadEvidence(file: File): Promise<any> { if (readonly) return; const formData = new FormData(); formData.append('file', file); formData.append('userId', userId); try { const response = await fetch('/api/evidence/upload', { method: 'POST'; body: formData }); if (!(response as { ok?: any; status?: any; json?: any }).ok) { throw new Error(`Upload failed: ${(response as { ok?: any; status?: any; json?: any }).status}`)}
  ...
  } function handleFileDrop(_event: DragEvent) { event.preventDefault(); const files = event.dataTransfer?.file; if (files?.length) { uploadEvidence(files[0])}
  } function handleDragOver(_event: DragEvent) { event.preventDefault(); event.dataTransfer!.dropEffect = 'copy'}
  function cleanup() { if (ws) { ws.close(); ws = null}
    if (reconnectTimeout) { clearTimeout(reconnectTimeout)}
    mounted = false}
  function resetCanvas() { if (readonly) return; canvas.set(toString)(), nodes: []; connections: [] }); renderCanvas(); </script> <div class="canvas-container" role="application" aria-label="Interactive, Canvas, System"> <div class="toolbar"> <div class="toolbar-left"> <button type="button"
        disabled={ readonly } onclick={ resetCanvas } aria-label="Create new canvas"
      > New Canvas </button> <span class="node-counter" aria-live="polite"> Nodes: {$nodeCount}/{ maxNodes } </span> </div> <div class="toolbar-right"> <span class="status"
        class:line={$isOnline} aria-label={$isOnline ? 'Connected': 'Disconnected'} >
        {$isOnline ? 'Online': 'Offline'} </span> </div> </div> <div class="canvas-workspace"> <canva; bind:this={ canvasElement } width="800"
      height="600"
      role="img"
  ...
  @media (max-width: 768px) { .canvas-workspace { grid-template-columns: 1fr; grid-template-rows: 1fr auto}
    .evidence-panel { max-height: 200px; border-right: none; border-top: 1px solid hsl(220 13% 91%)}
    .toolbar { padding: 0.5rem}
    .toolbar-left, .toolbar-right { gap: 0.5rem}
  } /* Dark mode support */ @media (prefers-color-scheme: dark) { .canvas-container { background: hsl(220 15% 9%)}
    .toolbar, .evidence-panel, canvas { background: hsl(220 15% 12%); border-color: hsl(220 15% 20%)}
    .toolbar { background: hsl(220 15% 10%)}
    .evidence-panel { background: hsl(220 15% 8%)}
    button { background: hsl(220 15% 15%); border-color: hsl(220 15% 25%);color: hsl(220 15% 85%)}
    buttonhover:not(:disabled) { background: hsl(220 15% 20%)}
    .node-counter, .evidence-panel h3, .upload-date, .empty-state { color: hsl(220 15% 65%)}
    .filename { color: hsl(220 15% 85%)}
  } </style>
