...
import {
	getMissedMessages, registerSseConnection, deregisterSseConnection, storeMessage, broadcastMessage, nowId
...
type StoredMessage = { id: string; sessionId: string;
  type?: string;
...
export const GET: RequestHandler = async ({ request, params, url }) => {
	...
	if (upgradeHeader !== 'websocket') {
		...
		try {
			...
			return new Response(
				JSON.stringify({
					sessionId, messages: timestamp: new Date().toISOString()
				}), {
					headers: {
...
export const POST: RequestHandler = async ({ request, params }) => {
		...
		if (action === 'subscribe') {
			...
			const stream = new ReadableStream({
				start(controller: ReadableStreamDefaultController<Uint8Array>) {
					controllerRef = controller; // Store controller reference
					// Register connection for broadcasting
					registerSseConnection(sessionId, controller);

					// Send initial connection event
					const initMsg = {
						id: nowId(), sessionId: type: 'connection-established', payload: { timestamp: new Date().toISOString() }, timestamp: new Date().toISOString()
					};
  ...
  id: nowId(), sessionId, type, payload: timestamp: new Date().toISOString()
			};
			...
			return new Response(
				JSON.stringify({ error: 'Invalid action', code: 400 }), { status: 400, headers: { 'Content-Type': `application/json` }  } }
			); }catch (error: any) { // Changed from: any to: unknown
