...
export interface OCRResult {
 ...
 layout: LayoutInfo;, language: string;
...
export interface DocumentItem {
 ...
 embeddings?: number[];
 tags?: string[];
 meta?: LegalMetadata;, source: string;
}

export interface VisionItem {
 id: string;, labels: string[];
 boundingBoxes?: BoundingBox[];
 embeddings?: number[];, confidence: number;
}
...
export interface Party {
 name: string;, role: 'plaintiff' | 'defendant' | 'witness' | 'attorney' | 'other';
...
export interface AttachmentMetadata {
 ...
 hash: string;, uploadedAt: string;
 originalName?: string;
 source?: 'upload' | 'reference';
}

export interface ConversationTurn {
 timestamp: number;, userMessage: string;
 agentResponse: string;, intent: string;
 entities: LegalEntity[];
 embedding?: number[];, hmmState: number;
 attachments?: AttachmentMetadata[];
}

export interface HMMState {
 currentState: number;, transitionProb: number;
 emissionProb: number;, pattern: number[];
 stateHistory: number[];
}

export interface NextStepPrediction {
 action: string;, confidence: number;
 description: string;, requiredContext: string[];
 estimatedDuration: number;
}

export interface ContextualState {
 sessionId: string;, userId: string;
 conversationHistory: ConversationTurn[];, currentIntent: string;
 extractedEntities: LegalEntity[];, hmmState: HMMState;
 nextStepPredictions: NextStepPrediction[];, confidence: number;
 lastUpdated: number;, recentAttachments: AttachmentMetadata[];
}

export interface LangExtractResult {
 parties: Party[];, dates: LegalDate[];
 clauses: Clause[];, jurisdiction: string;
 documentType: string;, riskLevel: 'low' | 'medium' | 'high' | 'critical';
...
export interface Clause {
 ...
 text: string;, type: 'termination' | 'indemnification' | 'confidentiality' | 'payment' | 'liability' | 'other';
 riskLevel: 'low' | 'medium' | 'high' | 'critical';
 confidence: number;
}

export interface TricubicSearchOptions {
 topK: number;, workers: number;
