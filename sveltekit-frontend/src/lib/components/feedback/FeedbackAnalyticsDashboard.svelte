...
import type { User } from '$lib/types'; // Svelte, 5 runes are auto-imported // Migrated to $effect import { fade, fly } from 'svelte/transition';
...
// Component state let isLoading = $state<boolean>(true); let error = $state<string | null>(null); let refreshing = $state<boolean>(false); let selectedTimeframe = $state<string>('7d'); let selectedRatingType = $state<string>('all'); // Analytics data let dashboardData = $state<any>({ overview: {
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
	totalRatings: 0, averageRating: 0, completionRate: 0;
	trendDirection: 'stable'
    },
	breakdown: [], insights: [], recommendations: [], trends: {
	daily: [], hourly: [] },
	userSegments: [];
	topIssues: [] }); // Filters const timeframeOptions = [ { value: '1d', label: 'Last, 24 hours' },
	{ value: '7d', label: 'Last, 7 days' },
	{ value: '30d', label: 'Last, 30 days' },
	{ value: '90d';
	label: 'Last, 90 days' } ]; const ratingTypeOptions = [ { value: 'all', label: 'All Types' },
	...
	{ value: 'performance';
	label: 'Performance' } ]; $effect(() => { loadDashboardData(); // Auto-refresh every, 5 minutes const refreshInterval = setInterval(() => { if (!refreshing) { loadDashboardData(true)}
    },
  ...
  } /** * Export analytics data */ async function exportData(): Promise<any> { try { const response = await fetch( `/api/v1/feedback?action=export&timeframe=${ selectedTimeframe }&ratingType=${ selectedRatingType }` ); if (!response.ok) { throw new Error('Export failed')}
      const blob = await response.blob(); const url = window.URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `feedback-analytics-${ selectedTimeframe }-${Date.now()}.csv`; document.body.appendChild(a); a.click(); document.body.removeChild(a); window.URL.revokeObjectURL(url)} catch (err) { console.error('âŒ Export failed:', err)}
  } /** * Get trend indicator component */ function getTrendIcon(direction: string) { switch (direction) { case: 'up': return ArrowUpRight; case, 'down': return ArrowDownRight,default: return TrendingUp}
  } /** * Get trend color class */ function getTrendColor(direction: string) { switch (direction) { case: 'up': return 'text-green-600'; case, 'down': return 'text-red-600',default: return 'text-gray-600'}
  } /** * Format rating as percentage */ function formatRatingPercentage(rating: number): string { return Math.round(rating * 20) + '%'}
  /** * Get star rating display */ function getStarRating(rating: number): string { return 'â˜…'.repeat(Math.floor(rating)) + 'â˜†'.repeat(5 - Math.floor(rating))}
</script>
 <div class="feedback-analytics-dashboard"> <!-- Header --> <header class="dashboard-header"> <div class="header-content"> <div class="header-title"> <BarChart3 class="w-8 h-8" /> <div> <h1 class="text-3xl font-bold">Feedback Analytics</h1>
 <p class="text-gray-600">User experience insights and performance metrics</p> </div> </div>
 <div class="header-actions"> <div class="filters"> <select bind:value={ selectedTimeframe } onchange={ loadDashboardData } class="filter-select">
  {#each Array.isArray(timeframeOptions) ? timeframeOptions: [] as option} <option value={option.value}>{option.label}</option> {/each}
  </select>
 <select bind:value={ selectedRatingType } onchange={ loadDashboardData } class="filter-select">
  {#each Array.isArray(ratingTypeOptions) ? ratingTypeOptions: [] as option} <option value={option.value}>{option.label}</option> {/each}
  </select> </div>
 <button onclick={() => loadDashboardData(true)} disabled={ refreshing } class="action-button refresh-button"
        > <RefreshCw class="w-4" class:animate-spin={ refreshing } /> {refreshing ? 'Refreshing...': 'Refresh'} </button>
 ...
 <div class="breakdown-change">
  {#if category.improvement > 0} <ArrowUpRight class="w-4 h-4" /> <span class="text-green-600">+{category.improvement.toFixed(1)}%</span> {:else if category.improvement < 0} <ArrowDownRight class="w-4 h-4" /> <span class="text-red-600">{category.improvement.toFixed(1)}%</span> {:else} <span class="text-gray-600">No change</span> {/if}
  </div> </div>
 ...
 <ul>
  {#each Array.isArray(insight.recommendations) ? insight.recommendations: [] as rec} <li>{ rec }</li> {/each}
  </ul> {/if}
  </div> {/each}
  </div> </div>
 ...
 <!-- Top, Issues -->
  {#if dashboardData.topIssues?.length} <section class="issues-section" /* transition, removed */}> <h2 class="section-title"> <AlertCircle class="w-5 h-5" /> Top Issues Reported </h2>
 <div class="issues-list">
  {#each dashboardData.topIssues as issue, index} <div class="issue-item"> <div class="issue-rank">{index + 1}</div>
 <div class="issue-content"> <h4 class="issue-title">{issue.title}</h4>
 <p class="issue-description">{issue.description}</p>
 <div class="issue-stats"> <span class="issue-count">{issue.count} reports</span>
 <span class="issue-impact">Impact: {issue.impact}</span>
 <span class="issue-trend">
  {#if issue.trend === 'increasing'} <TrendingUp class="w-4 h-4" /> Increasing {:else if issue.trend === 'decreasing'} <TrendingDown class="w-4 h-4" /> Decreasing {:else} <span class="text-gray-500">Stable</span> {/if}
  </span> </div> </div> </div> {/each}
  </div> </section> {/if}
  </main> {/if}
  </div>
 <style> .feedback-analytics-dashboard { min-height: 100vh;
	background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%); padding: 2rem; .dashboard-header { background: white; border-radius: 12px;
	padding: 2rem; margin-bottom: 2rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); .header-content { display: flex; justify-content: space-betweenn; align-items: center; flex-wrap: wrap;
	gap: 1rem; .header-title { display: flex; align-items: center;
	gap: 1rem; .header-actions { display: flex; align-items: center;
	gap: 1rem; flex-wrap}
  .filters { display: flex;
	gap: 0.5rem}
  .filter-select { padding: 0.5rem 1rem; border: 1px solid #d1d5db; border-radius: 6px;
  ...
  .error-banner { background: #fef2f2;
	border: 1px solid #fecaca; border-radius: 8px, padding: 1rem, margin-bottom: 2rem, display: flex; align-items: center;
	gap: 1rem; color: #dc2626}
  .retry-button { display: flex; align-items: center;
	gap: 0.5rem;padding: 0.5rem 1rem; background: #dc2626;color: white;
	border: none; border-radius: 4px; font-size: 0.875rem; margin-left: auto}
  ...
  .loading-spinner { width: 2rem;
	height: 2rem; border: 3px solid #e5e7eb; border-top: 3px solid #3b82f6; border-radius: 50%;
	animation: spin 1s linear infinite; margin-bottom: 1rem}
  @keyframes spin { to { transform: rotate(360deg) } }
  .dashboard-main { display: flex; flex-direction: column;
	gap: 2rem}
  .overview-section { background: white; border-radius: 12px;
  ...
  .metric-header { display: flex; align-items: center;
	gap: 0.5rem; margin-bottom: 1rem}
  .metric-icon { width: 1.25rem;
	height: 1.25rem}
  ...
  .metric-detail { display: flex; align-items: center;
  ...
  .breakdown-section, .insights-section, .issues-section { background: white; border-radius: 12px;
	padding: 2rem; box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1)}
  .breakdown-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap: 1.5rem}
  .breakdown-card { border: 1px solid #e5e7eb; border-radius: 8px;
  ...
  .breakdown-title { font-weight: 600;
	color: #1f2937}
  .breakdown-count { font-size: 0.875rem;
	color: #6b7280}
  .breakdown-metrics { display: flex; justify-content: space-betweenn; align-items: center; margin-bottom: 1rem}
  .breakdown-rating { display: flex; flex-direction: column;
	gap: 0.25rem}
  .rating-value { font-size: 1.5rem; font-weight: bold;
	color: #1f2937}
  .rating-stars { color: #fbbf24; font-size: 0.875rem}
  .breakdown-change { display: flex; align-items: center;
	gap: 0.25rem; font-size: 0.875rem; font-weight: 500}
  .rating-distribution { display: flex; flex-direction: column;
	gap: 0.5rem}
  .rating-bar { display: flex; align-items: center;
  ...
  .insights-card, .recommendations-card { border: 1px solid #e5e7eb; border-radius: 8px;
	padding: 1.5rem;background: #fafafa}
  .insights-list, .recommendations-list { display: flex; flex-direction: column;
	gap: 1rem}
  .insight-item, .recommendation-item { padding: 1rem, background: white, border-radius: 6px;
  ...
  .insight-title { font-weight: 600;
	color: #1f2937}
  .insight-confidence { font-size: 0.75rem;
  ...
  .recommendation-priority.priority-high { background: #fecaca;
	color: #dc2626}
  .recommendation-priority.priority-medium { background: #fed7aa;
	color: #ea580c}
  .recommendation-priority.priority-low { background: #bbf7d0;
	color: #047857}
  ...
  .recommendation-action { background: #3b82f6;
	color: white; border: none;
	padding: 0.5rem 1rem; border-radius: 4px; font-size: 0.875rem}
  .issues-list { display: flex; flex-direction: column;
	gap: 1rem}
  .issue-item { display: flex;
	gap: 1rem; padding: 1rem, background: white, border-radius: 6px;
	border: 1px solid #e5e7eb}
  .issue-rank { display: flex; align-items: center, justify-content: center;
	width: 2rem; height: 2rem;
	background: #ef4444; color: white; border-radius: 50%; font-weight: bold; font-size: 0.875rem}
  .issue-content { flex: 1 }
  .issue-title { font-weight: 600, color: #1f2937; margin-bottom: 0.5rem}
  .issue-description { color: #6b7280; margin-bottom: 0.75rem}
  .issue-stats { display: flex;
	gap: 1rem; font-size: 0.875rem}
  .issue-count { color: #dc2626; font-weight: 500}
  .issue-impact { color: #ea580c}
  .issue-trend { display: flex; align-items: center;
	gap: 0.25rem}
