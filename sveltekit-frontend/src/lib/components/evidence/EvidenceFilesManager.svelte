<script lang="ts">
	...
	let {
		...
		readonly?: boolean;
		maxFiles?: number;
		onupload?: (_event: { files: File[] }) => void;
		onremove?: (_event: { index: number }) => void;
	} = $props();
...
<div class="evidence-files-manager">
	{#if files.length === 0}
		<p class="no-files">No evidence files added yet.</p>
	{/if}
	...
	<ul>
		{#each files as f, i}
			<li class="file-item">
				...
				<div class="actions">
					<button type="button" onclick={() => removeFile(i)} disabled={readonly}>
						Remove
					</button>
				</div>
			</li>
