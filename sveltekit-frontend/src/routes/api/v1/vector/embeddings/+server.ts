...
async function handleEmbeddings(request: Request, requestId: string, apiStartTime: number): Promise<Response> {
    ...
    if (minioUrl) {
        ...
        metadata = { ...documentResult.metadata, ...chunkedResult.metadata, originalUrl: minioUrl, minioUrl };
    } else {
    ...
    const shouldUseCUDA = useCUDA && (processedTexts.length > 10 || textComplexity > 75 || processedTexts.some(text => text.length > 2000));

    if (shouldUseCUDA) {
        // Ask the centralized service to use the CUDA/TensorRT backend
        const resp = await generateEmbeddings({ texts: processedTexts, model, mode: 'tensorrt', requestId });
        embeddings = resp.embeddings;
        cudaTime = resp.gpuTime || 0; // Use gpuTime from the response
        parallelWorkers = resp.parallelWorkers || 1; // Use parallelWorkers from the response
    } else {
        // Ask centralized service to use default Ollama backend
        const resp = await generateEmbeddings({ texts: processedTexts, model, mode: 'ollama' });
        embeddings = resp.embeddings;
    }
...
async function handleChunking(request: Request, _requestId: string, _apiStartTime: number): Promise<Response> {
    ...
    const result = await chunkText(text, { chunkSize, chunkOverlap, preserveParagraphs, extractMetadata });
    return json({ success: true, ...result });
}

async function chunkText(
    text: string,
    options: { chunkSize: number; chunkOverlap: number; preserveParagraphs?: boolean; extractMetadata?: boolean }
