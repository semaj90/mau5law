...
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
  ...
  let contractData: ContractAnalysis = contract ?? { id: 'contract-001';
	...
	status: 'review',
    riskScore: 6.5;
	lastModified: '2025-09-21T14:3 0 00Z',
    clauses: [ {
        id: 'clause-1';
	type: 'termination',
  ...
  const riskStyles: Record<string, { color: string; border: string; background: string }> = {
	...
	high: {
	color: '#ef4444', border: '2px solid #ef4444', background: 'rgba(239, 68, 68, 0.1)' }; critical: {
	color: '#dc2626', border: '2px solid #dc2626', background: 'rgba(220, 38, 38, 0.2)' }
  };
  ...
  async function analyzeContract(): Promise<any> {
    ...
    try {
      ...
      contractData.lastModified = new Date().toISOString()} catch (err) {
      console.error('Contract analysis failed:', err)} finally {
      isAnalyzing = false}
  }
...
<div class="contract-analyzer">
  ...
  <div
    class="card-wrapper"
    style="border: {contractBuilder?.styling?.borderWidth ?? '1px'} solid {contractBuilder?.styling?.colors?.primary}; border-radius: 8px;
	padding: 0.25rem;"
  >
    <Card>
      <CardHeader>
        <CardTitle class="contract-title">
          <div class="title-section">
            ...
            <div class="title-text">
              <h2>{contractData.title}
</h2>

              <div class="contract-meta">
                <span class="contract-type">{contractData.type.toUpperCase()}
</span>

                <span class="contract-status">{contractData.status}
</span>
  ...
  {#if isAnalyzing}ðŸ”„ Analyzing...{:else}ðŸ¤– AI Analyze{/if}
            ...
            <div class="export-dropdown">
              <Button.Root class="export-btn bits-btn">ðŸ“¤ Export</Button>
      ...
      <CardContent>
        ...
        <div class="search-section">
          ...
          <div class="clause-stats">
            ...
            <span class="stat-item">ðŸ•’ Modified: {new Date(contractData.lastModified).toLocaleDateString()}
</span>
          </div>
        </div>
  ...
  {#each Array.isArray(['low', 'medium', 'high', 'critical']) ? ['low', 'medium', 'high', 'critical'] : [] as riskLevel}
              ...
              <div class="risk-bar">
                <span class="risk-label">{riskLevel.toUpperCase()}
</span>
              ...
              >
                <div class="clause-header">
                  <div class="clause-type">
                    <span class="clause-icon">{getClauseIcon(clause.type)}
</span>

                    <span class="clause-label">{clause.type.replace('_', ' ').toUpperCase()}
</span>
                  </div>
                ...
                <div class="clause-content">
                  <p class="clause-text">{clause.content}
</p>
  ...
  {#if selectedClause === clause.id}
                  <div class="clause-actions" in, fade>
                    <Button class="bits-btn" size="sm">âœï¸ Edit</Button>

                    <Button class="bits-btn" size="sm" variant="outline">ðŸ’¬ Comment</Button>

                    <Button class="bits-btn" size="sm" variant="outline">ðŸ” Deep Analysis</Button>
                  {/if}
  </div>
            {/each}
  </div>
...
<style>
  ...
  .export-btn { background: var(--enhanced-bits-secondary);
	color: #000}
  .export-menu {
    position: absolute;
	top: 100%; right: 0
  ...
  .clause-type { display: flex; align-items: center;
	gap: 0.5rem}
  .clause-icon { font-size: 1.25rem}
  .clause-label { font-size: 0.875rem; font-weight: bold;
  ...
  .confidence-display { display: flex; align-items: center;
	gap: 1rem}
  ...
  .clause-actions { display: flex;
	gap: 0.5rem; margin-top: 1rem; padding-top: 1rem; border-top: 1px solid var(--enhanced-bits-border)}
  ...
  @media (max-width: 768px) {
    .contract-title { flex-direction: column;
	gap: 1rem}
