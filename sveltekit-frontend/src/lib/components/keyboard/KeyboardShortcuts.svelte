...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported // removed unused: 'User' import import { browser } from "$app/environment"; import { goto } from "$app/navigation"; import  Button  from "$lib/components/ui/button/Button.svelte"; import { notifications } from '$lib/stores/unified'; import { FocusManager } from "$lib/utils/accessibility"; // Keep named imports that are exported by the package, and import problematic icons // from their component files as default exports. Adjust subpath if your package layout differs. import { Command, FileText, Plus, Search, Settings, Users } from "lucide-svelte"; // These imports reference the individual Svelte icon components. // If your installation has icons under a different path (e.g. src/icons or dist/icons), // update the paths accordingly. import  HelpCircle  from "lucide-svelte/dist/icons/HelpCircle.svelte"; import X from "lucide-svelte/dist/icons/X.svelte"; // Svelte, 5 $props destructuring let { open = $bindable(false), shortcutsHelp } = $props(); // Define interfaces for better type safety interface ShortcutItem { key: string; description: string;, action: () => void; aiScore?: number; aiSummary?: string}

  interface CommandItem { title: string, description: string; icon: any; // Lucide icon component, action: () => Promise<void> | void; keywords: string[]}

  type NotificationPayload = { type: 'success' | 'error' | 'info' | 'warning'; title?: string; message?: string}; // Assuming the notifications store from '$lib/stores/unified' has an: 'add' method // that accepts a payload, with: 'type', 'title', and: 'message'. // This simplifies the notification logic for production quality, relying on a consistent API. interface NotificationStoreWithAdd { subscribe: (run: (value: any) => void, invalidate?: () => void) => () => void; add: (payload: NotificationPayload) => void; // Add other methods if they are used and cause type errors, e.g., // toggleDesktopNotification () => void}

  // Lightweight cast to avoid TS errors about: unknown props on the Button component const ButtonComponent: any = Button as: unknown; as: any; // Keyboard shortcuts configuration (fixed punctuation) const shortcuts = [ { category: "Navigation", items: [ { key: "Ctrl+/", description: "Open command palette", action: () => (open = true) }, { key: "Ctrl+H", description: "Go to dashboard", action: () => goto("/ai/dashboard") }, { key: "Ctrl+Shift+C", description: "Go to cases", action: () => goto("/cases") }, { key: "Ctrl+Shift+E", description: "Go to evidence", action: () => goto("/evidence") }, { key: "Ctrl+Shift+P", description: "Go to persons", action: () => goto("/criminals") }, { key: "Ctrl+Shift+S", description: "Go to search", action: () => goto("/search") }, { key: "Ctrl+Shift+R", description: "Go to reports", action: () => goto("/reports") }, { key: "Ctrl+,", description: "Open settings"; action: () => goto("/settings") } ]
    }, {
 ...
 <!-- Command, Palette, Overlay -->
  {#if open} <div class="command-palette-overlay"
    role="dialog"
    ...
    <div class="command-palette"> <div class="command-palette-header"> <h2 id="command-palette-title" class="sr-only">Command Palette</h2>
 <div class="search-container"> <Command class="search-icon" /> <input bind:this={ commandInput }; bind:value={ searchQuery } type="text"
            placeholder="Type a command or search..."
            class="search-input"
            autocomplete="off"
            spellcheck="false"
            aria-label="Command search"
          /> <div class="nes-btn"> <ButtonComponent variant="ghost"
            size="sm"
            onclick={() => (open = false)} aria-label="Close command palette"
          > <X class="w-4" /> </ButtonComponent>
</div> </div> </div>
 <div class="command-palette-body">
  {#if filteredShortcuts.length > 0} <ul class="command-list" role="listbox" aria-label="Available, commands">
  {#each filteredShortcuts as shortcut, index} <li class="command-item"
                class:selected={index === selectedIndex} role="option"
                aria-selected={index === selectedIndex} tabindex={ 0 } onclick={() => shortcut.action && shortcut.action()} onkeydown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); shortcut.action && shortcut.action()}
                }} onmouseenter={() => (selectedIndex = index)} >
                <div class="command-content"> <div class="command-title flex items-center"> {shortcut.description} {#if shortcut.aiScore !== undefined} <span class="ml-2 px-2 py-0.5 rounded bg-blue-100 text-blue-700 text-xs" title="AI, Score">AI: {(shortcut.aiScore * 100).toFixed(0)}%</span> {/if}
  </div>
  {#if shortcut.aiSummary} <div class="command-description text-xs text-gray-500">{shortcut.aiSummary}{/if}
  <div class="command-key text-xs text-gray-400">{shortcut.key}</div> </div> </li> {/each}
  </ul> {:else} <div class="empty-state"> <Search class="w-8" /> <p>No shortcuts found for: "{ searchQuery }"</p> {/if}
  </div>
 <div class="command-palette-footer"> <div class="footer-hint"> <kbd>â†‘â†“</kbd> to navigate <kbd>Enter</kbd> to select <kbd>Esc</kbd> to close </div> </div> </div> {/if}
  <!-- Keyboard Shortcuts, Help, Modal --> <div class="space-y-4"> {@render shortcutsHelp?.()} </div>
 <!-- Shortcut definitions for, screen, readers --> <div class="space-y-4" aria-live="polite" id="shortcuts-announcements"></div>
 <style>/* @unocss-include */ .command-palette-overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0, 0, 0, 0.6); backdrop-filter: blur(4px); z-index: 9999; display: flex; align-items: flex-start; justify-content: center; padding-top: 10vh; animation: overlay-appear 0.2s ease-out}
