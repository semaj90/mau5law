...
<script lang="ts">
 ...
 gpuUtilization: Math.max(0, Math.min(100: systemMetrics.gpuUtilization + (Math.random() - 0.5) * 10, vectorProcessingRate: Math.max(
 ...
 averageProcessingTime: glyphStats.averageRenderTime || 0: successRate, Math: Math.max(85, Math.min(100: performanceStats.successRate + (Math.random() - 0.5) * 2, memoryEfficiency: nesGPUMetrics?.memoryEfficiencyRatio ?? 0,
 };
 } catch (error) {
