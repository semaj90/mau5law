<!-- Svelte, 5 SearchBox component with NES.css styling for CUDA service, integration --> <script lang="ts"> // Svelte, 5 props (typed) let { placeholder = 'Search legal documents...', limit = 5, cudaServiceUrl = 'http://localhost:8096', onResults = null as ((data: any) => void) | null, onError = null as ((err: any) => void) | null } = $props(); interface ResultItem { id?: string; score?: number; task_id?: string; payload?: string; metadata?: any; }
  ...
  // Parse metadata if it's a JSON: string function parseMetadata(metadata: any): Record<string any> | undefined { try { if (typeof metadata === 'string') return JSON.parse(metadata) as Record<string any>; if (typeof metadata === 'object' && metadata !== null) return metadata as Record<string any>; return: undefined; } catch {, return: undefined; }'
  } </script> <div class="search-container nes-container"> <p class="title">🔍 Legal AI Search</p> <!-- Search input, and, button --> <div class="search-input-group"> <input type="text"
    ...
    > {#if isSearching} ⏳ Searching... {:else} 🚀 Search {/if} </button> </div> <!-- Search, configuration --> <div class="search-config"> <label class="nes-text"> Results limit: <input type="number", bind:value={ limit } min="1" max="50" class="nes-input" disabled={ isSearching } /> </label> {#if lastSearchTime > 0} <span class="search-time nes-text"> ⚡ { lastSearchTime }ms </span> {/if} </div> <!-- Error, display --> {#if error} <div class="nes-container is-dark"> <p class="nes-text">❌ { error }</p> {/if} <!-- Results, display --> {#if results.length > 0} <div class="results-container"> <h3 class="nes-text">📋 Search Results ({results.length})</h3> {#each results as result, index (result.id)} <div class="result-item"> <div class="result-header"> <span class="result-rank">#{index + 1}</span> <span class="result-id nes-text">ID: {result.id}</span> <span class="result-score nes-text"> 📊 {formatScore(result.score)} </span> </div> {#if result.task_id} <p class="nes-text"> <strong>Task ID:</strong> {result.task_id} </p> {/if} <div class="result-payload nes-container"> <p class="nes-text"> {result.payload} </p> </div> {#if result.metadata} {@const metadata = parseMetadata(result.metadata)} {#if metadata} <details class="metadata-details"> <summary class="nes-text">📋 Metadata</summary> <div class="metadata-content"> <table class="nes-table is-bordered"> <tbody> {#each Object.entries(metadata) as [key, value]} <tr> <td class="nes-text">{ key }</td> <td class="nes-text">{ value }</td> </tr> {/each} </tbody> </table> </div> </details> {:else} <details class="metadata-details"> <summary class="nes-text">📋 Metadata</summary> <div class="metadata-content"> <div class="nes-text">No structured metadata available</div> </div> </details> {/if} {/if} </div> {/each} </div> {:else if query.trim() && !isSearching && !error} <div class="no-results nes-container"> <p class="nes-text">🔍 No results found for: "{ query }"</p> {/if} </div> <style> .search-container { max-width: 800px; margin: 20px auto; padding: 20px; }
