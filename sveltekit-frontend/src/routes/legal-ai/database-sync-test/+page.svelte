...
<style>
.database-sync-test { max-width: 1400px; margin: 0 auto; padding: 2rem; font-family: system-ui, sans-serif; line-height: 1.6}
  header { text-align: center; margin-bottom: 3rem}
  .subtitle { color: #666; font-size: 1.1rem}
  section { margin: 3rem 0; padding: 2rem; border: 1px solid #e0e0e0; border-radius: 8px; background: #fafafa}
  section h2 { margin-top: 0; color: #333; border-bottom: 2px solid #0066cc; padding-bottom: 0.5rem}
  /* Status Dashboard */ .status-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem; margin: 1.5rem 0}
  .status-nier-bits-card { /* Corrected selector */ display: flex; align-items: center; gap: 0.75rem; padding: 1rem; background: white; border: 1px solid #ddd; border-radius: 6px; transition: border-color 0.2s}
  .status-nier-bits-card.online { /* Corrected selector */ border-color: #28a745}
  .status-indicator { width: 12px; height: 12px; border-radius: 50%; background: #dc3545}
  .status-nier-bits-card.online .status-indicator { /* Corrected selector */ background: #28a745}
  .last-checked { color: #666; font-size: 0.9rem; margin-top: 1rem}
  /* Data Grid */ .data-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem}
  ...
  .doc-type { background: #e3f2fd; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.8rem; text-transform: uppercase}
  .key-terms { display: flex; flex-wrap: wrap; gap: 0.5rem; margin-top: 0.75rem}
  .term { background: #f0f0f0; padding: 0.25rem 0.5rem; border-radius: 3px; font-size: 0.85rem; color: #555}
  .empty-state { color: #999; font-style: italic; text-align: center; padding: 2rem}
  /* Test Controls */ .test-actions { display: flex; flex-wrap: wrap; gap: 1rem; margin: 1.5rem 0}
  .test-btn { padding: 0.75rem 1.5rem; border: none; border-radius: 6px; font-size: 1rem; cursor: pointer; transition: all 0.2s; display: flex; align-items: center; gap: 0.5rem}
  ...
  .progress-fill { height: 100%; background: #0066cc; transition: width 0.3s ease}
  @keyframes spin { 0% { transform: rotate(0deg)} 100% { transform: rotate(360deg)} }
  /* Results Display */ .result-display { background: white; padding: 2rem; border-radius: 6px; border: 1px solid #ddd; margin: 1.5rem 0}
  .result-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 1rem; margin: 1rem 0}
  .result-item { display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; background: #f8f9fa; border-radius: 4px}
  .result-item label { font-weight: 600; color: #555}
  /* Replaced unused `.result-item .value` selectors with explicit input selectors that match the template */ .result-item input[readonly] { background: #e9ecef; padding: 0.25rem 0.5rem; border-radius: 3px; font-family: monospace; font-size: 0.9rem; border: 1px solid transparent}
  /* Apply: "hit" appearance to the readonly input (template uses; class:hit on the cache-hit input) */ .result-item input.hit { color: #28a745; font-weight: 600}
  /* Error Display */ .error-display { background: #ffebee; /* Corrected color code */ border: 1px solid #f44336; border-radius: 6px; padding: 1rem; color: #c62828; margin: 1rem 0}
  /* Test Log */ .log-container { background: #1e1e1e; /* Corrected color code */ color: #f0f0f0; padding: 1.5rem; border-radius: 6px; font-family: monospace; font-size: 0.9rem; max-height: 400px; overflow-y: auto}
  .log-entry { margin: 0.25rem 0; padding: 0.25rem 0; border-bottom: 1px solid #333}
  .empty-log { color: #888; text-align: center; font-style: italic}
  /* Sample Document */ .document-editor { width: 100%; padding: 1rem; border: 1px solid #ddd; border-radius: 6px; font-family: monospace; font-size: 0.9rem; resize: vertical; background: white}
  /* Responsive Design */ @media (max-width: 768px) { .data-grid { grid-template-columns: 1fr}
