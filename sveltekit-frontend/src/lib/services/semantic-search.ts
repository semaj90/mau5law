...
export interface SemanticSearchResult { results: Array<{
	id: string, content: string, metadata: { [key: string]: unknown } similarity: number, score, number}>; analytics: {
	searchStrategy: string, queryComplexity: string, string[], cacheHit: boolean, processingTime: number} suggestions?: string[]}
