...
export interface OllamaChatRequest { model: string; messages: OllamaChatMessage[];
  stream?: boolean;
  options?: OllamaOptions;
 }
export interface OllamaChatResponseChunk { model: string; created_at: string;
...
function getDefaultHost(): string {
  // Delegate host resolution to the centralized helper which prefers Vite env, // Node env and falls back to localhost. This keeps client/server behavior consistent.
...
export async function* generateStream(
  ...
  try {
    ...
    if (buffer.trim()) {
      try {
        const last = JSON.parse(buffer.trim()) as OllamaGenerateResponse;
        yield last;
       }catch {
        // ignore
       }
...
export async function* chatStream(
  ...
  try {
    ...
    if (buffer.trim()) {
      try {
        const last = JSON.parse(buffer.trim()) as OllamaChatResponseChunk;
        yield last;
       }catch {
        // ignore
       }
...
export const Ollama = {
  generate, generateStream, chat, chatStream, embeddings, listModels
};
