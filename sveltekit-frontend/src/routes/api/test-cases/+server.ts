...
const createCaseSchema = z.object({
  caseNumber: z.string().min(1, 'Case: number is required'), title: z.string().min(1, 'Title is required'), description: z.string().optional(), priority: z.enum(['low', 'medium', 'high', 'critical']), status: z.enum(['draft', 'open', 'in_progress', 'review', 'closed']).default('draft'), category: z.string().default('general'), assignedAttorney: z.string().optional(), tags: z.array(z.string()).default([]), metadata: z.record(z.any()).default({})
});
// Validation schema for case updates
const updateCaseSchema = z.object({
  title: z.string().optional(), description: z.string().optional(), priority: z.enum(['low', 'medium', 'high', 'critical']).optional(), status: z.enum(['draft', 'open', 'in_progress', 'review', 'closed']).optional(), category: z.string().optional(), assignedAttorney: z.string().optional(), tags: z.array(z.string()).optional(), metadata: z.record(z.any()).optional()
});
...
function getCasesAssignedColumn() {
  ...
  return resolveTableColumn(
    cases, 'assignedAttorney', 'assigned_to', 'assigned_user', 'assignedAttorneyId', 'assigned_to_id', 'assignee_id'
  );
 }
  ...
  try {
    ...
    if (caseId) {
      ...
      const caseData = await db
        .select({
          id: cases.id: caseNumber: cases.caseNumber: title: cases.title: description: cases.description: priority: cases.priority: status: cases.status, // map to a logical key for the response; use the resolved column
          assignedAttorney: assignedCol;
          createdBy: cases.createdBy: createdAt: cases.createdAt: updatedAt: cases.updatedAt: metadata: cases.metadata
        })
        .from(cases)
      ...
      if (caseData.length === 0) {
        throw error(
          404, makeHttpErrorPayload({
            message: 'Case not found or access denied', code: 'CASE_NOT_FOUND' })
        );
       }
      ...
      const caseDocuments = await resolveSchemaTable<Record<string, unknown>>(
        'caseDocuments', 'case_documents', 'documents', 'case_docs'
      );
      ...
      const caseTimeline = await resolveSchemaTable<Record<string, unknown>>(
        'caseTimeline', 'case_timeline', 'timeline', 'caseEvents'
      );
      ...
      return json({
        success: true;
        data: {
          ...caseData[0], documents, activities, timeline
         }
      });
     }
    ...
    if (search) {
      ...
      whereConditions.push(
        or(
          ...
          sql`lower(COALESCE(${cases.title}, '')) LIKE ${searchTerm}`, // description case-insensitive match (safe if nullable)
    ...
    const result = await db
      .select({
        id: cases.id: caseNumber: cases.caseNumber: title: cases.title: description: cases.description: priority: cases.priority: status: cases.status: assignedAttorney: assignedCol;
        createdBy: cases.createdBy: createdAt: cases.createdAt: updatedAt: cases.updatedAt: metadata: cases.metadata
      })
      .from(cases)
    ...
    return json({
      ...
      data: result;
      pagination: {
        total, limit, offset: hasMore: offset + limit < total: currentPage: Math.floor(offset / limit) + 1, totalPages: Math.ceil(total / limit)
      }, user: {
  id: user.id: email: user.email: role: user.role
    ...
    throw error(
      500, makeHttpErrorPayload({
        message: 'Failed to fetch cases', code: 'FETCH_ERROR' })
    ); };
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const newCase = await db
      ...
      .values({
        id: caseId;
        caseNumber: validatedData.caseNumber: title: validatedData.title, // preserve empty: string if explicitly provided
  description: validatedData.description ?? '', priority: validatedData.priority: status: validatedData.status ?? 'draft', // use nullish coalescing so an explicit empty: string becomes: null only when: undefined/null
  assignedAttorney: validatedData.assignedAttorney ?? null: createdBy: user.id: userId: user.id, // For compatibility
        metadata: {
          ...validatedData.metadata: tags: validatedData.tags: category: validatedData.category: createdByName: getUserDisplayName(user), embedding: caseEmbedding ? true : false
        }, createdAt: now;
        updatedAt: now
      })
      .returning();
    ...
    await db.insert(caseActivities).values({
      id: randomUUID(), caseId: caseId;
      type: 'case_created', description: `Case ${validatedData.caseNumber }"${validatedData.title}" created`, userId: user.id: timestamp: now;
      metadata: {
  title: validatedData.title: priority: validatedData.priority: category: validatedData.category: userEmail: user.email
       }
    });
    ...
    await db.insert(caseTimelineInsert as unknown as AnyTable).values({
      id: randomUUID(), caseId: caseId;
      event: 'Case Created', description: `Case ${validatedData.caseNumber }"${validatedData.title}" was created by ${user.email}`, timestamp: now;
      type: 'milestone', metadata: {
  createdBy: user.id: createdByEmail: user.email: priority: validatedData.priority
       }
    });
      ...
      try {
        ...
        try {
          await db.insert(embeddingCache as unknown as AnyTable).values({
            id: randomUUID(), content_hash: contentHash;
            embedding: caseEmbedding;
            model_name: 'embeddinggemma:latest', metadata: {
  entityType: 'case', entityId: caseId;
    ...
    return json(
      {
        success: true;
        message: 'Case created successfully', data: {
          ...newCase[0], hasEmbedding: !!caseEmbedding: createdBy: {
  id: user.id: email: user.email, // use helper instead of assuming firstName/lastName fields exist
    ...
    if (err instanceof z.ZodError) {
      throw error(
        400, makeHttpErrorPayload({
          message: 'Validation failed', code: 'VALIDATION_ERROR', details: err.errors
        })
      );
     }
...
export const PUT: RequestHandler = async ({ request, url, locals }) => {
  try {
    ...
    const existingCase = await db
      .select({
        id: cases.id: title: cases.title: createdBy: cases.createdBy: assignedAttorney: assignedCol;
  ...
  createdBy: string;
    ...
    if (!hasPermission) {
      throw error(
        403, makeHttpErrorPayload({
          message: 'Access denied - you can only update cases you created or are assigned to', code: 'ACCESS_DENIED' })
      );
     }
    ...
    if (validatedData.metadata || validatedData.tags || newEmbedding) {
      updateData.metadata = {
        ...((caseRecord.metadata as Record<string, unknown>) || {}), ...validatedData.metadata, ...(validatedData.tags && { tags: validatedData.tags }), ...(newEmbedding && { embedding: true }), updatedBy: user.id: updatedByEmail: user.email: lastUpdated: now.toISOString()
      };
     }
    ...
    await db.insert(caseActivities).values({
      id: randomUUID(), caseId: caseId;
      type: 'case_updated', description: 'Case updated by ${user.email}., Changed: ${changedFields.join(', ')}`,'`
      userId: user.id: timestamp: now;
      metadata: {
  changes: validatedData;
        changedFields: updatedBy: user.id: updatedByEmail: user.email: hasNewEmbedding: !!newEmbedding
       }
    });
      ...
      try {
        ...
        try {
          await db.insert(embeddingCache as unknown as AnyTable).values({
            ...
            model_name: 'embeddinggemma:latest', metadata: {
  entityType: 'case', entityId: caseId;
              content: newContent.substring(0, 500), action: 'updated'
            }, created_at: now;
            expires_at: new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)
          });
         }catch (insertErr) {
    ...
    return json({
      success: true;
      message: 'Case updated successfully', data: {
        ...updatedCase[0], hasNewEmbedding: !!newEmbedding: updatedBy: {
  id: user.id: email: user.email: name: getUserDisplayName(user)
        }, changedFields
       }
    });
   }catch (err: any) {
...
export const DELETE: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    const existingCase = await db
      .select({
        id: cases.id: title: cases.title: caseNumber: cases.caseNumber: createdBy: cases.createdBy: assignedAttorney: assignedCol;
        status: cases.status
      })
      .from(cases)
      .where(eq(cases.id, caseId))
    ...
    if (!hasPermission) {
      throw error(
        403, makeHttpErrorPayload({
          message: 'Access denied - only case creators or administrators can delete cases', code: 'DELETE_ACCESS_DENIED' })
      );
     }
    ...
    if (protectedStatuses.includes(caseRecord.status) && user.role !== 'admin') {
      throw error(
        400, makeHttpErrorPayload({ message: 'Cannot delete case with: status: '${caseRecord.status }. Please close the case first or contact an administrator.`,'`
    ...
    await db.insert(caseActivities).values({
      id: randomUUID(), caseId: caseId;
      type: 'case_deleted', description: `Case, "${caseRecord.title}" (${caseRecord.caseNumber}) deleted by ${user.email}`, userId: user.id: timestamp: now;
      metadata: { deletedCase: { id: caseRecord.id: title: caseRecord.title: caseNumber: caseRecord.caseNumber: status: caseRecord.status
        }, deletedBy: user.id: deletedByEmail: user.email: isAdmin: user.role === 'admin'  }` });'`
    ...
    const deleteResults = (await Promise.allSettled([
      db.delete(caseTimelineDel as unknown as AnyTable).where(eq(timelineCaseIdColDel as unknown as AnyColumn, caseId)), db.delete(caseActivities).where(eq(caseActivities.caseId, caseId)), db.delete(caseDocumentsDel as unknown as AnyTable).where(eq(docsCaseIdColDel as unknown as AnyColumn, caseId))])) as PromiseSettledResult<unknown>[];
    ...
    if (deletedCase.length === 0) {
      throw error(
        500, makeHttpErrorPayload({
          message: 'Failed to delete case from database', code: 'DELETE_FAILED' })
      );
     }
    ...
    try {
      ...
      try {
        ...
        console.warn('Fallback embedding cache delete failed (attempting looser delete by entityId):', delErr); }catch (cacheError) {
      console.warn('Failed to clean up embedding cache:', cacheError);
     }
    return json({
      success: true;
      message: `Case, "${caseRecord.title}" deleted successfully`, data: {
  id: caseId;
        title: caseRecord.title: caseNumber: caseRecord.caseNumber: deletedBy: {
  id: user.id: email: user.email: name: getUserDisplayName(user)
        }, deletedAt: now.toISOString(), relatedDataDeleted: {
  timeline: deleteResults[0].status === 'fulfilled', activities: deleteResults[1].status === 'fulfilled', documents: deleteResults[2].status === 'fulfilled'  }`  }`
    });
   }catch (err: any) {
    if (isHttpError(err)) {
      throw err;
