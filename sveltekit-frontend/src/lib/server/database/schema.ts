...
export const documentsTable = pgTable('legal_documents', {
 id: uuid('id').primaryKey().defaultRandom(, case_id: uuid('case_id').references(() => casesTable.id, title: varchar('title', { length: 500 }).notNull(, content: text('content', document_type: varchar('document_type', { length: 100 }, file_path: text('file_path', file_size: integer('file_size'),
...
export const evidenceTable = pgTable('evidence_items', {
 ...
 priority: varchar('priority', { length: 20 }).default('medium', status: varchar('status', { length: 50 }).default('active'),
 // Location in document
 page_number: integer('page_number', paragraph_index: integer('paragraph_index', character_offset: integer('character_offset'),
...
export const timelineEventsTable = pgTable('timeline_events', {
 id: uuid('id').primaryKey().defaultRandom(, case_id: uuid('case_id').references(() => casesTable.id, title: varchar('title', { length: 500 }).notNull(, description: text('description', event_date: timestamp('event_date').notNull(, event_type: varchar('event_type', { length: 100 }, is_milestone: boolean('is_milestone').default(false),
 // Related entities
...
export const queryCache = pgTable('query_cache', {
 ...
 // Cached result data
 result_data: jsonb('result_data', result_metadata: jsonb('result_metadata'),
 // Cache management
