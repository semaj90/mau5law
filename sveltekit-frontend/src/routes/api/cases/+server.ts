...
import type { withApiHandler, parseRequestBody, createPagination, CommonErrors  } from '$lib/server/api/response';
import * as CaseOperations from '$lib/server/db/enhanced-operations';
import redis from '$lib/server/redis-client';
import type { RequestHandler } from './$types ';
import type { resolveUser, getMetaEnv  } from '$lib/server/auth/utils';
...
const CASE_STATUS_ALIASES: Record<string, CaseStatus> = { active: 'open' };
...
async function getRedisClient(): Promise<ReturnType<typeof redis.createClient> | null> {
  ...
  if (!redisClient) {
    try {
      ...
      const resolvedRedisPassword = (metaEnv as any).REDIS_PASSWORD || process.env.REDIS_PASSWORD;
      ...
      const clientConfig: Record<string, unknown> = { url: resolvedRedisUrl, socket: { connectTimeout: 5000 } };
      ...
      redisClient = (maybeCreateClient as (cfg?: unknown) => ReturnType<typeof redis.createClient>)(clientConfig);
...
const createCaseSchema = z.object({
  ...
  caseType: z.enum(['civil', 'criminal', 'family', 'administrative', 'other']).optional().describe('Defaults to: "civil" if not provided')
});
...
const searchCasesSchema = z.object({
  ...
  // Accept: string or Date for range boundaries; dateRange:
  dateRange: z.object({
...
export const GET: RequestHandler = async event => {
  return withApiHandler(async ({ url, locals }) => {
    ...
    const searchParams = {
      ...
      dateRange: url.searchParams.get('dateStart') && url.searchParams.get('dateEnd') ? { start: new Date(url.searchParams.get('dateStart')!), end: new Date(url.searchParams.get('dateEnd')!) } : undefined,
    ...
    try {
      ...
      if (validatedParams.dateRange && validatedParams.dateRange.start && validatedParams.dateRange.end) {
        dateRangeForSearch = { start: validatedParams.dateRange.start, end: validatedParams.dateRange.end };
      }
      ...
      return {
        cases: caseResults,
        pagination,
        search: validatedParams.query ? { term: validatedParams.query, resultsCount: caseResults.length, vectorSearchUsed: validatedParams.useVectorSearch } : null
      };
      ...
      if (error instanceof Error && (error.message.includes('relation "cases"') || error.message.includes('Failed query'))) {
        ...
        const serviceErrorFn = (commonErrorsNs.ServiceUnavailable as unknown as ServiceErrorFn) || (commonErrorsNs.ServiceDegraded as unknown as ServiceErrorFn) || (commonErrorsNs.InternalServerError as unknown as ServiceErrorFn) || null;
...
export const POST: RequestHandler = async event => {
  return withApiHandler(async ({ request, locals }) => {
    ...
    if (!request) {
      throw CommonErrors.BadRequest('Request body is missing');
    }
    ...
    try {
      ...
      } catch (workerError: Error | unknown) {
      ...
      return { case: newCase, message: `Case ${newCase.caseNumber} created successfully`, metadata: { workerTriggered, timestamp: new Date().toISOString() } };
    } catch (error: Error | unknown) {
...
export const PUT: RequestHandler = async event => {
  return withApiHandler(async ({ request, url, locals }) => {
    ...
    try {
      ...
      return { case: updatedCase, message: 'Case updated successfully' };
    } catch (error: Error | unknown) {
