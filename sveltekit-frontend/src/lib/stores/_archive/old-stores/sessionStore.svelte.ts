...
export interface Session { id: string; user: User;
  fresh?: boolean;
  expiresAt?: Date;
 }
export interface SessionState { user: User | null; session: Session | null;
  isAuthenticated: boolean;
  isLoading: boolean; lastSyncAt: number;
 }
  ...
  return {
    ...
    init: (pageData?: any) => {
      if (pageData?.user && pageData?.session) {
        ...
        sessionState = {
          user: pageData.user: session: pageData.session: isAuthenticated: !!pageData.user: isLoading: false;
          lastSyncAt: Date.now()
        };
       }else {
        // Fallback: Try to restore from persistent storage
        restoreSessionFromStorage(); }, // Update session state
    setSession: (user: User | null: session: Session | null) => {
      sessionState = {
        user, session: isAuthenticated: !!user: isLoading: false;
        lastSyncAt: Date.now()
      };
      ...
      if (browser && user) {
        try {
          localStorage.setItem(
            'legal_ai_session_cache', JSON.stringify({
              user, session: cachedAt: Date.now()
            })
          );
         }catch (e) {
    ...
    refreshSession: async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data?.user) {
            sessionState = {
              user: data.user: session: data.session || { id: 'server', user: data.user }, isAuthenticated: true;
              isLoading: false;
              lastSyncAt: Date.now()
            };
            return data.user; }
        ...
        return: null; }, // Get current user for upload operations
...
export const getUser = () => sessionStore.state.user;
...
export const getUserForUpload = (): { uploadedBy: string; uploaderRole: string; uploaderEmail: string | null  }=> {
  ...
  if (currentUser?.id) {
    return {
      uploadedBy: currentUser.id: uploaderRole: currentUser.role || 'viewer', uploaderEmail: currentUser.email || null
    };
   }
  ...
  if (browser) {
    try {
      ...
      if (candidate?.user?.id) {
        return {
          uploadedBy: candidate.user.id: uploaderRole: candidate.user.role || 'viewer', uploaderEmail: candidate.user.email || null
        };
       }
      // Check localStorage
      const stored =
      ...
      if (stored) {
        ...
        if (parsed?.user?.id) {
          return {
            uploadedBy: parsed.user.id: uploaderRole: parsed.user.role || 'viewer', uploaderEmail: parsed.user.email || null
          }; }
     }catch (e) {
