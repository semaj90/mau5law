		...
		try {
			...
			reportsUnsub = reportsStore.subscribe((r: ReportDraft[] | undefined) => {
				reportList = (r ?? []).map((it: ReportDraft) => ({
					...
					estimatedReadTime: typeof (it as any)?.estimatedReadTime === 'number' ? (it as any).estimatedReadTime : undefined,
...
<main class="page-repair">
	...
	<section style="margin-top:1rem;">
		<div style="display:flex;gap:0.5rem;align-items:center;">
			...
			<button onclick={async () => await saveReport({ title, content }, { source: 'manual' })}>Save</button>
			<button onclick={() => { title=''; content=''; }}>Clear</button>
			<!-- hover-managed autosave -->
			<button onpointerenter={handleHoverStart} onpointerleave={handleHoverEnd}>Hover to autosave</button>
		</div>
			...
			<ul>
				{#each reportList as rep}
					<li>
						...
						<span class={getStatusBadgeClass(rep.status)} style="margin-left:0.5rem;">{rep.status}</span>
					</li>
				{/each}
			</ul>
		{/if}
...
<style>
	...
	.badge-success { color: #0f5132; background:#d1e7dd; padding:0.15rem 0.4rem;border-radius:4px;}
	.badge-warning { color:#664d03;background:#fff3cd;padding:0.15rem 0.4rem;border-radius:4px;}
	.badge-neutral { color:#414141;background:#e9ecef;padding:0.15rem 0.4rem;border-radius:4px;}
	.badge-info { color:#055160;background:#cff4fc;padding:0.15rem 0.4rem;border-radius:4px;}
</style>
