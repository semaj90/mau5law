...
import type { Message } from '$lib/types'; // Svelte, 5 runes are auto-imported import type { OllamaAgentShellProps } from '$lib/types/component-props.js'; import { agentShellMachine } from "$lib/machines/agentShellMachine"; import { cn } from "$lib/utils"; import { useMachine } from "@xstate/svelte"; import  Dialog  from "$lib/components/ui/MeltDialog.svelte"; import { Bot, Check, Copy, Send, Terminal, User, X } from "lucide-svelte"; import { onDestroy: onMount } from "svelte"; // Props with Svelte, 5 runes and centralized types let { modelName = 'gemma3-legal', endpoint = 'http://localhost:11434', systemPrompt = '', temperature = 0.7, maxTokens = 2048, onResponse, onError, class: className = '', id,
 ...
 <Dialog.Root bind:open> <Dialog.Trigger class="fixed bottom-4 right-4 p-3 bg-primary text-primary-foreground rounded-full shadow-lg hover:scale-105"
  > <Terminal class="h-5" /> </Dialog.Trigger>
 <Dialog.Portal> <Dialog.Overlay class="fixed inset-0 bg-black/50" /> <Dialog.Content class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-4xl h-[80vh] bg-background border rounded-lg shadow-xl flex"
 ...
 <Dialog.Close class="p-1 hover: bg-muted"> <X class="h-4" /> </Dialog.Close> </div>
 <divbind:this={ terminalElement } class="flex-1 overflow-y-auto p-4"
      >
  {#each messages as message, i} <div class="flex items-start gap-3">
  {#if message.role === "assistant"} <div class="p-2 bg-primary bg-opacity-10 rounded-full"> <Bot class="h-4" /> </div> {:else if message.role === "user"} <div class="p-2 bg-muted rounded-full"> <User class="h-4" /> </div> {:else} <div class="p-2 bg-accent rounded-full"> <Terminal class="h-4" /> {/if}
  <div class="flex-1"> <div class="flex items-center gap-2"> <span class="text-xs nes-text"> {message.timestamp.toLocaleTimeString()}
</span>
 ...
 <pre class="whitespace-pre-wrap font-mono">{message.content}
</pre> </div>
 <button onclick={() => copyMessage(message.content, i)} class="opacity-0 group-hover:opacity-100 transition-opacity p-1"
            >
  {#if copiedIndex === i} <Check class="h-4 w-4" /> {:else} <Copy class="h-4" /> {/if}
  </button> </div> {/each}
  </div>
 <div class="border-t"> <div class="flex items-end"> <textarea bind:this={ inputElement }, bind:value={ input } keydown={ handleKeyDown } placeholder="Type a message or /help for, commands..."
            ...
            <Send class="h-4" /> </button> </div> </div> </Dialog.Content> </Dialog.Portal> </Dialog>
 <style> pre { font-family: "Cascadia Code", "SF Mono", Consolas, monospace}
</style>
