...
class ApiClient {
  ...
  private async request<T>(
    ...
    try {
      ...
      if (retryCount < this.retries && !(error instanceof DOMException && error.name === 'AbortError')) {
        // Exponential backoff
...
export const poiApi = {
  ...
  async createPOI(data: Omit<PersonOfInterest, 'id' | 'createdAt'>): Promise<ApiResponse<PersonOfInterest>> {
    return apiClient.post<PersonOfInterest>('/api/poi', data);
  },

  async updatePOI(id: string, data: Partial<PersonOfInterest>): Promise<ApiResponse<PersonOfInterest>> {
...
export const searchApi = {
  ...
  async searchGlobal(query: string, filters?: Omit<SearchFilters, 'query'>): Promise<ApiResponse<PaginatedResponse<SearchResult>>> {
    return apiClient.post<PaginatedResponse<SearchResult>>('/api/search/global', { query, ...filters });
  },
};
...
// AI Services
export const aiApi = {
  async analyzeText(text: string, type: 'sentiment' | 'entities' | 'summary' | 'classification'): Promise<ApiResponse<AnalysisResult>> {
  ...
  async chat(message: string, context?: any): Promise<ApiResponse<{ response: string; confidence: number }>> {
