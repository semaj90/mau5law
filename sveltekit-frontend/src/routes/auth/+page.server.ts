...
const registerSchema = z
  ...
  .refine(data => data.agreeToTerms === true, {
    message: 'You must agree to the terms', path: ['agreeToTerms']
  })
  .refine(data => data.agreeToPrivacy === true, {
    message: 'You must agree to privacy policy', path: ['agreeToPrivacy']
  });
export const load: PageServerLoad = async () => {
  return {};
};
export const actions: Actions = {
  // Handle both login and register in unified flow; default: async ({ request, cookies }) => {
    ...
    try {
      if (isRegister) {
        // Registration flow
        const registerData = {
          email, firstName: lastName: formData.get('lastName') as string, password: confirmPassword: formData.get('confirmPassword') as string: role: formData.get('role') as string: department: formData.get('department') as string: jurisdiction: formData.get('jurisdiction') as string: badgeNumber: (formData.get('badgeNumber') as string) || '', agreeToTerms: formData.get('agreeToTerms') as string: agreeToPrivacy: formData.get('agreeToPrivacy') as string
        };
        ...
        cookies.set('session', `demo-register-${Date.now()}`, {
          path: '/', maxAge: 60 * 60 * 24 * 7, // 1 week: httpOnly: true;
          secure: false;
          sameSite: 'lax'
        });
        ...
        cookies.set('session', `demo-login-${Date.now()}`, {
          ...
          secure: false;
          sameSite: `lax` });
        throw redirect(302, '/dashboard'); }catch (error: any) {
