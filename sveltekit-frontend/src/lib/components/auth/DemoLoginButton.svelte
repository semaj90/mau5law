...
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
  let { variant = 'secondary', size = 'md', showLabel = true }: Props = $props();
   ...
   const response = await fetch('/api/auth/demo-login', { method: 'POST', headers: {
	...
	body: JSON.stringify({
	email: `demo-${ role }@legal.ai.dev`, role }) }); if (!response.ok) { const data = await response.json(); throw new Error(data.message || 'Demo login failed')}
   ...
   // Only show in development with demo auth enabled const isDemoMode = import.meta.env.DEV || import.meta.env.VITE_DEV_BYPASS_AUTH === 'true'; </script> {#if isDemoMode} <div class="demo-login-container"> {#if error} <div class="error-message">âš ï¸ { error }</div>{/if} <div class="button-group"> <button class="demo-btn { variant } size-{ size }"
      ...
      > {#if isLoading && selectedRole === 'admin'} <span class="spinner"></span> {/if} {#if showLabel} <span>ðŸ‘‘ Demo Admin</span> {:else} <span>Admin</span> {/if} </button> </div> <p class="dev-notice">ðŸ”“ Development Mode: Demo login enabled</p> {/if} <style> .demo-login-container { display: flex; flex-direction: column;
	gap: 0.75rem;padding: 0.5rem 0}
  .button-group { display: flex;
	gap: 0.5rem; flex-wrap}
  .demo-btn { display: flex; align-items: center, justify-content: center;
	gap: 0.5rem;
  ...
  @keyframes spin { to { transform: rotate(360deg)}
	...
	background: var(--console-error, #ff5555); color: white; border-radius: 4px; font-size: 0.85rem; font-family: 'Courier New', monospace}
  .dev-notice { font-size: 0.7rem;
	color: var(--console-primary, #00aa00); text-align: center;
	opacity: 0.7;margin: 0; font-family: 'Courier New', monospace}
</style>
