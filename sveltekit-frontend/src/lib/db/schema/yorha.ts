import { pgTable, uuid, varchar, text, timestamp, integer, jsonb, serial } from 'drizzle-orm/pg-core';
...
export const yorhaEvidenceConnections = pgTable('yorha_evidence_connections', {
  id: uuid('id').primaryKey().defaultRandom(),
  caseId: uuid('case_id').notNull().references(() => yorhaCases.id, { onDelete: 'cascade' }),
  sourceNodeId: uuid('source_node_id').notNull().references(() => yorhaEvidenceNodes.id, { onDelete: 'cascade' }),
  targetNodeId: uuid('target_node_id').notNull().references(() => yorhaEvidenceNodes.id, { onDelete: 'cascade' }),
...
export const yorhaChatMessages = pgTable('yorha_chat_messages', {
  id: uuid('id').primaryKey().defaultRandom(),
  sessionId: uuid('session_id').notNull().references(() => yorhaChatSessions.id, { onDelete: 'cascade' }),
