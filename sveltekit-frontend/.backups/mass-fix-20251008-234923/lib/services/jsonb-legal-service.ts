...
export class JsonbLegalService {
  ...
  async findDocumentsByLegalCriteria(criteria: {
    ...
    try {
      ...
      if (criteria.documentTypes?.length) conditions.push(inArrayOp(legalDocumentsJsonb.documentType, criteria.documentTypes));
      if (criteria.practiceAreas?.length) conditions.push(inArrayOp(legalDocumentsJsonb.practiceArea, criteria.practiceAreas));
      if (criteria.jurisdictions?.length) conditions.push(inArrayOp(legalDocumentsJsonb.jurisdiction, criteria.jurisdictions));
      if (criteria.confidentialityLevels?.length) conditions.push(inArrayOp(legalDocumentsJsonb.confidentialityLevel, criteria.confidentialityLevels));
      if (criteria.minConfidence !== undefined) conditions.push(sql`(metadata->'aiMetadata'->>'confidence')::real >= ${criteria.minConfidence}`);
      if (criteria.hasHumanVerification !== undefined) conditions.push(sql`(metadata->'aiMetadata'->>'humanVerified')::boolean = ${criteria.hasHumanVerification}`);
      if (criteria.dateRange) {
        // Use a single parameterized SQL fragment for the date range
        conditions.push(sql`${legalDocumentsJsonb.createdAt} > ${criteria.dateRange.start} AND ${legalDocumentsJsonb.createdAt} < ${criteria.dateRange.end}`);
      }
      ...
      if (criteria.parties?.length) {
        for (const party of criteria.parties) {
          ...
          if (party.role) {
            conditions.push(sql`EXISTS (SELECT 1 FROM jsonb_array_elements(metadata->'parties') AS p WHERE p->>'name' ILIKE ${likeTerm} AND p->>'role' = ${party.role})`);
          } else {
            conditions.push(sql`EXISTS (SELECT 1 FROM jsonb_array_elements(metadata->'parties') AS p WHERE p->>'name' ILIKE ${likeTerm})`);
          }
        }
      }
      ...
      query = query.orderBy(desc(legalDocumentsJsonb.updatedAt)).limit(criteria.limit || 50).offset(criteria.offset || 0);
   ...
   */
  async analyzeLegalConcepts(documentIds: string[]): Promise<{ concepts: any[]; totalDocuments: number; conceptNetwork: Record<string, string[]> }> {
    ...
    try {
      ...
      if (typeof (LegalJsonbOperations as any).getDocumentConcepts === 'function') {
        ...
        const rows = await db.select({ id: legalDocumentsJsonb.id, metadata: legalDocumentsJsonb.metadata }).from(legalDocumentsJsonb).where(inArray(legalDocumentsJsonb.id, documentIds));
        docRows = (rows || []).map((r: any) => ({ documentId: r.id, concepts: ((r.metadata as any)?.semantics?.legalConcepts) || [] }));
      }
      ...
      await logger.logEvent?.({ type: 'ai_interaction', message: 'legal_concept_analysis', metadata: { operation: 'legal_concept_analysis', inputSize: documentIds.length, outputSize: (conceptResults || []).length, processingTime: duration, success: true } }).catch(() => {});
      return { concepts: conceptResults as any[] || [], totalDocuments: documentIds.length, conceptNetwork };
    } catch (err) {
      const duration = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start;
      await logger.logEvent?.({ type: 'ai_interaction', message: 'legal_concept_analysis_failed', metadata: { inputSize: documentIds.length, processingTime: duration, success: false, error: err instanceof Error ? err.message : String(err) } }).catch(() => {});
  ...
  async createCase(data: Omit<NewCase, 'id' | 'createdAt' | 'updatedAt'>): Promise<Case> {
    ...
    try {
      ...
      const duration = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start;
      await logger.logDocumentProcessing?.({ documentId: (caseRecord as any)?.id, operation: 'create_case', documentType: 'case', processingTime: duration, metadataSize: JSON.stringify(((data as any)?.metadata) ?? {}).length, success: true }).catch(() => {});
      return caseRecord as Case;
    } catch (err) {
      const duration = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start;
      await logger.logDocumentProcessing?.({ operation: 'create_case', documentType: 'case', processingTime: duration, metadataSize: JSON.stringify(((data as any)?.metadata) ?? {}).length, success: false, error: err instanceof Error ? err.message : String(err) }).catch(() => {});
      throw err;
    }
  }
  ...
  async addCaseTimelineEvent(caseId: string, event: { date: string; event: string; significance: 'low' | 'medium' | 'high' | 'critical' }): Promise<Case> {
    const start = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
    try {
      ...
      const updated = await db.update(casesJsonb).set({ metadata: newMeta, updatedAt: new Date() }).where(eq(casesJsonb.id, caseId)).returning();
      const updatedCase = Array.isArray(updated) ? updated[0] : updated;
      const duration = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start;
      await logger.logDocumentProcessing?.({ documentId: caseId, operation: 'update_timeline', documentType: 'case', processingTime: duration, metadataSize: JSON.stringify(event).length, success: true }).catch(() => {});
      return updatedCase as Case;
    } catch (err) {
      const duration = (typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now()) - start;
      await logger.logDocumentProcessing?.({ documentId: caseId, operation: 'update_timeline', documentType: 'case', processingTime: duration, metadataSize: JSON.stringify(event).length, success: false, error: err instanceof Error ? err.message : String(err) }).catch(() => {});
      throw err;
    }
  }
  ...
  // ========================================================================
  async createEvidence(_data: Omit<NewEvidence, 'id' | 'createdAt' | 'updatedAt'>, embedding?: VectorEmbedding): Promise<Evidence> {
    const startTime = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
    try {
   ...
   */
  async addCustodyTransfer(evidenceId: string, transfer: {
    timestamp: string;
    custodian: string;
    ...
    try {
      ...
      const updated = await db.update(evidenceJsonb)
        .set({
          metadata: sql`
            jsonb_set(
              metadata,
   ...
   */
  async verifyEvidenceChain(evidenceId: string): Promise<{ isValid: boolean; evidence: Evidence | null; chainValidation: any }> {
    const startTime = typeof performance !== 'undefined' && performance.now ? performance.now() : Date.now();
    try {
