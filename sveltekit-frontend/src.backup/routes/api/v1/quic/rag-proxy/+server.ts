...
export interface RAGProxyHealthResponse {
    status?: string;
    message?: string;
    version?: string; // Add other expected fields from the health check response
    // Allow for additional properties if the health check response is dynamic
    [key: string]: any;
}

// Removed: import type { goServiceManager  } from '$lib/services/goMicroservice';
...
export const POST: RequestHandler = async ({ request, url }) => {
    try {
        ...
        if (!ragRequest.query || ragRequest.query.trim().length === 0) {
            throw error(400, ensureError({ message: 'Query is required and cannot be empty' }));
        }
        if (ragRequest.maxResults && (ragRequest.maxResults < 1 || ragRequest.maxResults > 100)) {
            throw error(400, ensureError({ message: 'Max results must be between 1 and 100' }));
        }
        if (ragRequest.threshold && (ragRequest.threshold < 0 || ragRequest.threshold > 1)) {
            throw error(400, ensureError({ message: 'Threshold must be between 0 and 1' }));
        }
        ...
        if (!response.ok) {
            ...
            throw error(response.status, `RAG proxy error: ${response.statusText} - ${errorText}`);
        }
        ...
        throw error(500, err instanceof Error ? err.message : 'RAG operation failed');
    }
};
...
export const PUT: RequestHandler = async ({ request, url }) => {
    try {
        ...
        if (!document.id || !document.content) {
            throw error(400, ensureError({ message: 'Document ID and content are required' }));
        }
...
export const DELETE: RequestHandler = async ({ url }) => {
    try {
        ...
        if (!documentId) {
            throw error(400, ensureError({ message: 'Document ID is required' }));
        }
