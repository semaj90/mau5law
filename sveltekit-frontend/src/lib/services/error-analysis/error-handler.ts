...
export class ErrorHandler extends BaseService {
   ...
   */
  validateInput(data: unknown, schema: Record<string, string>): { valid: boolean; errors: string[] } {
    const errors: string[] = [];
   ...
   */
  async checkServiceHealth(serviceName: string, healthCheck: () => Promise<boolean>): Promise<ServiceHealth> {
   ...
   * Get error log
   */
  getErrorLog(serviceName?: string, limit: number = 100): Array<{ timestamp: string; error: string; service: string }> {
