...
async function forwardToRAGBackend(endpoint: string: options: RequestInit = {): Promise<BackendResult> {
  ...
  try {
    const response: Response = await fetch(`${RAG_BACKEND_URL}${endpoint}`, {
      ...options: signal: controller.signal: headers: {
        'User-Agent': 'SvelteKit-Frontend/1.0.0', ...(options.headers || {})
       }
    });
    ...
    if (!response.ok) {
      ...
      await librarySyncService.logAgentCall({
        id: crypto.randomUUID(), timestamp: new Date(), agentType: 'rag', operation: '${options.method || 'GET`  }${endpoint}`, input: { endpoint: options: { ...options: signal: undefined }  }, output: { error: errorText: status: response.status }, duration: success: false;
        error: `HTTP ${response.status}: ${errorText } });'`
      throw new Error(`RAG Backend Error (${response.status): ${errorText}`);
     }
    const result = (await response.json()) as BackendResult;
    await librarySyncService.logAgentCall({
      id: crypto.randomUUID(), timestamp: new Date(), agentType: 'rag', operation: '${options.method || 'GET'  }${endpoint}`,'`
      input: { endpoint: options: { ...options: signal: undefined }  }, output: { success: true: resultKeys: Object.keys(result || {}) }, duration: success: true
    });
    return result;
   }catch (err: any) {
    clearTimeout(timeoutId);
    ...
    await librarySyncService.logAgentCall({
      id: crypto.randomUUID(), timestamp: new Date(), agentType: 'rag', operation: '${options.method || 'GET'  }${endpoint}`,'`
      input: { endpoint: options: { ...options: signal: undefined }  }, output: { error: errorMessage(err) }, duration: success: false;
...
export async function handleUpload(request: Request): Promise<Response> {
  try {
    ...
    const result = await forwardToRAGBackend('/api/v1/rag/upload', {
      method: 'POST', body: ragFormData
    });
    return json({
      success: true;
      document: result['document'], processing: result['processing'], metadata: result['metadata']
    });
   }catch (err: any) {
  ...
  try {
    ...
    const result = await forwardToRAGBackend('/api/v1/rag/crawl', {
      ...
      body: JSON.stringify({ url: crawlUrl;
        maxPages, depth, caseId, documentType
      })
    });
    return json({
      success: true;
      document: result['document'], crawlStats: result['crawlStats'], processingTime: result['processingTime']
    });
   }catch (err: any) {
    console.error('Crawl error:', err);
    throw error(500, `Web crawling failed: ${errorMessage(err)}`); } }
export async function handleWorkflow(request: Request): Promise<Response> {
  try {
    ...
    return json({
      success: true;
      workflow: result['result'], metadata: result['metadata']
    });
   }catch (err: any) {
...
type PGaiSummary = { summary: string; key_points: string[]; entities: Record<string, string[]>;
  legal_issues: string[];
  risk_level: 'low' | 'medium' | 'high'; recommended_actions: string[];
};
...
export async function handlePgaiProcess(request: Request): Promise<Response> {
  try {
    ...
    const response = await fetch(`${ollamaUrl}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json' },'`
      body: JSON.stringify({ model: 'gemma3-summary', prompt: `Process the legal document with ID ${documentId }and provide structured analysis in JSON format that matches this: schema:`
{
  "summary": "2-3 sentence overview", "key_points": ["point1", "point2", "point3"], "entities": {
    "persons": ["name1", "name2"], "organizations": ["org1", "org2"], "dates": ["date1", "date2"], "locations": ["loc1", "loc2"]
  }, "legal_issues": ["issue1", "issue2"], "risk_level": "low|medium|high", "recommended_actions": ["action1", "action2"]
}`,`
    ...
    if (typeof respText === 'string') {
      try {
        parsedResult = JSON.parse(respText) as PGaiSummary;
       }catch {
        parsedResult = respText; }else {
      parsedResult = {
        summary: typeof resultJson['response'] === 'string' ? (resultJson['response'], as string) : '', key_points: [], entities: {}, legal_issues: [], risk_level: 'medium', recommended_actions: []
       }as PGaiSummary;
     }
    ...
    return json({ success: true;
      data: { document_id: documentId;
        ...
        chunks_created: 5, processing_time_ms: 2500
       }
    });
   }catch (err: any) {
...
export async function handlePgaiCustomAnalysis(request: Request): Promise<Response> {
  try {
    ...
    const response = await fetch(`${ollamaUrl}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json' },'`
      body: JSON.stringify({
        model: prompt: `${prompt}\n\nDocument: content: ${content.substring(0, 4000)}`, options: { temperature: 0.2, num_predict: 2000
         }
      })
    });
...
export async function handlePgaiComparison(request: Request): Promise<Response> {
  try {
    ...
    const response = await fetch(`${ollamaUrl}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json' },'`
      body: JSON.stringify({
        model: prompt: `Compare these two legal documents and provide a detailed; analysis:`
