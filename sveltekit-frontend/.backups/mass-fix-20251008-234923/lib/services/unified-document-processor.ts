...
export interface DocumentStructure {
  ...
  sections: Array<{ id: string; title: string; content: string; subsections: Array<Record<string, unknown>>; type: string; pageRange: { start: number; end: number } }>;
  footnotes: string[];
...
class UnifiedDocumentProcessor extends EventEmitter {
  ...
  private addError(errors: ProcessingError[], stage: string, message: string, severity: "warning" | "error" | "critical"): void {
    errors.push({
      stage,
      error: message,
      severity,
   ...
   */
  public async processDocument(file: File, config: DocumentProcessingConfig, metadata: Record<string, unknown>): Promise<ProcessingResult> {
    ...
    const baseResult: ProcessingResult = {
      ...
      analysis: {
        ...
        complexity: "low",
        legalDomains: [],
        documentStructure: { headers: [], sections: [], footnotes: [], references: [], signatures: [], tables: [], images: [] }
      },
    ...
    try {
      ...
      async function performLegalAnalysis(text: string): Promise<{ summary: string; keywords: string[]; legalDomains: string[] }> {
        // quick guard
      ...
      if (config.enableEmbeddings && baseResult.ocr.extractedText) {
        ...
        for (let i = 0; i < chunks.length; i++) {
          ...
          vectors.push(emb);
          textChunks.push({ id: `${documentId}-c-${i}`, content: c, startIndex: i * (config.chunkSize || 500), endIndex: (i + 1) * (config.chunkSize || 500), confidence: 0.9, metadata: { documentId, chunkIndex: i, ...(metadata || {}) } });
        }
      ...
      if (config.enableMinIOStorage) {
        const t4 = Date.now();
        baseResult.storage = { documentHash: "simulated-hash", encryptionStatus: false, minioUrl: undefined, databaseId: undefined, backupLocation: undefined };
      ...
      this.emit("document_processed", { documentId, success: baseResult.success, stagesCompleted: stagesCompleted.length, errors: errors.length });
      return baseResult;
    } catch (err: unknown) {
  ...
  private async searchEmbeddings(queryEmbedding: number[], options: { limit?: number; documentType?: string; filter?: Record<string, unknown> } = {}): Promise<any[]> {
      ...
      if (VECTOR_DB === "qdrant") {
        ...
        if (res.status === 204) {
          ...
          try {
            ...
            throw new Error(`Qdrant returned invalid JSON. status=${res.status} statusText=${res.statusText} body=${txt}`);
          }
        } else {
          const txt = await res.text().catch(() => "");
          throw new Error(`Unexpected content-type from Qdrant: ${contentType}. status=${res.status} statusText=${res.statusText} body=${txt}`);
        }
  ...
  public async healthCheck(): Promise<{ overall: boolean; services: Record<string, boolean>; details: Record<string, unknown> }> {
...
export const documentProcessingUtils = {
  ...
  validateResult: (result: ProcessingResult): { valid: boolean; issues: string[] } => {
    ...
    if (result.success && result.metadata.stagesCompleted.length === 0) issues.push("Success claimed but no stages completed");
    if (result.ocr.extractedText && result.ocr.confidence < 50) issues.push("Low OCR confidence may affect accuracy");
    if (result.embeddings.searchReady && result.embeddings.vectors.length === 0) issues.push("Embeddings marked ready but no vectors generated");
