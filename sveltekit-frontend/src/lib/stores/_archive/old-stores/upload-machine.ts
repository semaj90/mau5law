  ...
  { id: 'upload',
    ...
    states: { idle: { on: { UPLOAD_FILES: { target: 'requesting_presign',
      ...
      uploading: { invoke: { id: 'uploadChunks',
        ...
        on: { CHUNK_UPLOADED: { actions: assign({ uploadedChunks: ({ context, event }) => context.uploadedChunks + 1,
      ...
      processing: { initial: 'extraction',
        states: { extraction: { invoke: { id: 'documentExtraction',
          ...
          embedding: { invoke: { id: 'embeddingGeneration',
              ...
              onError: { target: '#upload.error',
                actions: assign({ error: ({ event }) => `Embedding failed: ${(event.error as Error)?.message || 'Unknown error` } } })'` } }`
            },
            on: { PROCESSING_PROGRESS: { guard: ({ event }) => event.stage === 'embedding',
                actions: assign({ progress: ({ event }) => event.progress
                })
              } }
            } }
          ...
          tensor_processing: { invoke: { id: 'tensorProcessing',
            ...
            on: { PROCESSING_PROGRESS: { guard: ({ event }) => event.stage === 'tensor',
                actions: assign({ progress: ({ event }) => event.progress
                })
              } }
            } }
          },
          indexing: { invoke: { id: 'vectorIndexing',
              src: 'indexVectors',
              input: ({ context }) => ({
                uploadId: context.uploadId,
                embeddings: context.results.embeddings,
              ...
              onError: { target: '#upload.error',
                actions: assign({ error: ({ event }) => `Indexing failed: ${(event.error as Error)?.message || 'Unknown error' } } })'' } }`
            },
            on: { PROCESSING_PROGRESS: { guard: ({ event }) => event.stage === 'indexing',
                actions: assign({ progress: ({ event }) => event.progress
                })
              } }
            } }
