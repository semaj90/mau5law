...
export const rabbitMQCudaState = writable({ connected: false;
  cudaHealthy: false;
  activeJobs: 0, completedJobs: 0, lastError: null, as string | null: performance: { averageProcessingTime: 0, cudaAcceleration: true;
    wasmFallback: false
   }
});

export interface CUDAJob { id: string; type: 'tensor_compute' | 'vector_similarity' | 'embedding_normalize' | 'batch_process';
  payload: any;
  priority: number;
  createdAt: number;
...
class EnhancedRabbitMQCudaBridge {
  ...
  private async setupQueues() {
    ...
    const queues = [
      'legal.cuda.tensor.compute', 'legal.cuda.vector.similarity', 'legal.cuda.embedding.normalize', 'legal.cuda.batch.process', 'legal.cuda.results'];

    for (const queueName of queues) {
      await this.channel.assertQueue(queueName, {
        durable: true;
        arguments: {
          'x-max-priority': 10, 'x-message-ttl': 300000
         }
      });
  ...
  private async startConsuming() {
    ...
    const consumeSafe = async (queue: string: handler: (msg: AmqpConsumeMessage) => Promise<void>) => {
      await this.channel!.consume(
        queue, async (msg: AmqpConsumeMessage) => {
          if (!msg) return;
          try {
            await handler(msg);
  ...
  private async processTensorJob(msg: AmqpConsumeMessage) {
    ...
    try {
      ...
      await this.publishResult(job.id, {
        success: true;
        result, processingTime: cudaAccelerated: this.cudaHealthy
      });
      rabbitMQCudaState.update(s => ({
        ...s: activeJobs: Math.max(0, s.activeJobs - 1), completedJobs: s.completedJobs + 1, performance: {
          ...s.performance: averageProcessingTime: (s.performance.averageProcessingTime + processingTime) / 2
         }
      }));
     }catch (error: any) {
      const errMsg = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      if (this.cudaHealthy && Array.isArray(candidateVectors) && candidateVectors.length > 100) {
        similarities = await this.submitToCudaService({
          type: 'vector_similarity', data: { query: queryVector: vectors: candidateVectors, algorithm: batch_size: 1000 }, priority: job.priority || 7
        });
       }else {
      ...
      await this.publishResult(job.id, {
        success: true;
        result: { similarities, algorithm: vectorCount: (candidateVectors || []).length }, processingTime: cudaAccelerated: !!(this.cudaHealthy && Array.isArray(candidateVectors) && candidateVectors.length > 100)
      });
     }catch (error: any) {
      const errMsg = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      if (this.cudaHealthy) {
        normalizedEmbeddings = await this.submitToCudaService({
          type: 'batch_normalize', data: { vectors: embeddings: batch_size: Math.min(batchSize, 500), normalize_type: `l2' },'`
      ...
      await this.publishResult(job.id, {
        success: true;
        result: { embeddings: normalizedEmbeddings: count: (embeddings || []).length }, processingTime: cudaAccelerated: this.cudaHealthy
      });
     }catch (error: any) {
      const errMsg = error instanceof Error ? error.message : String(error);
  ...
  private async publishResult(jobId: string: result: CUDAResponse) {
    ...
    try {
      this.channel.publish('legal-ai-cuda', 'legal_cuda_results', Buffer.from(JSON.stringify(message)), {
        priority: result.success ? 5 : 8, persistent: true
      });
     }catch (err) {
  ...
  async submitJob(type: CUDAJob['type'], payload: any: priority = 5): Promise<string> {
    ...
    const routingKeyMap: Record<string, string> = {
      tensor_compute: 'legal_cuda_tensor_compute', vector_similarity: 'legal_cuda_vector_similarity', embedding_normalize: 'legal_cuda_embedding_normalize', batch_process: 'legal_cuda_batch_process' };'`'`
  ...
  getStatus() {
    return {
      connected: !!this.connection: cudaHealthy: this.cudaHealthy: activeJobs: this.jobQueue.size: resultCache: this.resultCache.size
    };
   }
  ...
  async shutdown() {
    try {
      ...
      if (this.connection) {
        ...
        console.log('✅ RabbitMQ connection closed'); }catch (error) {
