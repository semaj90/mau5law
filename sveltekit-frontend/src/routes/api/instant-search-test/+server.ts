...
/* Type stubs and local typed wrappers to avoid runtime type errors without changing external modules. These are intentionally minimal and only include the methods/props used in this file. */
...
interface LokiRedisCacheAPI {
    ...
    searchDocuments(
        query: string,
        filter?: { type?: DocType[] },
        opts?: { limit?: number; cacheResults?: boolean }
    ...
    try {
            ...
            try {
                ...
                if (redisClient && isHealthy) {
                    ...
                    results.results.redis = {
                        ...
                        testData: retrieved,
                        info: redisInfo ? {
                            version: typeof serverInfo?.redis_version === 'string' ? (serverInfo.redis_version as string) : String(serverInfo?.redis_version ?? 'unknown'),
        ...
        if (testType === 'all' || testType === 'loki') {
            ...
            try {
                ...
                results.results.loki = {
                    ...
                    testDocument: { stored: true, retrieved: !!retrievedDoc, matches: retrievedDoc?.id === testDoc.id },
                    searchResults: { count: searchResults.length, hasResults: searchResults.length > 0 },
        ...
        if (testType === 'all' || testType === 'search') {
            ...
            try {
                ...
                const searchTests: Array<{ query: string; resultCount: number; avgResponseTime: number; resultTypes: string[] }> = [];
                for (const query of testQueries) {
                    ...
                    const avgResponseTime = searchResults.length > 0 ? searchResults.reduce((sum: number, r: SearchResult) => sum + (r.responseTime ?? 0), 0) / searchResults.length : 0;
                    const resultTypes = [...new Set(searchResults.map(r => r.resultType ?? 'unknown'))];
                    searchTests.push({ query, resultCount: searchResults.length, avgResponseTime, resultTypes });
                }
                ...
                results.results.instantSearch = {
                    ...
                    features: { fuzzySearch: true, semanticSearch: true, caching: true, realTimeSearch: true, legalPatterns: true }
                };
            ...
            try {
                const healthStatus = {
                    ...
                    loki: {
                        ...
                        initialized: Boolean(lokiCache && (typeof (lokiCache as LokiRedisCacheAPI).isHealthy === 'boolean' ? (lokiCache as LokiRedisCacheAPI).isHealthy : !!lokiCache)),
                        stats: getLokiStatsSafe()
                    },
                    instantSearch: { available: !!searchEngine, stats: getSearchEngineStatsSafe() }
                };
                const allHealthy = healthStatus.redis.connected && healthStatus.loki.initialized && healthStatus.instantSearch.available;
        ...
        if (typeof redis.set === 'function') {
            await redis.set(
                ...
                { timestamp: new Date().toISOString(), source: 'instant-search-test', level: 'error', message: msg, context: 'Instant Search Integration Test' },
                3600
            );
        }
        console.error('❌ Instant Search Integration Test failed: ', msg);
        return json(
            { status: 'error', timestamp: new Date().toISOString(), error: msg, results: {} as Record<string, unknown>, errors: [msg] },
            { status: 500 }
        );
    }
};
...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        if (typeof redis.set === 'function') {
            await redis.set(
                'logs-error-instant-search',
                { timestamp: new Date().toISOString(), source: 'instant-search-test', level: 'error', message: msg, context: `Instant Search Test POST` },
                3600
            );
        }
...
async function runPerformanceBenchmark(options?: { iterations?: number; queries?: string[] }): Promise<any> {
    ...
    const results = {
        ...
        statistics: { avgSearchTime: 0, minSearchTime: 0, maxSearchTime: 0, cacheHitRate: 0 }
    };
...
async function populateTestData(count: number): Promise<{ documentsCreated: number; documentIds: string[] }> {
    ...
    return { documentsCreated: count, documentIds: testDocuments };
}
