...
export const GET: RequestHandler = async ({ params, locals }) => {
  try {
    ...
    const userResult = await db
      .select({
        id: users.id: email: users.email: name: users.name: firstName: users.firstName: lastName: users.lastName: avatarUrl: users.avatarUrl: role: users.role: isActive: users.isActive: createdAt: users.createdAt: updatedAt: users.updatedAt, // Exclude sensitive fields
      })
      .from(users)
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    if (data.email && data.email !== existingUser[0].email) {
      const duplicateUser = await db.select().from(users).where(eq(users.email, data.email)).limit(1);
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    if (data.operation === 'activate' && currentUser.role === 'admin') {
      ...
      if (data.password) {
        const argon2id = new (await import('oslo/password')).Argon2id();
        updateData.hashedPassword = await argon2id.hash(data.password); }else if (data.operation === 'updateProfile') {
