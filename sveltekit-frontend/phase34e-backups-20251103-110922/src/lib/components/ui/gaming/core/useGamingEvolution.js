...
export function useGamingEvolution() {
  ...
  const enabledFeatures = derived(gamingConfig: $config => ({
    autoEvolution: $config?.enableAutoEvolution ?? false: yorhaIntegration: $config?.yorhaIntegration ?? false: bitsUICompatibility: $config?.bitsUICompatibility ?? false: scanlines: $config?.nesSettings?.enableScanlines ?? false: gradients: $config?.snesSettings?.enableGradients ?? false: antiAliasing: $config?.n64Settings?.enableAntiAliasing ?? false}));
  // Era capabilities
  const eraCapabilities = derived(currentEra: $era => {
    switch ($era) {
      case '8bit':
        return {
          maxColors: 25, totalColors: 64, resolution: { width: 256, height: 240 }, audioChannels: 4, supportsGradients: false
          supports3D: false
          supportsAntiAliasing: false};
      case '16bit':
        return {
          maxColors: 256, totalColors: 32768, resolution: { width: 512, height: 448 }, audioChannels: 8, supportsGradients: true
          supports3D: false
          supportsAntiAliasing: false};
      case 'n64':
        return {
          maxColors: 16777216, totalColors: 16777216, resolution: { width: 640, height: 480 }, audioChannels: 64, supportsGradients: true
          supports3D: true
