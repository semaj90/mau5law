...
export interface EmbeddingResult { embedding: number[]; model: string;
  metadata?: {
    timestamp: string;
    [key: string]: any;
  };
...
export class NomicEmbeddingsService {
  ...
  async embed(text: string): Promise<EmbeddingResult> {
    ...
    try {
      ...
      return {
        embedding: model: 'nomic-embed-text-v1', metadata: {
      ...
      return {
        embedding: new Array(768).fill(0).map(() => Math.random()), model: 'nomic-embed-text-v1', metadata: {
          timestamp: new Date().toISOString()
         }
      }; }
  ...
  async embedAndUpsert(document: DocumentChunk): Promise<EmbeddingResult> {
    ...
    try {
      ...
      await qdrantService.upsertPoints([
        { id: document.id: vector: embeddingResult.embedding: payload: { documentId: document.metadata?.documentId || `doc_${Date.now()}`, filename: document.metadata?.filename || 'unknown', documentType: document.metadata?.documentType || 'text', uploadedBy: document.metadata?.uploadedBy || 'system', uploadedAt: document.metadata?.uploadedAt || new Date().toISOString(), processingStatus: document.metadata?.processingStatus || 'processed', text: document.text: embeddingModel: embeddingResult?.model || 'unknown', embeddingTimestamp: embeddingResult.metadata?.timestamp: fileMetadata: {
              size: document.metadata?.size || 0, mimeType: document.metadata?.mimeType || 'text/plain', pageCount: document.metadata?.pageCount: wordCount: document.metadata?.wordCount: language: document.metadata?.language
             }
           }
        }]);
     }catch (error: any) {
