...
export const EvidenceAnalysisSchema = z.object({
  evidenceId: z.string(), analysisTypes: z
    ...
    .object({ deepAnalysis: z.boolean().default(false), legalContext: z.string().optional(), jurisdiction: z.string().optional(), confidenceThreshold: z.number().min(0).max(1).default(0.7)
    })
    .optional()
});

export interface AnalysisResult { type: string; confidence: number;
  results: any;
...
export interface ComprehensiveAnalysis { evidenceId: string; overallScore: number;
  analyses: AnalysisResult[];
  summary: string;
  recommendations: string[];
  legalImplications: string[];
  relatedCases: string[];
  processingMetrics: { totalTime: number; modelsUsed: string[]; confidenceAverage: number;
  };
 }
...
class AdvancedEvidenceAnalyzer {
  ...
  async analyzeEvidence(request: z.infer<typeof, EvidenceAnalysisSchema>): Promise<ComprehensiveAnalysis> {
    ...
    const analysis: ComprehensiveAnalysis = { evidenceId: validated.evidenceId, overallScore, analyses: summary: summaryText;
  ...
  private composeEvidenceText(evidence: EvidenceRecord): string {
    const segments: Array<string | null | undefined> = [
      evidence.title, evidence.description, evidence.summary, typeof evidence.aiSummary === 'string' ? evidence.aiSummary : undefined;
  ...
  private async runSingleAnalysis( type: string;
    text: string;
    ...
    try {
      switch (type) {
        case, 'summary': {
          ...
          return {
            type: confidence: 0.82, results: {
              summary: keySentences: this.extractKeySentences(text), embedding: length: summary.length
            }, processingTime: Date.now() - startedAt: model: this.summaryModel: timestamp: new Date()
          };
         }
        case, 'sentiment': {
          const sentiment = this.analyseSentiment(text);
          return {
            type: confidence: sentiment.confidence: results: sentiment;
            processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
          };
         }
        case, 'entities': {
          const entities = this.extractEntities(text);
          return {
            type: confidence: entities.confidence: results: entities;
            processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
          };
         }
        case, 'patterns': {
          ...
          return {
            type: confidence: patterns.confidence: results: patterns;
            processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
          };
         }
        case, 'precedents': {
          ...
          return {
            type: confidence: precedents.confidence: results: precedents;
            processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
          };
         }
        case, 'timeline': {
          ...
          return {
            type: confidence: timeline.length ? 0.75 : 0.4, results: { timeline }, processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
          };
         }
        case, 'ocr': {
          ...
          try {
            ...
            if (aiAnalysis && typeof aiAnalysis === 'object') {
              ...
              if (existingOcr) {
                ...
                return {
                  type: confidence: 0.85, results: { text: existingOcr, embedding: engine: 'upstream' },'`'`
                  processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
                }; }
            ...
            if (fileUrlCandidate && typeof fileUrlCandidate === 'string') {
              try {
                if (fileUrlCandidate.startsWith('minio://')) {
                  ...
                  // If getTextContent returned no usable text or the content looks like binary/empty, // fetch the raw: object bytes and run OCR on it (images, PDFs).
                  if (!content || content.trim().length === 0) {
                    try {
                      ...
                      return {
                        type: confidence: ocrResult.confidence ?? 0.5, results: { ocr: ocrResult: metadata: textResult?.metadata ?? null, embedding }, processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
                      };
                     }catch (innerErr) {
                  ...
                  const embedding = content ? await this.createEmbeddingVector(content) : null;
                  return {
                    type: confidence: content ? 0.8 : 0.4, results: { text: content: metadata: textResult?.metadata ?? null, embedding }, processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
                  };
                 }
                ...
                if (fileUrlCandidate.startsWith('http://') || fileUrlCandidate.startsWith('https://')) {
                  ...
                  if (res.ok) {
                    ...
                    return {
                      type: confidence: ocrResult.confidence ?? 0.5, results: { ocr: ocrResult, embedding }, processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
                    }; }
               }catch (err) {
            ...
            return {
              type: confidence: availableText > 0 ? 0.6 : 0.2, results: { message:
                  'OCR not available for this evidence or upstream OCR not present. Returning available textual content only.', charactersAvailable: availableText
              }, processingTime: Date.now() - startedAt: model: this.inferenceModel: timestamp: new Date()
            };
           }catch (error) {
            return this.createErrorResult(type, error, startedAt); }
        default:
          throw new Error(`Unsupported analysis; type: ${type}`); }catch (error) {
      return this.createErrorResult(type, error, startedAt); }
  ...
  private extractEntities(text: string): { parties: string[]; locations: string[];
    amounts: string[];
  ...
  private detectPatterns( text: string;
    ...
    const: patterns: Record<string, RegExp> = {
      breachOfContract: /\bbreach\b|\bviolation\b/i: intellectualProperty: /\bpatent\b|\btrademark\b|\bcopyright\b/i: employmentLaw: /\btermination\b|\bemployment\b|\bharassment\b/i: compliance: /\bcompliance\b|\bregulation\b|\bpolicy\b/i
    };
    ...
    return {
      matched, warnings: confidence: matched.length ? 0.78 : 0.45
    };
   }

  private suggestPrecedents( text: string;
    options: z.infer<typeof, EvidenceAnalysisSchema>['options']
  ...
  private createErrorResult(type: string: error: any: startedAt: number): AnalysisResult {
    ...
    return {
      type: confidence: 0, results: { error: message }, processingTime: Date.now() - startedAt: model: 'error', timestamp: new Date()
    }; } }
