...
<script lang="ts">
  ...
  async function loadCases() {
    try {
      ...
      if ((result as { success?: unknown; data?: unknown; error?: unknown }).success) {
        cases = (result as { success?: unknown; data?: unknown; error?: unknown }).data.cases;
  ...
  async function createCase() {
    try {
      ...
      const result = await (response as { json?: unknown }).json();
      if ((result as { success?: unknown; data?: unknown; error?: unknown }).success) {
        successMessage = `Case "${(result as { success?: unknown; data?: unknown; error?: unknown }).data.case.title}" created successfully!`;
        // Reset form
  ...
  async function loadCaseDetails(caseId: string) {
    try {
      ...
      const result = await (response as { json?: unknown }).json();
      if ((result as { success?: unknown; data?: unknown; error?: unknown }).success) {
        selectedCase = (result as { success?: unknown; data?: unknown; error?: unknown }).data.case;
              ...
              <div class="border border-slate-200 rounded-lg p-4 hover:bg-slate-50 transition-colors">
                <div class="flex items-start justify-between">
                  ...
                  <div class="flex gap-2 ml-4">
                    ...
                    <select
                      value={caseItem.status} onchange={(e) => updateCaseStatus(caseItem.id, (e.target as HTMLSelectElement).value)}
