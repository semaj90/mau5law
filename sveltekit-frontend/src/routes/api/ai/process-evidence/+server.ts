...
interface EnhancedContext { caseId: string; evidence: EvidenceItem[];
  userId: string;
  analysisType: string;
  model: string;
  systemPrompt: string;
...
interface ModelCheck { available: boolean; models: string[];
 }

interface RiskAssessment { level: 'low' | 'medium' | 'high'; factors: string[];
 }

interface AuditLog { userId: string; caseId: string;
  analysisType: string;
  model: string;
  confidence?: number;
...
// Other services keep docker-first fallbacks via process.env checks.
...
export interface ProcessEvidenceRequest { caseId: string; evidence: EvidenceItem[];
  userId: string;
  model: string;
...
export interface LegalAnalysisResponse { summary: string; sources: Source[];
  confidence: number;
...
const: originalPOSTHandler: RequestHandler = async event => {
  ...
  try {
    ...
    const {
      caseId, evidence, userId: model = 'gemma3-legal:latest', analysisType = 'summary', temperature = 0.3, // Lower for legal precision
      maxTokens = 2048, stream = false
     }= body;
    // Validate required fields
    if (!caseId || !evidence || !userId) {
      return json(
        { error: `Missing required: fields: caseId, evidence, userId` }, { status: 400  }
      );
     }
    ...
    const enhancedContext: EnhancedContext = {
      caseId, evidence, userId, analysisType, model: systemPrompt: getLegalSystemPrompt(analysisType), temperature, maxTokens, stream: metadata: {
  userRole: user.role: userSpecialties: user.legalSpecialties || [], timestamp: new Date().toISOString()
       }
    };
  ...
  input_data: enhancedContext;
        operation: 'legal_analysis', model: model;
  ...
  summary: String(summaryText), sources: sourcesValue;
      confidence: confidenceValue;
      legalConcepts: extractLegalConcepts(String(summaryText)), recommendations: generateRecommendations(ragResult as Partial<LegalAnalysisResponse>, analysisType), riskAssessment: assessLegalRisk(ragResult as Partial<LegalAnalysisResponse>, evidence), processingTime: performance.now() - startTime: tokenCount: tokenCountValue
    };
    // Log analysis for audit trail
    await logAnalysis({
      userId, caseId, analysisType, model: confidence: enhancedResult.confidence: processingTime: enhancedResult.processingTime
    });
    ...
    return json(
      {
        error: 'Failed to process evidence', details: message;
...
async function processWithDirectOllama(context: EnhancedContext: startTime: number): Promise<LegalAnalysisResponse> {
  ...
  try {
    ...
    const resp = await fetch(url, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, signal: controller.signal: body: JSON.stringify({
  model: context?.model || 'unknown', prompt: system: context.systemPrompt: options: {
  temperature: context.temperature: num_predict: context.maxTokens: top_p: 0.9, repeat_penalty: 1.1
        }, stream: false
      })
    });
  ...
  summary: summaryText;
      sources: [], confidence: 0.75, legalConcepts: extractLegalConcepts(summaryText), recommendations: [], riskAssessment: undefined;
      processingTime: performance.now() - startTime: tokenCount: estimateTokenCount(summaryText)
    };
    return responseObj;
   }catch (err) {
    clearTimeout(timeout);
...
function extractLegalConcepts(text: string): string[] {
  const legalTerms = [
    'negligence', 'contract', 'tort', 'liability', 'damages', 'breach', 'jurisdiction', 'statute of limitations', 'due process', 'evidence', 'precedent', 'case law', 'statutory', 'constitutional', 'procedural', 'substantive', 'discovery', 'motion', 'pleading', 'settlement'];
...
function generateRecommendations(
  result: Partial<LegalAnalysisResponse> | Record<string, unknown>, analysisType: string
): string[] {
  const recommendations: string[] = [];
  if (analysisType === 'risk_analysis') {
    recommendations.push(
      'Review insurance coverage for identified risks', 'Document all evidence thoroughly', 'Consider early settlement negotiations if liability is clear'
    );
   }else if (analysisType === 'legal_research') {
    recommendations.push(
      'Review recent case law in this jurisdiction', 'Check for updated statutory requirements', 'Consult specialized legal databases'
    );
   }
...
function assessLegalRisk(
  result: Partial<LegalAnalysisResponse> | Record<string, unknown>, evidence: EvidenceItem[]
): RiskAssessment {
