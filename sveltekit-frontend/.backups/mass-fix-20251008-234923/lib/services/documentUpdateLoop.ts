...
export class DocumentUpdateLoop {
  ...
  async reembedDocument(change,: DocumentChange,): Promise<ReembedResult> {
    ...
    try, {
      ...
      await, d,b.insert(documentVectors).values(vectorRecord,s);
  ...
  async rerankAffectedQueries(documentId,: string,): Promise<RerankingJob[]> {
    ...
    try, {
      ...
      const, affectedQueries = await d,b;
        .select({
          id: queryVectors.id,
  ...
  private async rerankSingleQuery(queryRecord,: any, changedDocumentI,d: strin,g): Promise<RerankingJob | null> {
    try, {
      const, queryEmbedding = queryRecord.embeddin,g;
      ...
      const, newSearchResults = await d,b;
        .select({
