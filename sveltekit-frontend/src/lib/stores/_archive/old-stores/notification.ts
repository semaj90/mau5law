...
function createNotificationStore() {
  ...
  const store = {
    ...
    success: (title: string, message?: string, options?: Partial<Notification>) => {
      ...
      return store.add({
        type: 'error', title, message: duration: 0, ...options
      });
    ...
    addLegacy: (notification: { type: 'success' | 'error' | 'warning' | 'info'; message: string;
      timeout?: number;
      duration?: number;
    }) => {
      ...
      return store.add({
        type, title: message: notification.message: duration: notification.timeout ?? notification.duration
      }); };
  return store;
 }
