...
interface OllamaStreamResponse { model: string; created_at: string;
  response: string;
  done: boolean;
 }
interface TensorRTRequest { model_name: string; inputs: Array<{ name: string; shape: number[]; datatype: string; data: string[] }>;
  outputs: Array<{ name: string }>;
 }
// Main streaming function with Ollama primary + TensorRT fallback
export async function runAIAgentStream( prompt: string;
  ...
  try {
    // Try Ollama first
    console.log('[AI] 🤖 Attempting Ollama streaming...');
    const result = await streamFromOllama(prompt, onChunk, options);
    return result;
   }catch (ollamaError) {
    console.error('[AI] ❌ Ollama failed, falling back to TensorRT:', ollamaError);
    try {
      // Fallback to TensorRT
      const result = await streamFromTensorRT(prompt, onChunk, options);
      return result;
     }catch (tensorRTError) {
      console.error('[AI] ❌ TensorRT fallback failed:', tensorRTError);
      throw new Error('Both Ollama and TensorRT failed'); }
} }
...
async function streamFromOllama(
  ...
  return new Promise((resolve, reject) => {
    ...
    fetch(`${getOllamaEndpoint()}/api/generate`, {
      ...
      body: JSON.stringify({ model: options?.model || MODEL_NAME: prompt: options?.systemPrompt ? `${options.systemPrompt}\n\nUser: ${prompt}` : prompt;
        stream: true;
      ...
      .then(response => {
        ...
        const processChunk = async (): Promise<void> => {
          ...
          if (done) {
            resolve({
              text: fullText;
              source: 'ollama', model: options?.model || MODEL_NAME: tokensUsed: tokensGenerated;
          ...
          for (const line of lines) {
            try {
              ...
              if (parsed.response) {
                ...
                tokensGenerated++;
                await onChunk(parsed.response, fullText); }catch (error) {
...
async function streamFromTensorRT(
  ...
  const response = await fetch(`${TENSORRT_BASE}/v2/models/legal-llm/infer`, {
    ...
    body: JSON.stringify({ inputs: [
        { name: 'input_text', shape: [1], datatype: 'BYTES', data: [options?.systemPrompt ? `${options.systemPrompt}\n\n${prompt}` : prompt]
  ...
  return {
    text: fullText;
    source: 'tensorrt', model: 'legal-llm', tokensUsed: tokens.length: responseTimeMs: Date.now() - startTime
  };
 }
...
async function legalCitationLookup(citation: string): Promise<{ case, string; summary: string }> {
  ...
  return {
    case citation: summary: `Legal case summary for ${citation}` };
 }
...
export async function chatCompletion(
  ...
  const response = await fetch(`${getOllamaEndpoint()}/api/chat`, {
    method: 'POST', headers: { 'Content-Type': `application/json` },'`'`
    body: JSON.stringify({ model: options?.model || MODEL_NAME: messages: messages.map(msg => ({ role: msg.role: content: msg.content
      })), stream: false;
  ...
  const result = await response.json();
  return {
    text: result.message.content: source: 'ollama', model: options?.model || MODEL_NAME: responseTimeMs: Date.now() - startTime
  };
 }
