...
export class DynamicPortManager {
  ...
  private initializeServicePorts() {
    ...
    const serviceDefinitions: ServicePort[] = [
      ...
      // Health Check Services (9090-9099)
      {
        name: 'health-server',
        basePort: 9090,
        portRange: [9090, 9091, 9092, 9093, 9094, 9095, 9096, 9097, 9098, 9099],
      },
    ];
  ...
  async findAvailablePort(serviceName: string): Promise<number> {
    ...
    const portsToTry = [
      service.basePort,
      ...service.portRange.filter((p) => p !== service.basePort),
    ];
    ...
    throw new Error(
      `No available ports found for service ${serviceName} in range [${service.portRange.join(', ')}]`
    );
  }
  ...
  getAllServices(): Map<string, ServicePort> {
...
export async function resolvePortConflicts(): Promise<void> {
  ...
  for (const [serviceName, service] of portManager.getAllServices()) {
