  ...
  try {
    ...
    try {
      const bridgeResponse = await fetch(`${TENSORRT_BRIDGE_URL}/api/generate`, {
        ...
        body: JSON.stringify({
          messages, model, temperature: max_tokens: requestData.max_tokens || 1024, stream: false
        })
      });
      ...
      if (!bridgeResponse.ok) {
        ...
        return json(
          {
            error: 'TensorRT bridge failed', detail: errorText;
      ...
      return json({
        id: `tensorrt_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`, object: 'chat.completion', created: Math.floor(Date.now() / 1000), model: choices: [
          { index: 0, message: {
  role: 'assistant', content: bridgeData.response || bridgeData.output || 'No response generated` },'`
            finish_reason: `stop`  }`'`
        ], usage: {
  total_tokens: Math.ceil((fullPrompt + (bridgeData.output || '')).length / 4), prompt_tokens: Math.ceil(fullPrompt.length / 4), completion_tokens: Math.ceil((bridgeData.output || '').length / 4)
        }, tensorrt: {
  bridge_used: true;
      ...
      return json(
        {
          error: 'TensorRT bridge connection failed', detail: suggestion: `Ensure TensorRT bridge is reachable at ${TENSORRT_BRIDGE_URL}`, fallback_available: false
        }, { status: 503  }
      ); }catch (error: any) {
    ...
    return json(
      {
        error: 'Failed to generate response', detail: timestamp: new Date().toISOString()
      }, { status: 500  }
    ); }
