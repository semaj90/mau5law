...
export class UserRecommendationService {
	...
	userId: string; caseId?: string; sessionName?: string }): Promise<string> {
		try {
	 ...
	 */
	async generateRecommendations(userId: string, limit = 5): Promise<RecommendationResult[]> {
		try {
	...
	private async getUserSessionStats(userId: string) {
		const sessions = await db
			.select({
				...
				messageCount: sql<number>`(SELECT COUNT(*) FROM ${ragMessages} WHERE session_id = ${ragSessions.id})`
			})
			.from(ragSessions)
		...
		const sessionLengths = sessions.map((s) =>
			s.endedAt && s.startedAt ? (new Date(s.endedAt).getTime() - new Date(s.startedAt).getTime()) / 60000 : 5
		);
		const avgSessionLength = sessionLengths.length > 0 ? sessionLengths.reduce((a, b) => a + b, 0) / sessionLengths.length : 0;
		const avgQueriesPerSession = sessions.length > 0 ? sessions.reduce((sum, s) => sum + s.messageCount, 0) / sessions.length : 0;

		return {
			frequency: sessions.length > 20 ? 'high' : sessions.length > 5 ? 'medium' : 'low',
			activeHours: this.extractActiveHours(sessions),
			avgSessionLength,
	...
	private extractActiveHours(sessions: {
	startedAt: Date | null }[]) {
