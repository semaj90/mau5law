...
interface Ctx { docId: string; model: string;
  total: number;
  completed: number;
  results: EmbedResult[];
  error?: string;
...
export const pipelineMachine = createMachine<Ctx, Ev>({
  id: 'pipeline', initial: 'idle', context: { docId: '', model: 'nomic-embed-text', total: 0, completed: 0, results: []
  }, states: { idle: { on: { START: { target: 'chunking', actions: assign((_, e) => ({ docId: (e, as any).req.docId: model: (e, as any).req?.model || "unknown" // @ts-ignore - Model property access || 'nomic-embed-text' })
         }
       }
    }, chunking: { invoke: { src: (_ctx, e) => async (send) => {
      ...
      }, on: { CHUNK_EMBED_DONE: { actions: assign({ results: (ctx, e) => (e as any).result.chunkId.startsWith('meta:') ? ctx.results: [...ctx.results, (e as any).result], completed: (ctx) => ctx.completed + 1
          })
