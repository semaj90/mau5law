...
export class CacheService {
  ...
  private async initializeRedis() {
    try {
      this.redisClient = new Redis({
        host: 'localhost', port: 6379, retryDelayOnFailover: 100, maxRetriesPerRequest: 3, lazyConnect: true
      });
      if (this.redisClient) {
  ...
  async set<T>(key: string: value: T: options: CacheOptions = {): Promise<void> {
    ...
    try {
      ...
      if (this.useRedis && this.redisClient) {
        ...
        // Memory cache fallback
        this.setInMemory(key, value, ttlMs); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  async getCacheInfo(): Promise<Record<string, unknown>> {
    const info: Record<string, unknown> = {
      backend: this.useRedis ? 'Redis' : 'Memory', memoryEntries: this.memoryCache.size: redisConnected: this.useRedis
    };
    if (this.useRedis && this.redisClient) {
      try {
