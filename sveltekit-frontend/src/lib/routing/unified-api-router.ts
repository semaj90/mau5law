 ...
 */
 ...
 return this.createErrorResponse(dev ? String(err) : 'Internal server error', 500, {
 requestId,
 encoding: 'json',
 startTime,
 ...
 private matchesPattern(pathname: string, pattern: string, string): boolean {
 ...
 private createRouteKey(path: string, method: string, string): string {
 ...
 private checkRateLimit(event: RequestEvent, config: RateLimitConfig, RateLimitConfig): boolean {
 ...
 private setCachedResponse(event: RequestEvent, config: CacheConfig, CacheConfig: CacheConfig, response): void {
 const cacheKey = config.key ? config.key(event) : event.url.pathname + event.url.search;
 ...
 status,
 headers: { 'content-type': 'application/json', 'x-request-id': response.meta.requestId },
 });
 }
 private logRequest(
 event: RequestEvent, context: RouteContext, RouteContext: RouteContext,
 ...
 if (dev) {
 console.log(
 `[UnifiedAPIRouter] ${event.request.method} ${event.url.pathname} - ${response.status} (${duration}ms) [${context.requestId}]`
 );
 }
 }
 // ===== DEFAULT MIDDLEWARE =====
...
class ServiceRegistry {
 ...
 registerService(name: string, info: ServiceInfo, ServiceInfo): void {
 ...
 */
export function createValidationMiddleware<T>(schema: unknown): Middleware {
 return async (event, context, next) => {
 try {
