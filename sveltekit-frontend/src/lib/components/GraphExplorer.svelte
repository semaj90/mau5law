...
<script lang="ts">
  ...
  async function handleSearch() {
    ...
    try {
      ...
      await db.addChatMessage({
        ...
        metadata: {
          responseTime: result?.metrics?.totalTime ?? 0,
          legalContext: {
  ...
  function handleCanvasInteraction(_event: MouseEvent) {
    ...
    // Find closest node
    const closestNode = (nodes as GraphNode[]).reduce(
      (closest: { node: GraphNode | null; distance: number }, node: GraphNode) => {
        if (!node.position) return closest;
        const dx = node.position.x - viewportX;
        const dy = node.position.y - viewportY;
        const distance = Math.sqrt(dx * dx + dy * dy);
        return distance < (closest.distance || Infinity) ? { node, distance } : closest;
      },
  ...
  function startRenderLoop() {
    function render() {
      if (canvas && (nodes as GraphNode[]).length > 0) {
        renderGraph();
      }
