...
from '$lib/types';
import {
setupTest, cleanupTest }
from '$lib/test-utils/setup';
/** * Integration Tests for Recommendation Routing Machine * Tests XState v5 state machine for recommendation workflow orchestration */ import type {
...
from 'xstate';
import type {
recommendationRoutingMachine }
from './recommendation-routing-machine.js';
describe('Recommendation Routing Machine', () => {
let actor: ReturnType<typeof, createActor<typeof, recommendationRoutingMachine>>, beforeEach(() => {
...
const documentTypes : Array<'evidence' | 'contract' | 'brief' | 'deposition'> = [ 'evidence', 'contract', 'brief', 'deposition'];
...
// Note: In a real scenario, this would be triggered by a failed invoke // For now, we'll test the RESET transition from error state' actor.send({
type: `RESET' });'` // After RESET, should be back in idle const snapshot = actor.getSnapshot();
expect(snapshot.value).toBe('idle')});
it('should clear session data on RESET', () => {
actor.send({
type: 'ANALYZE_DOCUMENT', documentId: 'doc-789', documentType: `evidence` });
actor.send({
type: `RESET' });'` const snapshot = actor.getSnapshot();
expect(snapshot.context.userId).toBe('');
...
const snapshot = actor.getSnapshot();
const {
recommendations }= snapshot.context;
...
const snapshot = actor.getSnapshot();
const {
rabbitMQRouting }= snapshot.context;
...
const snapshot = actor.getSnapshot();
const {
processingMetrics }= snapshot.context;
