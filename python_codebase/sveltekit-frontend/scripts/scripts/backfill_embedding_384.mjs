#!/usr/bin/env node
/*
 Backfill script for embedding_768
 Usage: set DB env vars or rely on defaults, then run:
   node scripts/backfill_embedding_384.mjs

 The script queries `public.vector_embeddings` for rows with NULL embedding_768,
 requests embeddings from local Ollama (or other service), and updates the DB in batches.
*/
...
// Default model that produces 768-d embeddings. Override with EMBEDDING_MODEL env if you have a different model.
