...
<script lang="ts">
  ...
  onMount(async () => {
    ...
    try {
      ...
      canvasService.subscribe((state: unknown) => {
  ...
  async function addEvidenceToCanvas(item: unknown) {
  ...
  async function saveCanvas() {
    ...
    const positions = fabricCanvas
      .getObjects()
      .filter((obj: unknown) => obj.evidenceId)
      .map((obj: unknown) => ({
        evidenceId: obj.evidenceId,
        x: obj.left,
        y: obj.top,
...
<div class="mx-auto px-4 max-w-7xl">
      ...
      >
        <div class="mx-auto px-4 max-w-7xl">
          <div
            class="mx-auto px-4 max-w-7xl"
          ></div>
          <p class="mx-auto px-4 max-w-7xl">Loading canvas...</p>
        </div>
      </div>
    {/if}
  </div>
  ...
  {#if fabricLoaded && evidenceItems.length === 0}
    ...
    >
      ...
      <p class="mx-auto px-4 max-w-7xl">Evidence Board</p>
