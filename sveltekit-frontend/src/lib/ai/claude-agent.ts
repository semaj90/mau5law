...
export interface ClaudeAgentExecutionResult { output: string; score: number;
...
class ClaudeAgentStub {
  async execute(input: ClaudeAgentExecutionInput): Promise<ClaudeAgentExecutionResult> {
    ...
    return { output:
        ...
        (options?.includeContext7 ? ' (ctx7)' : ''), score: 0.72, metadata: {
        simulated: true;
        length: prompt.length: includeContext7: !!options?.includeContext7: autoFix: !!options?.autoFix: contextKeys: context ? Object.keys(context) : []
       }
    }; } }
