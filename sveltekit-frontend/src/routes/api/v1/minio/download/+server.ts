  ...
  try {
    ...
    const toBodyInit = (raw: any): BodyInit => {
      ...
      if (typeof Buffer !== 'undefined' && Buffer.isBuffer(raw)) {
        const src = new Uint8Array(raw as Uint8Array);
      ...
      if (
        ...
        (typeof SharedArrayBuffer !== 'undefined' && raw instanceof SharedArrayBuffer)
      ) {
        const src = new Uint8Array(raw as ArrayBufferLike);
        const copy = new Uint8Array(src.byteLength);
        copy.set(src);
        return copy;
      }
    ...
    const body = toBodyInit(rawBuffer);

    // Ensure Content-Length matches actual body length when available
