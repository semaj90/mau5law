...
export class AccessibilityUtils {
  ...
  static setProgressBarAria(element: HTMLElement, percentage: number, stage: string): void {
    ...
    element.setAttribute('aria-label', `Document processing progress: ${percentage}% complete, current stage: ${stage}`);
  }
  ...
  static setStatusIndicatorAria(element: HTMLElement, status: string, pageNumber: number): void {
    ...
    element.setAttribute('aria-label', `Page ${pageNumber} status: ${statusLabels[status] || status}`);
  }
