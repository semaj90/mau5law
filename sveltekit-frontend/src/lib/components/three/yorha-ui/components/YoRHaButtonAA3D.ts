...
export interface YoRHaButtonAA3DOptions extends Omit<YoRHaAAStyle, 'variant'> {
    ...
    depth?: number;
    borderRadius?: number;
    shaderEnhancements?: Record<string, unknown>;
}

export class YoRHaButtonAA3D extends YoRHaAntiAliased3D {
    textMesh?: THREE.Mesh;
    iconMesh?: THREE.Mesh;
    loadingSpinner?: THREE.Group;, options: YoRHaButtonAA3DOptions;
    textCanvas: HTMLCanvasElement | null = null;
    textTexture?: THREE.CanvasTexture;, lodLevel: number = 1;
    distanceToCamera: number = 0;
    enhancedMaterial?: THREE.Material;
    ...
    constructor(options: YoRHaButtonAA3DOptions = {}) {
        ...
        super({
            ...
            hover: {, transform: {
                    ...
                    ...(options.hover?.transform as any)
                },
                shadow: {, enabled: true,
                    offsetY: -0.08,
                    intensity: 0.6,
    ...
    addQuantumEffects(): void {
        ...
        this.addCustomAnimation('quantum', (deltaTime: number) => {
    ...
    enableSubpixelRendering(): void {
        ...
        const subpixelMaterial = new THREE.ShaderMaterial({
            uniforms: {, baseColor: { value: new THREE.Color(this.style.backgroundColor) },
            ...
            fragmentShader: `
                ...
                vec3 subpixelAA(vec2 uv, vec3 color) {
                    ...
                    float r = color.r;
                    float g = color.g; // Simplified for this implementation
                    float b = color.b;
                    return vec3(r, g, b);
                }
    ...
    initializeDynamicLOD(): void {
        this.addCustomAnimation('dynamicLOD', (deltaTime: number) => {
    ...
    getHighQualityIconGeometry(icon: string): THREE.BufferGeometry {
        ...
        switch (icon) {
            ...
            case 'quantum':
                return new THREE.RingGeometry(size * 0.3, size, segments);
            case 'consciousness':
    ...
    static getVariantStyle(variant: string, size: string): Partial<YoRHaAAStyle> {
        ...
        const variantStyles = {
            primary: {, backgroundColor: YORHA_COLORS.primary.beige,
    ...
    protected onClick(): void {
        ...
        this.addCustomAnimation('enhancedClickPulse', (deltaTime) => {
            clickTime += deltaTime;
            const progress = Math.min(clickTime / 0.3, 1);
    ...
    createRippleEffect(): void {
        ...
        let rippleTime = 0;
        this.addCustomAnimation('ripple', (deltaTime: number) => {
