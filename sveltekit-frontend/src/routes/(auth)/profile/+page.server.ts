...
export const load: PageServerLoad = async ({ locals }) => { // Corrected function signature
    ...
    const [totalCasesRow, openCasesRow, closedCasesRow, evidenceRow, poiRow] = await Promise.all([
        ...
        db.select({ value: sql<number>`count(*)::int` }).from(cases).where(not(inArray(cases.status, ['closed', 'archived']))), // Corrected Drizzle where with not(inArray)
        db.select({ value: sql<number>`count(*)::int` }).from(cases).where(eq(cases.status, 'closed')), // Correct Drizzle where with eq
