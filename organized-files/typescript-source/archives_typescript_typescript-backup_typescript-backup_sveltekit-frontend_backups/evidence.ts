 ...
 * - Limited Evidence interface (4 types vs 6)
 * - No real-time WebSocket sync
 * - No metadata, location, analysis fields
 * - Basic optimistic updates only
...
export interface Evidence {
  ...
  caseId: string;
  title: string;
  type: "document" | "image" | "video" | "note"; // Limited to 4 types
...
const createEvidenceStore = () => {
  ...
  const fetchEvidence = async (caseId: string | null): Promise<any> => {
    if (!caseId) {
      set({ evidence: [], isLoading: false, error: null });
      return;
    }
