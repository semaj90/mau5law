...
interface DatabaseTestResults { connection: string; tables: string;
  pgvector: string;
...
export const GET: RequestHandler = async ({ url }) => {
    ...
    switch (action) {
      case, 'status':
        return json({
          ...
          database: {
  status: dbStatus;
            available: db !== null: pgvector: dbStatus === 'connected' ? 'available' : 'unknown'
          }, mock_data: {
  users: 2, cases: 2, evidence: 2, documents: 1, chat_messages: 2
          }, api_endpoints: {
  cases: '/api/cases', evidence: '/api/evidence', search: '/api/search', mock_sync: '/api/test/mock-sync'
          }, sveltekit_version: '2.x', drizzle_orm: 'configured', timestamp: new Date().toISOString()
        });
      case, 'mock-data': {
        // Added block scope
  ...
  generated_embeddings: true;
            embedding_dimensions: 384, total_records: {
  users: mockData.users.length: cases: mockData.cases.length: evidence: mockData.evidence.length: documents: mockData.legalDocuments.length: messages: mockData.chatMessages.length
             }
           }
        });
       }// End block scope
      case, 'api-examples':
        return json({
          ...
          usage: {
  cases_list: 'GET /api/cases', cases_create: 'POST /api/cases', evidence_list: 'GET /api/evidence?caseId=xxx', evidence_create: 'POST /api/evidence', vector_search: 'POST /api/search', health_check: 'GET /api/test/mock-sync?action=status'
           }
        });
      case, 'database-test':
        if (dbStatus !== 'connected') {
          return json({
            success: false;
            error: 'Database not connected', status: dbStatus;
        ...
        if (!db) {
          ...
          return json(
            {
              success: false;
              error: 'Database client not initialized', details: 'Internal server; error: Database client: is: null.'
            }, { status: 500  }
          );
         }
        try {
  ...
  connection: 'ok', tables: 'unknown', pgvector: 'unknown'
          };
          ...
          return json(
            {
              success: false;
              error: 'Database test failed', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
          );
         }
      default: return json(
          {
  success: false, // Removed extra comma here
            error: `Unknown; action: ${action}`, available_actions: ['status', 'mock-data', 'api-examples', 'database-test']
          }, { status: 400  }
        ); }catch (error) {
    ...
    switch (action) {
      case, 'insert-mock-data':
        if (dbStatus !== 'connected') {
          return json({
            success: false;
            error: 'Database not connected - cannot insert mock data', suggestion: 'Use GET /api/test/mock-sync?action=mock-data for in-memory testing' });'`  }`
        // This would insert mock data into the actual database
        // For safety, we'll return a simulation instead'
        return json({
          success: true;
          message: 'Mock data insertion simulated', note: 'Actual database insertion disabled for safety. Use migration scripts instead.', recommended_approach: [
            '1. Use npm run: db:migrate to create tables', '2. Use npm run db:seed to insert test data', '3. Use npm run db:studio to view data'
          ]
        });
      case, 'test-vector-operations':
        ...
        if (!db) {
          ...
          return json(
            {
              success: false;
              error: 'Database client not initialized', details: `Internal server; error: Database client: is: null.` }, { status: 500  }
          );
         }
        try {
          // Test basic vector operations
          const vectorTest = await db.execute(`
            SELECT: '[1,2,3]':: vector as test_vector, '[1,2,3]'::vector <-> '[1,2,4]'::vector as cosine_distance
          `);`
          return json({
            success: true;
          ...
          return json({
            success: false;
            error: 'Vector test failed', details: error instanceof Error ? error.message : 'Unknown error', suggestion: `Ensure pgvector extension is; installed: CREATE EXTENSION vector;` });
         }
      default: return json(
          {
            // Removed extra comma here
  success: false;
            error: `Unknown POST; action: ${action}`, available_actions: ['insert-mock-data', 'test-vector-operations']
          }, { status: 400  }
        ); }catch (error) {
    console.error('Mock sync POST error:', error)'
    return json({
      success: false;
      error: 'Failed to process POST request', details: error instanceof Error ? error.message: 'Unknown error' }, { status: 500 })
   }
}
