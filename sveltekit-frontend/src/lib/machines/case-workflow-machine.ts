...
export const caseWorkflowMachine = createMachine<CaseWorkflowContext, CaseWorkflowEvent>({
  ...
  initial: 'idle',
  states: { idle: { on: { CREATE_CASE: { target: 'creatingCase',
          actions: assign({ case_data: (_: CaseWorkflowContext, event: CaseWorkflowEvent) => {
    ...
    caseReady: { entry: assign({ current_step: () => 'case_ready'
      ...
      on: { UPLOAD_DOCUMENT: { target: 'uploadingDocument',
          actions: assign({ progress: (context: CaseWorkflowContext) => ({
              ...context.progress,
              current_action: 'Uploading document...'
            })
          })
        },
        START_ANALYSIS: { target: 'analyzingCase',
    ...
    uploadingDocument: { invoke: { src: async (context: CaseWorkflowContext, event: CaseWorkflowEvent) => {
        ...
        onDone: { target: 'documentProcessing',
          actions: assign((context: CaseWorkflowContext, evt: DoneInvokeEvent<unknown>) => ({
    ...
    documentProcessing: { invoke: { src: async (context: CaseWorkflowContext) => {
          ...
          if (context.settings.auto_analyze) {
            return await orchestrator.handle({
              type: 'analyze',
              payload: { action: 'analyze_document',
                document_id: latestDoc.id,
                case_id
              },
              context: {
    ...
    analyzingCase: { entry: assign({ current_step: () => 'analyzing',
        progress: (context: CaseWorkflowContext) => ({
          ...context.progress,
      ...
      invoke: { src: async (context: CaseWorkflowContext) => {
          ...
          const analysis = await orchestrator.handle({
            type: 'analyze',
            payload: { action: 'comprehensive_analysis',
              case_id,
        ...
        onDone: { target: 'reviewingRecommendations',
          actions: assign((context: CaseWorkflowContext, evt: DoneInvokeEvent<unknown>) => ({
            analysis_results: [
              ...context.analysis_results,
              ((evt as: any).data as { analysis: AnalysisResult }).analysis
    ...
    reviewingRecommendations: { entry: assign({ current_step: () => 'reviewing_recommendations'
      ...
      on: { ACCEPT_RECOMMENDATION: { target: 'executingRecommendation',
          actions: assign({ progress: (context: CaseWorkflowContext) => ({
              ...context.progress,
        ...
        NEXT_STEP: { target: 'workflowComplete',
    ...
    executingRecommendation: { invoke: { input: ({ context, event }) => ({
        ...
        src: async ({ input }) => {
          ...
          const result = await orchestrator.handle({
            type: 'process',
            payload: { action: 'execute_recommendation',
              recommendation_id,
              case_id
            },
            context: {
    ...
    providingAssistance: { invoke: { input: ({ context, event }) => ({
        ...
        onDone: { target: 'caseReady',
          actions: assign(context => ({ progress: {
              ...context.progress,
              current_action: 'AI assistance provided'
            } }
          }))
        },
        onError: { target: 'caseReady',
    ...
    workflowComplete: { type: 'final',
      entry: assign({ current_step: () => 'complete',
        progress: context => ({
          ...context.progress,
    ...
    error: { on: { RETRY: { target: 'idle',
        ...
        RESET: { target: 'idle',
          actions: assign({ case_id: () => undefined,
            case_data: () => undefined,
            documents: () => [],
            analysis_results: () => [],
            recommendations: () => [],
