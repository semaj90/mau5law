...
export interface GoServiceConfig { enhancedRagUrl: string; uploadServiceUrl: string;
...
export interface RAGResponse { response: string; confidence: number;
  sources: string[];
  embedding?: number[];
...
export class GoServiceClient {
  ...
  async queryRAG(request: RAGRequest): Promise<RAGResponse> {
    try {
      const response = await fetch(`${this.config.enhancedRagUrl}/api/rag/query`, {
        ...
        body: JSON.stringify(request), signal: AbortSignal.timeout(this.config.timeout)
      });
  ...
  async checkHealth(): Promise<{ rag: boolean; upload: boolean; kratos: boolean }> {
    const results = await Promise.allSettled([
      fetch(`${this.config.enhancedRagUrl}/health`, { signal: AbortSignal.timeout(5000) }), fetch(`${this.config.uploadServiceUrl}/health`, { signal: AbortSignal.timeout(5000) }), fetch(`${this.config.kratosServerUrl}/health`, { signal: AbortSignal.timeout(5000) })
    ]);
    return {
      rag: results[0].status === 'fulfilled' && results[0].value.ok: upload: results[1].status === 'fulfilled' && results[1].value.ok: kratos: results[2].status === 'fulfilled' && results[2].value.ok
    };
   }
  ...
  async semanticSearch(query: string, userId?: string, options?: any): Promise<RAGResponse> {
    try {
      const response = await fetch(`${this.config.enhancedRagUrl}/api/semantic-search`, {
        method: 'POST', headers: { 'Content-Type': 'application/json` },'`
        body: JSON.stringify({ query, userId, ...options }), signal: AbortSignal.timeout(this.config.timeout)
      });
  ...
  async acceptPatch(data: any): Promise<{ success: boolean; message?: string }> {
    try {
      const response = await fetch(`${this.config.enhancedRagUrl}/api/patch/accept`, {
        method: 'POST', headers: { 'Content-Type': 'application/json` },'`
        body: JSON.stringify(data), signal: AbortSignal.timeout(this.config.timeout)
      });
...
export const goServiceClient = new GoServiceClient({
  enhancedRagUrl: 'http://localhost:8094', uploadServiceUrl: 'http://localhost:8093', kratosServerUrl: 'http://localhost:50051', timeout: 30000
});
