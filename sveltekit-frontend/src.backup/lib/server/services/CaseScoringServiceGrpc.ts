...
type GrpcResponse = {
  ...
  metadata?: { model_name?: string; model_version?: string; compression_stats?: { compression_ratio?: number }; performance?: { throughput_cases_per_second?: number } };
};
...
export class CaseScoringServiceGrpc extends EventEmitter {
  ...
  private initializeGrpcClient() {
    try {
      ...
      const CaseScoringService = proto.legal_ai?.case_scoring?.CaseScoringService as
        | ({ new (...args: unknown[]): GrpcClientType } | undefined);
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const r of requests) {
        ...
        const req = {
          ...
          criteria: this.convertCriteriaToProto(
            r.scoring_criteria ??
              (r as unknown as { criteria?: Partial<ScoringCriteria> }).criteria
          ),
          parameters: {
            model: this.SCORING_MODEL,
  ...
  private async generateAIAnalysis(request: CaseScoringRequest): Promise<string> {
    ...
    const defendants = Array.isArray(caseData['defendants']) ? (caseData['defendants'] as string[]).join(', ') : 'N/A';
    ...
    const providedCriteria = (request as unknown as { scoring_criteria?: Partial<ScoringCriteria>; criteria?: Partial<ScoringCriteria> }).scoring_criteria ??
  ...
  private async calculateComponentScores(request: CaseScoringRequest, aiAnalysis: string): Promise<ScoringCriteria> {
    ...
    try {
      const aiScoresRaw = await this.callOllamaGenerate(this.SCORING_MODEL, aiScorePrompt, { temperature: 0.3, max_tokens: 200 });
  ...
  private async generateRecommendations(
    ...
    if (scores.witness_reliability < 0.6) recommendations.push('Assess witness credibility and consider additional witnesses');
    ...
    try {
      ...
      for (const [k, v] of Object.entries(parsed)) {
        if (typeof v === 'number') {
          ...
          if (k === 'evidence_strength' || k === 'witness_reliability' || k === 'legal_precedent' ||
              k === 'public_interest' || k === 'case_complexity' || k === 'resource_requirements') {
  ...
  private async saveScoring(result: CaseScoringResult): Promise<void> {
    try {
      ...
      try {
        const g = globalThis as unknown as { redisLogger?: { logError?: (payload: unknown) => Promise<void> | void } };
        if (g.redisLogger && typeof g.redisLogger.logError === 'function') {
          await g.redisLogger.logError(errorLog);
        } else {
  ...
  async phoenixWrightSearch(request: PhoenixWrightSearchRequest): Promise<PhoenixWrightSearchResult> {
    ...
    const searchResults: PhoenixWrightSearchResult = {
      ...
      disclaimer: 'This analysis is generated by AI and should be reviewed by qualified legal professionals. Not legal advice.'
    };
...
Return analysis in JSON format.`;
  ...
  private async generateRankingExplanation(results: PhoenixWrightSearchResult): Promise<string> {
    const prompt = `Generate a Phoenix Wright-style explanation for these search results:

Precedents found: ${results.precedents.length}
Contradictions identified: ${results.contradictions.length}
Evidence matches: ${results.evidenceMatches.length}

Key precedents: ${results.precedents.slice(0, 3).map(p => p.title).join(', ')}
...
Write a dramatic, attorney-style summary explaining the search results and their implications for the case.`;
  ...
  private calculateSearchConfidence(results: PhoenixWrightSearchResult): number {
    ...
    const contradictionScore = (results.contradictions.length > 0 ? 0.8 : 1) * weights.contradictions;
    const evidenceScore = results.evidenceMatches.reduce((sum, match) => sum + match.confidence, 0) /
                         Math.max(results.evidenceMatches.length, 1) * weights.evidenceMatches;
  ...
  updateYohaUI(results: PhoenixWrightSearchResult, config: YohaUIConfig): YohaUIState {
    const state: YohaUIState = {
      ...
      evidenceStrength: results.evidenceMatches.reduce((sum, match) => sum + match.relevance, 0) / Math.max(results.evidenceMatches.length, 1),
