 ...
 *
 * Stack: SvelteKit 2 + Drizzle ORM + PostgreSQL (legal_ai_db) + pgvector
 */
...
export class ClientServerSyncService {
   ...
   * Hits legal_ai_db via /api/vector/search for pgvector similarity
   */
  ...
  /**
   * Server-side vector search via legal_ai_db pgvector
   */
  private async performServerVectorSearch(
    ...
    // Fallback to standard vector search API (hits legal_ai_db)
  ...
  /**
   * Client-side vector search using cached embeddings (IndexedDB)
   */
  ...
  /**
   * Get cached vector search results from IndexedDB
   */
  ...
  /**
   * Cache vector search results to IndexedDB
   */
  private async cacheVectorSearchResults(
    ...
    try {
      ...
      await legalDB.vectorSearchCache.put(cacheEntry as VectorSearchCache);
  ...
  /**
   * Sync document to server (legal_ai_db evidence table)
   */
  ...
  /**
   * Pull documents from server (legal_ai_db)
   */
  async pullDocumentsFromServer(lastSyncTimestamp?: Date): Promise<void> {
    ...
    for (const serverDoc of documents) {
      await this.processServerDocument(serverDoc as Record<string, unknown>);
    }
  ...
  /**
   * Process document from server into IndexedDB cache
   */
  private async processServerDocument(serverDoc: Record<string, unknown>): Promise<void> {
    ...
    if (existingDoc) {
      ...
      const clientDoc: Omit<DocumentCache, 'id'> = {
        ...
        metadata: {
          aiSummary: (serverDoc.ai_analysis as Record<string, unknown>)?.summary as string,
      ...
      await legalDB.documentCache.put(clientDoc as DocumentCache);
    }
  }
  ...
  private async handleSyncConflict(
    ...
    switch (resolution.resolution) {
      case 'server_wins':
        // Overwrite client with server version
        await this.executeOperation({
             ...operation,
             data: serverVersion as Record<string, unknown>,
             type: 'update' // Force update on client side if needed, but sync usually pushes TO server.
             // If server wins, we usually just drop the client operation or update client state.
             // Here we assume we accept server state.
        });
        break;
      case 'client_wins':
        ...
        break;
      case 'merge':
        // Basic shallow merge strategy
        const merged = { ...(serverVersion as object), ...(operation.data as object) };
        await this.executeOperation({ ...operation, data: merged });
        break;
      case 'manual':
        console.warn('[Sync] Manual conflict resolution required for op:', operation.id);
  ...
  private async manageCacheSize(): Promise<void> {
    const count = await legalDB.vectorSearchCache.count();
    if (count > this.config.maxCacheSize) {
      const deleteCount = count - this.config.maxCacheSize;
      // Delete oldest entries based on timestamp
      const keys = await legalDB.vectorSearchCache
        .orderBy('timestamp')
        .limit(deleteCount)
        .primaryKeys();

      if (keys.length > 0) {
        await legalDB.vectorSearchCache.bulkDelete(keys);
        console.log(`[Sync Service] Evicted ${keys.length} cached vector search entries`);
      }
    }
  }
  ...
  dispose(): void {
    ...
    // Remove listeners if possible or set flag
    this.updateSyncStatus({ isOnline: false });
  }
}
