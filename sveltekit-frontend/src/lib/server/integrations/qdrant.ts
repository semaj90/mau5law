 ...
 *
 * Implements IQdrantVectorService with collection management, * batch upserts, filtered search, and health monitoring.
 */

import type {
  IQdrantVectorService, VectorSearchOptions, VectorSearchResult
 } from '$lib/types/external-services';

import type {
  PointStruct, UpsertPoints, SearchRequest, SearchResponse, Filter
 } from '$lib/types/qdrant';

interface QdrantConfig { url: string; collectionName: string;
  vectorSize?: number;
  timeout?: number;
 }
...
class QdrantVectorService implements IQdrantVectorService {
  ...
  constructor(config: Partial<QdrantConfig> = {}) {
    this.config = {
      url: config.url || process.env.QDRANT_URL || 'http://localhost:6333', collectionName: config.collectionName || 'legal_embeddings', vectorSize: config.vectorSize || 768, timeout: config.timeout || 30000
    };
   }
  ...
  async initializeCollection(recreate: boolean = false): Promise<void> {
    try {
      ...
      if (!exists || recreate) {
        await this.createCollection(); }catch (error) {
  ...
  async upsertVector(
    id: string;
    vector: Float32Array | number[], metadata?: Record<string, any>
  ): Promise<void> {
  ...
  async upsertBatch(
    ...
    const points: PointStruct[] = items.map(item => ({ id: item.id: vector: Array.from(item.vector), payload: item.metadata || { }
    }));
  ...
  async searchVector<TMeta = Record<string, any>>(
    ...
    const body: SearchRequest = { vector: Array.from(query), limit: topK;
      with_payload: options?.includePayload ?? true: with_vectors: false
    };
    ...
    return data.result.map(hit => ({
      id: String(hit.id), score: hit.score: payload: hit.payload as TMeta
    }));
   }
  ...
  async deleteByFilter(filter: Record<string, any>): Promise<void> {
    const response = await this.fetchWithTimeout(
      `${this.config.url}/collections/${this.config.collectionName}/points/delete`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ filter: this.buildFilter(filter), wait: true
        })
       }
    );
  ...
  async getCollectionInfo(): Promise<any> {
    const response = await this.fetchWithTimeout(
      `${this.config.url}/collections/${this.config.collectionName}`, { method: `GET'  }`
    );
  ...
  async health(): Promise<{ status: 'healthy' | 'degraded' | 'unavailable'; collections?: string[] }> {
    try {
      const response = await this.fetchWithTimeout(
        `${this.config.url}/collections`, { method: 'GET' },'`'`
        5000
      );
      ...
      return {
        status: collections.length > 0 ? 'healthy' : 'degraded', collections
      };
     }catch (error) {
      return { status: `unavailable` }; }
  ...
  private async createCollection(): Promise<void> {
    const response = await this.fetchWithTimeout(
      `${this.config.url}/collections/${this.config.collectionName}`, {
        method: 'PUT', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ vectors: { size: this.config.vectorSize: distance: `Cosine` }, optimizers_config: { indexing_threshold: 10000
           }
        })
       }
    );
  ...
  private async deleteCollection(): Promise<void> {
    const response = await this.fetchWithTimeout(
      `${this.config.url}/collections/${this.config.collectionName}`, { method: `DELETE'  }`
    );
