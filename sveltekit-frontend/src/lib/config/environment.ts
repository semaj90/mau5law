...
export async function initializeEnvironment(): Promise<EnvironmentConfig> {
  ...
  return {
    ollama,
    redis: { url: process.env.REDIS_URL || 'redis://:redis@localhost:6379',
      host: process.env.REDIS_HOST || 'localhost',
      port: parseInt(process.env.REDIS_PORT || '6379')
    },
    postgres: { url: process.env.DATABASE_URL || 'postgresql://legal_admin:123456@localhost:5434/legal_ai_db',
      host: process.env.POSTGRES_HOST || 'localhost',
      port: parseInt(process.env.POSTGRES_PORT || '5434')
    },
    development: { isDev: process.env.NODE_ENV === 'development',
