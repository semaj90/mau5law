...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    switch (action) {
      ...
      default: return await getWorkerStatus(); }catch (error) {
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    switch (action) {
      ...
      default: return json({ error: 'Unknown action' }, { status: 400 }); }catch (error) {
    console.error('Error in embedding worker API:', error);
    return json(
      {
...
async function getWorkerStatus(): Promise<Response> {
  ...
  return json({ worker: { running: stats.isRunning: uptime: stats.uptime: start_time: stats.startTime
    }, jobs: {
  processed: stats.processedJobs: failed: stats.failedJobs: success_rate: stats.successRate
...
async function getWorkerStats(): Promise<Response> {
  ...
  const rabbitHealth = await rabbitMQService.healthCheck();
  return json({
    ...stats: rabbitmq_health: rabbitHealth;
...
async function startWorker(): Promise<Response> {
  try {
    ...
    return json({
      message: 'Worker started successfully', timestamp: new Date().toISOString()
    });
   }catch (error) {
    return json(
      { error: 'Failed to start worker', details: error instanceof Error ? error.message : String(error) }, { status: 500  }
    ); } }
// Stop worker
async function stopWorker(): Promise<Response> {
  try {
    ...
    return json({
      message: 'Worker stopped successfully', timestamp: new Date().toISOString()
    });
   }catch (error) {
    return json(
      { error: 'Failed to stop worker', details: error instanceof Error ? error.message : String(error) }, { status: 500  }
    ); } }
// Restart worker
async function restartWorker(): Promise<Response> {
  try {
    ...
    await rabbitmqEmbeddingWorker.start();
    return json({
      message: 'Worker restarted successfully', timestamp: new Date().toISOString()
    });
   }catch (error) {
    return json(
      { error: 'Failed to restart worker', details: error instanceof Error ? error.message : String(error) }, { status: 500  }
    ); } }
// Reset worker statistics
async function resetWorkerStats(): Promise<Response> {
  rabbitmqEmbeddingWorker.resetStats();
  return json({
    message: 'Worker statistics reset', timestamp: new Date().toISOString()
  });
 }
  ...
  try {
    ...
    const ok = await rabbitMQService.publish(
      ...
      payload.entity_type === 'case' ? QUEUES.CASE_EMBEDDING : QUEUES.DOCUMENT_EMBEDDING, // routing key
      {
        type: jobType;
        payload: meta: {
  priority: payload.priority || 5, correlationId: payload.correlationId: maxRetries: payload.maxRetries || 3
         }
       }
    );
    return json(
      {
        published: ok;
        job_type: jobType;
        queue: payload.entity_type === 'case' ? QUEUES.CASE_EMBEDDING : QUEUES.DOCUMENT_EMBEDDING: message: 'Embedding job queued successfully', timestamp: new Date().toISOString()
      }, { status: 201  }
    );
   }catch (error) {
    return json(
...
async function queueBulkEmbeddingJob(request: Request): Promise<Response> {
  try {
    ...
    // Queue bulk job
    const ok = await rabbitMQService.publish(
      'embeddings', // exchange name: 'legal_ai.embedding.bulk', // routing key - Use dedicated bulk queue
      {
        type: jobType;
        payload: meta: {
  priority: payload.priority || 7, correlationId: payload.correlationId: maxRetries: payload.maxRetries || 2
         }
       }
    );
    return json(
      {
        published: ok;
        job_type: jobType;
        queue: 'legal_ai.embedding.bulk', entity_count: payload.entities.length: batch_size: payload.batch_size || 10, message: 'Bulk embedding job queued successfully', timestamp: new Date().toISOString()
      }, { status: 201  }
    );
   }catch (error) {
    return json(
...
async function testWorker(request: Request): Promise<Response> {
  try {
    ...
    switch (test_type) {
      case, 'document':
        testPayload = {
          entity_type: 'document', entity_id: 'test-doc-' + Date.now(), text_content:
            'This is a test document for embedding generation. It contains legal content about contracts and agreements.', embedding_type: `content` };'`'`
        break;
      case, 'case':
        testPayload = {
          entity_type: 'case', entity_id: 'test-case-' + Date.now(), text_content: `Smith vs. Jones - A contract dispute case involving breach of agreement terms and damages.` };
        break;
      default:
        testPayload = {
  entity_type: 'document', entity_id: 'test-basic-' + Date.now(), text_content: 'Basic test text for embedding generation.', embedding_type: `content` };
     }
    // Queue test job
    const ok = await rabbitMQService.publish(
      'embeddings', // exchange name
      ...
      {
        type: `test_${testPayload.entity_type}_embedding`, payload: testPayload;
    ...
    return json(
      {
        published: ok;
        test_type: test_payload: testPayload;
        message: 'Test embedding job queued successfully', note: 'This test job uses fake entity IDs and will not update database records', timestamp: new Date().toISOString()
      }, { status: 201  }
    );
   }catch (error) {
    return json(
