...
import { cases, evidence, documentChunks } from '../server/db/schema-postgres.js';
...
export class AIEnrichmentQueue {;
  ...
  async processCompletedEnrichment(jobId: string, result: any): Promise<void> {
    ...
    switch (job.type) {
      case 'case_analysis':
        await this.updateCaseWithAIResults(String(job.entity_id), result);
        break;
      case 'evidence_processing':
        await this.updateEvidenceWithAIResults(String(job.entity_id), result);
        break;
      case 'document_embedding':
        await this.storeDocumentEmbedding(String(job.entity_id), result);
        break;
    }
  ...
  async getJobStatus(jobId: string): Promise<EnrichmentJob | null> {
    ...
    return result.length ? (result[0] as unknown as EnrichmentJob) : null;
  }
  ...
  async getPendingJobs(type?: string, limit: number = 10): Promise<EnrichmentJob[]> {
    ...
    return result as unknown as EnrichmentJob[];
  }
}
