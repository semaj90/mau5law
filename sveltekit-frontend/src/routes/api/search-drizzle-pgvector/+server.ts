...
async function searchEvidenceWithDrizzle(
  ...
  try {
    ...
    return results.map((row: EvidenceRow) => ({
      id: row.id,
      title: row.title,
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const params = VectorSearchRequestSchema.parse(body);
    ...
    if (err && typeof err === 'object' && 'status' in err) {
      // Ensure the propagated error conforms to App.Error if it's an object
      const customError = err as { status: number; body: App.Error | string };
      return error(customError.status, customError.body);
    }
