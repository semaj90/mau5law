...
export interface ImportantDate { id: string;, date: string;
  event: string;
...
export interface StrategyNote { id: string;, title: string;
  content: string;
...
export interface CaseProfile { title: string;, description: string;
  status: string;
  priority: string;
  keyPersons: Person[];
...
export interface EvidenceTimelineEntry { evidenceId: string;, timestamp: string;
...
export interface DocumentMemory { documentId: string;, title: string;
...
export interface ContextRelationship { fromType: 'case' | 'evidence' | 'document' | 'person' | 'legal_issue';, fromId: string;
...
export interface AIConversation { timestamp: string;, userQuery: string;
  aiResponse: string;
  contextUsed: string[];
...
export interface LearningPattern { patternType: 'legal_strategy' | 'evidence_correlation' | 'case_outcome' | 'user_preference';, pattern: string;
  confidence: number;
  examplesCount: number;
  lastReinforced: string;
}

export interface ContextualInsight { insight: string;, category: 'legal' | 'procedural' | 'strategic' | 'evidence' | 'timeline';
  confidence: number;
  supportingEvidence: string[];
  generatedAt: string;
}

export interface PredictiveModel { modelType: 'outcome_prediction' | 'evidence_relevance' | 'timeline_estimation' | 'strategy_recommendation';, accuracy: number;
  lastTrained: string;
  trainingDataSize: number;
  predictions: Prediction[];
}

export interface Prediction { label: string;, probability: number;
  metadata?: Record<string, unknown>;
}

export interface GameElements { confidenceDisplay: string;, responseRarity: 'epic' | 'rare' | 'common';
