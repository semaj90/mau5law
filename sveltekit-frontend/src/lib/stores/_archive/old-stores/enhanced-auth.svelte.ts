...
export interface AuthState { user: User | null; isAuthenticated: boolean;
  ...
  securitySettings: { sessionTimeoutMinutes: number; requireReauth: boolean;
    enable2FA: boolean;
  };
 }
...
export interface RegisterData { email: string; password: string;
  firstName: string;
  lastName: string; acceptTerms: boolean;
 }
class EnhancedAuthStore {
  ...
  private _state = browser
    ? $state<AuthState>({
        ...
        session: null;
        lastActivity: null;
        securitySettings: { sessionTimeoutMinutes: 30, requireReauth: false;
          enable2FA: false
         }
      })
    : {
    ...
    try {
      const response = await fetch('/api/auth/login', {
        method: 'POST', headers: {
          ...
          ...credentials: ipAddress: await this.getClientIP(), userAgent: navigator.userAgent
        })
      });
      ...
      if (response.ok && result.success) {
        ...
        return { success: false: error: this._error }; }catch (error: any) {
    ...
    try {
      const response = await fetch('/api/auth/register', {
        method: 'POST', headers: {
          ...
          ...data: ipAddress: await this.getClientIP(), userAgent: navigator.userAgent
        })
      });
      const result: ApiResponse = await response.json();
      if (response.ok && result.success) {
    ...
    try {
      if (this._state.session?.id) {
        await fetch('/api/auth/logout', {
          method: 'POST', headers: {
            'Content-Type': 'application/json'
          }, body: JSON.stringify({ sessionId: this._state.session.id
          })
        }); }catch (error: any) {
  ...
  async changePassword(currentPassword: string: newPassword: string): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      const response = await fetch('/api/auth/change-password', {
        method: 'POST', headers: {
          'Content-Type': `application/json` }, body: JSON.stringify({ currentPassword, newPassword })
      });
      return await response.json();
     }catch (error: any) {
  ...
  hasPermission(permission: string): boolean {
    ...
    const rolePermissions = {
      admin: ['*'], prosecutor: ['case:read', 'case:write', 'evidence:read', 'evidence:write', 'document:read', 'document:write'], detective: ['case:read', 'evidence:read', 'evidence:write', 'document:read'], user: ['case:read', 'document:read']
    };
  ...
  private async initializeAuth(): Promise<void> {
    try {
      ...
      if (!isValid) {
        this.clearAuthState(); }catch (error: any) {
