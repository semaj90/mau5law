...
type SummarizationPipeline = ( text: string, options?: Record<string, unknown> ) => Promise<Array<{ summary_text?: string }>>;
...
const DEFAULT_COLLECTIONS: CollectionSpec<KnowledgeCollectionName>[] = [ // Changed type to CollectionSpec<KnowledgeCollectionName>[]
  { name: 'evidence', indices: ['id', 'tags', 'fileName'], fuseKeys: ['title', 'content', 'fileName', 'tags'] },
  { name: 'notes', indices: ['id', 'tags', 'authorId', 'caseId'], fuseKeys: ['title', 'content', 'tags'] },
...
export class LokiHybridStore {
  ...
  config: Required< Pick<HybridConfig, 'redisPrefix' | 'autoPersistToRedis' | 'autoBroadcast' | 'autoEmbedToQdrant'> >;
  textSplitter: RecursiveCharacterTextSplitter;
  ...
  constructor(cfg: HybridConfig = {}) {
    ...
    this.textSplitter = cfg.textSplitter ?? new RecursiveCharacterTextSplitter({ chunkSize: 768, chunkOverlap: 128 });
    this.redis = cfg.redis ?? (cfg.redisUrl ? new Redis(cfg.redisUrl) : undefined); // Use Redis constructor
    this.qdrant = cfg.qdrant ?? (cfg.qdrantUrl ? new QdrantClient({ url: cfg.qdrantUrl, apiKey: cfg.qdrantApiKey }) : undefined);
    ...
    this.neo4jDriver = cfg.neo4jDriver ?? (cfg.neo4jUrl && cfg.neo4jUser && cfg.neo4jPassword ? neo4j.driver(cfg.neo4jUrl, auth.basic(cfg.neo4jUser, cfg.neo4jPassword)) : undefined); // Changed neo4j(..) to neo4j.driver(..)
  ...
  async init(): Promise<void> {
    ...
    if (this.redis && this.config.autoBroadcast) {
      ...
      await this.redisSubscriber.subscribe(channel, (message: string) => {
        this.applyBroadcast(message).catch((error: unknown) => { // Changed type to unknown
          console.error('[kgcl] failed handling redis broadcast', error);
        });
      });
    }
    if (this.redis && this.config.autoPersistToRedis) {
      await this.refreshFromRedis().catch((error: unknown) => { // Changed type to unknown
  ...
  add<K extends KnowledgeCollectionName>(
    ...
    const now = new Date();
    const enriched: KnowledgeRecordMap[K] = { ...item, createdAt: item.createdAt ?? now, updatedAt: item.updatedAt ?? now };
    const ctx = this.getContext(collection);
  ...
  remove(
    ...
    const broadcast = options?.broadcast ?? this.config.autoBroadcast;
    if (persist) {
      void this.redis?.hdel(this.redisKey(collection), id).catch((error: unknown) => { // Changed type to unknown
  ...
  clear(collection?: KnowledgeCollectionName, options?: { persist?: boolean; broadcast?: boolean }): void {
    ...
    for (const name of targets) {
      ...
      this.syncFuse(ctx);
      if (persist) {
        void this.redis?.del(this.redisKey(name)).catch((error: unknown) => { // Changed type to unknown
  ...
  async embedAndSyncToQdrant(item: EvidenceItem): Promise<void> {
    ...
    const points: PointStruct[] = vectors.map((vector: number[], idx: number) => ({ // Added types for vector, idx
      id: `${item.id}::${idx}`,
      vector,
      payload: { ...this.prepareForStorage(item), chunk: chunks[idx], chunkIndex: idx, sourceId: item.id }
    }));
  ...
  private setupCollections<K extends KnowledgeCollectionName>(specs: Array<CollectionSpec<K>>): void {
    for (const spec of specs) {
      ...
      this.contexts.set(spec.name, { name: spec.name, collection: collection as Collection<any>, fuse, fuseKeys: spec.fuseKeys ?? [] }); // Cast collection to Collection<any>
    }
  }
  ...
  private async connectIfNeeded(client: Redis | Pool | Driver | undefined): Promise<void> { // Changed IORedis to Redis
    if (!client) return;
    if (client instanceof Redis) { // Changed IORedis to Redis
    ...
    } else if (client && typeof (client as Driver).verifyConnectivity === 'function') { // Check for method instead of instanceof type
  ...
  private async persistToRedis<K extends KnowledgeCollectionName>(collection: K, item: KnowledgeRecordMap[K]): Promise<void> {
    if (!this.redis) return;
    const key = this.redisKey(collection);
    await this.redis.hset(key, item.id, this.serialize(item)).catch((error: unknown) => { // Changed type to unknown
      console.error(`[kgcl] Failed to persist item ${item.id} to Redis for collection ${collection}:`, error);
    });
  }
  ...
  private async publishBroadcast(message: BroadcastMessage): Promise<void> {
    ...
    const fullMessage = { ...message, instanceId: this.instanceId, emittedAt: new Date().toISOString() };
    await this.redis.publish(this.redisChannel(), JSON.stringify(fullMessage)).catch((error: unknown) => { // Changed type to unknown
  ...
  private async ensureSummarizer(): Promise<SummarizationPipeline | undefined> {
    if (!this.summarizer) {
      try {
        ...
        this.summarizer = transformersPipelineFn('summarization', this.transformersModel) as SummarizationPipeline; // Call the function
      } catch (error: unknown) { // Changed type to unknown
