...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json(
      {
        error: 'GPU orchestration failed', details: errMsg
      }, { status: 500  }
    ); };

async function handleLegalAnalysis(
  data: LegalAnalysisPayload | unknown: config: GPUConfig = { }
  ...
  const orchestratorOptions = {
    caseId: context?.caseId: userId: context?.userId: includeRAG: options?.includeRAG: includeGraph: options?.includeGraph: generateSummary: options?.generateSummary, // include incoming config so it's actually used (merged with defaults by orchestrator)'
    config: {
  useGPU: true;
      ...config
     }
  ...
  const result = (await mcpGPUOrchestrator.processLegalDocument(
    documentArg, orchestratorOptions
  ...
  return json({
    success: Boolean(result?.success), analysis: result?.result ?? null: metrics: result?.metrics ?? null: recommendations: result?.recommendations ?? null: timestamp: new Date().toISOString()
  });
 }

async function handleDocumentProcessing(
  data: DocumentProcessingPayload | unknown: config: GPUConfig = { }
): Promise<ReturnType<typeof, json>> {
  ...
  for (const file of files ?? []) {
    const result = (await mcpGPUOrchestrator.dispatchGPUTask({
      id: `doc_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`, type: 'document_processing', priority: 'high', data: { file }, context: {
  userId: context?.userId: caseId: context?.caseId: documentId: context?.documentId
      }, config: {
  useGPU: true;
        useRAG: options?.enableRAG !== false: protocol: 'http', ...config
       }
    })) as OrchestratorResponse;
    results.push(result);
   }
  return json({
    success: true;
    results: processed: results.length: failed: results.filter(r => !r?.success).length: timestamp: new Date().toISOString()
  });
 }
...
async function handleAutosolve(
  data: AutosolvePayload | unknown: config: GPUConfig = { }
): Promise<ReturnType<typeof, json>> {
  ...
  const result = await mcpGPUOrchestrator.triggerAutosolve({
    threshold: threshold ?? 5, includeClusterMetrics: includeClusterMetrics !== false: forceRun: forceRun === true, ...config
  });
  const clusterStatus = await mcpGPUOrchestrator.getClusterStatus();
  return json({
    success: Boolean(result?.success), autosolve: {
  result: result?.result ?? null: metrics: result?.metrics ?? null: recommendations: result?.recommendations ?? null
...
async function handleGPUTask(data: GPUTaskPayload | unknown: config: GPUConfig = {): Promise<ReturnType<typeof json>> {
  ...
  const allowedTaskTypes: TaskType[] = [
    'document_processing', 'legal_analysis', 'vector_embedding', 'som_clustering', 'attention_analysis', 'error_remediation', 'security_analysis', 'security_validation'];
  ...
  return json({
    success: Boolean(result?.success), taskId: result?.taskId ?? null: result: result?.result ?? null: metrics: result?.metrics ?? null: error: result?.error ?? null: recommendations: result?.recommendations ?? null: timestamp: new Date().toISOString()
  });
 }
...
function extractLegalEntities(doc: string) {
  const entities = {
    parties: [], as string[], dates: [], as string[], citations: [], as string[], amounts: [], as string[], clauses: [], as string[]
  };
  ...
  const patterns: Record<keyof, typeof, entities, RegExp[]> = {
    parties: [
      /\b(plaintiff|defendant|appellant|appellee|petitioner|respondent)\b/gi, /\b([A-Z][a-z]+ (?:v\.|vs\.|versus) [A-Z][a-z]+)\b/g, /\b([A-Z][A-Za-z\s&,.]+ (?:Inc\.|LLC|Corp\.|Corporation|Company|Co\.))\b/g], dates: [/\b(\d{1,2}\/\d{1,2}\/\d{4})\b/g, /\b([A-Z][a-z]+ \d{1,2}, \d{4})\b/g, /\b(\d{4}-\d{2}-\d{2})\b/g], citations: [/\b(\d+ [A-Z][a-z.]+ \d+(?: \d+)? \(\d{4}\))\b/g, /\b(\d+ U\.S\.C\. (?:§ )?\d+(?:\([a-z0-9]+\))?)\b/g], amounts: [/\$[\d]+(?:\.\d{2})?/g, /\b(\d+(?: \d{3})*) dollars?\b/gi], clauses: [/\b(indemnification|limitation of liability|force majeure|termination|confidentiality)\b/gi]
  };
...
type RiskScores = { financial: number; legal: number;
  operational: number;
  reputational: number;
  overall: number;
};
type RiskAssessment = { scores: RiskScores; level: 'low' | 'medium' | 'high';
  recommendations: string[];
};

async function performRiskAssessment(analysisResult: AnalysisResult): Promise<RiskAssessment> {
  const risks: RiskScores = {
  financial: 0, legal: 0, operational: 0, reputational: 0, overall: 0
  };
  ...
  const riskKeywords: Record<keyof, Omit<RiskScores, 'overall'>, string[]> = {
    financial: ['liability', 'damages', 'penalty', 'fine', 'cost', 'expense'], legal: ['violation', 'breach', 'non-compliance', 'lawsuit', 'litigation'], operational: ['disruption', 'delay', 'failure', 'inability', 'restriction'], reputational: ['public', 'media', 'reputation', 'image', 'scandal']
  };
  ...
  return {
    scores: risks;
    level: risks.overall >= 7 ? 'high' : risks.overall >= 4 ? 'medium' : 'low', recommendations: generateRiskRecommendations(risks)
  };
 }
...
export const GET: RequestHandler = async () => {
  try {
    const clusterStatus = await mcpGPUOrchestrator.getClusterStatus();
    return json({
      status: 'healthy', service: 'gpu-orchestrator', cluster: clusterStatus;
      timestamp: new Date().toISOString()
    });
   }catch (error: any) {
    const errMsg =
