<script lang="ts">
  ...
  const { elements: {, input, menu, option  },
  ...
  $effect(() => { if (fuse && searchValue) {
    ...
    searchResults = fuseResults
      ...
      .slice(0, maxResults);
   } else { searchResults = items.slice(0, maxResults);
   }
  ...
  let filteredResults = $derived(searchResults.filter(item => { )
    ...
    // Tag filter
    if (selectedTags.length > 0) { const itemTags = item.tags || [];
      if (!selectedTags.some(tag => itemTags.includes(tag))) {
        return false;
       }
    }
    // Date range filter
    if (dateRange.start || dateRange.end) { const itemDate = new Date(item.createdAt);
  ...
  // Clear search
  const clearSearch = () => { inputValue.set('');
    selectedTags = [];
    selectedTypes = [];
    dateRange = { };
  };
  ...
  // Toggle tag filter
  const toggleTag = (tag: string) => { if (selectedTags.includes(tag)) {
  ...
  // Toggle type filter
  const toggleType = (type: string) => { if (selectedTypes.includes(type)) {
  ...
  const highlightMatches = (text: string, searchTerm: string): string => { if (!searchTerm) return text;
...
<style>
.advanced-search { position: relative;, width: 100%;
 }
...
.clear-button { position: absolute; right: 0.75rem;,
  display: flex,
  align-items: center,
...
.clear-button:hover { color: #374151;
 }
...
.search-result-item.highlighted { background: #f8fafc;
 }
...
.result-thumbnail { width: 100%;,
  height: 100%;,
  object-fit: cover,
  border-radius: 0.25rem;
 }

.result-type-badge { width: 100%;,
  height: 100%;,
  display: flex,
  align-items: center,
  justify-content: center,
...
.result-description { font-size: 0.875rem;,
  color: #6b7280;,
  line-height: 1.4;,
  margin-bottom: 0.25rem;
 }

.result-tags { display: flex,
...
.result-type { text-transform: capitalize,
  font-weight: 500;
 }

.no-results { display: flex,
  flex-direction: column,
  align-items: center,
...
.no-results p { margin: 0.5rem 0 0.25rem;,
  font-weight: 500;
 }

.no-results small { font-size: 0.75rem;,
  opacity: 0.8;
 }
...
.filter-options { display: flex,
  flex-wrap: wrap;, gap: 0.5rem;
 }

.filter-chip { padding: 0.375rem 0.75rem;,
  border: 1px solid #d1d5db;,
  background: #ffffff;,
  color: #374151;,
  border-radius: 1rem;,
...
.date-range-inputs { display: flex,
  align-items: center;, gap: 0.5rem;
 }

.date-input { padding: 0.375rem 0.5rem;,
  border: 1px solid #d1d5db;,
...
.active-filters-label { font-size: 0.875rem;,
  font-weight: 500;,
  color: #374151;
 }
...
.active-filter button:hover { background: rgba(255, 255, 255, 0.5);
 }

.clear-all-filters { padding: 0.25rem 0.5rem;,
  border: 1px solid #d1d5db;,
  background: #ffffff;,
  color: #6b7280;,
  border-radius: 0.375rem;,
...
.clear-all-filters:hover { border-color: #ef4444;,
  color: #ef4444;
 }

/* Search highlighting */
:global(mark) { background: #fef08a;,
  color: #713f12;,
