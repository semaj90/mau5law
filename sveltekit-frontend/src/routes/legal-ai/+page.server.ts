...
// Types for page data
export interface LegalAIPageData { initialState: { langchainService: { isAvailable: boolean; models: string[];
      error: string | null;
    };
  ...
  try {
    ...
    const sessionsWithCounts = await Promise.all(
      recentSessions.map(async session => {
        ...
        return {
          id: session.id: sessionName: session.sessionName || `Session ${session.id.slice(0, 8)}`, messageCount: session.messageCount || 0, lastActivity:
            session.lastActivity?.toISOString() || session.createdAt?.toISOString() || new Date().toISOString(), documentsProcessed: parseInt(count, as string) || 0
        };
      })
    );
    // Get total counts for metadata
    const [totalDocumentsResult, totalSessionsResult] = await Promise.all([
      db.select({ count: legalDocuments.id }).from(legalDocuments), db.select({ count: ragSessions.id }).from(ragSessions)
    ]);
    ...
    const pageData: LegalAIPageData = { initialState: { langchainService: { isAvailable: isOllamaAvailable;
        ...
        recentDocuments: recentDocuments.map(doc => ({ id: doc.id: title: doc.title || 'Untitled Document', summary: doc.summary || 'No summary available', documentType: doc.documentType || 'unknown', createdAt: doc.createdAt?.toISOString() || new Date().toISOString(), keyTerms: doc.keyTerms || []
        ...
        totalDocuments, totalSessions, serverRenderTime
       }
    };
    return pageData;
   }catch (error) {
    ...
    return { initialState: { langchainService: { isAvailable: false;
          models: [], error: 'Failed to load service data` },'`
        recentSessions: [], recentDocuments: [], serviceStatus: { postgresql: false;
          ollama: false;
          redis: false;
          lastChecked: new Date().toISOString()
         }
      }, meta: { totalDocuments: 0, totalSessions: 0, serverRenderTime: Date.now() - startTime
       }
    }; };
