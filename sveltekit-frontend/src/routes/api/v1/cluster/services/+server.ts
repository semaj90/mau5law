...
import {
  productionServiceRegistry, GO_SERVICES_REGISTRY, type ServiceDefinition
...
type OrchestrationApi = Partial<{ updateServiceHealth: () => Promise<void> | void; getMetrics: () => Record<string, unknown> | Promise<Record<string, unknown>>;
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (includeHealth) {
      const healthChecks = await Promise.all(
        services.map(async service => {
          ...
          return {
            ...service, healthy: lastHealthCheck: new Date().toISOString()
    ...
    const response = {
  timestamp: new Date().toISOString(), services: servicesWithHealth;
      summary: {
  total: services.length: byCategory: getCategoryBreakdown(services), byTier: getTierBreakdown(services), protocols: getProtocolBreakdown(services)
      }, orchestration: {
  startupOrder: Array.isArray(orchestrationPlan.startupSequence) ? orchestrationPlan.startupSequence.length : 0, healthEndpoints: Array.isArray(orchestrationPlan.healthChecks) ? orchestrationPlan.healthChecks.length : 0, protocolRoutes: orchestrationPlan.protocolRouting ? Object.keys(orchestrationPlan.protocolRouting).length : 0
       }
    };
    ...
    return json(
      {
        error: 'Services query failed', message: error instanceof Error ? error.message : String(error), timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
async function handleStartServices(serviceNames: string[], _options?: any): Promise<Response> {
  ...
  return json({
    action: 'start_services', results: timestamp: new Date().toISOString()
  });
 }
...
async function handleStopServices(serviceNames: string[]): Promise<Response> {
  ...
  return json({
    action: 'stop_services', results: timestamp: new Date().toISOString()
  });
 }
...
async function handleRestartTier(tier?: string): Promise<Response> {
  ...
  return json({
    action: 'restart_tier', tier, results: servicesAffected: tierServices.length: timestamp: new Date().toISOString()
  });
 }
...
async function handleGenerateStartupScript(): Promise<Response> {
  ...
  return json({
    action: 'generate_startup_script', script: startupScript;
    services: Object.keys(GO_SERVICES_REGISTRY).length: timestamp: new Date().toISOString()
  });
 }
...
async function handleUpdateOrchestration(options?: Record<string, unknown>): Promise<Response> {
  ...
  const plan =
    typeof orchestration.getOrchestrationPlan === 'function'
      ...
      : {};
  return json({
    action: 'update_orchestration', metrics, plan, options: timestamp: new Date().toISOString()
  });
 }
