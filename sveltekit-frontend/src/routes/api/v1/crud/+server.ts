...
import {
  users, cases, criminals, evidence, legalDocuments, reports, personsOfInterest, ragMessages, ragSessions
...
const CrudBodySchema = z.object({
  action: z.enum(['create', 'read', 'update', 'delete', 'list', 'search', 'vector_search']).optional().default('list'), entity: z.string(), id: z.string().optional(), data: z.any().optional(), filters: z.record(z.any()).optional(), pagination: z.object({ page: z.number().optional(), limit: z.number().optional() }).optional(), search: z.object({ query: z.string().optional(), similarity_threshold: z.number().optional() }).optional()
});
...
export const PUT: RequestHandler = async ({ request, url }) => {
  ...
  request: new Request(request.url, {
      method: 'POST', headers: request.headers: body: JSON.stringify({ ...body: action: 'update', id })
    })
