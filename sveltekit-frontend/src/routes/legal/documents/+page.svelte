<script lang="ts">
import { Input } from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/input'; import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/dialog';
import type { Case } from '$lib // TODO: Verify store subscription is correct for Svelte 5/types';
import type { Document } from '$lib // TODO: Verify store subscription is correct for Svelte 5/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { goto } from '$app // TODO: Verify store subscription is correct for Svelte 5/navigation'; import { Plus, Upload, FileText, Search, Filter, Eye, Download, Trash2, Edit2, Bot, Zap, AlertCircle, CheckCircle, Clock, BarChart3 } from 'lucide-svelte'; import Button from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/enhanced-bits.svelte'; import  Input  from "$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/enhanced-bits.svelte"; import Label from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/label/Label.svelte'; import * as Card from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/Card.svelte'; import * as Dialog from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/Dialog.svelte'; import * as Select from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/select.svelte'; import Badge from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/badge/Badge.svelte'; import Progress from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/progress/Progress.svelte'; import { toast } from 'svelte-sonner'; import { cn } from '$lib // TODO: Verify store subscription is correct for Svelte 5/utils'; interface Document { id: string, title: string, type: 'legal_brief' | 'contract' | 'evidence' | 'report' | 'template' | 'other'; status: 'draft' | 'processing' | 'review' | 'final' | 'archived'; created: string, updated?: string; size: number, author: string, caseId?: string; tags: string[], aiAnalysis?: { summary: string, keyPoints: string[], confidence: number, legalConcepts: string[]}
   ...
   // State management with Svelte, 5 runes let documents = $state // TODO: Verify store subscription is correct for Svelte 5<Document[]>([]); let filteredDocuments = $state // TODO: Verify store subscription is correct for Svelte 5<Document[]>([]); let loading = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(true); let uploading = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let uploadProgress = $state // TODO: Verify store subscription is correct for Svelte 5<number>(0); let searchQuery = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); let statusFilter = $state // TODO: Verify store subscription is correct for Svelte 5<string>('all'); let typeFilter = $state // TODO: Verify store subscription is correct for Svelte 5<string>('all'); let showUploadDialog = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let showAIAnalysisDialog = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let selectedDocument = $state // TODO: Verify store subscription is correct for Svelte 5<Document | null>(null); let dragOver = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); // Upload form state let uploadTitle = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); let uploadType = $state // TODO: Verify store subscription is correct for Svelte 5<string>('other'); let uploadFile = $state // TODO: Verify store subscription is correct for Svelte 5<File | null>(null); let uploadCaseId = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); let uploadTags = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); let enableAIProcessing = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(true); // Computed properties let documentStats = $derived // TODO: Verify store subscription is correct for Svelte 5(() => { const total = documents.length; const processing = documents.filter(item => item.processingStatus && (item.processingStatus.ocr === 'processing' || item.processingStatus.analysis === 'processing' || item.processingStatus.embeddings === 'processing') ).length; const completed = documents.filter(item => item.status === 'final' || (item.processingStatus && item.processingStatus.analysis === 'completed') ).length; const withAI = documents.filter(item => !!item.aiAnalysis).length; return { total, processing, completed, withAI }}); $effect // TODO: Verify store subscription is correct for Svelte 5(() => { (async () => { await loadDocuments()})()});
  ...
  async function deleteDocument(doc: Document): Promise<void> { if (!confirm('Are you sure you want to delete this document?')) return; try { const response = await fetch(`/api/legal/documents/${doc.id}`, { method: 'DELETE'
   ...
   // Watch for search/filter changes $effect // TODO: Verify store subscription is correct for Svelte 5(() => { filterDocuments()});
</script>
