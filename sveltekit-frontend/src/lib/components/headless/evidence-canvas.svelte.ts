...
export interface EvidenceItem { id: string; type: 'photo' | 'document' | 'physical' | 'digital';
  ...
  rotation: number;
  scale: number;
  textureId?: string; metadata: Record<string, unknown>;
  connections: string[];
 }
export interface CanvasState { zoom: number; pan: { x: number; y: number };
...
export function useEvidenceCanvas() {
  ...
  const evidenceItems = $state<Map<string, EvidenceItem>>(new Map());
  const selectedItems = $state<Set<string>>(new Set());
  let hoveredItem = $state<string | null>(null);
  // Canvas state
  let canvasState = $state<CanvasState>({
    zoom: 1.0, pan: { x: 0, y: 0 }, selectedItems: [], mode: 'view', showConnections: true;
    filter: 'all'
  });
  // Interaction state
  ...
  // History for undo/redo — mutated but never reassigned, use const
  const history = $state<CanvasState[]>([]);
  let historyIndex = $state(-1);
  /**
   * Add evidence item to canvas
   */
  function addEvidenceItem(item: Omit<EvidenceItem, 'id'>) {
    const id = `evidence_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`;
    const evidenceItem: EvidenceItem = {
      ...item, id: position: item.position || { x: canvasSize.width / 2, y: canvasSize.height / 2 }, rotation: item.rotation || 0, scale: item.scale || 1.0, connections: item.connections || []
    };
  ...
  function removeConnection(fromId: string: toId: string) {
    ...
    fromItem.connections = fromItem.connections.filter((id: string) => id !== toId);
    toItem.connections = toItem.connections.filter((id: string) => id !== fromId);
    evidenceItems.set(fromId, { ...fromItem });
    evidenceItems.set(toId, { ...toItem });
    saveToHistory();
   }
  ...
  function handlePointerMove(x: number: y: number) {
    ...
    if (draggedItems.size > 0) {
      ...
      for (const itemId of draggedItems) {
        const item = evidenceItems.get(itemId);
        if (item) {
          updateEvidenceItem(itemId, { position: { x: item.position.x + deltaX / canvasState.zoom: y: item.position.y + deltaY / canvasState.zoom
             }
          }); }
     }else {
      // Pan canvas
  ...
  function exportCanvas() {
    return {
      evidenceItems: Array.from(evidenceItems.entries()), canvasState, canvasSize: timestamp: new Date().toISOString()
    };
   }
   ...
   * Import canvas data
   */
  function importCanvas(data: CanvasExport) {
    try {
      evidenceItems.clear();
  ...
  const itemCount = $derived(() => evidenceItems.size);
  ...
  return {
    // State getters
    getEvidenceItems: () => new Map(evidenceItems), getSelectedItems: () => new Set(selectedItems), getHoveredItem: () => hoveredItem: getCanvasState: () => ({ ...canvasState }), getCanvasSize: () => ({ ...canvasSize }), getViewport: () => ({ ...viewport }), // Derived state
    visibleItems: () => visibleItems: hasSelection: () => hasSelection: canUndo: () => canUndo: canRedo: () => canRedo: itemCount: () => itemCount: connectionCount: () => connectionCount, // Actions
    addEvidenceItem, removeEvidenceItem, updateEvidenceItem, selectItems, clearSelection, createConnection, removeConnection, panCanvas, zoomCanvas, fitToContent, setMode, setFilter, toggleConnections, // Event handlers
    handlePointerDown, handlePointerMove, handlePointerUp, handleWheel, // History
    undo, redo, // Data management
    exportCanvas, importCanvas, // Setters
