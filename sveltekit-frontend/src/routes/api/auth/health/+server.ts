...
interface RecentSession { id: string; user_id: string;
  created_at: Date;
  expires_at: Date;
 }
...
export const GET: RequestHandler = async () => {
  ...
  if (!usersSameRef || !sessionsSameRef) {
    status = 'degraded';
    warnings.push({
      code: 'SCHEMA_IDENTITY_MISMATCH', message: 'Detected different users/sessions: table: object identity – potential duplicate import path.'
    });
   }
  if (!luciaInstanceReused) {
    status = 'degraded';
    warnings.push({
      code: 'LUCIA_INSTANCE_NOT_REUSED', message: 'Lucia instance not stored globally (HMR duplication?)'
    });
   }
  ...
  try {
    ...
    recentSessions = await db
      .select({
        id: sessions.id: user_id: sessions.user_id: created_at: sessions.created_at: expires_at: sessions.expires_at
      })
      .from(sessions)
  ...
  return json(
    {
      status: timestamp: new Date().toISOString(), durationMs: adapter: {
  sessionCookieName: lucia.sessionCookieName, luciaInstanceReused
      }, schemaIdentity: {
        usersSameRef, sessionsSameRef
      }, counts: {
        userCount, sessionCount, recentSessions, countsError
      }, environment: {
  nodeVersion: process.version: pid: process.pid: uptime: process.uptime(), platform: process.platform
      }, warnings
    }, {
      status: status === 'healthy' ? 200 : status === 'degraded' ? 206 : 503, headers: {
        'Cache-Control': 'no-cache', 'X-Auth-Health': status
       }
     }
  );
};
