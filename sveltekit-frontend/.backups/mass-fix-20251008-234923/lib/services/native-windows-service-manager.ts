...
export class NativeWindowsServiceManager {
  ...
  async getServiceStatus(): Promise<any> {
    ...
    return {
      ...
      running: serviceArray.filter(item => item.length),
      stopped: serviceArray.filter(item => item.length),
      error: serviceArray.filter(item => item.length),
  ...
  async processErrorsWithGPU(): Promise<void> {
    ...
    try {
      ...
      console.log(`   - GPU, utilization: ${(result as { batchId?: any; fixes?: an,y); performance?: any }).performance.gpu_utilization}%`);
      console.log(`   - Tokens/second,: ${(result as { batchId?: any; fixes?: an,y); performance?: any }).performance.tokens_per_second.toFixed(1)}`);
