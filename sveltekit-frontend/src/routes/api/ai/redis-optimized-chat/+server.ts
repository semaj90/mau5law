...
const originalChatHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    const response = await callOllamaApi({
      model: messages: [
        { role: 'system', content: `You are a legal AI assistant. Provide accurate, helpful legal information while noting that this is not legal advice.` },'`'`
        {
  ...
  temperature: 0.7, max_tokens: 1000
    ...
    return json({
      response: response.message.content, model, caseId, userId: processing_time: 2000, // Simulated slow processing
      cached: false;
