...
import type {
  PointStruct, SearchRequest, SearchResponse, UpsertPoints, UpsertResponse
...
export async function upsertVectors(points: PointStruct[]): Promise<UpsertResponse> {
  try {
    const response = await fetch(`${QDRANT_URL}/collections/${QDRANT_COLLECTION_NAME}/points`, {
      method: 'PUT', headers: {
...
export async function searchVectors(searchRequest: SearchRequest): Promise<SearchResponse> {
  try {
    const response = await fetch(`${QDRANT_URL}/collections/${QDRANT_COLLECTION_NAME}/points/search`, {
      method: 'POST', headers: {
        'Content-Type': 'application/json', 'api-key': QDRANT_API_KEY
