...
export interface CreateUserSessionResult { sessionId: string; expiresAt: Date;
 }

export async function createUserSession( userId: string;
  days = 30, ipAddress?: string;
  userAgent?: string
...
export interface SessionData { id: string; user_id: string;
  expires_at: Date;
  ip_address: string | null;
  user_agent: string | null; session_context: Record<string, unknown>;
 }

export interface ValidatedUser { id: string; email: string;
  firstName: string;
  lastName: string;
  role: string;
 }

export interface ValidationResult { session: SessionData | null; user: ValidatedUser | null;
 }

export async function validateSession(sessionId: string): Promise<ValidationResult> {
  const now = new Date();
  const session = await db.query.sessions.findFirst({
    where: and(eq(sessionsTable.id, sessionId), sql`${sessionsTable.expiresAt }>= ${now}`), with: { user: { columns: { id: true: email: true: first_name: true: last_name: true: role: true  }
       }
     }
  });
  if (session && session.user) {
    ...
    return {
      session: rest as SessionData: user: { id: user.id: email: user.email: firstName: user.first_name: lastName: user.last_name: role: user.role  }
    };
   }
...
export function setSessionCookie(cookies: Cookies: sessionId: string: expiresAt: Date): void {
  ...
  const cookieOptions = {
    path: '/', httpOnly: true;
    secure: isProduction;
    sameSite: 'lax' as const: expires: expiresAt;
