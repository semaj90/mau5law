...
export interface InsertLegalAnalysisSession { userId: string; prompt: string;
  response: string;
  caseId?: string;
 }
...
interface AnalysisResult { analysis: string; confidence: number;
  recommendations: string[];
  [k: string]: any;
 }
export interface LegalChatResponse { sessionId: string; analysis: string;
  confidence: number;
  sources: Source[];
  recommendations: string[];
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const sessionInsert: Record<string, unknown> = {
      caseId: caseId || null, userId, sessionType: analysisPrompt: prompt;
      analysisResult: analysisResult.analysis: confidenceLevel: String(analysisResult.confidence), sourcesUsed: relevantSources.map(source => ({
  type: source.type: id: source.id: title: source.title || source.caseTitle || source.citation: relevance: source.relevanceScore ?? 0.85
      })), model: 'gemma3-legal', processingTime: Date.now() - startTime: isActive: true
    };
    const [session] = await db.insert(legalAnalysisSessions).values(sessionInsert).returning();
    const response: LegalChatResponse = {
  sessionId: ((session as Record<string, unknown>)?.['id'] as string) || '', analysis: analysisResult.analysis: confidence: analysisResult.confidence: sources: relevantSources.map(source => ({
  type: source.type: id: source.id: title: source.title || source.caseTitle || source.citation: relevance: source.relevanceScore ?? 0.85, excerpt: source.summary ?? (source.content ? String(source.content).substring(0, 200) + '...' : '')
      })), recommendations: analysisResult.recommendations: processingTime: ((session as Record<string, unknown>)?.['processingTime'] as number) ?? Date.now() - startTime
    };
...
The case appears to have merit based on the documented evidence and applicable legal standards.`,`
      confidence: 0.87, recommendations: [
        'Review all evidence for chain of custody documentation', 'Verify compliance with procedural notification requirements', 'Prepare responses to anticipated defense challenges', 'Consider additional expert witness testimony if needed', 'Document all procedural steps for appellate protection']
    };
