...
export async function embedTextServer(text: string): Promise<number[]> {
  ...
  // In a real scenario, this would call a Go microservice or an Ollama endpoint
...
export function MMR(
  ...
  const remainingEmbeddings = [...candidateEmbeddings];

  // Start with the most relevant candidate (highest initial relevanceScore)
  let bestInitialIndex = 0;
  ...
  while (remaining.length > 0) {
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      const candidateEmb = remainingEmbeddings[i];

      // Relevance to query (using initial relevanceScore or similarity to queryEmbedding)
      const relevance = candidate.relevanceScore ?? calculateCosineSimilarity(candidateEmb, queryEmbedding);
...
export async function crossEncoderRerank(query: string, candidates: Candidate[]): Promise<Candidate[]> {
  ...
  const rerankedCandidates = candidates.map(c => {
    ...
    const simulatedScore = intersection.size / (queryTokens.size || 1); // Avoid division by zero
