...
export type AvatarUploadFile = File | Blob | MinimalFileLike | undefined;
...
async function getArrayBufferFromFile(file: AvatarUploadFile): Promise<ArrayBuffer> {
    ...
    if (isMinimalFileLike(file) || isDomFile(file) || isDomBlob(file))
...
export async function handleAvatarUpload(
    ...
    try {
        ...
        const fileName = generateAvatarFileName(
            userId,
            originalName || `upload.${mimeToExtension(declaredType)}`
        );
