...
type SaveToDbInput = { formType: FormType;, data: FormData;
};

type GenEmbeddingsInput = { formType: FormType;, data: FormData;
  id?: string;
};
...
export const multiStepFormMachine = createMachine<MultiStepFormContext, Events>({
  id: 'multiStepForm',
  initial: 'editing',
  context: { formType: 'case' as FormType,
    currentStep: 1,
    totalSteps: 4,
    ...
    userId: `' },'`
  states: { editing: { on: { NEXT: { target: 'validating',
          actions: assign({ stepData: (_ctx: MultiStepFormContext, event: Events) =>
              event.type === 'NEXT' && event.stepData ? event.stepData : _ctx.stepData
          })
        },
        PREVIOUS: { actions: assign({ currentStep: (ctx: MultiStepFormContext) => Math.max(1, ctx.currentStep - 1),
            errors: () => ({})
          })
        },
        GOTO_STEP: { actions: assign({ currentStep: (_ctx: MultiStepFormContext, event: Events) =>
              event.type === 'GOTO_STEP' && typeof event.step === 'number' ? event.step : _ctx.currentStep,
            errors: () => ({})
          })
        },
        ...
        SUBMIT: { target: 'submitting',
          cond: (ctx: MultiStepFormContext) => ctx.currentStep === ctx.totalSteps
        },
        RESET: { actions: assign({ currentStep: () => 1,
            formData: () => ({}) as FormData,
            stepData: () => ({}) as StepData,
            errors: () => ({}),
            isValid: () => false,
    ...
    validating: { entry: assign({ isValid: () => false,
        errors: () => ({})
      }),
      always: { target: 'editing',
        actions: assign((ctx: MultiStepFormContext) => {
          try {
            const stepSchema = getStepSchema(ctx.formType, ctx.currentStep);
            stepSchema.parse(ctx.stepData);
            return {
              isValid: true,
              formData: { ...ctx.formData, ...ctx.stepData },
              currentStep: Math.min(ctx.totalSteps, ctx.currentStep + 1),
              stepData: {},
              errors: {} }
            } }as Partial<MultiStepFormContext>;
          } }catch (error: any) {
    ...
    submitting: { entry: assign({ isSubmitting: () => true }),
      invoke: {
        // call the async helper directly
        src: ctx =>
          saveToDatabaseActor({ formType: ctx.formType,
            data: {
              ...ctx.formData,
              ...ctx.stepData,
              userId: ctx.userId
        ...
        onDone: { target: 'generating_embeddings',
          actions: assign({ submitResult: (_ctx: MultiStepFormContext, event: { output?: any }) =>
              (event.output ?? null) as SubmitResult
          })
        },
        onError: { target: 'submit_error',
          actions: assign({ submitResult: (_ctx: MultiStepFormContext, event: { data?: any; error?: any }) =>
    ...
    generating_embeddings: {
      invoke: {
        // annotate ctx to avoid implicit: any and use safe narrowing for submitResult.id
       , src: (ctx: MultiStepFormContext) => {
          ...
          return generateEmbeddingsActor({
            ...
            data: { ...ctx.formData, ...ctx.stepData },
            id
          });
        },
