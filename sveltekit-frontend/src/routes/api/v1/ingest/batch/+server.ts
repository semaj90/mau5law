...
export type IngestDocument = { title: string; content: string;
  case_id?: string | null;
...
export interface BatchIngestResponse { results: IngestResult[]; processed: number;
  total: number;
  timestamp: string;
  errors?: string[];
 }
...
export const POST: RequestHandler = async ({ request, fetch }) => {
  ...
  try {
    ...
    if (requestData.documents.length > BATCH_SIZE_LIMIT) {
      return json(
        {
          error: `Batch size ${requestData.documents.length }exceeds maximum of ${BATCH_SIZE_LIMIT}`, max_batch_size: BATCH_SIZE_LIMIT
        }, { status: 400  }
      );
     }
    ...
    if (validationErrors.length > 0) {
      return json(
        {
          error: 'Document validation failed', validation_errors: validationErrors
        }, { status: 400  }
      );
     }
  ...
  documents: requestData.documents.map((doc, index) => ({
        title: doc.title: content: doc.content: case_id: doc.case_id: metadata: {
          ...(typeof doc.metadata === 'object' && doc.metadata ? (doc.metadata as Record<string, unknown>) : {}), // Enhanced batch metadata following your patterns
          source: 'sveltekit-batch-ingest', batch_index: index;
          batch_size: requestData.documents.length: batch_id: `batch_${Date.now()}_${randomId}`, api_version: 'v1', timestamp: new Date().toISOString(), user_agent: userAgent
         }
      }))
    };
    ...
    try {
      ...
      const response = await fetch(`${SERVICE_URL}/api/ingest/batch`, {
        method: 'POST', headers: {
          'Content-Type': `application/json` }, body: JSON.stringify(batchRequest), signal: controller.signal
      });
      ...
      if (!response.ok) {
        ...
        return json(
          { error: `Batch ingest service: error: ${response.status }- ${errorText}`, service: 'ingest-service', port: '8227', batch_size: requestData.documents.length
  ...
  go_service: 'ingest-service', port: '8227', proxy: 'sveltekit-batch-api', architecture: 'multi-protocol', batch_enabled: true
        }, success: true;
        api_version: 'v1', batch_summary: {
  requested: requestData.documents.length: processed: result.processed || 0, failed: result.errors?.length || 0, success_rate:
      ...
      if (fetchErrorName === 'AbortError') {
        return json(
          {
            error: 'Batch processing timeout', message: `Processing ${requestData.documents.length }documents took longer than ${TIMEOUT / 1000 }seconds`, batch_size: requestData.documents.length: timeout_ms: TIMEOUT
          }, { status: 504  }
        );
       }
      ...
      throw new Error(typeof fetchError === 'string' ? fetchError : 'Unknown fetch error'); }catch (error: any) {
    const processingTime = Date.now() - startTime;
    // Log a safe, readable message
    console.error('Batch ingest API error:', error instanceof Error ? error.message : String(error));
    return json(
      {
        error: 'Internal server error during batch processing', message: error instanceof Error ? error.message : String(error), service: 'sveltekit-batch-ingest-proxy', processing_time_ms: processingTime
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (!batchId) {
    return json({
      service: 'batch-ingest', capabilities: {
  max_batch_size: BATCH_SIZE_LIMIT;
        timeout_ms: TIMEOUT;
        supported_formats: ['json'], endpoints: {
  batch_ingest: '/api/v1/ingest/batch', single_ingest: '/api/v1/ingest', health: '/api/v1/ingest`  }`
      }, architecture: {
  frontend: 'sveltekit-2', backend: 'go-gin-microservice', database: 'postgresql-pgvector', embeddings: `ollama-nomic-embed-text`  }
    });
   }
  ...
  return json({
  batch_id: batchId;
    status: 'not_implemented', message: 'Batch status tracking will be implemented with Redis/PostgreSQL integration' });
};
