...
async function getQuicEngine() {
	if (!QuicEngine) {
		try {
			...
			throw error(503, { message: 'QUIC recommendation engine unavailable' });
		}
	}
	return QuicEngine;
}
...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		if (!query) {
			throw error(400, { message: 'Query parameter (q or query) is required' });
		}
		...
		response.headers.set('X-GPU-Used', recommendations.tensorMetrics.tensorCoresUsed ? 'true' : 'false');
		response.headers.set('X-SIMD-Optimized', recommendations.metadata.simdOptimized ? 'true' : 'false');
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!query && !batchQueries) {
			throw error(400, { message: 'Query or batchQueries required' });
		}
