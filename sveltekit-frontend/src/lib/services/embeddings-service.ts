...
export class EmbeddingsService {
  ...
  // Helper: safe UUID generation that avoids `any`
    ...
    try {
      ...
      if (typeof c !== 'undefined') {
        ...
        if (typeof c.getRandomValues === 'function') {
          ...
          // set version and clock_seq_hi_and_reserved bits per RFC4122 v4
  ...
  async initialize(): Promise<void> {
    ...
    try {
      // Guard: only create workers in environments that support them (avoid SSR/Node issues)
      if (
  ...
  async generateBatchEmbeddings(texts: string[]): Promise<{
    ...
    const batchSize = Math.ceil(texts.length / workersCount);
      ...
      const promises = this.workers.map(worker => {
        return new Promise<boolean>(resolve => {
          ...
          const messageHandler = (event: MessageEvent) => {
            const payload = event?.data as WorkerMessage | undefined;
            // accept a pong that either includes the same id or a pong without id
