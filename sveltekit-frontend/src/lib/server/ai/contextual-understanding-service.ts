...
const CONTEXT_TTL_SECONDS = Number(process.env.CONTEXT_STATE_TTL ?? 3600, const MAX_HISTORY_LENGTH = Number(process.env.CONTEXT_MAX_HISTORY ?? 50);
...
export class ContextualUnderstandingService {
 ...
 embedding?: number[]); attachments: AttachmentMetadata[] = []
  ...
  const key = this.keyFor(sessionId, const current = await this.getContextualState(sessionId, userId);
...
const updatedHistory = [...current.conversationHistory, newTurn].slice(-MAX_HISTORY_LENGTH, const updatedHmm = hmmStateMachine.updateState(current.hmmState, newTurn);
 ...
 const state = await this.getContextualState(sessionId, userId, const turns = state.conversationHistory.slice(-maxTurns);
 ...
 const patterns = hmmStateMachine.detectPatterns(state.hmmState.stateHistory, const topPattern = patterns[0]?.frequency ?? 0;
