<script lang="ts">
  ...
  async function initializeEditor() {
    try {
      ...
      hugerte = {
        ...
        off: () => {},
        insertContent: (newContent: string) => { content += newContent; },
        selection: {
        ...
        config: {
          ...
          setup: (editor: any) => {
            ...
            editor.on('input', () => {
              ...
              content = newContent;
              const wc = updateCounts(newContent);
              ondispatch?.({ content: newContent, wordCount: wc });
            });
            // Save handler
            editor.on('save', () => {
  ...
  async function processAIRequest() {
    ...
    try {
      ...
      if (data.success) {
        ...
        ondispatch?.({ type: 'ai', selectedText, action: aiQuery });
      } else {
  ...
  function insertCitation(citation: any) {
    const citationHtml = `
        <div class="citation" contenteditable="false">
          <strong>${citation.title}</strong>
          <p><em>${citation.citation}</em></p>
        </div>
      `;
...
<!-- Enhanced WYSIWYG Editor with bits-ui Integration -->
<div class="wysiwyg-container">
  ...
  <!-- Main Editor -->
  <!-- contenteditable fallback/plain HTML5 editor surface for accessibility -->
  <div
    ...
    <div class="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-600px -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6" role="dialog" aria-modal="true" aria-labelledby="cite-title">
      ...
      <div class="pt-4">
        <div class="space-y-4">
          ...
          />
          <button onclick={searchCitations} disabled={!citationQuery.trim()} class="btn btn-primary"> Search </button>
