...
export const POST: RequestHandler = async ({ request }) => {
  ...
  query: body.query.toLowerCase().trim(), limit: body.limit || 5, type: 'semantic-search'
    ...
    const { data: searchData, cacheHit  }= await defaultQueryCache.getOrQuery(
      cacheQuery, async () => {
        ...
        const searchResponse = await fetch(
          `${GO_SEARCH_URL}?q=${encodeURIComponent(body.query)}&limit=${body.limit || 5}`, {
            method: 'GET', headers: {
              'Content-Type': `application/json` }, signal: AbortSignal.timeout(15000), // 15 second timeout
           }
        );
    ...
    return json({
      success: searchData.success: query: searchData.query: results: searchData.results || [], count: searchData.count || 0, timestamp: new Date().toISOString(), cache: {
  hit: cacheHit.hit: source: cacheHit.source: latency: '${cacheHit.latency.toFixed(2)}ms'  }
    });
   }catch (error: any) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      status: 'healthy', service: 'semantic-search', backend_status: healthData.status: pgvector_enabled: true;
      timestamp: new Date().toISOString()
    });
   }catch (error: any) {
    return json(
      {
        status: 'error', service: 'semantic-search', error: error instanceof Error ? error.message : 'Unknown error', timestamp: new Date().toISOString()
      }, { status: 503  }
    ); };
