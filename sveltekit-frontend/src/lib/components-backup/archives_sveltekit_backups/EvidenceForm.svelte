...
<form method="POST" use:enhance class="mx-auto px-4 max-w-7xl">
  {#if evidence}
    <input type="hidden" name="id" bind:value={$form.id} />
  {/if}

  {#if data?.form?.message}
    <div class="mx-auto px-4 max-w-7xl">{data.form.message}</div>
  {/if}

  <div>
    <Label for_="title">Title</Label>
    <Input id="title" name="title" bind:value={$form.title} required />
    {#if $errors.title}<span class="mx-auto px-4 max-w-7xl">{$errors.title}</span
      >{/if}
  </div>
  ...
  <div>
    ...
    <div use:melt={$menu} class="mx-auto px-4 max-w-7xl">
    ...
    {#if $errors.type}<span class="mx-auto px-4 max-w-7xl">{$errors.type}</span
      >{/if}
  </div>
  ...
  <div>
    ...
    />
    {#if $errors.url}<span class="mx-auto px-4 max-w-7xl">{$errors.url}</span>{/if}
  </div>

  <div>
    <Label for_="tags">Tags (comma-separated)</Label>
    <Input id="tags" name="tags" bind:value={$form.tags} placeholder="tag1, tag2, tag3" />
    {#if $errors.tags}<span class="mx-auto px-4 max-w-7xl">{$errors.tags}</span
      >{/if}
  </div>
