...
async function qdrantSearch(collection: string, vector: number[], limit: number, filter?: Record<string, unknown>) {
...
async function stageCodebaseChunks(limit: number, clusterId?: number): Promise<PipelineResult> {
	...
	const chunks = await cachedStage(cacheKey, RETRIEVAL_TTL.chunks, async () => {
		...
		return points.map((p: any) => ({
			...
			tags:       Array.isArray(p.payload?.tags) ? p.payload.tags
			            : typeof p.payload?.tags === 'string' ? p.payload.tags.split(',').map((t: string) => t.trim())
			            : [],
	...
	return { stage: 'codebase-chunks', data: chunks, durationMs: performance.now() - t0, source: 'qdrant' };
}

/** Stage 2: Fetch error clusters + cards for ACE analysis (Redis-memoized 2 min) */
async function stageErrorAnalysis(errorIds?: string[], clusterId?: number): Promise<PipelineResult> {
	const t0 = performance.now();
	...
	const data = await cachedStage(cacheKey, RETRIEVAL_TTL.errors, async () => {
		const [clusters, cards] = await Promise.all([
			...
			qdrantScroll('phase90_error_cards', 100,
				errorIds?.length
...
async function stageRAGSearch(
	...
	const ragData = await cachedStage(cacheKey, RETRIEVAL_TTL.rag, async () => {
		...
		const chunks = errHits.length > 0
			? rrfMerge(contentHits, errHits, limit)
			: contentHits.slice(0, limit);

		// ── RAG retrieval diagnostics ─────────────────────────────────────────
		const rrfTopSources = chunks.slice(0, 5).map((c: any) => ({
			id: c.id, path: c.path, score: c.score, rrfScore: c.rrfScore ?? null, source: c.source,
		}));

		const fusedScoreBreakdown = errHits.length > 0
			? {
...
async function stageKAGContext(caseId?: string): Promise<PipelineResult> {
	const t0 = performance.now();
	if (!caseId) {
		return { stage: 'kag-context', data: { neighbors: [], message: 'no caseId' }, durationMs: performance.now() - t0, source: 'skipped' };
	}
	try {
		...
		return { stage: 'kag-context', data: { neighbors, count: neighbors.length }, durationMs: performance.now() - t0, source: 'postgresql' };
	} catch {
		return { stage: 'kag-context', data: { neighbors: [], degraded: true }, durationMs: performance.now() - t0, source: 'error' };
	}
}
...
async function stageDAGOrder(caseId?: string): Promise<PipelineResult> {
	const t0 = performance.now();
	if (!caseId) {
		return { stage: 'dag-order', data: { order: [], message: 'no caseId' }, durationMs: performance.now() - t0, source: 'skipped' };
	}
	try {
		...
		if (!docs.length) {
			return { stage: 'dag-order', data: { order: [], docCount: 0, message: 'no documents for case' }, durationMs: performance.now() - t0, source: 'qdrant-empty' };
		}
		...
		return { stage: 'dag-order', data: { order: [], degraded: true }, durationMs: performance.now() - t0, source: 'error' };
	}
}
...
async function stageLLMSynthesis(
	...
	const topChunks = codebaseChunks
		...
		.slice(0, 10);
	if (topChunks.length) {
		contextParts.push('## Codebase Files (by PageRank)\n' + topChunks.map((c: any) =>
	...
	if (errorCards.length) {
		contextParts.push('## Active Errors\n' + errorCards.slice(0, 8).map((e: any) =>
			`- [${e.errorCode}] ${e.message} (${e.file})`
		).join('\n'));
	}

	// RAG hits
	if (ragChunks.length) {
		contextParts.push('## RAG Retrieved Chunks\n' + ragChunks.slice(0, 5).map((r: any) =>
			`- [score=${r.score?.toFixed(3)}] ${r.path}: ${r.content?.slice(0, 200)}`
		).join('\n'));
	}
	...
	try {
		...
		return {
			stage: 'llm-synthesis',
			data: { response, model: ENV.OLLAMA_CHAT_MODEL, tokenCached: true, cacheKey: cacheKey.slice(-16) },
			durationMs: performance.now() - t0,
			source: 'bifrost',
		};
	} catch (err) {
		return { stage: 'llm-synthesis', data: { response: null, error: (err as Error).message, degraded: true }, durationMs: performance.now() - t0, source: 'error' };
	}
}
...
async function stageLangExtract(
	...
	return { stage: 'lang-extract', data: results, durationMs: performance.now() - t0, source: 'bifrost' };
}

/** Stage 7: Tag enrichment — write analysis tags back to Qdrant points */
async function stageTagEnrichment(chunkIds: (string | number)[], tags: string[]): Promise<PipelineResult> {
	const t0 = performance.now();
	if (!chunkIds.length || !tags.length) {
		return { stage: 'tag-enrichment', data: { updated: 0 }, durationMs: performance.now() - t0, source: 'skipped' };
	}
	...
	try {
		...
		fetch(`${QDRANT_URL}/collections/codebase_chunks_768/index`, {
			method: 'PUT',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ field_name: 'pipeline_tags', field_schema: { type: 'keyword', lookup: true } }),
		}).catch(() => {});
		...
		return { stage: 'tag-enrichment', data: { updated: 0, degraded: true }, durationMs: performance.now() - t0, source: 'error' };
	}
}
...
async function stageObsidianExport(
	...
	if (chunks.length) {
		...
		for (const c of sorted) {
			const tagStr = (c.tags ?? []).slice(0, 5).map((t: string) => `#${t}`).join(' ');
...
export const POST: RequestHandler = async ({ request, locals, fetch: svelteKitFetch }) => {
	...
	if (action === 'fix' || action === 'analyze' || action === 'full-pipeline') {
		...
		if ((errorData.cards ?? []).length > 0) {
			stageErrorVectorEnrichment(errorData.cards).then(r => stages.push(r)).catch(() => {});
		}
	}
	...
	if (action === 'tag' || action === 'full-pipeline') {
		const chunkIds = codebaseChunks.slice(0, 50).map((c: any) => c.id).filter(Boolean);
		const inferredTags = [
			...(errorData.clusters ?? []).map((c: any) => `error:${c.label?.toLowerCase().replace(/\s+/g, '-').slice(0, 30)}`),
