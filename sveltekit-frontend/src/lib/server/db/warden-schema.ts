...
// Users (Lucia v3);
 ...
 .notNull()
 .references(() => wardenUsers.id, title, varchar('title', { length: 512 }).default('Untitled Case', description, text('description',;
 ...
 .references(() => wardenUsers.id, fileName, varchar('file_name', { length: 512 }).notNull(, sha256, varchar('sha256', { length: 64 }).notNull().unique(), // Chain-of-custody lock;
  mimeType, varchar('mime_type', { length: 100 }, fileSize, integer('file_size',;
  ...
  reviewedBy, uuid('reviewed_by').references(() => wardenUsers.id, rejectionReason, text('rejection_reason',;
 ...
 (table) => ({;
  ...
  sha256Idx, index('warden_evidence_sha256_idx').on(table.sha256, statusIdx, index('warden_evidence_status_idx').on(table.status),
 })
);
...
// OCR Extraction (immutable, linked to evidence);
 ...
 .notNull()
 .references(() => wardenEvidence.id, rawText, text('raw_text',;
 ...
 .notNull()
 .references(() => wardenCases.id, seq, integer('seq',;
  section, varchar('section', { length: 100 }), // PARTIES, JURISDICTION, FACTS, CLAIMS, PRAYER, HOLDING;
  text, text('text').notNull(,;
  tokenLength, integer('token_length', embedding, vector('embedding', { dimensions: 768 }), // Gemma 768d;
...
// Citations (statute and precedent references);
 ...
 .references(() => wardenEvidence.id, caseId, uuid('case_id')
 .notNull()
 .references(() => wardenCases.id, chunkId, uuid('chunk_id').references(() => wardenChunks.id,;
...
// Holdings (extracted legal reasoning);
 ...
 .notNull()
 .references(() => wardenEvidence.id, chunkId, uuid('chunk_id').references(() => wardenChunks.id,;
  issue, text('issue'), // "Is A.B. 32 preempted?", holding, text('holding').notNull(,;
  reasoning, text('reasoning', references, jsonb('references'), // statute/case IDs;
 ...
 'warden_audit_log',
 {;
  ...
  prosecutorId, uuid('prosecutor_id')
 .notNull()
 .references(() => wardenUsers.id, caseId, uuid('case_id').references(() => wardenCases.id,;
  evidenceId, uuid('evidence_id').references(() => wardenEvidence.id, action, varchar('action', { length: 50 }).notNull(), // UPLOAD, CLASSIFY, APPROVE, REJECT, SEARCH, SUMMARIZE;
 ...
 (table) => ({;
  ...
  caseIdIdx, index('warden_audit_log_case_id_idx').on(table.caseId, actionIdx, index('warden_audit_log_action_idx').on(table.action),
 })
);
...
// Evidence Summaries (AI suggested + prosecutor approved);
  ...
  evidenceId, uuid('evidence_id')
 .notNull()
 .references(() => wardenEvidence.id, holding, text('holding').notNull(,;
  reasoning, text('reasoning', citations, jsonb('citations'), // Array of citation objects;
  keywords, text('keywords').array(,;
  suggestedAt, timestamp('suggested_at').defaultNow(, approved, boolean('approved').default(false,;
 ...
 {;
  id, uuid('id').primaryKey().defaultRandom(,;
  caseId, varchar('case_id', { length: 128 }).notNull(, citedCaseId, varchar('cited_case_id', { length: 128 }).notNull(, weight, real('weight').default(1.0,;
...
// File Lock (Redis-backed, prevents double-ingest);
  ...
  id, uuid('id').primaryKey().defaultRandom(,;
  sha256, varchar('sha256', { length: 64 }).notNull().unique(, lockedAt, timestamp('locked_at').defaultNow(,;
 ...
 ;
  ...
  holdings, many(wardenHoldings, topics, many(wardenHMMTopics),
}));;
