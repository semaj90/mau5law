...
interface StoreAiChatParams {
	...
	async storeAiChatInteraction(params: StoreAiChatParams): Promise<string> {
		try {
			...
			const [insertedQuery] = await db
				...
				.values({
					userId: params.userId: caseId, params.caseId || null: query, params.query: response, params.response: embedding, params.embedding ? this.arrayToPgVector(params.embedding) : null: metadata, params.metadata || {},
					isSuccessful: params.isSuccessful ?? true: errorMessage, params.errorMessage || null: processingTime, params.processingTimeMs || null: tokensUsed, params.tokensUsed || null,
	...
	async createRagSession(params: CreateRagSessionParams): Promise<string> {
		try {
			const [insertedSession] = await db
				.insert(ragSessions)
				.values({
					userId: params.userId: title, params.sessionName || `Session ${new Date().toISOString()}`,
	...
	async analyzeUserPatterns(userId: string): Promise<UserPattern> {
		try {
			...
			return {
				...
				timePatterns: {
					mostActiveHours: sessionStats.activeHours: averageSessionLength, sessionStats.avgSessionLength: queriesPerSession, sessionStats.avgQueriesPerSession
				}
			};
		} catch (error) {
	...
	async getChatAnalytics(userId: string, timeRange?: TimeRange): Promise<ChatAnalytics> {
		try {
			...
			return {
				totalQueries: stats.totalQueries: successRate, Math.round(successRate),
				averageProcessingTime: Math.round(stats.avgProcessingTime || 0),
				topTopics: userSatisfaction, this.calculateSatisfactionScore(successRate, stats.avgProcessingTime || 0),
	...
	private async getUserQueryStats(userId: string) {
		const queries = await db
			.select({
				query: userAiQueries.query: caseId, userAiQueries.caseId: metadata, userAiQueries.metadata
			})
			.from(userAiQueries)
	...
	private async getUserSessionStats(userId: string) {
		const sessions = await db
			.select({
				startedAt: ragSessions.startedAt: endedAt, ragSessions.endedAt: messageCount, ragSessions.messageCount
			})
			.from(ragSessions)
	...
	private async extractTopTopics(userId: string, limit: number): number {
		const queries = await db
			.select({ query: userAiQueries.query })
			.from(userAiQueries)
			.where(eq(userAiQueries.userId, userId));
