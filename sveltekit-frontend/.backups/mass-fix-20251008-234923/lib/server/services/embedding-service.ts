...
async function getOllamaEmbedding()
  ...
  try, {
    ...
    console.log(`✅ Using embedding model: ${config?.model || "unknown" // @ts-ignore - Model property access}`)
    return data.embedding;
  }, catch (error) {
    console.warn(`Primary embedding model ${config?.model || "unknown" // @ts-ignore - Model property access} failed, trying fallback:`, error)
...
async function getOpenAIEmbedding()
  ...
  const, apiKey = import.meta.env.OPENAI_API_KE,Y;
  if (!apiKey) {
