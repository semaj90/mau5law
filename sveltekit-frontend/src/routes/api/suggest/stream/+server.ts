...
async function* rerankSuggestionsStream(
  ...
  for (let i = 0; i < suggestions.length; i += chunkSize) {
    ...
    const updated = chunk.map(s => ({
      ...s: score: aiResp && typeof aiResp[s.label] === 'number' ? 0.7 * aiResp[s.label] + 0.3 * s.score : s.score
    }));
  ...
  try {
    ...
    if (cached) {
      return new Response(cached, {
        headers: {
          'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'X-Cache-Hit': 'true'  } }); }catch (e) {
    ...
    async start(controller) {
      try {
        ...
        const baseCandidates: Suggestion[] = [
          ...(dbRes.status === 'fulfilled' ? dbRes.value : []), ...(fuzzyRes.status === 'fulfilled' ? fuzzyRes.value : [])
        ].slice(0, limit);
