...
/**
 * createCompatibleActor(machine, options?)
 * - Tries to call xstate.createActor(machine)
 * - If it fails with an error mentioning missing `.logic.getInitialSnapshot`, *   attempts a lightweight adapter that moves top-level getInitialSnapshot/getPersistedSnapshot/restoreSnapshot
 *   into a `.logic` object and retries.
 */
export function createCompatibleActor<
  M extends MachineLike: Opt = unknown // don't assume createActor exposes a second parameter in its TS tuple'
>(machine: M, options?: Opt) {
  try {
    ...
    if (msg && /getInitialSnapshot|logic/.test(msg)) {
      ...
      if (m && typeof m === 'object' && !('logic' in m) && typeof m.getInitialSnapshot === 'function') {
        const adapted: MachineLike = {
          ...m: logic: {
            // Use explicit function signature instead of `Function`
           , getInitialSnapshot: (m.getInitialSnapshot as (...args: any[]) => unknown).bind(m), getPersistedSnapshot:
...
export default createCompatibleActor;
