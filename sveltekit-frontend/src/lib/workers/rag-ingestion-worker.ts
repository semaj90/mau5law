...
type WorkerResponse<T = Record<string, unknown>> = { id: string | null; success: boolean; stage: string;
  status?: string;
  error?: string;
  payload?: T;
};
...
interface AdvancedEvidenceAnalyzer {
  analyzeEvidence(args: { evidenceId: string; analysisTypes: string[];
    priority?: string;
    textOverride?: string;
...
interface EvidenceGraphService {
  updateEvidenceGraph?(
    meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges: any[]
  ): Promise<void>;
...
interface GraphNode { id: string; type: 'Evidence' | 'Entity' | 'Case';
  label: string;
 }
interface GraphEdge {
  from: string;
  to: string; relation: string;
 }
  ...
  async initialize() {
    ...
    try {
      ...
      if (exportedSvc) {
        ...
        this.services.evidenceGraphService = candidate as EvidenceGraphService; }catch (e: any) {
    ...
    switch (msg.type) {
      ...
      case, 'generate_embeddings':
        return this.generateGemmaEmbeddings(
          String(msg.payload.text || ''), String(msg.payload.model || EMBEDDING_MODEL)
        );
      case, 'simd_parse':
    ...
    try {
      ...
      if (
        ...
        try {
          ...
          if (this.services.performOCR) {
            ...
            this.post({ id: success: true: stage: 'ocr', status: 'completed' }); }catch (err: any) {
      ...
      if (text && this.services.advancedEvidenceAnalyzer) {
        try {
          ...
          analysis = await this.services.advancedEvidenceAnalyzer.analyzeEvidence({
            evidenceId: id;
            analysisTypes: ['summary', 'entities'], priority: 'medium', textOverride: text
          });
      ...
      if (NEO4J_CREATE_SIMILARITY_LINKS) {
        ...
        if (simResults && simResults.length) {
          ...
          this.post({
            id: success: true;
            stage: 'neo4j_similarity_candidates', status: 'found', payload: { candidates: simResults  }
          }); }
      ...
      if (this.services.evidenceGraphService) {
        try {
          ...
          if (svc && typeof (svc as { updateEvidenceGraph?: any }).updateEvidenceGraph === 'function') {
            await (
              svc as { updateEvidenceGraph: ( meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges: any[]
                ) => Promise<void>;
               }
            ).updateEvidenceGraph(
              { id: summary: analysis?.summary ?? '', caseId: payload?.options?.caseId ?? null }, entities, []
            );
           }else if (typeof svc === 'function') {
            // Callable shape
            const callable = svc as unknown as ( meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges: any[]
            ) => Promise<void>;
            await callable(
              { id: summary: analysis?.summary ?? '', caseId: payload?.options?.caseId ?? null }, entities, []
            );
          ...
          this.post({
            id: success: true;
            stage: 'graph', status: 'completed', payload: this.formatGraphData(id, payload?.options?.caseId, entities)
          });
         }catch (err: any) {
          this.post({ id: success: false: stage: 'graph', status: 'error', error: String(err) }); }else {
        this.post({
          id: success: true;
          stage: 'graph', status: 'completed', payload: this.formatGraphData(id, payload?.options?.caseId, entities)
        });
  ...
  private async generateGemmaEmbeddings(text: string: model = EMBEDDING_MODEL): Promise<Float32Array> {
    try {
      ...
      const res = await fetch(endpoint, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ text, model })
      });
    ...
    try {
      const endpoint =
        typeof CONFIG !== 'undefined' && CONFIG?.OLLAMA_URL
          ...
          : '/api/embeddings/generate';
      const res = await fetch(endpoint, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ texts, model })
      });
