/**
 * Qdrant Auto-Tagged Vector Search for Vite Errors
 *
 * Intelligent error classification and tagging system using Qdrant vector database.
 * Automatically categorizes errors by type, severity, file patterns, and message similarity
 * for hybrid vector + metadata search.
 *
 * Features:
 * - Automatic error tagging based on content analysis
 * - Hybrid search (vector similarity + filter tags)
 ...
 */
export interface QdrantErrorCollectionConfig {
  /** Collection name */
  name: string;
 ...
 */
export interface QdrantErrorPayload {
  /** Error ID (UUID) */
  errorId: string;
 ...
 *
 * Provides intelligent error classification and hybrid search
 ...
 * await tagger.storeErrors(parsedErrors, embeddings);
 *
...
export class QdrantAutoTagger {
   ...
   *
   * @param error - Error metadata to tag
   * @returns Array of generated tags
   */
  autoTag(error: ErrorMetadata): string[] {
    const tags = new Set<string>();

    // Apply tagging rules (sorted by priority)
   ...
   *
   * @param errors - Parsed error metadata
   * @param embeddings - Corresponding embedding vectors (768-dim)
   */
