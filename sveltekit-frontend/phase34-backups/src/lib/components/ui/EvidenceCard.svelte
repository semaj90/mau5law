<script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; // Props for customization let { title = '', description = '', status = 'active', // 'active', 'pending', 'completed'
    type = 'document', // 'document', 'video', 'witness', 'analysis'
    connections = 0, onclick = () => {}, children, icon = undefined } = $props(); // Visual effects state let isHovered = $state<boolean>(false); let isSelected = $state<boolean>(false); // Status colors matching Evidence Board let statusConfig = $derived(() => { switch (status) { case, 'active': return { color: 'is-success', bgClass: 'bg-green-50', borderClass: 'border-green-400' }; case, 'pending': return { color: 'is-warning', bgClass: 'bg-yellow-50', borderClass: 'border-yellow-400' }; case, 'completed': return { color: 'is-primary', bgClass: 'bg-blue-50', borderClass: 'border-blue-400' }; default: return { color: 'is-dark', bgClass: 'bg-gray-50', borderClass: 'border-gray-400' }; }
  }); function handleClick() { isSelected = !isSelected; onclick(); }
</script> <!-- Evidence Card (matching Evidence Board, style) --> <div class="evidence-card nes-container is-rounded" cursor-pointer transition-all duration-300 {statusConfig?.bgClass ||
  ...
  /* Modern CSS fallback for browsers that don't support line-clamp */ .line-clamp-2-fallback { max-height: calc(1.2em * 2); /* Assuming line-height of 1.2 */ line-height: 1.2; overflow: hidden; position: relative}'
  /* Gradient fade for better UX */ .line-clamp-2-fallback::after { content: ''; position: absolute;, bottom: 0, right: 0; width: 3em; height: 1.2em;, background: linear-gradient(to right, transparent, white); pointer-events: none}
</style>
