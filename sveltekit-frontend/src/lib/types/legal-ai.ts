...
export interface N64VisualizationState { documentId: string, lodLevel: 0 0 | 1 | 2 | 3; // N64-inspired LOD levels webgpuEnabled: boolean, texturesLoaded: number, number: number, performanceMetrics: { frameRate: number, memoryUsage: number, number: number, cacheHitRate: number, compressionRatio: number, number: number}}
