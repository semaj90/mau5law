 ...
 * - Multi-vector search (query multiple errors at once)
 * - Error pattern detection
 * - Automatic fix suggestion matching
 *
...
export interface VectorSearchParams {
  ...
  queryVector?: number[];
  /** Error ID to find similar errors */
  errorId?: string;
  /** Maximum results */
  limit?: number;
  /** Minimum similarity threshold (0-1) */
  threshold?: number;
  /** Include Qdrant results */
  includeQdrant?: boolean;
  /** Include pgvector results */
...
export interface ClusteringParams {
  ...
  epsilon?: number;
  /** Minimum similarity for cluster membership (0-1) */
 ...
 * const similar = await searcher.findSimilar('error-uuid-123', 5);
 *
 * // Cluster all active errors
 * const clusters = await searcher.clusterErrors({ minClusterSize: 3 });
 * ```
 */
export class VectorSearchErrors {
   ...
   * @private
   */
  private async searchPgvector(queryVector: number[], params: VectorSearchParams): Promise<VectorSearchResult[]> {
    try {
      // Build WHERE conditions
      const conditions: any[] = [eq(viteErrors.isActive, true)];
  ...
  private async searchQdrant(queryVector: number[], params: VectorSearchParams): Promise<VectorSearchResult[]> {
    try {
      ...
      for (const result of qdrantResults) {
        // Fetch full error from database
        const error = await db
          .select()
          .from(viteErrors)
          .where(eq(viteErrors.id, result.payload.errorId))
          .limit(1);
  ...
  async clusterErrors(params: ClusteringParams = {}): Promise<ErrorClusterGroup[]> {
    ...
    const epsilon = params.epsilon || 0.3; // Distance threshold
    const minSimilarity = params.minSimilarity || 0.7;
    const maxClusters = params.maxClusters || 20;

    try {
      // Get all active errors with embeddings
      const errors = await db
        .select()
        .from(viteErrors)
      ...
      for (let i = 0; i < Math.min(clusters.length, maxClusters); i++) {
        ...
        const clusterErrors = clusterIndices.map((idx) => errors[idx]);

        // Calculate centroid
        const centroid = this.calculateCentroid(clusterIndices.map((idx) => vectors[idx]));
        ...
        // Generate cluster name from most common error code
        const errorCodeCounts = new Map<string, number>();
        clusterErrors.forEach((e) => {
          errorCodeCounts.set(e.errorCode, (errorCodeCounts.get(e.errorCode) || 0) + 1);
        });
   ...
   *
   * @param threshold - Minimum similarity to store
   */
  async precomputeSimilarities(threshold: number = 0.8): Promise<void> {
    console.log('🔄 Precomputing error similarities...');
  ...
  async getPrecomputedSimilar(errorId: string, limit: number = 10): Promise<VectorSearchResult[]> {
    try {
      ...
      for (const sim of similarities) {
        const otherErrorId = sim.errorId1 === errorId ? sim.errorId2 : sim.errorId1;

        const error = await db.select().from(viteErrors).where(eq(viteErrors.id, otherErrorId)).limit(1);
