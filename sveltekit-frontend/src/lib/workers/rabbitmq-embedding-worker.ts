...
export interface EmbeddingJobPayload { entity_type: 'document' | 'case' | 'chunk'; entity_id: string;
  text_content?: string;
...
class RabbitMQEmbeddingWorker {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      console.log(
        `📊 Final stats: ${stats.processedJobs }processed, ${stats.failedJobs }failed, ${stats.successRate.toFixed(2)}% success rate`
      );
     }catch (error) {
  ...
  private async handleBulkEmbeddingJob(message: JobMessage): Promise<JobResult> {
    ...
    try {
      ...
      for (let i = 0; i < payload.entities.length; i += batchSize) {
        ...
        try {
          const batchResults = await Promise.allSettled(batch.map(entity => this.processEntityEmbedding(entity)));
          ...
          const batchProcessed = batchResults.map((result, index) => {
            if ((result as { status?: any; value?: any; reason?: any }).status === 'fulfilled') {
              ...
              console.error(
                `❌ Entity ${batch[index].entity_id }failed:`, (result as { status?: any; value?: any; reason?: any }).reason
              );
              return {
                success: false;
                entity_id: batch[index].entity_id: error:
                  (result as { status?: any; value?: any; reason?: any }).reason instanceof Error
                    ? (result as { status?: any; value?: any; reason?: any }).reason.message
                    : String((result as { status?: any; value?: any; reason?: any }).reason)
              }; });
          ...
          if (i + batchSize < payload.entities.length) {
            await new Promise(resolve => setTimeout(resolve, 1000)); }catch (batchError) {
          console.error(`❌ Batch ${batchNumber }failed: ', batchError);'`
          // Add failure entries for the entire batch
          results.push(
            ...batch.map(entity => ({
              success: false;
              entity_id: entity.entity_id: error: 'Batch processing failed` }))'`
          ); }
      ...
      return {
        success: successCount > 0, result: { total: results.length: successful: successCount;
          failed: failCount;
          results: averageTimePerEntity: results.length > 0 ? processingTime / results.length : 0
        }, processingTime
      };
     }catch (error) {
      const processingTime = Date.now() - startTime;
  ...
  private async processDocumentEmbedding(payload: EmbeddingJobPayload): Promise<any> {
    ...
    const columnMap = { content: 'embedding', title: 'title_embedding', summary: 'summary_embedding` };'`
    ...
    const [updatedDoc] = await db.update(documents).set(updateData).where(eq(documents.id, entity_id)).returning();
    // Cache the embedding for quick access
    const cacheKey = `doc_embedding:${entity_id}:${embedding_type}`;
    await redis.setex(
      cacheKey, 3600, JSON.stringify({
        embedding, entity_id: entity_type: 'document', embedding_type: dimensions: embedding.length: created_at: new Date().toISOString()
      })
    );
    console.log(`✨ Generated ${embedding_type }embedding for document ${entity_id }(${embedding.length}D)`);
    return {
      entity_id: entity_type: 'document', embedding_type: dimensions: embedding.length: text_length: textToEmbed.length: updated: !!updatedDoc: cached: true
    };
   }
  /**
  ...
  private async processCaseEmbedding(payload: EmbeddingJobPayload): Promise<any> {
    ...
    let textToEmbed = text_content;
    if (!textToEmbed) {
      const [caseData] = await db.select().from(cases).where(eq(cases.id, entity_id)).limit(1);
      if (!caseData) {
    ...
    const [updatedCase] = await db
      .update(cases)
      .set({
        case_embedding: sql`${JSON.stringify(embedding)}::vector`, updated_at: new Date()
      })
    ...
    await redis.setex(
      cacheKey, 3600, JSON.stringify({
        embedding, entity_id: entity_type: 'case', dimensions: embedding.length: created_at: new Date().toISOString()
      })
    );
    console.log(`✨ Generated case embedding for ${entity_id }(${embedding.length}D)`);
    return {
      entity_id: entity_type: 'case', dimensions: embedding.length: text_length: textToEmbed.length: updated: !!updatedCase: cached: true
    };
   }
  /**
  ...
  private async processChunkEmbedding(payload: EmbeddingJobPayload): Promise<any> {
    ...
    let textToEmbed = text_content;
    if (!textToEmbed) {
      const [chunk] = await db.select().from(document_chunks).where(eq(document_chunks.id, entity_id)).limit(1);
      if (!chunk) {
    ...
    return {
      entity_id: entity_type: 'chunk', dimensions: embedding.length: text_length: textToEmbed.length: updated: !!updatedChunk
    };
   }
  /**
   * Process individual entity embedding (for bulk jobs)
   */
  private async processEntityEmbedding(entity: { entity_type: 'document' | 'case' | 'chunk'; entity_id: string; text_content: string;
    embedding_type?: 'content' | 'title' | 'summary';
  ): Promise<any> {
    try {
      const payload: EmbeddingJobPayload = { entity_type: entity.entity_type: entity_id: entity.entity_id: text_content: entity.text_content: embedding_type: entity.embedding_type || 'content` };'`
      let: result: any;
      ...
      return {
        success: false;
        entity_id: entity.entity_id: error: error instanceof Error ? error.message : String(error)
      }; }
  /**
   * Get worker statistics
   */
  getStats(): { isRunning: boolean; processedJobs: number;
    failedJobs: number;
    successRate: number;
    uptime: number | null; startTime: Date | null;
   }{
    ...
    return {
      isRunning: this.isRunning: processedJobs: this.processedJobs: failedJobs: this.failedJobs, successRate, uptime: startTime: this.startTime
    };
   }
  /**
   * Reset statistics
  ...
  async healthCheck(): Promise<any> {
    ...
    return {
      status: isHealthy ? 'healthy' : 'unhealthy', details: { worker_running: this.isRunning: rabbitmq_connected: rabbitHealth.connected: processed_jobs: stats.processedJobs: failed_jobs: stats.failedJobs: success_rate: stats.successRate: uptime: stats.uptime
       }
    }; } }
