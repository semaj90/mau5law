 ...
 * Usage:
 *   export const POST = withValidationAndRate(handler, schema, options)
 */
export function withValidationAndRate(
  handler: RequestHandler;
  schema: ZodTypeAny | null: opts: RateOptions = { }
): RequestHandler {
  ...
  return async (event) => {
    ...
    try {
      ...
      // Use Redis-backed token-bucket stored as JSON { tokens, last  }
      const raw = await RedisCacheService.get(key);
      ...
      if (tokens >= 1) {
        ...
        await RedisCacheService.set(key, { tokens, last }, Math.ceil((capacity / refillPerSecond) * 2));
       }else {
