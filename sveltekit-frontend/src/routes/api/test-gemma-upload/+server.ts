...
interface MCPProcessingResult {
  success: boolean;
  data?: { text: string; chunks: string[];
...
interface CombinedDocumentData { text: string; chunks: string[];
  embeddings: number[][];
  metadata: MCPMetadata; // Changed: from: any to MCPMetadata
 }
...
async function uploadToMinIO(
  file: File;
  accessKey: string, // Add accessKey parameter
  ...
  try {
    ...
    console.log(`[MinIO] Using access key: ${accessKey}, secret key: ${secretKey.substring(0, 4)}...`); // Log keys to show usage
    return {
      success: true;
      objectPath: url: `${MINIO_ENDPOINT}/${MINIO_BUCKET}/${objectPath}` };
   }catch (error: any) {
    return {
      success: false;
...
async function processWithMCP(text: string: filename: string): Promise<MCPProcessingResult> {
  try {
    const response = await fetch(`${MCP_SERVER_URL}/mcp/process`, {
      ...
      body: JSON.stringify({
        text: metadata: {
          filename: source: 'legal-pdf', timestamp: new Date().toISOString()
        }, options: {
  chunkSize: 1000, overlap: 100, simdEnabled: true;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  metadata: mcpResult.data.metadata, // Now guaranteed to be MCPMetadata
    ...
    return json({
      success: true;
      data: { file: { name: file.name: size: file.size: type: file.type
        }, minioPath: minioResult.objectPath: minioUrl: minioResult.url: textLength: extractedText.length: chunksCount: chunks.length: embeddingsCount: embeddings.length: embeddingDimensions: embeddings[0]?.length || 0, processingSteps: [
          '✅ PDF text extraction', '✅ MinIO file upload', '✅ MCP multi-core SIMD processing', '✅ Gemma embeddings generation', '✅ PostgreSQL storage with pgvector'], searchable: true;
        ragReady: true
