...
export async function exportCases(
  cases: Case[],
  options: ExportOptions = { format: 'json', includeMetadata: true, includeFiles: false }
  ...
  try {
    ...
    const exportData = {
      metadata: options.includeMetadata
        ...
        : undefined,
      cases: processedData.map((c: Case) => {
        const { internalNotes, systemMetadata, ...safeCase } = c;
        return safeCase;
      }),
    };
...
export async function exportEvidence(
  ...
  options: ExportOptions = { format: 'json', includeMetadata: true, includeFiles: true }
): Promise<ExportResult> {
  try {
...
export async function importCases(
  ...
  try {
    ...
    const casesArray = (data as { cases?: Case[] }).cases || (data as Case[]);
    ...
    logSecurityEvent({
      type: 'data_import',
      details: { action: 'import_cases', imported, skipped, errorCount: errors.length },
      severity: 'medium',
    });
    ...
    return {
      ...
      errors,
      warnings,
      summary: { total: imported + skipped, successful: imported, failed: skipped },
    };
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : String(error);
    return {
...
async function generatePDF(data: Record<string, unknown>[]): Promise<Blob> {
  ...
  const pdfContent = `Detective Mode Case Report
...
${data
    .map(
      (c) =>
        `Case: ${(c as Case).title}
Status: ${(c as Case).status}
Priority: ${(c as Case).priority}
Created: ${new Date((c as Case)?.createdAt ?? 0).toLocaleDateString()}
Description: ${(c as Case).description}
---`
    )
    .join('\n\n')}`;
