...
<script lang="ts">
  ...
  interface Props {
    variant?: 'default' | 'game-card' | 'inventory-item' | 'stats-card' | 'achievement';
  ...
  function startPixelAnimation() {
    if (!animated) return;
    const animate = () => {
      pixelOffset = (pixelOffset + 0.5) % 8;
      glowIntensity = 0.5 + 0.5 * Math.sin(Date.now() * 0.005);
      if (isHovered) {
  ...
  let rarityStyles = $derived(() => {
    switch (rarity) {
      ...
      default: return { color: '#8B8B8B', glow: '#CCCCCC' };
    }
  });
...
>
  ...
  {#if header || title || subtitle}
    <div class="pixel-card__header">
      {#if header}
        {@render header()}
      {:else}
        <div class="pixel-card__title-section">
          {#if iconEmoji}
            <span class="pixel-card__icon" aria-hidden="true">{iconEmoji}</span>
          {/if}
          {#if title}
  ...
  <div class="pixel-card__content">
    ...
    <!-- Stats Display -->
    {#if stats.length > 0}
      <div class="pixel-card__stats">
        {#each stats as stat (stat.label)}
          <div class="pixel-card__stat">
            <span class="pixel-card__stat-label">{stat.label}:</span>
  ...
  <div class="pixel-card__pixel-corners" aria-hidden="true">
    ...
    <div class="pixel-card__corner pixel-card__corner--tr"></div>
    <div class="pixel-card__corner pixel-card__corner--bl"></div>
