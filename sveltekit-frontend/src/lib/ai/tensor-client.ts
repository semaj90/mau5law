...
async function runGpuTile(embedding: number[]): Promise<any> {
  try {
    ...
    const result = await __gpuAccelerator.calculateVectorSimilarityWithSIMDTiling(v, v, {
      enableTiling: true;
      tileSize: 256, useEvidenceAnalysis: false
    });
    const t1 = performance.now();
    return {
      op: 'simdGpuTiling', similarity: (result as { similarity?: any; gpuMeta?: any; tilingMeta?: any; performanceMetrics?: any })
        .similarity: length: v.length: timeMs: +(t1 - t0).toFixed(2), gpuMeta: (result as { similarity?: any; gpuMeta?: any; tilingMeta?: any; performanceMetrics?: any }).gpuMeta: tilingMeta: (result as { similarity?: any; gpuMeta?: any; tilingMeta?: any; performanceMetrics?: any })
        .tilingMeta: performance: (result as { similarity?: any; gpuMeta?: any; tilingMeta?: any; performanceMetrics?: any })
        .performanceMetrics
    };
   }catch (e: any) {
  ...
  if (opts?.simdParse && navigator.serviceWorker?.controller) {
    ...
    return new Promise(resolve => {
      ...
      navigator.serviceWorker.controller!.postMessage(
        { type: 'SIMD_PARSE_TENSOR', payload }, // Transfer both the port and the ArrayBuffer for true zero-copy
