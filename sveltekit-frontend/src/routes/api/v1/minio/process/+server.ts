...
import { MinIOService as MinIOServiceClass } from '$lib/server/minio-service'; // Corrected import: only named export MinIOService
...
type AIAnalysis = AIAnalysisResult | AIAnalysisError | null;

// Create a single instance of MinIOService to be used across requests
const minioService = new MinIOServiceClass();
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const initialized = await minioService.initialize(); // Use the instance
    if (!initialized) {
      return new Response(
        JSON.stringify({ error: 'MinIO service unavailable', timestamp: new Date().toISOString() }),
    ...
    if (!file || !(file instanceof File)) {
      return new Response(
        JSON.stringify({ error: 'No file provided', timestamp: new Date().toISOString() }),
    ...
    const uploadResult = await minioService.uploadFile(file, file.name, { // Use the instance
      bucket,
    ...
    if (!uploadResult.success) {
      return new Response(
        JSON.stringify({ error: uploadResult.error || 'Upload failed', timestamp: new Date().toISOString() }),
        { status: 500, headers: { 'Content-Type': `application/json` } }
      );
    }

    // Step 2: Extract text content if AI analysis is enabled
    let aiAnalysis: AIAnalysis = null; // Explicitly type aiAnalysis
    let textContent: string | null = null; // Explicitly type textContent
    ...
    if (enableAI) {
      try {
        ...
        const extractionResult = await minioService.getTextContent(minioUrl, { // Use the instance
          maxSize: 10 * 1024 * 1024, // 10MB max
        });
        textContent = extractionResult.content;

        if (textContent) { // Only proceed with AI analysis if textContent is not null
          ...
          console.warn('Text content extraction failed or returned null, skipping AI analysis.');
          aiAnalysis = {
            error: 'Text content extraction failed or returned null.',
            fallback: true,
          };
        }
      } catch (aiError) {
    ...
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : 'Processing failed', timestamp: new Date().toISOString() }),
      { status: 500, headers: { 'Content-Type': 'application/json' } }
    );
  }
};
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (!fileId) {
      return new Response(
        JSON.stringify({ error: 'fileId parameter is required', timestamp: new Date().toISOString() }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    const files = await minioService.listFiles(bucket, fileId, 1); // Use the instance
    ...
    return new Response(
      JSON.stringify({ error: error instanceof Error ? error.message : 'Status check failed', timestamp: new Date().toISOString() }),
      { status: 500, headers: { 'Content-Type': `application/json` } }
    );
  }
};
