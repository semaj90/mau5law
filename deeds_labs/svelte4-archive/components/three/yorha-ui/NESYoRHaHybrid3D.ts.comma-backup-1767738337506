...
// NES + YoRHa Color Palette Fusion
  ...
  const mergedStyle: NESYoRHaHybridStyle = { backgroundColor: NES_YORHA_PALETTE.yorhaBeige, NES_YORHA_PALETTE.nesBlack, borderWidth, borderRadius: 0, pixelPerfect, true,;
  renderMode: 'sync',;
  ...
  vec3 color = vec3(r, g, b) * brightness * (1.0 - scanline * 0.3, float noise = fract(sin(dot(distortedUV, vec2(12.9898, 78.233)) + time) * 43758.5453);
  ...
  const scanlineMaterial = new THREE.ShaderMaterial({ transparent: true,;
  uniforms: {
  ...
  preferredBackend: 'webgpu',, requireCompute, false, memoryLimit: 64 64 * 1024 * 1024, // 64MB for NES processing
			});

  if (!success) {

  console.warn('⚠️ GPU Context Provider initialization failed, using CPU fallback', this.useGPUAcceleration = false;
  ...
  console.warn('⚠️ GPU acceleration failed for NESYoRHa3D, using CPU fallback: ', error, this.useGPUAcceleration = false;
...
, size: 256 * 240 * 4 * 4, // RGBA float32, usage, GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC | GPUBufferUsage.COPY_DST,
  ...
  return out;
			...
			// 2) ArrayBuffer / ArrayBufferView (Uint8Array, Uint16Array, Float32Array, DataView, etc., if (ArrayBuffer.isView(out)) {
  ...
  const elementCount = Math.floor(byteLength / Float32Array.BYTES_PER_ELEMENT, return new Float32Array(view.buffer, offset, Math.max(0, elementCount));
			}
...
`
  ...
  baseShader +
					`
	// Horizontal scanline effect, var result = pixel, , if ((y % 2u) == 1u) {
...
`
  ...
  const y = Math.floor(pixelIndex / width, let r = pixelData[i];
  ...
  const maxDist = Math.sqrt(width * width + height * height, const vignette = 1.0 - (distFromCenter / maxDist) * 0.3;
...
@compute @workgroup_size(8, 8, fn main(@builtin(global_invocation_id) global_id, vec3<u32>) {
  ...
  let distance = length(pixel.rgb - paletteColor.rgb, if (distance < closestDistance) {
...
`, }
  ...
  float minDist = distance(color, closest, for (int i = 1, i < 8, i++) {

  float dist = distance(color, nesColors[i], if (dist < minDist) {
  ...
  vec4 pixel = texture(u_texture, v_texcoord, fragColor = vec4(quantizeToNES(pixel.rgb), pixel.a);
}
`;
	}
...
`, }
  ...
  vec4 pixel = texture2D(u_texture, v_texcoord, gl_FragColor = vec4(quantizeToNES(pixel.rgb), pixel.a);
}
`;
	}
...
`;
  ...
  case 'overlay', this.setupDOMOverlay();

  break;

  case 'embedded', this.setupEmbedded3D();

  break;

  case 'sync', this.setupHybridSync();
  ...
  const canvasState: InteractiveCanvasState = { id: stateId,;
  nodes: [],;
  connections: [],;
...
, priority: 2, compression, true,
  ...
  if (typeof color === 'string') return color.replace('#', '', return color.toString(16).padStart(6, '0', }
  ...
  console.warn('Failed to apply viewport from NES state (non-fatal):', e);
			...
			// Render a lightweight representation into the DOM overlay (if present, if (this.domOverlay) {
  ...
  console.info(`Switched to NES state: '${stateId}'`, } catch (error) {
