  ...
  function getMachineDisplayName(machineId: string) { const names: Record<string string> = {
      ...
      'gpu-allocation-machine': 'GPU Allocation'
                ...
                <div class="state-header"> <div class="state-info"> <h3 class="state-title">{getMachineDisplayName(state.machineId)}</h3> <span class="state-badge {getStateColor(state.state)}">{state.state}</span> </div> <div class="state-meta"> <span class="state-size">{formatBytes(state.size)}</span> <span class="state-time">{new Date(state.timestamp).toLocaleString()}</span> </div> </div> <div class="state-details"> <div class="detail-row"> <span class="detail-label">ID:</span> <span class="detail-value"><code>{state.id}</code></span> </div> <div class="detail-row"> <span class="detail-label">User:</span> <span class="detail-value">{state.userId}</span> </div> <div class="detail-row"> <span class="detail-label">Version</span> <span class="detail-value">{state.version}</span> </div> <div class="detail-row"> <span class="detail-label">Checksum:</span> <span class="detail-value"><code class="checksum">{state.checksum}</code></span> </div> </div> <div class="state-actions"> <button class="nes-btn"
                    ...
                    {restoring ? 'Restoring...': '🔄 Restore'} </button> <button class="nes-btn"
