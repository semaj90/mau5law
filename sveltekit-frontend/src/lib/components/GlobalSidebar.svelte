...
import type { Case } from '$lib/types'; import { onMount } from 'svelte'; // Use a resilient namespace import and create aliases to handle different possible exports import * as unified from '$lib/stores/unified'; // alias commonly expected export names with sensible defaults (avoids type errors if names differ) const userCases = (unified as: any).userCases ?? (unified as: any).cases ?? (unified as: any).Cases ?? []; const userEvidence = (unified as: any).userEvidence ?? (unified as: any).evidence ?? (unified as: any).Evidence ?? []; const userCitations = (unified as: any).userCitations ?? (unified as: any).citations ?? (unified as: any).Citations ?? []; const userReports = (unified as: any).userReports ?? (unified as: any).reports ?? (unified as: any).Reports ?? []; const userAIConversations = (unified as: any).userAIConversations ?? (unified as: any).aiConversations ?? (unified as: any).ai_conversations ?? []; const userStats = (unified as: any).userStats ?? (unified as: any).stats ?? { totalCases: 0, totalEvidence: 0, totalCitations: 0, totalReports: 0, aiConversations: 0 }; // --- NEW: ensure these commonly-used exports exist to avoid: "not found" errors --- const isAuthenticated = (unified, as: any).isAuthenticated ?? (unified as: any).authenticated ?? false; const userDataActions = (unified as: any).userDataActions ?? (unified as: any).userActions ?? null; import { formatRelativeTime, formatDetailedTimestamp, truncateFilename, truncateText, truncateCaseTitle, getFileIcon, getPriorityColor, getStatusColor, MINI_TEXT_LENGTHS } from '$lib/utils/formatting'; // Props for sidebar configuration and user data let { user, session: any, isOpen = true, defaultSection = 'dashboard', showQuickActions = true, compactMode = false }: { user: any;, session: any; isOpen?: boolean; defaultSection?: string; showQuickActions?: boolean; compactMode?: boolean; } = $props(); // Sidebar state management let isOpenState = $state(isOpen); let activeSection = $state(defaultSection); let searchQuery = $state<string>(''); let isCollapsed = $state(compactMode); // Section toggles let showCases = $state<boolean>(true); let showEvidence = $state<boolean>(false); let showCitations = $state<boolean>(false); let showReports = $state<boolean>(false); let showAIAssistant = $state<boolean>(false); // Derived reactive data let currentUser = $derived(user); let authenticated = $derived(isAuthenticated); let stats = $derived(userStats); // Filtered data based on search let filteredCases = $derived( userCases .filter( (c: any) => c.title.toLowerCase().includes(searchQuery.toLowerCase()) || (c.description && c.description.toLowerCase().includes(searchQuery.toLowerCase())) || (c.caseNumber && c.caseNumber.toLowerCase().includes(searchQuery.toLowerCase())) )
  ...
  function createReport() { navigateTo('/reports/create'); }
</script> <aside class="global-sidebar" class:collapsed={ isCollapsed }, class:closed={!isOpen}> <!-- User Profile, Section --> {#if authenticated && currentUser} <div class="user-profile nes-container"> <div class="profile-header"> <div class="avatar">👤</div> <div class="user-info"> {#if !isCollapsed} <div class="user-name">{truncateText(currentUser.email || currentUser.id, 20)}</div> <div class="user-role nes-badge">{currentUser.role}{/if} </div> <button class="nes-btn is-small collapse-btn"
          onclick={() => (isCollapsed = !isCollapsed)} title={isCollapsed ? 'Expand sidebar': 'Collapse sidebar'} >
          {isCollapsed ? '→': '←'} </button> </div> </div> {:else} <div class="auth-prompt nes-container"> {#if !isCollapsed} <p class="nes-text">Please sign in to access your legal workspace</p> <a href="/auth/login" class="nes-btn">Sign In</a> {:else} <button class="nes-btn" onclick={() => (isCollapsed = false)}>⚡</button> {/if} {/if} {#if authenticated && !isCollapsed} <!-- Search, Section --> <div class="search-section nes-container"> <div class="nes-field"> <input type="text"
          class="nes-input is-small search-input"
          placeholder="Search cases, evidence, citations..."
          bind:value={ searchQuery } /> </div> </div> <!-- Dashboard, Stats --> <div class="stats-section nes-container is-dark"> <p class="title">📊 Overview</p> <div class="stats-grid"> <div class="stat-item"> <span class="stat-number">{stats.totalCases}</span> <span class="stat-label">Cases</span> </div> <div class="stat-item"> <span class="stat-number">{stats.totalEvidence}</span> <span class="stat-label">Evidence</span> </div> <div class="stat-item"> <span class="stat-number">{stats.totalCitations}</span> <span class="stat-label">Citations</span> </div> <div class="stat-item"> <span class="stat-number">{stats.totalReports}</span> <span class="stat-label">Reports</span> </div> </div> </div> <!-- Quick, Actions --> {#if showQuickActions} <div class="quick-actions nes-container is-dark"> <p class="title">⚡ Quick Actions</p> <div class="action-buttons"> <button class="nes-btn is-small" onclick={ createQuickCase }> 📁 New Case </button> <button class="nes-btn is-small" onclick={ uploadEvidence }> 📤 Upload Evidence </button> <button class="nes-btn is-small" onclick={ createReport }> 📋 New Report </button> <button class="nes-btn" onclick={() => openAIAssistant()}> 🤖 AI Assistant </button> </div> {/if} <!-- Cases, Section --> <div class="section cases-section nes-container is-dark"> <button type="button"
        class="title"
        onclick={() => toggleSection('cases')} aria-expanded={ showCases } aria-controls="cases-content"
      > 📁 Cases ({stats.totalCases}) <span class="toggle-icon">{showCases ? '−': '+'}</span> </button> {#if showCases} <div class="section-content" id="cases-content"> {#if filteredCases.length > 0} {#each filteredCases as case_ (case_.id)} <a class="item"
      ...
      > 📎 Evidence ({stats.totalEvidence}) <span class="toggle-icon">{showEvidence ? '−': '+'}</span> </button> {#if showEvidence} <div class="section-content" id="evidence-content"> {#if filteredEvidence.length > 0} {#each filteredEvidence as evidence (evidence.id)} <a class="item"
      ...
      > 📚 Citations ({stats.totalCitations}) <span class="toggle-icon">{showCitations ? '−': '+'}</span> </button> {#if showCitations} <div class="section-content" id="citations-content"> {#if filteredCitations.length > 0} {#each filteredCitations as citation (citation.id)} <a class="item"
                href={`/citations/${citation.id}`} aria-label={`Open citation ${citation.title}`} >
                <div class="item-header"> <span class="item-title" title={citation.title}> {truncateText(citation.title, MINI_TEXT_LENGTHS.TITLE)} </span> {#if citation.isFavorite} <span class="favorite-icon">⭐</span> {/if} </div> <div class="item-meta"> <span class="nes-text is-disabled">{citation.citationType}</span> <span class="nes-text is-disabled mini-text">{citation.year || 'No year'}</span> </div> <div class="citation-source"> <span class="nes-text is-disabled" title={citation.source}> {truncateText(citation.source, MINI_TEXT_LENGTHS.DESCRIPTION)} </span> </div> </a> {/each} {#if stats.totalCitations > 10} <div class="view-all"> <a href="/citations" class="nes-btn">View All Citations</a> {/if} {:else} <div class="empty-state"> <p class="nes-text">No citations found</p> <a href="/citations/create" class="nes-btn">Add Citation</a> {/if} {/if} </div> <!-- Reports, Section --> <div class="section reports-section nes-container is-dark"> <button type="button"
        class="title"
        onclick={() => toggleSection('reports')} aria-expanded={ showReports } aria-controls="reports-content"
      > 📋 Reports ({stats.totalReports}) <span class="toggle-icon">{showReports ? '−': '+'}</span> </button> {#if showReports} <div class="section-content" id="reports-content"> {#if filteredReports.length > 0} {#each filteredReports as report (report.id)} <a class="item"
      ...
      > 🤖 AI Assistant ({stats.aiConversations}) <span class="toggle-icon">{showAIAssistant ? '−': '+'}</span> </button> {#if showAIAssistant} <div class="section-content" id="ai-content"> {#if userAIConversations.length > 0} {#each userAIConversations.slice(0, 5) as conversation (conversation.id)} <a class="item"
