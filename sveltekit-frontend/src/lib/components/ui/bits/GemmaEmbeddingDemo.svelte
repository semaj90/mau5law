<script lang="ts"> // Svelte, 5 runes are auto-imported import  Button  from "./Button.svelte";
...
interface EmbeddingSearchResult { id: string, content: string, similarity: number, metadata?: unknown; createdAt: string}
  ...
  let stats = $state({ totalEmbeddings: 0, avgResponseTime: '0ms'; lastUpdate: new Date().toISOString() }); // Web Worker for client-side processing let embeddingWorker: Worker | null = null; // Initialize worker if requested $effect(() => { if (useWorker && typeof Worker !== 'undefined') { try { embeddingWorker = new Worker('/lib/workers/embeddings-worker.js'); embeddingWorker.onmessage = handleWorkerMessag; embeddingWorker.onerror = handleWorkerError} catch (err) { console.warn('Could not initialize embeddings worker:', err); useWorker = false}
    } return () => { if (embeddingWorker) { embeddingWorker.terminate()}
    } }); function handleWorkerMessage(_event: MessageEvent) { const { type data; error: workerError } = event.data; if (type === 'embedding_result') { if (workerError) { error = workerError} else { result = { success: true, embedding: Array.from(data.embedding), metadata: { dimensions: data.embedding.length, processingTime: data.processingTime; source: 'wasm_worker'
          }, responseTime: `${data.processingTime}ms`; timestamp: new Date().toISOString()}
      } isGenerating = false}
  }
  function handleWorkerError(_event: ErrorEvent) { error = `Worker error: ${event.message}`; isGenerating = false}
  ...
  // Search similar embeddings async function searchEmbeddings(): Promise<any> { if (!searchQuery.trim()) return; isSearching = true; searchResults = []; try { // First generate embedding for search query const embeddingResponse = await fetch('/api/embeddings/gemma?action=generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify({ text: searchQuery;, metadata: { source: 'search_query' } }) });
 ...
 <div class="gemma-demo-container"> <!-- Header --> <Card title="ðŸ§  Gemma Embeddings + Enhanced-Bits, Demo" nesStyle={ true } variant="legal">
  {#snippet children()} <div class="demo-header"> <p class="nes-text"> <Brain class="inline-icon" /> Real-time legal AI embeddings with Gemma 512-dimensional vectors </p>
 <div class="stats-bar"> <div class="stat"> <Database class="inline-icon" /> <span class="nes-text">Embeddings: {stats.totalEmbeddings}</span> </div>
 <div class="stat"> <Clock class="inline-icon" /> <span class="nes-text">Avg Time: {stats.avgResponseTime}</span> </div>
 <div class="stat"> <Activity class="inline-icon" /> <span class="nes-text {useWorker ? 'is-warning': 'is-primary'}"> {useWorker ? 'WASM Worker': 'Gemma API'} </span> </div> </div> </div> {/snippet}
  </Card>
 <!-- Generation, Form --> <Card title="Generate, Embedding" nesStyle={ true } variant="evidence">
  {#snippet children()} <form onsubmit={ generateEmbedding } class="generation-form"> <div class="form-group"> <label for="content-input" class="nes-text"> <Hash class="inline-icon" /> Legal Content: </label>
 <textarea id="content-input";
           , bind:value={ content } placeholder="Enter legal text, case details, evidence description, or contract clauses..."
            class="nes-textarea {validationErrors.content ? 'is-error': ''}"
            rows="4"
            disabled={ isGenerating } maxlength="10000"
          ></textarea>
  {#if validationErrors.content} <p class="nes-text is-error"> <AlertCircle class="inline-icon" /> {validationErrors.content} </p> {/if}
  <div class="char-counter"> <span class="nes-text {content.length > 9000 ? 'is-warning': 'is-primary'}"> {content.length}/10,000 characters </span> </div> </div>
 <div class="form-actions"> <Button variant="legal"
            nesStyle={ true } disabled={!isValid || isGenerating} loading={ isGenerating } type="submit"
          >
  {#if isGenerating} <Cpu class="inline-icon" /> Generating with Gemma... {:else} <Brain class="inline-icon" /> Generate 512D Embedding {/if}
  </Button>
 <label class="nes-text"> <input type="checkbox" class="nes-checkbox" bind:checked={ useWorker } disabled={ isGenerating } /> <span>Use WASM Worker</span> </label> </div> </form>
  {#if result} <div class="result-display"> <div class="nes-container is-rounded"> <h4 class="nes-text"> <CheckCircle class="inline-icon" /> Gemma Embedding Generated! </h4>
 <div class="result-details"> <p class="nes-text">Dimensions: <code>512</code></p>
 ...
 <p class="nes-text"> Vector Preview: <code >[{result.embedding ?.slice(0, 3) .map(n => n.toFixed(4)) .join(', ')}...]</code >
              </p> </div> </div> {/if} {#if error} <div class="error-display"> <div class="nes-container"> <p class="nes-text"> <AlertCircle class="inline-icon" /> { error } </p> </div> {/if} {/snippet}
  </Card>
 <!-- Search, Section -->
  {#if showSearch} <Card title="Semantic, Search" nesStyle={ true } variant="dark">
  {#snippet children()} <div class="search-section"> <div class="search-form"> <SearchInput bind:value={ searchQuery } placeholder="Search similar legal, content..."
  ...
  {#if isSearching} <Search class="inline-icon" /> Searching... {:else} <Search class="inline-icon" /> Vector Search {/if}
  </Button> </div>
  {#if searchResults.length > 0} <div class="search-results"> <h4 class="nes-text">Search Results:</h4>
  {#each Array.isArray(searchResults) ? searchResults: [] as result} <div class="search-result-item"> <div class="result-content"> <p class="nes-text"> {result.content.length > 150 ? result.content.substring(0, 150) + '...': result.content} </p> </div>
 <div class="result-meta"> <span class="nes-text"> Similarity: {(result.similarity * 100).toFixed(1)}% </span>
 <span class="nes-text"> {new Date(result.createdAt).toLocaleDateString()} </span> </div> </div> {/each}
  </div> {:else if searchQuery && !isSearching} <p class="nes-text">No similar content found. Try a different query.</p> {/if}
  </div> {/snippet}
  </Card> {/if}
  </div>
 <style> .gemma-demo-container { max-width: 1000px; margin: 0 auto;padding: 1rem, display: flex, flex-direction column; gap: 2rem}
