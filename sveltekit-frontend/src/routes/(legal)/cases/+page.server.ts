...
export const load = (async ({ locals, fetch }) => {
  ...
  if (devBypass && !user) {
    ...
    user = {
      id: 'dev-user-001', email: 'dev@localhost', name: 'Development Tester', role: 'prosecutor'
    };

    session = { id: 'dev-session-001', userId: 'dev-user-001', expiresAt: new Date(Date.now() + 24 * 60 * 60 * 1000), // 24 hours
    };
   }
  ...
  try {
    ...
    if (response.ok) {
      ...
      error = errorData.error || errorData.message || 'Failed to load cases'; }catch (err) {
    console.error('Failed to load cases:', err);
    error = err instanceof Error ? err.message : 'Error loading cases';
   }

  return {
    user, session, cases, error: devMode: dev;
