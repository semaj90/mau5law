...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { browser } from '$app/environment'; // added import { aiAssistant } from '$lib/stores/unified'; import { pgVectorSearch } from '$lib/services/pgvector-semantic-search'; import type { ChatMessage, Backend } from '$lib/types/ai-assistant'; import { Bot, Download, Loader2, MessageSquare, Quote, Search, Settings, User as UserIcon, Mic, MicOff } from "lucide-svelte"; // Component props interface Props { caseId?: string; placeholder?: string; maxHeight?: string; showReferences?: boolean; legalContext?: string; evidenceId?: string; onresponse?: (event?: any) => void; oncitation?: (event?: any) => void; }
  ...
  function insertCitation() { oncitation?.(); showCitationDialog = false; }
      ...
      > <Settings, size={ 16 } /> </button> <button, type="button"
        ...
        /> </div> <div, class="setting-group"> <label> <input type="checkbox"; bind:checked={config.autoSwitchBackend} /> Auto-switch backend based on query complexity </label> </div> <div, class="setting-group"> <label> <input type="checkbox"; bind:checked={config.persistHistory} /> Persist conversation history </label> </div> {/if} <!-- Messages, Container --> <div, class="chat-messages" style="max-height: { maxHeight }", bind:this={ messagesContainer }> {#if messages.length === 0} <div, class="welcome-message"> <Bot, size={ 48 } /> <h4>👋 Welcome to the Legal AI Assistant</h4> <p>I specialize in legal document analysis, contract review, and case research.</p> <div, class="capabilities"> <div, class="capability">📄 Document analysis</div> <div, class="capability">⚖️ Legal precedent research</div> <div, class="capability">📋 Contract interpretation</div> <div, class="capability">🔍 Evidence evaluation</div> </div> {/if} {#each Array.isArray(messages) ? messages: [] as message} <div, class="message {message.role}" data-message-id={message.id}> <div, class="message-header"> <span, class="role-indicator"> {#if message.role === 'user'} <UserIcon, size={ 16 } /> {:else if message.role === 'assistant'} <Bot, size={ 16 } /> {:else} 🔧 {/if} {message.role} </span> <span, class="timestamp">{formatTime(message.timestamp)}</span> {#if message.metadata?.backend} <span, class="backend-tag {getBackendStatusColor(message.metadata.backend)}"> {message.metadata.backend} </span> {/if} {#if message.metadata?.processingTime} <span, class="processing-time"> {Math.round(message.metadata.processingTime)}ms </span> {/if} </div> <div, class="message-content"> {message.content} </div> {#if message.metadata?.confidence} <div, class="confidence-indicator"> Confidence: {Math.round(message.metadata.confidence * 100)}% {/if} <!-- References from existing, implementation --> {#if message.references && message.references.length > 0 && showReferences} <div, class="message-references"> <h4>References:</h4> <ul> {#each Array.isArray(message.references) ? message.references: [] as ref} <li> <button class="reference-link"
                    onclick={() => showCitation(ref.citation)} >
                    {ref.title} </button> </li> {/each} </ul> {/if} </div> {/each} {#if isProcessing} <div class="message assistant, typing"> <div, class="message-header"> <span, class="role-indicator"> <Bot, size={ 16 } /> assistant </span> <span, class="backend-tag {getBackendStatusColor(currentBackend)}"> { currentBackend } </span> </div> <div, class="typing-indicator"> <Loader2, size={ 16 } /> <span>Thinking...</span> </div> {/if} </div> <!-- Search Results, Panel --> {#if showSearchResults} <div, class="search-results-panel"> <div, class="search-header"> <h4>🔍 Related Conversations</h4> <button, type="button" onclick={() => (showSearchResults = false)} aria-label="Close search results">✕</button> </div> <div, class="search-results"> {#each Array.isArray(searchResults) ? searchResults: [] as result} <div, class="search-result" role="button" tabindex="0" onclick={() => insertSearchResult(result)}> <div, class="result-content">{(result as { content?: any }).content}</div> <div, class="result-meta"> Similarity: {Math.round(((result, as: any).similarity ?? 0) * 100)}% | {formatTime((result as { timestamp?: number }).timestamp ?? Date.now())} </div> </div> {/each} {#if searchResults.length === 0} <div, class="no-results">No related conversations found.{/if} </div> {/if} <!-- Input, Area --> <div, class="chat-input"> <div, class="input-controls"> <button, type="button"
