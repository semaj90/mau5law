<script lang="ts"> // Svelte, 5 runes are auto-imported // Svelte, 5 runes pattern let inputText = $state<string>('');
   let result: string | null = null;
   let loading = $state<boolean>(false);
   let error: string | null = null; async function analyzeWWWH(): Promise<any> { if (!inputText.trim()) return; loading = true; error = null; result = null; try { const res = await fetch('/api/ai/wwwh', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: inputText }) });
   const data = await res.json().catch(() => ({})); if (res.ok) { // fixed typo and provide defensive fallback in case backend property differs result = data.analysis ?? data.result ?? JSON.stringify(data)} else { error = data.error || 'Unknown error'}
