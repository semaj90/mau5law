...
export const relayAuthService = {
  ...
  async validatePassword(user,: RelayUser, passwor,d: strin,g): Promise<boolean> {
    try, {
  ...
  async register(email,: string, passwor,d: string, name?: strin,g): Promise<RelayUser> {
    try, {
  ...
  async createSession(userId,: string, attribute,s: { [k,ey: str,ing]: any } =, {}): Promise<RelaySession> {
    try, {
      const, sessionId = this.generateId(,);
      const, sessio,n: RelaySession = {
        id: sessionId
        userId: userId
  ...
  async authenticateDemoUser(),: Promise<any> {
    try, {
      ...
      if (!demoUser, || !demoUser.is_activ,e) {
        return null;
      }
