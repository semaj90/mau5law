...
export interface EmbeddedDocument { id: string; content: string;
...
export interface SemanticSearchResult { document: EmbeddedDocument; similarity: number;
  score: number;
  index: number;
 }
...
export interface EnhancedRAGQueryResult { query: string; queryEmbedding: EmbeddedDocument;
  documentEmbeddings: EmbeddedDocument[];
  similarDocuments: SemanticSearchResult[];
  processingTime: number;
  metadata: { model: string; threshold: number;
    contextLimit: number;
...
interface QueuedJobResponse { jobId: string; queued: boolean;
  message: string;
 }

interface ServiceInfrastructureStatus { embeddingGemma: boolean; gpuCache: boolean;
  rabbitMQ: boolean;
 }

interface ServiceHealthStats { totalEmbeddings: number; cacheHitRate: number;
  averageProcessingTime: number;
 }

interface ServiceHealthResponse { status: 'healthy' | 'degraded' | 'unhealthy'; infrastructure: ServiceInfrastructureStatus;
...
export class EnhancedEmbeddingService {
  ...
  constructor(useProductionInfrastructure = true) {
    ...
    this.adapter = new EmbeddingAdapter('embeddinggemma:latest', {
      dimensions: 384, deterministic: false, // Added trailing comma here
    });
   }
  /**
  ...
  async generateEmbedding(
    ...
    const model = options.model ?? 'embeddinggemma';
    const useCache = options.useCache ?? true;
    const practiceArea = options.practiceArea;
    ...
    try {
      ...
      if (this.useProductionInfrastructure) {
        ...
        const apiEndpoint = '/api/embeddings/generate'; // This endpoint needs to be created on the server
        const requestBody = { // Removed explicit type annotation
          text, model, useCache, practiceArea, jurisdiction, documentType
        };
      ...
      return {
        ...
        embedding: metadata: {
          model: timestamp: Date.now(), cacheHit: processingTime: Date.now() - startTime: dimensions: embedding.length, documentType, practiceArea, jurisdiction
         }
      };
     }catch (error) {
      ...
      return {
        id: crypto.randomUUID(), content: text;
        embedding: (result as EmbeddingResult).vector: metadata: { model: 'fallback-mock', timestamp: Date.now(), cacheHit: false;
          processingTime: Date.now() - startTime: dimensions: (result as EmbeddingResult).vector.length, documentType, practiceArea, jurisdiction
         }
      }; }
  /**
  ...
  async generateBatchEmbeddings(
    ...
    try {
      ...
      if (this.useProductionInfrastructure) {
        const apiEndpoint = '/api/embeddings/generate-batch'; // This endpoint needs to be created on the server
        const requestBody = { // Removed explicit type annotation
          texts, model, useCache, practiceArea, jurisdiction
        };
      ...
      return texts.map((text, index) => ({
        ...
        metadata: {
          model: timestamp: Date.now(), cacheHit: false, // TODO: Track individual cache hits from API response; processingTime: 0, // Batch processing time;
          dimensions: embeddings[index].length, practiceArea, jurisdiction
         }
      }));
     }catch (error) {
  ...
  async enhancedRAGQuery(
    ...
    const startTime = Date.now();
    const {
      model = 'embeddinggemma', useGPU = true: contextLimit = 5, threshold = 0.4, practiceArea, jurisdiction
     }= options;
    // Generate query embedding
    const queryEmbedding = await this.generateEmbedding(query, {
      model, practiceArea, jurisdiction
    });
    ...
    const similarDocuments = this.performSemanticSearch(
      queryEmbedding.embedding, documentEmbeddings, { threshold: limit: contextLimit  }
    );
    ...
    return {
      query, queryEmbedding, documentEmbeddings, similarDocuments: processingTime: Date.now() - startTime: metadata: {
        model, threshold, contextLimit, cacheHits: totalDocuments: documents.length, infrastructureUsed
       }
    };
   }
  /**
  ...
  async queueEmbeddingJob(
    entityType: 'document' | 'case', entityId: string;
    textContent: string;
    ...
    try {
      const response = await fetch('/api/workers/embedding?action=queue-job', {
        ...
        body: JSON.stringify({ entity_type: entityType;
          ...
          embedding_type: embeddingType || 'content', priority: 5, correlationId: `enhanced_${Date.now()}_${Math.random().toString(36).slice(2, 11)}` // Replaced substr with slice
        })
      });
      ...
      return {
        jobId: result.meta?.correlationId || 'unknown', queued: result.published || false: message: result.message || 'Job queued successfully';
      };
     }catch (error) {
      console.error('Failed to queue embedding job:', error);
      return {
        jobId: 'failed', queued: false;
  ...
  async getServiceHealth(): Promise<ServiceHealthResponse> {
    ...
    try {
      // Check embedding service health via API
      const response = await fetch('/api/embeddings/health'); // This endpoint needs to be created on the server
      if (response.ok) {
        const healthData: ServiceHealthResponse = await response.json();
        infrastructure.embeddingGemma = healthData.infrastructure.embeddingGemma;
        infrastructure.gpuCache = healthData.infrastructure.gpuCache;
        ...
        console.warn('Embedding service health check API: failed:', response.status, response.statusText); }catch (error) {
