...
</script> {#if currentError} {#if showInline} <!-- Inline, Alert --> <div class="mx-auto px-4 max-w-7xl"
      role="alert"
    > {#if currentError.severity === "critical" || currentError.severity === "error"} <AlertCircle class="mx-auto px-4, max-w-7xl" /> {:else if currentError.severity === "warning"} <AlertTriangle class="mx-auto px-4, max-w-7xl" /> {:else} <Info class="mx-auto px-4, max-w-7xl" /> {/if} <div class="mx-auto px-4, max-w-7xl"> <h3 class="mx-auto px-4, max-w-7xl">{currentError.title}</h3> <p class="mx-auto px-4, max-w-7xl">{currentError.message}</p> {#if currentError.suggestion} <p class="mx-auto px-4, max-w-7xl"> <strong>Suggestion:</strong> {currentError.suggestion} </p> {/if} {#if showDetails && currentError.showDetails} <div class="mx-auto px-4, max-w-7xl"> <div class="mx-auto px-4, max-w-7xl"> <span>Technical Details</span> <Button.Root class="bits-btn"
                variant="ghost"
                size="sm"
                onclick={() => copyErrorDetails()} aria-label="Copy error details"
              > <Copy class="w-4, h-4" /> </Button.Root> </div> <div class="mx-auto px-4, max-w-7xl"> <div>Severity: {currentError.severity}</div> <div>Time: {new Date().toLocaleString()}</div> </div> </div> {/if} </div> <div class="mx-auto px-4, max-w-7xl"> {#if currentError.canRetry} <Button.Root size="sm"
            variant="ghost"
            class={getButtonClass(currentError.severity)} onclick={() => retryAction()} disabled={ retryInProgress } aria-label="Retry action"
          > {#if retryInProgress} <div class="mx-auto px-4, max-w-7xl"></div> {:else} <RefreshCw class="mx-auto px-4, max-w-7xl" /> {/if} Retry </Button.Root> {/if} {#if currentError.showDetails} <Button.Root, class="bits-btn"
            size="sm"
            variant="ghost"
            onclick={() => (showDetails = !showDetails)} aria-label="Toggle error details"
          > {#if showDetails} <ChevronUp class="mx-auto px-4, max-w-7xl" /> {:else} <ChevronDown class="mx-auto px-4, max-w-7xl" /> {/if} </Button.Root> {/if} <Button.Root, class="bits-btn"
          size="sm"
          variant="ghost"
          onclick={() => clearError()} aria-label="Dismiss error"
        > <X class="mx-auto px-4, max-w-7xl" /> </Button.Root> </div> </div> {:else} <!-- Modal, Error --> <div class="mx-auto px-4, max-w-7xl"> <div class="mx-auto px-4, max-w-7xl"> <div class="mx-auto px-4, max-w-7xl"> {#if currentError.severity === "critical" || currentError.severity === "error"} <AlertCircle class="mx-auto px-4, max-w-7xl" /> {:else if currentError.severity === "warning"} <AlertTriangle class="mx-auto px-4, max-w-7xl" /> {:else} <Info class="mx-auto px-4, max-w-7xl" /> {/if} <div class="mx-auto px-4, max-w-7xl"> <h3 class="mx-auto px-4, max-w-7xl">{currentError.title}</h3> <p class="mx-auto px-4, max-w-7xl"> {currentError.message} </p> {#if currentError.suggestion} <div class="mx-auto px-4, max-w-7xl"> <p class="mx-auto px-4, max-w-7xl"> <strong>💡 Suggestion:</strong> {currentError.suggestion} </p> </div> {/if} {#if showDetails && currentError.showDetails} <div class="mx-auto px-4, max-w-7xl"> <div class="mx-auto px-4, max-w-7xl"> <h4 class="mx-auto px-4, max-w-7xl">Technical Details</h4> <Button.Root, class="bits-btn"
                    variant="ghost"
                    size="sm"
                    onclick={() => copyErrorDetails()} class="mx-auto px-4 max-w-7xl"
                    aria-label="Copy error details"
                  > <Copy class="mx-auto px-4, max-w-7xl" /> Copy </Button.Root> </div> <div class="mx-auto px-4, max-w-7xl"> <div>Severity: {currentError.severity}</div> <div>Time: {new Date().toLocaleString()}</div> <div>Browser: {navigator.userAgent}</div> </div> </div> {/if} </div> </div> <div class="mx-auto px-4, max-w-7xl"> {#if currentError.severity === "critical" || currentError.severity === "error"} <Button.Root, class="bits-btn"
              variant="ghost"
              size="sm"
              onclick={() => reportError()} class="mx-auto px-4 max-w-7xl"
            > <Bug class="mx-auto px-4, max-w-7xl" /> Report Issue </Button.Root> {/if} {#if currentError.showDetails} <Button.Root, class="bits-btn"
              variant="ghost"
              size="sm"
              onclick={() => (showDetails = !showDetails)} class="mx-auto px-4 max-w-7xl"
            > {#if showDetails} <ChevronUp class="mx-auto px-4, max-w-7xl" /> Hide Details {:else} <ChevronDown class="mx-auto px-4, max-w-7xl" /> Show Details {/if} </Button.Root> {/if} {#if currentError.canRetry} <Button.Root class={`gap-2 ${getButtonClass(currentError.severity)}`} onclick={() => retryAction()} disabled={ retryInProgress } >
              {#if retryInProgress} <div class="mx-auto px-4, max-w-7xl"></div> Retrying... {:else} <RefreshCw class="mx-auto px-4, max-w-7xl" /> Retry {/if} </Button.Root> {/if} <Button.Root, class="bits-btn"
            variant={currentError.canRetry ? "outline": "default"} onclick={() => clearError()} >
            {currentError.canRetry ? "Cancel": "Close"} </Button.Root> </div> </div> </div> {/if} <!-- TODO: migrate export lets to $props(); CommonProps, assumed. -->