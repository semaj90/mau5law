...
export class PgVectorFAISSBridge {
  ...
  async initialize(config: Partial<HybridSearchConfig> = {}): Promise<boolean> {
    ...
    try {
      ...
      if (faissReady && fullConfig.use_faiss_gpu) {
        // Create FAISS index
        const indexInfo = await this.faissEngine.createIndex(
          'ivf',
          'inner_product',
          fullConfig.faiss_nlist
        );
  ...
  async hybridSearch(
    ...
    try {
      ...
      pgvectorResults = await this.pgvectorBridge.searchSimilar(
        queryEmbedding,
        fullConfig.pgvector_limit,
        0.7
      );
      ...
      if (faissResults && faissResults.indices.length > 0) {
        ...
        finalResults = this.fuseResults(
          faissDocuments,
          pgvectorResults,
          faissResults.scores,
          fullConfig.hybrid_fusion
        );
      } else {
    ...
    switch (fusionMethod) {
      ...
      case 'score':
        ...
        return combinedResults
          .sort((a, b) => (b as any).fusionScore - (a as any).fusionScore)
          .slice(0, 50);
