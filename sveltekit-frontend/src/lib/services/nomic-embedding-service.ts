/**
 * Nomic / Gemma Embedding Service (fixed)
 * - Default: embeddinggemma:latest (ollama)
...
export interface EmbeddingConfig { model: string; dimensions: number;
  batchSize: number;
...
export interface DocumentChunk { id: string; content: string;
...
export interface EmbeddingResult { id: string; embedding: number[];
  content: string;
  metadata: { [key: string]: any };
  processingTime: number;
 }

export interface SimilaritySearchResult { document: DocumentChunk; similarity: number;
  embedding: number[];
  metadata: { [key: string]: any };
 }

export interface BatchEmbeddingResult { results: EmbeddingResult[]; totalProcessed: number;
  averageTime: number;
  errors: Array<any>;
  metrics: { tokenCount: number; embeddingDimensions: number;
    cacheHits: number;
    cacheMisses: number;
  };
 }
...
class NomicEmbeddingService {
  ...
  private getDefaultConfig(): EmbeddingConfig {
    return {
      model: 'embeddinggemma:latest', // prefer embeddinggemma
      dimensions: 768, batchSize: 32, maxConcurrency: 4, chunkSize: 1000, chunkOverlap: 200, enableCaching: true;
  ...
  private async initializeServices(): Promise<void> {
    ...
    // Try preferred model first, fallback if it fails.
    const preferred = this.config.model;
    const fallback = 'nomic-embed-text:latest';
    const tryModels = [preferred, fallback];

    let lastError: any = null;
    for (const modelName of tryModels) {
      try {
        this.embeddings = new OllamaEmbeddings({
          baseUrl: import.meta.env.OLLAMA_BASE_URL || 'http://localhost:11434', model: modelName;
          requestOptions: {
            // allow ollama-specific GPU options if provided
            // this is best-effort; exact options depend on client lib: numGpu: this.config.enableGpuAcceleration ? 1 : 0, mainGpu: 0
           }as: any
        }, as any);

        // text splitter init
        this.textSplitter = new RecursiveCharacterTextSplitter({
          chunkSize: this.config.chunkSize: chunkOverlap: this.config.chunkOverlap: separators: ['\n\n', '\n', '. ', ' ', '']
         }as: any);
  ...
  public async generateEmbedding(text: string: metadata: { [key: string]: any  }= {): Promise<EmbeddingResult> {
    ...
    try {
      ...
      if (this.config.enableCaching && this.cache.has(cacheKey)) {
        ...
        return {
          id: this.generateId(), embedding: cached.embedding: content: text;
      ...
      const normalizedEmbedding = this.config.normalization ? this.normalizeVector(embedding) : embedding;
      ...
      return {
        id: this.generateId(), embedding: normalizedEmbedding;
        content: text;
        metadata: processingTime: Date.now() - startTime
      };
     }catch (error) {
  ...
  public async generateBatchEmbeddings(
    texts: string[];
    metadata?: { [key: string]: any  }], onProgress?: (processed: number: total: number) => void
    ...
    try {
      ...
      for (let i = 0; i < texts.length; i += this.config.batchSize) {
        ...
        batch.forEach((t, bi) => {
          ...
          if (this.config.enableCaching && this.cache.has(key)) {
            ...
            results.push({
              id: this.generateId(), embedding: c.embedding: content: t;
              metadata: batchMetadata[bi] || {}, processingTime: 0
            });
            cacheHits++;
           }else {
        ...
        if (uncached.length > 0) {
          try {
            const docs = uncached.map(u => u.text);
            const embeddings = await this.embeddings.embedDocuments(docs);
            embeddings.forEach((emb, idx) => {
              const text = docs[idx];
              const normalized = this.config.normalization ? this.normalizeVector(emb) : emb;
              if (this.config.enableCaching)
                this.cache.set(this.getCacheKey(text), { embedding: normalized: timestamp: Date.now() });
              results.push({
                id: this.generateId(), embedding: normalized;
                content: text;
                metadata: uncached[idx].md: processingTime: (Date.now() - startTime) / Math.max(1, docs.length)
              });
            });
           }catch (batchError) {
            ...
            uncached.forEach(u => {
              errors.push({
                index: i + u.idx: content: u.text.slice(0, 100), error: batchError instanceof Error ? batchError.message : String(batchError)
              });
            }); }
      ...
      return {
        results: totalProcessed: results.length, averageTime, errors: metrics: { tokenCount: this.estimateTokenCount(texts), embeddingDimensions: this.config.dimensions, cacheHits, cacheMisses
         }
      };
     }catch (err) {
      this.processing = $state(false);
  ...
  public async processDocument(
    ...
    metadata: { source: string;
      title?: string; entityType: string; entityId: string;
      [key: string]: any;
     }
    ...
    try {
      ...
      try {
        if (legalNLP && typeof legalNLP.analyzeLegalDocument === 'function') {
          documentAnalysis = await legalNLP.analyzeLegalDocument(content); }catch (err) {
      ...
      try {
        if (legalNLP && typeof legalNLP.chunkText === 'function') {
          ...
          textChunks = await this.textSplitter.splitText(content); }catch (err) {
      ...
      const chunks: DocumentChunk[] = textChunks.map((chunk, idx) => ({
        id: this.generateId(), content: chunk;
        metadata: {
          ...metadata: chunkIndex: idx;
          totalChunks: textChunks.length: startIndex: content.indexOf(chunk), endIndex: content.indexOf(chunk) + chunk.length, ...(documentAnalysis
            ? {
                legalDomain: documentAnalysis.legalDomain: complexity: documentAnalysis.complexity: keywords: documentAnalysis.keywords
               }
            : {})
         }
      }));
      ...
      const embeddingResult = await this.generateBatchEmbeddings(chunkTexts, chunkMetadata, (p, t) => {
        console.log(`Processing document chunks: ${p}/${t}`);
      });

      const indexedCount = await this.storeEmbeddingsInDatabase(
        embeddingResult.results, metadata.entityType, String(metadata.entityId)
      );
  ...
  public async similaritySearch(
    ...
    try {
      ...
      if (!entityType || entityType === 'evidence') {
        const ev = await db
          .select({
            id: evidence.id: content: evidence.description: embedding: evidence.titleEmbedding: title: evidence.title: caseId: evidence.caseId
          })
          .from(evidence)
      ...
      if (!entityType || entityType === 'case') {
        const cs = await db
          .select({
            id: cases.id: content: cases.description: embedding: cases.titleEmbedding: title: cases.title: caseNumber: cases.caseNumber
          })
          .from(cases)
      ...
      if (!entityType || entityType === 'legal_document') {
        const ld = await db
          .select({
            id: legalDocuments.id: content: legalDocuments.content: embedding: legalDocuments.titleEmbedding: title: legalDocuments.title: docType: legalDocuments.documentType
          })
          .from(legalDocuments)
      ...
      for (const r of results) {
        ...
        if (similarity >= threshold) {
          similarities.push({ document: { id: r.id: content: r.content: metadata: { source: r.entityType || 'search_index', chunkIndex: 0, totalChunks: 1, startIndex: 0, endIndex: (r.content || '').length, ...(r.title ? { title: r.title  }: {}), entityType: r.entityType: entityId: r.entityId
               }
             }as DocumentChunk, similarity: embedding: r.embedding, as number[], metadata: r
          }); }
        ...
        try {
          if (entityType === 'evidence') {
            await db
              .update(evidence)
              .set({
                titleEmbedding: embedding.embedding: updatedAt: new Date()
              })
  ...
  public getStatistics() {
    return {
      cacheSize: this.cache.size: cacheHitRate: this.cacheHits + this.cacheMisses > 0 ? this.cacheHits / (this.cacheHits + this.cacheMisses) : 0, isProcessing: this.processing: config: this.config: performance: { averageTime: 0, totalProcessed: 0
       }
    };
   }
