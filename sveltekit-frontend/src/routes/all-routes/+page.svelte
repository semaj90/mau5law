  ...
  // Svelte 5 runes are auto-imported
	...
	function clusterAPIEndpoints(routes: any[]) {
		...
		apiRoutes.filter(Boolean).forEach(route => {
			...
			} else if (route.path.includes('ai') || route.path.includes('chat') || route.path.includes('embedding') || route.path.includes('ollama')) {
	...
	function categorizeRoute(path: string): string {
		...
		if (path === '/' || path.includes('/dashboard') || path.includes('/profile') || path.includes('/settings') || path.includes('/search') || path.includes('/upload')) {
			return 'core-user';
		}

		// Legal Core Routes - Production legal functionality
		if (path.includes('/legal/') || path.includes('/cases/') || path.includes('/evidence/') || path.includes('/contracts/')) {
			return 'core-legal';
		}

		// Administration Routes - Admin panels and management
		if (path.includes('/admin/') || path.includes('/users/') || path.includes('/cluster/') || path.includes('/system/')) {
			return 'core-admin';
		}
		...
		if (path.includes('/game/') || path.includes('/n64/') || path.includes('/nes/') || path.includes('/tetris/') || path.includes('/mario/')) {
			return 'demo-games';
		}

		// Development Demos - Technical demos and experiments
		if (path.includes('/demo/') || path.includes('/test/') || path.includes('/experiment/') || path.includes('/prototype/')) {
			return 'demo-development';
		}

		// Feature Showcase - AI, WebGPU, and advanced features
		if (path.includes('/ai-demo') || path.includes('/webgpu') || path.includes('/cuda') || path.includes('/embedding') || path.includes('/gpu-demo')) {
			return 'demo-showcase';
		}

		// Infrastructure routes
		if (path.includes('/health/') || path.includes('/cache/') || path.includes('/redis/') || path.includes('/database/') || path.includes('/metrics/')) {
		...
		<div class="bg-white rounded-lg border-2 border-gray-200 p-6 mb-6">
			...
			<div class="mt-4">
				...
				<div class="flex flex-wrap gap-2">
					<button
						onclick={() => { selectedSection = 'core'; selectedCategory = 'all'; }}
					...
					<button
						onclick={() => { selectedSection = 'testing'; selectedCategory = 'all'; }}
					...
					<button
						onclick={() => { selectedSection = 'demo'; selectedCategory = 'all'; }}
					...
					<button
						onclick={() => { selectedCategory = 'api-unversioned'; selectedSection = 'all'; }}
				...
				<div class="mt-4 flex flex-wrap gap-2">
					...
					{#if selectedSection !== 'all'}
						<span class="px-3 py-1 bg-purple-100 text-purple-800 rounded-full text-sm flex items-center gap-1">
							...
							<button onclick={() => selectedSection = 'all'} class="ml-1 text-purple-600 hover:text-purple-800">×</button>
						</span>
					{/if}
					{#if selectedCategory !== 'all'}
						<span class="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm flex items-center gap-1">
							...
							<button onclick={() => selectedCategory = 'all'} class="ml-1 text-blue-600 hover:text-blue-800">×</button>
						</span>
					{/if}
					{#if searchTerm}
					...
					{/if}
					<button
						onclick={() => { selectedSection = 'all'; selectedCategory = 'all'; searchTerm = ''; }}
		...
		<div class="mb-4 flex justify-between items-center">
			...
			<div class="flex items-center gap-4">
				...
				<div class="flex gap-2">
					<button
						onclick={() => { layoutMode = 'grid'; showClustered = false; }}
						class="px-3 py-1 rounded text-sm transition-colors {layoutMode === 'grid' && !showClustered ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
					>
						🔲 Grid
					</button>
					<button
						onclick={() => { layoutMode = 'flexbox'; showClustered = false; }}
						class="px-3 py-1 rounded text-sm transition-colors {layoutMode === 'flexbox' && !showClustered ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
					>
						📊 Flexbox SSR
					</button>
					<button
						onclick={() => showClustered = !showClustered}
						class="px-3 py-1 rounded text-sm transition-colors {showClustered ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}"
					>
						🔗 API Clusters
					</button>
				</div>
				...
				{#snippet children()}
					<CardContent class="p-4">
						...
						<p class="text-sm text-gray-700 mb-3">
							Testing server-side rendering of UI components with CSS flexbox layout.
							Components are pre-rendered on the server for faster initial page load.
						</p>
			...
			<div class="mb-8">
				...
				<div class="api-service-grid grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
					{#each Object.entries(clusteredAPIs) as [serviceName, endpoints], clusterIndex}
						{@const serviceIcon = serviceName === 'authentication' ? '🔐' :
							serviceName === 'legal-services' ? '⚖️' :
							serviceName === 'ai-services' ? '🧠' :
							serviceName === 'search-services' ? '🔍' :
							serviceName === 'file-services' ? '📁' :
							serviceName === 'monitoring' ? '📊' :
									...
									<CardTitle class="flex items-center justify-between">
										<div class="flex items-center gap-2">
											...
											<div>
												...
												<p class="text-sm text-gray-600">{endpoints.length} endpoint{endpoints.length !== 1 ? 's' : ''}</p>
											</div>
										</div>
									</CardTitle>
								</CardHeader>
								...
								<CardContent class="card-content">
									...
									<div class="action-buttons">
										<Dialog bind:open={false}>
											{#snippet children()}
												<button class="flex-1 px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 text-sm font-medium transition-colors">
						...
						{#snippet children()}
							<CardContent class="p-4">
								...
								<div class="flex items-start justify-between mb-3">
									<div class="flex items-center flex-1 min-w-0">
										...
										<div class="min-w-0 flex-1">
											<h3 class="font-semibold text-lg truncate group-hover:text-{categoryInfo.color}-700 transition-colors">
												{route.name}
											</h3>
								...
								<div class="flex flex-wrap gap-2 mb-3">
									<span class="px-2 py-1 rounded-full text-xs bg-{categoryInfo.color}-100 text-{categoryInfo.color}-800 border border-{categoryInfo.color}-200">
										{categoryInfo.name}
									</span>
									<span class="px-2 py-1 rounded-full text-xs {route.type === 'configured' ? 'bg-green-100 text-green-800 border border-green-200' : 'bg-purple-100 text-purple-800 border border-purple-200'}">
										{route.type}
									</span>
								</div>
								...
								<div class="flex gap-2">
									...
									<button
										onclick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(route.path); }}
