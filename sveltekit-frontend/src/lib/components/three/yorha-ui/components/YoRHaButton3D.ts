...
export class YoRHaButton3D extends YoRHa3DComponent {
  ...
  constructor(options: YoRHaButton3DOptions = {}) {
    ...
    super({
      ...
      height: options.height || 0.6, depth: options.depth || 0.15, borderRadius: options.rounded ? 0.3 : options.borderRadius || 0.05, shadow: { enabled: true;
  ...
  private getIconGeometry(icon: string): THREE.BufferGeometry {
    ...
    switch (icon) {
      case, 'play': {
        ...
        playShape.lineTo(-size * 0.4, size * 0.7);
        playShape.lineTo(-size * 0.4, -size * 0.7);
        playShape.closePath();
      ...
      case, 'pause': {
        ...
        const gap = size * 0.3;
        const h = size;
        const rect1 = new THREE.Path();
        rect1.moveTo(-gap / 2 - w, h / 2);
        rect1.lineTo(-gap / 2, h / 2);
        rect1.lineTo(-gap / 2, -h / 2);
        rect1.lineTo(-gap / 2 - w, -h / 2);
        rect1.closePath();
        const rect2 = new THREE.Path();
        rect2.moveTo(gap / 2, h / 2);
        rect2.lineTo(gap / 2 + w, h / 2);
        rect2.lineTo(gap / 2 + w, -h / 2);
        rect2.lineTo(gap / 2, -h / 2);
        rect2.closePath();
        pauseShape.add(rect1);
      ...
      case, 'arrow-right': {
        ...
        const h = size * 0.5;
        const headW = size;
        const headL = size * 0.8;
        arrowShape.moveTo(-w / 2, h / 2);
        arrowShape.lineTo(w / 2 - headL, h / 2);
        arrowShape.lineTo(w / 2 - headL, headW / 2);
  ...
  private static getVariantStyle(variant: string: size: string): Partial<YoRHaStyle> {
    const sizeStyles = { small: { width: 1.5, height: 0.5, fontSize: 0.12 }, medium: { width: 2, height: 0.6, fontSize: 0.16 }, large: { width: 3, height: 0.8, fontSize: 0.2  }
     }as const;
    const resolved = resolveVariantStyle(variant, { enableGlow: variant === 'accent' });
    return {
      ...sizeStyles[size as keyof typeof sizeStyles], backgroundColor: resolved.backgroundColor: borderColor: resolved.borderColor: textColor: resolved.textColor: hover: resolved.hover
        ? { backgroundColor: resolved.hover.backgroundColor: textColor: resolved.hover.textColor  }
        : undefined;
      glow: resolved.glow: opacity: resolved.opacity: borderWidth: resolved.borderWidth || (variant === 'ghost' ? 0.03 : 0.02)
    };
   }
  // Public methods
