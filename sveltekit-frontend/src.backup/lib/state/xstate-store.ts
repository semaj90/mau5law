...
import { legalCaseMachine, legalCaseSelectors, type LegalCaseEvents } from './legal-case-machine.js';
...
interface ErrorPayload { message: string; code?: string | number; details?: unknown }
// Minimal window shape for Redux DevTools availability check
interface DevtoolsWindow extends Window { __REDUX_DEVTOOLS_EXTENSION__?: { connect: (opts?: { name?: string; [key: string]: any }) => { send: (action: any, state?: unknown) => void; init: (state: unknown) => void } } }
// Inspection event shape used by the devtools inspector callback
interface InspectionEvent { type?: string; event?: unknown; snapshot?: unknown }
// Store persistence interface
export interface StoreState { appState: unknown; legalCaseState: unknown; timestamp: number }
// Configuration for store behavior
export interface XStateStoreConfig { persist?: boolean; persistKey?: string; devtools?: boolean; logTransitions?: boolean; syncAcrossTabs?: boolean }
class XStateStoreManager {
  ...
  private constructor(config: XStateStoreConfig = {}) {
    this.config = { persist: true, persistKey: 'legal-ai-state', devtools: browser && import.meta.env.NODE_ENV === 'development', logTransitions: browser && import.meta.env.NODE_ENV === 'development', syncAcrossTabs: true, ...config };
    if (browser) { this.initializeBrowserFeatures() }
  }
  public static getInstance(config?: XStateStoreConfig): XStateStoreManager {
    if (!XStateStoreManager.instance) { XStateStoreManager.instance = new XStateStoreManager(config) }
  ...
  private initializeBrowserFeatures(): void {
    ...
    if (this.config.syncAcrossTabs) {
      ...
      this.syncChannel.addEventListener('message', (event: MessageEvent) => { this.handleCrossTabSync(event.data) })
    }
    ...
    window.addEventListener('online', () => { this.appActor?.send({ type: 'ONLINE' }) });
    window.addEventListener('offline', () => { this.appActor?.send({ type: 'OFFLINE' }) });
    // Set up performance monitoring
    this.setupPerformanceMonitoring()
  }
  /** * Initialize the application machine and store */
  public initializeApp(): { appStore: Readable<unknown>; appActor: ActorRefFrom<typeof appMachine>; send: (_event: AppEvents) => void; selectors: typeof appSelectors } {
    if (this.appActor) { throw new Error('App machine already initialized') }
    // Load persisted state if available
    const persistedState = this.loadPersistedState();
    // Create app actor with persistence
    this.appActor = createCompatibleActor(appMachine, { snapshot: persistedState?.appState, inspect: this.config.devtools ? this.createDevtoolsInspector('app') : undefined });
    ...
    const { subscribe } = readable(this.appActor.getSnapshot(), (set: (v: unknown) => void) => {
      ...
      const subscription = this.appActor!.subscribe((state: any) => {
        if (this.config.logTransitions) { const snap = state as unknown as MachineSnapshot; console.log('🔄 App State Transition: ', snap?.value, snap?.context) }
        set(state);
        // Broadcast to other tabs
        if (this.syncChannel) { this.syncChannel.postMessage({ type: 'app-state-change', state }) }
      });
      // Start the machine
      this.appActor!.start();
    ...
    const send = (event: AppEvents) => { if (this.config.logTransitions) { console.log('📤 App Event: ', event) } this.appActor?.send(event) };
    return { appStore: { subscribe }, appActor: this.appActor, send, selectors: appSelectors }
  }
  /** * Initialize the legal case machine and store */
  public initializeLegalCase(): { legalCaseStore: Readable<unknown>; legalCaseActor: ActorRefFrom<typeof legalCaseMachine>; send: (_event: Event) => void; selectors: typeof legalCaseSelectors } {
    if (this.legalCaseActor) { throw new Error('Legal case machine already initialized') }
    // Load persisted state if available
    const persistedState = this.loadPersistedState();
    // Create legal case actor
    this.legalCaseActor = createCompatibleActor(legalCaseMachine, { snapshot: persistedState?.legalCaseState, inspect: this.config.devtools ? this.createDevtoolsInspector('legalCase') : undefined });
    // Create reactive Svelte store
    const { subscribe: subscribeCase } = readable(this.legalCaseActor.getSnapshot(), (set: (v: unknown) => void) => {
      ...
      const subscription = this.legalCaseActor!.subscribe((state: any) => {
        if (this.config.logTransitions) { const snap = state as unknown as MachineSnapshot; console.log('⚖️ Legal Case State Transition: ', snap?.value, snap?.context) }
        set(state);
        // Broadcast to other tabs
        if (this.syncChannel) { this.syncChannel.postMessage({ type: 'legal-case-state-change', state }) }
      });
      // Start the machine
    ...
    // Send function for dispatching events
    const sendCase = (event: Event) => { if (this.config.logTransitions) { console.log('📤 Legal Case Event: ', event) } this.legalCaseActor?.send(event) };
    return { legalCaseStore: { subscribe: subscribeCase }, legalCaseActor: this.legalCaseActor, send: sendCase, selectors: legalCaseSelectors }
  }
  ...
  public createUtilities(appSend: (_event: AppEvents) => void) {
    return {
      ...
      notify: {
        success: (title: string, message: string) => appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'success', title, message } }),
        error: (title: string, message: string) => appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'error', title, message } }),
        warning: (title: string, message: string) => appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'warning', title, message } }),
        info: (title: string, message: string) => appSend({ type: 'ADD_NOTIFICATION', notification: { type: 'info', title, message } }),
  ...
  private createDevtoolsInspector(machineId: string) {
    return (inspectionEvent: Event) => {
      ...
      if (win && win.__REDUX_DEVTOOLS_EXTENSION__) {
        const devtools = win.__REDUX_DEVTOOLS_EXTENSION__.connect({ name: `XState: ${machineId}`, trace: true });
        const ev = inspectionEvent as unknown as InspectionEvent;
        switch (ev?.type) {
          case '@xstate.event': devtools.send(ev.event, ev.snapshot); break;
          case '@xstate.snapshot': devtools.init(ev.snapshot); break;
        }
      }
    }
  }
  private persistState(): void {
    ...
    try {
      const state: StoreState = { appState: this.appActor?.getSnapshot() ?? null, legalCaseState: this.legalCaseActor?.getSnapshot() ?? null, timestamp: Date.now() };
  ...
  private setupPerformanceMonitoring(): void {
    ...
    if (typeof window !== 'undefined' && 'performance' in window) {
      const observer = new PerformanceObserver((list: PerformanceObserverEntryList) => {
        for (const entry of list.getEntries()) {
          if (entry.entryType === 'navigation') {
            ...
            this.appActor?.send({ type: 'UPDATE_PERFORMANCE_METRICS', metrics: { pageLoadTime: navEntry.loadEventEnd - navEntry.loadEventStart } } as unknown as AppEvents)
          }
        }
      });
      ...
      if (perfMem) {
        setInterval(() => {
          this.appActor?.send({ type: 'UPDATE_PERFORMANCE_METRICS', metrics: { memoryUsage: perfMem.usedJSHeapSize } } as unknown as AppEvents)
  ...
  public destroy(): void {
    ...
    if (browser && this.config.persist) { this.persistState() }
  }
}
...
export function createXStateStore(config?: XStateStoreConfig) { return XStateStoreManager.getInstance(config) }
...
export function useXStateStore() { return initializeStores() }
