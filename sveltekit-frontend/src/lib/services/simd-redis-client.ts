...
export class SIMDRedisClient {
  ...
  constructor() {
    ...
    this.baseUrl = browser ? 'http://localhost:8080' : 'http://localhost:8080';
    this.gpuUrl = browser ? 'http://localhost:8081' : 'http://localhost:8081';
  ...
  async healthCheck(): Promise<SIMDHealthStatus> {
    try {
      ...
      try {
        const gpuResponse = await this.fetchWithTimeout(`${this.gpuUrl}/health`, {
          method: 'GET'
        }, 5000);
  ...
  private async parseJSONGPU(jsonString: string): Promise<SIMDParseResult> {
    const response = await this.fetchWithTimeout(`${this.gpuUrl}/parse/simd`, {
      method: 'POST',
      headers: {
    ...
    return { ...result, parser: 'simd_avx2_cuda' };
  }
  ...
  private async parseJSONSIMD(jsonString: string): Promise<SIMDParseResult> {
    const response = await this.fetchWithTimeout(`${this.baseUrl}/parse/simd`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json'
    ...
    const result = (await response.json()) as SIMDParseResult;
    return { ...result, parser: 'simdjson-go' };
  }

  /**
   * Cache JSON in Redis with SIMD parsing
   */
  async cacheJSON(key: string, data: unknown): Promise<{
    success: boolean;
    key: string;
    size: number;
    cached_at: string;
    ...
    const response = await this.fetchWithTimeout(`${this.baseUrl}/parse/cache`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json'
      },
      body: JSON.stringify({ key, data, operation: 'cache' })
    }, this.timeout);
  ...
  async benchmark(data: any, iterations: number = 100): Promise<SIMDBenchmarkResult> {
    ...
    const stringDocuments = documents.map((doc: any) => (typeof doc === 'string' ? doc : JSON.stringify(doc)));
  ...
  private async benchmarkGPU(documents: string[], iterations: number): Promise<SIMDBenchmarkResult> {
    const response = await this.fetchWithTimeout(`${this.gpuUrl}/parse/batch-gpu`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json'
      },
      body: JSON.stringify({ documents, iterations })
    }, this.timeout * 2); // Longer timeout for benchmarks
    ...
    const result = await response.json();
    return { ...result, gpu_processed: true };
  }
  ...
  private async benchmarkSIMD(documents: string[], iterations: number): Promise<SIMDBenchmarkResult> {
    const response = await this.fetchWithTimeout(`${this.baseUrl}/parse/batch`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Accept: 'application/json'
    ...
    const result = await response.json();
    return { ...result, gpu_processed: false };
  }
