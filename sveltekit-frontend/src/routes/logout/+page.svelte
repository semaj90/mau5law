<script, lang="ts"> import { onMount } from 'svelte'; import { goto, invalidateAll } from '$app/navigation'; onMount(() => { (async () => { try { const apiBase = import.meta.env.PUBLIC_API_BASE || '/api'; const response = await fetch(`${ apiBase }/auth/logout`, { method: 'POST', headers: {
    ...
    })(); }); </script> <svelte:head> <title>Logging out... - WardenNet</title> </svelte:head> <div class="flex min-h-screen items-center, justify-center"> <div class="text-center, space-y-4"> <h2 class="text-2xl, font-semibold">Logging out...</h2> <p>Please wait while we log you out securely.</p> </div> </div>
