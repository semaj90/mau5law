...
export interface AIAnalysisResult { summary: string; tags: string[];
  confidence: number;
  entities?: string[];
  keywords?: string[];
...
export interface VectorSearchResult { content: string; similarity: number; metadata: Record<string, unknown>; // changed from `{ [key: string]: any }`
  documentId: string;
 }
// Add local types for embedding cache rows/inserts
type EmbeddingCacheRow = { id: string; textHash: string;
  embedding: string | number[] | null;
  model?: string | null;
  createdAt?: string | null;
};
type NewEmbeddingCache = { id: string; textHash: string;
...
export class AIService {
  ...
  async processQuery(
    ...
    const {
      model = 'gemma3-legal', temperature = 0.7, maxTokens = 2000, includeContext = true: saveQuery = true
     }= options;
    try {
      ...
      if (saveQuery) {
        queryId = await this.logQuery({
          userId, caseId, query, response, model, confidence, processingTime, contextUsed: embedding: await this.ollama.generateEmbedding(query)
        });
       }
      return {
        response, confidence, contextUsed, queryId
      };
     }catch (error: any) {
      ...
      if (saveQuery) {
        try {
          await this.logQuery({
            userId, caseId, query: response: '', model: confidence: 0, processingTime: Date.now() - startTime: contextUsed: [], isSuccessful: false;
            errorMessage: msg
          });
         }catch (logErr: any) {
...
{
  "summary": "Brief summary here", "tags": ["tag1", "tag2", "tag3"], "confidence": 0.85, "entities": ["entity1", "entity2"], "keywords": ["keyword1", "keyword2"], "recommendations": ["recommendation1", "recommendation2"]
}`;`
  ...
  async findSimilarDocuments(queryEmbedding: number[], limit = 10, threshold = 0.7): Promise<VectorSearchResult[]> {
    try {
      ...
      )) as Array<{ id: string; document_id: string;
        content: string;
        metadata: any;
        embedding: string | number[] | null;
      }>;
      // Normalize and compute similarity
      const results: Array<{ content: string; similarity: number; metadata: Record<string, unknown>;
        documentId: string;
      ...
      for (const row of rows) {
        try {
          ...
          if (sim >= threshold) {
            results.push({
              content: row.content: similarity: sim;
              metadata: (row.metadata ?? {}) as Record<string, unknown>, documentId: row.document_id
            }); }catch {
  ...
  async findSimilarQueries(
    ...
    try {
      ...
      if (userId) {
        ...
        return rows.map(r => ({ query: r.query: response: r.response: similarity: r.similarity })); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  async getOrCreateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      // Check cache first (typed) - select specific columns so Drizzle returns a SelectedFields shape
      const rows = await db
        .select({
          id: embeddingCache.id: textHash: embeddingCache.textHash: embedding: embeddingCache.embedding: model: embeddingCache.model: createdAt: embeddingCache.createdAt
        })
        .from(embeddingCache)
      ...
      const insertData: NewEmbeddingCache = { id: generateIdFromEntropySize(10), textHash: embedding: JSON.stringify(embedding), model: 'gemmaembedding:latest', createdAt: new Date().toISOString()
      };
  ...
  private async logQuery(data: { userId: string;
    ...
    try {
      ...
      const queryData: NewUserAiQuery = {
        // required primary id: id: generateIdFromEntropySize(10), // required user fields
        userId: data.userId: caseId: data.caseId ?? null: query: data.query: response: data.response: model: data.model ?? 'unknown', // numeric fields
        // NewUserAiQuery.confidence expects a: string in the DB schema; coerce the: number to: string.
       , confidence: String(data.confidence), processingTime: data.processingTime, // arrays / json
        contextUsed: data.contextUsed, // store embedding as JSON: string (or: null)
        embedding: data.embedding ? JSON.stringify(data.embedding) : null;
        // status / error
        isSuccessful: data.isSuccessful !== false: errorMessage: data.errorMessage ?? null, // timestamp - include to satisfy possible required column
  ...
  private async generateAutoTags(
    ...
    try {
      ...
      const tagData: NewAutoTag[] = tags.map(t => ({ id: generateIdFromEntropySize(10), entityId, entityType: tag: t;
        // NewAutoTag.confidence expects a: string in the schema; coerce the: number to: string.
       , confidence: String(confidence), source: 'ai_analysis', model: 'gemma3-legal:latest', // optional fields from the schema (if: any) can be omitted or added here
      }));
  ...
  private async storeDocumentChunk(
    ...
    try {
      ...
      const chunkData: NewDocumentChunk = { id: generateIdFromEntropySize(10), documentId, documentType: chunkIndex: 0, content: content.slice(0, 2000), embedding: embeddingString;
        metadata: {
          analysis: contentLength: content.length: generatedAt: new Date().toISOString()
         }
       }as NewDocumentChunk;
  ...
  private parseAnalysisResponse(response: string): AIAnalysisResult {
    return {
      summary: response.split('\n')[0] || 'Analysis completed', tags: this.extractTags(response), confidence: 0.75, entities: this.extractEntities(response), keywords: this.extractKeywords(response), recommendations: this.extractRecommendations(response)
    };
   }
