<script lang="ts"> import { createEventDispatcher } from 'svelte';
 ...
 <svelte: window | onkeydown={ handleKeydown } />
  {#if open} <!-- Modal, Backdrop --> <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50"
  ...
  {#if title} <CardHeader class="pb-4"> <CardTitle class="enhanced-modal-title {variant === 'diamond' ? 'nes-diamond-text': ''}">{ title }
</CardTitle >
          </CardHeader> {/if}
