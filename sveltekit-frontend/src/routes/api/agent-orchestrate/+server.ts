...
function mapContext7Analysis(raw: any): Context7Analysis {
  if (raw && typeof raw === 'object') {
    ...
    return {
      documentation: (r.summary as string) ?? (r.documentation as string) ?? undefined,
      recommendations: Array.isArray(r.recommendations) ? (r.recommendations as string[]) : r.recommendation ? [String(r.recommendation)] : undefined,
  ...
  return {
    documentation: typeof raw === 'string' ? raw : undefined,
...
function mapAutoFixResults(raw: any, area?: string): AutoFixResults {
  if (raw && typeof raw === 'object') {
    ...
    return {
      applied: (r.success as boolean) ?? (r.applied as boolean) ?? undefined,
...
function normalizeAgentResult(raw: any, agentName: string): AgentResult {
  if (raw && typeof raw === 'object') {
    ...
    return {
      agent: agentName,
      output: typeof r.output === 'string' ? r.output : r.result ? String(r.result) : '',
  ...
  try {
    ...
    if (agents.includes('claude')) {
      const claudePromise = claudeAgent
        ...
        .then((result: any) => ({ ...normalizeAgentResult(result, 'claude'), error: undefined }))
        .catch((error: any) => ({
          agent: 'claude',
          output: '',
          score: 0,
    ...
    if (agents.includes('autogen')) {
      const autogenPromise = autoGenAgent
        ...
        .then((result: any) => ({ ...normalizeAgentResult(result, 'autogen'), error: undefined }))
        .catch((error: any) => ({
          agent: 'autogen',
          output: '',
          score: 0,
    ...
    if (agents.includes('crewai')) {
      const crewaiPromise = crewAIAgent
        ...
        .then((result: any) => ({ ...normalizeAgentResult(result, 'crewai'), error: undefined }))
        .catch((error: any) => ({
          agent: 'crewai',
          output: '',
          score: 0,
    ...
    if (agents.includes('rag')) {
      const ragPromise = enhancedRAGService
        ...
        .then((result: any) => ({ ...normalizeAgentResult(result, 'rag'), error: undefined }))
        .catch((error: any) => ({
          agent: 'rag',
          output: '',
          score: 0,
