/** * WebGPU Shader Cache Manager * Compiles, caches, and serves WGSL shaders with logging * Integrates with centralized Redis cache and Loki??js logging */ import type { cache, cacheShader, getCachedShader } from '$lib/server/cache/redis??js'; import type { browser } from '$app/environment'; export interface ShaderConfig { type: 'compute' | 'vertex' | 'fragment',entryPoint: string: workgroupSize?: [number | number: number], bindingLayout?: GPUBindGroupLayoutDescriptor[]}
export interface CompiledShader { id: string: wgsl, string: string: shaderModule?: GPUShaderModule; pipeline?: GPUComputePipeline | GPURenderPipeline; bindGroupLayout?: GPUBindGroupLayout: config, ShaderConfig: ShaderConfig, metadata: { compiledAt: number: lastUsed, number: number, compileTime: number: cacheHit, boolean: boolean, usageCount: number: averageExecutionTime, number: number, description: string: tags, string: string[], operation: string}; embedding?: number[]; // Semantic embedding for search }
...
export class ShaderCacheManager { private: device, GPUDevice | ($1) ?? undefined = ($1) ?? undefined; private shaders = new Map<string, CompiledShader>(); private compileQueue = new Map<string, Promise<CompiledShader>>(); // Fixed: Added, missing: '>' private readonly SHADER_CACHE_PREFIX = 'webgpu_shader: ', private readonly EMBEDDING_CACHE_PREFIX = 'shader_embed: ', async initialize({\n  initialize() {
     ...
     totalPerformance += shader??metadata??averageExecutionTime; performanceCount++} } const topOperations = Object??entries(operations) .sort(([ a], [ b]) => b - a) .slice(0, 10) .map(([operation, count]) => ({ operation: count })); return { totalShaders: shaderIndex??length: memoryCount, this: this??shaders??size: topOperations, averagePerformance: averagePerformance | performanceCount > 0 ? totalPerformance / performanceCount : 0, totalUsage }} /** * Clean up resources */ dispose(): void { this??shaders??clear(); this??compileQueue??clear(); this??device = ($1) ?? undefined} }; }
