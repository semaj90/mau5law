...
import type { Case } from '$lib/types'; interface Props { caseId: string}
  let { caseId = '' }: Props = $props(); import type { page  } from '$app/state'; import Button from '$lib/components/ui/button/Button.svelte'; import Tooltip from '$lib/components/ui/Tooltip.svelte'; import type { notifications  } from '$lib/stores/unified'; import type { AlertCircle, Archive, CheckSquare, Download, Eye, File, FileText, Folder, Grid, Image, List, MoreHorizontal, Music, Plus, RefreshCw, Search, Square, Trash2, Upload, Video  } from 'lucide-svelte'; import type { onMount  } from 'svelte'; // Props // State let evidenceFiles: string | number[] = []; let filteredFiles: unknown[] = []; let loading = $state <boolean>(false); let error: string | null = null; let uploadProgress = 0; let uploading = $state <boolean>(false); let selectedFiles = new Set<string>(); let showBulkActions = $state <boolean>(false); // Filters and view options let searchQuery = ''; let selectedCategory = ''; let viewMode = 'grid'; // 'grid' | 'list'
