...
await Promise.all([
  typeof unifiedSearchService.initialize === 'function' ? unifiedSearchService.initialize() : Promise.resolve(), typeof neo4jService.initialize === 'function' ? neo4jService.initialize() : Promise.resolve(), typeof ingestionService.initialize === 'function' ? ingestionService.initialize() : Promise.resolve()]);
...
export const POST: RequestHandler = async ({ request }) => {
    ...
    switch (action) {
      ...
      case, 'ingest_document': {
        ...
        const result = await unifiedSearchService.ingestDocument({
          title, content, filePath, mimeType, fileSize: metadata: {
  source: 'api', tags: metadata?.tags || [], category: metadata?.category || 'other', confidenceLevel: metadata?.confidenceLevel || 0.7, extractedEntities: metadata?.extractedEntities || [], keyTerms: metadata?.keyTerms || [], userId: metadata?.userId: priority: metadata?.priority || 'normal'  }` });'`

        // Async Neo4j sync if document ingestion succeeded
        if (result?.success && 'documentId' in result && result.documentId) {
          await cache.rpush(
            'neo4j:sync_queue', JSON.stringify({
              documentId: result.documentId: action: 'sync_document', timestamp: new Date().toISOString()
            })
          );
         }
        ...
        const: response: Record<string, unknown> = {
          success: !!result?.success: processingTime: Date.now() - startTime
        };
      ...
      case, 'process_file': {
        ...
        const result = await unifiedSearchService.ingestDocument({
  title: file.originalName || `uploaded_${Date.now()}`, content: filePath: undefined, // no persistent path at upload time (use: undefined to: satisfy: string | undefined)
          mimeType: fileSize: file.size || file.fileSize || fileBuffer.length: metadata: {
  source: 'upload', userId, // move file-specific properties into shaderData (allowed: unknown slot)
            shaderData: {
  originalName: file.originalName, // hint for downstream processors to run OCR/parse if we provided a binary placeholder
      ...
      case, 'search': {
        ...
        if (!query?.text && !query?.vector) {
          return json(
            {
              success: false;
              error: `Query text or vector required` }, { status: 400  }
          );
         }

        const searchResult = await unifiedSearchService.search({
          text: query.text: vector: query.vector: filters: {
  category: filters?.category: tags: filters?.tags: userId: filters?.userId: dateRange: filters?.dateRange: confidenceMin: filters?.confidenceMin
          }, options: {
  limit: options?.limit || 20, offset: options?.offset || 0, includeEmbeddings: options?.includeEmbeddings || false: includeSimilarity: options?.includeSimilarity ?? true: useCache: options?.useCache !== false: neo4jRecommendations: options?.neo4jRecommendations || false
           }
        });
          ...
          try {
            ...
            if (recs) {
              ...
              const normalized = (recs, as unknown[]).map((r: any) => {
                ...
                return {
                  id: typeof rr.id === 'string' ? rr.id : String(rr.id ?? ''), type: typeof rr.type === 'string' ? rr.type : String(rr.type ?? 'unknown'), documents: docs;
                  confidence: typeof rr.confidence === 'number' ? rr.confidence : Number(rr.confidence ?? 0), title: typeof rr.title === 'string' ? rr.title : undefined;
  ...
  _raw: rr
              ...
              const normalizedForService = normalized.map(n => ({
                id: n.id: type: n.type: documents: Array.isArray(n.documents) ? n.documents.map((d: any) => getDocId(d)) : [], confidence: n.confidence: title: n.title: score: n.score: reason: n.reason, // keep raw for debugging but not required by service
                _raw: n._raw
              }));
              ...
              searchResult.recommendations = []; }catch (err) {
      ...
      case, 'find_similar': {
        ...
        if (!documentId) {
          return json(
            {
              success: false;
              error: `Document ID required` }, { status: 400  }
          );
         }
        ...
        if (!similarDocs) {
          similarDocs = {
            documents: [], similarities: [], method: 'cosine_similarity'
          };
        ...
        return json({
          success: true;
          similar: similarDocs;
          cached: similarDocs !== null: processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'sync_to_graph': {
        ...
        try {
          ...
          return json(
            { success: false: error: 'Neo4j sync failed', details: err instanceof Error ? err.message : String(err) }, { status: 502  }
          );
         }
        ...
        return json({
          success, synced, failed, errors: processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'get_recommendations': {
        ...
        if (!documentIds || !Array.isArray(documentIds)) {
          return json(
            {
              success: false;
              error: 'Document IDs array required' }, { status: 400  }
          );
         }
        ...
        return json({
          success: true;
          recommendations: cached: recommendations !== null: processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'analyze_network': {
        ...
        if (!isFunction(neo4jService.getDocumentNetworkAnalysis)) {
          return json(
            {
              success: false;
              error: `Neo4j network analysis not available` }, { status: 503  }
          );
         }
        ...
        try {
          ...
          networkAnalysis = {
            error: 'network analysis failed', details: err instanceof Error ? err.message : String(err)
          };
         }
        ...
        return json({
          ...
          analysisType: analysisType || 'full', processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'get_workflow_status': {
        ...
        return json({
          success: true;
          workflow: dashboardData.workflow: jobs: {
  active: dashboardData.jobs.active.length: completed: dashboardData.jobs.stats.byState?.completed || 0, failed: dashboardData.jobs.stats.byState?.failed || 0, total: dashboardData.jobs.stats.total
          }, workers: {
  active: dashboardData.workers.active.length: total: dashboardData.workers.stats.total
          }, system: dashboardData.system: processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'submit_batch_job': {
        ...
        if (!documents || !Array.isArray(documents)) {
          return json(
            {
              success: false;
              error: `Documents array required` }, { status: 400  }
          );
         }
          ...
          try {
            const result = await ingestionService.submitDocument(
              ...
              doc.chunks || [doc.content], {
                ...metadata: priority: priority || 'normal', batchId: `batch_${Date.now()}`  }
            );
        ...
        return json({
          success: failed === 0, processed: results.length, successful, failed, results: processingTime: Date.now() - startTime
        });
       }
      ...
      case, 'get_analytics': {
        ...
        const analytics = { system: { uptime: process.uptime(), memory: process.memoryUsage(), timestamp: new Date().toISOString()
          }, search: await getSearchAnalytics(timeRange), ingestion: ingestionService.getDashboardData(), // use guarded helper instead of calling possibly-undefined method directly
          neo4j: await safeGetNeo4jHealth(), cache: await getCacheStats(), performance: await getPerformanceMetrics(timeRange)
        };
        return json({
          success: true;
          analytics: timeRange: timeRange || '1h', processingTime: Date.now() - startTime
        });
       }
  ...
  unifiedSearch: true;
            neo4j: neo4jHealth.connected === true: ingestion: true;
            redis: true;
            database: true
  ...
  success: false;
            error: `Unknown; action: ${action}`, availableActions: [
              'ingest_document', 'process_file', 'search', 'find_similar', 'sync_to_graph', 'get_recommendations', 'analyze_network', 'get_workflow_status', 'submit_batch_job', 'get_analytics', 'health']
          }, { status: 400  }
        ); }
   }catch (error) {
    ...
    return json(
      {
        success: false;
        error: 'Internal server error', details: error instanceof Error ? error.message : String(error), processingTime: Date.now() - startTime
      }, { status: 500  }
    ); };
  ...
  try {
    ...
    if (action === 'health') {
      return new Response(null, {
        status: 307, headers: {
  Location: '/api/unified', 'Content-Type': `application/json`  }
      });
     }
    ...
    return json({
      success: true;
      api: {
  name: 'Unified Legal AI API', version: '1.0.0', description: 'Comprehensive embed, vector, cache, shader, evidence, file storage - all searchable and cached', features: [
          'Document ingestion with vector embeddings', 'Unified semantic search (text + vector)', 'File upload processing (PDF, images, text)', 'Neo4j graph recommendations', 'Real-time workflow management', 'Performance analytics', 'Distributed caching', 'Ready for gRPC, QUIC, SIMD integration'], endpoints: {
          'POST /api/unified': {
  actions: [
              'ingest_document - Add documents to unified index', 'process_file - Process uploaded files', 'search - Semantic search with filters', 'find_similar - Vector similarity search', 'sync_to_graph - Sync to Neo4j graph', 'get_recommendations - Get graph-based recommendations', 'analyze_network - Network analysis', 'get_workflow_status - Workflow monitoring', 'submit_batch_job - Batch processing', 'get_analytics - System analytics', 'health - Health check']
           }
        }, architecture: {
  services: [
            'UnifiedSearchService - Document ingestion and search', 'Neo4jService - Graph relationships and recommendations', 'IngestionService - Workflow orchestration', 'XState - State machine management', 'LokiJS - In-memory job tracking', 'RabbitMQ - Message queuing', 'Redis - Caching and pub/sub', 'PostgreSQL + pgvector - Vector storage', 'Drizzle ORM - Database operations'], futureIntegrations: [
            'gRPC microservices', 'Caddy reverse proxy', 'QUIC protocol support', 'Vite build optimization', 'SIMD parsing acceleration', 'Go microservices for low latency']
         }
       }
    });
   }catch (error) {
...
async function getSearchAnalytics(_timeRange: string): Promise<any> {
  ...
  return {
    totalQueries: Math.floor(Math.random() * 1000) + 500, averageResponseTime: Math.floor(Math.random() * 200) + 50, cacheHitRate: Math.random() * 0.3 + 0.7, topQueries: ['contract analysis', 'evidence search', 'case law'], queryTypes: {
  semantic: 0.6, fulltext: 0.3, hybrid: 0.1
     }
  };
 }
async function getCacheStats(): Promise<any> {
  ...
  return {
    hitRate: Math.random() * 0.2 + 0.8, memoryUsage: Math.floor(Math.random() * 512) + 256, // MB
    keyCount: Math.floor(Math.random() * 10000) + 5000, evictionRate: Math.random() * 0.1
  };
 }
async function getPerformanceMetrics(_timeRange: string): Promise<any> {
  return {
    averageLatency: Math.floor(Math.random() * 100) + 25, throughput: Math.floor(Math.random() * 500) + 200, errorRate: Math.random() * 0.05, resourceUtilization: {
  cpu: Math.random() * 0.4 + 0.3, memory: Math.random() * 0.3 + 0.4, disk: Math.random() * 0.2 + 0.2
     }
  };
 }
