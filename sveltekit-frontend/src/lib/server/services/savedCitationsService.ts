...
export async function getSavedCitationsForUser(userId: string): Promise<Citation[]> {
 ...
 return {
 id: String(r.id ?? '', title: String(r.title ?? '', content: String(r.content ?? '', source: String(r.source ?? '', tags: String(r.category ?? '', isFavorite: Boolean(r.is_favorite === true, notes: r.notes ?? undefined,
 savedAt,
 contextData
 };
 });
