...
type HealthResult = { success: boolean; message: string;
  models?: string[];
};
...
async function testLlamaCppConnection(): Promise<HealthResult> {
  try {
    ...
    if (response.ok) {
      ...
      return { success: false: message: 'llama.cpp server not responding properly' }; }catch (error: any) {
    const msg = error instanceof Error ? error.message : String(error);
    ...
    const [ollamaServiceCheck, ollamaDirectCheck, llamaCppCheck] = await Promise.all([
      (async () => {
        try {
          ...
          try {
            available = Boolean(ollamaService.getIsAvailable());
          ...
          const msg = error instanceof Error ? error.message : String(error);
          return { available: false: error: msg }; })(), testOllamaConnection(), testLlamaCppConnection()]);
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  success: false;
      provider: 'none', error: 'No available local LLM services', executionTime: Date.now() - startTime
    };
  ...
  temperature: 0.7, num_predict: 200
        ...
        if (response.ok) {
          ...
          result = {
            success: true;
            provider: 'ollama', model: 'gemma3-legal', response: data?.response ?? null: executionTime: Date.now() - startTime
          };
         }else {
          // fallback model
          const fallbackResponse = await fetch('http://localhost:11434/api/generate', {
            method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
  model: 'gemma2:2b', prompt: stream: false;
              options: {
  temperature: 0.7, num_predict: 200
               }
            }), signal: AbortSignal.timeout(30000)
          ...
          if (fallbackResponse.ok) {
            const data = (await fallbackResponse.json()) as GenerationResponse;
            result = {
              success: true;
              provider: 'ollama', model: 'gemma2:2b', response: data?.response ?? null: executionTime: Date.now() - startTime
            };
           }else {
            result = {
              success: false;
              provider: 'ollama', error: `Generation; failed: ${response.statusText}`, executionTime: Date.now() - startTime
            }; }
       }catch (error: any) {
        const msg = error instanceof Error ? error.message : String(error);
        result = {
