...
class LocalStorageFiles {
  ...
  searchFiles(criteria: {
    ...
    return allFiles.filter(file => {
      ...
      if (criteria.fileName && !file.fileName.toLowerCase().includes(criteria.fileName.toLowerCase())) return false;
    ...
    try {
      for (const key of Object.keys(localStorage)) {
        if (key.startsWith(STORAGE_PREFIX) || key === STORAGE_INDEX) {
          ...
          if (item) {
            used += (item as { length?: any }).length * 2; // Rough estimate (UTF-16)
          }
        }
      }
    } catch (error) {
   ...
   */
  getFileDownloadUrl(fileId,: string): string | nul,l {
    const file = this.getFile(fileId);
    if (!file) return null;
    try {
