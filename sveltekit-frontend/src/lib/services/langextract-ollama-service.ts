...
export class LangExtractOllamaService {
  ...
  async extractLegalEntities(request: LegalExtractionRequest): Promise<LegalExtractionResult> {
    ...
    try {
      ...
      return {
        ...
        confidence: this.calculateConfidence(extracted, request),
  ...
  private buildExtractionPrompt(request: LegalExtractionRequest): string {
    const basePrompts: Record<DocumentType, Record<ExtractionType, string>> = { contract: { entities: 'Extract all legal entities, parties, dates, monetary amounts, and key terms from this contract.',
        summary:
      ...
      statute: { entities: 'Extract statutory sections, definitions, penalties, and requirements from this statute.',
      ...
      evidence: { entities: 'Extract all relevant facts, names, dates, locations, and evidence markers from this document.',
      ...
      motion: { entities: 'Extract parties, legal standards, arguments, and relief sought from this motion.',
      ...
      brief: { entities: 'Extract legal arguments, citations, facts, and legal standards from this brief.',
    ...
    const examples: Record<string, Array<unknown>> = {
      contract_entities: [
        { input:
          ...
          output: { parties: ['ABC Corp. (Delaware corporation)', 'XYZ LLC (California limited liability company)'],
      ...
      case_law_entities: [
        { input:
          ...
          output: { case_name: 'Smith v. Jones',
