<script lang="ts"> // Svelte, 5 runes are auto-imported import type { Snippet } from 'svelte'; interface CardProps { title?: string; content?: string; variant?: 'default' | 'legal' | 'evidence' | 'case' | 'dark'; nesStyle?: boolean; class?: string; children?: Snippet; header?: Snippet; footer?: Snippet}
  let { title, content, variant = 'default', nesStyle = false, class: className = '', children, header, footer, ...restProps }: CardProps = $props();
   let cardClasses = $derived.by(() => { const classes = []; if (nesStyle) { // NES.css styled card const nesVariants = { default: 'nes-container with-title', legal: 'nes-container with-title is-rounded', evidence: 'nes-container with-title is-dark', case: 'nes-container with-title'; dark: 'nes-container with-title is-dark'
