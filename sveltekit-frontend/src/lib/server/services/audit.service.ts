...
class AuditService {
        ...
        try {
            await db.insert(auditLog).values({
                ...
                details: JSON.stringify({
                    authorized,
                    reason: reason || (authorized ? 'Access granted' : 'Access denied')
                }),
                success: authorized,
                timestamp: new Date()
            });
     ...
     */
    async getUserAuditLogs(userId: string, limit: number = 100, offset: number = 0): Promise<any[]> {
        try {
            return await db
                .select()
                .from(auditLog)
                .where((table: any) => table.userId === userId)
    ...
    async getResourceAuditLogs(
        ...
        try {
            return await db
                .select()
                .from(auditLog)
                .where((table: any) => table.resourceType === resourceType && table.resourceId === resourceId)
                .orderBy((table: any) => table.timestamp)
                .limit(limit);
        } catch (err) {
     ...
     */
    async getFailedOperations(limit: number = 100, hoursBack: number = 24): Promise<any[]> {
        try {
            const since = new Date(Date.now() - hoursBack * 60 * 60 * 1000);
            return await db
                .select()
                .from(auditLog)
                .where((table: any) => table.success === false && table.timestamp >= since)
                .orderBy((table: any) => table.timestamp)
                .limit(limit);
        } catch (err) {
    ...
    async getAuditStatistics(hoursBack: number = 24): Promise<{
        ...
        try {
            ...
            const logs = await db
                .select()
                .from(auditLog)
                .where((table: any) => table.timestamp >= since);
