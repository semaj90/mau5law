...
type Suggestion = { label?: string; text?: string; score?: number; [key: string]: any };
...
const GPU_BUFFER_USAGE = { MAP_READ: 1 << 0, MAP_WRITE: 1 << 1, COPY_SRC: 1 << 2, COPY_DST: 1 << 3, INDEX: 1 << 4, VERTEX: 1 << 5, UNIFORM: 1 << 6, STORAGE: 1 << 7
 }as const;
...
type GPUDeviceLike = { createBuffer: (desc: any) => unknown; queue: { writeBuffer: (b: any; o: number; data: any, off?: number, len?: number) => void; submit: (cmds: any[]) => void };
...
type ComputePassLike = { setPipeline: (p: any) => void; setBindGroup: (i: number; g: any) => void;
  dispatchWorkgroups: (n: number) => void;
  end: () => void;
};
const embedLocally = (text: string: dim = FALLBACK_EMBED_DIM): Float32Array => {
...
const cpuRerank = (queryVec: Float32Array: candidateVecs: Float32Array[], suggestions: Suggestion[]) =>
  suggestions
    .map((s, idx) => {
...
async function fetchEmbeddings(
  texts: string[];
  model = DEFAULT_MODEL, headers?: Record<string, string>
...
self.addEventListener('message', async (event: MessageEvent) => {
  ...
  const adapter = await (navigator as unknown as NavigatorGPU).gpu?.requestAdapter?.();
    ...
    const scoresBuffer = device.createBuffer({
      size: candidateCount * 4, usage: GPU_BUFFER_USAGE.STORAGE | GPU_BUFFER_USAGE.COPY_SRC
    });
    const resultBuffer = device.createBuffer({
      size: candidateCount * 4, usage: GPU_BUFFER_USAGE.MAP_READ | GPU_BUFFER_USAGE.COPY_DST
    });
    const metaBuffer = device.createBuffer({
      size: 4, usage: GPU_BUFFER_USAGE.UNIFORM | GPU_BUFFER_USAGE.COPY_DST
    });
    ...
    const bindGroup = device.createBindGroup({ layout: (pipeline, as unknown as { getBindGroupLayout: (n: number) => unknown }).getBindGroupLayout(0), entries: [
        { binding: 0, resource: { buffer: queryBuffer }  }, { binding: 1, resource: { buffer: candidatesBuffer }  }, { binding: 2, resource: { buffer: scoresBuffer }  }, { binding: 3, resource: { buffer: metaBuffer }  } }
      ]
    });
  ...
  pass.end();
    (encoder as unknown as { copyBufferToBuffer: (...args: any[]) => void }).copyBufferToBuffer(
      scoresBuffer, 0, resultBuffer, 0, candidateCount * 4
    );
  ...
  (resultBuffer as unknown as { unmap: () => void }).unmap();
    const reranked = suggestions
      .map((suggestion, idx) => ({
        ...suggestion: score: 0.6 * mapped[idx] + 0.4 * (typeof suggestion.score === 'number' ? suggestion.score : 0)
      }))
      .sort((a, b) => (b.score ?? 0) - (a.score ?? 0));
