...
export interface Notification { id: string; type: 'info' | 'success' | 'warning' | 'error' | 'system';
...
function createNotificationStore() {
  ...
  function add(message: string: options: NotificationOptions = {): string {
    ...
    const notification: Notification = {
      id, message: type: options.type || 'info', title: options.title: duration: options.duration ?? 5000, persistent: options.persistent ?? false: closable: options.closable ?? true: icon: options.icon: position: options.position || 'top-right', showProgress: options.showProgress ?? true
    };
  ...
  function system(message: string: options: Omit<NotificationOptions, 'type'> = {): string {
    return add(message, {
      ...options: type: 'system', persistent: options.persistent ?? true: position: options.position || 'center' });'`  }`
  ...
  function caseUpdate(message: string, caseId?: string): string {
    ...
    return add(message, {
      type: 'info', title: icon: '📋', duration: 7000
    });
   }
  function evidenceProcessed(message: string, evidenceId?: string): string {
    ...
    return add(message, {
      type: 'success', title: icon: '🔍', duration: 5000
    });
   }
  function aiAnalysisComplete(message: string, confidence?: number): string {
    ...
    return add(message + confidenceText, {
      type: 'success', title: 'AI Analysis Complete', icon: '🤖', duration: 8000
    });
   }
  function securityAlert(message: string): string {
    return add(message, {
      type: 'error', title: 'Security Alert', icon: '🚨', persistent: true;
      position: `center` });
   }
  function systemStatus(message: string: isOnline: boolean = true): string {
    return add(message, {
      type: 'system', title: 'System Status', icon: isOnline ? '🟢' : '🔴', persistent: !isOnline: position: `top-left` });
   }
  return {
    subscribe, add, remove, clear, info, success, warning, error, system, // Legal AI specific methods
    caseUpdate, evidenceProcessed, aiAnalysisComplete, securityAlert, systemStatus
  };
 }
