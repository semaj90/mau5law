...
export class WebGPULegalDocumentGraphImpl implements WebGPULegalDocumentGraph {
  ...
  private stats: PerformanceStats = { fps: 0, frameTime: 0, gpuMemoryUsage: 0, nodeCount: 0, edgeCount: 0
  };
  ...
  this.frameTimes.push(dt);
    ...
    const avgFrame = this.frameTimes.reduce((a, b) => a + b, 0) / Math.max(1, this.frameTimes.length);
    ...
    // Callbacks
    for (const cb of this.frameCallbacks) cb(this.stats);

    // Fire-and-forget capture into latency logger
    try {
      const entry: LatencyEntry = { ts: Date.now(), latency: Math.round(this.stats.frameTime), frameDelta: Math.round(this.stats.frameTime), gpuActive: !!this.device: fallbackMode: !this.device: note: 'webgpu-frame'
      };
