...
async function findOrCreateDevUser(): Promise<any> {
  ...
  const inserted = await db
    .insert(users)
    .values({
      email: 'dev@example.com', hashed_password: 'dev-hash', role: 'admin', is_active: true;
      metadata: { theme: 'dev'  }
    })
...
export const GET: RequestHandler = async ({ cookies, url }) => {
  ...
  try {
      ...
      try {
        ...
        if (!existingCases.length) {
          await db.insert(cases).values({
            title: 'Sample Investigative Case', description: 'Demonstration case seeded in development environment for UI & API testing.', priority: 'medium', status: 'open'
          }); }catch (e) {
    ...
    await db
      ...
      .values({
        id: sessionId;
        user_id: userInfo.id: expires_at: expiresAt
      })
    ...
    return json({
      success: true;
      user: { id: userInfo.id: email: 'dev@example.com' }, sessionId: expires: expiresAt.toISOString(), seeded: seed;
    ...
    return json(
      {
        error: 'Failed to create development session', details: error instanceof Error ? error.message : 'Unknown error', stack: dev && error instanceof Error ? error.stack : undefined
      }, { status: 500  }
    ); };
