...
import type { vectors, vectorJobs, evidence, reports  } from '$lib/server/db/schema-postgres';
...
let redis: InstanceType<typeof createRedisInstance> | null = null;
try {
  redis = createRedisInstance();
} catch {
  console.warn(
    'createRedisInstance failed; continuing without low-level Redis. Use higher-level RedisCacheService for cached ops.'
  );
  redis = null;
}
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    return json({
      success: true,
      jobId,
      result: { message: `Vector ${event} completed successfully` },
    });
...
type SourceDataType = {
  ...
  metadata?: Record<string, unknown>; // added optional fields to avoid `any` casts
  evidenceType?: string;
  caseId?: string;
  tags?: string[];
  reportType?: string;
...
async function ensureQdrantCollectionFallback(
  collectionName: string,
  dimension: number
): Promise<void> {
  try {
    ...
    if (!checkResp.ok) {
      await fetch(`${qdrantUrl}/collections/${encodeURIComponent(collectionName)}`, {
        ...
        body: JSON.stringify({ vectors: { size: dimension, distance: 'Cosine' } }), // minimal default config; adjust if your platform uses HNSW or other params
      });
    }
  } catch (err) {
...
async function handleVectorUpsert(
  ...
  if (typeof qdrantExt.ensureCollection === 'function') {
    ...
    await ensureQdrantCollectionFallback(
      collectionName,
      pointVector.length || DEFAULT_VECTOR_DIMENSION
    );
  }
  ...
  if (typeof qdrantExt.upsertVector === 'function') {
    ...
    try {
      const qdrantUrl = (env.QDRANT_URL || 'http://localhost:6333').replace(/\/$/, '');
      const body = { points: [{ id: ownerId, vector: pointVector, payload }] };
      ...
      qdrantResult = resp.ok
        ? await resp.json()
        : { ok: false, status: resp.status, text: await resp.text() };
    } catch (err) {
      ...
      if (typeof qdrantExt.upsertVector === 'function') {
        ...
        await fetch(
          `${qdrantUrl}/collections/${encodeURIComponent(collectionName)}/points?wait=true`,
          {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              points: [{ id: ownerId, vector: [], payload: { deleted: true } }],
            }),
          }
        );
      }
    ...
    return json({
      ...
      services: {
        qdrant: {
          ...
          collections: collections?.result?.collections || collections || [],
        },
