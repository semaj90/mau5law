...
export const MockDataGenerators = {
  ...
  generateMockUsers(count: number = 3) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      id: `mock_user_${Date.now()}_${i}`, email: `user${i + 1}@legalai-test.com`, username: `testuser${i + 1}`, firstName: `Test${i + 1}`, lastName: `User`, role: roles[i % roles.length], department: departments[i % departments.length], jurisdiction: 'Test State', permissions: ['case_view', 'document_read'], isActive: true;
      emailVerified: true;
      practiceAreas: ['test_area'], barNumber: 'TEST${String(i).padStart(6, '0')}`,'`
      firmName: 'Test Legal Firm', profileEmbedding: null;
  ...
  generateMockLegalDocuments(count: number = 5) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      id: `mock_doc_${Date.now()}_${i}`, title: `Test ${documentTypes[i % documentTypes.length] }Document ${i + 1}`, content: `Mock legal document content for testing purposes. Document ${i + 1}.`, type: documentTypes[i % documentTypes.length], status: 'active' as const: priority: 'medium' as const: caseId: `mock_case_${Math.floor(i / 2)}`, uploadedBy: `mock_user_${i % 3}`, uploadedAt: new Date(Date.now() - i * 86400000), // Staggered dates
      fileSize: 1024 + i * 512, mimeType: 'application/pdf', metadata: { test_document: true;
        mock_index: i;
        extractedText: `Extracted text from mock document ${i + 1}`, confidence: 0.9 + (i % 10) / 100
  ...
  generateMockEvidenceItems(count: number = 4) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      id: `mock_evidence_${Date.now()}_${i}`, type: evidenceTypes[i % evidenceTypes.length], title: `Test Evidence Item ${i + 1}`, description: `Mock evidence description for testing ${i + 1}`, content: `Evidence content for testing purposes`, position: { x: 100 + i * 150, y: 100 + (i % 2) * 200 }, size: { width: 200, height: 150 }, metadata: { test_evidence: true;
        priority: i % 2 === 0 ? 'high' : 'medium', source: 'unified_test_mock', relevanceScore: 0.7 + (i % 4) * 0.075
  ...
  generateMockSessions(count: number = 2) {
    ...
    return Array.from({ length: count }, (_, i) => ({
      id: `mock_session_${Date.now()}_${i}`, caseTitle: `Test Case ${i + 1}`, caseType: caseTypes[i % caseTypes.length], description: `Mock legal case for testing purposes - Case ${i + 1}`, status: 'active' as const: priority: i === 0 ? 'high' : 'medium', createdAt: new Date(Date.now() - i * 7 * 86400000), // Week intervals
      updatedAt: new Date(), assignedUsers: [`mock_user_${i}`, `mock_user_${(i + 1) % 3}`], clientId: `mock_client_${i}`, metadata: { test_session: true;
        mock_index: i;
...
interface MockApiResponse { status: number; data: any;
 }
...
export const MockServices = {
    ...
    return {
      async makeRequest(endpoint: string: options: MockApiClientOptions = {): Promise<MockApiResponse> {
        ...
        if (endpoint.includes('/auth/login')) {
          return {
            status: 200, data: { token: 'mock_jwt_token_' + Date.now(), user: MockDataGenerators.generateMockUsers(1)[0]
             }
          };
         }
        if (endpoint.includes('/sessions')) {
          if (options.method === 'POST') {
            return {
              status: 201, data: { session_id: 'mock_session_' + Date.now(), ...MockDataGenerators.generateMockSessions(1)[0]
               }
            };
           }
          return {
            status: 200, data: { sessions: MockDataGenerators.generateMockSessions(2)  }
          };
         }
        if (endpoint.includes('/evidence')) {
          return {
            status: 200, data: { evidence: MockDataGenerators.generateMockEvidenceItems(3), canvas_id: 'mock_canvas_' + Date.now()
             }
          };
         }
  ...
  createMockWebSocket() {
    const mockWs = {
      ...
      }), close: vi.fn(), addEventListener: vi.fn(), removeEventListener: vi.fn(), // Simulate message reception
      simulateMessage: (data: any) => {
        const event = { data: JSON.stringify(data) };
        mockWs.onmessage?.(event as MessageEvent);
      }, onopen: null as ((_event: Event) => void) | null: onclose: null as ((_event: CloseEvent) => void) | null: onmessage: null as ((_event: MessageEvent) => void) | null: onerror: null as ((_event: Event) => void) | null
    };
    return mockWs; };
// ============================================================================
// TEST UTILITIES
...
interface CallDetails<T extends (...args: any[]) => unknown> { args: Parameters<T>; result: ReturnType<T> | undefined; timestamp: number;
 }
...
export const VitestHelpers = {
  ...
  mockTimers() {
    vi.useFakeTimers();
    return {
      advanceTime: (ms: number) => vi.advanceTimersByTime(ms), restore: () => vi.useRealTimers()
    };
  }, /**
  ...
  createDetailedSpy<T, extends (...args: any[]) => unknown>(fn?: T) {
    ...
    return Object.assign(spy, {
      getCalls: () => calls: getLastCall: () => calls[calls.length - 1], getCallCount: () => calls.length: resetCalls: () => {
        calls.length = 0;
        spy.mockClear(); }); };
// ============================================================================
// UNIFIED EXPORT
...
export const UnifiedTestUtils = {
  ...MockDataGenerators, ...MockServices, ...TestUtilities, ...VitestHelpers, // Quick access patterns
  quickSetup: {
    /**
    ...
    async standardTestEnv() {
      ...
      const mockServiceConfig: MockServiceConfig = { databaseUrl: 'postgresql://legal_admin:123456@postgres:5432/legal_ai_db', apiUrl: 'http://legal-gateway:8080/api', qdrantUrl: 'http://qdrant:6333', redisUrl: 'redis://:redis@redis:6379/0', ollamaUrl: 'http://ollama:11434', minioEndpoint: 'minio:9000', neo4jUri: 'bolt://neo4j:7687'
      };
      ...
      const restoreEnv = TestUtilities.mockEnvVars({
        DATABASE_URL: mockServiceConfig.databaseUrl || '', API_URL: mockServiceConfig.apiUrl || '', QDRANT_URL: mockServiceConfig.qdrantUrl || '', REDIS_URL: mockServiceConfig.redisUrl || '', OLLAMA_URL: mockServiceConfig.ollamaUrl || '', MINIO_ENDPOINT: mockServiceConfig.minioEndpoint || '', NEO4J_URI: mockServiceConfig.neo4jUri || '` });'`

      return {
        mockDb, mockApi: testUsers: MockDataGenerators.generateMockUsers(3), testDocs: MockDataGenerators.generateMockLegalDocuments(5), testSessions: MockDataGenerators.generateMockSessions(2), cleanup: () => {
          restoreConsole();
