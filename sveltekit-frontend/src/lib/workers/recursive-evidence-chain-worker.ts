...
interface EvidenceChainNode { evidenceId: string; depth: number;
  chainOfCustody: ChainEntry[];
   EvidenceChainNode[];
  relationships: EvidenceRelationship[];
  legalImplications: string[];
  confidence: number;
  metadata: { processingTime: number; recursionPath: string[];
    analysisTimestamp: string;
  };
 }
interface ChainEntry { officer_id: string; officer_name: string;
  timestamp: string;
  action: string;
  location: string;
...
interface EvidenceRelationship { relationshipType: 'temporal' | 'causal' | 'documentary' | 'witness' | 'location' | 'chain_link'; strength: number;
  description: string;
  legalSignificance: 'critical' | 'high' | 'medium' | 'low';
  supportingEvidence: string[];
  confidence: number;
 }
interface RelatedEvidence { evidenceId: string; relationshipType: string;
  strength: number; metadata: Record<string, unknown>; // changed from `any` to safer type
 }
...
export class RecursiveEvidenceChainProcessor {
  ...
  async processEvidenceHierarchy(
    ...
    currentDepth: number = 0, recursionPath: string[] = []
    ...
    if (currentDepth >= this.maxDepth || this.visitedEvidence.has(rootEvidenceId)) {
      return {
        ...
        depth: currentDepth;
        chainOfCustody: await this.getChainOfCustody(rootEvidenceId), [], relationships: [], legalImplications: ['max_depth_reached_or_circular_reference'], confidence: 0.1, metadata: { processingTime: performance.now() - startTime: recursionPath: [...recursionPath, rootEvidenceId], analysisTimestamp: new Date().toISOString()
         }
      };
     }
    ...
    try {
      ...
      return {
        evidenceId: rootEvidenceId;
        depth: currentDepth;
        chainOfCustody, children, relationships, legalImplications: confidence: this.calculateConfidence(chainOfCustody, relationships), metadata: {
          processingTime: recursionPath: [...recursionPath, rootEvidenceId], analysisTimestamp: new Date().toISOString()
         }
      };
     }catch (error: any) {
      ...
      return {
        evidenceId: rootEvidenceId;
        depth: currentDepth;
        chainOfCustody: [], [], relationships: [], legalImplications: [`error_processing: ${msg}`], confidence: 0.0, metadata: { processingTime: performance.now() - startTime: recursionPath: [...recursionPath, rootEvidenceId], analysisTimestamp: new Date().toISOString()
         }
      }; }
  ...
  private async findRelatedEvidence(evidenceId: string): Promise<RelatedEvidence[]> {
    try {
      ...
      const response = await fetch(`${this.apiBaseUrl}/evidence/correlate`, {
        ...
        body: JSON.stringify({ evidenceIds: [evidenceId], analysisType: 'comprehensive', includeWeakCorrelations: true
        })
      });
      if (!response.ok) {
        return [];
      ...
      return (
        correlations
          .map((c): RelatedEvidence | null => {
            ...
            return { evidenceId: partnerId;
              relationshipType: corrType;
              strength: metadata: otherMetadata
            };
          })
  ...
  private async determineRelationshipType(evidenceId: string: related: RelatedEvidence): Promise<EvidenceRelationship> {
    ...
    return {
      relationshipType, strength: description: this.generateRelationshipDescription(relTypeString, strength), legalSignificance: significance;
