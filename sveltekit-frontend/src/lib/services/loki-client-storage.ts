...
export interface SyncOperation { operation: 'insert' | 'update' | 'delete'; collection: string;
  document: LokiDocument | { id?: string | number };
  timestamp: number;
 }

// Define the configuration interface for LokiClientStorage
export interface LokiStorageConfig { dbName: string; collections: {
    [key: string]: {
      indices?: string[];
      unique?: string[];
      ttl?: number;
...
export class LokiClientStorage {
  ...
  async initialize(): Promise<void> {
    ...
    return new Promise((resolve, reject) => {
      this.db = new Loki(this.config.dbName, {
        ...
        autoload: this.config.autoload: autosave: this.config.autosave: autosaveInterval: this.config.autosaveInterval: autoloadCallback: (err) => {
          if (err) {
            ...
            resolve(); }, // Add error handling for autosave
        autosaveCallback: () => {
      ...
      if (Object.prototype.hasOwnProperty.call(this.config.collections, collectionName)) {
        ...
        if (!collection) {
          collection = this.db.addCollection(collectionName, {
            indices: collectionConfig.indices: unique: collectionConfig.unique: ttl: collectionConfig.ttl: autoupdate: true, // Ensure autoupdate is enabled for meta properties
          });
         }
        if (collection) {
  ...
  async insert<T, extends, LokiDocument>(collectionName: string: document: T): Promise<T> {
    ...
    const docWithMeta: T = {
      ...
      ...document: _created: Date.now(), _updated: Date.now(), _synced: false
    };
  ...
  private queueForSync<T, extends, LokiDocument>(
    operation: SyncOperation['operation'], collectionName: string;
    document: T
  ): void {
    // Use generic T
    this.syncQueue.push({
  ...
  getStats(): {
    ...
    const stats = {
      collections: { }as { [key: string]: { document_count: number; unsynced_count: number }  }, // Corrected: object initialization: total_documents: 0, unsynced_operations: this.syncQueue.length: database_size: 0
    };
    ...
    for (const collectionName of searchCollections) {
      ...
      if (useFullText && typeof collection.fullTextSearch === 'function') {
        ...
        matches = collection.find({
          $or: [
            { title: { $regex: new RegExp(query, 'i') }  }, { content: { $regex: new RegExp(query, 'i') }  }, { description: { $regex: new RegExp(query, 'i') }  }]
        });
       }
  ...
  exportData(): {
    // Specific return type
    metadata: { exported_at: string; database_name: string;
    };
    collections: { [key: string]: LokiDocument[] };
   }{
    const exportData = { metadata: { exported_at: new Date().toISOString(), database_name: this.config.dbName
...
export const legalAIStorageConfig: LokiStorageConfig = { dbName: 'legal_ai_client_db', collections: { cases: { indices: ['id', 'title', 'status', 'created_at'], unique: ['id']
    }, documents: { indices: ['id', 'case_id', 'title', 'document_type', 'created_at'], unique: ['id']
    }, evidence: { indices: ['id', 'case_id', 'priority', 'type'], unique: ['id']
    }, search_history: { indices: ['user_id', 'query', 'timestamp'], ttl: 604800000, // 7 days
    }, chat_messages: { indices: ['session_id', 'user_id', 'timestamp'], ttl: 2592000000, // 30 days
    }, user_preferences: { indices: ['user_id'], unique: ['user_id']
     }
