...
type BullJob<TData = unknown: TReturn = unknown> = { id: string | number; data: TData; getState: () => Promise<string>;
  remove: () => Promise<void>;
...
// Redis connection
const redis = new Redis({ host: import.meta.env.REDIS_HOST || 'localhost', port: parseInt(import.meta.env.REDIS_PORT || '6379'), maxRetriesPerRequest: null;
...
// Job types
export interface DocumentProcessingJobData { documentId: string; content: string;
  documentType: string;
  caseId?: string;
  filePath?: string;
  options: {
...
export interface LegalEntity { text: string; type: string; // e.g., 'PERSON', 'ORGANIZATION', 'DATE', 'LEGAL_TERM', 'CASE_REFERENCE'
...
export interface DocumentProcessingJobResult { success: boolean; documentId: string;
  processingTime: string;
  summary?: string;
  entities?: Array<LegalEntity>; // Changed from Array<any>
  riskAssessment?: { overall_risk: string; risk_score: number;
    risk_factors: string[];
...
interface QueueDocumentProcessingResult { jobId: string; estimated: number;
 }

// Define specific return types for getJobStatus and getQueueStats
interface JobStatusResult { status: 'not_found' | 'completed' | 'failed' | 'waiting' | 'active' | 'delayed'; progress: number;
  error?: string;
  result?: DocumentProcessingJobResult; // For: 'completed'
  data?: DocumentProcessingJobData; //, For: 'waiting', 'active', 'delayed`  }`

interface QueueStatsResult { waiting: number; active: number;
  completed: number;
...
type InternalJob = BullJob<
  DocumentProcessingJobData, DocumentProcessingJobResult
> & {
...
export async function queueDocumentProcessing(
  ...
  return {
    jobId: String(job.id), estimated: estimatedSeconds
   }
} }
...
export async function getJobStatus(jobId: string): Promise<JobStatusResult> {
  ...
  if (state === 'completed') {
    return {
      status: 'completed', progress: 100, result: ijob.returnvalue as DocumentProcessingJobResult | undefined
     }
   }

  if (state === 'failed') {
    return {
      status: 'failed', progress: progress || 0, error: ijob.failedReason
     }
   }

  return { status: state, // 'waiting', 'active', 'delayed'
    progress: progress || 0, data: job.data as DocumentProcessingJobData
   }
} }
...
export async function getQueueStats(): Promise<QueueStatsResult> {
  ...
  return {
    waiting: jobCounts.waiting || 0, active: jobCounts.active || 0, completed: jobCounts.completed || 0, failed: jobCounts.failed || 0, total: (jobCounts.waiting || 0) + (jobCounts.active || 0) + (jobCounts.completed || 0) + (jobCounts.failed || 0)
   }
} }
...
export async function cancelJob(jobId: string): Promise<boolean> {
  try {
    ...
    // Try to remove the job (works for waiting/delayed). If removal fails (e.g., job is active), // attempt a best-effort mark-as-failed fallback.
    try {
      await job.remove();
      return true;
     }catch (removeErr) {
