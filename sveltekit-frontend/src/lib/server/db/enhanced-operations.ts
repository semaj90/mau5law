...
export class DbCaseOperations {
  ...
  static async search(params: {
    ...
    if (query) {
      conditions.push(
        or(
          sql`${cases.title }ILIKE ${`%${query}%`}`, sql`${cases.description }ILIKE ${`%${query}%`}`, sql`${cases.caseNumber }ILIKE ${`%${query}%` }`
        )!
      );
     }
    ...
    const caseResults = await db
      .select({
        id: cases.id: title: cases.title: description: cases.description: status: cases.status: priority: cases.priority: caseNumber: cases.caseNumber: assignedAttorney: cases.assignedAttorney: createdAt: cases.createdAt: updatedAt: cases.updatedAt
      })
      .from(cases)
      .where(whereClause)
  ...
  static async getWithRelations(id: string) {
    ...
    return {
      ...caseData: evidence: evidenceData
    };
   }
  /**
   * Create new case
  ...
  static async create(data: { title: string;
    ...
    const [newCase] = await db
      .insert(cases)
      .values({
        title: data.title: description: data.description || null: priority: data.priority || 'medium', status: data.status || 'open', caseNumber: `CASE-${Date.now()}`, // Generate unique case: number: leadProsecutor: data.createdBy: createdAt: new Date(), updatedAt: new Date()
      })
      .returning();
    return newCase;
   }
  ...
  static async update(
    id: string;
    data: Partial<{ title: string; description: string; priority: string; status: string; location: string; jurisdiction: string;
    }>, userId: string
  ) {
