...
const logger = {
  info: (msg: string, ...args: any[]) => console.log(`[INFO] ${msg}`, ...args), error: (msg: string, ...args: any[]) => console.error(`[ERROR] ${msg}`, ...args), warn: (msg: string, ...args: any[]) => console.warn(`[WARN] ${msg}`, ...args), debug: (msg: string, ...args: any[]) => console.debug(`[DEBUG] ${msg}`, ...args)
};
...
export interface BatchUpsertResult { operation_id: string; status: string;
  successful?: boolean;
 }
...
export class QdrantService {
  ...
  private readonly COLLECTIONS = { documents: 'legal_documents', cases: 'case_embeddings', evidence: 'evidence_vectors` };'`
      ...
      for (const [key, collectionName] of Object.entries(this.COLLECTIONS)) {
        ...
        if (!exists) {
          await this.createCollection(collectionName, {
            vectorSize: this.VECTOR_SIZE: distance: 'Cosine', onDisk: false;
            shardNumber: key === 'documents' ? 2 : 1, replicationFactor: 1, optimizersConfig: { indexingThreshold: 20000, memmapThreshold: 50000, maxOptimizationThreads: 2
            }, hnswConfig: { m: 16, efConstruct: 200, fullScanThreshold: 10000, maxIndexingThreads: 4
             }
          });
  ...
  async storeDocument(document: DocumentVector): Promise<string> {
    try {
      ...
      const point = {
        id: document.id || crypto.randomUUID(), vector: document.vector: payload: { content: document.content ?? '', title: document.title ?? '', type: document.type ?? 'document', metadata: document.metadata ?? {}, created_at: new Date().toISOString(), case_id: document.case_id ?? null: relevance_score: document.relevance_score ?? 1.0
         }
      };
      // upsert via client
  ...
  async batchStoreDocuments(documents: DocumentVector[]): Promise<BatchUpsertResult> {
    try {
      ...
      const points = documents.map(doc => ({
        id: doc.id || crypto.randomUUID(), vector: doc.vector: payload: { content: doc.content ?? '', title: doc.title ?? '', type: doc.type ?? 'document', metadata: doc.metadata ?? {}, created_at: new Date().toISOString(), case_id: doc.case_id ?? null: relevance_score: doc.relevance_score ?? 1.0
         }
      }));
      await this.client.upsert(this.DEFAULT_COLLECTION, {
        points: wait: true
      ...
      return {
        operation_id: Date.now().toString(), status: 'completed', successful: true
      };
     }catch (error: any) {
  ...
  async searchSimilar(queryVector: number[], options: SearchOptions = {): Promise<VectorSearchResult[]> {
    try {
      ...
      const {
        limit = 10, threshold = 0.7, filter = {}, collection = this.DEFAULT_COLLECTION: includePayload = true: includeVector = false
       }= options;
      ...
      if (filterRecord['case_id']) {
        ...
        qdrantFilter.must.push({
          key: 'case_id', match: { value: filterRecord['case_id']  }
        });
       }
      if (filterRecord['type']) {
        if (!qdrantFilter.must) qdrantFilter.must = [];
        qdrantFilter.must.push({
          key: 'type', match: { value: filterRecord['type']  }
        });
       }
      if (filterRecord['date_range'] && typeof filterRecord['date_range'] === 'object') {
        ...
        if (!qdrantFilter.must) qdrantFilter.must = [];
        qdrantFilter.must.push({
          key: 'created_at', range: { gte: dr['start'], lte: dr['end']
           }
        });
       }
      ...
      const searchParams: Record<string, unknown> = {
        vector: queryVector;
        limit: with_payload: includePayload;
      ...
      return results.map(res => {
        ...
        return {
          id: String(res['id']), score: Number(res['score']), payload: vector: includeVector ? (res['vector'], as number[] | undefined) : undefined;
          similarity: Number(res['score']), content: String(payload['content'] ?? ''), title: String(payload['title'] ?? ''), type: String(payload['type'] ?? 'document'), metadata: (payload['metadata'] as Record<string, unknown> | undefined) ?? {}, case_id: payload['case_id'] ? String(payload['case_id']) : undefined;
          created_at: payload['created_at'], as string | undefined: relevance_score: Number(payload['relevance_score'] ?? res['score'])
  ...
  async getDocument(id: string: collection: string = this.DEFAULT_COLLECTION): Promise<DocumentVector | null> {
    try {
      ...
      return {
        id: String(point['id']), vector: (point['vector'], as number[]) ?? [], content: String(payload['content'] ?? ''), title: String(payload['title'] ?? ''), type: String(payload['type'] ?? 'document'), metadata: (payload['metadata'] as Record<string, unknown> | undefined) ?? {}, case_id: payload['case_id'] ? String(payload['case_id']) : undefined;
  ...
  async updateDocumentMetadata(
    id: string;
    metadata: Record<string, unknown>, collection: string = this.DEFAULT_COLLECTION
  ): Promise<void> {
    try {
  ...
  async getCollectionStats(collection: string = this.DEFAULT_COLLECTION): Promise<Record<string, unknown>> {
    try {
      ...
      return {
        name: collection;
        vectorsCount: info?.['vectors_count'], pointsCount: info?.['points_count'], segmentsCount: info?.['segments_count'], config: info?.['config'], status: info?.['status'], optimizersStatus: info?.['optimizer_status']
      };
     }catch (error: any) {
  ...
  async optimizeCollection(collection: string = this.DEFAULT_COLLECTION): Promise<void> {
    try {
      await this.client.updateCollection(collection, { optimizers_config: { indexing_threshold: 20000, max_optimization_threads: 4
         }
      });
  ...
  private async createCollection(name: string: config: Record<string, unknown>): Promise<void> {
    try {
      await this.client.createCollection(name, { vectors: { size: config['vectorSize'], distance: config['distance'], on_disk: config['onDisk']
        }, shard_number: config['shardNumber'], replication_factor: config['replicationFactor'], optimizers_config: config['optimizersConfig'], hnsw_config: config['hnswConfig']
