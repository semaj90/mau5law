...
<BitsDialog.Root {open} onOpenChange={handleOpenChange}>
  ...
  <BitsDialog.Portal>
    <!-- Overlay -->
    <BitsDialog.Overlay class={cn(defaultOverlayClass, overlayClass)} />
    <!-- Content -->
    <BitsDialog.Content class={cn(defaultContentClass, contentClass, className)}>
      <!-- Close button -->
      <BitsDialog.Close
      ...
      {#if title}
        <div class="flex flex-col space-y-1.5 text-center sm:text-left">
          <BitsDialog.Title class="text-lg font-semibold leading-none tracking-tight">
            {title}
          </BitsDialog.Title>
        </div>
      {/if}
      <!-- Description -->
      {#if description}
        <BitsDialog.Description class="text-sm text-gray-500 dark:text-gray-400">
          {description}
