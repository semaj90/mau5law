      ...
      for (let i = 0; i < k; i++) {
        ...
        if (clusterErrors.length > 0) {
          const cluster: Cluster = {
            ...
            rootCause: await this.identifyRootCause({ id: '', errors: clusterErrors, rootCause: '', impact: 0, createdAt: new Date() }),
            impact: await this.calculateImpact({ id: '', errors: clusterErrors, rootCause: '', impact: 0, createdAt: new Date() }),
            createdAt: new Date(),
          };
