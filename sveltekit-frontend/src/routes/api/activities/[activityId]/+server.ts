...
export const GET: RequestHandler = async ({ params, locals }) => {
  try {
    ...
    const activityResult = await db.select().from(caseActivities).where(eq(caseActivities.id, activityId)).limit(1);
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    type UpdateData = Partial<{ title: string | null; description: string | null;
      activityType: string | null;
  ...
  try {
    ...
    if (data.operation === 'complete') {
      ...
      if (!currentEvidence.includes(data.evidenceId)) {
        updateData.relatedEvidence = [...currentEvidence, data.evidenceId]; }else if (data.operation === 'removeEvidence') {
      const currentEvidence = (existingActivity[0].relatedEvidence as string[]) || [];
      ...
      if (!currentCriminals.includes(data.criminalId)) {
        updateData.relatedCriminals = [...currentCriminals, data.criminalId]; }else if (data.operation === 'removeCriminal') {
      const currentCriminals = (existingActivity[0].relatedCriminals as string[]) || [];
