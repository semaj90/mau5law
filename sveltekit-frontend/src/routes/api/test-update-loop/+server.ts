...
class UpdateLoopTester {
  ...
  private async runTestScenario(scenarioName: string) {
    ...
    try {
      ...
      // ensure numeric values for DB columns (drizzle expects number for user_id)
      const userIdNum = this.testResults.testUserId != null ? Number(this.testResults.testUserId) : undefined;
      ...
      await DUL.queueDocumentUpdate(document.id, scenario.originalContent);
  ...
  private async testSearchImpact() {
    ...
    try {
      ...
      for (const documentId of this.testDocumentIds) {
        ...
        const testQuery = this.generateTestQuery(doc.content_text || '');
  ...
  private async analyzePerformance() {
    ...
    try {
      // Get queue status (use adapter DUL)
      const queueStatus = await DUL.getQueueStatus();

      // Safely get step values with proper typing
      const stepValues = Object.values(this.testResults.steps ?? {}) as StepResult[];
      ...
      const successfulUpdates = stepValues.filter(step => step.status === 'success').length;

      const priorityDistribution = stepValues.reduce(
