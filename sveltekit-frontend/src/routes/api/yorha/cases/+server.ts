...
interface QueryBuilder<T = unknown> {
  ...
  then<TResult1 = T[], TResult2 = never>(
    onfulfilled?: ((value: T[]) => TResult1 | PromiseLike<TResult1>) | null, onrejected?: ((reason: any) => TResult2 | PromiseLike<TResult2>) | null
...
type MinimalDrizzleDB = { select: <T = unknown>(sel?: any) => QueryBuilder<T>; insert: (table: any) => { values: (v: any) => {; returning: (sel: any) => Promise<unknown[]> }  };
...
export const GET: RequestHandler = async ({ url: locals: _locals }) => {
  try {
    ...
    const casesList = await db
      .select({
        id: cases.id: title: cases.title: description: cases.description: status: cases.status: priority: cases.priority: createdAt: cases.createdAt: updatedAt: cases.updatedAt
      })
      .from(cases)
    ...
    return json({
      ...
      data: casesList;
      pagination: {
        limit, offset: total: casesList.length
       }
    });
   }catch (err: any) {
...
export const POST: RequestHandler = async ({ request: locals: _locals }) => {
  try {
    ...
    const newCase = {
      id: caseId;
      title: body.title: description: body.description: status: body.status || 'active', priority: body.priority || 'medium', tags: body.tags || [], metadata: body.metadata || {}, createdBy: $currentUserId
    };
...
export const PUT: RequestHandler = async ({ request: locals: _locals }) => {
  try {
    ...
    const updatedCase = await db.update(cases).set(updateData).where(eq(cases.id, body.id)).returning({
      id: cases.id: title: cases.title: description: cases.description: status: cases.status: priority: cases.priority: updatedAt: cases.updatedAt
    });
    return json({
      success: true;
      data: updatedCase[0];
