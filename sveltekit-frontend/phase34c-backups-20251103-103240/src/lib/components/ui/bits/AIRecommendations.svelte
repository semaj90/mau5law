...
<script lang="ts">
  // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { UiCard, as Card, UiCardHeader as CardHeader, UiCardTitle as CardTitle, UiCardContent as CardContent } from '$lib/index.js'; interface Props { userContext?: any; neo4jContext?: any; analyticsLog?: (_event: any) => void; onRecommendations?: (results: any) => void}
  const {
    userContext = null,
    neo4jContext = null,
    analyticsLog = () => {},
...
<div class="w-full">
  <div class="yorha-panel-header"><h3 class="nes-text">AI Recommendations</h3></div>
      ...
      <ul class="space-y-2">
        {#each Array.isArray(recommendations) ? recommendations : [] as rec}
          <li class="border-b last:border-b-0">
            ...
            <div class="text-xs">Score: {rec.finalScore ?? rec.rerankScore}</div>
          </li>
        {/each}
      </ul>
    {/if}
