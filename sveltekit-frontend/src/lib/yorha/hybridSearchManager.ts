...
export async function refreshRemote(opts: RefreshOpts = {): Promise<any> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const raw =
        (data as { results?: any; documents?: any; matches?: any }).results ||
        (data as { results?: any; documents?: any; matches?: any }).documents ||
        [];
      ...
      lastRefresh = Date.now(); }catch (e: any) {
...
export async function reRankWithPgVector(
  ...
  try {
    ...
    const scores: any[] =
      (data as { results?: any; documents?: any; matches?: any }).results ||
      (data as { results?: any; documents?: any; matches?: any }).matches ||
      [];
