<script lang="ts">
  ...
  function handleKeydown(_event: KeyboardEvent) {
    ...
    const shortcut = `${_event.ctrlKey ? 'Ctrl+' : ''}${_event.altKey ? 'Alt+' : ''}${_event.shiftKey ? 'Shift+' : ''}${_event.key.toUpperCase()}`;
    ...
    <div class="flex items-center">
      ...
      <div class="flex items-center space-x-1">
        {#each group.actions as action (action.id)}
          ...
          {:else if action.type === 'dropdown'}
            <Select.Root
              onValueChange={(v: string) => {
                if (!v) return;
                const selectedOption = action.options?.find((o) => o.value === v);
                if (selectedOption) {
            ...
            >
              ...
              >
                ...
                {#if showLabels && !compact}
                  <span class="ml-2 text-sm">{action.label}</span>
                {/if}
                <Select.Icon class="ml-1 text-xs" />
              </Select.Trigger>
              <Select.Content
                class={`
          ...
          {:else}
            <!-- Regular Button or Toggle -->
            <Tooltip content={`${action.label}${action.shortcut ? ` (${action.shortcut})` : ''}`}
              {...(theme === 'default' || theme === 'legal' || theme === 'gaming' ? { theme } : {})}
            >
              {#snippet children()}
                <button
                  ...
                  class={`
                    flex items-center px-3 py-2 rounded transition-all duration-200
                    ${themeClasses[theme].button}
                    ${action.active ? themeClasses[theme].activeButton : ''}
                    ${action.disabled ? 'opacity-50 cursor-not-allowed' : ''}
                    ${compact ? 'px-2 py-1' : ''}
                    ${theme === 'yorha' ? 'font-mono tracking-wide' : ''}
                  `}
