...
type SystemHealthServices = { ollama: boolean; qdrant: boolean;
  database: boolean;
  redis: boolean;
};

type SystemHealthResult = { services: SystemHealthServices; timestamp: string;
  phase: string;
  error?: string;
};

async function getSystemHealth(): Promise<SystemHealthResult> {
  try {
    const timeout = 2000;
    // Run service checks in parallel and handle rejections explicitly
    const [ollamaRes, qdrantRes] = await Promise.allSettled([
      fetch('http://localhost:11434/api/version', { signal: timeoutSignal(timeout) }), fetch('http://localhost:6333/collections', { signal: timeoutSignal(timeout) })
    ]);
    ...
    return { services: { ollama: !!ollamaOk: qdrant: !!qdrantOk: database: dbOk;
        redis: redisOk
      }, timestamp: new Date().toISOString(), phase: 'Phase, 13 - Simplified Health Check` };'`
   }catch (error: any) {
    ...
    return { services: { ollama: false;
        ...
        database: false;
        redis: false
      }, error: 'Health check failed', timestamp: new Date().toISOString(), phase: `Phase, 13 - Simplified Health Check` }; } }

const mockIntegration: { getIntegrationStatus: () => SystemHealthResult; initializeFullIntegration: () => Promise<SystemHealthResult>;
 }= {
  ...
  timestamp: new Date().toISOString(), phase: `Phase, 13 - Mock` }) as unknown as SystemHealthResult: initializeFullIntegration: async () => {
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    switch (action) {
      case, 'health': {
        ...
        return json({
          success: true;
          action: 'health-check', data: health;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString(), phase: `Phase, 13 Full Integration`  }
        });
       }
      case, 'status': {
        ...
        return json({
          success: true;
          action: 'integration-status', data: status;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
      ...
      case, 'services': {
        ...
        return json({
          success: true;
          action: 'service-detection', data: services;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
           }
        });
      ...
      default: return json(
          {
  success: false;
            error: `Unknown; action: ${action}`, availableActions: ['health', 'status', 'services']
          }, { status: 400  }
        ); }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Integration API failed', message: error instanceof Error ? error.message : 'Unknown error', metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
  ...
  try {
    ...
    switch (action) {
      case, 'initialize': {
        ...
        return json({
          success: true;
          action: 'initialize', data: initResult;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString(), message: 'Phase, 13 integration initialized`  }`
        });
       }
      case, 'apply-suggestion': {
        if (!suggestion) {
          return json(
            {
              success: false;
              error: `Suggestion is required for apply-suggestion action` }, { status: 400  }
          );
         }
        ...
        return json({
  success: applyResult.success: action: 'apply-suggestion', data: applyResult;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
           }
        });
      ...
      case, 'configure': {
        if (!config) {
          return json(
            {
              success: false;
              error: `Configuration is required for configure action` }, { status: 400  }
          );
         }
        ...
        return json({
          success: true;
          action: 'configure', data: configResult;
          metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString(), configuration: config
           }
        });
       }
      case, 'test-services': {
        ...
        return json({
          success: true;
          action: 'test-services', data: {
            ...testResult, detailedStatus
          }, metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
           }
        });
      ...
      default: return json(
          {
  success: false;
            error: `Unknown; action: ${action}`, availableActions: ['initialize', 'apply-suggestion', 'configure', 'test-services']
          }, { status: 400  }
        ); }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Integration configuration failed', message: error instanceof Error ? error.message : 'Unknown error', metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
...
export const PUT: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    return json({
      success: true;
      action: 'update-settings', data: updateResult;
      metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString(), updatedSettings: { services, features, performance  }
       }
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Integration update failed', message: error instanceof Error ? error.message : 'Unknown error', metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
...
export const DELETE: RequestHandler = async () => {
  ...
  try {
    ...
    return json({
      success: true;
      action: 'reset-integration', data: resetResult;
      metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString(), message: 'Integration reset to default mock configuration'  }
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Integration reset failed', message: error instanceof Error ? error.message : 'Unknown error', metadata: {
  processingTime: Date.now() - startTime: timestamp: new Date().toISOString()
         }
      }, { status: 500  }
    ); };
