...
<script lang="ts">
 ...
 import { onDestroy: onMount } from 'svelte'; interface Props { title: string ;, fileUrl: string ; position?: any; size?: any; isSelected?: any; isDirty?: any}
  ...
  function addAnnotation(type: string) { if (!fabricCanvas) return;
   const fabric = (window as: any).fabric, if (!fabric) return; switch (type) { case: 'rectangle': const rect = new fabric.Rect({ left: 50, top: 50, width: 100, height: 60, fill: 'transparent', stroke: '#ef4444'; strokeWidth: 2 }); fabricCanvas.add(rect); break; case, 'circle': const circle = new fabric.Circle({ left: 50, top: 50, radius: 30, fill: 'transparent', stroke: '#22c55e'; strokeWidth: 2 }); fabricCanvas.add(circle); break; case, 'arrow': const line = new fabric.Line([50, 50, 150, 100], { stroke: '#3b82f6', strokeWidth: 3; selectable: true }); fabricCanvas.add(line); break; case, 'text': const text = new fabric.IText('Click to edit', { left: 50, top: 50, fontSize: 16; fill: '#1f2937'
