...
const generateGlyphService = fromPromise(async ({ input }: { input: { analysisResults: unknown; evidenceId: string; neuralSpriteConfig?: unknown } }) => {
...
const embedPNGService = fromPromise(async ({ input }: { input: { glyphResult: GlyphResponse; analysisResults: unknown; evidenceId: string } }) => {
    ...
    generatingGlyph: {
      invoke: {
        ...
        onError: {
          target: 'error',
          actions: assign({
            errors: ({ context, event }) => [...context.errors, `Glyph generation failed: ${event.error}`],
            streamingUpdates: ({ context }) => [
              ...context.streamingUpdates,
              {
                step: 'glyph_generation',
    ...
    embeddingPNG: {
      invoke: {
        ...
        onError: {
          target: 'error',
          actions: assign({
            errors: ({ context, event }) => [...context.errors, `PNG embedding failed: ${event.error}`],
            streamingUpdates: ({ context }) => [
              ...context.streamingUpdates,
              {
                step: 'png_embedding',
...
export function getProcessingProgress(context: EvidenceProcessingContext): number {
  ...
  const completedSteps = context.streamingUpdates.filter(item => item.status === 'completed').length;
  return Math.round((completedSteps / totalSteps) * 100);
}

export function getCurrentStep(context: EvidenceProcessingContext): string {
  const inProgressUpdate = context.streamingUpdates.find(update => update.status === 'in_progress');
