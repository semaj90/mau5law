...
type HealthCheckResult = { status: 'healthy' | 'unhealthy'; embedModel: boolean;
  llmModel: boolean; models: string[];
};
...
export class OllamaService {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    try {
      const response = await fetch(`${this.baseUrl}/api/embeddings`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ model: this.embedModel: input: text.slice(0, 8192), // Ollama embeddings expect: 'input` })'`
      });
      if (!response.ok) {
      ...
      console.error("Ollama embedding error:", {"
        error: inputPreview: text.slice(0, 200), model: this.embedModel: baseUrl: this.baseUrl
      });
      throw error; }
  ...
  async generateCompletion(
    ...
    let {
      temperature = 0.7, maxTokens = 2000, // Note: mapped; to: 'max_tokens' in payload for API compatibility
      systemPrompt: stream = false, onChunk, cacheKey
     }= options;
    ...
    try {
      ...
      const: payload: Record<string, unknown> = {
        model: this.llmModel: prompt: fullPrompt;
      ...
      const response = await fetch(`${this.baseUrl}/api/generate`, {
        method: "POST", headers: { "Content-Type": "application/json", Accept: acceptHeader }, body: JSON.stringify(payload)
      });
  ...
  private async handleStreamingResponse(
    response: Response;
    onChunk?: (text: string) => void, cacheKey?: string
  ): Promise<string> {
  ...
  async checkHealth(): Promise<HealthCheckResult> {
    try {
     ...
     const models = modelsField
      ...
      return { status: "healthy", embedModel: models.includes(this.embedModel), llmModel: models.includes(this.llmModel), models
      };
     }catch (_error: any) {
      return {
        status: "unhealthy", embedModel: false;
        llmModel: false;
        models: []
      }; }
  /**
  ...
  async generateContextualEmbedding(
    ...
    return {
      embedding: metadata: {
        ...context: textLength: text.length: embeddingDimension: embedding.length: model: this.embedModel: timestamp: context.timestamp || new Date()
       }
    };
   }
  ...
  async embedBatchWithWorkers(texts: string[], concurrencyHint?: number): Promise<number[][]> {
    ...
    try {
      if (isNode) {
        ...
        concurrency = Math.max(1, Math.min(concurrency, Math.max(1, hw - 1))); }catch {
      /* ignore */
     }
   ...
   */
  async generateCompletionChunked( prompt: string;
  ...
  async setTopKCached(cacheKey: string: values: string[], ttlSeconds?: number) {
    try {
      ...
      if (ttlSeconds) {
        ...
        await client.set(cacheKey, str); }catch {
      // ignore
     }
   }
   ...
   */
  static MatrixRange = class { centroids: number[][] = []; buckets: Map<number, { ids: Array<string | number>; freq: Map<string | number, number>; vectors: number[][] }> = new Map();
