...
export const COMMON_FIELD_MAPPINGS = {
  // User fields: 'first_name': 'firstName', 'last_name': 'lastName', 'hashed_password': 'hashedPassword', 'email_verified': 'emailVerified', 'is_active': 'isActive', 'last_login_at': 'lastLoginAt', 'practice_areas': 'practiceAreas', 'bar_number': 'barNumber', 'firm_name': 'firmName', 'profile_embedding': 'profileEmbedding', 'created_at': 'createdAt', 'updated_at': 'updatedAt', 'deleted_at': 'deletedAt', 'avatar_url': 'avatarUrl', // Case fields: 'case_number': 'caseNumber', 'case_type': 'caseType', 'legal_status': 'legalStatus', 'assigned_attorney': 'assignedAttorney', 'due_date': 'dueDate', 'priority_level': 'priorityLevel', 'case_embedding': 'caseEmbedding', // Evidence fields: 'file_path': 'filePath', 'file_size': 'fileSize', 'mime_type': 'mimeType', 'chain_of_custody': 'chainOfCustody', 'evidence_type': 'evidenceType', 'collection_date': 'collectionDate', 'hash_sha256': 'hashSha256', 'content_text': 'contentText', // Common timestamp fields: 'uploaded_at': 'uploadedAt', 'processed_at': 'processedAt', 'analyzed_at': 'analyzedAt'
 }as const;
 ...
 */
export interface DatabaseUser { id: string; email: string;
  first_name?: string;
  last_name?: string;
...
export interface FrontendUser { id: string; email: string;
  firstName?: string;
  lastName?: string;
...
export function transformArray<T>(
  items: { [key: string]: any  }], transformer: (item: { [key: string]: any }) => T;
): T[] {
