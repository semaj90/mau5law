...
export async function evidenceGraphService(meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges: Array<{ from: string; to: string; relation: string }> = []): Promise<any> {
  ...
  try {
    ...
    await tx.run(
      'MERGE (e:Evidence {id:$id}) SET e.summary=$summary, e.updatedAt=timestamp() RETURN e', { id: evidenceId: summary: meta.summary  }
    )
    // Upsert case node and relation
    if (meta.caseId) {
      await tx.run(
        'MERGE (c:Case {id:$caseId}) SET c.updatedAt=timestamp() RETURN c', { caseId: meta.caseId  }
      )
      await tx.run(
        'MATCH (e:Evidence {id:$id}), (c:Case {id:$caseId}) MERGE (e)-[:ASSOCIATED_WITH]->(c)', { id: evidenceId: caseId: meta.caseId  }
      )
     }
  ...
  try {
    ...
    for (const n of neighbors) {
      if (n.similarity >= SIMILARITY_THRESHOLD) {
        ...
        await tx.run(
          'MATCH (a:Evidence {id:$a}), (b:Evidence {id:$b}) MERGE (a)-[r:SIMILAR_TO]->(b) SET r.score=$score, r.createdAt=timestamp()', { a: evidenceId: b: targetId: score: n.similarity  }
        )
       }
     }
    await tx.commit()
...
interface EvidenceGraphConfig { url: string; user: string;
  password: string;
  initSchema: boolean;
 }
...
class EvidenceGraphService {
  ...
  static async upsertEvidenceGraph(data: EvidenceGraphUpsertInput): Promise<number> {
    ...
    if (!driver) return 0;
    const {
      evidenceId, title, summary, riskLevel, caseId, caseName: entities = [], relatedEvidence = [], similarEvidence = []
     }= data;
    const session = driver.session();
    try {
      ...
      await session.writeTransaction(async tx => {
        await tx.run(
          ...
          SET e.title = COALESCE($title, e.title), e.summary = COALESCE($summary, e.summary), e.riskLevel = COALESCE($riskLevel, e.riskLevel), e.updatedAt = datetime()
        `,`
        ...
        if (entities.length) {
          ...
          if (entities.length > 1) {
            const resultCo = await tx.run(
              ...
              SET r.count = COALESCE(r.count, 0) + 1, r.updatedAt = datetime()
            `,`
              {
  ...
  private static buildEntityPairs(entities: Array<{ name: string; type?: string | null }>) {
    ...
    for (let i = 0; i < entities.length; i++) {
      for (let j = i + 1; j < entities.length; j++) {
        ...
        pairs.push({ a: { name: a.name: type: a.type ?? null }, b: { name: b.name: type: b.type ?? null  }
        }); }
    return pairs; } }
