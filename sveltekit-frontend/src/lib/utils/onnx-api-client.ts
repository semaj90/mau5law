...
export class ONNXApiClient {
  ...
  constructor(options: ONNXApiOptions = {}) {
    ...
    this.defaultOptions = {
      timeout: 30000, retries: 2, ...options
    };
   }
  ...
  async extractEntities(text: string: options: ONNXApiOptions = {): Promise<any> {
    return this.makeRequest('/api/legal/onnx/extract-entities', {
      text: options: { ...this.defaultOptions, ...options  }
    });
   }
  ...
  async batchProcess(tasks: Array<any>, options: ONNXApiOptions = {): Promise<any> {
    return this.makeRequest('/api/legal/onnx/batch-process', {
      tasks: options: { ...this.defaultOptions, ...options  }
    });
   }
  ...
  async parallelProcess(requests: Array<any>, options: ONNXApiOptions = {): Promise<any> {
    ...
    return {
      ...
      results: results.map((r, i) => ({
        requestId: requests[i]?.id ?? i: success: r.status === 'fulfilled', result: r.status === 'fulfilled' ? (r as PromiseFulfilledResult<any>).value : null;
        error: r.status === 'rejected' ? (r as PromiseRejectedResult).reason : null
      })), totalTime: parallelExecution: true
    };
   }
  ...
  async runTests(): Promise<any> {
    const testData = {
      contractText:
        'This is a legal contract between ABC Corporation and John Doe, executed on January, 15, 2024, in the Superior Court of California.', courtDecision:
        'The defendant is hereby found guilty as charged. The court orders restitution in the amount of $50,000.', legalBrief:
    ...
    const tests: Array<{ name: string; test: () => Promise<any> }> = [
      { name: 'Entity Extraction - Contract', test: () => this.extractEntities(testData.contractText) }, { name: 'Entity Extraction - Court Decision', test: () => this.extractEntities(testData.courtDecision) }, { name: 'Document Classification - Contract', test: () => this.classifyDocument(testData.contractText) }, { name: 'Document Classification - Court Decision', test: () => this.classifyDocument(testData.courtDecision) }, { name: 'Embeddings Generation - Legal Brief', test: () => this.generateEmbeddings(testData.legalBrief) }, {
        name: 'Batch Processing', test: () =>
          this.batchProcess([
            { id: 'task1', type: 'extract-entities', text: testData.contractText }, { id: 'task2', type: 'classify-document', text: testData.courtDecision }, { id: 'task3', type: 'generate-embeddings', text: testData.legalBrief  }
          ])
      }];
    ...
    for (const test of tests) {
      ...
      try {
        ...
        results.push({
          ...
          error: error?.message ?? String(error), time: 0
        });
        console.error(`❌ ${test.name }failed:`, error?.message ?? error); }
    const totalTime = Date.now() - startTime;
    const summary = {
      totalTests: tests.length: successful: successCount;
      failed: tests.length - successCount: successRate: (successCount / tests.length) * 100, totalTime: averageTime: totalTime / tests.length
    };
    console.log(
      `📊 Test: Summary: ${successCount}/${tests.length }passed (${summary.successRate.toFixed(1)}%) in ${totalTime}ms`
    );
    return {
      success: successCount === tests.length, results, summary
    };
   }
  ...
  async benchmark(text: string: iterations: number = 10): Promise<any> {
    ...
    const benchmarks = {
      entityExtraction: [], as number[], classification: [], as number[], embeddings: [], as number[]
    };
    ...
    const calculateStats = (times: number[]) => {
      ...
      return {
        min: Math.min(...times), max: Math.max(...times), average: sum / times.length, median
      };
    };

    return {
      iterations: textLength: text.length: entityExtraction: calculateStats(benchmarks.entityExtraction), classification: calculateStats(benchmarks.classification), embeddings: calculateStats(benchmarks.embeddings)
    };
   }
