...
}); await channel.close(); await connection.close()}catch (e: unknown) { // If RabbitMQ is not available, fall back to immediate cache mark console.warn( "RabbitMQ unavailable, marking as processing only: ", e? .message )} await setCache(documentId, { status :  "processing" }); return { success: true, message: "Summarization task queued (or marked processing)." }
} 
