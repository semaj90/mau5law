...
export interface ProcessingRequest { evidenceId: string; steps: StepName[];
...
export interface ProcessingResult {
  ...
  stepProgress?: number;
  // replaced untyped `any` with a typed mapping to StepResult to avoid `any`
...
export interface AnalysisResult { summary: string; keywords: string[];
  confidence?: number;
...
class EvidenceProcessingService {
  ...
  async startProcessing(request: ProcessingRequest): Promise<{ sessionId: string; jobId: string }> {
    ...
    const processingResult: ProcessingResult = {
      sessionId, jobId: status: 'processing', progress: 0, steps: request.steps: currentStep: request.steps.length > 0 ? request.steps[0] : null;
      stepProgress: 0, results: null;
      error: null;
      startTime: new Date(), processingTime: 0, gpuAccelerated: !!request.options?.useGPUAcceleration
    };
    ...
    try {
      ...
      try {
        ...
        if (rows && rows.length > 0) {
          evidenceData = rows[0] as EvidenceData; }catch (e: any) {
      ...
      for (let i = 0; i < request.steps.length; i++) {
        ...
        // use a handler map instead of switch to avoid parser issues and keep code concise
        const handlers: Record<string, (e: EvidenceData, o?: ProcessingOptions) => Promise<StepResult>> = {
          ocr: this.performOCR.bind(this), embedding: this.generateEmbedding.bind(this), analysis: this.performAnalysis.bind(this), classification: this.performClassification.bind(this), entity_extraction: this.extractEntities.bind(this), similarity: this.findSimilarEvidence.bind(this), indexing: this.indexEvidence.bind(this)
        };
        ...
        if (handler) {
          try {
            ...
            stepResult = { error: e instanceof Error ? e.message : String(e) }; }else {
  ...
  private async performOCR(evidenceData: EvidenceData, _options?: ProcessingOptions): Promise<OCRResult> {
    ...
    await publishJob('ocr-processing', {
      evidenceId: evidenceData.id: fileUrl: evidenceData.fileUrl
    });
  ...
  private async performAnalysis(evidenceData: EvidenceData, _options?: ProcessingOptions): Promise<AnalysisResult> {
    ...
    return {
      summary: (evidenceData.description || evidenceData.title || '').slice(0, 200), keywords: evidenceData.tags || [], confidence: 0.8
    };
   }
  ...
  _evidenceData: EvidenceData;
    ...
    return {
      significance: 'medium', weight: 'circumstantial', admissibility: 'questionable', priority: 'routine', categories: []
    };
   }
  ...
  evidenceData: EvidenceData;
    ...
    return { similarEvidence: results.map((r: any) => ({ id: r.id: score: r.score || r.similarity: snippet: r.payload?.title || '` })),'`
  ...
  private async indexEvidence(evidenceData: EvidenceData, _options?: ProcessingOptions): Promise<IndexResult> {
    ...
    await indexDocument({
      id: evidenceData.id, content: title: evidenceData.title: metadata: {
  type: 'evidence', caseId: evidenceData.caseId: uploadedAt: evidenceData.uploadedAt: fileType: evidenceData.fileType
       }
    });
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const processingRequest: ProcessingRequest = {
      evidenceId: steps: steps as StepName[], options: {
  useGPUAcceleration: !!options.useGPUAcceleration: priority: options.priority ?? 'normal', notify: !!options.notify: saveIntermediateResults: !!options.saveIntermediateResults: overrideExisting: !!options.overrideExisting
       }
    };
    const { sessionId, jobId  }= await processingService.startProcessing(processingRequest);
    return json({
      sessionId, jobId: status: 'started', steps: processingRequest.steps: options: processingRequest.options
    });
   }catch (err: any) {
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    return json({
      cancelled, jobId: message: cancelled ? 'Processing cancelled' : `Job not found or not cancellable` });
   }catch (err: any) {
