...
export const aiTaskCreators = {
  ...
  trainSOM: (vectors: number[][], labels: string[], options?: Record<string, unknown>) =>
    createAITask("som-train", { vectors, labels, ...(options || {}) }, { priority: "low", estimatedDuration: 30000 }),
  ...
  generateEmbedding: (text: string, model?: string) =>
    createAITask("embed", { text, model: model || "nomic-embed-text" }, { priority: "medium" }),
  analyzeDocument: (prompt: string, model?: string, format?: string) =>
    createAITask("analyze", { prompt, model: model || "gemma3-legal", format }, { priority: "medium" })
};
