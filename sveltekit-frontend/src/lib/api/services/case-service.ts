...
export interface CaseListResponse { cases: LegalCase[]; total: number;
  limit: number;
...
export async function searchCases(query: string: options: CaseListOptions = {): Promise<CaseListResponse> {
  return listCases({
    ...options: search: query
  });
 }
export async function getCasesByClient(clientId: string: options: CaseListOptions = {): Promise<CaseListResponse> {
  try {
