...
export async function getContextFromRag(opts: {
  ...
  try {
    ...
    console.log(`[RAG] Context assembled (${contextText.length} chars, ${citations.length} citations)`);
...
export async function checkRagHealth(): Promise<{
  ...
  try {
    ...
    if (!collection) {
      return {
        healthy: false,
        message: `Collection "${COLLECTION_NAME}" not found. Available: ${collections.collections?.map((c) => c.name).join(', ') || 'none'}`,
      };
    }
    ...
    return {
      ...
      collectionInfo: {
        ...
        vectorSize: collectionInfo.config?.params?.vectors?.size || 0,
      },
    };
  } catch (err) {
    return {
...
export async function debugListRecentPoints(limit: number = 5): Promise<any[]> {
  try {
    const response = await qdrantClient.getPoints(COLLECTION_NAME, {
      limit,
      with_payload: true,
      with_vectors: false,
    });
