...
export const GET: RequestHandler = async ({ request }) => {
	try {
		...
		return json({
			status: allHealthy ? 'healthy' : 'degraded', connections: health;
			responseTime: timestamp: new Date().toISOString()
		}, {
			status: allHealthy ? 200 : 503, headers: {
				'Cache-Control': 'no-cache, no-store, must-revalidate', 'X-Response-Time': '${responseTime}ms'  }
		});
	 }catch (error) {
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		return json({
			service: healthy: health[service as keyof typeof health], timestamp: new Date().toISOString()
		});
	 }catch (error) {
		return json({
