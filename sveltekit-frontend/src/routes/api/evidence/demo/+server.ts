...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const response = {
      success: true;
      id: evidenceId;
      url: `/uploads/${caseId}/${filename}`, filename: file.name: size: file.size: type: file.type, caseId: uploadDate: timestamp;
      metadata: {
  originalName: file.name: contentType: file.type: evidenceType: file.type.startsWith('image/') ? 'image' : 'document', processingStatus: 'uploaded', demoMode: true
       }
    };
    ...
    return json(
      {
        success: false;
        error: error instanceof Error ? error.message : 'Upload failed', timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
/**
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const sampleEvidence = [
      { id: 'sample-1', type: 'document', title: 'Contract Agreement.pdf', url: '/demo/sample-contract.pdf', x: 100, y: 100, metadata: {
  pages: 15, signed: true;
          uploadDate: new Date().toISOString()
         }
      }, {
        id: 'sample-2', type: 'image', title: 'Evidence Photo.jpg', url: '/demo/sample-evidence.jpg', x: 350, y: 150, metadata: {
  timestamp: '2024-09-08T10:30:00Z', uploadDate: new Date().toISOString()
         }
      }];
    return json({
      success: true;
      evidence: sampleEvidence;
      caseId: count: sampleEvidence.length
    });
   }catch (error: any) {
