...
export interface CSSOutput {
  classes: string[];
  variables: Record<string, string>; // Fixed syntax
  animations: string[];
  unoCSS: string;
}
...
export class MatrixUICompiler {
  ...
  private generateEnhancedWebGLBuffers(
    ...
    return {
      ...
      colors,
      texCoords,
      matrices: matricesBuffer, // Fixed property name
      metadata: {
        vertexCount,
  ...
  private createWebGLBuffer(node: MatrixUINode, matrix: Float32Array): WebGLBuffer | undefined {
    ...
    if (buffer) {
      ...
      this.gl.bufferData(this.gl.ARRAY_BUFFER, transformedVertices, this.gl.STATIC_DRAW); // Use transformedVertices
