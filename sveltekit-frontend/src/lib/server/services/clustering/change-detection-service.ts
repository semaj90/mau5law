...
export async function emitOperatorAlert(result: ChangeDetectionResult): Promise<void> {
  ...
  console.warn(`Changed statutes: ${result.changedStatutes.slice(0, 10).join(', ')}${result.changedStatutes.length > 10 ? '...' : ''}`);
...
──────────────────────────────────────────────────────────────
${result.changedStatutes.slice(0, 20).map((id) => `  • ${id}`).join('\n')}
...
export function exportChangeDataAsCSV(result: ChangeDetectionResult): string {
  const rows = [
    ['Statute ID', 'Previous Label', 'Current Label', 'Changed'],
  ];
