...
const validateFileService = fromPromise(
	async ({ input }: { input: DocumentUploadContext }) => {
		...
		const hasSuspicious = SUSPICIOUS_EXTENSIONS.some((ext) =>
			lowerName.endsWith(ext)
		);
		if (hasSuspicious) {
			errors.push(
				'File appears to be executable or script-like and is not allowed'
			);
		}
...
const uploadFileService = fromPromise(
	async ({ input }: { input: DocumentUploadContext }) => {
		if (!input.file) {
...
const extractTextService = fromPromise(
	async ({ input }: { input: DocumentUploadContext }) => {
		if (!input.file) {
			...
			invoke: {
				...
				onDone: {
					...
					actions: assign(({ event, context }) => ({
						...
						extractedText:
							event.output.extractedText ?? context.extractedText,
...
export type DocumentUploadState = ReturnType<
	typeof documentUploadMachine['getInitialState']
>;
...
export const isValidating = (state: any): boolean =>
	['validating', 'calculatingHash', 'extractingText'].includes(
		state.value as string
	);

export const hasValidationErrors = (state: any): boolean =>
	Array.isArray(state.context.validationErrors) &&
	state.context.validationErrors.length > 0;
...
export const getUploadProgress = (state: any): number =>
	state.context.uploadProgress ?? 0;
...
export const getUploadMetrics = (state: any) => {
	...
	return {
		uploadTime: context.uploadEndTime
			? context.uploadEndTime - context.uploadStartTime
			: 0,
