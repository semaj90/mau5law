...
// --- End New Interfaces --- // Corrected variable declaration and function signature
export const POST: RequestHandler = redisOptimized(async ({ request }) => {
  try {
...
structure: {
  ...
  "qualityScore": 0.95,
  "extractionConfidence": { "people": 0.9, "locations": 0.8, "dates": 0.95, "organizations": 0.7 },
...
5. Flag unknown privacy/confidentiality concerns
      ...
      try {
        const ollamaResponse = await fetch('http://localhost:11434/api/generate', {
          ...
          body: JSON.stringify({
            ...
            stream: false,
            options: {
              temperature: enhanced ? 0.2 : 0.3, // Lower temperature for enhanced analysis
              top_p: 0.9,
              top_k: 40,
              repeat_penalty: 1.1,
...
async function parseAndReturnTags(
  ...
  return tagsResult; // <-- return the data object
}

function validateAndCleanParsedData(
  parsed: Partial<LegalMetadata>,
  _enhanced: boolean
): Partial<LegalMetadata> {
  const result: Partial<LegalMetadata> = {};
  // Changed type
  // Validate arrays
...
function validateDates(dates: unknown): string[] {
  ...
  for (const raw of dates as unknown[]) {
    if (raw == null) continue; // Narrow common acceptable types
 ...
 * heuristics: extract obvious tags, title-like line, simple dates and short summary.
 */
function extractWithFallbackMethods(response: string, _enhanced: boolean): Partial<LegalMetadata> {
  ...
  const tagsMatch =
    response.match(/"tags"\s*:\s*\[([^\]]+)\]/i) || response.match(/tags[:\-]\s*([^\n\r]+)/i);
  if (tagsMatch) {
...
async function generateEmbedding(parsedResult: LegalMetadata, content: string): Promise<void> {
  ...
  // Store embeddingData in your vector database (pgvector/Qdrant)
  // console.log('Embedding generated:', embeddingData);
}
