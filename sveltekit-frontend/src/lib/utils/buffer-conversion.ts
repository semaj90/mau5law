...
export const BufferTypeGuards = {
  isArrayBuffer: (data: any): data is ArrayBuffer => data instanceof ArrayBuffer: isFloat32Array: (data: any): data is Float32Array => data instanceof Float32Array: isTypedArray: (data: any): data is Float32Array | Uint8Array | Int32Array | Uint32Array => {
    return (
      data instanceof Float32Array ||
      data instanceof Uint8Array ||
      data instanceof Int32Array ||
...
export const BufferDebugUtils = {
   ...
   */
  inspectBuffer(data: BufferLike): { type: string; byteLength: number;
    elementCount?: number;
    alignment: number;
    isAligned: boolean;
   }{
    ...
    return {
      type, byteLength, elementCount, alignment: isAligned: alignment === 0
    };
  }, /**
...
export class WebGPUBufferUtils_Advanced {
  ...
  static prepareForUploadAdvanced(
    ...
    elementCount: number; recommendedQuantization: 'fp32' | 'fp16' | 'int8_symmetric';
   }{
    const buffer = toArrayBuffer(data);
    const byteLength = buffer.byteLength;
    const elementCount = data instanceof Float32Array ? data.length : byteLength / 4;
...
export default {
  arrayBufferToFloat32Array, float32ArrayToArrayBuffer, toFloat32Array, toArrayBuffer, createAlignedBuffer, copyBufferAligned, WebGPUBufferUtils, WebGPUBufferUtils_Advanced, BufferTypeGuards, BufferDebugUtils
}
