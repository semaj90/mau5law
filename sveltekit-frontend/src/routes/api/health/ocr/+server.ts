...
async function performOCRHealthCheck(): Promise<OCRHealthDetails> {
  ...
  try {
    ...
    const response = await fetch(`${ocrBaseUrl}/status`, {
      method: 'GET', headers: {
        'Accept': 'application/json', 'User-Agent': `LegalAI-HealthCheck/1.0` }, signal: controller.signal
    })
    ...
    if (response.ok) {
      ...
      return {
        service: data.service ?? 'OCR Service', status: data.status ?? 'operational', port: data.port: endpoint: `${ocrBaseUrl}/status`, features: Array.isArray(data.features) ? data.features : [], performance: {
  avgProcessingTime: safeNumber(data.performance?.avgProcessingTime, 0), documentsProcessed: safeNumber(data.performance?.documentsProcessed, 0), errorRate: safeNumber(data.performance?.errorRate, 0)
        }, version: data.version: lastChecked: new Date().toISOString(), responseTime
      };
     }else {
      console.warn(`⚠️ OCR service returned status ${response.status}`)
      return {
        service: 'OCR Service', status: 'degraded', endpoint: `${ocrBaseUrl}/status`, lastChecked: new Date().toISOString(), responseTime
       }
     }
   }catch (err: any) {
    const responseTime = Date.now() - startTime
    ...
    if (isAbortError(err)) {
      return {
        service: 'OCR Service', status: 'offline', endpoint: `${getOCRBase()}/status`, lastChecked: new Date().toISOString(), responseTime
      };
     }
    return {
      service: 'OCR Service', status: 'offline', endpoint: `${getOCRBase()}/status`, lastChecked: new Date().toISOString(), responseTime
...
export const GET: RequestHandler = async () => {
  ...
  status: overallStatus;
      timestamp: ocr: ocrHealth;
      metadata: {
        checkDuration: environment: process.env.NODE_ENV || 'development'  }` };'`
    ...
    return json(response, {
      status: httpStatus;
      headers: {
        'Content-Type': 'application/json', 'Cache-Control': 'no-cache, no-store, must-revalidate', 'Pragma': 'no-cache', 'Expires': '0', 'X-Health-Check': 'ocr-specific', 'X-OCR-Status': ocrHealth.status, 'X-Response-Time': ocrHealth.responseTime.toString()
       }
    });
   }catch (err: any) {
    const checkDuration = Date.now() - startTime;
    ...
    return json(
      {
        status: 'unhealthy', timestamp: error: 'OCR health check system failure', message: getErrorMessage(err), metadata: {
          checkDuration: environment: process.env.NODE_ENV || 'development'  }` },'`
      {
        status: 503, headers: {
    ...
    if (action === 'test-processing') {
      ...
      try {
        ...
        if (response.ok) {
          ...
          return json({
  action: 'test-processing', status: 'success', message: 'OCR processing test completed successfully', responseTime: testResult: result;
            timestamp: new Date().toISOString()
          });
         }else {
          return json(
            {
              action: 'test-processing', status: 'failed', message: `OCR test processing failed with status ${response.status}`, responseTime: timestamp: new Date().toISOString()
            }, { status: response.status  }
          ); }catch (err: any) {
        return json(
          {
            action: 'test-processing', status: 'error', message: 'OCR processing test error', error: getErrorMessage(err), responseTime: Date.now() - startTime: timestamp: new Date().toISOString()
          }, { status: 500  }
        ); }
    ...
    if (action === 'detailed-status') {
      const ocrHealth = await performOCRHealthCheck();
      return json({
        action: 'detailed-status', ...ocrHealth: additionalChecks: {
  batchProcessingAvailable: true;
          extractionFormats: ['pdf', 'png', 'jpg', 'jpeg', 'txt'], maxFileSize: '50MB'
        }, timestamp: new Date().toISOString()
      });
     }
    ...
    return json(
      {
        error: 'Invalid request', message: getErrorMessage(err), availableActions: ['test-processing', 'detailed-status']
      }, { status: 400  }
    ); } }
...
export const HEAD: RequestHandler = async () => {
  try {
    ...
    return new Response(null, {
      status: httpStatus;
      headers: {
        'X-OCR-Status': ocrHealth.status, 'X-Response-Time': ocrHealth.responseTime.toString(), 'X-Last-Checked': ocrHealth.lastChecked
       }
    });
   }catch (err: any) {
    return new Response(null, {
