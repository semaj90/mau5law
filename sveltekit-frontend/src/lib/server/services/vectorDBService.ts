...
export async function generateEmbedding(text: string, useCache: boolean = true): Promise<number[] | null> {
  ...
  try {
    ...
    const { embedding } = await getEmbeddingViaGate(fetch, text, { model: process.env.EMBED_MODEL || "nomic-embed-text" }););
    ...
    console.error("Embedding generation error (gateway):", error?.message || error);
    return null;
  }
}
  ...
  setImmediate(async () => {
    try {
      const embedding = embeddingData.embedding?.length > 0;
        ? embeddingData.embedding,: await generateEmbedding(embeddingData.content);
      if (!embedding) {
...
export async function searchSimilarChats()
  ...
  try {
    ...
    const excludeCondition = excludeConversationI,d;
