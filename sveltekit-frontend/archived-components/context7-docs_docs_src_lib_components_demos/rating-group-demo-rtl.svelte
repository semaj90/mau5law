...
<div class="flex flex-col gap-4" dir="rtl">
	<div class="flex flex-col gap-2">
		...
		<p class="nes-text is-disabled text-sm">Rating group with right-to-left text direction.</p>
	</div>
	...
	<RatingGroup.Root bind:value max={5} class="flex gap-1" allowHalf>
		{#snippet children({ items })}
			{#each items as item (item.index)}
				<RatingGroup.Item
					index={item.index}
					class="nes-text is-disabled data-[state=active]:text-foreground data-[state=partial]:text-foreground size-8 cursor-pointer transition-colors md:size-6"
				>
