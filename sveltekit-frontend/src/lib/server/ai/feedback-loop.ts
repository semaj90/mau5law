...
class FeedbackLoop {
  ...
  async getPersonalizedRecommendations(userId: string): Promise<any> {
    try {
      ...
      // Analyze patterns
      const patterns: UserPatterns = { commonTopics: this.extractCommonTopics(userInteractions),
      ...
      const recommendations = {
        ...
        personalizedSettings: { defaultMaxSources: patterns.averageComplexity > 0.7 ? 15 : 10,
  ...
  getStats(): any {
    ...
    return {
      ...
      metrics: { queryPatterns: this.learningMetrics.queryPatterns.size,
