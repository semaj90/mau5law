...
// Lightweight WS-ish connection holder (works with ws WebSocket instances that expose .send)
type WsLike = { send: (data: string) => void; terminate?: () => void };
export const wsConnections = new Map<string, Set<WsLike>>();
...
export function registerWsConnection(sessionId: string, ws: WsLike): void {
...
export function deregisterWsConnection(sessionId: string, ws: WsLike): void {
...
export function storeMessage(sessionId: string, payload: unknown, type = 'message') {
...
export function broadcastMessage(sessionId: string, msg: StoredMessage): StoredMessage {
    // SSE broadcast
    const sseSet = sseConnections.get(sessionId);
