...
export const GET: RequestHandler = async ({ request }) => {
  ...
  if (upgrade !== 'websocket') {
    return json(
      {
        error: 'This endpoint requires WebSocket upgrade', info: 'Use WebSocket connection to receive real-time progress updates'
      }, { status: 400  }
    );
   }
  ...
  return json(
    {
      message: 'WebSocket endpoint ready', endpoint: '/api/security/validate/progress', usage: 'Connect via WebSocket for real-time validation progress updates'
    }, { status: 200  }
  );
 }
/*
...
export interface ValidationProgressMessage {
  type: 'progress' | 'complete' | 'error', message: string; percentage: number; timestamp: string;
  stage?: string
  details?: any
 }
/*
