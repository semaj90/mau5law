/** * SIMD JSON Index Processor - Optimized Copilot Context Processing (Fixed) * Integrates with Enhanced RAG system and Context7 MCP for high-performance index parsing */
import { enhancedRAGStore } from '../lib/stores/unified';
import type { RAGDocument, RAGSearchResult, TextChunk } from '../lib/types/rag';

// Add a local semantic chunk type to avoid `any`
type SemanticChunk = {
    id: string | undefined;
    content: string;
...
export interface VectorEmbeddingConfig {
    model: 'nomic-embed-text' | 'all-MiniLM-L6-v2' | 'text-embedding-ada-002' | 'embeddinggemma:latest';
    dimensions: 384 | 768 | 1536 | 3072;
    backend: 'pgvector' | 'qdrant' | 'hybrid' | 'ollama';
    chunkSize: number;
    overlap: number;
}

// New: explicit raw index / entry and SOM cluster types
...
export class SIMDJSONIndexProcessor {
        ...
        try {
            ...
            switch (this.vectorConfig.backend) {
                ...
                case 'hybrid':
                    ...
                    break;
                case 'ollama':
                    embedding = await this.generateOllamaEmbedding(content);
                    break;
                default:
                    embedding = await this.generateFallbackEmbedding(content);
            }
    ...
    private async generateOllamaEmbedding(content: string): Promise<number[]> {
        try {
            const response = await fetch(this.getOllamaEndpoint(), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ model: this.vectorConfig.model, prompt: content })
            });
            if (!response.ok) {
            ...
            const msg = error instanceof Error ? error.message : String(error);
            console.warn('Ollama embedding failed, using fallback: ', msg);
    ...
    async semanticSearch(
        query: string,
        index: CopilotIndex,
        options: { limit?: number; threshold?: number; preferEnhanced?: boolean } = {}
    ...
    private async generateSemanticChunks(content: string): Promise<SemanticChunk[]> {
    ...
    private async generateSemanticClusters(entries: CopilotIndexEntry[]): Promise<CopilotCluster[]> {
        try {
            ...
            return somClusters.map((cluster) => ({
                id: cluster.id,
    ...
    private calculateIndexStatistics(entries: CopilotIndexEntry[]) {
        ...
        const avgEmbeddingTime = this.performanceMetrics.embeddingTime / Math.max(entries.length, 1);
    ...
    private generateFallbackEmbedding(content: string): number[] {
        ...
        let index = 0;
        // Use the value (count) from wordCount entries
    ...
    private generateId(): string {
        return `idx_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`;
    }
