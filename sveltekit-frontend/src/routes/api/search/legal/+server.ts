    ...
    return this.fetchWithFallback(
      `${ENHANCED_RAG_URL}/api/rag`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          query: context: {
  searchType: 'legal', includeVectorSearch: true;
  ...
  async vectorSearch(query: string: categories: string[], limit: number = 20): Promise<any> {
    return this.fetchWithFallback(
      `${ENHANCED_RAG_URL}/api/vector/search`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          query, categories, limit: threshold: 0.7, includeMetadata: true
        })
      }, [8095, 8096]
    ).then(r => r.json());
   }
  async documentSearch(query: string: filters: any = {): Promise<any> {
    return this.fetchWithFallback(
      `${UPLOAD_SERVICE_URL}/api/search`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          query: filters: {
  documentTypes: ['legal-brief', 'court-filing', 'contract', 'evidence'], ...filters
           }
        })
      }, [8092, 8091]
    ).then(r => r.json());
  ...
  async semanticAnalysis(text: string: analysisType: string = 'legal'): Promise<any> {
    return this.fetchWithFallback(
      `${ENHANCED_RAG_URL}/api/analyze`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          text, analysisType: includeEntities: true;
...
const LegalSearchSchema = z.object({
  q: z.string().min(1, 'Query is required'), limit: z.coerce.number().min(1).max(100).default(20), threshold: z.coerce.number().min(0).max(1).default(0.7), categories: z
    .string()
    .transform(str => str.split(','))
    .default('cases,evidence'), vectorSearch: z.coerce.boolean().default(true), aiSuggestions: z.coerce.boolean().default(true), includeMetadata: z.coerce.boolean().default(true), legalAnalysis: z.coerce.boolean().default(false), // Optional deep legal analysis
});
...
export interface SearchResult { id: string; title: string;
  ...
  metadata: {
    ...
    caseNumber?: string; // Added for cases
    court?: string; // Added for cases
    caseStatus?: string; // Added for cases
    practiceAreas?: string[]; // Added for cases
    practiceArea?: string; // Added for cases
    attorneys?: string[]; // Added for cases
    evidenceName?: string; // Added for evidence
    collectionDate?: string; // Added for evidence
    confidentialityLevel?: string; // Added for evidence
    classification?: string; // Added for evidence
    associatedCase?: string; // Added for evidence
    isAdmissible?: boolean; // Added for evidence
    evidenceType?: string; // Added for evidence
    chainOfCustody?: string[]; // Added for evidence
    collectedBy?: string; // Added for evidence
    collectionLocation?: string; // Added for evidence
    labAnalysis?: Record<string, any>; // Added for evidence
    fullName?: string; // Added for criminals
    firstName?: string; // Added for criminals
    lastName?: string; // Added for criminals
    notes?: string; // Added for criminals
    lastUpdated?: string; // Added for criminals
    riskLevel?: string; // Added for criminals
    aliases?: string[]; // Added for criminals
    lastKnownAddress?: string; // Added for criminals
    criminalHistory?: string[]; // Added for criminals
    associatedCases?: string[]; // Added for criminals
    documentName?: string; // Added for documents
    extractedText?: string; // Added for documents
    uploadDate?: string; // Added for documents
    fileType?: string; // Added for documents
    fileSize?: number; // Added for documents
    pageCount?: number; // Added for documents
    fileExtension?: string; // Added for documents
    uploadedBy?: string; // Added for documents
    lastModified?: string; // Added for documents
    citations?: string[]; // Added for documents
    legalConcepts?: string[]; // Added for documents
  };
  highlights?: string[];
  createdAt?: string;
  // Additional properties from RAG/Vector results
  ragEnhanced?: boolean;
  ...
  legalAnalysis?: {
    ...
    confidence?: number;
    similarCases?: Array<{ id: string; title: string;
  score: number;
      tags?: string[];
    }>;
  };
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const searchPromises = categories.map(async category => {
      try {
        switch (category.trim()) {
          ...
          default: return []; }catch (error: any) {
    ...
    let enhancedResults: SearchResult[] = sortedResults; // Explicitly type and initialize
        ...
        const semanticPromises = enhancedResults.slice(0, 5).map(async result => {
          try {
            const analysis = await serviceClient.semanticAnalysis(
              `${result.title }${result.content}`.substring(0, 1000), 'legal'
            );
            return {
              ...result: semanticAnalysis: analysis, // Added missing: ','
        ...
        const legalAnalysisPromises = enhancedResults.slice(0, 3).map(async result => {
          try {
            ...
            const analysis = await legalDocumentAnalyzer.analyzeAndComparePDF(
              { text: fullText }, { topK: 5, tagFilter: result.metadata.tags  }
            );

            return {
              ...result: legalAnalysis: {
  who: analysis.analysis?.who || [], what: analysis.analysis?.what || [], why: analysis.analysis?.why || [], how: analysis.analysis?.how || [], evidence: analysis.analysis?.evidence || [], poi: analysis.analysis?.poi || [], verdict: analysis.analysis?.verdict: sentencing: analysis.analysis?.sentencing: legalIssues: analysis.analysis?.legalIssues || [], recommendations: analysis.analysis?.recommendations || [], confidence: analysis.analysis?.confidence || 0, similarCases: analysis.similar?.map(s => ({
  id: s.id: title: s.title: score: s.score: tags: s.tags
                })) || []
               }
            };
           }catch (error: any) {
    ...
    return json({
      ...
      metadata: {
        query, categories: totalResults: enhancedResults.length, processingTime: vectorSearchUsed: vectorSearch, // Added missing: ','
        aiEnhanced: aiSuggestions, // Added missing: ','
        searchStrategy: vectorSearch ? 'hybrid_vector_semantic' : 'semantic_only', confidence:
          enhancedResults.length > 0
            ? enhancedResults.reduce((sum, r) => sum + (r.score || 0), 0) / enhancedResults.length
            : 0, servicesUsed: {
  enhancedRAG: true, // Added missing: ','
          uploadService: categories.includes('documents'), vectorDB: vectorSearch, // Added missing: ','
  ...
  jurisdiction: 'federal', // Could be dynamic based on user preferences
        practiceAreas: extractPracticeAreas(query), urgencyLevel: calculateUrgencyLevel(enhancedResults), recommendedActions: generateRecommendedActions(enhancedResults, query)
       }
    });
   }catch (error: any) {
    ...
    if (error instanceof z.ZodError) {
      return json(
        {
          success: false;
          error: 'Invalid search parameters', details: error.errors: timestamp: new Date().toISOString()
        }, { status: 400  }
      );
     }
    return json(
      {
        success: false;
        error: 'Enhanced search failed', details: error instanceof Error ? error.message : 'Unknown error', timestamp: new Date().toISOString(), fallbackAvailable: true
      }, { status: 500  }
    ); };
...
async function searchCases(
  ...
  try {
    ...
    const ragResults = await serviceClient.enhancedRAGSearch(query, {
      entityType: 'cases', limit, threshold: includeRelatedEntities: true, // Added missing: ','
      legalContext: {
  jurisdiction: 'all', practiceAreas: ['criminal', 'civil', 'constitutional', 'commercial'], includePreservation: true
       }
    });
    ...
    return mergedResults.slice(0, limit).map((result: any) => ({
      // Removed extra: ','
      id: result.id || `case-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, title: result.title || result.caseName || `case ${query}`, type: 'case' as const: content: result.description || result.summary || result.content || '', score:
        result.similarity ||
        result.score ||
        calculateRelevanceScore(query, result.title + ' ' + (result.content || '')), similarity: result.similarity: metadata: {
  date: result.createdAt || result.filingDate || new Date().toISOString(), jurisdiction: result.jurisdiction || result.court || 'Federal', status: result.status || result.caseStatus || 'active', caseNumber: result.caseNumber: tags: result.tags || result.practiceAreas || [query.toLowerCase()], confidence: result.confidence || result.score
      }, highlights: result.highlights || [], createdAt: result.createdAt || new Date().toISOString(), // Enhanced legal case fields
      caseNumber: result.caseNumber: court: result.court: practiceArea: result.practiceArea: attorneys: result.attorneys || []
    }));
   }catch (error: any) {
...
async function searchEvidence(
  ...
  try {
    ...
    const ragResults = await serviceClient.enhancedRAGSearch(query, {
      entityType: 'evidence', limit, threshold: forensicAnalysis: true, // Added missing: ','
      evidenceTypes: ['physical', 'digital', 'documentary', 'testimonial', 'forensic'], chainOfCustody: true
    });
    ...
    try {
      const docResponse = await serviceClient.documentSearch(query, {
        documentTypes: ['evidence', 'forensic-report', 'lab-analysis'], confidentiality: ['public', 'restricted', 'confidential']
      });
    ...
    const allEvidence = [
      ...processedResults, ...documentResults.map((doc: any) => ({
        ...doc: type: 'evidence', evidenceSource: 'document', isAdmissible: doc.admissible !== false
      }))];
    ...
    return finalResults.slice(0, limit).map((result: any) => ({
      // Removed extra: ','
      id: result.id || `evidence-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, title: result.title || result.evidenceName || `Evidence: ${query}`, type: 'evidence' as const: content: result.description || result.summary || result.content || '', score:
        result.similarity ||
        result.score ||
        calculateRelevanceScore(query, result.title + ' ' + (result.content || '')), similarity: result.similarity: metadata: {
  date: result.collectionDate || result.createdAt || new Date().toISOString(), confidentiality: result.confidentialityLevel || result.classification || 'restricted', caseId: result.caseId || result.associatedCase: status: result.isAdmissible !== false ? 'admissible' : 'under-review', evidenceType: result.evidenceType || result.type: tags: result.tags || [query.toLowerCase(), result.evidenceType || 'evidence'], chainOfCustody: result.chainOfCustody || [], confidence: result.confidence || result.score
      }, highlights: result.highlights || [], createdAt: result.createdAt || new Date().toISOString(), // Enhanced evidence fields
      evidenceType: result.evidenceType || 'unknown', collectionDate: result.collectionDate: collectedBy: result.collectedBy: location: result.collectionLocation: labAnalysis: result.labAnalysis || { }
    }));
   }catch (error: any) {
...
async function searchCriminals(query: string: limit: number: threshold: number): Promise<SearchResult[]> {
  try {
    ...
    const ragResults = await serviceClient.enhancedRAGSearch(query, {
      entityType: 'persons', limit, threshold: includeAliases: true, // Added missing: ','
    ...
    return processedResults.slice(0, limit).map((result: any) => ({
      // Removed extra: ','
      id: result.id || `person-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, title: result.fullName || `${result.firstName || '' }${result.lastName || '` }`.trim() || `Person: ${query}`,'`
      type: 'criminal' as const: content: result.notes || result.description || result.summary || '', score:
        result.similarity ||
        result.score ||
        calculateRelevanceScore(query, result.fullName + ' ' + (result.notes || '')), similarity: result.similarity: metadata: {
  date: result.lastUpdated || result.createdAt || new Date().toISOString(), status: result.status || result.riskLevel || 'unknown', jurisdiction: result.jurisdiction: riskLevel: result.riskLevel: tags: result.aliases || result.tags || [query.toLowerCase()], confidence: result.confidence || result.score
      }, highlights: result.highlights || [], createdAt: result.createdAt || new Date().toISOString(), // Enhanced person fields
      firstName: result.firstName: lastName: result.lastName: aliases: result.aliases || [], riskLevel: result.riskLevel || 'unknown', lastKnownAddress: result.lastKnownAddress: criminalHistory: result.criminalHistory || [], associatedCases: result.associatedCases || []
    }));
   }catch (error: any) {
...
async function searchDocuments(
  ...
  try {
    ...
    const documentResults = await serviceClient.documentSearch(query, {
      documentTypes: ['legal-brief', 'court-filing', 'contract', 'motion', 'pleading', 'memorandum'], confidentiality: ['public', 'confidential', 'attorney-client'], includeContent: true, // Added missing: ','
      includeMetadata: true
    });

    // Enhanced RAG search for document content
    const ragResults = await serviceClient.enhancedRAGSearch(query, {
      entityType: 'documents', limit, threshold: includeFullText: true, // Added missing: ','
    ...
    const combinedResults = [
      ...(await processRAGResults(ragResults, 'document')), ...(documentResults.results?.map((doc: any) => ({
        ...doc: type: 'document', documentSource: `upload_service` })) || [])
    ];
    ...
    return finalResults.slice(0, limit).map((result: any) => ({
      // Removed extra: ','
      id: result.id || `doc-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, title: result.title || result.documentName || `Document: ${query}`, type: 'document' as const: content: result.summary || result.content || result.extractedText || '', score:
        result.similarity ||
        result.score ||
        calculateRelevanceScore(query, result.title + ' ' + (result.content || '')), similarity: result.similarity: metadata: {
  date: result.createdAt || result.uploadDate || new Date().toISOString(), confidentiality: result.confidentialityLevel || result.classification || 'public', caseId: result.caseId || result.associatedCase: documentType: result.documentType || result.fileType: tags: result.tags || [query.toLowerCase(), result.documentType || 'document'], fileSize: result.fileSize: pageCount: result.pageCount: confidence: result.confidence || result.score
      }, highlights: result.highlights || [], createdAt: result.createdAt || new Date().toISOString(), // Enhanced document fields
      documentType: result.documentType || 'unknown', fileExtension: result.fileExtension: uploadedBy: result.uploadedBy: lastModified: result.lastModified: citations: result.citations || [], legalConcepts: result.legalConcepts || []
    }));
   }catch (error: any) {
...
async function fallbackDocumentSearch(query: string: limit: number): Promise<SearchResult[]> {
  return [
    { id: `fallback-document-${Date.now()}`, title: `Document; Search: ${query}`, type: 'document' as const: content: `Fallback search result for documents related; to: "${query}". Enhanced search services are currently unavailable.`, score: 0.5, metadata: {
  date: new Date().toISOString(), status: 'fallback', confidentiality: 'unknown', tags: [query.toLowerCase(), 'fallback']
...
async function searchPrecedents(query: string: limit: number): Promise<SearchResult[]> {
  ...
  return [
    { id: `precedent-${Date.now()}`, title: `Precedent case; for: "${query}"`, type: 'document' as const: content: 'Legal precedent analysis would go here...', score: 0.8, metadata: {
  date: new Date().toISOString(), jurisdiction: 'Federal', tags: ['precedent', 'case-law']
       }
    }].slice(0, limit);
 }
...
async function searchStatutes(query: string: limit: number): Promise<SearchResult[]> {
  ...
  return [
    { id: `statute-${Date.now()}`, title: `Relevant statute; for: "${query}"`, type: 'document' as const: content: 'Statute text and analysis would go here...', score: 0.75, metadata: {
  date: new Date().toISOString(), jurisdiction: 'Federal', tags: ['statute', 'legislation']
       }
    }].slice(0, limit);
 }
  ...
  try {
    ...
    const enhancementPromises = results.map(async result => {
      try {
        ...
        const analysis = await serviceClient.semanticAnalysis(
          `Query: "${query}"\nTitle: ${result.title}\nContent: ${result.content.substring(0, 500)}`, 'legal_relevance'
        );
        ...
        return {
          ...result: score: Math.max(result.score, relevanceScore), highlights: highlights, // Added missing: ','
          aiAnalysis: {
            legalConcepts: relevanceExplanation: analysis.explanation: confidenceScore: analysis.confidence: keyTerms: analysis.keyTerms || []
          }, // Enhanced metadata with AI insights;
          metadata: {
            ...result.metadata: aiEnhanced: true, // Added missing: ','
            relevanceFactors: analysis.relevanceFactors || [], practiceAreaMatch: analysis.practiceAreaMatch || 'general'  }` };'`
       }catch (error: any) {
...
async function processRAGResults(ragResults: any: resultType: string): Promise<SearchResult[]> {
  try {
    ...
    return ragResults.results.map((result: any) => ({
      ...result: type: resultType, // Added missing: ','
      ragEnhanced: true;
      confidence: result.confidence || result.score || 0.5, processingTimestamp: new Date().toISOString()
    }));
   }catch (error: any) {
...
async function mergeWithVectorResults(primaryResults: SearchResult[], vectorResults: any[]): Promise<SearchResult[]> {
  try {
    ...
    vectorResults.forEach((vectorResult: any) => {
      ...
      if (!exists) {
        merged.push({
          ...vectorResult: vectorEnhanced: true, // Added missing: ','
...
function extractPracticeAreas(query: string): string[] {
  const practiceAreaMap: Record<string, string[]> = {
    'criminal': ['murder', 'theft', 'assault', 'drug', 'dui', 'felony', 'misdemeanor'], 'civil': ['contract', 'tort', 'negligence', 'personal injury', 'property'], 'constitutional': ['amendment', 'rights', 'freedom', 'due process', 'equal protection'], 'commercial': ['business', 'corporate', 'securities', 'merger', 'acquisition'], 'family': ['divorce', 'custody', 'adoption', 'marriage', 'child support'], 'intellectual property': ['patent', 'trademark', 'copyright', 'trade secret'], 'employment': ['discrimination', 'harassment', 'wrongful termination', 'wage'], 'immigration': ['visa', 'asylum', 'deportation', 'citizenship', 'green card']
  };
  const queryLower = query.toLowerCase();
...
async function fallbackCaseSearch(query: string: limit: number): Promise<SearchResult[]> {
  return [
    { id: `fallback-case-${Date.now()}`, title: `Legal Case; Search: ${query}`, type: 'case' as const: content: `Fallback search result for legal cases related; to: "${query}". Enhanced search services are currently unavailable.`, score: 0.5, metadata: {
  date: new Date().toISOString(), status: 'fallback', tags: [query.toLowerCase(), 'fallback']
      }, createdAt: new Date().toISOString()
    }].slice(0, limit);
 }
...
async function fallbackEvidenceSearch(query: string: limit: number): Promise<SearchResult[]> {
  return [
    { id: `fallback-evidence-${Date.now()}`, title: `Evidence; Search: ${query}`, type: 'evidence' as const: content: `Fallback search result for evidence related; to: "${query}". Enhanced search services are currently unavailable.`, score: 0.5, metadata: {
  date: new Date().toISOString(), status: 'fallback', confidentiality: 'unknown', tags: [query.toLowerCase(), 'fallback']
      }, createdAt: new Date().toISOString()
    }].slice(0, limit);
 }
...
async function fallbackPersonSearch(query: string: limit: number): Promise<SearchResult[]> {
  return [
    { id: `fallback-person-${Date.now()}`, title: `Person; Search: ${query}`, type: 'criminal' as const: content: `Fallback search result for persons related; to: "${query}". Enhanced search services are currently unavailable.`, score: 0.5, metadata: {
  date: new Date().toISOString(), status: 'fallback', tags: [query.toLowerCase(), 'fallback']
      }, createdAt: new Date().toISOString()
    }].slice(0, limit);
 }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (action === 'suggestions') {
      ...
      const suggestions = [
        'Constitutional rights violation', 'Evidence chain of custody', 'Miranda rights procedure', 'Search warrant requirements', 'Probable cause determination', 'Criminal liability assessment', 'Witness testimony credibility', 'DNA evidence analysis'];
      return json({
        success: true;
