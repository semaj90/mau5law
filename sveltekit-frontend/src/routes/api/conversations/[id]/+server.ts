...
export const GET: RequestHandler = withErrorHandling(async (event) => {
  ...
  if (!conversationId) {
    return apiError(
      'Conversation ID is required', 400, 'INVALID_INPUT', undefined, requestId
    )
   }
  try {
    ...
    return apiSuccess(
      {
        conversation: conversationData.conversation: messages: chatMessages
      }, 'Conversation retrieved successfully', requestId
    );
   }catch (err: any) {
