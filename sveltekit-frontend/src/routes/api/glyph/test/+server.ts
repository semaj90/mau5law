...
export const GET: RequestHandler = async () => {
  ...
  return json({
    success: true;
    message: 'Glyph diffusion test endpoint is working', status: 'ok'
  });
};
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const mockGlyphResponse = {
      success: true;
      data: {
  evidence_id: body.evidence_id || 123, glyph_url: `data:image/svg+xml,${encodeURIComponent(`
        ...
        `)}`, generation_time_ms: Math.floor(Math.random() * 200 + 50), cache_hits: 0, tensor_ids: ['mock_tensor_1', 'mock_tensor_2'], neural_sprite_results: null;
        metadata: {
  style: body.style || 'legal', prompt: body.prompt || 'test', dimensions: body.dimensions || [256, 256]
         }
       }
    };
