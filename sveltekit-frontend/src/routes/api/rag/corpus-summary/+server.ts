...
export const GET: RequestHandler = async () => {
  try {
    const rows = await db.execute(
      sql`SELECT id, generated_at, model, embed_model, docs_count, topics, parameters, created_at FROM corpus_summaries ORDER BY id DESC LIMIT 1`
    );
    if (!rows?.rows?.length) {
      return json({ success: true, summary: null });
    }
