...
<script lang="ts">
  ...
  onMount(() => {
    ...
    try {
      ...
      resetProcessorFn = typeof m.resetProcessor === 'function' ? m.resetProcessor : undefined} catch {
      // store module not available â€” we'll use the MinIO fallback below'
    }
  		})();
	});
  ...
  async function uploadToMinio(file: File, key: string | bucket = 'evidence'): Promise<string> {
    ...
    // return public URL â€” with default MinIO settings path-style may work; adjust if you use virtual-hosted style or proxy.
    return url}
  ...
  const iconFor = (s: string) =>
    s === 'completed' ? 'âœ…' : s === 'error' ? 'âŒ' : s === 'validating' ? 'ðŸ”Ž' : s === 'analyzing' ? 'ðŸ§ ' : s === 'embedding' ? 'ðŸ“¦' : 'â¬†ï¸';
  ...
  {#if !selectedFile && !processing}
    <div
     ...
     , class:border-gray-300={!dragover}
      ...
      tabindex="0"
    >
      <div class="w-12 h-12 mx-auto mb-4 text-gray-400">â¬†ï¸</div>
  ...
  {#if selectedFile && !processing && !completed}
    <div class="file-info bg-gray-50 p-4 rounded-lg" transition:fade>
      <div class="flex items-center">
        <div>
          <p class="font-medium">{selectedFile.name}
</p>
          ...
          <p class="text-sm text-blue-600">Evidence ID: {evidenceId}
</p>
        </div>
  ...
  {#if errorMsg}
    <Alert variant="destructive" class="mt-4">
      <div class="flex items-start">
        <div class="text-red-600">âŒ</div>

        <AlertDescription>
          <div class="mb-2">{errorMsg}
</div>
