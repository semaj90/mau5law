<!--
  Advanced 3D Evidence Visualization Component
...
if (!browser) return;
  ...
  async function initializeVisualization() {
    ...
    // Create CSS3D renderer for HTML overlays
    ...
    containerElement.appendChild(cssRenderer.domElement);
  ...
  function setupLighting() {
    ...
    const pointLight1 = new THREE.PointLight(0x4a90e2, 0.6, 50);
    pointLight1.position.set(-20, 10, 10);
    scene.add(pointLight1);

    const pointLight2 = new THREE.PointLight(0xe24a4a, 0.4, 30);
  ...
  function setupEventListeners() {
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      const intersects = raycaster.intersectObjects(Array.from(evidenceNodes.values()));

      if (intersects.length > 0) {
        const intersectedObject = intersects[0].object;
        const evidenceId = intersectedObject.userData.evidenceId;
        const evidence = evidenceData.find(e => e.id === evidenceId);

        if (evidence !== hoveredEvidence) {
          hoveredEvidence = evidence;
          highlightEvidence(intersectedObject, true);
        }
      } else if (hoveredEvidence) {
        if (hoveredEvidence) {
          const hoveredNode = evidenceNodes.get(hoveredEvidence.id);
          if (hoveredNode) highlightEvidence(hoveredNode, false);
        }
        hoveredEvidence = null;
      }

      // Broadcast cursor position in collaborative mode
      if (collaborativeMode && websocketStore.connected) {
  ...
  async function createNetworkVisualization() {
    ...
    evidenceData.forEach((evidence, index) => {
      const node = createNetworkNode(evidence, positions[index]);
      evidenceNodes.set(evidence.id, node);
      scene.add(node);
    });
  }
  ...
  async function createTimelineVisualization() {
    ...
    sortedEvidence.forEach((evidence, index) => {
      const position = new THREE.Vector3(
        (index - sortedEvidence.length / 2) * 5,
        0,
        0
      );

      const node = createTimelineNode(evidence, position);
      evidenceNodes.set(evidence.id, node);
      scene.add(node);
    });
  ...
  function createEvidenceNode(evidence: any, position: THREE.Vector3): THREE.Object3D {
    ...
    labelDiv.className = 'evidence-label';
    labelDiv.textContent = evidence.title || `Evidence ${evidence.id}`;
  ...
  function createNetworkNode(evidence: any, position: THREE.Vector3): THREE.Object3D {
    const group = new THREE.Group();
    group.position.copy(position);
    group.userData = { evidenceId: evidence.id, evidence };
  ...
  function createTimelineNode(evidence: any, position: THREE.Vector3): THREE.Object3D {
    const group = new THREE.Group();
    group.position.copy(position);
    group.userData = { evidenceId: evidence.id, evidence };
    ...
    // Add date label
    const date = new Date(evidence.createdAt).toLocaleDateString();
    const labelDiv = document.createElement('div');
    labelDiv.textContent = date;
    labelDiv.style.cssText = `
      color: white;
      font-family: Arial, sans-serif;
      font-size: 10px;
    ...
    const labelObject = new CSS3DObject(labelDiv);
    labelObject.position.copy(position);
    labelObject.position.y -= 1.5;
    labelObject.scale.set(0.008, 0.008, 0.008);
  ...
  function createRelationshipVisualization() {
    relationshipLines.clear();

    // Create connections between related evidence
    evidenceData.forEach(evidence1 => {
      evidenceData.forEach(evidence2 => {
        if (evidence1.id !== evidence2.id && areRelated(evidence1, evidence2)) {
          const line = createConnectionLine(evidence1.id, evidence2.id);
          if (line) relationshipLines.add(line);
        }
      });
    });

    scene.add(relationshipLines);
  }

  function createConnectionLine(evidenceId1: string, evidenceId2: string): THREE.Line | null {
    const node1 = evidenceNodes.get(evidenceId1);
    const node2 = evidenceNodes.get(evidenceId2);
  ...
  function createTimelinePath(sortedEvidence: any[]) {
    if (sortedEvidence.length < 2) return;

    const points = sortedEvidence.map((evidence, index) =>
  ...
  function generateSpatialLayout(data: any[]): THREE.Vector3[] {
    // 3D spiral layout
    ...
    data.forEach((_, index) => {
      ...
      const y = (index / data.length) * height - height / 2;
  ...
  function generateNetworkLayout(data: any[]): THREE.Vector3[] {
  ...
  function getEvidenceColor(evidence: any): number {
    const colors = {
      'document': 0x4CAF50,
      'photo': 0x2196F3,
      'video': 0x9C27B0,
  ...
  function highlightEvidence(node: THREE.Object3D, highlight: boolean) {
    ...
    if (mesh && mesh.material instanceof THREE.MeshLambertMaterial) {
      ...
      node.scale.setScalar(highlight ? 1.2 : 1.0);
    }
  }

  function selectEvidence(evidence: any) {
    // Clear previous selection
    if (selectedEvidence) {
      const prevNode = evidenceNodes.get(selectedEvidence.id);
      if (prevNode) highlightEvidence(prevNode, false);
    }

    selectedEvidence = evidence;

    if (evidence) {
      const node = evidenceNodes.get(evidence.id);
      if (node) {
        highlightEvidence(node, true);
        ...
        function animateCamera() {
          ...
          controls.target.lerp(node.position, easeProgress);
      ...
      // Broadcast selection in collaborative mode
      if (collaborativeMode && websocketStore.connected) {
        websocketStore.broadcastEvidenceEdit(caseId, 'select', { evidenceId: evidence.id });
      }
    }
  }
  ...
  function setupCollaboration() {
    ...
    // Listen for other users' selections and cursor movements
    // This would integrate with the websocket store events
  }
  ...
  function changeViewMode(mode: '3d' | 'network' | 'timeline') {
    viewMode = mode;
    loadEvidenceData();
  }
  ...
  function applyFilter(type: string) {
    ...
    evidenceNodes.forEach((node, evidenceId) => {
      const evidence = evidenceData.find(e => e.id === evidenceId);
      const visible = type === 'all' || evidence?.type === type;
  ...
  function exportVisualization() {
    ...
    link.download = `evidence-visualization-${caseId}-${Date.now()}.png`;
...
<div class="visualization-container" style="position: relative; width: {width}px; height: {height}px;">
  ...
  {#if selectedEvidence}
    <div class="evidence-panel">
      ...
      <p><strong>Created:</strong> {new Date(selectedEvidence.createdAt).toLocaleDateString()}</p>
      {#if selectedEvidence.aiSummary}
