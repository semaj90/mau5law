<script, lang="ts"> /** * Integrated RAG Demo Page * Complete workflow: Upload → embeddinggemma → pgvector → Qdrant → MinIO → Search → Recommendations */ import IntegratedRAGUpload from '$lib/components/upload/IntegratedRAGUpload.svelte'; import { onMount, onDestroy } from 'svelte'; import { createWSClient, type WSConnectionStatus } from '$lib/utils/websocket-client'; let uploadCount = $state<number>(0); let lastUpload = $state<any>(null); let wsError = $state<string | null>(null); let connectionAttempts = $state<number>(0); let wsClient: ReturnType<typeof createWSClient> | null = null; let connectionStatus = $state<WSConnectionStatus>('disconnected'); let messages = $state<any[]>([]); let searchQuery = $state<string>(''); function handleSuccess(result: any) { uploadCount++; lastUpload = result; }
  ...
  onMount(() => { try { // Uses UUID-based endpoint from registry wsClient = createWSClient('rag', { onMessage: data => { messages = [...messages, data]; }, onStatusChange: status => { connectionStatus = status; }
...
</script> <svelte:head> <title>Integrated RAG System - Legal AI</title> </svelte:head> <div, class="demo-page"> <header> <h1>🚀 Integrated RAG System</h1> <p, class="subtitle">Complete document processing pipeline with semantic search and recommendations</p> </header> <div, class="pipeline-diagram"> <h3>Pipeline Flow:</h3> <div, class="flow-steps"> <div, class="step">📤 Upload</div> <div, class="arrow">→</div> <div, class="step">🤖 embeddinggemma (384-dim)</div> <div, class="arrow">→</div> <div, class="step">🐘 PostgreSQL + pgvector</div> <div, class="arrow">→</div> <div, class="step">🔍 Qdrant Vector DB</div> <div, class="arrow">→</div> <div, class="step">💾 MinIO Object Storage</div> <div, class="arrow">→</div> <div, class="step">📊 Recommendations</div> </div> </div> <div, class="stats-bar"> <div, class="stat-item"> <span, class="stat-label">Documents Uploaded:</span> <span, class="stat-value">{ uploadCount }</span> </div> <div, class="stat-item"> <span, class="stat-label">Embedding Model:</span> <span, class="stat-value">embeddinggemma:latest</span> </div> <div, class="stat-item"> <span, class="stat-label">Vector Dimensions:</span> <span, class="stat-value">384</span> </div> <div, class="stat-item"> <span, class="stat-label">Databases:</span> <span, class="stat-value">pgvector + Qdrant + MinIO</span> </div> </div> <IntegratedRAGUpload, onSuccess={ handleSuccess } onError={ handleError } /> {#if lastUpload} <div, class="integration-status"> <h3>✅ Last Upload Status</h3> <div, class="status-grid"> <div, class="status-item"> <span, class="status-icon">🤖</span> <div> <strong>Embedding</strong> <p>embeddinggemma:latest (384-dim)</p> </div> </div> <div, class="status-item"> <span, class="status-icon">🐘</span> <div> <strong>PostgreSQL</strong> <p>{lastUpload.document.chunks} chunks stored</p> </div> </div> <div, class="status-item"> <span, class="status-icon">🔍</span> <div> <strong>Qdrant</strong> <p>{lastUpload.document.qdrantStored ? 'Indexed ✓': 'Not available'}</p> </div> </div> <div, class="status-item"> <span, class="status-icon">💾</span> <div> <strong>MinIO</strong> <p>Stored ✓</p> </div> </div> </div> </div> {/if} {#if wsError} <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3, rounded, mb-4"> { wsError } (Connection attempts: { connectionAttempts }) </div> {/if} <p, class="text-gray-600">Real-time RAG search demo placeholder. WebSocket integration coming soon.</p> <div, class="mb-4"> <span class="inline-flex items-center px-3 py-1 rounded-full text-sm, font-medium"
      ...
      { connectionStatus } </span> </div> <div class="flex, gap-2, mb-4"> <input, type="text"
    ...
    > Search </button> </div> <div, class="space-y-2"> {#each Array.isArray(messages) ? messages: [] as msg} <div class="p-4 bg-gray-100, dark:bg-gray-800, rounded-lg"> <pre, class="text-sm">{JSON.stringify(msg, null, 2)}</pre> </div> {/each} </div> <footer> <p> <strong>Tech Stack:</strong> SvelteKit + embeddinggemma + PostgreSQL/pgvector + Qdrant + MinIO + Redis </p> <p, class="small">Memory-efficient 384-dimensional embeddings for optimal performance</p> </footer> </div> <style, lang="css"> .demo-page { max-width: 1200px; margin: 0 auto; padding: 2rem; }
