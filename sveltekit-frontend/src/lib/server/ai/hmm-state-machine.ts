...
export class HMMStateMachine {
  ...
  private initializeStates(): void {
    for (let i = 0; i < this.numStates; i++) {
      this.states.set(i, {
        currentState: i,
        transitionProb: 1.0 / this.numStates,
        emissionProb: 0,
        pattern: [],
        stateHistory: []
      });
  ...
  private generatePrediction(
    ...
    const stateActions: Record<number, { action: string;, reasoning: string;
      ...
      [LegalConversationState.CASE_INQUIRY]: {
        ...
        reasoning: 'User needs to provide case information for analysis',
        requiredContext: ['case_number', 'jurisdiction', 'parties'],
        estimatedDuration: 30000
      },
      [LegalConversationState.DOCUMENT_ANALYSIS]: {
        action: 'analyze_documents',
        reasoning: 'Documents need OCR and legal entity extraction',
        requiredContext: ['document_ids', 'document_types'],
        estimatedDuration: 60000
      },
      [LegalConversationState.LEGAL_RESEARCH]: {
        action: 'search_precedents',
        reasoning: 'Similar cases and legal precedents are needed',
        requiredContext: ['case_facts', 'jurisdiction', 'practice_area'],
        estimatedDuration: 45000
      },
      [LegalConversationState.RISK_ASSESSMENT]: {
        action: 'assess_legal_risks',
        reasoning: 'Multi-agent risk analysis required',
        requiredContext: ['case_details', 'documents', 'precedents'],
        estimatedDuration: 90000
      },
      [LegalConversationState.RECOMMENDATION]: {
        action: 'generate_recommendations',
        reasoning: 'Synthesize findings into actionable recommendations',
        requiredContext: ['risk_assessment', 'precedents', 'case_strategy'],
        estimatedDuration: 40000
      },
      [LegalConversationState.FOLLOW_UP]: {
        action: 'address_follow_up_questions',
        reasoning: 'User has additional questions or clarifications',
        requiredContext: ['previous_analysis'],
  ...
  updateState(
    ...
    return {
      ...
      transitionProb,
      emissionProb: currentHMMState.emissionProb,
  ...
  detectPatterns(stateHistory: number[]): { commonPatterns: number[][];, frequency: number;
    ...
    const patterns = Array.from(patternMap.entries())
      .map(([key, freq]) => ({
        commonPatterns: [key.split('-').map(Number)],
        frequency: freq
      }))
