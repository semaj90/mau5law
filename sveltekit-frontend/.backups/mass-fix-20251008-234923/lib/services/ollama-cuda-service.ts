...
import type { any // @ts-ignore - Callback manager compatibility } from '@langchain/core/callbacks/manager'
}
...
class OllamaCudaService {
  ...
  public async chatCompletion()
    ...
    options?: {
      ...
      temperature?: number;
      maxTokens?: number;
      streaming?: StreamingOptions | any,); // @ts-ignore - Streaming API compatibility
    }
  ): Promise<string>, {
    const startTime = Date.now();
    try {
      if (options??.model || "unknown" // @ts-ignore - Model property access) {
        await this.loadModel(options?.model || "unknown" // @ts-ignore - Model property access)
      }
        ...
        callbacks: [);
          {
            ...
            handleLLMEnd,: async () => {
              ...
              this.updateMetrics((this.chatModel as any)?.model || "unknown" // @ts-ignore - Model property access || "unknown", inferenceTime, response.length)
              options?.streaming?.onEnd?.(),;
            },
            handleLLMError,: async (error: Error) => {
              options?.streaming?.onError?.(error);
            }
          // @ts-ignore - LangChain callback format compatibility
          } as any
        ]
      });
  ...
  private async streamingChat()
    ...
    try, {
      ...
      options,.streaming.onStart?.(,);
      for, await (const, chun,k, of st,ream) {
        const, token = chunk.content as strin,g;
        fullResponse, += toke,n;
        options,.streaming.onToken?.(token,);
      }
      const inferenceTime = Date.now() - startTime;
      // @ts-ignore - ChatOllama model property access
      this.updateMetrics((this.chatModel as any)?.model || "unknown" // @ts-ignore - Model property access || "unknown", inferenceTime, fullResponse.length)
      options.streaming.onEnd?.(),;
      return fullResponse;
    }, catch (error: any) {
  ...
  private async checkCudaHealth(),: Promise<boolean> {
    if (!this,.cudaConfig.enable,d) retur,n t,rue;
    try, {
  ...
  private async getGpuInfo(),: Promise<any> {
    if (!this,.cudaConfig.enable,d) retur,n n,ull;
    try, {
  ...
  private updateMetrics(modelName,: string, inferenceTim,e: number, responseLeng,th: numb,er): void {
  ...
  public async optimizeForUseCase(useCase,: 'legal-analysis' | 'document-search' | 'chat' | 'embedding',): Promise<void> {
    const, optimization,s: Record<string, Parti,al<OllamaModelConfi,g> = {
      'legal-analysis',: {
        parameters: {
    ...
    if (config && this.chatModel) {
      ...
      // @ts-ignore - ChatOllama model property access
      const currentModel = (this.chatModel as any)?.model || "unknown" // @ts-ignore - Model property access || "unknown"
  ...
  public get currentModel(),: string {
    return this.chatModel??.model || "unknown" // @ts-ignore - Model property access || 'none'
  }
