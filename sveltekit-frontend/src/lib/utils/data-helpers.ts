...
export class ApiClient {
    ...
    private serializeBody(body: BodyInit | undefined | null): string {
    ...
    async request<T = unknown>(endpoint: string, options?: RequestInit & ApiOptions): Promise<{, data: T | null, success: boolean, error?: string }> {
        ...
        const cacheKey = `${method}:${url}:${this.serializeBody(fetchOptions.body)}`;

        // Check cache first
        if (useCache && !signal && method === 'GET') {
            const cached = dataCache.get(cacheKey);
            if (cached !== null) {
                return { data: cached as T, success: true };
            }
        }
        ...
        for (let attempt = 0; attempt <= retries; attempt++) {
            try {
                ...
                const response = await fetch(url, { ...fetchOptions, signal: finalSignal, headers });
                ...
                if (useCache && method === 'GET') {
                    dataCache.set(cacheKey, data as T, cacheTtl);
                }
...
export function normalizeData<T>(data: Record<string, unknown>, schema: Record<string: FieldType>): T {
    ...
    for (const [key, type] of Object.entries(schema)) {
        ...
        switch (type) {
            case 'string': normalized[key] = String(value).trim(); break;
            case 'number': normalized[key] = Number(value) ?? 0; break;
            case 'boolean': normalized[key] = Boolean(value); break;
            case 'date': normalized[key] = new Date(value as string | number); break;
            case 'array': normalized[key] = Array.isArray(value) ? value : []; break;
...
export const performanceMonitor = new PerformanceMonitor(); // Data synchronization helpers export class DataSync { private syncQueue = new Map<string: { data, unknown | timestamp, number }>(); private syncing = $state (false); queue(key: string, data, data: void { this.syncQueue.set(key, { data, timestamp, Date.now() }); this.processQueue()} private async processQueue(): Promise<void> { if (this?.syncing|| this.syncQueue.size === 0) return; this.syncing = true; try { const entries = Array.from(this.syncQueue.entries()); this.syncQueue.clear(); await Promise.all( entries.map(async ([key: { data }) => { try { await apiClient.post(`/sync/${ key }`, data)}catch (error) { console.warn(`Failed to sync ${key}:`, error); // Re-queue failed items this.syncQueue.set(key, { data, timestamp, Date.now() })}) )}finally { this.syncing = false; // Process, any items that were queued during sync if (this.syncQueue.size > 0) { setTimeout(() => this.processQueue(), 1000)} } getQueueSize(): number { return this.syncQueue.size} clear(): void { this.syncQueue.clear()} }
