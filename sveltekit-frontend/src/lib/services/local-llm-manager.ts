...
export interface LocalLLMConfig { models: { embedding: { name: string;, url: string;
  size: string;
  dimensions: number;
  description: string;
    } }
...
export const localLLMConfig: LocalLLMConfig = { models: { embedding: { name: "legal-bert-base-uncased",
  ...
  paths: { modelsDir: dev ? "./local-llms" : "%LOCALAPPDATA%/LegalRAG/models",
    ...
    try {
      if (typeof window !== "undefined" && window.__TAURI__) {
        ...
        await fetch("/api/llm/setup-directories", {
          ...
          body: JSON.stringify({ modelsPath: this.modelsPath,
