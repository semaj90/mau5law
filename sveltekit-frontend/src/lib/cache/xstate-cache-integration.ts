...
type CacheActorResult =
  | { success: true; hit: true; data: Record<string, unknown>; metadata: CacheContext['cacheMetadata'] }
...
export const cacheActions = {
  ...
  /** * Clear cache state */
  clearCacheState: assign((): Partial<BaseMachineContext> => ({
    cache: {
      cacheKey: null,
      cachedData: null,
      cacheHit: false,
        ...
        onDone: [
          {
            ...
            guard: (_ctx: BaseMachineContext, _ev: DoneInvokeEvent<CacheActorResult>) => {
              ...
              return !!_ev.output && 'hit' in _ev.output && (_ev.output as { hit: boolean }).hit === true;
            },
