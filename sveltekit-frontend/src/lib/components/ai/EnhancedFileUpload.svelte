...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { createUploadMachine } from '$lib/machines/uploadMachine'; import type { ProcessingPipeline } from '$lib/types/upload'; import { toast } from '$lib/utils/toast'; // import only stable icons; replace problematic icons with inline fallbacks below import { FileText, Search, Upload } from 'lucide-svelte'; import { onMount } from 'svelte'; import { createActor } from 'xstate'; // Props interface interface Props { onUploadComplete?: (doc: any) => void; accept?: string; maxSize?: number; enableOCR?: boolean; enableEmbedding?: boolean; enableRAG?: boolean; class?: string}

  // Svelte, 5 props with defaults let { onUploadComplete = () => {}, accept = '.pdf,.docx,.txt,.jpg,.png,.tiff', maxSize = 50 * 1024 * 1024, // 50MB enableOCR = true, enableEmbedding = true, enableRAG = true, class: className = ''
 ...
 <div class="space-y-3">
  {#each Array.isArray(searchResults) ? searchResults: [] as result} <div class="result-item p-4 border rounded-lg"> <div class="flex items-start"> <div class="flex-1"> <p class="font-medium">{(result as: any).filename}</p>
 <p class="text-sm text-gray-600">{(result as: any).excerpt}</p>
 <div class="flex items-center gap-4 mt-2 text-xs"> <span>Similarity: {((result, as: any).similarity * 100).toFixed(1)}%</span>
 <span>Type: {(result, as: any).searchType}</span>
  {#if (result as: any).matchedBy} <span>Matched by: {(result, as: any).matchedBy.join(', ')}</span> {/if}
  </div> </div>
 ...
 <style> .enhanced-file-upload { padding: 1.5rem; background: #fff; border-radius: 0.5rem; box-shadow: 0 1px 2px rgba(0, 0, 0, 0.05)}
  ...
  .upload-label { display: flex; flex-direction: column; align-items: center; justify-content: center; cursor: pointer}
  .file-item { padding: 1rem; border: 1px solid #e5e7eb; border-radius: 0.5rem; margin-bottom: 0.75rem; background: #f9fafb}
  .search-section { border-top: 1px solid #e5e7eb; padding-top: 1.5rem}
  .result-item { cursor: pointer; transition: background-color 0.15s}
