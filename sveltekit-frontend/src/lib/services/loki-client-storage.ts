...
export class LokiClientStorage {
  ...
  exportData(): {
    ...
    const exportData = { metadata: { exported_at: new Date().toISOString(),
...
export const legalAIStorageConfig: LokiStorageConfig = { dbName: 'legal_ai_client_db',
  collections: { cases: { indices: ['id', 'title', 'status', 'created_at'],
      unique: ['id']
    },
    documents: { indices: ['id', 'case_id', 'title', 'document_type', 'created_at'],
      unique: ['id']
    },
    evidence: { indices: ['id', 'case_id', 'priority', 'type'],
      unique: ['id']
    },
    search_history: { indices: ['user_id', 'query', 'timestamp'],
      ttl: 604800000, // 7 days
    },
    chat_messages: { indices: ['session_id', 'user_id', 'timestamp'],
      ttl: 2592000000, // 30 days
    },
    user_preferences: { indices: ['user_id'],
      unique: ['user_id']
    } }
  },
