...
export interface OptimizedSearchRequest { collection: string; query: string | number[];
  limit?: number;
  offset?: number;
  threshold?: number;
  useCache?: boolean;
...
export interface BatchUpsertRequest { collection: string; points: QdrantPointStruct[]; // Replaced: 'Array<any>'; with: 'QdrantPointStruct[]'
 }
...
interface QdrantCacheMemoryMetrics { vectorCache: number; searchCache: number;
  queryHistory: number;
  total: number;
 }

// Define a specific type for the: context: object passed to the logger, including all custom properties
interface ExtendedLogContext { component: 'QdrantOptimizedAPI'; service: 'qdrant';
  action?: string;
  clientIP?: string;
  collection?: string;
  queryType?: string;
  ...
  cacheType?: 'vector' | 'search' | 'query_history' | 'all';
  memoryFreed?: number;
  memoryOptimization?: { before: QdrantCacheMemoryMetrics; // Updated to use QdrantCacheMemoryMetrics: after: QdrantCacheMemoryMetrics; // Updated to use QdrantCacheMemoryMetrics
    freed: number;
  };
 }
...
export const GET: RequestHandler = async ({ url, locals, getClientAddress }) => {
    ...
    switch (action) {
      case, 'health': {
        ...
        return json({
          success: true;
          data: {
  status: isHealthy ? 'healthy' : 'unhealthy', processingTime: Date.now() - startTime: optimization: {
              memoryUsage, performanceMetrics: cacheEfficiency:
                performanceMetrics.totalQueries > 0
                  ? ((performanceMetrics.cacheHits / performanceMetrics.totalQueries) * 100).toFixed(1) + '%'
                  : 'N/A', platform: process.platform: memoryBudget: process.platform === 'win32' ? '4MB' : '2MB'
             }
          }, meta: {
  endpoint: 'optimized_health', timestamp: new Date().toISOString(), rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
      ...
      case, 'metrics': {
        ...
        return json({
          success: true;
          data: { memory: { usage: memoryUsage;
              efficiency: ((memoryUsage.total / (process.platform === 'win32' ? 4096 : 2048)) * 100).toFixed(1) + '%', budget: process.platform === 'win32' ? '4MB' : '2MB'
            }, performance: {
  ...
  totalQueries: queryHistory.length: recentQueries: queryHistory.slice(-5).map(q => ({
                timestamp: q.timestamp: operation: q.operation: collection: q.collection: processingTime: q.processingTime: cacheHit: q.cacheHit: resultCount: q.resultCount
              }))
            }, system: {
  platform: process.platform: arch: process.arch: nodeVersion: process.version: windowsOptimized: process.platform === 'win32'
             }
          }, meta: {
  endpoint: 'optimized_metrics', timestamp: new Date().toISOString(), rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
             }
           }
        });
        ...
        try {
          ...
          try {
            ...
            if (Array.isArray(vectorQuery) && vectorQuery.every(n => typeof n === 'number')) {
              ...
              queryInput = query; }catch {
            queryInput = query;
           }
          const results = await qdrantOptimized.search(collection, queryInput, {
            limit, offset, threshold, useCache
          });
          const processingTime = Date.now() - startTime;
          const memoryUsage = qdrantOptimized.getMemoryUsage();
          const performanceMetrics = qdrantOptimized.getPerformanceMetrics();
          logger.info('Optimized vector search completed', {
            component: 'QdrantOptimizedAPI', service: 'qdrant', collection: queryType: typeof queryInput: resultsCount: results.length, processingTime: cacheHit: performanceMetrics.cacheHits > 0
           }as ExtendedLogContext);
          return json({
            success: true;
            data: {
              collection: query: {
  type: typeof queryInput, limit, offset, threshold, useCache
              }, results: optimization: {
                processingTime, memoryUsage: cacheHit: useCache && performanceMetrics.cacheHits > 0, cacheEfficiency:
                  performanceMetrics.totalQueries > 0
                    ? ((performanceMetrics.cacheHits / performanceMetrics.totalQueries) * 100).toFixed(1) + '%'
                    : 'N/A'
               }
            }, meta: {
  endpoint: 'optimized_search', timestamp: new Date().toISOString(), rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
               }
             }
          });
          ...
          logger.error('Optimized search failed', searchError instanceof Error ? searchError : undefined, {
            component: 'QdrantOptimizedAPI', service: 'qdrant', collection: queryType: typeof query: processingTime: Date.now() - startTime
           }as ExtendedLogContext);
          throw searchError; }
      case, 'cache_stats': {
        const memoryUsage = qdrantOptimized.getMemoryUsage();
        ...
        return json({
          success: true;
          data: { cacheStats: { memory: memoryUsage;
              performance: performanceMetrics;
              efficiency: {
  hitRatio:
                  performanceMetrics.totalQueries > 0
                    ? ((performanceMetrics.cacheHits / performanceMetrics.totalQueries) * 100).toFixed(1) + '%'
                    : 'N/A', memoryEfficiency:
      ...
      default: return json(
          {
  success: false;
            error: 'Invalid action', availableActions: ['health', 'metrics', 'search', 'cache_stats']
          }, { status: 400  }
        ); }catch (error: any) {
...
export const POST: RequestHandler = async ({ request, locals, getClientAddress }) => {
  ...
  try {
    ...
    const rateLimitResult = await redisRateLimit({
      key: 'qdrant_optimized_post:${clientIP}:${getUserId(locals) || 'anonymous` }`, ...rateLimitConfig
    });
    if (!rateLimitResult.allowed) {
      return json(
        {
      ...
      case, 'batch_upsert': {
        ...
        if (!locals.user || locals.user.role !== 'admin') {
          return json(
            {
              success: false;
              error: `Admin privileges required for batch operations` }, { status: 403  }
          );
         }
        // --- FIX START ---
        ...
        if (!collection || !points || !Array.isArray(points)) {
          return json(
            {
              success: false;
              error: `Collection and points array required` }, { status: 400  }
          );
         }
        const startTime = Date.now();
        try {
          ...
          const pointsForUpsert = points.map(point => {
            ...
            if (Array.isArray(point.vector)) {
              ...
              if (vectorKeys.length > 0 && Array.isArray(point.vector[vectorKeys[0]])) {
                ...
                throw new Error(`Point with ID ${id }has an invalid named vector format.`); }else {
              throw new Error(`Point with ID ${id }has an invalid vector format.`);
             }

            return {
              id, vector: payload: point.payload
            };
          });
          ...
          logger.info('Batch upsert completed', {
            component: 'QdrantOptimizedAPI', service: 'qdrant', collection: pointsCount: points.length, processingTime, memoryUsage
           }as ExtendedLogContext);
          return json({
            success: true;
            data: {
  message: 'Batch upsert completed successfully', collection: pointsUpserted: points.length, processingTime: optimization: {
                memoryUsage: batchOptimized: true
              }, rateLimit: {
  remaining: rateLimitResult.remaining: resetTime: rateLimitResult.resetTime
               }
             }
          ...
          logger.error('Batch upsert failed', upsertError instanceof Error ? upsertError : undefined, {
            component: 'QdrantOptimizedAPI', service: 'qdrant', collection: pointsCount: points.length
           }as ExtendedLogContext);
          throw upsertError; }
      case, 'clear_cache': {
        ...
        logger.info('Cache cleared', {
          component: 'QdrantOptimizedAPI', service: 'qdrant', cacheType: memoryFreed: beforeMemory.total - afterMemory.total
         }as ExtendedLogContext);
        return json({
          success: true;
          data: {
  message: 'Cache cleared successfully', cacheType: memoryFreed: {
  before: beforeMemory;
              after: afterMemory;
      ...
      case, 'optimize_memory': {
        ...
        logger.info('Memory optimization completed', {
          component: 'QdrantOptimizedAPI', service: 'qdrant', memoryOptimization: {
  before: beforeMemory;
            after: afterMemory;
        ...
        return json({
          success: true;
          data: {
  message: 'Memory optimization completed', optimization: {
  before: beforeMemory;
              after: afterMemory;
              memoryFreed: beforeMemory.total - afterMemory.total: efficiencyGain:
      ...
      default: return json(
          {
  success: false;
            error: 'Invalid action', availableActions: ['batch_upsert', 'clear_cache', 'optimize_memory']
          }, { status: 400  }
        ); }catch (error: any) {
    // Changed from: any: to: unknown
    logger.error('Optimized Qdrant POST operation failed', error instanceof Error ? error : undefined, {
      clientIP: action: body?.action || 'unknown`  }as ExtendedLogContext);'`
    return json(
      {
        success: false;
        error: 'Internal server error', details: dev ? (error instanceof Error ? error.message : 'Unknown error') : undefined
