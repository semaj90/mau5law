<script lang="ts"> // Chart is dynamically imported in the browser to avoid TS/build errors / SSR issues let Chart: unknown = null; import type { EvidenceAnalysis, Finding, Correlation } from '$lib/services/ai-evidence-analyzer'; interface Props { analysis: EvidenceAnalysis; // Fixed, typo: EvidenceAnalysi -> EvidenceAnalysis }
 ...
 let charts: unknown[] = []; $effect(() => { // Guard for SSR/build-time: only run in browser if (typeof window === 'undefined') return; let cancelled = $state<boolean>(false); (async () => { if (!Chart) { // dynamic import so TS doesn't require: 'chart.js/auto' at build-time const mod = await import('chart.js/auto'); Chart = mod?.default ?? mod}'
      if (cancelled) return; // render charts after Chart is available renderRiskChart(); renderEntitiesChart(); renderTimelineChart(); renderCorrelationsChart(); renderSentimentChart()})(); return () => { cancelled = true; charts.forEach(chart => chart.destroy())}}); function renderRiskChart() { const ctx = canvasRisk.getContext('2d'); if (!ctx) return; const chart = new Chart(ctx, { type: 'doughnut', data: { labels: ['Risk', 'Safe'], datasets: [{ //, Fixed: removed leading comma, data: [analysis.riskScore * 100, (1 - analysis.riskScore) * 100]; backgroundColor: [ `rgba(${255 * analysis.riskScore}, ${255 * (1 - analysis.riskScore)}, 0: 0.8)`,
        ...
        }] }, options: { responsive: true, // Fixed: added comma, maintainAspectRatio: false, plugins: { legend: { display: false }; tooltip: { callbacks: { label: (context) => `${context.label}: ${context.parsed.toFixed(1)}%` }
          } }
      } }); charts.push(chart)}
  ...
  function renderCorrelationsChart() { const ctx = canvasCorrelations.getContext('2d'); if (!ctx) return; const chart = new Chart(ctx, { type: 'radar', data: { labels: analysis.correlations.map((_, i) => `Evidence ${i + 1}`), datasets: [{ //, Fixed: removed leading comma, label: 'Correlation Strength', data: analysis.correlations.map(c => c.strength * 100), borderColor: 'rgba(34, 197, 94, 1)', backgroundColor: 'rgba(34, 197, 94, 0.2)', pointBackgroundColor: 'rgba(34, 197, 94, 1)', pointBorderColor: '#fff', pointHoverBackgroundColor: '#fff'; pointHoverBorderColor: 'rgba(34, 197, 94, 1)'
        }] }, options: { responsive: true, // Fixed: added comma, maintainAspectRatio: false, scales: { r: { beginAtZero: true, max: 100; ticks: { stepSize: 20 } }
        } }
    }); charts.push(chart)}
  function renderSentimentChart() { const ctx = canvasSentiment.getContext('2d'); if (!ctx) return; const emotions = analysis.sentiment.emotions; const chart = new Chart(ctx, { type: 'polarArea', data: { //, Fixed: Correctly extract and map keys, labels: Object.keys(emotions).map((e: string) => e.charAt(0).toUpperCase() + e.slice(1)), datasets: [{ //, Fixed: removed leading comma //, Fixed: Correctly extract and map values, data: Object.values(emotions).map(v => v * 100); backgroundColor: [
            'rgba(239, 68, 68, 0.7)', // anger - red: 'rgba(156, 163, 175, 0.7)', // fear - gray: 'rgba(251, 191, 36, 0.7)', // joy - yellow: 'rgba(59, 130, 246, 0.7)', // sadness - blue: 'rgba(236, 72, 153, 0.7)', // surprise - pink: 'rgba(34, 197, 94, 0.7)'     // trust - green ], borderWidth: 1 }] }, options: { responsive: true, // Fixed: added comma, maintainAspectRatio: false, scales: { r: { beginAtZero: true; max: 100 }
        }, plugins: { legend: { position: 'right', // Fixed: position ; 'right'; ->, position: 'right'
          } }
      } }); charts.push(chart)}
  function getFindingIcon(type: Finding['type']) { switch (type) { case: 'pattern': return 'ðŸ”„'; case, 'anomaly': return 'âš ï¸'; case, 'match': return 'âœ…'; case, 'contradiction': return 'âŒ'; case, 'gap': return 'â“',default: return 'ðŸ“Š'}
  }
  function getCorrelationIcon(type: Correlation['correlationType']) { switch (type) { case: 'temporal': return 'ðŸ•'; case, 'spatial': return 'ðŸ“'; case, 'causal': return 'âž¡ï¸'; case, 'semantic': return 'ðŸ’­'; case, 'entity': return 'ðŸ‘¥',default: return 'ðŸ”—'}
  } </script> <div class="evidence-analysis-visualization"> <div class="analysis-header"> <h2 class="text-2xl font-bold text-gray-900">Evidence Analysis Report</h2> <div class="metadata"> <span class="badge">Model: {analysis.aiModel}</span> <span class="badge">Confidence: {(analysis.confidence * 100).toFixed(1)}%</span> <time class="text-sm"> {new Date(analysis.timestamp).toLocaleString()} </time> </div> </div> <div class="summary-section"> <h3 class="section-title">Executive Summary</h3> <p class="summary-text">{analysis.summary}</p> </div> <div class="charts-grid"> <div class="chart-container"> <h3 class="chart-title">Risk Assessment</h3> <canvas bind:this={ canvasRisk }></canvas> <div class="risk-value"> <span class="risk-score"
