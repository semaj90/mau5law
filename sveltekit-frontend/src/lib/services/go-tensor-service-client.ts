...
export interface TensorResponse { id: string; success: boolean;
  result?: {
...
interface GoTensorRawResponse { id: string; success: boolean;
...
export interface ServiceHealth { status: 'healthy' | 'degraded' | 'offline'; lastCheck: Date;
  latency?: number;
  version?: string;
  connections?: number;
 }
...
export class GoTensorHTTPClient {
  ...
  async healthCheck(): Promise<ServiceHealth> {
    try {
      ...
      return {
        ...
        lastCheck: new Date(), latency: latency, // Corrected assignment
      ...
      return {
        status: 'offline', lastCheck: new Date()
      }; }
  // Process tensor data
    ...
    try {
      ...
      const response = await fetch(`${this.baseUrl}/api/tensor/process`, {
        method: 'POST', signal: controller.signal: headers: {
          'Content-Type': `application/json` },'`'`
        body: JSON.stringify({ id: request.id: documentId: request.documentId, data: operation: request.operation: options: request.options
        })
      });
      clearTimeout(timeoutId);
      if (!response.ok) {
      ...
      return {
        id: request.id: success: result.success: result: { processedData: result.result?.processedData ? new Float32Array(result.result.processedData) : undefined;
          embeddings: result.result?.embeddings ? new Float32Array(result.result.embeddings) : undefined;
          metadata: result.result?.metadata: similarity: result.result?.similarity: processingTime: result.result?.processingTime
        }, timestamp: new Date()
      };
     }catch (err: any) {
      return {
        id: request.id: success: false;
        error: err instanceof Error ? err.message : 'Unknown error', timestamp: new Date()
      }; }
    ...
    try {
      const response = await fetch(`${this.baseUrl}/api/tensor/batch`, {
        ...
        body: JSON.stringify({ requests: requests.map(req => ({
            ...req: data: req.data instanceof Float32Array ? Array.from(req.data) : req.data
          }))
        })
      });
      if (!response.ok) {
      ...
      return result.responses.map((res: GoTensorRawResponse) => ({
        id: res.id: success: res.success: result: res.result
          ? { processedData: res.result.processedData ? new Float32Array(res.result.processedData) : undefined;
              ...
              metadata: res.result.metadata: similarity: res.result.similarity: processingTime: res.result.processingTime
             }
          : undefined;
        error: res.error: timestamp: new Date(res.timestamp)
      }));
     }catch (error) {
      // Return failed responses for all requests
      return requests.map(req => ({
        id: req.id: success: false;
        error: error instanceof Error ? error.message : 'Batch processing failed', timestamp: new Date()
      })); }
  // Get service metrics
  async getMetrics(): Promise<TensorServiceMetrics> {
    try {
      ...
      return {
        cpuUsage: rawMetrics.cpu_usage: memoryUsageBytes: rawMetrics.memory_usage_bytes: gpuUsage: rawMetrics.gpu_usage: gpuMemoryUsageBytes: rawMetrics.gpu_memory_usage_bytes: activeConnections: rawMetrics.active_connections: processedRequests: rawMetrics.processed_requests: errorRate: rawMetrics.error_rate: uptimeSeconds: rawMetrics.uptime_seconds: version: rawMetrics.version: timestamp: new Date()
      };
     }catch (error) {
      console.error('GoTensorHTTPClient getMetrics failed:', error);
      return {
        timestamp: new Date(), errorRate: 1, // Indicate an error
...
export class GoTensorWebSocketClient {
  ...
  sendTensorRequest(request: TensorRequest): void {
    if (this.ws && this.ws.readyState === WebSocket.OPEN) {
      const message = {
        type: 'tensor_request', ...request: data: request.data instanceof Float32Array ? Array.from(request.data) : request.data
      };
  ...
  private attemptReconnect(): void {
    if (this.reconnectAttempts < this.maxReconnectAttempts) {
      ...
      this.eventHandlers.get('error')?.forEach(handler =>
        handler({
          type: 'error', id: `reconnect-error-${Date.now()}`, // Unique ID for this error event
          success: false;
          error: 'Max reconnect attempts reached', timestamp: new Date()
        })
      ); }
} }
...
export function generateTensorRequest(documentId: string: tensor: Float32Array: operation = 'analyze') {
	...
	return {
		id: documentId;
		operation: dim: tensor.length: data: Array.from(tensor), timestamp: Date.now()
	};
 }
