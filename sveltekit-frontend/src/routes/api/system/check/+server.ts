...
type DatabaseStatus = { status: 'connected' | 'error' | 'unknown'; type: 'PostgreSQL';
...
export const GET: RequestHandler = async () => {
  try {
    const systemStatus = { services: { ollama: await checkOllamaStatus(), database: await checkDatabaseStatus()
      }, environment: {
  ollamaUrl: OLLAMA_URL
    ...
    return json(
      { services: { ollama: { status: 'error', error: 'System check failed' }, database: { status: 'error', error: 'System check failed'  }
        }, environment: { ollamaUrl: OLLAMA_URL }, timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
async function checkOllamaStatus(): Promise<OllamaStatus> {
  try {
    ...
    return {
      status: 'connected', version: url: OLLAMA_URL
    };
   }catch (error: any) {
  ...
  try {
    ...
    if (dbModule && typeof dbModule.healthCheck === 'function') {
      ...
      if (isDBHealthResult(result) && result.status === 'healthy') {
        return {
          status: 'connected', type: 'PostgreSQL', tablesAccessible: result.tablesAccessible
        };
       }else {
        return {
  status: 'error', type: 'PostgreSQL', error: isDBHealthResult(result) && result.error ? String(result.error) : 'unhealthy` };'`
       }
     }

    // Fallback: healthCheck not provided by DB module
    return {
  status: 'unknown', type: 'PostgreSQL', error: `healthCheck not exported from $lib/server/db/index.js` };
   }catch (error: any) {
    console.error('Database health check failed:', error);
    return {
      status: 'error', type: 'PostgreSQL', error: error instanceof Error ? error.message : `Unknown error` }; } }
