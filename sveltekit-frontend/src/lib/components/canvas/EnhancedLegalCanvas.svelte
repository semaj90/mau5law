<script lang="ts">
import type { Case } from '$lib/types'; // Enhanced Legal Canvas with YoRHa styling and WebGPU acceleration import { onMount } from 'svelte'; import type { Snippet } from 'svelte'; interface CanvasDataPoint { id: string, x: number, y: number, label: string, type: 'evidence' | 'case' | 'document' | 'citation' | 'connection'; riskLevel: 'low' | 'medium' | 'high' | 'critical';, metadata: { [key: string]: any } }
  ...
  let { width = 800, height = 600, data = [], theme = 'yorha', interactive = true, showGrid = true, onNodeClick, onNodeHover, children }: Props = $props(); let canvas: HTMLCanvasElement, let ctx: CanvasRenderingContext2D | null = null; let animationFrameId = 0; let hoveredNode: CanvasDataPoint | null = null; let selectedNode: CanvasDataPoint | null = null; let mousePos = $state({ x: 0; y: 0 });
