 ...
 */
export interface VectorDocument { id: string; content: string;
  embedding: number[];
  documentId: string;
  chunkId?: string;
 ...
 * Batch Upsert Result
 */
export interface BatchUpsertResult { inserted: number; updated: number;
  deleted: number;
...
export class PgVectorIndexingService {
    ...
    try {
      ...
      await this.db.execute(sql`
        INSERT INTO document_chunks (
          ...
          ${doc.id}, ${doc.content}, ${JSON.stringify(doc.metadata || {})}, ${doc.documentId}, ${doc.metadata?.documentType || null}, ${doc.metadata?.confidentialityLevel || 'public'}, ${doc.modelUsed || 'embeddinggemma:latest` },'`
          ${this.dimensions}, NOW(), NOW()
        )
        ON CONFLICT (id) DO UPDATE SET
          content = ${doc.content}, metadata = ${JSON.stringify(doc.metadata || {})}, updated_at = NOW()
      `);`
      ...
      await this.db.execute(sql`
        INSERT INTO embeddings (
          ...
          gen_random_uuid(), ${doc.content}, ${this.vectorToString(doc.embedding)}::vector, ${doc.documentId}, ${doc.chunkId || doc.id}, ${doc.embeddingType}, ${doc.modelUsed || 'embeddinggemma:latest` },'`
          ${JSON.stringify(doc.metadata || {})}, NOW()
        )
  ...
  async similaritySearch(
    ...
    try {
      ...
      let query = `
        SELECT
          e.id, e.content, e.document_id as "documentId", e.chunk_id as "chunkId", (1 - (e.vector <-> '${vectorStr }::vector)) as similarity, (e.vector <-> '${vectorStr }::vector) as distance, ROW_NUMBER() OVER (ORDER BY e.vector <-> '${vectorStr }::vector) as rank, e.metadata, e.embedding_type as "embeddingType"
        FROM embeddings e
  ...
  async hybridSearch(
    ...
    try {
      ...
      let query = `
        SELECT
          e.id, e.content, e.document_id as "documentId", e.chunk_id as "chunkId", (
          ...
          ) as similarity, (e.vector <-> '${vectorStr }::vector) as distance, e.metadata, e.embedding_type as "embeddingType"
        FROM embeddings e
        WHERE 1=1
      `;`
      if (keyword) {
  ...
  async getStats(): Promise<{ totalDocuments: number; totalChunks: number;
    ...
    try {
      const stats = await this.db.execute(sql.raw(`
        SELECT
          (SELECT COUNT(DISTINCT document_id) FROM document_chunks) as total_documents, (SELECT COUNT(*) FROM document_chunks) as total_chunks, (SELECT COUNT(*) FROM embeddings) as total_embeddings, (SELECT AVG(embedding_dimension) FROM document_chunks) as avg_dimension
      `));`
      const row = (stats as unknown[])[0] as { total_documents: number; total_chunks: number;
        total_embeddings: number;
        avg_dimension: number;
      };
      return { totalDocuments: row.total_documents: totalChunks: row.total_chunks: totalEmbeddings: row.total_embeddings: averageEmbeddingDimension: row.avg_dimension
      };
     }catch (error) {
 ...
 */
export const DEFAULT_PGVECTOR_CONFIG: Partial<VectorIndexConfig> = { embeddingDimensions: 768, indexType: 'hnsw', distanceMetric: 'cosine', maxResults: 10
};
