...
const MCP_SERVER_URL = env.MCP_SERVER_URL;
...
interface MCPMetadata {
	...
	source: string;
	timestamp: string;
	// Add other known properties that MCP might return in its metadata
...
interface MCPProcessingResult {
	...
	data?: {
		...
		metadata: MCPMetadata; // Changed from unknown to MCPMetadata
	};
	error?: string;
}
...
async function uploadToMinIO(
	...
	try {
		...
		console.log(
			`[MinIO] Using access key: ${accessKey}, secret key: ${secretKey.substring(0, 4)}...`
		); // Log keys to show usage
		return { success: true, objectPath, url: `${MINIO_ENDPOINT}/${MINIO_BUCKET}/${objectPath}` };
...
async function processWithMCP(text: string, filename: string): Promise<MCPProcessingResult> {
	try {
		const response = await fetch(`${MCP_SERVER_URL}/mcp/process`, {
			...
			body: JSON.stringify({
				text,
				metadata: { filename, source: 'legal-pdf', timestamp: new Date().toISOString() },
				options: { chunkSize: 1000, overlap: 100, simdEnabled: true, fastJsonEnabled: true }
			})
		});
		if (!response.ok) {
	...
	try {
		...
		for (const chunk of chunks) {
			const response = await fetch(`${OLLAMA_BASE_URL}/api/embeddings`, {
				method: 'POST',
				headers: { 'Content-Type': 'application/json' },
				body: JSON.stringify({ model: 'gemma', prompt: chunk })
			});
			if (!response.ok) {
				// Use response.ok directly
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		if (!mcpResult.success) {
			return json(
				{ success: false, error: `MCP processing failed: ${mcpResult.error}` },
				{ status: 500 }
			);
		}
		...
		if (!mcpResult.data) {
			return json(
				{ success: false, error: 'MCP processing succeeded but returned no data.' },
				{ status: 500 }
			);
		}
