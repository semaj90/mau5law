...
import type { Document } from, '$lib/types'; // Svelte, 5 runes are auto-imported interface Props { content?: string; placeholder?: string; readonly?: boolean; height?: string; theme?: 'default' | 'nes' | 'dark' | 'retro'; enableAI?: boolean; enableCitation?: boolean; enableCollaboration?: boolean; ondispatch?: (detail: any) => void; }
  ...
  // AI Assistant state let aiQuery = $state<string>(''); let aiResults = $state<string>(''); let isProcessingAI = $state<boolean>(false); let selectedText = $state<string>(''); // Citation state let citationQuery = $state<string>(''); let citationResults = $state<Array<{ title: string; citation: string;, relevance: number }>>([]); $effect(() => { (async () => { await initializeEditor(); })(); }); async function initializeEditor(): Promise<void> { try { // Dynamically import TinyMCE as an alternative // const { default: Hugerte } = await import('hugerte') // Initialize basic editor for now hugerte = { getContent: () => content, setContent: (newContent: string) => { content = newContent; }, destroy: () => {}, on: (event: string, callback: () => void) => {}, off: () => {}, insertContent: (newContent: string) => { content += newContent; }, selection: {, getContent: () => selectedText }, ui: { registry: {, addButton: (name: string, options: any) => {}, addIcon: () => {} }
          ...
          `, setup: (editor: any) => { // Custom AI Assistant button editor.ui.registry.addButton('ai-assistant', { text: '🤖 AI', tooltip: 'AI Assistant', onAction: () => openAIAssistant(editor.selection.getContent()) }); // Custom Citation Helper button editor.ui.registry.addButton('citation-helper', { text: '📚 Cite', tooltip: 'Citation Helper', onAction: () => openCitationHelper(editor.selection.getContent()) }); // Auto-save functionality editor.on('NodeChange', () => { updateCounts(editor.getContent()); }); // Content change listener editor.on('input', () => { const newContent = editor.getContent(); content = newContent; const wc = updateCounts(newContent); ondispatch?.({ content: newContent, wordCount: wc }); }); // Save handler editor.on('save', () => { ondispatch?.({ content: editor.getContent() }); }); }, save_enablewhendirty: true, save_onsavecallback: () => { ondispatch?.({ content: hugerte.getContent() }); }, autosave_interval: '10s', autosave_retention: '30m', // Legal document specific settings spellchecker_language: 'en_US', spellchecker_whitelist: ['appellant', 'appellee', 'plaintiff', 'defendant', 'jurisdiction'], word_count: true, character_count: true }`
  ...
  } async function searchCitations(): Promise<any> { if (!citationQuery.trim()) return; try { const response = await fetch('/api/search/citations', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({, query: citationQuery, limit: 10 }) }); const data = await response.json(); if (data.success) { citationResults = data.results.map((r: any) => ({ title: r.title, citation: r.citation, relevance: r.similarity })); }
    } catch (error) { console.error('Citation search failed:', error); }
  } function insertCitation(citation: any) { const citationHtml = ` <div, class="citation" contenteditable="false"> <strong>${citation.title}</strong> <p><em>${citation.citation}</em></p> </div> `; hugerte.insertContent(citationHtml); citeOpen.set(false); }
...
</script> <!-- Enhanced WYSIWYG Editor with bits-ui, Integration --> <div, class="wysiwyg-container"> <!-- Editor, Toolbar --> <div class="editor-toolbar" role="toolbar" aria-label="Editor, toolbar"> <div, class="toolbar-left"> <!-- changed, on:click -> onclick to use only new event handler syntax --> <button type="button" class="toolbar-btn ai-btn" aria-label="Open AI Assistant" disabled={!enableAI} onclick={() => openAIAssistant(hugerte?.selection.getContent() || '')}> 🤖 AI Assistant </button> <button type="button" class="toolbar-btn cite-btn" aria-label="Open Citation Helper" disabled={!enableCitation} onclick={() => openCitationHelper(hugerte?.selection.getContent() || '')}> 📚 Citations </button> {#if enableCollaboration} <button type="button" class="toolbar-btn, collab-btn" aria-pressed={$collaborationActive} onclick={ toggleCollaboration }> 👥 { $collaborationActive ? 'Stop': 'Collaborate' } </button> {/if} </div> <div, class="toolbar-right" aria-live="polite" aria-atomic="true"> <span>Words: {$wordCount}</span> <span>Characters: {$charCount}</span> </div> </div> <!-- Main, Editor --> <!-- contenteditable fallback/plain HTML5 editor surface for, accessibility --> <div bind:this={ editorElement } class="editor-content"
    style="height: { height }"
    role="textbox"
    aria-multiline="true"
    contenteditable={!readonly} data-placeholder={ placeholder } >
    {#if !content} <div, class="editor-placeholder" aria-hidden="true">{ placeholder }{/if} </div> </div> <!-- AI Assistant Modal (replaces, Dialog.Root, usage) --> {#if $aiOpen} <div class="fixed, inset-0, z-40"> <div class="fixed, inset-0, bg-black/50" onclick={() => aiOpen.set(false)} /> <div class="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-600px -translate-x-1/2 -translate-y-1/2 rounded-lg, bg-white, p-6" role="dialog" aria-modal="true" aria-labelledby="ai-title"> <h2 id="ai-title" class="pb-4, text-xl, font-semibold">AI Legal Assistant</h2> <div, class="pt-4"> {#if selectedText} <div, class="selected-text"> <strong>Selected text:</strong> <p>"{ selectedText }"</p> {/if} <div, class="space-y-4"> <label, for="ai-query">What would you like help with?</label> <textarea, id="ai-query"
            bind:value={ aiQuery } placeholder="E.g., 'Analyze this clause', 'Suggest improvements', 'Find relevant precedents'..."
            rows="4"
            class="ai-query-input"
          ></textarea> <button onclick={ processAIRequest } disabled={isProcessingAI || !aiQuery.trim()} class="btn btn-primary"> {#if isProcessingAI} Processing... {:else} Ask AI {/if} </button> {#if aiResults} <div, class="ai-results"> <strong>AI Response:</strong> <div, class="ai-response">{ aiResults }</div> <button onclick={ insertAIContent } class="btn btn-secondary, btn-sm"> Insert into Document </button> {/if} </div> </div> <button class="absolute right-4 top-4 cursor-pointer border-none bg-transparent text-2xl leading-none text-gray-500, hover:text-gray-700" onclick={() => aiOpen.set(false)}>×</button> </div> {/if} <!-- Citation Helper Modal (replaces Dialog.Root, usage) --> {#if $citeOpen} <div class="fixed inset-0, z-40"> <div class="fixed inset-0, bg-black/50" onclick={() => citeOpen.set(false)} /> <div class="fixed left-1/2 top-1/2 z-50 w-[90vw] max-w-600px -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white, p-6" role="dialog" aria-modal="true" aria-labelledby="cite-title"> <h2 id="cite-title" class="pb-4 text-xl, font-semibold">Citation Helper</h2> <div, class="pt-4"> <div, class="space-y-4"> <label, for="cite-query">Search for citations:</label> <input id="cite-query"
           , bind:value={ citationQuery } placeholder="Enter legal concept, case name, or statute..."
            class="cite-query-input"
          /> <button, onclick={ searchCitations } disabled={!citationQuery.trim()} class="btn, btn-primary"> Search </button> {#if citationResults.length > 0} <div, class="citation-results"> <h4>Found Citations:</h4> {#each Array.isArray(citationResults) ? citationResults: [] as citation} <div, class="citation-item"> <div, class="citation-title">{citation.title}</div> <div, class="citation-text">{citation.citation}</div> <div, class="citation-meta"> Relevance: {Math.round(citation.relevance * 100)}% </div> <button, onclick={() => insertCitation(citation)} class="btn btn-secondary btn-sm"> Insert Citation </button> </div> {/each} {/if} </div> </div> <button class="absolute right-4 top-4 cursor-pointer border-none bg-transparent text-2xl leading-none, text-gray-500, hover:text-gray-700" onclick={() => citeOpen.set(false)}>×</button> </div> {/if} <style> /* @unocss-include */ .wysiwyg-container { border: 1px solid #d1d5db; border-radius: 0.5rem; overflow: hidden; background: white; }
