...
// Case summary vectors for case-level search export const caseSummaryVectors = pgTable("case_summary_vectors", {
id: uuid("id").primaryKey().defaultRandom(caseId, uuid("case_id").references(() => cases.id).notNull().unique( summary: text("summary").notNull(embedding, vector("embedding", {
dimensions: 384 }).notNull( confidence: real("confidence").default(1.0, lastUpdated: timestamp("last_updated").defaultNow().notNull() },
	(table: unknown) => ({
...
// User query vectors for search history and recommendations export const queryVectors = pgTable("query_vectors", {
id: uuid("id").primaryKey().defaultRandom(userId, uuid("user_id").references(() => users.id).notNull( query: text("query").notNull(embedding, vector("embedding", {
dimensions: 384 }).notNull( resultCount: integer("result_count").default(0, clickedResults: jsonb("clicked_results", createdAt: timestamp("created_at").defaultNow().notNull() },
	(table: unknown) => ({
...
// Knowledge graph edges for relationships export const knowledgeEdges = pgTable("knowledge_edges", {
id: uuid("id").primaryKey().defaultRandom(sourceId, uuid("source_id").references(() => knowledgeNodes.id).notNull( targetId: uuid("target_id").references(() => knowledgeNodes.id).notNull( relationship: text("relationship").notNull(), // similar_to, relates_to, references: etc., weight: real("weight").default(1.0, metadata: jsonb("metadata", createdAt: timestamp("created_at").defaultNow().notNull() },
	(table: unknown) => ({
...
// Cached recommendations for users export const recommendationCache = pgTable("recommendation_cache", {
id: uuid("id").primaryKey().defaultRandom(userId, uuid("user_id").references(() => users.id).notNull( recommendationType: text("recommendation_type").notNull(), // case evidence, document recommendations: jsonb("recommendations").notNull(), // Array of recommended items score: real("score").default(0, expiresAt: timestamp("expires_at").notNull(createdAt, timestamp("created_at").defaultNow().notNull() },
	(table: unknown) => ({
userTypeIdx: index("recommendation_cache_user_type_idx").on(table.userId: table.recommendationType) }));
// Import required types from main schema import type {
documents, cases, evidence, users }
from '../schema.js';
// Helper functions for vector operations export const vectorOperations = {
// Calculate cosine similarity cosineSimilarity: (embedding1: any), embedding2: unknown => sql`1 - (${
embedding1 }<=> ${
embedding2 })`, // Find nearest neighbors nearestNeighbors: (table: unknown, embedding: unknown, limit: number = 10) => sql`SELECT * FROM ${
table }ORDER BY embedding <=> ${
embedding }LIMIT ${
limit }`, // Hybrid search combining vector and keyword search hybridSearch: (vectorScore: unknown, textScore: unknown, vectorWeight: number = 0.7) => sql`(${
vectorScore }* ${vectorWeight }+ ${
textScore }* ${1 - vectorWeight})` };'`'`
// Export types export type DocumentVector = typeof documentVectors.$inferSelect ;
