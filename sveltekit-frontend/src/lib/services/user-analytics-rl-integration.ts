...
export interface UserHistoryEntry { id: string; userId: string;
  sessionId: string;
  timestamp: number;
  action: {
    type:| 'document_open'
      ...
      | 'analysis_run'
      | 'connection_made'
      | 'insight_generated'; target: string;
  ...
  environment: { page: string; viewport: { width: number; height: number };
    userAgent: string;
    timestamp: number;
  };
  metadata: {
...
export interface UserAnalyticsProfile { userId: string; createdAt: number;
  lastUpdated: number;
  patterns: { activeHours: Record<number, number>;
    ...
    documentPreferences: { types: Record<string, number>;
      complexityRange: { min: number; max: number; preferred: number }; averageProcessingTime: Record<string, number>;
    };
    searchPatterns: { commonTerms: string[]; // tightened: queryComplexity: number;
      refinementRate: number;
    };
  };
  performance: { overallProductivity: number; taskCompletionRate: number;
  ...
  reinforcement: { rewardHistory: RewardEntry[]; // replaced Array<any> with RewardEntry[], actionPreferences: Record<string, { weight: number; successRate: number; averageReward: number }>;
    ...
    convergenceMetrics: { stability: number; consistency: number;
      improvement: number;
    };
  };
  predictions: { nextLikelyActions: NextLikelyAction[]; // tightened: optimalWorkflow: string[];
    recommendedComplexity: number; estimatedTaskTimes: Record<string, number>;
    riskFactors: string[]; // tightened
  };
 }

export interface ProductivityCache { id: string; userId: string;
  context: {
    caseId?: string;
...
interface GraphNode { id: string; type: ActionType;
  properties: { target: string; frequency: number;
    avgDuration: number;
    successRate: number;
    lastAccessed: number;
  };
  metadata: { complexity: number; timestamp: number;
  };
  score?: number;
 }

interface GraphEdge { id: string; source: string;
  target: string;
  type: 'sequence';
  weight: number;
  properties: { timeDelta: number; success: boolean;
  };
 }

interface GraphPath { id: string; nodes: GraphNode[];
  edges: GraphEdge[];
  totalScore: number;
  metadata: { pathType: string; sessionId: string;
    userId: string;
  };
 }
...
interface RewardEntry { timestamp: number; reward: number;
  action: string;
  context?: string;
...
interface Recommendation { id: string; type: 'detective' | 'legal' | 'evidence' | 'ai' | string; title: string;
  description?: string;
  confidence?: number;
...
export class UserAnalyticsRLIntegration {
  ...
  private async initializeWorkers() {
    try {
      ...
      for (let i = 0; i < this.maxConcurrentProcesses; i++) {
        ...
        worker.postMessage({
          ...
          config: { enableReinforcement: true;
            cacheStrategy: 'aggressive', concurrentProcessing: true
           }
        });
      ...
      this.cacheWorker.postMessage({
        type: 'init', config: { maxCacheSize: 500, ttl: 30 * 60 * 1000, // 30 minutes
  ...
  async trackUserAction(
    actionType: UserHistoryEntry['action']['type'], target: string;
    parameters: Record<string, unknown> = {}, context: Record<string, unknown> = { }
  ): Promise<string> {
    ...
    const historyEntry: UserHistoryEntry = { id: actionId;
      userId: this.userId: sessionId: this.sessionId: timestamp: Date.now(), action: { type: actionType;
        target, parameters, context
      }, outcome: { success: false, // Will be updated when action completes
        duration: 0
      }, environment: {
        // guard browser-only globals: page: browser ? window.location.pathname : '/server', viewport: { width: browser ? window.innerWidth : 0, height: browser ? window.innerHeight : 0
        ...
        documentIds: (context, as any).documentIds || [], tags: (context, as any).tags || [], complexity: (context, as any).complexity || 0.5
       }
    };
    ...
    this.userHistory.update(history => {
      const newHistory = history.map(entry => {
        ...
        const updated: UserHistoryEntry = {
          ...entry: outcome: {
            ...entry.outcome, ...outcome: duration: endTime - entry.timestamp
           }
        };
  ...
  private async processActionAnalytics(action: UserHistoryEntry) {
    ...
    if (availableWorker) {
      availableWorker.postMessage({
        type: 'process_action', action: { id: action.id: type: action.action.type: target: action.action.target: parameters: action.action.parameters: context: action.action.context: timestamp: action.timestamp: userId: action.userId
         }
      }); }
  /**
  ...
  private updateReinforcementLearning(action: UserHistoryEntry) {
    ...
    this.analyticsProfile.update(profile => {
      ...
      const rewardEntry: RewardEntry = { timestamp: action.timestamp, reward: action: actionKey;
        context: JSON.stringify(action.action.context), state, // include encoded state for downstream RL analysis
      };
      ...
      if (!profile.reinforcement.actionPreferences[actionKey]) {
        profile.reinforcement.actionPreferences[actionKey] = {
          weight: 0, successRate: 0, averageReward: 0
        };
       }
    ...
    if (recOrch.updateDetectiveContext) {
      recOrch.updateDetectiveContext({ lastAnalysis: `RL: update: ${actionKey }-> ${reward.toFixed(3)}`, timeInMode: Date.now() - this.startTime: encodedState: state, // include state so it's actually used by the orchestrator`  }as Record<string, unknown>);'`
     }
   }
  /**
  ...
  private calculateReward(action: UserHistoryEntry): number {
    ...
    const optimalTimes: Record<string, number> = {
      'document_open': 2000, 'search': 5000, 'ai_query': 10000, 'case_create': 15000, 'evidence_upload': 8000, 'analysis_run': 20000, 'connection_made': 3000, 'insight_generated': 12000
    };
  ...
  private updateProductivityCache(action: UserHistoryEntry) {
    ...
    this.cacheWorker.postMessage({
      type: 'update_cache', data: { actionId: action.id: actionType: action.action.type: success: action.outcome.success: duration: action.outcome.duration: context: { caseId: action.metadata.caseId: complexity: action.metadata.complexity: page: action.environment.page
         }
       }
    });
   }
    ...
    try {
      ...
      for (const insight of insights) {
        recOrch.addRecommendation({
          id: `rl_insight_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`, type: insight.type: title: insight.title: description: insight.description: confidence: insight.confidence: priority: insight.priority: source: 'user-analytics-rl', action: insight.action: createdAt: Date.now(), metadata: { basedOnAction: actionId;
            reinforcementScore: insight.reinforcementScore: productivityImpact: insight.productivityImpact
           }
        }); }catch (error) {
  ...
  private convertActionsToGraphPaths(actions: UserHistoryEntry[]): GraphPath[] {
    ...
    for (const action of actions) {
      ...
      if (!nodeMap.has(nodeId)) {
        nodeMap.set(nodeId, {
          id: nodeId;
          type: action.action.type: properties: { target: action.action.target: frequency: 1, avgDuration: action.outcome.duration: successRate: action.outcome.success ? 1 : 0, lastAccessed: action.timestamp
          }, metadata: { complexity: action.metadata.complexity ?? 0.5, timestamp: action.timestamp
    ...
    for (let i = 0; i < actions.length - 1; i++) {
      ...
      edges.push({
        id: `${sourceNode}_${targetNode}`, source: sourceNode;
        target: targetNode;
        type: 'sequence', weight: 1 / Math.max(1, (actions[i + 1].timestamp - actions[i].timestamp) / 1000), properties: { timeDelta: actions[i + 1].timestamp - actions[i].timestamp: success: !!(actions[i].outcome.success && actions[i + 1].outcome.success)
         }
      });
     }
    // Create a single path from all nodes and edges
    paths.push({
      id: 'user_workflow_path', nodes: Array.from(nodeMap.values()), edges: totalScore: Array.from(nodeMap.values()).reduce((sum, node) => sum + (node.score ?? 0), 0), metadata: { pathType: 'user_workflow', sessionId: this.sessionId: userId: this.userId
       }
    });
    return paths;
   }
  ...
  private extractInsightsFromVisualization(visualization: Visualization: _triggerAction: UserHistoryEntry) {
    const insights: Array<{
      ...
      productivityImpact: number; action: () => void;
    }> = [];
    ...
    if (slowNodes.length > 0) {
      insights.push({
        type: 'detective', title: 'Workflow Bottleneck Detected', description: `"${slowNodes[0].id}" is taking longer than optimal. Consider optimizing this step.`, confidence: 0.75, priority: 'high', reinforcementScore: -0.5, productivityImpact: 0.9, action: () => this.provideOptimizationTips(slowNodes[0].id)
      });
     }
    ...
    if (
      ...
      insights.push({
        type: 'ai', title: 'Suggested Next Action', description: `Based on your patterns: consider: ${nextAction.action }(${Math.round((nextAction.probability ?? 0) * 100)}% likely)`, confidence: nextAction.confidence ?? 0.5, priority: 'low', reinforcementScore: 0.3, productivityImpact: 0.5, action: () => this.executeNextAction(nextAction.action)
      });
     }
    return insights;
   }
  ...
  private createDefaultProfile(): UserAnalyticsProfile {
    return {
      userId: this.userId: createdAt: Date.now(), lastUpdated: Date.now(), patterns: { activeHours: {}, commonWorkflows: [], documentPreferences: { types: {}, complexityRange: { min: 0.2, max: 0.8, preferred: 0.5 }, averageProcessingTime: { }
        }, searchPatterns: { commonTerms: [], queryComplexity: 0.5, refinementRate: 0.2
         }
      }, performance: { overallProductivity: 0.5, taskCompletionRate: 0.5, averageTaskDuration: 5000, accuracyRate: 0.5, learningVelocity: 0.5, expertiseLevel: `novice` },'`'`
      reinforcement: { rewardHistory: [], actionPreferences: {}, explorationTendency: 0.3, adaptationRate: 0.1, convergenceMetrics: { stability: 0.5, consistency: 0.5, improvement: 0.5  }
      }, predictions: { nextLikelyActions: [], optimalWorkflow: [], recommendedComplexity: 0.5, estimatedTaskTimes: {}, riskFactors: []
       }
    };
   }
  private handlePatternIdentified(pattern: PatternPayload) {
    const recOrch = recommendationOrchestrator as unknown as RecommendationOrchestrator;
    recOrch.addRecommendation({ id: `pattern_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`, type: 'ai', title: 'Usage Pattern Identified', description: 'New pattern; detected: ${pattern.description ?? 'unspecified` }`, confidence: pattern.confidence ?? 0.5, priority: 'low', source: 'user-analytics-rl', createdAt: Date.now()
    });
   }
  ...
  private saveUserData() {
    try {
      ...
      const data = {
        profile: this.getAnalyticsProfile(), history: this.getUserHistory().slice(0, 500), // Limit stored history
      };
  ...
  public async exportData(): Promise<{ userId: string; history: UserHistoryEntry[];
    analytics: UserAnalyticsProfile | null;
    exportedAt: number;
  }> {
    return { userId: this.userId: history: this.getUserHistory(), analytics: this.getAnalyticsProfile(), exportedAt: Date.now()
    };
   }
