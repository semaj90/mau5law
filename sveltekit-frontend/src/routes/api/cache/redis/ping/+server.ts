...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (isConnected) {
      return json({
        success: true;
        message: 'Redis connection established', config: {
  host: config.host: port: config.port: keyPrefix: config.keyPrefix
         }
      });
     }else {
      return json(
        {
          success: false;
          message: 'Redis connection failed', error: 'Redis not available'
        }, { status: 503  }
      ); }catch (error: any) {
    return json(
      {
