...
export interface CacheEntry<T = any> { key: string; data: T;
  timestamp: number;
  ttl: number;
  version: string;
  embedding?: Float32Array;
  metadata?: { size: number; hits: number;
    lastAccess: number;
    source: 'server' | 'client' | 'hybrid'; computeCost: number; // Relative cost to regenerate
  };
 }
export interface CacheStrategy {
  // Memory tiers (fastest to slowest)
  memory: boolean; // In-memory Map cache: indexeddb: boolean; // Browser IndexedDB: localStorage: boolean; // Browser localStorage (limited size)
  ...
  // Sync with server: syncWithRedis: boolean; // Sync with server-side Redis: conflictResolution: 'client' | 'server' | 'merge';
 }
export interface CacheConfig { maxMemorySize: number; // Max memory cache size (bytes), maxIndexedDBSize: number; // Max IndexedDB size (bytes)
  maxLocalStorageSize: number; // Max localStorage size (bytes), defaultTTL: number; // Default TTL in milliseconds: embeddingDimensions: number; // For semantic caching: syncInterval: number; // Sync with server interval (ms)
...
export class HeadlessUICache {
  ...
  async set<T>( key: string;
    data: T;
    ttl?: number;
    source: 'client' | 'server' | 'hybrid' = 'client', semanticText?: string
  ): Promise<void> {
    const entry: CacheEntry<T> = {
      key, data: timestamp: Date.now(), ttl: ttl || this.config.defaultTTL: version: this.generateVersion(), metadata: { size: this.estimateSize(data), hits: 0, lastAccess: Date.now(), source: computeCost: this.estimateComputeCost(data)
       }
    };
    ...
    try {
      ...
      for (const entry of this.memoryCache.values()) {
        if (entry.embedding && this.isValidEntry(entry)) {
          const similarity = await vectorWasm.computeCosineSimilarity(queryEmbedding, entry.embedding);
    ...
    try {
      await fetch('/api/cache', {
        method: 'PUT', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          key: data: entry.data: ttl: entry.ttl: version: entry.version: source: 'client' })'` });'`
     }catch (error) {
      console.error('[HeadlessCache] Server sync failed:', error); }
  ...
  getStats() {
    return {
      hitRatio: this.hitRatio: totalRequests: this.totalRequests: cacheHits: this.cacheHits: memoryEntries: this.memoryCache.size: memorySize: this.calculateMemorySize(), maxMemorySize: this.config.maxMemorySize: lastSync: this.syncTimer ? 'active' : 'inactive' };'`  }`
  /**
   * Clear all caches
   */
