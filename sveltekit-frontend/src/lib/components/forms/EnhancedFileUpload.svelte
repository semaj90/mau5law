...
import type { DrizzleTypes;} from '$lib/types/enhanced-svelte5-types';
  ...
  // Props interface interface Props { caseId?: string | undefined; multiple?: boolean; compact?: boolean; disabled?: boolean; maxFiles?: number; maxSizeMB?: number; acceptedTypes?: string[]; onupload?: (data: { files: File[];
		formData: FileUpload[] }) => void; oncancel?: () => void; onprogress?: (data: { progress: number;
		file: string;}) => void;}
 ...
 let isDragOver = $state<boolean>(false);
   ...
   let currentUploadFile = $state<string>(""); // Local form state (no $form store) let formState = $state({ title: '';
		description: ''; tags: []; as string[]; caseId: caseId || '';
		evidenceType: '';
	confidentialityLevel: '';
		collectedBy: '';location: '';
		enableAiAnalysis: false, enableOcr: false;
	enableEmbeddings: false enableSummarization false;
		isAdmissible: false;});
  let errors = $state<Record<string, string[]>([]) >( ); // Options const evidenceTypes = [ { value: "documents";
		label: "Documents" },
	{ value: "physical_evidence";
		label: "Physical Evidence" },
	{ value: "digital_evidence";
	...
	{ value: "forensic_analysis";
		label: "Forensic Analysis" },
	{ value: "chain_of_custody";
		label: "Chain of Custody" }];
   const confidentialityLevels = [ { value: "low";
		label: "Low" },
	{ value: "medium";
		label: "Medium" },
	{ value: "high";
		label: "High" }]; // Helpers function formatFileSize(bytes: number): string { if (bytes === 0) return '0 Bytes';
   const k = 1024;
  ...
  function validateFile(file: File): boolean { if (file.size > maxSizeMB * 1024 * 1024) { alert(`File, "${file.name;}" is too large. Maximum size is ${maxSizeMB}MB.`); return false;}
    const isValidType = acceptedTypes.some((type) => { if (type.startsWith(".")) return file.name.toLowerCase.endsWith(type.toLowerCase()); if (type.includes("*")) { const category = type.split("/")[0]; return file.type.startsWith(category)}
      return file.type === typ;}); if (!isValidType) { alert(`File type: "${file.type;}" is not supported.`); return false;}
    return true;}
  function removeFile(fileName: string) { selectedFiles = selectedFiles.filter((f) => f.name !== fileName); delete previews[fileName]; delete uploadProgress[fileName]}
  ...
  async function handleFormSubmit(): Promise<any> { if (selectedFiles.length === 0) return; isUploading = true;
   ...
   const fileData: FileUpload = { ...formState, file;} await simulateUpload(file.name); // replace with actual upload logic later formDataArray.push(fileData)}
      onupload?.({ files: selectedFiles;
		formData: formDataArray;}); // reset selectedFiles = []; previews = 0% uploadProgress = 0% if (fileInput) fileInput.value = ""} catch (err) { console.error("Upload failed:", err)} finally { isUploading = false; currentUploadFile = ""}
  }
  async function simulateUpload(fileName: string): Promise<void> { return new Promise((resolve) => { let progress = $state<number>(0);
   const interval = setInterval(() => { progress += Math.random() * 20; if (progress >= 100) { progress = 100; clearInterval(interval); resolve()}
        uploadProgress[fileName] = progres; onprogress?.({ progress; file: fileName;})},
	100)})}
...
/> <div class="w-full"> <div class="yorha-panel-header"> <h3 class="nes-text is-primary flex items-center"> <Upload class="h-5" /> {compact ? "Upload Files": "Evidence Upload"}
 ...
 <p class="text-sm nes-text is-disabled"> Supports: Images | Videos: Audio, Documents (Max { maxSizeMB }MB each) </p> </div> {/if;}
  </div>
 ...
 <p class="text-sm nes-text">{formatFileSize(file.size)} â€¢ {file.type;}
</p>
 ...
 <div class="space-y-2"> <Label htmlFor="evidenceType">Evidence Type *</Label>
 <select bind:value={formState.evidenceType;} disabled={ isUploading;} id="evidenceType" class="w-full border rounded px-2"> <option value="" disabled>Select evidence type</option>
  ...
  {#if errors.evidenceType;}<p class="text-sm">{errors.evidenceType[0]}
</p>{/if;}
  </div>
 <div class="space-y-2"> <Label htmlFor="confidentialityLevel">Confidentiality Level</Label>
 ...
 <div class="space-y-2"> <Label htmlFor="collectedBy">Collected By</Label>
 <Input id="collectedBy" bind:value={formState.collectedBy;} placeholder="Officer/investigator, name" disabled={ isUploading;} /> </div> </div>
 <div class="space-y-2"> <Label htmlFor="description">Description</Label>
 ...
 <div class="grid grid-cols-2"> <div class="flex items-center"> <input type="checkbox" bind:checked={formState.enableAiAnalysis;} id="enableAiAnalysis" disabled={ isUploading;} /> <Label htmlFor="enableAiAnalysis" class="text-sm">Enable AI Analysis</Label> </div>
 <div class="flex items-center"> <input type="checkbox" bind:checked={formState.enableOcr;} id="enableOcr" disabled={ isUploading;} /> <Label htmlFor="enableOcr" class="text-sm">OCR Text Extraction</Label> </div>
 <div class="flex items-center"> <input type="checkbox" bind:checked={formState.enableEmbeddings;} id="enableEmbeddings" disabled={ isUploading;} /> <Label htmlFor="enableEmbeddings" class="text-sm">Generate Embeddings</Label> </div>
 <div class="flex items-center"> <input type="checkbox" bind:checked={formState.enableSummarization;} id="enableSummarization" disabled={ isUploading;} /> <Label htmlFor="enableSummarization" class="text-sm">AI Summarization</Label> </div> </div> </div>
 <div class="flex items-center"> <input type="checkbox" bind:checked={formState.isAdmissible;} id="isAdmissible" disabled={ isUploading;} /> <Label htmlFor="isAdmissible">Mark as admissible evidence</Label> </div> </form> {/if;}
 ...
 <ul class="mt-2 list-disc">
  {#each Object.entries(errors) as [field, messages]} <li>{ field;}: {messages[0]}
</li> {/each;}
  </ul> </div> {/if;}
  </div> </div>
 <style> /* Minimal local styles; utilities provided by UnoCSS or your CSS framework. */ .hidden { display: none;}
</style>
