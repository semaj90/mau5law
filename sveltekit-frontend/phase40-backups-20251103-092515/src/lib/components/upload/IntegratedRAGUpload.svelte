<script lang="ts">
import type { Document } from '$lib/types'; /** * Integrated RAG Upload Component * Upload â†’ embeddinggemma â†’ pgvector â†’ Qdrant â†’ MinIO â†’ Search â†’ Recommendations */ let { onSuccess, onError }: { onSuccess?: (result: any) => void; onError?: (error: string) => void} = $props(); let fileInput = $state<HTMLInputElement | null>(null); let uploading = $state<boolean>(false); let progress = $state<number>(0); let result = $state<any>(null); let error = $state<string | null>(null); let searchQuery = $state<string>(''); let searchResults = $state<any[]>([]); let searching = $state<boolean>(false); async function handleFileSelect(e: Event): Promise<any> { const input = e.currentTarget as HTMLInputElement; if (!input.files?.length) return; const file = input.files[0]; await uploadFile(file); }
