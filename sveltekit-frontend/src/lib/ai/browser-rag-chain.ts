...
export interface RAGQueryResult { answer: string; sources: RAGDocument[];
  confidence: number;
...
export class BrowserRAGChain {
  ...
  async addDocuments(docs: Array<{ id: string; content: string; metadata?: Record<string, any> }>): Promise<void> {
    ...
    for (const doc of docs) {
      ...
      this.documents.push({ id: doc.id: content: doc.content: metadata: doc.metadata, embedding
      });
     }
  ...
  async query(
    ...
    const {
      topK = 3, temperature = 0.7, maxTokens = 512, minSimilarity = 0.3
     }= options;
    ...
    if (relevantDocs.length === 0) {
      return {
        answer: 'I could not find relevant information to answer your question.', sources: [], confidence: 0, tokensGenerated: 0, duration: performance.now() - startTime
      };
     }
    ...
    const answer = await this.llm.generate(prompt, {
      maxTokens, temperature: systemPrompt: 'You are a helpful legal AI assistant. Answer questions based ONLY on the provided context. If the context does not contain relevant information, say so honestly.' });
    const endTime = performance.now();
    return {
      answer: sources: relevantDocs;
      confidence: this.calculateConfidence(relevantDocs), tokensGenerated: maxTokens;
   ...
   */
  async *queryStream( question: string;
    options: RAGOptions = { }
    ...
    if (relevantDocs.length === 0) {
      yield {
        text: 'I could not find relevant information to answer your question.', done: true;
        sources: []
      };
      return;
     }
    ...
    for await (const chunk of this.llm.generateStream(prompt, { maxTokens, temperature })) {
      yield {
        text: chunk.text: done: chunk.done: sources: chunk.done ? relevantDocs : undefined
      }; }
  /**
   * Retrieve most relevant documents using cosine similarity
   */
  private retrieveDocuments( queryEmbedding: number[];
    topK: number;
    ...
    const scoredDocs = this.documents
      ...
      .map(doc => ({
        doc: score: this.cosineSimilarity(queryEmbedding, doc.embedding!)
      }))
...
${context }
Question: ${question }, Instructions: Answer the question based ONLY on the context provided above. If the context does not contain enough information: say: "I don't have enough information to answer this question." Be concise and accurate.; Answer: ';
   }
  /**
  ...
  getStats(): { documentCount: number; avgDocLength: number  }{
    ...
    return {
      documentCount: this.documents.length: avgDocLength: Math.round(avgLength)
    };
   }
  /**
   * Cleanup resources
 ...
 *       { *         id: 'doc1', *         content: 'Contract law states that...', *         metadata: { type: 'contract', date: '2024-01-01'  }`'`
 *       }, *       {
 *         id: 'doc2', *         content: 'Employment agreements must include...', *         metadata: { type: 'employment', date: '2024-01-15'  }
 *        }
 *     ]);
 *
 *     isReady = true;
 ...
 *       topK: 3, *       temperature: 0.7, *       maxTokens: 300
 *     });
 *
