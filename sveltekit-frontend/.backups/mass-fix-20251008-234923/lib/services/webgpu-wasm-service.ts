...
export class WebGPUWASMService {
  ...
  private readonly GEMMA3_LEGAL_CONFIG: ModelConfig = {
    ...
    maxTokens: 8192,
    dimensions: 384,
    // Default LLM parameters for Gemma3 Legal
      ...
      if (navGPU) {
        try {
          ...
          if (this.adapter) {
            ...
            console.log('✅ WebGPU detection result:', { webgpu: capabilities.webgpuSupported, adapterInfo: capabilities.adapterInfo });
          }
        } catch (webgpuError) {
  ...
  private async initializeWASM(modelBytes: ArrayBuffer, tokenizerData: Record<string, unknown>): Promise<void> {
    try {
      ...
      const instanceExports = instSource && instSource.instance ? instSource.instance.exports : (instantiateResult as unknown as WebAssembly.Exports);
   ...
   */
  async generateEmbedding(text: string): Promise<{ embedding: number[]; dimensions: number; processingTimeMs: number; device: string }> {
    if (!this.currentModel || !this.wasmModule) {
      throw new Error('Model not loaded');
    }
    const startTime = Date.now();
