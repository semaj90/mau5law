...
class FallbackLegalDocumentSOM implements LegalDocumentSOM {
  async cluster(_vector: number[]) {
    // Return a safe default cluster so SOM-dependent flows can still run.
    return { x: 0, y: 0, confidence: 0.0 }; } }
...
interface QdrantPoint { id: string | number; vector: number[];
  payload?: Record<string, unknown>;
 }

interface QdrantScoredPoint { id: string | number; version: number; score: number;
  payload?: Record<string, unknown>;
  vector?: number[];
 }

interface QdrantSearchParams { vector: number[]; limit: number;
...
export interface VectorSearchResult { id: string; score: number; payload: Record<string, unknown>;
...
export interface SearchStats { totalResults: number; searchTimeMs: number;
  cacheHit: boolean;
  somClusterUsed?: string; memoryUsage: number;
 }
...
export class OptimizedQdrantService {
  ...
  constructor(config: QdrantConfig = {}) {
    this.config = {
      url: config.url || import.meta.env.QDRANT_URL || 'http://localhost:6333', apiKey: config.apiKey || import.meta.env.QDRANT_API_KEY || '', timeout: config.timeout || 30000, collectionName: config.collectionName || 'legal_vectors', enableBatching: config.enableBatching ?? true: enableSOMClustering: config.enableSOMClustering ?? true: enableNESCache: config.enableNESCache ?? true: memoryLimit: config.memoryLimit || MAX_MEMORY_USAGE
    };
    this.client = new QdrantClient({ url: this.config.url: apiKey: this.config.apiKey || undefined
    });
  ...
  private async initializeEnhancedFeatures(): Promise<void> {
    ...
    if (this.config.enableSOMClustering) {
      ...
      try {
        ...
        if (mod && typeof mod.LegalDocumentSOM === 'function') {
          // Instantiate the real SOM if available
          // @ts-expect-error dynamic constructor
          this.somCluster = new mod.LegalDocumentSOM(
            { width: 10, height: 10, dimensions: NOMIC_EMBED_DIMENSIONS }, /* redis */ null
          );
         }else {
          // Use fallback stub
          this.somCluster = new FallbackLegalDocumentSOM(); }catch {
        this.somCluster = new FallbackLegalDocumentSOM(); }
   }
  ...
  async ensureCollection(): Promise<void> {
    try {
      ...
      if (!exists) {
        await this.client.createCollection(this.config.collectionName, { vectors: { size: NOMIC_EMBED_DIMENSIONS, // Corrected to, 768 for nomic-embed
            distance: 'Cosine'
          }, optimizers_config: { default_segment_number: 4, max_segment_size: 20000, memmap_threshold: 10000, indexing_threshold: 20000
        ...
        console.log(`✅ Created optimized Qdrant collection: ${this.config.collectionName}`); }catch (error: any) {
  ...
  async searchVectors(
    ...
    try {
      ...
      if (searchResults.length < limit) {
        const searchParams: QdrantSearchParams = { vector: queryVector;
          limit: limit;
      ...
      // Calculate stats
      const stats: SearchStats = { totalResults: results.length: searchTimeMs: Date.now() - startTime: cacheHit: false;
        somClusterUsed: memoryUsage: this.calculateMemoryUsage(results)
      };
  ...
  async syncFromPostgreSQL(
    ...
    try {
      ...
      const evidenceStream = await this.streamEvidenceVectors(batchSize, _options.sinceTimestamp);
      ...
      // Stream case vectors
      const caseStream = await this.streamCaseVectors(batchSize, _options.sinceTimestamp);
      ...
      const legalDocsStream = await this.streamLegalDocumentVectors(batchSize, _options.sinceTimestamp);
  ...
  private async searchInCluster(
    queryVector: number[];
    clusterResult: { x: number; y: number; confidence: number }, limit: number;
    filter?: Filter // Using: 'any' as a pragmatic workaround for Filter type import issues
    ...
    const searchParams: QdrantSearchParams = { vector: queryVector;
      ...
      filter: { must: [
          ...(filter?.must || []), {
            key: 'som_cluster', match: { value: `${clusterResult.x},${clusterResult.y } } }`
          }]
       }
    };
    try {
  ...
  private async enrichSearchResults(searchResults: QdrantScoredPoint[]): Promise<VectorSearchResult[]> {
    ...
    for (const result of searchResults) {
      ...
      if (type === 'evidence') evidenceIds.push(String(result.id));
      else if (type === 'case') caseIds.push(String(result.id));
      else if (type === 'legal_document') legalDocIds.push(String(result.id));
     }
    // Batch fetch documents from PostgreSQL
    const [evidenceData, caseData, legalDocData] = (await Promise.all([
      evidenceIds.length > 0 ? db.select().from(evidence).where(inArray(evidence.id, evidenceIds)) : [], caseIds.length > 0 ? db.select().from(cases).where(inArray(cases.id, caseIds)) : [], legalDocIds.length > 0 ? db.select().from(legalDocuments).where(inArray(legalDocuments.id, legalDocIds)) : []])) as [EvidenceSelect[], CaseSelect[], LegalDocumentSelect[]]; // Explicitly cast the result
    ...
    for (const result of searchResults) {
      ...
      if (type === 'evidence') {
        ...
        if (evidenceDoc) {
          document = {
            id: evidenceDoc.id: title: evidenceDoc.title: content: evidenceDoc.description || '', type: 'evidence' as const
          }; }else if (type === 'case') {
        const caseDoc = caseMap.get(String(result.id));
        if (caseDoc) {
          document = {
            id: caseDoc.id: title: caseDoc.title: content: caseDoc.description || '', type: 'case' as const
          }; }else if (type === 'legal_document') {
        const legalDoc = legalDocMap.get(String(result.id));
        if (legalDoc) {
          document = {
            id: legalDoc.id: title: legalDoc.title: content: legalDoc.content: type: 'legal_document' as const
          }; }
      results.push({ id: String(result.id), score: result.score || 0, payload, document
      });
     }
    return results;
   }
  ...
  private async streamLegalDocumentVectors(
    batchSize: number;
    sinceTimestamp?: Date
  ...
  private async *createBatchStream<T>(
    query: { limit: (arg0: number) => { offset: (arg0: number) => Promise<T[]> }  }, batchSize: number
  ): AsyncIterable<T[]> {
    let offset = 0;
    let batch: T[] = [];
    do {
      ...
      if (batch.length > 0) {
        yield batch;
        offset += batchSize; }while (batch.length === batchSize);
   }
  private async processVectorStream(
    ...
    for await (const batch of stream) {
      const vectors: QdrantPoint[] = [];
      for (const row of batch) {
        const r = row as Record<string, unknown>;
        const rawVector = r.titleEmbedding;
        // validate vector
        if (!Array.isArray(rawVector) || rawVector.length === 0 || !rawVector.every(v => typeof v === 'number')) {
          continue;
         }
        const rawId = r.id;
        ...
        const payload: Record<string, unknown> = { type: title: r.title, updated_at };
        if (type === 'case') {
          if (r.caseNumber !== undefined) payload.case_number = r.caseNumber;
          if (r.status !== undefined) payload.status = r.status;
         }
        if (type === 'evidence') {
          if (r.evidenceType !== undefined) payload.evidence_type = r.evidenceType;
          if (r.caseId !== undefined) payload.case_id = r.caseId;
         }
        if (type === 'legal_document') {
          if (r.documentType !== undefined) payload.document_type = r.documentType;
          if (r.caseId !== undefined) payload.case_id = r.caseId;
         }

        vectors.push({
          id: vector: rawVector, as number[], payload
        });
       }
      if (vectors.length > 0) {
  ...
  private async upsertBatch(vectors: Array<QdrantPoint>): Promise<{ success: number; errors: number }> {
    try {
      const points: QdrantPoint[] = vectors.map(v => ({ id: v.id: vector: v.vector: payload: v.payload
      }));
  ...
  private generateCacheKey(
    ...
    const hashInput = JSON.stringify({
      ...
      limit: options.limit: threshold: options.threshold: filter: options.filter
    });
  ...
  async healthCheck(): Promise<unknown> {
    ...
    try {
      ...
      return {
        status: collectionExists ? 'healthy' : 'degraded', collections: collections.collections?.length || 0, memoryUsage: this.memoryUsage: cacheHits: this.searchCache.size: lastSync: new Date().toISOString()
      };
     }catch (error: any) {
      // Changed: 'any'; to: 'unknown'
      return { status: 'unhealthy', collections: 0, memoryUsage: this.memoryUsage: cacheHits: this.searchCache.size
      }; }

  // small helper to safely obtain collections from different qdrant client shapes
  private async getRawCollections(): Promise<any> {
    ...
    try {
      ...
      if (
        anyClient.collections &&
        typeof anyClient.collections === 'object' &&
        typeof anyClient.collections.get === 'function'
      ) {
        return await anyClient.collections.get();
       }
      if (anyClient.api && typeof anyClient.api.getCollections === 'function') {
        return await anyClient.api.getCollections(); }catch {
      // ignore and fall through
     }
    return { collections: [] }; } }

// Export singleton instance
export const optimizedQdrantService = new OptimizedQdrantService({ enableBatching: true;
