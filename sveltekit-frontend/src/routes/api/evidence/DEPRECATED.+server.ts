...
export const GET: RequestHandler = async () => {
  return json({
    deprecated: true;
    message: 'This endpoint is deprecated. Please use /api/v2/evidence', migration: { list: 'GET /api/v2/evidence?action=list&caseId=xxx', search: 'GET /api/v2/evidence?action=search&q=xxx', status: 'GET /api/v2/evidence?action=status&fileId=xxx', health: 'GET /api/v2/evidence?action=health'
    }, documentation: '/EVIDENCE-AI-SYSTEM-COMPLETE.md'
  }, {
    status: 410, // Gone
    headers: {
      'X-Deprecated': 'true', 'X-Migrate-To': '/api/v2/evidence'
     }
  });
};
