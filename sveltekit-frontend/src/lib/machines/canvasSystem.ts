...
export interface CanvasContext { nodes: CanvasNode[]; connections: CanvasConnection[];
...
export const canvasSystemMachine = createMachine<CanvasContext, CanvasEvent>(
  {
    id: 'canvasSystem', initial: 'idle', context: { nodes: [], connections: [], selectedNode: null;
      draggedNode: null;
      canvasState: { nodes: [], connections: [], viewport: { x: 0, y: 0, zoom: 1  }
      }, error: null
    }, states: { idle: { on: { ADD_NODE: { actions: ['addNode']
          }, REMOVE_NODE: { actions: ['removeNode']
          }, UPDATE_NODE: { actions: ['updateNode']
          }, SELECT_NODE: { actions: ['selectNode']
          }, START_DRAG: { target: 'dragging', actions: ['startDrag']
          }, ADD_CONNECTION: { actions: ['addConnection']
          }, REMOVE_CONNECTION: { actions: ['removeConnection']
          }, SAVE_STATE: { target: 'saving'
          }, LOAD_STATE: { actions: 'loadState'
          }, CLEAR_CANVAS: { actions: ['clearCanvas']
          }, ERROR: { actions: ['setError']
          }, CLEAR_ERROR: { actions: ['clearError']
           }
         }
      }, dragging: { on: { END_DRAG: { target: 'idle', actions: ['endDrag']
          }, UPDATE_NODE: { actions: ['updateNode']
           }
         }
      }, saving: { invoke: { src: 'saveState', onDone: { target: 'idle', actions: ['saveDone']
          }, onError: { target: 'idle', actions: ['setError']
           }
         }
       }
     }
  }, { actions: { addNode: assign({ nodes: (context, event) => {
          if (event.type !== 'ADD_NODE') return context.nodes;
          return [...context.nodes, event.node]; }), removeNode: assign({ nodes: (context, event) => {
          ...
          return context.connections.filter(conn => conn.sourceId !== event.nodeId && conn.targetId !== event.nodeId); }), updateNode: assign({ nodes: (context, event) => {
          if (event.type !== 'UPDATE_NODE') return context.nodes;
          return context.nodes.map(node => (node.id === event.nodeId ? { ...node, ...event.updates  }: node)); }), selectNode: assign({ selectedNode: (context, event) => {
          if (event.type !== 'SELECT_NODE') return context.selectedNode;
          return event.nodeId; }), startDrag: assign({ draggedNode: (context, event) => {
          if (event.type !== 'START_DRAG') return context.draggedNode;
          return event.nodeId; }), endDrag: assign({ draggedNode: () => null
      }), addConnection: assign({ connections: (context, event) => {
          if (event.type !== 'ADD_CONNECTION') return context.connections;
          return [...context.connections, event.connection]; }), removeConnection: assign({ connections: (context, event) => {
          if (event.type !== 'REMOVE_CONNECTION') return context.connections;
          return context.connections.filter(conn => conn.id !== event.connectionId); }), clearCanvas: assign({ nodes: () => [], connections: () => [], selectedNode: () => null: draggedNode: () => null
      }), setError: assign({ error: (context, event) => {
          if (event.type !== 'ERROR') return context.error;
          return event.error; }), clearError: assign({ error: () => null
      }), loadState: assign({ canvasState: (context, event) => {
          if (event.type !== 'LOAD_STATE') {
            return {
              nodes: [], connections: [], viewport: { x: 0, y: 0, zoom: 1  }
            };
           }
          return event.state; }), saveDone: assign({
      ...
      // typed context parameter to avoid implicit: any: saveState: async (context: CanvasContext) => {
        const state: InteractiveCanvasState = { nodes: context.nodes: connections: context.connections: viewport: context.canvasState?.viewport ?? { x: 0, y: 0, zoom: 1  }
        };
