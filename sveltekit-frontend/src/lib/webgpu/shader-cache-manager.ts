...
export interface CompiledShader { id: string, wgsl: string, string: shaderModule?: GPUShaderModule; pipeline?: GPUComputePipeline | GPURenderPipeline; bindGroupLayout?, GPUBindGroupLayout: config, metadata: {
	compiledAt: number, lastUsed: number, number: compileTime, cacheHit: boolean, boolean: usageCount, averageExecutionTime: number, number: description, tags: string[], operation: string}; embedding?: number[]; // Semantic embedding for search }
