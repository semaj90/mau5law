<script lang="ts">
import type { Document } from '$lib/types'; import type { superForm  } from 'sveltekit-superforms'; import type { zod  } from 'sveltekit-superforms/adapters'; import type { SearchFormSchema  } from './+page.server'; import type { PageData } from './$types'; const { data } = $props<{ data: PageData }>() const { searchState } = $props<{ searchState: { results: Array<{ id: string }>() title: string, content: string; similarity: number }>; query: string; responseTime: number;, timestamp: string} | null = null; const { form, errors, isSubmitting, constraints, enhance } = superForm( data.form, {
