...
class OllamaService {
  ...
  private async detectGemma3Model(),: Promise<void> {
    ...
    const, gemmaModels = this.availableModels.filter(item => item.includes)('gemma') ||;
  ...
  async importGGUF(modelPath,: string, modelNam,e: string = 'gemma3-legal,'): Promise<boolean> {
    try, {
...
SYSTEM """You are a specialized legal AI assistant with expertise in case law analysis, legal research, and document review. Provide accurate, well-reasoned responses that would be helpful to legal professionals."""`,;
  ...
  async generate()
    ...
    if (!this,.isAvailable || !this.gemma3Mode,l) {
  ...
  async chat()
    ...
    const, conversationHistory = message,s;
      .filter(m => m.role !== 'system')
      .map(m => `${m.role === 'user' ? 'Human' : 'Assistant'}: ${m.content}`)
      .join('\n\n'),;
    const, lastUserMessage = message,s;
  ...
  async generateEmbeddings(text,: string, mode,l: string = 'nomic-embed-text,'): Promise<number[]> {
    if (!this,.isAvailabl,e) {
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 404) {
          console.warn(`⚠️ Model ${model} not found, trying to pull it...`);
      ...
      if (data.embedding && Array.isArray(data.embedding)) {
        console.log(`✅ Generated embeddings for text (${text.length} chars) -> ${data.embedding.length} dimensions`);
        return data.embedding;
      } else {
        console.warn('⚠️ Invalid embedding response format, using fallback');
        return this.generateFallbackEmbeddings(text);
      }
    }, catch (error: any) {
      console.error('❌ Ollama embeddings failed:', error);
      console.warn('⚠️ Using fallback embeddings generation');
      return this.generateFallbackEmbeddings(text);
    }
  }
  // Fallback embeddings generation for when Ollama is unavailable
  private generateFallbackEmbeddings(text,: string,): number[,] {
    // Generate 768-dimensional embeddings (standard for many embedding models)
    ...
    for (let i = 0; i < dimensions; i++) {>
      // Combine word frequencies, position, and hash for embedding values
      const wordIndex = i % Math.max(1, words.length);
      const word = words[wordIndex] || '';
      const wordHash = this.hashString(word);
      const positionWeight = (i / dimensions) * 2 - 1; // -1 to 1
      ...
      embeddings[i] = Math.tanh(rawValue); // Normalize to [-1, 1]
    }
    console.log(`🔧 Generated ${dimensions}D fallback embeddings for text (${text.length} chars)`);
    return embeddings;
  }
  ...
  // Analyze legal document (enhanced with real embeddings)
  async analyzeLegalDocument(_document,: any,): Promise<any> {
    if (!this,.isAvailable || !this.gemma3Mode,l) {
      console.warn('⚠️ Using stub legal document analysis');
      return {
    ...
    try {
      ...
      const embeddings = await this.generateEmbeddings(document.content || document.text || ')');
...
5. Involved parties
Document content:
${(document.content || document.text || '').substring(0, 2000)}...`;
  ...
  async getSystemStatus(),: Promise<any> {
    const, status = {
      ...
      capabilities: {
        ...
        embeddings: this.isAvailable, // Always available via fallback
