<script lang="ts">
  ...
  function connect() {
    try {
      ...
      eventSource.addEventListener('error-data', (event: MessageEvent) => {
        const data = JSON.parse(event.data);
        if (data.code) {
...
<div class="error-stream-container">
  ...
  <div class="error-list">
    ...
    {:else}
      {#each Array.from(errors.values()).sort((a, b) => b.severity - a.severity) as err (err.code)}
        <div class="error-card {getPriorityColor(err.priority)} border-l-4">
          <div class="error-header">
            <div class="error-code">{err.code}</div>
            <div class="error-priority">{err.priority}</div>
            <div class="error-count">{err.count} occurrences</div>
          </div>

          <div class="severity-bar-container">
            <div class="severity-bar">
              <div class="bar-fill" style="width: {getSeverityBar(err.severity)}%"></div>
            </div>
            <span class="severity-label">{err.severity}/100</span>
          </div>

          <div class="affected-files">
            <span class="file-count">🗂️ {err.affectedFiles} files affected</span>
            {#if err.samples.length > 0}
              <div class="samples">
                {#each err.samples as file}
