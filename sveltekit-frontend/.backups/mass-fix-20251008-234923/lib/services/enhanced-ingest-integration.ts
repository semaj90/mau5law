...
export class EnhancedIngestService {
    ...
    try {
      ...
      try {
        ...
        if (Array.isArray(similarDocs) && similarDocs.length > 0) {
          embeddingPreview = (similarDocs[0] as any).embedding?.slice(0, 5) || [];
        }
      } catch (embedError) {
      ...
      aiAgentStore.update(state => ({
        ...
        errors: [
          ...state.errors,),;
          {
            id: `ingest_${Date.now()}`,
            type,: 'processing',
            message,: `Document ingest failed: ${(error && error.message) ? error.message: String(error)}`,
            timestamp,: new Date(),
            context,: { request },
            resolved: false;
            retryable: true
   ...
   */;
  async intelligentChunking(content,: string, option,s: ChunkingOptions = {,}): Promise<ChunkedDocument[]> {
    const, {
      maxChunkSize = 600,
      overlap = 60,
    ...
    const, chunk,s: ChunkedDocume,nt,[], = [];
    if (legalAware) {
  ...
  private async callIngestAPI(endpoint,: string, dat,a: an,y): Promise<any> {
  ...
  private splitLegalSection(section,: LegalSection, maxSiz,e: number, overl,ap: numb,er): strin,g[] {
  ...
  private standardChunking(content,: string, maxSiz,e: number, overl,ap: number, preserveSenten,ces: bool,ean): ChunkedDocume,nt[] {
