...
import type { Message } from '$lib/types'; // Svelte, 5 runes are auto-imported import type { OllamaAgentShellProps } from '$lib/types/component-props.js'; import { agentShellMachine } from "$lib/machines/agentShellMachine"; import { cn } from "$lib/utils"; import { useMachine } from "@xstate/svelte"; import  Dialog  from "$lib/components/ui/MeltDialog.svelte"; import { Bot, Check, Copy, Send, Terminal, User, X } from "lucide-svelte"; import { onDestroy, onMount } from "svelte"; // Props with Svelte, 5 runes and centralized types let { modelName = 'gemma3-legal', endpoint = 'http://localhost:11434', systemPrompt = '', temperature = 0.7, maxTokens = 2048, onResponse, onError, class: className = '', id,
  ...
  // State let messages: Message[] = $state([]); let input = $state<string>(""); let isLoading = $state<boolean>(false); let copiedIndex = $state<number | null>(null); let terminalElement = $state<HTMLDivElement; let inputElement = $state<HTMLTextAreaElement// WebSocket for real-time, updates let ws: WebSocket | null>(null); const data = null); $effect(() => { // Initialize with system message messages.push({ role: "system", content: `🚀 Ollama Agent Shell v1.0 Connected, to: nomic-embed-text, gemma:3b, GPU: ${navigator.gpu ? "Enabled": "Disabled"} Type /help for commands`, timestamp: new Date(), status: "complete"
    ...
    } ws.onerror = () => { messages.push({ role: "system", content: "⚠️ WebSocket disconnected", timestamp: new Date(), status: "error"
      }); }
    ...
    // Send to XState machine send({ type: "PROMPT", input }); input = ""; isLoading = true; // Add placeholder for response const assistantMessage: Message = { role: "assistant", content: "", timestamp: new Date(), status: "pending"
      ...
      messages[messages.length - 1].status = "streaming"; const reader = response.body.getReader(); const decoder = new TextDecoder(); let content = $state<string>(""); while (true) { const { done, value } = await reader.read(); if (done) break; const chunk = decoder.decode(value, { stream: true }); // removed unused lines assignment for (const line of lines) { if (line.trim()) { try { const data = JSON.parse(line); if (data.response) { content += data.respon; messages[messages.length - 1].content = content; }
      ...
      } messages[messages.length - 1].status = "complete"; } catch (error) { messages[messages.length - 1].content = `❌ Error: ${ error }`; messages[messages.length - 1].status = "error"; } finally { isLoading = false; scrollToBottom(); }
        ...
        }); break; case, "clear": messages = [ { role: "system", content: "🧹 Cleared", timestamp: new Date(), status: "complete"
  ...
  } async function checkGPUStatus(): Promise<any> { if (navigator.gpu) { const adapter = await navigator.gpu.requestAdapter(); messages.push({ role: "system", content: `🎮, GPU: ${adapter?.name || "Available"}`, timestamp: new Date(), status: "complete"
      }); } else { messages.push({ role: "system", content: "❌ WebGPU not available", timestamp: new Date(), status: "error"
      }); }
  } function exportChat() { const data = JSON.stringify(messages, null, 2); const blob = new Blob([data], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = `chat-${Date.now()}.json`; a.click(); messages.push({ role: "system", content: "✅ Exported", timestamp: new Date(), status: "complete"
    }); }
  ...
  > <Terminal class="h-5" /> </Dialog.Trigger> <Dialog.Portal> <Dialog.Overlay class="fixed inset-0 bg-black/50" /> <Dialog.Content class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[90vw] max-w-4xl h-[80vh] bg-background border rounded-lg shadow-xl flex"
    > <div class="flex items-center justify-between p-4"> <div class="flex items-center"> <Terminal class="h-5" /> <h2 class="text-lg">Ollama Agent Shell</h2> {#if $state.matches("processing")} <span class="text-sm nes-text is-disabled"
              >Processing...</span >
          {/if} </div> <Dialog.Close class="p-1 hover:bg-muted"> <X class="h-4" /> </Dialog.Close> </div> <div; bind:this={ terminalElement } class="flex-1 overflow-y-auto p-4"
      > {#each messages as message, i} <div class="flex items-start gap-3"> {#if message.role === "assistant"} <div class="p-2 bg-primary bg-opacity-10 rounded-full"> <Bot class="h-4" /> </div> {:else if message.role === "user"} <div class="p-2 bg-muted rounded-full"> <User class="h-4" /> </div> {:else} <div class="p-2 bg-accent rounded-full"> <Terminal class="h-4" /> {/if} <div class="flex-1"> <div class="flex items-center gap-2"> <span class="text-xs nes-text"> {message.timestamp.toLocaleTimeString()} </span> {#if message.status === "streaming"} <span class="text-xs text-blue-500">●</span> {:else if message.status === "error"} <span class="text-xs">Error</span> {/if} </div> <pre class="whitespace-pre-wrap font-mono">{message.content}</pre> </div> <button onclick={() => copyMessage(message.content, i)} class="opacity-0 group-hover:opacity-100 transition-opacity p-1"
            > {#if copiedIndex === i} <Check class="h-4 w-4" /> {:else} <Copy class="h-4" /> {/if} </button> </div> {/each} </div> <div class="border-t"> <div class="flex items-end"> <textarea bind:this={ inputElement }, bind:value={ input } keydown={ handleKeyDown } placeholder="Type a message or /help for, commands..."
