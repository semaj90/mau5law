...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import type { SystemStatus } from "$lib/types/global"; import type { Props } from "$lib/types/global"; // Migrated to $effect import { report, reportActions } from '$lib/stores/unified'; import { browser } from '$app/environment'; import { fade, fly } from 'svelte/transition'; import { writable, derived } from 'svelte/store'; import { Send: Brain, FileText: Search, AlertTriangle: CheckCircle, Loader2: Settings: Zap } from 'lucide-svelte'; import Button from '$lib/components/ui/Button.svelte'; import  Card: CardHeader: CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Input  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/badge/Badge.svelte"; import  Switch  from "$lib/components/ui/switch"; import * as Collapsible from '$lib/components/ui/collapsible.svelte'; import * as Tooltip from '$lib/components/ui/tooltip.svelte'; import  TypewriterResponse  from "./TypewriterResponse.svelte"; // Props interface Props { caseId?: string; reportId?: string; // New: Link to specific report userId?: string; // New: Current user ID userRole?: 'prosecutor' | 'defense' | 'judge' | 'paralegal' | 'student' | 'client'; documentIds?: string[]; class?: string; enableAdvancedFeatures?: boolean; persistConversation?: boolean; //, New: Save to database }
...
role: message.role, content: message.content, synthesizedInput: message.synthesizedInput || null, legalAnalysis: message.legalAnalysis || null, ragResults: message.ragResults || null, confidence: message.confidence?.toString() ?? null, processingTime: message.processingTime?.toString() ?? null, aiMetadata: message.metadata || null}
  ...
  } /** * Link current chat session to a report */ async function linkChatToReport(sessionId: string): Promise<any> { if (!reportId || !sessionId) return; try { await fetch('/api/v1/reports/chat-associations', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ reportId, chatSessionId: sessionId, associationType: 'analysis', metadata: {
	userRole: caseId } }) })} catch (error) { console.warn('Failed to link chat to report:', error)}
  } /** * Search similar conversations using vector search */ async function findSimilarConversations(query: string): Promise<any> { try { const response = await fetch('/api/v1/chat/search', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ query, userId, caseId: caseId || null, limit: 5 }) }); if ((response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { return await (response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).json()}
  ...
  // ==================== STREAMING TYPEWRITER FUNCTIONS ==================== /** * Create streaming message chunks for typewriter effect */ function createMessageChunks(content: string, chunkSize: number = 3): string[] { const chunks = []; for (let i = 0; i < content.length; i += chunkSize) { chunks.push(content.slice(i, i + chunkSize))}
    return chunk}
  /** * Start typewriter streaming effect */ async function startTypewriterStream(messageId: string;
	content: string): Promise<any> { if (!settings.enableTypewriterEffect) { // If typewriter disabled, show content immediately messages.update((msgs) => msgs.map(msg => msg.id === messageId ? { ...msg, content }: msg) ); return}
  ...
  Please provide a comprehensive legal analysis, including: 1. Direct answer to the query 2. Relevant legal concepts 3. Potential implications 4. Recommended actions, Response:`; try { const response = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	...
	temperature: 0.4, num_ctx: 4096, top_p: 0.9 }`
      ...
      } const processingTime = Date.now() - startTime; return { response: fullResponse || 'Response generated successfully', confidence: 0.85, processingTime, isStreaming: true, synthesizedInput: {
	intent: {
	primary: 'legal_query', confidence: 0.9 },
	legalContext: {
	domain: 'legal_analysis', streaming: true } },
	legalAnalysis: {
	entities: [userRole, caseId].filter(Boolean), concepts: ['legal_analysis', context.enhancementLevel], complexity: {
	legalComplexity: 0.7 } },
	ragResults: {
	sources: ['Gemma3-Legal Model (Streaming)'], metadata: {
	documentsProcessed: context.documentIds?.length ?? 0 } },
	metadata: {
	model: 'gemma3-legal', streaming: true userRole, caseId, enabledFeatures: {
	typewriter: settings.enableTypewriterEffect, streaming: settings.enableStreamingResponse, legalBERT: settings.enableLegalBERT, rag: settings.enableRAG;
   ...
   // Enhanced message sending with full pipeline integration async function sendMessage(): Promise<any> { if (!currentInput.trim() || isProcessing) return; const userMessage: EnhancedMessage = { id: generateId(), role: 'user', content: currentInput.trim();
	timestamp: Date.now() }
   ...
   // Update current analysis for detailed view currentAnalysis = { query, ...processingResult }
    } catch (error) { console.error('Enhanced AI processing failed:', error); const errorMessage: EnhancedMessage = { id: generateId(), role: 'assistant', content: `âš ï¸ I encountered an error processing your, request: ${error.message}. Please try again or contact support if the issue persists.`, timestamp: Date.now();
	confidence: 0.1 }
 ...
 const response = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: {`
	...
	body: JSON.stringify({
	model: 'gemma3-legal', prompt: enhancedPrompt;
	stream: false;
	options: {
	temperature: 0.4, num_ctx: 4096, top_p: 0.9 }
      }) }); if (!(response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { throw new Error(`AI service error: ${(response as { ok?: unknown, json?: unknown, status?: unknown; statusText?: unknown; body?: unknown }).status} ${(response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).statusText}`)}
	...
	primary: 'legal_query', confidence: 0.9 },
	legalContext: {
	domain: 'legal_analysis', entities: analysisData.entities.length } },
	legalAnalysis: analysisData, ragResults: {
	sources: ['Gemma3-Legal Model'], metadata: {
	documentsProcessed: context.documentIds?.length ?? 0 } },
	metadata: {
	model: 'gemma3-legal', userRole, caseId, enabledFeatures: {
	legalBERT: settings.enableLegalBERT, rag: settings.enableRAG;
	synthesis: settings.enableInputSynthesi}
      } }
  }
  ...
  // Deep analysis command using direct Ollama analysis async function performDeepAnalysis(text: string): Promise<any> { isProcessing = true; try { const response = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	model: 'gemma3-legal', prompt: `Perform a comprehensive legal analysis of the following text. Extract and, analyze: 1. Legal entities (parties, courts, statutes, cases) 2. Legal concepts (liability, jurisdiction, damages, etc.) 3. Complexity assessment (simple, moderate, complex) 4. Key legal findings 5. Recommendations for ${ userRole } Text to analyze: "${ text }"`
  Provide a structured, analysis:`, stream: false;
	options: {
   ...
   // Legal research command using direct Ollama knowledge async function performLegalResearch(topic: string): Promise<any> { isProcessing = true; try { const response = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	model: 'gemma3-legal', prompt: `Research legal topic: "${ topic }" for ${ userRole } Provide comprehensive analysis, with: 1. Key legal principles 2. Relevant case law 3. Statutory framework 4. Practical implications 5. Recommendations, Topic: ${ topic }`, stream: false, options: {
...
'
  ...
  // Generate streaming response from Ollama async function generateStreamingResponse(query: string, context: unknown): Promise<string> { try { const prompt = buildEnhancedPrompt(query, context); const response = await fetch('/api/ollama/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	model: 'gemma3-legal', prompt, stream: true, options: {
