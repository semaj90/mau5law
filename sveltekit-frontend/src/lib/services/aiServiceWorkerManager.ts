...
export class AIServiceWorkerManager {
  ...
  private async createWorker(workerId: string, type: string): Promise<void> {
    try {
      ...
      worker.onerror = (error: ErrorEvent) => this.handleWorkerError(workerId, error);
  ...
  public async processParallel(tasks: Omit<AITask, 'id'>[]): Promise<AITaskResult[]> {
    ...
    for (const t of tasks) {
      // await to ensure queue ordering and stable id generation
      // queueTask already updates stores and triggers processing
      // awaiting here keeps the flow explicit and easier to parse by TS tooling
      // (avoids nested Promise.all of mapped async callbacks)
    ...
    const taskPromises: Promise<AITaskResult>[] = taskIds.map((taskId: string) => {
      return new Promise<AITaskResult>(resolve => {
        this.onTaskComplete(taskId, (result: AITaskResult) => {
          resolve(result);
        });
      });
    });
  ...
  private handleTaskComplete(workerId: string, result: AITaskResult): void {
    ...
    workerStatus.tasksCompleted += 1;

    // update average task time (simple moving average)
    const prevAvg = workerStatus.averageTaskTime || 0;
    const count = workerStatus.tasksCompleted;
    workerStatus.averageTaskTime = (prevAvg * (count - 1) + (result.duration || 0)) / count;

    // Cleanup active tasks
    this.activeTasksMap.delete(result.taskId);

    // Invoke completion callbacks
    const cbs = this.completionCallbacks.get(result.taskId) || [];
