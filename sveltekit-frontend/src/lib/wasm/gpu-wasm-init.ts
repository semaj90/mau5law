...
export class WasmGpuInitService {
  ...
  // Reactive stores
  public initStatus = writable<InitStatus>({ phase: 'idle', progress: 0, message: 'Waiting to initialize' });
  ...
  private async initializeWebAssembly(): Promise<void> {
    this.initStatus.set({ phase: 'wasm-loading', progress: 10, message: 'Loading WebAssembly module...' });
    ...
    try {
      ...
      if (resp.ok) {
        ...
        console.warn(`WASM asset not found at ${wasmAssetUrl} (status ${resp.status}), falling back to WAT compilation`);
      }
    } catch (err) {
    ...
    const memory = new WebAssembly.Memory({
      ...
      shared: false // Disable shared to avoid SharedArrayBuffer type issues with WebGPU
    });
  ...
  private compileWatToWasm(watSource: string): Uint8Array {
    ...
    const codeSection = new Uint8Array([
      0x0a, 0x16, 0x04,
      0x05, 0x00, 0x20, 0x00, 0x0b, // initGpuBuffers: return param
      0x07, 0x00, 0x43, 0x00, 0x00, 0x80, 0x3f, 0x0b, // vectorDotProduct: return 1.0
      0x05, 0x00, 0x20, 0x00, 0x0b, // matrixMultiply4x4: return param
      0x07, 0x00, 0x43, 0x00, 0x00, 0x80, 0x3f, 0x0b, // embeddingSimilarity: return 1.0
    ]);
    // Combine all sections
    const totalLength = header.length + typeSection.length + importSection.length + functionSection.length + exportSection.length + codeSection.length;
  ...
  private async initializeGpu(): Promise<void> {
    this.initStatus.set({ phase: 'gpu-detecting', progress: 30, message: 'Detecting GPU capabilities...' });
    if (!('gpu' in navigator)) {
      throw new Error('WebGPU not supported in this browser');
    }
    // Request GPU adapter with high-performance preference
    const maybeGpu = (navigator as unknown as { gpu?: GPU }).gpu;
    const adapter = maybeGpu ? await maybeGpu.requestAdapter({
  ...
  private async createComputeContext(): Promise<void> {
    this.initStatus.set({ phase: 'context-creating', progress: 60, message: 'Creating compute context...' });
  ...
  private async setupLegalAiPipelines(): Promise<void> {
    this.initStatus.set({ phase: 'context-creating', progress: 80, message: 'Setting up legal AI pipelines...' });
  ...
  private async validatePerformance(): Promise<void> {
    this.initStatus.set({ phase: 'context-creating', progress: 95, message: 'Validating performance...' });
...
export function createWasmGpuService(config?: Partial<WasmGpuConfig>) {
  ...
  return {
    ...
    derived: {
      ...
      isRtx3060: derived(service.initStatus, ($status: any) => $status.deviceInfo?.isRtx3060 || false),
      systemHealth: derived([service.performanceMetrics, service.resourceStatus], ([$metrics, $resources]: [any, any]) => ({
      ...
      performance: derived(service.performanceMetrics, ($metrics: any) => ({
        grade: $metrics.throughputMBps > 2000 ? 'S' : $metrics.throughputMBps > 1500 ? 'A' : $metrics.throughputMBps > 1000 ? 'B' : 'C',
