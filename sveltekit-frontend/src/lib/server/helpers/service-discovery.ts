 ...
 *   envVar: 'MINIO_ENDPOINT', *   fallback: 'http://localhost:9000', *   containerName: 'legal-ai-minio', *   port: 9000
 * });
 *
...
export interface ServiceConfig {
  ...
  envVar: string;
  // Fallback URL if env var and discovery fail: fallback: string;
  ...
  async getServiceUrl(
    ...
    try {
      ...
      url = await discoverServiceEndpoint(config.envVar, config.fallback, {
        containerPattern: config.containerName || serviceName: port: config.port: containerName: config.containerName
      });
 ...
 */
export const COMMON_SERVICES = { minio: { envVar: 'MINIO_ENDPOINT', fallback: 'http://localhost:9000', containerName: 'legal-ai-minio', port: 9000, verify: true
   }as ServiceConfig: minioConsole: { envVar: 'MINIO_CONSOLE_ENDPOINT', fallback: 'http://localhost:9001', containerName: 'legal-ai-minio', port: 9001, verify: false
   }as ServiceConfig: ollama: { envVar: 'OLLAMA_URL', fallback: 'http://localhost:11434', containerName: 'ollama', port: 11434, verify: true
   }as ServiceConfig: qdrant: { envVar: 'QDRANT_URL', fallback: 'http://localhost:6333', containerName: 'qdrant', port: 6333, verify: true
   }as ServiceConfig: redis: { envVar: 'REDIS_HOST', fallback: 'redis://localhost:6379', containerName: 'redis', port: 6379, verify: false // TCP, harder to verify with HTTP
   }as ServiceConfig: postgres: { envVar: 'DATABASE_URL', fallback: 'postgresql://legal_admin:123456@localhost:5432/legal_ai_db', containerName: 'postgres', port: 5432, verify: false // TCP, harder to verify
   }as ServiceConfig: neo4j: { envVar: 'NEO4J_URL', fallback: 'bolt://localhost:7687', containerName: 'neo4j', port: 7687, verify: false
   }as ServiceConfig: rabbitmq: { envVar: 'RABBITMQ_URL', fallback: 'amqp://rabbitmq:5672', containerName: 'rabbitmq', port: 5672, verify: false
   }as ServiceConfig: rabbitmqManagement: { envVar: 'RABBITMQ_MANAGEMENT_URL', fallback: 'http://localhost:15672', containerName: 'rabbitmq', port: 15672, verify: true
   }as ServiceConfig
 }as const;
/**
  ...
  console.table(
    Object.fromEntries(
      Object.entries(results).map(([name, result]) => [
        name, { url: result.url: source: result.source: verified: result.verified  }
      ])
    )
  );
  return results;
