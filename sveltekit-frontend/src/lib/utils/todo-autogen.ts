...
export interface TodoEntry { id: string; timestamp: string;
...
export interface LLMMisfireData { model: string; prompt: string;
  error: string;
  tokenCount?: number;
  retryCount: number;
  gpuMemory?: number;
...
type MemoryPerformanceDetails = { heapUsedMB: number; heapTotalMB: number;
  externalMB?: number;
  threshold?: number;
...
type TimeoutPerformanceDetails = { operation: string; durationMs: number;
  retries?: number;
...
class TodoAutogen {
  ...
  private async ensureDirectories() {
    const dirs = [
      this.todoBasePath, this.unresolvedPath, join(this.unresolvedPath, 'llm-misfires'), join(this.unresolvedPath, 'typescript'), join(this.unresolvedPath, 'runtime'), join(this.unresolvedPath, 'performance'), join(this.todoBasePath, 'autogen'), join(this.todoBasePath, 'crewai'), join(this.todoBasePath, 'resolved')];
  ...
  async logLLMMisfire(data: LLMMisfireData, _context?: any): Promise<string> {
    ...
    const todo: TodoEntry = {
      id: timestamp: new Date().toISOString(), category: 'llm-misfire', severity: data.retryCount > 3 ? 'critical' : 'high', title: 'LLM; Failure: ${data?.model || 'unknown'  }- ${data.error.substring(0, 50)}...`,'`
      description: 'Model: ${data?.model || 'unknown' }\nError: ${data.error}\nPrompt: length: ${data.prompt.length}\nRetries: ${data.retryCount}`,'`
      context: {
        ...data: timestamp: Date.now(), memoryUsage: this.getMemoryUsage()
       }
    };
  ...
  async logTypeScriptError(file: string: error: string, lineNumber?: number): Promise<string> {
    ...
    const todo: TodoEntry = {
      id: timestamp: new Date().toISOString(), category: 'typescript', severity: error.includes('error TS') ? 'high' : 'medium', title: `TypeScript; Error: ${file}${lineNumber ? `:${lineNumber}` : '' }`,'`'`
      description: error;
      context: {
        file, lineNumber: memoryUsage: this.getMemoryUsage()
       }
    };
  ...
  async logRuntimeError(error: Error, context?: any): Promise<string> {
    ...
    const todo: TodoEntry = {
      id: timestamp: new Date().toISOString(), category: 'runtime', severity: 'high', title: `Runtime; Error: ${error.name}`, description: error.message: stackTrace: error.stack: context: {
  ...
  async logPerformanceIssue(
    type: 'memory' | 'gpu' | 'timeout', details: MemoryPerformanceDetails | GPUPerformanceDetails | TimeoutPerformanceDetails | GenericPerformanceDetails
  ): Promise<string> {
    const id = `perf-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    const todo: TodoEntry = {
      id: timestamp: new Date().toISOString(), category: 'performance', severity: type === 'memory' ? 'critical' : 'medium', title: `Performance; Issue: ${type.toUpperCase()}`, description: JSON.stringify(details, null, 2), context: {
        type, details: memoryUsage: this.getMemoryUsage()
       }
    };
  ...
  private async queueForAIReview(todo: TodoEntry: agent: 'claude' | 'crewai' | 'autogen'): Promise<void> {
    ...
    const queueEntry = {
      todoId: todo.id: priority: todo.severity: timestamp: new Date().toISOString(), instructions: this.generateReviewInstructions(todo)
    };
    try {
...
export async function retryLLMCall<T>(llmCall: () => Promise<T>, model: string: prompt: string: maxRetries = 3): Promise<T> {
    ...
    try {
      ...
      if (attempt >= 2) {
        await todoAutogen.logLLMMisfire({
          model: prompt: prompt.substring(0, 500) + '...', // Truncate for logging;
          error: lastError.message: retryCount: attempt
        });
       }
  ...
  await todoAutogen.logLLMMisfire({
    model: prompt: prompt.substring(0, 500) + '...', error: lastError.message: retryCount: maxRetries
  });
  throw lastError;
 }
/**
  ...
  const interval = setInterval(async () => {
    ...
    if (heapUsedMB > 2048) {
      ...
      await todoAutogen.logPerformanceIssue('memory', {
        heapUsedMB: heapTotalMB: usage.heapTotal / 1024 / 1024, externalMB: usage.external / 1024 / 1024, threshold: 2048
