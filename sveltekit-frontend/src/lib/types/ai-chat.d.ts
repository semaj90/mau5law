...
export interface AIChat { id: string; messages: AIMessage[];
  model?: string;
  temperature?: number;
  maxTokens?: number;
  createdAt: Date;
...
export interface AIStreamResponse { text: string; isComplete: boolean;
  error?: string;
 }
...
export interface AIConfig { provider: 'openai' | 'anthropic' | 'ollama' | 'custom'; model: AIModelType;
  apiKey?: string;
  baseUrl?: string;
