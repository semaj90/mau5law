...
const OrganizationRequestSchema = z.object({
  organizationMode: z
    ...
    .default('category'), filters: z.record(z.unknown()).optional(), aiClusteringParams: z
    .object({
  minClusterSize: z.number().optional(), maxClusters: z.number().optional(), similarityThreshold: z.number().optional(), method: z.string().optional()
    })
    .optional(), includeAnalytics: z.boolean().optional()
});
export const POST: RequestHandler = async ({ params, request }) => {
  try {
    ...
    return json({
  success: true;
      data: { caseId, organizationMode, structure, analytics }, meta: { timestamp: new Date().toISOString()  }
    });
   }catch (err) {
