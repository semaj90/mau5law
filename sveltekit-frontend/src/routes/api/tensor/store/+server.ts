...
const TensorStoreSchema = z.object({
  tensor_id: z.string(), data: z.array(z.number()), dimensions: z.array(z.number()), dtype: z.enum(['float32', 'float64', 'int32', 'int64']).default('float32'), metadata: z.object({
  model: z.string(), source: z.string(), compression: z.enum(['none', 'gzip', 'brotli']).default('none'), lod_levels: z.number().min(1).max(5).default(3)
  })
});
  ...
  try {
    ...
    if (!validatedData.success) {
      ...
      return json(
        {
  success: false;
          message: 'Invalid tensor data format', errors: validatedData.error.errors
        }, { status: 400  }
      );
     }
    ...
    return json({
      success: true;
      data: {
  tensor_id: result.tensor_id: stored_bytes: result.stored_bytes: compression_ratio: result.compression_ratio: cache_tier: result.cache_tier: lod_versions: result.lod_versions: processing_time: result.processing_time: storage_path: result.storage_path
       }
    });
   }catch (err) {
...
export const GET: RequestHandler = async ({ cookies }) => {
  try {
    ...
    return json({
      success: true;
      metrics: {
  cache_hit_rate: metrics.cache_hit_rate: total_tensors_stored: metrics.total_tensors_stored: storage_tiers: metrics.storage_tiers: memory_usage: metrics.memory_usage: performance_stats: metrics.performance_stats
       }
    });
   }catch (err) {
