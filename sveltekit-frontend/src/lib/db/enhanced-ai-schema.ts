...
export const aiResponses = pgTable('ai_responses', {
  id: uuid('id').defaultRandom().primaryKey(), // Core query-response pair
  query: text('query').notNull(), response: text('response').notNull(), // GRPO-thinking structured reasoning (formatted for feedback)
  ...
  model: text('model').notNull().default('gemma3-legal:latest'), confidence: decimal('confidence', { precision: 5, scale: 4 }), processingTime: integer('processing_time'), // milliseconds
  ...
  sessionId: uuid('session_id'), userId: uuid('user_id'), caseId: uuid('case_id'), // Timestamps for recency scoring
  createdAt: timestamp('created_at').defaultNow().notNull(), lastAccessed: timestamp('last_accessed').defaultNow(), lastUpdated: timestamp('last_updated').defaultNow(), // Metadata for advanced features
  metadata: jsonb('metadata').default(sql`'{ }::jsonb`), tags: jsonb('tags').default(sql`'[]'::jsonb`), // Searchable tags array
  // Quality indicators
...
export const recommendationScores = pgTable('recommendation_scores', {
  id: uuid('id').defaultRandom().primaryKey(), // Target and recommended items
  queryId: uuid('query_id')
    .references(() => aiResponses.id)
    .notNull(), recommendedId: uuid('recommended_id')
    .references(() => aiResponses.id)
    .notNull(), // Scoring components
  ...
  finalScore: decimal('final_score', { precision: 8, scale: 6 }).notNull(), // Recommendation metadata
  algorithm: text('algorithm').notNull(), // 'semantic', 'collaborative', 'hybrid'
  confidence: decimal('confidence', { precision: 5, scale: 4 }), createdAt: timestamp('created_at').defaultNow().notNull(), updatedAt: timestamp('updated_at').defaultNow().notNull()
});
...
export const grpoFeedback = pgTable('grpo_feedback', {
  id: uuid('id').defaultRandom().primaryKey(), // Response being evaluated
  responseId: uuid('response_id')
    .references(() => aiResponses.id)
    .notNull(), // Human preference data
  ...
  sessionContext: jsonb('session_context').default(sql`'{ }::jsonb`), // User providing feedback
  userId: uuid('user_id'), userRole: text('user_role'), // lawyer, paralegal, judge, etc.
  createdAt: timestamp('created_at').defaultNow().notNull()
});
// Vector similarity cache for performance
export const similarityCache = pgTable('similarity_cache', {
  id: uuid('id').defaultRandom().primaryKey(), // Cached similarity pair
  embedding1Id: uuid('embedding1_id')
    .references(() => aiResponses.id)
    .notNull(), embedding2Id: uuid('embedding2_id')
    .references(() => aiResponses.id)
    .notNull(), // Cached similarity score
  similarity: decimal('similarity', { precision: 8, scale: 6 }).notNull(), algorithm: text('algorithm').notNull(), // 'cosine', 'euclidean', 'dot'
  // Cache metadata
  computedAt: timestamp('computed_at').defaultNow().notNull(), validUntil: timestamp('valid_until').notNull(), // Expiry for cache invalidation
});
...
export const legalEntityRelations = pgTable('legal_entity_relations', {
  id: uuid('id').defaultRandom().primaryKey(), // Source and target entities
  sourceResponseId: uuid('source_response_id')
    .references(() => aiResponses.id)
    .notNull(), targetResponseId: uuid('target_response_id')
    .references(() => aiResponses.id)
    .notNull(), // Relationship type and strength
  relationType: text('relation_type').notNull(), // 'cites', 'contradicts', 'supports', 'extends'
  relationStrength: decimal('relation_strength', { precision: 5, scale: 4 }), // Legal context
  legalBasis: text('legal_basis'), // statute, case law, regulation
  jurisdiction: text('jurisdiction'), // Discovery metadata
  discoveredBy: text('discovered_by'), // 'nlp', 'manual', 'graph_algorithm'
  confidence: decimal('confidence', { precision: 5, scale: 4 }), createdAt: timestamp('created_at').defaultNow().notNull()
});
// Performance analytics for recommendation tuning
export const recommendationAnalytics = pgTable('recommendation_analytics', {
  id: uuid('id').defaultRandom().primaryKey(), // Recommendation context
  algorithm: text('algorithm').notNull(), queryType: text('query_type'), // classification from query analysis
  // Performance metrics
  clickThroughRate: decimal('click_through_rate', { precision: 5, scale: 4 }), userSatisfaction: decimal('user_satisfaction', { precision: 5, scale: 4 }), averageRating: decimal('average_rating', { precision: 3, scale: 2 }), // Temporal aggregation
  periodStart: timestamp('period_start').notNull(), periodEnd: timestamp('period_end').notNull(), totalRecommendations: integer('total_recommendations'), createdAt: timestamp('created_at').defaultNow().notNull()
});
...
export const RECOMMENDATION_WEIGHTS = {
  SEMANTIC_SIMILARITY: 0.35, TEMPORAL_SCORE: 0.15, CONTEXT_RELEVANCE: 0.25, USER_PREFERENCE: 0.2, USAGE_COUNT: 0.05
 }as const;
