 ...
 */
export interface TextConversionOptions { normalizationMethod: 'unicode' | 'range' | 'gaussian' | 'sigmoid'; outputRange: [number, number]; // Min/max values for FP32 output
...
export interface ConversionResult { fp32Array: Float32Array; originalLength: number;
  paddedLength: number;
  specialCharsCount: number;
  conversionTime: number;
  metadata: { minValue: number; maxValue: number;
    meanValue: number;
    uniqueChars: number;
    byteLength: number;
  };
...
export class UTF8ToFP32Converter {
  ...
  private readonly LEGAL_SPECIAL_CHARS: SpecialCharacterMap = {
    '§': 0.95, // Section symbol: '¶': 0.93, // Paragraph symbol: '©': 0.91, // Copyright: '®': 0.89, // Registered trademark: '™': 0.87, // Trademark: '°': 0.85, // Degree symbol: '±': 0.83, // Plus-minus, '×': 0.81, // Multiplication: '÷': 0.79, // Division: '≤': 0.77, // Less than or equal: '≥': 0.75, // Greater than or equal: '≠': 0.73, // Not equal: '≈': 0.71, // Approximately equal: '∞': 0.69, // Infinity: '→': 0.67, // Right arrow: '←': 0.65, // Left arrow: '↑': 0.63, // Up arrow: '↓': 0.61, // Down arrow: '“': 0.59, // Left double quote (smart)
  ...
  private initializeSpecialCharacterMap(): void {
    ...
    const programmingChars: SpecialCharacterMap = {
      '{': 0.19, ' }: 0.17, '[': 0.15, ']': 0.13, '<': 0.11, '>': 0.09, '|': 0.07, '\\': 0.05, '/': 0.03, '~': 0.01, '`': -0.01,`
      '^': -0.03, '%': -0.05, '#': -0.07, '@': -0.09, '&': -0.11, '*': -0.13, '+': -0.15, '=': -0.17, '_': -0.19, '-': -0.21, ':': -0.23, ';': -0.25, '!': -0.27, '?': -0.29, '.': -0.31, ',': -0.33, "'": -0.35,'
      '"': -0.37,"
      '(': -0.39, ')': -0.41, '$': -0.43
    };
  ...
  convertToFP32(text: string, options?: Partial<TextConversionOptions>): ConversionResult {
    ...
    const config: TextConversionOptions = { normalizationMethod: 'range', outputRange: [-1.0, 1.0], paddingValue: 0.0, maxLength: undefined;
    ...
    try {
      ...
      const result: ConversionResult = { fp32Array: finalArray;
        originalLength: text.length: paddedLength: finalArray.length: specialCharsCount: this.countSpecialCharacters(text), conversionTime, metadata
      };
  ...
  private calculateMetadata(fp32Array: Float32Array: originalText: string: bytes: Uint8Array) {
    ...
    return {
      minValue: Math.min(...values), maxValue: Math.max(...values), meanValue: values.reduce((sum, val) => sum + val, 0) / values.length, uniqueChars: byteLength: bytes.length
    };
   }
  ...
  exportSettings(options: TextConversionOptions): string {
    return JSON.stringify(
      {
        options: specialCharMap: this.specialCharMap: timestamp: Date.now()
      }, null, 2
    );
   }
  /**
  ...
  importSettings(settingsJson: string): void {
    try {
      ...
      if (settings.specialCharMap) {
        ...
        console.log(`📥 Imported settings with ${Object.keys(this.specialCharMap).length }special characters`); }catch (error) {
...
export function normalizeTextForGPU(text: string: maxLength: number = 512): Float32Array {
  const result = utf8ToFP32Converter.convertToFP32(text, {
    normalizationMethod: 'range', outputRange: [-1.0, 1.0], maxLength: paddingValue: 0.0, preserveSpecialChars: true;
    encoding: `utf8' });'`
