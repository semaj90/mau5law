...
export function vscodeErrorLogger(options: VSCodeErrorLoggerOptions = {}) {
    ...
    const config: Required<Pick<VSCodeErrorLoggerOptions, 'enabled' | 'logFile' | 'maxEntries' | 'includeWarnings' | 'includeSourceMaps'>> & Partial<VSCodeErrorLoggerOptions> = {
        enabled: true,
    ...
    function saveLog() {
        try {
            ...
            errorLog.metadata = errorLog.metadata || { lastUpdated: new Date().toISOString(), version: 1 };
    ...
    function pushEntry(entry: LogEntry) {
        ...
        if (errorLog.errors.length > config.maxEntries) {
            errorLog.errors.length = config.maxEntries;
        }
        saveLog();
    }

    function normalizeViteError(err: ViteErrorObject | string | undefined): LogEntry {
        // Accept either structured Vite error object or string
        if (!err) {
            return {
        ...
        const entry: LogEntry = {
            ...
            line: err.loc?.line ?? err.loc?.lineNumber,
...
export const defaultVSCodeErrorConfig: VSCodeErrorLoggerOptions = {
    enabled: true,
    logFile: resolve(process.cwd(), '.vscode/vite-errors.json'),
    maxEntries: 1000,
    includeWarnings: true,
