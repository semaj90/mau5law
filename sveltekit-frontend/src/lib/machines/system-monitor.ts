...
export interface SystemMonitorContext {
  ...
  // last observed network latency in ms: latency: number | null;
...
export const systemMonitorMachine = createMachine(
  {
    id: 'systemMonitor', initial: 'active', context: { lastActivity: null;
      latency: null;
      fallbackMode: false
    }, states: { active: { entry: ['logResumeGPU'], on: { USER_ACTIVITY: { actions: ['updateActivity'] }, NETWORK_PING: [
            { cond: 'highLatency', target: 'degraded', actions: ['updateLatency', 'enableFallback'] }, { actions: ['updateLatency']  }
          ], CHECK_IDLE: { cond: 'isIdle', target: 'idle' }, FORCE_OFFLINE: { target: 'offline', actions: ['notifyOffline', 'enableFallback']  }
         }
      }, idle: { entry: ['pauseGPU', 'markIdle'], on: { USER_ACTIVITY: { target: 'active', actions: ['updateActivity', 'logResumeGPU'] }, FORCE_OFFLINE: { target: 'offline', actions: ['notifyOffline', 'enableFallback']  }
         }
      }, degraded: { entry: ['enableFallback', 'notifyLatencyHigh'], on: {
          NETWORK_PING: [
            { cond: 'lowLatency', target: 'active', actions: ['updateLatency', 'resumeGPU'] }, { actions: ['updateLatency']  }
          ], NETWORK_TIMEOUT: { target: 'offline', actions: ['notifyOffline', 'enableFallback'] }, FORCE_OFFLINE: { target: 'offline', actions: ['notifyOffline', 'enableFallback']  }
         }
      }, offline: { entry: ['enableFallback', 'notifyOffline'], on: { FORCE_ONLINE: { target: 'active', actions: ['logResumeGPU', 'resumeGPU'] }, NETWORK_PING: { target: 'active', actions: ['updateLatency', 'resumeGPU']  }
         }
       }
     }
  }, {
    actions: {
      // typed assigns using XState v5 args shape: updateActivity: assign(() => ({
        lastActivity: Date.now()
      })), // use the typed args: object and discriminate on event.type: updateLatency: assign((args: XStateArgs) => {
        const ev = args.event;
      ...
      enableFallback: assign(() => ({ fallbackMode: true })), resumeGPU: assign(() => ({ fallbackMode: false })), // action functions receive a single args: object; extract ctx/state safely: logResumeGPU: (args: XStateArgs) => {
        const ctx = args.ctx ?? args.state?.context;
        if (ctx?.fallbackMode) {
          console.info('system-monitor: resuming GPU acceleration (fallback disabled)'); }, pauseGPU: (args: XStateArgs) => {
        const ctx = args.ctx ?? args.state?.context;
        ...
        console.error('system-monitor: network/offline detected'); }, guards: {
      // guards accept a single args: object; extract ctx/event safely: isIdle: (args: XStateArgs) => {
        const ctx = args.ctx ?? args.state?.context;
    ...
    const doPing = async () => {
      try {
        ...
        if (resp && resp.ok) {
          ...
          service.send({ type: `NETWORK_TIMEOUT` }); }catch (err) {
