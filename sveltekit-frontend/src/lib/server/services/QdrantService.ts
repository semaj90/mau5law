...
export class QdrantService {
  ...
  async initializeCollections(): Promise<void> {
	...
	m: 16,
          ...
          const configParams = info?.config as Record<string, unknown> | undefined;
          const vectors = configParams?.params as Record<string, unknown> | undefined;
          const actual = (vectors?.vectors as Record<string, unknown>)?.size;
  ...
  async batchStoreDocuments(documents: DocumentVector[]): Promise<BatchUpsertResult> {
    try {
      const invalidDocs = documents.filter(
        (d) => !Array.isArray(d.vector) || d.vector.length !== this.VECTOR_SIZE
      );
  ...
  private async collectionExists(name: string): Promise<boolean> {
    try {
      ...
      return (
        ...
        collections.collections.some((c) => c.name === name)
      );
    } catch (error) {
