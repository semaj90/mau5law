...
interface ProcessingResult {
    ...
    legal_analysis?: {
        ...
        jurisdiction: string;
        key_clauses: string[];
        risk_assessment: { level: 'low' | 'medium' | 'high' | 'critical'; factors: string[]; score: number };
    ...
    performance_metrics: {
        ...
        summary_generation_ms?: number; // Added: Track summary generation time
...
async function processCudaAccelerated(content: string, options: unknown): Promise<any> {
    try {
        ...
        if (embedding.length > 0) {
            const indexResponse = await fetch(`${CUDA_SERVICE_URL}/api/v1/index/hnsw`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ vectors: [embedding], dimensions: embedding.length, max_elements: 10000 }),
                signal: AbortSignal.timeout(60000)
            });
            if (indexResponse.ok) {
                const indexResult = await indexResponse.json();
                return { embedding: embedding, cuda_index: indexResult, gpu_accelerated: true };
            }
        }
        return { embedding: embedding, gpu_accelerated: false };
    } catch (error: unknown) {
        console.error('CUDA failed: ', error);
        return { embedding: null, gpu_accelerated: false, error: (error as Error).message };
    }
}
...
4. Compliance status
5. Jurisdiction
Content: ${content.substring(0, 4000)}...`, // Limit for performance
                stream: false,
        ...
        if (!response.ok) {
            throw new Error(`Legal failed: ${response.status}`);
        }
        const result = await response.json();
        ...
        console.error('Legal failed: ', error);
        return {
...
function extractComplianceStatus(
    analysis: string
): { status: 'compliant' | 'non_compliant' | 'requires_review'; issues: string[] } {
...
const originalPOSTHandler: SvelteKitRequestHandler = async ({ request }) => {
    try {
        ...
        let cudaResult: unknown = {};
        let embeddingResult: unknown = {};
        let legalAnalysis: unknown = {};
        let summaryResult = '';
        let summaryGenerationMs = 0; // Initialize variable to store summary generation time

        // Step 1: Generate embeddings and CUDA indexing
        if (generateEmbeddings && documentContent) {
            const embeddingStartTime = Date.now();
            if (use_cuda_acceleration) {
                cudaResult = await processCudaAccelerated(documentContent, { legal_domain, chunk_size });
                embeddingResult = {
        ...
        if (generateSummary && documentContent) {
            ...
            try {
                ...
                summaryGenerationMs = Date.now() - summaryStartTime; // Calculate and store summary generation time
            }
        }

        // 4: Entity extraction (simplified - use NER models in production)
        ...
        const response: ProcessingResult = {
            ...
            performance_metrics: {
                ...
                summary_generation_ms: summaryGenerationMs, // Added to performance metrics
            ...
            metadata: {
                ...
                tokens_processed: Math.ceil(documentContent.length / 4), // Rough
        ...
        return json(
            {
                ...
                documentId: (error as any).documentId || `mock-doc-${Date.now()}`,
                ...
                embeddings: {
                    document_embedding: Array(768).fill(0).map(() => Math.random() * 0.1), // Mock 768-dim
                ...
                legal_analysis: {
                    ...
                    risk_assessment: { level: 'medium' as const, factors: ['Standard contract terms', 'Mock risk assessment'], score: 0.6 },
                    compliance_check: { status: 'requires_review' as const, issues: ['Mock compliance check - manual review recommended'] }
                },
                performance_metrics: {
                    ...
                    summary_generation_ms: 0, // Added to mock response as well
                    gpu_utilization: 0,
                    memory_usage_mb: 64
                },
                metadata: {
...
const originalGETHandler: SvelteKitRequestHandler = async ({ url }) => {
    try {
        const documentId = url.searchParams.get('documentId');
        if (!documentId) {
            return json({ error: 'Document ID parameter required' }, { status: 400 });
        }
        return json({ success: true, status: 'completed', documentId });
    } catch (error: Error | unknown) {
        const documentId = url.searchParams.get('documentId'); // Re-get documentId for mock response
        return json(
            {
                success: false,
                error: 'failure default to mock',
