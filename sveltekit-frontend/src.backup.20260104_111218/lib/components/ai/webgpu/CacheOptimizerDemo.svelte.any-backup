...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported // Component props let { ...props }: any = $props(); </script> import { onMount } from 'svelte'; let demoResults = $state<any>(null); let loading = $state<boolean>(false); let selectedOperation = $state<string>('benchmark'); let systemCapabilities = $state<any>(null); let errorMessage = $state<string>(''); // Demo configuration let config = $state({ batchSize: 64, iterations: 10, concurrency: 25, tensorSize: 512; textSamples: [
