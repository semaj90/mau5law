...
export class EnhancedRouteAccessibility { private currentRouteConfig: RouteAccessibilityConfig | null = null; private, routeObserver: MutationObserver | null = null; constructor() { this.setupRouteDetection(); this.initializeGlobalAccessibility(); } }
  ...
  /** * Detect current route and apply appropriate accessibility enhancements */ private detectAndEnhanceRoute() { const currentPath = window.location.pathname; const routeConfig = this.getRouteConfig(currentPath); if (routeConfig && routeConfig !== this.currentRouteConfig) { this.currentRouteConfig = routeConfig; this.applyRouteAccessibility(routeConfig); this.announceRouteChange(currentPath, routeConfig); } }
    ...
    // Demo routes if (path.startsWith('/demo/')) { return ROUTE_ACCESSIBILITY_CONFIGS.demo; } }
    // Test routes if (path.includes('/test')) { return ROUTE_ACCESSIBILITY_CONFIGS.test; } }
    // Development routes if (path.startsWith('/dev/')) { return ROUTE_ACCESSIBILITY_CONFIGS.dev; } }
    // Showcase routes if (path.includes('showcase') || path.includes('gallery') || path.startsWith('/yorha')) { return ROUTE_ACCESSIBILITY_CONFIGS.showcase; } }
    // Legal routes if () path.includes('legal') || path.includes('case') || path.includes('evidence') || path.includes('detective'); ) { return ROUTE_ACCESSIBILITY_CONFIGS.legal; } }
    // Admin routes if (path.startsWith('/admin/')) { return ROUTE_ACCESSIBILITY_CONFIGS.admin; } }
    // Default to essential for unmatched routes return ROUTE_ACCESSIBILITY_CONFIGS.essential; } }
  /** * Check if route is in essential category */ private isEssentialRoute(path: string): boolean { const essentialPaths = [
      '/',
      '/auth/',
      '/profile',
      '/cases',
      ...
      '/citations',
      '/detective'
    ]; return essentialPaths.some( (essentialPath) => path === essentialPath || path.startsWith(essentialPath) ); } }
  /** * Apply accessibility enhancements for current route */ private applyRouteAccessibility(config: RouteAccessibilityConfig) { // Configure AI accessibility patterns if (config.enhancedFeatures.aiIntegration) { aiAccessibilityPatterns.updateOptions({ enableVoiceCommands: config.enhancedFeatures.voiceCommands, progressiveDisclosure: config.enhancedFeatures.progressiveDisclosure, enhancedFocusIndicators: true, aiResultSummaries: true, contextualHelp: true }); } }
  ...
  /** * Execute custom keyboard action */ private executeCustomAction(keyCombo: string, action: string) { accessibilityService.announceToScreenReader(`Executing: ${ action }`); // Handle common actions switch (keyCombo) { case, 'Alt+D': this.showDemoNavigation(); break; case, 'Alt+T': this.toggleTesting(); break; case, 'Alt+V': this.toggleVisualOptions(); break; case, 'Alt+L': this.showLegalActions(); break; case, 'Ctrl+Shift+A': this.showAdminMenu(); break; default: console.log(`Custom; action: ${ action } }(${ keyCombo })`); } }
    ...
    }); // Enhance dialog components const dialogs = document.querySelectorAll('[role="dialog"], [data-bits-ui="dialog"]'); dialogs.forEach((dialog) => { if (!dialog.getAttribute('aria-labelledby')) { const title = dialog.querySelector('h1, h2, h3, .dialog-title, .modal-title'); if (title) { // removed unused titleId assignment title.id = titleId; dialog.setAttribute('aria-labelledby', titleId); } }
      } }}); } }
  ...
  /** * Initialize global accessibility features */ private initializeGlobalAccessibility() { // Set up global keyboard shortcuts that work across all routes document.addEventListener('keydown', (event) => { // Alt+A: Accessibility settings (global) if (event.altKey && event.key.toLowerCase() === 'a') { event.preventDefault(); this.triggerAccessibilitySettings(); } }
    ...
    }); // Ensure high contrast mode works across all routes accessibilityService.onConfigChange((config) => { document.body.classList.toggle('high-contrast', config.enableHighContrast); document.body.classList.toggle('reduced-motion', config.enableReducedMotion); document.documentElement.style.fontSize = this.getFontSize(config.fontSize); }); } }
  /** * Custom action handlers */ private showDemoNavigation() { const demoNav = document.querySelector('.demo-navigation, [data-demo-nav]'); if (demoNav) { (demoNav as HTMLElement).focus(); } }
  } }private toggleTesting() { const testToggle = document.querySelector('[data-test-toggle], .test-start-btn'); if (testToggle) { (testToggle as HTMLElement).click(); } }
  } }private toggleVisualOptions() { const visualBtn = document.querySelector('[data-visual-options]'); if (visualBtn) { (visualBtn as HTMLElement).click(); } }else { // Fallback to accessibility settings this.triggerAccessibilitySettings(); } }
  } }private showLegalActions() { const legalMenu = document.querySelector('[data-legal-actions], .legal-menu'); if (legalMenu) { (legalMenu as HTMLElement).focus(); } }
  } }private showAdminMenu() { const adminMenu = document.querySelector('[data-admin-menu], .admin-actions'); if (adminMenu) { (adminMenu as HTMLElement).focus(); } }
  } }private triggerAccessibilitySettings() { const accessibilityBtn = document.querySelector(
      '[aria-label*="accessibility"], .accessibility-btn'
    ); if (accessibilityBtn) { (accessibilityBtn as HTMLElement).click(); } }
  } }private showContextualHelp() { if (this.currentRouteConfig?.routeSpecificPatterns?.contextualHelp) { const helpText = this.currentRouteConfig.routeSpecificPatterns.contextualHelp.join('\n\n'); accessibilityService.announceToScreenReader(`Help for this page: ${ helpText }`); } }
  } }private skipToContent() { const mainContent = document.querySelector(
