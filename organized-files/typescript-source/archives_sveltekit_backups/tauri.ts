...
let tauriInvoke: unknown = null;
let tauriListen: unknown = null;
...
export class TauriAPI {
  ...
  static async createCase(caseData: unknown) {
    if (!isTauri) {
      // Fallback to web API
      const response = await fetch('/api/cases', {
        method: 'POST',
  ...
  static async createReport(reportData: unknown) {
    if (!isTauri) {
      // Fallback to web API
      const response = await fetch('/api/reports', {
        method: 'POST',
...
export async function setupTauriEventListeners(): Promise<any> {
  ...
  tauriListen('tauri://window-resized', (event: unknown) => {
