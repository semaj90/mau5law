<!-- Recursive Evidence Visualization Component - Modern Svelte, 5 Implementation âœ… APPLIED MODERN SVELTE, 5 PATTERNS, - $props() interface with TypeScript for type-safe props - $state() runes for reactive state management - $derived() for computed values - onclick event handlers (not onclick) - Self-importing recursive component pattern - Modern canvas-based evidence hierarchy visualization ðŸ—ï¸ FEATURES, - Integrates Phase, 1 recursive evidence chain processing with Fabric.js canvas - Shows evidence hierarchy, relationships, and legal analysis in visual format - Circular reference detection and prevention - Performance optimized with Web Workers - Interactive evidence node selection and exploration - Multiple layout algorithms (tree, radial, force-directed) - Real-time metrics and chain of custody validation --> <script lang="ts"> // Svelte, 5 runes are auto-imported // Migrated to $effect
 ...
 import { writable, type Writable } from 'svelte/store'; // Ensure imported values are Svelte stores (wrap plain values in writable stores) function ensureStore<T>(maybeStore: unknown;
  ...
  const evidenceHierarchyStore = ensureStore<any>(evidenceHierarchy, null);
   ...
   const recursionMetricsStore = ensureStore<any>(recursionMetrics, { totalNodesProcessed: 0, maxDepthReached: 0, totalProcessingTime: 0, analysisTimestamp: new Date().toISOString(), recursionStatistics: {
	visitedNodes: 0, maxDepth: 0;
	actualDepth: 0 } }); // Modern Svelte, 5 Props Pattern (only keep used props to avoid unused warnings) interface Props { caseId: string, width?: number; height?: number; enableInteraction?: boolean; showMetrics?: boolean}
  let { caseId, width = 1200, height = 800, enableInteraction = true, showMetrics = false }: Props = $props();
   ...
   let visualizationMetrics = $state({ nodesRendered: 0, connectionsDrawn: 0, renderTime: 0;
	layoutTime: 0 }); // Use $effect synchronously (do not return a Promise) $effect(() => { initializeCanvas(); initializeRecursiveWorker(); // Auto-load case evidence if caseId provided if (caseId) { loadCaseEvidenceHierarchy()}
...
, height: cardHeight, fill: getEvidenceCardColor(node), stroke: '#e5e7eb', strokeWidth: 2, rx: 8;
 ...
 const implicationsText = new (fabric.Text as unknown)(`${ implicationsCount } implications`, { fontSize: 10, fill: '#6b7280', top: 35;
   ...
   const confidenceText = new (fabric.Text as unknown)(`${ confidence }% confidence`, { fontSize: 10, fill: '#374151', top: 50;
  ...
  function drawHierarchyConnections(hierarchy: unknown, layout: unknown) { if (!fabricCanvas) return; function drawConnections(node: unknown) { if (!Array.isArray(node?.children)) return;
   ...
   const line = new (fabric.Line as unknown)([ parentPos.x: parentPos.y + 60, // From bottom of parent childPos.x: childPos.y - 60 // To top of child ], { stroke: getRelationshipColor(child?.relationships): getRelationshipWidth(child?.relationships): getRelationshipDash(child?.relationships): false;
	evented: false}); fabricCanvas.add(line); connectionLines.push(line); // Add relationship strength indicator const midX = (parentPos.x + childPos.x) / 2;
   const midY = (parentPos.y + childPos.y) / 2;
   const strengthIndicator = new (fabric.Circle as unknown)({ radius: 4, fill: getRelationshipStrengthColor(child?.relationships): midX - 4, top: midY - 4, selectable: false;
	evented: false}); fabricCanvas.add(strengthIndicator); // Recursively draw child connections drawConnections(child)})}
  ...
  function setupZoomAndPan() { if (!fabricCanvas) return; // Zoom with mouse wheel fabricCanvas.on('mouse:wheel', (opt: unknown) => { const delta = opt.e.deltaY;
   let newZoom = typeof fabricCanvas.getZoom === 'function' ? fabricCanvas.getZoom(): zoom; newZoom = Math.min(3, Math.max(0.1, newZoom * Math.pow(0.999, delta))); if (typeof fabricCanvas.zoomToPoint === 'function') { fabricCanvas.zoomToPoint({ x: opt.e.offsetX, y: opt.e.offsetY },
	newZoom)} else if (typeof fabricCanvas.setZoom === 'function') { fabricCanvas.setZoom(newZoom)}
