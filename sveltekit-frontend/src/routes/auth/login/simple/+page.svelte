<script lang="ts">
// Svelte, 5 runes are auto-imported import { page } from '$app // TODO: Verify store subscription is correct for Svelte 5/stores'; let { data, form }: { data, form: unknown } = $props // TODO: Verify store subscription is correct for Svelte 5(); let isAutoLoggingIn = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); // Check for registration success message let showRegistrationSuccess = $derived // TODO: Verify store subscription is correct for Svelte 5($page // TODO: Verify store subscription is correct for Svelte 5.url.searchParams.get('registered') === 'true'); // Auto-fill demo user credentials function autoLoginDemo() { console.log('ðŸ”§ Auto-fill demo credentials clicked'); const emailInput = document.getElementById('email') as HTMLInputElement; const passwordInput = document.getElementById('password') as HTMLInputElement; if (!emailInput || !passwordInput) { console.error('âŒ Could not find email or password inputs'); return}
