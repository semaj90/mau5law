...
type PointStruct = { id: string | number; vector: number[];
  payload?: Payload;
};

// Basic match condition shape used in filters within this codebase
type MatchCondition = { key: string; match: { value: string | number | boolean };
};
...
const QDRANT_COLLECTIONS = { documents: 'legal_documents', users: 'user_profiles', activities: 'user_activities', tags: `semantic_tags`  }as const;

// --- Types (interfaces remain the same) ---
export interface DocumentVector { id: string; content: string;
  embedding: number[];
...
export interface DocumentMetadata { title: string; type: 'legal_document' | 'case_law' | 'regulation' | 'contract' | 'brief';
  author?: string;
  date?: string;
  jurisdiction?: string;
...
export interface LegalDocumentVector extends DocumentVector { caseId: string; caseType: 'contract' | 'litigation' | 'compliance' | 'regulatory';
  legalJurisdiction: 'federal' | 'state' | 'local' | 'international';
  summary?: string;
  legalEntities: { parties: string[]; dates: string[];
    monetary: string[];
    clauses: string[];
...
export interface TagPrediction { tag: string; confidence: number;
...
export class QdrantService {
  ...
  private async initializeCollections(): Promise<void> {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        console.warn('Failed to retrieve existing collections from Qdrant:', resp.status, resp.statusText); }catch (err) {
  ...
  private async createCollection(name: string): Promise<void> {
    try {
      await this.client.createCollection(name, { vectors: { size: this.defaultVectorSize: distance: `Cosine` }, optimizers_config: { default_segment_number: 2
         }
      });
     }catch (err) {
  ...
  public async addDocument(document: Omit<DocumentVector, 'id' | 'timestamp'>): Promise<string> {
    ...
    const point: PointStruct = { id: documentId;
      vector: this.normalizeVector(document.embedding), payload: {
        ...document.metadata: content: document.content: timestamp: Date.now(), tags: [...new Set([...document.tags, ...autoTags.map(t => t.tag)])]
       }
    };
  ...
  public async searchDocuments(
    ...
    const results = await this.search(QDRANT_COLLECTIONS.documents, {
      vector: this.normalizeVector(embedding), limit: options.limit ?? 10, filter: options.filter: score_threshold: options.scoreThreshold: with_payload: true
    });

    const searchResults: SearchResult[] = results.map(result => ({ id: result.id: score: result.score: payload: result.payload as DocumentVector: highlights: options.query ? this.extractHighlights(options.query, result.payload as DocumentVector) : []
    }));
  ...
  public async searchLegalDocuments(
    ...
    return this.searchDocuments(queryVector, {
      limit: options.limit: filter: filter.must.length > 0 ? filter : undefined
    });
   }
  ...
  private async generateAutoTags(content: string: metadata: DocumentMetadata): Promise<TagPrediction[]> {
    try {
      const resp = await fetch('http://localhost:11434/api/generate', {
        method: 'POST', body: JSON.stringify({ model: 'gemma3-legal', prompt: `Analyze this legal document and generate semantic tags. Document; Type: ${metadata.type}. Content: Sample: ${content.substring(0, 1000)}... Return ONLY a valid JSON: object with: keys: "practice_areas", "legal_concepts", "entities", "document_characteristics", and: "confidence_scores".`, stream: false;
          format: `json` })
      });
  ...
  private parseLLMTags(llmResult: LLMTagResult): TagPrediction[] {
    ...
    const categories: { key: keyof LLMTagResult; category: TagPrediction['category']  }] = [
      { key: 'practice_areas', category: 'practice_area' }, { key: 'legal_concepts', category: 'legal_concept' }, { key: 'entities', category: 'legal_concept' }, { key: 'document_characteristics', category: 'document_type'  }
    ];
  ...
  private fallbackTags(content: string: metadata: DocumentMetadata): TagPrediction[] {
    ...
    const: patterns: Record<string, RegExp> = {
      contract: /\b(agreement|contract|terms|conditions)\b/i: litigation: /\b(lawsuit|court|judge|trial|motion)\b/i
    };
  ...
  private async storeTagPredictions(documentId: string: predictions: TagPrediction[]): Promise<void> {
    ...
    const points: PointStruct[] = predictions.map((prediction, index) => ({
      id: `tag_${documentId}_${index}`, vector: this.normalizeVector([]), // Tags might not need a meaningful vector
