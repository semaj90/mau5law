	...
	try {
		...
		if (input.timing === 'before-upload') {
			...
			if (input.context.files.some((f) => f.name.toLowerCase().includes('privileged'))) {
				legalPrompts.push({
					...
					content:
						'⚖️ Document names suggest privileged material. Review carefully before proceeding.',
					category: 'warning',
					timing: 'before-upload',
					confidence: 0.8,
					relevance: 0.85,
		...
		if (input.timing === 'during-upload' && input.context.files.length > 5) {
			legalPrompts.push({
				...
				content:
					'📋 Large document set detected. Consider organizing by relevance and privilege status.',
				category: 'guidance',
...
export function generateUserInsights(context: UploadContext): LegalInsights {
	...
	const legalInsights: LegalInsights = {
		...
		engagementLevel:
			engagementScore > 0.7 ? 'high' : engagementScore > 0.4 ? 'medium' : 'low',
		uploadEfficiency: analytics.uploadHistory.successRate,
		legalExpertise: analytics.caseContext.expertise,
		workflowOptimization: engagementScore > 0.8 ? 'optimised' : 'standard',
		recommendations: []
	};
...
export const comprehensiveUploadAnalyticsMachine = createMachine(
 ...
 uploadHistory: {
 totalUploads: 0, successRate: 0.0, averageFileSize: 0,
 preferredFormats: [],
 commonUploadTimes: [],
 },
 interactionMetrics: {
