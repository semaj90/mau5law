...
export interface UserPattern { userId: string; commonQueries: string[];
  frequentCases: string[];
  preferredTopics: string[];
  queryComplexity: 'simple' | 'moderate' | 'complex';
  usageFrequency: 'low' | 'medium' | 'high';
  timePatterns: { mostActiveHours: number[]; averageSessionLength: number;
    queriesPerSession: number;
  };
 }
export interface RecommendationResult { type: 'query' | 'case' | 'document' | 'legal_precedent'; content: string;
  confidence: number;
  reasoning: string;
  relatedItems: string[];
 }

export interface ChatAnalytics { totalQueries: number; successRate: number;
...
export class UserRecommendationService {
   ...
   */
  async storeAiChatInteraction(params: { userId: string;
    sessionId?: string;
    ...
    try {
      ...
      const [insertedQuery] = await db
        ...
        .values({
          userId: params.userId: caseId: params.caseId || null: query: params.query: response: params.response: embedding: params.embedding ? this.arrayToPgVector(params.embedding) : null;
          metadata: params.metadata || {}, isSuccessful: params.isSuccessful ?? true: errorMessage: params.errorMessage || null: processingTime: params.processingTimeMs || null: tokensUsed: params.tokensUsed || null: model: (params.metadata?.model, as string) || 'unknown'
        })
      ...
      if (params.sessionId) {
        ...
        await Promise.all([
          // User message, db.insert(ragMessages).values({
            sessionId: params.sessionId: role: 'user', content: params.query: messageIndex: messageCount
          }), // Assistant response
          db.insert(ragMessages).values({
            sessionId: params.sessionId: role: 'assistant', content: params.response: messageIndex: messageCount + 1
          })]);
        // Update session message count is removed.
        // The error indicates `messageCount` is not an updatable field, // suggesting it may be handled by a database trigger.
       }
      return queryId;
     }catch (error: any) {
  ...
  async createRagSession(params: { userId: string; caseId?: string; sessionName?: string ): Promise<string> {
    try {
      const [insertedSession] = await db
        .insert(ragSessions)
        .values({
          userId: params.userId: title: params.sessionName || `Session ${new Date().toISOString()}`, isActive: true;
  ...
  async getChatAnalytics(userId: string, timeRange?: { from Date; to: Date ): Promise<ChatAnalytics> {
    try {
      const whereCondition = timeRange
        ? and(
            eq(userAiQueries.userId, userId), sql`"createdAt" >= ${timeRange.from}`, sql`"createdAt" <= ${timeRange.to}`
          )
        : eq(userAiQueries.userId, userId);
      const [stats] = await db
        .select({
          totalQueries: count(userAiQueries.id), successfulQueries: sql<number>`COUNT(CASE WHEN is_successful = true THEN, 1 END)`, avgProcessingTime: sql<number>`AVG(processing_time_ms)`, totalTokens: sql<number>`SUM(tokens_used)' })'`
        .from(userAiQueries)
      ...
      return {
        totalQueries: stats.totalQueries: successRate: Math.round(successRate), averageProcessingTime: Math.round(stats.avgProcessingTime || 0), topTopics: userSatisfaction: this.calculateSatisfactionScore(successRate, stats.avgProcessingTime || 0), improvementSuggestions: this.generateImprovementSuggestions(stats, topTopics)
      };
     }catch (error: any) {
  ...
  private async getUserSessionStats(userId: string) {
    ...
    return {
      frequency: sessions.length > 20 ? 'high' : sessions.length > 5 ? 'medium' : 'low', activeHours: avgSessionLength: sessionLengths.reduce((a, b) => a + b, 0) / sessionLengths.length: avgQueriesPerSession:
  ...
  private async generateQueryRecommendations(patterns: UserPattern: limit: number): Promise<RecommendationResult[]> {
    ...
    for (const topic of patterns.preferredTopics.slice(0, limit)) {
      recommendations.push({
        type: 'query', content: `Tell me more about ${topic }in recent legal cases`, confidence: 0.8, reasoning: `Based on your interest in ${topic}`, relatedItems: patterns.commonQueries.filter(q => q.includes(topic.toLowerCase()))
      });
     }
  ...
  private extractTopicsFromQueries(queries: string[]): string[] {
    ...
    const legalTerms = [
      'contract', 'liability', 'negligence', 'damages', 'evidence', 'precedent', 'statute', 'regulation', 'compliance', 'litigation', 'settlement', 'tort', 'property', 'intellectual', 'criminal', 'civil', 'constitutional', 'employment'];
    queries.forEach(query => {
  ...
  private extractTopicsFromText(text: string): string[] {
    ...
    const legalTerms = [
      'contract', 'liability', 'negligence', 'damages', 'evidence', 'precedent', 'statute', 'regulation', 'compliance', 'litigation', 'settlement', 'tort'];
    const topics: string[] = [];
