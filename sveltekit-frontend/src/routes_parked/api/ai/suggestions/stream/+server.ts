...
export async function POST({ request }: RequestEvent): Promise<Response> {
    try {
        ...
        const { content, reportType = 'prosecution_memo', useOllamaStreaming = true, useRAGStreaming = true, maxSuggestions = 5 } = data;
        ...
        return new Response(
            JSON.stringify({ error: 'Failed to start streaming', details: msg }),
