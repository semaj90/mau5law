...
export class RecommendationOrchestrator {
  ...
  private setupRabbitMQListeners() {
    const mq = rabbitMQService;
    if (!mq) return;

    // Cast to the typed shape instead of using `any`
    const mqTyped = mq as RabbitMQLike;

    // Support multiple event APIs (on, addListener, subscribe, subscribeEvent)
    const binder = mqTyped.on ?? mqTyped.addListener ?? mqTyped.subscribe ?? mqTyped.subscribeEvent;
    ...
    bind('evidence_processed', (data: EvidenceProcessedPayload) => {
      this.handleEvidenceProcessed(data);
    });

    // Listen for vector search results
    bind('vector_search_complete', (data: VectorSearchCompletePayload) => {
   ...
   */
  private generateDetectiveIdleRecommendations(context: IdleContext) {
   ...
   */
  private generateEvidenceIdleRecommendations(context: IdleContext) {
    const recommendations: Recommendation[] = [];
   ...
   */
  private generateGeneralIdleRecommendations(context: IdleContext) {
    const recommendations: Recommendation[] = [];
