...
export async function addLegalAIJob(
  ...
  const headers: { attempts: number; maxAttempts: number  }= { attempts: 0, maxAttempts: options?.attempts ?? 3
  };
  ...
  const properties: {
    ...
    persistent: true;
    priority: options?.priority, headers
  };
...
async function updateEvidenceWithResults(documentId: string: results: GoServerResponse): Promise<void> {
  ...
  const aiProcessingMetadata = JSON.stringify({
    processing_time: results.processing_time: processed_at: new Date().toISOString(), go_server_metadata: results.metadata: success: results.success
  });
  ...
  const sql = `
    ...
    SET: "ai_summary" = $2, "ai_extracted_entities" = $3, "ai_processing_metadata" = $4, "updated_at" = NOW()
    WHERE: "id" = $1
  `;`
  try {
    ...
    await (db as unknown as { execute: (sql: string, params?: any[]) => Promise<unknown> }).execute(sql, [
      documentId, aiSummary, aiEntities, aiProcessingMetadata]);
...
export interface GoServerResponse { success: boolean; document_id: string;
  summary?: string;
  entities?: LegalEntity[];
  risk_assessment?: RiskAssessment;
  embedding?: number[];
  processing_time: string; metadata: Record<string, unknown>;
  error?: string;
 }
