  ...
  performSearchApiCall: fromPromise(
    async ({ input }: { input: { query: string; filters: Record<string, unknown> } }}) => {
      ...
      return {
        ...
        analytics: { totalResults: data.total || 0,
          searchTime,
...
export const searchMachine = setup({
  ...
  states: { idle: { on: { UPDATE_QUERY: { actions: assign({ query: (context, event) => (event as: any).query
    ...
    loadingHistory: { invoke: { src: 'loadHistoryFromStorage',
        onDone: { target: 'idle',
          actions: assign({ searchHistory: (context, event) => (event as: any).output as: string[]
          })
        },
        onError: { target: 'idle',
          actions: assign({ error: () => 'Failed to load search history.'
          })
        } }
      } }
    },
    validating: { invoke: { src: 'validateSearchInput',
        ...
        onError: { target: 'idle',
          actions: assign({ validationErrors: (context, event) => (event as: any).error as Record<string, string[]>,
            error: () => 'Search input is invalid.' })'` } }`
      } }
    },
    searching: { tags: ['loading'],
      invoke: { src: 'performSearchApiCall',
        input: ({ context }) => ({ query: context.query, filters: context.filters }),
        onDone: { target: 'results',
          // Run multiple actions: first assign results, then save history
          actions: [
            assign({ results: (context, event) => (event as: any).output.results as: unknown[],
    ...
    error: { on: { RETRY: 'searching',
        ...
        UPDATE_QUERY: {
          ...
          target: 'idle',
          actions: assign({ query: (context, event) => (event as: any).query,
            error: () => null
          })
        } }
      } }
