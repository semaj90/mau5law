<script lang="ts">
  ...
  > <!-- Modal --> <div class="w-full max-w-md bg-panel border-2 border-sand/20 shadow-[0_0_30px_rgba(0 0 0 0.5)] overflow-hidden"
      transition:scale={{
	duration: 200, start: 0.95 }}
      style:border-color={config.color}
    > <!-- Header --> <div class="bg-panel px-4 py-2 border-b flex justify-between items-center" style:border-color={config.color}>
        <div class="flex items-center gap-3"> <div class="w-5 h-5 flex items-center justify-center border font-bold text-xs" style:border-color={config.color} style:color={config.color}>
            ...
            class="text-sand/60 hover:text-white transition-colors"
          >
            ✕
          </button> {/if} </div> <!-- Content --> <div class="p-6"> {#if message} <p class="text-sand/40 text-xs leading-relaxed mb-4">{message}</p> {/if}
          {#if type === 'prompt'} <div class="mt-4"> <input
              type="text"
              bind:value={promptValue}
              class="w-full bg-panel border border-sand/20 p-2 text-info text-xs focus:border-info outline-none transition-all"
              placeholder="ENTER SYSTEM SEED..."
            /> </div> {/if}
          {#if children} <div class="mt-4"> {@render children()} </div> {/if} </div> <!-- Actions --> <div class="bg-panel/50 p-4 flex justify-end gap-3 border-t border-sand/20"> {#if type === 'confirm' || type === 'prompt'} <button
            onclick={handleCancel}
            class="px-4 py-1.5 text-[10px] text-sand/40 border border-sand/20 hover:bg-panelSoft transition-all uppercase"
          >
            CANCEL
          </button> {/if} <button
