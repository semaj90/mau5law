/** * MCP Context7 Embedding Integration * Multi-core parallel embedding generation with function calling support * Leverages MCP Context7 multicore server for distributed processing * * Features: * - Parallel embedding generation across multiple workers * - Function calling for gemma3 models (extractive QA, summarization) * - Load balancing and task distribution * - Real-time progress tracking and metrics * - Fallback to local Ollama if MCP unavailable * - Caching layer integration * * @author Legal AI Platform Team * @version 1.0.0 */ import fetch from 'node-fetch'; import type { GemmaEmbeddingService } from './gemma-embedding-service'; import type { PgVectorIndexingService } from './pgvector-indexing-service'; /** * MCP Context7 Configuration */ export interface MCPContext7Config { baseUrl: string; workers: number; timeout: number; retryAttempts: number; fallbackToLocal: boolean; }
...
try {
    ...
    /**
     * fallback: parallel embedding without MCP
     */
    ...
    /**
     * fallback: function call using direct Ollama
     */
    ...
    getWorkerStats(): {
        ...
        return {
            ...
            averageTasksPerWorker: this.workerPool.size > 0 ? totalTasks / this.workerPool.size : 0
        };
    }
