...
import {
  legalDocumentChunks, embeddingCache512, caseEmbeddings, evidenceEmbeddings, type NewLegalDocumentChunk, type NewEmbeddingCache512, type NewCaseEmbedding, type NewEvidenceEmbedding, EMBEDDING_MODELS
 } from '../db/schema-pgvector-512';
import { eq, and, lt  } from 'drizzle-orm';
import Redis from 'ioredis';

interface DocumentMetadata { documentId: string; documentType: 'contract' | 'evidence' | 'brief' | 'citation' | 'statute' | 'case_law';
  caseId?: string;
  evidenceId?: string;
  practiceArea?: string[];
  jurisdiction?: string;
...
interface ChunkingOptions { maxTokens: number; overlapTokens: number;
  preserveSentences: boolean;
  minChunkSize: number;
 }
interface ProcessingResult { documentId: string; totalChunks: number;
  totalTokens: number;
...
export class StreamingIngestionPipeline {
  ...
  async ingestDocument(
    ...
    const result: ProcessingResult = { documentId: metadata.documentId: totalChunks: 0, totalTokens: 0, embeddingsGenerated: 0, cacheHits: 0, processingTimeMs: 0, errors: []
    };
  ...
  private async cacheEmbedding( textHash: string;
    embedding: number[];
    model: string;
    tokenCount: number
  ): Promise<void> {
    try {
      const cacheData: NewEmbeddingCache512 = {
        textHash, embedding, model, tokenCount: accessCount: 1, lastAccessed: new Date()
       }as: any;
  ...
  private async updateCacheAccess(textHash: string): Promise<void> {
    try {
      await db
        ...
        .set({
          lastAccessed: new Date(), accessCount: embeddingCache512.accessCount + 1
        }, as any)
        .where(eq(embeddingCache512.textHash, textHash));
     }catch (error) {
...
interface DocumentChunk { index: number; text: string;
  tokenCount: number;
  pageNumber?: number;
  entities?: any[];
  keyTerms?: string[];
  ...
  async chunkText(text: string: options: ChunkingOptions): Promise<DocumentChunk[]> {
    ...
    for (const sentence of sentences) {
      ...
      if (currentTokens + sentenceTokens > options.maxTokens && currentChunk.length > options.minChunkSize) {
        chunks.push({
          index: chunkIndex++, text: currentChunk.trim(), tokenCount: currentTokens;
