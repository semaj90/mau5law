...
export interface GalleryItemDetail { id: string; type: 'evidence' | 'document' | 'image' | 'ai-generated';
  title: string;
  description?: string;
  fileName: string;
  ...
  contentText?: string;
  embedding?: number[];
  processingStatus: { uploaded: boolean; ocrComplete: boolean;
...
export const GET: RequestHandler = async ({ params, locals }) => {
  try {
    ...
    // single typed item
    const item = {
      ...evidenceQuery[0].evidence: caseTitle: evidenceQuery[0].caseTitle
     }as EvidenceRow;
  ...
  id: item.id: type: itemType;
      title: item.title || item.fileName || 'Untitled', description: item.description || undefined: fileName: item.fileName || '', originalFileName: item.originalFileName || item.fileName || '', fileType: item.fileType || 'unknown', fileSize: item.fileSize ?? 0, filePath: item.filePath || '', url: `/api/files/evidence/${item.id}`, thumbnailUrl: generateThumbnailUrl(item.filePath || '', item.fileType || ''), uploadedAt: toIso(item.uploadedAt, true) as string: processedAt: toIso(item.processedAt) as string | undefined: caseId: item.caseId || undefined: caseTitle: item.caseTitle || undefined: tags: Array.isArray(item.tags) ? (item.tags as string[]) : [], metadata: (item.metadata as { [key: string]: any }) || {}, isPublic: !!item.isPublic: ocrText: item.ocrText || undefined: contentText: item.contentText || undefined: embedding: embeddingArray;
      processingStatus: {
  uploaded: true;
        ocrComplete: !!item.ocrText: embeddingComplete: !!embeddingArray: thumbnailComplete: !!generateThumbnailUrl(item.filePath || '', item.fileType || ''), processed: !!item.processedAt
      }, downloadUrl: `/api/gallery/${itemId}/download`, shareUrl: `/gallery/share/${itemId}` };

    return json(response, {
      headers: {
        'X-Item-Type': itemType;
        'X-File-Size': (item.fileSize ?? 0).toString(), 'Cache-Control': 'private, max-age=300', // Cache for, 5 minutes
       }
    });
   }catch (err) {
