...
<script lang="ts">
  ...
  interface FileState {
    ...
    nextRetryAt?: number | null; // timestamp for scheduled retry
  ...
  function serializeSession() {
    ...
    const pending = fileStates
      .filter(f => !['completed', 'canceled'].includes(f.status))
      .map(f => ({
        name: f.file.name,
        size: f.file.size,
        type: f.file.type,
    ...
    try {
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify({ ts: Date.now(), files: pending }));
    } catch (e) {
      /* ignore */
    }
  }
  ...
  function matchPlaceholders(incoming: File[]) {
    for (const f of incoming) {
      const idx = fileStates.findIndex(
        ps => ps.placeholder && ps.file.name === f.name && ps.originalSize === f.size
      );
      if (idx !== -1) {
  ...
  function scheduleRetry(fs: FileState, reason: string) {
    ...
    const delay = Math.min(8000, 600 * Math.pow(2, Math.max(0, fs.attempts))) + Math.floor(Math.random() * 300);
    fs.status = 'pending';
  ...
  function cancelAllUploads() {
    ...
    fileStates = fileStates.map(fs => {
      ...
      if (fs.retryTimeoutId) {
        try { clearTimeout(fs.retryTimeoutId); } catch(e) { /* ignore */ }
        fs.retryTimeoutId = null;
      }
  ...
  function handleFileSelect(_event: Event) {
    // target assignment for file input
  ...
  function processFiles(newFiles: File[]) {
    ...
    // const existingNames = new Set(fileStates.map(f => f.file)); // This line was unused
  ...
  function removeFile(index: number) {
    ...
    if (target?.retryTimeoutId) {
      try { clearTimeout(target.retryTimeoutId); } catch(e) { /* ignore */ }
      target.retryTimeoutId = null;
    }
  ...
  async function processUploadQueue(): Promise<void> {
    while (uploadQueue.length > 0 && uploading) {
      ...
      if (enableToastNotifications && batchToastId) {
        const completed = fileStates.filter(fs => fs.status === 'completed').length;
        const total = performanceMetrics.totalFiles || 1;
  ...
  async function uploadSingleFile(fs: FileState) {
    ...
    if (enableToastNotifications) {
      fs.toastId = toastService.upload(
        ...
        'Starting upload...',
        {
          ...
          actions: [{
            label: 'Cancel',
            action: () => { // Fixed: action property syntax
              controller.abort();
              fs.status = 'canceled';
            },
            style: 'danger',
    ...
    formData.append('uploadData', JSON.stringify({
      caseId,
      title: file.name,
      description: `Uploaded via drag-and-drop: ${file.name}`, // Fixed: missing colon
    ...
    try {
      ...
      xhr.send(formData);
      const data = await resultPromise; // Fixed: resultPromi to resultPromise
      fs.progress = 90;
      ...
      if (data[0]) {
        ...
        try {
          // TODO: replace with real extracted / OCR text from the uploaded file
          ...
          console.warn('Outer embedding error:', outerEmbeddingErr); // Added missing console.warn for outer catch
        }
        ...
        if (fs.startTime && fs.endTime) {
          ...
          performanceMetrics.averageUploadTime = performanceMetrics.totalUploadTime / Math.max(1, performanceMetrics.completedFiles);
        }
...
<style>
  ...
  .drop-zone:hover .upload-icon:not(.uploading) { /* Fixed: selector */
    transform: scale(1.1);
  }
  .upload-text h3 {
    margin: 0;
  ...
  .sr-only {
    position: absolute; /* Fixed: missing semicolon */
    width: 1px;
    height: 1px;
    padding: 0; /* Fixed: missing semicolon */
    margin: -1px;
    overflow: hidden;
    clip: rect(0, 0, 0, 0);
    white-space: nowrap;
  ...
  .file-icon {
    flex-shrink: 0; /* Fixed: missing semicolon */
    color: #6b7280;
  }
  .file-info {
    flex: 1;
    min-width: 0; /* Fixed: missing semicolon */
  }
  .file-name {
    font-weight: 500;
    color: #374151;
  ...
  :global(.nes-theme) .upload-progress {
    background: #fff;
    border-radius: 0; /* Fixed: missing semicolon */
  ...
  :global(.nes-theme) .upload-button:hover:not(:disabled) { /* Fixed: selector */
    background: #1081c4;
  }
  ...
  .upload-button:disabled { /* Fixed: selector */
    opacity: 0.6;
    cursor: not-allowed;
  }
  .clear-button {
