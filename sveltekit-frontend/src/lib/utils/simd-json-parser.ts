...
class SIMDJSONParser {
	...
	async parse(
		...
		if (useGoService) {
			try {
				...
				console.warn(
					'Go SIMD service failed, falling back to native parsing:',
					error
				);
			}
		}
	...
	async benchmark(
		...
		const throughput =
			(jsonString.length * iterations) /
			(avgTime * iterations / 1000) /
			(1024 * 1024);
...
export async function parseEvidenceData(
	jsonString: string
): Promise<SIMDParseResult> {
	return simdParser.parse(jsonString, {
		useGoService: true,
...
export async function parseCaseScoring(
	jsonString: string
): Promise<SIMDParseResult> {
	return simdParser.parse(jsonString, {
		useGoService: true,
