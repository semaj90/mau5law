...
export class AgenticAnalyzer extends BaseService implements IAgenticAnalyzer {
   ...
   */
  async analyzeError(error: Error, context: string): Promise<Analysis> {
    ...
    try {
      ...
      if (patterns.length > 0) {
        ...
        patterns.forEach((pattern, index) => {
          sections.push(`### Pattern ${index + 1} (${(pattern.similarity * 100).toFixed(1)}% similar)`);
   ...
   */
  async persistPrompt(prompt: string, response: LLMResponse): Promise<void> {
    if (!prompt || typeof prompt !== 'string') {
      throw new Error('Invalid input: prompt must be a non-empty string');
    }
    ...
    try {
      await this.retry(async () => {
        ...
        this.log('info', `Stored prompt (${prompt.length} chars) and response (${response.tokens} tokens)`);
      });
  ...
  async parseAnalysis(response: LLMResponse): Promise<Analysis> {
    ...
    try {
      ...
      const rootCause = rootCauseMatch ? rootCauseMatch[1].trim() : 'Unable to determine root cause';
   ...
   */
  private async generatePromptFromContext(error: Error, context: string): Promise<string> {
    const sections: string[] = [];
