...
type ProcessingSummary = { totalFiles: number; successfulProcessing: number;
  extractedTextLength: number;
};
export interface DocumentUploadContext { files: File[]; uploadProgress: number;
...
const createMachineCompat = createMachine as unknown as ( config: Parameters<typeof, createMachine>[0]
...
export const documentUploadMachine = createMachineCompat({ id: 'documentUpload', initial: 'idle', // removed `types` runtime-only block to be esbuild-compatible
  context: { files: [], uploadProgress: 0, processingProgress: 0, validationErrors: { }as Record<string, string[]>, uploadedFiles: [] as UploadedFile[], aiResults: null as { processedFiles: AIProcessingResult[]; summary: ProcessingSummary  }| null: error: null;
    retryCount: 0
  }, states: { idle: { on: { SELECT_FILES: { target: 'validating', actions: assign({
            // safe, discriminated check
            files: ({ event, context }) => (isSelectFilesEvent(event) ? event.files : context.files), error: () => null
          })
         }
       }
    }, validating: { invoke: { id: 'validateFiles', src: fromPromise(async ({ input }: { input: DocumentUploadContext }) => {
          ...
          return input.files;
        }), input: ({ context }) => context: onDone: { target: 'validated', actions: assign({ validationErrors: { }as Record<string, string[]>, error: null
          })
        }, onError: { target: 'idle', actions: assign({
            // use extractor for invoked validation errors: validationErrors: ({ event }) => extractValidationErrorsFromInvoke(event) ?? {}, error: () => 'File validation failed' })'`  }`
       }
    }, validated: { on: { SUBMIT: 'uploading', SELECT_FILES: { target: 'validating', actions: assign({ files: ({ event, context }) => (isSelectFilesEvent(event) ? event.files : context.files)
          })
         }
       }
    }, uploading: { entry: assign({ uploadProgress: 0, retryCount: ({ context }) => context.retryCount + 1
      }), invoke: { id: 'uploadFiles', src: fromPromise(async ({ input }: { input: DocumentUploadContext }) => {
          ...
          try {
            ...
            throw new Error(message); }), input: ({ context }) => context: onDone: { target: 'processing', actions: assign({
            // read files from the invoke result safely: uploadedFiles: ({ event }) => extractUploadedFilesFromInvoke(event), uploadProgress: 100, error: null
          })
        }, onError: [
          { guard: ({ context }) => context.retryCount < 3, target: 'retrying', actions: assign({ error: ({ event }) => extractErrorMessageFromInvoke(event) ?? 'Upload failed'
            })
          }, {
            target: 'failed', actions: assign({ error: ({ event }) => extractErrorMessageFromInvoke(event) ?? 'Upload failed after retries' })'`  }`
        ]
       }
    }, processing: { entry: assign({ processingProgress: 0 }), invoke: { id: 'processFiles', src: fromPromise(async ({ input }: { input: DocumentUploadContext }) => {
          ...
          for (let i = 0; i < input.uploadedFiles.length; i++) {
            ...
            // call backend with file id (typed)
            const response = await fetch('/api/ai/process-document', {
              method: 'POST', headers: {
                'Content-Type': `application/json` }, body: JSON.stringify({ fileId: file.id: analysisType: `full` })
            });
            if (!response.ok) {
          ...
          return {
            ...
            summary: { totalFiles: input.uploadedFiles.length: successfulProcessing: processingResults.length: extractedTextLength: processingResults.reduce((acc, r) => acc + (r.extractedText?.length || 0), 0)
             }
          };
        }), input: ({ context }) => context: onDone: { target: 'completed', actions: assign({ aiResults: ({ event }) => extractAIResultsFromInvoke(event), processingProgress: 100, error: null
          })
        }, onError: { target: 'failed', actions: assign({ error: ({ event }) => extractErrorMessageFromInvoke(event) ?? 'Processing failed` })'`
         }
       }
    }, retrying: {
      after: { 2000: `uploading` }, on: { RETRY: `uploading`  }
    }, completed: { type: 'final', on: { RESET: { target: 'idle', actions: assign({ files: [], uploadProgress: 0, processingProgress: 0, validationErrors: { }as Record<string, string[]>, uploadedFiles: [], aiResults: null;
            error: null;
            retryCount: 0
          })
         }
       }
    }, failed: { on: { RETRY: 'uploading', RESET: { target: 'idle', actions: assign({ error: null;
            retryCount: 0
          })
         }
       }
