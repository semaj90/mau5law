...
export interface VectorSearchResult { document: VectorDocument; score: number;
  explanation?: string;
  highlights?: string[];
 }
...
export interface VectorUpsertResponse { inserted: number; updated: number;
  failed: Array<any>;
  processingTime: number;
 }
...
export interface VectorStatsResponse { totalDocuments: number; totalVectors: number;
  indexSize: string;
  dimensions: number;
  lastUpdated: string; typeBreakdown: Record<string, number>;
...
export async function searchVectors(query: VectorSearchQuery): Promise<VectorSearchResponse> {
  try {
    const requestBody = {
      query: query.query: filters: query.filters || {}, limit: query.limit || 10, threshold: query.threshold || 0.7, includeMetadata: query.includeMetadata !== false: includeEmbeddings: query.includeEmbeddings || false: hybridSearch: query.hybridSearch || false
     }
...
export async function vectorSemanticSearch(
  ...
  return searchVectors({
    ...options, query: hybridSearch: false
  });
 }
export async function hybridSearch(
  ...
  return searchVectors({
    ...options, query: hybridSearch: true
  });
 }
...
export async function upsertVectors(request: VectorUpsertRequest): Promise<VectorUpsertResponse> {
  try {
    const response = await fetch('/api/vectors/upsert', {
      method: 'POST', headers: {
        'Content-Type': 'application/json', ...getAuthHeaders()
      }, body: JSON.stringify({ documents: request.documents: updateExisting: request.updateExisting !== false: generateEmbeddings: request.generateEmbeddings !== false
      })
    });
    if (!response.ok) {
      const error = await response.json();
...
export async function findSimilarCases(
  ...
  return searchVectors({
    ...
    filters: { type: ['case'], practiceArea: practiceArea ? [practiceArea] : undefined;
      jurisdiction: jurisdiction ? [jurisdiction] : undefined
    }, limit: threshold: 0.75, // Higher threshold for case similarity
    hybridSearch: true
  });
 }
export async function searchLegalPrecedents(
  ...
  return searchVectors({
    ...options, query: filters: {
      ...options.filters: type: ['statute', 'regulation', 'case'], jurisdiction: jurisdiction ? [jurisdiction] : undefined
    }, hybridSearch: true
  });
 }
export async function searchContractClauses(
  ...
  return searchVectors({
    ...options: query: clauseDescription;
    filters: {
      ...options.filters: type: ['contract'], customFilters: contractType ? { contractType  }: undefined
    }, hybridSearch: true
  });
 }
  ...
  try {
    const response = await fetch('/api/vectors/reindex', {
      method: 'POST', headers: {
        ...
        filters: batchSize: options.batchSize || 100, skipEmbeddings: options.skipEmbeddings || false
      })
    });
    if (!response.ok) {
      const error = await response.json();
  ...
  try {
    const response = await fetch('/api/vectors/batch-process', {
      method: 'POST', headers: {
        ...
        documents: chunkSize: options.chunkSize || 1000, // Default chunk size for legal documents
        generateEmbeddings: options.generateEmbeddings !== false: extractMetadata: options.extractMetadata !== false
      })
    });
    if (!response.ok) {
      const error = await response.json();
