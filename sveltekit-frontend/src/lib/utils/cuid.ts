...
/** * Generate a CUID (Collision Resistant Unique Identifier) * Format: c + timestamp + counter + fingerprint + random */ export function cuid(): string { return 'c' + getTimestamp() + getCounter() + getFingerprint() + getRandom(); }
/** * Generate a slug version of CUID (shorter, URL-friendly) */ export function slug(): string { return getTimestamp().slice(-4) + getCounter() + getRandom().slice(0, 4); }
export default cuid; 