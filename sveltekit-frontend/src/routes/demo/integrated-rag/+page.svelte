<script lang="ts">
  /**
   * Integrated RAG Demo Page
   * Complete workflow: Upload → embeddinggemma → pgvector → Qdrant → MinIO → Search → Recommendations
   */
  import IntegratedRAGUpload from '$lib/components/upload/IntegratedRAGUpload.svelte';
  ...
  function sendSearch() {
    ...
    wsClient?.send({ type: 'search', query: searchQuery });
  }
</script>

<svelte:head>
  <title>Integrated RAG System - Legal AI</title>
</svelte:head>
...
<div class="demo-page">
  ...
  <div class="pipeline-diagram">
    ...
    <div class="flow-steps">
      ...
      <div class="arrow">→</div>
      <div class="step">🤖 embeddinggemma (384-dim)</div>
      <div class="arrow">→</div>
      <div class="step">🐘 PostgreSQL + pgvector</div>
      <div class="arrow">→</div>
      <div class="step">🔍 Qdrant Vector DB</div>
      <div class="arrow">→</div>
      <div class="step">💾 MinIO Object Storage</div>
      <div class="arrow">→</div>
      <div class="step">📊 Recommendations</div>
    </div>
  </div>
  ...
  <div class="stats-bar">
    ...
    <div class="stat-item">
      ...
      <span class="stat-value">embeddinggemma:latest</span>
    </div>
    <div class="stat-item">
      <span class="stat-label">Vector Dimensions:</span>
      <span class="stat-value">384</span>
    </div>
    <div class="stat-item">
      <span class="stat-label">Databases:</span>
      <span class="stat-value">pgvector + Qdrant + MinIO</span>
    </div>
  </div>

  <IntegratedRAGUpload onSuccess={handleSuccess} onError={handleError} />

  {#if lastUpload}
    <div class="integration-status">
      <h3>✅ Last Upload Status</h3>
      <div class="status-grid">
        <div class="status-item">
          <span class="status-icon">🤖</span>
          <div>
            <strong>Embedding</strong>
            <p>embeddinggemma:latest (384-dim)</p>
          </div>
        </div>
        <div class="status-item">
          <span class="status-icon">🐘</span>
          <div>
            ...
            <p>{lastUpload.document.chunks} chunks stored</p>
          </div>
        </div>
        <div class="status-item">
          <span class="status-icon">🔍</span>
          <div>
            ...
            <p>{lastUpload.document.qdrantStored ? 'Indexed ✓' : 'Not available'}</p>
          </div>
        </div>
        <div class="status-item">
          <span class="status-icon">💾</span>
  ...
  {#if wsError}
    <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded mb-4">
      {wsError} (Connection attempts: {connectionAttempts})
    </div>
  {/if}
