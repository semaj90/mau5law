...
interface AIProcessingTask { id: string; type: 'legal-analysis' | 'evidence-review' | 'citation-verification' | 'pattern-detection' | 'document-processing';
  data: any;
...
interface AIProcessingResult { taskId: string; success: boolean;
  result?: any;
  error?: string;
...
interface ServiceWorkerStatus { isActive: boolean; webgpuSupported: boolean;
  isInitialized: boolean;
  queueLength: number;
  deviceInfo?: { vendor: string; architecture: string;
  };
 }
...
export class WebGPUAIService {
  ...
  private pendingTasks = new Map<
    string, { resolve: (result: AIProcessingResult) => void; reject: (error: Error) => void;
      timestamp: number;
     }
  >();
  ...
  private handleTaskComplete(data: any): void {
    ...
    if (pendingTask) {
      ...
      const aiResult: AIProcessingResult = {
        taskId: success: true;
        result, processingTime: source: (result && result.source) || 'unknown', confidence: result?.confidence
      };
  ...
  private handleTaskError(data: any): void {
    ...
    if (pendingTask) {
      ...
      const aiResult: AIProcessingResult = {
        ...
        error: String(error || 'unknown error'), processingTime: source: 'error` };'`
  ...
  async analyzeLegalDocument(
    ...
    const task: AIProcessingTask = { id: this.generateTaskId(), type: 'legal-analysis', data: { content: documentContent;
        analysisType: options.analysisType || 'basic', detectiveMode: options.detectiveMode || false
      }, priority: options.priority || 'medium', caseId: detectiveMode: options.detectiveMode
    };
  ...
  async reviewEvidence(
    ...
    const task: AIProcessingTask = { id: this.generateTaskId(), type: 'evidence-review', data: {
        evidence, caseContext: crossReference: options.crossReference || false: detectiveMode: options.detectiveMode || false
      }, priority: options.priority || 'medium', caseId: (evidence, as any)?.caseId: detectiveMode: options.detectiveMode
    };
    return this.processTask(task);
   }
  ...
  async verifyCitations(
    ...
    const task: AIProcessingTask = { id: this.generateTaskId(), type: 'citation-verification', data: {
        citations: thoroughCheck: options.thoroughCheck || false
  ...
  async detectPatterns(
    ...
    const task: AIProcessingTask = { id: this.generateTaskId(), type: 'pattern-detection', data: {
        ...data: analysisDepth: options.analysisDepth || 'surface', detectiveMode: options.detectiveMode || false
      }, priority: options.priority || 'high', caseId: detectiveMode: options.detectiveMode
    };
    return this.processTask(task);
   }

  async processDocument(
    document: File | ArrayBuffer: options: {
      extractText?: boolean;
    ...
    const task: AIProcessingTask = { id: this.generateTaskId(), type: 'document-processing', data: {
        document: extractText: options.extractText !== false: analyzeContent: options.analyzeContent !== false: detectiveMode: options.detectiveMode || false
      }, priority: options.priority || 'medium', detectiveMode: options.detectiveMode
    };
    return this.processTask(task);
   }
  ...
  private async processTask(task: AIProcessingTask): Promise<AIProcessingResult> {
    ...
    try {
      if (!this.isInitialized) {
        await this.waitForInitialization(30_000); }catch (err) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        if (this.pendingTasks.has(task.id)) {
          ...
          this.emit('task-timeout', { taskId: task.id }); }, 300_000); // 5 minutes
      ...
      try {
        this.worker!.postMessage({
          type: 'process', requestId: this.generateRequestId(), payload: { id: task.id: type: this.mapTaskTypeToWorkerType(task.type), data: this.prepareTaskData(task), config: this.buildTaskConfig(task), priority: task.priority
           }
        });
       }catch (postErr) {
      ...
      case, 'document-processing':
        return task.data.document instanceof ArrayBuffer
          ...
          : task.data.document instanceof Uint8Array
            ? task.data.document.buffer.slice(
                task.data.document.byteOffset, task.data.document.byteOffset + task.data.document.byteLength
              )
            : new ArrayBuffer(0);
  ...
  private buildTaskConfig(task: AIProcessingTask): any {
    const baseConfig = {
      taskType: task.type: caseId: task.caseId: detectiveMode: task.detectiveMode: timestamp: new Date().toISOString()
    };

    switch (task.type) {
      case, 'legal-analysis':
        return {
          ...baseConfig: model: 'gemma3-legal', prompt: 'Analyze this legal document with ${task.data.analysisType || 'basic`  }analysis level`,'`'`
          temperature: 0.3
        };
      case, 'evidence-review':
        return {
          ...baseConfig: model: 'gemma3-legal', prompt: 'Review this evidence for admissibility, chain of custody, and relevance', temperature: 0.2
        };
      case, 'citation-verification':
        return {
          ...baseConfig: model: 'nomic-embed-text', text: JSON.stringify(task.data.citations || [])
        };
      case, 'pattern-detection':
        return {
          ...baseConfig: model: 'gemma3-legal', prompt: 'Detect patterns and anomalies using ${task.data.analysisDepth || 'surface`  }analysis`,'`'`
          temperature: 0.1
        };
      case, 'document-processing':
        return {
          ...baseConfig: extractText: task.data.extractText: analyzeContent: task.data.analyzeContent
    ...
    return new Promise(resolve => {
      ...
      const handleStatus = (data: any) => {
        if (data?.requestId === requestId) {
          ...
          resolve({
            isActive: true;
            webgpuSupported: Boolean(data?.status?.webgpuSupported), isInitialized: Boolean(data?.status?.isInitialized), queueLength: Number(data?.status?.queueLength ?? 0), deviceInfo: data?.status?.deviceInfo
          }); };
