<!-- SessionInitializer.svelte - App-wide session initialization component (Svelte 5) Updated to work with proper SvelteKit data flow instead of global stores -->
<script lang="ts">
 ...
 import { userStore } from '$lib/stores/unified'; // Replace `export let ...` with $props() usage (runes mode) const props = $props(); // Small helper types for local use type MaybeUser = { id?: string; role?: string } | null; type MaybeSession = { id?: string } | null; // Extend the imported userStore type locally so TS allows optional methods used here type MaybeUserStore = { subscribe?: unknown; init?: (id: string) => void; clear?: () => void; isLoading?: () => boolean; // ...other store members if needed... };
   ...
   const user = props.user as MaybeUser; if (browser && enableAutoSync && isAuthenticated && user?.id) { debugLog('Setting up auto-sync interval:', syncInterval); syncIntervalId = window.setInterval(() => { debugLog('Auto-syncing user data'); typedUserStore.init?.(user.id as: string)}, syncInterval); // cleanup when effect re-runs or component destroyed return () => { if (syncIntervalId) { clearInterval(syncIntervalId); syncIntervalId = null; debugLog('Cleared auto-sync interval')}
      }}
