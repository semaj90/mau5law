...
export interface ContainerMetrics { name: string; memoryMB: number;
  cpu: number;
 }
export interface ThroughputMetrics { requestsPerSec: number; latencyMs: number;
 }
export interface MemoryOptimizationResult { beforeMB: number; afterMB: number;
  freedMB: number;
  actions: string[];
 }
export class DockerMemoryOptimizer {
  ...
  async getResourceMetrics() {
    return { memory: { usage: this.containers.reduce((s, c) => s + c.memoryMB, 0) * 1024 * 1024 }, cpu: { usage: this.containers.reduce((s, c) => s + c.cpu, 0) }, containers: this.containers.length
    };
   }
