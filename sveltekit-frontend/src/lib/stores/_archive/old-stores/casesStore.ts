 import type { Case } from "$lib/types"; import { writable } from 'svelte/store'; export interface CaseStoreData { cases: any[]; stats: any[], filters: {
	search: string; status: string, string: priority, string: sort?: string; order?: string} }const initialData: CaseStoreData = { cases: [], stats: [], filters: {
	search: "", status: "all", priority: "all", sort: "openedAt", order: "desc" } } }
