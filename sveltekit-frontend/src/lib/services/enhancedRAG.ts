...
export interface QueryIntent { type: string; confidence: number;
  entities: string[];
 }
...
export interface RAGQuery { id: string; originalQuery: string;
  expandedQueries: string[];
  intent: QueryIntent;
  context: QueryContext;
  timestamp: number;
 }
export interface RAGResponse { queryId: string; synthesizedAnswer: string;
  sources: SearchResult[]; confidence: number;
 }
...
export class EnhancedRAGSystem {
  ...
  public async queryRAG(
    ...
    const resp: RAGResponse = { queryId: `stub_${Date.now()}`, synthesizedAnswer: '(stub)', sources: [], confidence: 0
    };
...
const _ollamaStub = {
  id: 'ollama-local', name: 'Ollama Local', type: 'ollama', endpoint: 'http://localhost:11434', models: [], as string[], capabilities: [], as string[], status: 'online` }, as unknown as Partial<LLMProvider>;'`
