...
import type {
  AITask as ProperAITask, AIResponse as ProperAIResponse, WorkerStatus as ProperWorkerStatus
...
declare global {
  ...
  type WorkerMessage = { taskId: string; type: 'status' | 'result' | 'error';
    data: any;
    timestamp: Date;
  };
  type EnhancedRAGEngine = { query: (input: string, options?: { limit?: number; threshold?: number }) => Promise<AIResponse>;
  ...
  type ErrorProcessingPipeline = { processErrors: (errors: Error[]) => Promise<Array<unknown>>; analyzeError: (error: Error) => Promise<unknown>;
  };
 }
// API Request/Response Types - Using proper definitions
import type {
  CaseCreateRequest as ProperCaseCreateRequest, CaseUpdateRequest as ProperCaseUpdateRequest, CaseSearchRequest as ProperCaseSearchRequest, CaseSearchResponse as ProperCaseSearchResponse, EvidenceCreateRequest as ProperEvidenceCreateRequest, EvidenceSearchRequest as ProperEvidenceSearchRequest, BulkOperationResponse as ProperBulkOperationResponse, FormSubmissionResult as ProperFormSubmissionResult
 } from '../types/svelte5-patterns';
declare global {
  ...
  type CommandSearchResponse = { results: Array<unknown>; total: number;
  };
...
import type {
  CaseState, EvidenceState, VectorSearchResult as ProperVectorSearchResult, VectorSearchOptions as ProperVectorSearchOptions
 } from '../types/svelte5-patterns';
declare module, '$lib/types/database' {
  export type LegalDocument = { id: string; title: string;
    content: string;
    fileType: 'PDF' | 'TEXT' | 'DOCX' | 'HTML';
    uploadedAt: Date;
    processedAt?: Date; metadata: Record<string, unknown>;
    embedding?: number[];
  };
  export type DocumentChunk = { id: string; documentId: string;
    chunkIndex: number;
    content: string;
    embedding?: number[]; metadata: Record<string, unknown>;
  };
  export type UserAiQuery = { id: string; userId: string;
    query: string;
    response?: string;
    createdAt: Date;
    embedding?: number[];
  };
  export type AutoTag = { id: string; entityId: string;
...
declare global {
  ...
  type QueryResult = { content: string; score: number;
    sources?: any[];
  };
 }
// XState Types
...
declare global {
  type GGUFInferenceRequest = { prompt: string; maxTokens: number;
    temperature: number;
    topP: number;
    topK: number;
