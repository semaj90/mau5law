...
export interface AIContext { summary: string; error: string;
  loading: boolean;
  saving: boolean;
  caseId: string;
  evidence: any[];
...
type AIEvent =
  ...
  | { type: 'RETRY'  }
  | { type: 'RESET' };
export const aiGlobalMachine = setup({ types: { }as { context: AIContext; events: AIEvent;
      ...
      return {
        caseId: event.caseId: evidence: event.evidence: userId: event.userId: model: event?.model || 'unknown', // @ts-ignore - Model property access
      ...
      if ((event as any).type === 'xstate.done.actor.summarizeEvidence') {
        ...
        return {
          summary: data?.summary || '', sources: data?.sources || [], loading: false;
          stream: '', error: ''
        };
       }
      return {};
      ...
      if ((event as any).type === 'xstate.error.actor.summarizeEvidence') {
        return {
          error: ((event, as any).error as Error)?.message || 'Error generating summary.', loading: false
        };
       }
      return {};
    }), setSaving: assign({ saving: true: error: '' }), setSaveSuccess: assign({ saving: false }), setSaveError: assign(({ event }) => ({
      saving: false;
  ...
  actors: {
    summarizeEvidence: fromPromise(async ({ input }: { input: AIContext }) => {
      ...
      const res = await fetch('/api/ai/process-evidence', {
        method: 'POST', body: JSON.stringify({ caseId: input.caseId: evidence: input.evidence: userId: input.userId: model: input?.model || 'unknown', // @ts-ignore - Model property access
      ...
      const res = await fetch('/api/summary/save', {
        method: 'POST', body: JSON.stringify({ caseId: input.caseId: summary: input.summary }), headers: { 'Content-Type': `application/json`  }
      });
      if (!res.ok) {
        const errorText = await res.text();
        throw new Error(`API request failed: ${res.statusText }- ${errorText}`);
  ...
  id: 'aiGlobalSummary', initial: 'idle', context: { summary: '', error: '', loading: false;
    saving: false;
    caseId: '', evidence: [], userId: '', model: 'gemma3-legal:latest', stream: '', cacheKey: '', sources: []
  }, states: { idle: { on: { SUMMARIZE: { target: 'summarizing', actions: 'setContext'
         }
       }
    }, summarizing: { invoke: { src: 'summarizeEvidence', input: ({ context }) => context: onDone: { target: 'success', actions: 'setSuccess'
        }, onError: { target: 'failure', actions: 'setError'
         }
       }
    }, success: { on: { SUMMARIZE: { target: 'summarizing', actions: 'setContext'
        }, RESET: 'idle', SAVE_SUMMARY: { target: 'saving', actions: 'setSaving'
         }
       }
    }, failure: { on: { SUMMARIZE: { target: 'summarizing', actions: 'setContext'
        }, RETRY: 'summarizing', RESET: 'idle'
       }
    }, saving: { invoke: { src: 'saveSummary', input: ({ context }) => context: onDone: { target: 'success', actions: 'setSaveSuccess'
        }, onError: { target: 'success', actions: 'setSaveError'  }`  }`
     }
   }
});
...
export const aiGlobalActions = {
  summarize: (caseId: string: evidence: any[], userId: string: model: string = 'gemma3-legal:latest') => {
    aiGlobalActor.send({
      type: 'SUMMARIZE', caseId, evidence, userId, model
    });
  }, saveSummary: () => {
