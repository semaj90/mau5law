...
export const accessibleClick: Action<HTMLElement, AccessibleClickParams> = (node, params) => {
  ...
  const onKeyDown = (event: KeyboardEvent) => {
    ...
    const key = event.key;
    if (key === 'Enter' || key === ' ') { // 'Spacebar' is redundant with ' '
  ...
  return {
    ...
    update(newParams) {
      ...
      if (!newParams) return;
      const { role: newRole = 'button', handler: newHandler, label: newLabel, description: newDescription } = newParams; // Corrected destructuring
...
export function accessibleButton(element: HTMLElement, params: { handler: (e: Event) => void; label?: string }) { // Corrected function signature
  return accessibleClick(element, { role: 'button', ...params });
}

/** * Specialized variant for menu items */
export function accessibleMenuItem(element: HTMLElement, params: { handler: (e: Event) => void; label?: string }) { // Corrected function signature
