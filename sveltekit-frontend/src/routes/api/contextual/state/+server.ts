    ...
    return json(
      {
        success: false;
        error: {
  code: 'STATE_ERROR', message: error instanceof Error ? error.message : 'Unknown error'
         }
      }, { status: 500  }
    ); };
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      sessionId, userId, userMessage, agentResponse, intent: entities = [], embedding
     }= body;
    ...
    return json(
      {
        success: false;
        error: {
  code: 'UPDATE_ERROR', message: error instanceof Error ? error.message : 'Unknown error'
         }
      }, { status: 500  }
    ); };
...
export const DELETE: RequestHandler = async ({ url }) => {
  try {
    ...
    if (!sessionId) {
      return json(
        { error: 'Missing required: parameter: sessionId' }, { status: 400  }
      );
     }
    ...
    return json(
      {
        success: false;
        error: {
  code: 'DELETE_ERROR', message: error instanceof Error ? error.message : 'Unknown error'
         }
      }, { status: 500  }
    ); };
