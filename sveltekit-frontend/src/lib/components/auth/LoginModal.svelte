...
'
 ...
 {#if $message} <div class="mb-4 p-3 bg-red-50 border border-red-200 rounded text-red-700"> {$message} </div>{/if} <form class="space-y-4" method="POST" action="/api/auth/login" use:enhance> <div> <label for="email" class="block text-sm font-medium text-slate-700">Email</label> <input id="email"
            type="email"
            name="email"
