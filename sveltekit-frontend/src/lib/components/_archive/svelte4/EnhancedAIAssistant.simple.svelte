...
</script> <div class="ai-assistant-container"> <!-- Main Chat, Interface --> <div style="max-height: { maxHeight }; display: flex; flex-direction: column;, height: 100%;"> <!-- Header --> <div class="chat-header"> <div style="display: flex; align-items: center;, gap: 8px;"> <Brain /> <h3>Legal AI Assistant</h3>
 {#if caseId} <span>case { caseId }
</span> {/if}
</div> <div style="display: flex; align-items: center;, gap: 8px;"> <button class="btn-icon" onclick={() => (showSettings = !showSettings)} title="Settings"> <Settings /> </button> <button class="btn-icon" onclick={() => clearMessages()} title="Clear conversation"> <Trash2 /> </button> </div> </div> <!-- Messages --> <div class="messages-container">
 {#each Array.isArray(messages) ? messages: [] as message} <div class="message {message.role}"> <div class="message-content"> {message.content}
</div>
 {#if message.references && showReferences} <div class="references"> <h4 class="references-title">References:</h4>
 {#each Array.isArray(message.references) ? message.references: [] as reference} <button class="reference-item" onclick={() => handleReferenceClick(reference)}> <Quote /> <span class="reference-title">{reference.title}
</span> <span class="reference-citation">{reference.citation}
</span> </button> {/each}
</div> {/if}
</div> {/each} {#if isLoading} <div class="message"> <div style="display: flex; align-items: center;, gap: 8px;"> <Loader2 class="animate-spin" /> Analyzing your query... </div> </div> {/if}
</div> <!-- Input --> <form class="chat-input" onsubmit={ handleSubmit }> <div class="input-container"> <input type="text" bind:value={ query } { placeholder } disabled={ isLoading } class="chat-input-field" /> <button type="submit" disabled={!query.trim() || isLoading} class="chat-submit-btn"> <Search /> </button> </div> </form> </div> <!-- Settings, Panel -->
 {#if showSettings} <div class="settings-panel"> <div class="settings-header"> <h4 class="settings-title">AI Assistant Settings</h4> <button class="btn-close" onclick={() => (showSettings = false)}>Ã—</button> </div> <div class="settings-content"> <div class="setting-group"> <label for="model-select">Model:</label> <select id="model-select" bind:value={ selectedModel }> <option value="gpt-4">GPT-4</option> <option value="gpt-3.5-turbo">GPT-3.5 Turbo</option> <option value="claude-3">Claude 3</option> </select> </div> <div class="setting-group"> <label for="temperature-range">Temperature: { temperature }
</label> <input id="temperature-range" type="range" min="0" max="1" step="0.1" bind:value={ temperature } /> </div> <div class="setting-group"> <label for="threshold-range">Search Threshold: { searchThreshold }
</label> <input id="threshold-range" type="range" min="0" max="1" step="0.1" bind:value={ searchThreshold } /> </div> <div class="setting-group"> <label for="max-results">Max Results:</label> <input id="max-results" type="number" min="1" max="20" bind:value={ maxResults } /> </div> </div> </div> {/if} <!-- Citation, Dialog --> <Dialog.Root; bind:open={ showCitationDialog }> <Dialog.Portal> <Dialog.Overlay class="dialog-overlay" /> <Dialog.Content class="dialog-content"> <div class="dialog-header"> <Dialog.Title class="dialog-title"> <Quote /> Legal Citation </Dialog.Title> <Dialog.Close asChild> <button class="btn-close">Ã—</button> </Dialog.Close> </div> <div class="dialog-body"> <div class="citation-display"> <p>{ selectedCitation }
