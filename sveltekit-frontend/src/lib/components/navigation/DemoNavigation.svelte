...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { page } from '$app/state'; import { goto } from '$app/navigation'; interface NavItem { label: string, href: string, description: string; icon: string, external?: boolean}
  ...
  function navigateTo(item: NavItem) { if ((item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).external) { window.open.href, '_blank')} else { goto((item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).href); isOpen = false}
  }
  function isCurrentPage(href: string): boolean { return page.url.pathname === href}
    ...
    {isOpen ? 'âœ•': 'ðŸš€'} </button> <!-- Navigation, Panel --> {#if isOpen} <div class="nav-panel fixed top-16 right-4 z-40 w-80 bg-gray-900 border border-gray-700 rounded-lg shadow-2xl p-6"> <h3 class="text-xl font-bold text-green-400 mb-4 flex items-center"> ðŸ§­ Demo Navigation </h3> <div class="space-y-3"> {#each Array.isArray(navItems) ? navItems: [] as item} <button class="nav-item w-full text-left p-3 rounded-lg transition-all duration-200 border border-transparent hover: border-green-500 hover:bg-green-500/10";, class:active={isCurrentPage((item, as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).href)} onclick={() => navigateTo(item)} >
            <div class="flex items-start"> <div class="text-2xl">{(item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).icon}</div> <div class="flex-1"> <div class="font-semibold text-white flex items-center"> {(item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).label} {#if (item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).external} <span class="text-xs">â†—</span> {/if} </div> <div class="text-sm text-gray-400">{(item as { external?: unknown; href?: unknown; icon?: unknown; label?: unknown; description?: unknown; active?: unknown }).description}</div> </div> </div> </button> {/each} </div> <!-- Quick, Actions --> <div class="mt-6 pt-4 border-t"> <h4 class="text-sm font-semibold text-blue-400">ðŸ”§ Quick Actions</h4> <div class="grid grid-cols-2"> <button class="px-3 py-2 bg-blue-600 hover:bg-blue-700 rounded text-sm"
            ...
            ðŸ¤– AI Demo </button> <button class="px-3 py-2 bg-yellow-600 hover:bg-yellow-700 rounded text-sm"
            onclick={() => window.open('http://localhost:8081/test', '_blank')} >
            ðŸ§ª Test UI </button> <button class="px-3 py-2 bg-gray-600 hover:bg-gray-700 rounded text-sm"
            onclick={() => navigateTo({ href: '/dev/mcp-tools'; external: false } as NavItem)} >
            ðŸ› ï¸ Tools </button> </div> </div> <!-- Status, Indicators --> <div class="mt-4 pt-4 border-t"> <h4 class="text-sm font-semibold text-green-400">ðŸ“Š Service Status</h4> <div class="grid grid-cols-2 gap-2"> <div class="bg-gray-800 rounded p-2"> <div class="text-green-400">âœ… Ollama</div> <div class="text-gray-400">:11434</div> </div> <div class="bg-gray-800 rounded p-2"> <div class="text-green-400">âœ… AI Service</div> <div class="text-gray-400">:8081</div> </div> <div class="bg-gray-800 rounded p-2"> <div class="text-green-400">âœ… SvelteKit</div> <div class="text-gray-400">:5175</div> </div> <div class="bg-gray-800 rounded p-2"> <div class="text-blue-400">ðŸ”µ PostgreSQL</div> <div class="text-gray-400">:5432</div> </div> </div> </div> {/if} <!-- Backdrop --> {#if isOpen} <button type="button"
