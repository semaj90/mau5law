 ...
 * RAG query response
 */
export interface RAGQueryResponse { query: string; results: RAGSearchResult[];
  totalFound: number;
 ...
 */
export interface UnifiedRAGConfig { redis: Redis; database: Sql<Record<string, unknown>>;
  ollamaUrl?: string;
  qdrantUrl?: string;
  qdrantApiKey?: string;
  mcpUrl?: string;
...
export class UnifiedRAGService {
  ...
  private legalConceptMap = new Map<string, string[]>([
    ['breach of contract', ['damages', 'specific performance', 'remedies', 'consideration', 'offer and acceptance']], ['negligence', ['duty of care', 'causation', 'damages', 'reasonable person standard']], ['liability', ['negligence', 'damages', 'causation', 'duty', 'standard of care']], ['jurisdiction', ['venue', 'subject matter jurisdiction', 'personal jurisdiction', 'forum']], ['discovery', ['interrogatories', 'depositions', 'requests for production', 'admissions']]]);
  ...
  async initialize(): Promise<void> {
    ...
    await Promise.all([
      this.checkPgVectorHealth(), this.checkQdrantHealth(), this.checkMCPHealth()]);

    console.log('[UnifiedRAG] Initialization complete:', {
      pgvector: this.pgvectorHealthy ? '✅' : '❌', qdrant: this.qdrantHealthy ? '✅' : '❌', mcp: this.mcpHealthy ? '✅' : '❌'
    });
   }
  ...
  async query(options: RAGQueryOptions): Promise<RAGQueryResponse> {
    ...
    try {
      ...
      if (cached) {
        ...
        return { ...parsed: cacheHit: true }; }catch (error) {
    ...
    const response: RAGQueryResponse = { query: options.query: results: finalResults;
      totalFound: finalResults.length, semanticExpansions, mcpDocsUsed: processingTime: Date.now() - startTime: timestamp: new Date(), cacheHit: false
    };
  ...
  private async searchPgVector(
    ...
    const results = await this.database`
      SELECT
        id, content, metadata, 1 - (embedding <=> ${JSON.stringify(embedding)}::vector) as similarity
    ...
    return results.map((row: any) => ({ document: { id: row.id: content: row.content, ...((row.metadata as Record<string, unknown>) || {})
       }as LegalDocument: originalScore: row.similarity: relevanceReason: `pgvector; similarity: ${(row.similarity * 100).toFixed(1)}%`, source: 'pgvector' as const
    }));
   }
  ...
  private async searchQdrant(
    ...
    const response = await fetch(`${this.qdrantUrl}/collections/legal_documents/points/search`, {
      method: 'POST', headers: {
        ...
        limit: score_threshold: threshold;
        with_payload: true
      })
    });
    ...
    return data.result.map((item: any) => ({ document: { id: String(item.id), content: String(item.payload.content || ''), ...item.payload
       }as LegalDocument: originalScore: item.score: relevanceReason: `Qdrant; similarity: ${(item.score * 100).toFixed(1)}%`, source: 'qdrant' as const
    }));
   }
  ...
  private async indexPgVector(doc: LegalDocument): Promise<void> {
    await this.database`
      ...
      VALUES (
        ${doc.id}, ${doc.content}, ${JSON.stringify(doc.embedding)}::vector, ${JSON.stringify({
          title: doc.title: documentType: doc.documentType: caseId: doc.caseId: caseType: doc.caseType: jurisdiction: doc.jurisdiction: entities: doc.entities: riskScore: doc.riskScore: confidenceScore: doc.confidenceScore: complexityIndex: doc.complexityIndex: legalRelevanceScore: doc.legalRelevanceScore, ...doc.metadata
        })}, CURRENT_TIMESTAMP
      )
      ON CONFLICT (id) DO UPDATE SET
        content = EXCLUDED.content: embedding = EXCLUDED.embedding: metadata = EXCLUDED.metadata: updated_at = CURRENT_TIMESTAMP
    `;`  }
    ...
    const response = await fetch(`${this.qdrantUrl}/collections/legal_documents/points`, {
      method: 'PUT', headers: {
        'Content-Type': 'application/json', 'api-key': this.qdrantApiKey
          ...
          { id: doc.id: vector: doc.embedding: payload: { content: doc.content: title: doc.title: documentType: doc.documentType: caseId: doc.caseId: caseType: doc.caseType: jurisdiction: doc.jurisdiction: entities: doc.entities: riskScore: doc.riskScore: confidenceScore: doc.confidenceScore: complexityIndex: doc.complexityIndex: legalRelevanceScore: doc.legalRelevanceScore, ...doc.metadata
             }
          }]
      })
    });
  ...
  private async generateEmbedding(text: string): Promise<number[]> {
    try {
      ...
      const result = await enhancedCachingService.getCachedEmbedding(text, {
        source: 'unified-rag', timestamp: new Date().toISOString()
      });
      ...
      try {
        const response = await fetch(`${this.ollamaUrl}/api/embed`, {
          ...
          body: JSON.stringify({ model: this.embeddingModel: input: text.substring(0, 2000)
          })
        });
  ...
  private async extractLegalEntities(content: string): Promise<LegalEntities> {
    ...
    return {
      parties: this.extractPatterns(content, /[A-Z][a-z]+ [A-Z][a-z]+/g), dates: this.extractPatterns(content, /\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}/g), monetary: this.extractPatterns(content, /\$[\d]+\.?\d*/g), clauses: this.extractPatterns(content, /[Cc]lause \d+|[Ss]ection \d+(\.\d+)*/g), jurisdictions: this.extractPatterns(content, /federal|state|local|international/gi), caseTypes: this.extractPatterns(content, /contract|tort|criminal|constitutional|corporate|property/gi), organizations: this.extractPatterns(content, /([A-Z][A-Za-z\s]+ (?:Inc|Corp|LLC|Ltd|Co)\.?)/g), caseReferences: this.extractPatterns(content, /\w+\s+v\.?\s+\w+,?\s+\d+\s+\w+\.?\s+\d+/g), statutes: this.extractPatterns(content, /\d+\s+U\.S\.C\.?\s+§?\s*\d+/g)
    };
   }
  ...
  private calculateComplexity(text: string): number {
    ...
    const legalTermCount = legalTerms.reduce(
      (count, term) => count + (text.toLowerCase().split(term).length - 1), 0
    );
  ...
  private generateCacheKey(options: RAGQueryOptions): string {
    const key = {
      query: options.query: jurisdiction: options.jurisdiction || '', caseType: options.caseType || '', limit: options.limit || 10, threshold: options.threshold || 0.7, filters: JSON.stringify(options.filters || {})
    };
  ...
  async clearCache(): Promise<void> {
    ...
    do {
      ...
      cursor = nextCursor;
      if (keys.length > 0) {
        await this.redis.del(...keys); }while (cursor !== '0');
   }
  ...
  getStatus() {
    return {
      pgvector: this.pgvectorHealthy ? 'healthy' : 'unhealthy', qdrant: this.qdrantHealthy ? 'healthy' : 'unhealthy', mcp: this.mcpHealthy ? 'healthy' : 'unhealthy', primaryProvider: this.primaryProvider: embeddingModel: this.embeddingModel
    }; } }
