<script, lang="ts"> interface YoRHaHarvardCardProps { variant?: 'default' | 'gaming' | 'terminal' | 'legal' | 'academic' | 'grey' | 'crimson-grey'; title?: string; subtitle?: string; glowing?: boolean; scanLines?: boolean; bordered?: boolean; children?: any; onclick?: () => void; }
  let { variant = 'default', title, subtitle, glowing = false, scanLines = false, bordered = false, children, onclick, ...restProps }: YoRHaHarvardCardProps = $props(); let cardClasses = $derived( [
      ...
      .filter(Boolean) .join(' ') ); </script> <div class={ cardClasses } { onclick } role={onclick ? 'button': undefined} tabindex={onclick ? 0: undefined} onkeydown={onclick ? e => { if (e.code === 'Enter' || e.code === 'Space') { e.preventDefault(); onclick(); }
      }: undefined} {...restProps} >
  {#if variant === 'gaming'} <div, class="gaming-header-accent">{/if} {#if variant === 'terminal'} <div, class="terminal-header"> <div, class="terminal-controls"> <span class="terminal-dot, terminal-dot--red"></span> <span class="terminal-dot, terminal-dot--yellow"></span> <span class="terminal-dot, terminal-dot--green"></span> </div> <span, class="terminal-title">YORHA_HARVARD_TERMINAL</span> {/if} {#if title || subtitle} <div, class="card-header"> {#if title} <h3, class="card-title">{ title }</h3> {/if} {#if subtitle} <p, class="card-subtitle">{ subtitle }</p> {/if} {/if} <div, class="card-content"> <slot /> </div> {#if variant === 'academic'} <div, class="academic-footer"> <span, class="harvard-shield">⚔</span> <span, class="academic-motto">VERITAS • GAMING • AI</span> {/if} </div> <style> .yorha-harvard-card { background: var(--enhanced-bg-secondary); border: 1px solid var(--enhanced-border); border-radius: 8px; padding: 1.5rem; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1); position: relative; overflow: hidden; }
