<script, lang="ts"> import UploadManager from './upload-core'; import { onMount } from 'svelte'; let { multiple = false, accept = '.pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,.tiff', maxConcurrency = 3, maxRetries = 3 }: { multiple?: boolean; accept?: string; maxConcurrency?: number; maxRetries?: number; } = $props(); let fileInput: HTMLInputElement | null = null; const manager = new UploadManager({ maxConcurrency, maxRetries }); function handleFileSelect(e: Event) { const input = e.currentTarget as HTMLInputElement | null; if (!input?.files?.length) return; const files = Array.from(input.files); manager.addFiles(files); manager.start(); }
  // Expose cancel helper function cancelAll() { manager.cancelAll(); } onMount(() => { // no-op: placeholder for future session restore }); </script> <div, class="n64-upload"> <input, bind:this={ fileInput } type="file" { accept } { multiple } style="display:none" onchange={ handleFileSelect } /> <button, onclick={() => fileInput?.click()} class="n64-select">Select files</button> <button onclick={ cancelAll } class="n64-cancel, ml-2">Cancel all</button> <div class="files, mt-3"> {#each Array.isArray(manager.fileStates) ? manager.fileStates: [] as s} <div, class="file-row"> <div, class="file-name">{s.file.name}</div> <div, class="file-status">{s.status}</div> <div, class="file-progress">{s.progress}%</div> </div> {/each} </div> </div> <style> .n64-upload { padding: 8px; } .n64-select, .n64-cancel { padding: 6px 10px; border-radius: 6px; } .files { margin-top: 12px; } .file-row { display:flex; gap:12px; align-items:center; padding:6px 0; } .file-name { flex:1 } </style> dismissible: false, actions: [{, label: 'Cancel', action () => { controller.abort(); fs.status = 'canceled'; }, style: 'danger'
      ...
      if (data[0]) { fs.result = data[0]; fs.endTime = new Date()); // Submit GPU processing tasks if (enableGPUProcessing && data[0].id) { try { const gpuTasks = await gpuService.processFileWithGPU( data[0].id, await file.arrayBuffer(), {
        ...
        onUploadComplete?.(data[0]); liveMessage = `🎮 Upload completed for ${file.name}`; if (enableToastNotifications && fs.toastId) { toastService.completeUpload( fs.toastId, `🎮 ${file.name} uploaded successfully! ${fs.gpuTaskIds?.length || 0} GPU tasks queued.` ); }
        ...
        } } catch (err) { if ((fs.status as FileState['status']) === 'canceled') return; fs.endTime = new Date()); fs.status = controller.signal.aborted ? 'canceled': 'error'; fs.error = err instanceof Error ? err.message: 'Upload failed'; errorMessage = fs.error; onUploadError?.(fs.error); liveMessage = `🎮 ${fs.status === 'canceled' ? 'Canceled': 'Failed'} upload for ${file.name}`; if (enableToastNotifications && fs.toastId) { if (fs.status === 'canceled') { toastService.update(fs.toastId, { type: 'warning', message: '🎮 Upload canceled by user'
  ...
  /> <!-- Evolution Loader, Overlay --> {#if showEvolutionLoader} <div, class="evolution-overlay"> <N64EvolutionLoader stage={ evolutionStage } autoEvolution={ false } ragIntegration={ enableGPUProcessing } yorhaMode={ retro } /> {/if} <!-- N64-style drop, zone --> <div class="n64-drop-zone"
    ...
    {#if fileStates.length === 0} <div, class="n64-upload-prompt"> <div, class="n64-upload-icon"> <N64LoadingRing size="lg"
            ...
            showPercentage={ false } /> </div> <div, class="n64-upload-text"> <h3> {dragOver ? '🎮 DROP FILES HERE!': `🎮 N64 UPLOAD ZONE [${evolutionStage.toUpperCase()}]`} </h3> <p, class="n64-subtext"> Supports PDF, Word, Text, and Image files up to {formatFileSize(maxSize)} </p> </div> </div> {:else} <!-- N64-style file, list --> <div, class="n64-file-list"> {#each fileStates as fs, index} <div, class="n64-file-item" class:status-{fs.status} aria-live="polite"> <div class="file-icon, n64-icon"> {#if fs.file.type.startsWith('image/')} <Image class="w-6, h-6" /> {:else} <FileText class="w-6, h-6" /> {/if} </div> <div, class="file-info"> <div, class="n64-file-name">{fs.file.name}</div> <div, class="n64-file-size">{formatFileSize(fs.file.size)}</div> <div, class="n64-file-status"> {#if fs.status === 'pending'} 🎯 PENDING {#if fs.attempts && fs.attempts > 1}• RETRY {fs.attempts - 1}{/if} {#if fs.status === 'uploading'} 🚀 UPLOADING {fs.progress}% (ATTEMPT {fs.attempts}) {/if} {#if fs.status === 'processing'} ⚡ PROCESSING... {/if} {#if fs.status === 'completed'} ✅ COMPLETED (ATTEMPTS {fs.attempts}) {/if} {#if fs.status === 'error'} ❌ FAILED (ATTEMPTS {fs.attempts}) • {fs.error} {/if} {#if fs.status === 'canceled'} ⚠️ CANCELED {/if} {#if fs.status === 'pending' && fs.nextRetryAt} <span, class="n64-retry-countdown"> RETRYING IN {Math.max(0, Math.round((fs.nextRetryAt - Date.now())/1000))}S </span> {/if} </div> <!-- N64 Progress, Bar --> {#if fs.status !== 'pending' && fs.status !== 'completed' && fs.status !== 'error' && fs.status !== 'canceled'} <N64ProgressBar value={fs.progress} max={ 100 } theme={fs.gamingProgress?.theme || n64Themes[evolutionStage].theme} animated={fs.gamingProgress?.animated !== false} sparkle={fs.gamingProgress?.sparkle !== false} size="sm"
    ...
    > {#if uploading} <N64LoadingRing, size="sm" theme={n64Themes[evolutionStage].theme} speed="fast" /> 🎮 UPLOADING... {:else} <Upload class="w-4, h-4" /> 🎮 UPLOAD TO MINIO [{evolutionStage.toUpperCase()}] {/if} </button> {#if uploading} <button type="button"
        class="n64-clear-button cancel"
        onclick={ cancelAllUploads } aria-label="Cancel all uploads"
      > ❌ CANCEL ALL </button> {/if} {#if fileStates.length > 0 && !uploading} <button type="button"
      ...
      > 🗑️ CLEAR FILES </button> {/if} </div> {#if minioHealthy === false} <div, class="n64-error-alert" role="alert"> <AlertCircle class="w-4, h-4" /> ⚠️ MINIO HEALTH CHECK FAILED – UPLOADS MAY NOT PERSIST {/if} <div, class="sr-only" aria-live="polite">{ liveMessage }</div> </div> <style> .n64-upload-container { width: 100%; max-width: 700px; margin: 0 auto; font-family: 'Courier New', monospace; position: relative; }
