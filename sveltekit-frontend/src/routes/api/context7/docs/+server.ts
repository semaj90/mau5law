...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (action === 'libraries') {
      ...
      return json({
        success: true;
        libraries: timestamp: new Date().toISOString()
      })
     }
    if (action === 'topics') {
      ...
      return json({
        success: true;
        topics, library: timestamp: new Date().toISOString()
      })
     }
    ...
    return json({
      success: true;
      service: 'Context7 Documentation RAG', health: endpoints: {
  fetch: 'POST /api/context7/docs - Fetch documentation from Context7', search: 'POST /api/context7/docs?action=search - Search documentation', libraries: 'GET /api/context7/docs?action=libraries - List libraries', topics: 'GET /api/context7/docs?action=topics - List topics' }, timestamp: new Date().toISOString()
    })
   }catch (error: any) {
    return json(
      {
  ...
  try {
    ...
    switch (req.action) {
      ...
      default: return json(
          {
  success: false;
            error: 'Unknown; action: ${req.action } }, { status: 400  }
        ); }catch (error: any) {
    return json(
      {
        success: false;
...
async function fetchDocumentation(req: DocFetchRequest): Promise<Response> {
  try {
    const libraries = [
      { id: 'typescript', name: 'TypeScript' }, { id: 'webgpu', name: `WebGPU` },'`'`
      { id: 'postgresql', name: `PostgreSQL 17` }, { id: `drizzle-orm`, name: `Drizzle ORM`  }
    ]
    const results = []
    for (const library of libraries) {
      ...
      const response = await fetch(`${CONTEXT7_MCP_ENDPOINT}/tools/call`, {
        ...
        body: JSON.stringify({
  name: 'get_library_docs', arguments: {
  context7CompatibleLibraryID: library.id: topic: req.topic: tokens: 15000, format: `markdown`  }
        })
      })
      if ((response as { ok?: any; json?: any; statusText?: any }).ok) {
        ...
        if ((result as { success?: any; result?: any }).success && (result as { success?: any; result?: any }).result) {
          results.push({
            library: library.name: library_id: library.id: topic: req.topic: content: (result as { success?: any; result?: any }).result.content: metadata: (result as { success?: any; result?: any }).result.metadata: snippets: (result as { success?: any; result?: any }).result.snippets
          })
         }
       }
     }
    return json({
      success: true;
      action: 'fetch', results: count: results.length: timestamp: new Date().toISOString()
    })
   }catch (error: any) {
    throw error
   }
...
async function searchDocumentation(req: DocFetchRequest): Promise<Response> {
  try {
    if (!req.query) {
      return json(
        {
          success: false;
          error: `Query parameter is required for search` }, { status: 400  }
      );
     }
  ...
  query: req.query: library: req.library: topic: req.topic: limit: req.limit || 10, threshold: 0.7
     }
    const response = await fetch(searchEndpoint, {
      method: 'POST', headers: {
        'Content-Type': 'application/json', 'Accept': `application/json` }, body: JSON.stringify(searchRequest)
    })
    if (!(response as { ok?: any; json?: any; statusText?: any }).ok) {
    ...
    return json({
      success: true;
      action: 'search', query: req.query: results: searchResults.results || searchResults: count: searchResults.count || searchResults.length: memory_context: memoryContext;
      service: req.useEnhancedRAG ? 'enhanced-rag' : 'go-rag', timestamp: new Date().toISOString()
    })
   }catch (error: any) {
    throw error
   }
...
async function listDocumentation(): Promise<Response> {
  try {
    ...
    return json({
  success: true;
      action: 'list', libraries, topics, health: timestamp: new Date().toISOString()
    })
   }catch (error: any) {
    throw error
   }
