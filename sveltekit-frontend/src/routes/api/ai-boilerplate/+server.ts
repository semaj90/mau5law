...
interface RawHighPerformingPhraseRow {
  phrase: string | null;
  avg_prosecution_score: number | null;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    console.log(`📄 boilerplate: ${validatedRequest.type}`);
...
async function getHighPerformingPhrases(
  ...
  const cachedResult =
    await legalAIResultCache.getCachedLegalResults<HighPerformingPhrase[]>(cacheKey);

  if (cachedResult) {
    console.log(`CACHE HIT for phrases: ${type}`);
    return cachedResult;
  }
  console.log(`CACHE MISS for phrases: ${type}`);

  const db = getDB();

  // note: The join condition `... ILIKE: '%' || spr.phrase || '%'` is inefficient
  ...
  const result = await db<RawHighPerformingPhraseRow[]>`
		SELECT DISTINCT spr.phrase, spr.avg_prosecution_score, spr.frequency, spr.correlation_strength, COUNT(ldp.id) AS usage_count
		FROM semantic_phrases_ranking spr
		JOIN legal_documents_processed ldp ON ldp.semantic_phrases::text ILIKE '%' || spr.phrase || '%'
		...
		ORDER BY spr.avg_prosecution_score DESC, usage_count DESC, spr.correlation_strength DESC
		LIMIT ${CONFIG.boilerplate.maxTemplates}
	`;

  const phrases = result.map((row) => ({
    phrase: row.phrase ?? '',
    avg_prosecution_score: Number(row.avg_prosecution_score ?? 0),
    frequency: Number(row.frequency ?? 0),
    correlation_strength: Number(row.correlation_strength ?? 0),
...
async function generateBoilerplate(
  ...
  const avgProsecutionScore =
    sourcePhrases.reduce((sum, p) => sum + p.avg_prosecution_score, 0) /
...
${contextPrompt}
Based on these high-performing legal phrases that have shown correlation:
${phraseText}
...
Generate text:`;
  ...
  try {
    ...
    return { text: generatedText, confidence, prosecutionStrength: avgProsecutionScore };
...
function buildSystemPrompt(type: BoilerplateType, tone: Tone): string {
  ...
  const typePrompts: Record<BoilerplateType, string> = {
    ...
    legal_motion:
      'Draft a professional legal motion with proper formatting and persuasive legal reasoning.',
    case_analysis:
      'Provide a thorough case analysis examining legal precedents, evidence, and potential outcomes.',
    sentencing_memo:
  ...
  const toneAdjustments: Record<Tone, string> = {
    ...
    aggressive:
      "Use strong, assertive language that emphasizes the strength of the prosecution's case.",
    neutral:
      'Use objective, fact-based language that presents information clearly and impartially.',
...
function buildContextPrompt(context?: BoilerplateRequestContext): string {
  if (!context) return '';
  let contextPrompt = 'Context document:\n';
  ...
  if (context.evidence_types && context.evidence_types.length > 0) {
    contextPrompt += `- Types: ${context.evidence_types.join(', ')}\n`;
  }
  if (context.precedents && context.precedents.length > 0) {
    contextPrompt += `- Precedents: ${context.precedents.join(', ')}\n`;
  }
  if (context.custom_context) {
    contextPrompt += `- Context: ${context.custom_context}\n`;
  }
...
function generateFallbackBoilerplate(type: BoilerplateType, sourcePhrases: HighPerformingPhrase[]) {
  const templates: Partial<Record<BoilerplateType, string>> = {
    ...
    legal_motion: `The prosecution respectfully moves the court for relief based on the grounds: ${sourcePhrases
      .slice(0, 3)
      .map((p) => p.phrase)
  ...
  const fallbackText =
    templates[type] ||
    `The prosecution presents the following legal argument incorporating elements: ${sourcePhrases
      .slice(0, 5)
      .map((p) => p.phrase)
      .join(', ')}.`;
  return { text: fallbackText, confidence: 0.6, prosecutionStrength: 75 };
}
...
async function generateSuggestedEdits(text: string, type: BoilerplateType): Promise<string[]> {
  ...
  if (!text.toLowerCase().includes('precedent') && type === 'legal_motion') {
  ...
  if (!text.toLowerCase().includes('defendant')) {
  ...
  const typeSpecificSuggestions: Partial<Record<BoilerplateType, string[]>> = {
    ...
    evidence_summary: [
      ...
      'Highlight most compelling evidence first',
    ],
