<!--
  N64-Style Gaming Button Component
  Nintendo 64 inspired button with authentic retro styling and legal AI integration
-->
<script lang="ts">
  ...
  interface Props {
    ...
    evidenceAction?: 'analyze' | 'classify' | 'correlate' | 'submit';
    confidence?: number;
      ...
      {#if evidenceAction}
        <div class="n64-button__evidence-icon" title="Evidence Action: {evidenceAction}">
          ...
          {:else if evidenceAction === 'classify'}
            📋
          {:else if evidenceAction === 'correlate'}
            🔗
