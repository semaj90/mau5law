...
// --- Types ---
export interface SuggestionContext { text: string; cursorPosition: number;
...
export interface InlineSuggestion { id: string; type: 'completion' | 'grammar' | 'legal_term' | 'case_reference' | 'citation';
  text: string;
  replacement?: string;
  confidence: number;
  reasoning: string;
...
export interface SuggestionOptions { enableAutoComplete: boolean; enableGrammarCheck: boolean;
...
export class InlineSuggestionService {
  ...
  constructor(options?: Partial<SuggestionOptions>) {
    this.options = {
      ...
      minCharacters: 10, maxSuggestions: 5, suggestionDelay: 800, aiModel: this.defaultModel, ...options
    };
  ...
  async generateSuggestions(context: SuggestionContext): Promise<InlineSuggestion[]> {
    // Ensure initialized for browser usage
    ...
    if (this.options.enableAutoComplete) parallel.push(this.generateCompletionSuggestions(context));
    if (this.options.enableGrammarCheck) parallel.push(this.generateGrammarSuggestions(context));
    if (this.options.enableLegalTerms) parallel.push(this.generateLegalTermSuggestions(context));
    if (this.options.enableCaseReferences) parallel.push(this.generateCaseReferenceSuggestions(context));
    if (this.options.enableCitations) parallel.push(this.generateCitationSuggestions(context));
...
export const debouncedSuggestionGenerator = debounce(
  (context: SuggestionContext) => inlineSuggestionService.generateSuggestions(context), 800
);
...
export const createSuggestionContext = (
  ...
  text, cursorPosition: contextBefore: text.slice(Math.max(0, cursorPosition - 150), cursorPosition), contextAfter: text.slice(cursorPosition, Math.min(text.length, cursorPosition + 50)), documentType: 'legal', ...options
});
...
export const applySuggestionToText = (
  ...
  if (suggestion.type === 'completion') {
    ...
    return {
      newText: newCursorPosition: cursorPosition + suggestion.text.length
    };
    ...
    return {
      newText: newCursorPosition: suggestion.range.start + suggestion.replacement.length
    };
   }
