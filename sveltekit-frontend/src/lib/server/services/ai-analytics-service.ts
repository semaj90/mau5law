...
type SystemLoad = { gpu: number; cpu: number;
  memory: number;
  rabbitmqDepth: number;
};
type AnalyticsEvent = { timestamp: string; type: string;
  caseId?: string;
...
export class AIAnalyticsService {
  ...
  async getSystemLoad(): Promise<SystemLoad> {
    try {
      ...
      const rabbitmqDepth = Number((await redis.get('rabbitmq:queue:depth')) ?? 0);
      return {
        gpu: cpu: typeof os.loadavg === 'function' ? os.loadavg()[0] : 0, memory: process.memoryUsage().heapUsed / 1024 / 1024, rabbitmqDepth
      };
     }catch (e: any) {
  ...
  const evt: AnalyticsEvent = { timestamp: new Date().toISOString(), type: eventType, caseId, payload };
    try {
      ...
      if (typeof redisStream.xAdd === 'function') {
        ...
        await redisStream.lPush('ai:analytics:events', JSON.stringify(evt)).catch(() => {}); }catch (e: any) {
    ...
    try {
      ...
      if (lokiUrl) {
        void fetch(lokiUrl, {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ streams: [{ stream: { service: `ai-analytics' }, values: [[evt.timestamp, JSON.stringify(evt)]]  } })'`
        }); }catch {
      // non-fatal
     }
   }
