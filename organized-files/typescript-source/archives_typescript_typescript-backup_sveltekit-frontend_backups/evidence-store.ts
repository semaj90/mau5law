...
export const evidenceActions = {
  ...
  async loadEvidence(caseId?: string) {
    evidenceGrid.update(state => ({ ...state, isLoading: true, error: undefined }));
...
export const uploadActions = {
  ...
  // Upload files
  async uploadFiles() {
    uploadModal.update(state => ({ ...state, isProcessing: true, error: undefined }));
    ...
    try {
      ...
      uploadModal.update(modalState => ({ ...modalState, isProcessing: false }));
    } catch (error: any) {
      uploadModal.update(state => ({
        ...state,
        isProcessing: false,
