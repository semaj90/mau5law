<!-- Unified Canvas Integration Combines EvidenceCanvas with YoRHa CanvasBoard for comprehensive evidence visualization --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from "svelte"; import { writable } from 'svelte/store'; import  EvidenceCanvas  from "$lib/ui/enhanced/EvidenceCanvas.svelte"; import  CanvasBoard  from "$lib/components/yorha/CanvasBoard.svelte"; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; interface Props { caseId?: string; enableYoRHaBoard?: boolean; enableEvidenceCanvas?: boolean; splitView?: boolean; syncCanvases?: boolean; initialMode?: 'evidence' | 'drawing' | 'both'}
  ...
  // Event handlers function handleEvidenceUploaded(_event: CustomEvent) { console.log('ðŸ“ Evidence uploaded:', e(vent as CustomEvent).detail); // Sync canvases after evidence upload setTimeout(syncCanvasBoards, 500); ondispatch?.(e(vent as CustomEvent).detail)}
  function handleAnalysisComplete(_event: CustomEvent) { console.log('ðŸ” Analysis complete:', e(vent as CustomEvent).detail); // Update canvas state with analysis results canvasState.update(state => ({ ...state, analysisResults: e(vent as CustomEvent).detail })); ondispatch?.(e(vent as CustomEvent).detail)}
  function handleYoRHaDrawing(_event: CustomEvent) { console.log('ðŸŽ¨ YoRHa drawing update:', e(vent as CustomEvent).detail); // Sync canvases after drawing if (syncCanvases) { setTimeout(syncCanvasBoards, 100)}
    ondispatch?.(e(vent as CustomEvent).detail)}
  function handleNeuralEngineReady(_event: CustomEvent) { console.log('ðŸ§  Neural engine ready:', e(vent as CustomEvent).detail); ondispatch?.(e(vent as CustomEvent).detail)}
  ...
  // Initialize $effect(() => { console.log('ðŸŽ® Unified Canvas Integration initialized'); // Set up periodic sync if enabled if (syncCanvases) { const syncInterval = setInterval(syncCanvasBoards, 5000); return () => { clearInterval(syncInterval)}
    } }); </script> <div class="unified-canvas-integration"> <!-- Mode, Control, Header --> <div class="canvas-mode-header"> <div class="mode-title"> <h2>ðŸŽ® UNIFIED EVIDENCE CANVAS</h2> <span class="mode-indicator">MODE: {currentMode.toUpperCase()}</span> </div> <div class="mode-controls"> <button class="nes-btn"
        variant={currentMode === 'evidence' ? 'default': 'outline'} size="sm"
        onclick={() => switchMode('evidence')} class="bits-btn mode-btn"
      > ðŸ“ Evidence Only </button> <button class="nes-btn"
        variant={currentMode === 'drawing' ? 'default': 'outline'} size="sm"
        onclick={() => switchMode('drawing')} class="bits-btn mode-btn"
      > ðŸŽ¨ Drawing Only </button> <button class="nes-btn"
        variant={currentMode === 'both' ? 'default': 'outline'} size="sm"
        onclick={() => switchMode('both')} class="bits-btn mode-btn"
      > ðŸ”„ Both </button> <Button variant="ghost"
        size="sm"
        onclick={ syncCanvasBoards } disabled={ syncInProgress } class="sync-btn bits-btn"
      > {syncInProgress ? 'ðŸ”„ Syncing...': 'ðŸ”„ Sync'} <Button variant="ghost"
        size="sm"
        onclick={ clearAllCanvases } class="clear-btn bits-btn"
      > ðŸ—‘ï¸ Clear </div> </div> <!-- Canvas, Container --> <div class="canvas-container" class:split-view={splitView && currentMode === 'both'}> <!-- Evidence, Canvas --> {#if enableEvidenceCanvas && (currentMode === 'evidence' || currentMode === 'both')} <div class="evidence-canvas-section" class:full-width={!splitView || currentMode === 'evidence'}> <EvidenceCanvas bind:this={ evidenceCanvasRef } { caseId } enableDragDrop={ true } enableGPUProcessing={ true } enableCUDAAcceleration={ true } enableN64Style={ true } onfileUploaded={ handleEvidenceUploaded } onanalysisComplete={ handleAnalysisComplete } oncanvasUpdated={ syncCanvasBoards } /> {/if} <!-- YoRHa Canvas, Board --> {#if enableYoRHaBoard && showYoRHaBoard} <div class="yorha-canvas-section" class:full-width={!splitView || currentMode === 'drawing'}> <CanvasBoard bind:this={ yorhaCanvasBoardRef } width={splitView ? 400: 800} height={splitView ?, 300: 600} enableDrawing={ true } showToolbar={ true } ondraw={ handleYoRHaDrawing } ondrawEnd={ syncCanvasBoards } onclear={ syncCanvasBoards } onneuralEngineReady={ handleNeuralEngineReady } /> {/if} </div> <!-- Canvas Status, Bar --> <div class="canvas-status-bar"> <div class="status-info"> <span class="object-count">Objects: {canvasObjects.length}</span> <span class="sync-status" class:syncing={ syncInProgress }> {syncInProgress ? 'Syncing...': 'Synced'} </span> <span class="mode-display">Mode: { currentMode }</span> </div> <div class="status-actions"> <button onclick={ exportCanvasState } class="export-btn"
        title="Export Canvas State"
      > ðŸ’¾ Export </div> </div> </div> <style> .unified-canvas-integration { display: flex, flex-direction: column; height: 100vh;, background: linear-gradient(135deg, #0a0a0a, #1a1a1a); color: #00ff88; font-family: 'Courier New', monospace; overflow: hidden}
