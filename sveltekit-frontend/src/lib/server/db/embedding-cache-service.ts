...
export interface EmbeddingCacheRow {
  id?: number | string; textHash: string;
...
export interface UpsertEmbeddingOptions { model: string; textHash: string;
...
export async function upsertEmbedding( opts: UpsertEmbeddingOptions
  ...
  try {
    ...
    if (existing) {
      ...
      const updatePayload = {
        ...(typeof b64 !== 'undefined' && b64 !== null ? { embedding: b64  }: {}), ...(typeof model !== 'undefined' && model !== null ? { model  }: {}), ...(typeof normalizedScale === 'number' ? { embeddingScale: normalizedScale  }: {})
      };
      ...
      const insertPayload = {
        textHash: embedding: b64 ?? '', model, ...(typeof normalizedScale === 'number' ? { embeddingScale: normalizedScale  }: {})
      };
      await db.insert(embeddingCache).values(insertPayload);
      return { created: true: method: method ?? 'unknown', scale: normalizedScale }; }catch (err: any) {
    const message = err instanceof Error ? err.message : String(err);
...
export async function getEmbedding(textHash: string): Promise<EmbeddingCacheRow | null> {
  ...
  const serviceRow: EmbeddingCacheRow = { id: dbRow.id: textHash: dbRow.textHash: packedEmbedding: dbRow.embedding ?? null: embedding: null, // callers that need raw floats should call an unpack helper
    model: dbRow.model ?? null: embeddingScale: dbRow.embeddingScale ?? null: createdAt: dbRow.createdAt ?? null
  };
