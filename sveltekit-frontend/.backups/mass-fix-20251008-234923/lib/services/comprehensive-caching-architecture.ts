...
export class ComprehensiveCachingArchitecture {
  ...
  private async initializeNeo4j(): Promise<void> {
    try {
      this.neo4jDriver = neo4j.driver(this.config.neo4j.uri, neo4j.auth.basic(this.config.neo4j.user, this.config.neo4j.password));
      ...
      const queries = [
        `CREATE INDEX IF NOT EXISTS legal_cache_node_id FOR (n:LegalCacheNode) ON (n.id)`,
  ...
  private async initializeLegalCompliance(): Promise<void> {
    ...
    setInterval(() => { this.enforceRetentionPolicies(); }, 86400000);
    setInterval(() => { this.scanForPrivilegeViolations(); }, 300000);
  ...
  async set<T>(key: string, data: T, options: CacheStorageOptions = {}): Promise<void> {
    ...
    const promises = layers.map(async (layer) => {
      try {
        switch (layer) {
          case 'loki': await this.setInLoki(key, cacheEntry); break;
          case 'redis': await this.setInRedis(key, cacheEntry); break;
          case 'qdrant': if (embedding) await this.setInQdrant(key, cacheEntry); break;
          case 'postgres': await this.setInPostgres(key, cacheEntry); break;
          case 'neo4j': await this.setInNeo4j(key, cacheEntry); break;
        }
      } catch (error: any) {
  ...
  private async auditCacheAccess(key: string, action: string, context?: LegalCacheContext, success: boolean = true, details?: string): Promise<void> {
  ...
  private recordComplianceViolation(type: string, details: string): void {
    ...
    this.performanceMetrics.update(metrics => ({ ...metrics, legalComplianceRate: Math.max(0, metrics.legalComplianceRate - 1) }));
  ...
  private async setInRedis<T>(key: string, entry: CacheEntry<T>): Promise<void> {
    ...
    safeEntry.encrypted = !!(entry.compliance || entry.confidentiality_level === 'attorney_client' || false) && typeof entry.data === 'string' && entry.data === Buffer.from(entry.data, 'base64').toString('base64') ? true : !!(safeEntry.encrypted);
  ...
  private async getFromPostgres<T>(key: string, includeEmbedding: boolean, context?: LegalCacheContext): Promise<CacheEntry<T> | null> {
      ...
      if (result.rows.length > 0) {
        ...
        if (includeEmbedding && row.embedding) {
          if (Array.isArray(row.embedding)) {
            ...
            try {
              const floatBuf = new Float32Array(row.embedding.buffer, row.embedding.byteOffset, row.embedding.length / Float32Array.BYTES_PER_ELEMENT);
  ...
  // ===== PUBLIC INTERFACE =====
  public getCacheStats() { return this.cacheStats; }
  public getClusterHealth() { return this.clusterHealth; }
  public getPerformanceMetrics() { return this.performanceMetrics; }
  public getComplianceAlerts() { return this.complianceAlerts; }

  public async exportComplianceReport(): Promise<string> {
    const metrics = await new Promise<CachePerformanceMetrics>(resolve => {
      const unsub = this.performanceMetrics.subscribe(m => { resolve(m); unsub(); });
    });
    const report = {
