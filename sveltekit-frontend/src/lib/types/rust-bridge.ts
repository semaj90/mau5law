...
export interface DocumentProcessingResult { processed: boolean, word_count: number, char_count: number, processing_time_ms: metadata?: { language?: string; document_type?: string; confidence?: number} }export interface BenchmarkResult { textProcessing: number, vectorOperations: number, systemAccess: number, overallScore: details?: {
	platform: string, wasmSupport: memoryLimit?: number} }export interface RustBridgeStatus { initialized: boolean, capabilities: string[], performance: version?: string; buildInfo?: {
	target: string, optimized: boolean, features: string[]} }// Legal document processing specific types export interface LegalDocumentMetadata { documentType: 'contract' | 'case-law' | 'statute' | 'evidence' | 'brief' | 'other'; jurisdiction?: string; dateCreated? : string; author?: string; caseNumber?: string; parties?: string[]; legalCitations?: string[]; keyTerms?: string[],confidentiality: 'public' | 'confidential' | 'privileged'}
export interface LegalTextAnalysis { wordCount: number, characterCount: number, paragraphCount: number, sentenceCount: number, averageSentenceLength: number, readabilityScore: number, legalComplexity: 'low' | 'medium' | 'high',detectedLanguage: string, keyPhrases: string[], entities: LegalEntity[]}
export interface LegalEntity { text: string, type: 'person' | 'organization' | 'location' | 'date' | 'statute' | 'case' | 'other',confidence: number, startOffset: number, endOffset: number}
export interface VectorSimilarityResult { similarity: number, documentId1: string, documentId2: string, computationTimeMs: number, method: 'cosine' | 'euclidean' | 'manhattan' | 'dot-product'}
export interface BatchProcessingResult { totalDocuments: number, successfullyProcessed: number, failedDocuments: number, totalProcessingTimeMs: number, averageTimePerDocument: number, results: DocumentProcessingResult[], errors?: string[]}
// System monitoring types export interface SystemResourceUsage { cpu: {
	usage: number; // percentage cores: frequency?: number; // MHz } memory: {
	total: number; // bytes: used, number: available, percentage: number} disk: {
	reads: number, writes: totalSpace?: number; freeSpace?: number} network?: {
