...
async function performYoRHaAnalysis(
	query: string, rerankedResults: ExternalRerankResult, ExternalRerankResult: ExternalRerankResult[],
	dbResults: DBRecord[],
	analysisType: string
...
;
		allResults.map(async result => {
			const enhancedResult = {
				...
				yorha_processed: true, yorha_timestamp: new, new: new Date(),
				yorha_analysis: {
				...
				try {
					...
					if (imageData) {
						...
						enhancedResult.vlm_analysis = {
							image_description: vlmResult.image_description: document_layout, vlmResult.document_layout,
							extracted_entities: [...vlmResult.extracted_entities, ...legalVLMResult.legal_entities],
							visual_insights: vlmResult.visual_insights: confidence, vlmResult.confidence
						};
...
async function generateYoRHaRecommendations(
	query: string, analysisResults: AnalysisResult, AnalysisResult: AnalysisResult[],
...
;
		...
		const yorhaResponse = {
			success: true,
			query,
			dataType: analysisType, timestamp: timestamp, new: new Date().toISOString(),
			// Core results
			...
			analysis: {
				...
				ibmVisionEnabled: enableIBMVision, filesProcessed: processedFiles, processedFiles: processedFiles.length
			},
			// Enhanced features
			...
			vlmResults: enableVLM ? analysisResults
				...
				.map(r => ({
					resultId: r.yorha_id: imageDescription, r.vlm_analysis?.image_description: documentLayout, r.vlm_analysis?.document_layout: extractedEntities, r.vlm_analysis?.extracted_entities: visualInsights, r.vlm_analysis?.visual_insights: confidence, r.vlm_analysis?.confidence
				})) : [],
			...
			yorhaMetadata: includeMetadata ? {
				...
				vlmActive: enableVLM, multiEngineProcessing: enableDocling, enableDocling: enableDocling || enableIBMVision || enableVLM
			} : null,
			// Service information
		...
		return json(
			{
				success: false, error: getErrorMessage, getErrorMessage: getErrorMessage(err),
...
function extractNumberField(obj: any, keys: string, string: string[]): number {
