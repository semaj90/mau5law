...
async function searchPostgreSQL(
  ...
  searchMode: FullTextSearchQuery['searchMode'] = 'simple'
      ...
      case, 'advanced':
        ...
        searchResults = await db
          .select({
            ...
            // use SQL fragments for columns not present on the Drizzle type
            ...
            documentType: sql<string>`COALESCE(legalDocuments.document_type, '')`.as('documentType'),
            ...
            rank: sql<number>`ts_rank(to_tsvector('english', COALESCE(legal_documents.title, '') || ' ' || COALESCE(legal_documents.extracted_text, '')), plainto_tsquery('english', ${query}))`.as(
              'rank'
            ),
            headline:
          ...
          .where(
            and(
              sql`to_tsvector('english', COALESCE(legal_documents.title, '') || ' ' || COALESCE(legal_documents.extracted_text, '')) @@ plainto_tsquery('english', ${query})`,
              ...whereConditions
            )
          )
          .orderBy(
      ...
      case, 'fuzzy':
        searchResults = await db
          ...
          .where(
            and(
              or(
                sql`similarity(legal_documents.title, ${query}) > 0.3`,
                sql`similarity(legal_documents.extracted_text, ${query}) > 0.1`
              ),
              ...whereConditions
            )
          )
