...
export const documentMetadataSchema = z.object({
  ...
  confidentialityLevel: z.enum(['public', 'restricted', 'confidential']).default('restricted'),
...
export const evidence = pgTable(
  'evidence',
  {
    ...
    posX: decimal('pos_x', { precision: 10, scale: 2 }).default('100').notNull(),
    posY: decimal('pos_y', { precision: 10, scale: 2 }).default('100').notNull(),
    width: decimal('width', { precision: 10, scale: 2 }).default('400').notNull(),
    height: decimal('height', { precision: 10, scale: 2 }).default('300').notNull(),
...
export const reports = pgTable('reports', {
  ...
  height: decimal('height', { precision: 10, scale: 2 }).default('450').notNull(),
  zIndex: integer('z_index').default(1).notNull(),
...
export const legalDocuments = pgTable(
  'legal_documents',
  {
    ...
    documentType: varchar('document_type', { length: 50 }).default('brief').notNull(),
    ...
    autoSaveData: jsonb('auto_save_data')
      ...
      .notNull(),
    collaborators: jsonb('collaborators').$type<Collaborator[]>().default([]).notNull(),
...
export const notes = pgTable(
  'notes',
  {
    ...
    relatedEvidenceIds: jsonb('related_evidence_ids').$type<string[]>().default([]).notNull(),
    relatedCriminalIds: jsonb('related_criminal_ids').$type<string[]>().default([]).notNull(),
    // AI features
    aiSummary: text('ai_summary'),
...
export const savedCitations = pgTable(
  'saved_citations',
  {
    id: text('id')
      ...
      .$defaultFn(() => createId()), // Custom CUID2 ID
    userId: uuid('user_id').references(() => users.id, { onDelete: 'cascade' }),
    citationPointId: uuid('citation_point_id').references(() => citationPoints.id, { onDelete: 'cascade' }),
    title: varchar('title', { length: 255 }),
    description: text('description'),
    // Type-safe JSON fields
...
export const userDocuments = pgTable(
  ...
  table => ({
    ...
    embeddingIdx: index('idx_user_documents_embedding').using('ivfflat', table.embedding.op('vector_cosine_ops')),
  })
);
export const userPatterns = pgTable(
  ...
  table => ({
    ...
    embeddingIdx: index('idx_user_patterns_embedding').using('ivfflat', table.embedding.op('vector_cosine_ops')),
  })
);
