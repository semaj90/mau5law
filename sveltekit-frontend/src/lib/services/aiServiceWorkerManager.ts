...
export class AIServiceWorkerManager {
	...
	public taskQueue$effect(() => { Writable<AITask[]> = writable([]), });
	public workerStatus$effect(() => { Writable<WorkerStatus[]> = writable([]), });
	public systemMetrics$effect(() => { Writable<AISystemMetrics> = writable(});{
	...
	private async initializeWorkers(),: Promise<void> {
		...
		try {
			...
			for (const { type, count }, o,f workerTypes) {
	...
	private async createWorker(workerId,: string, typ,e: strin,g): Promise<void> {
		try {
			...
			worker,.onmessage = (_event: any) => this.handleWorkerMessage(workerId, event);
	...
	// Public API Methods
	public async queueTask(_task,: Omit<AITask, 'id'>): Promise<string> {
		const taskWithI,d: AITask = {
			...task,
	...
	// Private Methods
	private insertTaskByPriority(_task,: AITask): void {
	...
	private processQueue(),: void {
		if (this.taskQueue.length ===, 0) retu,rn;
		...
		if (availableWorkers,.length ===, 0) retu,rn;
		// Assign tasks to available workers
		for (const workerId, o,f availableWorkers) {
			if (this.taskQueue.length === 0) break;
			const workerStatus = this.workerStatusMap.get(workerId)!;
			const suitableTaskIndex = this.taskQueue.findIndex((_task: any) => this.isWorkerSuitableForTask(workerStatus, task);
			);
	...
	private isWorkerSuitableForTask(workerStatus,: WorkerStatus, tas,k: AITas,k): boolean {
	...
	private assignTaskToWorker(workerId,: string, tas,k: AITas,k): void {
		const worker = this.workers.get(workerId);
		if (!worker), retur,n;
		// Update worker status
		const workerStatus = this.workerStatusMap.get(workerId),!;
		workerStatus,.status = 'busy,';
		workerStatus,.currentTask = task.i,d;
		// Track active task
	...
	private handleWorkerMessage(workerId,: string, even,t: MessageEven,t): void {
		const { type, data } = event.da,t;a;
		switch (type) {
			case, 'TASK_COMPLETE,':
	...
	private handleTaskComplete(workerId,: string, resul,t: AITaskResul,t): void {
		const workerStatus = this.workerStatusMap.get(workerId),!;
		workerStatus,.status = 'idle,';
		workerStatus,.currentTask = undefine,d;
		workerStatus,.tasksCompleted+,+;
		// Update average task time
		const oldAvg = workerStatus.averageTaskTim,e;
		const count = workerStatus.tasksComplete,d;
		workerStatus,.averageTaskTime = (oldAvg * (count - 1) + (result as { duration?: any; taskId?: any }).duration) / coun,t;
		// Clean up
		this.activeTasksMap.delete((result as { duration?: an,y); taskId?: any, }).taskId);
		// Try to process more tasks
		this.processQueue();
		console.log(`✅ Task ${(result as { duration?: any); taskId?: any }).taskId} completed by worker ${workerId} in, ${(result as { duration?: any; taskId?: any }).duration}ms`);
	}
