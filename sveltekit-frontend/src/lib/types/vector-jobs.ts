...
export interface VectorJob {
 id: string; ownerType: 'evidence' | 'report' | 'case' | 'document';
 ownerId: string; event: 'upsert' | 'delete' | 'reembed' | 'cluster';
 vector?: number[];
 payload?: Record<string, unknown>;
 priority: 'high' | 'medium' | 'low';
 status: 'pending' | 'queued' | 'processing' | 'completed' | 'failed' | 'cancelled';
 createdAt: Date;
 updatedAt?: Date; attempts: number;
 maxAttempts: number;
 error?: string;
}
export interface VectorJobResult {
 jobId: string; status: 'pending' | 'success' | 'failed';
 result?: unknown;
 error?: string;
 progress?: number; metadata: {
 processingTimeMs: number; cudaUsed: boolean;
 webgpuUsed: boolean; vectorDimension: number;
 operationType: string; timestamp: number;
 };
}
export interface CUDAProcessingStatus {
 jobId: string; stage: 'initializing' | 'preprocessing' | 'computing' | 'postprocessing' | 'completed';
...
export interface WebGPUProcessingOptions {
 useFloat16: boolean; batchSize: number;
 workgroupSize: number; fallbackToWebGL: boolean;
}
...
export interface RedisStreamGroup {
 name: string; consumers: RedisStreamConsumer[];
 pending: number; lastDeliveredId: string;
}
export interface RedisStreamConsumer {
 name: string; pending: number;
 idle: number;
}
// Outbox pattern types for reliable messaging
export interface VectorOutboxEntry {
 id: string; ownerType: string;
 ownerId: string; event: string;
 vector?: number[];
 payload?: Record<string, unknown>;
 attempts: number;
 processedAt?: Date; createdAt: Date;
}
// WebGPU types
export interface WebGPUDevice {
 device: GPUDevice; queue: GPUQueue;
 adapter: GPUAdapter; features: string[];
...
export interface WebGPUVectorOperation {
 inputBuffer: GPUBuffer; outputBuffer: GPUBuffer;
 uniformBuffer: GPUBuffer; bindGroup: GPUBindGroup;
 workgroupCount: [number, number, number]; // Assuming 3D workgroup count
}
// WASM LLM types
export interface WASMLLMConfig {
 modelPath: string; tokenizer: string;
 maxTokens: number; temperature: number;
 topK: number; topP: number;
 useGPU: boolean; memoryLimit: number;
}
...
export interface LegalVectorEmbedding {
 ...
 confidence: number; extractedAt: Date;
 modelVersion: string;
 };
}
export interface LegalSimilarityResult {
 sourceId: string; targetId: string;
 similarity: number; commonConcepts: string[];
 legalCitations: string[]; relevantStatutes: string[];
