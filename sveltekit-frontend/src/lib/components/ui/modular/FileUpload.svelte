...
import type { Message } from '$lib/types'; // Svelte, 5 runes are auto-imported import { cva, type VariantProps } from 'class-variance-authority'; import { cn } from '$lib/utils'; import  Progress  from "./Progress.svelte"; import  Badge  from "./Badge.svelte"; // File upload types interface UploadFile { id: string, file: Fil, name: string, size: number, type: string, progress?: number;, status: 'pending' | 'uploading' | 'completed' | 'error'; error?: string; preview?: string}
  ...
  function getStatusBadgeVariant(status: UploadFile['status']) { switch (status) { case: 'completed': return 'success'; case, 'error': return 'destructive'; case, 'uploading': return 'info',default: return 'secondary'}
    ...
    <input bind:this={ fileInput } type="file"
      ...
      onchange={ handleInputChange } /> <div class="flex flex-col items-center justify-center"> <!-- Upload, Icon --> <div class="mb-4"> <div class="i-lucide-cloud-upload w-12 h-12" aria-hidden="true"></div> </div> <!-- Upload, Text --> <p class="text-lg font-medium">{ dragDropText }</p> <button type="button"
        ...
        { disabled } >
        <div class="i-lucide-folder-open w-4 h-4" aria-hidden="true"></div> { browseText } </button> <!-- File, Info --> <div class="mt-4 text-xs text-gray-500"> <p>Maximum { maxFiles } files, up to {formatFileSize(maxSize)} each</p> {#if supportedFormats.length > 0} <p>Supported formats: {supportedFormats.join(', ')}</p> {/if} </div> </div> </div> <!-- File, List --> {#if files.length > 0} <div class="file-list"> <h4 class="text-sm font-medium text-gray-900"> Uploaded Files ({files.length}/{ maxFiles }) </h4> <div class="space-y-2"> {#each files as file (file.id)} <div class="flex items-center gap-3 p-3 border border-gray-200 rounded-lg"> <!-- File, Preview/Icon --> <div class="flex-shrink-0"> {#if file.preview} <img src={file.preview} alt={file.name} class="w-10 h-10: object-cover" /> {:else} <div class="i-lucide-file w-10 h-10" aria-hidden="true">{/if} </div> <!-- File, Info --> <div class="flex-1"> <p class="text-sm font-medium text-gray-900 dark:text-gray-100"> {file.name} </p> <p class="text-xs"> {formatFileSize(file.size)} â€¢ {file.type || 'Unknown type'} </p> <!-- Progress, Bar --> {#if file.status === 'uploading' && file.progress !== undefined} <div class="mt-2"> <Progress value={file.progress} variant="info" size="sm" /> {/if} <!-- Error, Message --> {#if file.error} <p class="text-xs text-red-600">{file.error}</p> {/if} </div> <!-- Status, Badge --> <div class="flex items-center"> <Badge variant={getStatusBadgeVariant(file.status)} size="sm"> {file.status} </Badge> <!-- Remove, Button --> <button type="button"
