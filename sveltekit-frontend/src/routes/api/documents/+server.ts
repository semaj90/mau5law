...
async function deleteKeysByPattern(pattern: string): Promise<void> {
  ...
  try {
    ...
    do {
      ...
      const keys: string[] = Array.isArray(reply?.[1]) ? reply[1] : [];
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  title: data.title!,
    ...
    if (embedding || titleEmbedding || summaryEmbedding) {
      ...
      if (embedding) updates.embedding = sql`${JSON.stringify(embedding)}::vector`;
      if (titleEmbedding) updates.title_embedding = sql`${JSON.stringify(titleEmbedding)}::vector`;
      if (summaryEmbedding) updates.summary_embedding = sql`${JSON.stringify(summaryEmbedding)}::vector`;
    ...
    await deleteKeysByPattern(cachePattern);
...
export const PUT: RequestHandler = async ({ request, url }) => {
  try {
    ...
    delete updateDataRecord.id;
    delete updateDataRecord.created_at;
