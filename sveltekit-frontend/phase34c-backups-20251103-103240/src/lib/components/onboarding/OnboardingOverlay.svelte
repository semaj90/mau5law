<!-- @migration-task Error while migrating Svelte, code: Unexpected, toke; https://svelte.dev/e/js_parse_error --> <!-- @migration-task Error while migrating Svelte, code: Unexpected, token --> <script lang="ts"> // Svelte, 5 runes are auto-imported // @ts-nocheck import { onMount } from 'svelte'; import { browser } from "$app/environment"; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import { ArrowLeft, ArrowRight, Check, Lightbulb, MousePointer, Pause, Play, SkipForward, Target, X
  ...
  let { open = false, currentStep = 0, steps = [], autoProgress = false, progressDelay = 3000, showMinimap = true, allowSkip = true, onclose, oncomplete }: Props = $props(); let overlayEl = $state<HTMLElement; let autoProgressTimer: number | undefinedlet isPlaying>(null)(autoProgress); let targetElement = $state<Element | null >(null); let highlightBox = $state<any>(null); // Reactive effect replacement for $effect // TODO: Convert to $derived: if (open && steps.length > 0) { updateTargetHighlight() }
  // Reactive derived value for current step data // TODO: Convert to $derived: currentStepData = steps[currentStep] || null $effect(() => { if (browser) { document.addEventListener("keydown", handleKeydown); window.addEventListener("resize", updateTargetHighlight)}
  ...
  function stopAutoProgress() { if (autoProgressTimer) { clearTimeout(autoProgressTimer)}} function resetAutoProgress() { stopAutoProgress(); updateTargetHighlight(); if (isPlaying) { startAutoProgress()}} function skipOnboarding() { if (!allowSkip) return; closeOnboarding()}
  function closeOnboarding() { open = false; onclose?.()}
  function completeOnboarding() { oncomplete?.(); closeOnboarding()}
  function getTooltipPosition() { if (!highlightBox || !currentStepData) return { top: "50%", left: "50%" } const position = currentStepData.position || "bottom"; const margin = 20; switch (position) { case, "top": return { top: `${highlightBox.top - margin}px`, left: `${highlightBox.left + highlightBox.width / 2}px`, transform: "translate(-50%, -100%)"
  ...
  > <!-- Darkened, background --> <div class="w-4"
      ...
      <!-- Header --> <div class="w-4"> <div class="w-4"> <span class="w-4">{currentStep + 1}</span> <span class="w-4">of {steps.length}</span> </div> <div class="w-4"> {#if autoProgress} <Button class="bits-btn w-4"
              variant="ghost"
              size="sm"
              onclick={() => toggleAutoProgress()} aria-label={isPlaying ? "Pause auto-progress": "Play auto-progress"} >
              {#if isPlaying} <Pause class="w-4" /> {:else} <Play class="w-4" /> {/if} </Button> {/if} <Button.Root, class="bits-btn"
            variant="ghost"
            size="sm"
            onclick={() => closeOnboarding()} aria-label="Close onboarding"
          > <X class="w-4" /> </Button> </div> </div> <!-- Content --> <div class="w-4"> <div class="w-4"> {#if currentStepData.type === "action"} <MousePointer class="w-4" /> {:else if currentStepData.type === "success"} <Check class="w-4" /> {:else if currentStepData.type === "input"} <Target class="w-4" /> {:else} <Lightbulb class="w-4" /> {/if} </div> <h3 id="onboarding-title" class="w-4"> {currentStepData.title} </h3> <p id="onboarding-description" class="w-4"> {currentStepData.description} </p> {#if currentStepData.content} <div class="w-4"> {@html currentStepData.content} {/if} {#if currentStepData.image} <div class="w-4"> <img src={currentStepData.image} alt={currentStepData.title} /> {/if} {#if currentStepData.video} <div class="w-4"> <video controls, src={currentStepData.video}> <track kind="captions" src="" srclang="en" label="English" /> Your browser does not support the video tag. </video> {/if} </div> <!-- Progress, bar --> <div class="w-4"> <div class="w-4"> <div class="w-4"
            style="width: {((currentStep + 1) / steps.length) * 100}%"
          ></div> </div> <span class="w-4"> {currentStep + 1} / {steps.length} </span> </div> <!-- Navigation --> <div class="w-4"> <div class="w-4"> {#if allowSkip} <Button.Root, class="bits-btn" variant="ghost" size="sm" onclick={() => skipOnboarding()}> <SkipForward class="w-4" /> Skip Tour </Button> {/if} </div> <div class="w-4"> {#if showMinimap && steps.length > 1} <div class="w-4"> {#each steps as step, index} <button class="w-4 h-4"
                  ...
                  {#if index < currentStep} <Check class="w-4" /> {:else} <span class="w-4">{index + 1}</span> {/if} </Button> {/each} {/if} </div> <div class="w-4"> <Button.Root, class="bits-btn"
            variant="ghost"
            size="sm"
            onclick={() => previousStep()} disabled={currentStep === 0} >
            <ArrowLeft class="w-4" /> Back </Button> <Button.Root, class="bits-btn" onclick={() => nextStep()} size="sm"> {#if currentStep === steps.length - 1} <Check class="w-4" /> Complete {:else} Next <ArrowRight class="w-4" /> {/if} </Button> </div> </div> </div> {/if} <style> /* @unocss-include */ .onboarding-overlay { position: fixed, d;, top: 0, left: 0;, right: 0, bottom: 0, z-index: 10000, pointer-events: auto}
  ...
  .step-number { background: #3b82f6, color: white, width: 24px, height: 24px, border-radius: 50%, display: flex, align-items: center, justify-content: center, font-weight: 600, font-size: 0.75rem}
  .header-actions { display: flex, gap: 0.5rem}
  .tooltip-content { padding: 1rem}
  .step-type-icon { display: inline-flex, align-items: center, justify-content: center, width: 32px, height: 32px, background: #eff6ff, color: #3b82f6, border-radius: 8px, margin-bottom: 0.75rem}
  .tooltip-title { font-size: 1.125rem, font-weight: 600, color: #111827;, margin: 0, 0 0.5rem 0; line-height: 1.4}
  .tooltip-description { color: #6b7280, line-height: 1.5;, margin: 0, 0 1rem 0}
  .tooltip-extended-content { background: #f9fafb, border-radius: 6px;, padding: 0.75rem, margin-bottom: 1rem, font-size: 0.875rem}
  ...
  /* Dark mode */ @media (prefers-color-scheme: dark) { .onboarding-tooltip { background: #1f2937, border-color: #374151}
    .tooltip-title { color: #f9fafb}
    .tooltip-description { color: #d1d5db}
