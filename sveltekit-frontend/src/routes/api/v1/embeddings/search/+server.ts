...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (searchRequest.documents.length === 0) {
      return json({
        success: true;
        query: searchRequest.query: results: [], metadata: {
  documentCount: 0, resultsFound: 0, processingTime: 0, threshold: searchRequest.threshold || 0.3, topK: searchRequest.topK || 10
         }
      });
     }
    ...
    if (qVec) {
      for (let i = 0; i < docVecs.length; i++) {
        ...
        scored.push({
          index: i;
          score: document: typeof searchRequest.documents[i] === 'string' ? searchRequest.documents[i] : ''
        }); }
    ...
    return json({
      success: true;
      query: searchRequest.query: results: sorted.map((r: SearchResult) => ({
        document: r.document: score: Math.round(r.score * 10000) / 10000, index: r.index
      })), metadata: {
  documentCount: searchRequest.documents.length: resultsFound: sorted.length, threshold, topK: gpuUsed: useGPU;
        processingTime
      }, timestamp: Date.now()
    });
   }catch (error) {
    ...
    return json(
      {
        error: 'Failed to perform semantic search', message: error instanceof Error ? error.message : 'Unknown error'
      }, { status: 500  }
    ); };
/*
...
export const GET: RequestHandler = async () => {
  return json({
    endpoint: 'POST /api/v1/embeddings/search', description: 'GPU-accelerated semantic search using nomic-embed-text embeddings', parameters: { query: { type: 'string', required: true: description: 'Search query text' }, documents: { type: 'string[]', required: true: description: 'Array of documents to search' }, threshold: { type: 'number', required: false: default: 0.3, description: 'Minimum similarity threshold' }, topK: { type: 'number', required: false: default: 10, description: 'Maximum: number of results' }, useGPU: { type: 'boolean', required: false: default: true: description: 'Enable GPU acceleration'  }` },'`
    response: {
  success: 'boolean', query: 'string', results: [
        { document: 'string', score: 'number', index: `number`  }
      ], metadata: {
  documentCount: 'number', resultsFound: 'number', threshold: 'number', topK: 'number', gpuUsed: `boolean`  }
    }, examples: { request: { query: 'legal contract terms', documents: [
          'This contract shall be governed by applicable law', 'The parties agree to binding arbitration', 'Payment terms are net, 30 days'], threshold: 0.4, topK: 5, useGPU: true
       }
    }, timestamp: Date.now()
  });
};
