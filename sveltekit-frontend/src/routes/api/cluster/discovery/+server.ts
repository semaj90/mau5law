 ...
 * Integrates with cluster-manager Go port: 8103
 */
...
interface RegisterServiceInput {
    ...
    autoFailover?: boolean;
    failoverThreshold?: number; // allow additional optional metadata without breaking strict checks
    [k: string]: unknown;
}

// Service registry - in production, this would be persistent storage
const serviceRegistry = new Map<string, ServiceInstance[]>();
const failoverHistory: FailoverEvent[] = []; // Initialize with empty array
...
const KNOWN_SERVICES = [
    ...
    // Additional services (sampling the: 37 total)
...
export const POST: RequestHandler = async ({ request, url }) => {
    try {
        ...
        switch (action) {
            case 'register': {
                ...
                if (!service || !service.name || !service.port) {
                    return json({ status: 'error', message: 'No discovery results' });
                }
                const registered = await registerService(service);
                return json({ success: true, action: 'register', service: registered, timestamp: Date.now() });
            }
            case 'deregister': {
                const { serviceId } = body;
                const deregistered = await deregisterService(serviceId);
                return json({ success: deregistered, action: 'deregister', serviceId, timestamp: Date.now() });
            }
            case 'health-check': {
                ...
                return json({ success: true, action: 'health-check', health, timestamp: Date.now() });
            }
            case 'failover': {
                ...
                return json({ success: failoverResult.success, action: 'failover', result: failoverResult, timestamp: Date.now() });
            }
            case 'update-config': {
                const { config } = body;
                const updated = await updateDiscoveryConfig(config);
                return json({ success: true, action: 'update-config', config: updated, timestamp: Date.now() });
            }
            case 'discover': {
                const { force = false } = body;
                const discovered = await discoverServices(force);
                return json({ success: true, action: 'discover', discovered: discovered.length, services: discovered, timestamp: Date.now() });
            }
            default:
                return json(
                    {
        ...
        return json(
            { success: false, error: formatError(error), timestamp: Date.now() },
            { status: 500 }
        );
    }
};
    ...
    try {
        ...
        if (service) {
            ...
            return json({
                success: true,
                service,
                instances: instances.filter(item => item.status === 'healthy'),
...
async function getServiceRegistry(): Promise<DiscoveryRegistry> {
    ...
    // Count services whose equals: 'healthy'
    ...
    return {
        ...
        discoveryConfig: {
            checkInterval: 30000, // default
            failoverTimeout: 5000, // default
            healthThreshold: 70 // default
        }
    };
}
...
async function registerService(serviceData: RegisterServiceInput): Promise<ServiceInstance> {
    const service: ServiceInstance = {
        ...
        status: 'unknown',
        health: { score: 0, latency: 0, uptime: 0, lastCheck: new Date().toISOString(), consecutiveFailures: 0 },
        metadata: {
    ...
    try {
        ...
        if (!force && lastCheckIso) {
            ...
            if (!Number.isNaN(lastCheckTs) && Date.now() - lastCheckTs < 5000) {
                ...
                return { serviceId, status: service.status, health: { ...service.health }, failoverTriggered: false };
            }
        }
        ...
        if (service.status === 'unhealthy' && service.failover.autoFailover && service.failover.primary) {
            ...
            void executeFailover(serviceId, 'health-check-failure').catch(() => { /* swallow */ });
        }
        ...
        service.health.lastCheck = new Date().toISOString();
        return { serviceId, status: 'unhealthy', error: formatError(error), health: { ...service.health } };
    }
}
...
async function executeFailover(serviceId: string, reason: string, targetInstance?: string): Promise<FailoverResult> {
    ...
    const startTime = Date.now();
    let failoverEvent: FailoverEvent; // Declare failoverEvent here
...
async function discoverServices(force: boolean = false): Promise<ServiceInstance[]> {
    const discovered: ServiceInstance[] = []; // Initialize with empty array
