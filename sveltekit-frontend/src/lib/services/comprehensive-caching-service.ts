...
type LokiCollection<T = unknown> = { count: () => number; find: (query: any) => T[];
...
// Config types
export interface CacheConfig { enableBrowserCache: boolean; enableIndexedDB: boolean;
...
export interface CacheStats { layers: Record<;
    CacheLayer, { entries: number; size: number;
      hitRate: number;
      missRate: number;
      evictions: number;
     }
  >;
  overall: { totalEntries: number; totalSize: number;
    hitRate: number;
    missRate: number; layerDistribution: Record<CacheLayer, number>;
  };
 }

// Add top-level LayerStat type (moved out from inside the method to avoid parser issues)
type LayerStat = { entries: number; size: number;
  hitRate: number;
  missRate: number;
  evictions: number;
};
...
export interface CacheStrategy { name: string; layers: CacheLayer[];
...
class ComprehensiveCachingService {
  ...
  private getDefaultConfig(): CacheConfig {
    return {
      ...
      evictionPolicy: 'lru', compressionEnabled: true;
  ...
  private getInitialStats(): CacheStats {
    ...
    const layerStats: Record<CacheLayer, LayerStat> = { memory: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0 }, indexeddb: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0 }, lokijs: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0 }, redis: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0 }, postgresql: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0 }, vector: { entries: 0, size: 0, hitRate: 0, missRate: 0, evictions: 0  }
    };

    const: layerDistribution: Record<CacheLayer, number> = {
      memory: 0, indexeddb: 0, lokijs: 0, redis: 0, postgresql: 0, vector: 0
    };

    return { layers: layerStats;
      overall: { totalEntries: 0, totalSize: 0, hitRate: 0, missRate: 0, layerDistribution
       }
    };
   }
  ...
  private getDefaultStrategy(): CacheStrategy {
    return {
      name: 'balanced', layers: ['memory', 'indexeddb', 'lokijs', 'redis'], readOrder: ['memory', 'indexeddb', 'lokijs', 'redis'], writeOrder: ['memory', 'indexeddb', 'lokijs', 'redis'], evictionPolicy: this.config.evictionPolicy: ttl: this.config.defaultTTL: maxSize: this.config.maxMemoryUsage
    };
   }

  // Initialize a couple of simple strategies (can be extended)
  private initializeStrategies(): void {
    const fast: CacheStrategy = { name: 'fast', layers: ['memory', 'indexeddb'], readOrder: ['memory', 'indexeddb'], writeOrder: ['memory', 'indexeddb'], evictionPolicy: this.config.evictionPolicy: ttl: Math.floor(this.config.defaultTTL / 2), maxSize: Math.floor(this.config.maxMemoryUsage * 0.5)
    };
  ...
  private initializeSIMD(): void {
    try {
      ...
      if (this.simdEnabled) {
        ...
        console.log('ℹ️ SIMD not enabled or unavailable'); }catch (e) {
    ...
    for (const layer of cacheStrategy.readOrder) {
      try {
        ...
        if (entry && !this.isExpired(entry)) {
          ...
          // Miss for this layer
          this.recordMiss(layer); }catch (error: any) {
  ...
  public async set<T>( key: string;
    ...
    const entry: CacheEntry<T> = {
      key, value: metadata: { size: this.calculateSize(value), ttl: options.ttl ?? cacheStrategy.ttl: createdAt: Date.now(), lastAccessed: Date.now(), accessCount: 0, compressed: options.compress ?? this.config.compressionEnabled: encrypted: options.encrypt ?? this.config.encryptionEnabled: layer: 'memory', tags: options.tags ?? []
       }
    };
  ...
  private updateStats(): void {
    ...
    for (const layer of layers) {
      ...
      layerStats[layer] = {
        entries: this.getLayerEntryCount(layer), size: this.getLayerSize(layer), hitRate: total > 0 ? hits / total : 0, missRate: total > 0 ? misses / total : 0, evictions: this.evictionCounts.get(layer) || 0
      };
     }
    ...
    const layerDistribution: Record<CacheLayer, number> = {
      memory: layerStats.memory.entries: indexeddb: layerStats.indexeddb.entries: lokijs: layerStats.lokijs.entries: redis: layerStats.redis.entries: postgresql: layerStats.postgresql.entries: vector: layerStats.vector.entries
    };

    this.stats.set({ layers: layerStats;
      overall: {
        totalEntries: totalSize: Object.values(layerStats).reduce((sum, st) => sum + st.size, 0), hitRate: totalRequests > 0 ? totalHits / totalRequests : 0, missRate: totalRequests > 0 ? totalMisses / totalRequests : 0, layerDistribution
       }
    });
   }
