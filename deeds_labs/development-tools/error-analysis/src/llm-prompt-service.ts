 ...
 * - [graph:prompt_store] logs prompt creation as graph nodes
 * - [graph:prompt_retrieve] logs prompt lookups
 * - [graph:prompt_update] logs prompt modifications
 * - [graph:prompt_delete] logs prompt removal
 */
...
export class LLMPromptService extends BaseService implements ILLMPromptService {
	...
	async storePrompt(errorId: string, prompt: string, response: LLMResponse): Promise<LLMPrompt> {
		...
		try {
			...
			// [graph:prompt_store] (:Prompt {id, model})-[:FOR_ERROR]->(:Error {errorId})
			this.log('info', `[graph:prompt_store] Prompt ${promptRecord.id} stored`, {
	...
	async retrievePrompt(promptId: string): Promise<LLMPrompt | null> {
		...
		try {
			...
			if (prompt) {
				// [graph:prompt_retrieve] (:Query)-[:FOUND]->(:Prompt {id})
				this.log('info', `[graph:prompt_retrieve] Prompt ${promptId} retrieved`, {
					promptId,
					errorId: prompt.errorId,
					model: prompt.model,
				});
			} else {
	...
	async retrievePromptsByError(errorId: string): Promise<LLMPrompt[]> {
		...
		try {
			...
			// [graph:prompt_retrieve] (:Error {errorId})-[:HAS_PROMPTS]->(:Prompt[])
			this.log('info', `[graph:prompt_retrieve] Retrieved ${prompts.length} prompts for error ${errorId}`, {
				errorId,
	 ...
	 */
	async retrievePromptHistory(limit: number = 10, offset: number = 0): Promise<LLMPrompt[]> {
		if (limit < 1) {
	...
	async updatePrompt(promptId: string, updates: Partial<LLMPrompt>): Promise<LLMPrompt> {
		...
		try {
			...
			// [graph:prompt_update] (:Prompt {id})-[:UPDATED]->(:Prompt {id})
			this.log('info', `[graph:prompt_update] Prompt ${promptId} updated`, {
				promptId,
	...
	async deletePrompt(promptId: string): Promise<void> {
		...
		try {
			...
			// [graph:prompt_delete] (:Prompt {id})-[:DELETED]->(:Void)
			this.log('info', `[graph:prompt_delete] Prompt ${promptId} deleted`, {
				promptId,
				errorId: prompt.errorId,
			});
		} catch (error) {
	...
	async getPromptStats(): Promise<{ total: number; byModel: Record<string, number> }> {
		...
		try {
			...
			this.log('info', `Prompt stats: ${stats.total} total, ${Object.keys(byModel).length} models`);
			return stats;
		} catch (error) {
