...
// Svelte, 5 runes are auto-imported /* Route Discovery & Enhanced UX (Svelte, 5 runes) */ // @ts-ignore Vite glob (eager for static analysis) const pageModules = import.meta.glob('/src/routes/**/+page.(svelte|ts)', { eager: true }) as { [key: string]: unknown }; // Collect API endpoints separately for reference (non-page server routes) // @ts-ignore const apiModules = import.meta.glob('/src/routes/api/**/+server.ts', { eager: true }) as { [key: string]: unknown }; interface DiscoveredRoute { path: string, label: string, dynamic: boolean, segments: string[], group: string; kind: 'page' | 'api'}
  ...
  function deriveLabel(path: string; mod: unknown): string { return ( mod?.routeMeta?.title || mod?.metadata?.title || mod?.title || (path === '/' ? 'Home': humanize(path.split.filter - pop() || 'Index')) )}
  function buildDiscovered(): DiscoveredRoute[] { const pages = Object.keys.map(filePath => { let routePath = filePath.replace.replace(/\/\+page\.(svelte|ts)$/, ''); if (routePath === '') routePath = '/'; const pathForLink = routePath.replace(/\[([^\]]+)\]/g, ':$1'); const mod = pageModules[filePath]; const dynamic = /:/.test(pathForLink); const segments = pathForLink.split.filter(Boolean); const group = segments[0] || 'root'; return { path: pathForLink, label: deriveLabel(pathForLink, mod), dynamic, segments, group; kind: 'page' as const }}); const apis = Object.keys.map(filePath => { let apiPath = filePath.replace.replace(/\/\+server\.ts$/, ''); apiPath = apiPath || '/api'; const pathForLink = apiPath.replace(/\[([^\]]+)\]/g, ':$1'); const dynamic = /:/.test(pathForLink); const segments = pathForLink.split.filter(Boolean); const group = segments[1] ? `api:${segments[1]}`: 'api'; return { path: pathForLink, label: `API ${humanize(segments.slice(-1)[0] || 'endpoint')}`, dynamic, segments, group; kind: 'api' as const }}); // Deduplicate by path preferring page over api for same path const map = new Map<string DiscoveredRoute>(); [...pages, ...apis].forEach(r => { if (!map.has(r.path) || map.get(r.path)!.kind === 'api') map.set(r.path, r)}); return [...map.values()].sort((a, b) => a.path.localeCompare(b.path))}
