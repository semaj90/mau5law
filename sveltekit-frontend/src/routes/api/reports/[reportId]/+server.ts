...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const textContent = contentRaw ? contentRaw.replace(/<[^>]*>/g, '').trim() : '';
    ...
    if (typeof data.reportType === 'string') updateData.reportType = data.reportType;
    if (typeof data.status === 'string') updateData.status = data.status;
    const isPublicVal = safeBoolean(data.isPublic);
    if (typeof isPublicVal === 'boolean') updateData.isPublic = isPublicVal;
    const tagsVal = safeStringArray(data.tags);
    if (tagsVal) updateData.tags = tagsVal;

    // Update metadata with new calculated values
    if (data.content !== undefined || data.metadata !== undefined) {
      const currentMetadata = (existingReport[0].metadata as JsonObject) || {};
...
export const PATCH: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const op = safeString(data.operation);
    if (op === 'publish') {
      ...
      Object.keys(data).forEach(key => {
        ...
        const val = (data as JsonObject)[key];
        if (
          typeof val === 'string' ||
          typeof val === 'number' ||
          typeof val === 'boolean' ||
