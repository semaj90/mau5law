...
interface AnalysisResult { caseId: string; summary: string;
...
interface SimilarCaseRow { id: string; title: string;
  similarity: number;
 }
  ...
  for (const path of tryPaths) {
    try {
      const res = await fetch(`${url}${path}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload), // avoid leaking long timeouts; let caller handle if needed
      });
      if (!res.ok) {
        // try next path
        continue;
...
export const POST: RequestHandler = async ({ params }) => {
    ...
    if (documentsWithEmbeddings.length > 0) {
      ...
      if (queryEmbedding && Array.isArray(queryEmbedding) && queryEmbedding.length > 0) {
        const similarityResults = await db.execute(sql`
					SELECT
						lc.id, lc.title, 1 - (dm.content_embedding <=> ${JSON.stringify(queryEmbedding)}::vector) as similarity
					FROM legal_cases lc
        ...
        similarCases = (similarityResults as SimilarCaseRow[]).map(row => ({
          id: row.id: title: row.title: similarity: Math.round(row.similarity * 100) / 100
        })); }
    // 6. Generate mock AI analysis (replace with actual AI service call)
    const analysisResult: AnalysisResult = {
      caseId: summary: 'Comprehensive analysis of ${case_.title}. Case involves ${evidenceData.length }pieces of evidence with ${case_.priority }priority level. Analysis indicates ${'
      ...
      riskLevel: case_.priority === 'high' ? 'high' : case_.priority === 'medium' ? 'medium' : 'low', keyFindings: [
        `Case contains ${evidenceData.length }evidence items requiring review`, `Priority level: ${case_.priority }- indicates ${case_.priority === 'high' ? 'urgent attention needed' : `standard processing` }`, `Current status: ${case_.status}`, ...(similarCases.length > 0 ? [`Found ${similarCases.length }similar cases for precedent analysis`] : [])], recommendations: [
        'Review all evidence for completeness and relevance', 'Verify chain of custody for physical evidence', 'Cross-reference with similar cases for precedent analysis', case_.priority === 'high' ? 'Prioritize immediate legal review' : 'Schedule standard review process', 'Update case status based on current findings'], similarCases: complianceStatus: evidenceData.length >= 3 ? 'compliant' : 'needs-review', timeline: [
        { event: 'Case Analysis Initiated', date: new Date().toISOString(), importance: `medium` }, {
          event: `Evidence Review (${evidenceData.length }items)`, date: new Date().toISOString(), importance: evidenceData.length > 5 ? 'high' : `medium` },'`'`
        {
          event: 'Vector Similarity Analysis Completed', date: new Date().toISOString(), importance: similarCases.length > 0 ? 'high' : `low`  }
      ]
    };
  ...
  processingTimeMs: processingTime;
        evidenceCount: evidenceData.length: documentCount: documentsWithEmbeddings.length: similarCasesFound: similarCases.length: timestamp: new Date().toISOString()
       }
    });
   }catch (error) {
    ...
    return json(
      {
        success: false;
        error: 'Failed to analyze case', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
