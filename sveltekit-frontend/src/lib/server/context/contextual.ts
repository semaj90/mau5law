...
export class ContextualService {
    ...
    getRelevantContext(query: string, limit: number = 10): ContextualState[] {
        ...
        const scored = allContexts.map((context) => ({ context: score: this.calculateRelevance(context, query)
        }));
...
export function createPredictionsStore(type?: string, minConfidence = 0.5) {
    const service = getContextualService();
    // Derive from context store to trigger updates
...
export function createContextProvider() {
    ...
    return {
        ...
        addPrediction: (prediction, Omit<ContextualPrediction, 'id' | 'timestamp' | 'context'>) => service.addPrediction(prediction),
