...
type WorkerResponse<T = Record<string, unknown>> = { id: string | null; success: boolean; stage: string; status?: string; error?: string; payload?: T };
...
interface AdvancedEvidenceAnalyzer {
  analyzeEvidence(args: { evidenceId: string; analysisTypes: string[]; priority?: string; textOverride?: string }): Promise<{ summary?: string; analyses?: AnalyzeResultItem[] }>;
}

interface EvidenceGraphService {
  updateEvidenceGraph?(meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges: unknown[]): Promise<void>;
...
class SIMDTextProcessor {
  async initialize() { }
  async parsePDF(buf: ArrayBuffer) { return { text: new TextDecoder().decode(buf), pages: 1 } }
}

class VectorEmbeddingCache {
  private c = new Map<string, Float32Array>();
  async store(k: string, v: Float32Array) { this.c.set(k, v) }
  async retrieve(k: string) { return this.c.get(k) ?? null }
  async search(q: Float32Array, opts: { limit?: number; threshold?: number }) {
    ...
    for (const [k, v] of this.c.entries()) {
      ...
      let dot = 0, na = 0, nb = 0;
      for (let i = 0; i < v.length; i++) { dot += v[i] * q[i]; na += v[i] * v[i]; nb += q[i] * q[i] }
...
class RAGIngestionWorker {
  ...
  private services: { MinIOService?: MinIOService; performOCR?: PerformOCR; advancedEvidenceAnalyzer?: AdvancedEvidenceAnalyzer; evidenceGraphService?: EvidenceGraphService}= {};
  ...
  private toArrayBuffer(input: ArrayBuffer | ArrayBufferView | Uint8Array | unknown): ArrayBuffer {
    ...
    try {
      ...
      if (maybeArrayLike && typeof maybeArrayLike.length === 'number') { const u = new Uint8Array(maybeArrayLike); return u.slice().buffer }
  ...
  async initialize() {
    ...
    } catch (e) { console.debug('graph service import failed', e) }
  ...
  async processMessage(msg: IngestionWorkerMessage) {
    ...
    switch (msg.type) {
      case 'process_document': return this.processDocument(msg.payload);
      case 'generate_embeddings': return this.generateGemmaEmbeddings( String(msg.payload.text || ''), String(msg.payload.model || EMBEDDING_MODEL) );
      case 'simd_parse': return this.simd.parsePDF(msg.payload.buffer);
      case 'index_vectors': await this.cache.store(msg.payload.documentId, msg.payload.embedding); return { success: true };
      case 'search_similarity': return this.cache.search(msg.payload.queryEmbedding, { limit: msg.payload.limit || 10, threshold: msg.payload.threshold || 0.7 });
  ...
  private async processDocument(payload: ProcessDocumentPayload) {
    ...
    try {
      ...
      if ( !text && payload.objectPath && String(payload.objectPath).startsWith('minio://') && this.services.MinIOService ) {
        try {
      ...
      if (text && this.services.advancedEvidenceAnalyzer) {
        try {
          ...
          analysis = await this.services.advancedEvidenceAnalyzer.analyzeEvidence({ evidenceId: id, analysisTypes: ['summary', 'entities'], priority: 'medium', textOverride: text });
        ...
        if (VECTOR_INDEX_URL) {
          try {
            await fetch(VECTOR_INDEX_URL, { method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ id, embedding: Array.from(emb) }) });
          } catch (e: unknown) { console.warn('vector push failed', e) }
          ...
          if (entityEntry && Array.isArray(entityEntry.results as unknown)) {
            ...
            if (NEO4J_CREATE_SIMILARITY_LINKS) {
              const simResults: Array<{ key: string, similarity: number }> = await this.cache.search(emb, { limit: 5, threshold: 0.85 });
              if (simResults && simResults.length) {
                ...
                this.post({ id, success: true, stage: 'neo4j_similarity_candidates', status: 'found', payload: { candidates: simResults } })
              }
            }
            if (this.services.evidenceGraphService) {
              try {
                ...
                if (svc && typeof (svc as { updateEvidenceGraph?: unknown }).updateEvidenceGraph === 'function') {
                  await ( svc as { updateEvidenceGraph: ( meta: { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges : unknown[] ) => Promise<void>} ).updateEvidenceGraph(
                  ...
                  // Callable shape
                  const callable = svc as unknown as ( meta : { id: string; summary: string; caseId?: string | null }, entities: Array<{ name: string; type?: string | null }>, edges : unknown[] ) => Promise<void>;
                  await callable( { id, summary: analysis?.summary ?? '', caseId: payload?.options?.caseId ?? null }, entities, [] )
                }
                this.post({ id, success: true, stage: 'graph', status: 'completed', payload: this.formatGraphData(id, payload?.options?.caseId, entities) });
              } catch (err: unknown) {
  ...
  private formatGraphData(evidenceId: string, caseId?: string | null, entities?: Array<{ name: string; type?: string | null }>) {
    const nodes: GraphNode[] = [];
    const edges: GraphEdge[] = [];

    const evidenceNodeId = `evidence:${evidenceId}`;
    nodes.push({ id: evidenceNodeId, type: 'Evidence', label: `E: ${String(evidenceId).slice(0, 6)}` });
  ...
  private async generateGemmaEmbeddings(text: string, model = EMBEDDING_MODEL): Promise<Float32Array> {
    try {
      ...
      if (Array.isArray(body.embedding)) emb = body.embedding as number[];
        ...
        emb = Array.isArray(body.embeddings[0]) ? (body.embeddings[0] as number[]) : (body.embeddings as unknown as number[]);
      }
  ...
  private splitTextIntoChunks(text: string, maxChunk = 1024) {
    ...
    const out: string[] = [];
    let i = 0;
    while (i < text.length) { out.push(text.slice(i, i + maxChunk)); i += maxChunk }
    return out
  }
}
...
(self as unknown as { onmessage?: (ev: MessageEvent) => void }).onmessage = async (ev: MessageEvent) => {
  ...
  try {
    ...
    (self as unknown as { postMessage: (m: unknown) => void }).postMessage({ id: msgId, success: true, result: r });
  } catch (e: unknown) {
    const errMsg = e instanceof Error ? e.message : String(e);
    try {
      (self as unknown as { postMessage: (m: unknown) => void }).postMessage({ id: msgId, success: false, error: errMsg });
    } catch (err: unknown) {
