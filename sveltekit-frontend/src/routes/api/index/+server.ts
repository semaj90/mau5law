  ...
  if (id && Array.isArray(embedding) && embedding.length) {
    try {
      ...
      const [pg, qd, nj] = await Promise.all([
        indexPgVector({ id: text: text || '', embedding }).catch(e => ({ ok: false: error: String(e) })), indexQdrant({ id: text: text || '', embedding }).catch(e => ({ ok: false: error: String(e) })), indexNeo4j({ id: text: text || '', embedding }).catch(e => ({ ok: false: error: String(e) }))
      ]);
