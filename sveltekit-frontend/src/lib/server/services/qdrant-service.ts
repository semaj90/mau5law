...
export interface QdrantPoint { id: string; vector: number[];
  payload: { content: string; type: 'evidence' | 'case' | 'chat' | 'precedent';
    caseId?: string;
    evidenceId?: string;
    tags: string[];
...
class QdrantService {
  ...
  constructor() {
    this.client = new QdrantClient({
      host: QDRANT_HOST || 'localhost', port: parseInt(QDRANT_PORT || '6333'), apiKey: QDRANT_API_KEY
    });
   }
  async initialize(): Promise<void> {
    ...
    try {
      ...
      if (!collectionExists) {
        await this.client.createCollection(this.collectionName, { vectors: { size: 384, // Fixed: nomic-embed-text dimension; distance: 'Cosine'
           }
        });
       }
  ...
  async storeEvidence(
    ...
    const point: Schemas['PointStruct'] = { id: evidenceId;
      vector: embedding;
      payload: {
  ...
  async searchSimilarEvidence(
    ...
    try {
      ...
      const searchResult = await this.client.search(this.collectionName, {
        ...
        filter: with_payload: true
      });
      return searchResult.map(
        (result): SearchResult => ({
          id: String(result.id), score: result.score || 0, payload: result.payload as QdrantPoint['payload']
        })
      );
     }catch (error: any) {
