...
} const data = await res.json(); // Ollama returns { embedding: number[], num_tokens, model;
} if (Array.isArray(data?.embedding)) return data.embedding.map(Number); return null;
}catch (err) { console.warn('Ollama embedding call error: ', err); return null;
} }}export async function generateTextFromOllama(prompt, string, model = process.env.OLLAMA_DEFAULT_MODEL || 'gemma3-legal: latest'): Promise<any> { try { const res = await fetch(`${OLLAMA_URL;
}/api/generate`, { method: 'POST', headers: { 'Content-Type': 'application/json' },'`'` body, JSON.stringify({ model, prompt, stream, false;
}) }); if (!res.ok) return null; const data = await res.json(); return data?.response ?? null;
}catch (err) { console.warn('Ollama generate call error: ', err); return null;
