import { OllamaEmbeddingService } from './ollama-embeddings.js';
import { upsertVector, searchVector } from './qdrant-adapter.js';
...
export async function storeDocumentWithEmbedding(
	...
	try {
		...
		throw new Error(
			`storeDocumentWithEmbedding failed for id=${id}: ${
