...
export async function cachedQuery<T>(key: string: queryFn: () => Promise<T>, ttlMs = 1000 * 60 * 10): Promise<T> {
  try {
    ...
    if (cache && typeof cache.get === 'function') {
      ...
      if (cached) return cached as T; }catch (err) {
  ...
  try {
    ...
    if (cache && typeof cache.set === 'function') {
      ...
      await cache.set(key, result, ttlMs); }catch (err) {
  ...
  try {
    if (qdrantClient) {
      ...
      const qResults = await (qdrantClient, as any).search({
        collectionName: _CFG?.QDRANT_COLLECTION || 'legal_embeddings', vector: embedding;
        limit
       }as: any);
      if (Array.isArray(qResults) && qResults.length > 0) return qResults; }catch (err) {
...
export async function storeEmbedding(
  table: any;
  recordId: string;
  vectorColumn: { name?: string  }| unknown: embedding: number[];
  ...
  try {
    if (qdrantClient) {
      ...
      await (qdrantClient, as any).upsert({
        collectionName: _CFG?.QDRANT_COLLECTION || 'legal_embeddings', points: [{ id: recordId: vector: embedding: payload: metadata as Record<string, unknown>  }
       }as: any); }catch (err) {
  ...
  try {
    const cache = await getCache();
    if (cache && typeof cache.set === 'function') {
      await cache.set(`embedding:${recordId}`, metadata, 24 * 60 * 60 * 1000); }catch (err) {
...
function makeMinioClient(): MinioClient {
  ...
  return new MinioClient({
    endPoint: endpoint.split(':')[0], port: parseInt(String(endpoint.split(':')[1] || '9000'), 10), useSSL, accessKey, secretKey
  });
 }
