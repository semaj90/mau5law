...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (action === 'health') {
      ...
      return json({
        success: true;
        status: 'healthy', service: 'ollama-chat', model: 'legal:latest', version: version.version || 'unknown', endpoints: {
  chat: '/api/ollama/chat', stream: '/api/ollama/chat?stream=true'
    ...
    if (action === 'models') {
      ...
      return json({
        success: true;
        models: modelsData.models || [], default: 'legal:latest', timestamp: new Date().toISOString()
      });
     }
    return json(
      {
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      message: model: reqModel;
      temperature: reqTemperature;
      maxTokens: reqMaxTokens;
      stream: reqStream;
      systemPrompt, conversationId: useVectorSearch: reqUseVectorSearch;
      context = []
     }= body;
    ...
    const temperature =
      reqTemperature !== undefined ? Number(reqTemperature) : Number(import.meta.env.OLLAMA_TEMPERATURE ?? 0.7);
    const maxTokens =
      reqMaxTokens !== undefined ? Number(reqMaxTokens) : Number(import.meta.env.OLLAMA_MAX_TOKENS ?? 2048);
    const stream = reqStream !== undefined ? Boolean(reqStream) : (import.meta.env.OLLAMA_STREAM ?? 'false') === 'true';
        ...
        async start(controller) {
          try {
            const streamGenerator = ollamaChatStream({
              message, model, temperature, maxTokens, systemPrompt, conversationId, useVectorSearch, context
            });
            ...
            const errorChunk = {
              text: 'An error occurred while processing your request.', metadata: { type: 'error', error: error.message  }
            };
    ...
    return json({
      ...
      metadata: {
        model, confidence, sources, conversationId: timestamp: new Date().toISOString()
       }
    });
   }catch (error: any) {
