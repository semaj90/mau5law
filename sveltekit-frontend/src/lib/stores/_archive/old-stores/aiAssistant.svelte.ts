...
export class AIAssistantManager {
  ...
  private async sendMessageWebAssembly(message: string, options?: any) {
    try {
      ...
      const userEntry: ConversationEntry = { id: crypto.randomUUID(),
        type: 'user',
        content: message,
        timestamp: new Date(),
        metadata: { model: response.metadata.modelUsed,
          temperature: options?.temperature || aiAssistantState.temperature,
          responseTime: 0,
      ...
      const assistantEntry: ConversationEntry = { id: crypto.randomUUID(),
        type: 'assistant',
  ...
  private async sendMessageLangChainRAG(message: string, options?: any) {
    try {
      ...
      const userEntry: ConversationEntry = { id: crypto.randomUUID(),
        ...
        content: message,
        timestamp: new Date(),
        metadata: { model: options?.model || aiAssistantState.model || 'unknown',
          temperature: options?.temperature || aiAssistantState.temperature,
          responseTime: 0,
          tokenCount: message.split(' ').length * 1.5,
          context7Used: options?.useContext7 || false
      ...
      const assistantEntry: ConversationEntry = { id: crypto.randomUUID(),
        ...
        timestamp: new Date(),
        metadata: { model: ragResult.metadata.processingMethod || 'hybrid',
          temperature: options?.temperature || aiAssistantState.temperature,
