...
function classifyCorpusType(
	...
	if (lower.includes('code') || lower.includes('usc') || lower.includes('statute')) return 'statute';
	if (lower.includes('regulation') || lower.includes('cfr') || lower.includes('rule')) return 'regulation';
	if (lower.includes('bill') || lower.includes('sb ') || lower.includes('ab ') || lower.includes('hr ')) return 'bill';
	if (
		lower.includes('v.') ||
...
async function ensureMinIOBucket(): Promise<void> {
	...
	try {
		...
		console.warn(`  ⚠ MinIO unavailable — uploads will be skipped (${err instanceof Error ? err.message : err})`);
	}
}
...
async function uploadToMinIO(objectKey: string, buffer: Buffer): Promise<boolean> {
	if (DRY_RUN) { console.log(`  [DRY_RUN] Would upload: ${MINIO_BUCKET}/${objectKey}`); return true; }
	try {
...
async function upsertToQdrant(points: Array<{
	id: string;
	vector: number[];
...
async function indexPdf(filepath: string, filename: string): Promise<'indexed' | 'skipped'> {
	...
	if (!chunks || chunks.length === 0) {
		...
		chunks = texts.map((text, i) => ({
			text, chunkIndex: i, sectionPath: [], heading: '',
			startOffset: 0, endOffset: text.length,
			tokenCount: Math.ceil(text.length / 4), citations: [],
		}));
	}
	...
	try {
		...
		const docRes = await client.query<{ id: string }>(
			...
			RETURNING id`,
			[corpusType, jurisdictionId, title, title.slice(0, 80),
			 objectKey, hash, pageCount, DEFAULT_USER_ID]
		);
		...
		const evRes = await client.query<{ id: string }>(
			...
			RETURNING id`,
			[
				DEFAULT_CASE_ID, DEFAULT_USER_ID, `LIB-${hash.slice(0, 8).toUpperCase()}`, title,
				`Auto-indexed from lawpdfs/${filename} (${corpusType}, ${jurisdictionCode})`,
				objectKey, fileStat.size, hash,
				JSON.stringify(tags),
				JSON.stringify({ corpusType, jurisdiction: jurisdictionCode, tags,
					minioKey: objectKey, sourceHash: hash, libraryDocId: docId }),
			]
		);
		...
		for (let i = 0; i < chunks.length; i++) {
			...
			const nodeRes = await client.query<{ id: string }>(
				...
				RETURNING id`,
				[docId, versionId || null, nType, String(i), heading,
				 chunk.citations?.[0] ?? null, nodePath,
				 chunk.sectionPath?.length ?? 0, chunk.text, chunk.text]
			);
		...
		console.log(`  Committed: docId=${docId}, ${chunks.length} nodes/chunks` +
	...
	for (let i = 0; i < nodeRows.rows.length; i++) {
		...
		qdrantBatch.push({
			...
			vector,
			payload: {
				chunk_id: row.id, document_id: docId,
				corpus_type: corpusType, jurisdiction: jurisdictionCode, tags,
...
async function main(): Promise<void> {
	...
	console.log('  Vectors: Qdrant legal_library_chunks + pgvector (768-dim)');
