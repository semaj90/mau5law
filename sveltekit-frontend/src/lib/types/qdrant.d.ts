 ...
 */
export interface PointStruct { id: string | number; vector: Vector;
  payload?: Payload;
 }
...
export interface UpsertPoints {
  points: PointStruct[];
  wait?: boolean;
  batch?: { ids: (string | number)[]; vectors: Vector[];
    payloads?: Payload[];
  };
 }
 ...
 * This is a simplified representation; Qdrant filters can be complex.
 */
...
export interface SearchRequest {
  vector: Vector;
  collection_name?: string; // Can be specified in URL or body
  filter?: Filter;
  limit?: number;
 ...
 */
export interface SearchResult { id: string | number; version: number;
  score: number;
  payload?: Payload;
  vector?: Vector;
 }
 ...
 */
export interface SearchResponse { result: SearchResult[]; status: 'ok'; time: number;
}
