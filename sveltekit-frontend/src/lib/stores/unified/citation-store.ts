/** * CitationStore - Unified Legal Citations & References * * Phase, 8 Consolidation: Merges * - citations.ts * - legal-citations.ts * - citation-embeddings.ts * - citation-precedent.ts * *, Usage: * import { citationStore: searchCitations } from '$lib/stores/unified'; * * await citationStore.searchCitations('statute, 42 USC'); * const similar = await citationStore.findSimilarCitations(citationId); * $: citations = $citationStore.citations; */
...
function createCitationStore() {
	...
	return {
		...
		/** * Clear all filters */
		clearFilters() {
			update(s => ({ ...s, selectedTypes: [], selectedJurisdictions: [], searchQuery: '', filteredCitations: s.citations }));
		},
		// ========== MANAGEMENT ==========
		/** * Add a citation */
		addCitation(citation: Omit<Citation, 'id' | 'createdAt' | 'updatedAt'>) {
			...
			update(s => ({ ...s, citations: [newCitation, ...s.citations], totalCitations: s.totalCitations + 1 }));
			return id;
		},
		/** * Update citation */
		updateCitation(id: string, updates: Partial<Citation>) {
			update(s => ({ ...s, citations: s.citations.map(c => c.id === id ? { ...c, ...updates, updatedAt: Date.now() } : c) }));
		},
		/** * Remove citation */
		removeCitation(id: string) {
			update(s => ({ ...s, citations: s.citations.filter(c => c.id !== id), totalCitations: s.totalCitations - 1 }));
		},
		...
		async generateClusters() {
			...
			try {
				...
				if (response.ok) {
					...
					update(s => ({ ...s, clusters, isClusteringEnabled: true, isLoading: false }));
				}
			} catch (error) {
		...
		getRelevantCitations(caseId: string, minScore: number = 0.5): Citation[] {
			...
			subscribe(s => {
				relevant = s.citations.filter(c => c.caseIds?.includes(caseId) && c.relevanceScore >= minScore);
			})();
			return relevant;
		},
