  ...
  function handleDrop(event: DragEvent) {
    ...
    if (files && files.length > 0) {
      ...
      if (file.type.startsWith('image/')) {
        ...
        error = 'Please drop an image file (PNG, JPG, etc.)';
      }
    }
  }
    ...
    <div class="status-card">
      ...
      <div class="status-grid">
        <div class="status-item">
          ...
          <span class="status-value" class:connected={status.system.redisConnected}>
            {status.system.redisConnected ? '✅ Connected' : '❌ Disconnected'}
          </span>
        </div>
        <div class="status-item">
          <span class="status-label">Controller:</span>
          <span class="status-value" class:connected={status.system.agenticControllerActive}>
            {status.system.agenticControllerActive ? '🟢 Active' : '🔴 Inactive'}
          </span>
        </div>
        <div class="status-item">
          <span class="status-label">AST Processing:</span>
          <span class="status-value">{status.activity.recentASTProcessing} files</span>
        </div>
        <div class="status-item">
          <span class="status-label">Pending Errors:</span>
  ...
  <div class="query-section">
    ...
    {#if fixSuggestions.length > 0}
      <div class="fix-suggestions">
        ...
        {#each fixSuggestions as fix, i}
          <div class="fix-card">
            <div class="fix-header">
              <span class="fix-relevance">Relevance: {(fix.relevance * 100).toFixed(1)}%</span>
              <span class="fix-success">Success: {(fix.successRate * 100).toFixed(1)}%</span>
            </div>
          ...
          <div class="error-item" class:resolved={errorItem.resolved}>
            <div class="error-header">
              ...
              <span class="error-status">
                {errorItem.resolved ? '✅ Resolved' : '🔄 Pending'}
              </span>
              <span class="error-date">
                {new Date(errorItem.createdAt).toLocaleDateString()}
              </span>
            </div>
...
<style>
  ...
  .refresh-btn {
    ...
    border: 1px solid var(--nier-accent-cool, #00ccff);
    color: var(--nier-accent-cool, #00ccff);
