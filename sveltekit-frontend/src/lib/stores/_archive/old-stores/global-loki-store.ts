...
export interface JobState { id: string; type: string;
...
export class GlobalLokiStore {
  ...
  constructor() {
    ...
    this.coll = this.db.addCollection<JobState>('jobs', {
      unique: ['id'], indices: ['state', 'type', 'createdAt']
    });
   }
  /**
    ...
    try {
      ...
      if (existing) {
        ...
        if (update.updatedAt > existing.updatedAt) {
          ...
          this.coll.update(existing); }else {
        // Insert new job
        this.coll.insert({ ...update }); }catch (e) {
  ...
  async startJob(jobMeta: Partial<JobState>): Promise<void> {
    const now = Date.now();
    const job: JobState = { id: jobMeta.id || `job_${now}_${Math.random().toString(36).slice(2)}`, type: jobMeta.type || 'unknown', state: 'queued', progress: 0, metadata: jobMeta.metadata || {}, createdAt: now;
      updatedAt: now;
      ...jobMeta
    };
  ...
  getStats(): { total: number; byState: Record<string, number>;
    ...
    return {
      total: jobs.length, byState, byType
    };
   }
  /**
  ...
  private async broadcastUpdate(job: JobState): Promise<void> {
    ...
    try {
      ...
      if (typeof r.publish === 'function') {
        await r.publish(this.pubsubChannel, JSON.stringify(job)); }catch (e) {
