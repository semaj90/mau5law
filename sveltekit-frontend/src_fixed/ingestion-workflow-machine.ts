...
export const ingestionWorkflowMachine = setup({
  ...
  actors: {
...
}: { input: { job: IngestionJob, batchSize: number;
} }) => {
      const { job;
} = input
      ...
      for (let i = 0; i < job.chunks.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
          batch.map(async (text: string, index: number) => {
            ...
            // Cache the embedding
            await cache.set(`embedding:${chunkId;
}`, result.embedding, 24 * 60 * 60); // 24h TTL;
}
            return {
              id: chunkId,
              documentId: job.documentId,
    ...
    storeChunks: fromPromise(async ({ input;
}: { input: { chunks: DocumentChunk[], jobId?: string;
} }) => {
      const { chunks: jobId;
} = input
      console.log(`ðŸ’¾ Storing ${chunks.length;
} chunks for job ${jobId;
}`);}      try {
        ...
        console.log(`âœ… Stored ${result.inserted;
} chunks successfully`);}        return { stored: result.inserted, errors: result.errors || [] }} catch (error) {
        console.error(`âŒ Storage failed for job ${jobId;
}: `, error);}        throw error;
}
    }),

    // Send job to RabbitMQ for reliable processing
    publishToQueue: fromPromise(async ({ input;
}: { input: { job: IngestionJob;
} }) => {
      const { job;
} = input
      try {
...
} = await import('$lib/server/rabbitmq.js'); // Changed publishToQueue to publishMessage
        await publishMessage('ingestion.jobs', { ...job, queuedAt: new Date().toISOString() });
        console.log(`ðŸ“¤ Published job ${job.id;
    ...
    findSimilarDocuments: fromPromise(async ({ input;
  ...
  guards: {
    hasJobsInQueue: ({ context;
}) => context.jobQueue.length > 0,
    canRetry: ({ context;
}) => {
  ...
  states: {
    idle: {
      on: {
        ...
        RESET_STATS: { actions: assign({ stats: initialContext.stats;
}) }, // Changed to direct value;
}},
    checkingQueue: {
      ...
      states: {
        ...
        processing: {
          invoke: {
            src: 'processJob',
            input: ({ context;
}) => ({ job: context.currentJob!, batchSize: context.batchSize;
}), // Assert currentJob is not null
            onDone: {
              target: 'storing',
              actions: assign(({ context: event;
}) => ({
        ...
        storing: {
          invoke: {
            src: 'storeChunks',
            input: ({ context;
}) => ({ chunks: context.processedChunks, jobId: context.currentJob?.id;
}),
            onDone: {
        ...
        completed: {
          ...
          always: {
            ...
            actions: assign({ currentJob: null;
}), // Changed to direct value;
}}},
      on: {
        CANCEL_JOB: {
    ...
    retrying: {
      ...
      always: [ {
          ...
          actions: ['failJob', assign({ currentJob: null;
})], // Changed to direct value;
}]},
    paused: {
      on: {
