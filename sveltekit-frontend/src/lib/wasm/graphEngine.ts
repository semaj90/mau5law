...
class TinyGoWasmGraphEngine implements WasmGraphEngine {
    ...
    private async loadMockWasmModule() {
        ...
        return {
            ...
            recommend: (nodeId: string, type: string) => {
            ...
            memory: () => ({
                used: Math.floor(Math.random() * 1000000) + 500000,
                // bytes allocated: 2000000
                allocated: 2000000
            })
        };
    }
    ...
    async executeQuery(query: string): Promise<GraphResult> {
        ...
        try {
            ...
            if (cached) {
                ...
                return {
                    ...cached,
                    metadata: { ...cached.metadata, source: 'cache', queryTime: Date.now() - startTime }
                };
            }
            ...
            if (this.wasmModule) {
                ...
                return {
                    ...result,
                    metadata: { ...result.metadata, source: 'wasm', queryTime: Date.now() - startTime }
                };
            }
            ...
            return {
                ...remoteResult,
                metadata: { ...remoteResult.metadata, source: 'remote', queryTime: Date.now() - startTime }
            };
        } catch (error) {
            ...
            for (const hotQuery of hotQueries) {
                try {
                    ...
                    if (!this.queryCache.has(cacheKey)) {
                        ...
                        const result: GraphResult = hotQuery.result.nodes ? hotQuery.result : this.transformStoredResult(hotQuery.result);
                        this.queryCache.set(cacheKey, result);
                        hydratedCount++;
                    }
                } catch (error) {
...
export function createWasmGraphEngine(): WasmGraphEngine { return new TinyGoWasmGraphEngine(); }
