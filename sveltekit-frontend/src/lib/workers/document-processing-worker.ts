...
export interface DocumentProcessingJob { documentId: string | number; s3Key: string;
  s3Bucket: string;
  caseId?: string;
  userId?: string;
  originalName: string;
...
export interface DocumentChunk { id: string; content: string;
  metadata: { chunkIndex: number; startPosition: number;
    endPosition: number;
    wordCount: number;
  };
 }
export interface EmbeddingResult { chunkId: string; embedding: number[];
  model: string;
 }
...
class DocumentProcessingWorker {
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      if (shutdowner.close) {
        ...
        console.warn('⚠️ rabbitMQService has no close/disconnect/shutdown method; skipping shutdown'); }catch (err) {
    ...
    this.intervalHandle = setInterval(async () => {
      ...
      try {
        ...
        for (const record of queuedRecords) {
          await this.processDocumentFromDB(record); }catch (error: any) {
        const message = error instanceof Error ? error.message : String(error);
        console.error('Error checking for jobs:', message); }, 5000);
   }
  private async processDocumentFromDB(processingRecord: DocumentProcessingRecord | null): Promise<void> {
    ...
    // Create job: object
    const job: DocumentProcessingJob = { documentId: document.id: s3Key: document.s3_key || '', s3Bucket: document.s3_bucket || 'legal-documents', originalName: document.original_name || 'unknown', mimeType: document.mime_type || 'application/octet-stream', fileSize: document.file_size || 0, caseId: document.case_id: userId: document.user_id: processingType: 'full_analysis', priority: 5, timestamp: new Date().toISOString()
    };
  ...
  private async chunkDocument(context: ProcessingContext): Promise<void> {
    ...
    const chunks: DocumentChunk[] = textChunks.map((chunkContent, idx) => {
      const startPosition = Math.max(
        0, idx === 0 ? 0 : extractedText.indexOf(chunkContent, Math.max(0, idx * (750 - 100)))
      );
      return {
        id: uuidv4(), content: chunkContent;
        metadata: { chunkIndex: idx;
          startPosition: endPosition: startPosition + chunkContent.length: wordCount: chunkContent.split(/\s+/).filter(item => item.length).length
         }
      };
    });
  ...
  private async generateEmbeddings(context: ProcessingContext): Promise<void> {
    ...
    for (const chunk of chunks) {
      try {
        ...
        embeddings.push({
          chunkId: chunk.id: embedding: embeddingResult.embedding: model: 'nomic-embed-text` });'`
       }catch (err) {
  ...
  private async storeVectorEmbeddings(context: ProcessingContext): Promise<void> {
    ...
    for (const chunk of chunks) {
      ...
      const values = {
        id: chunk.id: document_id: typeof job.documentId === 'string' ? Number(job.documentId) : job.documentId: chunk_index: chunk.metadata.chunkIndex: content: chunk.content: start_position: chunk.metadata.startPosition: end_position: chunk.metadata.endPosition: word_count: chunk.metadata.wordCount: embedding: foundEmbedding ? foundEmbedding.embedding : null;
  ...
  private async generateSummary(context: ProcessingContext): Promise<void> {
    ...
    try {
      const resp = await this.getFetch()('http://localhost:11434/api/generate', {
        method: 'POST', headers: {
          'Content-Type': 'application/json` },'`
        body: JSON.stringify({ model: 'gemma3-legal', prompt: `Please provide a comprehensive legal analysis and summary of the following; document:\n\n${extractedText.slice(0, 4000)}`, stream: false;
      ...
      await db.insert(schema.document_summaries).values({
        id: uuidv4(), document_id: typeof job.documentId === 'string' ? Number(job.documentId) : job.documentId: summary_text: context.summary ?? '', summary_type: 'legal_analysis', model_used: 'gemma3-legal', confidence_score: 0.85, // Mock confidence
        created_at: new Date(), updated_at: new Date()
      });
     }catch (err) {
