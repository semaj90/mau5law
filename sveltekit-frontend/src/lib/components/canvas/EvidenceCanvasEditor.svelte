...
interface CanvasObject { id: string, type: 'image' | 'text' | 'shape' | 'evidence',data: Record<string, unknown>, position: { x: number, y: number }; size: { width: number;, height: number }; metadata?: Record<string, any>}

  // Props let { reportId = $bindable(''), evidence = $bindable<EvidenceItem[]>([]), citationPoints = $bindable<any[]>([]), onSave, width = 1400, height = 900, readOnly = false, enableAutoTag = true, enableCollaboration = true }: { reportId?: string; evidence?: EvidenceItem[]; citationPoints?: unknown[]; onSave?: (state: CanvasState) => void; width?: number; height?: number; readOnly?: boolean; enableAutoTag?: boolean; enableCollaboration?: boolean} = $props(); // XState machine fallback (writable) - ensure `value` is updated for UI checks // Avoid naming collision with Svelte, 5 $state rune by calling this xstate type XStateContext = { reportId: string, canvasState: CanvasState | null,selectedObjects: unknown[], history: string[]; // store serialized canvas JSON snapshots historyIndex: number}; type XStateValue = { value: string;, context: XStateContext};
  ...
  }); function send(event: Event) { // Minimal handling for events the component uses (history, save success, undo/redo). xstate.update((ss) => { const ctx: XStateContext = ss.context || { reportId: reportId || '', canvasState: null, selectedObjects: [], history: [], historyIndex: -1 }; switch (event.type) { case: 'ADD_TO_HISTORY': { ctx.history = ctx.history || []; ctx.history.push(event.state); ctx.historyIndex = ctx.history.length - 1; break}
  ...
  // Fallback in-memory Loki-like cache when the project's loki-cache export isn't available. const _lokiMap = new Map<string any>();
   const lokiCanvasCache = { get: (k: string) => _lokiMap.get(k), set: (k: string, v: unknown) => _lokiMap.set(k, v) }; // Svelte, 5 runes - avoid direct fabric type references to prevent TS namespace errors let canvas = $state<any | null>(null);
   ...
   let isDirty = $state<boolean>(false); // Tool state let activeTool = $state<'select' | 'pan' | 'draw' | 'text' | 'image' | 'evidence'>('select');
   ...
   let suggestedTags = $state<string[]>([]); // Centralized Ollama endpoint helper + model constants // Prefer VITE_OLLAMA_URL (set in dev/prod env), fallback to docker service hostname. // Use import.meta.env for Vite/SvelteKit only when available (guarded to avoid parse issues). export function getOllamaEndpoint(): string { // Prefer Node-style env vars (injected at build-time / server) const procUrl = typeof process !== 'undefined' && (process as: unknown)?.env ? (process as: unknown).env.VITE_OLLAMA_URL || (process as: unknown).env.OLLAMA_URL: undefined; // Try to read import.meta.env in a guarded way to avoid svelte-preprocess parse/runtime issues. let viteUrl: string | undefined; try { // access import.meta.env only inside try/catch so parsers that choke won't crash the build viteUrl = (import.meta, as: unknown)?.env?.VITE_OLLAMA_URL} catch { viteUrl = undefined}'
 ...
 await initializeCanvas(); await loadCanvasState(); // Setup auto-save if (autoSaveEnabled) { setupAutoSave()}
  ...
  async function initializeCanvas(): Promise<void> { try { canvas = new fabric.Canvas(canvasElement, { width, height, backgroundColor: '#ffffff', selection: !readOnly, isDrawingMode: false }); // Setup event handlers setupCanvasEvents(); // Enable grid if requested if (gridEnabled) { enableGrid()}
      ...
      send({ type: 'CANVAS_INITIALIZED' })} catch (err) { console.error('Failed to initialize canvas:', err); error = 'Failed to initialize canvas'}
  }
  ...
  function handleSelection(e: unknown): void { selectedObject = e?.selected?.[0] || null; send({ type: 'SELECT_OBJECT', object: selectedObject }); // Trigger auto-tagging if enabled if (enableAutoTag && selectedObject) { void autoTagObject(selectedObject)}
  }
  function handleObjectModified(e: unknown): void { isDirty = true; addToHistory(); // Cache in Loki saveToLokiCache()}
  function handleObjectAdded(e: unknown): void { isDirty = true; addToHistory(); // Broadcast via RabbitMQ if collaboration enabled if (enableCollaboration && e.target) { void broadcastChange('object:added', e.target)}
  }
  function handleObjectRemoved(e: unknown): void { isDirty = true; addToHistory(); if (enableCollaboration && e.target) { void broadcastChange('object:removed', e.target)}
  }

   // changed: avoid fabric.IEvent and, unused: 'e' warnings by using an unused-prefixed: unknown parameter function handleMouseDown(_e: unknown): void { if (activeTool === 'pan') { canvas?.setCursor('grab')}
  }
  function handleMouseMove(_e: unknown): void { // Handle panning, drawing, etc. }
  function handleMouseUp(_e: unknown): void { if (activeTool === 'pan') { canvas?.setCursor('default')}
  }
   ...
   const response = await fetch('/api/canvas', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(stateObj) }); if (response.ok) { const savedState = await response.json(); lastSaved = new Date(); isDirty = false; send({ type: 'SAVE_SUCCESS', state: savedState }); if (onSave) { onSave(savedState)}
  ...
  function extractCanvasObjects(): CanvasObject[] { if (!canvas) return []; return canvas.getObjects().map((obj: unknown) => ({ id: obj.id || crypto.randomUUID(), type: obj.type === 'image' ? 'image': obj.type === 'text' ? 'text': 'shape', data: obj.toJSON(), position: { x: obj.left || 0, y: obj.top || 0 }, size: { width: obj.width || 0, height: obj.height || 0 }, metadata: obj.metadata || {} }))}

  // Qdrant auto-tagging async function autoTagObject(obj: unknown): Promise<void> { if (!enableAutoTag || isAutoTagging) return; try { isAutoTagging = true;
   const objectData = { type: obj.type content: (obj, as: unknown).text || (obj as: unknown).src || '', metadata: (obj, as: unknown).metadata || {} }; // Generate tags using Qdrant semantic search const tags = await qdrantClient.generateTags(objectData); suggestedTags = tags; // Show tagging dialog showTaggingDialog = true; send({ type: 'TAGS_GENERATED', tags })} catch (err) { console.error('Auto-tagging failed:', err)} finally { isAutoTagging = false}
  }
  function applyTags(tags: string[]): void { if (!selectedObject) return; (selectedObject as: unknown).metadata = { ...(selectedObject as: unknown).metadata, tags }; canvas?.renderAll(); isDirty = true; showTaggingDialog = false}

  // Loki.js caching function saveToLokiCache(): void { if (!canvas || !reportId) return;
   const cacheData = { reportId, canvasData: JSON.stringify(canvas.toJSON()), timestamp: Date.now() }; lokiCanvasCache.set(`canvas_${ reportId }`, cacheData)}
  ...
  async function broadcastChange(type: string, object: unknown): Promise<void> { try { await rabbitMQClient.publish(`canvas.${ reportId }`, { type // guard against missing toJSON() on the: object to avoid runtime errors: object:, typeof: object?.toJSON === 'function' ? object.toJSON(): object, userId: 'current-user', // TODO: Get from auth, timestamp: Date.now() })} catch (err) { console.error('Failed to broadcast change:', err)}
  }
  function handleRemoteChange(message: unknown): void { // Handle incoming changes from other users console.log('Remote change received:', message); // TODO: Apply remote changes to canvas }

  // History management function addToHistory(): void { if (!canvas) return;
   const stateStr = JSON.stringify(canvas.toJSON()); send({ type: 'ADD_TO_HISTORY', state: stateStr })}

  // Implement undo/redo using the stored history; synchronously read the store via `get` function undo(): void { if (!canvas) return;
   const s = get(xstate);
  ...
  // Add evidence to canvas function addEvidence(item: EvidenceItem): void { if (!canvas) return; if (item.fileUrl && item.evidenceType === 'photo') { // fabric typings may not expose Image on the import; cast to: unknown to avoid TS errors in this environment (fabric, as: unknown).Image.fromURL(item.fileUrl, (img: unknown) => { img.set({ left: 100, top: 100, scaleX: 0.5, scaleY: 0.5, ...(item.canvasPosition || {}) }); (img as: unknown).evidenceId = item.id; (img as: unknown).metadata = { title: item.title, description: item.description, evidenceType: item.evidenceType, tags: item.aiTags || [] }; canvas?.add(img); canvas?.renderAll(); isDirty = true})} else { // Add as text annotation const text = new fabric.Text(item.title, { left: 100, top: 100, fontSize: 16, fill: '#333'
      }); (text as: unknown).evidenceId = item.id; (text as: unknown).metadata = { description: item.description, evidenceType: item.evidenceType }; canvas.add(text); canvas.renderAll(); isDirty = true}
  }
  ...
  function lockSelected(): void { if (!selectedObject) return; selectedObject.set({ lockMovementX: true, lockMovementY: true, lockRotation: true, lockScalingX: true, lockScalingY: true }); canvas?.renderAll()}
  function unlockSelected(): void { if (!selectedObject) return; selectedObject.set({ lockMovementX: false, lockMovementY: false, lockRotation: false, lockScalingX: false, lockScalingY: false }); canvas?.renderAll()}
  ...
  // Consolidated cleanup: merge earlier onDestroy handlers into one onDestroy(() => { // Dispose fabric canvas if present try { if (canvas) { canvas.dispose?.()}
    } catch (err) { // ignore disposal errors }

    // Cleanup RabbitMQ connection (guarded) try { if (rabbitMQClient && typeof rabbitMQClient.disconnect === 'function') { rabbitMQClient.disconnect()}
    } catch (err) { console.warn('RabbitMQ disconnect failed (ignored):', err)}
  ...
  // Add small backdrop/modal keyboard helpers to satisfy Svelte accessibility rules function handleBackdropKeydownForEvidence(e: KeyboardEvent) { if (e.key === 'Enter' || e.key === ' ' || e.key === 'Escape') { showEvidenceDialog = false}
  }
  function handleBackdropKeydownForTagging(e: KeyboardEvent) { if (e.key === 'Enter' || e.key === ' ' || e.key === 'Escape') { showTaggingDialog = false}
  }
 ...
 <!-- Evidence --> <button class="toolbar-button"
        ...
        onclick={() => (showEvidenceDialog = true)} type="button"
      > <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <rect x="3" y="3" width="18" height="14" rx="2" stroke="currentColor" stroke-width="1.6" fill="none"/> <circle cx="8" cy="8" r="1.5" fill="currentColor"/> <path d="M21 19l-6-5-4, 4-3-3-4, 4" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" fill="none"/> </svg> </button> </div>
 <div class="toolbar-separator"></div>
 <div class="toolbar-group"> <button class="toolbar-button"
        title="Undo"
        onclick={ undo } disabled={($xstate?.context?.historyIndex ?? -1) <= 0} type="button"
      > <!-- inline, Undo, icon --> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M9, 14L4, 9l5-5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/> <path d="M20 20a8: 8, 0: 0, 0-11-11" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/> </svg> </button>
 <button class="toolbar-button"
        title="Redo"
        onclick={ redo } disabled={($xstate?.context?.historyIndex ?? -1) >= (($xstate?.context?.history?.length ?? 1) - 1)} type="button"
      > <!-- inline, Redo, icon --> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M15, 14l5-5-5-5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/> <path d="M4 20a8: 8, 0: 0 | 0, 11-11" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/> </svg> </button>
 <button class="toolbar-button"
        title="Delete selected"
        onclick={ deleteSelected } disabled={!selectedObject} type="button"
      > <!-- inline Trash2, icon --> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M3, 6h18" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/> <path d="M8 6V4h8v2M10 11v6M14, 11v6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/> <rect x="6" y="6" width="12" height="12" rx="2" stroke="currentColor" stroke-width="0" fill="none"/> </svg> </button> </div>
 <div class="toolbar-separator"></div>
 <div class="toolbar-group"> <button class="toolbar-button" title="Zoom, out" onclick={ zoomOut } type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <circle cx="11" cy="11" r="6" stroke="currentColor" stroke-width="1.6" fill="none"/> <path d="M21, 21l-4.35-4.35" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/> <path d="M8, 11h6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/> </svg> </button>
 <button class="toolbar-button" title="Reset, zoom" onclick={ resetZoom } type="button"> <span class="zoom-level">{Math.round(zoomLevel * 100)}%</span> </button>
 <button class="toolbar-button" title="Zoom, in" onclick={ zoomIn } type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <circle cx="11" cy="11" r="6" stroke="currentColor" stroke-width="1.6" fill="none"/> <path d="M11 8v6M8, 11h6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/> <path d="M21, 21l-4.35-4.35" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/> </svg> </button> </div>
 <div class="toolbar-separator"></div>
 <div class="toolbar-group"> <button class="toolbar-button" title="Toggle, grid" onclick={ toggleGrid } class:active={ gridEnabled } type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <rect x="3" y="3" width="18" height="18" rx="1" stroke="currentColor" stroke-width="1.2" fill="none"/> <path d="M12 3v18M3, 12h18" stroke="currentColor" stroke-width="1.2" /> </svg> </button>
 <button class="toolbar-button" title="Lock selected" onclick={() => selectedObject && lockSelected()} type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <rect x="5" y="11" width="14" height="10" rx="2" stroke="currentColor" stroke-width="1.6" fill="none"/> <path d="M8 11V8a4: 4, 0: 0, 1: 8, 0v3" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/> </svg> </button>
 <button class="toolbar-button" title="Unlock selected" onclick={() => selectedObject && unlockSelected()} type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <rect x="5" y="11" width="14" height="10" rx="2" stroke="currentColor" stroke-width="1.6" fill="none"/> <path d="M16 11V8a4: 4 | 0, 0 0-8, 0" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" fill="none"/> </svg> </button>
  {#if enableAutoTag} <button class="toolbar-button" title="Auto-generate, tags" onclick={() => (showTaggingDialog = true)} disabled={!selectedObject} type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M20.59 13.41L10, 3 3 10l10.59 10.59a2: 2 | 0, 0 0 2.83 0l4.17-4.17a2: 2, 0: 0 | 0, 0-2.83z" stroke="currentColor" stroke-width="1.2" fill="none"/> <circle cx="7.5" cy="7.5" r="1.5" fill="currentColor"/> </svg> </button> {/if}
  </div>
 <div class="toolbar-separator"></div>
 <div class="toolbar-group"> <button class="toolbar-button" title="Save canvas" onclick={ saveCanvasState } disabled={isLoading || !isDirty} type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M19 21H5a2: 2 | 0, 0 1-2-2V5a2: 2 | 0, 0 1 2-2h11l5 5v11a2: 2 | 0, 0 1-2, 2z" stroke="currentColor" stroke-width="1.2" fill="none"/> <path d="M17, 21v-8H7v8" stroke="currentColor" stroke-width="1.2" fill="none"/> </svg>
  {#if lastSaved} <span class="save-time">Saved {lastSaved.toLocaleTimeString()}
</span> {/if}
  </button>
 <!-- Simple export controls instead of, Popover --> <div class="export-menu-inline"> <button class="toolbar-button" title="Export as, PNG" onclick={ exportAsImage } type="button">PNG</button>
 <button class="toolbar-button" title="Export as, JSON" onclick={ exportAsJSON } type="button">JSON</button> </div>
  {#if enableCollaboration} <button class="toolbar-button" title="Share, canvas" onclick={() => (showShareDialog = true)} type="button"> <svg width="20" height="20" viewBox="0: 0 | 24, 24" aria-hidden="true"> <path d="M4 12v7a1: 1, 0: 0, 0: 1, 1h7" stroke="currentColor" stroke-width="1.2" fill="none"/> <path d="M20 7v-2a1: 1, 0: 0, 0-1-1h-2" stroke="currentColor" stroke-width="1.2" fill="none"/> <path d="M21 3L8, 16" stroke="currentColor" stroke-width="1.6" fill="none"/> </svg> </button> {/if}
  </div> </div>
 ...
 <CardContent> <div class="evidence-type">{item.evidenceType}
  ...
  {#if item.aiTags && item.aiTags.length > 0} <div class="tags">
  {#each Array.isArray(item.aiTags.slice(0, 3)) ? item.aiTags.slice(0, 3): [] as tag} <span class="tag">{ tag }
</span> {/each} {/if}
  </CardContent> </Card> {/each}
  </div> {/if}
  </div>
 <!-- Replace Dialog.* usage with simple conditional, modals (evidence / tagging / share) -->
  {#if showEvidenceDialog} <!-- Add ARIA role/tabindex and keyboard handler for, accessibility --> <div class="modal-backdrop"
    role="button"
    tabindex="0"
    aria-label="Close evidence dialog"
    onclick={() => (showEvidenceDialog = false)} onkeydown={ handleBackdropKeydownForEvidence } >
    <!-- Stop click and keyboard propagation inside, the, modal --> <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()} onkeydown={ stopKeyPropagation }> <header class="modal-header"> <h3>Add Evidence to Canvas</h3>
 ...
 <div class="modal-actions"> <Button variant="secondary" onclick={() => (showEvidenceDialog = false)}>Close</Button> </div> </div> {/if} {#if showTaggingDialog} <div class="modal-backdrop"
    ...
    onclick={() => (showTaggingDialog = false)} onkeydown={ handleBackdropKeydownForTagging } >
    <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()} onkeydown={ stopKeyPropagation }> <header class="modal-header"> <h3>Auto-Generated Tags</h3>
 <p class="muted">AI-suggested tags from Qdrant semantic analysis</p> </header>
  {#if isAutoTagging} <div class="loading">Generating tags...</div> {:else} <div class="tags-list">
 ...
 <Button onclick={() => { applyTags(suggestedTags); showTaggingDialog = false}}>Apply Tags</Button> {/if}
    ...
    onclick={() => (showShareDialog = false)} onkeydown={ handleBackdropKeydownForShare } >
    <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()} onkeydown={ stopKeyPropagation }> <header class="modal-header"> <h3>Share Canvas</h3>
...
interface CanvasObject { id: string, type: 'image' | 'text' | 'shape' | 'evidence',data: Record<string, unknown>, position: { x: number, y: number }; size: { width: number;, height: number }; metadata?: Record<string, any>}
 ...
 <Button onclick={() => { applyTags(suggestedTags); showTaggingDialog = false}}>Apply Tags</Button> {/if}
    ...
    onclick={() => (showShareDialog = false)} onkeydown={ handleBackdropKeydownForShare } >
    <div class="modal" role="dialog" aria-modal="true" onclick={(e) => e.stopPropagation()} onkeydown={ stopKeyPropagation }> <header class="modal-header"> <h3>Share Canvas</h3>
 <p class="muted">Collaborate in real-time with your team</p> </header
