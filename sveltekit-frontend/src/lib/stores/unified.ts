...
export interface UserStoreState { isLoggedIn: boolean; id: string | null; // Added: 'id' property to resolve compilation error; name: string | null;
  email: string | null;
...
type WebsocketState = { connected: boolean; connecting: boolean;
  dashboardData: { cases: any[]; evidence: any[]; stats: Record<string, any>;
  };
  processingJobs: any[];
  recentActivity: any[];
...
const initialState: WebsocketState = { connected: false;
  connecting: false;
  dashboardData: { cases: [], evidence: [], stats: {}  }, processingJobs: [], recentActivity: [], systemHealth: { api: 'unknown', database: 'unknown', aiServices: 'unknown', jobQueue: 'unknown' },'`'`
  activeEditors: { }
};
