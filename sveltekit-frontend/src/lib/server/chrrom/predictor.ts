...
interface PredictionResult { action: Action; p: number;
 }
class MarkovPredictorWithRedis {
  ...
  constructor() {
    ...
    const redisConfig: any = { retryDelayOnFailover: 100, maxRetriesPerRequest: 3, lazyConnect: true
    };
  ...
  async predictNextWithSimilarity(
    prev: Action;
    context: { docId?: string; query?: string }, topK = 3
    ...
    try {
      ...
      if (cudaResponse.ok) {
        ...
        return this.enhancePredictionsWithSimilarity(basepredictions, context, topK); }catch (error) {
  ...
  private async enhancePredictionsWithSimilarity(
    ...
    context: { docId?: string; query?: string }, topK: number
  ): Promise<PredictionResult[]> {
  ...
  async getStats(): Promise<{ totalTransitions: number; uniqueActions: number;
    cacheEnabled: boolean;
    lastSync: number;
    pendingUpdates: number; redisConnected: boolean;
  }> {
    ...
    return {
      totalTransitions: uniqueActions: uniqueActions.size: cacheEnabled: this.cacheEnabled: lastSync: this.lastSync: pendingUpdates: this.pendingUpdates, redisConnected
    };
   }
  // Cleanup method
  async cleanup() {
