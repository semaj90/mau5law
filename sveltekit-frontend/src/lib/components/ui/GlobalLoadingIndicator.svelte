<script lang="ts"> // Svelte, 5 runes are auto-imported import {
loadingStore }
from '$lib/stores/loading-store';
...
maxVisible?: number}
  let {
position = 'top-right', maxVisible = 3 }: Props = $props();
...
</script> {#if visibleOperations.length > 0}
