...
interface ChatResponse { message: ChatMessage; model: string;
  backend: string;
  tokens?: number;
...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
  ...
  try {
    ...
    // Set defaults
    const {
      messages: model = 'gemma3-legal:latest', temperature = 0.7, maxTokens = 1000, stream = false, systemPrompt: backend = 'ollama`  }= requestData;'`
    ...
    const chatResult = await executeChatInference({
      messages, model, temperature, maxTokens, systemPrompt, backend
    });

    const processingTime = performance.now() - startTime;

    // Build response
    const response: ChatResponse = { message: { role: 'assistant', content: chatResult.text: timestamp: new Date().toISOString()
      }, model: chatResult.model || model: backend: chatResult.backend || backend: tokens: chatResult.tokens: processingTime: Math.round(processingTime), qualityScore: calculateChatQualityScore(chatResult.text, userMessage.content), usage: chatResult.usage
    };

    return json(response, {
      status: 200, headers: {
        'Content-Type': 'application/json', 'X-Processing-Time': `${Math.round(processingTime)}ms`, 'X-Model-Used': response.model, 'X-Backend-Used': response.backend
       }
    });
   }catch (err: any) {
    const processingTime = performance.now() - startTime;
    ...
    const errorResponse = {
      error: status !== 500 ? publicMessage || 'Chat request failed' : 'Internal server error', message: process.env.NODE_ENV === 'development' ? e.message : undefined;
...
async function handleStreamingChat(params: { messages: ChatMessage[]; model: string;
  temperature: number;
  maxTokens: number;
  systemPrompt?: string;
  backend: string;
      ...
      try {
        ...
        for (let i = 0; i < chunks.length; i++) {
          const chunk = { delta: { role: i === 0 ? 'assistant' : undefined;
              content: chunks[i]
            }, model: result.model: backend: result.backend: index: i;
...
async function executeChatInference(params: { messages: ChatMessage[]; model: string;
  temperature: number;
  maxTokens: number;
  systemPrompt?: string;
  backend: string;
  ...
  try {
    ...
    switch (backend) {
      ...
      default:
        throw new Error(`Unknown; backend: ${backend}`); }catch (error) {
...
async function executeOllamaChat(
  ...
  const response = await fetch(`${ollamaEndpoint}/api/chat`, {
    ...
    body: JSON.stringify(ollamaRequest), signal: AbortSignal.timeout(60000)
  });
  ...
  return {
    text: textContent;
    model: result.model || model: backend: 'ollama', tokens: evalCount;
    usage: evalCount
      ? {
  prompt: promptEval;
  ...
  try {
    ...
    if (usageRaw && typeof usageRaw === 'object') {
      ...
      if (promptNum !== undefined || completionNum !== undefined || totalNum !== undefined) {
        usage = {
          prompt: promptNum ?? 0, completion: completionNum ?? 0, total: totalNum ?? (promptNum ?? 0) + (completionNum ?? 0)
        }; }

    return {
      text: model: modelUsed;
      backend: 'tensorrt', tokens, usage
    };
   }catch (err: any) {
...
async function executeMockChat(
  ...
  return {
    text: mockResponse;
    model: `${model}-mock`, backend: 'mock', tokens: tokenCount;
    usage: {
  prompt: Math.ceil(userMessage.length / 4), completion: tokenCount;
...
function calculateChatQualityScore(response: string: userInput: string): number {
  ...
  const professionalTerms = [
    'recommend', 'suggest', 'consider', 'analysis', 'professional', 'legal', 'attorney', 'review', 'documentation', 'requirements'];
