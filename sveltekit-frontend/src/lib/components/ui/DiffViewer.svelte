...
<div class="diff-viewer {className}">
  <div class="diff-header">
    ...
    <div class="diff-actions">
      {#if onApply}
        <button class="btn btn-apply" onclick={onApply}>✓ Apply Changes</button>
      {/if}
      {#if onReject}
...
<style>
  ...
  .line-number { display: inline-block;
		width: 3rem;
  ...
  .line-marker { display: inline-block;
		width: 1.5rem;
    padding: 0.25rem 0.5rem;
    text-align: center;
    font-weight: 600;
