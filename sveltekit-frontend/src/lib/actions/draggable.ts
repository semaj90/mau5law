...
export interface DraggableOptions {
 ...
 onDrag?: (x: number, y: number, id?: string) => void;
...
/** * Enhanced draggable action for making elements draggable on the detective board */ export function draggable(
 node: HTMLElement,
 ...
 onDragEnd?.(currentX, currentY, id);
 }
 ...
 if (!isDragging) return;
 const mouseEvent = new MouseEvent('mouseup', { clientX: 0, clientY: 0 }); // clientX/Y don't matter much on mouseup for touch
 handleMouseUp(mouseEvent);
 }

 // Initialize
 node.style.cursor = disabled ? '' : 'grab';
 node.style.position = 'absolute'; // Add event listeners
