...
type CudaStats = {
  simd_capabilities?: Record<string, unknown>;
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const uptime = Date.now() - (stats.lastSync || Date.now());
    const transitionsPerMinute = stats.totalTransitions && uptime > 0 ? stats.totalTransitions / (uptime / 60000) : 0;

    const estimatedMemoryUsage = {
      localTransitions: (stats.uniqueActions || 0) * 50, // bytes per transition estimate
      redisKeys: (stats.uniqueActions || 0) * 100, // bytes per Redis key estimate
