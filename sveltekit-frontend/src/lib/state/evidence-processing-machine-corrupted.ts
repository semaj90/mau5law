...
const uploadFileService = fromPromise<unknown, { input: {
	file: File }>(async ({ input }) }) => {
...
const analyzeEvidenceService = fromPromise<unknown, { input: {
	file: File }>(
 async ({ input }) }) => {
...
const embedPNGService = fromPromise<unknown, { input: {
	glyphResult: GlyphResponse }>(
 async ({ input, });
 }) => {
