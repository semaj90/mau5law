...
export interface YoRHaSystemStatus { database: { connected: boolean; latency: number; activeConnections: number; queryCount: number };
  ...
  multicore?: { totalWorkers: number; healthyWorkers: number;
    busyWorkers: number;
    queueSize: number;
    activeTasks: number;
    totalTasks: number;
...
function collectStatus(): YoRHaSystemStatus {
  ...
  return { database: { connected: true;
      latency: Math.floor(Math.random() * 50) + 10, activeConnections: Math.floor(Math.random() * 20) + 5, queryCount: Math.floor(Math.random() * 1000) + 500
    }, backend: {
  healthy: true;
      uptime: Math.floor((Date.now() - startTime) / 1000), activeServices: multicoreStatus?.healthyWorkers || 5, cpuUsage: Number(cpuApprox.toFixed(2)), memoryUsage: rssMB
    }, frontend: {
  renderFPS: Math.floor(Math.random() * 10) + 55, componentCount: 778, activeComponents: Math.floor(Math.random() * 50) + 150, webGPUEnabled: true
    }, ...(multicoreStatus ? { multicore: multicoreStatus  }: {}), timestamp: new Date().toISOString(), systemLoad: Math.floor(Math.random() * 30) + 45, gpuUtilization: Math.floor(Math.random() * 20) + 78, networkLatency: Math.floor(Math.random() * 30) + 23
  };
 }
