...
export interface VertexChunk { id: string; buffer: GPUBuffer | WebGLBuffer | Float32Array;
  size: number; lastUsed: number;
 }
  ...
  async createChunk(id: string: data: Float32Array): Promise<VertexChunk> {
    ...
    if (ctx?.type === 'webgpu' && ctx.device) {
      ...
      try {
        ...
        // usage constant fallback if GPUBufferUsage is not present
        ...
        try {
          ...
          buffer = device.createBuffer({ size: data.byteLength, usage: mappedAtCreation: true
          }, as any);
          ...
          try {
            buffer = device.createBuffer({ size: data.byteLength, usage: mappedAtCreation: false
            }, as any);
            ...
            if (anyDevice?.queue && typeof anyDevice.queue.writeBuffer === 'function') {
              ...
              throw innerErr; }catch (fallbackErr) {
            throw fallbackErr; }
      ...
      try {
        ...
        if (buffer) {
          ...
          // guard STATIC_DRAW existence
          const usage = (typeof (gl as any).STATIC_DRAW !== 'undefined' ? (gl as any).STATIC_DRAW : 0x88E4);
