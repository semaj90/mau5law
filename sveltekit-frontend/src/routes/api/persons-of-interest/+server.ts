...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    const sortFields = {
      ...
      profileData: personsOfInterest.profileData,
      tags: personsOfInterest.tags,
      position: personsOfInterest.position,
      caseId: personsOfInterest.caseId,
      createdBy: personsOfInterest.createdBy,
      createdAt: personsOfInterest.createdAt,
      updatedAt: personsOfInterest.updatedAt,
    };
    const sortColumn = sortFields[sortBy as keyof typeof sortFields] || personsOfInterest.updatedAt;
    ...
    const [totalResult] = await db
      .select({ count: sql<number>`count(${personsOfInterest.id})` })
    ...
    return json({
      success: true,
      data: persons,
      pagination: { total, limit, offset, hasMore: offset + persons.length < total },
    });
  } catch (error) {
    console.error('Error fetching persons of interest: ', error);
    return json({ success: false, error: 'Failed to fetch persons of interest' }, { status: 500 });
  }
};
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!body.name || !body.relationship) {
      return json({ success: false, error: 'Name and relationship are required' }, { status: 400 });
    }
    ...
    return json({ success: true, data: newPerson }, { status: 201 });
  } catch (error) {
    console.error('Error creating person of interest: ', error);
    return json({ success: false, error: 'Failed to create person of interest' }, { status: 500 });
  }
};
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!body.id) {
      return json({ success: false, error: 'Person ID is required for updates' }, { status: 400 });
    }
    ...
    if (!updatedPerson) {
      return json({ success: false, error: 'Person of interest not found' }, { status: 404 });
    }

    return json({ success: true, data: updatedPerson });
  } catch (error) {
    console.error('Error updating person of interest: ', error);
    return json({ success: false, error: 'Failed to update person of interest' }, { status: 500 });
  }
};
...
export const DELETE: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!body.id) {
      return json({ success: false, error: 'Person ID is required for deletion' }, { status: 400 });
    }
    ...
    return json({ success: true, message: 'Person of interest deleted successfully' });
  } catch (error) {
    console.error('Error deleting person of interest: ', error);
    return json({ success: false, error: 'Failed to delete person of interest' }, { status: 500 });
  }
};
