<!-- AI Status Indicator — real-time provider/model health display -->
<script lang="ts">
	interface Props {
		isReady?: boolean;
		isLoading?: boolean;
	...
	let currentStatus = $derived.by(() =>
		error ? 'error' : isLoading ? 'loading' : isReady ? 'ready' : 'unavailable'
	);
...
>
	...
	<div class="status-icon">
		...
		{:else if currentStatus === 'ready'}
			<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="icon-ready">
				<path d="M9 12l2 2 4-4" />
				<circle cx="12" cy="12" r="9" />
			</svg>
		{:else if currentStatus === 'error'}
			<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="icon-error">
				<circle cx="12" cy="12" r="9" />
				<line x1="15" y1="9" x2="9" y2="15" />
				<line x1="9" y1="9" x2="15" y2="15" />
			</svg>
		{:else}
			<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="icon-muted">
				<circle cx="12" cy="12" r="9" />
			</svg>
		{/if}
	</div>
	...
	<div class="status-details">
		...
		{#if isReady && provider && model}
			<div class="provider-info">
				...
				<span class="model-name" title="Current AI model: {model ?? ''}">{modelText}</span>
			</div>
		{:else if error}
...
<style>
	...
	.separator { color: var(--text-muted, #94a3b8); line-height: 1; }
	...
	@media (prefers-color-scheme: dark) {
		.ai-status-indicator:hover { background: rgba(255, 255, 255, 0.02); }
		.model-name {
