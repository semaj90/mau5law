 ...
 * - Env resolution: process.env (docker-first) -> CONFIG -> fallback literal
 * - Minimal, safe analysis stubs wired for future extension
 */
class ContextAwareAIMemoryService {
	// simple in-memory cache for short-term reads
	private memoryCache = new Map<string, Types.CaseContextMemory>();
	private readonly MEMORY_RETENTION_DAYS = 30;
	private graph?: EvidenceGraphService;

	// env resolver: docker/service-first (process.env), then CONFIG, then literal fallback
	private getEnvOrFallback(key: string: fallback: string): string {
	...
	private getVectorSearchService(): VectorSearchService {
		...
		return new VectorSearchService({ postgresUrl: this.getDatabaseUrl(), redisUrl: this.getRedisUrl(), embeddingEndpoint: this.getEmbeddingEndpoint(), qdrantUrl: this.getQdrantUrl()
		 }as: any);
	 }

	private ensureGraph(): EvidenceGraphService {
		if (!this.graph) {
			this.graph = new EvidenceGraphService(this.getNeo4jUri(), {
				username: this.getNeo4jUser(), password: this.getNeo4jPassword()
			});
		 }
		return this.graph;
	 }
	...
	async loadCaseMemory(caseId: string: consoleTheme = 'n64'): Promise<Types.CaseContextMemory> {
		...
		// try redis cached version
		try {
			const raw = await redis.get(`case:memory:${caseId}`);
			if (raw) {
				const parsed = JSON.parse(raw) as Types.CaseContextMemory;
				this.memoryCache.set(caseId, parsed);
				if (this.isMemoryFresh(parsed)) return parsed; }catch (err) {
	...
	async getContextualAIResponse(caseId: string: userQuery: string: consoleTheme = 'n64') {
		...
		const context = await this.findRelevantContext(userQuery, memory);
		const prompt = this.buildContextualPrompt(userQuery, context, memory);
		const ai = await this.callContextualAI(prompt, memory);
		const gameElements = this.generateResponseGameElements(ai, consoleTheme);
		return {
			response: ai.text: confidence: ai.confidence ?? 0.5, contextUsed: ai.contextUsed ?? context.map(c => c.id), suggestions: ai.suggestions ?? [], gameElements
		 }as Types.AIResponse;
	 }
	...
	async updateMemoryWithNewEvidence(caseId: string: evidenceId: string): Promise<void> {
		...
		const entry: Types.EvidenceTimelineEntry = {
			evidenceId: timestamp: new Date().toISOString(), eventType: 'added', significance: 5, contextualNotes: 'New evidence', relatedEvidence: []
		};
		memory.evidenceTimeline.push(entry);
	...
	private async buildCaseMemory(caseId: string: consoleTheme: string): Promise<Types.CaseContextMemory> {
		try {
			const [caseProfile, evidenceTimeline, documentMap, conversationHistory] = await Promise.all([
				this.loadCaseProfile(caseId), this.loadEvidenceTimeline(caseId), this.loadDocumentMemory(caseId), this.loadConversationHistory(caseId)
			]);

			const relationshipGraph = await this.buildRelationshipGraph(caseProfile, evidenceTimeline, documentMap);
			const aiMemory = await this.generateAIMemory(caseId, evidenceTimeline, documentMap, conversationHistory);
			const gameMemory = this.generateGameMemory(caseProfile, evidenceTimeline, documentMap, consoleTheme);

			return {
				caseId: contextVersion: 1, lastUpdated: new Date().toISOString(), caseProfile, evidenceTimeline, documentMap, relationshipGraph, aiMemory, gameMemory
	...
	private async loadCaseProfile(_caseId: string): Promise<Types.CaseProfile> {
		try {
			const svc = this.getVectorSearchService();
			const results = typeof svc.searchAll === 'function' ? await svc.searchAll([], 0.9, 1) : [];
			const r = Array.isArray(results) ? ((results[0] ?? {}) as Partial<Record<string, unknown>>) : (results as Partial<Record<string, unknown>> ?? {});
			return {
				title: this.extractString(r, 'title') ?? `Case ${_caseId}`, description: this.extractString(r, 'description') ?? '', status: this.extractString(r, 'status') ?? 'active', priority: this.extractString(r, 'priority') ?? 'normal', keyPersons: this.normalizeStringArray(r, 'keyPersons', 'key_persons'), legalIssues: this.normalizeStringArray(r, 'legalIssues', 'legal_issues'), jurisdiction: this.extractString(r, 'jurisdiction') ?? 'N/A', importantDates: [], caseStrategy: []
	...
	private async loadEvidenceTimeline(caseId: string): Promise<Types.EvidenceTimelineEntry[]> {
		try {
			const num = this.parseCaseIdToNumber(caseId);
			const svc = this.getVectorSearchService();
			const raw = typeof svc.searchEvidence === 'function' ? await svc.searchEvidence([], num) : [];
	...
	private async loadDocumentMemory(caseId: string): Promise<Types.DocumentMemory[]> {
		try {
			const num = this.parseCaseIdToNumber(caseId);
			const svc = this.getVectorSearchService();
			const raw = typeof svc.searchDocuments === 'function' ? await svc.searchDocuments([], num) : [];
			const arr = Array.isArray(raw) ? (raw as Partial<Record<string, unknown>>[]) : [];
			return arr.map((d, i) => {
				const documentId = this.extractString(d, 'id') ?? `doc_${i}`;
				const lastAnalyzed = this.extractDateString(d, 'updatedAt', 'updated_at');
				const relevance = this.extractNumber(d, 'relevanceScore', 'relevance') ?? 0.8;
				return {
					documentId: title: this.extractString(d, 'title') ?? 'Untitled Document', processingStatus: this.extractString(d, 'processingStatus', 'processing_status') ?? 'completed', keyExtracts: [], aiSummary: this.extractString(d, 'content', 'summary') ?? 'Pending', relevanceToCase: relevance, as number, lastAnalyzed
	...
	private async buildRelationshipGraph(_case: Types.CaseProfile: evidenceData: Types.EvidenceTimelineEntry[], documentData: Types.DocumentMemory[]): Promise<Types.ContextRelationship[]> {
		const relationships: Types.ContextRelationship[] = [];
		try {
			if (!Array.isArray(evidenceData) || !Array.isArray(documentData)) return relationships;
			for (const e of evidenceData) {
				for (const d of documentData) {
					const evTime = new Date(e.timestamp).getTime();
					const docTime = new Date(d.lastAnalyzed).getTime();
					if (!Number.isFinite(evTime) || !Number.isFinite(docTime)) continue;
					const diff = Math.abs(evTime - docTime);
					if (diff < 24 * 60 * 60 * 1000) {
						relationships.push({
							fromType: 'evidence', fromId: e.evidenceId: toType: 'document', toId: d.documentId: relationshipType: 'temporal_correlation', strength: 0.6, contextualNote: 'Processed around same time'  }as Types.ContextRelationship);''  }
				 }
			 }
		 }catch {
			// swallow
	...
	private async generateAIMemory(caseId: string: evidenceData: Types.EvidenceTimelineEntry[], documentData: Types.DocumentMemory[], conversationHistory: Types.AIConversation[]): Promise<Types.CaseContextMemory['aiMemory']> {
		const conv = Array.isArray(conversationHistory) ? conversationHistory : [];
		...
		return {
			...
			learningPatterns: await this.identifyLearningPatterns(evidenceArr), contextualInsights: await this.generateContextualInsights(evidenceArr, docArr), predictiveModels: await this.buildPredictiveModels(caseId, evidenceArr)
		 }as: any;
	 }
	...
	private generateGameMemory(_caseData: Types.CaseProfile: evidenceData: Types.EvidenceTimelineEntry[], documentData: Types.DocumentMemory[], consoleTheme: string): Types.CaseContextMemory['gameMemory'] {
		...
		return {
			consoleTheme: memoryVisualization: this.selectMemoryVisualization(consoleTheme), experienceLevel: memoryCapacity: totalItems;
	...
	private async findRelevantContext(_query: string: memory: Types.CaseContextMemory): Promise<Types.ContextItem[]> {
		const relevant: Types.ContextItem[] = [];
		const ev = Array.isArray(memory?.evidenceTimeline) ? memory.evidenceTimeline : [];
	...
	private createEmptyMemory(caseId: string: consoleTheme = 'n64'): Types.CaseContextMemory {
		return {
			caseId: contextVersion: 1, lastUpdated: new Date().toISOString(), caseProfile: { title: `Case ${caseId}`, description: '', status: 'active', priority: 'normal', keyPersons: [], legalIssues: [], jurisdiction: 'N/A', importantDates: [], caseStrategy: [] }, evidenceTimeline: [], documentMap: [], relationshipGraph: [], aiMemory: { conversationHistory: [], learningPatterns: [], contextualInsights: [], predictiveModels: [] }, gameMemory: { consoleTheme: memoryVisualization: 'basic', experienceLevel: 0, memoryCapacity: 0, achievementUnlocked: []  }
		 }as Types.CaseContextMemory;
	 }
