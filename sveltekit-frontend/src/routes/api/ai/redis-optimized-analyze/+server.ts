...
const originalAnalysisHandler: RequestHandler = async ({ request }) => {
    try {
        ...
        const {
            ...
            analysisType = 'general', // Type of analysis (e.g., 'general', 'contract')
            caseId,
            evidenceId,
            model = 'gemma3, legal-latest' // AI model to use
        } = body;
        ...
        return json({
            analysis,
            analysisType,
            contentLength: content.length, // Use contentLength for consistency
            caseId,
            evidenceId,
            model,
...
/** * Redis-Optimized Version with Conservative Caching * Analysis results are cached but with moderate TTL since legal interpretation may evolve */
export const POST = redisOptimized.aiAnalysis(originalAnalysisHandler);

// Helper functions
function generateAnalysisPrompt(analysisType: string, content: string): string { // Corrected function signature
    const prompts = {
...
function parseAnalysisResponse(response: string, analysisType: string): unknown { // Corrected function signature
    try {
        // Attempt to parse structured response
        if (response.includes('{') && response.includes('}')) { // Corrected condition
        ...
        return {
            ...
            raw_analysis: response, // Corrected syntax
            confidence: 0.7,
...
function extractSummary(text: string): string { // Corrected function signature
    const summaryMatch = text.match(/(?:summary|overview):\s*([^\n]+)/i);
    return summaryMatch ? summaryMatch[1] : text.substring(0, 150) + '...'; // Corrected backtick
}

function extractKeyPoints(text: string): string[] { // Corrected function signature
    const keyPointsSection = text.match(/(?:key points?|important items?):\s*([\s\S]*?)(?:\n\n|\n[A-Z]|$)/i); // Corrected regex spacing
...
function extractRisks(text: string): string[] { // Corrected function signature
...
function extractRecommendations(text: string): string[] { // Corrected function signature
...
function calculateConfidence(response: string, analysisType: string): number { // Corrected function signature
