  ...
  <aside class={cn(
    ...
    <div class="flex flex-col h-full">
      ...
      <nav class="flex-1 p-6 space-y-8 overflow-y-auto">
        ...
        <div class="space-y-3">
          {#if isSidebarOpen}
            <h3 class="text-xs uppercase tracking-wider text-slate-500 font-bold mb-4 border-b border-amber-500/20 pb-2">
              Core Functions
            </h3>
          {/if}
            ...
            <div class="relative group">
              <button class="flex items-center gap-3 p-3 text-slate-400 hover:text-amber-400 transition-all duration-300 rounded-lg hover:bg-slate-800/50">
                ...
                <div class="hidden sm:block text-left">
                  <div class="text-sm font-semibold text-white">{authStore.user?.firstName || 'User'}</div>
