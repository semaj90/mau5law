	...
	chat: async ({ request }) => {
					...
					try {
						...
						processedFiles.push({
							...
							metadata: {
								...
								blockCount: doclingResult.blocks.length,
								processingTimeMs: doclingResult.processingTimeMs
							},
							keywords,
							keyPhrases
						});

						console.log(`✅ Docling analysis complete: ${doclingResult.blocks.length} blocks, ${doclingResult.pageCount} pages`);
					} catch (doclingErr) {
						console.warn(`⚠️ Docling analysis failed for ${file.name}, falling back to upload:`, doclingErr);
		...
		try {
			...
			return {
				...
				keywords: chatResult.keywords || allKeywords,
				keyPhrases: chatResult.keyPhrases || allKeyPhrases,
				suggestions: chatResult.suggestions || [],
				uploadedCount: uploaded.length,
				processedCount: processedFiles.length,
				chatImages: chatImages.map((img) => img.url)
			};
		} catch (err) {
