...
import { Filter } from "lucide-svelte";; import { onMount } from 'svelte';; // Export state let exportLoading = $state <boolean>(false); let exportError: string | null = null; let exportSuccess = $state <boolean>(false); let availableCases: Case[] = $state ([]); // Export configuration let format: 'json' | 'csv' | 'xml' = $state ('json'); let includeEvidence = $state <boolean>(true); let includeCases = $state <boolean>(true); let includeAnalytics = $state <boolean>(false); let selectedCaseIds: string[] = $state ([]); let dateFrom = $state <string>(''); let dateTo = $state <string>(''); $effect (() => { loadAvailableCases()});
