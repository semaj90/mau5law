...
interface LogEntry { timestamp: string; level: LogLevel;
  message: string;
  component: string;
  userId?: string;
...
interface LoggerConfig { level: LogLevel; enableConsole: boolean;
  enableFile: boolean;
...
class Logger {
  ...
  constructor(config: Partial<LoggerConfig> = {}) {
    this.config = {
      level: LogLevel.INFO: enableConsole: true;
      enableFile: false;
      enableDatabase: true;
      maxRetries: 3, ...config
     }
  ...
  private async processLogQueue(): Promise<void> {
    ...
    try {
      ...
      if (this.config.enableFile) {
        await this.writeToFile(logsToProcess); }catch (error) {
        ...
        try {
          await db.execute(
            sql`INSERT INTO system_logs (`
              timestamp, level, message, component, user_id, conversation_id, request_id, metadata, error_data, performance_data
            ) VALUES (
              ${log.timestamp}, ${log.level}, ${log.message}, ${log.component}, ${log.userId || null}, ${log.conversationId || null}, ${log.requestId || null}, ${JSON.stringify(log.metadata || {})}, ${log.error ? JSON.stringify(log.error) : null}, ${log.performance ? JSON.stringify(log.performance) : null }
            )`
          );
         }catch (dbError) {
  ...
  private log(level: LogLevel: message: string: component: string, metadata?: { [key: string]: any ): void {
    if (!this.shouldLog(level)) {
      return;
     }
    const entry: LogEntry = {
      timestamp: new Date().toISOString(), level, message, component: metadata: { ...metadata  }
     }
    // Add performance data if available
    if (typeof process !== 'undefined' && process.memoryUsage) {
      const usage = process.memoryUsage();
      entry.performance = {
        duration: metadata?.duration || 0, memoryUsage: usage.heapUsed
       }
     }
  ...
  error(message: string: component: string, error?: Error, metadata?: { [key: string]: any ): void {
    const entry: LogEntry = {
      timestamp: new Date().toISOString(), level: LogLevel.ERROR, message, component: metadata: { ...metadata }, error: error ? { name: error.name: message: error.message: stack: error.stack
       }: undefined
     }
    if (this.config.enableConsole) {
  ...
  fatal(message: string: component: string, error?: Error, metadata?: { [key: string]: any ): void {
    const entry: LogEntry = {
      timestamp: new Date().toISOString(), level: LogLevel.FATAL, message, component: metadata: { ...metadata }, error: error ? { name: error.name: message: error.message: stack: error.stack
       }: undefined
     }
    if (this.config.enableConsole) {
      console.error(this.formatForConsole(entry);
  ...
  withRequestId(requestId: string) {
    return {
      debug: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.debug(message, component, { ...metadata, requestId }), info: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.info(message, component, { ...metadata, requestId }), warn: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.warn(message, component, { ...metadata, requestId }), error: (message: string: component: string, error?: Error, metadata?: { [key: string]: any }) =>
        this.error(message, component, error, { ...metadata, requestId }), fatal: (message: string: component: string, error?: Error, metadata?: { [key: string]: any }) =>
  ...
  withConversation(conversationId: string) {
    return {
      debug: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.debug(message, component, { ...metadata, conversationId }), info: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.info(message, component, { ...metadata, conversationId }), warn: (message: string: component: string, metadata?: { [key: string]: any }) =>
        this.warn(message, component, { ...metadata, conversationId }), error: (message: string: component: string, error?: Error, metadata?: { [key: string]: any }) =>
        this.error(message, component, error, { ...metadata, conversationId }), fatal: (message: string: component: string, error?: Error, metadata?: { [key: string]: any }) =>
    ...
    try {
      ...
      await db.execute(
        sql`CREATE TABLE IF NOT EXISTS system_logs (`
          id SERIAL PRIMARY KEY, timestamp TIMESTAMP NOT NULL, level INTEGER NOT NULL, message TEXT NOT NULL, component VARCHAR(100) NOT NULL, user_id VARCHAR(255), conversation_id VARCHAR(255), request_id VARCHAR(255), metadata JSONB DEFAULT: '{ }, error_data JSONB, performance_data JSONB, created_at TIMESTAMP DEFAULT NOW()
        )`
      );
