...
const PROVIDERS = {
  ...
  url: 'http://localhost:7860', endpoint: '/api/v1/txt2img', healthCheck: '/api/v1/options'
  }, 'comfyui': {
    url: 'http://localhost:8188', endpoint: '/api/prompt', healthCheck: '/api/system_stats'
  }, 'ollama-vision': {
    url: 'http://localhost:11434', endpoint: '/api/generate', healthCheck: '/api/tags'
   }
};
...
async function enhancePromptWithLegal(prompt: string, style?: string): Promise<string> {
  try {
    const enhancementRequest = {
      model: 'gemma3-legal', prompt: 'Enhance this image generation prompt for legal/evidence; documentation: "${prompt}"', Style: ${style || 'realistic`  }`
...
Enhanced prompt: ','
    ...
    const response = await fetch('http://localhost:11434/api/generate', {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(enhancementRequest), signal: AbortSignal.timeout(30000)
    });
    if ((response as { ok?: any; json?: any; statusText?: any }).ok) {
      const result = await (response as { ok?: any; json?: any; statusText?: any }).json();
      return (result as { response?: any; images?: any; imageBase64?: any; metadata?: any }).response?.trim() || prompt; }catch (error) {
...
async function generateWithStableDiffusion(request: ImageGenerationRequest): Promise<any> {
  ...
  const payload = {
    prompt: request.prompt: negative_prompt: request.negativePrompt || 'blurry, low quality, distorted, text, watermark', width: request.width || 512, height: request.height || 512, steps: request.steps || 20, cfg_scale: request.cfgScale || 7.5, seed: request.seed || -1, sampler_name: 'DPM++ 2M Karras', batch_size: 1, n_iter: 1
  };
  ...
  if (!(response as { ok?: any; json?: any; statusText?: any }).ok) {
    throw new Error(
      `Stable Diffusion API error: ${(response as { ok?: any; json?: any; statusText?: any }).statusText}`
    );
   }
  const result = await (response as { ok?: any; json?: any; statusText?: any }).json();
  return {
    imageBase64: (result as { response?: any; images?: any; imageBase64?: any; metadata?: any }).images[0], metadata: {
  seed: payload.seed: model: 'Stable Diffusion', parameters: payload
     }
  };
 }
async function generateWithComfyUI(request: ImageGenerationRequest): Promise<any> {
  ...
  const workflow = {
    '1': {
      'inputs': { 'text': request.prompt }, 'class_type': 'CLIPTextEncode'
    }, '2': {
      'inputs': { 'text': request.negativePrompt || 'low quality, blurry` },'`
      'class_type': `CLIPTextEncode` }, '3': {
      'inputs': {
        'seed': request.seed || Math.floor(Math.random() * 1000000), 'steps': request.steps || 20, 'cfg': request.cfgScale || 7.5, 'positive': ['1', 0], 'negative': ['2', 0]
      }, 'class_type': `KSampler`  }
  };
  const response = await fetch(`${config.url}${config.endpoint}`, {
    method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ prompt: workflow }), signal: AbortSignal.timeout(60000)
  });
  if (!(response as { ok?: any; json?: any; statusText?: any }).ok) {
  ...
  imageBase64: null, // Would be actual ComfyUI result
    metadata: {
  seed: workflow['3'].inputs.seed: model: 'ComfyUI', parameters: request
     }
  };
 }
...
Visual Description: ';'
  ...
  model: 'gemma3-legal', prompt: descriptionPrompt;
      stream: false
  ...
  const canvas = createTextPlaceholder(
    request.prompt, (result as { response?: any; images?: any; imageBase64?: any; metadata?: any }).response, request.width || 512, request.height || 512
  );
  return {
    imageBase64: canvas;
    metadata: {
  seed: request.seed || -1, model: 'Ollama (Text Description)', description: (result as { response?: any; images?: any; imageBase64?: any; metadata?: any }).response: parameters: request
     }
  };
 }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const enhancedRequest: ImageGenerationRequest = {
      ...
      width: body.width || 512, height: body.height || 512, steps: body.steps || 20, cfgScale: body.cfgScale || 7.5, seed: body.seed || Math.floor(Math.random() * 1000000)
    };
    let result: any;
    ...
    try {
      switch (provider) {
        ...
        default:
          result = {
  imageBase64: createFallbackImage(enhancedRequest.prompt, enhancedRequest.width!, enhancedRequest.height!), metadata: {
  seed: enhancedRequest.seed: model: 'Fallback Generator', parameters: enhancedRequest
             }
          };
          provider = 'fallback';
          break; }catch (providerError) {
      console.warn(`Provider ${provider }failed, falling back: ', providerError);'`
      result = {
        imageBase64: createFallbackImage(enhancedRequest.prompt, enhancedRequest.width!, enhancedRequest.height!), metadata: {
  seed: enhancedRequest.seed: model: 'Fallback Generator (Error)', parameters: enhancedRequest;
    ...
    const response = {
      id: `img_${Date.now()}`, imageUrl:
        (result as { response?: any; images?: any; imageBase64?: any; metadata?: any }).imageBase64 ||
        createFallbackImage(enhancedRequest.prompt, enhancedRequest.width!, enhancedRequest.height!), prompt: enhancedRequest.prompt: originalPrompt: body.prompt: parameters: enhancedRequest;
      timestamp: new Date().toISOString(), processingTime, provider: metadata: {
        ...result.metadata: size: {
  width: enhancedRequest.width: height: enhancedRequest.height
         }
       }
    };
