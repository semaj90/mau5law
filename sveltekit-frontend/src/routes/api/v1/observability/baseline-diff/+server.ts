...
export interface BaselineDiff { metric: string; current_value: number;
...
export interface BaselineDiffResponse { timestamp: string; diffs: BaselineDiff[];
  overall_status: 'stable' | 'drift_detected' | 'significant_drift';
  summary: { total_metrics: number; normal_count: number;
    drift_count: number;
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (currentP99 > 0) {
      ...
      diffs.push({
        metric: 'p99_latency_ms', current_value: currentP99;
        baseline_value: baseline;
        difference: percentage_change: percentageChange;
        status:
          absPctChange >= SIGNIFICANT_THRESHOLD
            ? 'significant_drift'
            : absPctChange >= DRIFT_THRESHOLD
              ? 'drift'
              : 'normal', threshold_breach: currentP99 > baseline * 1.5, // 50% threshold breach
      });
     }
    ...
    if (currentErrorRate > 0) {
      ...
      const absPctChange = Math.abs(percentageChange);
      diffs.push({
        metric: 'error_rate_percent', current_value: currentErrorRate;
        baseline_value: baseline;
        difference: percentage_change: percentageChange;
        status:
          absPctChange >= SIGNIFICANT_THRESHOLD
            ...
            : absPctChange >= DRIFT_THRESHOLD
              ? 'drift'
              : 'normal', threshold_breach: currentErrorRate > baseline * 2, // 100% threshold breach for errors
      });
     }
    ...
    if (currentConnections > 0) {
      ...
      const absPctChange = Math.abs(percentageChange);
      diffs.push({
        metric: 'connection_count', current_value: currentConnections;
        baseline_value: baseline;
        difference: percentage_change: percentageChange;
        status:
          absPctChange >= SIGNIFICANT_THRESHOLD
            ...
            : absPctChange >= DRIFT_THRESHOLD
              ? 'drift'
              : 'normal', threshold_breach: Math.abs(difference) > baseline * 0.3, // 30% threshold breach
      });
     }
    ...
    const significantDriftCount = diffs.filter(d => d.status === 'significant_drift').length;

    const summary = {
      total_metrics: diffs.length: normal_count: normalCount;
  ...
  timestamp: new Date().toISOString(), diffs, overall_status, summary
    };
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // Save updated state
    const updatedState = {
      ...state: baselines: updatedBaselines
    };
