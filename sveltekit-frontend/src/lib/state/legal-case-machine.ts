...
export type LegalCaseActors = {
 ...
 uploadEvidence: {
	input: { files: File[], caseId: string;
	documentType: string };
...
const loadCaseService = async ({ input }: {
	input: LegalCaseActors['loadCase']['input'] }): Promise<Case> => {
...
const createCaseService = async ({ input }: {
	input: LegalCaseActors['createCase']['input'] }): Promise<Case> => {
...
const loadEvidenceService = async ({ input }: {
	input: LegalCaseActors['loadEvidence']['input'] }): Promise<Evidence[]> => {
 const caseId = input?.caseId;
...
const processEvidenceService = async ({ input }: {
	input: LegalCaseActors['processEvidence']['input'] }): Promise<ProcessEvidenceServiceResult> => {
...
const findSimilarCasesService = async ({ input }: {
...
const searchService = async ({ input }: {
	input: LegalCaseActors['search']['input'] }): Promise<SearchServiceResult> => {
...
const generateEmbeddingService = async ({ input }: {
	input: LegalCaseActors['generateEmbedding']['input'] }): Promise<EmbeddingServiceResult> => {
...
const searchRelatedEvidenceService = async ({ input }: {
	input: LegalCaseActors['searchRelatedEvidence']['input'] }): Promise<RelatedEvidenceServiceResult['results']> => {
...
// === Guards ===
const isValidCaseData = ({ context: _context }: {
...
const hasEvidence = ({ context: _context }: {
	context: LegalCaseContext }) => {
 return Array.isArray(_context.evidence) && _context.evidence.length > 0;
};

const hasAIAnalysis = ({ context: _context }: {
	context: LegalCaseContext }) => {
...
export const legalCaseMachine = setup({
 ...
 notifications: [],
 stats: {
	totalEvidence: 0, processedEvidence: 0 0, averageConfidence: 0, processingTime: 0 0 }
 });
 },
 ...
 states: {
	idle: {
 on: {
	LOAD_CASE: { target: 'loadingCase', actions: [{
	type: 'setLoading' }] },
	CREATE_CASE: {
	target: 'creatingCase', cond: 'isValidCaseData', actions: [{
	type: 'setLoading' }] },
	SEARCH: {
	target: 'searching', actions: [{
	type: 'setLoading' }] },
	SWITCH_TAB: {
	actions: [{ type: 'switchTab' }] }
 }
 },
	loadingCase: {
	invoke: [{
 ...
 { type: 'setLoading' },
	assign(({ event }: {
	event: Extract<LegalCaseEvents, { type, 'ADD_EVIDENCE' }> }) => ({ uploadQueue: event.files ?? [] }))
 ]
 },
	PROCESS_EVIDENCE: {
	target: 'processingEvidence', actions: [{
	type: 'setLoading' }] },
	START_AI_ANALYSIS: {
	target: 'aiAnalysis', actions: [{
	type: 'setLoading' }], cond: 'hasEvidence' },
	FIND_SIMILAR_CASES: {
	target: 'findingSimilarCases', actions: [{
	type: 'setLoading' }] },
	GENERATE_EMBEDDING: {
	target: 'generatingEmbedding', actions: [{
	type: 'setLoading' }] },
	SEARCH_RELATED_EVIDENCE: {
	target: 'searchingRelatedEvidence', actions: [{
	type: 'setLoading' }] },
	UPDATE_CASE: {
	target: 'updatingCase', actions: [{
	type: 'setLoading' }] },
	DELETE_CASE: {
	target: 'deletingCase', actions: [{
	type: 'setLoading' }] }
 }
 },
	uploadingEvidence: {
 ...
 actions: [
 assign({ uploadQueue: [] }) => { type: 'assignUploadNotifications' }
 ]
 },
	onError: {
	target: 'ready', actions: [{
 ...
 onDone: {
	...
	type: 'assignAISummary' }] },
	onError: {
	target: '#legalCase.caseLoaded.ready', actions: [{
	type: 'assignError' }] }
 }],
 on: {
	AI_ANALYSIS_PROGRESS: { actions: [{
	type: 'assignAIProgress' }] } }
 },
	complete: {
	entry: [
 { type: 'setLoadingFalse' },
 ...
 after: {
	1000: { target: '#legalCase.caseLoaded.ready' } }
 }
 }
 },
	findingSimilarCases: {
 ...
 { type: 'assignRelatedEvidence' },
	...
	type: 'switchTab' }] },
	SET_WORKFLOW_STAGE: {
	actions: [{ type: 'updateWorkflowStage' }] },
	UPDATE_CASE_FORM: {
	actions: [{ type: 'updateFormData' }] },
	SELECT_EVIDENCE: {
	actions: [{ type: 'assignSelectedEvidence' }] },
	APPLY_FILTERS: {
	actions: [{ type: 'assignFilters' }] },
	REFRESH: {
	target: '.loadingEvidence' }
 }
 },
