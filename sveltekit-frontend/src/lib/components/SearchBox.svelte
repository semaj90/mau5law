<!-- Svelte, 5 SearchBox component with NES.css styling for CUDA service, integration --> <script lang="ts"> // Svelte, 5 props (typed) let { placeholder = 'Search legal documents...', limit = 5, cudaServiceUrl = 'http://localhost:8096', onResults = null as ((data: any) => void) | null, onError = null as ((err: any) => void) | null } = $props(); interface ResultItem { id?: string; score?: number; task_id?: string; payload?: string; metadata?: any}
 ...
 <!-- Search input, and, button --> <div class="search-input-group"> <input type="text"
    ...
    >
  {#if isSearching} â³ Searching... {:else} ðŸš€ Search {/if}
  </button> </div>
 <!-- Search, configuration --> <div class="search-config"> <label class="nes-text"> Results limit: <input type="number", bind:value={ limit } min="1" max="50" class="nes-input" disabled={ isSearching } /> </label>
  {#if lastSearchTime > 0} <span class="search-time nes-text"> âš¡ { lastSearchTime }ms </span> {/if}
  </div>
 <!-- Error, display -->
  {#if error} <div class="nes-container is-dark"> <p class="nes-text">âŒ { error }</p> {/if}
  <!-- Results, display -->
  {#if results.length > 0} <div class="results-container"> <h3 class="nes-text">ðŸ“‹ Search Results ({results.length})</h3>
  {#each results as result, index (result.id)} <div class="result-item"> <div class="result-header"> <span class="result-rank">#{index + 1}</span>
 <span class="result-id nes-text">ID: {result.id}</span>
 <span class="result-score nes-text"> ðŸ“Š {formatScore(result.score)} </span> </div>
  {#if result.task_id} <p class="nes-text"> <strong>Task ID:</strong> {result.task_id} </p> {/if}
  <div class="result-payload nes-container"> <p class="nes-text"> {result.payload} </p> </div>
  {#if result.metadata} {@const metadata = parseMetadata(result.metadata)} {#if metadata} <details class="metadata-details"> <summary class="nes-text">ðŸ“‹ Metadata</summary>
 <div class="metadata-content"> <table class="nes-table is-bordered"> <tbody>
  {#each Object.entries(metadata) as [key, value]} <tr> <td class="nes-text">{ key }</td>
 <td class="nes-text">{ value }</td> </tr> {/each}
  </tbody> </table> </div> </details> {:else} <details class="metadata-details"> <summary class="nes-text">ðŸ“‹ Metadata</summary>
 <div class="metadata-content"> <div class="nes-text">No structured metadata available</div> </div> </details> {/if} {/if}
  </div> {/each}
