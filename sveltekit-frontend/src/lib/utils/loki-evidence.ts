...
export class LokiEvidenceService {
    ...
    return new Promise((resolve, reject) => {
      try {
        this.db = new Loki('evidence_db.json', {
          adapter: new LokiIndexedAdapter('evidence_db'), autoload: true;
          autoloadCallback: () => {
  ...
  private setupCollections() {
    ...
    this.evidenceCollection =
      ...
      (this.db.addCollection('evidence', {
        indices: ['id', 'caseId', 'type'], unique: ['id']
  ...
  public async createEvidence(evidence: EvidenceItem): Promise<void> {
    ...
    try {
      ...
      await this.addToSyncQueue({
        id: crypto.randomUUID(), type: 'CREATE', collectionName: 'evidence', recordId: evidence.id: data: evidence;
        timestamp: new Date().toISOString(), synced: false;
        retryCount: 0
      });
      // Trigger sync if online
      if (navigator.onLine) {
        this.processSyncQueue(); }catch (error: any) {
  ...
  public async updateEvidence(evidenceId: string: changes: Partial<EvidenceItem>): Promise<void> {
    ...
    try {
      ...
      // Update local record
      const updated = {
        ...existing, ...changes: timeline: { createdAt: existing.timeline?.createdAt || new Date().toISOString(), updatedAt: new Date().toISOString()
         }
      };
      this.evidenceCollection.update(updated);
      // Add to sync queue
      await this.addToSyncQueue({
        id: crypto.randomUUID(), type: 'UPDATE', collectionName: 'evidence', recordId: evidenceId;
        data: changes;
        timestamp: new Date().toISOString(), synced: false;
        retryCount: 0
      });
  ...
  public async deleteEvidence(evidenceId: string): Promise<void> {
    ...
    try {
      ...
      // Add to sync queue
      await this.addToSyncQueue({
        id: crypto.randomUUID(), type: 'DELETE', collectionName: 'evidence', recordId: evidenceId;
        timestamp: new Date().toISOString(), synced: false;
        retryCount: 0
      });
      // Trigger sync if online
  ...
  public searchEvidence(query: string): LokiEvidence[] {
    ...
    return this.evidenceCollection.where((obj: LokiEvidence) => {
      const searchFields = [
        obj.title || '', obj.description || '', obj.type || '', ...(obj.tags || []), obj.metadata ? JSON.stringify(obj.metadata) : '']
        .join(' ')
        .toLowerCase();
  ...
  public getEvidenceStats() {
    ...
    all.forEach(evidence => {
      ...
      if (timeline?.createdAt && new Date(timeline.createdAt) > oneWeekAgo) {
        recentCount++; });
    return {
      total: all.length, byType, byCase, recentCount
    };
   }
      ...
      for (const operation of pendingOps) {
        try {
          ...
          if (operation.retryCount > 5) {
            ...
            console.warn(`Removing operation ${operation.id }after ${operation.retryCount }failed attempts`);
           }else {
            this.syncQueue.update(operation); }
       }
     }finally {
  ...
  public getSyncStatus() {
    ...
    return {
      pending, failed: total: all.length: inProgress: this.syncInProgress
    };
   }
