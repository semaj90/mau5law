# 📦 Module Import Fixes Script - Fixed Version
...
foreach ($file in $allFiles) {
    try {
        ...
        # Fix import/export patterns - Use \x22 for quotes to avoid issues
...
foreach ($file in $allFiles) {
    try {
        ...
        # Fix bits-ui imports
        $content = $content -replace 'import\s*\{\s*([^}]+)\s*\}\s*from\s*["''`]bits-ui/dist/[^"''`]*["''`]', 'import { $1 } from "bits-ui"'

        # Fix melt-ui imports
        $content = $content -replace 'import\s*\{\s*([^}]+)\s*\}\s*from\s*["''`]melt-ui["''`]', 'import { $1 } from "@melt-ui/svelte"'

        # Fix drizzle-orm imports
        $content = $content -replace 'import\s*\{\s*([^}]+)\s*\}\s*from\s*["''`]drizzle-orm["''`]', 'import { $1 } from "drizzle-orm"'
        $content = $content -replace 'import\s*\{\s*([^}]+)\s*\}\s*from\s*["''`]drizzle-orm/pg["''`]', 'import { $1 } from "drizzle-orm/pg-core"'

        # Fix XState imports
        $content = $content -replace 'import\s*\{\s*([^}]+)\s*\}\s*from\s*["''`]xstate["''`]', 'import { $1 } from "xstate"'

        # Fix Tailwind imports
        $content = $content -replace 'import\s+["''`]tailwindcss["''`]', 'import "tailwindcss/tailwind.css"'
