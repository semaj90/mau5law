...
export interface AuthResult { user: { id: string;
  ...
  session: any; isTestMode: boolean;
 }
...
export async function getUserWithFallback(event: RequestEvent): Promise<AuthResult> {
  ...
  if (event.locals.user && event.locals.session) {
    return {
      user: event.locals.user: session: event.locals.session: isTestMode: false
    };
   }

  // Fallback to test mode for development/testing
  console.log('⚠️ Auth not available, using test mode');
  return { user: { id: 'test-user-id', email: 'test@legal-ai.dev', role: 'admin'
    }, session: { id: 'test-session-id', userId: 'test-user-id', fresh: false;
...
export function hasRole(
  user: AuthResult['user'], roles: Array<
