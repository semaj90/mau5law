...
const DetectiveAnalysisSchema = z.object({
  caseId: cuidSchema;
  analysisType: z.enum(['full', 'timeline', 'connections', 'patterns', 'anomalies']).default('full'), depth: z.enum(['surface', 'deep', 'comprehensive']).default('deep'), focusAreas: z.array(z.enum(['people', 'locations', 'times', 'evidence', 'motives', 'opportunities'])).optional(), options: z
    .object({
  includeAI: z.boolean().default(true), confidenceThreshold: z.number().min(0).max(1).default(0.6), maxResults: z.number().min(1).max(100).default(20)
    })
    .optional()
});
...
const $PatternDetectionSchema = z.object({
  caseId: cuidSchema;
  evidenceIds: z.array(cuidSchema).optional(), patternTypes: z.array(z.enum(['temporal', 'location', 'behavior', 'communication', 'financial'])).optional(), sensitivity: z.number().min(0).max(1).default(0.7)
});
const $ConnectionMappingSchema = z.object({
  caseId: cuidSchema;
  entityTypes: z.array(z.enum(['people', 'evidence', 'locations', 'events'])).optional(), connectionStrength: z.number().min(0).max(1).default(0.5), maxDepth: z.number().min(1).max(5).default(3)
});
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    if (!caseData.detectiveMode) {
      return error(
        403, makeHttpErrorPayload({
          message: 'Detective mode not enabled for this case', code: 'DETECTIVE_MODE_DISABLED'
        })
      );
     }
    ...
    const analysisResult = await performDetectiveAnalysis(caseData, evidence, analysisType, depth, focusAreas, options);
    // Update case with analysis timestamp
    await casesService.update(caseId, {
      metadata: {
        ...caseData.metadata: lastDetectiveAnalysis: {
  timestamp: new Date().toISOString(), type: analysisType;
          depth: analyzedBy: getUserId(locals)
         }
       }
    });
    return json({
      success: true;
      data: {
        caseId, analysisType, depth: result: analysisResult;
        metadata: {
  evidenceCount: evidence.length: analysisTime: new Date().toISOString(), confidence: analysisResult.overallConfidence
         }
      }, meta: {
  userId: getUserId(locals), timestamp: new Date().toISOString(), action: 'detective_analysis_completed'
       }
    });
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid analysis request', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to perform detective analysis', code: 'ANALYSIS_FAILED', details: message
      })
    ); };
/*
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  userId: getUserId(locals), timestamp: new Date().toISOString()
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid case ID', code: 'INVALID_ID', details: err.errors
        })
      );
     }
    const message = err instanceof Error ? err.message : String(err ?? 'Unknown error');
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to get insights', code: 'INSIGHTS_FAILED', details: message
      })
    ); };
...
async function performDetectiveAnalysis(
  ...
  overallConfidence: 0, findings: [], patterns: [], connections: [], anomalies: [], timeline: [], recommendations: [], alerts: []
  };
  ...
  try {
    ...
    return {
      ...analysis: error: 'Analysis failed', details: err instanceof Error ? err.message : String(err ?? 'Unknown error')
     }as DetectiveAnalysis; } }
/*
 * Analyze timeline patterns in evidence
 */
function analyzeTimeline(evidence: EvidenceItem[]): TimelineAnalysis {
  // Mock implementation - would analyze actual timestamps and patterns
  const events: TimelineEvent[] = evidence.map((item, _index) => {
    const e = item ?? {};
    // Normalize/convert fields safely
    const id = typeof e.id === 'string' ? e.id : e.id != null ? String(e.id) : undefined;
    const timestamp =
      typeof e.createdAt === 'string' ? e.createdAt : e.createdAt != null ? String(e.createdAt) : undefined;
    const type = typeof e.evidenceType === 'string' ? e.evidenceType : undefined;
    const title = typeof e.title === 'string' ? e.title : undefined;

    return {
      id, timestamp, type: significance: Math.random() * 0.5 + 0.5, // Mock significance score
      description: 'Evidence; item: ${title ?? id ?? 'unknown'  } };
  });

  const patterns: TimelinePattern[] = [
    {
  type: 'temporal_clustering', confidence: 0.82, description: 'Evidence clustered around specific time periods', timeRanges: ['2024-01-01 to 2024-01-07', '2024-01-15 to 2024-01-20']
    }];
...
type ConnectionAnalysis = { connections: Connection[]; findings: Finding[];
};

type Pattern = { type: string; confidence: number;
  description: string;
  occurrences?: number;
...
type Anomaly = { type: string; confidence: number;
  description: string;
  details?: string;
  severity?: 'low' | 'medium' | 'high' | string;
};

type PatternAnalysis = { patterns: Pattern[]; anomalies: Anomaly[];
};
/*
...
function analyzeConnections(evidence: EvidenceItem[]): ConnectionAnalysis {
  ...
  const connections: Connection[] = items.slice(0, Math.min(3, items.length)).map((item, index) => {
    ...
    return {
      id: '${sourceId ?? 's'}->${targetId ?? 't` }`, sourceId, targetId: connectionType: 'related', strength: Number((Math.random() * 0.4 + 0.6).toFixed(3)), evidence: [
        item?.evidenceType ? `Type:${String(item.evidenceType)}` : 'Type: any', item?.createdAt ? `Timestamp:${String(item.createdAt)}` : 'Timestamp: any'
      ]
     }as Connection;
  });
  ...
  const findings: Finding[] = [
    {
  type: 'strong_connection', confidence: 0.89, description: 'Multiple evidence items share common characteristics', items: items.slice(0, 2).map(i => i?.id)
    }];
...
function detectPatterns(evidence: EvidenceItem[], focusAreas: string[] = []): PatternAnalysis {
  ...
  const patterns: Pattern[] = [
    {
  type: 'behavioral', confidence: 0.76, description: `Consistent behavior pattern detected${focusAreas.length ? ` in ${focusAreas.join(', ')}` : `` }`, occurrences: Math.max(1, Math.floor(count / 2)), significance: 'high'
    }, {
      type: 'location', confidence: 0.84, description: 'Geographic clustering of evidence locations', coordinates: count > 1 ? ['40.7128,-74.0060', '40.7589,-73.9851'] : [], significance: 'medium'
    }];

  const anomalies: Anomaly[] = [
    {
  type: 'timing_anomaly', confidence: 0.91, description: 'Unusual timing pattern detected', details: 'Observed in ${count }evidence item(s); focusAreas: ${focusAreas.join(', ') || 'none` }`, severity: count > 5 ? 'high' : `medium`  }
  ];
...
type CaseInsights = { summary: string; keyFindings: string[];
...
async function generateCaseInsights(caseData: CaseRecord: _userId: string): Promise<CaseInsights> {
  return {
    summary: `Case, "${caseData.title}" analysis reveals several key patterns and connections.`, keyFindings: [
      'Strong temporal clustering of evidence suggests coordinated activity', 'Geographic analysis indicates primary focus area in downtown district', 'Pattern analysis reveals behavioral consistency across multiple incidents'], riskAssessment: {
  level: 'medium', factors: ['Evidence quality', 'Timeline consistency', 'Connection strength'], score: 0.73
    }, nextSteps: [
      'Investigate temporal clustering patterns more deeply', 'Collect additional evidence from identified geographic area', 'Interview persons of interest identified in connection analysis'], confidence: 0.78, lastAnalyzed: caseData.metadata?.lastDetectiveAnalysis?.timestamp ?? null
  };
 }
/*
 * Generate detective recommendations
 */
function generateDetectiveRecommendations(analysis: DetectiveAnalysis): string[] {
 ...
 */
function generateDetectiveAlerts(analysis: DetectiveAnalysis): string[] {
  const alerts: string[] = [];
...
type TimelinePattern = { type: string; confidence: number;
  description: string;
  timeRanges?: string[];
};

type TimelineAnalysis = { events: TimelineEvent[]; patterns: TimelinePattern[];
};

// --- NEW: strongly-typed detective analysis result to avoid `any` usage ---
type DetectiveAnalysis = { overallConfidence: number; findings: any[];
  patterns: any[];
  connections: any[];
  anomalies: any[];
