...
export const authMachine = createMachine(
  {
    ...
    states: {
      idle: {
        on: {
          START_LOGIN: { target: 'loggingIn' },
          START_REGISTRATION: { target: 'registering' },
          SESSION_EXPIRED: { target: 'loggedOut' },
        },
      },
      loggingIn: {
        invoke: {
    ...
    actors: {
      ...
      registrationService: async ({ event }) => {
        ...
        if (event.data.email && event.data.password) {
          return Promise.resolve({
            user: { id: '124', email: event.data.email, firstName: event.data.firstName },
            session: { id: 'def' },
          });
        }
