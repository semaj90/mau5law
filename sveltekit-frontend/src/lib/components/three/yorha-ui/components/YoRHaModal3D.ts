...
export class YoRHaModal3D extends YoRHa3DComponent {
  ...
  constructor(options: YoRHaModal3DOptions = {}) {
    ...
    super({
      ...
      opacity: 0,
      transform: { scale: new THREE.Vector3(0.8, 0.8, 0.8)
      } }
    });
  ...
  private createBackdrop(): void {
    ...
    switch (this.options.backdrop) {
      ...
      case, 'transparent':
        ...
        break;
      default:
        backdropMaterial = new THREE.MeshBasicMaterial({ color: YORHA_COLORS.primary.black,
          transparent: true,
          opacity: 0.5
        });
    } }
  ...
  private static getVariantStyle(variant: string, _size: string): Partial<YoRHaStyle> {
    ...
    const variantStyles = { default: { backgroundColor: YORHA_COLORS.primary.beige,
      ...
      alert: { backgroundColor: YORHA_COLORS.status.warning,
      ...
      confirm: { backgroundColor: YORHA_COLORS.primary.white,
      ...
      fullscreen: { backgroundColor: YORHA_COLORS.primary.black,
        borderColor: YORHA_COLORS.accent.gold,
        borderWidth: 0.05,
        opacity: 0.95
      },
