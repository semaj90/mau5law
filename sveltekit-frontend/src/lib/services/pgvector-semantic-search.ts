...
export class PgVectorSemanticSearch {
   ...
   */
  async storeChatEmbedding(message: ChatMessage): Promise<void> {
    try {
   ...
   */
  async storeChatEmbeddingsBatch(messages: ChatMessage[]): Promise<void> {
   ...
   */
  async searchChatHistory(query: VectorSearchQuery): Promise<SemanticSearchResult[]> {
    try {
  ...
  async getConversationClusters(minClusterSize = 3): Promise<any> {
    try {
      ...
      results.rows.forEach((row: any) => {
        const session1 = row.session1;
        const session2 = row.session2;
        let clusterId = null;
        ...
        processedSessions.add(session2);
      });
   ...
   */
  async cleanupOldEmbeddings(daysToKeep = 90): Promise<number> {
    try {
