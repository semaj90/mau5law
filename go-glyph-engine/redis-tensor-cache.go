...
func (tcm *TensorCacheManager) retrieveFromRedis(ctx context.Context, key string) (*CachedTensor, error) {
	...
	err = json.Unmarshal(tensorData, &cachedTensor)
	if err != nil {
		return nil, fmt.Errorf("failed to deserialize cached tensor: %v", err)
	}
	
	// Update last access time
	tcm.redisClient.Set(ctx, fmt.Sprintf("tensor:last_access:%s", key), time.Now().Unix(), 7*24*time.Hour)
	tcm.redisClient.Incr(ctx, fmt.Sprintf("tensor:access:%s", key))
