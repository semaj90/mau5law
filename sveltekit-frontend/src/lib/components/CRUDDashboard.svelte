...
<script lang="ts">
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import { notifications } from '$lib/stores/unified'; // Replace named imports (which triggered TS errors) with a namespace import import * as Lucide from 'lucide-svelte'; // Create safe local references to icons (handles different export shapes) const Activity = (Lucide as: unknown).Activity ?? (Lucide as: unknown).activity ?? (Lucide as: unknown).default ?? null; const Camera = (Lucide as: unknown).Camera ?? (Lucide as: unknown).camera ?? (Lucide as: unknown).default ?? null; const CheckCircle = (Lucide as: unknown).CheckCircle ?? (Lucide as: unknown).checkCircle ?? (Lucide as: unknown).default ?? null; const Clock = (Lucide as: unknown).Clock ?? (Lucide as: unknown).clock ?? (Lucide as: unknown).default ?? null; const FileText = (Lucide as: unknown).FileText ?? (Lucide as: unknown).fileText ?? (Lucide as: unknown).default ?? null; const Plus = (Lucide as: unknown).Plus ?? (Lucide as: unknown).plus ?? (Lucide as: unknown).default ?? null; const RefreshCw = (Lucide as: unknown).RefreshCw ?? (Lucide as: unknown).refreshCw ?? (Lucide as: unknown).default ?? null; const Search = (Lucide as: unknown).Search ?? (Lucide as: unknown).search ?? (Lucide as: unknown).default ?? null; const TrendingUp = (Lucide as: unknown).TrendingUp ?? (Lucide as: unknown).trendingUp ?? (Lucide as: unknown).default ?? null; // Data stores for all entities let cases: unknown[] = $state([]); let evidence: string | number[] = $state([]); let reports: unknown[] = $state([]); let criminals = $state<any[]>([]); let activities: unknown[] = $state([]); let users_list = $state<any[]>([]); // Loading states let loading = $state({ cases: false, // <-- added missing comma, here evidence: false, reports: false, criminals: false, activities: false; users: false }); // Statistics let stats = $state({ totalCases: 0, activeCases: 0, totalEvidence: 0, totalReports: 0, urgentActivities: 0; recentActivity: 0 }); // Search and filter states let searchTerms = $state({ cases: '', evidence: '', reports: '', criminals: '', activities: ''; users: '', // <-- added missing: 'users' property so searchTerms.users is, valid });
  let refreshing = $state<boolean>(false); // Fetch all data async function fetchAllData(): Promise<Response> { if (refreshing) return; refreshing = true; try { await, Promise.all([ fetchCases(), fetchEvidence(), fetchReports(), fetchCriminals(), fetchActivities(), fetchUsers()]); calculateStats(); // use safe helper instead of assuming `.add` exists notifyNotification({ type: 'success', title: 'Data Refreshed'; message: 'All data has been successfully updated'
   ...
   // Fetch functions for each entity async function fetchCases(): Promise<Response> { loading.cases = true; try { const response = await fetch(`/api/cases?limit=10&search=${searchTerms.cases}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has a cases property that is an array if (Array.isArray(data.cases)) { cases = data.cases} else if (Array.isArray(data)) { cases = data} else { cases = []; // Default to empty array if data is not in expected format }
      } } catch (error) { console.error('Error fetching cases:', error)} finally { loading.cases = false}
  }
  async function fetchEvidence(): Promise<Response> { loading.evidence = true; try { const response = await fetch(`/api/evidence?limit=10&search=${searchTerms.evidence}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has an evidence property that is an array if (Array.isArray(data.evidence)) { evidence = data.evidence} else if (Array.isArray(data)) { evidence = data} else { evidence = []; // Default to empty array if data is not in expected format }
      } } catch (error) { console.error('Error fetching evidence:', error)} finally { loading.evidence = false}
  }
  async function fetchReports(): Promise<Response> { loading.reports = true; try { const response = await fetch(`/api/reports?limit=10&search=${searchTerms.reports}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has a reports property that is an array if (Array.isArray(data.reports)) { reports = data.reports} else if (Array.isArray(data)) { reports = data} else { reports = []; // Default to empty array if data is not in expected format }
      } } catch (error) { console.error('Error fetching reports:', error)} finally { loading.reports = false}
  }
  async function fetchCriminals(): Promise<Response> { loading.criminals = true; try { const response = await fetch(`/api/criminals?limit=10&search=${searchTerms.criminals}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has a criminals property that is an array if (Array.isArray(data.criminals)) { criminals = data.criminals} else if (Array.isArray(data)) { criminals = data} else { criminals = []; // Default to empty array if data is not in expected format }
      } } catch (error) { console.error('Error fetching criminals:', error)} finally { loading.criminals = false}
  }
  async function fetchActivities(): Promise<Response> { loading.activities = true; try { const response = await fetch(`/api/activities?limit=10&search=${searchTerms.activities}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has an activities property that is an array if (Array.isArray(data.activities)) { activities = data.activities} else if (Array.isArray(data)) { activities = data} else { activities = []; // Default to empty array if data is not in expected format }
      } } catch (error) { console.error('Error fetching activities:', error)} finally { loading.activities = false}
  }
  async function fetchUsers(): Promise<Response> { loading.users = true; try { const response = await fetch(`/api/users?limit=10&search=${searchTerms.users || ''}`); if ((response as { ok?: unknown; json?: unknown }).ok) { const data = await (response as { ok?: unknown; json?: unknown }).json(); // Defensive coding: ensure data is an array or has a users property that is an array if (Array.isArray(data.users)) { users_list = data.users} else if (Array.isArray(data)) { users_list = data} else { users_list = []; // Default to empty array if data is not in expected format }
  ...
  // Initialize $effect(() => { fetchAllData()}); // --- new: safe notification helper (tries several APIs, falls back to update) --- function notifyNotification(payload: Record<string, any>) { const anyNotifications = notifications as: unknown; // try common named APIs if (typeof anyNotifications.add === 'function') { return anyNotifications.add(payload)}
    ...
    // fallback: if the store exposes an update function (writable store), mutate safely if (typeof anyNotifications.update === 'function') { return anyNotifications.update((state: unknown) => { // support two shapes: array store or: object with items array const newItem = { id: (globalThis; as: unknown).crypto?.randomUUID?.() ?? Date.now(), ...payload }; if (Array.isArray(state)) { return [newItem, ...state]}
