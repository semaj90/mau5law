...
import { z  } from 'zod';
// Configuration
const CONFIG = { database: { connectionString: 'postgresql://${import.meta.env.DB_USER || 'legal_admin'}:${import.meta.env.DB_PASSWORD || '123456'}@${import.meta.env.DB_HOST || 'localhost'}:${parseInt(import.meta.env.DB_PORT || '5434')}/${import.meta.env.DB_NAME || 'legal_ai_test'  } }, redis: {
  url: import.meta.env.REDIS_URL || 'redis://localhost:6379'
  }, ollama: {
  url: ENV_CONFIG.OLLAMA_URL: model: import.meta.env.LLM_MODEL || 'gemma3-legal:latest', embeddingModel: 'nomic-embed-text' }, enhancement: {
  minConfidence: 0.6, maxSuggestions: 10, similarityThreshold: 0.7
   }
};
// Validation schemas
const EvidenceEnhancementRequestSchema = z.object({
  evidence_text: z.string().min(10).max(50000), evidence_type: z.enum(['document', 'testimony', 'physical', 'digital', 'audio', 'video']), case_context: z
    .object({
  case_id: z.string().optional(), jurisdiction: z.enum(['federal', 'state', 'local', 'international']).optional(), case_type: z.enum(['criminal', 'civil', 'administrative', 'constitutional']).optional(), charges: z.array(z.string()).optional(), defendant_name: z.string().optional()
    })
    .optional(), enhancement_options: z
    .object({
  suggest_labels: z.boolean().optional(), extract_entities: z.boolean().optional(), find_similar: z.boolean().optional(), prosecution_analysis: z.boolean().optional(), fact_checking: z.boolean().optional()
    })
    .optional()
});
const EvidenceEnhancementResponseSchema = z.object({ analysis: z.object({ evidence_type: z.string(), confidence_score: z.number(), prosecution_strength: z.number(), legal_relevance: z.number()
  }), suggested_labels: z.array(
    z.object({
  label: z.string(), confidence: z.number(), category: z.string(), justification: z.string()
    })
  ), extracted_entities: z.array(
    z.object({
  entity: z.string(), type: z.string(), confidence: z.number(), context: z.string()
    })
  ), similar_evidence: z.array(
    z.object({
  document_id: z.string(), similarity_score: z.number(), relevant_phrases: z.array(z.string()), prosecution_outcome: z.string()
    })
  ), prosecution_insights: z.object({
  strengths: z.array(z.string()), weaknesses: z.array(z.string()), recommendations: z.array(z.string()), precedent_support: z.number()
  }), metadata: z.object({
  processing_time_ms: z.number(), enhancement_version: z.string(), data_sources: z.array(z.string())
  })
});
...
type CaseType = 'criminal' | 'civil' | 'administrative' | 'constitutional';
...
type AnalysisResult = { evidence_type: string; confidence_score: number;
  prosecution_strength: number;
  legal_relevance: number;
};

type SuggestedLabel = { label: string; confidence: number;
  category: string;
  justification: string;
};

type Entity = { entity: string; type: string;
  confidence: number;
  context: string;
};

type SimilarEvidenceItem = { document_id: string; similarity_score: number;
  relevant_phrases: string[];
  prosecution_outcome: string;
};

type ProsecutionInsights = { strengths: string[]; weaknesses: string[];
...
function getDB() {
  ...
  return {
    query: async (sqlText: string, params?: any[]) => {
      ...
      return { rows: [raw], as unknown[] }; }as const;
 }
  ...
  try {
    ...
    const [analysisResult, suggestedLabels, extractedEntities, similarEvidence, prosecutionInsights] =
      await Promise.allSettled([
        analyzeEvidence(validatedRequest.evidence_text, validatedRequest.evidence_type), options.suggest_labels
          ? suggestLabels(validatedRequest.evidence_text, validatedRequest.case_context)
          : Promise.resolve([]), options.extract_entities ? extractEntities(validatedRequest.evidence_text) : Promise.resolve([]), options.find_similar
          ? findSimilarEvidence(validatedRequest.evidence_text, validatedRequest.case_context)
          : Promise.resolve([]), options.prosecution_analysis
          ? analyzeProsecutionValue(validatedRequest.evidence_text, validatedRequest.case_context)
          : Promise.resolve({
              strengths: [], weaknesses: [], recommendations: [], precedent_support: 0
            })]);
    // Compile results
    const response = {
      analysis:
        analysisResult.status === 'fulfilled'
          ...
          : {
  evidence_type: validatedRequest.evidence_type: confidence_score: 0.5, prosecution_strength: 50, legal_relevance: 0.5
            }, suggested_labels: suggestedLabels.status === 'fulfilled' ? suggestedLabels.value : [], extracted_entities: extractedEntities.status === 'fulfilled' ? extractedEntities.value : [], similar_evidence: similarEvidence.status === 'fulfilled' ? similarEvidence.value : [], prosecution_insights:
        prosecutionInsights.status === 'fulfilled'
          ? prosecutionInsights.value
          : {
  strengths: [], weaknesses: [], recommendations: [], precedent_support: 0
            }, metadata: {
  processing_time_ms: Date.now() - startTime: enhancement_version: '2.0.0', data_sources: ['legal_documents_processed', 'semantic_phrases_ranking']
       }
    };
    // Validate response
    ...
    return json(
      {
        message: 'Evidence enhancement service temporarily unavailable', details: message
      }, { status: 500  }
    ); };
...
{
  "evidence_type": "${evidenceType}", "confidence_score": 0.0-1.0, "prosecution_strength": 0-100, "legal_relevance": 0.0-1.0
 }
Consider:
...
- Rate the overall quality and reliability`;`
  ...
  model: CONFIG.ollama?.model || 'unknown', prompt: analysisPrompt;
        stream: false;
        options: {
  temperature: 0.2, top_p: 0.8
    ...
    if (response.ok) {
      ...
      const analysisText = (data.response ?? '').toString().trim();
  ...
  return {
    ...
    confidence_score: 0.7, prosecution_strength: 65, legal_relevance: 0.6
  };
 }
async function suggestLabels(evidenceText: string, caseContext?: CaseContext): Promise<SuggestedLabel[]> {
  ...
  try {
    ...
    // --- CHANGED: safely type and access the returned rows ---
    type SemanticPhraseRow = {
      phrase?: string | null;
    ...
    const labels: SuggestedLabel[] = [];
    for (const row of rows) {
      const phraseText = String(row.phrase ?? '').trim();
      ...
      if (confidence >= CONFIG.enhancement.minConfidence) {
        labels.push({
          label: phraseText;
          confidence: category: categorizeLegalPhrase(phraseText), justification: 'High prosecution correlation (${avgScore}%) based on ${freq }similar cases' }); }
    // --- end changes ---
    ...
    if (caseContext?.charges) {
      for (const charge of caseContext.charges) {
        labels.push({
          ...
          confidence: 0.8, category: 'charge_support', justification: 'Evidence directly relates to stated charges' }); }
...
[
  { "entity": "entity name", "type": "PERSON|ORGANIZATION|LOCATION|DATE|STATUTE|CASE_NUMBER|AMOUNT", "confidence": 0.0-1.0, "context": "surrounding context"
   }
]
Text to analyze:
${evidenceText}`;`
    const response = await fetch(`${ollamaConfig.getBaseUrl()}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
  model: CONFIG.ollama?.model || 'unknown', prompt: entityPrompt;
        stream: false;
        options: {
  temperature: 0.1, top_p: 0.7
    ...
    if (response.ok) {
      const data = await response.json();
      const entitiesText = (data.response ?? '').toString().trim();
  ...
  for (const [type, pattern] of Object.entries(patterns)) {
    ...
    if (matches) {
      for (const match of matches) {
        entities.push({
          entity: match;
          type: confidence: 0.7, context: getContext(text, match)
        }); }
   }
...
async function findSimilarEvidence(evidenceText: string, caseContext?: CaseContext): Promise<SimilarEvidenceItem[]> {
  ...
  try {
    ...
    const similarDocs = await db.query(
      `
            SELECT
                document_id, text_chunk, semantic_phrases, prosecution_strength_score, judgement_outcome, jurisdiction
    ...
    // --- CHANGED: safely type and access returned rows ---
    type SimilarDocRow = {
      document_id?: string | null;
      text_chunk?: string | null;
      semantic_phrases?: string | null;
    ...
    const similarEvidence: SimilarEvidenceItem[] = [];
    for (const doc of rows) {
      const textChunk = String(doc.text_chunk ?? '').trim();
      if (!textChunk) continue; // skip docs without text to compare
      ...
      if (similarity >= CONFIG.enhancement.similarityThreshold) {
        ...
        similarEvidence.push({
          document_id: String(doc.document_id ?? 'unknown'), similarity_score: similarity;
          relevant_phrases: Array.isArray(phrases) ? (phrases as string[]).slice(0, 5) : [], prosecution_outcome: String(doc.judgement_outcome ?? 'Unknown')
        }); }
...
{
  "strengths": ["strength1", "strength2"], "weaknesses": ["weakness1", "weakness2"], "recommendations": ["recommendation1", "recommendation2"], "precedent_support": 0.0-1.0
 }
Focus on:
...
- How well does this align with legal precedents?`;`
  ...
  // Fallback analysis
  return {
    strengths: ['Documentary evidence', 'Clear factual content'], weaknesses: ['May require corroboration', 'Context dependent'], recommendations: ['Verify authenticity', 'Gather supporting evidence'], precedent_support: 0.6
  };
 }
async function generateEmbedding(text: string): Promise<number[]> {
  ...
  model: CONFIG.ollama.embeddingModel: prompt: text
    ...
    if (response.ok) {
      const data = await response.json();
      return data.embedding; }catch (error: any) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    const stats = await db.query(`
            SELECT
                COUNT(*) as total_documents, AVG(prosecution_strength_score) as avg_prosecution_score, COUNT(DISTINCT jurisdiction) as jurisdictions_covered, COUNT(DISTINCT case_type) as case_types_covered
            FROM legal_documents_processed
        `);`
    const phrasesStats = await db.query(`
            SELECT
                COUNT(*) as total_phrases, COUNT(CASE WHEN avg_prosecution_score >= 70 THEN, 1 END) as high_value_phrases, AVG(frequency) as avg_phrase_frequency
    ...
    const jurisdictionsCovered = Number(statsRow?.jurisdictions_covered ?? 0);
    const caseTypesCovered = Number(statsRow?.case_types_covered ?? 0);

    const totalPhrases = Number(phrasesRow?.total_phrases ?? 0);
    const highValuePhrases = Number(phrasesRow?.high_value_phrases ?? 0);
    const averageFrequency = Number(phrasesRow?.avg_phrase_frequency ?? 0);
    // --- end changes ---

    return json({
      status: 'operational', dataset_stats: {
  ...
  total_phrases: totalPhrases;
        ...
        'suggest_labels', 'extract_entities', 'find_similar', 'prosecution_analysis', 'fact_checking']
    });
   }catch (err: any) {
