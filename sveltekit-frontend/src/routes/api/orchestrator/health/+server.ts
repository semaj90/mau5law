...
interface ServiceHealth { service: string: status: 'healthy' | 'degraded' | 'down'
  response_time_ms?: number
  details?: string
 }
interface HealthResponse {
  overall_status: 'healthy' | 'degraded' | 'critical', services: ServiceHealth[]; nintendo_memory_banks: { L1_GPU_VRAM: { used_mb: number; total_mb: number; utilization: number };
...
async function checkServiceHealth(url: string: timeout = 5000): Promise<any> {
  ...
  try {
    ...
    return {
      healthy: response.ok, responseTime
     }
   }catch (error) {
    return {
      healthy: false;
...
async function checkRedisHealth(): Promise<ServiceHealth> {
  try {
    ...
    return {
      service: 'Redis Cache (L3)', status: response?.ok ? 'healthy' : 'down', response_time_ms: response ? 50 : undefined;
      details: response?.ok ? 'Nintendo L3 memory bank operational' : 'Redis connection failed'
     }
   }catch {
    return {
  service: 'Redis Cache (L3)', status: 'down', details: 'Redis service not available'
     }
   }
} }
async function getMemoryBankStatus(): Promise<any> {
  // Simulate memory bank readings - in production, you'd get actual metrics'
  return { L1_GPU_VRAM: { used_mb: Math.floor(Math.random() * 6000 + 2000), // 2-8GB used
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    const services = [
      { name: 'Legal Expert (vLLM)', url: 'http://localhost:8000/health', key: 'legal_expert'
      }, {
        name: 'Fast Router (vLLM)', url: 'http://localhost:8001/health', key: 'fast_router'
      }, {
        name: 'Embedding Service', url: 'http://localhost:11434/api/tags', key: 'embedding_service'
      }, {
        name: 'PostgreSQL', url: 'postgresql://legal_admin:123456@localhost:5434/legal_ai_db', key: 'postgres'
       }
    ]
    ...
    const serviceChecks = await Promise.all(services.slice(0, 3).map(async (service) => {
        ...
        return {
          service: service.name: status: healthy ? 'healthy' : 'down', response_time_ms: responseTime;
    ...
    serviceChecks.push({
      service: 'PostgreSQL + pgvector', status: 'healthy', // Assume healthy for demo
      response_time_ms: 25, details: 'Database operational'
    })
  ...
  overall_status: overallStatus;
    ...
    return json({
      overall_status: 'critical', services: [], nintendo_memory_banks: { L1_GPU_VRAM: { used_mb: 0, total_mb: 0, utilization: 0 }, L2_SYSTEM_RAM: { used_mb: 0, total_mb: 0, utilization: 0 }, L3_REDIS_CACHE: { used_mb: 0, total_mb: 0, utilization: 0  }
      }, timestamp: new Date().toISOString(), error: 'Health check system failure'
    }, { status: 500 })
   }
}
