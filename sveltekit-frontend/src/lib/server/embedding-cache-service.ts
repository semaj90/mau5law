...
class EmbeddingCacheService {
  ...
  async cacheEmbedding(text: string, embedding: number[], model: string = 'embeddinggemma:latest'): Promise<void> {
    ...
    try {
      ...
      await typedRedisService.set(`${this.EMBEDDING_PREFIX}${key}`, JSON.stringify(cacheData), this.EMBEDDING_TTL);
  ...
  async getEmbedding(text: string, model: string = 'embeddinggemma:latest'): Promise<number[] | null> {
    try {
  ...
  async getQueryResults(query: string, metadata: Record<string, unknown> = {}): Promise<unknown[] | null> {
    ...
    try {
      ...
      const enrichedMetadata = { ...metadata, queryComplexity: this.calculateQueryComplexity(query) };
  ...
  async batchCacheEmbeddings(items: Array<{ text: string; embedding: number[]; model?: string }>): Promise<void> {
    ...
    try {
      ...
      for (const item of items) {
        ...
        await typedRedisService.set(`${this.EMBEDDING_PREFIX}${key}`, JSON.stringify(entry), this.EMBEDDING_TTL);
        cached++;
      }
  ...
  private async getCacheSize(type: 'embeddings' | 'queries' | 'sessions'): Promise<number> {
    try {
      const prefix = type === 'embeddings' ? this.EMBEDDING_PREFIX : type === 'queries' ? this.QUERY_PREFIX : this.SESSION_PREFIX;
