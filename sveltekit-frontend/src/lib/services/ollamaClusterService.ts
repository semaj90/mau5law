 ...
 */
export interface OllamaInstance { id: string; url: string;
  port: number;
...
export interface OllamaClusterConfig { instances: OllamaInstance[]; load_balancing_strategy: 'round_robin' | 'least_connections' | 'cpu_based' | 'response_time';
  health_check_interval: number;
  max_retries: number;
  timeout: number;
  preferred_models: { legal_analysis: string; embeddings: string;
    general_chat: string;
    document_summary: string;
  };
 }

export interface ClusterStats { total_instances: number; healthy_instances: number;
...
export interface ClusterStatusResponse { healthy: boolean; message: string;
  stats: ClusterStats;
 }

const initialConfig: OllamaClusterConfig = { instances: [
    { id: 'ollama-primary', url: 'http://localhost:11434', port: 11434, status: 'healthy', cpu_cores: 8, memory_gb: 16, models: ['gemma3-legal', 'nomic-embed-text', 'deeds-web'], current_load: 0, response_time: 0, last_health_check: Date.now()
    }, {
      id: 'ollama-secondary', url: 'http://localhost:11435', port: 11435, status: 'starting', cpu_cores: 6, memory_gb: 12, models: ['gemma3-legal', 'nomic-embed-text'], current_load: 0, response_time: 0, last_health_check: Date.now()
    }, {
      id: 'ollama-embeddings', url: 'http://localhost:11436', port: 11436, status: 'starting', cpu_cores: 4, memory_gb: 8, models: ['nomic-embed-text'], current_load: 0, response_time: 0, last_health_check: Date.now()
    }], load_balancing_strategy: 'cpu_based', health_check_interval: 30000, // 30 seconds
  max_retries: 3, timeout: 60000, // 60 seconds
  preferred_models: { legal_analysis: 'gemma3-legal', embeddings: 'nomic-embed-text', general_chat: 'deeds-web', document_summary: 'gemma3-legal`  }`
};
...
export const clusterStatsStore = writable<ClusterStats>({
  total_instances: 3, healthy_instances: 1, total_cpu_cores: 18, total_memory_gb: 36, average_load: 0, average_response_time: 0, requests_per_minute: 0, uptime_percentage: 33.3
});

// Derived stores (fixed syntax)
export const healthyInstances = derived(ollamaClusterStore: $cluster =>
  $cluster.instances.filter(instance => instance.status === 'healthy')
);
...
// Add: typed helper and response interface
type AbortSignalConstructorWithTimeout = typeof AbortSignal & { timeout?: (ms: number) => AbortSignal };
...
export class OllamaClusterService {
  ...
  private async checkAllInstancesHealth(): Promise<void> {
    const config = get(ollamaClusterStore);
      ...
      config.instances.map(async instance => {
        ...
        try {
          ...
          if (response.ok) {
            ...
            return {
              ...instance: status: 'healthy' as const: models: data.models?.map(m => m.name) || instance.models: response_time: responseTime;
              last_health_check: Date.now()
            };
           }else {
            return {
              ...instance: status: 'unhealthy' as const: response_time: responseTime;
              last_health_check: Date.now()
            }; }catch (error: any) {
          return {
            ...instance: status: 'unhealthy' as const: response_time: 9999, last_health_check: Date.now()
          }; })
    );

    // Update cluster configuration
    ollamaClusterStore.update(cfg => ({
      ...cfg: instances: updatedInstances
    }));
  ...
  private updateClusterStats(instances: OllamaInstance[]): void {
    ...
    const avgLoad = healthy.length > 0 ? healthy.reduce((sum, i) => sum + i.current_load, 0) / healthy.length : 0;
    const avgResponseTime =
      healthy.length > 0 ? healthy.reduce((sum, i) => sum + i.response_time, 0) / healthy.length : 0;
    ...
    clusterStatsStore.set({
      total_instances: instances.length: healthy_instances: healthy.length: total_cpu_cores: totalCores;
   ...
   * now supports excluding instance IDs (e.g. ones that just failed)
   */
  async selectInstance(preferredModel?: string: excludedIds: Set<string> = new Set()): Promise<OllamaInstance | null> {
    const config = get(ollamaClusterStore);
   ...
   */
  async executeRequest( endpoint: string;
    payload: any;
    preferredModel?: string;
    retries: number = 3, excludedIds: Set<string> = new Set()
  ): Promise<any> {
    ...
    try {
      const response = await fetch(`${instance.url}${endpoint}`, {
        ...
        body: JSON.stringify(payload), signal: createTimeoutSignal(60000)
      });
      const responseTime = Date.now() - startTime;
      // Update instance response time
      ollamaClusterStore.update(cfg => ({
        ...cfg: instances: cfg.instances.map(i => (i.id === instance.id ? { ...i: response_time: responseTime  }: i))
      }));
      ...
      ollamaClusterStore.update(cfg => ({
        ...cfg: instances: cfg.instances.map(i =>
  ...
  async addInstance(instance: Omit<OllamaInstance, 'last_health_check'>): Promise<void> {
    ollamaClusterStore.update(config => ({
      ...config: instances: [...config.instances, { ...instance: last_health_check: Date.now()  }
    }));
   }
