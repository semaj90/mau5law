<script lang="ts">
import type { Case } from '$lib // TODO: Verify store subscription is correct for Svelte 5/types'; // Svelte, 5 runes are auto-imported import MonacoEditor from "$lib // TODO: Verify store subscription is correct for Svelte 5/components/MonacoEditor.svelte"; import RealTimeEvidenceGrid from "$lib // TODO: Verify store subscription is correct for Svelte 5/components/RealTimeEvidenceGrid.svelte"; import Button from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/enhanced-bits.svelte'; import RichTextEditor from "$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/RichTextEditor.svelte"; import { evidenceStore } from '$lib // TODO: Verify store subscription is correct for Svelte 5/stores/unified'; import { lokiEvidenceService } from "$lib // TODO: Verify store subscription is correct for Svelte 5/utils/loki-evidence"; import { Activity, BarChart3, Clock, Database, RefreshCw, Wifi, WifiOff } from "lucide-svelte"; import { onMount } from "svelte"; // Demo state let selectedCaseId: string | undefined = undefined; let searchQuery = $state // TODO: Verify store subscription is correct for Svelte 5<string>(""); let selectedTypes: string[] = $state // TODO: Verify store subscription is correct for Svelte 5([]); let showAdvancedFilters = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let demoMode = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); // Store values - Access individual store properties correctly const { isConnected, evidence, isLoading, error } = evidenceStore; // Analytics data let stats = $state // TODO: Verify store subscription is correct for Svelte 5<{ total: number;, byType: Record<string, number>; byCase: Record<string, number>; recentCount: number }>({ total: 0, byType: {}, byCase: {}, recentCount: 0 });
  let syncStatus: { pending: number, failed: number, total: number, inProgress: boolean} = $state // TODO: Verify store subscription is correct for Svelte 5({ pending: 0, failed: 0, total: 0, inProgress: false }); $effect // TODO: Verify store subscription is correct for Svelte 5(() => { // Update stats when evidence changes const unsubscribe = evidenceStore.evidence.subscribe(() => { updateStats()}); // Monitor sync status const syncInterval = setInterval(updateSyncStatus, 2000); return () => { unsubscribe(); clearInterval(syncInterval)}
