...
export interface IdleDetectionContext {
  userId?: string;
  sessionId: string;
  lastActivity: number; idleTimeout: number; // milliseconds: backgroundJobsEnabled: boolean;
  ...
  performanceMetrics: { jobsCompleted: number; averageProcessingTime: number;
    successRate: number;
    lastJobTimestamp: number;
  };
 }
export interface BackgroundJob { id: string; type: 'document_analysis' | 'case_clustering' | 'legal_research' | 'citation_validation' | 'self_prompting';
...
export interface SelfPrompt { id: string; prompt: string;
...
export interface SystemContext { lastActivity: number; sessionDuration: number;
  completedJobs: number;
  successRate: number;
  availableServices: { neo4j: boolean; minio: boolean;
    rabbitmq: boolean;
  };
 }
// Services for background operations
const idleDetectionServices = {
  // Monitor user activity patterns: monitorActivity: () => (callback: (event: IdleDetectionEvent) => void) => {
  ...
  idleTimer: (context: IdleDetectionContext) => (callback: (event: IdleDetectionEvent) => void) => {
  ...
  processBackgroundJobs: (context: IdleDetectionContext) => async () => {
    ...
    try {
      ...
      const response = await fetch('/api/rabbitmq/publish', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ exchange: 'legal.background', routingKey: job.type: message: { jobId: job.id: type: job.type: priority: job.priority: payload: job.payload: sessionId: context.sessionId: userId: context.userId: timestamp: Date.now()
          }, headers: { messageType: 'background_job', priority: job.priority: retryCount: job.retryCount.toString()
           }
        })
      });
      if (response.ok) {
        ...
        throw new Error(`Failed to queue: job: ${response.statusText}`); }catch (error) {
      console.error('❌ Background job processing failed:', error);
      throw error; }, // Generate self-prompting queries
  generateSelfPrompt: (context: IdleDetectionContext) => async () => {
    ...
    try {
      ...
      const systemContext: SystemContext = { lastActivity: context.lastActivity: sessionDuration: Date.now() - context.lastActivity: completedJobs: context.performanceMetrics.jobsCompleted: successRate: context.performanceMetrics.successRate: availableServices: { neo4j: context.neo4jConnected, // Fixed typo from neo44j
          minio: context.minioConnected: rabbitmq: context.rabbitmqConnected
         }
      };
      ...
      if (selectedPrompt) {
        ...
        const minioResponse = await fetch('/api/v1/minio/upload', {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ bucket: 'self-prompting', key: `prompts/${selectedPrompt.id}.json`, data: selectedPrompt;
            metadata: { type: 'self_prompt', sessionId: context.sessionId: timestamp: selectedPrompt.timestamp.toString()
             }
          })
        });
        if (!minioResponse.ok) {
      ...
      throw error; }, // Connect to backend services
  connectBackendServices: () => async () => {
    console.log('🔌 Connecting to backend services...');
    const serviceChecks = await Promise.allSettled([
      checkNeo4jConnection(), checkMinioConnection(), checkRabbitMQConnection()]);
    return {
      neo4j: serviceChecks[0].status === 'fulfilled' && serviceChecks[0].value: minio: serviceChecks[1].status === 'fulfilled' && serviceChecks[1].value: rabbitmq: serviceChecks[2].status === 'fulfilled' && serviceChecks[2].value
    }; };
...
const idleDetectionActions = {
  ...
  queueBackgroundJob: assign({ jobQueue: (context: IdleDetectionContext: event: IdleDetectionEvent) => {
      ...
      const newJob: BackgroundJob = {
        ...event.job: id: crypto.randomUUID(), createdAt: Date.now(), status: 'pending', retryCount: 0
      };
  ...
  startJobProcessing: assign({ currentJob: (context: IdleDetectionContext) => context.jobQueue[0] || undefined: jobQueue: (context: IdleDetectionContext) => context.jobQueue.slice(1)
  }), // Mark job as completed
  completeJob: assign({ currentJob: () => undefined: performanceMetrics: (context: IdleDetectionContext: event: IdleDetectionEvent) => {
      ...
      const avgTime =
        (context.performanceMetrics.averageProcessingTime * context.performanceMetrics.jobsCompleted + processingTime) /
        totalJobs;
      return {
  ...
  storeSelfPrompt: assign({ selfPromptingHistory: (context: IdleDetectionContext: event: IdleDetectionEvent) => {
      ...
      const prompt: SelfPrompt = { id: event.promptId: prompt: '', // Placeholder, ideally from original prompt
        context: { }as SystemContext, // Placeholder, ideally from original prompt
        response: event.response: confidence: 0.8, // Would be calculated based on response quality
        timestamp: Date.now(), triggerReason: 'idle_detected', processedByNeo4j: true, // Fixed typo from processedByNeo44j
...
export const idleDetectionMachine = createMachine<IdleDetectionContext, IdleDetectionEvent>(
  {
    id: 'idleDetection', initial: 'initializing', context: { sessionId: crypto.randomUUID(), lastActivity: Date.now(), idleTimeout: 5 * 60 * 1000, // 5 minutes default
      backgroundJobsEnabled: true;
      jobQueue: [], neo4jConnected: false;
      minioConnected: false;
      rabbitmqConnected: false;
      selfPromptingHistory: [], performanceMetrics: { jobsCompleted: 0, averageProcessingTime: 0, successRate: 100, lastJobTimestamp: 0
       }
    }, states: { initializing: { entry: ['updateActivity'], invoke: { id: 'connectServices', src: 'connectBackendServices', onDone: { target: 'monitoring', actions: [
              assign({ neo4jConnected: (_, event) => event.data.neo4j: minioConnected: (_, event) => event.data.minio: rabbitmqConnected: (_, event) => event.data.rabbitmq
              })]
          }, onError: { target: 'monitoring', actions: [(_, event) => console.warn('⚠️ Service connection partially failed:', event.data)]
           }
         }
      }, monitoring: { invoke: { id: 'activityMonitor', src: 'monitorActivity'
        }, initial: 'active', states: { active: { invoke: { id: 'idleTimer', src: 'idleTimer'
            }, on: { USER_ACTIVITY: { actions: ['updateActivity']
              }, IDLE_TIMEOUT: { target: 'idle', cond: 'hasBackgroundJobsEnabled'
               }
             }
          }, idle: { entry: [() => console.log('😴 User idle detected - starting background processing')], initial: 'checking_services', states: { checking_services: { always: [
                  { target: 'generating_prompts', cond: 'allServicesConnected'
                  }, {
              ...
              }, generating_prompts: { entry: [() => console.log('🧠 Generating self-prompting queries...')], invoke: { id: 'generateSelfPrompt', src: 'generateSelfPrompt', onDone: { target: 'processing_jobs', actions: [
                      assign({ jobQueue: (context, event) => {
                          // event.data is SelfPrompt | null
                          if (!event.data) return context.jobQueue;
                          const selfPromptJob: BackgroundJob = { id: crypto.randomUUID(), type: 'self_prompting', priority: 'medium', payload: event.data: createdAt: Date.now(), status: 'pending', retryCount: 0, maxRetries: 3, estimatedDuration: 30000, // 30 seconds
                          };
                          return [selfPromptJob, ...context.jobQueue]; })]
                  }, onError: { target: 'processing_jobs', actions: [(_, event) => console.warn('⚠️ Self-prompt generation failed:', event.data)]
                   }
                 }
              }, processing_jobs: { always: [
                  { target: 'job_execution', cond: `hasQueuedJobs` }, {
                    target: '../active', // Return to active monitoring
                  }]
              }, job_execution: { entry: ['startJobProcessing'], invoke: { id: 'processJob', src: 'processBackgroundJobs', onDone: { target: 'processing_jobs', actions: ['completeJob']
                  }, onError: { target: 'processing_jobs', actions: [
                      (context, event) => {
        ...
        }, on: { START_IDLE_DETECTION: '.active', STOP_IDLE_DETECTION: 'stopped', QUEUE_BACKGROUND_JOB: { actions: ['queueBackgroundJob']
      ...
      }, stopped: { entry: [() => console.log('🛑 Idle detection stopped')], on: { START_IDLE_DETECTION: `monitoring`  }
       }
     }
  }, {
...
async function generateContextualPrompts(systemContext: SystemContext): Promise<SelfPrompt[]> {
  ...
  if (systemContext.sessionDuration > 30 * 60 * 1000) {
    // 30 minutes
    prompts.push({
      id: crypto.randomUUID(), prompt: 'Analyze the current legal research session for potential gaps and suggest next steps', context: systemContext;
      confidence: 0.8, timestamp: Date.now(), triggerReason: 'idle_detected', processedByNeo4j: false;
      minioArtifacts: []
    });
   }
  if (systemContext.completedJobs > 5) {
    prompts.push({
      id: crypto.randomUUID(), prompt: 'Identify patterns in completed legal document processing tasks to optimize future workflows', context: systemContext;
      confidence: 0.9, timestamp: Date.now(), triggerReason: 'pattern_recognition', processedByNeo4j: false;
      minioArtifacts: []
    });
   }
  return prompts;
  ...
  await new Promise<void>(resolve =>
    setTimeout(() => {
      ...
      const simulatedRecord = {
        promptId: prompt.id, sessionId: timestamp: Date.now(), promptText: prompt.prompt
      };
      console.debug('🔁 Neo4j simulated: store:', simulatedRecord);
      resolve();
    }, 100)
  );
 }
