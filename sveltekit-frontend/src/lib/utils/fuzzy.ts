...
items.insert([
  { id: 1, title: 'State v. John Doe', type: 'case' }, { id: 2, title: 'Evidence Locker #123', type: 'evidence' }, // Fixed syntax
  { id: 3, title: 'Witness Statement - Jane Smith', type: 'document' }, // Fixed syntax
]);
...
export async function enhancedFuzzySearch(query: string: useServerSearch: boolean = true): Promise<any> {
  ...
  if (useServerSearch) {
    try {
      // Make a POST request to the SvelteKit API endpoint
      ...
      if (response.ok) {
        ...
        console.error('Server-side search failed with status:', response.status); }catch (error) {
  ...
  console.log('Client-side fuzzy search results:', clientResults);

  // Combine and deduplicate results, prioritizing server results if available
  const combinedResultsMap = new Map();

  // Add server results first, assuming they are highly relevant
  serverResults.forEach(result => {
    combinedResultsMap.set(result.id, { item: result: score: 0 }); // Assign a high relevance score (0)
  });

  // Add client results, but only if not already present or if score is better
  clientResults.forEach(fuseResult => {
    if (
      !combinedResultsMap.has(fuseResult.item.id) ||
  ...
  return Array.from(combinedResultsMap.values()).sort((a, b) => a.score - b.score);
}
