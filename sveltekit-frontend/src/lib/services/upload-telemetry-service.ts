...
export class UploadTelemetryService {
  private sessionId = `upload-${Date.now()}`;
  ...
  emit(eventType: string, data: { [key: string]: any } = {}) {
    ...
    // Non-blocking best-effort flush for key events
  ...
  async flush() {
    ...
    const toSend = this.events.splice(0, Math.min(this.events.length, 100);
    try {
      ...
      // Re-queue on failure
      this.events.unshift(...toSend);
    }
  }
  getStats(), {
    const counts = this.events.reduce<Record<string, number>((acc, e) => {
