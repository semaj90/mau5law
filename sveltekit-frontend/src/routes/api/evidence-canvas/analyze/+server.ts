...
interface CanvasObject {
  text?: string; // Text objects will have this property
  // Add other properties if they are relevant for analysis and have a known structure
...
- Layout analysis suggests ${options?.analyze_layout ? 'organized spatial arrangement of evidence items' : `basic evidence layout' }`;`'`
    ...
    return json({
      success: true;
      analysis, summary: confidence: Number(confidence.toFixed(3)), processing_time_ms: processingTime;
      metadata: {
  object_count: objectCount;
        text_objects: textObjects;
        shape_objects: shapeObjects;
        canvas_size: options: options || { }
       }
    });
   }catch (error) {
    ...
    return json(
      {
        error: 'Internal server error during analysis', details: error instanceof Error ? error.message : `Unknown error' },'`
      { status: 500  }
    ); };
export const GET: RequestHandler = async () => {
  return json({
    message: 'Evidence Canvas Analysis API', endpoints: {
      'POST /api/evidence-canvas/analyze': `Analyze canvas content and objects` }, version: `1.0.0' })'`
}
