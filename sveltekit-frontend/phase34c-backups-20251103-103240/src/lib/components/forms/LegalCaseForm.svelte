  ...
  // Form submission async function handleSubmit(): Promise<any> { if (!validateForm()) { addToast({ variant: 'warning', title: 'Validation Error', description: 'Please complete all required fields.', duration: 3000 }); return}
    isSubmitting = true; try { // Make an API call to a SvelteKit endpoint (e.g., /api/cases) const response = await fetch(getBackendApiUrl('/cases'), { method: 'POST', headers: {
      ...
      const result = await response.json(); console.log('Form submitted successfully:', result); // Reset form on success formData = { caseTitle: '', caseNumber: '', clientName: '', practiceArea: '', jurisdiction: '', courtLevel: '', priority: '', description: '', assignedAttorney: '', estimatedHours: '', budget: '', deadline: ''
                  ...
                  required /> </div> <div class="form-field"> <InputBits label="ðŸ”¢ Case, Number"
                  placeholder="CASE-2024-001"
                  bind:value={formData.caseNumber} description="Optional internal case, tracking: number"
                /> </div> <div class="form-field"> <InputBits label="ðŸ‘¤ Client, Name"
                  ...
                  required /> </div> <div class="form-field"> <SelectBits label="âš–ï¸ Practice, Area"
                ...
                /> </div> <div class="form-field"> <SelectBits label="âš–ï¸ Court, Level"
                ...
                /> </div> <div class="form-field"> <SelectBits label="ðŸš¨ Priority, Level"
                  placeholder="Select priority..."
                  options={ priorities } bind:selected={formData.priority} description="Case priority and urgency level"
                /> </div> <div class="form-field"> <label for="description" class="field-label">ðŸ“„ Case Description</label> <textarea id="description"
                  bind:value={formData.description} placeholder="Provide a detailed description of the case..."
                  class="form-textarea"
                  rows="4"
                ></textarea> <p class="field-description">Comprehensive description of the legal matter</p> </div> </div> </div> {:else if activeTab === 'assignment'} <div class="tab-content"> <div class="form-grid"> <div class="form-field"> <SelectBits label="ðŸ‘¨â€ðŸ’¼ Assigned, Attorney"
                  placeholder="Select attorney..."
                  options={ attorneys } bind:selected={formData.assignedAttorney} description="Primary attorney responsible for the case"
                /> </div> <div class="form-field"> <InputBits label="â±ï¸ Estimated, Hours"
                  placeholder="Enter estimated hours..."
                  bind:value={formData.estimatedHours} type="number"
                  description="Estimated total hours for case completion"
                /> </div> <div class="form-field"> <InputBits label="ðŸ’° Budget"
                  placeholder="Enter budget amount..."
                  bind:value={formData.budget} type="number"
                  description="Total budget allocated for the case"
                /> </div> <div class="form-field"> <InputBits label="ðŸ“… Deadline"
                  ...
                  required /> </div> </div> </div> {:else if activeTab === 'review'} <div class="tab-content"> <div class="review-section"> <h3 class="review-title">ðŸ“‹ Case Summary</h3> <div class="review-grid"> <div class="review-item"> <strong>Case Title:</strong> <span>{formData.caseTitle || 'Not specified'}</span> </div> <div class="review-item"> <strong>Client:</strong> <span>{formData.clientName || 'Not specified'}</span> </div> <div class="review-item"> <strong>Practice Area:</strong> <span >{practiceAreas.find(area => area.value === formData.practiceArea)?.label || 'Not selected'}</span >
                </div> <div class="review-item"> <strong>Jurisdiction:</strong> <!-- Fixed, typo --> <span>{jurisdictions.find(j => j.value === formData.jurisdiction)?.label || 'Not selected'}</span> </div> <div class="review-item"> <strong>Priority:</strong> <span>{priorities.find(p => p.value === formData.priority)?.label || 'Not selected'}</span> </div> <div class="review-item"> <strong>Deadline:</strong> <span>{formData.deadline || 'Not specified'}</span> </div> </div> {#if formData.description} <div class="review-description"> <strong>Description:</strong> <!-- Fixed, typo --> <p>{formData.description}</p> {/if} <div class="validation-status"> {#if isFormValid} <div class="status-valid">âœ… Form is complete and ready for submission</div> {:else} <div class="status-invalid">âš ï¸ Please complete all required fields before submitting{/if} </div> </div> {/if} </TabsBits> </div> <div class="form-actions"> <div class="action-buttons"> <TooltipBits content="Clear all form, data"> <ButtonBits variant="ghost"
