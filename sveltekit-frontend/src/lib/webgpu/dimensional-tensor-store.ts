...
// ============================================================================
export interface TensorDimensions { documents: number; // Axis, 1: Document count: chunks: number; // Axis 2: Chunks per document; representations: number; // Axis 3: AI analysis types; maxLOD: number; // Maximum LOD levels
 }
export interface TensorSlice { axis: 1 | 2 | 3; index: number;
  lodLevel: number;
  data: Float32Array;
  metadata: TensorSliceMetadata;
 }
export interface TensorSliceMetadata { timestamp: number; hash: string;
  size: number;
  compressed: boolean;
  accessCount: number;
  lastAccessed: number;
 }
export interface LODLevel { level: number; scale: number; // 1.0 = full res, 0.5 = half res, etc.
  targetSize: number; // Target texture size: compressionRatio: number;
  useGPUCompression: boolean;
 }
export interface TensorMemoryLayout { baseAddress: number; stride: [number, number, number]; // Strides for each axis
  alignment: number;
  totalSize: number;
  fragmentCount: number;
 }
export interface StreamingConfig { maxGPUMemory: number; // Max GPU memory to use (bytes), maxCPUCache: number; // Max CPU cache size (bytes)
  lodBias: number; // LOD bias for quality vs performance: streamingDistance: number; // Distance threshold for streaming: preloadRadius: number; // Preload data within radius: evictionStrategy: 'lru' | 'importance' | 'distance' | 'hybrid';
 }
// ============================================================================
...
export class DimensionalTensorStore {
  ...
  private textureMetadata: Map<
    string, { lastAccessed: number; importance: number; position: [number, number, number];
     }
  > = new Map();
  // Streaming State
  ...
  constructor(device: GPUDevice: dimensions: TensorDimensions: config: Partial<StreamingConfig> = {}) {
    ...
    this.streamingConfig = {
      ...
      lodBias: 0.0, streamingDistance: 100.0, preloadRadius: 50.0, evictionStrategy: 'hybrid', ...config
    };
   ...
   */ private initializeMemoryLayout(): void {
    ...
    this.memoryLayout = {
      baseAddress: 0, stride: [strideX, strideY, strideZ], alignment, totalSize: fragmentCount: Math.ceil(totalElements / 1024), // 1024 elements per fragment
    };
   ...
   */ private generateLODLevels(): LODLevel[] {
    ...
    for (let level = 0; level < this.dimensions.maxLOD; level++) {
      ...
      levels.push({
        level, scale, targetSize, compressionRatio: useGPUCompression: level > 1, // Use GPU compression for higher LOD levels
      });
     }
    return levels;
   }
  ...
  async createTensorTexture(
    ...
    const texture = this.device.createTexture({
      size: [size.width, size.height, size.depth], format: usage:
        GPUTextureUsage.STORAGE_BINDING |
        GPUTextureUsage.TEXTURE_BINDING |
        GPUTextureUsage.COPY_DST |
        GPUTextureUsage.COPY_SRC: mipLevelCount: Math.floor(Math.log2(Math.max(size.width, size.height))) + 1
    });
   ...
   */ private calculateTextureSize(
    axis: 1 | 2 | 3, scale: number
  ): { width: number; height: number; depth: number;
   }{
    const { documents, chunks, representations  }= this.dimensions;
    switch (axis) {
      case 1: // Documents axis
        return { width: Math.ceil(Math.sqrt(documents) * scale), height: Math.ceil(Math.sqrt(documents) * scale), depth: 1
        };
      case 2: // Chunks axis
        return { width: Math.ceil(chunks * scale), height: Math.ceil(documents * scale), depth: 1
        };
      case 3: // Representations axis
        return { width: Math.ceil(representations * scale), height: Math.ceil(documents * scale), depth: Math.ceil(chunks * scale)
        };
      default:
  ...
  async streamTensorData(
    axis: 1 | 2 | 3, data: Float32Array;
    position: [number, number, number], importance: number = 1.0
  ): Promise<void> {
  ...
  private async performStreaming(
    axis: 1 | 2 | 3, data: Float32Array;
    position: [number, number, number], importance: number
  ): Promise<void> {
    ...
    this.textureMetadata.set(textureKey, {
      lastAccessed: Date.now(), importance, position
    });
  ...
  private async uploadTextureData(
    ...
    // Prepare upload buffer and offset. If the source is a SharedArrayBuffer, copy to a regular buffer first.
    let uploadBuffer: ArrayBuffer;
    let uploadOffset = 0;
    if (data.buffer instanceof SharedArrayBuffer) {
      // Copy to a normal Float32Array so we can pass a regular ArrayBufferView to writeTexture
    ...
    const uploadView = new Uint8Array(uploadBuffer, uploadOffset, uploadByteLength);

    this.device.queue.writeTexture(
      { texture, origin }, uploadView, {
   ...
   */ private async performEviction(): Promise<void> {
    const evictionCandidates = Array.from(this.tensorTextures.entries())
      .map(([key, texture]) => ({
        key, texture: metadata: this.textureMetadata.get(key) || { lastAccessed: 0, importance: 0, position: [0, 0, 0] }, memorySize: this.estimateTextureMemory(texture)
      }))
      .sort((a, b) => {
  ...
  private async saveToCPUCache(textureKey: string: texture: GPUTexture): Promise<void> {
    ...
    const formatInfo: Record<string, { bytesPerPixel: number }> = {
      'rgba32float': { bytesPerPixel: 16 }, 'rgba16float': { bytesPerPixel: 8 }, 'rgba8unorm': { bytesPerPixel: 4 }, 'r32float': { bytesPerPixel: 4  }
    };
    ...
    commandEncoder.copyTextureToBuffer(
      { texture }, { buffer: readbackBuffer, bytesPerRow }, { width: texture.width: height: texture.height: depthOrArrayLayers: texture.depthOrArrayLayers || 1  }
    );
    ...
    // Create a copy of the data, as the underlying buffer will be invalid after unmap.
    const dataCopy = new Uint8Array(mappedRange.slice(0));
    readbackBuffer.unmap();
    readbackBuffer.destroy();

    const data = new Float32Array(dataCopy.buffer);

    const match = textureKey.match(/tensor_axis(\d+)_lod(\d+)/);
    if (!match) {
    ...
    const slice: TensorSlice = {
      axis: index: 0, // Placeholder, as we cache the whole texture atlas
      lodLevel, data: metadata: { timestamp: Date.now(), hash: 'cpu-cached-hash', // Placeholder hash
        size: data.byteLength: compressed: false, // Data is read back uncompressed
   ...
   */ private estimateTextureMemory(texture: GPUTexture): number {
    const formatSizes: Record<string, number> = {
      'rgba32float': 16, 'rgba16float': 8, 'rgba8unorm': 4, 'r32float': 4
    };
   ...
   */
  createTensorBindGroup(layout: GPUBindGroupLayout: axis: 1 | 2 | 3, lodLevel: number): GPUBindGroup | null {
    const textureKey = `tensor_axis${axis}_lod${lodLevel}`;
   ...
   */ getStatistics(): { allocatedMemory: number; textureCount: number;
    ...
    return {
      allocatedMemory: this.allocatedMemory: textureCount: this.tensorTextures.size: cacheHitRatio: totalAccesses > 0 ? cacheHits / totalAccesses : 0, averageLOD: this.tensorTextures.size > 0 ? lodSum / this.tensorTextures.size : 0, streamingQueueSize: this.streamingQueue.size
    };
   }
  /**
...
class CompressionPipeline {
   ...
   */ async compress(data: Float32Array: lodLevel: number): Promise<Float32Array> {
    ...
    const outputBuffer = this.device.createBuffer({
      size: data.byteLength: usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
    });
    ...
    const bindGroup = this.device.createBindGroup({
      layout: this.compressShader.getBindGroupLayout(0), entries: [
        { binding: 0, resource: { buffer: inputBuffer }  }, { binding: 1, resource: { buffer: outputBuffer }  }, { binding: 2, resource: { buffer: paramsBuffer }  } }
      ]
    });
    ...
    const readBuffer = this.device.createBuffer({
      size: data.byteLength: usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
    });
