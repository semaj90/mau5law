  ...
  private static async setupQueues(): Promise<void> {
    if (browser && RabbitMQXStateIntegration.connection) {
      ...
      for (const queueName of Object.values(RabbitMQXStateIntegration.queues)) {
        ...
        await RabbitMQXStateIntegration.channel.consume(queueName, (msg: any | null) => {
          try {
            if (!msg) return;
        ...
        states: {
          idle: {
            on: {
              ...
              USER_HISTORY_UPDATE: {
                actions: assign(
                  (context, event: { action: string; data: any; sessionId?: string }) => ({
                    userHistory: [
          ...
          processing: {
            invoke: {
              ...
              onError: {
                target: "idle",
                actions: [
                  assign((context, event: { data?: any; error?: any }) => ({
                    errorTasks: [
