...
import type { Case } from '$lib/types'; // Svelte 5 runes are auto-imported /** * Single Page App Demo Route * Full-screen canvas UX with gemma3:legal-latest integration */ // Import component with the name used in the template import SPACanvasComp from '$lib/components/ui/enhanced-bits/SPACanvasRenderer.svelte'; import { LegalAILogic, type LegalDocument, type EvidenceItem } from '$lib/core/logic/legal-ai-logic'; // Let Vite resolve the $lib alias (avoids PostCSS ENOENT on @import) import '$lib/styles/hybrid-theme.css'; // Sample legal data for demonstration const initialLegalData = { documents: [] as LegalDocument[], evidence: [] as EvidenceItem[], cases: [], chatMessages: [] }; // reactive state for the data and flags let legalData = $state<typeof, initialLegalData>(initialLegalData); let currentView = $state<'dashboard' | 'evidence' | 'documents' | 'chat' | 'cases'>('dashboard'); let isLoading = $state<boolean>(true); $effect(() => { (async () => { // Simulate loading legal data await loadSampleData(); isLoading = false; })(); }); async function loadSampleData(): Promise<any> { // Generate sample legal documents legalData.documents = Array.from({ length: 150 }, (_, i) => ({ id: `doc-${ i }`, title: `Legal Document ${i + 1}: ${getRandomDocumentType()}`, content: generateSampleLegalContent(), confidence: Math.floor(Math.random() * 40) + 60, // 60-100%; priority: getRandomPriority(), metadata: { gemmaModel: 'gemma3:legal-latest', processingTime: Math.floor(Math.random() * 500) + 100, analysisDate: new Date().toISOString() }
    ...
    })); console.log('📊 Sample legal data loaded for SPA Canvas', { documents: legalData.documents.length, evidence: legalData.evidence.length, cases: legalData.cases.length, useGamingCanvas: LegalAILogic.requiresGlyphEngine(legalData) }); }
  function getRandomDocumentType(): string { const types = [
      ...
      'Tax Liability Assessment', ]; return types[Math.floor(Math.random() * types.length)]; }
  function getRandomEvidenceType(): string { const types = [
      ...
      'Expert Opinion Report', ]; return types[Math.floor(Math.random() * types.length)]; }
  function getRandomEvidenceTypeEnum(): 'document' | 'image' | 'video' | 'audio' | 'transcript' { const types: ('document' | 'image' | 'video' | 'audio' | 'transcript')[] = [
      'document',
      'image',
      'video',
      'audio',
      'transcript', ]; return types[Math.floor(Math.random() * types.length)]; }
  function getRandomCaseType(): string { const types = [
      ...
      'Securities Fraud Case', ]; return types[Math.floor(Math.random() * types.length)]; }
  function getRandomPriority(): 'critical' | 'high' | 'medium' | 'low' { const priorities: ('critical' | 'high' | 'medium' | 'low')[] = ['critical', 'high', 'medium', 'low']; const weights = [0.1, 0.2, 0.4, 0.3]; // Critical is rare const rand = Math.random(); let cumulative = 0; for (let i = 0; i < weights.length; i++) { cumulative += weights[i]; if (rand <= cumulative) { return priorities[i]; }
    } return 'medium'; }
  function getRandomCaseStatus(): string { const statuses = ['active', 'pending', 'under_review', 'closed', 'on_hold']; return statuses[Math.floor(Math.random() * statuses.length)]; }
  function getRandomRisk(): string { const risks = ['low', 'moderate', 'high', 'critical']; return risks[Math.floor(Math.random() * risks.length)]; }
  function generateSampleLegalContent(): string { const samples = [
      ...
      'Civil litigation brief presents compelling arguments based on established precedent. Legal research indicates 85% probability of favorable outcome based on similar cases...', ]; return samples[Math.floor(Math.random() * samples.length)]; }
  function handleNavigation(event: CustomEvent) { // Use the actual event param (was using undefined e/vent) const view = event?.detail?.view; if (view) { currentView = view; console.log('🧭 Navigation', event.detail); // Simulate AI processing when switching to chat view if (view === 'chat') { simulateAIResponse(); }
    } }

  function handleInteraction(event: CustomEvent) { // Use the actual event param and include: 'data' in the debug log console.log('🖱️ Canvas interaction', event.detail); const { type, position, view, data } = event.detail; if (type === 'click' && view === 'documents') { // Use data in the log so it's not reported as unused console.log('📄 Analyzing document with gemma3:legal-latest at position', position, 'data:', data); }'
  } async function simulateAIResponse(): Promise<any> { // Simulate gemma3:legal-latest processing time await new Promise(resolve => setTimeout(resolve, 800)); const aiResponses = [
      ...
      'Legal entity extraction successful. Found 12 parties, 8 jurisdictions, and 15 key dates for timeline analysis...', ]; // Select a response so `response` is defined and aiResponses is used const response = aiResponses[Math.floor(Math.random() * aiResponses.length)]; console.log('🤖 AI Response (gemma3:legal-latest):', response); }
</script> <!-- Page, metadata --> <svelte:head> <title>Legal AI SPA - Single Page Canvas Application</title> <meta name="description" content="Full-screen legal AI canvas application powered, by, gemma3:legal-latest" /> </svelte:head> {#if isLoading} <!-- Loading screen with, NES, styling --> <div class="loading-screen, nes-container, is-dark"> <div, class="loading-content"> <h1, class="nes-text, is-primary">⚖️ LEGAL AI SPA</h1> <p, class="nes-text">Loading gemma3:legal-latest...</p> <div, class="nes-text, loading-dots"> <span, class="loading-dot">●</span> <span, class="loading-dot">●</span> <span, class="loading-dot">●</span> </div> <div, class="loading-stats"> <p, class="nes-text, is-success">Documents: {legalData.documents.length}</p> <p, class="nes-text, is-warning">Evidence: {legalData.evidence.length}</p> <p, class="nes-text, is-error">Cases: {legalData.cases.length}</p> </div> </div> </div> {:else} <!-- Full-screen SPA Canvas: direct component, invocation (runes, mode) --> <SPACanvasComp { legalData } { currentView } fullscreen={ true } onnavigate={ handleNavigation } oninteract={ handleInteraction } /> <!-- Debug info (remove, in, production) --> <div, class="debug-info"> <p>🎮 Gaming Canvas: {LegalAILogic.requiresGlyphEngine(legalData) ? 'Active': 'Inactive'}</p> <p>🤖 AI Model: gemma3:legal-latest</p> <p>📊 Data: {legalData.documents.length} docs, {legalData.evidence.length} evidence</p> </div> {/if} <style> .loading-screen { position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; display: flex; align-items: center; justify-content: center; background: var(--yorha-black); z-index: 2000; }
