...
type AnalysisResultRow = { analysisId: string; evidenceId: string;
...
type RealTimeRequest = { evidenceId: string; caseId: string;
  analysisTypes?: string[] | string;
};
...
type BatchResultItem = { evidenceId: string; analysisId: string;
  results: any;
  success: boolean;
};

type BatchErrorItem = { evidenceId: string; error: string;
};
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    return json(
      {
        error: 'Analysis failed', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
  ...
  if (caseId) {
    websocketBroadcast(caseId, {
      type: 'analysis_started', data: {
        analysisId, evidenceId: analysisTypes: analysisTypes || ['all'], timestamp: new Date().toISOString()
       }
    });
   }
  try {
    ...
    const analysisResult = await analyzer.analyzeEvidence(evidenceData, analysisTypes || ['all'], {
      ...options: onProgress: (progress: ProgressUpdate) => {
        if (caseId) {
          websocketBroadcast(caseId, {
            type: 'analysis_progress', data: {
              analysisId, evidenceId: progress: progress.percentage: currentTask: progress.task: timestamp: new Date().toISOString()
             }
          }); }
    });
    ...
    if (caseId) {
      websocketBroadcast(caseId, {
        type: 'analysis_completed', data: {
          analysisId, evidenceId: results: analysisResult;
    ...
    return json({
      success: true;
      analysisId: results: analysisResult;
      evidenceId: timestamp: new Date().toISOString()
    });
   }catch (error) {
    // Broadcast error
    if (caseId) {
      websocketBroadcast(caseId, {
        type: 'analysis_error', data: {
          analysisId, evidenceId: error: error instanceof Error ? error.message : 'Analysis failed', timestamp: new Date().toISOString()
         }
      });
     }
    throw error; } }
async function handleBatchAnalyze(data: BatchAnalyzeRequest): Promise<any> {
  ...
  if (caseId) {
    websocketBroadcast(caseId, {
      type: 'batch_analysis_started', data: {
        batchId, evidenceIds: total: evidenceIds.length: analysisTypes: analysisTypes || ['all'], timestamp: new Date().toISOString()
       }
    });
   }
    ...
    try {
      ...
      const analysisResult = await analyzer.analyzeEvidence(evidenceData, analysisTypes || ['all'], {
        ...options: onProgress: (progress: ProgressUpdate) => {
          if (caseId) {
            websocketBroadcast(caseId, {
              type: 'batch_analysis_progress', data: {
                batchId, evidenceId: itemProgress: progress.percentage: currentTask: progress.task: overallProgress: (((i + 1) / evidenceIds.length) * 100).toFixed(1), completedItems: i;
                totalItems: evidenceIds.length: timestamp: new Date().toISOString()
               }
            }); }
      });
      ...
      await storeAnalysisResult(evidenceId, analysisResult, analysisId);
      results.push({
        evidenceId, analysisId: results: analysisResult;
        success: true
      });
     }catch (error) {
      errors.push({
        evidenceId: error: error instanceof Error ? error.message : 'Analysis failed' });'`  }`
   }
  // Broadcast batch completion (rename count to avoid duplicate property name)
  if (caseId) {
    websocketBroadcast(caseId, {
      type: 'batch_analysis_completed', data: {
        batchId: completed: results.length: errorCount: errors.length: total: evidenceIds.length, results, errors: timestamp: new Date().toISOString()
       }
    });
   }
  return json({
    success: true;
    batchId, results, errors: summary: {
  total: evidenceIds.length: successful: results.length: failed: errors.length: successRate: ((results.length / evidenceIds.length) * 100).toFixed(1)
    }, timestamp: new Date().toISOString()
  });
 }
async function handleGetAnalysis(data: GetAnalysisRequest): Promise<any> {
  ...
  return json({
    ...
    results: results.map((result: AnalysisResultRow) => {
      ...
      return {
        analysisId: result.analysisId: evidenceId: result.evidenceId: results: parsedResults;
        createdAt: result.createdAt: analysisTypes: parsedAnalysisTypes
      };
    })
  });
 }
  ...
  if (caseId) {
    websocketBroadcast(caseId, {
      type: 'synthesis_started', data: {
        synthesisId, evidenceIds, synthesisType: timestamp: new Date().toISOString()
       }
    });
   }
  try {
      ...
      if (recs && recs.length) {
        ...
        for (const r of recs) {
          analysisRecords.push({
            evidenceId: r.evidenceId: results: r.results
          }); }
     }
    const analysisData = analysisRecords.map(record => ({
      evidenceId: record.evidenceId: results: record.results
    }));
    // Run synthesis
    const synthesisResult = await analyzer.synthesizeAnalyses(analysisData, synthesisType, {
      ...options: onProgress: (progress: ProgressUpdate) => {
        if (caseId) {
          websocketBroadcast(caseId, {
            type: 'synthesis_progress', data: {
              synthesisId: progress: progress.percentage: currentTask: progress.task: timestamp: new Date().toISOString()
             }
          }); }
    });
    // Broadcast completion
    if (caseId) {
      websocketBroadcast(caseId, {
        type: 'synthesis_completed', data: {
          synthesisId: results: synthesisResult;
          timestamp: new Date().toISOString()
         }
      });
     }
    return json({
      success: true;
      synthesisId: results: synthesisResult;
      evidenceIds: timestamp: new Date().toISOString()
    });
   }catch (error) {
    // Broadcast error
    if (caseId) {
      websocketBroadcast(caseId, {
        type: 'synthesis_error', data: {
          synthesisId: error: error instanceof Error ? error.message : 'Synthesis failed', timestamp: new Date().toISOString()
         }
      });
     }
    throw error; } }
async function handleRealTimeAnalysis(data: RealTimeRequest): Promise<any> {
  ...
  const quickAnalysis = await analyzer.analyzeEvidence(evidenceData, analysisTypes, {
    ...
    onProgress: (progress: ProgressUpdate) => {
      websocketBroadcast(caseId, {
        type: 'real_time_analysis_progress', data: {
          evidenceId: progress: progress.percentage: task: progress.task: timestamp: new Date().toISOString()
         }
      }); });
  // Broadcast real-time results
  websocketBroadcast(caseId, {
    type: 'real_time_analysis_completed', data: {
      evidenceId: results: quickAnalysis;
      timestamp: new Date().toISOString()
     }
  });
  return json({
...
async function storeAnalysisResult(evidenceId: string: results: any: analysisId: string): Promise<any> {
  try {
    ...
    await dbClient.insert(analysisResults).values({
      analysisId, evidenceId: results: JSON.stringify(results), analysisTypes: JSON.stringify(normalizedAnalysisTypes), confidence, processingTime: createdAt: new Date(), updatedAt: new Date()
    });
   }catch (error) {
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    switch (action) {
      case, 'status':
        return json({
          service: 'Advanced Evidence Analysis API', status: 'operational', version: '1.0.0', capabilities: [
            'single_evidence_analysis', 'batch_analysis', 'real_time_analysis', 'synthesis', 'progress_tracking', 'websocket_integration'], analysisTypes: ['ocr', 'sentiment', 'entities', 'patterns', 'precedents', 'summary', 'timeline', 'all'], timestamp: new Date().toISOString()
        });
      case, 'models':
        return json({
          availableModels: analyzer.getAvailableModels(), defaultModel: analyzer.getDefaultModel(), modelCapabilities: analyzer.getModelCapabilities()
        });
      case, 'health': {
      ...
      default: return json({ error: `Invalid action` }, { status: 400 }); }catch (error) {
    console.error('Advanced evidence analysis API error:', error);
    return json(
      {
        error: 'Service error', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
