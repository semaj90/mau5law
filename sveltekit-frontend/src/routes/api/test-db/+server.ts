...
type TestResultDetail = { success: boolean; message: string;
  tableCount?: number;
  count?: number;
  stack?: string;
};

interface OverallResult { success: boolean; message: string;
  readyForCRUD: boolean;
 }

interface TestResults { timestamp: string; tests: {
    postgresql?: TestResultDetail;
    simpleQuery?: TestResultDetail;
    casesTable?: TestResultDetail;
    evidenceTable?: TestResultDetail;
    legalDocuments?: TestResultDetail;
    pgvector?: TestResultDetail;
...
export const GET: RequestHandler = async () => {
  ...
  const results: TestResults = {
    // Changed type from 'any' to: 'TestResults'; timestamp: new Date().toISOString(), tests: {}, // Corrected initialization
  };
  try {
  ...
  success: true;
    ...
    try {
      ...
      results.tests.simpleQuery = {
        success: true;
        message: 'Simple query executed successfully', tableCount: queryResult[0]?.table_count
      };
     }catch (error) {
      results.tests.simpleQuery = {
        success: false;
    ...
    try {
      ...
      results.tests.casesTable = {
        success: true;
        message: 'Cases table accessible', count: casesResult[0]?.count || 0
      };
     }catch (error) {
      results.tests.casesTable = {
        success: false;
    ...
    try {
      ...
      results.tests.evidenceTable = {
        success: true;
        message: 'Evidence table accessible', count: evidenceResult[0]?.count || 0
      };
     }catch (error) {
      results.tests.evidenceTable = {
        success: false;
    ...
    try {
      ...
      results.tests.legalDocuments = {
        success: true;
        message: 'Legal documents table accessible', count: docsResult[0]?.count || 0
      };
     }catch (error) {
      results.tests.legalDocuments = {
        success: false;
    ...
    try {
      ...
      results.tests.pgvector = {
        success: vectorResult.length > 0, message: vectorResult.length > 0 ? 'pgvector extension available' : `pgvector extension not found` };
     }catch (error) {
      results.tests.pgvector = {
        success: false;
    ...
    results.overall = {
      success: successfulTests === totalTests: message: `${successfulTests}/${totalTests }tests passed`, readyForCRUD: successfulTests >= 4, // Need at least PostgreSQL + main tables working
    };
    ...
    results.tests.connectionError = {
      success: false;
      message: `Connection; error: ${(error as Error).message}`, stack: (error as Error).stack
    };
    results.overall = {
      success: false;
      message: 'Database connection failed', readyForCRUD: false
    };
