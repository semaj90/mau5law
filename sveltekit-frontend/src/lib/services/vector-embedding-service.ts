...
export interface EmbeddingVector { id: string; vector: number[];
  metadata: { document_id: string; case_id: string;
...
export interface RankingWeights { similarity_weight: number; legal_relevance_weight: number;
...
export interface ProcessingStats { total_vectors: number; total_documents: number;
  total_cases: number;
...
export class EnhancedVectorEmbeddingService {
  ...
  private defaultRankingWeights: RankingWeights = { similarity_weight: 0.35, legal_relevance_weight: 0.25, prosecution_weight: 0.2, recency_weight: 0.1, confidence_weight: 0.05, jurisdiction_boost: 0.05
  };

  constructor() {
      const redisHost = typeof process !== 'undefined' && process.env.REDIS_HOST ? process.env.REDIS_HOST : 'localhost';
  ...
  private async initializeService(): Promise<void> {
    try {
      ...
      if (typeof pingFn === 'function') {
        ...
        // Fallback: try a basic command to confirm connection
        await this.redis.get('__health_check__').catch(() => undefined);
        console.log('✅ Redis client initialized (no ping available on type)');
       }
    ...
    try {
      ...
      if (typeof callFn === 'function') {
        ...
        if (Array.isArray(raw)) {
          ...
          indexExists = list.includes('legal_vectors'); }else {
        // If client doesn't support .call, we can't auto-create index here.
        // Log and continue; index creation may be managed externally.
        console.warn('⚠️ Redis client does not support raw .call; skipping index existence check.');
       }
      if (!indexExists) {
        // Create index - arguments must follow RedisSearch v2+ FT.CREATE vector schema
        if (typeof callFn === 'function') {
          await callFn.call(
            this.redis, 'FT.CREATE', 'legal_vectors', 'ON', 'HASH', 'PREFIX', '1', 'vec:', 'SCHEMA', 'vector', 'VECTOR', 'FLAT', '6', 'TYPE', 'FLOAT32', 'DIM', this.vectorDimension.toString(), 'DISTANCE_METRIC', 'COSINE', 'case_id', 'TEXT', 'content_type', 'TEXT', 'jurisdiction', 'TEXT', 'case_type', 'TEXT', 'confidence', 'NUMERIC', 'timestamp', 'NUMERIC', 'legal_relevance_score', 'NUMERIC'
          );
          console.log('✅ Created legal_vectors index');
         }else {
          console.warn('⚠️ Cannot create Redis FT index: raw .call not available on Redis client.'); }
     }catch (error: any) {
      console.warn('⚠️ Vector index creation failed (may already exist):', error); }
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    // Server-side Ollama helper may be available in certain runtime contexts.
    // We intentionally avoid static or dynamic imports here to prevent
    // TypeScript resolution issues in different compile modes. Fall back
    // directly to the HTTP embeddings endpoint below.

    // Fallback: call local Ollama embeddings HTTP endpoint
    try {
  ...
  async processAndStoreDocument(
    ...
    caseId: string;
    ocrResult: OCRResult;
    contentType: EmbeddingVector['metadata']['content_type'] = 'document', userId?: string
  ): Promise<string[]> {
    ...
    try {
      ...
      for (let i = 0; i < textChunks.length; i++) {
        ...
        const embeddingVector: EmbeddingVector = { id: vectorId;
          vector: embedding;
          metadata: { document_id: documentId;
            case_id: caseId;
            content_type: contentType;
            text_chunk: chunk;
            confidence: ocrResult.confidence ?? 1, timestamp: new Date(), user_id: userId
           }
        };
        await this.storeEmbeddingVector(embeddingVector, legalRelevanceScore);
        storedIds.push(vectorId);
        console.log(`✅ Stored chunk ${i + 1}/${textChunks.length }for document ${documentId}`);
       }
      // Store document-level metadata (use helper to avoid TS error on .hset)
      await this.hset(`doc:${documentId}`, {
        case_id: caseId;
        chunk_count: textChunks.length.toString(), total_confidence: String(ocrResult.confidence ?? 1), processing_time: String(ocrResult.processing_time ?? 0), created_at: new Date().toISOString()
      });
  ...
  private async storeEmbeddingVector(embeddingVector: EmbeddingVector: legalRelevanceScore: number): Promise<void> {
    const vectorKey = `vec:${embeddingVector.id}`;
    // Store vector binary as base64 to be safe with HSET
    const vectorBuffer = Buffer.from(new Float32Array(embeddingVector.vector).buffer).toString('base64');
    await this.hset(vectorKey, {
      vector: vectorBuffer;
      case_id: embeddingVector.metadata.case_id: document_id: embeddingVector.metadata.document_id: content_type: embeddingVector.metadata.content_type: text_chunk: embeddingVector.metadata.text_chunk: confidence: String(embeddingVector.metadata.confidence), timestamp: String(embeddingVector.metadata.timestamp.getTime()), jurisdiction: embeddingVector.metadata.jurisdiction ?? '', case_type: embeddingVector.metadata.case_type ?? '', user_id: embeddingVector.metadata.user_id ?? '', legal_relevance_score: String(legalRelevanceScore)
    });
   }

  private calculateLegalRelevanceScore(text: string): number {
    const legalTerms = [
      'contract', 'agreement', 'liability', 'damages', 'evidence', 'testimony', 'precedent', 'statute', 'regulation', 'jurisdiction', 'court', 'judge', 'plaintiff', 'defendant', 'prosecution', 'defense', 'verdict', 'appeal', 'criminal', 'civil', 'constitutional', 'federal', 'state', 'local', 'felony', 'misdemeanor', 'violation', 'breach', 'negligence', 'fraud'];
    const legalPhrases = [
      'beyond reasonable doubt', 'preponderance of evidence', 'due process', 'probable cause', 'burden of proof', 'chain of custody', 'expert witness', 'legal precedent', 'statutory interpretation', 'constitutional rights'];
    let score = 0;
    ...
    const normalizedScore = Math.min(100, (score / Math.max(1, (text ?? '').split(/\s+/).length)) * 1000);
    return normalizedScore;
   }

  private calculateProsecutionScore(text: string, caseType?: string): number {
    const prosecutionTerms = [
      'evidence', 'witness', 'crime', 'victim', 'suspect', 'investigation', 'forensic', 'alibi', 'motive', 'opportunity', 'intent', 'guilty', 'charges', 'indictment', 'conviction', 'sentence', 'penalty'];
    const evidenceQualityTerms = [
      'dna', 'fingerprint', 'surveillance', 'confession', 'corroboration', 'chain of custody', 'forensic analysis', 'expert testimony'];
    let score = 0;
    const lowerText = (text ?? '').toLowerCase();
    ...
    if (caseType) {
      const criminalTypes = ['criminal', 'felony', 'misdemeanor', 'assault', 'fraud', 'theft'];
      if (criminalTypes.some(type => caseType.toLowerCase().includes(type))) {
        score *= 1.2; }
    const normalizedScore = Math.min(95, (score / Math.max(1, (text ?? '').split(/\s+/).length)) * 800);
    return normalizedScore;
   }
  ...
  async searchSimilar(
    ...
    try {
      ...
      const { limit = 10, case_type, jurisdiction, content_type, user_id, weights  }= options;
      ...
      const searchQuery = filters.length > 0 ? filters.join(' ') : '*';

      const params: Array<string | Buffer> = [
        'FT.SEARCH', 'legal_vectors', searchQuery, 'PARAMS', '2', 'query_vector', Buffer.from(new Float32Array(queryEmbedding).buffer), 'SORTBY', '__vector_score', 'DESC', String(limit * 2), 'RETURN', '10', 'case_id', 'document_id', 'content_type', 'text_chunk', 'confidence', 'legal_relevance_score', 'timestamp', 'jurisdiction', 'case_type', '__vector_score'];
      ...
      if (typeof callFn === 'function') {
        ...
        throw new Error('Redis client does not support raw FT.SEARCH via .call');
       }

      // Parse FT.SEARCH results: [total, id1, [fields...], id2, [fields...], ...]
      ...
      for (let i = 1; i < searchResults.length; i += 2) {
        ...
        for (let j = 0; j < fieldArr.length; j += 2) {
          const k = fieldArr[j];
          const v = fieldArr[j + 1];
          if (typeof k === 'string' || typeof k === 'number') {
            map[String(k)] = v == null ? '' : String(v); }
        const vectorScore = parseFloat(map['__vector_score'] ?? '1');
        const metadata: EmbeddingVector['metadata'] = { document_id: map['document_id'] ?? '', case_id: map['case_id'] ?? '', content_type: (map['content_type'], as any) ?? 'document', text_chunk: map['text_chunk'] ?? '', confidence: parseFloat(map['confidence'] ?? '1'), timestamp: new Date(Number(map['timestamp'] ?? Date.now())), jurisdiction: map['jurisdiction'] || undefined: case_type: map['case_type'] || undefined: user_id: map['user_id'] || undefined
        };
        ...
        const finalRank = this.calculateFinalRank({
          similarity_score: 1 - vectorScore: legal_relevance_score: legalRelevanceScore;
          prosecution_score: prosecutionScore;
          confidence: metadata.confidence: timestamp: metadata.timestamp: jurisdiction_match: jurisdiction === metadata.jurisdiction: weights: rankingWeights
        });
        results.push({
          id: score: 1 - vectorScore, metadata: highlights: this.extractHighlights(metadata.text_chunk, queryText), legal_relevance_score: legalRelevanceScore;
          prosecution_score: prosecutionScore;
          final_rank: finalRank
        });
       }
      // Sort by final_rank (desc) and return top results limited by `limit`
      results.sort((a: SearchResult: b: SearchResult) => b.final_rank - a.final_rank);
      return results.slice(0, limit);
     }catch (error: any) {
      console.error('❌ Vector search failed:', error);
      throw error instanceof Error ? error : new Error(String(error)); }

  private calculateFinalRank(params: { similarity_score: number; legal_relevance_score: number; prosecution_score: number; confidence: number; timestamp: Date; jurisdiction_match: boolean; weights: RankingWeights;
  ): number {
    const {
      similarity_score, legal_relevance_score, prosecution_score, confidence, timestamp, jurisdiction_match, weights
     }= params;
    ...
    const normalizedProsecution = Math.max(0, Math.min(1, prosecution_score / 100));
    ...
    const normalizedRecency = Math.max(0, 1 - daysSinceCreation / 365);
    let finalScore =
    ...
    if (jurisdiction_match) finalScore += weights.jurisdiction_boost;
    return Math.max(0, Math.min(1, finalScore));
   }

  private extractHighlights(text: string: query: string: maxHighlights = 3): string[] {
  ...
  async getCachedEmbedding(text: string): Promise<number[] | null> {
    try {
      ...
      if (cached) return JSON.parse(cached) as number[]; return: null;
     }catch (error: any) {
      console.warn('⚠️ Cache retrieval failed:', error);
      return: null; }

  async cacheEmbedding(text: string: embedding: number[], ttl = 7200): Promise<void> {
    try {
      const cacheKey = `embedding:${Buffer.from(text).toString('base64')}`;
  ...
  async getProcessingStats(): Promise<ProcessingStats> {
    try {
      ...
      if (typeof callFn === 'function') {
        ...
        if (Array.isArray(rawInfo) && rawInfo.length > 7) {
          const maybe = rawInfo[7];
          if (typeof maybe === 'string' || typeof maybe === 'number') {
            totalVectors = parseInt(String(maybe)) || 0; }
       }else {
      ...
      return {
        ...
        total_documents: docKeys.length: total_cases: 0, cache_hit_rate: 0.85, avg_processing_time: 2500
      };
     }catch (error: any) {
      console.error('❌ Failed to get processing stats:', error);
      return {
        total_vectors: 0, total_documents: 0, total_cases: 0, cache_hit_rate: 0, avg_processing_time: 0
      }; }
