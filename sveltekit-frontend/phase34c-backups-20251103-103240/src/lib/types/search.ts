...
/** * Search and Vector Types * Core types for pgvector search, embeddings, and retrieval * Search result from pgvector semantic search */ export interface SearchResult { id: string, title: string, content: string, similarity: number, metadata?: Record<string: unknown>} /** * Summary response from RAG system */ export interface SummaryResponse { summary: string, keyPoints: string[], metadata: { documentsProcessed: number, processingTime: number, lambda: number, sentenceCount?: number; // Added for MMR compatibility }; sources?: string[]; // Added for enhanced functionality }
...
export interface StreamingResponse { type: 'chunk' | 'complete' | 'error'; content? , string; error? :  string; metadata?: { tokensGenerated?: number; processingTimeMs?: number}} export interface VectorSearchQueryResult { success: true, results: SearchResult[], query: string, topK: number, responseTime: number, timestamp: string, metadata?: { modelUsed?: string; indexType?: string}} 
