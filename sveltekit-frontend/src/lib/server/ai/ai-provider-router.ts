/**
 * AI Provider Router
 * Intelligent routing between TensorRT-LLM (Triton), vLLM, Ollama, and OpenAI
 * with automatic health monitoring and fallback strategies
 *
 * Provider Priority:
 * 1. TensorRT-LLM via Triton (fastest, local)
 * 2. vLLM (fast, local or remote)
 * 3. Ollama (reliable fallback, local)
...
export class AIProviderRouter {
   ...
   */
  private getBestProvider(): LLMProviderConfig | null {
   ...
   * Get from cache
   */
  private async getFromCache(key: string): Promise<LLMResponse | null> {
