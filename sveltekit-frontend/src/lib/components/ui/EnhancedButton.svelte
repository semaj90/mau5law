<script lang="ts">
 ...
 import { getBitsNamespace } from '$lib/utils/bits-ui-adapter'; _onMount(() => {
		(async () => {
 if (useBits) { try { const ns = await getBitsNamespace(); BitsComponent = ns.Button?.Root ?? ns.Button ?? ns.default?.Button ?? ns} catch { BitsComponent = null}
    } 		})();
