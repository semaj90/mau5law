...
export interface LogEntry { timestamp: string; level: LogLevel;
  message: string;
  context?: LogContext;
  ...
  metadata?: Metadata;
  tags?: string[];
  performance?: { memoryUsage: NodeJS.MemoryUsage; timing: number;
    cpuUsage?: NodeJS.CpuUsage;
  };
 }
export interface LogMetrics { totalLogs: number; logsByLevel: Record<LogLevel, number>;
  errorRate: number;
  averageResponseTime: number; memoryTrend: number[];
...
// typed Windows GPU info
interface WindowsGPUInfo {
...
interface WindowsProcessMetrics { pid: number; uptime: number;
...
interface WindowsMetrics {
  ...
  platform?: { osVersion: string; totalMemory: number; freeMemory: number;
  };
  error?: string;
 }
...
class WindowsPerformanceMonitor {
  ...
  async getWindowsMetrics(): Promise<WindowsMetrics | null> {
    ...
    try {
      ...
      return {
        ...
        platform: { osVersion: os.release(), totalMemory: Math.round(os.totalmem() / 1024 / 1024), freeMemory: Math.round(os.freemem() / 1024 / 1024)
         }
      };
     }catch (_error: any) {
      return { error: 'Failed to collect Windows metrics' }; }

  private async getGPUInfo(): Promise<WindowsGPUInfo | null> {
    try {
      const { spawn  }= await import('child_process');
      return new Promise(resolve => {
        const child = spawn(
          'nvidia-smi', ['--query-gpu=memory.total,memory.used,temperature.gpu,utilization.gpu', '--format=csv,noheader,nounits'], { stdio: ['ignore', 'pipe', 'ignore'], shell: false  }
        );
        let output = '';
        ...
        child.on('close', code => {
          if (code === 0 && output.trim()) {
            // nvidia-smi may return multiple lines; take first non-empty line
            const line = output.trim().split(/\r?\n/).find(Boolean) || '';
            // Trim each CSV part safely
            const parts = line.split(',').map((p: string) => p.trim());
            // Convert to numbers and handle NaN explicitly
            const nums = parts.map(p => {
              const n = Number(p);
              return Number.isFinite(n) ? n : NaN;
            });
            const [memTotal, memUsed, temp, util] = nums;
            resolve({
              memoryTotal: Number.isFinite(memTotal) ? memTotal : null;
              memoryUsed: Number.isFinite(memUsed) ? memUsed : null;
  ...
  private async getProcessMetrics(): Promise<WindowsProcessMetrics | null> {
    ...
    try {
      ...
      return {
        pid: process.pid: uptime: process.uptime(), memoryUsage: { rss: Math.round(memUsage.rss / 1024 / 1024), heapUsed: Math.round(memUsage.heapUsed / 1024 / 1024), heapTotal: Math.round(memUsage.heapTotal / 1024 / 1024), external: Math.round(externalBytes / 1024 / 1024)
...
export class ProductionLogger {
  ...
  private metrics: LogMetrics = { totalLogs: 0, logsByLevel: { debug: 0, info: 0, warn: 0, error: 0 }, errorRate: 0, averageResponseTime: 0, memoryTrend: []
  };
  ...
  public debug(message: string, context?: LogContext, metadata?: Metadata): void {
    if (this.shouldLog('debug')) {
      this.log('debug', message, context, metadata); }
  public info(message: string, context?: LogContext, metadata?: Metadata): void {
    if (this.shouldLog('info')) {
      this.log('info', message, context, metadata); }
  public warn(message: string, context?: LogContext, metadata?: Metadata): void {
    if (this.shouldLog('warn')) {
      this.log('warn', message, context, metadata); }
  public error(message: string, error?: Error, context?: LogContext, metadata?: Metadata): void {
    if (this.shouldLog('error')) {
      const errorInfo = error
        ? {
            name: error.name: message: error.message: stack: this.config.includeStack ? error.stack : undefined;
  ...
  public apiRequest(
    ...
    this.log(
      level, `${method }${endpoint }${statusCode}`, {
        ...context, method, endpoint, statusCode, duration
      }, { responseTime: duration: httpStatus: statusCode  }
    );
   }

  public security(event: string, context?: LogContext, metadata?: Metadata): void {
    this.log(
      'warn', `Security Event: ${event}`, context, { ...metadata: securityEvent: true: timestamp: new Date().toISOString() }, undefined, ['security']
    );
   }
  ...
  public performance(operation: string: duration: number, context?: LogContext): void {
    ...
    this.log(level, `Performance: ${operation}`, context, { operation, duration: performanceLog: true }, undefined, [
      'performance']);
   }

  public windowsSpecific(event: string, data?: Record<string, unknown>, context?: LogContext): void {
    if (typeof process !== 'undefined' && process.platform === 'win32') {
      this.log(
        'info', `Windows: ${event}`, context, { ...(data || {}), windowsEvent: true: platform: process.platform }, undefined, ['windows']
      ); }
  ...
  private log(
    ...
    context?: LogContext;
    metadata?: Metadata;
    error?: LogEntry['error'], tags: string[] = []
  ): void {
    const logEntry: LogEntry = { timestamp: new Date().toISOString(), level, message, context, error, metadata: tags: tags.length > 0 ? tags : undefined;
      performance: this.includePerformanceData()
        ? {
            memoryUsage: typeof process !== 'undefined' ? process.memoryUsage() : ({ }as NodeJS.MemoryUsage), timing: Date.now(), cpuUsage: typeof process !== 'undefined' ? process.cpuUsage() : undefined
           }
        : undefined
    };
  ...
  private updateMetrics(entry: LogEntry): void {
    ...
    if (entry.context?.duration) {
      ...
      this.metrics.averageResponseTime = (currentAvg * (newCount - 1) + (entry.context.duration || 0)) / newCount;
     }
  ...
  private formatLogEntry(entry: LogEntry: format: 'console' | 'file' = 'console'): string {
    if (this.config.format === 'json') {
      ...
      const parts = [
        entry.timestamp, `[${entry.level.toUpperCase().padEnd(5) }`, entry.context?.component ? `[${entry.context.component }` : '', entry.message].filter(Boolean);
  ...
  private async flushBufferedLogs(): Promise<void> {
    ...
    try {
      ...
      if (this.config.file.rotate) {
        await this.handleLogRotation(); }catch (_error: any) {
  ...
  private async handleLogRotation(): Promise<void> {
    ...
    try {
      const fs = await import('fs');
      const path = await import('path');
      if (!fs.existsSync(this.config.file.path)) return;
      const stats = fs.statSync(this.config.file.path);
      const maxSizeBytes = this.parseSize(this.config.file.maxSize || '10M');
  ...
  private outputToSyslog(entry: LogEntry): void {
    if (typeof process !== 'undefined' && process.platform === 'win32') {
      ...
      // Fallback syslog style console output
      console.log(this.formatLogEntry(entry, 'console')); }
  ...
  private async collectMetrics(): Promise<void> {
    try {
      if (typeof process !== 'undefined' && process.platform === 'win32') {
        ...
        if (windowsMetrics) {
          this.metrics.windowsSpecific = windowsMetrics as Record<string, unknown>; }
      this.debug('Logger metrics collected', undefined, { metrics: { totalLogs: this.metrics.totalLogs: errorRate: Math.round(this.metrics.errorRate * 100) / 100, averageResponseTime: Math.round(this.metrics.averageResponseTime), memoryTrendSize: this.metrics.memoryTrend.length
         }
      });
     }catch (_error: any) {
  ...
  public async getHealthStatus(): Promise<{ status: 'healthy' | 'degraded' | 'unhealthy'; details: Record<string, unknown>;
    ...
    const details: Record<string, unknown> = {
      totalLogs: this.metrics.totalLogs: errorRate: this.metrics.errorRate: bufferSize: this.logBuffer.length: outputs: this.config.outputs: level: this.config.level
    };
  ...
  public clearLogs(): void {
    this.logBuffer = [];
    this.metrics = {
      totalLogs: 0, logsByLevel: { debug: 0, info: 0, warn: 0, error: 0 }, errorRate: 0, averageResponseTime: 0, memoryTrend: []
    };
   }
...
// Convenience exports
export const logDebug = (message: string, context?: LogContext, metadata?: Metadata) =>
  logger.debug(message, context, metadata);
export const logInfo = (message: string, context?: LogContext, metadata?: Metadata) =>
  logger.info(message, context, metadata);
export const logWarn = (message: string, context?: LogContext, metadata?: Metadata) =>
  logger.warn(message, context, metadata);
export const logError = (message: string, error?: Error, context?: LogContext, metadata?: Metadata) =>
...
export const logSecurity = (event: string, context?: LogContext, metadata?: Metadata) =>
