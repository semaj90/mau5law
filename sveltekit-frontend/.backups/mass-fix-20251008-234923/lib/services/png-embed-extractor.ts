 ...
 *
 * Embeds legal AI analysis metadata directly into PNG files using custom chunks
 * Enables portable evidence artifacts that work across platforms
 */
...
export interface LegalAIMetadata {
  ...
  // Optional compatibility aliases used by tests/shared types
  processingId?: string;
  confidence?: number;
  entities?: Array<any>;
  semanticHash?: string;
...
export class PNGEmbedExtractor {
  ...
  static async embedMetadata(
    ...
    try {
      // Simple implementation using data URI approach
      // In production, would use proper PNG chunk manipulation
    const metadataJSON = JSON.stringify(metadata);
      const compressedMetadata = await this.compressString(metadataJSON);
      // For now, store as a comment-like structure in PNG
      // Production version would implement proper PNG chunk insertion
      const originalPNG = new Uint8Array(pngBuffer);
      const metadataSize = compressedMetadata.length;
      const newPNG = new ArrayBuffer(pngBuffer.byteLength + metadataSize + 8);
      const view = new Uint8Array(newPNG);
      // Copy original PNG
      view.set(originalPNG, 0);
      // Append metadata (simplified approach)
      const metadataOffset = originalPNG.length;
      view.set(new Uint8Array([0x4C, 0x41, 0x49, 0x44]), metadataOffset); // "LAID" marker
    ...
    try {
      ...
      for (let i = 0; i < view.length - 8; i++) {
        if (view[i] === 0x4C && view[i+1] === 0x41 && view[i+2] === 0x49 && view[i+3] === 0x44) {
          ...
          const sizeBytes = view.slice(i + 4, i + 8);
          const size = new Uint32Array(sizeBytes.buffer)[0];
          if (i + 8 + size <= view.length) {
            const compressedData = view.slice(i + 8, i + 8 + size);
            const metadataJSON = await this.decompressString(compressedData);
            const parsed = JSON.parse(metadataJSON) as LegalAIMetadata;
  ...
  */
  // Overload: accept either a PNG buffer or a metadata-like object directly (used in tests)
  static async validateMetadata(metadata: Record<string, unknown>): Promise<boolean>;
  static async validateMetadata(pngBuffer: ArrayBuffer): Promise<{ valid: boolean; version?: string; checksum_match?: boolean; error?: string }>;
      ...
      if (input && typeof input === 'object' && !(input instanceof ArrayBuffer)) {
        ...
        if (hash) {
          const calculated = await this.calculateSemanticHash({
            ...
            evidence_id: (meta as any).evidence_id || (meta as any).evidenceId || (meta as any).processingId || 'unknown',
  ...
  ): Promise<{ buffer: ArrayBuffer; metadata: { evidenceId: string; evidence_id: string }, integrityHash: string; isValid: boolean }>;
  static async createPortableArtifact(
    imageBuffer: ArrayBuffer
    evidenceId: string
    analysisResults: LegalAIMetadata['analysis_results'],
  ...
  static async createPortableArtifact(
    ...
    arg3?: any
    arg4?: any
  ): Promise<ArrayBuffer | { buffer: ArrayBuffer; metadata: { evidenceId: string; evidence_id: string }, integrityHash: string; isValid: boolean }> {
    // Normalize args
    ...
    if (!calledWithOptions) {
      ...
      const opts = arg2 as {
        caseId?: string; evidenceId?: string; chainOfCustody?: string[];
        analysisResults?: any; additionalData?: Partial<LegalAIMetadata>;
      }
