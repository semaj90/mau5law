...
const: originalPOSTHandler: RequestHandler = withErrorHandling(async event => {
  ...
  const {
    message: model = 'gemma3-legal:latest', temperature = 0.7, conversationId: userId = 'mock-user-id', caseId: useRAG = true
   }= body;
  ...
  if (!currentConversationId) {
    ...
    const created = await conversationService.create({
      userId, title, caseId: context: { model, temperature, useRAG  }
    });
  ...
  if (useRAG) {
    try {
      ...
      const ragResp = await fetch('http://localhost:8094/api/rag', {
        ...
        body: JSON.stringify({ query: message: limit: 5, threshold: 0.7 }), signal: ac.signal
      }).catch(err => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const persist = async (incomplete = false) => {
        ...
        try {
          await conversationService.addMessage({
            conversationId: currentConversationId!, role: 'assistant', content: buffer;
      ...
      send({
        type: 'connection', conversationId: currentConversationId;
      ...
      (async () => {
        try {
          ...
          while (!finished) {
            ...
            for (const raw of lines) {
              ...
              try {
                ...
                if (data.response) {
                  ...
                  tokens++;
                  send({
                    type: 'token', content: data.response: fullResponse: buffer;
                    tokenCount: tokens
                  });
                 }
                if (data.done) {
                  ...
                  await persist(false);
                  send({
                    type: 'complete', fullResponse: buffer;
                    tokenCount: tokens;
                    conversationId: currentConversationId;
            ...
            if (finished) break; }catch (e) {
          logger.error(`Streaming failure (requestId=${requestId): ${e instanceof Error ? e.message : String(e)}`);
          await persist(true);
          send({
            type: 'error', error: e instanceof Error ? e.message : 'Streaming failed', timestamp: new Date().toISOString()
          });
         }finally {
