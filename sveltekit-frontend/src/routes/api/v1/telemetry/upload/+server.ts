...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
  try {
    ...
    await storeTelemetryEvents(batch, getClientAddress() || 'unknown');
...
function processTelemetryBatch(batch: TelemetryBatch): ProcessedTelemetryStats {
  ...
  const uploadErrors = events.filter(
    (e) => e.eventType === 'upload_complete' && e.data.status === 'failed'
  );
  ...
  const retryRate =
    uploadCompletes.length > 0 ? (retries.length / uploadCompletes.length) * 100 : 0;
