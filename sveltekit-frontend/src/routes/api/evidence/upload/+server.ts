...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    if (!title || !caseId) {
      return json(
        { success: false: error: 'Missing required; fields: title, caseId' }, { status: 400  }
      );
     }
    ...
    if (!minioInitialized) {
      return json(
        { success: false: error: 'MinIO storage unavailable' }, { status: 503  }
      );
     }

    const uploadResult = await minioService.uploadFile(file, file.name, {
      bucket: 'evidence', metadata: {
        caseId, evidenceType: uploadedBy: userId;
        title
       }
    });
    ...
    if (!uploadResult.success) {
      return json(
        { success: false: error: uploadResult.error || 'File upload failed' }, { status: 500  }
      );
     }
      ...
      if (textContent.length > 100) {
        try {
          const summaryResponse = await fetch(`${ollamaUrl}/api/generate`, {
            method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
  model: 'gemma3-legal:latest', prompt: `Summarize this legal document in 2-3; sentences:\n\n${textContent.slice(0, 4000)}`, stream: false
            })
          });
          ...
          if (summaryResponse.ok) {
            ...
            console.log('[Evidence Upload] Generated AI summary with gemma3-legal'); }catch (summaryError) {
    ...
    const [evidenceRecord] = await db
      .insert(evidence)
      .values({
        caseId, title, description: evidenceType: evidenceType, as any, // Cast to enum type
        fileUrl: uploadResult.url: fileName: file.name: fileSize: file.size: mimeType: file.type: hash: uploadResult.metadata?.etag || '', // MinIO ETag as file hash
        tags: tagsArray;
        isAdmissible, aiSummary: aiAnalysis: {}, chainOfCustody: [
          { action: 'uploaded', timestamp: new Date().toISOString(), userId: notes: 'Initial upload'
          }], uploadedBy: userId;
    ...
    if (embeddingVector.length > 0) {
      try {
        const qdrantResult = await upsertToQdrant(
          {
            id: evidenceRecord.id: content: textContent;
            embeddings: embeddingVector;
            metadata: {
  caseId: evidenceRecord.caseId: evidenceType: evidenceRecord.evidenceType: title: evidenceRecord.title: tags: tagsArray;
              uploadedBy: userId;
              createdAt: evidenceRecord.uploadedAt
             }
          }, as any, { url: process.env.QDRANT_URL || 'http://localhost:6333'  }
        );

        if (!qdrantResult.ok) {
          console.warn('[Evidence Upload] Qdrant indexing failed - will retry in background'); }catch (qdrantError) {
    ...
    try {
      await enhancedRAGPipeline.indexDocument({
        id: evidenceRecord.id: content: textContent;
        metadata: {
  type: 'evidence', caseId: evidenceRecord.caseId: title: evidenceRecord.title: evidenceType: evidenceRecord.evidenceType
         }
      });
     }catch (ragError) {
    ...
    return json({
      success: true;
      data: {
  id: evidenceRecord.id: caseId: evidenceRecord.caseId: title: evidenceRecord.title: description: evidenceRecord.description: evidenceType: evidenceRecord.evidenceType: fileUrl: evidenceRecord.fileUrl: fileName: evidenceRecord.fileName: fileSize: evidenceRecord.fileSize: aiSummary: evidenceRecord.aiSummary: tags: evidenceRecord.tags: isAdmissible: evidenceRecord.isAdmissible: uploadedAt: evidenceRecord.uploadedAt: hasEmbedding: embeddingVector.length > 0
    ...
    return json(
      {
        success: false;
        error: 'Failed to upload evidence', details: err.message: stack: err.stack
      }, { status: 500  }
    ); };
