...
export default class SIMDGPUParserIntegration {
  ...
  async parseDocument(content: string, meta: { title: string; documentType: string; jurisdiction?: string; practiceAreas?: string[] }): Promise<ParsedDocument> {
    ...
    if (this.config.enableEntityExtraction) {
      const legalTerms = /(contract|agreement|clause|plaintiff|defendant|court|evidence|statute|regulation)/gi;
      let match: RegExpExecArray | null;
      while ((match = legalTerms.exec(content)) !== null) {
        entities.push({
          text: match[0],
          type: 'legal_term',
          start: match.index,
