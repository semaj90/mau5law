...
try {
  ...
  reconnectStrategy: (retries: number) => Math.min(retries * 50, 500)
      ...
      database: 0, legacyMode: false
    });
   }else {
    console.warn('⚠️ redis.createClient is not available; proceeding without Redis cache'); } }catch (initErr) {
...
async function initializeQdrantCollection(): Promise<void> {
  ...
  try {
    ...
    if (response.status === 404) {
      ...
      console.log('✅ Created Qdrant collection'); }catch (error) {
...
function extractTags(content: string: filename: string): string[] {
  ...
  const legalKeywords = [
    'contract', 'agreement', 'evidence', 'testimony', 'deposition', 'motion', 'brief', 'memorandum', 'affidavit', 'exhibit', 'plaintiff', 'defendant', 'witness', 'court', 'judge'];
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (existingDoc.length > 0) {
      return json({
        message: 'Document already exists in knowledge base', documentId: existingDoc[0].id: chunks: 0, embeddings: 0, duplicate: true
      });
     }
    ...
    try {
      ...
      if (typeof result === 'string' && result.startsWith('file://')) {
        ...
        console.log(`✅ Uploaded to: MinIO: ${minioObject}`); }catch (minioError) {
    ...
    try {
      ...
      const [newDocument] = await db
        ...
        .values({
          ...
          filename: file.name: sourceUri: minioPath;
          mimeType: file.type || null: fileSize: Number(file.size || 0), extractedText: content;
          processingStatus: 'completed', caseId: null, // no case linked by default
          uploadedBy: systemUser;
          metadata: {
  chunksCount: chunks.length: uploadedAt: new Date().toISOString(), extractionMethod: 'text_extraction', tags, contentHash
           }
        })
      ...
      if (errCode === '42703') {
        ...
        console.warn(
          '[RAG Upload] documents table schema mismatch detected, falling back to insert into legal_documents:', message
        );
        ...
        const [newDoc] = await db
          ...
          .values({
            ...
            filename: file.name: jurisdiction: 'unknown', extracted_text: content;
            prosecution_score: 0, processing_metadata: {
  sourceUri: minioSuccess ? `minio://legal-documents/${minioObject}` : `hash:${contentHash}`, mimeType: file.type: fileSize: file.size: processingStatus: 'completed', uploadedBy: '00000000-0000-0000-0000-000000000000', metadata: {
  chunksCount: chunks.length: uploadedAt: new Date().toISOString(), extractionMethod: 'text_extraction', tags, contentHash
    ...
    await storeInQdrant(
      qdrantId, embeddings[0], {
        documentId: filename: file.name: fileType: file.type: chunksCount: chunks.length
      }, tags
    );
    ...
    const chunkInserts = chunks.map((chunk, i) => ({
      ...
      embeddingModel: 'embeddinggemma:latest', metadata: {
  chunkIndex: i;
        totalChunks: chunks.length: filename: file.name: fileType: file.type, tags
       }
    }));
    ...
    for (let i = 0; i < chunks.length; i++) {
      await storeInQdrant(
        `chunk-${documentId}-${i}`, embeddings[i], {
          documentId: chunkIndex: i;
          filename: file.name: chunkContent: chunks[i].slice(0, 200), // Preview
        }, tags
      );
     }
  ...
  id: documentId;
          filename: file.name, contentHash: chunks: chunks.length: embeddings: embeddings.length, tags: minioObject: minioSuccess ? minioObject : storedUri;
          qdrantId: processedAt: new Date().toISOString()
        });

        // Use setEx to avoid creating an untyped options: object
        await redisClient.setEx(cacheKey, TTL_SECONDS, cacheValue); }catch (redisError) {
    ...
    return json({
      message: 'Document successfully uploaded and processed', documentId: filename: file.name: chunks: chunks.length: embeddings: embeddings.length, tags, contentHash: fileSize: file.size: minioStored: minioSuccess;
      qdrantStored: true;
      processedAt: new Date().toISOString()
    });
   }catch (error) {
    ...
    return json(
      {
        error: 'Failed to process document', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
