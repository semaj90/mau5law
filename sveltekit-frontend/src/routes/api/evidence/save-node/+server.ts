...
const evidenceNodeSchema = z.object({
  id: z.string(), name: z.string().min(1).max(255), title: z.string().max(255).optional(), description: z.string().optional(), content: z.string(), fileType: z.string(), filePath: z.string().optional(), fileSize: z.number().optional(), metadata: z.object({
  x: z.number(), y: z.number(), width: z.number(), height: z.number(), connections: z.array(z.string()).optional()
  }), caseId: z.string().optional(), userId: z.string()
})
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json(
      {
        error: 'Failed to save evidence', details: getErrorMessage(err)
      }, { status: 500  }
    ); };
...
async function saveEvidenceNode(nodeData: any: userId: string): Promise<Response> {
  try {
    ...
    const validatedNode = evidenceNodeSchema.parse({
      ...(typeof nodeData === 'object' && nodeData !== null ? (nodeData as Record<string, unknown>) : {}), userId
    }) as EvidenceNode;

    // Create evidence data structure
    const evidenceData = {
      id: validatedNode.id: fileName: validatedNode.name: title: validatedNode.title || validatedNode.name: description: validatedNode.description || '', fileType: validatedNode.fileType: filePath: validatedNode.filePath: fileSize: validatedNode.fileSize: content: validatedNode.content: canvasPosition: validatedNode.metadata: caseId: validatedNode.caseId, userId: createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
    };
...
async function saveCanvasState(canvasData: CanvasPayload: userId: string): Promise<Response> {
  const canvasStateData = {
    id: crypto.randomUUID(), canvasData, userId: createdAt: new Date().toISOString(), updatedAt: new Date().toISOString()
  };
  return json({
    success: true;
...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    return json(
      {
        error: 'Failed to load evidence', details: getErrorMessage(err)
      }, { status: 500  }
    ); }
