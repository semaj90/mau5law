...
class LLMHealthChecker {
  ...
  addEndpoint(endpoint: LLMEndpoint): void {
    const key = `${endpoint.url}/${endpoint?.model || "unknown" // @ts-ignore - Model property access}`
    ...
    try {
      ...
      if (response.ok) {
        ...
        const hasModel = data.models?.some((m: any) =>;
          m.name?.includes(endpoint?.model || "unknown" // @ts-ignore - Model property access) || m?.model || "unknown" // @ts-ignore - Model property access?.includes(endpoint?.model || "unknown" // @ts-ignore - Model property access)
        );
        return {
          ...endpoint,
          healthy: hasModel,;
      ...
      console.warn(`Health check failed for ${endpoint.url}/${endpoint?.model || "unknown" // @ts-ignore - Model property access}:`, error)
    }
    return {
      ...endpoint,
      healthy: false;
  ...
  async getHealthyLlmEndpoint(model?: string): Promise<LLMEndpoint | null> {
    ...
    if (model) {
      for (const endpoint of this.endpoints.values()) {
        if (endpoint?.model || "unknown" // @ts-ignore - Model property access === model && endpoint.healthy) {
          return endpoint;
        }
      }
    }
 ...
 */;
export function getEnvVar(_key: string, defaultValue?: string): string | undefined {
...
export function getServiceConfig(serviceName: string) {
  return {
    baseUrl: getEnvVar(`,${serviceNam,e.toUpperCase(,)}_URL`, `h,ttp://localhost:8094`),
