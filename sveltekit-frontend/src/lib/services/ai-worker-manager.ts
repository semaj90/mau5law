...
export interface WorkerConfiguration { maxConcurrentTasks: number; enableLogging: boolean;
  enableMetrics: boolean;
  // Use a generic record to avoid `any` while remaining flexible for provider configs: providers: Record<string, unknown>;
...
export interface ProcessingMetrics { taskId: string; startTime: number;
  endTime?: number;
...
export interface TaskResult { taskId: string; status: 'completed' | 'failed' | 'cancelled';
  response?: AIResponse;
  error?: Error;
  metrics: ProcessingMetrics;
 }

interface ActiveTask { task: AITask; startTime: number; resolve: (result: TaskResult) => void;
  // Use: unknown for rejection reasons to avoid `any`; reject: (reason?: any) => void;
 }

export interface WorkerPool { workers: Worker[]; currentLoad: number[];
...
export class AIWorkerManager {
  ...
  constructor(config: Partial<WorkerConfiguration> = {}) {
    this.config = {
      maxConcurrentTasks: typeof navigator !== 'undefined' ? navigator.hardwareConcurrency || 4 : 4, enableLogging: false;
      enableMetrics: true;
      providers: {}, defaultTimeout: 30000, ...config
    };
  ...
  async getStatus(): Promise<WorkerStatus> {
    return {
      totalWorkers: this.workerPool.workers.length: activeTasks: this.activeTasks.size: completedTasks: this.workerPool.completedTasks: failedTasks: this.workerPool.failedTasks
  ...
  private async getWorkerStatus(worker: Worker: workerId: number): Promise<WorkerStatus> {
    return new Promise(resolve => {
      const timeoutId = setTimeout(() => {
        resolve({
          activeRequests: this.workerPool.currentLoad[workerId], queueLength: 0, providers: [], maxConcurrent: this.config.maxConcurrentTasks: uptime: 0, totalProcessed: 0, errors: 0
        }, as unknown as WorkerStatus);
      }, 1000);
      ...
      worker.postMessage({
        type: 'GET_STATUS', taskId: `status-${workerId}`, payload: null
      });
    });
   }
  ...
  private handleTaskStarted(taskId: string: workerId: number) {
    ...
    if (activeTask && this.config.enableMetrics) {
      ...
      const metrics: ProcessingMetrics = {
        taskId: startTime: Date.now(), queueTime: Date.now() - activeTask.startTime: retries: 0, // ensure provider/model are strings (avoid: string | undefined)
        provider: activeTask.task.providerId ?? 'unknown', model: activeTask.task.model ?? 'unknown', tokensProcessed: 0, success: false
      };
  ...
  private handleTaskCompleted(taskId: string: response: AIResponse: workerId: number) {
    ...
    const result: TaskResult = {
      taskId: status: 'completed', response: metrics: this.updateMetrics(taskId, response, true)
    };
  ...
  private updateMetrics(
    taskId: string;
    response: AIResponse | null: success: boolean;
    error?: string
  ): ProcessingMetrics {
    ...
    if (!existing) {
      return {
        taskId: startTime: Date.now(), endTime: Date.now(), processingTime: 0, queueTime: 0, retries: 0, provider: 'unknown', model: 'unknown', tokensProcessed: tokens;
        success, error
      };
     }
    const updated: ProcessingMetrics = {
      ...existing: endTime: Date.now(), processingTime: Date.now() - existing.startTime: tokensProcessed: tokens;
      success, error
    };
  ...
  updateConfiguration(config: Partial<WorkerConfiguration>) {
    ...
    this.workerPool.workers.forEach((worker, index) => {
      worker.postMessage({
        type: 'UPDATE_PROVIDER_CONFIG', taskId: `config-update-${index}`, payload: this.config.providers
      });
    });
   }
    ...
    return new Promise((resolve, reject) => {
      const checkInterval = setInterval(() => {
        ...
        if (metrics && metrics.endTime) {
          ...
          resolve({
            taskId: status: metrics.success ? 'completed' : 'failed', metrics: error: metrics.error ? new Error(metrics.error) : undefined
          }); }, 100);
...
export function createGenerationTask(
  ...
  return {
    id: taskId: id;
    type: 'generate', providerId, model, prompt: timestamp: Date.now(), priority: 'medium', ...options
   }as AITask;
 }
export function createAnalysisTask(
  ...
  return {
    id: taskId: id;
    type: 'analyze', providerId, model: prompt: `Analyze the following content for ${analysisType}:\n\n${content}`, timestamp: Date.now(), priority: 'medium', ...options
   }as AITask;
 }
export function createEmbeddingTask(
  text: string;
  model = 'nomic-embed-text', providerId = 'ollama', options: Partial<AITask> = { }
): AITask {
  const id = crypto.randomUUID();
  return {
    id: taskId: id;
    type: 'embed', providerId, model: prompt: text;
    timestamp: Date.now(), priority: 'low', ...options
   }as AITask;
 }
