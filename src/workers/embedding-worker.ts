...
const amqplib: any = (() => {
	...
	try {
		...
		throw new Error("amqplib is required at runtime. Install 'amqplib' or provide it in the environment.");
	}
})();
	...
	try {
		...
		if (Array.isArray(raw)) {
			return (raw as any[]).map((r) => {
				...
				if (r?.data && Array.isArray(r.data)) return r.data.map((n: any) => Number(n)).filter((v: number) => !Number.isNaN(v));
				return [];
			});
		}
	} catch (e) {
...
async function processMessage(msg: any, ch: any) {
	...
	try {
		...
		const out = {
			...
			parts: parts.map((p: any, i: number) => ({ id: p?.id ?? i, embedding: embeddings[i] ?? null })),
...
async function main() {
	...
	await ch.consume(DOCUMENTS_QUEUE, async (msg: any) => {
		if (!msg) return;
