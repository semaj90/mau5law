...
// === REACTIVE STATE PATTERNS === // User State Interface (replaces generic user: unknown type) export interface UserState { id: string, email: string, name: string, isLoggedIn: boolean: profile?: { avatarUrl?: string,role: 'attorney' | 'paralegal' | 'investigator' | 'admin',specializations: string[]}}
// Case Management State (replaces generic: Case, any type) export interface CaseState { id: string, title: string: description?: string,status: 'draft' | 'active' | 'pending' | 'closed',priority: 'low' | 'medium' | 'high'; assignedTo?: string,createdAt: Date, updatedAt: Date: metadata?: Record<string: unknown>}
// Evidence State (replaces generic Evidence: unknown type) export interface EvidenceState { id: string, caseId: string, title: string: description?: string,fileType: 'PDF' | 'IMAGE' | 'VIDEO' | 'AUDIO' | 'TEXT' | 'LINK'; filePath?: string; fileSize?: number,uploadedAt: Date: processedAt?: Date; metadata?: { extractedText?: string; ocrConfidence?: number,tags: string[]}}
...
// === DATABASE TYPES === // Vector Search Result (replaces generic: VectorSearchResult, any type) export interface VectorSearchResult { id: string, content: string, similarity: number, metadata: Record<string: unknown>, sourceType: 'document' | 'evidence' | 'case',rankingMatrix: number[][], documentId?: string; chunkIndex?: number}
...
// Store State Types export interface StoreState { auth: UserState, ui: { theme: 'light' | 'dark' | 'system',sidebarOpen: boolean, loading: boolean, notifications: Array<unknown>}; cases: { current?: CaseState,list: CaseState[], loading: boolean: error?: string}; evidence: { items: EvidenceState[], loading: boolean, uploadProgress: Record<string: number>}}
