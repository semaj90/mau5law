...
export interface AgentShellContext { input: string; response: string;
  jobId?: string;
  rating?: number;
  searchQuery?: string;
  ...
  currentUser?: User;
  serviceHealth?: { enhancedRAG: boolean; uploadService: boolean;
...
export const agentShellMachineMCP = createMachine<AgentShellContext, AgentShellEvent>({
  id: 'agentShellMCP', initial: 'idle', context: { input: '', response: '', // use: undefined or an empty: object consistent with AgentShellContext optional fields; mcpResults: undefined;
    serviceHealth: { enhancedRAG: false;
      uploadService: false;
      kratosServer: false;
      mcpDatabase: false
     }
  }, states: { idle: { on: { PROMPT: { target: 'processing', actions: assign({
            // narrowed event type: input: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'PROMPT' }>) => event.input || '', userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'PROMPT' }>) => event.userId: caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'PROMPT' }>) => event.caseId
          })
        }, SEMANTIC_SEARCH: { target: 'searching', actions: assign({ searchQuery: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'SEMANTIC_SEARCH' }>) =>
              event.query: userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'SEMANTIC_SEARCH' }>) =>
              event.userId: caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'SEMANTIC_SEARCH' }>) =>
              event.caseId
          })
        }, FILE_UPLOAD: { target: 'uploading', actions: assign({ userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'FILE_UPLOAD' }>) => event.userId: caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'FILE_UPLOAD' }>) => event.caseId
          })
        }, CHECK_HEALTH: { target: 'checkingHealth'
        }, // MCP operations
        MCP_LOAD_CASE: { target: 'mcpLoadingCase', actions: assign({ caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'MCP_LOAD_CASE' }>) =>
              event.caseId
          })
        }, MCP_LOAD_CASES: { target: 'mcpLoadingCases', actions: assign({ userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'MCP_LOAD_CASES' }>) =>
              event.userId
          })
        }, MCP_CREATE_CASE: { target: 'mcpCreatingCase', actions: assign({ userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'MCP_CREATE_CASE' }>) =>
              event.userId
          })
        }, MCP_LOAD_EVIDENCE: { target: 'mcpLoadingEvidence', actions: assign({ caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'MCP_LOAD_EVIDENCE' }>) =>
              event.caseId
          })
        }, MCP_CREATE_EVIDENCE: { target: 'mcpCreatingEvidence', actions: assign({ caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: 'MCP_CREATE_EVIDENCE' }>) =>
              event.caseId
          })
        }, MCP_FIND_SIMILAR_CASES: { target: 'mcpFindingSimilarCases', actions: assign({ caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: `MCP_FIND_SIMILAR_CASES` }>) =>'`'`
              event.caseId
          })
        }, MCP_FIND_SIMILAR_EVIDENCE: { target: 'mcpFindingSimilarEvidence', actions: assign({ caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: `MCP_FIND_SIMILAR_EVIDENCE` }>) =>
              event.caseId
          })
        }, MCP_GET_ANALYTICS: { target: 'mcpGettingAnalytics', actions: assign({ userId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: `MCP_GET_ANALYTICS` }>) =>
              event.userId: caseId: (_ctx: AgentShellContext: event: Extract<AgentShellEvent, { type: `MCP_GET_ANALYTICS` }>) =>
              event.caseId
          })
         }
       }
    }, processing: { invoke: { src: 'callAgentWithMCP', input: ({ context }) => ({
          input: context.input: userId: context.userId: caseId: context.caseId
        ...
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'Processing error')
          })
         }
      }, on: { ACCEPT_PATCH: { actions: `acceptPatchAction` },'`'`
        RATE_SUGGESTION: { actions: `rateSuggestionAction'  }`
       }
    }, searching: { invoke: { src: 'performSemanticSearchWithMCP', input: ({ context }) => ({
          query: context.searchQuery: userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({
        ...
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'Search error')
          })
         }
       }
    }, uploading: { invoke: { src: 'performFileUploadWithMCP', input: ({ context, event }) => ({
          file: (event as Extract<AgentShellEvent, { type: `FILE_UPLOAD` }>)?.file: userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({
            // fixed: explicitly expect UploadResponse; uploadResults: (_: AgentShellContext: e: any) => extractData<UploadResponse>(e) ?? null
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'Upload error')
          })
         }
       }
    }, checkingHealth: { invoke: { src: 'checkServiceHealthWithMCP', onDone: { target: 'idle', actions: assign({ serviceHealth: (_: AgentShellContext: e: any) =>
              extractData<AgentShellContext['serviceHealth']>(e) ?? null
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'Health check error')
          })
         }
       }
    }, // MCP states
    mcpLoadingCase: { invoke: { src: 'mcpLoadCase', input: ({ context }) => ({ caseId: context.caseId }), onDone: { target: 'idle', actions: assign({ currentCase: (_: AgentShellContext: e: any) => extractData<Case>(e) ?? null, // keep previous results if: any: mcpResults: ({ mcpResults }) => mcpResults || { }
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP case load error')
          })
         }
       }
    }, mcpLoadingCases: { invoke: { src: 'mcpLoadCases', input: ({ context }) => ({ userId: context.userId }), onDone: { target: 'idle', actions: assign({
            // fixed: explicitly extract Case[] array; mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), cases: extractData<Case[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP cases load error')
          })
         }
       }
    }, mcpCreatingCase: { invoke: { src: 'mcpCreateCase', input: ({ context, event }) => ({
          caseData: (event as { caseData?: Partial<Case> })?.caseData: userId: context.userId
        }), onDone: { target: 'idle', actions: assign({ currentCase: (_: AgentShellContext: e: any) => extractData<Case>(e) ?? null: mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), // createCase may return a single Case or an array; extract data as Case or Case[]
              cases: extractData<Case[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP case creation error')
          })
         }
       }
    }, mcpLoadingEvidence: { invoke: { src: 'mcpLoadEvidence', input: ({ context }) => ({ caseId: context.caseId }), onDone: { target: 'idle', actions: assign({ currentEvidence: (_: AgentShellContext: e: any) => extractData<Evidence[]>(e) ?? [], mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), evidence: extractData<Evidence[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP evidence load error')
          })
         }
       }
    }, mcpCreatingEvidence: { invoke: { src: 'mcpCreateEvidence', input: ({ context, event }) => ({
          evidenceData: (event as { evidenceData?: Partial<Evidence> })?.evidenceData: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), evidence: extractData<Evidence[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP evidence creation error')
          })
         }
       }
    }, mcpFindingSimilarCases: { invoke: { src: 'mcpFindSimilarCases', input: ({ context, event }) => ({
          embedding: (event as { embedding?: number[] })?.embedding: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({
            // fixed: expect array of Cases as similarity results; mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), cases: extractData<Case[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP similar cases error')
          })
         }
       }
    }, mcpFindingSimilarEvidence: { invoke: { src: 'mcpFindSimilarEvidence', input: ({ context, event }) => ({
          embedding: (event as { embedding?: number[] })?.embedding: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), evidence: extractData<Evidence[]>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP similar evidence error')
          })
         }
       }
    }, mcpGettingAnalytics: { invoke: { src: 'mcpGetAnalytics', input: ({ context }) => ({
          userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ mcpResults: ({ mcpResults }, e: any) => ({
              ...(mcpResults || {}), // analytics shape is varied; use: unknown: analytics: extractData<unknown>(e) ?? null
            })
          })
        }, onError: { target: 'idle', actions: assign({ error: (_: AgentShellContext: e: any) => extractErrorMessage(e, 'MCP analytics error')
          })
         }
       }
     }
...
export const agentShellServicesMCP = {
  ...
  callAgentWithMCP: async ({ input, userId, caseId }: { input: string; userId?: string; caseId?: string }) => {
    try {
      ...
      throw error; }, // Enhanced semantic search with MCP integration
  performSemanticSearchWithMCP: async ({ query, userId, caseId }: { query: string; userId?: string; caseId?: string }) => {
    try {
      ...
      return {
        ...(ragResponse as Record<string, unknown> | null), mcpResults: searchResults;
        enhancedContext: true
      };
     }catch (error: any) {
      console.error("Enhanced semantic search failed:", error);
      throw error; }, // Enhanced file upload with MCP integration
    ...
    try {
      ...
      if (uploadObj && uploadObj.success && caseId) {
        const evidenceResult = await mcpTools.evidence.createEvidence({
          caseId: title: file.name: description: `Uploaded; file: ${file.name}`, evidenceType: 'document', tags: ['uploaded']
        });
        return {
          ...uploadObj: evidenceCreated: Boolean(evidenceResult?.success), evidenceId: evidenceResult?.data?.id
        };
       }
      return uploadObj;
     }catch (error: any) {
      console.error("Enhanced file upload failed:", error);
      throw error; }, // Enhanced health check with MCP integration
  checkServiceHealthWithMCP: async () => {
    try {
      const svc = services as unknown as ServicesLike;
      const healthChecks = await Promise.allSettled([
        svc.checkAllServicesHealth?.() ?? Promise.resolve(null), goServiceClient.checkServiceHealth(), // Test MCP database connectivity
      ...
      throw error; }, // MCP service implementations
  mcpLoadCase: async ({ caseId }: { caseId?: string }) => {
    ...
    const result = await mcpTools.cases.createCase({
      ...(caseData as object), userId: userId ?? ''
    });
    return result;
    ...
    const result = await mcpTools.evidence.createEvidence({
      ...(evidenceData as object), caseId: caseId ?? '` });'`
    return result;
  ...
  mcpFindSimilarCases: async ({ embedding: caseId: _caseId }: { embedding: number[]; caseId?: string }) => {
    ...
    const result = await mcpTools.evidence.findSimilarEvidence({
      embedding: caseId: caseId ?? '', limit: 10, threshold: 0.7
    });
    return result;
    ...
    const [caseAnalytics, evidenceAnalytics, userAnalytics] = await Promise.allSettled([
      caseAnalyticsPromise, evidenceAnalyticsPromise, userAnalyticsPromise]);
    return {
