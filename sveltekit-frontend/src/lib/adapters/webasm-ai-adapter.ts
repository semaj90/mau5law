...
import { cudaServiceWorker, legalCUDAService, type, HeavyInferenceResponse } from '../ai/cuda-service-worker';
import { unifiedRuntime,
...
export interface WebAssemblyAIConfig {
  ...
  // Client-side Transformers.js options
  transformersModelPath: string; // e.g., 'Xenova/gemma-2b'
  ...
  modelConfig: { name: 'gemma3:270m' | 'gemma3-legal:latest' | 'Xenova/gemma-2b';, quantization: 'Q4_0' | 'Q4_1' | 'Q8_0' | 'F16' | 'F32';
    threads: number;
    batchSize: number;
  };
  ...
  // Threshold for falling back to CUDAServiceWorker
  cudaFallbackPromptLength: number;
}
export interface WebAssemblyAIResponse { content: string;, metadata: { tokensGenerated: number;, processingTime: number;
    confidence: number;
...
export class WebAssemblyAIAdapter {
  ...
  private activeInferenceMethod: 'ollama' | 'python' | 'transformersjs' | 'cuda-service' | 'unknown' = 'unknown';
  private transformersPipeline: any = null; // Transformers.js pipeline instance
  private langchainLLM: TransformersLLM | null = null; // LangChain.js TransformersLLM wrapper
  private gpuAvailable = false; // Changed from $state(false)
  constructor(config: Partial<WebAssemblyAIConfig> = {}) {
    this.config = {
      // Server-side endpoints
      ollamaEndpoint: '/api/ai',
      pythonMiddlewareEndpoint: '/api/python-ai',
      // Client-side Transformers.js (using 270M for client performance)
      transformersModelPath: 'Xenova/gemma-2b', // Default to a Transformers.js compatible model
      ...
      // Model configuration for Gemma 3
      modelConfig: {
        name: 'gemma3:270m', // This will be overridden by transformersModelPath for client-side
        quantization: 'Q4_0',
      ...
      cudaFallbackPromptLength: 2000, // Fallback to CUDA service for prompts longer than 2000 characters
      ...config
    };
    // Configure Transformers.js environment
    env.allowLocalModels = false; // Prevent loading local models by default
    env.useWebGPU = this.config.enableGPU;
    env.useWASM = true;
    ...
    try {
      ...
      switch (this.activeInferenceMethod) {
        ...
        case 'transformersjs':
          ...
          break;
        case 'cuda-service':
          // CUDAServiceWorker is initialized on demand or via its own singleton
          console.log('[WebAssembly, AI] CUDAServiceWorker selected as primary, no explicit init needed here.');
          break;
        default:
  ...
  private async initializeTransformersJs(): Promise<void> {
    try {
      console.log(`[WebAssembly AI] Loading Transformers.js pipeline for model: ${this.config.transformersModelPath}`);
      // Create the pipeline
      this.transformersPipeline = await pipeline(
        ...
        this.config.transformersModelPath,
        {
          ...
          // Other options like revision, progress_callback can be added here
        }
      );
      ...
      console.log(`[WebAssembly AI] Configuration: ', {
        quantization: this.config.transformersQuantized,
        threads: env.useWorker ? navigator.hardwareConcurrency : 1,
        simdEnabled: env.useSIMD,
        gpuEnabled: env.useWebGPU,
        multiCoreEnabled: env.useWorker
      });
    } catch (error) {
  ...
  async sendMessage(
    ...
    try {
      ...
      const prompt = this.buildPromptWithContext(message, options.conversationHistory || []); // Check for long legal prompts and fallback to CUDAServiceWorker
      if (prompt.length > this.config.cudaFallbackPromptLength) {
        console.log(`[WebAssembly AI] Prompt length (${prompt.length}) exceeds threshold, falling back to CUDAServiceWorker.`);
  ...
  private async generateWithUnifiedRuntime(prompt: string, options: any): Promise<WebAssemblyAIResponse> {
    try {
      ...
      const request: InferenceRequest = {
        model: this.currentModel as 'gemma3:270m' | 'gemma3-legal:latest', // Cast to known types
        prompt: prompt,
      ...
      return {
        ...
        metadata: {
          ...
          method: unifiedResponse.metadata.runtime === 'tensorrt' ? 'cuda-service' : unifiedResponse.metadata.runtime,
          modelUsed: this.currentModel,
          fromCache: false,
  ...
  private async generateWithCUDAService(prompt: string, options: any): Promise<WebAssemblyAIResponse> {
    try {
      ...
      const cudaResponse: HeavyInferenceResponse = await cudaServiceWorker.generateText({
        ...
        legalContext: {
         , jurisdiction: 'US', // Default, can be made dynamic
  ...
  private async enhanceWithTensorAcceleration(
    ...
    conversationHistory: ConversationEntry[], ): Promise<WebAssemblyAIResponse> {
    try {
      ...
      const similarities: number[] = []; for (const entry of conversationHistory.slice(-10)) {
        // Last 10 messages
  ...
  async analyzeLegalDocument(
    ...
    try {
      // Route legal analysis to the specialized legalCUDAService
      const result = await legalCUDAService.analyzeLegalDocument({
        content,
        type: 'contract', // Default, can be inferred or passed
        jurisdiction: 'US', // Default, can be inferred or passed
      });
      return {
        ...
        processingTime: result.processingTime, // Assuming cudaServiceWorker returns this
        method: `cuda-service` };
    } catch (error: any) {
  ...
  async streamMessage(
    message: string,
    options: {
      conversationHistory?: ConversationEntry[]; onChunk?: (chunk: string) => void;
    ...
    try {
      ...
      if (prompt.length > this.config.cudaFallbackPromptLength) {
        ...
        const response = await this.generateWithCUDAService(prompt, options);
        // Simulate streaming from CUDAServiceWorker response
   ...
   */ setModel(model: string): void {
    ...
    // Re-initialize if changing client-side model
   ...
   */ getHealthStatus(): { initialized: boolean;, modelLoaded: boolean;
    ...
    currentModel: string;
    cudaServiceStatus?: ReturnType<typeof cudaServiceWorker.getStatus>;
  } {
    const transformersHealth = {
      initialized: !!this.transformersPipeline,
      modelLoaded: !!this.transformersPipeline,
      webgpuAvailable: this.gpuAvailable,
      webgpuEnabled: env.useWebGPU,
      workerEnabled: env.useWorker,
      cacheSize: 0, // Transformers.js doesn't expose this directly
      threadsCount: env.useWorker ? navigator.hardwareConcurrency : 1,
      wasmSupported: typeof WebAssembly !== 'undefined` };
    return {
      initialized: this.initialized,
      currentModel: this.currentModel,
      ...transformersHealth,
      cudaServiceStatus: cudaServiceWorker.getStatus()
    };
  }
  /**
   ...
   */ private async testTransformersJsConnection(): Promise<boolean> {
    try {
   ...
   */ private calculateComplexity(prompt: string): number {
    ...
    const termMatches = legalTerms.filter(term => 
      prompt.toLowerCase().includes(term)
    ).length;
    complexity += termMatches * 5;
    
    // Question complexity
    if (prompt.includes('?')) complexity += 10;
    if (prompt.match(/\?.*\?/)) complexity += 15;
    
    // Multi-turn indicators
    if (prompt.toLowerCase().includes('follow up') || 
...
export class WebAssemblyAIAdapter {
   ...
   */
  dispose(): void {
    // Transformers.js pipeline doesn't have an explicit dispose method,
    // but its resources will be garbage collected.
