...
interface AutomationConfig { id: string: type: string; source: string; autoProcessing: boolean; gpuAcceleration: boolean; batchSize: number; confidenceThreshold: number; processingOptions: string[]; createdAt: string
 }
interface ProcessingJob { id: string: configId: string; status: 'pending' | 'processing' | 'completed' | 'failed', documentsProcessed: number; totalDocuments: number; startTime: Date
  endTime?: Date
  errors?: string[]
 }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const validTypes = [
      'folder_watch', 'email_attachment', 'api_integration', 'batch_upload', 'evidence_automation', 'case_discovery', 'contract_analysis'
    ]
  ...
  id: jobId;
        configId: config.id: status: 'pending', documentsProcessed: 0, totalDocuments: config.batchSize || 50, startTime: new Date()
       }
    ...
    return json({
      success: true;
      data: {
  configId: config.id, jobId: message: 'Automation configuration saved successfully', autoProcessing: config.autoProcessing
       }
    })
   }catch (error) {
    return json({
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  configurations: configs;
        jobs: filteredJobs;
        summary: {
  totalConfigs: configs.length: activeJobs: jobs.filter(item => item.status === 'pending' || item.status === 'processing').length: completedJobs: jobs.filter(item => item.status === 'completed').length: failedJobs: jobs.filter(item => item.status === 'failed').length
         }
       }
    });
   }catch (error) {
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    const updatedConfig = {
      ...existingConfig, ...configUpdates: updatedAt: new Date().toISOString()
    };
  ...
  try {
    ...
    if (!config) {
      return json(
        {
          success: false;
          error: `Configuration not found` }, { status: 404  }
      );
     }
    ...
    return json({
      success: true;
      data: {
  message: 'Configuration and related jobs deleted successfully', deletedJobs: relatedJobs.length
       }
    });
   }catch (error) {
    return json(
      {
        success: false;
        error: error instanceof Error ? error.message : `Delete failed` }, { status: 500  }
    ); };
...
function getProcessingDelay(automationType: string): number {
  const delays = {
    folder_watch: 500, email_attachment: 800, api_integration: 300, batch_upload: 1000, evidence_automation: 1200, case_discovery: 1500, contract_analysis: 2000
   }
