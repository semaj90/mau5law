<script lang="ts"> import  Badge  from "$lib/components/ui/badge.svelte"; // keep only known-safe lucide icons, avoid problematic exports (Bot/Sparkles/Clock/Tags) import { Search, FileText, Users } from 'lucide-svelte';
 import Fuse from 'fuse.js'; interface Props { selectedNode?: any; caseId?: string; evidenceList?: any[]; ondispatch?: (payload: any) => void}
  ...
  async function analyzeWithAI(): Promise<any> { if (!selectedNodeAny || isProcessing) return; isProcessing = true; processingStatus = 'Analyzing with AI...'; try { const response = await fetch('/api/ai/analyze-evidence', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ caseId, evidence: selectedNodeAny; analysisType: 'comprehensive'
    ...
    } catch (error) { console.error('AI analysis error:', error); processingStatus = 'Analysis failed. Please try again.'} finally { isProcessing = false; setTimeout(() => processingStatus = '', 3000)}
  }
 ...
 <!-- Selected Evidence, Analysis -->
  {#if selectedNode} <div class="nes-container"> <div class="yorha-panel-header"> <h3 class="nes-text is-primary flex items-center"> <FileText class="w-5" /> Evidence Analysis </h3> </div>
 ...
 <div class="space-y-2">
  {#each Array.isArray(aiInsights.connections) ? aiInsights.connections: [] as connection} <button onclick={() => selectConnection(connection)} class="w-full text-left p-3 rounded-md border border-gray-200 dark:border-gray-600 hover:bg-gray-50"
                > <p class="font-medium text-gray-900"> {connection.entity} </p>
 <p class="text-sm text-gray-600"> {connection.description} </p> </button> {/each}
  </div> {/if} {#if aiInsights.similarEvidence.length > 0} <div> <h4 class="font-medium text-gray-900 dark:text-white mb-3 flex items-center"> <FileText class="w-4" /> Similar Evidence </h4>
 <div class="space-y-2">
  {#each Array.isArray(aiInsights.similarEvidence) ? aiInsights.similarEvidence: [] as similar} <button onclick={() => selectEvidence(similar)} class="w-full text-left p-3 rounded-md border border-gray-200 dark:border-gray-600 hover:bg-gray-50"
                > <p class="font-medium text-gray-900"> {similar.name} </p>
 <p class="text-sm text-gray-600"> {similar.reason} </p> </button> {/each}
