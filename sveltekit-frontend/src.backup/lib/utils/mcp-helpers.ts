...
const agentRegistry: Record<string, (prompt: string, context?: unknown) => Promise<AgentResult>> = {
  autogen: async (prompt: string, context?: unknown) => { // Added types
    try {
      ...
      if (typeof autoGenService?.executeLegalWorkflow === 'function') {
        return { agent: 'autogen', result: await autoGenService.executeLegalWorkflow('legal_research', prompt, context ?? {}) }; // Corrected prompt argument
      } else {
        return { agent: 'autogen', result: `AutoGen agent (mock): Analyzed: "${prompt}" - would provide legal research workflow results` };
      }
    } catch (err: unknown) {
  ...
  crewai: async (prompt: string, _context?: unknown) => { // Added types
    try {
      // guard legalTeam before invoking
      if (typeof legalTeam?.analyzeCase === 'function') {
        return { agent: 'crewai', result: await legalTeam.analyzeCase({ query: prompt, analysisType: 'legal_research', priority: 'medium' }) }; // Corrected priority string
      } else {
  ...
  copilot: async (prompt: string, _context?: unknown) => { // Added types
    try {
      ...
      const response = await fetch(`${ollamaBase}/api/generate`, {
        ...
        body: JSON.stringify({ model: 'gemma3-legal-latest', prompt: `As a coding assistant, analyze and provide suggestions for: ${prompt}`, stream: false }) // Corrected body syntax, model name, stream syntax
      });
      ...
      return { agent: 'copilot', result: `Copilot agent (mock) Code analysis; for: "${prompt}" - would provide coding suggestions and optimizations` }; // Corrected string
    }
  },
  claude: async (prompt: string, _context?: unknown) => { // Added types
    try {
      const ollamaBase = getOllamaEndpoint();
      const response = await fetch(`${ollamaBase}/api/generate`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model: 'gemma3-legal-latest', prompt: `As a legal AI assistant, provide detailed analysis for: ${prompt}`, stream: false }) // Corrected body syntax, model name, stream syntax
      });
      if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
      const data = (await response.json()) as Record<string, unknown>; // Corrected syntax
      return { agent: 'claude', result: data.response ?? `Claude legal analysis for: ${prompt}` }; // Corrected string
    } catch (_err: unknown) {
      return { agent: 'claude', result: `Claude agent (mock) Legal analysis; for: "${prompt}" - would provide detailed legal insights and case analysis` }; // Corrected string
    }
  },
  rag: async (prompt: string, _context?: unknown) => { // Added types
    try {
      // leave RAG URL resolution as-is for now (could be centralized similarly)
      const ragUrl = typeof window !== 'undefined' ? 'http://localhost:5173' : 'http://localhost:5173'; // Corrected URL string
      ...
      return { agent: 'rag', result: `RAG agent (mock) Enhanced retrieval; for: "${prompt}" - would provide context-aware document analysis` }; // Corrected string
    }
  }
};
...
export function getOllamaEndpoint(): string {
  ...
  const viteUrl = typeof import.meta !== 'undefined' ? ((import.meta as ImportMeta & { env?: ViteEnvShape }).env ?? {}).VITE_OLLAMA_URL : undefined; // Corrected syntax
  const nodeUrl = typeof process !== 'undefined' && typeof process.env !== 'undefined' ? (process.env as NodeJS.ProcessEnv).OLLAMA_URL : undefined; // Corrected syntax
  const dockerEnvUrl = typeof process !== 'undefined' && typeof process.env !== 'undefined' ? (process.env as NodeJS.ProcessEnv).DOCKER_OLLAMA_URL : undefined; // Corrected syntax
...
export function generateMCPPrompt(request: MCPToolRequest): string { // Added type
  const { tool, component, context, area, feature, requirements, library, topic, query, maxResults, caseId, filePath, documentType, documentId, integrationType } = request;
  switch (tool) {
    case 'analyze-stack':
...
export function validateMCPRequest(request: MCPToolRequest): { valid: boolean, errors: string[] } { // Added type
  ...
  switch (request.tool) {
    ...
    case 'rag-query':
      ...
      if (request.maxResults && (request.maxResults < 1 || request.maxResults > 50)) errors.push('Max results must be between 1 and 50'); // Corrected string
      if (request.confidenceThreshold && (request.confidenceThreshold < 0 || request.confidenceThreshold > 1)) errors.push('Confidence threshold must be between 0 and 1'); // Corrected string
      break;
    case 'rag-upload-document':
    ...
    case 'rag-integration-guide':
      ...
      if (
        request.integrationType && !['api-integration', 'component-integration', 'search-ui', 'document-upload'].includes(request.integrationType)
      ) {
        errors.push(
...
export const commonMCPQueries = {
  // Stack Analysis
  analyzeSvelteKit: (): MCPToolRequest => ({ tool: 'analyze-stack', component: 'sveltekit', context: 'legal-ai' }), // Corrected syntax
  analyzeDrizzle: (): MCPToolRequest => ({ tool: 'analyze-stack', component: 'drizzle', context: 'legal-ai' }),
  analyzeUnoCSS: (): MCPToolRequest => ({ tool: 'analyze-stack', component: 'unocss', context: 'performance' }),
  // Best Practices
  performanceBestPractices: (): MCPToolRequest => ({ tool: 'generate-best-practices', area: 'performance' }),
  securityBestPractices: (): MCPToolRequest => ({ tool: 'generate-best-practices', area: 'security' }),
  ...
  aiChatIntegration: (): MCPToolRequest => ({ tool: 'suggest-integration', feature: 'AI chat component', requirements: 'legal compliance and audit trails' }),
  documentUploadIntegration: (): MCPToolRequest => ({ tool: 'suggest-integration', feature: 'document upload system', requirements: 'security and virus scanning' }),
  gamingUIIntegration: (): MCPToolRequest => ({ tool: 'suggest-integration', feature: 'gaming-style UI components', requirements: 'professional legal interface' }),
  // Library Documentation
  svelteKitRouting: (): MCPToolRequest => ({ tool: 'get-library-docs', library: 'sveltekit', topic: 'routing' }),
  bitsUIDialog: (): MCPToolRequest => ({ tool: 'get-library-docs', library: 'bits-ui', topic: 'dialog' }),
  drizzleSchema: (): MCPToolRequest => ({ tool: 'get-library-docs', library: 'drizzle', topic: 'schema' }),
  // RAG System Queries
  ragStats: (): MCPToolRequest => ({ tool: 'rag-get-stats' }),
  ragLegalQuery: (query: string, caseId?: string): MCPToolRequest => ({ tool: 'rag-query', query, caseId, maxResults: 10, confidenceThreshold: 0.7, documentTypes: ['contract', 'case_law', 'statute', 'evidence'] }), // Corrected syntax
  ragContractAnalysis: (query: string): MCPToolRequest => ({ tool: 'rag-query', query, maxResults: 5, confidenceThreshold: 0.8, documentTypes: ['contract', 'agreement'] }), // Corrected syntax
  ragCaseLawSearch: (query: string): MCPToolRequest => ({ tool: 'rag-query', query, maxResults: 15, confidenceThreshold: 0.75, documentTypes: ['case_law', 'judgment', 'precedent'] }), // Corrected syntax
  ragEvidenceSearch: (query: string, caseId: string): MCPToolRequest => ({ tool: 'rag-query', query, caseId, maxResults: 20, confidenceThreshold: 0.6, documentTypes: ['evidence', 'exhibit', 'testimony'] }), // Corrected syntax
  ragApiIntegration: (): MCPToolRequest => ({ tool: 'rag-integration-guide', integrationType: 'api-integration' }),
  ragComponentIntegration: (): MCPToolRequest => ({ tool: 'rag-integration-guide', integrationType: 'component-integration' }), // Corrected backticks
  ragSearchUI: (): MCPToolRequest => ({ tool: 'rag-integration-guide', integrationType: 'search-ui' }), // Corrected backticks
  ragDocumentUpload: (): MCPToolRequest => ({ tool: 'rag-integration-guide', integrationType: 'document-upload' })
};
/** * Format MCP response for display */
export function formatMCPResponse(response: any): string { // Added type
...
function isRecord(value: any): value is Record<string, unknown> { // Added type
  return typeof value === 'object' && value !== null;
}
function formatContentItem(item: any): string { // Added type
...
function tryGetStringProp(obj: Record<string, unknown>, prop: string): string | undefined { // Added type
  const val = obj[prop];
...
export function generateClaudePrompt(request: MCPToolRequest): string { // Added type
...
export async function semanticSearch(query: string): Promise<unknown[]> { // Added type
  try {
    const response = await fetch('http://localhost:3000/api/semantic-search', { // Corrected URL string
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
export async function mcpMemoryReadGraph(): Promise<unknown[]> {
  try {
    return [
      { node: 'legal-workflow-memory', relations: ['case-evidence', 'document-analysis'], value: `Context7 memory graph integration ready` }
    ];
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err); // Corrected comma
    return [{ error: msg } as unknown];
...
export async function mcpCodebaseAnalyze(prompt: string): Promise<unknown[]> { // Added type
  try {
    return [
      {
...
export async function mcpReadDirectory(path: string): Promise<string[]> { // Added type
  try {
...
export async function mcpReadErrorLog(): Promise<unknown[]> {
  try {
    ...
    return [{ id: 'err-1', message: 'Sample error from MCP', severity: 'low', timestamp: new Date().toISOString() }]; // Corrected syntax
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err); // Corrected comma
    return [{ error: msg }];
  }
}
export async function mcpRankErrors(errorLog: any): Promise<unknown[]> { // Added type
  try {
    ...
    if (Array.isArray(errorLog)) {
      const ranked = (errorLog as Array<Record<string, unknown>>).slice().sort((a, b) => {
        const score = (e: Record<string, unknown>) => (e.severity === 'critical' ? 3 : e.severity === 'high' ? 2 : 1); // Corrected syntax
...
export function synthesizeLLMOutputs(results: any): string { // Added type
  try {
    ...
    if (Array.isArray(r.agentResults)) {
      for (const a of r.agentResults as AgentOutcome[]) {
        if (a.error) parts.push(`${a.agent} ERROR: ${a.error}`); // Corrected space
...
export async function mcpSuggestBestPractices(results: any): Promise<AutoMCPSuggestion[]> { // Added type
  try {
    const r = results as Record<string, unknown>; // Corrected syntax
    const suggestions: AutoMCPSuggestion[] = [];
    if (r.codebase) {
      suggestions.push({ type: 'enhancement', original: 'Codebase configuration', suggested: 'Enable SSR for sensitive legal routes and add strict CSP headers', reasoning: 'Prevents client-side leakage of sensitive legal documents and improves auditability', confidence: 0.85 });
    }
    if (r.changedFiles) {
      suggestions.push({ type: 'alternative', original: 'Manual changed-file review', suggested: 'Add CI diff analysis and automated security linting', reasoning: 'Automated checks reduce human error during releases', confidence: 0.75 });
    }
    if (suggestions.length === 0) {
      suggestions.push({ type: 'enhancement', original: 'No suggestions generated', suggested: 'Run multi-agent analysis with agents: ["autogen","crewai","copilot"] and enable synthesizeOutputs', reasoning: 'Gather broader diagnostics and synthesized insights', confidence: 0.6 }); // Corrected string
    }
    return suggestions;
  } catch (err: unknown) {
    const msg = err instanceof Error ? err.message : String(err); // Corrected comma
    return [
      { type: 'alternative', original: 'mcpSuggestBestPractices failed', suggested: 'Check MCP connectivity and input results', reasoning: msg, confidence: 0.1 }
    ];
  }
}
