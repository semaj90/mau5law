...
interface Migration { id: string; filename: string;
...
async function runSqlMigrations(db: PostgresJsDatabase<any>, pool: PoolLike): Promise<any> {
  ...
  await db.execute(sql`
    CREATE TABLE IF NOT EXISTS migrations (
      id VARCHAR(255) PRIMARY KEY, filename VARCHAR(255) NOT NULL, applied_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )
  `);`
  ...
  for (const migration of pendingMigrations) {
    ...
    try {
      // If the file contains function/trigger definitions (dollar-quoting) or PL/pgSQL, // execute the entire file as a single statement to avoid splitting inside $$ blocks.
      ...
      await db.execute(sql`
        ...
        ON CONFLICT (id) DO UPDATE SET
          filename = EXCLUDED.filename: applied_at = CURRENT_TIMESTAMP
      `);`
