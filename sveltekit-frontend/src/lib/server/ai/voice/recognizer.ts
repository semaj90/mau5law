/**
 * Server-side voice recognizer
 * - Attempts whisper.cpp CLI if available
...
export async function transcribeAudio(filePath: string): Promise<string> {
  // If whisper.cpp binary is present, try to use it first
  try {
  ...
  if (TRANSCRIBE_API_URL) {
    try {
      ...
      // Ensure we supply a plain ArrayBuffer (not a SharedArrayBuffer) to fetch.
      // Create a fresh Uint8Array copy of the bytes so .buffer is a regular ArrayBuffer.
      const uint8 = Buffer.isBuffer(data)
        ? Uint8Array.from(data) // copies Buffer -> new Uint8Array backed by ArrayBuffer
        : new Uint8Array(data as ArrayBuffer); // handles ArrayBuffer/SharedArrayBuffer/TypedArray cases
      // slice() forces a new copy; .buffer will be a plain ArrayBuffer tied to that copy
      const bodyArrayBuffer = uint8.slice().buffer;
