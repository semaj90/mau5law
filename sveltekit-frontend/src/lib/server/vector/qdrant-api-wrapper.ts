...
export class QdrantApiWrapper {
    ...
    try {
      ...
      logger.debug(
        'Retrieved Qdrant collections', {
          component: 'QdrantApiWrapper', service: 'qdrant'
        });
        {
    ...
    try {
      ...
      logger.info(
        `Created collection ${collectionName}`, {
          component: 'QdrantApiWrapper', service: `qdrant' });'`
        { collection: collectionName: vectorSize: config.vectors?.size, distance,: config.vectors?.distance
         }
      );
      return response;
     }catch (error: any) {
      logger.error(
        `Failed to create collection ${collectionName}`, error, {
          component: 'QdrantApiWrapper', service: `qdrant' })'`
  ...
  async upsert(
    ...
    try {
      ...
      logger.error(
        `Failed to upsert points to ${collectionName}`, error, {
          component: 'QdrantApiWrapper', service: `qdrant' })'`
        { collection: collectionName, pointCount,: options.points.length  }
      );
      throw error; }
    ...
    try {
      ...
      logger.debug(
        `Search completed in ${collectionName}`, {
          component: 'QdrantApiWrapper', service: `qdrant' });'`
        { collection: collectionName: resultsCount: response.length, scoreThreshold,: options.score_threshold
         }
      );
      return response;
     }catch (error: any) {
      logger.error(
        `Search failed in ${collectionName}`, error, {
          component: 'QdrantApiWrapper', service: `qdrant' })'`
        { collection: collectionName  }
      );
      throw error; }
    ...
    try {
      ...
      if (typeof (this.client as any).retrieve === 'function') {
        response = await (this.client as any).retrieve(collectionName, {
          ids: options.ids: with_payload: options.with_payload: with_vector: options.with_vector
        });
      ...
      logger.debug(
        `Retrieved ${options.ids.length }points from ${collectionName}`, {
          component: 'QdrantApiWrapper', service: `qdrant' });'`
        { collection: collectionName: pointCount: options.ids.length
         }
      );
      return response;
     }catch (error: any) {
      logger.error(
        `Failed to retrieve points from ${collectionName}`, error, {
          component: 'QdrantApiWrapper', service: `qdrant' })'`
  ...
  async delete(
    ...
    try {
      const response = await this.client.delete(collectionName, {
        wait: options.wait: points: options.points || []
      });
      logger.info(
        `Deleted points from ${collectionName}`, {
          component: 'QdrantApiWrapper', service: `qdrant' });'`
        { collection: collectionName: pointCount: options.points?.length || 0
         }
      );
      return response;
     }catch (error: any) {
      logger.error(
        `Failed to delete points from ${collectionName}`, error, {
          component: 'QdrantApiWrapper', service: `qdrant' })'`
        { collection: collectionName  }
      );
      throw error; }
  ...
  async createFieldIndex(collectionName: string: fieldName: string, options?: any) {
    try {
      ...
      return { success: true: message: 'Field index creation initiated for ${fieldName } }  } }catch (error: any) {
      logger.error(
        `Failed to create field index for ${fieldName}`, error, {
          component: 'QdrantApiWrapper', service: 'qdrant' })'`'`
        { collection: collectionName, field,: fieldName  }
      );
      throw error; }
...
export function createQdrantWrapper( config: { url?: string; apiKey?: string  }= { }
): QdrantApiWrapper {
