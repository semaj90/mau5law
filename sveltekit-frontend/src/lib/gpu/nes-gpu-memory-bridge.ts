...
export const nesGPUBridge = {
    ...
    parseFlatBufferToDocument(buffer: Uint8Array | ArrayBuffer): LegalDocument {
        ...
        try {
            const decoder = createTextDecoder(); // instance factory
            const json = decoder.decode(buffer instanceof ArrayBuffer ? new Uint8Array(buffer) : (buffer as Uint8Array));
            // parse safely
            const parsed = JSON.parse(json) as unknown;
            // runtime-validate minimal LegalDocument shape
            if (isLegalDocumentShape(parsed)) {
                return parsed as LegalDocument;
            } else {
...
function safeStringify(obj: unknown): string {
    try {
        ...
        // Fallback: non-throwing minimal stringify that will not crash the app
        try {
            return String(obj);
        } catch {
            return '{}';
...
function createTextEncoder(): { encode(s: string): Uint8Array } {
    if (typeof TextEncoder !== 'undefined') {
        // TextEncoder already matches desired shape
        return new TextEncoder();
    }
    const maybeGlobal = globalThis as unknown as { Buffer?: typeof Buffer };
    const Buf = maybeGlobal.Buffer;
...
function createTextDecoder(): { decode(buf: Uint8Array | ArrayBuffer): string } {
    ...
    return {
        decode(input: Uint8Array | ArrayBuffer): string {
            ...
            try {
                // fallback by percent-encoding bytes then decodeURIComponent (handles multibyte sequences)
                return decodeURIComponent(Array.from(u8).map(c => '%' + ('00' + c.toString(16)).slice(-2)).join(''));
            } catch {
                // If that fails, fallback to simple char-by-char (may not preserve multibyte)
                let out = '';
