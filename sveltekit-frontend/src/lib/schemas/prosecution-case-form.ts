...
export const CaseStatusEnum = z.enum(['open', 'pending_review', 'in_discovery', 'pre_trial', 'trial', 'post_trial', 'closed', 'archived']);
export const CasePriorityEnum = z.enum(['low', 'medium', 'high', 'critical', 'urgent']);
export const CaseTypeEnum = z.enum(['felony', 'misdemeanor', 'violation', 'infraction', 'civil', 'traffic', 'other']);
export const JurisdictionEnum = z.enum(['state', 'federal', 'county', 'municipal', 'tribal']);
export const EvidenceStatusEnum = z.enum(['available', 'pending', 'lost', 'destroyed', 'sealed', 'unavailable']);
...
export const EvidenceItemSchema = z.object({
  ...
  description: z.string().optional(),
  type: z.enum(['document', 'photo', 'video', 'audio', 'physical', 'digital', 'forensic', 'witness_statement']),
...
export function estimateCaseCompleteness(form: Partial<ProsecutionCaseForm>): number {
  const fields: (keyof ProsecutionCaseForm)[] = [
    'caseNumber', 'title', 'who', 'what', 'when', 'where',
    'narrative', 'defendants', 'evidence', 'charges'
  ];
  const filled = fields.filter(f => form[f] && (Array.isArray(form[f]) ? (form[f] as any[]).length > 0 : true)).length;
