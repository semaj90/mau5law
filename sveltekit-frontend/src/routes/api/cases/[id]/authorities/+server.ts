...
export const POST: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		console.error('[authorities] POST error:', err);
		return json({ error: 'Failed to link authority' }, { status: 500 });
	}
};
...
export const DELETE: RequestHandler = async ({ params, request, locals }) => {
	...
	try {
		...
		console.error('[authorities] DELETE error:', err);
		return json({ error: 'Failed to remove authority' }, { status: 500 });
	}
};