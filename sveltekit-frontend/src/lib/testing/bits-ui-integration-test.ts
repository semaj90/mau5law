    ...
    try {
      ...
      if (typeof document !== 'undefined') {
        ...
        if (ariaElements.length > 0) {
          this.addResult('Accessibility', 'pass', `Found ${ariaElements.length} ARIA-compliant elements`);
        } else {
          this.addResult('Accessibility', 'warning', 'No ARIA elements found - may not be loaded yet');
        }
        ...
        if (focusableElements.length > 0) {
          this.addResult('Accessibility', 'pass', `Found ${focusableElements.length} focusable elements`);
        } else {
          this.addResult('Accessibility', 'warning', 'No focusable elements found');
        }

      } else {
        this.addResult('Accessibility', 'warning', 'Running in SSR mode, accessibility check skipped');
      }
    ...
    try {
      ...
      setTimeout(() => {
        ...
        if (renderTime < 100) {
          ...
          this.addResult('Performance', 'warning', `Moderate render time: ${renderTime.toFixed(2)}ms`);
        } else {
