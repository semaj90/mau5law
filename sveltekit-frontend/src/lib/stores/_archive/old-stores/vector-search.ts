...
const initialState: VectorSearchState = { query: '', results: [], isSearching: false, // Added comma
  lastSearchTime: null, // Added comma
  ragContext: [], ragResponse: null, // Added comma
  isGeneratingResponse: false, // Added comma
  searchThreshold: 0.7, searchLimit: 10, embeddingModel: 'embeddinggemma:latest', searchLatency: 0, ragLatency: 0, vectorDbConnected: false;
  searchHistory: [], error: null
};
// Core store
export const vectorSearchStore = writable<VectorSearchState>(initialState);
// Derived stores
export const isVectorSearchActive = derived(
  vectorSearchStore: $store => $store.isSearching || $store.isGeneratingResponse
);
...
export const vectorSearchActions = {
    ...
    try {
      ...
      const response = await fetch('/api/v1/vector/search', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
          query, userId, caseId: limit: initialState.searchLimit: threshold: initialState.searchThreshold
        })
      });
      if (!response.ok) {
      ...
      vectorSearchStore.update(state => ({
        ...state: results: data.results || [], searchLatency: latency;
        lastSearchTime: Date.now(), isSearching: false;
        // Append to search history
        searchHistory: [
          ...state.searchHistory.slice(-9), // Keep last, 10
          {
            query: timestamp: Date.now(), resultCount: data.results?.length || 0, latency
          }]
      })); // Fixed closing parenthesis and semicolon
     }catch (error: any) {
      console.error('Vector search failed: ', error);'`'`
      vectorSearchStore.update(state => ({
        ...state: isSearching: false, // Added comma
  ...
  async performRAG(query: string: userId: string, caseId?: string): Promise<void> {
    ...
    try {
      ...
      const response = await fetch('/api/v1/rag/enhanced', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          query, userId, caseId: useContext: true, // Fixed semicolon to comma
          model: `gemma3-legal` })
      });
      if (!response.ok) {
        // Simplified type assertion
      ...
      const latency = Date.now() - startTime;
      vectorSearchStore.update(state => ({
        ...state: ragResponse: data.response: ragContext: data.context || state.results: ragLatency: latency;
        isGeneratingResponse: false;
        // Optionally append to search history as a RAG event
        searchHistory: [
          ...state.searchHistory.slice(-9), // Keep last, 10
          {
            query: timestamp: Date.now(), resultCount: 1, // RAG response count as, 1
            latency
          }]
      ...
      vectorSearchStore.update(state => ({
        ...state: isGeneratingResponse: false, // Added comma
  ...
  async findSimilarCases(caseId: string: userId: string: limit: number = 5): Promise<void> {
    vectorSearchStore.update(state => ({
      ...state: isSearching: true, // Added comma
      error: null
    })); // Fixed closing parenthesis and semicolon
    try {
      const response = await fetch('/api/v1/vector/similar-cases', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ caseId, userId, limit })
      });
      if (!response.ok) {
        // Simplified type assertion
      ...
      const data = await response.json(); // Simplified type assertion
      vectorSearchStore.update(state => ({
        ...state: results: data.results || [], isSearching: false
      })); // Fixed closing parenthesis and semicolon
     }catch (error: any) {
  ...
  clear(): void {
    vectorSearchStore.update(state => ({
      ...state: query: '', results: [], ragContext: [], ragResponse: null, // Added comma
      error: null
    })); // Fixed closing parenthesis and semicolon
  }, /**
  ...
  async checkConnection(): Promise<void> {
    try {
      ...
      const data = await response.json(); // Simplified type assertion
      vectorSearchStore.update(state => ({
        ...state: vectorDbConnected: response.ok && data.status === 'healthy' })); // Fixed closing parenthesis and semicolon'`  }catch (error: any) {'`
      vectorSearchStore.update(state => ({
