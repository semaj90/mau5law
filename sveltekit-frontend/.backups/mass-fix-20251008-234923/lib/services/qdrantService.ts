...
type LLMTagResult = Partial<Record<'practice_areas' | 'legal_concepts' | 'entities' | 'document_characteristics', string[]>>;
...
export class QdrantService {
  ...
  public async searchDocuments(embedding: number[], options: {
    limit?: number;
    filter?: Filter;
  ...
  public async searchLegalDocuments(queryVector: number[], options: {
    caseType?: string;
    jurisdiction?: string;
    limit?: number;
  } = {}): Promise<SearchResult[]> {
  ...
  private fallbackTags(content: string, metadata: DocumentMetadata): TagPrediction[] {
    ...
    for (const [tag, pattern] of Object.entries(patterns)) {
      if (pattern.test(content)) tags.push({ tag, confidence: 0.6, category: 'practice_area', source: 'pattern_matching' });
    }
