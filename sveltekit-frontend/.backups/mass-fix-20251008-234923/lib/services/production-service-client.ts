/**
 * Production Service Client - Minimal Stub
...
class ProductionServiceClient {
   ...
   */
  async makeRequest<T = unknown>(endpoint: string, data?: unknown, options?: RequestInit, requestTimeout?: number): Promise<ServiceResponse<T>> {
   ...
   */
  async semanticSearch(query: string, options?: SemanticSearchOptions): Promise<ServiceResponse<SemanticSearchResult[]>> {
   ...
   */
  async uploadFile(file: File, options?: UploadFileOptions): Promise<ServiceResponse<FileUploadResult>> {
    const startTime = Date.now();
    try {
  ...
  async checkAllServicesHealth(): Promise<Record<string, ServiceHealth>> {
    const services = ['enhanced-rag', 'upload-service', 'legal-gateway'];
