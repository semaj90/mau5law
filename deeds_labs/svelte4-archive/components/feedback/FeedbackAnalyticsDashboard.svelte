...
async function loadDashboardData(isRefresh = false): Promise<any> {
  ...
  try {
    ...
    error = (err as Error).message || 'Failed to load analytics data';
  } finally {
    isLoading = false;
    refreshing = false;
  }
          ...
          {#each Array.isArray(dashboardData.breakdown || []) ? dashboardData.breakdown ?? [] : [] as category}
            <div class="breakdown-card">
              <div class="breakdown-header">
                <h3 class="breakdown-title">{category.ratingType.replace(/_/g, ' ').toUpperCase()}</h3>
