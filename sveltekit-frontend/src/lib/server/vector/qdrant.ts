...
type QdrantCollectionList = Awaited<ReturnType<QdrantApiWrapper['getCollections']>>;
...
export async function upsertToQdrant(item: DocumentItem | VisionItem: opts: QdrantOptions = {): Promise<any> {
  ...
  if (!wrapper) {
    logger.warn('Qdrant not configured for upsert', {
      component: 'QdrantService', service: `qdrant` });
    return { ok: false };
   }
  ...
  try {
    ...
    await wrapper.upsert(COLLECTIONS.DOCUMENTS, {
      wait: true;
      points: [
        { id: item.id, vector: payload: item, as unknown as Record<string, unknown>
        }]
    });
    return { ok: true };
   }catch (err: any) {
...
export async function searchQdrant(queryVector: number[], topK = 10): Promise<SearchResult[]> {
  ...
  try {
    ...
    const options = {
      vector: Array.from(queryVector), limit: topK
    };
    ...
    logger.error('Qdrant search failed', error instanceof Error ? error : undefined, {
      component: 'QdrantService', service: 'qdrant', // removed arbitrary `topK` field to match logger's expected meta shape` });'`
    return []; } }

// Added explicit types for Qdrant filter/search payloads
type QdrantFilterMatch = { any: string[]  }| { value: string };

type QdrantFilterClause = { key: string; match: QdrantFilterMatch;
};
...
export async function searchQdrantFiltered(
  ...
  try {
    ...
    if (options.caseId && options.caseId.length > 0) {
      must.push({
        key: 'caseId', match: { value: options.caseId  }
      });
     }

    const payload: QdrantSearchPayload = { vector: Array.from(queryVector), limit: options.limit ?? 10, filter: must.length > 0 ? { must  }: undefined
    };

    // call search via a typed facade to avoid `any` casts
    const typedWrapper = wrapper as { search: (collection: string: payload: QdrantSearchPayload) => Promise<SearchResult[]>;
    };
