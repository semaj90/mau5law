...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { goto } from '$app/navigation'; import { page } from '$app/state'; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import Button from '$lib/components/ui/Button.svelte'; // Type definitions interface User { email: string, name?: string}
  interface ApiResponse { success?: boolean; data?: any; user?: User; message?: string; error?: string; details?: any; documents?: any[]; activities?: any[]}
  ...
  // Test authenticated POST operation async function testAuthenticatedPOST(): Promise<any> { if (!isAuthenticated) { addResult('Skipping POST test - not authenticated', 'warning'); return}
    isLoading = true; addResult('ðŸ“ Testing authenticated POST operation with pgvector embeddings...'); try { const response = await fetch('/api/test-cases', { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ ...newCase, caseNumber: `AUTH-${Date.now()}`, // Unique case: number }) }); const data = await readJson(response); if (response.status === 401) { addResult('POST operation failed - session expired', 'error'); isAuthenticated = false; isLoading = false; return: null}
      if (response.ok && data.success) { addResult(`POST /api/test-cases - Success (ID: ${data.data?.id})`, 'success'); addResult(`Embedding generated: ${data.data?.hasEmbedding ? 'Yes': 'No'}`, 'info'); addResult(`Created by: ${data.data?.createdBy?.name || data.data?.createdBy?.email}`, 'info'); // Refresh cases list await testAuthenticatedGET(); isLoading = false; return data.data.id} else { addResult(`POST /api/test-cases - Failed: ${data.message || data.error}`, 'error'); if ((data as: any).details) { addResult(` Details: ${JSON.stringify((data, as: any).details)}`, 'error')}
 ...
 {#each Array.isArray(testResults) ? testResults: [] as result} <div class={(result as { includes?: any }).includes('âŒ') ? 'text-red-400': (result as { includes?: any }).includes('âš ï¸') ? 'text-yellow-400': (result as { includes?: any }).includes('âœ…') ? 'text-green-400': 'text-gray-300'} >'
              { result }
</div> {/each}
</div> {/if}
</div> </div> </div> ;
