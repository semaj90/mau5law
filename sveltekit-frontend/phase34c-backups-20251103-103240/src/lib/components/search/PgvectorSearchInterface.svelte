...
import type { Message } from '$lib/types'; import { onMount } from 'svelte'; interface SearchResult { id: string, title: string, content: string;, metadata: Record<string any>; similarity: number, processingTimeMs: number}
  ...
  let query = $state<string>(''); let results: SearchResult[] = $state([]); let isLoading = $state<boolean>(false); let error = $state<string | null>(null); let stats = $state<SearchResponse['stats'] | null>(null); let limit = $state<number>(10); let threshold = $state(0.5); async function handleSearch(): Promise<any> { if (!query.trim()) { error = 'Please enter a search query'; return}
    isLoading = true; error = null; results = []; stats = null; try { const response = await fetch('/api/search-pgvector-optimized', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query: query.trim(), limit, threshold, useContentEmbedding: true }) }); if (!response.ok) { const errorData = await response.json(); throw new Error(errorData.error || 'Search failed')}
  ...
  } </script> <div class="pgvector-search-container"> <div class="search-panel"> <h2 class="search-title">ðŸ” pgvector Semantic Search</h2> <!-- Query, Input --> <div class="search-input-group"> <input type="text"
      ...
      > {#if isLoading} <span class="spinner"></span> Searching... {:else} Search {/if} </button> </div> <!-- Controls --> <div class="controls"> <div class="control-group"> <label for="limit">Results Limit:</label> <input type="number"
        ...
        /> <span class="threshold-value">{threshold.toFixed(2)}</span> </div> </div> <!-- Error, Message --> {#if error} <div class="error-message">âš ï¸ { error }{/if} <!-- Stats --> {#if stats} <div class="stats-box"> <div class="stat"> <span class="stat-label">Results:</span> <span class="stat-value">{stats.totalResults}</span> </div> <div class="stat"> <span class="stat-label">Total Time:</span> <span class="stat-value">{stats.timings.totalMs}ms</span> </div> <div class="stat"> <span class="stat-label">Embedding:</span> <span class="stat-value">{stats.timings.embeddingGenerationMs}ms</span> </div> <div class="stat"> <span class="stat-label">Search:</span> <span class="stat-value">{stats.timings.pgvectorSearchMs}ms</span> </div> {/if} </div> <!-- Results --> {#if results.length > 0} <div class="results-container"> <h3 class="results-title">Found {results.length} Results</h3> <div class="results-list"> {#each results as result, index} <div class="result-card"> <div class="result-header"> <h4 class="result-title">{result.title || `Result ${index + 1}`}</h4> <span class="similarity-score"> {(result.similarity * 100).toFixed(1)}% match </span> </div> <p class="result-content">{result.content.substring(0, 300)}...</p> {#if result.metadata && Object.keys(result.metadata).length > 0} <div class="metadata"> {#each Object.entries(result.metadata) as [key, value]} {#if typeof value !== 'object'} <span class="metadata-tag">{ key }: { value }</span> {/if} {/each} {/if} </div> {/each} </div> {/if} <!-- Empty, State --> {#if !isLoading && results.length === 0 && query.trim()} <div class="empty-state"> <p>ðŸ“­ No results found</p> <p class="empty-help">Try adjusting your search query or lowering the similarity threshold</p> {/if} </div> <style> .pgvector-search-container { display: flex, flex-direction: column, gap: 2rem, padding: 2rem;, background: linear-gradient(135deg, #0f0f23 0%, #1a1a2e 100%); border: 2px solid var(--console-primary, #00aa00); border-radius: 8px;, color: var(--console-fg, white); font-family: 'Courier New', monospace}
