...
async function displayDashboard() {
  ...
  if (stats.success) {
    ...
    log(
      `├─ Memory Usage:   ${createProgressBar(memoryPercent)} (${formatBytes(stats.memoryUsed * 1024 * 1024)}/${formatBytes(stats.memoryTotal * 1024 * 1024)})`
    );
    log(
      `├─ Temperature:    ${stats.temperature}°C ${stats.temperature > 80 ? colors.red + '🔥' + colors.reset : stats.temperature > 70 ? colors.yellow + '⚠️' + colors.reset : colors.green + '✅' + colors.reset}`
    );
    log(
      `└─ Power Usage:    ${createProgressBar(powerPercent)} (${stats.power}W/${stats.powerLimit}W)`
    );
