...
export function createUIStore() {
  ...
  const activeScene = derived(
    [markdownScenes, activeSceneId],
    ([$scenes, $id]) => $scenes.find(s => s.id === $id)
  );
  ...
  async function startTypewriter(promptId: string, speed = 50): Promise<void> {
    ...
    for (let i = 0; i <= prompt.text.length; i++) {
      typewriterPrompts.update(ps => ps.map(p =>
        p.id === promptId
          ? { ...p, isTyping: true, displayedText: prompt.text.slice(0, i) }
          : p
      ));
    ...
    typewriterPrompts.update(ps => ps.map(p =>
      p.id === promptId ? { ...p, isTyping: false } : p
    ));
  ...
  function updateFileStatus(fileId: string, status: UploadedFile['status']): void {
    uploadedFiles.update(files =>
      files.map(f => f.id === fileId ? { ...f, status } : f)
    );
  }
  ...
  function validateScene(sceneId: string, validatedBy: string): void {
    markdownScenes.update(scenes =>
      scenes.map(s => s.id === sceneId
        ? { ...s, validated: true, validatedBy, validatedAt: new Date() }
        : s
      )
    );
  }
