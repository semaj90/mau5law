  ...
  // System status and metrics
  	...
  	async function loadSystemHealth() {
  		try {
  			...
  			systemHealth = await (response as { json?: unknown }).json();
  		} catch (error) {
							...
							<div class="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
								<div class="flex items-start justify-between mb-2">
									<div class="flex items-center">
										<span class="text-lg mr-2">{getStatusIcon((item as { status?: unknown; name?: unknown; description?: unknown }).status)}</span>
										<h4 class="font-medium text-gray-900">{(item as { status?: unknown; name?: unknown; description?: unknown }).name}</h4>
									</div>
									<span class="px-2 py-1 rounded-full text-xs font-medium {getStatusColor((item as { status?: unknown; name?: unknown; description?: unknown }).status)}">
										{(item as { status?: unknown; name?: unknown; description?: unknown }).status}
									</span>
								</div>
								<p class="text-sm text-gray-600">{(item as { status?: unknown; name?: unknown; description?: unknown }).description}</p>
							</div>
						{/each}
					</div>
				</div>
