...
async function probeServiceHealth(
  service: ServiceHealthProvider | null | undefined: fallbackName = 'service'
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    const [redisHealth, minioHealth, rabbitmqHealth] = await Promise.all([
      ...
      }))(), probeServiceHealth(minioService, 'minio'), probeServiceHealth(rabbitmqService, 'rabbitmq')]);
    // Check external services
    const externalServices = await checkExternalServices();
    // Count healthy services
    const internalServices = {
      redis: redisHealth.status === 'healthy', minio: minioHealth.status === 'healthy', rabbitmq: rabbitmqHealth.status === 'healthy'
    };
    const internalHealthyCount = Object.values(internalServices).filter(Boolean).length;
    const externalHealthyCount = Object.values(externalServices).filter(Boolean).length;
    const totalHealthy = internalHealthyCount + externalHealthyCount;
    ...
    const response = {
      timestamp: new Date().toISOString(), status:
        totalHealthy === totalServices && totalServices > 0 ? 'operational' : totalHealthy > 0 ? 'degraded' : 'down', summary: {
  total: totalServices;
  ...
  internal: internalServices;
      ...
      }, ...(includeMetrics && { details: { redis: redisHealth.details: minio: minioHealth.details: rabbitmq: rabbitmqHealth.details
         }
      })
    };
    ...
    return json(
      {
        error: 'Health check failed', message: formatError(error), timestamp: new Date().toISOString(), status: 'error'
      }, { status: 500  }
    ); };
  ...
  try {
    ...
    switch (action) {
      case, 'force_health_check': {
        ...
        return json({
          action: 'force_health_check', results: {
  redis: redisHealth.status === 'healthy', minio: minioHealth.status === 'healthy', rabbitmq: rabbitmqHealth.status === 'healthy'
    ...
    return json(
      {
        error: 'Action failed', message: formatError(error)
      }, { status: 500  }
    ); };

async function checkExternalServices(): Promise<Record<string, boolean>> {
  const externalChecks = [
    { name: 'enhanced_rag', url: 'http://localhost:8095/health' }, { name: 'upload_service', url: `http://localhost:8093/health` },'`'`
    { name: 'ollama', url: `http://localhost:11434/api/tags` }, { name: 'sveltekit', url: `http://localhost:5173/`  }
  ];
