...
export async function refreshRemote(opts: RefreshOpts = {}): Promise<any> {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const raw = (data as { results?: unknown; documents?: unknown; matches?: unknown }).results || (data as { results?: unknown; documents?: unknown; matches?: unknown }).documents || [];
      const docs: LocalLegalDoc[] = (raw as any[]).map((d: any, i: number) => ({
        id: d.id || d.uuid || i + 1,
...
export async function reRankWithPgVector(
  ...
  try {
    ...
    const data = await res.json();
    const scores: unknown[] = (data as { results?: unknown; documents?: unknown; matches?: unknown }).results || (data as { results?: unknown; documents?: unknown; matches?: unknown }).matches || [];
