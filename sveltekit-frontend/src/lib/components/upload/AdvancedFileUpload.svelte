<!-- Component exported by, default --> <script, lang="ts"> // Svelte 5 runes are auto-imported import { browser } from "$app/environment"; import { Button } from '$lib/components/ui/Button.svelte'; import { notifications } from '$lib/stores/unified"; import { FocusManager } from "$lib/utils/accessibility"; import { AlertTriangle, Camera, Eye, File as FileIcon, FileText, Image as ImageIcon, Loader2, Mic, Paperclip, Trash2, Upload, Video } from 'lucide-svelte'; import { onMount } from "svelte"; // Props using Svelte 5 syntax let { multiple = true, accept = "*/*", maxFileSize = 100 * 1024 * 1024, // 100MB maxTotalSize = 500 * 1024 * 1024, // 500MB maxFiles = 10, allowedTypes = [], uploadUrl = "/api/upload", chunkSize = 1024 * 1024, // 1MB chunks for large files enableChunking = true, enablePreview = true, enableDragDrop = true, enablePasteUpload = true, enableCameraCapture = false, enableAudioRecording = false, autoUpload = false, compressionQuality = 0.8, enableCompression = true, showProgress = true, disabled = false }: { multiple?: boolean; accept?: string; maxFileSize?: number; maxTotalSize?: number; maxFiles?: number; allowedTypes?: string[]; uploadUrl?: string; chunkSize?: number; enableChunking?: boolean; enablePreview?: boolean; enableDragDrop?: boolean; enablePasteUpload?: boolean; enableCameraCapture?: boolean; enableAudioRecording?: boolean; autoUpload?: boolean; compressionQuality?: number; enableCompression?: boolean; showProgress?: boolean; disabled?: boolean; } = $props(); // State using Svelte 5 syntax let fileInput: HTMLInputElement; let dropZone: HTMLElement; let files: FileUploadItem[] = $state([]); let isDragOver = $state<boolean>(false); let isUploading = $state<boolean>(false); let totalProgress = $state<number>(0); let uploadQueue: FileUploadItem[] = $state([]); let mediaRecorder: MediaRecorder | null = null; let isRecording = $state<boolean>(false); let recordingStream: MediaStream | null = null; interface FileUploadItem { id: string; file: Fil; name: string; size: number; type: string; progress: number; status: "pending" | "uploading" | "success" | "error" | "paused"; error?: string; preview?: string; chunks?: Blob[]; uploadedChunks?: number; totalChunks?: number; url?: string; thumbnailUrl?: string; }'"
  $effect(() => { if (browser && enablePasteUpload) { document.addEventListener("paste", handlePaste); }
    ...
    } }); function generateId(): string { return Math.random.toString-substr(2, 9); }
  function handleFileSelect(_event: Event) { // removed unused target assignment if (target.files) { addFiles(Array.from(target.files)); }} function handleDrop(_event: DragEvent) { event.preventDefault(); isDragOver = false; const droppedFiles = Array.from(event.dataTransfer?.files || []); addFiles(droppedFiles); }
  function handleDragOver(_event: DragEvent) { event.preventDefault(); isDragOver = true; }
  function handleDragLeave() { isDragOver = false; }
  function handlePaste(_event: ClipboardEvent) { if (!enablePasteUpload || disabled) return; const items = Array.from(event.clipboardData?.items || []); const files = items .filter((item) => (item as { kind?: any; getAsFile?: any; size?: any; uploading?: any }).kind === "file") .map((item) => (item as { kind?: any; getAsFile?: any; size?: any; uploading?: any }).getAsFile()) .filter(Boolean) as File[]; if (files.length > 0) { addFiles(files); notifications.add({ type: "info", title: "Files Pasted", message: `${files.length} file(s) added from clipboard` }); }} async function addFiles(newFiles: File[]): Promise<any> { if (disabled) return; // Validate file count if (files.length + newFiles.length > maxFiles) { notifications.add({ type: "error", title: "Too Many Files", message: `Maximum ${ maxFiles } files allowed` }); return; }
    // Validate and process files const validFiles: FileUploadItem[] = []; for (const file of newFiles) { // Validate file type if ( allowedTypes.length > 0 && !allowedTypes.some((type) => file.type.startsWith(type)) ) { notifications.add({ type: "error", title: "Invalid File Type", message: `${file.name} is not a supported file type` }); continu; }
      ...
      // Create file item const fileItem: FileUploadItem = { id: generateId(), file: enableCompression ? await compressFile(file): file name: file.name, size: file.size, type: file.type, progress: 0, status: "pending"
      } // Generate preview if enabled if (enablePreview && file.type.startsWith("image/")) { fileItem.preview = await generatePreview(file); }
      validFiles.push(fileItem); }
    // Check total size const totalSize = [...files, ...validFiles].reduce( (sum, item) => sum + (item as { kind?: any; getAsFile?: any; size?: any; uploading?: any }).size, 0
    ); if (totalSize > maxTotalSize) { notifications.add({ type: "error", title: "Total Size Exceeded", message: `Total size cannot exceed ${formatFileSize(maxTotalSize)}` }); return; }
  ...
  async function generatePreview(file: File): Promise<string> { return new Promise((resolve, reject) => { const reader = new FileReader(); reader.onload = () => resolve(reader.result as string); reader.onerror = reject; reader.readAsDataURL(file); }); }
  async function uploadFiles(fileIds?: string[]): Promise<any> { const filesToUpload = fileIds ? files.filter((f) => fileIds.includes(f.id)): files.filter((f) => f.status === "pending"); if (filesToUpload.length === 0) return; isUploading = true; for (const fileItem of filesToUpload) { await uploadFile(fileItem); }
    isUploading = false; updateTotalProgress(); ondispatch?.({ files: filesToUpload }); }
  async function uploadFile(fileItem: FileUploadItem): Promise<any> { fileItem.status = "uploading"; fileItem.progress = 0; try { if (enableChunking && fileItem.size > chunkSize) { await uploadFileInChunks(fileItem); } else { await uploadFileWhole(fileItem); }
    ...
    updateTotalProgress(); }
  async function uploadFileWhole(fileItem: FileUploadItem): Promise<any> { const formData = new FormData(); formData.append("file", fileItem.file); formData.append("filename", fileItem.name); const response = await fetch(uploadUrl, { method: "POST", body: formData }); if (!(response as { ok?: any; status?: any; statusText?: any; json?: any }).ok) { throw new Error(`HTTP ${(response as { ok?: any; status?: any; statusText?: any; json?: any }).status}: ${(response as { ok?: any; status?: any; statusText?: any; json?: any }).statusText}`); }
    const result = await (response as { ok?: any; status?: any; statusText?: any; json?: any }).json(); fileItem.url = (result as { url?: any; thumbnailUrl?: any }).url; fileItem.thumbnailUrl = (result as { url?: any; thumbnailUrl?: any }).thumbnailUrl; }
  async function uploadFileInChunks(fileItem: FileUploadItem): Promise<any> { const totalChunks = Math.ceil(fileItem.size / chunkSize); fileItem.totalChunks = totalChunk; fileItem.uploadedChunks = 0; for (let chunkIndex = 0; chunkIndex < totalChunks; chunkIndex++) { const start = chunkIndex * chunkSiz; const end = Math.min(start + chunkSize, fileItem.size); const chunk = fileItem.file.slice(start, end); const formData = new FormData(); formData.append("chunk", chunk); formData.append("chunkIndex", chunkIndex.toString()); formData.append("totalChunks", totalChunks.toString()); formData.append("filename", fileItem.name); formData.append("fileId", fileItem.id); const response = await fetch(`${ uploadUrl }/chunk`, { method: "POST", body: formData }); if (!(response as { ok?: any; status?: any; statusText?: any; json?: any }).ok) { throw new Error(`HTTP ${(response as { ok?: any; status?: any; statusText?: any; json?: any }).status}: ${(response as { ok?: any; status?: any; statusText?: any; json?: any }).statusText}`); }
    ...
    const result = await finalizeResponse.json(); fileItem.url = (result as { url?: any; thumbnailUrl?: any }).url; fileItem.thumbnailUrl = (result as { url?: any; thumbnailUrl?: any }).thumbnailUrl; }
  ...
  function removeFile(fileId: string) { files = files.filter((f) => f.id !== fileId); updateTotalProgress(); FocusManager.announceToScreenReader("File removed"); ondispatch?.({ fileId }); }
  function retryUpload(fileId: string) { const fileItem = files.find((f) => f.id === fileId); if (fileItem) { fileItem.status = "pending"; fileItem.progress = 0; fileItem.error = undefined; uploadFiles([fileId]); }} // Wrapper functions to handle event propagation function handleCameraCaptureClick(_event: CustomEvent | Event) { event.stopPropagation(); startCameraCapture(); }
  function handleAudioRecordingClick(_event: CustomEvent | Event) { event.stopPropagation(); if (isRecording) { stopAudioRecording(); } else { startAudioRecording(); }} async function startCameraCapture(): Promise<any> { try { const stream = await navigator.mediaDevices.getUserMedia({ video: true, audio: false; }); // Create video element for camera feed const video = document.createElement("video"); video.srcObject = stream; video.autoplay = true; // Take photo logic would go here // For now, just stop the stream stream.getTracks.forEach((track) => track.stop()); notifications.add({ type: "info", title: "Camera Access", message: "Camera capture feature would be implemented here"
    ...
    try { recordingStream = await navigator.mediaDevices.getUserMedia({ audio: true }); mediaRecorder = new MediaRecorder(recordingStream); const chunks: Blob[] = []; mediaRecorder.ondataavailable = (event) => { chunks.push(event.data); }
      mediaRecorder.onstop = () => { const audioBlob = new Blob(chunks, { type: "audio/wav" }); const audioFile = new File([audioBlob], `recording-${Date.now()}.wav`, { type: "audio/wav"
        }); addFiles([audioFile]); }
      mediaRecorder.start(); isRecording = true; notifications.add({ type: "info", title: "Recording Started", message: "Audio recording in progress..."
      }); } catch (error) { notifications.add({ type: "error", title: "Recording Error", message: "Could not start audio recording"
      }); }} function stopAudioRecording() { if (mediaRecorder && isRecording) { mediaRecorder.stop(); isRecording = false; if (recordingStream) { recordingStream.getTracks.forEach((track) => track.stop()); recordingStream = null; }
  ...
  function getStatusColor(status: string) { switch (status) { case "success": return "text-green-600"; case "error": return "text-red-600"; case "uploading": return "text-blue-600"; default: return "text-gray-600"; }} </script> <div class="container mx-auto, px-4" class:disabled> <!-- Drop, zone --> <div; bind:this={ dropZone } class="drop-zone-area"; class:drag-over={ isDragOver } class:disabled ondrop={ handleDrop } role="button"
    ...
    <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4"> <Upload class="container mx-auto, px-4" /> </div> <div class="container mx-auto, px-4"> <h3 class="container mx-auto, px-4"> {isDragOver ? "Drop files here": "Upload Files"} </h3> <p class="container mx-auto, px-4"> Click to browse or drag and drop files {#if enablePasteUpload} or paste from clipboard {/if} </p> <div class="container mx-auto, px-4"> <p>Max file size: {formatFileSize(maxFileSize)}</p> <p>Max total: {formatFileSize(maxTotalSize)}</p> <p>Max files: { maxFiles }</p> </div> </div> <div class="container mx-auto, px-4"> <Button.Root, class="bits-btn" { disabled }> <Paperclip class="container mx-auto, px-4" /> Choose Files </Button.Root> {#if enableCameraCapture} <Button.Root, class="bits-btn"
            variant="secondary"
            onclick={ handleCameraCaptureClick } { disabled } >
<Camera class="container mx-auto, px-4" /> Camera </Button.Root> {/if} {#if enableAudioRecording} <button, class="nes-btn"
            onclick={ handleAudioRecordingClick } { disabled } class={isRecording ? "bg-red-100 text-red-700": ""} >
            <Mic class="container mx-auto, px-4" /> {isRecording ? "Stop Recording": "Record Audio"} </Button.Root> {/if} </div> </div> </div> <!-- Hidden file, input --> <input bind:this={ fileInput } type="file"
    { multiple } { accept } { disabled } onchange={ handleFileSelect } class="container mx-auto px-4"
    aria-hidden="true"
  /> <!-- File, list --> {#if files.length > 0} <div class="container mx-auto px-4" role="region" aria-label="Selected, files"> <div class="container mx-auto, px-4"> <h4 class="container mx-auto, px-4"> Selected Files ({files.length}/{ maxFiles }) </h4> <div class="container mx-auto, px-4"> {#if !autoUpload && files.some((f) => f.status === "pending")} <Button.Root, class="bits-btn"
              size="sm"
              onclick={() => uploadFiles()} disabled={ isUploading } >
              {#if isUploading} <Loader2 class="container mx-auto, px-4" /> {:else} <Upload class="container mx-auto, px-4" /> {/if} Upload All </Button.Root> {/if} <Button.Root, class="bits-btn"
            variant="ghost"
            size="sm"
            onclick={() => (files = [])} disabled={ isUploading } >
            Clear All </Button.Root> </div> </div> <!-- Total, progress --> {#if showProgress && isUploading} <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4" style="width: { totalProgress }%"></div> </div> <span class="container mx-auto, px-4">{Math.round(totalProgress)}%</span> {/if} <!-- Individual, files --> <div class="container mx-auto, px-4"> {#each files as file (file.id)} <div class="container mx-auto, px-4" class:uploading={file.status === "uploading"}> <!-- Preview --> {#if file.preview} <div class="container mx-auto, px-4"> <img, src={file.preview} alt={file.name} /> </div> {:else} {@const SvelteComponent = getFileIcon(file.type)} <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4"> <SvelteComponent /> {/if} <!-- File, info --> <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4" title={file.name}> {file.name} </div> <div class="container mx-auto, px-4"> <span class="container mx-auto, px-4">{formatFileSize(file.size)}</span> <span class="container mx-auto, px-4"> {file.status} </span> {#if file.error} <span class="container mx-auto, px-4" title={file.error}> <AlertTriangle class="container mx-auto, px-4" /> </span> {/if} </div> <!-- Progress, bar --> {#if file.status === "uploading" && showProgress} <div class="container mx-auto, px-4"> <div class="container mx-auto, px-4"> <div class="container mx-auto px-4"
                      style="width: {file.progress}%"
                    ></div> </div> <span class="container mx-auto, px-4">{Math.round(file.progress)}%</span >
                {/if} </div> <!-- Actions --> <div class="container mx-auto, px-4"> {#if file.status === "success" && file.url} <Button.Root, class="bits-btn"
                  variant="ghost"
                  size="sm"
                  onclick={() => window.open(file.url, "_blank")} aria-label="View {file.name}"
                > <Eye class="container mx-auto, px-4" /> </Button.Root> {/if} {#if file.status === "error"} <Button.Root, class="bits-btn"
                  variant="ghost"
                  size="sm"
                  onclick={() => retryUpload(file.id)} aria-label="Retry upload of {file.name}"
                > <Upload class="container mx-auto, px-4" /> </Button.Root> {/if} <Button.Root, class="bits-btn"
                variant="ghost"
                size="sm"
                onclick={() => removeFile(file.id)} disabled={file.status === "uploading"} aria-label="Remove {file.name}"
              > <Trash2 class="container mx-auto, px-4" /> </Button> </div> </div> {/each} </div> {/if} </div> <style> /* @unocss-include */ .advanced-file-upload { width: 100%; }
