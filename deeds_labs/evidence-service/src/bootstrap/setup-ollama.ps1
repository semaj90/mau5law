...
try {
    # Capture tags directly into $tags (avoid any unused 'response' variable)
    $tags = Invoke-RestMethod -Uri "http://localhost:11434/api/tags" -Method Get -ErrorAction Stop

    $count = 0
    if ($null -ne $tags) {
        if ($tags -is [System.Collections.ICollection]) {
            $count = $tags.Count
        } else {
            $count = 1
        }
    }
    Write-Host "✅ Ollama is running (tags found: $count)" -ForegroundColor Green
} catch {
    Write-Host "❌ Ollama is not running on port 11434" -ForegroundColor Red
    Write-Host "   Start Ollama first with: ollama serve --port 11434" -ForegroundColor Yellow
    exit 1
}
...
Write-Host "📥 Pulling gemma3-legal:latest..." -ForegroundColor Cyan
$pullBody = @{ name = "gemma3-legal:latest" } | ConvertTo-Json
Invoke-RestMethod -Uri "http://localhost:11434/api/pull" -Method Post -Body $pullBody -ContentType "application/json"
...
try {
    $result = Invoke-RestMethod -Uri "http://localhost:11434/api/embeddings" -Method Post -Body $embeddingTest -ContentType "application/json"
...
Write-Host "🧪 Testing chat generation with gemma3-legal:latest..." -ForegroundColor Cyan
$chatTest = @{
...
try {
    $result = Invoke-RestMethod -Uri "http://localhost:11434/api/generate" -Method Post -Body $chatTest -ContentType "application/json"
