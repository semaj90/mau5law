...
export async function analyzeRelationships(
  ...
  const relationships = [
    ...
    },
    {
      ...
      confidence: 0.87,
      reasoning:
        'Video timestamp aligns with phone records showing communication with victim',
    },
    {
      source: 'ev-003',
      target: 'ev-001',
...
export function parseFunctionCalls(
  ...
  const functionCallRegex =
    /FUNCTION_CALL:\s*(\w+)\s*\((.*?)\)(?=\s|$|FUNCTION_CALL)/gs;
  let match;
  ...
  if (typeof result === 'object' && result !== null) {
    ...
    if (result.found !== undefined) {
      ...
      result.relationships.forEach((r: any) => {
        lines.push(
          `  • ${r.source} --[${r.type}]--> ${r.target} (${r.confidence})`
        );
      });
