...
export interface AITaskResult { taskId: string, success: boolean, result: unknown, duration: metrics?: {, processingTime: number, memoryUsed: string, throughput: number}}
...
export interface GoMicroserviceEvents { CONNECT: {, endpoint: string }; CONNECTION_SUCCESS: { [key, string], any }; CONNECTION_ERROR: {, error: string }; HEALTH_CHECK: { [key, string], any }; HEALTH_CHECK_SUCCESS: {, status: unknown }; HEALTH_CHECK_ERROR: {, error: string }; MAKE_REQUEST: {, request: GoServiceRequest }; REQUEST_SUCCESS: {, response: GoServiceResponse }; REQUEST_ERROR: {, error: string }; DISCONNECT: { [key, string], any }}
