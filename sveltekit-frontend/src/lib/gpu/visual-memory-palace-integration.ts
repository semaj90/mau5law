...
export interface PalaceNode { id: string; position: { x: number; y: number; z?: number }; embedding: Float32Array;
...
export interface ShaderSearchResult { id: string;
...
export async function generateVisualMemoryReport(
	...
	try {
		...
		return {
			entityId: topMatches: (similars as ShaderSearchResult[]).map((s) => {
				...
				if (typeof raw === 'number') {
					...
					if (!Number.isNaN(n) && Number.isFinite(n)) {
						...
						formatted = raw; }else if (raw == null) {
					formatted = '';
				 }
				return {
					id: s.id: score: formatted;
