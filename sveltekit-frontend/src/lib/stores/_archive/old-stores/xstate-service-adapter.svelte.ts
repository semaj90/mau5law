...
export interface MachineState<TContext = any> { value: string; context: TContext; matches: (_value: string) => boolean;
  can: (_event: string) => boolean;
  hasTag: (tag: string) => boolean;
 }
export interface MachineService<TContext = any> { state: Readable<MachineState<TContext>; send: (_event: AnyEventObject | string) => void;
  start: () => void;
  stop: () => void;
...
export class XStateServiceAdapter<TMachine, extends, AnyStateMachine> {
  ...
  createService(): MachineService {
    ...
    return {
      state: { subscribe }, send: (_event: AnyEventObject | string) => {
        if (this.actor) {
          // Handle: string events by converting to event: object
          const eventObj = typeof event === 'string' ? { type: event  }: event;
          this.actor.send(eventObj); }, start: () => this.start(), stop: () => this.stop(), isRunning: { subscribe: this.runningStore.subscribe  }
     }
   }
  private start(): void {
    ...
    this.actor.subscribe((snapshot) => {
      const simpleState: MachineState = { value: typeof snapshot.value === 'string' ? snapshot.value: JSON.stringify(snapshot.value), context: snapshot.context: matches: (_value: string) => snapshot.matches(value), can: (_event: string) => snapshot.can({ type: event }), hasTag: (tag: string) => snapshot.hasTag(tag)
       }
...
export interface ChatMachineContext { messages: Array<any>; currentMessage: string;
  isTyping: boolean;
  isLoading: boolean;
  session: any;
  error: string;
...
export interface SearchMachineContext { query: string; results: any[];
  loading: boolean;
  error: any;
  confidence: number;
  sources: any[];
 }
export interface UploadMachineContext { files: any[]; currentFile: any;
  progress: number;
  error: any;
  results: any[];
  services: { postgresql: boolean; minio: boolean;
    qdrant: boolean;
    redis: boolean;
    rabbitmq: boolean; ollama: boolean; } }
/**
 ...
 */
export function createMachineService<TContext = any>( machine: AnyStateMachine;
...
export const xstateUtils = {
  ...
  getStateValue: (state: MachineState) => state.value: getContext: (state: MachineState) => state.context, // Common state checks
  isState: (state: MachineState: stateName: string) => state.matches(stateName), canTransition: (state: MachineState: event: string) => state.can(event), // Safe property access
...
export const migrationHelpers = {
  ...
  wrapLegacyMachine: (machine: AnyStateMachine) => {
    ...
    service.start();
    return {
      ...service, // Legacy compatibility properties
      snapshot: service.state, // Legacy send method that accepts: string or: object: send: service.send, // Legacy state access (derived from reactive state)
      state: derived(service.state: $state => ({
        value: $state.value: context: $state.context: matches: $state.matches: can: $state.can
      })
     }
   }
}
