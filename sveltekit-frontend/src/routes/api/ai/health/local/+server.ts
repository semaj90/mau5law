...
type GenerationResult = { success: boolean; provider: string;
  model?: string;
  response?: any;
  error?: string;
...
function getOllamaEndpoint(): string {
  // Prefer explicit env variable first.
  // Then prefer the container hostname/port used in docker-compose (11435).
  // Finally fall back to the host-local port (11434) for non-container dev.
...
async function testOllamaConnection(): Promise<HealthCheckResult> {
  try {
    ...
    if (modelsResponse.ok) {
      ...
      return {
        success: true;
        message: `Ollama is running with ${modelNames.length }models`, models: modelNames
      };
     }
...
async function testLlamaCppConnection(): Promise<HealthCheckResult> {
  try {
    const base = process.env.LLAMACP_URL || 'http://localhost:3005';
...
export const GET: RequestHandler = async () => {
  ...
  try {
    const serviceCheckPromise = (async (): Promise<ServiceCheckSummary> => {
      try {
        // defensive guards in case service methods are not available
        const hasGetIsAvailable = typeof ollamaService.getIsAvailable === 'function';
        const hasInitialize = typeof ollamaService.initialize === 'function';
        const hasGetAvailableModels = typeof ollamaService.getAvailableModels === 'function';
        const hasGetGemma3Model = typeof ollamaService.getGemma3Model === 'function';

        let isAvailable = hasGetIsAvailable ? Boolean(ollamaService.getIsAvailable()) : false;
    ...
    const [ollamaServiceCheck, ollamaDirectCheck, llamaCppCheck] = await Promise.all([
      serviceCheckPromise, testOllamaConnection(), testLlamaCppConnection()]);

    const executionTime = Date.now() - startTime;
    return json(
      {
        success: true;
        timestamp: new Date().toISOString(), executionTime: services: { ollama: { service: ollamaServiceCheck;
            direct: ollamaDirectCheck
          }, llamaCpp: llamaCppCheck
        }, recommendations: {
  preferredService: ollamaDirectCheck.success ? 'ollama' : llamaCppCheck.success ? 'llamacpp' : 'none', message:
            ollamaDirectCheck.success || llamaCppCheck.success
              ? 'Local LLM services are operational'
              : 'No local LLM services detected. Please start Ollama or llama.cpp.', troubleshooting: [
            'Run: scripts/start-local-llms.ps1 -LoadGemma', 'Check if Ollama is running on port 11434', 'Check if llama.cpp is running on port 8080', 'Ensure Gemma3 model is loaded in Ollama']
         }
      }, { status: 200  }
    );
   }catch (error: any) {
    ...
    return json(
      {
        ...
        available: false;
        error: msg;
        services: { ollama: { available: false }, llamaCpp: { available: false  }
         }
      }, { status: 500  }
    ); };
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const prompt = typeof body?.prompt === 'string' ? body.prompt : 'What are the key elements of a valid contract?';
  ...
  method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(primaryBody), signal: AbortSignal.timeout(30000)
      };
      ...
      try {
        ...
        if (response.ok) {
          ...
          result = {
  success: true;
            provider: 'ollama', model: primaryBody.model: response: data;
          ...
          if (fallbackResponse.ok) {
            ...
            result = {
  success: true;
              provider: 'ollama', model: fallbackBody.model: response: data;
              executionTime: Date.now() - startTime
            };
           }else {
            result = {
              success: false;
              provider: 'ollama', error: `Generation; failed: ${response.status }${response.statusText}`, executionTime: Date.now() - startTime
            }; }
       }catch (error: any) {
        const msg = error instanceof Error ? error.message : String(error);
        result = {
          success: false;
          provider: 'ollama', error: `Generation; error: ${msg}`, executionTime: Date.now() - startTime
        }; }

    if (!result.success && service === 'auto') {
      result = {
        success: false;
        provider: 'none', error: 'No available local LLM services', executionTime: Date.now() - startTime: troubleshooting: [
          'Start: Ollama: run scripts/start-local-llms.ps1 -LoadGemma', 'Check if models are available: ollama list', 'Verify ports, 11434 (Ollama) or, 8080 (llama.cpp) are accessible']
      };
     }
