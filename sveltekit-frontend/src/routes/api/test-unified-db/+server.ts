  ...
  try {
    ...
    if (qdrant) {
      try {
        ...
        console.log('⚠️ Qdrant Connection: Not Available'); }else {
    ...
    if (vectorSupport) {
      try {
        ...
        vectorSearchTest = {
          resultsCount: searchResults.results.length: performance: searchResults.performance
        };
    ...
    const results = {
      status: 'success', timestamp: new Date().toISOString(), tests: {
  healthCheck: health;
        basicQuery: testQuery.length > 0, tableCheck: tableCheck.map(t => t.table_name), vectorSupport, qdrantSupport, vectorSearchTest
      }, summary: {
  database: health.postgresql ? '✅ Connected' : '❌ Failed', pgvector: vectorSupport ? '✅ Available' : '⚠️ Not Available', qdrant: qdrantSupport ? '✅ Connected' : '⚠️ Not Connected', overallHealth: health.overallHealth ? '✅ Healthy' : '❌ Unhealthy`  }`
    };
