  ...
  return new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains(LOCAL_STORE)) {
        ...
        try { store.createIndex('title', 'title', { unique: false }); } catch (_) {}
      }
    };
...
function mergeResults(a: SearchResult[], b: SearchResult[]): SearchResult[] {
  ...
  const push = (item?: SearchResult) => {
    ...
    if (!existing) map.set(item.id, { ...item });
      ...
      map.set(item.id, { ...existing, ...item, relevance: mergedRelevance, metadata: { ...existing.metadata, ...item.metadata } });
    }
  };
  a.forEach(push);
  b.forEach(push);
...
async function performSemanticSearch(searchTerm: string = 'contract liability'): Promise<any> {
  ...
  const { promise, abort } = withAbort(async (signal?: unknown) => {
    ...
    const resp = await fetch(`/api/yorha/legal-data?search=${encodeURIComponent(searchTerm)}&limit=25`, { signal: s });
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    return resp.json();
  });
  try {
...
async function checkClusterHealth(): Promise<any> {
  ...
  try {
    ...
    systemData = {
      ...systemData,
      cpu_usage: typeof healthData.cpu === 'number' ? healthData.cpu : systemData.cpu_usage,
      memory_usage: typeof healthData.memory === 'number' ? healthData.memory : systemData.memory_usage,
      gpu_utilization: typeof healthData.gpu === 'number' ? healthData.gpu : systemData.gpu_utilization,
      network_latency: typeof healthData.latency === 'number' ? healthData.latency : systemData.network_latency,
      active_processes: typeof healthData.processes === 'number' ? healthData.processes : systemData.active_processes
    };
...
$effect(() => {
  ...
  (async () => {
    ...
    try {
      ...
      let client: { loadLayout?: (path: string) => Promise<any>; startDataStreams?: () => void; } | null = null;

      if (clientModule && typeof (clientModule as any).default === 'object' && (clientModule as any).default !== null) {
        // Case 1: Module exports a default object containing the functions
        client = (clientModule as any).default;
      } else if (clientModule && (typeof (clientModule as any).loadLayout === 'function' || typeof (clientModule as any).startDataStreams === 'function')) {
...
<main>
  <!-- pass current system metrics into the command center (cast to any to avoid prop typing mismatch) -->
  <YoRHaCommandCenter systemData={systemData as unknown as any} />

  {#if showCommandInterface}
    <YoRHaCommandInterface legalSession={legalSession as unknown as any} />
  {/if}
</main>
  {/if}
</main>
