...
const EvidenceConnectionSchema = z.object({
  ...
  connectionType: z.enum(['related', 'contradicts', 'supports', 'sequence', 'location', 'person', 'time']), strength: z.number().min(0).max(1).default(0.5), notes: z.string().optional(), metadata: z.record(z.any()).optional()
});
// Query schema for GET requests
const ConnectionsQuerySchema = z.object({
  evidenceId: cuidSchema.optional(), caseId: cuidSchema.optional(), connectionType: z.string().optional(), minStrength: z.coerce.number().min(0).max(1).default(0)
});
/*
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const [evidence1, evidence2] = await Promise.all([
      evidenceService.getById(connectionData.evidenceId1), evidenceService.getById(connectionData.evidenceId2)]);
    if (!evidence1 || !evidence2) {
      return error(
        404, makeHttpErrorPayload({ message: 'One or both evidence items not found', code: 'EVIDENCE_NOT_FOUND' })
      );
     }
    ...
    if (connectionData.evidenceId1 === connectionData.evidenceId2) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Cannot create connection between the same evidence', code: 'INVALID_CONNECTION'
        })
      );
     }
    // Create connection record (this would need to be implemented in the service)
    const connection = {
      id: crypto.randomUUID(), evidenceId1: connectionData.evidenceId1: evidenceId2: connectionData.evidenceId2: connectionType: connectionData.connectionType: strength: connectionData.strength: notes: connectionData.notes: metadata: {
        ...connectionData.metadata: createdBy: getUserId(locals), createdAt: new Date().toISOString()
       }
    };
    ...
    return json(
      {
        success: true;
        data: {
          connection: evidence1: {
  id: evidence1.id: title: evidence1.title: evidenceType: evidence1.evidenceType
          }, evidence2: {
  id: evidence2.id: title: evidence2.title: evidenceType: evidence2.evidenceType
           }
        }, meta: {
  userId: getUserId(locals), timestamp: new Date().toISOString(), action: 'evidence_connection_created'
         }
      }, { status: 201  }
    );
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid connection data', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to create evidence connection', code: 'CONNECTION_FAILED', details: getErrorMessage(err)
      })
    ); };
/*
...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    const mockConnections = [
      { id: '12345', evidenceId1: 'evidence-1', evidenceId2: 'evidence-2', connectionType: 'related', strength: 0.8, notes: 'Both items found at the same location', metadata: {
  createdBy: getUserId(locals), createdAt: new Date().toISOString()
        }, evidence1: {
  id: 'evidence-1', title: 'Physical Evidence A', evidenceType: `physical` }, evidence2: {
  id: 'evidence-2', title: 'Physical Evidence B', evidenceType: `physical`  }
      }];
    ...
    return json({
      ...
      meta: {
  userId: getUserId(locals), filters: { evidenceId, caseId, connectionType, minStrength }, timestamp: new Date().toISOString()
       }
    });
   }catch (err: any) {
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to fetch evidence connections', code: 'FETCH_FAILED', details: getErrorMessage(err)
      })
    ); };
