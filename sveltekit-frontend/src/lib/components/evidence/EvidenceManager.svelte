...
<div class="evidence-manager"> <!-- Embedding: Stats, Card --> <div class="mb-6"> <div class="yorha-panel-header"> <h3 class="nes-text">ðŸ“Š Embedding Status</h3> </div>
 ...
 <style>
  ...
  /* Retro / N64 inspired error box */ .error-box { display: flex;
	gap: 0.75rem;
		padding: 1rem 1.1rem 1.05rem; border: 1px solid rgba(248, 113, 113, 0.55); border-radius: 12px;
  ...
  .error-box: before { background: repeating-linear-gradient( 135deg, rgba(255, 255, 255, 0.15) 0 6px, transparent 6px 12px ); mix-blend-mode: overlay;
	opacity: 0.25;}
  .error-box: after { background: linear-gradient(90deg, transparent, rgba(255, 255, 255, 0.55), transparent), width: 60px;transform: translateX(-120%) skewX(-12deg);
  ...
  /* NES / N64 inspired dismiss button */ :global(.dismiss-btn) { --nes-border: #e11d48;
	...
	padding: .6rem .9rem .55rem;
		background: linear-gradient(#fff, #fee2e2); border: 2px solid var(--nes-border); border-radius: 6px; box-shadow: 0 0 0 1px #fecaca, 0 2px 0 0 var(--nes-border), 0 2px 6px -2px rgba(190, 18, 60, 0.55), inset 0 0 0 1px #fff; text-shadow: 0 0 4px rgba(254, 226, 226, 0.75), transition:transform 0.18s, box-shadow 0.25s, background 0.25s; will-change: transform;
	cursor: pointer;}
  ...
  .retro-scan: before { content: '';
	...
	opacity: .35;
		animation: scanMove 9s linear infinite; pointer-events: none;}
  ...
  @media (prefers-reduced-motion: reduce) { .error-box: after, .error-icon, .retro-scan: before, .flicker:global(.dismiss-btn), :global(.dismiss-btn:hover) { animation: none !important;
