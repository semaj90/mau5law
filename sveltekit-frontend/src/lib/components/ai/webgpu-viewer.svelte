...
<script lang="ts">
  ...
  fn main(@builtin(vertex_index) vertexIndex: u32) -> VertexOutput {
    ...
    if (quadVertex == 0u || quadVertex == 3u) {
      offset.x = -0.02; offset.y = -0.02;
    } else if (quadVertex == 1u) {
      offset.x = 0.02; offset.y = -0.02;
    } else if (quadVertex == 2u || quadVertex == 4u) {
      offset.x = 0.02; offset.y = 0.02;
    } else if (quadVertex == 5u) {
      offset.x = -0.02; offset.y = 0.02;
    }
    ...
    output.color = normalize(embedding) * vec3<f32>(0.5, 0.5, 0.5) + vec3<f32>(0.5, 0.5, 0.5);
    output.pointSize = 5.0 * uniforms.zoom;
    return output;
  }
  `;
  // Fragment shader (unchanged but syntactically valid)
  ...
  function updateEmbeddings(newEmbeddings: number[][]) {
    ...
    embedBuffer.unmap();
    // Create bind group
    if (pipeline && uniformBuffer && embedBuffer) {
