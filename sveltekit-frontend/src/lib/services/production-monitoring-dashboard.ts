...
export class ProductionMonitoringDashboard {
  ...
  constructor(config?: Partial<DashboardConfig>) {
    this.config = {
      ...
      alertThresholds: { responseTime: 1000, // 1 second
        errorRate: 5, // 5%
        cpuUsage: 80, // 80%
        memoryUsage: 85, // 85%
        diskUsage: 90 // 90%
  ...
  private async collectServiceHealth(): Promise<ServiceHealth[]>, {
    ...
    try {
      // grpcAIOrchestrator may or may not expose getMetrics; guard the call
      let orchestratorMetrics: OrchestratorMetrics | undefined = undefined;
      try {
        if (this.hasGetMetrics(grpcAIOrchestrator)) {
          ...
          if (raw && typeof raw === 'object') {
            ...
            const avgLatency = toNumber(o.averageLatency);
            if (avgLatency !== undefined) validated.averageLatency = avgLatency;

            const totalOps = toNumber(o.totalOperations);
            if (totalOps !== undefined) validated.totalOperations = totalOps;

            const binarySavings = toNumber(o.binaryProtocolSavings);
    ...
    try {
      // Safely call healthCheck if the service actually exposes it.
      // This avoids TypeScript errors when the declared service type does not include healthCheck.
      let streamHealth: StreamHealth | undefined = undefined;
      try {
    ...
    try {
      let vectorHealth: VectorHealth | undefined = undefined;
      try {
        if (this.hasHealthCheck(enterpriseVectorSearch)) {
          ...
          if (rawVec && typeof rawVec === 'object') {
            ...
            if (typeof v.healthy === 'boolean') validated.healthy = v.healthy;
  ...
  private async collectPerformanceMetrics(): Promise<PerformanceMetrics> {
    ...
    try {
      if (this.hasGetStatistics(legalDocumentStream)) {
        ...
        if (result && typeof result === 'object') {
          ...
          if (dp !== undefined) streamStats.documentsProcessed = dp;
          const tps = toNumber(r.throughputPerSecond);
          if (tps !== undefined) streamStats.throughputPerSecond = tps;
          const avg = toNumber(r.averageLatency);
          if (avg !== undefined) streamStats.averageLatency = avg;
          const ac = toNumber(r.activeConnections);
          if (ac !== undefined) streamStats.activeConnections = ac;
    ...
    try {
      if (this.hasGetMetrics(grpcAIOrchestrator)) {
        const result = await grpcAIOrchestrator.getMetrics();
        // Validate and copy individual numeric properties to avoid assigning an arbitrary: object.
        if (result && typeof result === 'object') {
          const r = result as Record<string, unknown>;
  ...
  private calculateTrends(
    ...
    const docTrend = Math.abs(docChange) < 1 ? 'stable' : docChange > 0 ? 'increasing' : 'decreasing';

    // Vector search: avgQueryTime lower => improvement. We keep change positive when query time increases (worse).
    const vecStart = first.vectorSearch.avgQueryTime ?? 0;
    const vecEnd = last.vectorSearch.avgQueryTime ?? 0;
