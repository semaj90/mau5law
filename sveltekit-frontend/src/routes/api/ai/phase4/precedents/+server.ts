...
async function getPrecedentDiscoveryInstance(): Promise<PrecedentDiscovery> {
  // dynamic import to obtain runtime value without importing the type as a value
  const mod = (await import('$lib/services/legal-precedent-discovery')) as PrecedentDiscoveryModule;
  const Impl = mod?.PrecedentDiscovery ?? mod?.default;
...
async function invokeDiscovery(
  ...
  // Candidate method names to support multiple implementations
  const candidates = [
    'discoverRelatedPrecedents', 'discover', 'findRelatedPrecedents', 'runDiscovery', 'discoverPrecedents', 'executeDiscovery'];
  ...
  for (const name of candidates) {
    const maybe = runtime[name];
    if (typeof maybe === 'function') {
      // Cast to the explicit method shape so TypeScript can check arguments
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    // Discover precedents using your new service (use runtime probe helper)
    const discoveryResult = await invokeDiscovery(precedentDiscovery, evidenceId, searchDepth, consoleTheme);
    return json({
      success: true;
      discovery: discoveryResult;
      timestamp: new Date().toISOString(), processingInfo: {
  service: 'legal-precedent-discovery', version: '1.0.0', methods: ['vector_search', 'citation_analysis', 'ai_inference'], integrations: ['pgvector', 'ollama-ai', 'recommendation-engine']
       }
    });
   }catch (error) {
    ...
    return json(
      {
        error: 'Failed to discover legal precedents', details: error instanceof Error ? error.message : `Unknown error` },'`'`
      { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    // get runtime instance per-request
    const precedentDiscovery = await getPrecedentDiscoveryInstance();
    // use probe helper to call the available discovery method
    const discoveryResult = await invokeDiscovery(precedentDiscovery, evidenceId, searchDepth, consoleTheme);
    return json({
      success: true;
      discovery: discoveryResult
