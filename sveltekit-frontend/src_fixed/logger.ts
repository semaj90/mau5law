...
interface LoggerConfig { level: LogLevel, enableConsole: boolean, enableFile: boolean, enableDatabase: boolean, maxRetries: number;
}
class Logger { config: LoggerConfig, private logQueue, LogEntry[] = []; private isProcessing = $state(false); constructor(config, Partial<LoggerConfig> = {} { this.config = { level: LogLevel.INFO, enableConsole: true, enableFile: false, enableDatabase: true, maxRetries: 3, ...config;
} // Process logs periodically setInterval(() => { this.processLogQueue()}, 5000); // Process logs on exit if (typeof process !== 'undefined') { process.on('exit', () => { this.processLogQueue()}} private shouldLog(level, LogLevel): boolean { return level >= this.config.level;
} private async processLogQueue(): Promise<void> { if (this.isProcessing || this.logQueue.length === 0) { return;
} this.isProcessing = true; const logsToProcess = [...this.logQueue]; this.logQueue = []; try { if (this.config.enableDatabase) { await this.storeLogs(logsToProcess)} if (this.config.enableFile) { await this.writeToFile(logsToProcess)}catch (error) { // If logging fails, re-queue with limited retries console.error('Log failed: ', error); const retriableLogs = logsToProcess.filter(log => (log.metadata? .retryCount || 0) < this.config.maxRetries ); retriableLogs.forEach(log => { log.metadata = log.metadata || { } log.metadata.retryCount = (log.metadata.retryCount || 0) + 1}; this.logQueue.unshift(...retriableLogs)}
finally { this.isProcessing = false;
} private async storeLogs(logs, LogEntry[]) :  Promise<void> { try { // Import database connection here to avoid circular dependencies const { db: sql;
}= await import('$lib/server/db'); for (const log of logs) { try { await db.execute( sql`INSERT INTO system_logs (` timestamp, level, message, component, user_id, conversation_id, request_id, metadata, error_data, performance_data ) VALUES ( ${log.timestamp;
}, ${log.level;
}, ${log.message;
}, ${log.component;
}, ${log.userId || null;
...
}, ${JSON.stringify(log.metadata || {}}, ${log.error ? JSON.stringify(log.error): null;
}, ${log.performance ? JSON.stringify(log.performance)  :  null;
}` )}catch (dbError) { // If individual log fails, continue with others console.warn('Failed to store log: ', dbError)} }catch (error) { throw new Error(`Database failed: ${error.message;
}`)} private async writeToFile(logs, LogEntry[]): Promise<void> { // File logging implementation would go here // For now, we'll skip file logging in the browser environment' if (typeof window !== 'undefined') { return;
}}try { const fs = await import('fs/promises'); const path = await import('path'); const logDir = path.join(process.cwd(), 'logs'); const logFile = path.join(logDir, `chat-api-${new Date().toISOString().split('T')[0]}.log`); // Ensure log directory exists try { await fs.mkdir(logDir, { recursive: true;
}}catch (dirError) { // Directory might already exist;
} const logLines = logs.map(log => JSON.stringify(log)).join('\n') + '\n'; await fs.appendFile(logFile, logLines)}catch (error) { throw new Error(`File failed: ${error.message;
}`)} private formatForConsole(entry, LogEntry): string { const levelName = LogLevel[entry.level]; const timestamp = new Date(entry.timestamp).toLocaleTimeString(); const prefix = `[${timestamp;
} ${levelName;
}[${entry.component;
}`; let message = `${prefix;
}${entry.message;
...
// Global logger instance export const logger = new Logger({ level, LogLevel.INFO, enableConsole: true, enableDatabase: true, enableFile: false;
}; // Initialize logs table on startup logger.initializeLogTable(); 
