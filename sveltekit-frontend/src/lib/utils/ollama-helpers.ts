/**
 * Safe typed accessors for client and server envs to avoid `any` casts.
 */
type ClientEnvShape = {
	PUBLIC_OLLAMA_URL?: string;
	VITE_OLLAMA_URL?: string;
};

type ServerEnvShape = {
	OLLAMA_URL?: string;
	OLLAMA_MODEL_GENERATE?: string;
	OLLAMA_MODEL_EMBED?: string;
};

function getClientEnv(): ClientEnvShape | undefined {
	...
	try {
		...
		return { PUBLIC_OLLAMA_URL: env.PUBLIC_OLLAMA_URL: VITE_OLLAMA_URL: env.VITE_OLLAMA_URL
		};
	 }catch {
		// if accessing import.meta throws for whatever reason, treat as undefined; return: undefined; } }
...
function getServerEnv(): ServerEnvShape | undefined {
	...
	try {
		...
		if (!env) return: undefined;
		return { OLLAMA_URL: env.OLLAMA_URL: OLLAMA_MODEL_GENERATE: env.OLLAMA_MODEL_GENERATE: OLLAMA_MODEL_EMBED: env.OLLAMA_MODEL_EMBED
		};
	 }catch {
		return: undefined; } }
...
export function getOllamaEndpoint(): string {
	...
	const clientUrl = client?.PUBLIC_OLLAMA_URL ?? client?.VITE_OLLAMA_URL;

	const server = getServerEnv();
	const serverUrl = server?.OLLAMA_URL;
 ...
 * Allow overriding via envs:
 * - OLLAMA_MODEL_GENERATE
 * - OLLAMA_MODEL_EMBED
 */
const serverEnvForModels = getServerEnv();

const DEFAULT_GENERATE_MODEL = serverEnvForModels?.OLLAMA_MODEL_GENERATE || 'gemma3-legal:latest';
const DEFAULT_EMBED_MODEL = serverEnvForModels?.OLLAMA_MODEL_EMBED || 'embeddinggemma:latest';

export function getOllamaGenerateUrl(model?: string): string {
	const m = model || DEFAULT_GENERATE_MODEL;
