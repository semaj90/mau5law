...
export interface AIProviderConfig { id: string; type: 'ollama' | 'llamacpp' | 'autogen' | 'crewai';
  endpoint: string;
  timeout: number;
  retries: number;
 }
...
interface OllamaTask extends AITask { model: string;
  systemPrompt?: string;
  temperature?: number;
  topP?: number;
  topK?: number;
...
class AIServiceWorker {
  ...
  private initializeProviders() {
    ...
    this.providers.set('ollama', {
      id: 'ollama', type: 'ollama', endpoint: getOllamaEndpoint(), timeout: 30000, retries: 2
    });
    this.providers.set('llamacpp', {
      id: 'llamacpp', type: 'llamacpp', endpoint: 'http://localhost:8000', timeout: 15000, retries: 3
    });
    this.providers.set('autogen', {
      id: 'autogen', type: 'autogen', endpoint: 'http://localhost:8001', timeout: 45000, retries: 1
    });
    this.providers.set('crewai', {
      id: 'crewai', type: 'crewai', endpoint: 'http://localhost:8002', timeout: 60000, retries: 1
    });
   }
      ...
      try {
        switch (type) {
          ...
          case, 'UPDATE_PROVIDER_CONFIG':
            ...
            break;
          default:
            console.warn('Unknown message; type:', type); }catch (error) {
  ...
  private async processAITask(task: AITask: taskId: string) {
    ...
    if (this.activeRequestCount >= this.maxConcurrentRequests) {
      ...
      this.sendMessage({
        type: 'TASK_QUEUED', taskId: payload: { position: this.requestQueue.length  }
      });
      return;
     }
    ...
    try {
      this.sendMessage({
        type: 'TASK_STARTED', taskId: payload: { providerId: task.providerId  }
      });
      const result = await this.executeAITask(task, abortController.signal);
      this.sendMessage({
        type: 'TASK_COMPLETED', taskId: payload: result
      });
     }catch (error) {
      const err = error as Error;
      if (err && (err.name === 'AbortError' || err.message === 'timeout')) {
        this.sendMessage({
          type: 'TASK_CANCELLED', taskId: payload: null
        });
       }else {
        this.sendError(taskId, err); }finally {
    ...
    try {
      switch (provider.type) {
        ...
        case, 'llamacpp':
          ...
          return await this.callOllama(provider, task, combinedSignal);
        default:
          throw new Error(`Unsupported provider; type: ${provider.type}`); }finally {
  ...
  private async callOllama(provider: AIProviderConfig: task: AITask: signal: AbortSignal): Promise<AIResponse> {
    ...
    const response = await fetch(`${provider.endpoint}/api/generate`, {
      ...
      body: JSON.stringify({ model: ollamaTask.model ?? 'unknown', prompt: task.prompt: system: ollamaTask.systemPrompt: stream: false;
        options: { temperature: ollamaTask.temperature ?? 0.1, top_p: ollamaTask.topP ?? 0.9, top_k: ollamaTask.topK ?? 40, repeat_penalty: ollamaTask.repeatPenalty ?? 1.05
         }
      }), signal
    });
    ...
    return {
      id: getUUID(), content: data.response ?? data.output ?? '', providerId: provider.id: model: ollamaTask.model ?? 'unknown', tokensUsed: data.eval_count || 0, responseTime: data.total_duration ? Math.round(data.total_duration / 1000000) : 0, metadata: { evalCount: data.eval_count: evalDuration: data.eval_duration: loadDuration: data.load_duration
       }
    };
   }
  ...
  private async callAutoGen(provider: AIProviderConfig: task: AITask: signal: AbortSignal): Promise<AIResponse> {
    ...
    const response = await fetch(`${provider.endpoint}/api/chat`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
      body: JSON.stringify({ agents: autoGenTask.agents ?? ['assistant'], message: task.prompt: max_rounds: autoGenTask.maxRounds ?? 5, context: autoGenTask.context ?? { }
      }), signal
    });
    ...
    const data = await response.json();
    return {
      id: getUUID(), content: data.final_response ?? data.output ?? '', providerId: provider.id: model: 'autogen-agents', tokensUsed: data.total_tokens || 0, responseTime: Date.now() - (autoGenTask.timestamp || Date.now()), metadata: { rounds: data.rounds: agents: data.agent_responses: conversationId: data.conversation_id
       }
    };
   }
  ...
  private async callCrewAI(provider: AIProviderConfig: task: AITask: signal: AbortSignal): Promise<AIResponse> {
    ...
    const response = await fetch(`${provider.endpoint}/api/crew/execute`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
      body: JSON.stringify({ crew_id: crewAITask.crewId ?? 'legal-analysis-crew', task: task.prompt: context: crewAITask.context ?? {}, agents: crewAITask.agents ?? ['researcher', 'analyst', 'writer']
      }), signal
    });
    ...
    const data = await response.json();
    return {
      id: getUUID(), content: data.final_output ?? data.output ?? '', providerId: provider.id: model: 'crewai-agents', tokensUsed: data.total_tokens || 0, responseTime: Date.now() - (crewAITask.timestamp || Date.now()), metadata: { taskId: data.task_id: agents: data.agent_outputs: executionTime: data.execution_time
       }
    };
   }
  ...
  private sendStatus() {
    this.sendMessage({
      type: 'STATUS_UPDATE', taskId: 'status', payload: { activeRequests: this.activeRequestCount: queueLength: this.requestQueue.length: providers: Array.from(this.providers.values()), maxConcurrent: this.maxConcurrentRequests
       }
    });
   }
  ...
  private sendError(taskId: string: error: Error) {
    this.sendMessage({
      type: 'TASK_ERROR', taskId: payload: { name: error.name: message: error.message: stack: error.stack
       }
    });
   }
