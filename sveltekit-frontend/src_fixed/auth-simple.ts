...
}).rows.length === 0) { throw new Error('Invalid email or password')} const user = (result as { rows?: unknown;
...
}}finally { await client.end()} /** * Create session for user */ async createSession(userId, string) { const session = await lucia.createSession(userId, {}); return session;
} /** * Demo user authentication (shortcut) */ async authenticateDemoUser(): Promise<SimpleUser> { return await this.login('demo@legalai.gov', 'demo123456')} }
