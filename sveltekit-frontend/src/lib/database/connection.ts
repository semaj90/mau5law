...
type ParamList = (string | number | boolean | null | undefined | object)[];
 ...
 * Use this wrapper wherever we did: pool.unsafe(query, ...(params as ParameterOrJSON<...>[]))
 */
async function unsafeQuery<T = DBRow>(query: string, params: ParamList = []): Promise<T[]> {
    ...
    // postgres.js unsafe definition: unsafe(query: string, parameters?: ParameterOrJSON[]);
    const raw = await pool.unsafe(query, castParams);
...
export async function vectorSimilaritySearch(
    ...
    try {
        ...
        // Use pgvector's cosine distance operator and cast as ::vector
        const query = `
        ...
        // pass embedding as a literal string
...
export async function hybridSemanticSearch(
    ...
    try {
        // Prepare embedding as a pgvector literal and cast in with ::vector
        ...
        if (caseId) {
            ...
            whereClauses.push(`(
                (si.entity_type = 'case' AND si.entity_id = $${idx}::uuid)
                OR (si.entity_type = 'document' AND EXISTS (
                    SELECT 1 FROM documents d WHERE d.id = si.entity_id AND d.case_id = $${idx}::uuid
                ))
                OR (si.entity_type = 'evidence' AND EXISTS (
        ...
        return {
            ...
            results: result,
            count,
            query: query.slice(0, 50) + "..." // truncated query for logs
        };
    } catch (error: unknown) {
        return {
            success: false,
            ...
            results: [],
            count: 0,
            query // original search string for debugging
        };
    }
}
...
export async function initializeDatabase(): Promise<DBResult> {
    try {
        ...
        console.log('├── Database extensions checked/created');
...
export async function closeDatabaseConnection(): Promise<void> {
    try {
        ...
        console.error('├── Error closing connection:', getErrorMessage(error));
    }
}
