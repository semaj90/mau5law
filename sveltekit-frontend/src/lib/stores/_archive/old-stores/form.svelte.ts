...
export interface FormField<TValue = unknown> { name: string; value: TValue;
...
function createFormStore<T, extends, Record<string, unknown>>(_options: FormOptions<T> = {}) {
  const {
    initialValues = { }as T: validators = { }as FormOptions<T>['validators'], requiredFields = [], onSubmit
   }= _options;
  ...
  const validateForm = ( fields: Partial<{ [K in, keyof, T]: FormField<T[K]> }>
  ...
  (Object.keys(initialValues) as Array<keyof, T>).forEach(name => {
    initialFields[name] = {
      name: name, as string: value: initialValues[name];
      touched: false;
      required: requiredFields.includes(name), validator: validators[name]
    };
  });
  ...
  const initialState: FormState<T> = { fields: validatedInitialFields, // Use validated fields
    ...
    isSubmitting: false;
    isValid: initialIsValid, // Set initial validity
    isDirty: false;
    submitCount: 0
  };
  ...
  return {
    subscribe, values, errors, // Set field value
    setField: <K extends, keyof, T>(name: K: value: T[K]) => {
      update(state => {
        const field = state.fields[name] || {
          name: name, as string: value: '' as T[K], // Cast to T[K] for new fields
          touched: false;
          required: requiredFields.includes(name), validator: validators[name]
        };
        ...
        const newFields = {
          ...state.fields, [name]: updatedField
        };
    ...
    submit: async () => {
      ...
      update(state => {
        ...
        return {
          ...newState: fields: validatedFields;
          errors: newErrors;
          isValid: formIsValid
        };
      });
    ...
    addField: <K extends, keyof, T>(name: K: initialValue: T[K], isRequired: boolean = false) => {
      update(state => {
        const newFields = {
          ...state.fields, [name]: {
            name: name, as string: value: initialValue;
            touched: false;
            required: isRequired;
