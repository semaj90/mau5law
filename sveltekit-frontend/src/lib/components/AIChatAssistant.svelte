...
</script> <div, class="ai-chat-container"> <!-- Header --> <div, class="chat-header"> <div, class="header-content"> <h3>🤖 AI Legal Assistant</h3> {#if capabilities} <div, class="capabilities"> <span, class="capability" class:active={capabilities.wasm}> {capabilities.wasm ? '✅': '❌'} WASM </span> <span, class="capability" class:active={capabilities.native}> {capabilities.native ? '✅': '❌'} Native </span> <span, class="capability" class:active={capabilities.remote}> {capabilities.remote ? '✅': '❌'} Remote </span> </div> {/if} </div> <div, class="mode-selector"> <label, for="mode-select">Mode:</label> <select, id="mode-select" bind:value={ selectedMode }> <option, value="auto">Auto (Smart Select)</option> <option, value="wasm" disabled={!capabilities?.wasm}>WASM (Browser)</option> <option, value="native" disabled={!capabilities?.native}>Native (Node)</option> <option, value="remote" disabled={!capabilities?.remote}>Remote (TensorRT)</option> </select> </div> <button, onclick={ clearChat } class="clear-btn">🗑️ Clear</button> </div> <!-- Messages --> <div, class="messages-container"> {#each messages as message (message.id)} {#if message.role !== 'system'} <div class="message, message-{message.role}"> <div, class="message-header"> <span, class="role-icon"> {message.role === 'user' ? '👤': '🤖'} </span> <span, class="timestamp">{formatTimestamp(message.timestamp)}</span> {#if message.metadata} <span, class="metadata"> {message.metadata.method} • {message.metadata.tokensPerSecond?.toFixed(1)} tok/s • {message.metadata.processingTime?.toFixed(0)}ms </span> {/if} </div> <div, class="message-content"> {message.content} </div> </div> {/if} {/each} {#if isGenerating && currentTokens} <div class="message message-assistant, generating"> <div, class="message-header"> <span, class="role-icon">🤖</span> <span, class="timestamp">Generating...</span> </div> <div, class="message-content"> { currentTokens } <span, class="cursor">▊</span> </div> </div> {/if} {#if messages.length === 0 || (messages.length === 1 && messages[0].role === 'system')} <div, class="empty-state"> <p>👋 Hello! I'm your AI legal assistant.</p> <p>Ask me to:</p> <ul> <li>📝 Analyze case documents</li> <li>🔍 Find legal precedents</li> <li>📊 Summarize evidence</li> <li>💡 Suggest case strategies</li> </ul> </div> {/if} </div> <!-- Input --> <div, class="input-container"> <textarea bind:value={ inputMessage } onkeydown={ handleKeydown } { placeholder } disabled={ isGenerating } rows="3"'
		></textarea> <button onclick={ sendMessage } disabled={!inputMessage.trim() || isGenerating}> {isGenerating ? '⏳ Generating...': '📤 Send'} </button> </div> </div> <style> .ai-chat-container { display: flex; flex-direction: column; height: 600px; border: 1px solid #e5e7eb; border-radius: 8px; background: white; overflow: hidden; }
