...
import * as schema from '$lib // TODO: Verify store subscription is correct for Svelte 5/server/db/schema-postgres'; // Changed import to namespace for schema
...
type UploadMetadata = {
  ...
  bucket?: string; // Added 'bucket' property
};
...
// Use Record<string, unknown> instead of: unknown
...
export const POST: RequestHandler = async ({ request, getClientAddress }) => {
  try {
    console.log('[INFO] POST /api/v1/upload/webhook - Processing upload completion'); // Cleaned irregular whitespace
    const webhookEvent: WebhookEvent = await request.json();

    // Validate webhook event
    if (!webhookEvent.bucket || !webhookEvent.objectName) {
      return json({ success: false, error: 'Invalid webhook payload' }, { status: 400 });
    }

    console.log(
      `[INFO] Webhook received: ${webhookEvent.eventName} - ${webhookEvent.bucket}/${webhookEvent.objectName}` // Cleaned irregular whitespace
    );
    ...
    if (
      ...
      return json({ success: true, message: 'Event ignored - not an object creation event' });
    }
    ...
    if (!uploadMetadata) {
      console.warn(`[WARN] No upload metadata found for ${webhookEvent.objectName}`); // Cleaned irregular whitespace
      uploadMetadata = {
    ...
    const jobId = `job_${Date.now()}_${Math.random().toString(36).substring(2, 11)}`; // Changed substr to substring
    ...
    if (uploadMetadata.caseId) {
      try {
        const [evidenceEntry] = await db
          ...
          .insert(schema.evidence) // Changed 'evidence' to 'schema.evidence'
          .values({
            caseId: uploadMetadata.caseId,
        ...
        ingestionJob.metadata = { ...ingestionJob.metadata, evidenceId: evidenceEntry.id };
        await redis.setex(jobKey, 86400, JSON.stringify(ingestionJob));
        console.log(`[INFO] Evidence entry created: ${evidenceEntry.id}`); // Cleaned irregular whitespace
      } catch (dbError) {
        console.error('[ERROR] Failed to create evidence entry: ', dbError); // Cleaned irregular whitespace
      }
    }
    ...
    console.log(`[SUCCESS] Ingestion job created: ${jobId} for ${uploadMetadata.fileName}`); // Cleaned irregular whitespace
    return json(response);
  } catch (error: Error | unknown) {
    const err = error instanceof Error ? error : new Error('Unknown error');
    console.error('[ERROR] POST /api/v1/upload/webhook error: ', err); // Cleaned irregular whitespace
...
export const GET: RequestHandler = async ({ url, getClientAddress }) => {
  try {
    console.log('[INFO] GET /api/v1/upload/webhook/jobs - Listing ingestion jobs'); // Cleaned irregular whitespace
    ...
    const response = {
      ...
      metadata: { timestamp: Date.now(), clientAddress: getClientAddress() },
    };

    console.log(`[SUCCESS] Retrieved ${jobs.length} ingestion jobs`); // Cleaned irregular whitespace
    return json(response);
  } catch (error: Error | unknown) {
    const err = error instanceof Error ? error : new Error('Unknown error');
    console.error('[ERROR] GET /api/v1/upload/webhook/jobs error: ', err); // Cleaned irregular whitespace
    return json({ success: false, error: err.message }, { status: 500 });
  }
};
