...
vi.mock('$lib/server/law-mapping.js', () => ({
  findStateBySlug: vi.fn((slug: string) =>
    slug === 'california' ? { canonical: 'California' } : null
  ),
}));
...
describe('/api/persons-of-interest (GET/POST)', () => {
  ...
  describe('POST', () => {
    ...
    it('creates POI with all optional fields', async () => {
      const { db } = await import('$lib/server/db/client');
      // Mock select for case validation — route awaits db.select().from().where() directly
      (db.select as any).mockReturnValueOnce({
        from: vi.fn(() => ({
          where: vi.fn(async () => [{ id: VALID_UUID }]),
        })),
      });
      const res = await POST({
        request: mkRequest({
...
describe('/api/persons-of-interest/[id] (GET/PATCH/DELETE)', () => {
  ...
  describe('DELETE', () => {
    ...
    it('deletes POI when found', async () => {
      const { db } = await import('$lib/server/db/client');
      // Mock select to find the POI (existence check)
      (db.select as any).mockReturnValueOnce(buildSelectChain([{ id: VALID_UUID }]));
      const res = await DELETE({
        params: { id: VALID_UUID },
        request: new Request('http://localhost'),
        locals: authedLocals,
...
describe('/api/citations (GET/POST/DELETE)', () => {
  ...
  describe('POST', () => {
    it('returns 401 when unauthenticated', async () => {
      const res = await POST({ request: mkRequest({ statute_code: 'test' }), locals: anonLocals });
      expect(res.status).toBe(401);
    });
  ...
  describe('DELETE', () => {
    it('returns 401 when unauthenticated', async () => {
      const res = await DELETE({
        request: mkDeleteRequest({ citationId: 'x' }),
        locals: anonLocals,
      });
      expect(res.status).toBe(401);
    });
...
describe('/api/conversations/[id] (PUT/DELETE)', () => {
  ...
  describe('DELETE', () => {
    ...
    it('deletes conversation and messages', async () => {
      ...
      (db.select as any).mockReturnValueOnce(
        buildSelectChain([{ chatId: VALID_UUID, userId: 'user-1' }])
      );
