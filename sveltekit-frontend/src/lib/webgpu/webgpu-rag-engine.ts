...
export class WebGPURAGEngine {
    ...
    cudaInterop: boolean = $state(false);
    ...
    constructor(
        ...
        ptxConfig: Partial<PTXKernelConfig> = { architecture: 'sm_86', optimizationLevel: 3 }
    ) {
        this.config = {
            deviceType: 'discrete',
            enableDebug: false,
    ...
    async initialize(): Promise<boolean> {
        try {
            ...
            console.log(`🚀 Adapter: ${adapterInfo.vendor} ${adapterInfo.device}`);
