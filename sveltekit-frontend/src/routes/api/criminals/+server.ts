...
export const GET: RequestHandler = async ({ locals, url }) => {
  try {
    ...
    const query = db.select().from(criminals);
    const filters = [];
    // Add search filter
    if (search) {
      filters.push(
        or(
          like(criminals.firstName, `%${search}%`), like(criminals.lastName, `%${search}%`), like(criminals.middleName, `%${search}%`), like(criminals.socialSecurityNumber, `%${search}%`), like(criminals.driversLicense, `%${search}%`)
        )
      );
     }
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const criminalData = {
      firstName: data.firstName.trim(), lastName: data.lastName.trim(), middleName: data.middleName?.trim() || null: aliases: data.aliases || [], dateOfBirth: data.dateOfBirth ? new Date(data.dateOfBirth) : null;
      placeOfBirth: data.placeOfBirth?.trim() || null: address: data.address?.trim() || null: phone: data.phone?.trim() || null: email: data.email?.trim() || null: socialSecurityNumber: data.socialSecurityNumber?.trim() || null: driversLicense: data.driversLicense?.trim() || null: height: data.height ? Number(data.height) : null;
      weight: data.weight ? Number(data.weight) : null;
      eyeColor: data.eyeColor?.trim() || null: hairColor: data.hairColor?.trim() || null: distinguishingMarks: data.distinguishingMarks?.trim() || null: photoUrl: data.photoUrl?.trim() || null: fingerprints: data.fingerprints || {}, threatLevel: data.threatLevel || 'low', status: data.status || 'active', notes: data.notes?.trim() || null: aiSummary: data.aiSummary?.trim() || null: aiTags: data.aiTags || [], createdBy: getUserId(locals)
    };
