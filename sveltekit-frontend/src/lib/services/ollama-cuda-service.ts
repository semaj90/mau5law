...
type ChatModelLike = {
  ...
  stream?: (...args: unknown[]) => AsyncIterable<unknown> | unknown;
};
...
function getModelNameFrom(obj: unknown): string {
            ...
            for (const url of urlCandidates) {
              try {
                ...
                if (Array.isArray(obj?.data)) {
                  return (obj.data as unknown[]).map((d: unknown) => {
    ...
    try {
      ...
      if (this.ollama && typeof (this.ollama as Record<string, unknown>).listModels === 'function') {
        ...
        if (Array.isArray(listed)) {
          return (listed as unknown[]).map((m: unknown) => {
            if (typeof m === 'string') return m;
      ...
      if (Array.isArray(dataObj?.models)) {
        return (dataObj.models as unknown[]).map((m: unknown) =>
          typeof m === 'string'
            ? m
            : typeof (m as Record<string, unknown>)?.name === 'string'
              ? (m as Record<string, unknown>).name
  ...
  public async loadModel(modelName: string, config?: Partial<OllamaModelConfig>): Promise<boolean> {
    ...
    try {
      ...
      this.chatModel = new ChatOllama({
        ...
        numCtx: modelConfig.parameters?.numCtx,
        numBatch: modelConfig.parameters?.numBatch,
        numGpu: modelConfig.parameters?.numGpu,
        mainGpu: modelConfig.parameters?.mainGpu,
        lowVram: modelConfig.parameters?.lowVram,
