...
import { webgpuPolyfill } from '$lib // TODO: Verify store subscription is correct for Svelte 5/webgpu/webgpu-polyfill';
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!Array.isArray(body.vector1) || !Array.isArray(body.vector2)) {
      return json(
        { error: 'Both vector1 and vector2 must be arrays of numbers' } as VectorSimilarityResponse,
        { status: 400 }
      );
    }
    ...
    if (body.vector1.length === 0) {
      return json({ error: 'Vectors cannot be empty' } as VectorSimilarityResponse, {
        status: 400,
      });
    }
    ...
    for (let i = 0; i < body.vector1.length; i++) {
      if (typeof body.vector1[i] !== 'number' || typeof body.vector2[i] !== 'number') {
        return json({ error: 'All vector elements must be numbers' } as VectorSimilarityResponse, {
          status: 400,
        });
      }
    }
      ...
      if (mode === 'webgpu' || mode === 'auto') {
        try {
          ...
          const webgpuAvailable =
            !!(initResult && initResult.isWebGPUAvailable) ||
            (typeof navigator !== 'undefined' && 'gpu' in navigator);

          if (webgpuAvailable) {
            if (typeof polyfill.computeSimilarityWebGPU === 'function') {
              similarity = await resolveNumberOrPromise(
                polyfill.computeSimilarityWebGPU(body.vector1, body.vector2)
              );
            } else if (typeof polyfill.computeSimilarity === 'function') {
              similarity = await resolveNumberOrPromise(
                polyfill.computeSimilarity(body.vector1, body.vector2)
              );
            } else {
      ...
      if (actualMode !== 'webgpu' && (mode === 'webgl' || mode === 'auto')) {
        try {
          if (typeof polyfill.computeSimilarityWebGL === 'function') {
            similarity = await resolveNumberOrPromise(
              polyfill.computeSimilarityWebGL(body.vector1, body.vector2)
            );
            actualMode = 'webgl';
          } else {
    ...
    } catch (computeError: Error | unknown) {
    ...
    if (body.returnDiagnostics) {
      const stats: PerformanceStats =
        typeof polyfill.getPerformanceStats === 'function'
          ? (polyfill.getPerformanceStats() ?? {})
          : {};
      const hasWebGLFallback =
        typeof stats === 'object' &&
        stats !== null &&
        Boolean((stats as Record<string, unknown>)['hasWebGLFallback']);
