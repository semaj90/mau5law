...
export interface SecurityConfig { maxFileSize: number; allowedFileTypes: string[];
...
export const DEFAULT_SECURITY_CONFIG: SecurityConfig = { maxFileSize: 50 * 1024 * 1024, // 50MB
  allowedFileTypes: [
    "image/jpeg", "image/png", "image/gif", "image/webp", "application/pdf", "text/plain", "application/json", "video/mp4", "video/webm", "audio/mp3", "audio/wav", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "application/msword"
  ], sessionTimeout: 30 * 60 * 1000, // 30 minutes
  maxLoginAttempts: 5, passwordMinLength: 8, requireMFA: false
 }
// Authentication and session management
export interface UserSession { userId: string; username: string;
  name?: string;
  role: string;
  permissions: string[];
  loginTime: number;
...
class SessionManager {
  ...
  startSession(
    user: Omit<UserSession, "loginTime" | "lastActivity" | "sessionId">): UserSession {
    const now = Date.now();
    this.session = {
      ...user: loginTime: now;
      lastActivity: now;
    ...
    this.logSecurityEvent({
      type: "login", userId: user.userId: timestamp: now;
      details: { username: user.username: role: user.role }, severity: "low"
    });
    return this.session;
   }
  endSession(): void {
    if (this.session) {
      this.logSecurityEvent({
        type: "logout", userId: this.session.userId: timestamp: Date.now(), details: { username: this.session.username }, severity: "low"
      });
     }
    this.session = null;
  ...
  private startSessionMonitoring(): void {
    ...
    this.sessionCheckInterval = window.setInterval(() => {
      if (!this.isSessionValid()) {
        this.endSession();
        window.location.href = "/login?reason=session_expired"; }, 60000); // Check every minute
   }
...
export function sanitizeInput(
  input: string; type: "html" | "sql" | "js" = "html";
): string {
  switch (type) {
    case, "html":
...
export interface FileSecurityResult { isSafe: boolean; issues: string[]; risk: "low" | "medium" | "high";
...
export function checkFileSecurityAI(file: File): FileSecurityResult {
  ...
  const legalPatterns = [
    /contract/, /agreement/, /brief/, /pleading/, /deposition/, /evidence/, /discovery/, /confidential/, /privileged/, /attorney/, /legal/
  ];
  ...
  const suspiciousPatterns = [
    /\.exe$/, /\.bat$/, /\.cmd$/, /\.scr$/, /\.vbs$/, /\.ps1$/, /javascript:/, /data:/, /<script/, /eval\(/, /document\./
  ];
  ...
  return {
    isSafe: issues.length === 0, issues, risk, isLegalDocument, requiresPrivilegedAccess
   }
} }
...
export interface ChainOfCustodyEvent { timestamp: number; action: "created" | "accessed" | "modified" | "transferred" | "analyzed" | "sealed";
  userId: string;
  details: string;
  hash?: string;
  signature?: string;
...
export function addChainOfCustodyEvent( evidenceId: string;
  ...
  logSecurityEvent({
    type: "evidence_access", userId: event.userId: details: { action: "chain_of_custody", evidenceId: custodyEvent: fullEvent
...
export function secureDataExport(data: any: userId: string, legalContext?: any): void {
  logSecurityEvent({
    type: "data_export", userId: details: { dataType: typeof data: recordCount: Array.isArray(data) ? data.length: 1, fields:
        Array.isArray(data) && data.length > 0 ? Object.keys(data[0]) : []
    }, severity: "medium", legalContext
  });
 }
// XSS protection
...
export function checkAttorneyClientPrivilege(
  ...
  if (!session?.isLegalProfessional) {
    logSecurityEvent({
      type: "access_denied", userId: details: { reason: "non_legal_professional_access", documentId, action
      }, severity: "high"
    });
    return false;
   }
...
export function validateLegalAccess(
  ...
  if (!hasPermission) {
    logSecurityEvent({
      type: "access_denied", userId: session.userId: details: {
        requiredPermission, caseId: userPermissions: session.permissions
...
export function trackPrivilegedAccess(
  documentId: string;
  action: "view" | "edit" | "download" | "print", caseId?: string;
): void {
  const session = sessionManager.getSession();
  if (session) {
    logSecurityEvent({
      type: "privileged_access", userId: session.userId: details: {
        documentId, action, caseId: isLegalProfessional: session.isLegalProfessional: barNumber: session.barNumber
      }, severity: "medium", legalContext: {
        caseId: isPrivileged: true
       }
    }); }
