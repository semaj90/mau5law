...
import type { Document } from '$lib/types';
  ...
  async function performAutomatedAnalysis(): Promise<any> {
    ...
    try {
      ...
      currentAnalysisStep.set('Analysis complete!')} catch (error) {
      console.error('Analysis failed:', error);
      alert('Analysis failed. Please try again.')} finally {
      isAnalyzing = false}
  }
    ...
    for (const result of ocrResults) {
      ...
      for (const sentence of sentences) {
        const factScore = factIndicators.reduce((score, indicator) => {
          return score + (sentence.toLowerCase().includes(indicator) ? 1 : 0)}, 0);
  ...
  async function analyzeLegalIssues(): Promise<string[]> {
    ...
    for (const pattern of issuePatterns) {
      const score = pattern.keywords.reduce((acc, keyword) => {
        return acc + (combinedText.includes(keyword) ? 1 : 0)}, 0);
      if (score >= 2) {
