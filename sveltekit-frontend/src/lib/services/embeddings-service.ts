...
export class EmbeddingsService {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      if (typeof Worker === 'undefined' || typeof import.meta === 'undefined') {
        console.warn('Workers not available - skipping worker pool creation');
  ...
  private handleWorkerError(event: ErrorEvent): void {
    console.error('Worker error:', event?.message || event.error || event);
  }

  private async sendWorkerMessage<T = unknown>(
    worker: Worker,
    type: string,
    data?: unknown
  ): Promise<T> {
