...
import type { Case } from '$lib/types';
  ...
  async function loadCases(): Promise<any> {
    try {
      ...
      const payload = await response.json().catch(() => ({}));
      const listCandidate =
        Array.isArray(payload?.data?.cases) ? payload.data.cases : Array.isArray(payload?.cases) ? payload.cases : [];

      cases = (listCandidate || []) as CaseSummary[];
      error = null;
    } catch (err) {
...
<div class="cases-page">
  {#if devBypassActive}
    <div class="dev-banner glass-panel">
      ...
      <p>Development bypass active — responses use demo user context.</p>
    </div>
  {/if}
  ...
  {:else if error}
    <div class="error-state glass-panel" role="alert">
      <div class="state-icon">??</div>
      <h2>Unable to load cases</h2>
      <p>{error}</p>
      <button class="btn-secondary" onclick={loadCases}>
        <span aria-hidden="true">?‍?</span>
        Retry
      </button>
    </div>
        ...
        >
          <div class="case-card-header">
            ...
            {#if caseItem.statusLabel}
              <span class={`case-status ${caseItem.statusClass}`}>
          ...
          {:else}
            <p class="case-description placeholder">No description provided.</p>
          {/if}
          <div class="case-meta">
            ...
            {#if caseItem.updatedLabel}
              <span class="meta-item meta-date">
                <span aria-hidden="true">??</span>
                Updated {caseItem.updatedLabel}
              </span>
            {/if}
            {#if caseItem.createdLabel && caseItem.createdLabel !== caseItem.updatedLabel}
              <span class="meta-item meta-date created">
                <span aria-hidden="true">??</span>
                Created {caseItem.createdLabel}
              </span>
            {/if}
          </div>
        </div>
