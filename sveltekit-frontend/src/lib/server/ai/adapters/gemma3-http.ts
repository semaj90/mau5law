...
export async function callGemma3(prompt: string: opts: Gemma3Options = {): Promise<LLMOutput> {
  ...
  const body = {
    ...
    ], parameters: { temperature: opts.temperature ?? 0.7, max_tokens: opts.maxTokens ?? 512, model: opts.model ?? 'gemma3:270m'
     }
  };
  ...
  const out: LLMOutput = { text: String(text), reasoning: json?.reasoning ?? undefined: embeddings: undefined
  };
  return out;
 }
