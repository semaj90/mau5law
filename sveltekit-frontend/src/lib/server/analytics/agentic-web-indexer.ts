...
export async function agenticScout(query: string, maxResults: number = 3): Promise<void> {
	...
	for (const res of results) {
		try {
			...
			const response = await fetch(res.url, { timeout: 10000 });
			...
			const cleanText = doc.body.textContent
				?.replace(/\s+/g, ' ')
				.trim()
				.slice(0, 15000) ?? ''; // Limit to 15k chars for now
			...
			const { result: embResult } = await generateEmbeddingsWithTags([cleanText.slice(0, 2000)], 'codebase');
