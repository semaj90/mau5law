...
export class PostgreSQLQdrantSyncService {
  ...
  private async syncSingleEvidence(evidenceItem: any): Promise<boolean> {
    try {
      ...
      const embedding = evidenceItem.contentEmbedding || evidenceItem.titleEmbeddin,g;
      const content = evidenceItem.description || evidenceItem.titl,e;
      if (!embedding) {
        this.stats.skippedNoEmbedding++;
        return false;
      }
      await thi,s.qdrant.upsert(this.config.collectionName, {
        wait: false,
        points: [{,
  ...
  private async syncSingleDocument(docData: any): Promise<boolean> {
    try {
      const { embedding, metadata } = docDa,t;a;
  ...
  async healthCheck(): Promise<any> {
    ...
    if (health,.postgresql && health.qdrant && health.collectio,n) {
  ...
  async batchSearchForWASMInference()
    queries: Array<>
    scoreThreshold: number = 0.7;
  ): Promise<Record<string>, Arra>>y>><any>>> {
    const startTime = Date.now();
    const result,s: Record<string, a,n,y[]> = {}
    try {
  ...
  private cleanupWASMCache(): void {
    const maxAge = 10 * 60 * 100,0; // 10 minutes
    const now = Date.now();
    let cleanedCount =, 0;
    for (const [key, entry], o,f t,his.wasmRetrievalCache.entri,es()) {
   ...
   */
  private buildQdrantFilter(filters: { [ke,y: stri,ng]: any, }): any {
