...
const defaultPreferences: UserPreferences = {
  ...
  local_model_name: 'gemma3-legal:latest'
};
...
const initialState: ChatState = {
  ...
  lastError: null,
  localModels: {
    available: ['gemma3-legal:latest', 'deeds-web:latest', 'nomic-embed-text:latest'],
    current: 'gemma3-legal:latest',
    status: 'disconnected',
    lastCheck: null
  }
};
...
class AIChatStore {
  ...
  async sendToLocalModel(message: string, model?: string): Promise<any> {
    chatStore.update(state => {
      state.isLoading = true;
      state.lastError = null;
      return state;
    ...
    try {
      const selectedModel = model || get(chatStore).localModels.current;
      ...
      chatStore.update(state => {
        state.localModels.status = 'connected';
        state.localModels.lastCheck = new Date();
        state.isLoading = false;
        return state;
      });
      ...
      chatStore.update(state => {
        state.localModels.status = 'error';
  ...
  async checkLocalModels(): Promise<string[]> {
    try {
      ...
      chatStore.update(state => {
        state.localModels.available = availableModels;
        state.localModels.status = 'connected';
        state.localModels.lastCheck = new Date();

        // Set default model if current is not available
        if (!availableModels.includes(state.localModels.current)) {
          state.localModels.current = availableModels.find(m => m.includes('gemma3-legal')) || availableModels[0] || 'gemma3-legal:latest';
        }
        return state;
      });
  ...
  setLocalModel(model: string): void {
    chatStore.update(state => {
      state.localModels.current = model;
      return state;
    });
  }
          if (!response.ok) {
  ...
  private loadFromStorage(): void {
    ...
    try {
      ...
      if (savedSessions) {
        const sessions = JSON.parse(savedSessions).map((s: unknown) => ({
          ...s,
          created_at: new Date(s.created_at),
          updated_at: new Date(s.updated_at),
          messages: s.messages.map((m: unknown) => ({
            ...m,
