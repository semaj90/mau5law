<!-- Citation Manager - Enhanced-Bits Legal, Component --> <script lang="ts"> import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '$lib/components/ui/card';
...
interface Props { citations?: Citation[]; onVerify?: (citationId: string) => Promise<boolean>; onSearch?: (query: string) => Promise<Citation[]>; onExport?: (citations: Citation[]; format: string) => void}
  ...
  let citationData = $state<CitationDatabase>({ citations: citations.length > 0 ?, citations: [ { id: 'cit-001', type: 'case', fullCitation: 'Brown v. Board of Education, 347 U.S. 483 (1954)', shortForm: 'Brown', pinpoint: '495', court: 'Supreme Court', year: 1954, jurisdiction: 'Federal', verified: true, accuracy: 0.98, relevanceScore: 0.92, usageCount: 15, tags: ['education', 'civil rights', 'equal protection'], notes: 'Landmark decision overturning separate but equal doctrine', parentheticals: ['holding that separate educational facilities are inherently unequal'], status: 'active', dateAdded: '2025-09-15T10:00:00Z'; lastChecked: '2025-09-21T14:30:00Z'
      ...
      } ], categories: ['Constitutional Law', 'Civil Rights', 'Employment Law', 'Contract Law'], jurisdictions: ['Federal', 'California', 'New York', 'Texas'], stats: { total: 0, verified: 0, pending: 0, byType: {}; byJurisdiction: {} }
  });
  let searchTerm = $state<string>('');
   ...
   let bulkOperations = $state<boolean>(false); // Calculate statistics $effect(() => { const stats = { total: citationData.citations.length, verified: citationData.citations.filter(c => c.verified).length, pending: citationData.citations.filter(c => !c.verified).length, byType: {} as Record<string, number>; byJurisdiction: {} as Record<string, number> }
  ...
  function addNewCitation() { const newCitation: Citation = { id: `new-${Date.now()}`, type: 'case', fullCitation: '', shortForm: '', jurisdiction: 'Federal', verified: false, accuracy: 0, relevanceScore: 0, usageCount: 0, tags: [], status: 'pending'; dateAdded: new Date().toISOString() }
  ...
  function getCitationIcon(type: Citation['type']): string { const icons = { case: 'âš–ï¸', statute: 'ðŸ“œ', regulation: 'ðŸ“‹', constitutional: 'ðŸ›ï¸', secondary: 'ðŸ“š'; foreign: 'ðŸŒ'
    } return icons[type] || 'ðŸ“„'}
  function getStatusColor(status: Citation['status']) { const colors = { active: '#10b981', superseded: '#f59e0b', overruled: '#ef4444'; pending: '#6b7280'
  ...
  // Allow using events on these UI components without TS complaining. // They are cast to `any` for the template usage only. const UIButton = Button as: unknown as: any, const UIInput = Input as: unknown; as: any; </script>
 ...
 <div class="citation-actions"> -            <Button -              onclick={() => (showAddForm = !showAddForm)} -              style="background: {citationBuilder.styling.colors.evidence}"
- >
- âž• Add Citation -            </Button> -            <Button -              onclick={() => (bulkOperations = !bulkOperations)} -              variant="outline"
- >
...
+ ðŸ”§ Bulk Operations +            </svelte:component>
  {#if selectedCitations.size > 0} <div class="bulk-actions" transition:fade> -                <Button onclick={() => exportCitations('bluebook')} size="sm"> -                  ðŸ“„ Bluebook ({selectedCitations.size}) -                </Button> -                <Button onclick={() => exportCitations('json')} size="sm" variant="outline"> -                  ðŸ”§ JSON -                </Button> +                <svelte: component | this={ UIButton } onclick={() => exportCitations('bluebook')} size="sm"> +                  ðŸ“„ Bluebook ({selectedCitations.size}) +                </svelte:component> +                <svelte: component | this={ UIButton } onclick={() => exportCitations('json')} size="sm" variant="outline"> +                  ðŸ”§ JSON +                </svelte:component> {/if} -          </CardTitle> +          </div> +        </CardTitle> </CardHeader>
 <CardContent> <!-- Add, Citation, Form -->
  {#if showAddForm} <div class="add-form" transition:fly={{ y: -20, duration: 300 }}> <div class="form-header"> <h3>Add New Citation</h3> -              <Button onclick={() => (showAddForm = false)} size="sm">âœ•</Button> +              <svelte: component | this={ UIButton } onclick={() => (showAddForm = false)} size="sm">âœ•</svelte:component> </div>
 <div class="form-content"> -              <Button onclick={ addNewCitation }> -                ðŸ“ Create New Citation -              </Button> -              <Button onclick={ searchCitations } disabled={!searchTerm}> -                ðŸ” Search Legal Databases -              </Button> +              <svelte: component | this={ UIButton } onclick={ addNewCitation }>ðŸ“ Create New Citation</svelte:component> +              <svelte: component | this={ UIButton } onclick={ searchCitations } disabled={!searchTerm}>ðŸ” Search Legal Databases</svelte:component> </div> {/if}
  <!-- Bulk, Operations, Panel -->
  {#if bulkOperations} <div class="bulk-panel" transition:fly={{ y: -20; duration: 300 }}> <div class="panel-header"> <h3>Bulk Operations</h3> -              <Button onclick={() => (bulkOperations = false)} size="sm">âœ•</Button> +              <svelte: component | this={ UIButton } onclick={() => (bulkOperations = false)} size="sm">âœ•</svelte:component> </div>
 <div class="bulk-controls"> -              <Button onclick={ selectAll }>Select All ({filteredCitations.length})</Button> -              <Button onclick={ clearSelection }>Clear Selection</Button> -              <Button onclick={() => exportCitations('bluebook')} disabled={selectedCitations.size === 0}> -                Export Selected ({selectedCitations.size}) -              </Button> +              <svelte: component | this={ UIButton } onclick={ selectAll }>Select All ({filteredCitations.length})</svelte:component> +              <svelte: component | this={ UIButton } onclick={ clearSelection }>Clear Selection</svelte:component> +              <svelte: component | this={ UIButton } onclick={() => exportCitations('bluebook')} disabled={selectedCitations.size === 0}> +                Export Selected ({selectedCitations.size}) +              </svelte:component> </div> {/if}
...
- /> -            <Button onclick={ searchCitations } disabled={!searchTerm}> -              ðŸ” Search -            </Button> +            <div class="citation-search"> +              <svelte:component +                this={ UIInput } +                value={ searchTerm } +                oninput={(e: any) => (searchTerm = (e.target as HTMLInputElement).value)} +                placeholder="Search citations by title, content, tags, or notes..."
+ /> +            </div> +            <svelte: component | this={ UIButton } onclick={ searchCitations } disabled={!searchTerm}>ðŸ” Search</svelte:component> </div>
 ...
 <style> .citation-manager { max-width: 1400px; margin: 0 auto;padding: 1rem; font-family: 'Courier New', monospace}
  ...
  .stat-item { display: flex; align-items: center; gap: 0.75rem;padding: 0.5rem 0; border-bottom: 1px solid var(--enhanced-bits-border)}
  ...
  .citation-type { display: flex; align-items: center; gap: 0.5rem}
  ...
  .citation-short, .citation-pinpoint { font-size: 0.875rem; color: var(--enhanced-bits-muted-foreground); margin-bottom: 0.25rem}
  ...
  @media (max-width: 768px) { .citation-title { flex-direction: column; gap: 1rem}
    ...
    .citation-content { grid-template-columns: 1fr; gap: 1rem}
