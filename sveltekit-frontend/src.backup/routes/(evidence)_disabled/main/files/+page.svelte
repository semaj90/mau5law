...
import { Video } from "lucide-svelte";; import { onMount } from 'svelte';; // Props // State let evidenceFiles: string | number[] = []; let filteredFiles: unknown[] = []; let loading = $state <boolean>(false); let error: string | null = null; let uploadProgress = 0; let uploading = $state <boolean>(false); let selectedFiles = new Set<string>(); let showBulkActions = $state <boolean>(false); // Filters and view options let searchQuery = ''; let selectedCategory = ''; let viewMode = 'grid'; // 'grid' | 'list'
