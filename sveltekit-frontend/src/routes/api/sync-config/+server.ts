...
type PostgresConnection = {
  ...
  sql: (query: TemplateStringsArray | string, ...args: unknown[]) => Promise<any[]>;
};
type MinIOServiceShape = {
  client?: { listBuckets?: () => Promise<Array<{ name: string }>>; // other client methods may exist };
...
// Move type definitions here to make them accessible throughout the file
type ServiceStatus = {
  status?: string;
  message?: string;
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    if (postgresConnection) {
      testResults.postgresql = { status: 'testing', message: 'Connecting to PostgreSQL...' };
      const dbConnected = await postgresConnection.initializeDatabase();
      if (dbConnected) {
        // Test a simple query
        const result = await postgresConnection.sql`
          SELECT version() as pg_version, current_database() as database_name, current_user as connected_user, EXISTS(SELECT 1 FROM pg_extension WHERE extname = 'vector') as pgvector_installed
        `;
        ...
        testResults.postgresql = { status: 'error', message: 'PostgreSQL connection failed' };
      }
    } else {
      testResults.postgresql = { status: 'unavailable', message: 'PostgreSQL module not loaded' };
    }
  } catch (error) {
    testResults.postgresql = { status: 'error', message: `PostgreSQL error: ${error}` };
  }

  // Test Redis Connection (using native test)
  try {
    testResults.redis = { status: 'testing', message: 'Testing Redis connection...' };
    // Try to connect to Redis using native fetch to test endpoint
    const redisTestResponse = await fetch('http://localhost:6379', { method: 'GET' }).catch(() => null);
  ...
  try {
    if (minioService) {
      testResults.minio = { status: 'testing', message: 'Testing MinIO connection...' };
      ...
      testResults.minio = { status: 'unavailable', message: 'MinIO service not initialized' };
    }
  } catch (error) {
    testResults.minio = { status: 'error', message: `MinIO initialization error: ${error}` };
  }
  ...
  const overallStatus = {
    ...
    status: healthyServices.length === services.length ? 'all_healthy' : healthyServices.length >= services.length / 2 ? 'mostly_healthy' : 'unhealthy',
    responseTime
  };
