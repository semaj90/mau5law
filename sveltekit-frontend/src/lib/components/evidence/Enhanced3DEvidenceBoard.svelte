<!-- 🎯 Enhanced 3D Evidence Board with WebGPU Rendering -->
<script lang="ts">
  ...
  function getVisibleNodes(): EvidenceNode[] {
    ...
    return nodes
      .filter(node => {
        const distance = Math.sqrt(
          Math.pow(node.position.x - $camera.x, 2) +
          Math.pow(node.position.y - $camera.y, 2) +
          Math.pow(node.position.z - $camera.z, 2)
        );
        return distance < 1200; // Culling distance
      })
      .sort((a, b) => {
        // Sort by distance for depth sorting
        const distA = Math.sqrt(
          Math.pow(a.position.x - $camera.x, 2) +
          Math.pow(a.position.y - $camera.y, 2) +
          Math.pow(a.position.z - $camera.z, 2)
        );
        const distB = Math.sqrt(
          Math.pow(b.position.x - $camera.x, 2) +
          Math.pow(b.position.y - $camera.y, 2) +
          Math.pow(b.position.z - $camera.z, 2)
        );
  ...
  function renderNodes() {
    ...
    for (const node of visibleNodes) {
      ...
      if (lodLevel <= 1) {
        ...
        drawDetailedNode(ctx, screenPos, size, node, nodeColor);
      } else if (lodLevel <= 2) {
        // Medium detail: Simple circle
      ...
      if (lodLevel === 0) {
        ctx.shadowBlur = 0;
        ctx.fillStyle = palette.colors.foreground;
  ...
  function exportBoard() {
    ...
    a.href = url;
    a.download = 'evidence-board-export.json';
    a.click();
