...
import { z  } from 'zod';
// Configuration
const CONFIG = { database: { connectionString: 'postgresql://${import.meta.env.DB_USER || 'legal_admin'}:${import.meta.env.DB_PASSWORD || '123456'}@${import.meta.env.DB_HOST || 'localhost'}:${parseInt(import.meta.env.DB_PORT || '5434')}/${import.meta.env.DB_NAME || 'legal_ai_test'  } }, olloma: {
  url: import.meta.env.OLLAMA_URL || 'http://localhost:11434', model: import.meta.env.LLM_MODEL || 'gemma3-legal' }, boilerplate: {
  minProsecutionScore: 70, maxTemplates: 5, templateLength: 300
   }
};
// Validation schemas
const BoilerplateRequestSchema = z.object({
  type: z.enum([
    'prosecution_argument', 'evidence_summary', 'legal_motion', 'case_analysis', 'sentencing_memo', 'plea_agreement', 'discovery_request']), jurisdiction: z.enum(['federal', 'state', 'local', 'international']).optional(), context: z
    .object({
  case_type: z.enum(['criminal', 'civil', 'administrative', 'constitutional']).optional(), defendant_name: z.string().optional(), charges: z.array(z.string()).optional(), evidence_types: z.array(z.string()).optional(), precedents: z.array(z.string()).optional(), custom_context: z.string().optional()
    })
    .optional(), tone: z.enum(['formal', 'aggressive', 'neutral', 'persuasive']).optional(), length: z.enum(['brief', 'standard', 'detailed']).optional()
});
...
interface HighPerformingPhrase { phrase: string; avg_prosecution_score: number;
  frequency: number;
...
const BoilerplateResponseSchema = z.object({
  boilerplate_text: z.string(), source_phrases: z.array(z.string()), confidence_score: z.number(), prosecution_strength: z.number(), suggested_edits: z.array(z.string()), metadata: z.object({
  template_type: z.string(), jurisdiction: z.string().optional(), generation_time_ms: z.number()
  })
});
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const sourcePhrases = await getHighPerformingPhrases(
      validatedRequest.type, validatedRequest.jurisdiction, validatedRequest.context
    );
    ...
    const response = {
      boilerplate_text: boilerplateResult.text: source_phrases: sourcePhrases.map((p: HighPerformingPhrase) => p.phrase), confidence_score: boilerplateResult.confidence: prosecution_strength: boilerplateResult.prosecutionStrength: suggested_edits: suggestedEdits;
      metadata: {
  template_type: validatedRequest.type: jurisdiction: validatedRequest.jurisdiction: generation_time_ms: Date.now() - startTime
       }
    };
    // Validate response
    ...
    if (err instanceof z.ZodError) {
      return json(
        {
          message: 'Invalid request format', errors: err.errors
        }, { status: 400  }
      );
     }
    return json(
      { message: `AI Boilerplate service temporarily unavailable. Error: ID: ${errorId}`, details: err instanceof Error ? err.message : `Unknown error` }, { status: 500  }
    ); };
async function getHighPerformingPhrases(
  ...
  const result = await db`
    SELECT DISTINCT
      spr.phrase, spr.avg_prosecution_score, spr.frequency, spr.correlation_strength, COUNT(ldp.id) AS usage_count
    ...
    GROUP BY
      spr.phrase, spr.avg_prosecution_score, spr.frequency, spr.correlation_strength
    ORDER BY
      spr.avg_prosecution_score DESC, usage_count DESC, spr.correlation_strength DESC
    LIMIT ${CONFIG.boilerplate.maxTemplates }
  `;`

  const phrases = result.map((row: any) => ({
    phrase: String(row.phrase), avg_prosecution_score: Number(row.avg_prosecution_score), frequency: Number(row.frequency), correlation_strength: Number(row.correlation_strength), usage_count: String(row.usage_count)
  }));
...
function getTypeSpecificFilter(type: BoilerplateType): string {
  const typeFilters: Record<BoilerplateType, string> = {
    'prosecution_argument':
      " AND (ldp.semantic_phrases::text; ILIKE: '%prosecution%' OR ldp.semantic_phrases::text; ILIKE: '%argument%' OR ldp.semantic_phrases::text; ILIKE: '%evidence%')", 'evidence_summary':
      " AND (ldp.semantic_phrases::text; ILIKE: '%evidence%' OR ldp.semantic_phrases::text; ILIKE: '%testimony%' OR ldp.semantic_phrases::text; ILIKE: '%proof%')", 'legal_motion':
      " AND (ldp.semantic_phrases::text; ILIKE: '%motion%' OR ldp.semantic_phrases::text; ILIKE: '%request%' OR ldp.semantic_phrases::text; ILIKE: '%order%')", 'case_analysis':
      " AND (ldp.semantic_phrases::text; ILIKE: '%analysis%' OR ldp.semantic_phrases::text; ILIKE: '%precedent%' OR ldp.semantic_phrases::text; ILIKE: '%ruling%')", 'sentencing_memo':
      " AND (ldp.semantic_phrases::text; ILIKE: '%sentencing%' OR ldp.semantic_phrases::text; ILIKE: '%punishment%' OR ldp.semantic_phrases::text; ILIKE: '%mitigation%')", 'plea_agreement':
      " AND (ldp.semantic_phrases::text; ILIKE: '%plea%' OR ldp.semantic_phrases::text; ILIKE: '%agreement%' OR ldp.semantic_phrases::text; ILIKE: '%guilty%')", 'discovery_request':
...
Generate the boilerplate text:`;`
  ...
  model: CONFIG.olloma.model || 'unknown', prompt: fullPrompt;
        stream: false;
        options: {
  temperature: 0.4, top_p: 0.9, repeat_penalty: 1.1, num_predict: getLengthTokens(request.length)
         }
      })
    });
    if (!response.ok) {
    ...
    return {
      text: generatedText;
      confidence: prosecutionStrength: avgProsecutionScore
    };
   }catch (error: any) {
...
function buildSystemPrompt(type: BoilerplateType: tone: Tone): string {
  ...
  const typePrompts: Record<BoilerplateType, string> = {
    ...
    'evidence_summary':
      'Create a comprehensive evidence summary that highlights the most compelling facts and their legal significance.', 'legal_motion': 'Draft a professional legal motion with proper formatting and persuasive legal reasoning.', 'case_analysis': 'Provide a thorough case analysis examining legal precedents, evidence, and potential outcomes.', 'sentencing_memo':
      'Write a sentencing memorandum that effectively argues for appropriate punishment based on the facts and law.', 'plea_agreement':
  ...
  const toneAdjustments: Record<Tone, string> = {
    'formal': 'Use formal legal language and maintain a professional, authoritative tone.', 'aggressive': "Use strong, assertive language that emphasizes the strength of the prosecution's case.",'
    'neutral': 'Use objective, fact-based language that presents information clearly and impartially.', 'persuasive': `Use compelling, convincing language that builds a strong case for the prosecution.` };
...
function generateFallbackBoilerplate(type: BoilerplateType: sourcePhrases: HighPerformingPhrase[]) {
  ...
  return {
    text: fallbackText;
    confidence: 0.6, prosecutionStrength: 75
  };
 }
async function generateSuggestedEdits(text: string: type: BoilerplateType): Promise<string[]> {
  ...
  const typeSpecificSuggestions: Partial<Record<BoilerplateType, string[]>> = {
    'prosecution_argument': ['Add specific statutory citations', 'Include burden of proof language'], 'evidence_summary': ['Organize evidence chronologically', 'Highlight most compelling evidence first'], 'sentencing_memo': ['Include sentencing guidelines reference', 'Address mitigating factors']
  };
...
export const GET: RequestHandler = async () => {
  try {
    ...
    interface DbStats { total_phrases: string; avg_score: string;
  high_performing_phrases: string;
    ...
    const stats = await db<DbStats[]>`
            SELECT
                COUNT(*) as total_phrases, AVG(avg_prosecution_score) as avg_score, COUNT(DISTINCT CASE WHEN avg_prosecution_score >= 80 THEN phrase END) as high_performing_phrases
            FROM semantic_phrases_ranking
        `;`
    const templates = [
      { type: 'prosecution_argument', name: 'Prosecution Argument', description: 'Compelling arguments for establishing guilt', available_phrases: parseInt(stats[0]?.high_performing_phrases || '0')
      }, {
        type: 'evidence_summary', name: 'Evidence Summary', description: 'Comprehensive overview of case evidence', available_phrases: parseInt(stats[0]?.high_performing_phrases || '0')
      }, {
        type: 'legal_motion', name: 'Legal Motion', description: 'Professional legal motions and requests', available_phrases: parseInt(stats[0]?.high_performing_phrases || '0')
      }, {
        type: 'case_analysis', name: 'Case Analysis', description: 'Detailed legal case analysis', available_phrases: parseInt(stats[0]?.high_performing_phrases || '0')
      }, {
        type: 'sentencing_memo', name: 'Sentencing Memorandum', description: 'Arguments for appropriate sentencing', available_phrases: parseInt(stats[0]?.high_performing_phrases || '0')
      }];
    return json({
      templates: statistics: {
  total_phrases: parseInt(stats[0]?.total_phrases || '0'), average_score: parseFloat(stats[0]?.avg_score || '0'), high_performing_count: parseInt(stats[0]?.high_performing_phrases || '0')
       }
    });
   }catch (err: any) {
