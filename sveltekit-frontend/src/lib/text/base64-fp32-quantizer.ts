...
export class Base64FP32Quantizer {
  ...
  private initializeCUDAThreadPool(): void {
    ...
    for (let i = 0; i < maxThreads; i++) {
      const context: CUDAThreadContext = {
        ...
        sharedMemory: new ArrayBuffer(48 * 1024),
  ...
  private async quantizationKernel(data: Uint8Array, blockId: number, threadsPerBlock: number, config: QuantizationOptions): Promise<Float32Array> {
  ...
  private generateCacheKey(base64Data: string, config: QuantizationOptions): string {
    return btoa(base64Data.substring(0, 100)).replace(/[^a-zA-Z0-9]/g, '').substring(0, 32);
  }
  ...
  async processGemmaLegalOutput(modelOutput: string, options?: Partial<QuantizationOptions>): Promise<GemmaOutputQuantization> {
...
export async function quantizeGemmaLegalOutput(base64Output: string, options?: Partial<QuantizationOptions>): Promise<QuantizationResult> {
  return await base64FP32Quantizer.quantizeGemmaOutput(base64Output, options);
}

export async function processGemmaResponse(modelResponse: string, cudaThreads: number = 256): Promise<GemmaOutputQuantization> {
  return await base64FP32Quantizer.processGemmaLegalOutput(modelResponse, { cudaThreads, quantizationBits: 8, scalingMethod: 'sigmoid', targetLength: 2048, cacheStrategy: 'aggressive' });
}
