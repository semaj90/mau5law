...
export const caseEmbeddings = pgTable(
  ...
  table => ({
    ...
    // IVFFlat index for exact search on smaller datasets (create via manual SQL migration if needed)
...
export const legalDocumentChunks = pgTable(
  'legal_document_chunks',
  {
    ...
    practiceArea: jsonb('practice_area').default(sql`'[]'::jsonb`),
    jurisdiction: text('jurisdiction'),
    confidenceLevel: real('confidence_level'), // 0-1
    riskLevel: text('risk_level'), // low, medium, high, critical
    // Processing metadata
    extractedEntities: jsonb('extracted_entities').default(sql`'[]'::jsonb`),
    keyTerms: jsonb('key_terms').default(sql`'[]'::jsonb`),
  ...
  table => ({
    ...
    // GIN indexes for JSONB fields (create via migration if needed)
