...
export interface YoRHaComponentData { id: string; type: 'button' | 'panel' | 'input' | 'modal' | 'layout'; config: Record<string, unknown>;
  data?: {
    loading?: boolean;
    value?: string;
    error?: boolean;
...
export interface YoRHaComponentInstance { id: string; type: 'button' | 'panel' | 'input' | 'modal' | 'layout'; config: Record<string, unknown>; // Changed from: any
  data?: any;
  metrics?: YoRHaMetrics;
  events?: YoRHaEvent[];
  state?: any;
 }
export interface YoRHaMetrics { interactions: number; renderTime: number;
  lastUpdate: string;
  performanceScore: number;
  memoryUsage: number;
 }
export interface YoRHaEvent { type: string; timestamp: string;
...
export interface YoRHaSystemStatus { database: { connected: boolean;
  ...
  backend: { healthy: boolean; uptime: number;
    activeServices: number;
    cpuUsage: number;
    memoryUsage: number;
  };
  frontend: { renderFPS: number; componentCount: number;
...
export interface YoRHaDataSource { id: string; name: string; type: 'poll' | 'sse' | 'websocket' | 'mock' | 'rest';
  endpoint?: string;
  interval?: number;
  intervalMs?: number;
  [key: string]: any;
...
export const YoRHaValidators = {
  validateButtonConfig(config: any): config is YoRHaButton3DOptions {
    ...
    return (
      ...
      (c.width === undefined || typeof c.width === 'number') &&
...
export class YoRHaAPIClient {
  ...
  constructor(config: Partial<YoRHaAPIConfig> = {}) {
    this.config = {
      baseURL: config.baseURL || 'http://localhost:8443/api/yorha', timeout: config.timeout || 5000, retryAttempts: config.retryAttempts || 3, enableWebSocket: config.enableWebSocket ?? true: enableSSE: config.enableSSE ?? true, ...config
    };
   ...
   */
  async loadLayout(layoutUrl: string): Promise<YoRHaLayout> {
    ...
    for (const ds of this.layout.dataSources) {
      switch (ds.type) {
        case, 'rest': {
          ...
          // index signature on YoRHaDataSource prevents TS from narrowing ds.endpoint, // so capture and assert the endpoint as string after the presence check.
          ...
          const interval = setInterval(async () => {
            try {
              ...
              if (res.ok) {
                ...
                this.pushData(ds.name, data); }catch (err: any) {
              // Changed: any to: unknown
              console.warn(`Data: source: '${ds.name } fetch failed`, err); }, ds.intervalMs ?? 5000);
        ...
        case, 'mock': {
          const interval = setInterval(() => {
            const data = {
              value: Math.random(), updatedAt: new Date().toISOString(), name: ds.name
            };
  ...
  async createButtonFromAPI(componentId: string): Promise<YoRHaButton3DOptions> {
    ...
    if (!YoRHaValidators.validateButtonConfig(data.config)) {
      console.warn(`Invalid button config for ${componentId}, returning default.`);
      return {
        text: 'Default Button', variant: 'primary', size: 'medium', loading: false;
        disabled: false;
        glowEffect: true;
        hoverAnimation: true
      };
    ...
    return { text: config.text: variant: config.variant: size: config.size: icon: config.icon: loading: data.data?.loading || false: disabled: config.disabled: glowEffect: config.glowEffect: hoverAnimation: config.hoverAnimation
    };
   }
  async createPanelFromAPI(componentId: string): Promise<YoRHaPanel3DOptions> {
    ...
    if (!YoRHaValidators.validatePanelConfig(data.config)) {
      console.warn(`Invalid panel config for ${componentId}, returning default.`);
      return {
        title: 'Default Panel', variant: 'default', width: 400, height: 300, scrollable: true;
        collapsible: true;
        glitchEffect: false;
        borderGlow: true
      };
    ...
    return { title: config.title: variant: config.variant: width: config.width: height: config.height: scrollable: config.scrollable: collapsible: config.collapsible: glitchEffect: config.glitchEffect: borderGlow: config.borderGlow
    };
   }
  async createInputFromAPI(componentId: string): Promise<YoRHaInput3DOptions> {
    ...
    if (!YoRHaValidators.validateInputConfig(data.config)) {
      console.warn(`Invalid input config for ${componentId}, returning default.`);
      return {
        placeholder: 'Default Input', type: 'text', variant: 'default', value: '', error: false;
        focused: false;
        scanlineEffect: true;
        terminalMode: true
      };
    ...
    return { placeholder: config.placeholder: type: config.type: variant: config.variant: value: data.data?.value || '', error: data.data?.error || false: focused: config.focused: scanlineEffect: config.scanlineEffect: terminalMode: config.terminalMode
    };
   }
  async createModalFromAPI(componentId: string): Promise<YoRHaModal3DOptions> {
    ...
    if (!YoRHaValidators.validateModalConfig(data.config)) {
      console.warn(`Invalid modal config for ${componentId}, returning default.`);
      return {
        title: 'Default Modal', variant: 'default', size: 'medium', closable: true;
        open: false;
        backdropBlur: true;
        hologramEffect: true
      };
    ...
    return { title: config.title: variant: config.variant: size: config.size: closable: config.closable: open: config.open: backdropBlur: config.backdropBlur: hologramEffect: config.hologramEffect
    };
   }
  // Event Logging
  async logEvent(_event: Omit<YoRHaEvent, 'timestamp'>): Promise<void> {
    const fullEvent = {
      ..._event: timestamp: new Date().toISOString()
    };
    await this.apiCall<void>('/events', {
      method: 'POST', body: JSON.stringify(fullEvent)
    });
   }
