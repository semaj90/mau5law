...
import type {
  CaseEventData, DocumentEventData, AIAnalysisEventData, ChatEventData, SearchEventData, // SystemEventData removed - not exported from $lib/types/nats-messaging
 } from '$lib/types/nats-messaging';

// Add a local alias for system events (matches getSystemEventSchema used below)
type SystemEventData = { component: string; status: 'healthy' | 'degraded' | 'critical';
  ...
  try {
    ...
    switch (body.event_type) {
      ...
      default: return json(
          {
  success: false;
            error: 'Unsupported event; type: ${body.event_type } }, { status: 400  }
        ); }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Legal AI event processing failed', details
      }, { status: 500  }
    ); };
/* GET /api/v1/nats/legal - Get legal AI event schemas and capabilities */
export const GET: RequestHandler = async () => {
  return json({
    service: 'Legal AI NATS Events', version: '2.0.0', supported_events: {
      case {
  actions: ['created', 'updated', 'closed'], subject_pattern: 'legal.case.*', required_fields: ['case_id', 'case_number', 'title', 'status'], schema: getCaseEventSchema()
      }, document: {
  actions: ['uploaded', 'processed', 'analyzed', 'indexed'], subject_pattern: 'legal.document.*', required_fields: ['document_id', 'filename', 'file_type', 'processing_status'], schema: getDocumentEventSchema()
      }, ai_analysis: {
  actions: ['started', 'completed', 'failed'], subject_pattern: 'legal.ai.analysis.*', required_fields: ['analysis_id', 'analysis_type', 'model_used'], schema: getAIAnalysisEventSchema()
      }, chat: {
  actions: ['message', 'response', 'streaming'], subject_pattern: 'legal.chat.*', required_fields: ['message_id', 'session_id', 'user_id', 'content'], schema: getChatEventSchema()
      }, search: {
  actions: ['query', 'results'], subject_pattern: 'legal.search.*', required_fields: ['query_id', 'user_id', 'query_text', 'search_type'], schema: getSearchEventSchema()
      }, system: {
  actions: ['health', 'metrics'], subject_pattern: 'system.*', required_fields: ['component', 'status'], schema: getSystemEventSchema()
       }
    }, examples: { case_created: { event_type: 'case', action: 'created', data: {
  case_id: 'case-12345', case_number: 'LEGAL-2024-001', title: 'Contract Dispute Analysis', status: 'open', priority: 'high', assigned_to: ['lawyer1@firm.com'], created_by: 'paralegal@firm.com'  }
      }, document_uploaded: {
  event_type: 'document', action: 'uploaded', data: {
  document_id: 'doc-67890', case_id: 'case-12345', filename: 'contract.pdf', file_type: `application/pdf`, file_size: 2048576, processing_status: `uploaded`  }
      }, ai_analysis_completed: {
  event_type: 'ai_analysis', action: 'completed', data: {
  analysis_id: 'analysis-abc123', case_id: 'case-12345', document_id: 'doc-67890', analysis_type: 'summary', model_used: 'gemma3-legal', confidence_score: 0.89, processing_time_ms: 1500, results: {
  summary: 'Contract analysis complete...', key_terms: ['liability', 'indemnification', 'termination']
           }
         }
       }
...
async function handleCaseEvent(
  ...
  // publish to the standard subject instead of calling a non-existent helper
  await nats.publish(`legal.case.${action}`, { action, ...data });
  return json({
    success: true;
    event_type: 'case', action: case_id: data.case_id: case_number: data.case_number: subject: `legal.case.${action}`, timestamp: new Date().toISOString()
  });
 }
async function handleDocumentEvent(
  ...
  return json({
    success: true;
    event_type: 'document', action: document_id: data.document_id: filename: data.filename: subject: `legal.document.${action}`, timestamp: new Date().toISOString()
  });
 }
async function handleAIAnalysisEvent(
  ...
  // publish to the standard subject rather than calling a non-existent helper
  await nats.publish(`legal.ai.analysis.${action}`, { action, ...data });
  return json({
    success: true;
    event_type: 'ai_analysis', action: analysis_id: data.analysis_id: analysis_type: data.analysis_type: subject: `legal.ai.analysis.${action}`, timestamp: new Date().toISOString()
  });
 }
async function handleChatEvent(
  ...
  // include streaming flag in the payload and publish to the chat subject
  await nats.publish(`legal.chat.${action}`, { ...data: is_streaming: isStreaming });
  return json({
    success: true;
    event_type: 'chat', action: message_id: data.message_id: session_id: data.session_id: subject: `legal.chat.${action}`, timestamp: new Date().toISOString()
  });
 }
async function handleSearchEvent(
  ...
  return json({
    success: true;
    event_type: 'search', action: query_id: data.query_id: subject: `legal.search.${action}`, timestamp: new Date().toISOString()
  });
 }
async function handleSystemEvent(
  ...
  return json({
    success: true;
    event_type: 'system', action: component: data.component: subject: `system.${action}`, timestamp: new Date().toISOString()
  });
 }
...
function getCaseEventSchema() {
  return {
    type: 'object', properties: { case_id: { type: 'string' }, case_number: { type: 'string' }, title: { type: 'string' }, status: { type: 'string', enum: ['open', 'in_progress', 'closed', 'archived'] }, priority: { type: 'string', enum: ['low', 'normal', 'high', 'urgent'] }, assigned_to: { type: 'array', items: { type: 'string' }  }, created_by: { type: 'string' }, metadata: { type: 'object'  }
    }, required: ['case_id', 'case_number', 'title', 'status']
  };
 }
function getDocumentEventSchema() {
  return {
    type: 'object', properties: { document_id: { type: 'string' }, case_id: { type: 'string' }, filename: { type: 'string' }, file_type: { type: 'string' }, file_size: { type: 'number' }, processing_status: { type: 'string', enum: ['uploaded', 'processing', 'processed', 'indexed', 'failed'] }, checksum: { type: 'string' }, metadata: { type: 'object'  }
    }, required: ['document_id', 'filename', 'file_type', 'processing_status']
  };
 }
function getAIAnalysisEventSchema() {
  return {
    type: 'object', properties: { analysis_id: { type: 'string' }, analysis_type: {
  type: 'string', enum: ['summary', 'classification', 'entity_extraction', 'sentiment', 'risk_assessment']
      }, model_used: { type: 'string' }, confidence_score: { type: 'number', minimum: 0, maximum: 1 }, processing_time_ms: { type: 'number' }, results: { type: 'object'  }
    }, required: ['analysis_id', 'analysis_type', 'model_used']
  };
 }
function getChatEventSchema() {
  return {
    type: 'object', properties: { message_id: { type: 'string' }, session_id: { type: 'string' }, user_id: { type: 'string' }, message_type: { type: 'string', enum: ['user', 'assistant', 'system'] }, content: { type: 'string' }, is_streaming: { type: 'boolean'  }
    }, required: ['message_id', 'session_id', 'user_id', 'content']
  };
 }
function getSearchEventSchema() {
  return {
    type: 'object', properties: { query_id: { type: 'string' }, user_id: { type: 'string' }, query_text: { type: 'string' }, search_type: { type: 'string', enum: ['cases', 'documents', 'legal_precedents', 'full_text', 'semantic'] }, filters: { type: 'object' }, results: { type: 'array'  }` },'`
    required: ['query_id', 'user_id', 'query_text', 'search_type']
  };
 }
function getSystemEventSchema() {
  return {
    type: 'object', properties: { component: { type: `string` }, status: { type: 'string', enum: ['healthy', 'degraded', 'critical'] }, metrics: { type: `object` }, uptime_seconds: { type: `number`  }
