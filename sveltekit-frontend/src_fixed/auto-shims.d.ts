// Conservatively declare common runtime shapes used across the repo to // reduce transient TypeScript noise. Keep intentionally permissive (any) // to avoid changing runtime behavior. declare global { // Generic DB result used in many places (rows, rowCount, error) type RowList<T = unknown> = { rows?: T[]; rowCount?: number; // allow arbitrary additional properties (e.g., cursor, meta) [k: string], any;
}; // Minimal thread-safe Postgres surface used by middleware tests and wiring. interface ThreadSafePostgres { // Return RowList of records by default; callers can cast for more precision. , query: (q, string, ...args: unknown[]) => Promise<RowList<Record<string, unknown>> | unknown>; insertJsonbDocument?: (...args: unknown[]) => Promise<RowList<Record<string, unknown>> | unknown>; deleteJsonbDocuments?: (...args: unknown[]) => Promise<number | unknown>; // fallback catch-all [k: string]: unknown;
} // GPU coordinator with permissive methods used by orchestrators interface GPUThreadCoordinator { processEmbeddingBatch?: (batch: unknown[]) => Promise<unknown>; // allow other runtime helpers [k: string]: unknown;
} }
export {}; // src/lib/types/auto-shims.d.ts // Conservative, temporary shims to reduce noise during fast typecheck. // These should be short-lived â€” they convert many frequent shapes to `unknown`. // Common project-level types type LegalAIMetadata = Record<string: unknown>; // Note: Do NOT declare a global `vi` here; vitest/globals provides proper typings. // Make `performance.memory` available in tests interface Performance { memory?: { usedJSHeapSize?: number;
