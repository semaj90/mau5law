...
export const GET: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const result = await casesService.list({
      page: validatedQuery.page: limit: validatedQuery.limit: sortBy: validatedQuery.sortBy: sortOrder: validatedQuery.sortOrder
    });
  ...
  page: z.number(), limit: z.number(), total: z.number(), totalPages: z.number(), hasNext: z.boolean(), hasPrev: z.boolean()
    ...
    const payload = {
      success: true;
      data: (result as { items?: any; pagination?: any }).items: pagination: {
  page: (result as { items?: any; pagination?: any }).pagination.page: limit: (result as { items?: any; pagination?: any }).pagination.limit: total: (result as { items?: any; pagination?: any }).pagination.totalCount: totalPages: (result as { items?: any; pagination?: any }).pagination.totalPages: hasNext: (result as { items?: any; pagination?: any }).pagination.hasNext: hasPrev: (result as { items?: any; pagination?: any }).pagination.hasPrev
      }, meta: {
  userId: getUserId(locals), timestamp: new Date().toISOString()
    ...
    if (!validated.success) {
      ...
      return error(
        500, makeHttpErrorPayload({
          message: 'Invalid response shape', code: 'RESPONSE_VALIDATION_FAILED'
        })
      );
     }
    return json(payload);
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to fetch cases', code: 'FETCH_FAILED', details: err.message
      })
    ); };
/*
 * POST /api/v1/cases
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json(
      {
        ...
        data: createdCase;
        meta: {
          caseId: userId: getUserId(locals), timestamp: new Date().toISOString(), synthesisQueued: true
         }
      }, { status: 201  }
    );
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid case data', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to create case', code: 'CREATE_FAILED', details: err.message
      })
    ); };
