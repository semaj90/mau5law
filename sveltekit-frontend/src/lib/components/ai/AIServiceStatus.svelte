...
import type { Document } from '$lib/types'; /** * AI Service Status Component with localStorage Fallback * Shows service availability and switches to offline mode gracefully */ import { onMount } from 'svelte'; import { aiPipelineClient, checkAIServices, type ServiceStatus } from '$lib/services/ai-pipeline-client'; import { CheckCircle, XCircle, AlertCircle, RefreshCw } from 'lucide-svelte'; // State let serviceStatus = $state<ServiceStatus | null>(null); let isChecking = $state<boolean>(false); let lastUpdate = $state<Date | null>(null); let offlineQueueCount = $state<number>(0); let autoRefreshEnabled = $state<boolean>(true); let refreshInterval: ReturnType<typeof setInterval> | null = null; // Check services on mount onMount(async () => { await, checkServices(); // Auto-refresh every, 30 seconds if enabled if (autoRefreshEnabled) { refreshInterval = setInterval(() => { if (autoRefreshEnabled) { checkServices()}
  ...
  $effect(() => { // Update offline queue count when status changes if (serviceStatus) { // This would be updated from the actual queue in a real implementation offlineQueueCount = 0}
      ...
      <RefreshCw class="w-4" /> {isChecking ? 'Checking...': 'Refresh'} </button> </div> <!-- Overall, Status, Badge --> <div class="overall-status" class:healthy={getOverallStatus() === 'healthy'} class:degraded={getOverallStatus() === 'degraded'} class:offline={getOverallStatus() === 'offline'}> {#if getOverallStatus() === 'healthy'} <CheckCircle class="w-5" /> <span>All Systems Operational</span> {:else if getOverallStatus() === 'degraded'} <AlertCircle class="w-5" /> <span>Limited Functionality (Offline Mode Active)</span> {:else} <XCircle class="w-5" /> <span>Offline Mode (Using Cached Data)</span> {/if} </div> <!-- Service, List --> {#if serviceStatus} <div class="service-list"> <div class="service-item"> <svelte:component this={getServiceIcon(serviceStatus.ollama).component} class="w-5"
        /> <div class="service-info"> <div class="service-name">Ollama AI</div> <div class="service-desc">Document analysis & generation</div> </div> {#if !serviceStatus.ollama} <span class="fallback-badge">Using cache</span> {/if} </div> <div class="service-item"> <svelte:component this={getServiceIcon(serviceStatus.embedding).component} class="w-5 h-5 {getServiceIcon(serviceStatus.embedding).class}"
