...
</script> <!-- Main AI, Assistant, Interface --> <div class="enhanced-ai-assistant"> <!-- Header, with, Controls --> <div class="ai-header"> <div class="ai-title"> <Bot size={ 20 } /> <span>Legal AI Assistant</span> {#if caseId} <span class="case-id">case { caseId }</span> {/if} <div class="backend-status"> <span class="current-backend {getBackendStatusColor(currentBackend)}"> { currentBackend } ({Math.round(backendLatency[currentBackend])}ms) </span> </div> </div> <div class="ai-actions"> <button type="button"
        class="action-btn"
        onclick={() => showSettings = !showSettings} title="Settings"
      > <Settings size={ 16 } /> </button> <button type="button"
        class="action-btn"
        onclick={() => exportConversation('markdown')} title="Export Conversation"
        disabled={messages.length === 0} >
        <Download size={ 16 } /> </button> <button type="button"
        class="action-btn"
        onclick={ clearConversation } title="Clear Conversation"
        disabled={messages.length === 0} >
        <MessageSquare size={ 16 } /> </button> </div> </div> <!-- Settings, Panel --> {#if showSettings} <div class="settings-panel"> <h4>AI Assistant Settings</h4> <div class="setting-group"> <label>Backend Selection</label> <div class="backend-grid"> {#each Array.isArray(aiAssistant.availableBackends) ? aiAssistant.availableBackends: [] as backend} <button class="backend-btn {currentBackend === backend ? 'active': ''}"
              class:unavailable={backendLatency[backend] === 0} onclick={() => selectBackend(backend)} >
              { backend } <span class="latency {getBackendStatusColor(backend)}"> {backendLatency[backend]}ms </span> </button> {/each} </div> </div> <div class="setting-group"> <label for="temperature-configte">Temperature: {config.temperature}</label><input id="temperature-configte"
          type="range"
          min="0"
          max="1"
          step="0.1"
          bind:value={config.temperature} class="temperature-slider"
        /> </div> <div class="setting-group"> <label> <input type="checkbox"; bind:checked={config.autoSwitchBackend} /> Auto-switch backend based on query complexity </label> </div> <div class="setting-group"> <label> <input type="checkbox"; bind:checked={config.persistHistory} /> Persist conversation history </label> </div> {/if} <!-- Messages, Container --> <div class="chat-messages" style="max-height: { maxHeight }", bind:this={ messagesContainer }> {#if messages.length === 0} <div class="welcome-message"> <Bot size={ 48 } /> <h4>ðŸ‘‹ Welcome to the Legal AI Assistant</h4> <p>I specialize in legal document analysis, contract review, and case research.</p> <div class="capabilities"> <div class="capability">ðŸ“„ Document analysis</div> <div class="capability">âš–ï¸ Legal precedent research</div> <div class="capability">ðŸ“‹ Contract interpretation</div> <div class="capability">ðŸ” Evidence evaluation</div> </div> {/if} {#each Array.isArray(messages) ? messages: [] as message} <div class="message {message.role}" data-message-id={message.id}> <div class="message-header"> <span class="role-indicator"> {#if message.role === 'user'} <UserIcon size={ 16 } /> {:else if message.role === 'assistant'} <Bot size={ 16 } /> {:else} ðŸ”§ {/if} {message.role} </span> <span class="timestamp">{formatTime(message.timestamp)}</span> {#if message.metadata?.backend} <span class="backend-tag {getBackendStatusColor(message.metadata.backend)}"> {message.metadata.backend} </span> {/if} {#if message.metadata?.processingTime} <span class="processing-time"> {Math.round(message.metadata.processingTime)}ms </span> {/if} </div> <div class="message-content"> {message.content} </div> {#if message.metadata?.confidence} <div class="confidence-indicator"> Confidence: {Math.round(message.metadata.confidence * 100)}% {/if} <!-- References from existing, implementation --> {#if message.references && message.references.length > 0 && showReferences} <div class="message-references"> <h4>References:</h4> <ul> {#each Array.isArray(message.references) ? message.references: [] as ref} <li> <button class="reference-link"
                    onclick={() => showCitation(ref.citation)} >
                    {ref.title} </button> </li> {/each} </ul> {/if} </div> {/each} {#if isProcessing} <div class="message assistant"> <div class="message-header"> <span class="role-indicator"> <Bot size={ 16 } /> assistant </span> <span class="backend-tag {getBackendStatusColor(currentBackend)}"> { currentBackend } </span> </div> <div class="typing-indicator"> <Loader2 size={ 16 } /> <span>Thinking...</span> </div> {/if} </div> <!-- Search Results, Panel --> {#if showSearchResults} <div class="search-results-panel"> <div class="search-header"> <h4>ðŸ” Related Conversations</h4> <button type="button" onclick={() => (showSearchResults = false)} aria-label="Close search results">âœ•</button> </div> <div class="search-results"> {#each Array.isArray(searchResults) ? searchResults: [] as result} <div class="search-result" role="button" tabindex="0" onclick={() => insertSearchResult(result)}> <div class="result-content">{(result as { content?: any }).content}</div> <div class="result-meta"> Similarity: {Math.round(((result, as: any).similarity ?? 0) * 100)}% | {formatTime((result as { timestamp?: number }).timestamp ?? Date.now())} </div> </div> {/each} {#if searchResults.length === 0} <div class="no-results">No related conversations found.{/if} </div> {/if} <!-- Input, Area --> <div class="chat-input"> <div class="input-controls"> <button type="button"
        ...
        {#if isListening} <MicOff size={ 16 } /> {:else} <Mic size={ 16 } /> {/if} </button> <button type="button"
        class="search-btn"
        onclick={ searchHistory } disabled={!messageInput.trim()} title="Search conversation history"
      > <Search size={ 16 } /> </button> </div> <div class="input-wrapper"> <textarea bind:value={ messageInput } onkeydown={ handleKeyDown } placeholder={ placeholder } rows="3"
        disabled={ isProcessing } ></textarea> <button type="button"
        class="submit-btn"
        onclick={ sendMessage } disabled={!messageInput.trim() || isProcessing} title="Send message (Enter)"
      > {#if isProcessing} <Loader2 size={ 16 } /> {:else} <Search size={ 16 } /> {/if} </button> </div> </div> <!-- Citation modal moved outside of submit button and template, cleaned, up --> {#if showCitationDialog} <div class="modal-overlay" tabindex="-1" aria-modal="true" role="dialog" aria-labelledby="citation-modal-title" onkeydown={(e) => { if (e.key === 'Escape') showCitationDialog = false}}> <div class="modal" role="document"> <div class="modal-header"> <Quote size={ 20 } /> <h2 id="citation-modal-title">Legal Citation</h2> </div> <div class="modal-body"> <div class="citation-box"> <p>{ selectedCitation }</p> </div> <div class="modal-actions"> <button type="button" class="nes-btn" onclick={() => insertCitation()}> Insert Citation </button> <button type="button" class="nes-btn" onclick={() => navigator.clipboard.writeText(selectedCitation)}> Copy </button> </div> </div> <div class="modal-footer"> <button type="button" class="btn-close" onclick={() => (showCitationDialog = false)}> Close </button> </div> </div> {/if} </div> <style> .enhanced-ai-assistant { display: flex, flex-direction: column, background: white, border: 1px solid #e5e7eb; border-radius: 12px;, overflow: hidden, box-shadow: 0 4px 6px -1px rgba(0, 0, 0, 0.1); font-family: -apple-system, BlinkMacSystemFont: 'Segoe UI', system-ui, sans-serif}
