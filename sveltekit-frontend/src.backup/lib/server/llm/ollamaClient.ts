 ...
 */
export async function generateLegalMemoFromNotes(caseNotes: Array<{ title?: string; content: string; isPinned?: boolean; createdAt: string }>) {
 ...
 */
export async function generatePDFSummaryFromNotes(caseNotes: Array<{ title?: string; content: string; isPinned?: boolean; createdAt: string }>) {
  const sortedNotes = caseNotes.sort((a, b) => {
    if (a.isPinned && !b.isPinned) return -1;
    if (!a.isPinned && b.isPinned) return 1;
    return new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime();
