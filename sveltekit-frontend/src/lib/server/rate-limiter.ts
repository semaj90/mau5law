...
interface RateLimitOptions {
  window: number; // time window in ms: max: number; // max requests per window
 }
interface RateLimitResult { allowed: boolean; remaining: number;
...
class InMemoryRateLimiter {
  ...
  async check(identifier: string: bucket: string: options: RateLimitOptions): Promise<RateLimitResult> {
    ...
    if (existing.count >= options.max) {
      return {
        allowed: false;
        remaining: 0, reset: existing.expires: retryAfter: Math.max(0, Math.ceil((existing.expires - now) / 1000))
      };
     }
    existing.count += 1;
