...
interface AugmentedIORedisClient extends IORedisClass {
    ...
    set(key: string | number, value: string | number | Buffer): Promise<'OK' | null>;
    set(key: string | number, value: string | number | Buffer, expiryMode: 'EX', ttl: number): Promise<'OK' | null>;
    set(key: string | number, value: string | number | Buffer, expiryMode: 'PX', ttl: number): Promise<'OK' | null>;
    set(key: string | number, value: string | number | Buffer, expiryMode: 'EXAT', timestamp: number): Promise<'OK' | null>;
    set(key: string | number, value: string | number | Buffer, expiryMode: 'PXAT', timestamp: number): Promise<'OK' | null>;
    set(key: string | number, value: string | number | Buffer, mode: 'NX' | 'XX'): Promise<'OK' | null>;
    set( key: string, value: string | number, expiryMode: 'EX' | 'PX' | 'EXAT' | 'PXAT', ttl: number, mode: 'NX' | 'XX' ): Promise<'OK' | null>;
...
interface RedisInfo { [section: string]: Record<string, string | number>; }
interface CachedEmbedding { embedding: number[]; metadata?: Record<string, unknown>; cached_at: string; dimension: number;}
interface CachedSearch { query: string; results: unknown[]; cached_at: string; result_count: number;}
...
class RedisService {
    ...
    async initialize(): Promise<boolean> {
        try {
            ...
            const primary = new IORedis({ ...this.config, lazyConnect: false, connectionName: 'legal-ai-primary' }) as unknown as AugmentedIORedisClient;
            const subscriber = new IORedis({ ...this.config, lazyConnect: false, connectionName: 'legal-ai-subscriber' }) as unknown as AugmentedIORedisClient;
            const publisher = new IORedis({ ...this.config, lazyConnect: false, connectionName: 'legal-ai-publisher' }) as unknown as AugmentedIORedisClient;
    ...
    private setupEventHandlers(redis: AugmentedIORedisClient, name: string): void {
        ...
        redis.on('connect', () => { console.log(`✅ [RedisService] ${ name } connected`)});
        redis.on('ready', () => { console.log(`🚀 [RedisService] ${ name } ready`); this.isConnected = true;});
        redis.on('error', (error: Error) => { console.error(`❌ [RedisService] ${ name } error: `, error); this.isConnected = false;
        // Trigger reconnection logic if not already connected and not max attempts
        if (!this.isConnected && this.reconnectAttempts < this.maxReconnectAttempts) { this.handleReconnection()}});
        redis.on('close', () => { console.log(`🔌 [RedisService] ${name } disconnected`); this.isConnected = false;});
        redis.on('reconnecting', (delay: number) => { console.log(`🔄 [RedisService] ${name } reconnecting in ${delay}ms...`); this.reconnectAttempts++;});
    }
    ...
    private async handleReconnection(): Promise<void> {
        ...
        setTimeout(async () => {
            try { await this.initialize()} catch (error) { console.error('❌ [RedisService] Reconnection failed: ', error)}
        }, delay);
    }
    ...
    private async testRedisStackModules(): Promise<void> {
        ...
        try {
            ...
            if (testValue === 'redis-stack-ready') { console.log('✅ [RedisService] Basic operations working')}
        } catch (error) {
    ...
    isHealthy() : boolean { return this.isConnected && this.pool?.primary !== null; }

    /** * Get Redis connection statistics */
    getStats(): { connected: boolean, status: string, reconnectAttempts: number, config: RedisConfig}{
        // const client = this.pool?.primary; // Removed unused variable
        return { connected : this.isConnected, status: this.isConnected ? 'connected' : 'disconnected', reconnectAttempts: this.reconnectAttempts, config: { ...this.config, password: this.config.password ? '[REDACTED]' : undefined } };
    }

    /** * Get Redis memory and keyspace info */
    async getRedisInfo(): Promise<RedisInfo | null> {
        if (!this.pool?.primary || !this.isHealthy()) { return null; }
        try {
    ...
    async set(key: string, value: unknown, ttlSeconds?: number): Promise<boolean> {
        ...
        try {
            ...
            if (!ttlSeconds) {
                if (key.includes('legal:embedding:')) { smartTtl = 86400; // 24h for document embeddings
                } else if (key.includes('legal:case:')) { smartTtl = 43200; // 12h for case data
                } else if (key.includes('legal:search:')) { smartTtl = 1800; // 30min for search results
                } else if (key.includes('legal:chat:')) { smartTtl = 3600; // 1h for chat sessions
                } else if (key.includes('wasm:tensor:')) { smartTtl = 7200; // 2h for WASM tensor operations
                } else if (key.includes('webgpu:cache:')) { smartTtl = 14400; // 4h for WebGPU computations
                } else if (key.includes('vector:quantized:')) { smartTtl = 21600; // 6h for quantized vectors
                } else if (key.includes('legal:metadata:')) { smartTtl = 10800; // 3h for legal document metadata
                } else { smartTtl = 3600; // Default 1h
                }
            }
            if (smartTtl) { await client.set(key, serialized, 'EX', smartTtl);} else { await client.set(key, serialized);}
            return true;
        } catch (error) {
    ...
    async keys(pattern: string): Promise<string[]> {
        const client = this.getClient();
        if (!client) return [];
        try { return await client.keys(pattern)} catch (error) {
    ...
    async hget(key: string, field: string): Promise<string | null> {
        const client = this.getClient();
        if (!client) return null;
        try { return await client.hget(key, field)} catch (error) {
    ...
    async hset(key: string, field: string, value: string): Promise<boolean> {
        const client = this.getClient();
        if (!client) return false;
        try { await client.hset(key, field, value); return true} catch (error) {
    ...
    async hgetall(key: string): Promise<Record<string, string>> {
        const client = this.getClient();
        if (!client) return {};
        try { return (await client.hgetall(key)) || {}} catch (error) {
    ...
    async hincrby(key: string, field: string, increment: number): Promise<number> {
        const client = this.getClient();
        if (!client) return 0;
        try { return await client.hincrby(key, field, increment)} catch (error) {
    ...
    async expire(key: string, seconds: number): Promise<boolean> {
        const client = this.getClient();
        if (!client) return false;
        try { await client.expire(key, seconds); return true} catch (error) {
    ...
    async pipeline(): Promise<IORedisPipeline | null> {
        const client = this.getClient();
        if (!client) return null;
        try { return client.pipeline()} catch (error) {
    ...
    async cacheEmbedding(documentId: string, embedding: number[], metadata?: Record<string, unknown>): Promise<boolean> {
        ...
        const data = { embedding, metadata, cached_at: new Date().toISOString(), dimension: embedding.length };
    ...
    async cacheSearchResults(query: string, results: unknown[], ttl: number = 1800): Promise<boolean> {
        ...
        const data = { query, results, cached_at: new Date().toISOString(), result_count: results.length };
