...
export const GET: RequestHandler = async () => {
    ...
    const healthyCount = serviceStatuses.filter(item => item === 'healthy').length;
    const totalServices = serviceStatuses.length;

    if (healthyCount === totalServices && totalServices > 0) {
    ...
    healthData.summary = {
        ...
        unavailable_services: serviceStatuses.filter(item => item === 'unavailable' || item === 'unhealthy').length,
    ...
    const httpStatus = healthData.overall === 'healthy' ? 200 : healthData.overall === 'degraded' ? 207 : 503;
