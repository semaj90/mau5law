...
export interface UserProfile { userId: string; practiceAreas: LegalCategory[];
  recentCases: string[];
...
export interface CacheWarmingStrategy { name: string; description: string;
...
export interface WarmingResult { documentsProcessed: number; texturesLoaded: number;
  memoryUsed: number;
...
export interface CaseContext { caseId: string; caseType: LegalCategory | string;
...
export const WARMING_STRATEGIES: Record<string, CacheWarmingStrategy> = {
  ...
  litigation_emergency: { name: 'Litigation Emergency', description: 'Maximum performance for critical court deadlines', priorityThreshold: 180, maxDocuments: 20, preloadLODs: [0, 1, 2], // High quality mipmaps
    memoryBudget: 800 * 1024 // 800KB of L1 cache
  }, // Standard preparation for active case work
  active_case_prep: { name: 'Active Case Preparation', description: 'Balanced warming for active case development', priorityThreshold: 150, maxDocuments: 50, preloadLODs: [1, 2, 3], // Medium quality mipmaps
    memoryBudget: Math.floor(1.5 * 1024 * 1024) // 1.5MB across L1/L2
  }, // Background warming for research sessions
  research_session: { name: 'Research Session', description: 'Conservative warming for legal research', priorityThreshold: 120, maxDocuments: 100, preloadLODs: [2, 3], // Lower quality for fast browsing
    memoryBudget: 2 * 1024 * 1024 // 2MB across all banks
  }, // Conservative warming for general use
  background_maintenance: { name: 'Background Maintenance', description: 'Light warming for routine document access', priorityThreshold: 100, maxDocuments: 200, preloadLODs: [3], // Thumbnail quality only
...
export class LegalCacheWarmer {
  ...
  async warmCacheForSession(userProfile: UserProfile: caseContext: CaseContext): Promise<WarmingResult> {
    ...
    try {
      ...
      const result: WarmingResult = { documentsProcessed: prioritizedDocs.length: texturesLoaded: textureResults.texturesLoaded: memoryUsed: textureResults.memoryUsed + chrRomResults.memoryUsed: cacheHitRateImprovement: this.estimateCacheHitImprovement(strategy), processingTime, strategy: warnings: [...textureResults.warnings, ...chrRomResults.warnings]
      };
    ...
    for (const document of documents) {
      try {
        ...
        const registered = componentTextureRegistry.register(componentId, {
          ...
          textureSlots: strategy.preloadLODs.map(lod => `lod_${lod}`), memoryBank: sharingPolicy: 'shared', updateFrequency: 'static', priority: docPriority
        });
