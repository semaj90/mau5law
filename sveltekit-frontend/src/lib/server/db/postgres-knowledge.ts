...
export async function insertKnowledgeDocument(
  ...
  try {
    ...
    const insertedId = pgRows<{ id: number }>(result)[0]?.id;
...
export async function updateKnowledgeDocument(
  ...
  try {
    ...
    if (updates.content !== undefined) {
      setParts.push(sql`content = ${updates.content}`);
    }
    ...
    if (updates.metadata !== undefined) {
      setParts.push(sql`metadata = ${updates.metadata}`);
    }
    if (updates.couchdb_id !== undefined) {
      setParts.push(sql`couchdb_id = ${updates.couchdb_id}`);
    }
    if (updates.qdrant_id !== undefined) {
      setParts.push(sql`qdrant_id = ${updates.qdrant_id}`);
    }
    if (updates.blob_url !== undefined) {
      setParts.push(sql`blob_url = ${updates.blob_url}`);
    }
    if (updates.blob_metadata !== undefined) {
      setParts.push(sql`blob_metadata = ${updates.blob_metadata}`);
    }
...
export async function searchByEmbedding(
  ...
  try {
    ...
    return pgRows(result).map((row: any) => ({
      id: row.id,
      title: row.title,
      content: row.content,
...
export async function getRelatedDocuments(
  ...
  try {
    ...
    return pgRows(result) as unknown as KnowledgeDocument[];
  } catch (error) {
...
export async function postgresHealthCheck(): Promise<boolean> {
  try {
    ...
    return pgRows(result)[0]?.health === 1;
  } catch {
    return false;
  }
}
