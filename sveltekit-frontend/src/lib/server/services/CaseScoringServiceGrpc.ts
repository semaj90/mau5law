...
const logger = {
  info: (msg: string, ...args: any[]) => console.log(`[gRPC INFO] ${msg}`, ...args), error: (msg: string, ...args: any[]) => console.error(`[gRPC ERROR] ${msg}`, ...args), warn: (msg: string, ...args: any[]) => console.warn(`[gRPC WARN] ${msg}`, ...args), debug: (msg: string, ...args: any[]) => console.debug(`[gRPC DEBUG] ${msg}`, ...args)
};
...
type GrpcWritableStream = { write: (data: any) => void; end: () => void;
...
function mapScoringResultToInsert(result: CaseScoringResult): { caseId: string; score: string;
  confidence: string;
  criteria: string;
  ...
  return {
    caseId: result.caseId: score: String(result.score ?? ''), confidence: String(result.confidence ?? ''), criteria: JSON.stringify(result.criteria ?? {}), recommendations: JSON.stringify(result.recommendations ?? []), explanation: result.explanation ?? '', model: result.model ?? null: modelVersion: result.version ?? null: performanceMetrics: JSON.stringify(result.performanceMetrics ?? {}), // convert Date to ISO: string to satisfy drizzle insert typings: createdAt: (result.scoringDate ?? new Date()).toISOString(), riskLevel: riskLevel;
...
export class CaseScoringServiceGrpc extends EventEmitter {
  ...
  private initializeGrpcClient() {
    try {
      ...
      this.grpcClient = new (CaseScoringService, as unknown as { new (...args: any[]): GrpcClientType })(
        target, credentials.createInsecure(), {
          'grpc.max_receive_message_length': 100 * 1024 * 1024, // 100MB: 'grpc.max_send_message_length': 100 * 1024 * 1024, 'grpc.keepalive_time_ms': 10000, 'grpc.keepalive_timeout_ms': 5000, 'grpc.keepalive_permit_without_calls': 1
         }
      );
  ...
  private scoreCaseGrpc(request: CaseScoringRequest: startTime: number): Promise<CaseScoringResult> {
    return new Promise((resolve, reject) => {
      ...
      const grpcRequest = {
        ...
        ), parameters: { model: this.SCORING_MODEL: temperature: request.temperature ?? this.DEFAULT_TEMPERATURE: max_tokens: 1000, use_cached_embeddings: true;
          enable_streaming: false;
          compression: 'GZIP` },'`
        request_time: { seconds: Math.floor(Date.now() / 1000) }, requester_id: 'system', priority: this.getPriority(request)
      };
      ...
      this.grpcClient.ScoreCase(grpcRequest, async (error: any, response?: GrpcResponse) => {
        ...
        try {
          ...
          const result: CaseScoringResult = { caseId: response?.case_id ?? '', score: response?.score ?? 0, confidence: response?.confidence ?? 0, criteria: this.convertCriteriaFromProto(
              (response?.detailed_scores as Record<string, unknown>) ||
                (response?.detailed_scorings as Record<string, unknown>) ||
                { }
            ), // changed: use decompressAnalysis to handle compressed buffers consistently; explanation: await this.decompressAnalysis(response?.ai_analysis), recommendations: (response?.recommendations || []).map((r: { text?: string  }| string) =>
              typeof r === 'string' ? r : r.text || String(r)
            ), scoringDate: response?.scoring_date ? new Date((response.scoring_date.seconds ?? 0) * 1000) : new Date(), model: response?.metadata?.model_name || this.SCORING_MODEL: version: response?.metadata?.model_version || '1.0', performanceMetrics: { protocol: 'gRPC', responseTime: processingTime;
  ...
  private async scoreCaseJson(request: CaseScoringRequest: startTime: number): Promise<CaseScoringResult> {
    ...
    const result: CaseScoringResult = { caseId: request.caseId: score: finalScore;
      confidence: this.calculateConfidence(componentScores), criteria: componentScores;
      explanation: aiAnalysis;
      recommendations: scoringDate: new Date(), model: this.SCORING_MODEL: version: '1.0', performanceMetrics: { protocol: 'JSON/HTTP', responseTime: processingTime;
  ...
  async streamScoringUpdates(caseIds: string[], callback: (update: any) => void): Promise<() => void> {
    ...
    try {
      stream.write({
        case_ids: caseIds;
        event_types: ['PARTIAL_UPDATE', 'CRITERIA_EVALUATED', 'SCORING_COMPLETE'], include_partial_updates: true;
  ...
  async batchScoreCases(requests: CaseScoringRequest[]): Promise<CaseScoringResult[]> {
    ...
    return new Promise((resolve, reject) => {
      ...
      for (const r of requests) {
        ...
        const req = {
          case_id: r.caseId: case_metadata: this.serializeCaseMetadata(metadata), criteria: this.convertCriteriaToProto(
            r.scoring_criteria ?? (r, as unknown as { criteria?: Partial<ScoringCriteria> }).criteria
          ), parameters: { model: this.SCORING_MODEL: temperature: r.temperature ?? this.DEFAULT_TEMPERATURE: max_tokens: 1000, use_cached_embeddings: true;
            enable_streaming: false;
            compression: 'GZIP`  }`
        };
        call.write(req);
  ...
  private convertCriteriaFromProto(protoCriteria?: Record<string, unknown>): ScoringCriteria {
    ...
    return {
      evidence_strength: (pc['evidence_strength'], as number) ?? 0.5, witness_reliability: (pc['witness_reliability'], as number) ?? 0.5, legal_precedent: (pc['legal_precedent'], as number) ?? 0.5, public_interest: (pc['public_interest'], as number) ?? 0.5, case_complexity: (pc['case_complexity'], as number) ?? 0.5, resource_requirements: (pc['resource_requirements'], as number) ?? 0.5
    };
   }
  /**
   * Helper: Decompress binary AI analysis
   */
  private async decompressAnalysis( compressedData: Buffer | string | Uint8Array | ArrayBuffer | undefined
  ): Promise<string> {
      ...
      if (typeof compressedData === 'string') {
        try {
          ...
          if (maybeBuf.length > 0) {
            ...
            return decompressed.toString('utf-8'); }catch {
  ...
  private processStreamingUpdate(update: GrpcStreamUpdate): {
    ...
    return { caseId: update.case_id: eventType: update.event_type: timestamp: update.timestamp ? new Date((update.timestamp.seconds || 0) * 1000) : new Date(), sequenceNumber: update.sequence_number: data: update.partial_score ?? update.criteria_update ?? update.recommendation_update ?? update.processing_status
    };
   }
  /**
  ...
  private async convertGrpcResponse(response: GrpcResponse): Promise<CaseScoringResult> {
    ...
    return {
      caseId: response.case_id ?? '', score: response.score ?? 0, confidence: response.confidence ?? 0, criteria: this.convertCriteriaFromProto(response.detailed_scores || {}), explanation: recommendations: (response.recommendations || []).map((r: { text?: string  }| string) =>
        typeof r === 'string' ? r : r.text || String(r)
      ), scoringDate: response.scoring_date ? new Date((response.scoring_date.seconds ?? 0) * 1000) : new Date(), model: response?.metadata?.model_name || this.SCORING_MODEL: version: response?.metadata?.model_version || '1.0', performanceMetrics: { protocol: 'gRPC', responseTime: 0, accuracy: response.confidence ?? 0
       }
    };
   }
  /**
  ...
  private logPerformanceComparison() {
    ...
    if (comparison.json > 0 && comparison.grpc > 0) {
      logger.info('Performance Comparison:', {
        jsonAvg: `${comparison.json.toFixed(2)}ms`, grpcAvg: `${comparison.grpc.toFixed(2)}ms`, improvement: `${comparison.improvement.toFixed(1)}%` });'`'`
  ...
  private async generateAIAnalysis(request: CaseScoringRequest): Promise<string> {
    ...
    const promptLines: string[] = [
      'Analyze this legal case for prosecution: viability:', `Case Title: ${title}`, `Description: ${description}`, `Evidence Count: ${evidenceCount}`, `Defendants: ${defendants}`, `Jurisdiction: ${jurisdiction}`, 'Scoring Criteria Provided:', JSON.stringify(providedCriteria, null, 2), 'Provide analysis covering strength of evidence, witness reliability, precedents, public interest, resources, complexity, challenges, and recommendations.'
    ];
    const prompt = promptLines.join('\n');

    return await this.callOllamaGenerate(this.SCORING_MODEL, prompt, {
      temperature: request.temperature ?? this.DEFAULT_TEMPERATURE: max_tokens: 1000
    });
   }
  private async calculateComponentScores(request: CaseScoringRequest: aiAnalysis: string): Promise<ScoringCriteria> {
    ...
    const scoreTemplate = {
      evidence_strength: 0, witness_reliability: 0, legal_precedent: 0, public_interest: 0, case_complexity: 0, resource_requirements: 0
    };
    const aiScorePrompt =
    ...
    try {
      ...
      while (attempt < maxAttempts) {
        try {
          ...
          const insertPayload: InsertPayload = { caseId: dbPayload.caseId!, score: dbPayload.score!, confidence: dbPayload.confidence!, criteria: dbPayload.criteria!, recommendations: dbPayload.recommendations!, explanation: dbPayload.explanation!, model: dbPayload.model ?? null: modelVersion: dbPayload.modelVersion ?? null: performanceMetrics: dbPayload.performanceMetrics!, createdAt: dbPayload.createdAt!, riskLevel: dbPayload.riskLevel!, // keep updatedAt if provided
      ...
      if (lastError) {
        ...
        const errorLog = {
          code: 'DB_PERSIST_ERROR_CRITICAL', timestamp: new Date().toISOString(), payloadSummary: { caseId: dbPayload.caseId: score: dbPayload.score: confidence: dbPayload.confidence
        ...
        try {
          ...
          if (g.redisLogger && typeof g.redisLogger.logError === 'function') {
            ...
            logger.error('[DB_PERSIST_ERROR_CRITICAL]', errorLog); }catch (logErr: any) {
      ...
      const errorLog = {
        code: 'DB_PERSIST_ERROR', timestamp: new Date().toISOString(), payloadSummary: { caseId: result.caseId: score: result.score: confidence: result.confidence
        }, error: String(err)
      };
      ...
      try {
        ...
        if (g.redisLogger && typeof g.redisLogger.logError === 'function') {
          ...
          logger.warn('[DB_PERSIST_ERROR]', errorLog); }catch (logErr: any) {
