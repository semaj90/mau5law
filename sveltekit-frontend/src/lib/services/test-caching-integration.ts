...
export class CachingIntegrationTester {
  ...
  async runAllTests(): Promise<CacheTestResult[]> {
    console.log('🧪 Starting comprehensive caching integration tests...');
    this.testResults = [];
    // Test 1: Embedding cache functionality
    await this.testEmbeddingCache();
    // Test 2: Batch embedding cache
    await this.testBatchEmbeddingCache();
    // Test 3: Query result cache
    await this.testQueryCache();
    // Test 4: Full RAG pipeline with caching
  ...
  private async testEmbeddingCache(): Promise<void> {
    const testName = 'Embedding Cache Test';
    const startTime = Date.now();
    try {
      console.log('🔬 Testing embedding cache...');
      const testText = "This is a test legal document about contract law and breach of contract.";
   ...
   */
  private async testBatchEmbeddingCache(): Promise<void> {
    const testName = 'Batch Embedding Cache Test';
    const startTime = Date.now();
    try {
   ...
   */
  private async testQueryCache(): Promise<void> {
    const testName = 'Query Cache Test';
    const startTime = Date.now();
    try {
      console.log('🔬 Testing query cache...');
      const testQuery = "What constitutes breach of contract?";
   ...
   */
  private async testFullRAGPipeline(): Promise<void> {
    const testName = 'Full RAG Pipeline Test';
    const startTime = Date.now();
    try {
  ...
  private async testCacheMetrics(): Promise<void> {
    const testName = 'Cache Metrics Test';
    const startTime = Date.now();
    try {
  ...
  private async testDocumentIngestion(): Promise<void> {
    const testName = 'Document Ingestion Test';
    const startTime = Date.now();
    try {
  ...
  private async testCacheWarmup(): Promise<void> {
    const testName = 'Cache Warmup Test';
    const startTime = Date.now();
    try {
  ...
  private generateTestSummary() {
    const successful = this.testResults.filter(item => item.length);
