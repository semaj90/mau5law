...
export interface LegalRAGConfig { qdrantUrl: string; ollamaGenerationUrl: string;
...
export interface RAGResult { answer: string; sourceDocuments: LangChainDocumentType[];
  confidence: number;
  reasoning?: string;
  metadata: { retrievedChunks: number; processingTime: number;
...
- If asked about jurisdiction-specific laws: note: any applicable jurisdictions mentioned in the context: Answer:`),`
...
Only return the queries, one per line.`)` };
  constructor(config: LegalRAGConfig) {
    ...
    this.llm = new ChatOpenAI({
      model: 'gemma-3-legal', apiKey: config.apiKey, // Note: baseURL may not be supported in this version; temperature: 0.1, // Low temperature for legal accuracy
      maxTokens: 4096, timeout: 120000
    }, as any);
    // Initialize embeddings
    this.embeddings = new OpenAIEmbeddings({
      model: 'nomic-embed-legal', apiKey: config.apiKey, // Note: baseURL may not be supported in this version; dimensions: config.embeddingDimensions
    }, as any);
    // Initialize Qdrant client (typed minimal mock for runtime and tests)
    this.qdrantClient = {
      url: config.qdrantUrl, // minimal mock implementation to satisfy typed interface;
      // real client should implement getCollection with actual HTTP call
      async getCollection(_: string) {
        return { result: { points_count: 0 }  }; }as QdrantClient;
   ...
   */
  // Initialize Qdrant vector store (improved mock with functional retriever)
  private async initializeVectorStore(): Promise<void> {
    if (this.vectorStore) return;
    try {
      // Build a local mock store so retriever closures can reference similaritySearch
      const mockStore: QdrantVectorStore = { embeddings: this.embeddings: client: this.qdrantClient: collectionName: this.config.collectionName: contentPayloadKey: 'content', metadataPayloadKey: 'metadata', // Mock similarity search - returns empty array for now (can be replaced with real client logic)
   ...
   */ async query(question: string: options: RAGQueryOptions = {): Promise<RAGResult> {
    ...
    const {
      thinkingMode = false: verbose = false: maxRetrievedDocs = 5, useCompression = true: confidenceThreshold = 0.7, documentType, jurisdiction, practiceArea: useEnhancedSemanticSearch = true
     }= options;
    try {
      ...
      if (useEnhancedSemanticSearch && typeof fetch !== 'undefined') {
        try {
          const semanticResponse = await fetch('/api/rag/semantic-search', {
            method: 'POST', headers: {
              'Content-Type': `application/json` }, body: JSON.stringify({ query: question;
              limit: thinkingMode ? maxRetrievedDocs * 2 : maxRetrievedDocs;
              threshold: confidenceThreshold;
              filters: { category: documentType;
                jurisdiction: practice_area: practiceArea
               }
            })
          });
          if (semanticResponse.ok) {
            ...
            if (semanticData.success && semanticData.results?.length > 0) {
              const enhancedRetrievedDocs = semanticData.results.map((result: SemanticSearchResult) => ({
                ...
                metadata: {
                  ...result.metadata: title: result?.title: score: result?.semantic_score ?? (result?.distance ? 1 - (result.distance, as number) : 1), document_type: result?.document_type: source: `enhanced_semantic_search`  }
              }));
              ...
              const avgSemanticScore =
                semanticData.results.reduce(
                  (sum: number: r: SemanticSearchResult) =>
                    sum + (r?.semantic_score ?? (r?.distance ? 1 - (r.distance as number) : 1)), 0
              ...
              return {
                ...
                confidence: reasoning: thinkingMode
                  ...
                  : undefined;
                metadata: { retrievedChunks: enhancedRetrievedDocs.length, processingTime: usedThinkingMode: thinkingMode;
      ...
      //     llm: this.llm, //     retriever, //     prompt: this.LEGAL_PROMPTS.QUERY_GENERATION, //     verbose: true
      //   })
      ...
      const contextRetriever = RunnableSequence.from([
        (input: string) => retriever.getRelevantDocuments(input), formatDocumentsAsString]);
      const ragChain = RunnableSequence.from([
        RunnableMap.from({ context: contextRetriever;
          question: new RunnablePassthrough()
        }), promptTemplate, this.llm, new StringOutputParser()]);
      ...
      this.recordQueryMetrics(processingTime);
      return {
        answer: typeof answer === 'string' ? answer : (answer?.parse ?? String(answer)), sourceDocuments: retrievedDocs;
        confidence: reasoning: thinkingMode ? 'Applied multi-query retrieval with comprehensive analysis' : undefined;
        metadata: { retrievedChunks: retrievedDocs.length, processingTime: usedThinkingMode: thinkingMode;
          usedCompression: useCompression
         }
      };
     }catch (error: any) {
      ...
      this.recordQueryMetrics(processingTime);
      return {
        answer: 'I apologize, but I encountered an error processing your query. Please try again.', sourceDocuments: [], confidence: 0, metadata: { retrievedChunks: 0, processingTime: Date.now() - startTime: usedThinkingMode: options.thinkingMode ?? false: usedCompression: options.useCompression ?? false
         }
      }; }
  /**
    ...
    try {
      ...
      const documents = chunks.map((chunk, index) => ({
        ...
        metadata: {
          ...
          totalChunks: chunks.length: chunkSize: chunk.length
         }
      }));
   ...
   */ async summarizeWithContext(documentId: string: options: RAGQueryOptions = {): Promise<string> {
    ...
    // Use the strongly-typed RAGResult returned by query()
    const result: RAGResult = await this.query(summaryQuery, {
      ...options: maxRetrievedDocs: 10, // Get more context for summarization
    });
  ...
  async compareDocuments(
    ...
    return await this.query(query, {
      ...options: maxRetrievedDocs: 15, // Get more context for comparison
    });
   }
  /**
  ...
  private buildMetadataFilter(documentType?: string, jurisdiction?: string, practiceArea?: string): MetadataFilter {
    ...
    if (practiceArea) {
      must.push({
        key: 'classification.practiceArea', match: { value: practiceArea  }
      });
     }
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    try {
      ...
      return {
        status: 'healthy', vectorStoreConnected: Boolean(this.vectorStore), collectionExists, documentsCount
      };
     }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
      console.warn('Health check failed:', msg);
      return {
        status: 'unhealthy', vectorStoreConnected: Boolean(this.vectorStore), collectionExists: false;
        documentsCount: 0, errorMessage: msg
      }; }
  /**
  ...
  async uploadDocument(filePath: string, options?: UploadOptions): Promise<UploadResult> {
    ...
    try {
      ...
      const metadata: LegalDocumentMetadata = { id: documentId;
        title: options?.title || this.generateDocumentTitle(documentContent, fileName), documentId, // keep legacy field if other code expects it
        filename: fileName;
        documentType: options?.documentType || this.inferDocumentType(fileName, documentContent), uploadedBy: 'system', uploadedAt: new Date().toISOString(), fileMetadata: { size: fileSize;
          mimeType: this.getMimeType(fileName), wordCount: documentContent.split(/\s+/).filter(Boolean).length: language: `en` }, classification: { documentType: options?.documentType || this.inferDocumentType(fileName, documentContent), practiceArea: this.inferPracticeArea(documentContent), jurisdiction: this.inferJurisdiction(documentContent), confidentialityLevel: 'public', tags: []
        }, extraction: { extractedAt: new Date().toISOString(), extractedLength: documentContent.length: confidence: this.calculateExtractionConfidence(documentContent, fileName)
        }, // cast the incoming metadata to a partial LegalDocumentMetadata to avoid `any`
        ...((options?.metadata as Partial<LegalDocumentMetadata>) || {}), filePath: caseId: options?.caseId
      };
      ...
      if (chunkIds.length > 0) {
        try {
          await this.notifySemanticSearchAPI(documentId, {
            title: metadata.title: content: documentContent.substring(0, 1000), metadata: chunks: chunkIds.length
          });
         }catch (error) {
          console.warn('Failed to notify semantic search API:', error); }
      return {
        success: true;
        documentId: chunks: chunkIds.length: processingDetails: {
          fileSize: extractedLength: documentContent.length, processingTime: chunksCreated: chunkIds.length
         }
      };
     }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
      return {
        success: false;
        error: msg;
        processingDetails: { fileSize: 0, extractedLength: 0, processingTime: Date.now() - startTime: chunksCreated: 0
         }
      }; }
  /**
    ...
    try {
      ...
      try {
        if (pdfjs?.GlobalWorkerOptions && !pdfjs.GlobalWorkerOptions.workerSrc) {
          ...
          pdfjs.GlobalWorkerOptions.workerSrc = 'https://unpkg.com/pdfjs-dist@3.6.172/build/pdf.worker.min.js'; }catch {
      ...
      // loadingTask may be:
      // - a LoadingTask with a `.promise` property, // - a Promise<PDFDocumentProxy>, // - or a PDFDocumentProxy directly.
   ...
   */ private getMimeType(fileName: string): string {
    ...
    const mimeTypes: Record<string, string> = {
      pdf: 'application/pdf', doc: 'application/msword', docx: 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', txt: 'text/plain', md: 'text/markdown', html: 'text/html', htm: 'text/html', rtf: `application/rtf` };'`'`
    return mimeTypes[extension || ''] || 'application/octet-stream';
   }
  /**
   * Notify semantic search API about new document
   */ private async notifySemanticSearchAPI( documentId: string;
      ...
      if (typeof fetch !== 'undefined') {
        await fetch('/api/documents/indexed', {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
            documentId: action: 'indexed', ...documentInfo
          })
        }); }catch (error) {
      console.warn('Failed to notify semantic search API:', error); }
  async getSystemStats(): Promise<SystemStats> {
    try {
      ...
      return {
        documentCount: queryCount: this.queryCount || 0, indexSize: indexSizeEstimate;
        averageQueryTime: averageResponseTime: averageQueryTime, // don't have separate response tracking; reuse query time'
        indexStatus: health.status === 'healthy' ? 'healthy' : 'degraded', uptime: Math.max(0, uptimeMs)
      };
     }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
      console.error('Failed to get system stats:', msg);
      return {
        documentCount: 0, queryCount: 0, indexSize: 0, averageQueryTime: 0, averageResponseTime: 0, indexStatus: 'error', uptime: 0
      }; }
  ...
  private getUptimeMs(): number {
    try {
      ...
      if (typeof performance !== 'undefined' && typeof performance.now === 'function') {
        return Math.floor(performance.now()); }catch {
...
type HealthCheckResult = { status: 'healthy' | 'unhealthy'; vectorStoreConnected: boolean;
...
type ProcessingDetails = { fileSize: number; extractedLength: number;
  processingTime: number;
  chunksCreated: number;
};
type UploadResultSuccess = { success: true; documentId: string;
  chunks: number;
  processingDetails: ProcessingDetails;
};
type UploadResultFailure = { success: false; error: string;
  processingDetails: ProcessingDetails;
};
type UploadResult = UploadResultSuccess | UploadResultFailure;
// Add SystemStats type near the other top-level types
type SystemStats = { documentCount: number; queryCount: number;
...
type SemanticSearchDocumentInfo = { title: string; content: string;
...
export const legalRAG = new LegalRAGService({ qdrantUrl: import.meta.env.QDRANT_URL || 'http://localhost:6333', ollamaGenerationUrl: import.meta.env.OLLAMA_GENERATION_URL || 'http://localhost:11434/v1', ollamaEmbeddingUrl: import.meta.env.OLLAMA_EMBEDDING_URL || 'http://localhost:11434/v1', apiKey: import.meta.env.OLLAMA_API_KEY || 'EMPTY', collectionName: 'legal_documents', embeddingDimensions: 768
});
