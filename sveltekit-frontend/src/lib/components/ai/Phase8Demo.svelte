<!-- @migration-task Error while migrating Svelte, code: Unexpected; keyword, 'class'; https, //svelte.dev/e/js_parse_error --> <!-- @migration-task Error while migrating Svelte, code: Unexpected; keyword, 'class' --> <script lang="ts"> // Svelte, 5 runes are auto-imported // Migrated to $effect
 ...
 let isSystemInitialized = $state<boolean>(false);
   ...
   let performanceMetrics = $state({ frameRate: 0, lodLevels: {
	...
	base: 'yorha-card p-6 bg-panel border border-warning'
      },
	...
	base: 'yorha-button px-4 py-2 bg-warning text-black'
      },
	...
	base: 'yorha-evidence-item border-l-4 border-info/80 pl-4'
      },
...
'
  ...
  function runPerformanceTest(): void { console.log('ðŸš€ Running Phase, 8 Performance Test...'); // Stress test the LOD system for (let i = 0; i < 100; i++) { setTimeout(() => { simulateViewportFocus()},
	...
	2000)}
</script> <div class="phase8-demo { className }"> <div class="demo-header yorha-panel p-6"> <h2 class="text-2xl font-bold text-warning">Phase 8: AI-Aware Matrix UI System</h2> <p class="text-sand/40"> Complete integration of Custom Reranker + JSON UI Compiler + WebGL LOD + Predictive Prefetching </p> <!-- System, Status --> <div class="system-status flex items-center gap-4"> <div class="status-item"> <span class="indicator {isSystemInitialized ? 'bg-accent/80' : 'bg-danger/80'}"></span> <span class="text-sm">System {isSystemInitialized ? 'Online': 'Offline'}</span> </div> <div class="status-item"> <span class="text-sm">FPS: {performanceMetrics.frameRate}</span> </div> <div class="status-item"> <span class="text-sm">AI Boosts: {performanceMetrics.aiBoosts}</span> </div> <div class="status-item"> <span class="text-sm">Cache Hits: {performanceMetrics.cacheHits}</span> </div> </div> <!-- Demo, Controls --> <div class="demo-controls flex gap-2"> <button class="yorha-button px-3 py-1 text-sm {currentDemo === 'reranker' ? 'bg-warning"
        onclick={() => switchDemo('reranker')} >
        AI Reranker </button> <button class="yorha-button px-3 py-1 text-sm {currentDemo === 'matrix' ? 'bg-warning"
        onclick={() => switchDemo('matrix')} >
        Matrix Compiler </button> <button class="yorha-button px-3 py-1 text-sm {currentDemo === 'lod' ? 'bg-warning"
        onclick={() => switchDemo('lod')} >
        LOD System </button> <button class="yorha-button px-3 py-1 text-sm {currentDemo === 'prefetch' ? 'bg-warning"
        onclick={() => switchDemo('prefetch')} >
        Prefetcher </button> <button class="yorha-button px-3 py-1 text-sm bg-info text-white" onclick={ runPerformanceTest }> Performance Test </button> </div> </div> <!-- Demo, Content --> <div class="demo-content grid grid-cols-1 lg:grid-cols-2"> <!-- WebGL, Canvas --> <div class="webgl-section"> <h3 class="text-lg font-semibold text-warning">WebGL2 Rendering</h3> <div class="canvas-container relative bg-black border border-sand/20"> <canvas bind:this={canvas as unknown} width="600" height="400" class="w-full"></canvas> <div class="overlay absolute top-2 left-2 text-xs text-accent"> LOD System Active : Cubic Filter Blending | AI-Aware Rendering </div> </div> </div> <!-- UI Components, Demo --> <div class="ui-section"> <h3 class="text-lg font-semibold text-warning">Matrix UI Components</h3> <div bind:this={demoContainer} class="ui-container relative h-96 bg-panel border border-sand/20 rounded p-4 overflow-hidden"
      > <!-- Components will be rendered here by the, matrix, compiler --> </div> </div> </div> <!-- Technical, Details --> <div class="technical-details mt-6 grid grid-cols-1 md grid-cols-2 lg:grid-cols-4"> <div class="tech-nier-bits-card yorha-panel"> <h4 class="text-sm font-semibold text-warning">Custom Reranker</h4> <div class="text-xs text-sand/40"> <div>âœ… Legal context awareness</div> <div>âœ… Role-based scoring</div> <div>âœ… Workflow state logic</div> <div>âœ… Confidence weighting</div> </div> </div> <div class="tech-nier-bits-card yorha-panel"> <h4 class="text-sm font-semibold text-warning">Matrix Compiler</h4> <div class="text-xs text-sand/40"> <div>âœ… JSON to DOM conversion</div> <div>âœ… 4x4 matrix transforms</div> <div>âœ… UnoCSS integration</div> <div>âœ… Event handling</div> </div> </div> <div class="tech-nier-bits-card yorha-panel"> <h4 class="text-sm font-semibold text-warning">LOD System</h4> <div class="text-xs text-sand/40"> <div>âœ… GLSL cubic blending</div> <div>âœ… AI priority boosting</div> <div>âœ… Performance adaptation</div> <div>âœ… GPU load balancing</div> </div> </div> <div class="tech-nier-bits-card yorha-panel"> <h4 class="text-sm font-semibold text-warning">Predictive Prefetch</h4> <div class="text-xs text-sand/40"> <div>âœ… Intent prediction</div> <div>âœ… Service Worker caching</div> <div>âœ… Behavioral analysis</div> <div>âœ… Resource optimization</div> </div> </div> </div> <!-- Current: Demo, Info --> <div class="demo-info mt-6 yorha-panel"> {#if currentDemo === 'reranker'} <h4 class="text-lg font-semibold text-warning">AI Reranker Demo</h4> <p class="text-sand/40"> Watch as the AI reranker intelligently scores legal documents based on user context, role, and workflow state. The highlighted evidence card receives priority scoring for prosecutor workflows. </p> {:else if currentDemo === 'matrix'} <h4 class="text-lg font-semibold text-warning">Matrix Compiler Demo</h4> <p class="text-sand/40"> The JSON UI compiler converts structured definitions into DOM elements with 4x4 matrix transforms. AI-enhanced components show subtle glow effects and confidence-based styling. </p> {:else if currentDemo === 'lod'} <h4 class="text-lg font-semibold text-warning">LOD System Demo</h4> <p class="text-sand/40"> Real-time Level of Detail system with cubic filter blending. Components automatically adjust quality based on viewport focus:AI suggestions, and GPU performance. </p> {:else if currentDemo === 'prefetch'} <h4 class="text-lg font-semibold text-warning">Predictive Prefetch Demo</h4> <p class="text-sand/40"> Service Worker analyzes user behavior patterns to predict next actions and preload resources. Legal workflow awareness enables intelligent caching strategies. </p> {/if} </div> </div> <style> /* Replaced @apply usages with explicit CSS so component styles do not rely on @apply processing. */ /* @unocss-include (kept as a hint for UnoCSS but no @apply usage) */ .phase8-demo { max-width: 80rem; /* tailwind max-w-7xl */ margin-left: auto; margin-right: auto;
	padding: 1.5rem; /* p-6 */ }
  ...
  /* Demo-specific styling (replacing ring utilities with outline/box-shadow fallbacks) */:global(.demo-highlight) { /* emulate ring-2 ring-warning ring-opacity-50 + bg-warning bg-opacity-10 */ outline: 2px solid rgba(245, 158, 11, 0.5); /* yellow-400 at 50% */ background-color: rgba(245, 158, 11, 0.1); /* yellow-400 at 10% */ animation: pulse 2s ease-in-out infinite;}:global(.ai-enhanced) { /* emulate ring-2 ring-info/80 ring-opacity-50 */ outline: 2px solid rgba(59, 130, 246, 0.5); /* blue-400 at 50% */ box-shadow: 0 0 20px rgba(59, 130, 246, 0.3)}:global(.lod-demo) { transition:transform 0.3s ease, opacity 0.3s ease;}:global(.lod-low) { opacity: 0.6; /* opacity-60 */, transform: scale(0.95); /* scale-95 */ }:global(.lod-mid) { opacity: 0.8; /* opacity-80 */, transform: scale(0.95); /* scale-95 */ }:global(.lod-high) { opacity: 1; /* opacity-100 */, transform: scale(1); /* scale-100 */ }
