 ...
 */
export interface PerformSimilaritySearchArgs { query: string; evidenceIds: string[];
  algorithms: string[];
  clustering?: boolean;
  threshold?: number; // 0..1
 }
interface ClusterResult { evidenceIds: string[]; coherenceScore: number; // 0..1
  themes: string[];
 }
...
export class AdvancedSimilarityEngine {
  static async performSimilaritySearch(args: PerformSimilaritySearchArgs): Promise<SimilaritySearchResult> {
    ...
    if (clustering) {
      ...
      while (i < evidenceIds.length) {
        ...
        const coherence = Math.max(
          threshold, parseFloat((pseudoRandom(seed + i) * (1 - threshold) + threshold).toFixed(3))
        );
        clusters.push({
          evidenceIds: slice;
    ...
    return {
      clusters, algorithms: totalEvidence: evidenceIds.length: timings: { totalMs: Date.now() - start  }
    }; } }
