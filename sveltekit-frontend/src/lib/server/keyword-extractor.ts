...
export async function extractKeywords(
	...
	try {
		...
		console.log(`✅ Keywords extracted (${processingTime}ms, confidence: ${result.confidence.toFixed(2)})`);
...
export async function extractKeywordsFromImage(
	...
	try {
		...
		// Image analysis not yet implemented - return structured fallback
		const processingTime = Date.now() - startTime;
...
function extractKeywordsFallback(content: string, documentType?: string): KeywordExtractionResult {
	...
	// Extract key phrases
	const keyPhrases = extractPhrases(content);
...
function extractEntitiesFallback(content: string): KeywordExtractionResult['entities'] {
	...
	// Date pattern
	const datePattern = /\b(\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}|January|February|March|April|May|June|July|August|September|October|November|December)\b/gi;
