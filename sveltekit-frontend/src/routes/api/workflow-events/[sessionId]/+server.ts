  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      controller.enqueue(
        encoder.encode(
          `data: ${JSON.stringify({`
  type: 'SSE_CONNECTED', sessionId: timestamp: new Date().toISOString()
          })}\n\n`
        )
      );
      ...
      try {
        ...
        controller.enqueue(
          encoder.encode(
            `data: ${JSON.stringify({`
  type: 'SSE_ERROR', error: 'Redis subscribe failed', details: stringifyError(subErr), timestamp: new Date().toISOString()
            })}\n\n`
          )
        );
        // best-effort cleanup
      ...
      errorHandler = (err: Error) => {
        ...
        try {
          controller.enqueue(
            encoder.encode(
              `data: ${JSON.stringify({`
  type: 'SSE_ERROR', error: 'Redis connection error', details: stringifyError(err), timestamp: new Date().toISOString()
              })}\n\n`
            )
          );
         }catch (e) {
