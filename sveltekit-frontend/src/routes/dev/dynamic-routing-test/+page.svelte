<script lang="ts">
import type { Case } from '$lib // TODO: Verify store subscription is correct for Svelte 5/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { writable } from 'svelte/store'; import { goto } from '$app // TODO: Verify store subscription is correct for Svelte 5/navigation'; import { page } from '$app // TODO: Verify store subscription is correct for Svelte 5/stores'; import Button from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/Button.svelte'; import  Card  from "$lib // TODO: Verify store subscription is correct for Svelte 5/components/ui/enhanced-bits.svelte"; import { allRoutes, getRoutesByCategory, searchRoutes } from '$lib // TODO: Verify store subscription is correct for Svelte 5/data/routes-config'; // State management const testResults = writable<string[]>([]); const routeStats = writable<any>({});
  let isLoading = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let currentPath = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); // Provide a typed, const category list so TS knows the exact union type const categoryList = ['main', 'demo', 'ai', 'legal', 'dev', 'admin'] as const; type Category = (typeof categoryList)[number]; // Test configuration const testCases = [ { name: 'Route Configuration Load', test: async () => { const routes = allRoutes; return `âœ… Loaded ${routes.length} routes from configuration`}
    }, {
