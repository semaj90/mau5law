...
export async function getVectorSearchService(): Promise<VectorSearchService> {
  if (!serviceInstance) {
    serviceInstance = new VectorSearchService({
      pgvectorUrl: process.env.PGVECTOR_URL || 'postgresql://localhost:5432/vectors', qdrantUrl: process.env.QDRANT_URL || 'http://localhost:6333', qdrantApiKey: process.env.QDRANT_API_KEY, redis: database: db, as any, // Type casting for now - db is a Drizzle client
      cacheTtl: 3600, primaryProvider: 'pgvector'
    });
    try {
