<script lang="ts">
  ...
  onMount(async () => { if (browser && editorElement) {
      setupEditor();
  ...
  function insertCitation(citation: CitationPoint) { if (readOnly) return;
    ...
    if (currentSelection) { // Insert at current cursor position
  ...
  async function saveReport() { if (!isDirty || isLoading) return;
    ...
    try {
      ...
      const response = await fetch("/api/reports", { method: report ? "PUT" : "POST",
        headers: {
      ...
      if (response.ok) { const savedReport = await response.json();
        report = savedReport;
        isDirty = false;
        lastSaved = new Date();
  ...
  async function generateAiSuggestions() { if (isGeneratingAi) return;
    ...
    try {
      const response = await fetch("/api/ai/suggestions", {
        ...
        body: JSON.stringify({ content: editorElement.textContent,
          caseId,
...
<style>
  /* @unocss-include */
  .report-editor-container { display: flex,
  ...
  .editor-controls { display: flex,
    align-items: center;, gap: 16px;
 }
  .editor-stats { display: flex; gap: 12px;,
    font-size: 12px;,
    color: #64748b;
 }
  ...
  .formatting-toolbar { display: flex,
    align-items: center; padding: 8px 16px;,
    border-bottom: 1px solid #e2e8f0;,
    background: #f8fafc;,
    gap: 8px;
 }
  .formatting-toolbar button { padding: 6px 10px;,
  ...
  .content-editor.read-only { background: #f9fafb;,
    cursor: default }
  /* Citation token styling */
  .content-editor :global(.citation-token) { background: #dbeafe;,
    color: #1d4ed8;,
    padding: 2px 6px;,
    border-radius: 4px;,
    font-size: 12px;,
    ...
    white-space: nowrap }
  .content-editor :global(.citation-token:hover) { background: #bfdbfe;
 }
  .citation-sidebar { position: absolute; right: 0;,
    top: 0;,
    width: 300px;,
    height: 100%;,
  ...
  .sidebar-content { padding: 16px;,
    ...
    overflow-y: auto }
  .citation-search input { width: 100%;,
    padding: 8px 12px;,
    border: 1px solid #d1d5db;,
    border-radius: 6px;,
    margin-bottom: 16px;
  ...
  .add-citation-btn { padding: 4px 8px;,
    background: #3b82f6;,
  ...
  .ai-suggestions-panel { position: absolute; bottom: 0;,
    left: 0;,
    right: 0;,
    height: 200px;,
  ...
  .use-suggestion-btn { padding: 4px 8px;,
    background: #10b981;,
    color: white; border: none,
    border-radius: 4px;,
    cursor: pointer,
  ...
  .selected-citations { padding: 16px;,
    border-top: 1px solid #e2e8f0;,
    background: #f8fafc;
 }
  .selected-citations h4 { margin: 0 0 8px 0;,
    font-size: 14px;,
    color: #374151;
 }
  .selected-citation { display: inline-flex;,
  ...
  .remove-citation { background: none; border: none,
    color: #6b7280;,
    cursor: pointer,
    font-size: 14px;,
    line-height: 1;
