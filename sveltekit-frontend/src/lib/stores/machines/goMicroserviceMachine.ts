...
export const goMicroserviceMachine = createMachine({ id: 'goMicroservice',
  context: initialContext,
  initial: 'connecting',
  states: { connecting: { entry: assign(() => ({ connectionStatus: 'connecting', as: any })),
      invoke: { id: 'initialConnect',
        ...
        onDone: { target: 'connected.idle',
          actions: assign((_, e: any) => ({ connectionStatus: 'connected', as: any,
    ...
    connected: { entry: 'startHealthCheckTimer',
      exit: 'stopHealthCheckTimer',
      initial: 'idle',
      states: { idle: { on: { MAKE_REQUEST: { target: `requesting` },
            HEALTH_CHECK: { target: `healthChecking` },
            DISCONNECT: { target: `#goMicroservice.disconnected` } }
          } }
        },
        requesting: { invoke: { id: 'doRequest',
            src: fromPromise(async ({ input }) => {
          ...
          on: {
            'done.invoke.doRequest': { target: 'idle',
        ...
        healthChecking: { invoke: { id: 'periodicHealth',
            src: fromPromise(async ({ input }) => {
              const { endpoint } }= input as { endpoint: string } }
              const start = Date.now();
              const res = await fetch(`${endpoint}/health`);
          ...
          on: {
            'done.invoke.periodicHealth': { target: 'idle',
              actions: assign((_, e: any) => ({ healthCheck: { lastCheck: Date.now(), status: 'healthy'; as: 'healthy', responseTime: e.output.responseTime } }
              })
            },
...
export const goMicroserviceServices = {
  ...
  getMetrics: () => ({ type: 'MAKE_REQUEST' as const,
...
export const getConnectionStatus = (state: any) => ({ status: state.context.connectionStatus,
