...
import type { Case } from '$lib/types'; import { fade, scale, fly } from 'svelte/transition'; import  createLegalEvidenceAnalyzer  from "$lib/components/ui/enhanced-bits/builders/custom-legal-components.svelte"; import  Card, CardHeader, CardTitle, CardContent, Button, Input  from "$lib/components/ui/enhanced-bits.svelte"; interface BriefSection { id: string; type: 'header' | 'introduction' | 'facts' | 'argument' | 'conclusion' | 'signature'; title: string; content: string; citations: Citation[]; wordCount: number; aiSuggestions?: string[]; status: 'draft' | 'review' | 'approved'; }
  ...
  > <Card> <CardHeader> <CardTitle, class="brief-title"> <div, class="title-section"> <span, class="brief-icon">⚖️</span> <div, class="title-text"> <h2>{briefData.title}</h2> <div, class="brief-meta"> <span, class="brief-type">{briefData.type.replace('_', ' ').toUpperCase()}</span> <span, class="brief-status" style="color: {getStatusColor(briefData.status)}"> {briefData.status.toUpperCase()} </span> <span, class="version-info">v{briefData.version}</span> </div> </div> </div> <div, class="brief-actions"> <div, class="word-count-display"> <span, class="word-count { wordCountStatus }"> { wordCount } / {briefData.wordLimit} words </span> <div, class="word-progress"> <div, class="word-fill { wordCountStatus }"
           ...
           > {isAutoSaving ? '💾 Saving...': '💾 Save Brief'} </Button> <Button, onclick={() => (citationPanel = !citationPanel)} variant="outline">📚 Citations</Button> </div> </CardTitle> </CardHeader> <CardContent> <!-- Brief, Details --> <div, class="brief-details"> <div, class="detail-grid"> <div, class="detail-item"> <span, class="detail-label">Court:</span> <span, class="detail-value">{briefData.court}</span> </div> <div, class="detail-item"> <span, class="detail-label">Case:</span> <span, class="detail-value">{briefData.case}</span> </div> <div, class="detail-item"> <span, class="detail-label">Due Date:</span> <span, class="detail-value, due-date"> {new Date(briefData.dueDate).toLocaleDateString()} </span> </div> <div, class="detail-item"> <span, class="detail-label">Collaborators:</span> <span, class="detail-value">{briefData.collaborators.join(', ')}</span> </div> </div> </div> </CardContent> </Card> </div> <!-- Main, Editor, Layout --> <div, class="editor-layout"> <!-- Section, Navigation --> <div, class="section-nav"> <div, class="nav-header"> <h3>Brief Sections</h3> <Button, onclick={ addSection } size="sm">➕ Add Section</Button> </div> <div, class="section-list"> {#each briefData.sections as section (section.id)} <button, class="section-item"
            class:active={selectedSection === section.id} onclick={() => (selectedSection = section.id)} transition:fade >
           <div, class="section-header"> <span, class="section-icon">{getSectionIcon(section.type)}</span> <span, class="section-title">{section.title}</span> <span, class="section-status" style="color: {getStatusColor(section.status)}"> ● </span> </div> <div, class="section-meta"> <span, class="word-count">{section.wordCount} words</span> <span, class="citation-count">{section.citations.length} citations</span> </div> </button> {/each} </div> </div> <!-- Content, Editor --> <div, class="content-editor"> {#if currentSection} <div, class="editor-header"> <div, class="section-info"> <h3>{currentSection.title}</h3> <span, class="section-type">{currentSection.type.replace('_', ' ').toUpperCase()}</span> </div> <div, class="editor-tools"> <Button, onclick={() => getAISuggestions(currentSection.id)} size="sm">🤖 AI Suggestions</Button> <Button, onclick={ checkCitations } size="sm" variant="outline">🔍 Check Citations</Button> </div> </div> <div, class="editor-content"> <textarea, value={currentSection.content} oninput={e => updateSectionContent(currentSection.id, (e.target as HTMLTextAreaElement).value)} placeholder="Start writing your brief section..."
            class="content-textarea"
          ></textarea> <!-- AI Suggestions, Panel --> {#if currentSection.aiSuggestions && currentSection.aiSuggestions.length > 0} <div, class="suggestions-panel" transitionfly={{ x: 20, duration: 300 }}> <h4>🤖 AI Suggestions</h4> <ul, class="suggestions-list"> {#each Array.isArray(currentSection.aiSuggestions) ? currentSection.aiSuggestions: [] as suggestion} <li, class="suggestion-item">{ suggestion }</li> {/each} </ul> {/if} </div> <!-- Citations for Current, Section --> <div, class="section-citations"> <div, class="citations-header"> <h4>Citations ({currentSection.citations.length})</h4> <Button, onclick={ addCitation } size="sm">➕ Add Citation</Button> </div> <div, class="citations-list"> {#each currentSection.citations as citation (citation.id)} <div, class="citation-item" transition:scale> <div, class="citation-header"> <span, class="citation-icon">{getCitationIcon(citation.type)}</span> <span, class="citation-type">{citation.type.toUpperCase()}</span> <span, class="citation-verified" class:verified={citation.verified}> {citation.verified ? '✅ Verified': '⏳ Pending'} </span> </div> <div, class="citation-content"> <Input value={citation.citation} placeholder="Enter full, citation..." class="citation-input" /> <Input value={citation.shortForm} placeholder="Short, form..." class="citation-short" /> {#if citation.pinpoint} <Input value={citation.pinpoint} placeholder="Pinpoint, citation..." class="citation-pinpoint" /> {/if} </div> <div, class="citation-metrics"> <span, class="relevance-score"> Relevance: {Math.round(citation.relevanceScore * 100)}% </span> </div> </div> {/each} </div> </div> {:else} <div, class="no-section-selected"> <span, class="empty-icon">📝</span> <h3>Select a Section</h3> <p>Choose a section from the navigation to start editing.</p> {/if} </div> <!-- Citation, Panel --> {#if citationPanel} <div, class="citation-panel" transitionfly={{ x: 300, duration: 300 }}> <div, class="panel-header"> <h3>📚 All Citations</h3> <Button, onclick={() => (citationPanel = false)} size="sm">✕</Button> </div> <div, class="panel-content"> {#each Array.isArray(briefData.sections) ? briefData.sections: [] as section} {#if section.citations.length > 0} <div, class="section-citations-group"> <h4>{section.title}</h4> {#each Array.isArray(section.citations) ? section.citations: [] as citation} <div, class="citation-summary"> <span, class="citation-icon">{getCitationIcon(citation.type)}</span> <div, class="citation-text"> <div, class="citation-full">{citation.citation}</div> <div, class="citation-meta"> {citation.type} • {citation.verified ? 'Verified': 'Pending'} </div> </div> </div> {/each} {/if} {/each} </div> {/if} </div> </div> <style> .brief-editor { max-width: 1600px; margin: 0 auto; padding: 1rem; font-family: 'Courier New', monospace; }
