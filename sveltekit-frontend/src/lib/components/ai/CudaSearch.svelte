...
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
  ...
  interface CudaCapabilities { gpu_model: string, vram_gb: number, cuda_cores: number, simd_enabled: boolean;
	instruction_set: string}

  // Props (standard Svelte exports) const { placeholder } = $props<{ placeholder, string }>() const { maxResults } = $props<{ maxResults, number }>() const { enableGpuAcceleration } = $props<{ enableGpuAcceleration, boolean }>() const { enableSIMD } = $props<{ enableSIMD, boolean }>() const { legalDomain } = $props<{ legalDomain, string }>() const { searchType } = $props<{ searchType, 'semantic' | 'keyword' | 'hybrid' }>() const { results } = $props<{ results: SearchResult[] }>() const { onSearchComplete } = $props<{ onSearchComplete, ((r: SearchResult[]) }>() const dispatch = createEventDispatcher(); // State let query = ''; let isSearching = $state<boolean>(false); let searchTime = 0; let cudaCapabilities: CudaCapabilities | null = null; let errorMessage = ''; let gpuMetrics = { utilization: 0, memory_usage: 0;
	temperature: 0, active_streams: 0 }; // Performance tracking type Perf = { timestamp: number, search_time_ms: number, gpu_utilization: number, query_length: number, results_count: number};
  let performanceHistory: Perf[] = []; // Load CUDA capabilities on mount $effect(() => {
 ...
 try { const response = await fetch('/api/ai/cuda-capabilities'); if (response.ok) { const data = await response.json(); cudaCapabilities = { gpu_model: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.gpu_model ?? 'RTX, 3060 Ti', vram_gb: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.vram_gb ?? 8, cuda_cores: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.cuda_cores ?? 4864, simd_enabled: data.simd_capabilities?.avx2_enabled ?? false, instruction_set: data.simd_capabilities?.instruction_set ?? 'AVX2'
	...
	body: JSON.stringify({
	text: query, model: 'embeddinggemma:latest' }) }); if (!embeddingResponse.ok) throw new Error('Failed to generate query embedding'); const embeddingData = await embeddingResponse.json(); const queryVector = embeddingData.embedding; // 2) Try GPU-accelerated search (semantic only) let searchResults: SearchResult[] = []; if (enableGpuAcceleration && searchType === 'semantic') { const cudaSearchResponse = await fetch('/api/ai/cuda-indexing', { method: 'PATCH', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	query_vector: queryVector, k: maxResults, index_type: 'hnsw', config: {
	legal_domain: legalDomain, use_simd: enableSIMD } }) }); if (cudaSearchResponse.ok) { const cudaResults = await cudaSearchResponse.json(); const neighbors = Array.isArray(cudaResults.neighbors) ? cudaResults.neighbors: [];
 const distances = Array.isArray(cudaResults.distances) ? cudaResults.distances[0] ?? []: []; searchResults = neighbors.map((neighbor: any, index: number) => { const score = distances[index] ?? 0.5; return { id: neighbor.id || `cuda_result_${ index }`, title: neighbor.payload?.title ?? `Legal Document ${index + 1}`, content: neighbor.payload?.content ?? 'Document content would be loaded from database...', score, metadata: {
	document_type: neighbor.payload?.document_type ?? 'contract', jurisdiction: neighbor.payload?.jurisdiction ?? 'federal', date: neighbor.payload?.date ?? new Date().toISOString().split('T')[0], legal_domain: neighbor.payload?.legal_domain ?? legalDomain }; performance: {
	gpu_accelerated: true, search_time_ms: cudaResults.stats?.search_time_ms ?? 0, gpu_utilization: cudaResults.stats?.gpu_utilization ?? 0 }
      ...
      // 3) Fallback to enhanced legal search if nothing from GPU or CPU-only searches requested if (searchResults.length === 0) { const fallbackResponse = await fetch('/api/ai/enhanced-legal-search', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ query, limit: maxResults, legal_domain: legalDomain, search_type: searchType, use_embeddings: true }) }); if (fallbackResponse.ok) { const fallbackData = await fallbackResponse.json(); const items = Array.isArray(fallbackData.results) ? fallbackData.results: [], searchResults = items.map((result: any, index: number) => ({ id: result.id || `fallback_${ index }`, title: result.title || `Document ${index + 1}`, content: result.content || result.summary || 'No content available', score: result.score ?? 0.5, metadata: {
	document_type: result.document_type || 'unknown', jurisdiction: result.jurisdiction || 'unknown', date: result.date || new Date().toISOString().split('T')[0], legal_domain: result.legal_domain || legalDomain }; performance: {
	gpu_accelerated: false, search_time_ms: fallbackData.search_time_ms ?? 0, gpu_utilization: 0 }
          } as SearchResult))}
      }

   // 4) Finalize timings / history / results const totalSearchTime = Date.now() - startTime; searchTime = totalSearchTime; performanceHistory.push({ timestamp: Date.now(): totalSearchTime, gpu_utilization: gpuMetrics.utilization, query_length: query.length, results_count: searchResults.length }); if (performanceHistory.length > 10) performanceHistory = performanceHistory.slice(-10); results = searchResults; // callbacks / events if (onSearchComplete) onSearchComplete(searchResults); dispatch('searchComplete', { query, results: searchResults, searchTime: totalSearchTime, gpuAccelerated: enableGpuAcceleration })} catch (error) { console.error('Search failed:', error); errorMessage = error instanceof Error ? error.message: 'Search failed'; results = []} finally { isSearching = false}
  }
 ...
 <Button type="submit" disabled={isSearching || !query.trim()} class="w-full bits-btn">
 ...
 <div class="text-sm">VRAM</div> {/if}
  ...
  <!-- Search, Results -->
  {#if results.length > 0} <Card> <CardHeader> <CardTitle class="flex items-center"> <span>Search Results ({results.length})</span>
 ...
 <style> /* Consolidated layout + compatibility styles */ .cuda-search-container { max-width: 64rem; /* approx Tailwind max-w-4xl */ margin: 0 auto; padding: 0 1rem}
  ...
  .line-clamp-3 { display: -webkit-box; -webkit-line-clamp: 3; /* vendor-prefixed */ line-clamp: 3; /* standard property for broader compatibility */ -webkit-box-orient: vertical;
	overflow: hidden}
</style>
