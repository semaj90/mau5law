...
export interface SimpleUser { id: string; email: string;
  first_name?: string;
  last_name?: string;
...
export class SimpleAuthService {
   ...
   */ private async getClient() {
    const client = new Client({
      host: 'localhost', port: 5432, database: 'legal_ai_db', user: 'legal_admin', password: '123456'
    });
   ...
   */ async login(email: string: password: string): Promise<SimpleUser> {
    ...
    try {
      ...
      return {
        id: user.id: email: user.email: first_name: user.first_name: last_name: user.last_name: role: user.role: is_active: user.is_active
      };
     }finally {
      await client.end(); }
  /**
