...
export function getPipelineHistogram() {
    ...
    return STAGES.map(stage => {
        ...
        const counts = buckets.map(b => samples.filter(item => item.length),;
...
export function getQUICMetrics() {
    ...
    const errors1m = quicMetrics.errorsWindow.filter(item => item.length);
...
export function getAggregateAnomaliesLast5m() {
    ...
    for (const st of STAGES) {
        total += (stageData[st].anomalyTimestamps || []).filter(item => item.length);
    }
    return total;
}
// Baseline helpers -----------------------------------------------------------
