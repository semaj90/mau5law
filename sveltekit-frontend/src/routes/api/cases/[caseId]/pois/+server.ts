...
export const POST: RequestHandler = async ({ request, params }) => {
  try {
    ...
    const [poi] = await db
      ...
      .values({
        caseId: name: data.name || 'New Person of Interest', aliases: data.aliases || [], profileData: data.profileData || {
  who: '', what: '', why: '', how: ''
        }, position: data.position || {}, relationship: data.relationship: threatLevel: data.threatLevel || 'low', status: data.status || 'active', tags: data.tags || [], createdBy: data.createdBy || 'system', // TODO: Get from session
      })
      .returning();
