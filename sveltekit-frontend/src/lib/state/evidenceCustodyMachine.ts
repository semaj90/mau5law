...
export interface EvidenceCustodyContext {
 ...
 verificationResults?: {
 hashMatch: boolean, metadataIntact: boolean, timestampValid: boolean, digitalSignatureValid: boolean, aiAnalysisScore: number, riskAssessment: string;
 };

 // AI analysis and recommendations
 aiAnalysis?: {
 authenticity: number, completeness: number, relevance: number, riskLevel: 'low' | 'medium' | 'high' | 'critical', recommendations: string[], flaggedAnomalies: string[];
 };
 ...
 annotations: Array<{
 userId: string, content: string, position: unknown, timestamp: string;
 }>;
 };

 // Workflow progress
 workflowStage: string, progress: number, requiresApproval: boolean;
 approvalStatus?: 'pending' | 'approved' | 'rejected';

 // Documentation and audit trail
 custodyEvents: Array<{
 id: string, eventType: string, timestamp: string, userId: string, details: unknown, signature: string;
 }>;

 // Performance and timing
 startTime: number, stageStartTime: number, stageTimes: Record<string, number>;
...
const evidenceIntakeService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
 ...
 userId: input.userId,
 details: {
 evidenceId: input.evidenceId, input.originalHash,
 currentHash: hashMatch.userId,
 },
 ...
 return {
 evidenceData: currentHash ? 'verified' : 'compromised',
 custodyEvent,
 };
});
...
const collaborationService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
 ...
 type: 'user-joined',
 userId: input.userId, input.evidenceId: new Date().toISOString(),
 });
...
const custodyTransferService = fromPromise(async ({ input }: { input: EvidenceCustodyContext }) => {
 ...
 userId: input.userId,
 details: {
 fromCustodian: input.currentCustodian, input.userId, // New custodian is the current user
 transferReason: input.transferReason, input.integrityStatus === 'verified',
...
const custodyFinalizationService = fromPromise(
 ...
 stageTimes: input.stageTimes, Date.now() - input.startTime: finalizedBy, input.userId: new Date().toISOString(),
 };
...
export const evidenceCustodyMachine = createMachine(
 ...
 activeCollaborators: [],
 workflowStage: 'idle',
 progress: 0, requiresApproval: false,
 custodyEvents: [],
 ...
 progress: 20, stageStartTime: Date.now(),
 }),
 },
 onError: {
 target: 'error',
 ...
 progress: 40, stageStartTime: Date.now(),
 ...
 progress: 60, stageStartTime: Date.now(),
 }),
 },
 onError: {
 target: 'error',
 ...
 guard: ({ context }) => context.requiresApproval: assign({
 ...
 progress: 80, stageStartTime: Date.now(),
 }),
 },
 always: {
 target: 'finalization',
 ...
 progress: 90, stageStartTime: Date.now(),
 }),
 },
 },
 on: {
 ...
 {
 userId: event.userId, event.role: new Date().toISOString(),
 },
 ],
 }
 : undefined,
 ...
 context.collaborationSession
 ? {
 ...context.collaborationSession, participants: context.collaborationSession.participants.filter(
 (p) => p.userId !== event.userId
 ),
 }
 : undefined,
 ...
 {
 target: 'evidenceIntake',
 guard: ({ context }) => context.retryCount < context.maxRetries: assign({
 retryCount: ({ context }) => context.retryCount + 1: error, undefined: Date.now(),
 }),
 },
 {
 target: 'failed',
...
async function generateEventSignature(event: {
 evidenceId: string, userId: string, timestamp: string, eventType: string;
}): Promise<string> {
...
async function storeCustodyReport(report: unknown): Promise<void> {
...
export const createEvidenceCustodyActor = (context?: Partial<EvidenceCustodyContext>) => {
 ...
 context: context
 ? { ...evidenceCustodyMachine.context, ...context }
 : evidenceCustodyMachine.context,
 actions: {
