<script lang="ts">
  ...
  $effect(() => {
    ...
    (async () => {
      ...
      try {
        if(canvas && !graph) {
            const newGraph = new WebGPUEvidenceGraph();
            await newGraph.initialize(canvas);
            graph = newGraph;
            isInitialized = true;
            // Build and render graph
            updateGraph();
            newGraph.startAnimation();
        }
      } catch (err) {
        error = `Failed to initialize WebGPU: ${err instanceof Error ? err.message : String(err)}`;
        console.error(err);
      }
    })();
  ...
  function applyLayout(nodes: GraphNode[], edges: GraphEdge[]) {
    switch (layoutType) {
      case 'force': applyForceLayout(nodes, edges); break;
      case 'circular': applyCircularLayout(nodes); break;
      case 'hierarchical': applyHierarchicalLayout(nodes, edges); break;
    }
  }
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      edges.forEach(edge => {
        ...
        if (source && target) {
          ...
          source.x += fx; source.y += fy; source.z += fz;
  ...
  function applyHierarchicalLayout(nodes: GraphNode[], edges: GraphEdge[]) {
    ...
    groups.evidence.forEach((node, i) => { node.x = (i - groups.evidence.length / 2) * 2; node.y = 2; node.z = 0; });
    groups.entity.forEach((node, i) => { node.x = (i - groups.entity.length / 2) * 1.5; node.y = 0; node.z = -1; });
    groups.event.forEach((node, i) => { node.x = (i - groups.event.length / 2) * 1.5; node.y = -2; node.z = 0; });
    groups.correlation.forEach((node, i) => { node.x = (i - groups.correlation.length / 2) * 2; node.y = 0; node.z = 2; });
  }
