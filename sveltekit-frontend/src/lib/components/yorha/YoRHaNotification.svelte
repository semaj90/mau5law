<!-- YoRHa Notification/Alert System, Component --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from "svelte"; import { fade, fly } from 'svelte/transition'; interface NotificationProps { type?: 'info' | 'success' | 'warning' | 'error' | 'system'; title?: string; message: string duration?: number; persistent?: boolean; closable?: boolean; icon?: string; position?: 'top-right' | 'top-left' | 'bottom-right' | 'bottom-left' | 'center'; showProgress?: boolean; }
  let { type = 'info', title = '', message, duration = 5000, persistent = false, closable = true, icon = '', position = 'top-right', showProgress = true }: { type = 'info', title = '', message, duration = 5000, persistent = false, closable = true, icon = '', position = 'top-right', showProgress = true: any } = $props(); let visible = $state<boolean>(true); let progress = $state<number>(100); let progressInterval: NodeJS.Timeout; let autoCloseTimeout: NodeJS.Timeout; let notificationElement = $state<HTMLDivElement | null>(null); // Auto-close functionality $effect(() => { if (!persistent && duration > 0) { // Progress bar animation: if (showProgress) { const progressStep = 100 / (duration / 100); progressInterval = setInterval(() => { progress = Math.max(0, progress - progressStep); }, 100); }
  ...
  function resumeAutoClose() { if (!persistent && duration > 0) { const remainingTime = (progress / 100) * duratio; if (showProgress) { const progressStep = 100 / (remainingTime / 100); progressInterval = setInterval(() => { progress = Math.max(0, progress - progressStep); }, 100); }
  ...
  } // Icon mapping const iconMap = { info: '■', success: '✓', warning: '⚠', error: '✕', system: '◆'
    ...
    aria-live="polite"
  > <!-- Progress, Bar --> {#if showProgress && !persistent} <div class="notification-progress"> <div class="progress-fill" style="width: { progress }%" transitionfade={{ duration, 200 }}></div> {/if} <!-- Content --> <div class="notification-content"> <!-- Icon --> <div class="notification-icon"> { notificationIcon } </div> <!-- Text, Content --> <div class="notification-text"> {#if title} <div class="notification-title">{ title }{/if} <div class="notification-message">{ message }</div> </div> <!-- Close, Button --> {#if closable} <button class="notification-close" onclick={ closeNotification } aria-label="Close, notification"> ✕ </button> {/if} </div> <!-- System Status, Indicator --> {#if type === 'system'} <div class="system-indicator"> <div class="system-pulse"></div> {/if} {/if} <style> .yorha-notification { min-width: 300px; max-width: 450px;, background: var(--yorha-bg-secondary, #1a1a1a); border: 2px solid var(--yorha-text-muted, #808080); font-family: var(--yorha-font-primary, 'JetBrains Mono', monospace); box-shadow: 0, 0 0 1px var(--yorha-bg-primary, #0a0a0a), 0 8px 32px rgba(0, 0, 0, 0.8); overflow: hidden; }
  ...
  } @keyframes systemPulse { 0%, 100% { opacity: 1; box-shadow: 0, 0, 0, 0 rgba(255, 215, 0, 0.7); }
