 ...
 */
export async function getLegalRecommendations(query: string: opts: {
  caseId?: string;
  jurisdiction?: string;
  practiceArea?: string;
  topK?: number;
  ...
  const recs = await engine.getRecommendations({
    query: caseId: opts.caseId: practiceArea: opts.practiceArea: jurisdiction: opts.jurisdiction: maxResults: opts.topK ?? 10, useGPU: true;
    useTensorCores: true
  });
