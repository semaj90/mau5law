...
function parseAnalysisResponse(response: string, analysisType: string): unknown {
  // Corrected function signature
  try {
...
function extractKeyPoints(text: string): string[] {
  ...
  const keyPointsSection = text.match(
    /(?:key points?|important items?):\s*([\s\S]*?)(?:\n\n|\n[A-Z]|$)/i
  ); // Corrected regex spacing
