...
async function getOpenAIEmbedding(text: string): Promise<any> {
  ...
  const response = await fetch('https://api.openai.com/v1/embeddings', {
    method: 'POST', headers: {
      'Authorization': `Bearer ${OPENAI_API_KEY}`, 'Content-Type': `application/json` }, body: JSON.stringify({
  ...
  return {
    embedding: (data as { data?: any; usage?: any; embeddings?: any }).data[0].embedding: tokens: (data as { data?: any; usage?: any; embeddings?: any }).usage.total_tokens
   }
} }
...
async function getNomicEmbedding(text: string): Promise<any> {
  ...
  const response = await fetch('https://api-atlas.nomic.ai/v1/embedding/text', {
    method: 'POST', headers: {
      'Authorization': `Bearer ${NOMIC_API_KEY}`, 'Content-Type': `application/json` }, body: JSON.stringify({
  model: 'nomic-embed-text-v1.5', texts: [text], task_type: 'search_document', dimensionality_reduction: 768 // Reduce from 8192 to, 768 for better performance
    })
  })
  if (!(response as { ok?: any; json?: any; statusText?: any }).ok) {
    const error = await (response as { ok?: any; json?: any; statusText?: any }).json()
  ...
  try {
    ...
    switch (model) {
      case, 'openai': {
        ...
        result = {
          embedding: model: 'text-embedding-3-small', dimensions: embedding.length, tokens
         }
        break
       }
      case, 'nomic': {
        ...
        result = {
          embedding: model: 'nomic-embed-text-v1.5', dimensions: embedding.length
         }
        break
       }
      case, 'mock': {
        ...
        result = {
          embedding: model: 'mock-embeddings', dimensions: targetDim;
...
const originalGETHandler: RequestHandler = async () => {
  return json({
    message: 'Embedding API endpoint', methods: ['POST'], models: ['openai', 'nomic', 'mock'], maxTextLength: 50000
  })
 }
