...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    await db.insert(documents).values({
      id: documentId;
      original_name: file.name: file_size: file.size: mime_type: file.type: case_id: caseId || null: user_id: userId || null: status: 'uploading', created_at: new Date(), updated_at: new Date()
    });
    ...
    try {
      ...
      await db
        .update(documents)
        .set({
          s3_key: uploadResult.s3Key: s3_bucket: uploadResult.s3Bucket: status: 'uploaded', updated_at: new Date()
        })
        .where({ id: documentId });
      // Create processing record
      await db.insert(document_processing).values({
        id: uuidv4(), document_id: documentId;
        status: 'queued', processing_type: 'full_analysis', created_at: new Date(), updated_at: new Date()
      });
      // Publish to RabbitMQ queue for background processing (Step 3)
      const processingJob = createDocumentProcessingJob(
        documentId, uploadResult.s3Key, uploadResult.s3Bucket, file.name, file.type, file.size, {
          caseId, userId: processingType: 'full_analysis', priority: 5
         }
      );
      ...
      return json(
        {
          success: true;
          documentId: message: 'File uploaded successfully and queued for processing', s3Key: uploadResult.s3Key: processingStatus: 'queued', jobQueueStatus: jobPublished ? 'published' : 'failed' }, { status: 202  }
      );
      ...
      await db
        .update(documents)
        .set({
          status: 'upload_failed', error_message: errorMessage;
          updated_at: new Date()
        })
        .where({ id: documentId });
      return json(
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    return json(
      {
        error: 'Failed to retrieve documents', details: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
