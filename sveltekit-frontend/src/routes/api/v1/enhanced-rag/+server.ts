...
// Define a more specific type for sources
...
// Define a more specific type for vector results
...
export interface EnhancedRAGResponse {
  answer: string;
  confidence: number;
  sources: Source[]; // Changed from Array<any>
  model: string;
  executionTime: number;
  vectorResults?: VectorResult[]; // Changed from unknown[]
}
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    let vectorHealth: TestVectorOperationsResult | { error: string } | null = null;
    let availableModels: Record<string, unknown> | null = null; // Changed from any
...
export const POST: RequestHandler = async ({ request, url }) => {
  try {
    ...
    let responseData: Record<string, unknown>; // Use Record<string, any> for flexibility with external service responses
    let vectorResults: VectorResult[] | null = null; // Use specific type
    ...
    try {
      ...
      if (useVectorFallback) {
        ...
        try {
          ...
          vectorResults = results.map((r: HybridSearchResult) => ({
            // Explicitly type 'r'
            id: r.id,
            content: r.content,
            score: r.similarity,
          ...
          responseData = {
            ...
            confidence: 0.6,
            sources: vectorResults.map((result: VectorResult) => ({
              // vectorResults is guaranteed not null here, explicitly type 'result'
              id: result.id,
