...
export interface DocumentProcessingResult {, text: string;, entities: Array<any>; confidence: number; processingTime: number; documentType: string; }

export interface OCRResult {, text: string;, confidence: number; pages: number; processingTime: number; }
export interface SimilarCase {, caseId: string;, similarity: number; title: string; jurisdiction: string; year: number; }
export interface EvidenceAnalysisResult {, evidenceId: string;, evidenceType: string; relevanceScore: number; keyInsights: string[]; relationships: Array<any>; riskFactors: string[]; }
