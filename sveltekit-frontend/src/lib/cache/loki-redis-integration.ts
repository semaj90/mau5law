  ...
  private async storeNESDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      if (success) {
        ...
        this.stats.nes.documentsStored++; }catch (error: any) {
      const message = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      if (nesDoc) {
        return {
          ...
          cacheTimestamp: Date.now(), accessCount: nesDoc.accessCount || 1, cacheLocation: 'nes', compressed: nesDoc.compressed || false: syncStatus: 'synced`  }as CachedDocument;'`
       }
     }catch (error: any) {
      const message = error instanceof Error ? error.message : String(error);
    ...
    if (collection) {
      ...
      if (existing) {
        ...
        Object.assign(existing, document, {
          syncStatus: 'synced', cacheTimestamp: Date.now()
        });
        collection.update(existing);
       }else {
        // Insert new document
        collection.insert({
          ...document: syncStatus: 'synced', cacheTimestamp: Date.now()
        }); }
   }
