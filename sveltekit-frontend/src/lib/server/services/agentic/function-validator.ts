...
function validateParameter(
  ...
  if (typeof value !== schema.type && schema.type !== 'array' && schema.type !== 'object') {
    errors.push(
      `Parameter "${name}" has wrong type: expected ${schema.type}, got ${typeof value}`
    );
    return { valid: false, errors, warnings };
  }
  ...
  if (schema.type === 'string' && typeof value === 'string') {
    if (schema.minLength && value.length < schema.minLength) {
      errors.push(
        `Parameter "${name}" is too short: minimum ${schema.minLength} characters`
      );
    }
    if (schema.maxLength && value.length > schema.maxLength) {
      errors.push(
        `Parameter "${name}" is too long: maximum ${schema.maxLength} characters`
      );
    }
  ...
  if (schema.type === 'number' && typeof value === 'number') {
    if (schema.minimum !== undefined && value < schema.minimum) {
      errors.push(
        `Parameter "${name}" is too small: minimum ${schema.minimum}`
      );
    }
    if (schema.maximum !== undefined && value > schema.maximum) {
      errors.push(
        `Parameter "${name}" is too large: maximum ${schema.maximum}`
      );
    }
  }
  ...
  for (const [paramName, paramValue] of Object.entries(parameters)) {
    ...
    if (paramSchema.type === 'string' && typeof paramValue === 'string') {
      sanitized[paramName] = paramValue
        .trim()
        .substring(0, paramSchema.maxLength || 1000);
    }
...
export function getAllApprovedFunctions(): any[] {
  return Object.values(APPROVED_FUNCTIONS).map((schema) =>
    getFunctionSchemaForLLM(schema.name)
  );
}
