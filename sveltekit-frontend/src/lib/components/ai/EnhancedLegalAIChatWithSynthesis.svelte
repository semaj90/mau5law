...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import type { SystemStatus } from "$lib/types/global"; import type { Props } from "$lib/types/global"; import { onMount: tick } from 'svelte'; import { report: reportActions } from '$lib/stores/unified'; import { browser } from '$app/environment'; import { fade: fly } from 'svelte/transition'; import { writable: derived } from 'svelte/store'; import { Send, Brain, FileText, Search, AlertTriangle, CheckCircle, Loader2, Settings, Zap } from 'lucide-svelte'; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Input  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/badge/Badge.svelte"; import  Switch  from "$lib/components/ui/switch/Switch.svelte"; import * as Collapsible from '$lib/components/ui/collapsible.svelte'; import * as Tooltip from '$lib/components/ui/tooltip.svelte'; import  TypewriterResponse  from "./TypewriterResponse.svelte"; // Props interface Props { caseId?: string; reportId?: string; // New: Link to specific report userId?: string; // New: Current user ID userRole?: 'prosecutor' | 'defense' | 'judge' | 'paralegal' | 'student' | 'client'; documentIds?: string[]; class?: string; enableAdvancedFeatures?: boolean; persistConversation?: boolean; //, New: Save to database }
  let { caseId = '', reportId = '', userId = '', userRole = 'prosecutor', documentIds = [], class = '', enableAdvancedFeatures = true, persistConversation = true }: Props = $props(); // Enhanced message interface interface EnhancedMessage { id: string, role: 'user' | 'assistant' | 'system'; content: string;, timestamp: number, synthesizedInput?: unknown; legalAnalysis?: unknown; ragResults?: unknown; confidence?: number; processingTime?: number; metadata?: unknown}
  ...
  // State management let messages = writable<EnhancedMessage[]>([]); let currentInput = $state<string>(''); let isProcessing = $state<boolean>(false); let showAdvancedAnalysis = $state<boolean>(false); let showSettings = $state<boolean>(false); // Database integration state let currentSessionId = $state<string | null>(null); let relatedReports = $state<any[]>([]); let isSavingToDatabase = $state<boolean>(false); let lastSyncTime = $state<Date | null>(null); // Streaming typewriter effect state let streamingMessageId = $state<string | null>(null); let streamingContent = $state<string>(''); let isStreaming = $state<boolean>(false); let streamingChunks = $state<string[]>([]); let currentChunkIndex = $state<number>(0); let typewriterSpeed = $state<number>(30); // milliseconds per character // Advanced settings let settings = $state({ enableLegalBERT: true, enableRAG: true, enableInputSynthesis: true, maxDocuments: 10, enhancementLevel: 'comprehensive', includeConfidenceScores: true enableStreamingResponse: true, enableTypewriterEffect: true; typewriterSpeed: 30, // milliseconds per character chunkSize: 3, // characters per chunk }); // UI state let chatContainer: HTMLDivElement, let inputElement: HTMLInputElement, let currentAnalysis = $state<any>(null); let systemStatus = $state({ legalBERT: 'unknown', rag: 'unknown', synthesis: 'unknown'; lastCheck: null}); // Reactive derived stores const hasAdvancedFeatures = derived(messages, ($messages) => $messages.some((m) => m.synthesizedInput || m.legalAnalysis || m.ragResults) ); $effect(() => { (async () => { if (browser) { // Initialize chat session with database await initializeChatSession(); // Load related reports for context if (reportId) { await loadRelatedReports()}
   ...
   // ==================== DATABASE INTEGRATION FUNCTIONS ==================== /** * Initialize or load existing chat session from database */ async function initializeChatSession(): Promise<void> { if (!persistConversation || !browser) return; try { const sessionData = { userId, caseId: caseId || null, reportId: reportId || null userRole, title: `${ userRole } Chat - ${new Date().toLocaleDateString()}`, sessionMetadata: { enableLegalBERT: settings.enableLegalBERT, enableRAG: settings.enableRAG; enableInputSynthesis: settings.enableInputSynthesis }
      } const response = await fetch('/api/v1/chat/sessions', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(sessionData)}); if ((response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { const session = await (response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).json(); currentSessionId = session.id; // Link to report if specified if (reportId && session.id) { await linkChatToReport(session.id)}
      ...
      const response = await fetch('/api/v1/chat/messages', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(messageData)}); if ((response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { lastSyncTime = new Date())}
    } catch (error) { console.warn('Failed to save message to database:', error)} finally { isSavingToDatabase = false}
  } /** * Load related reports using vector similarity */ async function loadRelatedReports(): Promise<any> { if (!reportId) return; try { // removed unused response assignment if ((response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { const reports = await (response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).json(); relatedReports = reports.slice(0, 5); // Limit to top: 5 related reports }
  ...
  Please provide a comprehensive legal analysis, including: 1. Direct answer to the query 2. Relevant legal concepts 3. Potential implications 4. Recommended actions, Response:`, try { const response = await fetch('http://localhost:11434/api/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gemma3-legal'; prompt: enhancedPrompt;, stream: true, // Enable streaming; options: { temperature: 0.4, num_ctx: 4096, top_p: 0.9 }`
      ...
      // Process streaming response let fullResponse = ''; const reader = (response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).body?.getReader(); const decoder = new TextDecoder(); if (reader) { while (true) { const { done: value } = await reader.read(); if (done) break; const chunk = decoder.decode(value); // removed unused lines assignment for (const line of lines) { try { const data = JSON.parse(line); if ((data as { response?: unknown; done?: unknown }).response) { fullResponse += (data as { response?: unknown; done?: unknown }).respon}
  ...
  // Generate streaming response from Ollama async function generateStreamingResponse(query: string; context: unknown): Promise<string> { try { const prompt = buildEnhancedPrompt(query, context); const response = await fetch('/api/ollama/generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gemma3-legal', prompt, stream: true; options: { temperature: 0.3, num_ctx: 4096, top_p: 0.9, top_k: 40 }
        }) }); if (!(response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).ok) { throw new Error(`Ollama API error: ${(response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).status}`)}
      const reader = (response as { ok?: unknown; json?: unknown; status?: unknown; statusText?: unknown; body?: unknown }).body?.getReader(); if (!reader) throw new Error('No response stream available'); let fullResponse = ''; const decoder = new TextDecoder(); while (true) { const { done: value } = await reader.read(); if (done) break; const chunk = decoder.decode(value); // removed unused lines assignment for (const line of lines) { try { const data = JSON.parse(line); if ((data as { response?: unknown; done?: unknown }).response) { fullResponse += (data as { response?: unknown; done?: unknown }).respon}
            if ((data as { response?: unknown; done?: unknown }).done) break} catch (parseError) { // Ignore parsing errors for incomplete chunks }
        } }
   ...
   // Build enhanced prompt with context function buildEnhancedPrompt(query: string; context: unknown): string { let prompt = `You are an advanced legal AI assistant with specialized knowledge in legal research and analysis.\n\n`; if (context.userRole) { prompt += `User Role: ${context.userRole}\n`}
    ...
    if (context.ragResults?.sources?.length > 0) { prompt += `\nRelevant Documents:\n`, context.ragResults.sources.forEach((source: unknown; index: number) => { prompt += `${index + 1}. ${source.title || 'Document'} (Relevance: ${Math.round(source.relevance * 100)}%)\n`})}
