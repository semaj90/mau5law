...
class CitationService {
  ...
  async saveCitation(
    ...
    try {
      const citation: Citation = {
        ...
        statute_title: data.statute_title,
  ...
  async getCitationDetail(id: string): Promise<Citation | null> {
    try {
      ...
      const citations = await db.raw(
        'SELECT * FROM saved_citations WHERE id = $1',
        [id]
      );
  ...
  async deleteCitation(id: string, userId: string): Promise<void> {
    try {
      await db.raw('DELETE FROM saved_citations WHERE id = $1 AND user_id = $2', [
        id,
        userId,
      ]);
    ...
    try {
      ...
      return {
        ...
        bySeverity: Object.fromEntries(
          bySeverity.map((row: any) => [row.severity, row.count])
        ),
