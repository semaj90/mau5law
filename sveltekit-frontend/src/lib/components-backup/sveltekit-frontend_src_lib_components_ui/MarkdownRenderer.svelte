...
<script lang="ts">
  ...
  async function renderMarkdown() { if (!markdown) {
    ...
    try { // Configure marked with our options
      ...
      renderer.link = ({ href, title, tokens  }: { href: string title?: string; tokens?:, any[]  }) => { const titleAttr = title ? ` title="${title }"` : "";
      ...
      renderer.image = ({ href, title, text  }) => { const titleAttr = title ? ` title="${title }"` : "";
        const altAttr = text ? ` alt="${ text }"` : "";
        return `<img src="${ href }"${ titleAttr }${ altAttr } class="space-y-4" loading="lazy" />`;
      };

      // Customize code block rendering
      renderer.code = ({ text, lang  }) => { const langClass = lang ? ` class="space-y-4"` : "";
...
<style>
  ...
  :global(.prose) { color: #111827;,
    max-width: none }
  :global(.dark .prose) { color: #f3f4f6;
 }
  /* Headings */
  :global(.prose h1) { color: #111827;,
    font-weight: 700;,
    font-size: 1.875rem;,
    line-height: 1.2;,
    margin-top: 0;,
  ...
  :global(.dark .prose h1) { color: #f3f4f6;
 }
  :global(.prose h2) { color: #111827;,
    font-weight: 600;,
    font-size: 1.5rem;,
    line-height: 1.3;,
    margin-top: 2rem;,
  ...
  :global(.dark .prose h3) { color: #f3f4f6;
 }
  /* Paragraphs */
  :global(.prose p) { margin-top: 0;,
    margin-bottom: 1rem;,
    line-height: 1.6;
 }
  /* Lists */
  :global(.prose ul, .prose ol) { margin-top: 0;,
    margin-bottom: 1rem;,
    padding-left: 1.5rem;
 }
  :global(.prose li) { margin: 0.25rem 0;
 }
  :global(.prose li > p) { margin: 0;
 }
  /* Code */
  :global(.prose code) { color: #db2777;,
    background-color: #f3f4f6;,
    padding: 0.125rem 0.25rem;,
    border-radius: 0.25rem;,
    font-size: 0.875em;
 }
  :global(.dark .prose code) { color: #f472b6;,
    background-color: #1f2937;
 }
  :global(.prose pre) { background-color: #f3f4f6;,
    border-radius: 0.5rem;,
    padding: 1rem;,
    overflow-x: auto;, margin: 1rem 0;
 }
  :global(.dark .prose pre) { background-color: #1f2937;
 }
  :global(.prose pre code) { background-color: transparent; padding: 0;,
    color: inherit }
  /* Tables */
  :global(.prose table) { width: 100%;,
    border-collapse: collapse;, margin: 1rem 0;
 }
  :global(.prose th, .prose td) { border: 1px solid #d1d5db;,
    padding: 0.5rem;,
    text-align: left }
  :global(.dark .prose th, .dark .prose td) { border-color: #4b5563;
 }
  :global(.prose th) { background-color: #f3f4f6;,
    font-weight: 600;
 }
  :global(.dark .prose th) { background-color: #1f2937;
 }
  /* Blockquotes */
  :global(.prose blockquote) { border-left: 4px solid #3b82f6;,
    padding-left: 1rem;,
    margin: 1rem 0;,
    font-style: italic;, color: #4b5563;
 }
  :global(.dark .prose blockquote) { border-left-color: #60a5fa;,
    color: #9ca3af;
 }
  /* Links */
  ...
  :global(.dark .prose a) { color: #60a5fa;
 }
  :global(.dark .prose a:hover) { color: #93c5fd;
 }
  /* Images */
  :global(.prose img) { border-radius: 0.5rem;,
    box-shadow: 0 4px 6px -1px, rgba(0, 0, 0, 0.1);
 }
  /* Horizontal rules */
  :global(.prose hr) { border: none,
