...
export class RecommendationOrchestrator {
  ...
  private nesRLAgent: any = null;
  private detectiveContext: DetectiveContext = {
    evidenceCount: 0,
    pendingTasks: [],
    timeInMode: 0
  }
  private userActivityTimer: number = 0;
  private lastUserAction: number = Date.now();
  constructor() {
  ...
  private async initializeServices() {
    if (!browser) return;
    try {
      // Initialize NES-RL Worker
      await this.initializeNESRLWorker();
      // Setup user activity detection
      this.setupActivityDetection();
      // Setup RabbitMQ listeners
      this.setupRabbitMQListeners();
   ...
   */;
  private async initializeNESRLWorker() {
    try {
      ...
      this.worker.postMessage({
        ...
        config: {
          stateSize: 384,
          actionSize: 256,
          populationSize: 25, // Smaller for web worker
          learningRate: 0.02
        }
      });
    } catch (error) {
  ...
  private setupActivityDetection() {
    ...
    const updateActivity = (activity: 'idle' | 'typing' | 'active') => {
      this.lastUserAction = Date.now();
      this.updateUserActivity(activity);
      // Reset idle timer
    ...
    activityEvents.forEach(event => {
      document.addEventListener(event, () => updateActivity('active'), { passive: true });
    });
    // Detect typing specifically
    document.addEventListener('input', () => updateActivity('typing'), { passive: true });
  }
  /**
   * Setup RabbitMQ message listeners for real-time recommendations
   */;
  private setupRabbitMQListeners() {
    // Listen for evidence processing completion
    rabbitMQ?.on?.('evidence_processed', (data) => {
      this.handleEvidenceProcessed(data);
    });
    // Listen for vector search results
    rabbitMQ?.on?.('vector_search_complete', (data) => {
      this.handleVectorSearchComplete(data);
    });
    // Listen for system alerts
    rabbitMQ?.on?.('system_alert', (data) => {
  ...
  private async handleUserIdle() {
    ...
    // Generate idle-specific recommendations based on current context
   ...
   */;
  private generateDetectiveIdleRecommendations(context: any) {
   ...
   */;
  private generateEvidenceIdleRecommendations(context: any) {
    const recommendations: Recommendation[] = [];
   ...
   */;
  private generateGeneralIdleRecommendations(context: any) {
    const recommendations: Recommendation[] = [];
   ...
   */;
  private handleActionSuggestion(data: any) {
   ...
   */;
  private handleEvidenceProcessed(data: any) {
   ...
   */;
  private handleVectorSearchComplete(data,: any), {
  ...
  private addRecommendation(recommendation,: Recommendation), {
    this.recommendations.update(state => {
      ...
      const updated = [...state.recommendations, recommendation],);
  ...
  private async getCurrentContext(), {
    ...
    return {
      ...
      recentUploads: 0, // Get from your evidence store
      unprocessedImages: 0, // Get from your processing queue
      hasUnanalyzedContent: false // Check your content store
    }
  }
  /**
   ...
   * Update NES-RL stats
   */;
  private updateNESRLStats(stats,: any), {
    this.recommendations.update(state => ({
      ...state,
      nesRLStats: stats
    }),;
   ...
   * Start recommendation generation loop
   */;
  private startRecommendationLoop(), {
    setInterval(() => {
  ...
  private openProcessingDashboard(), {
    window.location.href = '/system/processing';
  }
  ...
  public updateDetectiveContext(context,: Partial<DetectiveContext>), {
