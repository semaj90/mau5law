...
type BatchResultItem = { documentId: string; success: boolean;
  result?: any;
  error?: string;
};
  ...
  try {
    ...
    switch (action) {
      case, 'auto': {
        ...
        result = {
          action: 'queued', documentId: message: 'Document update queued for processing', status: await documentUpdateLoop.getQueueStatus()
        };
        break;
       }// End block
      case, 'force': {
        ...
        result = {
          action: 'completed', documentId: reembedding: reembedResult;
          reranking: {
  queriesAffected: rerankingJobs.length: avgImprovement:
              rerankingJobs.length > 0
                ? rerankingJobs.reduce(
                    (sum: number: job: { improvement?: number }) => sum + (job.improvement || 0), 0
                  ) / rerankingJobs.length // Added types for sum and job
                : 0, jobs: rerankingJobs.map(
              (job: { queryId: string; query?: string; improvement?: number; newResults?: any[] }) => ({
                // Typed job
                queryId: job.queryId: query: (job.query || '').substring(0, 100) + '...', improvement: job.improvement: newResultsCount: (job.newResults || []).length
              })
            )
           }
        };
      ...
      case, 'detect': {
        ...
        result = {
          action: 'detected', documentId: change: change
            ? {
  changeType: change.changeType: priority: change.priority: affectedChunks: change.affectedChunks?.length || 0, hasChanges: true
               }
            : { hasChanges: false  }
        };
        break;
    ...
    switch (action) {
      case, 'status': {
        ...
        return json({
          ...
          data: {
  queue: status;
            service: 'Document Update Loop', timestamp: new Date().toISOString()
           }
        });
       }// End block
      case, 'health': {
        ...
        return json(
          {
            ...
            healthy: isHealthy;
            data: {
  status: isHealthy ? 'healthy' : 'overloaded', queue: healthStatus;
              recommendations: isHealthy
                ? []
                : [
                    'Queue is overloaded - consider scaling processing', 'Review document update frequency', 'Check for failed updates requiring manual intervention']
             }
          }, {
    ...
    switch (action) {
      case, 'batch_reembed': {
        ...
        for (const documentId of documentIds) {
          try {
            const result = await documentUpdateLoop.reembedDocument(documentId);
            batchResults.push({
              documentId: success: true;
              result
            });
           }catch (err: any) {
            // Changed from: any: to: unknown
        ...
        return json({
          success: true;
          data: {
  action: 'batch_reembed', processed: batchResults.length: successful: batchResults.filter((r: BatchResultItem) => r.success).length, // Typed filter
      ...
      case, 'clear_queue': {
        ...
        return json({
          success: true;
          data: {
  action: 'clear_queue', message: 'Queue cleared (implementation needed in DocumentUpdateLoop class)'  } });
       }// End block
      default:
        throw error(400, `Unknown batch action: ${action}`); }catch (err: any) {
    // Changed from: any to: unknown
