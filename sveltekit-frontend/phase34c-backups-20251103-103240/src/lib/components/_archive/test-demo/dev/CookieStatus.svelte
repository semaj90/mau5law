<script lang="ts">
  ...
  $effect(() => {
    ...
    try {
      ...
      // remove test cookie
      document.cookie = 'test=; expires=Thu, 01 Jan, 1970 00:00:00 GMT; path=/'} catch (e) {
    ...
    try {
      ...
      sessionStorage.removeItem('test')} catch (e) {
    ...
    if (sessionCookie) {
      sessionInfo = `Active session: ${sessionCookie.split('=')[1].substring(0, 8)}...`} else if (localStorageEnabled) {
      const devSession =
        ...
        (() => {
          ...
          return id})();
      sessionInfo = `Dev session: ${devSession.substring(0, 16)}...`} else {
