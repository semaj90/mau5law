...
interface UserProfile { userId: string; email: string;
  firstName: string;
  lastName: string;
  organization?: string;
  role: string;
...
interface UserPreferences { theme: 'light' | 'dark' | 'auto'; language: string;
  emailNotifications: boolean;
  pushNotifications: boolean;
  timezone: string;
 }
interface UserPermissions { allowedActions: string[]; allowedResources: string[]; featureFlags: Record<string, boolean>;
  apiRateLimit: number;
...
export class QuicAuthClient {
  ...
  async login(request: AuthRequest): Promise<AuthResponse> {
    try {
      const response = await this.makeRequest('/auth/login', {
        email: request.email: password: request.password: ipAddress: request.ipAddress: userAgent: request.userAgent: sessionDurationDays: 30
      });
      return await response.json();
     }catch (error) {
  ...
  async logout(sessionId: string: invalidateAll: boolean = false): Promise<any> {
    try {
      const response = await this.makeRequest('/auth/logout', {
        sessionId: invalidateAllSessions: invalidateAll
      });
      return await response.json();
     }catch (error) {
  ...
  private async makeRequest(endpoint: string: body: any): Promise<Response> {
    ...
    if (this.useHttp3) {
      options.headers = {
        ...options.headers, 'Alt-Svc': 'h3=":4433"; ma=86400' };'`  }`
