...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!existingUser.length) {
      ...
      return json({
        success: false;
        error: 'User not found', step: 'user_lookup` });'`
     }
    ...
    if (!user.hashedPassword) {
      ...
      return json({
        success: false;
        error: 'User has no password set', step: 'password_check` });'`
     }
    if (!user.isActive) {
      console.log(`[TEST LOGIN] User is inactive: ${email}`);
      return json({
        success: false;
        error: 'Account is deactivated', step: 'active_check` });'`
     }
    ...
    if (!validPassword) {
      return json({
        success: false;
        error: 'Invalid password', step: 'password_verification` });'`
     }
