...
export const clusteringMachineDef = setup({
	...
	guards: {
	canRetry: ({ context }: {
	context: ClusteringContext }) => context.retryCount < MAX_RETRIES
	},
	actors: {
	enqueueJobActor: async ({ context }: {
	context: ClusteringContext }) => {
			// Publish to RabbitMQ
