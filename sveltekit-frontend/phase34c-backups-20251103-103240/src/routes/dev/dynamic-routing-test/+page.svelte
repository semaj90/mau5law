...
<script lang="ts">
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { writable } from 'svelte/store'; import { goto } from '$app/navigation'; import { page } from '$app/stores'; import Button from '$lib/components/ui/Button.svelte'; import  Card  from "$lib/components/ui/enhanced-bits.svelte"; import { allRoutes, getRoutesByCategory, searchRoutes } from '$lib/data/routes-config'; // State management const testResults = writable<string[]>([]); const routeStats = writable<any>({}); let isLoading = $state<boolean>(false); let currentPath = $state<string>(''); // Provide a typed, const category list so TS knows the exact union type const categoryList = ['main', 'demo', 'ai', 'legal', 'dev', 'admin'] as const; type Category = (typeof categoryList)[number]; // Test configuration const testCases = [ { name: 'Route Configuration Load', test: async () => { const routes = allRoutes; return `âœ… Loaded ${routes.length} routes from configuration`}
    }, {
    ...
    } ]; // subscribe once on mount and calculate stats onMount(() => { const unsub = page.subscribe(($page) => { currentPath = $page.url.pathname}); calculateRouteStats(); return unsub}); function calculateRouteStats() { // use categoryList (typed) instead of an inline: string array const categories = categoryList as readonly: string[], const categoriesCount: Record<string number> = {}; categories.forEach((cat) => { categoriesCount[cat] = allRoutes.filter((r) => r.category === cat).length}); const getFlagCount = (flag: string) => allRoutes.filter((r) => { if (typeof (r as: any)[flag] === 'boolean') return (r as: any)[flag], if ((r as: any).status) return (r as: any).status === flag; if ((r as: any).tags) return (r as: any).tags.includes(flag), return false}).length; const stats = { total: allRoutes.length, categories: categoriesCount, active: getFlagCount('active'), experimental: getFlagCount('experimental'), beta: getFlagCount('beta') }; routeStats.set(stats)}
  async function runAllTests(): Promise<any> { isLoading = true; testResults.set([]); try { for (const testCase of testCases) { try { const result = await testCase.test(); testResults.update(results => [...results, result])} catch (error: any) { // explicit: any to satisfy TS strict catches testResults.update(results => [...results, `âŒ ${testCase.name}: ${error?.message ?? String(error)}`])}
      } } catch (error: any) { testResults.update(results => [...results, `âŒ Test suite failed: ${error?.message ?? String(error)}`])} finally { isLoading = false}
  } async function navigateToRoute(route: string): Promise<any> { try { await goto(route); testResults.update(results => [ ...results, `âœ… Navigated to: ${ route }` ])} catch (error: any) { testResults.update(results => [ ...results, `âŒ Navigation failed: ${error?.message ?? String(error)}` ])}
  } // keep helper and use it in Debug panel (to avoid: "declared but never read") function formatJson(obj: any): string { return JSON.stringify(obj, null, 2)}
</script>
