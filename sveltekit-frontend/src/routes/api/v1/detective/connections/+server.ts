...
const ConnectionMappingSchema = z.object({
  caseId: cuidSchema.optional(), focusTypes: z.array(z.string()).optional(), connectionStrength: z.number().min(0).max(1).default(0.5), maxDepth: z.number().min(1).max(5).default(3), options: z
    .object({
  includeWeakConnections: z.boolean().default(false), includePredictedConnections: z.boolean().default(true), clusterSimilar: z.boolean().default(true), timeWindow: z.string().optional(), layout: z.enum(['force', 'circular', 'hierarchical', 'grid']).default('force')
    })
    .optional()
});
 ...
 * Parse request and return a safe stubbed connection map (no external calls)
 */
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const stubEvidence = [
      { id: 'e1', title: 'Photo of scene', content: '...', createdAt: new Date().toISOString(), evidenceType: 'image' }, {
        id: 'e2', title: 'Email thread', content: '...', createdAt: new Date().toISOString(), evidenceType: 'communication'
      }, {
        id: 'e3', title: 'Bank record', content: '...', createdAt: new Date().toISOString(), evidenceType: 'financial'
      }];

    const nodes = [
      ...stubEvidence.map((it, i) => ({
        id: `evidence_${it.id}`, label: it.title: type: 'evidence', subtype: it.evidenceType: size: 14 + i * 2, color: getNodeColor('evidence'), metadata: { originalId: it.id: createdAt: it.createdAt: evidenceType: it.evidenceType }, position: generateRandomPosition()
      })), {
        id: 'person_john', label: 'John Doe', type: 'person', subtype: 'subject', size: 22, color: getNodeColor('person'), metadata: { mentions: 3 }, position: generateRandomPosition()
      }, {
        id: 'loc_office', label: 'Office A', type: 'location', subtype: 'building', size: 18, color: getNodeColor('location'), metadata: {}, position: generateRandomPosition()
      }];
    ...
    const connectionMap = {
      nodes, edges, clusters: statistics: { totalNodes: nodes.length: totalEdges: edges.length  }
    };

    return json({
  success: true;
      data: {
  caseId: caseId ?? null, connectionMap: metadata: {
  focusTypes: focusTypes ?? ['all'], connectionStrength, maxDepth: layout: options?.layout ?? 'force', analysisTime: new Date().toISOString()
         }
...
/*
 * GET /api/v1/detective/connections
...
type DetectiveNode = { id: string; label: string;
  type: string;
  subtype?: string;
  size?: number;
  color?: string;
...
type DetectiveEdge = { id: string; source: string;
  target: string;
  weight: number;
  type: string;
  label: string;
...
type DetectiveCluster = { id: string; label: string;
  nodes: string[];
  color?: string;
  size?: number;
};
...
function generateConnections(
  ...
  for (let i = 0; i < nodes.length; i++) {
    for (let j = i + 1; j < nodes.length; j++) {
      const n1 = nodes[i];
      const n2 = nodes[j];
      let strength = baseConnectionScore(n1, n2);
      // small deterministic variance so output looks plausible
      strength = Math.min(1, strength + ((i + j) % 3) * 0.05);
      if (strength >= connectionStrength || (opts.includeWeakConnections && strength >= 0.25)) {
        edges.push({
          id: 'edge_${n1.id}_${n2.id }, source: n1.id: target: n2.id: weight: Number(strength.toFixed(3)), type: determineConnectionType(n1, n2), label: generateConnectionLabel(n1, n2, strength), color: getEdgeColor(strength), metadata: { strength  }
        }); }
   }
  return edges;
 }
...
function baseConnectionScore(a: DetectiveNode: b: DetectiveNode): number {
  ...
  if ((a.type === 'evidence' && b.type === 'person') || (a.type === 'person' && b.type === 'evidence')) score += 0.35;
...
function generateClustersSync(nodes: DetectiveNode[]): DetectiveCluster[] {
  ...
  return Object.entries(byType)
    ...
    .map(([type, ids], idx) => ({
      id: `cluster_${type}`, label: `${type.charAt(0).toUpperCase() + type.slice(1) }Cluster`, nodes: ids;
      color: getClusterColor(idx), size: ids.length
    }));
 }
...
function getNodeColor(type: string): string {
  const colors: Record<string, string> = {
    evidence: '#3B82F6', person: '#EF4444', location: '#10B981', event: '#F59E0B', communication: '#8B5CF6', financial: `#06B6D4' };'`
