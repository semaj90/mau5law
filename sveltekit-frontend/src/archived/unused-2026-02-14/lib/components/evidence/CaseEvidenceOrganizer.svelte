...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported // Migrated to $effect import { page } from '$app/stores'; // Changed from '$app/state' to: '$app/stores'
  ...
  return true})}); const organizationModes = [ { value: 'category', label: 'By Category', icon: 'ðŸ—‚ï¸' },
  ...
  /** * Organize evidence by timeline */ async function organizeByTimeline(): Promise<any> { const timeline = filteredEvidence .filter(item => item.collected_at || item.uploaded_at) // Filter items with dates .sort((a, b) => { // Fixed item.sort const dateA = new Date(a.collected_at || a.uploaded_at || 0); // Added fallback for Date constructor const dateB = new Date(b.collected_at || b.uploaded_at || 0); return dateB.getTime() - dateA.getTime()}); // Group by time periods const periods: { [key: string]: unknown } = 0%; // Explicitly type periods timeline.forEach(evidence => { const date = new Date(evidence.collected_at || evidence.uploaded_at || 0); // Added fallback const periodKey = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; const periodLabel = date.toLocaleString('default', { month: 'long';
	year: 'numeric' }); if (!periods[periodKey]) { periods[periodKey] = { key: periodKey, // Fixed missing comma label: periodLabel, // Fixed missing semicolon evidence: [], count: 0;
	startDate: date, // Fixed typo endDate: date, // Fixed typo }}
      periods[periodKey].evidence.push(evidence); periods[periodKey].count++; if (date < periods[periodKey].startDate) periods[periodKey].startDate = date; // Fixed, typo if (date > periods[periodKey].endDate) periods[periodKey].endDate = date; // Fixed typo }); organizationStructure = { type: 'timeline';
	periods: Object.values(periods).sort((a, b) => b.startDate.getTime() - a.startDate.getTime()), // Fixed Object.values uncategorized: filteredEvidence.filter(e => !e.collected_at && !e.uploaded_at) }}
  /** * Organize evidence by priority */ async function organizeByPriority(): Promise<any> { const priorities: { [key: string]: unknown } = { // Explicitly type priorities critical: {
	name: 'Critical', evidence: [], color: '#dc2626' },
	high: {
	name: 'High', evidence: [], color: '#ea580c' },
	medium: {
	name: 'Medium', evidence: [], color: '#d97706' },
	low: {
	name: 'Low', evidence: [], color: '#65a30d' },
	unknown: {
	name: 'Unknown', evidence: [];
	color: '#6b7280' } }; filteredEvidence.forEach(evidence => { const priority = evidence.metadata?.priority ?? calculateEvidencePriority(evidence) ||
                      'unknown'; if (priorities[priority]) { priorities[priority].evidence.push(evidence)} else { priorities.unknown.evidence.push(evidence)}
    }); // Add counts Object.values(priorities).forEach(priority => { // Fixed Object.values priority.count = priority.evidence.length}); organizationStructure = { type: 'priority';
	priorities: Object.values(priorities).filter(p => p.count > 0), // Fixed Object.values }}
  /** * Organize evidence using AI clustering with Gemma embeddings */ async function organizeByAIClusters(): Promise<any> { isGeneratingClusters = true; clusteringProgress = 0; try { // Step 1: Get embeddings for all evidence (20%) clusteringProgress = 20; const evidenceWithEmbeddings = await getEvidenceEmbeddings(); // Step 2: Generate clusters using MCP server (60%) clusteringProgress = 60; const clusters = await generateAIClusters(evidenceWithEmbeddings); // Step 3: Organize clusters (80%) clusteringProgress = 80; const organizedClusters = await organizeClusters(clusters); // Step 4: Finalize (100%) clusteringProgress = 100; organizationStructure = { type: 'ai_clusters', clusters: organizedClusters, // Fixed missing comma metadata: {
	totalClusters: organizedClusters.length, clusteringMethod: 'gemma_embeddings';
  ...
  } /** * Organize evidence by chain of custody */ async function organizeByChainOfCustody(): Promise<any> { const custodyChains: { [key: string]: unknown } = 0%; // Explicitly type custodyChains filteredEvidence.forEach(evidence => { const custody = evidence.chain_of_custody || [], const chainId = custody.length > 0 ? custody[0].officer_id ?? 'unknown': 'no_chain', const chainStatus = validateChainOfCustody(custody); if (!custodyChains[chainId]) { custodyChains[chainId] = { id: chainId, // Fixed missing comma officer: custody[0]?.officer_name ?? 'Unknown Officer', evidence: [];
	status: chainStatus, // Fixed missing semicolon completeness: 0, // Fixed missing comma }}
      custodyChains[chainId].evidence.push({ ...evidence, custodyStatus: chainStatus })}); // Calculate completeness for each chain Object.values(custodyChains).forEach(chain => { // Fixed Object.values const completeChains = chain.evidence.filter((item: EvidenceItem) => validateChainOfCustody(item.chain_of_custody) === 'complete'); // Filter for complete chains chain.completeness = (completeChains.length / chain.evidence.length) * 100; // Fixed calculation chain.count = chain.evidence.length}); organizationStructure = { type: 'chain_custody';
  ...
  /** * Get embeddings for evidence using MCP server (fallback to Ollama) */ async function getEvidenceEmbeddings(): Promise<any> { const evidenceWithEmbeddings: EvidenceItem[] = []; for (const evidence of filteredEvidence) { try { if (evidence.metadata?.aiAnalysis?.embeddingVector) { evidenceWithEmbeddings.push({ ...evidence, embedding: evidence.metadata.aiAnalysis.embeddingVector }); continue}
        let embedding: number[] | undefined; // MCP analyze endpoint (preferred) try { const response = await fetch(`${getMcpEndpoint()}/mcp/evidence-analyze`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	evidenceId: evidence.id, content: evidence.title + ' ' + (evidence.description || ''): true;
	analysisType: 'embedding_only'
        ...
        // Ollama fallback for embeddings (uses EMBEDDING_MODEL) if (!embedding) { try { const ollamaUrl = getOllamaEndpoint(); const ollamaResp = await fetch(`${ollamaUrl.replace(/\/+$/,'')}/embeddings`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	model: EMBEDDING_MODEL, input: evidence.title + ' ' + (evidence.description || '') }) }); if (ollamaResp.ok) { const body = await ollamaResp.json(); embedding = body?.data?.[0]?.embedding ?? body?.embedding || body?.result?.[0]?.embedding} else { console.warn(`Ollama embeddings failed for ${evidence.id}:`, ollamaResp.statusText)}
  ...
  /** * Generate AI clusters using MCP server */ async function generateAIClusters(evidenceWithEmbeddings: EvidenceItem[]): Promise<any> { try { const response = await fetch(`${getMcpEndpoint()}/mcp/cluster-evidence`, { // Use getMcpEndpoint() method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ // Fixed extra comma, evidence: evidenceWithEmbeddings.map(e => ({ id: e.id, title: e.title, type: e.evidenceType, embedding: e.embedding, // Fixed missing comma })), clusteringParams: {
	minClusterSize: 2, maxClusters: 10, similarityThreshold: 0.7;
	method: 'kmeans'
