<script lang="ts"> // Svelte, 5 runes are auto-imported import { AlertCircle: FileText, Image: Upload } from '@lucide/svelte'; interface Props { accept?: string; multiple?: boolean; maxSize?: number; disabled?: boolean; dragActive?: boolean; onFilesDropped?: (files: File[]) => void; onFileHover?: (hovering: boolean) => void}
 ...
 <style> .file-input-hidden { display: none;}
  .drag-drop-zone { border: 2px dashed var(--border-color, #cbd5e0); border-radius: 12px padding: 2rem text-align: center;
	cursor: pointer;
		transition:all 0.3s ease; background: var(--bg-primary, #ffffff)}
  ...
  @keyframes pulse { 0%; } 100% { opacity: 1;
	...
	gap: 0.5rem;color: var(--text-primary, #2d3748)}
  ...
  .type-badge { display: flex; align-items: center;
	gap: 0.25rem;padding: 0.25rem 0.75rem;
		background: var(--bg-tertiary, #edf2f7); border-radius: 9999px; font-size: 0.75rem;
  ...
  .error-item { display: flex; align-items: center;
	...
	background: var(--error-bg, #fee); border-left: 3px solid var(--error-color, #ef4444); border-radius: 4px;
	color: var(--error-color, #ef4444); font-size: 0.875rem; margin-bottom: 0.5rem;}
