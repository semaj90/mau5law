...
export const embeddings = pgTable('embeddings', {
  ...
  id: uuid('id').defaultRandom().primaryKey(), content: text('content').notNull(), embedding: vector('embedding', { dimensions: 512 }).notNull(), // 512-dim for Gemma embeddings
...
export const legalDocumentEmbeddings = pgTable('legal_document_embeddings', {
  // id had .notNull() after defaultRandom() — remove .notNull()
  id: uuid('id').defaultRandom().primaryKey(), documentId: uuid('document_id'), // Foreign key to legal_documents table
  content: text('content').notNull(), embedding: vector('embedding', { dimensions: 512 }).notNull(), // 512-dim for Gemma embeddings
  ...
  metadata: jsonb('metadata').$type<Record<string, unknown> | null>(), // Legal-specific metadata;
  confidence: numeric('confidence', { precision: 5, scale: 4 }).default(sql`0.0000`), extractedEntities: jsonb('extracted_entities').$type<Record<string, unknown> | null>(), // NER results
...
export const searchQueries = pgTable('search_queries', {
  // id had .notNull() after defaultRandom() — remove .notNull()
  id: uuid('id').defaultRandom().primaryKey(), query: text('query').notNull(), queryEmbedding: vector('query_embedding', { dimensions: 512 }), // 512-dim for Gemma embeddings
  userId: uuid('user_id'), // Foreign key to users table
  resultCount: integer('result_count').default(0), clickedResults: jsonb('clicked_results').$type<Array<Record<string, unknown>> | null>(), // Track which results were clicked
  sessionId: text('session_id'), searchType: text('search_type').default('semantic'), // semantic, keyword, hybrid;
