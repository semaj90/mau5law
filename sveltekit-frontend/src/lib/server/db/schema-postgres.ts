...
export const evidence = pgTable(
 ...
 title: varchar('title', { length: 255 }).notNull(, description: text('description', evidenceType: evidenceTypeEnum('evidence_type').notNull(, type: varchar('type', { length: 50 }, evidenceNumber: integer('evidence_number', summary: text('summary', posX: doublePrecision('pos_x', posY: doublePrecision('pos_y', collectedAt: timestamp('collected_at', { mode: 'string' }, collectedBy: uuid('collected_by', verifiedAt: timestamp('verified_at', { mode: 'string' }, verified: boolean('verified', createdAt: timestamp('created_at', { mode: 'string' }, fileSize: integer('file_size', mimeType: varchar('mime_type', { length: 100 }, hash: varchar('hash', { length: 255 }, tags: jsonb('tags', aiAnalysis: jsonb('ai_analysis', aiTags: jsonb('ai_tags', aiSummary: text('ai_summary', fileType: varchar('file_type', { length: 50 }, subType: varchar('sub_type', { length: 50 }, fileUrl: text('file_url', fileName: varchar('file_name', { length: 255 }, canvasPosition: jsonb('canvas_position').default({}).notNull(, uploadedBy: uuid('uploaded_by'), // Foreign key to users.id
...
export const canvasStates = pgTable('canvas_states', {
	id: uuid('id').default(sql`gen_random_uuid()`).primaryKey().notNull(, caseId: uuid('case_id').references(() => cases.id, { onDelete: 'cascade' }).notNull(, name: varchar('name', { length: 255 }).default('Untitled Board').notNull(, canvasData: jsonb('canvas_data').notNull(, version: integer('version').default(1, isDefault: boolean('is_default').default(false, createdBy: uuid('created_by'), // FK to users.id (references users.id if available)
	createdAt: timestamp('created_at', { mode: 'string' }).defaultNow().notNull(, updatedAt: timestamp('updated_at', { mode: 'string' }).defaultNow().notNull(),
});
...
export const savedReports = pgTable('saved_reports', {
 ...
 caseId: uuid('case_id'), // FK to cases.id
 savedAt: timestamp('saved_at').defaultNow().notNull(, notes: text('notes', exportFormat: varchar('export_format', { length: 50 }, version: integer('version', wordCount: integer('word_count', tags: jsonb('tags', metadata: jsonb('metadata', sharedWith: jsonb('shared_with', lastExported: timestamp('last_exported', { mode: 'string' }, createdBy: uuid('created_by', createdAt: timestamp('created_at').defaultNow(, updatedAt: timestamp('updated_at').defaultNow(),
});
...
export const userEmbeddings = pgTable('user_embeddings', {
 ...
 .default(sql`gen_random_uuid()`)
 .primaryKey()
 .notNull(, userId: uuid('user_id').notNull(, content: text('content', metadata: jsonb('metadata', embedding: text('embedding').notNull(), // Store vector as text
 model: varchar('model', { length: 100 }).notNull(, createdAt: timestamp('created_at').defaultNow().notNull(),
});
...
export const canvasStatesRelations = relations(canvasStates, ({ one, many }) => ({
 case: one(cases, { fields: [canvasStates.caseId], references: [cases.id] }, user: one(users, { fields: [canvasStates.createdBy], references: [users.id] }, annotations: many(canvasAnnotations, autosaves: many(canvasAutosaves),
}));
...
export const workspaceEvidence = pgTable(
 ...
 .notNull()
 .references(() => evidence.id, { onDelete: 'cascade' }, relevanceScore: real('relevance_score').default(0, addedBy: varchar('added_by', { length: 50 }).default('user'), // 'system', 'user'
  createdAt: timestamp('created_at', { withTimezone: true })
    .default(sql`now()`)
    .notNull(),
 },
...
export const workspaceStatutes = pgTable(
 ...
 relevanceScore: real('relevance_score').default(0, source: varchar('source', { length: 50 }).default('user'), // 'ai', 'user', 'citation'
  createdAt: timestamp('created_at', { withTimezone: true })
 .default(sql`now()`)
 .notNull(),
 },
...
export const workspaceCitations = pgTable(
 ...
 .notNull(, workspaceId: uuid('workspace_id')
 .notNull()
 .references(() => workspaces.id, { onDelete: 'cascade' }, messageId: uuid('message_id').references(() => ragMessages.id, { onDelete: 'cascade' }, citationText: text('citation_text').notNull(), // e.g., "Penal Code 187(a)", citationURL: text('citation_url', citationType: varchar('citation_type', { length: 50 }).default('statute'), // 'statute', 'case', 'regulation', 'precedent'
  createdAt: timestamp('created_at', { withTimezone: true })
 .default(sql`now()`)
 .notNull(),
 },
...
export const ragIndexMetadata = pgTable('rag_index_metadata', {
	id: uuid('id').default(sql`gen_random_uuid()`).primaryKey().notNull(, chunkId: uuid('chunk_id', evidenceId: uuid('evidence_id', qdrantPointId: text('qdrant_point_id', tags: text('tags').array(, jurisdiction: text('jurisdiction', indexedAt: timestamp('indexed_at', { withTimezone: true, mode: 'string' }, tagWeight: real('tag_weight', embeddingModel: text('embedding_model', createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }).defaultNow(, updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' }).defaultNow(),
});

export const citationTags = pgTable('citation_tags', {
	id: uuid('id').default(sql`gen_random_uuid()`).primaryKey().notNull(, name: text('name').notNull(, jurisdiction: text('jurisdiction', description: text('description', createdAt: timestamp('created_at', { withTimezone: true, mode: 'string' }).defaultNow(, namespace: text('namespace', synonyms: text('synonyms').array(, updatedAt: timestamp('updated_at', { withTimezone: true, mode: 'string' }).defaultNow(),
});
