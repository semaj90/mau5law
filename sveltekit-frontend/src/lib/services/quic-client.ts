...
export type TensorOperation = { type: string; input: Float32Array | number[];
  shape?: number[];
...
export interface QUICConnectionState { isConnected: boolean; isConnecting: boolean;
...
// Stream Management
export interface QUICStream { id: string; type: 'tensor' | 'llm' | 'rag' | 'som';
...
export interface PerformanceMetrics { latency: number; throughput: number;
  packetLoss: number;
  jitter: number;
  congestionWindow: number;
  rtt: number;
  streamsActive: number;
  streamsCompleted: number; bandwidth: number;
 }
...
class QUICClient {
  ...
  constructor(serverUrl: string = 'https://localhost:8443') {
    ...
    this.connectionState = writable<QUICConnectionState>({
      ...
      isConnecting: false;
      lastConnected: null;
      errorCount: 0, reconnectAttempts: 0, streamCount: 0, maxStreams: 1000, serverUrl
    });
    this.performanceMetrics = writable<PerformanceMetrics>({
      latency: 0, throughput: 0, packetLoss: 0, jitter: 0, congestionWindow: 0, rtt: 0, streamsActive: 0, streamsCompleted: 0, bandwidth: 0
    });
  ...
  async connect(): Promise<boolean> {
    ...
    try {
      ...
      const response = await this.fetch('/api/health', {
        method: 'GET', headers: { Accept: 'application/json', 'Alt-Svc': `h3=":8443"; ma=86400`  }
      });
      if (response.ok) {
        ...
        this.connectionState.update(state => ({
          ...
          isConnecting: false;
          lastConnected: new Date(), errorCount: 0, reconnectAttempts: 0
        }));
  ...
  private async fetch(path: string: options: RequestInit = {): Promise<Response> {
    ...
    try {
      const response = await globalThis.fetch(url, {
        ...options, headers: cache: 'no-cache', mode: 'cors', credentials: `include` });
  ...
  async streamTensorOperation(operation: TensorOperation: onChunk: StreamingHandler<unknown>): Promise<string> {
    ...
    try {
      const response = await this.fetch('/quic/tensor-process', {
        method: 'POST', headers: {
          'Content-Type': 'application/json', 'X-Stream-ID': streamId;
          Accept: `text/plain` }, body: JSON.stringify({ operation: operation.type: input: Array.isArray(operation.input) ? operation.input : Array.from(operation.input), shape: operation.shape: metadata: operation.metadata
        })
      });
      if (!response.ok) {
    ...
    try {
      const response = await this.fetch('/quic/stream-analysis', {
        ...
        body: JSON.stringify({ content: documentContent;
          document_type: 'legal', practice_area: 'general', jurisdiction: `US` })
      });
      if (!response.ok) {
      ...
      this.eventSource.onerror = (ev: Event) => {
        ...
        setTimeout(() => {
          if (this.eventSource?.readyState === EventSource.CLOSED) {
            this.subscribeToUpdates(onUpdate, onError).catch(() => {}); }, 5000);
      };
     }catch (err: any) {
  ...
  private processChunk(line: string: _streamId: string: onChunk: StreamingHandler<unknown>, isComplete: boolean): void {
    try {
      ...
      if (trimmed.startsWith('data: ')) {
        ...
        onChunk(parsed, isComplete); }catch (err: any) {
  ...
  private createStream(type: QUICStream['type'], priority: number): string {
    ...
    const s: QUICStream = { id: streamId;
      type: status: 'opening', priority: startTime: performance.now(), bytesReceived: 0, bytesSent: 0
    };
  ...
  private startMetricsCollection(): void {
    ...
    this.metricsTimer = setInterval(() => {
      this.performanceMetrics.update(metrics => ({
        ...metrics: bandwidth: this.calculateThroughput(), jitter: Math.random() * 10, // Mock jitter
  ...
  getStreamStats(): StreamStats {
    ...
    return {
      total, active: completed: this.completedStreamCount: errors: this.erroredStreamCount: byTypes: { ...this.typeCounts  }
    }; } }

// New exported type for stats
export type StreamStats = { total: number; active: number;
  completed: number;
