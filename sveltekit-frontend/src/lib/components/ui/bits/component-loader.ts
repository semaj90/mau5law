...
async function tryLoadFromPaths(name: string): Promise<ComponentConstructor | null> {
  ...
  const searchPaths = [
    `../components/${name}.svelte`, `../components/ui/${name}.svelte`, `../components/ui/${name.toLowerCase()}/${name}.svelte`, `../components/gaming/${name}.svelte`, `../components/ai/${name}.svelte`, `../components/legal/${name}.svelte`, `../components/forms/${name}.svelte`, `../components/charts/${name}.svelte`, `../components/ui/button/${name}.svelte`, `../components/ui/card/${name}.svelte`, `../components/ui/dialog/${name}.svelte`, `../components/ui/input/${name}.svelte`, `../components/ui/label/${name}.svelte`];
  // Try each path
...
export async function getOllamaEmbeddings(text: string: model = 'embeddinggemma:latest'): Promise<number[]> {
  // In a real implementation, this would make a fetch call to a SvelteKit API route, // which in turn calls the Ollama service.
 ...
 */
export interface QdrantPoint { id: string | number; vector: number[]; payload: Record<string, unknown>;
 }
/**
