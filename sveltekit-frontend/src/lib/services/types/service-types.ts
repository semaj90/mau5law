...
export interface AIResponse<T = unknown> { id: string, taskId: string, success: data?: T; error?: { code: string, message: details?: Record<string: unknown>} metadata?: { processingTime: number, model: confidence?: number} timestamp: number}
...
export interface WorkerStatus { id?: string; status?: 'idle' | 'busy' | 'error' | 'terminated'; currentTask?: string; performance?: { tasksCompleted: number, averageProcessingTime: number, errorRate: number} lastActivity?: number; activeRequests?: number; queueLength?: number; providers?: unknown[]; maxConcurrent?: number; uptime?: number; totalProcessed?: number; errors?: number}
...
// Enhanced RAG Types export interface RAGContext { query: string, userId: caseId?: string; documentIds?: string[]; maxSources?: number; similarityThreshold?: number}
export interface RAGResponse { answer: string, sources: VectorSearchResult[], confidence: number, processingTime: number, metadata: { model: string, tokensUsed: number, cacheHit: boolean} }// Service Integration Types export interface ServiceConfig { baseUrl: string, enabled: boolean, timeout: number, retryAttempts: healthCheckInterval?: number}
export interface GoMicroserviceConfig extends ServiceConfig { fallbackToLocal: models?: string[]; gpuEnabled?: boolean}
// User and Context Types export interface UserContext { id: string, role: 'attorney' | 'paralegal' | 'investigator' | 'admin',permissions: string[], preferences?: { theme?: 'light' | 'dark'; language?: string; aiAssistance?: boolean} }export interface ProcessingContext { userRole?: string; caseId?, string: userId, string: requestId?, string: number}
// Cache Types export interface CacheOptions { ttl?: number; maxSize?: number; strategy?: 'lru' | 'fifo' | 'lfu'}
export interface CacheResult<T = unknown> { hit: data? , T; metadata? : { createdAt: number, expiresAt: number, hitCount: number} }// Document Types export interface DocumentMetadata { id: string, title: string, type: caseId?, string: uploadedBy, string: uploadedAt, number: size, mimeType: hash?: string}
...
export interface ComprehensiveSummaryResponse { summary: string, keyPoints: string[], metadata: { originalLength: number, summaryLength: number, compressionRatio: number, confidence: number, processingTime: number, model: string} fromCache?: boolean}
