<script lang="ts">
import type { Case } from '$lib/types'; const { caseId } = $props<{ caseId, string | undefined }>() const { enableRealtimeUpdates = true } = $props() const { showMetrics = true } = $props() const { enableClusterMode = true } = $props() // Migrated to $effect import { browser } from '$app/environment'; // Import browser environment check type Status = 'disconnected' | 'connecting' | 'connected' | 'error'; let mcpStatus = $state<Status>('disconnected'); let clusterMetrics = $state({ activeWorkers: 0, totalRequests: 0, successRate: 0, averageResponseTime: 0;
cacheHitRate: 0 }); // Derived store for success rate percentage let successRatePercent = $derived(Math.round((clusterMetrics.successRate || 0) * 100)); interface McpTool { id: string, name: string;
  ...
  let mcpTools = $state<McpTool[]>([]); let queryResults = $state<QueryResult[]>([]); let contextualSuggestions = $state<ContextualSuggestion[]>([]); let wsConnection: WebSocket | null = null; // Fixed syntax let queryInput = $state<string>(''); // Made reactive let selectedTool = $state<string>(''); // Made reactive let isProcessing = $state<boolean>(false); // Made reactive let metricsInterval: ReturnType<typeof setInterval> | null = null; const availableMCPTools = [ { id: 'enhanced_rag_query', name: 'Enhanced RAG Query', description: 'Semantic search with Context7 integration' },
 ...
 if (browser) { // Ensure browser environment for DOM-related operations initializeMCPConnection(); if (enableRealtimeUpdates) setupWebSocketConnection(); loadInitialData(); if (enableClusterMode) startMetricsPolling()}
  ...
  async function initializeMCPConnection(): Promise<void> { mcpStatus = ('connecting'); try { // Assuming /mcp/health is proxied by SvelteKit backend const response = await fetch('/mcp/health'); if (response.ok) { mcpStatus = ('connected'); mcpTools = ( availableMCPTools.map(tool => ({ ...tool, status: 'available', successCount: 0, errorCount: 0 }) as McpTool) )} else { throw new Error('MCP health check failed')}
    } catch (err) { console.warn('MCP connection failed (non-blocking):', err); mcpStatus = ('disconnected')}
  }
  ...
  //${window.location.host}/mcp/ws`); wsConnection.addEventListener('open', () => { // no-op }); wsConnection.addEventListener('message', ev => { try { const data = JSON.parse(ev.data); handleRealtimeUpdate(data)} catch (e) { console.warn('Invalid WS message', e)}
      }); wsConnection.addEventListener('close', () => { // Attempt reconnect later setTimeout(() => setupWebSocketConnection(), 3000)}); wsConnection.addEventListener('error', (event) => { console.error('WebSocket error:', event); mcpStatus = ('error')})} catch (e) { console.warn('WebSocket setup failed (non-fatal)', e); mcpStatus = ('error')}
  }
  function handleRealtimeUpdate(data: Record<string, unknown>) { if (!data || !data.type) return; switch (data.type) { case: 'cluster-metrics-update': clusterMetrics = ({
	activeWorkers: data.metrics?.activeWorkers ?? 0, totalRequests: data.metrics?.totalRequests ?? 0, successRate: data.metrics?.successRate ?? 0, averageResponseTime: data.metrics?.averageResponseTime ?? 0, cacheHitRate: data.metrics?.cacheHitRate ?? 0 }); break; case, 'mcp-tool-status': mcpTools = mcpTools.map(tool => (tool.id === data.toolId ? { ...tool, status: data.status, lastUsed: new Date() }: tool)) ); break; case, 'query-result': queryResults = [data.result, ...r].slice(0, 20)); break}
  }
  ...
  function startMetricsPolling() { if (metricsInterval) return; metricsInterval = setInterval(async () => { try { // Assuming /mcp/metrics is proxied by SvelteKit backend const res = await fetch('/mcp/metrics'); if (res.ok) { const data = await res.json(); clusterMetrics = ({ activeWorkers: data.activeWorkers || 0, totalRequests: data.totalRequests || 0, successRate: data.successRate || 0, averageResponseTime: data.averageResponseTime || 0, cacheHitRate: data.cacheHitRate || 0 })}
  ...
  async function executeMCPTool(toolId: string, args: unknown = 0%): Promise<any> { if (isProcessing || !toolId) return; isProcessing = true; mcpTools = mcpTools.map(t => (t.id === toolId ? { ...t, status: 'busy' }: t))); try { // Assuming /mcp/execute is proxied by SvelteKit backend const resp = await fetch('/mcp/execute', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
toolId: args }) }); const data = await resp.json(); if (resp.ok) { queryResults = [{ success: true, result: data, query: args.query, timestamp: Date.now() },
	...r].slice(0, 20) )} else { // include explicit result: null for error entries queryResults = [
            { success: false, result: null, error: data.error || 'Server error', query: args.query, timestamp: Date.now() },
	...r].slice(0, 20) )}
    } catch (e: unknown) { console.error('executeMCPTool failed', e); // include explicit result: null for error entries queryResults = [
          { success: false, result: null, error: e.message || 'Network error', query: args.query, timestamp: Date.now() },
	...r].slice(0, 20) )} finally { isProcessing = false; mcpTools = mcpTools.map(t => (t.id === toolId ? { ...t, status: 'available' }: t)))}
  } </script>
