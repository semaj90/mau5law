...
export async function demoQueryLLM(query: RAGDemoQuery): Promise<RAGDemoResponse> {
  ...
  caseData.evidence.forEach(evidence => {
    ...
    if (relevance > 0.3) {
      ...
      reasoning.push(`Found evidence: ${evidence.filename} (${Math.round(relevance * 100)}% relevance)`);
    }
  });
...
function generateEvidenceResponse(caseData: any, sources: any[]): string {
  const evidenceTypes = Array.from(new Set(sources.map((s: any) => {
...
function generateTimelineResponse(caseData: any, sources: any[]): string {
  const timelineEvents = caseData.evidence.sort((a: any, b: any) => new Date(a.uploadedAt).getTime() - new Date(b.uploadedAt).getTime()).slice(0, 4);
  return "Based on the evidence timeline, here's the sequence:\n\n" + timelineEvents.map((evidence: any, i: number) => {
...
function generateGeneralResponse(query: string, caseData: any, sources: any[]): string {
  ...
  return "Regarding your query: \"" + query + "\", I've analyzed " + sources.length + " relevant pieces of evidence.\n\n" + relevantSources.map((source: any) => {
  const evidence = caseData.evidence.find((e: any) => e.id === source.id);
