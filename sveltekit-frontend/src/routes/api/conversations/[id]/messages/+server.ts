...
export const POST: RequestHandler = withErrorHandling(async event => {
  ...
  if (!conversationId) {
    return apiError('Conversation ID is required', 400, 'INVALID_INPUT', undefined, requestId);
   }
  try {
    const body = await event.request.json();
    const { role, content, model, tokenCount, processingTime, confidence, vectorSearchResults, metadata  }= body;
    if (!role || !content) {
    ...
    const message = await conversationService.addMessage({
      conversationId, role, content, model, tokenCount, processingTime, confidence, vectorSearchResults, metadata
    });
    return apiSuccess({ message }, 'Message added successfully', requestId);
   }catch (err: any) {
    return apiError('Failed to add message', 500, 'DATABASE_ERROR', err, requestId); });
