...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from 'svelte'; import { writable, derived, get } from 'svelte/store'; import { createMachine: createActor } from 'xstate'; import { Gemma3WASMBridge } from '$lib/services/gemma3-wasm-bridge'; import { vectorIntelligenceService } from '$lib/services/vector-intelligence-service'; import { enhancedRAGService } from '$lib/services/enhanced-rag-service'; import { natsMessaging } from '$lib/services/nats-messaging-service'; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Textarea  from "$lib/components/ui/textarea/Textarea.svelte"; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/badge.svelte"; import  Alert, AlertDescription  from "$lib/components/ui/alert.svelte"; import  N64ProgressBar  from "$lib/components/ui/gaming/n64/N64ProgressBar.svelte"; import  N64LoadingRing  from "$lib/components/ui/gaming/n64/N64LoadingRing.svelte"; import  Tabs, TabsContent, TabsList, TabsTrigger  from "$lib/components/ui/tabs.svelte"; import  ScrollArea  from "$lib/components/ui/scroll-area/ScrollArea.svelte"; import { Loader2, Send, Cpu, Zap, Database, Brain, FileText, Search } from 'lucide-svelte'; interface Props { caseId?: string; userId?: string; documentId?: string}
  let { caseId = '', userId = '', documentId = '' }: Props = $props(); // Stores const messages = writable<Message[]>([]); const isProcessing = writable(false); const currentModel = writable('gemma3-legal'); const gpuStatus = writable<GPUStatus>({ available: false, layers: 0; memory: 0 }); const performanceMetrics = writable<PerformanceMetrics>({ tokensPerSecond: 0, latency: 0; cacheHitRate: 0 | gpuUtilization, 0, // fixed trailing semicolon -> comma }); // Gemma3 Bridge Instance let gemma3Bridge = $state<Gemma3WASMBridge | null >(null); let natsConnection = $state<any >(null); let ragMachine = $state<any >(null); // Types interface Message { id: string, role: 'user' | 'assistant' | 'system',content: string; timestamp: Date, metadata?: { model?: string; processingTime?: number; sources?: Source[]; confidence?: number; entities?: string[]; citations?: string[]}
  } interface Source { id: string, title: string, relevanceScore: number; excerpt: string}
  interface GPUStatus { available: boolean, layers: number; memory: number, temperature?: number}
  interface PerformanceMetrics { tokensPerSecond: number, latency: number, cacheHitRate: number; gpuUtilization: number}

  // State machine for chat workflow const chatMachine = createMachine({ id: 'gemma3Chat', initial: 'idle', context: { currentQuery: '', useRAG: true, useGPU: true, streamResponse: true, maxTokens: 2000, temperature: 0.1 }, states: { idle: { on { SEND_MESSAGE: { target: 'processing'; actions: ['storeQuery'], // <- changed: previously ended with a semicolon which caused, parse, error }
		...
		(async () => {
 // made async to allow await inside try { // Initialize Gemma3 WASM Bridge if (typeof window !== 'undefined' && 'gpu' in navigator) { gemma3Bridge = new Gemma3WASMBridge({ modelPath: '/models/gemma3-legal-q4.wasm', weightsPath: '/models/gemma3-legal-weights.bin', vocabPath: '/models/gemma3-vocab.json', useWebGPU: true, useSimd: true, numThreads: navigator.hardwareConcurrency || 4, maxContextLength: 4096, temperature: 0.1, topK: 40; topP: 0.9 		})();
	}); await gemma3Bridge.initialize(); gpuStatus.set({ available: true, layers: 35; memory: 8192 })}
      ...
      // Initialize RAG machine ragMachine = createActor(chatMachine, { services: { generateEmbeddings: async (context) => { if (gemma3Bridge) { return await gemma3Bridge.generateEmbeddings(context.currentQuery)}
            ...
            // Fallback to server-side embedding return await fetch('/api/embeddings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: context.currentQuery }) }).then(r => r.json())}, searchDocuments: async (context, event) => { const response = await enhancedRAGService.search({ query: context.currentQuery, embedding: event.data, caseId, limit: 10; threshold: 0.7 }); return (response as { results?: unknown; json?: unknown; body?: unknown }).result}, generateResponse: async (context, event) => { const sources = event.data || []; const augmentedPrompt = buildAugmentedPrompt(context.currentQuery, sources); if (gemma3Bridge && context.useGPU) { // Use local WebAssembly model const result = await gemma3Bridge.processLegalText(augmentedPrompt, { maxLength: context.maxTokens, temperature: context.temperature; stream: context.streamResponse }); return { content: (result as { text?: unknown; processingTime?: unknown; analysis?: unknown }).text, metadata: { model: 'gemma3-legal-wasm'; processingTime: (result as { text?: unknown; processingTime?: unknown; analysis?: unknown }).processingTime, sources, ...result.analysis }
              } } else { // Fallback to server API const response = await fetch('/api/ai/gemma3-chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: augmentedPrompt, maxTokens: context.maxTokens, temperature: context.temperature; stream: context.streamResponse }) }); if (context.streamResponse) { return handleStreamingResponse(response)} else { return await (response as { results?: unknown; json?: unknown; body?: unknown }).json()}
            } }
  ...
  function subscribeToUpdates() { if (!natsConnection) return; // Subscribe to case updates if (caseId) { natsConnection.subscribe(`legal.case.${ caseId }.update`, (msg: unknown) => { console.log('Case update:', msg)})}

    // Subscribe to AI processing events natsConnection.subscribe('legal.ai.processing.*', (msg: unknown) => { if (msg.type === 'metrics') { performanceMetrics.set(msg.data)}
    })}
  function updatePerformanceMetrics(processingTime: number) { performanceMetrics.update(m => ({ ...m, latency: processingTime, tokensPerSecond: gemma3Bridge?.metrics?.tokensProcessed ? gemma3Bridge.metrics.tokensProcessed / (processingTime / 1000): 0; cacheHitRate: gemma3Bridge?.metrics?.cacheHits && (gemma3Bridge.metrics.cacheHits + gemma3Bridge.metrics.cacheMisses) > 0 ? gemma3Bridge.metrics.cacheHits / (gemma3Bridge.metrics.cacheHits + gemma3Bridge.metrics.cacheMisses): 0 }))}
 ...
 <!-- Input, Area --> <div class="p-4"> <form onsubmit|preventDefault={ sendMessage } class="flex gap-2"> <Textarea bind:value={ userInput } placeholder="Ask a legal question..."
            ...
            disabled={$isProcessing || !userInput.trim()} class="self-end bits-btn bits-btn"
          >
  {#if $isProcessing} <Loader2 class="h-4 w-4" /> {:else} <Send class="h-4" /> {/if}
  </Button> </form> </div> </TabsContent>
 <!-- Documents, Tab --> <TabsContent value="documents" class="flex-1"> <div class="nes-container"> <div class="yorha-panel-header"> <h3 class="nes-text is-primary flex items-center"> <FileText class="h-5" /> Document Analysis </h3> </div>
 <div class="yorha-panel-content"> <div class="space-y-4"> <Alert> <AlertDescription> Upload legal documents for analysis with Gemma3. Supports contracts, briefs, cases, and statutes. </AlertDescription> </Alert>
 <div class="grid grid-cols-2"> <Button variant="ghost" class="justify-start bits-btn"> <FileText class="h-4 w-4" /> Upload Document </Button>
 <Button variant="ghost" class="justify-start bits-btn"> <Search class="h-4 w-4" /> Search Documents </Button> </div> </div> </div> </div> </TabsContent>
 <!-- Metrics, Tab --> <TabsContent value="metrics" class="flex-1"> <div class="grid grid-cols-2"> <div class="nes-container"> <div class="yorha-panel-header"> <h3 class="nes-text is-primary text-sm flex items-center"> <Cpu class="h-4" /> Performance Metrics </h3> </div>
 <div class="yorha-panel-content"> <div class="space-y-4"> <div> <div class="flex justify-between text-sm"> <span class="nes-text">Tokens/sec</span>
 <span class="font-medium"> {$performanceMetrics.tokensPerSecond.toFixed(1)} </span> </div>
 <N64ProgressBar value={Math.min($performanceMetrics.tokensPerSecond, 150)} max={ 150 } size="sm"
                  theme="green"
                  animated={$performanceMetrics.tokensPerSecond > 0} showPercentage={ false } /> </div>
 <div> <div class="flex justify-between text-sm"> <span class="nes-text">Cache Hit Rate</span>
 <span class="font-medium"> {($performanceMetrics.cacheHitRate * 100).toFixed(1)}% </span> </div>
 <N64ProgressBar value={$performanceMetrics.cacheHitRate * 100} max={ 100 } size="sm"
                  theme="blue"
                  animated={ true } showPercentage={ false } sparkle={$performanceMetrics.cacheHitRate > 0.8} /> </div>
 ...
 <N64ProgressBar value={$gpuStatus.layers} max={ 35 } size="sm"
                  theme="purple"
                  animated={$gpuStatus.available} showPercentage={ false } sparkle={$gpuStatus.layers === 35} /> </div>
