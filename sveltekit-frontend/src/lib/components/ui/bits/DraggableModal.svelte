<script lang="ts">
  ...
  function handleResize(_event: MouseEvent) {
    ...
    if (resizeDirection.includes('w')) {
      newWidth = Math.max(minWidth, Math.min(maxWidth, resizeStart.width - deltaX));
      newX = position.x + (dimensions.width - newWidth);
    }
...
{#if open}
  ...
  <div
    ...
    class={`
      fixed rounded-lg border shadow-2xl overflow-hidden
      ${themeClasses[theme].modal}
  ...
  >
    ...
    >
      <div class="flex items-center space-x-2">
        <div class={`
          text-lg
          ${theme === 'yorha' ? 'filter drop-shadow-[0_0_8px_currentColor]' : ''}
        `}>
          🔍
        </div>
        <h3 class={`
          font-semibold
          ${theme === 'yorha' ? 'text-green-400 font-mono tracking-wider' : ''}
        `}>
          {title}
        </h3>
        {#if theme === 'yorha'}
