...
export const GET: RequestHandler = withErrorHandling(async event => {
  ...
  try {
    ...
    return apiSuccess({ conversations }, `Retrieved ${conversations.length }conversations`, requestId);
   }catch (err: any) {
    return apiError('Failed to retrieve conversations', 500, 'DATABASE_ERROR', err, requestId); });
...
export const POST: RequestHandler = withErrorHandling(async event => {
  ...
  try {
    ...
    if (!title || !userId) {
      return apiError('Missing required fields: title, userId', 400, 'INVALID_INPUT', undefined, requestId);
     }
    const conversation = await conversationService.createConversation({
      userId, title, caseId, context
    });
    return apiSuccess({ conversation }, 'Conversation created successfully', requestId);
   }catch (err: any) {
    return apiError('Failed to create conversation', 500, 'DATABASE_ERROR', err, requestId); });
