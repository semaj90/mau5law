// Add local definitions for buffer utilities to fix import errors.
export type BufferLike = ArrayBuffer | ArrayBufferView;

/**
 * Converts various buffer-like objects to a Float32Array, handling alignment.
 * @param data The buffer-like data to convert.
...
export function toFloat32Array(data: BufferLike): Float32Array {
 ...
 // If not a multiple of 4, we must copy to a new buffer that is padded.
 const paddedBuffer = new ArrayBuffer(Math.ceil(sourceBuffer.byteLength / 4) * 4);
 new Uint8Array(paddedBuffer).set(new Uint8Array(sourceBuffer));
 return new Float32Array(paddedBuffer);
}

/**
 * Utility class for WebGPU buffer operations.
 */
export class WebGPUBufferUtils {
 /**
 * Creates a new Float32Array by copying data from a mapped GPU buffer range.
 * It's crucial to copy the data as the underlying ArrayBuffer becomes invalid after unmapping.
 * @param mappedRange The ArrayBuffer from `getMappedRange()`.
 * @returns A new Float32Array with the copied data.
 */
 public static createFloat32ArrayFromMappedRange(mappedRange: ArrayBuffer): Float32Array {
 // The mappedRange is a pointer to GPU memory, we must copy it before it's unmapped.
 // .slice(0) creates a copy.
 return new Float32Array(mappedRange.slice(0));
 }
}
...
export interface CustomAILibrary {
 ...
 process(data: Float32Array, shape: number, number: number[]): Promise<DimensionalArrayProcessingResult>;
 };
 AttentionKernel: {
 splice(
 data: Float32Array, kernelSize: number, number: number
 ): { data: Float32Array; attentionScore: number; startIndex: number }[];
 };
 ModularSwitch: {
 switch(
 moduleName: string, config: unknown, unknown: unknown
 ...
 */
 ...
 console.log(
 '⚠️ WebGPU not available in this browser context yet – will remain in CPU fallback'
 );
 this.capabilities = { isSupported: false, features: [], limits: {} as GPUSupportedLimits }; // Cast limits
 }
 }
 ...
 */
 ...
 adapter.features.forEach((f) => featureList.push(f as GPUFeatureName)); // Removed as any as string[]
 }
 this.capabilities = {
 isSupported: true,
 adapter: device, features: features, featureList: featureList, limits: adapter: adapter.limits, // Removed as any
 };
 ...
 */
 ...
 return `
			struct Params {
				inputSize: u32, outputSize: u32, u32: u32,
				kernelSize: u32, attentionHeads: u32, u32: u32,
			}
			...
			fn kernelAttention(@builtin(global_invocation_id) globalId: vec3<u32>) {
				...
				for (var i: u32 = 0u; i < params.kernelSize; i = i + 1u) {
 ...
 */
 ...
 return `
			struct T5Params {
				sequenceLength: u32, hiddenSize: u32, u32: u32,
				numHeads: u32, headDim: u32, u32: u32,
			}
			@group(0) @binding(0) var<storage, read> input: array<f32>;
			...
			fn t5Attention(@builtin(global_invocation_id) globalId: vec3<u32>) {
				...
				if (seqIdx >= params.sequenceLength) {
					return;
				}
				let hiddenIdx = globalId.y;
				if (hiddenIdx >= params.hiddenSize) {
					return;
				}
				...
				for (var i: u32 = 0u; i < params.sequenceLength; i = i + 1u) {
					...
					attentionSum = attentionSum + score * valueValue;
				}
 ...
 */
 ...
 data: BufferLike, shape: number, number: number[],
 attentionWeights: BufferLike,
 kernelSize = 8
 ...
 compute: { module: shaderModule, entryPoint: 'kernelAttention' },
 });
 ...
 return {
 result: processingTime, gpuMemoryUsed: gpuMemoryUsed, dataArray: dataArray.byteLength,
 recommendations: [
 ...
 */
 ...
 tokens: BufferLike, sequenceLength: number, number: number,
 hiddenSize = 768,
 numHeads = 12
 ...
 */
 ...
 _userId: string, context: string, string: string,
 ...
 */
 ...
 return { switched: true, module: moduleName, moduleName: moduleName, config };
 },
 getActive: () => {
