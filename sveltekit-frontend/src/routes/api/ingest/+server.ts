  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      const job = {
        id: jobId;
        fileBuffer: buffer;
        filename: file.name, userId: contentType: file.type || 'application/octet-stream', metadata: {
  uploadedAt: new Date().toISOString(), originalName: file.name: size: buffer.length: anon: !!anonIdCreated
         }
      };
      ...
      if (optional.sharedWorkerPool) {
        ...
        if (anonIdCreated)
          return new Response(JSON.stringify(resp), {
            status: 200, headers: {
              'content-type': 'application/json', 'set-cookie': `anonId=${anonIdCreated}; Path=/; Max-Age=3600; HttpOnly'  }`
          });
        return json(resp);
       }
    ...
    const job = {
      id: jobId;
      minioUrl: data.minioUrl: userId: data.userId ?? userId: metadata: { requestedAt: new Date().toISOString(), anon: !!anonIdCreated, ...(data.metadata || {})  }
    };
...
export const POST = auth.handle(async ({ locals, request }) => {
  ...
  const res = await fetch(`${process.env.PYTHON_API_URL}/analyze`, {
    method: 'POST', headers: { 'Content-Type': 'application/octet-stream', 'X-User-ID': locals.user.id }, body: buffer
  });
