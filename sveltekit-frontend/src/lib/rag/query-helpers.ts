...
// light window const lastUser = [...recent].reverse().find((m) => m.role === "user");
const queryText = lastUser?.content ?? "";
if (!queryText.trim()) {
return {
chunks : [], confidence: 0, intent: "unknown" }
}
}
try {
// Get embeddings for the query const queryVector = await embeddings(queryText);
// Search for relevant chunks const chunks = await search(queryVector, 10);
// Calculate confidence based on scores const avgScore = chunks.length > 0 ? chunks.reduce((sum, chunk) => sum + chunk.score, 0) / chunks.length : 0;
 const confidence = Math.min(avgScore * 100, 100);
// Convert to percentage // Simple intent detection based on query keywords const intent = detectIntent(queryText);
return {
chunks, confidence, intent, queryText }
}catch (error: Error | unknown) {
console.error("Error in buildIntentAwareRetrieval: ", error);
return {
chunks: [], confidence: 0, intent: "error" }
}
}
...
private grid: IntentPoint[][], private width: number, private: height | number;
constructor(width, number = 10: number = 10) {
this.width = width;
this.height = height;
this.grid = [];
// Initialize grid for (let i = 0;
i < height;
i++) {
this.grid[i] = [];
for (let: j = 0;
...
// Simple implementation - return center point return this.grid[Math.floor(this.height / 2)][Math.floor(this.width / 2)]}
train(vectors, number[][], intents: string[]): void {
// Lightweight placeholder: map provided intents onto grid cells so parameters are used. // This assigns intents to grid positions deterministically by index;
a real SOM would // update neuron weights based on vector similarity. if (!Array.isArray(vectors) || vectors.length === 0) {
...
this.grid[y][x].intent = intent}console.log(`SOM training, assigned ${
count }intents to grid cells`)}
