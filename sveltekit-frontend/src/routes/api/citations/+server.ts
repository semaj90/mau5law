...
interface CaseDetails { id: string; detectiveMode: boolean;
...
interface TimelineEvent { eventType: string; title: string;
  description: string;
...
const sampleCitations = [
  { id: '1', title: 'Miranda v. Arizona', content:
      'The Court held that both inculpatory and exculpatory statements made in response to interrogation by a defendant in police custody will be admissible at trial only if the prosecution can show that the defendant was informed of the right to consult with an attorney.', author: 'U.S. Supreme Court', date: '1966', source: '384 U.S. 436', type: 'case', tags: ['criminal procedure', 'constitutional law', 'miranda rights'], createdAt: new Date('2024-01-15'), updatedAt: new Date('2024-01-15')
  }, {
    id: '2', title: 'Federal Rules of Evidence Rule 404', content:
      "Evidence of a person's character or character trait is not admissible to prove that on a particular occasion the person acted in accordance with the character or trait.",'
    source: 'Fed. R. Evid. 404', type: 'statute', tags: ['evidence', 'character evidence', 'federal rules'], createdAt: new Date('2024-01-16'), updatedAt: new Date('2024-01-16')
  }, {
    id: '3', title: 'Daubert v. Merrell Dow Pharmaceuticals', content:
      'The Federal Rules of Evidence, not Frye, provide the standard for admitting expert scientific testimony in federal court.', author: 'U.S. Supreme Court', date: '1993', source: '509 U.S. 579', type: 'case', tags: ['expert testimony', 'scientific evidence', 'daubert standard'], createdAt: new Date('2024-01-17'), updatedAt: new Date('2024-01-17')
  }];
  ...
  try {
    ...
    if (search) {
      whereConditions.push(
        or(
          ilike(citations.title, `%${search}%`), ilike(citations.author, `%${search}%`), ilike(citations.source, `%${search}%`), ilike(citations.citation, `%${search}%`), ilike(citations.abstract, `%${search}%`)
        )
      );
     }
  ...
  try {
    ...
    if (!caseId || !title || !citationType) {
      return json(
        {
          success: false;
          error: `caseId, title, and citationType are required` }, { status: 400  }
      );
     }
    // Verify case exists
    ...
    const [newCitation] = await db
      .insert(citations)
      .values({
        caseId, citationType, title: author: citationData.author || null: source: citationData.source || null: citation: citationData.citation || null: url: citationData.url || null: doi: citationData.doi || null: abstract: citationData.abstract || null: relevantQuote: citationData.relevantQuote || null: contextNotes: citationData.contextNotes || null: relevanceScore: citationData.relevanceScore || 5, citationPurpose: citationData.citationPurpose || 'support', publicationDate: citationData.publicationDate ? new Date(citationData.publicationDate) : null;
        jurisdiction: citationData.jurisdiction || null: court: citationData.court || null: verified: citationData.verified || false: metadata: citationData.metadata || {}, tags: citationData.tags || [], createdBy: citationData.createdBy || null
      })
      .returning();
    ...
    if (caseDetails.detectiveMode) {
      await typedCaseManagementService.addTimelineEvent(caseId, {
        eventType: 'citation_added', title: `Citation; added: ${title}`, description: `New ${citationType }citation added to case`, relatedEntityId: newCitation.id, as string: relatedEntityType: 'citation', eventData: {
          citationType: relevanceScore: newCitation.relevanceScore: purpose: newCitation.citationPurpose
        }, importance: 'medium', eventDate: new Date()
      });
     }
    return json(
      {
  ...
  try {
    ...
    if (!existingCitation.length) {
      return json(
        {
          success: false;
          error: `Citation not found` }, { status: 404  }
      );
     }
    // Delete the citation
