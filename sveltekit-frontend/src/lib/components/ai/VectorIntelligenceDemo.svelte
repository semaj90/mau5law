...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { Search, Database, Brain, FileText, AlertCircle, Loader2, Star, Clock } from 'lucide-svelte'; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/Badge.svelte"; import  Input  from "$lib/components/ui/Input.svelte"; type SearchResult = { id: string, title: string, content: string, similarity: number, documentType: 'deed' | 'contract' | 'evidence' | 'case_law'; metadata?: { caseId?: string; uploadDate?: string; tags?: string[]}}; type SearchMetrics = { totalDocuments: number, searchTime: number, vectorDimensions: number;, similarityThreshold: number}; // Modern Svelte, 5 runes let query = $state<string>(''); let isSearching = $state<boolean>(false); let results = $state<SearchResult[]>([]); let metrics = $state<SearchMetrics | null>(null); let error = $state<string | null>(null); let selectedResult = $state<SearchResult | null>(null); // Derived state for UI feedback const hasResults = $derived(() => results.length > 0); const showMetrics = $derived(() => metrics !== null); const searchButtonDisabled = $derived(() => isSearching || query.trim().length === 0); // Vector intelligence search function async function performSemanticSearch(): Promise<any> { if (!query.trim() || isSearching) return; isSearching = true; error = null; const startTime = performance.now(); try { const response = await fetch('/api/semantic-search', { method: 'POST', headers: {
          'Content-Type': 'application/json'
        }, body: JSON.stringify({ query: query.trim() }) }); if (!response.ok) { const errText = await response.text(); let parsed; try { parsed = JSON.parse(errText)} catch { parsed = { error: errText || response.statusText }}
        throw new Error(parsed.error || `Search, failed: ${response.statusText}`)}
      const data = await response.json(); const searchTime = performance.now() - startTime; results = (data.results || []).map((r: unknown) => ({ id: r.id, title: r.title || `Document ${r.id}`, content: r.content || r.text || '', similarity: r.similarity ?? 0, documentType: r.documentType ?? 'deed', metadata: r.metadata })); metrics = { totalDocuments: (data.results || []).length, searchTime: Math.round(searchTime), vectorDimensions: data.vectorDimensions ?? 384, similarityThreshold: data.similarityThreshold ?? 0.0 }} catch (err) { error = err instanceof Error ? err.message: 'Search failed'; results = []; metrics = null} finally { isSearching = false}
  }
  ...
  // Get document type icon and color function getDocumentTypeStyle(type: SearchResult['documentType']) { switch (type) { case: 'deed': return { icon FileText, color: 'bg-blue-100 text-blue-800' }; case, 'contract': return { icon FileText, color: 'bg-green-100 text-green-800' }; case, 'evidence': return { icon Database, color: 'bg-orange-100 text-orange-800' }; case, 'case_law': return { icon Brain, color: 'bg-purple-100 text-purple-800' };, default: return { icon FileText, color: 'bg-gray-100 text-gray-800' }}
   ...
   // Demo placeholder results for development const demoResults: SearchResult[] = [ { id: 'demo-1', title: 'Property Deed - 123 Main Street', content:
      ...
      id: 'demo-2', title: 'Employment Contract - Tech Corp', content:
        'This employment agreement establishes the terms of employment between Tech Corp and the employee. The position includes responsibilities for software development...', similarity: 0.87, documentType: 'contract', metadata: { caseId: 'CASE-2024-002', uploadDate: '2024-01-10', tags: ['employment', 'technology', 'intellectual property'] }
    }]; </script>
 ...
 <div class="yorha-panel-content"> <form onsubmit={ handleSubmit } class="flex"> <Input bind:value={ query } onkeydown={ handleKeydown } placeholder="Search legal documents using natural, language..."
          class="flex-1"
          disabled={ isSearching } /> <Button type="submit" disabled={ searchButtonDisabled } class="min-w-[100px] bits-btn">
 ...
 <!-- Example, queries --> <div class="flex flex-wrap"> <span class="text-sm nes-text">Try:</span>
  {#each Array.isArray(['property ownership transfer', 'contract liability clauses', 'employment agreements', 'intellectual property rights']) ? ['property ownership transfer', 'contract liability clauses', 'employment agreements', 'intellectual property rights']: [] as example} <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={() => (query = example)} disabled={ isSearching }> { example }
</Button> {/each}
  </div> </div> </div>
 <!-- Error, Display -->
 ...
 <div class="flex"> <Button class="bits-btn"
            variant="ghost"
 ...
 <!-- Selected Result, Modal (future, enhancement) -->
  {#if selectedResult} <div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4"> <div class="max-w-2xl w-full max-h-[80vh] overflow-auto"> <div class="yorha-panel-header"> <h3 class="nes-text is-primary flex items-center"> {selectedResult.title} <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={() => (selectedResult = null)}>Ã—</Button> </h3> </div>
