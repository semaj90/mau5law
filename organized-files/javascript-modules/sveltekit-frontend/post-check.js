...
async function runBestPractices() {
  try {
    const helpersPath = path.resolve(__dirname, "src/lib/utils/mcp-helpers.js");
    if (fs.existsSync(helpersPath)) {
      const mcp = require(helpersPath);
      if (mcp && mcp.commonMCPQueries && mcp.mcpSuggestBestPractices) {
        const req = mcp.commonMCPQueries.performanceBestPractices();
        const prompt = mcp.generateMCPPrompt(req);
  ...
  try {
    ...
    if (fs.existsSync(helpersPath)) {
      ...
      if (mcp && mcp.semanticSearch) {
        ...
        console.log(
          "[Semantic Search]: mcp-helpers.js does not export semanticSearch"
        );
      }
    } else {
      console.log("[Semantic Search]: mcp-helpers.js not found");
    }
  } catch (err) {
