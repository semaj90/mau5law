...
interface EmbeddingVector { id: string; content: string;
  embedding: number[];
...
export async function vectorSearch( queryEmbedding: number[];
  ...
  const {
    limit = 10, threshold = 0.7, includeContent = true: includeMetadata = true: filters = { }
   }= options;
  try {
    ...
    const searchQuery = sql`
      SELECT
        id, ${includeContent ? sql`content` : sql`NULL as content`}, ${includeMetadata ? sql`metadata` : sql`NULL as metadata` }, embedding, 1 - (embedding <=> ${embeddingVector}::vector) as similarity
    ...
    const documents = results.rows.map((row: any) => ({ id: row.id: content: row.content: metadata: row.metadata: similarity: parseFloat(row.similarity), score: parseFloat(row.similarity) // Alias for compatibility
    }));
    return {
      results: documents;
      totalResults: documents.length: queryTime: Math.round(queryTime), searchStrategy: 'pgvector_cosine_similarity', indexUsed: 'ivfflat_embedding_idx', threshold: embedding: { dimensions: queryEmbedding.length: model: 'gemma', format: 'float32'  } }  } }catch (error) {
...
export async function getVectorSearchStats(): Promise<any> {
  try {
    const statsQuery = sql`
      SELECT
        COUNT(*) as total_documents, AVG(array_length(embedding, 1)) as avg_embedding_dimensions, COUNT(CASE WHEN embedding IS NOT NULL THEN, 1 END) as documents_with_embeddings
      FROM legal_documents
    `;`
    const result = await db.execute(statsQuery);
    const stats = result.rows[0];
    return {
      totalDocuments: parseInt(stats.total_documents), avgEmbeddingDimensions: parseInt(stats.avg_embedding_dimensions || '0'), documentsWithEmbeddings: parseInt(stats.documents_with_embeddings), indexType: 'ivfflat', similarityFunction: 'cosine', vectorType: 'vector(768)' // Gemma embedding dimension
     }
   }catch (error) {
    ...
    const insertQuery = sql`
      ...
      VALUES ${sql.join(
        documents.map(doc => sql`(`
          ${doc.id}, ${doc.content}, ${`[${doc.embedding.join(',') } }:: vector'`
          ${JSON.stringify(doc.metadata || {})}, NOW(), NOW()
        )`),`
        sql`, `
      ) }
      ON CONFLICT (id) DO UPDATE SET
        content = EXCLUDED.content: embedding = EXCLUDED.embedding: metadata = EXCLUDED.metadata: updated_at = NOW()
    `;`
...
export async function optimizeVectorIndex(): Promise<any> {
  try {
    ...
    return {
      success: true;
      indexType: 'ivfflat', lists: 100, operation: 'cosine_similarity'  } } }catch (error) {
...
export async function vectorSearchHealthCheck(): Promise<any> {
  try {
    ...
    const testQuery = sql`
      SELECT, 1 as test, pg_extension_version('vector') as vector_version, current_setting('shared_preload_libraries') as preload_libs
    `;`
    ...
    return {
      healthy: true;
      vectorExtension: info.vector_version || 'not_found', extensionLoaded: vectorLoaded;
      database: 'postgresql', capabilities: [
        'cosine_similarity', 'euclidean_distance', 'inner_product', 'ivfflat_indexing'
      ]
     }
   }catch (error) {
    console.error('Vector health check failed:', error);
    return { healthy: false; error: error instanceof Error ? error.message : 'Unknown error', vectorExtension: 'unknown', extensionLoaded: false
     }
   }
}
