...
export class CaseScoringService {
   ...
   */
  async phoenixWrightSearch(request: PhoenixWrightSearchRequest): Promise<PhoenixWrightSearchResult> {
    const startTime = Date.now();
    const searchId = `pw_${request.caseId}_${Date.now()}`;

    try {
      logger.info('Starting Phoenix Wright AI search', { searchId, caseId: request.caseId, query: request.query });

      // Parallel search operations
      const [precedents, contradictions, evidenceMatches] = await Promise.all([
        this.semanticPrecedentSearch(request),
        request.detectContradictions !== false ? this.detectContradictions(request) : Promise.resolve([]),
        this.matchEvidence(request)
      ]);

      // Generate ranking explanation using AI
      const rankingExplanation = await this.generateRankingExplanation(request, precedents, contradictions, evidenceMatches);

      // Calculate overall confidence
      const confidence = this.calculateSearchConfidence(precedents, contradictions, evidenceMatches);

      // Generate YOᴿHa UI state
      const yohaUIState = await this.updateYohaUI(request, precedents, contradictions, evidenceMatches);
   ...
   */
  private async semanticPrecedentSearch(request: PhoenixWrightSearchRequest): Promise<LegalPrecedent[]> {
...
Format as JSON array of precedent objects.`;
   ...
   */
  private async detectContradictions(request: PhoenixWrightSearchRequest): Promise<ContradictionAnalysis[]> {
...
Be thorough, objective, and provide specific reasoning for your conclusions.`;
  ...
  private calculateSearchConfidence(
    ...
    const precedentConfidence = precedents.length > 0 ?
      precedents.reduce((sum, p) => sum + p.relevanceScore, 0) / precedents.length : 0.5;

    const contradictionPenalty = contradictions.length * 0.1;
    const evidenceStrength = evidenceMatches.length > 0 ?
      evidenceMatches.reduce((sum, e) => sum + e.legalWeight, 0) / evidenceMatches.length : 0.5;

    const confidence = (precedentConfidence * 0.4 + evidenceStrength * 0.4 + (1 - contradictionPenalty) * 0.2);
  ...
  private async updateYohaUI(
    ...
    const hasStrongEvidence = evidenceMatches.some(e => e.strength === 'strong' || e.strength === 'conclusive');
    const hasSevereContradictions = contradictions.some(c => c.severity === 'severe' || c.severity === 'critical');
    const precedentStrength = precedents.reduce((sum, p) => sum + p.relevanceScore, 0) / Math.max(1, precedents.length);
  ...
  private async generateAIAnalysis(request: CaseScoringRequest): Promise<string> {
    ...
    const defendants = Array.isArray(caseData.defendants) ? caseData.defendants.join(', ') :
                      caseData.defendants ? String(caseData.defendants) : 'N/A';
...
Respond in JSON format with keys: evidence_strength, witness_reliability, legal_precedent, public_interest, case_complexity, resource_requirements`;
  ...
  private async generateRecommendations(
    ...
    if (resources > 0.7) recommendations.push('Resource requirements are reasonable - proceed with standard allocation');
...
Provide 2-3 specific strategic recommendations for the prosecution team.`;
    ...
    try {
      const aiRecommendationsRaw = await ollama.generateCompletion(this.SCORING_MODEL, strategyPrompt, {
        temperature: 0.5,
        max_tokens: 200
      });

      if (aiRecommendationsRaw) {
        const parsed = String(aiRecommendationsRaw).split(/\r?\n/).map(l => l.trim()).filter(l => l.length > 5).slice(0, 3);
  ...
  private validateRequest(request: CaseScoringRequest): void {
    ...
    const desc = (request.metadata as { description?: string })?.description ||
  ...
  async getCaseScoreHistory(caseId: string): Promise<CaseScoringResult[]> {
    try {
      const rows = await db.select().from(caseScores).where(eq(caseScores.caseId, caseId)).orderBy(caseScores.calculatedAt);
