...
class UnifiedSearchService {
  ...
  async ingestDocument(document: Omit<UnifiedDocument, 'id' | 'searchable' | 'cached'>): Promise<IngestResult> {
    try {
      ...
      const source =
        typeof incomingMeta.source === 'string' &&
        ['upload', 'manual', 'api', 'evidence'].includes(incomingMeta.source)
      ...
      const confidenceLevel =
        typeof incomingMeta.confidenceLevel === 'number'
          ? incomingMeta.confidenceLevel
          : typeof incomingMeta.confidenceLevel === 'string' && incomingMeta.confidenceLevel.trim() !== '' && !Number.isNaN(Number(incomingMeta.confidenceLevel))
      ...
      const priority = typeof incomingMeta.priority === 'string' ? incomingMeta.priority : undefined;
      ...
      const metadata: UnifiedDocument['metadata'] & Record<string, unknown> = {
        ...
        ...Object.keys(incomingMeta).reduce((acc, k) => {
          if (!['source', 'userId', 'tags', 'category', 'confidenceLevel', 'extractedEntities', 'keyTerms', 'neo4jNodeId', 'shaderData', 'priority', 'semantic_hash'].includes(k)) {
  ...
  private async getNeo4jRecommendations(docs: UnifiedDocument[]): Promise<Recommendation[]> {
    try {
      // gather entities and categories from the provided docs
      ...
      // derive simple signals from the raw lists and use them below so vars are not unused
      const uniqueEntities = Array.from(new Set(entities));
      const topEntities = uniqueEntities.slice(0, 5);
      // prefer an explicit emptiness check instead of relying on `join(...) || 'none'`
      const topEntitiesStr = topEntities.length > 0 ? topEntities.join(', ') : 'none';
      const categoryCounts = categories.reduce((acc: Record<string, number>, c) => {
        acc[c] = (acc[c] || 0) + 1;
        return acc;
      }, {});
      const mostFrequentCategory = Object.keys(categoryCounts).sort((a, b) => (categoryCounts[b] || 0) - (categoryCounts[a] || 0))[0] || 'unknown';

      // simple heuristic to adjust confidence based on entity signal strength
      const entitySignal = Math.min(1, uniqueEntities.length / 5); // 0..1
      const baseConfidence = 0.7 + entitySignal * 0.2; // 0.7 .. 0.9

     // Use numeric rounding to two decimal places in a safe way
  ...
  private extractKeywords(doc: Partial<UnifiedDocument>): string[] {
    ...
    const stop = new Set(['the','and','for','with','that','this','from','are','was','were','has','have','but','not','you','your','a','an','of','in','to','is']);
    const counts = text
      .toLowerCase()
      .replace(/[^a-z0-9\s]/g, ' ')
      .split(/\s+/)
  ...
  private async updateAccessCounts(ids: string[]): Promise<void> {
    ...
    try {
      // Lightweight cache-only increment to avoid heavy DB writes here.
      await Promise.all(ids.map(async id => {
        const key = `doc:${id}`;
        const raw = await cache.get(key);
        if (!raw) return;
        const parsed = typeof raw === 'string' ? JSON.parse(raw) : raw;
        parsed.cached = parsed.cached || parsed.cached === undefined ? parsed.cached : {};
  ...
  private convertToUnifiedDocument(row: DbDocumentRow): UnifiedDocument {
    ...
    const safeStringArray = (v: unknown) => (Array.isArray(v) ? v.filter((x): x is string => typeof x === 'string') : []);
    ...
    const rawCategory = safeStringOrDefault(pm.category, 'other');
    const category = this.normalizeCategory(rawCategory);
    const confidenceLevel = safeNumberFromUnknown(pm.confidenceLevel, 0);
    const priority = safeString(pm.priority);
    const extractedEntities = safeStringArray(pm.extractedEntities);
    const keyTerms = safeStringArray(pm.keyTerms);
    const neo4jNodeId = safeString(pm.neo4jNodeId);
    const shaderData = pm.shaderData !== undefined ? pm.shaderData : undefined;
