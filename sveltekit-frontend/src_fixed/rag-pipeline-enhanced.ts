...
class MetricsCollector {
  ...
  private timings: Map<string, { total: number; count: number; last: number;
}> = new Map();
...
}=${String(tags[key]).replace(/[^a-zA-Z0-9_-]/g, '_')}`,}        name,
      );
  ...
  getMetrics(): Record<string, unknown> {
    ...
    this.counters.forEach((value, key) => (metrics[`counter_${key;
}`] = value));}    this.timings.forEach((value, key) => {
      metrics[`timing_${key;
}_total`] = value.total;}      metrics[`timing_${key;
}_count`] = value.count;}      metrics[`timing_${key;
}_last`] = value.last;}      metrics[`timing_${key;
}_avg`] = value.count > 0 ? value.total / value.count : 0;}    });
    return metrics;
  }
}
...
export class EnhancedLegalRAGPipeline {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`[RAG] Pipeline initialized successfully in ${Date.now() - startTime;
}ms`);}    } catch (err: unknown) {
  ...
  private async initializeDatabase(): Promise<void> {
    try {
      ...
      this.sql = postgres(this.config.database.databaseUrl, {
        ...
        onparameter: (key: string, value: unknown) => console.debug(`[DB] Parameter ${key;
}:`, value),}      });
  ...
  private async verifyConnections(): Promise<void> {
    try {
      ...
      if (testEmbedding.length !== this.config.ollama.embeddingDimensions) {
        console.warn(
          `[RAG] Warning, expected ${this.config.ollama.embeddingDimensions;
} dims, got ${testEmbedding.length;
}`,}        );
      }
  ...
  async ingestLegalDocument(params: DocumentIngestionParams): Promise<IngestionResult> {
    ...
    try {
      ...
      await this.ensureInitialized();
      const { caseId, metadata = {}, confidentialityLevel = 'public', jurisdiction, clientId;
} = params;
      ...
      console.log(`[RAG] Split into ${chunks.length;
          ...
          const chunkRecords = await Promise.all(
            batch.map(async (chunk, idx) => {
              try {
                ...
                const errorMsg = `Failed to process chunk ${i + idx;
}: ${error;
      ...
      console.log(
        `[RAG] Document ingestion completed in ${processingTime;
}ms (${successfulChunks;
}/${chunks.length;
} chunks successful)`,}      );
...
Extract relevant legal tags from this {documentType;
} document. Focus on legal concepts, practice areas, jurisdictions, case types, parties, and key legal topics.
...
`);
  ...
  private async checkOllamaHealth() {
    ...
    if (testEmbedding.length !== this.config.ollama.embeddingDimensions) {
      throw new Error(
        `Expected ${this.config.ollama.embeddingDimensions;
} dimensions, got ${testEmbedding.length;
}`,}      );
    }
  }
  ...
  async close(): Promise<void> {
    try {
      const redisClosePromise = this.redis
        ? (this.redis as unknown as { quit?: () => Promise<void>; disconnect?: () => void;
}).quit?.() ||
          Promise.resolve((this.redis as unknown as { disconnect?: () => void;
}).disconnect?.())
        : Promise.resolve();
