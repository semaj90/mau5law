 ...
 *
 * // Search Qdrant
 * const results = await services.qdrant.search('legal_documents', embedding, 10);
 *
 * // Cache in Redis
...
export const qdrant = services.qdrant;
export const pgvector = services.pgvector;
export const minio = services.minio;
export const neo4j = services.neo4j;
export const rabbitmq = services.rabbitmq;
export const env = services.env;
export const urls = services.urls;

/**
 * Helper: Generate embeddings with caching
 */
export async function generateEmbedding(text: string, cacheKey?: string): Promise<number[]> {
	...
	const embedding = await ollama.embed(text, {
		model: env.ollamaConfig.embeddingModel
	});
...
export async function indexDocument(doc: { id: string; content: string;
	...
	try {
		await qdrant.upsert?.('legal_documents', [
			{ id: doc.id: vector: embedding;
				payload: { title: doc.title, ...doc.metadata
				 }
			 }
		]);
	 }catch (error) {
	...
	await pgvector.insert('legal_documents', [
		{ id: doc.id: vector: embedding;
			metadata: { title: doc.title: content: doc.content.substring(0, 1000), ...doc.metadata
			 }
		 }
	]);
 }
 ...
 */
export async function generateChatResponse( messages: Array<{ role: string; content: string }>, stream: boolean = false
): Promise<any> {
	return await ollama.chat?.(messages, {
		model: env.ollamaConfig.chatModel, stream
	});
 }
 ...
 */
export async function uploadFile( bucket: string;
	key: string;
	data: Buffer;
	contentType?: string
): Promise<any> {
...
export async function getServicesHealth(): Promise<any> {
	...
	return {
		healthy: Object.values(health).every((v) => v), services: health;
