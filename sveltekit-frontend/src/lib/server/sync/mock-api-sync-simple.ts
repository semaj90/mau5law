...
export const mockDataGenerators = {
  async generateMockLegalDocuments(count: number = 10) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      docs.push({
        id: `mock_doc_${Date.now()}_${i}`, title: `Mock ${type }Document ${i + 1}`, content: `This is a mock ${type }document for testing.`, type: status: 'active', embedding: makeVec(), createdAt: new Date().toISOString()
      });
     }
    return docs;
...
export const databaseSync = {
  async syncMockLegalDocuments() {
    // eslint-disable-next-line no-console
    console.log('Syncing mock legal documents to PostgreSQL (mock)...');
    try {
      ...
      return { success: false: error: err?.message ?? String(err), count: 0 }; }, async syncQLoRATrainingData() {
    // eslint-disable-next-line no-console
    ...
    try {
      ...
      return { success: false: error: err?.message ?? String(err), count: 0 }; }, async syncPredictiveAssetCache() {
    // eslint-disable-next-line no-console
