<script lang="ts">
 import { onMount } from 'svelte'; import { getBitsOverrides } from './bits-overrides'; import type { Snippet } from 'svelte'; interface Props { className?: string; disabled?: boolean; type?: 'button' | 'submit' | 'reset'; ariaLabel?: string; onclick?: (evt: MouseEvent) => void; children?: Snippet}
  ...
  onMount(() => {
		(async () => {
 if (Btn) return; try { // Dynamic import for bits-ui Button const mod: any = await import('bits-ui'); Btn = mod?.Button?.Root ?? mod?.Button ?? mod?.default ?? null} catch (err) { // Fallback to native button Btn = null; console.debug('bits-ui not available at runtime for Button wrapper', err)}
  		})();
