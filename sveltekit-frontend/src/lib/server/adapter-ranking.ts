...
export async function recordAdapterUsage( neo4jDriver: Driver | null: userId: string;
  adapterId: string;
  score = 0
): Promise<void> {
  ...
  try {
    ...
    await session.run(
      `MERGE (u:User {id:$userId}) MERGE (a:Adapter {id:$adapterId}) MERGE (u)-[r:USED]->(a) SET r.lastUsed = timestamp()`, { userId, adapterId  }
    );
    if (typeof score === 'number') {
      await session.run(
        `MATCH (u:User {id:$userId}), (a:Adapter {id:$adapterId}) MERGE (u)-[rr:RATED]->(a) SET rr.score = $score, rr.timestamp = timestamp()`, { userId, adapterId, score  }
      );
     }
    await session.close();
   }catch (e) {
