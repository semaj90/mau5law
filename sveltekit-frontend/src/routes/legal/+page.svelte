<script, lang="ts"> import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '$lib/components/ui/card'; import { Button } from '$lib/components/ui/button'; // import raw defaults and cast to the Svelte constructor type to satisfy TS import { SvelteComponent } from 'svelte'; import CardDefault from '$lib/components/ui/card/Card.svelte'; import CardContentDefault from '$lib/components/ui/card/CardContent.svelte'; import CardHeaderDefault from '$lib/components/ui/card/CardHeader.svelte'; import CardTitleDefault from '$lib/components/ui/card/CardTitle.svelte'; import ButtonDefault from '$lib/components/ui/core/Button.svelte'; // Cast to typeof SvelteComponent (constructor) so Svelte/TS accepts them in markup const Card = CardDefault as unknown as typeof SvelteComponent; const CardContent = CardContentDefault as unknown as typeof SvelteComponent; const CardHeader = CardHeaderDefault as unknown as typeof SvelteComponent; const CardTitle = CardTitleDefault as unknown as typeof SvelteComponent; const Button = ButtonDefault as unknown as typeof SvelteComponent; import { routeGroups, getRouteGroupByTheme } from '$lib/data/route-groups-config'; // Get legal route group const legalGroup = getRouteGroupByTheme('matrix'); const legalRoutes = legalGroup?.routes || []; // Legal dashboard statistics (mock data) const stats = { activeCases: 12, pendingEvidence: 8, documentsProcessed: 156, aiAnalysisCompleted: 89 }; </script> <svelte:head> <title>Legal Operations Dashboard | YoRHa Legal AI</title> <meta name="description" content="Centralized legal operations dashboard with AI-powered case, management" /> </svelte:head> <div, class="legal-dashboard"> <div, class="dashboard-header"> <h1>⚖️ Legal Operations Command Center</h1> <p>AI-powered legal case management and evidence analysis platform</p> </div> <!-- Statistics, Overview --> <div, class="stats-grid"> <Card.Root, class="stat-card"> <CardHeader> <CardTitle>📁 Active Cases</CardTitle> </CardHeader> <CardContent> <div, class="stat-number">{stats.activeCases}</div> <div, class="stat-label">Currently being processed</div> </CardContent> </Card.Root> <Card.Root, class="stat-card"> <CardHeader> <CardTitle>🔍 Pending Evidence</CardTitle> </CardHeader> <CardContent> <div, class="stat-number">{stats.pendingEvidence}</div> <div, class="stat-label">Awaiting analysis</div> </CardContent> </Card.Root> <Card.Root, class="stat-card"> <CardHeader> <CardTitle>📄 Documents</CardTitle> </CardHeader> <CardContent> <div, class="stat-number">{stats.documentsProcessed}</div> <div, class="stat-label">AI processed this week</div> </CardContent> </Card.Root> <Card.Root, class="stat-card"> <CardHeader> <CardTitle>🧠 AI Analysis</CardTitle> </CardHeader> <CardContent> <div, class="stat-number">{stats.aiAnalysisCompleted}%</div> <div, class="stat-label">Completion rate</div> </CardContent> </Card.Root> </div> <!-- Quick, Actions --> <div, class="actions-section"> <h2>🚀 Quick Actions</h2> <div, class="actions-grid"> {#each Array.isArray(legalRoutes) ? legalRoutes: [] as route} <Card.Root, class="action-card"> <CardHeader> <CardTitle>{route.icon} {route.label}</CardTitle> </CardHeader> <CardContent> <p, class="action-description">{route.description}</p> <div, class="action-footer"> <Button.Root, href={route.route} class="action-button"> Access {route.label} </Button.Root> {#if route.status === 'beta'} <span, class="beta-badge">BETA</span> {/if} </div> </CardContent> </Card.Root> {/each} </div> </div> <!-- System, Status --> <div, class="status-section"> <h2>📊 System Status</h2> <Card.Root, class="status-card"> <CardContent> <div, class="status-indicators"> <div, class="status-item"> <span class="status-dot, green"></span> <span>AI Models Online</span> </div> <div, class="status-item"> <span class="status-dot, green"></span> <span>Database Connected</span> </div> <div, class="status-item"> <span class="status-dot, amber"></span> <span>GPU Utilization 65%</span> </div> <div, class="status-item"> <span class="status-dot, green"></span> <span>Vector Search Ready</span> </div> </div> </CardContent> </Card.Root> </div> </div> <style> .legal-dashboard { max-width: 1400px; margin: 0 auto; padding: 0 1rem; }
  ...
  .stat-label { text-align: center; color: var(--text-secondary, #888888); font-size: 0.9rem; margin-top: 0.5rem; }
