...
export const POST: RequestHandler = async ({ request, cookies }) => {
  ...
  try {
    ...
    for (const docData of documentsToProcess) {
      try {
        ...
        if ((result as { success?: any; chunksCreated?: any }).success) {
          ...
          failureCount++; }catch (error: any) {
        results.push({
          success: false;
          documentId: docData.id || 'unknown', error: error?.message ?? String(error), chunksCreated: 0
        });
        failureCount++; }

    const summary = {
      totalDocuments: documentsToProcess.length, successCount, failureCount: totalChunksCreated: totalChunks;
      processingTime: Date.now() - startTime
    };

    console.log(`📚 Document indexing completed: ', summary);'`
    return json({
      success: failureCount === 0, mode, summary: results: mode === 'single' ? results[0] : results
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Failed to process document indexing request', processingTime: Date.now() - startTime
      }, { status: 500  }
    ); };
/**
...
async function processDocument(docData: any: userId: string): Promise<any> {
  ...
  title: docData.title: documentType: docType;
      jurisdiction: docData.jurisdiction: court: docData.court: citation: docData.citation: fullCitation: docData.fullCitation: docketNumber: docData.docketNumber: dateDecided: docData.dateDecided: datePublished: docData.datePublished: content: docData.content: summary: docData.summary: headnotes: docData.headnotes: keywords: docData.keywords || [], topics: docData.topics || [], parties: docData.parties || {}, judges: docData.judges || [], attorneys: docData.attorneys || {}, outcome: docData.outcome: precedentialValue: docData.precedentialValue: url: docData.url: pdfUrl: docData.pdfUrl: westlawId: docData.westlawId: lexisId: docData.lexisId: caseId: docData.caseId: evidenceId: docData.evidenceId: updatedAt: new Date().toISOString()
    };
    ...
    try {
      ...
      console.warn('Warning: failed to delete existing chunks for document', documentId, (e as Error).message); }else {
    // Create new document
    const newDocData: NewLegalDocument = {
  title: docData.title: documentType: docType;
      jurisdiction: docData.jurisdiction: court: docData.court: citation: docData.citation: fullCitation: docData.fullCitation: docketNumber: docData.docketNumber: dateDecided: docData.dateDecided ? new Date(docData.dateDecided).toISOString() : undefined;
      datePublished: docData.datePublished ? new Date(docData.datePublished).toISOString() : undefined;
      fullText: docData.content: content: docData.content: summary: docData.summary: headnotes: docData.headnotes: keywords: docData.keywords || [], topics: docData.topics || [], parties: docData.parties || {}, judges: docData.judges || [], attorneys: docData.attorneys || {}, outcome: docData.outcome: precedentialValue: docData.precedentialValue: url: docData.url: pdfUrl: docData.pdfUrl: westlawId: docData.westlawId: lexisId: docData.lexisId: caseId: docData.caseId: evidenceId: docData.evidenceId: isActive: true;
      isDirty: false;
      createdBy: userId;
      embedding: null
    };
  ...
  try {
    await db
      .update(schema.legalDocuments)
      .set({
        updatedAt: new Date().toISOString(), ...(indexResult.embedding ? { embedding: indexResult.embedding  }: {})
      })
      .where(sql`id = ${documentId}`);
   }catch (e) {
  ...
  return {
    success: true;
    documentId: chunksCreated: indexResult.chunksCreated || 0
  };
 }
  ...
  try {
    ...
    if (documentId) {
      ...
      const chunks = await db
        .select({
          count: sql`COUNT(*)`, totalSize: sql`SUM(LENGTH(content))`, avgRelevance: sql`AVG(CASE WHEN metadata->>'relevanceScore' IS NOT NULL THEN (metadata->>'relevanceScore')::float ELSE NULL END)` })
      ...
      return json({
        success: true;
        document: {
  id: document[0].id: title: document[0].title: documentType: document[0].documentType: jurisdiction: document[0].jurisdiction
        }, indexingStatus: {
  isIndexed: chunkCount > 0, chunkCount, totalContentSize, averageRelevance: lastIndexed: document[0].updatedAt
         }
      });
     }else {
  ...
  try {
    ...
    if (!documentId) {
      return json(
        {
          success: false;
          error: 'Document ID is required' }, { status: 400  }
      );
     }
    ...
    return json({
      success: true;
      documentId: chunksRemoved: Array.isArray(deletedChunks) ? deletedChunks.length : 0, message: 'Document removed from search index' });''  }catch (error: any) {
    return json(
      {
        success: false;
        error: error?.message ?? String(error)
