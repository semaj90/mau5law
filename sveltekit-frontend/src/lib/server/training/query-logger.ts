...
class QueryLogger {
  ...
  async logToolUsage(query: string, toolName: string, args: any, result: any): Promise<void> {
    await this.logQuery({
  ...
  async logSearchAndClick(query: string, toolUsed: string, results: any[], clickedIds: string[]): Promise<void> {
    await this.logQuery({
      timestamp: new Date().toISOString(),
      userQuery: query,
      toolsUsed: [toolUsed],
...
export async function logToolCall(query: string, toolName: string, args: any, result: any) {
  await queryLogger.logToolUsage(query, toolName, args, result);
}

export async function logSearchInteraction(query: string, toolUsed: string, results: any[], clickedIds: string[]) {
  await queryLogger.logSearchAndClick(query, toolUsed, results, clickedIds);
}

export async function logQueryWithContext(query: string, tools: string[], context: string[], answer: string) {
