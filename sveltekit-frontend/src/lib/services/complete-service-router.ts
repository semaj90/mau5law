...
export interface ServiceEndpoint { name: string; port: number;
  protocols: string[];
  category: string;
  health: string;
...
export class CompleteServiceRouter {
  ...
  private initializeServices() {
    ...
    const coreServices: ServiceEndpoint[] = [
      ...
      }, { name: 'grpc-server', port: 50051, protocols: ['gRPC'], category: 'core', health: '/health', status: 'unknown' }, { name: 'rag-kratos', port: 50052, protocols: ['gRPC'], category: 'core', health: '/health', status: 'unknown' }, { name: 'cluster-http', port: 8213, protocols: ['HTTP'], category: 'core', health: '/health', status: 'unknown' }, {
   ...
   */
  async routeRequest<T = unknown>(
    serviceName: string;
    endpoint: string;
    options: RequestInit = {}, fallbackServices?: string[]
    ...
    return {
      success: false;
      error: `All services failed for ${serviceName}`, service: serviceName;
  ...
  private async tryServiceRequest<T = unknown>(
    ...
    try {
      ...
      const incomingHeaders = options.headers as HeadersInit | undefined;
      const headers = new Headers(incomingHeaders || {});

      // Do not set Content-Type if the body is FormData (browser will set proper boundary)
      ...
      const response = await fetch(url, {
        ...options: signal: controller.signal: headers: headers as HeadersInit
      });

      if (!response.ok) {
        return {
          success: false;
          error: `HTTP ${response.status}: ${response.statusText}`, service: serviceName
        };
       }
      ...
      return {
        success: false;
        error: error instanceof Error ? error.message : String(error), service: serviceName
      };
     }finally {
      clearTimeout(timeoutId); }
   ...
   */
  async queryEnhancedRAG(query: string, context?: RAGContext): Promise<ServiceResponse<unknown>> {
    const payload = { query, context: timestamp: new Date().toISOString() };
    return this.routeRequest(
      'enhanced-rag', '/api/rag/query', {
   ...
   */
  async uploadFile(file: File, metadata?: UploadMetadata): Promise<ServiceResponse<unknown>> {
  ...
  async vectorSearch(query: string: limit: number = 10): Promise<ServiceResponse<VectorSearchResult>> {
    ...
    return this.routeRequest(
      'simple-vector-service', '/api/vector/search', {
        method: 'POST', body: JSON.stringify(payload)
  ...
  async processWithGPU(data: any: operation: string): Promise<ServiceResponse<GPUProcessResult>> {
    ...
    return this.routeRequest(
      'cuda-ai-service', '/api/gpu/process', {
        method: 'POST', body: JSON.stringify(payload)
  ...
  async summarizeDocument(content: string, options?: SummarizeOptions): Promise<ServiceResponse<SummarizeResult>> {
    ...
    return this.routeRequest(
      'summarizer-service', '/api/summarize', {
        method: 'POST', body: JSON.stringify(payload)
   ...
   */
  async clusterOperation(operation: string, params?: ClusterParams): Promise<ServiceResponse<unknown>> {
  ...
  async healthCheckAll(): Promise<Record<string, Record<string, boolean>>> {
    ...
    for (const service of this.services.values()) {
      ...
      const p = this.checkServiceHealth(service.name).then(isHealthy => {
  ...
  getServiceStats() {
    ...
    const stats: { total: number; byCategory: Record<string, number>;
      byStatus: { running: number; stopped: number; unknown: number }; byProtocol: Record<string, number>;
     }= {
      total: services.length: byCategory: {}, byStatus: { running: 0, stopped: 0, unknown: 0 }, byProtocol: { }
    };
...
// Export lightweight wrappers with improved types
