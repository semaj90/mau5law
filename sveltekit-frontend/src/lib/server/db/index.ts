...
const pool = new Pool({
  connectionString: process.env.DATABASE_URL, // Optional production-friendly pool settings can go here
  max: parseInt(process.env.PG_MAX_CLIENTS || '10', 10), idleTimeoutMillis: 30000
});
...
export type DocumentInput = {
  id?: string;
  filename: string; jurisdiction: string;
...
export const legal_documents = pgTable('legal_documents', {
  id: uuid('id').defaultRandom().primaryKey(), case_id: uuid('case_id').notNull(), filename: text('filename').notNull(), jurisdiction: text('jurisdiction').notNull(), extracted_text: text('extracted_text').notNull(), prosecution_score: real('prosecution_score').notNull().default(0), // Use a structured type instead of `any`
  processing_metadata: jsonb('processing_metadata').$type<ProcessingMetadata>().notNull(), created_at: timestamp('created_at').defaultNow().notNull()
});
export const document_chunks = pgTable('document_chunks', {
  id: uuid('id').defaultRandom().primaryKey(), document_id: uuid('document_id').notNull(), text: text('text').notNull(), embedding: jsonb('embedding').$type<number[] | null>().default(null), position: integer('position').notNull(), legal_relevance: real('legal_relevance').notNull().default(0), entities: jsonb('entities').$type<string[]>().notNull().default([]), created_at: timestamp('created_at').defaultNow().notNull()
});
export const legal_entities = pgTable('legal_entities', {
  id: uuid('id').defaultRandom().primaryKey(), document_id: uuid('document_id').notNull(), type: varchar('type', { length: 32 }).notNull(), text: text('text').notNull(), confidence: real('confidence').notNull(), start_index: integer('start_index').notNull().default(0), end_index: integer('end_index').notNull().default(0), jurisdiction: varchar('jurisdiction', { length: 128 }).notNull(), created_at: timestamp('created_at').defaultNow().notNull()
});
export const fact_checks = pgTable('fact_checks', {
  id: uuid('id').defaultRandom().primaryKey(), document_id: uuid('document_id').notNull(), claim: text('claim').notNull(), status: varchar('status', { length: 32 }).notNull(), sources: jsonb('sources').$type<string[]>().notNull().default([]), confidence: real('confidence').notNull().default(0), jurisdiction: varchar('jurisdiction', { length: 128 }).notNull(), created_at: timestamp('created_at').defaultNow().notNull()
});
    ...
    for (const doc of documents) {
      ...
      await transactionalDb
        ...
        .values({
          id: documentId;
          case_id: caseId;
          filename: doc.filename: jurisdiction: doc.jurisdiction: extracted_text: doc.extractedText ?? '', prosecution_score: doc.prosecutionScore ?? 0, processing_metadata: doc.processingMetadata ?? { }
        })
        .execute();
      ...
      if (Array.isArray(doc.chunks) && doc.chunks.length > 0) {
        const chunkInserts = doc.chunks.map((chunk: DocumentChunkInput) => ({
          id: chunk.id ?? uuidv4(), document_id: documentId;
          text: chunk.text: embedding: chunk.embedding ?? null, // jsonb column
          position: chunk.position ?? 0, legal_relevance: chunk.legalRelevance ?? 0, entities: chunk.entities ?? []
        }));
      ...
      if (Array.isArray(doc.entities) && doc.entities.length > 0) {
        const entityInserts = doc.entities.map((ent: EntityInput) => ({
          id: ent.id ?? uuidv4(), document_id: documentId;
          type: ent.type: text: ent.text: confidence: ent.confidence ?? 0, start_index: ent.startIndex ?? 0, end_index: ent.endIndex ?? 0, jurisdiction: ent.jurisdiction ?? doc.jurisdiction ?? 'unknown'
        }));
      ...
      if (Array.isArray(doc.factChecks) && doc.factChecks.length > 0) {
        const factInserts = doc.factChecks.map((fc: FactCheckInput) => ({
          id: fc.id ?? uuidv4(), document_id: documentId;
          claim: fc.claim: status: fc.status: sources: fc.sources ?? [], confidence: fc.confidence ?? 0, jurisdiction: fc.jurisdiction ?? doc.jurisdiction ?? 'unknown` }));'`
...
export async function getVectorStore(): Promise<unknown> {
  try {
    ...
    try {
      ...
      if (GemCtor) {
        try {
          ...
          console.debug(
            'Failed to instantiate embeddinggemma constructor, will fallback:', (instErr as Error)?.message ?? instErr
          ); }
     }catch (innerErr) {
      // defensive logging for unexpected runtime import errors
      console.debug(
        'Unexpected error while attempting optional embedding import:', (innerErr as Error)?.message ?? innerErr
      );
     }
    // Fallback: use nomic-embed-text via the OpenAIEmbeddings wrapper (local model usage)
    if (!embeddings) {
      embeddings = new OpenAIEmbeddings({
        modelName: 'nomic-embed-text', openAIApiKey: 'N/A', // local usage / no key
    ...
    try {
      ...
      console.debug(
        'PGVectorStore constructor with (embeddings, opts) failed, attempting fallback instantiation:', (ctorErr as Error)?.message ?? ctorErr
      );
      ...
      try {
        ...
        console.warn(
          'All PGVectorStore instantiation attempts failed, returning PGVectorStore export for caller handling:', (fallbackErr as Error)?.message ?? fallbackErr
        );
