...
// ===== TYPES =====
export interface UserCase { id: string; title: string;
  description?: string;
...
export interface UserEvidence { id: string; caseId: string;
  filename: string;
  fileType: string;
  fileSize: number;
  minioUrl: string;
...
export interface UserCitation { id: string; userId: string;
  caseId?: string;
  title: string;
  source: string;
...
export interface UserReport { id: string; userId: string;
  caseId?: string;
  title: string;
...
export interface AIAssistantMessage { id: string; userId: string;
...
export interface AIConversation { id: string; userId: string;
  title: string;
  contextType?: 'case' | 'evidence' | 'citation' | 'general';
  contextId?: string;
  messageCount: number;
...
export interface UserActivity { id: string; userId: string;
  action: string;
...
export interface UserDataState { cases: UserCase[]; evidence: UserEvidence[];
...
const createUserDataStore = () => {
  ...
  let userDataState = $state<UserDataState>({
    cases: [], evidence: [], citations: [], reports: [], aiConversations: [], recentActivity: [], isLoading: false;
    lastSyncAt: 0, cachedAt: 0
  });
  ...
  return {
    ...
    init: async (userId: string) => {
      ...
      try {
        ...
        userDataState.isLoading = $state(false); }, // Sync specific data types
    syncCases: async (userId: string) => {
      try {
        ...
        if (response.ok) {
          ...
          userDataState.lastSyncAt = Date.now(); }catch (error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          userDataState.lastSyncAt = Date.now(); }catch (error) {
        console.error('Failed to sync evidence:', error); }, syncCitations: async (userId: string) => {
      try {
        // removed unused response assignment
        if (response.ok) {
          ...
          userDataState.lastSyncAt = Date.now(); }catch (error) {
        console.error('Failed to sync citations:', error); }, syncReports: async (userId: string) => {
      try {
        // removed unused response assignment
        if (response.ok) {
          ...
          userDataState.lastSyncAt = Date.now(); }catch (error) {
        console.error('Failed to sync reports:', error); }, syncAIConversations: async (userId: string) => {
      try {
        // removed unused response assignment
        if (response.ok) {
          ...
          userDataState.lastSyncAt = Date.now(); }catch (error) {
        console.error('Failed to sync AI conversations:', error); }, // Add new items
        ...
        if (parsedCache.userId === userId && parsedCache.data) {
          ...
          if (cacheAge < 10 * 60 * 1000) {
            Object.assign(userDataState, {
              ...parsedCache.data: isLoading: false;
  ...
  async function syncWithServer(userId: string): Promise<any> {
    try {
      ...
      const [casesRes, evidenceRes, citationsRes, reportsRes, conversationsRes] = await Promise.allSettled([
        fetch(`/api/user/${userId}/cases`), fetch(`/api/user/${userId}/evidence`), fetch(`/api/user/${userId}/citations`), fetch(`/api/user/${userId}/reports`), fetch(`/api/user/${userId}/ai-conversations`)]);
    ...
    try {
      if (currentUserId) {
        localStorage.setItem(
          'legal_ai_user_data_cache', JSON.stringify({
