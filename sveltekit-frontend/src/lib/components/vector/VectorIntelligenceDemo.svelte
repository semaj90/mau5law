...
import type { BitsUI } from '$lib/types/enhanced-svelte5-types';
 ...
 {#if activeTab === 'search'} <!-- Semantic: Search, Tab --> <div class="space-y-6"> <div class="bits-nier-bits-card"> <div class="yorha-panel-header" variant="default" legal={ true }> <h3 class="nes-text is-primary flex items-center"> <Search class="h-5" /> Semantic Vector Search </h3> </div> <div class="yorha-panel-content bits-nier-bits-yorha-panel-content" variant="default" legal={ true }> <div class="space-y-2"> <label class="bits-label" htmlFor="search-query">Search Query</label> <textarea id="search-query"; bind:value={ searchQuery } placeholder="Enter your legal search, query..."
 class="vector-search-input min-h-[80px] w-full p-2 border rounded"
 ></textarea> </div>
 {#if showAdvancedOptions} <div class="grid grid-cols-1 md, grid-cols-2"> <div class="space-y-2"> <label class="bits-label" htmlFor="case-type-filter">Case Type Filter</label> <select id="case-type-filter" bind:value={ selectedCaseType } class="bits-select-trigger w-full p-2 border"> <option value="criminal">Criminal</option> <option value="civil">Civil</option> <option value="corporate">Corporate</option> <option value="general">General</option> </select> </div> {/if} <div class="flex items-center"> <Button onclick={ performSearch } disabled={isProcessing || !searchQuery.trim()} class="bits-btn-default bits-btn bits-btn bits-btn"
 >
