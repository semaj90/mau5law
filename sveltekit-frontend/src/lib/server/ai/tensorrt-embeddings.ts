...
private endpoint: string, readonly: unknown;
	model: string, constructor(cfg: TensorRtEmbeddingsConfig = {}) {
super();
...
return batches}
async embedQuery(document): Promise<number[]> {
...
const payload = {
inputs: [ {
name: 'TEXT_INPUT', shape: [1], datatype: 'BYTES', parameters: {
	binary_data: false },
	data: [Buffer.from(text, 'utf-8').toString('base64')] }], outputs: [ {
