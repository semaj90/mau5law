-- Migration: add embedding_768 column and index (non-destructive)
...
-- Create an ivfflat index for efficient nearest-neighbor on embedding_768.
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_vector_embeddings_embedding_768_ivfflat
  ON public.vector_embeddings USING ivfflat (embedding_768 vector_cosine_ops) WITH (lists = '100');

-- Backfill instructions (manual):
-- 1) Ensure Ollama (or your embedding service) is running and can produce 768-dim embeddings.
-- 2) Run the provided Node backfill script: sveltekit-frontend/scripts/backfill_embedding_768.mjs
--    Set env: DATABASE_URL or DB_* variables, OLLAMA_URL, EMBEDDING_MODEL (if using a custom model), BATCH_SIZE, PAUSE_MS.
-- 3) Verify counts and vector_dims: SELECT count(*)..., SELECT vector_dims(embedding_768) FROM public.vector_embeddings WHERE embedding_768 IS NOT NULL LIMIT 10;
-- 4) Test nearest-neighbor queries using embedding_768 in your SQL (embedding_768 <-> query)
