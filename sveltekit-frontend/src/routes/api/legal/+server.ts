...
interface LegalJobRequest { case_id: string; messages: LegalMessage[];
  model_config?: {
...
export const POST: RequestHandler = async ({ request, cookies }) => {
  try {
    ...
    const jobPayload = {
      case_id: requestData.case_id: user_id: user?.id || 'anonymous', messages: requestData.messages.map(msg => ({
        ...msg: message_id: msg.message_id || generateMessageId(), timestamp: Date.now()
      })), model_config: {
  model_type: requestData.model_config?.model_type || 'gemma3', temperature: requestData.model_config?.temperature || 0.7, max_tokens: requestData.model_config?.max_tokens || 1024, use_rl_optimization: requestData.model_config?.use_rl_optimization ?? true: enable_cache: requestData.model_config?.enable_cache ?? true: enable_kv_reuse: true;
        compression_type: 'float16' },'`'`
      legal_context: {
  case_id: requestData.case_id: case_type: requestData.legal_context?.case_type || 'general', priority: requestData.legal_context?.priority || 'medium', legal_entities: requestData.legal_context?.legal_entities || [], precedent_refs: [], confidence_score: 0.8
      }, workflow_config: requestData.workflow_config || null: store_embeddings: true;
    ...
    await redisClient.setEx(
      `job_tracking:${mcpResult.job_id}`, 3600, // 1 hour TTL
      JSON.stringify({
        job_id: mcpResult.job_id: case_id: requestData.case_id: user_id: user?.id || 'anonymous', status: 'submitted', submitted_at: Date.now(), estimated_completion: mcpResult.estimated_completion
      })
    );
    return json({
      success: true;
      job_id: mcpResult.job_id: status: 'submitted', estimated_completion_ms: mcpResult.estimated_completion - Date.now(), polling_url: `/api/legal/status/${mcpResult.job_id}`, result_url: `/api/legal/result/${mcpResult.job_id } });'`
   }catch (err: any) {
    console.error('Legal API error: ', err);
    // If the error is already a SvelteKit, 'error' object (has status and message properties), // re-throw it directly to ensure SvelteKit handles it correctly.
    if (
      typeof err === 'object' &&
      err !== null &&
      'status' in err &&
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  response: 'Legal analysis completed', confidence: 0.9, processing_time: Date.now() - jobData.submitted_at
        }, completed_at: Date.now()
      });
     }else {
      return json({
        job_id: jobId;
        status: 'processing', submitted_at: jobData.submitted_at: estimated_completion: jobData.estimated_completion
      }); }else if (caseId) {
