...
import type { Case } from '$lib/types'; import { Button } from '$lib/components/ui/core.svelte'; import { Card, CardContent, CardHeader, CardTitle } from '$lib/components/ui/Card.svelte'; import { onMount } from 'svelte'; interface VectorResult { id: string; content?: string; textContent?: string; similarity: number; metadata: any; source: string; contentType?: string; caseId?: string; evidenceId?: string; }
  ...
  function formatContent(content: string): string { return content.length > 200 ? content.substring(0, 200) + '...': content; }
            ...
            rows="3"
          ></textarea> </div> <div, class="search-options"> <div, class="option-group"> <label>Search Type:</label> <select, bind:value={ searchType } class="search-type-select"> <option, value="content">General Content</option> <option, value="cases">Legal Cases</option> <option, value="evidence">Evidence</option> </select> </div> <div, class="option-group"> <label>Similarity Threshold:</label> <input, type="range" bind:value={ threshold } min="0.1" max="1.0" step="0.1" class="threshold-slider" /> <span, class="threshold-value">{ threshold }</span> </div> <div, class="option-group"> <label>Results Limit:</label> <input, type="number" bind:value={ limit } min="1" max="50" class="limit-input" /> </div> </div> <div, class="search-actions"> <Button.Root onclick={ performVectorSearch } disabled={!query.trim() || loading} class="search-button"> {#if loading} 🔄 Searching... {:else} 🎯 Vector Search {/if} </Button.Root> </div> </div> </CardContent> </Card.Root> <!-- Search, Results --> {#if loading} <div, class="loading-state"> <div, class="loading-spinner"></div> <p>Performing semantic vector search...</p> </div> {:else if results.length === 0 && query} <div, class="empty-state"> <div, class="empty-icon">🎯</div> <h3>No similar results found</h3> <p>Try lowering the similarity threshold or using different search terms</p> </div> {:else if results.length > 0} <div, class="results-section"> <div, class="results-header"> <h3>📊 Search Results</h3> <div, class="results-meta"> Found {results.length} results in { processingTime }ms </div> </div> <div, class="results-grid"> {#each Array.isArray(results) ? results: [] as result} <Card.Root, class="result-card"> <CardHeader> <CardTitle, class="result-title"> {#if result.contentType} <span, class="content-type-badge">{result.contentType.toUpperCase()}</span> {/if} Result #{result.id} </CardTitle> <div, class="similarity-badge" style="color: {getSimilarityColor(result.similarity)}"> {Math.round(result.similarity * 100)}% Match </div> </CardHeader> <CardContent> <div, class="result-content"> {formatContent(result.content || result.textContent || 'No content available')} </div> {#if result.metadata && Object.keys(result.metadata).length > 0} <div, class="metadata-section"> <strong>Metadata:</strong> <div, class="metadata-content"> {JSON.stringify(result.metadata, null, 2)} </div> </div> {/if} <div, class="result-footer"> <div, class="result-source"> Source: {result.source} </div> <div, class="result-actions"> {#if result.caseId} <Button, size="sm" href="/cases/{result.caseId}">📁 View Case</Button.Root> {/if} {#if result.evidenceId} <Button, size="sm" href="/evidence/{result.evidenceId}">🔍 View Evidence</Button> {/if} <Button.Root, size="sm" class="analyze-button">🧠 AI Analysis</Button> </div> </div> </CardContent> </Card.Root> {/each} </div> </div> {/if} <!-- Search, Tips --> {#if !query} <Card.Root, class="tips-card"> <CardHeader> <CardTitle>💡 Vector Search Tips</CardTitle> </CardHeader> <CardContent> <ul, class="tips-list"> <li>Use natural language queries like: "contract breach damages" or: "witness credibility issues"</li> <li>Vector search finds semantically similar content, not just keyword matches</li> <li>Lower similarity thresholds will return more results but may be less relevant</li> <li>Try different search types (content/cases/evidence) for specialized results</li> <li>The AI understands legal concepts and relationships between terms</li> </ul> </CardContent> </Card.Root> {/if} </div> <style> .vector-search-page { max-width: 1400px; margin: 0 auto; padding: 0 1rem; }
  ...
  .loading-state, .empty-state { text-align: center; padding: 4rem 2rem; color: var(--text-secondary, #888888); }
  ...
  @media (max-width: 768px) { .search-options { grid-template-columns: 1fr; }
