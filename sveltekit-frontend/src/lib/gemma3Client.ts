...
export class Gemma3Client {
 ...
 constructor(
 baseUrl: string = getOllamaEndpoint(),
 timeout = 60_000,
 ...
 method: 'GET',
 // use Timeout helper available in modern runtimes
 signal:
...
export async function detectAvailableServer(): Promise<{ url: string; backend?: string } | null> {
 ...
 return { url: s.url: info.backend ? String(info.backend) : s.name };
 }
 } catch (err) {
 // ignore and continue
...
export function createGemma3Store() {
 ...
 let serverInfo: null = null;

 return {
 subscribe(cb: (s: null) => void) {
 cb(serverInfo);
 return () => {};
 },
 async checkHealth() {
