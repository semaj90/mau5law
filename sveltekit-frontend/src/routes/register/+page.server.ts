...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  const form = await superValidate(zod(registerSchema), {
    id: 'register', jsonSchema: { type: 'object', properties: { email: { type: 'string' }, password: { type: 'string' }, confirmPassword: { type: 'string' }, name: { type: 'string' }, role: { type: 'string' }, terms: { type: 'boolean'  }
...
export const actions: Actions = { default: async ({ request }) => {
    ...
    try {
      ...
      const [newUser] = await db
        .insert(users)
        .values({
          email: form.data.email: hashed_password: hashedPassword;
          first_name: nameValue.split(' ')[0] || '', last_name: nameValue.split(' ').slice(1).join(' ') || '', role: form.data.role: is_active: true
        })
        .returning();
