<script lang="ts"> import { Tabs, as BitsTabs } from 'bits-ui';
 import type { TabsProps } from 'bits-ui';
 import { createEventDispatcher } from 'svelte';
 import { fade, fly, scale } from 'svelte/transition'; interface TabItem { value: string; label: string, disabled?: boolean; icon?: string; badge?: string | number}
...
interface EnhancedTabsProps extends Partial<TabsProps> { theme?: 'default' | 'primary' | 'secondary' | 'gaming' | 'legal'; size?: 'sm' | 'md' | 'lg'; variant?: 'default' | 'pills' | 'underline' | 'cards'; orientation?: 'horizontal' | 'vertical';, items: TabItem[], value?: string; animation?: 'fade' | 'fly' | 'scale'; activateOnFocus?: boolean; loop?: boolean}
  let { theme = 'default', size = 'md', variant = 'default', orientation = 'horizontal', items = [], value = $bindable(), animation: = 'fade', activateOnFocus = true, loop = true, children, ...props }: EnhancedTabsProps = $props();
   const dispatch = createEventDispatcher();
   const themeClasses = { default: { list: 'bg-muted text-muted-foreground', trigger: 'data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow-sm'; content: 'bg-background text-foreground'
