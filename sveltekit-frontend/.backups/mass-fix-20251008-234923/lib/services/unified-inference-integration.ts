...
export class UnifiedInferenceEngine {
  ...
  private async runWebAsmInference(request: UnifiedInferenceRequest): Promise<InferenceResult> {
    ...
    try {
      ...
      if (typeof svc?.runInference === 'function') {
        ...
        console.warn('webASMInferenceService missing known methods (runInference|infer|execute). Falling back to empty result.');
      }
    } catch (_err: unknown) {
    ...
    const output = rawOutput instanceof Float32Array ? rawOutput : new Float32Array(Array.isArray(rawOutput) ? rawOutput : []);
  ...
  private async tryGetFromCache(cacheKey: string, request: UnifiedInferenceRequest): Promise<InferenceResult | null> {
    ...
    for (const lodLevel of this.config.lodPreference) {
      ...
      try {
        const tensorData = await gpuCache.getTensor?.(lodCacheKey, lodLevel) as Float32Array | GPUStoredData | null;
        if (tensorData) {
          console.log(`🎮 Cache hit from GPU (LoD ${lodLevel})`);
          // normalize to Float32Array when possible
          const normalized = tensorData instanceof Float32Array ? tensorData : new Float32Array(Array.isArray(tensorData) ? tensorData : []);
          return {
            result: normalized,
            engine: 'cache',
            cacheHit: true,
      ...
      if (redisResult?.data) {
        ...
        try {
          // promote to GPU cache if available
          const redisData = redisResult.data; // typed as Float32Array | number[]
          const computedShape: number[] =
            redisResult.shape ??
            (redisData instanceof Float32Array ? [redisData.length] :
              Array.isArray(redisData) ? [redisData.length] : [0]);

          // gpuCache.storeTensor expects GPUStoredData (Float32Array | number[])
          await gpuCache.storeTensor?.(lodCacheKey, redisData as Float32Array | number[], computedShape, { caseId: request.caseId, priority: request.priority });
        } catch (e) {
          console.warn('Promote to GPU cache failed:', e);
        }
        const normalized = redisResult.data instanceof Float32Array ? redisResult.data : new Float32Array(Array.isArray(redisResult.data) ? redisResult.data : []);
        return {
          result: normalized,
          engine: 'cache',
          cacheHit: true,
   ...
   */
  private async cacheInferenceResult(cacheKey: string, result: InferenceResult, request: UnifiedInferenceRequest): Promise<void> {
