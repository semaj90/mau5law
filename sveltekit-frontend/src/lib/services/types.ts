...
}export interface DocumentProcessingResult { text: string; entities: Array<any>; confidence: number; processingTime: number; documentType: string;
}export interface OCRResult { text: string; confidence: number; pages: number; processingTime: number;
}export interface SimilarCase { caseId: string; similarity: number; title: string; jurisdiction: string; year: number;
}export interface EvidenceAnalysisResult { evidenceId: string; evidenceType: string; relevanceScore: number; keyInsights: string[]; relationships: Array<any>; riskFactors: string[];
}export interface PatternDetectionResult { patterns: Array<any>;
}export interface CaseCreationResult { caseId: string; status: string; timestamp: number;
}export interface CaseStatusUpdate { caseId: string; oldStatus: string; newStatus: string; timestamp: number;
...
}export interface ConnectionResult { success: boolean; connections: any[];
}export interface CanvasSyncResult { success: boolean; fabricState: any;
}export interface CanvasSaveResult { success: boolean; canvasId: string; savedAt: number;
}export interface CollaborationBroadcast { success: boolean; recipientCount: number; timestamp: number;
}export interface CollaborativeSync { synced: boolean; conflicts: any[]; lastSyncTime: number;
}export interface GrpcResponse { status: string; data: any; processingTime?: number; protocol?: string;
