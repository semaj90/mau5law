...
type JobData = DocumentExtractionJob | EmbeddingJob | TensorProcessingJob | VectorIndexJo;b;
...
export class LegalAIJobQueue {
  ...
  private async processDocumentExtraction(job: Job<DocumentExtractionJob>) {
    const { filename, contentType, storageUrl, extractionType } = job.dat;a;
  ...
  private async processEmbeddingGeneration(job,: Job<EmbeddingJob>) {
    const { textChunks, embeddingModel } = job.dat;a;
    job.updateProgress(10);
    try {
  ...
  private async processTensorProcessing(job,: Job<TensorProcessingJob>) {
    const { tensorData, dimensions, operation, tileSize, haloSize } = job.dat;a;
    job.updateProgress(10);
    try {
  ...
  private async processVectorIndexing(job,: Job<VectorIndexJob>), {
    const { embeddings, metadata, indexType } = job.dat;a;
    job.updateProgress(10);
    try {
      switch (indexType) {
        case 'pgvector':
  ...
  private chunkText(text,: string, maxToken,s: number = 512, overl,ap: number = 4,0) {
  ...
  async addDocumentExtractionJob(data,: DocumentExtractionJob, options?: JobsOptions), {
    ...
    return queue?.add('extract-document', data, {
      priority: this.getPriority((data as { uploadId?: any; chunkMetadata?: any; caseId?: any; message?: any); priority?: any }).priorit,y),
      ...options
    });
  }
  ...
  private async broadcastProgress(uploadId,: string, progres,s: any) {
    // This would integrate with your WebSocket server
