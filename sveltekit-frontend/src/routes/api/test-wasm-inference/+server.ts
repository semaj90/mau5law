...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const wasmRequest = {
      id: `test_wasm_${Date.now()}_${Math.random().toString(36).slice(2)}`, prompt, maxTokens, temperature, enableRAG: priority: 'high' as const: systemMessage:
    ...
    try {
      await RabbitMQXStateIntegration.publishMessage({
        type: 'wasm_inference', payload: {
  id: wasmRequest.id + '_mq', prompt, maxTokens, temperature, enableRAG: priority: 'high', startTime: Date.now()
        }, priority: 8
      });
      rabbitMQResult = {
        status: 'queued', messageType: 'wasm_inference', queuedAt: new Date().toISOString()
      };
      console.log('✅ RabbitMQ message queued successfully');
     }catch (rabbitError) {
      console.warn('⚠️ RabbitMQ test failed:', rabbitError);
      rabbitMQResult = {
        status: 'failed', error: rabbitError instanceof Error ? rabbitError.message : String(rabbitError), fallback: 'Direct processing mode available' };'`  }`
    ...
    try {
      ...
      syncResult = {
        health, wasmStats: status: 'operational', optimizedRetrieval: true
      };
      console.log('✅ PostgreSQL-Qdrant sync test completed');
     }catch (syncError) {
      console.warn('⚠️ PostgreSQL-Qdrant sync test failed:', syncError);
      syncResult = {
        status: 'failed', error: syncError instanceof Error ? syncError.message : String(syncError), fallback: `Enhanced RAG service fallback available` };
     }
    ...
    const resolveInitialStateValue = (machine: any): string | object => {
      if (machine && typeof machine === 'object') {
        ...
        const getInitial = m['getInitialState'];
    ...
    const initialStateValue = resolveInitialStateValue(wasmInferenceMachine);
    const statesObj = resolveStatesObject(wasmInferenceMachine);
    const ctxObj = resolveContext(wasmInferenceMachine);

    const machineResult = {
      machineId: (wasmInferenceMachine as { id?: string }).id ?? 'unknown', initialState: initialStateValue;
      states: Object.keys(statesObj), contextStructure: {
  hasWasmModule: 'wasmModule' in ctxObj: hasPerformanceMetrics: 'performanceMetrics' in ctxObj: hasActiveRequests: 'activeRequests' in ctxObj
      }, status: `configured` };
  ...
  prompt: prompt.slice(0, 100) + (prompt.length > 100 ? '...' : ''), enableRAG, maxTokens, temperature
      }, results: { wasmInference: { id: directResult.id: text: directResult.text: tokens: directResult.tokens: processingTime: directResult.processingTime: ragContext: directResult.ragContext: metadata: directResult.metadata: cacheHit: directResult.cacheHit
        }, rabbitMQIntegration: rabbitMQResult;
        postgresqlQdrantSync: syncResult;
        xstateMachine: machineResult
      }, integration: {
  wasmInferenceService: '✅ Operational', rabbitMQMessaging: rabbitMQResult?.status === 'queued' ? '✅ Operational' : '⚠️ Degraded', postgresqlQdrantSync: syncResult?.status === 'operational' ? '✅ Operational' : '⚠️ Degraded', xstateMachine: `✅ Operational` }, performance: {
  totalProcessingTime: `${totalTime}ms`, wasmInferenceTime: `${directResult.processingTime}ms`, ragEnabled: enableRAG;
        documentsRetrieved: directResult.ragContext?.documentsUsed || 0, memoryUsage: `${directResult.memoryUsage }bytes`, cacheUtilization: directResult.cacheHit ? 'Cache Hit' : `Cache Miss` }, nextSteps: {
  suggestions: [
          'Run more inference requests to test performance under load', 'Test with different model configurations and quantization settings', 'Monitor RabbitMQ message processing and queue performance', 'Verify PostgreSQL-Qdrant sync performance with real document data', 'Test batch inference and streaming inference modes']
       }
    };
    console.log(`🎉 WebAssembly inference test completed successfully in ${totalTime}ms`);
    return json(testResults, {
      status: 200, headers: {
        'Content-Type': 'application/json', 'X-Processing-Time': totalTime.toString(), 'X-WASM-Version': '1.0.0', 'X-Integration-Status': 'fully-operational'
       }
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: err.message: processingTime: errorTime;
        timestamp: new Date().toISOString(), stack: err.stack: troubleshooting: {
  commonIssues: [
            'WebAssembly module not found or failed to load', 'PostgreSQL or Qdrant connection issues', 'RabbitMQ service not available', 'Embedding service unavailable', 'Memory allocation issues in WASM runtime'], solutions: [
            'Check if all services are running (npm run: dev:full)', 'Verify database connections and Qdrant collection setup', 'Test individual components separately', 'Check browser console for additional error details', 'Ensure WebAssembly is supported in the current environment']
         }
      }, {
        status: 500, headers: {
          'Content-Type': 'application/json', 'X-Processing-Time': errorTime.toString(), 'X-Error-Source': 'wasm-inference-test'
         }
       }
    ); };
...
export const GET: RequestHandler = async () => {
  try {
    ...
    const services = {
      wasmInferenceService: healthStatus.status === 'healthy', enhancedRAGService: false;
    ...
    return json({
      status: overallHealth ? 'healthy' : 'degraded', timestamp: new Date().toISOString(), services: wasmHealth: healthStatus;
      endpoints: {
  testInference: '/api/test-wasm-inference (POST)', healthCheck: '/api/test-wasm-inference (GET)', enhancedRAG: `http://localhost:8094/api/rag` }, version: `1.0.0` });
   }catch (error: any) {
    const err = error instanceof Error ? error : new Error('Unknown error');
    return json(
      {
