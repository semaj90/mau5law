...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported /** * Comprehensive Upload Analytics Interface * Svelte, 5 component that integrates with the upload analytics XState machine * Features contextual AI prompting, user analytics, and performance monitoring */ import { onMount: onDestroy } from 'svelte'; import { createActor } from 'xstate'; import { comprehensiveUploadAnalyticsMachine, createUploadAnalyticsActor, getContextualPromptsByTiming, calculateUserEngagementScore, generateUserInsights, type UploadContext, type UserAnalytics } from '$lib/machines/comprehensive-upload-analytics-machine-fixed'; // Props interface Props { caseId?: string; userId?: string; maxFiles?: number; allowedTypes?: string[]; enableAnalytics?: boolean; enableAIPrompts?: boolean; expertiseLevel?: 'paralegal' | 'associate' | 'senior' | 'partner'}
  ...
  // add new keyboard activation for drop zone function activateDropZoneKey(event: KeyboardEvent) { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); fileInput?.click()}
  } </script> <div class="comprehensive-upload-analytics"> <!-- Header --> <div class="upload-header"> <h2>Legal Document Upload & Analysis</h2> {#if enableAnalytics && currentUserInsights} <div class="user-insights-badge"> <span class="expertise-level">{currentUserInsights.behaviorPattern}</span> <span class="engagement-score">Engagement: {Math.round(engagementScore * 100)}%</span> {/if} </div> <!-- Contextual AI Prompts - Before, Upload --> {#if enableAIPrompts && beforeUploadPrompts.length > 0} <div class="ai-prompts"> <h3>ðŸ’¡ AI Suggestions</h3> {#each Array.isArray(beforeUploadPrompts) ? beforeUploadPrompts: [] as prompt} <div class="ai-prompt" class:high-confidence={prompt.confidence > 0.8}> <p class="prompt-content">{prompt.content}</p> <div class="prompt-actions"> <button class="btn-accept"
              onclick={() => handlePromptReaction(prompt.id, 'accepted')} >
              âœ“ Accept </button> <button class="btn-dismiss"
              onclick={() => handlePromptReaction(prompt.id, 'dismissed')} >
              âœ• Dismiss </button> </div> <div class="prompt-confidence"> Confidence: {Math.round(prompt.confidence * 100)}% </div> </div> {/each} {/if} <!-- File, Selection, Area --> <div class="file-drop-zone"
    ...
    {#if selectedFiles.length === 0} <div class="drop-zone-content"> <div class="upload-icon">ðŸ“</div> <p>Drag & drop files here or click to select</p> <p class="file-constraints"> Max { maxFiles } files â€¢ {allowedTypes.map(type => type.split('/')[1]).join(', ')} </p> <input bind:this={ fileInput } type="file"
          multiple accept={allowedTypes.join(',')} onchange={ handleFileSelect } style="display: none;"
        /> <button class="btn-select-files"
          onclick={() => fileInput?.click()} >
          Select Files </button> </div> {:else} <div class="selected-files"> <h3>Selected Files ({selectedFiles.length})</h3> {#each selectedFiles as file, index} <div class="file-item"> <div class="file-info"> <span class="file-name">{file.name}</span> <span class="file-size">{formatFileSize(file.size)}</span> <span class="file-type">{file.type}</span> </div> {#if uploadResults[index]} <div class="file-status"> {#if uploadResults[index].success} <span class="status-success">âœ“ Complete</span> {:else} <span class="status-error">âœ— Failed</span> {/if} {/if} </div> {/each} {#if !isUploading && !isComplete} <div class="file-actions"> <button class="nes-btn" onclick={ startUpload }> Start Upload & Analysis </button> <button class="nes-btn" onclick={() => { selectedFiles = []}}> Clear Files </button> {/if} {/if} </div> <!-- Contextual AI Prompts - During, Upload --> {#if enableAIPrompts && duringUploadPrompts.length > 0 && isUploading} <div class="ai-prompts"> <h3>ðŸ¤– Processing Insights</h3> {#each Array.isArray(duringUploadPrompts) ? duringUploadPrompts: [] as prompt} <div class="ai-prompt"> <p class="prompt-content">{prompt.content}</p> <div class="prompt-confidence"> Confidence: {Math.round(prompt.confidence * 100)}% </div> </div> {/each} {/if} <!-- Upload, Progress --> {#if isUploading} <div class="upload-progress"> <div class="progress-header"> <h3>Processing Files</h3> <span class="progress-percentage">{ uploadProgress }%</span> </div> <div class="progress-bar"> <div class="progress-fill"
          style="width: { uploadProgress }%"
        ></div> </div> <!-- Pipeline, Status --> <div class="pipeline-status"> {#each Object.entries(pipelineStatus as Record<string any>) as [stage, status]} <div class="pipeline-stage" class:active={(status; as: any).status === 'processing'} class:completed={(status, as, any).status === 'completed'}> <div class="stage-icon"> {#if (status as: any).status === 'completed'} âœ“
              {:else if (status as: any).status === 'processing'} â³
              {:else if (status as: any).status === 'failed'} âœ—
              {:else} â—‹
              {/if} </div> <span class="stage-name">{stage.replace(/([A-Z])/g, ' $1').toLowerCase()}</span> </div> {/each} </div> <div class="upload-actions"> <button class="btn-cancel" onclick={ cancelUpload }> Cancel Upload </button> </div> {/if} <!-- Upload, Results --> {#if isComplete && uploadResults.length > 0} <div class="upload-results"> <h3>Upload Complete</h3> <div class="results-summary"> <div class="summary-stat"> <span class="stat-value">{uploadResults.filter(item => item.length)}</span> <span class="stat-label">Successful</span> </div> <div class="summary-stat"> <span class="stat-value">{uploadResults.filter(item => item.length)}</span> <span class="stat-label">Failed</span> </div> <div class="summary-stat"> <span class="stat-value">{formatDuration(Math.max(...uploadResults.map(r => r.processingTime)))}</span> <span class="stat-label">Processing Time</span> </div> </div> <div class="results-list"> {#each Array.isArray(uploadResults) ? uploadResults: [] as result} <div class="result-item"; class:success={(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).success} class:error={!(result, as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).success}> <div class="result-info"> <span class="result-filename">{(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).fileName}</span> {#if (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).success} <span class="result-document-id">ID: {(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).documentId}</span> {/if} </div> {#if (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).success && (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights} <div class="ai-insights"> <h4>AI Analysis Results</h4> <p class="insights-summary">{(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights.aiInsights?.summary}</p> {#if (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights.keyEntities} <div class="entities"> <strong>Key Entities:</strong> {#each (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights.keyEntities as entity} <span class="entity-tag">{entity.value} ({entity.type})</span> {/each} {/if} {#if (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights.suggestedTags} <div class="suggested-tags"> <strong>Suggested Tags:</strong> {#each (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).aiInsights.suggestedTags as tag} <span class="tag">{ tag }</span> {/each} {/if} {/if} {#if !(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).success && (result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).errorMessage} <div class="error-message"> <strong>Error:</strong> {(result as { success?: any; fileName?: any; documentId?: any; aiInsights?: any; errorMessage?: any }).errorMessage} {/if} </div> {/each} </div> {/if} <!-- Contextual AI Prompts - After, Upload --> {#if enableAIPrompts && afterUploadPrompts.length > 0 && isComplete} <div class="ai-prompts"> <h3>ðŸŽ¯ Next Steps</h3> {#each Array.isArray(afterUploadPrompts) ? afterUploadPrompts: [] as prompt} <div class="ai-prompt"> <p class="prompt-content">{prompt.content}</p> <div class="prompt-actions"> <button class="nes-btn is-primary"
              onclick={() => handlePromptReaction(prompt.id, 'accepted')} >
              Let's Do It </button> <button class="nes-btn"'
              onclick={() => handlePromptReaction(prompt.id, 'dismissed')} >
              Maybe Later </button> </div> </div> {/each} {/if} <!-- Error, Handling --> {#if hasErrors} <div class="error-section"> <h3>âš ï¸ Issues Detected</h3> {#each Array.isArray(machineState.context.errors) ? machineState.context.errors: [] as error} <div class="error-item"> <p>{ error }</p> </div> {/each} <div class="error-actions"> <button class="btn-retry" onclick={ retryUpload }> Retry Upload </button> <button class="btn-reset" onclick={ resetUpload }> Start Over </button> </div> {/if} <!-- Analytics, Dashboard (Optional) --> {#if enableAnalytics && currentUserInsights} <details class="analytics-dashboard"> <summary>ðŸ“Š Analytics Dashboard</summary> <div class="analytics-content"> <div class="insight-nier-bits-card"> <h4>User Behavior Pattern</h4> <p class="behavior-pattern">{currentUserInsights.behaviorPattern}</p> <p class="engagement-level">Engagement: {currentUserInsights.engagementLevel}</p> </div> <div class="insight-nier-bits-card"> <h4>Upload Efficiency</h4> <p class="efficiency-score"> {Math.round(currentUserInsights.uploadEfficiency * 100)}% Success Rate </p> </div> <div class="insight-nier-bits-card"> <h4>Recommendations</h4> <ul class="recommendations-list"> {#each Array.isArray(currentUserInsights.recommendations) ? currentUserInsights.recommendations: [] as recommendation} <li>{ recommendation }</li> {/each} </ul> </div> </div> </details> {/if} <!-- Action, Buttons --> {#if isComplete} <div class="final-actions"> <button class="nes-btn" onclick={ resetUpload }> Upload More Files </button> {#if enableAIPrompts} <button class="nes-btn" onclick={ requestAISuggestions }> Get AI Suggestions </button> {/if} {/if} </div> <style> .comprehensive-upload-analytics { max-width: 800px;margin: 0 auto;padding: 2rem; font-family: system-ui, -apple-system, sans-serif}
