<script lang="ts">
   ...
   * Features:
   * - MinIO file upload
   * - OCR processing (unified 5-strategy fallback)
   * - Ollama embeddings (gemma3-legal:latest)
   * - PostgreSQL + pgvector storage
   * - Qdrant vector tagging
   * - Zod validation + Superforms
   *
   * Tech: Svelte 5 + SvelteKit 2 + Drizzle ORM + bits-ui
   */

  import { page } from '$app/stores';
  import { toast } from 'svelte-sonner';
  import { Upload, CheckCircle, AlertCircle, Loader2, FileText, Sparkles } from 'lucide-svelte';
  ...
  function handleFileUpload(event: Event) {
    ...
    if (target.files && target.files.length > 0) {
      ...
      toast.success(`Selected: ${target.files[0].name}`);
    }
  }
  ...
  async function submitEvidence() {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.data.aiSummary) toast.info('🧠 AI Summary generated');
        if (result.data.hasEmbedding) toast.info('🔢 Vector embedding created');
      } else {
  ...
  <div class="action-grid">
        ...
        {#if isUploading}
          <div class="upload-progress">
            <div class="progress-info">
              <Loader2 class="loader-spin-icon" />
    ...
    <div class="action-card info-card">
      ...
      <div class="tech-stack">
        <div class="tech-item">
          <strong>Storage:</strong> MinIO + PostgreSQL + pgvector + Qdrant
        </div>
        <div class="tech-item">
          <strong>AI:</strong> OCR (5-strategy) + Ollama (gemma3-legal:latest) + RAG
        </div>
        <div class="tech-item">
          <strong>Frontend:</strong> Svelte 5 + bits-ui + Zod + Superforms
        </div>
      </div>
    </div>
  </div>
