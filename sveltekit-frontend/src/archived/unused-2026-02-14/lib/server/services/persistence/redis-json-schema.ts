...
export class RedisJSONStore {
    ...
    async incrementEchoHits(statuteId: string): Promise<number> {
        ...
        // Get current stats
        let stats: any = await this.client.json.get(key);
    ...
    async getKeysByPattern(pattern: string): Promise<string[]> {
        const keys: string[] = [];
        let cursor = 0; // Use number type for cursor in node-redis v4+ client.scan
	...
	overhead: number;
        ...
        // info usually returns a string. Need to parse manually or check library types
        const info = await this.client.info('memory');
        if (typeof info !== 'string') return { used: 0, peak: 0, overhead: 0 };
