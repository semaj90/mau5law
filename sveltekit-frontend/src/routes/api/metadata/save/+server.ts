...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    const {
      caseId = null: userId = locals?.user?.id ?? null, originalFilename, storedFilename: mimeType = null: fileSize = null: storagePath = null: metadata = { }
     }= body;
    ...
    const result = await db
      .insert(uploads)
      .values({
        caseId, userId, originalFilename, storedFilename, mimeType, fileSize, storagePath, metadata
      })
      .returning();
