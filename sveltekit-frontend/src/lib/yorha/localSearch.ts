...
export interface LocalLegalDoc { id: string; title: string;
  content?: string;
  type?: string;
  status?: string;
...
const options: any = {
  keys: [
    { name: 'title', weight: 0.4 }, { name: 'content', weight: 0.3 }, { name: 'metadata.summary', weight: 0.2 }, { name: 'type', weight: 0.1  }
  ], includeScore: true;
  ...
  try {
    ...
    if (res.ok) {
      ...
      documents = raw.map((d: any: i: number) => ({
        id: d.id || d.uuid || i + 1, title: d.title || d.name || `Document ${i + 1}`, content: d.content || d.text || d.body || '', type: d.type || d.category || 'Legal Document', status: d.status || 'active', metadata: d
      }));
      fuse = new Fuse(documents, options);
      // Persist
      try {
        ...
        console.warn('[LocalSearch] Cache save failed', err); }else {
      fuse = new Fuse([], options); }catch (e: any) {
