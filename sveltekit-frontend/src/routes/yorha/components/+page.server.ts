  ...
  try {
    ...
    const componentSpecs = {
      ...
      dataviz: {
        ...
        defaultConfig: { type: 'line', animated: true, realTime: true, theme: 'cyberpunk' },
      },
    };
    ...
    const galleryData = {
      ...
      totalComponents: Object.keys(componentSpecs).length, // Corrected from colon to comma
      previewModes: [
    ...
    const codeSnippets = {
      button: `
<script lang="ts">
    import type { YoRHaButton3D  } from '$lib/components/three/yorha-ui.svelte';
...
`,
      panel: `
<script lang="ts">
    import type { YoRHaPanel3D  } from '$lib/components/three/yorha-ui.svelte';
</script>
...
`,
      input: `
<script lang="ts">
    import type { YoRHaInput3D  } from '$lib/components/three/yorha-ui.svelte';
    let inputValue = '';
</script>
...
`,
      modal: `
<script lang="ts">
    import type { YoRHaModal3D  } from '$lib/components/three/yorha-ui.svelte';
...
`,
    ...
    return {
      ...
      gallery: galleryData,
      codeSnippets, // Use the variable directly
      initialLoad: true, // Corrected property assignment
      timestamp: new Date().toISOString(),
    };
  } catch (error: Error | unknown) {
    console.error('Error loading YoRHa data: ', error); // Removed trailing backtick
    return {
      specs: {},
