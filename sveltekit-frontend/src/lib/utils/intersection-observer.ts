...
class LazyLoadManager {
  ...
  private handleIntersection(entries: IntersectionObserverEntry[]) {
    entries.forEach(entry => {
      ...
      if (callback) {
        const lazyEntry: LazyLoadEntry = { element: entry.target as Element,
...
class LazyLoadProfiler {
  private metrics: LazyLoadMetrics = { totalObserved: 0,
    totalLoaded: 0,
    averageLoadTime: 0,
    loadTimes: []
  };
...
export const LAZY_LOAD_PRESETS = {
  ...
  HEAVY_COMPONENT: { rootMargin: '200px',
    threshold: 0,
    once: true,
    fallbackDelay: 1000
  },
  // For images and media
  MEDIA: { rootMargin: '50px',
    threshold: 0,
    once: true
  },
  // For continuous monitoring (animations, etc.)
  CONTINUOUS: { rootMargin: '0px',
