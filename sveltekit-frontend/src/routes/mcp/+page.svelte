<script lang="ts">
import type { Document } from '$lib // TODO: Verify store subscription is correct for Svelte 5 // TODO: Verify store subscription is correct for Svelte 5/types'; // Svelte, 5 runes are auto-imported import { goto } from '$app // TODO: Verify store subscription is correct for Svelte 5 // TODO: Verify store subscription is correct for Svelte 5/navigation'; import { mcpApi } from '$lib // TODO: Verify store subscription is correct for Svelte 5 // TODO: Verify store subscription is correct for Svelte 5/api/mcp-client.js'; import { writable } from 'svelte/store'; // Server status const serverStatus = writable({ status: 'checking', workers: 0, uptime: 0, version: 'Unknown'
    ...
    }]; // Recent activity (mock data) const recentActivity = [ { time: '2 min ago', action: 'Processed employment contract', status: 'completed', risk: 25 }, { time: '5 min ago', action: 'Analyzed NDA template', status: 'completed', risk: 15 }, { time: '8 min ago', action: 'Real estate agreement review', status: 'completed', risk: 42 }, { time: '12 min ago', action: 'Corporate merger LOI', status: 'completed', risk: 38 }, { time: '15 min ago', action: 'Partnership agreement scan', status: 'completed', risk: 22 }]; // Check server status on mount / reactive effect $effect // TODO: Verify store subscription is correct for Svelte 5 // TODO: Verify store subscription is correct for Svelte 5(() => { (async () => { try { const health = await mcpApi.getHealth(); serverStatus.set({ status: health.status, workers: health.workers, uptime: health.uptime, version: health.version })} catch (error) { console.error('Failed to get server status:', error)}
