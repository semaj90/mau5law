...
import type { Document } from '$lib/types';
  ...
  function getConfidenceColor(conf: number): string {
    ...
    return (
      (conf >= 0.8 && (palette.colors?.success ?? palette.colors?.accent?.[0])) ?? (conf >= 0.6 && (palette.colors?.warning ?? palette.colors?.accent?.[1])) ?? (palette.colors?.error ?? palette.colors?.accent?.[2]) ?? '#888'
    )}
