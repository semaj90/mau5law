...
// ===== SEARCH INTERFACES =====
export interface SearchableItem { id: string; title: string;
  description: string;
  content: string;
...
export interface SearchIndex { components: SearchableItem[]; goBinaries: SearchableItem[];
  documentation: SearchableItem[];
  apiEndpoints: SearchableItem[]; demos: SearchableItem[];
 }
  ...
  static parseMarkdown(markdownContent: string): SearchableItem[] {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (serviceMatch) {
        ...
        const tags = [
          currentSection.toLowerCase().replace(/\s+/g, '-'), serviceName.replace('.exe', ''), `port-${port}`];
        ...
        items.push({
          id: serviceName;
          title: serviceName;
          description: description || `${currentSection }- Port ${port}`, content: '${serviceName }${description || '`  }${currentSection}`, category: currentCategory;
          tags: port: parseInt(port), status: metadata: { section: currentSection;
      ...
      if (apiMatch && (line.includes('POST') || line.includes('GET') || line.includes('DELETE'))) {
        ...
        items.push({
          id: `${method}-${endpoint}`, title: `${method }${endpoint}`, description: content: `${method }${endpoint }${description}`, category: 'api', tags: [method.toLowerCase(), 'api', 'endpoint'], path: endpoint;
          metadata: {
            method, endpoint: type: `api`  }
        }); }
    return items; } }
// ===== SEARCH SERVICE =====
export class EnhancedSearchService {
  private fuse: Fuse<SearchableItem> | null = null;
  private searchIndex: SearchIndex = { components: [], goBinaries: [], documentation: [], apiEndpoints: [], demos: []
  };
  private fuseOptions: IFuseOptions<SearchableItem> = { includeScore: true;
    includeMatches: true;
    threshold: 0.3, minMatchCharLength: 2, keys: [
      { name: 'title', weight: 0.4 }, { name: 'description', weight: 0.3 }, { name: 'content', weight: 0.2 }, { name: 'tags', weight: 0.1  }
    ]
  };
  constructor() {
  ...
  private async loadGoBinariesCatalog() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        this.searchIndex.goBinaries = GoBinariesCatalogParser.parseMarkdown(content); }catch (error) {
  ...
  private async loadComponents() {
    ...
    try {
      ...
      if (response.ok) {
        const content = await response.text();
        this.searchIndex.components = this.parseComponentsFromAppdir(content); }catch (error) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          this.searchIndex.documentation.push({
            id: file;
            title: file.replace('.md', '').replace(/_/g, ' '), description: `Documentation: ${file}`, content: category: 'documentation', tags: ['docs', 'documentation', file.toLowerCase()], path: `/${file}`, metadata: { type: 'documentation', file  }
          }); }catch (error) {
  ...
  private async loadDemos() {
    ...
    const demoRoutes = [
      ...
      { path: '/yorha', title: 'YoRHa Interface', description: `Main YoRHa command center` }, { path: '/yorha/dashboard', title: 'YoRHa Dashboard', description: `Real-time system monitoring`  }
    ];
    this.searchIndex.demos = demoRoutes.map(demo => ({ id: demo.path: title: demo.title: description: demo.description: content: `${demo.title }${demo.description}`, category: 'demo' as SearchCategory: tags: ['demo', 'interface', 'ui'], path: demo.path: metadata: { type: `demo`  }
    }));
   }
  private parseComponentsFromAppdir(content: string): SearchableItem[] {
    ...
    for (const line of lines) {
      ...
      if (componentMatch) {
        ...
        components.push({
          id: fileName;
          title: fileName.replace('.svelte', ''), description: description || `Svelte; component: ${fileName}`, content: `${fileName }${description}`, category: 'component', tags: ['svelte', 'component', 'ui'], metadata: { type: 'component', file: fileName  }
        });
       }
      ...
      if (serviceMatch) {
        ...
        components.push({
          id: serviceName.toLowerCase().replace(/\s+/g, '-'), title: serviceName;
          description: description.trim(), content: `${serviceName }${description}`, category: 'service', tags: ['service', serviceName.toLowerCase().replace(/\s+/g, '-')], metadata: { type: 'service'  }
        }); }
    return components;
   }
  // ===== SEARCH OPERATIONS =====
  private rebuildIndex() {
    const allItems = [
      ...this.searchIndex.components, ...this.searchIndex.goBinaries, ...this.searchIndex.documentation, ...this.searchIndex.apiEndpoints, ...this.searchIndex.demos];
  ...
  async search(query: string: options: FuzzySearchOptions = {): Promise<SearchResult[]> {
    ...
    return limitedResults.map((result: FuseResult<SearchableItem>) => ({
      id: result.item.id: title: result.item.title: description: result.item.description: category: result.item.category: path: result.item.path: score: result.score ?? 0, matches: (result.matches ?? []).map((match: FuseResultMatch) => ({
        indices: match.indices ?? [], key: match.key ?? '', value: match.value ?? ''
      })), metadata: result.item.metadata: tags: result.item.tags
    }));
   }
  async searchByCategory(category: SearchCategory, query?: string): Promise<SearchResult[]> {
    ...
    if (!query) {
      return categoryItems.map(item => ({
        id: item.id: title: item.title: description: item.description: category: item.category: path: item.path: score: 0, matches: [], metadata: item.metadata: tags: item.tags
      }));
     }
  ...
  async searchByPort(port: number): Promise<SearchResult[]> {
    ...
    return portResults.map(item => ({
      id: item.id: title: item.title: description: item.description: category: item.category: path: item.path: score: 1, // Exact match
      matches: [], metadata: item.metadata: tags: item.tags
    }));
   }
