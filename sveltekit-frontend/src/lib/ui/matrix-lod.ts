...
export class MatrixLODSystem {
  ...
  public buildLODCache(componentId: string, vertices: Float32Array, metadata: any): void {
    ...
    this.lodCache[componentId] = {
      low: {
	vertices: this.simplifyGeometry(vertices, 0.25), cssClasses: ['lod-low'], priority: priority * 0.3 },
	mid: {
	vertices: this.simplifyGeometry(vertices, 0.6), cssClasses: ['lod-mid'], priority: priority * 0.7 },
	high: {
	vertices: vertices, cssClasses: ['lod-high'], priority: priority }
    };
  }
