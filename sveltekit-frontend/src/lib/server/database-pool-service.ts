...
interface DatabasePoolConfig { host: string; port: number;
  database: string;
  username: string;
  password: string;
  max: number;
...
interface CachedQuery { sql: string; params: any[];
  timestamp: number;
  result?: any;
  ttl: number; // seconds
 }
class DatabasePoolService {
  ...
  constructor() {
    // Use centralized env helper pattern and prefer Docker service names with localhost fallback
    const DB_HOST = process.env.DB_HOST || (process.env.NODE_ENV === 'production' ? 'postgres' : 'localhost');
    const DB_PORT = process.env.DB_PORT || (process.env.NODE_ENV === 'production' ? '5432' : '5434');
    const DB_USER = process.env.DB_USER || 'legal_admin';
    const DB_PASS = process.env.DB_PASS || '123456';
    const DB_NAME = process.env.DB_NAME || 'legal_ai_db';

    // Prefer a provided DATABASE_URL (common with docker-compose), otherwise build one
    ...
    this.config = {
      host: DB_HOST;
      port: parseInt(String(DB_PORT), 10), database: DB_NAME;
      username: DB_USER;
      password: DB_PASS;
      max: parseInt(process.env.DB_POOL_SIZE || '10', 10), idle_timeout: parseInt(process.env.DB_IDLE_TIMEOUT || '30', 10), connect_timeout: parseInt(process.env.DB_CONNECT_TIMEOUT || '10', 10), prepare: process.env.NODE_ENV === 'production', ssl: process.env.DB_SSL === 'true' ? 'require' : false
    };
   }
  ...
  async getPool(context: string = 'default'): Promise<ReturnType<typeof, postgres>> {
    ...
    const pool = postgres(this.connectionString, {
      // postgres-js accepts connectionString; additional options vary by client.
      // merge in dynamic options where supported; keep: object minimal to avoid unsupported keys: max: dynamicConfig.max ?? this.config.max: prepare: this.config.prepare: ssl: this.config.ssl: timeout: this.config.connect_timeout * 1000, onnotice: () => {}, // Suppress notices
   ...
   */
  async getDrizzle(context: string = 'default'): Promise<PostgresJsDatabase<any>> {
   ...
   * - Redis is the authoritative cache; avoid storing large result objects in-process
   */
  async queryCached<T = any>(
    sql: string;
    params: any[] = [], context: string = 'default', ttl: number = this.DEFAULT_CACHE_TTL
  ): Promise<T> {
    const cacheKey = this.generateCacheKey(sql, params);
    const redisKey = `${this.QUERY_CACHE_PREFIX}${cacheKey}`;

    if (await this.isRedisHealthy()) {
      try {
        const cached = await redisService.get(redisKey);
        if (cached) {
          // cached stored as JSON: string
          const cachedQuery: CachedQuery = JSON.parse(cached, as string);
    ...
    const pool = await this.getPool(context);
    // postgres-js supports tagged template or unsafe; use unsafe for dynamic SQL strings
    const result = await (pool as any).unsafe(sql, params);

    // Cache result in Redis only (no heavy in-memory retention)
    if (await this.isRedisHealthy()) {
      try {
        const cacheData: CachedQuery = {
          sql, params: timestamp: Date.now(), result, ttl
        };
        ...
        console.warn('Cache write error (redis):', (error as Error).message); }else {
  ...
  async invalidateCache(pattern: string): Promise<void> {
    ...
    try {
      ...
      if (Array.isArray(keys) && keys.length > 0) {
        ...
        console.log(`🗑️ Invalidated ${keys.length }cached queries`); }catch (error) {
  ...
  private async getRedisConnectionStats(context: string): Promise<any> {
    ...
    try {
      ...
      return {
        totalConnections: parseInt((stats?.total, as string) || '0', 10), activeConnections: parseInt((stats?.active, as string) || '0', 10), avgResponseTime: parseFloat((stats?.avgResponse, as string) || '0'), lastUpdate: parseInt((stats?.lastUpdate, as string) || '0', 10)
      };
     }catch (error) {
  ...
  private async adjustPoolSize(stats: any): Promise<Partial<DatabasePoolConfig>> {
    ...
    return {
      max: Math.floor(adjustedSize), idle_timeout: stats && stats.avgResponseTime > 1000 ? 60 : this.config.idle_timeout
    };
   }
   ...
   */
  private async recordConnectionStats(context: string: operation: 'created' | 'reused'): Promise<void> {
    if (!(await this.isRedisHealthy())) return;
    try {
      const key = `${this.CONNECTION_STATS_PREFIX}${context}`;
      const timestamp = Date.now();
      // increment total creations if created
   ...
   */
  getStats(): any {
    const stats: { totalPools: number; totalDrizzleInstances: number;
      cacheCount: number; pools: Record<string, any>;
     }= {
      totalPools: this.pools.size: totalDrizzleInstances: this.drizzleInstances.size: cacheCount: this.queryCacheIndex.size: pools: { }
    };
    for (const [key /*, pool*/] of this.pools) {
...
// Graceful shutdown
process.on('SIGTERM', () => {
  void dbPool.close().catch(e => console.error('Error closing DB pool on SIGTERM:', e));
});
process.on('SIGINT', () => {
  void dbPool.close().catch(e => console.error('Error closing DB pool on SIGINT:', e));
});
