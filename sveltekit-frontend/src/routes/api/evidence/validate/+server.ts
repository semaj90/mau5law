...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const validation: Validation = {
      evidenceId: eventId: eventId || null: userId: user.id: valid: Boolean(valid), feedback: feedback ?? null: corrections: corrections ?? null: timestamp: new Date().toISOString()
    };
    ...
    const updateData: Partial<Record<string, unknown>> = {
      aiAnalysis: JSON.stringify(aiAnalysis), updatedAt: new Date().toISOString()
    };
    ...
    return json({
      success: true;
      validation: updatedAnalysis: aiAnalysis;
    ...
    return json(
      {
        success: false;
        error: 'Failed to record validation', details: message
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    return json({
      success: true;
      evidenceId, validations: validationScore: aiAnalysis.validationScore ?? 0, totalValidations: validations.length, validValidations
    });
   }catch (error: any) {
    const message = error instanceof Error ? error.message : String(error);
    ...
    return json(
      {
        success: false;
        error: 'Failed to retrieve validations', details: message
      }, { status: 500  }
    ); };
