  ...
  try {
    ...
    if (existing.length > 0) {
      ...
      [result] = await db
        .insert(canvasStates)
        .values({
          id: randomUUID(), caseId: name: `Canvas State ${new Date().toISOString()}`, canvasData: canvasState;
          version: 1, createdBy: null, // Set to actual user ID when available
        })
        .returning();
     }
    return json({
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    return json(
      {
        error: 'Failed to load canvas state', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
