...
export async function findDocumentsByPracticeAreaAndConfidence(
  ...
  return db
    ...
    .where(
      and(
        sql`${legalDocuments.metadata}->'classification'->'practiceArea' @> ${JSON.stringify([practiceArea])}`,
...
export async function getPaginatedDocuments(
  ...
  if (filters?.searchQuery) {
    conditions.push(
      or(
        like(legalDocuments.title, `%${filters.searchQuery}%`),
...
export async function getCachedSearch(queryHash: string): Promise<any> {
  const result = await db
    ...
    .where(
      and(
        eq(vectorSearchCache.queryHash, queryHash),
        gt(vectorSearchCache.expiresAt, new Date())
      )
    );
  if (result[0]) {
    // Update hit count
