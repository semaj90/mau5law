...
export interface DocumentProcessingOutput {
    ...
    summary?: string;
    entities?: Array<any>;
    embeddings?: { chunks: number; dimensions: number; }; // Fixed syntax
    processingTime: number;
    success: boolean;
}
...
export const legalAnalysisActor = fromPromise(async ({ input }: { input: LegalAnalysisInput }) => {
    ...
    try {
        ...
        throw new Error(`Legal analysis failed: ${error instanceof Error ? error.message : String(error)}`);
    }
});
...
export function createEmbeddingActor(input: EmbeddingActorInput): ActorRefFrom<typeof embeddingActor> { return createActor(embeddingActor, { input }); }
export function createDocumentProcessingActor(input: DocumentProcessingInput): ActorRefFrom<typeof documentProcessingActor> { return createActor(documentProcessingActor, { input }); }
export function createLegalAnalysisActor(input: LegalAnalysisInput): ActorRefFrom<typeof legalAnalysisActor> { return createActor(legalAnalysisActor, { input }); }
export function createRAGSearchActor(input: RAGSearchInput): ActorRefFrom<typeof ragSearchActor> { return createActor(ragSearchActor, { input }); }
...
export const workflowActor = fromPromise(async ({ input }: { input: WorkflowInput }): Promise<WorkflowOutput> => { // Fixed arrow function and added return type
    const startTime = Date.now();
            ...
            const promises = input.steps.map(async (step, index) => {
                ...
                try {
                    ...
                    const result = await new Promise((resolve, reject) => {
                        const subscription = actor.subscribe((snapshot) => { // Removed 'any' parameter
                    ...
                    success = false;
                    errors.push({ step: stepId, error: error instanceof Error ? error.message : String(error) });
                }
            });
...
export function createWorkflowActor(input: WorkflowInput): ActorRefFrom<typeof workflowActor> { return createActor(workflowActor, { input }); }

// ===== UTILITY FUNCTIONS =====
export async function runActor<T>(actor: ActorRefFrom<any>): Promise<T> {
    return new Promise((resolve, reject) => {
        const subscription = actor.subscribe({
            next: (snapshot) => { // Fixed syntax
                if (snapshot.status === 'done') {
                    subscription.unsubscribe();
                    resolve(snapshot.output as T);
                }
            },
            error: (error) => { // Fixed syntax
                subscription.unsubscribe();
                reject(error);
            }
        });
...
export function getActorOutput<T>(actor: ActorRefFrom<any>): T | undefined { const snapshot = actor.getSnapshot(); return snapshot.output as T}
export function isActorDone(actor: ActorRefFrom<any>): boolean { const snapshot = actor.getSnapshot(); return snapshot.status === 'done'} // Fixed syntax
export function hasActorError(actor: ActorRefFrom<any>): boolean { const snapshot = actor.getSnapshot(); return snapshot.status === 'error'} // Fixed syntax
