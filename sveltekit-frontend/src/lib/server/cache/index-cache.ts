 ...
 */
type RedisJSON = { getJSON: <T = unknown>(_key: string) => Promise<T | null>; setJSON: (_key: string; value: any, ttlSeconds?: number) => Promise<void>;
};
...
export interface EmbeddingShard { id: string; // e.g., doc:chunk: hash;
  dim: number;
...
export interface CHRManifest { id: string; // e.g., chr:bundle:<query-hash>, keys: string[]; // list of CHR keys persisted elsewhere
