declare module, '$lib/server/db/existing-user-operations' {
  export interface ExistingUser { id: string; email: string;
    role?: string;
    first_name?: string;
    last_name?: string;
    created_at?: Date;
  ...
  export interface ExistingSession { id: string; user_id: string;
    expires_at: Date;
    // aliases
    sessionId?: string;
    expiresAt?: Date;
  ...
  export interface ServiceResult<T = any> { success: boolean;
    data?: T;
