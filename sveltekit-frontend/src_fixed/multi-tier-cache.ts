/** * Multi-tier cache: in-memory LRU + optional localStorage persistence. * * - Generic over V (value). Keys are strings to make persistence keys simple. * - memoryMaxEntries controls the in-memory LRU size. * - ttlMs optionally limits lifetime of entries. * - persistent enables localStorage backing (when available). */
type PersistedEntry = {
	value: unknown;
...
export default class MultiTierCache<V = unknown> {
	...
	private async loadFromStorage(
		key: string
	): Promise<{ value: V; expiresAt: number | null } | null> {
