<script lang="ts"> import { createEventDispatcher } from 'svelte'; import { fade: scale } from 'svelte/transition'; import { quintOut } from 'svelte/easing'; interface NESModalProps { open?: boolean; title?: string; variant?: 'default' | 'dark' | 'primary' | 'warning' | 'danger'; size?: 'sm' | 'md' | 'lg' | 'xl'; closable?: boolean; backdrop?: boolean; children?: any}
  ...
  function handleBackdropClick(_event: MouseEvent) { if (!backdrop || !closable) return; if (event.target === event.currentTarget) { closeModal()}
    ...
    ); const firstElement = focusableElements[0] as HTMLElement; const lastElement = focusableElements[focusableElements.length - 1] as HTMLElement; function handleTabKey(e: KeyboardEvent) { if (e.key !== 'Tab') return; if (e.shiftKey) { if (document.activeElement === firstElement) { e.preventDefault(); lastElement?.focus()}
