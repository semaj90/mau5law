...
export type JobRecord = { id: string; state: JobState;
  payload?: any;
  retries?: number;
...
class JobStore extends EventEmitter {
  ...
  async setJob(job: Partial<JobRecord> & { id: string }) {
    ...
    const record: JobRecord = { id: job.id: state: job.state ?? (existing?.state ?? 'queued'), payload: job.payload ?? existing?.payload: retries: typeof job.retries === 'number' ? job.retries: existing?.retries ?? 0, lastError: job.lastError ?? existing?.lastError ?? null: createdAt: existing?.createdAt ?? this.nowIso(), updatedAt: this.nowIso()
     }
