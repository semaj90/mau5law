...
export const caseManagementMachine = createMachine<CaseManagementContext, CaseManagementEvent>({
  ...
  states: { idle: { on: { LOAD_CASE: 'loadingCase',
        ...
        SET_PAGE: { target: 'loadingUserCases',
          actions: assign({ pagination: ({ context, event }) => ({
              ...context.pagination,
              page: event.page
            })
          })
...
}).provide({ actors: { loadCase: fromPromise(caseManagementServices.loadCase),
