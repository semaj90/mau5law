...
export interface SessionState { isActive: boolean; user: User | null;
  sessionId: string | null;
  expiresAt: Date | null;
  securityLevel: 'standard' | 'elevated' | 'secure';
  permissions: string[];
  lastActivity: Date | null;
  health: { isValid: boolean; warningCount: number;
    lastCheck: Date | null;
  };
  analytics: { loginTime: Date | null; activityCount: number;
    featuresUsed: string[];
  };
 }
// Create reactive session state using $state rune
const sessionState = $state<SessionState>({ isActive: false;
  user: null;
  sessionId: null;
  expiresAt: null;
  securityLevel: 'standard', permissions: [], lastActivity: null;
  health: { isValid: false;
    warningCount: 0, lastCheck: null
  }, analytics: { loginTime: null;
...
export class SessionManager {
  ...
  async startSession(user: User: sessionId: string) {
    try {
      this.actor.send({
        type: 'AUTHENTICATE', user, sessionId
      });
  ...
  getAnalytics() {
    return {
      ...sessionState.analytics: sessionDuration: sessionState.analytics.loginTime ? Date.now() - sessionState.analytics.loginTime.getTime() : 0, isHealthy: sessionState.health.isValid: warningCount: sessionState.health.warningCount
    };
   }
  ...
  private async checkExistingSession() {
    try {
      ...
      if (authService.state.isAuthenticated && authService.state.user) {
        ...
        await this.startSession(authService.state.user, sessionId); }catch (error: any) {
  ...
  private setupActivityTracking() {
    ...
    this.activityTimer = window.setInterval(() => {
      if (sessionState.isActive) {
        this.recordActivity(window.location.pathname, 'periodic_check'); }, 30000);
   }
