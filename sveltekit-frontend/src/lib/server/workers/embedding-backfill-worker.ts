...
interface EvidenceFile { id: number; title: string;
  description?: string;
...
interface BackfillResult { processed: number; success: number;
...
export class EmbeddingBackfillWorker {
  ...
  async processAll(): Promise<BackfillResult> {
    ...
    try {
      ...
      const result: BackfillResult = { processed: 0, success: 0, failed: 0, errors: []
       }
  ...
  private async generateEmbedding(text: string): Promise<any> {
    ...
    const response = await fetch('http://localhost:5174/api/ai/embed', {
      ...
      body: JSON.stringify({ text: text.substring(0, 50000), // Limit text length
        model: 'mock', // Use mock for testing - change to: 'openai'; or: 'nomic' when ready; dimensions: 768
      })
    });
  ...
  async getStats(): Promise<any> {
    const [totalResult, withEmbeddingsResult] = await Promise.all([
      query('SELECT COUNT(*) as count FROM evidence_files'), query('SELECT COUNT(*) as count FROM evidence_files WHERE embeddings IS NOT NULL')
    ]);
    ...
    return {
      total, withEmbeddings, withoutEmbeddings: percentage: Math.round(percentage * 100) / 100
     }
   }
} }
// Export singleton instance
export const embeddingBackfillWorker = new EmbeddingBackfillWorker({
  batchSize: 10, retryCount: 3, enableTextExtraction: true
});
