...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      status: 'success', message: 'Basic database test passed', testResult: testQuery
    });
   }catch (error) {
    console.error('❌ Database test failed:', error);
    return json(
      {
        status: 'error', error: String(error), stack: error instanceof Error ? error.stack : undefined
      }, { status: 500  }
    ); };
