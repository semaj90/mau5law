// Clustering utility using Ollama API (gemma3-legal:latest) for embeddings
...
export function kMeans(vectors: number[][], k: number, maxIter = 100): { centroids: number[][], labels: number[] } {
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    ...
    // Update centroids
    const newCentroids = Array.from({ length: k }, () => Array(vectors[0].length).fill(0));
    const counts = Array(k).fill(0);
    vectors.forEach((v, i) => {
      const label = labels[i];
      counts[label]++;
      v.forEach((val, j) => { newCentroids[label][j] += val; });
    });
    for (let i = 0; i < k; i++) {
