/**
 * 🚀 UNIFIED EVIDENCE API v2 - Production Ready
 *
 * Consolidates all evidence-related endpoints into a single, powerful API
 * Integrates Python FastAPI AI backend (localhost:8000) with TypeScript services
 *
 * ENDPOINTS:
 * - GET    /api/v2/evidence          - List/search evidence with filters
 * - POST   /api/v2/evidence          - Create new evidence
 * - PUT    /api/v2/evidence          - Update evidence
...
class TypeScriptEvidenceService {
  async listEvidence(filters: ListFilters = {}) {
    ...
    if (evidenceType) whereClauses.push(eq(evidence.evidenceType, evidenceType));
    if (search) whereClauses.push(ilike(evidence.title, `%${search}%`));
...
/**
 * GET /api/v2/evidence
 * List and search evidence with intelligent fallback
 */
 ...
 * Create evidence OR upload file to Python AI backend
 */
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    const contentType = request.headers.get('content-type') || '';

    // Handle multipart/form-data (file upload to Python AI)
    if (contentType.includes('multipart/form-data')) {
      ...
      const userId = locals.user?.id || formData.get('user_id')?.toString() || 'anonymous';
