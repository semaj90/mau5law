...
export class ConcurrentIndexedDBSearch {
 ...
 const workerBlob = new Blob(['(' + workerFn.toString() + ')()'], {
 ...
 case 'indexUpdated':
 console.log(
 `✅ Worker ${msg.workerId} indexed ${msg.data?.documentsIndexed ?? 0} documents`
 );
 break;
 case 'cacheCleared':
 ...
 try {
 worker.postMessage({ type: 'search', data: { query, documents, options }, workerId });
 } catch (err) {
 clearTimeout(timeout);
 worker.removeEventListener('message', messageHandler);
 reject(err);
