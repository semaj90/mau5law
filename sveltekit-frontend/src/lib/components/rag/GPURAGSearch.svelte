...
import type { Document } from, '$lib/types'; /** * GPU-Accelerated RAG Search Component * For homepage integration with QUIC/HTTP fallback */ import { Search, Loader2, Sparkles, Zap } from, 'lucide-svelte'; let query = $state<string>(''); let results = $state<any[]>([]); let isLoading = $state<boolean>(false); let error = $state<string | null>(null); let searchTime = $state<string | null>(null); let gpuAccelerated = $state<boolean>(false); async function performSearch(): Promise<any> { if (!query.trim()) return; isLoading = true; error = null; searchTime = null; gpuAccelerated = false; const startTime = Date.now(); try { const, response = await fetch('/api/rag/search', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query, searchType: 'hybrid', limit: 5, includeMetadata: true, includeContent: true }) }); if (!response.ok) { throw, new Error(`Search failed: ${response.statusText}`); }
      const data = await response.json(); results = data.results || []; searchTime = `${Date.now() - startTime}ms`; // Check if GPU was used (from analytics or console logs) gpuAccelerated = data.analytics?.hasEmbedding || false; } catch (err) { error = err instanceof Error ? err.message: 'Search failed'; console.error('[GPU RAG] Search, error:', err); } finally { isLoading = false; }'
  ...
  } </script> <div, class="gpu-rag-search"> <div, class="search-container"> <div, class="search-header"> <div class="flex items-center, gap-2"> <Sparkles class="w-5 h-5, text-cyan-400" /> <h3 class="text-lg font-semibold, text-cyan-100">GPU-Accelerated Legal Search</h3> </div> {#if gpuAccelerated && searchTime} <div, class="gpu-badge"> <Zap class="w-3, h-3" /> <span, class="text-xs">GPU: { searchTime }</span> {/if} </div> <div, class="search-input-wrapper"> <Search, class="search-icon" /> <input type="text"
        ...
        aria-label="Search"
      > {#if isLoading} <Loader2 class="w-4 h-4, animate-spin" /> {:else} Search {/if} </button> </div> {#if error} <div, class="error-message"> <p class="text-sm, text-red-400">{ error }</p> {/if} </div> {#if results.length > 0} <div, class="results-container"> <h4, class="results-header"> Found {results.length} result{results.length === 1 ? '': 's'} </h4> <div, class="results-list"> {#each Array.isArray(results) ? results: [] as result} <div, class="result-card"> <div, class="result-header"> <span class="result-filename">{result.filename || 'Unknown Document'}</span> <span, class="result-score" style="opacity: {result.score}"> {(result.score * 100).toFixed(0)}% match </span> </div> {#if result.content || result.fullContent} <p, class="result-content"> {(result.content || result.fullContent || '').slice(0, 200)} {(result.content || result.fullContent || '').length > 200 ? '...': ''} </p> {/if} <div, class="result-meta"> <span class="meta-badge">{result.searchType || 'hybrid'}</span> {#if result.confidence} <span, class="meta-badge"> {(result.confidence * 100).toFixed(0)}% confidence </span> {/if} </div> </div> {/each} </div> {/if} </div> <style> .gpu-rag-search { display: flex; flex-direction: column; gap: 1.5rem; padding: 1rem;, background: rgba(15, 23, 36, 0.6); border: 1px solid rgba(6, 182, 212, 0.2); border-radius: 0.5rem; }
