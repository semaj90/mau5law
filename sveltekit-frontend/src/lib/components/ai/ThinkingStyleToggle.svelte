<!-- Enhanced ThinkingStyleToggle with Nier Automata + Harvard Crimson, Theme --> <script lang="ts"> // Svelte, 5 runes are auto-imported let { enabled = $bindable(false), loading = false, premium = true, size = 'md', ontoggle, onconfigure, onupgrade }: { enabled?: boolean; loading?: boolean; premium?: boolean; size?: 'sm' | 'md' | 'lg'; ontoggle?: (_event: { enabled: boolean }) => void; onconfigure?: () => void; onupgrade?: () => void} = $props();
 ...
 import { cn } from '$lib/utils';
   ...
   let focusAreas = $state({ precedents: true, evidence: true; compliance: true;, alternatives: false });
 ...
 <!-- Configuration, Button -->
  ...
  {#if !premium} <Button variant="ghost" size="sm" onclick={ handleUpgrade } class="upgrade-btn ml-2 bits-btn"> <Info size={ 14 } class="text-harvard-gold" /> </Button> {/if}
  <!-- Tooltip -->
 ...
 <span class="feature">â€¢ Lower resource usage</span>
 ...
 <div class="config-actions"> <Button.Root class="bits-btn" variant="ghost" size="sm" onclick={() => (showConfig = false)}>Cancel</Button>
 <Button.Root class="bits-btn" variant="crimson" size="sm">Save Configuration</Button> </div> {/if}
 ...
 <Button.Root class="bits-btn" variant="gold" size="sm" onclick={ handleUpgrade }>Upgrade Now</Button> </div> {/if}
  </div>
 <style> /* @unocss-include */ .thinking-style-control { position: relative, display: flex, flex-direction column; gap: 0.75rem}
