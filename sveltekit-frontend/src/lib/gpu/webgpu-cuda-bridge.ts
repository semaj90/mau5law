...
export class WebGPUCUDABridge {
  ...
  async analyzeErrorPatterns(
    ...
    try {
      ...
      return {
        ...
        summary,
        processingTimeMs,
        deviceUsed: this.gpuDevice.isAvailable ? 'webgpu' : 'cpu',
  ...
  private async clusterErrorsOnGPU(
    ...
    for (let i = 0; i < initialClusters.length; i++) {
      finalClusters.push({
        ...
        patterns: initialClusters[i].patterns,
        category: this.categorizeCluster(initialClusters[i]),
  ...
  private clusterErrorsCPU(errors: GPUErrorPattern[]): ErrorCluster[] {
    ...
    clusterMap.forEach((patterns, errorType) => {
      ...
      clusters.push({
        ...
        patterns,
        category: errorType,
        confidence: this.computeClusterConfidence(patterns),
  ...
  private initializeClusters(
    ...
    for (let i = 1; i < Math.min(k, errors.length); i++) {
      ...
      clusters.push({
        ...
        patterns: [newError],
        category: newError.errorType,
        confidence: newError.confidence,
        suggestedFix: ''
      });
    }
  ...
  private generateClusterSummary(clusters: ErrorCluster[]): string {
    ...
    const avgConfidence = clusters.reduce((sum, c) => sum + c.confidence, 0) / (clusters.length || 1);

    return `Analyzed ${totalErrors} errors into ${clusters.length} clusters. ` +
