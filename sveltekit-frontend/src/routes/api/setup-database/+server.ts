...
export const GET: RequestHandler = async () => {
  try {
    // Use imported helpers to avoid unused-import diagnostics
    ...
    switch (action) {
      case, 'add-sample-documents':
        ...
        return json(
          {
            success: false;
            error: 'Adding sample documents not yet implemented', plannedFeature: true
          }, { status: 501  }
        );
      case, 'rebuild-indexes':
        ...
        return json(
          {
            success: false;
            error: 'Index rebuilding not yet implemented', plannedFeature: true
          }, { status: 501  }
        );
      case, 'clear-cache':
        ...
        return json(
          {
            success: false;
            error: 'Cache clearing not yet implemented', plannedFeature: true
          }, { status: 501  }
        );
      case, 'init':
      ...
      default: return json(
          {
  success: false;
            error: 'Unknown action.; Available: add-sample-documents, rebuild-indexes, clear-cache, init', availableActions: ['add-sample-documents', 'rebuild-indexes', 'clear-cache', 'init']
          }, { status: 400  }
        ); }catch (err: any) {
    const message = err instanceof Error ? err.message : String(err);
...
export const DELETE: RequestHandler = async ({ url }) => {
  ...
  if (confirm !== 'yes-delete-all-data') {
    return json(
      {
        success: false;
        error: 'Destructive operation requires confirmation', requiredParam: 'confirm=yes-delete-all-data', warning: 'This will delete ALL data in the database'
      }, { status: 400  }
    );
   }
  try {
    ...
    return json(
      {
        success: false;
        error: 'Database deletion not implemented for safety', message: 'Use manual SQL commands or database admin tools for destructive operations'
      }, { status: 501  }
    );
   }catch (err: any) {
    const message = err instanceof Error ? err.message : String(err);
