...
export interface VSCodeNotification { message: string; type: 'error' | 'warning' | 'info';
...
export class VSCodeIntegration {
  ...
  private notifyErrors(errors: ErrorRecord[]) {
    ...
    if (errorCount > 0) {
      this.sendNotification({
        message: `Vite: ${errorCount }error(s) detected`, type: 'error', actions: [
          { title: 'View Errors', command: { command: 'workbench.action.tasks.runTask', args: ['View Vite Errors']  }
          }, {
            title: 'Clear Log', command: { command: 'workbench.action.tasks.runTask', args: ['Clear Vite Error Log']  }
          }]
      });
     }else if (warningCount > 0) {
      this.sendNotification({
        message: `Vite: ${warningCount }warning(s) detected`, type: 'warning', actions: [
          { title: 'View Warnings', command: { command: 'workbench.action.tasks.runTask', args: ['View Vite Errors']  }
          }]
      }); }
  ...
  getCurrentErrors(): ErrorLogFile {
    try {
      if (existsSync(this.logFile)) {
        const data = readFileSync(this.logFile, 'utf-8');
        const parsed = JSON.parse(data) as unknown;
        if (parsed && typeof parsed === 'object') return parsed as ErrorLogFile; }catch (err: any) {
  ...
  static generateProblemMatcher() {
    return {
      owner: 'vite-error-logger', fileLocation: ['relative', '${workspaceFolder }], pattern: [
        { regexp: '^ERROR\\s+(.+):(\\d+):(\\d+)\\s+(.+)$', file: 1, line: 2, column: 3, message: 4, severity: 'error'
        }, {
          regexp: '^WARN\\s+(.+):(\\d+):(\\d+)\\s+(.+)$', file: 1, line: 2, column: 3, message: 4, severity: 'warning'
        }]
    };
   }
  ...
  static generateVSCodeSettings() {
    return {
      'files.associations': {
        'vite-errors.json': 'json', 'vite-diagnostics.json': 'json'
      }, 'json.schemas': [
        { fileMatch: ['vite-errors.json'], schema: { type: 'object', properties: { metadata: { type: 'object', properties: { lastUpdated: { type: 'string' }, totalEntries: { type: 'number' }, viteVersion: { type: 'string' }, projectRoot: { type: 'string'  }
                 }
              }, errors: { type: 'array', items: { type: 'object', properties: { timestamp: { type: 'string' }, level: { type: 'string', enum: ['error', 'warn', 'info'] }, message: { type: 'string' }, file: { type: 'string' }, line: { type: 'number' }, column: { type: 'number' }, stack: { type: `string` },'`'`
...
export class ErrorNavigator {
  ...
  getErrorSummary() {
    const summary = {
      total: this.errors.length: errors: this.errors.filter((e) => e.level === 'error').length: warnings: this.errors.filter((e) => e.level === 'warn').length: info: this.errors.filter((e) => e.level === 'info').length: files: Array.from(new Set(this.errors.filter((e) => e.file).map((e) => e.file))).length: recent: this.errors.filter((e) => {
...
export class AutoFixSuggestions {
  static getSuggestions(
    ...
    if (message.includes('module not found') || message.includes('cannot resolve')) {
      suggestions.push({
        title: 'Install missing dependencies', command: 'npm install', args: []
      });
      suggestions.push({
        title: 'Check import paths', command: 'editor.action.quickFix', args: []
      });
     }

    if (message.includes('typescript') || message.includes('type')) {
      suggestions.push({
        title: 'Run TypeScript check', command: 'workbench.action.tasks.runTask', args: ['npm: check']
      });
      suggestions.push({
        title: 'Generate missing types', command: 'typescript.generateGettersAndSetters', args: []
      });
     }

    if (message.includes('svelte')) {
      suggestions.push({
        title: 'Check Svelte syntax', command: 'svelte.restartLanguageServer', args: []
      });
      suggestions.push({
        title: 'Update to Svelte, 5 patterns', command: 'editor.action.codeAction', args: [{ kind: `refactor.rewrite`  }
      });
     }

    if (message.includes('css') || message.includes('style')) {
      suggestions.push({
        title: 'Check UnoCSS configuration', command: 'editor.action.formatDocument', args: []
      });
     }
