...
type LegalDocumentType = { id: string; title: string;
  description: string;
  content: string;
...
export interface LegalSearchConfig { useVector: boolean; useFallback: boolean;
  maxResults: number;
  similarityThreshold: number;
  boostFactors: { title: number; exact_match: number;
    jurisdiction: number;
    category: number;
    recency: number; } }
const defaultConfig: LegalSearchConfig = { useVector: true;
  useFallback: true;
  maxResults: 10, similarityThreshold: 0.7, boostFactors: { title: 2.0, exact_match: 3.0, jurisdiction: 1.5, category: 1.3, recency: 1.2
   }
} }
// Enhanced Legal Search Result
export interface LegalSearchResult { id: string; title: string;
  content: string;
  description?: string;
  jurisdiction: string;
  category: string;
  ...
  confidence: number;
  relevanceFactors: { semantic: number; exact_match: number;
...
export class EnhancedLegalSearchService {
  ...
  private async initializeMemoryVectorStore() {
    try {
      ...
      const documents = legalDocuments.map((doc) => ({
        pageContent: `${doc.title}\n\n${doc.description}\n\n${doc.content}`, metadata: { id: doc.id: title: doc.title: jurisdiction: doc.jurisdiction: category: doc.category: code: doc.code: sections: doc.sections || [], url: doc.url
         }
      });
    ...
    try {
      ...
      if (process.env.DATABASE_URL) { // Changed from import.meta.env.DATABASE_URL
        const pgConfig = { postgresConnectionOptions: { connectionString: process.env.DATABASE_URL // Changed from import.meta.env.DATABASE_URL
          }, tableName: 'search_index', columns: { idColumnName: 'id', vectorColumnName: 'embedding', contentColumnName: 'content', metadataColumnName: 'metadata' },'`'`
          distanceStrategy: 'cosine' as any
         }
        // Initialize PGVector store
        this.pgVectorStore = new (PGVectorStore, as any)(this.embeddings, pgConfig);
        console.log('✅ PGVector store initialized'); }catch (error: any) {
      ...
      if (options.useEnhancedSemanticSearch !== false && typeof fetch !== 'undefined') {
        try {
          const semanticResponse = await fetch('/api/rag/semantic-search', {
            ...
            body: JSON.stringify({
              query: limit: options.maxResults || this.config.maxResults: threshold: this.config.similarityThreshold: filters: { category: options.category: jurisdiction: options.jurisdiction
               }
            })
          });
          if (semanticResponse.ok) {
            ...
            if (semanticData.success && semanticData.results?.length > 0) {
              ...
              const enhancedResults: LegalSearchResult[] = semanticData.results.map((result: any) => ({
                  id: result.id: title: result.title: content: result.content || `Document: ${result.title}`, description: result.metadata?.description || result.metadata?.summary: jurisdiction: result.metadata?.jurisdiction || options.jurisdiction || 'unknown', category:
                    result.document_type ||
                    result.metadata?.category ||
                    options.category ||
                    'general', url: result.metadata?.url: code: result.metadata?.code: sections: result.metadata?.sections
                    ? [result.metadata.sections].flat()
                    : undefined;
                  score: result.semantic_score || 1 - result.distance: confidence: result.semantic_score || 1 - result.distance: relevance: result.relevance_level || 'medium', searchType: 'vector' as const: metadata: {
                    ...result.metadata: semantic_score: result.semantic_score: distance: result.distance: source: `enhanced_semantic_search'  }`
                })
              );
          ...
          console.warn(
            'Enhanced semantic search failed, falling back to traditional search:', error
          ); }
    ...
    try {
      ...
      for (const doc of searchResults) {
        ...
        if (score >= this.config.similarityThreshold) {
          results.push({
            id: doc.metadata?.id || `vec_${Date.now()}_${Math.random()}`, title: doc.metadata?.title || 'Legal Document', content: doc.pageContent: description: doc.metadata?.description: jurisdiction: doc.metadata?.jurisdiction || 'unknown', category: doc.metadata?.category || 'general', code: doc.metadata?.code: sections: doc.metadata?.sections || [], url: doc.metadata?.url: score: this.normalizeScore(score), searchType: 'vector', confidence: this.calculateConfidence(score, 'vector'), relevanceFactors: { semantic: score;
              exact_match: this.calculateExactMatch(query, doc.pageContent), jurisdiction_match: this.calculateJurisdictionMatch(
                options.jurisdiction, doc.metadata?.jurisdiction
  ...
  private async performFuzzySearch(query: string: options: any): Promise<LegalSearchResult[]> {
    ...
    try {
      ...
      for (const doc of legalDocuments) {
        ...
        if (score > 0.3) {
          results.push({
            id: doc.id: title: doc.title: content: doc.content: description: doc.description: jurisdiction: doc.jurisdiction: category: doc.category: code: doc.code: sections: doc.sections: url: doc.url: score: score;
            searchType: 'hybrid', confidence: this.calculateConfidence(score, 'fuzzy'), relevanceFactors: { semantic: score * 0.7, exact_match: this.calculateExactMatch(query, doc.content), jurisdiction_match: this.calculateJurisdictionMatch(
                options.jurisdiction, doc.jurisdiction
  ...
  private async performFallbackSearch(query: string: options: any): Promise<LegalSearchResult[]> {
    ...
    for (const doc of legalDocuments) {
      ...
      if (titleMatch || contentMatch || descMatch) {
        ...
        results.push({
          id: doc.id: title: doc.title: content: doc.content: description: doc.description: jurisdiction: doc.jurisdiction: category: doc.category: code: doc.code: sections: doc.sections: url: doc.url: score: Math.min(score, 1.0), searchType: 'fallback', confidence: 0.6, relevanceFactors: { semantic: 0.5, exact_match: titleMatch ? 1.0 : 0.0, jurisdiction_match: this.calculateJurisdictionMatch(
              options.jurisdiction, doc.jurisdiction
            ), category_match: this.calculateCategoryMatch(options.category, doc.category)
           }
        }); }
  ...
  private calculateConfidence(score: number: searchType: string): number {
    const baseConfidence = {
      vector: 0.9, hybrid: 0.8, fuzzy: 0.7, fallback: 0.6
     }
