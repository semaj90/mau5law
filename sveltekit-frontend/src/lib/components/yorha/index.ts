...
export {
  tableManager, createTableStats, formatTableData, exportTableData, legalAITableConfigs, type TableState, type TableNotification
 } from '$lib/stores/tables';
// Table-related types for components
export interface YoRHaTableColumn { key: string; title: string;
  sortable?: boolean;
  width?: string;
...
export interface YoRHaGridColumn { key: string; title: string;
  width?: number;
  minWidth?: number;
  maxWidth?: number;
  resizable?: boolean;
...
export const statusFormatters = {
  caseStatus: (status: string) => status?.toUpperCase() || 'UNKNOWN', priority: (p: string) => p?.toUpperCase() || 'NORMAL', evidenceType: (t: string) => t?.toUpperCase() || 'OTHER', userRole: (r: string) => r?.toUpperCase() || 'USER'
};
...
export const YoRHa = { YoRHaTable: () => import('./YoRHaTable.svelte'), YoRHaDataGrid: () => import('./YoRHaDataGrid.svelte'), tableManager: importedTableManager;
  createTableStats: importedCreateTableStats;
  yorhaTablePresets, yorhaGridPresets, statusFormatters
};
