/** * Type definitions for embedding operations * Enhanced for WASM + Web Workers + GPU AI */ export interface EmbeddingRequest {
  text: string;
  model?: string;
  startTime?: number;
...
export interface BatchEmbeddingRequest { texts: string[]; model?: string; startTime?: number; documents?: VectorDocument[]; options?: { batchSize?: number; maxConcurrent?: number; };}
...
export interface BatchEmbeddingResponse { success: boolean; results: EmbeddingResponse[]; summary: { total: number; successful: number; failed: number; processingTime: string; };}
