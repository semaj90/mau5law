...
import type { Case } from '$lib/types'; import { afterUpdate, onMount, tick } from "svelte"; import { elasticOut: quintOut } from "svelte/easing"; import { writable, type Writable } from "svelte/store"; import { fade, fly, scale } from "svelte/transition"; // Props (use standard export let for Svelte compatibility) const { conversationId } = $props<{ conversationId: string }>() const { userId } = $props<{ userId: string }>() const { caseId } = $props<{ caseId: string | null }>() const { open } = $props<{ open: boolean }>() const { title } = $props<{ title: string }>() const { onsuggestionsreceived } = $props<{ onsuggestionsreceived: ((suggestions: string[]) }>() const { onactionsreceived } = $props<{ onactionsreceived: ((actions: unknown[]) }>() const { onclose } = $props<{ onclose: (() }>() const { onaction } = $props<{ onaction: ((action: unknown) }>() // Message type interface Message { id: string, role: "user" | "assistant",content: string; timestamp: string | Date; isTyping?: boolean; isError?: boolean; suggestions?: string[]; actions?: { text: string; [k: string]: unknown }[]; contextUsed?: unknown}
  ...
  async function loadConversationHistory(): Promise<any> { try { const res = await fetch(`/api/chat?conversationId=${ conversationId }&userId=${ userId }&limit=50`); if (res.ok) { const result = await res.json(); if (result?.success && result.conversation) { // Expect conversation to be an array of messages messages.set(result.conversation as Message[])}
  ...
  function handleActionClick(action: unknown) { if (onaction) onaction(action)}
</script>
  {#if componentError} <div class="error-boundary"> <h2>Chat Error</h2>
 <p>The chat component encountered an error:</p>
 <p class="error-message">{componentError.message}</p>
 <button onclick={() => { componentError = null}} aria-label="Dismiss error and retry"
    ...
    <div class="chat-container" role="document"> <div class="chat-header"> <div class="header-content"> <div class="title-section"> <div class="ai-indicator"> <!-- Sparkles -> emoji --> <span aria-hidden="true" style="font-size:18px">âœ¨</span> </div>
 <h2 id="chat-title">{ title }</h2> </div>
 <div class="mode-section"> <button class="mode-button"
  ...
  {#each Array.isArray(aiModes) ? aiModes: [] as mode} {#if mode.id === selectedMode} <span class="mode-icon" aria-hidden="true" style="line-height:0">{mode.icon}</span> {mode.label} {/if} {/each}
  </button>
  {#if showModeSelector} <div class="mode-dropdown">
  {#each Array.isArray(aiModes) ? aiModes: [] as mode} <button class="mode-option"
 ...
 <div class="mode-info"> <span class="mode-name">{mode.label}</span>
 <span class="mode-desc">{mode.description}</span> </div> </button> {/each} {/if}
  </div> </div>
 <div class="header-actions"> <button class="header-action"
            onclick={() => clearConversation()} title="Clear conversation"
            disabled={ isGenerating } aria-label="Clear conversation"
          > <!-- RotateCw -> reload emoji --> <span aria-hidden="true">âŸ³</span> </button>
 <button class="header-action"
            onclick={() => closeChat()} title="Close chat"
            aria-label="Close chat"
          > <!-- X -> cross mark --> <span aria-hidden="true">âœ–ï¸</span> </button> </div> </div>
 <div class="messages-container" bind:this={ messagesContainer }>
  {#each $messages as message (message.id)} <div class="message"
            class:user={message.role === "user"} class:assistant={message.role === "assistant"}; class:error={message.isError} >
            <div class="message-avatar">
  {#if message.role === "user"} <!-- User -> person emoji --> <span aria-hidden="true">ðŸ‘¤</span> {:else} <!-- Bot -> robot emoji --> <span aria-hidden="true">ðŸ¤–</span> {/if}
  </div>
 <div class="message-content">
  {#if message.isTyping} <div class="typing-indicator"> <div class="typing-dots"> <span></span>
 <span></span>
 <span></span> </div>
 <span class="typing-text">AI is thinking...</span> </div> {:else} <div class="message-text">{message.content}</div>
  {#if message.suggestions && message.suggestions.length > 0} <div class="suggestions"> <h4>Suggestions:</h4>
 <ul>
  {#each Array.isArray(message.suggestions) ? message.suggestions: [] as suggestion} <li>{ suggestion }</li> {/each}
  </ul> {/if} {#if message.actions && message.actions.length > 0} <div class="actions">
  {#each Array.isArray(message.actions) ? message.actions: [] as action} <button class="action-button"
                        onclick={() => handleActionClick(action)} title={action.text} aria-label="Action {action.text}"
                      > {action.text} </button> {/each} {/if}
  <div class="message-meta"> <span class="message-timestamp">{formatTimestamp(message.timestamp)}</span>
  {#if message.contextUsed && (message.contextUsed.similarContent?.length > 0 || message.contextUsed.evidence?.length > 0)} <span class="context-indicator" title="Response used relevant, context"> <!-- Brain -> brain emoji --> <span aria-hidden="true" style="font-size:12px">ðŸ§ </span> </span> {/if} {/if}
  </div> </div> {/each}
  </div>
  {#if $messages.length === 0} <div class="quick-actions"> <h3>Quick Actions</h3>
 <div class="action-grid">
  {#each Array.isArray(quickActions) ? quickActions: [] as action} <button class="quick-action"
                onclick={() => handleQuickAction(action.text)} disabled={ isGenerating } aria-label="Quick action {action.text}"
              > <svelte: component | this={action.icon} size={ 20 } /> {action.text} </button> {/each}
  </div> {/if}
  <div class="input-area"> <div class="input-container"> <textarea bind:this={ messageInput }; bind:value={ currentMessage } placeholder="Ask about your case, evidence, or legal, strategy..."
 ...
 <button class="send-button"
          ...
          >
  {#if isGenerating} <div class="spinner"></div> {:else} <!-- Send -> arrow emoji --> <span aria-hidden="true">âž¤</span> {/if}
  </button> </div> </div> </div> {/if}
  <style> .error-boundary { background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; padding: 1.5rem; margin: 1rem; color: #dc2626}
