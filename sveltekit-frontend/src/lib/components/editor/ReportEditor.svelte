<!-- @migration-task Error while migrating Svelte code: Unexpected | token,https: //svelte.dev/e/js_parse_error --> <!-- @migration-task Error while migrating Svelte, code: Unexpected, token --> <!-- @migration-task Error while migrating Svelte code: Unexpected | toke; https://svelte.dev/e/js_parse_error --> <script lang="ts"> import { Button } from '$lib/components/ui/button'; // Removed unused imports: onDestroy | quintOut, Modal, Component import { onMount } from 'svelte';
 ...
 import { legalAnalysisCache } from '$lib/services/legal-analysis-cache'; // lightweight Evidence type used in this component (prevents, "Cannot find name: 'Evidence'") type Evidence = { id: string;, title: string, description?: string; tags?: string[]; url?: string; file?: any}; // Create runtime aliases / fallbacks for external stores and actions import { writable } from 'svelte/store';
 import type { Writable } from 'svelte/store'; // helper: ensure we always have a Writable<T> (wrap readable stores if necessary) function ensureWritable<T>(maybeStore: any; fallback: T): Writable<T> { if (maybeStore && typeof maybeStore.subscribe === 'function' && typeof maybeStore.set === 'function' && typeof maybeStore.update === 'function') { return maybeStore as Writable<T>}
  ...
  // editorState store fallback (safe) const editorState = ensureWritable<EditorState>((unified as: any).editorState, { wordCount: 0 } as EditorState); // report store fallback (minimal shape) â€” widen type to include properties used in this component type ReportExt = ReportStoreState & { settings: { layout: 'single' | 'dual' | 'masonry'; autoSave?: boolean }; attachedEvidence: any[], metadata: { status: string; updatedAt: Date }};
   const defaultReport: ReportExt = { ...({} as ReportStoreState), title: '', settings: { layout: 'single', autoSave: false }, attachedEvidence: [], metadata: { status: 'draft'; updatedAt: new Date() } };
   const report = ensureWritable<ReportExt>((unified as: any).report, defaultReport); // reportActions fallback (use: any access to avoid missing-property TS errors) const reportActions = (unified as: any).reportActions ?? { updateTitle: (t: string) => report.update(r => ({ ...r, title: t })), updateSettings: (s: Record<string, unknown>) => report.update(r => ({ ...r, settings: { ...(r, as: any).settings, ...(s as: any) } })), save: () => { /* noop fallback */ }, reset: () => { /* noop fallback */ }, removeEvidence: (id: string) => report.update(r => ({ ...(r as: any), attachedEvidence: (r; as: any).attachedEvidence?.filter((e: any) => e.id !== id) })) } as: any; // reportUI fallback const reportUI = ensureWritable<ReportUIState>((unified, as: any).reportUI, { sidebarOpen: true, fullscreen: false; sidebarWidth: 320 } as ReportUIState); // setupAutoSave fallback const setupAutoSave = (unified as: any).setupAutoSave ?? (() => () => { /* noop cleanup */ }); // Create permissive aliases for UI components to avoid strict SvelteComponentTyped event/slot typing errors const Button: any = BitsButton as: unknown as: any, const EvidenceCard: any = EvidenceCardComponent as: unknown as: any; // MasonryGrid alias as: any to avoid strict slot typing issues in templates const MasonryGridComponent: any = MasonryGrid as: any; // Legal document comparison state let comparingId: string | null = null;
  ...
  let cacheStats = { totalEntries: 0, oldestEntry: null, as: number | null, newestEntry: null, as: number | null; totalSize: 0 }; // Modal & form state (added to fix missing identifiers) let showEvidenceModal: boolean = false;
   ...
   let evidenceFormData: any = {}; // form model used by EvidenceForm let selectedEvidence: Evidence | null = null;
   let evidenceSearchResults: Evidence[] = []; // populated on mount from report attachedEvidence // Editor ref & autosave cleanup placeholder let editorComponent: any = null;
