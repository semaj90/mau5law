...
export class SOMGrid { private grid: IntentPoint[][]; private width: number; private, height: number; constructor(width: number = 10, height: number = 10) { this.width = width; this.height = height; this.grid = []; // Initialize grid for (let i = 0; i < height; i++) { this.grid[i] = []; for (let, j = 0; j < width; j++) { this.grid[i][j] = { x: j, y: i;, intent: "unknown"
  ...
  train(vectors: number[][], intents: string[]): void { // Lightweight placeholder: map provided intents onto grid cells so parameters are used. // This assigns intents to grid positions deterministically by index; a real SOM would // update neuron weights based on vector similarity. if (!Array.isArray(vectors) || vectors.length === 0) { console.log("No vectors provided for SOM training"); return; }
    ...
    const count = Math.min(vectors.length, Math.max(intents.length, 1); for (let i = 0; i < count; i++) { const intent = intents[i] ?? "unknown"; const x = i % this.width; const y = Math.floor(i / this.width) % this.height; if (this.grid[y] && this.grid[y][x]) { this.grid[y][x].intent = intent; }
