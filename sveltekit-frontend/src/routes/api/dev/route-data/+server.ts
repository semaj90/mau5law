...
export async function GET(): Promise<any> {
  try {
    ...
    return json(
      {
        error: 'Failed to load route data', message: error instanceof Error ? error.message : 'Unknown error', timestamp: new Date().toISOString()
      }, { status: 500  }
    ); } }
function generateEnhancedAnalytics(routeData: RouteDataInput): EnhancedAnalyticsResult {
  ...
  ).length; const apiRoutes = fileRoutes.filter(
  ...
  const deepestNesting = Math.max(
    ...fileRoutes.map((route: FileRoute) => (route.path?.split('/') || []).length - 1), 0
  );
  ...
  return {
    statusBreakdown, categoryBreakdown, tagUsage: complexityMetrics: {
      dynamicRoutes, apiRoutes, staticPages, deepestNesting: totalConfiguredRoutes: totalRoutes;
