...
export interface EnvironmentConfig { ollama: { baseUrl: string;
    port: number;
    isDetected: boolean;
   }
  redis: { url: string; host: string;
    port: number;
   }
  postgres: { url: string; host: string;
    port: number;
   }
  development: { isDev: boolean; debug: boolean;
    verbose: boolean;
   }
...
async function detectOllamaConfig(): Promise<any> {
  ...
  // Default fallback
  return {
    baseUrl: 'http://localhost:11434', port: 11434, isDetected: false
   }
} }
/**
...
export async function initializeEnvironment(): Promise<EnvironmentConfig> {
  ...
  return {
    ollama: redis: { url: process.env.REDIS_URL || 'redis://:redis@localhost:6379', host: process.env.REDIS_HOST || 'localhost', port: parseInt(process.env.REDIS_PORT || '6379')
    }, postgres: { url: process.env.DATABASE_URL || 'postgresql://legal_admin:123456@localhost:5434/legal_ai_db', host: process.env.POSTGRES_HOST || 'localhost', port: parseInt(process.env.POSTGRES_PORT || '5434')
    }, development: { isDev: process.env.NODE_ENV === 'development', debug: process.env.DEBUG === 'true' || process.env.VITE_DEBUG === 'true', verbose: process.env.VERBOSE === 'true' || process.env.VITE_VERBOSE === 'true' },'`'`
...
export const ENV_CONFIG: EnvironmentConfig = {
  ...
  }, minioEndpoint: import.meta.env.VITE_MINIO_ENDPOINT || 'http://localhost:9000', minioAccessKey: import.meta.env.VITE_MINIO_ACCESS_KEY || import.meta.env.VITE_MINIO_ACCESS_KEY || 'minio', minioSecretKey: import.meta.env.VITE_MINIO_SECRET_KEY || 'minio123', // Support both new purpose-specific envs and legacy MINIO_BUCKET
  minioBucket:
