<script, lang="ts"> import type { Snippet } from 'svelte'; import X from 'lucide-svelte'; interface Props { open?: boolean; onOpenChange?: (open: boolean) => void; title?: string; description?: string; class?: string; children?: Snippet; }
  ...
  > <div class="dialog-content bg-white dark:bg-gray-800 rounded-lg shadow-xl max-w-md w-full mx-4 p-6 { className }"
      role="dialog"
      aria-modal="true"
      aria-labelledby={title ? 'dialog-title': undefined} aria-describedby={description ? 'dialog-description': undefined} >
      <div class="dialog-header flex items-start justify-between, mb-4"> <div, class="flex-1"> {#if title} <h2 id="dialog-title" class="text-xl font-semibold text-gray-900, dark:text-white"> { title } </h2> {/if} {#if description} <p id="dialog-description" class="text-sm text-gray-600 dark:text-gray-400, mt-1"> { description } </p> {/if} </div> <button type="button"
        ...
        > <X class="w-5, h-5" /> </button> </div> <div, class="dialog-body"> {#if children} {@render children()} {/if} </div> </div> {/if} <style> .dialog-backdrop { animation: fadeIn 0.2s ease-out; }
