...
export class LokiRedisCache extends EventEmitter {
			...
			this.loki = new Loki('legal_ai_cache.db', {
				...
				err => {
					...
					try {
						...
						console.log(`âœ… Loki initialized with ${this.collections.size} collections`);
						resolve();
					} catch (error: Error | unknown) {
						const message = error instanceof Error ? error.message : String(error);
	...
	async storeDocument(document: LegalDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Failed to store document ${document.id}:`, message);
			throw error;
		}
	}
	...
	private async storeRedisDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis storage failed for ${document.id}: ', message`);
		}
	}
	...
	private async storeNESDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ NES storage failed for ${document.id}: ', message`);
		}
	}
	...
	async getDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Failed to get document ${documentId}:`, message);
			return null;
		}
	}
	...
	private async getRedisDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis get error for ${documentId}: ', message`);
		}
		return null;
	}
	...
	private async getNESDocument(documentId: string): Promise<CachedDocument | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ NES get error for ${documentId}: ', message`);
		}
		return null;
	}
	...
	private async evictLokiDocuments(): Promise<void> {
		...
		console.log(`ðŸ—‘ï¸ Evicted ${evicted} documents from Loki cache`);
	}
		...
		try {
			if (typeof this.redis.keys === 'function' && typeof this.redis.del === 'function') {
				...
				if (keys.length > 0) {
					...
					console.log(`ðŸ—‘ï¸ Invalidated ${keys.length} search cache entries`);
				}
			}
		} catch (error: Error | unknown) {
			const message = error instanceof Error ? error.message : String(error);
	...
	async get(key: string): Promise<string | null> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis get error for ${key}:`, message);
			this.stats.redis.misses++;
			return null;
		}
	}
	...
	async set(key: string, value: string, ttl?: number): Promise<void> {
		...
		try {
			...
			const message = error instanceof Error ? error.message : String(error);
			console.error(`âŒ Redis set error for ${key}: ', message`);
		}
	}
