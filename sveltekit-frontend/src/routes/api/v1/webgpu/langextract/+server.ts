...
// Cast to unknown first, then read properties with safe checks
...
webgpuOptimization: !!(s?.webgpuOptimizer?.gpuMetrics?.availableComputeUnits && s?.webgpuOptimizer?.gpuMetrics?.availableComputeUnits > 0),
...
console.log(`📄 Processing document: ${request.text.length} chars`);
...
const config = {
useWebGPUCache: true,
batchSize: 64, // Optimized for cache
cacheEmbeddings: true,
compressVectors: true,
practiceArea: 'legal-ai',
documentType: 'general' as const,
...
console.log(`📦 Batch: ${request.documents.length} documents`);

// processBatchDocumentsWithWebGPU may return ProcessingResult[] where
// extraction.risks may be string[]. Normalize to WebGPUResult[].
...
const avgThroughput = results.length ? results.reduce((sum, r) => sum + (r.performance?.throughput ?? 0), 0) / results.length : 0;
...
const avgCompressionRatio = compressionRatios.length ? compressionRatios.reduce((sum, v) => sum + v, 0) / compressionRatios.length : 0;

return {
batchResults: results.map(({ extraction = {}, performance = {}, embeddings = {} }: WebGPUResult): {
summary: string | null;
keyTerms: string[];
entities: RecordObject[];
risks: RecordObject[];
performance: { processingTime: number | null; webgpuUtilized: boolean; cacheHit: boolean };
} => ({
...
const result = (await processLegalDocumentWithWebGPU(doc, { useWebGPUCache: true, compressVectors: true })) as unknown as WebGPUResult;
...
for (let i = 0; i < iterations; i++) {
const doc = sampleDocuments[i % sampleDocuments.length];
const result = (await processLegalDocumentWithWebGPU(doc, { useWebGPUCache: false, compressVectors: false })) as unknown as WebGPUResult;
standardResults.push(result);
}
standardTime = Date.now() - standardStartTime;
}

const avgCacheHitRatio = iterations ? webgpuResults.reduce((sum, item) => sum + (item?.embeddings?.cacheHit ? 1 : 0), 0) / iterations : 0;
const avgCompressionRatio = iterations ? webgpuResults.reduce(
