...
export class CitationsManager {
    ...
    getSavedCitations(): SavedCitation[] {
        ...
        try {
            ...
            if (!saved) return [];
            const allCitations: any[] = JSON.parse(saved);
            return allCitations
                .filter((c: any) => c.userId === this.currentUser!.id)
                .map((c: any) => ({
                    ...c,
    ...
    getCollections(): CitationCollection[] {
        ...
        try {
            ...
            if (!saved) return [];
            const allCollections: any[] = JSON.parse(saved);
    ...
    async importCitationsToReport(citationIds: string[], reportId: string, options: CitationImportOptions): Promise<boolean> {
    ...
    searchSavedCitations(query: string, filters?: { category?: string; court?: string; relevance?: string; collection?: string }): SavedCitation[] {
        ...
        if (filters) {
            ...
            if (filters.collection) {
                const collections = this.getCollections();
                const collection = collections.find(col => col.id === filters.collection);
                if (collection) {
    ...
    private storeSavedCitations(citations: SavedCitation[]): void {
        try {
            ...
            // We need to keep other users' citations if we want to simulate multi-user in local storage
