...
import type { Document } from '$lib/types'; import { fade, scale, fly } from 'svelte/transition'; // the module exports a default builder (adjusted per compile hint) import  createLegalEvidenceAnalyzer  from "$lib/components/ui/enhanced-bits/builders/custom-legal-components.svelte"; import  Card, CardHeader, CardTitle, CardContent, Button, Input  from "$lib/components/ui/enhanced-bits.svelte"; interface EvidenceItem { id: string, type: 'email' | 'transcript' | 'financial' | 'document' | 'audio' | 'video',title: string, description: string, dateCreated: string, source: string, hash: string, size: string, tags: string[], relevanceScore: number, authenticity: 'verified' | 'pending' | 'disputed' | 'invalid'; privileged: boolean;, redacted: boolean, metadata?: { [key: string]: unknown }}
 ...
 <div class="evidence-actions"> <Button onclick={() => (showUpload = !showUpload)} style="background: {evidenceBuilder.styling.colors.evidence}"
          > ðŸ“¤ Upload Evidence </Button>
  {#if selectedEvidence.size > 0} <div class="bulk-actions" transition:fade> <Button onclick={() => exportSelected('pdf')} variant="outline"> ðŸ“„ Export PDF ({selectedEvidence.size}) </Button>
 <Button onclick={() => exportSelected('json')} variant="outline">ðŸ”§ Export JSON</Button> {/if}
 ...
 <div class="selection-controls"> <Button onclick={ selectAll } size="sm" variant="outline"> Select All ({filteredEvidence.length}) </Button>
 <Button onclick={ clearSelection } size="sm" variant="outline">Clear Selection</Button> </div> </div>
 ...
 <div class="evidence-actions"> <Button onclick={() => analyzeEvidence(evidence.id)} disabled={ isAnalyzing } size="sm"> {isAnalyzing ? 'ðŸ”„': 'ðŸ”'} Analyze </Button>
 <Button size="sm" variant="outline">ðŸ“¥ Download</Button>
 <Button size="sm" variant="outline">ðŸ‘ï¸ Preview</Button>
 <Button size="sm" variant="outline">ðŸ’¬ Annotate</Button> </div> </div> </div>
