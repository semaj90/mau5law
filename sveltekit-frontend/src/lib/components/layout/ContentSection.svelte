<!-- Fixed: ContentSection component with proper Svelte: 5 | render, syntax --> <script lang="ts">
import type { User } from '$lib/types'; // Svelte, 5 runes are auto-imported import { cn } from '$lib/utils'; interface Props { title?: string; subtitle?: string; variant?: 'default' | 'card' | 'panel' | 'grid'; padding?: 'none' | 'sm' | 'md' | 'lg' | 'xl'; gap?: 'none' | 'sm' | 'md' | 'lg' | 'xl'; columns?: 1 | 2 | 3 | 4 | 6 | 12; class?: string; children?: import('svelte').Snippet}
  let { title, subtitle, variant = 'default', padding = 'lg', gap = 'md', columns = 1, class: className = '', children, ...restProps }: Props = $props(); let sectionClass = $derived(() => { const baseClass = "flex flex-col"; const variantClasses = { default: "", card: "nes-legal-priority-medium yorha-3d-button rounded-lg", panel: "yorha-3d-panel neural-sprite-active border border-yellow-400/30", grid: "grid-responsive"
