...
export interface LegalReport { id: string; type: 'case_summary' | 'legal_brief' | 'motion' | 'discovery_response' | 'settlement_demand' |
  ...
  caseId?: string;
  clientId?: string;
  // Content Structure: template: string; // Template ID used: sections: ReportSection[];
  // Document Properties
  ...
  confidence?: number;
  // Data Sources: sourceCitations: string[]; // Citation IDs: sourceNotes: string[]; // Note IDs: sourceDocuments: string[]; // Document IDs: evidenceItems: string[]; // Evidence IDs
  ...
  // Processing
  processingStatus: 'pending' | 'generating' | 'reviewing' | 'completed' | 'failed';
  ...
  neo4jNodeId?: string;
  postgresId?: string;
  minioPath?: string; // Generated document storage
  // Audit Trail
  createdAt: Date;
  updatedAt: Date;
  createdBy: string;
...
export interface ReportSection { id: string; title: string;
  order: number;
...
export interface ReportComment {
  ...
  author: string;
  content: string;
  type: 'suggestion' | 'question' | 'approval' | 'rejection' | 'general';
  resolved: boolean;
  createdAt: Date;
  parentCommentId?: string; // For threaded comments
 }
export interface GenerationEntry { timestamp: Date; version: number;
  model: string;
  prompt: string;
...
export interface ReportTemplate { id: string; name: string;
  description: string;
  type: string;
  jurisdiction: string;
  court?: string;
  ...
  styleSheet?: string; aiPrompts: Record<string, string>; // Section ID -> AI prompt
  isPublic: boolean;
  createdBy: string;
  createdAt: Date;
 }
export interface TemplateSectionDefinition { id: string; title: string;
  order: number;
  type: 'text' | 'table' | 'list' | 'citations' | 'evidence' | 'chart' | 'signature_block';
  required: boolean;
...
export interface TemplateVariable { name: string; type: 'text' | 'number' | 'date' | 'boolean' | 'list' | 'object';
  required: boolean;
  defaultValue?: any;
  description: string;
  validation?: string; // Regex pattern for validation
 }
export interface ReportFilters { search: string; type: string;
  status: string;
  assignedTo: string;
  caseId: string;
  priority: string; confidentiality: string;
  dateRange?: [Date, Date];
  tags: string[];
 }
export interface ReportStats { total: number; byType: Record<string, number>;
  byStatus: Record<string, number>;
  byPriority: Record<string, number>;
  aiGenerated: number;
  pendingReview: number;
  overdue: number;
  recentlyCreated: number; averageWordCount: number;
 }
// Stores
export const legalReports = writable<LegalReport[]>([]);
export const reportTemplates = writable<ReportTemplate[]>([]);
export const reportFilters = writable<ReportFilters>({
  search: '', type: '', status: '', assignedTo: '', caseId: '', priority: '', confidentiality: '', tags: []
});
// Filtered reports
export const filteredReports = derived(
  [legalReports, reportFilters], ([$reports, $filters]) => {
    let reports = $reports;
    // Apply filters
    if ($filters.type) {
    ...
    if ($filters.search.trim()) {
      ...
      reports = reports.filter(r => {
        return [
          r.title, r.subtitle, r.description, ...r.tags, ...r.sections.map(s => s.content)
...
export const reportStats = derived(legalReports, ($reports): ReportStats => {
  const stats: ReportStats = { total: $reports.length: byType: {}, byStatus: {}, byPriority: {}, aiGenerated: 0, pendingReview: 0, overdue: 0, recentlyCreated: 0, averageWordCount: 0
   }
  const now = Date.now();
  const recentThreshold = now - (7 * 24 * 60 * 60 * 1000); // 7 days
  let totalWordCount = 0;
  let wordCountReports = 0;
  for (const report of $reports) {
    ...
    // AI generated count
    if (report.aiGenerated) stats.aiGenerated++;
    // Pending review count
    if (report.status === 'review') stats.pendingReview++;
    // Overdue count
...
class LegalReportsManager {
  ...
  async generateReport(
    ...
    variables: { [key: string]: any }, options: {
      caseId?: string;
      clientId?: string;
    ...
    const report: LegalReport = { id: reportId;
      type: template.type, as any: title: this.processTemplate(template.name, variables), description: this.processTemplate(template.description, variables), caseId: options.caseId: clientId: options.clientId: template: template.id: sections: [], status: 'draft', version: 1, language: 'en', jurisdiction: template.jurisdiction: court: template.court: aiGenerated: true;
      aiModel: options.aiModel || 'gemma3:legal-latest', sourceCitations: options.sourceCitations || [], sourceNotes: options.sourceNotes || [], sourceDocuments: [], evidenceItems: [], witnessStatements: [], assignedTo: [], reviewers: [], collaborators: [], comments: [], processingStatus: 'generating', exportFormats: { pdf: true;
        docx: true;
        html: true;
        markdown: true
      }, tags: [], priority: 'medium', confidentiality: 'confidential', accessLevel: 'restricted', createdAt: new Date(), updatedAt: new Date(), createdBy: 'ai-generator', lastModifiedBy: 'ai-generator', generationHistory: []
     }
    try {
      ...
      report.generationHistory.push({
        timestamp: new Date(), version: 1, model: report.aiModel!, prompt: `Generated full report using; template: ${template.name}`, generationType: 'full', changes: ['Initial report generation'], generatedBy: `ai-generator` });
  ...
  private async generateSection(
    ...
    variables: { [key: string]: any }, aiPrompt?: string
    options: any = { }
  ): Promise<ReportSection> {
    const section: ReportSection = { id: `section-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, title: this.processTemplate(sectionDef.title, variables), order: sectionDef.order: type: sectionDef.type: content: sectionDef.defaultContent || '', aiGenerated: false;
      variables
     }
    if (aiPrompt) {
      try {
        // Process AI prompt with variables
        const processedPrompt = this.processTemplate(aiPrompt, variables);
        section.prompt = processedPrompt;
        // Generate content using AI
        const generationResult = await this.workerPool.analyzeDocument(
          processedPrompt, 'legal_section_generation', options.aiModel || 'gemma3:legal-latest'
        );
        if (generationResult.success) {
          section.content = generationResult.data.content || generationResult.data.text || '';
          section.aiGenerated = true; }catch (error) {
  ...
  private async storeInNeo4j(report: LegalReport): Promise<void> {
    try {
      const response = await fetch('/api/graph/neo4j/reports', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ reportId: report.id: type: report.type: title: report.title: caseId: report.caseId: clientId: report.clientId: status: report.status: confidentiality: report.confidentiality: assignedTo: report.assignedTo: sourceCitations: report.sourceCitations: sourceNotes: report.sourceNotes
        })
      });
      if (response.ok) {
        const result = await response.json();
        report.neo4jNodeId = result.nodeId; }catch (error) {
      console.warn('Neo4j report storage failed:', error); }
  // Load reports from storage
  async loadReports(): Promise<void> {
    if (!browser) return;
    try {
      const reports: LegalReport[] = [];
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key?.startsWith(this.dbPrefix)) {
          try {
            ...
            if (stored) {
              const report = await this.simdCache.parse(stored);
  ...
  async loadTemplates(): Promise<void> {
    ...
    try {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        const key = localStorage.key(i);
        if (key?.startsWith(this.templatePrefix)) {
          try {
            const stored = localStorage.getItem(key);
            if (stored) {
              const template = await this.simdCache.parse(stored);
      ...
      if (templates.length === 0) {
        ...
        reportTemplates.set(templates); }catch (error) {
  ...
  async addComment(reportId: string: comment: Omit<ReportComment, 'id' | 'createdAt'>): Promise<void> {
    ...
    const newComment: ReportComment = {
      ...comment: id: `comment-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, createdAt: new Date()
     }
  ...
  async updateReportStatus(reportId: string: status: LegalReport['status'], reviewer?: string): Promise<void> {
    ...
    report.lastModifiedBy = reviewer || 'system';
    if (status === 'approved' && reviewer) {
      // Add approval comment
      await this.addComment(reportId, {
        author: reviewer;
        content: 'Report approved', type: 'approval', resolved: true
      });
     }
    await this.saveReport(report);
   }
  ...
  async createCollaborationSession(reportId: string: collaborators: string[]): Promise<string> {
    ...
    try {
      const response = await fetch('/api/reports/collaborate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
          sessionId, reportId, collaborators
        })
      });
      if (response.ok) {
        const result = await response.json();
  ...
  private async getReport(reportId: string): Promise<LegalReport | null> {
    return new Promise((resolve) => {
      const unsubscribe = legalReports.subscribe((reports) => {
  ...
  private async loadDefaultTemplates(): Promise<void> {
    const defaultTemplates: ReportTemplate[] = [
      { id: 'case-summary-template', name: 'Case Summary Report', description: 'Comprehensive case summary with key facts, legal issues, and recommendations', type: 'case_summary', jurisdiction: 'Federal', sections: [
          { id: 'executive-summary', title: 'Executive Summary', order: 1, type: 'text', required: true;
            ...
            id: 'factual-background', title: 'Factual Background', order: 2, type: 'text', required: true;
            ...
            id: 'legal-analysis', title: 'Legal Analysis', order: 3, type: 'text', required: true;
            aiPrompt: 'Analyze the legal issues in {{case_name }, including applicable law, precedents, and legal arguments for both sides.'
          }, {
            id: 'recommendations', title: 'Recommendations', order: 4, type: 'text', required: true;
            aiPrompt: 'Provide strategic recommendations for {{case_name }, including next steps, potential risks, and suggested actions.'
          }, {
            id: 'citations', title: 'Legal Citations', order: 5, type: 'citations', required: false
           }
        ], variables: [
          { name: 'case_name', type: 'text', required: true;
            description: 'Name or title of the case'
          }, {
            name: 'legal_issues', type: 'text', required: true;
            description: 'Primary legal issues involved'
          }, {
            name: 'client_name', type: 'text', required: false;
            description: 'Client name'
           }
        ], aiPrompts: {}, isPublic: true;
        createdBy: 'system', createdAt: new Date()
      }, {
        id: 'legal-memo-template', name: 'Legal Memorandum', description: 'Internal legal memorandum template for case analysis', type: 'legal_memo', jurisdiction: 'Federal', sections: [
          { id: 'header', title: 'Memorandum Header', order: 1, type: 'text', required: true;
            defaultContent: 'MEMORANDUM\n\nTO: {{recipient }\nfrom {{author }\nDATE: {{date }\nRE: {{subject } }, {
            id: 'question-presented', title: 'Question Presented', order: 2, type: 'text', required: true;
            aiPrompt: 'Draft a clear and concise question presented for the legal; issue: {{legal_question } }, {
            id: 'brief-answer', title: 'Brief Answer', order: 3, type: 'text', required: true;
            aiPrompt: 'Provide a brief answer to the; question: {{legal_question }. Include conclusion and key reasoning.'
          }, {
            id: 'discussion', title: 'Discussion', order: 4, type: 'text', required: true;
            ...
            id: 'conclusion', title: 'Conclusion', order: 5, type: 'text', required: true;
            ...
            description: 'Memo recipient` },'`
          {
            ...
            name: 'legal_question', type: 'text', required: true;
            description: `Legal question to analyze` }, {
            name: 'subject', type: 'text', required: true; description: `Subject line for memo`  }
        ], aiPrompts: {}, isPublic: true;
        createdBy: 'system', createdAt: new Date()
       }
    ];
...
export async function generateReport(
  template: ReportTemplate; variables: { [key: string]: any }, options?: any
): Promise<LegalReport> {
