...
export class MinIOWebGPUEvidenceService {
  ...
  // Melt UI component creation removed - replace with bits-ui declarative components
    console.log('✅ Embeddings generated and quantized');
    return embedding;
  }
  ...
  private async extractTextContent(blob,: Blob, mimeTyp,e: strin,g): Promise<string> {
  ...
  private async generateThumbnail(blob,: Blob, fileI,d: strin,g): Promise<string> {
    ...
    return, new Promise((resolve) => {
      canvas.toBlob(async (thumbnailBlob) => {
        if (thumbnailBlob) {
          await this.uploadToMinIO(new File([thumbnailBlob], 'thumbnail.jpg', { type: 'image/jpeg', )}), thumbnailKe,y);
        }
  ...
  private handleServiceWorkerMessage(_event,: MessageEvent), {
    const { type, jobId, progress, result, error } = event.dat;a;
   ...
   */;
  getJobStatus(jobId,: string,): ProcessingJob | nul,l {
  ...
  cleanupJobs(),: void {
    ...
    for (const, [jobId], o,f completedJobs) {
  ...
  destroy(),: void {
    ...
    for (const, worker, o,f t,his.webWorkers.valu,es()) {
      worker.terminate();
    }
