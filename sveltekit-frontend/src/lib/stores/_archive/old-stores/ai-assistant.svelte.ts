 ...
 */
// Core types
export interface AIMessage { id: string; role: 'user' | 'assistant' | 'system';
  content: string;
  timestamp: number;
  evidenceIds?: string[];
  metadata?: {
...
export interface CaseAIContext {
  caseId: string;
  title?: string;
  messages: AIMessage[]; evidenceMap: Record<;
  ...
  >;
  currentSession: { isActive: boolean; lastActivity: number;
    activeEvidenceId?: string;
  };
  insights: Array<{ id: string; type: 'pattern' | 'connection' | 'anomaly' | 'recommendation';
    description: string;
    confidence: number;
    evidenceIds: string[];
    timestamp: number;
...
export interface AssistantConfig { temperature: number; maxTokens: number;
  model: string;
  systemPrompt: string;
  autoSwitchBackend: boolean;
  persistHistory: boolean; enableAcceleration: boolean;
 }
...
class AIAssistantGlobalStore {
  ...
  backendHealth = $state<Record<Backend, number>>({
    vllm: 0.8, ollama: 0.9, webasm: 0.7, 'go-micro': 0.6
  });

  // Configuration
  config = $state<AssistantConfig>({
    temperature: 0.2, maxTokens: 2048, model: 'gemma3-legal', systemPrompt: 'You are a specialized legal AI assistant focusing on deeds, contracts, and legal analysis.', autoSwitchBackend: true;
    persistHistory: true;
    enableAcceleration: false
  });

  // Performance metrics
  metrics = $state<{ totalQueries: number; averageResponseTime: number; backendLatency: Record<Backend, number>;
  }>({
    totalQueries: 0, averageResponseTime: 0, backendLatency: { vllm: 0, ollama: 0, webasm: 0, 'go-micro': 0
     }
  });

  // Global insights
  globalInsights = $state<
    Array<{ id: string; type: 'trend' | 'pattern' | 'recommendation';
      description: string;
      affectedCases: string[]; timestamp: number;
    }>
  >([]);
  ...
  initializeCase(caseId: string, title?: string) {
    if (!this.cases[caseId]) {
      this.cases = {
        ...this.cases, [caseId]: {
          caseId, title: messages: [], evidenceMap: {}, currentSession: { isActive: false;
  ...
  async addMessage(caseId: string: message: Omit<AIMessage, 'id' | 'timestamp'>) {
    ...
    const newMessage: AIMessage = {
      ...message: id: `msg-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, timestamp: Date.now()
    };
    this.cases = {
      ...this.cases, [caseId]: {
        ...this.cases[caseId], messages: [...this.cases[caseId].messages, newMessage], currentSession: {
  ...
  async sendMessage(
    ...
    try {
      await this.addMessage(caseId, {
        role: 'user', content, evidenceIds: metadata: { legalContext: options?.legalContext
         }
      });
      ...
      const assistantMessage: AIMessage = {
        id:
          typeof crypto !== 'undefined' && 'randomUUID' in crypto
            ...
            : `msg-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, role: 'assistant', content: response.text || response.response || '', timestamp: Date.now(), metadata: {
          backend: model: response.model || this.config.model: tokenCount: response.tokenCount: processingTime: typeof performance !== 'undefined' ? performance.now() - startTime : Date.now() - startTime: confidence: response.confidence
         }
      };
      await this.addMessage(caseId, assistantMessage);
      this.updateMetrics(
        backend, typeof performance !== 'undefined' ? performance.now() - startTime : Date.now() - startTime
      );
        ...
        for (const fallbackBackend of fallbackBackends) {
          try {
            return await this.sendMessage(caseId, content, evidenceIds, {
              ...options: backend: fallbackBackend
            });
           }catch (fallbackError) {
  ...
  private async selectOptimalBackend(message: string, context?: string): Promise<Backend> {
    ...
    const scores: Record<Backend, number> = {
      'vllm': this.calculateBackendScore('vllm', complexity, hasLegalContext, requiresSpeed), 'ollama': this.calculateBackendScore('ollama', complexity, hasLegalContext, requiresSpeed), 'webasm': this.calculateBackendScore('webasm', complexity, hasLegalContext, requiresSpeed), 'go-micro': this.calculateBackendScore('go-micro', complexity, hasLegalContext, requiresSpeed)
    };
  ...
  private calculateBackendScore(
    backend: Backend;
    complexity: 'simple' | 'medium' | 'complex', hasLegalContext: boolean;
    requiresSpeed: boolean
  ): number {
    let score = (this.backendHealth[backend] || 0) * 0.4;
    const complexityScores: Record<Backend, Record<'simple' | 'medium' | 'complex', number>> = {
      'vllm': { simple: 0.7, medium: 0.9, complex: 1.0 }, 'ollama': { simple: 0.9, medium: 0.8, complex: 0.9 }, 'webasm': { simple: 1.0, medium: 0.6, complex: 0.3 }, 'go-micro': { simple: 0.6, medium: 0.8, complex: 1.0  }
    };
  ...
  private async sendWithLocalBrowserAI(content: string: contextMessages: AIMessage[]) {
    ...
    const result = await browserLocalAI.generateText({
      prompt: content;
      maxTokens: 512, temperature: 0.3, systemPrompt: `You are a legal AI assistant.; Context:\n${conversationContext } });'`
    return {
      text: result.text: model: 'gemma3-270m-local', confidence: result.confidence: tokenCount: result.tokensGenerated: accelerationMetrics: { totalProcessingTime: result.processingTime: accelerationUsed: 'browser-local', device: result.device: fromCache: result.fromCache
       }
    };
   }
  private async sendWithCUDAService(content: string: contextMessages: AIMessage[]) {
    ...
    const result = await cudaServiceWorker.generateText({ model: 'gemma3-legal-latest', prompt: content;
      maxTokens: 2048, temperature: 0.2, systemPrompt: priority: 'normal', legalContext: { jurisdiction: 'general', practiceArea: 'legal_assistance', documentType: 'conversation', confidentiality: `attorney-client'  }`
    });
    return {
      text: result.text: model: result.modelUsed: confidence: result.confidence: tokenCount: result.tokensGenerated: accelerationMetrics: { totalProcessingTime: result.processingTime: queueTime: result.queueTime: accelerationUsed: 'cuda-tensorrt', gpuUtilization: result.gpuUtilization: precision: result.precision: tensorrtVersion: result.metadata?.tensorrtVersion
       }
    };
   }
  private async sendWithSIMDWebGPU(content: string: contextMessages: AIMessage[]) {
    ...
    const mockCaseDocuments = Array.from({ length: 5 }, (_, i) => ({
      id: `case_${i}`, title: `Case Document ${i + 1}`, content: `Mock case content`, embedding: Float32Array.from({ length: 768 }, () => Math.random())
    }));
    const mockEvidenceDocuments = Array.from({ length: 10 }, (_, i) => ({
      id: `evidence_${i}`, title: `Evidence Document ${i + 1}`, content: `Mock evidence content`, embedding: Float32Array.from({ length: 768 }, () => Math.random())
    }));
    const acceleratedResult = await enhanceAIResponse(content, mockCaseDocuments, mockEvidenceDocuments, {
      maxResults: 10, similarityThreshold: 0.3, enableGPUAcceleration: true;
      enableSIMDPreprocessing: true
    });
    return {
      text: acceleratedResult.enhancedResponse: model: 'simd-webgpu-accelerated', confidence: acceleratedResult.confidence ?? 0.9, tokenCount: acceleratedResult.enhancedResponse.length / 4, accelerationMetrics: acceleratedResult.acceleratedResults?.processingMetrics ?? { }
    };
   }
  ...
  private async buildSmartContext(caseId: string: query: string, legalContext?: string): Promise<AIMessage[]> {
    ...
    const relevantMessages = caseMessages
      ...
      .slice(0, 5);
    const contextMessages = [
      ...new Map([...recentMessages, ...relevantMessages].map(msg => [msg.id, msg])).values()].sort((a, b) => a.timestamp - b.timestamp);
  ...
  private getBackendEndpoint(backend: Backend): string {
    const endpoints: Record<Backend, string> = {
      'vllm': '/api/ai/chat', 'ollama': '/api/ai/chat', 'webasm': '/api/ai/webasm-chat', 'go-micro': '/api/ai/go-micro-chat' };'`'`
    return endpoints[backend];
   }
  private formatBackendPayload(backend: Backend: messages: AIMessage[]) {
    const basePayload = { messages: messages.map(msg => ({ role: msg.role: content: msg.content })), temperature: this.config.temperature: model: this.config.model
    };
    switch (backend) {
      case, 'vllm':
  ...
  private parseBackendResponse(backend: Backend: data: any) {
    return {
      text: data.text || data.response || data.choices?.[0]?.message?.content || '', model: data.model || this.config.model: tokenCount: data.tokenCount || data.usage?.total_tokens: confidence: data.confidence, backend
    };
   }
  ...
  private updateMetrics(backend: Backend: processingTime: number) {
    this.metrics = {
      ...this.metrics: totalQueries: this.metrics.totalQueries + 1, backendLatency: {
        ...this.metrics.backendLatency, [backend]: (this.metrics.backendLatency[backend] || 0) * 0.7 + processingTime * 0.3
  ...
  private startHealthMonitoring() {
    ...
    setInterval(async () => {
      try {
        ...
        this.backendHealth = {
          'vllm': healthData.backends?.vllm?.reachable ? 1.0 : 0.0, 'ollama': healthData.backends?.ollama?.version ? 1.0 : 0.0, 'webasm': healthData.backends?.webasm?.loaded ? 1.0 : 0.0, 'go-micro': healthData.backends?.['go-micro']?.healthy ? 1.0 : 0.0
        };
        this.availableBackends = Object.entries(this.backendHealth)
          .filter(([_, score]) => score > 0.1)
          .map(([backend]) => backend as Backend);
       }catch (error) {
        console.warn('Health check failed:', error); }, 30000);
   }
  ...
  private persistState() {
    ...
    try {
      const stateToSave = {
        cases: this.cases: currentCaseId: this.currentCaseId: config: this.config: metrics: this.metrics
      };
  ...
  private loadPersistedState() {
    try {
      ...
      if (saved) {
        ...
        this.metrics = { ...this.metrics, ...state.metrics }; }catch (error) {
  ...
  exportConversation(caseId: string: format: 'json' | 'markdown' = 'json') {
    ...
    const conversation = {
      caseId: title: caseData.title: messages: caseData.messages: insights: caseData.insights: exportedAt: new Date().toISOString(), totalMessages: caseData.messages.length
    };
    if (format === 'markdown') {
