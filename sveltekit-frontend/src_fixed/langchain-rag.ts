...
type RunnableInvokeInput = string | { prompt: string; [key: string]: unknown;
...
class OllamaHTTPLLMInternal {
  ...
  async invoke(input: RunnableInvokeInput): Promise<RunnableInvokeOutput> {
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        throw new Error(`Ollama generation API error: ${response.status;
} ${response.statusText;
} - ${errorText;
}`);}      }
...
Only return the queries, one per line.`),
      ...
      try {
...
} = await import('@langchain/community/vectorstores/qdrant');
        ...
        this.vectorStore = {
          ...
          addDocuments: async (docs: LangChainDocumentType[]) => {
            console.log(`Mock QdrantVectorStore: Adding ${docs.length;
          ...
          similaritySearch: async (query: string, k: number) => {
            console.log(`Mock QdrantVectorStore: Similarity search for "${query;
}" (k=${k;
          ...
          asRetriever: (opts: { k?: number; filter?: MetadataFilter;
            ...
            getRelevantDocuments: async (query: string) => {
              console.log(`Mock QdrantVectorStore: Retrieving documents for "${query;
}" (k=${opts.k;
  ...
  async indexDocument(text: string, metadata: LegalDocumentMetadata): Promise<string[]> {
    ...
    try {
      ...
      console.log(`✅ Indexed ${chunks.length;
} chunks for document ${metadata.documentId ?? metadata.id ?? 'unknown'}`);}      return ids || [];
    } catch (error: unknown) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  async extractLegalEntities(query: string, documentType?: string, options: RAGQueryOptions = {}): Promise<RAGResult> {
    const entityQuery = `Extract and list all ${query;
} mentioned in the legal documents. Provide specific references to where each item is mentioned.`;}    return await this.query(entityQuery, { ...options, documentType;
});
  }
...
}): PDFLoadingTask | Promise<PDFDocumentProxy> | PDFDocumentProxy;
