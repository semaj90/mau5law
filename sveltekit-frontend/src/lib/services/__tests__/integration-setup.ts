...
beforeAll(async () => {
  ...
  const services = [
    { name: 'Legal Gateway', url: 'http://localhost:8080/health' }, { name: 'Auth Service', url: 'http://localhost:8081/health' }, { name: 'Session Service', url: 'http://localhost:8082/health' }, { name: 'Canvas Service', url: 'http://localhost:8083/health' }, { name: 'Database', url: 'http://localhost:5432'  }
  ];
  ...
  for (const service of services) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(`   ⚠️  ${service.name }- Responding but not healthy (${response.status})`); }catch (error) {
...
;(global as any).integrationTestConfig = {
  baseUrl: 'http://localhost:8080', timeout: 5000, retries: 2, skipOnServiceUnavailable: true
 }
