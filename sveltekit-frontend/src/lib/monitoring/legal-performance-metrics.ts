...
export interface CacheHitMetrics {
    ...
    L2_Memory: number;
    L3_Redis: number;
    L4_Database: number; // Added since it's used in calculation
    overall: number; // Added since it's used
}
...
export const ALERT_THRESHOLDS: AlertThreshold[] = [
    { metric: 'cache_hits.overall', threshold: 0.7, severity: 'warning', message: 'Overall cache hit rate below 70%' },
    { metric: 'latency.total_query_time', threshold: 1000, severity: 'warning', message: 'Query latency exceeds 1 second' },
    { metric: 'resources.gpu_vram_usage', threshold: 6800, severity: 'critical', message: 'GPU VRAM usage critical (>6.8GB on RTX 3060 Ti)' },
    { metric: 'resources.gpu_utilization', threshold: 95, severity: 'warning', message: 'GPU utilization extremely high (>95%)' },
    { metric: 'legal_processing.legal_confidence_score', threshold: 0.8, severity: 'info', message: 'Legal analysis confidence below 80%' },
    { metric: 'throughput.queries_per_second', threshold: 10, severity: 'info', message: 'Throughput below target (10 QPS)' }
];
...
export class LegalPerformanceMonitor {
    ...
    private initializeMonitoring() {
        ...
        if (typeof setInterval !== 'undefined') {
            ...
            // Clean up old metrics every 10 minutes (keep last 120 snapshots)
    ...
    private calculateCacheHitRates(): CacheHitMetrics {
        ...
        const overall = (l1 + l2 + l3 + (1 - l4)) / 4; // L4 is miss rate, so invert logic if L4 represents DB hit which is slow

        return {
            L1_GPU: l1,
            L2_Memory: l2,
            L3_Redis: l3,
            L4_Database: 1 - l4, // Assuming 1-l4 means DB Avoidance Rate? Original code logic was murky.
            overall
        }
    }
