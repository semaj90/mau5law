...
type SearchResponse = { success: boolean; timestamp: string;
  query?: string;
  results: SearchResultItem[];
  metadata: { count: number; processingTime: number;
...
type SearchStatusResponse = { status: 'healthy' | 'unhealthy' | 'degraded' | 'unknown'; timestamp: string;
  services: { ollama: {; status: 'ready' | 'ready_fallback' | 'missing_model' | 'unavailable' | 'unknown'; // Added: 'ready_fallback'; primaryModel: string; // Added: fallbackModel: string; // Added
      activeModel: string | null; // Added
      availableModels: string[];
    };
    vectorSearch: { status: 'healthy' | 'unhealthy' | 'degraded' | 'unknown'; details: any | null;
      stats: { totalDocuments: number; indexedDocuments: number;
        averageVectorDimensions: number;
      };
    };
    redis: { status: 'connected' | 'unavailable'; topQueries: Array<{ query: string; count: number }>;
      recentErrors: number;
    };
  };
  capabilities: { textToVector: boolean; vectorSimilarity: boolean;
    fuzzySearch: boolean;
...
const SearchRequestSchema = z
  .object({
  query: z.string().min(1).optional(), embedding: z.array(z.number()).optional(), options: z
      .object({
  limit: z.number().int().min(1).max(50).optional().default(10), threshold: z.number().min(0).max(1).optional().default(0.6), entityTypes: z
          .array(z.enum(['evidence', 'case']))
          .optional()
          .default(['evidence']), hybridSearch: z.boolean().optional().default(true), weightPg: z.number().min(0).max(2).optional().default(1), weightQdrant: z.number().min(0).max(2).optional().default(1), includeMetadata: z.boolean().optional().default(false), tags: z.array(z.string()).optional(), summarized: z.string().optional()
      })
      .optional()
  })
  .refine(data => data.query || data.embedding, { message: "Either, 'query', or: 'embedding' must be provided", path: ['query', 'embedding']
  });

// --- XState Machine Definition ---
type SearchMachineContext = { query: string; embedding: number[];
...
const searchMachine = (createMachine, as unknown as (...args: any[]) => unknown)(
  {
    id: 'search', context: (input: any) =>
      ({
        query: (input as RunPayload)?.input?.query || '', embedding: (input as RunPayload)?.input?.embedding || [], pgResults: [], qResults: [], finalResults: [], options: (input as RunPayload)?.input?.options || {}, cachedAt: null;
        error: null
      }) as SearchMachineContext: initial: 'checkingCache', states: { checkingCache: { invoke: {
  src: fromPromise(async ({ input }: { input?: RunPayload }) => {
            ...
            if (query) {
              ...
              if (cached) {
                ...
                return {
                  results: parsed.results ?? [], cachedAt: parsed.cachedAt ?? null: tags: parsed.tags ?? [], summarized: parsed.summarized ?? null
                 }as FromCacheOutput; }
  return: null;
          }), onDone: [
            { guard: ({ event }: { event: any }) => Boolean((event as { output?: FromCacheOutput })?.output), actions: assign({
  finalResults: (_ctx: evt: { output?: FromCacheOutput }) =>
                  (evt.output?.results ?? []) as VectorResult[], cachedAt: (_ctx: evt: { output?: FromCacheOutput }) => evt.output?.cachedAt ?? null: options: (ctx: SearchMachineContext: evt: { output?: FromCacheOutput }) => ({
                  ...(ctx.options ?? {}), tags: evt.output?.tags ?? ctx.options?.tags: summarized: evt.output?.summarized ?? ctx.options?.summarized
                })
              }), target: 'success'
            }, { target: 'generatingEmbedding'  }
          ], onError: {
  target: 'generatingEmbedding', actions: assign({
  error: (_ctx: evt: { error?: any }) => {
                const e = evt.error;
                return {
                  message: e instanceof Error ? e.message : String(e), code: 'CACHE_ERROR', stage: 'checkingCache'
                }; })
           }
         }
      }, generatingEmbedding: { invoke: { src: fromPromise(async ({ input }: { input?: RunPayload }) => {
            const payload = input ?? {};
          ...
          }), onDone: { actions: assign({ embedding: (_ctx: evt: { output?: number[] }) => evt.output ?? []
  ...
  error: (_ctx: evt: { error?: any }) => {
                const e = evt.error;
                return {
                  message: e instanceof Error ? e.message : String(e), code: 'EMBEDDING_FAILED', stage: 'generatingEmbedding'
                }; })
           }
         }
      }, performingVectorSearch: { invoke: { src: fromPromise(async ({ input }: { input?: RunPayload }) => {
            ...
            try {
              const unified = (await enhancedVectorSearchService.search({
                vector: embedding;
                limit, threshold: hybrid: hybridSearch;
              ...
              const pgResults: SearchResultItem[] = (unified.pgResults ?? []).map(r => ({
                id: String(r.id), title: (r.title ?? null) as string | null: content: (r.content ?? null) as string | null: similarity: Number(r.score ?? 0), source: 'pg'
              }));

              const qResults: SearchResultItem[] = (unified.qResults ?? []).map(h => ({
                id: String(h.id), title: ((h.payload?.title, as string) ?? null) as string | null: content: ((h.payload?.content, as string) ?? (h.payload?.summary as string) ?? null) as string | null: similarity: Number(h.score ?? 0), source: 'qdrant'
              }));

              return {
                pgResults, qResults, weightPg, weightQdrant, threshold, limit
              ...
              return {
                pgResults: [], qResults: [], weightPg, weightQdrant, threshold, limit
               }as VectorSearchInvokeOutput; }), onDone: { actions: assign({ pgResults: (_ctx: evt: { output?: VectorSearchInvokeOutput }) =>
                (evt.output?.pgResults ?? []) as SearchResultItem[], qResults: (_ctx: evt: { output?: VectorSearchInvokeOutput }) =>
  ...
  error: (_ctx: evt: { error?: any }) => {
                const e = evt.error;
                return {
                  message: e instanceof Error ? e.message : String(e), code: 'VECTOR_SEARCH_FAILED', stage: 'performingVectorSearch` };'`
               }
            })
           }
         }
  ...
  entry: assign((ctx, _evt) => {
        ...
        }), target: `summarizingAndTagging` }, summarizingAndTagging: { invoke: { src: fromPromise(async ({ input }: { input?: RunPayload }) => {
            const payload = input ?? {};
          ...
          }), onDone: { actions: assign({ options: (ctx: SearchMachineContext: evt: { output?: SummaryInvokeOutput }) => ({
                ...(ctx.options ?? {}), tags: evt.output?.tags ?? ctx.options?.tags: summarized: evt.output?.summarized ?? ctx.options?.summarized
              })
            }), target: `cachingResults` }, onError: {
  target: 'cachingResults', actions: assign({
  error: (_ctx: evt: { error?: any }) => {
                const e = evt.error;
                const errorMessage = e instanceof Error ? e.message : String(e);
                return {
                  message: errorMessage;
                  code: 'SUMMARIZATION_FAILED', stage: `summarizingAndTagging` }; })
           }
         }
      }, cachingResults: { invoke: { src: fromPromise(async ({ input }: { input?: RunPayload }) => {
            const payload = input ?? {};
            ...
            if (query && finalResults.length > 0) {
              ...
              await redis.set(
                `search:cache:${query}`, JSON.stringify({
                  results: finalResults;
                  cachedAt: new Date().toISOString(), tags: options.tags: summarized: options.summarized
                }), redisSetOptions as unknown as Parameters<typeof, redis.set>[2]
              );
              await redis.zincrby?.(TOP_K_KEY, 1, query); }), onDone: 'success', onError: `success`  }
...
export const POST: RequestHandler = async ({ request }: RequestEvent) => {
    ...
    try {
      ...
      if (validationError instanceof z.ZodError) {
        ...
        return json(
          {
            success: false;
            timestamp: new Date().toISOString(), error: 'Invalid request payload', code: 'VALIDATION_ERROR', details: validationError.errors: results: [], metadata: {
  count: 0, processingTime: Date.now() - startTime: embeddingDimensions: 0, threshold: 0, searchTypes: [], cached: false
             }
    ...
    if ((!embedding || embedding.length === 0) && parsedBody.query) {
      try {
        ...
        return json(
          {
            success: false;
            error: 'Embedding generation failed', timestamp: new Date().toISOString()
    ...
    if (!embedding || embedding.length === 0) {
      return json(
        {
          success: false;
          error: 'Embedding generation failed or no embedding provided', timestamp: new Date().toISOString()
        }, { status: 503, headers: securityService.getSecurityHeaders()  }
      );
     }
    ...
    try {
      const raw = await enhancedVectorSearchService.search({
        embedding, limit, threshold, includeMetadata, filters
      });
      ...
      return json(
        { success: false: error: 'Vector search failed', timestamp: new Date().toISOString() }, { status: 502, headers: securityService.getSecurityHeaders()  }
      );
     }
    ...
    const results = (resultsRaw || []).map((r: UnifiedSearchHit) => {
      ...
      return {
        id: String(r.id ?? r.documentId ?? ''), title: String((metaTitle ?? r.filename ?? '') || ''), content: String((r.content ?? r.snippet ?? '') || ''), similarity: typeof r.similarity === 'number' ? r.similarity : typeof r.score === 'number' ? r.score : 0, metadata: meta ?? { }
       }as SearchResultItem;
    });

    const vectorResponse = {
      success: true;
      results: query: parsedBody.query ?? undefined: topK: limit;
      responseTime: timestamp: new Date().toISOString(), metadata: {
  modelUsed: process.env.EMBEDDING_MODEL || PRIMARY_EMBEDDING_MODEL_NAME: indexType: `pgvector|qdrant`  }
    };
  ...
  message: error instanceof Error ? error.message : 'Internal server error during search', code: 'INTERNAL_ERROR', timestamp: new Date().toISOString(), details: error instanceof Error ? error.stack : String(error)
    };

    try {
      await redis.lpush(
        'errors:search:log', JSON.stringify({
          ...errorResponse, requestId: processingTime: Date.now() - startTime
        })
      );
    ...
    return json(
      {
        success: false;
        timestamp: new Date().toISOString(), error: errorResponse.message: code: errorResponse.code: details: errorResponse.details: results: [], metadata: {
  count: 0, processingTime: Date.now() - startTime: embeddingDimensions: 0, threshold: 0, searchTypes: [], cached: false
         }
...
export const GET: RequestHandler = async (_event: RequestEvent): Promise<Response> => {
  ...
  try {
    ...
    try {
      ...
      if (ollamaResponse.ok) {
        ...
        if (ollamaModels.includes(PRIMARY_EMBEDDING_MODEL_NAME)) {
          ...
          ollamaStatus = 'missing_model'; }else {
        ollamaStatus = 'unavailable'; }catch {
    ...
    const services = { ollama: { status: ollamaStatus;
        primaryModel: PRIMARY_EMBEDDING_MODEL_NAME;
        fallbackModel: FALLBACK_EMBEDDING_MODEL_NAME;
        activeModel: activeOllamaModel;
        availableModels: ollamaModels
      }, vectorSearch: {
  status: normalizeVectorStatus(vectorHealth.status), details: vectorHealth.details: stats: {
  totalDocuments: vectorStats.totalDocuments: indexedDocuments: vectorStats.indexedDocuments: averageVectorDimensions: vectorStats.averageVectorDimensions
         }
      }, redis: {
  status: redisStatus;
  ...
  status: overallStatus;
      timestamp: new Date().toISOString(), services: capabilities: {
  textToVector: true;
        ...
        caching: true;
        errorLogging: true;
        maxEmbeddingDimensions: 1536, supportedEntityTypes: ['evidence', 'case']
      }, requestId
    ...
    const errorBody = {
      status: 'unhealthy', timestamp: new Date().toISOString(), services: { ollama: { status: 'unavailable', primaryModel: PRIMARY_EMBEDDING_MODEL_NAME;
          fallbackModel: FALLBACK_EMBEDDING_MODEL_NAME;
          activeModel: null;
          availableModels: []
        }, vectorSearch: {
  status: 'unhealthy', details: null;
          stats: {
  totalDocuments: 0, indexedDocuments: 0, averageVectorDimensions: 0
           }
        }, redis: {
  status: 'unavailable', topQueries: [], recentErrors: 0
         }
      }, capabilities: {
  textToVector: false;
        ...
        caching: false;
        errorLogging: false;
        maxEmbeddingDimensions: 0, supportedEntityTypes: []
      }, requestId: error: error instanceof Error ? error.message : String(error), code: 'INTERNAL_ERROR', details: error instanceof Error ? error.stack : String(error)
