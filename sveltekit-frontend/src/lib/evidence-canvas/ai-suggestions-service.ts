...
import { EvidenceNode, SimilarityResult } from './case-similarity-service';
  ...
  private async generateEvidenceSuggestions(
    ...
    for (const missingType of missingTypes) {
      const suggestion: AISuggestion = {
        ...
        confidence: 0.8,
        relatedNodes: context.selectedNodes.map(n => n.id),
        priority: 'high',
        actionItems: [
...
Provide 2-3 strategic recommendations with confidence levels.`;
  ...
  private analyzeEvidenceGaps(nodes: EvidenceNode[]): Array<{
    ...
    if (dates.length > 1) {
      ...
      if (daysSpan > 365) {
        gaps.push({
          ...
          confidence: 0.7,
          mitigationSteps: ['Document timeline gaps', 'Explain missing periods', 'Gather bridging evidence']
        });
      }
    }
    ...
    if (witnessStatements.length === 0) {
      gaps.push({
        ...
        confidence: 0.9,
        mitigationSteps: ['Identify potential witnesses', 'Prepare witness questionnaires', 'Schedule depositions']
      });
    }
  ...
  private generateCacheKey(context: SuggestionContext): string {
    return `${context.caseType}_${context.jurisdiction}_${context.currentPhase}_${context.selectedNodes.map(n => n.id).sort().join(',')}`;
  }
