...
export class CacheInvalidationService {
	 ...
	 */
	async invalidatePattern(pattern: string, options: InvalidationOptions): Promise<InvalidationResult> {
		const result: InvalidationResult = {
			memoryKeysCleared: 0,
			redisKeysCleared: 0,
			queuePublished: false,
		...
		console.log(`🗑️ Cache pattern invalidated: ${pattern} (type: ${options.type}, memory: ${result.memoryKeysCleared})`);
		return result;
	}
	 ...
	 */
	async invalidateMultiple(patterns: string[], options: InvalidationOptions): Promise<InvalidationResult> {
 ...
 */
export const invalidateReportCache = async (reportId: string, type: InvalidationType, userId?: string) => {
...
export const invalidateEvidenceCache = async (evidenceId: string, caseId: string | undefined, type: InvalidationType, userId?: string) => {
	const patterns = [
		CACHE_PATTERNS.EVIDENCE(evidenceId),
		CACHE_PATTERNS.EVIDENCE_SEARCH
	];
