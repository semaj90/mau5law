...
class VectorPipelineService {
  ...
  async processDocuments(request: VectorPipelineRequest): Promise<ProcessingResult> {
    ...
    try {
      ...
      console.log(
        `Processing ${objectsToProcess.length} objects in bucket: ${request.bucket_name}`
      );
    ...
    try {
      ...
      try {
        if (isEmbeddingService(embeddingService)) {
          ...
          if (
            Array.isArray(maybeResult) &&
            maybeResult.length > 0 &&
            Array.isArray(maybeResult[0])
          ) {
          ...
          console.warn(
            'embeddingService returned unexpected shape, falling back to FastEmbed HTTP endpoint'
          );
        }
      } catch (svcErr: any) {
