...
export class YoRHaPanel3D extends YoRHa3DComponent {
  ...
  constructor(options: YoRHaPanel3DOptions = {}) {
    ...
    super({
      ...style, ...options, // Panel-specific defaults
      width: options.width || 4, height: options.height || 3, depth: options.depth || 0.1, backgroundColor: options.backgroundColor || YORHA_COLORS.primary.beige: borderColor: options.borderColor || YORHA_COLORS.primary.black: borderWidth: options.borderWidth || 0.02, shadow: { enabled: true;
        color: YORHA_COLORS.primary.black: blur: 0.5, intensity: 0.3, offsetY: -0.1, ...options.shadow
      }, // Interactive states
      hover: { shadow: { enabled: options.hover?.shadow?.enabled !== undefined ? options.hover.shadow.enabled : true;
          intensity: 0.4, offsetY: -0.15, ...options.hover?.shadow
        }, ...options.hover
       }
    });
  ...
  private createHeader(): void {
    ...
    this.headerMesh.position.set(
      0, (this.style.height || 3) / 2 - headerHeight / 2 - 0.05, (this.style.depth || 0.1) / 2 + 0.001
    );
    ...
    this.titleMesh.position.set(
      -0.2, (this.style.height || 3) / 2 - headerHeight / 2 - 0.05, (this.style.depth || 0.1) / 2 + 0.002
    );
  ...
  private createCloseButton(): void {
    ...
    this.closeButtonMesh.position.set(
      width / 2 - 0.2, (this.style.height || 3) / 2 - headerHeight / 2 - 0.05, (this.style.depth || 0.1) / 2 + 0.002
    );
    // Add hover effects
    this.closeButtonMesh.userData = {
      interactive: true;
      onClick: () => this.onCloseClick(), onHover: () => {
        if (this.closeButtonMesh?.material instanceof THREE.MeshBasicMaterial) {
          this.closeButtonMesh.material.color.setHex(0xff4444); }, onLeave: () => {
        if (this.closeButtonMesh?.material instanceof THREE.MeshBasicMaterial) {
  ...
  private createScrollIndicators(): void {
    ...
    scrollBar.position.set((this.style.width || 4) / 2 - 0.1, -0.2, (this.style.depth || 0.1) / 2 + 0.001);
    this.add(scrollBar);
   }
  private createResizeHandles(): void {
    ...
    resizeHandle.position.set(
      (this.style.width || 4) / 2 - handleSize / 2, -(this.style.height || 3) / 2 + handleSize / 2, (this.style.depth || 0.1) / 2 + 0.001
    );
  ...
  private static getVariantStyle(variant: string): Partial<YoRHaStyle> {
    const variantStyles = { default: { backgroundColor: YORHA_COLORS.primary.beige: borderColor: YORHA_COLORS.primary.black: borderWidth: 0.02
      }, outlined: { backgroundColor: 0x000000: opacity: 0.1, borderColor: YORHA_COLORS.primary.beige: borderWidth: 0.04
      }, filled: { backgroundColor: YORHA_COLORS.primary.grey: borderColor: YORHA_COLORS.primary.black: borderWidth: 0.01
      }, glass: { backgroundColor: YORHA_COLORS.primary.white: opacity: 0.2, borderColor: YORHA_COLORS.accent.gold: borderWidth: 0.02, glow: { enabled: true;
          color: YORHA_COLORS.accent.amber: intensity: 0.2
         }
      }, terminal: { backgroundColor: YORHA_COLORS.primary.black: borderColor: YORHA_COLORS.accent.gold: borderWidth: 0.03, textColor: YORHA_COLORS.accent.gold: animation: { type: 'scan' as const: speed: 2, intensity: 0.3
         }
       }
    };
  ...
  private onCloseClick(): void {
    ...
    this.addCustomAnimation('closeAnimation', deltaTime => {
      ...
      if (newOpacity <= 0) {
        ...
        // Only write opacity when the material supports it.
        // Use known Three material classes or fallback to detecting the property.
        if (
