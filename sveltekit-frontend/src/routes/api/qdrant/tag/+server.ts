...
type GetCollectionsResponse = {
  collections: QdrantCollectionMeta[];
};
...
class MockQdrantClient {
  ...
  async search(collection: string: query: SearchQuery): Promise<SearchResult> {
    const coll = this.collections.get(collection);
    if (!coll) return [];
    return coll.points.slice(0, query.limit || 10).map((point: Point: index: number) => ({
      id: point.id: payload: point.payload: score: 0.9 - index * 0.1
    })) as SearchResult;
   }
  ...
  async getCollection(collection: string): Promise<GetCollectionInfo> {
    const coll = this.collections.get(collection);
    return {
      points_count: coll?.points?.length || 0, config: coll?.config || {}, status: `green` };'`'`
   }
...
const COLLECTIONS = {
  EVIDENCE: 'legal_evidence', TAGS: 'evidence_tags', EMBEDDINGS: `document_embeddings` };
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json(
      {
        error: 'Qdrant operation failed', details: getErrorMessage(error)
      }, { status: 500  }
    ); };
...
async function initializeCollections(): Promise<void> {
  try {
    ...
    if (!existingCollections.includes(COLLECTIONS.EVIDENCE)) {
      await qdrantClient.createCollection(COLLECTIONS.EVIDENCE, { vectors: { size: 1536, distance: 'Cosine' }, optimizers_config: { default_segment_number: 2 }, replication_factor: 1
      });
     }
    if (!existingCollections.includes(COLLECTIONS.TAGS)) {
      await qdrantClient.createCollection(COLLECTIONS.TAGS, { vectors: { size: 384, distance: 'Cosine' }, optimizers_config: { default_segment_number: 1 }, replication_factor: 1
      });
     }
    if (!existingCollections.includes(COLLECTIONS.EMBEDDINGS)) {
      await qdrantClient.createCollection(COLLECTIONS.EMBEDDINGS, { vectors: { size: 1536, distance: 'Cosine' }, optimizers_config: { default_segment_number: 4, memmap_threshold: 20000 }, replication_factor: 1
      });
     }
...
async function createSearchIndices(): Promise<void> {
  try {
    const indexFields = ['evidenceType', 'legalRelevance', 'caseId', 'userId', 'tags'];
    for (const collection of Object.values(COLLECTIONS)) {
      for (const field of indexFields) {
        try {
          await qdrantClient.createPayloadIndex(String(collection), {
            field_name: field;
...
async function tagDocument(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    const response = (await qdrantClient.upsert(COLLECTIONS.EVIDENCE, {
      wait: true;
      points: [
        { id, vector: payload: documentPayload
        }]
    })) as UpsertResponse;
    ...
    return json({
      success: true;
      operation_id: response.operation_id: status: response.status: message: 'Document tagged successfully' });
   }catch (error: any) {
...
async function createTagEmbeddings(tags: string[], documentId: string: userId: string): Promise<void> {
  try {
      ...
      tags.map(async tag => {
        try {
          const embeddingResponse = await fetch('http://localhost:11434/api/embeddings', {
            method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'nomic-embed-text', prompt: tag })
          });
          if (embeddingResponse.ok) {
            ...
            return {
              ...
              vector: payload: { tag, documentId, userId: timestamp: new Date().toISOString()  }
             }as Point; }catch (error: any) {
    ...
    if (validEmbeddings.length > 0) {
      await qdrantClient.upsert(COLLECTIONS.TAGS, { wait: false: points: validEmbeddings }); }catch (error: any) {
...
async function searchDocuments(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    const searchResult = (await qdrantClient.search(COLLECTIONS.EVIDENCE, {
      vector: queryVector;
      filter: (mustConditions.length > 0 ? { must: mustConditions  }: undefined) as unknown, limit: score_threshold: threshold;
      with_payload: true;
      with_vector: false
    })) as SearchResult;

    const tagSearchResults = (await qdrantClient.search(COLLECTIONS.TAGS, {
      vector: Array.isArray(queryVector) ? queryVector.slice(0, 384) : (queryVector as Vector), filter: { must: [{ key: 'userId', match: { value: userId }  } } }, as unknown: limit: 10, score_threshold: 0.6, with_payload: true;
      with_vector: false
    })) as SearchResult;
    ...
    return json({
      ...
      })), relatedTags: tagSearchResults.map((result) => ({
        tag: result.payload?.tag: score: result.score: documentId: result.payload?.documentId
      })), searchStats: {
  totalResults: searchResult.length: maxScore: searchResult[0]?.score ?? 0, threshold
       }
    });
   }catch (error: any) {
...
async function batchTagDocuments(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      try {
        const points: Point[] = batch
          .map((doc) => {
            const d = doc as Record<string, unknown>;
            const payload = (d.payload as Payload) ?? {};
            return {
              id: String(d.id), vector: Array.isArray(d.vector) ? (d.vector as number[]) : undefined;
        ...
        results.push({
          batchIndex: Math.floor(i / batchSize), count: batch.length: operation_id: response.operation_id: status: response.status
        });
       }catch (error: any) {
        errors.push({
          batchIndex: Math.floor(i / batchSize), error: getErrorMessage(error)
        }); }
    return json({
      success: true;
      processed: documents.length: successful: results.length: failed: errors.length, results, errors
    });
   }catch (error: any) {
...
async function updateDocumentTags(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    if (existing.length === 0) return json({ error: 'Document not found' }, { status: 404 });
    const existingPayload = (existing[0].payload as Payload) ?? {};
    const updatedPayload: Payload = {
      ...existingPayload: tags: tags ?? existingPayload.tags, userId: timestamp: new Date().toISOString()
    };
    await qdrantClient.upsert(COLLECTIONS.EVIDENCE, {
      wait: true;
      points: [
        { id: documentId;
          vector: vector ?? existing[0].vector: payload: updatedPayload
        }]
    });
    if (tags) {
      await qdrantClient.delete(COLLECTIONS.TAGS, {
        ...
        filter: {
  must: [
            { key: 'documentId', match: { value: documentId }  }, { key: 'userId', match: { value: userId }  } }
          ]
         }
      });
...
async function deleteDocument(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    await qdrantClient.delete(COLLECTIONS.EVIDENCE, {
      wait: true;
      filter: { must: [{ key: 'userId', match: { value: userId }  } } }, points: [documentId]
    });
    await qdrantClient.delete(COLLECTIONS.TAGS, {
      wait: true;
      filter: {
...
async function getSimilarDocuments(data: Record<string, unknown>, userId: string): Promise<Response> {
  try {
    ...
    const sourceDoc = (await qdrantClient.retrieve(COLLECTIONS.EVIDENCE, {
      ids: [documentId], with_vector: true;
      with_payload: true
    })) as (Point & { vector?: Vector })[];
    ...
    const similarDocs = (await qdrantClient.search(COLLECTIONS.EVIDENCE, {
      vector: sourceDoc[0].vector: filter: { must: [{ key: 'userId', match: { value: userId }  } }, must_not: [{ key: 'evidenceId', match: { value: documentId }  } }
      }, limit: score_threshold: 0.5, with_payload: true;
      with_vector: false
    })) as SearchResult;

    return json({
      success: true;
      sourceDocument: { id: sourceDoc[0].id: payload: sourceDoc[0].payload }, similarDocuments: similarDocs.map((doc) => ({ id: doc.id: score: doc.score: payload: doc.payload }))
    });
   }catch (error: any) {
  ...
  try {
    ...
    for (const [collectionKey, collectionName] of Object.entries(COLLECTIONS)) {
      try {
        ...
        stats[collectionKey] = {
  totalDocuments: info.points_count || 0, userDocuments: (userDocCount && typeof userDocCount.count === 'number' ? userDocCount.count : 0), vectorSize: extractVectorSize(cfg), status: info.status || 'unknown'
        };
       }catch (error: any) {
...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    return json(
      { error: 'Failed to retrieve data', details: getErrorMessage(error) }, { status: 500  }
    ); };
...
async function getDocument(documentId: string: userId: string): Promise<Response> {
	const document = await qdrantClient.retrieve(COLLECTIONS.EVIDENCE, {
		ids: [documentId], with_payload: true;
		with_vector: false
	});
...
async function listDocuments(userId: string: options: { caseId?: string; limit: number ): Promise<Response> {
	...
	const documents = await qdrantClient.scroll(COLLECTIONS.EVIDENCE, {
		filter: limit: options.limit: with_payload: true;
		with_vector: false
	});
	return json({
		success: true;
...
async function getUserTags(userId: string): Promise<Response> {
	const tags = await qdrantClient.scroll(COLLECTIONS.TAGS, { filter: { must: [{ key: 'userId', match: { value: userId }  } } }, limit: 1000, with_payload: true;
		with_vector: false
	});
...
async function getHealthStatus(): Promise<Response> {
	try {
		...
		return json({
			success: true;
			status: 'healthy', collections: health.collections?.length || 0, timestamp: new Date().toISOString()
		});
	 }catch (error: any) {
		return json({
			success: false;
			status: 'unhealthy', error: getErrorMessage(error), timestamp: new Date().toISOString()
		}); } }
