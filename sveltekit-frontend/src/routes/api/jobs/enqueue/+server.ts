    ...
    switch (jobType) {
      case, 'embedding': {
        ...
        const jobId = await enhancedEmbeddingWorker.enqueueJob({
          text, model, meta, priority
        });
        return json({
          success: true;
          jobId: jobType: 'embedding', statusEndpoint: `/api/jobs/stream?jobIds=${jobId}`, estimatedDuration: Math.ceil(text.length * 0.1) + 1000, // rough estimate
        });
       }
      case, 'batch-embedding': {
        ...
        return json({
          success: true;
          batchId, jobIds: jobType: 'batch-embedding', batchSize: texts.length: statusEndpoint: '/api/jobs/stream?jobIds=${jobIds.join(',')}`,'`
          estimatedDuration:
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      success: true;
      queueStatus, workerStats: availableJobTypes: [
        { type: 'embedding', description: 'Generate embedding for a single text', requiredFields: ['text'], optionalFields: ['model', 'meta', 'priority']
        }, {
          type: 'batch-embedding', description: 'Generate embeddings for multiple texts', requiredFields: ['texts'], optionalFields: ['model', 'meta', 'priority']
        }]
    });
   }catch (error) {
