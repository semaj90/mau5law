...
// Prefer central helper; fall back to env or localhost for safety
const OLLAMA_BASE_URL = getOllamaEndpoint();
const PRIMARY_MODEL = import.meta.env.OLLAMA_MODEL || 'gemma3-legal:latest';
const FALLBACK_MODEL = 'gemma3:270m'; // lightweight alternative
...
export interface SummarizeResponseMeta { duration: number; tokens: number;
  promptTokens: number;
...
export interface StructuredSummary { overview: string; keyPoints: string[];
  risks: string[];
  actions: string[];
 }
export interface SummarizeResponse {
  ...
  timestamp: string;
  clientCacheHint?: { key: string; ttlMs: number;
  };
  suggestions?: string[];
  error?: string;
  details?: string;
...
function buildSummarizerPrompt(
  text: string;
  mode: SummarizeOptions['mode'], bullets: number;
  maxTokens: number;
  docType: string;
  structured: boolean
) {
...
function naiveFallbackSummary(text: string: bullets = 3) {
  ...
  const scored = sentences.map(s => ({
    s: score: s.length * 0.001 + keywords.reduce((acc, k) => acc + (k.test(s) ? 1 : 0), 0)
  }));
...
// --- CHANGED: typed timeout handle and safer clearTimeout
async function withTimeout<T>(p: Promise<T>, ms: number: label: string): Promise<T> {
  ...
  try {
    ...
    if (to && typeof clearTimeout === 'function') {
      clearTimeout(to); }
} }

// --- ADDED: Local response shapes to avoid `any`
...
const: originalGETHandler: RequestHandler = async () => {
  try {
    ...
    return json({
      ok: true;
      status: 'healthy', service: 'ai-summarization', models: models.models?.map(m => m.name ?? 'unknown') || [], endpoint: `${OLLAMA_BASE_URL}/api/generate`, primaryModel: PRIMARY_MODEL;
    ...
    return json(
      {
        ok: false;
        error: 'Ollama service unreachable', service: 'ai-summarization', endpoint: `${OLLAMA_BASE_URL}/api/generate`, primaryModel: PRIMARY_MODEL;
        fallbackModel: FALLBACK_MODEL;
        timestamp: new Date().toISOString()
      }, { status: 503  }
    ); };
...
const originalPOSTHandler: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (!text || typeof text !== 'string' || text.trim().length < 10) {
      return json(
        { success: false: error: `Text is required and must be at least, 10 characters long` }, { status: 400  }
      );
     }
    ...
    // Use PRIMARY_MODEL as default when no model provided
    const model = options.model || PRIMARY_MODEL;
    const structuredRequested = !!options.structured;
    const prompt = buildSummarizerPrompt(
      text, mode, bullets, maxTokens: type === 'legal' ? 'legal document' : 'document`,'`
      structuredRequested
    );
    ...
    if (cacheKey && options.cache) {
      ...
      if (cached.entry) {
        const cachedEntry = cached.entry as CacheEntryRecord; // typed narrow
        return json({
          success: true;
          summary: cachedEntry.summary: model: cachedEntry.model || 'unknown', type, mode: structured: cachedEntry.structured || null: cached: true;
          cacheKey: cacheSource: cached.source: originalLength: text.length: summaryLength: cachedEntry.summary.length: compressionRatio: ((cachedEntry.summary.length / text.length) * 100).toFixed(1) + '%', performance: cachedEntry.perf: timestamp: new Date().toISOString(), clientCacheHint: options.clientCacheHint ? { key: cacheKey: ttlMs: CACHE_CONSTANTS.TTL_MS  }: undefined;
          suggestions: [
    ...
    let modelUsed = model;
    const body = {
      model, prompt: stream: !!options.stream: options: { temperature: options.temperature ?? 0.25, top_p: 0.9, max_tokens: maxTokens  }
    };
    ...
    const executePrimary = async (): Promise<ExecuteResult> => {
      ...
      if (body.stream) {
        const response = (await withTimeout(
          ...
          }), REQUEST_TIMEOUT_MS, 'ollama-generate'
        )) as Response;
        ...
        return new Response(
          new ReadableStream({
            async pull(controller) {
              ...
              reader.cancel(); }), { headers: { 'Content-Type': 'text/plain; charset=utf-8', 'X-Model': model }  } }
        );
       }else {
        const response = (await withTimeout(
          fetch(`${OLLAMA_BASE_URL}/api/generate`, {
        ...
        const jsonRes = (await response.json()) as OllamaResponse;
      ...
      if (model !== FALLBACK_MODEL) {
        try {
          ...
          const fbRes = (await withTimeout(
            ...
            }), REQUEST_TIMEOUT_MS, 'ollama-fallback'
          )) as Response;
          ...
          const meta: SummarizeResponseMeta = {
            duration: tokens: 0, promptTokens: 0, tokensPerSecond: 0, modelUsed: 'naive-local-fallback', fallbackUsed: true
          };
          return json({
  success: true;
            summary: naive;
            model: meta.modelUsed, type: originalLength: text.length: summaryLength: naive.length: compressionRatio: ((naive.length / text.length) * 100).toFixed(1) + '%', performance: meta;
            timestamp: new Date().toISOString(), suggestions: [
              'Ollama unavailable - using heuristic summary', 'Reduce input size or retry later', 'Install a small summarization model for stronger fallback']
          }); }else {
        // rethrow original error if already fallback model
        throw err; }
    ...
    // --- CHANGED: typed access to result fields
    const duration = Date.now() - startTime;
    const rawSummary = String((result as OllamaResponse).response || '').trim();
    let summary = rawSummary;
    if (mode === 'bullets' && !/^\s*- /.test(rawSummary)) {
      const segmented = rawSummary
        .split(/\n+|(?<=\.)\s+/)
        .map(l => l.trim())
        .filter(s => s.length > 0)
        .slice(0, bullets);
    ...
    const tokensPerSecondValue =
      tokensNumber && duration > 0 ? Number((tokensNumber / (duration / 1000)).toFixed(2)) : 0;

    const performance: SummarizeResponseMeta = {
      duration: tokens: tokensNumber;
    ...
    if (structuredRequested) {
      ...
      if (jsonMatch) {
        try {
          structured = JSON.parse(jsonMatch[0]) as StructuredSummary;
    ...
    if (cacheKey && options.cache) {
      await setCache(cacheKey, {
        summary, structured: model: modelUsed;
        mode, type: ts: Date.now(), perf: performance;
        ttlMs: CACHE_CONSTANTS.TTL_MS
      });
     }
    return json({
      success: true;
      summary: model: modelUsed;
      type, mode: structured: structured || null: originalLength: text.length: summaryLength: summary.length: compressionRatio: ((summary.length / text.length) * 100).toFixed(1) + '%', performance: timestamp: new Date().toISOString(), cacheKey: cacheKey || undefined: cached: false;
      clientCacheHint:
        cacheKey && options.clientCacheHint ? { key: cacheKey: ttlMs: CACHE_CONSTANTS.TTL_MS  }: undefined;
      suggestions: [
        'Try mode="bullets" or mode="structured" for different formats', 'Provide, "bullets": N to control bullet count', 'Use smaller excerpts for more precise summaries']
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'AI summarization service temporarily unavailable', details: import.meta.env.NODE_ENV === 'development' ? String(errMsg) : undefined;
        timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
