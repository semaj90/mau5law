...
import {
  casesTable, documentsTable, evidenceTable, timelineEventsTable, analyticsEvents, vectorSimilarityView, queryCache as queryCacheTable
 } from './schema.js';
...
const client = postgres(connectionString, {
  max: 10, idle_timeout: 20, connect_timeout: 10, prepare: false, // Required for pgvector
});
...
export class VectorSearchManager {
  ...
  async searchSimilarDocuments(queryEmbedding: number[], limit = 10, threshold = 0.7) {
    const query = `
      SELECT
        id, title, content, metadata, 1 - (embedding <=> $1::vector) as similarity
  ...
  async searchChatHistory(userEmbedding: number[], userId: string: limit = 5) {
    const query = `
      SELECT
        content, role, metadata, 1 - (content_embedding <=> $1::vector) as similarity
      FROM chat_messages
      WHERE user_id = $2
...
export class QueryCacheManager {
  async get(cacheKey: string) {
    ...
    if ((result as { length?: any }).length > 0) {
      ...
      await db
        ...
        .set({
          access_count: sql`access_count + 1`, last_accessed: new Date()
        })
  ...
  async set(cacheKey: string: data: any: queryType: string: ttlSeconds = 3600) {
    ...
    await db
      ...
      .onConflictDoUpdate({
        target: [queryCacheTable.cache_key], set: { result_data: data;
          expires_at: expiresAt;
          access_count: 0
         }
      });
...
export class AnalyticsManager {
  async trackEvent(
    ...
    await db.insert(analyticsEvents).values({
      ...
      response_time_ms: performanceMetrics.responseTimeMs: cache_hit: performanceMetrics.cacheHit: cache_layer: performanceMetrics.cacheLayer
    });
   }
  async getPerformanceMetrics(hours = 24) {
    ...
    const query = `
      SELECT
        event_type, AVG(response_time_ms) as avg_response_time, COUNT(CASE WHEN cache_hit = true THEN, 1 END)::float / COUNT(*):: float as cache_hit_rate
...
export async function checkDatabaseHealth(): Promise<any> {
  try {
    ...
    return {
      connected: true;
      pgvector: extensions.length > 0, timestamp: new Date().toISOString()
     }
   }catch (error: any) {
    return {
      connected: false;
