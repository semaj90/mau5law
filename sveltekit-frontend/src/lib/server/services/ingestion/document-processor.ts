...
export function splitSentences(text: string): string[] {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if ((char === '.' || char === '?' || char === '!') && nextChar === ' ') {
      ...
      const isAbbreviation = /^[A-Z]\.?$/.test(lastWord) ||
                            /\b(U\.S|Cal|F\.\d|App|Ct|Inc|Ltd|Co|Dr|Mr|Ms|Mrs|Prof|Rev|St|Ave|Blvd|Dept|Div|Sec|Supp|Cir|Dist|Ct|App|Ct|Supp|Ct|Ct|Ct)\.?$/.test(lastWord);
...
export function chunkDocument(
  ...
  while (startIdx < tokens.length) {
    ...
    const endCharIdx = text.lastIndexOf(chunkTokens[chunkTokens.length - 1]) +
...
export function extractKeywords(text: string): string[] {
  // Legal keywords
  const legalKeywords = [
    'child abuse', 'neglect', 'dependency', 'restitution', 'custody',
    'visitation', 'guardianship', 'adoption', 'termination', 'parental rights',
    'best interests', 'clear and convincing', 'preponderance', 'beyond reasonable doubt',
