...
interface OCRResultData { text: string; confidence: number;
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    for (let pageNum = 1; pageNum <= pdf.numPages; pageNum++) {
      ...
      if (textContent.items.length > 0) {
        ...
        const worker = await createWorker({
          ...
          workerPath: '/tesseract-worker.js', corePath: `/tesseract-core.js` });
        ...
        ocrResults.push({
          page: pageNum;
          text: ocrData.text: confidence: ocrData.confidence: method: `ocr` });
    ...
    const result = {
      filename: file.name: pages: pdf.numPages, totalCharacters: averageConfidence: Math.round(averageConfidence), text: allText;
      pageResults: ocrResults;
      legalConcepts, citations: extractedAt: new Date().toISOString(), processingStats: {
  ocrPages: ocrResults.filter(item => item.length), extractedPages: ocrResults.filter(item => item.length)
       }
    };
    return json(result);
   }catch (err: any) {
...
function extractLegalConcepts(text: string): string[] {
  ...
  const patterns = [
    // Contract terms, /(?:breach\s+of\s+contract|contract\s+formation|consideration|offer\s+and\s+acceptance)/gi, // Liability terms
    /(?:negligence|liability|damages|indemnification|limitation\s+of\s+liability)/gi, // Intellectual property
    /(?:copyright|trademark|patent|intellectual\s+property|trade\s+secret)/gi, // Corporate terms
    /(?:shareholder|board\s+of\s+directors|merger|acquisition|corporate\s+governance)/gi, // Legal procedures
...
function extractCitations(text: string): string[] {
  ...
  // Citation patterns
  const patterns = [
    // Federal courts, /\b\d+\s+F\.\s*(?:2d|3d)\s+\d+/g, // Supreme Court
    /\b\d+\s+U\.S\.\s+\d+/g, // State courts
    /\b\d+\s+[A-Z][a-z]*\s+(?:2d|3d)?\s*\d+/g, // Statutes
    /\b\d+\s+U\.S\.C\.\s*§?\s*\d+/g, // Code of Federal Regulations
    /\b\d+\s+C\.F\.R\.\s*§?\s*\d+/g];
  patterns.forEach(pattern => {
    const matches = text.match(pattern);
    if (matches) {
