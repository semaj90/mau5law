...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
  async function uploadFile(file: File): Promise<any> { uploading = true; progress = 0; error = null; result = null; try { const formData = new FormData(); formData.append('file', file); // Simulate progress const progressInterval = setInterval(() => { progress = Math.min(progress + 10, 90)},
	200); const response = await fetch('/api/integrated/upload', { method: 'POST';
...
'
  ...
  async function searchDocuments(): Promise<any> { if (!searchQuery.trim()) return; searching = true; searchResults = []; try { const response = await fetch('/api/integrated/search', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	query: searchQuery, limit: 5 }) }); if (!response.ok) { throw new Error('Search failed')}
  ...
  function handleClick() { fileInput?.click()}
    ...
    /> </div> {#if result} <div class="result-section"> <h4>âœ… Upload Successful</h4> <div class="result-details"> <p><strong>File:</strong> {result.document.filename}</p> <p><strong>Chunks:</strong> {result.document.chunks}</p> <p><strong>Document; ID:</strong> {result.document.id}</p> <p> <strong>Storage:</strong> {result.document.qdrantStored ? 'âœ“ Qdrant': ''} âœ“ pgvector âœ“ MinIO </p> </div> {#if result.recommendations?.length > 0} <div class="recommendations"> <h5>ðŸ“Š Similar Documents Found:</h5> {#each Array.isArray(result.recommendations) ? result.recommendations: [] as rec} <div class="recommendation-item"> <span class="similarity">{Math.round(rec.similarity * 100)}%</span> <span class="content">{rec.content}</span> </div> {/each} {/if} {/if} {#if error} <div class="result-section"> <h4>âŒ Error</h4> <p>{ error }</p> {/if} <div class="search-section"> <h3>ðŸ” Semantic Search</h3> <div class="search-input-group"> <input type="text"
        ...
        onkeydown={e => e.key === 'Enter' && searchDocuments()} /> <button onclick={ searchDocuments } disabled={searching || !searchQuery.trim()}> {searching ? 'Searching...': 'Search'} </button> </div> {#if searchResults.length > 0} <div class="search-results"> <h5>Results ({searchResults.length}):</h5> {#each Array.isArray(searchResults) ? searchResults: [] as result} <div class="search-result-item"> <div class="result-header"> <span class="similarity-badge">{Math.round(result.similarity * 100)}%</span> {#if result.source} <span class="source">{result.source}</span> {/if} </div> <p class="result-content">{result.content}</p> </div> {/each} {/if} </div> </div> <style> .integrated-rag-upload { display: flex; flex-direction: column;
	gap: 2rem; max-width: 800px;
	margin: 0 auto;padding: 2rem}
  .upload-section, .search-section { background: #f8f9fa; border-radius: 8px;
	padding: 1.5rem}
  ...
  .result-section { border-radius: 8px;
	padding: 1.5rem}
  .result-section.success { background: #d4edda;
	border: 1px solid #c3e6cb}
  .result-section.error { background: #f8d7da;
  ...
  .recommendation-item { display: flex;
	...
	margin: 0.5rem 0;background: white; border-radius: 4px}
  ...
  .search-input-group button { padding: 0.75rem 1.5rem; background: #007bff;color: white;
	border: none; border-radius: 4px;
	cursor: pointer; font-weight: 600}
  .search-input-group buttondisabled { opacity: 0.5;
	cursor:not-allowed}
  .search-results { margin-top: 1.5rem}
  .search-result-item { background: white;
	padding: 1rem; margin: 0.75rem 0; border-radius: 6px; border-left: 4px solid #007bff}
  .result-header { display: flex;
	gap: 1rem; align-items: center; margin-bottom: 0.5rem}
  .similarity-badge { background: #007bff;
	color: white; padding: 0.25rem 0.5rem; border-radius: 4px; font-size: 0.875rem; font-weight: 600}
