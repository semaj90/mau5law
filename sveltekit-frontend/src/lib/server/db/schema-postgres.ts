...
export const users = pgTable(
  'users',
  {
    ...
    role: userRoleEnum('role').default('prosecutor').notNull(),
...
export const cases = pgTable(
  'cases',
  {
    ...
    status: varchar('status', { length: 50 }).notNull(),  // case_status enum (stored as varchar)
  },
  table => ({
    indexes: [
...
export const legalDocuments = pgTable(
  'legal_documents',
  {
    ...
    attorneys: jsonb('attorneys').default({}),
  ...
  table => [
    ...
    foreignKey({
      ...
      name: `legal_documents_evidence_id_evidence_id_fk` }).onDelete('cascade'),
    foreignKey({
...
export const citations = pgTable(
  ...
  table => [
    ...
    foreignKey({
      ...
      name: `citations_document_id_legal_documents_id_fk` }).onDelete('cascade'),
    foreignKey({
      columns: [table.createdBy],
      foreignColumns: [users.id],
...
export const legalAnalysisSessions = pgTable(
  ...
  table => [
    ...
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
      name: `legal_analysis_sessions_user_id_users_id_fk` }).onDelete('cascade')
  ]
);
// === LEGAL RESEARCH ===
...
export const userAiQueries = pgTable(
  ...
  table => [
    ...
    foreignKey({
      columns: [table.caseId],
      foreignColumns: [cases.id],
      name: `user_ai_queries_case_id_cases_id_fk` }).onDelete('cascade')
  ]
);
// === AUTO-TAGGING SYSTEM ===
...
export const conversationSessions = pgTable(
  ...
  table => [
    ...
    foreignKey({
      columns: [table.userId],
      foreignColumns: [users.id],
      name: `conversation_sessions_user_id_users_id_fk` }).onDelete('cascade'),
    foreignKey({
      columns: [table.caseId],
      foreignColumns: [cases.id],
...
export const extractedEntities = pgTable(
  ...
  table => [
    ...
    foreignKey({
      ...
      name: `extracted_entities_turn_id_turns_id_fk` }).onDelete('cascade'),
