...
class StackAnalyzer {
     ...
     */
    async analyzePackageJson(stack) {
        try {
            ...
            if (fs.existsSync(packageJsonPath)) {
                ...
                if (allDeps['react'] || allDeps['@types/react'])
                    stack.frontend.push('react');
                if (allDeps['vue'] || allDeps['@vue/cli'])
                    stack.frontend.push('vue');
                if (allDeps['svelte'] || allDeps['@sveltejs/kit'])
                    stack.frontend.push('svelte');
                if (allDeps['@angular/core'])
                    stack.frontend.push('angular');
                if (allDeps['react-native'])
                    stack.mobile.push('react-native');
                if (allDeps['flutter'])
                    stack.mobile.push('flutter');
                // Backend
                if (allDeps['express'] || allDeps['fastify'] || allDeps['koa'])
                    stack.backend.push('node');
                // Databases
                ...
                if (allDeps['neo4j-driver'])
                    stack.databases.push('neo4j');
                // AI/ML
                if (allDeps['@tensorflow/tfjs'] || allDeps['tensorflow'])
                    stack.aiml.push('tensorflow');
                if (allDeps['torch'] || allDeps['pytorch'])
                    stack.aiml.push('pytorch');
                if (allDeps['opencv-js'] || allDeps['opencv4nodejs'])
                    stack.aiml.push('opencv');
                if (allDeps['@huggingface/transformers'])
                    stack.aiml.push('transformers');
                if (allDeps['langchain'])
                    stack.aiml.push('langchain');
                if (allDeps['ollama'])
                    stack.aiml.push('ollama');
                // Cloud & DevOps
                if (allDeps['aws-sdk'] || allDeps['@aws-sdk/client-s3'])
                    stack.cloud.push('aws');
                if (allDeps['@google-cloud/storage'])
                    stack.cloud.push('gcp');
                if (allDeps['@azure/storage-blob'])
                    stack.cloud.push('azure');
                // Scientific Computing
                if (allDeps['numpy'] || allDeps['pandas'])
                    stack.scientific.push('numpy');
                if (allDeps['jupyter'])
                    stack.scientific.push('jupyter');
                // Web3/Blockchain
                if (allDeps['ethers'] || allDeps['web3'])
                    stack.web3.push('ethereum');
                if (allDeps['hardhat'])
                    stack.web3.push('hardhat');
                if (allDeps['truffle'])
                    stack.web3.push('truffle');
                // Gaming
                if (allDeps['three'] || allDeps['babylonjs'])
                    stack.gaming.push('opengl');
            }
        }
        catch (error) {
     ...
     */
    async analyzeSourceFiles(stack) {
        ...
        for (const file of workspaceFiles) {
            ...
            switch (extension) {
                case '.py':
                    ...
                    break;
                case '.rs':
                    if (!stack.backend.includes('rust'))
                        stack.backend.push('rust');
                    break;
                case '.go':
                    if (!stack.backend.includes('go'))
                        stack.backend.push('go');
                    break;
                case '.java':
                    if (!stack.backend.includes('java'))
                        stack.backend.push('java');
                    break;
                case '.cpp':
                case '.cc':
                case '.cxx':
                    if (!stack.backend.includes('cpp'))
                        stack.backend.push('cpp');
                    await this.analyzeCppFile(filePath, stack);
                    break;
                case '.c':
                    if (!stack.backend.includes('c'))
                        stack.backend.push('c');
                    await this.analyzeCFile(filePath, stack);
                    break;
                case '.cs':
                    if (!stack.backend.includes('csharp'))
                        stack.backend.push('csharp');
                    break;
                case '.cu':
                case '.cuh':
                    ...
                    break;
                case '.cl':
                    if (!stack.gpu.includes('opencl'))
                        stack.gpu.push('opencl');
                    break;
                case '.v':
                    if (!stack.embedded.includes('verilog'))
                        stack.embedded.push('verilog');
                    break;
                case '.vhd':
                case '.vhdl':
                    if (!stack.embedded.includes('vhdl'))
                        stack.embedded.push('vhdl');
                    break;
                case '.ino':
                    if (!stack.embedded.includes('arduino'))
                        stack.embedded.push('arduino');
                    break;
                case '.m':
                    if (fileName.includes('matlab') || this.containsMatlabSyntax(filePath)) {
                        if (!stack.scientific.includes('matlab'))
                            stack.scientific.push('matlab');
                    }
                    break;
                case '.r':
                    if (!stack.scientific.includes('r-lang'))
                        stack.scientific.push('r-lang');
                    break;
                case '.ipynb':
                    if (!stack.scientific.includes('jupyter'))
                        stack.scientific.push('jupyter');
                    break;
                case '.sol':
                    if (!stack.web3.includes('solidity'))
                        stack.web3.push('solidity');
                    break;
                case '.unity':
                case '.unitypackage':
                    if (!stack.gaming.includes('unity'))
                        stack.gaming.push('unity');
                    break;
                case '.uproject':
                    if (!stack.gaming.includes('unreal'))
                        stack.gaming.push('unreal');
                    break;
            }
            // Special file name patterns
            if (fileName.includes('dockerfile')) {
                if (!stack.cloud.includes('docker'))
                    stack.cloud.push('docker');
            }
            if (fileName.includes('makefile') || fileName === 'cmake') {
                // Could indicate systems programming
                if (!stack.systems.includes('bare-metal'))
                    stack.systems.push('bare-metal');
            }
        }
    }
    /**
     ...
     */
    async analyzePythonFile(filePath, stack) {
        try {
            ...
            if (content.includes('import cv2') || content.includes('import opencv')) {
                if (!stack.aiml.includes('opencv'))
                    stack.aiml.push('opencv');
            }
            if (content.includes('from transformers')) {
                if (!stack.aiml.includes('transformers'))
                    stack.aiml.push('transformers');
            }
            // Scientific computing
            ...
            if (content.includes('import pandas') || content.includes('import pd')) {
                if (!stack.scientific.includes('pandas'))
                    stack.scientific.push('pandas');
            }
            if (content.includes('import scipy')) {
                if (!stack.scientific.includes('scipy'))
                    stack.scientific.push('scipy');
            }
            // GPU computing
            if (content.includes('import cupy') || content.includes('from cupy')) {
                if (!stack.gpu.includes('cupy'))
                    stack.gpu.push('cupy');
            }
            if (content.includes('import numba') || content.includes('@numba.jit')) {
                if (!stack.gpu.includes('numba'))
                    stack.gpu.push('numba');
            }
     ...
     */
    async analyzeCppFile(filePath, stack) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            // CUDA includes
            if (content.includes('#include <cuda') || content.includes('#include <thrust') ||
                ...
                if (!stack.gpu.includes('nvidia-toolkit'))
                    stack.gpu.push('nvidia-toolkit');
            }
            // Thrust library
            if (content.includes('#include <thrust')) {
                if (!stack.gpu.includes('thrust'))
                    stack.gpu.push('thrust');
            }
            // OpenCL
            ...
            if (content.includes('#include <GL/gl.h>') || content.includes('opengl')) {
                if (!stack.gaming.includes('opengl'))
                    stack.gaming.push('opengl');
            }
            if (content.includes('#include <vulkan') || content.includes('VkInstance')) {
                if (!stack.gaming.includes('vulkan'))
                    stack.gaming.push('vulkan');
            }
     ...
     */
    async analyzeCFile(filePath, stack) {
        try {
            const content = fs.readFileSync(filePath, 'utf8');
            // Embedded indicators
     ...
     */
    async analyzeConfigFiles(stack) {
        const configFiles = [
        ...
        for (const configFile of configFiles) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                if (configFile.includes('kubernetes') || configFile.includes('k8s')) {
                    if (!stack.cloud.includes('kubernetes'))
                        stack.cloud.push('kubernetes');
                }
                if (configFile.includes('terraform')) {
                    if (!stack.cloud.includes('terraform'))
                        stack.cloud.push('terraform');
                }
                if (configFile.includes('ansible')) {
                    if (!stack.cloud.includes('ansible'))
                        stack.cloud.push('ansible');
                }
     ...
     */
    analyzePythonRequirements(content, stack) {
        ...
        for (const buildFile of buildFiles) {
            ...
            if (fs.existsSync(filePath)) {
                ...
                if (buildFile === 'pubspec.yaml') {
                    if (!stack.mobile.includes('flutter'))
                        stack.mobile.push('flutter');
                }
        ...
        for (const indicator of cudaIndicators) {
            ...
            if (files.length > 0) {
                ...
                if (indicator.includes('tensorrt')) {
                    if (!stack.gpu.includes('tensorrt'))
                        stack.gpu.push('tensorrt');
                }
            }
        }
    }
     ...
     */
    async analyzeEmbeddedFiles(stack) {
        // Arduino files
        ...
        for (const indicator of embeddedLinuxFiles) {
            const files = await vscode.workspace.findFiles(`**/*${indicator}*`, '**/node_modules/**', 10);
            if (files.length > 0) {
                if (!stack.systems.includes('embedded-linux'))
                    stack.systems.push('embedded-linux');
            }
        }
    }
    /**
    ...
    determineProjectType(stack) {
        ...
        if (stack.frontend.length > 0 && stack.backend.length > 0) {
            if (stack.frontend.includes('react'))
                return 'react-nextjs';
            if (stack.frontend.includes('vue'))
                return 'vue-nuxt';
...
exports.StackAnalyzer = StackAnalyzer;
//# sourceMappingURL=stackAnalyzer.js.map