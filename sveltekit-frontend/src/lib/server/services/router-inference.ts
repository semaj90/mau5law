/**
 * Lightweight router inference runtime.
 * Tries to call a local ML microservice (QLoRA/PyTorch adapter) at ROUTER_INFERENCE_URL.
 * If unavailable or the response is invalid, falls back to a conservative heuristic.
 */
...
function validateDecision(obj: unknown): obj is RouterDecision {
  ...
  return (
    typeof o.useGPU === 'boolean' &&
    typeof o.useQUIC === 'boolean' &&
...
export async function predictWithRouter(features: RouterFeatures): Promise<RouterDecision> {
  ...
  try {
    ...
    const gpuLoad = Number(features.gpuLoad ?? 0);
    const cpuLoad = Number(features.cpuLoad ?? 0);
    const rabbitDepth = Number(features.rabbitDepth ?? 0);
    const textLength = Number(features.textLength ?? 0);

    const useGPU = gpuLoad < 0.6 && fileSize > 256 * 1024 && cpuLoad > 1.5;
    const useCache = rabbitDepth > 50 || cpuLoad > 4;
    const useQUIC = (textLength > 4000 && cpuLoad < 2) || gpuLoad > 0.7;
