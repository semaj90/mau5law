...
const mockOllamaFetch = vi.fn(async () =>
	new Response(JSON.stringify({
		...
		response: '{"synthesis":{"mainThemes":["Theme1"],"supportingEvidence":[],"gaps":[],"contradictions":[],"legalImplications":[],"nextSteps":[]}}',
...
vi.mock('$lib/server/db/client', () => ({
	db: {
		select: vi.fn(() => mockChain),
		execute: vi.fn(async () => ({ rows: [{ version: 'PostgreSQL 16', current_database: 'deeds' }] })),
		insert: vi.fn(() => ({
			values: vi.fn(() => {
...
vi.mock('$lib/server/db/schema-postgres.js', () => ({
	documents: { id: 'id', title: 'title', content: 'content', fileType: 'file_type', status: 'status', summary: 'summary', metadata: 'metadata', createdAt: 'created_at', updatedAt: 'updated_at' },
...
vi.mock('$lib/server/db/schema.js', () => ({
	documents: { id: 'id', title: 'title', content: 'content', fileType: 'file_type', status: 'status', updatedAt: 'updated_at' },
	...
	it('returns analysis results on success', async () => {
		...
		const event = makeEvent('POST', '/api/summarize/analyze', {
			body: {
				documentId: 'doc-xyz',
				sections: [
					{ title: 'Facts', content: 'Key facts of the case.' },
				],
			},
		});
		const res = await POST(event as any);
