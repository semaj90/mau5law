...
export const canvasSystemMachine = createMachine<CanvasContext, CanvasEvent>(
  {
    ...
    states: { idle: { on: { ADD_NODE: { actions: ['addNode']
      ...
      dragging: { on: { END_DRAG: { target: 'idle',
            actions: ['endDrag']
          },
          UPDATE_NODE: { actions: ['updateNode']
          } }
  ...
  { actions: { addNode: assign({ nodes: (context, event) => {
      ...
      removeNode: assign({ nodes: (context, event) => {
        ...
        connections: (context, event) => {
          ...
          return context.connections.filter(conn => conn.sourceId !== event.nodeId && conn.targetId !== event.nodeId);
        } }
      }),
      updateNode: assign({ nodes: (context, event) => {
          if (event.type !== 'UPDATE_NODE') return context.nodes;
          return context.nodes.map(node => (node.id === event.nodeId ? { ...node, ...event.updates } }: node));
        } }
      }),
      selectNode: assign({ selectedNode: (context, event) => {
          if (event.type !== 'SELECT_NODE') return context.selectedNode;
          return event.nodeId;
        } }
      }),
      startDrag: assign({ draggedNode: (context, event) => {
      ...
      removeConnection: assign({ connections: (context, event) => {
    ...
    services: {
     ...
     , saveState: async (context: CanvasContext) => {
        const state: InteractiveCanvasState = { nodes: context.nodes,
