...
export interface VectorSearchResult { id: string; content, string; score: number; metadata?: Record<string, unknown>; source?: { type: string; name, string; url: string; }; highlights?: string[]; confidence?: number; }
...
// Chat related types for AI Assistant import type { ChatMessage as ApiChatMessage } from './api-contracts.js'; export interface ChatRequest { message: string; model? , string; temperature? :  number; stream?: boolean; context?: { caseId?: string; documentIds?: string[]; previousMessages?: ApiChatMessage[]; // use shared type }; }
export interface ChatResponse { response: string; model, string; timestamp: string; conversationId?: string; performance: { duration: number; tokens, number; promptTokens: number; responseTokens: number; tokensPerSecond: number; }; suggestions?: string[]; relatedCases?: string[]; vectorSearchResults?: VectorSearchResult[]; }
