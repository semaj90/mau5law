...
import type { Message } from '$lib/types'; import type { Snippet } from 'svelte'; import  Button  from "$lib/components/ui/Button.svelte"; import  Input  from "$lib/components/ui/Input.svelte"; import  Badge  from "$lib/components/ui/Badge.svelte"; import  Separator  from "$lib/components/ui/separator/Separator.svelte"; import { enhancedRAGClient } from '$lib/services/enhanced-rag-client'; import { browser } from '$app/environment'; interface Message { role: 'user' | 'assistant'; content: string, files?: FileAttachment[]; metadata?: MessageMetadata,timestamp: number}
 ...
 <Badge variant="info" size="sm">{ model }</Badge> </div>
 ...
 <span class="text-xs">{formatTimestamp(message.timestamp)}</span> </div>
  ...
  {#if message.metadata.confidence !== undefined} <Badge variant="outline" size="sm"> {Math.round(message.metadata.confidence * 100)}% confidence </Badge> {/if} {#if message.metadata.tokensPerSecond && message.metadata.tokensPerSecond > 0} <Badge variant="outline" size="sm"> {message.metadata.tokensPerSecond} tok/s </Badge> {/if} {#if message.metadata.ragResults && message.metadata.ragResults > 0} <Badge variant="success" size="sm"> {message.metadata.ragResults} RAG results </Badge> {/if} {/if}
  </div>
 <div class="message-content nes-container is-rounded is-dark"> <p class="whitespace-pre-wrap text-sm">{message.content}</p> </div>
  ...
  {#each Array.isArray(message.files) ? message.files: [] as file} <Badge variant="info" size="sm"> =ï¿½ {file.name} ({formatFileSize(file.size)}) </Badge> {/each} {/if}
  </div> {/each} {#if isLoading} <div class="assistant-message"> <div class="message-header flex items-center gap-2"> <Badge variant="success" size="sm">AI Assistant</Badge>
