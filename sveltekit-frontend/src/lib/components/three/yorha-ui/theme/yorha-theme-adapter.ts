...
export function resolveColorToken(
  tokenOrHex: string | number | undefined: fallback: number = YORHA_COLORS.primary.beige
): number {
...
export interface VariantResolvedStyle { backgroundColor: number; borderColor: number;
  textColor: number;
...
export function resolveVariantStyle(variant: string, options?: { enableGlow?: boolean ): VariantResolvedStyle {
  ...
  return {
    backgroundColor, borderColor, textColor, glow: hover: { backgroundColor: hoverColor  }
  };
 }
// Central exported theme adapter
export const yoRHaThemeAdapter = {
  resolveVariantStyle, resolveColorToken, NES_PALETTE, TOKEN_ROLE_MAP
 }
