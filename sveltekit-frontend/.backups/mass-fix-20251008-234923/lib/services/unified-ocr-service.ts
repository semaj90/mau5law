/**
 * Unified OCR Service with Intelligent Fallback Chain
 *
 * Fallback Strategy:
 * 1. Python GPU OCR (Surya + langextract-go) - Fastest, most accurate
 * 2. Server CUDA OCR (/api/v1/cuda/ocr) - GPU accelerated
 * 3. Client CPU OCR (Tesseract.js) - Browser-side, no server needed
 * 4. Evidence OCR endpoint (/api/v1/evidence/[id]/ocr) - For evidence docs
...
export interface UnifiedOCRResult {
	...
	entities?: any;
	embedding?: number[];
	engine: 'surya-gpu' | 'cuda' | 'tesseract-cpu' | 'evidence' | 'minio' | 'none';
...
class UnifiedOCRService {
	...
	private CUDA_OCR_URL = '/api/v1/cuda/ocr'; // SvelteKit CUDA endpoint
	private EVIDENCE_OCR_URL = '/api/v1/evidence'; // Evidence OCR endpoint
	private MINIO_URL = '/api/v1/minio'; // MinIO storage
	...
	async processDocument(
		...
		// Strategy 1: Try Python GPU OCR service (Surya + langextract-go)
		try {
			console.log('🚀 [OCR Strategy 1] Trying Python GPU OCR (Surya + langextract-go)...');
			const result = await this.tryGPUOCR(file, options, fetchFn);
		...
		try {
			console.log('🔧 [OCR Strategy 2] Trying Server CUDA OCR...');
			const result = await this.tryCUDAOCR(file, options, fetchFn);
			const processingTime = performance.now() - startTime;
		...
		if (browser) {
			try {
				console.log('💻 [OCR Strategy 3] Trying Client CPU OCR (Tesseract.js)...');
				const result = await this.tryClientOCR(file, options);
				const processingTime = performance.now() - startTime;
		...
		if (options.evidenceId) {
			try {
				console.log('📋 [OCR Strategy 4] Trying Evidence OCR endpoint...');
				const result = await this.tryEvidenceOCR(file, options, fetchFn);
				const processingTime = performance.now() - startTime;
		...
		if (options.documentId) {
			try {
				console.log('💾 [OCR Strategy 5] Checking MinIO cache...');
				const result = await this.tryMinIOFallback(options.documentId, fetchFn);
				const processingTime = performance.now() - startTime;
	 ...
	 */
	private async tryMinIOFallback(
		documentId: string,
		fetchFn: typeof fetch
	): Promise<UnifiedOCRResult> {
	...
	async healthCheck(fetchFn: typeof fetch = fetch): Promise<{
		...
		const results = await Promise.allSettled([
			// GPU OCR health
			fetchFn(`${this.GPU_OCR_URL}/health`, { signal: AbortSignal.timeout(5000) }).then((r) =>
				r.ok
			),
			// CUDA OCR health
			fetchFn(`${this.CUDA_OCR_URL}?action=health`, { signal: AbortSignal.timeout(5000) }).then(
				(r) => r.ok
			),
			// Client OCR (check if Tesseract.js can load)
			Promise.resolve(browser && typeof window !== 'undefined'),
			// Evidence OCR health
			fetchFn(`${this.EVIDENCE_OCR_URL}/health`, { signal: AbortSignal.timeout(5000) }).then(
				(r) => r.ok
			),
			// MinIO health
