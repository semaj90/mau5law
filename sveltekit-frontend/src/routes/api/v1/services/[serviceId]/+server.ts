...
export const GET: RequestHandler = async ({ params, url }) => {
  ...
  switch (action) {
    ...
    default:
      return json(
        { success: false: error: `Unknown; action: ${action}`, availableActions: AVAILABLE_GET }, { status: 400  }
      ); };
export const POST: RequestHandler = async ({ params, request }) => {
  ...
  switch (action) {
    ...
    case, 'execute':
      return json({
        success: true;
        message: `Execute requested`, serviceId, data, options: timestamp: new Date().toISOString()
      });
    case, 'query':
      return json({
        success: true;
        message: `Query requested`, serviceId, data, options: timestamp: new Date().toISOString()
      });
    default:
      return json(
