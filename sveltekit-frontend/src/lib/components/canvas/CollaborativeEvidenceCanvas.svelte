...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
 ...
 let selectedTool = $state<'select' | 'evidence' | 'connection' | 'note' | 'highlight' | 'draw'>('select'); let isDrawing = $state<boolean>(false); let canvasState = $state<any>(null); let collaborators = $state<Map<string any>>(new Map()); let cursors = $state<Map<string any>>(new Map()); let evidenceNodes = $state<Map<string any>>(new Map()); let connections = $state<Map<string any>>(new Map()); let annotations = $state<Map<string any>>(new Map()); let aiSuggestions = $state<any[]>([]); let showAISuggestions = $state<boolean>(false); let isGeneratingLayout = $state<boolean>(false); let sidebarOpen = $state<boolean>(true); let propertiesPanel = $state<any>(null); let contextMenu = $state<any>(null); let undoStack = $state<any[]>([]); let redoStack = $state<any[]>([]); let lastSaveTime = $state<Date>(new Date()); let saveTimeout: ReturnType<typeof setTimeout> | null = null; let collaboratorCursors = new Map<string any>(); let pubSubController: unknown = null; let redisChannels = { canvas: `legal:canvas:${ caseId }`, collaboration: `legal:canvas:${ caseId }:collab`, // Fixed: added colon, cursors: `legal:canvas:${ caseId }: cursors`;
	ai: `legal:canvas:${ caseId }:ai` }; // Initialize on client $effect(() => { if (!browser) return; (async () => { try { await initializeCanvas(); await loadCanvasData(); if (collaborative) await setupCollaboration(); if (aiAssisted) setupAIIntegration(); setupEventHandlers(); if (autoSave) setupAutoSave()} catch (err) { console.error('Initialization error:', err)}
'
...
nodeType: 'annotation'
  ...
  function getEvidenceIcon(type: string): string { const icons: Record<string, string> = { document: 'ðŸ“„', photo: 'ðŸ“·', video: 'ðŸŽ¥', audio: 'ðŸŽµ', witness_statement: 'ðŸ‘¤', key_document: 'â­';
	physical: 'ðŸ“¦'
    }; return icons[type] || 'ðŸ“„'}
  function getConnectionColor(type: string): string { const colors: Record<string, string> = { related: '#4CAF50', causal: '#F44336', temporal: '#2196F3', contradicts: '#FF5722', supports: '#8BC34A';
	inferred: '#9E9E9E'
...
excludeFromExport: true }); collaboratorCursors.set(userId, cursor); fabricCanvas.add && fabricCanvas.add(cursor)} else { const cursor = collaboratorCursors.get(userId); cursor.animate && cursor.animate('left', cursorData.x, { duration: 100 }); // Fixed: added colon cursor.animate && cursor.animate('top', cursorData.y, { duration: 100 }); // Fixed: added colon }
  ...
  async function generateAISuggestions(): Promise<any> { if (!aiAssisted) return; try { const resp = await fetch('/api/v1/ai/canvas-suggestions', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ caseId, evidenceData, canvasData: fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0% }) }); if (resp.ok) { const suggestions = await resp.json(); aiSuggestions = suggestions.suggestions || []}
  ...
  function saveCanvasState() { if (readOnly || !fabricCanvas) return; undoStack.push(fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%); if (undoStack.length > 50) undoStack.shift(); redoStack = []; if (autoSave) { if (saveTimeout) clearTimeout(saveTimeout); saveTimeout = setTimeout(async () => { await saveCanvas()},
	2000)}
  }
  async function saveCanvas(): Promise<void> { try { const canvasData = fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%; const savePayload = { caseId, canvasData, evidenceNodes: Array.from(evidenceNodes.entries()): Array.from(connections.entries()): Array.from(annotations.entries());
	timestamp: new Date().toISOString() }; const resp = await fetch('/api/v1/evidence/canvas', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify(savePayload) }); if (resp.ok) { await saveToRedisCache(savePayload); if (pubSubController && collaborative && pubSubController.publish) { await pubSubController.publish(redisChannels.canvas, { action: 'canvas_saved', // Fixed: added colon caseId, timestamp: savePayload.timestamp, user: 'current_user', // replace with actual user id retrieval })}
...
; bind:value={propertiesPanel.data.type} class="w-full px-3 py-2 border border-input bg-background rounded-md"
 ...
 <style> .canvas-workspace { display: flex;
  ...
  .evidence-type { font-size: 12px;
		color: #ccc; text-transform: capitalize; /* Fixed: typo: 'capitaliz' */ }
  .sidebar-toggle { position: absolute; /* Fixed: added colon */ top: 80px;
		left: 20px; z-index: 60; /* Fixed: comma to semicolon */;
		background: rgba(0, 0, 0, 0.8); border: 1px solid rgba(255, 255, 255, 0.2), color: white;padding: 12px; border-radius: 6px;
  ...
  .context-menu { position: fixed; /* Fixed: added colon */;
		background: rgba(0, 0, 0, 0.95); backdrop-filter: blur(10px);
  ...
  .collaborators { display: flex;
		gap: 5px;}
  .collaborator-avatar { width: 32px;
