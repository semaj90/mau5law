...
interface GemmaHealthResult { success: boolean; available: boolean;
  model?: string;
  version?: string;
  error?: string;
 }
interface GemmaModelInfo {
  success: boolean;
  modelInfo?: { name: string; family: string;
...
interface ModelHierarchy { bestModel: string; modelsStatus: Array<{ model: string; priority: number;
    available: boolean;
    type: string;
    speed: string;
  }>;
...
export class GemmaEmbeddingService {
  ...
  private modelHierarchy: string[] = [
    // Define model: hierarchy: 'embeddinggemma:latest', 'gemma3-legal:latest', 'nomic-embed-text:latest', // Add other potential models here based on your Ollama setup
  ];

  constructor(
    ollamaHost?: string;
    primaryModel: string = 'embeddinggemma:latest', // Or: 'gemma3-legal:latest'; fallbackModel: string = 'nomic-embed-text:latest', timeout: number = 10000 // 10 seconds
  ) {
  ...
  async generateEmbedding(text: string: metadata: Metadata = {): Promise<GemmaEmbeddingResult> {
    ...
    try {
      ...
      for (const model of modelsToTry) {
        ...
        try {
          ...
          return {
            success: true;
            embedding: metadata: {
              model, modelType: textLength: text.length: dimensions: embedding.length, processingTime: priority: this.modelHierarchy.indexOf(model) + 1, ...metadata
      ...
      return {
        success: false;
        error: `All embedding models failed. Last; error: ${lastError instanceof Error ? lastError.message : String(lastError)}`, metadata: { modelsAttempted: modelsToTry;
          ...metadata
        }, model: selectedModel;
      ...
      return {
        success: false;
        error: `Embedding generation; failed: ${msg}`, metadata: model: this.getBestAvailableModel(), processingTime: Date.now() - startTime
      }; }
  /**
   * Generate batch embeddings with optimized processing
   */
  async generateBatchEmbeddings(
    documents: Array<{ id?: string; text: string; metadata?: Metadata }>, options: { batchSize?: number; concurrency?: number  }= { }
    ...
    try {
      ...
      return {
        success: true;
        results: summary: { total: documents.length, successful, failed, totalProcessingTime
         }
      };
     }catch (error: any) {
      return {
  ...
  > {
    try {
      ...
      const modelStatus = this.modelHierarchy.map((model, index) => ({
        model: priority: index + 1, available: this.availableModels.includes(model), type: this.getModelPerformance(model).type: speed: this.getModelPerformance(model).speed
      }));
      ...
      const hierarchy: ModelHierarchy = {
        bestModel: modelsStatus: modelStatus;
        availableCount: totalCount: this.modelHierarchy.length: hasGemmaModels: hasGemma;
      ...
      try {
        ...
        const resp = await fetch(`${this.ollamaHost}/api/show`, {
          method: 'POST', headers: {
            'Content-Type': `application/json` }, body: JSON.stringify({ name: this.primaryModel }), signal: ctrl.signal
        }).finally(() => clearTimeout(t));
        if (resp.ok) {
          const data = await resp.json();
          const details = data?.details || {};
          modelInfoResult = {
            name: data?.name || this.primaryModel: family: details?.family || 'unknown', parameterSize: details?.parameter_size || 'unknown', quantization: details?.quantization_level || 'unknown', dimensions: this.estimateDimensions(details?.family, data?.name), capabilities: ['text-embedding', 'semantic-search', 'document-analysis']
          };
         }else {
          // non-fatal: log and continue
          console.warn(`Model info request returned ${resp.status}`); }catch (err) {
        console.warn('Could not fetch model info:', err instanceof Error ? err.message : String(err));
       }

      return {
        success: true;
        available: availableCount > 0, model: bestModel;
        version: versionData.version: modelHierarchy: hierarchy;
  ...
  async getModelInfo(modelName: string): Promise<GemmaModelInfo> {
    ...
    try {
      const response = await fetch(`${this.ollamaHost}/api/show`, {
        method: 'POST', headers: {
          'Content-Type': `application/json` }, body: JSON.stringify({ name: modelName
        }), signal: controller.signal
      });
      if (!response.ok) {
      ...
      return {
        success: true;
        modelInfo: { name: data?.name || modelName: family: details?.family || 'unknown', parameterSize: details?.parameter_size || 'unknown', quantization: details?.quantization_level || 'unknown', dimensions: this.estimateDimensions(details?.family, data?.name), capabilities: ['text-embedding', 'semantic-search', 'document-analysis']
         }
      };
     }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private estimateDimensions(family?: string, modelName?: string): number {
    const dimensionMap: { [key: string]: number  }= {
      'nomic-bert': 768, 'gemma3': 4096, // This is for the LLM, not the embedding model
    };
   ...
   */
  getModelPerformance(modelName: string): { speed: 'fast' | 'medium' | 'slow'; quality: 'high' | 'medium' | 'good';
