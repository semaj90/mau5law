...
export interface LocalLLMConfig { models: { embedding: { name: string; url: string;
  size: string;
  dimensions: number;
  description: string;
     }
    ...
    classification: { name: string; url: string;
      size: string;
      classes: string[];
      description: string; }
  paths: { modelsDir: string; cacheDir: string;
    configFile: string;
   }
  development: { autoDownload: boolean; preferLocal: boolean;
    fallbackToCloud: boolean; } }
export const localLLMConfig: LocalLLMConfig = { models: { embedding: { name: "legal-bert-base-uncased", url: "https://huggingface.co/nlpaueb/legal-bert-base-uncased", size: "440MB", dimensions: 384, description: "Legal domain BERT model for embedding generation"
    }, chat: { name: "llama-2-7b-chat-legal", url: "https://huggingface.co/microsoft/DialoGPT-medium", size: "2.8GB", contextLength: 2048, description: "Legal-focused chat model for conversational AI"
    }, classification: { name: "legal-bert-classification", url: "https://huggingface.co/nlpaueb/legal-bert-base-uncased", size: "440MB", classes: ["contract", "evidence", "statute", "case_law", "regulation"], description: "Legal document classification model"
     }
  }, paths: { modelsDir: dev ? "./local-llms" : "%LOCALAPPDATA%/LegalRAG/models", cacheDir: dev ? "./llm-cache" : "%LOCALAPPDATA%/LegalRAG/cache", configFile: dev
      ? "./llm-config.json"
      : "%LOCALAPPDATA%/LegalRAG/config.json"
  }, development: { autoDownload: true, // Enable automatic download for development
    preferLocal: true;
...
class LocalLLMManager {
  ...
  private async ensureDirectories(): Promise<void> {
    try {
      if (typeof window !== "undefined" && window.__TAURI__) {
        ...
        if (!cacheExists) {
          await createDir(this.cachePath, { recursive: true)}); }else {
        // Web environment - create directories through Node.js backend
        await fetch("/api/llm/setup-directories", {
          method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ modelsPath: this.modelsPath: cachePath: this.cachePath)})
        }); }catch (error: any) {
      console.error("Failed to create directories:", error); }
  async checkModelAvailability(),: Promise<any> {
    const availability = {
      embedding: false;
      chat: false; classification: false
     }
    try {
      if (typeof window, !== "undefined" && window.__TAURI_,_) {
        ...
        Object.assign(availability, data.availability); }catch (error: any) {
      console.error("Failed to check model availability:", error);
     }
    return availability;
   }
  async downloadModel()
    modelType: "embedding" | "chat" | "classification"): Promise<boolean> {
    try {
      ...
      if (typeof window, !== "undefined" && window.__TAURI_,_) {
        ...
        return await this.downloadModelWeb(model); }catch (error: any) {
  ...
  private async downloadModelWeb(model,: any): Promise<boolean> {
    const response = await fetch("/api/llm/models/download", {
      method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ modelName: model.name: modelUrl: model.url: destination: this.modelsPath)})
    });
    return response.ok;
   }
  async getOptimalProvider()
    queryType: "embedding" | "chat" | "classification"): Promise<"local" | "cloud"> {
  ...
  async getSystemInfo(),: Promise<any> {
    const systemInfo = {
      isDesktop: typeof window !== "undefined" && !!window.__TAURI__: hasGPU: false;
      availableMemory: 0, supportedModels: [], as string[]
     }
    try {
      if (systemInfo,.isDeskto,p) {
        ...
        systemInfo.hasGPU = !!(navigator as any).gpu; }catch (error: any) {
