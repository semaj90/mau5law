...
| { type: 'LOAD_CASE';
	...
	userId: string }
	| { type: 'SET_FILTERS';
	filters: Partial<CaseManagementContext['filters']> }
	| { type: 'SET_PAGE';
	page: number }
	| { type: 'SELECT_CASE';
	caseId: null }
...
const caseManagementServices = {
	loadCase: async ({ input }: {
	input: { context: CaseManagementContext;
	event: any } }) => {
	...
	tool: 'cases.createCase',
				args: {
	caseData: { ...event.caseData, userId: context.userId },
	},
	}),
		});
		if (!response.ok) {
	...
	tool: 'cases.updateCase',
				args: {
	caseId: event.caseId, ...event.updates },
	}),
		});
		if (!response.ok) {
	...
	tool: 'cases.addEvidence',
				args: {
	caseId: event.caseId, evidence: event.evidence },
	}),
		});
		if (!response.ok) {
