...
export interface DocumentExtractionJob extends BaseJobData { filename: string; contentType: string;
  storageUrl: string;
  extractionType: 'pdf' | 'image' | 'video' | 'audio' | 'text';
 }
export interface EmbeddingJob extends BaseJobData { textChunks: string[]; chunkMetadata: Array<Record<string, unknown>>;
  embeddingModel: 'sentence-transformers' | 'ollama' | 'openai';
 }
export interface TensorProcessingJob extends BaseJobData { tensorData: number[]; dimensions: [number, number, number, number]; // 4D tensor
  operation: 'tricubic' | 'som_cluster' | 'attention' | 'convolution';
  tileSize?: [number, number, number, number];
  haloSize?: number;
 }
export interface VectorIndexJob extends BaseJobData { embeddings: number[][]; metadata: Array<Record<string, unknown>>;
  indexType: 'qdrant' | 'pgvector' | 'faiss';
 }

export interface NotificationJob extends BaseJobData { message: string; type: 'completion' | 'error' | 'progress';
 }

type JobData = DocumentExtractionJob | EmbeddingJob | TensorProcessingJob | VectorIndexJob | NotificationJob;

interface ProgressUpdate { stage: string; status: 'processing' | 'completed' | 'failed';
  progress?: number;
  result?: any;
  error?: string;
 }
...
export class LegalAIJobQueue {
  ...
  private initializeQueues() {
    const queueConfigs = [
      { name: 'document-extraction', concurrency: 5 }, { name: 'embedding-generation', concurrency: 3 }, { name: 'tensor-processing', concurrency: 2 }, { name: 'vector-indexing', concurrency: 4 }, { name: 'notification', concurrency: 10  }
    ];
    queueConfigs.forEach(({ name, concurrency }) => {
      const queue = new Queue(name, {
        connection: redisConnection;
        defaultJobOptions: { removeOnComplete: 100, removeOnFail: 50, attempts: 3, backoff: { type: 'exponential', delay: 2000
           }
         }
      });
      ...
      const worker = new BullMQWorker(name, this.createJobProcessor(name), {
        connection: redisConnection;
        concurrency: limiter: { max: concurrency * 2, duration: 1000
         }
      });
      ...
      any.on('progress', (job: BullMQJob<JobData>, progress: number | object) => {
        ...
        this.broadcastProgress(job.data.uploadId, {
          stage: name;
          status: 'processing', progress: typeof progress === 'number' ? progress : undefined
        });
      });
  ...
  private async processDocumentExtraction(job: BullMQJob<DocumentExtractionJob>) {
    ...
    try {
      ...
      switch (extractionType) {
        case, 'pdf': {
          ...
          const pdfResponse = await fetch('http://localhost:8082/extract/pdf', {
            method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ url: storageUrl, filename })
          });
      ...
      await this.addEmbeddingJob({
        ...job.data: textChunks: chunks.map(c => c.text), chunkMetadata: chunks.map(c => c.metadata), embeddingModel: 'sentence-transformers` });'`
      await job.updateProgress(100);
      return {
        extractedText: chunkCount: chunks.length, metadata: nextStage: `embedding-generation` };
     }catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < textChunks.length; i += batchSize) {
        ...
        const response = await fetch('http://localhost:8083/embed', {
          method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ texts: batch;
      ...
      await this.addVectorIndexJob({
        ...job.data, embeddings: metadata: textChunks.map((text: string: idx: number) => ({
          docId: job.data.uploadId: chunkId: `${job.data.uploadId}-${idx}`, text, ...job.data.chunkMetadata[idx]
        })), indexType: 'pgvector` });'`
      await job.updateProgress(100);
      return {
        embeddingCount: embeddings.length: dimensions: embeddings[0]?.length || 0, nextStage: `vector-indexing` };
     }catch (error) {
  ...
  private async processTensorProcessing(job: BullMQJob<TensorProcessingJob>) {
    ...
    try {
      ...
      const response = await fetch('https://localhost:4433/tensor/process', {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ job_id: job.id: upload_id: job.data.uploadId: tensor_tile: { tile_id: `${job.data.uploadId}-${job.id}`, dimensions: halo_size: haloSize || 2, data: tensorData;
            metadata: { case_id: job.data.caseId, operation
             }
      ...
      while (attempts < maxAttempts) {
        ...
        const resultResponse = await fetch(`https://localhost:4433/tensor/result?job_id=${job.id}`);
      ...
      await job.updateProgress(100);
      return {
        tensorResult: metrics: tensorResult.metrics: outputSize: tensorResult.output_data?.length || 0
      };
     }catch (error) {
  ...
  private async processVectorIndexing(job: BullMQJob<VectorIndexJob>) {
    ...
    try {
      switch (indexType) {
        case, 'pgvector': {
          ...
          const pgResponse = await fetch('http://localhost:5434/api/vectors/insert', {
            method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
              embeddings, metadata: table: `legal_embeddings` })
          });
          if (!pgResponse.ok) {
        ...
        case, 'qdrant': {
          ...
          const qdrantResponse = await fetch('http://localhost:6333/collections/legal-docs/points', {
            method: 'PUT', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ points: embeddings.map((vector: number[], idx: number) => ({
                id: metadata[idx]['chunkId'], as string, vector: payload: metadata[idx]
              }))
            })
          });
      ...
      await this.addNotificationJob({
        ...job.data: message: `Document ${job.data.uploadId }successfully processed and indexed`, type: `completion` });
      await job.updateProgress(100);
      return {
        indexedCount: embeddings.length, indexType: status: `completed` };
     }catch (error) {
  ...
  private chunkText(
    text: string;
    maxTokens: number = 512, overlap: number = 40, overlapSentences: number = 2 // configurable overlap window
  ) {
    // Choose encoding based on model (adjust as needed)
    const encoding = encoding_for_model('gpt-3.5-turbo');
    const sentences = text.split(/[.!?]+/).filter(item => item.length > 0);
    const chunks = [];
    let currentChunk = '';
    let currentTokens = 0;
    let chunkStartSentenceIdx = 0;
    for (let i = 0; i < sentences.length; i++) {
      const sentence = sentences[i].trim() + '.';
      const sentenceTokens = encoding.encode(sentence).length;
      if (currentTokens + sentenceTokens > maxTokens && currentChunk.length > 0) {
        chunks.push({
          text: currentChunk.trim(), metadata: { sentenceStart: chunkStartSentenceIdx  }
        });
        // Start new chunk with configurable overlap
        let adaptiveOverlap = overlapSentences;
        // Adapt overlap based on sentence length (long sentences = less overlap)
        if (sentence.length > 200) adaptiveOverlap = Math.max(1, overlapSentences - 1);
        if (sentence.length < 50) adaptiveOverlap = overlapSentences + 1;
        const overlapStart = Math.max(0, i - adaptiveOverlap);
        const overlapSentencesArr = sentences.slice(overlapStart, i);
  ...
  async addDocumentExtractionJob(data: DocumentExtractionJob, options?: JobsOptions) {
    ...
    return queue.add('extract-document', data, {
      priority: this.getPriority(data.priority), ...options
    });
   }
