...
export const GET: RequestHandler = async ({ request }) => {
  ...
  if (request.headers.get('upgrade')?.toLowerCase() === 'websocket') {
    return new Response(
      JSON.stringify({
        endpoint: '/ws/precedents',
        protocol: 'websocket',
        status: 'upgrade_required',
        message: 'This route accepts WebSocket upgrade requests. Connect using a WebSocket client to the same URL; upgrades are handled by the global WS server.'
      }),
      {
        status: 426,
