...
type GPUDeviceLike = {
  ...
  queue: { writeBuffer: (b: unknown, o: number, data: unknown, off?: number, len?: number) => void; submit: (cmds: unknown[]) => void };
  createShaderModule: (opts: unknown) => unknown;
  createComputePipeline: (opts: unknown) => unknown;
  getBindGroupLayout?: (idx: number) => unknown;
  createBindGroup: (opts: unknown) => unknown;
...
self.addEventListener('message', async (event: MessageEvent) => {
  ...
  pass.end();
    (encoder as unknown as { copyBufferToBuffer: (...args: unknown[]) => void }).copyBufferToBuffer(
      scoresBuffer,
      0,
      resultBuffer,
      0,
