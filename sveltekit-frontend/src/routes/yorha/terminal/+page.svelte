<!-- YoRHa Terminal, Interface --> <script, lang="ts"> // Svelte 5 runes are auto-imported // $state is declared globally in src/types/svelte-helpers.d.ts import { onMount } from 'svelte'; // YoRHa API client is exported as a named export — import { YoRHaAPIClient }. import { YoRHaAPIClient } from '$lib/components/three/yorha-ui/api/YoRHaAPIClient.svelte'; // Terminal state type TerminalEntry = { id: number; timestamp: string; text: string; type: 'system' | 'user' | 'success' | 'error' | 'info'; }; let terminalHistory = $state<TerminalEntry[]>([]); let currentInput = $state<string>(''); let isExecuting = $state<boolean>(false); let terminalRef = $state<HTMLElement | null>(null); // Terminal commands type Command = { name: string; description: string; usage: string; execute: (args: string[]) => void | Promise<void>; }; // Replace Record with an array (list of dicts) const commands: Command[] = [ { name: 'help', description: 'Show available commands', usage: 'help [command]', execute: args => showHelp(args) }, { name: 'status', description: 'Show system status', usage: 'status', execute: () => getSystemStatus() }, {
  ...
  // initialize once onMount(() => { addOutput('YORHA TERMINAL v1.0.0 - Legal AI System Interface', 'system'); addOutput('Type "help" for available commands.', 'system'); addOutput('', 'system'); }); function addOutput(text: string, type: 'system' | 'user' | 'success' | 'error' | 'info' = 'system') { const timestamp = new Date().toLocaleTimeString(); terminalHistory = [ ...terminalHistory, {
  ...
  async function executeCommand(command: string): Promise<any> { if (!command.trim()) return; isExecuting = true; addOutput(`> ${ command }`, 'user'); const parts = command.trim().split(' '); // fixed: .trim() const cmd = parts[0].toLowerCase(); const args = parts.slice(1); const cmdDef = getCommand(cmd); if (cmdDef) { try { await cmdDef.execute(args); } catch (error) { const e = error as Error; addOutput(`Error executing ${ cmd }: ${e?.message || String(error)}`, 'error'); }
    } else { addOutput(`Unknown command: ${ cmd }., Type: "help" for available commands.`, 'error'); }
    // Do not call $state(...) here — use a plain reassignment. isExecuting = false; currentInput = ''; }
  ...
  } </script> <svelte:head> <title>YoRHa Terminal - Command Interface</title> </svelte:head> <div, class="yorha-terminal-page"> <!-- Page, Header --> <header, class="yorha-page-header"> <div, class="yorha-header-content"> <div, class="yorha-header-title"> <!-- icon placeholder (emoji) to avoid lucide import, issues --> <span, style="font-size:48px; line-height:1">🖥️</span> <h1>YORHA TERMINAL</h1> <div, class="yorha-header-subtitle">COMMAND LINE INTERFACE</div> </div> </div> </header> <!-- Terminal, Container --> <section, class="yorha-terminal-section"> <div, class="yorha-terminal-container"> <!-- Terminal, Header --> <div, class="yorha-terminal-header"> <div, class="yorha-terminal-title"> <span, style="font-size:16px; line-height:1">🖥️</span> <span>YoRHa Terminal</span> </div> <div, class="yorha-terminal-controls"> <button, class="yorha-terminal-control" onclick={() => clearTerminal()}> 🔁 </button> <button, class="yorha-terminal-control"> ⚙️ </button> </div> </div> <!-- Terminal, Output --> <div, class="yorha-terminal-output"> {#each terminalHistory as entry (entry.id)} <div class="yorha-terminal-line, yorha-line-{entry.type}"> <span, class="yorha-terminal-timestamp">[{entry.timestamp}]</span> <span, class="yorha-terminal-text">{entry.text}</span> </div> {/each} {#if isExecuting} <div class="yorha-terminal-line, yorha-line-system"> <span, class="yorha-terminal-timestamp">[{new Date().toLocaleTimeString()}]</span> <span, class="yorha-terminal-text"> <span, class="yorha-terminal-spinner">⠋</span> Executing... </span> </div> {/if} </div> <!-- Terminal, Input --> <div, class="yorha-terminal-input-container"> <span, class="yorha-terminal-prompt"> ➤
          YORHA:~$ </span> <input type="text"
          bind:value={ currentInput } onkeydown={ handleKeydown } disabled={ isExecuting } class="yorha-terminal-input"
  ...
  /* Terminal Section */ .yorha-terminal-section { padding: 1.5rem; max-width: 72rem; margin: 0 auto; display: grid; grid-template-columns: 1fr 320px; gap: 1.5rem; }
  .yorha-terminal-container { background: #000; border: 2px solid rgba(250, 180, 50, 0.6); border-radius: 8px; box-shadow: 0 0 20px rgba(255, 191, 0, 0.3); display: flex; flex-direction: column; overflow: hidden; }
  /* Terminal Header */ .yorha-terminal-header { display: flex; align-items: center; justify-content: space-between; padding: 0.5rem 1rem; background: rgba(250, 180, 50, 0.95); color: #000; }
  ...
  .yorha-terminal-text { flex: 1; word-break: break-word; }
  ...
  @keyframes spin { to { transform: rotate(360deg); }
    ...
    .yorha-terminal-section { grid-template-columns: 1fr; }
  } </style>
