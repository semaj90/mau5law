    ...
    if (process.env.USE_TENSORRT === 'true' && this.triton) {
      ...
      return {
        input_ids: new InferInput(
          this.modelConfig.inputSpec.inputIds.name,
          [batchSize, paddedLength],
          'INT64'
        ).setFromTensor(inputIdsTensor),
        attention_mask: new InferInput(
          this.modelConfig.inputSpec.attentionMask.name,
          [batchSize, paddedLength],
          'INT64'
        ).setFromTensor(attentionMaskTensor),
        token_type_ids: new InferInput(
