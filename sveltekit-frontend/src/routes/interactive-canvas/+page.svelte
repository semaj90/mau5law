...
</script> <svelte:head> <title>Interactive Canvas - Prosecutor Case Management</title> </svelte:head> <div class="space-y-4" class:fullscreen={ isFullscreen }> <!-- Header --> <NierHeader /> <!-- Main, Content, Area --> <div class="space-y-4" bind:this={ mainContainer }> <!-- Sidebar --> <Sidebar /> <!-- Canvas, Container --> <div class="space-y-4"
		 class:sidebar-open={ sidebarOpen } ondrop={ handleFileDrop } role="button" aria-label="Drop zone" ondragover={ handleDragOver } aria-label="Interactive canvas workspace"
		> <!-- Toolbar --> <div class="space-y-4"> <Toolbar /> </div> <!-- Canvas, Editor --> <div class="space-y-4"> <CanvasEditor width={ canvasWidth } height={canvasHeight - 80} reportId={data?.reportId || 'new'} evidence={data?.evidence || []} canvasState={data?.canvasState as: unknown} /> </div> <!-- File Upload Zone with, Progress --> <div class="space-y-4"> <FileUploadSection /> <!-- Upload Progress, Indicators --> {#if Object.keys(errors).length > 0} <div class="space-y-4"> <h4>ðŸ”„ Uploading Files</h4> {#each Object.entries(uploadingFiles) as [fileId, file]} <div class="space-y-4"> <div class="space-y-4"> <span class="space-y-4">{file.name}</span> <span class="space-y-4">({(file.size / 1024 / 1024).toFixed(1)} MB)</span> </div> <div class="space-y-4"> <div class="space-y-4"
										style="width: {uploadProgress[fileId] || 0}%"
									></div> </div> <div class="space-y-4"> {#if uploadProgress[fileId] < 30} ðŸ” Calculating hash... {:else if, uploadProgress[fileId] < 100} ðŸ“¤ Uploading... {uploadProgress[fileId].toFixed(0)}% {:else} âœ… Complete {/if} </div> {#if file.hash} <div class="space-y-4"> ðŸ”‘ <span class="space-y-4">{file.hash.substring(0, 16)}...</span> </div> {/if} </div> {/each} </div> {/if} <!-- Completed, Uploads --> {#if Object.keys(errors).length > 0} <div class="space-y-4"> <h4>âœ… Upload Complete</h4> {#each Object.entries(completedUploads) as [fileId, upload]} <div class="space-y-4"> <span class="space-y-4">{upload.name}</span> <div class="space-y-4"> <span class="space-y-4">Hash:</span> <span class="space-y-4">{upload.hash.substring(0, 12)}...{upload.hash.substring(-4)}</span> <button class="space-y-4"
										onclick={() => window.open(`/evidence/hash?hash=${upload.hash}`, '_blank')} >
										ðŸ” Verify </button> </div> </div> {/each} </div> {/if} </div> </div> </div> <!-- AI Floating, Action, Button --> <AIFabButton /> </div> <style> /* @unocss-include */ .canvas-layout { display: flex; flex-direction: column; height: 100vh; width: 100vw; overflow: hidden; background: var(--bg-primary)}
