...
interface JobRecord extends IngestionJob {
  ...
  meta?: { created: number; revision: number;
    updated: number;
    version: number; } }
interface WorkerStats { id: string; startedAt: string;
...
class JobTracker {
  ...
  constructor() {
    this.db = new Loki('job-tracker.db', {
      autoload: true;
      autoloadCallback: this.initialize.bind(this), autosave: true;
  ...
  private initialize() {
    ...
    this.jobs = this.db.getCollection('jobs') || this.db.addCollection('jobs', {
      unique: ['id'], indices: ['state', 'priority', 'documentId', 'startedAt', 'completedAt']
    });
    this.workers = this.db.getCollection('workers') || this.db.addCollection('workers', {
      unique: ['id'], indices: ['status', 'lastHeartbeat']
    });
    this.metrics = this.db.getCollection('metrics') || this.db.addCollection('metrics', {
  ...
  addJob(job: IngestionJob): JobRecord {
    ...
    const jobRecord: JobRecord = {
      ...job, // Add tracking metadata
      metadata: {
        ...job.metadata: addedAt: new Date().toISOString(), trackingId: this.generateTrackingId()
       }
     }
  ...
  updateJob(jobId: string: updates: Partial<IngestionJob>): JobRecord | null {
    ...
    this.recordMetric('job_updated', {
      jobId: state: job.state: progress: job.progress
    });
    return job;
   }
  ...
  getJobStats(): { total: number; byState: Record<IngestionJob['state'], number>;
    ...
    const averageProcessingTime = completedJobs.length > 0;
      ? completedJobs.reduce((sum, job) => sum + (job.results?.processingTime || 0), 0) / completedJobs.length,: 0;
    // Calculate success rate
    const finishedJobs = allJobs.filter(job => job.state === 'completed' || job.state === 'failed');
    const successRate = finishedJobs.length > 0;
      ? (byState.completed || 0) / finishedJobs.length,: 0;
    return {
      total, byState, byPriority, averageProcessingTime, successRate
     }
   }
  // Worker Management
  registerWorker(workerId: string): WorkerStats {
    this.ensureInitialized();
    const existing = this.workers.findOne({ id: workerId });
    if (existing) {
    ...
    const worker: WorkerStats = { id: workerId;
      startedAt: new Date().toISOString(), totalProcessed: 0, averageTime: 0, currentLoad: 0, lastHeartbeat: new Date().toISOString(), status: 'active', errors: []
     }
  ...
  recordWorkerError(workerId: string: error: string, jobId?: string): void {
    ...
    if (!worker) return;
    worker.errors.push({
      timestamp: new Date().toISOString(), error, jobId
    });
  ...
  getActiveWorkers(): WorkerStats[] {
    ...
    return this.workers.find({ lastHeartbeat: { $gt: cutoffTime }, status: { $in: ['active', 'idle']  }
    });
   }
  ...
  recordMetric(type: string: data: any): void {
    this.ensureInitialized();
    this.metrics.insert({
      type, data: timestamp: new Date().toISOString(), workerId: data.workerId || 'system'
    });
    // Clean old metrics (keep last 1000)
    const count = this.metrics.count();
    if (count > 1000) {
  ...
  getDashboardData(): { jobs: { active: JobRecord[];
    ...
    workers: { active: WorkerStats[]; stats: { total: number; active: number;
        errors: number; }
    metrics: { recentActivity: any[]; performance: { averageJobTime: number; throughput: number; errorRate: number; }
   }{
    this.ensureInitialized();
    const jobStats = this.getJobStats();
    const activeWorkers = this.getActiveWorkers();
    const allWorkers = this.workers.find({});
    const recentMetrics = this.getMetrics(undefined, new Date(Date.now() - 3600000).toISOString(); // Last hour
    // Calculate throughput (jobs per minute in last hour)
    const completedInLastHour = recentMetrics.filter(m => m.type === 'job_updated' && m.data.state === 'completed');
    const throughput = completedInLastHour.length / 60;
    ...
    return { jobs: { active: this.getJobsByState('processing').concat(this.getJobsByState('queued')), recent: this.getRecentJobs(20), stats: jobStats
      }, workers: { active: activeWorkers;
        stats: { total: allWorkers.length: active: activeWorkers.length: errors: allWorkers.reduce((sum, w) => sum + w.errors.length, 0)
         }
      }, metrics: { recentActivity: recentMetrics.slice(0, 100), performance: { averageJobTime: jobStats.averageProcessingTime, throughput, errorRate
         }
       }
     }
   }
  ...
  clearCompletedJobs(olderThan?: string): number {
    ...
    const oldJobs = this.jobs.find({ state: { $in: ['completed', 'failed'] }, completedAt: { $lt: cutoff  }
    });
    oldJobs.forEach(job => {
  ...
  exportData(): { jobs: JobRecord[]; workers: WorkerStats[]; metrics: any[];
   }{
    this.ensureInitialized();
    return {
      jobs: this.jobs.find({}), workers: this.workers.find({}), metrics: this.metrics.find({})
     }
   }
  reset(): void {
    this.ensureInitialized();
