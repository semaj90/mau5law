...
chunks.map(async (chunk, index) => {
    ...
    /** * Process embedding chunk via MCP worker */ private async processEmbeddingChunk( texts: string[], workerId: string, embeddingType: string ): Promise<{
	embeddings: number[][], cacheHitCount: number;
	success: boolean; }> { try { const response = await fetch(`${this.config.baseUrl}/embed`, { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({ texts, workerId, embeddingType, model: 'embedding-gemma:latest' },
...
requests.map((req) => {
     ...
     */
    getWorkerStats(): {
	totalWorkers: number, busyWorkers: number;
...
export const DEFAULT_MCP_CONFIG: Partial<MCPContext7Config> = {
    ...
    workers: 8, timeout: 30000, retryAttempts: 3, fallbackToLocal: true
};
