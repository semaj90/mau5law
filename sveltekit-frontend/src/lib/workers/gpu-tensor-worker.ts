...
// Consolidated GPU Tensor Worker
// - Supports messages: INITIALIZE, PROCESS_TENSOR, PROCESS_BATCH, GET_STATS, CLEAR_CACHE
// - Optional WebGPU/WebAssembly initialization (non-fatal)
...
, data: Float32Array | number[];
  ...
  cacheKey?: string;
  timestamp?: number;
  // optional metadata for advanced paths
  lodLevel?: number;
  dimensions?: number;
};
...
class GPUTensorWorker {
  ...
  private updateCacheStats(isHit: boolean) {
    ...
    this.stats.totalProcessed = prevTotal + 1;
  ...
  private async processWithWebGPUorCPU(tensor: MultiDimArray): Promise<MultiDimArray> {
    if (this.gpuDevice) {
      // Real WebGPU pipeline would live here. For this stub we log and fallback to CPU processing.
      try {
  ...
  async processGPUTensor(tensor: MultiDimArray): Promise<MultiDimArray> {
    ...
    try {
      const key = tensor.cacheKey ?? JSON.stringify(tensor.shape) + '::' + (tensor.layout ?? '');
