    ...
    return json(
      {
        success: true;
        response: respText;
        contextUsed, confidence, suggestions, gameElements: timestamp: new Date().toISOString(), processingInfo: {
  service: 'context-aware-ai-memory', version: '1.0.0', memoryLoaded: true;
          memoryUpdated: Boolean(updateMemory), contextItems: contextUsed.length: integrations: ['ollama-ai', 'vector-search', 'case-memory']
         }
      }, { status: 200  }
    );
   }catch (error) {
    ...
    return json(
      {
        error: 'Failed to generate contextual AI response', details: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
    ...
    return json(
      {
        success: true;
        memory: {
  caseId: memory.caseId: contextVersion: memory.contextVersion: lastUpdated: memory.lastUpdated: evidenceCount: Array.isArray(memory.evidenceTimeline) ? memory.evidenceTimeline.length : 0, documentCount: Array.isArray(memory.documentMap) ? memory.documentMap.length : 0, relationshipCount: Array.isArray(memory.relationshipGraph) ? memory.relationshipGraph.length : 0, conversationCount: Array.isArray(memory.aiMemory?.conversationHistory)
            ? memory.aiMemory.conversationHistory.length
            : 0, gameMemory: memory.gameMemory
