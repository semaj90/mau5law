 ...
 * Exposes a small API for resolving Ollama endpoints based on
 * runtime env and deployment profile.
 */
 ...
 * 2. process.env.OLLAMA_ENDPOINT / process.env.OLLAMA_URL (server)
 * 3. unified config file fallbacks (production-config.ts)
...
export function resolveOllamaConfig(): OllamaConfig {
  ...
  if (typeof process !== 'undefined' && typeof process.env !== 'undefined') {
    ...
    const processUrl =
      env.OLLAMA_ENDPOINT || env.OLLAMA_URL ||
      env.VITE_OLLAMA_ENDPOINT || env.VITE_OLLAMA_URL;
    if (processUrl) {
      return {
  ...
  // 3) Optional unified production config
  try {
    ...
    const prodConfigRaw: unknown =
      prodModule?.CONFIG ?? prodModule?.PRODUCTION_CONFIG ?? prodModuleRaw;
  ...
  try {
    ...
    if (g && typeof g.OLLAMA_ENDPOINT === 'string') {
      return {
        baseUrl: g.OLLAMA_ENDPOINT.replace(/\/$/, ''),
        enabled: true
      } as OllamaConfig;
    }
  } catch {
