...
export class CaseSimilarityService {
  ...
  async computeSimilarities(nodes: EvidenceNode[]): Promise<SimilarityResult[]> {
    ...
    try {
      ...
      // Cache results
      nodeIds.forEach(id => {
        this.similarityCache.set(id, results.filter(r =>
...
Provide a brief explanation of their relationship.`;
  ...
  async clusterCases(nodes: EvidenceNode[], similarities: SimilarityResult[]): Promise<CaseCluster[]> {
    ...
    for (const node of nodes) {
      ...
      // Find similar nodes
      const similarResults = similarities.filter(r =>
        (r.sourceId === node.id || r.targetId === node.id) && r.similarity > 0.7
      );
      ...
      if (clusterNodes.length > 1) {
        ...
        clusters.push({
          ...
          nodes: clusterNodes,
          centroid,
          similarity: similarResults.reduce((sum, r) => sum + r.similarity, 0) / similarResults.length,
          theme
        });
      }
    }
