<script lang="ts">
  ...
  async function upload(): Promise<any> {
    ...
    try {
      ...
      const res = await fetch('/api/upload', { method: 'POST', body: fd }); // Safely parse JSON only if content-type is JSON const ct = res.headers.get('content-type') ?? ''; let data: any = null; if (ct.includes('application/json')) { data = await res.json(); }
      if (res.ok) {
