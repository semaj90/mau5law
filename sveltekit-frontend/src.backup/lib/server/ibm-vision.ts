...
export class IBMVisionService {
   ...
   */
  private async classifyImage(imageBase64: string): Promise<Array<{ class: string; confidence: number }>> {
    const params = {
      images_file: {
        value: Buffer.from(imageBase64, 'base64'),
        options: {
    ...
    if (response.result.images && response.result.images[0]?.classifiers) {
      return response.result.images[0].classifiers[0]?.classes?.map((cls: any) => ({
        class: cls.class,
   ...
   */
  private async detectFaces(imageBase64: string): Promise<Array<{
    bbox: number[];
    age?: { min: number; max: number };
    gender?: string;
    emotions?: Record<string, number>;
