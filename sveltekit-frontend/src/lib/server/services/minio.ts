...
// Not exporting S3 constant as it needs to be awaited.
...
export async function uploadMinioObject(bucket: string, file: File, userId: string): Promise<string> {
    const buffer = Buffer.from(await file.arrayBuffer());
    const client = await getS3Client();
    // const key = file.name; // Assuming file name logic needs to be handled
    const key = file.name;
