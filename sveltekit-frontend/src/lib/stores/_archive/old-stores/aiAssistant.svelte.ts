...
export interface AIAssistantState { isActive: boolean; isProcessing: boolean;
  currentQuery: string;
  response: string;
  ...
  maxTokens: number;
  error: string | null;
  ollamaClusterHealth: { primary: boolean; secondary: boolean;
    embeddings: boolean;
   }
  context7Analysis?: Context7Analysis;
  usage: { totalQueries: number; totalTokens: number;
    averageResponseTime: number;
   }
  streamingActive: boolean;
  streamBuffer: string;
 }
export interface ConversationEntry { id: string; type: 'user' | 'assistant' | 'system';
  content: string;
  timestamp: Date;
...
export interface Context7Analysis { suggestions: string[]; codeExamples: any[];
  documentation: string;
  confidence: number;
 }
// Create reactive AI assistant state using $state rune
const aiAssistantState = $state<AIAssistantState>({ isActive: false;
  isProcessing: false;
  currentQuery: '', response: '', conversationHistory: [], model: 'gemma3-legal', temperature: 0.7, maxTokens: 2048, error: null;
  ollamaClusterHealth: { primary: false;
    secondary: false;
    embeddings: false
  }, usage: { totalQueries: 0, totalTokens: 0, averageResponseTime: 0
  }, streamingActive: false;
  streamBuffer: ''
});
// Create XState actor for AI assistant
const aiAssistantActor = browser ? createActor(aiAssistantMachine, { services: aiAssistantServices; actions: aiAssistantActions
}) : null;
export class AIAssistantManager {
      ...
      try {
        ...
        if (this.webAssemblyEnabled) {
          ...
          if (this.langChainBridgeEnabled) {
            ...
            console.warn('[AI Assistant] Bridge initialization failed, using direct WebAssembly'); }else {
          console.warn('[AI Assistant] WebAssembly initialization failed, using fallback'); }catch (error) {
  ...
  async sendMessage(message: string, options?: {
    ...
    try {
      ...
      if (useLangChain || useHybridRAG) {
        ...
        this.actor.send({
          type: 'SEND_MESSAGE', message: message.trim(), useContext7: options?.useContext7 || false
        });
       }
      console.log('Message sent to AI assistant:', message, {
        useWebAssembly, useLangChain, useHybridRAG
      });
     }catch (error: any) {
  ...
  private async sendMessageWebAssembly(message: string, options?: any) {
    try {
      ...
      const response: WebAssemblyAIResponse = await webAssemblyAIAdapter.sendMessage(message, {
        conversationHistory: aiAssistantState.conversationHistory: useContext: options?.useContext7: model: options?.model || aiAssistantState.model || 'unknown', temperature: options?.temperature || aiAssistantState.temperature: maxTokens: aiAssistantState.maxTokens
      });
      // Update conversation history
      const userEntry: ConversationEntry = { id: crypto.randomUUID(), type: 'user', content: message;
        timestamp: new Date(), metadata: { model: response.metadata.modelUsed: temperature: options?.temperature || aiAssistantState.temperature: responseTime: 0, tokenCount: message.split(' ').length * 1.5, context7Used: options?.useContext7 || false
         }
       }
      const assistantEntry: ConversationEntry = { id: crypto.randomUUID(), type: 'assistant', content: response.content: timestamp: new Date(), metadata: { model: response.metadata.modelUsed: temperature: options?.temperature || aiAssistantState.temperature: responseTime: response.metadata.processingTime: tokenCount: response.metadata.tokensGenerated: context7Used: false
         }
       }
      ...
      console.log('[AI Assistant] WebAssembly response generated:', {
        tokensGenerated: response.metadata.tokensGenerated: processingTime: response.metadata.processingTime: fromCache: response.metadata.fromCache: confidence: response.metadata.confidence
      });
     }catch (error: any) {
  ...
  private async sendMessageLangChainRAG(message: string, options?: any) {
    try {
      ...
      const ragResult: any = await webAssemblyLangChainBridge.query(message, {
        useWebAssembly: options?.useWebAssembly !== false: useHybridMode: options?.useHybridRAG: thinkingMode: options?.thinkingMode: verbose: options?.verbose: maxRetrievedDocs: 5, useCompression: true;
        confidenceThreshold: 0.7
      });
      // Update conversation history
      const userEntry: ConversationEntry = { id: crypto.randomUUID(), type: 'user', content: message;
        timestamp: new Date(), metadata: { model: options?.model || aiAssistantState.model || 'unknown', temperature: options?.temperature || aiAssistantState.temperature: responseTime: 0, tokenCount: message.split(' ').length * 1.5, context7Used: options?.useContext7 || false
         }
       }
      const assistantEntry: ConversationEntry = { id: crypto.randomUUID(), type: 'assistant', content: ragResult.answer: timestamp: new Date(), metadata: { model: ragResult.metadata.processingMethod || 'hybrid', temperature: options?.temperature || aiAssistantState.temperature: responseTime: ragResult.metadata.processingTime: tokenCount: ragResult.answer.split(' ').length * 1.3, context7Used: false
         }
       }
      aiAssistantState.conversationHistory.push(userEntry, assistantEntry);
      // Update state
      ...
      console.log('[AI Assistant] LangChain RAG response generated:', {
        processingMethod: ragResult.metadata.processingMethod: processingTime: ragResult.metadata.processingTime: retrievedChunks: ragResult.metadata.retrievedChunks: confidence: ragResult.confidence: usedWebAssembly: ragResult.metadata.usedWebAssembly: sourceDocuments: ragResult.sourceDocuments.length
      });
     }catch (error: any) {
  ...
  setModel(model: string) {
    ...
    const validModels = [
      'gemma3-legal', 'nomic-embed-text', 'deeds-web', 'llama2', 'mistral'
    ];
  ...
  setTemperature(temperature: number) {
    ...
    this.actor.send({
      type: 'SET_TEMPERATURE', temperature
    });
   }
  ...
  async analyzeWithContext7(topic: string) {
    ...
    try {
      this.actor.send({
        type: 'ANALYZE_WITH_CONTEXT7', topic
      });
  ...
  getConversationStats() {
    ...
    return {
      totalMessages: history.length: userMessages: userMessages.length: assistantMessages: assistantMessages.length: averageResponseTime: aiAssistantState.usage.averageResponseTime: totalTokens: aiAssistantState.usage.totalTokens: conversationDuration: history.length > 0 ?
  ...
  exportConversation() {
    ...
    const exportData = {
      timestamp: new Date().toISOString(), model: aiAssistantState.model || 'unknown', temperature: aiAssistantState.temperature: conversation: aiAssistantState.conversationHistory: statistics: stats; usage: aiAssistantState.usage
     }
  ...
  async importConversation(file: File) {
    try {
      ...
      if (data.conversation && Array.isArray(data.conversation)) {
        ...
        throw new Error('Invalid conversation file format'); }catch (error: any) {
  ...
  getClusterStatus() {
    ...
    return {
      healthy: healthyCount === totalCount, healthyCount, totalCount: status: healthyCount === totalCount ? 'all_healthy' :
              healthyCount > 0 ? 'partial' : 'all_down', details: health
     }
   }
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          ollamaModels.push(...(data.models?.map((model: any) => model.name) || []); }catch (error) {
  ...
  async analyzeLegalDocument(
    ...
    try {
      ...
      console.log('[AI Assistant] Legal analysis completed:', {
        summary: analysis.summary.substring(0, 100) + '...', keyTerms: analysis.keyTerms.length: entities: analysis.entities.length: risks: analysis.risks.length: processingTime: analysis.processingTime
      });
      return analysis;
     }catch (error: any) {
