...
export interface AIResponse { id: string; content: string;
  providerId?: string;
  model?: string;
  tokensUsed?: number;
  responseTime?: number;
...
export interface AutoGenAgent { name: string; systemMessage: string;
...
export interface AutoGenMessage { id: string; sender: string;
  recipient?: string;
  content: string;
  timestamp: number; messageType: 'text' | 'function_call' | 'function_response';
  metadata?: Record<string, unknown>;
 }

export interface AutoGenConversation { id: string; participants: AutoGenAgent[];
  messages: AutoGenMessage[];
  status: 'active' | 'completed' | 'failed' | 'terminated';
  startTime: number;
  endTime?: number; metadata: Record<string, unknown>;
 }

export interface LegalAgentTeam { prosecutor: AutoGenAgent; legalResearcher: AutoGenAgent;
...
export class AutoGenService {
  ...
  createLegalAgentTeam(): LegalAgentTeam {
    const prosecutor: AutoGenAgent = { name: 'prosecutor', systemMessage: `You are an experienced prosecutor with expertise in criminal law, evidence evaluation, and case strategy.`
      Your role is to:
      ...
      llmConfig: { model: 'gemma3-legal:latest', temperature: 0.1, maxTokens: 1024, apiBase: getOllamaEndpoint()
      }, humanInputMode: 'NEVER', maxConsecutiveAutoReply: 3, tools: ['legal_database_search', 'case_precedent_lookup', 'statute_analysis']
    };

    const legalResearcher: AutoGenAgent = { name: 'legal_researcher', systemMessage: `You are a skilled legal researcher specializing in case law, statutes, and legal precedents.`
      Your role is to:
      ...
      llmConfig: { model: 'llama3:8b-instruct', temperature: 0.2, maxTokens: 1536, apiBase: getOllamaEndpoint()
      }, humanInputMode: 'NEVER', maxConsecutiveAutoReply: 2, tools: ['westlaw_search', 'lexis_search', 'statute_lookup', 'citation_formatter']
    };

    const evidenceAnalyst: AutoGenAgent = { name: 'evidence_analyst', systemMessage: `You are a forensic evidence analyst with expertise in digital and physical evidence evaluation.`
      Your role is: to:
      ...
      llmConfig: { model: 'gemma3-legal:latest', temperature: 0.1, maxTokens: 1024, apiBase: getOllamaEndpoint()
      }, humanInputMode: 'NEVER', maxConsecutiveAutoReply: 2, tools: ['evidence_validator', 'chain_custody_tracker', 'admissibility_checker']
    };

    const coordinator: AutoGenAgent = { name: 'coordinator', systemMessage: 'You are a case coordination specialist responsible for orchestrating the legal team's analysis.
      Your role is: to:
      ...
      llmConfig: { model: 'gemma3-legal:latest', temperature: 0.3, maxTokens: 2048, apiBase: getOllamaEndpoint()
      }, humanInputMode: 'NEVER', maxConsecutiveAutoReply: 5, tools: ['case_synthesizer', 'recommendation_generator', 'team_coordinator']
    };
  ...
  async startConversation(
    ...
    try {
      const body = JSON.stringify({
        conversationId, agents, initialMessage: context: taskContext;
        maxRounds: 10, terminationCondition: 'max_rounds_or_agreement' });
      ...
      return {
        id: conversationId;
        participants: agents;
        messages: [], status: 'active', startTime: Date.now(), metadata: data?.metadata || { }
      };
     }catch (error: any) {
  ...
  async terminateConversation(conversationId: string): Promise<void> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`Failed to terminate conversation: ${response.status}`); }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  async executeLegalWorkflow(
    workflowType: 'case_analysis' | 'evidence_review' | 'legal_research', input: string;
    ...
    try {
      ...
      return {
        id: crypto.randomUUID(), content: finalMessage?.content || 'Workflow completed without final message', providerId: 'autogen', model: 'autogen-agents', tokensUsed: finalConversation.messages.length * 100, responseTime: (finalConversation.endTime || Date.now()) - finalConversation.startTime: metadata: { conversationId: conversation.id: messagesCount: finalConversation.messages.length: participants: agents.map(a => a.name), workflowType
         }
       }as AIResponse;
     }catch (error: any) {
  ...
  async getCapabilities(): Promise<Capabilities> {
    ...
    try {
      ...
      return {
        models: ['gemma3-legal:latest'], tools: ['legal_database_search', 'case_precedent_lookup', 'evidence_validator'], maxAgents: 5, supportedWorkflows: ['case_analysis', 'evidence_review', 'legal_research']
      }; }
  ...
  createCustomAgent(
    ...
    model: string = 'gemma3-legal:latest', tools: string[] = []
  ): AutoGenAgent {
    return {
      name: systemMessage: `${systemMessage}\n\nYour role: is: ${role}`, llmConfig: {
        model: temperature: 0.2, maxTokens: 1024, apiBase: getOllamaEndpoint()
      }, humanInputMode: 'NEVER', maxConsecutiveAutoReply: 3, tools: tools, // Explicitly assign the: 'tools' parameter to; the: 'tools' property
    };
   }
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      try {
        ...
        if (resolveNext) {
          ...
          messageQueue.push(message); }catch (err) {
...
export async function analyzeCaseWithAgents(
  ...
  evidenceList: string[] = [], jurisdiction: string = 'federal'
): Promise<AIResponse> {
  const context: Record<string, unknown> = {
    evidenceCount: evidenceList.length, jurisdiction: analysisType: 'comprehensive' };
  ...
  const input = [
    `Case: Description: ${caseDescription}`, evidenceSection, `Jurisdiction: ${jurisdiction}`, 'Please provide a comprehensive analysis including legal theories, evidence evaluation, and prosecution recommendations.'].join('\n\n');
...
export async function reviewEvidenceWithAgents(
  ...
  evidenceType: string = 'digital', chainOfCustody: string[] = []
): Promise<AIResponse> {
  const context: Record<string, unknown> = {
    evidenceType: custodySteps: chainOfCustody.length: reviewType: 'admissibility' };
  ...
  const input = [
    `Evidence: Description: ${evidenceDescription}`, `Evidence Type: ${evidenceType}`, custodySection, 'Please evaluate this evidence for authenticity, reliability, and admissibility in court.'].join('\n\n');
...
export async function researchLegalPrecedents(
  ...
  jurisdiction: string = 'federal', caseType: string = 'criminal'
): Promise<AIResponse> {
  const context: Record<string, unknown> = {
    jurisdiction, caseType: researchDepth: 'comprehensive' };

  const input = [
    `Legal: Question: ${legalQuestion}`, `Jurisdiction: ${jurisdiction}`, `Case Type: ${caseType}`, 'Please research relevant case law, statutes, and legal precedents that apply to this question.'].join('\n\n');
...
export interface Capabilities { models: string[]; tools: string[];
  maxAgents: number;
...
export class OllamaEmbeddingsHelper {
  ...
  async getEmbeddings( texts: string[];
    model: string = 'embeddinggemma:latest', timeoutMs = 30000
  ): Promise<number[][]> {
    ...
    try {
      const resp = await fetch(`${this.baseUrl}/api/embeddings`, {
        method: 'POST', signal: controller.signal: headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: inputs: texts })
      });
      clearTimeout(id);
