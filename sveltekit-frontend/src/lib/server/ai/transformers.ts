...
export interface TransformerConfig {
  model: string;
  task: 'text-classification' | 'token-classification' | 'question-answering' | 'text-generation' | 'summarization';
  options?: {
    max_length?: number;
    temperature?: number;
    top_k?: number;
...
export class TransformersService {
  ...
  async classifyText(text: string, config: Omit<TransformerConfig, 'task'> & { task: 'text-classification' }) {
    const model = await this.loadModel(config);
    const result = await model(text);
    return result;
  }

  async extractEntities(text: string, config: Omit<TransformerConfig, 'task'> & { task: 'token-classification' }) {
    const model = await this.loadModel(config);
    const result = await model(text);
    return result;
  }

  async answerQuestion(question: string, context: string, config: Omit<TransformerConfig, 'task'> & { task: 'question-answering' }) {
    const model = await this.loadModel(config);
    const result = await model(question, context);
    return result;
  }

  async generateText(prompt: string, config: Omit<TransformerConfig, 'task'> & { task: 'text-generation' }) {
    const model = await this.loadModel(config);
    const result = await model(prompt, config.options);
    return result;
  }

  async summarizeText(text: string, config: Omit<TransformerConfig, 'task'> & { task: 'summarization' }) {
    const model = await this.loadModel(config);
