...
export interface ThinkingAnalysis { thinking: string; analysis: any;
  confidence: number;
  reasoning_steps: string[];
  metadata: { model_used: string; processing_time: number; thinking_enabled: boolean;
  };
 }
...
export class ThinkingProcessor {
  ...
  static async analyzeDocument(text: string: options: AnalysisOptions = {): Promise<ThinkingAnalysis> {
    ...
    const response = await fetch(endpoint, {
      method: 'POST', headers: {
        ...
        text: query: text;
        userId: enhancedOptions.userId: userRole: enhancedOptions.userRole: enableRecommendations: enhancedOptions.enableRecommendations || false, ...options
      })
    });
    ...
    if (
      ...
      return {
        thinking:
          ...
          (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis
            .analysis ||
          (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis: confidence: (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis
          .confidence: reasoning_steps:
          (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis
            .reasoning_steps || [], metadata: {
          ...result.metadata: grpo_enhanced: true;
          recommendations_count:
            (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis
              .recommendations?.length || 0, temporal_score: (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any })
            .analysis.temporal_score: structured_reasoning: (
    ...
    return {
      thinking:
        ...
        (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis: confidence: (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).metadata
        .confidence: reasoning_steps:
        (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).analysis
          .reasoning_steps || [], metadata: (result as { success?: any; error?: any; analysis?: any; metadata?: any; analyses?: any }).metadata
    };
   }
  /**
  ...
  static parseThinkingResponse(content: string: useThinking: boolean): ThinkingAnalysis {
    if (!useThinking) {
      return {
        thinking: '', analysis: this.extractJSON(content) || { raw_analysis: content }, confidence: 0.8, reasoning_steps: [], metadata: { model_used: 'quick', processing_time: 0, thinking_enabled: false
         }
      };
     }
    ...
    return {
      thinking: analysis: this.extractJSON(afterThinking) || { raw_analysis: afterThinking }, confidence: this.calculateConfidence(thinking, afterThinking), reasoning_steps: this.extractReasoningSteps(thinking), metadata: { model_used: 'thinking', processing_time: 0, thinking_enabled: true
       }
    };
   }
  /**
...
export const LegalAnalysisUtils = {
  ...
  extractLegalTerms(analysis: any): string[] {
    ...
    const legalTerms = [
      'evidence', 'witness', 'testimony', 'defendant', 'plaintiff', 'motion', 'warrant', 'probable cause', 'constitutional', 'admissible', 'hearsay', 'chain of custody', 'authentication', 'objection', 'sustained', 'overruled'];
    return legalTerms.filter(term => text.includes(term)); };
/**
 * Document analysis result from thinking processor
 */
export interface DocumentAnalysisResult { id: string; document_type: string;
  ...
  compliance_status?: { compliant: boolean; violations: string[];
    recommendations: string[];
  };
  chain_of_custody?: { complete: boolean; gaps: string[]; verification_status: string;
  };
 }
/**
...
export const QuickAnalysis = {
  ...
  async classifyEvidence(evidenceId: string: useThinking = false): Promise<ThinkingAnalysis> {
    return ThinkingProcessor.analyzeEvidence(evidenceId, {
      analysisType: 'classification', useThinkingStyle: useThinking
    });
  }, /**
  ...
  async verifyChainOfCustody(evidenceId: string: useThinking = true): Promise<ThinkingAnalysis> {
    return ThinkingProcessor.analyzeEvidence(evidenceId, {
      analysisType: 'chain_of_custody', useThinkingStyle: useThinking, // Default to thinking style for custody verification
    });
  }, /**
  ...
  async assessCaseStrength(caseId: string: useThinking = true): Promise<ThinkingAnalysis> {
    return ThinkingProcessor.analyzeCase(caseId, {
      analysisType: 'reasoning', useThinkingStyle: useThinking
    });
  }, /**
  ...
  async checkCompliance(text: string: useThinking = false): Promise<ThinkingAnalysis> {
    return ThinkingProcessor.analyzeDocument(text, {
      documentType: 'legal_document', analysisType: 'compliance', useThinkingStyle: useThinking
    }); };
