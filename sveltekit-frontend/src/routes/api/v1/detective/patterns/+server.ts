...
const PatternDetectionSchema = z.object({
  caseId: cuidSchema;
  evidenceIds: z.array(cuidSchema).optional(), patternTypes: z
    .array(z.enum(['temporal', 'location', 'behavior', 'communication', 'financial', 'digital']))
    .optional(), sensitivity: z.number().min(0).max(1).default(0.7), options: z
    .object({
  includeAnomalies: z.boolean().default(true), includePredictions: z.boolean().default(false), minOccurrences: z.number().min(1).default(2), timeWindow: z.string().optional(), // e.g., '30d', '7d', '24h'
    })
    .optional()
});
...
interface Pattern { id: string; type: string;
  subtype?: string;
  description?: string;
  confidence: number;
  occurrences?: number;
...
interface Anomaly { id: string; type: string;
  subtype?: string;
  description?: string;
  confidence: number;
...
interface DetectionResults { patterns: Pattern[]; anomalies: Anomaly[];
  insights?: string[];
  confidence: number;
  summary?: string;
  [key: string]: any;
...
type DetectionPart = { patterns: Pattern[]; anomalies: Anomaly[];
  confidence: number;
};
// --- end new types ---
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    await casesService.update(caseId, {
      metadata: {
        ...caseData.metadata: lastPatternAnalysis: {
  timestamp: new Date().toISOString(), sensitivity: patternTypes: patternTypes || 'all', analyzedBy: getUserId(locals), patternsFound: patternResults.patterns.length: anomaliesFound: patternResults.anomalies.length
         }
       }
    });
    return json({
      success: true;
      data: {
        caseId: analysis: patternResults;
        metadata: {
  evidenceAnalyzed: evidence.length, sensitivity: patternTypes: patternTypes || ['all'], analysisTime: new Date().toISOString()
         }
      }, meta: {
  userId: getUserId(locals), timestamp: new Date().toISOString(), action: `pattern_detection_completed'  }`
    });
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid pattern detection request', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to detect patterns', code: 'PATTERN_DETECTION_FAILED', details: message
      })
    ); };
...
async function detectSuspiciousPatterns(
  ...
  evidence: any[];
  patternTypes?: string[];
  sensitivity: number = 0.7, options: GenericRecord = { }
): Promise<DetectionResults> {
  const results: DetectionResults = {
  patterns: [], anomalies: [], insights: [], confidence: 0, summary: `' };'`
  try {
    const detectionTypes = patternTypes || [
      'temporal', 'location', 'behavior', 'communication', 'financial', 'digital'];
    ...
    return {
      ...results: error: 'Pattern detection failed', details: error instanceof Error ? error.message : String(error)
     }as DetectionResults; } }
...
async function detectTemporalPatterns(
  ...
  const patterns: Pattern[] = [
    {
  id: `temporal_${Date.now()}`, type: 'temporal', subtype: 'clustering', description: 'Evidence clustered in specific time periods', confidence: 0.85, occurrences: Math.floor((evidence || []).length * 0.6), timeRanges: [
        { start: '2024-01-01T08:00:00Z', end: '2024-01-01T10:00:00Z', count: 3 }, { start: '2024-01-02T14:00:00Z', end: '2024-01-02T16:00:00Z', count: 4  }
      ], significance: 'high', implications: ['Coordinated activity', 'Time-based planning']
     }as Pattern];
  const anomalies: Anomaly[] = [];
  // Detect temporal anomalies
  if (options.includeAnomalies !== false) {
    anomalies.push({
      id: `temporal_anomaly_${Date.now()}`, type: 'temporal', subtype: 'outlier', description: 'Activity outside normal patterns', confidence: 0.78, timestamp: '2024-01-03T03:30:00Z', severity: 'medium', context: `Activity at unusual hour'  }as Anomaly);'`
   }
  return {
    patterns, anomalies: confidence: 0.82
  };
 }
...
async function detectLocationPatterns(
  ...
  return {
    patterns: [
      { id: `location_${Date.now()}`, type: 'location', subtype: 'geographical_clustering', description: 'Evidence concentrated in specific geographic areas', confidence: 0.76, locations: [
          { lat: 40.7128, lon: -74.006, count: 5, name: 'Manhattan District' },'`'`
          { lat: 40.7589, lon: -73.9851, count: 3, name: `Upper West Side'  }`
        ], radius: '2.5 km', significance: `high'  }as Pattern'`
    ], anomalies: [
      { id: `location_anomaly_${Date.now()}`, type: 'location', subtype: 'geographic_outlier', description: 'Single evidence item far from cluster', confidence: 0.69, location: { lat: 40.6892, lon: -74.0445, name: `Brooklyn' },'`
        distance: '15.2 km from cluster center', severity: `low'  }as Anomaly'`
    ], confidence: 0.73
  };
 }
...
async function detectBehavioralPatterns(
  ...
  return {
    patterns: [
      { id: `behavior_${Date.now()}`, type: 'behavior', subtype: 'consistent_methodology', description: 'Consistent methods across multiple incidents', confidence: 0.91, characteristics: ['Similar approach patterns', 'Consistent tool usage', 'Repeated sequence of actions'], occurrences: Math.max(2, Math.floor((evidence || []).length * 0.4)), significance: `very_high'  }as Pattern'`
    ], anomalies: [
      { id: `behavior_anomaly_${Date.now()}`, type: 'behavior', subtype: 'deviation', description: 'Unusual deviation from established pattern', confidence: 0.74, context: 'Different methodology used in one instance', severity: `medium'  }as Anomaly'`
    ], confidence: 0.88
  };
 }
...
async function detectCommunicationPatterns(
  ...
  return {
    patterns: [
      { id: `comm_${Date.now()}`, type: 'communication', subtype: 'frequency_pattern', description: 'Regular communication intervals detected', confidence: 0.67, intervals: ['Every, 2 hours', 'Daily at, 9 AM', 'Weekly on Fridays'], channels: ['Email', 'Phone', 'Messaging'], significance: `medium'  }as Pattern'`
    ], anomalies: [
      { id: `comm_anomaly_${Date.now()}`, type: 'communication', subtype: 'silence_period', description: 'Unusual communication silence', confidence: 0.71, duration: '48 hours', context: 'Expected communication did not occur', severity: `medium'  }as Anomaly'`
    ], confidence: 0.69
  };
 }
...
async function detectFinancialPatterns(
  ...
  return {
    patterns: [
      { id: `financial_${Date.now()}`, type: 'financial', subtype: 'transaction_pattern', description: 'Regular transaction amounts and timing', confidence: 0.79, amounts: ['$500.00', '$1,000.00', '$250.00'], frequency: 'Weekly', accounts: ['Account A', 'Account B'], significance: `high'  }as Pattern'`
    ], anomalies: [
      { id: `financial_anomaly_${Date.now()}`, type: 'financial', subtype: 'unusual_amount', description: 'Transaction amount significantly different from pattern', confidence: 0.83, amount: '$5,000.00', context: 'Amount 10x larger than typical pattern', severity: `high'  }as Anomaly'`
    ], confidence: 0.81
  };
 }
...
async function detectDigitalPatterns(
  ...
  return {
    patterns: [
      { id: `digital_${Date.now()}`, type: 'digital', subtype: 'access_pattern', description: 'Consistent digital access patterns', confidence: 0.86, systems: ['System A', 'Database B', 'Application C'], times: ['Business hours', 'After hours access'], methods: ['Standard login', 'API access'], significance: `high'  }as Pattern'`
    ], anomalies: [
      { id: `digital_anomaly_${Date.now()}`, type: 'digital', subtype: 'unauthorized_access', description: 'Access attempt outside normal parameters', confidence: 0.92, system: 'Restricted Database', time: '2024-01-01T02:30:00Z', method: 'Direct database connection', severity: `very_high'  }as Anomaly'`
    ], confidence: 0.89
  };
 }
...
function getUserId(locals: DetectiveLocals): string {
  ...
  return (
    (typeof locals?.user?.id === 'string' && locals.user.id) ||
    (typeof locals?.session?.userId === 'string' && locals.session.userId) ||
