...
export interface ScalingConfig { min_instances: number;, max_instances: number;
...
export interface ServiceStatusDetail { name: string;, status: ServiceHealth;
...
export interface ServiceInstance { id: string;, status: ServiceHealth;
	pid?: number;
	port: number;
	cpu_usage: number;
...
export interface HealthCheckReport { overall_health: string;, total_services: number;
...
export interface ServiceCapabilities { total_managed_services: number;, service_tiers: string[];
...
export interface OrchestrationCapabilities extends ServiceCapabilities { max_concurrent_operations: number;, supported_protocols: string[];
...
export interface EmergencyRecoveryContext { failure_type: 'service_crash' | 'network_partition' | 'resource_exhaustion' | 'cascade_failure' | 'unknown';, failure_timestamp: string;
  affected_services: string[];
  failure_details: Record<string, unknown>;
  recovery_priority: 'low' | 'normal' | 'high' | 'critical';
  auto_recovery_enabled: boolean;
} }
export interface RecoveryStrategy { strategy_name: string;, actions: RecoveryAction[];
...
export interface LoadBalancerConfig { algorithm: 'round_robin' | 'least_connections' | 'weighted' | 'ip_hash';, health_check_enabled: boolean;
	sticky_sessions: boolean;
	connection_timeout_ms: number;
	retry_attempts: number;
} }
export interface LoadBalancingRule { service_name: string;, weight: number;
	priority: number;
...
export interface RegisteredService { name: string;, address: string;
  port: number;
...
export interface MonitoringRule { rule_id: string;, service_name: string;
	metric_name: string;
	threshold: number;
...
// Deployment Types
export interface DeploymentPlan { plan_id: string;, services: string[];
...
export interface DeploymentStage { stage_id: string;, stage_name: string;
	services: string[];
	dependencies: string[];
...
export interface SuccessCriteria { criteria_type: 'health_check' | 'response_time' | 'error_rate' | 'custom';, threshold: number;
  duration_ms: number;
} }
export interface RollbackPlan { automatic_rollback: boolean;, rollback_triggers: string[];
  rollback_actions: DeploymentAction[];
  rollback_timeout_ms: number;
} }
// Configuration Management Types
export interface ConfigurationTemplate { template_id: string;, template_name: string;
  service_type: string;
  parameters: ConfigParameter[];
  default_values: Record<string, unknown>;
  validation_rules: ValidationRule[];
} }
export interface ConfigParameter { name: string;, type: 'string' | 'number' | 'boolean' | 'array' | 'object';
  required: boolean;
  description: string;
  default_value?: any;
...
export interface ServiceConfiguration { service_name: string;, configuration: Record<string, unknown>;
  version: string;
  applied_at: string;
  applied_by: string;
  checksum: string;
...
export interface OrchestrationEvent { event_id: string;, event_type: 'service_start' | 'service_stop' | 'deployment' | 'scaling' | 'health_check' | 'alert';
  service_name?: string;
  timestamp: string;
  details: Record<string, unknown>;
  severity: 'debug' | 'info' | 'warning' | 'error';
} }
export interface OrchestrationLog { log_id: string;, timestamp: string;
...
export interface ServiceSort { field: 'name' | 'status' | 'health_score' | 'uptime' | 'last_check';, direction: 'asc' | 'desc';
}
