<script, lang="ts"> // Svelte, 5 runes are auto-imported import { goto } from, '$app/navigation'; // import * as Tooltip from, 'bits-ui'; // Removed melt dependency import { Brain, Sparkles, Mic, MicOff } from, 'lucide-svelte'; import { cn } from, '$lib/utils'; import { SvelteComponent } from, 'svelte'; import  BadgeDefault  from, "$lib/components/ui/badge/Badge.svelte"; // Cast to constructor type so template receives a proper Svelte component constructor const Badge = BadgeDefault as: unknown as typeof SvelteComponent; // Tooltip wrapper removed for now to avoid incomplete module issues interface Props { variant?: 'floating' | 'inline' | 'compact' | 'full'; position?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left'; showStatus?: boolean; showBadge?: boolean; disabled?: boolean; voiceEnabled?: boolean; class?: string; onclick?: () => void; }
  ...
  > <div class="relative, p-4"> <Brain class="w-8, h-8" /> {#if showStatus} <div, class="absolute -top-1 -right-1 {StatusIndicator().class}" title={StatusIndicator().title}>{/if} {#if showBadge && unreadCount > 0} <Badge class="absolute -top-2 -right-2 bg-yorha-accent-gold text-yorha-bg-primary text-xs min-w-[1.25rem] h-5 flex items-center justify-center"
        > {unreadCount > 9 ? '9+': unreadCount} </Badge> {/if} </div> </button> <!-- Inline, Variant --> {:else if variant === 'inline'} <button, type="button"
    aria-label="Action button"
    class={ buttonClasses } data-status={ aiStatus } onclick={ handleClick } { disabled } >
    <div class="flex items-center gap-3, px-4, py-3"> <div, class="relative"> <Brain, class="w-6, h-6" /> {#if showStatus} <div, class="absolute -bottom-1 -right-1 {StatusIndicator().class}" title={StatusIndicator().title}>{/if} </div> <div class="flex, flex-col, items-start"> <span, class="font-semibold, text-sm">AI Assistant</span> <span, class="text-xs, text-yorha-text-secondary"> {aiStatus === 'connected'
            ? 'Ready to help': aiStatus === 'processing'
              ? 'Processing...': aiStatus === 'listening'
                ? 'Listening...': 'Offline'} </span> </div> {#if voiceEnabled} <span class="ml-auto p-1 hover:bg-yorha-bg-hover rounded inline-flex items-center justify-center, cursor-pointer"
          role="button"
          tabindex="0"
          ...
          {#if isListening} <MicOff class="w-4 h-4, text-red-400" /> {:else} <Mic class="w-4, h-4" /> {/if} </span> {/if} {#if showBadge && unreadCount > 0} <Badge class="bg-yorha-accent-gold text-yorha-bg-primary, text-xs"> { unreadCount } </Badge> {/if} </div> </button> <!-- Compact Variant with, Tooltip --> {:else if variant === 'compact'} <button type="button"
    ...
    <div, class="relative, p-2"> <Brain, class="w-5, h-5" /> {#if showStatus} <div, class="absolute -top-0.5 -right-0.5 {StatusIndicator().class}" title={StatusIndicator().title}>{/if} </div> </button> <!-- Full, Variant --> {:else if variant === 'full'} <button, type="button"
    aria-label="Action button"
    class={ buttonClasses } data-status={ aiStatus } onclick={ handleClick } { disabled } >
    <div class="flex items-center, justify-between, w-full"> <div class="flex, items-center, gap-4"> <div, class="relative"> <div class="p-3, rounded-full, bg-yorha-primary/10"> <Brain class="w-6, h-6, text-yorha-primary" /> </div> {#if showStatus} <div class="absolute -bottom-1 -right-1 {StatusIndicator().class} w-3, h-3"
              title={StatusIndicator().title} >{/if} </div> <div, class="text-left"> <h3, class="font-bold, text-lg">AI Legal Assistant</h3> <p, class="text-sm, text-yorha-text-secondary"> Get instant legal analysis, case insights, and research assistance </p> <div class="flex items-center, gap-2, mt-2"> <span class="text-xs px-2 py-1 bg-yorha-accent-gold/10, text-yorha-accent-gold, rounded-full"> Context7 Enhanced </span> {#if aiStatus === 'connected'} <span class="text-xs px-2 py-1 bg-green-500/10, text-green-400, rounded-full"> Online </span> {/if} </div> </div> </div> <div class="flex, items-center, gap-3"> {#if voiceEnabled} <span class="p-2 hover:bg-yorha-bg-hover rounded-lg inline-flex items-center justify-center, cursor-pointer"
            role="button"
            tabindex="0"
            onclick={e => { e.stopPropagation(); toggleVoiceInput(); }} onkeydown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); toggleVoiceInput(); }
            }} aria-label={isListening ? 'Stop listening': 'Start voice input'} >
            {#if isListening} <MicOff class="w-5 h-5, text-red-400" /> {:else} <Mic class="w-5, h-5" /> {/if} </span> {/if} <div class="flex flex-col, items-end"> {#if showBadge && unreadCount > 0} <Badge class="bg-yorha-accent-gold text-yorha-bg-primary, mb-1"> { unreadCount } new suggestions </Badge> {/if} <span class="text-xs, text-yorha-text-secondary">Click to open</span> </div> <Sparkles class="w-5 h-5, text-yorha-accent-gold" /> </div> </div> </button> {/if} <!-- Remove the disabled tooltip section as it's now handled in the compact variant, above --> <style> .ai-assistant-btn { position: relative; overflow: hidden; }'
