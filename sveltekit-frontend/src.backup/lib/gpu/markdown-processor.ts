...
export class GPUMarkdownScanner {
  ...
  async scanMarkdown(text: string): Promise<{
    ...
    headingPass.setBindGroup(0, this.device!.createBindGroup({
    ...
    sectionPass.setBindGroup(0, this.device!.createBindGroup({
...
export class GPUMarkdownProcessor {
  ...
  private scanMarkdownCPU(text: string): { headings: Array<{ position: number; level: number }>; sections: Array<{ position: number; type: string }> } {
    const headings: Array<{ position: number; level: number }> = [];
    const sections: Array<{ position: number; type: string }> = [];
    ...
    for (const line of lines) {
      ...
      if (headingMatch) {
        ...
        else if (title.includes('reasoning') || title.includes('analysis')) sections.push({ position: currentPos, type: 'reasoning' });
        else if (title.includes('holding') || title.includes('decision')) sections.push({ position: currentPos, type: 'holding' });
        else if (title.includes('conclusion') || title.includes('result')) sections.push({ position: currentPos, type: 'conclusion' });
      }
  ...
  private createSectionsCPU(
    ...
    const allMarkers = [...headings.map(h => ({ ...h, type: 'heading' as const })),
    ...
    for (let i = 0; i < allMarkers.length; i++) {
      ...
      result.push({
        type: marker.type,
        level: 'level' in marker ? (marker as any).level : 1,
