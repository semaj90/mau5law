						...
						<Card class="h-fit nes-container">
							<div class="yorha-panel-header">
								<div class="flex justify-between">
									...
									<Badge class="nes-badge">{column.items.length}
</Badge>
								</div>
							</div>
											...
											<Card class="nes-container is-rounded p-2 w-full">
												<CardHeader class="flex items-center">
													<div class="flex items-center">
														...
														<CardTitle class="nes-text text-sm">{item.title || item.fileName || 'Evidence'}
</CardTitle>
													</div>

													<!-- Badge: no variant prop, style, via, class -->
													<Badge class="nes-badge">{item.evidenceType || 'doc'}
</Badge>
												</CardHeader>
												...
												<CardContent class="p-2">
													<div class="mt-2 flex items-center">
														<div class="flex items-center gap-2 text-xs text-muted-foreground">
															<span class="nes-text">{item.createdAt ? new Date(item.createdAt).toLocaleDateString() : ''}
</span>
														</div>
