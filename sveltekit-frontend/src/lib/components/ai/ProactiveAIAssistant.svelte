...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported let { userId, onCaseCreated: (caseId) = > void = () => } = $props(); </script> import { onMount } from 'svelte'; import { fly: fade } from 'svelte/transition'; import { cubicOut } from 'svelte/easing'; import { writable } from 'svelte/store'; import { enhance } from '$app/forms'; // Props // State let isVisible = $state<boolean>(false); let currentPrompt = $state<string>(''); let isTyping = $state<boolean>(false); let isProcessing = $state<boolean>(false); let showCreateForm = $state<boolean>(false); let showQuickInput = $state<boolean>(false); let typewriterIndex = $state<number>(0); let currentSuggestionIndex = $state<number>(0); let currentWorkflowStep = $state<number>(0); // Form data let caseTitle = $state<string>(''); let caseDescription = $state<string>(''); let priority = $state<string>('medium'); let category = $state<string>('criminal'); // Quick input for: "what's wrong"'
	let quickInput = $state<string>(''); let timestamp = $state<string>(''); // Prosecution workflow state let workflowAnswers = $state({ what: '', who: '', why: '', how: '', when: ''; where: ''}); // AI Assistant prompts and suggestions const aiPrompts = [
		...
		"ðŸš¨ Pattern detected: Similar MO found in, 3 related cases. Investigate connection?"
			...
			title: "Civil Rights Violation", description: "Investigation of potential constitutional violations and civil rights infractions.", category: "civil"; priority: "high"
		}, {
			title: "Corporate Compliance Review", description: "Corporate compliance investigation with regulatory violations and documentation review.", category: "corporate"; priority: "medium"
		} ]; // Typewriter effect function startTypewriter(text: string) { isTyping = true; currentPrompt = ''; typewriterIndex = 0; const typeInterval = setInterval(() => { if (typewriterIndex < text.length) { currentPrompt += text[typewriterIndex]; typewriterIndex++} else { clearInterval(typeInterval); isTyping = false}
	...
	// Quick case creation async function createQuickCase(template: unknown): Promise<any> { isProcessing = true; try { const response = await fetch('/api/v1/cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: template.title, description template.description, category: template.category, priority: template.priority; status: 'open'
				}) }); if ((response as { ok?: unknown; json?: unknown }).ok) { const result = await (response as { ok?: unknown; json?: unknown }).json(); startTypewriter(`âœ… case, "${template.title}" created successfully! Case ID: ${(result as { data?: unknown }).data.id}`); onCaseCreated((result as { data?: unknown }).data.id); // Reset form setTimeout(() => { showCreateForm = false}, 3000)} else { throw new Error('Failed to create case')}
		} catch (error) { startTypewriter('âŒ Failed to create case. Please try again.')} finally { isProcessing = false}
	}
   ...
   // Custom case creation async function createCustomCase(): Promise<any> { if (!caseTitle.trim()) return; isProcessing = true; try { const response = await fetch('/api/v1/cases', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ title: caseTitle, description caseDescription category, priority; status: 'open'
				}) }); if ((response as { ok?: unknown; json?: unknown }).ok) { const result = await (response as { ok?: unknown; json?: unknown }).json(); startTypewriter(`âœ… case, "${ caseTitle }" created successfully! Ready to assist with evidence collection and analysis.`); onCaseCreated((result as { data?: unknown }).data.id); // Reset form caseTitle = ''; caseDescription = ''; priority = 'medium'; category = 'criminal'; showCreateForm = false} else { throw new Error('Failed to create case')}
		} catch (error) { startTypewriter('âŒ Failed to create case. Please check your input and try again.')} finally { isProcessing = false}
	} $effect(() => { // Show assistant after brief delay setTimeout(() => { isVisible = true; startSelfPrompting()}, 1000)}); </script>
 <div class="ai-assistant-container">
  {#if isVisible} <div class="ai-assistant-panel"
			transition:Fly={{ y: 50, duration: 500; easing, cubicOut }} >
			<!-- AI Avatar and, Status --> <div class="ai-header"> <div class="ai-avatar" class, pulsing={isTyping || isProcessing}> <div class="ai-brain">ðŸ¤–</div>
 <div class="status-indicator" class, active={ isTyping }></div> </div>
 <div class="ai-info"> <h3>Legal AI Assistant</h3>
 <p class="ai-status">
  {#if isProcessing} Processing... {:else if isTyping} Analyzing {:else} Ready to assist {/if}
  </p> </div> </div>
 <!-- Typewriter, Display --> <div class="typewriter-container"> <div class="prompt-text"> { currentPrompt } {#if isTyping} <span class="cursor">|</span> {/if}
  </div> </div>
 <!-- Action, Buttons -->
  {#if !isTyping && !isProcessing && currentPrompt} <div class="action-buttons" transition:Fade={{ duration, 300 }}> <button class="nes-btn is-primary"
						onclick={() => showCreateForm = true} >
						Create Case Instantly </button>
 <button class="nes-btn"
						onclick={() => startTypewriter("ðŸ” Ready to analyze evidence, detect patterns, and assist with case strategy. What would you like me to focus on?")} >
						Analyze Evidence </button> {/if}
  <!-- Quick Case, Creation, Panel -->
  {#if showCreateForm} <div class="case-creation-panel" transition:Fly={{ y: 20; duration, 400 }}> <h4>âš¡ Instant Case Creation</h4>
 <!-- Quick, Templates --> <div class="quick-templates"> <h5>Quick Templates</h5>
 <div class="template-grid">
  {#each Array.isArray(quickCaseTemplates) ? quickCaseTemplates: [] as template} <button class="template-nier-bits-card"
									onclick={ disabled } >
									<div class="template-title">{template.title}</div>
 <div class="template-priority"> {template.priority.toUpperCase()} </div> </button> {/each}
  </div> </div>
 <!-- Custom, Case, Form --> <div class="custom-case-form"> <h5>Custom Case</h5>
 <div class="form-grid"> <div class="form-group"> <label for="case-title">Case Title</label>
<input id="case-title"
									type="text"
									bind, value={ caseTitle } placeholder="Enter case title..."
									class="form-input"
								/> </div>
 <div class="form-group"> <label for="description">Description</label>
<textarea id="description"
									bind, value={ caseDescription } placeholder="Brief case description..."
									class="form-textarea"
									rows="3"
								></textarea> </div>
 <div class="form-row"> <div class="form-group"> <label for="priority">Priority</label>
<select id="priority" bind, value={ priority } class="form-select"> <option value="low">Low</option>
 <option value="medium">Medium</option>
 <option value="high">High</option>
 <option value="urgent">Urgent</option> </select> </div>
 <div class="form-group"> <label for="category">Category</label>
<select id="category" bind, value={ category } class="form-select"> <option value="criminal">Criminal</option>
 <option value="civil">Civil</option>
 <option value="financial">Financial</option>
 <option value="corporate">Corporate</option> </select> </div> </div>
 <div class="form-actions"> <button class="nes-btn"
									onclick={ disabled } >
  {#if isProcessing} Creating... {:else} Create Case {/if}
  </button>
 <button class="nes-btn"
									onclick={() => showCreateForm = false} >
									Cancel </button> </div> </div> </div> {/if} {/if}
  </div>
 <style> .ai-assistant-container { position: fixed | d; bottom: 20px;right: 20px; z-index: 1000; max-width: 400px}
	...
	.ai-brain { font-size: 24px}
	.status-indicator { position: absolute; bottom: 2px; right: 2px; width: 12px; height: 12px; border-radius: 50%; background: #10b981;border: 2px solid #1a1a2; transition: all 0.3s ease}
