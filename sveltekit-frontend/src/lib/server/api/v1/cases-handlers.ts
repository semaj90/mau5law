...
export async function getCases(user: UserType, request: Request, db: any, schema: any) {
  try {
...
export async function getCase(user: UserType, caseId: string, db: any, schema: any) {
  try {
    const drizzleDb = db as PostgresJsDatabase<typeof schema>;
...
export async function handleCreateCase(user: UserType, request: Request, db: any, schema: any) {
  try {
    ...
    const drizzleDb = db as PostgresJsDatabase<typeof schema>;
    const [newCase] = await drizzleDb.insert(schema.casesTable).values({
      name,
      description,
      userId: user.id,
    }).returning();
