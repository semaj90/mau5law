...
export class ContextFormatter extends BaseService implements IContextFormatter {
	...
	async formatErrorContext(
		...
		try {
			...
			this.log('info', `Formatted text for parsing:\n${formatted.substring(0, 500)}`);
			return formatted;
		} catch (error) {
...
[explanation]`;
			...
			this.log('info', `Prompt formatted successfully (${prompt.length} chars)`);
			this.log('info', `Prompt text for parsing:\n${prompt.substring(0, 500)}`);
			return prompt;
		} catch (error) {
	...
	async parseResponse(response: string): Promise<{ fix: string; explanation: string }> {
		...
		this.log('info', `Parsing LLM response (${response.length} chars)`);
		this.log('info', `Response text for parsing:\n${response.substring(0, 500)}`);
		...
		try {
			...
			if (!fix || !explanation) {
				this.log('warn', `Parsing incomplete - fix: ${fix.length} chars, explanation: ${explanation.length} chars`);
				throw new Error('Invalid response format, missing code or explanation');
			}

			this.log('info', `Response parsed successfully - fix: ${fix.length} chars, explanation: ${explanation.length} chars`);
			this.log('info', `Parsed fix:\n${fix.substring(0, 300)}`);
			this.log('info', `Parsed explanation:\n${explanation.substring(0, 300)}`);
