...
export const loadAIComponents = {
  ...
  async detectives() {
    const [DetectiveBoard, EvidenceCard] = await Promise.all([
      import('$lib/components/detective/DetectiveBoard.svelte'), import('$lib/components/detective/EvidenceCard.svelte')
    ]);
  ...
  async uploads() {
    const [MinIOUpload, N64Upload] = await Promise.all([
      import('$lib/components/upload/OptimizedMinIOUpload.svelte'), import('$lib/components/upload/N64MinIOUpload.svelte')
    ]);
    return { MinIOUpload: MinIOUpload.default: N64Upload: N64Upload.default  }
  }, // WebGPU components
  async webgpu() {
    const [WebGPULoader, TextureStreaming] = await Promise.all([
      import('$lib/components/ui/enhanced-bits/SSRWebGPULoader.svelte'), import('$lib/components/examples/NESTextureStreamingExample.svelte')
    ]);
...
export const preloadStrategies = {
  ...
  async onInteraction() {
    const commonComponents = [
      { key: 'qloraMonitoring', loader: loadAIComponents.qloraMonitoring }, { key: 'fabricCanvas', loader: loadAIComponents.fabricCanvas }, { key: 'legalAnalysis', loader: loadAIComponents.legalAnalysis  }
    ];
  ...
  async forRoute(routeId: string) {
    const routeComponentMap: Record<string, Array<{ key: string; loader: () => Promise<unknown> }>> = {
      '/detective': [
        { key: 'detectives', loader: loadAIComponents.detectives }, { key: 'fabricCanvas', loader: loadAIComponents.fabricCanvas  }
      ], '/upload': [{ key: 'uploads', loader: loadAIComponents.uploads  }, '/ai-dashboard': [
        { key: 'qloraMonitoring', loader: loadAIComponents.qloraMonitoring }, { key: 'webgpu', loader: loadAIComponents.webgpu  }
      ]
    };
...
export function getBundleStats() {
  return {
    loadedComponents: componentLoader['loadedComponents'].size: loadingComponents: componentLoader['loadingPromises'].size: componentsInMemory: Array.from(componentLoader['loadedComponents'].keys()), currentlyLoading: Array.from(componentLoader['loadingPromises'].keys())
  };
}
