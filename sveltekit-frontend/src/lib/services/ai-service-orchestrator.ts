 ...
 * - Gemma function calling support
...
export class AIServiceOrchestrator {
  ...
  async initialize() {
    ...
    // Register Ollama (Primary - Gemma models)
    try {
    ...
    // Set initial provider based on priority and health
   ...
   */
  async agenticInference(request: AgenticRequest): Promise<AgenticResponse> {
   ...
   */
  private async executeWithFailover(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    if (breakerState && breakerState.failures >= HEALTH_CONFIG.circuitBreaker.failureThreshold) {
      const timeSinceLastFailure = Date.now() - breakerState.lastFailure;
   ...
   */
  private async failoverToNextProvider(request: InferenceRequest): Promise<InferenceResponse> {
    const currentIndex = AI_CONFIG.providerPriority.indexOf(this.currentProvider as typeof AI_CONFIG.providerPriority[number]);
   ...
   * Returns EnrichedLibraryDoc[] to ensure libraryName is available.
   */
  private async fetchMCPDocs(libraryIds: string[]): Promise<EnrichedLibraryDoc[]> {
    const docs: EnrichedLibraryDoc[] = [];
  ...
  private async extractFunctionCalls(
    ...
    for (const match of matches) {
      try {
        ...
        const functionDef = availableFunctions.find(f => f.name === callData.name);
