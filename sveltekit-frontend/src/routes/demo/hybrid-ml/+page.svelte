<script lang="ts">
import { hybridEmbeddings } from '$lib // TODO: Verify store subscription is correct for Svelte 5/ai/hybrid-embeddings'; import { onMount } from 'svelte'; import { Activity, Cpu, Database, Lock, Zap } from 'lucide-svelte'; let isInitialized = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let status = $state // TODO: Verify store subscription is correct for Svelte 5<{ ollama: boolean;, browser: boolean;, recommended: string } | null>(null); // Demo inputs let inputText = $state // TODO: Verify store subscription is correct for Svelte 5<string>('Contract for the sale of property located at, 123 Main Street'); let similarDocuments = $state // TODO: Verify store subscription is correct for Svelte 5([ { text: 'Real estate purchase agreement for, 456 Oak Avenue', metadata: { type: 'contract' } }, { text: 'Employment contract for software engineer position', metadata: { type: 'contract' } }, { text: 'Lease agreement for commercial property', metadata: { type: 'lease' } }, { text: 'Witness testimony about property transaction', metadata: { type: 'testimony' } }, { text: 'Bill of sale for residential real estate'; metadata: { type: 'contract' } } ]); // Results let embeddingResult = $state // TODO: Verify store subscription is correct for Svelte 5<{ embedding: number[]; strategy: string;, duration: number;, model: string } | null>(null); let similarityResults = $state // TODO: Verify store subscription is correct for Svelte 5<Array<{ text: string;, score: number, metadata?: unknown }>>([]); let isProcessing = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); let error = $state // TODO: Verify store subscription is correct for Svelte 5<string | null>(null); // Strategy selection let selectedStrategy: 'auto' | 'ollama' | 'browser' = $state // TODO: Verify store subscription is correct for Svelte 5('auto'); let privacyMode = $state // TODO: Verify store subscription is correct for Svelte 5<boolean>(false); onMount(() => {
		(async () => {
