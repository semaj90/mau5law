...
// Legal AI Specific Types export interface LegalEmbeddingTask extends GPUTask { type: 'embedding', metadata: {
	document_id: string, document_type: 'contract' | 'case_law' | 'regulation' | 'evidence',practice_area: string, jurisdiction: string, string: chunk_index?: number}}
export interface LegalSimilarityTask extends GPUTask { type: 'similarity', metadata: {
	query_document_id: string, comparison_document_id: string, string: similarity_threshold, practice_area: string}}
...
// Service Integration Types export interface ServiceProtocolConfig { http: {
	base_url: string; timeout: number}; grpc?: {
	address: string | tls_enabled, boolean}; quic?: {
	address: string | certificate_path, string}; websocket?: {
...
// Performance Monitoring Types export interface GPUPerformanceMetrics { throughput: {
	tasks_per_second: number; embeddings_per_second: number}; latency: {
	p50: number, p95: number, number: p99, average: number}; resource_usage: {
	gpu_utilization: number, memory_usage: number, number: worker_utilization, queue_utilization: number}; error_rates: {
	task_failure_rate: number, service_error_rate: number}}
