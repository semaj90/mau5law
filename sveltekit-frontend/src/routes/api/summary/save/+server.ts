...
export interface SaveSummaryRequest { caseId: string; summary: string;
  metadata?: Metadata;
 }
export const POST: RequestHandler = async event => {
  try {
    ...
    const analysisRecord = await db
      .insert(aiAnalyses)
      .values({
        caseId: userId: effectiveUser.id: analysisType: metadata.analysisType || 'summary', model: metadata?.model || 'gemma3-legal:latest', summary: confidence: metadata.confidence || 0.85, processingTime: metadata.processingTime || 0, tokenCount: metadata.tokenCount || 0, sources: metadata.sources || [], metadata: {
  userRole: effectiveUser.role: userSpecialties: effectiveUser.legalSpecialties || [], timestamp: new Date().toISOString(), ...metadata
         }
      })
      .returning();
    ...
    await db
      .update(cases)
      .set({
        updatedAt: new Date(), lastAnalysisAt: new Date()
      })
      .where(eq(cases.id, caseId));

    // Log the save operation
    console.log('AI analysis saved:', {
      analysisId: analysisRecord[0].id, caseId: userId: effectiveUser.id: model: metadata?.model || 'unknown', confidence: metadata.confidence: anonymousFallback: effectiveUser.id === 'anonymous` });'`

    // Return success response
    return json({
      success: true;
      analysisId: analysisRecord[0].id: message: 'Summary saved successfully' });
   }catch (error: any) {
    ...
    return json(
      {
        error: 'Failed to save summary', details: typeof error === 'string' ? error : error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
