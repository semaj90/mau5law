...
export async function detectCodebaseClusters(
	...
	try {
		...
		// 5 — Update Neo4j
		const { updated, errors: neoErrors } = await updateNeo4jClusters(assignments, validFiles.map(f => f.fileId));
		result.neo4jNodesUpdated = updated;
		result.errors.push(...neoErrors);

		// 6 — Write CouchDB summaries
		result.couchDbDocsWritten = await writeClusters(k, assignments, validFiles, clusterResult.source);
	} catch (err) {
