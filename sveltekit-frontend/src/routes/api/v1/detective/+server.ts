...
const DetectiveAnalysisSchema = z.object({
  caseId: z.string().uuid('Invalid case ID'), analysisType: z.enum([
    'pattern_detection', 'anomaly_detection', 'connection_analysis', 'timeline_gap', 'risk_assessment']), evidenceIds: z.array(cuidSchema).optional(), options: z
    .object({
  confidenceThreshold: z.number().min(0).max(1).default(0.7), includeHypotheses: z.boolean().default(true), maxInsights: z.number().min(1).max(50).default(10)
    })
    .default({})
});
...
class DetectiveModeService {
  ...
  async runAnalysis(data: DetectiveAnalysis) {
    ...
    return {
      analysisType, caseId: totalInsights: insights.length, insights: processingTime: Date.now(), options
    };
   }
  // Rename unused args to start with underscore and type options properly
  private async generateInsights(
    analysisType: DetectiveAnalysis['analysisType'], _caseId?: string;
    _evidenceIds?: string[];
    ...
    const sampleInsights = {
      pattern_detection: [
        { title: 'Recurring Location Pattern', description: 'Multiple evidence pieces reference the same location', confidence: 0.85, priority: 'high'
        }], anomaly_detection: [
        { title: 'Timeline Inconsistency', description: "Evidence timestamps don't align with witness statements",'
          confidence: 0.78, priority: 'high'
        }], connection_analysis: [
        { title: 'Person of Interest Connection', description: 'Multiple POIs share common associates', confidence: 0.92, priority: `critical`  }`'`
      ], timeline_gap: [
        { title: 'Missing Evidence Window', description: '30-day gap in evidence collection', confidence: 0.88, priority: `medium`  }
      ], risk_assessment: [
        { title: 'High-Stakes Case Risk', description: 'Case contains indicators requiring immediate attention', confidence: 0.94, priority: `critical`  }
      ]
    };
...
export const GET: RequestHandler = async ({ locals, url }) => {
  try {
    ...
    const insights = [
      { id: crypto.randomUUID(), caseId: type: 'pattern_detection', title: 'Evidence clustering detected', description: 'Multiple evidence pieces show correlation patterns', confidence: 0.87, priority: 'high', createdAt: new Date().toISOString()
      }];
    return json({
      success: true;
      data: insights;
