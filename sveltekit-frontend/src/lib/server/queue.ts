		...
		try {
			...
			if (isImage) {
				...
				try {
					...
					if (ocrResult.error) {
						console.warn(`[RAG Worker] OCR failed for ${obj.objectName} (${ocrResult.method}): ${ocrResult.error}`);
						text = `Image file: ${obj.objectName} (OCR extraction failed)`;
					} else {
						text = `Image content from ${obj.objectName} (via ${ocrResult.method}):\n${ocrResult.text}`;
						console.log(`[RAG Worker] OCR successful for ${obj.objectName} using ${ocrResult.method}`);
					}
				} catch (ocrError) {
			...
			await client.upsert('phase72_evidence_embeddings', {
				...
				points: [{
					...
					payload: {
						...
						processingEngines: processed.engines,
						timestamp: new Date().toISOString()
					}
				}]
			});

			console.log(`[RAG Worker] Indexed processed file: ${processed.filename} (engines: ${processed.engines.join(', ')})`);
		} catch (error) {
