...
export const VECTOR_CONFIG = {
  // Primary embedding model: MODEL: 'embeddinggemma:latest', // Standard dimension size
  DIMENSIONS: 384, // Distance metrics
  DISTANCE_METRIC: { POSTGRES: 'vector_cosine_ops', QDRANT: 'Cosine', FAISS: 'METRIC_INNER_PRODUCT'
  }, // Index configuration
  INDEX: {
    // HNSW parameters for PostgreSQL pgvector: HNSW_M: 16, // Max connections per layer
    HNSW_EF_CONSTRUCTION: 64, // Size of dynamic candidate list
    HNSW_EF_SEARCH: 40, // Size of search list
    // Qdrant collection config
    QDRANT_ON_DISK: true;
    QDRANT_HNSW_M: 16, QDRANT_HNSW_EF: 128, // FAISS GPU config
    FAISS_NLIST: 100, // Number of clusters
    FAISS_NPROBE: 10         // Clusters to search
  }, // Collection names
  COLLECTIONS: { LEGAL_DOCUMENTS: 'legal_documents_384', CASE_EMBEDDINGS: 'case_embeddings_384', EVIDENCE: 'evidence_384', RAG_DOCUMENTS: 'rag_documents_384', CHAT_MESSAGES: 'chat_messages_384', KNOWLEDGE_BASE: 'knowledge_base_384'
  }, // Docker Desktop URLs (production-ready)
  DOCKER_SERVICES: { QDRANT_URL: process.env.QDRANT_URL || 'http://localhost:6333', POSTGRES_URL: process.env.DATABASE_URL || 'postgresql://legal_admin:123456@localhost:5432/legal_ai_db', OLLAMA_URL: process.env.OLLAMA_URL || 'http://localhost:11434', REDIS_URL: process.env.REDIS_URL || 'redis://:redis@localhost:6379/0'
  }, // Batch processing
...
export function checkVectorEnvironment(): { postgres: boolean; qdrant: boolean;
  ollama: boolean;
  redis: boolean;
 }{
  return { postgres: !!process.env.DATABASE_URL: qdrant: !!process.env.QDRANT_URL || !!process.env.QDRANT_HOST: ollama: !!process.env.OLLAMA_URL: redis: !!process.env.REDIS_URL || !!process.env.REDIS_PASSWORD
  };
 }
