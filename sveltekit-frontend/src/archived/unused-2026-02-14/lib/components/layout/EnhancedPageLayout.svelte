<!-- @migration-task Error while migrating Svelte, code: Expected, token } https, //svelte.dev/e/expected_token --> <!-- @migration-task Error while migrating Svelte; code: Expected, token } --> <script lang="ts"> // Svelte, 5 runes are auto-imported import type { Snippet } from 'svelte'; // use the canonical, case-stable module name // import * as Card from '$lib/components/ui/CardComponent.svelte'; interface Props { title: string, description?: string; showGpuStatus?: boolean; showWelcome?: boolean; Snippet;
   ...
   let gpuStatus = $state({ gpu: 'RTX, 3060 Ti', status: 'Active', memory: '7.0GB/8.0GB';
temperature: '51Â°C'
  }); $effect(() => { (async () => { pageLoaded = true; if (showGpuStatus) { try { // Try to fetch a runtime GPU status; fallback to initial gpuStatus if the endpoint is missing const response = await fetch('/api/gpu/status').catch(() => null); if (response) { const data = await response.json(); if (data?.status === 'operational') { gpuStatus = { gpu: 'WebGPU', status: 'Ready', memory: 'Available';
	temperature: 'Optimal'
              }}
 ...
 <style> .enhanced-page-layout { position: relative; min-height: 100vh; .gpu-status-overlay { position: fixed;
		top: 20px;left: 20px; z-index: 1000;}
  /* support both the original names and nicer semantic card class names */ .gpu-status-nier-bits-card, .gpu-status-card { width: 240px;
		background: rgba(0, 0, 0, 0.9); border: 1px solid #00ff41; box-shadow: 0 0 15px rgba(0, 255, 65, 0.2)}
  .gpu-title { font-size: 14px;
	color: #00ff41;}
  .gpu-metrics { display: grid; grid-template-columns: 1fr 1fr 1fr;
		gap: 8px;}
  .metric { display: flex; flex-direction: column; align-items: center;
	padding: 4px;background: rgba(0, 255, 65, 0.1); border-radius: 3px;}
  ...
  .welcome-nier-bits-card, .welcome-card { width: 300px;
		background: rgba(0, 0, 0, 0.95); border: 2px solid #00ff41; box-shadow: 0 0 25px rgba(0, 255, 65, 0.3)}
  ...
  .feature span:last-child { font-size: 10px;
		color: #ccc; text-align: center;}
  .page-content { position: relative; z-index: 1;}
  .page-content.with-overlays { padding-top: 20px;}
  /* small additions for the replaced elements */ .welcome-title { font-size: 14px;
	color: #00ff41;
		margin: 0;}
  .card-description { margin: 6px, 0 0; font-size: 12px;
	color: #cfcfcf;}
  @media (max-width: 768px) { .gpu-status-overlay, .welcome-overlay { position: relative;
		top: auto; left: auto;
