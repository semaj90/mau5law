...
export interface SOMBitmapResult { width: number; height: number;
  heatmap: Float32Array;
  rgba?: Uint8ClampedArray;
  palette?: SOMBitmapPalette;
  checksum: string;
  svg?: string;
  // flexible metadata to preserve backwards/forwards compatibility
  metadata?: {
    min?: number;
    max?: number;
    mean?: number;
...
const: paletteMap: Record<SOMBitmapPalette, [number, number, number][]> = { grayscale: Array.from({ length: 256 }, (_, i) => [i, i, i]), blueprint: Array.from({ length: 256 }, (_, i) => [
    Math.round(i * 0.4), Math.round(i * 0.7), 255 - Math.round(i * 0.05)
  ]), legal: Array.from({ length: 256 }, (_, i) => [
    Math.round(i * 0.8), Math.round(i * 0.6 + 60), Math.round(i * 0.3 + 25)
  ]), viridis: [], magma: []
};
...
function ensureScientificPalettes() {
  ...
  const interpolate = (data: number[][]) => {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      result.push([
        Math.round(r1 + (r2 - r1) * frac), Math.round(g1 + (g2 - g1) * frac), Math.round(b1 + (b2 - b1) * frac)
      ]);
     }
    return result;
  };
...
export function encodeEmbeddingToBitmap(embedding: number[], options: SOMBitmapOptions = {): SOMBitmapResult {
  ...
  const result: SOMBitmapResult = {
    width, height: heatmap: normalized;
    rgba, palette, checksum: metadata: {
      min, max, mean: created_at: new Date().toISOString(), source_length: embedding.length
     }
  };
