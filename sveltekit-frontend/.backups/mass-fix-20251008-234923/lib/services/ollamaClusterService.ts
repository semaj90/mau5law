...
export class OllamaClusterService {
  ...
  private updateClusterStats(instances: OllamaInstance[]): void {
    ...
    const avgLoad =
      healthy.length > 0 ? healthy.reduce((sum, i) => sum + i.current_load, 0) / healthy.length : 0;
    ...
    clusterStatsStore.set({
      ...
      healthy_instances: healthy.length,
    ...
    try {
      ...
      ollamaClusterStore.update((cfg) => ({
        ...
        instances: cfg.instances.map((i) =>
          i.id === instance.id ? { ...i, response_time: responseTime } : i
        )
      }));
      ...
      ollamaClusterStore.update((cfg) => ({
        ...
        instances: cfg.instances.map((i) =>
          i.id === instance.id
            ? { ...i, status: 'unhealthy', response_time: 9999, last_health_check: Date.now() }
            : i
        )
      }));
