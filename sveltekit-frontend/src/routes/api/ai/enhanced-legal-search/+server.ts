...
function calculateAdvancedAnalytics(
  ...
  if (count > 0) {
    Object.keys(relevanceFactors).forEach((key) => {
      relevanceFactors[key as keyof typeof relevanceFactors] =
        relevanceFactors[key as keyof typeof relevanceFactors] / count;
    });
  }
...
async function generateAIEnhancement(
  ...
  try {
    const resultsSummary = topResults
      .map((r) => `${r.title} (${r.category}, ${r.jurisdiction})`)
      .join('; ');
    return {
...
async function handleSearch(options: {
  ...
  const aiEnhancement =
    useAI && results.length > 0 ? await generateAIEnhancement(query, results.slice(0, 5)) : null;
