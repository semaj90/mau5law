...
<script lang="ts">
  ...
  interface Props {
    variant?: 'window' | 'panel' | 'dialog' | 'card' | 'screen';
  ...
  let containerClasses = $derived(() => {
    ...
    const stateClasses = [
      bordered && 'nes-container--bordered',
...
>
  <!-- Header -->
  {#if title || subtitle || header || closable}
    <div class="nes-container__header">
      <!-- Custom Header Slot -->
      {#if header}
        <div class="nes-container__header-custom">
          {@render header()}
        </div>
      {:else}
        <!-- Default Header -->
        <div class="nes-container__title-section">
          {#if title}
            <h3 class="nes-container__title">{title}</h3>
          {/if}
          {#if subtitle}
  ...
  <!-- Corner Decorations -->
  <div class="nes-container__corners" aria-hidden="true">
    <div class="nes-container__corner nes-container__corner--tl"></div>
    <div class="nes-container__corner nes-container__corner--tr"></div>
    <div class="nes-container__corner nes-container__corner--bl"></div>
...
/* Header Styling */ {}
  ...
  .nes-container__title {
    margin: 0;
    font-size: 1.1em;
    letter-spacing: 0.5px;
    text-transform: uppercase;
  }
  .nes-container__subtitle {
    margin: 0;
    font-size: 0.8em;
    opacity: 0.9;
    font-weight: normal;
  }
  .nes-container__controls {
    display: flex;
    gap: 4px;
  }
...
/* Content Area */ {}
  ...
  .nes-container--scrollable .nes-container__content {
    overflow-y: auto;
    max-height: 60vh;
  }
/* Custom Scrollbar */ {}
...
/* Corner Decorations */ {}
  ...
  .nes-container__corner--tr {
    top: -3px;
    right: -3px;
    border-bottom: none;
    border-left: none;
  }
  .nes-container__corner--bl {
    bottom: -3px;
    left: -3px;
    border-top: none;
    border-right: none;
  }
  .nes-container__corner--br {
    bottom: -3px;
    right: -3px;
    border-top: none;
    border-left: none;
