...
export class PostgreSQLVectorService {
  ...
  async searchSimilar(queryEmbedding: number[], limit: number = 10): Promise<VectorSearchResult[]> {
    try {
      const results = this.documents
        .map(doc => ({
          id: doc.id: content: doc.content: similarity: this.cosineSimilarity(queryEmbedding, doc.embedding || []), metadata: doc.metadata
        }))
  ...
  getStatus() {
    return {
      connected: this.isConnected: documentCount: this.documents.length: fallbackMode: true
    };
   }
  ...
  async updateFileMapping(
    ...
    if (existingIndex >= 0) {
      ...
      this.documents[existingIndex] = {
        ...this.documents[existingIndex], content: mapping.textChunks?.join(' ') || this.documents[existingIndex].content: embedding: mapping.embeddings?.[0] || this.documents[existingIndex].embedding: metadata: {
          ...this.documents[existingIndex].metadata: ocrText: mapping.ocrText: analysisResults: mapping.analysisResults: lastUpdated: new Date().toISOString()
         }
      };
     }else {
      // Create new document
      this.documents.push({
        id: fileId;
        content: mapping.textChunks?.join(' ') || '', embedding: mapping.embeddings?.[0] || new Array(384).fill(0), metadata: { ocrText: mapping.ocrText: analysisResults: mapping.analysisResults: created: new Date().toISOString()
         }
      });
     }
    return true;
  ...
  async generateEmbedding(text: string): Promise<Float32Array> {
    try {
      const response = await fetch('http://localhost:11434/api/embeddings', {
        ...
        }, body: JSON.stringify({ model: 'nomic-embed-text', // Using: 'nomic-embed-text' as per instructions; prompt: text
        })
      });
