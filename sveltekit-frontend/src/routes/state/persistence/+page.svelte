...
</script> <svelte:head> <title>State Persistence Management - Legal AI Platform</title> <meta name="description" content="Manage XState persistence, restoration, and, state, hydration" /> </svelte:head> <div, class="page-container"> <header, class="page-header"> <div, class="header-content"> <div, class="breadcrumb"> <a, href="/state/machines" class="breadcrumb-link">State Machines</a> <span, class="breadcrumb-separator">→</span> <span, class="breadcrumb-current">Persistence</span> </div> <h1>💾 State Persistence Manager</h1> <p>Manage state snapshots, restoration, and hydration across the legal AI platform</p> </div> <div, class="stats-grid"> <div, class="stat-nier-bits-card"> <span, class="stat-number">{persistedStates.length}</span> <span, class="stat-label">Persisted States</span> </div> <div, class="stat-nier-bits-card"> <span, class="stat-number">{formatBytes(persistedStates.reduce((sum, s) => sum + s.size, 0))}</span> <span, class="stat-label">Total Storage</span> </div> <div, class="stat-nier-bits-card"> <span, class="stat-number">{new Set(persistedStates.map(s => s.machineId)).size}</span> <span, class="stat-label">Machines</span> </div> <div, class="stat-nier-bits-card"> <span, class="stat-number">{new Set(persistedStates.map(s => s.userId)).size}</span> <span, class="stat-label">Users</span> </div> </div> </header> <main, class="page-content"> {#if loading} <div, class="loading-state"> <div, class="spinner"></div> <p>Loading persisted states...</p> </div> {:else if persistedStates.length === 0} <div, class="empty-state"> <h2>💾 No Persisted States</h2> <p>No state snapshots have been saved yet. States are automatically persisted during critical transitions.</p> </div> {:else} <div, class="states-grid"> <div, class="states-list"> <h2>📋 Persisted States ({persistedStates.length})</h2> <div, class="filter-controls"> <button, class="nes-btn, ghost" onclick={ loadPersistedStates }> 🔄 Refresh </button> </div> <div, class="states-container"> {#each Array.isArray(persistedStates.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())) ? persistedStates.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()): [] as state} <div, class="state-card {selectedState?.id === state.id ? 'selected': ''}"
                role="button"
                tabindex="0"
