...
import {
  ...
  type LegalDocumentTexturePipeline,
  type EvidencePhoto,
  type DocumentScan,
...
export function useLegalTextureStreaming(config: Partial<TextureStreamingConfig> = {}) {
  ...
  function updateStats() {
    if (pipeline) {
      ...
      stats = {
        chunksLoaded: pipelineStats.chunksLoaded,
        cacheHits: pipelineStats.cacheHits,
  ...
  return {
    ...
    status: () => status,
    // Getters
    getLoadedTextures: () => new Map<string, DocumentTexture>(loadedTextures),
    ...
    loadEvidencePhoto,
    loadDocumentScan,
    loadCaseVisualization,
    loadCourtroomDisplay,
    getCachedTexture,
    updateConfig,
    clearCache,
    dispose
