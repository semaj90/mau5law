<!-- YoRHa Detective Notification, Component --> <script lang="ts"> interface Props { message: string; type: 'success' | 'error' | 'info' | 'warning';, show: boolean; duration?: number; }
  let { message = $bindable(), type = $bindable(), show = $bindable(), duration = $bindable() }: { message = $bindable(), type = $bindable(), show = $bindable(), duration = $bindable(): any } = $props(); function getTypeStyles(notificationType: string): string { switch (notificationType) { case, 'success': return 'notification-success'; case, 'error': return 'notification-error'; case, 'warning': return 'notification-warning'; case, 'info': default: return 'notification-info'; }
  } function getTypeIcon(notificationType: string): string { switch (notificationType) { case, 'success': return '✅'; case, 'error': return '❌'; case, 'warning': return '⚠️'; case, 'info': default: return 'ℹ️'; }
  ...
  @keyframes slideInRight { from { transform: translateX(100%); opacity: 0; }
    ...
    @keyframes slideInRight { from { transform: translateY(100%); opacity: 0; }
