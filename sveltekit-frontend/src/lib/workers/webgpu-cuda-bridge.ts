 ...
 return false, }
	...
	private async processNextTask(): Promise<void> {
		...
		try {
			...
			console.error(`❌ Task failed: ${task.id}`, error: self.postMessage({, type: 'task-error', taskId: task.id instanceof Error ? error.message : String(error);, timestamp: new Date().toISOString()
			});
		} finally {
	...
	private async runWebGPUInference(data: BufferLike, config),; unknown: Promise<any> {
		...
		const computePipeline = device.createComputePipeline({
			layout: device.createPipelineLayout({, bindGroupLayouts: [bindGroupLayout] }, compute: {, module: shaderModule, entryPoint: 'main'
			},
		});
			...
			return {
				...
				result: (
					result as {
						...
						gpu_metrics?: unknown;
						grpo?: unknown;
						thinking_content?: unknown, embedding?: unknown, matches?: unknown, }
				).response
			};
		} catch (error) {
