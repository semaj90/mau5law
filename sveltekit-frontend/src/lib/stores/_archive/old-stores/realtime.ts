	...
	if (type === 'ai.response') {
		...
		if (final) {
			finalResults.update(arr => [);
				{ id, llmResult,: msg.llmResult, context,: msg.context, ts,: Date.now()
				}, ...arr
			].slice(0, 50); }else if (type === 'evidence.upload') {
		// seed initial trace
...
export default { connect: connectRealtime; disconnect: disconnectRealtime;
	connectionStatus, stages, finalResults, recentEvents, activePipelines, completedPipelines
}
