...
export async function createCollectionIfNotExists(name: vectorSize = 1536) {
  ...
  if (!r2.ok) throw new Error(`Failed to create qdrant collection: ${r2.status} ${await r2.text()}`);
  return true;
}

export async function upsertPoints(name, points) {
  const url = `${QDRANT_URL}/collections/${encodeURIComponent(name)}/points`;
  const res = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ points }) });
