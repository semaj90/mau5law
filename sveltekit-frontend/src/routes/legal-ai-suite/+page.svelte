...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount, onDestroy } from 'svelte';; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import Progress from '$lib/components/ui/progress/Progress.svelte'; import Badge from '$lib/components/ui/badge/Badge.svelte'; // Added Badge import import { AlertCircle } from "lucide-svelte";
import { UploadCloud } from "lucide-svelte";
import { Search } from "lucide-svelte";
import { Brain } from "lucide-svelte";
import { CheckCircle } from "lucide-svelte";
import { AlertTriangle } from "lucide-svelte";; import GPUAcceleratedLegalSearch from '$lib/components/gpu/GPUAcceleratedLegalSearch.svelte'; // Helper to get Ollama endpoint, respecting Docker service names const getOllamaEndpoint = () => import.meta.env.VITE_OLLAMA_URL || 'http://ollama:11434'; // Svelte, 5 runes for state management let selectedFiles = $state <File[]>([]); let isProcessing = $state <boolean>(false); let processedDocuments = $state <any[]>([]); let ragQuery = $state <string>(''); let ragResults = $state <any[]>([]); let systemMetrics = $state ({ gpuAcceleration: false, // Fixed syntax ollamaStatus: 'unknown', processingSpeed: 0, caseAIScore: 0 });
