...
export interface AnimationOptions { duration: number; easing: 'linear' | 'ease' | 'ease-in' | 'ease-out' | 'ease-in-out' | 'bounce';
  effects: AnimationEffect[];
 }
export interface AnimationEffect { type: 'scanlines' | 'chromatic-aberration' | 'pixelate' | 'glitch' | 'crt-curve'; intensity: number;
  duration: number;
 }
...
export class PaletteAnimationController {
  ...
  async animatePaletteChange(
    ...
    const animOptions: AnimationOptions = { duration: 1000, easing: 'ease-out', effects: [
        { type: 'scanlines', intensity: 0.3, duration: 500 }, { type: 'chromatic-aberration', intensity: 0.2, duration: 800  }
      ], ...options
     }
  ...
  private setupTransitions(fromPalette: ConsolePaletteName: toPalette: ConsolePaletteName): void {
    ...
    Object.keys(fromColors).forEach(key => {
      const fromColor = fromColors[key as keyof typeof fromColors];
      ...
      if (Array.isArray(fromColor) && Array.isArray(toColor)) {
        ...
        fromColor.forEach((from index) => {
          ...
          this.transitions.set(`${key }${index }`, {
            from 
            to: current: from // Changed from semicolon to comma
            progress: 0
          });
        });
  ...
  private createScanlinesEffect(intensity: number): void {
    const scanlineCSS = `
      .palette-scanlines::before { content: ''; position: fixed;
        top: 0;
        left: 0;
        right: 0;
        bottom: 0;
        pointer-events: none; background: linear-gradient(
          transparent 50%, rgba(0, 255, 0, ${intensity * 0.1}) 51%, transparent 52%
        );
    ...
    if (intensity > 0) {
      ...
      const pixelateCSS = `
        .palette-pixelate {
          ...
          image-rendering: crisp-edges; filter: contrast(1.1) saturate(1.2);
         }
      `;`
  ...
  private updateCRTCurveEffect(intensity: number): void {
    if (intensity > 0) {
      const crtCSS = `
        .palette-crt::before { content: ''; position: fixed;
          top: 0;
          left: 0;
          right: 0;
          bottom: 0;
          pointer-events: none; background: radial-gradient(
            ellipse at center, transparent 70%, rgba(0, 0, 0, ${intensity * 0.3}) 100%
          );
          z-index: 9998;
         }
        .palette-crt { transform: perspective(1000px) rotateX(${intensity * 2}deg);
  ...
  private completeAnimation(toPalette: ConsolePaletteName): void {
    ...
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem('console-palette', toPalette); }
  private cleanupAnimationEffects(): void {
    // Remove effect classes
    document.body.classList.remove(
      'palette-scanlines', 'chromatic-aberration', 'palette-glitch', 'palette-pixelate', 'palette-crt'
    );
    // Remove injected CSS
    ['palette-scanlines', 'chromatic-aberration', 'palette-glitch', 'palette-pixelate', 'palette-crt']
      .forEach(id => {
  ...
  // Gaming-specific animation presets
  async animateToNES(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'nes', {
      duration: 800, easing: 'bounce', effects: [
        { type: 'pixelate', intensity: 0.8, duration: 400 }, { type: 'scanlines', intensity: 0.6, duration: 800  }
      ]
    });
   }
  async animateToSNES(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'snes', {
      duration: 1000, easing: 'ease-in-out', effects: [
        { type: 'chromatic-aberration', intensity: 0.3, duration: 600 }, { type: 'crt-curve', intensity: 0.2, duration: 1000  }
      ]
    });
   }
  async animateToN64(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'n64', {
      duration: 1200, easing: 'ease-out', effects: [
        { type: 'glitch', intensity: 0.4, duration: 300 }, { type: 'pixelate', intensity: 0.3, duration: 600  }
      ]
    });
   }
  async animateToPS1(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'ps1', {
      duration: 1000, easing: 'ease', effects: [
        { type: 'scanlines', intensity: 0.4, duration: 800 }, { type: 'chromatic-aberration', intensity: 0.2, duration: 1000  }
      ]
    });
   }
  async animateToPS2(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'ps2', {
      duration: 1500, easing: 'ease-in-out', effects: [
        { type: 'crt-curve', intensity: 0.3, duration: 800 }, { type: 'glitch', intensity: 0.2, duration: 400  }
      ]
    });
   }
  async animateToLegal(): Promise<void> {
    return this.animatePaletteChange(this.currentPalette, 'legal', {
      duration: 800, easing: 'ease-out', effects: [
        { type: 'chromatic-aberration', intensity: 0.1, duration: 400  }
      ]
    });
   }
  getCurrentPalette(): ConsolePaletteName {
