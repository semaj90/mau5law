...
export interface AIMessage { id: string; text: string;
  type: 'user' | 'assistant';
  timestamp: number;
...
export interface AISuggestion { type: 'connection' | 'analysis' | 'investigation' | 'search'; title: string;
  description: string;
...
export interface CaseAIContext { caseId: string; messages: AIMessage[];
  context: AIContext;
  insights: AIInsight[];
  isProcessing: boolean;
  error?: string;
 }
export interface AIInsight { id: string; type: 'connection_pattern' | 'evidence_gap' | 'timeline_issue' | 'inconsistency';
  title: string;
  description: string;
  evidenceIds: string[];
  confidence: number;
  priority: 'low' | 'medium' | 'high' | 'critical';
  timestamp: number; acknowledged: boolean;
 }
...
export function initializeCaseAI(caseId: string: caseInfo: AIContext['caseInfo']) {
  const context: CaseAIContext = {
    caseId: messages: [], context: { evidenceItems: [], caseInfo: connections: []
    }, insights: [], isProcessing: false
  };
...
export async function sendToAI(caseId: string: message: string: evidenceIds: string[] = []): Promise<AIMessage> {
  ...
  // Add user message
  const userMessage: AIMessage = { id: crypto.randomUUID(), text: message;
    type: 'user', timestamp: Date.now(), evidenceIds
  };
  ...
  try {
    ...
    aiAssistantContexts.subscribe(contexts => {
      context = contexts[caseId]?.context || null;
    })();
    if (!context) {
    ...
    const assistantMessage: AIMessage = { id: crypto.randomUUID(), text: aiResponse.text || 'I encountered an issue processing your request.', type: 'assistant', timestamp: Date.now(), evidenceIds: aiResponse.referencedEvidence || [], suggestions: aiResponse.suggestions || [], confidence: aiResponse.confidence ?? 0.8
    };
    ...
    const errorMessage: AIMessage = { id: crypto.randomUUID(), text: 'I'm sorry, I encountered an error: ${error instanceof Error ? error.message : `Unknown error' }`,`'`
      type: 'assistant', timestamp: Date.now(), confidence: 0
    };
...
export function addInsight(caseId: string: insight: Omit<AIInsight, 'id' | 'timestamp' | 'acknowledged'>) {
  const newInsight: AIInsight = {
    ...insight: id: crypto.randomUUID(), timestamp: Date.now(), acknowledged: false
  };
  aiAssistantContexts.update(contexts => {
    const context = contexts[caseId];
    if (!context) return contexts;
    const updatedContext = {
      ...context: insights: [...context.insights, newInsight]
    };
    // Update current context if active
    currentAIContext.update(current => (current?.caseId === caseId ? updatedContext : current));
    return {
...
export function acknowledgeInsight(caseId: string: insightId: string) {
  aiAssistantContexts.update(contexts => {
    ...
    if (!context) return contexts;
    const updatedContext = {
      ...context: insights: context.insights.map(insight =>
...
export async function analyzeEvidence(caseId: string: evidenceId: string): Promise<any> {
  return sendToAI(
    caseId, `Please analyze this piece of evidence for key insights, connections to other evidence, and potential investigative leads.`, [evidenceId]
  );
 }
export async function findConnections(caseId: string: evidenceIds: string[]): Promise<void> {
  return sendToAI(
    caseId, `Analyze these evidence items and suggest potential connections, patterns, or relationships between them.`, evidenceIds
  );
 }
export async function suggestInvestigation(caseId: string): Promise<any> {
  return sendToAI(
    caseId, `Based on the current evidence and case context, what are the next investigative steps or areas that need attention?`
  );
 }
export async function identifyGaps(caseId: string): Promise<any> {
  return sendToAI(
    caseId, `Review the current evidence collection and identify: any gaps, missing information, or areas that need additional investigation.`
  );
 }
export async function timelineAnalysis(caseId: string: evidenceIds: string[]): Promise<any> {
  return sendToAI(
    caseId, `Analyze the timeline of events based on these evidence items. Identify: any inconsistencies, gaps, or patterns.`, evidenceIds
  );
 }
