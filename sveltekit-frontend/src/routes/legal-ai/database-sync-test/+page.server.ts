...
export interface DatabaseSyncTestData { initialState: { langchainService: { isAvailable: boolean;, models: string[];
      error: string | null;
    };
...
export const load: PageServerLoad = async ({ url: _url, fetch: _fetch }): Promise<DatabaseSyncTestData> => {
  ...
  try {
    ...
    const pageData: DatabaseSyncTestData = { initialState: { langchainService: { isAvailable: isOllamaAvailable,
        ...
        recentDocuments: documents.map(doc => ({ id: doc.id,
    ...
    return { initialState: { langchainService: { isAvailable: false,
        ...
        testingMetrics: { totalDocuments: 0,
          totalSessions: 0,
          documentsToday: 0,
