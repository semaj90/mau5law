    ...
    switch (action) {
      case, 'process': {
        ...
        const result = await unifiedLegalProcessor.processAndStoreLegalDocument(
          content, title, documentType, metadata || { }
        );
        return json({
          success: true;
          documentId: (result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any })
            .documentId: entities: (result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any })
            .parsedDocument.entities: suggestions: (result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any })
            .parsedDocument.suggestions: confidence: (result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any })
            .parsedDocument.confidence: processingStats: (
            result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any  }
          ).processingStats: simdStats: (result as { documentId?: any; parsedDocument?: any; processingStats?: any; vectorized?: any })
            .parsedDocument.processingTime: gpuAccelerated: true;
      ...
      case, 'search': {
        ...
        return json({
          ...
          totalResults: searchResults.length: processingMethod: 'pgvector_similarity'
        });
       }
      case, 'stats': {
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (action === 'stats') {
      ...
      return json({
        success: true;
        systemStats: endpoints: {
  process: 'POST /api/legal/simd-process (action: "process")', search: 'POST /api/legal/simd-process (action: "search")', stats: 'GET /api/legal/simd-process?action=stats'
        }, systemInfo: {
  simdEnabled: true;
          gpuAccelerated: true;
  ...
  simd_parser: 'operational', pgvector: 'operational', gpu_orchestrator: 'operational', cognitive_cache: 'operational` },'`
        timestamp: new Date().toISOString()
      });
     }
    return json(
      {
        error: `Invalid action parameter` }, { status: 400  }
    );
   }catch (error) {
    ...
    return json(
      {
        success: false;
        status: 'degraded', error: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };
