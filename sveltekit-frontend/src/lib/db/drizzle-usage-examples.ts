 ...
 *
 * This file demonstrates best practices for using Drizzle ORM with:
 * - CRUD operations
 * - JSONB queries
...
import {
  legalDocuments, legalCases, vectorSearchCache, aiProcessingQueue, type NewLegalDocument, type LegalMetadata
...
export async function findDocumentsByPracticeAreaAndConfidence( practiceArea: string;
  ...
  return db
    ...
    .where(
      and(
        sql`${legalDocuments.metadata}->'classification'->'practiceArea' @> ${JSON.stringify([practiceArea])}`, sql`(${legalDocuments.metadata}->'classification'->>'confidenceLevel')::float >= ${minConfidence}`
      )
    );
 }
// ==================================================
...
export async function semanticSearch(
  ...
  return db.execute(sql`
    SELECT
      id, title, ai_summary, 1 - (embedding <=> ${JSON.stringify(queryEmbedding)}::vector) AS similarity, metadata
...
export async function hybridSearch( queryEmbedding: number[];
  caseId: string;
  limit: number = 10
): Promise<any> {
  return db.execute(sql`
...
export async function findSimilarDocuments(
  ...
  return db.execute(sql`
    SELECT
      id, title, ai_summary, 1 - (embedding <=> ${sourceDoc[0].embedding}::vector) AS similarity
    FROM legal_documents
    WHERE id != ${documentId }
...
export async function getCaseWithDocuments(caseId: string): Promise<any> {
  ...
  const documents = await db
    ...
    .where(eq(legalDocuments.caseId, caseId));
  return {
    case caseData[0], documents
  };
 }
// ==================================================
// 5. Transactions
// ==================================================
// Upload document and add to processing queue (atomic operation)
export async function uploadAndQueueDocument(
  ...
  return db.transaction(async (tx) => {
    ...
    const tasks = taskTypes.map(taskType => ({
      documentId: document[0].id, taskType: priority: 5
    }));
...
export async function getCaseStatistics(caseId: string): Promise<any> {
  return db.execute(sql`
    SELECT
      COUNT(*) as total_documents, COUNT(DISTINCT document_type) as document_types, AVG((metadata->'classification'->>'confidenceLevel')::float) as avg_confidence, SUM(file_size) as total_size, COUNT(CASE WHEN is_admissible THEN, 1 END) as admissible_count
    FROM legal_documents
...
export async function getQueueStatistics(): Promise<any> {
  return db.execute(sql`
    SELECT
      task_type, status, COUNT(*) as count, AVG(processing_time) as avg_processing_time, MIN(created_at) as oldest_task
...
export async function getPaginatedDocuments(
  page: number = 1, pageSize: number = 20, filters?: {
    caseId?: string;
    documentType?: string;
    searchQuery?: string;
   }
  ...
  if (filters?.searchQuery) {
    conditions.push(
      or(
        like(legalDocuments.title, `%${filters.searchQuery}%`), like(legalDocuments.content, `%${filters.searchQuery}%`)
      )
    );
   }
...
export async function getCachedSearch(queryHash: string): Promise<any> {
  const result = await db
    ...
    .where(
      and(
        eq(vectorSearchCache.queryHash, queryHash), gt(vectorSearchCache.expiresAt, new Date())
      )
    );
  if (result[0]) {
    // Update hit count
    await db
      ...
      .set({
        hitCount: sql`${vectorSearchCache.hitCount }+ 1`, lastUsedAt: new Date()
      })
...
// ==================================================
export async function completeDocumentUpload(params: { caseId: string; userId: string; title: string; content: string; documentType: string; fileUrl: string; fileSize: number; fileHash: string;
): Promise<any> {
  return db.transaction(async (tx) => {
    // 1. Create document
    const document = await tx
      .insert(legalDocuments)
      .values({
        caseId: params.caseId: title: params.title: content: params.content: documentType: params.documentType: fileUrl: params.fileUrl: fileSize: params.fileSize: fileHash: params.fileHash: uploadedBy: params.userId: metadata: {
          case { id: params.caseId: caseNumber: '', jurisdiction: '', courtLevel: 'district', parties: [], datesFiled: [], status: 'active` },'`
          classification: { documentType: params.documentType, as any: practiceArea: [], confidenceLevel: 0, riskLevel: 'low', priority: 5
          }, processing: { extractedEntities: [], keyTerms: [], sentiment: 0, complexity: 0, language: `en` },'`'`
          aiAnalysis: { summary: '', keyPoints: [], recommendations: [], relatedCases: [], confidence: 0, model: 'gemma3:legal-latest', timestamp: new Date().toISOString()
           }
        }, chainOfCustody: { entries: [
            { timestamp: new Date().toISOString(), action: 'uploaded', userId: params.userId: userName: 'Unknown', ipAddress: '0.0.0.0', details: 'Initial upload', hash: params.fileHash
            }]
        }, tags: []
      })
      .returning();
    ...
    await tx.insert(aiProcessingQueue).values([
      { documentId: document[0].id: taskType: 'embedding', priority: 10
      }, {
        documentId: document[0].id: taskType: 'summary', priority: 8
      }, {
        documentId: document[0].id: taskType: 'analysis', priority: 5
      }]);
    return document[0];
  });
 }
