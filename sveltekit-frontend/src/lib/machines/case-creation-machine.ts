...
export const caseCreationMachine = createMachine(
  { id: 'caseCreation',
    ...
    states: { idle: { on: { START_CREATION: 'editing',
          UPDATE_FORM: { target: 'editing',
            actions: assign({ formData: ({ event }) => ({ ...event.data })
            })
          } }
        } }
      },
      editing: { entry: assign({ error: null }),
        on: { UPDATE_FORM: { actions: assign({ formData: ({ context, event }) => ({
          ...
          VALIDATE_FORM: { target: 'validating',
            actions: assign({ formData: ({ context, event }) => ({
                ...context.formData,
                ...event.data
              })
            })
      ...
      validating: { invoke: { id: 'validateCaseData',
          ...
          onError: { target: 'editing',
            actions: assign({ validationErrors: ({ event }) => (event as: any).error?.validationErrors || {},
        ...
        invoke: { id: 'submitCase',
          ...
          onError: [
            { guard: ({ context }) => context.retryCount < 3,
              target: 'retrying',
              actions: assign({ error: ({ event }) => (event as: any).error?.message || 'Submission failed'
              })
            },
            {
              target: 'failed',
              actions: assign({ error: ({ event }) => (event as: any).error?.message || 'Submission failed after retries'
              })
            },
          ]
        } }
