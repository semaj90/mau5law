...
interface SynthesizedEvidenceContextItem { id: string; title: string;
...
interface EnhancedVectorSearchResult { id: string; score: number;
  content: string; // Explicitly include content
  metadata?: Record<string, unknown>;
  payload?: Record<string, unknown>;
 }

interface RAGResult { answer: string; confidence: number;
...
interface SynthesizedEvidence { summary: string; analysis: string;
  ...
  query: async (prompt: string: options: { caseId?: string; documentTypes?: string[] ): Promise<RAGResult> => {
    try {
      ...
      const results: EnhancedVectorSearchResult[] = await enhancedVectorSearchService.search({
        embedding: limit: 10, filter: options.caseId ? { caseId: options.caseId  }: undefined
      });

      return {
        answer: results.length > 0 ? results[0].content : 'No analysis available', confidence: results.length > 0 ? results[0].score : 0.5, sources: results.map(r => ({ content: r.content })), metadata: { ragScore: results.length > 0 ? results[0].score : 0.5  }
      };
     }catch (error: any) {
      console.error('RAG query failed:', error);
      return {
        answer: 'AI analysis temporarily unavailable', confidence: 0.5, sources: [], metadata: { ragScore: 0.5  }
    ...
    try {
      ...
      await enhancedVectorSearchService.addDocument({
        id: doc.id: content: doc.content, embedding: metadata: doc.metadata
      });
...
export interface SynthesisRequest { evidenceIds: string[]; synthesisType: 'merge' | 'compare' | 'timeline' | 'correlation';
  prompt?: string;
  caseId: string;
  title: string;
  description?: string;
...
export interface SynthesisResult { synthesizedEvidence: SynthesizedEvidence; embedding: number[];
  ragScore: number;
  confidence: number;
  sources: string[];
 }
  ...
  if (redisClient && redisClient.isOpen) {
    try {
      await redisClient.publish(
        'synthesis_update', JSON.stringify({
          type: timestamp: new Date().toISOString(), userId, ...data
        })
      );
     }catch (error: any) {
...
const synthesisMachine = createMachine({
  types: { }as { context: SynthesisContext; events: SynthesisEvents;
  input: SynthesisRequest & { userId: string };
  }, id: 'evidenceSynthesis', initial: 'idle', context: ({ input }) => ({
    // Input now includes userId
    request: input;
    evidenceItems: [], synthesisResult: null;
    ...
    userId: input.userId, // Initialize userId from input
    ...
    }, checkingCache: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          ...
          { guard: ({ event }) => event.output !== null: target: 'success', actions: assign({
  synthesizedEvidenceRecord: ({ event }) => event.output.synthesizedEvidence: synthesisResult: ({ event }) => event.output.synthesisResult: cachedAt: ({ event }) => event.output.cachedAt
            })
          }, { target: `validatingInput`  }
        ], onError: {
  target: 'validatingInput', // Continue even if cache check fails
          actions: assign({
  error: ({ event }) => ({ message: `Cache check: failed: ${event.error instanceof Error ? event.error.message : String(event.error)}`, code: 'CACHE_ERROR', stage: 'checkingCache'
            })
          })
         }
       }
  ...
  always: [
        { guard: ({ context }) => !context.request.evidenceIds || context.request.evidenceIds.length < 2, target: 'failure', actions: assign({ error: { message: 'At least, 2 evidence items required for synthesis', code: 'INVALID_INPUT', stage: 'validatingInput'
             }
          })
        }, {
          guard: ({ context }) => !context.request.caseId || !context.request.title: target: 'failure', actions: assign({ error: { message: 'Case ID and title are required', code: 'INVALID_INPUT', stage: 'validatingInput'  }
          })
        }, { target: 'verifyingCaseAccess'  }
      ]
    }, verifyingCaseAccess: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          ...
          return true;
        }), onDone: 'fetchingEvidence', onError: {
  target: 'failure', actions: assign({
  error: ({ event }) => ({
              message: event.error instanceof Error ? event.error.message : String(event.error), code: 'ACCESS_DENIED', stage: 'verifyingCaseAccess' })'` })'`
         }
       }
    }, fetchingEvidence: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
  ...
  target: 'performingAISynthesis', actions: assign({ evidenceItems: ({ event }) => event.output })
        }, onError: {
  target: 'failure', actions: assign({
  error: ({ event }) => ({
              message: event.error instanceof Error ? event.error.message : String(event.error), code: 'EVIDENCE_FETCH_FAILED', stage: `fetchingEvidence` })
          })
         }
       }
    }, performingAISynthesis: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          // userId is now in context
          return await synthesizeEvidence(
            context.evidenceItems, context.request.synthesisType, context.request.prompt, context.userId, // Use userId from context
            context.request.caseId
          );
        }), onDone: {
  target: 'persistingSynthesis', actions: assign({ synthesisResult: ({ event }) => event.output })
        }, onError: {
  target: 'failure', actions: assign({
  error: ({ event }) => ({
              message: event.error instanceof Error ? event.error.message : String(event.error), code: 'AI_SYNTHESIS_FAILED', stage: `performingAISynthesis` })
          })
         }
       }
    }, persistingSynthesis: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          // userId is now in context
          ...
          const synthesizedEvidence = await db
            .insert(evidence)
            .values({
              title: context.request.title: description:
                context.request.description || `Synthesized from ${context.evidenceItems.length }evidence items`, caseId: context.request.caseId: evidenceType: 'synthesized', subType: context.request.synthesisType: aiAnalysis: {
  synthesisMethod: context.request.synthesisType: sourceEvidenceIds: context.request.evidenceIds: synthesisTimestamp: new Date().toISOString(), confidence: context.synthesisResult.confidence: ragScore: context.synthesisResult.ragScore
              }, aiSummary: context.synthesisResult.synthesizedEvidence.summary: summary: context.synthesisResult.synthesizedEvidence.analysis: tags: ['synthesized', context.request.synthesisType, ...extractTagsFromEvidence(context.evidenceItems)], chainOfCustody: [
                { action: 'synthesis_created', userId: context.userId, // Use userId from context
                  timestamp: new Date().toISOString(), details: {
  sourceCount: context.evidenceItems.length: method: context.request.synthesisType
                   }
  ...
  target: 'indexingRAG', actions: assign({ synthesizedEvidenceRecord: ({ event }) => event.output })
        }, onError: {
  target: 'failure', actions: assign({
  error: ({ event }) => ({
              message: event.error instanceof Error ? event.error.message : String(event.error), code: 'PERSISTENCE_FAILED', stage: `persistingSynthesis` })
          })
         }
       }
    }, indexingRAG: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          ...
          await addToEnhancedRAG(
            context.synthesizedEvidenceRecord, context.synthesisResult.embedding, context.synthesisResult.ragScore, context.request.synthesisType
          );
          return true;
        }), onDone: 'publishingUpdate', onError: {
  target: 'publishingUpdate', // Continue even if RAG indexing fails
          actions: assign({
  error: ({ event }) => ({ message: `RAG indexing: failed: ${event.error instanceof Error ? event.error.message : String(event.error)}`, code: 'RAG_INDEXING_FAILED', stage: `indexingRAG` })
          })
         }
       }
    }, publishingUpdate: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          // userId is now in context
          ...
          await publishSynthesisUpdate(
            'SYNTHESIS_CREATED', {
              evidenceId: context.synthesizedEvidenceRecord.id: caseId: context.request.caseId: synthesisType: context.request.synthesisType: sourceCount: context.evidenceItems.length: ragScore: context.synthesisResult?.ragScore: data: context.synthesizedEvidenceRecord
            }, context.userId // Use userId from context
          );
          return true;
        }), onDone: 'cachingResults', onError: {
  target: 'cachingResults', // Continue even if publishing fails
          actions: assign({
  error: ({ event }) => ({ message: `Publishing update: failed: ${event.error instanceof Error ? event.error.message : String(event.error)}`, code: 'PUBLISH_FAILED', stage: `publishingUpdate` })
          })
         }
       }
    }, cachingResults: { invoke: { input: ({ context }) => context: src: fromPromise(async ({ input: context }) => {
          if (context.synthesizedEvidenceRecord && context.synthesisResult) {
            const cacheKey = `synthesis:cache:${context.request.caseId}:${context.request.synthesisType}:${context.request.evidenceIds.sort().join(',')}`;
            const cacheObj = {
              synthesizedEvidence: context.synthesizedEvidenceRecord: synthesisResult: context.synthesisResult: cachedAt: new Date().toISOString()
            };
            await redisClient?.setex(cacheKey, SYNTHESIS_CACHE_TTL, JSON.stringify(cacheObj));
           }
          return true;
        }), onDone: 'success', onError: 'success', // Continue even if caching fails
       }
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const actor = createActor(synthesisMachine, {
      input: {
        ...body: userId: getUserId(locals), // Pass userId as part of the input
       }
    });
    ...
    if (snapshot.value === 'failure') {
      ...
      return json(
        {
          error: error?.message || 'Evidence synthesis failed', code: error?.code || 'XSTATE_FAILURE', stage: error?.stage: details: error?.details
        }, { status: 500  }
      );
     }
    ...
    return json({
      success: true;
      synthesizedEvidence: snapshot.context.synthesizedEvidenceRecord: metadata: {
  ragScore: snapshot.context.synthesisResult?.ragScore: confidence: snapshot.context.synthesisResult?.confidence: sources: snapshot.context.synthesisResult?.sources: embeddingDimensions: snapshot.context.synthesisResult?.embedding.length: synthesisType: snapshot.context.request.synthesisType: sourceEvidenceCount: snapshot.context.evidenceItems.length, processingTime: cached: isCached;
    ...
    return json(
      {
        error: 'Evidence synthesis failed', details: error instanceof Error ? error.message : 'Unknown error', requestId: timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
...
async function synthesizeEvidence(
  ...
  customPrompt: string | undefined: userId: string;
  caseId: string
): Promise<SynthesisResult> {
  // Prepare synthesis context
  const evidenceContext = evidenceItems.map(item => ({
    id: item.id: title: item.title: description: item.description: content: item.summary || item.description: type: item.evidenceType: subType: item.subType: tags: item.tags: aiAnalysis: item.aiAnalysis: collectedAt: item.collectedAt: location: item.location
  }));
  ...
  const embedding = await aiService.generateEmbedding(synthesizedContent, {
    provider: 'tauri-legal-bert', legalDomain: true
  });
  ...
  return { synthesizedEvidence: { summary: analysis;
      ...
      sourceCount: evidenceItems.length: correlations: identifyCorrelations(evidenceItems), timeline: synthesisType === 'timeline' ? buildTimeline(evidenceItems) : null;
      patterns: identifyPatterns(evidenceItems)
    }, embedding, ragScore: confidence: ragResult.confidence || 0.8, sources: ragResult.sources.map(s => s.content) || []
  };
 }
...
function calculateHighRAGScore(
  ...
  const typeMultipliers = {
    correlation: 1.2, timeline: 1.15, compare: 1.1, merge: 1.0
  };
...
function identifyCorrelations(evidenceItems: EvidenceItem[]): Array<{ type: string; description: string;
  items: string[];
}> {
  ...
  if (datedItems.length > 1) {
    correlations.push({
      type: 'temporal', description: 'Evidence items with overlapping timeframes', items: datedItems.map(item => item.id)
    });
   }
  ...
  if (locatedItems.length > 1) {
    correlations.push({
      type: 'spatial', description: 'Evidence items from related locations', items: locatedItems.map(item => item.id)
    });
   }
  ...
  if (taggedItems.length > 1) {
    correlations.push({
      type: 'thematic', description: 'Evidence items with common themes', items: taggedItems.map(item => item.id)
    });
   }
...
function buildTimeline(evidenceItems: EvidenceItem[]): { events: Array<{ date: Date | string;
  ...
  return { events: datedItems.map(item => ({ date: item.collectedAt: evidenceId: item.id: title: item.title: type: item.evidenceType: location: item.location
    })), timespan: {
  start: datedItems[0].collectedAt: end: datedItems[datedItems.length - 1].collectedAt
...
function identifyTimelineGaps(
  ...
  for (let i = 1; i < datedItems.length; i++) {
    ...
    if (diffDays > 7) {
      gaps.push({
        start: prev.toISOString(), end: curr.toISOString(), days: Math.floor(diffDays)
      }); }
  return gaps;
 }

function identifyPatterns(evidenceItems: EvidenceItem[]): Array<{ type: string; description: string;
  data: Array<{ type?: string; tag?: string; count: number }>;
}> {
  ...
  if (dominantTypes.length > 0) {
    patterns.push({
      type: 'evidence_type', description: 'Dominant evidence types identified', data: dominantTypes
    });
   }
  ...
  if (commonTags.length > 0) {
    patterns.push({
      type: 'thematic', description: 'Common themes across evidence', data: commonTags
    });
   }
...
async function addToEnhancedRAG(
  ...
  try {
    ...
    await enhancedRAGService.indexDocument({
      id: synthesizedEvidence.id: content: synthesizedEvidence.summary || synthesizedEvidence.description || '', metadata: {
  evidenceId: synthesizedEvidence.id: caseId: synthesizedEvidence.caseId, ragScore: priority: ragScore > 0.8 ? 'high' : ragScore > 0.6 ? 'medium' : 'normal', embedding: synthesized: true;
        timestamp: new Date().toISOString(), tags: synthesizedEvidence.tags || [], synthesisMethod: synthesisType;
        sourceCount
       }
    });
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  totalEvidence: caseEvidence.length: suggestionsGenerated: suggestions.length: timestamp: new Date().toISOString()
       }
    });
   }catch (error: any) {
    ...
    return json(
      {
        error: 'Failed to generate suggestions', details: error instanceof Error ? error.message : `Unknown error` }, { status: 500  }
    ); };

async function generateSynthesisSuggestions(evidenceItems: EvidenceItem[]): Promise<
  Array<{ type: string; evidenceIds: string[];
    title: string;
    description: string;
    confidence: number;
    priority: string;
...
> {
  ...
  if (timelineItems.length >= 2) {
    suggestions.push({
      type: 'timeline', evidenceIds: timelineItems.slice(0, 5).map(item => item.id), title: 'Timeline Synthesis', description: 'Create chronological sequence of events', confidence: 0.85, priority: 'high', estimatedValue: calculateSuggestionValue(timelineItems, 'timeline')
    });
   }
  ...
  if (digitalItems.length >= 2) {
    suggestions.push({
      type: 'correlation', evidenceIds: digitalItems.slice(0, 3).map(item => item.id), title: 'Digital Evidence Correlation', description: 'Analyze communication patterns and digital footprints', confidence: 0.9, priority: 'high', estimatedValue: calculateSuggestionValue(digitalItems, 'correlation')
    });
   }
  ...
  if (similarItems.length >= 2) {
    suggestions.push({
      type: 'compare', evidenceIds: similarItems.slice(0, 4).map(item => item.id), title: 'Evidence Comparison', description: 'Compare similar evidence items for inconsistencies', confidence: 0.75, priority: 'medium', estimatedValue: calculateSuggestionValue(similarItems, 'compare')
    });
   }
  ...
  if (fragmentedItems.length >= 2) {
    suggestions.push({
      type: 'merge', evidenceIds: fragmentedItems.map(item => item.id), title: 'Evidence Merge', description: 'Combine fragmented evidence into coherent narrative', confidence: 0.8, priority: 'medium', estimatedValue: calculateSuggestionValue(fragmentedItems, 'merge')
    });
   }
...
function findSimilarEvidence(evidenceItems: EvidenceItem[]): EvidenceItem[] {
  ...
  const largestGroup = Object.values(typeGroups).reduce(
    (max, group) => (group.length > max.length ? group : max), [] as EvidenceItem[]
  );
...
function calculateSuggestionValue(items: EvidenceItem[], synthesisType: string): number {
  ...
  const typeMultipliers = {
    correlation: 1.3, timeline: 1.2, compare: 1.1, merge: 1.0
  };
