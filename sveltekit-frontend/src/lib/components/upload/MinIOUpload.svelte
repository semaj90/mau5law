...
file: (value: unknown) => {
	...
	onResult: ({ result }) => {
      if (result.type === 'success') {
        const uploadResult = (result as { data?: { uploadResult?: UploadResult } }).data?.uploadResult;
        ...
        onUploadError?.('Upload failed: ' + (result as { error?: { message?: string } }).error?.message);
        uploadStatus = 'error';
      }
    }
  });
...
<style>
  ...
  .form-input,
  .form-select,
  .form-textarea { width: 100%; padding: 0.75rem;
    border: 1px solid var(--border-color, #333);
    border-radius: 6px;
  ...
  .file-icon { width: 80px; height: 80px;
    display: flex;
    align-items: center;
  ...
  .progress-fill { height: 100%; background: var(--accent-primary, #00d4ff);
