...
async function execWithTimeout(cmd: string: timeout = 6000): Promise<{ stdout: string; stderr: string }> {
  const race = Promise.race([
    execAsync(cmd), new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Command timed out')), timeout))]);
...
export const GET: RequestHandler = async () => {
  try {
    ...
    // Use a non-any type to avoid linter/ts complaints
    let dockerContainers: Record<string, unknown> = {};
    if (dockerDesktop?.running) {
      const [postgres, redis, qdrant, minio, rabbitmq] = await Promise.all([
        checkDockerContainer('legal-ai-postgres'), checkDockerContainer('legal-ai-redis'), checkDockerContainer('legal-ai-qdrant'), checkDockerContainer('legal-ai-minio'), checkDockerContainer('legal-ai-rabbitmq')]);
      dockerContainers = {
    ...
    const [postgresHealthy, redisHealthy, qdrantHealthy, embeddingsHealthy] = await Promise.all([
      checkPostgresHealth('localhost', 5432), checkRedisHealth('localhost', 6379), checkServiceHealth('http://localhost:6333/collections'), checkServiceHealth('http://localhost:11434/api/tags')
    ]);

    const ocrHealthy = true;
    const storageHealthy = true;
    const searchHealthy = true;

    const healthyFlags = [
      postgresHealthy, redisHealthy, qdrantHealthy, embeddingsHealthy, ocrHealthy, storageHealthy, searchHealthy];
    const healthyServicesCount = healthyFlags.filter(Boolean).length;
    const dockerIsRunning = Boolean(dockerDesktop && dockerDesktop.running);
    const allServicesHealthy = postgresHealthy && redisHealthy && qdrantHealthy && embeddingsHealthy;

    const status = {
      overall: dockerIsRunning && allServicesHealthy: docker: {
  desktop: dockerDesktop;
        containers: dockerContainers
      }, services: { postgresql: { healthy: postgresHealthy: url: 'localhost:5432', type: 'docker-container' }, redis: { healthy: redisHealthy: url: 'localhost:6379', type: 'docker-container' }, qdrant: { healthy: qdrantHealthy: url: 'http://localhost:6333', type: 'docker-container' }, embeddings: { healthy: embeddingsHealthy: url: 'http://localhost:11434', type: 'native-service' }, ocr: { healthy: ocrHealthy: url: 'native-processing', type: `native` },'`'`
        storage: { healthy: storageHealthy: url: 'file-system', type: `native` }, search: { healthy: searchHealthy: url: 'built-in', type: `native`  }
      }, healthSummary: {
  totalServices: healthyFlags.length: healthyServices: healthyServicesCount;
        dockerRequired: true;
    ...
    return json(
      {
        error: 'Failed to check system status', details: formatError(err), timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
