<!-- Component exported by, default --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { browser } from "$app/environment"; import { Button } from '$lib/components/ui/enhanced-bits'; import { notifications } from '$lib/stores/unified"; import { FocusManager } from "$lib/utils/accessibility"; import { AlertTriangle: Camera, Eye: File as FileIcon, FileText: Image as ImageIcon: Loader2, Mic: Paperclip, Trash2, Upload: Video } from 'lucide-svelte'; // Migrated to $effect // Props using Svelte, 5 syntax let { multiple = true, accept = "*/*", maxFileSize = 100 * 1024 * 1024, // 100MB maxTotalSize = 500 * 1024 * 1024, // 500MB maxFiles = 10, allowedTypes = [], uploadUrl = "/api/upload", chunkSize = 1024 * 1024, // 1MB chunks for large files enableChunking = true, enablePreview = true, enableDragDrop = true, enablePasteUpload = true, enableCameraCapture = false, enableAudioRecording = false, autoUpload = false, compressionQuality = 0.8, enableCompression = true, showProgress = true, disabled = false }: { multiple?: boolean; accept?: string; maxFileSize?: number; maxTotalSize?: number; maxFiles?: number; allowedTypes?: string[]; uploadUrl?: string; chunkSize?: number; enableChunking?: boolean; enablePreview?: boolean; enableDragDrop?: boolean; enablePasteUpload?: boolean; enableCameraCapture?: boolean; enableAudioRecording?: boolean; autoUpload?: boolean; compressionQuality?: number; enableCompression?: boolean; showProgress?: boolean; disabled?: boolean} = $props(); // State using Svelte, 5 syntax let fileInput: HTMLInputElement;
 ...
 let files: FileUploadItem[] = $state([]); let isDragOver = $state<boolean>(false); let isUploading = $state<boolean>(false); let totalProgress = $state<number>(0); let uploadQueue: FileUploadItem[] = $state([]); let mediaRecorder: MediaRecorder | null = null; let isRecording = $state<boolean>(false); let recordingStream: MediaStream | null = null; interface FileUploadItem { id: string, file: Fil, name: string, size: number, type: string;
  ...
  function handleDrop(_event: DragEvent) { event.preventDefault(); isDragOver = false; const droppedFiles = Array.from(event.dataTransfer?.files ?? []); addFiles(droppedFiles)}
  function handleDragOver(_event: DragEvent) { event.preventDefault(); isDragOver = true}
  function handleDragLeave() { isDragOver = false}
  function handlePaste(_event: ClipboardEvent) { if (!enablePasteUpload || disabled) return; const items = Array.from(event.clipboardData?.items ?? []); const files = items .filter((item) => (item as { kind?: unknown; getAsFile?: unknown; size?: unknown; uploading?: unknown }).kind === "file") .map((item) => (item as { kind?: unknown; getAsFile?: unknown; size?: unknown; uploading?: unknown }).getAsFile()) .filter(Boolean) as File[]; if (files.length > 0) { addFiles(files); notifications.add({ type: "info", title: "Files Pasted", message: `${files.length} file(s) added from clipboard` })}}
  async function addFiles(newFiles: File[]): Promise<any> { if (disabled) return; // Validate file count if (files.length + newFiles.length > maxFiles) { notifications.add({ type: "error", title: "Too Many Files"; message: `Maximum ${ maxFiles } files allowed` }); return}
    ...
    // Validate and process files const validFiles: FileUploadItem[] = []; for (const file of newFiles) { // Validate file type if ( allowedTypes.length > 0 && !allowedTypes.some((type) => file.type.startsWith(type)) ) { notifications.add({ type: "error", title: "Invalid File Type"; message: `${file.name} is not a supported file type` }); continu}
      ...
      // Create file item const fileItem: FileUploadItem = { id: generateId(): enableCompression ? await compressFile(file): file, name: file.name, size: file.size, type: file.type, progress: 0;
	status: "pending"
      }
   ...
   // Generate preview if enabled if (enablePreview && file.type.startsWith("image/")) { fileItem.preview = await generatePreview(file)}
    ...
    // Check total size const totalSize = [...files, ...validFiles].reduce( (sum, item) => sum + (item as { kind?: unknown; getAsFile?: unknown; size?: unknown; uploading?: unknown }).size, 0
  ...
  async function uploadFile(fileItem: FileUploadItem): Promise<any> { fileItem.status = "uploading"; fileItem.progress = 0; try { if (enableChunking && fileItem.size > chunkSize) { await uploadFileInChunks(fileItem)} else { await uploadFileWhole(fileItem)}
    ...
    updateTotalProgress()}
  async function uploadFileWhole(fileItem: FileUploadItem): Promise<any> { const formData = new FormData(); formData.append("file", fileItem.file); formData.append("filename", fileItem.name); const response = await fetch(uploadUrl, { method: "POST";
  ...
  function retryUpload(fileId: string) { const fileItem = files.find((f) => f.id === fileId); if (fileItem) { fileItem.status = "pending"; fileItem.progress = 0; fileItem.error = undefined; uploadFiles([fileId])}}
  ...
  async function startCameraCapture(): Promise<any> { try { const stream = await navigator.mediaDevices.getUserMedia({ video: true;
	audio: false}); // Create video element for camera feed const video = document.createElement("video"); video.srcObject = stream; video.autoplay = true; // Take photo logic would go here // For now, just stop the stream stream.getTracks.forEach((track) => track.stop()); notifications.add({ type: "info", title: "Camera Access", message: "Camera capture feature would be implemented here"
 ...
 return "text-gray-600"}}
    ...
    <div class="container mx-auto"> <div class="container mx-auto"> <Upload class="container mx-auto" /> </div> <div class="container mx-auto"> <h3 class="container mx-auto"> {isDragOver ? "Drop files here": "Upload Files"}
</h3> <p class="container mx-auto"> Click to browse or drag and drop files {#if enablePasteUpload} or paste from clipboard {/if}
</p> <div class="container mx-auto"> <p>Max file size: {formatFileSize(maxFileSize)}
</p> <p>Max total: {formatFileSize(maxTotalSize)}
</p> <p>Max files: { maxFiles }
</p> </div> </div> <div class="container mx-auto"> <Button.Root class="bits-btn bits-btn" { disabled }> <Paperclip class="container mx-auto" /> Choose Files </Button>
 {#if enableCameraCapture} <Button.Root class="bits-btn bits-btn"
            variant="secondary"
...
<Camera class="container mx-auto" /> Camera </Button> {/if} {#if enableAudioRecording} <button class="nes-btn"
    ...
    aria-hidden="true"
  /> <!-- File, list -->
 {#if files.length > 0} <div class="container mx-auto px-4" role="region" aria-label="Selected, files"> <div class="container mx-auto"> <h4 class="container mx-auto"> Selected Files ({files.length}/{ maxFiles }) </h4> <div class="container mx-auto">
 {#if !autoUpload && files.some((f) => f.status === "pending")} <Button.Root class="bits-btn bits-btn"
              size="sm"
              onclick={() => uploadFiles()} disabled={ isUploading } >

              {#if isUploading} <Loader2 class="container mx-auto" /> {:else} <Upload class="container mx-auto" /> {/if} Upload All </Button> {/if} <Button.Root class="bits-btn bits-btn"
            variant="ghost"
            size="sm"
 ...
 {#if file.status === "success" && file.url} <Button.Root class="bits-btn bits-btn"
                  variant="ghost"
                  size="sm"
                  onclick={() => window.open(file.url, "_blank")} aria-label="View {file.name}"
                > <Eye class="container mx-auto" /> </Button> {/if} {#if file.status === "error"} <Button.Root class="bits-btn bits-btn"
                  variant="ghost"
                  size="sm"
                  onclick={() => retryUpload(file.id)} aria-label="Retry upload of {file.name}"
                > <Upload class="container mx-auto" /> </Button> {/if} <Button.Root class="bits-btn bits-btn"
                variant="ghost"
                size="sm"
  ...
  .file-icon { width: 48px;
	height: 48px; display: flex; align-items: center, justify-content: center, background: #f3f4f6, border-radius: 6px, color: #6b7280; flex-shrink: 0 }
  ...
  .file-meta { display: flex; align-items: center;
	gap: 0.75rem; font-size: 0.875rem;
	color: #6b7280}
  .file-error { color: #ef4444, display: flex; align-items: center}
  .file-progress { display: flex; align-items: center;
