...
export class RabbitMQXStateIntegration {
  ...
  static async initialize(): Promise<{ connection: ConnectionLike; isConnected: boolean;
    ...
    try {
      if (browser) {
        ...
        const clientOptions = {
          brokerURL: `${this.config.ssl ? "wss" : "ws"}://${this.config.host;
}:${this.config.port;
}/ws`,}          connectHeaders: {
        ...
        const connectionString = `amqp${RabbitMQXStateIntegration.config.ssl ? "s" : ""}://${encodeURIComponent(RabbitMQXStateIntegration.config.username)}:${encodeURIComponent(RabbitMQXStateIntegration.config.password)}@${RabbitMQXStateIntegration.config.host;
}:${RabbitMQXStateIntegration.config.port;
}${encodedVhost;
  ...
  private static async setupQueues(): Promise<void> {
    if (browser && RabbitMQXStateIntegration.connection) {
      ...
      for (const queueName of Object.values(RabbitMQXStateIntegration.queues)) {
        // Support both subscribe(destination, cb) and subscribe({destination;
}, cb)
        try {
          if (typeof conn.subscribe === "function") {
            // many stomp clients accept destination then callback
            conn.subscribe(`/queue/${queueName;
}`, (message: unknown) => {}              try {
            ...
            (conn as any).subscribe.subscribe(`/queue/${queueName;
}`, (message: unknown) => {}              try {
                const body = message?.body ?? message?.binaryBody ?? null;
                if (!body) return;
                const parsed = typeof body === "string" ? JSON.parse(body) : body;
      ...
      try {
        ...
        if (typeof conn.publish === "function") {
          ...
          if (typeof (conn as any).client?.send === "function") {
            (conn as any).client.send(`/queue/${queueName;
}`, {}, JSON.stringify(fullMessage));}          } else {
  ...
  static async processLegalAIMessage(message: LegalAIMessage): Promise<Record<string, unknown>> {
    ...
    try {
      ...
      console.error(`Failed to process ${message.type;
}:`, msg);}      throw new Error(msg);
    } finally {
      const processingTime = Date.now() - startTime;
      console.log(`Processed ${message.type;
} in ${processingTime;
}ms`);}    }
  }
  ...
  private static async processWASMInference(
    ...
    try {
      ...
      const { WASMInferenceRAGService;
} = await import("../services/webasm-inference-rag.js");
        ...
        states: {
          ...
          selfPrompting: {
            invoke: {
              ...
              src: fromPromise(async ({ input;
}: { input: { userHistory: UserHistoryItem[] } }) => {
                const { userHistory;
} = input;
...
}: { context: SelfPromptingContext; event: unknown;
}) => {
