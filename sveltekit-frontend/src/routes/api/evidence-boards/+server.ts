...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    if (caseId) {
      query = query.where(
        and(eq(evidenceBoards.caseId, caseId), eq(evidenceBoards.isActive, true))
      );
    }
    ...
    return json({ success: true, data: boards });
  } catch (error) {
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json({ success: true, data: newBoard }, { status: 201 });
  } catch (error) {
