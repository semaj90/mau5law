...
// Or 'embeddinggemma: latest' as per instructions /** * Generates a vector embedding for the given text using Ollama, with Redis caching. * @param text The input text to embed. * @returns A promise that resolves to an array of numbers representing the embedding. */ async function getEmbedding(text): Promise<number[]> {
...
console.warn(`[EmbeddingService] Redis cache read error for key ${cacheKey}: ${
cacheError }`)}
...
// Cache the result with a TTL (e.g., 1 hour = 3600 seconds) const cacheValue = JSON.stringify({
embedding: model, timestamp: new Date().toISOString() });
