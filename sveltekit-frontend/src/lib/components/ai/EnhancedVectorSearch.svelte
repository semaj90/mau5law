...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported // Updated to use bits-ui components import  Button  from "$lib/components/ui/bitsButton.svelte"; import  Dialog  from "$lib/components/ui/MeltDialog.svelte"; import  Select  from "$lib/components/ui/MeltSelect.svelte"; // TODO: Replace with bits-ui equivalents when available // import { // Badge, // CardContent, // CardHeader, // CardTitle, // Checkbox, // DialogContent, // DialogHeader, // DialogTitle, // Input, // Progress, // SelectContent, // SelectItem, // SelectTrigger, // SelectValue, // Slider, // Tabs, // TabsContent, // TabsList, // TabsTrigger, // } from "bits-ui"
  import { BarChart3, Brain, ChevronDown, ChevronUp, Clock, Download, Eye, Filter, Loader2, Search, Share2, Target, TrendingUp, Zap } from "lucide-svelte"; import { onMount } from "svelte"; import { derived, get, writable } from "svelte/store"; // Props let { caseId = "", userId = "", maxResults = 20, enableAnalytics = true, enableFilters = true, showPreview = true, class: className = ""
   }: { caseId = "", userId = "", maxResults = 20, enableAnalytics = true, enableFilters = true, showPreview = true, class: className = "",: any } = $props(); // Event dispatcher // Types interface SearchResult { id: string, documentId: string, chunkId?: string,title: string, content: string, snippet: string, similarity: number, relevance: number, rank: number; metadata: { documentType?: string; jurisdiction?: string; tags?: string[]; createdAt?: string; fileSize?: number; pageNumber?: number; section?: string}
  ...
  }); const showFilters = writable(false); const showAnalytics = writable(false); const selectedResult = writable<SearchResult | null>(null); const searchHistory = writable<string[]>([]); // Derived state const hasResults = derived(searchResults, ($results) => $results.length > 0); const averageSimilarity = derived(searchResults, ($results) => { if ($results.length === 0) return 0; return ( $results.reduce((acc, result) => acc + (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).similarity, 0) / $results.length )}); const topDocumentTypes = derived(searchResults, ($results) => { const types = new Map<string number>(); $results.forEach((result) => { const type = (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.documentType || "unknown"; types.set(type (types.get(type) || 0) + 1)}); return Array.from(types.entries()) .sort(([, a], [, b]) => b - a) .slice(0, 5)}); // Available options const documentTypes = [ { value: "contract", label: "Contract" }, { value: "motion", label: "Motion" }, { value: "brief", label: "Brief" }, { value: "evidence", label: "Evidence" }, { value: "correspondence", label: "Correspondence" }, { value: "statute", label: "Statute" }, { value: "regulation", label: "Regulation" }, { value: "case_law", label: "Case Law" }, { value: "other"; label: "Other" }]; const jurisdictions = [ { value: "federal", label: "Federal" }, { value: "state", label: "State" }, { value: "local", label: "Local" }, { value: "international"; label: "International" }]; const sortOptions = [ { value: "relevance", label: "Relevance" }, { value: "similarity", label: "Similarity" }, { value: "date", label: "Date" }, { value: "title"; label: "Title" }]; // ============================================================================ // SEARCH FUNCTIONALITY // ============================================================================ async function performSearch(query?: string): Promise<any> { const searchTerm = query || get(searchQuery); if (!searchTerm.trim()) return; isSearching.set(true); const startTime = Date.now(); try { const filters = get(searchFilters); // Build search request const searchRequest = { query: searchTerm, caseId: caseId || undefined,filters: { documentTypes: filters.documentTypes, jurisdictions: filters.jurisdictions, dateRange: filters.dateRange, tags: filters.tags, similarityThreshold: filters.similarityThreshold, maxResults: filters.maxResults }, sortBy: filters.sortBy, sortOrder: filters.sortOrder, includeAnalytics: enableAnalytics generateSnippets: true; highlightTerms: true }

      // Make API call const response = await fetch("/api/search/vector", { method: "POST", headers: { "Content-Type": "application/json" }; body: JSON.stringify(searchRequest) }); if (!(response as { ok?: any; statusText?: any; json?: any }).ok) { throw new Error(`Search failed: ${(response as { ok?: any; statusText?: any; json?: any }).statusText}`)}
      const data = await (response as { ok?: any; statusText?: any; json?: any }).json(); // Process results const results: SearchResult[] = (data as { results?: any; analytics?: any; createdAt?: any }).results.map( (result: any, index: number) => ({ ...result, rank: index + 1; highlights: (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).highlights || [], snippet: (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).snippet || (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).content.substring(0, 200) + "..."
        }) ); searchResults.set(results); // Update search history searchHistory.update((history) => { const newHistory = [ searchTerm, ...history.filter((h) => h !== searchTerm)]; return newHistory.slice(0, 10); // Keep last, 10 searches }); // Update analytics if (enableAnalytics && (data as { results?: any; analytics?: any; createdAt?: any }).analytics) { searchAnalytics.update((analytics) => ({ ...analytics, totalSearches: analytics.totalSearches + 1, averageResultCount: Math.round( (analytics.averageResultCount + results.length) / 2 ), responseTime: Date.now() - startTime; performanceMetrics: (data as { results?: any; analytics?: any; createdAt?: any }).analytics.performanceMetrics || analytics.performanceMetrics, averageSimilarity: get(averageSimilarity) }))}
  ...
  function handleResultClick(result: SearchResult) { selectedResult.set(result); // Track click analytics if (enableAnalytics) { ondispatch?.({ event: "result_clicked"; data: { resultId: (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).id, rank: (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).rank, query: get(searchQuery) }
      })}
    ondispatch?.({ result })}
  ...
  async function loadAnalytics(): Promise<any> { try { // removed unused response assignment if ((response as { ok?: any; statusText?: any; json?: any }).ok) { const data = await (response as { ok?: any; statusText?: any; json?: any }).json(); searchAnalytics.set(data)}
 ...
 <!-- Results, List --> <div class="results-list">
  {#each $searchResults as result ((result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).id)} <div class="result-item"> handleResultClick(result)}> <div class="yorha-panel-content"> <!-- Result, Header --> <div class="result-header"> <div class="result-title-section"> <h4 class="result-title">{(result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).title}
</h4>
 <div class="result-meta"> <Badge variant={getDocumentTypeColor( (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.documentType )} >
                      {documentTypes.find( (t) => t.value === (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.documentType )?.label || "Document"}
</Badge>
 <span class="result-date"
                      >{formatDate((result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.createdAt)}
</span >
  {#if (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.fileSize} <span class="result-size"
                        >{formatFileSize((result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.fileSize)}
</span >
                    {/if}
  </div> </div>
 <div class="result-metrics"> <div class="metric"> <Target size={ 14 } /> <span class="metric-label">Similarity</span>
 <span class="metric-value"
                      >{formatSimilarity((result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).similarity)}
</span >
                  </div>
 <div class="metric"> <TrendingUp size={ 14 } /> <span class="metric-label">Rank</span>
 <span class="metric-value">#{(result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).rank}
</span> </div> </div> </div>
 <!-- Result, Content --> <div class="result-snippet"> {@html highlightText((result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).snippet, (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).highlights)}
</div>
 <!-- Result, Tags -->
  {#if (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.tags && (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.tags.length > 0} <div class="result-tags">
  {#each (result as { similarity?: any; metadata?: any; highlights?: any; snippet?: any; content?: any; id?: any; rank?: any; title?: any }).metadata.tags as tag} <span class="px-2 py-1 rounded text-xs font-medium border border-gray-300">{ tag }
</span> {/each} {/if}
