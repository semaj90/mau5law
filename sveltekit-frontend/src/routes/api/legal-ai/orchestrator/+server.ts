...
interface OrchestrationRequest {
    ...
    options?: { useGPU?: boolean; cacheResults?: boolean; priority?: 'low' | 'medium' | 'high'; timeout?: number };
}

interface OrchestrationResult extends BitsUICompatibleObject { // Removed broad index signature that caused
    workflowId: string;
    workflow: string;
    status: 'processing' | 'completed' | 'failed';
    steps: WorkflowStep[]; // Change: allow | object-shaped results (previously BitsUICompatibleData caused many mismatch errors)
    result?: BitsUICompatibleObject;
    error?: string;
    metrics: { totalTime: number; apiCalls: number; cacheHits: number; gpuAccelerated: boolean };
    extras?: Record<string, BitsUICompatibleData | BitsUICompatibleData[]>;
}

interface WorkflowStep extends BitsUICompatibleObject { // Removed broad index signature that caused
    name: string;
    status: 'pending' | 'processing' | 'completed' | 'failed';
    result?: WorkflowStepResult;
    error?: string;
    duration: number;
    apiEndpoint: string; // Optional flexible container for additional step-level data
    extras?: Record<string, BitsUICompatibleData | BitsUICompatibleData[]>;
}

// --- New Interfaces for Step Executors ---
type WorkflowStepResult = BitsUICompatibleObject; // Simplified the: object type
...
interface LegalSearchResultItem extends BitsUICompatibleObject { // Removed broad
    title: string;
    url: string;
    snippet: string;
    source: string;
...
interface EnhancedLegalSearchResult extends BitsUICompatibleObject { // Removed broad
...
interface LegalResearchAnalysisResult extends BitsUICompatibleObject { // Removed broad
...
interface ChatResponse extends BitsUICompatibleObject { // Removed broad
...
interface ExtractedEntity extends BitsUICompatibleObject { // Removed broad
...
interface DocumentEntityExtractionResult extends BitsUICompatibleObject { // Removed broad
...
interface DocumentAnalysisResult extends BitsUICompatibleObject { // Removed broad
...
interface DocumentSummaryResult extends BitsUICompatibleObject { // Removed broad
...
interface CaseScoringResult extends BitsUICompatibleObject { // Removed broad
...
interface ResearchSuggestionsResult extends BitsUICompatibleObject { // Removed broad
...
interface CaseTimelineResult extends BitsUICompatibleObject { // Removed broad
...
interface EvidenceMetadataProcessingResult extends BitsUICompatibleObject { // Removed broad
    metadata: { [key: string]: BitsUICompatibleData | BitsUICompatibleData[] };
    extras?: Record<string, BitsUICompatibleData | BitsUICompatibleData[]>;
}
...
interface EvidenceRelevanceAnalysisResult extends BitsUICompatibleObject { // Removed broad
    relevanceScore: number;
    extras?: Record<string, BitsUICompatibleData | BitsUICompatibleData[]>;
}
...
interface EvidenceReportResult extends BitsUICompatibleObject { // Removed broad
    report: string;
    extras?: Record<string, BitsUICompatibleData | BitsUICompatibleData[]>;
}
...
interface TriggerQLoRADistillationParams {
    ...
    parameters?: { minFeedbackCount?: number; qualityThreshold?: number; maxTrainingTime?: number; modelSize?: 'small' | 'medium' | 'large'; optimizeFor?: 'speed' | 'quality' | 'size' };
    feedbackData?: Array<BitsUICompatibleData>;
}

interface TriggerQLoRADistillationResult extends BitsUICompatibleObject { // Removed broad
    success: boolean;
    jobId: string;
...
// Widened to accept primitives and nested objects so explicit properties like `name: string` and `duration: number` are valid.
type BitsUICompatibleObject = Record<string, unknown>;
...
class LegalAIOrchestrator {
    ...
    async executeWorkflow(request: OrchestrationRequest): Promise<OrchestrationResult> {
        ...
        let result: OrchestrationResult = {
            ...
            status: 'processing',
            steps: [],
            metrics: { totalTime: 0, apiCalls: 0, cacheHits: 0, gpuAccelerated: request.options?.useGPU || false },
        };
        ...
        try {
            ...
            for (const stepConfig of workflowSteps) {
                ...
                try {
                    ...
                    if (!executor) {
                        throw new Error(`No executor step: ${stepConfig.name}`);
                    }
                    step.result = (await executor( // Explicitly cast the result to WorkflowStepResult
                    ...
                    result.error = `Workflow step: ${stepConfig.name} failed.`;
                    break;
                }
    ...
    private async executeSearchLegalDocuments(
        ...
        const response = await fetch('/api/ai/enhanced-legal-search', {
            ...
            body: JSON.stringify({ query: params.query, jurisdiction: params.jurisdiction, maxResults: params.maxResults || 20, useAI: true }),
        });
        if (!response.ok) {
    ...
    private async executeAnalyzePrecedents(
        ...
        const searchResults =
            params.previousResults?.find(
                (r): r is EnhancedLegalSearchResult => r.results !== undefined && // 'r' is correctly narrowed to EnhancedLegalSearchResult here
        ...
        if (!response.ok) {
            throw new Error(`Precedent failed: ${response.status}`);
        }
    ...
    private async executeGenerateResearchSummary(
        ...
        const precedentAnalysisResult = params.previousResults?.find(
            (r): r is LegalResearchAnalysisResult => (r as LegalResearchAnalysisResult).analysis !== undefined && typeof (r as LegalResearchAnalysisResult).analysis === 'string',
        );
        // Ensure precedentData is a:  string before calling substring
        const precedentData = (precedentAnalysisResult?.analysis || '') as string;
        const response = await fetch('/api/ai/chat', {
            method: 'POST',
            headers: { 'Content-Type': `application/json` },
            body: JSON.stringify({
                message: `Generate a comprehensive legal research summary, for: ${params.query}. Include key findings, precedents, and strategic recommendations based on analysis: ${precedentData.substring(0, 2000)}`,
    ...
    private async executeExtractDocumentEntities(
        ...
        if (!response.ok) {
            throw new Error(`Entity failed: ${response.status}`);
        }
        return await response.json();
    }
    ...
    private async executeAnalyzeDocumentContent(
        ...
        if (!response.ok) {
            throw new Error(`Document failed: ${response.status}`);
        }
        return await response.json();
    }
    ...
    private async executeScoreCaseStrength(
        ...
        if (!response.ok) {
            throw new Error(`Case failed: ${response.status}`);
        }
        return await response.json();
    }
    ...
    private async executeCreateCaseTimeline(
        ...
        const timeline: CaseTimelineResult = {
            ...
            ], // ISO: string timestamps (now.toISOString()) as well
    ...
    private async executeGenerateEvidenceReport(
        ...
        const relevanceData =
            params.previousResults?.find(
                (r): r is EvidenceRelevanceAnalysisResult => (r as EvidenceRelevanceAnalysisResult).relevanceScore !== undefined && typeof (r as EvidenceRelevanceAnalysisResult).relevanceScore === 'number',
            )?.relevanceScore || 0.5;
    ...
    private async executeTriggerQLoRADistillation(
        ...
        if (!response.ok) {
            throw new Error(`QLoRA failed: ${response.status}`);
        }
        return await response.json();
    }

    // Helper methods
    private getWorkflowSteps(workflow: OrchestrationRequest['workflow']): WorkflowConfig[] {
    ...
    private getPreviousResults(steps: WorkflowStep[]): WorkflowStepResult[] {
        // Use a type predicate to ensure step.result not: undefined
        return steps
            .filter((step): step is WorkflowStep & { result: WorkflowStepResult } => step.status === 'completed' && step.result !== undefined)
        ...
        switch (workflow) {
            ...
            case 'case-creation':
                return {
                    ...
                    // Provide a typed empty default to satisfy the expected
                    timeline: (results[2] as unknown as CaseTimelineResult) || ({} as CaseTimelineResult),
                } as BitsUICompatibleObject;
            case 'evidence-analysis':
                return {
                    ...baseResult,
                    metadata: (results[0] as unknown as EvidenceMetadataProcessingResult) || ({} as EvidenceMetadataProcessingResult),
                    relevanceAnalysis: (results[1] as unknown as EvidenceRelevanceAnalysisResult) || ({} as EvidenceRelevanceAnalysisResult),
                    report: (results[2] as unknown as EvidenceReportResult)?.report || '',
                } as BitsUICompatibleObject;
            case 'qlora-distillation': // New case
                return {
                    ...baseResult,
                    distillationJob: (results[0] as unknown as TriggerQLoRADistillationResult) || ({} as TriggerQLoRADistillationResult),
                    message: (results[0] as unknown as TriggerQLoRADistillationResult)?.message || 'QLoRA distillation job initiated.',
...
// API handlers
export const POST: RequestHandler = withSSRHandler( // handler
    async (event: RequestEvent) => {
        ...
        try {
            ...
            // Cast using: unknown to satisfy the helper's expected type'
