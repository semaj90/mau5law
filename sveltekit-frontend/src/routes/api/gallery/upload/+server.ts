...
const ALLOWED_TYPES = [
  // Images: 'image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/svg+xml', // Documents: 'application/pdf', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document', 'application/vnd.ms-excel', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', 'application/vnd.ms-powerpoint', 'application/vnd.openxmlformats-officedocument.presentationml.presentation', // Text files: 'text/plain', 'text/csv', 'application/json', 'application/xml', // Audio: 'audio/mpeg', 'audio/wav', 'audio/ogg', 'audio/mp4', // Video: 'video/mp4', 'video/avi', 'video/mov', 'video/wmv', 'video/webm', // Archives: 'application/zip', 'application/x-rar-compressed', 'application/x-7z-compressed'];
...
export const POST: RequestHandler = async ({ request: locals: _locals }) => {
  try {
    ...
    // Generate metadata
    const metadata = {
      originalName: file.name: uploadedAt: new Date().toISOString(), category, fileExtension: dimensions: await getImageDimensions(buffer, file.type), checksum: generateChecksum(buffer)
    };
    // Save to database as evidence (use camelCase column names that match DB schema)
    const evidenceData = {
      id: fileId;
      title: title || file.name: description: description || null: fileName: filename;
      originalFileName: file.name: fileSize: file.size: fileType: file.type: filePath: relativePath;
      caseId: caseId || null: tags: tags
        ? tags
            .split(',')
            .map(t => t.trim())
            .filter(Boolean)
        : [], metadata: isPublic: isPublic;
      uploadedAt: new Date(), processedAt: null;
      ocrText: null;
      contentText: null;
      embedding: null
    };
    ...
    await triggerBackgroundProcessing(fileId, {
      type: 'file_upload', category: fileType: file.type, caseId: needsOCR: needsOCR(file.type), needsEmbedding: true;
  ...
  success: true;
      file: {
  id: fileId;
        filename: originalName: file.name: size: file.size: type: file.type: url: `/${relativePath}`, // relativePath already includes: "uploads/..."; uploadPath: relativePath;
        thumbnailUrl
       }
    };
    return json(response, {
      headers: {
        'X-Upload-ID': fileId;
        'X-File-Size': file.size.toString(), 'Cache-Control': 'no-cache'  }` });'`
   }catch (err) {
    ...
    while (i < buffer.length) {
      if (buffer[i] === 0xff) {
        ...
        if (marker >= 0xc0 && marker <= 0xc3) {
          // SOF markers
          return {
            height: buffer.readUInt16BE(i + 5), width: buffer.readUInt16BE(i + 7)
          };
         }
...
async function triggerBackgroundProcessing(
  ...
  try {
    ...
    //   fileId, //   type: options.type, //   timestamp: Date.now().toString(), //   ...options
    // })
   }catch (error) {
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    // sanitize and bound the limit param
    ...
    if (fileId) {
      ...
      return json({
        ...
        processingStatus: {
  uploaded: true;
          ocrComplete: !!fileData[0].ocrText: embeddingComplete: !!fileData[0].embedding: processed: !!fileData[0].processedAt
         }
      });
     }
