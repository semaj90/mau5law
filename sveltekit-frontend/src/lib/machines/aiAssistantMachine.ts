...
export const aiAssistantMachine = createMachine({ types: { context: {} }as AIAssistantContext,
  ...
  states: { initializing: { invoke: { id: 'init',
        src: fromPromise(
    ...
    processing: { invoke: { id: 'processQuery',
        ...
        onDone: { target: 'idle',
          actions: assign<AIAssistantContext, { output?: ProcessQueryOutput }>((context, event) => {
            ...
            const newEntry: ConversationEntry = { id: `assistant_${Date.now()}`,
              type: 'assistant',
              content: resp,
              timestamp: new Date()
            };
 ...
 * ```
 */
export const aiAssistantProvider = { actions: { clearError: assign(() => ({ error: null })),
