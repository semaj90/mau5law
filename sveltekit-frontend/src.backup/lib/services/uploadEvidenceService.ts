...
const ALLOWED_TYPES = ['application/pdf', 'image/png', 'image/jpeg', 'image/tiff', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document'];
...
export async function uploadEvidence(
  ...
  try {
    ...
    // Initiate upload
    const initiation = await initiateUpload(
      caseId,
      file.name,
      file.size,
      file.type
    ...
    if (onProcessingEvent) {
      streamProcessingEvents(completion.job_id, onProcessingEvent, onError).catch(
        (error) => {
          if (onError) {
            onError(error);
          }
