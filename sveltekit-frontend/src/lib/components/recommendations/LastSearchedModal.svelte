<!-- 🔍 Last Searched Items Modal with AI Integration -->
<script lang="ts">
  ...
  let typeFilter = $state<SearchItem['searchType'] | 'all'>('all');
  ...
  async function generateAISuggestions() {
    ...
    // Generate smart suggestions (in real app, this would call your AI service)
    const suggestions = [
  ...
  function getConfidenceColor(confidence: number): string {
    ...
    if (confidence > 0.8) return palette.colors.success;
    ...
    <div class="search-list">
      ...
      {:else if filteredSearches.length === 0}
        <div class="empty-state">
          ...
          <p>Try adjusting your filters or start a new search</p>
        </div>
      {:else}
            ...
            <div class="search-main" onclick={() => selectedSearch = selectedSearch?.id === searchItem.id ? null : searchItem}>
              <div class="search-header">
                ...
                <div class="search-info">
                  <h4 class="search-query">{searchItem.query}</h4>
                  <div class="search-meta">
                    <span class="search-type">{searchItem.searchType}</span>
                    <span class="search-time">{formatTimeAgo(searchItem.timestamp)}</span>
                    <span class="result-count">{searchItem.resultCount} results</span>
                  </div>
                </div>
                <div class="search-stats">
                  ...
                  <div class="time-spent">{formatTimeSpent(searchItem.timeSpent)}</div>
                </div>
              </div>
                  ...
                  {#if searchItem.filters && Object.keys(searchItem.filters).length > 0}
                    <div class="filters-section">
                      ...
                      <div class="filter-tags">
                        {#each Object.entries(searchItem.filters) as [key, value]}
