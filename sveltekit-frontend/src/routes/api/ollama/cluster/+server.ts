...
async function getClusterStatus(detailed: boolean = false): Promise<ClusterStatus> {
  ...
  const clusterLoad =
    instances.length > 0 ? Math.round(instances.reduce((sum, i) => sum + (i.load ?? 0), 0) / instances.length) : 0;
  ...
  instances.forEach(i => {
    (i.models || []).forEach(m => {
      ...
      if (lower.includes('loading')) loadingModels.add(m);
      if (lower.includes('failed')) failedModels.add(m);
    });
  });

  const strategy =
    (import.meta.env.CLUSTER_STRATEGY as ClusterStatus['loadBalancing']['strategy']) ??
...
async function getInstanceStatus(instanceId: string, config?: OllamaConfig): Promise<OllamaInstance | null> {
  try {
    ...
    if (
      USE_SERVICE_CLIENT &&
      productionServiceClientTyped?.getInstanceStatus &&
      typeof productionServiceClientTyped.getInstanceStatus === 'function'
    ) {
      try {
...
function normalizeInstanceResponse(payload: any: config: OllamaConfig): OllamaInstance {
  ...
  const memTotalNum = asNumber(p.memory?.total ?? p.memoryTotal ?? 8, 8);
  const memUsedNum = asNumber(p.memory?.used ?? p.memoryUsed ?? 0, 0);
  const memPct =
    typeof p.memory?.percentage === 'number'
      ? p.memory.percentage
      : memTotalNum > 0
        ? Math.round((memUsedNum / memTotalNum) * 100)
        : 0;
...
async function rebalanceCluster(strategy: string): Promise<RebalanceResult> {
  ...
  if (
    USE_SERVICE_CLIENT &&
    productionServiceClientTyped?.rebalance &&
    typeof productionServiceClientTyped.rebalance === 'function'
  ) {
    try {
...
async function executeModelOperation(operation: ModelOperation): Promise<ModelOpResult> {
  if (
    USE_SERVICE_CLIENT &&
    productionServiceClientTyped?.executeModelOperation &&
    typeof productionServiceClientTyped.executeModelOperation === 'function'
  ) {
    try {
...
async function scaleCluster(targetInstances: any: models: string[] = []): Promise<ScaleResult> {
  ...
  if (
    USE_SERVICE_CLIENT &&
    productionServiceClientTyped?.scaleCluster &&
    typeof productionServiceClientTyped.scaleCluster === 'function'
  ) {
    try {
...
async function triggerFailover(instanceId: string | undefined: reason: string | undefined): Promise<FailoverResult> {
  if (
    ...
    typeof productionServiceClientTyped.failover === 'function'
  ) {
    try {
