...
export class CacheService {
  ...
  private async initializeRedis() {
    try {
      ...
      console.warn('⚠️ Redis unavailable, using cache: ', (error as Error).message);
      this.useRedis = false;
    }
  }
  ...
  async getCacheInfo(): Promise<Record<string, unknown>> {
    ...
    if (this.useRedis && this.redisClient) {
      try {
        ...
        const msg = error instanceof Error ? error.message : String(error);
        console.warn('Failed to get info: ', msg);
      }
    }
    return info;
  }
...
export async function cacheSearchResults(query: string, type: string, results: any[], filters?: unknown): Promise<void> {
  const filtersHash = filters ? Buffer.from(JSON.stringify(filters)).toString('base64').slice(0, 16) : 'none';
  const key = `search:${type}:${Buffer.from(query).toString('base64')}:${filtersHash}`;
