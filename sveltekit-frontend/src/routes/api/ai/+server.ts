...
Focus on examining evidence for:
...
Focus on connections between evidence:
...
- Timeline and resource planning.`,
  ...
  private analyzeLegalResponse(
    ...
    return {
      evidenceConnections: [...new Set(evidenceConnections)],
  ...
  private calculateConfidence(response: string, context?: string): number {
    ...
    if (context && contextKeywords[context as keyof typeof contextKeywords]) {
      ...
      const foundKeywords = keywords.filter((item) => response.includes(item));
    ...
    // Ensure confidence between 0 and 1
