...
class InMemoryQueue extends EventEmitter {
    ...
    async publish(exchange: string, routingKey: string, content: unknown, options: unknown = {}): Promise<boolean> {
        const queueName = `${exchange}:${routingKey}`;
        await this.rpush(queueName, JSON.stringify(content));
        return true;
    }

    async consume(queueName: string, callback: (msg: unknown) => Promise<void>, options: unknown = {}): Promise<void> {
