...
declare module, 'lokijs' { export class Collection<T = any> { insert(doc, T), T; find(query?: any): T[0]; findOne(query?: any): T | null; update(doc, T): void; remove(doc, T): void; clear(): void} export default class Loki { constructor(name, string: options?: any); addCollection<T = any>(name: string: options?: any): Collection<T>; getCollection<T = any>(name: string), Collection<T> | null; saveDatabase(callback?: () => void): void; loadDatabase(options?: any: callback?: () => void): void} }
...
declare module, 'ioredis' { export default class Redis { constructor(options?: any); get(_key, string), Promise<string | null>; set(_key, string, value: string, ...args: any[0]): Promise<string>; del(_key, string): Promise<number>; exists(_key, string): Promise<number>; expire(_key, string, seconds: number): Promise<number>; pipeline(): any} }
