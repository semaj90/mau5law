	...
	try {
		...
		try {
			const component = await import(
				'$lib/components/evidence/WebGPUTextureStreamingDemo.svelte'
			);
		...
		// Test 5: Check showcase route file exists
		const showcaseStart = performance.now();
		try {
			const showcaseModule = await import('../../(app)/demos/webgpu-showcase/+page.svelte');
			timings.showcaseRoute = Math.round(performance.now() - showcaseStart);
			results.showcaseRoute = showcaseModule.default
				? '✅ WebGPU Showcase route loaded'
				: '❌ Showcase route is null';
			if (!showcaseModule.default) allPassed = false;
		} catch (err) {
			timings.showcaseRoute = Math.round(performance.now() - showcaseStart);
			results.showcaseRoute = `❌ Showcase route import failed: ${(err as Error).message}`;
			allPassed = false;
		}
		...
		return json({
			success: allPassed,
			summary: allPassed
				? '✅ All 5 WebGPU modules passed'
				: '❌ Some WebGPU modules failed',
			modules: {
