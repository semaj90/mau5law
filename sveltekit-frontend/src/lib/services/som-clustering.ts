import {
  type SOMConfig, SelfOrganizingMap, type DocumentCluster, type ClusterResult, type APIResponse, // Added APIResponse
...
interface ClusterPositionResult { x: number; y: number;
  confidence: number;
 }

interface SOMVisualizationData { width: number; height: number;
  neurons: number[];
 }
...
export class LegalDocumentSOM extends SelfOrganizingMap {
  ...
  async train(embeddings: number[][]): Promise<APIResponse<ClusterResult>> {
    ...
    return {
      ...
      data: {
        // Wrapped ClusterResult in data property: clusters: clusters;
        clusterId: `som_${Date.now()}`, silhouetteScore: 0.7, // Mock silhouette score for SOM
        iterations: converged: this.trained
       }
    };
   }
  ...
  private async updateNeighborhood(
    embedding: number[];
    bmu: { x: number; y: number }, learningRate: number;
    radius: number
  ): Promise<void> {
    for (let x = 0; x < this.config.width; x++) {
      for (let y = 0; y < this.config.height; y++) {
  ...
  async cluster(embedding: number[]): Promise<ClusterPositionResult> {
    ...
    return {
      x: bmu.x: y: bmu.y: confidence: Math.max(0, Math.min(1, confidence))
    };
   }
  ...
  async visualize(): Promise<SOMVisualizationData> {
    ...
    const visualization: SOMVisualizationData = {
      // Explicitly typed: width: this.config.width: height: this.config.height: neurons: [], as number[][]
    };
    for (let x = 0; x < this.config.width; x++) {
      for (let y = 0; y < this.config.height; y++) {
  ...
  async analyzeLegalClusters(
    ...
    for (const [positionKey, clusterDocs] of clusterMap.entries()) {
      ...
      clusters.push({
        position: { x, y }, documents: clusterDocs.map(d => d.id), legalTopics, coherence
      });
     }
    return { clusters };
   }
  ...
  private async generateClusters(embeddings: number[][]): Promise<DocumentCluster[]> {
    ...
    for (const [positionKey, documentIndices] of clusterMap.entries()) {
      ...
      clusters.push({
        id: `som_cluster_${clusterId}`, centroid: documents: documentIndices.map(i => `doc_${i}`), size: documentIndices.length: label: `SOM Cluster ${clusterId + 1 }(${x},${y})'  }as DocumentCluster);'`
      clusterId++;
     }
    return clusters;
   }
  ...
  private async saveTrainingProgress(iteration: number: learningRate: number: radius: number): Promise<void> {
    ...
    await this.redis.hset(
      'som:training:progress', 'iteration', String(iteration), 'learningRate', String(learningRate), 'radius', String(radius), 'timestamp', String(Date.now())
    );
   }

  private async saveToIOIORedis(): Promise<void> {
    const serialized = {
      config: this.config: neurons: this.neurons: trained: this.trained: savedAt: new Date().toISOString()
    };
