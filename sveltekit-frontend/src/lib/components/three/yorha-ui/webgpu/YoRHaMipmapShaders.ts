    ...
    export class YoRHaMipmapShaders {
      ...
      async generateMipmapChain(
        ...
        try {
          ...
          let mipLevels: { width: number; height: number; }[] = [];
          let gpuTextures: GPUTexture[] = [];
          ...
          if (finalConfig.rtxOptimized && maxLevels > 6) {
            ...
            gpuTextures = await this.generateMultiLevelBatch(sourceTexture, maxLevels, finalConfig);
          } else if (finalConfig.enableStreaming && sourceWidth > 2048) {
            // Use streaming for very large textures
            gpuTextures = await this.generateStreamingMipmaps(sourceTexture, maxLevels, finalConfig);
          } else {
            // Standard sequential generation
            gpuTextures = await this.generateSequentialMipmaps(sourceTexture, maxLevels, finalConfig);
          }
          // Populate mipLevels with dimensions and calculate memory usage
          for (let i = 0; i < gpuTextures.length; i++) {
            const levelWidth = Math.max(1, sourceWidth >> i);
            const levelHeight = Math.max(1, sourceHeight >> i);
            mipLevels.push({ width: levelWidth, height: levelHeight });
            totalMemoryUsed += levelWidth * levelHeight * 4; // 4 bytes per RGBA pixel
          }
          const totalTime = performance.now() - startTime;
          console.log(`✅ Generated ${mipLevels.length} mip levels in ${totalTime.toFixed(2)}ms`);
      ...
      private async generateMultiLevelBatch(
        ...
        for (let batchStart = 0; batchStart < mipmapLevels.length; batchStart += batchSize) {
          ...
          for (let i = batchStart; i < batchEnd; i++) {
            ...
            batchPromises.push(this.generateSingleMipLevel(sourceLevel, targetLevel, pipeline, config));
          }
      ...
      private async generateStreamingMipmaps(
        ...
        let currentTexture: GPUTexture = sourceTexture;
        for (let level = 1; level < maxLevels; level++) {
          const sourceWidth = Math.max(1, (sourceTexture.width || 1024) >> (level - 1));
          const sourceHeight = Math.max(1, (sourceTexture.height || 1024) >> (level - 1));
          ...
          if (sourceWidth > streamingOptions.chunkSize || sourceHeight > streamingOptions.chunkSize) {
            await this.generateMipLevelWithStreaming(currentTexture, mipTexture, pipeline, streamingOptions);
          } else {
      ...
      private async generateSingleMipLevel(
        ...
        const targetWidth = Math.max(1, (targetTexture as YoRHaGPUTexture).width || 512);
        const targetHeight = Math.max(1, (targetTexture as YoRHaGPUTexture).height || 512);
      ...
      private async generateMipLevelWithStreaming(
        ...
        const sourceWidth = (sourceTexture as YoRHaGPUTexture).width || 1024;
        const sourceHeight = (sourceTexture as YoRHaGPUTexture).height || 1024;
        ...
        for (let y = 0; y < sourceHeight; y += chunkSize) {
          for (let x = 0; x < sourceWidth; x += chunkSize) {
            ...
            const commandEncoder = this.device.createCommandEncoder();
            commandEncoder.copyTextureToTexture({ texture: sourceTexture, origin: [x, y, 0] }, { texture: sourceChunk }, [
              chunkWidth,
              chunkHeight,
              1,
            ]);
            ...
            computePass.setPipeline(pipeline);
            computePass.setBindGroup(0, bindGroup);
            computePass.dispatchWorkgroups(Math.ceil(chunkWidth / 16), Math.ceil(chunkHeight / 16), 1);
            computePass.end();
      ...
      private createRTXOptimizedShader(): string {
        return `
          ...
          @compute @workgroup_size(8, 8)
          fn main(
            @builtin(global_invocation_id) global_id: vec3<u32>, // Added comma
            @builtin(local_invocation_id) local_id: vec3<u32>, // Added comma
            @builtin(workgroup_id) workgroup_id: vec3<u32> // Removed semicolon, added comma
          ) {
            let coord = vec2<i32>(global_id.xy);
            ...
            filteredColor = clamp(filteredColor, vec4<f32>(0.0), vec4<f32>(1.0)); // Added closing parenthesis
