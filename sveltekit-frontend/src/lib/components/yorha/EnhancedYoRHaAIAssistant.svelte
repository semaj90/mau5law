...
enabled:true, storage: 'session' }
  ...
  let evidenceEditorRef = $state<HTMLDivElement | null>(null);
  ...
  function addMessage(role: 'user' | 'assistant', content: string) {
    const message = { id: crypto.randomUUID(), role, content, timestamp: new Date() };
    ...
    <div class="ai-assistant-container"
      ...
      <header class="assistant-header">
        ...
        <div class="header-controls">
          <div class="mode-switcher">
            ...
            <button class="mode-btn" class:active={currentMode === 'evidence'} onclick={() => switchMode('evidence')}> 📁 Evidence </button>
            <button class="mode-btn" class:active={currentMode === 'analysis'} onclick={() => switchMode('analysis')}> 📊 Analysis </button>
          </div>
      ...
      <div class="search-section">
        <div class="search-container">
          ...
          />
          <button class="search-btn" onclick={handleSearch} disabled={isProcessing || !searchQuery.trim()}>
            {isProcessing ? '⚡' : '🔍'}
          </button>
        </div>
        <!-- Context Toggle -->
        <div class="context-controls">
          <button class="context-toggle" onclick={() => (contextExpanded = !contextExpanded)}> 📋 Context ({evidenceItems.length}) </button>
          {#if ragStatus !== 'idle'}
            <div class="rag-status" class:processing={ragStatus === 'streaming'}> RAG: {ragStatus} ({ragTokenCount} tokens) </div>
          {/if}
        </div>
      </div>
          ...
          <div class="evidence-container">
            <div class="evidence-header">
              ...
              <div class="evidence-controls">
                <input type="file" id="evidence-upload" style="display: none;" onchange={(e: any) => { const file = e.target?.files?.[0]; if (file) addEvidence(file)}} />
                <button onclick={() => document.getElementById('evidence-upload')?.click()}> 📁 Upload </button>
            ...
            <div class="analysis-sections">
              <section class="analysis-section">
                ...
                <div class="analysis-content">
                  <p> Analysis based on {evidenceItems.length} evidence items and {chatMessages.length} interactions. </p>
                  <ul>
                    <li> Key legal issues identified: Contract interpretation, liability assessment </li>
...
<style>
  .ai-assistant-overlay { position: fixed;
		inset: 0;
  ...
  .context-panel { background: #1a1a1a; border-bottom: 1px solid #333;
		padding: 1rem 1.5rem; }
  ...
  .analysis-section { background: #1a1a1a;
		border: 1px solid #333; border-radius: 8px;
	padding: 1.5rem; }
  .analysis-section h3 { margin: 0 0 1rem 0;
		color: #ffd700; border-bottom: 1px solid #333; padding-bottom: 0.5rem; }
  ...
  @media (max-width: 1000px) {
    .ai-assistant-container { width: 95vw !important;
		height: 85vh !important; max-width: none; }
