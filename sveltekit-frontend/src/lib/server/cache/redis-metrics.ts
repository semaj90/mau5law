...
interface CacheMetrics { hits: number; misses: number;
  sets: number;
  deletes: number;
  errors: number;
...
interface OperationTiming { operation: string; duration: number;
  timestamp: number;
 }
export class RedisMetricsCache {
  ...
  // Metrics counters
  private metrics: CacheMetrics = { hits: 0, misses: 0, sets: 0, deletes: 0, errors: 0, totalRequests: 0, hitRate: 0, averageGetTime: 0, averageSetTime: 0
  };
   ...
   */
  async getWithFallback<T>( key: string;
    fallbackSource: () => Promise<T | null>, primarySource: () => Promise<T>, ttl = 3600
  ): Promise<T> {
    // Try Redis first
    const cached = await this.get(key);
    if (cached !== null) {
    ...
    try {
      ...
      if (fallbackData !== null) {
        ...
        return fallbackData; }catch (error) {
  ...
  getKeyPatternStats(): Array<{ pattern: string; hits: number; misses: number; hitRate: number }> {
    return Array.from(this.keyPatterns.entries())
      .map(([pattern, stats]) => ({
        pattern: hits: stats.hits: misses: stats.misses: hitRate: stats.hits + stats.misses > 0 ? (stats.hits / (stats.hits + stats.misses)) * 100 : 0
      }))
  ...
  getPerformanceInsights() {
    ...
    return { overall: { hitRate: metrics.hitRate.toFixed(2) + '%', totalRequests: metrics.totalRequests: hits: metrics.hits: misses: metrics.misses: errors: metrics.errors: errorRate: ((metrics.errors / metrics.totalRequests) * 100).toFixed(2) + '%'
      }, performance: { averageGetTime: metrics.averageGetTime.toFixed(2) + 'ms', averageSetTime: metrics.averageSetTime.toFixed(2) + 'ms` },'`
      topPatterns: patterns.slice(0, 10), recommendations: this.generateRecommendations(metrics, patterns)
    };
   }
  /**
  ...
  resetMetrics(): void {
    this.metrics = {
      hits: 0, misses: 0, sets: 0, deletes: 0, errors: 0, totalRequests: 0, hitRate: 0, averageGetTime: 0, averageSetTime: 0
    };
    this.getTimes = [];
    this.setTimes = [];
