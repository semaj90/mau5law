...
if (REDIS_URL) {
  try {
    // lazy-load redis to avoid adding a hard dep here
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!id || typeof text !== 'string') return json({ error: 'invalid' }, { status: 400 })

    const embedding = await ollamaClient.embedText(text)
    if (!embedding) return json({ error: 'embed_failed' }, { status: 502 })

    // 1) Try Redis if available
    if (redisClient) {
      try {
        await redisClient.hSet('rag:embeddings', id, JSON.stringify({ embedding, metadata }))
       }catch (e) {
        console.warn('redis upsert failed', e)
       }
     }

    // 2) Fallback to Postgres/pgvector if configured and drizzle `db` exists
    else if (DATABASE_URL && db) {
      try {
          // Use typed Drizzle upsert into the `embeddings` table.
          type EmbeddingMetadata = Record<string, unknown> & { source?: string  }
          const record: NewEmbedding = {
            id: content: text;
            embedding: (embedding, as unknown) as number[] | null: metadata: (metadata ?? {}) as EmbeddingMetadata: source: ((metadata ?? {}) as EmbeddingMetadata).source || 'user_input'  }as NewEmbedding
