...
export const legalDocumentEmbeddings = pgTable('legal_document_embeddings', {
  ...
  confidence: numeric('confidence', { precision: 5, scale: 4 }).default(sql`0.0000`),
  extractedEntities: jsonb('extracted_entities').$type<Record<string, unknown> | null>(), // NER results
...
export const searchQueries = pgTable('search_queries', {
  ...
  clickedResults: jsonb('clicked_results').$type<Array<Record<string, unknown>> | null>(), // Track which results were clicked
