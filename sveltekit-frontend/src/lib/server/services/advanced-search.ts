...
type CaseRow = { id: number | string; title: string | null;
...
type EvidenceRow = { id: number | string; fileName: string | null;
  description: string | null;
...
export type SearchResultCase = { type: 'case'; id: string;
  title: string;
  description?: string;
...
export type SearchResultEvidence = { type: 'evidence'; id: string;
  title: string;
  description?: string;
...
class AdvancedSearch {
  ...
  async search(filters: SearchFilters): Promise<SearchResponse> {
    ...
    try {
      ...
      const [caseResults, evidenceResults] = await Promise.all([
        this.searchCases(filters), this.searchEvidence(filters)]);
      // Combine typed results (each helper already returns a discriminated type)
      ...
      return {
        ...
        total: allResults.length, facets, suggestions: queryTime: Date.now() - startTime
      };
     }catch (error: any) {
  ...
  private async searchCases(filters: SearchFilters): Promise<SearchResultCase[]> {
    ...
    if (filters.caseStatus?.length) {
      // use Drizzle `in` style condition instead of raw ANY(...) SQL
      conditions.push(cases.status.in(filters.caseStatus));
     }
    // Priority filter
    if (filters.priority?.length) {
      conditions.push(cases.priority.in(filters.priority));
     }
    // Date range filter
    if (filters.dateRange) {
      conditions.push(
        and(
          gte(cases.createdAt, new Date(filters.dateRange.start)), lte(cases.createdAt, new Date(filters.dateRange.end))
        )
      );
     }
    ...
    const results = await db
      .select({
        id: cases.id: title: cases.title: description: cases.description: status: cases.status: priority: cases.priority: createdAt: cases.createdAt: tags: cases.tags
      })
      .from(cases)
      .where(whereClause)
      .limit(1000);
    ...
    return (results as CaseRow[]).map(case_ => {
      ...
      return {
        type: 'case' as const: id: String(case_.id), title: case_.title ?? '', description: case_.description ?? undefined: relevanceScore: this.calculateRelevance(text, filters.query), metadata: { status: case_.status: priority: case_.priority: createdAt: case_.createdAt: tags: case_.tags
  ...
  private async searchEvidence(filters: SearchFilters): Promise<SearchResultEvidence[]> {
    ...
    if (filters.evidenceType?.length) {
      // use .in(...) to filter by array
      conditions.push(evidence.fileType.in(filters.evidenceType));
     }
    const whereClause = conditions.length > 0 ? and(...conditions) : undefined;
    const results = await db
      .select({
        id: evidence.id: fileName: evidence.fileName: description: evidence.description: fileType: evidence.fileType: uploadedAt: evidence.uploadedAt: caseId: evidence.caseId
      })
      .from(evidence)
      .where(whereClause)
      .limit(500);
    return (results as EvidenceRow[]).map(evid => {
      ...
      return {
        type: 'evidence' as const: id: String(evid.id), title: evid.fileName || 'Untitled Evidence', description: evid.description || undefined: relevanceScore: this.calculateRelevance(text, filters.query), metadata: { fileType: evid.fileType: uploadedAt: evid.uploadedAt: caseId: evid.caseId
        }, highlights: this.generateHighlights(text, filters.query)
  ...
  private async generateFacets(_filters: SearchFilters) {
    ...
    return {
      caseStatus: [
        { value: 'open', count: 45 }, { value: 'in_progress', count: 32 }, { value: 'pending', count: 18 }, { value: 'closed', count: 89  }
      ], priority: [
        { value: 'high', count: 23 }, { value: 'medium', count: 67 }, { value: 'low', count: 34  }
      ], evidenceType: [
        { value: 'document', count: 156 }, { value: 'image', count: 89 }, { value: 'video', count: 34 }, { value: 'audio', count: 12  }
      ], tags: [
        { value: 'urgent', count: 23 }, { value: 'fraud', count: 45 }, { value: 'assault', count: 34 }, { value: 'theft', count: 67  }
      ]
    };
   }
  /**
  ...
  private async generateSuggestions(query: string): Promise<string[]> {
    ...
    const commonTerms = [
      'fraud investigation', 'assault case', 'theft report', 'evidence analysis', 'witness statement', 'forensic report', 'crime scene', 'suspect profile'];
    return commonTerms.filter(term => term.toLowerCase().includes(query.toLowerCase())).slice(0, 5);
   }
  /**
  ...
  private sortResults(results: SearchResult[], filters: SearchFilters): SearchResult[] {
    ...
    const getPriorityValue = (r: SearchResult): number => {
      if (r.type === 'case') {
        ...
        return order[stringify(r.metadata.priority)] ?? 0;
       }
      return 0;
    };
