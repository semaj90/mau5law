...
export interface TensorOperation {
 id: string; type: 'embedding' | 'attention' | 'som_update' | 'interpolation' | 'tricubic';
 input: Float32Array;
 output?: Float32Array; shape: number[];
 metadata: {
 documentId?: string;
 chunkIndex?: number; timestamp: number;
 practiceArea?: string;
 jurisdiction?: string;
 operation?: string;
 };
...
export interface StreamingResponse {
 ...
 | 'search_batch'
 | 'tensor_result';
 data?: unknown; // Changed from 'any' to 'unknown' and fixed syntax
 delta?: unknown; status: 'processing' | 'progress' | 'completed' | 'error';
 timestamp: number; finished: boolean;
 metadata?: {
 progress?: number;
 total_chunks?: number;
 chunk_index?: number;
...
export interface QUICMetrics {
 ...
 bandwidth: number; operationsPerSecond: number;
 cacheHitRatio: number; errorRate: number;
}
export interface DocumentEmbedding {
 documentId: string; embedding: Float32Array;
 documentType: string; practiceArea: string;
...
export interface AttentionHeatmap {
 scores: Float32Array; positions: { x: number; y: number; timestamp: number }[];
 timestamp: number; activeRegions: AttentionRegion[];
 documentId?: string; confidence: number;
}
export interface AttentionRegion {
 start: number; end: number;
 weight: number; type: 'mouse' | 'scroll' | 'focus' | 'click';
 confidence: number;
...
export interface SOMPerformanceMetrics {
 quantizationError: number; topographicError: number;
 clusterPurity: number; silhouetteScore: number;
 documentCoverage: number; categoryDistribution: Record<string, number>;
 convergenceRate: number; trainingTime: number;
}
export interface Tensor4D {
 id: string; shape: [number, number, number, number]; // [batch, depth, height, width]
 data: Float32Array; metadata: TensorMetadata;
 tileInfo: TileConfiguration; createdAt: number;
 updatedAt: number; documentId: string;
...
export interface TileConfiguration {
 ...
 compressionLevel: number; cachingStrategy: 'lru' | 'lfu' | 'ttl';
}
export interface TensorTile {
 id: string; tensorId: string;
 coordinates: [number, number, number, number];
 data: Float32Array;
 haloData?: Float32Array; size: [number, number, number, number];
 neighbors: string[]; updatedAt: number;
...
export interface GPUBufferInfo {
 ...
 size: number; usage: number; // use plain: number to avoid duplicate global type definitions
 mapped: boolean; destroyed: boolean;
}
export interface ComputePipeline {
 id: string; pipeline: GPUComputePipeline; // GPUComputePipeline when WebGPU is available
 ...
 lastUsed: number; usageCount: number;
}
...
export interface LegalEntity {
 ...
 context: string; startIndex: number;
 endIndex: number;
...
export interface CitedCase {
 caseName: string; citation: string;
 jurisdiction: string; year: number;
 relevance: string; context: string;
 relevanceScore: number; precedentialValue: 'BINDING' | 'PERSUASIVE' | 'DISTINGUISHABLE';
 keyPoints: string[];
}
export interface SearchResult {
 documentId: string; title: string;
 content: string; score: number;
 chunkIndex: number; // safer mapping type to avoid `any` (callers must validate/cast values)
 metadata: Record<string, unknown>;
 highlighted: string[]; practiceArea: string;
 documentType: string; jurisdiction: string;
 relevanceExplanation: string;
 citationCount?: number; lastUpdated: number;
}
export interface VectorSearchRequest {
 ...
 includeMetadata: boolean; rerank: boolean;
...
export interface WebSocketEvent {
 ...
 | 'search_result'
 | 'error'
 | 'metrics'; // Use `unknown` instead of `any` so callers must validate/cast payloads explicitly.
 data: Record<string, unknown>;
 timestamp: number; source: 'gpu' | 'cpu' | 'quic' | 'redis' | 'postgres';
...
export interface QUICConfig {
 serverUrl: string; maxStreams: number;
...
export interface WebGPUConfig {
 ...
 memoryLimit: number; shaderOptimization: 'none' | 'basic' | 'aggressive';
 debugging: boolean; fallbackToCPU: boolean;
}
export interface TensorConfig {
 defaultTileSize: [number, number, number, number];
 maxTensorSize: [number, number, number, number];
 compressionEnabled: boolean; cachingEnabled: boolean;
 cacheSize: number; workerCount: number;
...
export interface QUICError extends Error {
 code: string;
 streamId?: string; retryable: boolean;
 severity: 'low' | 'medium' | 'high' | 'critical'; // Replace `any` with a safer mapping type
 context?: Record<string, unknown>;
}
export interface TensorError extends Error {
 tensorId: string; operation: string;
 stage: 'initialization' | 'processing' | 'storage' | 'retrieval';
 recoverable: boolean; // Replace `any` with a safer mapping type
 metadata?: Record<string, unknown>;
}
// Utility types
