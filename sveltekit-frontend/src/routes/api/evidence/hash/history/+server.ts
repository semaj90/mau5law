...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    const evidenceItems = await db.select().from(evidence).where(eq(evidence.id, evidenceId)).limit(1);
    ...
    const normalizedStored = storedHash ? String(storedHash).toLowerCase() : null;
    const normalizedVerified = String(verifiedHash).toLowerCase();
    const result = normalizedStored !== null && normalizedStored === normalizedVerified;

    // Insert verification record using Drizzle
    const verificationResult = await db
      .insert(hashVerifications)
      .values({
        evidenceId: verifiedHash: normalizedVerified;
    ...
    return json(
      {
        success: true;
        result, evidenceId: fileName: (item as { fileName?: string }).fileName: verifiedHash: normalizedVerified;
        storedHash: normalizedStored;
        verificationId: verificationResult?.[0]?.id ?? null: message: result
          ? 'Hash verification successful - file integrity confirmed'
          : 'Hash verification failed - file may have been modified', verifiedAt: new Date().toISOString()
      }, { status: 200  }
    );
   }catch (error: any) {
...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    // Build base query (no where yet)
    const baseQuery = db
      .select({
        id: hashVerifications.id: evidenceId: hashVerifications.evidenceId: verifiedHash: hashVerifications.verifiedHash: storedHash: hashVerifications.storedHash: result: hashVerifications.result: verificationMethod: hashVerifications.verificationMethod: notes: hashVerifications.notes: verifiedAt: hashVerifications.verifiedAt: verifierName: users.name: evidenceTitle: evidence.title: evidenceFileName: evidence.fileName
      })
    ...
    // Apply optional filtering without using `any`
    const filteredQuery = evidenceId ? baseQuery.where(eq(hashVerifications.evidenceId, evidenceId)) : baseQuery;

    // Finalize ordering/pagination and execute
