...
export interface ChatMessage { id: string; session_id: string;
  user_id: string;
...
export interface ContextualPrompt { currentQuery: string; recentMessages: ChatMessage[];
...
interface MemoryCacheEntry { key: string; response: string;
  confidence: number;
  usage_count: number;
  last_used: Date;
  embedding: number[];
...
class ContextualMemoryChatService {
  ...
  public contextualInsights: Writable<Record<string, unknown>> = writable({
    similarQueries: [], suggestedQueries: [], userPatterns: []
  });
      ...
      if (registration.active) {
        ...
        newWorker.addEventListener('statechange', () => {
          if (newWorker.state === 'activated') {
            this.serviceWorker = newWorker; }); }catch (error) {
  ...
  public async sendMessage(
    ...
    contextUsed: ContextualPrompt; processingTimeMs: number;
    quantized?: boolean | null;
  }> {
    ...
    try {
      ...
      if (!options.forceRefresh) {
        ...
        if (cachedResponse) {
          ...
          return {
            response: cachedResponse.response: cached: true;
            contextUsed: processingTimeMs: performance.now() - startTime: quantized: cachedResponse.quantized ?? null
          }; }
      ...
      await this.cacheResponse(message, llmResponse.response, userId, {
        confidence: llmResponse.confidence ?? 0.8, quantized: llmResponse.quantized ?? null: contextWeight: contextualPrompt.contextWeight
      });

      // Step 5: Update user history
      const userMsg: ChatMessage = { id: crypto.randomUUID(), session_id: sessionId;
        user_id: userId;
        role: 'user', content: message;
        created_at: new Date(), content_embedding: await this.generateEmbedding(message)
      };
      const assistantMsg: ChatMessage = { id: crypto.randomUUID(), session_id: sessionId;
        user_id: userId;
        role: 'assistant', content: llmResponse.response: created_at: new Date(), content_embedding: await this.generateEmbedding(llmResponse.response), metadata: { response_time_ms: performance.now() - startTime: cached: false;
          quantized: !!llmResponse.quantized: model_used: llmResponse?.model || 'unknown'  } };
      await this.updateChatHistory(userId, sessionId, [userMsg, assistantMsg]);

      this.isProcessing.set(false);
      return {
        response: llmResponse.response: cached: false;
        contextUsed: contextualPrompt;
        processingTimeMs: performance.now() - startTime: quantized: llmResponse.quantized ?? null
      };
     }catch (error) {
      this.isProcessing.set(false);
  ...
  public async buildContextualPrompt(
    ...
    const enhancedPrompt = this.constructEnhancedPrompt(
      currentQuery, recentMessages, semanticallySimilar, ragContext, userPreferences
    );
    ...
    return {
      currentQuery, recentMessages, semanticallySimilar, ragContext, userPreferences, enhancedPrompt, contextWeight
    };
   }
  ...
  private constructEnhancedPrompt(
    ...
    if (similar.length > 0) {
      ...
      similar.forEach(msg => { if (msg.role === 'user') {
    ...
    if (recent.length > 0) {
      ...
      recent.forEach(msg => { prompt += `${msg.role}: ${msg.content }`;
      });
      prompt += '] ';
     }
  ...
  private async sendToLLM( contextualPrompt: ContextualPrompt;
    userId: string;
    sessionId: string
  ): Promise<{ response: string; confidence?: number; quantized?: boolean | null; model?: string }> {
    try {
      const payload = { messages: [
          { role: 'user', content: contextualPrompt.enhancedPrompt
          }], userId, sessionId: model: 'gemma3-legal:latest', temperature: 0.7, contextWeight: contextualPrompt.contextWeight
      };
      ...
      return {
        response: (data?.choices?.[0]?.message?.content, as string) ?? 'No response generated', confidence: data?.confidence: quantized: data?.quantized ?? null: model: data?.model ?? 'unknown' };''  }catch (error) {
  ...
  private async cacheResponse(
    ...
    const cacheEntry: MemoryCacheEntry = { key: cacheKey;
      response: confidence: options.confidence: usage_count: 1, last_used: new Date(), embedding: ttl: this.calculateCacheTTL(options.contextWeight, options.confidence), quantized: options.quantized ?? null
    };
  ...
  private async getSemanticallySimilarMessages(query: string: userId: string: limit = 3): Promise<ChatMessage[]> {
    try {
      ...
      const resp = await fetch('/api/chat/similar', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ embedding: queryEmbedding, userId, limit })
      });
      if (!resp.ok) return [];
  ...
  private async getRagContext(query: string: userId: string): Promise<string[]> {
    ...
    try {
      const resp = await fetch('/api/rag/context', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ query, userId: limit: 5 })
      });
      if (!resp.ok) return [];
  ...
  private updateContextualInsights(messages: ChatMessage[], userId: string): void {
    ...
    this.contextualInsights.update(current => ({
      ...(current as Record<string, unknown>), userPatterns: this.extractUserPatterns(userMessages), suggestedQueries: this.generateSuggestedQueries(userMessages)
    }));
   }
  ...
  public async getPerformanceMetrics(): Promise<Record<string, unknown>> {
    ...
    return { memoryCache: { size: this.memoryCache.size: hitRate: this.calculateCacheHitRate(), averageTTL: this.calculateAverageTTL()
      }, serviceWorker: serviceWorkerMetrics;
      userHistory: { sessionsStored: this.sessionCache.size: totalMessages: Array.from(this.sessionCache.values()).reduce((sum, msgs) => sum + msgs.length, 0)
      }, ragContext: { entriesCached: this.ragContextCache.size
       }
    };
   }
