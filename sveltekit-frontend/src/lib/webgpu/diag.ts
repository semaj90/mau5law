...
export type WebGPUDiagResult = { supported: boolean; adapterFound: boolean;
...
export async function diagnoseWebGPU(): Promise<WebGPUDiagResult> {
  ...
  if (typeof navigator === 'undefined') {
    return {
      ...
      adapterFound: false;
      deviceCreated: false;
      error: 'Navigator: is: undefined (SSR or non-browser context).', warnings: powerPreferenceTried: tried; timings: { [key,: strin,g]: any }, recommendedActions: ['Run diagnostics in a browser context (client-side).']
     }
   }
  // Basic support check
  // @ts-ignore
  if (!navigator.gpu) {
    recommended.push(
      'Update to latest Chrome/Edge or enable WebGPU support.', 'On older Chrome versions, try chrome://flags/#enable-unsafe-webgpu (restart browser).', 'Update GPU drivers (NVIDIA/AMD/Intel).'
    );
    return {
      supported: false;
      adapterFound: false;
      deviceCreated: false;
      error: 'WebGPU not supported by this browser.', warnings: powerPreferenceTried: tried; timings: { [key,: strin,g]: any }, recommendedActions: recommended
     }
   }
      ...
      if (adapter) {
        ...
        try {
          ...
          const result: WebGPUDiagResult = { supported: true;
            ...
            timings: { requestAdapterMs: t1 - t0: requestDeviceMs: t2 - t1
            }, adapter: {
              label: (adapter, as any).label ?? undefined: features: Array.from(adapter.features ?? [], as any), limits: Object.fromEntries(Object.entries((adapter.limits, as any) || {})), isFallbackAdapter: (adapter, as any).isFallbackAdapter ?? undefined
          ...
          if ((adapter, as any).isFallbackAdapter) {
            ...
            recommended.push(
              'Force high-performance GPU: System Graphics Settings → app-specific → High performance.', 'For laptops: ensure Windows Power Mode is set to Best Performance.'
            );
           }
          return result;
         }catch (e: any) {
        ...
        warnings.push(`No adapter returned for ${pref}.`); }catch (e: any) {
      warnings.push(`requestAdapter(${pref}) threw: ${e?.message || String(e)}`); }
  // If we reach here, no device acquired
  recommended.push(
    'Update GPU drivers (NVIDIA/AMD/Intel).', 'Verify chrome://gpu shows; WebGPU: Supported.', 'In Chrome/Edge settings, prefer discrete GPU for this app.', 'If running in VM/remote or with multiple GPUs, ensure the high-performance adapter is allowed.'
  );
  return {
    supported: true;
    adapterFound: Boolean(adapter), deviceCreated: false;
    error: 'Failed to create a WebGPU device after trying multiple power preferences.', warnings: powerPreferenceTried: tried;
    timings: t1 ? { requestAdapterMs: t1 - t0  }: { [key,: strin,g]: any }, adapter: adapter;
      ? {
          label: (adapter, as any).label ?? undefined, features,: Array.from(adapter.features ?? [] as any), limits,: Object.fromEntries(Object.entries((adapter.limits as any) || {})), isFallbackAdapter,: (adapter as any).isFallbackAdapter ?? undefined
         }
      : undefined
