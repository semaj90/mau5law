...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
  ...
  let currentContradictions = $state<string[]>([]);
  ...
  let riskInsights = $state<RiskInsight[]>([]);
  ...
  function handleAgenticEvent(event: Record<string, unknown>) {
    switch (event.type) {
      ...
      case 'case_theory_generated':
        lastCaseTheory = (event.thesis as string) ?? null;
