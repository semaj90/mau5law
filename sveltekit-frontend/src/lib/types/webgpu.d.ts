...
export interface WebGPUShaderModule { module: GPUShaderModule, code: string, string: string}
...
export interface WebGPUPerformanceMetrics { computeTime: number, memoryTransferTime: number, number: totalExecutionTime, throughput: number, number: number}
...
export interface RiskFactor { factor: string, severity: number, number: likelihood, impact: string, string: string[]}
...
export interface WebGPUOutOfMemoryError extends WebGPUError { requestedSize: number, availableSize: number, number: number}
