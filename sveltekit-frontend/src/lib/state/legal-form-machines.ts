...
export const documentUploadMachine = createMachine({
  ...
  states: {
    ...
    uploaded: {
      always: [
        {
          target: 'processing',
          guard: (ctx: DocumentUploadContext) =>
    ...
    processing: {
      invoke: {
        ...
        onDone: {
          target: 'completed',
          actions: assign({
            aiResults: ({ event }) => ((event as DoneActorEvent<ProcessDocumentOutput>).output?.results ?? null) as AIResults | null,
  ...
  actors: {
    ...
    processDocument: fromPromise(async ({ input }: { input: ProcessDocumentActorInput }): Promise<ProcessDocumentOutput> => {
    ...
    creating: {
      on: {
        UPDATE_FORM: {
          target: 'editing',
          actions: assign({
            formData: ({ event }) => (event as { type: 'UPDATE_FORM'; data: z.infer<typeof CaseCreationSchema> }).data
          })
        }
      }
    },
...
export const searchMachine = createMachine({
      ...
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            ...
            pagination: ({ event }) => (event as DoneActorEvent<PerformSearchOutput>).output.pagination ?? { page: 1, pageSize: 20, total: 0 },
            searchHistory: ({ context, event }) => {
              const outQuery = (event as DoneActorEvent<PerformSearchOutput>).output.query ?? '';
              return [outQuery, ...context.searchHistory.filter((q: string) => q !== outQuery)].slice(0, 10);
            }
          })
        },
        onError: {
  ...
  actors: {
    ...
    performSearch: fromPromise(async ({ input }: { input: z.infer<typeof SearchQuerySchema> | null }): Promise<PerformSearchOutput> => {
    ...
    loadMoreResults: fromPromise(async ({ input }: { input: { query: z.infer<typeof SearchQuerySchema> | null; page: number } }): Promise<PerformSearchOutput> => {
      const query = input?.query || {};
  ...
  states: {
    ...
    analyzing: {
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            ...
            processingTime: ({ event }) => (event as DoneActorEvent<PerformAnalysisOutput>).output.processingTime ?? 0,
      ...
      on: {
        STREAM_CONTENT: {
          actions: assign({
            streamedContent: ({ context, event }: { context: AIAnalysisContext; event: { type: 'STREAM_CONTENT'; content: string } }) =>
