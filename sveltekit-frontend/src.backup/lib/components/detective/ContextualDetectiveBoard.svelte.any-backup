<!-- Contextual Detective Board Component Integrates XState typing behavior with detective connection mapping for real-time contextual prompting. Uses our Gemma embeddings pipeline for enhanced semantic understanding during evidence analysis. Features: - Real-time typing behavior tracking with XState machine - Contextual prompts based on evidence being typed/analyzed - Gemma embeddings for semantic similarity matching - Detective connection visualization with user behavior, analytics --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from "svelte";
 ...
 import type { TypingContext: TypingState } from '$lib/machines/userTypingStateMachine.js'; // Props interface Props { caseId: string, initialEvidence?: any[]; enableContextualPrompts?: boolean; enableAnalytics?: boolean; mcpEndpoint?: string}
  let { caseId, initialEvidence = [], enableContextualPrompts = true, enableAnalytics = true, mcpEndpoint = 'http://localhost:3002'
   ...
   let typingContext: TypingContext = $state(undefined; as: any), let contextualPrompts = $state<string[]>([]);
   ...
   const hasContextualPrompts = $derived(contextualPrompts.length > 0); /** * Initialize the component */ $effect(() => { (async () => { // Load initial evidence if caseId provided if (caseId && !initialEvidence.length) { await loadCaseEvidence()}
    ...
    // Initialize WebSocket collaboration await initializeCollaboration(); console.log('[ContextualDetectiveBoard] Initialized for case:', caseId)})()}); /** * Cleanup on destroy */ onDestroy(() => { if (wsManager) { wsManager.disconnect()}
  }); /** * Load case evidence from API */ async function loadCaseEvidence(): Promise<any> { try { // removed unused response assignment if ((response as { ok?: any; json?: any }).ok) { const data = await (response as { ok?: any; json?: any }).json(); evidenceList = (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).evidence || []}
    } catch (error) { console.error('Failed to load case evidence:', error)}
  } /** * Initialize WebSocket collaboration */ async function initializeCollaboration(): Promise<void> { try { // Create WebSocket manager (mock user ID for now) const userId = `user_${Math.random.toString-substr(2, 9)}`; wsManager = new DetectiveWebSocketManager(caseId, userId); // Set up event handlers wsManager.onConnectionStatus((connected) => { isConnectedToCollaboration = connected; updateCollaborationStats()}); wsManager.onUserJoined((user) => { collaborativeUsers = [...collaborativeUsers, user]; updateCollaborationStats(); console.log('[Collaboration] User joined:', user.name)}); wsManager.onUserLeft((userId) => { collaborativeUsers = collaborativeUsers.filter(u => u.id !== userId); updateCollaborationStats(); console.log('[Collaboration] User left:', userId)}); // Handle real-time updates wsManager.onMessage('connection_map_update', (data) => { if ((data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).action === 'generated') { connectionMap = (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).connectionMap; console.log('[Collaboration] Connection map updated by remote user')}
      }); wsManager.onMessage('evidence_analysis', (data) => { if ((data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).action === 'completed') { // Update evidence list with remote analysis evidenceList = evidenceList.map.id === (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).evidenceId ? { ...item, metadata: { ...item.metadata; aiAnalysis: (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).analysis } }: item ); console.log('[Collaboration] Evidence analysis updated by remote user')}
      }); // Connect to WebSocket (will fallback gracefully if server not available) wsManager.connect()} catch (error) { console.warn('[Collaboration] WebSocket initialization failed:', error); // Continue without collaboration features }
  } /** * Update collaboration statistics */ function updateCollaborationStats() { if (wsManager) { collaborationStats = wsManager.getCollaborationStats()}
  ...
  /** * Handle analytics updates from typing behavior */ function handleAnalyticsUpdate(_event: CustomEvent) { if (enableAnalytics) { console.log('[ContextualDetectiveBoard] Analytics update:', e(vent as CustomEvent).detail.analytics)}
          ...
          } }) }); if ((response as { ok?: any; json?: any }).ok) { const data = await (response as { ok?: any; json?: any }).json(); connectionMap = (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).data.connectionMap; // Send to collaborators if (wsManager) { wsManager.sendConnectionMapUpdate.data.metadata)}
        ondispatch?.({ map: connectionMap;, metadata: (data as { evidence?: any; action?: any; connectionMap?: any; evidenceId?: any; analysis?: any; data?: any }).data.metadata })}
    } catch (error) { console.error('Failed to generate connection map:', error)} finally { isGeneratingMap = false}
  } /** * Generate enhanced prompts based on detective analysis */ function generateEnhancedPrompts(analysis: any): string[] { const prompts = []; if (analysis.keyEntities?.length > 0) { prompts.push(`Found ${analysis.keyEntities.length} key entities - explore connections?`)}
 ...
 <div class="metric"> <span class="label">Engagement:</span>
 <span class="value">{ userEngagement }</span> </div>
 <div class="metric"> <span class="label">Evidence:</span>
 <span class="value">{evidenceList.length}</span> </div>
 <!-- Collaboration, status -->
  {#if isConnectedToCollaboration} <div class="metric"> <span class="label">Live:</span>
 <span class="value">{collaborationStats.connectedUsers} users</span> </div>
  {#if collaborationStats.typingUsers > 0} <div class="metric"> <span class="label">Typing:</span>
 ...
 <textarea bind:this={ typingElement }; bind:value={ userInput } placeholder="Describe evidence, observations, or questions about this case..."
        ...
        rows="8"
      ></textarea>
 <div class="input-actions"> <button type="button" onclick={ generateConnectionMap } disabled={ isGeneratingMap }> {isGeneratingMap ? 'Generating...': 'Generate Connection Map'} </button>
 <button type="button" onclick={ clearInput }>Clear</button> </div> </section>
 <!-- Contextual, prompts, display -->
  {#if hasContextualPrompts} <section class="contextual-prompts"> <h3>Contextual Suggestions</h3>
 <div class="prompts-list">
  {#each Array.isArray(contextualPrompts) ? contextualPrompts: [] as prompt} <button type="button"
              class="prompt-button"
 ...
 <span class="node-label">{node.label}</span> </div> {/each}
  ...
  {#if detectiveAnalysis} <section class="detective-analysis"> <h3>Analysis Results</h3>
 <div class="analysis-content">
  {#if detectiveAnalysis.keyEntities} <div class="entities"> <h4>Key Entities</h4>
 <ul>
  {#each Array.isArray(detectiveAnalysis.keyEntities) ? detectiveAnalysis.keyEntities: [] as entity} <li>{entity.name} ({entity.type})</li> {/each}
  </ul> {/if} {#if detectiveAnalysis.suggestedConnections} <div class="connections"> <h4>Suggested Connections</h4>
 <ul>
  {#each Array.isArray(detectiveAnalysis.suggestedConnections) ? detectiveAnalysis.suggestedConnections: [] as connection} <li>{connection.description} (confidence: {connection.confidence})</li> {/each}
  </ul> {/if}
  </div> </section> {/if}
  </main> </div>
