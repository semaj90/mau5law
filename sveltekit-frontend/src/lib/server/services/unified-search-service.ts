    ...
    try {
      ...
      const confidenceLevel =
        typeof incomingMeta.confidenceLevel === 'number'
          ...
          : typeof incomingMeta.confidenceLevel === 'string' &&
              incomingMeta.confidenceLevel.trim() !== '' &&
              !Number.isNaN(Number(incomingMeta.confidenceLevel))
  ...
  private async getNeo4jRecommendations(docs: UnifiedDocument[]): Promise<Recommendation[]> {
    try {
      ...
      const mostFrequentCategory =
        Object.keys(categoryCounts).sort((a, b) => (categoryCounts[b] || 0) - (categoryCounts[a] || 0))[0] || 'unknown';
