...
/*
 * AI Enhancement using local LLM (Ollama) — payload and parsing fixed
 */
...
`
    ...
    return {
      enhanced: parsed.enhancedResults || [],
      aiAnalysis: parsed.overallAnalysis || {}
    }
  } catch (error: any) {
    console.warn('AI enhancement failed:', error)
    // Basic fallback enhancement
    const enhanced = results.map((r, i) => ({
      id: r.id,
...
async function performMCPAnalysis(query: string): Promise<any | null> {
  try {
    const mcpResults = await copilotOrchestrator(`Analyze legal search context: "${query}"`, {
 ...
 */
function generateAutoSuggestions(query: string, mcpContext: any): any[] {
 ...
 */
async function updateMemoryGraph(query: string, results: any[], metadata: any): Promise<void> {
  try {
    const memoryEntry = {
  ...
  try {
    ...
    if (!rateLimitResult.allowed) {
      ...
      return json({
        success: false,
        results: [],
        metadata: { query, totalResults: 0, processingTime, aiAnalysis: false, mcpAnalysis: false, fromCache: false, error: 'Rate limit exceeded' }
      } as AIFindResponse, {
        status: 429,
        headers: {
    ...
    // Step 5: Format results and merge AI enhancements
    const enhancedMap = new Map<string, any>((aiAnalysis?.enhancedResults || []).map((e: any) => [e.id, e]))
    let results = dbResults.map((item: any) => {
      ...
      return {
        ...
        metadata: { priority: item.priority, status: item.status, evidenceType: item.evidenceType, documentType: item.documentType, isAdmissible: item.isAdmissible, wordCount: item.wordCount, caseNumber: item.caseNumber, location: item.location, dangerScore: item.dangerScore, jurisdiction: item.jurisdiction, collectedBy: item.collectedBy, tags: item.tags, aiSummary: item.aiSummary },
    ...
    if (useAI) {
      results = results
        .filter(r => (r.aiConfidence ?? 0) >= confidenceThreshold)
    ...
    const response: AIFindResponse = {
      success: true,
      results,
      metadata: { query, totalResults: results.length, processingTime, aiAnalysis: !!aiAnalysis, mcpAnalysis: !!mcpContext, fromCache, model: 'gemma3-legal:latest', confidence: avgConfidence },
    ...
    return json(response, {
      headers: { 'X-Cache': 'MISS', 'X-RateLimit-Remaining': rateLimitResult.remaining.toString(), 'X-Processing-Time': processingTime.toString() }
    })
  } catch (error: any) {
...
const originalGETHandler: RequestHandler = async ({ url, request }) => {
  ...
  try {
    ...
    if (suggestions.length === 0) {
      const legalTerms = [ 'contract liability', 'evidence admissibility', 'case precedent', 'statute of limitations', 'witness testimony', 'expert opinion', 'criminal procedure', 'civil litigation', 'discovery process', 'motion to dismiss', 'summary judgment', 'plea bargain', 'constitutional rights', 'due process', 'search warrant', 'habeas corpus', 'chain of custody', 'reasonable doubt' ]
...
/*
 * Health check endpoint (OPTIONS) — cleaned
 */
