...
export async function GET({ url ): Promise<any> {
  try {
    ...
    const enhancedContext: any = {
      ...
      aiModel: {
  name: "gemma3-legal", endpoint: env.OLLAMA_URL || "http://ollama:11434", type: "local_llm", capabilities: [
          "legal_document_analysis", "case_strategy_recommendations", "evidence_evaluation", "contract_review", "chain_of_custody_analysis"
        ], configuration: {
  temperature: 0.1, maxTokens: 2048, contextWindow: 8192, streamingEnabled: true
         }
      }, // System architecture context
      architecture: {
  frontend: "SvelteKit, 2 with Svelte, 5 runes", backend: "Drizzle ORM + PostgreSQL", aiInfrastructure: "Ollama GPU + Docker", vectorSearch: "Qdrant + pgvector", caching: "Redis", messaging: "RabbitMQ"
      }, // Development guidelines
      guidelines: { svelte: { runesRequired: true;
          stateDeclaration: "let (never const)", derivedFunctions: "$derived.by(() => ...)", errorBoundaries: "component rendering only"
        }, aiIntegration: {
  priorityIndex: "enhanced_legal_ai_index", contextBoost: 0.2, mcpActive: true;
    ...
    if (includeDocumentation) {
      enhancedContext.documentation = {
        copilotContext: "Enhanced Legal AI Index with Context7 MCP", claudeContext: "Comprehensive legal AI system documentation", errorResolution: "SvelteKit troubleshooting and best practices"
       }
     }
    // Add case-specific context if requested
    if (includeCaseData && contextType === "legal") {
      enhancedContext.legalDomain = {
        focus: "prosecutor_case_management", features: [
          "evidence_timeline_construction", "witness_interview_analysis", "legal_precedent_research", "case_strategy_development", "compliance_verification"
  ...
  success: true;
    ...
    return json({
        success: false;
        error: "Failed to retrieve AI context", message: error instanceof Error ? error.message: "Unknown error"
      })
      { status: 500 })
   }
} }
