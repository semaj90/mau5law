...
class WorkflowOrchestrator {
  ...
  async startLegalCaseManagement(
    ...
    const workflow: WorkflowInstance = {
      ...
      metadata: { title, caseType, jurisdiction, createdBy },
    };
  ...
  async sendToWorkflow(
    ...
    try {
      workflow.actor.send(event as any);
  ...
  getWorkflowStatus(workflowId: string): WorkflowInstance | null {
    ...
    if (workflow.actor) {
      ...
      workflow.context = snapshot.context as any;
      workflow.progress = (snapshot.context as any)?.progress ?? 0;
    }
    return workflow;
  }
  ...
  private onWorkflowStateChange(workflowId: string, snapshot: any): void {
    ...
    this.emitEvent({
      ...
      payload: {
        ...
        stage:
          snapshot.context?.processingStage || snapshot.context?.workflowStage || snapshot.value,
      },
      timestamp: Date.now(),
    });
  ...
  private async persistWorkflow(workflowId: string): Promise<void> {
    ...
    try {
      ...
      if (redis)
        await redis.set(`workflow:${workflowId}`, JSON.stringify(serializable), 'EX', 86400);
    } catch (error) {
  ...
  async loadWorkflow(workflowId: string): Promise<WorkflowInstance | null> {
    try {
      const redis = getRedis();
      const raw = redis ? await redis.get(`workflow:${workflowId}`) : null;
      const cached = raw ? (JSON.parse(raw) as WorkflowInstance) : null;
  ...
  async cleanup(olderThanMs: number = 24 * 60 * 60 * 1000): Promise<number> {
    ...
    for (const [workflowId, workflow] of this.workflows) {
      if (workflow.status === 'completed' && workflow.updatedAt < cutoff) {
        if (workflow.actor) {
          workflow.actor.stop();
        }
        this.workflows.delete(workflowId);
        const redis = getRedis();
        if (redis) await redis.del(`workflow:${workflowId}`);
        cleaned++;
      }
    }
