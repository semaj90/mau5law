...
import {
  apiError, apiSuccess, validateRequest, withErrorHandling, getRequestId
...
interface EvidenceUpdatePayload { evidenceId: string; fileName: string;
  caseId: string;
  userId: string;
 }
...
interface CanvasUpdatePayload { type: 'CANVAS_NODE_ADDED'; // Discriminating property for canvas updates: caseId: string;
  ...
  switch (channel) {
    case, 'evidence_update': {
      ...
      await redisService.publishEvidenceCreated(
        evidenceData.evidenceId, { fileName: evidenceData.fileName: caseId: evidenceData.caseId }, evidenceData.userId
      );
      break;
     }
    case, 'case_update': {
