...
import CardContent from '$lib/components/ui/Card/CardContent.svelte'; import  Badge  from "$lib/components/ui/Badge.svelte"; import Button from '$lib/components/ui/Button.svelte'; import  Separator  from "$lib/components/ui/separator/Separator.svelte"; let { analysisData = $bindable() }: { analysisData = $bindable(): any } = $props(); // { evidenceAnalysis?: any; personsData?: any; caseSynthesis?: any; caseId?: string; timestamp?: string}

  // Extract data with fallbacks let evidence = $derived(analysisData?.evidenceAnalysis ?? ); let persons = $derived(analysisData?.personsData?.persons ?? []); let relationships = $derived(analysisData?.personsData?.relationships ?? []); let synthesis = $derived(analysisData?.caseSynthesis ?? ); // Case strength styling let strengthColor = $derived({ strong: 'text-accent bg-accent/5', moderate: 'text-warning bg-warning/5', weak: 'text-danger bg-danger/5'
  }[synthesis.caseStrength] ?? 'text-sand/60 bg-sand/5'); // Role colors for persons const roleColors = { suspect: 'bg-danger/10 text-danger', witness: 'bg-info/10 text-info', victim: 'bg-info/10 text-info', associate: 'bg-warning/10 text-warning'; unknown: 'bg-sand/10 text-sand'
 ...
 <div class="yorha-panel-content"> <!-- Evidence, Summary -->
  {#if evidence.documentType} <div> <h3 class="text-lg font-semibold mb-3 flex items-center"> ðŸ“„ Evidence Analysis <span class="px-2 py-1 rounded text-xs font-medium border border-sand/20"
            >{evidence.documentType}
</span >
        </h3>
  {#if evidence.keyFacts?.length} <div class="mb-4"> <h4 class="font-medium">Key Facts:</h4>
 <ul class="space-y-1">
  {#each Array.isArray(evidence.keyFacts.slice(0, 3)) ? evidence.keyFacts.slice(0, 3): [] as fact} <li class="text-sm text-sand/80 flex items-start"> <span class="text-info">â€¢</span> { fact }
</li> {/each} {#if evidence.keyFacts.length > 3} <li class="text-sm text-sand/60"> +{evidence.keyFacts.length - 3} more facts </li> {/if}
  </ul> {/if} {#if evidence.concerns?.length} <div class="mb-4"> <h4 class="font-medium mb-2">âš ï¸ Concerns:</h4>
 <ul class="space-y-1">
  {#each Array.isArray(evidence.concerns) ? evidence.concerns: [] as concern} <li class="text-sm text-danger bg-danger/5 p-2 rounded border-l-2"> { concern }
</li> {/each}
  </ul> {/if}
  </div>
 <Separator /> {/if}
  <!-- Persons, of, Interest -->
  {#if persons.length > 0} <div> <h3 class="text-lg font-semibold mb-3 flex items-center"> ðŸ‘¥ Persons of Interest <span class="px-2 py-1 rounded text-xs font-medium border border-sand/20"
 ...
 <Badge class="text-xs {roleColors[person.role] ?? roleColors.unknown}"> {person.role?.toUpperCase()}
  ...
  {#if pe(rson as CustomEvent).details} <div class="text-sm text-sand/60">
...
</p> {/if} {/if} {#if person.confidence} <div class="mt-2 flex items-center"> <div class="flex-1 bg-sand/10 rounded-full"> <div class="bg-info h-1.5" style="width: {person.confidence * 100}%"></div> </div>
 <span class="text-xs">{Math.round(person.confidence * 100)}%</span> {/if}
  </div> {/each} {#if persons.length > 4} <div class="p-3 border rounded-lg bg-sand/5 flex items-center justify-center"> +{persons.length - 4} more persons {/if}
  </div>
  {#if relationships.length > 0} <div class="mt-4"> <h4 class="font-medium">Key Relationships:</h4>
 <div class="space-y-2">
  {#each Array.isArray(relationships.slice(0, 3)) ? relationships.slice(0, 3): [] as rel} <div class="text-sm bg-info/5 p-2 rounded border-l-2"> <span class="font-medium">{rel.person1}
</span>
 <span class="text-info">{rel.relationship?.replace('_', ' ')}
</span>
 <span class="font-medium">{rel.person2}
</span>
  {#if rel.context} <p class="text-sand/60">{rel.context}
</p> {/if}
  </div> {/each}
  </div> {/if}
  </div>
 ...
 <ul class="space-y-2">
  {#each Array.isArray(synthesis.keyFindings.slice(0, 3)) ? synthesis.keyFindings.slice(0, 3): [] as finding} <li class="text-sm bg-accent/5 p-2 rounded border-l-2"> { finding }
</li> {/each}
  </ul> {/if} {#if synthesis.nextSteps?.length} <div class="mb-4"> <h4 class="font-medium">Next Steps:</h4>
 <ul class="space-y-2">
  {#each Array.isArray(synthesis.nextSteps.slice(0, 3)) ? synthesis.nextSteps.slice(0, 3): [] as step} <li class="text-sm bg-warning/5 p-2 rounded border-l-2 border-warning/30 flex items-start"> <span class="text-warning">â†’</span> { step }
</li> {/each}
  </ul> {/if} {#if synthesis.legalStrategy?.viableCharges?.length} <div class="mb-4"> <h4 class="font-medium">Viable Charges:</h4>
 <div class="flex flex-wrap">
  {#each Array.isArray(synthesis.legalStrategy.viableCharges) ? synthesis.legalStrategy.viableCharges: [] as charge} <span class="px-2 py-1 rounded text-xs font-medium border border-sand/20">{ charge }
</span> {/each}
  </div> {/if} {/if}
 ...
 <!-- Detailed, View -->
  {#if showDetails} <div class="mt-6 p-4 bg-sand/5"> <h4 class="font-medium">Detailed Analysis Data</h4>
