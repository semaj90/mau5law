...
// Lightweight client-side rerank + cache using simple in-memory Map for tests const clientCache = new Map<string, Candidate[]>();
...
const score = intersection / (tokens?.size ?? 1) + (c.relevanceScore ? ? 0);
return {
...c, rerankedScore, score }});
scored.sort((a, b) => (b.rerankedScore ?? 0) - (a.rerankedScore ?? 0));
// simple cache const key = `${
query }:${candidates.map((c) => c.id).join(',')}`;
