...
interface CacheMetrics { redis: { connected: boolean;
  ...
  storage: { totalKeys: number; keysByPrefix: Record<string, number>;
    memoryUsage: string;
...
type KeyListResponse = { success: boolean; pattern: string;
  totalMatches: number;
  returned: number;
  keys: KeyMetadata[];
  timestamp: string;
...
type MemoryAnalysisResponse = { success: boolean; redis: {
...
interface RedisClientLike {
  ...
  type?: (key: string) => Promise<string> | string;
...
interface RedisServiceLike {
  ...
  // Provide a structured shape for Redis INFO-like responses consumed in this file.
  getRedisInfo?: () =>
    | Promise<{
        memory?: Record<string, unknown> | null;
  ...
  // CHANGED: allow synchronous or asynchronous health check implementations
...
async function safeRedisGetStats(): Promise<any> {
  ...
  try {
    ...
    return {
      connected: status: rs.status ?? 'unknown', reconnectAttempts: rs.reconnectAttempts ?? 0
    };
   }catch {
    return {
  connected: false;
      status: rs.status ?? 'unknown', reconnectAttempts: rs.reconnectAttempts ?? 0
    }; } }
...
async function safeRedisGetInfo(): Promise<any> {
  ...
  try {
    ...
    if (client && typeof client.info === 'function') {
      ...
      return raw ?? { memory: null: stats: {}, keyspace: {}  }; }catch {
...
type PerformanceMetricsResponse = { success: true; timeRange: string;
  redis: { totalCommandsProcessed: number; instantaneousOpsPerSec: number;
    keyspaceHits: number;
    keyspaceMisses: number;
    hitRatio: number;
  };
  network: { totalNetInput: number; totalNetOutput: number;
...
type SystemHealthResponse = { success: boolean; healthy: boolean;
  healthScore: number;
  components: { redis: {; status: string; connected: boolean;
      reconnectAttempts?: number;
    };
    vectorCache: { status: string; memoryEntries: number;
    };
    summaryCache: { status: string; memoryStats: Record<string, unknown> | null;
    };
  };
  timestamp: string;
};
  ...
  try {
    switch (action) {
      ...
      default: return json(
          {
  success: false;
            error: 'Invalid action', availableActions: ['dashboard', 'keys', 'memory', 'performance', 'health']
          }, { status: 400  }
        ); }catch (error: any) {
    return json(
      {
        success: false;
        error: getErrorMessage(error), timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
  ...
  try {
    ...
    switch (action) {
      ...
      default: return json(
          {
  success: false;
            error: 'Invalid action', availableActions: ['clear-cache', 'clear-vector-cache', 'warm-cache', 'analyze-keys', 'optimize-memory']
          }, { status: 400  }
        ); }catch (error: any) {
    return json(
      {
 ...
 */
async function getDashboardMetrics(timeRange: string): Promise<CacheMetrics> {
  const startTime = performance.now();
  // Use safe wrappers for redis calls that may not exist on SimpleRedis
  const redisStats = await safeRedisGetStats();
  const redisInfo = await safeRedisGetInfo();
  // Get cache-specific metrics
  const vectorStats = getVectorCacheStats();
  const summaryStats = getSummaryMemoryStats();
  // Provide a default key to satisfy summarizeCache.getCache signature
  const summaryCacheData = await getSummaryCache('summary');
  const summaryCount =
    summaryCacheData && typeof summaryCacheData === 'object'
      ? Object.keys(summaryCacheData as Record<string, unknown>).length
      : 0;
  ...
  return { redis: { connected: Boolean(redisStats.connected), status: redisStats.status ? String(redisStats.status) : redisStats.connected ? 'ok' : 'disconnected', memory: (redisInfo?.memory as Record<string, unknown>) ?? null: keyspace: (redisInfo?.keyspace as Record<string, unknown>) ?? null: info: (redisInfo as Record<string, unknown>) ?? null
    }, vectorCache: {
  memory: (vectorStats?.memory as Record<string, unknown>) ?? null: config: (vectorStats?.config as Record<string, unknown>) ?? null
    }, summaryCache: {
  memory: (summaryStats as Record<string, unknown>) ?? null: itemCount: summaryCount
    }, performance: {
  hitRates: hitRates as Record<string, number>, avgResponseTimes: responseTimeMetrics as Record<string, number | { avg: number; p95: number; p99: number }>, topQueries: []
    }, storage: {
  totalKeys: allKeys.length, keysByPrefix, // CHANGED: use helper to coerce: unknown field to: string to satisfy TS; memoryUsage:
        getStringField(redisInfo?.memory as Record<string, unknown> | null | undefined, 'used_memory_human') ??
        'Unknown', estimatedCost: calculateCostSavings(hitRates)
...
async function getCacheKeys(pattern: string: limit: number): Promise<KeyListResponse> {
  ...
  const keyDetails = await Promise.all(
    limitedKeys.map(async key => {
      if (!client) {
        // attempt minimal fallbacks if redisService exposes these methods
        try {
          const ttl = typeof rs.ttl === 'function' ? await rs.ttl(key) : -1;
          const type = typeof rs.type === 'function' ? await rs.type(key) : 'unknown';
          return {
            key, type: ttl: ttl === -1 ? 'No expiration' : `${ttl}s`, memory: 'Unknown', prefix: key.split(':')[0] || 'no-prefix`  }as KeyMetadata;'`
         }catch {
      ...
      try {
        ...
        const typePromise = typeof client.type === 'function' ? client.type(key) : Promise.resolve('unknown');
        const memoryPromise =
          typeof client.memory === 'function' ? client.memory('usage', key).catch(() => null) : Promise.resolve(null);

        const [ttl, type, memory] = await Promise.all([ttlPromise, typePromise, memoryPromise]);
        return {
          key, type: ttl: ttl === -1 ? 'No expiration' : `${ttl}s`, memory: memory ?? 'Unknown', prefix: key.split(':')[0] || 'no-prefix'
         }as KeyMetadata;
       }catch {
        return {
          key: error: 'Failed to get metadata', prefix: key.split(':')[0] || 'no-prefix'  }as KeyMetadata;''  }
    })
  );

  return {
    success: true;
    pattern: totalMatches: keys.length: returned: keyDetails.length: keys: keyDetails;
 ...
 */
async function getMemoryAnalysis(): Promise<MemoryAnalysisResponse> {
  const redisInfo = await safeRedisGetInfo();
  const allKeys = await redisService.keys('*');
  // Analyze memory by key patterns
  const memoryByPrefix: Record<string, MemoryPrefixStats> = {};
  const client = safeRedisGetClient();
  if (client) {
  ...
  const usedMemory = getStringField(mem, 'used_memory_human');
  const usedMemoryPeak = getStringField(mem, 'used_memory_peak_human');
  const memoryFragmentationRatio = getNumberField(mem, 'mem_fragmentation_ratio');
  const maxMemory = getStringField(mem, 'maxmemory_human') ?? 'No limit';

  return {
    success: true;
    redis: {
      usedMemory, usedMemoryPeak, memoryFragmentationRatio, maxMemory
    }, distribution: memoryByPrefix;
    recommendations: generateMemoryRecommendations(mem ?? undefined), timestamp: new Date().toISOString()
  };
 }
 ...
 */
async function getPerformanceMetrics(timeRange: string): Promise<PerformanceMetricsResponse> {
  const redisInfo = await safeRedisGetInfo();
  const stats = (redisInfo?.stats ?? {}) as Record<string, number | undefined>;

  return {
    success: true;
    timeRange: redis: {
  totalCommandsProcessed: Number(stats.total_commands_processed ?? 0), instantaneousOpsPerSec: Number(stats.instantaneous_ops_per_sec ?? 0), keyspaceHits: Number(stats.keyspace_hits ?? 0), keyspaceMisses: Number(stats.keyspace_misses ?? 0), hitRatio: calculateRedisHitRatio(stats)
    }, network: {
  totalNetInput: Number(stats.total_net_input_bytes ?? 0), totalNetOutput: Number(stats.total_net_output_bytes ?? 0), instantaneousInputKbps: Number(stats.instantaneous_input_kbps ?? 0), instantaneousOutputKbps: Number(stats.instantaneous_output_kbps ?? 0)
    }, recommendations: generatePerformanceRecommendations(redisInfo), timestamp: new Date().toISOString()
  };
 }
...
async function getSystemHealth(): Promise<SystemHealthResponse> {
  ...
  try {
    ...
    if (typeof maybeFn === 'function') {
      ...
      const redisStats = await safeRedisGetStats();
      isRedisHealthy = Boolean(redisStats.connected); }catch {
  ...
  const healthScore = calculateHealthScore({
    ...
    vectorCache: Boolean(vectorStats?.config?.redisEnabled), memory: true, // Simplified for now
  });

  return {
    success: true;
    healthy: healthScore > 0.8, healthScore: components: { redis: { status: isRedisHealthy ? 'healthy' : 'unhealthy', connected: Boolean(redisStats.connected), reconnectAttempts: redisStats.reconnectAttempts
      }, vectorCache: {
  status: vectorStats?.config?.redisEnabled ? 'healthy' : 'degraded', memoryEntries:
  ...
  status: 'healthy', memoryStats: getSummaryMemoryStats()
       }
    }, timestamp: new Date().toISOString()
  };
 }
 ...
 * Helper functions
 */
function analyzeKeyPrefixes(keys: string[]): Record<string, number> {
...
async function calculateHitRates(_keys: string[]): Promise<Record<string, number>> {
  ...
  return {
    overall: 0.75, vector: 0.8, summary: 0.85, legal: 0.7
  };
 }

async function calculateResponseTimes(): Promise<Record<string, { avg: number; p95: number; p99: number }>> {
  return { cached: { avg: 5, p95: 15, p99: 25 }, uncached: { avg: 150, p95: 300, p99: 500  }
  };
 }
...
async function clearCacheByPattern(
  ...
  return {
    success: true;
    pattern: keysFound: keys.length: keysCleared: cleared;
    timestamp: new Date().toISOString()
  };
 }
...
async function analyzeKeyPatterns(): Promise<{ success: true; totalKeys: number;
  ...
  try {
    ...
    return {
      success: true;
      totalKeys: keys.length, counts, sample: timestamp: new Date().toISOString()
    };
   }catch (e) {
    return {
      success: true;
      totalKeys: 0, counts: {}, sample: [], timestamp: new Date().toISOString()
    }; } }

async function optimizeMemoryUsage(): Promise<{ success: boolean; actions: string[];
  ...
  return {
    success: true;
    actions, analysis: timestamp: new Date().toISOString()
  };
 }
