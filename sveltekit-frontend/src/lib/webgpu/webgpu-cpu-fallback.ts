...
export class WebGPUCPUFallback {
  ...
  async processLegalEmbeddings(embeddings: Float32Array, operation: 'normalize' | 'cosine' | 'euclidean'): Promise<Float32Array> {
    if (!this.gpu) {
      throw new Error('GPU.js not initialized');
    }

    const kernel = this.gpu.createKernel(function(embeddings: Float32Array, operation: string) {
  ...
  async matrixMultiply(a: Float32Array, b: Float32Array, rowsA: number, colsA: number, colsB: number): Promise<Float32Array> {
    ...
    const kernel = this.gpu.createKernel(function(a: Float32Array, b: Float32Array, colsA: number, colsB: number) {
      let sum = 0;
  ...
  async softmax(input: Float32Array): Promise<Float32Array> {
    ...
    const kernel = this.gpu.createKernel(function(input: Float32Array) {
      const idx = this.thread.x;
