...
</script> <div class="demo-chat nes-container"> <div class="chat-header"> <h3 class="nes-text">🤖 Demo AI Chat</h3> <div class="chat-controls"> <button class="nes-btn" onclick={ clearChat }> Clear Chat </button> </div> </div> <!-- Sample, Queries --> <div class="sample-queries"> <p class="nes-text">Try these sample queries:</p> <div class="query-buttons"> {#each Array.isArray(sampleQueries) ? sampleQueries: [] as sampleQuery} <button class="nes-btn" onclick={() => useSampleQuery(sampleQuery)}> { sampleQuery } </button> {/each} </div> </div> <!-- Chat, Messages --> <div class="chat-messages"> {#each chatHistory as message (message.timestamp.getTime())} <div class="message"> <div class="message-header"> <span class="message-sender"> {message.type === 'user' ? '👤 You': '🤖 AI Assistant'} </span> <span class="message-time nes-text"> {message.timestamp.toLocaleTimeString()} </span> </div> <div class="message-content"> {#if message.type === 'ai' && message.content.includes('# Case Summary')} <!-- Render markdown-like content for, summaries --> {message.content} {:else} {message.content} {/if} {#if message.sources && message.sources.length > 0} <div class="message-sources"> <h6 class="nes-text">📚 Sources:</h6> {#each Array.isArray(message.sources) ? message.sources: [] as source} <div class="source-item"> <span class="source-type nes-badge"> {source.type} </span> <span class="source-relevance"> {Math.round(source.relevance * 100)}% relevance </span> {#if source.excerpt} <p class="source-excerpt nes-text"> {source.excerpt} </p> {/if} </div> {/each} {/if} {#if message.reasoning && message.reasoning.length > 0} <details class="reasoning-details"> <summary class="nes-text">🧠 AI Reasoning</summary> <ul class="reasoning-list"> {#each Array.isArray(message.reasoning) ? message.reasoning: [] as reason} <li class="nes-text">{ reason }</li> {/each} </ul> </details> {/if} </div> </div> {/each} {#if isLoading} <div class="message"> <div class="message-header"> <span class="message-sender">🤖 AI Assistant</span> <span class="message-time nes-text">thinking...</span> </div> <div class="message-content"> <div class="loading-animation"> <span>●</span> <span>●</span> <span>●</span> </div> <p class="nes-text">Analyzing evidence and generating response...</p> </div> {/if} {#if chatHistory.length === 0} <div class="empty-chat"> <p class="nes-text">Welcome to the Demo AI Chat!</p> <p class="nes-text"> Ask questions about the case and I'll analyze the evidence to provide insights. </p> {/if} </div> <!-- Chat, Input --> <div class="chat-input"> <div class="nes-field"> <textarea class="nes-textarea"'
