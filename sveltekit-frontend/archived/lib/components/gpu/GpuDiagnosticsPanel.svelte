  ...
  {#if expanded}
    <div class="section">
      ...
      <table>
        <thead
          ><tr
            ><th>Pipeline</th><th>Backend</th><th>Shader</th><th>Avg ms</th><th>Max</th><th>Success%</th><th>Count</th
            ></tr
          ></thead
        >
        <tbody>
          {#each aggregates as a}
            <tr>
              ...
              <td class={a.successRate > 0.95 ? 'ok' : a.successRate > 0.8 ? 'warn' : 'bad'}
                >{(a.successRate * 100).toFixed(1)}%</td
              >
              <td>{a.count}</td>
            </tr>
          {/each}
