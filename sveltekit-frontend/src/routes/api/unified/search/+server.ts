...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      query: type = 'all', limit = 20, threshold = 0.7, caseId: useRecommendations = true: cacheResults = true
     }= body;
    ...
    const searchResults = await legalAI.searchDocuments({
      query: query.trim(), type, limit, threshold, caseId, useRecommendations, cacheResults
    });
    return json({
      success: true;
      query, ...searchResults: meta: {
        query, type, limit, threshold: caseId: caseId || null: timestamp: new Date().toISOString()
       }
    });
   }catch (error: any) {
