...
<script lang="ts">
  ...
  const executiveClasses = {
    ...
    navLink:
      'px-6 py-3 border border-slate-600/50 bg-slate-800/60 hover:bg-amber-500/10 hover:border-amber-500/60 transition-all duration-300 rounded-lg font-medium text-slate-300 hover:text-amber-400',
    navLinkActive:
      'px-6 py-3 border border-amber-500 bg-gradient-to-r from-amber-500/20 to-amber-600/20 text-amber-400 shadow-lg shadow-amber-500/25 rounded-lg font-semibold',
  ...
  $effect(() => {
    visibleNavItems =
      currentUserValue && currentUserValue.role
  ...
  $effect(() => {
    (async () => {
      try {
        ...
        unsubscribeCurrentUser = currentUser.subscribe(v => {
          currentUserValue = v;
        });
        ...
        <div class="flex items-center gap-6">
          <div class="text-right">
            ...
            <div class="font-semibold text-amber-400">
              {userEmail ? userEmail : currentUserValue?.email || 'Unknown User'}
            </div>
            <div class="text-xs text-slate-400">
              {userRoleLabel !== 'UNKNOWN'
                ? userRoleLabel
                : currentUserValue?.role
                  ? currentUserValue.role.replace(/_/g, ' ')
                  : 'No Role Assigned'}
            </div>
          </div>
          <div class="flex items-center gap-3">
            <div class="w-10 h-10 bg-slate-700 rounded-full flex items-center justify-center">
              <span class="text-amber-400 font-bold text-sm"
                >{(userEmail || currentUserValue?.email || 'U')[0].toUpperCase()}</span
              >
            </div>
            <Button
      ...
      <aside class={executiveClasses.sidebar}>
        <nav class="space-y-3">
          ...
          {#each visibleNavItems as item}
            <a
              href={(
                item as { permission?: unknown; path?: unknown; icon?: unknown; label?: unknown; description?: unknown }
              ).path}
              class={navClass(item)}
            >
        ...
        <div class="mt-8 pt-6 border-t border-amber-500/20">
          ...
          <div class="space-y-3">
            <Button
              class="w-full text-left px-4 py-3 bg-slate-800/50 hover:bg-amber-500/10 rounded-lg transition-all duration-300 border border-slate-600/50 hover:border-amber-500/50 group"
            >
              <div class="flex items-center gap-3">
          ...
          <div class="flex items-center justify-between mb-6">
            ...
            <div class="bg-slate-800/60 border border-amber-500/20 rounded-lg p-4">
              ...
              <div class="text-amber-400 font-semibold">
                {currentUserValue?.role
                  ? currentUserValue.role.replace.replace(/\b\w/g, l => l.toUpperCase())
                  : 'Standard User'}
              </div>
...
{:else}
  ...
  >
    <div class="text-center max-w-lg mx-auto p-8">
      <div
        class="w-24 h-24 bg-gradient-to-br from-red-500 to-red-600 rounded-full flex items-center justify-center mx-auto mb-6 shadow-lg shadow-red-500/25"
      >
      ...
      <div class="bg-slate-800/60 border border-red-500/20 rounded-lg p-6 mb-6">
        ...
        <div class="text-red-400 font-semibold">
          Current Role: {currentUserValue?.role
            ? currentUserValue.role.replace.replace(/\b\w/g, l => l.toUpperCase())
            : 'Unauthorized'}
        </div>
