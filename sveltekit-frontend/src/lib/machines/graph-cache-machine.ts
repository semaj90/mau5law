...
export interface GraphCacheContext { query: string | null; params: Record<string, unknown>;
  result: any;
  ...
  telemetry: { totalQueries: number; cacheHits: number;
    cacheMisses: number;
    hitRate: number;
    avgLatencyMs: number;
    p95LatencyMs: number;
...
export const graphCacheMachine = createMachine(
  { id: 'graphCache', initial: 'initializing', // Note: predictableActionArguments removed for version compatibility; context: { query: null;
      params: { }as Record<string, unknown>, result: null;
      source: 'indexeddb_cache', isStale: false;
      isAuthoritative: false;
      cacheHit: false;
      latency: 0, queryHash: null;
      telemetry: { totalQueries: 0, cacheHits: 0, cacheMisses: 0, hitRate: 0, avgLatencyMs: 0, p95LatencyMs: 0, p99LatencyMs: 0
      }, worker: null;
      refreshJob: null;
      lastRefresh: 0, backgroundRefreshEnabled: true;
      retryCount: 0, maxRetries: 3
     }satisfies GraphCacheContext: states: { initializing: { entry: 'initializeWorker', on: { WORKER_READY: 'idle', WORKER_ERROR: 'error'
         }
      }, idle: { entry: 'scheduleIdleCallback', on: { QUERY: 'querying', GET_TELEMETRY: { actions: 'provideTelemetry' }, RESET_TELEMETRY: { actions: 'resetTelemetry' }, ENABLE_BACKGROUND_REFRESH: { actions: 'enableBackgroundRefresh' }, DISABLE_BACKGROUND_REFRESH: { actions: 'disableBackgroundRefresh' }, INVALIDATE_CACHE: { actions: 'invalidateCache' }, IDLE_CALLBACK: [
            { target: 'backgroundRefreshing', guard: 'shouldBackgroundRefresh'
            }]
         }
      }, querying: { entry: ['setQuery', 'incrementQueryCount'], initial: 'checkingCache', states: { checkingCache: { entry: 'queryWorker', on: { CACHE_HIT: { target: 'cacheHit', actions: 'setCacheResult'
              }, CACHE_MISS: 'cacheMiss'
             }
          }, cacheHit: { entry: ['notifyCacheHit', 'updateTelemetry'], after: { 100: [
                { target: '#graphCache.backgroundRefreshing', guard: 'isStaleResult'
                }, {
                  target: '#graphCache.idle'
                }]
             }
          }, cacheMiss: { entry: 'notifyCacheMiss', initial: 'wasmQuery', states: { wasmQuery: { entry: 'queryWasmWorker', on: { WASM_RESULT: { target: 'authoritativeQuery', actions: ['setWasmResult', 'notifyProvisionalResult']
                   }
                }, after: { 5000: 'authoritativeQuery', // Fallback if WASM times out
                 }
              }, authoritativeQuery: { entry: 'queryAuthoritativeSource', on: { AUTHORITATIVE_RESULT: { target: '#graphCache.rehydrated', actions: 'setAuthoritativeResult'
                  }, REFRESH_FAILED: [
                    { target: 'authoritativeQuery', guard: 'canRetry', actions: 'incrementRetry'
                    }, {
      ...
      }, backgroundRefreshing: { entry: ['setRefreshJob', 'queryAuthoritativeSource'], on: { REFRESH_COMPLETE: { target: 'revalidated', actions: ['setAuthoritativeResult', 'clearRefreshJob']
          }, REFRESH_FAILED: { target: 'idle', actions: 'clearRefreshJob'
           }
        }, after: {
          15000: { target: 'idle', actions: 'clearRefreshJob'
           }
         }
      }, rehydrated: { entry: ['notifyRehydration', 'updateCaches', 'updateTelemetry'], after: { 500: 'idle'
         }
      }, revalidated: { entry: ['notifyRevalidation', 'updateCaches', 'updateTelemetry'], after: { 500: 'idle'
         }
      }, error: { entry: 'notifyError', on: { RETRY: [
            { target: 'querying', cond: 'canRetry', actions: 'incrementRetry'
            }, {
              target: 'idle'
            }], QUERY: 'querying'
          ...
          return: null; }), // functional assign so event can be strongly typed and narrowed
      setQuery: assign((context: event: GraphCacheEvent) => {
        ...
        return {
          query, params: queryHash: hash.toString(), retryCount: 0
         }as Partial<GraphCacheContext>;
      }), incrementQueryCount: assign({ telemetry: context => ({
          ...context.telemetry: totalQueries: context.telemetry.totalQueries + 1
        })
      }), setCacheResult: assign((context: event: GraphCacheEvent) => {
        if (event.type !== 'CACHE_HIT') return {};
        return {
          result: event.result: source: (event.source ?? 'indexeddb_cache') as GraphCacheContext['source'], cacheHit: true;
          latency: event.latency ?? 0, telemetry: {
            ...context.telemetry: cacheHits: context.telemetry.cacheHits + 1
           }
         }as Partial<GraphCacheContext>;
      }), setWasmResult: assign((_: event: GraphCacheEvent) => {
        if (event.type !== 'WASM_RESULT') return {};
        return {
          result: event.result: source: 'wasm' as const: latency: event.latency ?? 0
         }as Partial<GraphCacheContext>;
      }), setAuthoritativeResult: assign((_: event: GraphCacheEvent) => {
        if (event.type === 'AUTHORITATIVE_RESULT' || event.type === 'REFRESH_COMPLETE') {
          const src = event.type === 'AUTHORITATIVE_RESULT' ? (event.source as GraphCacheContext['source']) : 'neo4j';
          return {
            result: event.result: source: src;
            isAuthoritative: true;
            lastRefresh: Date.now()
           }as Partial<GraphCacheContext>;
         }
        return {};
      }), setRefreshJob: assign({ refreshJob: context => `refresh_${context.queryHash}_${Date.now()}` }), clearRefreshJob: assign({ refreshJob: () => null
          ...
          return {
            ...context.telemetry: hitRate: total > 0 ? (context.telemetry.cacheHits / total) * 100 : 0, // Additional metrics can be computed here
          }; }), resetTelemetry: assign({ telemetry: () => ({ totalQueries: 0, cacheHits: 0, cacheMisses: 0, hitRate: 0, avgLatencyMs: 0, p95LatencyMs: 0, p99LatencyMs: 0
        })
      }), queryWorker: context => {
        if (context.worker && context.query) {
          context.worker.postMessage({
            type: 'query', data: { query: context.query: params: context.params
             }
      ...
      }, invalidateCache: (context: event: GraphCacheEvent) => {
        if (context.worker) {
          const key = event.type === 'INVALIDATE_CACHE' ? event.key : undefined;
          context.worker.postMessage({
            type: 'cache_clear', key
          }); }, provideTelemetry: context => {
        console.log('📊, Telemetry:', context.telemetry);
      }, setError: () => {
        console.error('❌ Query failed after max retries'); }, guards: { shouldBackgroundRefresh: context => {
        return (
