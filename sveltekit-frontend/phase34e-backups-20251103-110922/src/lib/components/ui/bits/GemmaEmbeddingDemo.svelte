<script lang="ts"> // Svelte, 5 runes are auto-imported import  Button  from "./Button.svelte"; import  Input  from "./Input.svelte"; import  Card  from "./Card.svelte"; import  SearchInput  from "./SearchInput.svelte"; import { z } from "zod"; import { Brain, Database, Zap, Search, AlertCircle, CheckCircle, Cpu, Activity, Hash, Clock } from 'lucide-svelte'; // Types for Gemma API integration interface GemmaEmbeddingResult { success: boolean, embedding?: number[]; metadata?: any; error?: string; responseTime?: string; timestamp?: string}
  ...
  // Search similar embeddings async function searchEmbeddings(): Promise<any> { if (!searchQuery.trim()) return; isSearching = true; searchResults = []; try { // First generate embedding for search query const embeddingResponse = await fetch('/api/embeddings/gemma?action=generate', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify({ text: searchQuery;, metadata: { source: 'search_query' } }) }); const embeddingData = await embeddingResponse.json(); if (embeddingData.success && embeddingData.embedding) { // Search for similar embeddings const searchResponse = await fetch( `/api/embeddings/enhanced?action=search&query=${encodeURIComponent(searchQuery)}&embedding=${encodeURIComponent(JSON.stringify(embeddingData.embedding))}&limit=5&threshold=0.7` ); const searchData = await searchResponse.json(); if (searchData.success) { searchResults = searchData.data} else { error = searchData.error || 'Search failed'}
  ...
  } // Load stats on mount $effect(() => { loadStats()}); // Reactive validation let isValid = $derived(content.length > 0 && content.length <= 10000); let hasValidationErrors = $derived(Object.keys(validationErrors).length > 0); let canSearch = $derived(searchQuery.trim().length > 0); </script> <div class="gemma-demo-container"> <!-- Header --> <Card title="ðŸ§  Gemma Embeddings + Enhanced-Bits, Demo" nesStyle={ true } variant="legal"> {#snippet children()} <div class="demo-header"> <p class="nes-text"> <Brain class="inline-icon" /> Real-time legal AI embeddings with Gemma 512-dimensional vectors </p> <div class="stats-bar"> <div class="stat"> <Database class="inline-icon" /> <span class="nes-text">Embeddings: {stats.totalEmbeddings}</span> </div> <div class="stat"> <Clock class="inline-icon" /> <span class="nes-text">Avg Time: {stats.avgResponseTime}</span> </div> <div class="stat"> <Activity class="inline-icon" /> <span class="nes-text {useWorker ? 'is-warning': 'is-primary'}"> {useWorker ? 'WASM Worker': 'Gemma API'} </span> </div> </div> </div> {/snippet} </Card> <!-- Generation, Form --> <Card title="Generate, Embedding" nesStyle={ true } variant="evidence"> {#snippet children()} <form onsubmit={ generateEmbedding } class="generation-form"> <div class="form-group"> <label for="content-input" class="nes-text"> <Hash class="inline-icon" /> Legal Content: </label> <textarea id="content-input";
          ...
          ></textarea> {#if validationErrors.content} <p class="nes-text is-error"> <AlertCircle class="inline-icon" /> {validationErrors.content} </p> {/if} <div class="char-counter"> <span class="nes-text {content.length > 9000 ? 'is-warning': 'is-primary'}"> {content.length}/10,000 characters </span> </div> </div> <div class="form-actions"> <Button variant="legal"
            nesStyle={ true } disabled={!isValid || isGenerating} loading={ isGenerating } type="submit"
          > {#if isGenerating} <Cpu class="inline-icon" /> Generating with Gemma... {:else} <Brain class="inline-icon" /> Generate 512D Embedding {/if} </Button> <label class="nes-text"> <input type="checkbox" class="nes-checkbox" bind:checked={ useWorker } disabled={ isGenerating } /> <span>Use WASM Worker</span> </label> </div> </form> {#if result} <div class="result-display"> <div class="nes-container is-rounded"> <h4 class="nes-text"> <CheckCircle class="inline-icon" /> Gemma Embedding Generated! </h4> <div class="result-details"> <p class="nes-text">Dimensions: <code>512</code></p> <p class="nes-text">Response Time: <code>{result.responseTime}</code></p> <p class="nes-text">Source: <code>{result.metadata?.source || 'gemma'}</code></p> <p class="nes-text"> Vector Preview: <code >[{result.embedding ?.slice(0, 3) .map(n => n.toFixed(4)) .join(', ')}...]</code >
              </p> </div> </div> {/if} {#if error} <div class="error-display"> <div class="nes-container"> <p class="nes-text"> <AlertCircle class="inline-icon" /> { error } </p> </div> {/if} {/snippet} </Card> <!-- Search, Section --> {#if showSearch} <Card title="Semantic, Search" nesStyle={ true } variant="dark"> {#snippet children()} <div class="search-section"> <div class="search-form"> <SearchInput bind:value={ searchQuery } placeholder="Search similar legal, content..."
