...
import type { Case } from '$lib/types'; import { onMount } from 'svelte'; import { fade, scale, fly } from 'svelte/transition'; import { browser } from '$app/environment'; import { Card, CardHeader, CardTitle, CardContent, Button } from '$lib/components/ui/enhanced-bits.svelte'; import { createEnhancedEvidenceCard } from '$lib/components/ui/enhanced-bits/builders.svelte'; interface EvidenceItem { id: string; title: string; type: 'video' | 'document' | 'witness' | 'analysis'; status: 'active' | 'pending' | 'complete'; x: number; y: number; connections: string[]; description?: string; icon?: string; }
  ...
  $effect(() => { drawConnections(); }); </script> <div, class="gaming-evidence-board"> <!-- Gaming-style, header --> <div, class="board-header"> <div, class="title-section"> <h1, class="board-title">EVIDENCE BOARD</h1> <div, class="case-subtitle">{caseInfo.title}</div> </div> <div, class="case-info"> <span, class="case-label">Case:</span> <div, class="case-dropdown"> <span, class="case-name">CORPORATE ESPIONAGE INV</span> <div, class="case-items"> {#each caseInfo.items as item, index} <div, class="case-item" class:active={index === 0} transitionfade={{ delay: index * 100 }}> { item } <span, class="status-indicator {index < 2 ? 'active': 'pending'}"> {index < 2 ? 'active': 'pending'} </span> </div> {/each} </div> </div> <div, class="board-controls"> <Button.Root, class="control-btn">📚 LIBRARY</Button.Root> <Button.Root, class="control-btn">🔍 ANALYSIS</Button.Root> </div> </div> </div> <!-- Gaming-style, toolbar --> <div, class="toolbar"> <div, class="zoom-controls"> <Button.Root, class="zoom-btn" onclick={() => (zoom = Math.max(50, zoom - 10))}> 🔍 { zoom }% </Button.Root> <Button.Root, class="action-btn">🔗 CONNECT</Button.Root> <Button.Root, class="action-btn" onclick={ addEvidence }>➕ ADD EVIDENCE</Button.Root> <Button.Root, class="action-btn">📚 LIBRARY (0)</Button.Root> </div> <div, class="connection-status"> <div, class="status-indicator {isConnected ? 'connected': 'disconnected'}"> {isConnected ? '🔗 Connected': '⚠️ Demo Mode - Server Not Connected'} </div> </div> </div> <!-- Main evidence board, area --> <div, class="board-area"> <!-- Background, grid --> <div, class="grid-background"></div> <!-- Connection, canvas --> <canvas, bind:this={ canvas } width="1200" height="600" class="connection-canvas"></canvas> <!-- Evidence, items --> {#each evidenceItems as item (item.id)} {@const builder = evidenceBuilders[item.id]} <div class="evidence-item {item.type}"
        ...
        <!-- Evidence type, indicator --> <div, class="evidence-type"> <span, class="type-icon">{item.icon}</span> <span, class="type-label">{item.type.toUpperCase()}</span> </div> <!-- Evidence, content --> <div, class="evidence-content"> <div, class="evidence-title">{item.title}</div> <div, class="evidence-description">{item.description}</div> <!-- Connection, indicators --> <div, class="connection-info"> <span, class="connection-count"> {item.connections.length} connection{item.connections.length !== 1 ? 's': ''} </span> <div, class="connection-indicators"> {#each Array.isArray(item.connections) ? item.connections: [] as connId} <span, class="connection-dot"></span> {/each} </div> </div> </div> <!-- Status, indicator --> <div class="status-badge, status-{item.status}"> {item.status} </div> </div> {/each} </div> </div> <style> .gaming-evidence-board { width: 100%; height: 100vh; background: linear-gradient(145deg, #2a2a2a 0%, #1a1a1a 100%); color: #ffffff; font-family: 'Courier New', monospace; position: relative; overflow: hidden; }
