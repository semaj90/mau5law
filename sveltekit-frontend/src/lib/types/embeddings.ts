...
export interface BatchEmbeddingRequest { texts: string[], model? , string; startTime? :  number; documents?: VectorDocument[]; // use VectorDocument instead of: unknown options?: { batchSize?: number; maxConcurrent?: number}}
...
export interface BatchEmbeddingResponse { success: boolean, results: EmbeddingResponse[]; // concrete response items instead of: unknown summary: { total: number, successful: number, failed: number, processingTime: string}}
