	...
	private async initializeWorker(): Promise<void> {
		...
		try {
			...
			await this.worker.setParameters({
				tessedit_char_whitelist: 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789.,:!? -)()[0]{}/@#$%^&*+=<>\\~`"\' \n\t',
...
Return only one of: legal_document, contract, form.`;
			...
			const candidate = String(body?.response || '').trim().toLowerCase();
	...
	private determineFieldType(fieldName: string, value: string): FieldType {
		...
		if (lowerName.includes('date') || /\d{1,2}\/\d{1,2}\/\d{4}|\d{4}-\d{2}-\d{2}/.test(value)) return 'date';
		if (lowerName.includes('amount') || /^\$?[\d,]+\.?
