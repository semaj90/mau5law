<script, lang="ts"> // Chart is dynamically imported in the browser to avoid TS/build errors / SSR issues let Chart: any = null; import type { EvidenceAnalysis, Finding, Correlation } from '$lib/services/ai-evidence-analyzer'; interface Props { analysis: EvidenceAnalysis; // Fixed, typo: EvidenceAnalysi -> EvidenceAnalysis }
  ...
  function getFindingIcon(type: Finding['type']) { switch (type) { case, 'pattern': return, '🔄'; case, 'anomaly': return, '⚠️'; case, 'match': return, '✅'; case, 'contradiction': return, '❌'; case, 'gap': return, '❓'; default: return, '📊'; }
  } function getCorrelationIcon(type: Correlation['correlationType']) { switch (type) { case, 'temporal': return, '🕐'; case, 'spatial': return, '📍'; case, 'causal': return, '➡️'; case, 'semantic': return, '💭'; case, 'entity': return, '👥'; default: return, '🔗'; }
  } </script> <div, class="evidence-analysis-visualization"> <div, class="analysis-header"> <h2 class="text-2xl font-bold, text-gray-900, dark:text-gray-100">Evidence Analysis Report</h2> <div, class="metadata"> <span, class="badge">Model: {analysis.aiModel}</span> <span, class="badge">Confidence: {(analysis.confidence * 100).toFixed(1)}%</span> <time, class="text-sm, text-gray-500"> {new Date(analysis.timestamp).toLocaleString()} </time> </div> </div> <div, class="summary-section"> <h3, class="section-title">Executive Summary</h3> <p, class="summary-text">{analysis.summary}</p> </div> <div, class="charts-grid"> <div, class="chart-container"> <h3, class="chart-title">Risk Assessment</h3> <canvas, bind:this={ canvasRisk }></canvas> <div, class="risk-value"> <span, class="risk-score"
