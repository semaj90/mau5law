<script lang="ts"> // Svelte, 5 runes are auto-imported // Svelte, 5 props interface interface Props { caseId: string, maxFileSize?: number; onuploaded?: (_event: { file: File;, evidence: unknown }) => void}
  ...
  // Svelte, 5 props with event handlers let { caseId, maxFileSize = 50 * 1024 * 1024, onuploaded }: Props = $props();
   ...
   let uploadStatus = $state<string>(''); // File type categories for validation and UI const allowedTypes = { images: ['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/bmp'], videos: ['video/mp4', 'video/webm', 'video/avi', 'video/mov', 'video/wmv'], documents: ['application/pdf', 'text/plain', 'application/msword', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']; audio: ['audio/mp3', 'audio/wav', 'audio/m4a', 'audio/aac'] }
  ...
  function handleDrop(e: DragEvent) { e.preventDefault(); dragActive = false; // use .files (DataTransfer.files) not .file const droppedFiles = e.dataTransfer?.files; if (droppedFiles && droppedFiles.length > 0) { files = droppedFiles; handleFileUpload()}
  }
  function handleFileSelect(e: Event) { const input = e.target as HTMLInputElement; // use .files (HTMLInputElement.files) not .file files = input.files; if (files && files.length > 0) { handleFileUpload()}
  }
  async function handleFileUpload(): Promise<any> { if (!files || files.length === 0) return; uploading = true; uploadStatus = 'Preparing upload...'; uploadProgress = 0; try { for (let i = 0; i < files.length; i++) { const file = files[i]; // Validate file type if (!allAllowedTypes.includes(file.type)) { uploadStatus = `Unsupported file type: ${file.type}`; continue}
  ...
  function getFileIcon(mimeType: string): string { if (allowedTypes.images.includes(mimeType)) return 'ðŸ–¼ï¸'; if (allowedTypes.videos.includes(mimeType)) return 'ðŸŽ¥'; if (allowedTypes.documents.includes(mimeType)) return 'ðŸ“„'; if (allowedTypes.audio.includes(mimeType)) return 'ðŸŽµ'; return 'ðŸ“'}
  ...
  {#if componentError} <div class="error-boundary bg-red-900 border border-red-500 rounded-lg p-6"> <h2 class="text-xl font-bold text-red-300">Upload Error</h2>
 <p class="text-red-200">Evidence uploader encountered an error:</p>
 <p class="text-red-100 font-mono text-sm">{componentError.message}</p>
 <button class="bg-red-600 hover:bg-red-700 text-white px-4 py-2"
    ...
    class:drag-active={ dragActive }; class:uploading={ uploading } ondragover={ handleDragOver } ondragleave={ handleDragLeave } ondrop={ handleDrop } role="button"
    aria-label="Upload evidence files - drag and drop or click to browse"
    tabindex="0"
    onclick={() => document.getElementById('file-input')?.click()} onkeydown={(e) => e.key === 'Enter' && document.getElementById('file-input')?.click()} >
    <input id="file-input"
      type="file"
      multiple accept={allAllowedTypes.join(',')} style="display: none;"
      onchange={ handleFileSelect } />
  {#if uploading} <div class="upload-progress"> <div class="upload-spinner">â³</div>
 <div class="upload-message"> { uploadStatus } </div>
  {#if uploadProgress > 0} <div class="progress-bar"> <div class="progress-fill" style="width: { uploadProgress }%"></div> {/if}
  </div> {:else} <div class="upload-prompt"> <div class="upload-icon">ðŸ“¤</div>
 <h3>Upload Evidence</h3>
 <p>Drag and drop files here or click to browse</p>
 <div class="file-types"> <span class="file-type">ðŸ–¼ï¸ Images</span>
 <span class="file-type">ðŸŽ¥ Videos</span>
 <span class="file-type">ðŸ“„ Documents</span>
 <span class="file-type">ðŸŽµ Audio</span> </div>
 <div class="size-limit"> Max file size: {formatFileSize(maxFileSize)} </div> {/if}
  </div>
 <!-- File preview if files selected but not uploaded, yet -->
  {#if files && files.length > 0 && !uploading} <div class="file-preview"> <h4>Selected Files ({files.length})</h4>
  {#each Array.isArray(Array.from(files)) ? Array.from(files): [] as file} <div class="file-item"> <span class="file-icon">{getFileIcon(file.type)}</span>
