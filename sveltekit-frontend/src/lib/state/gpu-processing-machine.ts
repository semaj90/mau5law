...
interface GPUProcessingContext {
 processingQueue: DocumentInput[];, activeProcessing: Map<string, DocumentInput>; completedDocuments: ProcessingResult[];, errorDocuments: ProcessingResult[]; serviceHealth: ServiceHealth;, metrics: ProcessingMetrics; maxConcurrent: number;, retryCount: Map<string, number>;
}
...
const startProcessing = ({ context }: {, context: GPUProcessingContext }) => {
 ...
 total > 0 ? (context.completedDocuments.length / total) * 100 : 100;
 },
 Math.random() * 3000 + 1000
 ); // 1-4 second processing time
 }
};
...
export const gpuProcessingMachine = setup({
 ...
 guards: {, canProcessMore: hasQueuedDocuments,
 canRetry,
 },
}).createMachine({
 id: 'gpuProcessing',
