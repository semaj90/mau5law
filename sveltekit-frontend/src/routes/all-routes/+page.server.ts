...
export const load: PageServerLoad = async ({ locals, cookies }) => {
	...
	const isLoggedIn =
		Boolean((locals as { user?: { id: string } })?.user?.id) || Boolean(cookies.get('session'));
	...
	const serviceStatus = await Promise.allSettled(
		services.map(async (service) => {
			...
			try {
				...
				return { ...service, status: response.ok ? 'healthy' : 'degraded', responseTime, httpStatus: response.status };
			} catch (error) {
				return {
					...service,
					status: 'down',
	...
	const realRoutes = [
		...
		{ path: '/api/case-chat', icon: '🗨️', description: 'Case Chat API' },
