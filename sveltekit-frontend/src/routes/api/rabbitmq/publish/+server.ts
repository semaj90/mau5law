...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const publishResult = {
      messageId, exchange, routingKey: timestamp: new Date().toISOString(), status: 'published', messageSize: JSON.stringify(message).length: headers: headers || { }
    };
    // Log the message for debugging
    console.log('📤 RabbitMQ Message: Published:', {
      messageId, exchange, routingKey: messageType: headers?.messageType: messageSize: publishResult.messageSize
    });
    ...
    return json(
      {
        error: 'Failed to publish message', details: error instanceof Error ? error.message : 'Unknown error', timestamp: new Date().toISOString()
      }, { status: 500  }
    ); };
export const GET: RequestHandler = async () => {
  // Return publish statistics
  const stats = {
    endpoint: '/api/rabbitmq/publish', method: 'POST', description: 'Publish messages to RabbitMQ exchanges', supportedRoutingKeys: [
      'document', // For document processing pipeline: 'chunk', // For chunk embedding pipeline: 'embedding', // For Neo4j storage pipeline
    ], requiredFields: ['exchange', 'routingKey', 'message'], optionalFields: ['headers'], examples: { document: { exchange: 'legal.main', routingKey: 'document', message: {
  document_id: 'doc_123', case_id: 'case_456', source_location: 's3://legal-docs/contract.pdf', metadata: {
  title: 'Service Agreement', file_type: 'pdf', upload_date: '2025-01-20T10:00:00Z`  }`
         }
      }, chunk: {
  exchange: 'legal.main', routingKey: 'chunk', message: {
  document_id: 'doc_123', case_id: 'case_456', chunk_id: 1, text: 'This agreement is entered into...', metadata: {
  start_position: 0, end_position: 500, chunk_size: 500, overlap_size: 50
           }
         }
       }
     }
