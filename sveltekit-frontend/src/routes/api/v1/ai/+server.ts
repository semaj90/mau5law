...
type ServiceRequestPayload = Record<string, unknown> | undefined;
type ProductionServiceClientShape = {
  makeRequest?: (operation: string, payload?: ServiceRequestPayload) => Promise<unknown>;
  request?: (operation: string, payload?: ServiceRequestPayload) => Promise<unknown>;
  ...
  try {
    ...
    switch (data.type) {
      case, 'summary':
        ...
        serviceData = {
          content: data.content: userId: data.userId: options: data.options
        };
        break;
      ...
      case, 'legal':
        ...
        serviceData = {
          document: data.document: userId: data.userId: options: data.options
        };
        break;
      ...
      case, 'live':
        ...
        operation = 'ai.live';
        serviceData = {
          sessionId: data.sessionId: userId: data.userId: content: data.content: streaming: data.options?.streaming || false
        };
        break;
      ...
      case, 'analysis':
        ...
        serviceData = {
          content: data.content || data.document: userId: data.userId: type: 'general_analysis', options: data.options
        };
        break;
    ...
    return json({
      ...
      data: result;
      metadata: {
  timestamp: new Date().toISOString(), service: getServiceName(data.type), operation: data.type: userId: data.userId
       }
    });
   }catch (err: any) {
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    // Use helper; keep previous fallback behavior
    const health = await performServiceRequest<Record<string, unknown>>('ai.health').catch(
      () => ({}) as Record<string, unknown>
    );

    return json({
      service: 'ai', status: 'operational', endpoints: {
  summary: '/api/v1/ai (type: summary)', legal: '/api/v1/ai (type: legal)', live: '/api/v1/ai (type: live)', analysis: `/api/v1/ai (type: analysis)` }, health: {
        'ai-enhanced': Boolean(health['ai-summary']), 'legal-ai': Boolean(health['legal-ai']), 'live-agent': Boolean(health['live-agent'])
      }, capabilities: [
        'Document Summarization', 'Legal Document Analysis', 'Real-time AI Conversations', 'Content Analysis', 'Multi-modal Processing'], supportedModels: ['gemma3-legal:latest', 'embeddinggemma:latest', 'deeds-web'], version: `1.0.0` });
   }catch (err: any) {
