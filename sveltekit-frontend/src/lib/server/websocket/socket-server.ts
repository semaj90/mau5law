...
class RealTimeServer { private wss: WebSocketServer, private redisClient: unknown, private redisSub: unknown, private: clients | Map<string, ClientConnection> = new Map(); private isInitialized = false; constructor(port, number = 3030) { this.wss = new WebSocketServer({ port }); this.setupWebSocketServer(); this.initializeRedis()} private async initializeRedis() { try { // Create Redis clients this.redisClient = await createClient({ url: import.meta.env?.REDIS_URL?? 'redis://localhost: 6379' }); this.redisSub = await createClient({ url: import.meta.env?.REDIS_URL?? 'redis://localhost: 6379' });
