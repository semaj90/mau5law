...
export const actions: Actions = {
  upload: async ({ request, locals }) => {
    ...
    if (!file || file.size === 0) {
      return fail(400, { form: { ...form, errors: { file: ['Please select a file to upload'] } } });
    }

    // Validate file size
    if (!validateFileSize(file)) {
      return fail(400, { form: { ...form, errors: { file: ['File size exceeds the maximum limit of 100MB'] } } });
    }
    ...
    if (!validateFileType(file, evidenceType)) {
      return fail(400, { form: { ...form, errors: { file: [`File type ${file.type} is not supported for ${evidenceType} evidence`] } } });
    }
    ...
    try {
      ...
      if (form.data.case_id) {
        ...
        if (caseRecord.length === 0) {
          return fail(400, { form: { ...form, errors: { case_id: ['Selected case not found'] } } });
        }
      }
        ...
        try {
          ...
          if (ocrResponse.ok) {
            ...
            console.log('OCR processing completed: ', {
              filename: file.name,
              pages: (ocrResult as any).pages,
              averageConfidence: (ocrResult as any).averageConfidence,
              legalConceptsFound: (ocrResult as any).legalConcepts?.length || 0,
              citationsFound: (ocrResult as any).citations?.length || 0
            });
          } else {
      ...
      switch (evidenceType) {
        case 'PDF':
          metadata = {
            ...metadata,
            kind: 'PDF',
            pageCount: (ocrResult as any)?.pages || 1,
            isEncrypted: false,
            title: file.name,
            extractedText: (ocrResult as any)?.text,
            legalConcepts: (ocrResult as any)?.legalConcepts || [],
            citations: (ocrResult as any)?.citations || [],
            ocrConfidence: (ocrResult as any)?.averageConfidence
          };
          break;
        case 'IMAGE':
          metadata = {
      ...
      const evidenceRecord = await db
        ...
        .values({
          ...
          metadata: {
            ...
            ocrResult: ocrResult ? {
              extractedText: (ocrResult as any).text,
              confidence: (ocrResult as any).averageConfidence,
              legalConcepts: (ocrResult as any).legalConcepts,
              citations: (ocrResult as any).citations,
              pageCount: (ocrResult as any).pages
            } : null
          }
        })
        .returning();
      ...
      return fail(500, { form: { ...form, errors: { file: ['Failed to upload file. Please try again.'] } } });
    }
