...
const mockOllamaFetch = vi.fn(async () => {
	return new Response(JSON.stringify({
		response: '{"type":"document","icon":"file-text","color":"#6b7280","label":"test","confidence":0.8}',
...
vi.mock('$lib/server/gpu/libtorch-bridge.js', () => ({
	graphSimilarity: vi.fn(async (embs: number[][]) => ({ matrix: embs.map(() => embs.map(() => 0.5)), device: 'cpu' })),
	clusterEmbeddings: vi.fn(async (embs: number[][], k: number) => ({ clusters: [{ centroid: 0, members: [0, 1] }], k, device: 'cpu' })),
	computeCaseEmbedding: vi.fn(async (w: number[], embs: number[][]) => ({ embedding: new Array(768).fill(0.1), device: 'cpu' })),
...
vi.mock('$lib/server/inference/gpu-arbiter.js', () => ({
	acquireGpuLease: vi.fn(async (backend: string, ttl: number) => ({
		backend, ttlSeconds: ttl, acquiredAt: Date.now(), expiresAt: Date.now() + ttl * 1000,
	})),
...
vi.mock('$lib/machines/gpu-process-machine.js', () => ({
	createGpuTask: vi.fn((type: string, payload: any, opts: any) => ({
		id: 'task-1', type, payload, priority: 'medium', backend: opts.backend,
		submittedAt: Date.now(), startedAt: null, completedAt: null,
	})),
...
describe('/api/gpu/compute (POST)', () => {
	...
	it('runs similarity operation', async () => {
		const { POST } = await import('../src/routes/api/gpu/compute/+server.js');
		const event = makeEvent('POST', 'http://localhost/api/gpu/compute', {
			body: { operation: 'similarity', embeddings: [[0.1, 0.2], [0.3, 0.4]] },
		});
		const res = await POST(event as any);
		const data = await jsonBody(res);
	...
	it('runs weighted_embedding operation', async () => {
		const { POST } = await import('../src/routes/api/gpu/compute/+server.js');
		const event = makeEvent('POST', 'http://localhost/api/gpu/compute', {
			body: { operation: 'weighted_embedding', embeddings: [[0.1, 0.2], [0.3, 0.4]], weights: [0.5, 0.5] },
		});
		const res = await POST(event as any);
		const data = await jsonBody(res);
	...
	it('runs embed_compare operation', async () => {
		const { POST } = await import('../src/routes/api/gpu/compute/+server.js');
		const event = makeEvent('POST', 'http://localhost/api/gpu/compute', {
			body: { operation: 'embed_compare', texts: ['contract law', 'property law'], query: 'lease agreement' },
		});
		const res = await POST(event as any);
		const data = await jsonBody(res);
...
describe('/api/tools/execute', () => {
	...
	it('GET returns 401 for unauthenticated', async () => {
		const { GET } = await import('../src/routes/api/tools/execute/+server.js');
		const res = await GET(makeEvent('GET', 'http://localhost/api/tools/execute', {
			locals: { user: null },
		}) as any);
		expect(res.status).toBe(401);
	});
...
describe('/api/topology (GET)', () => {
	...
	it('returns 401 for unauthenticated', async () => {
		const { GET } = await import('../src/routes/api/topology/+server.js');
		const res = await GET(makeEvent('GET', 'http://localhost/api/topology', {
			locals: { user: null },
		}) as any);
		expect(res.status).toBe(401);
	});
