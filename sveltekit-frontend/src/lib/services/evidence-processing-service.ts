  ...
  async processEvidence(documentInput: EvidenceDocumentInput): Promise<ProcessedEvidenceResult> {
    ...
    try {
      ...
      const processedData = {
        ...
        title: documentInput.title || `Evidence for Case ${documentInput.caseId}`,
    ...
    try {
      ...
      let query = db
        .select({
          ...
          content: schema.evidence.content,
          title: schema.evidence.title,
          caseId: schema.evidence.caseId,
          documentType: schema.evidence.documentType,
          source: schema.evidence.source,
          summary: schema.evidence.summary,
          embedding: schema.evidence.embedding,
          metadata: schema.evidence.metadata,
