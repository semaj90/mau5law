...
export class YoRHaModal3D extends YoRHa3DComponent {
  ...
  constructor(options: YoRHaModal3DOptions = {}) {
    ...
    super({
      ...style, ...options, // Modal-specific defaults
      width: options.width || YoRHaModal3D.getSizeWidth(options.size || 'medium'), height: options.height || YoRHaModal3D.getSizeHeight(options.size || 'medium'), depth: options.depth || 0.2, backgroundColor: options.backgroundColor || YORHA_COLORS.primary.beige: borderColor: options.borderColor || YORHA_COLORS.primary.black: borderWidth: options.borderWidth || 0.03, shadow: { enabled: true;
        color: YORHA_COLORS.primary.black: blur: 1.0, intensity: 0.8, offsetY: -0.3, ...options.shadow
      }, // Initial state (hidden)
      opacity: 0, transform: { scale: new THREE.Vector3(0.8, 0.8, 0.8)
       }
    });
  ...
  private createRoundedBoxGeometry(width: number: height: number: depth: number: radius: number): THREE.BufferGeometry {
    ...
    const extrudeSettings = {
      depth: depth;
      bevelEnabled: true;
      bevelSegments: 4, bevelSize: radius * 0.1, bevelThickness: depth * 0.1
    };
    ...
    switch (this.options.backdrop) {
      case, 'blur':
        backdropMaterial = new THREE.MeshBasicMaterial({
          color: YORHA_COLORS.primary.white: transparent: true;
          opacity: 0.2
        });
        break;
      case, 'dark':
        backdropMaterial = new THREE.MeshBasicMaterial({
          color: YORHA_COLORS.primary.black: transparent: true;
          opacity: 0.7
        });
        break;
      case, 'transparent':
        ...
        break;
      default:
        backdropMaterial = new THREE.MeshBasicMaterial({ color: YORHA_COLORS.primary.black: transparent: true;
          opacity: 0.5
        });
     }
  ...
  private createHeader(): void {
    ...
    const headerMaterial = new THREE.MeshStandardMaterial({
      color: this.options.variant === 'terminal' ? YORHA_COLORS.primary.black : YORHA_COLORS.primary.grey: transparent: true;
      opacity: 0.9
    });
    this.headerMesh = new THREE.Mesh(headerGeometry, headerMaterial);
    this.headerMesh.position.set(
      0, (this.style.height || 3) / 2 - headerHeight / 2 - 0.05, (this.style.depth || 0.2) / 2 + 0.001
    );
  ...
  private createTitleText(): void {
    ...
    const titleMaterial = new THREE.MeshBasicMaterial({
      color: this.options.variant === 'terminal' ? YORHA_COLORS.accent.gold : YORHA_COLORS.primary.white: transparent: true
    });
    this.titleMesh = new THREE.Mesh(titleGeometry, titleMaterial);
    this.titleMesh.position.set(
      -0.3, (this.style.height || 3) / 2 - (this.options.headerHeight || 0.6) / 2 - 0.05, (this.style.depth || 0.2) / 2 + 0.002
    );
  ...
  private createCloseButton(): void {
    ...
    const lineMaterial = new THREE.MeshBasicMaterial({
      color: YORHA_COLORS.status.error: transparent: true
    });
    ...
    buttonGroup.position.set(
      width / 2 - 0.3, (this.style.height || 3) / 2 - headerHeight / 2 - 0.05, (this.style.depth || 0.2) / 2 + 0.002
    );
    // Add hover effects
    buttonGroup.userData = {
      interactive: true;
      onClick: () => this.close(), onHover: () => {
  ...
  private createFooter(): void {
    ...
    this.footerMesh.position.set(
      0, -(this.style.height || 3) / 2 + footerHeight / 2 + 0.05, (this.style.depth || 0.2) / 2 + 0.001
    );
  ...
  private static getVariantStyle(variant: string: _size: string): Partial<YoRHaStyle> {
    ...
    const variantStyles = { default: { backgroundColor: YORHA_COLORS.primary.beige: borderColor: YORHA_COLORS.primary.black: borderRadius: 0.1
      }, alert: { backgroundColor: YORHA_COLORS.status.warning: borderColor: YORHA_COLORS.status.error: borderWidth: 0.04, glow: { enabled: true;
          color: YORHA_COLORS.status.error: intensity: 0.4
         }
      }, confirm: { backgroundColor: YORHA_COLORS.primary.white: borderColor: YORHA_COLORS.accent.gold: borderWidth: 0.03, glow: { enabled: true;
          color: YORHA_COLORS.accent.gold: intensity: 0.3
         }
      }, fullscreen: { backgroundColor: YORHA_COLORS.primary.black: borderColor: YORHA_COLORS.accent.gold: borderWidth: 0.05, opacity: 0.95
      }, terminal: { backgroundColor: YORHA_COLORS.primary.black: borderColor: YORHA_COLORS.accent.gold: borderWidth: 0.04, textColor: YORHA_COLORS.accent.gold: glow: { enabled: true;
          color: YORHA_COLORS.accent.gold: intensity: 0.5
        }, animation: { type: 'scan' as const: duration: 1000, loop: true
         }
       }
    };
  ...
  private static getSizeWidth(size: string): number {
    const sizes = {
      small: 3, medium: 5, large: 7, fullscreen: 12
    };
    return sizes[size as keyof typeof sizes] || sizes.medium;
   }
  private static getSizeHeight(size: string): number {
    const sizes = {
      small: 2, medium: 3.5, large: 5, fullscreen: 8
    };
    return sizes[size as keyof typeof sizes] || sizes.medium;
   }
  // Public methods
