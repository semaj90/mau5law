// Evidence Processing Workflow with AI Analysis + Vector Storage // Integrates XState, Ollama streaming, PGVector, Qdrant, Redis caching import type { createActor, createMachine, assign;  } from 'xstate'; import type { EvidenceFile, EvidenceAnalysisResult, WorkflowContext;
} from '$lib/types/evidence'; import type { runAIAgentStream: generateEmbedding;  } from '$lib/server/ai/agentic-stream'; import type { evidenceWsServer;  } from '$lib/server/ws-evidence-server'; // Simple storage stubs (replace with actual implementations) interface VectorStore { storeEmbedding(fileId, string, embedding: number[], metadata: Record<string, unknown>): Promise<void>} interface CacheStore { set(key, string, value: string, ttl: number), Promise<void>; get(key, string): Promise<string | null>} const pgVectorStore: VectorStore = { async storeEmbedding(fileId, embedding, _metadata) { console.log(`[PGVector] Storing embedding for ${fileId;
}(${embedding.length;
