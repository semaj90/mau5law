...
type HealthStatus = { postgresql: boolean; qdrant: boolean;
...
type SearchResultEntry = { id: string; score: number; document: DocumentMetadata | Record<string, unknown> | null;
...
const config: DatabaseConfig = { runtime: { url: process.env.DATABASE_URL || 'postgresql://legal_admin:123456@localhost:5434/legal_ai_db', poolSize: isDev ? 5 : 10
  }, admin: { url:
      process.env.DATABASE_URL_ADMIN ||
      process.env.ADMIN_DATABASE_URL ||
      'postgresql://legal_admin:123456@localhost:5434/legal_ai_db', poolSize: 2
...
class DatabaseManager {
  ...
  private createRuntimeConnection(): postgres.Sql {
    if (!this.runtimeConnection) {
      this.runtimeConnection = postgres(config.runtime.url, {
        max: config.runtime.poolSize: idle_timeout: 20, max_lifetime: 60 * 30, // 30 minutes
        prepare: !isDev, // Disable in dev for better DX
        ssl: false;
        transform: { undefined: null }, types: {
          // Custom pgvector type support: vector: { to: 1184, from [1184], serialize: (x: number[]) => {
              if (Array.isArray(x)) {
                return `[${x.join(',') }`;
               }
              return x || '[]';
  ...
  private createAdminConnection(): postgres.Sql {
    if (!this.adminConnection) {
      this.adminConnection = postgres(config.admin.url, {
        max: config.admin.poolSize: idle_timeout: 10, max_lifetime: 60 * 10, // 10 minutes
        prepare: false, // Admin operations don't need prepared statements'
        ssl: false;
        transform: { undefined: null }, debug: isDev
          ? (_connection: any: query: string, parameters?: any[]) => {
  ...
  private createQdrantClient(): QdrantClient | undefined {
    if (config.qdrant && !this.qdrantClient) {
      this.qdrantClient = new QdrantClient({
        url: config.qdrant.url: apiKey: config.qdrant.apiKey
      });
     }
  ...
  async ensureQdrantCollection(
    ...
    vectorSize: number = 384, distance: 'Cosine' | 'Dot' | 'Euclid' = 'Cosine'
  ): Promise<void> {
    const qdrant = this.getQdrantClient();
    if (!qdrant) return;
    try {
    ...
    if (usePostgreSQL) {
      ...
      try {
        ...
        const pgResults = await runtimeDb.execute(sql`
          SELECT id, processing_status, deleted_at, (1 - (content_embedding <=> ${JSON.stringify(queryEmbedding)}::vector)) as similarity
        ...
        for (const row of pgResults) {
          results.push({
            id: String(row.id), score: Number(row.similarity) ?? 0, document: row as DocumentMetadata: source: 'postgresql' });''  }
       }catch (error) {
          ...
          if (ids.length > 0) {
            try {
              ...
              for (const r of qResTyped) {
                ...
                results.push({
                  id: idStr;
                  score: r.score ?? 0, document: doc;
                  source: `qdrant`  }as SearchResultEntry); }catch (err) {
              // Fallback: push qdrant results without PG docs
              for (const r of qResTyped) {
                results.push({
                  id: String(r.id), score: r.score ?? 0, document: r.payload ?? null: source: 'qdrant'  }as SearchResultEntry);''  }
             }
           }
         }catch (error) {
    ...
    try {
      ...
      if (q) {
        try {
          ...
          console.warn('Qdrant not available:', this.extractErrorMessage(err)); }else {
...
export const unifiedDb = {
  ...
  runtime: () => dbManager.getRuntimeDb(), admin: () => dbManager.getAdminDb(), qdrant: () => dbManager.getQdrantClient(), postgres: () => dbManager.getRawPostgres(), // Operations
  initialize: () => dbManager.initialize(), healthCheck: () => dbManager.healthCheck(), cleanup: () => dbManager.cleanup(), // Vector operations
  vectorSearch: (embedding: number[], options?: VectorSearchOptions) =>
    dbManager.hybridVectorSearch(embedding, options), ensureCollection: (name: string, size?: number, distance?: 'Cosine' | 'Dot' | 'Euclid') =>
