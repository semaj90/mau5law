...
export const boardActions = {
  ...
  addEvidenceToBoard(evidenceId: string, evidenceUrl: string, evidenceType: string, position = { x: 100, y: 100 }) {
...
export const objectsByType = derived(
  [boardObjects],
  ([$objects]) => ({
...
export const boardStats = derived(
  [boardObjects],
  ([$objects]) => ({
    ...
    connectionCount: $objects.reduce((acc, obj) => acc + (obj.connections?.length || 0), 0)
  })
);