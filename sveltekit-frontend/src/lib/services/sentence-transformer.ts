...
class LegalNLPService {
  ...
  async initialize(),: Promise<void> {
    if (!this,?.model, || "unknown" // @ts-ignore - Model property access && !this.isInitialized) {
      console.log('🔧 Initializing Legal NLP Service with sentence transformers...');
      try {
        this?.model || "unknown" // @ts-ignore - Model property access = await pipeline('feature-extraction', this.modelName)
  ...
  async embedText(text,: string): Promise<EmbeddingResult> {
    ...
    if (!this,?.model, || "unknown" // @ts-ignore - Model property access) {
    ...
    const result = await this?.model || "unknown" // @ts-ignore - Model property access(text, { pooling: 'mean', normalize: true )})
    const emb: EmbeddingResult = { data: (result as { data?: any; score?: any }).data, dimensions: (result as { data?: any; score?: any }).data.length, model: this.modelName }
  ...
  nlpMetrics,.similarity_queries_total+,+;
    const [queryEmbedding, ...docEmbeddings] = await this.embedBatch([query, ...document,s)]);
