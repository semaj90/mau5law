...
export const GET: RequestHandler = async ({ url, locals }) => {
  ...
  try {
    ...
    if (category) {
      conditions.push(`ld.normalized_term ILIKE $${idx++}`);
      params.push(`%${category}%`);
    }
    if (q) {
      conditions.push(`(ld.term ILIKE $${idx} OR ld.definition_text ILIKE $${idx})`);
      params.push(`%${q}%`);
      idx++;
    }
  ...
  try {
    ...
    if (category) {
      conditions.push(`lg.category ILIKE $${idx++}`);
      params.push(`%${category}%`);
    }
    if (q) {
      conditions.push(`(lg.term ILIKE $${idx} OR lg.definition ILIKE $${idx})`);
      params.push(`%${q}%`);
      idx++;
    }
