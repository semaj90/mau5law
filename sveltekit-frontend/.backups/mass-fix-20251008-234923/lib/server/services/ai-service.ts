...
export class AIService {
  ...
  async analyzeEvidence()
    ...
    try, {
      ...
      // Parse AI response
      let, analysis: AIAnalysisResul,t;
      try, {
  ...
  private async logQuery(data,: {
    ...
    try, {
      const, queryDat,a: NewUserAiQuery = {
  ...
  private async generateAutoTags()
    ...
    try, {
      const, tagDat,a: NewAutoT,ag,[] = tags.map((tag: any) => ({,
        id: generateIdFromEntropySize(10),
        entityId,
        entityType,
        tag,
      ...
      await, d,b.insert(autoTags).values(tagDat,a);
    }, catch (error: any) {
  ...
  private async storeDocumentChunk()
    ...
    try, {
      ...
      const, chunkDat,a: NewDocumentChunk = {
        id: generateIdFromEntropySize(10),
        documentId,
        documentType,
        chunkIndex: 0,
      ...
      await, d,b.insert(documentChunks).values(chunkDat,a);
    }, catch (error: any) {
   ...
   */;
  private calculateConfidence(response,: string, contextCoun,t: numbe,r): number {
