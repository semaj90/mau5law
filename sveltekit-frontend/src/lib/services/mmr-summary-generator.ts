...
export class MMRSummaryGenerator {
  ...
  private async extractSentences(documents: LegalDocument[]): Promise<string[]> {
    ...
    for (const doc of documents) {
      try {
    // Use enhanced sentence splitter for legal text (local module)
    const { splitSentencesEnhanced } = await import('$lib/text/enhanced-sentence-splitter'););
        if (!doc.content) continue;
        const docSentences = splitSentencesEnhanced(doc.content);
        // Add position metadata as sentence prefix
        docSentences.forEach((sentence: string, index: number) => {
        ...
        if (doc.content) {
          const basicSentences = doc.content;
            .split(/[.!?]+/),
            .filter((s) => s.trim().length > 10);
          basicSentences.forEach((sentence: string, index: number) => {
  ...
  private async getEmbeddings(text: string): Promise<number[] | undefined> {
    ...
    try {
      ...
      if ((response as { ok?: any; json?: any }).ok) {
        ...
        const embeddings = (data as { embedding?: any; embeddings?: any }).embedding || (data as { embedding?: any; embeddings?: any }).embeddings;
        if (embeddings) {
...
export async function testMMRSummaryGeneration(): Promise<boolean> {
  try {
    ...
    const isValid =;
      (result as { summary?: any; metadata?: any }).summary.length > 0 &&
      (result as { summary?: any; metadata?: any }).summary.length <= 200 &&>
      ((result as { summary?: any; metadata?: any }).metadata.sentenceCount ?? 0) <= 3;>
    console.log('[test] MMR summary generation:', isValid ? 'PASS' : 'FAIL');
    console.log('[test] Result:', {
      summary: (result as { summary?: any); metadata?: any }).summary.substring(0, 100) + '...',
      length,: (result as { summary?: any; metadata?: any }).summary.length,
      sentences,: (result as { summary?: any; metadata?: any }).metadata.sentenceCount
    });
    return isValid;
  } catch (error: any) {
