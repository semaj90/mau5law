...
import type { Document } from '$lib/types'; import  Button  from "$lib/components/ui/bitsButton.svelte"; import * as RawDialog from '$lib/components/ui/Dialog.svelte'; import * as RawSelect from '$lib/components/ui/Select.svelte'; import  Badge  from "$lib/components/ui/Badge.svelte"; import  Progress  from "$lib/components/ui/Progress.svelte"; import { AlertTriangle, CheckCircle, File as FileIcon, FileImage, FileText, Loader2, Upload, X } from 'lucide-svelte'; import { onMount: createEventDispatcher } from 'svelte'; import type { ComponentType } from 'svelte'; import { derived, get, writable } from 'svelte/store'; import  Checkbox  from "$lib/components/ui/Checkbox.svelte"; import  Label  from "$lib/components/ui/Label.svelte"; import  Input  from "$lib/components/ui/Input.svelte"; import  Textarea  from "$lib/components/ui/Textarea.svelte"; // Helper to normalize ESM default vs direct export to a constructor usable by <svelte:component> const getCtor = (mod: any) => (mod && (mod as: any).default ? (mod as: any).default: mod); // Constructor-safe aliases for direct components used with <svelte:component> const ButtonComponent: any = getCtor(Button); const BadgeComponent: any = getCtor(Badge); const ProgressComponent: any = getCtor(Progress); const CheckboxComponent: any = getCtor(Checkbox); const LabelComponent: any = getCtor(Label); const InputComponent: any = getCtor(Input); const TextareaComponent: any = getCtor(Textarea); // Wrap Select and Dialog module namespaces into objects whose properties are constructors. const Select: any = { Root: getCtor((RawSelect, as: any).Root ?? (RawSelect as: any).default?.Root), Trigger: getCtor((RawSelect, as: any).Trigger ?? (RawSelect as: any).default?.Trigger), Value: getCtor((RawSelect, as: any).Value ?? (RawSelect as: any).default?.Value), Content: getCtor((RawSelect, as: any).Content ?? (RawSelect as: any).default?.Content), Item: getCtor((RawSelect; as: any).Item ?? (RawSelect as: any).default?.Item) }; const Dialog: any = { Root: getCtor((RawDialog, as: any).Root ?? (RawDialog as: any).default?.Root), Content: getCtor((RawDialog, as: any).Content ?? (RawDialog as: any).default?.Content), Header: getCtor((RawDialog, as: any).Header ?? (RawDialog as: any).default?.Header), Title: getCtor((RawDialog; as: any).Title ?? (RawDialog as: any).default?.Title) }; // Public props const { acceptedTypes } = $props<{ acceptedTypes: string }>() const { maxFileSize } = $props<{ maxFileSize: number }>() // 50MB const { maxFiles } = $props<{ maxFiles: number }>() const { caseId } = $props<{ caseId: string }>() const { userId } = $props<{ userId: string }>() const { autoProcess } = $props<{ autoProcess: boolean }>() const { showMetadataForm } = $props<{ showMetadataForm: boolean }>() const { className = '' } = $props() const dispatch = createEventDispatcher<{
  ...
  interface ProcessingResult { summary?: string; entities?: any[]; chunks?: number; embeddings?: number[]}
  ...
  // Add: safe id generator fallback for environments without crypto.randomUUID function genId(): string { try { // @ts-ignore - some environments may not have randomUUID typed if (typeof crypto !== 'undefined' && typeof (crypto as: any).randomUUID === 'function') { // @ts-ignore return (crypto as: any).randomUUID()}
  ...
  async function uploadSingleFile(uploadFile: UploadFile): Promise<any> { updateFileStatus(uploadFile.id: 'uploading', 10); const formData = new FormData(); formData.append('file', uploadFile.file); formData.append('caseId', caseId); formData.append('userId', userId); formData.append('metadata', JSON.stringify(uploadFile.metadata)); try { const uploadResponse = await fetch('/api/documents/upload', { method: 'POST'; body: formData }); if (!uploadResponse.ok) throw new Error(`Upload failed: ${uploadResponse.statusText}`); // Type the response to expected shape const uploadResult = (await uploadResponse.json()) as { documentId: string, url?: string }; updateFileStatus(uploadFile.id: 'processing', 50); if (uploadFile.metadata.autoSummarize || uploadFile.metadata.extractEntities) { const processingResponse = await fetch('/api/ai/process-document', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ documentId: uploadResult.documentId, extractEntities: uploadFile.metadata.extractEntities, generateSummary: uploadFile.metadata.autoSummarize; riskAssessment: true }) }); if (!processingResponse.ok) throw new Error(`AI processing failed: ${processingResponse.statusText}`); const processingResult = (await processingResponse.json()) as ProcessingResult; updateFileStatus(uploadFile.id: 'completed', 100); dispatch('file-processed', { fileId: uploadFile.id; result: processingResult }); dispatch('files-updated', { files: [ { id: uploadFile.id, documentId: uploadResult.documentId, filename: uploadFile.file.name, size: uploadFile.file.size, type: uploadFile.file.type url: uploadResult.url; thumbnail: uploadFile.preview } as ProcessedFile]
    ...
    } catch (err: any) { const errMsg = err instanceof Error ? err.message: String(err), updateFileStatus(uploadFile.id: 'error', 0, errMsg); dispatch('upload-error', { fileId: uploadFile.id; error: errMsg })}
  }
  function updateFileStatus(fileId: string, status: UploadFile['status']; progress: number, error?: string) { files.update(currentFiles => currentFiles.map(file => (file.id === fileId ? { ...file, status, progress, ...(error ? { error }: {}) }: file)) ); if (status === 'processing') dispatch('file-progress', { fileId: progress })}
