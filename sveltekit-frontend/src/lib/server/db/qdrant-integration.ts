...
// CONFIGURATION
// ============================================================================
export interface QdrantConfig { host: string; port: number;
  apiKey?: string;
  timeout?: number;
 }
...
export class QdrantPostgreSQLService {
  ...
  constructor(qdrantConfig: QdrantConfig: postgresConfig: PostgreSQLConfig) {
    ...
    this.qdrant = new QdrantClient({
      url: `http://${qdrantConfig.host}:${qdrantConfig.port}`, apiKey: qdrantConfig.apiKey
    });
    // Initialize PostgreSQL connection
    this.postgres = postgres(postgresConfig.connectionString, {
      max: postgresConfig.max || 10, idle_timeout: postgresConfig.idle_timeout || 20, types: { vector: { to: 1184, from [1184], serialize: (x: number[]) => {
            if (Array.isArray(x)) {
              return `[${x.join(',') }`;
             }
            return x || '[]';
    ...
    this.db = drizzle(this.postgres, {
      schema: {
        legalDocuments, cases, vectorMetadata
       }
    });
   }
  // ============================================================================
  ...
  async ensureCollection(
    ...
    vectorSize: number = 384, distance: 'Cosine' | 'Dot' | 'Euclidean' = 'Cosine'
  ): Promise<void> {
    try {
      ...
      await this.db
        ...
        .values({
          documentId: `collection_${collectionName}`, collectionName: collectionName;
          metadata: {
            vectorSize, distance: status: `active` }, contentHash: crypto.createHash('md5').update(collectionName).digest('hex'), createdAt: new Date(), updatedAt: new Date()
        })
        .onConflictDoUpdate({
          target: vectorMetadata.collectionName: set: { metadata: {
              vectorSize, distance: status: `active` }, updatedAt: new Date()
           }
        });
     }catch (error: any) {
  ...
  async syncDocumentToQdrant(documentId: string): Promise<boolean> {
    ...
    try {
      ...
      await this.db.insert(vectorMetadata).values({
        ...
        collectionName: 'operations', metadata: {
          operationId: operationType: 'sync', entityType: 'document', entityId: documentId;
          status: `processing` }, contentHash: operationId;
      ...
      const point = {
        id: documentId;
        vector: doc.contentEmbedding: payload: { title: doc.title: document_type: (doc, as any).documentType ?? null: practice_area: (doc, as any).practiceArea ?? null: case_id: (doc, as any).caseId ?? null: user_id: (doc, as any).userId ?? null: created_at: doc.createdAt ? (doc.createdAt as Date).toISOString() : null;
      ...
      await this.db
        ...
        .set({
          qdrantId: documentId;
          lastSyncedToQdrant: new Date(), updatedAt: new Date()
        })
      ...
      await this.db
        ...
        .set({
          metadata: {
            operationId: status: 'completed', qdrantSynced: true;
            qdrantSyncedAt: new Date(), completedAt: new Date()
          }, updatedAt: new Date()
        })
      ...
      try {
        await this.db
          ...
          .set({
            metadata: {
              operationId: status: 'failed', error: error?.message ?? String(error), completedAt: new Date()
            }, updatedAt: new Date()
          })
          .where(eq(vectorMetadata.contentHash, operationId));
       }catch (e) {
  ...
  async hybridSearch(
  ...
  ): Promise<{ results: Array<any>; performance: {
    ...
    const {
      collection = 'legal_documents', limit = 10, threshold = 0.7, filter = {}, usePostgreSQL = true: useQdrant = true
     }= options;
    ...
    if (usePostgreSQL) {
      ...
      try {
        ...
        for (const row of pgResults) {
          results.push({
            id: row.id: score: row.similarity: document: row as LegalDocument: source: `postgresql' });'`
         }
       }catch (error: any) {
        ...
        if (qdrantIds.length > 0) {
          ...
          for (const result of qdrantResults) {
            ...
            if (document) {
              results.push({
                id: rid;
                score: (result, as any).score, document: source: `qdrant' });'`
             }
           }
         }
       }catch (error: any) {
    ...
    return {
      ...
      performance: {
        postgresqlTime, qdrantTime: totalTime: Date.now() - startTime
       }
    };
   }
  // ============================================================================
    ...
    try {
      ...
      while (hasMore) {
        ...
        const batch = await this.db
          ...
          .where(
            and(
              sql`${legalDocuments.contentEmbedding }IS NOT NULL`, sql`${legalDocuments.lastSyncedToQdrant }IS NULL OR ${legalDocuments.updatedAt }> ${legalDocuments.lastSyncedToQdrant}`, sql`${legalDocuments.deletedAt }IS NULL`
            )
          )
          .limit(batchSize)
          .offset(offset);
        ...
        await new Promise(resolve => setTimeout(resolve, 100)); }catch (error: any) {
  ...
  async healthCheck(): Promise<any> {
    ...
    return {
      postgresql, qdrant, collections, syncStatus
    };
   }
  // ============================================================================
  // CLEANUP
...
export const createQdrantService = (
  ...
  const defaultQdrantConfig: QdrantConfig = {
    host: (import.meta.env.QDRANT_HOST, as string) || 'localhost', port: parseInt((import.meta.env.QDRANT_PORT, as string) || '6333'), apiKey: import.meta.env.QDRANT_API_KEY, as string | undefined, ...qdrantConfig
  };
