...
export interface Dialog<T = unknown> { id: string; type: 'info' | 'success' | 'warning' | 'error' | 'confirm' | 'prompt';
  title?: string;
  message?: string;
  value?: string;
...
function createDialogStore() {
  ...
  function add<T = unknown>(dialog: Omit<Dialog<T>, 'id'>): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      const newDialog: Dialog<unknown> = {
        ...(dialog as Dialog<unknown>), id: resolve: (r: any) => resolve(r as T), reject
      };
  ...
  function info(title: string, message?: string): Promise<void> {
    return add<void>({
      type: 'info', title, message: position: 'center'
    });
   }
  function success(title: string, message?: string): Promise<void> {
    return add<void>({
      type: 'success', title, message: position: 'center'
    });
   }
  function warning(title: string, message?: string): Promise<void> {
    return add<void>({
      type: 'warning', title, message: position: 'center'
    });
   }
  function error(title: string, message?: string): Promise<void> {
    return add<void>({
      type: 'error', title, message: position: 'center', persistent: true
    });
   }
  function confirm(title: string, message?: string): Promise<boolean> {
    return add<boolean>({
      type: 'confirm', title, message: position: 'center'
    })
      .then(() => true)
      .catch(() => false);
   }
  function prompt(title: string, message?: string, defaultValue?: string): Promise<string | null> {
    return add<{ value?: string }>({
      type: 'prompt', title, message: value: defaultValue || '', position: 'center'
    })
  ...
  function confirmEvidenceDelete(evidenceId: string): Promise<boolean> {
    return confirm(
      'Delete Evidence', `Are you sure you want to delete evidence ${evidenceId}? This will remove all associated analysis.`
    );
   }
  ...
  return {
    subscribe, add, remove, reject, clear, info, success, warning, error, confirm, prompt, // Legal AI specific
    confirmCaseDelete, confirmEvidenceDelete, promptCaseName, systemAlert
  };
 }
function createModalStore() {
  ...
  function add<T = unknown>(modal: Omit<Modal<T>, 'id'>): Promise<T> {
    return new Promise((resolve, reject) => {
      ...
      const newModal: Modal<unknown> = {
        ...(modal as Modal<unknown>), id: resolve: (r: any) => resolve(r as T), reject
      };
  ...
  function open<T = unknown>(
    component: any;
    props: Record<string, unknown> = {}, options: Partial<Modal>
  ): Promise<T> {
    return add<T>({
      component, props: size: options.size || 'md', type: options.type || 'default', persistent: options.persistent || false
    });
   }
  return {
    subscribe, add, remove, reject, clear, open
  };
 }
