<!-- N64-Style Button Component -->
<script lang="ts">
  import { createEventDispatcher } from 'svelte';
  import type { Snippet } from 'svelte';
  interface Props {
    variant?: 'primary' | 'secondary' | 'c-up' | 'c-down' | 'c-left' | 'c-right' | 'start' | 'z' | 'a' | 'b';
...
>
  <!-- Loading State -->
  {#if loading}
    <div class="n64-button__loading">
      <div class="n64-button__spinner"></div>
    </div>
  {:else}
    <!-- Button Content -->
    <div class="n64-button__content">
      {#if buttonContent}
        <span class="n64-button__icon">{buttonContent}</span>
      {/if}
      {#if children}
        <span class="n64-button__text">
          {@render children()}
        </span>
      {/if}
    </div>
...
/* Color Variants */ {}
  ...
  .n64-button--secondary {
    background: linear-gradient(145deg, #0055FF, #0044CC);
    color: white;
    text-shadow: 1px 1px 2px rgba(0, 0, 0, 0.5);
  }
/* C Button Variants */ {}
...
/* Console theme integration */ {}
  ...
  .theme-snes .n64-button--primary {
    background: linear-gradient(145deg, #B266FF, #9C44FF);
  }
  .theme-ps1 .n64-button--primary {
    background: linear-gradient(145deg, #003791, #002570);
  }
  .theme-ps2 .n64-button--primary {
    background: linear-gradient(145deg, #1B3A6B, #0F2951);
  }
