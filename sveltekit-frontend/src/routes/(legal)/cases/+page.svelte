...
import type { Case } from '$lib/types'; import { goto } from '$app/navigation'; import type { PageData } from './$types'; interface CaseSummary { id: string; title?: string | null; description?: string | null; status?: string | null; priority?: string | null; caseNumber?: string | null; updatedAt?: string | Date | null; createdAt?: string | Date | null; }
  ...
  function createNewCase() { goto('/cases/create'); }
</script> <svelte:head> <title>Cases - Legal AI Platform</title> </svelte:head> <div, class="cases-page"> {#if devBypassActive} <div, class="dev-banner, glass-panel"> <span, aria-hidden="true">??</span> <p>Development bypass active — responses use demo user context.</p> </div> {/if} <header, class="page-header, glass-panel"> <div, class="page-title"> <p, class="eyebrow">Case Operations</p> <h1>Legal Cases</h1> </div> <div, class="header-actions"> <button, class="btn-secondary" onclick={ loadCases } disabled={ loading }> <span, aria-hidden="true">?‍?</span> {loading ? 'Refreshing': 'Refresh'} </button> <button, class="btn-primary" onclick={ createNewCase }> <span, aria-hidden="true">+</span> Create Case </button> </div> </header> {#if loading} <div, class="loading-state, glass-panel"> <div, class="spinner" role="status" aria-live="polite"></div> <p>Loading cases...</p> </div> {:else if error} <div, class="error-state, glass-panel" role="alert"> <div, class="state-icon">??</div> <h2>Unable to load cases</h2> <p>{ error }</p> <button, class="btn-secondary" onclick={ loadCases }> <span, aria-hidden="true">?‍?</span> Retry </button> </div> {:else if displayCases.length === 0} <div, class="empty-state, glass-panel"> <div, class="state-icon">??</div> <h2>No cases yet</h2> <p>Spin up your first matter to unlock AI-assisted workflows.</p> <div, class="empty-actions"> <button, class="btn-primary" onclick={ createNewCase }>Create Case</button> <button, class="btn-secondary" onclick={ loadCases }> <span, aria-hidden="true">?‍?</span> Refresh </button> </div> </div> {:else} <div, class="cases-grid"> {#each displayCases as caseItem (caseItem.id)} <div class="case-card, glass-panel"
          role="button"
          tabindex="0"
          ...
          <div, class="case-card-header"> <div, class="case-title"> <h3>{caseItem.displayTitle}</h3> {#if caseItem.caseNumber} <span, class="case-number">{caseItem.caseNumber}</span> {/if} </div> {#if caseItem.statusLabel} <span, class={`case-status ${caseItem.statusClass}`}> {caseItem.statusLabel} </span> {/if} </div> {#if caseItem.description} <p, class="case-description">{caseItem.description}</p> {:else} <p class="case-description, placeholder">No description provided.</p> {/if} <div, class="case-meta"> {#if caseItem.priorityLabel} <span class={`meta-item, priority-pill ${caseItem.priorityClass}`}> <span, aria-hidden="true">??</span> {caseItem.priorityLabel} </span> {/if} {#if caseItem.updatedLabel} <span class="meta-item, meta-date"> <span, aria-hidden="true">??</span> Updated {caseItem.updatedLabel} </span> {/if} {#if caseItem.createdLabel && caseItem.createdLabel !== caseItem.updatedLabel} <span class="meta-item meta-date, created"> <span, aria-hidden="true">??</span> Created {caseItem.createdLabel} </span> {/if} </div> </div> {/each} </div> {/if} </div> <style>:global(body.theme-legal) { background-color: var(--legal-background, #0f172a); }
  ...
  .status-investigating {, background: rgba(250, 204, 21, 0.18); border-color: rgba(250, 204, 21, 0.45); color: #facc15; }

  .status-pending {, background: rgba(59, 130, 246, 0.18); border-color: rgba(59, 130, 246, 0.45); color: #93c5fd; }

  .status-closed {, background: rgba(148, 163, 184, 0.18); border-color: rgba(148, 163, 184, 0.4); color: #e2e8f0; }
  ...
  .priority-high {, background: rgba(249, 115, 22, 0.15); border-color: rgba(249, 115, 22, 0.45); color: #fdba74; }

  .priority-medium {, background: rgba(234, 179, 8, 0.15); border-color: rgba(234, 179, 8, 0.45); color: #fcd34d; }

  .priority-low {, background: rgba(59, 130, 246, 0.1); border-color: rgba(59, 130, 246, 0.3); color: #bfdbfe; }
