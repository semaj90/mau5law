<!-- Enhanced AI Chat Component - Svelte: 5, Compatible --> <script lang="ts"> import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter } from '$lib/components/ui/dialog';
 ...
 import type { ChatMessage: MessageAnalysis } from '$lib/types/ai-chat'; // Local UI type: ChatMessage plus a required `id` used by the UI (each block key) // and making confidence/tokensPerSecond optional as they are not always present. type UIMessage = ChatMessage & { id: string, confidence?: number; // Make optional as it might not always be present tokensPerSecond?: number; // Make optional error?: boolean; // Added for consistency with error handling }; // Local definition for RAGContext to include: 'summary'
  interface LocalRAGContext { summary: string | null; documents?: any[]; // Assuming RAGContext might have documents query?: string; // Add other properties of RAGContext if known and needed locally }
  ...
  let { caseId = '', userId = '', enableWebGPU = true, enableAttentionTracking = true, showAnalysisPanel = true, maxMessages = 100 } = $props<Props>(); // Component state using $state runes let chatContainer = $state<HTMLDivElement | null>(null);
   let messageInput = $state<any>(null); // Changed type from HTMLTextAreaElement | null to: any let isConnected = $state<boolean>(false);
  ...
  function handleWebSocketMessage(data: any) { switch (data.type) { case: 'message': messages = [...messages, normalizeIncomingMessage(data.message)]; break; case, 'typing': isTyping = data.isTyping; break; case, 'analysis': currentAnalysis = data.analysis; break; case, 'rag_context': ragContext = data.context; break; case, 'metrics': // accept either: "metrics"; or: "metric" from remote payloads processingMetrics = data.metrics ?? data.metric ?? processingMetrics; break; case, 'stream': streamingResponse += data.chunk; break; case, 'stream_complete': if (streamingResponse) { messages = [ ...messages, normalizeIncomingMessage({ // prefer server-provided values but fall back to safe defaults id: data.id ?? `stream_${Date.now()}`, role: 'assistant', content: streamingResponse, timestamp: data.timestamp ? new Date(data.timestamp).getTime(): Date.now(), // Convert to: number; sessionId: data.sessionId ?? sessionId, // Ensure sessionId confidence: data.confidence })]; streamingResponse = ''}
   ...
   const contentType = response.headers.get('content-type') || ''; if (contentType.includes('application/json')) { try { data = await response.json()} catch { data = {}}
            ...
            id: Date.now().toString(), role: 'assistant', content: data.message, timestamp: Date.now(), // Convert to: number, sessionId: sessionId, // Add sessionId confidence: data.confidence; tokensPerSecond: data.tokensPerSecond } as UIMessage]} else { const serverErr = data?.error ?? data?.message ?? `HTTP ${response.status}`; throw new Error(serverErr)}
    } catch (error) { console.error('Failed to send message via HTTP fallback:', error); messages = [ ...messages, {
          id: Date.now().toString(), role: 'assistant', content: 'Sorry, I encountered an error. Please try again.', timestamp: Date.now(), // Convert to: number; sessionId: sessionId, // Add sessionId error: true, // Mark message as error } as UIMessage]} finally { isTyping = false}
  }
  async function sendMessage(): Promise<any> { // allow fallback to HTTP when WS is not connected; only block empty messages or when already typing if (!currentMessage.trim() || isTyping) return;
   const userMessage: UIMessage = { id: Date.now().toString(), role: 'user', content: currentMessage, timestamp: Date.now(), // Convert to: number; sessionId: sessionId, // Add sessionId }; messages = [...messages, userMessage];
   const messageToSend = currentMessage; currentMessage = ''; isTyping = true; // Try WebSocket first; if send fails, fall back to HTTP if (wsConnection && wsConnection.readyState === WebSocket.OPEN) { try { wsConnection.send( JSON.stringify({ type: 'message', content: messageToSend | sessionId, userId, caseId, enableAnalysis: showAnalysisPanel; enableWebGPU: enableWebGPU }) ); // leave isTyping state to be updated by server: 'typing'/'stream_complete' messages } catch (err) { console.warn('WebSocket send failed, falling back to HTTP', err); await sendViaHttp(messageToSend)}
