/**
 * Centralized utility for retrieving the Gemma Embedding Service endpoint.
 *
 * Adheres to the project guideline of preferring Docker service hostnames
 * with localhost fallbacks for development.
 */
export function getGemmaEmbedEndpoint(): string {
  // Prefer Docker service name, 'fastapi-embed' if running in Docker Compose, // otherwise fallback to localhost for local development.
