...
const updateCasePoiRelationSchema = z.object({
  relationshipType: z.enum(['suspect', 'witness', 'victim', 'informant', 'other']).optional(), role: z.string().optional(), involvementLevel: z.enum(['primary', 'secondary', 'peripheral', 'unknown']).optional(), notes: z.string().optional()
});
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const [existingRelation] = await db
      ...
      .where(
        and(
          eq(casePoiRelations.id, relationId), eq(casePoiRelations.caseId, caseId), eq(casePoiRelations.isActive, true)
        )
      );
