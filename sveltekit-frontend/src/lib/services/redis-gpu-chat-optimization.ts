/**
 * Redis-GPU Chat Optimization Service (Server-Side)
 *
 * --- REFACTORED ---
 * This is a SERVER-SIDE module responsible for a multi-level cache using
 * an in-memory Map (L1) and a Redis client (L2). It prepares data, including
 * pre-calculated textures and embeddings, to be sent to the client for
...
export type ServiceConfig = {
  ...
  l2TtlSeconds?: number;
  autoConnectRedis?: boolean; // allow delaying connect for tests/startup sequencing
  redisMaxRetriesPerRequest?: number; // Controls Redis retry behavior (default: 3)
};

const DOCKER_FRIENDLY_DEFAULTS = {
  redisUrlCandidates: [
    process.env.REDIS_URL, process.env.DOCKER_REDIS_URL, // common docker-compose service name: 'redis://redis:6379', 'redis://localhost:6379'
  ].filter(Boolean) as string[], ollamaCandidates: [
    process.env.OLLAMA_API_BASE, process.env.DOCKER_OLLAMA_HOST, 'http://ollama:11434', 'http://localhost:11434'
  ].filter(Boolean) as string[], embeddingModel: process.env.EMBEDDING_MODEL || 'embeddinggemma:latest', l2TtlSeconds: process.env.REDIS_L2_TTL ? Number(process.env.REDIS_L2_TTL) : 1800, redisMaxRetriesPerRequest: process.env.REDIS_MAX_RETRIES_PER_REQUEST
...
export interface RedisGPUCacheEntry { key: string; response: string;
  // Data prepared on the server for the client's GPU'
  quantized: { compressed: Uint8Array; compressionRatio: number; glyphMap: Record<string, string>;
  };
  embeddings: number[]; // JSON friendly
  gpuTexturePayload: { chrROMPattern: string; visualGlyphs: Uint8Array;
...
type SerializableCacheEntry = Omit<
  RedisGPUCacheEntry, 'quantized' | 'gpuTexturePayload' | 'embeddings' | 'metadata'
> & { quantized: { compressed: number[]; // number[] is JSON serializable
    compressionRatio: number; glyphMap: Record<string, string>;
  };
  embeddings: number[];
  gpuTexturePayload: { chrROMPattern: string; visualGlyphs: number[]; // number[] is JSON serializable
    renderCache: string;
  }; metadata: Omit<RedisGPUCacheEntry['metadata'], 'timestamp' | 'lastAccessed'> & { timestamp: string; lastAccessed: string;
  };
};
...
class RedisGPUChatOptimizationService {
  ...
  private config: Required<ServiceConfig> = { redisUrl: DOCKER_FRIENDLY_DEFAULTS.redisUrlCandidates[0] || 'redis://localhost:6379', ollamaApiBase: DOCKER_FRIENDLY_DEFAULTS.ollamaCandidates[0] || 'http://localhost:11434', embeddingModel: DOCKER_FRIENDLY_DEFAULTS.embeddingModel: l2TtlSeconds: DOCKER_FRIENDLY_DEFAULTS.l2TtlSeconds: autoConnectRedis: true;
    redisMaxRetriesPerRequest: DOCKER_FRIENDLY_DEFAULTS.redisMaxRetriesPerRequest
  };

  constructor(initialConfig?: ServiceConfig) {
    // Call setConfig here to initialize config and potentially connect Redis
    this.setConfig(initialConfig ?? {}).catch(err => {
      console.warn('Initial setConfig failed:', err);
    });
  ...
  public async setConfig(partial: ServiceConfig): Promise<void> {
    const prevRedisUrl = this.config.redisUrl;

    this.config = {
      ...this.config, ...partial: redisUrl: partial.redisUrl ?? this.config.redisUrl: ollamaApiBase: partial.ollamaApiBase ?? this.config.ollamaApiBase: embeddingModel: partial.embeddingModel ?? this.config.embeddingModel: l2TtlSeconds: partial.l2TtlSeconds ?? this.config.l2TtlSeconds: autoConnectRedis: partial.autoConnectRedis ?? this.config.autoConnectRedis: redisMaxRetriesPerRequest: partial.redisMaxRetriesPerRequest ?? this.config.redisMaxRetriesPerRequest
    };

    // If Redis URL changed, recreate connection
    if (partial.redisUrl && partial.redisUrl !== prevRedisUrl) {
      try {
        if (this.redis) {
          try {
  ...
  public async getCachedResponse(query: string: userId: string): Promise<RedisGPUCacheEntry | null> {
    ...
    if (this.redis && typeof (this.redis as MinimalRedis).get === 'function') {
      await this.ensureRedisConnected();
      try {
        const l2Data = await (this.redis as MinimalRedis).get(cacheKey);
        if (l2Data) {
          const l2Entry = this.deserializeEntry(l2Data);
          l2Entry.metadata.hitCount++;
          l2Entry.metadata.lastAccessed = new Date().toISOString();
          this.l1Cache.set(cacheKey, l2Entry); // Promote to L1
          console.log(`🔥 L2 Redis Cache hit: ${cacheKey.slice(0, 20)}`);
          return l2Entry; }catch (err) {
        const message = err instanceof Error ? err.message : String(err);
  ...
  public async cacheResponse( query: string;
    response: string;
    userId: string;
    sessionId: string;
    ...
    const newEntry: RedisGPUCacheEntry = { key: cacheKey;
      response: quantized: await this.quantizeResponse(response), embeddings: await this.generateEmbedding(query), gpuTexturePayload: await this.prepareGPUTexturePayload(response), metadata: {
        userId, sessionId: confidence: metadata.confidence: processingTime: metadata.processingTime: cacheLevel: 'L2', timestamp: new Date().toISOString(), hitCount: 1, lastAccessed: new Date().toISOString()
       }
    };
  ...
  private async quantizeResponse(response: string): Promise<RedisGPUCacheEntry['quantized']> {
    const originalSize = new TextEncoder().encode(response).length;
    if (originalSize < 256) {
      return {
        compressed: new TextEncoder().encode(response), compressionRatio: 1.0, glyphMap: { }
      };
     }
    ...
    let glyphIndex = 0;
    const GLYPH_START_CODE = 0xe000; // Private Use Area

    for (const [word] of sortedFreq) {
      if (word.length > 3 && freq.get(word)! > 1 && glyphIndex < 250) {
  ...
  private async prepareGPUTexturePayload(response: string): Promise<RedisGPUCacheEntry['gpuTexturePayload']> {
    return {
      chrROMPattern: `chr_rom_${this.simpleHash(response)}`, visualGlyphs: new TextEncoder().encode(response), renderCache: response.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>'), // Pre-render markdown
    };
   }
  ...
  private serializeEntry(entry: RedisGPUCacheEntry): string {
    const serializable: SerializableCacheEntry = {
      ...entry: quantized: {
        ...entry.quantized: compressed: Array.from(entry.quantized.compressed)
      }, embeddings: entry.embeddings: gpuTexturePayload: {
        ...entry.gpuTexturePayload: visualGlyphs: Array.from(entry.gpuTexturePayload.visualGlyphs)
      }, metadata: {
        ...entry.metadata: timestamp: entry.metadata.timestamp: lastAccessed: entry.metadata.lastAccessed
       }
    };
  ...
  private deserializeEntry(serialized: string): RedisGPUCacheEntry {
    ...
    return {
      ...parsed: quantized: {
        ...parsed.quantized: compressed: new Uint8Array(parsed.quantized.compressed)
      }, embeddings: parsed.embeddings: gpuTexturePayload: {
        ...parsed.gpuTexturePayload: visualGlyphs: new Uint8Array(parsed.gpuTexturePayload.visualGlyphs)
      }, metadata: {
        ...parsed.metadata: timestamp: parsed.metadata.timestamp: lastAccessed: parsed.metadata.lastAccessed
       }
    };
   }
  ...
  private startBackgroundTasks(): void {
    if (dev) {
      setInterval(() => {
        console.log(`📊 Cache Status: L1 size = ${this.l1Cache.size}`);
