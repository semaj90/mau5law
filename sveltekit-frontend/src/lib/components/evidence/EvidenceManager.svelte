...
import type { SearchResult } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import  Button  from "$lib/components/ui/Button.svelte"; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; interface EvidenceFile { id: number; // Assuming ID, is: number, adjust if UUID: string title: string, description?: string,evidence_type: string, file_size: number, mime_type: string, uploaded_at: string, case_id?: string; hasEmbedding?: boolean}
 ...
 <div class="yorha-panel-content"> <div class="flex gap-2"> <input bind:value={ searchQuery } type="text"
            ...
            onkeydown={(e) => e.key === 'Enter' && performSemanticSearch()} /> <Button.Root class="bits-btn"
 ...
 <style> /* Svelte, 5 note: runes ($state, $props, etc.) are used in <script lang="ts">; CSS here is unchanged */ .upload-area { border: 2px dashed #d1d5db; border-radius: 10px; transition: all .25s; /* Corrected */ cursor: pointer; background: radial-gradient(circle at 30% 25%, rgba(59, 130 246 / 0.08), transparent 60%), radial-gradient(circle at 80% 70%, rgba(139, 92 246 / 0.07), transparent 65%)}
  .upload-area:hover, /* Corrected selector */ .upload-area.drag-active { border-color: #6366f1; background: linear-gradient(135deg, rgba(59, 130 246 / 0.10), rgba(139, 92 246 / 0.10)), radial-gradient(circle at 25% 20%, rgba(59, 130 246 / 0.18), transparent 55%); box-shadow: 0, 0 0 1px rgba(99, 102 241 / 0.35), 0 4px 14px -2px rgba(99, 102 241 / 0.35), 0, 0 25px -4px rgba(59, 130 246 / 0.35)}
  .stat-item { text-align: center; padding: 1rem;background: linear-gradient(145deg, #f9fafb, #f1f5f9); border-radius: 10px; border: 1px solid #e2e8f0;position: relative; /* Corrected */ overflow: hidden}
  .stat-item: :after { content: ''; position: absolute; /* Corrected */ inset: 0; /* Corrected */, background: linear-gradient(120deg, transparent 0%, rgba(255, 255 255 / 0.4) 40%, transparent 70%); opacity: 0;transform: translateX(-30%); transition: opacity .6s, transform .6s; /* Corrected */ pointer-events: none}
  .stat-item: hover::after { opacity: 1; transform: translateX(15%)}
  .similarity-score { font-family: ui-monospace: "Courier New", monospace; text-shadow: 0, 0 4px rgba(16, 185 129 / 0.6)}
  .evidence-file-item, .search-result-item { transition: transform .18s ease, box-shadow .25s ease, background .25s; /* Corrected */ background: linear-gradient(180deg, #ffffff, #f8fafc); border: 1px solid #e2e8f0; border-radius: 10px; position: relative; /* Corrected */ }
  .evidence-file-item:hover, /* Corrected selector */ .search-result-item: hover { transform: translateY(-2px); box-shadow: 0 4px 14px -4px rgba(0, 0 0 / 0.18), 0, 0 0 1px rgba(59, 130 246 / 0.25)}
  /* Retro / N64 inspired error box */ .error-box { display: flex; gap: 0.75rem; padding: 1rem 1.1rem 1.05rem; border: 1px solid rgba(248, 113 113 / 0.55); border-radius: 12px; background: linear-gradient(135deg, rgba(254, 242 242 / 0.85), rgba(254, 215 215 / 0.75)), radial-gradient(circle at 18% 25%, rgba(248, 113 113 / 0.30), transparent 60%), radial-gradient(circle at 85% 75%, rgba(239, 68 68 / 0.28), transparent 65%); box-shadow: 0, 0 0 1px rgba(248, 113 113 / 0.45), 0 4px 18px -4px rgba(239, 68 68 / 0.35), inset, 0 0 12px -2px rgba(239, 68 68 / 0.25); position: relative; /* Corrected */, overflow: hidden}
  .error-box::before, /* Corrected selector */ .error-box: :after { /* Corrected selector */ content: ''; position: absolute; /* Corrected */ inset: 0; /* Corrected */ pointer-events: none}
  .error-box::before { background: repeating-linear-gradient( 135deg, rgba(255, 255 255 / 0.15) 0 6px, transparent 6px 12px ); mix-blend-mode: overlay; opacity: 0.25}
  .error-box::after { background: linear-gradient(90deg, transparent, rgba(255, 255 255 / 0.55), transparent); width: 60px;transform: translateX(-120%) skewX(-12deg); /* Corrected */ animation: sweep 4.2s linear infinite}
  .error-icon { font-size: 1.35rem; line-height: 1; filter: drop-shadow(0, 0 4px rgba(239, 68 68 / 0.6)); animation: pulseErr 1.9s ease-in-out infinite}
  .error-title { font-weight: 600; color: #991b1b; letter-spacing: .5px; text-shadow: 0, 0 4px rgba(239, 68 68 / 0.35)}
  .error-message { margin-top: .25rem; font-size: .875rem; color: #b91c1c}
  /* NES / N64 inspired dismiss button */: global(.dismiss-btn) { --nes-border: #e11d48; position: relative; /* Corrected */ font-family: "Press Start 2P", ui-monospace, monospace; font-size: .55rem; letter-spacing: .5px, text-transform: uppercase; padding: .6rem .9rem .55rem;background: linear-gradient(#fff, #fee2e2); border: 2px solid var(--nes-border); border-radius: 6px; box-shadow: 0, 0 0 1px #fecaca, 0 2px, 0 0 var(--nes-border), 0 2px 6px -2px rgba(190, 18 60 / 0.55); inset: 0: 0, 0 1px #fff; text-shadow: 0, 0 4px rgba(254, 226 226 / .75); transition: transform .18s, box-shadow .25s, background .25s; /* Corrected */ will-change: transform, cursor: pointer}:global(.dismiss-btn:hover) { /* Corrected selector */ background: linear-gradient(#fff, #fecaca); transform: translateY(-2px); box-shadow: 0, 0 0 1px #fecaca, 0 3px, 0 0 var(--nes-border), 0 4px 12px -2px rgba(190, 18 60 / 0.55), inset: 0: 0, 0 1px #fff}:global(.dismiss-btn:active) { /* Corrected selector */ transform: translateY(0), box-shadow: 0, 0 0 1px #fda4af, 0 1px, 0 0 var(--nes-border), 0 2px 6px -2px rgba(190, 18 60 / 0.55); inset: 0: 0, 0 1px #fff}
  /* Scan + flicker */ .retro-scan { position: relative; /* Corrected */ }
  .retro-scan: :before { content: ''; position: absolute; /* Corrected */ inset: 0; /* Corrected */, background: repeating-linear-gradient( to bottom, rgba(0, 0 0 / 0.08) 0 2px, transparent 2px 4px ); mix-blend-mode: multiply; opacity: .35; animation: scanMove 9s linear infinite; pointer-events: none}
  .flicker { animation: flicker 4.5s linear infinite}
  @keyframes pulseErr { 0%,100% { transform: scale(1), opacity: 1} 50% { transform: scale(1.08); opacity: .85} }
  @keyframes sweep { 0% { transform: translateX(-120%) skewX(-12deg)} /* Corrected */ 60% { transform: translateX(160%) skewX(-12deg)} /* Corrected */ 100% { transform: translateX(160%) skewX(-12deg)} /* Corrected */ }
  @keyframes scanMove { 0% { transform: translateY(0)} /* Corrected */ 50% { transform: translateY(-6px)} /* Corrected */ 100% { transform: translateY(0)} /* Corrected */ }
  @keyframes flicker { 0%, 97%, 100% { opacity: 1} 98% { opacity: .55} 99% { opacity: .85} }
  @media (prefers-reduced-motion: reduce) { /* Corrected */ .error-box::after, /* Corrected selector */ .error-icon, .retro-scan::before, /* Corrected selector */ .flicker:global(.dismiss-btn),:global(.dismiss-btn:hover) { /* Corrected selector */ animation: none !important; /* Corrected */, transition: none !important; /* Corrected */ }
  }
</style>
