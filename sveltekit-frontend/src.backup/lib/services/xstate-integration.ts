  ...
  getGlobalState(): Snapshot<LegalAIContext> {
    try {
      ...
      return {
        ...
        context: {
          user: { id: null, email: null, role: null, permissions: [], isAuthenticated: false },
          cases: { items: [], currentCase: null, filters: { search: '', status: 'all', priority: 'all', category: 'all' }, pagination: { page: 1, limit: 10, total: 0 }, loading: false, error: 'XState unavailable' },
          ai: { isProcessing: false, currentQuery: '', lastResponse: null, error: 'XState unavailable', models: { primary: 'gemma3-legal', embedding: 'nomic-embed-text', available: [] } },
          system: { connected: false, services: { database: false, redis: false, ollama: false, gpu: false, pgvector: false, qdrant: false, neo4j: false }, metrics: { errorCount: 1, performanceScore: 0, uptime: 0 } },
        },
        status: 'active' as const,
        output: undefined,
        error: undefined,
  ...
  sendEvent(machineId: string, event: LegalAIEvent): void {
    try {
      ...
      if (machineId === 'legalAI' && legalAIActor && typeof legalAIActor.send === 'function') {
        ...
        console.warn(`[XState] Cannot send event: actor not available or unknown machine ID: ${machineId}`);
      }
    } catch (error) {
