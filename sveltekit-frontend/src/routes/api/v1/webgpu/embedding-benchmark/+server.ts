...
async function runSingleDocumentBenchmark(config: BenchmarkConfig): Promise<BenchmarkResult> {
  ...
  return {
    ...
    throughput: safeDivide(documents.length, totalTime / 1000),
    cacheHitRatio: documents.length ? cacheHits / documents.length : 0,
...
async function runBatchProcessingBenchmark(config: BenchmarkConfig): Promise<BenchmarkResult> {
  ...
  return {
    ...
    totalDocuments,
    processingTime: totalTime,
    avgTimePerDocument: safeDivide(totalTime, totalDocuments),
    throughput: safeDivide(totalDocuments, totalTime / 1000),
    cacheHitRatio: totalDocuments ? cacheHits / totalDocuments : 0,
...
async function runStressTestBenchmark(config: BenchmarkConfig): Promise<BenchmarkResult> {
  ...
  return {
    ...
    avgTimePerDocument: completedDocs ? actualDuration / completedDocs : 0,
    throughput: safeDivide(completedDocs, actualDuration / 1000),
...
async function runComparisonBenchmark(config: BenchmarkConfig): Promise<BenchmarkResult> {
  ...
  return {
    mode: 'comparison',
    totalDocuments: totalDocs, // Both runs
    processingTime: totalTime,
    avgTimePerDocument: safeDivide(totalTime, totalDocs),
    throughput: safeDivide(totalDocs, totalTime / 1000),
