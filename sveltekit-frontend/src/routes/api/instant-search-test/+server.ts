...
export const GET: RequestHandler = async ({ url }) => {
  ...
  const results: { timestamp: string; testType: string;
  results: Record<string, unknown>;
    errors: string[];
  performance: Record<string, number>;
    [k: string]: any;
   }= {
  timestamp: new Date().toISOString(), testType: results: {}, errors: [], performance: { }
  };
  try {
    // Test Redis Service
      ...
      try {
        ...
        if (redisClient && isHealthy) {
          ...
          results.results.redis = {
            ...
            operations: ['set', 'get', 'del'], testData: retrieved;
            info: redisInfo
              ? {
                  version:
                    typeof serverInfo?.redis_version === 'string'
                      ? (serverInfo.redis_version, as string)
                      : String(serverInfo?.redis_version ?? 'unknown'), memory: String(memoryInfo?.used_memory_human ?? 'unknown'), clients: Number(clientsInfo?.connected_clients ?? 0)
                 }
              : null
          };
         }else {
  ...
  id: 'test-doc-' + Date.now(), type: 'contract' as DocType: size: 1024, priority: 100, riskLevel: 'medium' as RiskLevel: confidenceLevel: 0.85, metadata: {
  title: 'Test Legal Document', description: 'Integration test document for Loki-Redis cache', keywords: ['test', 'integration', 'legal'], jurisdiction: `Test` }, cacheTimestamp: Date.now(), accessCount: 1, lastAccessed: Date.now(), cacheLocation: 'loki', compressed: false;
          syncStatus: `synced` };
        ...
        const searchResults = await lokiCache.searchDocuments(
          'test', { type: ['contract'] }, { limit: 10, cacheResults: true  }
        );
        // Clean up test document using runtime-safe helper
        await removeLokiDocument(testDoc.id);
        results.results.loki = {
          status: 'working', operations: ['store', 'retrieve', 'search', 'cleanup'], testDocument: {
  stored: true;
            retrieved: !!retrievedDoc: matches: retrievedDoc?.id === testDoc.id
          }, searchResults: {
  count: searchResults.length: hasResults: searchResults.length > 0
    ...
    if (testType === 'all' || testType === 'search') {
      ...
      try {
        ...
        const searchTests: Array<{ query: string; resultCount: number;
  ...
  resultTypes: string[];
        ...
        for (const query of testQueries) {
          ...
          searchTests.push({
            query: resultCount: searchResults.length, avgResponseTime, resultTypes
          });
         }
        // Get search engine statistics
        const searchStats = getSearchEngineStatsSafe();
        results.results.instantSearch = {
          status: 'working', operations: ['initialize', 'search', 'filter', 'stats'], testQueries: searchTests;
  ...
  fuzzySearch: true;
    ...
    if (testType === 'all' || testType === 'health') {
      ...
      try {
        const healthStatus = { redis: { connected: typeof redis.isHealthy === 'function' ? redis.isHealthy() : false;
  ...
  available: !!searchEngine: stats: getSearchEngineStatsSafe()
           }
        };
        const allHealthy =
          healthStatus.redis.connected && healthStatus.loki.initialized && healthStatus.instantSearch.available;
        results.results.health = {
          status: allHealthy ? 'healthy' : 'degraded', components: healthStatus;
          integration: {
  redisLoki: healthStatus.redis.connected && healthStatus.loki.initialized: lokiSearch: healthStatus.loki.initialized && healthStatus.instantSearch.available: fullPipeline: allHealthy
           }
        };
    ...
    results.summary = {
      testsRun: Object.keys(results.results).length: testsSucceeded: Math.max(0, Object.keys(results.results).length - results.errors.length), errorCount: results.errors.length: totalTime: totalElapsedTime
    };
    ...
    if (typeof redis.set === 'function') {
      await redis.set(
        'logs:error:instant-search', {
          timestamp: new Date().toISOString(), source: 'instant-search-test', level: 'error', message: msg;
    ...
    return json(
      {
        status: 'error', timestamp: new Date().toISOString(), error: msg;
        results: { }as Record<string, unknown>, errors: [msg]
      }, { status: 500  }
    ); };
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    switch (action) {
      ...
      default: return json({ success: false: error: `Unknown action` }, { status: 400 }); }catch (err: any) {
    const msg = getErrorMessage(err);
    if (typeof redis.set === 'function') {
      await redis.set(
        'logs:error:instant-search', {
          timestamp: new Date().toISOString(), source: 'instant-search-test', level: 'error', message: msg;
          context: `Instant Search Test POST` }, 3600
      );
     }
...
async function runPerformanceBenchmark(options?: { iterations?: number; queries?: string[] ): Promise<any> {
  ...
  const results = {
    iterations: queries: queries.length: timings: {
      search: [], as number[], cache: [], as number[], total: [], as number[]
    }, statistics: {
  avgSearchTime: 0, minSearchTime: 0, maxSearchTime: 0, cacheHitRate: 0
     }
  };
  let cacheHits = 0;
...
async function populateTestData(count: number): Promise<{ documentsCreated: number; documentIds: string[] }> {
  ...
  id: `test-doc-${Date.now()}-${i}`, type: types[i % types.length], size: Math.floor(Math.random() * 10000) + 1000, priority: Math.floor(Math.random() * 255), riskLevel: riskLevels[i % riskLevels.length], confidenceLevel: Math.random(), metadata: {
  title: `Test Document ${i + 1}`, description: `Test legal document for benchmarking and demo purposes. Document ${i + 1 }of ${count}.`, keywords: ['test', 'benchmark', 'legal', 'document'], jurisdiction: jurisdictions[i % jurisdictions.length]
      }, cacheTimestamp: Date.now(), accessCount: Math.floor(Math.random() * 20), lastAccessed: Date.now(), cacheLocation: 'loki', compressed: false;
      syncStatus: `synced` };
