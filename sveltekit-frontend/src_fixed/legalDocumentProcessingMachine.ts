...
export type LegalDocumentEvent =
  ...
  | { type: 'MCP_ANALYSIS_COMPLETE'; mcpAnalysis: unknown; recommendations: string[] }
          ...
          states: {
            idle: {
              always: [
                {
                  target: 'processing',
                  guard: ({ context }) => context.options.extractEntities
                },
                { target: 'skipped' }
              ]
            },
          ...
          states: {
            idle: {
              always: [
                {
                  target: 'processing',
                  guard: ({ context }) => context.options.generateSummary
                },
                { target: 'skipped' }
              ]
            },
          ...
          states: {
            idle: {
              always: [
                {
                  target: 'processing',
                  guard: ({ context }) => context.options.generateEmbedding
                },
                { target: 'skipped' }
              ]
            },
          ...
          states: {
            idle: {
              always: [
                {
                  target: 'processing',
                  guard: ({ context }) => context.options.assessRisk
                },
                { target: 'skipped' }
              ]
            },
          ...
          states: {
            idle: {
              always: [
                {
                  target: 'processing',
                  guard: ({ context }) => context.options.useContext7
                },
                { target: 'skipped' }
              ]
            },
      ...
      invoke: {
        ...
        onDone: {
          ...
          actions: assign({
            ...
            processingDuration: ({ context }) => context.startTime ? Date.now() - context.startTime : 0
          })
        },
        onError: {
          target: 'error',
...
export const selectors = {
  isProcessing: (state: any) => state.matches('extractingContent') || state.matches('analyzing') || state.matches('storing'),
