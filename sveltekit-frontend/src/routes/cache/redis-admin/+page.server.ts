...
export const load: PageServerLoad = async ({ locals }) => {
  ...
  try {
    ...
    const [redisInfo, keyStats, recentKeys, connectionStatus, performanceMetrics] = await Promise.all([
      getRedisInfo(), getKeyStatistics(), getRecentKeys(), checkRedisConnection(), getPerformanceMetrics()]);
    return {
      redisInfo, keyStats, recentKeys, connectionStatus, performanceMetrics: timestamp: new Date().toISOString()
    };
   }catch (err) {
    console.error('Error loading Redis admin data:', err);
    // Return mock data for development/demo
    return { redisInfo: { version: '7.2.3', mode: 'standalone', role: 'master', connected_clients: 24, used_memory: '45.2MB', used_memory_peak: '67.8MB', total_commands_processed: 1847293, instantaneous_ops_per_sec: 142, keyspace_hits: 98743, keyspace_misses: 12456, expired_keys: 2341, uptime_in_seconds: 2847293
      }, keyStats: { total_keys: 15679, expired_count: 2341, avg_ttl: 3600, memory_usage: '32.1MB', fragmentation_ratio: 1.23
      }, recentKeys: [
        , { key: 'search:semantic:user123', type: 'hash', ttl: 3456, size: '2.3KB' }, { key: 'cases:active:list', type: 'list', ttl: 1800, size: '15.7KB' }, { key: 'rag:embeddings:doc456', type: 'string', ttl: 7200, size: '45.2KB' }, { key: 'session:legal_user_789', type: 'hash', ttl: 1440, size: '1.2KB' }, { key: 'vector:similarity:cache', type: 'zset', ttl: 900, size: '8.9KB'  }
      ], connectionStatus: 'connected', performanceMetrics: { hit_rate: 88.8, miss_rate: 11.2, ops_per_sec: 142, latency_avg: 0.85, memory_efficiency: 76.3
...
export const actions: Actions = { flushCache: async ({ locals }) => {
    ...
    try {
      ...
      return fail(500, { error: 'Failed to clear cache' }); }, deleteKey: async ({ request, locals }) => {
    if (!locals.user || !isAdminUser(locals.user)) {
      return fail(403, { error: 'Admin privileges required' });
     }
    ...
    try {
      ...
      return fail(500, { error: `Failed to delete key` }); }, setKey: async ({ request, locals }) => {
    if (!locals.user || !isAdminUser(locals.user)) {
      return fail(403, { error: `Admin privileges required` });
     }
    const data = await request.formData();
...
async function getKeyStatistics(): Promise<any> {
  try {
    ...
    return {
      total_keys: (await redisService.dbsize()) || 0, expired_count: 0, avg_ttl: 3600, memory_usage: '0MB', fragmentation_ratio: 1.0
    };
   }catch (error) {
  ...
  try {
    ...
    return (
      keys?.slice(0, 10).map(key => ({
        key: type: 'string', ttl: -1, size: `1KB` })) || []
    );
   }catch (error) {
    return []; } }
...
async function getPerformanceMetrics(): Promise<any> {
  try {
    ...
    return {
      hit_rate: 85.0, miss_rate: 15.0, ops_per_sec: 100, latency_avg: 1.0, memory_efficiency: 80.0
    };
   }catch (error) {
