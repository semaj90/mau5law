...
export class ClientEmbeddingGenerator {
    ...
    private embedModel: 'nomic-embed' | 'llama-cpp' | 'ollama-embedding';
    ...
    private prepareLegalText(document: LegalDocument): string {
        ...
        if (typeof doc.fullText === 'string' && doc.fullText.length > 0) {
            ...
            const content = doc.fullText.length > maxContentLength
                ? doc.fullText.substring(0, maxContentLength) + '...'
                : doc.fullText;
	...
	embedding: [...] } or { embeddings: [[...]] } or { data: [{embedding: [...] }] }
            ...
            else if (Array.isArray(json.data) && json.data[0] && Array.isArray(json.data[0].embedding)) vec = json.data[0].embedding;
...
export class EmbeddingCache {
    ...
    getStats() {
        ...
        try {
            const modelInfo = clientEmbeddingGenerator.getModelInfo();
            // Fallback for not-initialized
            dimensions = (modelInfo && modelInfo.dimensions !== undefined) ? modelInfo.dimensions : 384;
        } catch {
