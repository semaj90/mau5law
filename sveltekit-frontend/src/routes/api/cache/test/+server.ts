...
import {
  cacheService, getCachedEmbedding, setCachedEmbedding, getCachedSearchResults, cacheSearchResults
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (action === 'info') {
      ...
      return json({
        success: true;
        action: 'cache_info', data: cacheInfo;
    ...
    if (action === 'test') {
      ...
      const testData = {
  message: 'Hello Redis!', numbers: [1, 2, 3, 4, 5], nested: { a: 1, b: 2, c: { deep: 'value' }  } }
      };
      // Set test data
      ...
      return json({
        success: true;
        action: 'basic_test', data: {
          testPassed: original: testData;
          retrieved: cacheInfo: await cacheService.getCacheInfo()
        }, timestamp: new Date().toISOString()
      });
     }
    return json({
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (action === 'embedding') {
      ...
      return json({
        success: true;
        action: 'embedding_test', data: {
          testPassed: original: embedding;
    ...
    if (action === 'search') {
      ...
      return json({
        success: true;
        action: 'search_test', data: {
          testPassed: original: results;
    ...
    if (action === 'large_payload') {
      ...
      type LargeItem = { id: number; data: string;
      ...
      const largeArray: LargeItem[] = Array.from({ length: 1000 }, (_, i) => ({
        id: i;
        data: `This is test data item ${i}`, metadata: {
  created: new Date().toISOString(), tags: ['test', 'large', 'payload'], values: Array.from({ length: 10 }, () => Math.random())
         }
      }));
      ...
      return json({
        success: true;
        action: 'large_payload_test', data: {
  arraySize: largeArray.length: compressedTest: compressedPassed;
          uncompressedTest: uncompressedPassed;
          processingTime: endTime - startTime: compressionBenefit: `Large arrays benefit significantly from gzip compression'  }`
      });
     }
    return json({
      success: false;
