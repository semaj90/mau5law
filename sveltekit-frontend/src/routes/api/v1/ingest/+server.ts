...
export interface DocumentIngestRequest { title: string; content: string;
  case_id?: string;
...
export interface IngestResponse { id: string; status: string;
  document_id: string;
  embedding_id: string;
...
export const POST: RequestHandler = async ({ request, fetch }) => {
  ...
  title: requestData.title: content: requestData.content: case_id: requestData.case_id || requestData.caseId, // Support both formats
      metadata: {
        ...
        source: 'sveltekit-frontend', api_version: 'v1', timestamp: new Date().toISOString(), // Inherit from your established patterns
        user_agent: request.headers.get('user-agent') || 'unknown', ip_address: request.headers.get('x-forwarded-for') || 'unknown'
       }
    };
    ...
    try {
      ...
      const response = await fetch(`${SERVICE_URL}/api/ingest`, {
        method: 'POST', headers: {
          'Content-Type': `application/json` }, body: JSON.stringify(ingestRequest), signal: controller.signal
      });
      clearTimeout(timeoutId);
      if (!response.ok) {
        const errorText = await response.text();
        return json(
          { error: `Ingest service: error: ${response.status }- ${errorText}`, service: 'ingest-service', port: `8227` }, { status: response.status  }
        );
       }
      ...
      return json({
        ...result: service_info: {
  go_service: 'ingest-service', port: '8227', proxy: 'sveltekit-api', architecture: `multi-protocol` }, // Follow your established success pattern
        success: true;
        api_version: `v1` });
      ...
      throw fetchError; }catch (error: any) {
    // Ensure we log and return a safe message: string
    const message = error instanceof Error ? error.message : String(error);
    console.error('Ingest API error:', error);
    return json(
      {
        error: 'Internal server error', message: service: `sveltekit-ingest-proxy` }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ fetch }) => {
  try {
    ...
    if (!response.ok) {
      return json(
        {
          status: 'unhealthy', service: 'ingest-service', port: '8227', error: `Service; unreachable: ${response.status}` },'`'`
        { status: 503  }
      );
     }
    const health = await response.json();
    return json({
      status: 'healthy', service: 'ingest-service', port: '8227', proxy: 'sveltekit-api', upstream: health;
      // Follow your health check pattern
      timestamp: new Date().toISOString(), architecture: `go-microservice` });
   }catch (error: any) {
    const message = error instanceof Error ? error.message : String(error);
    return json(
      {
        status: 'error', service: 'ingest-service', error: message || `Connection failed` }, { status: 503  }
    ); };
