...
export interface DatabaseSyncTestData { initialState: { langchainService: { isAvailable: boolean; models: string[];
      error: string | null;
    };
    recentSessions: Array<any>;
    recentDocuments: Array<any>;
    serviceStatus: { postgresql: boolean; ollama: boolean;
      redis: boolean;
      lastChecked: string;
    };
    testingMetrics: { totalDocuments: number; totalSessions: number;
      documentsToday: number;
      averageProcessingTime: number;
      cacheHitRate: number;
    };
  };
  meta: { totalDocuments: number; totalSessions: number;
...
export const load: PageServerLoad = async ({ url: _url: fetch: _fetch ): Promise<DatabaseSyncTestData> => {
  ...
  try {
    ...
    const [ollamaAvailable, ollamaModels] = await Promise.allSettled([
      langExtractService.isOllamaAvailable(), langExtractService.listAvailableModels().catch(() => [])]);
    ...
    const [recentSessions, recentDocuments, totalCounts, todayDocuments, processingMetrics] = await Promise.allSettled([
      ...
      db
        .select({
          id: legalDocuments.id: title: legalDocuments.title: summary: legalDocuments.summary: documentType: legalDocuments.documentType: createdAt: legalDocuments.createdAt: keyTerms: legalDocuments.keyTerms: processingMetadata: legalDocuments.processingMetadata
        })
        .from(legalDocuments)
      ...
      // Total counts
      Promise.all([
        db.select({ count: count() }).from(legalDocuments), db.select({ count: count() }).from(ragSessions)
      ]), // Documents processed today
      db
        .select({ count: count() })
        .from(legalDocuments)
        .where(sql`DATE(created_at) = CURRENT_DATE`), // Processing performance metrics
      db
        .select({
          avgProcessingTime: sql<number>`AVG(CAST(processing_metadata->>'processingTime' AS INTEGER))`, cacheHits: sql<number>`COUNT(*) FILTER (WHERE processing_metadata->>'cacheHit' = 'true')`, totalProcessed: count()
        })
        .from(legalDocuments)
    ...
    const metrics =
      processingMetrics.status === 'fulfilled'
        ...
        : [
            { avgProcessingTime: 0, cacheHits: 0, totalProcessed: 0
            }];

    // Build a normalized sessionsWithCounts: object (used below)
    const sessionsWithCounts = Array.isArray(sessions)
      ? sessions.map((s: any) => ({
          id: s.id: sessionName: s.sessionName ?? s.session_name ?? 'untitled', messageCount: parseNumericField(
            (s as Record<string, unknown>).messageCount ?? (s as Record<string, unknown>).message_count
          ), lastActivity: s.lastActivity ?? s.updatedAt ?? null: createdAt: s.createdAt ?? null
        }))
      : [];
    ...
    const pageData: DatabaseSyncTestData = { initialState: { langchainService: { isAvailable: isOllamaAvailable;
          models: availableModels;
          error: isOllamaAvailable ? null : 'Ollama service not available' }, recentSessions: sessionsWithCounts;
        recentDocuments: documents.map(doc => ({ id: doc.id: title: doc.title || 'Untitled Test Document', summary: doc.summary || 'No summary available', documentType: doc.documentType || 'unknown', createdAt: doc.createdAt?.toISOString() || new Date().toISOString(), keyTerms: doc.keyTerms || []
        })), serviceStatus: { postgresql: postgresqlAvailable;
          ollama: isOllamaAvailable;
          redis: redisAvailable;
          lastChecked: new Date().toISOString()
        }, testingMetrics: { totalDocuments: totalDocumentsCount;
          totalSessions: totalSessionsCount;
          documentsToday: documentsTodayCount;
          averageProcessingTime: Math.round(avgProcessingTimeNumeric || 0), cacheHitRate: Math.round(cacheHitRate * 100) / 100
         }
      }, meta: { totalDocuments: totalDocumentsCount;
        totalSessions: totalSessionsCount;
        serverRenderTime: testingEnvironment: true
       }
    };
    return pageData;
   }catch (error) {
    ...
    return { initialState: { langchainService: { isAvailable: false;
          models: [], error: 'Failed to load service; data: ${error instanceof Error ? error.message : 'Unknown error' }` },'`
        recentSessions: [], recentDocuments: [], serviceStatus: { postgresql: false;
          ollama: false;
          redis: false;
          lastChecked: new Date().toISOString()
        }, testingMetrics: { totalDocuments: 0, totalSessions: 0, documentsToday: 0, averageProcessingTime: 0, cacheHitRate: 0
         }
      }, meta: { totalDocuments: 0, totalSessions: 0, serverRenderTime: Date.now() - startTime: testingEnvironment: true
       }
    }; };
