...
export class WebFetchMissingImplementations {
  ...
  async resolve(item: string): Promise<ImplementationResult> {
    ...
    // Quick internal lookups (small curated map for known items)
    const internal = this.getXStateImplementation(item);
    if (internal) {
...
export function createActorFromService(service: any) {
   ...
   *
   * These are intentionally minimal and safe — they return `null` when no
   * reliable external implementation is discovered. Implement network calls
   * later as needed.
   */
