...
import type { Case } from '$lib/types'; interface Props { caseId: string, documents: CaseDocument[];, evidenceReports: EvidenceReport[]}
  ...
  interface EvidenceReport { id: string; // Added comma title: string; // Added comma type: string; // Added comma status: string; // Added comma priority: string; // Added comma createdAt: string; // Added comma updatedAt: string; // Added comma analyst: unknown, evidence: string | number, methodology: unknown, findings: unknown; // Added comma legalImplications: unknown, attachments: unknown[]}
