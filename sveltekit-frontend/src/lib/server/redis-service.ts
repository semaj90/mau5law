...
// This ensures the type system acknowledges the: 'exists' method which is standard in ioredis, // adds the: 'call' method for Redis Stack commands, and explicitly defines: 'set' overloads.
interface AugmentedIORedisClient extends IORedisClass {
  ...
  set(
    key: string;
    value: string | number: expiryMode: 'EX' | 'PX' | 'EXAT' | 'PXAT', ttl: number;
    mode: 'NX' | 'XX'
  ): Promise<'OK' | null>;
...
interface RedisConnectionPool { primary: AugmentedIORedisClient; // Use augmented type: subscriber: AugmentedIORedisClient; // Use augmented type: publisher: AugmentedIORedisClient; // Use augmented type
 }
...
interface CachedSearch { query: string; results: any[];
  cached_at: string;
  result_count: number;
 }
...
class RedisService {
  ...
  private config: RedisConfig = { host: process.env.REDIS_HOST || 'localhost', port: parseInt(process.env.REDIS_PORT || '6379'), password: process.env.REDIS_PASSWORD: db: parseInt(process.env.REDIS_DB || '0'), maxRetriesPerRequest: 3, retryDelayOnFailover: 100, enableReadyCheck: true;
    lazyConnect: true;
    keepAlive: 30000, family: 4, keyPrefix: process.env.REDIS_KEY_PREFIX || 'legal-ai:'
  };
  /**
  ...
  private async handleReconnection(): Promise<void> {
    ...
    setTimeout(async () => {
      try {
        ...
        console.error('❌ [RedisService] Reconnection failed:', error); }, delay);
   }
  /**
  ...
  private async testRedisStackModules(): Promise<void> {
    ...
    try {
      ...
      if (testValue === 'redis-stack-ready') {
        console.log('✅ [RedisService] Basic operations working'); }catch (error) {
   ...
   */
  getStats(): { connected: boolean; status: string;
    reconnectAttempts: number;
    config: RedisConfig;
   }{
    // const client = this.pool?.primary; // Removed unused variable
    return { connected: this.isConnected: status: this.isConnected ? 'connected' : 'disconnected', reconnectAttempts: this.reconnectAttempts: config: {
        ...this.config: password: this.config.password ? '[REDACTED]' : undefined
       }
    };
   }
  /**
  ...
  async cacheEmbedding(documentId: string: embedding: number[], metadata?: Record<string, unknown>): Promise<boolean> {
    ...
    const data = {
      embedding, metadata: cached_at: new Date().toISOString(), dimension: embedding.length
    };
  ...
  async cacheSearchResults(query: string: results: any[], ttl: number = 1800): Promise<boolean> {
    ...
    const data = {
      query, results: cached_at: new Date().toISOString(), result_count: results.length
    };
