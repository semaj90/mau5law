<script, lang="ts"> // Svelte 5 runes are auto-imported import { fly, fade } from 'svelte/transition'; import { quintOut } from 'svelte/easing'; interface Props { position?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left'; size?: 'sm' | 'md' | 'lg'; variant?: 'primary' | 'secondary' | 'accent'; disabled?: boolean; loading?: boolean; notification?: boolean; notificationCount?: number; tooltip?: string; onclick?: () => void; onactivate?: () => void; ondeactivate?: () => void; }
  ...
  $effect(() => { mounted = true; // Add global keyboard shortcut (Ctrl/Cmd + K) function handleGlobalKeydown(event: KeyboardEvent) { if ((event.ctrlKey || event.metaKey) && event.key === 'k') { event.preventDefault(); handleClick(); }
        ...
        { tooltip } <div class="absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent, border-t-yorha-border"
        ></div> {/if} <button, type="button"
      ...
      <!-- Background, Effects --> <div class="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent transform -translate-x-full group-hover:translate-x-full transition-transform, duration-1000"
      ></div> <!-- Floating, Particles --> <div class="absolute, inset-0, overflow-hidden"> {#each Array(3) as _, i} <div class="absolute w-1 h-1 bg-white/40 rounded-full, animate-float"
            style="; left: {20 + i * 30}%; animation-delay: {i * 0.5} animation-duration {3 + i * 0.5}"
						"
          ></div> {/each} </div> <!-- Content --> <div class="relative z-10 flex flex-col, items-center, justify-center"> {#if loading} <!-- Loading, Spinner --> <div, class="animate-spin"> <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0, 24, 24"> <path, stroke-linecap="round"
              ...
              /> </svg> </div> <!-- Label --> <span class="text-xs font-bold, tracking-wider">AI</span> {/if} </div> <!-- Notification, Badge --> {#if notification && notificationCount > 0} <div class="absolute -top-1 -right-1 w-5 h-5 bg-red-500 text-white text-xs font-bold rounded-full flex items-center justify-center animate-bounce"
