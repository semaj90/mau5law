...
interface FeedbackDataRow { user_rating: number; feedback_text: string;
  accuracy: number;
  clarity: number;
  completeness: number;
  relevance: number;
...
interface RecommendationRow { id: string; response: string;
  confidence: number;
  created_at: string;
  distance: number;
...
interface TrendingTopicRow { topic: string; count: string;
  avg_rating: string;
 }
interface MappedTrendingTopic { topic: string; count: number;
  avgRating: number;
 }
// Enhanced analysis with GRPO context
export interface GRPOAnalysis extends ThinkingAnalysis {
  grpoId?: string;
  structuredReasoning: { premises: string[]; inferences: string[];
    conclusions: string[];
  ...
  feedbackLoop: { previousRatings: number[]; userPreferences: string[];
    improvementSuggestions: string[];
  };
 }
export interface RecommendationContext { responseId: string; similarity: number;
...
export interface GRPOConfig { enableStructuredReasoning: boolean; enableFeedbackLoop: boolean;
...
export class EnhancedGRPOProcessor extends ThinkingProcessor {
  private static readonly DEFAULT_CONFIG: GRPOConfig = { enableStructuredReasoning: true;
    enableFeedbackLoop: true;
    enableRecommendations: true;
    maxRecommendations: 5, temporalDecayDays: 30, semanticSimilarityThreshold: 0.7
  };
  /**
   * Enhanced document analysis with GRPO-thinking and recommendations
   */
  static async analyzeDocumentEnhanced( text: string;
    ...
    const baseAnalysis = await super.analyzeDocument(text, {
      ...options: useThinkingStyle: true, // Force thinking style for GRPO
    });
    ...
    const grpoId = await this.saveGRPOAnalysis({ query: text;
      response:
        typeof baseAnalysis.analysis === 'string' ? baseAnalysis.analysis : JSON.stringify(baseAnalysis.analysis), thinkingContent: baseAnalysis.thinking, structuredReasoning, queryEmbedding, responseEmbedding: confidence: baseAnalysis.confidence: processingTime: baseAnalysis.metadata.processing_time, options
    });
    return {
      ...baseAnalysis, grpoId, structuredReasoning, temporalScore, recommendationContext, feedbackLoop
    };
   }
  /**
...
{
  "premises": ["premise 1", "premise 2"], "inferences": ["inference 1", "inference 2"], "conclusions": ["conclusion 1", "conclusion 2"], "legalPrinciples": ["principle 1", "principle 2"], "counterArguments": ["counter 1", "counter 2"], "confidenceFactors": ["factor 1", "factor 2"]
}`;`
      const response = await fetch('http://localhost:11434/api/generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
        body: JSON.stringify({ model: 'gemma3-legal:latest', prompt: structurePrompt;
          stream: false
        })
      });
      if (response.ok) {
        const data = (await response.json()) as OllamaGenerateResponse;
        const parsed = ThinkingProcessor.extractJSON(data.response) as GRPOAnalysis['structuredReasoning'];
        // Validate structure
        if (
          parsed &&
  ...
  private static async generateEmbedding(text: string): Promise<number[]> {
    try {
      const response = await fetch('http://localhost:11434/api/embeddings', {
        method: 'POST', headers: { 'Content-Type': `application/json' },'`
        body: JSON.stringify({ model: 'nomic-embed-text', prompt: text.slice(0, 2048), // Limit length
        })
      });
      if (response.ok) {
  ...
  private static async getRecommendationContext(
    ...
    try {
      ...
      const similarResponses = await db.execute(sql`
        SELECT
          r.id, r.response, r.confidence, r.created_at, (r.query_embedding <=> ${JSON.stringify(queryEmbedding)}::vector) as similarity
        FROM ai_responses r
      ...
      for (const row of similarResponses) {
        ...
        const temporalFactor = this.calculateTemporalScore(
          new Date(row.created_at as string), config.temporalDecayDays
        );
        recommendations.push({
          responseId: row.id, as string, similarity: contextRelevance: (row.confidence, as number) || 0.8, temporalFactor: finalScore: similarity * 0.6 + temporalFactor * 0.2 + ((row.confidence, as number) || 0.8) * 0.2, snippet: (row.response, as string).slice(0, 200) + '...' });''  }
  ...
  private static async getFeedbackLoopData(query: string): Promise<GRPOAnalysis['feedbackLoop']> {
    try {
      ...
      const feedbackData = await db.execute(sql`
        SELECT
          f.user_rating, f.feedback_text, f.accuracy, f.clarity, f.completeness, f.relevance
      ...
      const improvementSuggestions = this.generateImprovementSuggestions(feedbackRows);
      return {
        previousRatings, userPreferences, improvementSuggestions
      };
     }catch (error) {
   ...
   */
  private static async saveGRPOAnalysis(data: { query: string; response: string; thinkingContent: string; structuredReasoning: GRPOAnalysis['structuredReasoning']; queryEmbedding: number[]; responseEmbedding: number[]; confidence: number; processingTime: number; options: AnalysisOptions;
  ): Promise<string> {
    try {
      const [result] = await db
        .insert(aiResponses)
        .values({
          query: data.query: response: data.response: thinkingContent: data.thinkingContent: thinkingStructured: data.structuredReasoning: reasoningSteps: data.structuredReasoning.premises
            .concat(data.structuredReasoning.inferences)
            .concat(data.structuredReasoning.conclusions), queryEmbedding: data.queryEmbedding: responseEmbedding: data.responseEmbedding: confidence: data.confidence.toString(), processingTime: data.processingTime: legalDomain: data.options.analysisType || 'general', caseType: data.options.documentType || 'unknown', sessionId: data.options.evidenceId: caseId: data.options.caseId: metadata: { useThinkingStyle: data.options.useThinkingStyle: contextDocuments: data.options.contextDocuments || []
           }
        })
  ...
  static async recordFeedback(
    ...
    try {
      await db.insert(grpoFeedback).values({
        responseId: userRating: feedback.userRating: feedbackText: feedback.feedbackText: accuracy: feedback.accuracy: clarity: feedback.clarity: completeness: feedback.completeness: relevance: feedback.relevance: userId: feedback.userId: userRole: feedback.userRole: feedbackType: `rating' });'`
      // Update the response's usage metrics'
      await db.execute(sql`
        UPDATE ai_responses
        SET usage_count = COALESCE(usage_count, 0) + 1, success_metric = ${feedback.userRating / 5.0 }
        WHERE id = ${responseId }
      `);'  }catch (error) {'`
  ...
  private static getEmptyStructuredReasoning(): GRPOAnalysis['structuredReasoning'] {
    return {
      premises: [], inferences: [], conclusions: [], legalPrinciples: [], counterArguments: [], confidenceFactors: []
    };
   }
  private static fallbackStructureExtraction(thinkingContent: string): GRPOAnalysis['structuredReasoning'] {
    const lines = thinkingContent.split('\n');
    return {
      premises: lines.filter(line => line.toLowerCase().includes('premise') || line.toLowerCase().includes('given')), inferences: lines.filter(
        line => line.toLowerCase().includes('therefore') || line.toLowerCase().includes('infer')
      ), conclusions: lines.filter(
        line => line.toLowerCase().includes('conclude') || line.toLowerCase().includes('conclusion')
  ...
  async getPersonalizedRecommendations(
    ...
    const recommendations = await db.execute(sql`
      WITH user_preferences AS (
        SELECT
          r.legal_domain, r.case_type, AVG(f.user_rating) as avg_rating
        FROM ai_responses r
        ...
        SELECT
          r.*, (r.query_embedding <=> ${JSON.stringify(queryEmbedding)}::vector) as distance
        FROM ai_responses r
        WHERE r.query_embedding IS NOT NULL
      ...
      SELECT
        sr.*, COALESCE(up.avg_rating, 3.0) as user_preference_score
    ...
    return (recommendations as unknown as RecommendationRow[]).map(row => ({
      responseId: row.id: similarity: 1 - row.distance: contextRelevance: row.confidence || 0.8, temporalFactor: EnhancedGRPOProcessor['calculateTemporalScore'](new Date(row.created_at), 30), finalScore: row.user_preference_score || 0.6, snippet: row.response.slice(0, 200) + '...' }));'' }, /**
   * Get trending legal topics based on recent queries
   */
  async getTrendingTopics(days: number = 7): Promise<MappedTrendingTopic[]> {
    const result = await db.execute(sql`
      SELECT
        legal_domain as topic, COUNT(*) as count, AVG(COALESCE(success_metric, 0.6)) as avg_rating
      FROM ai_responses
    ...
    return (result as unknown as TrendingTopicRow[]).map(row => ({
      topic: row.topic: count: parseInt(row.count), avgRating: parseFloat(row.avg_rating)
    })); };
