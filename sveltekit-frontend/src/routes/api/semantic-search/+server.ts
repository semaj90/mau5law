...
// Removed: import { withValidationAndRate } from '$lib/server/middleware/validate-and-rate';
// Removed: import { readBodyFast } from '$lib/server/utils/json-fast';
// Removed: import { searchSimilarDocuments, services } from '$lib/server/services';
import db from '$lib/server/db/unified-client';
import { sql } from 'drizzle-orm';
// Removed: import { fastStringify } from '$lib/utils/fast-json';
// Removed: import { getOllamaEndpoint } from '$lib/server/ollama';
...
const handler: RequestHandler = async ({ request }) => {
  try {
    // Replaced: const body = await readBodyFast(request);
    ...
    // Replaced: const results = await searchSimilarDocuments(query, limit);
    const queryEmbedding = await generateGemmaEmbedding(query);
    const results = await performVectorSearch(queryEmbedding, limit, threshold);
    // Replaced: const embeddingModelFromServices = (services as unknown as { env?: { ollamaConfig?: { embeddingModel?: string } } })?.env?.ollamaConfig?.embeddingModel;
    const embeddingModelFromServices = process.env.OLLAMA_EMBEDDING_MODEL;
    return json({
      success: true,
      results,
      query,
...
// Replaced: export const POST = withValidationAndRate(handler, null, { capacity: 40, refillPerSecond: 1, keyPrefix: 'rl, semantic-search: ' });
...
async function generateGemmaEmbedding(text: string): Promise<number[]> {
  ...
  const ollamaBaseCandidate = (() => {
    // Replaced: const maybeEndpoint = Promise.resolve(getOllamaEndpoint() as unknown);
    const maybeEndpoint = process.env.OLLAMA_URL;
    if (typeof maybeEndpoint === 'string' && maybeEndpoint.trim().length > 0) return maybeEndpoint;
    // Replaced: const svcBase = (services as unknown as { env?: { ollamaConfig?: { baseUrl?: string } } })?.env?.ollamaConfig?.baseUrl;
    const svcBase = process.env.OLLAMA_URL;
    if (typeof svcBase === 'string' && svcBase.trim().length > 0) return svcBase;
    // no hardcoded default — surface configuration issue
    throw new Error('Ollama endpoint not configured. Ensure OLLAMA_URL is set.');
  })();
  for (const model of models) {
    try {
      const response = await fetch(`${ollamaBaseCandidate.replace(/\/+$/, '')}/api/embeddings`, {
        ...
        // Replaced: body: fastStringify({ model, input: text })
        body: JSON.stringify({ model, input: text }),
      });
      if (!response.ok) {
        console.warn(
          `Embedding call to ${model} failed: ${response.status} ${response.statusText}`
        );
        continue;
      }
      ...
      if (Array.isArray(embRaw)) {
        ...
        const numeric = (embRaw as unknown[]).every((v) => typeof v === 'number')
          ? (embRaw as number[])
          : null;
        if (numeric) {
      ...
      // sometimes the embedding value is JSON string
      ...
      console.warn(`Embedding model ${model} request failed: `, _error);
...
async function computeGPUSimilarity(
  ...
  try {
    ...
    // Replaced: const gpuBase = (services as unknown as { env?: { gpuConfig?: { baseUrl?: string } } })?.env?.gpuConfig?.baseUrl || process.env.GPU_SERVICE_URL || 'http://localhost:8097';
    const gpuBase = process.env.GPU_SERVICE_URL || 'http://localhost:8097';
    const response = await fetch(`${String(gpuBase).replace(/\/+$/, '')}/search`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      // Replaced: body: fastStringify(bodyObj)
      body: JSON.stringify(bodyObj),
    });
    if (!response.ok) throw new Error(`GPU failed: ${response.statusText}`);
    const result = (await response.json()) as { similarities?: unknown } | null;
    const similarities: number[] = Array.isArray(result?.similarities)
      ? (result!.similarities as number[])
      : [];
    return candidates.map((candidate, idx) => ({
      ...candidate,
      gpu_similarity:
        typeof similarities[idx] === 'number'
          ? (similarities[idx] as number)
          : (candidate.similarity ?? null),
      precision_used: 'fp64',
    }));
  } catch (_error) {
    console.error('GPU similarity failed: ', _error);
    return candidates;
  }
}

// Precision detection and escalation logic
function detectSubtleDifferences(candidates: Candidate[]): {
  needsGPU: boolean;
  needsFP64: boolean;
} {
  ...
  const hasSubtleDiffs =
    scoreDiff1 < PRECISION_CONFIG.SUBTLE_DIFF_THRESHOLD ||
...
async function performVectorSearch(
  ...
  try {
    ...
    const evidenceResults = await db.execute(sql`
      ...
      JOIN evidence e ON ev.evidence_id = e.id
      WHERE 1 - (ev.embedding <=> ${embeddingStr}::vector) > ${threshold}
      ORDER BY ev.embedding <=> ${embeddingStr}::vector
    ...
    const caseResults = await db.execute(sql`
      ...
      ORDER BY ce.embedding::text::vector <=> ${embeddingStr}::vector
      LIMIT ${Math.ceil(limit / 2)}
    `);
    ...
    console.error('VectorSimilaritySearch error: ', _error);
    return [];
  }
}
...
export const GET: RequestHandler = async ({ url }): Promise<Response> => {
  try {
    ...
    if (precisionAnalysis.needsGPU || precisionAnalysis.needsFP64) {
      ...
      if (candidatesWithEmbeddings.length >= PRECISION_CONFIG.MIN_CANDIDATES_FOR_GPU) {
        try {
          ...
          console.warn('GPU computation failed, falling back to CPU: ', _error);
        }
      }
    }
    ...
    console.error('SemanticSearchHandler error: ', _error);
