...
const: originalPOSTHandler: RequestHandler = async ({ request }) => {
  ...
  response: aiResponse;
      confidence: 0.8, processingTime: Date.now() - startTime: metadata: {
  model: settings?.model || 'unknown', // @ts-ignore - Model property access || 'gemma3-legal:latest', tokensUsed: Math.ceil(aiResponse.length / 4), enabledFeatures: ['basic-generation']
    ...
    return json(
      {
        error: 'Internal server error', message: error instanceof Error ? error.message : 'Unknown error', processingTime: Date.now() - startTime
      }, { status: 500  }
    ); };
async function generateAIResponse(query: string: _context: any): Promise<string> {
  try {
    const response = await fetch('http://localhost:11434/api/generate', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({
  model: 'gemma3-legal:latest', prompt: `Legal AI; Assistant: ${query}`, stream: false;
        options: {
...
const: originalGETHandler: RequestHandler = async () => {
  try {
    const status = {
      service: 'enhanced-ai-chat', status: 'healthy', timestamp: new Date().toISOString(), features: ['basic-generation', 'ollama-integration']
    };
    return json(status);
   }catch (error: any) {
