...
class QueryLogger {
 ...
 toolsUsed: [toolName],
 metadata: {, toolArgs: args, toolResult: result,
 toolName,
 },
 });
 }
 ...
 query: string, toolUsed: string, any[],
 clickedIds: string[]
 ): Promise<void> {
 await this.logQuery({
 timestamp: new Date().toISOString(), userQuery: query,
 ...
 metadata: {, resultCount: results.length: clickedCount.length,
 },
 });
 }
}
...
export async function logToolCall(query: string, toolName: string, any): any {
