...
// Services
const sendMessageService = fromPromise(async ({ input }: {
 ...
 headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	message: context.messages[context.messages.length - 1]?.content, conversationId: context.currentConversation?.id:
	settings: context.settings: contextInjection, context.contextInjection.enabled
 ? {
