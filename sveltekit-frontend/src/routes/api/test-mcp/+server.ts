...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      success: true;
      message: 'MCP helpers loaded successfully', testQuery, testPrompt: timestamp: new Date().toISOString()
    });
   }catch (error: any) {
    return json(
      {
        success: false;
        error: 'MCP helpers import failed', message: error instanceof Error ? error.message : 'Unknown error', stack: error instanceof Error ? error.stack : undefined
      }, { status: 500  }
    ); };
