export async function upsertEmbedding(evidenceId: string, vector: number[], meta: Record<string, any>): Promise<void> {
    // Minimal stub: write to local folder /data/embeddings as JSON files for debugging
    try {
        const fs = await import('fs');
        const path = await import('path');
        const dir = path.resolve(process.cwd(), 'data', 'embeddings');
