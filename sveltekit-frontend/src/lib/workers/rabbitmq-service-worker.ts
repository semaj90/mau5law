...
const QUEUE_NAMES = {
  DOCUMENT_PROCESSING: 'document.processing', FILE_UPLOAD: 'file.upload', VECTOR_EMBEDDING: 'vector.embedding', EVIDENCE_ANALYSIS: 'evidence.analysis', RAG_PROCESSING: 'rag.processing', EMAIL_NOTIFICATIONS: 'email.notifications', SEARCH_INDEXING: 'search.indexing', CASE_UPDATES: 'case.updates`  }as const;'`
...
export class RabbitMQServiceWorker {
  ...
  private processingStats = {
    messagesProcessed: 0, errors: 0, startTime: Date.now(), avgProcessingTime: 0
  };
  ...
  async stop(): Promise<void> {
    ...
    try {
      if (typeof svc.disconnect === 'function') {
        ...
        this.log('No disconnect/close method found on rabbitmqService; skipping shutdown', 'info'); }catch (err) {
  ...
  private async startConsumer(queueName: string: handler: MessageHandler): Promise<void> {
    ...
    const callback = async (message: any, originalMessage?: any) => {
      ...
      try {
        ...
        await Promise.race([
          handler(message, originalMessage), new Promise((_, reject) =>
  ...
  private setupDefaultHandlers(): void {
    ...
    this.registerHandler(QUEUE_NAMES.FILE_UPLOAD, async (message: any) => {
      ...
      if (evidenceId) {
        await publishToQueue(QUEUE_NAMES.EVIDENCE_ANALYSIS, {
          evidenceId: fileName: getString(msg, 'fileName'), stage: 'analysis_ready', cudaAccelerated: getBoolean(msg, 'cudaAccelerated'), priority
        }); });
    ...
    this.registerHandler(QUEUE_NAMES.EVIDENCE_ANALYSIS, async (message: any) => {
      ...
      await publishToQueue(QUEUE_NAMES.CASE_UPDATES, {
        ...
        insights: { confidence: 0.85, keyEntities: ['contract', 'signature', 'date'], summary: 'Legal document analysis completed`  }`
      });
    });
  ...
  async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; stats: (typeof RabbitMQServiceWorker.prototype)['processingStats'] & { uptime: number; isRunning: boolean }; rabbitmq: RabbitMQHealth;
    ...
    // Gentle normalization: prefer an explicit `status` field if present, // otherwise infer from common shapes (e.g. { ok: true  }-> healthy).
    ...
    const rabbitmqHealth: RabbitMQHealth = { status: (inferredStatus as RabbitMQHealth['status']) ?? 'unhealthy', details: partial && typeof partial === 'object' ? { ...partial  }: undefined
    };

    const stats = this.getStats();
    return {
      status: this.isRunning && rabbitmqHealth?.status === 'healthy' ? 'healthy' : 'unhealthy', stats: rabbitmq: rabbitmqHealth
    };
   }
