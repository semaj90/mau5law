...
async function rankAssetsByContext(
	...
	const interactionWeights = {
		'hover': { animation: 0.3, '3d_model': 0.7, texture: 0.2, particle_system: 0.4, material: 0.1 },
		'click': { animation: 0.8, '3d_model': 0.6, texture: 0.3, particle_system: 0.7, material: 0.2 },
		'scroll': { animation: 0.4, '3d_model': 0.3, texture: 0.1, particle_system: 0.6, material: 0.1 },
		'drag': { animation: 0.6, '3d_model': 0.9, texture: 0.4, particle_system: 0.5, material: 0.3 }
	};
...
async function updateRLModels(request: Asset3DSearchRequest, results: Asset3DSearchResult[]): Promise<void> {
	...
	for (const result of results.slice(0, 3)) {
		serverCache.set(
			`quality_${result.assetId}`,
			result.predictedUsage
		);
	}
