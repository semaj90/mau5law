...
| { type: 'CONNECT';
	...
	| {
			type: 'DISPATCH_JOB';
	job: Omit<JobState, 'status' | 'submittedAt' | 'startedAt' | 'completedAt'>;
	  }
	| { type: 'JOB_STARTED';
	jobId: string }
	| { type: 'JOB_PROGRESS';
...
export function isConnected(state: {
	context: RabbitMQContext }): boolean {
	return state.context.isConnected;
}

export function getActiveJobCount(state: {
	context: RabbitMQContext }): number {
