...
export class NeuralSpriteEngine {
  ...
  /**
   * Native highlight implementation for document indices
   * Centralized highlighting behavior that integrates with SOM and canvas
   */
  public async highlightDocumentIndices(indices: number[]): Promise<void> {
    try {
      console.log(`🎯 Highlighting document indices: ${indices.join(', ')}`);

      // Check if native C++ accelerator is available
      let highlightResults: any[] = [];
      
      // Try to load and use the WASM BVH accelerator
      try {
        // Check if WASM is available via window global or dynamic script loading
        if (typeof window !== 'undefined' && (window as any).wasmBVH) {
          console.log('🚀 Using native C++ BVH accelerator for highlighting');
          highlightResults = (window as any).wasmBVH.highlightDocuments(indices);
        } else {
          throw new Error('WASM module not available');
        }
      } catch (wasmError) {
        console.warn('⚠️ WASM BVH accelerator not available, falling back to JavaScript implementation');
        
        // JavaScript fallback implementation
        highlightResults = indices.map(index => ({
          index,
          position: { x: 100 + (index * 80), y: 150 + (index % 3) * 60 },
          highlight: true,
          confidence: 0.85 + Math.random() * 0.15
        }));
      }

      // Apply visual highlights to the canvas
      this.applyHighlightsToCanvas(highlightResults);

      // Update SOM nodes with highlighting information
      this.updateSOMWithHighlights(indices, highlightResults);

      // Log the highlighting activity for AI learning
      this.logUserActivity('document_highlight', {
        indices,
        highlightCount: indices.length,
        accelerationType: highlightResults.length > 0 ? 'native' : 'fallback',
        timestamp: Date.now()
      });

    } catch (error) {
      console.error('❌ Error in highlightDocumentIndices:', error);
      
      // Ultimate fallback - call window hook if available
      if (typeof window !== 'undefined' && window.__DEMO_HIGHLIGHT__ && typeof window.__DEMO_HIGHLIGHT__ === 'function') {
        console.log('🔄 Using window.__DEMO_HIGHLIGHT__ as ultimate fallback');
  ...
  /**
   * Apply visual highlights to canvas objects
   */
  private applyHighlightsToCanvas(highlightResults: any[]): void {
    // Clear existing highlights
    this.clearExistingHighlights();

    highlightResults.forEach((result, index) => {
      ...
      const highlight = new fabric.Circle({
        ...
        selectable: false,
        evented: false,
        opacity: result.confidence || 0.8,
        name: `highlight_${result.index}`,
        data: { type: 'highlight', documentIndex: result.index }
      });
      ...
      // Create text label
      const label = new fabric.Text(`Doc ${result.index}`, {
        left: result.position.x - 15,
        top: result.position.y + 35,
        fontSize: 12,
        ...
        selectable: false,
        evented: false,
        name: `highlight_label_${result.index}`
      });
    ...
    this.canvas.renderAll();
    console.log(`✅ Applied ${highlightResults.length} visual highlights to canvas`);
  }

  /**
   * Clear existing highlight objects from canvas
   */
  private clearExistingHighlights(): void {
    const highlightObjects = this.canvas.getObjects().filter(obj => 
      obj.name && (obj.name.startsWith('highlight_') || obj.data?.type === 'highlight')
    );

    highlightObjects.forEach(obj => this.canvas.remove(obj));
    
    if (highlightObjects.length > 0) {
      this.canvas.renderAll();
      console.log(`🗑️ Cleared ${highlightObjects.length} existing highlights`);
    }
  }

  /**
   * Update SOM nodes with highlighting information for learning
   */
  private updateSOMWithHighlights(indices: number[], highlightResults: any[]): void {
    // Create feature vector for highlighting pattern
    const highlightFeatures = new Float32Array(16);
    highlightFeatures[0] = indices.length / 10; // Normalized count
    highlightFeatures[1] = Math.max(...indices) / 100; // Max index normalized
    highlightFeatures[2] = Math.min(...indices) / 100; // Min index normalized
    highlightFeatures[3] = (Math.max(...indices) - Math.min(...indices)) / 100; // Range
    
    // Fill remaining features with highlight-specific data
    for (let i = 4; i < 16; i++) {
      highlightFeatures[i] = Math.sin(i * indices.length * 0.1);
    }

    // Find best matching SOM node for this highlight pattern
    let bestNode: SOMNode | null = null;
    let bestDistance = Infinity;
    ...
    for (const node of this.somNodes.values()) {
      ...
      for (let i = 0; i < highlightFeatures.length; i++) {
        const diff = highlightFeatures[i] - node.weights[i];
        distance += diff * diff;
      }
      distance = Math.sqrt(distance);
    ...
    if (bestNode) {
      // Submit SOM update task for highlight pattern learning
      this.submitTask({
        id: `som_highlight_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        type: "som_update",
        priority: "high",
        data: {
          node: bestNode,
          inputVector: highlightFeatures,
          learningRate: this.globalLearningRate * 1.2, // Boost learning for interactive highlights
          neighborhoodRadius: bestNode.neighborhoodRadius,
        },
      });

      console.log(`🧠 Updated SOM node ${bestNode.id} with highlight pattern`);
    }
  }
