...
export class QLoRARLLangExtractIntegration {
    ...
    async initialize(): Promise<void> {
        if (typeof window !== 'undefined') {
            this.rlAgent = new Worker(new URL('../workers/rl-legal-agent.worker.ts', import.meta.url), { type: 'module' });
            this.qloraTrainer = new Worker(new URL('../workers/qlora-trainer.worker.ts', import.meta.url), { type: 'module' });
