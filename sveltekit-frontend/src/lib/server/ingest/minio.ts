...
// Replace top-level S3 client with a lazy factory to avoid side-effects at module import
export function getS3Client(): InstanceType<typeof S3Client> {
  ...
  let cfg: S3ClientConfig = { region, credentials: { accessKeyId, secretAccessKey }, forcePathStyle: true };
  try {
    if (endpoint) cfg.endpoint = endpoint;
  } catch (err) {
    // If unusual runtime environment causes assignment issues, fall back and log minimally
    console.warn('getS3Client: endpoint assignment failed, using minimal config');
    cfg = { region, credentials: { accessKeyId, secretAccessKey }, forcePathStyle: true };
  }

  return new S3Client(cfg);
}

/* Helper: runtime check for ReadableStream (works without relying on DOM lib types) */
type WebReadableLike = {
  getReader: () => {
...
// Add minimal typed shapes to avoid `any` casts when converting web streams
...
export async function streamToBuffer(body: any): Promise<Buffer> {
  ...
  // Web ReadableStream (convert to Node Readable when possible)
  if (isReadableStream(body)) {
    // Prefer Node's Readable.fromWeb when available (Node 18+)
    const fromWeb = (Readable as unknown as { fromWeb?: FromWebFn }).fromWeb;
    if (typeof fromWeb === 'function') {
      const nodeStream: Readable = fromWeb(body as unknown as WebReadableLike);
      const chunks: Buffer[] = [];
      for await (const chunk of nodeStream) chunks.push(Buffer.isBuffer(chunk) ? chunk : Buffer.from(chunk));
      return Buffer.concat(chunks);
    }

    // Fallback: consume the readable stream via a reader
    const reader = (body as unknown as WebReadableLike).getReader();
  ...
  if (ArrayBuffer.isView(body)) {
    ...
    return Buffer.from(view.buffer, view.byteOffset ?? 0, view.byteLength ?? undefined);
  }
  ...
  throw new Error('Unable to convert body to Buffer - unsupported type');
}
...
export async function fetchMinioObject(url: string): Promise<Response> {
  ...
  // pass res.Body directly to streamToBuffer (now accepts unknown)
...
/**
 * Uploads a file/object to MinIO. Accepts Buffer | Blob | Readable | File (browser) on the server.
 * Returns a minio:// URL string.
 */
export async function uploadMinioObject(
  bucket: string,
  key: string,
  data: Buffer | Blob | Readable | File | Uint8Array | ArrayBuffer | ReadableStream,
  userId: string
): Promise<string> {
  let buffer: Buffer;
  // Buffer
  if (Buffer.isBuffer(data)) {
    ...
    // streamToBuffer accepts unknown so pass directly
    buffer = await streamToBuffer(data);
    ...
    throw new Error(
      'Unsupported data type for uploadMinioObject. Provide Buffer, Readable, ReadableStream, Blob, File, Uint8Array or ArrayBuffer.'
    );
  }
  ...
  try {
    ...
    console.error('uploadMinioObject: upload failed', err);
    throw err;
  }
...
export async function healthCheck(): Promise<any> {
  try {
    ...
    console.error('MinIO service health check failed:', error);
