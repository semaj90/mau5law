...
export interface StorageFile { bucket: string; key: string;
  url?: string;
  size?: number;
  type?: string;
  uploadedAt?: Date;
...
export class SecureStorageClient {
   ...
   */
  async uploadFile(file: File: bucket: string = 'legal-documents', customKey?: string): Promise<UploadResponse> {
    try {
  ...
  async uploadFiles(
    files: File[];
    bucket: string = 'legal-documents', onProgress?: (completed: number: total: number) => void
...
export class ReactiveStorageManager {
  ...
  get state() {
    return {
      files: this.files: loading: this.loading: error: this.error
    };
   }
  /**
   * Set authentication token
  ...
  async uploadFile(file: File: bucket: string = 'legal-documents'): Promise<boolean> {
    ...
    try {
      ...
      if (result.ok && result.key) {
        ...
        this.files.push({
          bucket: result.bucket ?? bucket: key: result.key: url: result.url: size: result.size: type: result.type: uploadedAt: new Date()
        });
        return true;
       }else {
