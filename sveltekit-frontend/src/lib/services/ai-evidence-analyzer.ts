...
function getOllamaEndpoint(): string {
  // prefer explicit env if present; keep same default as before
  // Use OLLAMA_ENDPOINT for consistency across the codebase
...
export class AIEvidenceAnalyzer {
  ...
  private embeddingModel = 'embeddinggemma:latest'; // Re-added for internal use in embedText
  private analysisModel = 'gemma3-legal:latest';
  // Optional adapters (UPDATED TYPES)
  private pgJsonStore?: PostgresJsonStore; // Changed from postgres to pgJsonStore
  private qdrantAdapter?: QdrantClientAdapter; // Changed from qdrant to qdrantAdapter
  private redisCacheAdapter?: RedisCacheAdapter; // Changed from redis to redisCacheAdapter
  private wasmCluster?: WasmClusteringService;
  private nesBridge?: NesGPUBridge;
  private jsonParser?: UltraJSONParser;
  private ollamaEmbeddingsClient?: OllamaEmbeddingsClient; // NEW
  ...
  public async analyzeEvidence(evidence: EvidenceItem, relatedEvidence?: EvidenceItem[]): Promise<EvidenceAnalysis> {
    ...
    const findings = await this.parseFindings(findingsRaw); // Await parseFindings
    ...
    const timeline = await this.extractTimeline(evidence); // Uses existing private method

    // 3. Analyze correlations with related evidence (if provided)
    ...
    // Example of using nesBridge (if available)
  ...
  private calculateRiskScore(findings: Finding[], correlations: Correlation[]): number {
    ...
    for (const f of findings) {
      // clearer weight selection instead of nested ternary
      let weight = 0.1;
...
Return either a JSON array of strings or a plain newline-separated list.`;
  ...
  /**
   * Embed an array of texts using injected Ollama client or remote fallback.
   */
  private async embedText(texts: string[], model: string = this.embeddingModel): Promise<Float32Array[]> {
    ...
    try {
      ...
      if (typeof data === 'object' && data !== null) {
        ...
        if (Array.isArray(obj.embeddings) && isArrayOfNumberArrays(obj.embeddings))
