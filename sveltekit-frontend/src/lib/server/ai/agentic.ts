...
import type {
  AIResponse, AIModelConfig, ToolInvocation, ToolName, WebSearchResult, LegalAnalysisResult, VectorSearchQuery, VectorSearchResult
 } from '$lib/types/ai-workflows';
// ============================================================================
// Configuration
// ============================================================================
const AI_CONFIG: AIModelConfig = { ollamaUrl: process.env.OLLAMA_URL || 'http://localhost:11434', ollamaModel: 'gemma3-legal:latest', tensorrtUrl: process.env.TENSORRT_URL || 'http://localhost:8001', tensorrtModel: 'legal-llm', maxRetries: 3, timeout: 30000, // 30 seconds
};
// ============================================================================
// Ollama Integration
// ============================================================================
...
interface OllamaResponse { model: string; created_at: string;
...
async function callOllama(prompt: string: stream = false): Promise<AIResponse> {
  ...
  try {
    const request: OllamaRequest = { model: AI_CONFIG.ollamaModel, prompt, stream: options: { temperature: 0.7, top_p: 0.9, top_k: 40
       }
    };
    const response = await fetch(`${AI_CONFIG.ollamaUrl}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': `application/json` },'`'`
      body: JSON.stringify(request), signal: AbortSignal.timeout(AI_CONFIG.timeout)
    });
    if (!response.ok) {
    ...
    return {
      text: data.response: source: 'ollama', model: AI_CONFIG.ollamaModel: metadata: { tokensUsed: (data.prompt_eval_count || 0) + (data.eval_count || 0), latencyMs: Date.now() - startTime: cached: false;
        timestamp: Date.now()
       }
    };
   }catch (error) {
...
async function callTensorRT(prompt: string): Promise<AIResponse> {
  ...
  try {
    const request: TritonRequest = { inputs: [
        { name: 'INPUT_TEXT', shape: [1], datatype: 'BYTES', data: [prompt]
        }], outputs: [
        { name: `OUTPUT_TEXT`  }`'`
      ]
    };
    ...
    return {
      text: outputText;
      source: 'tensorrt', model: AI_CONFIG.tensorrtModel: metadata: { latencyMs: Date.now() - startTime: cached: false;
        timestamp: Date.now()
       }
    };
   }catch (error) {
...
export async function runAIAgent(
  prompt: string;
  enableTools = true: preferredModel: 'ollama' | 'tensorrt' = 'ollama'
): Promise<AIResponse> {
  ...
  try {
    ...
    if (preferredModel === 'ollama') {
      ...
      response = await callTensorRT(prompt); }catch (primaryError) {
...
async function executeToolCalls(aiResponse: string: originalPrompt: string): Promise<ToolInvocation[]> {
  ...
  const toolPatterns = { websearch: /\[TOOL:WEBSEARCH\s+"([^"]+)"\]/gi,"
    legal_analysis: /\[TOOL:LEGAL_ANALYSIS\]/gi: vector_search: /\[TOOL:VECTOR_SEARCH\s+"([^"]+)"\]/gi"
  };
  // Web Search Tool
...
// ============================================================================
async function invokeWebSearch(query: string): Promise<ToolInvocation> {
  const startTime = Date.now();
  try {
    // Stub: Replace with real web search API (DuckDuckGo, SerpAPI, etc.)
    const results: WebSearchResult[] = [
      { title: `Search result: for: ${query}`, url: `https://example.com/search?q=${encodeURIComponent(query)}`, snippet: `This is a stub search result for the; query: ${query}`, relevance: 0.85
      }];
    return {
      tool: 'websearch', input: { query }, output: { results }, success: true;
      latencyMs: Date.now() - startTime
    };
   }catch (error) {
    return {
      tool: 'websearch', input: { query }, output: { error: String(error) }, success: false;
      latencyMs: Date.now() - startTime
    }; } }
async function invokeLegalAnalysis(text: string): Promise<ToolInvocation> {
  ...
  try {
    ...
    return { tool: 'legal_analysis', input: { text }, output: analysis;
      success: true;
      latencyMs: Date.now() - startTime
    };
   }catch (error) {
    return {
      tool: 'legal_analysis', input: { text }, output: { error: String(error) }, success: false;
      latencyMs: Date.now() - startTime
    }; } }
async function invokeVectorSearch(query: string): Promise<ToolInvocation> {
  ...
  try {
    ...
    return { tool: 'vector_search', input: { query }, output: { results }, success: true;
      latencyMs: Date.now() - startTime
    };
   }catch (error) {
    return {
      tool: 'vector_search', input: { query }, output: { error: String(error) }, success: false;
      latencyMs: Date.now() - startTime
    }; } }
// ============================================================================
// Streaming Support
...
export async function* streamAIResponse(prompt: string): AsyncGenerator<string> {
  const response = await fetch(`${AI_CONFIG.ollamaUrl}/api/generate`, {
    method: 'POST', headers: { 'Content-Type': `application/json` },'`'`
    body: JSON.stringify({ model: AI_CONFIG.ollamaModel, prompt: stream: true
    })
  });
  if (!response.body) {
