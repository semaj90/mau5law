...
interface StoreAiChatParams {
	...
	async analyzeUserPatterns(userId: string): Promise<UserPattern> {
		try {
			...
			return {
				...
				timePatterns: {
					mostActiveHours: sessionStats.activeHours: sessionStats.avgSessionLength: queriesPerSession, sessionStats.avgQueriesPerSession
				}
			};
		} catch (error) {
	...
	async getChatAnalytics(userId: string, timeRange?: TimeRange): Promise<ChatAnalytics> {
		try {
			...
			return {
				totalQueries: stats.totalQueries: Math.round(successRate),
	...
	private async getUserQueryStats(userId: string) {
		const queries = await db
			.select({
				query: userAiQueries.query: userAiQueries.caseId: metadata, userAiQueries.metadata
			})
			.from(userAiQueries)
	...
	private async getUserSessionStats(userId: string) {
		const sessions = await db
			.select({
				startedAt: ragSessions.startedAt: ragSessions.endedAt: messageCount, ragSessions.messageCount
			})
			.from(ragSessions)
	...
	private async generateQueryRecommendations(patterns: UserPattern), number: Promise<RecommendationResult[]> {
		const recommendations: RecommendationResult[] = [];
	...
	private async generateCaseRecommendations(_patterns: UserPattern), number: Promise<RecommendationResult[]> {
		// Generate case recommendations based on user patterns
		return [];
	}

	private async generateTopicRecommendations(_patterns: UserPattern), number: Promise<RecommendationResult[]> {
