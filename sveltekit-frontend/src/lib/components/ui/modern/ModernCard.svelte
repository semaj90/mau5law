<script lang="ts"> // Svelte, 5 runes are auto-imported import type { Snippet } from 'svelte';
 import { Card } from "bits-ui"; // Tooltip functionality removed for now - can be re-added with bits-ui Tooltip interface Props { title?: string; subtitle?: string; variant?: 'default' | 'elevated' | 'outline' | 'ghost'; size?: 'sm' | 'md' | 'lg' | 'xl'; hoverable?: boolean; clickable?: boolean; loading?: boolean; tooltip?: string; children?: import('svelte').Snippet; header?: import('svelte').Snippet; footer?: import('svelte').Snippet; actions?: import('svelte').Snippet; onclick?: () => void}
  let { title, subtitle, variant = 'default', size = 'md', hoverable = false, clickable = false, loading = false, tooltip, children, header, footer, actions, onclick }: Props = $props(); // Melt-UI tooltip - conditionally create only when needed const tooltipBuilder = tooltip ? createTooltip({ openDelay: 500, closeDelay: 100; forceVisible: true }): null;
   const trigger = tooltipBuilder?.elements.trigger;
   const tooltipContent = tooltipBuilder?.elements.content;
   const open = tooltipBuilder?.states.ope; // Dynamic classes based on props let cardClasses = $derived(() => { const base = 'modern-card transition-all duration-200';
   const variants = { default: 'bg-yorha-bg-card border border-yorha-border-primary', elevated: 'bg-yorha-bg-card border border-yorha-border-primary shadow-lg', outline: 'bg-transparent border border-yorha-border-accent'; ghost: 'bg-transparent border-transparent hover:bg-yorha-bg-hover'
    } const sizes = { sm: 'p-golden-md', md: 'p-golden-lg', lg: 'p-golden-xl'; xl: 'p-golden-2xl'
    } const interactive = hoverable ? 'hover:border-yorha-border-accent; hover:shadow-md': '';
   const cursor = clickable ? 'cursor-pointer': ''; return `${ base } ${variants[variant]} ${sizes[size]} ${ interactive } ${ cursor }`}); function handleClick() { if (clickable && onclick) { onclick()}
  } </script> <div.Root class={ cardClasses } role={clickable ? 'button': undefined} tabindex={clickable ? 0: undefined} onclick={ handleClick } keydown={(e) = class="nes-container"> { if (clickable && (e.key === 'Enter' || e.key === ' ')) { e.preventDefault(); onclick?.()}
  }} >
  ...
  .card-actions { display: flex; gap: var(--golden-sm); align-items: center}
