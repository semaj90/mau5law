...
export interface PaginatedResponse<T = unknown> { data: T[]; page: number;
  limit: number;
...
export class LegalAIApiClient {
  ...
  async updateCase(
    id: string;
    caseData: Partial<{ title: string; description: string; caseNumber: string; status: 'open' | 'closed' | 'pending' | 'archived'; priority: 'low' | 'medium' | 'high' | 'urgent'; category: string; metadata: Record<string, unknown>;
    }>, signal?: AbortSignal
  ): Promise<ApiResponse<unknown>> {
    return this.request<ApiResponse<unknown>>(`/cases/${id}`, {
      method: 'PUT', body: caseData;
      signal
    });
   }
  /**
  ...
  async updateEvidence(
    id: string;
    evidenceData: Partial<{ title: string; evidenceType: string;
      description: string;
      fileUrl: string;
      fileName: string;
      fileSize: number;
      mimeType: string; hash: string; tags: string[]; chainOfCustody: any[]; aiSummary: string; summary: string; isAdmissible: boolean; confidentialityLevel: string;
    }>, signal?: AbortSignal
  ): Promise<ApiResponse<unknown>> {
    return this.request<ApiResponse<unknown>>(`/evidence/${id}`, {
      method: 'PUT', body: evidenceData;
      signal
    });
   }
  /**
  ...
  async updateReport(
    id: string;
    reportData: Partial<{ title: string; description: string; reportType: string; caseId: string; content: string; status: string; metadata: Record<string, unknown>;
    }>, signal?: AbortSignal
  ): Promise<ApiResponse<unknown>> {
    return this.request<ApiResponse<unknown>>(`/reports/${id}`, {
      method: 'PUT', body: reportData;
      signal
    });
   }
  /**
  ...
  async updatePersonOfInterest(
    id: string;
    personData: Partial<{ name: string; description: string; riskLevel: string; caseId: string; contactInfo: Record<string, unknown>;
      aliases: string[]; metadata: Record<string, unknown>;
    }>, signal?: AbortSignal
  ): Promise<ApiResponse<unknown>> {
    return this.request<ApiResponse<unknown>>(`/persons-of-interest/${id}`, {
      method: 'PUT', body: personData;
      signal
    });
   }
  /**
      ...
      xhr.addEventListener('load', () => {
        try {
          ...
          if (status >= 200 && status < 300) {
            ...
            reject(new ApiError(status, 'UPLOAD_FAILED', 'File upload failed', parsed)); }catch (err) {
          reject(err); });
