...
describe('GraphService', () => {
  ...
  describe('createCaseStatuteRelationships', () => {
    it('should create relationships between case and statutes', async () => {
      ...
      expect(graphService.createCaseStatuteRelationships).toHaveBeenCalledWith(
        caseId,
        statutes
      );
    });
  ...
  describe('createPrecedentRelationship', () => {
    it('should create precedent relationship between cases', async () => {
      ...
      await graphService.createPrecedentRelationship(
        caseId1,
        caseId2,
        relevanceScore
      );
  ...
  describe('error handling', () => {
    it('should handle Neo4j connection errors', async () => {
      ...
      await expect(graphService.getCaseNode(caseId)).rejects.toThrow(
    ...
    it('should handle query errors gracefully', async () => {
      ...
      vi.mocked(graphService.findSimilarCases).mockRejectedValueOnce(
        new Error('Query error')
      );

      await expect(graphService.findSimilarCases(caseId, limit)).rejects.toThrow(
        'Query error'
      );
    });
  });
