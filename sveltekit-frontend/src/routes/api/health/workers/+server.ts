...
type ConnectableRedisClient = RedisClientType & { connect?: () => Promise<void> };
...
async function ensureRedisConnection(
  client: ConnectableRedisClient | null,
  label: string
): Promise<boolean> {
...
async function fetchWorkerRedisState(
  ...
  { ok: true; heartbeat: string | null; stats: string | null } | { ok: false; reason: string }
> {
  const client = redis.duplicate() as ConnectableRedisClient;
...
async function checkAutotagWorker(): Promise<WorkerStatus> {
  ...
  try {
    ...
    autotagRedisClient = redis.duplicate() as ConnectableRedisClient;
    ...
    await safeQuit(autotagRedisClient, 'Autotag Worker');
  }
}
