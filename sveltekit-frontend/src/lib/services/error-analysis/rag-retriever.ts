  ...
  async queryPatterns(error: Error, topK: number = 5): Promise<Pattern[]> {
    ...
    try {
      ...
      const response = await this.retry(async () => {
        const res = await fetch(`${this.config.qdrantUrl}/collections/${this.COLLECTION_NAME}/points/search`, {
          method: 'POST',
  ...
  async formatContext(patterns: Pattern[]): Promise<string> {
    ...
    try {
      ...
      patterns.forEach((pattern, idx) => {
        contextLines.push(`### Pattern ${idx + 1} (Similarity: ${(pattern.similarity * 100).toFixed(1)}%)`);
  ...
  async storePattern(pattern: Pattern): Promise<void> {
    ...
    try {
      ...
      await this.retry(async () => {
        const res = await fetch(`${this.config.qdrantUrl}/collections/${this.COLLECTION_NAME}/points`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            points: [
