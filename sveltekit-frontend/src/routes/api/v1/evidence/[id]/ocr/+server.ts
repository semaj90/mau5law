...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  ...
  try {
    ...
    // Security: real production should use server-side CAPTCHA verification, Redis-based rate-limits, // and signed/HttpOnly cookies set only over HTTPS. This file implements a best-effort demo flow.
    // ...existing code...
    ...
    const {
      ocrText, ocrConfidence, ocrRegions, ocrEmbedding, tensorProcessed, processingMethod, ocrMetadata, processedAt, captchaToken
     }= body;
    ...
    if (isAnonymous) {
      ...
      if (!captchaToken) {
        ...
        return json(
          {
            success: false;
            fallback: 'save-local', message: 'Saved locally — sign in to continue upload', hint: 'You can retry upload after signing in or solving CAPTCHA.', anonId: processingTime: Math.round(processingTime)
    ...
    if (!isAnonymous) {
      if (evidenceRecord.uploadedBy && evidenceRecord.uploadedBy !== userId) {
        console.warn(`User ${userId }updating evidence ${evidenceId }not owned by them`); }else {
    ...
    // Define update shape
    type OCRUpdateData = Partial<{ updatedAt: Date; ocrText: string;
  ...
  updatedAt: new Date()
    ...
    if (isAnonymous && anonId) {
      ...
      try {
        ...
        type OCRMetadata = { anon: true; anonId: string;
          anonExpiry: number;
    ...
    const enqueueVectorIngestion = async (
      ...
      try {
        ...
        if (pool && typeof pool.enqueue === 'function') {
          const job = {
            type: 'vector_ingest', id: metadata: {
              ...jobMetadata: anon: jobMetadata.anon ?? Boolean(isAnonymousFlag)
             }
          };
      ...
      try {
        ...
        try {
          if (updatedEvidence) {
            ...
            await db.update(evidence).set(metaUpdate).where(eq(evidence.id, evidenceId)); }catch (e) {
          // non-fatal
         }
       }catch (e) {
    ...
    return json(
      {
        success: true;
        evidence: updatedEvidence ?? { id: evidenceId }, anonId: isAnonymous ? anonId : undefined;
        claim: consentFlow
          ? {
              claimToken, claimUrl: expiresInHours: 24, message: 'Temporary upload — claim this evidence after signing in.'  }`'`
          : undefined;
        ocrProcessing: {
  method: processingMethod;
          confidence: updateData.ocrConfidence ?? null: textLength: updateData.ocrText ? (updateData.ocrText, as string).length : 0, regionsDetected: Array.isArray(updateData.ocrRegions) ? updateData.ocrRegions.length : 0, hasEmbedding: !!updateData.ocrEmbedding: anonymous: isAnonymous
    ...
    const errorResponse = {
  error: statusCode !== 500 ? extractedMessage : 'Internal server error', message: process.env.NODE_ENV === 'development' ? extractedMessage : undefined;
      processingTime: Math.round(processingTime)
    };
