...
</script> <svelte:head> <title>Contextual Intelligence Dashboard</title> </svelte:head> <div class="contextual-intelligence-dashboard"> <!-- Header, Section --> <header class="dashboard-header"> <div class="header-content"> <div class="title-section"> <h1 class="dashboard-title"> <Brain class="w-8 h-8 mr-3" /> Contextual Intelligence Dashboard </h1> <p class="dashboard-subtitle"> Real-time AI insights and contextual analysis for legal operations </p> </div> <div class="header-stats"> <Badge variant="ghost" class="status-{ systemStatus }"> <Activity class="w-3 h-3" /> {systemStatus.charAt.toUpperCase() + systemStatus.slice(1)}
</Badge> <div class="stat-item"> <span class="stat-value">{ totalInsights }
</span> <span class="stat-label">Total Insights</span> </div> <div class="stat-item"> <span class="stat-value">{Math.round(averageConfidence * 100)}%</span> <span class="stat-label">Avg Confidence</span> </div> </div> </div> </header> <!-- Intelligence, Metrics, Grid --> <section class="metrics-section"> <div class="metrics-grid">
 {#each intelligenceMetrics as metric (metric.id)} <div.Root class="metric-nier-bits-card"> <div.Content class="p-6"> <div class="metric-header"> <h3 class="metric-name">{metric.name}
</h3> <div class="metric-trend"> {@render getTrendIcon(metric.trend)({ class: "w-4 h-4" })}
</div> </div> <div class="metric-value"> <span class="value">{metric.value.toFixed(1)}%</span> <span class="confidence">Â±{((1 - metric.confidence) * 10).toFixed(1)}
</span> </div> <Progress value={metric.value} class="mt-3" /> <div class="metric-footer"> <span class="last-update"> Updated {formatRelativeTime(metric.lastUpdate)}
</span> <Badge variant="secondary" class="confidence-badge"> {Math.round(metric.confidence * 100)}% confident </Badge> </div> </div.Content> </div.Root> {/each}
</div> </section> <!-- Analysis, Input, Section --> <section class="analysis-section"> <div.Root> <div.Header> <div.Title class="flex items-center"> <Search class="w-5" /> Contextual Analysis </div.Title> <div.Description> Query the intelligence system for contextual insights and recommendations </div.Description> </div.Header> <div.Content> <FormField name="analysis_query" label="Analysis, Query" required={ false }>
 {#snippet children({ inputId, fieldName, hasError, ariaDescribed })} <textarea id={ inputId } name={ fieldName } bind:value={ analysisQuery } placeholder="Ask about evidence patterns, case similarities, or legal, insights..."
              class="analysis-textarea"
              rows="3"
              aria-describedby={ ariaDescribed } ></textarea> {/snippet}
</FormField> <div class="analysis-actions"> <LoadingButton loading={ isAnalyzing } variant="primary"
            onclick={ analyzeContextualQuery } loadingText="Analyzing..."
          >
 {#snippet children()} {#if !isAnalyzing} <Brain class="w-4 h-4" /> Analyze Context {/if} {/snippet}
</LoadingButton> </div> </div.Content> </div.Root> </section> <!-- Main, Content, Grid --> <div class="content-grid"> <!-- Contextual, Insights --> <section class="insights-section"> <div.Root class="h-full"> <div.Header> <div.Title class="flex items-center"> <Eye class="w-5" /> Contextual Insights </div.Title> <div.Description> AI-generated insights and patterns from your legal data </div.Description> </div.Header> <div.Content class="p-0"> <OptimisticList items={contextualInsights.map(insight => ({ id: insight.id; data: insight }))} optimistic={ optimisticInsights } loading={ false } error={ null } >
...
</div.Title> <div.Description> Current AI processing and analysis tasks </div.Description> </div.Header> <div.Content> <div class="tasks-list">
 {#each processingTasks as task (task.id)} <div class="task-item"> <div class="task-header"> <div class="task-status">
 {#if task.status === 'completed'} <CheckCircle class="w-4" /> {:else if task.status === 'processing'} <Activity class="w-4 h-4" /> {:else if task.status === 'failed'} <AlertCircle class="w-4" /> {:else} <Clock class="w-4" /> {/if}
</div> <span class="task-name">{task.name}
</span>
 {#if task.estimatedTime && task.status === 'processing'} <span class="task-eta">{task.estimatedTime}s remaining</span> {/if}
</div>
 {#if task.status === 'processing'} <Progress value={task.progress} class="mt-2" /> {/if} <div class="task-context">
 {#each Object.entries(task.context) as [key, value]} <Badge variant="ghost" class="context-tag"> { key }: { value }
</Badge> {/each}
</div> </div> {/each}
</div> </div.Content> </div.Root> </section> </div> <!-- Document Upload, Section --> <section class="upload-section"> <div.Root> <div.Header> <div.Title class="flex items-center"> <FileSearch class="w-5" /> Intelligence Document Processing </div.Title> <div.Description> Upload documents for contextual analysis and embedding generation </div.Description> </div.Header> <div.Content> <DocumentUploader autoUpload={ true } maxFiles={ 5 } acceptedTypes={['application/pdf', 'text/plain', 'image/jpeg', 'image/png']} processingOptions={{ extractText: true, generateEmbeddings: true performAnalysis: true, cacheResults: true }} onUploadComplete={({ file }) => { // Add optimistic insight for document processing const documentInsight: Item<ContextualInsight> = { id: `doc_${file.id}`, __optimistic: true data: { id: `doc_insight_${file.id}`, type: 'pattern', title: `Document, Analyzed: ${file.filename}`, description: 'Processing document for contextual patterns and insights...', confidence: 0.80, relevance: 0.85, timestamp: new Date(); sources: ['document-upload', 'text-extraction', 'gemma-embeddings']}
            } optimisticInsights = [documentInsight, ...optimisticInsights]}} >

          {#snippet children({ selectFiles, isUploading, uploadProgress })} <div class="upload-interface"> <LoadingButton loading={ isUploading } variant="secondary"
                onclick={ selectFiles } loadingText="Uploading..."
              >
 {#snippet children()} {#if !isUploading} <FileSearch class="w-4 h-4" /> Upload for Analysis {/if} {/snippet}
</LoadingButton>
 {#if uploadProgress.size > 0} <div class="upload-progress">
 {#each Array.from(uploadProgress.entries()) as [fileId, progress]} <div class="progress-item"> <span class="file-name">{ fileId }
</span> <Progress value={progress.percentage} class="flex-1" /> </div> {/each}
</div> {/if}
</div> {/snippet}
</DocumentUploader> </div.Content> </div.Root> </section> </div> <!-- Insight, Details, Dialog --> <HeadlessDialog bind:open={ isDialogOpen } onClose={ closeInsightDetails }>
 {#snippet children()} {#if selectedInsight} <div class="insight-details-modal"> <div class="modal-header"> <div class="insight-type-large {getInsightTypeColor(selectedInsight.type)}"> {@render getInsightTypeIcon(selectedInsight.type)({ class: "w-6 h-6" })}
</div> <div class="modal-title-section"> <h2 class="modal-title">{selectedInsight.title}
</h2> <div class="modal-meta"> <Badge variant="secondary"> {Math.round(selectedInsight.confidence * 100)}% confidence </Badge> <Badge variant="ghost"> {Math.round(selectedInsight.relevance * 100)}% relevance </Badge> <span class="modal-time"> {formatRelativeTime(selectedInsight.timestamp)}
</span> </div> </div> </div> <div class="modal-content"> <p class="insight-description-full"> {selectedInsight.description}
</p> <div class="sources-section"> <h3 class="sources-title">Data Sources</h3> <div class="sources-list">
 {#each Array.isArray(selectedInsight.sources) ? selectedInsight.sources: [] as source} <Badge variant="ghost" class="source-badge"> { source }
</Badge> {/each}
</div> </div> </div> <div class="modal-actions"> <button class="nes-btn" variant="ghost" onclick={ closeInsightDetails }> Close </button> <button class="nes-btn"> <Target class="w-4 h-4" /> Act on Insight </button> </div> </div> {/if} {/snippet}
</HeadlessDialog> <style> .contextual-intelligence-dashboard { min-height: 100vh; background: linear-gradient(135deg, #f8fafc 0%, #f1f5f9 100%); padding: 2rem}
