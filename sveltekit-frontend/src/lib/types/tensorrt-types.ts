...
export interface TensorRTError {
    code: 'TIMEOUT' | 'NETWORK_ERROR' | 'PARSING_ERROR' | 'SERVICE_UNAVAILABLE' | 'RATE_LIMITED' | 'UNKNOWN';
    message: string;
    details?: { [key: string]: any };
    timestamp: number;
    request_id?: string;
...
export namespace TensorRTTypes {
    export type Request = LegalAIRequest;
    export type Response = LegalAIResponse;
    export type StreamingResp = StreamingResponse;
    export type Metrics = PerformanceMetrics;
    export type Config = TensorRTClientConfig;
    export type Error = TensorRTError;
    export type Health = ServiceHealth;
    export type Analytics = PerformanceAnalytics;
}