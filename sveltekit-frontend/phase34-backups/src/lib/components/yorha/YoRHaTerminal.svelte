...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; interface TerminalProps { title?: string; prompt?: string; history?: string[]; currentInput?: string; isActive?: boolean; onCommand?: (command: string) => void; maxLines?: number}
  ...
  function processCommand(cmd: string) { const [command, ...args] = cmd.toLowerCase().split(' '); switch (command) { case, 'help': terminalHistory = [ ...terminalHistory,
          ...
          '  exit - Close terminal',
          '']; break; case, 'clear': terminalHistory = []; break; case, 'status': terminalHistory = [ ...terminalHistory,
          ...
          '']; break; case, 'cases': terminalHistory = [ ...terminalHistory,
          'Active Legal Cases:',
          ...
          '']; break; case, 'evidence': if (args[0] === 'list') { terminalHistory = [ ...terminalHistory,
            'Evidence Repository:',
