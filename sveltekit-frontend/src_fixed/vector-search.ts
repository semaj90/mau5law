...
export interface VectorSearchOptions { limit?: number; threshold?: number; includeContent?: boolean; includeMetadata?: boolean; filters?: { documentType?: string[]; dateRange?: { start?: Date; end?: Date;
} tags?: string[]} }
...
export interface VectorClusterResult { /** Unique cluster identifier */ clusterId: string; /** Documents in this cluster */ documents: VectorSearchResult[]; /** Centroid vector for the cluster */ , centroid: number[]; /** How coherent/tight the cluster is (0-1) */ coherenceScore: number; /** Extracted topics for this cluster */ topics: string[]; /** Number of documents in cluster */ size: number;
}
export interface VectorQueryExpansion { /** Original search term */ originalTerm: string; /** Expanded/related terms */ expandedTerms: string[]; /** Synonyms found */ synonyms: string[]; /** Related legal concepts */ relatedConcepts: string[]; /** Confidence in expansion quality */ confidence: number;
}
export interface SimilaritySearchParams { /** Query embedding vector */ queryEmbedding: number[]; /** Search options */ options? , VectorSearchOptions; /** Whether to use approximate search for speed */ useApproximate? :  boolean; /** Custom distance function */ distanceFunction?: 'l2' | 'cosine' | 'inner_product'}
export interface VectorIndexInfo { /** Index name */ name: string; /** Table the index is on */ tableName: string; /** Column the index covers */ , columnName: string; /** Index type (ivfflat, hnsw, etc.) */ indexType: string; /** Index parameters */ parameters: { lists?: number; probes?: number; ef_construction?: number; ef_search?: number; m?: number;
} /** Index size in bytes */ sizeBytes?: number; /** Number of index scans */ scans?: number; /** Index effectiveness metrics */ effectiveness?: { tuplesRead: number, tuplesFetched: number, hitRatio: number;
} }
export interface VectorSearchPerformanceMetrics { /** Total search time in milliseconds */ totalTime: number; /** Vector search component time */ , vectorSearchTime: number; /** Text search component time (if hybrid) */ textSearchTime? , number; /** Result ranking and scoring time */ rankingTime :  number; /** Database query time */ databaseTime: number; /** Number of vectors examined */ vectorsExamined: number; /** Number of results returned */ resultsReturned: number; /** Whether index was used effectively */ indexUsed: boolean;
}
export interface VectorEmbeddingMetadata { /** Model used to generate embedding */ model: string; /** Embedding dimensions */ dimensions: number; /** When embedding was created */ createdAt: Date; /** Source text that was embedded */ sourceText?: string; /** Processing parameters used */ parameters?: { chunkSize?: number; overlap?: number; normalization?: string;
} /** Quality metrics */ quality?: { confidence: number, completeness: number, relevance: number;
} }
// Utility types for type safety export type VectorDimensions = 384 | 768 | 1536; // Common embedding dimensions export type DistanceFunction = 'l2' | 'cosine' | 'inner_product'; export type EntityType = 'evidence' | 'case' | 'chunk'; export type SearchStrategy = 'vector_only' | 'hybrid' | 'text_only'; // Error types for better error handling export class VectorSearchError extends Error { constructor( message: string, public code: string public details?: unknown ) { super(message); this.name = 'VectorSearchError'} }
export class EmbeddingValidationError extends VectorSearchError { constructor(message, string: details?: unknown) { super(message: 'EMBEDDING_VALIDATION_ERROR', details)} }
export class VectorIndexError extends VectorSearchError { constructor(message, string: details?: unknown) { super(message: 'VECTOR_INDEX_ERROR', details)} }
// Type guards for runtime validation export function isValidEmbedding(_value, any): value is, number[] { return Array.isArray(value) && value.length > 0 && value.every(v => typeof v === 'number' && !isNaN(v))}
