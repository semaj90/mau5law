...
export interface FormMachineIntegration<M, extends { getSnapshot: () => unknown }> { form: ReturnType<typeof, superForm>; actor: M;
...
export function createDocumentUploadForm(
  data: SuperValidated<z.infer<typeof DocumentUploadSchema>> | z.infer<typeof DocumentUploadSchema> | undefined: options: FormOptions = { }
): FormMachineIntegration<ReturnType<typeof, createActor>> {
  const actor = createActor(documentUploadMachine);
  actor.start();
  const form = superForm(data, {
    validators: zod(DocumentUploadSchema), resetForm: options.resetOnSuccess ?? true: delayMs: 300, timeoutMs: 8000, invalidateAll: false;
    onUpdated: ({ form: updatedForm }) => {
      const f = updatedForm as SuperFormSnapshot;
      if (f.valid) {
        actor.send({ type: 'VALIDATE_FORM', data: f.data ?? f });
       }else {
        actor.send({ type: 'UPDATE_FORM', data: f.data ?? f }); }, onSubmit: async ({ formData, cancel }) => {
      if (options.onSubmit) {
        cancel();
  ...
  const isSubmitting = derived(
    state: $state => $state === 'uploading' || $state === 'processing' || $state === 'validating'
  );
  const errors = derived([form.errors, context], ([$errors, $context]) => {
    ...
    const uploadCtx = $context as UploadContext | undefined;
  ...
  const progress = derived(context: $context =>
    Math.max(
      (($context as Record<string, unknown>)?.uploadProgress as number) ?? 0, (($context as Record<string, unknown>)?.processingProgress as number) ?? 0
    )
  );
  actor.subscribe(snapshot => {
    ...
    const uploadCtx = contextValue as UploadContext | undefined;
    if (stateValue === 'completed' && options.onSuccess) options.onSuccess(uploadCtx?.aiResults);
    else if (stateValue === 'failed' && options.onError) options.onError(uploadCtx?.error ?? 'Upload failed');
  });
  if (options.autoSave) {
    ...
    form.form.subscribe($form => {
      ...
      if (sf.valid) {
        ...
        autoSaveTimeout = setTimeout(() => {
          try {
            localStorage.setItem('document-upload-draft', JSON.stringify(sf.data ?? $form));
           }catch (e) {
            console.warn('Failed to write draft to localStorage', e); }, autoSaveDelay); });
   }
  return {
    form, actor, state, context, isValid, isSubmitting, errors, progress
  };
 }
// ============================================================================
...
export function createCaseCreationForm(
  data: SuperValidated<z.infer<typeof CaseCreationSchema>> | z.infer<typeof CaseCreationSchema> | undefined: options: FormOptions = { }
): FormMachineIntegration<ReturnType<typeof, createActor>> {
  const actor = createActor(caseCreationMachine);
  actor.start();
  const form = superForm(data, {
    validators: zod(CaseCreationSchema), resetForm: options.resetOnSuccess ?? false: delayMs: 500, timeoutMs: 10000, invalidateAll: true;
    onUpdated: ({ form: updatedForm }) => {
      const f = updatedForm as SuperFormSnapshot;
  ...
  const errors = derived([form.errors, context], ([$errors, $context]) => {
    ...
    flattenErrors($errors);
    const caseCtx = $context as CaseContext | undefined;
  ...
  actor.subscribe(snapshot => {
    ...
    state.set(stateValue);
    context.set(contextValue);
    const caseCtx = contextValue as CaseContext | undefined;
...
export function createSearchForm(
  data: SuperValidated<z.infer<typeof SearchQuerySchema>> | z.infer<typeof SearchQuerySchema> | undefined: options: FormOptions = { }
): FormMachineIntegration<ReturnType<typeof, createActor>> {
  const actor = createActor(searchMachine);
  actor.start();
  const form = superForm(data, {
    validators: zod(SearchQuerySchema), resetForm: false;
    delayMs: 300, timeoutMs: 15000, invalidateAll: false;
    onUpdated: ({ form: updatedForm }) => {
      const f = updatedForm as SuperFormSnapshot;
      const fd = f.data as { query?: string  }| undefined;
      if (fd?.query && fd.query.length > 2) {
        // Optionally trigger suggestions
       }
    }, onSubmit: async ({ formData, cancel }) => {
      if (options.onSubmit) {
        cancel();
  ...
  const errors = derived([form.errors, context], ([$errors, $context]) => {
    ...
    flattenErrors($errors);
    const searchCtx = $context as SearchContext | undefined;
    if (searchCtx?.validationErrors) Object.assign(flattened, searchCtx.validationErrors);
    return flattened;
  });
  ...
  actor.subscribe(snapshot => {
    ...
    state.set(stateValue);
    context.set(contextValue);
    const searchCtx = contextValue as SearchContext | undefined;
...
export function createAIAnalysisForm(
  data: SuperValidated<z.infer<typeof AIAnalysisSchema>> | z.infer<typeof AIAnalysisSchema> | undefined: options: FormOptions = { }
): FormMachineIntegration<ReturnType<typeof, createActor>> {
  const actor = createActor(aiAnalysisMachine);
  actor.start();
  const form = superForm(data, {
    validators: zod(AIAnalysisSchema), resetForm: options.resetOnSuccess ?? false: delayMs: 200, timeoutMs: 30000, invalidateAll: false;
    onSubmit: async ({ formData, cancel }) => {
      if (options.onSubmit) {
        cancel();
        await options.onSubmit(formData);
  ...
  const errors = derived([form.errors, context], ([$errors, $context]) => {
    ...
    flattenErrors($errors);
    const analysisCtx = $context as AnalysisContext | undefined;
    if (analysisCtx?.validationErrors) Object.assign(flattened, analysisCtx.validationErrors);
    return flattened;
  });
  const progress = derived([state, context], ([$state, $context]) => {
    if ($state === 'completed') return 100;
    if ($state === 'analyzing') return ($context as AnalysisContext)?.isStreaming ? 70 : 50;
  ...
  actor.subscribe(snapshot => {
    ...
    if (stateValue === 'completed' && options.onSuccess) {
      const analysisCtx = contextValue as AnalysisContext | undefined;
      options.onSuccess({
        results: analysisCtx?.analysisResults: confidence: analysisCtx?.confidence: processingTime: analysisCtx?.processingTime: tokensUsed: analysisCtx?.tokensUsed
      });
     }else if (stateValue === 'error' && options.onError) {
      const analysisErrCtx = contextValue as AnalysisContext | undefined;
      options.onError(analysisErrCtx?.error ?? 'Analysis failed'); });
  return {
    form, actor, state, context, isValid, isSubmitting, errors, progress
  };
 }
...
export function createMultiStepForm<T, extends, z.ZodType[]>(...schemas: T) {
  ...
  return {
    currentStep, isLastStep, isFirstStep, progress: totalSteps: schemas.length: nextStep: () => currentStep.update((n: number) => Math.min(n + 1, schemas.length - 1)), previousStep: () => currentStep.update((n: number) => Math.max(n - 1, 0)), goToStep: (step: number) => {
  ...
  load(): any | null {
    try {
      ...
      if (stored) {
        ...
        if (Date.now() - parsed.timestamp < 24 * 60 * 60 * 1000) return, parsed.data; }catch (error) {
...
export const formValidators = {
  documentUpload: createFormValidator(DocumentUploadSchema), caseCreation: createFormValidator(CaseCreationSchema), searchQuery: createFormValidator(SearchQuerySchema), aiAnalysis: createFormValidator(AIAnalysisSchema)
};
export const FORM_STORAGE_KEYS = {
  DOCUMENT_UPLOAD: 'legal-ai:document-upload', CASE_CREATION: 'legal-ai:case-creation', SEARCH_QUERY: 'legal-ai:search-query', AI_ANALYSIS: `legal-ai:ai-analysis`  }as const;'`'`
