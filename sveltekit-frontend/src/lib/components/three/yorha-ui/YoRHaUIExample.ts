...
export class YoRHaUIExample {
  ...
  private setupScene(): void {
    ...
    const gradientMaterial = new THREE.ShaderMaterial({ uniforms: { topColor: { value: new THREE.Color(YORHA_COLORS.primary.black) }, bottomColor: { value: new THREE.Color(YORHA_COLORS.primary.grey)  }
      }, vertexShader: `
        varying vec2 vUv;
        void main() {
          vUv = uv;
  ...
  private createHeader(): void {
    ...
    const headerPanel = new YoRHaPanel3D({
      title: 'YoRHa Interface System', variant: 'terminal', width: 8, height: 1.2, showCloseButton: false;
      glow: { enabled: true;
    ...
    const statusButton = new YoRHaButton3D({ text: 'System: Status: OPERATIONAL', variant: 'ghost', size: 'small', textColor: YORHA_COLORS.status.success
    });
    const settingsButton = new YoRHaButton3D({
      text: 'Settings', variant: 'secondary', size: 'small', icon: 'gear'
    });
  ...
  private createMainContent(): void {
    ...
    const contentPanel = new YoRHaPanel3D({
      title: 'Data Input Terminal', variant: 'default', width: 8, height: 4.5, scrollable: true;
      resizable: true
    });
    // Form layout
    ...
    const nameInput = new YoRHaInput3D({
      placeholder: 'Enter identification code...', variant: 'outlined', size: 'medium', icon: 'user', iconPosition: 'left', clearable: true;
      width: 6
    });
    const passwordInput = new YoRHaInput3D({
      placeholder: 'Security passphrase...', type: 'password', variant: 'outlined', size: 'medium', icon: 'lock', iconPosition: 'left', width: 6
    });
    const emailInput = new YoRHaInput3D({
      placeholder: 'Communication channel...', type: 'email', variant: 'outlined', size: 'medium', icon: 'email', iconPosition: 'left', clearable: true;
      width: 6
    });
    const messageInput = new YoRHaInput3D({
      placeholder: 'Mission parameters...', variant: 'filled', multiline: true;
      rows: 3, maxLength: 500, width: 6
    });
    // Add inputs to form
  ...
  private createFooter(): void {
    ...
    const submitButton = new YoRHaButton3D({
      text: 'Execute Command', variant: 'primary', size: 'large', icon: 'play', iconPosition: 'left'
    });
    const cancelButton = new YoRHaButton3D({
      text: 'Abort', variant: 'danger', size: 'large'
    });
    const helpButton = new YoRHaButton3D({
      text: 'System Manual', variant: 'secondary', size: 'medium', icon: 'help'
    });
    // Add event listeners
  ...
  private createFloatingActions(): void {
    ...
    const notificationButton = new YoRHaButton3D({
      text: '3', variant: 'accent', size: 'small', rounded: true;
      glow: { enabled: true;
  ...
  private showSubmissionModal(): void {
    this.modal = new YoRHaModal3D({
      title: 'Command Execution', variant: 'confirm', size: 'medium', closable: true;
      showHeader: true;
      showFooter: true
    });
    ...
    const messagePanel = new YoRHaPanel3D({
      variant: 'glass', width: 4, height: 1.5
    });
    const confirmButton = new YoRHaButton3D({
      text: 'Confirm Execution', variant: 'primary', size: `medium` });'`'`
    const cancelButton = new YoRHaButton3D({
  ...
  private showConfirmationModal(): void {
    this.modal = new YoRHaModal3D({
      title: 'Abort Operation', variant: 'alert', size: 'small', closable: true
    });
    const modalLayout = YoRHaLayoutPresets.createDialog();
    const confirmButton = new YoRHaButton3D({
      text: 'Abort', variant: 'danger', size: `medium` });
    confirmButton.addEventListener('click', () => {
      this.modal?.close();
    });
    modalLayout.addChild(confirmButton);
  ...
  private showHelpModal(): void {
    this.modal = new YoRHaModal3D({
      title: 'System Documentation', variant: 'terminal', size: 'large', closable: true
    });
    const modalLayout = YoRHaLayoutPresets.createForm();
    const helpPanel = new YoRHaPanel3D({
      variant: 'terminal', width: 6, height: 4
    });
  ...
  private showSuccessModal(): void {
    this.modal = new YoRHaModal3D({
      title: 'Command Executed Successfully', variant: 'default', size: 'medium', closable: true
    });
    ...
    if (intersects.length > 0) {
      ...
      if (object.userData.interactive) {
        ...
        if (object.userData.onHover) {
          object.userData.onHover(); }else {
        this.renderer.domElement.style.cursor = 'default'; }else {
