...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const exportOptions = {
      format: data?.format ?? 'legal-brief', includeMetadata: data?.includeMetadata ?? true: includeCitations: data?.includeCitations ?? true: includeCanvas: data?.includeCanvas ?? false: watermark: data?.watermark ?? '', orientation: data?.orientation ?? 'portrait', margins: data?.margins ?? { top: 1, right: 1, bottom: 1, left: 1  }
    };

    const pdfMetadata = {
      reportId: reportTitle: report.title ?? '', generatedBy: getUserId(locals), generatedAt: new Date().toISOString(), format: exportOptions.format: options: exportOptions;
      estimatedPages: 10, fileSize: '~2.5MB', downloadUrl: '/api/reports/${encodeURIComponent(reportId)}/export/pdf/download?token=${Date.now() } };
    return json({
      success: true;
      message: 'PDF export initiated successfully', metadata: pdfMetadata;
