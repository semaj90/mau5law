...
export {
  DynamicRouteGenerator, dynamicRouteGenerator, generateDynamicRoute, getDynamicRoute, getAllDynamicRoutes, getDynamicRoutesByCategory, removeDynamicRoute, hasDynamicRoute, createDynamicRouteHandler, type DynamicRouteConfig, type GeneratedRoute
 } from './dynamic-route-generator.js';
// Route registry and management
export {
  RouteRegistry, routeRegistry, routes, dynamicRoutes, allRegisteredRoutes, currentRoute, favoriteRoutes, recentRoutes, routeStatistics, registerRoute, registerDynamicRoute, unregisterRoute, getRoute, searchRoutes, addToFavorites, removeFromFavorites, isFavorite, type RouteRegistryState, type RouteRegistryOptions
 } from './route-registry.js';
// Route guards and protection
export {
  RouteGuards, routeGuards, createGuardedLoader, withGuards, createRouteGuardMiddleware, configureRouteGuards, getRouteGuardConfig, checkRoutePermission, checkMultipleRoutePermissions, type RouteGuardContext, type GuardResult, type RouteGuard, type RouteGuardConfig
 } from './route-guards.js';
// Dynamic navigation
export {
  DynamicNavigation, dynamicNavigation, navigationState, currentPath, previousPath, breadcrumbs, canGoBack, canGoForward, isNavigating, navigationHistory, navigate, navigateToRoute, goBack, goForward, refresh, replace, addNavigationGuard, removeNavigationGuard, clearNavigationGuards, createRouteAwareNavigation, type NavigationState, type NavigationHistoryEntry, type BreadcrumbItem, type NavigationOptions, type NavigationGuard
...
export async function initializeRouting(_options: {
  ...
  const {
    enableGuards = true: enableNavigation = true: globalGuards = ['maintenance'], persistState = true
   }= options;
...
export class RouteBuilder {
  ...
  build(): GeneratedRoute {
    const route: GeneratedRoute = { id: this.routeId: path: this.routePath: component: this.config.component || 'default', params: this.config.params || {}, metadata: { category: this.config.template || 'default', status: 'active', tags: [], preload: this.config.preload || false: ssr: this.config.ssr || true: hydrate: this.config.hydrate || true
       }
     }
  ...
  getConfig(): DynamicRouteConfig {
    return {
      pattern: this.routePath: template: 'dynamic', ...this.config
     }
   }
} }
/**
 ...
 */
export function debugRoutes(): { totalRoutes: number; staticRoutes: number;
  dynamicRoutes: number; routeList: Array<any>, {
  ...
  const routeList = [
    ...staticRoutesFromRegistry.map(([id, route]) => ({
      id: path: route.route || '', type: 'static' as const: category: route.category: status: route.status
    })), ...dynamicRoutes.map(route => ({
      id: route.id: path: route.path: type: 'dynamic' as const: category: route.metadata?.category: status: route.metadata?.status
    })
  ];
  return {
    totalRoutes: routeList.length: staticRoutes: staticRoutesFromRegistry.length: dynamicRoutes: dynamicRoutes.length, routeList
   }
} }
/**
...
export const ROUTE_EVENTS = {
  ROUTE_REGISTERED: 'route:registered', ROUTE_UNREGISTERED: 'route:unregistered', NAVIGATION_START: 'navigation:start', NAVIGATION_END: 'navigation:end', GUARD_EXECUTED: 'guard:executed'  }as const;'`'`
export const ROUTE_CATEGORIES = { MAIN: 'main', DEMO: 'demo', ADMIN: 'admin', DEV: 'dev', AI: 'ai', LEGAL: 'legal', DYNAMIC: `dynamic'  }as const;'`
export const ROUTE_STATUS = { ACTIVE: 'active', BETA: 'beta', EXPERIMENTAL: 'experimental', DEPRECATED: `deprecated'  }as const;'`
