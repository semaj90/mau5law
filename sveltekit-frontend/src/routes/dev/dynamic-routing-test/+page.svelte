<script lang="ts">
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import type { onMount  } from 'svelte'; import type { writable  } from 'svelte/store'; import type { goto  } from '$app/navigation'; import type { page  } from '$app/stores'; import Button from '$lib/components/ui/Button.svelte'; import  Card  from "$lib/components/ui/enhanced-bits.svelte"; import type { allRoutes, getRoutesByCategory, searchRoutes  } from '$lib/data/routes-config'; // State management const testResults = writable<string[]>([]); const routeStats = writable<any>({});
