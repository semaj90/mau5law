...
class AIChatStore {
  ...
  currentSession = $derived(this.state.currentSession);
  sessions = $derived(this.state.sessions);
  preferences = $derived(this.state.preferences);
  isLoading = $derived(this.state.isLoading);
  connectionStatus = $derived(this.state.connectionStatus);
  lastError = $derived(this.state.lastError);
  ...
  private saveToStorage(): void {
    ...
    try {
      localStorage.setItem(STORAGE_KEYS.SESSIONS, JSON.stringify(this.state.sessions.map(s => ({
        ...s,
...
// Export convenience functions that work with the class instance
export const createNewSession = (title?: string) => aiChatStore.createNewSession(title);
...
export const updateMessage = (messageId: string, updates: Partial<ChatMessage>) => aiChatStore.updateMessage(messageId, updates);
...
export const exportSession = (sessionId: string, format?: 'json' | 'markdown' | 'txt') => aiChatStore.exportSession(sessionId, format);
export const loadChatHistory = () => aiChatStore.loadChatHistory();
export const clearHistory = () => aiChatStore.clearHistory();
export const updatePreferences = (updates: Partial<UserPreferences>) => aiChatStore.updatePreferences(updates);

// Export reactive state accessors
export const currentSession = () => aiChatStore.currentSession;
export const allSessions = () => aiChatStore.sessions;
export const userPreferences = () => aiChatStore.preferences;
