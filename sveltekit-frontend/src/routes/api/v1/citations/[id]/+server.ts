...
const UpdateCitationSchema = z.object({
  title: z.string().min(1).optional(), citation: z.string().min(1).optional(), citationType: z.enum(['case_law', 'statute', 'regulation', 'constitutional', 'secondary', 'other']).optional(), jurisdiction: z.string().optional(), court: z.string().optional(), year: z.number().int().min(1800).max(2030).optional(), volume: z.string().optional(), page: z.string().optional(), url: z.string().url().optional(), notes: z.string().optional(), relevance: z.enum(['high', 'medium', 'low']).optional(), verified: z.boolean().optional(), metadata: z.record(z.any()).optional()
});
  ...
  try {
    ...
    return json({
      ...
      data: {
        ...
        // use safe helper
  userId: getUserIdFromLocals(locals), timestamp: new Date().toISOString()
       }
    });
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid citation ID', code: 'INVALID_ID', details: err.errors
        })
      );
     }
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to fetch citation', code: 'FETCH_FAILED', details: msg
      })
    ); };
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  ...
  userId: getUserIdFromLocals(locals), citationId: timestamp: new Date().toISOString(), action: 'citation_updated'
       }
    });
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid citation data', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    const msg = err instanceof Error ? err.message : String(err);
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to update citation', code: 'UPDATE_FAILED', details: msg
      })
    ); };
...
export const DELETE: RequestHandler = async ({ params, locals }) => {
  try {
    ...
    return json({
      success: true;
      data: {
  message: 'Citation deleted successfully', deletedCitation: {
  id: citationId;
          title: existingCitation.title
         }
      }, meta: {
  userId: getUserIdFromLocals(locals), deletedCitationId: citationId;
        timestamp: new Date().toISOString(), action: `citation_deleted`  }
    });
   }catch (err: any) {
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to delete citation', code: 'DELETE_FAILED', details: msg
      })
    ); };
