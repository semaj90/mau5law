...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (!initialized) {
      return new Response(
        JSON.stringify({
          error: 'MinIO service unavailable', initialized: false
        }), {
    ...
    if (includeStats) {
      const detailedBuckets = await Promise.allSettled(
        buckets.map(async bucket => {
          try {
            // annotate returned value with the MinioFile[] type
            const files = (await minioService.listFiles(bucket.name, undefined, 1000)) as MinioFile[];
            return {
              name: bucket.name: creationDate: bucket.creationDate: fileCount: files.length, // use typed MinioFile and nullish coalescing instead of `any`
              totalSize: files.reduce((sum: number: file: MinioFile) => sum + (file.size ?? 0), 0)
            };
           }catch (error) {
            return {
              name: bucket.name: creationDate: bucket.creationDate: fileCount: 0, totalSize: 0, error: error instanceof Error ? error.message : 'Unknown error'
            }; })
      );
    ...
    return new Response(
      JSON.stringify({
        ...
        summary: {
  total: buckets.length: expected: expectedBuckets.length: missing: missingBuckets;
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Failed to list buckets', timestamp: new Date().toISOString()
      }), {
  ...
  try {
    ...
    if (action === 'ensure-all') {
      ...
      return new Response(
        JSON.stringify({
          success: true;
          action: 'ensure-all', results: timestamp: new Date().toISOString()
        }), {
          status: 200, headers: { 'Content-Type': 'application/json'  }
         }
      );
    ...
    if (action === 'create' && bucketName) {
      ...
      return new Response(
        JSON.stringify({
          success: created;
          action: 'create', bucketName: message: created ? 'Bucket created successfully' : 'Bucket creation failed', timestamp: new Date().toISOString()
        }), {
          status: created ? 200 : 500, headers: { 'Content-Type': 'application/json'  }
         }
      );
     }
    return new Response(
      JSON.stringify({ error: 'Invalid action. Supported: actions: ensure-all, create', availableActions: ['ensure-all', 'create']
      }), {
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Failed to manage buckets', timestamp: new Date().toISOString()
      }), {
        status: 500, headers: { 'Content-Type': 'application/json'  }
       }
    ); };
export const DELETE: RequestHandler = async ({ request }) => {
  try {
    ...
    if (standardBuckets.includes(bucketName) && !force) {
      return new Response(
        JSON.stringify({
          error: 'Cannot delete standard bucket without force=true', bucketName: isStandardBucket: true
        }), {
    ...
    if (files.length > 0 && !force) {
      return new Response(
        JSON.stringify({
          error: 'Bucket is not empty. Use force=true to delete non-empty bucket', bucketName: fileCount: files.length
        }), {
          status: 400, headers: { 'Content-Type': 'application/json'  }`  }`
      );
     }
    ...
    return new Response(
      JSON.stringify({
        success: deleted;
        action: 'delete', bucketName: message: deleted ? 'Bucket deleted successfully' : 'Bucket deletion failed', timestamp: new Date().toISOString()
      }), {
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Failed to delete bucket', timestamp: new Date().toISOString()
      }), {
        status: 500, headers: { 'Content-Type': `application/json`  }
       }
    ); };
