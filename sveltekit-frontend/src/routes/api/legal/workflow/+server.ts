...
export const POST: RequestHandler = async ({ request }) => {
  ...
  id: `doc_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`, documentId: `${caseId}_${Date.now()}`, caseId: documentType: documentType as LegalDocumentMessage['documentType'], content: metadata: {
  submittedAt: new Date().toISOString(), source: 'api', contentLength: content.length, ...body.metadata
      }, priority: priority as LegalDocumentMessage['priority'], retryCount: 0, timestamp: Date.now()
    };
    ...
    if (!published) {
      return json(
        {
          success: false;
          error: `Failed to queue document for processing` }, { status: 500  }
      );
     }
    ...
    return json({
      success: true;
      documentId: documentMessage.id: caseId: documentMessage.caseId: queuedAt: new Date().toISOString(), expectedProcessingTime: priority === 'urgent' ? '2-5 minutes' : '5-15 minutes', responseTime: `${responseTime}ms`, message: 'Document queued for legal analysis workflow' });
   }catch (error) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      status: 'operational', services: { rabbitmq: { connected: rabbitmqService.connected: queues: queueStats;
          totalMessages
        }, xstate: {
  status: 'ready', machineTypes: ['legalDocumentWorkflow', 'contractAnalysis', 'evidenceProcessing']
         }
      }, endpoints: {
  submit: 'POST /api/legal/workflow', status: 'GET /api/legal/workflow', managementUI: `http://localhost:15672 (legal_admin:123456)` }, usage: { submitDocument: { method: 'POST', url: '/api/legal/workflow', body: {
  content: 'Document content here...', documentType: 'contract | evidence | brief | citation | discovery', caseId: 'CASE-2025-001', priority: 'low | normal | high | urgent', metadata: {
  fileName: 'optional-file-name.pdf', tags: ['contract', 'employment']
             }
           }
         }
