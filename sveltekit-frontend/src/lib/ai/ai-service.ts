...
class AIService {
	...
	async sendToAI(options: SendToAIOptions): Promise<AIServiceResponse> {
		...
		try {
			...
			const enhancedPrompt = this.buildEnhancedPrompt({ prompt, caseContext, evidenceIds, context, includeHistory });

			// Add user message to store
			assistant.addMessage?.(caseId, { role: 'user', content: prompt, evidenceIds: evidenceIds.length > 0 ? evidenceIds : undefined });
	...
	private buildEnhancedPrompt(options: {
		...
		if (caseContext) {
			...
			if (includeHistory && Array.isArray(caseContext.messages) && caseContext.messages.length > 0) {
	...
	async analyzeEvidence(caseId: string, evidenceId: string, specificQuestion?: string): Promise<AIServiceResponse> {
		...
		return this.sendToAI({ caseId, prompt, evidenceIds: [evidenceId], context: 'analysis', includeHistory: false });
	}
	...
	async findConnections(caseId: string, evidenceIds: string[]): Promise<AIServiceResponse> {
		...
		return this.sendToAI({ caseId, prompt, evidenceIds, context: 'connection', includeHistory: true });
	}
	...
	async annotateEvidence(caseId: string, evidenceId: string, annotation: string): Promise<AIServiceResponse> {
		...
		return this.sendToAI({ caseId, prompt, evidenceIds: [evidenceId], context: 'annotation', includeHistory: false });
	}
