...
export function trainSOM(points: Vector[], options: SOMOptions = {}): SOMModel {
  ...
  const iterations = options.iterations && options.iterations > 0 ? Math.floor(options.iterations) : 1000;
  const learningRate0 = options.learningRate && options.learningRate > 0 ? options.learningRate : 0.1;
  const radius0 = options.radius && options.radius > 0 ? options.radius : Math.max(width, height) / 2;
