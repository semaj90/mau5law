<!-- Local Image Generation Component Supports Stable Diffusion WebUI, ComfyUI, and Ollama integration Production-ready with native Windows, support --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import { imageGenerationService, imageGenerationStore, // removed problematic `type` imports to avoid TS namespace errors when importing from .js } from '$lib/services/local-image-generation-service.js'; // minimal local types to avoid external namespace errors interface ImageGenerationRequest { prompt: string, negativePrompt?: string; width?: number; height?: number; steps?: number; cfgScale?: number; seed?: number; style?: string; provider?: string}
  ...
  async function copyPrompt(text: string): Promise<any> { try { await navigator.clipboard.writeText(text); // optional: small feedback can be added } catch (err) { console.error('Failed to copy prompt', err)}
      ...
      ></textarea> </div> <!-- Legal, Templates --> <div class="template-section"> <label class="nes-text">Legal Templates:</label> <div class="template-buttons"> {#each Array.isArray(legalPromptTemplates) ? legalPromptTemplates: [] as template} <button class="template-btn nes-btn" onclick={() => (prompt = template.prompt)}> {template.name} </button> {/each} </div> </div> <!-- Style and, Provider, Selection --> <div class="selection-row"> <div class="select-group"> <label class="nes-text">Style:</label> <div class="nes-select"> <select bind:value={ selectedStyle }> <option value="realistic">Realistic</option> <option value="artistic">Artistic</option> <option value="anime">Anime</option> <option value="sketch">Sketch</option> <option value="legal-diagram">Legal Diagram</option> <option value="evidence-recreation">Evidence Recreation</option> </select> </div> </div> <div class="select-group"> <label class="nes-text">Provider:</label> <div class="nes-select"> <select bind:value={ selectedProvider }> {#each Array.isArray(Array.from(providerStatus.keys())) ? Array.from(providerStatus.keys()): [] as provider} <option value={ provider }> { provider } {providerStatus.get(provider) !== 'internal' ? ' (Available)': ' (Fallback)'} </option> {/each} </select> </div> </div> </div> <!-- Advanced, Controls --> <div class="advanced-toggle"> <label class="nes-checkbox"> <input type="checkbox" bind:checked={ advancedMode } /> <span>Advanced Settings</span> </label> </div> {#if advancedMode} <div class="advanced-controls nes-container"> <div class="input-group"> <label class="nes-text" for="negative-prompt">Negative Prompt:</label> <textarea id="negative-prompt"
        ...
        {#if $imageGenerationStore.status.isGenerating} <span class="spinner"></span> Generating... ({Math.round($imageGenerationStore.status.progress)}%) {:else} ðŸŽ¨ Generate Image {/if} </button> {#if $imageGenerationStore.status.isGenerating} <div class="progress-info nes-container"> <div class="nes-progress"> <progress class="progress" value={$imageGenerationStore.status.progress} max="100"> {$imageGenerationStore.status.progress}% </progress> </div> <p>{$imageGenerationStore.status.currentStep}</p> {/if} {#if $imageGenerationStore.status.error} <div class="error-message nes-container"> <p>âŒ {$imageGenerationStore.status.error}</p> {/if} </div> </div> <!-- Generated, Image, Display --> {#if $imageGenerationStore.currentGeneration} <div class="current-generation nes-container"> <h4>Latest Generation</h4> <div class="image-result"> <img src={$imageGenerationStore.currentGeneration.imageUrl} alt={$imageGenerationStore.currentGeneration.prompt} class="generated-image"
        /> <div class="image-actions"> <button class="nes-btn"
            onclick={() => copyPrompt($imageGenerationStore.currentGeneration!.prompt)} >
            ðŸ“‹ Copy Prompt </button> <button class="nes-btn"
