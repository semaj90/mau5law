...
export interface LegalWebGPUContext { device: GPUDevice; adapter: GPUAdapter;
  context: GPUCanvasContext;
...
export interface LegalDocumentGPUBuffers { embeddings: GPUBuffer; metadata: GPUBuffer;
  colors: GPUBuffer;
  indices: GPUBuffer;
  uniforms: GPUBuffer;
 }
// Shader module definitions
export interface LegalAIShaderModules { documentVertex: GPUShaderModule; documentFragment: GPUShaderModule;
...
export interface LegalComputePipelineDescriptor { type: 'embedding_normalization' | 'similarity_search' | 'document_clustering' | 'risk_assessment'; shaderModule: GPUShaderModule; workgroupSize: [number, number, number];
  bufferLayout: GPUBufferBindingLayout[];
 }
// Render pipeline for legal document visualization
export interface LegalRenderPipelineDescriptor { vertex: { module: GPUShaderModule;
    entryPoint: string;
...
export interface WebGPUPerformanceMetrics { renderTime: number; computeTime: number;
  memoryUsage: { buffers: number; textures: number;
    total: number;
  };
  pipelineStats: { drawCalls: number; computeDispatches: number;
...
export class WebGPUError extends Error {
  constructor( message: string;
    public code: 'DEVICE_LOST' | 'OUT_OF_MEMORY' | 'VALIDATION_ERROR' | 'OPERATION_ERROR', public details?: {
      pipeline?: string;
      buffer?: string;
      operation?: string;
     }
...
export interface LegalDocumentTexture { texture: GPUTexture; view: GPUTextureView;
  sampler: GPUSampler;
...
export interface CHRROMGPUPattern { patternId: string; texture: GPUTexture;
  buffer: GPUBuffer;
  metadata: { bankId: number; tileIndex: number;
    riskLevel: 'low' | 'medium' | 'high' | 'critical'; documentType: string;
  };
 }
...
export interface LegalDocumentVertex {
  ...
  documentId: number; riskLevel: number; // 0=low, 1=medium, 2=high, 3=critical
  confidence: number; // 0.0-1.0
 }
// WebGPU command encoder utilities
export interface LegalCommandEncoder { encoder: GPUCommandEncoder; renderPass: GPURenderPassEncoder | null; computePass: GPUComputePassEncoder | null;
...
export interface LegalVisualizationState { camera: { position: [number, number, number];
  ...
  lighting: { ambient: [number, number, number];
    directional: { direction: [number, number, number];
      color: [number, number, number];
      intensity: number;
    };
  };
  interaction: { selectedDocument: string | null; hoveredDocument: string | null;
    filterLevel: 'all' | 'low' | 'medium' | 'high' | 'critical';
  };
 }
// WebGPU resource management
export interface WebGPUResourceManager { buffers: Map<string, GPUBuffer>;
