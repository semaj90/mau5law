...
export class LangChainSIMDBridge {
  ...
  async processDocument(
    ...
    const enhancedResult: SIMDProcessingResult = {
      ...
      simdData: {
        compressedTiles: (simdResult.compressedTiles as TextTile[]).map(tile => ({
          id: tile.id,
        ...
        compressionStats = {
          sourceCompression:
            ...
            (simdResult.compressedTiles as TextTile[]).reduce(
              (sum: number, t: TextTile) => sum + (t.compressedData?.length || 0),
              0
            ),
  ...
  private calculateMemoryEfficiency(originalSize: number, simdResult: TextEmbeddingResult): number {
    const compressedSize = ((simdResult.compressedTiles as TextTile[]) || []).reduce(
      (sum: number, tile: TextTile) => sum + (tile.compressedData?.length || 0),
      0
    );
