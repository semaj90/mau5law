...
export interface ExternalChatMessage { // Renamed from ChatMessage
  id?: string;
  role: ChatRole;
  content: string;
...
export interface IQdrantVectorService {
  upsertVector(id: string, vector: Float32Array | number[], metadata?: Record<string, unknown>): Promise<void>;
  upsertBatch(items: Array<{ id: string; vector: Float32Array | number[]; metadata?: Record<string, unknown> }>): Promise<void>;
