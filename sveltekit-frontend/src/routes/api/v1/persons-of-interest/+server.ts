...
import { db, sql, eq } from '$lib/server/db';
import { personsOfInterest } from '$lib/server/db/schema-postgres';
import { z } from 'zod';
import { authenticateUser } from '$lib/server/api/auth-middleware';
...
class PersonsOfInterestCRUDService {
	...
	async listByRiskLevel(
		...
		const [totalRow] = await db
			.select({ count: sql<number>`count(*)` })
			.from(personsOfInterest)
			.where(eq(personsOfInterest.threatLevel, riskLevel));
...
/* * GET /api/v1/persons-of-interest * List user's persons of interest with pagination and filtering' */
export const GET: RequestHandler = async (event) => {
	...
	try {
		...
		const userId = locals.user!.id; // Add non-null assertion
		...
		const result = validatedQuery.riskLevel
			...
			: await personsService.list({ page: validatedQuery.page, limit: validatedQuery.limit });
		...
		if (err instanceof z.ZodError) {
			return json(
				{ success: false, message: 'Invalid query parameters', code: 'INVALID_QUERY', details: err.errors },
				{ status: 400 }
			);
		}

		return json(
			{ success: false, message: 'Failed to fetch persons of interest', code: 'FETCH_FAILED', details: err instanceof Error ? err.message : String(err) },
			{ status: 500 }
		);
	}
};
...
export const POST: RequestHandler = async (event) => {
	...
	try {
		...
		return json(
			{
				success: true,
				data: createdPerson,
				meta: { personId, userId, caseIds: validatedData.caseIds, timestamp: new Date().toISOString() }
			},
			{ status: 201 }
		);
	} catch (err) {
		...
		if (err instanceof z.ZodError) {
			return json(
				{ success: false, message: 'Invalid person data', code: 'INVALID_DATA', details: err.errors },
				{ status: 400 }
			);
		}
		...
		if (errorMessage.includes('not found') || errorMessage.includes('access denied')) {
			return json(
				{ success: false, message: errorMessage, code: 'ACCESS_DENIED' },
				{ status: 403 }
			);
		}

		return json(
			{ success: false, message: 'Failed to create person of interest', code: 'CREATE_FAILED', details: errorMessage },
			{ status: 500 }
		);
	}
};
