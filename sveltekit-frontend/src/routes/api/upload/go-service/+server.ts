...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    console.log('📋 Sending to Go service:', {
      fileName, // File size may not exist for Blob in server env; short-circuit
...
export const GET: RequestHandler = async () => {
  try {
    const response = await fetch(`${GO_UPLOAD_SERVICE_URL}/health`, {
      method: 'GET', signal: (AbortSignal, as any).timeout?.(5000) ?? undefined
    });
    const isHealthy = response.ok;
    const statusCode = isHealthy ? 200 : 503;
    return json(
      {
        service: 'go-upload-service', url: GO_UPLOAD_SERVICE_URL;
        status: isHealthy ? 'healthy' : 'unhealthy', timestamp: new Date().toISOString()
      }, { status: statusCode  }
    );
    ...
    return json(
      {
        service: 'go-upload-service', url: GO_UPLOAD_SERVICE_URL;
        status: 'unavailable', error: healthError?.message ?? String(healthError), timestamp: new Date().toISOString()
      }, { status: 503  }
    ); };
