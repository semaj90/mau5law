...
export const GET: RequestHandler = async ({ url }) => {
  ...
  endPoint: minioEndpoint.split(':')[0], port: parseInt(minioEndpoint.split(':')[1]) || 9000, useSSL, accessKey, secretKey
    ...
    return json({
      ...
      expires_in: 3600, generated_at: new Date().toISOString()
    });
   }catch (error) {
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  endPoint: minioEndpoint.split(':')[0], port: parseInt(minioEndpoint.split(':')[1]) || 9000, useSSL, accessKey, secretKey
    ...
    return new Response(fileBuffer, {
      headers: {
        'Content-Type': stat.metaData['content-type'] || 'application/octet-stream', 'Content-Length': stat.size.toString(), 'Content-Disposition': 'attachment; filename="${objectPath.split('/').pop()}"`,'`
        'x-amz-meta-original-name': stat.metaData['x-amz-meta-original-name'] || '', 'x-amz-meta-case-id': stat.metaData['x-amz-meta-case-id'] || '', 'x-amz-meta-document-type': stat.metaData['x-amz-meta-document-type'] || '`  }`
    });
   }catch (error) {
    console.error('MinIO download error: ', error);
    return json(
