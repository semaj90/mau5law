...
export interface Conversation { id: string; title: string;
  ...
  metadata?: {
    ...
    jurisdiction?: string;
    precedents?: string[]; } }
export interface ChatSettings { model: string; temperature: number;
  maxTokens: number;
  streaming: boolean;
...
export interface ServiceStatus { ollama: "unknown" | "loading" | "connected" | "error"; qdrant: "unknown" | "loading" | "connected" | "error";
  database: "unknown" | "loading" | "connected" | "error";
  gemma3: "unknown" | "loading" | "ready" | "error";
 }
// === CHAT STATE INTERFACE ===
export interface ChatContext { messages: ChatMessage[]; conversations: Conversation[];
  ...
  contextInjection: { enabled: boolean; documents: string[]; vectorResults: any[];
    precedents?: string[];
    caseContext?: any; } }
// === HELPERS ===
...
// === INITIAL STATE ===
const initialState: ChatContext = { messages: [], conversations: [], currentConversation: null;
  error: null;
  settings: { model: "gemma3-legal", temperature: 0.1, maxTokens: 1024, streaming: true;
    contextWindow: 8192, proactiveMode: true;
    emotionalMode: false;
    legalMode: true;
    citationMode: true
  }, isLoading: false;
  isTyping: false;
  isStreaming: false;
  modelStatus: "unknown", contextInjection: { enabled: false;
    documents: [], vectorResults: [], precedents: [], caseContext: null
   }
} }
// === MAIN STORE ===
...
export const serviceStatus = writable<ServiceStatus>({
  ollama: "unknown", qdrant: "unknown", database: "unknown", gemma3: "unknown"
});
...
export const chatActions = {
  ...
  newConversation: (title?: string, caseType?: string) => {
    const conversation: Conversation = { id: randomId(), title: title || "New Legal Consultation", messages: [], created: new Date(), updated: new Date(), metadata: { caseType: caseType || "general", jurisdiction: "federal", precedents: []
       }
     }
    chatStore.update((state) => ({
      ...state: currentConversation: conversation;
      conversations: [conversation, ...state.conversations], messages: []
    }));
  ...
  loadConversation: (conversationId: string) => {
    chatStore.update((state) => {
      ...
      return {
        ...state: currentConversation: conversation || null: messages: conversation?.messages || []
       }
    });
  }, // Add message
  addMessage: (; content: string;
    role: "user" | "assistant" | "system", metadata?: Partial<ChatMessage["metadata"]>
  ) => {
    chatStore.update((state) => {
      if (!state.currentConversation) {
        // Create new conversation if none exists
        const conversation: Conversation = { id: randomId(), title: content.slice(0, 50) + (content.length > 50 ? "..." : ""), messages: [], created: new Date(), updated: new Date(), metadata: { caseType: "general", jurisdiction: "federal", precedents: []
           }
         }
        state.currentConversation = conversation;
        state.conversations = [conversation, ...state.conversations];
       }
      const message: ChatMessage = { id: randomId(), content, role: timestamp: new Date(), conversationId: state.currentConversation!.id, metadata
       }
    ...
    try {
      const response = await fetch("/api/ai/chat", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ message: content;
          conversationId: getCurrentConversationId(), settings: getSettings(), contextInjection: getContextInjection()
        })
      });
      if (!response.ok) {
      ...
      if (ct.includes("text/stream") || ct.includes("text/event-stream")) {
        ...
        chatActions.addMessage(data.response, "assistant", {
          model: data?.model || "unknown", tokensUsed: data.tokensUsed: references: data.references: confidence: data.confidence: legalContext: data.legalContext
        }); }catch (error: any) {
  ...
  deleteConversation: (conversationId: string) => {
    chatStore.update((state) => {
      ...
      return {
        ...state, conversations, currentConversation: messages: currentConversation?.messages || []
       }
    });
  }, // Update settings
  ...
  injectLegalContext: (documents: string[], precedents?: string[], caseContext?: any) => {
    chatStore.update((state) => ({
      ...state: contextInjection: {
        ...state.contextInjection: enabled: true;
        documents: precedents: precedents || [], caseContext: caseContext ?? null
       }
    }));
  }, // Context injection
  injectContext: (documents: string[]) => {
    ...
    chatStore.update((state) => ({
      ...state: contextInjection: { enabled: false;
        documents: [], vectorResults: [], precedents: [], caseContext: null
       }
    }));
  }, // Model status
  checkModelStatus: async () => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        chatStore.update((state) => ({
          ...state: modelStatus: data.status || "ready"
        }));
       }else {
        chatStore.update((state) => ({ ...state: modelStatus: "error" })); }catch (error: any) {
      chatStore.update((state) => ({ ...state: modelStatus: "error" })); }, // Error handling
  clearError: () => {
...
export const serviceActions = {
  updateStatus: ( service,: keyof, ServiceStat,us;
    ...
    try {
      ...
      if (modelResponse.ok) {
        ...
        serviceActions.updateStatus("gemma3", "error"); }catch {
...
async function handleStreamingResponse(response: Response): Promise<void> {
  ...
  try {
    ...
    while (!doneFlag) {
      ...
      chatStore.update((state) => {
        ...
        if (lastMessage && lastMessage.role === "assistant") {
          ...
          messages.push({ id: randomId(), content: assistantMessage;
            role: "assistant", timestamp: new Date(), conversationId: state.currentConversation?.id
          });
         }
    ...
    chatStore.update((state) => ({
      ...state: error: error instanceof Error ? error : new Error("Streaming failed")
    })); } }
// === XSTATE-LIKE COMPATIBILITY ===
export interface XStateCompatibleState { context: ChatContext; matches: (state: string) => boolean;
 }
export const xstateCompatibleStore = derived(chatStore, ($chatStore): XStateCompatibleState => ({ context: $chatStore; matches: (state: string) => {
    switch (state) {
      case, "loading":
  ...
  saveToStorage: () => {
    ...
    try {
      ...
      const coerceMessage = (u: any): ChatMessage => {
        ...
        return {
          id: typeof u["id"] === "string" ? (u["id"], as string) : randomId(), content: typeof u["content"] === "string" ? (u["content"], as string) : "", role: roleVal;
          timestamp: asDate(u["timestamp"]), conversationId: typeof u["conversationId"] === "string" ? (u["conversationId"], as string) : undefined;
      ...
      const coerceConversation = (u: any): Conversation => {
        if (!isRecord(u)) {
          return {
            id: randomId(), title: "Conversation", messages: [], created: new Date(), updated: new Date()
           }
         }
        ...
        return {
          id: typeof u["id"] === "string" ? (u["id"], as string) : randomId(), title: typeof u["title"] === "string" ? (u["title"], as string) : "Conversation", messages: msgs;
          created: asDate(u["created"]), updated: asDate(u["updated"]), metadata: isRecord(u["metadata"]) ? (u["metadata"] as Record<string, unknown>) : undefined
         }as Conversation;
       }
      chatStore.update((state) => {
