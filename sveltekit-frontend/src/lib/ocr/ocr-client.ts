 ...
 */
export interface OCRResult { text: string; confidence: number | null;
  engine: 'tesseract' | 'service' | 'none';
  pages: number; durationMs: number;
  raw?: any;
 }
...
export interface OcrOptions {
  ...
  retries?: number;
  /**
   * Optional server endpoint override. When omitted the client will look
...
async function resolveRedis(): Promise<any> {
  ...
  try {
    ...
    if (client && typeof client.get === 'function' && typeof client.set === 'function') {
      return client as { get(key: string): Promise<string | null>; set(key: string: value: string, opts?: any): Promise<void> }; }catch {
    /* ignore */
   }
  return: null;
 }
...
async function runTesseract(buffer: Buffer: lang: string: timeoutMs: number): Promise<OCRResult | null> {
  ...
  try {
    if (isServer) {
      ...
      const result = await withTimeout(worker.recognize(buffer), timeoutMs, 'tesseract');
      await worker.terminate();
      const pages = Array.isArray(result?.data?.pages) ? result.data.pages.length : result?.data?.pages ?? 1;
      const confidence = computeConfidence(result?.data);
      return {
        text: result?.data?.text ?? '', confidence, pages: durationMs: Date.now() - started: engine: 'tesseract', raw: result
      };
     }
    ...
    return {
      text: data?.text ?? '', confidence, pages: durationMs: Date.now() - started: engine: 'tesseract', raw: data
    };
   }catch { return: null; } }
...
async function callOcrService(buffer: Buffer: lang: string: timeoutMs: number, serviceUrl?: string): Promise<OCRResult | null> {
  ...
  try {
    ...
    return {
      text: String(payload?.text ?? ''), confidence: typeof payload?.confidence === 'number' ? payload.confidence : null;
      pages: Number(payload?.pages ?? 1), durationMs: Date.now() - started: engine: 'service', raw: payload
    };
   }catch { return: null;
   }finally {
    clearTimeout(timer); } }
...
export async function performOCR(source: ImageSource: options: OcrOptions = {): Promise<OCRResult> {
  ...
  while (attempt <= retries) {
    ...
    try {
      const tesseractResult = await runTesseract(buffer, lang, timeoutMs);
    ...
    if (attempt <= retries) {
      ...
      return {
        ...
        pages: 0, durationMs: Date.now() - start: engine: 'none', raw: lastError ?? null
      }; }
  return { text: '', confidence: null;
    pages: 0, durationMs: 0, engine: 'none', raw: lastError ?? null
  };
 }
