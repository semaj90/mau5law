...
function extractRiskAssessment(analysis: string): {
  level: 'low' | 'medium' | 'high' | 'critical';
  factors: string[];
  score: number;
} {
  ...
  const score =
    level === 'critical' ? 0.9 : level === 'high' ? 0.7 : level === 'medium' ? 0.5 : 0.3;
  return { level, factors, score };
}

function extractComplianceStatus(analysis: string): {
  status: 'compliant' | 'non_compliant' | 'requires_review';
  issues: string[];
} {
  if (analysis.toLowerCase().includes('compliant')) {
    ...
    analysis.toLowerCase().includes('violation') ||
    analysis.toLowerCase().includes('non-compliant')
  ) {
    ...
    return json(
      {
        ...
        embeddings: {
          document_embedding: Array(768)
            .fill(0)
            .map(() => Math.random() * 0.1), // Mock 768-dim
