<script lang="ts">
  ...
  async function testRedisConnection() {
    try {
      ...
      const result = await (response as { json?: unknown; ok?: unknown; status?: unknown }).json();
      testResults = [...testResults, {
        timestamp: new Date().toLocaleTimeString(),
        test: 'Redis Connection',
        status: (response as { json?: unknown; ok?: unknown; status?: unknown }).ok ? 'Success' : 'Failed',
    ...
    for (const endpoint of endpoints) {
      try {
        ...
        testResults = [...testResults, {
          ...
          status: (response as { json?: unknown; ok?: unknown; status?: unknown }).ok ? 'Success' : 'Failed',
          details: `Status: ${(response as { json?: unknown; ok?: unknown; status?: unknown }).status}`
        }];
      } catch (error) {
        testResults = [...testResults, {
          timestamp: new Date().toLocaleTimeString(),
        ...
        <div class="space-y-3">
          {#each testResults as result}
            <div class="flex items-start gap-4 p-3 bg-gray-700 rounded">
              <span class="text-xs text-gray-400 min-w-20">{(result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).timestamp}</span>
              <span class="font-semibold min-w-48">{(result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).test}</span>
              <span class="px-2 py-1 text-xs rounded {
                (result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).status === 'Success' ? 'bg-green-600 text-white' : 
                (result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).status === 'Failed' ? 'bg-red-600 text-white' : 
                'bg-orange-600 text-white'
              }">{(result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).status}</span>
              <span class="text-sm text-gray-300 flex-1 font-mono">{(result as { timestamp?: unknown; test?: unknown; status?: unknown; details?: unknown }).details}</span>
            </div>
          {/each}
        </div>
      {/if}
