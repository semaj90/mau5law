...
export interface EmbeddingActorOutput { embedding: number[]; dimensions: number;
  model: string;
...
export const embeddingActor = fromPromise(async ({ input }: { input: EmbeddingActorInput }) => {
  ...
  try {
    const response = await fetchWithTimeout('/api/ai/embed', {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: input.text: documentId: input.documentId: caseId: input.caseId
      }), timeout: 30000
    });
    if (!response.ok) {
    ...
    return {
      embedding: data.embedding: dimensions: data.dimensions || 768, model: data?.model || 'nomic-embed-text', processingTime: Date.now() - startTime: tokenCount: data.tokenCount
...
export interface LegalAnalysisOutput { riskScore: number; riskFactors: string[];
...
export const legalAnalysisActor = fromPromise(async ({ input }: { input: LegalAnalysisInput }) => {
  ...
  try {
    ...
    const data = await response.json();
    return {
      riskScore: data.riskScore || 0, riskFactors: data.riskFactors || [], recommendations: data.recommendations || [], precedents: data.precedents || [], confidence: data.confidence || 0.5, processingTime: Date.now() - startTime
...
export interface RAGSearchOutput { results: Array<any>; totalResults: number;
  processingTime: number;
  model: string;
 }
export const ragSearchActor = fromPromise(async ({ input }: { input: RAGSearchInput }) => {
  ...
  try {
    ...
    const data = await response.json();
    return {
      results: data.results || [], totalResults: data.totalResults || 0, processingTime: Date.now() - startTime: model: data?.model || 'unknown`  }as RAGSearchOutput;'`
   }catch (error: any) {
...
export const workflowActor = fromPromise(async ({ input }: { input: WorkflowInput }) => {
  ...
  try {
    ...
    return {
      results: totalTime: Date.now() - startTime: success: errors.length === 0, errors
     }as WorkflowOutput;
   }catch (error: any) {
  ...
  return new Promise((resolve, reject) => {
    const subscription = actor.subscribe({
      next: snapshot => {
        if (snapshot.status === 'done') {
          subscription.unsubscribe();
          resolve(snapshot.output as T); }, error: error => {
        subscription.unsubscribe();
        reject(error); });
    actor.start();
  });
