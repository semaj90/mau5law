...
<div
  ...
  class={`
    relative overflow-hidden border-2 rounded-lg
        ...
        {#if item.connections}
          {#each item.connections as connectionId}
            <path
              ...
              fill="none"
              class={themeStyles[theme].connection}
              stroke-width="2"
  ...
  {#each items as item (item.id)}
    <div
      class={`
        ...
        ${selectedItems.has(item.id) ? 'ring-2 ring-blue-500 ring-opacity-60' : ''}
        ${draggedItem?.id === item.id ? 'z-50 shadow-2xl scale-105' : 'z-10'}
        ${isConnecting && connectionStart !== item.id ? 'cursor-crosshair' : ''}
        ${theme === 'yorha' ? 'backdrop-blur-sm' : ''}
      `}
      style="
      ...
      <div class="flex items-center justify-between mb-2">
        <div class="flex items-center space-x-2">
          ...
          <h4 class={`
            ...
            ${theme === 'yorha' ? 'font-mono tracking-wide' : ''}
          `}>
            {item.title}
          </h4>
        </div>
        ...
        {#if !readonly}
          <button
            ...
            class={`
              w-5 h-5 rounded text-xs opacity-50 hover:opacity-100 transition-opacity
              ${theme === 'yorha' ? 'hover:bg-green-400/20 text-green-400' : 'hover:bg-gray-200 dark:hover:bg-gray-700'}
            `}
            title="Delete item"
          >
            ✕
      ...
      {#if item.connections && item.connections.length > 0}
        <div class="absolute -top-1 -right-1">
          <div class={`
            ...
            ${theme === 'yorha'
              ? 'bg-green-400 text-black shadow-[0_0_8px_rgba(0,255,65,0.6)]'
  ...
  {#if items.length === 0}
    <div class="absolute inset-0 flex items-center justify-center">
      ...
      `}>
        ...
        <h3 class="text-lg font-semibold mb-2">Evidence Board</h3>
        <p class="text-sm">
          Click toolbar buttons to add evidence items<br>
...
<style>
  ...
  @keyframes yorha-item-glow {
    ...
    to {
      box-shadow: 0 0 25px rgba(0, 255, 65, 0.5), 0 0 35px rgba(0, 255, 65, 0.2);
    }
  }
</style>