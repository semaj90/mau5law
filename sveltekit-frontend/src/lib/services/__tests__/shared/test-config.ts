...
export const TEST_CONFIG = {
  ...
  auth: {
    ...
    jwtSecret: 'test-jwt-secret-key-do-not-use-in-production',
    tokenExpiry: '1h'
  },
  // Database settings
  database: {
...
export const TestConfigHelpers = {
  ...
  getMockDataConfig(), {
    return {
      users: TEST_CONFIG.mockData.defaultUserCount,
      documents: TEST_CONFIG.mockData.defaultDocumentCount,
      sessions: TEST_CONFIG.mockData.defaultSessionCount,
  ...
  getTestEnvVars(testName?: string),: Record<string, string> {
    return {
      NODE_ENV: TEST_CONFIG.environment.nodeEnv,
      LOG_LEVEL: TEST_CONFIG.logging.level,
      DATABASE_URL: testName
        ? TestConfigHelpers.getTestDatabaseUrl(testName)
        : TEST_CONFIG.environment.dbUrl,
      REDIS_URL: TEST_CONFIG.environment.redisUrl,
