...
export async function withTransaction<T>(
    ...
    try {
        ...
        // NOTE: Drizzle specific syntax varies. Assuming raw execute for setting isolation level if needed,
        // or just using the transaction callback.

        // This execution might be driver-specific (Postgres)
        // await db.execute(sql`SET TRANSACTION ISOLATION LEVEL ${sql.raw(isolationLevel)}`);

        // Execute the transaction function
        const result = await db.transaction(async (tx) => {
             // If we really need isolation level per transaction, we might need to set it inside the tx
...
export class TransactionManager {
    ...
    async createSavepoint(name?: string): Promise<string> {
        ...
        // Drizzle doesn't have direct savepoint API top-level usually, depends on driver.
        // Assuming raw SQL execution is available via db.execute or similar
        // @ts-ignore
...
export async function handleConstraintViolation(
    ...
    const isConstraintViolation = error instanceof Error &&
