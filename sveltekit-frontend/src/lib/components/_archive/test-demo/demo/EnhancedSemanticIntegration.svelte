...
<div
  class="enhanced-semantic-dashboard bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 p-6 rounded-xl text-white min-h-screen"
>
  ...
  {#if $isProcessing}
    <div class="processing-indicator mb-6">
      ...
      <div class="w-full bg-slate-700 rounded-full h-2">
        <div
          class="bg-gradient-to-r from-blue-400 to-purple-400 h-2 rounded-full transition-all duration-300"
      ...
      <div class="space-y-4">
        ...
        {#if $cacheStats}
          ...
          <div class="cache-usage mt-4">
            ...
            <div class="w-full bg-slate-700 rounded-full h-2">
              <div
                class="bg-gradient-to-r from-purple-400 to-blue-400 h-2 rounded-full transition-all duration-300"
                style="width: {($cacheStats.cache_size / $cacheStats.max_size) * 100}%"
              ></div>
            </div>
          </div>
        {:else}
          <div class="text-center py-6">
            <div
              class="animate-spin w-6 h-6 border-2 border-purple-400 border-t-transparent rounded-full mx-auto mb-2"
            ></div>
  ...
  <footer class="mt-8 pt-6 border-t border-slate-700">
    <div class="text-center text-xs text-slate-400">
      ...
      <p class="mt-1">
        Status: {Object.values.filter - length}/{Object.keys(errors).length} services online | Cache: {$webGPUSupported
          ? 'WebGPU Accelerated'
          : 'CPU Optimized'}
