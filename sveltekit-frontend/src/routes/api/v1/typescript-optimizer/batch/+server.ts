...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!body.errors || !Array.isArray(body.errors)) {
      return json(
        { success: false, error: 'Invalid batch request, errors array required' },
        { status: 400 }
      );
    }
    ...
    if (body.errors.length === 0) {
      return json({
        ...
        successful_count: 0,
        results: [],
        processing_stats: { total_time: 0, processed_count: 0, successful_count: 0 },
    ...
    const successRate =
      resultProcessedCount > 0 ? (resultSuccessfulCount / resultProcessedCount) * 100 : 0;
