...
const JURISDICTION_PATTERNS: Record<string, JurisdictionPattern> = {
    'federal': { keywords: ['federal', 'supreme court', 'circuit court', 'district court', 'fda', 'sec', 'ftc'], statutes: ['usc', 'cfr', 'federal register'], weight: 1.0 },
    'state': { keywords: ['state court', 'superior court', 'appellate court'], statutes: ['state code', 'revised statutes'], weight: 0.8 },
    'local': { keywords: ['municipal', 'county court', 'magistrate'], statutes: ['ordinance', 'municipal code'], weight: 0.6 },
    'international': { keywords: ['international court', 'treaty', 'convention'], statutes: ['un charter', 'geneva convention'], weight: 0.9 }
};
...
const TRUSTED_LEGAL_SOURCES = [
    'supreme court opinions', 'circuit court decisions', 'federal statutes', 'state case law',
    'legal encyclopedias', 'bar associations', 'law reviews'
];
...
export const POST: RequestHandler = async ({ request }) => {
    ...
    try {
        ...
        if (enableRAGEnhancement) { // Used renamed variable
        ...
        const response = {
            ...
            summary: {
                ...
                averageProsecutionScore: processedDocuments.length > 0 ? processedDocuments.reduce((sum, doc) => sum + doc.prosecutionScore, 0) / processedDocuments.length : 0,
                factCheckResults: {
        ...
        console.log(`🎉 Legal document ingestion complete: ${processedDocuments.length} documents, ${totalTime}ms`);
        ...
        return json(
            { success: false, error: errMsg, processingTime },
            { status: 500 }
        );
    }
};
...
function extractLegalEntities(text: string, jurisdiction: string): LegalEntity[] {
    ...
    Object.entries(LEGAL_ENTITY_PATTERNS).forEach(([type, patterns]) => {
        (patterns as RegExp[]).forEach((pattern: RegExp) => { // Corrected syntax
    ...
    return entities
        .filter(
            (entity: LegalEntity, index: number, self: LegalEntity[]) => // Corrected syntax
...
function calculateLegalRelevance(text: string, entities: string[]): number {
    ...
    const textLower = text.toLowerCase();
    const keywordMatches = legalKeywords.filter((keyword: string) => textLower.includes(keyword.toLowerCase())).length; // Corrected syntax
...
type EmbeddingsResponse =
    ...
    | { data?: Array<Embedding | { embedding?: Embedding } | { vector?: Embedding } | { value?: Embedding } | Record<string, unknown>> }
    ...
    try {
        ...
        if (Array.isArray(payload) && payload.length > 0 && isNumberArray(payload[0])) {
            ...
            if (hasEmbeddingsProp(obj)) {
                embeddingsArray = (obj.embeddings as unknown[]).map(normalizeItem).filter((e): e is Embedding => e !== null); // Corrected syntax
            } else if (hasDataProp(obj)) {
                // Typical OpenAI-like shape: { data: [...] }
                embeddingsArray = (obj.data as unknown[]).map(normalizeItem).filter((e): e is Embedding => e !== null); // Corrected syntax
            } else {
        ...
        return chunks.map((chunk: DocumentChunk, i: number) => ({ ...chunk, embedding: embeddingsArray[i] })); // Corrected syntax
    } catch (err: unknown) { // Corrected syntax
        console.warn('Embedding service unavailable or failed; falling back to random embeddings: ', err); // Corrected message
        ...
        return chunks.map(chunk => ({ ...chunk, embedding: Array.from({ length: dim }, () => Math.random() - 0.5) })); // Corrected syntax
    }
}
...
function performFactChecking(entities: LegalEntity[], jurisdiction: string): FactCheck[] {
    ...
    const claims = entities.filter((entity: LegalEntity) => entity.type === 'WHAT' || entity.type === 'WHY').slice(0, 5); // Corrected syntax

    // Limit for performance
    claims.forEach((entity: LegalEntity) => { // Corrected syntax
...
function calculateProsecutionScore(
    ...
    const avgChunkRelevance = chunks.length > 0 ? chunks.reduce((sum, chunk) => sum + (chunk.legalRelevance || 0), 0) / chunks.length : 0; // Corrected syntax
...
function calculateCaseAISummaryScore(documents: LegalDocument[]): number {
    ...
    const avgEntityCount = documents.reduce((sum, doc) => sum + doc.entities.length, 0) / documents.length; // Corrected syntax
