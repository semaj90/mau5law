...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; // Use a flexible type for items to avoid many repetitive: unknown casts type GalleryItem = any; // Gallery state let mediaItems = $state<GalleryItem[]>([]); let filteredItems = $state<GalleryItem[]>([]); let isLoading = $state<boolean>(false); let error = $state<string | null>(null); // Filter and view options let searchQuery = $state<string>(''); let selectedCategory = $state<'all' | 'evidence' | 'images' | 'documents' | 'ai-generated'>('all'); let selectedCaseId = $state<string>('all'); let viewMode = $state<'grid' | 'list' | 'masonry'>('grid'); let sortBy = $state<'date' | 'name' | 'type' | 'case'>('date'); let sortOrder = $state<'asc' | 'desc'>('desc'); // UI state let selectedItem = $state<GalleryItem | null>(null); let showUploadModal = $state<boolean>(false); let availableCases = $state<any[]>([]); // Gallery stats let galleryStats = $derived(() => { return { total: mediaItems.length, evidence: mediaItems.filter((it: GalleryItem) => it.category === 'evidence').length, images: mediaItems.filter((it: GalleryItem) => it.category === 'images').length, documents: mediaItems.filter((it: GalleryItem) => it.category === 'documents').length, aiGenerated: mediaItems.filter((it: GalleryItem) => it?.metadata?.aiGenerated).length }; }); // Filtered and sorted items let processedItems = $derived(() => { let items = [...mediaItems]; // Filter by category if (selectedCategory !== 'all') { if (selectedCategory === 'ai-generated') { items = items.filter((it: GalleryItem) => it?.metadata?.aiGenerated); } else { items = items.filter((it: GalleryItem) => it.category === selectedCategory); }
	...
	function isAudioItem(it: GalleryItem): boolean { return it?.type === 'audio'; }
</script> <svelte:head> <title>Gallery - Legal Case Management</title> </svelte:head> <div class="gallery-page"> <!-- Header --> <header class="gallery-header nes-container"> <div class="header-content"> <div class="header-info"> <h1>📁 Media Gallery</h1> <p class="gallery-description"> Unified gallery for all case media, evidence, generated images, and documents </p> <div class="gallery-stats"> <span class="stat-item">Total: {galleryStats.total}</span> <span class="stat-item nes-badge">Evidence: {galleryStats.evidence}</span> <span class="stat-item nes-badge">Images: {galleryStats.images}</span> <span class="stat-item nes-badge">Documents: {galleryStats.documents}</span> <span class="stat-item nes-badge">AI Generated: {galleryStats.aiGenerated}</span> </div> </div> <div class="header-actions"> <button class="nes-btn"
					onclick={() => showUploadModal = true} >
					📤 Upload Files </button> <button class="nes-btn"
					onclick={() => loadGalleryData()} disabled={ isLoading } >
					{isLoading ? '🔄': '↻'} Refresh </button> </div> </div> </header> <!-- Filters, and, Controls --> <div class="gallery-controls nes-container"> <div class="controls-grid"> <!-- Search --> <div class="control-group"> <label class="nes-text" for="search">Search:</label> <input id="search"
					type="text"
					class="nes-input"
					...
					{sortOrder === 'desc' ? '↓': '↑'} </button> </div> <!-- View, Mode --> <div class="control-group"> <button class={ `nes-btn ${viewMode === 'grid' ? 'is-primary': 'is-normal'}` } onclick={() => viewMode = 'grid'} >
					...
					⊡ </button> </div> <!-- Clear, Filters --> <div class="control-group"> <button class="nes-btn"
					onclick={ clearFilters } >
					🗑️ Clear </button> </div> </div> </div> <!-- Loading, State --> {#if isLoading} <div class="loading-state nes-container"> <div class="loading-content"> <div class="nes-spinner"></div> <p>Loading gallery...</p> </div> </div> {/if} <!-- Error, State --> {#if error} <div class="error-state nes-container"> <p>❌ { error }</p> <button class="nes-btn" onclick={() => loadGalleryData()}> Retry </button> </div> {/if} <!-- Gallery, Content --> {#if !isLoading && !error} {#if filteredItems.length === 0} <div class="empty-state nes-container"> <h3>No Items Found</h3> <p> {searchQuery || selectedCategory !== 'all' || selectedCaseId !== 'all'
						? 'No items match your current filters.': 'No media items in the gallery yet.'} </p> <div class="empty-actions"> <button class="nes-btn"
						onclick={() => showUploadModal = true} >
						📤 Upload Files </button> {#if searchQuery || selectedCategory !== 'all' || selectedCaseId !== 'all'} <button class="nes-btn is-normal"
							onclick={ clearFilters } >
							Clear Filters </button> {/if} </div> </div> {:else} <div class="gallery-grid"> {#each Array.isArray(filteredItems) ? filteredItems: [] as item} <div class="gallery-item nes-container"> <div class="item-preview" role="button" tabindex="0"
					...
					onclick={ closeModal }> <div class="modal-content detail-modal nes-container" role="button" tabindex="0"
					on:click|stopPropagation={() => {}}> <div class="modal-header"> <h3>{selectedItem.title || 'Gallery Item'}</h3> <button class="nes-btn" onclick={ closeModal }>×</button> </div> <div class="modal-body"> <div class="detail-content"> {#if selectedItem.type === 'image' || selectedItem.category === 'images'} <img src={getItemPreview(selectedItem)} alt={selectedItem.title} class="detail-image"
