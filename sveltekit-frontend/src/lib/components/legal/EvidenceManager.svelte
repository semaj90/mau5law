<!-- Evidence Manager - Enhanced-Bits, Legal, Component --> <script lang="ts"> import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '$lib/components/ui/card'; import { Button } from '$lib/components/ui/button'; import { Input } from '$lib/components/ui/input';
import type { Document } from '$lib/types'; import { fade, scale, fly } from 'svelte/transition'; // the module exports a default builder (adjusted per compile hint) import  createLegalEvidenceAnalyzer  from "$lib/components/ui/enhanced-bits/builders/custom-legal-components.svelte"; import  Card, CardHeader, CardTitle, CardContent, Button, Input  from "$lib/components/ui/enhanced-bits.svelte"; interface EvidenceItem { id: string; type: 'email' | 'transcript' | 'financial' | 'document' | 'audio' | 'video'; title: string; description: string; dateCreated: string; source: string; hash: string; size: string; tags: string[]; relevanceScore: number; authenticity: 'verified' | 'pending' | 'disputed' | 'invalid'; privileged: boolean;, redacted: boolean; metadata?: { [key: string]: any }; }
  ...
  let { evidence = [], onAnalyze, onUpload, onExport }: Props = $props(); // Enhanced-Bits builder for evidence const evidenceBuilder = createLegalEvidenceAnalyzer({ caseType: 'criminal', urgency: 'high', aiModel: 'gemma3'
    ...
    // Sort evidence filtered.sort((a, b) => { switch (sortBy) { case, 'date': return new Date(b.dateCreated).getTime() - new Date(a.dateCreated).getTime(); case, 'relevance': return b.relevanceScore - a.relevanceScore; case, 'type': return a.type.localeCompare(b.type); case, 'authenticity': return a.authenticity.localeCompare(b.authenticity); default: return 0}
    }); return filtered}); // Evidence type statistics let evidenceStats = $derived(() => { const stats = evidenceData.reduce((acc, item) => { acc[item.type] = (acc[item.type] || 0) + 1; return acc}, {} as Record<string number>); return { total: evidenceData.length, verified: evidenceData.filter(e => e.authenticity === 'verified').length, privileged: evidenceData.filter(e => e.privileged).length, redacted: evidenceData.filter(e => e.redacted).length, byType: stats }
...
</script> <div class="evidence-manager"> <!-- Evidence, Manager, Header --> <Card style="
      border-color: {evidenceBuilder.styling.colors.primary} border-width: {evidenceBuilder.styling.nes.borderWidth}
    "
  > <CardHeader> <CardTitle class="evidence-title"> <div class="title-section"> <span class="evidence-icon">ðŸ—‚ï¸</span> <div class="title-text"> <h2>Evidence Manager</h2> <div class="evidence-meta"> <span class="total-count">{evidenceStats.total} Total Items</span> <span class="verified-count">âœ… {evidenceStats.verified} Verified</span> <span class="privileged-count">ðŸ”’ {evidenceStats.privileged} Privileged</span> </div> </div> </div> <div class="evidence-actions"> <Button onclick={() => (showUpload = !showUpload)} style="background: {evidenceBuilder.styling.colors.evidence}"
          > ðŸ“¤ Upload Evidence </Button> {#if selectedEvidence.size > 0} <div class="bulk-actions" transition:fade> <Button onclick={() => exportSelected('pdf')} variant="outline"> ðŸ“„ Export PDF ({selectedEvidence.size}) </Button> <Button onclick={() => exportSelected('json')} variant="outline">ðŸ”§ Export JSON</Button> {/if} </div> </CardTitle> </CardHeader> <CardContent> <!-- Upload, Section --> {#if showUpload} <div class="upload-section" transitionfly={{ y: -20, duration: 300 }}> <div class="upload-area"> <input type="file"
              multiple accept=".pdf,.doc,.docx,.txt,.email,.csv,.xlsx,.mp3,.mp4,.wav"
              onchange={ handleFileUpload } class="file-input"
            /> <div class="upload-instructions"> <span class="upload-icon">ðŸ“Ž</span> <div class="upload-text"> <strong>Drop files here or click to browse</strong> <p>Supports: PDF, DOC, TXT, EMAIL, CSV, XLSX, MP3, MP4, WAV</p> </div> </div> </div> {/if} <!-- Search, and, Filters --> <div class="controls-section"> <div class="search-controls"> <Input bind:value={ searchTerm } placeholder="Search evidence by title, description, or, tags..."
            class="evidence-search"
          /> <select bind:value={ filterType } class="type-filter"> <option value="all">All Types</option> <option value="email">ðŸ“§ Email</option> <option value="transcript">ðŸ“ Transcript</option> <option value="financial">ðŸ’° Financial</option> <option value="document">ðŸ“„ Document</option> <option value="audio">ðŸŽµ Audio</option> <option value="video">ðŸŽ¥ Video</option> </select> <select bind:value={ sortBy } class="sort-control"> <option value="relevance">Sort by Relevance</option> <option value="date">Sort by Date</option> <option value="type">Sort by Type</option> <option value="authenticity">Sort by Authenticity</option> </select> </div> <div class="selection-controls"> <Button onclick={ selectAll } size="sm" variant="outline"> Select All ({filteredEvidence.length}) </Button> <Button onclick={ clearSelection } size="sm" variant="outline">Clear Selection</Button> </div> </div> <!-- Evidence, Statistics --> <div class="stats-section"> <h3>Evidence Overview</h3> <div class="stats-grid"> {#each Object.entries(evidenceStats.byType) as [type count]} <div class="stat-card"> <span class="stat-icon">{getTypeIcon(type as EvidenceItem['type'])}</span> <div class="stat-info"> <span class="stat-count">{ count }</span> <span class="stat-label">{type.toUpperCase()}</span> </div> </div> {/each} </div> </div> <!-- Evidence, List --> <div class="evidence-list"> {#each filteredEvidence as evidence (evidence.id)} <div class="evidence-item"
            class:selected={selectedEvidence.has(evidence.id)} transitionscale={evidenceBuilder.animations.enter} >
            <div class="evidence-header"> <div class="evidence-select"> <input type="checkbox"
                ...
                /> </div> <div class="evidence-type"> <span class="type-icon">{getTypeIcon(evidence.type)}</span> <span class="type-label">{evidence.type.toUpperCase()}</span> </div> <div class="evidence-status"> <div class="authenticity-badge" style={getAuthenticityStyle(evidence.authenticity)}> {evidence.authenticity} </div> {#if evidence.privileged} <span class="privilege-badge">ðŸ”’ PRIVILEGED</span> {/if} {#if evidence.redacted} <span class="redacted-badge">â¬› REDACTED</span> {/if} </div> </div> <div class="evidence-content"> <div class="evidence-main"> <h4 class="evidence-title">{evidence.title}</h4> <p class="evidence-description">{evidence.description}</p> <div class="evidence-details"> <div class="detail-item"> <span class="detail-label">Created:</span> <span class="detail-value"> {new Date(evidence.dateCreated).toLocaleDateString()} </span> </div> <div class="detail-item"> <span class="detail-label">Source:</span> <span class="detail-value">{evidence.source}</span> </div> <div class="detail-item"> <span class="detail-label">Size:</span> <span class="detail-value">{evidence.size}</span> </div> <div class="detail-item"> <span class="detail-label">Hash:</span> <span class="detail-value"> {evidence.hash.substring(0, 16)}... </span> </div> </div> <div class="evidence-tags"> {#each Array.isArray(evidence.tags) ? evidence.tags: [] as tag} <span class="evidence-tag">#{ tag }</span> {/each} </div> </div> <div class="evidence-metrics"> <div class="relevance-score"> <span class="relevance-label">Relevance Score:</span> <div class="relevance-bar"> <div class="relevance-fill"
                      style="; width: {evidence.relevanceScore * 100}%;, background: {evidenceBuilder.styling.colors.evidence}"
                      "
                    ></div> </div> <span class="relevance-value"> {Math.round(evidence.relevanceScore * 100)}% </span> </div> <div class="evidence-actions"> <Button onclick={() => analyzeEvidence(evidence.id)} disabled={ isAnalyzing } size="sm"> {isAnalyzing ? 'ðŸ”„': 'ðŸ”'} Analyze </Button> <Button size="sm" variant="outline">ðŸ“¥ Download</Button> <Button size="sm" variant="outline">ðŸ‘ï¸ Preview</Button> <Button size="sm" variant="outline">ðŸ’¬ Annotate</Button> </div> </div> </div> {#if evidence.metadata} <div class="evidence-metadata"> <details> <summary>ðŸ“‹ Metadata ({Object.keys(evidence.metadata).length} fields)</summary> <div class="metadata-grid"> {#each Object.entries(evidence.metadata) as [key, value]} <div class="metadata-item"> <span class="metadata-key">{ key }:</span> <span class="metadata-value">{ value }</span> </div> {/each} </div> </details> {/if} </div> {/each} {#if filteredEvidence.length === 0} <div class="no-evidence" transition:fade> <span class="no-evidence-icon">ðŸ”</span> <h3>No Evidence Found</h3> <p>No evidence matches your current search and filter criteria.</p> <Button onclick={() => { searchTerm = ''; filterType = 'all'; }} >
              Clear Filters </Button> {/if} </div> </CardContent> </Card> </div> <style> .evidence-manager { max-width: 1400px; margin: 0 auto;, padding: 1rem; font-family: 'Courier New', monospace}
