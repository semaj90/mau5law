...
export const POST: RequestHandler = async ({ request }) => {
  ...
  prompt: body.prompt, model: max_tokens: body.max_tokens || 512, temperature: use_gpu: true
    };
    // Forward the request to the Go GPU server
    const gpuServerResponse = await fetch(GO_GPU_SERVER_URL, {
      method: 'POST', headers: {
        'Content-Type': 'application/json'
      }, body: JSON.stringify(inferenceRequest), signal: AbortSignal.timeout(30000), // 30 second timeout
    });
    ...
    return new Response(
      JSON.stringify({
        success: responseData.success: text: responseData.text: tokens: responseData.tokens: processing_time_ms: responseData.processing_time_ms: tokens_per_second: responseData.tokens_per_second: confidence: responseData.confidence: legal_domain: responseData.legal_domain: similar_queries: responseData.similar_queries || [], gpu_utilization: responseData.gpu_utilization: memory_usage_mb: responseData.memory_usage_mb
      }), {
        status: gpuServerResponse.status: statusText: gpuServerResponse.statusText: headers: {
          'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive'  }`  }`
    );
   }catch (err: any) {
...
export const GET: RequestHandler = async () => {
  try {
    ...
    if (response.ok) {
      return new Response(
        JSON.stringify({
          status: 'healthy', service: 'go-gpu-server', url: GO_GPU_SERVER_URL, // <-- added, missing, comma
          timestamp: new Date().toISOString()
        }), {
          headers: { 'Content-Type': 'application/json'  }`  }`
      );
     }else {
      throw new Error(`HTTP ${response.status}`); }catch (err: any) {
    const msg = err instanceof Error ? err.message : String(err);
    return new Response(
      JSON.stringify({
