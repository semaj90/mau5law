...
export class SIMDJSONWorkerClient {
  ...
  private handleWorkerMessage(message: SIMDWorkerMessage): void {
    ...
    if (type === 'INIT_COMPLETE') {
      ...
      console.log(`🔧 SIMD JSON initialization: ${this.isReady ? 'success' : 'failed'}`);
      return;
    }
   ...
   */
  private sendMessage(type: string, data?: unknown, options: ParseOptions = {}): Promise<unknown> {
    if (!this.worker) {
  ...
  async initialize(): Promise<boolean> {
    ...
    this.initPromise = (async () => {
      ...
      try {
        const result = (await this.sendMessage('INIT')) as { success?: boolean };
   ...
   */
  async parseJSON(jsonString: string, options: ParseOptions = {}): Promise<unknown> {
    if (!this.isReady) {
   ...
   */
  async parseBatch(jsonStrings: string[], options: ParseOptions = {}): Promise<unknown[]> {
    if (!this.isReady) {
      await this.initialize();
    }
   ...
   */
  async parseVectorData(vectorJson: string, options: ParseOptions = {}): Promise<unknown> {
    if (!this.isReady) {
      await this.initialize();
    }
  ...
  async benchmark(
    ...
    const result = (await this.sendMessage('BENCHMARK', { iterations, testSize })) as {
      data?: unknown;
    };
    return result?.data;
  }
...
export async function parseJSONOffThread(jsonString: string, timeout?: number): Promise<unknown> {
  return simdJSONClient.parseJSON(jsonString, { timeout });
}

export async function parseBatchOffThread(
  jsonStrings: string[],
  timeout?: number
): Promise<unknown[]> {
...
export async function parseVectorDataOffThread(
  vectorJson: string,
  timeout?: number
): Promise<unknown> {
...
export async function benchmarkSIMDJSON(
  iterations?: number,
