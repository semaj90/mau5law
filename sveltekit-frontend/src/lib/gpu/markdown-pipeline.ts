...
export class GPUMarkdownPipeline {
   ...
   */
  async processDocument(markdown: string, options: {
    ...
    try {
      ...
      if (options.cache !== false && this.documentCache.has(cacheKey)) {
        this.metrics.cacheHitRate = (this.metrics.cacheHitRate * (this.metrics.processedDocuments - 1) + 1) / this.metrics.processedDocuments;
      ...
      this.metrics.averageProcessingTime =
        (this.metrics.averageProcessingTime * (this.metrics.processedDocuments - 1) + processingTime) / this.metrics.processedDocuments;
   ...
   */
  async processBatch(documents: string[], options: {
    includeEmbeddings?: boolean;
    priority?: 'low' | 'normal' | 'high';
  } = {}): Promise<MarkdownProcessingResult[]> {
    ...
    for (const batch of batches) {
      ...
      const batchResults = await this.processWithConcurrency(batchPromises, this.config.maxConcurrency);
...
export class LegalDocumentProcessor {
   ...
   */
  async generateSemanticChunks(markdown: string): Promise<Array<{
    content: string;
    type: string;
    embedding?: number[];
