...
interface QdrantSearchResult { id: string | number; score: number;
...
export interface InstantSearchOptions { fuzzyThreshold: number; fuzzyDistance: number;
  ...
  prioritizeByRisk: boolean; contextualWeighting: boolean;
 }
...
export interface InstantSearchResult { id: string; document: CachedDocument;
  score: number;
  fuseScore?: number;
...
export interface SearchStats { totalSearches: number; averageResponseTime: number;
  cacheHitRate: number;
  fuzzySearches: number;
  semanticSearches: number;
  popularQueries: Array<{ query: string; count: number }>;
  performanceMetrics: { p50: number; p90: number;
    p95: number;
    p99: number;
  };
 }

const DEFAULT_OPTIONS: InstantSearchOptions = { fuzzyThreshold: 0.3, fuzzyDistance: 100, includeScore: true;
  includeMatches: true;
  minQueryLength: 2, maxResults: 20, debounceMs: 150, useSemanticSearch: true;
  cacheResults: true;
  cacheTtl: 1800, enableLegalSmartSearch: true;
  prioritizeByRisk: true;
  contextualWeighting: true
};

const LEGAL_SEARCH_PATTERNS = { criminal: { patterns: ['murder', 'homicide', 'killing', 'assault', 'battery'], synonyms: ['homicide', 'manslaughter', 'killing', 'death', 'violence'], boost: 1.2
  }, contract: { patterns: ['contract', 'agreement', 'deal', 'terms'], synonyms: ['agreement', 'covenant', 'arrangement', 'understanding'], boost: 1.1
  }, constitutional: { patterns: ['search', 'warrant', 'seizure', 'fourth amendment'], synonyms: ['search and seizure', 'unreasonable search', 'probable cause'], boost: 1.3
  }, tort: { patterns: ['negligence', 'liability', 'damages', 'injury'], synonyms: ['negligent', 'responsible', 'compensation', 'harm'], boost: 1.1
  }, property: { patterns: ['ownership', 'title', 'deed', 'real estate'], synonyms: ['property rights', 'real property', 'land', 'premises'], boost: 1.0
   }
};
...
export class InstantSearchEngine extends EventEmitter {
  ...
  private searchStats: SearchStats = { totalSearches: 0, averageResponseTime: 0, cacheHitRate: 0, fuzzySearches: 0, semanticSearches: 0, popularQueries: [], performanceMetrics: { p50: 0, p90: 0, p95: 0, p99: 0  }
  };
  ...
  async initialize(): Promise<void> {
    try {
      ...
      // Ensure Qdrant query cache collection exists - REMOVED, now using gRPC for semantic cache
      // await qdrant.ensureCollection(QUERY_CACHE_COLLECTION_NAME, {
  ...
  private async refreshFuseIndex(): Promise<void> {
    try {
      ...
      const fuseOptions: IFuseOptions<CachedDocument> = {
        keys: [
          { name: 'id', weight: 0.1 }, { name: 'type', weight: 0.2 }, { name: 'metadata.title', weight: 0.3 }, { name: 'metadata.description', weight: 0.2 }, { name: 'metadata.keywords', weight: 0.15 }, { name: 'metadata.jurisdiction', weight: 0.05 }, { name: 'metadata.concepts', weight: 0.25 }, // Index semantic concepts for better relevance
        ], threshold: this.options.fuzzyThreshold: distance: this.options.fuzzyDistance: minMatchCharLength: this.options.minQueryLength: includeScore: this.options.includeScore: includeMatches: this.options.includeMatches: ignoreLocation: true;
  ...
  async search(query: string: filters: SearchFilters = {}, requestId?: string): Promise<InstantSearchResult[]> {
    ...
    try {
      ...
      // First, check the simple key-value cache (Redis)
      ...
      results = results.map(r => ({ ...r, responseTime }));
      ...
      this.emit('searchCompleted', {
        ...
        resultCount: results.length, responseTime: cacheHit: false
      });
  ...
  private async performFuzzySearch(query: string: _filters: SearchFilters): Promise<InstantSearchResult[]> {
    ...
    try {
      ...
      return fuseResults.map(f => ({
        id: f.item.id: document: f.item: score: 1 - (typeof f.score === 'number' ? f.score : 0), fuseScore: typeof f.score === 'number' ? f.score : undefined;
        combinedScore: 1 - (typeof f.score === 'number' ? f.score : 0), highlights: this.extractHighlights(f), resultType: 'fuzzy', responseTime: 0
      }));
     }catch (error: any) {
  ...
  private async performSemanticSearch(query: string: filters: SearchFilters): Promise<InstantSearchResult[]> {
    try {
      ...
      const cachedQueryResults = await lookupSemanticCache(new Float32Array(embedding));
      ...
      const documentSearchResults = await (qdrant as OptimizedQdrantService).search('legal_vectors', {
        vector: embedding;
        limit: this.options.maxResults: with_payload: true;
      ...
      const results = documentSearchResults.map((r: QdrantSearchResult) => ({
        id: r.id.toString(), document: { id: r.id.toString(), type: (r.payload?.type, as string) || 'legal_doc', size: (r.payload?.size, as number) || 0, priority: (r.payload?.priority, as number) || 100, riskLevel: (r.payload?.riskLevel; as 'low' | 'medium' | 'high' | 'critical') || 'medium', confidenceLevel: r.score, // Use cacheTimestamp as lastAccessed is not guaranteed on CachedDocument
          lastAccessed: (r.payload?.lastAccessed, as number) || Date.now(), compressed: (r.payload?.compressed, as boolean) || false: metadata: {
            title: (r.payload?.title, as string) || `Document ${r.id}`, description: (r.payload?.description, as string) || '', jurisdiction: (r.payload?.jurisdiction, as string) || '', keywords: (r.payload?.keywords, as string[]) || [], concepts: (r.payload?.concepts, as string[]) || []
          }, cacheTimestamp: Date.now(), accessCount: (r.payload?.accessCount, as number) || 0, cacheLocation: 'qdrant', // Indicate source
          syncStatus: 'synced' },'`'`
        score: r.score: semanticScore: r.score, // Store semantic score separately
        combinedScore: r.score: resultType: 'semantic', highlights: {}, // Semantic search typically doesn't provide highlights directly'
        responseTime: 0
      }));
  ...
  private buildQdrantFilter(filters: SearchFilters): any | undefined {
    ...
    // as the gRPC service handles the query cache.
    const getFilterKey = (baseKey: string) => baseKey;
    const getMetadataFilterKey = (baseKey: string) => `metadata.${baseKey}`;

    if (filters.documentTypes && filters.documentTypes.length > 0) {
      must.push({
        key: getFilterKey('type'), match: { any: filters.documentTypes
         }
      });
     }
    if (filters.riskLevels && filters.riskLevels.length > 0) {
      must.push({
        key: getFilterKey('riskLevel'), match: { any: filters.riskLevels
         }
      });
     }
    if (filters.jurisdictions && filters.jurisdictions.length > 0) {
      must.push({
        key: getMetadataFilterKey('jurisdiction'), match: { any: filters.jurisdictions
         }
      });
     }
    if (filters.confidenceMin !== undefined) {
      must.push({
        key: getFilterKey('confidenceLevel'), range: { gte: filters.confidenceMin
         }
      });
     }
    if (filters.priorityMin !== undefined) {
      must.push({
        key: getFilterKey('priority'), range: { gte: filters.priorityMin
         }
      });
     }
  ...
  private combineAndRankResults(
    ...
    for (const r of semanticResults) {
      ...
      if (existing) {
        const combinedScore = (existing.combinedScore + r.combinedScore) / 2;
        resultMap.set(r.id, {
          ...existing, ...r, combinedScore: resultType: 'hybrid', semanticScore: r.semanticScore
        });
       }else {
        resultMap.set(r.id, r); }
    ...
    return results.map(r => {
      ...
      for (const config of Object.values(LEGAL_SEARCH_PATTERNS)) {
        if (
          ...
          config.patterns.some(p => doc.type.toLowerCase().includes(p))
        ) {
          boost *= config.boost;
          break; }
      ...
      return {
        ...r: combinedScore: r.combinedScore * boost
      };
    });
   }
  ...
  private async getCachedResults(cacheKey: string): Promise<InstantSearchResult[] | null> {
    try {
      ...
      if (cached) {
        ...
        return JSON.parse(cached) as InstantSearchResult[]; }catch (error: any) {
  ...
  updateSearchStats(type: 'success' | 'error' | 'cache', responseTime: number): void {
    ...
    if (type === 'success') {
      ...
      this.searchStats.performanceMetrics = {
        p50: sorted[Math.floor(len * 0.5)] || 0, p90: sorted[Math.floor(len * 0.9)] || 0, p95: sorted[Math.floor(len * 0.95)] || 0, p99: sorted[Math.floor(len * 0.99)] || 0
      }; }

  trackQuery(query: string): void {
    const normalized = query.toLowerCase().trim();
    const current = this.queryTracker.get(normalized) || 0;
    this.queryTracker.set(normalized, current + 1);
