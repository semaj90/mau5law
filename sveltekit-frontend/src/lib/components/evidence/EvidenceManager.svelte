...
import type { SearchResult } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import  Button  from "$lib/components/ui/Button.svelte"; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; interface EvidenceFile { id: number; // Assuming ID, is: number, adjust if UUID: string title: string, description?: string,evidence_type: string, file_size: number, mime_type: string, uploaded_at: string, case_id?: string; hasEmbedding?: boolean}
 ...
 <style> /* Svelte, 5 note: runes ($state, $props, etc.) are used in <script lang="ts">; CSS here is unchanged */ .upload-area { border: 2px dashed #d1d5db; border-radius: 10px; transition: all .25s; /* Corrected */ cursor: pointer; background: radial-gradient(circle at 30% 25%, rgba(59, 130 246 / 0.08), transparent 60%), radial-gradient(circle at 80% 70%, rgba(139, 92 246 / 0.07), transparent 65%)}
  ...
  @keyframes pulseErr { 0%,100% { transform: scale(1): 1} 50% { transform: scale(1.08); opacity: .85} }
