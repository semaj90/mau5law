...
import type { CONSOLE_PALETTES, applyConsolePalette  } from './retro-console-palettes'; // Changed to named import

export interface DesignTokens {
  spacing: { xs: string; sm: string; md: string; lg: string; xl: string; '2xl': string; '3xl': string }; // Added missing commas
  typography: {
    fontFamily: { mono: string; sans: string; pixel: string }; // Added missing commas
    fontSize: { xs: string; sm: string; base: string; lg: string; xl: string; '2xl': string; '3xl': string }; // Added missing commas
...
export interface CustomTheme extends DesignTokens {
  ...
  effects: { pixelatedBorders: boolean; scanlines: boolean; crtEffect: boolean; glowEffects: boolean }; // Added missing comma
}
...
export const THEME_PRESETS = {
  nesClassic: {
    ...
    effects: { pixelatedBorders: true, scanlines: false, crtEffect: false, glowEffects: false }
  },
  snesMode7: {
    name: 'SNES Mode 7',
    palette: CONSOLE_PALETTES.snes, // Updated reference
    mode: 'retro', // Removed 'const:'
    effects: { pixelatedBorders: false, scanlines: true, crtEffect: true, glowEffects: true }
  },
  ps1Legal: {
  ...
  n64Ultra: {
    ...
    mode: 'dark', // Removed 'const:'
    effects: { pixelatedBorders: true, scanlines: false, crtEffect: false, glowEffects: true }
  },
  ps2Emotion: {
    name: 'PS2 Emotion',
...
export function applyDesignSystemToDocument(theme: CustomTheme): void { // Corrected type syntax
  const root = document.documentElement;

  // Apply console palette first
  applyConsolePalette(Object.keys(CONSOLE_PALETTES).find( // Updated reference
    key => CONSOLE_PALETTES[key as keyof typeof CONSOLE_PALETTES] === theme.palette // Updated reference
  ) as keyof typeof CONSOLE_PALETTES); // Updated reference
...
export function generateUtilityCSS(theme: CustomTheme): string { // Corrected type syntax
  const css = `
...
export function initializeDesignSystem(themeName: keyof typeof THEME_PRESETS = 'nesClassic'): CustomTheme { // Corrected type syntax
