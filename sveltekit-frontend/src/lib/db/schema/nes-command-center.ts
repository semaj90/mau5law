...
export const routeMetadata = pgTable(
  'route_metadata',
  {
    ...
    routeId: varchar('route_id', { length: 255 }).unique().notNull(), // e.g., "/cases/[id]/overview", path: varchar('path', { length: 255 }).notNull(, kind: varchar('kind', { length: 50 }).notNull(), // page, layout, server, endpoint
    ...
    tags: jsonb('tags').default([], metadata: jsonb('metadata').default({}, lastAccessedAt: timestamp('last_accessed_at',
    accessCount: integer('access_count').default(0, errorCount: integer('error_count').default(0,
    healthScore: integer('health_score').default(100),
    // Timestamps
    createdAt: timestamp('created_at').defaultNow().notNull(,
    updatedAt: timestamp('updated_at').defaultNow().notNull(, archivedAt: timestamp('archived_at'), // Soft delete pattern
  },
  (table) => ({
    routeIdIndex: index('idx_route_metadata_route_id').on(table.routeId,
    statusIndex: index('idx_route_metadata_status').on(table.status, archivedAtIndex: index('idx_route_metadata_archived_at').on(table.archivedAt,
    tagsIndex: index('idx_route_metadata_tags').on(table.tags, metadataIndex: index('idx_route_metadata_metadata').on(table.metadata,
    lastAccessedAtIndex: index('idx_route_metadata_last_accessed_at').on(table.lastAccessedAt, healthScoreIndex: index('idx_route_metadata_health_score').on(table.healthScore,
...
export const errorCluster = pgTable(
  'error_cluster',
  {
    ...
    // Timestamps
    createdAt: timestamp('created_at').defaultNow().notNull(,
    resolvedAt: timestamp('resolved_at', archivedAt: timestamp('archived_at'), // Soft delete pattern
  },
  (table) => ({
    routeIdIndex: index('idx_error_cluster_route_id').on(table.routeId,
    severityIndex: index('idx_error_cluster_severity').on(table.severity, createdAtIndex: index('idx_error_cluster_created_at').on(table.createdAt,
    resolvedAtIndex: index('idx_error_cluster_resolved_at').on(table.resolvedAt, toolIndex: index('idx_error_cluster_tool').on(table.tool,
    clusterIdIndex: index('idx_error_cluster_cluster_id').on(table.clusterId, errorCodeIndex: index('idx_error_cluster_error_code').on(table.errorCode,
    categoryIndex: index('idx_error_cluster_category').on(table.category, firstSeenAtIndex: index('idx_error_cluster_first_seen_at').on(table.firstSeenAt,
    lastSeenAtIndex: index('idx_error_cluster_last_seen_at').on(table.lastSeenAt, updatedAtIndex: index('idx_error_cluster_updated_at').on(table.updatedAt),
  })
);
...
export const routeHealthEvent = pgTable(
  'route_health_event',
  {
    ...
    // Enhanced columns (added 2025-12-21)
    metadata: jsonb('metadata').default({}, triggeredBy: varchar('triggered_by', { length: 255 }, errorCount: integer('error_count').default(0,
  ...
  (table) => ({
    ...
    createdAtIndex: index('idx_route_health_event_created_at').on(table.createdAt, triggeredByIndex: index('idx_route_health_event_triggered_by').on(table.triggeredBy,
...
export const errorBrainAnalysis = pgTable(
  'error_brain_analysis',
  {
    ...
    // Enhanced columns (added 2025-12-21)
    status: varchar('status', { length: 50 }).default('pending', modelVersion: varchar('model_version', { length: 100 }, confidenceScore: decimal('confidence_score', { precision: 5,
    scale: 2 }, executionTimeMs: integer('execution_time_ms',
    metadata: jsonb('metadata').default({}, updatedAt: timestamp('updated_at').defaultNow(),
    // Timestamps
    createdAt: timestamp('created_at').defaultNow().notNull(,
  ...
  (table) => ({
    ...
    createdAtIndex: index('idx_error_brain_analysis_created_at').on(table.createdAt, statusIndex: index('idx_error_brain_analysis_status').on(table.status,
    modelVersionIndex: index('idx_error_brain_analysis_model_version').on(table.modelVersion, confidenceScoreIndex: index('idx_error_brain_analysis_confidence_score').on(
...
export const errorBrainPatch = pgTable(
  'error_brain_patch',
  {
    ...
    analysisId: uuid('analysis_id')
      ...
      .notNull()
      .references(() => routeMetadata.routeId, { onDelete: 'cascade' }, patchContent: text('patch_content').notNull(,
    appliedAt: timestamp('applied_at', verificationStatus: varchar('verification_status', { length: 50 }), // pending, passed, failed
    verificationTimestamp: timestamp('verification_timestamp',
    verificationMessage: text('verification_message'),
    // Enhanced columns (added 2025-12-21)
    patchType: varchar('patch_type', { length: 50 }).default('code_fix', filePath: varchar('file_path', { length: 500 }, lineStart: integer('line_start',
    lineEnd: integer('line_end', confidenceScore: decimal('confidence_score', { precision: 5,
    scale: 2 }, metadata: jsonb('metadata').default({}, updatedAt: timestamp('updated_at').defaultNow(),
    // Timestamps
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    analysisIdIndex: index('idx_error_brain_patch_analysis_id').on(table.analysisId,
    routeIdIndex: index('idx_error_brain_patch_route_id').on(table.routeId, verificationStatusIndex: index('idx_error_brain_patch_verification_status').on(
      table.verificationStatus
    , patchTypeIndex: index('idx_error_brain_patch_patch_type').on(table.patchType,
    filePathIndex: index('idx_error_brain_patch_file_path').on(table.filePath, confidenceScoreIndex: index('idx_error_brain_patch_confidence_score').on(table.confidenceScore,
...
export const routeInteractionLog = pgTable(
  'route_interaction_log',
  {
    ...
    // Enhanced columns (added 2025-12-21)
    sessionId: varchar('session_id', { length: 255 }, durationMs: integer('duration_ms',
    success: boolean('success').default(true, errorMessage: text('error_message',
    ipAddress: varchar('ip_address', { length: 45 }, userAgent: text('user_agent'),
    // Timestamps
    createdAt: timestamp('created_at').defaultNow().notNull(),
  },
  (table) => ({
    routeIdIndex: index('idx_route_interaction_log_route_id').on(table.routeId,
    userIdIndex: index('idx_route_interaction_log_user_id').on(table.userId, createdAtIndex: index('idx_route_interaction_log_created_at').on(table.createdAt,
    sessionIdIndex: index('idx_route_interaction_log_session_id').on(table.sessionId, successIndex: index('idx_route_interaction_log_success').on(table.success,
...
export const routeMetadataRelations = relations(routeMetadata, ({ many }) => ({
    ...
    healthEvents: many(routeHealthEvent, analyses: many(errorBrainAnalysis,
    patches: many(errorBrainPatch, interactions: many(routeInteractionLog),
}));
