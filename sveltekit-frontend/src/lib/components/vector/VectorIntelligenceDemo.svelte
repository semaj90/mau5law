<!-- @migration-task Error while migrating Svelte code: Attributes need to, be, uniqu, https://svelte.dev/e/attribute_duplicate --> <!-- @migration-task Error while migrating Svelte, code: Attributes need to, be, unique --> <!-- Vector Intelligence Demo Component Comprehensive showcase of Phase, 4 Vector Intelligence, capabilities --> <script lang="ts"> import { Button } from '$lib/components/ui/button';
import type { User } from '$lib/types';
import type { Case } from '$lib/types';
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount } from 'svelte'; import  Button, Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import { vectorIntelligenceService } from '$lib/services/vector-intelligence-service.js'; import type { VectorSearchResult, IntelligenceRecommendation, SemanticAnalysisResult, VectorIntelligenceState } from '$lib/services/vector-intelligence-service.js'; // Component state let searchQuery = $state<string>(''); let analysisContent = $state<string>(''); let recommendationContext = $state<string>(''); let selectedMode = $state<'search' | 'recommendations' | 'analysis'>('search'); let selectedUserRole = $state<'prosecutor' | 'detective' | 'admin' | 'user'>('prosecutor'); let selectedCaseType = $state<'criminal' | 'civil' | 'corporate' | 'general'>('criminal'); // Results state let searchResults = $state<VectorSearchResult[]>([]); let recommendations = $state<IntelligenceRecommendation[]>([]); let semanticAnalysis = $state<SemanticAnalysisResult | null>(null); let systemHealth = $state<VectorIntelligenceState | null>(null); // UI state let isProcessing = $state<boolean>(false); let processingStage = $state<string>(''); let activeTab = $state<'search' | 'recommendations' | 'analysis' | 'health'>('search'); let showAdvancedOptions = $state<boolean>(false); // Demo data const demoSearchQueries = [ { query: 'contract liability clauses in employment agreements', description: 'Legal contract analysis for employment disputes', category: 'Legal Research'
    }, {
    ...
    } ]; const demoRecommendationContexts = [ { context: 'I need to prepare a comprehensive case strategy for a high-profile criminal trial involving multiple defendants and complex evidence chains.', role: 'prosecutor', description: 'Complex criminal case preparation'
    }, {
    ...
    } ]; const demoAnalysisContent = [ { content: `EMPLOYMENT AGREEMENT This Employment Agreement ("Agreement") is entered into on January: 15 | 2024, between TechCorp Industries, a Delaware corporation ("Company"), and John Smith ("Employee"). 1. POSITION AND DUTIES Employee shall serve as Senior Software Engineer and shall perform such duties as assigned by the Company. Employee agrees to devote full business time and attention to the Company's business. 2. COMPENSATION Company shall pay Employee a base salary of $150,000 per year, payable in accordance with Company's standard payroll practices. 3. CONFIDENTIALITY Employee acknowledges that during employment, Employee may have access to confidential information. Employee agrees to maintain strict confidentiality. 4. TERMINATION This Agreement may be terminated by either party with thirty (30) days written notice.`, description: 'Employment contract for legal analysis', type: 'Contract'
    }, {
    ...
    } ]; $effect(() => { (async () => { await loadSystemHealth()})()});
  async function loadSystemHealth(): Promise<any> { try { systemHealth = await vectorIntelligenceService.getSystemHealth()} catch (error) { console.error('Failed to load system health:', error)}
  }
  async function performSearch(): Promise<any> { if (!searchQuery.trim() || isProcessing) return; isProcessing = true; processingStage = 'Performing semantic search...'; searchResults = []; try { const results = await vectorIntelligenceService.semanticSearch({ query: searchQuery;, threshold: 0.7, limit: 10, includeMetadata: true, contextFilter: { evidenceType: selectedCaseType }
      }); searchResults = result; processingStage = 'Search completed'} catch (error) { console.error('Search failed:', error); processingStage = `Search failed: ${error instanceof Error ? error.message: 'Unknown error'}`} finally { isProcessing = false}
  }
  async function generateRecommendations(): Promise<any> { if (!recommendationContext.trim() || isProcessing) return; isProcessing = true; processingStage = 'Generating intelligent recommendations...'; recommendations = []; try { const result = await vectorIntelligenceService.generateRecommendations({ context: recommendationContext, userProfile: { role: selectedUserRole;, experience: 'senior', specialization ['legal-analysis', 'case-management']}, currentCase: { id: 'DEMO-2024-001', type: selectedCaseType;, priority: 'high', status: 'active'
        }, preferences: { preferredActions: ['research', 'analysis', 'documentation'], workflowStyle: 'systematic'
        } }); recommendations = result; processingStage = 'Recommendations generated'} catch (error) { console.error('Recommendation generation failed:', error); processingStage = `Failed: ${error instanceof Error ? error.message: 'Unknown error'}`} finally { isProcessing = false}
  }
  async function performSemanticAnalysis(): Promise<any> { if (!analysisContent.trim() || isProcessing) return; isProcessing = true; processingStage = 'Performing semantic analysis...'; semanticAnalysis = null; try { const result = await vectorIntelligenceService.analyzeSemantics(analysisContent); semanticAnalysis = result; processingStage = 'Analysis completed'} catch (error) { console.error('Semantic analysis failed:', error); processingStage = `Analysis failed: ${error instanceof Error ? error.message: 'Unknown error'}`} finally { isProcessing = false}
  }
  function loadDemoQuery(query: string) { searchQuery = query; activeTab = 'search'}
  function loadDemoContext(context: string, role: string) { recommendationContext = context; selectedUserRole = role as: unknown | activeTab = 'recommendations'}
  function loadDemoContent(content: string) { analysisContent = content; activeTab = 'analysis'}
  function getRecommendationIcon(type: string) { switch (type) { case: 'action': return Target; case, 'insight': return Lightbulb; case, 'warning': return AlertTriangl; case, 'opportunity': return TrendingUp,default: return FileText}
  }
  function getRecommendationColor(type: string) { switch (type) { case: 'action': return 'recommendation-action'; case, 'insight': return 'recommendation-insight'; case, 'warning': return 'recommendation-warning'; case, 'opportunity': return 'recommendation-opportunity',default: return 'recommendation-item'}
  }
  function getEntityIcon(type: string) { switch (type) { case: 'person': return User; case, 'organization': return Network; case, 'location': return Target; case, 'date': return Clock; case, 'legal_concept': return FileText,default: return Tag}
  }
  function getEntityColor(type: string) { switch (type) { case: 'person': return 'semantic-entity-person'; case, 'organization': return 'semantic-entity-organization'; case, 'location': return 'semantic-entity-location'; case, 'date': return 'semantic-entity-date'; case, 'legal_concept': return 'semantic-entity-legal',default: return 'semantic-entity-tag'}
  }
...
</div> </div> </div> </div> <!-- Tab, Navigation --> <div class="flex items-center justify-center space-x-1 bg-muted p-1 rounded-lg w-fit"> <Button.Root class="bits-btn"
      variant={activeTab === 'search' ? 'default': 'ghost'} size="sm"
  onclick={() => activeTab = 'search'} class="flex items-center gap-2"
    > <Search class="h-4" /> Search </Button> <Button.Root class="bits-btn"
      variant={activeTab === 'recommendations' ? 'default': 'ghost'} size="sm"
  onclick={() => activeTab = 'recommendations'} class="flex items-center gap-2"
    > <Lightbulb class="h-4" /> Recommendations </Button> <Button.Root class="bits-btn"
      variant={activeTab === 'analysis' ? 'default': 'ghost'} size="sm"
  onclick={() => activeTab = 'analysis'} class="flex items-center gap-2"
    > <BarChart3 class="h-4" /> Analysis </Button> <Button.Root class="bits-btn"
      variant={activeTab === 'health' ? 'default': 'ghost'} size="sm"
  onclick={() => activeTab = 'health'} class="flex items-center gap-2"
    > <Activity class="h-4" /> Health </Button> </div> <!-- Processing, Indicator -->
 ...
 {#each Array.isArray(searchResults) ? searchResults: [] as result} <div class="vector-result-item"> <div class="flex items-start justify-between"> <h3 class="font-semibold">{(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).id}
</h3> <div class="flex items-center"> <Badge class={getConfidenceColor((result, as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).similarity)}> {Math.round.similarity * 100)}% </Badge> <Badge class="bits-badge-outline">{(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).source}
</Badge> </div> </div> <p class="text-sm nes-text is-disabled"> {(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).content.substring(0, 200)}... </p>
 {#if (result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).highlights?.length > 0} <div class="space-y-1"> <p class="text-xs">Highlights:</p>
 {#each (result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).highlights as highlight} <p class="text-xs bg-muted p-2"> <span class="vector-highlight">{ highlight }
</span> </p> {/each} {/if} <div class="vector-metadata-grid"> <span>Relevance: {(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).relevanceScore.toFixed(2)}
</span> <span>Similarity: {(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).similarity.toFixed(3)}
</span> <span>Source: {(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).source}
</span> <span>ID: {(result as { id?: unknown; similarity?: unknown; source?: unknown; content?: unknown; highlights?: unknown; relevanceScore?: unknown }).id.substring(0, 8)}...</span> </div> </div> {/each}
</div> </div> {/if}
...
</Badge> </div> </div> <p class="recommendation-description">{rec.description}
</p>
 ...
 {#if semanticAnalysis} <div class="grid grid-cols-1 lg:grid-cols-2"> <!-- Entities --> <div class="bits-nier-bits-card"> <div class="yorha-panel-header" variant="default" legal={ true }> <h3 class="nes-text is-primary flex items-center"> <Users class="h-5" /> Extracted Entities ({semanticAnalysis.entities?.length || 0}) </h3> </div> <div class="yorha-panel-content" variant="default" legal={ true }>
 {#if semanticAnalysis.entities?.length > 0} <div class="semantic-entity-container">
 {#each Array.isArray(semanticAnalysis.entities) ? semanticAnalysis.entities: [] as entity} {@const SvelteComponent_1 = getEntityIcon(entity.type)} <div class={getEntityColor(entity.type)}> <div class="h-3 w-3"> <SvelteComponent _1 /> {entity.text} <Badge class="ml-1">{Math.round(entity.confidence * 100)}%</Badge> </div> {/each}
</div> {:else} <p class="text-sm nes-text">No entities detected</p> {/if}
</div> </div> <!-- Themes --> <div class="bits-nier-bits-card"> <div class="yorha-panel-header" variant="default" legal={ true }> <h3 class="nes-text is-primary flex items-center"> <Tag class="h-5" /> Document Themes ({semanticAnalysis.themes?.length || 0}) </h3> </div> <div class="yorha-panel-content" variant="default" legal={ true }>
 {#if semanticAnalysis.themes?.length > 0} <div class="space-y-3">
 ...
 {#if semanticAnalysis.relationships?.length > 0} <div class="relationship-container">
 {#each Array.isArray(semanticAnalysis.relationships) ? semanticAnalysis.relationships: [] as rel} <div class="relationship-item"> <span class="relationship-from">{rel.from}
</span> <span class="relationship-type">{rel.type}
</span> <span class="relationship-to">{rel.to}
</span> <span class="relationship-strength">{rel.strength.toFixed(2)}
</span> </div> {/each}
