            ...
            {#each processingResults as result}
              <div class="border border-gray-600/30 rounded p-4 bg-gray-800/30">
                <div class="flex justify-between items-center mb-3">
                  <h4 class="text-white font-mono">{(result as { documentId?: unknown; totalTime?: unknown; compressionAchieved?: unknown; stages?: unknown }).documentId}</h4>
                  <div class="text-green-400 font-mono text-sm">
                    {formatDuration((result as { documentId?: unknown; totalTime?: unknown; compressionAchieved?: unknown; stages?: unknown }).totalTime)} • {(result as { documentId?: unknown; totalTime?: unknown; compressionAchieved?: unknown; stages?: unknown }).compressionAchieved.toFixed(1)}:1 compression
                  </div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-5 gap-2 text-xs">
                  {#each (result as { documentId?: unknown; totalTime?: unknown; compressionAchieved?: unknown; stages?: unknown }).stages as stage}
