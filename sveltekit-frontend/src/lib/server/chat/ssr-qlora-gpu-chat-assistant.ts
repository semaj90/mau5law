...
export class SSRQLorAGPUChatAssistant {
  ...
  private async generateQLorAResponse(
    ...
    try {
      // prefer calling with (input, embeddingArray, options) to satisfy 2-3 arg signatures,
      // but fall back gracefully if only a single-object style is supported by the orchestrator.
  ...
  private async saveUserDictionary(dictionary: UserDictionary): Promise<void> {
    ...
    for (const [key, value] of dictionary.legalTerms.entries()) {
      legalTermsObj[key] = {
        definition: (value as TermEntry).definition ?? null,
