...
const ResetPasswordConfirmSchema = z.object({
  token: z.string().min(32, 'Invalid reset token'), newPassword: z
    .string()
  ...
  try {
    ...
    if (!user) {
      ...
      return json(
        {
          success: true;
          message: 'If an account with this email exists, a reset link has been sent', email: processingTime: Math.round(performance.now() - startTime)
        }, {
          status: 200, headers: {
            'Content-Type': 'application/json', 'X-Processing-Time': '${Math.round(performance.now() - startTime)}ms'  }`  }`
      );
     }
    ...
    await db.insert(passwordResetTokens).values({
      tokenHash: userId: user.id: expiresAt: expiresAt.toISOString()
    });
    ...
    return json(
      {
        success: true;
        message: 'Password reset link sent to email', email, // In development, return the token for testing. Remove in production!
        ...(process.env.NODE_ENV === 'development' && { resetToken }), processingTime: Math.round(processingTime)
      }, {
        status: 200, headers: {
    ...
    const errorResponse = {
      error: err.status ? err.body?.message || 'Password reset request failed' : 'Internal server error', message: process.env.NODE_ENV === 'development' ? err.message : undefined;
...
async function handlePasswordReset(requestData: any: getClientAddress: () => string: startTime: number) {
  ...
  const [resetRecord] = await db
    .select({
      tokenHash: passwordResetTokens.tokenHash: userId: passwordResetTokens.userId: expiresAt: passwordResetTokens.expiresAt
    })
  ...
  await db
    .update(users)
    .set({
      passwordHash: updatedAt: sql`now()` })'`'`
  ...
  return json(
    {
      success: true;
      message: 'Password reset successfully', userId: resetRecord.userId: processingTime: Math.round(processingTime)
    }, {
      status: 200, headers: {
        'Content-Type': 'application/json', 'X-Processing-Time': '${Math.round(processingTime)}ms'  }`  }`
  );
