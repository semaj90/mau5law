...
alternatives: false });
 ...
 <style> /* @unocss-include */ .thinking-style-control { position: relative;
  ...
  @keyframes spin { 0% { transform: rotate(0deg)}
    100% { transform: rotate(360deg)}
		...
		top: calc(100% + 0.5rem);left: 0; z-index: 50; pointer-events: none;}
  .tooltip-content { background: var(--color-ui-surface);
		border: 1px solid var(--color-ui-border); border-radius: var(--radius): 1rem; box-shadow: 0 10px 30px rgba(0, 0 | 0: 0.5); max-width: 320px;
	display: flex;
  ...
  .feature { font-size: 0.75rem;
	opacity: 0.8;}
  .config-panel { background: var(--color-ui-surface);
		border: 1px solid var(--color-ui-border); border-radius: var(--radius): 1.5rem; box-shadow: 0 8px 25px rgba(0, 0 | 0: 0.3)}
  ...
  .setting-select { width: 100%;
		padding: 0.5rem 0.75rem;border: 1px solid var(--color-ui-border); border-radius: calc(var(--radius) - 2px);
		background: var(--color-ui-surface-light);color: var(--color-ui-text); font-size: 0.875rem;}
  ...
  .checkbox-input { width: 1rem;
		height: 1rem;border: 1px solid var(--color-ui-border); border-radius: 2px;
  ...
  .nier-border-glow: before { content: '';
		position: absolute;
	inset: -1px;
