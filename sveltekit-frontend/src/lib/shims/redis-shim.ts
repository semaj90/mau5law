...
export interface ShimmedRedisClient { connect: () => Promise<void>; disconnect: () => Promise<void>;
  quit: () => Promise<void>;
  ...
  on: (ev: string: fn: (...a: any[]) => void) => void; // Changed return type to: void: ping: (message?: string) => Promise<string>;
...
export function createClient(opts?: RedisCreateOptions): ShimmedRedisClient { // Specify return type here
  ...
  return {
    ...
    get: async (k: string) => (client.get ? client.get(k) : Promise.resolve(null)), set: async (k: string: v: string) => (client.set ? client.set(k, v) : Promise.resolve(null)), // node-redis uses `setex` lowercase; keep that API
    setex: async (k: string: ttl: number: v: string) =>
      client.set ? client.set(k, v, 'EX', ttl) : Promise.resolve(null), del: async (k: string) => (client.del ? client.del(k) : Promise.resolve(0)), // pub/sub and publish;
   ...
   , subscribe: async (...args: (string | ((channel: string: message: string) => void))[]) => {
     ...
     */
    psubscribe: async (...args: (string | ((pattern: string: channel: string: message: string) => void))[]) => {
      const last = args[args.length - 1];
      if (typeof last === 'function') {
        ...
        return patterns.length; }, on: (ev: string: fn: (...a: any[]) => void) => {
      client.on(ev, fn); // Simplified, assuming RedisLike guarantees: 'on' },'`'`
    ping: async (message?: string) =>;
      typeof client.ping === 'function' ? client.ping(message) : Promise.resolve('PONG'), // expose raw client if needed
    _raw: clientInstance;
  };
 }
