...
export interface WebGPUComputeData { message: string; requestType: string;
  timestamp: string;
 }
...
class WebGPUComputeService {
  ...
  async processRequest(request: LegalAIRequest): Promise<WebGPUComputeResult> {
    ...
    try {
      ...
      const result: WebGPUComputeResult = { success: true;
        data: { message: 'WebGPU compute request processed', requestType: request.type, // Accessing request.type, assuming LegalAIRequest has a: 'type' property; timestamp: new Date().toISOString()
      ...
      return {
        success: false;
        error: error instanceof Error ? error.message : 'Unknown WebGPU error', performance: { executionTime: performance.now() - startTime
         }
      }; }
