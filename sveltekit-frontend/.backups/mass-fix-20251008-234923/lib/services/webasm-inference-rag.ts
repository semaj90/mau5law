              ...
              actions: [
                assign({
                  ...
                  performanceMetrics: ({ performanceMetrics }, event: any) => {
                    ...
                    const avg = performanceMetrics.totalInferences === 0
                      ? out.processingTime
                      : (performanceMetrics.averageLatency * performanceMetrics.totalInferences + out.processingTime) / total;
                    return {
                      ...performanceMetrics,
...
export class WASMInferenceRAGService {
  ...
  static async initialize(config: WASMInferenceConfig): Promise<void> {
    ...
    try {
      ...
      console.log(`📊 Config: ${config.quantization}, ${config.threads} threads, GPU: ${config.enableGPU}, Shared Memory: ${config.threads > 1}`);
  ...
  private static createWASMImports(memory: WebAssembly.Memory, config: WASMInferenceConfig) {
    ...
    if (config.threads > 1) {
      ...
      imports.env.__cxa_throw = () => { throw new Error('WASM C++ exception thrown'); };
      imports.env._emscripten_memcpy_big = (dest: number, src: number, num: number) => {
        const mem = new Uint8Array(memory.buffer);
        mem.copyWithin(dest, src, src + num);
      };
      imports.env._emscripten_thread_sleep = (ms: number) => { /* no-op or actual sleep */ };
