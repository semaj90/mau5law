...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const streamId = await redis.xAdd(
      'vec_requests', // Stream name for vector processing
      '*', // Auto-generate ID
      streamMessage
    );

    console.log(`📨 Enqueued to Redis stream vec_requests: ${streamId}`);
...
function getEstimatedTime(type: string, data: unknown): number {
  switch (type) {
    case 'upsert': {
