...
export class VectorSearchService {
  ...
  private async searchPgVector(request: VectorSearchRequest): Promise<VectorSearchResult[]> {
    ...
    const queryText = `
      ...
      FROM embeddings
      WHERE (vector <=> $1::vector) < (1 - $2)
      ORDER BY vector <=> $1::vector
      LIMIT $3
    `;

    try {
      const results = await this.database.query(queryText, [
        JSON.stringify(request.embedding),
        threshold,
        limit
      ]);
  ...
  private async indexPgVector(doc: {
    ...
    const insertQuery = `
      ...
      VALUES ($1, $2, $3, $4, $5, 'legal_context')
   ...
   */
  async batchIndex(documents: Array<{
    id: string;
    content: string;
    embedding: number[];
    metadata?: Record<string, unknown>;
    documentId?: string;
  }>, parallelism: number = 10): Promise<void> {
  ...
  private async checkPgVectorHealth(): Promise<void> {
    ...
    try {
      const response = await this.database.query('SELECT 1');
      this.pgvectorStatus = {
        ...this.pgvectorStatus,
