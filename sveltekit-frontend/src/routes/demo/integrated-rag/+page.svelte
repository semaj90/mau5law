<script lang="ts">
/** * Integrated RAG Demo Page * Complete workflow: Upload â†’ embeddinggemma â†’ pgvector â†’ Qdrant â†’ MinIO â†’ Search â†’ Recommendations */ import IntegratedRAGUpload from '$lib // TODO: Verify store subscription is correct for Svelte 5/components/upload/IntegratedRAGUpload.svelte'; import { onMount, onDestroy } from 'svelte'; import { createWSClient, type WSConnectionStatus } from '$lib // TODO: Verify store subscription is correct for Svelte 5/utils/websocket-client'; let uploadCount = $state // TODO: Verify store subscription is correct for Svelte 5<number>(0); let lastUpload = $state // TODO: Verify store subscription is correct for Svelte 5<any>(null); let wsError = $state // TODO: Verify store subscription is correct for Svelte 5<string | null>(null); let connectionAttempts = $state // TODO: Verify store subscription is correct for Svelte 5<number>(0); let wsClient: ReturnType<typeof createWSClient> | null = null; let connectionStatus = $state // TODO: Verify store subscription is correct for Svelte 5<WSConnectionStatus>('disconnected'); let messages = $state // TODO: Verify store subscription is correct for Svelte 5<any[]>([]); let searchQuery = $state // TODO: Verify store subscription is correct for Svelte 5<string>(''); function handleSuccess(result: unknown) { uploadCount++; lastUpload = result}
