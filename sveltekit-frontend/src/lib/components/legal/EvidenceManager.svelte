...
import type { Document } from '$lib/types'; import { fade, scale, fly } from 'svelte/transition'; // the module exports a default builder (adjusted per compile hint) import Card from '$lib/components/ui/Card/Card.svelte';
import CardHeader from '$lib/components/ui/Card/CardHeader.svelte';
import CardTitle from '$lib/components/ui/Card/CardTitle.svelte';
import CardContent from '$lib/components/ui/Card/CardContent.svelte';
import Button from '$lib/components/ui/Button.svelte';
import Input from '$lib/components/ui/Input.svelte'; interface EvidenceItem { id: string, type: 'email' | 'transcript' | 'financial' | 'document' | 'audio' | 'video',title: string, description: string, dateCreated: string, source: string, hash: string, size: string, tags: string[], relevanceScore: number, authenticity: 'verified' | 'pending' | 'disputed' | 'invalid', privileged: boolean;
...
aiModel: 'gemma3'
 ...
 <style> .evidence-manager { max-width: 1400px;
  ...
  .total-count, .verified-count, .privileged-count { padding: 0.25rem 0.5rem;
		background: rgba(255, 255, 255, 0.1); border-radius: 4px;}
  .evidence-actions { display: flex;
  ...
  .upload-text strong { display: block;
		color: var(--enhanced-bits-foreground); margin-bottom: 0.5rem;}
  .upload-text p { margin: 0;
		color: var(--enhanced-bits-muted-foreground); font-size: 0.875rem;}
  ...
  .type-filter, .sort-control { background: var(--enhanced-bits-background);
  ...
  .selection-controls { display: flex;
		gap: 0.5rem;}
  ...
  .evidence-content { display: grid; grid-template-columns: 1fr auto;
  ...
  .no-evidence-icon { font-size: 3rem;
