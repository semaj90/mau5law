...
export class EnhancedOCRProcessor {
  ...
  async processDocument(filePath: string): Promise<OCRResult> {
    ...
    try {
      ...
      (result as { processing_time?: any }).processing_time = Date.now() - startTime;
      return result;
    } catch (error: any) {
  ...
  private extractTextBlocks(data: Tesseract.Page): TextBlock[] {
    ...
    if ((data as { page_count?: any; text?: any; confidence?: any; blocks?: any }).blocks) {
      for (const block of (data as { page_count?: any; text?: any; confidence?: any; blocks?: any }).blocks) {
        blocks.push({
          text: block.text,
          bbox: {
            x: block.bbox.x0,
  ...
  async batchProcessDocuments(filePaths: string[]): Promise<OCRResult[]> {
    ...
    for (let i = 0; i < filePaths.length; i++) {>
      ...
      try {
        ...
        results.push(result);
        console.log()
          `✅ Completed processing ${path.basename(filePath)} in ${(result as { processing_time?: any }).processing_time}ms`
        );
      } catch (error: any) {
        console.error()
