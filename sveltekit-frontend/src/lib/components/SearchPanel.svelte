<script lang="ts">
	...
	async function handleSearch() {
		...
		await searchStore.executeSearch(query, {
			include_kag: includeKag,
			include_reasoning: includeReasoning,
		});
	}

	function handleKeydown(e: KeyboardEvent) {
		if (e.key === 'Enter' && !searchStore.loading) {
			handleSearch();
		}
	}
...
<div class="search-panel space-y-4">
	...
	<div class="search-input-group flex gap-2 items-center">
		...
		<button
			...
			disabled={searchStore.loading || !query.trim()}
		>
		...
		{#if searchStore.results.length > 0}
			<button class="btn btn-ghost btn-sm" onclick={() => searchStore.clearSearch()}>
				Clear
			</button>
		{/if}
	</div>
		...
		<div class="alignment-hud p-3 rounded border border-base-300 bg-base-100">
			<div class="text-xs opacity-80 flex flex-wrap gap-4">
				<div>
					...
					<span class="font-mono ml-1">{searchStore.alignment.intent}</span>
				</div>
				<div>
					<span class="font-semibold">Route:</span>
					<span class="font-mono ml-1">{searchStore.alignment.route_decision}</span>
				</div>
				<div>
					<span class="font-semibold">On-task:</span>
					<span class="font-mono ml-1"
						>{formatScore(searchStore.alignment.on_task_score)}%</span
					>
				</div>
				<div>
					<span class="font-semibold">Negativity:</span>
					<span class="font-mono ml-1"
						>{formatScore(searchStore.alignment.negativity_score)}%</span
					>
				</div>
				<div>
					<span class="font-semibold">Latency:</span>
					<span class="font-mono ml-1"
						>{formatLatency(searchStore.alignment.latency_ms)}ms</span
					>
				</div>
			</div>
			{#if searchStore.alignment.web_search_suggested}
	...
	{#if searchStore.reasoning}
		<div class="reasoning-summary p-3 rounded border border-base-300 bg-base-100">
			...
			<p class="text-sm">{searchStore.reasoning}</p>
		</div>
	{/if}
	...
	{#if searchStore.results.length > 0}
		<div class="search-results space-y-3">
			<div class="text-sm opacity-80">
				Found {searchStore.results.length} result{searchStore.results.length !== 1 ? 's' : ''}
			</div>

			{#each searchStore.results as chunk (chunk.id)}
				<div
					class="result-chunk p-3 rounded border border-base-300 bg-base-100 hover:border-primary transition"
				>
					<!-- Header -->
					...
					{#if chunk.kag_context}
						<details class="text-xs opacity-80">
							<summary class="cursor-pointer hover:opacity-100">
								KAG Context ({chunk.kag_context.nodes.length} nodes, {chunk.kag_context.edges
									.length} edges)
							</summary>
