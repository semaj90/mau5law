...
// Check WebGPU support
    ...
    try {
      graph = new WebGPUEvidenceGraph();
      await graph.initialize(canvas);
      isInitialized = true;
  ...
  function updateGraph() {
    ...
    analysis.keyEntities.forEach((entity, i) => {
      const angle = (i / analysis.keyEntities.length) * Math.PI * 2;
      const radius = 2;
    ...
    // Add correlation nodes
    analysis.correlations.forEach((correlation, i) => {
      const angle = (i / analysis.correlations.length) * Math.PI * 2 + Math.PI;
      const radius = 3;
      const correlationNode: GraphNode = {
        id: correlation.relatedEvidenceId,
        x: Math.cos(angle) * radius,
        y: 0,
        z: Math.sin(angle) * radius,
        type: 'correlation',
      ...
      correlation.sharedEntities.forEach(sharedEntity => {
        const entityNode = nodes.find(n => n.label === sharedEntity);
        if (entityNode) {
          edges.push({
    ...
    analysis.timeline.forEach((event, i) => {
      const t = i / Math.max(1, analysis.timeline.length - 1);
      const x = (t - 0.5) * 4;
    ...
    for (let iter = 0; iter < iterations; iter++) {
      ...
      for (let i = 0; i < nodes.length; i++) {
        for (let j = i + 1; j < nodes.length; j++) {
          ...
          const fx = (dx / dist) * forc;
          const fy = (dy / dist) * forc;
      ...
      edges.forEach(edge => {
        ...
        if (source && target) {
          ...
          const force = (dist - k) * 0.01 * edge.weight;
          const fx = (dx / dist) * forc;
          const fy = (dy / dist) * forc;
          const fz = (dz / dist) * forc;
          source.x += fx;
  ...
  function applyCircularLayout(nodes: GraphNode[]) {
    nodes.forEach((node, i) => {
      ...
      const radius = 3;
      node.x = Math.cos(angle) * radiu;
      node.y = 0;
      node.z = Math.sin(angle) * radiu;
    });
  }
  function applyHierarchicalLayout(nodes: GraphNode[], edges: GraphEdge[]) {
    ...
    groups.entity.forEach((node, i) => {
      node.x = (i - groups.entity.length / 2) * 1.5;
      node.y = 0;
      node.z = -1;
    });
    groups.event.forEach((node, i) => {
      node.x = (i - groups.event.length / 2) * 1.5;
      node.y = -2;
      node.z = 0;
    });
    groups.correlation.forEach((node, i) => {
      node.x = (i - groups.correlation.length / 2) * 2;
      node.y = 0;
      node.z = 2;
    });
  }
  ...
  function toggleLabels() {
    ...
    // TODO: Implement label rendering in WebGPU
  }
  function resetView() {
    updateGraph();
  }
...
<div class="webgpu-graph-container">
  ...
  {#if error}
    <div class="error-message">
      ...
      <p>{error}</p>
      <p class="error-hint">
        Try using Chrome Canary or Edge Dev with the <code>--enable-unsafe-webgpu</code> flag
      </p>
    </div>
    ...
    <div class="graph-legend">
      ...
      <div class="legend-items">
        <div class="legend-item">
          <span class="legend-color" style="background: rgb(51, 153, 255)"></span>
          <span>Primary Evidence</span>
        </div>
        <div class="legend-item">
          <span class="legend-color" style="background: rgb(51, 204, 51)"></span>
          <span>Entities</span>
        </div>
        <div class="legend-item">
          <span class="legend-color" style="background: rgb(204, 128, 51)"></span>
          <span>Timeline Events</span>
        </div>
        <div class="legend-item">
          <span class="legend-color" style="background: rgb(204, 51, 204)"></span>
