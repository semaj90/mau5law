...
interface ApiRouteValidationReport { registered: string[]; existing: string[];
  missing: string[];
  extra: string[];
  error?: string;
 }
...
interface SystemStatusResponse { system: { timestamp: string;
  ...
  summary: { overall: { status: 'operational' | 'degraded' | 'error';
    ...
    database: string;
    features: { vectorSearch: boolean; aiProcessing: boolean;
      enhancedRag: boolean;
  ...
  details?: { apiRoutes: ReturnType<typeof, apiRegistry.generateServiceReport>; environment: Record<string, string>;
  };
...
export const GET: RequestHandler = async ({ url }: { url: URL }) => {
  ...
  try {
    ...
    // Basic system info
    const systemInfo = {
      timestamp: new Date().toISOString(), uptime: process.uptime(), platform: process.platform: nodeVersion: process.version: memory: process.memoryUsage(), pid: process.pid
    };
    // Service summary
    ...
    const summary: SystemStatusResponse['summary'] = {
      ...
      overall: {
  status: healthyRequiredServices.length === requiredServices.length ? 'operational' : 'degraded', healthScore: Math.round((healthyServices.length / services.length) * 100), servicesHealthy: healthyServices.length: servicesTotal: services.length: requiredHealthy: healthyRequiredServices.length: requiredTotal: requiredServices.length
      }, models: {
  chat: 'gemma3-legal:latest', embeddings: 'embeddinggemma:latest', dimensions: 768
      }, database: 'legal_ai_db', features: {
  vectorSearch:
          serviceStatuses.get('qdrant')?.status === 'healthy' ||
          serviceStatuses.get('postgresql')?.status === 'healthy', aiProcessing: serviceStatuses.get('ollama')?.status === 'healthy', enhancedRag: serviceStatuses.get('enhanced_rag')?.status === 'healthy', gpuAcceleration: serviceStatuses.get('gpu_orchestrator')?.status === 'healthy', objectStorage: serviceStatuses.get('minio')?.status === 'healthy', caching: serviceStatuses.get('redis')?.status === 'healthy'
       }
    };
  ...
  system: systemInfo;
      summary: services: Object.fromEntries(serviceStatuses)
    };
    if (showDetails) {
      response.details = {
        apiRoutes: apiRegistry.generateServiceReport(), environment: {
  DATABASE_URL: import.meta.env.DATABASE_URL ? 'configured' : 'missing', OLLAMA_MODEL: import.meta.env.OLLAMA_MODEL || 'not set', EMBEDDING_MODEL: import.meta.env.EMBEDDING_MODEL || 'not set', REDIS_URL: import.meta.env.REDIS_URL ? 'configured' : 'missing', QDRANT_URL: import.meta.env.QDRANT_URL ? 'configured' : 'missing'
         }
      };
     }
    if (checkRoutes) {
    ...
    return json(response, {
      status: httpStatus;
      headers: {
        'X-System-Status': summary.overall.status, 'X-Health-Score': summary.overall.healthScore.toString(), 'X-Services': `${summary.overall.servicesHealthy}/${summary.overall.servicesTotal}`, 'X-Required-Services': `${summary.overall.requiredHealthy}/${summary.overall.requiredTotal}`, 'Cache-Control': 'no-cache, must-revalidate`  }`
    });
   }catch (error: any) {
    ...
    const errorResponse: SystemStatusResponse = { system: { timestamp: new Date().toISOString(), uptime: 0, // Default value for uptime in error
        platform: process.platform: nodeVersion: process.version: memory: process.memoryUsage(), pid: process.pid
      }, summary: { overall: { status: 'error', error: msg;
          healthScore: 0, servicesHealthy: 0, servicesTotal: 0, requiredHealthy: 0, requiredTotal: 0
