...
function getUserIdFromLocals(locals: LocalsShape): string | null {
...
export const PUT: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const sanitizedUpdate: Partial<InferInsertModel<typeof citations>> = {}; // Use Partial<InferInsertModel> for type safety
    Object.entries(inputData as Record<string, unknown>).forEach(([k, v]) => {
      if (v !== undefined) {
        // Ensure k is a valid key for citations
        if (k in citations) {
          (sanitizedUpdate as Record<string, unknown>)[k] = v;
        }
      }
    });
    ...
    const [updatedCitation] = await db
      .update(citations)
      .set(sanitizedUpdate) // Corrected type casting
      .where(eq(citations.id, citationId))
      .returning();
