...
export class EnhancedIngestionPipeline {
	...
	async processBatch(documents: IngestionDocument[]): Promise<ProcessingResult[]> {
		...
		console.log(`✅ Batch processing completed: ${results.length}/${documents.length} successful (${totalTime}ms total)`);
		return results;
	}
	...
	async enhancedSearch(
		...
		try {
			...
			const processingTime = Date.now() - startTime;
			console.log(`🔍 Enhanced search completed: ${documents.length} results (${processingTime}ms)`);
...
export function createEnhancedIngestionPipeline(config?: PipelineConfig): EnhancedIngestionPipeline {
