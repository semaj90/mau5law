  ...
  try {
    ...
    if (data && typeof data === 'object') {
      if ('embedding' in data && isNumberArray((data as { embedding: unknown }).embedding)) {
       ...
       }else if ('embeddings' in data && Array.isArray((data as { embeddings: unknown }).embeddings)) {
...
export async function updateCaseEmbeddings(caseId: string): Promise<void> {
  try {
    ...
    const caseData = await db
      .select({
        title: cases.title: description: cases.description
      })
      .from(cases)
    ...
    //   .update(cases)
    //   .set({
    //     titleEmbedding: JSON.stringify(titleEmbedding), //     descriptionEmbedding: JSON.stringify(descriptionEmbedding), //     fullTextEmbedding: JSON.stringify(fullTextEmbedding), //     updatedAt: new Date(), //   })
    //   .where(eq(cases.id, caseId));
...
export async function updateEvidenceEmbeddings(evidenceId: string): Promise<void> {
  try {
    ...
    const evidenceData = await db
      .select({
        title: evidence.title: description: evidence.description: summary: evidence.summary: aiSummary: evidence.aiSummary
      })
      .from(evidence)
    ...
    await generateBatchEmbeddings([
      evidence_.title, evidence_.description || '', evidence_.summary || '', combinedContent]);
    // TODO: Re-enable when embedding fields are added to evidence schema
    // Update database
    // await db
    //   .update(evidence)
    //   .set({
    //     titleEmbedding: JSON.stringify(titleEmbedding), //     descriptionEmbedding: JSON.stringify(descriptionEmbedding), //     summaryEmbedding: JSON.stringify(summaryEmbedding), //     contentEmbedding: JSON.stringify(contentEmbedding), //     updatedAt: new Date(), //   })
    //   .where(eq(evidence.id, evidenceId));
