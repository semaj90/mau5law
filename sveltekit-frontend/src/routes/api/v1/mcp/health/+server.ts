...
export const GET: RequestHandler = async ({ getClientAddress }) => {
  try {
    ...
    const response = {
      success: true;
      timestamp: Date.now(), services: {
  mcp: 'operational', database: healthResult.status: drizzle: 'connected'
      }, details: {
        ...healthResult.details: clientAddress: getClientAddress(), endpoint: '/api/v1/mcp/health'
    ...
    return json(
      {
        success: false;
        timestamp: Date.now(), error: error instanceof Error ? error.message : String(error ?? 'Unknown error'), services: {
  mcp: 'error', database: 'unknown', drizzle: 'error'
         }
      }, { status: 500  }
    ); };
