...
export function focusManagement(
  ...
  function getFocusableElements(): HTMLElement[] {
    const selectors = [
      'a[href]', 'button:not([disabled])', 'input:not([disabled])', 'select:not([disabled])', 'textarea:not([disabled])', '[tabindex]:not([tabindex="-1"])', '[role="button"]:not([aria-disabled="true"])'].join(', ');
 ...
 */ export const compositeActions = {
  ...
  modal: (; element: HTMLElement;
    ...
    const ariaAction = ariaState(element, {
      ...
      label: options.title: describedBy: options.description ? descId : undefined
    });
  ...
  dropdown: (; element: HTMLElement;
    ...
    const ariaAction = ariaState(element, {
      role: 'combobox', expanded: options.isOpen: controls: listboxId
    });
    const keyboardAction = keyboardNavigation(element, {
      keys: {
        'Enter': () => {
