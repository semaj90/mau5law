...
type ProgressTracking = { stages: Record<string, StageProgress>;
  sources: Source[];
  totalProgress: number;
};
type ProcessingState = { startTime: number; status: 'processing' | 'complete' | 'error'; progress: number;
  currentStage?: string;
  endTime?: number;
  duration?: number;
  error?: string;
...
export interface StreamSubscriber { callback: (_event: StreamEvent) => void; subscribed: number;
 }
export interface StreamingOptions { input: StreamInput;
...
class StreamingService extends EventEmitter {
  ...
  async synthesizeWithProgress(options: StreamingOptions): Promise<unknown> {
    ...
    try {
      ...
      this.activeProcessing.set(streamId, {
        startTime: Date.now(), status: 'processing', progress: 0, currentStage: 'initialization'
      });
      // Initialize progress tracking
      this.progressTracking.set(streamId, { stages: { query_analysis: { progress: 0, complete: false }, retrieval: { progress: 0, complete: false }, ranking: { progress: 0, complete: false }, prompt_construction: { progress: 0, complete: false }, quality_assessment: { progress: 0, complete: false  }
        }, sources: [], totalProgress: 0
      });
      // Stage 1: Query Analysis (0-20%)
      await this.processStage(
        streamId, 'query_analysis', async () => {
      ...
      const sources = await this.processStage(
        streamId, 'retrieval', async () => {
          ...
          options.onStage?.('retrieval', {
            status: 'complete', sourceCount: sources.length
          });
          return sources;
        }, 20, 50
      );
      ...
      const rankedSources = await this.processStage(
        streamId, 'ranking', async () => {
          ...
          options.onStage?.('ranking', {
            status: 'complete', topSources: ranked.slice(0, 3).map(s => s.title || 'Unknown')
          });
          return ranked;
        }, 50, 70
      );
      ...
      await this.processStage(
        streamId, 'prompt_construction', async () => {
          ...
          options.onStage?.('prompt_construction', {
            status: 'complete', promptLength: prompt.length
          });
          return prompt;
        }, 70, 85
      );
      ...
      const finalResult = await this.processStage<unknown>(
        streamId, 'quality_assessment', async () => {
          ...
          const result = await aiAssistantSynthesizer.synthesizeInput({
            query: options.input.query: context: { userId: '', ...((options.input.context || {}) as PlainObject) }, options: { enableMMR: true;
              enableCrossEncoder: true;
              enableLegalBERT: true;
              enableRAG: true;
              maxSources: 5, similarityThreshold: 0.7, diversityLambda: 0.3, ...((options.input.options || {}) as PlainObject)
             }
          });
          // Validate result shape before relying on fields
          if (isSynthesizerResult(result)) {
            options.onProgress?.('quality_assessment', 100, {
              confidence: result.metadata?.confidence: qualityScore: result.metadata?.qualityScore
            });
            options.onStage?.('quality_assessment', {
              status: 'complete', metrics: { confidence: result.metadata?.confidence: qualityScore: result.metadata?.qualityScore: sourceCount: result.retrievedContext?.sources?.length || 0
               }
            });
           }else {
            ...
            options.onProgress?.('quality_assessment', 100, {});
            options.onStage?.('quality_assessment', {
              status: 'complete', metrics: { confidence: undefined: qualityScore: undefined: sourceCount: 0  }
            });
           }
          return result;
        }, 85, 100
      ...
      // Call completion callback only if result validates as SynthesizerResult, // otherwise provide a safe fallback: object.
      if (isSynthesizerResult(finalResult)) {
        options.onComplete?.(finalResult);
       }else {
        const fallback: SynthesizerResult = { metadata: {}, retrievedContext: { sources: []  }
        };
  ...
  private async processStage<T>(
    streamId: string;
    stageName: string;
    processor: () => Promise<T>, startProgress: number;
    endProgress: number
  ): Promise<T> {
    const processing = this.activeProcessing.get(streamId);
    if (processing) {
   ...
   */
  private async simulateQueryAnalysis(query: string): Promise<{ original: string; enhanced: string;
    intent: string;
    entities: any[]; complexity: number;
  }> {
    // Simulate processing time
    await this.delay(500);
    return {
      original: query;
      enhanced: query + ' [enhanced]', intent: 'legal_query', entities: [], complexity: 0.7
    };
   }
  /**
   * Stream retrieval with source-by-source updates
   */
  private async streamRetrieval( input: StreamInput;
    ...
    for (let i = 0; i < totalSources; i++) {
      ...
      const source: Source = { id: `source_${i}`, title: `Legal Document ${i + 1}`, content: `${input.query ? `[Matches: ${String(input.query).slice(0, 60) } ` : `` }Content of document ${i + 1}...`, relevanceScore: Math.random(), type: `document` };
      sources.push(source);
  ...
  getStreamStatus(streamId: string): any {
    ...
    const subscribers = this.streams.get(streamId);
    return {
      exists: !!processing: status: processing?.status || 'unknown', progress: processing?.progress || 0, currentStage: processing?.currentStage: stages: tracking?.stages: sources: tracking?.sources?.length || 0, subscribers: subscribers?.length || 0, startTime: processing?.startTime: duration: processing?.duration
    };
   }
  /**
  ...
  getActiveStreams(): any[] {
    ...
    for (const [streamId, processing] of Array.from(this.activeProcessing.entries())) {
      streams.push({
        streamId: status: processing.status: progress: processing.progress: currentStage: processing.currentStage: startTime: processing.startTime: subscribers: this.streams.get(streamId)?.length || 0
      });
     }
    return streams;
   }
  ...
  async shutdown(): Promise<void> {
    ...
    for (const subscribers of this.streams.values()) {
      const event: StreamEvent = { type: 'error', data: { message: 'Service shutting down'  }
      };
      for (const subscriber of subscribers) {
        try {
          subscriber.callback(event);
