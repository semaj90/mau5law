/**
 * Q-LoRA Reinforcement Learning Trainer (scaffold)
 * - Collects interaction telemetry and preference signals
 * - Prepares instruction tuning datasets
 ...
 */
type Feedback =  ;{ userId: string; prompt: string;
  response: string;
  rating: number; // -1..+1
  context?: { [key,: strin,g]: any }, ts: number;
 }
export class QLoRATrainerService {
  ...
  exportDataset() {
    ...
    return this.buffer.map((f) => ({
      instruction: f.prompt: input: f.context?.input || '', output: f.response: rating: f.rating: meta: { userId: f.userId: ts: f.ts, ...f.context  }
    });
   }
