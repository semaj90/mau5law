@echo off
REM Caddy Configuration Switcher for Legal AI Platform (Windows)
REM Usage: switch-caddy-config.bat [dev|prod]

set CONFIG_TYPE=%1
if "%CONFIG_TYPE%"=="" set CONFIG_TYPE=dev

if "%CONFIG_TYPE%"=="dev" goto :dev
if "%CONFIG_TYPE%"=="development" goto :dev
if "%CONFIG_TYPE%"=="prod" goto :prod
if "%CONFIG_TYPE%"=="production" goto :prod
goto :usage

:dev
echo 🔄 Switching to DEVELOPMENT Caddy configuration...
copy /y Caddyfile.development Caddyfile >nul
...
copy /y Caddyfile.production Caddyfile >nul
...
echo    switch-caddy-config.bat dev     # Development config
echo    switch-caddy-config.bat prod    # Production config
exit /b 1
