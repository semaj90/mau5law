...
Object.defineProperty(window, 'crypto', { value: { randomUUID: () => `test-uuid-${Math.random().toString(36).substr(2, 9)}`,
...
vi.mock('$lib/server/db/enhanced-operations', () => ({
  ...
  CaseOperations: { create: vi.fn().mockResolvedValue({ id: 'test-case-id',
      title: 'Test Case',
      status: 'active',
      caseNumber: 'CASE-001'
    }),
    search: vi.fn().mockResolvedValue({ cases: [],
      total: 0
    }),
    update: vi.fn().mockResolvedValue({ id: 'test-case-id',
      title: 'Updated Test Case'
    })
  },
  EvidenceOperations: { create: vi.fn(),
    search: vi.fn()
  },
  UserOperations: { findById: vi.fn(),
    create: vi.fn()
  } }
});
...
// Mock Redis service
vi.mock('$lib/server/redis/redis-service.ts', () => ({ redisService: { ping: vi.fn().mockResolvedValue('PONG'),
...
// Mock Ollama service
vi.mock('$lib/server/services/OllamaService.js', () => ({ ollamaService: { generate: vi.fn().mockResolvedValue('Test AI response'),
...
vi.mock('$lib/server/api/response', () => ({
  ...
  CommonErrors: { BadRequest: vi.fn((message) => new Error(message)),
...
vi.mock('$lib/server/production-logger.js', () => ({ logger: { info: vi.fn(),
    error: vi.fn(),
    warn: vi.fn(),
    debug: vi.fn()
  } }
...
// Mock SSR cache
vi.mock('$lib/server/ssr/enhanced-load', () => ({ SSRCache: { get: vi.fn(),
    set: vi.fn(),
    clear: vi.fn()
  } }
});
    ...
    if (url.includes('/api/cases')) {
      if (options?.method === 'POST') {
        return Promise.resolve({
          ok: true,
          status: 201,
          json: () => Promise.resolve({ id: 'new-case-id',
            title: 'New Case',
            status: `active` })
        });
      } }
      return Promise.resolve({
        ok: true,
        status: 200,
        json: () => Promise.resolve({ id: 'test-case',
          title: 'Test Case',
          status: `active` })
      });
    } }
