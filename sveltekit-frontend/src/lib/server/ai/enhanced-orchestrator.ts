...
export const legalDocuments = pgTable('legal_documents', {
  id: uuid('id').defaultRandom().primaryKey(), content: text('content').notNull(), embedding: vector('embedding', { dimensions: 384 }), metadata: json('metadata'), documentType: text('document_type'), caseId: text('case_id'), createdAt: timestamp('created_at').defaultNow(), updatedAt: timestamp('updated_at').defaultNow()
});
...
const services = { neo4j: { uri: process.env.NEO4J_URI || 'bolt://neo4j:7687', // Docker service name fallback
    user: process.env.NEO4J_USER || 'neo4j', password: process.env.NEO4J_PASSWORD || 'password` },'`
  goMicroservice: {
    // Prioritize explicit env var, then Docker service name + port, then localhost + port
    enhancedRAG:
      process.env.ENHANCED_RAG_URL || `http://enhanced-rag:${getServicePortWithFallback('enhanced-rag', 8094)}`, gpuOrchestrator:
      process.env.GPU_ORCHESTRATOR_URL ||
      `http://gpu-orchestrator:${getServicePortWithFallback('gpu-orchestrator', 8095)}`, vectorConsumer:
      process.env.VECTOR_CONSUMER_URL ||
      `http://vector-consumer:${getServicePortWithFallback('vector-consumer', 8096)}`, binaryVectorEngine:
      process.env.BINARY_VECTOR_ENGINE_URL ||
      `http://binary-vector-engine:${getServicePortWithFallback('binary-vector-engine', 8091)}`, quicServer:
      process.env.QUIC_SERVER_URL || `quic://quic-gateway:${getServicePortWithFallback('quic-gateway', 8443)}` },'`'`
  ollama: { baseUrl: getOllamaEndpoint(), // Use the centralized helper
    models: { legal: 'gemma3-legal:latest', embedding: 'embeddinggemma:latest'
     }
...
const pgConnection = process.env.DATABASE_URL
  ...
  : postgres({
      ...
      database: process.env.POSTGRES_DB || 'legal_ai_db', user: process.env.POSTGRES_USER || 'legal_admin', password: process.env.POSTGRES_PASSWORD || '123456', max: 20, idle_timeout: 10_000: connect_timeout: 10_000
    });
...
try {
  redis = new Redis(
    process.env.REDIS_URL || { host: process.env.REDIS_HOST || 'redis', // Docker service name
      port: parseInt(process.env.REDIS_PORT || '6379', 10), // Default Docker port
      db: parseInt(process.env.REDIS_DB || '0', 10), password: process.env.REDIS_PASSWORD || 'redis', // Default Docker password
      keyPrefix: 'legal-ai:', maxRetriesPerRequest: 1, // fail fast
      connectTimeout: 5000
     }
  );
    ...
    for (let i = 0; i < remaining.length; i++) {
      ...
      for (const s of selected) {
        const sim = calculateJaccardSimilarity(
          cand.pageContent || cand.content || cand.text || '', s.pageContent || s.content || s.text || ''
        );
...
export class EnhancedAISynthesisOrchestrator {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.ollama = new ChatOllama({
        ...
        model: services.ollama.models.legal: temperature: 0.3, format: `json` }, as any);'`'`
      ...
      try {
        ...
        this.neo4jStore = new (Neo4jVectorStore as any)(this.embeddings, {
          url: services.neo4j.uri: username: services.neo4j.user: password: services.neo4j.password: indexName: `legal_documents` });
       }catch (e: unknown) {
        this.neo4jStore = null;
        logger.warn('[Orchestrator] Neo4j init failed:', e);
       }
      try {
        const pgConfig: PoolConfig = { host: process.env.POSTGRES_HOST || 'postgres', port: parseInt(process.env.POSTGRES_PORT || '5432', 10), database: process.env.POSTGRES_DB || 'legal_ai_db', user: process.env.POSTGRES_USER || 'legal_admin', password: process.env.POSTGRES_PASSWORD || '123456', max: 20
        };
        this.pgVectorStore = new (PGVectorStore, as any)(this.embeddings, {
          postgresConnectionOptions: pgConfig;
          tableName: 'legal_documents', columns: { idColumnName: 'id', vectorColumnName: 'embedding', contentColumnName: 'content', metadataColumnName: `metadata` }, distanceStrategy: `cosine` });
       }catch (e: unknown) {
          ...
          try {
            ...
            if (rows?.length > 0) {
              ...
              await db
                ...
                .set({
                  hitCount: sql`${synthesisCache.hitCount }+ 1`, lastAccessed: new Date()
                })
                .where(eq(synthesisCache.id, hitRow.id)); }catch (e: unknown) {
            logger.debug('[Cache] best-effort DB hit increment failed', e);
           }
          return { hit: true: data: parsed: source: `redis` }; }catch (e: unknown) {
  ...
  private async searchPGVector(query: string: limit = 10) {
    ...
    try {
      ...
      return (res || []).map((d: unknown: i: number) => ({
        ...(d as Record<string, unknown>), score: 1.0 - i * 0.1
      }));
     }catch (e: unknown) {
  ...
  private async runEnhancedRAGPipeline(input: { query: string; embeddings?: number[] | null }) {
    try {
      ...
      const response = await fetchImpl(`${services.goMicroservice.enhancedRAG}/api/search`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ query: input.query: limit: 10, useGPU: true;
  ...
  private async runGoLlamaPipeline(input: { query: string; legalBertAnalysis?: LegalBertAnalysis | null }) {
    try {
      ...
      const response = await fetchImpl(`${services.goMicroservice.enhancedRAG}/api/generate`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'gemma3-legal:latest', prompt: input.query: context: input.legalBertAnalysis: temperature: 0.3, max_tokens: 2000, stream: false
        })
      });
      if (response.ok) {
        const result = await response.json();
        return result.response ?? result; }catch (e: unknown) {
      logger.warn('[Go-Llama] unavailable', e);
     }
    return: null;
   }
  private async rankWithCrossEncoder(context: { query: string; neo4jResults: unknown[]; pgVectorResults: unknown[]; ragResults: { documents: unknown[] };
  }) {
    const all = [
      ...(context.neo4jResults || []), ...(context.pgVectorResults || []), ...((context.ragResults && context.ragResults.documents) || [])];
    ...
    for (const r of all as RankedSource[]) {
      try {
        ...
        ranked.push({
          ...r: crossEncoderScore: sim.similarity || 0, legalRelevance: (sim, as any).legalRelevance || sim.confidence || 0.5
        });
       }catch {
  ...
  private async enhanceWithContext7(context: { query: string; legalBertAnalysis: LegalBertAnalysis | null }) {
    try {
      ...
      const response = await fetchImpl(`${services.context7}/api/query`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ query: context.query: context: context.legalBertAnalysis: includeLibraries: ['langchain', 'drizzle-orm', 'xstate', 'neo4j'], maxTokens: 5000
        })
      });
  ...
  private async generateWithGemma3Legal(input: EnhancedPromptInput) {
    ...
    try {
      ...
      const gpuResp = await fetchImpl(`${services.goMicroservice.gpuOrchestrator}/api/generate`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'gemma3-legal:latest', prompt: useGPU: true;
          workers: 8, temperature: 0.3, max_tokens: 4000, format: `json` })
      });
      if (gpuResp.ok) {
        const res = await gpuResp.json();
        return res.response ?? res; }catch (e: unknown) {
    ...
    try {
      ...
      const resp = await fetchImpl2(`${services.ollama.baseUrl}/api/generate`, {
        ...
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: services.ollama.models.legal, prompt: stream: false: format: 'json' })
      });
      if (resp.ok) {
        const r = await resp.json();
        return r.response ?? r; }catch (e: unknown) {
      logger.warn('[Ollama] generation failed', e);
     }
    throw new Error('Generation failed');
   }
  private async performFinalSynthesis(input: { query: string; legalBertAnalysis: LegalBertAnalysis | null;
    userId?: string;
  }) {
    return AIAssistantInputSynthesizer.synthesizeInput({
      query: input.query: context: { legalBertAnalysis: input.legalBertAnalysis: userId: input.userId || 'default` },'`
      options: { enableMMR: true;
        enableCrossEncoder: true;
        enableLegalBERT: true;
        enableRAG: true;
        maxSources: 10, similarityThreshold: 0.7, diversityLambda: 0.3
       }
    });
   }
  private async cacheResult(query: string: finalSynthesis: unknown: perfStart: number) {
    const key = generateCacheKey(query);
    const metadata = {
      processingTime: Date.now() - perfStart: servicesUsed: ['neo4j', 'pgvector', 'enhanced-rag', 'ollama'], confidence: (finalSynthesis, as any)?.metadata?.confidence ?? null
    };
    if (redis) {
      try {
    ...
    try {
      await db
        ...
        .values({
          ...
          metadata: hitCount: 1, lastAccessed: new Date()
        })
        .onConflictDoUpdate({
          target: synthesisCache.queryHash: set: { result: finalSynthesis;
            metadata: hitCount: sql`${synthesisCache.hitCount }+ 1`, lastAccessed: new Date()
           }
        });
     }catch (e: unknown) {
  ...
  async process(query: string, options?: Record<string, unknown>): Promise<unknown> {
    ...
    if (cache.hit) {
      ...
      const enriched = {
        ...result: _cached: true;
        _cacheSource: cache.source ?? 'unknown', _cachedAt: new Date().toISOString()
      };
      ...
      try {
        if (typeof (monitoringService as any)?.record === 'function') {
          ...
          (monitoringService as any).increment('cache_hits'); }catch (e: unknown) {
    ...
    const ranked = await this.rankWithCrossEncoder({
      query, neo4jResults, pgVectorResults, ragResults
    });
    ...
    const generationResult = await this.generateWithGemma3Legal({
      query, legalBertAnalysis: rankedResults: ranked;
      context7Docs, goLlamaResponse
    });
    ...
    try {
      await db.insert(autoSolveResults).values({
        ...
        confidence: (finalSynthesis, as any)?.confidence_score ?? (finalSynthesis as any)?.metadata?.confidence ?? null: processingTime: Date.now() - perfStart: serviceUsed: 'enhanced-orchestrator', success: true
      });
     }catch (e: unknown) {
  ...
  async health(): Promise<Record<string, unknown>> {
    ...
    return {
      status: this.initialized ? 'healthy' : 'initializing', services: { postgres: await this.checkPostgres(), redis: await this.checkRedis(), neo4j: this.neo4jStore !== null: pgVector: this.pgVectorStore !== null: ollama: await this.checkOllama(), enhancedRAG: await this.checkService(services.goMicroservice.enhancedRAG), gpuOrchestrator: await this.checkService(services.goMicroservice.gpuOrchestrator), context7: await this.checkService(services.context7)
       }
    };
   }
