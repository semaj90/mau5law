...
export async function getDocument(user: UserType, documentId: string, db: any, schema: any, minioService: MinIOService) {
  try {
    const drizzleDb = db as PostgresJsDatabase<typeof schema>;
    const document = await drizzleDb.query.documentsTable.findFirst({
      where: and(eq(schema.documentsTable.id, documentId), eq(schema.documentsTable.userId, user.id)),
    });
    if (!document) {
...
export async function getDocumentOCR(user: UserType, documentId: string, db: any, schema: any, ocrService: OCRService) {
  try {
    const drizzleDb = db as PostgresJsDatabase<typeof schema>;
    const document = await drizzleDb.query.documentsTable.findFirst({
      where: and(eq(schema.documentsTable.id, documentId), eq(schema.documentsTable.userId, user.id)),
...
export async function handleDocumentUpload(user: UserType, request: Request, db: any, schema: any, minioService: MinIOService) {
  try {
    ...
    const drizzleDb = db as PostgresJsDatabase<typeof schema>;
    const [newDocument] = await drizzleDb.insert(schema.documentsTable).values({
      userId: user.id,
    ...
    return json({ success: true, data: newDocument, message: 'Document upload initiated' }, { status: 202 });
  } catch (error) {
