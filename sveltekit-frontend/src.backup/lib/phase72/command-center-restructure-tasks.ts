...
export const phase6_72_restructure_tasks = [
	// System Tab Tasks
	{
		...
		description: 'Merge duplicated handlers under src/routes/api/yorha/**, api/ai/**, and api/rag/** into api/legal/… tree',
		...
		actions: [
			{
				...
				command: 'grep -r "export const POST" src/routes/api/yorha src/routes/api/ai src/routes/api/rag',
	...
	},
	{
		...
		description: 'GPU-accelerated error vectorization using embeddinggemma (384-d) or native LibTorch addon',
		...
		actions: [
			...
			},
			{
				...
				command: 'curl -X POST http://localhost:5173/api/phase72/capture-error -d \'{"file_path":"test.ts","message":"Property foo does not exist"}\'',
				expected: 'Error stored with embedding'
			},
			{
				label: 'Test similarity search',
				command: 'curl -X POST http://localhost:5173/api/phase72/similar-errors -d \'{"message":"Property does not exist","threshold":0.85}\'',
	...
	// Evidence Tab Tasks
	{
		...
		description: 'Retire RealTimeEvidenceGrid, keep only YoRHa EvidenceGrid after Svelte 5 stabilizes',
		...
		actions: [
			...
			},
			{
				...
				command: 'rm src/lib/components/evidence/RealTimeEvidenceGrid.svelte src/lib/components/evidence/EvidenceGrid.svelte',
		...
		actions: [
			{
				...
				command: 'Get-ChildItem -Recurse src/routes/archive -Filter "*.svelte" | Select-Object Name',
	...
	},
	{
		...
		description: 'Continuous ts-check/svelte-check across unified /api, legal, RAG, GPU, archive segments',
