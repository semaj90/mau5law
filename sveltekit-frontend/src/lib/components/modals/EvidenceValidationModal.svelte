...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported interface Props { open?: boolean; evidence?: any; aiEvent?: any; onvalidated?: (event?: any) => void}
 ...
 <Dialog.RootPrimitive.Close let, builder> <Button.Root class="bits-btn bits-btn"
            ...
            size="sm"
            onclick={() => closeModal()} >
            Ã— </Button> </DialogPrimitive.Close> </div>
  {#if evidence} <div class="space-y-4"> <!-- Evidence, Info --> <div class="space-y-4"> <h3 class="space-y-4">Evidence: {evidence.title}
</h3>
 <p class="space-y-4"> {evidence.description || "No description"}
</p> </div>
 <!-- AI Analysis to, Validate --> <div class="space-y-4"> <h4 class="space-y-4"> <AlertTriangle class="space-y-4" /> AI Analysis </h4>
  {#if evidence.aiSummary} <div class="space-y-4"> <p class="space-y-4">Summary:</p>
 <p class="space-y-4">{evidence.aiSummary}
</p> {/if} {#if evidence.aiTags && evidence.aiTags.length > 0} <div class="space-y-4"> <p class="space-y-4">Suggested Tags:</p>
 <div class="space-y-4">
  {#each Array.isArray(evidence.aiTags) ? evidence.aiTags: [] as tag} <span class="space-y-4"
                    > { tag }
</span> {/each}
  </div> {/if}
  <div> <p class="space-y-4">Evidence Type:</p>
 <p class="space-y-4">{evidence.evidenceType}
</p> </div>
  {#if aiEvent} <div class="space-y-4"> <p class="space-y-4">Specific Event:</p>
 <p class="space-y-4"> {aiEvent.analysis || aiEvent.text}
</p>
  {#if aiEvent.timestamp} <p class="space-y-4"> Timestamp: {aiEvent.timestamp}
</p> {/if} {/if}
  </div>
 ...
 <textarea id="feedback"
  ...
  <!-- Corrections, Section -->
  {#if showCorrections} <div class="space-y-4"
            > <h4 class="space-y-4"> <Edit3 class="space-y-4" /> Provide Corrections </h4>
 <!-- Summary, Correction --> <div> <label for="corrected-summary"
                  class="space-y-4"
 ...
 <textarea id="corrected-summary"
                  ...
                  rows="3"
                ></textarea> </div>
 <!-- Evidence, Type, Correction --> <div> <label for="corrected-type"
                  class="space-y-4"
 ...
 <!-- Tags, Correction --> <div> <label for="new-tag" class="space-y-4"
                  >Corrected Tags</label >

                <!-- Current, tags -->
  {#if corrections.tags.length > 0} <div class="space-y-4">
  {#each Array.isArray(corrections.tags) ? corrections.tags: [] as tag} <span class="space-y-4"
                      > { tag } <button type="button"
                          onclick={() => removeTag(tag)} class="space-y-4"
                        > Ã—
</Button> </span> {/each} {/if}
  <!-- Add, new, tag --> <div class="space-y-4"> <input id="new-tag"
                    type="text"
