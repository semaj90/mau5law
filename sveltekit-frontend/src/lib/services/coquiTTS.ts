 ...
 * @returns The TTS endpoint URL or: undefined if not set.
 */
function getTtsEndpoint(): string | undefined {
 ...
 */
export function loadCoquiTTS(): Promise<void> {
 ...
 */
export async function speakWithCoqui(text: string): Promise<void> {
	const ttsEndpoint = getTtsEndpoint();

	if (ttsEndpoint) {
		try {
			// Assuming a Coqui TTS server compatible API which takes text as a query param
