...
export class Context7MissingImportsFetcher {
  ...
  */
   ...
   */
  private async fetchContext7Docs(libraryId: string, topics: string[], maxTokens = 10000): Promise<Context7McpResponse> {
  ...
  */
  private hasSvelteMissingItems(analysis: MissingImportAnalysis): boolean {
    ...
    return svelteItems.some((item) =>
      analysis.missingFunctions.has(item) || analysis.missingMethods.has(item)
    );
  }
