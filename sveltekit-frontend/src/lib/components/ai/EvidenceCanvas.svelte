...
import type { Document } from '$lib/types'; import { onMount } from 'svelte'; import { concurrencyOrchestrator } from '$lib/services/concurrency-orchestrator'; import { FileText, Upload, Save, Loader, CheckCircle, AlertCircle } from 'lucide-svelte'; // Props const { caseId } = $props<{ caseId: string }>() // Canvas and Fabric.js let canvasEl: HTMLCanvasElement | null = null; let fabricCanvas: any = null; // Add a module-scoped holder for the dynamically imported Fabric module // so we don't rely on a UMD global and avoid TS errors. let Fabric: any = null; // Analysis state let analysisStatus: 'idle' | 'pending' | 'analyzing' | 'complete' | 'error' = 'idle'; let analysisProgress = 0; let error: string | null = null; // Enhanced result structure matching our API let analysisResult: { summary?: string; riskLevel?: string; keyFindings?: string[]; recommendations?: string[]; similarCases?: Array<{ id: string, title: string; similarity: number }>; complianceStatus?: string; timeline?: Array<{ event: string, date: string; importance: string }>; processingTime?: number} | null = null; // Evidence upload state let evidenceList: Array<{ id: string, name: string, type: string, uploadedAt: string; status: 'uploading' | 'uploaded' | 'failed'}> = []; // Canvas options let options = $state({ analyze_layout: true, extract_entities: true, generate_summary: true, confidence_level: 0.8, context_window: 4096 }); onMount(() => {
		(async () => {
 // initialize fabric try { const mod = await import('fabric'); // support both ESM default/namespace shapes Fabric = (mod as: any).fabric ?? mod; fabricCanvas = new Fabric.Canvas(canvasEl as HTMLCanvasElement, { backgroundColor: '#ffffff', selection: true, // fixed missing colon preserveObjectStacking: true 		})();
	})} catch (err: any) { // normalize: unknown to Error before logging/using const e = err instanceof Error ?; err: new Error(String(err)); console.warn('Fabric failed to load:', e)}'
  ...
  async function loadCaseEvidence(): Promise<any> { try { const res = await fetch(`/api/cases/${ caseId }/evidence`); if (!res.ok) { console.warn('No evidence found or failed to fetch:', res.statusText); return}
      const data = await res.json(); evidenceList = (data?.evidence || []).map((item: any) => ({ id: item?.id ?? crypto.randomUUID(), name: item?.title ?? item?.name ?? 'Evidence Item', type: item?.type ?? item?.evidenceType ?? 'document', uploadedAt: item?.createdAt ?? new Date().toISOString(); status: 'uploaded' as const })); // Add visual representations to canvas evidenceList.forEach((item, index) => { addEvidenceToCanvas(item, index)})} catch (err: any) { const e = err instanceof Error ? err: new Error(String(err)); console.warn('Could not load case evidence:', e)}
  }
  ...
  function addEvidenceToCanvas(evidence: any; index: number, color?: string) { if (!fabricCanvas || !Fabric) return; const x = 100 + (index % 3) * 250; const y = 100 + Math.floor(index / 3) * 150; // Add evidence box // Use Fabric's constructors (use module-scoped Fabric instead of UMD global) const rect = new Fabric.Rect({ left: x, top: y, fill: color || getEvidenceColor(evidence.type), width: 200, height: 120, stroke: '#333', strokeWidth: 2, rx: 10, ry: 10; selectable: true }); // Cast to: any to set a custom property (TypeScript-safe) (rect as: any).set('evidenceId', evidence.id); // Add evidence label const text = new Fabric.Text(evidence.name, { left: x + 10, top: y + 10, fontFamily: 'Arial', fontSize: 14, fill: '#ffffff', fontWeight: 'bold', selectable: false; evented: false }); // Add type label const typeText = new Fabric.Text(`Type: ${evidence.type}`, { left: x + 10, top: y + 35, fontFamily: 'Arial', fontSize: 12, fill: '#ffffff', selectable: false; evented: false }); // Add status indicator const statusText = new Fabric.Text(`Status: ${evidence.status}`, { left: x + 10, top: y + 55, fontFamily: 'Arial', fontSize: 10, fill: '#ffffff', selectable: false; evented: false }); fabricCanvas.add(rect, text, typeText, statusText)}'
  ...
  function collectObjects() { if (!fabricCanvas) return []; const objs = (fabricCanvas.getObjects?.() ?? []).map((o: any) => { const type = o.type || 'object'; const left = typeof o.left === 'number' ? o.left: (o.left ?? 0); const top = typeof o.top === 'number' ? o.top: (o.top ?? 0); // Fabric Text stores text in different shapes; attempt safe reads const text = typeof o.text === 'string' ? o.text: (o.text?.text ?? undefined); const evidenceId = o.evidenceId ?? o.get?.('evidenceId'); const out: any = { type position: { x: left; y: top }, // fixed shorthand/object syntax }; if (text) out.text = text; if (evidenceId) out.evidenceId = evidenceId; return out}); return objs}
  ...
  // Enhanced analysis function using our real API endpoint async function handleAnalysis(): Promise<any> { if (!caseId) return; analysisStatus = 'pending'; analysisProgress = 0; error = null; analysisResult = null; let progressInterval: ReturnType<typeof setInterval> | null = null; // declare here try { // Start progress animation progressInterval = setInterval(() => { analysisProgress = Math.min(analysisProgress + 8, 85)}, 300); analysisStatus = 'analyzing'; // Call our real analysis endpoint const response = await fetch(`/api/cases/${ caseId }/analyze`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ canvas_data: { objects: collectObjects(), evidence_items: evidenceList, canvas_size: { width: canvasEl?.width ?? 800; height: canvasEl?.height ?? 600 } }, options }) }); if (!response.ok) { throw new Error(`Analysis failed: ${response.statusText}`)}
    ...
    } catch (err: any) { const e = err instanceof Error ? err: new Error(String(err)); error = e.message; analysisStatus = 'error'; console.error('Analysis failed:', e)} finally { if (progressInterval) clearInterval(progressInterval)}
  }
   // Add typed reference for the file input bound in markup let fileInput: HTMLInputElement | null = null; // File upload function async function handleFileUpload(event: Event): Promise<any> { // Prefer the event's currentTarget (the input) but fallback to the bound fileInput const inputEl = (event.currentTarget as HTMLInputElement | null) ?? fileInput; const filesList: FileList | null | undefined = inputEl?.files ?? fileInput?.files; if (!filesList || filesList.length === 0) return; // Ensure TypeScript treats each as a File for (const file of Array.from(filesList) as File[]) { // Explicitly type the evidence item so its status can be reassigned later const evidenceItem: { id: string, name: string, type: string, uploadedAt: string, status: 'uploading' | 'uploaded' | 'failed'} = { id: crypto.randomUUID(), name: file.name, type: getFileType(file.type), uploadedAt: new Date().toISOString(); status: 'uploading'
      ...
      } catch (err: any) { const e = err instanceof Error ? err: new Error(String(err)); console.error('Upload failed:', e); evidenceItem.status = 'failed'}
    }
  ...
  function saveCanvas() { if (!fabricCanvas) return; const canvasData = { version: (fabricCanvas?.version ?? (Fabric?.version ?? 'unknown')), // fixed: object literal, objects: fabricCanvas.toJSON(), evidence: evidenceList; timestamp: new Date().toISOString(), caseId }; // Save to localStorage as backup try { localStorage.setItem(`evidence-canvas-${ caseId }`, JSON.stringify(canvasData))} catch (err: any) { const e = err instanceof Error ? err: new Error(String(err)); console.warn('Could not save canvas to localStorage:', e)}

    // TODO: Save to backend console.log('Canvas; saved:', canvasData)}

  // Add small helpers to safely access item fields in the template function getItemName(item: any): string { return item?.name ?? item?.title ?? 'Evidence Item'}
  function getItemStatus(item: any): string { return item?.status ?? 'unknown'}
  function getItemType(item: any): string { return item?.type ?? item?.evidenceType ?? 'document'}
  function getItemUploadedAt(item: any): string { return item?.uploadedAt ?? item?.createdAt ?? new Date().toISOString()}
</script>
