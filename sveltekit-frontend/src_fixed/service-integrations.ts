 ...
 * - Ollama embeddings & chat (gemma3: legal-latest)
...
export interface OllamaClient {
    ...
    chat(messages: Array<{ role: string; content: string }>, opts?: { model?: string; stream?: boolean }): Promise<string | AsyncIterable<string>>;
}
...
export interface QdrantClient {
    ...
    search<T = Record<string, unknown>>(collection: string, vector: number[], limit?: number): Promise<QdrantSearchResult<T>[]>;
...
export interface PgVectorClient {
    ...
    search(collection: string, vector: number[], limit?: number): Promise<Array<{ id: string; similarity: number; metadata: Record<string, unknown> }>>;
    insert(collection: string, vectors: Array<{ id: string; vector: number[]; metadata?: Record<string, unknown> }>): Promise<void>;
    disconnect(): Promise<void>;
}
...
export interface MinIOClient {
    ...
    putObject(bucket: string, key: string, data: any, metadata?: Record<string, string>): Promise<{ etag: string }>;
...
export function getServiceUrls(env: ServiceEnvironment): ServiceUrls {
    return {
        ...
        rabbitmqManagement: env.rabbitmqConfig.url.replace('amqp://', 'http://').replace(':5672', ':15672'),
...
export class PgVectorAdapter implements PgVectorClient {
    ...
    async search(
        ...
        const sql = `
            ...
            LIMIT $2
        `;
        const result = await this.query<{ id: string; similarity: number; metadata: any }>(sql, [vectorStr, limit || 10]);
    ...
    async insert(
        ...
        const values = vectors.map(
            (_, i) => `($${i * 3 + 1}, $${i * 3 + 2}::vector, $${i * 3 + 3}::jsonb)`
        ).join(',');
...
export class RabbitMQAdapter implements RabbitMQClient {
    ...
    private async ensureChannel() {
        ...
        this.connection = await amqp.connect(this.config.url, { heartbeat: this.config.heartbeat || 60 });
    ...
    async publishJob(queue: string, payload: unknown): Promise<void> {
        await this.ensureChannel();
...
export function getServiceAdapters() {
    ...
    return {
        ollama: cachedServices.ollama || (cachedServices.ollama = new OllamaAdapter(env.ollamaConfig)),
        redis: cachedServices.redis || (cachedServices.redis = new RedisAdapter(env.redisConfig)),
        qdrant: cachedServices.qdrant || (cachedServices.qdrant = new QdrantAdapter(env.qdrantConfig)),
        pgvector: cachedServices.pgvector || (cachedServices.pgvector = new PgVectorAdapter(env.postgresConfig)),
        minio: cachedServices.minio || (cachedServices.minio = new MinIOAdapter(env.minioConfig)),
        neo4j: cachedServices.neo4j || (cachedServices.neo4j = new Neo4jAdapter(env.neo4jConfig)),
        rabbitmq: cachedServices.rabbitmq || (cachedServices.rabbitmq = new RabbitMQAdapter(env.rabbitmqConfig)),
        env,
...
export async function healthCheckServices(): Promise<any> {
    ...
    try {
        await services.ollama.embed('test', { model: (services.env as ServiceEnvironment).ollamaConfig.embeddingModel });
        results.ollama = true;
    } catch {
