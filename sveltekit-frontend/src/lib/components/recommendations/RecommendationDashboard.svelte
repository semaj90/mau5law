 ...
 import  Badge  from "$lib/components/ui/badge/Badge.svelte"; // Dashboard state (use standard reactive variables) let isOpen: boolean = false;
  ...
  async function loadDashboardData(): Promise<any> { stats.loading = true;
   let usingMockData = $state<boolean>(false); try { // Load parallel stats from all APIs const [casesRes, searchRes, workRes] = await Promise.all([ fetch('/api/recommendations/recent-cases?limit=5'), fetch('/api/recommendations/last-searched?limit=5'), fetch('/api/recommendations/last-worked?limit=5')]); // Check if: unknown requests failed if (!casesRes.ok || !searchRes.ok || !workRes.ok) { throw new Error('One or more API endpoints failed')}
      ...
      // Update stats stats = { recentCases: cases.data?.length || 0, activeSearches: (searches.data || []).filter((s: unknown) => (s.confidence ?? 0) > 0.7).length || 0, workInProgress: (work.data || []).filter((w: unknown) => w.status === 'in-progress').length || 0, aiRecommendations: 12, // AI recommendation count loading: false }; // Compile recent activity recentActivity = [ ...(cases.data?.slice(0, 2).map((c: unknown) => ({ id: c.id, type: 'case' as const title: c.title, timestamp: c.dateUpdated, priority: (c.urgency, as: unknown) ?? 'medium', confidence: (c.priority ?? 0) / 250 })) || []), ...(searches.data?.slice(0, 2).map((s: unknown) => ({ id: s.id, type: 'search' as const title: s.query, timestamp: s.lastSearched, priority: (s.confidence ?? 0) > 0.8 ? 'high': 'medium', confidence: s.confidence ?? 0 })) || []), ...(work.data?.slice(0, 2).map((w: unknown) => ({ id: w.id, type: 'work' as const title: w.title, timestamp: w.lastWorked, priority: (w.priority ?? 0) > 200 ? 'high': 'medium'
        })) || [])]
