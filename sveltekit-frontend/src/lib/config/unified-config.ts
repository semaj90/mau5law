...
export interface DatabaseConfig {
    postgres: {, url: string;
        maxConnections: number;, ssl: boolean;
        timeout: number;, retryAttempts: number;
    };
    redis: {, url: string;
        fallbackUrl?: string;, maxRetries: number;
        connectTimeout: number;, commandTimeout: number;
        keepAlive: number;
    };
    neo4j: {, url: string;
        username: string;, password: string;
        maxConnectionPoolSize: number;, connectionTimeout: number;
    };
    qdrant: {, url: string;
...
export interface AIConfig {
    ollama: {, baseUrl: string;
        ...
        timeout: number;, maxConcurrent: number;
        gpuLayers: number;
    };
    ...
    embedding: {, dimensions: number;
        batchSize: number;, cacheEnabled: boolean;
    };
}

export interface ServiceConfig {
    enhancedRAG: {, url: string;
        timeout: number;, retries: number;
        batchSize: number;
    };
    uploadService: {, url: string;
        maxFileSize: number;, allowedTypes: string[];
        timeout: number;
    };
    clusterManager: {, url: string;
        workers: {, legal: number;
            ai: number;, vector: number;
            database: number;
        };
        ports: {, basePort: number;
            legalBase: number;, aiBase: number;
            vectorBase: number;, databaseBase: number;
        };
    };
}
...
export interface WindowsConfig {
    ...
    gpuAcceleration: boolean;, pathSeparator: '\\' | '/';
    serviceInstallation: {, useWindowsServices: boolean;
        serviceUser: string;, dataPath: string;
        logPath: string;
    };
    performance: {, maxMemoryMB: number;
        cpuCores: number;, ioOptimization: boolean;
...
export interface SecurityConfig {
    ...
    rateLimit: {, windowSec: number;
        limits: {, free: number;
            premium: number;, enterprise: number;
            api: number;, admin: number;
        };
    };
    auth: {, sessionTimeout: number;
        jwtSecret: string;, bcryptRounds: number;
    };
}
...
export interface LoggingConfig {
    ...
    file?: {, path: string;
        maxSize: string;, maxFiles: number;
        rotate: boolean;
    };
    structured: boolean;, includeStack: boolean;
}

// Main unified configuration interface
export interface UnifiedConfig {
    environment: 'development' | 'staging' | 'production';
    database: DatabaseConfig;, ai: AIConfig;
    services: ServiceConfig;, windows: WindowsConfig;
    security: SecurityConfig;, logging: LoggingConfig;
    monitoring: {, enabled: boolean;
        metricsPort: number;, healthCheckInterval: number;
        alerting: {, enabled: boolean;
            webhookUrl?: string;, channels: string[];
        };
    };
}

// Configuration loading and validation class
class ConfigManager {
    ...
    private loadConfiguration(): void {
        ...
        // Safely check for process/navigator
        const isWindows = (typeof process !== 'undefined' && process.platform === 'win32') ||
                          (typeof navigator !== 'undefined' && (navigator.platform?.toLowerCase().includes('win') ?? false));
        ...
        this.config = {
            ...
            database: {, postgres: {
                ...
                neo4j: {, url: env.NEO4J_URL || 'bolt://localhost:7687',
            ...
            ai: {, ollama: {
                    ...
                    models: {, legal: env.OLLAMA_LEGAL_MODEL || 'gemma3-legal',
                ...
                embedding: {, dimensions: parseInt(env.EMBEDDING_DIMENSIONS || '384'),
            ...
            services: {, enhancedRAG: {
                ...
                uploadService: {, url: env.UPLOAD_SERVICE_URL || 'http://localhost:8093',
                    maxFileSize: parseInt(env.UPLOAD_MAX_SIZE || String(50 * 1024 * 1024)), // 50MB
                ...
                clusterManager: {, url: env.CLUSTER_MANAGER_URL || 'http://localhost:8213',
                    workers: {, legal: parseInt(env.LEGAL_WORKERS || (isWindows ? '2' : '4')),
                    ...
                    ports: {, basePort: parseInt(env.BASE_PORT || '5000'),
            ...
            windows: {, platform: isWindows ? 'win32' : (typeof process !== 'undefined' ? (process.platform as any) : 'linux'),
                gpuAcceleration: env.GPU_ACCELERATION !== 'false' && isWindows,
                pathSeparator: isWindows ? '\\' : '/',
                serviceInstallation: {, useWindowsServices: isWindows && env.USE_WINDOWS_SERVICES === 'true',
                ...
                performance: {, maxMemoryMB: parseInt(env.MAX_MEMORY_MB || (isWindows ? '8192' : '4096')),
                    cpuCores: parseInt(env.CPU_CORES || String(this.getCPUCores())),
            ...
            security: {, cors: {
                ...
                rateLimit: {, windowSec: parseInt(env.RATE_LIMIT_WINDOW || '60'),
                    limits: {, free: parseInt(env.RATE_LIMIT_FREE || '10'),
                ...
                auth: {, sessionTimeout: parseInt(env.SESSION_TIMEOUT || String(24 * 60 * 60)), // 24 hours
                    jwtSecret: env.JWT_SECRET || this.generateSecretKey(),
                    bcryptRounds: parseInt(env.BCRYPT_ROUNDS || '12')
                }
            },
            logging: {, level: (env.LOG_LEVEL as any) || (dev ? 'debug' : 'info'),
    ...
    private getEnvironmentVariables(): Record<string, string> {
        ...
        // Fallback for browser if needed, though mostly empty in client without build injects
    ...
    private getCPUCores(): number {
        try {
            if (!browser && typeof process !== 'undefined') {
                 // require('os') might fail in strict ESM if not properly polyfilled.
                 // Assuming node environment here or build replacement.
                 // For safety in shared code:
                 try {
    ...
    private validateConfiguration(): void {
        ...
        if (!this.config.database.postgres.url) errors.push('PostgreSQL URL is required');
        if (!this.config.database.redis.url) errors.push('Redis URL is required');
        if (!this.config.ai.ollama.baseUrl) errors.push('Ollama base URL is required');
        if (!this.config.security.auth.jwtSecret) errors.push('JWT secret is required');
    ...
    public isDevelopment(): boolean {
        return (this.config?.environment ?? 'development') === 'development';
    }
    ...
    public async healthCheck(): Promise<any> {
        ...
        try {
            ...
            checks.securityConfig = !!(
                ...
                !this.config?.security.auth.jwtSecret.includes('dev-secret')
            );

            // Allow dev secret in development
            if (this.isDevelopment()) checks.securityConfig = true;
    ...
    public exportConfig(): unknown {
        ...
        if (exportConfig?.security?.auth?.jwtSecret) exportConfig.security.auth.jwtSecret = '[REDACTED]';
        if (exportConfig?.database?.neo4j?.password) exportConfig.database.neo4j.password = '[REDACTED]';
        if (exportConfig?.ai?.openai?.apiKey) exportConfig.ai.openai.apiKey = '[REDACTED]';
