...
async function fastTypeScriptCheck() {
  ...
  const { code, output } = await runCommand('npx', [
    'tsc', '--noEmit', '--skipLibCheck', '--incremental', '--assumeChangesOnlyAffectDirectDependencies'
  ]);
...
async function lightweightSvelteCheck() {
  ...
  const { code, output } = await runCommand('npx', [
    'svelte-check', '--threshold', 'error', '--output', 'human', '--diagnostic-sources', 'svelte', '--no-tsconfig'
  ]);
...
async function main() {
  ...
  try {
    ...
    const [tsResult, svelteResult] = await Promise.all([
      fastTypeScriptCheck(), lightweightSvelteCheck()
    ]);
