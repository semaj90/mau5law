...
class MockSIMDGPUParserIntegration {
  ...
  constructor(_config: ParsingConfig) {}
  async initializeGPU(): Promise<void> {
    console.log('🔧 Mock GPU initialization');
  }
  // metadata is intentionally unused - prefix with _
  // narrow metadata shape used by the parser to avoid `any`
  async parseDocument(
    content: string,
    _metadata?: {
      title?: string;
...
class MockPhysicsAwareGPUOrchestrator {
  ...
  // typed scheduleTask to avoid implicit any usage
  async scheduleTask(task: GPUOrchestratorTask): Promise<GPUOrchestratorTaskResult> {
    console.debug('Mock scheduleTask', { type: task.type, priority: task.priority });
    return { id: `mock-task-${Date.now()}`, queuedAt: Date.now(), status: 'queued' };
  }
  getGPUStats() {
...
interface RedisPipeline {
  ...
  exec(): Promise<Array<string | null | 'OK'>>;
}
...
export class UnifiedLegalSIMDPGVector {
    ...
    try {
      ...
      if (!db) {
        try {
          ...
          if (isDBClient(dbModule)) {
            db = dbModule;
          } else {
            const modAny = dbModule as Record<string, unknown>;
            if (isDBClient(modAny.db)) db = modAny.db;
            else if (isDBClient(modAny.default)) db = modAny.default as DBClient;
          }

          // Normalize sql tag (drizzle.sql) with guard
          if (drizzleModule && isSQLTag(drizzleModule.sql)) {
  ...
  private async getDB(): Promise<{ db: DBClient; sql: SQLTag }> {
    ...
    // non-null assertion is safe after ensurePGVectorExtension
  ...
  private async createOptimizedIndexes(): Promise<void> {
    try {
      // Ensure database is initialized (get typed instances)
      const { db: _db, sql: _sql } = await this.getDB();
  ...
  async processAndStoreLegalDocument(
    ...
    try {
      ...
      // scheduleTask expects a task descriptor - invoke without keeping unused var
      await physicsAwareGPUOrchestrator.scheduleTask({
        type: 'inference',
        requiredMemory,
        estimatedDuration,
        priority: 'high',
      ...
      // ensure typing matches ProcessingStats
  ...
  private async generateDocumentEmbeddings(parsedDocument: ParsedDocument): Promise<{
    ...
    const ensureFetch = async (): Promise<typeof fetch> => {
      ...
      const mod = (await import('node-fetch')) as unknown as NodeFetchModule;
      const nodeFetch = mod.default ?? mod.fetch;
      if (!nodeFetch) {
        // as a last resort, throw a clear error (calling code will fallback to simulated embeddings)
        throw new Error('No fetch available (node-fetch import failed)');
      }
      return nodeFetch;
    };
    ...
    if (allInputs.length) {
      try {
        ...
        // Try candidate endpoints (resolved via getOllamaEndpoint and sensible fallbacks)
        const endpoints = getOllamaEndpointCandidates();

        for (const ep of endpoints) {
          try {
            // small per-endpoint timeout using AbortController when available
            ...
            console.warn('Embedding attempt failed for endpoint', ep, err);
            // try next endpoint
          }
        }
      } catch (err) {
  ...
  async semanticSearch(
    ...
    try {
      ...
      // removed unused queryEmbedding generation
      ...
      const vectorResults: VectorSearchResult[] = rows.map((row: DBRow) => {
        const doc: LegalDocumentVector = {
          ...
          extracted_entities: (row.extracted_entities as ExtractedEntity[]) || [],
          suggestions: (row.suggestions as DidYouMeanSuggestion[]) || [],
          confidence_score: (row.confidence_score as number) || 0,
          processing_time_ms: (row.processing_time_ms as number) || 0,
      ...
      try {
        ...
        /* ignore cache set errors (best-effort caching) */
        console.debug?.('redis.set ignored error', err);
      }
  ...
  async getSystemStats(): Promise<SIMDPGVectorStats> {
    try {
      ...
      const docCountRes = await _db.execute('SELECT COUNT(*) as total FROM legal_documents_vectorized;');
      const avgProcessingRes = await _db.execute(
        'SELECT AVG(processing_time_ms) as avg_time FROM legal_documents_vectorized;'
      );
      const vectorCountRes = await _db.execute(
...
// small concrete types for the mock orchestrator (avoid `any`)
interface GPUOrchestratorTask {
  type: 'inference' | 'preprocess' | string;
...
interface GPUOrchestratorTaskResult {
  id: string;
  queuedAt?: number;
  status?: 'queued' | 'running' | 'completed' | 'failed';
}

// Add a single canonical default Ollama port used by the module
...
/**
 * Resolve the Ollama endpoint using the following precedence:
...
export function getOllamaEndpoint(): string {
  try {
    ...
    // 1) Vite / SvelteKit env override (typed access)
    try {
      const meta = import.meta as unknown as { env?: Record<string, string | undefined> };
      if (meta && meta.env) {
        candidates.push(meta.env.VITE_OLLAMA_ENDPOINT);
        candidates.push(meta.env.OLLAMA_ENDPOINT);
      }
    } catch {
      // ignore
    }
    ...
    if (typeof globalAny.OLLAMA_ENDPOINT === 'string' && (globalAny.OLLAMA_ENDPOINT as string).length) {
      candidates.push(globalAny.OLLAMA_ENDPOINT as string);
    }

    // 3) Node/process.env (SSR or build-time)
    try {
      ...
      if (procEnv) {
        candidates.push(procEnv.OLLAMA_ENDPOINT);
        candidates.push(procEnv.VITE_OLLAMA_ENDPOINT);
      }
    } catch {
      // ignore
    }

    const found = candidates.find(c => typeof c === 'string' && c && c.length > 0);
    ...
    const protocol =
      typeof window !== 'undefined' && window.location && window.location.protocol
        ...
        : 'http';
    // default Ollama port used by local host
    return `${protocol}://${host}:${DEFAULT_OLLAMA_PORT}`;
  } catch {
    // Use the same canonical default (no hardcoded literal URL)
    return `http://localhost:${DEFAULT_OLLAMA_PORT}`;
  }
}

/**
 * Return a prioritized list of Ollama endpoint candidates.
 * The first entry is the resolved endpoint from getOllamaEndpoint(),
 * followed by common fallbacks (docker port, localhost variants).
 */
function getOllamaEndpointCandidates(): string[] {
  // primary resolved endpoint (may come from env/import.meta/global)
  const primary = getOllamaEndpoint();
  const parsed = (() => {
    try {
  ...
  // add primary as-is
  candidates.add(primary.replace(/\/$/, ''));
  // docker default (alternate Ollama port)
  candidates.add(`${proto}://${host}:11435`);
  // host-local variants
  candidates.add(`http://${host}:11434`);
  candidates.add(`http://${host}:11435`);
  // localhost fallbacks
