 ...
 */
export interface DocumentMetadata { filename: string; fileSize: number;
  mimeType: string;
  uploadedAt: string;
  caseId?: string;
  documentType?: string;
...
export interface ProcessingResult { documentId: string; status: 'processing' | 'completed' | 'failed';
  extractedText?: string;
  embeddings?: number[][];
  analysis?: { summary: string; entities: Record<string, unknown>[]; // replaced: any[] -> Record<string, unknown>[]
    sentiment: string;
...
export interface UploadProgress { documentId: string; progress: number;
  stage: string;
...
export interface ProcessingAnalytics { totalDocuments: number; processingStats: { completed: number; processing: number; failed: number };
...
export class DocumentApiService {
  ...
  async listDocuments(
    ...
    try {
      ...
      return {
        documents: [], total: 0, page: options.page ?? 1, limit: options.limit ?? 10
      }; }
  ...
  async getProcessingAnalytics(caseId?: string): Promise<ProcessingAnalytics | null> {
    try {
      ...
      return {
        totalDocuments: 0, processingStats: { completed: 0, processing: 0, failed: 0 }, averageProcessingTime: 0, documentTypes: { }
      }; }

  /**
   * Integrate with legal ingest API
   */
  async processLegalDocuments( files: File[];
    options: { caseId: string; jurisdiction?: string; enhanceRAG?: boolean  }
  ): Promise<
    | { success: boolean; caseId: string;
        documentsProcessed: number;
        totalProcessingTime: number; documents: Record<string, unknown>[]; // replaced: any[] -> Record<string, unknown>[]
        error?: string;
       }
  ...
  > {
    try {
      ...
      return (await response.json()) as
        | { success: boolean; caseId: string;
            documentsProcessed: number;
            totalProcessingTime: number; documents: Record<string, unknown>[];
            error?: string;
           }
        | { success: false; error: string };
     }catch (error: any) {
