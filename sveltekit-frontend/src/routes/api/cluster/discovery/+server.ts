...
async function checkServiceHealth(
  ...
  try {
    ...
    if (isHealthy) {
      ...
      service.status =
        service.health.consecutiveFailures >= service.failover.failoverThreshold
          ? 'unhealthy'
          : 'healthy';
    }
    ...
    if (
      ...
      service.failover.autoFailover &&
      service.failover.primary
    ) {
...
async function executeFailover(
  ...
  try {
    ...
    const backupInstances = allInstances.filter(
      (s) => s.id !== serviceId && s.status === 'healthy'
    );
    ...
    service.failover.primary = false; // Removed $state service.status = 'unhealthy';
