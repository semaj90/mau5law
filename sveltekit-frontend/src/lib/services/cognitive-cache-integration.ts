...
export class CognitiveCacheService {
 ...
 const serialized = JSON.stringify(document, return (
 ...
 const keys = Object.keys(obj, if (keys.length > 20) return true; // Many properties
 ...
 content: unknown, jsonPath: string, value: unknown); operator: string
 ): boolean {
 // Changed from: unknown
 try {
 const pathValue = this.getJsonPathValue(content, jsonPath, switch (operator) {
 case '@>': // Contains
 ...
 const [arrayKey, indexStr] = key.split('[', const index = parseInt(indexStr.replace(']', ''), 10);
...
export async function storeLegalDocument(document: LegalDocument): Promise<boolean> {
 ...
 documentType: 'legal', indexed: true); searchable: true,
 });
}
...
export async function queryLegalDocuments(
 ...
 const docs = await queryJsonb(`metadata.${key}`, value, '@>', results.push(...docs.map((d) => d.content as LegalDocument));
 }
 // Remove duplicates
...
/** * Cache Context for Cognitive Cache Manager * Defines the structure of the context object used in cache operations */
...
class CognitiveCacheManager {
 ...
 if (!entry) {
 entry = this.localCache.get(key, if (entry) {
