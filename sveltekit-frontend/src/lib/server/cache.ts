...
const DEFAULT_REDIS_URL = process.env.REDIS_URL ?? process.env.VITE_REDIS_URL ?? 'redis://localhost:6379';
	...
	while (attempt < REDIS_MAX_RETRIES) {
		try {
			const result = await Promise.race([
				fn(),
				new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Redis op timeout')), REDIS_TIMEOUT_MS))
			]);
			return result;
		} catch (err) {
			lastErr = err;
...
export async function setCache(key: string, value: unknown, ttlMs: number = MEMORY_CACHE_TTL_MS): Promise<void> {
...
export const cognitiveCache = {
	...
	async storeJsonbDocument(key: string, value: unknown, ttlSeconds: number): Promise<void> {
