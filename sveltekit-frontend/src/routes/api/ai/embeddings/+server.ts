...
import type { gemmaEmbeddingsService  } from '$lib/services/gemma-embeddings-service'; // Changed to named import

// Define a type for the results of embedding generation, based on usage
interface EmbeddingGenerationResult {
    success: boolean;
    // Add other properties if they are consistently present in batch results
    // e.g., embedding?: number[]; error?: string;
}
...
export const POST: RequestHandler = async ({ request, url }) => {
    try {
        ...
        switch (action) {
            case 'embed': {
                ...
                if (!validatedData.success) {
                    return json({ success: false, error: 'Invalid request data', details: validatedData.error.flatten() }, { status: 400 });
                }
                ...
                if (result.success) {
                    ...
                    return json({ success: false, error: result.error || 'Failed to generate embedding' }, { status: 500 });
                }
            }
            case 'search': {
                ...
                if (!searchData.success) {
                    return json({ success: false, error: 'Invalid search request', details: searchData.error.flatten() }, { status: 400 });
                }

                // First generate embedding for the query
                const queryEmbedding = await gemmaEmbeddingsService.generateEmbedding({ text: searchData.data.query });
                if (!queryEmbedding.success || !queryEmbedding.embedding) {
                    return json({ success: false, error: 'Failed to generate query embedding' }, { status: 500 });
                }
            ...
            case 'batch': {
                ...
                if (!batchData.success) {
                    return json({ success: false, error: 'Invalid batch request', details: batchData.error.flatten() }, { status: 400 });
                }

                // Process batch embeddings
                const batchResults: EmbeddingGenerationResult[] = await gemmaEmbeddingsService.batchGenerateEmbeddings(
        ...
        console.error('Gemma Embeddings API error:', error);
        return json({ success: false, error: error instanceof Error ? error.message : 'Processing failed' }, { status: 500 });
    }
};
    ...
    try {
        ...
        switch (action) {
            ...
            case 'optimize':
                ...
                return json({ success: true, data: { message: 'Vector indexes optimized successfully' } });
            default:
                return json({ success: false, error: 'Unknown action' }, { status: 400 });
        }
    } catch (error) {
        return json({ success: false, error: error instanceof Error ? error.message : 'Request failed' }, { status: 500 });
    }
};
