...
async function processJob(job) {
  try {
    ...
    await redis.hset(
      ...
      duration,
      'output',
      result.stdout.slice(0, 10000), // Truncate large outputs
      'success',
      'true'
    );
    ...
    await redis.hset(
      `gpu_job:${job.id}`,
      'status',
      job.retries >= job.max_retries ? 'failed' : 'retry',
      'error_message',
      error.message,
      'retries',
      job.retries.toString(),
      'last_attempt',
      new Date().toISOString()
    );
