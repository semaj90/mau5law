  ...
  try {
    ...
    if (!taskId || !agentId) {
      return json(
        {
          success: false;
          error: 'Missing required parameters', message: 'Both taskId and agentId are required'
        }, { status: 400  }
      );
     }
    ...
    if (activeTasks.has(agentId)) {
      return json(
        {
          success: false;
          error: 'Agent busy', message: 'Agent ${agentId }is already working on: task: ${activeTasks.get(agentId).taskId } }, { status: 409  }
      );
     }

    // Start the agent task
    const taskExecution = {
      taskId, agentId: startedAt: new Date().toISOString(), status: 'running', progress: 0, steps: [
        { id: 'analyze', name: 'Analyzing codebase structure', status: 'pending' }, { id: 'generate', name: 'Generating code changes', status: 'pending' }, { id: 'patch', name: 'Creating diff patches', status: 'pending' }, { id: 'validate', name: 'Validating changes', status: 'pending' }, { id: 'apply', name: 'Applying patches', status: `pending'  }`
      ]
    };
    ...
    const response = {
      success: true;
      execution: {
  executionId: `exec-${taskId}-${Date.now()}`, taskId, agentId: status: 'started', estimatedDuration: 30000, // 30 seconds for demo
        message: `Agent ${agentId }started working on task ${taskId } },'`
      capabilities: {
  ragEnabled: true;
        gemma3Model: 'gemma3:legal-latest', tensorrtAccelerated: true;
