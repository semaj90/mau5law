...
<script lang="ts">
 // Svelte, 5 runes are auto-imported interface Props { content?: string; placeholder?: string}
  let { content = '', placeholder = 'Initialize data input...' }: Props = $props(); import { Editor } from '@tiptap/core'; import StarterKit from '@tiptap/starter-kit'; let editor = $state<InstanceType<typeof Editor> | null>(null); let editorElement: HTMLElement | null = null; let selectedFont = $state<string>('Inter'); // Create editor only once when editorElement is available. $effect(() => { if (!editorElement) return; if (editor) return; // guard: don't recreate the editor if it already exists if (typeof window === 'undefined') return; // client-only // capture initial font to avoid creating a reactivity dependency on selectedFont const initialFont = selectedFont; editor = new Editor({ element: editorElement, extensions: [StarterKit], content, editorProps: { attributes: { class: 'nier-editor-content, focus:outline-none', placeholder: placeholder }'
      } }); // apply initial captured font const pmInitial = editorElement?.querySelector('.ProseMirror') as HTMLElement | null; if (pmInitial && initialFont) pmInitial.style.fontFamily = initialFont; return () => { editor?.destroy(); editor = null}; }); // Ensure font re-applies when user changes it $effect(() => { if (!editorElement) return; const pm = editorElement.querySelector('.ProseMirror') as HTMLElement | null; if (pm) pm.style.fontFamily = selectedFont}); const fontOptions = [ { value: 'JetBrains Mono', label: 'JetBrains Mono' }, { value: 'Courier New', label: 'Courier New' }, { value: 'Inter', label: 'Inter' }];
</script>
