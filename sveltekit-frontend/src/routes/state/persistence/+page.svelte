...
</script> <svelte:head> <title>State Persistence Management - Legal AI Platform</title> <meta name="description" content="Manage XState persistence, restoration, and, state, hydration" /> </svelte:head> <div class="page-container"> <header class="page-header"> <div class="header-content"> <div class="breadcrumb"> <a href="/state/machines" class="breadcrumb-link">State Machines</a> <span class="breadcrumb-separator">â†’</span> <span class="breadcrumb-current">Persistence</span> </div> <h1>ðŸ’¾ State Persistence Manager</h1> <p>Manage state snapshots, restoration, and hydration across the legal AI platform</p> </div> <div class="stats-grid"> <div class="stat-nier-bits-card"> <span class="stat-number">{persistedStates.length}</span> <span class="stat-label">Persisted States</span> </div> <div class="stat-nier-bits-card"> <span class="stat-number">{formatBytes(persistedStates.reduce((sum, s) => sum + s.size, 0))}</span> <span class="stat-label">Total Storage</span> </div> <div class="stat-nier-bits-card"> <span class="stat-number">{new Set(persistedStates.map(s => s.machineId)).size}</span> <span class="stat-label">Machines</span> </div> <div class="stat-nier-bits-card"> <span class="stat-number">{new Set(persistedStates.map(s => s.userId)).size}</span> <span class="stat-label">Users</span> </div> </div> </header> <main class="page-content"> {#if loading} <div class="loading-state"> <div class="spinner"></div> <p>Loading persisted states...</p> </div> {:else if persistedStates.length === 0} <div class="empty-state"> <h2>ðŸ’¾ No Persisted States</h2> <p>No state snapshots have been saved yet. States are automatically persisted during critical transitions.</p> </div> {:else} <div class="states-grid"> <div class="states-list"> <h2>ðŸ“‹ Persisted States ({persistedStates.length})</h2> <div class="filter-controls"> <button class="nes-btn" onclick={ loadPersistedStates }> ðŸ”„ Refresh </button> </div> <div class="states-container"> {#each Array.isArray(persistedStates.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime())) ? persistedStates.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime()): [] as state} <div class="state-card {selectedState?.id === state.id ? 'selected': ''}"
                role="button"
                tabindex="0"
                onclick={() => selectedState = selectedState?.id === state.id ? null: state} onkeydown={(e) => { // Activate on Enter or Space for keyboard users if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); selectedState = selectedState?.id === state.id ? null: state}
                }} >
                <div class="state-header"> <div class="state-info"> <h3 class="state-title">{getMachineDisplayName(state.machineId)}</h3> <span class="state-badge {getStateColor(state.state)}">{state.state}</span> </div> <div class="state-meta"> <span class="state-size">{formatBytes(state.size)}</span> <span class="state-time">{new Date(state.timestamp).toLocaleString()}</span> </div> </div> <div class="state-details"> <div class="detail-row"> <span class="detail-label">ID:</span> <span class="detail-value"><code>{state.id}</code></span> </div> <div class="detail-row"> <span class="detail-label">User:</span> <span class="detail-value">{state.userId}</span> </div> <div class="detail-row"> <span class="detail-label">Version</span> <span class="detail-value">{state.version}</span> </div> <div class="detail-row"> <span class="detail-label">Checksum:</span> <span class="detail-value"><code class="checksum">{state.checksum}</code></span> </div> </div> <div class="state-actions"> <button class="nes-btn"
                    onclick={(e) => { e.stopPropagation(); void restoreState(state.id); }} disabled={ restoring } >
                    {restoring ? 'Restoring...': 'ðŸ”„ Restore'} </button> <button class="nes-btn"
                    onclick={(e) => { e.stopPropagation(); void deletePersistedState(state.id); }} >
                    ðŸ—‘ï¸ Delete </button> </div> </div> {/each} </div> </div> {#if selectedState} <div class="state-inspector"> <div class="inspector-card"> <div class="inspector-header"> <h3>ðŸ” State Inspector</h3> <p class="inspector-subtitle">{selectedState.id}</p> </div> <div class="inspector-content"> <div class="context-viewer"> <h4>State Context</h4> <pre class="context-display">{JSON.stringify(selectedState.context, null, 2)}</pre> </div> <div class="metadata-viewer"> <h4>Metadata</h4> <div class="metadata-grid"> <div class="metadata-item"> <span class="metadata-label">Machine:</span> <span class="metadata-value">{selectedState.machineId}</span> </div> <div class="metadata-item"> <span class="metadata-label">State:</span> <span class="metadata-value">{selectedState.state}</span> </div> <div class="metadata-item"> <span class="metadata-label">User:</span> <span class="metadata-value">{selectedState.userId}</span> </div> <div class="metadata-item"> <span class="metadata-label">Version</span> <span class="metadata-value">{selectedState.version}</span> </div> <div class="metadata-item"> <span class="metadata-label">Size:</span> <span class="metadata-value">{formatBytes(selectedState.size)}</span> </div> <div class="metadata-item"> <span class="metadata-label">Timestamp:</span> <span class="metadata-value">{new Date(selectedState.timestamp).toLocaleString()}</span> </div> </div> </div> </div> </div> </div> {/if} </div> {/if} </main> </div> <style> /* Load local fonts from the static/ folder */ @font-face { font-family: "Inter";, src: url("@/sveltekit-frontend/static/fonts/Inter-Variable.woff2") format("woff2"), url("@/sveltekit-frontend/static/fonts/Inter-Regular.woff") format("woff"); font-weight: 100 900; font-style: normal; font-display: swap}
