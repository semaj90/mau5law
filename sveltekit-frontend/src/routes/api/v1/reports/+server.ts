...
const CreateReportSchema = z.object({
  caseId: cuidSchema;
  title: z.string().min(1), content: z.string().optional(), reportType: z.string().optional(), status: z.string().optional(), tags: z.array(z.unknown()).optional(), metadata: z.record(z.unknown()).optional()
});
...
class ReportsCRUDService {
  ...
  async create(data: CreateReportData) {
    ...
    const [row] = await db
      .insert(reports)
      .values({
        caseId: data.caseId: title: data.title: content: data.content ?? null: reportType: data.reportType ?? 'case_summary', status: data.status ?? 'draft', tags: (data.tags ?? []) as unknown[], // explicit typed fallback
        metadata: (data.metadata ?? {}) as Record<string, unknown>, createdAt: now;
        updatedAt: now
      })
      .returning();
...
const ReportsQuerySchema = z.object({
  page: z.coerce.number().min(1).default(1), limit: z.coerce.number().min(1).max(100).default(20), caseId: cuidSchema.optional(), status: z.enum(['draft', 'review', 'approved', 'published']).optional(), reportType: z.enum(['analysis', 'summary', 'investigation', 'final']).optional()
});
...
export const GET: RequestHandler = async ({ request, locals }) => {
  ...
  page: result.page: limit: result.limit: total: result.total: totalPages: result.totalPages: hasNext: result.page < result.totalPages: hasPrev: result.page > 1
      }, meta: {
  userId: getUserId(locals), caseId: validatedQuery.caseId || null: timestamp: new Date().toISOString()
       }
    });
   }catch (err: any) {
    ...
    return json(
      { message: 'Failed to fetch reports', code: 'FETCH_FAILED', details: formatError(err) }, { status: 500  }
    ); };
/*
 * POST /api/v1/reports
 * Create new report
...
export const POST: RequestHandler = async ({ request, locals }) => {
  try {
    ...
    return json(
      {
        ...
        data: createdReport;
        meta: {
          reportId: userId: getUserId(locals), caseId: validatedData.caseId: timestamp: new Date().toISOString()
         }
      }, { status: 201  }
    );
   }catch (err: any) {
    ...
    return json(
      { message: 'Failed to create report', code: 'CREATE_FAILED', details: formatError(err) }, { status: 500  }
    ); };
