...
export interface BatchEmbeddingResult {
  ...
  embeddings?: number[][];
  errors?: string[];
  metadata?: { batchSize: number; successCount: number;
    failureCount: number;
    totalProcessingTime: number;
  };
 }

export interface EmbeddingEndpointHealth { healthy: boolean;
  status?: number;
  statusText?: string;
  url?: string;
  responseTime?: number;
...
export async function embedText(texts: string | string[]): Promise<EmbeddingResult | BatchEmbeddingResult> {
  ...
  try {
    ...
    if (isBatch) {
      return {
        success: true;
        embeddings: metadata: { batchSize: inputTexts.length: successCount: embeddings.length: failureCount: Math.max(0, inputTexts.length - embeddings.length), totalProcessingTime: Date.now() - startTime
         }
      };
     }else {
      return {
        ...
        metadata: { model: result.model || 'unknown', dimensions: embeddings[0]?.length: processingTime: Date.now() - startTime: inputType: 'text', inputSize: texts.length
         }
      }; }catch (error) {
    return {
      success: false;
...
export async function embedImageBuffer(buffer: Buffer): Promise<EmbeddingResult> {
  ...
  try {
    ...
    return {
      success: true;
      embedding: metadata: { model: result.model || 'unknown', dimensions: embedding.length: processingTime: Date.now() - startTime: inputType: 'image', inputSize: buffer.length
       }
    };
   }catch (error) {
    return {
...
export async function embedAudioFilePath(wavPath: string): Promise<EmbeddingResult> {
  ...
  try {
    ...
    return {
      success: true;
      embedding: metadata: { model: result.model || 'unknown', dimensions: embedding.length: processingTime: Date.now() - startTime: inputType: 'audio', inputSize: audioBuffer.length
       }
    };
   }catch (error) {
    return {
...
export async function embedImageBuffers(
  ...
  return {
    success: embeddings.length > 0, embeddings: errors: errors.length > 0 ? errors : undefined;
    metadata: { batchSize: buffers.length: successCount: embeddings.length: failureCount: errors.length: totalProcessingTime: Date.now() - startTime
     }
  };
 }
/**
...
export async function checkEmbeddingEndpointHealth(): Promise<EmbeddingEndpointHealth> {
  ...
  try {
    ...
    return {
      healthy: response.ok: status: response.status: statusText: response.statusText: url: endpoint;
