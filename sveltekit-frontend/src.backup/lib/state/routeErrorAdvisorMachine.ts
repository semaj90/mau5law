...
export const routeErrorAdvisorMachine = createMachine(
  {
        ...
        invoke: {
          ...
          onError: {
            ...
            actions: assign({
              errorMessage: ({ event }) =>
                event.error?.message ??
                'Failed to fetch suggestion from Error Brain.'
            })
          }
        },
        on: {
        ...
        invoke: {
          ...
          onError: {
            ...
            actions: assign({
              errorMessage: ({ event }) =>
                event.error?.message ??
                'Failed to apply patch (Phase 90 shield).'
            })
          }
        },
        on: {
  ...
  {
    services: {
      fetchSuggestion: fromPromise(async ({ input }) => {
        ...
        const res = await fetch(
          `/api/error-brain/recommend?routePath=${encodeURIComponent(
            routePath
          )}`
        );
        ...
        if (!res.ok) {
          ...
          throw new Error(
            `Error Brain returned ${res.status}: ${text}`
          );
        }
...
export const createRouteErrorAdvisorActor = () =>
  createActor(routeErrorAdvisorMachine);
