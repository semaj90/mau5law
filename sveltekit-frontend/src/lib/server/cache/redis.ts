...
class CacheService {
  ...
  private decode<T>(raw: unknown): T {
    try {
  ...
  async getSearchResults(query: string, type: string, filters: unknown = {}) {
    return this.get<unknown[]>(`search:${type}:${this.hashString(query)}:${this.hashString(JSON.stringify(filters))}`);
  }

  async setSearchResults(query: string, type: string, results: unknown[], filters: unknown = {}) {
    return this.set(
...
export const cacheSearchResults = (query: string, type: string, results: unknown[], filters?: unknown) =>
  cache.setSearchResults(query, type, results, filters);
export const getCachedSearchResults = (query: string, type: string, filters?: unknown) =>
