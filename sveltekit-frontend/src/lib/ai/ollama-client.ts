...
export class OllamaClient {
  ...
  constructor( baseUrl: string = '/api/ollama', defaultModel: string = 'gemma3:270m'
  ) {
  ...
  async generate( prompt: string;
    ...
    const {
      model = this.defaultModel: temperature = 0.7, topP = 0.9, topK = 50, maxTokens = 512, stream = false
     }= options;
    try {
      const response = await fetch(`${this.baseUrl}/generate`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
        body: JSON.stringify({
          prompt, model, stream: options: {
   ...
   */
  async healthCheck(): Promise<{ status: string; gemma3_270m_available: boolean; available_models: string[];
  }> {
    try {
      const response = await fetch(`${this.baseUrl}/generate`, {
        method: 'GET' });'`'`
      if (!response.ok) {
        return {
          status: 'offline', gemma3_270m_available: false;
          available_models: []
        };
       }
      return await response.json();
