...
<div class="bg-slate-800/50 backdrop-blur rounded-lg border border-slate-700/50">
 ...
 <span class="text-slate-300 capitalize">{item.type}</span>
 </div>
 <div class="flex justify-between">
 <span class="text-slate-400">Size:</span>
 <span class="text-slate-300">{item.size}</span>
 </div>
 <div class="flex justify-between">
 <span class="text-slate-400">Case:</span>
 <span class="text-cyan-400">{item.case}</span>
 </div>
 <div class="flex justify-between">
 ...
 <span class="px-2 py-1 bg-slate-600/50 text-slate-400 text-xs rounded">
 +{item.tags.length - 3}
 </span>
 {/if}
 </div>
 ...
 <td class="px-4 py-4 text-sm text-slate-300 capitalize">{item.type}</td>
 <td class="px-4 py-4 text-sm text-slate-300">{item.size}</td>
 <td class="px-4 py-4 text-sm text-cyan-400">{item.case}</td>
 <td class="px-4 py-4">
 <span class="px-2 py-1 text-xs rounded-full bg-slate-600/50 {getStatusColor(item.status)}">
 {item.status}
 </span>
 </td>
 ...
 <div class="flex items-center space-x-2">
 <span class={getConfidenceColor(item.confidence)}>{item.confidence}%</span>
 <div class="w-12 bg-slate-600 rounded-full h-1.5">
 <div
 class="h-1.5 rounded-full {item.confidence >= 90 ? 'bg-green-400' : item.confidence >= 70 ? 'bg-yellow-400' : 'bg-red-400'}"
 style="width: {item.confidence}%"
 ></div>
 </div>
 </div>
 {:else}
 <span class="text-slate-500">Not analyzed</span>
 {/if}
 </td>

 <td class="px-4 py-4 text-sm text-slate-400">{formatDate(item.uploaded)}</td>
