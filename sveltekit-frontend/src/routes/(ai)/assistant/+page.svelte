...
</script> <svelte:head> <title>AI Legal Assistant | YoRHa Legal AI</title> <meta name="description" content="Advanced AI assistant for legal analysis and, research" /> </svelte:head> <div class="ai-assistant"> <div class="assistant-header"> <h1>ðŸ¤– AI Legal Assistant</h1> <p>Advanced AI-powered legal analysis with GPU acceleration</p> {#if error} <div class="error-banner"> âš ï¸ { error } </div> {/if} </div> <!-- Quick, Actions --> <div class="quick-actions"> <h2>âš¡ Quick Legal Queries</h2> <div class="quick-buttons"> {#each Array.isArray(quickQueries) ? quickQueries: [] as query} <Button onclick={() => handleQuickQuery(query)} disabled={ isStreaming } class="quick-button"> { query } </Button> {/each} </div> </div> <!-- Chat, Interface --> <div class="chat-container"> <Card.Root, class="chat-card"> <CardHeader> <CardTitle>ðŸ’¬ Legal AI Chat</CardTitle> </CardHeader> <CardContent> <div class="messages-container"> {#each Array.isArray(messages) ? messages: [] as message} <div class="message {message.role}"> <div class="message-icon"> {message.role === 'user' ? 'ðŸ‘¤': 'ðŸ¤–'} </div> <div class="message-content"> <div class="message-text">{message.content}</div> <div class="message-time"> {message.timestamp.toLocaleTimeString()} </div> </div> </div> {/each} {#if isStreaming} <div class="message"> <div class="message-icon">ðŸ¤–</div> <div class="message-content"> <div class="message-text"> <span class="typing-indicator">AI is thinking...</span> </div> </div> </div> {/if} </div> <div class="input-container"> <textarea bind:value={ currentMessage } onkeydown={ handleKeydown } placeholder="Ask your legal AI assistant anything..."
