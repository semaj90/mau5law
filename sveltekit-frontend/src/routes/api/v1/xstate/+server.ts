...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      ...
      data: result;
      metadata: {
  timestamp: new Date().toISOString(), service: 'xstate-manager', operation: 'event', event_type: eventData.type
       }
    });
   }catch (err: any) {
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    return json({
      service: 'xstate', status: 'operational', endpoints: {
  event: '/api/v1/xstate (POST)', machine_status: '/api/v1/xstate?machineId={id }, actor_status: '/api/v1/xstate?actorId={id }, machines: '/api/v1/xstate/machines', actors: '/api/v1/xstate/actors` },'`
      health: {
        'xstate-manager': health['xstate-manager'] || false
      }, capabilities: [
        'Event Processing', 'State Machine Management', 'Actor Coordination', 'Workflow Orchestration', 'Real-time State Updates'], supported_events: [
        'PROMPT', 'SEMANTIC_SEARCH', 'FILE_UPLOAD', 'ACCEPT_PATCH', 'RATE_SUGGESTION', 'CHECK_HEALTH', 'TRANSITION', 'COMPLETE', 'ERROR', 'RESET'], version: '1.0.0' });'`  }catch (err: any) {'`
