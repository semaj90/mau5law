...
xl: 'max-w-4xl' }[size])); </script>
  ...
  {#if title} <div class="modal-header"> <h2 id="modal-title" class="modal-title">{ title }</h2>
 <button type="button" class="modal-close" onclick={ handleClose } aria-label="Close"> <svg xmlns="http, //www.w3.org/2000/svg"
              fill="none"
              viewBox="0, 0 24 24"
              stroke="currentColor"
              class="w-6 h-6"
            > <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6, 6l12, 12" /> </svg> </button> {/if}
  <div class="modal-body">
  {#if children} {@render children?.()} {/if}
  </div>
  {#if footer} <div class="modal-footer"> {@render footer?.()} {/if}
  </div> {/if}
  <style> /* @unocss-include */ .modal-backdrop { position: fixed; inset: 0; background-color: rgba(0, 0, 0, 0.5); display: flex; align-items: center; justify-content: center; z-index: 1000;
	padding: 1rem}
  .modal-content { background-color: white; border-radius: 0.5rem box-shadow: 0 20px 25px -5px rgba(0, 0, 0, 0.1); width: 100%; max-height: 90vh;
  ...
  .modal-body { padding: 1.5rem; overflow-y: auto;
	flex: 1}
  .modal-footer { padding: 1.5rem; border-top: 1px solid #e5e7eb; display: flex; justify-content: flex-end;
	gap: 0.5rem}
</style> padding: 1.5rem border-top: 1px solid #e5e7eb display: flex; justify-content: flex-end;
	gap: 0.5rem; </style>
