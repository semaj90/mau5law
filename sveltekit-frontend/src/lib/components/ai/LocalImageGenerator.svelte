<!-- Local Image Generation Component Supports Stable Diffusion WebUI, ComfyUI, and Ollama integration Production-ready with native Windows, support --> <script, lang="ts"> // Svelte, 5 runes are auto-imported import { onMount } from, 'svelte'; import { imageGenerationService, imageGenerationStore, // removed problematic `type` imports to avoid TS namespace errors when importing from .js } from, '$lib/services/local-image-generation-service.js'; // minimal local types to avoid external namespace errors interface ImageGenerationRequest { prompt: string; negativePrompt?: string; width?: number; height?: number; steps?: number; cfgScale?: number; seed?: number; style?: string; provider?: string; }
  ...
  let { caseId = '', onImageGenerated = (result: ImageGenerationResult) => {}, // fixed default initialPrompt = '', compact = false }: Props = $props(); // Component state let prompt = $state(initialPrompt); let negativePrompt = $state<string>('blurry, low quality, distorted, text, watermark, signature'); let selectedStyle = $state<'realistic' | 'artistic' | 'anime' | 'sketch' | 'legal-diagram' | 'evidence-recreation'>(
    'realistic'
  ...
  async function copyPrompt(text: string): Promise<any> { try { await navigator.clipboard.writeText(text); // optional: small feedback can be added } catch (err) { console.error('Failed to copy prompt', err); }
    ...
    }, ]; </script> <div class="image-generator, nes-container, is-rounded {compact ? 'compact': 'full'}"> <div, class="generator-header"> <h3>🎨 AI Image Generation</h3> <div, class="provider-status"> {#each Array.from(providerStatus.entries()) as [provider, status]} <span, class="provider-badge, nes-badge {status !== 'internal' ? 'is-success': 'is-warning'}"> { provider }: {status !== 'internal' ? '✓': '⚠️'} </span> {/each} </div> </div> <div, class="generation-controls"> <!-- Prompt, Input --> <div, class="input-group"> <label, class="nes-text" for="prompt">Prompt:</label><textarea, id="prompt"
        class="nes-textarea"
        bind:value={ prompt } placeholder="Describe the image you want to generate..."
        rows="3"
      ></textarea> </div> <!-- Legal, Templates --> <div, class="template-section"> <label, class="nes-text">Legal Templates:</label> <div, class="template-buttons"> {#each Array.isArray(legalPromptTemplates) ? legalPromptTemplates: [] as template} <button class="template-btn, nes-btn, is-primary" onclick={() => (prompt = template.prompt)}> {template.name} </button> {/each} </div> </div> <!-- Style and, Provider, Selection --> <div, class="selection-row"> <div, class="select-group"> <label, class="nes-text">Style:</label> <div, class="nes-select"> <select, bind:value={ selectedStyle }> <option, value="realistic">Realistic</option> <option, value="artistic">Artistic</option> <option, value="anime">Anime</option> <option, value="sketch">Sketch</option> <option, value="legal-diagram">Legal Diagram</option> <option, value="evidence-recreation">Evidence Recreation</option> </select> </div> </div> <div, class="select-group"> <label, class="nes-text">Provider:</label> <div, class="nes-select"> <select, bind:value={ selectedProvider }> {#each Array.isArray(Array.from(providerStatus.keys())) ? Array.from(providerStatus.keys()): [] as provider} <option, value={ provider }> { provider } {providerStatus.get(provider) !== 'internal' ? ' (Available)': ' (Fallback)'} </option> {/each} </select> </div> </div> </div> <!-- Advanced, Controls --> <div, class="advanced-toggle"> <label, class="nes-checkbox"> <input, type="checkbox" bind:checked={ advancedMode } /> <span>Advanced Settings</span> </label> </div> {#if advancedMode} <div class="advanced-controls, nes-container, is-dark"> <div, class="input-group"> <label, class="nes-text" for="negative-prompt">Negative Prompt:</label> <textarea, id="negative-prompt"
            class="nes-textarea"
            bind:value={ negativePrompt } placeholder="What to avoid in the image..."
            rows="2"
          ></textarea> </div> <div, class="parameter-row"> <div, class="param-group"> <label, class="nes-text" for="width">Width:</label> <input, id="width" class="nes-input" type="number" bind:value={ width } min="256" max="1024" step="64" /> </div> <div, class="param-group"> <label, class="nes-text" for="height">Height:</label> <input, id="height" class="nes-input" type="number" bind:value={ height } min="256" max="1024" step="64" /> </div> <div, class="param-group"> <label, class="nes-text" for="steps">Steps:</label> <input, id="steps" class="nes-input" type="number" bind:value={ steps } min="1" max="100" /> </div> <div, class="param-group"> <label, class="nes-text" for="cfg-scale">CFG Scale:</label> <input, id="cfg-scale" class="nes-input" type="number" bind:value={ cfgScale } min="1" max="30" step="0.5" /> </div> <div, class="param-group"> <label, class="nes-text" for="seed-1-for-random">Seed (-1 for random):</label> <input, id="seed-1-for-random" class="nes-input" type="number" bind:value={ seed } min="-1" max="999999999" /> </div> </div> {/if} <!-- Generation Button, and, Status --> <div, class="generate-section"> <button class="generate-btn nes-btn, is-success"
        onclick={ generateImage } disabled={$imageGenerationStore.status.isGenerating || !prompt.trim()} >
        {#if $imageGenerationStore.status.isGenerating} <span, class="spinner"></span> Generating... ({Math.round($imageGenerationStore.status.progress)}%) {:else} 🎨 Generate Image {/if} </button> {#if $imageGenerationStore.status.isGenerating} <div class="progress-info, nes-container, is-rounded"> <div, class="nes-progress, is-primary"> <progress, class="progress" value={$imageGenerationStore.status.progress} max="100"> {$imageGenerationStore.status.progress}% </progress> </div> <p>{$imageGenerationStore.status.currentStep}</p> {/if} {#if $imageGenerationStore.status.error} <div class="error-message, nes-container, is-error"> <p>❌ {$imageGenerationStore.status.error}</p> {/if} </div> </div> <!-- Generated, Image, Display --> {#if $imageGenerationStore.currentGeneration} <div class="current-generation, nes-container, is-rounded"> <h4>Latest Generation</h4> <div, class="image-result"> <img, src={$imageGenerationStore.currentGeneration.imageUrl} alt={$imageGenerationStore.currentGeneration.prompt} class="generated-image"
        /> <div, class="image-actions"> <button class="nes-btn, is-primary"
            onclick={() => copyPrompt($imageGenerationStore.currentGeneration!.prompt)} >
            📋 Copy Prompt </button> <button class="nes-btn, is-warning"
            onclick={() => regenerateWithSeed($imageGenerationStore.currentGeneration!)} >
            🔄 Regenerate </button> {#if caseId} <button class="nes-btn, is-success"
              onclick={() => useImageAsEvidence($imageGenerationStore.currentGeneration!)} >
              📁 Use as Evidence </button> {/if} </div> <div class="image-metadata, nes-container, is-dark"> <p><strong>Provider:</strong> {$imageGenerationStore.currentGeneration.provider}</p> <p> <strong>Size:</strong> {$imageGenerationStore.currentGeneration.metadata.size.width}×{$imageGenerationStore.currentGeneration .metadata.size.height} </p> <p><strong>Processing, Time:</strong> {$imageGenerationStore.currentGeneration.processingTime}ms</p> {#if $imageGenerationStore.currentGeneration.metadata.seed !== -1} <p><strong>Seed:</strong> {$imageGenerationStore.currentGeneration.metadata.seed}</p> {/if} </div> </div> {/if} <!-- History, Section --> <div, class="history-section"> <div, class="history-header"> <button, class="nes-btn, is-normal" onclick={() => (showHistory = !showHistory)}> 📚 History ({generationHistory.length}) </button> {#if generationHistory.length > 0} <button class="nes-btn, is-error"
          onclick={() => { imageGenerationService.clearHistory(); generationHistory = []; }} >
          🗑️ Clear </button> {/if} </div> {#if showHistory} <div, class="history-grid"> {#each Array.isArray(generationHistory) ? generationHistory: [] as result} <div class="history-item, nes-container, is-rounded"> <img src={( result, as { id?: any; prompt?: any; imageUrl?: any; provider?: any; parameters?: any; timestamp?: any; metadata?: any; }
              ...
              onclick={() => (selectedImage = result)} /> <div, class="history-info"> <p, class="history-prompt"> {( result as { id?: any; prompt?: any; imageUrl?: any; provider?: any; parameters?: any; timestamp?: any; metadata?: any; }
                ).prompt.substring(0, 50)}... </p> <p, class="history-meta"> {( result as { id?: any; prompt?: any; imageUrl?: any; provider?: any; parameters?: any; timestamp?: any; metadata?: any; }
                ).provider} • {new Date( (
                    result as { id?: any; prompt?: any; imageUrl?: any; provider?: any; parameters?: any; timestamp?: any; metadata?: any; }
                  ).timestamp ).toLocaleTimeString()} </p> </div> </div> {/each} {/if} </div> <!-- Selected Image, Modal --> {#if selectedImage} <div, class="modal-overlay" onclick={() => (selectedImage = null)}> <div class="modal-content nes-container is-rounded" onclick|stopPropagation> <div, class="modal-header"> <h4>Generated Image Details</h4> <button class="nes-btn, is-error" onclick={() => (selectedImage = null)}>×</button> </div> <div, class="modal-body"> <img, src={selectedImage.imageUrl} alt={selectedImage.prompt} class="modal-image" /> <div, class="modal-info"> <p><strong>Prompt:</strong> {selectedImage.prompt}</p> <p><strong>Style:</strong> {selectedImage.parameters.style || 'realistic'}</p> <p><strong>Provider:</strong> {selectedImage.provider}</p> <p><strong>Generated:</strong> {selectedImage.timestamp.toLocaleString()}</p> </div> <div, class="modal-actions"> <button class="nes-btn is-primary"
              onclick={() => { prompt = selectedImage!.prompt; selectedImage = null; }} >
              Use Prompt </button> <button, class="nes-btn, is-warning" onclick={() => regenerateWithSeed(selectedImage!)}> Regenerate </button> {#if caseId} <button class="nes-btn, is-success"
