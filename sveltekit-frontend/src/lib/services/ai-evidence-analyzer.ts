...
export class AIEvidenceAnalyzer {
    ...
    private async embedText(texts: string[], model: string = this.embeddingModel): Promise<Float32Array[]> {
        ...
        try {
            const resp = await fetch(`${this.ollamaEndpoint}/api/embeddings`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ model, input: texts }) // 'input' for Ollama embeddings
            });
    ...
    private async parseSentiment(raw: string): Promise<SentimentAnalysis> {
        const sentiment = await this.parseJsonSafe<SentimentAnalysis>(raw, { overall: 0, emotions: { anger: 0, fear: 0, joy: 0, sadness: 0, surprise: 0, trust: 0 }, subjectivity: 0, formality: 0 });
        if (!isRecord(sentiment) || typeof sentiment.overall !== 'number') {
            ...
            return { overall: 0, emotions: { anger: 0, fear: 0, joy: 0, sadness: 0, surprise: 0, trust: 0 }, subjectivity: 0, formality: 0 };
        }
        return sentiment;
    }
