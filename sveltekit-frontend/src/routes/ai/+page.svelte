    ...
    try {
      await webAssemblyAIAdapter.streamMessage(prompt, {
        ...
        onComplete: (response) => {
          ...
          adapterHealth = webAssemblyAIAdapter.getHealthStatus(); // Update health status after completion
        },
        onError: (error) => {
          streaming = false;
          output = `Error: ${error.message}`;
          console.error('Streaming error:', error);
          adapterHealth = webAssemblyAIAdapter.getHealthStatus(); // Update health status on error
        },
      });
    } catch (error: any) {
      streaming = false;
...
<div class="p-6 max-w-2xl mx-auto">
  <h2 class="text-xl font-bold mb-2">🧩 Gemma 270 M (WebGPU + CUDA Hybrid)</h2>
  ...
  <div class="mb-4 text-sm text-gray-600">
    ...
    {/if}
    {#if adapterHealth}
      | Current Model: {adapterHealth.currentModel}
      | Method: {lastResponseMetadata?.method || 'N/A'}
      | GPU: {adapterHealth.webgpuAvailable ? '✅' : '❌'}
      | CUDA Service: {adapterHealth.cudaServiceStatus?.online ? '✅' : '❌'}
    {/if}
  </div>
  ...
  <textarea
    ...
    placeholder="Ask a question or paste a contract snippet…"
  />
  <button class="btn mt-3" onclick={send} disabled={streaming || !adapterInitialized}>
    {streaming ? 'Processing…' : 'Generate'}
  </button>

  <pre class="mt-4 p-3 bg-gray-900 text-green-200 rounded">{output}</pre>

  {#if lastResponseMetadata}
    <div class="mt-4 p-3 bg-gray-800 text-gray-200 rounded text-sm">
      <h3 class="font-bold mb-1">Response Metadata:</h3>
      <p>Method: <span class="font-mono text-blue-300">{lastResponseMetadata.method}</span></p>
      <p>Model Used: <span class="font-mono text-blue-300">{lastResponseMetadata.modelUsed}</span></p>
      ...
      <p>GPU Accelerated: {lastResponseMetadata.gpuAccelerated ? '✅' : '❌'}</p>
      <p>Tensor Acceleration: {lastResponseMetadata.tensorAccelerationUsed ? '✅' : '❌'}</p>
    </div>
  {/if}
</div>
...
<style>
.btn {
  background:#2563eb;color:#fff;padding:.5rem 1rem;border-radius:.25rem;
}
</style>
