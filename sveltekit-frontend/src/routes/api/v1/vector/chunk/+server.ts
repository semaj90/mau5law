    ...
    try {
        ...
        const response: ChunkingResponse = {
            ...
            summary: {
                ...
                totalTokens,
                processingTime: useSemanticChunking ? 'semantic' : preserveParagraphs ? 'paragraph-aware' : 'basic', // Corrected chunkingMethod
                usedCUDA: useCUDA,
            },
...
async function performSemanticChunking(
    text: string,
    options: { chunkSize: number; chunkOverlap: number; minChunkSize: number; maxChunkSize: number; extractMetadata: boolean }
