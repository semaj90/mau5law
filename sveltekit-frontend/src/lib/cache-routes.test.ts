...
import { setupTest, cleanupTest } from '$lib/test-utils/setup'; function makeReq(body, any: headers, Record: Record<string,string> = {}) { return { request: { json, async () => body, // minimal headers shim for Node/Vitest: supports headers.get(name),headers: { get: (name: string) => { // header names are case-insensitive return headers[name.toLowerCase()] ? ? null} } }as any} describe('cache route handlers (direct imports)', () => {
  beforeEach(async () => {
    await setupTest();
  });
