...
type ParseResult<T> = { data: T; backend: 'http'|'wasm'|'wasi'|'fallback'; parseTimeMs: number };

const DEFAULT_HTTP_ENDPOINT = typeof window !== 'undefined' ? (import.meta.env.VITE_SIMD_ACCELERATOR_URL || 'https://localhost:8095/json') : 'http://localhost:8095/json';
...
async function parseViaHttp<T = unknown>(payload: string, endpoint = DEFAULT_HTTP_ENDPOINT): Promise<ParseResult<T>> {
    const start = performance.now();
...
export async function detectBestMode(): Promise<'http'|'wasm'|'wasi'|'fallback'> {
    ...
    if (DEFAULT_HTTP_ENDPOINT.includes('localhost') || DEFAULT_HTTP_ENDPOINT.includes('127.0.0.1')) return 'http';
