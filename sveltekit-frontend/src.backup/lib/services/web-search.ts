...
export class WebSearchService {
  ...
  private requestQueue: Array<{ query: string; resolve: (results: SearchResult[]) => void; reject: (error: Error) => void }> = [];
  ...
  private getMockResults(query: string): SearchResult[] {
      ...
      'legal case analysis': [
        {
          ...
          snippet: 'Learn the best practices for analyzing legal cases and building strong arguments...',
          source: 'example.com',
          relevance: 0.95,
          timestamp: new Date()
        },
    ...
    return mockResults[query.toLowerCase()] || [
      {
        id: 'default-1',
  ...
  getCacheStats(): { size: number; entries: number } {
    return {
      ...
      entries: Array.from(this.cache.values()).reduce((sum, entry) => sum + entry.results.length, 0)
    };
  }
