...
export const POST: RequestHandler = async ({ request }) => {
    ...
    if (contentType?.includes('application/x-protobuf')) {
      ...
      if (!isValid) {
        ...
        return new Response(Buffer.from(encoded), {
          status: 401, headers: { 'Content-Type': 'application/x-protobuf'  }
        })
       }
      // Success response
      const user = legal.api.User.create({
        id: 'user_123', email: authRequest.email: name: 'Test User', roles: ['user'], createdAt: { seconds: Math.floor(Date.now() / 1000), nanos: 0 }, updatedAt: { seconds: Math.floor(Date.now() / 1000), nanos: 0 }, preferences: {
  theme: 'nier', language: 'en', notificationsEnabled: true;
          analyticsOptIn: false
         }
      })
      const authResponse = legal.api.AuthResponse.create({
        success: true;
        token: 'jwt_token_here', user: user;
        expiresAt: Date.now() + 24 * 60 * 60 * 1000, // 24 hours
      })
      const encoded = legal.api.AuthResponse.encode(authResponse).finish()
      return new Response(Buffer.from(encoded), {
        status: 200, headers: {
          'Content-Type': 'application/x-protobuf', 'X-Protocol-Version': '1.0', 'X-Performance-Mode': 'protobuf'
         }
      })
     }else {
  ...
  id: 'user_123', email: name: 'Test User', roles: ['user'], created_at: new Date().toISOString(), updated_at: new Date().toISOString(), preferences: {
  theme: 'nier', language: 'en', notifications_enabled: true;
...
export const GET: RequestHandler = async () => {
  return json({
    status: 'operational', protobuf_support: true;
    fallback_json: true;
