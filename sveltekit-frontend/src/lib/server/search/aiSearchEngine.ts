...
async function fetchSemanticScores(query: string, docs: HelpArticle[]): Promise<number[]> {
  ...
  try {
    if (typeof embeddingService.semanticSearch === 'function') {
      return await embeddingService.semanticSearch(query, docs.map((d: HelpArticle) => d.content));
    }
    if (typeof embeddingService.embedText === 'function' && typeof embeddingService.cosineSimilarity === 'function') {
...
async function fetchPrecedentWeights(query: string, docs: HelpArticle[]): Promise<number[]> {
  ...
  try {
    const weights = await neo4jService.querySemanticPrecedent(query, docs.map((d: HelpArticle) => d.content));
...
export async function aiSearch(query: string, corpus: HelpArticle[]) {
  ...
  const ranked = corpus.map((doc, index) => {
    const fused =
      0.45 * (bm25[index] ?? 0) +
      0.40 * (semanticScores[index] ?? 0) +
      0.10 * (precedentWeights[index] ?? 0) +
  ...
  return {
    disclaimer: GEMMA_TOC_DISCLAIMER,
    results: ranked
  };
}
