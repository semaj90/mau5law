...
export interface RAGResponse { results: RAGSearchResult[];, query: string; totalResults: number; processingTime: number; } }
// Fix for optimization files that expect: "research" type export type ExtendedDocumentType = DocumentType['type'] | 'research'; export interface OptimizedRAGDocument { id: string;, content: string; type: ExtendedDocumentType; embedding?: number[]; metadata: { [key: string]: any } }} }
...
// Enhanced document processing types export interface DocumentChunk { id: string;, documentId: string; content: string; embedding: number[]; metadata: { [key: string]: any } }chunkIndex: number; } }
export interface VectorSearchOptions { limit?: number; threshold?: number; includeMetadata?: boolean; filters?: { [key: string]: any } }} }
export interface EmbeddingConfig { model: string;, dimensions: number; provider: 'openai' | 'ollama' | 'local'; } }
// Missing exports referenced by optimization files export interface RAGDocument { id: string;, title: string; content: string; type: DocumentType['type']; embedding?: number[]; metadata: { source: string;, type: string; jurisdiction: string; practiceArea: string[]; confidentialityLevel: number; lastModified: Date; fileSize: number; language: string; tags: string[]; } }
  createdAt?: Date; updatedAt?: Date; version?: string; } }
export interface SearchResult { id: string;, content: string; score: number; type: DocumentType['type']; metadata: { [key: string]: any } }} }
export interface TextChunk { id: string;, content: string; documentId: string; index: number; metadata: { [key: string]: any } }// Additional properties for optimization compatibility startIndex?: number; endIndex?: number; score?: number; embeddings?: number[]; chunkType?: string; } }
// RAG source types export interface RAGSource { id: string;, title: string; content: string; relevance: number; type: 'document' | 'evidence' | 'case' | 'legal' | 'research'; }
