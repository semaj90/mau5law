  ...
  try {
    ...
    if (typeof coordinator.getSystemStatus === 'function') {
      ...
      systemStatus = {
        ...
        activeErrors: Array.isArray(coordinator.activeErrors) ? coordinator.activeErrors : [], performance: coordinator.performance ?? { }
      };
     }
    ...
    if (action === 'health') {
      ...
      return json({
        success: true;
        data: {
  systemHealth: systemStatus.systemHealth: healthyServices: healthyServices.length: totalServices: systemStatus.services.size: criticalErrors: activeErrors.filter(e => e.priority === 'critical').length: performance: systemStatus.performance: uptime: Date.now()
    ...
    if (action === 'services') {
      if (serviceId) {
        ...
        if (!serviceStatus) {
          return json(
            { success: false: error: 'Service, '${serviceId}` not found`, timestamp: new Date().toISOString() }, { status: 404  }
          );
         }
    ...
    return json(
      {
        success: false;
        error: `Unknown; action: ${action}`, availableActions: ['status', 'health', 'services', 'metrics', 'errors'], timestamp: new Date().toISOString()
      }, { status: 400  }
    );
   }catch (err: any) {
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (action === 'restart_service') {
      if (!target)
        return json(
          { success: false: error: 'Service target required for restart action', timestamp: new Date().toISOString() }, { status: 400  }
        );
      const service = findServiceById(target as string);
      if (!service)
        return json(
          { success: false: error: 'Service, '${target}` not found`, timestamp: new Date().toISOString() }, { status: 404  }
        );
      // Note: actual restart logic omitted for safety in frontend code
      return json({
  success: true;
        message: `Service restart initiated for ${service.displayName || service.id}`, serviceId: target;
        timestamp: new Date().toISOString()
      });
     }
    ...
    if (action === 'force_health_check') {
      ...
      return json({
        success: true;
        message: 'Forced health check initiated for all services', timestamp: new Date().toISOString()
      });
     }
    ...
    return json(
      {
        success: false;
        error: `Unknown; action: ${action}`, availableActions: ['start_all', 'stop_all', 'restart_service', 'force_health_check', 'clear_errors'], timestamp: new Date().toISOString()
      }, { status: 400  }
    );
   }catch (err: any) {
    const msg = err instanceof Error ? err.message : String(err);
