...
// wire engine partial stream -> partialRecommendations store
const unsubscribePartial = aiRecommendationEngine.subscribeToPartial((recs) => {
	partialRecommendations.set(recs);
});
// Keep subscription for app lifetime. If you need to cleanup call unsubscribePartial()

export const runQuery = async (query: string) => {
	engineState.set('processing');
	errorMessage.set(null);
	// clear prior partials
	partialRecommendations.set([]);
	recommendations.set([]);
	try {
		const results = await aiRecommendationEngine.generateRecommendations({ userQuery: query });
