...
const FALLBACK_PATHS: Record<OllamaEndpoint, string> = {
  generate: '/api/generate', chat: '/api/chat', embeddings: '/api/embeddings', models: '/api/tags', health: '/api/version', pull: '/api/pull', version: '/api/version'
};
...
const BASE_URL = normalizeBaseUrl(
  getEnv('OLLAMA_BASE_URL') || getEnv('OLLAMA_URL'), 'http://localhost:11434'
);
...
export const MODELS: Record<string, ModelConfig> = {
  'gemma3-legal:latest': { name: 'gemma3-legal:latest', type: 'local', capabilities: ['text-generation', 'embeddings', 'legal-analysis'], contextWindow: 8192, embeddingDimension: 768, temperature: 0.7, topP: 0.9, topK: 40, systemPrompt: `You are a sophisticated legal AI assistant powered by Gemma3, specialized in legal document analysis, contract review, and case law research.`
    ...
    options: { num_gpu: 1, // Use GPU acceleration
      ...
      repeat_penalty: 1.1, seed: 42, stop: ['User:', 'Human:', '\n\n\n']
     }
  }, 'nomic-embed-text': {
    name: 'nomic-embed-text', type: 'embedding', capabilities: ['embeddings'], embeddingDimension: 768, contextWindow: 8192
  }, embeddinggemma: { name: 'embeddinggemma', type: 'embedding', capabilities: ['embeddings'], embeddingDimension: 768, contextWindow: 8192, temperature: 0.0, // Deterministic embeddings
    systemPrompt:
...
export const OLLAMA_CONFIG: OllamaConfig = { baseUrl: BASE_URL;
  endpoints: OLLAMA_ENDPOINTS;
  defaultModel: 'gemma3-legal:latest', embeddingModel: 'embeddinggemma', fallbackModel: 'gemma3-legal:latest', fallbackModels: { legal: 'gemma3-legal:latest', general: `gemma3-legal:latest' },'`
  timeout: 60000, // 60 seconds for complex legal analysis
  maxRetries: 3, streamEnabled: true;
  // GPU acceleration settings
  gpu: { enabled: true;
    layers: 35, // Number of layers to offload to GPU
    mainGpu: 0, tensorSplit: null
  }, // Performance optimization
  performance: { batchSize: 32, parallelRequests: 4, cacheEnabled: true;
...
export function getOptimalModel(_task: 'embedding' | 'generation' | 'legal-analysis'): string[] {
  const taskMap = {
    'embedding': FALLBACK_CHAIN['embeddings'], 'generation': FALLBACK_CHAIN['text-generation'], 'legal-analysis': FALLBACK_CHAIN['legal-analysis']
   }
...
export function isLegalTask(prompt: string): boolean {
  const legalKeywords = [
    'contract', 'agreement', 'legal', 'law', 'court', 'case', 'statute', 'regulation', 'compliance', 'liability', 'clause', 'jurisdiction', 'plaintiff', 'defendant', 'litigation', 'intellectual property', 'patent', 'trademark', 'copyright', 'tort', 'negligence', 'breach', 'damages', 'remedy', 'arbitration', 'mediation', 'settlement', 'precedent', 'deed', 'title', 'evidence', 'testimony', 'witness', 'prosecutor', 'defense', 'attorney', 'counsel', 'judge'
  ];
