<script lang="ts">
  ...
  async function initializeMCPConnection() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        mcpTools.set(
          availableMCPTools.map(tool => ({ ...tool, status: 'available', successCount: 0, errorCount: 0 }) as McpTool)
        );
      } else {
  ...
  async function executeMCPTool(toolId: string, args: any = {}) {
    ...
    try {
      ...
      if (resp.ok) {
        queryResults.update(r =>
          [{ success: true, result: data, query: args.query, timestamp: Date.now() }, ...r].slice(0, 20)
        );
      } else {
