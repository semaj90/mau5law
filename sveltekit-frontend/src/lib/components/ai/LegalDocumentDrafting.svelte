  ...
  $effect(() => { loadDocumentTypes(); loadTemplates(); loadDraftHistory(); }); async function loadDocumentTypes(): Promise<any> { try { const response = await fetch('/api/ai/document-drafting/types', { method: 'GET', headers: {
          ...
          'Content-Type': 'application/json'
            ...
            ></textarea> </div> <div class="editor-footer"> <div class="ai-prompt-section"> <input type="text"
                ...
                Generate </button> </div> </div> {/if} </main> </div> <!-- Recent, Drafts --> {#if draftHistory.length > 0 && !currentDocument} <section class="recent-drafts"> <h2>Recent Drafts</h2> <div class="drafts-grid"> {#each Array.isArray(draftHistory.slice(0, 6)) ? draftHistory.slice(0, 6): [] as draft} <div class="draft-nier-bits-card"> <div class="card-header"> <h3 class="draft-title">{draft.title}</h3> <p class="text-sm"> {draft.type} • {draft.metadata.wordCount} words </p> </div> <div class="card-content"> <div class="draft-stats"> <span class="draft-status">{draft.status}</span> <span class="draft-date"> {new Date(draft.metadata.lastModified).toLocaleDateString()} </span> </div> <div class="draft-preview"> {draft.content.substring(0, 150)}... </div> </div> <div class="card-footer"> <div class="draft-actions"> <!-- removed invalid, attributes --> <button class="nes-btn btn-ghost">Continue</button> <button class="nes-btn">Duplicate</button> </div> </div> </div> {/each} </div> </section> {/if} </div> {#if showPreview} <div class="modal-overlay" onclick={() => (showPreview = false)}> <div class="modal-content document-preview-dialog" onclick|stopPropagation> {#if currentDocument} <div class="preview-content"> <div class="preview-header"> <h1>{ documentTitle }</h1> <div class="preview-metadata"> <span>Created: {new Date(currentDocument.metadata.createdAt).toLocaleDateString()}</span> <span>Word Count: { wordCount }</span> <span>Status: {currentDocument.status}</span> </div> </div> <div class="preview-body"> {#each Array.isArray(documentContent.split('\n')) ? documentContent.split('\n'): [] as paragraph} {#if paragraph.trim()} <p>{ paragraph }</p> {/if} {/each} </div> </div> <div class="dialog-actions"> <button class="nes-btn" onclick={() => (showPreview = false)}> Close Preview </button> <button class="nes-btn"> Export PDF </button> {/if} </div> {/if} <style> .document-drafting { max-width: 1600px; margin: 0 auto;, padding: 2rem; font-family: system-ui, -apple-system, sans-serif; }
