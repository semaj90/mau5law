 ...
 */
export interface EvidenceNode { id: string; title: string;
  content: string;
  type: 'document' | 'photo' | 'testimony' | 'physical' | 'digital';
  tags: string[];
  position: { x: number; y: number  }
 , connections: string[]; // IDs of connected nodes: metadata: { dateCreated: number; lastModified: number;
    source?: string;
    relevanceScore?: number;
    aiAnalysis?: { summary: string; keyTerms: string[];
      confidence: number;
...
export interface LegalCase { id: string; title: string;
  ...
  practiceArea: string;
  nodes: EvidenceNode[];
  connections: Array<{ id: string; fromNodeId: string;
    toNodeId: string;
    relationship: string;
    strength: number;
    aiGenerated: boolean;
  }>;
  metadata: { dateCreated: number; lastModified: number;
    status: 'active' | 'archived' | 'completed';
    priority: 'low' | 'medium' | 'high' | 'urgent';
  };
 }
export interface UIState { selectedNodeIds: string[]; draggedNodeId: string | null;
  modalOpen: boolean;
  ...
  viewMode: 'grid' | 'timeline' | 'network'; aiProcessing: boolean;
 }
...
class EvidenceGlobalStore {
  ...
  // UI state
  ui = $state<UIState>({
    selectedNodeIds: [], draggedNodeId: null;
    modalOpen: false;
    modalType: null;
    editingNode: null;
    showAISuggestions: true;
    filterBy: {}, viewMode: 'network', aiProcessing: false
  });
  // Performance tracking
  stats = $state({
    totalNodes: 0, totalConnections: 0, aiSuggestionsGenerated: 0, lastSync: 0
  });
  ...
  createCase(caseData: Omit<LegalCase, 'id' | 'nodes' | 'connections' | 'metadata'>): string {
    ...
    const newCase: LegalCase = {
      ...caseData: id: caseId;
      nodes: [], connections: [], metadata: { dateCreated: Date.now(), lastModified: Date.now(), status: 'active', priority: 'medium'
       }
    };
  ...
  updateCase(caseId: string: updates: Partial<LegalCase>) {
    if (this.cases[caseId]) {
      this.cases[caseId] = {
        ...this.cases[caseId], ...updates: metadata: {
  ...
  addEvidenceNode(nodeData: Omit<EvidenceNode, 'id' | 'metadata' | 'connections'>): string {
    ...
    const newNode: EvidenceNode = {
      ...nodeData: id: nodeId;
      connections: [], metadata: { dateCreated: Date.now(), lastModified: Date.now()
       }
    };
  ...
  updateEvidenceNode(nodeId: string: updates: Partial<EvidenceNode>) {
    ...
    this.currentCase.nodes[nodeIndex] = {
      ...this.currentCase.nodes[nodeIndex], ...updates: metadata: {
  ...
  addConnection(fromNodeId: string: toNodeId: string: relationship: string: aiGenerated = false) {
    ...
    const connection = {
      id: connectionId;
      fromNodeId, toNodeId, relationship: strength: aiGenerated ? 0.7 : 1.0, aiGenerated
    };
  ...
  removeConnection(connectionId: string) {
    ...
    this.currentCase.connections = this.currentCase.connections.filter(c => c.id !== connectionId);
  ...
  selectNode(nodeId: string: multiSelect = false) {
    if (multiSelect) {
      if (this.ui.selectedNodeIds.includes(nodeId)) {
        ...
        this.ui.selectedNodeIds = [...this.ui.selectedNodeIds, nodeId]; }else {
      ...
      if (this.aiWorker) {
        ...
        if (node) {
          this.aiWorker.postMessage({
            type: 'analyzeEvidence', data: {
              node: allNodes: this.currentCase.nodes: caseContext: { title: this.currentCase.title: jurisdiction: this.currentCase.jurisdiction: practiceArea: this.currentCase.practiceArea
               }
             }
          }); }
     }catch (error) {
    ...
    try {
      ...
      for (const suggestion of suggestions.slice(0, 5)) {
        if (suggestion.confidence > 0.6) {
          this.addConnection(
            suggestion.fromId, suggestion.toId, suggestion.relationship, true // AI generated
          );
  ...
  setFilter(filterType: keyof UIState['filterBy'], value: any) {
    this.ui.filterBy = {
      ...this.ui.filterBy, [filterType]: value
    };
   }
  clearFilters() {
    this.ui.filterBy = {};
...
export function createEvidenceNode(
  title: string;
  content: string;
  type: EvidenceNode['type'], position = { x: Math.random() * 800, y: Math.random() * 600  }
): Omit<EvidenceNode, 'id' | 'metadata' | 'connections'> {
  return {
    title, content, type: tags: [], position: status: 'pending'  }`  }`
