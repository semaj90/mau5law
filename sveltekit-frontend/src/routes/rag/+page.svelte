...
<main class="rag-page">
	<header>
		<h1>RAG — Documents</h1>
		<nav class="tabs">
			<button onclick={() => (activeTab = 'upload')} aria-pressed={activeTab === 'upload'}>Upload</button>
			<button onclick={() => (activeTab = 'documents')} aria-pressed={activeTab === 'documents'}>Documents</button>
			<button onclick={() => (activeTab = 'search')} aria-pressed={activeTab === 'search'}>Search</button>
		</nav>
	</header>
				...
				<ul class="doc-list">
					{#each documents as doc (doc.id)}
						<li>
							...
							<div class="actions">
								<button onclick={() => window.open(`/api/rag/documents/${doc.id}/download`)}><FileText /></button>
								<button onclick={() => deleteDocument(doc.id)} disabled={deletingId === doc.id}>Delete</button>
							</div>
						</li>
					{/each}
				</ul>
	...
	{:else}
		<section class="panel">
			...
			<div style="margin-top:0.75rem;">
				<button onclick={searchDocuments} disabled={searching}>{searching ? 'Searching…' : 'Search'}</button>
			</div>
