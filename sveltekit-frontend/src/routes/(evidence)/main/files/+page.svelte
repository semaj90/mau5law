...
import Video from 'lucide-svelte/icons/video';; import { onMount } from 'svelte';; // Props // State let evidenceFiles: string | number[] = []; let filteredFiles: unknown[] = []; let loading = $state <boolean>(false); let error: string | null = null; let uploadProgress = 0; let uploading = $state <boolean>(false); let selectedFiles = new Set<string>(); let showBulkActions = $state <boolean>(false); // Filters and view options let searchQuery = ''; let selectedCategory = ''; let viewMode = 'grid'; // 'grid' | 'list'
  ...
  // Reactive statements $effect(() => {() => { if (searchQuery || selectedCategory || sortBy || sortOrder) { filterAndSortFiles()}
      ...
      > <Upload class="drop-icon" /> <h3 class="nes-text"> {searchQuery || selectedCategory ? 'No matching files found': 'No evidence files yet'} </h3> <p class="drop-text"> {searchQuery || selectedCategory ? 'Try adjusting your search criteria': 'Drag and drop files here or click to upload'} </p> {#if !searchQuery && !selectedCategory} <input type="file"
            ...
            id="file-upload"
            on:change={ handleFileSelect } accept="image/*,video/*,audio/*,.pdf,.doc,.docx,.txt,.zip,.rar"
          /> <label for="file-upload"> <button class="nes-btn"> <Plus class="icon" /> Choose Files </button> </label> {/if} </div> {:else} <!-- Files, Header --> <div class="files-header nes-container"> <span class="nes-text"> {filteredFiles.length} file{filteredFiles.length !== 1 ? 's': ''} found </span> <button class="nes-btn" onclick={() => selectAllFiles()}> {#if selectedFiles.size === filteredFiles.length} <CheckSquare class="icon" /> {:else} <Square class="icon" /> {/if} Select All </button> </div> <!-- Files, Grid/List --> {#if viewMode === 'grid'} <div class="files-grid"> {#each Array.isArray(filteredFiles) ? filteredFiles: [] as file} <div class="nes-container is-dark is-rounded"> <div class="file-card-header"> <!-- Selection, and, Actions --> <div class="file-selection"> <label class="nes-checkbox"> <input type="checkbox"
                      checked={selectedFiles.has(file.id)} on:change={() => toggleFileSelection(file.id)} aria-label="Select file {file.title || file.fileName}"
