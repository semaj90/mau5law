...
type SerializableCacheEntry = Omit<RedisGPUCacheEntry, 'quantized' | 'gpuTexturePayload' | 'embeddings' | 'metadata'> & {
    quantized: {
...
class RedisGPUChatOptimizationService {
  ...
  public async setConfig(partial: ServiceConfig): Promise<void> {
    ...
    this.config = {
      ...this.config,
      ...partial,
      redisUrl: partial.redisUrl ?? this.config.redisUrl,
      ollamaApiBase: partial.ollamaApiBase ?? this.config.ollamaApiBase,
  ...
  public async cacheResponse(
    ...
    const newEntry: RedisGPUCacheEntry = {
      ...
      gpuTexturePayload: await this.prepareGPUTexturePayload(response),
      metadata: {
        userId,
        sessionId,
  ...
  private async prepareGPUTexturePayload(response: string): Promise<RedisGPUCacheEntry['gpuTexturePayload']> {
    return {
        chrROMPattern: `chr_rom_${this.simpleHash(response)}`,
