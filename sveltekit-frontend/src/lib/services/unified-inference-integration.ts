...
export class UnifiedInferenceEngine {
  ...
  private async runWebAsmInference(request: UnifiedInferenceRequest): Promise<InferenceResult> {
    ...
    const output =
      rawOutput instanceof Float32Array ? rawOutput : new Float32Array(Array.isArray(rawOutput) ? rawOutput : []);
      ...
      if (redisResult?.data) {
        ...
        try {
          ...
          const computedShape: number[] =
            redisResult.shape ??
            (redisData instanceof Float32Array
              ? [redisData.length]
              : Array.isArray(redisData)
                ? [redisData.length]
                : [0]);
