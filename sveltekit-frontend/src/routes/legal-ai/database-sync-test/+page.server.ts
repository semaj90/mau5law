...
export const load: PageServerLoad = async ({ url: _url, fetch: _fetch }): Promise<DatabaseSyncTestData> => {
  ...
  try {
    ...
    const metrics = processingMetrics.status === 'fulfilled' ? processingMetrics.value : [{ avgProcessingTime: 0, cacheHits: 0, totalProcessed: 0 }];

    // Build a normalized sessionsWithCounts: object (used below)
    const sessionsWithCounts = Array.isArray(sessions) ? sessions.map((s: any) => ({
      id: s.id,
      sessionName: s.sessionName ?? s.session_name ?? 'untitled',
      messageCount: parseNumericField((s as Record<string, unknown>).messageCount ?? (s as Record<string, unknown>).message_count),
    ...
    const metricsData = (Array.isArray(metrics) && metrics[0]) || { avgProcessingTime: 0, cacheHits: 0, totalProcessed: 0 };
