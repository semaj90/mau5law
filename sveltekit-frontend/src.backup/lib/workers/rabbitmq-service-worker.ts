...
type RabbitMQServiceLike = {
  ...
  consume?: (queue: string, cb: (message: any, originalMessage?: unknown) => Promise<void> | void) => Promise<void> | void;
  subscribe?: (queue: string, cb: (message: unknown, originalMessage?: unknown) => Promise<void> | void) => Promise<void> | void;
  createConsumer?: (queue: string, cb: (message: unknown, originalMessage?: unknown) => Promise<void> | void) => Promise<void> | void;
...
export class RabbitMQServiceWorker {
  ...
  private processingStats = { messagesProcessed: 0, errors: 0, startTime: Date.now(), avgProcessingTime: 0 };
    ...
    const callback = async (message: any, originalMessage?: unknown) => {
      ...
      try {
        ...
        await Promise.race([
          ...
          new Promise<never>((_, reject) => setTimeout(() => reject(new Error('Processing timeout')), this.config.processingTimeout))
        ]);
  ...
  private setupDefaultHandlers(): void {
    ...
    const getField = (m: Record<string, unknown> | undefined, key: string): unknown => m && typeof m === 'object' ? (m as Record<string, unknown>)[key] : undefined;
    ...
    this.registerHandler(QUEUE_NAMES.DOCUMENT_PROCESSING, async (message: any) => {
      ...
      await publishToQueue(QUEUE_NAMES.VECTOR_EMBEDDING, { ...msg, stage: 'embedding_ready', processedAt: Date.now() });
    });
    // File upload handler
    this.registerHandler(QUEUE_NAMES.FILE_UPLOAD, async (message: any) => {
      ...
      if (evidenceId) {
        await publishToQueue(QUEUE_NAMES.EVIDENCE_ANALYSIS, { evidenceId, fileName: getString(msg, 'fileName'), stage: 'analysis_ready', cudaAccelerated: getBoolean(msg, 'cudaAccelerated'), priority });
      }
    });
    ...
    this.registerHandler(QUEUE_NAMES.VECTOR_EMBEDDING, async (message: any) => {
      ...
      await publishToQueue(QUEUE_NAMES.SEARCH_INDEXING, { ...msg, embeddings: 'generated', stage: 'indexing_ready' });
    });
    ...
    this.registerHandler(QUEUE_NAMES.EVIDENCE_ANALYSIS, async (message: any) => {
      ...
      await publishToQueue(QUEUE_NAMES.CASE_UPDATES, { caseId: getString(msg, 'caseId'), evidenceId: getString(msg, 'evidenceId'), analysisComplete: true, insights: { confidence: 0.85, keyEntities: ['contract', 'signature', 'date'], summary: 'Legal document analysis completed' } });
    });
  ...
  getStats(): typeof this.processingStats & { uptime: number; isRunning: boolean } {
    return { ...this.processingStats, uptime: Date.now() - this.processingStats.startTime, isRunning: this.isRunning };
  }

  async healthCheck(): Promise<{ status: 'healthy' | 'unhealthy'; stats: (typeof RabbitMQServiceWorker.prototype)['processingStats'] & { uptime: number; isRunning: boolean }; rabbitmq: RabbitMQHealth }> {
    ...
    const inferredStatus = partial && typeof partial === 'object' && 'status' in partial ? (String((partial as Record<string, unknown>)['status']) as RabbitMQHealth['status']) : partial && typeof partial === 'object' && partial['ok'] === true ? 'healthy' : 'unhealthy';
    const rabbitmqHealth: RabbitMQHealth = { status: inferredStatus ?? 'unhealthy', details: partial && typeof partial === 'object' ? { ...partial } : undefined };
    const stats = this.getStats();
    return { status: this.isRunning && rabbitmqHealth?.status === 'healthy' ? 'healthy' : 'unhealthy', stats, rabbitmq: rabbitmqHealth };
  }

  async publishMessage(queueName: string, message: Record<string, unknown>): Promise<boolean> {
    try {
      const publishResult = await rabbitmqService.publish('workers', queueName, { ...message, publishedAt: Date.now(), workerVersion: '1.0.0' });
