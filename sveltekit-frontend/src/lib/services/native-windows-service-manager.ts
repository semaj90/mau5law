...
export class NativeWindowsServiceManager {
  ...
  async startAllServices(): Promise<any> {
    ...
    console.log(`🎉 Service complete: ${results.started.length} started, ${results.failed.length} failed`);
    return results;
  }
  ...
  async processErrorsWithGPU(): Promise<void> {
    ...
    try {
      ...
      console.log(` - ID: ${(result as { batchId?: any; fixes?: any; performance?: any }).batchId}`);
      console.log(` - Fixes generated: ${(result as { batchId?: any; fixes?: any; performance?: any }).fixes.length}`);
      console.log(` - Processing time: ${(result as { batchId?: any; fixes?: any; performance?: any }).performance.processing_time_ms.toFixed(2)}ms`);
      console.log(` - GPU utilization: ${(result as { batchId?: any; fixes?: any; performance?: any }).performance.gpu_utilization}%`);
      console.log(` - Tokens/second: ${(result as { batchId?: any; fixes?: any; performance?: any }).performance.tokens_per_second.toFixed(1)}`);
