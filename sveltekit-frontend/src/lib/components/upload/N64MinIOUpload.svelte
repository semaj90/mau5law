<script lang="ts"> import UploadManager from './upload-core'; // Migrated to $effect let { multiple = false, accept = '.pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,.tiff', maxConcurrency = 3, maxRetries = 3 }: { multiple?: boolean; accept?: string; maxConcurrency?: number; maxRetries?: number} = $props(); let fileInput: HTMLInputElement | null = null; const manager = new UploadManager({ maxConcurrency: maxRetries }); function handleFileSelect(e: Event) { const input = e.currentTarget as HTMLInputElement | null; if (!input?.files?.length) return; const files = Array.from(input.files); manager.addFiles(files); manager.start()}
 ...
 // no-op: placeholder for future session restore 
	...
	style: 'danger'
          }] ); const formData = new FormData(); formData.append('file', file); formData.append('uploadData', JSON.stringify({ caseId, title: file.name, description `N64-style upload: ${file.name}`, evidenceType: getEvidenceType(file);
	enableAiAnalysis: true enableEmbeddings: true;
  ...
  let embeddingVector: number[] = []; let embeddingDims = 0; let embeddingModel = ''; try { const embedding = await embeddingService.generateEmbedding(textContent, { preferRagService: false }); embeddingVector = embedding.vector; embeddingDims = embedding.dimension; embeddingModel = embedding.model; telemetry.emit('embedding_complete', { file: file.name, model: embedding.model, dims: embedding.dimensions;
	...
	error: e instanceof Error ? e.message: 'unknown'
          ...
          try { await vectorService.updateFileMapping(data[0].id, { textChunks: [textContent], embeddings: [embeddingVector], ocrText: file.type.startsWith('image/') ? 'OCR extracted text': undefined, analysisResults: {
	fileType: file.type, size: file.size, embeddingDims, embeddingModel } })} catch (error) { console.warn('Vector storage failed:', error)}
        } catch (outerEmbeddingErr) { console.warn('Embedding/vector pipeline error:', outerEmbeddingErr)}
'
        // Publish Redis event fetch('/api/v1/redis/publish', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify(toISOString)();
	gpuTaskIds: fs.gpuTaskId}
	...
	message: 'ðŸŽ® Upload canceled by user'
  ...
  /> <!-- Evolution: Loader, Overlay --> {#if showEvolutionLoader} <div class="evolution-overlay"> <N64EvolutionLoader stage={ evolutionStage } autoEvolution={ false } ragIntegration={ enableGPUProcessing } yorhaMode={ retro } /> {/if} <!-- N64-style, drop, zone --> <div class="n64-drop-zone"
    ...
    > {#if uploading} <N64LoadingRing size="sm" theme={n64Themes[evolutionStage].theme} speed="fast" /> ðŸŽ® UPLOADING... {:else} <Upload class="w-4" /> ðŸŽ® UPLOAD TO MINIO [{evolutionStage.toUpperCase()}] {/if} </button> {#if uploading} <button type="button"
      ...
      > ðŸ—‘ï¸ CLEAR FILES </button> {/if} </div> {#if minioHealthy === false} <div class="n64-error-alert" role="alert"> <AlertCircle class="w-4" /> âš ï¸ MINIO HEALTH CHECK FAILED â€“ UPLOADS MAY NOT PERSIST {/if} <div class="sr-only" aria-live="polite">{ liveMessage }</div> </div> <style> .n64-upload-container { width: 100%; max-width: 700px;
	margin: 0 auto; font-family: 'Courier New', monospace; position: relative}
  .retro { image-rendering: pixelated; image-rendering: -moz-crisp-edge; image-rendering: crisp-edge}
  .evolution-overlay { position: fixed;
	top: 0;left: 0;
	right: 0;bottom: 0;
  ...
  .n64-drop-zone.has-files { border-color: #40FF40;
	background: linear-gradient(135deg, #1a2e1a 0%, #163e16 100%)}
  .n64-drop-zone.uploading { cursor: not-allowed;
  ...
  .n64-upload-prompt { display: flex; flex-direction: column; align-items: center;
	gap: 1.5rem}
  .n64-upload-icon { animation: float 3s ease-in-out infinite}
  @keyframes float { 0%; } 100% { transform: translateY(0px) } 50% { transform: translateY(-10px) } }
  .n64-upload-text h3 { margin: 0; font-size: 1.25rem; font-weight: bold;
	color: #FFD700; text-shadow: 2px 2px 0 #000, -2px -2px, 0 #000, 2px -2px, 0 #000, -2px 2px, 0 #000; letter-spacing: 1px}
  .n64-subtext { color: #CCCCCC; font-size: 0.9rem;
	margin: 0.5rem, 0 0 0; text-shadow: 1px 1px 0 #000}
  .n64-file-list { width: 100%, display: flex; flex-direction: column;
	gap: 1rem}
  ...
  .n64-file-item::before { content: '';
	position: absolute; top: 4px;
	left: 4px; right: 4px;
	bottom: 4px;border: 1px solid rgba(255, 215, 0, 0.3); pointer-events: none}
  .n64-file-.status-uploading { border-color: #4090FF;
	background: linear-gradient(135deg, #1a1a2e 0%, #0a0a1a 100%); animation: processingGlow 1.5s ease-in-out infinite alternate}
  .n64-file-.status-completed { border-color: #40FF40;
	background: linear-gradient(135deg, #1a2e1a 0%, #0a1a0a 100%)}
  .n64-file-.status-error { border-color: #FF3030;
  ...
  .n64-file-name { font-weight: bold;
	color: #FFD700; word-break: break-word; text-shadow: 1px 1px 0 #000}
  .n64-file-size { font-size: 0.8rem;
	color: #CCCCCC; text-shadow: 1px 1px 0 #000}
  .n64-file-status { font-size: 0.75rem;
	color: #FFFFFF; font-weight: bold; text-shadow: 1px 1px 0 #000; letter-spacing: 0.5px}
  .n64-retry-countdown { color: #FFA500;
	animation: blink 1s infinite}
  @keyframes blink { 0%; } 50% { opacity: 1} 51%; } 100% { opacity: 0.3} }
  .n64-file-actions { display: flex;
	gap: 0.5rem}
  ...
  .n64-action-btn.cancel { background: #FF3030; border-color: #CC0000;
	color: #FFF}
  .n64-action-btn.retry { background: #40FF40; border-color: #00CC00;
	color: #000}
  .n64-upload-progress { margin-top: 1.5rem;
	padding: 1.5rem;background: linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 100%); border: 2px solid #FFD700; box-shadow: inset 0 0 20px rgba(255, 215, 0, 0.1), 0, 0 10px rgba(0, 0, 0, 0.5)}
  .n64-progress-text { display: flex; align-items: center;
	gap: 0.75rem; font-size: 0.9rem;
	color: #FFD700; font-weight: bold; margin-top: 1rem; text-shadow: 1px 1px 0 #000; letter-spacing: 0.5px}
  .n64-performance-metrics { margin-top: 1.5rem;
	padding: 1.5rem;background: linear-gradient(135deg, #1a1a2e 0%, #0f0f1a 100%); border: 2px solid #4090FF; box-shadow: inset 0 0 20px rgba(64, 144, 255, 0.1), 0, 0 15px rgba(64, 144, 255, 0.3)}
  .n64-metrics-header { display: flex; align-items: center;
  ...
  .n64-metric-item { display: flex; justify-content: space-between; align-items: center;
	padding: 0.75rem;background: linear-gradient(135deg, #2a2a2a 0%, #1a1a1a 100%); border: 1px solid #666; box-shadow: inset 1px 1px 0 rgba(255, 255, 255, 0.1)}
  .n64-metric-label { font-size: 0.75rem;
	color: #CCCCCC; font-weight: bold; text-shadow: 1px 1px 0 #000}
  .n64-metric-value { font-size: 0.8rem;
	color: #FFD700; font-weight: bold; text-shadow: 1px 1px 0 #000}
  .n64-gpu-status { text-align: center;
	padding: 0.75rem;background: linear-gradient(135deg, #1a2e1a 0%, #0a1a0a 100%); border: 1px solid #40FF40}
  .n64-gpu-indicator { font-size: 0.8rem;
	color: #40FF40; font-weight: bold; text-shadow: 1px 1px 0 #000; letter-spacing: 0.5px}
  .n64-upload-actions { margin-top: 1.5rem;
	display: flex; gap: 1rem}
  .n64-upload-button { flex: 1;
	display: flex; align-items: center; justify-content: center;
  ...
  .n64-error-alert { margin-top: 1rem;
	padding: 1rem;background: linear-gradient(135deg, #2e1a1a 0%, #1a0a0a 100%); border: 2px solid #FF3030;color: #FF6666, display: flex; align-items: center;
	gap: 0.75rem; font-weight: bold; text-shadow: 1px 1px 0 #000; box-shadow: inset 0 0 10px rgba(255, 48, 48, 0.1), 0, 0 10px rgba(255, 48, 48, 0.3)}
