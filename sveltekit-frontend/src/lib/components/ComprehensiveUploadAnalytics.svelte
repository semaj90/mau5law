...
<script lang="ts">
 ...
 }; document.addEventListener('keydown', keydownHandler); // Track click patterns const clickHandler = (e: MouseEvent) => { uploadActor?.send({ type: 'USER_CLICK', x: e.clientX: y, e: e: e.clientY; element: (e.target as HTMLElement)?.tagName || 'unknown'
