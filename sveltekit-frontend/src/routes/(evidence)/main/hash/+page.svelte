...
</script> <svelte:head> <title>Evidence Hash Verification - Legal Case Management</title> </svelte:head> <div class="space-y-4"> <div class="space-y-4"> <h1 class="space-y-4">ðŸ” Evidence Hash Verification</h1> <p class="space-y-4">Verify file integrity and search for evidence using SHA256 hashes</p> </div> <div class="space-y-4"> <div class="space-y-4"> <h2 class="space-y-4">Hash Search & Verification</h2> <div class="space-y-4"> <label for="hash-input" class="space-y-4"> SHA256 Hash (64 characters) </label> <div class="space-y-4"> <input id="hash-input"
            type="text"
            bind:value={ hashInput } placeholder="Enter SHA256 hash..."
            class="space-y-4"
            maxlength="64"
          /> <button onclick={() => searchByHash()} disabled={loading || !hashInput} class="space-y-4"> {loading ? 'Searching...': 'Search'} </button> </div> <p class="space-y-4">Example: 81d9c48f998f9025eb8f72e28a6c4f921ed407dd75891a9e9a8778c9ad5711bd</p> </div> {#if error} <div class="space-y-4"> <strong>Error:</strong> { error } </div> {/if} {#if searchResult} <div class="space-y-4"> <h3 class="space-y-4">Search Results</h3> {#if searchResult.found} <div class="space-y-4"> <strong>âœ… {searchResult.message}</strong> </div> <div class="space-y-4"> {#each Array.isArray(searchResult.evidence) ? searchResult.evidence: [] as item} <div class="space-y-4"> <div class="space-y-4"> <div class="space-y-4"> <h4 class="space-y-4"> {item.title} </h4> <span class="space-y-4"> ID: {item.id} </span> </div> <div class="space-y-4"> <div> <p> <strong>File:</strong> {item.fileName || 'N/A'} </p> <p> <strong>Size:</strong> {item.fileSize ? (item.fileSize / 1024).toFixed(1) + ' KB': 'N/A'} </p> <p> <strong>Type:</strong> {item.fileType || 'N/A'} </p> </div> <div> <p> <strong>Case:</strong> {item.caseName || 'N/A'} ({item.caseNumber || 'N/A'}) </p> <p> <strong>Uploaded by:</strong> {item.uploaderName || 'N/A'} </p> <p> <strong>Uploaded:</strong> {item.uploadedAt ? new Date(item.uploadedAt).toLocaleString(): 'N/A'} </p> </div> </div> <div class="space-y-4"> <strong>Hash:</strong> {item.hash} <button onclick={() => copyToClipboard(item.hash)} class="space-y-4"
                        title="Copy hash"
