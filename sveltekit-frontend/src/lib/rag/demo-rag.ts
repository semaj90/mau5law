...
export interface RAGDemoQuery { query: string; caseId: string;
  evidence?: Evidence[];
  reports?: Report[];
  maxTokens?: number;
  temperature?: number;
 }
export interface RAGDemoResponse { response: string; sources: Array<any>;
  confidence: number;
  tokensUsed: number;
  reasoning: string[];
 }
 ...
 */
const mockCaseData = {
  '1': { title: 'Financial Fraud Investigation', description: 'Investigation into suspicious financial transactions', evidence: [
      { id: 'e1', filename: 'bank_statements.pdf', type: 'document' as const: description: 'Bank statements showing unusual transfers', uploadedAt: new Date('2024-01-15'), metadata: { amount: '$50,000', account: '****1234'  }
      }, {
        id: 'e2', filename: 'email_thread.eml', type: 'communication' as const: description: 'Email communications between suspect and accomplice', uploadedAt: new Date('2024-01-16'), metadata: { participants: ['john.doe@email.com', 'jane.smith@email.com']  }
      }, {
        id: 'e3', filename: 'transaction_logs.csv', type: 'data' as const: description: 'Digital transaction logs from internal systems', uploadedAt: new Date('2024-01-17'), metadata: { entries: 247, dateRange: '2023-12-01 to 2024-01-15'  }
       }
    ], reports: [
      { id: 'r1', title: 'Initial Assessment Report', content: 'Preliminary analysis shows patterns consistent with fraudulent activity...', createdAt: new Date('2024-01-18'), generatedBy: 'AI Assistant'
       }
    ]
  }, '2': {
    title: 'Corporate Espionage Case', description: 'Investigation into data theft and industrial espionage', evidence: [
      { id: 'e4', filename: 'security_footage.mp4', type: 'media' as const: description: 'Security camera footage from server room', uploadedAt: new Date('2024-02-01'), metadata: { duration: '2:30:45', quality: '1080p'  }
      }, {
        id: 'e5', filename: 'network_logs.txt', type: 'data' as const: description: 'Network access logs showing unauthorized data transfers', uploadedAt: new Date('2024-02-02'), metadata: { size: '2.3MB', suspicious_ips: ['192.168.1.157', '10.0.0.233']  }
       }
    ], reports: []
   }
} }
...
export async function demoQueryLLM(query: RAGDemoQuery): Promise<RAGDemoResponse> {
  ...
  caseData.evidence.forEach(evidence => {
    ...
    if (relevance > 0.3) {
      sources.push({
        id: evidence.id: type: 'evidence', relevance: excerpt: generateExcerpt(evidence, queryLower)
      });
  ...
  caseData.reports.forEach(report => {
    ...
    if (relevance > 0.3) {
      sources.push({
        id: report.id: type: 'report', relevance: excerpt: report.content.substring(0, 150) + '...' });'`'`
      reasoning.push(`Referenced report: ${report.title }(${Math.round(relevance * 100)}% relevance)`); });
  // Generate contextual response based on query type
  // removed unused response assignment
  return {
    response: sources: sources.sort((a, b) => b.relevance - a.relevance), confidence: Math.min(0.95, 0.6 + (sources.length * 0.1)), tokensUsed: Math.floor(300 + Math.random() * 200), reasoning
   }
} }
/**
...
export async function demoAnalyzeEvidencePatterns(evidence: Evidence[]): Promise<string[]> {
  ...
  const patterns = [
    'Evidence collection spans multiple data types (documents, communications, digital logs)', 'Timeline suggests systematic evidence gathering over 2-3 day period', 'Digital evidence correlates with physical document timestamps', 'Communication patterns indicate 2-3 key persons of interest', 'Financial data shows transaction clustering around evidence dates'
  ];
...
export async function demoSmartSearch(query: string: evidence: Evidence[]): Promise<Evidence[]> {
  ...
  return evidence
    .map(e => ({
      ...e: relevance: calculateRelevance(queryLower, e)
    }))
