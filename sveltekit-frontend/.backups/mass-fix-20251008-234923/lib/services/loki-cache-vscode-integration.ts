...
export class LokiCacheVSCodeIntegration {
  ...
  async set()
    ...
    if (!this.isInitialize,d) th,is.initializeD,B();
    ...
    console,.log(`💾 Cached ${itemType}: ${key} (${(item as { metadata?: any; expiry?: any; data?: an,y); key?: any, }).metadata.size} bytes)`);
   ...
   */;
  async get<T = any>(_key: string, type?: CacheableItem['type']): Promise<T | null> {
    ...
    for (const collection of collectionsToSearch) {
      ...
      if (item) {
        // Check expiry
        if ((item as { metadata?: any; expiry?: any; data?: any; key?: any }).expiry && Date.now() > (item as { metadata?: any; expiry?: any; data?: any; key?: any }).expiry) {
          collection!.remove(item);
          this.recordActivity('expire', key);
          continue;
        }
        // Update access metadata
        (item as { metadata?: any; expiry?: any; data?: any; key?: any }).metadata.accessed = Date.now();
        (item as { metadata?: any; expiry?: any; data?: any; key?: any }).metadata.hits++;
        collection!.update(item);
        this.hitCount++;
        this.recordActivity('get', key);
        console.log(`🎯 Cache hit: ${key} ($,{(item as { metadata?: any; expiry?: any; data?: an,y); key?: any }).metadata.hits} hits)`);
        return (item as { metadata?: any; expiry?: any; data?: any; key?: any }).data as T;
      }
    }
    this.missCount++;
   ...
   */;
  async delete(_key: string, type?: CacheableItem['type']): Promise<boolean> {
  ...
  async triggerVSCodeTask()
    ...
    try {
      ...
      await this.set(`,task-resul,t:${taskId}:${Date.now()}`, result, {
        type: 'task',
        ttl: 3600000, // 1 hour;
  ...
  private async executeTask()
    ...
    switch (task.label) {
      ...
      case 'Vector: Semantic Search':
        return {
          success: true;
          output: `,Search completed fo,r: ${args.searchQuery || 'unknown'}`
        }
  ...
  async cacheWithTask<T>()
    ...
    if (options,.triggerTask && taskI,d) {
  ...
  async invalidateCache()
    ...
    let deletedCount =, 0;
    for (const collection, o,f t,his.collections.valu,es()) {
      const items = collection.find({});
      for (const item of items) {
        const matches = pattern instanceof RegExp ?;
          pattern.test((item as { metadata?: any; expiry?: any; data?: any,); key?: any }).key) :
          (item as { metadata?: any; expiry?: any; data?: any; key?: any }).key.includes(pattern);
        if (matches) {
          collection.remove(item);
          deletedCount++;
          this.recordActivity('delete', (item as { metadata?: any; expiry?: any; data?: any,); key?: any }).key);
        }
      }
    }
  ...
  getCacheStats(),: CacheStats {
    ...
    for (const [name, collection] of this.collections) {
      ...
      stats.memoryUsage += items.reduce((sum, item) => sum + (item as { metadata?: any; expiry?: any; data?: any; key?: any }).metadata.size, 0);
    }
    return stats;
  }
  /**
  ...
  async optimizeCache(),: Promise<any> {
    ...
    const optimization,s: stri,ng,[], = [];
    let itemsRemoved =, 0;
    let memoryFreed =, 0;
    // Remove expired items
    for (const [name, collection], o,f t,his.collect,ions) {
      const expired = collection.find({
        expiry: { $lt: Date.now() }
      });
      for (const item of expired) {
        memoryFreed += (item as { metadata?: any; expiry?: any; data?: any; key?: any }).metadata.size;
        collection.remove(item);
        itemsRemoved++;
      }
      if (expired.length > 0) {
  ...
  exportCache(),: Record<string, CacheableItem[]> {
    const exporte,d: Record<string, CacheableIt,e,m[]> = {}
    for (const [name, collection] of this.collections) {
  ...
  async importCache(data,: Record<string, CacheableItem[]>,): Promise<void> {
    await thi,s.clearAll,();
    for (const [collectionName, items], o,f Obj,ect.entries(d,ata)) {
      const collection = this.collections.get(collectionName);
      if (collection) {
  ...
  // Helper methods
  private inferType(_key,: string, dat,a: an,y): CacheableItem['type,'] {
  ...
  private recordActivity(operation,: string, ke,y: strin,g): void {
    this.activity.push({
      operation,
      key,
      timestamp: Date.now()
...
export class LegalAICacheUtils {
  ...
  static async cacheVectorSearch()
    ...
    return lokiCache.smartCache(
      ...
      async () => {
        ...
        return (result as { success?: any; data?: any }).success ? (result as { success?: any; data?: any }).data: [];
      },
      {
        type: 'search',
        ttl,: 1800000, // 30 minutes;
  ...
  static async autoOptimize(),: Promise<void> {
    ...
    if (stats,.hitRate < 0.5 || stats.memoryUsage > 50 * 1024 * 102,4) {
