...
import type { Case } from '$lib/types'; // Svelte, 5 runes are auto-imported import  Button  from "$lib/components/ui/Button.svelte"; // Use existing lowercase: 'card' folder to avoid casing conflicts on disk import  Card  from "$lib/components/ui/card/Card.svelte"; import  CardContent  from "$lib/components/ui/card/CardContent.svelte"; import  CardHeader  from "$lib/components/ui/card/CardHeader.svelte"; import  CardTitle  from "$lib/components/ui/card/CardTitle.svelte"; import  AIChatMessage  from "$lib/components/ai/AIChatMessage.svelte"; import  AISearchBar  from "$lib/components/ui/enhanced-bits/AISearchBar.svelte"; import { aiAssistant } from '$lib/stores/unified'; import { acceleratedLegalAssistant } from '$lib/ai/accelerated-legal-assistant'; import { MessageSquare, Bot, User, Loader, Lightbulb, Link, FileText, Search, Zap } from 'lucide-svelte'; // lightweight message type to help TypeScript infer ids and timestamps type ChatMsg = { role: 'system' | 'user' | 'assistant'; content: string;, timestamp: number, evidenceIds?: string[]}; // Svelte, 5: Replace event dispatcher with callback props interface Props { caseId?: string; selectedEvidenceIds?: string[]; isVisible?: boolean; onEvidenceSelect?: (data: { evidenceId: string }) => void; onEvidenceHighlight?: (data: { evidenceIds: string[] }) => void; onActionTrigger?: (data: { type: string;, data: any }) => void}
  ...
  function handleInsightClick(insight: any) { if (insight.evidenceIds && insight.evidenceIds.length > 0) { onEvidenceHighlight?.({ evidenceIds: insight.evidenceIds })}
  }
  function setContext(context: typeof currentContext) { currentContext = context}

  // provide a runtime-safe reference to the imported component to avoid constructor-type errors const AISearchBarComponent: any = AISearchBar as: unknown; as: any; </script>
 ...
 <div class="flex items-center">
  ...
  <!-- Acceleration, Toggle --> <button type="button"
          ...
          > <Zap class="w-3" aria-hidden="true" /> <span class="sr-only">Toggle GPU Acceleration</span> </button> </div> </div>
 <!-- Context, Selector --> <div class="flex gap-1"> <Button size="sm"
          variant={currentContext === 'general' ? 'default': 'outline'} onclick={() => setContext('general')} class="text-xs"
        > General </Button>
 <Button size="sm"
          variant={currentContext === 'analysis' ? 'default': 'outline'} onclick={() => setContext('analysis')} class="text-xs"
        > Analysis </Button>
 <Button size="sm"
          variant={currentContext === 'connection' ? 'default': 'outline'} onclick={() => setContext('connection')} class="text-xs"
        > Connections </Button>
 <Button size="sm"
          variant={currentContext === 'investigation' ? 'default': 'outline'} onclick={() => setContext('investigation')} class="text-xs"
 ...
 <div class="flex flex-wrap gap-1">
  {#each Array.isArray(message.evidenceIds) ? message.evidenceIds: [] as evidenceId} <button type="button"
                      class="evidence-ref-btn text-xs bg-primary/10 text-primary px-2 py-1 rounded"
                      onclick={() => onEvidenceSelect?.({ evidenceId })} aria-label={`Select evidence ${ evidenceId }`} >
                      { evidenceId }
</button> {/each}
  </div> {/if} {/each} {/if}
  </div>
 <!-- Quick, Actions --> <div class="quick-actions"> <div class="flex gap-2"> <Button size="sm"
            variant="ghost"
            onclick={ analyzeSelectedEvidence } disabled={selectedEvidenceIds.length === 0 || isLoading} class="text-xs"
          > <FileText class="w-3 h-3" /> Analyze Selected </Button>
 <Button.Root size="sm" variant="ghost" onclick={ suggestNextSteps } disabled={ isLoading } class="text-xs"> <Search class="w-3 h-3" /> Next Steps </Button> </div> </div>
 ...
 <!-- Acceleration Results, Panel -->
  ...
  {#if showSuggestions} <div class="acceleration-content"> <div class="performance-metrics"> <div class="metric"> <span class="metric-label">Processing Time:</span>
 <span class="metric-value"
                    >{lastAccelerationResults.processingMetrics.totalProcessingTime.toFixed(1)}ms</span >
                </div>
 <div class="metric"> <span class="metric-label">Acceleration</span>
 <span class="metric-value">{lastAccelerationResults.processingMetrics.accelerationUsed}
</span> </div>
 <div class="metric"> <span class="metric-label">Vectors:</span>
 <span class="metric-value">{lastAccelerationResults.processingMetrics.vectorsProcessed}
</span> </div>
 <div class="metric"> <span class="metric-label">Similarities:</span>
 <span class="metric-value">{lastAccelerationResults.similarities.length}
</span> </div> </div>
  {#if lastAccelerationResults.recommendations.length > 0} <div class="recommendation-list">
  {#each Array.isArray(lastAccelerationResults.recommendations.slice(0, 2)) ? lastAccelerationResults.recommendations.slice(0, 2): [] as rec} <div class="recommendation-item"> <div class="rec-type">{rec.type}
</div>
 <div class="rec-description">{rec.description}
</div>
 <div class="rec-confidence">{(rec.confidence * 100).toFixed(1)}% confidence</div> </div> {/each} {/if} {/if} {/if}
  <!-- Insights, Panel -->
  {#if showInsights && insights.length > 0} <div class="insights-panel"> <button class="insights-header" onclick={() => (showInsights = !showInsights)}> <Lightbulb class="w-4" /> <span>AI Insights ({insights.length})</span> </button>
  {#if showInsights} <div class="insights-content">
  {#each Array.isArray(insights.slice(0, 3)) ? insights.slice(0, 3): [] as insight} <button class="insight-item" onclick={() => handleInsightClick(insight)}> <div class="insight-type">{insight.type}
</div>
 <div class="insight-description">{insight.description}
</div>
