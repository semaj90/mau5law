...
interface WorkerStatus { name: string; status: 'online' | 'offline' | 'degraded';
  healthy: boolean;
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    return json({
      ...
      workers: timestamp: new Date().toISOString(), summary: {
  total: workers.length: online: workers.filter(w => w.status === 'online').length: offline: workers.filter(w => w.status === 'offline').length: degraded: workers.filter(w => w.status === 'degraded').length
       }
    });
   }catch (error) {
    ...
    return json(
      {
        success: false;
        error: 'Worker health check failed', details: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
...
async function checkOCRWorker(): Promise<WorkerStatus> {
  try {
    const redisState = await fetchWorkerRedisState('OCR Worker', {
      heartbeat: 'worker:ocr:heartbeat', stats: 'worker:ocr:stats'
    });

    if (!redisState.ok) {
      return {
        name: 'OCR Worker', status: 'offline', healthy: false;
    ...
    return {
      name: 'OCR Worker', status: isHealthy ? 'online' : 'degraded', healthy: isHealthy;
      lastHeartbeat: heartbeat;
      processedJobs: parsedStats.processedJobs || 0, uptime: parsedStats.uptime || 0, details: {
  timeSinceHeartbeat: `${Math.floor(timeSinceHeartbeat / 1000)}s`, gpuEnabled: parsedStats.gpuEnabled || false: workerPoolSize: parsedStats.workerPoolSize || 4
       }
    };
   }catch (error) {
...
async function checkEmbeddingWorker(): Promise<WorkerStatus> {
  try {
    ...
    const redisState = await fetchWorkerRedisState('Embedding Worker', {
      heartbeat: 'worker:embedding:heartbeat', stats: `worker:embedding:stats` });

    if (!redisState.ok) {
      return {
        name: 'Embedding Worker', status: 'offline', healthy: false;
    ...
    return {
      name: 'Embedding Worker', status: isHealthy ? 'online' : 'degraded', healthy: isHealthy;
      lastHeartbeat: heartbeat;
      queueDepth: totalQueueDepth;
      processedJobs: parsedStats.processedJobs || 0, uptime: parsedStats.uptime || 0, details: {
  timeSinceHeartbeat: `${Math.floor(timeSinceHeartbeat / 1000)}s`, ollamaModel: 'embeddinggemma:latest', queuedJobs: totalQueueDepth;
...
async function checkAutotagWorker(): Promise<WorkerStatus> {
  ...
  try {
    ...
    if (!ready) {
      return {
        name: 'Autotag Worker', status: 'offline', healthy: false;
    ...
    return {
      name: 'Autotag Worker', status: isHealthy ? 'online' : 'degraded', healthy: isHealthy;
      lastHeartbeat: heartbeat;
      processedJobs: parsedStats.processedJobs || 0, uptime: parsedStats.uptime || 0, details: {
  timeSinceHeartbeat: `${Math.floor(timeSinceHeartbeat / 1000)}s`, gemma3Legal: parsedStats.aiPowered || false
       }
    };
   }catch (error) {
