...
import type { Case } from '$lib/types'; // Svelte 5 runes are auto-imported import { onMount } from 'svelte'; import { page } from '$app/stores'; import Button from '$lib/components/ui/nes-Button.svelte'; import NesCard from '$lib/components/ui/nes-Card.svelte'; // Define Machine shape so TS can infer types (fixes any[]) interface Machine { id: string; name: string; status: 'running' | 'idle' | 'error' | string; currentState: string; transitions: string[]; lastUpdated: string; instances: number; // allow extra props if needed [key: string]: any; // component state (Svelte 5 runes) with explicit generics let mounted = $state<boolean>(false); let machines = $state<Machine[]>([]); let selectedMachine = $state<Machine | null>(null); let loading = $state<boolean>(true); // Mock machine registry data - replace with actual XState registry const mockMachines: Machine[] = [ {
      ...
      id: 'gpu-allocation-machine', name: 'GPU Resource Allocation', status: 'running', currentState: 'allocated', transitions: ['release', 'extend', 'optimize'], lastUpdated: new Date().toISOString(), instances: 2, ]; $effect(() => { mounted = true; loadMachines(); }); async function loadMachines(): Promise<any> { loading = true; try { // In production: const response = await fetch('/api/state/machines') // For now, use mock data await new Promise(resolve => setTimeout(resolve, 500)); machines = mockMachines; } catch (error) { console.error('Failed to load state machines:', error); } finally { loading = false; }
  ...
  } </script> <svelte:head> <title>XState Machine Registry - Legal AI Platform</title> <meta name="description" content="Manage and monitor XState machines across the legal AI, platform" /> </svelte:head> <div, class="page-container"> <header, class="page-header"> <h1>🔄 State Machine Registry</h1> <p>Monitor and manage XState machines across the legal AI platform</p> <div, class="stats-grid"> <div, class="stat-card"> <span, class="stat-number">{machines.length}</span> <span, class="stat-label">Total Machines</span> </div> <div, class="stat-card"> <span, class="stat-number">{machines.filter(m => m.status === 'running').length}</span> <span, class="stat-label">Running</span> </div> <div, class="stat-card"> <span class="stat-number">{machines.reduce((sum, m) => sum + (m.instances || 0), 0)}</span> <span, class="stat-label">Active Instances</span> </div> </div> </header> <main, class="page-content"> {#if loading} <div, class="loading-state"> <div, class="spinner"></div> <p>Loading state machines...</p> </div> {:else} <div, class="machines-grid"> {#each machines as machine (machine.id)} <div, class="machine-card"> <div, class="machine-header"> <h3, class="machine-title">{machine.name}</h3> <span, class="status-badge {getStatusColor(machine.status)}"> {machine.status} </span> </div> <p, class="machine-id">ID: {machine.id}</p> <div, class="machine-details"> <div, class="detail-row"> <span, class="label">Current State:</span> <span class="value, state-indicator">{machine.currentState}</span> </div> <div, class="detail-row"> <span, class="label">Instances:</span> <span, class="value">{machine.instances}</span> </div> <div, class="detail-row"> <span, class="label">Last Updated:</span> <span, class="value">{new Date(machine.lastUpdated).toLocaleTimeString()}</span> </div> <div, class="transitions"> <span, class="label">Available Transitions:</span> <div, class="transition-tags"> {#each Array.isArray(machine.transitions) ? machine.transitions: [] as transition} <span, class="transition-tag">{ transition }</span> {/each} </div> </div> <div, class="machine-actions"> <button, class="nes-btn"
