<script lang="ts">
  ...
  let testTexts = $state([
    ...
    'The defendant denies all allegations and moves for summary judgment.',
    'Jurisdiction is proper under 28 U.S.C. § 1332 diversity of citizenship.'
  ]);

  let queryText = $state('Contract dispute involving breach of agreement');
  ...
  async function findSimilar() {
    ...
    try {
      console.log('🔍 Finding similar documents for query:', queryText);
      ...
      // Find similarities
      similarities = ClientEmbeddingService.findSimilar(
        queryEmbedding,
...
<div class="client-embedding-demo">
  ...
  <section class="status-section">
    ...
    {#if isInitializing}
      <div class="status-card initializing">
        <h3>Initializing...</h3>
        <p>Loading EmbeddingGemma 300M ONNX model (~291MB)</p>
    ...
    {:else if isReady}
      <div class="status-card ready">
        ...
        <div class="model-info">
          ...
          <strong>Size:</strong> {service?.getModelInfo().size}
        </div>
      </div>
    {:else}
      ...
      <section class="search-section">
        ...
        <div class="search-input">
          ...
          <textarea
            ...
            class="query-text"
          ></textarea>
          <button onclick={findSimilar} class="search-btn">🔍 Find Similar</button>
        </div>

        {#if similarities.length > 0}
          <div class="similarities">
            <h3>Most Similar Documents</h3>
