...
export const errorEvents = pgTable(
	...
	(table) => ({
		...
		idxRouteCluster: index('idx_error_events_route_cluster').on(
			table.routePath,
			table.clusterId
		)
	})
...
export const errorPatchLog = pgTable(
	'error_patch_log',
	{
		...
		status: varchar('status', { length: 20 })
			.notNull()
			.default('applied'), // applied|rolled_back|reverted
...
export const errorSuggestionStates = pgTable(
	...
	(table) => ({
		suggestionRouteUserUnique: uniqueIndex(
			'error_suggestion_states_suggestion_route_user_idx'
		).on(table.suggestionId, table.routePath, table.userId)
	})
);
