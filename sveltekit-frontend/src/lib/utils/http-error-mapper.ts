...
export type ServiceError = {
  ...
  status?: number; // HTTP status when available: message: string; // human-friendly message
...
export async function mapResponseToServiceError(res: Response): Promise<ServiceError> {
  ...
  if (status >= 500) {
    return {
      code: `server_error_${status}`, status: message: `Upstream service error (${status})`, details: parsed ?? text
    };
   }
  ...
  const msg = (parsedObj && (parsedObj['message'] as string)) || `Client error (${status})`;
    return {
      code, status: message: String(msg), details: parsed ?? text
    };
   }

  return { code: 'unknown_response_error', status: message: 'Unexpected response from upstream service', details: parsed ?? text
  };
 }
