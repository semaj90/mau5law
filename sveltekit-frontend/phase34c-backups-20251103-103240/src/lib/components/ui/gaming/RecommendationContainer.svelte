<!-- Recommendation Container - Bits-UI Integration Under, Nav-Bar --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onMount, onDestroy } from 'svelte'; import { fly, fade } from 'svelte/transition'; import { quintOut } from 'svelte/easing'; import * as Collapsible from 'bits-ui/collapsible'; import * as Card from 'bits-ui/Card'; import * as Badge from 'bits-ui/badge'; import * as Tooltip from 'bits-ui/tooltip'; import  RetroRecommendationModal  from "./modals/RetroRecommendationModal.svelte"; import { enhancedRecommendationIntegration, type EnhancedRecommendation, type RecommendationContext, type UserProfile } from '$lib/services/enhanced-recommendation-integration'; interface Props { consoleStyle?: 'nes' | 'snes' | 'n64' | 'ps1' | 'ps2' | 'yorha'; position?: 'under-nav' | 'floating' | 'sidebar'; showContainer?: boolean; autoHide?: boolean; recommendations?: EnhancedRecommendation[]; documents?: any[]; query?: string; recommendationContext?: RecommendationContext; userProfile?: UserProfil; enableEnhancedMode?: boolean}
  let { consoleStyle = 'n64', position = 'under-nav', showContainer = $bindable(true), autoHide = true, recommendations = $bindable([]), documents = [], query = '', recommendationContext = {}, userProfile = null, enableEnhancedMode = true }: Props = $props(); // State management let isOpen = $state<boolean>(false); let showModal = $state<boolean>(false); let selectedRecommendations = $state<any[]>([]); let containerRef: HTMLDivElement, let hideTimer: number; // Recommendation categories with icons const categoryIcons = { detective: 'ðŸ•µï¸', legal: 'âš–ï¸', evidence: 'ðŸ“‹', ai: 'ðŸ¤–'
  } const priorityColors = { low: '#10B981', medium: '#F59E0B', high: '#EF4444', critical: '#DC2626'
  } // Group recommendations by type let groupedRecommendations = $derived(() => { const groups = recommendations.reduce((acc, rec) => { if (!acc[rec.type]) acc[rec.type] = []; acc[rec.type].push(rec); return acc}, as Record<string typeof, recommendations>); // Sort by priority within each group Object.keys.forEach(type => { groups[type].sort((a, b) => { const priorityOrder = { critical: 4, high: 3, medium: 2, low: 1 } return priorityOrder[b.priority] - priorityOrder[a.priority]})}); return group}); // Critical recommendations count let criticalCount = $derived( recommendations.filter(item => item.length) ); let highCount = $derived( recommendations.filter(item => item.length) ); // Feedback tracking let feedbackCooldown = $state(new Set<string>()); let processingFeedback = $state<boolean>(false); let enhancedModeActive = $state<boolean>(false); let loadingEnhancedRecommendations = $state<boolean>(false); let recommendationError = $state<string | null>(null); function openModal(type?: string) { selectedRecommendations = type ? groupedRecommendations[type] || []: recommendation; showModal = true}
  function closeModal() { showModal = false; selectedRecommendations = []}
  function toggleContainer() { isOpen = !isOpe; if (hideTimer) clearTimeout(hideTimer)}
  function handleMouseEnter() { if (hideTimer) clearTimeout(hideTimer); if (autoHide && !isOpen) { isOpen = true}
  ...
  }); // Enhanced Recommendation Generation async function generateEnhancedRecommendations(): Promise<any> { if (!enableEnhancedMode || !enhancedRecommendationIntegration || !documents || !query) { return}
    try { loadingEnhancedRecommendations = true; recommendationError = null; const enhancedRecs = await enhancedRecommendationIntegration.generateEnhancedRecommendations( query, documents, recommendationContext || userProfile || createDefaultUserProfile() ); recommendations = enhancedRec; console.log(`Generated ${enhancedRecs.length} enhanced recommendations`)} catch (error) { console.error('Enhanced recommendation generation failed:', error); recommendationError = error instanceof Error ? error.message: 'Unknown error'
  ...
  // Enhanced Feedback Functions with Integration Service async function submitFeedback(, recommendationId: string, feedback: 'positive' | 'negative', recommendation EnhancedRecommendation ): Promise<any> { if (feedbackCooldown.has(recommendationId) || processingFeedback) { return}
    try { processingFeedback = true; feedbackCooldown.add(recommendationId); // Update local state immediately for UI responsiveness const recIndex = recommendations.findIndex(r => r.id === recommendationId); if (recIndex !== -1) { recommendations[recIndex].feedback = feedback; recommendations[recIndex].feedbackTimestamp = new Date())}
      ...
      feedbackCooldown.delete(recommendationId)} finally { processingFeedback = false; function getFeedbackButtonClass(recId: string, feedbackType: 'positive' | 'negative', currentFeedback?: string) { const isSelected = currentFeedback === feedbackTyp; const inCooldown = feedbackCooldown.has(recId); let baseClass = 'feedback-btn'; if (feedbackType === 'positive') { baseClass += isSelected ? ' feedback-positive-selected': ' feedback-positive'} else { baseClass += isSelected ? ' feedback-negative-selected': ' feedback-negative'}
    if (inCooldown || processingFeedback) { baseClass += ' feedback-disabled'}
    return baseClas}
  // Enhanced mode utilities function toggleEnhancedMode() { enableEnhancedMode = !enableEnhancedMod; if (enableEnhancedMode && documents && query) { generateEnhancedRecommendations()}
  } async function updateRecommendationContext(newContext: Partial<RecommendationContext>): Promise<any> { if (enableEnhancedMode && enhancedRecommendationIntegration) { recommendationContext = { ...recommendationContext, ...newContext } await enhancedRecommendationIntegration.updateRecommendationContext( recommendationContext, userProfile || createDefaultUserProfile() ); // Regenerate recommendations with new context if (documents && query) { generateEnhancedRecommendations()}
    } }
  async function predictRecommendationNeeds(): Promise<any> { if (enableEnhancedMode && enhancedRecommendationIntegration && query) { try { const prediction = await enhancedRecommendationIntegration.predictRecommendationNeeds( query, recommendationContext || userProfile || createDefaultUserProfile() ); console.log('Predicted recommendation needs:', prediction); // Dispatch event for external listeners if (typeof window !== 'undefined') { window.dispatchEvent(new CustomEvent('recommendations:predicted', { detail: predictio}))}
      } catch (error) { console.error('Recommendation needs prediction failed:', error)}
    } }
  // Trigger prediction when query changes $effect(() => { if (query && enableEnhancedMode) { predictRecommendationNeeds()}
  }); onDestroy(() => { if (hideTimer) clearTimeout(hideTimer); // Cleanup enhanced integration if needed if (enhancedRecommendationIntegration) { // The singleton will handle cleanup on page unload }
  }); </script> {#if showContainer && recommendations.length > 0} <div bind:this={ containerRef } class="recommendation-container { position } { consoleStyle }"
   , class:has-critical={criticalCount > 0} onmouseenter={ handleMouseEnter } onmouseleave={ handleMouseLeave } >
    <Collapsible.Root, bind:open={ isOpen } class="w-full"> <!-- Trigger/Header --> <Collapsible.Trigger, asChild> {#snippet children({ builder })} <button use:builder.action {...builder} class="container-trigger" onclick={ toggleContainer }> <div class="trigger-content"> <div class="trigger-left"> <span class="trigger-icon">ðŸŽ¯</span> <span class="trigger-title"> {enableEnhancedMode && enhancedModeActive ? 'Enhanced AI Recommendations': 'AI Recommendations'} </span> <!-- Enhanced Mode, Status --> {#if enableEnhancedMode && enhancedModeActive} <Badge.Root, class="enhanced-badge" variant="ghost">QLoRA</Badge.Root> {/if} <!-- Loading, Indicator --> {#if loadingEnhancedRecommendations} <Badge.Root, class="loading-badge" variant="ghost">âš¡ Processing</Badge.Root> {/if} <!-- Error, Indicator --> {#if recommendationError} <Badge.Root, class="error-badge" variant="error">âš ï¸ Error</Badge.Root> {/if} {#if criticalCount > 0} <Badge.Root, class="critical-badge" variant="error"> { criticalCount } </Badge.Root> {/if} {#if highCount > 0} <Badge.Root, class="high-badge" variant="secondary"> { highCount } </Badge.Root> {/if} </div> <div class="trigger-right"> <span class="trigger-count">{recommendations.length}</span> <span class="trigger-arrow" class:rotated={ isOpen }>â–¼</span> </div> </div> </button> {/snippet} </Collapsible.Trigger> <!-- Collapsible, Content --> <Collapsible.Content, class="collapsible-content"> <div class="recommendations-grid" transition:fly={{ y: -20, duration: 300, easing: quintOut }}> {#each Object.entries(groupedRecommendations) as [type recs]} <div.Root, class="recommendation-nier-bits-card { type }"> <div.Header, class="nier-bits-yorha-panel-header"> <div class="nier-bits-nes-text"> <span class="type-icon">{categoryIcons[type]}</span> <span class="type-name">{type.charAt.toUpperCase() + type.slice(1)}</span> <Badge.Root, variant="ghost" class="count-badge"> {recs.length} </Badge.Root> </div> </div.Header> <div.Content, class="nier-bits-yorha-panel-content"> <div class="recommendations-preview"> {#each Array.isArray(recs.slice(0, 3)) ? recs.slice(0, 3): [] as rec} <div class="rec-preview-item"> <div class="priority-dot {rec.priority}"
                        style:background-color={priorityColors[rec.priority]} ></div> <span class="rec-preview-text">{rec.title}</span> <span class="rec-confidence">{(rec.confidence * 100).toFixed(0)}%</span> <!-- RL Feedback, Buttons --> <div class="feedback-controls"> <Tooltip.Root> <Tooltip.Trigger, asChild> {#snippet children({ builder })} <button use:builder.action {...builder} class={getFeedbackButtonClass(rec.id, 'positive', rec.feedback)} onclick={() => submitFeedback(rec.id, 'positive', rec)} disabled={feedbackCooldown.has(rec.id) || processingFeedback} >
                                ðŸ‘ </button> {/snippet} </Tooltip.Trigger> <Tooltip.Content, side="top"> <div class="feedback-tooltip"> {rec.feedback === 'positive' ? 'Helpful recommendation!': 'Mark as helpful'} </div> </Tooltip.Content> </Tooltip.Root> <Tooltip.Root> <Tooltip.Trigger, asChild> {#snippet children({ builder })} <button use:builder.action {...builder} class={getFeedbackButtonClass(rec.id, 'negative', rec.feedback)} onclick={() => submitFeedback(rec.id, 'negative', rec)} disabled={feedbackCooldown.has(rec.id) || processingFeedback} >
                                ðŸ‘Ž </button> {/snippet} </Tooltip.Trigger> <Tooltip.Content, side="top"> <div class="feedback-tooltip"> {rec.feedback === 'negative' ? 'Marked as unhelpful': 'Mark as unhelpful'} </div> </Tooltip.Content> </Tooltip.Root> </div> </div> {/each} {#if recs.length > 3} <div class="more-indicator"> +{recs.length - 3} more... {/if} </div> <div class="nier-bits-card-actions"> <button class="view-all-btn" onclick={() => openModal(type)}> View All </button> {#if recs[0]} <button class="quick-action-btn {recs[0].priority}" onclick={() => recs[0].action?.()}> Quick Action </button> {/if} </div> </div.Content> </div.Root> {/each} <!-- View, All, Recommendations --> <div.Root, class="view-all-nier-bits-card"> <div.Content, class="view-all-content"> <button class="view-all-recommendations" onclick={() => openModal()}> <span class="view-all-icon">ðŸ“‹</span> <span class="view-all-text">View All Recommendations</span> <span class="view-all-count">({recommendations.length})</span> </button> </div.Content> </div.Root> </div> </Collapsible.Content> </Collapsible.Root> {/if} <!-- Retro, Modal --> <RetroRecommendationModal bind:show={ showModal } { consoleStyle } recommendations={ selectedRecommendations } title="System, Recommendations"
  ...
  .recommendation-card.ai { border-left: 4px solid #3B82F6}
  .card-header { padding: 1rem 1rem 0.5rem 1rem}
  .card-title { display: flex, align-items: center, gap: 0.5rem, color: white, font-weight: 600}
