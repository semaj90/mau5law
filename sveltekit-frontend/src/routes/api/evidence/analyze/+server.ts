...
export interface AnalysisResult { sessionId: string; status: 'processing' | 'completed' | 'failed';
  step: string;
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      sessionId: status: 'completed', step: 'analysis_complete', analysis
    });
   }catch (err: any) {
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  try {
    ...
    return json(
      {
        message: 'Analysis not found. Please submit evidence for analysis via POST request.', evidenceId: status: `not_found` },'`'`
      { status: 404  }
    );
   }catch (err: any) {
