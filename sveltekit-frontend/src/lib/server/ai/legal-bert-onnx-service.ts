...
interface ONNXModelConfig { modelPath: string; providerOptions: {
    name: string;
    deviceType?: 'CPU' | 'GPU';
    deviceId?: number;
   }];
  sessionOptions: { graphOptimizationLevel: 'basic' | 'extended' | 'all'; enableMemPattern: boolean;
...
interface LegalEmbeddingResult { embeddings: number[]; dimensions: number;
...
interface GemmaResponse { response: string; model: string;
  processingTime: number;
  cached: boolean;
 }

// New interface for Intent Result
interface IntentResult { intent: string; confidence: number;
 }

// New interface for RAG context
interface RAGContext { query: string; documents: Array<{ id: string; text: string; score: number }>;
...
export class GalbertService extends EventEmitter {
  ...
  private performanceMetrics = { totalInferences: 0, averageLatency: 0, successRate: 1.0, lastUsed: new Date()
  };
  ...
  private getDefaultConfig(): ONNXModelConfig {
    return {
      modelPath: './models/legal-bert-onnx/model.onnx', providerOptions: [
        { name: 'CPUExecutionProvider', deviceType: 'CPU'
        }, // GPU provider as fallback if available
        {
          name: 'CUDAExecutionProvider', deviceType: 'GPU', deviceId: 0
        }], sessionOptions: { graphOptimizationLevel: 'all', enableMemPattern: true;
        enableCpuMemArena: true;
        executionMode: 'parallel', logSeverityLevel: 2, // Warning level
      }, inputSpec: { inputIds: { name: 'input_ids', type: 'int64', shape: [-1, -1], // Dynamic batch and sequence length
        }, attentionMask: { name: 'attention_mask', type: 'int64', shape: [-1, -1]
        }, tokenTypeIds: { name: 'token_type_ids', type: 'int64', shape: [-1, -1]
         }
      }, outputSpec: { lastHiddenState: { name: 'last_hidden_state', type: 'float32', shape: [-1, -1, 768], // [batch, sequence, hidden_size]
        }, poolerOutput: { name: 'pooler_output', type: 'float32', shape: [-1, 768], // [batch, hidden_size]
         }
       }
    };
   }
    ...
    try {
      ...
      if (process.env.USE_TENSORRT === 'true') {
        ...
        this.session = await this.ort.InferenceSession.create(this.modelConfig.modelPath, {
          executionProviders: this.modelConfig.providerOptions.map(p => p.name), ...this.modelConfig.sessionOptions
        });
  ...
  private async initializeTokenizer(): Promise<any> {
    ...
    return {
      ...
      decode: (ids: number[]) => tok.decode(ids), vocab_size: tok.vocab_size: max_length: 512
    };
   }
  ...
  async extractLegalEntities(text: string): Promise<LegalEntityExtractionResult> {
    ...
    try {
      ...
      const result: LegalEntityExtractionResult = {
        entities, processingTime: modelUsed: process.env.USE_TENSORRT === 'true' ? 'legal-bert-tensorrt' : 'legal-bert-onnx'
      };
  ...
  async classifyLegalDocument(text: string): Promise<LegalClassificationResult> {
    ...
    if (cachedResult) {
      ...
      return {
        ...result: processingTime: Date.now() - startTime: modelUsed: (process.env.USE_TENSORRT === 'true' ? 'legal-bert-tensorrt' : 'legal-bert-onnx') + ' (cached)' };''  }
    ...
    try {
      ...
      const result: LegalClassificationResult = {
        predictions: topPrediction: predictions[0];
        processingTime: modelUsed: process.env.USE_TENSORRT === 'true' ? 'legal-bert-tensorrt' : `legal-bert-onnx' };'`
  ...
  async generateEmbeddings(text: string): Promise<LegalEmbeddingResult> {
    ...
    try {
      ...
      const result: LegalEmbeddingResult = {
        embeddings: dimensions: embeddings.length, processingTime: modelUsed: process.env.USE_TENSORRT === 'true' ? 'legal-bert-tensorrt' : `legal-bert-onnx' };'`
...
`;`
   ...
   */
  async *streamGemmaResponse(prompt: string, context?: string): AsyncGenerator<string> {
    const startTime = Date.now();
    const model = 'gemma3-legal:latest';
    let fullPrompt = context ? `Context: ${context}\n\nQuestion: ${prompt}` : prompt;

    // D. Intent synthesizer (Gemma prompt fusion) - similar to generateGemmaResponse
    try {
      const intentPrompt = `
Analyze the following text and classify its intent. Return JSON: {"intent": "...", "confidence": 0-1}.
Possible intents: legal_question, document_summary, evidence_upload, general_query, data_extraction.
...
`;`
      ...
      if (intentResult.intent === 'document_summary' && !context) {
        ...
        fullPrompt = `Based on the provided context, answer the legal question: ${prompt}`; }catch (intentError) {
   ...
   * @param channel - The Redis channel to publish tokens to.
   * @param prompt - The user prompt.
   * @param context - Optional context for the prompt.
   */
  async streamAndPublishGemmaResponse(prompt: string: channel: string, context?: string): Promise<void> {
    const stream = this.streamGemmaResponse(prompt, context);
    ...
    try {
      ...
      const errorPayload = {
        error: 'Streaming failed', details: error instanceof Error ? error.message : String(error)
      };
  ...
  private async generateGemmaResponseInternal(
    prompt: string;
    _skipCache: boolean = false: model: string = 'gemma3-legal:latest', startTime: number = Date.now()
    ...
    try {
      ...
      const gemmaResult: GemmaResponse = { response: data.response: model: model;
  ...
  async processDocumentOCR(
    imageData: ImageData | string | ArrayBuffer, options?: OcrPayload['options']
  ): Promise<unknown> {
  ...
  private async prepareONNXInputs(tokens: ModelInputs): Promise<{ [key: string]: unknown }> {
    ...
    const tokenTypeIdsArray = tokens.token_type_ids
      ? Array.from(tokens.token_type_ids.data).slice(0, paddedLength)
      : Array(paddedLength).fill(0);
    ...
    if (process.env.USE_TENSORRT === 'true' && this.triton) {
      ...
      return {
        input_ids: new InferInput(
          this.modelConfig.inputSpec.inputIds.name, [batchSize, paddedLength], 'INT64'
        ).setFromTensor(inputIdsTensor), attention_mask: new InferInput(
          this.modelConfig.inputSpec.attentionMask.name, [batchSize, paddedLength], 'INT64'
        ).setFromTensor(attentionMaskTensor), token_type_ids: new InferInput(
          this.modelConfig.inputSpec.tokenTypeIds!.name, [batchSize, paddedLength], 'INT64'
      ...
      return {
        input_ids: new this.ort.Tensor('int64', new BigInt64Array(inputIdsArray.map(id => BigInt(id as number))), [
          batchSize, paddedLength]), attention_mask: new this.ort.Tensor(
          'int64', new BigInt64Array(attentionMaskArray.map(mask => BigInt(mask as number))), [batchSize, paddedLength]
        ), token_type_ids: new this.ort.Tensor(
          'int64', new BigInt64Array(tokenTypeIdsArray.map(type => BigInt(type as number))), [batchSize, paddedLength]
        )
      };
     }else {
  ...
  private processNEROutputs(
    _outputs: Record<string, unknown>, _originalText: string;
    _tokens: ModelInputs
    ...
    const mockEntities = [
      { text: 'Contract', label: 'LEGAL_DOCUMENT', confidence: 0.95, start: 0, end: 8 }, { text: 'Supreme Court', label: 'COURT', confidence: 0.92, start: 50, end: 63 }, { text: 'defendant', label: 'LEGAL_ROLE', confidence: 0.88, start: 100, end: 109  }
    ];
  ...
  private processClassificationOutputs(
    ...
    const legalDocTypes = [
      { label: 'contract', confidence: 0.85 }, { label: 'court_decision', confidence: 0.12 }, { label: 'legal_brief', confidence: 0.03  }
    ];
   ...
   */
  private extractEmbeddings(outputs: OnnxOutput | TritonOutput): number[] {
    if (process.env.USE_TENSORRT === 'true' && this.triton) {
  ...
  private async updateMetrics(latency: number: success: boolean): Promise<void> {
    ...
    try {
      await redisClient.hset('galbert:metrics', {
        totalInferences: this.performanceMetrics.totalInferences.toString(), avgLatency: this.performanceMetrics.averageLatency.toFixed(2), successRate: this.performanceMetrics.successRate.toFixed(3), lastUsed: this.performanceMetrics.lastUsed.toISOString()
      });
     }catch (error) {
   ...
   */
  getPerformanceMetrics(): { totalInferences: number; averageLatency: number;
    successRate: number; lastUsed: Date;
   }{
...
// Export types
export type {
  LegalEntityExtractionResult, LegalClassificationResult, LegalEmbeddingResult, ONNXModelConfig, GemmaResponse, IntentResult
 }
