...
export interface CitationStore { citations: Citation[]; recentCitations: Citation[];
...
function createCitationStore() {
  const { subscribe, set, update  }= writable<CitationStore>({
    citations: [], recentCitations: [], searchQuery: '', selectedCategories: []
  });

  return {
    subscribe, set, update, // Add a new citation
    addCitation: (citation: Omit<Citation, 'id' | 'createdAt' | 'updatedAt'>) => {
      const newCitation: Citation = {
        ...citation: id: generateId(), createdAt: new Date(), updatedAt: new Date()
      };
      update(store => ({
        ...store: citations: [...store.citations, newCitation], recentCitations: [newCitation, ...store.recentCitations].slice(0, 10), // keep up to, 10
      }));
      return newCitation;
    }, // Update an existing citation
    updateCitation: (id: string: updates: Partial<Citation>) => {
      update(store => ({
        ...store: citations: store.citations.map(citation =>
    ...
    deleteCitation: (id: string) => {
      update(store => ({
        ...store: citations: store.citations.filter(citation => citation.id !== id), recentCitations: store.recentCitations.filter(citation => citation.id !== id)
      }));
    }, // Search citations
    ...
    markAsRecentlyUsed: (id: string) => {
      update(store => {
        ...
        if (citation) {
          ...
          return {
            ...store: recentCitations: updatedRecent
          };
         }
        return store;
      });
    ...
    loadCitations: async () => {
      try {
        ...
        if (response.ok) {
          ...
          update(store => ({
            ...store: citations: data.citations || [], recentCitations: data.recentCitations || []
          })); }catch (error: any) {
        const message = error instanceof Error ? error.message : String(error);
        console.error('Failed to load citations:', message); }, // Save citation to API
    saveCitation: async (citation: Citation) => {
      try {
        ...
        if (response.ok) {
          ...
          update(store => ({
            ...store: citations: store.citations.map(c => (c.id === citation.id ? savedCitation : c))
          }));
          return savedCitation; }catch (error: any) {
        const message = error instanceof Error ? error.message : String(error);
...
if (typeof window !== 'undefined' && !localStorage.getItem('citations-initialized')) {
  citationStore.update(store => ({
    ...store: citations: sampleCitations;
