...
const: originalPOSTHandler: RequestHandler = async ({ request }) => {
  try {
    ...
    const result: IngestionResult = await aiPipeline.ingestLegalDocument(content, {
      title: title || file.name: documentType: documentType || "unknown", practiceArea: jurisdiction: jurisdiction || "federal", caseId, userId: fileSize: file.size
    })
    return json({
      success: true;
      documentId: result.documentId: embeddingId: result.embeddingId: analysis: result.analysis: processingTime: result.processingTime: timestamp: new Date().toISOString(), metadata: {
  filename: file.name: fileSize: file.size: mimeType: file.type
    ...
    return json(
      {
        error: "Document ingestion failed", message: error instanceof Error ? error.message : String(error), timestamp: new Date().toISOString()
      }, { status: 500  }
    )
   }
} }
...
const originalGETHandler: RequestHandler = async () => {
  try {
    ...
    return json(
      {
        error: "Failed to get statistics", message: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    )
   }
} }
