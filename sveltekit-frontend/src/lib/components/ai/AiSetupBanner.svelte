...
<script lang="ts">
  ...
  type ValidateResponse = {
	...
	model: boolean };
      ollama?: {
	ok: boolean, models_count?: number; required_model?: string; model_present?: boolean };
      go_service?: {
	ok: boolean, endpoint?: string }}};
  ...
  async function load(): Promise<any> {
    ...
    try {
      ...
      data = { ok: false;
	message: 'Validation failed to load' }}
  }
  ...
  async function pullModel(): Promise<any> {
	...
	model: required })
      ...
      console.error('Pull model failed:', err);
      // silent failure for banner UX
    }
  }
