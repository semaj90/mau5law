<!--
  ...
  - LOD 3, Low detail (256x256 texture) - N64 style pixelated: Features, - WebGPU texture streaming for PDF pages
  ...
  async function loadPageWithLOD(pageNumber: number; lodLevel: number): Promise<void> {
    ...
    try {
      ...
      if (!page) {
        page = {
          pageNumber,
          textContent: pageData.textContent || ''; annotations: pageData.annotations || [],
          lodTextures: new Map(); currentLOD: lodLevel}
  ...
  <div class="document-controls">
    ...
    <div class="view-controls">
      <LoadingButton onclick={handleZoomIn} variant="ghost" size="sm">
        {#snippet children()}<ZoomIn class="w-4" />{/snippet}
      </LoadingButton>
      <span class="zoom-info">
        {Math.round(zoomLevel * 100)}%
      </span>
      <LoadingButton onclick={handleZoomOut} variant="ghost" size="sm">
        {#snippet children()}<ZoomOut class="w-4" />{/snippet}
      </LoadingButton>
      <LoadingButton onclick={handleRotate} variant="ghost" size="sm">
        {#snippet children()}<RotateCw class="w-4" />{/snippet}
      </LoadingButton>
    </div>
    ...
    {#if isLoading}
      <div class="loading-overlay">
        <div class="nes-progress">
          <div class="nes-progress-bar"></div>
        </div>
...
<style>
  ...
  .document-canvas-container {
    position: relative, height: 500px, background: #2a2a3, border: 2px solid #444
