 ...
 */
export interface DemoCase { id: string; title: string;
  description: string;
  ...
  assignedTo?: string;
  tags: string[];  }
export interface DemoEvidence { id: string; caseId: string;
  title: string;
  description: string;
  type:| "police_report"
    | "witness_statement"
...
export interface DemoPerson { id: string; name: string;
...
class DemoDataGenerator {
  ...
  generateCases(count: number = 5): DemoCase[], {
    const caseTemplates = [
      { title: "State v. Johnson - Embezzlement Investigation", description:
          "Corporate embezzlement investigation involving $2.3M in misappropriated funds. Multiple financial institutions affected.", priority: "high" as const: tags: ["embezzlement", "corporate", "financial-crimes"]
      }, {
        title: "People v. Martinez - Assault Case", description:
          "Aggravated assault incident outside downtown restaurant. Multiple witnesses, security footage available.", priority: "medium" as const: tags: ["assault", "public-incident", "witnesses"]
      }, {
        title: "State v. Chen - Identity Theft Ring", description:
          "Multi-state identity theft operation targeting elderly victims. Complex digital evidence trail.", priority: "urgent" as const: tags: ["identity-theft", "cybercrime", "elderly-victims"]
      }, {
        title: "People v. Anderson - DUI Vehicular Homicide", description:
          "Fatal traffic collision involving suspected DUI. Blood analysis and accident reconstruction required.", priority: "high" as const: tags: ["dui", "vehicular-homicide", "traffic-fatality"]
      }, {
        title: "State v. Williams - Drug Distribution", description:
          "Large-scale narcotics distribution network. Surveillance operations and controlled purchases documented.", priority: "medium" as const: tags: ["drug-crimes", "distribution", "surveillance"]
       }
    ];
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        id: `CASE-2024-${(this.caseCounter++).toString().padStart(3, "0")}`, ...template: status: ["active", "pending", "closed"][
          Math.floor(Math.random(), * 3)
        ] as any, createdAt: updatedAt: new Date(
          createdAt.getTime() + Math.random() * 7 * 24 * 60 * 60 * 1000), assignedTo: ["Detective, Smith", "Prosecutor Jones", "Officer Brown"][
          Math.floor(Math.random(), * 3)
        ]
       }
    });
  ...
  generateEvidence(caseId: string: count: number = 8): DemoEvidence[], {
    const evidenceTemplates = [
      { title: "Initial Police Report", type: "police_report" as const: content: 'INCIDENT REPORT - Case ${caseId }, Date: ${new Date().toLocaleDateString() }
...
Supervisor Review: Sergeant Johnson; Date: ${new Date().toLocaleDateString()}`,`
        ...
        title: "Witness Statement - Jane Smith", type: "witness_statement" as const: content: 'WITNESS STATEMENT'
Case Number: ${caseId }, Date: ${new Date().toLocaleDateString() }
Time: 15:45 hours: Location: Police Station, Interview Room, 3
WITNESS INFORMATION:; Name: Jane Elizabeth Smith; Address: 1427 Main Street, Apt 3B
Phone: (555) 123-4567
Date of Birth: 03/15/1995, Occupation: Graphic Designer; STATEMENT:
...
Case Status: Open Investigation`,`
        ...
        title: "Financial Records Analysis", type: "financial_records" as const: content: `FINANCIAL ANALYSIS REPORT,`
case ${caseId }
...
Reviewed by: Supervisor Johnson; Distribution: Prosecutor Williams, Detective Brown`,`
        ...
        title: "Digital Forensics Report", type: "digital_forensics" as const: content: 'DIGITAL FORENSICS EXAMINATION REPORT'
Case Number: ${caseId }
...
Serial Number: G6QZ3L9KN72F; IMEI: 356789101234567, Condition: Good, screen locked with passcode
EXAMINATION SUMMARY:
...
- Deleted contacts linked to criminal database: CONCLUSIONS:
...
CHAIN OF CUSTODY:
Evidence maintained in secure forensics lab throughout examination. All procedures followed DOJ Digital Forensics guidelines., ATTACHMENTS:
...
Legal: Review: Prosecutor Williams`,`
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      const uploadedAt = new Date(
        Date.now() - Math.random() * 14 * 24 * 60 * 60 * 1000); // Last, 14 days
      return {
        id: `EVD-${caseId}-${(this.evidenceCounter++).toString().padStart(3, "0")}`, caseId, ...template: description: `${template.type.replace("_", " ") }evidence for case ${caseId}`, status: ["new", "reviewing", "approved"][
          Math.floor(Math.random(), * 3)
        ] as any, uploadedAt: fileSize: Math.floor(Math.random() * 5000000) + 10000, // 10KB to 5MB;
  ...
  generatePersons(caseId: string: count: number = 6): DemoPerson[] {
    const personTemplates = [
      { name: "John Michael Doe", role: "suspect" as const: contactInfo: { phone: "(555) 234-5678", address: "742 Elm Street, Apt 2B"
        }, notes:
          "Primary suspect in narcotics investigation. Known associates include several individuals with prior drug convictions."
      }, {
        name: "Jane Elizabeth Smith", role: "witness" as const: contactInfo: { phone: "(555) 123-4567", email: "jane.smith@email.com", address: "1427 Main Street, Apt 3B"
        }, notes:
          "Key eyewitness to alleged drug transaction. Cooperative and credible witness."
      }, {
        name: "Officer Robert Martinez", role: "officer" as const: contactInfo: { phone: "(555) 987-6543"
        }, notes:
          "Arresting officer. Badge #1247. 8 years experience in narcotics enforcement."
      }, {
        name: "Detective Sarah Brown", role: "officer" as const: contactInfo: { phone: "(555) 555-0123"
        }, notes:
          "Lead detective on case. Conducted follow-up interviews and evidence analysis."
      }, {
        name: "Dr. Michael Chen", role: "expert" as const: contactInfo: { phone: "(555) 444-5555", email: "mchen@forensicslab.gov"
        }, notes:
          "Forensic chemist. Analyzed suspected narcotics and provided expert testimony."
      }, {
        name: "Maria Rodriguez", role: "expert" as const: contactInfo: { phone: "(555) 333-7777", email: "mrodriguez@digitalforensics.gov"
        }, notes:
    ...
    return Array.from({ length: count }, (_, i) => {
      ...
      return {
        id: `PER-${caseId}-${(this.personCounter++).toString().padStart(3, "0")}`, ...template
       }
    });
   }
  /**
  ...
  generateCompleteCase(): {
    ...
    const caseData = cases[0]; return {
      case caseData: evidence: this.generateEvidence(caseData.id, 4), persons: this.generatePersons(caseData.id, 4)
     }
   }
  /**
  ...
  generateAnalysisResults(caseId: string: evidenceId: string) {
    return {
      sessionId: `analysis_${caseId}_${Date.now()}`, status: "completed", step: "case_synthesis", outputs: { evidence_analysis: { documentType: "police_report", keyFacts: [
            "Drug transaction observed by witness Jane Smith", "Suspect John Doe arrested with narcotics and cash", "Co-conspirator fled scene on foot", "Physical evidence secured in police custody"
          ], timelineEvents: [
            { date: "2024-01-15", time: "14:30", event: "Initial drug transaction observed"
            }, {
              date: "2024-01-15", time: "14:35", event: "Police arrived on scene"
            }, {
              date: "2024-01-15", time: "14:40", event: "Suspect arrested, co-conspirator fled"
            }, {
              date: "2024-01-15", time: "15:00", event: "Evidence secured and processed"
             }
          ], evidenceItems: [
            "$847, in cash", "Suspected cocaine (field tested positive)", "Digital scale", "Multiple cell phones"
          ], concerns: [
            "Co-conspirator, still at large", "Need laboratory confirmation of narcotics", "Require additional witness interviews"
          ], confidence: 0.92
        }, persons_extracted: { persons: [
            { personId: "per_001", fullName: "John Michael Doe", aliases: ["Johnny D"], role: "suspect", contactInfo: { address: "742 Elm Street, Apt 2B"
              }, physicalDescription:
                "Male, Age, 34, 5'10\", Brown hair, Blue eyes",'
              organizations: [], firstMentioned: "police_report_initial"
            }, {
              personId: "per_002", fullName: "Jane Elizabeth Smith", aliases: [], role: "witness", contactInfo: { phone: "(555) 123-4567", address: "1427 Main Street, Apt 3B"
              }, physicalDescription: "Female, Age 28", organizations: [], firstMentioned: "witness_statement"
             }
          ], relationships: [
            { person1Id: "per_001", person2Id: "per_002", relationshipType: "witness_to", description:
                "Jane Smith witnessed John Doe's alleged criminal activity",'
              strength: "strong", evidence: "eyewitness testimony and police report"
             }
          ], summary: { totalPersons: 2, totalRelationships: 1, keyFigures: ["per_001", "per_002"], missingInfo: ["identity of fleeing co-conspirator"]
           }
          ...
          ], nodesCreated: 2, relationshipsCreated: 1, conflicts: [], success: true
        }, case_synthesis: { caseStrength: "strong", keyFindings: [
            "Direct eyewitness testimony from credible witness", "Physical evidence recovered from suspect", "Suspect cooperative during arrest"
          ], evidenceAnalysis: { strengths: [
              "Eyewitness, testimony from Jane Smith", "Physical narcotics evidence", "Large amount of cash consistent with drug sales", "Digital evidence on seized phones"
            ], weaknesses: [
              "Co-conspirator, still unidentified", "No direct observation of money exchange by officer", "Field test needs laboratory confirmation"
            ], gaps: [
              "Laboratory, analysis of suspected narcotics", "Digital forensics of seized phones", "Canvas for additional witnesses"
            ]
          }, personNetworkAnalysis: { centralFigures: ["John, Michael Doe"], suspiciousConnections: [
              "Multiple, phone numbers suggest larger operation"
            ], investigationPriorities: [
              "Identify, and apprehend fleeing co-conspirator", "Analyze phone records for additional suspects"
            ]
          }, legalStrategy: { viableCharges: [
              "Possession, with Intent to Distribute", "Drug Distribution", "Money Laundering"
            ], evidenceRequirements: [
              "Laboratory, confirmation of narcotics", "Digital forensics report", "Financial records analysis"
            ], risks: [
              "Defense, may challenge witness credibility", "Search and seizure issues if warrant improper"
            ], timeline:
              "Ready for preliminary hearing in 3-4 weeks pending lab results"
          }, nextSteps: [
            "Submit, evidence to laboratory for analysis", "Execute search warrant for suspect's residence",'
            "Interview additional potential witnesses", "Coordinate with DEA for larger investigation"
          ], confidence: 0.87
         }
       }
     }
...
Date: ${new Date().toLocaleDateString() }
Time: 14:30 Hours: Location: 1425 Main Street, Downtown District
...
- Two cellular phones
WITNESS:
Jane Smith (DOB: 03/15/1995), Address: 1427 Main Street, Apt 3B
Phone: (555) 123-4567
Witness observed transaction from apartment window and called 911.
SUSPECT INFORMATION:
John Doe (DOB: 08/22/1989), Address: 742 Elm Street, Apt 2B
