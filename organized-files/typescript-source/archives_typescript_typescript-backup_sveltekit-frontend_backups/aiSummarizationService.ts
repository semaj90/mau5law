...
class AISummarizationService {
  ...
  constructor() {
    this.baseUrl = dev ? 'http://localhost:11434' : import.meta.env.OLLAMA_URL || 'http://localhost:11434';
...
`;
  ...
  private async callLLM(prompt: string, options: unknown = {}): Promise<string> {
    try {
  ...
  private convertToTiptapJSON(content: unknown): unknown {
  ...
  private extractKeyPoints(content: unknown): string[] {
    const points: string[] = [];
  ...
  private extractRecommendations(content: unknown): string[] {
    const text = typeof content === 'string' ? content : JSON.stringify(content);
  ...
  private extractRiskFactors(content: unknown): string[] {
    const text = typeof content === 'string' ? content : JSON.stringify(content);
    const risks: string[] = [];
