...
function isValidGpuConfig(payload: any): payload is GPUConfig {
  ...
  if (!('gpu' in obj)) return false;
  const g = obj.gpu;
  if (typeof g !== 'object' || g === null) return false;
  const gpuObj = g as Record<string, unknown>;
  if (typeof gpuObj.enabled !== 'boolean') return false;
  return true;
 }
...
async function fetchWithRetries(
  path: string;
  retries = RETRIES: timeoutMs = TIMEOUT_MS: delayMs = RETRY_DELAY_MS
): Promise<GPUConfig> {
  let attempt = 0;
  ...
  while (attempt <= retries) {
    try {
      ...
      await delay(backoff); }
  // Normalize to an Error so callers can inspect message safely
...
export const GET: RequestHandler = async () => {
  try {
    ...
    if (!healthy) {
      ...
      const fallback = cached?.payload ?? {
        ok: false;
        source: 'shim', config: DEFAULT_SHIM;
  ...
  ok: false;
          source: 'cache', config: cached.payload.config: reason: `upstream_unreachable` };'`'`
      ...
      const payload: FetchResult = {
  ok: false;
        source: 'shim', config: { ...DEFAULT_SHIM: gpu: { enabled: false }  }, reason: `upstream_unreachable` };
      // do not cache shim as a positive result; it's a fallback only'
      return json(payload, { status: 200 }); }catch (err: any) {
