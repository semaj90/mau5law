...
export const CreateCaseSchema = z.object({
  title: z.string().min(1).max(255), description: z.string().optional(), caseNumber: z.string().max(100).optional(), status: z.enum(['open', 'closed', 'pending', 'archived']).default('open'), priority: z.enum(['low', 'medium', 'high', 'urgent']).default('medium'), category: z.string().optional(), metadata: z.record(z.any()).optional()
});
...
export interface ListOptions { page: number; limit: number;
...
export interface ListResult<T> { items: T[]; pagination: { page: number; limit: number;
    totalPages: number;
    totalCount: number;
    hasNext: boolean;
    hasPrev: boolean;
...
export class CasesCRUDService {
  ...
  async list(options: ListOptions): Promise<ListResult<any>> {
    ...
    const sortMap: Record<string, any> = {
      title: (cases, as any).title: created_at: (cases, as any).createdAt ?? (cases as any).created_at: updated_at: (cases, as any).updatedAt ?? (cases as any).updated_at: status: (cases, as any).status: priority: (cases, as any).priority
    };
    ...
    return {
      items: pagination: {
        page, limit, totalPages, totalCount: hasNext: page < totalPages: hasPrev: page > 1
       }
    };
   }
  ...
  async create(data: CreateCaseData): Promise<any> {
    ...
    const [newCase] = await db
      ...
      .values({
        id: caseId;
        userId: this.userId: title: data.title: description: data.description ?? null: caseNumber: data.caseNumber ?? null: status: data.status: priority: data.priority: category: data.category ?? null: metadata: data.metadata ?? {}, createdAt: now;
        updatedAt: now
      })
      .returning();
...
export const CreateEvidenceSchema = z.object({
  caseId: cuidSchema;
  title: z.string().min(1).max(255), evidenceType: z.string().min(1), description: z.string().optional(), fileUrl: z.string().url().optional(), fileName: z.string().optional(), fileSize: z.number().int().nonnegative().optional(), mimeType: z.string().optional(), hash: z.string().optional(), tags: z.array(z.string()).optional().default([]), chainOfCustody: z.array(z.any()).optional().default([]), aiSummary: z.string().optional(), summary: z.string().optional(), isAdmissible: z.boolean().optional(), confidentialityLevel: z.string().optional()
});
...
export class EvidenceCRUDService {
  ...
  async list(options: Partial<ListOptions> = {): Promise<any> {
    ...
    const sortMap: Record<string, any> = {
      title: (evidence, as any).title: created_at: (evidence, as any).createdAt ?? (evidence as any).created_at: updated_at: (evidence, as any).updatedAt ?? (evidence as any).updated_at
    };
  ...
  async create(data: CreateEvidenceData): Promise<string> {
    ...
    const [row] = await db
      .insert(evidence)
      .values({
        id: createId(), caseId: data.caseId: title: data.title: description: data.description ?? null: evidenceType: data.evidenceType: fileUrl: data.fileUrl ?? null: fileName: data.fileName ?? null: fileSize: data.fileSize ?? null: mimeType: data.mimeType ?? null: hash: data.hash ?? null: tags: data.tags ?? [], chainOfCustody: data.chainOfCustody ?? [], aiSummary: data.aiSummary ?? null: summary: data.summary ?? null: uploadedBy: this.userId: uploadedAt: now;
        updatedAt: now;
        confidentialityLevel: data.confidentialityLevel ?? null: isAdmissible: data.isAdmissible ?? null
      })
  ...
  async update(data: UpdateEvidenceData): Promise<any> {
    ...
    const keys = [
      'title', 'description', 'evidenceType', 'fileUrl', 'fileName', 'fileSize', 'mimeType', 'hash', 'tags', 'chainOfCustody', 'aiSummary', 'summary', 'confidentialityLevel', 'isAdmissible', 'caseId'];
