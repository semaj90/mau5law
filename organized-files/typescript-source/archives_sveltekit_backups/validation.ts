...
export function validateField(value: unknown, config: FormFieldConfig): ValidationResult {
  ...
  if (config.custom) {
    for (const rule of config.custom) {
      ...
      const validationResult = typeof result === 'boolean' 
        ? createValidationResult(result, result ? [] : [rule.message || `${rule.name} validation failed`])
        : result;
  ...
  const config: CaseValidationConfig = {
    ...
    status: { 
      ...
      custom: [
        {
          name: 'valid-status',
          validate: (value: string) => ['Open', 'In-Progress', 'Closed', 'Archived'].includes(value),
...
export class FormValidator {
  ...
  setValue(name: string, value: unknown): ValidationResult {
...
export function validateObject(obj: unknown, schema: Record<string, FormFieldConfig>): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];
