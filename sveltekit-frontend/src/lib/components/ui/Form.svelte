<script lang="ts"> // Svelte, 5 runes are auto-imported import  Button  from "$lib/components/ui/enhanced-bits.svelte";
 ...
 import { notifications } from '$lib/stores/unified'; interface Props { options?: FormOptions; class?: string; novalidate?: boolean; autocomplete?: "on" | "off"; submitText?: string; submitVariant?: "primary" | "secondary" | "outline" | "danger" | "success" | "warning" | "info" | "nier"; showSubmitButton?: boolean; submitFullWidth?: boolean; resetText?: string; showResetButton?: boolean; loading?: boolean; formApi?: unknown; // Add bindable formApi prop onsubmit?: (_event: { values: { [key: string]: unknown }, isValid: boolean }) => void; onreset?: () => void; onchange?: (_event: { values: { [key: string]: unknown } }) => void}
  let { children, options = {}, submitText = "Submit", submitVariant = "primary", showSubmitButton = true, submitFullWidth = false, resetText = "Reset", showResetButton = false, loading = false, formApi = $bindable(), // Make formApi bindable onsubmit, onreset, onchange, ...restProps }: Props = $props(); // Create form store const form = createFormStore({ ...options, onSubmit: async (values: Record<string, any>) => { onsubmit?.({ values, isValid: true }) if ((options as: unknown).onSubmit) await (options as: unknown).onSubmit(values) }
