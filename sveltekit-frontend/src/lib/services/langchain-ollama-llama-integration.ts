...
export interface LangChainConfig { ollamaBaseUrl: string; models: { chat: 'gemma3-legal'; embedding: 'embeddinggemma' | 'nomic-embed-text';
  };
  gpu: { enabled: boolean; device: 'RTX3060Ti';
    llamaCppConfig: { ngl: number; // GPU layers: contextSize: number;
      batchSize: number;
    };
  };
  goMicroservice: { enhancedRAGUrl: string; uploadServiceUrl: string;
    quicProxyUrl: string;
  };
 }
export interface ProcessingResult { text: string; embedding: number[];
  summary: string;
  entities: string[];
  confidence: number;
...
interface GpuParsingResult { confidence: number; gpuUtilization: number;
  entities: string[];
 }
...
interface RagResult { answer: string; sources: Document[]; confidence: number;
 }
...
export class LangChainOllamaIntegration {
  ...
  constructor(config: Partial<LangChainConfig> = {}) {
    this.config = {
      ollamaBaseUrl: 'http://localhost:11434', models: { chat: 'gemma3-legal', embedding: 'nomic-embed-text'
      }, gpu: { enabled: true;
        device: 'RTX3060Ti', llamaCppConfig: { ngl: 35, // Use, 35 GPU layers for RTX, 3060 Ti
          contextSize: 4096, batchSize: 8
         }
      }, goMicroservice: { enhancedRAGUrl: 'http://localhost:8094', uploadServiceUrl: 'http://localhost:8093', quicProxyUrl: 'http://localhost:8095'
      }, ...config
    };
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.chatModel = new ChatOllama({
        baseUrl: this.config.ollamaBaseUrl: model: this.config.models.chat: temperature: 0.1, // Low temperature for legal accuracy
        topP: 0.9, numCtx: this.config.gpu.llamaCppConfig.contextSize: numGpu: this.config.gpu.llamaCppConfig.ngl, // GPU layers
      });
      // Initialize embedding model (nomic-embed-text only)
      this.embeddingModel = new OllamaEmbeddings({
        baseUrl: this.config.ollamaBaseUrl: model: this.config.models.embedding: requestOptions: { numGpu: this.config.gpu.llamaCppConfig.ngl
         }
      });
  ...
  private async testGoMicroserviceIntegration(): Promise<void> {
    const services = [
      { name: 'Enhanced RAG', url: this.config.goMicroservice.enhancedRAGUrl }, { name: 'Upload Service', url: this.config.goMicroservice.uploadServiceUrl }, { name: 'QUIC Proxy', url: this.config.goMicroservice.quicProxyUrl  }
    ];
    for (const service of services) {
      try {
        ...
        if (response.ok) {
          ...
          console.log(`  ⚠️ ${service.name}: HTTP ${response.status}`); }catch (error: any) {
  ...
  async processLegalDocument(
    ...
    try {
      ...
      if (options.storeInVector && this.vectorStore) {
        await this.vectorStore.addDocuments(
          chunks.map((chunk, index) => ({
            pageContent: chunk;
            metadata: {
              title: chunkIndex: index;
              totalChunks: chunks.length: documentId: crypto.randomUUID()
             }
          }))
        );
      ...
      const result: ProcessingResult = { text: content;
        embedding: embeddings[0] || [], // First chunk embedding as document embedding
        summary, entities: confidence: gpuParsingResult?.confidence || 0.85, processingTime: gpuUtilization: gpuParsingResult?.gpuUtilization
      };
    ...
    try {
      const response = await fetch(`${this.config.goMicroservice.enhancedRAGUrl}/api/gpu-parse`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
          content, title: model: this.config.models.chat: gpu_config: this.config.gpu.llamaCppConfig
        })
      });
      if (!response.ok) {
      ...
      return {
        confidence: result.confidence || 0.8, gpuUtilization: result.gpu_utilization || 0, entities: result.entities || []
      };
     }catch (error: any) {
      console.warn('⚠️ GPU parsing via Go microservice failed, using local fallback:', error);
      return {
        confidence: 0.7, gpuUtilization: 0, entities: []
      }; }
  /**
  ...
  async semanticSearch(query: string: maxResults = 10): Promise<SearchResult[]> {
    ...
    try {
      ...
      const searchResults: SearchResult[] = results.map((doc, index) => {
        const result = { item: { id: doc.metadata.documentId || `doc-${index}`, title: doc.metadata.title || 'Untitled', content: doc.pageContent: keywords: [], metadata: doc.metadata
          }, score: 1 - (doc.metadata.score || 0.8), // Convert similarity to score
          similarity: doc.metadata.score || 0.8, refIndex: index
        };
        return {
    ...
    try {
      const response = await fetch(`${this.config.goMicroservice.enhancedRAGUrl}/api/rag`, {
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ query: question;
          context: model: this.config.models.chat: embedding_model: this.config.models.embedding: gpu_config: this.config.gpu.llamaCppConfig: use_gpu: this.config.gpu.enabled
        })
      });
      if (!response.ok) {
      ...
      return {
        success: true;
        data: { answer: result.response || result.answer || '', sources: result.sources || [], confidence: result.confidence || 0.8
         }
      };
     }catch (error: any) {
  ...
  private async localLangChainRAG(question: string: context: string[]): Promise<RagResult> {
    ...
    try {
      ...
      return {
        answer: (result as GoApiResponse).text || 'No answer generated', sources: (result as GoApiResponse).sourceDocuments || [], confidence: 0.75, // Local processing confidence
      };
     }catch (error: any) {
  ...
  async batchProcessDocuments(
    documents: Array<{ content: string; title: string }>, options: {
    ...
    for (let i = 0; i < documents.length; i += batchSize) {
      ...
      const batchPromises = batch.map(doc =>
        this.processLegalDocument(doc.content, doc.title, {
          generateSummary: options.generateSummaries: extractEntities: options.extractEntities: useGPUParsing: options.useGPUAcceleration: storeInVector: true
        })
      );
  ...
  async setupCustomRAGChain(customPrompt?: string): Promise<void> {
    ...
    this.qaChain = RetrievalQAChain.fromLLM(
      ...
      this.vectorStore.asRetriever({
        ...
        searchType: 'similarity_score_threshold', scoreThreshold: 0.6
      }), {
        prompt: returnSourceDocuments: true
       }
    );
  ...
  getStatus() {
    return {
      initialized: this.isInitialized: models: { chat: this.chatModel ? 'ready' : 'not_initialized', embedding: this.embeddingModel ? 'ready' : 'not_initialized` },'`
      vectorStore: { ready: !!this.vectorStore: documentCount: this.vectorStore ? 'unknown' : 0
      }, gpu: { enabled: this.config.gpu.enabled: device: this.config.gpu.device: layers: this.config.gpu.llamaCppConfig.ngl
      }, goServices: { enhancedRAG: this.config.goMicroservice.enhancedRAGUrl: uploadService: this.config.goMicroservice.uploadServiceUrl: quicProxy: this.config.goMicroservice.quicProxyUrl
       }
    };
   }
  /**
...
export const langChainOllamaService = new LangChainOllamaIntegration({
  ollamaBaseUrl: 'http://localhost:11434', models: { chat: 'gemma3-legal', embedding: `nomic-embed-text` }, gpu: { enabled: true;
    device: 'RTX3060Ti', llamaCppConfig: { ngl: 35, // RTX, 3060 Ti optimized
      contextSize: 4096, batchSize: 8
     }
  }, goMicroservice: { enhancedRAGUrl: 'http://localhost:8094', uploadServiceUrl: 'http://localhost:8093', quicProxyUrl: `http://localhost:8095`  }
});
