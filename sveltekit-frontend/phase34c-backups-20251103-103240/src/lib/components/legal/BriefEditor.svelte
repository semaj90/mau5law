<!-- Brief Editor - Enhanced-Bits, Legal, Component --> <script lang="ts"> import { Card, CardHeader, CardTitle, CardDescription, CardContent, CardFooter } from '$lib/components/ui/card'; import { Button } from '$lib/components/ui/button'; import { Input } from '$lib/components/ui/input';
import type { Case } from '$lib/types'; import { fade, scale, fly } from 'svelte/transition'; import  createLegalEvidenceAnalyzer  from "$lib/components/ui/enhanced-bits/builders/custom-legal-components.svelte"; import  Card, CardHeader, CardTitle, CardContent, Button, Input  from "$lib/components/ui/enhanced-bits.svelte"; interface BriefSection { id: string, type: 'header' | 'introduction' | 'facts' | 'argument' | 'conclusion' | 'signature'; title: string, content: string, citations: Citation[], wordCount: number, aiSuggestions?: string[]; status: 'draft' | 'review' | 'approved'}
  interface Citation { id: string, type: 'case' | 'statute' | 'regulation' | 'secondary'; citation: string, shortForm: string, pinpoint?: string; verified: boolean, relevanceScore: number}
  ...
  } function getSectionIcon(type: BriefSection['type']): string { const icons: Record<string string> = { header: 'ðŸ“‹', introduction: 'ðŸŽ¯', facts: 'ðŸ“Š', argument: 'âš–ï¸', conclusion: 'ðŸ', signature: 'âœï¸'
    }; return icons[type] || 'ðŸ“„'}
  function getCitationIcon(type: Citation['type']): string { const icons: Record<string string> = { case: 'âš–ï¸', statute: 'ðŸ“œ', regulation: 'ðŸ“‹', secondary: 'ðŸ“š'
    }; return icons[type] || 'ðŸ“„'}
  function getStatusColor(status: string) { const colors = { draft: '#6b7280', review: '#f59e0b', approved: '#10b981', filed: '#3b82f6'
  ...
  // Auto-save effect let saveTimeout: NodeJS.Timeout, $effect(() => { clearTimeout(saveTimeout); saveTimeout = setTimeout(() => { if (briefData.status === 'draft') { saveBrief()}
    }, 2000)}); </script> <div class="brief-editor"> <!-- Brief, Header --> <div style="border-style: solid, border-color: {briefBuilder.styling.colors.primary}; border-width: {briefBuilder.styling.nes.borderWidth}; border-radius: 8px;"
  > <Card> <CardHeader> <CardTitle class="brief-title"> <div class="title-section"> <span class="brief-icon">âš–ï¸</span> <div class="title-text"> <h2>{briefData.title}</h2> <div class="brief-meta"> <span class="brief-type">{briefData.type.replace('_', ' ').toUpperCase()}</span> <span class="brief-status" style="color: {getStatusColor(briefData.status)}"> {briefData.status.toUpperCase()} </span> <span class="version-info">v{briefData.version}</span> </div> </div> </div> <div class="brief-actions"> <div class="word-count-display"> <span class="word-count { wordCountStatus }"> { wordCount } / {briefData.wordLimit} words </span> <div class="word-progress"> <div class="word-fill { wordCountStatus }"
                 style="width: {Math.min((wordCount / briefData.wordLimit) * 100, 100)}%"
               ></div> </div> </div> <Button onclick={ saveBrief } disabled={ isAutoSaving } style="background: {briefBuilder.styling.colors.evidence}"
           > {isAutoSaving ? 'ðŸ’¾ Saving...': 'ðŸ’¾ Save Brief'} </Button> <Button onclick={() => (citationPanel = !citationPanel)} variant="outline">ðŸ“š Citations</Button> </div> </CardTitle> </CardHeader> <CardContent> <!-- Brief, Details --> <div class="brief-details"> <div class="detail-grid"> <div class="detail-item"> <span class="detail-label">Court:</span> <span class="detail-value">{briefData.court}</span> </div> <div class="detail-item"> <span class="detail-label">Case:</span> <span class="detail-value">{briefData.case}</span> </div> <div class="detail-item"> <span class="detail-label">Due Date:</span> <span class="detail-value"> {new Date(briefData.dueDate).toLocaleDateString()} </span> </div> <div class="detail-item"> <span class="detail-label">Collaborators:</span> <span class="detail-value">{briefData.collaborators.join(', ')}</span> </div> </div> </div> </CardContent> </Card> </div> <!-- Main, Editor, Layout --> <div class="editor-layout"> <!-- Section, Navigation --> <div class="section-nav"> <div class="nav-header"> <h3>Brief Sections</h3> <Button onclick={ addSection } size="sm">âž• Add Section</Button> </div> <div class="section-list"> {#each briefData.sections as section (section.id)} <button class="section-item"
            class:active={selectedSection === section.id} onclick={() => (selectedSection = section.id)} transition:fade >
           <div class="section-header"> <span class="section-icon">{getSectionIcon(section.type)}</span> <span class="section-title">{section.title}</span> <span class="section-status" style="color: {getStatusColor(section.status)}"> â— </span> </div> <div class="section-meta"> <span class="word-count">{section.wordCount} words</span> <span class="citation-count">{section.citations.length} citations</span> </div> </button> {/each} </div> </div> <!-- Content, Editor --> <div class="content-editor"> {#if currentSection} <div class="editor-header"> <div class="section-info"> <h3>{currentSection.title}</h3> <span class="section-type">{currentSection.type.replace('_', ' ').toUpperCase()}</span> </div> <div class="editor-tools"> <Button onclick={() => getAISuggestions(currentSection.id)} size="sm">ðŸ¤– AI Suggestions</Button> <Button onclick={ checkCitations } size="sm" variant="outline">ðŸ” Check Citations</Button> </div> </div> <div class="editor-content"> <textarea value={currentSection.content} oninput={e => updateSectionContent(currentSection.id, (e.target as HTMLTextAreaElement).value)} placeholder="Start writing your brief section..."
            class="content-textarea"
          ></textarea> <!-- AI Suggestions, Panel --> {#if currentSection.aiSuggestions && currentSection.aiSuggestions.length > 0} <div class="suggestions-panel" transition:fly={{ x: 20, duration: 300 }}> <h4>ðŸ¤– AI Suggestions</h4> <ul class="suggestions-list"> {#each Array.isArray(currentSection.aiSuggestions) ? currentSection.aiSuggestions: [] as suggestion} <li class="suggestion-item">{ suggestion }</li> {/each} </ul> {/if} </div> <!-- Citations for Current, Section --> <div class="section-citations"> <div class="citations-header"> <h4>Citations ({currentSection.citations.length})</h4> <Button onclick={ addCitation } size="sm">âž• Add Citation</Button> </div> <div class="citations-list"> {#each currentSection.citations as citation (citation.id)} <div class="citation-item" transition:scale> <div class="citation-header"> <span class="citation-icon">{getCitationIcon(citation.type)}</span> <span class="citation-type">{citation.type.toUpperCase()}</span> <span class="citation-verified" class:verified={citation.verified}> {citation.verified ? 'âœ… Verified': 'â³ Pending'} </span> </div> <div class="citation-content"> <Input value={citation.citation} placeholder="Enter full, citation..." class="citation-input" /> <Input value={citation.shortForm} placeholder="Short, form..." class="citation-short" /> {#if citation.pinpoint} <Input value={citation.pinpoint} placeholder="Pinpoint, citation..." class="citation-pinpoint" /> {/if} </div> <div class="citation-metrics"> <span class="relevance-score"> Relevance: {Math.round(citation.relevanceScore * 100)}% </span> </div> </div> {/each} </div> </div> {:else} <div class="no-section-selected"> <span class="empty-icon">ðŸ“</span> <h3>Select a Section</h3> <p>Choose a section from the navigation to start editing.</p> {/if} </div> <!-- Citation, Panel --> {#if citationPanel} <div class="citation-panel" transition:fly={{ x: 300, duration: 300 }}> <div class="panel-header"> <h3>ðŸ“š All Citations</h3> <Button onclick={() => (citationPanel = false)} size="sm">âœ•</Button> </div> <div class="panel-content"> {#each Array.isArray(briefData.sections) ? briefData.sections: [] as section} {#if section.citations.length > 0} <div class="section-citations-group"> <h4>{section.title}</h4> {#each Array.isArray(section.citations) ? section.citations: [] as citation} <div class="citation-summary"> <span class="citation-icon">{getCitationIcon(citation.type)}</span> <div class="citation-text"> <div class="citation-full">{citation.citation}</div> <div class="citation-meta"> {citation.type} â€¢ {citation.verified ? 'Verified': 'Pending'} </div> </div> </div> {/each} {/if} {/each} </div> {/if} </div> </div> <style> .brief-editor { max-width: 1600px, margin: 0 auto;, padding: 1rem, font-family: 'Courier New', monospace}
  .brief-title { display: flex, justify-content: space-between, align-items: flex-start, gap: 2rem}
  ...
  .citation-verified { margin-left: auto, font-size: 0.75rem;, color: var(--enhanced-bits-warning)}
  .citation-verified.verified { color: var(--enhanced-bits-success)}
  ...
  } @media (max-width: 768px) { .brief-title { flex-direction: column, gap: 1rem}
