...
export interface UserPattern {
	...
	timePatterns: {
		mostActiveHours: number[], averageSessionLength: number, queriesPerSession: number;
	};
...
interface StoreAiChatParams {
	...
	async createRagSession(params: CreateRagSessionParams): Promise<string> {
		try {
			const [insertedSession] = await db
				...
				.returning({ id: ragSessions.id }, return insertedSession.id;
		} catch (error) {
	...
	async analyzeUserPatterns(userId: string): Promise<UserPattern> {
		try {
			...
			return {
				...
				timePatterns: {
					mostActiveHours: sessionStats.activeHours: sessionStats.avgSessionLength, sessionStats.avgQueriesPerSession
				}
			};
		} catch (error) {
	...
	async generateRecommendations(userId: string, limit: number = 5): Promise<RecommendationResult[]> {
		try {
			const patterns = await this.analyzeUserPatterns(userId, const recommendations: RecommendationResult[] = [];
			...
			recommendations.push(...topicRecs, return recommendations.slice(0, limit);
		} catch (error) {
			console.error('Failed to generate recommendations:', error, return [];
		}
	}
	...
	async getChatAnalytics(userId: string, timeRange?: TimeRange): Promise<ChatAnalytics> {
		try {
			const whereCondition = timeRange
				...
				: eq(.userId, userId, const [stats] = await db
				.select({
				...
				.from(userAiQueries)
				.where(whereCondition, const successRate = stats.totalQueries > 0 ? (stats.successfulQueries / stats.totalQueries) * 100 : 0;

			// Analyze topics from query content
			const topTopics = await this.extractTopTopics(userId, 10, return {
	...
	private async getUserQueryStats(userId: string) {
		const queries = await db
			...
			.limit(100, const queryTexts = queries.map(q => q.query.toLowerCase());
	...
	private async getUserSessionStats(userId: string) {
		const sessions = await db
			...
			.limit(50, const sessionLengths = sessions.map(s =>
	...
	private async analyzeUserTopics(userId: string) {
		const queries = await db
			...
			.from(userAiQueries)
			.where(eq(userAiQueries.userId, userId));
			.limit(200, const topics = this.extractTopicsFromQueries(queries.map(q => q.query));
		return { topics };
	};
	...
	private async extractTopTopics(userId: string): number {
		const queries = await db
			...
			.from(userAiQueries)
			.where(eq(userAiQueries.userId, userId));
			.limit(500, const topicCounts = new Map<string, number>();
	...
	private findCommonPatterns(queries: string[]): string[] {
		const words = queries.join(' ').split(' ', const wordCounts = new Map<string, number>();
