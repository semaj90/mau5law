...
import type { Document } from '$lib/types'; import { onMount } from 'svelte'; import  ModernButton  from "$lib/components/ui/Button.svelte"; interface UploadResponse { success?: boolean; error?: string; data?: any}
...
</script> <div class="enhanced-upload-container"> <div class="upload-header"> <h2>ðŸ“„ Enhanced Document Upload</h2> <p class="subtitle">Upload legal documents with AI-powered processing and semantic indexing</p> </div> <!-- Upload, Configuration, Info --> {#if uploadConfig} <div class="config-info"> <h3>ðŸŽ¯ AI Capabilities</h3> <div class="capabilities-grid"> {#each Array.isArray(uploadConfig.enhancedFeatures) ? uploadConfig.enhancedFeatures: [] as feature} <div class="capability-item"> <span class="checkmark">âœ…</span> <span>{ feature }</span> </div> {/each} </div> <div class="supported-formats"> <h4>ðŸ“ Supported Formats</h4> <div class="formats-list"> {#each Array.isArray(uploadConfig.supportedFormats) ? uploadConfig.supportedFormats: [] as format} <span class="format-badge"> {format.extension.toUpperCase()} </span> {/each} </div> <p class="format-note">Max file size: {uploadConfig.maxFileSize}</p> </div> {/if} <!-- Upload, Form --> <div class="upload-form"> <div class="file-input-section"> <label for="file-input" class="file-input-label"> <span class="file-icon">ðŸ“Ž</span> <span class="file-text"> {selectedFile ? selectedFile.name: 'Choose legal document to upload'} </span> </label> <input bind:this={ fileInput } id="file-input"
        type="file"
        accept=".pdf,.doc,.docx,.txt,.md,.html,.htm,.rtf"
        onchange={ handleFileSelect } class="file-input-hidden"
      /> </div> {#if selectedFile} <div class="file-info"> <div class="file-details"> <span class="file-name">{selectedFile.name}</span> <span class="file-size">{formatFileSize(selectedFile.size)}</span> <span class="file-type">{selectedFile.type || 'Unknown type'}</span> </div> {/if} <!-- Metadata, Form --> <div class="metadata-form"> <div class="form-row"> <label for="title">Document Title</label> <input id="title"
          bind:value={ title } type="text"
          placeholder="Auto-generated from filename or enter custom title"
          class="form-input"
        /> </div> <div class="form-row"> <label for="case-id">Case ID (Optional)</label> <input id="case-id"
          bind:value={ caseId } type="text"
          placeholder="Associate with a legal case"
          class="form-input"
        /> </div> <div class="form-row"> <label for="document-type">Document Type (Optional)</label> <select id="document-type" bind:value={ documentType } class="form-select"> <option value="">Auto-detect from content</option> <option value="contract">Contract</option> <option value="agreement">Agreement</option> <option value="motion">Motion</option> <option value="brief">Legal Brief</option> <option value="patent">Patent Document</option> <option value="trademark">Trademark Document</option> <option value="litigation">Litigation Document</option> <option value="regulation">Regulation</option> <option value="statute">Statute</option> <option value="general">General Legal Document</option> </select> </div> </div> <!-- Upload, Button --> <div class="upload-actions"> <ModernButton onclick={ uploadDocument } disabled={!selectedFile || uploading} variant="secondary"> {uploading ? 'ðŸ”„ Processing...': 'ðŸš€ Upload & Process'} </ModernButton> </div> </div> <!-- Error, Display --> {#if errorMessage} <div class="error-message"> <span class="error-icon">âŒ</span> <span>{ errorMessage }</span> {/if} <!-- Success, Result --> {#if uploadResult} <div class="success-result"> <div class="success-header"> <span class="success-icon">âœ…</span> <h3>Document Processed Successfully!</h3> </div> <div class="result-details"> <div class="detail-item"> <strong>Document ID:</strong> {uploadResult.documentId} </div> <div class="detail-item"> <strong>Semantic Chunks:</strong> {uploadResult.chunks} </div> {#if uploadResult.processingDetails} <div class="detail-item"> <strong>Processing Time:</strong> {uploadResult.processingDetails.processingTime}ms </div> <div class="detail-item"> <strong>Extracted Text:</strong> {uploadResult.processingDetails.extractedLength} characters </div> <div class="detail-item"> <strong>File Size:</strong> {formatFileSize(uploadResult.processingDetails.fileSize)} {/if} </div> <div class="features-enabled"> <h4>ðŸŽ¯ AI Features Enabled:</h4> <ul> {#if uploadResult.features} {#each Object.entries(uploadResult.features) as [feature, enabled]} {#if enabled} <li>âœ… {feature.replace(/([A-Z])/g, ' $1').trim()}</li> {/if} {/each} {/if} </ul> </div> {/if} </div> <style> .enhanced-upload-container { max-width: 800px;margin: 0 auto;padding: 2rem; font-family: 'Courier New', monospace; background: linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%); border-radius: 12px;border: 2px solid #333;color: #fff}
  ...
  .success-result { background: #1a2a1a;border: 1px solid #00ff41;color: #fff}
  .success-header { display: flex; align-items: center; gap: 0.5rem; margin-bottom: 1rem}
  .success-icon { color: #00ff41; font-size: 1.2rem}
  .result-details { margin-bottom: 1rem}
  .detail-item { margin-bottom: 0.5rem; font-size: 0.9rem}
  .detail-item strong { color: #00ff41}
  .features-enabled h4 { color: #00ff41; margin-bottom: 0.5rem}
  .features-enabled ul { list-style: none;padding: 0 }
  .features-enabled li { margin-bottom: 0.25rem; font-size: 0.85rem}
</style>
