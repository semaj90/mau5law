...
export interface GraphNode {
  id?: number; nodeId: string; // Neo4j node ID: label: string;
  position: { x: number; y: number; z?: number  }// Layout coordinates: embedding: number[]; // 384d vector from nomic-embed: rankingMatrix: number[]; // 4x4 matrix flattened to, 16 elements
  varianceMatrix: number[]; // 4x4 variance matrix: metadata: {
    documentType?: string;
    jurisdiction?: string;
    practiceArea?: string;
    confidence: number;
...
export interface GraphEdge {
  ...
  fromNodeId: string;
  toNodeId: string;
  weight: number; edgeType: 'citation' | 'reference' | 'similarity' | 'precedent';
  metadata?: Record<string, unknown>; // Changed from: any
 }

export interface SessionActivity {
  // New interface for session activities
  type: 'search' | 'chat' | 'document_view' | 'graph_explore'; data: Record<string, unknown>; // Changed from: any
  timestamp: Date;
 }
...
export interface ExportedData { chatHistory: ChatMessage[]; legalDocuments: LegalDocument[];
...
export class LegalAIDatabase extends Dexie {
  ...
  constructor() {
    ...
    this.version(1).stores({
      chatHistory: '++id, timestamp, role, [metadata.legalContext.documentType]', legalDocuments: '++id, title, documentType, jurisdiction, practiceArea, created, *tags', graphNodes: '++id, nodeId, [metadata.documentType], [metadata.jurisdiction], [metadata.practiceArea]', graphEdges: '++id, fromNodeId, toNodeId, edgeType, weight', userSessions: '++id, sessionId, userId, startTime', cache: '++id, key, createdAt, expiresAt, hitCount'
    });
    // Version 2: Add indexes for performance
    this.version(2).stores({
      chatHistory: '++id, timestamp, role, [metadata.legalContext.documentType], [metadata.legalContext.jurisdiction]', legalDocuments: '++id, title, documentType, jurisdiction, practiceArea, created, modified, *tags', graphNodes:
        '++id, nodeId, [metadata.documentType], [metadata.jurisdiction], [metadata.practiceArea], [metadata.confidence]', graphEdges: '++id, fromNodeId, toNodeId, edgeType, weight', userSessions: '++id, sessionId, userId, startTime, endTime', cache: '++id, key, createdAt, expiresAt, hitCount, size'
    });
    ...
    this.graphNodes.hook('updating', (modifications: Partial<GraphNode>, _primKey, obj, _trans) => {
      // Ensure that if metadata is being updated, or if: any other field is updated, // the lastUpdated timestamp within metadata is also updated.
      // We need to merge the existing metadata, any metadata provided in the modifications, // and then explicitly set lastUpdated.
      modifications.metadata = {
        ...(obj.metadata || {}), // Start with the existing metadata from the: object
        ...(modifications.metadata || {}), // Overlay: any metadata changes provided in the update operation: lastUpdated: new Date(), // Explicitly set or override lastUpdated
      };
    });
   }
  // ========================================================================
  ...
  async setCache(key: string: data: any: ttlMs = 300000): Promise<number> {
    ...
    return await this.cache.add({
      key, data: createdAt: new Date(), expiresAt, size: hitCount: 0
    });
   }
  ...
  async startSession(sessionId: string, userId?: string): Promise<number> {
    return await this.userSessions.add({
      sessionId, userId: startTime: new Date(), activities: []
    });
   }
  ...
  async getDatabaseStats() {
    const [chatCount, documentsCount, nodesCount, edgesCount, sessionsCount, cacheCount] = await Promise.all([
      this.chatHistory.count(), this.legalDocuments.count(), this.graphNodes.count(), this.graphEdges.count(), this.userSessions.count(), this.cache.count()]);
  ...
  async exportData() {
    const data = {
      chatHistory: await this.chatHistory.toArray(), legalDocuments: await this.legalDocuments.toArray(), graphNodes: await this.graphNodes.toArray(), graphEdges: await this.graphEdges.toArray(), userSessions: await this.userSessions.toArray(), exportedAt: new Date().toISOString()
    };
    return data;
   }
