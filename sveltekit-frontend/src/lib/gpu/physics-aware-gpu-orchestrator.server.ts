...
class PhysicsAwareGpuOrchestrator {
  ...
  private updateDeviceMetrics(): void {
    ...
    this.totalGpuUtilization =
      this.availableDevices.reduce((sum, device) => sum + device.load, 0) / this.availableDevices.length;
  }
  ...
  private canExecuteImmediately(_task: GpuTask, device: GpuDevice): boolean {
    // Check if device can handle the task immediately
    const runningTasksOnDevice = Array.from(this.runningTasks.values()).filter(t => t.deviceId === device.id);
  ...
  private executeTask(_task: GpuTask, device: GpuDevice): void {
    task.status = 'running';
  ...
  private calculateEstimatedWaitTime(_task: GpuTask): number {
   ...
   */ getGPUStats(): {
    ...
    const averageTemperature =
      this.availableDevices.reduce((sum, device) => sum + device.temperature, 0) / this.availableDevices.length;
    const totalPowerUsage = this.availableDevices.reduce((sum, device) => sum + device.powerUsage, 0);
    return {
