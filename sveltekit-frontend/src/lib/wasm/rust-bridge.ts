...
export interface WasmModule { WindowsServiceBridge: new () => WindowsServiceBridge; process_legal_text_fast: (text: string) => string;
...
export function getBridgeStatus(): { initialized: boolean; capabilities: string[];
  performance: boolean;
 }{
  return { initialized: isRustBridgeAvailable(), capabilities: isRustBridgeAvailable() ? [
      'System Information', 'Secure File Access', 'GPU Detection', 'Performance Monitoring', 'Document Processing', 'Service Monitoring', 'Vector Operations', 'Batch Processing'
