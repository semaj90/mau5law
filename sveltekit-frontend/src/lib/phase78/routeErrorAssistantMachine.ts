...
async function simulateRouteAnalysis(route: RouteMeta): Promise<AnalyzeRouteOutput> {
	...
	const cluster: RouteErrorCluster = {
		routeId: route.id,
		errorCode: route.hasLoad ? 'TS2345' : 'TS2339',
	...
	const suggestions: PatchSuggestion[] = [
		{
			...
			severity: 'warning',
			patch: ['--- a', route.file || 'src/routes/__unknown.svelte', '+++ b', route.file || 'src/routes/__unknown.svelte', '@@',
				'-const data = await load()',
				"+const data = await load() as PageData"
			].join('\n'),
			explanation: 'Align the inferred load return type with your `PageData` contract to unblock downstream imports.',
			confidence: 0.68,
			hints: ['Review types exported from +page.ts', 'Ensure derived stores narrow to concrete fields']
		},
		{
...
export const routeErrorAssistantMachine = setup({
	...
	actions: {
		...
		assignSelectedSuggestion: assign(({ context, event }) => {
			const index = Math.min(Math.max((event as { index?: number }).index ?? 0, 0), context.suggestions.length - 1);
			return {
				selectedSuggestionIndex: index,
				suggestion: context.suggestions[index],
