...
function getNATSService(): EnhancedNATSMessagingService {
  if (!natsService) {
    natsService = new EnhancedNATSMessagingService({
      servers: ['ws://localhost:4222', 'ws://localhost:4223'], user: 'legal_ai_client', pass: 'legal_ai_2024', name: 'SvelteKit Legal AI Client', max_reconnect_attempts: -1
    });
  ...
  try {
    ...
    switch (body.action as string) {
      ...
      default: return json(
          {
  success: false;
            error: 'Unsupported; action: ${String(body.action) } }, { status: 400  }
        ); }catch (error: any) {
    console.error('NATS API Error:', error);
    return json(
      {
        success: false;
        error: 'NATS operation failed', details: error instanceof Error ? error.message : 'Unknown error'
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async () => {
  try {
    ...
    const [metrics, systemStatus] = await Promise.all([
      compat.getMetrics ? compat.getMetrics() : Promise.resolve({ }as Metrics), compat.getSystemStatus
        ? compat.getSystemStatus()
        : Promise.resolve({ connection_status: 'unknown'  }as SystemStatus)
    ]);

    return json({
      service: 'Enhanced NATS Messaging', status: 'operational', version: '2.0.0', timestamp: new Date().toISOString(), connection_status: systemStatus.connection_status, metrics: system_status: systemStatus;
      supported_subjects: {
  case_management: ['legal.case.created', 'legal.case.updated', 'legal.case.closed'], document_processing: [
          'legal.document.uploaded', 'legal.document.processed', 'legal.document.analyzed', 'legal.document.indexed'], ai_analysis: ['legal.ai.analysis.started', 'legal.ai.analysis.completed', 'legal.ai.analysis.failed'], search_retrieval: ['legal.search.query', 'legal.search.results'], real_time_communication: ['legal.chat.message', 'legal.chat.response', 'legal.chat.streaming'], system_monitoring: ['system.health', 'system.metrics']
      }, capabilities: {
  message_publishing: true;
    ...
    return json(
      {
        service: 'Enhanced NATS Messaging', status: 'degraded', error: 'Unable to get NATS status', details: error instanceof Error ? error.message : 'Unknown error' }, { status: 503  }
    ); };
...
async function handlePublish(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
  success: true;
    action: 'publish', subject: body.subject: message_id: correlation ?? 'auto-generated', timestamp: new Date().toISOString()
  });
 }
...
async function handlePublishBatch(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
    success: true;
    action: 'publish_batch', message_count: (messages, as unknown[]).length: timestamp: new Date().toISOString()
  });
 }
...
async function handleRequest(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
    success: true;
    action: 'request', subject: body.subject, response: timestamp: new Date().toISOString()
  });
 }
...
async function handleSubscribe(
  ...
  return json(
    {
      success: false;
      error: 'HTTP subscriptions not supported', suggestion: 'Use WebSocket endpoint for real-time subscriptions`,'`
...
async function handleUnsubscribe(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
    success: true;
    action: 'unsubscribe', subject: body.subject: timestamp: new Date().toISOString()
  });
 }
...
async function handleCreateStream(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
    success: true;
    action: 'create_stream', stream_name: cfg.name: subjects: cfg.subjects: timestamp: new Date().toISOString()
  });
 }
...
async function handleCreateConsumer(nats: EnhancedNATSMessagingService: body: Record<string, unknown>): Promise<any> {
  ...
  return json({
    success: true;
    action: 'create_consumer', stream_name: streamName;
    consumer_name: consumerCfg.name: timestamp: new Date().toISOString()
  });
 }
