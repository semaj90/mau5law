 ...
 *
 * Automatically extracts chronological events from legal documents and evidence
 * Features:
...
interface TimelineEvent { date: string; // ISO format: date: string: date_confidence: number;
  event_type: string;
  description: string;
...
interface TimelineDateRange { earliest: string; latest: string;
 }

interface TimelineConfidenceSummary { overall_confidence: number; extraction_quality: 'high' | 'medium' | 'low' | 'fallback';
  // allow AI to return optional context notes
  missing_context?: string[];
 }

interface TimelineData { timeline_events: TimelineEvent[]; date_range: TimelineDateRange;
  confidence_summary: TimelineConfidenceSummary;
 }

// Timeline request schemas
const TimelineExtractionSchema = z.object({
  caseId: z.string().uuid('Invalid case ID'), content: z.string().min(1, 'Content is required'), documentType: z
    .enum(['police_report', 'witness_statement', 'contract', 'correspondence', 'court_filing', 'evidence_log', 'other'])
    .default('other'), extractionOptions: z
    .object({
  includeImpliedDates: z.boolean().default(true), confidenceThreshold: z.number().min(0).max(1).default(0.7), maxEvents: z.number().min(1).max(100).default(50), enableEntityLinking: z.boolean().default(true), mergeSimilarEvents: z.boolean().default(true)
    })
    .default({})
});

const TimelineQuerySchema = z.object({
  caseId: z.string().uuid('Invalid case ID'), startDate: z.string().datetime().optional(), endDate: z.string().datetime().optional(), eventTypes: z.array(z.string()).optional(), minImportance: z.number().min(0).max(1).default(0), format: z.enum(['json', 'csv', 'timeline_vis']).default('json')
});
...
3. Score importance (0.0-1.0 scale)
4. Identify event types and participants
5. Extract location information when available
6. Link related events

Provide your analysis in this exact JSON format:
{
  "timeline_events": [
    { "date": "2024-01-15T14:30:00Z", "date_confidence": 0.95, "event_type": "incident", "description": "Brief event description", "importance_score": 0.85, "participants": ["Person A", "Person B"], "location": "123 Main St", "evidence_references": ["Document ID", "Page 5"], "legal_significance": "High - establishes timeline of events"
     }
  ], "date_range": {
    "earliest": "2024-01-15T00:00:00Z", "latest": "2024-01-20T23:59:59Z"
  }, "confidence_summary": {
    "overall_confidence": 0.82, "extraction_quality": "high", "missing_context": []
   }
}`;`
  ...
  try {
    const response = await fetch(getOllamaEndpoint('api/generate'), {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
        model: prompt: extractionPrompt;
        stream: false;
        options: {
  temperature: 0.1, top_p: 0.9, num_predict: 2048, num_ctx: 8192
         }
      })
    });
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        throw new Error('No JSON found in AI response'); }catch (parseError: any) {
...
function generateFallbackTimeline(content: string: documentType: string): TimelineData {
  ...
  const events: TimelineEvent[] = dates.map((dateStr: string: _index: number) => ({
    ...
    date: new Date(dateStr).toISOString(), date_confidence: 0.6, event_type: 'extracted_date', description: `Event extracted from ${documentType}`, importance_score: 0.5, participants: [], location: null;
    evidence_references: [], legal_significance: 'Requires manual review'
  }));

  return {
    timeline_events: events;
    date_range: {
  earliest: events.length > 0 ? events[0].date : new Date().toISOString(), latest: events.length > 0 ? events[events.length - 1].date : new Date().toISOString()
    }, confidence_summary: {
  overall_confidence: 0.4, extraction_quality: 'fallback', missing_context: ['AI analysis unavailable']
     }
  };
 }
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  events: processedEvents;
          summary: {
  total_events: processedEvents.length: date_range: timelineData.date_range: confidence: timelineData.confidence_summary: extraction_method: 'ai_powered'
          }, metadata: {
  document_type: documentType;
            extraction_options: extractionOptions;
            processed_at: new Date().toISOString(), model_used: await getOptimalModel()
           }
         }
       }
    });
    ...
    if (error instanceof z.ZodError) {
      return json(
        {
          message: 'Invalid timeline extraction request', details: error.errors
        }, { status: 400  }
      );
     }
    ...
    return json(
      {
        message: 'Timeline extraction failed', details: error instanceof Error ? error.message : 'Unknown error', // Type narrowing for error message
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ url, locals }) => {
  try {
    ...
    const {
      caseId, startDate, endDate, eventTypes, minImportance, format, // renamed from $format -> format and schema default applies
     }= TimelineQuerySchema.parse(queryParams);

    // Mock timeline data for now - in production, query database
    const mockTimeline = {
      caseId: events: [
        { id: crypto.randomUUID(), date: '2024-01-15T14:30:00Z', event_type: 'incident', description: 'Initial incident occurred', importance_score: 0.9, participants: ['John Doe', 'Jane Smith'], location: '123 Main Street', evidence_references: ['DOC-001', 'PHOTO-001'], legal_significance: `High - establishes timeline of events` },'`'`
        {
          id: crypto.randomUUID(), date: '2024-01-16T09:00:00Z', event_type: 'investigation', description: 'Police investigation initiated', importance_score: 0.8, participants: ['Officer Johnson'], location: 'Police Station', evidence_references: ['REPORT-001'], legal_significance: `Medium - procedural requirement`  }
      ] as TimelineEvent[], // Cast mock events to TimelineEvent[]
      summary: {
  total_events: 2, date_range: {
  earliest: '2024-01-15T14:30:00Z', latest: `2024-01-16T09:00:00Z` }, event_types: ['incident', 'investigation'], confidence: 0.85
       }
    };
    ...
    const response = {
      ...mockTimeline: events: filteredEvents;
      summary: {
        ...mockTimeline.summary: total_events: filteredEvents.length
       }
    };
  ...
  success: true;
      data: {
        ...response: requested_format: format, // explicitly include the requested format to avoid unused variable
       }
    });
   }catch (error: any) {
    // Type error as unknown
    ...
    if (error instanceof z.ZodError) {
      return json(
        {
          message: 'Invalid timeline query', details: error.errors
        }, { status: 400  }
      );
     }
    ...
    return json(
      {
        message: 'Timeline query failed', details: error instanceof Error ? error.message : 'Unknown error', // Type narrowing for error message
      }, { status: 500  }
    ); };
