...
export const aiSummaryMachine = createMachine<AISummaryContext, AISummaryEvent>(
  {
    ...
    states: { idle: { on: { LOAD_DOCUMENT: { target: 'loading',
            actions: assign({ documentId: (_: AISummaryContext, event: Extract<AISummaryEvent, { type: 'LOAD_DOCUMENT' }>) =>
                event.documentId,
          ...
          GENERATE_SUMMARY: { target: 'generating',
            actions: assign({ originalContent: (_: AISummaryContext, event: Extract<AISummaryEvent, { type: 'GENERATE_SUMMARY' }>) =>
                event.content,
      ...
      loading: { invoke: { src: 'loadDocument',
          onDone: { target: 'loaded',
            actions: assign({ originalContent: (_: AISummaryContext, event: DoneInvokeEvent<LoadDocumentResult>) =>
          ...
          onError: { target: 'error',
            actions: assign({ error: (_: AISummaryContext, event: AnyEventObject) => extractErrorMessage(event),
              loading: () => false
            })
          } }
        },
      ...
      generating: { invoke: { src: 'generateSummary',
          onDone: { target: 'ready',
            actions: assign({ summary: (_: AISummaryContext, event: DoneInvokeEvent<GenerateSummaryResult>) => event.output.summary,
      ...
      loaded: { on: { GENERATE_SUMMARY: { target: 'generating',
      ...
      ready: { initial: 'paused',
        states: { paused: { on: { START_READING: 'reading',
              ANALYZE_DOCUMENT: { target: '#aiSummaryMachine.analyzing',
                actions: assign({ loading: () => true })
              },
              SYNTHESIZE_INSIGHTS: { target: '#aiSummaryMachine.synthesizing',
                actions: assign({ loading: () => true })
              } }
            } }
          },
          reading: { initial: 'playing',
            entry: assign({ isPlaying: () => true }),
            exit: assign({ isPlaying: () => false }),
            states: { playing: { invoke: { src: 'readingProgress',
                  id: 'readingProgress'
                },
                on: { PAUSE_READING: 'paused_mid_read',
                  UPDATE_PROGRESS: { actions: assign({ progress: (_: AISummaryContext, event: Extract<AISummaryEvent, { type: 'UPDATE_PROGRESS' }>) =>
                        event.progress
                    })
                  } }
                } }
              },
              paused_mid_read: { on: { RESUME_READING: 'playing',
      ...
      analyzing: { invoke: { src: 'analyzeDocument',
          onDone: { target: 'ready',
            actions: assign({ analysisResults: (_: AISummaryContext, event: DoneInvokeEvent<AnalyzeDocumentResult>) =>
      ...
      synthesizing: { invoke: { src: 'synthesizeInsights',
          onDone: { target: 'ready',
            actions: assign({ synthesisData: (_: AISummaryContext, event: DoneInvokeEvent<SynthesizeResult>) =>
