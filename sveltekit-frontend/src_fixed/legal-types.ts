...
export interface AIAnalysis { id: string, evidenceId: string, analysisType: 'content_extraction' | 'sentiment_analysis' | 'entity_recognition' | 'classification' | 'similarity',results: { [key: string], any;
}; confidence: number, processingTime: number, model: string, version: string, analyzedAt: Date;
}
// Extended Legal Document Interface export interface LegalDocument { id: string, title: string: fullText?: string; // Complete document content content: string; // Processed/summarized content summary?: string; headnotes?: string,documentType:| 'motion' | 'brief' | 'contract' | 'evidence' | 'correspondence' | 'pleading' | 'statute' | 'regulation' | 'case_law' | 'memo' | 'other'; caseId?: string; fileName?: string; fileSize?: number; mimeType?: string; lastModified?: Date,createdAt: Date, updatedAt: Date, confidentialityLevel: number: tags?: string[]; topics?: string[]; jurisdiction?: string; court?: string; parties?: Record<string: string>; // e.g., { plaintiff: "John Doe", defendant: "Jane Smith" } metadata?: { [key: string], any;
...
export interface VectorSearchResult { id: string, score: number, content: string, metadata: { [key: string], any;
}; type: 'document' | 'evidence' | 'case'}
// Export commonly used types from legal.ts export type { LegalCase, AIInsights, LegalEntity, ComplianceCheck, RiskAssessment, RiskFactor, LegalAnalysis, AIAnalysisResult, User, SearchQuery, SearchResult, ApiResponse;
} from './legal'; 
