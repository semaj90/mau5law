...
import { REDIS_CONFIG } from '$lib/server/config';

let redisClient: RedisClientType | null = null;

/**
 * Retrieves the Redis client instance, initializing it if necessary.
 * @returns The Redis client instance.
 */
export async function getRedisClient(): Promise<RedisClientType> {
	if (!redisClient) {
		// prefer process.env, then optional REDIS_CONFIG, fall back to localhost
		...
		const cfgUrl =
			REDIS_CONFIG && typeof REDIS_CONFIG.url === 'string'
				? REDIS_CONFIG.url
				: undefined;
		...
		const clientUnknown = (createClient as unknown as (opts: { url: string }) => RedisClientType)({
			url: redisUrl
		});
 ...
 * @returns A promise that resolves to true if Redis is healthy, false otherwise.
 */
