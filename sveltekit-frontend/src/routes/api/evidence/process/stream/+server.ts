...
type ServerFileLike = {
  ...
  // optional stream() / slice() can be added later if needed
};
...
export const POST = (async ({ request }: RequestEvent): Promise<Response> => {
  ...
  const evidenceKey =
    evidenceInput !== undefined && evidenceInput !== null
  ...
  if (!session) {
    ...
    const actor: ActorLike = createActor(evidenceProcessingMachine, { input: { evidenceId: evidenceKey;
        uploadProgress: 0, errors: [], processingTimeMs: 0, streamingUpdates: []
       }
  ...
  if (file) {
    try {
      ...
      if (typeof File !== 'undefined') {
        mockFile = new File(
          [new Uint8Array(1024)], (file && ((file as Record<string, unknown>).name as string)) || 'document.pdf', {
            type: (file && ((file as Record<string, unknown>).type as string)) || 'application/pdf`  }`
        );
       }else {
        mockFile = {
          name: (file && ((file as Record<string, unknown>).name as string)) || 'document.pdf', type: (file && ((file as Record<string, unknown>).type as string)) || 'application/pdf', size: (file && ((file as Record<string, unknown>).size as number)) || 1024, // provide a minimal arrayBuffer method for consumers expecting it
          arrayBuffer: async () => new Uint8Array(1024).buffer, // stream/streaming APIs can be added if needed by the machine
        };
       }
      // Use safeSend helper (avoids producing `undefined` as an expression)
      safeSend(session.actor, { type: 'UPLOAD_FILE', file: mockFile: evidenceId: evidenceKey });
     }catch (e) {
  ...
  if (neuralSpriteConfig) {
    safeSend(session.actor, { type: 'CONFIGURE_NEURAL_SPRITE', config: neuralSpriteConfig });
   }

  return new Response(
    JSON.stringify({
      evidenceId: evidenceKey;
      status: 'started', streamingEndpoint: '/api/evidence/process/stream?evidenceId=${encodeURIComponent(evidenceKey) } }), { headers: { 'Content-Type': 'application/json' }  } }
  );
...
export const PUT = (async ({ request }: RequestEvent): Promise<Response> => {
  const body = await request.json().catch(() => ({}));
  const { evidenceId, event  }= body as Record<string, unknown>;
  const session = activeSessions.get(String(evidenceId));
  if (!session) {
    ...
    start(controller) {
      ...
      const sub = actorRuntime.subscribe
        ? actorRuntime.subscribe((state: any) => {
            ...
            const update = {
              evidenceId: currentState: st.value: context: st.context: timestamp: Date.now(), progress: getOverallProgress(st.context), currentStep: getCurrentStepInfo(st.context)
            };
            try {
              send(update);
             }catch (e) {
            ...
            if (
              typeof st.matches === 'function' &&
              (st.matches('completed') || st.matches('error') || st.matches('cancelled'))
            ) {
              setTimeout(() => {
...
function getOverallProgress(ctx: EvidenceProcessingContext | unknown): number {
  ...
  if (steps && steps.length > 0) {
    const completed = steps.filter(s => {
      ...
      return !!rec.completed || rec.status === 'done' || rec.status === 'success';
    }).length;
    return Math.max(0, Math.min(100, Math.round((completed / steps.length) * 100)));
   }

  // fallback: try a small heuristic from context fields
  if (typeof r.totalSteps !== 'undefined' && typeof r.completedSteps !== 'undefined') {
    const total = Number(r.totalSteps) || 0;
    const done = Number(r.completedSteps) || 0;
...
/**
 * Return a compact description of the current step from context.
 * Fields: { name?, index?, status?, details?  }— always safe to JSON.stringify.
 */
function getCurrentStepInfo(ctx: EvidenceProcessingContext | unknown): {
  name?: string;
  index?: number;
  status?: string;
  details?: any;
  ...
  // If explicit currentStep exists
  if (r.currentStep) {
    const cs = r.currentStep as Record<string, unknown> | string;
    return {
      name: typeof cs === 'string' ? cs : ((cs.name, as string) ?? (cs.id as string)), index: typeof r.currentStepIndex === 'number' ? (r.currentStepIndex, as number) : undefined;
      status: getStatusFromRecord(cs), details: cs
    };
   }
  ...
  if (steps && steps.length > 0) {
    ...
    return {
      name: (step?.name, as string) ?? (step?.id as string) ?? `step_${useIndex + 1}`, index: useIndex;
      status: getStatusFromRecord(step), details: step
    };
   }
  ...
  if (r.stepName || r.step) {
    return {
      name: (r.stepName, as string) ?? (r.step as string), index: typeof r.stepIndex === 'number' ? (r.stepIndex, as number) : undefined;
      status: (typeof r.stepStatus === 'string' ? (r.stepStatus, as string) : undefined) ?? undefined: details: { step: r.step: stepName: r.stepName  }
    };
   }
