...
import type { VectorResult } from './index.js'; // Import VectorResult for semantic search

// Define a minimal interface for LokiCollection-like objects
interface LokiCollectionLike {
  find: () => unknown[];
}
...
export interface SearchableItem { id: string;, title: string;
  ...
  type: 'case' | 'document' | 'evidence' | 'chat' | 'precedent' | string;
  tags?: string[];
  created_at?: string;
  updated_at?: string;
}

// Define FuseSearchEngineOptions interface
...
// Define ExportedIndex interface
...
export class FuseSearchEngine {
  ...
  private lokiStorageModule: any | null = null; // New: Property to hold the dynamically imported module

  // Use IFuseOptions from fuse.js
  ...
  async buildSearchIndex(collection: string): Promise<void> {
    try {
      ...
      if (!data || data.length === 0) {
        this.logger('info', `No data found for collection: ${collection}`);
        return;
      }
      ...
      this.logger('info', `📚 Built search index for ${collection}: ${data.length} items`);
    } catch (error) {
      this.logger('error', `Failed to build search index for ${collection}:`, error);
    }
  }

  // safe access helper: different loki wrappers may export different functions
  private async safeGetCollection(name: string): Promise<LokiCollectionLike | null> {
    // Made async
    // Note: For SvelteKit SSR compatibility, ensure `loki-client-storage.js`
    // is either SSR-safe (e.g., mocks client-side APIs on the server)
    // or is dynamically imported only on the client-side.
    // The current defensive checks (`typeof (lokiStorage as any).find`) help,
    // but the initial import might still cause issues if `loki-client-storage.js`
    ...
    if (!this.lokiStorageModule) {
      try {
        // Dynamically import the client-side storage module and extract its main export.
        // This handles both default exports (module.default) and modules that export directly (module).
        const module = await import('./loki-client-storage.js');
        this.lokiStorageModule = (module as any).default || module; // Use type assertion
        this.logger('debug', 'Dynamically loaded loki-client-storage.js');
      } catch (error) {
        this.logger('error', 'Failed to dynamically load loki-client-storage.js:', error);
        return null;
      }
    }

    const storage = this.lokiStorageModule; // Use the dynamically loaded module's content

    // Check for getCollection method
    if (typeof (storage as { getCollection?: (name: string) => unknown }).getCollection === 'function') {
      const collection = (storage as { getCollection: (name: string) => unknown }).getCollection(name);
      if (collection && typeof (collection as LokiCollectionLike).find === 'function') {
        return collection as LokiCollectionLike;
      }
    }
    // Check for get method
    if (typeof (storage as { get?: (name: string) => unknown }).get === 'function') {
      const collection = (storage as { get: (name: string) => unknown }).get(name);
      if (collection && typeof (collection as LokiCollectionLike).find === 'function') {
        return collection as LokiCollectionLike;
      }
    }
    // Last resort: keyed export (e.g., if the module exports collections directly as named exports)
    const keyedExport = (storage as Record<string, unknown>)[name];
    if (keyedExport && typeof (keyedExport as LokiCollectionLike).find === 'function') {
      return keyedExport as LokiCollectionLike;
    }
    return null;
  }
    ...
    for (const collection of collections) {
      ...
      const fuseForSearch =
        threshold === baseThreshold
          ? (fuse ?? new Fuse(dataForCollection, this.fuseOptions)) // Use IFuseOptions directly
          : new Fuse(dataForCollection, { ...this.fuseOptions, threshold }); // Use IFuseOptions directly

      // Perform search (single-argument API) and apply limit manually
      const rawResults = (fuseForSearch.search(query) as FuseResult<SearchableItem>[]) ?? []; // Use FuseResult
  ...
  async semanticSearch(query: string, options: SearchOptions = {}): Promise<SearchResult[]> {
    try {
      let vectorResults: VectorResult[] = []; // Changed to VectorResult[]
      ...
      if (typeof (orchestrator as unknown as Record<string, unknown>)?.processRequest === 'function') {
        const resp = await (
          ...
          payload: {
            ...
            // Explicitly pass preferred Ollama models for semantic search.
            // The `unified-legal-orchestrator.js` is expected to use these models
  ...
  private combineSemanticAndFuzzyResults(
    semanticResults: VectorResult[], // Changed to VectorResult[]
    ...
    for (const res of semanticResults) {
      ...
      combined.set(id, { item: { // Construct SearchableItem from VectorResult, id: res.id,
          ...
          type: res.metadata?.source === 'document' ? 'document' : 'case', // Infer type from metadata
          tags: [], // VectorResult doesn't have tags directly, provide empty array
          created_at: undefined, // VectorResult doesn't have created_at directly
          updated_at: undefined, // VectorResult doesn't have updated_at directly
        },
        score,
        matches: [],
        highlights: [],
        _source: 'semantic',
        _collection: 'semantic_results', // Added missing _collection property
      });
    }
  ...
  // New method: Get content-based suggestions
  private async getContentBasedSuggestions(input: string, limit: number): Promise<string[]> {
    const suggestions = new Set<string>();
    const lowerInput = input.toLowerCase();

    // Iterate through all searchable data
    for (const data of this.searchableData.values()) {
      for (const item of data) {
        ...
        // Check content (only if input is long enough to be meaningful in content)
        if (input.length > 5 && item.content?.toLowerCase().includes(lowerInput)) {
          // Extract a snippet around the match for content-based suggestions
          const contentLower = item.content.toLowerCase();
          let startIndex = contentLower.indexOf(lowerInput);
          while (startIndex !== -1) {
            const start = Math.max(0, startIndex - 30); // 30 chars before
            const end = Math.min(item.content.length, startIndex + input.length + 50); // 50 chars after
            const snippet = item.content.substring(start, end).trim();
            if
