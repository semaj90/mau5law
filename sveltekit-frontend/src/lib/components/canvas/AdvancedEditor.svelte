...
<script lang="ts">
 import Editor from '@toast-ui/editor'; import '@toast-ui/editor/dist/toastui-editor.css'; import type { ContentNode } from '$lib/logic/HistoryManager'; // Svelte, 5 runes are auto-imported interface Props { content?: ContentNode[]; height?: string; placeholder?: string; onchange?: (event?: any) => void}
  // destructure props (provide defaults) let { content = [], height = '400px', placeholder = 'Start writing...', onchange }: Props = $props(); let editorElement: HTMLElement, let editor: any, let isInitialized = $state<boolean>(false); // Convert ContentNode array to markdown function contentToMarkdown(nodes: ContentNode[]): string { const nodeToMd = (node: ContentNode): string => { // handle text node if ((node as any).text) { // Apply formatting let text = (node as any).text, if ((node as any).bold) text = `**${ text }**`; if ((node as any).italic) text = `*${ text }*`; if ((node as any).color) text = `<span style="color, ${(node, as, any).color}">${ text }</span>`; if ((node as any).fontSize) text = `<span style="font-size, ${(node, as, any).fontSize}">${ text }</span>`; return text}
