...
type EndToEndModule = {
  executeFullPipeline?: (queries: string[]) => Promise<Partial<RawMetrics> | RawMetrics>;
...
const loadAdvancedPipeline = async (): Promise<AdvancedModule> =>
  loadModule<AdvancedModule>('./advanced-simd-pipeline.js');
const loadEndToEndPipeline = async (): Promise<EndToEndModule> =>
  loadModule<EndToEndModule>('./end-to-end-pipeline.js');
...
export type PipelineSearchHit = {
  ...
  score?: number;
  snippet?: string;
  source?: PipelineType | 'end-to-end' | string;
  metadata?: Record<string, unknown>;
};

// new typed health interfaces to replace `any` in getSystemHealth
...
export class PipelineManager {
  ...
  async batchProcess(
    ...
    for (let i = 0; i < requests.length; i += batchSize) {
      ...
      const batchPromises = batch.map(request => this.executePipeline(request.cacheKey, request.config));
  ...
  async searchAllPipelines(
    ...
    const optimizedSearch = safeSearchFn(optimizedMod as Record<string, unknown>, ['searchOptimizedResults', 'search']);
    const advancedSearch = safeSearchFn(advancedMod as Record<string, unknown>, ['searchProcessedTensors', 'search']);
  ...
  async cleanup(): Promise<void> {
    ...
    if (optimizedMod && typeof optimizedMod.cleanup === 'function') cleanupPromises.push(optimizedMod.cleanup());
    if (advancedMod && typeof advancedMod.cleanup === 'function') cleanupPromises.push(advancedMod.cleanup());
    if (endToEndMod && typeof endToEndMod.cleanup === 'function') cleanupPromises.push(endToEndMod.cleanup());
  ...
  private transformMetrics(rawMetrics: Partial<RawMetrics> | undefined, _type: PipelineType): PipelineMetrics {
    ...
    const workerThreads = rawMetrics?.workerThreads ?? 1;
    const gpuAccelerated = rawMetrics?.gpuAccelerated ?? false;

    const throughput = totalResults && processingTime ? totalResults / (processingTime / 1000) : 0;
