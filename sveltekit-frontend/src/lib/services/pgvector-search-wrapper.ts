/**
 * pgvector Optimized Search Wrapper
 * Provides a simplified interface to the new ultra-fast pgvector search endpoint
 *
 * This wrapper replaces the complex XState machine + Python subprocess approach
...
): Promise<PgvectorSearchResponse> {
 ...
 * Useful for displaying search results with matching excerpts
 *
 * @param request Search request parameters
 ...
 * @param contextLength Number of characters to show around match
...
export async function pgvectorSearchHealth(): Promise<{ healthy: boolean;, status: string;
  ...
  try {
    const response = await fetch('/api/search-pgvector-optimized/health');
