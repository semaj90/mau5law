...
export class LegalRecommendationEngine {
    ...
    private static async semanticRecommendations(
        ...
        const temporalWindow = request.temporalWindow ?? 30;
        const max = request.maxResults ?? 10;

        // Use raw SQL for embedding similarity + filtering
        // Assuming pgvector extension and <-> operator or cosine similarity
        const query = sql`
            SELECT
                r.id, r.query, r.response, r.confidence, r.legal_domain,
                r.jurisdiction, r.usage_count, r.created_at, r.last_accessed, r.metadata
            FROM ai_responses r
            WHERE
                r.metadata->>'embedding' IS NOT NULL
                AND r.created_at >= NOW() - INTERVAL '${sql.raw(String(temporalWindow))} days'
            ORDER BY r.created_at DESC
            LIMIT ${max * 2}
        `;

        // Note: Real semantic search would use: ORDER BY embedding <-> ${JSON.stringify(queryEmbedding)}
        ...
        return rows.map((row: any) => this.buildRecommendationResult(
            row,
            'semantic',
            { semanticSimilarity: 0.8 } // Placeholder score
        ));
    }
    ...
    private static async collaborativeRecommendations(
        ...
        const query = sql`
            SELECT
                r.id, r.query, r.response, r.confidence, r.legal_domain,
                r.created_at, AVG(f.user_rating) AS avg_rating, COUNT(f.user_rating) AS rating_count
            FROM ai_responses r
            LEFT JOIN grpo_feedback f ON f.response_id = r.id
            WHERE
                r.created_at >= NOW() - INTERVAL '${sql.raw(String(request.temporalWindow ?? 60))} days'
                AND r.legal_domain IN ${domains}
            GROUP BY r.id
    ...
    private static async temporalRecommendations(
        ...
        const query = sql`
            SELECT
                ...
                r.jurisdiction, r.usage_count, r.created_at, r.last_accessed, r.metadata
            FROM ai_responses r
            WHERE r.created_at >= NOW() - INTERVAL '${sql.raw(String(request.temporalWindow ?? 30))} days'
            ORDER BY r.created_at DESC
            LIMIT ${max}
        `;
    ...
    private static async hybridRecommendations(
        ...
        const scored = merged.map(r => {
            const semanticScore = (r.metadata.semanticSimilarity ?? 0);
            const temporalScore = (r.metadata.temporalScore ?? 0);
            const userPref = (r.metadata.userPreference ?? 0.5);
            const usage = (r.metadata.usageScore ?? 0);

            const final =
                semanticScore * this.SCORING_WEIGHTS.SEMANTIC +
                temporalScore * this.SCORING_WEIGHTS.TEMPORAL +
                userPref * this.SCORING_WEIGHTS.USER_PREFERENCE +
                usage * this.SCORING_WEIGHTS.USAGE_POPULARITY;
        ...
        for (const item of scored) {
            if (!seen.has(item.r.id)) {
                seen.add(item.r.id);
                // Update the score property
                item.r.score = item.final;
                unique.push(item.r);
            }
        }
    ...
    private static async getPopularRecommendations(request: RecommendationRequest): Promise<RecommendationResult[]> {
        ...
        const query = sql`
            ...
            WHERE
                r.created_at >= NOW() - INTERVAL '${sql.raw(String(request.temporalWindow ?? 30))} days'
                AND r.usage_count > 0
            GROUP BY r.id
        ...
        return rows.map((row: any) => this.buildRecommendationResult(
            row,
            'popular',
            { popularityRating: parseFloat(row.avg_rating) || 0 }
        ));
    }
    ...
    private static buildRecommendationResult(
        ...
        const snip = response.length > 200 ? response.slice(0, 200) + '...' : response;
        const title = this.extractTitle(row.query ?? '', response);

        // Calculate a final score based on available partial scores
        const finalScore = scores.finalScore ?? scores.semanticSimilarity ?? scores.userRating ?? 0.5;
    ...
    private static generateExplanation(algorithm: string, scores: { [k: string]: number }): string {
        const topFactors = Object.entries(scores || {})
            .sort(([a], [b]) => (b as any) - (a as any)) // Simplify sort
            .slice(0, 3)
    ...
    private static ensureDiversity<T extends { metadata: { [k: string]: any } }>(
        ...
        for (const item of items) {
            const domain = (item.metadata && item.metadata[field]) ?? 'general';
    ...
    static async recordInteraction(
        ...
        try {
            ...
            if (typeof rating === 'number' && rating >= 1 && rating <= 5) {
                // await db.insert(grpoFeedback).values({ responseId, userId, feedbackType: 'interaction' }); // Needs actual schema
            }
        } catch (error) {
...
function execRows(results: any): any[] {
    return Array.isArray(results) ? results : (results && results.rows) || [];
}
