...
const: originalPOSTHandler: RequestHandler = async ({ request }) => {
  ...
  model: 'gemma3-legal:latest', prompt: `Analyze this UI element for legal; relevance: `
...
Format as: JSON: {"relevance": "...", "legalContext": "evidence|case|statute|procedure|other", "actionable": true}`,`
    ...
    if (!result) {
      analysis = {
        relevance: 'Content may have legal significance', legalContext: 'general', actionable: false
      };
     }else if (typeof result === 'string') {
      try {
        analysis = JSON.parse(result) as Analysis;
       }catch {
        analysis = { relevance: 'unknown', raw: result }; }else if (typeof result === 'object' && result !== null) {
      ...
      if ('response' in r) {
        ...
        if (typeof inner === 'string') {
          try {
            ...
            analysis = { relevance: 'unknown', response: inner }; }else {
          analysis = inner as unknown as Analysis; }else {
        analysis = r as Analysis; }else {
