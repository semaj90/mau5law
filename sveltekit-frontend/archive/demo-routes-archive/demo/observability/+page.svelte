...
<script lang="ts">
  ...
  async function checkServerHealth() {
    ...
    try {
      ...
      addDemoResult('Health Check', `${(data as { status?: unknown; score?: unknown; totalStoredMetrics?: unknown; message?: unknown }).status} (${Math.round(duration)}ms)`, 'success');
      // Track custom event
      trackCustomEvent('health-check-completed', {
        status: (data as { status?: unknown; score?: unknown; totalStoredMetrics?: unknown; message?: unknown }).status,
        score: (data as { status?: unknown; score?: unknown; totalStoredMetrics?: unknown; message?: unknown }).score,
        duration
      });
    } catch (error) {
  ...
  async function loadClientMetrics() {
    ...
    try {
      ...
      clientMetrics = data;
      const duration = performance.now() - startTime;
      addDemoResult('Load Metrics', `${(data as { status?: unknown; score?: unknown; totalStoredMetrics?: unknown; message?: unknown }).totalStoredMetrics} metrics (${Math.round(duration)}ms)`, 'info');
    } catch (error) {
  ...
  async function clearMetrics() {
    ...
    try {
      ...
      const data = await (response as { json?: unknown }).json();
      addDemoResult('Clear Metrics', (data as { status?: unknown; score?: unknown; totalStoredMetrics?: unknown; message?: unknown }).message || 'Success', 'info');
      // Reload metrics
      await loadClientMetrics();
    } catch (error) {
          ...
          {#each demoResults.slice.reverse() as result ((result as { id?: unknown; timestamp?: unknown; action?: unknown; type?: unknown; result?: unknown }).id)}
            <div class="flex justify-between items-center py-2 border-b border-gray-700 last:border-b-0">
              <div class="flex items-center space-x-3">
                ...
                <span class="text-white font-medium">{(result as { id?: unknown; timestamp?: unknown; action?: unknown; type?: unknown; result?: unknown }).action}</span>
              </div>
              <span class="{getResultColor((result as { id?: unknown; timestamp?: unknown; action?: unknown; type?: unknown; result?: unknown }).type)} text-sm">
                {(result as { id?: unknown; timestamp?: unknown; action?: unknown; type?: unknown; result?: unknown }).result}
              </span>
            </div>
          {/each}
        </div>
