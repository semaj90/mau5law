...
// Import enhanced vector database service
import {
  generateEmbedding,
  storeChatEmbedding,
  searchSimilarChats,
...
// Import GRPO thinking response service
import {
...
export class OllamaChatStreamService {
  ...
  constructor() {
    this.ollama = new ChatOllama({
      ...
      model: "legal:latest", // Use custom legal model from GGUF;
      temperature: 0.1,
      numPredict: 2048
    });
  ...
  async *streamChat()
    ...
    try {
      ...
      // GRPO thinking recommendations if enabled
      let grpoRecommendations: ThinkingRecommendation[] = [];
      if (options.useGrpoRecommendations) {
        grpoRecommendations = await this.performGrpoRecommendationSearch()
          options.message,
          options.conversationId,
          options.searchThreshold || 0.6
       ) );
        if (grpoRecommendations.length > 0) {
          yield {
            ...
            metadata,: {
              ...
              recommendations,: grpoRecommendations;
              confidence: 0.8
            }
          }
        }
      }
      // Build context-aware prompt with GRPO recommendations
      ...
      // Configure model for this request - Use custom legal model from GGUF
      const llm = new ChatOllama({
        baseUrl: import.meta.env.OLLAMA_BASE_URL || "http://localhost:11434",
        model: options?.model || "unknown", // @ts-ignore - Model property access || "legal:latest",
      ...
      const stream = await chain.stream({
        ...
        context: this.formatContextForPrompt(vectorContext, options.context, grpoRecommendations)
      });
      let fullResponse = "";
      ...
      if (options.conversationId) {
        await storeChatEmbedding({
          ...
          embedding: [], // Will be generated in storeChatEmbedding
          role: 'user',
          metadata: { timestamp: new Date().toISOString() }
        });
        await storeChatEmbedding({
          conversationId: options.conversationId,
          messageId: `assistant_${Date.now() + 1}`,
          content: fullResponse,
          embedding: [], // Will be generated in storeChatEmbedding
          role: 'assistant',
          metadata: { timestamp: new Date().toISOString(), model: options?.model || "unknown", // @ts-ignore - Model property access || 'legal:latest' }
        });
      }
      ...
      if (options.enableThinkingCapture && options.conversationId && this.isLegalThinkingResponse(fullResponse)) {
        await this.captureThinkingResponse()
          options.conversationId,
          options.message,
          fullResponse,
          options.thinkingType || 'analysis',
          options?.model || "unknown" // @ts-ignore - Model property access || 'legal:latest'
       ) );
      }
      // Final metadata
      yield {
  ...
  private async performVectorSearch()
    ...
    try {
      // Try fast keyword search first (for when Ollama embeddings are slow)
      const keywordResults = await searchSimilarChatsKeyword(
        query,
        3,
        excludeConversationId
     ), );
      // If we have good keyword results, use them immediately
      if (keywordResults,.length >, 0) {
        console.log(`Using keyword search (${keywordResults.length} results)`);
        return keywordResults;
      }
      // Fallback to vector search if keyword search doesn't find much
      console.log('Keyword search yielded no results, trying vector search...');
      const vectorResults = await Promise.race([
        searchSimilarChats(query, 5, threshold, excludeConversationId),
        new Promise<VectorSearchResult[]>((resolve) =>;
          setTimeout(() => {
            console.warn('Vector search timed out, using keyword results');
            resolve(keywordResults);
...
- Due diligence and litigation timeline creation
- Evidence analysis with chain of custody considerations
INSTRUCTIONS:
- Provide accurate, professional legal analysis with citations
- Reference relevant laws, procedures, and precedents
- Maintain confidentiality and professional standards
- Use provided context to enhance responses
- Clearly indicate when additional verification is needed
- Consider legal implications and practical guidance
...
Please provide a detailed, professional response based on the context and your legal expertise.`
  ...
  private formatContextForPrompt()
    ...
    let contextText = "";
    if (vectorContext.length > 0) {
      contextText += "Relevant Previous Conversations (similarity-based):\n";
      vectorContext.forEach((item, index) => {
        const similarity = ((item as { similarity?: any; role?: any; content?: any }).similarity * 100).toFixed(1);
        contextText += `${index + 1}. [${(item as { similarity?: any; role?: any; content?: any }).role.toUpperCase()} - ${similarity}% similar] ${(item as { similarity?: any; role?: any; content?: any }).content.substring(0, 200)}...\n`;
      });
      contextText += "\n";
    }
    if (grpoRecommendations && grpoRecommendations.length > 0) {
      contextText += "Legal Reasoning Recommendations (GRPO Analysis):\n";
      grpoRecommendations.forEach((rec, index) => {
        const similarity = (rec.similarity * 100).toFixed(1);
        const recency = (rec.recencyScore * 100).toFixed(1);
        contextText += `${index + 1}. [Reasoning Chain - ${similarity}% similar, ${recency}% recent]\n`;
  ...
  // GRPO recommendation search
  private async performGrpoRecommendationSearch()
    query: string
    conversationId?: string;
    ...
    try {
      ...
      console.error('GRPO recommendation search error:', error);
      return [];
    }
  }
  // Detect if response contains legal thinking patterns
  private isLegalThinkingResponse(response,: string): boolean {
    const thinkingPatterns = [
      ...
      /legal analysis/i,
      /constitutional.*requires/i,
      /burden of proof/i,
      /reasonable.*standard/i
    ];
    const hasThinkingPatterns = thinkingPatterns.some(pattern => pattern.test(response);
    const hasLegalTerms = (response as { length?: any; includes?: any; split?: any; match?: any }).length > 200; // Substantial response
    const hasLogicalStructure = (response as { length?: any; includes?: any; split?: any; match?: any }).includes('.') && (response as { length?: any; includes?: any; split?: any; match?: any }).split('.').length >= 3;
    return hasThinkingPatterns && hasLegalTerms && hasLogicalStructure;
  }
  ...
  private async captureThinkingResponse()
    ...
    try {
      console,.log('Capturing GRPO thinking (response as { length?: any; includes?: any; split?: any); match?: any })...');
      // Extract conclusion (last meaningful sentence)
      const sentences = fullResponse.split(/[.!?]+/).filter(item => item.length) > 2,0);
      const conclusion = sentences.slice(-2).join('. ').trim() + '.,';
      // Extract reasoning steps (sentences with logical indicators)
      const reasoningSteps = sentences.filter(sentence => {
        const s = sentence.toLowerCase();
        return s.includes('because') || s.includes('therefore') ||;
               s.includes('since') || s.includes('given') ||
               s.includes('based on') || s.includes('due to') ||
               s.includes('pursuant to') || s.includes('under');
      }).slice(0, 8); // Limit to 8 steps
      // Extract legal principles and evidence
      ...
      const grpoRespons,e: GrpoThinkingResponse = {
        conversationId,
        messageId: `grpo_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`,
        originalQuery,
        thinkingChain: fullResponse,
        conclusion,
        confidenceLevel,
        reasoningSteps,
        evidenceCited,
        legalPrinciples,
        thinkingType,
        embedding: [], // Will be generated in storeGrpoThinkingResponse
        metadata: {
          model,
          complexity: this.determineComplexity(fullResponse),
          timestamp: new Date().toISOString(),
          processingTime: 0 // Will be calculated in service
        }
      }
      await storeGrpoThinkingResponse(grpoResponse);
      console,.log('GRPO thinking response captured successfully');
    } catch (error: any) {
      console.error('Failed to capture GRPO thinking response:', error);
      // Don't throw - this is a non-critical enhancement
    }
  }
  // Extract legal principles from response
  private extractLegalPrinciples(text,: string): string[,] {
    const principles = [];
    // Pattern matching for legal principles
    const principlePatterns = [
      /the principle that .+?[.!?]/gi,
      /under the doctrine of .+?[.!?]/gi,
      /pursuant to .+?[.!?]/gi,
      /the law requires .+?[.!?]/gi,
      /constitutional principle .+?[.!?]/gi,
      /legal standard .+?[.!?]/gi,
      /burden of proof .+?[.!?]/gi
    ];
    principlePatterns.forEach(pattern => {
      ...
      if (matches) {
        principles.push(...matches.map(m => m.trim();
      }
    });
  ...
  private extractEvidenceCitations(text,: string): string[,] {
    ...
    const citationPatterns = [
      /\b\w+ v\. \w+.+?(?:\d{4})/gi, // Case citations
      /\d+ U\.S\.C\.? (?:§|Section) \d+/gi, // USC citations
      /Fed\.? R\.? (?:Civ\.? P\.?|Crim\.? P\.?|Evid\.?) \d+/gi, // Federal Rules
      /\d+ F\.\d*d \d+/gi, // Federal reporter citations
      /\d+ S\.Ct\. \d+/gi // Supreme Court citations
    ];
    ...
    return [...new Set(citations)].slice(0, 10); // Remove duplicates, limit to 10
  }
  // Calculate confidence level for thinking response
  private calculateThinkingConfidence(response,: string, reasoningStepsCoun,t: numbe,r): number {
    let confidence = 0.5; // Base confidence
    // Length factor (longer responses generally more comprehensive)
    const lengthFactor = Math.min((response as { length?: any; includes?: any; split?: any); match?: any }).length / 2000, 1) * 0.2;
    confidence += lengthFactor;
    // Reasoning steps factor
    const reasoningFactor = Math.min(reasoningStepsCount / 5, 1) * 0.2;
    confidence += reasoningFactor;
    // Legal citation factor
    const hasCitations = /\b\w+ v\. \w+|\d+ U\.S\.C|Fed\. R\./.test(response);
    if (hasCitations) confidence += 0.15;
    // Logical structure factor
    const logicalIndicators = ((response as { length?: any; includes?: any; split?: any; match?: any }).match(/therefore|consequently|thus|hence|accordingly/gi) || []).length;
  ...
  // Determine complexity of thinking response
  private determineComplexity(response,: string): 'low' | 'medium' | 'high,' {
    const factors = {
      length: (response as { length?: any; includes?: any; split?: any; match?: any }).length,
      sentences: ((response as { length?: any; includes?: any; split?: any; match?: any }).match(/[.!?]+/g) || []).length,
      legalTerms: ((response as { length?: any; includes?: any; split?: any; match?: any }).match(/statute|regulation|precedent|constitutional|jurisprudence|doctrine/gi) || []).length,
      citations: ((response as { length?: any; includes?: any; split?: any; match?: any }).match(/\b\w+ v\. \w+|\d+ U\.S\.C/gi) || []).length
    }
    const complexityScore =;
      (factors.length > 1500 ? 2 : factors.length > 800 ? 1 : 0) +
...
export async function* ollamaChatStream()
  ...
  yield,* servic,e.streamChat(option,s);
}
