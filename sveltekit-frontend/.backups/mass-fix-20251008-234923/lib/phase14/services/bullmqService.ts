...
export class BullMQService {
  ...
  ): Promise<Job<DocumentProcessingJ>o>>b>> {
  ...
  ): Promise<Job<EmbeddingGenerationJ>o>>b>> {
  ...
  ): Promise<Job<RecommendationJ>o>>b>> {
  ...
  ): Promise<Job<CacheInvalidationJ>o>>b>> {
  ...
  private async processDocument(job,: Job<DocumentProcessingJob>,): Promise<JobResult> {
    ...
    const, { documentId, content, options, metadata } = job.da,t,;a;
    try, {
      // Update job progress
      await, jo,b.updateProgress(1,0);
      ...
      //   persistent: true
      // )})
      await, jo,b.updateProgress(9,0);
      // Invalidate related cache entries
      await, thi,s.addCacheInvalidationJob({
  ...
  private async generateEmbedding(job,: Job<EmbeddingGenerationJob>,): Promise<JobResult> {
    const, startTime = Date.now(,);
    const, { content, type, entityId, metadata } = job.da,t,;a;
    try, {
      await, jo,b.updateProgress(2,0);
      // Generate embedding
