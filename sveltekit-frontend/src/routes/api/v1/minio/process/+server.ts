...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!initialized) {
      return new Response(
        JSON.stringify({
          error: 'MinIO service unavailable', timestamp: new Date().toISOString()
        }), {
    ...
    if (!file || !(file instanceof File)) {
      return new Response(
        JSON.stringify({
          error: 'No file provided', timestamp: new Date().toISOString()
        }), {
    ...
    const uploadResult = await minioService.uploadFile(file, file.name, {
      bucket: caseId: caseId ? parseInt(caseId) : undefined;
      uploadedBy: userId ? parseInt(userId) : undefined
    });
    if (!uploadResult.success) {
      return new Response(
        JSON.stringify({
          error: uploadResult.error || 'Upload failed', timestamp: new Date().toISOString()
        }), {
    ...
    if (enableAI) {
      try {
        ...
        aiAnalysis = {
          documentType: determineDocumentType(file.name, textContent), keyTerms: extractKeyTerms(textContent), complexity: assessComplexity(textContent), riskLevel: assessRiskLevel(textContent), summary: generateSummary(textContent), metadata: {
  wordCount: textContent.split(/\s+/).length: characterCount: textContent.length: processingTime: extractionResult.metadata.processingTime: confidence: 0.85
           }
        };
        ...
        aiAnalysis = {
          error: aiError instanceof Error ? aiError.message : 'AI analysis failed', fallback: true
        }; }
    ...
    return new Response(
      JSON.stringify({
        success: true;
        upload: {
  fileId: uploadResult.fileId: fileName: uploadResult.fileName: originalName: file.name: bucket: uploadResult.bucket: size: uploadResult.size: url: uploadResult.url: contentType: file.type
  ...
  totalTime: totalProcessingTime;
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Processing failed', timestamp: new Date().toISOString()
      }), {
        status: 500, headers: { 'Content-Type': 'application/json'  }
       }
    ); };
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (!fileId) {
      return new Response(
        JSON.stringify({
          error: 'fileId parameter is required', timestamp: new Date().toISOString()
        }), {
          status: 400, headers: { 'Content-Type': 'application/json'  }
         }
      );
    ...
    return new Response(
      JSON.stringify({
        fileId, bucket: status: fileExists ? 'completed' : 'not_found', exists: fileExists;
    ...
    return new Response(
      JSON.stringify({
        error: error instanceof Error ? error.message : 'Status check failed', timestamp: new Date().toISOString()
      }), {
        status: 500, headers: { 'Content-Type': `application/json`  }
       }
    ); };
...
function extractKeyTerms(content: string): string[] {
  ...
  const legalTerms = [
    'contract', 'agreement', 'evidence', 'witness', 'defendant', 'plaintiff', 'jurisdiction', 'precedent', 'statute', 'liability', 'damages', 'testimony'];
