...
export class QdrantSyncWorker {
  ...
  private async processBatch(): Promise<{ synced: number; errors: number }> {
    ...
    const chunks = await getChunksPendingQdrantSync(
      this.db,
      ...
      while (attempts < this.config.retryAttempts && !success) {
        try {
          ...
          if (attempts >= this.config.retryAttempts) {
            console.error(`❌ Failed to sync chunk ${chunk.id} after ${attempts} attempts:`, errorMessage);
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command === 'once') {
    // Run once and exit
    worker.syncAll().then(({ synced, errors }) => {
