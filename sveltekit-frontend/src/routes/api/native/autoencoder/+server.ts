...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    try {
      const { runAutoencoder } = await import('$lib/wasm/autoencoder-wasm');
      const output = await runAutoencoder(input);
      return json({ success: true, output });
    } catch (wasmErr) {
      return json({ success: false, error: String(wasmErr) }, { status: 500 });
    }
  }
};
