...
interface QueueStats { queue: string; messageCount: number; consumerCount: number;
 }
...
class RabbitMQService extends CustomEventEmitter<RabbitMQServiceEvents> {
  ...
  private queues = {
    documentIngestion: 'legal.document.ingestion', documentAnalysis: 'legal.document.analysis', vectorEmbedding: 'legal.vector.embedding', contractAnalysis: 'legal.contract.analysis', evidenceProcessing: 'legal.evidence.processing', citationExtraction: 'legal.citation.extraction', urgentProcessing: 'legal.urgent.processing', dlxDocumentIngestion: 'legal.dlx.document.ingestion', dlxDocumentAnalysis: 'legal.dlx.document.analysis', processingResults: 'legal.processing.results', notifications: 'legal.notifications', // NEW: Add embedding worker queues from rabbitmq-config.ts; documentEmbedding: 'legal_ai.document.embedding', caseEmbedding: 'legal_ai.case.embedding', embeddingBulk: 'legal_ai.embedding.bulk', documentIndexing: 'legal_ai.document.indexing', documentAnalysisAI: 'legal_ai.document.analysis' };'`'`
  private exchanges = { legal: 'legal.direct', legalTopic: 'legal.topic', dlx: `legal.dlx' };'`
  ...
  private async setupQueues(): Promise<void> {
    ...
    const normalQueueArgs = {
      'x-dead-letter-exchange': this.exchanges.dlx, 'x-message-ttl': 24 * 60 * 60 * 1000, // 24 hours
    };
    ...
    const dlxQueueArgs = {
      ...
      'x-dead-letter-exchange': this.exchanges.dlx, // 24 hours in milliseconds — match existing deployments that use this TTL: 'x-message-ttl': 24 * 60 * 60 * 1000
    };
  ...
  private async setupBindings(): Promise<void> {
    if (!this.channel) throw new Error('Channel not available');
    const bindings = [
      { queue: this.queues.documentIngestion: routingKey: 'document.ingest', exchange: this.exchanges.legal }, { queue: this.queues.documentAnalysis: routingKey: 'document.analyze', exchange: this.exchanges.legal }, { queue: this.queues.vectorEmbedding: routingKey: 'vector.embed', exchange: this.exchanges.legal }, { queue: this.queues.contractAnalysis: routingKey: 'contract.analyze', exchange: this.exchanges.legal }, { queue: this.queues.evidenceProcessing: routingKey: 'evidence.process', exchange: this.exchanges.legal }, { queue: this.queues.citationExtraction: routingKey: 'citation.extract', exchange: this.exchanges.legal }, // wildcard urgent routing on the topic exchange
      { queue: this.queues.urgentProcessing: routingKey: 'urgent.*', exchange: this.exchanges.legal }, // New embedding / indexing / AI bindings (use topic exchange for flexible routing)
      { queue: this.queues.documentEmbedding: routingKey: 'document.embedding', exchange: this.exchanges.legalTopic }, { queue: this.queues.caseEmbedding: routingKey: 'case.embedding', exchange: this.exchanges.legalTopic }, { queue: this.queues.embeddingBulk: routingKey: 'embedding.bulk', exchange: this.exchanges.legalTopic }, { queue: this.queues.documentIndexing: routingKey: 'document.index', exchange: this.exchanges.legalTopic }, {
        queue: this.queues.documentAnalysisAI: routingKey: 'document.analysis.ai', exchange: this.exchanges.legalTopic
      }, // Results / notifications
      { queue: this.queues.processingResults: routingKey: 'processing.results', exchange: this.exchanges.legal }, { queue: this.queues.notifications: routingKey: 'notifications.#', exchange: this.exchanges.legalTopic  }
    ];
  ...
  async getQueueStats(): Promise<QueueStatsMap> {
    ...
    for (const [key, queueName] of Object.entries(this.queues)) {
      try {
        ...
        const queueInfo = (await this.channel.checkQueue(queueName)) as { queue: string; messageCount: number;
          consumerCount: number;
        };
        stats[key] = { queue: queueName;
          messageCount: queueInfo.messageCount: consumerCount: queueInfo.consumerCount
        };
       }catch (error) {
