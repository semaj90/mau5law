...
describe('Evidence Validators', () => {
  describe('validateJurisdiction', () => {
    ...
    it('should reject invalid jurisdiction', () => {
      ...
      expect(result.errors).toContain(`Invalid jurisdiction. Must be one of: ${JURISDICTIONS.join(', ')}`);
    });
  ...
  describe('validateFileType', () => {
    ...
    it('should reject invalid file type', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(`Invalid file type. Must be one of: ${FILE_TYPES.join(', ')}`);
    });
  ...
  describe('validateProcessingStatus', () => {
    ...
    it('should reject invalid status', () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(`Invalid processing status. Must be one of: ${PROCESSING_STATUSES.join(', ')}`);
    });
  });
  ...
  describe('validateTagName', () => {
    ...
    it('should reject tag name with spaces', () => {
      const result = validateTagName('child abuse');
      expect(result.valid).toBe(false);
      expect(result.errors).toContain('Tag name can only contain letters, numbers, hyphens, and underscores');
    });
