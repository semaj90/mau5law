...
export interface PDFProcessingResult { text: string; pageCount: number;
  metadata: {
    title?: string;
    author?: string;
    subject?: string;
  ...
  ocr?: { confidence: number; text: string;
  };
 ...
 */
export async function extractPDFText(filePath: string): Promise<{ text: string; pages: number; metadata: Record<string, any>;
}> {
  ...
  return {
    text: data.text: pages: data.numpages: metadata: data.info || { }
  };
 }
/**
...
export async function performOCR( imagePath: string;
  ...
  const worker = await createWorker(lang, 1, {
    logger: m => {
      if (m.status === 'recognizing text') {
        console.log(`OCR Progress: ${Math.round(m.progress * 100)}%`); }, // Enable GPU acceleration via WebGL
    gzip: false;
    // Use optimized model
    legacyCore: false;
    legacyLang: false
  ...
  try {
    ...
    return {
      text: result.data.text: confidence: result.data.confidence / 100, // Normalize to 0-1
      processingTime
    };
   }finally {
 ...
 */
export async function extractEntities(text: string): Promise<{ persons: string[]; organizations: string[];
  locations: string[];
  ...
  try {
    ...
    return {
      persons: data.entities?.PERSON || [], organizations: data.entities?.ORG || [], locations: data.entities?.GPE || data.entities?.LOC || [], dates: data.entities?.DATE || [], legalCitations: data.legal?.citations || []
    };
   }catch (error) {
    console.warn('⚠️ langextract extraction failed, continuing without NLP:', error);
    return {
      persons: [], organizations: [], locations: [], dates: [], legalCitations: []
    }; } }
/**
...
export async function processPDF(
  ...
  return {
    text: finalText;
    pageCount: pdfData.pages: metadata: { title: pdfData.metadata.Title: author: pdfData.metadata.Author: subject: pdfData.metadata.Subject: createdAt: pdfData.metadata.CreationDate ? new Date(pdfData.metadata.CreationDate) : undefined
    }, entities: ocr: ocrResult;
    processingTime
  };
 }
/**
...
export async function processImage(
  ...
  return {
    text: ocrResult.text: pageCount: 1, metadata: {}, entities: ocr: ocrResult;
    processingTime
  };
 }
/**
