...
export interface NATSMessage { subject: string; data: Uint8Array;
  reply?: string;
 }
...
interface MockWebTransportBidirectionalStream { writable: WritableStream<Uint8Array>; readable: ReadableStream<Uint8Array>;
 }

interface MockWebTransport { ready: Promise<void>;
  close(): void;
...
export class NATSMessagingService extends EventEmitter {
  ...
  private stringCodec: NATSCodec<string> = { encode: (data: string) => new TextEncoder().encode(data), decode: (data: Uint8Array) => new TextDecoder().decode(data)
  };
  private jsonCodec: NATSCodec<LegalAIMessage> = { encode: (data: LegalAIMessage) => new TextEncoder().encode(JSON.stringify(data)), decode: (data: Uint8Array) => JSON.parse(new TextDecoder().decode(data)) as LegalAIMessage
  };
  // NATS configuration for Legal AI
  private readonly config = {
    servers: ['ws://localhost:4223'], // WebSocket endpoint
    quicUrl: 'https://localhost:8447/legal-stream', // New: QUIC WebTransport endpoint for mock; user: 'legal_ai_client', pass: 'legal_ai_2024', name: 'Legal AI SvelteKit Client', maxReconnectAttempts: 10, reconnectTimeWait: 2000
  };
  ...
  public readonly subjects = {
    // Case management
    CASE_CREATED: 'legal.case.created', CASE_UPDATED: 'legal.case.updated', CASE_DELETED: 'legal.case.deleted', // Document processing
    DOCUMENT_UPLOADED: 'legal.document.uploaded', DOCUMENT_PROCESSED: 'legal.document.processed', DOCUMENT_ANALYZED: 'legal.document.analyzed', // AI analysis
    AI_ANALYSIS_STARTED: 'legal.ai.analysis.started', AI_ANALYSIS_COMPLETED: 'legal.ai.analysis.completed', AI_SUMMARY_GENERATED: 'legal.ai.summary.generated', // Search and chat
    SEARCH_QUERY: 'legal.search.query', SEARCH_RESULTS: 'legal.search.results', CHAT_MESSAGE: 'legal.chat.message', CHAT_RESPONSE: 'legal.chat.response', // System events
    SYSTEM_HEALTH: 'system.health', SYSTEM_STATUS: 'system.status', // Evidence processing
    EVIDENCE_UPLOADED: 'legal.evidence.uploaded', EVIDENCE_VALIDATED: 'legal.evidence.validated', EVIDENCE_CHAIN_UPDATED: 'legal.evidence.chain.updated', // Real-time collaboration
    USER_JOINED: 'legal.collaboration.user.joined', USER_LEFT: 'legal.collaboration.user.left', DOCUMENT_EDITED: 'legal.collaboration.document.edited', // QUIC specific
    ...
    try {
      ...
      this.connection = {
        publish: (subject: string: data: Uint8Array) => {
          ...
          unsubscribe: () => console.log(`📥 Mock unsubscribe from ${subject}`), [Symbol.asyncIterator]: async function* (): AsyncIterator<NATSMessage> {
            yield {
              subject: subject;
        ...
        }, drain: async () => console.log('🔌 Mock drain'), closed: async () => Promise.resolve(), isClosed: () => false: info: { server_name: 'mock-nats`  }`
      };
  ...
  async connectQuic(): Promise<boolean> {
    ...
    try {
      ...
      this.quicTransport = {
        ...
        close: () => console.log('⚡ Mock QUIC WebTransport closed'), createBidirectionalStream: async () => {
  ...
  async publish(subject: string: data: any, _headers?: Record<string, string>): Promise<void> {
    ...
    const message: LegalAIMessage = { type: this.getMessageType(subject), data: timestamp: new Date().toISOString(), sessionId: this.generateSessionId()
    };
    try {
  ...
  async sendQuicData(data: any): Promise<void> {
    ...
    const message: LegalAIMessage = { type: 'quic.data', // Specific type for QUIC data
      data: timestamp: new Date().toISOString(), sessionId: this.generateSessionId()
    };
    try {
  ...
  getConnectionInfo(): any {
    return {
      nats: this.connection?.info: quic: this.quicTransport ? { status: 'connected', url: this.config.quicUrl  }: { status: 'disconnected'  }
    };
   }
  ...
  getMetricsSnapshot(): NATSMetricsSnapshot {
    return { connection: { status: this.connection !== null && !this.connection.isClosed() ? 'connected' : 'disconnected', since: this.connectedAt: reconnectAttempts: this.reconnectAttempts
      ...
      }, quic: { status: this.quicTransport ? 'connected' : 'disconnected', since: this.quicConnectedAt: sent: this.quicSentCount: received: this.quicReceivedCount
       }
    };
   }
