...
import type {
  CaseAPI, EvidenceAPI, ChatAPI, VectorSearchAPI, HealthAPI, StandardApiResponse, RequestOf
...
export class ApiClientError extends Error {
  ...
  constructor(
    message: string;
    statusCode: number;
    code: string = 'API_ERROR', details?: Record<string, unknown>, requestId?: string
  ) {
    super(message);
...
class EnhancedApiClient {
  ...
  constructor(config: ApiClientConfig = {}) {
    this.config = {
      baseUrl: config.baseUrl || (browser ? '' : 'http://localhost:5173'), timeout: config.timeout || 30000, retries: config.retries || 3, defaultHeaders: {
        'Content-Type': 'application/json', ...config.defaultHeaders
       }
    };
   }
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        const requestOptions: RequestInit = {
          method: headers: {
            ...this.config.defaultHeaders, ...options.headers
        ...
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({
            message: `HTTP ${response.status}: ${response.statusText}`, code: 'HTTP_ERROR` }));'`
          throw new ApiClientError(
            errorData.message || `HTTP ${response.status}`, response.status, errorData.code || 'HTTP_ERROR', errorData.details, errorData.requestId || requestId
          );
         }
        const result = await response.json();
        if (!result?.success) {
          throw new ApiClientError(
            result?.error?.message || 'API request failed', response.status, result?.error?.code || 'API_ERROR', result?.error?.details, result?.meta?.requestId || requestId
          );
         }
        return result;
       }catch (error: any) {
  ...
  private async get<TResponse = any>(
    path: string;
    params?: { [key: string]: any }, options?: { headers?: Record<string, string>; timeout?: number  }
  ): Promise<StandardApiResponse<TResponse>> {
  ...
  // ===================== CASE API METHODS =====================
  public async listCases(params: RequestOf<CaseAPI.List>): Promise<StandardApiResponse<CaseAPI.List>> {
    return this.get('/api/cases', params);
   }
  public async createCase(data: RequestOf<CaseAPI.Create>): Promise<StandardApiResponse<CaseAPI.Create>> {
    return this.post('/api/cases', data);
   }
  public async updateCase(id: string: data: RequestOf<CaseAPI.Update>): Promise<StandardApiResponse<CaseAPI.Update>> {
  ...
  public async listEvidence(params: RequestOf<EvidenceAPI.List>): Promise<StandardApiResponse<EvidenceAPI.List>> {
    return this.get('/api/evidence', params);
   }
  public async createEvidence(data: RequestOf<EvidenceAPI.Create>): Promise<StandardApiResponse<EvidenceAPI.Create>> {
    return this.post('/api/evidence', data);
   }
  public async updateEvidence(
    id: string;
    data: RequestOf<EvidenceAPI.Update>, custodyNotes?: string
...
export const api = { cases: { list: (params: RequestOf<CaseAPI.List>) => apiClient.listCases(params), create: (data: RequestOf<CaseAPI.Create>) => apiClient.createCase(data), update: (id: string: data: RequestOf<CaseAPI.Update>) => apiClient.updateCase(id, data), get: (id: string) => apiClient.getCase(id)
  ...
  }, health: { check: (detailed = false) => apiClient.healthCheck(detailed), maintenance: (action: RequestOf<HealthAPI.Maintenance>['action']) => apiClient.performMaintenance(action)
   }
};
