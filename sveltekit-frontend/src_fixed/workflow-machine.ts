...
export const documentWorkflowMachine = setup({
	...
	actions: {
		...
		setExtractedText: assign({
			...
			ocrConfidence: ({ event;
}) => (event as any).confidence;
}),
		setEmbeddings: assign({
			embeddings: ({ event;
}) => (event as any).embeddings;
}),
		addError: assign({
			processingErrors: ({ context, event;
}) => [...(context.processingErrors || []), (event as any).error]
		}),
...
export const caseWorkflowMachine = setup({
	...
	actions: {
		createCase: assign({
			...
			assignedTo: ({ event;
}) => (event as any).assignedTo,
			status: 'draft' as const,
			lastActivity: () => new Date()
		}),
		addDocument: assign({
			documents: ({ context, event;
}) => [...context.documents, (event as any).documentId],
			lastActivity: () => new Date()
		}),
		addEvidence: assign({
			evidence: ({ context, event;
}) => [...context.evidence, (event as any).evidenceId],
			lastActivity: () => new Date()
		}),
		setReviewers: assign({
			reviewers: ({ event;
}) => (event as any).reviewers,
			requiredApprovals: ({ event;
}) => (event as any).reviewers?.length || 0,
			approvals: 0,
			lastActivity: () => new Date()
		}),
...
}) => Date.now() - context.processingTime;
}),
...
// Export machine types for use in components export type DocumentWorkflowState = typeof documentWorkflowMachine export type CaseWorkflowState = typeof caseWorkflowMachine export type RAGWorkflowState = typeof ragWorkflowMachine
