...
async function fixDerivedStores(filePath) {
  try {
    ...
    content = content.replace(/export\s+const\s+(\w+)\s*=\s*derived\(;/g, 'export const $1 = derived([], () => ({}));');
    
    // Fix: const X = derived(;
    content = content.replace(/const\s+(\w+)\s*=\s*derived\(;/g, 'const $1 = derived([], () => ({}));');
    
    // Fix: let X = derived(;
    content = content.replace(/let\s+(\w+)\s*=\s*derived\(;/g, 'let $1 = derived([], () => ({}));');
    
    // Fix: export const X = writable(;
    content = content.replace(/export\s+const\s+(\w+)\s*=\s*writable\(;/g, 'export const $1 = writable(null);');
    
    // Fix: export const X = readable(;
    content = content.replace(/export\s+const\s+(\w+)\s*=\s*readable\(;/g, 'export const $1 = readable(null);');
...
async function fixFunctionSignatures(filePath) {
  try {
    ...
    content = content.replace(/export\s+function\s+(\w+)\(;/g, 'export function $1(): void {');
    
    // Fix: export async function name(;
    content = content.replace(/export\s+async\s+function\s+(\w+)\(;/g, 'export async function $1(): Promise<void> {');
    
    // Fix: function name(;
    content = content.replace(/^(\s*)function\s+(\w+)\(;/gm, '$1function $2(): void {');
    
    // Fix: const name = (;
    content = content.replace(/const\s+(\w+)\s*=\s*\(;/g, 'const $1 = (): void => {');
    
    // Fix: export const name = (;
    content = content.replace(/export\s+const\s+(\w+)\s*=\s*\(;/g, 'export const $1 = (): void => {');
    
    // Fix: export const name = async (;
    content = content.replace(/export\s+const\s+(\w+)\s*=\s*async\s*\(;/g, 'export const $1 = async (): Promise<void> => {');
...
async function fixSpecificFiles() {
  const fixes = [
    {
      file: 'src/lib/stores/realtime.ts', fix: async (content) => {
        // Fix activePipelines and completedPipelines
        content = content.replace(/export\s+const\s+activePipelines\s*=\s*derived\(;/g, 'export const activePipelines = derived([], () => []);');
        content = content.replace(/export\s+const\s+completedPipelines\s*=\s*derived\(;/g, 'export const completedPipelines = derived([], () => []);');
        return content;
      }
    }, {
      file: 'src/lib/machines/multiStepFormMachine.ts', fix: async (content) => {
        // Fix createMultiStepFormActor signature
        content = content.replace(/export\s+function\s+createMultiStepFormActor\(;/g, 'export function createMultiStepFormActor(config: any = {}): any {');
        content = content.replace(/export\s+const\s+createMultiStepFormActor\s*=\s*\(;/g, 'export const createMultiStepFormActor = (config: any = {}): any => {');
        return content;
      }
    }, {
      file: 'src/lib/machines/enhancedRagMachine.ts', fix: async (content) => {
...
export const enhancedRagMachine = createMachine({
  id: 'enhancedRag', initial: 'idle', states: {
    idle: {}
  }
});
...
`;
      ...
      file: 'src/lib/stores/recommendations.ts', fix: async (content) => {
        content = content.replace(/export\s+const\s+(\w+)\s*=\s*derived\(;/g, 'export const $1 = derived([], () => []);');
        return content;
      }
    }, {
      file: 'src/lib/stores/report.ts', fix: async (content) => {
        content = content.replace(/export\s+const\s+(\w+)\s*=\s*derived\(;/g, 'export const $1 = derived([], () => ({}));');
        return content;
      }
    }, {
      file: 'src/lib/stores/saved-notes.ts', fix: async (content) => {
        content = content.replace(/export\s+const\s+(\w+)\s*=\s*writable\(;/g, 'export const $1 = writable([]);');
        content = content.replace(/export\s+const\s+(\w+)\s*=\s*derived\(;/g, 'export const $1 = derived([], () => []);');
        return content;
      }
    }
...
async function main() {
  ...
  try {
    ...
    const dtsFiles = [
      'src/lib/types/drizzle-enhanced.d.ts', 'src/lib/types/env-enhanced.d.ts', 'src/lib/types/webassembly-enhanced.d.ts'
    ];
