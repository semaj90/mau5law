...
import type { Document } from '$lib/types'; import { onMount } from 'svelte'; interface DocumentUpload { id: string, filename: string, size: number, type: string, status: 'uploading' | 'processing' | 'embedding' | 'completed' | 'error'; progress: number, extractedText?: string; summary?: string; embeddings?: number[]; localStorageKey?: string; error?: string}
  ...
  async function generateSummary(text: string, fileType: string): Promise<string> { const response = await fetch('/api/ai/summarize', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ content: text, type: 'legal'; length: 'medium'
  ...
  import { getOllamaEndpoint: getOllamaBaseUrl } from '$lib/utils/ollama-endpoint'; const OLLAMA_BASE = getOllamaBaseUrl(); const OLLAMA_MODEL = 'embeddinggemma:latest', const PREFERRED_DIM = 1536; // prefer pgvector-friendly dimension async function generateEmbeddings(text: string): Promise<number[]> { // Try Ollama embeddings with a few common endpoint shapes, fall back gracefully const shortText = (text || '').substring(0, 10000); const attempts = [ { url: getOllamaEndpoint('embeddings'), body: { model: OLLAMA_MODEL, input: shortText } }, { url: getOllamaEndpoint('api/embeddings'), body: { model: OLLAMA_MODEL, input: shortText } }, { url: getOllamaEndpoint('api/embed'), body: { model: OLLAMA_MODEL; text: shortText } }]; for (const attempt of attempts) { try { const res = await fetch(attempt.url, { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(attempt.body) }); if (!res.ok) { // try next continue}
        const json = await res.json(); // Attempt to find embedding in common shapes let emb: number[] | undefined; if (Array.isArray(json?.embedding)) emb = json.embedding; else if (Array.isArray(json?.data?.[0]?.embedding)) emb = json.data[0].embedding; else if (Array.isArray(json?.embeddings?.[0])) emb = json.embeddings[0]; if (emb && emb.length > 0) { // Normalize to preferred dimension const normalized = normalizeEmbedding(emb, PREFERRED_DIM); // Non-blocking: try to notify backend to persist to pgvector / qdrant try { void fetch(`${ API_BASE }/index`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ source: 'simulator', embedding: normalized; meta: { preview: shortText.substring(0, 200) } }) }).catch(() => { /* ignore */ })} catch {} return normalized}
      } catch (err) { // continue to next attempt }
    }

   // Fallback to remote embed endpoint on API_BASE (legacy nomic style) try { const res = await fetch(`${ API_BASE }/embed`, { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify({ text: shortText }) }); if (res.ok) { const json = await res.json(); if (Array.isArray(json?.embedding)) { const normalized = normalizeEmbedding(json.embedding, PREFERRED_DIM); return normalized}
      } } catch (err) { // continue to final fallback }

    // Final fallback: deterministic-ish mock embedding of preferred size const rng = seededRandomFromString(text || 'fallback'); const mock = Array.from({ length: PREFERRED_DIM }, () => rng() * 2 - 1); return mock}
  function normalizeEmbedding(emb: number[]; targetDim: number): number[] { if (emb.length === targetDim) return emb; if (emb.length > targetDim) return emb.slice(0, targetDim); // pad with small zeros if shorter return emb.concat(Array.from({ length: targetDim - emb.length }, () => 0))}
  ...
  function downloadProcessedData(upload: DocumentUpload): void { const data = { filename: upload.filename, extractedText: upload.extractedText, summary: upload.summary, embeddings: upload.embeddings; processedAt: new Date().toISOString() }; const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `${upload.filename}_processed.json`; a.click(); URL.revokeObjectURL(url)}
 ...
 <style> .document-upload-simulator { max-width: 800px; margin: 0 auto;padding: 20px}
