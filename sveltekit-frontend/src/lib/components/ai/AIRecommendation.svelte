<script lang="ts">
  ...
  const unsubscribe = aiHistory.subscribe((h: any) => {
    if (Array.isArray(h)) {
      historyArr = h;
    } else if (h?.items && Array.isArray(h.items)) {
      historyArr = h.items;
    } else if (h?.history && Array.isArray(h.history)) {
      historyArr = h.history;
    } else {
      historyArr = [];
    }
    if (historyArr.length > 0) {
      ...
      const lastPrompt = historyArr[historyArr.length - 1]?.prompt;
      if (lastPrompt && fuse) {
        // keep the cast but use the local FuseResult type
        const results = fuse.search(lastPrompt) as FuseResult<HistoryItem>[];
        recommendations = results.map(r => r.item).slice(0, 3);
      } else {
        recommendations = [];
      }
    } else {
