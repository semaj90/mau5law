...
export const enhanced_documents = pgTable('enhanced_documents', {
  id: uuid('id').defaultRandom().primaryKey(), ownerId: text('owner_id').notNull(), title: text('title'), content: text('content'), language: text('language').default('en'), source: text('source'), status: text('status').default('active'), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(), updatedAt: timestamp('updated_at', { withTimezone: true }).defaultNow(), metadata: jsonb('metadata').$type<Record<string, unknown> | null>()
});
...
export const enhanced_embeddings = pgTable('enhanced_embeddings', {
  id: serial('id').primaryKey(), documentId: uuid('document_id')
    .references(() => enhanced_documents.id)
    .onDelete('CASCADE'), model: text('model').notNull().default('nomic-embed-text'), embedding: vector('embedding', { dimensions: 1536 }), // adjust dims to your embedding model
  vectorChecksum: text('vector_checksum'), // optional checksum for deduping
  confidence: real('confidence').default(1.0), tokens: integer('tokens').default(0), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(), additional: jsonb('additional').$type<Record<string, unknown> | null>()
});
...
export const enhanced_search_cache = pgTable('enhanced_search_cache', {
  id: serial('id').primaryKey(), queryHash: text('query_hash').notNull().unique(), results: jsonb('results').$type<Array<Record<string, unknown>> | null>(), model: text('model').notNull(), createdAt: timestamp('created_at', { withTimezone: true }).defaultNow(), expiresAt: timestamp('expires_at', { withTimezone: true })
});
