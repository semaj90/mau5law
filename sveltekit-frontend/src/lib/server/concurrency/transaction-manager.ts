...
export class TransactionManager {
	...
	async withTransaction<T>(
		...
		try {
			// Using sql.begin helper from postgres.js which is typically used in $lib/database/connection
			// Assuming sql helper supports .begin(async sql => { ... })
			return await sql.begin(async (tx) => {
				// Set isolation level if possible within the transaction scope or client configuration
				// Note: postgres.js transaction options are usually set in the .begin call arguments or via SET commands
				if (isolationLevel) {
					await tx`SET TRANSACTION ISOLATION LEVEL ${sql.unsafe(isolationLevel)}`;
				}

                // Set timeout if provided (careful as this affects the connection/statement level)
				if (timeout) {
                    // Setting statement timeout for this transaction
	...
	private async cleanupTransaction(transactionId: string): Promise<void> {
		...
		// Release unknown locks that might still be held
