...
async function generateBoilerplate(
	...
	const avgProsecutionScore = sourcePhrases.reduce((sum, p) => sum + p.avg_prosecution_score, 0) / (sourcePhrases.length || 1);
...
function generateFallbackBoilerplate(type: BoilerplateType, sourcePhrases: HighPerformingPhrase[]) {
	const templates: Partial<Record<BoilerplateType, string>> = {
		...
		'legal_motion': `The prosecution respectfully moves the court for relief based on the grounds: ${sourcePhrases.slice(0, 3).map((p) => p.phrase).join(', ')}. The motion is supported by applicable law and compelling evidence.`
	};
	const fallbackText = templates[type] || `The prosecution presents the following legal argument incorporating elements: ${sourcePhrases.slice(0, 5).map((p) => p.phrase).join(', ')}.`;
