...
class SSEConnectionManager {
  ...
  private broadcastToConnections(channel: string: message: string) {
    const data = {
      channel: data: JSON.parse(message), timestamp: new Date().toISOString()
    };
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      const initialMessage = {
        type: 'connection_established', connectionId, userId, subscriptions: timestamp: new Date().toISOString()
      };
      ...
      const heartbeatInterval = setInterval(() => {
        try {
          ...
          clearInterval(heartbeatInterval); }, 30000);
...
export const POST: RequestHandler = async () => {
  const status = {
    sseConnections: sseManager.getConnectionCount(), redisInitialized: sseManager['isInitialized'], timestamp: new Date().toISOString()
  };
