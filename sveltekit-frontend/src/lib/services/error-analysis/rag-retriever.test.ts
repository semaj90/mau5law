...
describe('RAGRetriever - Property-Based Tests (Task 6.1)', () => {
  ...
  beforeEach(() => {
    ...
    // Mock fetch for embedding and Qdrant queries
  ...
  describe('Property 2: RAG Context Relevance - Pattern Ranking', () => {
    it('should return patterns ranked by similarity descending', async () => {
      ...
      // Mock Qdrant response with patterns
      const mockPatterns = [
        { id: 'p1', score: 0.95, payload: { filePath: 'a.ts', lineNumber: 5, code: 'code1', errorType: 'TypeError' } },
        { id: 'p2', score: 0.87, payload: { filePath: 'b.ts', lineNumber: 10, code: 'code2', errorType: 'TypeError' } },
        { id: 'p3', score: 0.72, payload: { filePath: 'c.ts', lineNumber: 15, code: 'code3', errorType: 'TypeError' } },
      ];
      ...
      vi.spyOn(global, 'fetch').mockResolvedValueOnce({
        ok: true,
        json: async () => ({ result: mockPatterns }),
      } as Response);
    ...
    it('should respect topK limit', async () => {
      ...
      // Mock Qdrant response with many patterns
      const mockPatterns = Array.from({ length: 10 }, (_, i) => ({
        id: `p${i}`,
        score: 1 - i * 0.05,
        payload: { filePath: `file${i}.ts`, lineNumber: i, code: `code${i}`, errorType: 'TypeError' },
      }));
      ...
      vi.spyOn(global, 'fetch').mockResolvedValueOnce({
        ok: true,
        json: async () => ({ result: mockPatterns.slice(0, 3) }),
      } as Response);
  ...
  describe('Property: Pattern Ranking Consistency', () => {
    it('should rank patterns by similarity descending', async () => {
      const patterns: Pattern[] = [
        { id: 'p1', filePath: 'a.ts', lineNumber: 1, code: 'code1', errorType: 'TypeError', similarity: 0.5 },
        { id: 'p2', filePath: 'b.ts', lineNumber: 2, code: 'code2', errorType: 'TypeError', similarity: 0.9 },
        { id: 'p3', filePath: 'c.ts', lineNumber: 3, code: 'code3', errorType: 'TypeError', similarity: 0.7 },
      ];
  ...
  describe('Property: Context Formatting', () => {
    it('should format patterns as markdown context', async () => {
      const patterns: Pattern[] = [
        { id: 'p1', filePath: 'a.ts', lineNumber: 1, code: 'const x = 5;', errorType: 'TypeError', similarity: 0.95 },
        { id: 'p2', filePath: 'b.ts', lineNumber: 2, code: 'const y = "hello";', errorType: 'TypeError', similarity: 0.87 },
      ];
    ...
    it('should include code snippets', async () => {
      const patterns: Pattern[] = [
        { id: 'p1', filePath: 'test.ts', lineNumber: 10, code: 'function test() { return 42; }', errorType: 'TypeError', similarity: 0.9 },
      ];
  ...
  describe('Property: Error Handling', () => {
    it('should reject null error', async () => {
      await expect(retriever.queryPatterns(null as any)).rejects.toThrow();
    });
