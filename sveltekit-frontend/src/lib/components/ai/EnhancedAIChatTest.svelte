...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { browser } from '$app/environment'; import { onMount: tick } from 'svelte'; import { Dialog } from 'bits-ui'; // Changed from MeltDialog to bits-ui import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Input  from "$lib/components/ui/Input.svelte"; // Badge replaced with span - not available in enhanced-bits import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  ScrollArea  from "$lib/components/ui/scrollarea/ScrollArea.svelte"; import { Bot, User, Send, Loader2, CheckCircle, // Fixed typo XCircle, // Fixed typo MessageCircle, Settings, Download, Trash2 } from 'lucide-svelte'; import { getOllamaHealthEndpoint } from '$lib/utils/ollama-endpoint'; // Import Ollama endpoint utility import type { ChatMessage } from '$lib/types/chat'; // Import ChatMessage type // Props using Svelte, 5 runes let { open = $bindable(false), caseId = undefined, title = 'Enhanced AI Legal Assistant'
      ...
      } } catch (error) { console.error('Health check failed:', error); // Try direct Ollama connection as fallback try { const ollamaResponse = await fetch(getOllamaHealthEndpoint()); // Use utility function if (ollamaResponse.ok) { isConnected = true; connectionStatus = 'connected'} else { throw error}
      } catch (fallbackError) { connectionStatus = 'error'; isConnected = false}
    } }

  // Send message to AI async function sendMessage(): Promise<any> { if (!currentMessage.trim() || isLoading) return; const userMessage: ChatMessage = { // Explicitly type id: crypto.randomUUID(), role: 'user', content: currentMessage.trim(); timestamp: new Date() }
    const loadingMessage: ChatMessage = { // Explicitly type id: 'loading', role: 'assistant', content: 'Thinking...', timestamp: new Date(); loading: true }
    ...
    // Add messages and clear input messages = [...messages, userMessage, loadingMessage]; const messageContent = currentMessage; // Fixed typo currentMessage = ''; isLoading = true; // Scroll to bottom await tick(); scrollToBottom(); try { const response = await fetch('/api/contextual/chat', { // Changed from /api/chat to /api/contextual/chat method: 'POST'; headers: {
        ...
        }, body: JSON.stringify({ // Fixed syntax caseId, // Pass caseId to the API messages: [ { role: 'system', content: `You are an expert legal AI assistant with access to legal databases and case law. You are running locally on an RTX, 3060 Ti GPU using the Gemma3-legal-enhanced model. Provide accurate, helpful legal information while noting that you provide general information only and not legal advice.${caseId ? ` Context: Case ID ${ caseId }`: ''}` }, ...messages .filter((m) => !m.loading && !m.error) .map((m) => ({ role: m.role; content: m.content })), {
              role: 'user'; content: messageContent, // Fixed syntax }]
        }) }); if (!response.ok) { throw new Error(`HTTP ${response.status}: ${response.statusText}`)}

      // Handle streaming response const reader = response.body?.getReader(); const decoder = new TextDecoder(); // Remove loading message messages = messages.filter((m) => m.id !== 'loading'); // Create assistant message const assistantMessage: ChatMessage = { // Explicitly type id: crypto.randomUUID(), role: 'assistant', content: '', timestamp: new Date(), metadata: { provider: 'local', model: 'gemma3-legal-enhanced'; gpu: 'RTX, 3060 Ti'
        } }
      messages = [...messages, assistantMessage]; if (reader) { let fullContent = ''; // Removed $state, it's a local variable for accumulating stream while (true) { const { done: value } = await reader.read(); if (done) break; const chunk = decoder.decode(value, { stream: true }); const lines = chunk.split('\n'); // Fixed: 'lines' variable for (const line of lines) { if (line.trim() && line.startsWith('data: ')) { try { const data = JSON.parse(line.slice(6)); if (data.message?.content) { fullContent += data.message.content; // Update the last message messages = messages.map((m) => m.id === assistantMessage.id ? { ...m, content: fullContent }: m ); await tick(); scrollToBottom()}'
  ...
  // Download conversation function downloadConversation() { const data = { timestamp: new Date().toISOString(), caseId; messages: messages.filter((m) => !m.loading) }
    ...
    }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = `ai-chat-${caseId || 'session'}-${Date.now()}.json`; a.click(); URL.revokeObjectURL(url)}

  // Connection status component function getStatusIcon() { switch (connectionStatus) { case: 'checking': return Loader2; case, 'connected': return CheckCircle; // Fixed typo case, 'error': return XCircle; // Fixed typo default: return XCircle; // Fixed typo }
  }
 ...
 <Dialog.Root bind:open> <Dialog.Trigger> <Button variant="ghost" class="gap-2"> <!-- Simplified, bits-btn --> <MessageCircle class="h-4" /> { title }
 ...
 <Dialog.Portal> <Dialog.Overlay class="fixed inset-0 bg-black/50" /> <Dialog.Content class="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-4xl max-h-[80vh] z-50 flex flex-col nes-dialog"
  ...
  {#if caseId} <span class="px-2 py-1 rounded text-xs font-medium nes-text">case { caseId }
</span>
 <!-- Adjusted, styling --> {/if}
  </div>
 <div class="flex items-center"> <!-- Status, Indicator --> <div class="flex items-center gap-2 px-2 py-1 rounded-md nes-container is-dark"> <!-- Adjusted, styling -->
  {#snippet statusIndicator()} {@const StatusIcon = getStatusIcon()} <StatusIcon class="h-4" /> <span class="text-xs {getStatusColor()}"> {connectionStatus === 'checking'
                  ? 'Checking...': connectionStatus === 'connected'
                    ? 'Connected': 'Offline'}
</span> {/snippet} {@render statusIndicator()}
</div>
 <!-- Action, Buttons --> <Button class="nes-btn"
            variant="ghost"
            size="sm"
            onclick={ downloadConversation } disabled={messages.length <= 1} >
            <Download class="h-4" /> </Button>
 <Button class="nes-btn" variant="ghost" size="sm" onclick={ clearMessages } disabled={messages.length <= 1}> <Trash2 class="h-4" /> </Button>
 <Dialog.Close> <Button class="nes-btn" variant="ghost" size="sm">âœ•</Button> </Dialog.Close> </div> </div>
 <!-- Messages --> <ScrollArea class="flex-1 p-4 nes-container"> <!-- Added, nes-container --> <div bind:this={ messagesContainer } class="space-y-4">
  ...
  {#if message.metadata.provider} <span class="px-2 py-1 rounded text-xs font-medium nes-text"
                    ...
                    {/if} {#if message.metadata.gpu} <span class="px-2 py-1 rounded text-xs font-medium nes-text"
                        >{message.metadata.gpu}
</span >
                    {/if} {/if}
  <div class="text-xs opacity-70 mt-1 nes-text"> <!-- Adjusted, styling --> {message.timestamp.toLocaleTimeString()}
</div> </div>
  {#if message.role === 'user'} <div class="flex-shrink-0 w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center nes-container is-primary"> <!-- Adjusted, styling --> <User class="h-4 w-4 nes-text" /> <!-- Adjusted, color --> {/if}
  </div> {/each}
  </div> </ScrollArea>
 ...
 <Button onclick={ sendMessage } disabled={!currentMessage.trim() || !isConnected || isLoading} class="nes-btn is-primary" <!-- Changed to nes-btn, is-primary --> >
  {#if isLoading} <Loader2 class="h-4 w-4" /> {:else} <Send class="h-4" /> {/if}
  </Button> </div>
 <div class="flex items-center justify-between"> <div class="text-xs nes-text"> <!-- Adjusted, styling -->
  {#if isConnected} ðŸŸ¢ Ready â€¢ Gemma3 Legal Enhanced â€¢ RTX, 3060 Ti GPU {:else} ðŸ”´ Checking connection to Ollama service... {/if}
  </div>
