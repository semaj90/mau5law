...
// Declare external worker manager if provided at runtime (keeps TS happy)
declare const aiWorkerManager: any | undefined;
...
class VectorIntelligenceService {
  ...
  async generateRecommendations(request: RecommendationRequest): Promise<IntelligenceRecommendation[]> {
    try {
      ...
      const personalizedRecommendations = await this.personalizeRecommendations(semanticRecommendations, request.userProfile);
  ...
  private async initializeVectorDB(): Promise<void> {
    // Minimal safe probe - replace with real connection logic
    try {
  ...
  private async rankRecommendations(recommendations: IntelligenceRecommendation[], _request: RecommendationRequest): Promise<IntelligenceRecommendation[]> {
    ...
    return recommendations
      ...
      .sort((a, b) => {
        ...
        const confidenceWeight = (b.confidence - a.confidence) * 10;
  ...
  // Ensure an in-memory collection exists for the stubbed vector DB operations.
  // Replaces stray console.log that previously caused a missing method / syntax issues.
  private async createVectorCollection(): Promise<void> {
    try {
      if (!this.vectorCache) {
        // initialize in case constructor ordering changed
  ...
  private async storeVector(
    ...
    try {
      ...
      const vec = embedding instanceof Float32Array
        ? embedding
        : new Float32Array(Array.isArray(embedding) ? embedding : []);

      // store in in-memory cache for quick local lookups
      const key = `doc_${documentId}`;
      this.vectorCache.set(key, vec);

      // best-effort push to Qdrant (non-blocking and tolerant of failures)
      try {
