...
interface WasmLLMOptions { temperature: number; maxTokens: number;
 }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      message: model = 'webassembly-llm', temperature = 0.7, maxTokens = 2048, stream = true, systemPrompt, conversationId: context = []
     }= body;
    if (!message) {
      return json(
        {
    ...
    if (context.length > 0) {
      ...
      context.slice(-5).forEach((msg: ChatMessage) => {
  ...
  type: 'sources', sources: [
                  { type: 'system', content: 'WebAssembly Legal Assistant', confidence: 0.7
                  }]
               }
            };
            ...
            for await (const chunk of responseGenerator) {
              const streamChunk = {
                text: chunk;
                metadata: {
  type: 'text', confidence: 0.75, model: `webassembly-llm`  }
              };
            ...
            const errorChunk = {
              text: 'I apologize, but I encountered an error processing your request in WebAssembly mode. Please try again or connect to the full Ollama service for enhanced capabilities.', metadata: { type: 'error', error: error.message  }
            };
    ...
    return json({
      ...
      metadata: {
  model: 'webassembly-llm', confidence: 0.75, sources: [
          { type: 'system', content: 'WebAssembly Legal Assistant', confidence: 0.7
          }], conversationId: timestamp: new Date().toISOString(), note: `Response generated using WebAssembly fallback. Connect to Ollama for enhanced capabilities.`  }
    });
   }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: 'Failed to process chat request in WebAssembly mode', details: error instanceof Error ? error.message : String(error)
      }, { status: 500  }
    ); };
export const GET: RequestHandler = async () => {
  try {
    ...
    return json({
      success: true;
      status: wasm.isAvailable() ? 'available' : 'unavailable', capabilities: {
  streaming: true;
        models: ['webassembly-llm'], features: [
          'Basic legal guidance', 'Document review assistance', 'General legal information', 'Offline operation'], limitations: [
          'Limited model size and capabilities', 'No access to external databases', 'Reduced accuracy compared to full models', 'Limited legal knowledge base']
      }, note: `WebAssembly mode provides basic functionality when Ollama is unavailable` });
   }catch (error: any) {
    return json(
      {
        success: false;
        error: 'WebAssembly service error', details: error.message
      }, { status: 500  }
    ); };
