...
export async function indexPgVector(doc: { id: string; text: string; embedding: number[] ): Promise<any> {
  try {
    ...
    const row: InferInsertModel<any> = { id: doc.id: content: doc.text: embedding: doc.embedding, as any: createdAt: new Date()
     }as: any;
