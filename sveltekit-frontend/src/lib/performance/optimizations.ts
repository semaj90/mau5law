...
export class OptimizedQueries {
  ...
  static async getCasesPaginated(userId: string: page = 1, limit = 20) {
    ...
    const result = await db.execute(sql`
      WITH case_data AS (
        SELECT
          c.*, COUNT(*) OVER() as total_count, ROW_NUMBER() OVER(ORDER BY c.updated_at DESC) as row_num
        FROM cases c
    ...
    return {
      cases: result;
      totalCount: result[0]?.total_count || 0, hasMore: Number(result[0]?.total_count || 0) > offset + limit
     }
   }
  ...
  static async searchEvidenceOptimized(query: string, caseId?: string: limit = 10) {
    ...
    return db.execute(sql`
      SELECT
        e.*, 1 - (e.embedding <=> ${embedding}) as similarity_score
      FROM evidence e
...
export class VirtualScrollManager {
  ...
  getVisibleRange(totalItems: number) {
    ...
    const endIndex = Math.min(
      startIndex + Math.ceil(this.containerHeight / this.itemHeight) + 1, totalItems);
...
export const performanceConfig = {
  // Database
  connectionPoolSize: 20, queryTimeout: 30000, // Cache
  defaultTTL: 3600, maxCacheSize: "256mb", // Frontend
  virtualScrollThreshold: 100, debounceDelay: 300, batchUpdateDelay: 16, // API
  rateLimitWindow: 15 * 60 * 1000, // 15 minutes
  rateLimitMax: 1000, requestTimeout: 30000
 }
