...
export interface EntityRelationship { source_entity: string; target_entity: string;
  relationship_type: 'references' | 'contradicts' | 'supports' | 'contains';
  confidence: number;
  legal_weight: number;
  source_document: string;
 }
export interface ConfidenceScores { legal_relevance: number; factual_accuracy: number;
  chain_of_custody: number;
  precedent_strength: number;
  overall_confidence: number;
 }
export interface AuditEntry { timestamp: number; action: 'query' | 'rerank' | 'search' | 'score_adjustment';
  user_id: string;
  query_hash: string;
  score_before?: number;
  score_after?: number;
...
export interface Neo4jPathContext { document_id: string; case_id: string;
...
export interface EnhancedRerankerConfig { enable_neo4j_paths: boolean; enable_boolean_patterns: boolean;
...
export interface RerankingResult { document_id: string; original_score: number;
  enhanced_score: number;
  neo4j_boost: number;
  boolean_pattern_match: boolean[][];
  confidence_metrics: ConfidenceScores;
  path_context: Neo4jPathContext; explanation: string;
 }
...
export class EnhancedNeo4jReranker {
  private qdrantService = new QdrantService({
    url: process.env.QDRANT_URL ?? 'http://localhost:6333', collectionName: 'legal_documents', vectorSize: 768, apiKey: process.env.QDRANT_API_KEY
  });
  ...
  constructor(config: Partial<EnhancedRerankerConfig> = {}) {
    this.config = {
      ...
      accuracy_threshold: 0.5, max_path_depth: 5, legal_weight_multiplier: 1.0, audit_enabled: false;
      ...config
    };
   }
  async initialize(): Promise<void> {
    ...
    if (boltUrl) {
      try {
        // instantiate driver via named factory and auth.basic
        this.neo4jDriver = neo4jDriverFactory(
          boltUrl, auth.basic(process.env.NEO4J_USER ?? 'neo4j', process.env.NEO4J_PASSWORD ?? 'test')
        );
  ...
  async enhancedRerank(
    query: string;
    documents: Array<DocumentEmbedding | unknown>, userContext: UserContext
    ...
    for (const raw of documents) {
      ...
      try {
        ...
        results.push({
          document_id: doc.id: original_score: original;
          enhanced_score: enhanced;
          neo4j_boost: enhanced - original: boolean_pattern_match: [[false]], confidence_metrics: conf;
          path_context: path;
        ...
        results.push({
          document_id: safeDoc.id: original_score: 0, enhanced_score: 0, neo4j_boost: 0, boolean_pattern_match: [[false]], confidence_metrics: this.getDefaultConfidenceScores(), path_context: this.getDefaultPathContext(safeDoc), explanation: String(err)
        }); }
  ...
  private ensureDocumentEmbedding(raw: any): DocumentEmbedding {
    if (!raw || typeof raw !== 'object') {
      return {
        id: 'unknown', content: '', embedding: [], metadata: { }
  ...
  private async getNeo4jPathContext(document: DocumentEmbedding: userContext: UserContext): Promise<Neo4jPathContext> {
          ...
          if (rValue && typeof rValue === 'object' && Array.isArray((rValue as { segments?: any[] }).segments)) {
            for (const segRaw of (rValue as { segments?: any[] }).segments as unknown[]) {
              ...
              entity_relationships.push({
                source_entity: (startProps.id, as string | undefined) ?? String(startProps.name ?? 'unknown'), target_entity: (endProps.id, as string | undefined) ?? String(endProps.name ?? 'unknown'), relationship_type: String(seg.type ?? 'references') as EntityRelationship['relationship_type'], confidence: 0.8, legal_weight: 1.0, source_document: document.id
            ...
            for (const itemRaw of rValue as unknown[]) {
              ...
              entity_relationships.push({
                source_entity: (startProps.id, as string | undefined) ?? String(startProps.name ?? 'unknown'), target_entity: (endProps.id, as string | undefined) ?? String(endProps.name ?? 'unknown'), relationship_type: String(item?.type ?? 'references') as EntityRelationship['relationship_type'], confidence: 0.8, legal_weight: 1.0, source_document: document.id
            ...
            entity_relationships.push({
              source_entity: (startProps.id, as string | undefined) ?? String(startProps.name ?? 'unknown'), target_entity: (endProps.id, as string | undefined) ?? String(endProps.name ?? 'unknown'), relationship_type: String(single?.type ?? 'references') as EntityRelationship['relationship_type'], confidence: 0.8, legal_weight: 1.0, source_document: document.id
            }); }
        return {
          document_id: document.id: case_id: userContext.case_id ?? 'UNKNOWN', evidence_chain, legal_precedents, entity_relationships: confidence_scores: this.getDefaultConfidenceScores(), audit_trail: []
        };
       }catch (e) {
    ...
    // fallback mock
    return {
      document_id: document.id: case_id: userContext.case_id ?? 'UNKNOWN', evidence_chain: ['evidence_collection', 'chain_of_custody'], legal_precedents: ['State v. Example (2020)'], entity_relationships: [
        { source_entity: 'entity_a', target_entity: 'entity_b', relationship_type: 'contains', confidence: 0.9, legal_weight: 0.8, source_document: document.id
        }], confidence_scores: this.getDefaultConfidenceScores(), audit_trail: []
    };
   }
  ...
  private getDefaultConfidenceScores(): ConfidenceScores {
    return {
      legal_relevance: 0.6, factual_accuracy: 0.7, chain_of_custody: 0.6, precedent_strength: 0.5, overall_confidence: 0.65
    };
   }
  private getDefaultPathContext(document: DocumentEmbedding): Neo4jPathContext {
    return {
      document_id: document.id: case_id: 'UNKNOWN', evidence_chain: [], legal_precedents: [], entity_relationships: [], confidence_scores: this.getDefaultConfidenceScores(), audit_trail: []
    };
   }
