...
import type { Document } from '$lib/types'; import  Button, Card, CardContent, CardHeader, CardTitle  from "$lib/components/ui/enhanced-bits.svelte"; import { createEventDispatcher: onMount } from 'svelte'; interface SearchResult { id: string, title: string, content: string, score: number, metadata: { document_type: string, jurisdiction: string; date: string, legal_domain: string}; performance: { gpu_accelerated: boolean, search_time_ms: number, gpu_utilization: number}}
  ...
  let performanceHistory: Perf[] = []; // Load CUDA capabilities on mount onMount(async () => { try { const response = await fetch('/api/ai/cuda-capabilities'); if (response.ok) { const data = await response.json(); cudaCapabilities = { gpu_model: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.gpu_model || 'RTX, 3060 Ti', vram_gb: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.vram_gb || 8, cuda_cores: data.cuda_indexing_capabilities?.rtx_3060_ti_specs?.cuda_cores || 4864, simd_enabled: data.simd_capabilities?.avx2_enabled || false, instruction_set: data.simd_capabilities?.instruction_set || 'AVX2'
   ...
   // 4) Finalize timings / history / results const totalSearchTime = Date.now() - startTime; searchTime = totalSearchTime; performanceHistory.push({ timestamp: Date.now(), search_time_ms: totalSearchTime, gpu_utilization: gpuMetrics.utilization, query_length: query.length, results_count: searchResults.length }); if (performanceHistory.length > 10) performanceHistory = performanceHistory.slice(-10); results = searchResults; // callbacks / events if (onSearchComplete) onSearchComplete(searchResults); onsearchcomplete?.({ query, results: searchResults, searchTime: totalSearchTime, gpuAccelerated: enableGpuAcceleration })} catch (error) { console.error('Search failed:', error); errorMessage = error instanceof Error ? error.message: 'Search failed'; results = []} finally { isSearching = false}
  }
