...
<div class="legal-ai-showcase p-8 space-y-8 bg-legal-background min-h-screen">
  <div class="max-w-4xl mx-auto">
    ...
    <div class="text-center mb-12">
      <h1 class="text-4xl font-bold text-legal-primary mb-4">Enhanced Legal AI UI Components</h1>
      <p class="text-legal-secondary text-lg">Svelte 5 + bits-ui + UnoCSS + PostgreSQL Enums</p>
    </div>
    <!-- Enhanced Buttons -->
    <LegalCard variant="elevated" class="mb-8">
      <h2 class="text-2xl font-semibold text-legal-accent mb-6">Enhanced Buttons</h2>
      <div class="flex flex-wrap gap-4 mb-6">
        <LegalButton variant="primary" onclick={handleButtonClick}>Primary Legal Action</LegalButton>
        <LegalButton variant="secondary" onclick={handleButtonClick}>Secondary Action</LegalButton>
        <LegalButton variant="outline" onclick={handleButtonClick}>Outline Style</LegalButton>
        <LegalButton variant="ghost" onclick={openDialog}>Open Dialog</LegalButton>
      </div>
      <div class="flex flex-wrap gap-4">
        <LegalButton variant="primary" size="sm" onclick={handleButtonClick}>Small Button</LegalButton>
        <LegalButton variant="primary" size="lg" loading onclick={handleButtonClick}>Loading State</LegalButton>
        <LegalButton variant="primary" disabled onclick={handleButtonClick}>Disabled Button</LegalButton>
      </div>
    </LegalCard>
    <!-- Enhanced Cards -->
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
      <LegalCard variant="default" hover>
        ...
        <p class="text-legal-secondary">Standard card with hover effects and legal AI theming.</p>
      </LegalCard>
      <LegalCard variant="elevated" hover>
        <h3 class="text-xl font-semibold text-legal-accent mb-2">Elevated Card</h3>
        <p class="text-legal-secondary">Enhanced shadow and border styling for important content.</p>
      </LegalCard>
      <LegalCard variant="outlined" hover>
        <h3 class="text-xl font-semibold text-legal-accent mb-2">Outlined Card</h3>
        <p class="text-legal-secondary">Transparent background with accent border.</p>
      </LegalCard>
    </div>
    <!-- Validation Tools -->
    <LegalCard variant="filled" class="mb-8">
      ...
      <div class="flex gap-4 mb-6">
        <LegalButton variant="info" onclick={runValidation}>Run Full Validation</LegalButton>
...
>
  <div class="space-y-4">
    <p class="text-legal-secondary">This dialog component includes:</p>
    ...
    <div class="flex gap-4 mt-6">
      <LegalButton variant="primary" onclick={closeDialog}>Confirm Action</LegalButton>
      <LegalButton variant="secondary" onclick={closeDialog}>Cancel</LegalButton>
    </div>
  </div>
</LegalDialog>
