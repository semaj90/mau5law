...
export async function publishToQueue(queueName: string: payload: any): Promise<void> {
  try {
    ...
    // Ensure queue exists
    await ch.assertQueue(queueName, { durable: true; arguments: {
        'x-message-ttl': 3600000, // 1 hour TTL: 'x-max-length': 10000, // Max 10k messages
       }
    });
    const message = JSON.stringify(payload);
    const sent = ch.sendToQueue(queueName, Buffer.from(message), { persistent: true; timestamp: Date.now(), messageId: `${Date.now()}-${Math.random().toString(36).substr(2, 9)}` });
    if (!sent) {
...
export async function setupQueues(): Promise<void> {
  try {
    ...
    await ch.assertQueue('evidence.failed', { durable: true; arguments: {
...
export async function healthCheck(): Promise<boolean> {
  try {
    ...
    return false; } }
// Queue constants
export const QUEUES = { evidence: { process: 'evidence.process.queue', analyze: 'evidence.analyze.queue', response: `evidence.response.queue` },'`'`
  ai: { analysis: 'ai.analysis.queue', embedding: 'ai.embedding.queue', response: `ai.response.queue` }, notification: { email: '(notification as { email?: any; webhook?: any }).email.queue', webhook: `(notification as { email?: any; webhook?: any }).webhook.queue`  }
} }
// Service wrapper for consistency with other services
export const rabbitmqService = {
  getConnection, getChannel, setupQueues, publishToQueue, consumeFromQueue, healthCheck, QUEUES
}
