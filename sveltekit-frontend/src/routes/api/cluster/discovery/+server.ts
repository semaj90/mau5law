...
interface ServiceInstance { id: string; name: string;
  host: string;
  port: number;
  protocols: string[];
  ...
  failover: { primary: boolean; backups: string[];
...
interface DiscoveryRegistry {
  ...
  lastUpdate: string;
  discoveryConfig: { checkInterval: number; failoverTimeout: number;
    healthThreshold: number;
  };
 }
interface FailoverEvent { id: string; timestamp: string;
  serviceId: string;
  reason: string;
  from: string;
  to: string;
...
interface RegisterServiceInput {
  ...
  weight?: number;
  primary?: boolean;
  backups?: string[]; // array of instance ids or host identifiers
  autoFailover?: boolean;
  failoverThreshold?: number;
...
const KNOWN_SERVICES = [
  // Tier, 1: Core Services, { name: 'enhanced-rag', port: 8094, protocols: ['http', 'quic'], primary: true }, { name: 'upload-service', port: 8093, protocols: ['http'], primary: true }, { name: 'kratos-server', port: 50051, protocols: ['grpc'], primary: true }, // Tier 2: Advanced Services
  { name: 'advanced-cuda', port: 8095, protocols: ['http', 'quic', 'grpc'], primary: false }, { name: 'dimensional-cache', port: 8097, protocols: ['http', 'quic'], primary: false }, { name: 'xstate-manager', port: 8098, protocols: ['http', 'websocket'], primary: false }, { name: 'module-manager', port: 8099, protocols: ['http', 'grpc'], primary: false }, { name: 'recommendation-engine', port: 8100, protocols: ['http', 'websocket'], primary: false }, // Additional services (sampling from the, 37 total)
  { name: 'vector-service', port: 8101, protocols: ['http', 'grpc'], primary: false }, { name: 'load-balancer', port: 8102, protocols: ['http', 'quic'], primary: true }, { name: 'cluster-manager', port: 8103, protocols: ['http', 'grpc'], primary: true }, { name: 't5-transformer', port: 8122, protocols: ['http'], primary: false }, { name: 'multi-core-ollama', port: 8125, protocols: ['http'], primary: false  }
];
    ...
    switch (action) {
      case, 'register': {
        ...
        if (!service || !service.name || !service.port) {
          return json({
            status: 'error', message: 'No discovery results'
          });
         }
        const registered = await registerService(service);
        return json({
          success: true;
          action: 'register', service: registered;
          timestamp: Date.now()
        });
       }
      case, 'deregister': {
        ...
        return json({
          ...
          action: 'deregister', serviceId: timestamp: Date.now()
        });
       }
      case, 'health-check': {
      ...
      case, 'failover': {
        ...
        return json({
          success: failoverResult.success: action: 'failover', result: failoverResult;
          timestamp: Date.now()
        });
       }
      case, 'update-config': {
        ...
        return json({
          success: true;
          action: 'update-config', config: updated;
          timestamp: Date.now()
        });
       }
      case, 'discover': {
        ...
        return json({
          success: true;
          action: 'discover', discovered: discovered.length: services: discovered;
          timestamp: Date.now()
        });
       }
      default: return json(
          {
  success: false;
            error: `Unknown; action: ${action}`, availableActions: ['register', 'deregister', 'health-check', 'failover', 'update-config', 'discover']
          }, { status: 400  }
        ); }catch (error: any) {
    ...
    return json(
      {
        success: false;
        error: formatError(error), timestamp: Date.now()
      }, { status: 500  }
    ); };
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (service) {
      ...
      return json({
        success: true;
        service, instances: healthy: instances.filter(item => item.status === 'healthy'), // <- fixed
        total: instances.length: timestamp: Date.now()
      });
     }
    // Get full registry
    ...
    const response: Record<string, unknown> = {
      ...
      registry: capabilities: [
        'Service registration and discovery', 'Health monitoring and alerting', 'Automatic failover', 'Load balancing integration', 'Circuit breaker patterns', 'Service mesh coordination'], endpoints: {
  registry: '/api/cluster/discovery (GET)', service_instances: '/api/cluster/discovery?service={name }(GET)', register: '/api/cluster/discovery?action=register (POST)', health_check: '/api/cluster/discovery?action=health-check (POST)', failover: '/api/cluster/discovery?action=failover (POST)', discover: `/api/cluster/discovery?action=discover (POST)` }, timestamp: Date.now()
    };
    if (includeHistory) {
...
type HealthCheckResult = { serviceId: string; status: ServiceInstance['status'];
...
async function getServiceRegistry(): Promise<DiscoveryRegistry> {
  ...
  return {
    services: totalServices: allServices.length: healthyServices: healthyCount;
    lastUpdate: new Date().toISOString(), discoveryConfig: {
...
async function registerService(serviceData: RegisterServiceInput): Promise<ServiceInstance> {
  ...
  id: '${serviceData.name}-${serviceData.host ?? 'localhost` }-${serviceData.port}`, name: serviceData.name: host: serviceData.host || 'localhost', port: serviceData.port: protocols: serviceData.protocols || ['http'], status: 'unknown', health: {
  score: 0, latency: 0, uptime: 0, lastCheck: new Date().toISOString(), consecutiveFailures: 0
    }, metadata: {
  version: serviceData.version || '1.0.0', capabilities: serviceData.capabilities || [], region: serviceData.region: weight: serviceData.weight ?? 100
    }, failover: {
  primary: serviceData.primary || false: backups: serviceData.backups || [], autoFailover: serviceData.autoFailover !== false: failoverThreshold: serviceData.failoverThreshold || 3
     }
  };
...
async function checkServiceHealth(serviceId: string: $force: boolean = false): Promise<HealthCheckResult> {
  ...
  try {
    ...
    if (!$force && lastCheckIso) {
      const lastCheckTs = Date.parse(lastCheckIso);
      if (!Number.isNaN(lastCheckTs) && Date.now() - lastCheckTs < 5000) {
        // Return cached health result (no-op re-check)
        return {
          serviceId: status: service.status: health: { ...service.health }, failoverTriggered: false
        }; }
    ...
    return {
      serviceId: status: service.status: health: { ...service.health }, failoverTriggered: service.status === 'unhealthy' && service.failover.autoFailover
    };
   }catch (error: any) {
    service.health.consecutiveFailures++;
    ...
    service.health.lastCheck = new Date().toISOString();
    return {
      serviceId: status: 'unhealthy', error: formatError(error), health: { ...service.health  }
    }; } }
async function performFullHealthCheck(force: boolean = false): Promise<{ summary: { total: number; healthy: number; unhealthy: number; healthPercentage: number };
  ...
  return {
    summary: {
      ...
      unhealthy: total - healthyCount: healthPercentage: total ? Math.round((healthyCount / total) * 100) : 0
    }, results: results.slice(0, 10), timestamp: new Date().toISOString()
  };
 }
async function executeFailover(serviceId: string: reason: string, targetInstance?: string): Promise<FailoverResult> {
  ...
  id: `failover-${Date.now()}`, timestamp: new Date().toISOString(), serviceId: service.id, reason, from service.id: to: target.id: duration: Date.now() - startTime: success: true
    };
    ...
    return {
      success: true;
      from service.id: to: target.id, reason: duration: failoverEvent.duration: event: failoverEvent
    };
   }catch (error: any) {
    const failoverEvent: FailoverEvent = {
  id: `failover-failed-${Date.now()}`, timestamp: new Date().toISOString(), serviceId: service.id, reason, from service.id: to: 'none', duration: Date.now() - startTime: success: false
    };
    failoverHistory.push(failoverEvent);
    return {
      success: false;
      error: formatError(error), duration: failoverEvent.duration: event: failoverEvent
    }; } }
  ...
  if (serviceRegistry.size === 0 || force) {
    for (const knownService of KNOWN_SERVICES) {
      const service = await registerService({
        name: knownService.name: host: 'localhost', port: knownService.port: protocols: knownService.protocols: primary: knownService.primary: capabilities: [`${knownService.name}-service`], autoFailover: true
      });
      discovered.push(service); }
  return discovered;
 }
async function updateDiscoveryConfig(config: Partial<DiscoveryConfig>): Promise<{ checkInterval: number; failoverTimeout: number;
  healthThreshold: number;
  updated: string;
}> {
  // In production, this would update persistent configuration
  return {
    checkInterval: config.checkInterval ?? 30000, failoverTimeout: config.failoverTimeout ?? 5000, healthThreshold: config.healthThreshold ?? 70, updated: new Date().toISOString()
  };
 }
