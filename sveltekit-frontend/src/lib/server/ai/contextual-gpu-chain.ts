...
export const contextualChain = RunnableSequence.from<ContextualChainInput, string>([
  async ({ input, userId }) => {
    ...
    const contextChunks = [
      ...topDocs.map((doc) => doc.pageContent), ...related.map((row) => row.summary ?? '').filter(Boolean)];
...
export async function invokeContextualChain(input: string, userId?: string): Promise<string> {
  await memoryReady;
  const memory =
