...
<script lang="ts">
  ...
  function loadMore() {
    ...
    // If consumer provided an onloadMore hook and we've reached the end of currently available items,
    // allow them to fetch/append more data.
    if (onloadMore && endIndex >= currentItems.length) {
      onloadMore();
    }
  }
  ...
  // Clean, reliable icon resolver (fixes misspelled component names)
  function getItemIcon(item?: any) {
    if (itemType === "notes") return FileEdit;
    if (itemType === "canvas") return Palette;
    const fileType = item ? (item.fileType || item.type || "") : "";
...
>
  ...
  {:else}
    <div class="items-list">
      {#each displayedItems as item, index ((item as any).id || index)}
        <div
          class="list-item"
            ...
            <div class="item-header">
              <h4 class="item-title">
                {#if itemType === 'evidence'}
                  {(item as any).fileName || (item as any).title || 'Untitled Evidence'}
                {:else if itemType === 'notes'}
                  {(item as any).title || 'Untitled Note'}
                {:else}
                  {(item as any).name || `Canvas ${formatDate((item as any).lastModified)}`}
                {/if}
              </h4>
              <span class="item-date">
                {formatDate((item as any).createdAt || (item as any).lastModified || (item as any).updatedAt)}
              </span>
            </div>
            <p class="item-description">
              {#if itemType === 'evidence'}
                {truncateText((item as any).description)}
              {:else if itemType === 'notes'}
                {truncateText((item as any).content)}
              {:else}
                Canvas state with {(item as any).objectCount || 0} objects
              {/if}
            </p>
            {#if (item as any).tags && (item as any).tags.length > 0}
              <div class="item-tags">
                ...
                {#if (item as any).tags.length > 3}
                  <span class="tag-more">+{(item as any).tags.length - 3}</span>
                {/if}
              </div>
            {/if}
          </div>
  ...
  {#if !hasMore && displayedItems.length > 0}
    <div class="end-indicator" in:fade={{ duration: 200 }}>
...
<style>
  /* Use bits-ui / uno.css friendly variables with sensible fallbacks */
  .infinite-scroll-container {
    flex: 1;
    overflow-y: auto;
    padding: 0,
    background: var(--bits-background, #ffffff);
  }
  .empty-state {
    display: flex;
  ...
  .list-item:hover {
    background: var(--bits-secondary-background, #f8fafc);
    border-color: var(--bits-border, #e5e7eb);
    transform: translateY(-1px);
    box-shadow: 0 2px 8px rgba(0, 0, 0, 0.06);
  }
  .list-item:focus {
    outline: 2px solid var(--bits-primary, #2563eb);
    outline-offset: 2px;
  }
  .list-item:active {
  ...
  .item-icon {
    ...
    height: 40px;
    border-radius: 8px;
    background: var(--bits-primary-background, #e6f0ff);
  ...
  .tag-more {
    font-size: 0.7rem;
    padding: 0.15rem 0.5rem;
    background: var(--bits-muted-background, #f3f4f6);
    color: var(--bits-muted, #6b7280);
    border-radius: 12px;
  ...
  .spinner {
    width: 20px;
    height: 20px;
    border: 2px solid var(--bits-border, #e5e7eb);
    border-top: 2px solid var(--bits-primary, #2563eb);
    border-radius: 50%;
  ...
  .infinite-scroll-container::-webkit-scrollbar-thumb {
    background: var(--bits-border, #e5e7eb);
    border-radius: 3px;
  }
