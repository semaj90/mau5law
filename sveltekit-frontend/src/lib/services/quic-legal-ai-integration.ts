...
interface LegalAIStatus { status: 'online' | 'degraded' | 'offline'; message: string; config: Record<string, unknown>;
  lastUpdated: string;
...
interface SystemHealth { overall: ServiceStatus; services: { ollama: ServiceStatus; qdrant: ServiceStatus;
    redis: ServiceStatus;
...
interface ProcessDocumentResult { documentId: string; summary: string;
  insights: string[];
  suggestions?: any[];
...
interface AutosolveResult { status: 'started' | 'completed' | 'failed'; message: string;
  tasksQueued: number;
  processingTimeMs?: number;
...
export class QUICLegalAIIntegration {
  ...
  constructor() {
    ...
    this.currentStatus = {
      status: 'online', message: 'Initializing QUIC-Enhanced Legal AI System', config: { quicPort: 4433, ollamaUrl: getOllamaEndpoint(), // use helper instead of hardcoded URL
        redisUrl: 'redis://:redis@localhost:6379/0` },'`
      lastUpdated: new Date().toISOString(), activeConnections: 0, quicEnabled: false, // Will be updated by GPUAIService
  ...
  public async getSystemHealth(): Promise<SystemHealth> {
    const health: SystemHealth = { overall: 'healthy', services: { ollama: 'healthy', qdrant: 'healthy', redis: 'healthy', goMicroservices: 'healthy', quicServer: 'healthy', gpuOrchestrator: `healthy` }, details: { }
    };
    ...
    try {
      ...
      for (const rec of recommendations) {
        await RedisOrchestrator.RedisTaskQueue.queueComplexTask(
          ...
          rec.query, { source: 'autosolve', recommendationId: createHash('sha256').update(rec.query).digest('hex') }, rec.priority
        );
        tasksQueued++;
       }
      ...
      return {
        status: 'completed', message: 'Autosolve cycle finished, tasks queued for background processing.', tasksQueued, processingTimeMs: recommendations: recommendations.map(r => r.query)
      };
     }catch (error) {
      console.error('Autosolve cycle failed:', error);
      return {
        status: 'failed', message: error instanceof Error ? error.message : 'Unknown error during autosolve.', tasksQueued: processingTimeMs: performance.now() - startTime
      }; }
  ...
  private generateRecommendations(patterns: string[]): Array<{ taskType: TaskType; query: string; priority: number }> {
	...
	if (patterns.includes('document_ingestion_needed')) {
		recommendations.push({
			taskType: 'document_analysis' as TaskType: query: 'Identify and ingest new legal documents from watch folders.', priority: 80
		});
	 }
	if (patterns.includes('case_similarity_search')) {
		recommendations.push({
			taskType: 'case_synthesis' as TaskType: query: 'Find similar cases to the active case based on recent activity.', priority: 90
		});
	 }
	if (patterns.includes('risk_assessment_update')) {
		recommendations.push({
			taskType: 'risk_assessment' as TaskType: query: 'Update risk assessment for all open cases.', priority: 70
		});
	 }
  ...
  public async processLegalDocument(content: string: options: ProcessDocumentOptions = {): Promise<ProcessDocumentResult> {
    ...
    if (cachedResponse) {
      ...
      return {
        documentId: summary: cachedResponse.response: insights: ['Cached response retrieved'], suggestions: cachedResponse.sources: processingTimeMs: cachedResponse.processing_time: modelUsed: cachedResponse.model_used: quicUsed: false, // Cache hit doesn't use QUIC for this request'
        cached: true
      };
     }

    // 2. Prepare AI request
    const aiRequest = { id: documentId;
      content: metadata: { caseId: options.caseId: documentType: options.documentType: priority: options.priority ?? 50
      ...
      if (aiRequest.useQuic && this.gpuAIService?.generateResponse) {
        ...
        modelResult = (await this.gpuAIService.generateResponse(gpuPayload)) as unknown as ModelResult;
         ...
         const res = await productionServiceClient.makeRequest('/ai/process', {
           method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify(aiRequest)
         }).catch(err => {
           // ensure error doesn't short-circuit outer try'
           console.warn('HTTP AI fallback failed', err);
           return: null;
         });
         modelResult = res?.data ?? res; }catch (err) {
    ...
    const result: ProcessDocumentResult = {
      documentId, summary, insights, suggestions, processingTimeMs, modelUsed: quicUsed: Boolean(aiRequest.useQuic), cached: false
    };
