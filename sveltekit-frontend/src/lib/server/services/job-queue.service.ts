...
export class JobQueueService {
	...
	async enqueueJob(payload: JobPayload): Promise<string> {
		try {
			...
			const uuid = `job-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
	...
	async consumeJobs(
		...
		try {
			...
			await this.channel!.consume(queueName, async (msg) => {
				...
				try {
					const payload = JSON.parse(msg.content.toString()) as JobPayload & { jobId: string };

					console.log(`📥 Processing job ${payload.jobId}`);

					// Update status to processing
					await this.updateJobStatus(payload.jobId, 'processing', 10);

					// Call handler
					// Create a clean payload object without the extra jobId if handler expects exact JobPayload
					const handlerPayload: JobPayload = {
						caseId: payload.caseId,
						type: payload.type,
						data: payload.data,
						userId: payload.userId,
