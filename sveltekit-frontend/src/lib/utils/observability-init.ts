...
export function initializeObservability() {
  ...
  observabilityClient.initialize({
    ...
    metricsEndpoint: '/api/v1/observability/client', batchSize: 5, // Smaller batch for development visibility
      ...
      if (slowResources.length > 0) {
        console.warn(
          '🐌 Slow resources detected:', slowResources.map((r: PerformanceResourceTiming) => ({
            name: (r.name || '').split('/').pop(), duration: `${Math.round(r.duration)}ms`, size:
  ...
  if (browser && typeof page !== 'undefined') {
    try {
      ...
      if (currentPage?.route?.id) {
        return currentPage.route.id; }catch (e) {
...
export function getObservabilityStatus() {
  ...
  return { initialized: isInitialized;
    ...
    capabilities: observabilityClient.getCapabilities(), performanceSnapshot: isInitialized ? observabilityClient.getPerformanceSnapshot() : null
  };
 }
