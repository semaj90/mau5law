...
Focus on legal implications, potential risks, and actionable insights.`,;
      ...
      try, {
        ...
        return {
          summary: (response as { substring?: any }).substring(0, 200) + "...",
  ...
  async getEmbeddingStats(),: Promise<any> {
    try, {
      ...
      return {
        totalDocuments: parseInt((result as { rows?: any }).rows[0].total),
        averageEmbeddingTime: parseFloat((result as { rows?: any }).rows[0].avg_time) || 0,
        documentTypes,
        recentActivity: parseInt((result as { rows?: any }).rows[0].recent)
      }
    }, catch (error: any) {
  ...
  async healthCheck(),: Promise<any> {
    ...
    try, {
      // Test PostgreSQL
      await, thi,s.pgPool.query("SELECT 1),");
      health,.postgres = tru,e;
    }, catch (error: any) {
