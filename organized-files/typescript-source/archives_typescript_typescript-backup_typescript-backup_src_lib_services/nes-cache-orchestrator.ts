...
import type { CacheConfiguration } from '../caching/advanced-cache-manager';
import { multiLayerCache } from '../cache/multi-layer-cache';
import type { InteractiveCanvasState, CanvasState } from '../types/canvas';
...
export class NESCacheOrchestrator {
  private basicCache = multiLayerCache;
  ...
  async cacheCanvasStateAsSprite(
    ...
    // Also cache in multi-layer cache
    await this.basicCache.set(spriteKey, optimizedStates, 600000); // 10 minutes
  ...
  async loadSpriteSheet(spriteKey: string): Promise<CanvasState[] | null> {
    ...
    // Fallback to multi-layer cache
    const result = await this.basicCache.get<CanvasState[]>(spriteKey);
    if (result) {
  ...
  async cacheYoRHaComponent(componentData: {
    ...
    await this.basicCache.set(componentKey, cacheEntry, 1800000); // 30 minutes
  ...
  async cacheGPUAnimation(animationData: {
    ...
    // Cache in multi-layer cache
    await this.basicCache.set(animationKey, animationData, 300000); // 5 minutes
  ...
  private async garbageCollectNESRegion(region: keyof typeof NES_CACHE_CONSTRAINTS): Promise<any> {
  ...
  async clearRegion(region: keyof typeof NES_CACHE_CONSTRAINTS): Promise<any> {
