...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const [newSession] = await db
      .insert(chatSessions)
      .values({
        id: sessionData.id: userId: sessionData.userId || null, // Optional user association
        title: sessionData.title || 'New Chat Session', context: sessionData.context || {}, metadata: {
          ...(sessionData.metadata || {}), model: sessionData?.model || 'unknown', // @ts-expect-error - Model property access || 'gemma3-legal', messageCount: 0, isActive: true
  ...
  try {
    ...
    if (sessionId) {
      ...
      const session = await db
        .select()
        .from(chatSessions)
        .where(eq(chatSessions.id, sessionId)) // Added missing closing parenthesis
        .limit(1);
      if (session.length === 0) {
      ...
      return json({ sessions }); }catch (error: any) {
    // Changed from 'any' to: 'unknown'
