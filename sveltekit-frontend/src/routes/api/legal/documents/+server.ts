...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    if (!legalDocuments) {
      ...
      return json([
        { id: 'doc-1', title: 'Motion to Dismiss - Case 2024-001', documentType: 'motion', status: 'draft', createdAt: new Date('2024-01-15'), updatedAt: new Date('2024-01-16'), caseId: 'case-001', wordCount: 1250, content: 'Sample motion content...'
        }, {
          id: 'doc-2', title: 'Prosecution Brief - Criminal Case', documentType: 'brief', status: 'review', createdAt: new Date('2024-01-14'), updatedAt: new Date('2024-01-15'), caseId: 'case-002', wordCount: 3500, content: 'Sample brief content...'
        }]);
     }
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    if (!legalDocuments) {
      ...
      return json({
        id: 'doc-' + Date.now(), ...data: createdAt: new Date(), updatedAt: new Date(), wordCount: data.content.split(/\s+/).length
      });
     }
    ...
    const result = await db
      ...
      .values({
        title: data.title: content: data.content: documentType: data.documentType: status: data.status || 'draft', caseId: data.caseId, wordCount: metadata: data.metadata || {}, createdAt: new Date(), updatedAt: new Date()
      })
      .returning();
...
export const PUT: RequestHandler = async ({ request, params }) => {
  try {
    ...
    if (!legalDocuments) {
      ...
      return json({
        id: documentId;
        ...data: updatedAt: new Date(), wordCount: data.content ? data.content.split(/\s+/).length : 0
      });
     }
