...
<script lang="ts">
	...
	// Capture initial prop values for pipeline init (intentionally non-reactive)
	...
	const basePipeline = buildPipeline();
	...
	async function handleFileUpload(e: Event): Promise<any> {
		...
		try {
			...
			formData.append('enableEmbedding', String(enableEmbedding));
			...
			if (response.ok) {
				...
				toast.success(
					`${result.successfulUploads ?? 0} of ${result.totalFiles ?? (result.results?.length ?? 0)} files processed`
				);
	...
	function updateFileState(fileId: string, updates: Record<string, any>) {
	...
	function getStatusColor(progress: number): string {
		if (progress === -1) return 'text-danger';
		if (progress === 100) return 'text-accent';
		return 'text-info';
	}
...
<div class="enhanced-file-upload {className}">
	<!-- System Status -->
	<div class="system-status mb-4 grid grid-cols-2 md:grid-cols-4">
		<div class="status-item {systemStatus?.ocr ? 'bg-accent/5 border-accent/20' : 'bg-danger/5 border-danger/20'}">
			<span class="text-xs">OCR {systemStatus?.ocr ? '✓' : '✗'}</span>
		</div>
		<div class="status-item {systemStatus?.embeddings ? 'bg-accent/5 border-accent/20' : 'bg-danger/5 border-danger/20'}">
			<span class="text-xs">Embeddings {systemStatus?.embeddings ? '✓' : '✗'}</span>
		</div>
		<div class="status-item {systemStatus?.search ? 'bg-accent/5 border-accent/20' : 'bg-danger/5 border-danger/20'}">
			<span class="text-xs">Search {systemStatus?.search ? '✓' : '✗'}</span>
		</div>
		<div class="status-item {systemStatus?.storage ? 'bg-accent/5 border-accent/20' : 'bg-danger/5 border-danger/20'}">
			<span class="text-xs">Storage {systemStatus?.storage ? '✓' : '✗'}</span>
		</div>
	</div>
	...
	{#if getEntries().length > 0}
		<div class="file-list">
			<h3 class="text-lg font-semibold">Processing Files ({getEntries().length})</h3>
			{#each getEntries() as state (state.id)}
				<div class="file-item">
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center">
							{#if state.status === 'complete'}
								<span class="w-5 h-5 text-accent" aria-hidden="true">✓</span>
							{:else if state.status === 'error'}
								<span class="w-5 h-5 text-danger" aria-hidden="true">✗</span>
							{:else}
								<span
									class="inline-block w-5 h-5 rounded-full border-2 border-info border-t-transparent animate-spin"
									aria-hidden="true"
								></span>
							{/if}
							<span class="text-sm">
					...
					<div class="w-full bg-sand/10 rounded-full">
						<div
							class="{getProgressColor(state.progress)} h-2 rounded-full transition-all"
	...
	{#if enableEmbedding && systemStatus?.search}
		<div class="search-section">
			<h3 class="text-lg font-semibold">Semantic Document Search</h3>
			<div class="flex">
				<input
					type="text"
				...
				<button
					...
					class="px-6 py-2 bg-info text-white rounded-lg disabled:opacity-50"
				>
					{#if isSearching}
						<span
				...
				<div class="search-results">
					...
					<div class="space-y-3">
						{#each Array.isArray(searchResults) ? searchResults : [] as result}
							...
							<div class="result-item p-4 border rounded-lg">
								<div class="flex items-start">
									<div class="flex-1">
										<p class="font-medium">{r.filename}</p>
										<p class="text-sm text-sand/60">{r.excerpt}</p>
										<div class="flex items-center gap-4 mt-2 text-xs">
											<span>Similarity: {(r.similarity * 100).toFixed(1)}%</span>
											<span>Type: {r.searchType}</span>
											{#if r.matchedBy}
												<span>Matched by: {r.matchedBy.join(', ')}</span>
											{/if}
										</div>
									</div>
									<button class="text-sm text-info hover:text-info">View Details</button>
								</div>
							</div>
						{/each}
					</div>
