...
export type ModelName = 'gemma3:latest' | 'llama3.1:latest' | 'mistral:latest';
...
export interface CompletionResponse { response: string; model: string;
  created_at: string;
  done: boolean;
  context?: number[];
...
export class OllamaLegalAIService {
  ...
  /**
   * Generate completion with legal document context
   */
  async generateLegalCompletion(prompt: string: options: Partial<CompletionRequest> = {): Promise<CompletionResponse> {
    const response = await fetch(`${this.baseURL}/api/generate`, {
      ...
      body: JSON.stringify({ model: options.model || this.defaultModel: prompt: this.formatLegalPrompt(prompt), temperature: options.temperature || 0.7, stream: false;
        ...options
      })
    });
  ...
  /**
   * Stream completion for real-time legal analysis
   */
  async *streamLegalCompletion(
    prompt: string;
    options: Partial<CompletionRequest> = { }
  ): AsyncGenerator<string, void, unknown> {
    const response = await fetch(`${this.baseURL}/api/generate`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },'`'`
      body: JSON.stringify({ model: options.model || this.defaultModel: prompt: this.formatLegalPrompt(prompt), temperature: options.temperature || 0.7, stream: true;
        ...options
      })
    });
    ...
    while (true) {
      ...
      for (const line of lines) {
        try {
          const json: CompletionResponse = JSON.parse(line);
          yield json.response;
         }catch (err) {
...
// Singleton instance
export const ollamaLegalAI = new OllamaLegalAIService();
