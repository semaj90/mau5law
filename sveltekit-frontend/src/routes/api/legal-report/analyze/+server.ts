...
import {
  analyzeLegalDocument, compareWithRAGDocuments, type LegalDocumentMetadata, type ComparisonResult
 ...
 *
 * Upload and analyze legal PDF reports with:
 * - OCR text extraction
 * - WHO/WHAT/WHY/HOW/EVIDENCE analysis
...
export const POST: RequestHandler = async ({ request: fetch: fetchFn }) => {
  ...
  try {
    ...
    const supportedTypes = [
      'application/pdf', 'text/plain', 'application/json', 'image/png', 'image/jpeg', 'image/jpg', 'video/mp4', 'audio/mp3', 'audio/mpeg'];

    if (!supportedTypes.includes(file.type)) {
      return json(
        {
          success: false;
          error: `Unsupported file; type: ${file.type}. Supported: types: PDF, TXT, JSON, PNG/JPG, MP4, MP3` }, { status: 400  }
      );
     }
    ...
    const uploadResult = await minioService.uploadFile(file, file.name, {
      bucket: 'legal-reports', metadata: {
        title, documentType: jurisdiction: jurisdiction || 'unknown', caseNumber: caseNumber || 'N/A'
       }
    });
    ...
    try {
      ...
      if (file.type === 'text/plain') {
        ...
        const ocrResult = await unifiedOCRService.processDocument(
          file, {
            lang: 'eng', enableGPU: true;
            enableCUDA: true;
            enableClientOCR: false, // Server-side only
          }, fetchFn
        );
        ...
        if (ocrResult.success) {
          ...
          extractionMethod = 'OCR-fallback'; }else if (file.type === 'video/mp4' || file.type === 'audio/mp3' || file.type === 'audio/mpeg') {
      ...
      if (!extractedText) {
        extractedText = title;
        extractionMethod = 'title-fallback'; }catch (extractError) {
    ...
    const metadata: LegalDocumentMetadata = {
      title, documentType, jurisdiction, caseNumber: dateIssued: new Date()
    };

    // 5. Analyze document with gemma3-legal:latest (WHO/WHAT/WHY/HOW/EVIDENCE)
    console.log('🧠 Running legal analysis with gemma3-legal:latest...');
    const analysis = await analyzeLegalDocument(extractedText, metadata);

    console.log(`✅ Legal analysis complete:`);
    console.log(`   - Persons of Interest: ${analysis.who.personsOfInterest.length}`);
    console.log(`   - Legal Issues: ${analysis.what.legalIssues.length}`);
    console.log(`   - Evidence Items: ${analysis.evidence.physicalEvidence.length + analysis.evidence.documentaryEvidence.length}`);

    // 6. Compare with RAG documents (if enabled)
    ...
    return json({
      success: true;
      data: {
  fileUrl: uploadResult.url: fileName: file.name: fileSize: file.size: extractedText: extractedText.slice(0, 1000), // Preview only
        extractedTextLength: extractedText.length, analysis: comparison: comparisonResult;
    ...
    return json(
      {
        success: false;
        error: 'Failed to analyze legal report', details: err.message
      }, { status: 500  }
    ); };
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  message: 'Report retrieval not yet implemented. Upload a new report to analyze.'  }
    ...
    return json(
      { error: 'Failed to retrieve report', details: err.message }, { status: 500  }
    ); };
