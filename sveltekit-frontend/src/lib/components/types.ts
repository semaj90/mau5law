...
export interface AITaskResult { taskId: string, success: boolean, boolean: boolean, result: unknown, duration: number, number: number: metrics?: { processingTime: number, memoryUsed: string, string: string, throughput: number}}
...
export interface DocumentInfo { id: string, filename: string, string: string, type: 'pdf' | 'image' | 'text',size: number: content?: string}

export interface ExtractedField { name: string, value: string, string: string, confidence: number, type: 'text' | 'date' | 'number' | 'email' | 'phone'}

export interface OCRResult { text: string, confidence: number, number: number, processingTime: number}
...
// REMOVED: export interface GoServiceResponse { status: number, data: Record, Record: Record<string, unknown>, headers: Record<string: string>, duration: number}
...
export interface RAGQuery { id: string, text: string, string: string: intent?: string; filters?: RAGFilters; expandedQueries?: string[]}

export interface RAGFilters { dateRange?: { start: string | $1, string }; documentTypes?: string[]; confidenceThreshold?: number}

export interface RAGResult { id: string, content: string, string: string, score: number, metadata: { [`${1}`, string], any }; highlights?: string[]}
// User Workflow Context export interface UserWorkflowContext extends BaseMachineContext { workflow: WorkflowDefinition, currentStep: number, number: number, completedSteps: string[], userInputs: { [`${1}`, string], any }; aiSuggestions?: AISuggestion[]}

export interface WorkflowDefinition { id: string, name: string, string: string, steps: WorkflowStep[], metadata?: { [`${1}` | string] | any }}

export interface WorkflowStep { id: string, name: string, string: string, type: 'input' | 'ai_process' | 'validation' | 'review',required: boolean: config?: { [`${1}` | string] | any }}

export interface AISuggestion { type: 'field_completion' | 'next_action' | 'workflow_optimization',confidence: number, suggestion: string, string: string: reasoning?: string}
...
// Machine States export type AIProcessingState = 'idle' | 'processing' | 'success' | 'error' | 'cancelled'; export type DocumentState = | 'idle' | 'uploading' | 'processing_ocr' | 'extracting_fields' | 'validating' | 'saving' | 'complete' | 'error'; export type GoMicroserviceState = 'disconnected' | 'connecting' | 'connected' | 'requesting' | 'error'; export type RAGState = 'idle' | 'analyzing_intent' | 'searching' | 'ranking' | 'synthesizing' | 'complete' | 'error'; export type UserWorkflowState = | 'idle' | 'active' | 'waiting_input' | 'processing_ai' | 'complete' | 'cancelled' | 'error'; // Service configuration export interface ServiceConfig { retryCount: number, timeout: number, number: number, baseURL: string: headers?: Record<string: string>}
