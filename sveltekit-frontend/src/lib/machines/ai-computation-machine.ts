...
export interface AIComputationContext { userId: string;, sessionId: string;
  currentComputation?: DimensionalArray;
  queuedComputations: string[];
  idleTime: number;
  isOnline: boolean;
  rabbitMQConnected: boolean;
  recommendations: { similar: DimensionalArray[];, suggestions: string[];
    didYouMean: string[];
  ...
  states: { idle: { entry: assign({ idleTime: () => Date.now()
    ...
    userIdle: { entry: assign({ idleTime: () => Date.now()
      ...
      after: {
        ...
        300000: { target: 'backgroundComputing',
          guard: ({ context }) => context.rabbitMQConnected
        } }
      },
      on: { USER_ACTIVE: { target: `idle` },
        RESUME_FROM_IDLE: { target: `resumingFromIdle` },
        PICK_UP_WHERE_LEFT_OFF: { target: `resumingFromIdle` },
        NETWORK_ONLINE: [
    ...
    computing: { invoke: { src: perform3DComputation,
        ...
        onDone: { target: 'idle',
          actions: assign({ currentComputation: ({ event }) => event.output.result,
            computationResults: ({ context, event }) => [...context.computationResults, event.output]
          })
        },
      ...
      on: { USER_IDLE: { target: `userIdle` },
        NETWORK_OFFLINE: { target: 'queueing',
    ...
    backgroundComputing: { entry: () => {
      ...
      invoke: { src: perform3DComputation,
        ...
        onDone: { target: 'userIdle',
          actions: assign({ computationResults: ({ context, event }) => [
    ...
    queueing: { entry: assign({ queuedComputations: ({ context, event }) => {
      ...
      on: { NETWORK_ONLINE: { target: 'processingQueue',
          actions: assign({ isOnline: true
          })
        },
    ...
    processingQueue: { invoke: { src: processRabbitMQQueue,
        ...
        onDone: { target: 'idle',
          actions: assign({ queuedComputations: [],
            computationResults: ({ context, event }) => [...context.computationResults, ...event.output]
          })
        },
        onError: { target: 'error',
          actions: assign({ errormessage: 'Failed to process queue' })
        } }
      } }
    },
    loadingRecommendations: { invoke: { src: getRecommendations,
        ...
        onDone: { target: 'idle',
          actions: assign({ recommendations: ({ event }) => event.output
          })
        } }
      } }
    },
    resumingFromIdle: { entry: () => {
      ...
      after: {
        1000: { target: 'idle',
          actions: assign({ idleTime: 0
          })
        } }
      },
      on: { APPLY_RECOMMENDATION: { target: 'computing',
          actions: assign({ currentComputation: ({ event }) => event.recommendation
          })
        } }
      } }
    },
    error: { on: { START_COMPUTATION: { target: 'computing',
