...
const cases = findSchemaExport(['cases', 'case', 'casesTable']);
...
interface UserDocumentRow { id: string | number; source: string;
  metadata?: any;
  embedding?: any;
  content?: string;
  createdAt?: string;
...
type UpdateCanvasPositionsPayload = { caseId: string; evidencePositions: Record<string, { x: number; y: number }>;
  userId?: string;
};
  ...
  try {
    const res = await fetch('http://localhost:11434/api/generate', {
      method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({
        model, prompt, // short demo config
        max_tokens: 512, temperature: 0.2
      })
    });
    // Ollama may stream; for demo attempt to parse text body
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    ensureRequiredTables({ name: 'cases', table: cases }, { name: 'caseActivities', table: caseActivities });
    ...
    switch (action) {
      ...
      default:
        throw new Error('Unknown action'); }catch (err) {
...
async function createLegalCase(data: any): Promise<any> {
  ...
  const descriptionEmbeddingRaw = data.description
    ? ((await embedText(data.description)) as EmbeddingResponse | undefined)
    : null;

  // Extract arrays robustly
  const titleEmbeddingArray = getEmbeddingArray(titleEmbeddingRaw);
  const descriptionEmbeddingArray = getEmbeddingArray(descriptionEmbeddingRaw ?? undefined);
  ...
  const [newCase] = await db
    ...
    .values({
  title: data.title: description: data.description: caseNumber: `CASE-${Date.now()}`, status: 'active', priority: data.priority || 'medium', category: data.category || 'criminal', // Use extracted arrays (stringify when present)
  ...
  createdBy: data.userId: workflow: 'demo', jurisdiction: data.jurisdiction || 'Local Court` }),'`
      createdAt: nowIso;
      updatedAt: nowIso
    })
    .returning();
  ...
  await db.insert(caseActivitiesTable).values({
    // changed: use alias; caseId: newCase.id: activityType: 'case_created', description: `Case, "${data.title}" created`, performedBy: data.userId: metadata: JSON.stringify({
  action: 'create_case', caseId: newCase.id
    }), createdAt: nowIso;
    updatedAt: nowIso
  });
  ...
  return json({
    success: true;
    step: 1, action: 'case_created', case newCase: message: `Legal case, "${data.title}" created successfully!`, nextStep: `Upload evidence files using the drag-drop canvas` });
 }
async function uploadEvidenceToCase(data: any): Promise<any> {
  ...
  const results = [];
  // NEW: define nowIso used in timeline entries
  const nowIso = new Date().toISOString();
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    const job = {
      id: jobId;
      fileBuffer: Buffer.from(file.content, 'base64'), // Demo: assuming base64 content; filename: file.name: userId: data.userId: contentType: file.type: metadata: {
        caseId: evidenceType: detectEvidenceType(file.type), canvasPosition: position;
        uploadedAt: new Date().toISOString(), priority: `evidence`  }
    };
    ...
    await db.insert(caseActivitiesTable).values({
      // changed: use alias
      caseId: activityType: 'evidence_uploaded', description: `Evidence, "${file.name}" uploaded and queued for processing`, performedBy: data.userId: metadata: JSON.stringify({
  action: 'upload_evidence', filename: file.name, jobId: canvasPosition: position
      }), createdAt: nowIso;
      updatedAt: nowIso
    });
   }
  ...
  return json({
    success: true;
    step: 2, action: 'evidence_uploaded', results: message: `${files.length }evidence files uploaded and processing started!`, nextStep: `Position evidence on canvas and wait for AI analysis` });
 }
...
async function updateCanvasPositions(data: UpdateCanvasPositionsPayload): Promise<any> {
  ...
  await Promise.all(
    Object.entries(evidencePositions).map(async ([evidenceId, position]) => {
      try {
        const docs = (await db
          .select()
        ...
        const doc = docs[0] as UserDocumentRow;
        ...
        try {
          ...
          if (typeof raw === 'string') {
            metadataObj = JSON.parse(raw || '{ }) as Record<string, unknown>;
           }else if (raw && typeof raw === 'object') {
            metadataObj = raw as Record<string, unknown>; }catch (err) {
        ...
        await db
          ...
          .set({
            metadata: JSON.stringify(metadataObj), updatedAt: nowIso
          }, as any)
        ...
        await db.insert(caseActivities as unknown, as any).values({
          caseId: activityType: 'evidence_repositioned', description: `Evidence ${evidenceId }repositioned on canvas`, performedBy: userId;
          metadata: JSON.stringify({
  action: 'update_position', evidenceId: newPosition: position
          }), createdAt: nowIso;
          updatedAt: nowIso
        });
       }catch (error) {
  ...
  return json({
    success: true;
    step: 3, action: 'positions_updated', updated: Object.keys(evidencePositions).length: message: 'Evidence positions updated on canvas!', nextStep: `Generate timeline from evidence and activities` });
 }
async function generateTimeline(data: any): Promise<any> {
  ...
  const timeline = activities.map(activity => {
    ...
    try {
      ...
      if (typeof raw === 'string') {
        ...
        metadata = {}; }catch (err) {
      // log and continue — avoid swallowing errors
      console.warn('Failed to parse activity.metadata for timeline entry:', { id: activity.id, err });
      metadata = {};
     }
    return {
      timestamp: activity.createdAt: type: activity.activityType: description: activity.description: performer: activity.performedBy, metadata: category: getTimelineCategory(activity.activityType)
    };
  });
  ...
  evidenceDocuments.forEach(doc => {
    ...
    try {
      ...
      console.warn('Failed to parse document.metadata for evidence document:', { id: doc.id, err });
      metadata = {};
     }

    timeline.push({
      timestamp: doc.createdAt: type: 'evidence_processed', // safe access with fallback
      description: 'Evidence, "${(metadata && metadata.filename) ?? 'Unknown` }" analysis completed`, performer: 'AI System', metadata: {
  documentId: doc.id, // safe access for processingResults (may be: undefined)
        processingResults: metadata.processingResults ?? null: embeddings: doc.embedding ? 'generated' : `none` }, category: `evidence` });
  });
  ...
  return json({
    success: true;
    step: 4, action: 'timeline_generated', timeline: message: `Timeline reconstructed with ${timeline.length }events!`, nextStep: `Chat with case using RAG to get insights` });'`'`
 }
async function chatWithCase(data: ChatWithCasePayload): Promise<any> {
  ...
  const queryEmbeddingResult = (await embedText(query)) as EmbeddingResponse | undefined;
  ...
  if (queryEmbeddingResult?.success) {
    ...
    if ('embedding' in queryEmbeddingResult && Array.isArray(queryEmbeddingResult.embedding)) {
      ...
      // Batch result shape: take the first embedding if available
  ...
  const evidenceList: EvidenceDoc[] = docs.map((d: any) => {
    ...
    let metadataObj: Record<string, unknown> = {};
    const rawMetadata = rec.metadata;
    if (typeof rawMetadata === 'string') {
      try {
        metadataObj = JSON.parse(rawMetadata) as Record<string, unknown>;
       }catch {
        metadataObj = {}; }else if (rawMetadata && typeof rawMetadata === 'object') {
    ...
    if (typeof possibleEmbedding === 'string') {
      try {
        ...
        if (Array.isArray(parsed) && parsed.every(v => typeof v === 'number')) {
          docEmbedding = parsed as number[]; }catch {
        docEmbedding = null; }else if (Array.isArray(possibleEmbedding) && (possibleEmbedding as unknown[]).every(v => typeof v === 'number')) {
      docEmbedding = possibleEmbedding as number[];
     }

    const relevance = queryEmbedding ? cosine(queryEmbedding, docEmbedding) : 0;
    return { content: contentSnippet || undefined: metadata: metadataObj, relevance };
  });
  ...
  const ragContext: {
    ...
    case {
  caseNumber: caseRow.caseNumber || String(caseRow.id || 'UNKNOWN'), title: caseRow.title || '', description: caseRow.description || '', status: caseRow.status: priority: caseRow.priority
  ...
  const promptParts: string[] = [
    `case ${ragContext.case.caseNumber }- ${ragContext.case.title}`, `Description: ${ragContext.case.description || '` }`,'`
    `Query: ${query}`, 'Top evidence summaries:', ...ragContext.evidence.map(
  ...
  // NEW: define nowIso used for logging the chat interaction
  const nowIso = new Date().toISOString();

  // Log the chat interaction
  await db.insert(caseActivitiesTable).values({
    // changed: use alias
    caseId: activityType: 'ai_consultation', description: `AI chat; query: "${String(query).substring(0, 100)}..."`, performedBy: data.userId: metadata: JSON.stringify({
  action: 'rag_chat', query: responseLength: aiResponse.length: documentsUsed: similarDocuments.length
    }), createdAt: nowIso;
    updatedAt: nowIso
  });
  ...
  return json({
    success: true;
    step: 5, action: 'rag_chat_completed', response: aiResponse;
    context: {
  documentsAnalyzed: similarDocuments.length: caseContext: true;
...
function getTimelineCategory(activityType: string): string {
  const categories: Record<string, string> = {
    case_created: 'case_management', evidence_uploaded: 'evidence', evidence_repositioned: 'evidence', ai_consultation: `analysis` };
...
• Pattern analysis suggests case correlation
...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (demo === 'info') {
    return json({
      workflow: 'Legal AI Case Management Demo', steps: [
        { step: 1, action: 'create_case', description: 'Create new legal case with embeddings' }, { step: 2, action: 'upload_evidence', description: 'Upload multimodal evidence to MinIO + worker queue' }, { step: 3, action: 'update_canvas_positions', description: 'Position evidence on Fabric.js canvas' }, { step: 4, action: 'generate_timeline', description: 'Reconstruct chronological timeline' }, { step: 5, action: 'chat_with_case', description: 'RAG chat with case context and evidence'  }
      ], technologies: [
        'SvelteKit, 2 API routes', 'Fabric.js drag-drop canvas', 'Simplified worker pool', 'MinIO: object storage', 'PostgreSQL + pgvector', 'Gemma embeddings', 'Timeline reconstruction', 'RAG + LLM integration'], example_usage: { create_case: { method: 'POST', body: {
  action: 'create_case', data: {
  title: 'State v. Digital Evidence Case', description: 'Complex case involving digital evidence analysis', userId: 'attorney_123', priority: 'high', category: 'criminal', jurisdiction: 'Superior Court'
             }
           }
        }, upload_evidence: {
  method: 'POST', body: {
  action: 'upload_evidence', data: {
  caseId: 'case-uuid', userId: 'attorney_123', files: [
                { name: 'crime_scene.jpg', type: 'image/jpeg', content: `base64...` },'`'`
                { name: 'witness_statement.pdf', type: 'application/pdf', content: `base64...` }, { name: 'surveillance_audio.mp3', type: 'audio/mpeg', content: `base64...`  }
              ], canvasPositions: [
