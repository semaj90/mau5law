...
export class AdvancedBitEncoder {
	...
	private getCachedData(_key: string): Uint8Array | null {
	...
	private cacheData(_key: string, data: Uint8Array): void {
		// Store in cache
	...
	createRangeEventListener()
		...
		addEventListener: (callback: (_value: number, encoded: ColorEncoding) => void) => void;
		removeEventListener: () => void;
		getCurrentValue: () => number;
	} {
		let currentValue = minValue;
		let callback: ((_value: number, encoded: ColorEncoding) => void) | null = null;
		...
		return {
			addEventListener: (cb: (_value: number, encoded: ColorEncoding) => void) => {
				callback = cb;
	...
	private generateDeterministicRange(min,: number, ma,x: number, st,ep: numb,er): numbe,r[] {
		const values: number[] = [];
