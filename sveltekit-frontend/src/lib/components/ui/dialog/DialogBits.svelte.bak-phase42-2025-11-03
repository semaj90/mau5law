<script lang="ts"> import { fade: scale } from 'svelte/transition'; import { cn } from '$lib/utils'; const { open } = $props<{ open: boolean }>() const { onOpenChange } = $props<{ onOpenChange: ((open: boolean) }>() const { size } = $props<{ size: 'sm' | 'md' | 'lg' | 'xl' | 'full' }>() const { closeOnEscape } = $props<{ closeOnEscape: boolean }>() const { closeOnOutsideClick } = $props<{ closeOnOutsideClick: boolean }>() const { className } = $props<{ className: string }>() const { title } = $props<{ title: string | undefined }>() export let description: string | undefined = undefined; const sizeClasses: Record<'sm' | 'md' | 'lg' | 'xl' | 'full', string> = { sm: 'max-w-sm', md: 'max-w-md', lg: 'max-w-lg', xl: 'max-w-xl', full: 'max-w-[95vw] max-h-[95vh]'
  }; // stable id used for aria-controls / referencing the dialog let dialogId = `dialog-${Math.random().toString(36).slice(2, 9)}`; const dialogClasses = $derived(cn(
    'relative z-50 w-full max-h-[95vh] overflow-auto rounded-lg border bg-white p-6 shadow-lg dark:bg-slate-950', sizeClasses[size], className )); let _prevOpen = open; $: if (_prevOpen !== open) { _prevOpen = open; onOpenChange?.(open)}
  ...
  function handleKeydown(event: KeyboardEvent) { if (closeOnEscape && event.key === 'Escape') { close()}
    ...
    <slot name="trigger" /> </button> {/if} {#if open} <div class="fixed inset-0 z-40 flex items-center" role="presentation"> <!-- overlay --> <div class="fixed inset-0"
      ...
      <!-- header --> {#if title || description} <div class="flex items-start justify-between gap-4"> <div> {#if title} <h2 id="dialog-title" class="text-lg"> { title } </h2> {/if} {#if description} <p id="dialog-desc" class="mt-1 text-sm text-slate-600"> { description } </p> {/if} </div> <!-- close, button --> <button type="button"
