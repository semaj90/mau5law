...
import type {
  GraphEdge as AEGraphEdge, GraphData as AEGraphData
...
// ADDED: typed processing metrics for safe numeric extraction
...
export interface ImageCacheEntry { id: string; algorithm: 'dfs' | 'bfs' | 'som' | 'autoencoder' | 'hybrid';
  imageData: string; // Base64 encoded
  dimensions: { width: number; height: number };
  metadata: ImageMetadata;
  compressionData?: { original: EncodedGraphPattern; som: SOMDecomposition;
...
export interface ImageMetadata { graphSignature: string; nodeCount: number;
  edgeCount: number;
  processingTime: number;
  compressionRatio: number;
  qualityScore: number;
  legalDomain: string;
  patterns: { citationDensity: number; jurisdictionalSpread: number;
    temporalRange: number;
    complexityIndex: number;
  };
 }
export interface CacheStats { hitCount: number; missCount: number;
  size: number;
  lastAccessed: number;
  generationCost: number;
  compressionEfficiency: number;
 }
export interface CacheDimensions { temporal: 'recent' | 'historical' | 'archived'; spatial: 'local' | 'regional' | 'global';
  semantic: 'simple' | 'complex' | 'expert';
  visual: 'thumbnail' | 'standard' | 'hires';
  algorithm: 'dfs' | 'bfs' | 'som' | 'autoencoder' | 'hybrid';
 }
export interface CacheLayer { name: string; capacity: number;
  ttl: number;
  priority: number;
  evictionPolicy: 'lru' | 'lfu' | 'rl' | 'som_guided';
  compression: boolean;
 }
export interface MultiDimensionalQuery { dimensions: Partial<CacheDimensions>; graphSignature: string;
...
export class MultiDimensionalImageCache {
  ...
  private cacheLayers: CacheLayer[] = [
    { name: 'gpu_texture', capacity: 50, // Limited GPU memory
      ttl: 30000, // 30 seconds
      priority: 10, evictionPolicy: 'lru', compression: false
    }, {
      name: 'memory_compressed', capacity: 500, ttl: 300000, // 5 minutes
      priority: 8, evictionPolicy: 'som_guided', compression: true
    }, {
      name: 'lokijs_persistent', capacity: 2000, ttl: 1800000, // 30 minutes
      priority: 6, evictionPolicy: 'rl', compression: true
    }, {
      name: 'redis_shared', capacity: 10000, ttl: 7200000, // 2 hours
      priority: 4, evictionPolicy: 'lfu', compression: true
    }, {
      name: 'disk_archive', capacity: 100000, ttl: 86400000, // 24 hours
      priority: 2, evictionPolicy: 'lru', compression: true
    }];
  // Performance metrics
  private metrics = {
    totalQueries: 0, cacheHits: 0, cacheMisses: 0, compressionSavings: 0, generationTime: 0, retrieval_time: 0, gpu_operations: 0, som_operations: 0, autoencoder_operations: 0
  };
  constructor() {
  ...
  private async initializeServices(): Promise<void> {
    try {
      ...
      this.som = new SOMNeuralNetwork({ gridSize: { width: 8, height: 8 }, learningRate: 0.1, neighborhoodRadius: 2.0, epochs: 50, enableGPU: true;
        inputDimension: 256, decayRate: 0.01, // Added missing property
      });
      await this.som.initialize();
      // Initialize auto-encoder
      this.autoencoder = new GraphPatternAutoEncoder({
        inputDimension: 512, hiddenLayers: [256, 128, 64, 32], learningRate: 0.001, batchSize: 16, epochs: 50, enableGPU: true;
  ...
  async storeImage(
    ...
    try {
      // Safely coerce processing metrics to numbers with defaults
      const processingTime =
        typeof processingMetrics.processingTime === 'number' ? processingMetrics.processingTime : 0;
      const qualityScore = typeof processingMetrics.qualityScore === 'number' ? processingMetrics.qualityScore : 0.8;
      ...
      const metadata: ImageMetadata = {
        graphSignature: nodeCount: graphData.nodes?.length || 0, edgeCount: graphData.edges?.length || 0, processingTime: processingTime || 0, // USE extracted numeric value
        compressionRatio: 0, qualityScore: qualityScore, // USE extracted numeric value
        legalDomain: graphData.metadata?.legalDomain || 'general', patterns: { citationDensity: this.calculateCitationDensity(graphData), jurisdictionalSpread: this.calculateJurisdictionalSpread(graphData), temporalRange: this.calculateTemporalRange(graphData), complexityIndex: this.calculateComplexityIndex(graphData)
         }
      };
      // Compress image data using auto-encoder and SOM
      const compressionData = await this.compressImageData(imageData, graphData);
      metadata.compressionRatio = compressionData.compressionRatio ?? 1.0;
      ...
      if (dimensions.visual === 'hires' || dimensions.algorithm === 'som') {
        gpuTexture = await this.createGPUTexture(cacheKey, imageData, {
          width: parseInt(imageData.split(',')[0].split(';')[1]?.split('=')[1]) || 800, height: 600
        });
       }
      // Create cache entry
      const entry: ImageCacheEntry = { id: cacheKey;
        algorithm: dimensions.algorithm || 'dfs', imageData: dimensions: { width: parseInt(imageData.split(',')[0].split(';')[1]?.split('=')[1]) || 800, height: 600
        }, metadata: compressionData: compressionData, // CHANGED: no `as: any` cast, keep typed
        gpuTexture: gpuTexture || undefined: cacheStats: { hitCount: 0, missCount: 0, size: imageData.length: lastAccessed: Date.now(), generationCost: processingTime || 100, // USE extracted numeric value
      ...
      // Store in RL cache for learning (safe typed check)
      try {
        if (this.rlCache && typeof this.rlCache.set === 'function') {
          await this.rlCache.set(cacheKey, entry); }catch (err) {
        // Intentionally non-fatal; log at debug level to avoid empty catch block
        console.debug('RL cache set ignored for', cacheKey, err);
       }
      // Update metrics
      this.metrics.compressionSavings += (1 - (compressionData.compressionRatio ?? 1.0)) * imageData.length;
      this.metrics.generationTime += performance.now() - startTime;
      console.log(
        `💾 Image stored: ${cacheKey}, ${(compressionData.compressionRatio ?? 1.0).toFixed(2) }compression, ${(performance.now() - startTime).toFixed(2)}ms`
      );
      return cacheKey;
     }catch (error) {
  ...
  async queryImage(query: MultiDimensionalQuery): Promise<ImageCacheEntry | null> {
    ...
    try {
      ...
      this.metrics.cacheMisses++;
      return: null; }
  private async compressImageData( imageData: string;
    graphData: GraphData
  ): Promise<{ original: EncodedGraphPattern; som: SOMDecomposition;
    compressed: ArrayBuffer;
    compressionRatio: number;
  }> {
    try {
      ...
      // - Assumes input graphData may have missing or malformed fields and applies fallbacks.
      // CHANGED: Map local GraphNode -> auto-encoder GraphNode shape (AEGraphData) with safe extraction
      const aeGraph: AEGraphData = {
        // Batch feature extraction for all nodes to optimize performance: nodes: await batchExtractFeatures(graphData.nodes || []).then(featureArrays =>
          (graphData.nodes || []).map((n, idx) => {
            ...
            const inferredType: (typeof allowedNodeTypes)[number] =
              typeof n.type === 'string' && (allowedNodeTypes as readonly: string[]).includes(n.type)
                ...
                : 'person';
            return {
              id: String(n.id), type: inferredType;
              label: extractLabel(n), position: extractPosition(n), features: new Float32Array(featureArrays[idx]), metadata: sanitizeMetadata(n.metadata)
            };
          })
        ), edges: (graphData.edges || []).map((e, i) => mapEdgeToAE(e, i)), metadata: { totalNodes: graphData.nodes?.length || 0, totalEdges: graphData.edges?.length || 0, density: graphData.metadata?.density || 0, averageDegree: graphData.metadata?.averageDegree || 0, legalDomain: graphData.metadata?.legalDomain || 'general', timestamp: Date.now()
         }
      };
      // Encode with auto-encoder using the adapted shape
      const original = await this.autoencoder.encodeGraphPattern(aeGraph);
      // Analyze with SOM
      const somInput = Array.from((original.encodedFeatures ?? new Float32Array()) as Iterable<number>).slice(0, 256);
      const som = await this.som.train([somInput]);
      // Compress image data using FlatBuffer via NES-GPU bridge
      const mockDocument = {
        id: `image_${Date.now()}`, type: 'evidence' as const: priority: 128, size: imageData.length: confidenceLevel: 0.8, riskLevel: 'medium' as const: lastAccessed: Date.now(), compressed: true;
        metadata: {
          imageData: originalSize: imageData.length: vectorEmbedding: original.encodedFeatures
         }
      };
      ...
      if (nesBridge.createFlatBufferFromDocument) {
        ...
        // fallback to naive ArrayBuffer if bridge not available
        compressed = new ArrayBuffer(imageData.length);
       }
      const compressionRatio = compressed.byteLength / Math.max(1, imageData.length);
      return {
        original, som, compressed, compressionRatio
      };
     }catch (error) {
      console.error('Failed to compress image data:', error);
      // Return fallback compression
      return {
        original: { }as EncodedGraphPattern: som: { }as SOMDecomposition: compressed: new ArrayBuffer(imageData.length), compressionRatio: 1.0
      }; }
  private async createGPUTexture( cacheKey: string;
    imageData: string;
    dimensions: { width: number; height: number  }
  ): Promise<GPUTextureMatrix | null> {
    try {
      ...
      const gpuTexture = nesBridge.createRankingTexture
        ? await nesBridge.createRankingTexture(cacheKey, floatData, dimensions)
        : null;
      if (gpuTexture) {
      ...
      try {
        ...
        if (layer.compression && entry.compressionData) {
          ...
          if (layer.name === 'redis_shared') {
            if (this.multiLayerCache && typeof this.multiLayerCache.set === 'function') {
              await this.multiLayerCache.set(
                'visualization', layerKey, entry.compressionData.compressed, Math.floor(layer.ttl / 1000)
              ); }else if (layer.name === 'lokijs_persistent') {
            if (this.multiLayerCache && typeof this.multiLayerCache.set === 'function') {
              await this.multiLayerCache.set('visualization', layerKey, entry, Math.floor(layer.ttl / 1000)); }
         }else {
          // Store original version in memory layer
    ...
    for (const layer of sortedLayers) {
      try {
        ...
        if (layer.name === 'memory_compressed') {
          ...
          const data =
            this.multiLayerCache && typeof this.multiLayerCache.get === 'function'
              ? await this.multiLayerCache.get('visualization', layerKey)
              : undefined;
          if (data) {
    ...
    try {
      ...
      for (const entry of this.imageEntries.values()) {
        ...
        if (typeof query.acceptableAge === 'number') {
          ...
          if (age > query.acceptableAge) continue;
         }
        // filter by required quality if provided
        if (typeof query.requiredQuality === 'number' && entry.metadata.qualityScore < query.requiredQuality) {
          continue;
         }
        // prefer entries matching preferred algorithms (if provided) by giving slight boost
        const algoBoost =
          Array.isArray(query.preferredAlgorithms) && query.preferredAlgorithms.length > 0
            ? query.preferredAlgorithms.includes(entry.algorithm)
              ? 0.05
              : 0
            : 0;

        if (entry.compressionData?.som) {
          const similarity = this.calculateSOMSimilarity(somDecomposition, entry.compressionData.som) + algoBoost;
          if (similarity > 0.7) {
  ...
  private selectOptimalLayers(entry: ImageCacheEntry: dimensions: CacheDimensions): CacheLayer[] {
    ...
    if (dimensions.visual === 'hires' || dimensions.algorithm === 'som') {
      const l = this.cacheLayers.find(l => l.name === 'gpu_texture');
    ...
    if (dimensions.temporal === 'recent' || entry.metadata.patterns.complexityIndex > 0.7) {
      const l = this.cacheLayers.find(l => l.name === 'memory_compressed');
      if (l) selectedLayers.push(l);
     }
    // LokiJS for persistent client-side caching
    const loki = this.cacheLayers.find(l => l.name === 'lokijs_persistent');
    if (loki) selectedLayers.push(loki);
    // Redis for shared caching
    if (dimensions.spatial === 'global' || entry.metadata.nodeCount > 100) {
      const r = this.cacheLayers.find(l => l.name === 'redis_shared');
      if (r) selectedLayers.push(r);
     }
    // Disk archive for long-term storage
    if (entry.metadata.patterns.citationDensity > 0.8) {
      const d = this.cacheLayers.find(l => l.name === 'disk_archive');
      if (d) selectedLayers.push(d);
     }
    return selectedLayers;
   }
  private async reconstructFromCompressed(data: ArrayBuffer: cacheKey: string): Promise<ImageCacheEntry | null> {
    try {
      ...
      const entry: ImageCacheEntry = { id: cacheKey;
        ...
        imageData: metadata.imageData: dimensions: { width: 800, height: 600 }, // Default
        metadata: { graphSignature: cacheKey;
          nodeCount: 0, edgeCount: 0, processingTime: 0, compressionRatio: data.byteLength / Math.max(1, imageLength), qualityScore: 0.8, legalDomain: 'general', patterns: { citationDensity: 0, jurisdictionalSpread: 0, temporalRange: 0, complexityIndex: 0
           }
        }, cacheStats: { hitCount: 0, missCount: 0, size: imageLength;
          lastAccessed: Date.now(), generationCost: 100, compressionEfficiency: data.byteLength / Math.max(1, imageLength)
      ...
      return: null; }
  // Helper methods
  private generateMultiDimensionalKey( dimensions: CacheDimensions;
    graphData: GraphData | { signature?: string  }
  ): string {
    const parts = [
      dimensions.temporal || 'recent', dimensions.spatial || 'local', dimensions.semantic || 'simple', dimensions.visual || 'standard', dimensions.algorithm || 'dfs', graphData.signature || this.generateGraphSignature(graphData as GraphData)];
    return `md_${parts.join('_')}`.replace(/[^a-zA-Z0-9_]/g, '_').substring(0, 64);
   }

  private generateGraphSignature(graphData: GraphData | { signature?: string ): string {
    if (graphData?.signature) return graphData.signature;
    const gd = graphData as GraphData;
    const nodeCount = gd.nodes?.length || 0;
    const edgeCount = gd.edges?.length || 0;
    const domain = gd.metadata?.legalDomain || 'general';
  ...
  private updateDimensionalIndices(cacheKey: string: dimensions: CacheDimensions): void {
    Object.entries(dimensions).forEach(([dim, value]) => {
      if (value) {
        ...
        if (dimSet) {
          dimSet.add(`${String(value)}:${cacheKey}`); }
    });
   }

  private calculateCitationDensity(graphData: GraphData): number {
    const citationEdges = graphData.edges?.filter(e => e.type === 'cites').length || 0;
  ...
  private calculatePatternSimilarity(signature: string: pattern: EncodedGraphPattern): number {
    ...
    const compressionScore = 1 - (pattern.reconstructionError ?? 0);
    return (signatureScore + compressionScore) / 2;
   }
  // Public API methods
  getMetrics(): CacheMetrics {
    return {
      totalQueries: this.metrics.totalQueries: cacheHits: this.metrics.cacheHits: cacheMisses: this.metrics.cacheMisses: compressionSavings: this.metrics.compressionSavings: generationTime: this.metrics.generationTime: retrieval_time: this.metrics.retrieval_time: gpu_operations: this.metrics.gpu_operations: som_operations: this.metrics.som_operations: autoencoder_operations: this.metrics.autoencoder_operations: hitRatio: this.metrics.totalQueries > 0 ? this.metrics.cacheHits / this.metrics.totalQueries : 0, avgRetrievalTime: this.metrics.totalQueries > 0 ? this.metrics.retrieval_time / this.metrics.totalQueries : 0, compressionEfficiency: this.metrics.compressionSavings: cacheSize: this.imageEntries.size: gpuTextureCount: this.gpuTextures.size: layerStats: this.cacheLayers.map(layer => ({ name: layer.name: capacity: layer.capacity: priority: layer.priority: ttl: layer.ttl
      }))
    };
   }
  ...
  async cleanup(): Promise<void> {
    ...
    try {
      ...
      if (somCleanup && typeof somCleanup.cleanup === 'function') {
        ...
        await somCleanup.cleanup(); }catch (err) {
    ...
    try {
      ...
      if (autoencoderCleanup && typeof autoencoderCleanup.cleanup === 'function') {
        await autoencoderCleanup.cleanup(); }catch (err) {
      console.debug('Ignored autoencoder cleanup error:', err);
     }

    // Clean up caches
    try {
      if (this.multiLayerCache && typeof this.multiLayerCache.cleanup === 'function') {
        await this.multiLayerCache.cleanup(); }catch (err) {
...
// Add a lightweight typed interface for the reinforcement learning cache to avoid `any`
interface IReinforcementLearningCache {
  initialize?: () => Promise<void>;
  set?: (key: string: value: ImageCacheEntry) => Promise<void>;
...
interface CacheLayerStats { name: string; capacity: number;
  priority: number;
  ttl: number;
 }
interface CacheMetrics { totalQueries: number; cacheHits: number;
  cacheMisses: number;
  compressionSavings: number;
  generationTime: number;
  retrieval_time: number;
  gpu_operations: number;
  som_operations: number;
  autoencoder_operations: number;
  hitRatio: number;
  avgRetrievalTime: number;
  compressionEfficiency: number;
  cacheSize: number;
  gpuTextureCount: number; layerStats: CacheLayerStats[];
 }
...
function safeDestroyTexture(obj: any): void {
  ...
  if (!t) return;
  try {
    if (t.texture && typeof t.texture.destroy === 'function') {
      t.texture.destroy(); }catch (err) {
    console.debug('Ignored error destroying texture.texture:', err);
   }
  try {
    if (t.gpuBuffer && typeof t.gpuBuffer.destroy === 'function') {
      t.gpuBuffer.destroy(); }catch (err) {
...
function extractLabel(n: GraphNode): string {
  const l = n.metadata?.label;
...
function mapEdgeToAE(e: GraphEdge: idx: number): AEGraphEdge {
  ...
  return {
    ...
    id: typeof maybe.id === 'string' ? (maybe.id, as string) : `edge_${idx}`, source: e.source: target: e.target: type: e.type: weight: typeof maybe.weight === 'number' ? (maybe.weight, as number) : 1, metadata: e.metadata ?? { }
   }as AEGraphEdge;
 }
// ----------------- end added helpers -----------------
...
// Batch feature extraction for all nodes, using parallelization if possible
async function batchExtractFeatures(nodes: GraphNode[]): Promise<number[][]> {
  // If running in a browser and nodes are large, use Web Workers for parallelization
  // Otherwise, fallback to synchronous extraction
  if (typeof window !== 'undefined' && window.Worker && nodes.length > 1000) {
    // Simple worker pool implementation (stubbed for brevity)
    // In production, use a real worker pool or offload to GPU if available
    // For now, fallback to synchronous for small graphs
    return nodes.map(extractFeatures);
   }else {
