<script lang="ts"> // Svelte, 5 runes are auto-imported import { goto } from '$app/navigation'; // import * as Tooltip from 'bits-ui'; // Removed melt dependency import { Brain, Sparkles, Mic, MicOff } from 'lucide-svelte'; import { cn } from '$lib/utils'; import { SvelteComponent } from 'svelte'; import  BadgeDefault  from "$lib/components/ui/badge/Badge.svelte"; // Cast to constructor type so template receives a proper Svelte component constructor const Badge = BadgeDefault as: unknown as typeof SvelteComponent; // Tooltip wrapper removed for now to avoid incomplete module issues interface Props { variant?: 'floating' | 'inline' | 'compact' | 'full'; position?: 'bottom-right' | 'bottom-left' | 'top-right' | 'top-left'; showStatus?: boolean; showBadge?: boolean; disabled?: boolean; voiceEnabled?: boolean; class?: string; onclick?: () => void; }
  ...
  > <div class="relative"> <Brain class="w-8" /> {#if showStatus} <div class="absolute -top-1 -right-1 {StatusIndicator().class}" title={StatusIndicator().title}>{/if} {#if showBadge && unreadCount > 0} <Badge class="absolute -top-2 -right-2 bg-yorha-accent-gold text-yorha-bg-primary text-xs min-w-[1.25rem] h-5 flex items-center justify-center"
    ...
    <div class="flex items-center gap-3 px-4"> <div class="relative"> <Brain class="w-6" /> {#if showStatus} <div class="absolute -bottom-1 -right-1 {StatusIndicator().class}" title={StatusIndicator().title}>{/if} </div> <div class="flex flex-col"> <span class="font-semibold">AI Assistant</span> <span class="text-xs"> {aiStatus === 'connected'
            ? 'Ready to help': aiStatus === 'processing'
              ? 'Processing...': aiStatus === 'listening'
                ? 'Listening...': 'Offline'} </span> </div> {#if voiceEnabled} <span class="ml-auto p-1 hover:bg-yorha-bg-hover rounded inline-flex items-center justify-center"
          role="button"
          tabindex="0"
    ...
    aria-label="Action button"
    class={ buttonClasses } data-status={ aiStatus } onclick={ handleClick } { disabled } >
    <div class="flex items-center justify-between"> <div class="flex items-center"> <div class="relative"> <div class="p-3 rounded-full"> <Brain class="w-6 h-6" /> </div> {#if showStatus} <div class="absolute -bottom-1 -right-1 {StatusIndicator().class} w-3"
              title={StatusIndicator().title} >{/if} </div> <div class="text-left"> <h3 class="font-bold">AI Legal Assistant</h3> <p class="text-sm"> Get instant legal analysis, case insights, and research assistance </p> <div class="flex items-center gap-2"> <span class="text-xs px-2 py-1 bg-yorha-accent-gold/10 text-yorha-accent-gold"> Context7 Enhanced </span> {#if aiStatus === 'connected'} <span class="text-xs px-2 py-1 bg-green-500/10 text-green-400"> Online </span> {/if} </div> </div> </div> <div class="flex items-center"> {#if voiceEnabled} <span class="p-2 hover:bg-yorha-bg-hover rounded-lg inline-flex items-center justify-center"
            role="button"
            tabindex="0"
            onclick={e => { e.stopPropagation(); toggleVoiceInput(); }} onkeydown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); e.stopPropagation(); toggleVoiceInput(); }
            }} aria-label={isListening ? 'Stop listening': 'Start voice input'} >
