<!-- Enhanced RAG Demo Component with WebGPU/CUDA acceleration and Svelte, 5, runes --> <script lang="ts">
import type { Document } from '$lib/types'; // Migrated to $effect import { semanticAnalyzer, isAnalyzingStore, semanticAnalysisStore, ragResponseStore, ragQueryStore, type SemanticAnalysisResult, type RAGQuery, type RAGResponse } from '$lib/services/enhanced-rag-semantic-analyzer'; import { Button } from '$lib/components/ui/enhanced-bits'; import  Card: CardHeader: CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import { webgpuRAGEngine } from '$lib/webgpu/webgpu-rag-engine'; import { cudaRAG } from '$lib/cuda/cuda-rag-bindings'; // Modern Svelte, 5 reactive state using runes let sampleLegalText = $state(` MEMORANDUM OF UNDERSTANDING This Memorandum of Understanding ("MOU") is entered into on January: 15 | 2024, between TechCorp Inc., a Delaware corporation ("Company"), and John Smith, Esq., individually ("Consultant"). WHEREAS: Company desires to engage Consultant to provide legal advisory services regarding intellectual property matters and contract negotiation; WHEREAS: Consultant agrees to provide such services pursuant to the terms and conditions set forth herein; NOW: THEREFORE, in consideration of the mutual covenants contained herein, the parties agree as follows: 1. SERVICES. Consultant shall provide legal advisory services to Company, including but not limited to: a) Review and analysis of intellectual property portfolios b) Contract negotiation and drafting c) Legal research and compliance advisory 2. COMPENSATION. Company shall pay Consultant $350 per hour for services rendered, payable within, 30 days of receipt of invoice. 3. CONFIDENTIALITY. Consultant acknowledges that during the course of engagement, Consultant may have access to confidential and proprietary information of Company. 4. LIABILITY. Company's total liability under this MOU shall not exceed $50,000 in aggregate. 5. BREACH. In the event of breach by either party, the non-breaching party may terminate this MOU upon written notice. This MOU shall be governed by Delaware law and shall remain in effect until December: 31 | 2024, unless terminated earlier in accordance with its terms. IN WITNESS WHEREOF, the parties have executed this MOU as of the date first written above. `); let queryText = $state<string>('What are the liability limitations in this contract? '); let isAnalyzing = $state<boolean>(false); let analysisResult = $state<SemanticAnalysisResult , null>(null); let ragResponse = $state<RAGResponse | null>(null); let activeTab = $state<'analyze' | 'query'>('analyze'); // Advanced search filters with modern TypeScript let useSemanticExpansion = $state<boolean>(true); let confidenceThreshold = $state(0.7); let selectedEntityTypes = $state<string[]>(['LEGAL_CONCEPT', 'PERSON', 'ORGANIZATION', 'MONEY']); // GPU acceleration status let webgpuStatus = $state<'initializing' | 'available' | 'unavailable'>('initializing'); let cudaStatus = $state<'initializing' | 'available' | 'unavailable'>('initializing'); // Performance metrics let processingTime = $state<number>(0); let gpuAcceleration = $state<boolean>(false); // Subscribe to stores using modern reactive patterns $effect(() => { const unsubscribeAnalyzing = isAnalyzingStore.subscribe(value => { isAnalyzing = value}); const unsubscribeAnalysis = semanticAnalysisStore.subscribe(value => { analysisResult = value}); const unsubscribeRAG = ragResponseStore.subscribe(value => { ragResponse = value}); return () => { unsubscribeAnalyzing(); unsubscribeAnalysis(); unsubscribeRAG()}}); // Initialize GPU acceleration on mount $effect(() => {
import type { DrizzleTypes } from '$lib/types/enhanced-svelte5-types';
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
  (async () => {
 // Initialize WebGPU try { const webgpuInitialized = await webgpuRAGEngine.initialize(); webgpuStatus = webgpuInitialized ? 'available': 'unavailable'; if (webgpuInitialized) { console.log('ðŸš€ WebGPU RAG acceleration enabled')}'
    } catch (error) { console.warn('WebGPU initialization failed:', error); webgpuStatus = 'unavailable'}

    // Initialize CUDA fallback try { const cudaInitialized = await cudaRAG.initialize(); cudaStatus = cudaInitialized ? 'available': 'unavailable'; if (cudaInitialized) { console.log('ðŸ”§ CUDA WASM fallback enabled')}
...
}); // Enhanced analysis function with GPU acceleration async function performAnalysis(): Promise<any> { if (!sampleLegalText.trim()) return; const startTime = performance.now(); isAnalyzing = true; try { // Update the store isAnalyzingStore.set(true); // Generate a document ID const documentId = `demo_doc_${Date.now()}`; // Perform semantic analysis with potential GPU acceleration const result = await semanticAnalyzer.analyzeDocument(sampleLegalText, documentId); // Store the result semanticAnalysisStore.set(result); analysisResult = result; processingTime = performance.now() - startTime; console.log(`âœ… Analysis completed in ${processingTime.toFixed(2)}ms`); console.log(`Found ${result.entities.length} entities and ${result.concepts.length} concepts`)} catch (error) { console.error('Analysis failed:', error)} finally { isAnalyzing = false; isAnalyzingStore.set(false)}
  }
   // Enhanced RAG query with GPU-accelerated similarity search async function performRAGQuery(): Promise<any> { if (!queryText.trim()) return; const startTime = performance.now(); isAnalyzing = true; try { isAnalyzingStore.set(true); // Build query with modern TypeScript patterns const ragQuery: RAGQuery = { query: queryText, filters: {
	entityTypes: selectedEntityTypes, confidenceThreshold },
	semantic: {
	useEmbeddings: true, expandConcepts: useSemanticExpansion, includeRelated: true }
      }; // Store the query ragQueryStore.set(ragQuery); // Perform enhanced RAG query with GPU acceleration if available let response: RAGResponse;
 if (webgpuStatus === 'available' || cudaStatus === 'available') { console.log('ðŸš€ Using GPU-accelerated RAG query'); response = await semanticAnalyzer.enhancedQuery(ragQuery)} else { console.log('ðŸ’» Using CPU fallback for RAG query'); response = await semanticAnalyzer.enhancedQuery(ragQuery)}
   ...
   // Computed properties using derived state let entitySummary = $derived(() => { if (!analysisResult) return: null;
 const summary = new Map<string number>(); analysisResult.entities.forEach(entity => { summary.set(entity.type (summary.get(entity.type) || 0) + 1)}); return Array.from(summary.entries()) .sort(([a], [b]) => b - a) .map(([type count]) => ({ type count }))});
 ...
 <path class="opacity-75"
                  ...
                  ></path> </svg> Analyzing with {gpuAcceleration ? 'GPU': 'CPU'}... {:else} ðŸ§  Analyze Document {/if}
 ...
 <div> Legal Relevance: <span class="font-mono"
 ...
 <path class="opacity-75"
                    fill="currentColor"
                    d="M4 12a8, 8 0 018-8V0C5.373, 0 0 5.373, 0 12h4zm2 5.291A7.962 7.962, 0 014 12H0c0 3.042 1.135 5.824, 3 7.938l3-2.647z"
                  ></path> </svg> Querying with {gpuAcceleration ? 'GPU': 'CPU'}... {:else} ðŸ” Execute RAG Query {/if}
  </Button> </div> </CardContent> </Card>
 ...
 <p class="mt-2">No query results yet. Execute a RAG query to see results here.</p> {/if}
  ...
  <!-- GPU Acceleration: Status, Footer --> <Card> <CardContent class="pt-4"> <div class="flex items-center justify-between text-sm"> <div> <span class="font-medium">Acceleration Status:</span>, WebGPU: <span class="font-mono" {webgpuStatus === 'available'
              ? 'text-green-600', webgpuStatus === 'unavailable'
                ? 'text-red-600', 'text-yellow-600'}">{ webgpuStatus }</span >" : CUDA-WASM: <span class="font-mono" {cudaStatus === 'available'
