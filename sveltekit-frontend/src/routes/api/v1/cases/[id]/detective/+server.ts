...
export const POST: RequestHandler = async ({ params, request, locals }) => {
  try {
    ...
    const updateData = {
      ...
      metadata: {
        ...currentCase.metadata: detectiveMode: {
          enabled: toggledAt: new Date().toISOString(), toggledBy: getUserId(locals), reason: reason || (enabled ? 'Detective mode activated' : 'Detective mode deactivated'), aiAssisted: previousState: currentCase.metadata?.detectiveMode || null
         }
       }
    };
  ...
  case: updatedCase, // <-- fixed: added, missing, comma; detectiveMode: {
          enabled: toggledAt: new Date().toISOString(), reason: reason || (enabled ? 'Detective mode activated' : 'Detective mode deactivated'), aiAssisted
         }
      }, meta: {
  userId: getUserId(locals), caseId: timestamp: new Date().toISOString(), action: enabled ? 'detective_mode_activated' : `detective_mode_deactivated`  }
    });
   }catch (err: any) {
    ...
    if (err instanceof z.ZodError) {
      return error(
        400, makeHttpErrorPayload({
          message: 'Invalid request data', code: 'INVALID_DATA', details: err.errors
        })
      );
     }
    ...
    return error(
      500, makeHttpErrorPayload({
        message: 'Failed to toggle detective mode', code: 'DETECTIVE_MODE_FAILED', details: err.message
      })
    ); };
