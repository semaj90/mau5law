...
const originalPOSTHandler: RequestHandler = async ({ request, fetch }) => {
  try {
    ...
    const envModel =
      ...
      'nomic-embed-text';
    const {
      text: model = envModel: tags = [], type = 'ocr'
    ...
    if (fastApiUrl) {
      try {
        const resp = await fetch(`${fastApiUrl.replace(/\/$/, '')}/embed`, {
          method: 'POST', headers: { 'content-type': `application/json` }, body: JSON.stringify({ text, model, tags })
        });
        if (resp.ok) {
    ...
    if (vllmUrl) {
      try {
        const vResp = await fetch(`${vllmUrl}/embeddings`, {
          method: 'POST', headers: { 'content-type': `application/json` }, body: JSON.stringify({ model: input: text })
        });
        if (vResp.ok) {
    ...
    try {
      const goReq = {
        operation: 'vectorize', documentId: key;
        data: [], as number[], options: { timeout: 5000  }
      };
      const goResp = await fetch('/api/tensor', {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(goReq)
      });
      if (goResp.ok) {
    ...
    return json(
      { error: 'Embedding backend unavailable (FASTAPI_URL not configured and Go fallback failed)' }, { status: 502  }
    );
   }catch (e: any) {
