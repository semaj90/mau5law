 ...
 */
export interface EmbeddingModelConfig { id: string; name: string;
...
export const EMBEDDING_MODELS: Record<string, EmbeddingModelConfig> = {
  ...
  'embeddinggemma': {
    id: 'embeddinggemma', name: 'Gemma Embeddings', provider: 'ollama', dimensions: 384, maxTokens: 2048, latency: 100, accuracy: 0.9, specialized: true;
    capabilities: ['general-text', 'semantic-search', 'similarity', 'legal-text', 'context-understanding']
  }, 'embeddinggemma:latest': {
    id: 'embeddinggemma:latest', name: 'Gemma Embeddings Latest', provider: 'ollama', dimensions: 384, maxTokens: 2048, latency: 95, accuracy: 0.91, specialized: true;
    capabilities: ['general-text', 'semantic-search', 'similarity', 'legal-text', 'context-understanding']
  }, // Fallback: nomic-embed-text (reliable backup, also 384-dim)
  'nomic-embed-text': {
    id: 'nomic-embed-text', name: 'Nomic Embed Text', provider: 'ollama', dimensions: 384, maxTokens: 2048, latency: 120, accuracy: 0.85, specialized: false;
    capabilities: ['general-text', 'semantic-search', 'similarity', 'legal-text']
  }, // Specialized legal embedding: 'legal-bert-embeddings': {
    id: 'legal-bert-embeddings', name: 'Legal-BERT Embeddings', provider: 'local', dimensions: 768, maxTokens: 512, latency: 50, accuracy: 0.9, specialized: true;
    capabilities: ['legal-text', 'case-law', 'legal-entity-extraction']
   }
};
export const EMBEDDING_FALLBACK_CHAINS = {
  'legal-general': ['embeddinggemma:latest', 'embeddinggemma', 'nomic-embed-text', 'legal-bert-embeddings'], 'legal-fast': ['embeddinggemma', 'legal-bert-embeddings', 'nomic-embed-text'], 'general': ['embeddinggemma:latest', 'embeddinggemma', 'nomic-embed-text']
};
export function getOptimalEmbeddingModel(
  taskType: 'legal-general' | 'legal-fast' | 'general' = 'legal-general', availableModels: string[] = []
): string[] {
