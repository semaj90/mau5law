  ...
  actors: {
    ...
    systemHealthCheck: fromPromise(async () => {
      try {
        ...
        const healthStatus = checks.every(check => check.status === 'fulfilled' && check.value.status === 'healthy') ? 'healthy' :
          checks.some(check => check.status === 'fulfilled' && check.value.status === 'healthy') ? 'degraded' : 'critical';
        ...
        vectorSearch: {
          invoke: {
            ...
            onDone: {
              ...
              actions: assign({
                vectorMatches: ({ event }) => (event.output.matches || []).map((match: Omit<VectorMatch, 'rank'>, index: number) => ({
                  ...match,
                  rank: index + 1
                }))
              })
