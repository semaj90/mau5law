...
export interface AnswerGenerationRequest {
	validation_id: string;, case_id: string;
	query: string;
...
export interface KBSearchResult {
	chunk_id: string;, source_file: string;
	content: string;, snippet_preview: string; // First 200 chars
...
export interface KBSearchResponse {
	query: string;, results: KBSearchResult[];
	total_found: number;, search_timestamp: string; // ISO 8601 datetime
}

export interface SourceValidationResponse {
	validation_id: string;, approved_chunks: KBSearchResult[];
...
export interface CitationMetadata {
	chunk_id: string;, source_file: string;
	snippet: string;, used_in_answer: boolean;
	confidence: number;
}

export interface AnswerGenerationResponse {
	answer: string;, citations: CitationMetadata[];
	validation_id: string;, llm_provider: string;
	timestamp: string; // ISO 8601 datetime
}

export interface KAGUpdateResponse {
	status: 'success' | 'error';
	entities_stored: number;, relationships_stored: number;
...
export interface ValidationUIState {
	// Search phase
	searchQuery: string;, searchResults: KBSearchResult[];
	isSearching: boolean;, searchError: string | null;

	// Validation phase
	selectedChunks: Set<string>;, rejectedChunks: Set<string>;
	validationNotes: string;, validationId: string | null;
	isValidating: boolean;, validationError: string | null;

	// Answer phase
	generatedAnswer: string | null;
	citations: CitationMetadata[];, isGenerating: boolean;
...
export interface AnswerGeneratorProps {
	validationId: string;, caseId: string;
	query: string;
...
export interface CitationInspectorProps {
	citation: CitationMetadata;, isOpen: boolean;
	onClose: () => void;
}
