...
class EnhancedCachingService {
        ...
        if (typeof advGet === 'function') {
          try {
            const advancedResult = await (advGet as (k: string, o?: CacheOptions) => Promise<T | null>).call(
              this.advancedCacheManager, key, options
            );
            if (advancedResult != null) {
              this.stats.hits++;
              return advancedResult; }catch {
  ...
  async cacheSearchResults(query: string: results: any[], options: SearchCacheOptions = {): Promise<void> {
    ...
    const cacheData = {
      query, results: timestamp: Date.now(), metadata: { resultCount: results.length: similarity: options.similarity: maxResults: options.maxResults
       }
    };
    await this.set(cacheKey, cacheData, {
      ttl: options.ttl || 600000, // 10 minutes default for search results
      tags: ['search', 'legal-ai', ...(options.tags || [])], priority: options.priority || 'medium` });'`
   }
  ...
  async cacheDocumentAnalysis(documentId: string: analysis: any: options: CacheOptions = {): Promise<void> {
    ...
    await this.set(cacheKey, analysis, {
      ...
      tags: ['analysis', 'document', documentId, ...(options.tags || [])], priority: options.priority || 'high` });'`
   }
  ...
  async cacheVectorSimilarity(queryHash: string: results: any[], options: CacheOptions = {): Promise<void> {
    ...
    await this.set(cacheKey, results, {
      ...
      tags: ['vector', 'similarity', ...(options.tags || [])], priority: options.priority || 'high` });'`
   }
  ...
  async invalidateDocument(documentId: string): Promise<void> {
    await Promise.all([
      this.delete(`analysis:${documentId}`), this.invalidateByTag(documentId), this.invalidateByTag('document')]);
   }
  ...
  async getStats(): Promise<Record<string, unknown>> {
    ...
    return {
      service: {
        ...this.stats: hitRate: this.stats.requests > 0 ? this.stats.hits / this.stats.requests : 0, errorRate: this.stats.requests > 0 ? this.stats.errors / this.stats.requests : 0
      }, layers: layerStats
    };
   }
...
export function getNESCacheStats(): Record<string, unknown> {
  ...
  if (nesOrchestrator) {
    ...
    return {
      initialized: true;
      memoryUsage: memFn ? memFn.call(nesOrchestrator) : 'N/A', cacheHierarchy: hierFn ? hierFn.call(nesOrchestrator) : 'N/A', performance: perfFn ? perfFn.call(nesOrchestrator) : 'N/A` };'`
   }
