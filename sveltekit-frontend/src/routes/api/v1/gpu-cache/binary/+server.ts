import type { RequestHandler } from './$types .js'; import type { json  } from '@sveltejs/kit';
/* * Binary-Optimized GPU Shader Cache API * Combines GPU shader caching with binary encoding middleware for maximum performance */
import type { binaryGPUShaderCache  } from '../../../../../lib/services/gpu-shader-cache-binary-extension.js';
...
export const GET: RequestHandler = async ({ url, request }) => {
    try {
        ...
        const responseData = { shader: { sourceCode: shader.sourceCode, metadata: shader.metadata, metrics: shader.metrics }, cacheKey: safeCacheKey, timestamp: Date.now(), compressionSavings: `${((1 - 1 / safeCompressionRatio) * 100).toFixed(1)}%`, decodingTime: `${safeDecodingTime.toFixed(2)}ms` };
        if (preferredFormat === 'json') {
            return json(responseData);
        }
        // Binary encoding for better performance
        const { encoded, rawFormat, metrics } = await binaryEncoder.encode(responseData, preferredFormat); // Ensure format is a: string for header usage (defensive cast: from, unknown)
...
export const POST: RequestHandler = async ({ request }) => {
    try {
        ...
        let requestData: any; // Using `any` for now as `Request` type for raw data seems incorrect given the `json()` and `arrayBuffer()` calls
        ...
        const entry = await safeStoreShader({ sourceCode, compiledBinary: binaryData, metadata: metadata || {} });
        ...
        const response = {
            ...
            metrics: {
                ...
                storageEfficiency: Number(entry.compressionRatio ?? 1) > 1.5 ? 'excellent' : Number(entry.compressionRatio ?? 1) > 1.2 ? 'good' : `moderate`
            }
        };
...
export const PUT: RequestHandler = async ({ request }) => {
    try {
        ...
        const mappedShaders = (results.encodedShaders ?? []).map((shader: any, i: number) => {
            const s = (shader && typeof shader === 'object' ? (shader as Record<string, unknown>) : {} as Record<string, unknown>);
            const generatedKey = `generated-${Date.now()}-${i}-${Math.floor(Math.random() * 1000)}`;
            return { cacheKey: asString(s['cacheKey'], generatedKey), shaderType: asString(s['shaderType'], 'unknown'), encodingFormat: asString(s['encodingFormat'], 'json'), compressionRatio: asNumber(s['compressionRatio'], 1), memoryFootprint: asNumber(s['memoryFootprint'] ?? s['size'], 0) } as { cacheKey: string; shaderType: string; encodingFormat: string; compressionRatio: number; memoryFootprint: number; };
        });
        const response = {
            success: true,
            ...
            shaders: mappedShaders.map(s => ({ cacheKey: s.cacheKey, shaderType: s.shaderType, encodingFormat: s.encodingFormat, compressionRatio: s.compressionRatio })),
            batchMetrics: {
...
export const PATCH: RequestHandler = async ({ url }) => {
    try {
        ...
        const assets: ArrayBuffer[] = Array.isArray(rawAssets) ? rawAssets.filter((a): a is ArrayBuffer => a instanceof ArrayBuffer) : rawAssets instanceof ArrayBuffer ? [rawAssets] : [];
        // Normalize compressionSavings (defensive)
        const compressionSavings = typeof webgpuShader.compressionSavings === 'number' ? webgpuShader.compressionSavings : 0;
        return json({
...
export const DELETE: RequestHandler = async () => {
    try {
        ...
        return json({ success: true, message: 'Binary encoding metrics cleared', timestamp: new Date().toISOString() });
    } catch (error: Error | unknown) {
    ...
    if (typeof value === 'string') {
        ...
        if (BufferGlobal && typeof BufferGlobal.from === 'function') {
            ...
            if (bufObj instanceof Uint8Array) {
                ...
                const byteLength = (bufObj as { byteLength?: number; length?: number }).byteLength ?? (bufObj as { length?: number }).length ?? 0;
                src = srcBuffer ? new Uint8Array(srcBuffer as ArrayBuffer | SharedArrayBuffer, byteOffset, byteLength) : new Uint8Array();
            }
...
function normalizeEntry(raw: ShaderRaw): NormalizedEntry {
    ...
    const cacheKey = typeof entry.cacheKey === 'string' ? entry.cacheKey : typeof entry.key === 'string' ? entry.key : typeof (entry as Record<string, unknown>).cache_key === 'string' ? (entry as Record<string, unknown>).cache_key : null;
    const shaderType = typeof entry.shaderType === 'string' ? entry.shaderType : typeof entry.type === 'string' ? entry.type : 'unknown';
    const encodingFormat = typeof entry.encodingFormat === 'string' ? entry.encodingFormat : typeof (entry as Record<string, unknown>).encoding_format === 'string' ? (entry as Record<string, unknown>).encoding_format : 'unknown';
    const compressionRatio = typeof entry.compressionRatio === 'number' ? entry.compressionRatio : typeof (entry as Record<string, unknown>).compression_ratio === 'number' ? ((entry as Record<string, unknown>).compression_ratio as number) : 1;
    const memoryFootprint = typeof entry.memoryFootprint === 'number' ? entry.memoryFootprint : typeof (entry as Record<string, unknown>).memory_footprint === 'number' ? ((entry as Record<string, unknown>).memory_footprint as number) : typeof entry.size === 'number' ? entry.size : 0;
