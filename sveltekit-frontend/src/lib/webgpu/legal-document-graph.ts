  ...
  async loadGraphFromDB(graphId: string): Promise<void> {
    ...
    try {
      ...
      this.nodes = (graphData.nodes || []).map(n => ({
        id: n.id,
        position: [n.position?.x ?? 0, n.position?.y ?? 0, n.position?.z ?? 0],
        velocity: [0, 0, 0],
        force: [0, 0, 0],
        mass: n.mass ?? 1,
        size: n.size ?? 5,
