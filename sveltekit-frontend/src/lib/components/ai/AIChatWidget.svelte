...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import * as Dialog from '$lib/components/ui/dialog/index.js'; // --- Types --- type AISourceContext = { title?: string; description?: string; fullText?: string } | null; type Props = { open?: boolean; context?: AISourceContext; title?: string; placeholder?: string; caseId?: string | null; documentId?: string | null}; interface ChatMessage { id: number, role: 'user' | 'assistant' | 'system'; content: string;, timestamp: string, type?: string; metadata?: unknown; suggestions?: string[]; error?: boolean}
  let { open = $bindable(false), context = null, title = 'AI Legal Assistant', placeholder = 'Ask about legal matters...', caseId = null, documentId = null } = $props() as Props; // Use typed state to avoid `never` inferences let messages = $state<ChatMessage[]>([]); let currentMessage = $state<string>(''); let isLoading = $state<boolean>(false); let chatContainer: HTMLElement | null = null; let inputElement: HTMLTextAreaElement | null = null; // Auto-scroll to bottom when messages change (capture ref to, avoid: "possibly: null" inside timeout) $effect(() => { if (messages.length > 0 && chatContainer) { const container = chatContainer; setTimeout(() => { if (container) container.scrollTop = container.scrollHeight}, 100)}
  }); // Focus input when dialog opens (capture ref) $effect(() => { try { if (open && inputElement) { const input = inputElement; setTimeout(() => { if (input) input.focus()}, 100)}
   ...
   // --- Fixed sendMessage: valid fetch, JSON parsing, and error handling --- async function sendMessage(): Promise<any> { if (!currentMessage.trim() || isLoading) return; const userMessage: ChatMessage = { id: Date.now(), role: 'user', content: currentMessage.trim(); timestamp: new Date().toISOString() }; messages = [...messages, userMessage]; const messageToSend = currentMessage.trim(); currentMessage = ''; isLoading = true; try { let contextText = ''; if (context) { contextText = `Context: ${context.title}\n${context.description || ''}\n${context.fullText || ''}`}
      const response = await fetch('/api/ai/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ message: messageToSend, context: contextText ? [contextText]: undefined | caseId, documentId; temperature: 0.7 }) }); if (!response.ok) { throw new Error(`AI API error: ${response.status}`)}
      let data: { response?: string; performance?: unknown; suggestions?: string[] } | undefined; try { data = await response.json()} catch (err) { console.error('JSON parsing failed:', err); throw new Error('Invalid JSON response from AI API')}
      const aiMessage: ChatMessage = { id: Date.now() + 1, role: 'assistant', content: data?.response || 'I apologize, but I could not generate a response.', timestamp: new Date().toISOString(), metadata: data?.performance || undefined; suggestions: data?.suggestions || [] }; messages = [...messages, aiMessage]} catch (error) { console.error('AI chat error:', error); const errorMessage: ChatMessage = { id: Date.now() + 1, role: 'assistant', content: 'I apologize, but I encountered an error. Please try again.', timestamp: new Date().toISOString(); error: true }; messages = [...messages, errorMessage]} finally { isLoading = false}
  }
  ...
  async function provideFeedback(messageId: number | string, feedback: 'positive' | 'negative'): Promise<any> { try { await fetch('/api/ai/feedback', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify({ messageId, feedback, caseId, documentId }) })} catch (error) { console.error('Failed to provide feedback:', error)}
  }
