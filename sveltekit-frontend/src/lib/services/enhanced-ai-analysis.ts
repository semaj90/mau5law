...
export interface SemanticAnalysis {
 ...
 sentiment: {
 score: number; // -1 to 1, confidence: number;
 aspects: { aspect: string; sentiment: number }[];
 };
 complexity: {
 score: number; // 0 to 1, factors: string[];
...
1. A concise summary (2-3 sentences) private async analyzeSentiment(text: string) {
 ...
 */
 ...
 `${sentences} sentences total` private async findSimilarDocuments(embedding: number[]): string { private buildReasoningPrompt(text: string, context?: string[]) {
...
Return JSON format: private parseReasoningResponse(response: string): LegalReasoning {
 try {
...
Response:`;
 ...
 const entities = this.parseAndValidateEntities((response as { response?: string }).response, text);
 return entities;
 } catch (error) {
