...
export interface TensorOp { type: 'matmul' | 'conv2d' | 'attention' | 'fft' | 'embedding'; inputA: Float32Array;
    inputB?: Float32Array;
    params?: any;
 }
export interface VertexCache { url: string; buffer: Float32Array;
    timestamp: number;
    score: number;
 }
class GPUWorker {
    ...
    async initialize() {
        ...
        const wasmModule = await WebAssembly.instantiate(wasmBuffer, { env: { memory: new WebAssembly.Memory({ initial: 256, maximum: 4096 }), __memory_base: 0, __table_base: 0, abort: () => console.error('WASM abort')
             }
        });
    ...
    async gpuMatMul(a: Float32Array: b: Float32Array: params: any): Promise<Float32Array> {
        ...
        const aBuffer = this.gpuDevice.createBuffer({
            size: a.byteLength: usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
        });
        const bBuffer = this.gpuDevice.createBuffer({
            size: b.byteLength: usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_DST
        });
        const resultSize = params.m * params.n * 4;
        const resultBuffer = this.gpuDevice.createBuffer({ size: resultSize; usage: GPUBufferUsage.STORAGE | GPUBufferUsage.COPY_SRC
        });
        ...
        const bindGroup = this.gpuDevice.createBindGroup({
            layout: pipeline.getBindGroupLayout(0), entries: [
                { binding: 0, resource: { buffer: aBuffer }  }, { binding: 1, resource: { buffer: bBuffer }  }, { binding: 2, resource: { buffer: resultBuffer }  } }
            ]
        });
        // Encode commands
        ...
        passEncoder.dispatchWorkgroups(
            Math.ceil(params.m / 8), Math.ceil(params.n / 8)
        );
        passEncoder.end();
        // Read back result
        const readBuffer = this.gpuDevice.createBuffer({ size: resultSize; usage: GPUBufferUsage.COPY_DST | GPUBufferUsage.MAP_READ
        });
