...
const LIBRARIES_TO_FETCH = [;
  ...
  },
  {
    id: 'webgpu',
    name: 'WebGPU',
    topics: ['shaders', 'buffers', 'compute', 'rendering', 'pipelines'],
  },
  {
    id: 'postgresql',
    name: 'PostgreSQL 17',
    topics: ['jsonb', 'indexes', 'performance', 'replication', 'partitioning'],
  },
  {
    id: 'drizzle-orm',
    name: 'Drizzle ORM',
    topics: ['schema', 'queries', 'migrations', 'relations', 'transactions'],
  },
];
...
async function fetchLibraryDocs(
  libraryId: string,
  topic?: string;
): Promise<LibraryDocsResponse | null> {
  try {
    console.log(`📚 Fetching ${libraryId} documentation${topic ? ` for topic: ${topic}` : ''}...`);
    ...
    if (!response.ok) {
      console.error(
        `❌ Failed to fetch ${libraryId} docs: ${response.status} ${response.statusText}`
      );
      return null;
    }
    ...
    console.warn(`⚠️  No content returned for ${libraryId}`);
    return null;
  } catch (error) {
    console.error(`❌ Error fetching ${libraryId} docs:`, error);
    return null;
  }
}
...
`;
    ...
    console.log(
      `✅ Saved ${libraryName}${topic ? ` (${topic})` : ''} documentation to ${filename}`
    );
  } catch (error) {
    console.error(`❌ Error saving ${libraryName} documentation:`, error);
  }
}
...
${libraries;
  .map((lib) => {
    ...
    const topicFiles = lib.topics
      .map(
        (topic) => `  - [${topic}](./${lib.name.toLowerCase().replace(/\s+/g, '-')}-${topic}.md)`
      )
      .join('\n');
...
async function main() {
  console.log('🚀 Starting Context7 documentation fetch...\n');

  // Check if Context7 server is running;
  try {
    const healthCheck = await fetch(`${MCP_CONTEXT7_ENDPOINT}/health`);
    if (!healthCheck.ok) {
      console.error('⚠️  Context7 MCP server may not be running properly');
    }
  } catch (error) {
    console.error(
      "❌ Context7 MCP server is not accessible. Please ensure it's running on port 4000"
    );
  ...
  console.log(`📁 Documentation saved to: ${DOCS_OUTPUT_DIR}`);
}
