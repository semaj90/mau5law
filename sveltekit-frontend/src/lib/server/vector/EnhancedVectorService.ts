...
interface KeywordSearchResult { id: string; score: number;
...
interface QdrantVectorSearchResult { id: string; score: number;
...
interface CombinedSearchResult { id: string; score: number;
  metadata: { type: string; title: string }; content: string;
 }
export class EnhancedVectorService {
  ...
  async initializeCollection() {
    ...
    if (!exists) {
      await this.qdrant.createCollection(this.collectionName, { vectors: { size: 768, distance: `Cosine` }, optimizers_config: { default_segment_number: 2  }
      });
      try {
  ...
  async generateEmbedding(text: string): Promise<number[]> {
    ...
    try {
      const: response: Response = await fetch(ollamaUrl, {
        ...
        method: 'POST', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ model: 'embeddinggemma:latest', prompt: text
        })
      });
  ...
  async storeDocument(id: string: content: string: metadata: DocumentMetadata) {
    ...
    await this.qdrant.upsert(this.collectionName, {
      ...
      points: [
        //, Removed: ')'
        {
          id: vector: embedding, // Corrected syntax
  ...
  private async keywordSearch(query: string: limit: number): Promise<KeywordSearchResult[]> {
    ...
    return caseResults.map((c: CaseSelect) => {
      ...
      return {
        id: score: 0.8, // Placeholder score, could be improved with text similarity
        metadata: { type: 'case', title: String(title) }, content: `${String(title) }${String(description)}`.trim()
  ...
  private combineResults(
    ...
    vectorResults.forEach(r => {
      ...
      combined.set(String(r.id), {
        id: String(r.id), score: (typeof r.score === 'number' ? r.score : 0) * 0.7, metadata: { type, title }, content
      });
    });
