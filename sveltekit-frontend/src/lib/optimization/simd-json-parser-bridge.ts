...
export class SIMDJSONParserBridge {
  ...
  private parseNative(jsonString: string, cacheKey?: string): SIMDParseResult {
    ...
    try {
      ...
      this.nativeParseStats.avgTimeMs = this.nativeParseStats.totalTimeMs / this.nativeParseStats.count;
  ...
  createParseStream(): Transform {
    ...
    return {
      write: async (chunk: Buffer | string, encoding: string, callback: (error?: Error | null) => void) => {
        ...
        for (let i = 0; i < buffer.length; i++) {
          ...
          if (char === '"' && (i === 0 || buffer[i - 1] !== '\\')) {
            ...
            if (depth === 0 && (char === '}' || char === ']')) {
              ...
              if (jsonStr.length > 0) {
                this.parse(jsonStr).then(result => {
                  if (result.success) {
                    // Emit parsed object
                  }
  ...
  getStats() {
    return {
      ...
      speedupRatio: this.nativeParseStats.avgTimeMs / (this.simdParseStats.avgTimeMs || this.nativeParseStats.avgTimeMs),
