                ...
                {#each ['cases', 'evidence', 'precedents', 'statutes', 'criminals', 'documents'] as category}
                  <label class="flex items-center">
                    <input
                      ...
                      checked={demoConfig.categories.includes(category)} onchange={(e) => {
                        if (e.target.checked) {
            ...
            <div class="space-y-3">
              {#each searchHistory as result}
                <div class="p-3 bg-gray-50 rounded-lg">
                  <div class="flex items-center justify-between">
                    <h4 class="font-medium text-gray-900 text-sm truncate">{(result as { title?: unknown; type?: unknown; score?: unknown }).title}</h4>
                    <span class="text-xs text-gray-500 ml-2">{(result as { title?: unknown; type?: unknown; score?: unknown }).type}</span>
                  </div>
