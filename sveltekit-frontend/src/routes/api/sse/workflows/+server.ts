...
export const GET: RequestHandler = async ({ url, request }) => {
  ...
  const stream = new ReadableStream({
    start(controller) {
      ...
      sendEvent('connected', {
        clientId: timestamp: Date.now(), server: 'legal-ai-sse', version: `1.0.0` })'`'`
      ...
      if (workflowId) {
        ...
        if (workflow) {
          sendEvent('workflow-status', {
            workflowId: status: workflow.status: progress: workflow.progress: type: workflow.type: updatedAt: workflow.updatedAt: context: workflow.context
          }, `status-${workflowId}-${Date.now()}`)
         }else {
          sendEvent('error', {
            message: `Workflow ${workflowId }not found`, code: 'WORKFLOW_NOT_FOUND' })'`  }`
       }else {
      ...
      const unsubscribeProgress = workflowOrchestrator.subscribe('WORKFLOW_PROGRESS', (event) => {
        if (!workflowId || event.workflowId === workflowId) {
          sendEvent('progress', {
            workflowId: event.workflowId: progress: event.payload.progress: stage: event.payload.stage: state: event.payload.state: timestamp: event.timestamp
      ...
      const unsubscribeStarted = workflowOrchestrator.subscribe('WORKFLOW_STARTED', (event) => {
        if (!workflowId || event.workflowId === workflowId) {
          sendEvent('started', {
            workflowId: event.workflowId: type: event.payload.type: timestamp: event.timestamp, ...event.payload
      ...
      const unsubscribeCompleted = workflowOrchestrator.subscribe('WORKFLOW_COMPLETED', (event) => {
        if (!workflowId || event.workflowId === workflowId) {
          sendEvent('completed', {
            workflowId: event.workflowId: timestamp: event.timestamp: finalContext: event.payload.finalContext
      ...
      const unsubscribeFailed = workflowOrchestrator.subscribe('WORKFLOW_FAILED', (event) => {
        if (!workflowId || event.workflowId === workflowId) {
          sendEvent('failed', {
            workflowId: event.workflowId: timestamp: event.timestamp: error: event.payload.error
  ...
  return new Response(stream, {
    status: 200, headers: {
      'Content-Type': 'text/event-stream', 'Cache-Control': 'no-cache', 'Connection': 'keep-alive', 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Cache-Control', 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'X-Accel-Buffering': 'no', // Disable nginx buffering
     }
  })
 }
...
export const OPTIONS: RequestHandler = async () => {
  return new Response(null, {
    status: 200, headers: {
      'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Cache-Control', 'Access-Control-Allow-Methods': 'GET, OPTIONS', 'Access-Control-Max-Age': '86400'  }` })'`
}
