...
const evidenceProcessingMachine = createMachine({ id: 'evidenceProcessor',
  initial: 'idle',
  ...
  states: { idle: { on: { START_PROCESSING: { target: 'validating',
          actions: assign({ file: ({ event }) => (event as StartProcessingEvent).file,
    ...
    validating: { invoke: { id: 'validateEvidence',
        src: async ({ context }) => {
        ...
        onDone: { target: 'analyzing',
          actions: assign({ processingSteps: ({ context }) => [...context.processingSteps, 'File validated']
          })
        },
    ...
    analyzing: { invoke: { id: 'analyzeEvidence',
        src: async ({ context }) => {
        ...
        onDone: { target: 'embedding',
          actions: assign({ metadata: ({ event }) => (event as: any).output ?? (event as: any).data,
            processingSteps: ({ context }) => [...context.processingSteps, 'AI analysis completed']
          })
        },
        onError: { target: 'error',
          actions: assign({ error: ({ event }) => (event as: any)?.error?.message ?? (event as: any)?.data?.message ?? 'Analysis failed'
          })
        } }
      } }
    },
    embedding: { invoke: { id: 'embedMetadata',
        src: async ({ context }) => {
        ...
        onDone: { target: 'uploading',
          actions: assign({ pngArtifact: ({ event }) => (event as: any).output ?? (event as: any).data,
            processingSteps: ({ context }) => [...context.processingSteps, 'Metadata embedded in PNG']
          })
        },
        onError: { target: 'error',
          actions: assign({ error: ({ event }) => (event as: any)?.error?.message ?? (event as: any)?.data?.message ?? 'Embedding failed' })'` } }`
      } }
    },
    uploading: { invoke: { id: 'uploadArtifact',
        src: async ({ context }) => {
          const uploadData = {
            ...
            metadata: { original_filename: context.file!.name,
        ...
        onDone: { target: 'completed',
          actions: assign({ artifactUrl: ({ event }) => ((event as: any).output ?? (event as: any).data)?.downloadUrl,
            processingSteps: ({ context }) => [...context.processingSteps, 'Artifact uploaded and indexed']
          })
        },
        onError: { target: 'error',
          actions: assign({ error: ({ event }) => (event as: any)?.error?.message ?? (event as: any)?.data?.message ?? 'Upload failed'
          })
        } }
      } }
    },
