...
export interface EvidenceRecord { id: string; title: string;
  description: string;
  caseId: string;
  ...
  updatedAt: Date;
  fileName: string;
  summary: string | null; aiSummary: string | null;
 }
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  const: newEvidence: EvidenceRecord = {
    id: title: file.name, description, caseId: criminalId: null;
    evidenceType: ext.replace('.', '') || 'document', fileUrl: `/uploads/${caseId}/${safeName}`, fileType: ext.replace('.', ''), fileSize: file.size, tags: uploadedBy: user.id: uploadedAt: now;
    updatedAt: now;
    fileName: file.name: summary: null;
    aiSummary: null
  };
  try {
