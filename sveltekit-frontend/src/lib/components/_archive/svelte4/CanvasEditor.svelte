...
<script lang="ts">
  ...
  let state: any = null;
  snapshot.subscribe(s => (state = s));

  // Helper wrapper to allow flexible event payloads (e.g. { type: 'SELECT_NODE', node })
  // Keeps TypeScript happy when events include custom properties not declared on EventObject.
  const sendEvent = (event: any) => (send as unknown as (e: any) => void)(event);
  ...
  // Core data structures (fixed from broken $state syntax)
  let fileNodes: any[] = [];
  ...
  function draw() {
    ...
    // Draw grid, connections, nodes
    drawGrid();
    drawConnections();
    fileNodes.forEach(node => drawFileNode(node));
  ...
  function drawConnections() {
    if (!ctx) return;
    nodeConnections.forEach((connection: any) => {
      ...
      ctx.lineWidth = Math.max(2, (connection.strength || 1) * 4) / zoomLevel;
      ctx.setLineDash([]);
      ctx.beginPath();
  ...
  function drawConnectionPreview() {
    // Minimal safe placeholder for preview (can be enhanced)
    // ...existing code...
  }
  ...
  function drawFileNode(node: any) {
    ...
    ctx.fillStyle = nodeGradient as any;
  ...
  function createSmartConnections(newNode: any) {
    ...
    const people = newNode.aiTags.people || [];
    const locations = newNode.aiTags.locations || [];
    const organizations = newNode.aiTags.organizations || [];

    fileNodes.forEach(existingNode => {
      if (existingNode.id === newNode.id || !existingNode.aiTags) return;

      const sharedPeople = people.filter((p: string) => existingNode.aiTags.people?.includes(p));
      const sharedLocations = locations.filter((l: string) => existingNode.aiTags.locations?.includes(l));
      const sharedOrganizations = organizations.filter((o: string) => existingNode.aiTags.organizations?.includes(o));
  ...
  function handleCanvasClick(event: MouseEvent) {
    ...
    if (clickedNode) {
      ...
      sendEvent({ type: 'SELECT_NODE', node: clickedNode });
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    if (newHoveredId !== hoveredNodeId) {
      ...
      if (canvas) canvas.style.cursor = hoveredNodeId ? 'pointer' : 'default';
      draw();
    }
  }
