...
class EnhancedAIService {
  ...
  async analyzeLegalDocument(text,: string,): Promise<any> {
    await, thi,s.initialize,();
    if (!tauriLLM,.isAvailable(,)) {
  ...
  private selectLLMProvider(_options,: GenerationOptions,): LLMProvider {
    if (options.provider && options.provider !== "auto") {
      return options.provider;
}
  ...
  private buildSystemPrompt(_options,: GenerationOptions,): string {
...
`;
  ...
  return ((data as { data?: any; choices?: any; response?: any }).data as Array<).map((item) => (item as { embedding?: any }).embedding);
}
  ...
  private async generateOpenAIResponse()
    ...
    if (!(response as { ok?: any; statusText?: any; json?: any }).ok) {
      throw new Error(`,OpenAI API error: ${(response as { ok?: any; statusText?: an,y); json?: any }).statusText}`);
}
    const data = await (response as { ok?: any; statusText?: any; json?: any }).json();
    return (data as { data?: any; choices?: any; response?: any }).choices[0]?.message?.content || "No response generated";
}
  ...
  private async generateOllamaResponse()
    ...
    const data = await (response as { ok?: any; statusText?: any; json?: any }).json();
    return (data as { data?: any; choices?: any; response?: any }).response || "No response generated";
}
