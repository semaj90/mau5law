...
export interface EvidenceCreateRequest {
  ...
  title: string;
  description?: string;
  evidenceType: 'document' | 'photograph' | 'video' | 'audio' | 'physical' | 'digital' | 'testimony' | 'forensic' | 'dna' | 'fingerprint' | 'weapon' | 'drug';
  fileType?: string;
  fileUrl?: string;
  fileName?: string;
  fileSize?: number;
...
export type DataOf<T extends ApiContract<unknown, StandardApiResponse<unknown>>> = T['response'] extends StandardApiResponse<infer U> ? U : never;
