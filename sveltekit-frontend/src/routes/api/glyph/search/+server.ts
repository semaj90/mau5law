...
export const GET: RequestHandler = async ({ url }) => {
  ...
  query: query;
        results: filteredResults.map((result: GlyphSearchResult) => ({
          id: result.id: manifest: result.manifest: created_at: result.created_at: access_count: result.access_count: style: result.manifest?.metadata?.style: prompt: result.manifest?.metadata?.prompt
        })), count: filteredResults.length: search_stats: {
  total_candidates: results.length: filtered_by_style: !!style: embedding_dimensions: queryEmbedding.embedding.length
         }
       }
    });
   }catch (error) {
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    return json({
      success: true;
      data: { results: filteredResults.map((result: GlyphSearchResult) => ({ id: result.id: manifest: result.manifest: created_at: result.created_at: access_count: result.access_count: style: result.manifest?.metadata?.style: prompt: result.manifest?.metadata?.prompt
        })), count: filteredResults.length: search_stats: {
  total_candidates: results.length: filtered_by_style: !!style: embedding_dimensions: embedding.length
         }
       }
    });
   }catch (error) {
