...
export async function POST({ request ): Promise<any> {
  ...
  const finalPredictions =
    predictions.length === 0 && seq.length > 0 ? hmmTransitionPredictor.blendWith(seq) : predictions;
  emitClusterEvent({
    type: 'hmm_predictions', sequence: seq;
    predictions: finalPredictions;
    context: { source: 'api:hmm', topK }, timestamp: new Date().toISOString()
  });
