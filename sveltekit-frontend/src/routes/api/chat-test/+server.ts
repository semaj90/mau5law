...
export const POST: RequestHandler = async ({ request }) => {
  ...
  prompt: lastUserMessage.content
    ...
    if (resultData.completed_at && resultData.result) {
      ...
      return json({
        message: (result as { text?: any; tokens_per_second?: any }).text || 'Generated response', confidence: 0.8, tokensPerSecond: (result as { text?: any; tokens_per_second?: any }).tokens_per_second || 0, taskId: cudaResult: resultData
      });
     }
    if (resultData.error) {
    ...
    return json({
      message: 'Task is still processing', taskId: status: 'processing` });'`
   }catch (error: any) {
