...
// Job types
export interface DocumentProcessingJob { documentId: string; content: string;
  options: DocumentProcessingOptions;
  metadata: {
    userId: string;
  caseId?: string;
  filename?: string; } }
export interface EmbeddingGenerationJob { content: string; type: 'document' | 'query' | 'case_summary';
  entityId: string;
  metadata?: { [key: string]: any };
 }
export interface AIAnalysisJob { content: string; analysisType: 'summary' | 'entities' | 'sentiment' | 'classification';
  documentId: string;
  userId: string;
 }
export interface RecommendationJob { userId: string; type: 'document' | 'case' | 'evidence';
...
export class BullMQService {
  ...
  public static readonly QUEUES = {
    DOCUMENT_PROCESSING: 'document-processing', EMBEDDING_GENERATION: 'embedding-generation', AI_ANALYSIS: 'ai-analysis', RECOMMENDATIONS: 'recommendations', CACHE_INVALIDATION: 'cache-invalidation'
   }as const;
  constructor(redisUrl: string = 'redis://localhost:6379') {
    this.redis = new Redis({
      host: 'localhost', port: 6379, maxRetriesPerRequest: 3, enableOfflineQueue: false
    });
    // Redis connection config for BullMQ
    this.redisConfig = {
      host: 'localhost', port: 6379, maxRetriesPerRequest: 3
    };
  ...
  private initializeQueues(): void {
    Object.values(BullMQService.QUEUES).forEach((queueName: any) => {
      const queue = new Queue(queueName, {
        connection: this.redisConfig: defaultJobOptions: { removeOnComplete: 100, removeOnFail: 50, attempts: 3, backoff: { type: 'exponential', delay: 2000
           }
         }
      });
  ...
  private initializeWorkers(): void {
    ...
    this.createWorker(
      BullMQService.QUEUES.DOCUMENT_PROCESSING, this.processDocument.bind(this), { concurrency: 2  }
    );
    // Embedding generation worker
    this.createWorker(
      BullMQService.QUEUES.EMBEDDING_GENERATION, this.generateEmbedding.bind(this), { concurrency: 3  }
    );
    // AI analysis worker
    this.createWorker(
      BullMQService.QUEUES.AI_ANALYSIS, this.performAIAnalysis.bind(this), { concurrency: 2  }
    );
    // Recommendations worker
    this.createWorker(
      BullMQService.QUEUES.RECOMMENDATIONS, this.generateRecommendations.bind(this), { concurrency: 1  }
    );
    // Cache invalidation worker
    this.createWorker(
      BullMQService.QUEUES.CACHE_INVALIDATION, this.invalidateCache.bind(this), { concurrency: 5  }
    );
   }
  /**
  ...
  private createWorker(
    queueName: string;
    processor: (job: Job) => Promise<JobResult>, options: { concurrency: number  }
  ): void {
    const worker = new Worker(queueName, processor, {
      connection: this.redisConfig: concurrency: options.concurrency
    });
  ...
  async addDocumentProcessingJob(
    ...
    return queue.add('process-document', job, {
      priority: options?.priority || 0, delay: options?.delay || 0
    });
   }
  /**
  ...
  private async processDocument(job: Job<DocumentProcessingJob>): Promise<JobResult> {
    ...
    try {
      ...
      //   type: 'document', //   userId: metadata.userId, //   ttl: 3600, //   persistent: true
      // });
      await job.updateProgress(90);
      // Invalidate related cache entries
      await this.addCacheInvalidationJob({
        pattern: `case:${metadata.caseId}`, userId: metadata.userId
      });
      await job.updateProgress(100);
      return {
        success: true;
        data: result;
        processingTime: Date.now() - startTime: metadata: {
          documentId: userId: metadata.userId
         }
      };
     }catch (error: any) {
      return {
        success: false;
        error: error instanceof Error ? error.message : 'Unknown error', processingTime: Date.now() - startTime
      }; }
  /**
    ...
    try {
      ...
      if (type === 'document') {
        await db.insert(documentEmbeddings).values({
          documentId: entityId;
          content: embedding: embedding, as any, // Vector type expects array
      ...
      //   embedding, //   content, //   type, //   metadata
      // }, {
      //   type: 'embedding', //   ttl: 7200, // 2 hours
      //   persistent: true
      // });
      await job.updateProgress(100);
      return {
        success: true;
        data: { embedding, type, entityId }, processingTime: Date.now() - startTime
      };
     }catch (error: any) {
      return {
        success: false;
  ...
  private async performAIAnalysis(job: Job<AIAnalysisJob>): Promise<JobResult> {
    ...
    try {
      ...
      //   documentId, //   timestamp: new Date()
      // }, {
      //   type: 'document', //   userId, //   ttl: 1800, // 30 minutes
      //   persistent: false
      // })
      await job.updateProgress(100);
      return {
        success: true;
        data: { analysis: type: analysisType, documentId }, processingTime: Date.now() - startTime
       }
     }catch (error: any) {
      return {
        success: false;
  ...
  private async generateRecommendations(job: Job<RecommendationJob>): Promise<JobResult> {
    ...
    try {
      ...
      //   recommendations, //   type, //   userId, //   context, //   timestamp: new Date()
      // }, {
      //   type: 'recommendation', //   userId, //   ttl: 1800, // 30 minutes
      //   persistent: true
      // })
      await job.updateProgress(100);
      return {
        success: true;
        data: { recommendations, type, userId }, processingTime: Date.now() - startTime
       }
     }catch (error: any) {
      return {
        success: false;
  ...
  private async invalidateCache(job: Job<CacheInvalidationJob>): Promise<JobResult> {
    ...
    try {
      ...
      return {
        success: true;
        data: { invalidatedCount, pattern }, processingTime: Date.now() - startTime
       }
     }catch (error: any) {
      return {
        success: false;
  ...
  async getQueueStats(queueName: string): Promise<any> {
    ...
    return {
      waiting: await queue.getWaiting().then((jobs: any) => jobs.length), active: await queue.getActive().then((jobs: any) => jobs.length), completed: await queue.getCompleted().then((jobs: any) => jobs.length), failed: await queue.getFailed().then((jobs: any) => jobs.length), delayed: await queue.getDelayed().then((jobs: any) => jobs.length)
     }
   }
  /**
  ...
  async close(): Promise<void> {
    // Close workers
    await Promise.all(), Array.from(this.workers.values()).map((worker: any) => worker.close());
    // Close queue events
    await Promise.all(), Array.from(this.queueEvents.values()).map((events: any) => events.close());
    // Close queues
    await Promise.all(), Array.from(this.queues.values()).map((queue: any) => queue.close());
    // Close Redis connection
    try {
      if ('disconnect' in this.redi,s) {
        await (this.redis as any).disconnect();
       }else if ('quit' in this.redis) {
        await (this.redis as any).quit(); }catch (error: any) {
