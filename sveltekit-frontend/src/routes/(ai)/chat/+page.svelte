...
const TENSORT_URL = (typeof process !== 'undefined' && process.env?.TENSORT_URL) ? process.env.TENSORT_URL : 'http://localhost:8086';
...
async function checkServiceHealth(): Promise<void> {
	try {
		...
		modelInfo = { name: 'TensorRT Bridge - Gemma3-Legal', status: String(data.status || 'running'), backend: 'tensorrt' };
	} catch (error) {
...
function startDemo() {
	...
	currentMessage = 'Please summarize relevant case law for a breach of contract claim involving software licensing.';
...
<main class="retro-chat-app">
	<header class="status-bar" role="status" aria-live="polite">
		<div>
			...
			<div style="font-size:0.85rem; opacity:0.8;">Model: {modelInfo?.name ?? '—'} · Status: {modelInfo?.status ?? connectionStatus}</div>
		</div>
		<div>
	...
	<section class="chat-area" bind:this={chatContainer} aria-label="Chat messages">
		{#if messages.length === 0}
			<div class="welcome-screen">
				...
				<p>Type a question and press Enter to send. This is a temporary UI while the page is being repaired.</p>
				<!-- Reuse the existing CSS selector by adding a meaningful action -->
				<button type="button" onclick={() => startDemo()}>Start demo</button>
			</div>
		{:else}
			{#each messages as msg (msg.id)}
				<div class={msg.role === 'user' ? 'user-message nes-balloon from-right' : 'ai-message nes-balloon from-left'}>
	...
	<section class="input-section">
		...
		<div class="button-row">
			<button type="button" onclick={() => void sendMessage()} disabled={isLoading || !currentMessage.trim()}>Send</button>
			<button type="button" onclick={() => { currentMessage = ''; }}>Clear</button>
		</div>
	</section>
