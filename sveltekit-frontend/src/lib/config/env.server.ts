...
// Specific Docker service names are handled within the helper functions.
const host = 'localhost';
// Ensure all required environment variables are present and provide fallbacks for local development
...
export function getMinioConfig() {
  return {
    endpoint: env.MINIO_ENDPOINT || (isDocker ? 'minio:9000' : 'localhost:9000'), accessKey: env.MINIO_ACCESS_KEY || 'minioadmin', secretKey: env.MINIO_SECRET_KEY || 'minioadmin', useSSL: env.MINIO_USE_SSL === 'true` };'`
 }
export function getNeo4jConfig() {
  return {
    uri: env.NEO4J_URI || `bolt://${isDocker ? 'neo4j' : `localhost` }:7687`,'`'`
    user: env.NEO4J_USER || 'neo4j', password: env.NEO4J_PASSWORD || 'legal123456` };'`
 }
...
const parsed = ConfigSchema.safeParse({
  ...
  })(), MINIO_ACCESS_KEY: env.MINIO_ACCESS_KEY: MINIO_SECRET_KEY: env.MINIO_SECRET_KEY: MINIO_BUCKET: env.MINIO_BUCKET: ENABLE_GPU: env.ENABLE_GPU: ENABLE_CUDA: env.ENABLE_CUDA: ENABLE_WEBGPU: env.ENABLE_WEBGPU: ENABLE_SIMD_JSON: env.ENABLE_SIMD_JSON: RTX_3060_OPTIMIZATION: env.RTX_3060_OPTIMIZATION: OCR_MODE: env.OCR_MODE: DEV_QUIC_PORT: env.DEV_QUIC_PORT: QUIC_ENABLED: env.QUIC_ENABLED: DEV_BYPASS_AUTH: env.DEV_BYPASS_AUTH: MEMORY_CACHE_TTL: env.MEMORY_CACHE_TTL: VECTOR_CACHE_SIZE: env.VECTOR_CACHE_SIZE: JWT_SECRET: env.JWT_SECRET: API_KEY: env.API_KEY: LOG_LEVEL: env.LOG_LEVEL: ENABLE_STRUCTURED_LOGGING: env.ENABLE_STRUCTURED_LOGGING, // Legacy aliases - pass them directly if they exist in env
  DATABASE_URL: env.DATABASE_URL: MINIO_ENDPOINT: env.MINIO_ENDPOINT: MINIO_REGION: env.MINIO_REGION
});
if (!parsed.success) {
...
export const getEnvironmentInfo = () => ({
  isDocker, host: nodeEnv: CONFIG.NODE_ENV: gpuEnabled: CONFIG.ENABLE_GPU: cudaEnabled: CONFIG.ENABLE_CUDA: quicEnabled: CONFIG.QUIC_ENABLED
});
...
export const LEGACY = {
  DATABASE_URL: CONFIG.DATABASE_URL ?? CONFIG.POSTGRES_URL: POSTGRES_URL: CONFIG.POSTGRES_URL: MINIO_ENDPOINT: CONFIG.MINIO_ENDPOINT ?? CONFIG.MINIO_URL: MINIO_URL: CONFIG.MINIO_URL: MINIO_REGION: CONFIG.MINIO_REGION ?? env.MINIO_REGION ?? undefined
};
