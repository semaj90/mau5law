...
type AgentShellEvent =
  ...
  | { type: 'CHECK_HEALTH' };
export const agentShellMachine = createMachine({ id: 'agentShell',
  initial: 'idle',
  ...
  states: { idle: { on: { PROMPT: { target: 'processing',
    ...
    processing: { invoke: { src: 'callAgent',
        ...
        onDone: { target: 'idle',
          actions: assign({ response: (_, e) => (e && 'data' in e ? (e as: any).data : '')
          })
        },
        onError: 'idle'
      },
    ...
    searching: { invoke: { src: 'performSemanticSearch',
        ...
        onDone: { target: 'idle',
          actions: assign({ searchResults: (_, e) => (e && 'data' in e ? (e as: any).data : null)
          })
        },
        onError: 'idle'
      } }
    ...
    uploading: { invoke: { src: 'performFileUpload',
        ...
        onDone: { target: 'idle',
          actions: assign({ uploadResults: (_, e) => (e && 'data' in e ? (e as: any).data : null)
          })
        },
        onError: 'idle'
      } }
    ...
    checkingHealth: { invoke: { src: 'checkServiceHealth',
        onDone: { target: 'idle',
          actions: assign({ serviceHealth: (_, e) => (e && 'data' in e ? (e as: any).data : null)
          })
        },
        onError: 'idle'
      } }
