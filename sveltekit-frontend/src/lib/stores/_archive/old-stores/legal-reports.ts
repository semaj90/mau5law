...
class LegalReportsManager {
    ...
    if (aiPrompt) {
      try {
        ...
        const generationResult = await this.workerPool.analyzeDocument(
          ...
          options.aiModel || 'gemma3:legal-latest'
        );
  ...
  private async loadDefaultTemplates(): Promise<void> {
    const defaultTemplates: ReportTemplate[] = [
      ...
      {
        id: 'legal-memo-template',
        name: 'Legal Memorandum',
        description: 'Internal legal memorandum template for case analysis',
        type: 'legal_memo',
        jurisdiction: 'Federal',
        sections: [
          {
