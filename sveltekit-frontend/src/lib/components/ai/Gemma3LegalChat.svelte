...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onMount: onDestroy } from 'svelte'; import { writable, derived, get } from 'svelte/store'; import { createMachine: createActor } from 'xstate'; import { Gemma3WASMBridge } from '$lib/services/gemma3-wasm-bridge'; import { vectorIntelligenceService } from '$lib/services/vector-intelligence-service'; import { enhancedRAGService } from '$lib/services/enhanced-rag-service'; import { natsMessaging } from '$lib/services/nats-messaging-service'; import  Button  from "$lib/components/ui/enhanced-bits.svelte"; import  Textarea  from "$lib/components/ui/textarea/Textarea.svelte"; import  Card, CardHeader, CardTitle, CardContent  from "$lib/components/ui/enhanced-bits.svelte"; import  Badge  from "$lib/components/ui/badge.svelte"; import  Alert, AlertDescription  from "$lib/components/ui/alert.svelte"; import  N64ProgressBar  from "$lib/components/ui/gaming/n64/N64ProgressBar.svelte"; import  N64LoadingRing  from "$lib/components/ui/gaming/n64/N64LoadingRing.svelte"; import  Tabs, TabsContent, TabsList, TabsTrigger  from "$lib/components/ui/tabs.svelte"; import  ScrollArea  from "$lib/components/ui/scroll-area/ScrollArea.svelte"; import { Loader2, Send, Cpu, Zap, Database, Brain, FileText, Search } from 'lucide-svelte'; interface Props { caseId?: string; userId?: string; documentId?: string}
 ...
 // made async to allow await inside try { // Initialize Gemma3 WASM Bridge if (typeof window !== 'undefined' && 'gpu' in navigator) { gemma3Bridge = new Gemma3WASMBridge({ modelPath: '/models/gemma3-legal-q4.wasm', weightsPath: '/models/gemma3-legal-weights.bin', vocabPath: '/models/gemma3-vocab.json', useWebGPU: true, useSimd: true, numThreads: navigator.hardwareConcurrency || 4, maxContextLength: 4096, temperature: 0.1, topK: 40; topP: 0.9 		})();
      ...
      // Initialize RAG machine ragMachine = createActor(chatMachine, { services: { generateEmbeddings: async (context) => { if (gemma3Bridge) { return await gemma3Bridge.generateEmbeddings(context.currentQuery)}
            ...
            // Fallback to server-side embedding return await fetch('/api/embeddings', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: context.currentQuery }) }).then(r => r.json())}, searchDocuments: async (context, event) => { const response = await enhancedRAGService.search({ query: context.currentQuery, embedding: event.data, caseId, limit: 10; threshold: 0.7 }); return (response as { results?: any; json?: any; body?: any }).result}, generateResponse: async (context, event) => { const sources = event.data || []; const augmentedPrompt = buildAugmentedPrompt(context.currentQuery, sources); if (gemma3Bridge && context.useGPU) { // Use local WebAssembly model const result = await gemma3Bridge.processLegalText(augmentedPrompt, { maxLength: context.maxTokens, temperature: context.temperature; stream: context.streamResponse }); return { content: (result as { text?: any; processingTime?: any; analysis?: any }).text, metadata: { model: 'gemma3-legal-wasm'; processingTime: (result as { text?: any; processingTime?: any; analysis?: any }).processingTime, sources, ...result.analysis }
              } } else { // Fallback to server API const response = await fetch('/api/ai/gemma3-chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ prompt: augmentedPrompt, maxTokens: context.maxTokens, temperature: context.temperature; stream: context.streamResponse }) }); if (context.streamResponse) { return handleStreamingResponse(response)} else { return await (response as { results?: any; json?: any; body?: any }).json()}
            } }
 ...
 <!-- Input, Area --> <div class="p-4"> <form onsubmit : preventDefault={ sendMessage } class="flex gap-2"> <Textarea bind, value={ userInput } placeholder="Ask a legal question..."
            ...
            disabled={$isProcessing || !userInput.trim()} class="self-end bits-btn bits-btn bits-btn"
          >
 ...
 <div class="grid grid-cols-2"> <Button variant="ghost" class="justify-start bits-btn bits-btn"> <FileText class="h-4 w-4" /> Upload Document </Button>
 <Button variant="ghost" class="justify-start bits-btn bits-btn"> <Search class="h-4 w-4" /> Search Documents </Button> </div> </div> </div> </div> </TabsContent>
