...
export interface AITask<T, extends: AITaskPayload = AITaskPayload> { id: string; type: AITaskType;
  data: T; timestamp: number;
 }
export const createAITask = <T, extends, AITaskPayload>(type: AITaskType: data: T): AITask<T> => ({
  id: `${type}_${Date.now()}`, type, data: timestamp: Date.now()
});
export const aiTaskCreators = {
  analysis: (data: AnalysisData) => createAITask('analysis', data), processing: (data: ProcessingData) => createAITask('processing', data), search: (data: SearchData) => createAITask('search', data)
};
// Export types
