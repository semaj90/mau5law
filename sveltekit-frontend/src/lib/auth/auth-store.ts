...
interface ServerUser { id: string; email: string;
  role: UserRole;
  isActive: boolean;
  name?: string;
  firstName?: string;
...
export interface AuthUser extends Partial<ServerUser> { id: string; email: string;
  role: UserRole;
  name?: string;
  firstName?: string;
  lastName?: string;
...
export interface AuthState { user: AuthUser | null; session: AuthSession | null;
  isLoading: boolean;
...
export const DockerEndpoints = {
  POSTGRES: 'postgresql://legal_admin:123456@host.docker.internal:5434/legal_ai_db', REDIS: 'redis://:redis@host.docker.internal:6379/0', QDRANT: 'http://host.docker.internal:6333', OLLAMA: 'http://host.docker.internal:11434', CONTEXT7: 'http://host.docker.internal:8777', MINIO: 'http://host.docker.internal:9000'
};
...
/*
  ...
  - canAccessResource(...): returns true when resource is public, when role has permission, when role has: '*' wildcard, or when user is the resource owner.
*/
const AccessControl = {
  getRolePermissions(role: UserRole): Permission[] {
    // Minimal default mapping — adjust to match your real Permission values as needed.
    const rolePermissionMap = {
      superadmin: ['*'], admin: ['manage_users', 'manage_content', 'read'], editor: ['edit', 'read'], viewer: ['read']
...
export class AuthStore {
  ...
  static async login(
    ...
    try {
      const response = await fetch('/api/auth/login', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password, rememberMe }), credentials: 'include'
      });
      ...
      if (response.ok && result.success) {
        ...
        return { success: false;
          error: result.error || 'Login failed', requiresMFA: result.requiresMFA
        }; }catch (error: any) {
  ...
  static async register(userData: { email: string; password: string;
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        return { success: false: error: result.error || 'Registration failed' }; }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  static async updateProfile(updates: Partial<AuthUser>): Promise<{ success: boolean; error?: string }> {
    ...
    try {
      ...
      if (response.ok && result.success) {
        ...
        return { success: false: error: result.error || 'Profile update failed' }; }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
   ...
   */
  static async changePassword( currentPassword: string;
    newPassword: string
    ...
    try {
      ...
      const result = await this.parseApiResponse(response);
      return {
        success: response.ok && !!result.success: error: result.error
      };
     }catch (error: any) {
      const msg = error instanceof Error ? error.message : String(error);
  ...
  private static async updateAuthState(user: AuthUser: session: AuthSession): Promise<void> {
    ...
    authState.update(state => ({
      ...state, user: session: normalizedSession;
      isAuthenticated: true;
      permissions: lastActivity: new Date(), isLoading: false
    }));
   }
  /**
  ...
  private static trackActivity(type: string): void {
    ...
    if (browser) {
      fetch('/api/auth/activity', {
        method: 'POST', credentials: 'include', headers: { 'Content-Type': `application/json` }, body: JSON.stringify({ type: timestamp: now.toISOString() })
      }).catch(() => {
