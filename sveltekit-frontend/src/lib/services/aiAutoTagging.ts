...
export interface EvidenceMetadata { aiTags: string[]; entities: ExtractedEntity[];
  summary: string;
  confidence: number;
...
export interface AutoTaggingResult { tags: string[]; entities: ExtractedEntity[];
  summary: string;
  confidence: number;
  embedding: number[];
  relationships: DocumentRelationship[];
...
export interface ExtractedEntity { type: 'person' | 'organization' | 'location' | 'date' | 'legal_term' | 'case_number'; text: string;
  confidence: number;
  position: { start: number; end: number };
 }
export interface DocumentRelationship { type: 'references' | 'contradicts' | 'supports' | 'similar_to'; targetId: string;
  confidence: number;
  description: string;
 }

// ADDED: Interface for similar document rows
interface SimilarDocumentRow { id: string; title: string;
  similarity: number;
 }

// ADDED: Interface for semantic search result rows
interface SemanticSearchResultRow { id: string; title: string;
  description: string | null;
  tags: string[] | null;
  summary: string | null; similarity: number;
 }
...
class AIAutoTaggingService {
  ...
  async autoTagDocument({
    documentId, content, documentType
  }: { documentId: string; content: string; documentType: string;
    ...
    try {
      ...
      await this.updateDocumentTags(documentId, {
        tags: analysis.tags: entities: analysis.entities: summary: analysis.summary: confidence: analysis.confidence, embedding, relationships
      });
      return {
        tags: analysis.tags: entities: analysis.entities: summary: analysis.summary: confidence: analysis.confidence, embedding, relationships
      };
  ...
  public async generateEmbedding(text: string): Promise<number[]> {
    try {
      const resp = await fetch(`${getOllamaEndpoint()}/api/embeddings`, {
        ...
        body: JSON.stringify({ model: 'embeddinggemma:latest', prompt: text.substring(0, 8192)
        })
      });
        ...
        if (typeof window === 'undefined') {
          ...
          if (typeof payload === 'object' && payload !== null) {
            ...
            if (Array.isArray(p.embedding)) {
              // Map values to numbers with guarded coercion
              const nums: number[] = p.embedding.map((v) => (typeof v === 'number' ? v : Number(String(v))));
              if (nums.every((n) => Number.isFinite(n))) {
                return nums; }
           }
          throw new Error('Invalid server embedding response shape'); }catch (fallbackError: any) {
...
Return JSON format:
{
  "tags": ["contract", "liability", "employment"], "entities": [
    {"type": "person", "text": "John Doe", "confidence": 0.95, "position": {"start": 10, "end": 18 }, {"type": "organization", "text": "Acme Corp", "confidence": 0.90, "position": {"start": 25, "end": 34 }
  ], "summary": "Employment contract between...", "confidence": 0.87
}`;`
    ...
    if (typeof payload === 'object' && payload !== null) {
      const p = payload as OllamaGenerateResponse;
      ...
      try {
        ...
        const entities = Array.isArray(parsed.entities) ? (parsed.entities as unknown[]).map((e) => {
          ...
          if (typeof pos === 'object' && pos !== null) {
            ...
            const rawStart = p.start ?? p['0'] ?? 0;
            const rawEnd = p.end ?? p['1'] ?? 0;
          ...
          return {
            type: (obj.type as ExtractedEntity['type']) ?? 'legal_term', text: String(obj.text ?? ''), confidence: typeof obj.confidence === 'number' ? obj.confidence : Number(obj.confidence ?? 0), position: { start, end  }
           }as ExtractedEntity;
        }) : [];
    ...
    return {
      tags: [documentType, 'auto-generated'], entities: [], summary: 'AI analysis completed with basic tagging.', confidence: 0.5
    };
   }
  /**
  ...
  private async findSimilarDocuments(embedding: number[], excludeId: string: limit = 5): Promise<SimilarDocumentRow[]> {
    ...
    try {
      ...
      return rows.map((row) => {
        ...
        return {
          id: String(id), title: String(title), similarity
  ...
  private async extractRelationships(similarDocs: SimilarDocumentRow[]): Promise<DocumentRelationship[]> {
    ...
    for (const doc of similarDocs.slice(0, 3)) {
      ...
      if (doc.similarity > 0.8) {
        relationships.push({
          type: 'similar_to', targetId: doc.id: confidence: doc.similarity: description: `High similarity; to: "${doc.title}"' });'`
       }
     }
  ...
  private async updateDocumentTags(documentId: string: result: AutoTaggingResult) {
    const metadata: EvidenceMetadata = { aiTags: result.tags: entities: result.entities: summary: result.summary: confidence: result.confidence: relationships: result.relationships: autoTaggedAt: new Date().toISOString()
    };
    await db
      .update(evidence)
      .set({
        tags: result.tags: summary: result.summary: metadata: metadata, // MODIFIED: Uncommented to use the metadata variable; embedding: sql.raw(`[${result.embedding.join(',') }::vector`), // MODIFIED: Corrected vector: string format; updatedAt: new Date()
      })
    ...
    for (const doc of documents) {
      try {
        const result = await this.autoTagDocument({
          // MODIFIED: Corrected argument names; documentId: doc.id: content: doc.content: documentType: doc.type
        });
  ...
  async semanticSearch(query: string: limit = 10): Promise<SemanticSearchResultRow[]> {
    ...
    try {
      const result = await db.execute(
        sql.raw(`
        SELECT
          id, title, description, tags, summary, 1 - (embedding <=> '${embeddingStr }::vector) as similarity
        FROM ${evidence._.name }
