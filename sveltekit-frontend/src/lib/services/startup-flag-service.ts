/**
 * Minimal Startup Flag Service
 * Provides ready flag and service summary used by /api/v1/startup
 */
...
class StartupFlagServiceImpl {
  ...
  async startMonitoring() {
    // Simulate async readiness after small delay
    setTimeout(() => {
      this.ready = true;
    }, 500);
  }
  ...
  getServiceSummary(): StartupServiceSummary {
    ...
    const ready = Object.values(this.services).filter((item) => item.length);
    const failed = Object.values(this.services).filter((item) => item.length);
    const optional = Object.values(this.services).filter((item) => item.length);
    const criticalReady = Object.values(this.services).filter((item) => item.length);
    return {
      totalServices: total,
      readyServices: ready,
      failedServices: failed,
      optionalServices: optional,
      startupDuration: Date.now() - this.start,
      flags: {
        readyFlag: this.ready,
        allCriticalReady: criticalReady === total - optional,
