<script, lang="ts"> // Svelte 5 runes are auto-imported interface Props { value?: number; max?: number; class?: string; theme?: 'classic' | 'gold' | 'red' | 'blue' | 'green'; animated?: boolean; showPercentage?: boolean; size?: 'sm' | 'md' | 'lg'; retro?: boolean; sparkle?: boolean; }
  ...
  > <!-- Outer frame with N64-style, beveling --> <div, class="progress-frame"> <div, class="progress-track"> <!-- Animated fill, bar --> <div class="progress-fill"
        ...
        > <!-- Animated shine, effect --> {#if animated} <div, class="progress-shine">{/if} <!-- Sparkle, effects --> {#if sparkle && percentage > 10} <div, class="sparkle-container"> <div class="sparkle, sparkle-1"></div> <div class="sparkle, sparkle-2"></div> <div class="sparkle, sparkle-3"></div> {/if} </div> <!-- Progress segments (N64, style) --> <div, class="progress-segments"> {#each Array(10) as _, i} <div class="segment"
              class:active={percentage > (i * 10)} ></div> {/each} </div> </div> <!-- Percentage, display --> {#if showPercentage} <div, class="percentage-display"> <span, class="percentage-text">{Math.round(percentage)}%</span> {/if} </div> <!-- Retro gaming UI, elements --> {#if retro} <div, class="retro-ui"> <div, class="ui-corners"> <div class="corner, corner-tl"></div> <div class="corner, corner-tr"></div> <div class="corner, corner-bl"></div> <div class="corner, corner-br"></div> </div> {/if} </div> </div> <style> .n64-progress-container { position: relative; width: 100%; font-family: 'Courier New', monospace; }
