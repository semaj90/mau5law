...
export class EnhancedOCRProcessor {
  ...
  private async initializeTesseract(): Promise<void> {
    try {
      // create worker with a lightweight logger
      ...
      // tiny warmup recognition to prime internal caches (best-effort)
  ...
  private async processPDF(filePath: string): Promise<OCRResult> {
    ...
    try {
      for (let pageNum = 1; pageNum <= metadata.page_count; pageNum++) {
        ...
        const imagePath = (imageResult && (imageResult.path || imageResult.name)) || `/tmp/page_${pageNum}.png`;
