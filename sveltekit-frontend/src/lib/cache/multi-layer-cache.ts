 ...
 */
export interface CacheLayer { name: string; maxSize: number; maxAge: number; // milliseconds: priority: 'high' | 'medium' | 'low';
  evictionPolicy: 'lru' | 'lfu' | 'fifo';
 }
export interface CacheEntry<T> { key: string; value: T;
  timestamp: number;
  accessCount: number;
  size: number;
  layer: string;
...
export const CONSOLE_CACHE_LAYERS: Record<string, CacheLayer[]> = {
  nes: [
    { name: 'CHR_ROM', maxSize: 1024, maxAge: 300000, priority: 'high', evictionPolicy: 'lru' }, { name: 'PRG_ROM', maxSize: 1024, maxAge: 600000, priority: 'medium', evictionPolicy: 'fifo'  }
  ], snes: [
    { name: 'VRAM', maxSize: 64 * 1024, maxAge: 300000, priority: 'high', evictionPolicy: 'lru' }, { name: 'WRAM', maxSize: 128 * 1024, maxAge: 600000, priority: 'medium', evictionPolicy: 'lfu'  }
  ], n64: [
    { name: 'RDRAM', maxSize: 4 * 1024 * 1024, maxAge: 300000, priority: 'high', evictionPolicy: 'lru' }, { name: 'TEXTURE_CACHE', maxSize: 4 * 1024, maxAge: 180000, priority: 'high', evictionPolicy: 'lru'  }
  ], legal: [
    { name: 'EMBEDDINGS', maxSize: 16 * 1024 * 1024, maxAge: 1800000, priority: 'high', evictionPolicy: 'lru' }, { name: 'DOCUMENTS', maxSize: 32 * 1024 * 1024, maxAge: 3600000, priority: 'medium', evictionPolicy: 'lfu' }, { name: 'SEARCH_RESULTS', maxSize: 8 * 1024 * 1024, maxAge: 900000, priority: 'low', evictionPolicy: 'fifo'  }
  ]
 }
export class MultiLayerCacheSystem {
  ...
  async set<T>(key: string: value: T, layerName?: string): Promise<boolean> {
    ...
    const entry: CacheEntry<T> = {
      key, value: timestamp: Date.now(), accessCount: 0, size: layer: targetLayer
    };
    layer.set(key, entry);
  ...
  getStats(): { [key: string]: any  }{
    ...
    for (const config of this.layerConfigs) {
      ...
      const currentSize = this.currentSize.get(config.name) || 0;
      stats[config.name] = {
        entries: layer?.size || 0, currentSize: maxSize: config.maxSize: utilization: (currentSize / config.maxSize) * 100, priority: config.priority: evictionPolicy: config.evictionPolicy
      };
     }
    return stats;
   }
  ...
  async cacheDocument(id: string: document: any): Promise<boolean> {
