/**
 * User-Owned Data Store (Svelte 5 Runes)
...
export const userStats = $derived({
  ...
  activeCases: userDataState.cases.filter(c => c.status === 'open' || c.status === 'pending').length,
  totalEvidence: userDataState.evidence.length,
  totalCitations: userDataState.citations.length,
  totalReports: userDataState.reports.length,
...
export const userDataActions = {
  ...
  clear() {
    userDataState.cases = [];
    userDataState.evidence = [];
    userDataState.citations = [];
    userDataState.reports = [];
    userDataState.aiConversations = [];
    userDataState.recentActivity = [];
    userDataState.isLoading = false;
...
$effect(() => {
  if (sessionState.user?.id && !sessionState.isLoading) {
    userDataActions.init(sessionState.user.id);
