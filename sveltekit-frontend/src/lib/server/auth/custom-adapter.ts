...
export class FixedDrizzlePostgreSQLAdapter implements Adapter {
	...
	async getSessionAndUser(
		...
		try {
			...
			const expires = session.expiresAt ?? new Date();
	...
	async setSession(session: DatabaseSession): Promise<void> {
		try {
			const values = {
                ...
                // Add default values for other non-null columns if any
			};
			try {
	...
	async deleteExpiredSessions(): Promise<void> {
		try {
			...
			await db.delete(sessions).where(sql`${sessions.expiresAt} <= ${new Date()}`);
		} catch (error) {
