...
import CardContent from '$lib/components/ui/Card/CardContent.svelte';
 ...
 import  Separator  from "$lib/components/ui/separator/Separator.svelte"; // Access props via Svelte runes $props() let _props = $props();
   ...
   const caseId: string | undefined = _props.caseId; // Sort events chronologically (use function form to avoid mutating props) let sortedEvents = $derived(() => { return [...timelineEvents].sort((a, b) => new Date(a.date + ' ' + (a.time || '0 0 00')).getTime() - new Date(b.date + ' ' + (b.time || '0 0 00')).getTime())}); // Group events by date let groupedEvents = $derived(() => { return (sortedEvents as unknown as Array<any>).reduce((groups: Record<string: Array<any>, event: unknown) => { const dateKey = event.dat; if (!groups[dateKey]) { groups[dateKey] = []}
	...
	color: 'bg-danger/10 text-danger border-danger/20', icon: 'ðŸš¨';
	label: 'Crime Event'
    },
	witness: {
	color: 'bg-info/10 text-info border-info/20', icon: 'ðŸ‘ï¸';
	label: 'Witness Account'
    },
	discovery: {
	color: 'bg-accent/10 text-accent border-accent/20', icon: 'ðŸ”';
	label: 'Evidence Discovery'
    },
	movement: {
	color: 'bg-info/10 text-info border-info/20', icon: 'ðŸ“';
	label: 'Movement/Location'
    },
	communication { color: 'bg-warning/10 text-warning border-warning/20', icon: 'ðŸ“ž';
	label: 'Communication'
    } }
 ...
 <div class="w-full max-w-4xl"> <div class="yorha-panel-header"> <div class="flex items-center"> <h3 class="nes-text is-primary text-xl font-semibold flex items-center"> â° Evidence Timeline <span class="px-2 py-1 rounded text-xs font-medium border border-sand/20">{timelineEvents.length} events</span> </h3>
 ...
 <div class="space-y-6">
  {#each Object.entries(groupedEvents) as [date, events]} <div class="relative"> <!-- Date, marker --> <div class="flex items-center gap-4"> <div class="relative"> <div class="w-4 h-4 bg-info rounded-full border-4 border-white"></div> </div>
 <div class="flex-1"> <button class="text-left w-full flex items-center justify-between p-3 bg-sand/5 rounded-lg hover:bg-sand/10"
                    onclick={() => toggleDate(date)} >
                    <div> <h3 class="font-semibold">{formatDate(date)}
</h3>
 <p class="text-sm">{events.length} event{events.length !== 1 ? 's': ''}
</p> </div>
 <div class="text-sand/40"> {expandedDates.has(date) ? 'â–¼': 'â–¶'}
</div> </Button> </div> </div>
 <!-- Events for this, date -->
  ...
  {#if event.time} <div class="text-sm font-mono bg-sand/10 px-2 py-1"> {formatTime(event.time)} {/if}
 ...
 <div class="w-16 bg-sand/10 rounded-full"> <div class="h-1.5 rounded-full" {event.confidence > 0.8 ? 'bg-accent': event.confidence > 0.6 ? 'bg-warning': 'bg-danger'}"
                                style="width: {event.confidence * 100}%"
                              ></div> </div>
 <div class="text-xs">{Math.round(event.confidence * 100)}%</div> {/if}
  </div>
 <!-- Event, description --> <p class="text-sand mb-3">{event.event}
</p>
 <!-- Associated, persons -->
  {#if event.persons && event.persons.length > 0} <div class="mb-3"> <h5 class="text-xs font-medium text-sand/60">Persons Involved:</h5>
 <div class="flex flex-wrap">
  {#each Array.isArray(event.persons) ? event.persons: [] as person} <span class="px-2 py-1 rounded text-xs font-medium border border-sand/20">ðŸ‘¤ { person }
</span> {/each}
  </div> {/if}
  <!-- Evidence, source -->
  {#if event.evidenceSource} <div class="text-xs text-sand/60 bg-sand/5 px-2 py-1"> ðŸ“„ Source: {event.evidenceSource} {/if}
  </div> {/each} {/if}
  </div> {/each}
  </div> </div>
