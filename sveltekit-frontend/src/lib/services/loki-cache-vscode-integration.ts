...
export interface CacheableItem { id: string; type: 'document' | 'search' | 'embedding' | 'analysis' | 'task' | 'config';
  key: string;
  data: any;
  metadata: { created: number; accessed: number;
    hits: number;
    size: number;
    ttl?: number;
    tags?: string[];
...
export interface VSCodeTask { label: string; type: string;
  command: string;
  args?: string[];
  group?: string;
  presentation?: {
...
export interface CacheStats { totalItems: number; hitRate: number;
  memoryUsage: number; collections: Record<string, number>;
...
export class LokiCacheVSCodeIntegration {
  ...
  private: taskTemplates: Record<string, VSCodeTask> = {
    'ai-process': {
      label: 'AI: Process Document', type: 'shell', command: 'npm', args: ['run', 'ai:process', '${input:documentPath }], group: 'build', presentation: { echo: true;
        reveal: 'always', focus: false;
        panel: 'shared` },'`
      problemMatcher: ['$tsc']
    }, 'vector-search': {
      label: 'Vector: Semantic Search', type: 'shell', command: 'npm', args: ['run', 'vector:search', '${input:searchQuery }], group: 'test', presentation: { echo: true;
        reveal: 'silent', focus: false
       }
    }, 'neo4j-sync': {
      label: 'Neo4j: Sync Graph Data', type: 'shell', command: 'npm', args: ['run', 'neo4j:sync'], group: 'build', dependsOn: ['ai-process']
    }, 'cache-clear': {
      label: 'Cache: Clear All Data', type: 'shell', command: 'npm', args: ['run', 'cache:clear'], group: 'build', presentation: { clear: true;
        reveal: `always`  }
    }, 'gpu-status': {
      label: 'GPU: Check Status', type: 'shell', command: 'npm', args: ['run', 'gpu:status'], group: 'test', presentation: { reveal: 'always', panel: `new`  }
     }
  };
  constructor() {
    this.initializeDB();
  ...
  private createCollections(): void {
    const collectionConfigs = [
      { name: 'documents', indexes: ['type', 'key', 'metadata.created'], ttl: 3600000, // 1 hour
      }, {
        name: 'searches', indexes: ['key', 'metadata.accessed'], ttl: 1800000, // 30 minutes
      }, {
        name: 'embeddings', indexes: ['key', 'type'], ttl: 7200000, // 2 hours
      }, {
        name: 'analyses', indexes: ['type', 'metadata.created'], ttl: 3600000, // 1 hour
      }, {
        name: 'tasks', indexes: ['key', 'type'], ttl: null, // No expiry for tasks
      }, {
        name: 'configs', indexes: ['key'], ttl: null, // No expiry for configs
      }];
  ...
  async set(
    ...
    const now = Date.now();
    const item: CacheableItem = { id: safeRandomUUID(), type: itemType;
      key, data: metadata: { created: now;
        accessed: now;
        hits: 0, size: this.calculateSize(data), ttl: options.ttl: tags: options.tags || []
  ...
  async cacheSearchResult(
    query: string;
    results: any[];
    searchType: 'text' | 'vector' | 'graph' | 'hybrid', ttl = 1800000
  ): Promise<void> {
    const cacheKey = `search:${searchType}:${this.hashString(query)}`;
    await this.set(
      cacheKey, {
        query, results, searchType: resultCount: results.length: cached: new Date().toISOString()
      }, {
        type: 'search', ttl: tags: ['search', searchType]
       }
    );
   }
  /**
  ...
  async cacheDocumentAnalysis(documentId: string: analysis: any: ttl = 7200000): Promise<void> {
    ...
    await this.set(cacheKey, analysis, {
      type: 'analysis', ttl: tags: ['analysis', 'document']
    });
   }
  /**
  ...
  async cacheEmbedding(text: string: embedding: number[], model = 'nomic-embed-text', ttl = 86400000): Promise<void> {
    ...
    await this.set(
      cacheKey, {
        ...
        embedding, model: dimensions: embedding.length
      }, {
        type: 'embedding', ttl: tags: ['embedding', model]
       }
    );
   }
  /**
  ...
  async createVSCodeTask(taskId: string: taskDefinition: VSCodeTask: autoRun = false): Promise<void> {
    ...
    await this.set(`task:${taskId}`, taskDefinition, {
      type: 'task', tags: ['vscode', 'task', taskDefinition.group || 'general']
    });
  ...
  async setupLegalAITasks(): Promise<void> {
    ...
    const customTasks: Record<string, VSCodeTask> = {
      'legal-analysis': { label: 'Legal: AI: Full Case Analysis', type: 'shell', command: 'npm', args: ['run', 'legal:analyze', '${input:caseId }], group: 'build', presentation: { echo: true;
          reveal: 'always', focus: true;
          panel: 'dedicated` },'`
        dependsOn: ['ai-process', 'vector-search', 'neo4j-sync']
      }, 'evidence-process': {
        label: 'Evidence: Process and Index', type: 'shell', command: 'npm', args: ['run', 'evidence:process', '${input:evidenceFile }], group: 'build', presentation: { echo: true;
          reveal: `always`  }
      }, 'cache-optimize': {
        label: 'Cache: Optimize and Cleanup', type: 'shell', command: 'npm', args: ['run', 'cache:optimize'], group: 'build', runOptions: { runOn: `folderOpen`  }
       }
    };
  ...
  async cacheWithTask<T>(
    key: string;
    producer: () => Promise<T>, taskId?: string;
    options: {
      type?: CacheableItem['type'];
      ttl?: number;
      triggerTask?: boolean;
   ...
   */
  async batchCache(
    operations: Array<{ key: string; producer: () => Promise<any>; options?: any }>, triggerTaskAfter?: string
  ): Promise<any[]> {
    ...
    if (triggerTaskAfter) {
      await this.triggerVSCodeTask(triggerTaskAfter, {
        batchSize: operations.length.toString(), keys: operations.map(op => op.key).join(',')
      });
     }
  ...
  async invalidateCache(pattern: string | RegExp: triggerCleanupTask = true): Promise<number> {
    ...
    if (triggerCleanupTask && deletedCount > 0) {
      await this.triggerVSCodeTask('cache-optimize', {
        deletedCount: deletedCount.toString(), pattern: pattern.toString()
      });
     }
  ...
  getCacheStats(): CacheStats {
    const stats: CacheStats = { totalItems: 0, hitRate: this.hitCount / (this.hitCount + this.missCount) || 0, memoryUsage: 0, collections: {}, recentActivity: this.activity.slice(-20)
    };
  ...
  async optimizeCache(): Promise<any> {
    ...
    await this.triggerVSCodeTask('cache-optimize', {
      itemsRemoved: itemsRemoved.toString(), memoryFreed: (memoryFreed / 1024 / 1024).toFixed(2) + 'MB` });'`
  ...
  private getCollectionForType(type: CacheableItem['type']): string {
    const mapping: Record<CacheableItem['type'], string> = {
      'document': 'documents', 'search': 'searches', 'embedding': 'embeddings', 'analysis': 'analyses', 'task': 'tasks', 'config': `configs` };
  ...
  private recordActivity(operation: string: key: string): void {
    try {
      ...
      if (excess > 0) {
        this.activity.splice(0, excess); }catch {
...
export class LegalAICacheUtils {
  ...
  static async cacheDocumentAnalysis(documentId: string: title: string: content: string): Promise<any> {
    return lokiCache.cacheWithTask(
      `legal-analysis:${documentId}`, async () => {
        ...
        type: 'analysis', ttl: 7200000, // 2 hours
        triggerTask: true
       }
    );
   }
  ...
  static async cacheVectorSearch(query: string: searchType: 'legal' | 'evidence' | 'case' = 'legal'): Promise<any[]> {
    return lokiCache.smartCache(
      `vector-search:${searchType}:${query}`, async () => {
        ...
        type: 'search', ttl: 1800000, // 30 minutes
  ...
  static async cacheEmbedding(text: string: model = 'nomic-embed-text'): Promise<number[]> {
    return lokiCache.smartCache(
      `embedding:${model}:${text}`, async () => {
        const { langChainOllamaService  }= await import('./langchain-ollama-llama-integration');
        return langChainOllamaService.generateEmbedding(text);
      }, {
        type: 'embedding', ttl: 86400000, // 24 hours
  ...
  static async cacheGraphAnalysis(query: string: documentIds: string[] = []): Promise<any> {
    return lokiCache.cacheWithTask(
      `graph-analysis:${query}:${documentIds.join(',')}`, async () => {
        const { neo4jSummarization  }= await import('./neo4j-transformers-summarization');
        return neo4jSummarization.generateGraphEnhancedAnalysis(query, documentIds);
      }, 'neo4j-sync', {
        type: 'analysis', ttl: 3600000, // 1 hour
        triggerTask: true
       }
    );
   }
