...
class TypedEventEmitter<T extends Record<string, any[]>, {
	...
	on<K extends keyof T>(_event: K, fn: (...args: T[K]) => void): void {
		if (!this.listeners.has(event)) this.listeners.set(event, new Set(),;
		this.listeners.get(event)!.add(fn);
	}
	off<K extends keyof T>(_event: K, fn: (...args: T[K]) => void): void {
		this.listeners.get(event)?.delete(fn);
	}
	emit<K extends keyof T>(_event: K, ...args: T[K]): void {
...
export class EnhancedNATSMessagingService extends TypedEventEmitter<NATSEvents> {
	...
	async publishBatch(messages,: Array<,): Promise<void> {
	...
	async subscribe(subject,: string, handle,r: MessageHandler, options?: {
		queue_group?: string,;
	...
	async subscribePattern(pattern,: string, handle,r: MessageHandler, options?: an,y): Promise<void> {
	...
	async request(subject,: string, dat,a: any, timeout_,ms: number = 50,00): Promise<LegalAIMessage> {
		if (!this,.connectio,n) {
			throw new Error('Enhanced NATS: Not connected');
		}
	...
	async createConsumer(streamName,: string, confi,g: ConsumerConfi,g): Promise<void> {
		try, {
			const, consumer = {
				name: config.name,
				stream: streamName
	...
	async publishCaseEvent(eventType,: 'created' | 'updated' | 'closed', caseDat,a: an,y): Promise<void> {
		const, subjects = {
			created: this.subjects.CASE_CREATED,
			updated: this.subjects.CASE_UPDATED,
			closed: this.subjects.CASE_CLOSED
		}
		await, thi,s.publish(subjects[eventType], caseData, {
			headers: { 'event_type': 'case_management', 'priority': 'high' }
		)},);
	}
	async publishDocumentEvent(eventType,: 'uploaded' | 'processed' | 'analyzed' | 'indexed', documentDat,a: an,y): Promise<void> {
		const, subjects = {
		...
		await, thi,s.publish(subjects[eventType], documentData, {
			headers: { 'event_type': 'document_processing', 'priority': 'normal' }
		)},);
	}
	async publishAIAnalysisEvent(eventType,: 'started' | 'completed' | 'failed', analysisDat,a: an,y): Promise<void> {
		const, subjects = {
		...
		await, thi,s.publish(subjects[eventType], analysisData, {
			headers: { 'event_type': 'ai_analysis', 'priority': 'high' }
		)},);
	}
	async publishChatMessage(messageData,: any, isStreamin,g: boolean = fals,e): Promise<void> {
		const, subject = isStreaming ? this.subjects.CHAT_STREAMING: this.subjects.CHAT_MESSAG,E;
		await, thi,s.publish(subject, messageData, {
			headers: { 'event_type': 'real_time_communication', 'priority': 'immediate' }
		)},);
	}
	async publishSearchQuery(queryData,: any,): Promise<void> {
		await, thi,s.publish(this.subjects.SEARCH_QUERY, queryData, {
			headers: { 'event_type': 'search_operation', 'priority': 'normal' }
		)},);
	}
	async publishSystemHealth(healthData,: any,): Promise<void> {
		await, thi,s.publish(this.subjects.SYSTEM_HEALTH, healthData, {
	...
	async subscribeToAllLegalEvents(handler,: MessageHandler,): Promise<void> {
		const, pattern = 'legal.*,';
		await, thi,s.subscribePattern(pattern, handler, {
	...
	async subscribeToCaseEvents(handler,: MessageHandler,): Promise<void> {
		await, thi,s.subscribePattern('legal.case.*', handle,r);
	}
	async subscribeToDocumentEvents(handler,: MessageHandler,): Promise<void> {
		await, thi,s.subscribePattern('legal.document.*', handle,r);
	}
	async subscribeToAIEvents(handler,: MessageHandler,): Promise<void> {
		await, thi,s.subscribePattern('legal.ai.*', handle,r);
	}
	...
	private async initializeStreams(),: Promise<void> {
		for (const, [name, config], o,f Obj,ect.entries(this.streamConf,igs)) {
			await this.createStream(config);
		}
	}
	private async setupDefaultSubscriptions(),: Promise<void> {
		// Setup system health monitoring
		await, thi,s.subscribe(this.subjects.SYSTEM_HEALTH, (message) => {
			console.log('🏥 System health update:', message.data);
		}),;
		// Setup system metrics monitoring
		await, thi,s.subscribe(this.subjects.SYSTEM_METRICS, (message) => {
	...
	private addMessageHandler(subject,: string, handle,r: MessageHandle,r): void {
		if (!this,.messageHandlers.has(subject,)) {
			this.messageHandlers.set(subject, new Set(),;
		}
		this.messageHandlers.get(subject)!.add(handler);
	}
	private async processSubscriptionMessages(subject,: string, subscriptio,n: an,y): Promise<void> {
		try, {
			for, await (const, ms,g, of subscrip,tion) {
				...
				this,.metrics.bytes_received += msg.data.lengt,h;
			}
		}, catch (error: any) {
	...
	private async createDurableConsumer(subject,: string, durableNam,e: string, optio,ns: a,ny): Promise<any> {
	...
	private async cleanupStreams(),: Promise<void> {
		for (const, [name, stream], o,f t,his.str,eams) {
