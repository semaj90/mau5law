...
export class ClientEmbeddingGemma {
  ...
  async generateEmbeddings(
    ...
    try {
      ...
      for (const text of textArray) {
        ...
        if (!outputTensor) {
          throw new Error(`No valid output tensor found. Available outputs: ${outputNames.join(', ')}`);
        }
  ...
  private poolEmbeddings(outputTensor: ort.Tensor, attentionMask: number[]): number[] {
    ...
    for (let seq = 0; seq < seqLen; seq++) {
      if (attentionMask[seq] === 1) { // Only consider non-padded tokens
...
class SimpleTokenizer {
  constructor(private vocab: any, private config: any) {}

  encode(text: string, maxLength: number = 512): { input_ids: BigInt64Array; attention_mask: number[] } {
    // Very basic tokenization - replace with proper implementation
    const words = text.toLowerCase().split(/\s+/).slice(0, maxLength - 2); // Leave room for BOS/EOS
