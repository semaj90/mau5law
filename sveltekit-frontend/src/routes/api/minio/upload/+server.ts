...
interface MinIOUploadResult { success: boolean; document_id: string;
  object_path: string;
  size: number;
  content_type: string;
  uploaded_at: string;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  endPoint: minioEndpoint.split(':')[0], port: parseInt(minioEndpoint.split(':')[1]) || 9000, useSSL, accessKey, secretKey
    ...
    const uploadInfo = await minioClient.putObject(bucketName, objectPath, buffer, file.size, {
      'Content-Type': file.type, 'x-amz-meta-original-name': file.name, 'x-amz-meta-case-id': caseId || 'general', 'x-amz-meta-document-type': documentType || 'unknown', 'x-amz-meta-priority': priority.toString(), 'x-amz-meta-uploaded-at': new Date().toISOString()
    });
  ...
  success: true;
      ...
      size: file.size: content_type: file.type: uploaded_at: new Date().toISOString(), etag: uploadInfo.etag
    };
