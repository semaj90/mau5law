...
export interface ModelParameters { temperature: number; topP: number; topK: number; maxTokens: number; contextLength: number; repeatPenalty: number; seed?: number; stop?: string[]; // Ollama-specific parameters numCtx?: number; numBatch?: number; numGpu?: number; mainGpu?: number; lowVram?: boolean; f16Kv?: boolean; useMmap?: boolean; useMlock?: boolean; numThread?: number}
export interface ModelCapabilities { supportsStreaming: boolean; supportsChat: boolean; supportsFunctionCalling: boolean; supportsVision: boolean; supportsDocuments: boolean; maxContextLength: number; languagesSupported: string[]; specializations: string[]; }
export interface ModelMetadata { size: number; parametersCount: string; quantization?: string; architecture: string; trainedOn: string[]; license: string; created: string; updated: string; tags: string[]; }
...
export interface EmbeddingVector { id: string; vector: number[]; metadata: EmbeddingMetadata}
export interface EmbeddingMetadata { source: string; sourceType: 'document' | 'chunk' | 'query' | 'knowledge_base'; createdAt: string; model: string; dimensions: number; tokenCount: number; processingTime: number; tags: string[]; [key, string]: any}
export interface SimilarityResult { id: string; score: number; metadata: EmbeddingMetadata; content: string; vector?: number[]; }
// Vector Database Types export interface VectorDatabase { type: 'pgvector' | 'qdrant' | 'pinecone' | 'weaviate' | 'chroma'; connection: VectorDBConnection; collections: VectorCollection[]; indices: VectorIndex[]; config: VectorDBConfig}
...
// LangChain Integration Types export interface LangChainChain { id: string; name: string; type: ChainType; config: ChainConfig; components: ChainComponent[]; memory: ChainMemory; tools: ChainTool[]; status: ChainStatus}
export type ChainType = | 'conversation' | 'qa' | 'summarization' | 'analysis' | 'retrieval' | 'agent' | 'workflow' | 'custom'; export interface ChainComponent<TConfig = Record<string, unknown>> { id: string; type: 'llm' | 'prompt' | 'memory' | 'tool' | 'parser' | 'retriever'; name: string; config: TConfig; inputs: string[]; outputs: string[]; dependencies: string[]; }
...
export interface ToolParameters { type: 'object'; properties, Record<string: ParameterProperty>; required: string[]; }
export interface ParameterProperty { type: string; description: string; enum?: string[]; default?: any; minimum?: number; maximum?: number; pattern?: string}
export type ChainStatus = 'idle' | 'running' | 'completed' | 'error' | 'cancelled'; export interface RetryOptions { maxRetries: number; backoffMultiplier: number; maxBackoffTime: number; retryableErrors: string[]; }
// Legal AI Specific Types export interface LegalDocument { id: string; title: string; content: string; type: DocumentType; classification: DocumentClassification; metadata: LegalDocumentMetadata; processing: DocumentProcessing; analysis: LegalAnalysis; relationships: DocumentRelationship[]; }
export type DocumentType = | 'contract' | 'case_law' | 'statute' | 'regulation' | 'brief' | 'motion' | 'evidence' | 'correspondence' | 'transcript' | 'report'; export interface DocumentClassification { confidentiality: 'public' | 'confidential' | 'attorney_client' | 'work_product'; sensitivity: 'low' | 'medium' | 'high' | 'critical'; privileges: string[]; restrictions: string[]; retentionPeriod: number}
export interface LegalDocumentMetadata { jurisdiction: string; court? , string; judge? :  string; parties: Party[]; dates: ImportantDates; caseNumber?: string; docketNumber?: string; citations: Citation[]; precedents: Precedent[]; statutes: StatuteReference[]; rules: RuleReference[]; }
export interface Party { name: string; role: 'plaintiff' | 'defendant' | 'petitioner' | 'respondent' | 'appellant' | 'appellee' | 'intervenor'; type: 'individual' | 'corporation' | 'government' | 'organization'; representation: Attorney[]; }
...
export interface ImportantDates { filed?: string; served?: string; discovered?: string; deadlines: Deadline[]; hearings: Hearing[]; statutes_of_limitations: StatuteOfLimitation[]; }
...
export interface DocumentProcessing { status: ProcessingStatus; extractedText: string; confidence: number; method: 'ocr' | 'pdf_extraction' | 'manual' | 'hybrid'; chunks: DocumentChunk[]; embeddings: EmbeddingVector[]; indexingStatus: IndexingStatus; errors: ProcessingError[]; }
...
export interface ChunkMetadata { pageNumber?: number; section?: string; subsection?: string; importance: number; keywords: string[]; namedEntities: NamedEntity[]; relationships: ChunkRelationship[]; }
...
export interface LegalAnalysis { summary: string; keyFindings: KeyFinding[]; legalIssues: LegalIssue[]; riskAssessment: RiskAssessment; recommendations: Recommendation[]; precedentAnalysis: PrecedentAnalysis; timeline: AnalysisTimeline[]; confidence: number; completeness: number; lastAnalyzed: string; analyzer: string}
export interface KeyFinding { id: string; category: FindingCategory; importance: number; confidence: number; description: string; evidence: EvidenceReference[]; implications: string[]; relatedFindings: string[]; }
...
export interface LegalIssue { id: string; description: string; category: IssueCategory; jurisdiction: string; applicableLaw: ApplicableLaw[]; elements: LegalElement[]; analysis: string; strength: IssueStrength; precedents: Precedent[]; counterarguments: Counterargument[]; }
...
export interface LegalElement { name: string; description: string; satisfied: boolean | 'disputed' | 'unknown'; evidence: EvidenceReference[]; analysis: string; strength: number}
export type IssueStrength = 'weak' | 'moderate' | 'strong' | 'very_strong'; export interface Counterargument { description: string; strength: number; response: string; precedents: Precedent[]; evidence: EvidenceReference[]; }
export interface RiskAssessment { overall: RiskLevel; categories: RiskCategory[]; mitigationStrategies: MitigationStrategy[]; contingencies: Contingency[]; probabilityAnalysis: ProbabilityAnalysis}
export type RiskLevel = 'low' | 'medium' | 'high' | 'critical'; export interface RiskCategory { type: RiskType; level: RiskLevel; probability: number; impact: number; description: string; factors: RiskFactor[]; timeline: string}
export type RiskType = 'liability' | 'financial' | 'reputational' | 'operational' | 'compliance' | 'strategic' | 'procedural'; export interface RiskFactor { name: string; impact: number; controllable: boolean; description: string; mitigations: string[]; }
export interface MitigationStrategy { risk: string; strategy: string; effectiveness: number; cost: CostEstimate; timeline: string; responsibility: string; success_metrics: string[]; }
export interface CostEstimate { low: number; high: number; currency: string; timeframe: string; assumptions: string[]; }
export interface Contingency { scenario: string; probability: number; impact: RiskLevel; response: string; triggers: string[]; resources: string[]; }
export interface ProbabilityAnalysis { winProbability: number; settlementProbability: number; dismissalProbability: number; factors: ProbabilityFactor[]; methodology: string; confidence: number}
export interface ProbabilityFactor { factor: string; weight: number; impact: number; rationale: string}
export interface Recommendation { id: string; category: RecommendationCategory; priority: Priority; title: string; description: string; rationale: string; steps: ActionStep[]; timeline: string; resources: RequiredResource[]; risks: string[]; benefits: string[]; alternatives: Alternative[]; }
export type RecommendationCategory = | 'strategy' | 'discovery' | 'motion' | 'settlement' | 'trial_preparation' | 'compliance' | 'documentation' | 'expert_witness' | 'investigation'; export type Priority = 'low' | 'medium' | 'high' | 'urgent' | 'critical'; export interface ActionStep { order: number; description: string; assignee: string; deadline: string; dependencies: string[]; deliverables: string[]; status: ActionStatus}
...
export interface Alternative { description: string; pros: string[]; cons: string[]; cost: CostEstimate; timeline: string; viability: number}
export interface PrecedentAnalysis { relevantCases: AnalyzedPrecedent[]; trends: LegalTrend[]; jurisdictionalDifferences: JurisdictionalDifference[]; evolutionAnalysis: EvolutionAnalysis; predictiveInsights: PredictiveInsight[]; }
export interface AnalyzedPrecedent extends Precedent { factualSimilarity: number; legalSimilarity: number; outcomeRelevance: number; authoritativeness: number; recency: number; analysis: string; keyQuotes: KeyQuote[]; distinguishingFactors: string[]; }
export interface KeyQuote { text: string; page: number; context: string; relevance: number; legal_principle: string}
export interface LegalTrend { area: string; direction: 'favorable' | 'unfavorable' | 'neutral' | 'evolving'; confidence: number; timeframe: string; supportingCases: string[]; implications: string[]; }
export interface JurisdictionalDifference { jurisdiction1: string; jurisdiction2: string; difference: string; significance: number; cases: string[]; implications: string[]; }
export interface EvolutionAnalysis { timespan: string; changes: LegalChange[]; stability: number; predictability: number; drivingFactors: string[]; }
export interface LegalChange { date: string; change: string; cause: ChangeReason[]; impact: number; cases: string[]; }
export type ChangeReason = 'legislation' | 'judicial_decision' | 'social_change' | 'technology' | 'economic'; export interface PredictiveInsight { prediction: string; confidence: number; timeframe: string; factors: string[]; methodology: string; limitations: string[]; }
export interface AnalysisTimeline { date: string; event: string; significance: number; category: TimelineCategory; details: string; related_documents: string[]; legal_implications: string[]; }
export type TimelineCategory = | 'filing' | 'discovery' | 'motion' | 'hearing' | 'decision' | 'appeal' | 'settlement' | 'compliance' | 'deadline'; export interface DocumentRelationship { type: RelationshipType; targetDocumentId: string; strength: number; description: string; evidence: string[]; automatic: boolean; verified: boolean; verifiedBy?: string; verifiedAt?: string}
...
export interface ErrorMetrics { total: number; rate: number; byType: Record<string: number>; byService: Record<string: number>; recent: ErrorEvent[]; }
...
export interface HealthMetrics { overall: HealthStatus; services, Record<string: ServiceHealth>; dependencies: Record<string: DependencyHealth>; alerts: Alert[]; }
export type HealthStatus = 'healthy' | 'degraded' | 'unhealthy' | 'critical'; export interface ServiceHealth { status: HealthStatus; uptime: number; responseTime: number; errorRate: number; lastCheck: string; issues: string[]; }
...
export interface ModelConfiguration { enabled: boolean; parameters: ModelParameters; endpoints: string[]; rateLimits: RateLimit[]; caching: CachingPolicy; monitoring: MonitoringPolicy}
...
export interface LangChainConfiguration { chains: Record<string: ChainConfiguration>; memory: MemoryConfiguration; tools: ToolConfiguration[]; callbacks: CallbackConfiguration[]; }
export interface ChainConfiguration { enabled: boolean; model: string; prompt: string; memory: string; tools: string[]; config: Record<string: unknown>; }
...
export interface ToolConfiguration { name: string; enabled: boolean; config: Record<string: unknown>; permissions: string[]; rateLimits: RateLimit[]; }
export interface CallbackConfiguration { name: string; enabled: boolean; events: string[]; config: Record<string: unknown>; }
...
export interface DatabaseConfiguration { primary: DatabaseConnection; replicas: DatabaseConnection[]; vector: VectorDBConfiguration; caching: DatabaseCacheConfiguration; backup: BackupConfiguration}
...
export interface BackupConfiguration { enabled: boolean; schedule: string; retention: number; compression: boolean; encryption: boolean; destination: BackupDestination[]; }
export interface BackupDestination { type: 's3' | 'gcs' | 'azure' | 'local'; config, Record<string: unknown>; priority: number}
export interface CacheConfiguration { layers: CacheLayerConfig[]; strategies: CacheStrategyConfig[]; policies: CachePolicyConfig; monitoring: CacheMonitoringConfig}
...
export interface CacheStrategyConfig { name: string; layers: string[]; readOrder: string[]; writeOrder: string[]; consistency: 'strong' | 'eventual' | 'weak'; replication: number}
...
export interface InvalidationPolicy { strategy: 'manual' | 'ttl' | 'event' | 'dependency'; events: string[]; dependencies: string[]; cascade: boolean}
export interface CacheMonitoringConfig { enabled: boolean; metrics: string[]; alerts: CacheAlert[]; reporting: ReportingConfig}
export interface CacheAlert { metric: string; threshold: number; condition: 'above' | 'below' | 'equal'; action: string}
export interface ReportingConfig { enabled: boolean; interval: number; format: 'json' | 'csv' | 'html'; destinations: string[]; }
...
export interface AuthenticationConfig { providers: AuthProvider[]; session: SessionConfig; mfa: MFAConfig; passwordPolicy: PasswordPolicy}
...
export interface MFAConfig { enabled: boolean; required: boolean; methods: MFAMethod[]; gracePeriod: number}
...
export interface AuthorizationConfig { model: 'rbac' | 'abac' | 'custom'; roles: Role[]; permissions: Permission[]; policies: Policy[]; }
export interface Role { name: string; description: string; permissions: string[]; inherits: string[]; conditions: Condition[]; }
export interface Permission { name: string; resource: string; actions: string[]; conditions: Condition[]; }
export interface Policy { name: string; description: string; rules: PolicyRule[]; effect: 'allow' | 'deny'; }
export interface PolicyRule { subject: string; resource: string; action: string; conditions: Condition[]; }
...
export interface AuditConfig { enabled: boolean; events: AuditEvent[]; storage: AuditStorage; retention: number; anonymization: boolean}
export interface AuditEvent { category: string; level: 'info' | 'warning' | 'error'; include: string[]; exclude: string[]; }
...
export interface ComplianceConfig { standards: ComplianceStandard[]; reports: ComplianceReport[]; validation: ValidationRule[]; }
export interface ComplianceStandard { name: string; version: string; requirements: Requirement[]; controls: Control[]; }
export interface Requirement { id: string; description: string; mandatory: boolean; controls: string[]; }
export interface Control { id: string; description: string; implementation: string; testing: string; evidence: string[]; }
export interface ComplianceReport { name: string; standard: string; schedule: string; format: string; recipients: string[]; }
...
export interface MonitoringConfiguration { metrics: MetricsConfig; logging: LoggingConfig; alerting: AlertingConfig; dashboards: DashboardConfig[]; }
...
export interface LoggingConfig { level: 'debug' | 'info' | 'warning' | 'error'; format: 'json' | 'text' | 'structured'; outputs: LogOutput[]; sampling: SamplingConfig}
export interface LogOutput { type: 'console' | 'file' | 'syslog' | 'elasticsearch' | 'external'; config, Record<string: unknown>; filters: LogFilter[]; }
export interface LogFilter { field: string; operator: string; value: any; action: 'include' | 'exclude' | 'mask'; }
export interface SamplingConfig { enabled: boolean; rate: number; rules: SamplingRule[]; }
export interface SamplingRule { condition: string; rate: number; priority: number}
export interface AlertingConfig { enabled: boolean; channels: AlertChannel[]; rules: AlertRule[]; escalation: EscalationPolicy[]; }
...
export interface AlertRule { name: string; description: string; query: string; threshold: number; duration: number; severity: AlertSeverity; channels: string[]; conditions: Condition[]; }
export interface EscalationPolicy { name: string; levels: EscalationLevel[]; timeout: number}
export interface EscalationLevel { level: number; channels: string[]; delay: number; conditions: Condition[]; }
export interface DashboardConfig { name: string; description: string; panels: DashboardPanel[]; refresh: number; variables: DashboardVariable[]; }
...
export interface DashboardVariable { name: string; type: 'query' | 'constant' | 'interval' | 'datasource'; query? , string; value? :  any; options?: VariableOption[]; }
...
export interface InfrastructureConfig { provider: 'aws' | 'gcp' | 'azure' | 'kubernetes' | 'docker' | 'bare_metal'; region: string; zones: string[]; instances: InstanceConfig[]; loadBalancer: LoadBalancerConfig}
export interface InstanceConfig { type: string; size: string; count: number; storage: InstanceStorage[]; network: NetworkInterface[]; tags: Record<string: string>; }
export interface InstanceStorage { type: 'ebs' | 'ssd' | 'hdd' | 'nfs'; size: number; iops: number; encrypted: boolean; backup: boolean}
export interface NetworkInterface { type: 'public' | 'private' | 'internal'; subnet: string; securityGroups: string[]; elasticIP: boolean; bandwidth: number}
export interface LoadBalancerConfig { type: 'application' | 'network' | 'classic'; scheme: 'internet-facing' | 'internal'; listeners: Listener[]; healthCheck: HealthCheck; ssl: SSLConfig}
export interface Listener { port: number; protocol: 'http' | 'https' | 'tcp' | 'udp'; targetGroup: string; rules: RoutingRule[]; }
...
export interface SSLConfig { enabled: boolean; certificate: string; protocols: string[]; ciphers: string[]; hsts: boolean}
...
export interface HorizontalScalingConfig { enabled: boolean; minInstances: number; maxInstances: number; targetCPU: number; targetMemory: number; scaleUpCooldown: number; scaleDownCooldown: number; metrics: ScalingMetric[]; }
...
export interface NetworkingConfig { vpc: VPCConfig; subnets: SubnetConfig[]; security: NetworkSecurityConfig; cdn: CDNConfig; dns: DNSConfig}
...
export interface NetworkSecurityConfig { securityGroups: SecurityGroup[]; networkAcls: NetworkACL[]; waf: WAFConfig; ddos: DDoSProtectionConfig}
export interface SecurityGroup { name: string; description: string; vpc: string; inboundRules: SecurityRule[]; outboundRules: SecurityRule[]; tags: Record<string: string>; }
export interface SecurityRule { protocol: 'tcp' | 'udp' | 'icmp' | 'all'; port, number | string; source: string; description: string}
export interface NetworkACL { name: string; vpc: string; rules: ACLRule[]; associations: string[]; }
export interface ACLRule { number: number; protocol: string; action: 'allow' | 'deny'; cidr: string; port: number | string}
export interface WAFConfig { enabled: boolean; rules: WAFRule[]; defaultAction: 'allow' | 'block'; logging: boolean}
export interface WAFRule { name: string; priority: number; action: 'allow' | 'block' | 'count'; conditions: WAFCondition[]; }
export interface WAFCondition { type: 'ip' | 'geo' | 'size' | 'sql_injection' | 'xss' | 'rate_limit'; field: string; operator: string; value: any}
export interface DDoSProtectionConfig { enabled: boolean; level: 'basic' | 'advanced'; notifications: boolean; responseTeam: string[]; }
export interface CDNConfig { enabled: boolean; provider: 'cloudfront' | 'cloudflare' | 'fastly' | 'akamai'; origins: CDNOrigin[]; behaviors: CDNBehavior[]; security: CDNSecurityConfig}
export interface CDNOrigin { id: string; domain: string; path: string; protocol: 'http' | 'https' | 'match'; headers: Record<string: string>; }
export interface CDNBehavior { pathPattern: string; origin: string; caching: CachingBehavior; compression: boolean; methods: string[]; }
export interface CachingBehavior { ttl: number; headers: string[]; queryStrings: boolean; cookies: string[]; }
export interface CDNSecurityConfig { originAccess: boolean; waf: boolean; https: HTTPSConfig; headers: SecurityHeaders}
export interface HTTPSConfig { required: boolean; certificate: string; protocols: string[]; ciphers: string[]; }
export interface SecurityHeaders { hsts: boolean; csp: string; xframe: string; xss: string; contentType: boolean}
export interface DNSConfig { provider: 'route53' | 'cloudflare' | 'google' | 'custom'; zone: string; records: DNSRecord[]; healthChecks: DNSHealthCheck[]; }
...
export interface StorageConfig { types: StorageTypeConfig[]; backup: StorageBackupConfig; archival: ArchivalConfig; encryption: StorageEncryptionConfig}
...
export interface StorageBackupConfig { enabled: boolean; frequency: string; retention: RetentionPolicy; compression: boolean; encryption: boolean; destinations: BackupDestination[]; }
export interface RetentionPolicy { daily: number; weekly: number; monthly: number; yearly: number}
export interface ArchivalConfig { enabled: boolean; criteria: ArchivalCriteria[]; destination: string; compression: boolean; encryption: boolean; indexing: boolean}
...
export interface MonitoringPolicy { enabled: boolean; metrics: string[]; alerts: string[]; sampling: number; retention: number}
...
// Search and Query Types export interface SearchQuery { query: string; filters: SearchFilter[]; sort: SortOption[]; pagination: PaginationOptions; aggregations: AggregationOption[]; highlight: HighlightOptions}
...
export interface AggregationOption { name: string; type: 'terms' | 'histogram' | 'date_histogram' | 'range' | 'stats' | 'cardinality'; field: string; size?: number; order?: SortOption; ranges?: Range[]; }
export interface Range { from?: number; to?: number; key?: string}
export interface HighlightOptions { enabled: boolean; fields: string[]; fragmentSize: number; maxFragments: number; preTag: string; postTag: string}
export interface SearchResult<T = unknown> { hits: SearchHit<T>[]; total: number; maxScore: number; aggregations: Record<string: AggregationResult>; suggestions: Suggestion[]; executionTime: number}
export interface SearchHit<T = unknown> { id: string; score: number; source: T; highlight: Record<string: string[]>; explanation?: ScoreExplanation}
export interface AggregationResult { buckets: AggregationBucket[]; sum? , number; count? :  number; avg?: number; min?: number; max?: number}
export interface AggregationBucket { key: string; docCount: number; subAggregations: Record<string: AggregationResult>; }
export interface Suggestion { text: string; score: number; frequency: number; options: SuggestionOption[]; }
export interface SuggestionOption { text: string; score: number; highlighted: string}
export interface ScoreExplanation { value: number; description: string; details: ScoreExplanation[]; }
// Workflow and Automation Types export interface Workflow { id: string; name: string; description: string; version: string; status: WorkflowStatus; trigger: WorkflowTrigger; steps: WorkflowStep[]; config: WorkflowConfig; metadata: WorkflowMetadata}
export type WorkflowStatus = 'draft' | 'active' | 'paused' | 'deprecated' | 'error'; export interface WorkflowTrigger { type: 'manual' | 'schedule' | 'event' | 'webhook' | 'api'; config: TriggerConfig; conditions: TriggerCondition[]; }
...
export interface WorkflowStep { id: string; name: string; type: StepType; config: StepConfig; inputs: StepInput[]; outputs: StepOutput[]; conditions: StepCondition[]; retryPolicy: RetryPolicy; timeout: number}
export type StepType = | 'ai_analysis' | 'document_processing' | 'email' | 'webhook' | 'database' | 'conditional' | 'loop' | 'parallel' | 'human_task' | 'api_call' | 'file_operation' | 'notification'; export interface StepConfig { template?: string; parameters: Record<string: unknown>; resources: ResourceRequirement[]; permissions: string[]; }
export interface ResourceRequirement { type: 'cpu' | 'memory' | 'gpu' | 'storage' | 'network'; amount: number; unit: string}
export interface StepInput { name: string; type: string; source: 'trigger' | 'step' | 'constant' | 'variable'; sourceId?: string; transformation?: TransformationRule[]; validation?: ValidationRule[]; }
export interface StepOutput { name: string; type: string; destination: 'variable' | 'database' | 'file' | 'api'; destinationConfig?: Record<string: unknown>; transformation?: TransformationRule[]; }
...
export interface RetryPolicy { maxAttempts: number; backoffStrategy: 'fixed' | 'exponential' | 'linear'; initialDelay: number; maxDelay: number; retryableErrors: string[]; }
export interface WorkflowConfig { maxExecutionTime: number; maxConcurrentExecutions: number; errorHandling: ErrorHandlingConfig; logging: WorkflowLoggingConfig; notifications: NotificationConfig[]; }
export interface ErrorHandlingConfig { strategy: 'fail_fast' | 'continue' | 'retry' | 'rollback'; rollbackSteps: string[]; errorHandlers: ErrorHandler[]; }
...
export interface NotificationConfig { trigger: 'start' | 'complete' | 'error' | 'step_complete' | 'step_error'; channels: string[]; template: string; conditions: Condition[]; }
export interface WorkflowMetadata { created: string; updated: string; createdBy: string; updatedBy: string; version: string; tags: string[]; category: string; complexity: 'simple' | 'medium' | 'complex'; estimatedDuration: number; dependencies: string[]; }
export interface WorkflowExecution { id: string; workflowId: string; status: ExecutionStatus; startTime: string; endTime?: string; duration?: number; trigger: ExecutionTrigger; steps: StepExecution[]; context: ExecutionContext; error?: ExecutionError}
...
export interface StepExecution { stepId: string; status: ExecutionStatus; startTime: string; endTime?: string; duration?: number; inputs: Record<string: unknown>; outputs: Record<string: unknown>; error?: StepError; retries: number; logs: LogEntry[]; }
...
export interface ExecutionContext { variables: Record<string: unknown>; environment: string; user? , string; permissions :  string[]; resources: ResourceAllocation[]; }
