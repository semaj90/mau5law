...
z: 0 });
  ...
  let animationFrame = $state<number | null>(null);
   ...
   let bindGroup = $state<GPUBindGroup | null>(null); // WebSocket for real-time updates let ws = $state<WebSocket | null>(null); // Vertex shader (fixed WGSL) const vertexShaderCode = ` struct Uniforms { matrix: mat4x4<f32>; time: f32;
...
storeOp: 'store'
 ...
 <style> .webgpu-viewer { position: relative;
  ...
  .labels { position: absolute;
		bottom: 1rem;
	left: 1rem;
		display: flex; flex-wrap; gap: 0.5rem; max-width: 300px; font-size: 0.75rem; z-index: 10;}
  .label { padding: 0.25rem 0.5rem;
		background: rgba(0, 0, 0, 0.5); border-radius: 4px; backdrop-filter: blur(10px)}
</style>
