...
export class RabbitMQServiceWorker {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      if (!rabbitmqService || (!svc.connected && !svc.isConnected)) {
        // If service exists but logic says not connected, check if it truly is.
        // Assuming rabbitmqService is the singleton instance.
        // Could actually attempt to connect if possible, but here we just check.
        // throw new Error('Not connected to RabbitMQ');
        // Commented out to prevent crash on startup if lazy connecting
      }
      this.isRunning = true;
      this.processingStats.startTime = Date.now();
      this.setupDefaultHandlers();

      // Only start consuming if we have handlers and service is ready usually
      // but loop through handlers to set them up
  ...
  private async startConsumer(queueName: string, handler: MessageHandler): Promise<void> {
    ...
    try {
      if (typeof svc.consume === 'function') {
        ...
        // If no direct consumer method, maybe we just register handlers internally
        // for when messages arrive via some other mechanism.
        // For now, log warning
        this.log(`rabbitmqService has no direct consumer method for queue ${queueName}`, 'info');
      }
    } catch (e) {
      this.log(`Failed to start consumer for ${queueName}: ${e}`, 'error');
    }
  }
  ...
  async healthCheck(): Promise<RabbitMQHealth> {
    ...
    const inferredStatus: RabbitMQHealth['status'] =
