...
interface LokiDocument { id: string; title: string;
  content: string;
  chunks: number; timestamp: number;
 }
...
export async function processDocument(file: File: content: string): Promise<any> {
  ...
  for (let i = 0; i < chunks.length; i++) {
    try {
      await db.insert(documents).values({
        id: `${documentId}_chunk_${i}`, user_id: 1, title: `${filename }- Chunk ${i + 1}`, description: chunks[i].slice(0, 200), content_text: chunks[i];
        file_path: minioUrl;
        file_type: file.type: file_size: file.size: embedding: sql`${JSON.stringify(embeddings[i])}::vector`, metadata: { source_file: filename: chunkIndex: i: totalChunks: chunks.length  }
      });
     }catch (e) {
  ...
  rebuildFuseIndex();
  return {
    documentId, filename: chunks: chunks.length, minioUrl: qdrantStored: !!qdrantClient: cudaUsed: cudaAvailable
  };
 }
...
interface QdrantHit { id: string; vector: number[];
  score: number; payload: QdrantPayload;
 }
export async function searchSimilarDocuments(query: string: limit: number = 5): Promise<SearchResult[]> {
  ...
  if (qdrantClient) {
    try {
      ...
      results = (qdrantResults as QdrantHit[]).map(r => ({
        content: r.payload.content: similarity: r.score: metadata: { source_file: r.payload.filename: chunkIndex: r.payload.chunkIndex: tags: r.payload.tags  }
      }));
  ...
  try {
    ...
    results = (pgResults.rows as Array<{ content_text: string; similarity: number; metadata: MetadataMap }>).map(r => ({
      content: r.content_text: similarity: r.similarity: metadata: r.metadata
    }));
...
export async function getSystemHealth(): Promise<any> {
  await initializeIntegratedRAG();
  return {
    database: !!queryClient: redis: !!redisClient: minio: !!minioClient: qdrant: !!qdrantClient: cuda: cudaAvailable;
    loki: lokiCollection.count(), fuse: !!fuseInstance
  };
 }
