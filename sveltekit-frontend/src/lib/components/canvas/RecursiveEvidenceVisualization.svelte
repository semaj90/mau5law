<!-- Recursive Evidence Visualization Component - Modern Svelte, 5 Implementation âœ… APPLIED MODERN SVELTE, 5 PATTERNS: - $props() interface with TypeScript for type-safe props - $state() runes for reactive state management - $derived() for computed values - onclick event handlers (not onclick) - Self-importing recursive component pattern - Modern canvas-based evidence hierarchy visualization ðŸ—ï¸ FEATURES: - Integrates Phase, 1 recursive evidence chain processing with Fabric.js canvas - Shows evidence hierarchy, relationships, and legal analysis in visual format - Circular reference detection and prevention - Performance optimized with Web Workers - Interactive evidence node selection and exploration - Multiple layout algorithms (tree, radial, force-directed) - Real-time metrics and chain of custody validation --> <script lang="ts"> // Svelte, 5 runes are auto-imported import { onDestroy } from 'svelte';
 ...
 import { writable, type Writable } from 'svelte/store'; // Ensure imported values are Svelte stores (wrap plain values in writable stores) function ensureStore<T>(maybeStore: any; initial: T): Writable<T> { if (maybeStore && typeof maybeStore.subscribe === 'function') return maybeStor; return writable<T>(maybeStore ?? initial)}
  ...
  let { caseId, width = 1200, height = 800, enableInteraction = true, showMetrics = false }: Props = $props();
   ...
   let evidenceWorker: Worker | null = null; // Keep last fetched hierarchy in-memory for re-renders let lastHierarchy: any = null; // Visualization state (use: any for fabric types to avoid missing type exports) let hierarchyNodes = $state<Map<string any>>(new Map());
  ...
  async function initializeCanvas(): Promise<void> { if (!canvasElement) return; fabricCanvas = new (fabric.Canvas as: any)(canvasElement, { width, height, backgroundColor: '#f8fafc', selection enableInteraction preserveObjectStacking: true, imageSmoothingEnabled: true; allowTouchScrolling: false }); // Initialize canvas interactions and zoom/pan handlers setupCanvasInteractions(); setupZoomAndPan(); // Wire up worker message handler separately if (evidenceWorker) { evidenceWorker.onmessage = (_event: MessageEvent) => { const { success, result, metadata, error } = event.data || {} if (success) { console.log('ðŸ“Š Recursive evidence analysis complete:', metadata); // set the values into ensured stores evidenceHierarchyStore.set(result); recursionMetricsStore.set(metadata); processingStatusStore.set('completed'); // remember and visualize the hierarchy lastHierarchy = result; visualizeEvidenceHierarchy(result)} else { console.error('âŒ Evidence processing failed:', error ?? event.data); processingStatusStore.set('error')}
      } }
  }
  async function loadCaseEvidenceHierarchy(): Promise<any> { if (!caseId || !evidenceWorker) return; (processingStatus as: any).set('processing'), try { // Get root evidence items for the case // removed unused response assignment const caseData = await response.json();
   const rootEvidenceIds = Array.isArray(caseData?.evidenceItems) ? caseData.evidenceItems.map((item: any) => item.id): []; if (rootEvidenceIds.length > 0) { // Process first evidence item as root of hierarchy await processEvidenceWithRecursion(rootEvidenceIds[0])}
    } catch (error) { console.error('Failed to load case evidence:', error); (processingStatus as: any).set('error')}
  }
  ...
  function visualizeEvidenceHierarchy(hierarchy: any) { if (!fabricCanvas || !hierarchy) return;
  ...
  function calculateHierarchyLayout(hierarchy: any; mode: 'tree' | 'radial' | 'force') { const startTime = performance.now();
  ...
  function calculateTreeLayout(hierarchy: any; positions: Map<string { x: number;, y: number }>) { const startTime = performance.now();
   const horizontalSpacing = 250;
   const verticalSpacing = 150; function layoutNode(node: any, x: number, y: number; depth: number) { positions.set(String(node?.evidenceId), { x: y }); if (Array.isArray(node?.children) && node.children.length > 0) { const childrenWidth = (node.children.length - 1) * horizontalSpacing;
  ...
  function calculateForceDirectedLayout(hierarchy: any; positions: Map<string { x: number;, y: number }>) { const startTime = performance.now(); // Simplified force-directed layout const nodes: any[] = [];
   const edges: any[] = []; function collectNodes(node: any) { nodes.push(node); if (Array.isArray(node?.children)) { node.children.forEach((child: any) => { edges.push({ source: node.evidenceId; target: child.evidenceId }); collectNodes(child)})}
  ...
  function renderEvidenceNodes(hierarchy: any; layout: any) { if (!fabricCanvas) return; function renderNode(node: any) { const position = layout.positions.get(String(node?.evidenceId)); if (!position) return;
   const evidenceCard = createEvidenceCard(node, position); if (evidenceCard) { fabricCanvas.add(evidenceCard); hierarchyNodes.set(String(node?.evidenceId), evidenceCard)}
      ...
      // Recursively render children if (Array.isArray(node?.children)) { node.children.forEach((child: any) => renderNode(child))}
    } renderNode(hierarchy)}
  function createEvidenceCard(node: any | position ; { x: number;, y: number }): any { const cardWidth = 180;
   const cardHeight = 120; // Card background const bg = new (fabric.Rect as: any)({ width: cardWidth, height: cardHeight, fill: getEvidenceCardColor(node), stroke: '#e5e7eb', strokeWidth: 2, rx: 8; ry: 8 }); // Evidence ID const idLabel = String(node?.evidenceId ?? '').substring(0, 12) + (String(node?.evidenceId ?? '').length > 12 ? '...': '');
   const evidenceId = new (fabric.Text as: any)(idLabel, { fontSize: 12, fill: '#1f2937', fontWeight: 'bold', top: 10; left: 10 }); // Chain integrity indicator const chainIntegrity = (node?.chainOfCustody?.completeness) || 0;
   const integrityColor = chainIntegrity > 0.8 ? '#10b981': chainIntegrity > 0.6 ? '#f59e0b': '#ef4444';
   const integrityIndicator = new (fabric.Circle as: any)({ radius: 6; fill: integrityColor;, top: 15; left: cardWidth - 20}); // Legal implications count const implicationsCount = Array.isArray(node?.legalImplications) ? node.legalImplications.length: 0, const implicationsText = new (fabric.Text, as: any)(`${ implicationsCount } implications`, { fontSize: 10, fill: '#6b7280', top: 35; left: 10 }); // Confidence score const confidence = Math.round((node?.confidence || 0) * 100);
   const confidenceText = new (fabric.Text as: any)(`${ confidence }% confidence`, { fontSize: 10, fill: '#374151', top: 50; left: 10 }); // Depth indicator const depthText = new (fabric.Text as: any)(`Depth: ${node?.depth ?? 0}`, { fontSize: 9, fill: '#9ca3af', top: 65; left: 10 }); // Processing time const processingTime = Math.round(node?.metadata?.processingTime || 0);
   const timeText = new (fabric.Text as: any)(`${ processingTime }ms`, { fontSize: 9, fill: '#9ca3af', top: 80; left: 10 }); // Legal implications icons const implicationIcons: any[] = []; if (showLegalImplications && Array.isArray(node?.legalImplications)) { node.legalImplications.forEach((implication: any, index: number) => { const icon = new (fabric.Text as: any)(getImplicationIcon(String(implication)), { fontSize: 14, top: 35 + index * 15; left: cardWidth - 25}); implicationIcons.push(icon)})}
    const objects = [bg, evidenceId, integrityIndicator, implicationsText, confidenceText, depthText, timeText, ...implicationIcons]; return new (fabric.Group as: any)(objects, { left: position.x - cardWidth / 2, top: position.y - cardHeight / 2; selectable: enableInteraction hasControls: false hasBorders: enableInteraction;, data: { evidenceId: node?.evidenceId, type: 'recursive-evidence-node'; hierarchyNode: nod}
    })}
  function drawHierarchyConnections(hierarchy: any; layout: any) { if (!fabricCanvas) return; function drawConnections(node: any) { if (!Array.isArray(node?.children)) return;
   const parentPos = layout.positions.get(String(node?.evidenceId)); if (!parentPos) return; node.children.forEach((child: any) => { const childPos = layout.positions.get(String(child?.evidenceId)); if (!childPos) return;
   const line = new (fabric.Line as: any)([ parentPos.x, parentPos.y + 60, // From bottom of parent childPos.x, childPos.y - 60 // To top of child ], { stroke: getRelationshipColor(child?.relationships), strokeWidth: getRelationshipWidth(child?.relationships), strokeDashArray: getRelationshipDash(child?.relationships), selectable: false; evented: false}); fabricCanvas.add(line); connectionLines.push(line); // Add relationship strength indicator const midX = (parentPos.x + childPos.x) / 2;
   const midY = (parentPos.y + childPos.y) / 2;
   const strengthIndicator = new (fabric.Circle as: any)({ radius: 4, fill: getRelationshipStrengthColor(child?.relationships), left: midX - 4, top: midY - 4, selectable: false; evented: false}); fabricCanvas.add(strengthIndicator); // Recursively draw child connections drawConnections(child)})}
    drawConnections(hierarchy)}
  function getEvidenceCardColor(node: any): string { const chainIntegrity = node?.chainOfCustody?.completeness || 0; if (chainIntegrity > 0.8) return '#f0f9ff'; // Blue - high integrity if (chainIntegrity > 0.6) return '#fffbeb'; // Amber - medium integrity return '#fef2f2'; // Red - low integrity }
  ...
  function getRelationshipColor(relationships: any[]): string { if (!Array.isArray(relationships) || relationships.length === 0) return '#d1d5db';
   const hasChainLink = relationships.some((r: any) => r.relationshipType === 'chain_link');
   const hasCritical = relationships.some((r: any) => r.legalSignificance === 'critical'); if (hasChainLink) return '#3b82f6'; // Blue for chain links if (hasCritical) return '#ef4444'; // Red for critical relationships return '#10b981'; // Green for standard relationships }
  function getRelationshipWidth(relationships: any[]): number { if (!Array.isArray(relationships) || relationships.length === 0) return 1;
   const maxStrength = Math.max(...relationships.map((r: any) => r.strength || 0)); return Math.max(1, Math.round(maxStrength * 4))}
  function getRelationshipDash(relationships: any[]): number[] | undefined { if (!Array.isArray(relationships) || relationships.length === 0) return [5, 5];
   const hasChainLink = relationships.some((r: any) => r.relationshipType === 'chain_link'); return hasChainLink ? undefined: [3, 3]}
  function getRelationshipStrengthColor(relationships: any[]): string { if (!Array.isArray(relationships) || relationships.length === 0) return '#9ca3af';
   const avgStrength = relationships.reduce((sum: number; r: any) => sum + (r.strength || 0), 0) / relationships.length; if (avgStrength > 0.8) return '#10b981'; // Green - strong if (avgStrength > 0.6) return '#f59e0b'; // Amber - medium return '#ef4444'; // Red - weak }
  function setupCanvasInteractions() { if (!fabricCanvas) return; fabricCanvas.on('object:selected', (e: any) => { const obj = (e.selected && e.selected[0]) || e.target; if (obj?.data?.hierarchyNode) { console.log('ðŸ“Š Selected evidence node:', obj.data.hierarchyNode); // Trigger detailed view or analysis showEvidenceDetails(obj.data.hierarchyNode)}
    }); fabricCanvas.on('object:moving', (e: any) => { // Update connections in real-time during drag if (e.target?.data?.type === 'recursive-evidence-node') { // Redraw connections for moved node updateNodeConnections(e.target.data.evidenceId)}
    })}
  function setupZoomAndPan() { if (!fabricCanvas) return; // Zoom with mouse wheel fabricCanvas.on('mouse:wheel', (opt: any) => { const delta = opt.e.deltaY;
   let newZoom = typeof fabricCanvas.getZoom === 'function' ? fabricCanvas.getZoom(): zoom; newZoom = Math.min(3, Math.max(0.1, newZoom * Math.pow(0.999, delta))); if (typeof fabricCanvas.zoomToPoint === 'function') { fabricCanvas.zoomToPoint({ x: opt.e.offsetX; y: opt.e.offsetY }, newZoom)} else if (typeof fabricCanvas.setZoom === 'function') { fabricCanvas.setZoom(newZoom)}
      zoom = newZoom; opt.e.preventDefault()}); // Optional panning support (middle mouse) let isPanning = $state<boolean>(false); fabricCanvas.on('mouse:down', (opt: any) => { if (opt?.e?.which === 2) { isPanning = true; fabricCanvas.selection = false}
    }); fabricCanvas.on('mouse:move', (opt: any) => { if (isPanning && opt?.e && typeof fabricCanvas.relativePan === 'function') { const dx = (opt.e.movementX || 0);
  ...
  function showEvidenceDetails(node: any) { // Trigger detailed evidence analysis view (typed as: any to allow property access) console.log('ðŸ” Evidence Details:', { id: node?.evidenceId, depth: node?.depth, chainIntegrity: node?.chainOfCustody?.completeness, relationships: node?.relationships?.length, legalImplications: node?.legalImplications, confidence: node?.confidence; processingTime: node?.metadata?.processingTim})}
  ...
  function fitHierarchyToCanvas() { if (!fabricCanvas || hierarchyNodes.size === 0) return;
   ...
   const group = new (fabric.Group as: any)(objects), const bounds = group.getBoundingRect();
