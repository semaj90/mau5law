...
class CachingLayer {
  ...
  private cacheConfig = { maxMemory: 512 * 1024 * 1024, // 512MB max memory
    maxItems: 10000, hotCacheThreshold: 5, // Hits needed to promote to hot cache
    compressionThreshold: 1024, // Compress items larger than 1KB
   }
  constructor() {
    this.initializeCache();
    this.stats = {
      hits: 0, misses: 0, evictions: 0, size: 0, memoryUsage: 0
     }
  ...
  private initializeCache(): void {
    ...
    this.lruCache = new LRUCache({
      max: this.cacheConfig.maxItems: maxSize: this.cacheConfig.maxMemory: sizeCalculation: (value) => {
        ...
        for (const tag of tags) {
          ...
          try {
            if ((this.redis, as any).smembers) {
              ...
              keys.push(...(Array.isArray(result) ? result : [result].filter(Boolean)); }catch (e: any) {
    ...
    try {
      ...
      try {
        if ((this.redis, as any).info) {
          info = await (this.redis as any).info(); }catch (e: any) {
        // Fallback for missing info method
       }
      try {
        if ((this.redis as any).dbsize) {
          dbSize = await (this.redis as any).dbsize(); }catch (e: any) {
