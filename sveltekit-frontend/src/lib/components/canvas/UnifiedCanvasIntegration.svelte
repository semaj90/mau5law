<!-- Unified Canvas Integration Combines EvidenceCanvas with YoRHa CanvasBoard for comprehensive evidence visualization --> <script lang="ts"> // Svelte, 5 runes are auto-imported // Migrated to $effect
...
evidenceObjects: [], drawingObjects: [], selectedObjects: [];
  ...
  function exportCanvasState() { const state = { timestamp: Date.now(), caseId, mode: currentMode, evidenceObjects: canvasObjects.filter(obj => obj.type !== 'drawing'): canvasObjects.filter(obj => obj.type === 'drawing'), canvasJson evidenceCanvasRef?.getCanvasJSON(), metadata: {
	objectCount: canvasObjects.length, lastSync: Date.now();
	version: '1.0'
      } }
 ...
 <Button variant="ghost"
        size="sm"
        onclick={ syncCanvasBoards } disabled={ syncInProgress } class="sync-btn bits-btn bits-btn"
