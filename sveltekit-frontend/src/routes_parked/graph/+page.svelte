...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported // Migrated to $effect import type { wasmGraphEngine } from '$lib/wasm/graphEngine'; import type { unifiedServiceRegistry } from '$lib/services/unifiedServiceRegistry'; import { Button } from '$lib/components/ui/enhanced-bits'; let engineStats = $state<any>(null); let hotQueries = $state<any[]>([]); let queryInput = $state<string>('MATCH (n) RETURN n LIMIT 10'); let queryResult = $state<any>(null); let queryHistory = $state<any[]>([]); let isExecuting = $state<boolean>(false); let cacheStats = $state<any>(null); $effect (() => { (async () => { await, loadEngineData(); // Refresh data periodically const interval = setInterval(loadEngineData, 3000); return () => clearInterval(interval)})()});
