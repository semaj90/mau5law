...
function generateFallbackPrompts(timing: string, context: any, legalContext: any) {
  ...
  if (timing === 'before-upload') {
    if (legalContext?.urgency === 'critical') {
      prompts.push({
        ...
        content:
          '🔥 Critical case detected. Ensure all documents are properly authenticated and chain of custody is documented before proceeding.',
        category: 'warning',
        confidence: 0.95,
        relevance: 0.9,
        actionable: true,
    ...
    if (context.files?.length > 10) {
      prompts.push({
        id: `bulk-${timestamp}`,
        content:
          '📋 Large document set detected. Consider organizing files by privilege status and relevance before upload to streamline review.',
    ...
    if (legalContext?.practiceArea === 'litigation') {
      prompts.push({
        ...
        content:
          '⚖️ Litigation documents detected. Remember to check for attorney-client privilege and work product protection.',
        category: 'guidance',
        confidence: 0.85,
        relevance: 0.8,
        actionable: true,
  ...
  if (timing === 'during-upload') {
    prompts.push({
      ...
      content:
        '🔍 AI analysis in progress. The system is examining documents for key entities, legal citations, and privilege status.',
      category: 'insight',
      confidence: 0.9,
      relevance: 0.7,
      actionable: false,
    ...
    if (context.files?.length > 5) {
      prompts.push({
        ...
        content:
          '⚡ Processing multiple documents. Results will include relevance scoring and evidence categorization.',
        category: 'insight',
        confidence: 0.8,
        relevance: 0.6,
        actionable: false,
  ...
  if (timing === 'after-upload') {
    prompts.push({
      ...
      content:
        '🎯 Upload complete! Review AI analysis results and consider organizing documents by relevance and privilege status.',
    ...
    prompts.push({
      ...
      content:
        '✅ Run a quality check on AI analysis results. Verify entity extractions and legal citations for accuracy.',
      category: 'guidance',
      confidence: 0.8,
      relevance: 0.7,
      actionable: true,
