...
<script lang="ts">
  ...
  async function analyzeEvidence(evidenceId: string) {
    ...
    isAnalyzing = true;
    try {
      await onAnalyze(evidenceId);
      // Update relevance score after analysis
      const evidenceIndex = evidenceData.findIndex(e => e.id === evidenceId);
      if (evidenceIndex >= 0) {
        evidenceData[evidenceIndex].relevanceScore = Math.min(1, evidenceData[evidenceIndex].relevanceScore + 0.05);
      }
    } catch (error) {
  ...
  function exportSelected(format: 'pdf' | 'json' | 'csv') {
    if (onExport) {
      onExport(Array.from(selectedEvidence), format);
    } else {
      console.log(`Exporting ${selectedEvidence.size} items as ${format.toUpperCase()}`);
    }
  }
  function getTypeIcon(type: EvidenceItem['type']): string {
    const icons = {
      email: '📧',
      transcript: '📝',
      financial: '💰',
...
<div class="evidence-manager">
  ...
  >
    <CardHeader>
      <CardTitle class="evidence-title">
        <div class="title-section">
          <span class="evidence-icon">🗂️</span>
          <div class="title-text">
            <h2>Evidence Manager</h2>
            <div class="evidence-meta">
              <span class="total-count">{evidenceStats.total} Total Items</span>
              <span class="verified-count">✅ {evidenceStats.verified} Verified</span>
              <span class="privileged-count">🔒 {evidenceStats.privileged} Privileged</span>
            </div>
          </div>
        </div>
    ...
    <CardContent>
        ...
        <div class="upload-section" transition:fly={{ y: -20, duration: 300 }}>
          <div class="upload-area">
            <input
              type="file"
              multiple
              accept=".pdf,.doc,.docx,.txt,.email,.csv,.xlsx,.mp3,.mp4,.wav"
            ...
            <div class="upload-instructions">
              ...
              <div class="upload-text">
                ...
                <p>Supports: PDF, DOC, TXT, EMAIL, CSV, XLSX, MP3, MP4, WAV</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="controls-section">
        <div class="search-controls">
          ...
          <select bind:value={sortBy} class="sort-control">
      ...
      <div class="stats-section">
        ...
        <div class="stats-grid">
          {#each Object.entries(evidenceStats.byType) as [type, count]}
            <div class="stat-card">
              <span class="stat-icon">{getTypeIcon(type as EvidenceItem['type'])}</span>
              <div class="stat-info">
          ...
          >
            <div class="evidence-header">
              ...
              <div class="evidence-type">
                <span class="type-icon">{getTypeIcon(evidence.type)}</span>
              ...
              <div class="evidence-status">
                ...
                {#if evidence.privileged}
                  <span class="privilege-badge">🔒 PRIVILEGED</span>
                {/if}
            ...
            <div class="evidence-content">
                ...
                <div class="evidence-details">
                  <div class="detail-item">
                    ...
                    <span class="detail-value">
                      {new Date(evidence.dateCreated).toLocaleDateString()}
                    </span>
                  </div>
                  <div class="detail-item">
                  ...
                  <div class="detail-item">
                    ...
                    <span class="detail-value">{evidence.size}</span>
                  </div>
                  <div class="detail-item">
              ...
              <div class="evidence-metrics">
                ...
                <div class="evidence-actions">
                  ...
                  <Button size="sm" variant="outline">👁️ Preview</Button>
            ...
            {#if evidence.metadata}
              <div class="evidence-metadata">
                <details>
                  <summary>📋 Metadata ({Object.keys(evidence.metadata).length} fields)</summary>
...
<style>
  ...
  .upload-section {
    margin-bottom: 2rem;
    padding: 2rem;
    border: 2px dashed var(--enhanced-bits-border);
    border-radius: 8px;
