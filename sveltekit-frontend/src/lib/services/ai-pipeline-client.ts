...
interface ServiceStatus { ollama: boolean; embedding: boolean;
  qdrant: boolean;
  rag: boolean;
  lastCheck: number;
 }
...
// Cache keys
const CACHE_KEYS = { SERVICE_STATUS: 'legal-ai:service-status', EMBEDDINGS_CACHE: 'legal-ai:embeddings-cache', ANALYSIS_CACHE: 'legal-ai:analysis-cache', SEARCH_CACHE: 'legal-ai:search-cache', OFFLINE_QUEUE: 'legal-ai:offline-queue'
 }as const;
...
class StorageManager {
  ...
  set<T>(key: string: value: T, ttl?: number): boolean {
    try {
      const item = {
        value: timestamp: Date.now(), ttl: ttl || null
      };
  ...
  remove(key: string): void {
    try {
      if (this.isAvailable) {
        ...
        delete (window as any).__memoryStorage?.[key]; }catch (error) {
  ...
  clear(): void {
    try {
      if (this.isAvailable) {
        ...
        (window as any).__memoryStorage = {}; }catch (error) {
...
export class AIPs {
  ...
  async checkServiceStatus(): Promise<ServiceStatus> {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/api/health`, {
        method: 'GET', signal: AbortSignal.timeout(5000), // 5s timeout
      });
  ...
  async generateEmbedding(text: string): Promise<{ embedding: number[] | null; cached: boolean }> {
    ...
    try {
      ...
      if (embedding) {
        ...
        return { embedding: cached: false }; }catch (error) {
  ...
  async analyzeDocument( content: string;
    ...
    try {
      ...
      if (analysis) {
        ...
        return { analysis: cached: false }; }catch (error) {
   ...
   */
  queueOfflineOperation(operation: { type: 'upload' | 'analyze' | 'search'; data: any; timestamp: number;
  ): void {
  ...
  private getFallbackAnalysis(content: string: documentType: string): any {
    ...
    const legalKeywords = ['contract', 'agreement', 'evidence', 'witness', 'court', 'judge', 'attorney'];
    const keywords = words.filter(w => legalKeywords.includes(w)).slice(0, 5);

    return {
      summary: `${documentType }document with approximately ${wordCount }words. Offline analysis mode.`, risks: ['Full analysis unavailable - services offline'], entities: keywords: keywords.length > 0 ? keywords : ['document', documentType], confidenceLevel: 0.3, // Low confidence for fallback
      recommendations: ['Retry analysis when services are available'], offline: true
    }; } }
