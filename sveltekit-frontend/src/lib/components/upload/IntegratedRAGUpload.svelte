...
<div class="integrated-rag-upload">
    ...
    >
      ...
      {:else}
        <div class="drop-zone-content">
          ...
          <small>Supported: .txt, .md, .json, .csv (max 10MB)</small>
        {/if}
    </div>
    <input
      type="file"
  ...
  {#if result}
    <div class="result-section success">
      ...
      <div class="result-details">
        <p><strong>File:</strong> {result.document.filename}</p>
        <p><strong>Chunks:</strong> {result.document.chunks}</p>
        <p><strong>Document ID:</strong> {result.document.id}</p>
        <p>
          <strong>Storage:</strong>
          {result.document.qdrantStored ? '✓ Qdrant' : ''} ✓ pgvector ✓ MinIO
        </p>
      </div>
      {#if result.recommendations?.length > 0}
        <div class="recommendations">
          <h5>📊 Similar Documents Found:</h5>
          {#each Array.isArray(result.recommendations) ? result.recommendations : [] as rec}
            <div class="recommendation-item">
              <span class="similarity">{Math.round(rec.similarity * 100)}%</span>
              <span class="content">{rec.content}</span>
            </div>
          {/each}
        {/if}
    {/if}
  ...
  <div class="search-section">
    ...
    {#if searchResults.length > 0}
      <div class="search-results">
        <h5>Results ({searchResults.length}):</h5>
