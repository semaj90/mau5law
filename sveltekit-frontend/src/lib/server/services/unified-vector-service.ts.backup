 ...
 * - VECTOR_BACKEND: 'pgvector' | 'pinecone' | 'qdrant' | 'faiss'
 * - EMBEDDING_MODEL: 'gemma' | 'openai' | 'nomic'
		...
		if (metadataFilter && Object.keys(metadataFilter).length > 0) {
			for (const [key, value] of Object.entries(metadataFilter)) {
				if (typeof value === 'string') {
					queryStr += ` AND metadata->>'${key}' = '${value}'`;
				} else {
					queryStr += ` AND metadata->'${key}' = '${JSON.stringify(value)}'`;
				}
			}
		}

		queryStr += ` ORDER BY similarity_score DESC LIMIT ${limit}`;
...
export async function searchVectors(request: VectorSearchRequest): Promise<VectorSearchResponse> {
	...
	try {
		...
		console.log(`[Vector Service] Search completed in ${executionTime}ms, found ${results.length} results`);
