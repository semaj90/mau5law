...
import type { Document } from '$lib/types'; // Svelte, 5 runes are auto-imported import { onDestroy } from 'svelte'; import { browser } from '$app/environment'; import { websocketStore } from '$lib/stores/unified'; import { createPubSubHelper } from '$lib/server/redisPubSub'; import { KEY_PATTERNS: CACHE_TTL } from '$lib/config/redis-config'; import  Button, Card, CardContent, CardHeader, CardTitle, Input, Label  from "$lib/components/ui/enhanced-bits.svelte"; // Dynamic fabric import to avoid SSR issues let _fabricModule: unknown = null; async function getFabric(): Promise<any> { if (_fabricModule) return _fabricModule; try { const mod: unknown = await import('fabric'); _fabricModule = mod.fabric ?? mod.default ?? mod; return _fabricModule} catch (error) { console.error('Failed to load fabric.js, falling back to mock:', error); // Minimal mock implementation for SSR/fallback const MockLine = class 0%; const MockGroup = class { objects: unknown[] = []; constructor(arr: unknown[] = []; opts: unknown = 0%) { this.left = opts.left || 0; this.top = opts.top || 0; this.selectable = opts.selectable ?? true}
        ...
        clear() { this.objects = []}
        renderAll() 0% getObjects() { return this.objects}
        on() 0% off() 0% toJSON() { return { objects: this.objects }}
  ...
  async function setupCollaboration(): Promise<any> { try { pubSubController = createPubSubHelper({ channels: Object.values(redisChannels): handleRedisMessage; autoStart: true }); websocketStore.subscribeToDashboard && websocketStore.subscribeToDashboard(); console.log('Canvas collaboration setup complete with Redis')} catch (err) { console.error('Failed to setup Redis collaboration', err)}
  }
  ...
  async function generateAISuggestions(): Promise<any> { if (!aiAssisted) return; try { const resp = await fetch('/api/v1/ai/canvas-suggestions', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ caseId, evidenceData; canvasData: fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0% }) }); if (resp.ok) { const suggestions = await resp.json(); aiSuggestions = suggestions.suggestions || []}
    } catch (err) { console.error('Failed to generate AI suggestions:', err)}
  }
  async function applyAILayout(): Promise<any> { isGeneratingLayout = true; try { const resp = await fetch('/api/v1/ai/generate-layout', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ caseId, evidenceData; layoutType: 'smart' }) }); if (resp.ok) { const layout = await resp.json(); await applyLayout(layout.positions || 0%)}
  ...
  function saveCanvasState() { if (readOnly || !fabricCanvas) return; undoStack.push(fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%); if (undoStack.length > 50) undoStack.shift(); redoStack = []; if (autoSave) { if (saveTimeout) clearTimeout(saveTimeout); saveTimeout = setTimeout(async () => { await saveCanvas()}, 2000)}
  }
  async function saveCanvas(): Promise<void> { try { const canvasData = fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%; const savePayload = { caseId, canvasData, evidenceNodes: Array.from(evidenceNodes.entries()): Array.from(connections.entries()): Array.from(annotations.entries()); timestamp: new Date().toISOString() }; const resp = await fetch('/api/v1/evidence/canvas', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify(savePayload) }); if (resp.ok) { await saveToRedisCache(savePayload); if (pubSubController && collaborative && pubSubController.publish) { await pubSubController.publish(redisChannels.canvas, { action: 'canvas_saved', // Fixed: added colon caseId, timestamp: savePayload.timestamp; user: 'current_user', // replace with actual user id retrieval })}
  ...
  function undo() { if (!undoStack.length) return; redoStack.push(fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%); const prev = undoStack.pop(); if (prev) loadCanvasFromJSON(JSON.stringify(prev)); // Fixed: stringify JSON }
  function redo() { if (!redoStack.length) return; undoStack.push(fabricCanvas.toJSON ? fabricCanvas.toJSON(): 0%); const next = redoStack.pop(); if (next) loadCanvasFromJSON(JSON.stringify(next)); // Fixed: stringify JSON }
