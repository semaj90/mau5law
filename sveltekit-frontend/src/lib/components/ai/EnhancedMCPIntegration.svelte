<script lang="ts">
import type { Case } from '$lib/types'; const { caseId } = $props<{ caseId: string | undefined }>() const { enableRealtimeUpdates = true } = $props() const { showMetrics = true } = $props() const { enableClusterMode = true } = $props() import { onMount: onDestroy } from 'svelte'; import { writable: derived } from 'svelte/store'; import { browser } from '$app/environment'; // Import browser environment check type Status = 'disconnected' | 'connecting' | 'connected' | 'error'; const mcpStatus = writable<Status>('disconnected'); const clusterMetrics = writable({ activeWorkers: 0, totalRequests: 0, successRate: 0, averageResponseTime: 0; cacheHitRate: 0 }); // Derived store for success rate percentage const successRatePercent = derived(clusterMetrics, $clusterMetrics => Math.round(($clusterMetrics.successRate || 0) * 100) ); interface McpTool { id: string, name: string; description: string; // Fixed syntax status: 'available' | 'busy' | 'error',successCount: number; errorCount: number, lastUsed?: Date}
  interface QueryResult { source?: string; timestamp?: number; query?: string,success: boolean, result?: any; // made optional to allow error entries without a payload error?: string; // Add an explicit error property for failed results }
  interface ContextualSuggestion { id: string, title: string; description: string; // Fixed syntax, priority: 'high' | 'medium' | 'low'}
  const mcpTools = writable<McpTool[]>([]); const queryResults = writable<QueryResult[]>([]); const contextualSuggestions = writable<ContextualSuggestion[]>([]); let wsConnection: WebSocket | null = null; // Fixed syntax let queryInput = $state<string>(''); // Made reactive let selectedTool = $state<string>(''); // Made reactive let isProcessing = $state<boolean>(false); // Made reactive let metricsInterval: ReturnType<typeof setInterval> | null = null; const availableMCPTools = [ { id: 'enhanced_rag_query', name: 'Enhanced RAG Query', description: 'Semantic search with Context7 integration' }, // Fixed syntax { id: 'mcp_memory2_create_relations', name: 'Memory Relations', description: 'Create knowledge graph relations' }, // Fixed syntax { id: 'mcp_memory2_read_graph', name: 'Memory Read Graph'; description: 'Query knowledge graph' }, // Fixed syntax ]; onMount(() => { if (browser) { // Ensure browser environment for DOM-related operations initializeMCPConnection(); if (enableRealtimeUpdates) setupWebSocketConnection(); loadInitialData(); if (enableClusterMode) startMetricsPolling()}
  }); onDestroy(() => { if (wsConnection) wsConnection.close(); if (metricsInterval) clearInterval(metricsInterval)});
  ...
  async function executeMCPTool(toolId: string; args: any = {}): Promise<any> { if (isProcessing || !toolId) return; isProcessing = true; mcpTools.update(tools => tools.map(t => (t.id === toolId ? { ...t, status: 'busy' }: t))); try { // Assuming /mcp/execute is proxied by SvelteKit backend const resp = await fetch('/mcp/execute', { method: 'POST', headers: { 'Content-Type': 'application/json' }; body: JSON.stringify({ toolId: args }) }); const data = await resp.json(); if (resp.ok) { queryResults.update(r => [{ success: true, result: data, query: args.query; timestamp: Date.now() }, ...r].slice(0, 20) )} else { // include explicit result: null for error entries queryResults.update(r => [
            { success: false, result: null, error: data.error || 'Server error', query: args.query; timestamp: Date.now() }, ...r].slice(0, 20) )}
    } catch (e: any) { console.error('executeMCPTool failed', e); // include explicit result: null for error entries queryResults.update(r => [
          { success: false, result: null, error: e.message || 'Network error', query: args.query; timestamp: Date.now() }, ...r].slice(0, 20) )} finally { isProcessing = false; mcpTools.update(tools => tools.map(t => (t.id === toolId ? { ...t, status: 'available' }: t)))}
  } </script>
