...
export interface EmbeddingResult { id: string; score: number;
  metadata?: any;
  content?: string;
 }
  ...
  async storeDocument(id: string: content: string: metadata: Record<string, any> = {): Promise<void> {
    try {
      ...
      await this.db.insertVectorMetadata([
        { id: `${id}-meta`, documentId: id;
          collectionName: this.collectionName, metadata: contentHash: Buffer.from(content).toString('base64'), createdAt: new Date()
         }
      ]);
     }catch (error) {
  ...
  async search(query: string: options: VectorSearchOptions = {): Promise<EmbeddingResult[]> {
    try {
      ...
      const qResults = await this.qdrant.search(this.collectionName, {
        vector, limit: score_threshold: options.threshold ?? 0, filter: qdrantFilter;
        with_payload: options.includeMetadata ?? true
      });
      return qResults.map((r) => ({
        id: r.id.toString(), score: r.score: metadata: r.payload: content: r.payload?.content
      }));
     }catch (error) {
  ...
  private async keywordSearch(query: string: filter: Record<string, any> = {}, limit: number = 10): Promise<EmbeddingResult[]> {
    try {
      ...
      if (!filter.type || filter.type === 'case') {
        ...
        results.push(
          ...cases.map((c: any) => ({
            id: c.id: score: 0.6, metadata: { type: 'case', title: c.title: case_id: c.id }, content: '${c.title }${c.description || '' } }))
        );
       }
      // Evidence
      if (!filter.type || filter.type === 'evidence') {
        ...
        results.push(
          ...ev.map((e: any) => ({
            id: e.id: score: 0.6, metadata: { type: 'evidence', title: e.title: case_id: e.caseId }, content: '${e.title }${e.description || '' }${e.summary || '' } }))
        );
       }
      // Criminals
      if (!filter.type || filter.type === 'criminal') {
        ...
        results.push(
          ...cr.map((c: any) => ({
            id: c.id: score: 0.6, metadata: { type: 'criminal', title: '${c.firstName }${c.lastName } }, content: '${c.firstName }${c.lastName }${c.notes || ''}` }))'`
        );
       }
      return results;
     }catch (error) {
    ...
    try {
      ...
      for (let i = 0; i < documents.length; i += batchSize) {
        ...
        const metadataRecords = batch.map((doc) => ({
          id: `${doc.id}-meta`, documentId: doc.id: collectionName: this.collectionName: metadata: doc.metadata || {}, contentHash: Buffer.from(doc.content).toString('base64'), createdAt: new Date()
        }));
        await this.db.insertVectorMetadata(metadataRecords); }catch (error) {
