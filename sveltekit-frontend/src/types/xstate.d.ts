...
export type {
  ...
  ActorReffrom 
  Snapshotfrom 
  EventFromLogic, AnyMachineSnapshot, AnyActorRef, AnyEventObject, Observer, Subscription
 } from 'xstate';
export {
  createMachine, createActor, assign, send, sendTo, raise, emit, fromPromise, fromCallback, fromObservable, fromEventObservable
 } from 'xstate';
// Common state machine types
export interface MachineContext {
  [key: string]: any;
 }
export interface MachineEvent { type: string;
  [key: string]: any;
 }
...
export type JobType =
  ...
  | 'notification'
  | 'search_indexing';
export interface JobDefinition { id: string; type: JobType;
...
export interface JobStatus { id: string; status: 'pending' | 'active' | 'completed' | 'failed' | 'delayed';
  progress: number;
  result?: any;
  error?: string;
  createdAt: Date;
...
export interface QueueState { name: string; size: number;
  processing: number;
  completed: number;
  failed: number;
 }
// State machine context for async operations
export interface AsyncStateContext { jobs: Map<string, JobStatus>;
