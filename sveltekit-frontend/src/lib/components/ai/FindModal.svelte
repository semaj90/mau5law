...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
   ...
   let systemHealth = $state<any>(null); // Load search history from localStorage and initialize Phase, 13 $effect(() => { (async () => { const saved = localStorage.getItem('ai-search-history'); if (saved) { searchHistory = JSON.parse(saved)}
      ...
      const response = await fetch('/api/ai/find', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	query: searchQuery, type: selectedType, useAI: true, mcpAnalysis: useMCPAnalysis, semanticSearch: useSemanticSearch, maxResults: 20, confidenceThreshold: aiConfidenceThreshold }) });
   const data = await response.json(); if (data?.success) { searchResults = data.results ?? data.result ?? []; mcpContext = data.mcpContext ?? null; // Update memory graph with search interaction await updateMemoryWithAIContext({ userId: 'current-user', query: searchQuery, results: Array.isArray(data.results) ? data.results.length: (data.results ?? 0): data.metadata?.model, confidence: data.metadata?.confidence;
	processingTime: data.metadata?.processingTime })} else { console.error('AI search returned error:', data?.error ?? data); searchResults = []}
'
   ...
   // Get search suggestions as user types async function getSuggestions(): Promise<any> { if (searchQuery.length < 3) { suggestions = []; return}
    try { const res = await fetch('/api/ai/suggest', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	query: searchQuery }) });
   ...
   // Update memory graph with AI context async function updateMemoryWithAIContext(interaction: unknown): Promise<any> { try { await fetch('/api/mcp/memory/create-relations', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	userId: interaction.userId, query: interaction.query, resultsCount: interaction.results, model: interaction.aiModel, confidence: interaction.confidence, processingTime: interaction.processingTime }) })} catch (error) { console.error('Failed to update memory graph:', error)}
  }
  ...
  }); // Public API export function open() { isOpen = true; // Auto-focus search input when modal opens setTimeout(() => { const input = document.querySelector('[data-testid="search-input"]') as HTMLInputElement; input?.focus()},
	100)}
  ...
  // Update Phase, 13 integration status async function updatePhase13Status(): Promise<any> { try { const res = await fetch('/api/phase13/status'); if (res.ok) { const data = await res.json(); systemHealth = data?.data ?? data; phase13Status = systemHealth?.phase13 ?? null}
   ...
   // Apply MCP auto-suggestion with Phase, 13 integration async function applyAutoSuggestion(suggestion: unknown): Promise<any> { try { const response = await fetch('/api/phase13/integration', { method: 'POST', headers: { 'Content-Type': 'application/json' },
	body: JSON.stringify({
	action: 'apply-suggestion', suggestion }) }); if (response.ok) { const result = await response.json(); console.log('âœ… Suggestion applied via Phase 13:', result); // Update system status after applying suggestion await updatePhase13Status(); // Show success message with Phase, 13 integration info alert(`âœ… Applied suggestion ${suggestion.suggestion || suggestion}\nðŸ”§ Implementation ${suggestion.implementation || ''}\nðŸ“Š Phase, 13 Status: ${phase13Status?.status ?? 'Updated'}`)} else { throw new Error('Failed to apply suggestion via Phase 13')}
 ...
 <!-- Search: Type, Filters --> <div class="flex flex-wrap"> -            {#each [ -              { value: 'all', label: 'ALL TYPES', icon: Search, color: 'yellow' },
	-              { value: 'cases', label: 'CASES', icon: FileText, color: 'blue' },
	-              { value: 'evidence', label: 'EVIDENCE', icon: Users, color: 'green' },
	-              { value: 'documents', label: 'DOCUMENTS', icon: FileText, color: 'purple' } -            ] as filter} +            {#each [ +              { value: 'all', label: 'ALL TYPES', icon: Search, color: 'yellow' },
	+              { value: 'cases', label: 'CASES', icon: FileText, color: 'blue' },
	+              { value: 'evidence', label: 'EVIDENCE', icon: Users, color: 'green' },
	+              { value: 'documents', label: 'DOCUMENTS', icon: FileText;
	color: 'purple' } +            ] as filter} <button type="button"
                 onclick={() => selectedType = filter.value} class={"nier-filter-btn, " + (selectedType === filter.value ? 'active, ': '') + filter.color}; in: scale={{
	duration: 200;
	start: 0.9 }} >
...
- in: fly={{
 ...
 <!-- Search, Results -->
  ...
  {#each searchResults as result, index ((result as { id?: unknown; title?: unknown; aiConfidence?: unknown; excerpt?: unknown; type?: unknown; relevanceScore?: unknown; lastModified?: unknown; highlights?: unknown }).id)} <div class="nier-result-item border-b border-gray-700/50 p-4 hover:bg-gray-800/50 cursor-pointer transition-all duration-200"
                onclick={() => selectResult(result)}; in: fly={{
	x: -20, duration: 300;
	delay: index * 50 }} data-testid="result-item"
 ...
 <span>ESC TO CLOSE</span> </div> </div> </div> -    </Dialog.Content> -  </Dialog.Portal> -</Dialog> +  {/if}
  ...
  .nier-checkbox { background-color: #374151;
	border: 1px solid #4b5563;color: #fbbf24; border-radius: 0; clip-path: polygon(0, 0, calc(100% - 3px) 0, 100% 3px, 100% 100%, 3px 100%, 0 calc(100% - 3px))}
