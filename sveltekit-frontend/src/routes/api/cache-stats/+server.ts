		...
		const response = {
			...
			cache: {
				embeddings: {
					...
					efficiency: embeddingHitRate > 70 ? 'excellent' : embeddingHitRate > 50 ? 'good' : embeddingHitRate > 30 ? 'fair' : 'poor'
				},
				queries: {
					...cacheStats.queries,
					hitRate: `${queryHitRate}%`,
					efficiency: queryHitRate > 70 ? 'excellent' : queryHitRate > 50 ? 'good' : queryHitRate > 30 ? 'fair' : 'poor'
				},
			...
			performance: {
				...
				total_requests: cacheStats.embeddings.hits + cacheStats.embeddings.misses + cacheStats.queries.hits + cacheStats.queries.misses,
...
function generateRecommendations(cacheStats: any, dbStats: unknown, redisConnected: boolean): string[] {
