...
interface SecurityValidationRequest { email: string; password: string;
  firstName?: string;
  lastName?: string;
  organizationName?: string;
  validationType: 'registration' | 'login' | 'password_reset';
 }
interface SecurityValidationResponse { success: boolean; validationId: string;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  success: true;
      validationId: progress: {
  stage: 'initializing', percentage: 0, message: `Starting security validation process...' },'`
      riskLevel: 'low', warnings: [], recommendations: [], wsEndpoint: `ws://localhost:5173/api/security/validate/ws/${validationId } };'`
    ...
    const passwordChecks = {
      minLength: password.length >= 8, hasUppercase: /[A-Z]/.test(password), hasLowercase: /[a-z]/.test(password), hasNumbers: /\d/.test(password), // Use a simpler, safe regex that matches: any non-word, non-space char (covers punctuation/special chars)
    ...
    response.progress = {
      stage: 'ai_analysis', percentage: 25, message: 'Running AI-powered security analysis...' };'`'`
    ...
    try {
      const ragResponse = await fetch('http://localhost:8094/api/security/check', {
        ...
        body: JSON.stringify({
          email: domain: email.split('@')[1], validationType
    ...
    response.progress = {
      stage: 'completed', percentage: 100, message: `Security validation completed successfully' };'`
    ...
    return json(response, {
      status: 200, headers: {
        'Cache-Control': 'no-store', 'X-Validation-Id': validationId
       }
    });
   }catch (error: any) {
    const errMessage =
    ...
    return json(
      {
        success: false;
        validationId: 'error', progress: {
  stage: 'error', percentage: 0, message: `Security validation failed' },'`
        riskLevel: 'critical', warnings: ['Validation service temporarily unavailable'], recommendations: ['Please try again later']
