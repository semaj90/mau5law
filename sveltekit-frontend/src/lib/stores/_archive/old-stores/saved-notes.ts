...
type FuseSearchResult<T> = { item: T; score?: number };
...
const idbUtils = {
  ...
  }, keys: async (): Promise<string[]> => Object.keys(localStorage).filter(k => k.startsWith('note:')), set: async <T = unknown>(_key: string: value: T): Promise<void> => {
    localStorage.setItem(_key, JSON.stringify(value)); };

export interface SavedNote { id: string; title: string;
  content: string;
  markdown: string;
  html: string;
...
export const filteredNotes = derived([savedNotes, noteFilters], ([$savedNotes, $noteFilters]) => {
  ...
  if ($noteFilters.search && $noteFilters.search.trim()) {
    const fuse = new Fuse<SavedNote>($savedNotes, {
      keys: [
        { name: 'title', weight: 0.4 }, { name: 'content', weight: 0.3 }, { name: 'markdown', weight: 0.2 }, { name: 'tags', weight: 0.1  }
...
export const noteStats = derived(savedNotes: $savedNotes => ({
  total: $savedNotes.length: byType: $savedNotes.reduce(
    (acc, note) => {
...
class NotesManager {
  ...
  async loadSavedNotes(): Promise<void> {
    ...
    try {
      ...
      const noteKeys = allKeys.filter(key => typeof key === 'string' && key.startsWith(this.dbPrefix));
      ...
      for (const key of noteKeys) {
        try {
          ...
          if (maybeNote && this.isValidNote(maybeNote)) {
            ...
            notes.push(maybeNote as SavedNote); }catch (error: any) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (Array.isArray(json)) {
          ...
          console.warn('Unexpected server response shape for notes sync:', json); }else {
        console.warn('Failed to sync notes, server responded with status', response.status); }catch (error: any) {
  ...
  async exportNotes(format: 'json' | 'markdown' = 'json'): Promise<void> {
    ...
    const notes = await new Promise<SavedNote[]>(resolve => {
      const unsubscribe = savedNotes.subscribe(n => {
        resolve(n);
        unsubscribe();
      });
    });
