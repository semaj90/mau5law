...
export interface Recommendation {
  ...
  description: string;
  score: number;
  confidence: number; aiGenerated: boolean;
...
export class Neo4jRecommendationEngine {
  ...
  async getRecommendations(query: { userId: string; context: string; type: string; useAI: boolean; limit: number; ): Promise<Recommendation[]> {
    ...
    return [
      { title: 'Related Case Law Analysis', description: 'Identified three highly relevant precedents based on your query.', score: 0.92, confidence: 0.95, aiGenerated: true
      }, {
