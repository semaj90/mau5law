...
interface Context7Client {
  ...
  getLibraryDocs?: (libraryId: string, options?: Record<string, unknown>) => Promise<{ content: string; metadata?: Record<string, unknown> }>;
...
type GPUCacheInterface = {
  ...
  // Use the typed GPUCacheStats instead of `any`
  getStats?: () => GPUCacheStats;
  stats?: GPUCacheStats | Record<string, unknown>;
  hitRate?: number;
  items?: number;
};
...
export class EvidenceSystemIntegration {
  ...
  async initialize(): Promise<boolean> {
    try {
      console.log('🚀 Initializing Evidence System Integration...');
  ...
  private async safeGPUSemanticSearch(embedding: Float32Array, limit = 10): Promise<Array<{ id: string; content?: string }>> {
    try {
      const cache = globalGPUCache as unknown as GPUCacheInterface;
   ...
   * Sync GPU cache with existing data
   */
  private async syncGPUCache(): Promise<void> {
    try {
  ...
  async storeEvidence(
    ...
    try {
      ...
      await this.minioClient.putObject(
        this.minioConfig.buckets.evidence,
        filePath,
        file
      );
      ...
      // 5. Store metadata in PostgreSQL with vector
      // compute safe file size/type without @ts-expect-error
  ...
  async semanticSearch(
    ...
    try {
      ...
      const cacheKey = `${this.redisConfig.keyPrefix}:search:${base64Key}`;
      ...
      if (cachedResult) {
        ...
        if (useGPUCache) {
          ...
          results.push(...gpuResults.map((entry) => ({
            id: entry.id,
            title: entry.id,
        ...
        if (results.length < limit) {
          ...
          results.push(...(pgResults.rows || []).map((row) => ({
            id: row.id,
        ...
        if (includeContext7 && results.length < limit && this.context7Client?.semanticSearch) {
          ...
          results.push(...(ctx.results || []).map((doc) => ({
            id: doc.id,
   ...
   */
  async getEvidenceFile(fileId: string): Promise<Blob> {
    try {
      // Get file path from PostgreSQL
      const result = (await this.postgresClient.query(
        'SELECT file_path FROM evidence_documents WHERE id = $1',
        [fileId]
      ...
      const raw = await this.minioClient.getObject(
        this.minioConfig.buckets.evidence,
        filePath
      );
   ...
   */
  async deleteEvidence(fileId: string): Promise<void> {
    try {
      // Get file info
      const result = (await this.postgresClient.query(
        'SELECT file_path FROM evidence_documents WHERE id = $1',
      ...
      await this.postgresClient.query(
        'DELETE FROM evidence_documents WHERE id = $1',
        [fileId]
      );
  ...
  async healthCheck(): Promise<SystemMetrics> {
    try {
      ...
      this.metrics.minioHealth = await this.testMinIOHealth();
      // Test PostgreSQL connection
      this.metrics.postgresHealth = await this.testPostgreSQLHealth();
      // Test Redis connection
      this.metrics.redisHealth = await this.testRedisHealth();
      // Test Context7 connection
      this.metrics.context7Health = await this.testContext7Health();
      // Get GPU cache stats (use safe helper)
  ...
  async optimize(): Promise<void> {
    try {
      ...
      // Optimize GPU cache via safe wrapper
      await this.safeGPUOptimize();
  ...
  private async generateEmbedding(text: string): Promise<Float32Array> {
    ...
    // 1) Compute a 32-bit seed from the text using FNV-1a
    // 2) Use Mulberry32 PRNG to produce dimension floats in [-0.5, 0.5]
    // 3) Normalize the vector and guard against zero magnitude
    const rawDim = this.postgresConfig?.vectorDimension ?? 768;
    const dimension = Math.max(1, Math.floor(rawDim));
    ...
    let sumSq = 0;
    for (let i = 0; i < dimension; i++) sumSq += embedding[i] * embedding[i];
    const magnitude = Math.sqrt(sumSq) || 1;
    for (let i = 0; i < dimension; i++) embedding[i] = embedding[i] / magnitude;
    // Ensure non-zero vector: if everything was exactly zero (extremely unlikely), set first element
