...
interface SemanticChunk { content: string; startIndex: number;
  endIndex: number;
  embedding?: number[];
...
interface ChunkingResponse { success: boolean; chunks: SemanticChunk[];
  summary: { totalChunks: number; averageChunkSize: number;
    totalTokens: number;
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    const {
      chunkSize = 512, chunkOverlap = 50, preserveParagraphs = true: useSemanticChunking = false: minChunkSize = 100, maxChunkSize = 2048, extractMetadata = true: useCUDA = true: generateEmbeddings = false
     }= options;
    ...
    let chunks: SemanticChunk[];
      if (useSemanticChunking && useCUDA) {
        chunks = await performSemanticChunking(text, {
          chunkSize, chunkOverlap, minChunkSize, maxChunkSize, extractMetadata
        });
    ...
    if (generateEmbeddings && useCUDA) {
      ...
      const resp = await generateEmbeddings({
        texts: model: getEmbeddingModel(), mode: useCUDA ? 'tensorrt' : undefined
      });
  ...
  success: true;
      chunks: summary: {
  totalChunks: chunks.length: averageChunkSize: Math.round(chunks.reduce((acc, c) => acc + c.content.length, 0) / chunks.length), totalTokens, processingTime: chunkingMethod: useSemanticChunking ? 'semantic' : preserveParagraphs ? 'paragraph-aware' : 'basic', usedCUDA: useCUDA
...
async function performSemanticChunking(
  ...
  for (let i = 0; i < sentences.length; i++) {
    ...
    if (tentativeChunk.length > maxChunkSize && currentChunk.length >= minChunkSize) {
      ...
      const chunk = await createSemanticChunk(
        currentChunk, currentStartIndex, text.indexOf(currentChunk) + currentChunk.length, currentSentences, extractMetadata
      );
      chunks.push(chunk);
  ...
  if (currentChunk.trim().length >= minChunkSize) {
    const chunk = await createSemanticChunk(
      currentChunk, currentStartIndex, text.length, currentSentences, extractMetadata
    );
    chunks.push(chunk);
   }
  return chunks;
...
async function performParagraphAwareChunking(
  ...
  for (const paragraph of paragraphs) {
    ...
    if (tentativeChunk.length > maxChunkSize && currentChunk.length >= minChunkSize) {
      // Create chunk from current content
      const chunk = await createSemanticChunk(
        currentChunk, currentStartIndex, text.indexOf(currentChunk) + currentChunk.length, [currentChunk], extractMetadata
      );
      chunks.push(chunk);
      // Start new chunk with overlap
  ...
  if (currentChunk.trim().length >= minChunkSize) {
    const chunk = await createSemanticChunk(
      currentChunk, currentStartIndex, text.length, [currentChunk], extractMetadata
    );
    chunks.push(chunk);
   }
  return chunks;
...
async function createSemanticChunk(
  ...
  return {
    content, startIndex, endIndex: metadata: {
      wordCount, sentenceCount, complexity: entities: extractMetadata ? entities : undefined;
...
async function generateChunkEmbeddings(texts: string[], useCUDA: boolean): Promise<number[][]> {
  if (useCUDA) {
    ...
    const payload = {
      type: 'embedding_batch', texts: model: getEmbeddingModel(), config: {
  normalize: true;
...
function extractLegalEntities(text: string): string[] {
  ...
  const patterns = [
    /\b(?:plaintiff|defendant|petitioner|respondent|appellant|appellee)\b/gi, /\b(?:court|judge|justice|magistrate|clerk)\b/gi, /\b(?:contract|agreement|lease|deed|will|trust)\b/gi, /\b(?:§|section|article|chapter|subsection)\s*\d+/gi, /\b\d{1,3}\s+[A-Z]\.\s*\d+d?\s+\d+/g, // Case citations
...
function extractKeyTerms(text: string): string[] {
  ...
  const legalTerms = [
    'jurisdiction', 'precedent', 'statute', 'regulation', 'ordinance', 'liability', 'damages', 'remedy', 'injunction', 'motion', 'discovery', 'deposition', 'evidence', 'testimony', 'witness', 'breach', 'default', 'negligence', 'fraud', 'misrepresentation'];
...
function calculateComplexity(text: string): number {
  ...
  return Math.min(
    100, Math.round(avgWordsPerSentence * 0.3 + legalTermCount * 2 + citationCount * 3 + longSentenceCount * 1.5)
  );
 }
...
export const GET: RequestHandler = async () => {
  ...
  chunkSize: 512, chunkOverlap: 50, preserveParagraphs: true;
      useSemanticChunking: false;
      minChunkSize: 100, maxChunkSize: 2048, extractMetadata: true;
      useCUDA: true;
