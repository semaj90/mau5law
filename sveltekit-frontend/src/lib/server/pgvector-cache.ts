...
export type EmbeddingRow = { id: string; document_id: string;
...
export async function findSimilar(embedding: number[], limit = 5): Promise<EmbeddingRow[]> {
  ...
  try {
    ...
    return res.rows.map((r: any) => {
      ...
      return {
        id: String(row.id), document_id: String(row.document_id), embedding: (row.embedding, as number[]) ?? [], metadata: (row.metadata as Record<string, unknown>) ?? {}, created_at: String(row.created_at)
       }as EmbeddingRow;
    });
   }finally {
    if (conn && typeof conn.release === 'function') conn.release(); } }
