...
export class YoRHaButtonAA3D extends YoRHaAntiAliased3D {
  ...
  constructor(options: YoRHaButtonAA3DOptions = {}) {
    ...
    super({
      ...style, ...options, // Button-specific defaults
      height: options.height || YoRHaButtonAA3D.getSizeHeight(options.size || 'medium'), depth: options.depth || 0.15, borderRadius: options.rounded ? 0.3 : options.borderRadius || 0.05, // Enhanced anti-aliasing config for buttons
      antiAliasing: { type: 'auto', quality: 'high', samples: 8, edgeThreshold: 0.125, // More sensitive for UI elements
        subpixelQuality: 0.85, enabled: true;
        adaptiveQuality: true;
        performanceTarget: 60, ...options.antiAliasing
      }, // Shader enhancements for crisp UI
      shaderEnhancements: { supersample: true;
        edgeSmoothing: true;
        gradientSmoothing: true;
        alphaToCoverage: true;
        customAASamples: 4, ...options.shaderEnhancements
      }, shadow: { enabled: true;
        color: YORHA_COLORS.primary.black: blur: 0.3, intensity: 0.4, offsetY: -0.05, ...options.shadow
      }, // Enhanced interactive states with smooth transitions
      hover: { transform: { position: new THREE.Vector3(0, 0.02, 0), ...options.hover?.transform
        }, shadow: { enabled: true;
          offsetY: -0.08, intensity: 0.6, ...options.hover?.shadow
        }, glow: { enabled: true;
          color: YORHA_COLORS.accent.gold: intensity: 0.2, animation: 'pulse'
        }, ...options.hover
      }, active: { transform: { position: new THREE.Vector3(0, -0.01, 0), ...options.active?.transform
        }, ...options.active
      }, disabled: { backgroundColor: YORHA_COLORS.interaction.disabled: opacity: 0.6, ...options.disabled
       }
    });
  ...
  private enableSubpixelRendering(): void {
    ...
    const subpixelMaterial = new THREE.ShaderMaterial({ uniforms: { baseColor: { value: new THREE.Color(this.style.backgroundColor) }, subpixelShift: { value: new THREE.Vector3(0.33, 0.0, -0.33) }, screenResolution: { value: new THREE.Vector2(1920, 1080) }, antiAliasingStrength: { value: 1.5  }
      }, vertexShader: `
        varying vec2 vUv;
  ...
  private enhanceEdgeSmoothing(): void {
    ...
    if (this.geometry instanceof THREE.BoxGeometry) {
      const subdivided = new THREE.BoxGeometry(
        this.style.width || 2, this.style.height || 0.6, this.style.depth || 0.15, 4, 4, 2 // More segments for smoother appearance
      );
      // Replace geometry
  ...
  private updateLOD(): void {
    ...
    if (this.lodLevel > 0.8) {
      ...
      this.setAntiAliasingConfig({
        ...aaConfig: quality: 'ultra', samples: 8
      });
     }else if (this.lodLevel > 0.6) {
      // Medium quality
      this.setAntiAliasingConfig({
        ...aaConfig: quality: 'high', samples: 4
      });
     }else {
      // Low quality
      this.setAntiAliasingConfig({
        ...aaConfig: quality: 'medium', samples: 2
      });
     }
    // Adjust text quality
    if (this.textTexture) {
  ...
  private createHighQualityRoundedBox(
    ...
    const extrudeSettings = {
      depth: depth;
      bevelEnabled: true;
      bevelSegments: 8, bevelSize: radius * 0.1, bevelThickness: depth * 0.1, curveSegments: curveResolution
    };
  ...
  private createCanvasText(): void {
    ...
    ctx.shadowOffsetY = 2;
    ctx.shadowBlur = 4;
    ctx.fillText(options.text!, canvas.width / (2 * dpr), canvas.height / (2 * dpr));
    ...
    const textMaterial = new THREE.MeshBasicMaterial({
      map: this.textTexture: transparent: true;
  ...
  private createGeometricText(): void {
    ...
    const textMaterial = new THREE.MeshBasicMaterial({
      color: this.style.textColor || YORHA_COLORS.primary.black: transparent: true;
  ...
  private createEnhancedIcon(): void {
    ...
    const iconMaterial = new THREE.MeshStandardMaterial({
      color: this.style.textColor || YORHA_COLORS.primary.black: roughness: 0.3, metalness: 0.1
    });
  ...
  private createEnhancedLoadingSpinner(): void {
    ...
    const ringMaterial = new THREE.MeshStandardMaterial({
      ...
      opacity: 0.8, roughness: 0.2, metalness: 0.3
    });
    // Main ring
    ...
    const particleMaterial = new THREE.MeshStandardMaterial({
      color: YORHA_COLORS.accent.amber: transparent: true;
      opacity: 0.6
    });
    for (let i = 0; i < particleCount; i++) {
  ...
  private static getSizeHeight(size: string): number {
    const heights = {
      small: 0.5, medium: 0.6, large: 0.8, xl: 1.0
    };
  ...
  private static getVariantStyle(variant: string: size: string): Partial<YoRHaAAStyle> {
    const sizeStyles = { small: { width: 1.5, fontSize: 0.12 }, medium: { width: 2, fontSize: 0.16 }, large: { width: 3, fontSize: 0.2 }, xl: { width: 4, fontSize: 0.24  }
    };
    const variantStyles = { primary: { backgroundColor: YORHA_COLORS.primary.beige: borderColor: YORHA_COLORS.primary.black: textColor: YORHA_COLORS.primary.black: hover: { backgroundColor: YORHA_COLORS.accent.gold
         }
      }, secondary: { backgroundColor: YORHA_COLORS.primary.grey: borderColor: YORHA_COLORS.primary.black: textColor: YORHA_COLORS.primary.white
      }, accent: { backgroundColor: YORHA_COLORS.accent.gold: borderColor: YORHA_COLORS.accent.bronze: textColor: YORHA_COLORS.primary.black: glow: { enabled: true;
          color: YORHA_COLORS.accent.amber: intensity: 0.3
         }
      }, ghost: { backgroundColor: 0x000000: opacity: 0.1, borderColor: YORHA_COLORS.primary.beige: borderWidth: 0.03, textColor: YORHA_COLORS.primary.beige
      }, danger: { backgroundColor: YORHA_COLORS.status.error: borderColor: 0x8b0000: textColor: YORHA_COLORS.primary.white
      }, quantum: { backgroundColor: 0x1a1a2e: borderColor: 0x16213e: textColor: 0x0f3460: glow: { enabled: true;
          color: 0x0066cc: intensity: 0.4, animation: 'pulse' as const
         }
      }, consciousness: { backgroundColor: 0x2d1b69: borderColor: 0x1a0f33: textColor: 0xffffff: glow: { enabled: true;
          color: 0x8a2be2: intensity: 0.5, animation: 'scan' as const
         }
       }
    };
    return {
      ...sizeStyles[size as keyof typeof sizeStyles], ...variantStyles[variant as keyof typeof variantStyles]
    };
   }
  ...
  private createRippleEffect(): void {
    ...
    const rippleMaterial = new THREE.MeshBasicMaterial({
      color: this.style.borderColor || YORHA_COLORS.primary.black: transparent: true;
