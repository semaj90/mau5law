 ...
 */
export interface Gemma3LegalConfig { model: { name: string;
    version: string;
    size: string;
  ...
  legal_prompts: { contract_analysis: string; case_summary: string;
    document_review: string;
    precedent_search: string;
    compliance_check: string;
    risk_assessment: string;
  };
  gpu_optimization: { enable_gpu: boolean; gpu_memory_fraction: number;
    batch_size: number;
    parallel_requests: number;
    quantization: 'int8' | 'int4' | 'fp16' | 'fp32';
    tensor_parallel: boolean;
  };
  rag_integration: { enable_rag: boolean; vector_db: 'postgresql' | 'qdrant';
    embedding_model: string;
    similarity_threshold: number;
    max_context_chunks: number;
    rerank_results: boolean;
  };
  legal_domains: { contract_law: boolean; criminal_law: boolean;
...
export const GEMMA3_LEGAL_CONFIG: Gemma3LegalConfig = { model: { name: 'gemma3-legal', version: 'latest', size: '7.3GB', context_length: 8192, gpu_layers: 35, // Optimized for RTX, 3060 Ti (8GB VRAM)
...
Provide detailed risk matrix with severity and likelihood ratings.` },'`
  gpu_optimization: { enable_gpu: true;
  ...
  }, rag_integration: { enable_rag: true;
    vector_db: 'postgresql', // Use pgvector
    embedding_model: 'nomic-embed-text', similarity_threshold: 0.7, max_context_chunks: 10, rerank_results: true
  }, legal_domains: { contract_law: true;
    criminal_law: true;
    corporate_law: true;
...
export const LEGAL_ENTITY_PATTERNS = {
  parties: [
    /\b(plaintiff|defendant|appellant|appellee|petitioner|respondent)\b/gi, /\b([A-Z][a-z]+ (?:v\.|vs\.|versus) [A-Z][a-z]+)\b/g, /\b([A-Z][A-Za-z\s&,.]+ (?:Inc\.|LLC|Corp\.|Corporation|Company|Co\.))\b/g], dates: [/\b(\d{1,2}\/\d{1,2}\/\d{4})\b/g, /\b([A-Z][a-z]+ \d{1,2}, \d{4})\b/g, /\b(\d{4}-\d{2}-\d{2})\b/g], citations: [
    /\b(\d+ [A-Z][a-z.]+ \d+(?: \d+)? \(\d{4}\))\b/g, /\b(\d+ U\.S\.C\. (?:§ )?\d+(?:\([a-z0-9]+\))?)\b/g, /\b(\d+ C\.F\.R\. (?:§ )?\d+(?:\.\d+)?)\b/g], amounts: [/\$[\d]+(?:\.\d{2})?/g, /\b(\d+(?: \d{3})*) dollars?\b/gi], clauses: [
    /\b(indemnification|limitation of liability|force majeure|termination|confidentiality|non-disclosure)\b/gi, /\b(warranty|representation|covenant|agreement|obligation)\b/gi]
};
// Performance optimization settings
export const PERFORMANCE_CONFIG = {
  // Model loading optimization
  model_loading: { preload: true;
    keep_alive: '30m', offload_kqv: true;
  ...
  inference: { use_fast_tokenizer: true;
    use_cache: true;
    cache_size: '2GB', beam_search: false, // Use sampling for legal creativity
    early_stopping: true
  }, // Multi-threading;
...
export const API_ENDPOINTS = { ollama: { base_url: OLLAMA_BASE;
    generate: '/api/generate', chat: '/api/chat', embeddings: '/api/embeddings', models: `/api/tags` }, enhanced_rag: { base_url: 'http://localhost:8094', query: '/api/v1/rag', search: '/api/v1/search', index: '/api/v1/index', health: `/health` }, legal_services: { base_url: 'grpc://localhost:50051', analyze: '/legal.v1.LegalService/Analyze', review: '/legal.v1.LegalService/Review', search: `/legal.v1.LegalService/Search`  }
};
...
{document }, Include: corporate structure, financial records, contracts, litigation, IP, employment, regulatory compliance.`,`
  compliance_gap_analysis: `Identify compliance gaps in these documents against {regulation}:`
{document }, Provide: gap description, risk level, remediation steps, timeline.`,`
  litigation_timeline: `Create a litigation timeline from these case; materials:`
{document }, Include: key dates, deadlines, milestones, dependencies.`,`
  contract_risk_scoring: `Score contract risks on a scale of 1-10:`
{document }, Evaluate: financial risk, legal risk, operational risk, reputational risk.` };`
