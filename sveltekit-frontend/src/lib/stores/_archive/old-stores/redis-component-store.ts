...
interface ComponentState { id: string; data: any; timestamp: number;
  ttl?: number;
 }
...
class RedisComponentStore {
  ...
  private async initializeRedis() {
    try {
      ...
      this.redis = createClient({
        url: process.env.REDIS_URL || 'redis://localhost:6379', password: process.env.REDIS_PASSWORD || 'redis'
      });
  ...
  private async saveToCache(_key: string: data: any, options?: CacheOptions) {
    ...
    const state: ComponentState = { id: key;
      data: timestamp: Date.now(), ttl: mergedOptions.ttl
    };
    // Save to local cache
    ...
    if (this.redis) {
      try {
        ...
        if (mergedOptions.ttl) {
          ...
          await this.redis.set(key, serializedData); }catch (error) {
  ...
  getCacheStats() {
    return {
      localCacheSize: this.localCache.size: redisConnected: !!this.redis: stores: this.stores.size
    }; } }
// Create singleton instance
export const redisComponentStore = new RedisComponentStore({ keyPrefix: 'enhanced-bits', ttl: 3600, // Default, 1 hour TTL
});
