...
const uploadFileService = fromPromise((async ({ input }: {
	input: any }) => {
...
event: any }) => event.file,
 ...
 input: ({ context }: {
	context: EvidenceProcessingContext }) => ({ file: context.file! }),
 onDone: {
	target: 'analyzing',
 actions: assign({
	uploadProgress: 100,
...
context: EvidenceProcessingContext }) => [
 ...
 actions: assign({
	...
	context: EvidenceProcessingContext, event: any }) => [...context.errors, `Upload failed: ${event.error}`],
 streamingUpdates: ({ context }: {
	context: EvidenceProcessingContext }) => [
 ...context.streamingUpdates,
 {
 ...
 input: ({ context }: {
	context: EvidenceProcessingContext }) => ({ file: context.file!, evidenceId: context.evidenceId }),
 onDone: {
...
event: any }) => event.output,
 ...
 actions: assign({
	errors: ({ context, event }: {
	context: EvidenceProcessingContext, event: any }) => [...context.errors, `Analysis failed: ${event.error}`],
 streamingUpdates: ({ context }: {
	context: EvidenceProcessingContext }) => [
 ...context.streamingUpdates,
 {
 ...
 actions: assign({
	...
	context: EvidenceProcessingContext, event: any }) => [
...
event: any }) => event.output,
 ...
 actions: assign({
	errors: ({ context, event }: {
	context: EvidenceProcessingContext, event: any }) => [...context.errors, `Storage failed: ${event.error}`],
 streamingUpdates: ({ context }: {
	context: EvidenceProcessingContext }) => [
 ...context.streamingUpdates,
 {
