...
export interface UserDictionary { preferredStyle: 'formal' | 'casual' | 'technical' | 'adaptive';, domainExpertise: string[];
  termCount: number;
  interactionCount: number;
  qloraCheckpoint?: string;
} }
export interface SystemStatus { nesMemoryReady: boolean;, gpuCacheReady: boolean;
  qloraReady: boolean;
...
export const ssrQloraChatMachine = createMachine<ChatContext, ChatEvent>({
  ...
  context: { userId: '',
    ...
    messages: [],
    currentMessage: '',
    userDictionary: { preferredStyle: 'adaptive',
      domainExpertise: [],
      termCount: 0,
      interactionCount: 0
    },
  ...
  states: { initializing: { entry: ['logInitialization'],
      on: { INITIALIZE: { actions: ['setUserSession'],
          target: 'loading'
        } }
      } }
    },
    loading: { entry: ['checkSystemStatus'],
      on: { CONTEXT_LOADED: { actions: ['loadContext'],
          target: 'idle'
        },
    ...
    idle: { entry: ['updateIdleMetrics'],
      on: { SEND_MESSAGE: { actions: ['addUserMessage'],
          target: 'processing'
        },
    ...
    processing: { initial: 'determiningMode',
      states: { determiningMode: { entry: ['analyzeMessage'],
          always: [
        ...
        instantResponse: { entry: ['setProcessingMode', 'handleInstantResponse'],
          on: { INSTANT_RESPONSE: { actions: ['addInstantResponse'],
              target: '#ssrQloraChat.idle'
            } }
          } }
        },
        cachedResponse: { entry: ['setProcessingMode', 'handleCacheHit'],
          on: { CACHE_HIT: { actions: ['addCachedResponse'],
              target: '#ssrQloraChat.idle'
            } }
          } }
        },
        qloraProcessing: { entry: ['setProcessingMode', 'startQLoRAProcessing'],
          on: { STREAM_STARTED: { actions: ['startStreaming'],
              target: 'streaming'
            },
            QLORA_RESPONSE: { actions: ['addQLoRAResponse'],
              target: '#ssrQloraChat.idle'
            },
            ERROR: { actions: ['setError'],
        ...
        gemma3Processing: { entry: ['setProcessingMode', 'startGemma3Processing'],
          on: { STREAM_STARTED: { actions: ['startStreaming'],
              target: 'streaming'
            },
            GEMMA3_RESPONSE: { actions: ['addGemma3Response'],
              target: '#ssrQloraChat.idle'
            },
            ERROR: { actions: ['setError'],
              target: '#ssrQloraChat.error'
        ...
        streaming: { on: { STREAM_CHUNK: { actions: ['addStreamChunk']
            ...
            STREAM_COMPLETE: { actions: ['completeStreaming'],
        ...
        postProcessing: { entry: ['generateNeuralSprite', 'updatePerformanceMetrics'],
          on: { NEURAL_SPRITE_GENERATED: { actions: ['attachNeuralSprite'],
              target: '#ssrQloraChat.idle'
            } }
          },
          after: {
      ...
      on: { ERROR: { actions: ['setError'],
          target: 'error'
        } }
      } }
    },
    error: { entry: ['logError'],
      on: { RETRY_MESSAGE: { actions: ['clearError'],
          target: 'processing'
        },
        CLEAR_CHAT: { actions: ['clearMessages', 'clearError'],
          target: 'idle'
        },
        INITIALIZE: { actions: ['clearError'],
          target: 'loading'
        } }
      } }
    } }
  ...
  actions: { logInitialization: () => {
    ...
    setUserSession: assign({ userId: (_, event) => event.type === 'INITIALIZE' ? event.userId: '',
      sessionId: (_, event) => event.type === 'INITIALIZE' ? event.sessionId : ''
    }),
    loadContext: assign({ userDictionary: (_, event) => event.type === 'CONTEXT_LOADED' ? event.context.userDictionary : { [key,: strin,g]: any },
    ...
    addCachedResponse: assign({ messages: (context, event) => {
    ...
    addQLoRAResponse: assign({ messages: (context, event) => {
    ...
    completeStreaming: assign({ messages: (context, event) => {
    ...
    recordFeedback: assign({ messages: (context, event) => {
    ...
    updateSystemStatus: assign({ systemStatus: (context, event) => {
    ...
    updatePerformanceMetrics: assign({ performanceMetrics: (context) => {
