...
class Context7MulticoreService extends EventEmitter {
  ...
  private async processQueuedTasks() {
    ...
    const availableWorkers = Array.from(this.workers.values()).filter((w) => w.status === 'healthy' && w.currentLoad < 0.8);
    if (availableWorkers.length === 0) return;

    const tasksToProcess = Math.min(this.taskQueue.length, availableWorkers.length, this.config.maxConcurrentTasks - this.activeTasks.size);
  ...
  private async executeTaskOnWorker(_task: ProcessingTask, _worker: WorkerInfo): Promise<unknown> {
    ...
    try {
      ...
      const contentType = (res.headers && (res.headers.get?.('content-type') ?? (res.headers['content-type'] as string))) || '';
  ...
  async parseJSON(jsonString: string, schema?: string, priority: ProcessingTask['priority'] = 'medium'): Promise<ProcessingTask> {
    const task: ProcessingTask = {
      id: this.generateTaskId(),
      type: 'json_parse',
