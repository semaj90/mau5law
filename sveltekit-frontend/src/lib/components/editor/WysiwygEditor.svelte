...
import { detectEnvironment } from '$lib/types/enhanced-svelte5-types';
 ...
 let isInitialized = $state<boolean>(false); const wordCount: Writable<number> = writable(0); const charCount: Writable<number> = writable(0); const aiOpen = writable(false); const citeOpen = writable(false); // collaboration toggle so `enableCollaboration` is used const collaborationActive = writable(false); function toggleCollaboration() { collaborationActive.update(v => !v)}'
  ...
  async function initializeEditor(): Promise<void> { try { // Dynamically import TinyMCE as an alternative // const { default: Hugerte } = await import('hugerte') // Initialize basic editor for now hugerte = { getContent: () => content, setContent: (newContent: string) => { content = newContent},
	...
	ui: {
	registry: { addButton: (name: string, options: unknown) => 0%; addIcon: () => 0% }
        },
	...
	plugins: [
          ...
          ], content_style: ` body { font-family: 'Inter', -apple-system; BlinkMacSystemFont: 'Segoe UI', sans-serif; font-size: 14px, line-height: 1.6, color: #374151, max-width: 800px;
	margin: 0 auto; padding: 20px}`
            h1, h2, h3, h4, h5, h6 { color: #1f2937, font-weight: 600, margin-top: 1.5em; margin-bottom: 0.5em}
            .citation { background: #eff6ff, border-left: 4px solid #3b82f6,padding: 0.5em;
	...
	margin: 0.5em 0}
          `, setup: (editor: unknown) => { // Custom AI Assistant button editor.ui.registry.addButton('ai-assistant', { text: 'ðŸ¤– AI', tooltip: 'AI Assistant', onAction: () => openAIAssistant(editor.selection.getContent()) }); // Custom Citation Helper button editor.ui.registry.addButton('citation-helper', { text: 'ðŸ“š Cite', tooltip: 'Citation Helper', onAction: () => openCitationHelper(editor.selection.getContent()) }); // Auto-save functionality editor.on('NodeChange', () => { updateCounts(editor.getContent())}); // Content change listener editor.on('input', () => { const newContent = editor.getContent(); content = newContent; const wc = updateCounts(newContent); ondispatch?.({ content: newContent;
	wordCount: wc })}); // Save handler editor.on('save', () => { ondispatch?.({ content: editor.getContent() })})},
...
; bind:value={ citationQuery } placeholder="Enter legal concept, case name, or statute..."
 ...
 <button class="absolute right-4 top-4 cursor-pointer border-none bg-transparent text-2xl leading-none text-gray-500" onclick={() => citeOpen.set(false)}>Ã—</button> </div> {/if}
  ...
  .selected-text { background: #eff6ff;
	border: 1px solid #bfdbfe; border-radius: 0.25rem;
	padding: 0.75rem; margin-bottom: 1rem}
  .ai-query-input, .cite-query-input { width: 100%;
	padding: 0.75rem; border: 1px solid #d1d5db; border-radius: 0.375rem; margin-bottom: 1rem;
	resize: vertical; font-family: inherit}
  .ai-results { margin-top: 1rem;
	padding: 1rem; background: #f0fdf4;
	border: 1px solid #bbf7d0; border-radius: 0.25rem}
  .ai-response { margin: 0.75rem 0; padding: 0.75rem;background: white;
