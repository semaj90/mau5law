...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const {
      documentId, content: processingPipeline = ['document-analysis', 'entity-extraction', 'legal-classification'], priority = 1, metadata = {}, evidenceCanvasId: analysisType = 'legal'
     }= body;
    if (!content) {
    ...
    const result = await unifiedLegalOrchestrationService.processLegalDocument({
      documentId, content, processingPipeline, priority, metadata, evidenceCanvasId, analysisType
    });
    ...
    return json({
  success: true;
      jobIds: (result as { statusStores?: any; jobIds?: any; processingMetrics?: any }).jobIds, jobStatuses: aggregateStatusEndpoint: '/api/legal/status/aggregate/${(result as { statusStores?: any; jobIds?: any; processingMetrics?: any }).jobIds.join(',')}`,'`
