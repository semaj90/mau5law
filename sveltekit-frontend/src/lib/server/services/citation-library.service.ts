...
class CitationLibraryService {
  ...
  async getCollectionDetail(collectionId: string): Promise<CitationCollection | null> {
    try {
      ...
      const collections = await db.raw(
        `SELECT * FROM citation_collections WHERE id = $1`,
        [collectionId]
      );
   ...
   * Add tag to citation
   */
  async addTag(citationId: string, tag: string, userId: string): Promise<CitationTag> {
    try {
   ...
   */
  async removeTag(citationId: string, tag: string, userId: string): Promise<void> {
    try {
      await db.raw(
        `DELETE FROM citation_tags WHERE citation_id = $1 AND tag = $2`,
