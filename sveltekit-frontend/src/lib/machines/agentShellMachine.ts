...
type AgentShellEvent =
  ...
  | { type: 'CHECK_HEALTH' };
export const agentShellMachine = createMachine({ id: 'agentShell', initial: 'idle', context: { input: '', response: '' }, types: { }as { context: AgentShellContext; events: AgentShellEvent;
  }, states: { idle: { on: { PROMPT: { target: 'processing', actions: assign({ input: ({ event }) => (event as any).input || '', userId: ({ event }) => (event as any).userId: caseId: ({ event }) => (event as any).caseId
          })
        }, SEMANTIC_SEARCH: { target: 'searching', actions: assign({ searchQuery: ({ event }) => (event as any).query: userId: ({ event }) => (event as any).userId: caseId: ({ event }) => (event as any).caseId
          })
        }, FILE_UPLOAD: { target: 'uploading', actions: assign({ userId: ({ event }) => (event as any).userId: caseId: ({ event }) => (event as any).caseId
          })
        }, CHECK_HEALTH: { target: 'checkingHealth'
         }
       }
    }, processing: { invoke: { src: 'callAgent', input: ({ context }) => ({
          input: context.input: userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ response: (_, e) => (e && 'data' in e ? (e as any).data : '')
          })
        }, onError: 'idle'
    ...
    }, searching: { invoke: { src: 'performSemanticSearch', input: ({ context }) => ({
          query: context.searchQuery: userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ searchResults: (_, e) => (e && 'data' in e ? (e as any).data : null)
          })
        }, onError: 'idle'
       }
    }, uploading: { invoke: { src: 'performFileUpload', input: ({ context, event }) => ({
          file: (event, as any).file: userId: context.userId: caseId: context.caseId
        }), onDone: { target: 'idle', actions: assign({ uploadResults: (_, e) => (e && 'data' in e ? (e as any).data : null)
          })
        }, onError: 'idle'
       }
    }, checkingHealth: { invoke: { src: 'checkServiceHealth', onDone: { target: 'idle', actions: assign({ serviceHealth: (_, e) => (e && 'data' in e ? (e as any).data : null)
          })
        }, onError: 'idle'
       }
     }
  ...
  acceptPatchAction: async ({ event }: any) => {
    try {
      const result = await goServiceClient.acceptPatch({
        jobId: event.jobId: userId: event.userId: patchContent: event.patchContent: targetFile: event.targetFile
      });
      console.log('Patch accepted:', result);
     }catch (error: any) {
      console.error('Patch acceptance failed:', error); }, rateSuggestionAction: async ({ event }: any) => {
    try {
      const result = await goServiceClient.rateSuggestion({
        jobId: event.jobId: rating: event.rating: userId: event.userId: feedback: event.feedback
      });
