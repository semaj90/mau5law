<script lang="ts">
  ...
  let winProbabilityColor = $derived(
    data.analysisResults.comprehensive.win_probability > 0.75
      ? 'text-green-600'
      : data.analysisResults.comprehensive.win_probability > 0.5
        ? 'text-yellow-600'
        : 'text-red-600',
  );
  let riskAssessmentColor = $derived(
    data.analysisResults.comprehensive.risk_assessment === 'low'
      ? 'text-green-600'
      : data.analysisResults.comprehensive.risk_assessment === 'medium'
        ? 'text-yellow-600'
        : 'text-red-600',
  );
  ...
  function getConfidenceColor(confidence: number): string {
    return confidence > 0.8 ? 'text-green-600' : confidence > 0.6 ? 'text-yellow-600' : 'text-red-600';
  }
            ...
            <div class="bg-white p-6 rounded-lg border border-gray-200">
              ...
              <div class="space-y-3">
                ...
                <div>
                  ...
                  {formatCurrency(data.caseDetails.estimated_value)}
                </div>
                ...
                <div class="space-y-3">
                  {#each data.analysisResults.comprehensive.key_issues as issue, index}
                    <div class="flex items-start gap-3">
                      <div
                        class="w-6 h-6 bg-red-100 text-red-600 rounded-full flex items-center justify-center text-sm font-medium"
                      >
                        {index + 1}
                      </div>
                  ...
                  <tbody>
                    {#each data.analysisResults.comprehensive.similar_cases as caseItem}
                      <tr class="border-b">
                        ...
                        <td class="py-2">
                          <span
                            class="px-2 py-1 text-xs rounded-full {caseItem.outcome === 'Plaintiff Victory'
                              ? 'bg-green-100 text-green-800'
                              : caseItem.outcome === 'Settlement'
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div class="bg-white p-6 rounded-lg border border-gray-200 text-center">
                <div class="text-3xl font-bold text-blue-600">
                  {data.analysisResults.document_analysis.total_documents}
                </div>
                <div class="text-gray-600">Total Documents</div>
              </div>
              <div class="bg-white p-6 rounded-lg border border-gray-200 text-center">
                <div class="text-3xl font-bold text-green-600">
                  {data.analysisResults.document_analysis.processed_documents}
                </div>
                  ...
                  <tbody>
                    {#each data.analysisResults.document_analysis.key_documents as doc}
                      <tr class="border-b">
                        ...
                        <td class="py-2">
                          <span
                            class="px-2 py-1 text-xs rounded-full capitalize {getRiskLevelColor(
                              doc.risk_level,
                            )} bg-opacity-10"
                          >
                            {doc.risk_level}
          ...
          <div class="space-y-6">
                ...
                <div class="space-y-3">
                  <div class="flex justify-between">
                    ...
                    <span class="font-bold"
                      >{formatCurrency(data.analysisResults.financial_analysis.claimed_damages)}</span
                    >
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-600">Estimated Actual:</span>
                    <span class="font-bold text-green-600"
                      >{formatCurrency(data.analysisResults.financial_analysis.estimated_actual_damages)}</span
                    >
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-600">Litigation Costs:</span>
                    <span class="font-bold text-red-600"
                      >{formatCurrency(data.analysisResults.financial_analysis.litigation_costs)}</span
                    >
                  </div>
                </div>
              </div>
              <div class="bg-white p-6 rounded-lg border border-gray-200">
                ...
                <div class="space-y-3">
                  <div class="flex justify-between">
                    ...
                    <span class="font-bold text-red-600"
                      >{formatCurrency(data.analysisResults.financial_analysis.settlement_range.min)}</span
                    >
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-600">Optimal:</span>
                    <span class="font-bold text-blue-600"
                      >{formatCurrency(data.analysisResults.financial_analysis.settlement_range.optimal)}</span
                    >
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-600">Maximum:</span>
                    <span class="font-bold text-green-600"
                      >{formatCurrency(data.analysisResults.financial_analysis.settlement_range.max)}</span
                    >
                  </div>
                </div>
              </div>
            ...
            <div class="bg-blue-50 p-6 rounded-lg border border-blue-200">
              ...
              <p class="text-blue-800">
                Based on ROI analysis, <strong>early mediation</strong> provides the highest return at 85% ROI. This approach
                minimizes litigation costs while maximizing potential settlement value.
              </p>
            </div>
          </div>
                ...
                {#each data.aiInsights.strategy_recommendations as rec}
                  <div class="border-l-4 {getPriorityColor(rec.priority)} border-opacity-50 pl-4 py-2">
                    <div class="flex justify-between items-start mb-2">
                      <span
                        class="px-2 py-1 text-xs rounded-full capitalize {getPriorityColor(rec.priority)} bg-opacity-10"
                      >
                ...
                {#each data.aiInsights.risk_factors as risk}
                  <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div>
                      ...
                      <span
                        class="ml-2 px-2 py-1 text-xs rounded-full capitalize {risk.impact === 'high'
                          ? 'bg-red-100 text-red-800'
                          : risk.impact === 'medium'
                            ? 'bg-yellow-100 text-yellow-800'
                            : 'bg-green-100 text-green-800'}"
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div class="bg-white p-4 rounded-lg border border-gray-200 text-center">
            <div class="text-2xl font-bold text-yellow-600">
              {formatCurrency(data.analysisResults.comprehensive.settlement_recommendation)}
            </div>
