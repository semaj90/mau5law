  ...
  // Svelte, 5 runes are auto-imported
  	...
  	$effect(() => {
  		...
  		const timeInterval = setInterval(() => {
  			const now = new Date());
  			currentTime = now.toLocaleTimeString()}, 1000);
...
d
	...
	;right: 0;
		...
		box-shadow: 0 3px 0 0 var(--yorha-secondary, #ffd700),
	...
	.status-indicator.online .status-dot { background: var(--yorha-accent, #00ff41);
		border-radius: 0;
		box-shadow: 0 0 0 1px var(--yorha-bg-secondary, #1a1a1a),
	...
	.stat-item:hover { background: var(--yorha-bg-tertiary, #2a2a2a);
		border-color: var(--yorha-secondary, #ffd700), transform translateY(-1px); box-shadow: 0 0 0 1px var(--yorha-secondary, #ffd700),
