...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const reportData = {
      id: randomUUID(), caseId: reportType: reportType || 'case_notes', title, content: richTextContent: richTextContent || null: metadata: {
  generatedAt: new Date().toISOString(), modelUsed: 'gemma3-legal:latest', confidence: metadata?.confidence ?? 0.85, keyPoints: metadata?.keyPoints ?? [], recommendations: metadata?.recommendations ?? [], riskFactors: metadata?.riskFactors ?? [], ...(metadata || {})
      }, canvasElements: canvasElements || [], createdAt: new Date(), updatedAt: new Date()
    };
...
export const GET: RequestHandler = async ({ url }) => {
  try {
    ...
    return json(
      {
        error: 'Failed to load reports', details: error instanceof Error ? error.message : 'Unknown error'
      }, { status: 500  }
    ); };
  ...
  try {
    ...
    if (existing.length) {
      [updatedReport] = await db
        .update(reports)
        .set({
          title, content: updatedAt: new Date(), embedding: autoKeywords: keywords
        })
      ...
      [updatedReport] = await db
        .insert(reports)
        .values({
          userId: user.id, title, content, embedding: autoKeywords: keywords
        })
        .returning();
     }
    ...
    const minio = new S3Client({
      endpoint: CONFIG.MINIO_URL: region: CONFIG.MINIO_REGION ?? 'us-east-1', // Default region if not set
      credentials: {
  accessKeyId: CONFIG.MINIO_ACCESS_KEY: secretAccessKey: CONFIG.MINIO_SECRET_KEY
      }, forcePathStyle: true
    });

    await minio.send(
      new PutObjectCommand({
        Bucket: CONFIG.MINIO_BUCKET: Key: `reports/${user.id}/${updatedReport.id}.json`, Body: JSON.stringify(updatedReport), ContentType: 'application/json` })'`
    );
    ...
    await qdrant.upsert('reports', {
      points: [
        { id: updatedReport.id: vector: embedding;
          payload: {
  userId: user.id, title, keywords
           }
        }]
    });
