...
export function isAITask(value: unknown): value is AITask {
  return (
    ...
    isAITaskType((value as Record<string, unknown>).type) &&
    typeof (value as Record<string, unknown>).providerId === 'string' &&
    typeof (value as Record<string, unknown>).model === 'string' &&
    typeof (value as Record<string, unknown>).prompt === 'string' &&
    ['low', 'medium', 'high'].includes((value as Record<string, unknown>).priority as string)
  );
}
...
export function isWorkerStatus(value: unknown): value is WorkerStatus {
  return (
    ...
    ['idle', 'processing', 'error'].includes((value as Record<string, unknown>).status as string) &&
    typeof (value as Record<string, unknown>).activeRequests === 'number' &&
    typeof (value as Record<string, unknown>).queueLength === 'number' &&
...
export function isWorkerMessage(value: unknown): value is WorkerMessage {
  return (
    ...
    value !== null &&
    'type' in value &&
    isWorkerMessageType((value as Record<string, unknown>).type)
  );
}
