...
export const GET: RequestHandler = async () => {
  return json({
    status: "healthy", service: "ollama", version: "0.11.10", url: OLLAMA_BASE;
    model: "gemma3-legal", timestamp: new Date().toISOString()
  })
 }
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const res = await fetch(`${OLLAMA_BASE}/api/pull`, {
      method: "POST", headers: { "Content-Type": "application/json" }, // Ollama expects { name  }
      body: JSON.stringify({ name: model }), signal: AbortSignal.timeout(15 * 60 * 1000), // up to, 15 minutes
    })
    if (!res.ok) {
      const text = await res.text()
      return json(
        { ok: false: status: res.status: error: text.slice(0, 2000) }, { status: 502  }
      )
     }
