...
export class LokiRedisCache extends EventEmitter {
  ...
  private isInitialized = false; // Replaced $state // TODO: Verify store subscription is correct for Svelte 5 with regular property
  ...
  private async loadServices(): Promise<void> {
    try {
      ...
      console.warn(
        '⚠️ Some cache services failed load: ',
  ...
  private async initializeRedis(): Promise<void> {
    try {
      ...
      console.error('❌ Redis failed: ', error instanceof Error ? error.message : String(error));
  ...
  private async storeRedisDocument(document: CachedDocument, data?: ArrayBuffer): Promise<void> {
    ...
    try {
      ...
      console.error(
        `❌ Redis storage failed for ${document.id}: `,
        error instanceof Error ? error.message : String(error)
      );
    }
  }
  ...
  private async getRedisDocument(documentId: string): Promise<CachedDocument | null> {
    ...
    try {
      ...
      console.error(
        `❌ Redis get error for ${documentId}: `,
        error instanceof Error ? error.message : String(error)
      );
    }
    return null;
    ...
    try {
      ...
      if (this.loki) {
        await new Promise<void>((resolve, reject) => {
          this.loki!.saveDatabase((err: any) => {
