...
function getUserId(locals: App.Locals): string {
  // Assuming locals.user.id exists and is a string
...
export const GET: RequestHandler = async ({ locals, url }) => {
  try {
    ...
    finalQuery = finalQuery.orderBy(sortOrder === 'asc' ? asc(orderColumn) : desc(orderColumn));

    // Add pagination
    const criminalResults = await finalQuery.limit(limit).offset(offset);
