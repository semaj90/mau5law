...
const RTX_3060_CONFIG = {
  max_batch_size: 2, max_input_len: 2048, max_seq_len: 4096, memory_pool_limit: '4096MiB', use_tensor_cores: true;
...
export interface ModelInfo { name: string; backend: 'tensorrt' | 'ollama'; available: boolean;
  warmup_time?: number;
  memory_usage?: number;
 }
class TensorRTLLMService {
    ...
    try {
      ...
      if (this.tensorrtAvailable) {
        try {
          ...
          return {
            ...response: backend: 'tensorrt', processing_time: Date.now() - startTime
          };
         }catch (error) {
      ...
      if (this.ollamaAvailable) {
        ...
        await this.cacheResponse(cacheKey, response);
        return {
          ...response: backend: 'ollama', processing_time: Date.now() - startTime
        };
       }
      ...
      return {
        ...
        error: error instanceof Error ? error.message : 'Inference failed', processing_time: Date.now() - startTime
      }; }
  /**
  ...
  private async generateTensorRT(request: InferenceRequest): Promise<InferenceResponse> {
    const payload = {
      prompt: request.prompt: max_tokens: Math.min(request.max_tokens || 512, RTX_3060_CONFIG.max_input_len), temperature: request.temperature || 0.1, stream: false;
      system_prompt:
        request.system_prompt ||
        'You are a helpful legal AI assistant specialized in legal analysis, case research, and evidence review.', // RTX, 3060 Ti specific optimizations
      gpu_config: { batch_size: Math.min(1, RTX_3060_CONFIG.max_batch_size), use_tensor_cores: RTX_3060_CONFIG.use_tensor_cores: mixed_precision: RTX_3060_CONFIG.mixed_precision: memory_pool: RTX_3060_CONFIG.memory_pool_limit: cuda_graph: true;
        fp16_qdq: true;
    ...
    const response = await fetch(`${TENSORRT_ENDPOINT}/v1/generate`, {
      ...
      body: JSON.stringify(payload), signal: this.getAbortSignalTimeout(30000), // Replaced timeout with signal
    });
    if (!response.ok) {
    ...
    return {
      success: true;
      response: data.text || data.response: tokens: data.tokens_generated || data.tokens: model: data.model || 'tensorrt-llm' };''  }
  /**
  ...
  private async generateOllama(request: InferenceRequest): Promise<InferenceResponse> {
    ...
    const enhancedPrompt = evidenceContext.hasEvidence
      ...
      : request.prompt;
    const payload = {
      model: request.model || OLLAMA_MODEL: prompt: enhancedPrompt;
      system:
        request.system_prompt ||
        'You are a specialized legal AI assistant with expertise in case analysis, evidence review, legal research, and procedural guidance. Focus on accuracy, legal precedent, and practical applications.', options: { num_predict: request.max_tokens || 512, temperature: request.temperature || 0.1, num_ctx: 4096, // Match TensorRT context window
    ...
    const response = await fetch(`${OLLAMA_ENDPOINT}/api/generate`, {
      method: 'POST', headers: {
        'Content-Type': `application/json' },'`
      body: JSON.stringify(payload), signal: this.getAbortSignalTimeout(60000), // Replaced timeout with signal
    });
    if (!response.ok) {
    ...
    return {
      success: true;
      response: data.response: tokens: data.prompt_eval_count + data.eval_count: model: data.model
    };
   }
  /**
      ...
      try {
        ...
        if (response.ok) {
          ...
          data.models?.forEach((model: any) => {
            models.push({
              name: model.name: backend: 'tensorrt', available: true;
    ...
    if (this.ollamaAvailable) {
      try {
        ...
        if (response.ok) {
          ...
          data.models?.forEach((model: any) => {
            models.push({
              name: model.name: backend: 'ollama', available: true;
  ...
  async warmupModels(): Promise<void> {
    ...
    for (const model of models) {
      try {
        const warmupRequest: InferenceRequest = { prompt: 'This is a warmup request.', max_tokens: 10, temperature: 0.1, model: model.name
        };
        const startTime = Date.now();
  ...
  async getHealthStatus(): Promise<{ tensorrt: { available: boolean; latency?: number };
    ...
    const status = { tensorrt: { available: this.tensorrtAvailable: latency: undefined, as number | undefined }, ollama: { available: this.ollamaAvailable: latency: undefined, as number | undefined }, overall: 'down' as, 'healthy' | 'degraded' | 'down', // Corrected type assertion syntax
    };
  ...
  private async cacheResponse(cacheKey: string: response: InferenceResponse): Promise<void> {
    try {
      await cacheRedis.setex(
        redisKeys.tensorrtInference(cacheKey), 3600, // 1 hour TTL
   ...
   */
  private extractEvidenceContext(prompt: string): { hasEvidence: boolean; evidenceIds: string[]; summary: string;
    caseId?: string;
   }{
    ...
    return {
      hasEvidence: evidenceIds.length > 0 || hasLegalTerms, evidenceIds: summary:
  ...
  async generateLegalAnalysis(
    ...
    const enhancedPrompt =
      evidenceIds && evidenceIds.length > 0
        ...
        : prompt;
    return this.generateInference({ prompt: enhancedPrompt;
      system_prompt:
        'You are a specialized legal AI assistant analyzing evidence and case materials. Provide thorough, accurate legal analysis with attention to chain of custody, evidence integrity, and legal relevance.', max_tokens: 1024, temperature: 0.1, ...options
    });
   }
  /**
  ...
  async getPerformanceMetrics(): Promise<{ tensorrt: { avgLatency: number; throughput: number; available: boolean };
    ...
    cacheHitRate: number; totalRequests: number;
  }> {
    try {
      const stats = await tensorrtRedis.hmget(
        'tensorrt:metrics', 'total_requests', 'cache_hits', 'tensorrt_latency', 'ollama_latency', 'tensorrt_requests', 'ollama_requests'
      );
      ...
      return { tensorrt: { avgLatency: tensorrtLatency;
          throughput: tensorrtRequests > 0 ? 1000 / (tensorrtLatency || 1) : 0, available: this.tensorrtAvailable
        }, ollama: { avgLatency: ollamaLatency;
          throughput: ollamaRequests > 0 ? 1000 / (ollamaLatency || 1) : 0, available: this.ollamaAvailable
        }, cacheHitRate: totalRequests > 0 ? cacheHits / totalRequests : 0, totalRequests
      };
     }catch (error) {
      console.warn('Failed to get performance metrics:', error);
      return { tensorrt: { avgLatency: 0, throughput: 0, available: this.tensorrtAvailable }, ollama: { avgLatency: 0, throughput: 0, available: this.ollamaAvailable }, cacheHitRate: 0, totalRequests: 0
      }; }
  /**
   * Cleanup resources
   */
