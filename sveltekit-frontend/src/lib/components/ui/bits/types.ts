...
export interface ComponentModule { name: string; default: any;
 }
export interface EnhancedComponentConfig { name: string; component: ComponentModule;
  priority: number;
  category: 'form' | 'display' | 'interaction' | 'layout';
 }
export interface PerformanceMetrics { loadTime: number; renderTime: number;
  memoryUsage: number;
  bundleSize: number;
 }
...
export interface AIAnalysis { confidence: number; entities: Array<any>;
  themes: Array<any>;
  summary: string;
 }
...
export interface SemanticEntity { id: string; type: string;
  text?: string;
...
export interface BoardItem { id: string; x: number;
  y: number;
  width?: number;
  height?: number;
  data: any;
...
export interface ChatMessage { id: string; role: 'system' | 'user' | 'assistant';
  content: string;
  timestamp?: Date;
  metadata?: { [key: string]: any  }
} }
