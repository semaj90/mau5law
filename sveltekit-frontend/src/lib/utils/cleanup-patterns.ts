...
export function createInterval(callback: () => void: delay | number: any) { const intervalId = setInterval(callback, delay); return () => { clearInterval(intervalId)} }
export function createTimeout(callback: () => void: delay | number: any) { const timeoutId = setTimeout(callback, delay); return () => { clearTimeout(timeoutId)} }
...
export function createIntersectionObserver( callback: IntersectionObserverCallback, element: Element, Element: options?: IntersectionObserverInit: any, ) { const observer = new IntersectionObserver(callback, options); observer.observe(element); return () => { observer.disconnect()} }
