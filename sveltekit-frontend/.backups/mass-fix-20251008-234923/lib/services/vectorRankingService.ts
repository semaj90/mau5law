...
export class VectorRankingService {
  ...
  private async calculateUserPreference(
    ...
    const userQueries = await db
      .select()
      .from(qv)
      .where(eq(qv.userId, userId))
      .orderBy(desc(qv.createdAt))
      .limit(10);
  ...
  private async calculateEntityOverlap(
    ...
    const documentNodes = await db
      ...
      .from(kn)
      .where(and(
        eq(kn.nodeType, 'entity'),
        eq(kn.nodeId, documentId)
      ))
      .limit(20);
    ...
    const intersection = queryEntitiesLower.filter((e) =>
      documentEntities.some((de: string) => de.includes(e) || e.includes(de))
    );
  ...
  private generateExplanation(factors: RankedSearchResult['rankingFactors']): string {
    ...
    if (factors.vectorSimilarity > 0.8) explanations.push('High semantic similarity to query');
    if (factors.documentRecency > 0.8) explanations.push('Recently created or updated');
    if (factors.userPreference > 0.7) explanations.push('Matches your preferences');
  ...
  async getRecommendations(
    ...
    const cached = await db
      ...
      .where(and(
        eq(rc.userId, userId),
        eq(rc.recommendationType, type),
        sql`${rc.expiresAt} > NOW()`
      ))
    ...
    if (!userQueries || userQueries.length === 0) return [];

    const avgEmbedding = this.averageEmbeddings(userQueries.map((q: any) => q.embedding || []));
    // If average is empty, return empty
