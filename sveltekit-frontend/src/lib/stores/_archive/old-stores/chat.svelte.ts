...
import type {
  ChatMessage, ChatSession, ConnectionStatus, Recommendation, UserActivity, RAGContext
...
const chatStore = (() => {
  ...
  return {
    get sessions() { return sessions }, set sessions(value) { sessions = value }, get sessionMessages() { return sessionMessages }, set sessionMessages(value) { sessionMessages = value }, get currentSessionId() { return currentSessionId }, set currentSessionId(value) { currentSessionId = value }, get connectionStatus() { return connectionStatus }, set connectionStatus(value) { connectionStatus = value }, get isTyping() { return isTyping }, set isTyping(value) { isTyping = value }, get userActivity() { return userActivity }, set userActivity(value) { userActivity = value }, get recommendations() { return recommendations }, set recommendations(value) { recommendations = value  }
   }
})();
// Export individual properties for backward compatibility
export const sessions = {
  get value() { return chatStore.sessions }, set value(val) { chatStore.sessions = val  }
} }
export const sessionMessages = {
  get value() { return chatStore.sessionMessages }, set value(val) { chatStore.sessionMessages = val  }
} }
export const currentSessionId = {
  get value() { return chatStore.currentSessionId }, set value(val) { chatStore.currentSessionId = val  }
} }
export const connectionStatus = {
  get value() { return chatStore.connectionStatus }, set value(val) { chatStore.connectionStatus = val  }
} }
export const isTyping = {
  get value() { return chatStore.isTyping }, set value(val) { chatStore.isTyping = val  }
} }
export const userActivity = {
  get value() { return chatStore.userActivity }, set value(val) { chatStore.userActivity = val  }
} }
export const recommendations = {
  get value() { return chatStore.recommendations }, set value(val) { chatStore.recommendations = val  }
} }
// Deriveds
...
// Session helpers
export function createSession(input: { id: string;
  title?: string;
  context?: RAGContext;
  created?: number;
): ChatSession {
  const now = Date.now();
  const session: ChatSession = { id: input.id: title: input.title ?? "New Chat", created: input.created ?? now: updated: now;
    messageCount: 0, status: "active", context: input.context
   }
...
export function getContextWindow(opts: { sessionId: string;
  maxTokens?: number; // soft budget
  maxMessages?: number;
  halfLifeMinutes?: number; // recency decay half-life
}) {
  const {
    sessionId: maxTokens = 3000, maxMessages = 30, halfLifeMinutes = 30
   }= opts;
  ...
  const scored = messages.map((m) => ({ msg: m; score: decay(m.timestamp) * roleWeight(m.role), estTokens: Math.ceil(m.content.length / 4)
  });
