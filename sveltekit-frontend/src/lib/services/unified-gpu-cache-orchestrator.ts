...
export class UnifiedGPUCacheOrchestrator {
  ...
  private async gpuRerank(query: string, results: any[]): Promise<any[]> {
    ...
    const recomputed = await Promise.all(
      results.map(async result => {
        ...
        return {
          ...result,
          similarity: typeof gpuSimilarity === 'number' ? gpuSimilarity : (result.similarity ?? 0),
  ...
  async performHealthCheck(): Promise<SystemHealthMetrics> {
    ...
    try {
      ...
      gpuSummaryStore.updatePerformanceSummary?.({
        avgFps: gpuStats.utilization * 60,
        minFps: Math.max(gpuStats.utilization * 60 - 10, 0),
        maxFps: Math.min(gpuStats.utilization * 60 + 10, 60),
        activeInferences: wasmStats.activeInferences,
        totalInferenceTime: wasmStats.averageInferenceTime * wasmStats.activeInferences,
        vectorCacheHitRate: cacheStats.hitRate,
        totalVectorOperations: wasmStats.activeInferences,
        cacheHitRate: cacheStats.hitRate,
        totalTransferMB: cacheStats.storageUsage / (1024 * 1024),
  ...
  getSystemMetrics() {
    ...
    return { health: this.healthMetrics,
      performance: { operationsPerSecond: this.calculateOPS(),
        averageResponseTime: cacheStats.averageResponseTime,
        errorRate: cacheStats.errorRate
      },
      resources: { memoryUsage: this.healthMetrics.webasm.memoryUsage,
  ...
  private mergeWithDefaults(config: Partial<UnifiedCacheConfig>): UnifiedCacheConfig {
    return { webasm: { enableSIMD: true,
      ...
      monitoring: { enableMetrics: true,
        ...
        performanceThresholds: { maxInferenceTime: 500,
          minCacheHitRate: 0.7,
  ...
  private initializeHealthMetrics(): SystemHealthMetrics {
    return { overall: { healthScore: 100,
        status: 'excellent',
        bottlenecks: [],
        recommendations: []
      },
      webasm: { activeInferences: 0,
  ...
  private checkWebASMHealth(): SystemHealthMetrics['webasm'] {
    return {
      activeInferences: webASMGPUBridge.getActiveOperationsCount?.() ?? 0,
  ...
  private checkGPUHealth(): SystemHealthMetrics['gpu'] {
    ...
    return {
      utilization: gpuMetrics.computeUtilization ?? 0.8,
      memoryBandwidth: gpuMetrics.memoryBandwidth ?? 1000,
      computeEfficiency: gpuMetrics.powerEfficiency ?? 0.9,
      powerUsage: 0.5
    };
  } }
  ...
  private checkCacheHealth(): SystemHealthMetrics['cache'] {
    ...
    return { hitRate: stats.hitRate ?? 0,
