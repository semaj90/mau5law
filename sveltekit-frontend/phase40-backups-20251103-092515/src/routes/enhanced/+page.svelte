    ...
    try {
      ...
      while (true) {
        ...
        while ((idx = buffer.indexOf('\n\n')) !== -1) {
          ...
          if (!raw) continue; // removed unused lines assignment let event = 'message'; let dataStr = ''; for (const line of lines) { if (line.startsWith('event:')) event = line.slice(6).trim(); else if (line.startsWith('data:')) dataStr += line.slice(5).trim(); }
          if (dataStr) {
            try {
