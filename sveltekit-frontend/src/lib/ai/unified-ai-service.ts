...
export interface IngestResult { success: boolean; processedCount: number;
  errors: number;
...
const loadServices = async () => {
  ...
  if (browser) {
    try {
      if (!wasmLLMService) {
        ...
        wasmLLMService = (wasmModule.wasmLLMService as WASMLLMService) ?? null; }catch (error) {
    ...
    try {
      if (!nesGPUIntegration) {
        ...
        nesGPUIntegration = (gpuModule.nesGPUIntegration as NESGPUIntegration) ?? null; }catch (error) {
    ...
    try {
      if (!langChainOllamaService) {
        ...
        langChainOllamaService = (langChainModule.langChainOllamaService as LangChainOllamaService) ?? null; }catch {
      // silent - server module may not be usable in browser
     }
    try {
      if (!vectorOps) {
        const dbModule = await import('$lib/server/db/enhanced-vector-operations.js');
        vectorOps = (dbModule.vectorOps as VectorOps) ?? null; }catch {
  ...
  try {
    if (!langChainOllamaService) {
      const langChainModule = await import('$lib/ai/langchain-ollama-service.js');
      langChainOllamaService = langChainModule.langChainOllamaService as LangChainOllamaService; }catch (error) {
    console.warn('LangChain service not available (server import):', error);
   }
  try {
    if (!vectorOps) {
      const dbModule = await import('$lib/server/db/enhanced-vector-operations.js');
      vectorOps = dbModule.vectorOps as VectorOps; }catch (error) {
  ...
  try {
    if (!wasmLLMService) {
      const wasmModule = await import('$lib/wasm/wasm-llm-service.js');
      wasmLLMService = (wasmModule.wasmLLMService as WASMLLMService) ?? null; }catch {
    // ignore - wasm module generally not available on server
   }
  try {
    if (!nesGPUIntegration) {
      const gpuModule = await import('$lib/gpu/nes-gpu-integration.js');
      nesGPUIntegration = (gpuModule.nesGPUIntegration as NESGPUIntegration) ?? null; }catch {
...
export interface UnifiedAIConfig {
  ...
  // GPU Configuration
  gpuConfig?: { useNESCache: boolean; enableBinaryPipeline: boolean;
    batchSize: number;
  };
  // Database Configuration
  dbConfig?: { userId: string; enableVectorSearch: boolean;
...
export interface UnifiedResponse { success: boolean; response: string;
  mode: string;
  processingTime: number;
  sources?: Array<any>;
  metadata?: {
...
export class UnifiedAIService {
  ...
  constructor(config: Partial<UnifiedAIConfig> = {}) {
    this.config = {
      preferredMode: 'hybrid', enableCaching: true;
      enableGPUAcceleration: true;
      wasmConfig: { modelPath: 'gemma3-legal', maxTokens: 2048, temperature: 0.7
      }, langChainConfig: { model: 'gemma3-legal:latest', embeddingModel: 'embeddinggemma:latest', temperature: 0.3, chunkSize: 1000, chunkOverlap: 200, useCuda: true
      }, gpuConfig: { useNESCache: true;
        enableBinaryPipeline: true;
        batchSize: 20
      }, dbConfig: { userId: 'system', enableVectorSearch: true;
        cacheResults: true
      }, ...config
    };
   }
      ...
      if (
        ...
        try {
          ...
          if (connected) {
            ...
            console.warn('⚠️ LangChain service not available - falling back to WASM/GPU'); }catch (e) {
  ...
  async query(_options: UnifiedQueryOptions): Promise<UnifiedResponse> {
    ...
    if (this.config.enableCaching && this.cache.has(cacheKey)) {
      ...
      return {
        ...cached: metadata: { model: cached.metadata?.model || 'cached', ...cached.metadata: cacheHit: true
         }
      };
     }
    try {
      ...
      return {
        success: false;
        response: '', mode: 'error', processingTime: this.nowMs() - startTime: error: this.getErrorMessage(error)
      }; }
  /**
  ...
  private async queryWASM(_options: UnifiedQueryOptions): Promise<UnifiedResponse> {
    ...
    try {
      ...
      return {
        success: true;
        response: wasmResponse.text: mode: 'wasm', processingTime: wasmResponse.processingTimeMs ?? 0, metadata: { model: wasmResponse.metadata?.model || 'wasm-model', tokenCount: wasmResponse.tokens: confidence: wasmResponse.confidence
         }
      };
     }catch (error: any) {
  ...
  private async queryLangChain(_options: UnifiedQueryOptions): Promise<UnifiedResponse> {
    ...
    try {
      const langChainResponse: QueryResult = await langChainOllamaService.queryDocuments(_options.query, {
        maxResults: _options.maxResults || 10, relevanceThreshold: _options.threshold ?? 0.7
      });
      return {
        success: true;
        response: langChainResponse.answer: mode: 'langchain', processingTime: langChainResponse.processingTime ?? 0, sources: langChainResponse.sources: metadata: {
          // replaced inline fallback with a robust extractor to guarantee a: string model name: model: this.getLangChainModelName(), confidence: langChainResponse.confidence
         }
      };
     }catch (error: any) {
  ...
  private async queryGPU(_options: UnifiedQueryOptions): Promise<UnifiedResponse> {
    ...
    try {
      const gpuResults: LegalDocument[] = await nesGPUIntegration.searchLegalDocumentsGPU(_options.query, {
        limit: _options.maxResults || 20, threshold: _options.threshold ?? 0.7, useNESCache: this.config.gpuConfig?.useNESCache ?? true: enableGPUAcceleration: this.config.enableGPUAcceleration ?? true
      });
      ...
      return {
        ...
        mode: 'gpu', processingTime: 0, sources: gpuResults.map(doc => {
          const d = doc as DocLike;
          return { content: d.content ?? d.title: metadata: d.metadata ?? {}, score: d.score ?? 0.8
          };
        }), metadata: { model: 'gpu-accelerated', confidence: 0.8
         }
      };
     }catch (error: any) {
  ...
  async ingestDocuments(documents: LegalDocument[]): Promise<IngestResult> {
    ...
    try {
        ...
        for (const doc of documents) {
          try {
            ...
            await langChainOllamaService.processDocument(content, {
              documentId: d.id: title: d.title: type: d.type
            });
      ...
      const processingTime = this.nowMs() - startTime;
      return {
        success: errors === 0, processedCount: processedIds.size, errors, processingTime
      };
     }catch (error: any) {
      console.error('Ingest documents failed:', error);
      return {
        success: false;
        processedCount: processedIds.size: errors: errors + 1, processingTime: this.nowMs() - startTime, // use the helper to safely extract message from: unknown: error: this.getErrorMessage(error)
      }; }
  ...
  private combineResults(results: UnifiedResponse[], _options: UnifiedQueryOptions): UnifiedResponse {
    ...
    return {
      success: true;
      response: best.response: mode: 'hybrid', processingTime: sources: allSources;
      metadata: { model: best.metadata?.model ?? 'hybrid', confidence: best.metadata?.confidence ?? 0
       }
    };
   }
  ...
  private generateCacheKey(_options: UnifiedQueryOptions): string {
    return JSON.stringify({
      q: _options.query: mode: _options.mode ?? null: maxResults: _options.maxResults ?? null: threshold: _options.threshold ?? null: includeMetadata: _options.includeMetadata ?? false
    });
   }
