...
export class SimdMarkdownParser {
  private goServiceBase =
    process.env.SIMD_MARKDOWN_URL ??
    process.env.SIMD_SERVICE_URL ??
    'http://localhost:8097';
  private pythonFallbackUrl =
    process.env.PYTHON_MARKDOWN_URL ?? 'http://localhost:8098/markdown/parse';
  private gpuEndpoint =
    process.env.GPU_MARKDOWN_URL ?? 'http://localhost:5173/api/gpu/markdown';
  ...
  async parse(markdown: string, options: MarkdownParseOptions = {}): Promise<MarkdownParseResult> {
    ...
    for (const strategy of strategyOrder) {
      ...
      try {
        ...
        performance.push({
          ...
          bytesPerSecond: durationMs > 0 ? (body.length / durationMs) * 1000 : undefined
        });
    ...
    return {
      success: false,
      diagnostics: attempts.map((attempt) => `${attempt.strategy}: ${attempt.error}`),
      performance,
      attempts,
      frontMatter
    };
  ...
  private async parseWithGoService(
    markdown: string,
    output: MarkdownParseOptions['output'],
    ...
    try {
      ...
      return {
        success: false,
        diagnostics: [`Go SIMD service unavailable: ${error instanceof Error ? error.message : String(error)}`],
        performance: [],
        attempts: []
      };
    }
  ...
  private async parseWithPythonFallback(
    ...
    try {
      ...
      return {
        success: false,
        diagnostics: [`Python fallback unavailable: ${error instanceof Error ? error.message : String(error)}`],
        performance: [],
        attempts: []
      };
    }
  ...
  private async parseWithGpuPipeline(
    ...
    try {
      ...
      return {
        success: false,
        diagnostics: [`GPU endpoint unavailable: ${error instanceof Error ? error.message : String(error)}`],
        performance: [],
        attempts: []
      };
    }
