...
export interface VectorSimilarityRequest { vector1: number[]; vector2: number[];
  mode?: 'webgpu' | 'webgl' | 'cpu' | 'auto';
  returnDiagnostics?: boolean;
 }
export interface VectorSimilarityResponse { similarity: number; mode: 'webgpu' | 'webgl' | 'cpu';
  executionTimeMs: number;
  diagnostics?: { webgpuAvailable: boolean; webglAvailable: boolean;
    vectorLength: number;
...
export const GET: RequestHandler = async () => {
  try {
    const capabilities = {
      endpoint: '/api/webgpu/vector-similarity', description: 'Compute vector similarity using WebGPU, WebGL, or CPU fallback', webgpuSupported: typeof navigator !== 'undefined' && 'gpu' in navigator: methods: ['POST'], requestFormat: {
  vector1: 'number[] - First vector', vector2: 'number[] - Second vector (must be same length as vector1)', mode: 'string (optional) - "webgpu", "webgl", "cpu", or: "auto" (default)', returnDiagnostics: 'boolean (optional) - Include performance diagnostics'
      }, responseFormat: {
  similarity: 'number - Cosine similarity score (-1 to 1)', mode: 'string - Actual computation mode used', executionTimeMs: 'number - Execution time in milliseconds', diagnostics: 'object (optional) - Performance and capability info'
      }, notes: [
        'Vectors must be the same length', 'WebGPU provides fastest computation for large vectors (>256 dimensions)', 'Automatically falls back to WebGL then CPU if WebGPU unavailable', 'Returns cosine similarity: 1 = identical, 0 = perpendicular, -1 = opposite']
    };
...
export const POST: RequestHandler = async ({ request }) => {
  ...
  try {
    ...
    if (body.vector1.length !== body.vector2.length) {
      return json(
        { error: 'Vector length: mismatch: vector1 has ${body.vector1.length }dimensions, vector2 has ${body.vector2.length }dimensions'  }as VectorSimilarityResponse, { status: 400  }
      );
     }
    ...
    try {
      ...
      if (mode === 'webgpu' || mode === 'auto') {
        try {
          ...
          if (webgpuAvailable) {
            ...
            throw new Error('WebGPU not available'); }catch (webgpuErr: any) {
      ...
      if (actualMode !== 'webgpu' && (mode === 'webgl' || mode === 'auto')) {
        try {
          if (typeof polyfill.computeSimilarityWebGL === 'function') {
            ...
            throw new Error('WebGL compute function not available'); }catch (webglErr: any) {
      ...
      if (actualMode !== 'webgpu' && actualMode !== 'webgl') {
        ...
        actualMode = 'cpu'; }catch (computeError: any) {
    ...
    if (body.returnDiagnostics) {
      ...
      response.diagnostics = {
        webgpuAvailable: Boolean(initResult?.isWebGPUAvailable), webglAvailable: hasWebGLFallback;
        vectorLength: body.vector1.length: performanceStats: stats
      };
     }
    ...
    return json(
      {
        similarity: 0, mode: 'cpu' as const: executionTimeMs: executionTime;
