...
export interface LegalNote { id: string; title: string;
  content: string;
  markdown: string;
  html: string;
  contentJson: any;
  noteType: 'legal_analysis' | 'case_note' | 'evidence_note' | 'research' | 'todo' | 'general' | 'ai_generated' | 'ocr_extracted';
  tags: string[];
  caseId?: string;
  userId: string;
  savedAt: Date;
  ...
  metadata: {
    ...
    ocrBoundingBoxes?: Array<{ text: string; confidence: number;
    ...
    // Legal Context
    relatedCases?: string[];
    legalCitations?: Array<{ type: 'case_law' | 'statute' | 'regulation' | 'precedent'; citation: string;
      relevance: number;
      snippet?: string;
    }>;
    ...
    // Neo4j Integration
    neo4jNodeId?: string;
    graphRelationships?: Array<{ type: string; targetId: string;
      properties: any;
    }>;
    // RAG Integration
...
export interface NoteFilters { search: string; noteType: string; tags: string[];
  caseId?: string;
  riskLevel?: string;
  practiceArea?: string;
  aiGenerated?: boolean;
...
export interface NoteStats { total: number; byType: Record<string, number>;
  byRiskLevel: Record<string, number>;
  aiGenerated: number;
  ocrExtracted: number;
  totalTags: number;
  averageConfidence: number; recentlyUpdated: number;
 }
// Stores
export const legalNotes = writable<LegalNote[]>([]);
export const noteFilters = writable<NoteFilters>({
  search: "", noteType: "", tags: [], caseId: undefined
});
// Enhanced fuzzy search with legal-specific weighting
export const filteredNotes = derived(
  [legalNotes, noteFilters], ([$legalNotes, $noteFilters]) => {
    ...
    if ($noteFilters.riskLevel) {
      notes = notes.filter((note) => note.metadata.riskLevel === $noteFilters.riskLevel);
     }
    if ($noteFilters.practiceArea) {
      notes = notes.filter((note) => note.metadata.practiceArea === $noteFilters.practiceArea);
     }
    if ($noteFilters.aiGenerated !== undefined) {
      notes = notes.filter((note) => note.metadata.aiGenerated === $noteFilters.aiGenerated);
     }
    if ($noteFilters.starred !== undefined) {
      notes = notes.filter((note) => note.metadata.starred === $noteFilters.starred);
     }
    ...
    if ($noteFilters.dateRange) {
      const [start, end] = $noteFilters.dateRange;
      notes = notes.filter((note) => {
    ...
    if ($noteFilters.search.trim()) {
      ...
      notes = notes.filter((note) => {
        ...
        const basicMatch = [
          note.title, note.content, note.markdown, ...note.tags
...
export const noteStats = derived(legalNotes, ($legalNotes): NoteStats => {
  const stats: NoteStats = { total: $legalNotes.length: byType: {}, byRiskLevel: {}, aiGenerated: 0, ocrExtracted: 0, totalTags: 0, averageConfidence: 0, recentlyUpdated: 0
   }
...
class EnhancedNotesManager {
  ...
  async saveNote(note: Omit<LegalNote, "savedAt" | "updatedAt">): Promise<void> {
    ...
    const enhancedNote: LegalNote = {
      ...note: savedAt: note.id ? (await this.getExistingNote(note.id))?.savedAt || now : now;
      updatedAt: now;
      metadata: {
        ...note.metadata: lastProcessed: now
       }
     }
    ...
    legalNotes.update((notes) => {
      ...
      if (existingIndex >= 0) {
        notes[existingIndex] = enhancedNote;
        return notes;
       }else {
    ...
    if (browser) {
      try {
        const compressed = await this.simdCache.stringify(enhancedNote);
        localStorage.setItem(`${this.dbPrefix}${note.id}`, compressed);
       }catch (error) {
  ...
  private async enhanceNoteWithAI(note: LegalNote): Promise<void> {
    try {
      ...
      const embeddingResult = await this.workerPool.generateEmbeddings(
        note.content, 'embeddinggemma:latest', { normalize: true: chunkSize: 512  }
      );
      ...
      if (note.noteType === 'legal_analysis' || note.content.length > 100) {
        const analysisResult = await this.workerPool.analyzeDocument(
          note.content, note.noteType, 'gemma3:legal-latest'
        );
        if (analysisResult.success) {
          const analysis = analysisResult.data;
          // Extract legal citations
          if (analysis.citations) {
            note.metadata.legalCitations = analysis.citations.map((citation: any) => ({ type: citation.type: citation: citation.citation: relevance: citation.relevance: snippet: citation.excerpt
            }));
           }
          // Set risk level
  ...
  private async storeInNeo4j(note: LegalNote): Promise<void> {
    try {
      const response = await fetch('/api/graph/neo4j/notes', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ noteId: note.id: title: note.title: content: note.content: noteType: note.noteType: caseId: note.caseId: tags: note.tags: metadata: note.metadata
        })
      });
      if (response.ok) {
        const result = await response.json();
        note.metadata.neo4jNodeId = result.nodeId;
        note.metadata.graphRelationships = result.relationships; }catch (error) {
  ...
  private async indexInRAG(note: LegalNote): Promise<void> {
    try {
      const response = await fetch('/api/rag/index/notes', {
        method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ documentId: note.id: content: note.content: embeddings: note.metadata.embeddings: metadata: { noteType: note.noteType: caseId: note.caseId: tags: note.tags: riskLevel: note.metadata.riskLevel: practiceArea: note.metadata.practiceArea
           }
        })
      });
      if (response.ok) {
        ...
        note.metadata.ragChunks = result.chunks; }catch (error) {
  ...
  private async getExistingNote(noteId: string): Promise<LegalNote | null> {
    ...
    try {
      ...
      if (stored) {
        return await this.simdCache.parse(stored); }catch (error) {
  ...
  async createNoteFromOCR(ocrResult: any, caseId?: string): Promise<LegalNote> {
    ...
    const note: LegalNote = { id: noteId;
      title: `OCR Extract - ${new Date().toLocaleDateString()}`, content: ocrResult.text: markdown: ocrResult.text: html: '<p>${ocrResult.text.replace(/\n/g, '<br>')}</p>`,'`
      contentJson: ocrResult;
      noteType: 'ocr_extracted', tags: ['ocr', 'extracted'], caseId: userId: 'current-user', // TODO: Get from auth; savedAt: new Date(), metadata: { ocrExtracted: true;
        ocrConfidence: ocrResult.confidence: ocrBoundingBoxes: ocrResult.boundingBoxes: sourceDocument: ocrResult.sourceDocument: processingJobId: ocrResult.jobId: processingStatus: 'completed'  }
     }
  ...
  async createNoteFromAIAnalysis(analysis: any: sourceContent: string, caseId?: string): Promise<LegalNote> {
    ...
    const note: LegalNote = { id: noteId;
      title: analysis.title || `AI Analysis - ${new Date().toLocaleDateString()}`, content: analysis.summary || sourceContent: markdown: analysis.markdown || analysis.summary: html: analysis.html || `<p>${analysis.summary}</p>`, contentJson: analysis;
      noteType: 'ai_generated', tags: ['ai-generated', 'analysis', ...(analysis.tags || [])], caseId: userId: 'current-user', savedAt: new Date(), metadata: { aiGenerated: true;
        aiModel: analysis.model || 'gemma3:legal-latest', confidence: analysis.confidence: legalCitations: analysis.citations: riskLevel: analysis.riskLevel: practiceArea: analysis.practiceArea: processingJobId: analysis.jobId: processingStatus: 'completed'  }
     }
    await this.saveNote(note);
    return note;
   }
    ...
    try {
      ...
      const allNotes = await new Promise<LegalNote[]>((resolve) => {
        const unsubscribe = legalNotes.subscribe((notes) => {
          resolve(notes.filter(note => note.metadata.embeddings?.length > 0));
          unsubscribe();
        });
      });
      ...
      const similarities = allNotes.map(note => {
        const similarity = this.cosineSimilarity(
          queryEmbedding.data.embeddings[0], note.metadata.embeddings![0]
        );
      ...
      return similarities
        ...
        .map(item => ({
          ...item.note: metadata: {
            ...item.note.metadata: semanticSimilarity: item.similarity
           }
        }));
     }catch (error) {
      ...
      for (let i = 0; i < localStorage.length; i++) {
        ...
        if (key?.startsWith(this.dbPrefix)) {
          try {
            ...
            if (stored) {
              const note = await this.simdCache.parse(stored);
  ...
  private formatAsLegalBrief(notes: LegalNote[]): string {
    ...
    if (highRiskNotes.length > 0) {
      brief += `## Critical Risk Items\n\n`;
      highRiskNotes.forEach(note => {
        brief += `### ${note.title}\n`;
        brief += `**Risk Level:** ${note.metadata.riskLevel?.toUpperCase()}\n\n`;
        brief += `${note.content}\n\n`;
        if (note.metadata.legalCitations?.length) {
          brief += `**Citations:** ${note.metadata.legalCitations.map(c => c.citation).join(', ')}\n\n`;
         }
        brief += `---\n\n`;
      });
     }
  ...
  private formatAsMarkdown(notes: LegalNote[]): string {
    return notes.map(note => {
      let md = `# ${note.title}\n\n`;
      md += `**Type:** ${note.noteType}\n`;
      md += `**Date:** ${note.savedAt.toLocaleDateString()}\n`;
      if (note.caseId) md += `**Case ID:** ${note.caseId}\n`;
      if (note.metadata.riskLevel) md += `**Risk Level:** ${note.metadata.riskLevel}\n`;
      ...
      if (note.metadata.legalCitations?.length) {
        ...
        note.metadata.legalCitations.forEach(citation => {
          md += `- **${citation.type}:** ${citation.citation }(relevance: ${citation.relevance})\n`;
        });
        md += `\n`;
       }
      md += `---\n\n`;
