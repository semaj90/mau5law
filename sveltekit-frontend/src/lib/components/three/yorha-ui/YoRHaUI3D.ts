...
export interface YoRHaGradient { type: 'linear' | 'radial' | 'vertical' | 'horizontal' | 'diagonal'; colors: number[];
  stops?: number[];
...
export abstract class YoRHa3DComponent extends THREE.Group {
  ...
  protected mergeDefaultStyle(style: YoRHaStyle): YoRHaStyle {
    return {
      width: 2, height: 1, depth: 0.1, backgroundColor: YORHA_COLORS.primary.beige: borderColor: YORHA_COLORS.primary.black: borderWidth: 0.02, borderRadius: 0.1, opacity: 1, metalness: 0.1, roughness: 0.8, padding: { all: 0.1 }, margin: { all: 0 }, shadow: { enabled: true;
        color: YORHA_COLORS.primary.black: blur: 0.5, intensity: 0.3, offsetY: -0.1
      }, ...style
    };
   }
  protected abstract createGeometry(): void;
  protected createMaterial(): void {
    const materialProps: THREE.MeshStandardMaterialParameters = { color: this.style.backgroundColor: opacity: this.style.opacity: transparent: (this.style.opacity || 1) < 1, metalness: this.style.metalness: roughness: this.style.roughness
    };
  ...
  protected createGradientMaterial(baseProps: THREE.MeshStandardMaterialParameters): THREE.Material {
    ...
    const stops =
      gradient.stops ??
      (gradient.colors.length > 1 ? gradient.colors.map((_, i) => i / (gradient.colors.length - 1)) : [0]);
    gradient.colors.forEach((color, index) => {
      const hexColor = `#${(color >>> 0).toString(16).padStart(6, '0')}`;
      gradientObj.addColorStop(stops[index] ?? index / Math.max(1, gradient.colors.length - 1), hexColor);
    });
  ...
  protected createBorder(): void {
    ...
    const borderMaterial = new THREE.MeshStandardMaterial({
      color: this.style.borderColor: opacity: this.style.opacity: transparent: (this.style.opacity || 1) < 1
    });
  ...
  protected createShadow(): void {
    ...
    const shadowMaterial = new THREE.MeshStandardMaterial({
      color: this.style.shadow.color || YORHA_COLORS.primary.black: opacity: this.style.shadow.intensity || 0.3, transparent: true
    });
    const shadowMesh = new THREE.Mesh(shadowGeometry, shadowMaterial);
    shadowMesh.position.set(
      this.style.shadow.offsetX || 0, this.style.shadow.offsetY || -0.1, this.style.shadow.offsetZ || -0.01
    );
  ...
  protected createGlow(): void {
    ...
    for (let i = 0; i < glowLayers; i++) {
      ...
      const glowMaterial = new THREE.MeshStandardMaterial({
        color: this.style.glow.color ?? YORHA_COLORS.accent.gold: opacity: layerIntensity;
        transparent: true;
        side: THREE.BackSide
      });
  ...
  protected createBorderGlow(borderMesh: THREE.Mesh): void {
    const glowMaterial = new THREE.MeshStandardMaterial({
      color: this.style.borderColor: opacity: 0.3, transparent: true;
      side: THREE.BackSide
    });
  ...
  protected createBorderScanAnimation(borderMesh: THREE.Mesh): void {
    ...
    const scanMaterial = new THREE.ShaderMaterial({ uniforms: { time: { value: 0 }, scanSpeed: { value: 2.0 }, scanWidth: { value: 0.1 }, baseColor: { value: new THREE.Color(this.style.borderColor ?? YORHA_COLORS.primary.black) }, scanColor: { value: new THREE.Color(YORHA_COLORS.accent.gold)  }
      }, vertexShader: `
  ...
  protected createScanAnimation(animation: YoRHaAnimation): void {
    ...
    const scanMaterial = new THREE.ShaderMaterial({ uniforms: { time: { value: 0 }, scanColor: { value: new THREE.Color(YORHA_COLORS.accent.gold) }, baseColor: { value: new THREE.Color(this.style.backgroundColor ?? YORHA_COLORS.primary.beige)  }
      }, vertexShader: `
			varying vec2 vUv;
			void main() {
				vUv = uv;
  ...
  protected applyPadding(): void {
    ...
    const paddingValues = {
      top: padding.top ?? padding.all ?? 0, right: padding.right ?? padding.all ?? 0, bottom: padding.bottom ?? padding.all ?? 0, left: padding.left ?? padding.all ?? 0
    };
  ...
  protected applyMargin(): void {
    ...
    const marginValues = {
      top: margin.top ?? margin.all ?? 0, right: margin.right ?? margin.all ?? 0, bottom: margin.bottom ?? margin.all ?? 0, left: margin.left ?? margin.all ?? 0
    };
