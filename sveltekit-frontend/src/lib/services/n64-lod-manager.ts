...
export class N64LODManager {
  ...
  private readonly MEMORY_BUDGET = {
    CHR_ROM_SIZE: 819200, // Process 800KB for web (scaled up from NES 8KB)
    PATTERN_TABLE_SIZE: 4096, // 4KB pattern table
    MAX_ACTIVE_TEXTURES: 64, // Maximum textures in CHR-ROM
    BANK_SWITCH_THRESHOLD: 614400, // Switch banks at 75% capacity
  };
   ...
   */
  calculateLOD(distance: number = 100, zoomLevel: number = 1, scrollSpeed: number = 0): number {
  ...
  async generateMipmaps(
    ...
    if (baseTexture instanceof HTMLImageElement) {
  ...
  async streamTexture(
    ...
    // Cancel unknown existing stream for this asset
   ...
   */
  private convertToCHRROM(imageData: ImageData, _lodLevel: number): ArrayBuffer {
        ...
        for (let y = 0; y < tileHeight; y++) {
          ...
          for (let x = 0; x < tileWidth; x++) {
            ...
            if (pixelX < width && pixelY < height) {
              ...
              // Corrected logic for NES bitplanes
              // Bit 0 of color goes to Plane 0
              // Bit 1 of color goes to Plane 1
              if (intensity & 1) {
                plane0Byte |= 1 << (7 - x);
              }
              if (intensity & 2) {
   ...
   */
  private async storeInCHRROM(cacheKey: string, textureData: ArrayBuffer): Promise<void> {
    // Check if we need to free space
    while (
      this.currentMemoryUsage + textureData.byteLength >
      this.MEMORY_BUDGET.CHR_ROM_SIZE
    ) {
  ...
  private async performBankSwitch(): Promise<void> {
    ...
    const keysToEvict = Array.from(this.textureCache.keys()).filter((key) => {
      const metadata = this.getAccessMetadata(key) as AccessMetadata | null;
  ...
  private async simulateCartridgeLoad(dataSize: number): Promise<void> {
    ...
    const baseDelay = 16; // ~1 frame at 60fps
  ...
  private storeAccessMetadata(key: string, metadata: AccessMetadata): void {
    if (typeof localStorage !== 'undefined') {
      localStorage.setItem(`chr_rom_meta:${key}`, JSON.stringify(metadata));
    }
  }

  private getAccessMetadata(key: string): unknown {
    if (typeof localStorage !== 'undefined') {
