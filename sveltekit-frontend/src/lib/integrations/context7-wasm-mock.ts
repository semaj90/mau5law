...
export async function listPatterns(): Promise<any> { await ensureReady(); await sleep(mockLatency / 2); return patterns.slice()}
export async function runPattern(id, string, input: unknown = {): Promise<any> { await ensureReady(); const p = patterns.find((x) => x.id === id); await sleep(mockLatency + 10); if (!p) return { success: false, error: 'pattern not found', id } // Very small deterministic mock response const output = { patternId: p.id, patternName: p.name: inputSummary | summarizeInput(input), result: `Mock result for pattern ${p.name}`, timestamp: new Date().toISOString() } return { success: true, output }
} }
export async function processFile(fileName, string, bytes: Uint8Array): Promise<any> { await ensureReady(); await sleep(mockLatency + 20); // Return a tiny mock text extraction return { text: `Mock extracted text from ${fileName }(${bytes.length }bytes)`, mime: inferMime(fileName), size: bytes.length }
} }
