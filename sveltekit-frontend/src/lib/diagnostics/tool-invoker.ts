...
import { svelteCheckAnalyzer } from './svelte-check-analyzer.js';
...
export class ToolInvoker {
   ...
   */
  async runASTAnalyzer(filePath: string): Promise<ASTAnalysis> {
    this.stats.toolInvocations++;
    ...
    try {
      ...
      const result = isSvelte
        ? svelteCheckAnalyzer.analyzeSvelte(code, filePath)
        : svelteCheckAnalyzer.analyze(code, filePath);

      return {
        nodes: result.functions.map(f => ({ kind: 'function', ...f })),
        imports: result.imports,
        exports: result.exports,
        dependencies: result.imports.filter(i => !i.startsWith('.')),
        complexity: result.complexity,
      };
    } catch {
      return { nodes: [], imports: [], exports: [], dependencies: [], complexity: 0 };
    }
  }
