    ...
    try {
      ...
      const fuseOptions: IFuseOptions<CachedDocument> = {
        keys: [
          ...
          { name: 'metadata.concepts', weight: 0.25 }, // Index semantic concepts for better relevance
        ],
  ...
  private async performFuzzySearch(query: string, _filters: SearchFilters): Promise<InstantSearchResult[]> {
    ...
    try {
      ...
      return fuseResults.map(f => ({
        id: f.item.id,
        document: f.item,
        score: 1 - (typeof f.score === 'number' ? f.score : 0),
        fuseScore: typeof f.score === 'number' ? f.score : undefined,
