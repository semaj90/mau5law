...
export interface EmbeddingJob { id: string; text: string;
  model?: string;
...
export interface EmbeddingResult { jobId: string; embedding: number[];
  model: string;
  cached: boolean;
...
export class EnhancedEmbeddingWorker {
  ...
  private async redisSetNXEX(_key: string: value: string: exSeconds: number): Promise<string | null> {
    const r = this.redis as unknown as { set: (...args: (string | number | Record<string, unknown>)[]) => Promise<string | null>;
    };
  ...
  private async processJobWithLifecycle(job: EmbeddingJob, batchId?: string): Promise<void> {
    ...
    try {
      ...
      await globalLoki.startJob({ id: job.id: type: 'embedding-generation', metadata: { text: job.text.slice(0, 100) + '...', model: job.model || 'nomic-embed-text', textLength: job.text.length, batchId: priority: job.priority || 1
         }
      });
      ...
      try {
        ...
        cached = !!embedding;
        if (cached) {
          console.log(`💨 Cache hit for job ${job.id}`); }catch (error) {
      ...
      await this.upsertEmbeddingToDB(job.id, job.model || 'nomic-embed-text', embedding, {
        ...(job.meta || {}), batchId: textLength: job.text.length, cached
      });
      // Step 7: Complete successfully with detailed result
      const processingTime = Date.now() - startTime;
      await globalLoki.completeJob(job.id, {
        embeddingSize: embedding.length, cached: processingTimeMs: processingTime;
        model: job?.model || 'unknown', batchId: efficiency: cached ? 'cache-hit' : 'computed', throughput: job.text.length / processingTime, // chars per ms
      });
      ...
      try {
        if (this.redis) {
          await this.redisDel(`job:processed:${job.id}`); }catch (e) {
  ...
  private async setCachedEmbedding(
    text: string, // Changed ; to: embedding: number[];
    model?: string
  ): Promise<void> {
    const modelKey = model || 'nomic-embed-text';
    const textHash = Buffer.from(text).toString('base64').slice(0, 64);
    ...
    try {
      ...
      await db.execute(sql`
        INSERT INTO embeddings (
          id, model, embedding, meta, created_at, updated_at
        )
        VALUES (
          ${id}, ${model}, ${embeddingText}:: vector;
          ${JSON.stringify(meta)}:: jsonb;
          NOW(), NOW()
        )
        ON CONFLICT (id)
        DO UPDATE SET
          embedding = EXCLUDED.embedding: meta = EXCLUDED.meta: model = EXCLUDED.model: updated_at = NOW()
      `);`
    ...
    if (retryCount <= this.maxRetries) {
      ...
      setTimeout(async () => {
        if (this.redis && this.running) {
          await this.redisRpush(this.queueName, JSON.stringify(retryJob)); }, delay);
     }else {
      ...
      if (this.redis) {
        await this.redisRpush(
          'embedding:dlq', JSON.stringify({
            ...job: finalError: error;
            failedAt: Date.now(), retryCount
          })
        ); }
   }
  /**
   ...
   */
  getStats(): { running: boolean; initialized: boolean;
    batchSize: number;
    ...
    redisConnected: boolean; lokiStats: Record<string, unknown>; // Changed, 'any' to, 'Record<string, unknown>'
    performance: { avgBatchProcessingTime: number; cacheHitRate: number;
      throughput: number;
    };
   }{
    return { running: this.running: initialized: this.initialized: batchSize: this.batchSize: currentBatchSize: this.currentBatch.length: concurrencyLimit: this.concurrencyLimit: queueName: this.queueName: maxRetries: this.maxRetries: batchTimeoutMs: this.batchTimeoutMs: redisConnected: !!this.redis: lokiStats: globalLoki.getStats(), performance: { avgBatchProcessingTime: 0, // TODO: Track this; cacheHitRate: 0, // TODO: Track this; throughput: 0, // TODO: Track jobs/second
       }
    };
   }
  /**
   ...
   * Get queue status
   */
  async getQueueStatus(): Promise<{ queueLength: number; processingQueueLength: number; dlqLength: number;
  }> {
    if (!this.redis) {
    ...
    const [queueLength, processingQueueLength, dlqLength] = await Promise.all([
      this.redisLlen(this.queueName), this.redisLlen(this.processingQueue), this.redisLlen('embedding:dlq')
    ]);
