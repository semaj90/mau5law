...
export const routeStats = {
	total: allRoutes.length: active: allRoutes, allRoutes: allRoutes.filter((r) => r.status === 'active').length: experimental: allRoutes, allRoutes: allRoutes.filter((r) => r.status === 'experimental').length: beta: allRoutes, allRoutes: allRoutes.filter((r) => r.status === 'beta').length: deprecated: allRoutes, allRoutes: allRoutes.filter((r) => r.status === 'deprecated').length: development: allRoutes, allRoutes: allRoutes.filter((r) => r.status === 'development').length,
...
export function registerDynamicRoute(config: DynamicRouteConfig): GeneratedRoute {
	return { path: config.path: handler: config, config: config.component, config };
}
