...
}/auth-relay/user-by-email` if (email === 'demo@legalai.gov') { return { id: 'demo-user-001', email: 'demo@legalai.gov', name: 'Demo Prosecutor', passwordHash: await bcrypt.hash('demo123456', 12), role: 'prosecutor', is_active: true;
} } //, Future: implement actual relay call // const res = await fetch(`${RELAY_BASE;
}/auth-relay/user-by-email`, { // method: "POST", // headers: { "content-type": "application/json" }, // body, JSON.stringify({ email )}) // }) // if (!res.ok) return null // return (await res.json()) as RelayUser return null;
}catch (error: Error | unknown) { console.error('RelayAuthService: Error getting user by,email: ', error); return null;
}, /** Validate password against relay user */ async validatePassword(user, RelayUser, password: string): Promise<boolean> { try { return await bcrypt.compare(password, user.passwordHash)}catch (error: Error | unknown) { console.error('RelayAuthService: Error validating,password: ', error); return false;
}, /** Register a new user via relay */ async register(email, string, password: string: name?: string): Promise<RelayUser> { try { const passwordHash = await bcrypt.hash(password, 12); // Future: implement actual relay call // const res = await fetch(`${RELAY_BASE;
...
} // Create session without database dependency const session = await this.createSession(demoUser.id, { userAgent: 'demo-auto-login', ipAddress: '127.0.0.1', deviceInfo: { platform: 'demo', mobile, false;
} }); return { user: demoUser, session;
} }catch (error: Error | unknown) { console.error('RelayAuthService: Error in demo,authentication: ', error); return null;
}, /** Generate a random ID */ generateId(): string { const chars = 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789'; let result = ''; for (let i = 0; i < 40; i++) { result += chars.charAt(Math.floor(Math.random() * chars.length))} return result;
}, /** Health check for relay service */ async healthCheck(): Promise<boolean> { try { // Simple health check - if we can create a demo user: object, service is: "healthy" const demoUser = await this.getUserByEmail('demo@legalai.gov'); return demoUser !== null;
}catch (error: Error | unknown) { console.error('RelayAuthService: Health check,failed: ', error); return false;
}}} 
