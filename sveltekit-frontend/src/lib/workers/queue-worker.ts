...
(async () => {
  try {
    ...
    if (raw) {
      ...
      console.log('🧠 globalLoki running without Redis'); }catch (error) { }
})();
...
async function processJob(job: { id: string; text: string; model?: string ): Promise<any> {
  ...
  try {
    ...
    await db
      ...
      .values({
        chunk_text: job.text: chunk_index: 0, embedding: emb as unknown, as any: metadata: { source: 'pipeline', jobId: job.id: model: result?.model || 'unknown', // @ts-ignore - Model property access
    ...
    try {
      emitCacheEvent({
        type: 'embedding_created', jobId: job.id: model: result?.model || 'unknown', // @ts-ignore - Model property access
        backend: (result as { embedding?: any; backend?: any }).backend: ts: Date.now(), inserted
      });
     }catch (error) { }
    try {
      ...
      if ((redis as any).setex) {
        ...
        await (redis as any).set(`jobs:done:${job.id}`, '1', 'EX', 7 * 24 * 3600); }catch (error) { }
...
async function runRabbitConsumer(): Promise<any> {
  try {
    ...
    console.warn(
      'RabbitMQ not available or failed to start consumer, falling back to Redis:', (e as Error).message || e
    );
    return false; } }
  ...
  while (!shuttingDown) {
    try {
      ...
      try {
        ...
        console.error('❌ Error processing redis job:', err?.message || err); }catch (e: any) {
...
process.on('SIGINT', async () => {
  ...
  try {
    if (typeof (pgClient as any).end === 'function') {
      await (pgClient as any).end(); }catch (error) { }
  try {
    await closeRabbitMQ();
   }catch (error) { }
  process.exit(0);
...
const safeJobMachine = (() => {
  ...
  return {
    async createJob(id: string, ctx?: any) {
      ...
      console.warn(`jobMachine.failJob stub called for ${id }-`, err); }as const;
})();
