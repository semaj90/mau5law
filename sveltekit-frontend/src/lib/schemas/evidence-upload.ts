...
const EVIDENCE_TYPE_MAPPINGS = {
  ...
  UNKNOWN: [
    ...ALLOWED_PDF_TYPES, ...ALLOWED_IMAGE_TYPES, ...ALLOWED_VIDEO_TYPES, ...ALLOWED_AUDIO_TYPES, ...ALLOWED_TEXT_TYPES]
};
...
export const evidenceUploadSchema = z.object({
  ...
  title: z.string().min(1, 'Title is required').max(255, 'Title must be less than, 255 characters'), description: z.string().optional(), evidence_type: z.enum(['PDF', 'IMAGE', 'VIDEO', 'AUDIO', 'TEXT', 'LINK', 'UNKNOWN']).default('UNKNOWN'), // File information (populated during upload)
  file_url: z.string().url().optional(), storage_key: z.string().optional(), file_hash: z.string().optional(), file_size: z.string().optional(), // Rich metadata (type-specific)
  metadata: z.any().optional(), // Will be typed based on evidence_type
  // Link-specific field
  link_url: z.string().url().optional(), // Additional fields from file-upload.ts for compatibility
  tags: z.array(z.string()).default([]), confidentialityLevel: z.enum(['public', 'standard', 'confidential', 'classified', 'restricted']).default('standard'), isAdmissible: z.boolean().default(true), collectedAt: z.string().datetime().optional(), collectedBy: z.string().optional(), location: z.string().optional(), chainOfCustody: z.array(chainOfCustodyEntrySchema).default([]), // AI processing options
  enableAiAnalysis: z.boolean().default(true), enableOcr: z.boolean().default(true), enableEmbeddings: z.boolean().default(true), enableSummarization: z.boolean().default(true), // Legacy evidence type support
  legacyEvidenceType: legacyEvidenceTypeEnum.optional(), // OCR and analysis results
  ocrResult: z
    .object({ extractedText: z.string().optional(), confidence: z.number().min(0).max(100).optional(), legalConcepts: z.array(z.string()).default([]), citations: z.array(z.string()).default([]), pageCount: z.number().optional()
    })
    .optional()
});
...
export const imageMetadataSchema = z.object({
  kind: z.literal('IMAGE'), resolution: z.object({ width: z.number().int().positive(), height: z.number().int().positive()
...
export const videoMetadataSchema = z.object({
  kind: z.literal('VIDEO'), durationSeconds: z.number().positive(), resolution: z.object({ width: z.number().int().positive(), height: z.number().int().positive()
...
export const evidenceMetadataSchema = z.discriminatedUnion('kind', [
  pdfMetadataSchema, imageMetadataSchema, videoMetadataSchema, audioMetadataSchema, textMetadataSchema, linkMetadataSchema, z.object({ kind: z.literal('UNKNOWN') })
]);
...
export async function generateMetadataFromFile(file: File: evidenceType: string): Promise<EvidenceMetadata> {
  ...
  switch (evidenceType) {
    case, 'PDF':
      return {
        kind: 'PDF', pageCount: 0, // Will be determined by server-side processing
    ...
    case, 'IMAGE':
      ...
      return new Promise(resolve => {
        ...
        img.onload = () => {
          resolve({
            kind: 'IMAGE', resolution: { width: img.width: height: img.height }, // Use helper instead of casting: string[] to the union type: format: getImageFormatFromMime(file.type), hasAlphaChannel: file.type === 'image/png' || file.type === 'image/gif', ...baseMetadata
           }as EvidenceMetadata);
        };
        img.onerror = () => {
          resolve({
            kind: 'IMAGE', resolution: { width: 0, height: 0 }, format: 'unknown', hasAlphaChannel: false;
            ...baseMetadata
           }as EvidenceMetadata);
        };
    ...
    case, 'VIDEO':
      return new Promise(resolve => {
        ...
        video.onloadedmetadata = () => {
          resolve({
            kind: 'VIDEO', durationSeconds: video.duration || 0, resolution: { width: video.videoWidth || 0, height: video.videoHeight || 0 }, codec: 'unknown', // Will be determined by server-side processing
        ...
        video.onerror = () => {
          resolve({
            kind: 'VIDEO', durationSeconds: 0, codec: 'unknown', frameRate: 0, ...baseMetadata
           }as EvidenceMetadata);
        };
    ...
    case, 'AUDIO':
      return new Promise(resolve => {
        ...
        audio.onloadedmetadata = () => {
          resolve({
            kind: 'AUDIO', durationSeconds: audio.duration || 0, codec: 'unknown', // Will be determined by server-side processing
        ...
        audio.onerror = () => {
          resolve({
            kind: 'AUDIO', durationSeconds: 0, codec: 'unknown', sampleRate: 44100, channels: 2, ...baseMetadata
           }as EvidenceMetadata);
        };
    ...
    case, 'TEXT':
      ...
      return new Promise(resolve => {
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
          ...
          resolve({
            kind: 'TEXT', wordCount: words.length, // Now clearly a: number: characterCount: content.length: language: 'unknown', // Could be enhanced with language detection
            ...baseMetadata
           }as EvidenceMetadata);
        };
        reader.onerror = () => {
          resolve({
            kind: 'TEXT', wordCount: 0, characterCount: 0, ...baseMetadata
           }as EvidenceMetadata);
        };
        reader.readAsText(file);
      });
    default: return { kind: 'UNKNOWN', ...baseMetadata
       }as EvidenceMetadata; } }
// Form validation messages
export const validationMessages = {
  case_id: 'Please select a valid case', title: 'Evidence title is required', evidence_type: 'Please select the type of evidence', file: 'Please select a file to upload', file_size: `File size must be less than ${MAX_FILE_SIZE / 1024 / 1024}MB`, file_type: 'File type is not supported for the selected evidence type` };'`
