...
<div class="mx-auto max-w-5xl p-6 space-y-6">
  ...
  <form
    class="grid gap-4 md:grid-cols-7 items-end bg-neutral-50 dark:bg-neutral-900/40 p-4 rounded-lg border border-neutral-200 dark:border-neutral-700"
    onsubmit={e => {
      e.preventDefault();
      submit();
    }}
  >
    <div class="md:col-span-3 flex flex-col gap-1">
      ...
      <input
        ...
        class="px-3 py-2 rounded border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-800 focus:outline-none focus:ring-2 focus:ring-indigo-500"
  ...
  {#if errorMsg}
    <div
      class="p-3 rounded border border-red-300 bg-red-50 dark:border-red-700 dark:bg-red-900/30 text-sm text-red-700 dark:text-red-300"
    >
      {errorMsg}
    </div>
  {/if}
    ...
    <section class="space-y-3">
      ...
      <ul class="space-y-3">
        {#each results as r (r.id)}
          <li
            class="p-4 rounded border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 shadow-sm"
          >
            <div class="flex justify-between items-start gap-4">
              <div class="text-sm font-mono truncate max-w-[60%]" title={r.id}>{r.id}</div>
              <div class="text-xs px-2 py-0.5 rounded font-semibold {scoreClass(r.score)}">
                {(r.score ?? 0).toFixed(3)}
              </div>
            </div>
  ...
  <footer class="pt-8 text-[11px] text-neutral-500 dark:text-neutral-500 space-y-2">
    ...
    <p>
      Streaming API: <code class="bg-neutral-100 dark:bg-neutral-800 px-1 rounded"
